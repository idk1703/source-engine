...
UtlSymId_t CUndoManager::GetUndoDescInternal(const char *context)
{
	if(m_nNesting <= 0)
	{
		...
		{
			Warning("CUndoManager::GetUndoDescInternal:  undoable operation missing CUndoScopeGuard in "
					"application\nContext( %s )\n",
					context);
...
UtlSymId_t CUndoManager::GetRedoDescInternal(const char *context)
{
	if(m_nNesting <= 0)
	{
		// Warning( "CUndoManager::GetRedoDescInternal:  undoable operation missing CUndoScopeGuard in
		// application\nContext( %s )", context );
		return s_UndoSymbolTable.AddString(context);
	}
	return m_RedoDesc;
