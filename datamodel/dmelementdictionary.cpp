...
// Constructor
//-----------------------------------------------------------------------------
CDmElementDictionary::CDmElementDictionary() : m_idmap(1024, 0, 0, DmIdPair_t::Compare, DmIdPair_t::HashKey) {}
...
DmElementHandle_t CDmElementDictionary::SetElementId(DmElementDictHandle_t hDictHandle, const DmObjectId_t &newId,
													 DmConflictResolution_t idConflictResolution)
{
	...
	if(newHandle != DMELEMENT_HANDLE_INVALID)
	{
		...
		return newHandle; // either keeping the old handle, with the new id, or found a new handle associated with that
						  // new id
	}
	...
	if(idConflictResolution == CR_DELETE_OLD)
	{
		...
		if(hExistingElement == DMELEMENT_HANDLE_INVALID)
			return DMELEMENT_HANDLE_INVALID; // unexpected error in ChangeElementId (failed due to something other than
											 // a conflict)
...
void CDmElementDictionary::HookUpElementAttributes()
{
	...
	for(int i = 0; i < n; ++i)
	{
		...
		// search id->handle table (both loaded and unloaded) for id, and if not found, create a new handle, map it to
