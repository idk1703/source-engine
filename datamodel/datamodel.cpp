...
public:
	...
	virtual CDmElement *Create(DmElementHandle_t handle, const char *pElementType, const char *pElementName,
							   DmFileId_t fileid, const DmObjectId_t &id)
	{
...
int CDataModel::EstimateMemoryOverhead() const
{
	...
	int nElementIdsOverhead =
		sizeof(DmElementHandle_t); // this also has a 80k static overhead, since hash tables can't grow
...
void CDataModel::DisplayMemoryStats()
{
	...
	for(int i = 0; i < c; ++i)
	{
		...
		int nMemory =
			CDmeElementAccessor::EstimateMemoryUsage(pElement, visited, TD_NONE, typeHistogram[j].m_pCategories);
	...
	ConMsg("Dm Memory usage: type\t\t\t\tcount\ttotalsize\twastage "
		   "%%\touter\t\tinner\t\tdatamodel\trefs\t\ttree\t\tatts\t\tdata\t(att count)\n");
	for(int i = 0; i < nCount; ++i)
	{
		const DmMemoryInfo_t &info = typeHistogram.Element(pSortInfo[i].m_nIndex);
		float flPercentOverhead =
			1.0f - ((info.m_nSize != 0)
						? ((float)info.m_pCategories[MEMORY_CATEGORY_ATTRIBUTE_DATA] / (float)info.m_nSize)
						: 0.0f);
		flPercentOverhead *= 100.0f;

		ConMsg("%-40s\t%6d\t%9d\t\t%5.2f", GetString(typeHistogram.Key(pSortInfo[i].m_nIndex)), info.m_nCount,
...
unsigned short CDataModel::GetSymbolCount()
	const // this can't ever overflow a ushort, since UtlSymId_t is a ushort, and one of its entries is invalid (0xffff)
{
...
bool CDataModel::SaveToFile(char const *pFileName, char const *pPathID, const char *pEncodingName,
							const char *pFormatName, CDmElement *pRoot)
{
...
DmFileId_t CDataModel::RestoreFromFile(char const *pFileName, char const *pPathID, const char *pFormatHint,
									   CDmElement **ppRoot,
									   ...
									   DmxHeader_t *pHeaderOut /*= NULL*/)
{
	...
	CUtlStreamBuffer buf(pFullPath, pPathID,
						 bIsBinary ? CUtlBuffer::READ_ONLY : CUtlBuffer::READ_ONLY | CUtlBuffer::TEXT_BUFFER);
	if(!buf.IsValid())
	{
		Warning("CDataModel: Unable to open file '%s'\n", pFullPath);
		return DMFILEID_INVALID;
	...
	if(!Unserialize(buf, pHeader->encodingName, pHeader->formatName, pFormatHint, pFullPath, idConflictResolution,
					hRootElement))
		return DMFILEID_INVALID;
...
//-----------------------------------------------------------------------------
bool CDataModel::Serialize(CUtlBuffer &outBuf, const char *pEncodingName, const char *pFormatName,
						   DmElementHandle_t hRoot)
{
	...
	if(pSerializer->IsBinaryFormat())
	{
		if(outBuf.IsText())
		{
			...
			{
				Warning("Serialize: Format %s expects to be written to a binary format, but the buffer is a "
						"text-format buffer\n",
						pFormatName);
				return false;
			}
	...
	if(pSerializer->StoresVersionInFile())
	{
		...
		pActualOutBuf->Printf("%s encoding %s %d format %s %d %s\n", DMX_VERSION_STARTING_TOKEN, pEncodingName,
							  pSerializer->GetCurrentVersion(), pFormatName, GetCurrentFormatVersion(pFormatName),
...
bool CDataModel::ReadDMXHeader(CUtlBuffer &inBuf, DmxHeader_t *pHeader) const
{
	...
	if(bOk)
	{
		...
		int nAssigned = sscanf_s(headerStr, "encoding %s %d format %s %d\n", pHeader->encodingName,
								 DMX_MAX_FORMAT_NAME_MAX_LENGTH, &(pHeader->nEncodingVersion), pHeader->formatName,
								 DMX_MAX_FORMAT_NAME_MAX_LENGTH, &(pHeader->nFormatVersion));
#else
		int nAssigned = sscanf(headerStr, "encoding %s %d format %s %d\n", pHeader->encodingName,
							   &(pHeader->nEncodingVersion), pHeader->formatName, &(pHeader->nFormatVersion));
#endif
		bOk = nAssigned == 4;
	}
...
//-----------------------------------------------------------------------------
bool CDataModel::Unserialize(CUtlBuffer &inBuf, const char *pEncodingName, const char *pSourceFormatName,
							 const char *pFormatHint, const char *pFileName,
							 DmConflictResolution_t idConflictResolution, DmElementHandle_t &hRoot)
{
	ClearUndo();
	...
	bool bIsCurrentVersion =
		true; // for formats that don't store a format, files are currently always at the current version
	...
	// if we're not in dmxconvert, and we're not at the latest version, call dmxconvert and unserialize from the
	// converted file
	if(!m_bOnlyCreateUntypedElements && !bIsCurrentVersion)
	{
		...
		char cmdline[256];
		V_snprintf(cmdline, sizeof(cmdline), "dmxconvert -allowdebug -i %s -o %s -oe %s -of %s", pFileName,
		...
		bool bSuccess;
		{
			...
			// yes, this passes in pFileName, even though it read from tempFileName - pFileName is only used for marking
			// debug messages and setting fileid
			bSuccess = Unserialize(strbuf, pDestEncodingName, pDestFormatName, pDestFormatName, pFileName,
	...
	CDmElement *pRoot;
	bool bOk = pSerializer->Unserialize(inBuf, pEncodingName, header.nEncodingVersion, pSourceFormatName,
...
bool CDataModel::UpdateUnserializedElements(const char *pSourceFormatName, int nSourceFormatVersion, DmFileId_t fileid,
...
// search id->handle table (both loaded and unloaded) for id, and if not found, create a new handle, map it to the id
// and return it
...
DmElementHandle_t CDataModel::ChangeElementId(DmElementHandle_t hElement, const DmObjectId_t &oldId,
...
DmElementReference_t *CDataModel::FindElementReference(DmElementHandle_t hElement, DmObjectId_t **ppId /* = NULL */)
{
	...
	for(UtlHashHandle_t h = m_unloadedIdElementMap.GetFirstHandle(); h != m_unloadedIdElementMap.InvalidHandle();
...
void CDataModel::RemoveUnreferencedElements()
{
	...
	for(int i = 0; i < nElementIds; ++i)
	{
		...
		if(m_unloadedIdElementMap[h].m_ref.IsWeaklyReferenced())
			continue; // don't remove if it's been referenced again - this allows an unref followed by a ref in the same
					  // edit phase
	...
	// this is intentionally calling Count() every time through, since DestroyElement may cause more elements to be
	// added to the list
...
void CDataModel::FindAndDeleteOrphanedElements()
{
	...
	for(DmElementHandle_t hElement = FirstAllocatedElement(); hElement != DMELEMENT_HANDLE_INVALID;
	...
	// JDTODO - this incorrectly deletes elements that are referenced by a counted handle, but aren't under the file
	// root
...
//-----------------------------------------------------------------------------
DmElementHandle_t CDataModel::CreateElement(UtlSymId_t typeSymbol, const char *pElementName, DmFileId_t fileid,
											...
											const DmObjectId_t *pObjectID)
{
	...
	{
		CDmElement *pElement =
			CreateElement(m_unloadedIdElementMap[h].m_ref, pElementType, pElementName, fileid, pObjectID);
		if(pElement)
		{
			m_unloadedIdElementMap.Remove(h);
	...
	else
	{
		DmElementHandle_t hElement = AcquireElementHandle();
		CDmElement *pElement =
			CreateElement(DmElementReference_t(hElement), pElementType, pElementName, fileid, pObjectID);
		if(pElement)
			return pElement->GetHandle();
...
public:
	CUndoCreateElement() : BaseClass("CUndoCreateElement"), m_bKill(false), m_hElement() {}
...
// this is kosher because the ref either is created on the fly and has no attributes, or is being removed from
// m_unloadedIdElementMap
//-----------------------------------------------------------------------------
CDmElement *CDataModel::CreateElement(const DmElementReference_t &ref, const char *pElementType,
									  const char *pElementName, DmFileId_t fileid, const DmObjectId_t *pObjectID)
{
	//	Msg( "Creating %s 0x%x '%s' in file \"%s\" - %d elements loaded\n", pElementType, ref.m_hElement, pElementName ?
	//pElementName : "", GetFileName( fileid ), m_elementIds.Count() );
	...
	if(g_pDataModel->IsUndoEnabled() &&
	   fileid != DMFILEID_INVALID) // elements not in any file don't participate in undo
	{
...
{
	DmPhase_t phase = g_pDmElementFramework->GetPhase();
	if(phase != PH_EDIT && phase != PH_EDIT_APPLY) // need to allow edit_apply to delete elements, so that cascading
												   // deletes can occur in one phase
	{
		Assert(0);
		return;
	}
...
void CDataModel::DeleteElement(DmElementHandle_t hElement, DmHandleReleasePolicy hrp /* = HR_ALWAYS */)
{
	...
	//	Msg( "Deleting %s element 0x%x \'%s\' in file \"%s\" - %d elements loaded\n", pElementType, hElement,
	//pInternal->GetName(), GetFileName( pInternal->GetFileId() ), m_elementIds.Count() );
