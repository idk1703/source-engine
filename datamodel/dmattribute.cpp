...
public:
	...
	virtual void SetMultiple(CDmAttribute * pAttribute, int i, int nCount, DmAttributeType_t valueType,
							 const void *pValue) = 0;
...
template<class T>
void CDmAttributeOp<T>::SetMultiple(CDmAttribute *pAttribute, int i, int nCount, DmAttributeType_t valueType,
									const void *pValue)
{
	Assert(0);
}
...
public:
	...
	CDmArrayAttributeOp(CDmAttribute *pAttribute)
		: m_pAttribute(pAttribute), m_pData((D *)m_pAttribute->GetAttributeData())
	{
	}
...
public:
	...
	virtual const char *GetDesc()
	{
		...
		Q_snprintf(buf, sizeof(buf), "%s(%s) = %s", base, g_pDataModel->GetString(m_symAttribute),
...
public:
	CUndoArrayAttributeSetValueElement(CDmAttribute *pAttribute, int slot, const T &newValue)
		: BaseClass(pAttribute, "CUndoArrayAttributeSetValueElement"), m_nSlot(slot)
	{
		Assert(pAttribute->GetOwner() && pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
...
public:
	CUndoArrayAttributeSetMultipleValueElement(CDmAttribute *pAttribute, int nSlot, int nCount, const T *pNewValue)
		: BaseClass(pAttribute, "CUndoArrayAttributeSetMultipleValueElement"), m_nSlot(nSlot), m_nCount(nCount)
	{
		Assert(pAttribute->GetOwner() && pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
	...
	~CUndoArrayAttributeSetMultipleValueElement()
	{
		...
		// (ie otherwise I'd just create a CUndoArrayAttributeSetMultipleValueElement< DmElementHandle_t,BaseClass>
		// version with this code) anyways, the casting hackiness only happens when the value is actually a
		// DmElementHandle_t, so it's completely safe
...
public:
	CUndoAttributeArrayInsertBefore(CDmAttribute *pAttribute, int slot, int count = 1)
		: BaseClass(pAttribute, "CUndoAttributeArrayInsertBefore"), m_nIndex(slot), m_nCount(count)
	{
		Assert(pAttribute->GetOwner() && pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
	}
...
public:
	CUndoAttributeArrayInsertCopyBefore(CDmAttribute *pAttribute, int slot, const T &newValue)
		: BaseClass(pAttribute, "CUndoAttributeArrayInsertCopyBefore"), m_nIndex(slot), m_newValue(newValue)
	{
		Assert(pAttribute->GetOwner() && pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
	}
...
public:
	...
	virtual const char *GetDesc()
	{
		...
		const char *base = BaseClass::GetDesc();
		Q_snprintf(buf, sizeof(buf), "%s (%s) = remove( pos %i, count %i )", base, this->GetAttributeName(), m_nIndex,
				   m_nCount);
		return buf;
	}
...
public:
	CUndoAttributeArrayCopyAllElement(CDmAttribute *pAttribute, const T *pNewValues, int nNewSize,
									  bool purgeOnRemove = false)
		: BaseClass(pAttribute, "CUndoAttributeArrayCopyAllElement"), m_bPurge(purgeOnRemove)
	{
		Assert(pAttribute->GetOwner() && pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
...
template<>
inline void CDmArrayAttributeOp<DmElementHandle_t>::OnAttributeArrayElementAdded(int nFirstElem, int nLastElem,
																				 bool bUpdateElementReferences)
{
	CDmElement *pOwner = m_pAttribute->GetOwner();
	if(m_pAttribute->IsFlagSet(FATTRIB_HAS_ARRAY_CALLBACK) && !CDmeElementAccessor::IsBeingUnserialized(pOwner))
	{
...
template<>
void CDmArrayAttributeOp<DmElementHandle_t>::OnAttributeArrayElementRemoved(int nFirstElem, int nLastElem)
{
	CDmElement *pOwner = m_pAttribute->GetOwner();
	if(m_pAttribute->IsFlagSet(FATTRIB_HAS_ARRAY_CALLBACK) && !CDmeElementAccessor::IsBeingUnserialized(pOwner))
	{
...
template<>
inline bool CDmArrayAttributeOp<DmElementHandle_t>::ShouldInsertElement(const DmElementHandle_t &src)
{
...
template<>
void CDmArrayAttributeOp<DmElementHandle_t>::PerformCopyArray(const DmElementHandle_t *pArray, int nCount)
{
	Data().RemoveAll();
	for(int i = 0; i < nCount; ++i)
	{
...
void CDmArrayAttributeOp<T>::SwapArray(CUtlVector<T> &src)
{
	...
	{
		CUndoAttributeArrayCopyAllElement<T> *pUndo =
			new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, src.Base(), src.Count());
		g_pDataModel->AddUndoElement(pUndo);
	}
...
template<>
void CDmArrayAttributeOp<DmElementHandle_t>::SwapArray(CUtlVector<DmElementHandle_t> &src)
{
...
void CDmArrayAttributeOp<T>::SetMultiple(int i, int nCount, const T *pValue)
{
	...
	if(g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner()))
	{
		CUndoArrayAttributeSetMultipleValueElement<T> *pUndo =
			new CUndoArrayAttributeSetMultipleValueElement<T>(m_pAttribute, i, nCount, pValue);
		g_pDataModel->AddUndoElement(pUndo);
	}
...
template<class T>
void CDmArrayAttributeOp<T>::SetMultiple(CDmAttribute *pAttribute, int i, int nCount, DmAttributeType_t valueType,
										 const void *pValue)
{
	if(valueType == ArrayTypeToValueType(pAttribute->GetType()))
	{
...
template<>
bool CDmArrayAttributeOp<DmElementHandle_t>::Unserialize(CDmAttribute *pAttribute, CUtlBuffer &buf)
{
...
CDmAttribute *CDmAttribute::CreateExternalAttribute(CDmElement *pOwner, DmAttributeType_t type,
													const char *pAttributeName, void *pExternalMemory)
{
	switch(type)
	{
		case AT_UNKNOWN:
...
//-----------------------------------------------------------------------------
CDmAttribute::CDmAttribute(CDmElement *pOwner, DmAttributeType_t type, const char *pAttributeName) : m_pData(NULL)
{
...
void CDmAttribute::OnChanged(bool bArrayCountChanged, bool bIsTopological)
{
	...
	{
		g_pDataModelImp->NotifyState(bArrayCountChanged ? NOTIFY_CHANGE_ATTRIBUTE_ARRAY_SIZE
...
int CDmAttribute::EstimateMemoryUsageInternal(CUtlHash<DmElementHandle_t> &visited, TraversalDepth_t depth,
...
void CDmaDecorator<T, B>::Init(CDmElement *pOwner, const char *pAttributeName, int nFlags)
{
	Assert(pOwner);
	this->m_pAttribute =
		pOwner->AddExternalAttribute(pAttributeName, CDmAttributeInfo<CUtlVector<T>>::AttributeType(), &this->Value());
...
void CDmrGenericArrayConst::Init(const CDmElement *pElement, const char *pAttributeName)
{
	const CDmAttribute *pAttribute =
		(pElement && pAttributeName && pAttributeName[0]) ? pElement->GetAttribute(pAttributeName) : NULL;
	Init(pAttribute);
}
