...
public:
	...
	virtual const char *GetDesc()
	{
		...
		const char *base = BaseClass::GetDesc();
		Q_snprintf(buf, sizeof(buf), "%s (%s) = remove( pos %i, count %i )", base, GetAttributeName(), m_nIndex,
				   m_nCount);
		return buf;
	}
...
void CDmaDecorator<T, B>::Init(CDmElement *pOwner, const char *pAttributeName, int nFlags = 0)
{
	Assert(pOwner);
	this->m_pAttribute =
		pOwner->AddExternalAttribute(pAttributeName, CDmAttributeInfo<CUtlVector<T>>::AttributeType(), &Value());
	Assert(m_pAttribute);
	if(nFlags)
	{
