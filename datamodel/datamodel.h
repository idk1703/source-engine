...
protected:
	...
	static bool CompareFunc(DmElementHandle_t const &a, DmElementHandle_t const &b)
	{
		return a == b;
	}
...
struct FileElementSet_t
{
	FileElementSet_t(UtlSymId_t filename = UTL_INVAL_SYMBOL, UtlSymId_t format = UTL_INVAL_SYMBOL)
		: m_filename(filename), m_format(format), m_hRoot(DMELEMENT_HANDLE_INVALID), m_bLoaded(true), m_nElements(0)
	{
	}
...
public:
	...
	virtual DmElementHandle_t CreateElement(UtlSymId_t typeSymbol, const char *pElementName, DmFileId_t fileid,
											const DmObjectId_t *pObjectID = NULL);
	virtual DmElementHandle_t CreateElement(const char *pTypeName, const char *pElementName, DmFileId_t fileid,
											const DmObjectId_t *pObjectID = NULL);
	...
	virtual bool Unserialize(CUtlBuffer &buf, const char *pEncodingName, const char *pSourceFormatName,
	...
	virtual bool SaveToFile(char const *pFileName, char const *pPathID, const char *pEncodingName,
							const char *pFormatName, CDmElement *pRoot);
	virtual DmFileId_t RestoreFromFile(char const *pFileName, char const *pPathID, const char *pFormatHint,
...
public:
	...
	// CreateElement references the attribute list passed in via ref, so don't edit or purge ref's attribute list
	// afterwards
	CDmElement *CreateElement(const DmElementReference_t &ref, const char *pElementType, const char *pElementName,
	...
	// search id->handle table (both loaded and unloaded) for id, and if not found, create a new handle, map it to the
	// id and return it
...
{
public:
	static void Purge(CDmElement *pElement)
	{
		pElement->Purge();
	}
	...
	static void MarkBeingUnserialized(CDmElement *pElement, bool beingUnserialized = true)
	{
	...
	static void RemoveAttributeByPtrNoDelete(CDmElement *pElement, CDmAttribute *ptr)
	{
	...
	static void SetReference(CDmElement *pElement, const DmElementReference_t &ref)
	{
		pElement->SetReference(ref);
	}
	static int EstimateMemoryUsage(CDmElement *pElement, CUtlHash<DmElementHandle_t> &visited, TraversalDepth_t depth,
								   int *pCategories)
	{
