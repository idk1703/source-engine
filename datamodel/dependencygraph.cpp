...
{
	COperatorNode(IDmeOperator *pOp = NULL) : m_state(TS_NOT_VISITED), m_operator(pOp), m_bInList(false) {}
...
{
public:
	CAttributeNode(CDmAttribute *attribute = NULL) : m_attribute(attribute), m_bIsOutputToOperator(false) {}
...
//-----------------------------------------------------------------------------
CDependencyGraph::CDependencyGraph() : m_attrNodes(4096, 0, 0, HashEntryCompareFunc, HashEntryKeyFunc) {}
		...
		for(int ai = 0; ai < an; ++ai)
		{
			...
			if(pOperator)
			{
				...
				if(operatorDict.Find(pOperator) == operatorDict.InvalidIndex())
				{
					...
					Warning("Found dependent operator '%s' referenced by operator '%s' that wasn't in the scene or "
							"trackgroups!\n",
...
void CDependencyGraph::FindRoots()
{
	...
	for(; h != m_attrNodes.InvalidHandle(); h = m_attrNodes.GetNextHandle(h))
	{
		...
		if(!pAttrNode->m_bIsOutputToOperator && pAttrNode->m_attribute->IsFlagSet(FATTRIB_OPERATOR_DIRTY))
		{
