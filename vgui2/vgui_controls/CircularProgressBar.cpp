...
void CircularProgressBar::PaintBackground()
{
	...
	int iTextureID =
		m_nTextureId[PROGRESS_TEXTURE_BG] != -1 ? m_nTextureId[PROGRESS_TEXTURE_BG] : m_nTextureId[PROGRESS_TEXTURE_FG];
...
namespace vgui
{
	...
	circular_progress_segment_t Segments[8] = {
		{0.0, 0.5, 0.0, 1.0, 0.0, 1, 0},		 {M_PI * 0.25, 1.0, 0.0, 1.0, 0.5, 0, 1},
		{M_PI * 0.5, 1.0, 0.5, 1.0, 1.0, 0, 1},	 {M_PI * 0.75, 1.0, 1.0, 0.5, 1.0, -1, 0},
		{M_PI, 0.5, 1.0, 0.0, 1.0, -1, 0},		 {M_PI * 1.25, 0.0, 1.0, 0.0, 0.5, 0, -1},
		{M_PI * 1.5, 0.0, 0.5, 0.0, 0.0, 0, -1}, {M_PI * 1.75, 0.0, 0.0, 0.5, 0.0, 1, 0},
	};
		...
		for(i = 0; i < 8; i++)
		{
			...
			if(flEndProgressRadians < segmentRadiansMax)
			{
				...
				if(flInternalProgress < SEGMENT_ANGLE)
				{
					...
					v[1].m_Position.Init(Segments[i].vert1x * flWide + flDeltaX,
										 Segments[i].vert1y * flTall + flDeltaY);
					v[1].m_TexCoord.Init(Segments[i].vert1x + (flDeltaX / flHalfWide) * 0.5,
	...
	for(int i = 0; i < 8; i++)
	{
		if(flEndProgressRadians > Segments[cur_wedge].minProgressRadians)
		{
			...
			if(flInternalProgress < SEGMENT_ANGLE)
			{
				...
				v[2].m_TexCoord.Init(Segments[i].vert1x + (flDeltaX / flHalfWide) * 0.5,
									 Segments[i].vert1y + (flDeltaY / flHalfTall) * 0.5);
			}
			else
			{
