...
//-----------------------------------------------------------------------------
static int __cdecl DefaultSortFunc(ListPanel *pPanel, const ListPanelItem &item1, const ListPanelItem &item2)
{
...
//-----------------------------------------------------------------------------
static int __cdecl FastSortFunc(ListPanel *pPanel, const ListPanelItem &item1, const ListPanelItem &item2)
{
...
//-----------------------------------------------------------------------------
void ListPanel::AddColumnHeader(int index, const char *columnName, const char *columnText, int width, int minWidth,
								int maxWidth, int columnFlags)
{
	...
	Button *pButton = SETUP_PANEL(new ColumnButton(
		this, columnName,
		columnText)); // the cell rendering mucks with the button visibility during the solvetraverse loop,
...
void ListPanel::OpenColumnChoiceMenu()
{
	...
	for(int i = 0; i < m_CurrentColumns.Count(); i++)
	{
		...
		int itemID =
			menu->AddCheckableMenuItem(name, new KeyValues("ToggleColumnVisible", "col", m_CurrentColumns[i]), this);
...
void ListPanel::ApplyUserConfigSettings(KeyValues *userConfig)
{
	...
	//    - In PerformLayout, it thinks the window has grown by 500 (since m_lastBarWidth is 500 and new window width is
	//    1000)
	//      so it pushes out any COLUMN_RESIZEWITHWINDOW columns to their max extent and shrinks everything else to its
	//      min extent.
