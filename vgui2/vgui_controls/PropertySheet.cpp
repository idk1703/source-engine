...
namespace vgui
{
	...
	public:
		PageTab(PropertySheet *parent, const char *panelName, const char *text, char const *imageName, int maxTabWidth,
				Panel *page, bool showContextButton, long hoverActivatePageTime = -1)
					...
					if(!m_pParent->IsWithin(screenx, screeny))
					{
						...
						if(tw)
						{
							...
							if(factory)
							{
								...
								factory->InstanceToolWindow(tw->GetParent(), sheet->ShouldShowContextButtons(), page,
															title, hasContextMenu);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
PropertySheet::PropertySheet(Panel *parent, const char *panelName, bool draggableTabs /*= false*/)
...
//-----------------------------------------------------------------------------
void PropertySheet::AddPage(Panel *page, const char *title, char const *imageName /*= NULL*/,
...
void PropertySheet::ChangeActiveTab(int index)
{
	...
	if(m_flPageTransitionEffectTime)
	{
		if(m_hPreviouslyActivePage.Get())
		{
			...
			GetAnimationController()->RunAnimationCommand(m_hPreviouslyActivePage, "Alpha", 0.0f, 0.0f,
		...
		GetAnimationController()->RunAnimationCommand(_activePage, "Alpha", 255.0f, m_flPageTransitionEffectTime / 2,
													  m_flPageTransitionEffectTime / 2,
													  AnimationController::INTERPOLATOR_LINEAR);
	}
	else
