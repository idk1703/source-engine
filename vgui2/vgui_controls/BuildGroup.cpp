	...
	if(_dragging)
	{
		...
		else
		{
			for(int i = 0; i < _controlGroup.Count(); ++i)
			{
				...
				groupMember->SetPos(_dragStartPanelPos[0] + _groupDeltaX[i] + (x - _dragStartCursorPos[0]),
									_dragStartPanelPos[1] + _groupDeltaY[i] + (y - _dragStartCursorPos[1]));
									 ...
									 KeyValues *pPreloadedKeyValues, KeyValues *pConditions)
{
	...
	{
		Assert(m_dictCachedResFiles.Find(controlResourceName) != m_dictCachedResFiles.InvalidIndex() || pConditions ||
			   pPreloadedKeyValues);
		rDat->deleteThis();
	}
}
		...
		for(; pSubKey != NULL; pSubKey = pSubKey->GetNextKey())
		{
			...
			for(; pCondition != NULL; pCondition = pCondition->GetNextKey())
			{
				...
				if(pConditionBlock)
				{
					...
					for(pOverridingKey = pConditionBlock->GetFirstSubKey(); pOverridingKey != NULL;
						pOverridingKey = pOverridingKey->GetNextKey())
					{
...
bool BuildGroup::SaveControlSettings(void)
{
	...
	if(m_pResourceName)
	{
		...
		if(!bSuccess)
		{
			MessageBox *dlg = new MessageBox("BuildMode - Error saving file",
