	  ...
	  m_HideTooltip(this, &HTML::BrowserHideToolTip)
{
	...
	if(m_SteamAPIContext.SteamHTMLSurface())
	{
		...
		SteamAPICall_t hSteamAPICall =
			m_SteamAPIContext.SteamHTMLSurface()->CreateBrowser(surface()->GetWebkitHTMLUserAgentString(), NULL);
	...
	m_iCopyLinkMenuItemID =
		m_pContextMenu->AddMenuItem("#vgui_HTMLCopyLink", new KeyValues("Command", "command", "copylink"), this);
	...
	m_pContextMenu->AddSeparator();
	m_nViewSourceAllowedIndex =
		m_pContextMenu->AddMenuItem("#vgui_HTMLViewSource", new KeyValues("Command", "command", "viewsource"), this);
}
...
void HTML::PerformLayout()
{
	...
	m_pFindBar->SetBounds(GetWide() - iSearchWide - iSearchInsetX - vbarInset,
						  m_pFindBar->BIsHidden() ? -1 * iSearchTall - 5 : iSearchInsetY, iSearchWide, iSearchTall);
}
...
{
	if(m_SteamAPIContext.SteamHTMLSurface())
		m_SteamAPIContext.SteamHTMLSurface()->KeyChar(m_unBrowserHandle, unichar,
...
void HTML::ShowFindDialog()
{
	...
	GetAnimationController()->RunAnimationCommand(m_pFindBar, "ypos", iSearchInsetY, 0.0f, flAnimationTime,
...
void HTML::HideFindDialog()
{
	...
	GetAnimationController()->RunAnimationCommand(m_pFindBar, "ypos", -1 * h - 5, 0.0f, flAnimationTime,
												  AnimationController::INTERPOLATOR_LINEAR);
...
void HTML::OnKeyCodeTyped(KeyCode code)
{
	...
	if(m_SteamAPIContext.SteamHTMLSurface())
		m_SteamAPIContext.SteamHTMLSurface()->KeyDown(m_unBrowserHandle, KeyCode_VGUIToVirtualKey(code),
													  (ISteamHTMLSurface::EHTMLKeyModifiers)GetKeyModifiers());
}
...
{
	if(m_SteamAPIContext.SteamHTMLSurface())
		m_SteamAPIContext.SteamHTMLSurface()->KeyUp(m_unBrowserHandle, KeyCode_VGUIToVirtualKey(code),
													(ISteamHTMLSurface::EHTMLKeyModifiers)GetKeyModifiers());
}
...
void HTML::BrowserResize()
{
	...
	*/
	...
	if(m_iWideLastHTMLSize != (w - m_iScrollBorderX - right) || m_iTalLastHTMLSize != (h - m_iScrollBorderY - bottom))
	{
		...
		{
			if(m_SteamAPIContext.SteamHTMLSurface())
				m_SteamAPIContext.SteamHTMLSurface()->SetSize(m_unBrowserHandle, m_iWideLastHTMLSize,
															  m_iTalLastHTMLSize);
		}

		// webkit forgets the scroll offset when you resize (it saves the scroll in a DC and a resize throws away the
		// DC) so just tell it after the resize
...
void HTML::PostChildPaint()
{
	...
	// TODO::STYLE
	// m_pInteriorPanel->SetPaintAppearanceEnabled( true ); // turn painting back on so the IE hwnd can render this
	// border
}
...
void HTML::BrowserNeedsPaint(HTML_NeedsPaint_t *pCallback)
{
	...
	if(m_iHTMLTextureID != 0 && ((_vbar->IsVisible() && pCallback->unScrollY > 0 &&
								  abs((int)pCallback->unScrollY - m_scrollVertical.m_nScroll) > 5) ||
								 (_hbar->IsVisible() && pCallback->unScrollX > 0 &&
								  abs((int)pCallback->unScrollX - m_scrollHorizontal.m_nScroll) > 5)))
	{
	...
	if(m_bNeedsFullTextureUpload || m_iHTMLTextureID == 0 || tw != (int)pCallback->unWide ||
	   tt != (int)pCallback->unTall)
	{
		m_bNeedsFullTextureUpload = false;
		if(m_iHTMLTextureID != 0)
			surface()->DeleteTextureByID(m_iHTMLTextureID);

		// if the dimensions changed we also need to re-create the texture ID to support the overlay properly (it won't
		// resize a texture on the fly, this is the only control that needs
		//   to so lets have a tiny bit more code here to support that)
		m_iHTMLTextureID = surface()->CreateNewTextureID(true);
		surface()->DrawSetTextureRGBAEx(
			m_iHTMLTextureID, (const unsigned char *)pCallback->pBGRA, pCallback->unWide, pCallback->unTall,
			IMAGE_FORMAT_BGRA8888); // BR FIXME - this call seems to shift by some number of pixels?
	...
	else if((int)pCallback->unUpdateWide > 0 && (int)pCallback->unUpdateTall > 0)
	{
		...
		surface()->DrawUpdateRegionTextureRGBA(m_iHTMLTextureID, pCallback->unUpdateX, pCallback->unUpdateY,
											   (const unsigned char *)pCallback->pBGRA, pCallback->unUpdateWide,
											   pCallback->unUpdateTall, IMAGE_FORMAT_BGRA8888);
	}
	else
	{
		surface()->DrawSetTextureRGBAEx(m_iHTMLTextureID, (const unsigned char *)pCallback->pBGRA, pCallback->unWide,
										pCallback->unTall, IMAGE_FORMAT_BGRA8888);
	}
...
bool HTML::OnStartRequest(const char *url, const char *target, const char *pchPostData, bool bIsRedirect)
{
	...
	{
		if(target &&
		   (!Q_stricmp(target, "_blank") || !Q_stricmp(target, "_new"))) // only allow NEW windows (_blank ones)
		{
			return true;
		}
		else
