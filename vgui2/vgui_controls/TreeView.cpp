...
#define CLICK_TO_EDIT_DELAY_MSEC 500
		...
		virtual void OnMousePressed(MouseCode code)
		{
			...
			else
			{
				...
				if(!shift && !ctrl && !m_bArmForEditing && IsLabelEditingAllowed() && bIsOnlyOneItemSelected &&
				   IsTextFullySelected() && !IsBeingDragged())
				{
	...
	private:
		void FindNodesInRange_R(CUtlVector<TreeNode *> &list, bool &finished, bool &foundStart, int startIndex,
								int endIndex);
	...
	TreeNode::TreeNode(Panel *parent, TreeView *pTreeView)
		: BaseClass(parent, "TreeNode"), m_nClickedItem(0), m_bClickedSelected(false)
	{
		m_pData = NULL;
	...
	{
		if(m_pText->IsLabelEditingAllowed() && !m_pText->IsBeingEdited())
		{
	...
	*/
	...
	void TreeNode::PerformLayout()
	{
		...
		int width = 0;
		if(m_pData->GetInt("SelectedImage", 0) == 0 && m_pData->GetInt("Image", 0) == 0)
		{
	...
	void TreeNode::OnMousePressed(MouseCode code)
	{
		...
		if(code == MOUSE_LEFT)
		{
			...
			if(x < TREE_INDENT_AMOUNT)
			{
				...
				// m_pTreeView->SetSelectedItem(m_ItemIndex);    // explorer doesn't actually select item when it
				// expands an item purposely commented out in case we want to change the behavior
			}
			else
			{
	...
	void TreeNode::FindNodesInRange_R(CUtlVector<TreeNode *> &list, bool &finished, bool &foundStart, int startIndex,
									  int endIndex)
	{
		if(finished)
			return;
...
bool TreeView::ModifyItem(int itemIndex, KeyValues *data)
{
	...
	if(bReSort)
	{
		...
		bool bRightBad =
			(nChildIndex < nChildren - 1) && m_pSortFunc(pParent->m_Children[nChildIndex + 1]->m_pData, pNode->m_pData);
