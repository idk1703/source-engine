...
CKeyBoardEditorPage::CKeyBoardEditorPage(Panel *parent, Panel *panelToEdit, KeyBindingContextHandle_t handle)
	: BaseClass(parent, "KeyBoardEditorPage"), m_pPanel(panelToEdit), m_Handle(handle)
{
	Assert(m_pPanel);
	...
	for(i = 0; i < c; ++i)
	{
		...
		for(j = 0; j < bindings; ++j)
		{
			...
			item->SetWString("Binding",
							 Panel::KeyCodeModifiersToDisplayString((KeyCode)kbMap->keycode, kbMap->modifiers));
...
void CKeyBoardEditorSheet::OnSaveChanges()
{
	...
	{
		m_hPanel->SaveKeyBindingsToFile(m_Handle, m_SaveFileName.String(),
										m_SaveFilePathID.IsValid() ? m_SaveFilePathID.String() : NULL);
	}
	else
	{
...
void CKeyBoardEditorDialog::OnCommand(char const *cmd)
{
	...
	else if(!Q_stricmp(cmd, "cancel") || !Q_stricmp(cmd, "Close"))
	{
