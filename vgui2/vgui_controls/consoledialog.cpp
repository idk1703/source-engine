...
void CConsolePanel::RebuildCompletionList(const char *text)
{
	...
	// if there is a space in the text, and the command isn't of the type to know how to autocomplet, then command
	// completion is over
...
void CConsolePanel::OnTextChanged(Panel *panel)
{
	...
	bool ctrlKeyDown =
		(vgui::input()->IsKeyDown(KEY_LCONTROL) || vgui::input()->IsKeyDown(KEY_RCONTROL)) ? true : false;
...
void CConsolePanel::AddToHistory(const char *commandText, const char *extraText)
{
	...
	if(command)
	{
		...
		// src\vgui2\vgui_controls\consoledialog.cpp(974): warning C6053: The prior call to 'strncpy' might not
		// zero-terminate string 'command'
...
//
//-----------------------------------------------------------------------------
CConsoleDialog::CConsoleDialog(vgui::Panel *pParent, const char *pName, bool bStatusVersion) : BaseClass(pParent, pName)
{
	// initialize dialog
	SetVisible(false);
