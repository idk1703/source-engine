...
static int ListBaseStringSortFunc(ListPanel *pPanel, const ListPanelItem &item1, const ListPanelItem &item2,
...
static int ListBaseIntegerSortFunc(ListPanel *pPanel, const ListPanelItem &item1, const ListPanelItem &item2,
								   char const *fieldName)
{
	bool dir1 = item1.kv->GetInt("directory") == 1;
	bool dir2 = item2.kv->GetInt("directory") == 1;
...
static int ListBaseInteger64SortFunc(ListPanel *pPanel, const ListPanelItem &item1, const ListPanelItem &item2,
									 char const *lowfield, char const *highfield)
{
	bool dir1 = item1.kv->GetInt("directory") == 1;
	bool dir2 = item2.kv->GetInt("directory") == 1;
...
static ColumnInfo_t g_ColInfo[] = {
	{"text", "#FileOpenDialog_Col_Name", 175, 20, 10000, ListPanel::COLUMN_UNHIDABLE, &ListFileNameSortFunc,
	 Label::a_west},
	...
	//	{	"created",			"#FileOpenDialog_Col_DateCreated",		125,	20, 10000, ListPanel::COLUMN_HIDDEN,
	//&ListFileCreatedSortFunc		, Label::a_west },
	{"attributes", "#FileOpenDialog_Col_Attributes", 50, 20, 10000, ListPanel::COLUMN_HIDDEN,
	 &ListFileAttributesSortFunc, Label::a_west},
};
...
FileOpenDialog::FileOpenDialog(Panel *parent, const char *title, FileOpenDialogType_t type,
...
void FileOpenDialog::Init(const char *title, KeyValues *pContextKeyValues)
{
	...
	for(int i = 0; i < ARRAYSIZE(g_ColInfo); ++i)
	{
		...
		m_pFileList->AddColumnHeader(i, info.columnName, info.columnText, info.startingWidth, info.minWidth,
	...
	Label *fileName = new Label(this, "FileNameLabel",
								(m_DialogType != FOD_SELECT_DIRECTORY) ? "#FileOpenDialog_File_name"
	...
	m_pOpenButton->SetCommand((m_DialogType != FOD_SELECT_DIRECTORY) ? new KeyValues("OnOpen")
	...
	// Because these call through virtual functions, we can't issue them in the constructor, so we post a message to
	// ourselves instead!!
...
void FileOpenDialog::OnNewFolder()
{
	...
	m_hInputDialog = new InputDialog(this, "#FileOpenDialog_NewFolder_InputTitle", "#FileOpenDialog_NewFolderPrompt",
...
void FileOpenDialog::NewFolder(char const *folderName)
{
	...
	do
	{
		...
		if(!g_pFullFileSystem->FileExists(pFullPath, NULL) && !g_pFullFileSystem->IsDirectory(pFullPath, NULL))
		{
...
void FileOpenDialog::ValidatePath()
{
	...
	struct _stat buf;
	if((0 == _stat(fullpath, &buf)) && (0 != (buf.st_mode & S_IFDIR)))
	{
		Q_AppendSlash(fullpath, sizeof(fullpath));
...
void FileOpenDialog::OnOpen()
{
	...
	bool bSpecifiedDirectory = (pFileName[nLen - 1] == '/' || pFileName[nLen - 1] == '\\') &&
							   (!IsOSX() || (IsOSX() && !Q_stristr(pFileName, ".app")));
	Q_StripTrailingSlash(pFileName);
	...
	if(g_pFullFileSystem->IsDirectory(pFullPath, NULL) && (!IsOSX() || (IsOSX() && !Q_stristr(pFullPath, ".app"))))
	{
