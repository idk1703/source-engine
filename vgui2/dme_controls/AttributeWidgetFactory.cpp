...
public:
	...
	//	vgui::Panel *CreateWidget( vgui::Panel *parent, CDmElement *obj, INotifyUI *pNotify, CDmeEditorAttributeInfo
	//*pWidgetInfo, bool bAutoApply );
...
static CAttributeWidgetFactory<CAttributeSurfacePropertyPickerPanel> g_AttributeSurfacePropertyPickerWidgetFactory(
	"surfacepropertypicker");
...
static CAttributeWidgetFactory<CAttributeDetailTypePickerPanel> g_AttributeDetailTypePickerWidgetFactory(
	"detailtypepicker");
...
static CAttributeWidgetFactory<CAttributeInterpolatorChoicePanel> g_AttributeInterpolatorChoiceWidgetFactory(
	"interpolatorchoice");
...
//-----------------------------------------------------------------------------
IAttributeWidgetFactory *CAttributeWidgetFactoryList::GetWidgetFactory(CDmElement *object, CDmAttribute *pAttribute,
																	   CDmeEditorTypeDictionary *pTypeDictionary)
{
	if(!object)
		return NULL;
	...
	if(pTypeDictionary)
	{
		...
		if(pEditorInfo)
		{
			...
			{
				IAttributeWidgetFactory *pOverriddenFactory =
					g_pWidgetFactoryFactoryList->FindWidgetFactory(pEditorInfo->GetWidgetName());
...
void CAttributeWidgetFactoryList::ApplyChanges(vgui::Panel *pWidget, vgui::Panel *pSender)
{
	...
	if(pPanel && pPanel->GetDirty())
	{
		...
		vgui::ipanel()->SendMessage(pWidget->GetVPanel(), new KeyValues("ApplyChanges"),
									pSender ? pSender->GetVPanel() : NULL);
	}
}
...
void CAttributeWidgetFactoryList::Refresh(vgui::Panel *pWidget, vgui::Panel *pSender)
{
	if(pWidget)
	{
		vgui::ipanel()->SendMessage(pWidget->GetVPanel(), new KeyValues("Refresh"),
									pSender ? pWidget->GetVPanel() : NULL);
	}
}