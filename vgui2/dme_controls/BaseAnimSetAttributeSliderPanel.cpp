...
bool CBaseAnimSetAttributeSliderPanel::ChannelToSliderLookup_t::Less(const ChannelToSliderLookup_t &lhs,
																	 const ChannelToSliderLookup_t &rhs)
{
...
//-----------------------------------------------------------------------------
static void BlendFlexValues(AttributeValue_t *pResult, const AttributeValue_t &src, const AttributeValue_t &dest,
							float flBlend, float flBalanceFilter = 0.5f)
{
	...
	ValueBalanceToLeftRight(&flSrcLeft, &flSrcRight, src.m_pValue[ANIM_CONTROL_VALUE],
							src.m_pValue[ANIM_CONTROL_BALANCE]);

	float flDestLeft, flDestRight;
	ValueBalanceToLeftRight(&flDestLeft, &flDestRight, dest.m_pValue[ANIM_CONTROL_VALUE],
	...
	LeftRightToValueBalance(
		&pResult->m_pValue[ANIM_CONTROL_VALUE], &pResult->m_pValue[ANIM_CONTROL_BALANCE], flTargetLeft, flTargetRight,
	...
	pResult->m_pValue[ANIM_CONTROL_MULTILEVEL] =
		...
		(dest.m_pValue[ANIM_CONTROL_MULTILEVEL] - src.m_pValue[ANIM_CONTROL_MULTILEVEL]) * flBlend;
}
...
//
//-----------------------------------------------------------------------------
CBaseAnimSetAttributeSliderPanel::CBaseAnimSetAttributeSliderPanel(vgui::Panel *parent, const char *className,
	  ...
	  m_nFaderChangeFlags(0)
{
	...
	m_Sliders->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0,
							 ANIMATION_SET_EDITOR_ATTRIBUTESLIDERS_BUTTONTRAY_HEIGHT, 0, 0);
...
void CBaseAnimSetAttributeSliderPanel::StampValueIntoLogs(CDmElement *control, AnimationControlType_t type,
														  float flValue)
{
}
...
void CBaseAnimSetAttributeSliderPanel::OnThink()
{
	...
	bool nameChanged =
		(fader.name && (m_PreviousPreviewFader.IsEmpty() || Q_stricmp(m_PreviousPreviewFader.Get(), fader.name)))
			? true
			: false;
	...
	for(int i = 0; i < c; ++i)
	{
		...
		if(m_CtrlKeyPreviewSlider.Get() == slider)
		{
			...
			// If we aren't over any of the preset sliders, then we need to be able to ramp down to the current value,
			// too
		...
		preview.m_pValue[ANIM_CONTROL_VALUE] =
			slider->IsTransform() ? 0.0f : slider->GetControlDefaultValue(ANIM_CONTROL_VALUE);
		...
		BlendFlexValues(&preview, current, preview, 1.0f,
						slider->IsControlActive(ANIM_CONTROL_BALANCE) ? m_pPresetSideFilter->GetPos() : 0.5f);

		bool simple =
			fader.isbeingdragged || (!fader.holdingctrl && !slider->IsRampingTowardPreview() && !ctrlKeyChanged);
		...
		if(bFaderChanged || fader.isbeingdragged)
		{
			...
			{
				// For the "ramp down" case (just released ctrl key), we need the original values instead of the preview
				// valuees
...
void CBaseAnimSetAttributeSliderPanel::ApplyPreset(float flScale, AttributeDict_t &values)
{
	...
	for(int i = 0; i < c; ++i)
	{
		...
		BlendFlexValues(&blend, current, target, flScale,
						slider->IsControlActive(ANIM_CONTROL_BALANCE) ? m_pPresetSideFilter->GetPos() : 0.5f);
...
void CBaseAnimSetAttributeSliderPanel::UpdatePreviewSliderTimes()
{
	...
	switch(m_hEditor->GetRecordingState())
	{
		...
		case AS_RECORD:
		{
			// Put one or all things into record mode (if dragging a single slider, other channels should be in playback
			// mode)
...
void CBaseAnimSetAttributeSliderPanel::GetChannelsForControl(CDmElement *control,
															 CDmeChannel *channels[LOG_PREVIEW_MAX_CHANNEL_COUNT])
{
...
void CBaseAnimSetAttributeSliderPanel::ActivateControlSetInMode(int mode, int otherChannelsMode, int hiddenChannelsMode,
																CAttributeSlider *whichSlider /*= NULL*/)
{
	int i, c;
	...
	if(c > 0)
	{
		if(channelMode == CM_RECORD)
		{
			...
			for(i = 0; i < c; ++i)
			{
				...
				g_pChannelRecordingMgr->AddChannelToRecordingLayer(updateChannels[i], GetCurrentMovie(),
																   GetCurrentShot());
			}
...
static const char *s_pDefaultAttributeName[ANIM_CONTROL_COUNT] = {"defaultValue", "defaultBalance",
																  "defaultMultilevel"};
...
void CBaseAnimSetAttributeSliderPanel::MaybeAddPreviewLog(CDmeFilmClip *shot, CUtlVector<LogPreview_t> &list,
														  CDmElement *control, bool bDragging, bool isActiveLog,
														  bool bSelected)
{
	CDmeChannel *ctrlChannels[LOG_PREVIEW_MAX_CHANNEL_COUNT];
	GetChannelsForControl(control, ctrlChannels);
...
void CBaseAnimSetAttributeSliderPanel::PerformRecomputePreview()
{
	...
	for(i = 0; i < c; ++i)
	{
		...
		MaybeAddPreviewLog(shot, m_CurrentPreview, control, slider->IsDragging(), (control == previewControl),
						   slider->IsSelected());
	}
}
