...
void CBaseAnimationSetEditor::OnButtonToggled(KeyValues *params)
{
	...
	{
	// FIXME, could do this with MESSAGE_FUNC_PARAMS and look up "panel" ptr and compare to figure out which button was
	manipulated... g_pMovieMaker->SetTimeSelectionModeType( !m_pSelectionModeType->IsSelected() ?
...
void CBaseAnimationSetEditor::ChangeLayout(EAnimSetLayout_t newLayout)
{
	...
	switch(m_Layout)
	{
		...
		case LAYOUT_SPLIT:
		{
			...
			m_Splitter->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0,
									  ANIMATION_SET_EDITOR_BUTTONTRAY_HEIGHT, 0, 0);
			m_Splitter->SetBounds(0, ANIMATION_SET_EDITOR_BUTTONTRAY_HEIGHT, GetWide(),
			...
			m_hControlGroup->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);

			sub = new Splitter(m_Splitter->GetChild(1), "AnimSetEditorSubSplitter", SPLITTER_MODE_HORIZONTAL, 1);
			sub->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);

			m_hPresetFader->SetParent(sub->GetChild(0));
			m_hPresetFader->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);
			m_hAttributeSlider->SetParent(sub->GetChild(1));
			m_hAttributeSlider->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);
		}
		break;
		case LAYOUT_VERTICAL:
		{
			...
			for(i = 0; i < list.Count(); ++i)
			{
				...
				list[i]->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);
			}
...
void CBaseAnimationSetEditor::TransformImportedChannel(CDmeChannel *pChannel)
{
	...
	for(int i = 0; i < nLayerCount; ++i)
	{
		if(logType == AT_VECTOR3)
		{
			...
			{
				Assert(fabs(postTransform[0][3]) < 1e-3 && fabs(postTransform[1][3]) < 1e-3 &&
...
void CBaseAnimationSetEditor::ImportAnimation(CDmeChannelsClip *pChannelsClip, bool bVisibleOnly)
{
	...
	COperationFileListFrame *pStatusFrame =
		new COperationFileListFrame(this, "Import the Following Channels?", "Target Control", false);
	...
	for(int i = 0; i < nCount; ++i)
	{
		for(int k = 0; k < LOG_PREVIEW_MAX_CHANNEL_COUNT; ++k)
		{
			...
			Q_snprintf(pChannelInfo, sizeof(pChannelInfo), "\"%s\" : %s", pChannel->GetToElement()->GetName(),
			...
			Q_snprintf(pImportInfo, sizeof(pImportInfo), "\"%s\" : %s", pImportChannel->GetToElement()->GetName(),
	...
	for(int i = 0; i < nSrcCount; ++i)
	{
		...
		char pImportInfo[512];
		Q_snprintf(pImportInfo, sizeof(pImportInfo), "\"%s\" : %s", pMissingChannel->GetToElement()->GetName(),
...
//-----------------------------------------------------------------------------
void CBaseAnimationSetEditor::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog, bool bOpenFile,
...
bool CBaseAnimationSetEditor::OnReadFileFromDisk(const char *pFileName, const char *pFileFormat,
...
//-----------------------------------------------------------------------------
bool CBaseAnimationSetEditor::OnWriteFileToDisk(const char *pFileName, const char *pFileFormat,
												KeyValues *pContextKeyValues)
{
...
void CBaseAnimationSetEditor::OnOpenContextMenu(KeyValues *params)
{
	...
	for(int i = 0; i < c; ++i)
	{
		...
		m_hContextMenu->AddMenuItem(data.contextmenulabel, new KeyValues("OnChangeLayout", "value", (int)data.type),
									this);
	}
