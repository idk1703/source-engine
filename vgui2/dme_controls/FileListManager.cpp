...
template<int C>
int ListPanelStringSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
...
static ColumnInfo_t g_ColInfo[] = {
	...
	{"numelements", "#BxFileManager_NumElements", 60, fixedflags, ListPanelStringSortFunc<CI_NUMELEMENTS>,
	 vgui::Label::a_east},
	{"changed", "#BxFileManager_Changed", 50, fixedflags, ListPanelStringSortFunc<CI_CHANGED>, vgui::Label::a_center},
	{"in_perforce", "#BxFileManager_P4Exists", 35, fixedflags, ListPanelStringSortFunc<CI_INPERFORCE>,
	 vgui::Label::a_center},
	{"open_for_edit", "#BxFileManager_P4Edit", 40, fixedflags, ListPanelStringSortFunc<CI_OPENFOREDIT>,
	 vgui::Label::a_center},
};
...
{
	pFileManager->AddColumnHeader(ci, g_ColInfo[ci].columnName, g_ColInfo[ci].columnText, g_ColInfo[ci].startingWidth,
								  g_ColInfo[ci].flags);
...
int CFileListManager::AddItem(DmFileId_t fileid, const char *pFilename, const char *pPath, bool bLoaded, int nElements,
							  bool bChanged, bool bInPerforce, bool bOpenForEdit)
{
...
int AddMenuItemHelper(vgui::Menu *pMenu, const char *pItemName, const char *pKVName, vgui::Panel *pTarget,
					  bool bEnabled)
{
...
void CFileListManager::OnOpenContextMenu(KeyValues *pParams)
{
	...
	if(itemID < 0)
	{
		AddMenuItemHelper(m_hContextMenu, "Open File...", "open", this,
						  true); // Is this how we should load other files???
	}
	else
	{
...
void CFileListManager::OnOpenFile(KeyValues *pParams)
{
	...
	vgui::FileOpenDialog *pFileOpenDialog =
		new vgui::FileOpenDialog(this, "Save .dmx File As", false, pContextKeyValues);
...
void CFileListManager::OnDataChanged(KeyValues *pParams)
{
	...
	if(!bPerformFullRefresh)
	{
		...
		for(int i = 0; i < nCount; ++i)
		{
			...
			bPerformFullRefresh =
				(fileid != (DmFileId_t)GetItemUserData(i)) || Q_stricmp(pFileName, pKeyValues->GetString(pNameKey));
			if(bPerformFullRefresh)
				break;
...
void CFileListManager::Refresh()
{
	...
	for(int i = 0; i < nFiles; ++i)
	{
		...
		AddItem(fileid, V_UnqualifiedFileName(pFileName), path, bLoaded, nElements, bChanged, bInPerforce,
				bOpenForEdit);
	}
}
