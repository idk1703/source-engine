...
CAnimGroupTree::CAnimGroupTree(Panel *parent, const char *panelName, CBaseAnimSetControlGroupPanel *groupPanel)
	: BaseClass(parent, panelName), m_pGroupPanel(groupPanel)
{
}
...
CBaseAnimSetControlGroupPanel::CBaseAnimSetControlGroupPanel(vgui::Panel *parent, const char *className,
															 CBaseAnimationSetEditor *editor)
	: BaseClass(parent, className), m_bStartItemWasSelected(false), m_SliderNames(0, 0, true)
{
	m_hEditor = editor;
...
void CBaseAnimSetControlGroupPanel::ChangeAnimationSet(CDmeAnimationSet *newAnimSet)
{
	...
	if(bRebuildGroups)
	{
		...
		// Create root
		int rootIndex =
			AddItemToTree(m_hGroups, "root", -1, Color(128, 128, 128, 255), -1, (int)DMELEMENT_HANDLE_INVALID);
		...
		for(int i = 0; i < c; ++i)
		{
			...
			int groupIndex = AddItemToTree(m_hGroups, group->GetName(), rootIndex, group->GetValue<Color>("treeColor"),
										   i, (int)group->GetHandle());
			...
			if(array.IsValid())
			{
				for(int j = 0; j < array.Count(); ++j)
				{
					AddItemToTree(m_hGroups, array[j], groupIndex, Color(200, 200, 200, 255), -1,
								  (int)DMELEMENT_HANDLE_INVALID);
				}
			}
...
//-----------------------------------------------------------------------------
void CBaseAnimSetControlGroupPanel::ExpandOpenItems(OpenItemTree_t &tree, int nOpenTreeIndex, int nItemIndex,
													bool makeVisible)
{
