...
static Color s_BarColor[2][2] = {{Color(45, 45, 45, 255), Color(150, 80, 0, 255)},
								 {Color(30, 255, 255, 80), Color(30, 180, 255, 255)}};

static Color s_ZeroColor[2][2] = {{Color(33, 33, 33, 255), Color(0, 255, 255, 60)},
...
//-----------------------------------------------------------------------------
CAttributeSlider::CAttributeSlider(CBaseAnimSetAttributeSliderPanel *parent, const char *panelName,
								   CDmElement *pControl)
...
void CAttributeSlider::SetPreview(const AttributeValue_t &value, const AttributeValue_t &full, bool instantaneous,
...
void CAttributeSlider::PaintBackground()
{
	...
	{
		float flBalance =
			bUsePreview ? m_Preview.m_Current.m_pValue[ANIM_CONTROL_BALANCE] : GetValue(ANIM_CONTROL_BALANCE);
		float flLeftValue, flRightValue;
		ValueBalanceToLeftRight(&flLeftValue, &flRightValue, flValue, flBalance);
	...
	{
		float flMultiValue =
			bUsePreview ? m_Preview.m_Current.m_pValue[ANIM_CONTROL_MULTILEVEL] : GetValue(ANIM_CONTROL_MULTILEVEL);
		GetControlRect(&rect, ANIM_CONTROL_MULTILEVEL);
		...
		int nCenterX = rect.x + rect.width / 2;
		int nCenterY = rect.y + rect.height / 2;
		DrawCircularTick(s_MidpointColor, GetControlDefaultValue(ANIM_CONTROL_MULTILEVEL), nCenterX, nCenterY,
