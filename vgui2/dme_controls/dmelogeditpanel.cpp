...
{
	bool bValid =
		pLog && (pLog->GetDataType() == AT_INT || pLog->GetDataType() == AT_FLOAT || pLog->GetDataType() == AT_COLOR);
	if(bValid)
	{
		m_hLog = pLog;
	}
...
//
//-----------------------------------------------------------------------------
CDmeLogEditFrame::CDmeLogEditFrame(vgui::Panel *pParent, const char *pTitle) : BaseClass(pParent, "DmeLogEditFrame")
{
...
void CDmeLogEditFrame::DoModal(CDmeLog *pLog, DmeTime_t startTime, DmeTime_t endTime, KeyValues *pKeyValues)
{
	...
	switch(pLog->GetDataType())
	{
		...
		case AT_COLOR:
			m_pFilter->AddItem("RGB Channel", new KeyValues("Mask", "Value",
															CDmeLogEditPanel::FIELD_R | CDmeLogEditPanel::FIELD_G |
																CDmeLogEditPanel::FIELD_B));
