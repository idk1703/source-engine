...
CElementTree::CElementTree(CElementPropertiesTreeInternal *parent, const char *panelName)
	: BaseClass((Panel *)parent, panelName), m_pEditMenu(0), m_pParent(parent)
{
...
int CElementTreeViewListControl::AddItem(KeyValues *data, bool allowLabelEditing, int parentItemIndex,
										 CUtlVector<vgui::Panel *> &columnPanels)
{
...
void CElementTreeViewListControl::OnCursorMoved(int x, int y)
{
	...
	if((x > m_iTreeColumnWidth - 12) && (x < m_iTreeColumnWidth + 12))
	{
		SetCursor(dc_sizewe);
...
public:
	CHistoryMenuButton(Panel *parent, const char *panelName, const char *text, CElementPropertiesTreeInternal *tree,
					   int whichMenu);
...
CHistoryMenuButton::CHistoryMenuButton(Panel *parent, const char *panelName, const char *text,
									   CElementPropertiesTreeInternal *tree, int whichMenu)
...
public:
	CSearchComboBox(CElementPropertiesTreeInternal *tree, vgui::Panel *parent, const char *panelName, int numLines,
					bool allowEdit);
...
CSearchComboBox::CSearchComboBox(CElementPropertiesTreeInternal *tree, vgui::Panel *parent, const char *panelName,
								 int numLines, bool allowEdit)
...
CPropertiesTreeToolbar::CPropertiesTreeToolbar(vgui::Panel *parent, const char *panelName,
											   CElementPropertiesTreeInternal *tree)
	: BaseClass(parent, panelName), m_pTree(tree)
{
	...
	m_pBack = new CHistoryMenuButton(this, "Nav_Back", "#Dme_NavBack", tree,
									 CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_BACKWARD);
	...
	m_pFwd = new CHistoryMenuButton(this, "Nav_Forward", "#Dme_NavForward", tree,
									CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_FORWARD);
...
{
	m_pBack->SetEnabled(
		m_pTree->GetHistoryMenuItemCount(CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_BACKWARD) > 0 ? true
																											   : false);
	m_pFwd->SetEnabled(
		m_pTree->GetHistoryMenuItemCount(CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_FORWARD) > 0 ? true
																											  : false);

	// m_pShowSearchResults->SetEnabled( m_pTree->GetHistoryMenuItemCount(
	// CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_SEARCHHSITORY ) > 0 ? true : false );
}
...
// Constructor
//-----------------------------------------------------------------------------
CElementPropertiesTreeInternal::CElementPropertiesTreeInternal(vgui::Panel *parent, IDmNotify *pNotify,
															   CDmElement *pObject, bool autoApply /* = true */,
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::Refresh(RefreshType_t rebuild /* = false */,
											 bool preservePrevSelectedItem /*= false*/)
{
	if(!m_hObject.Get())
		return;
...
{
	for(int idx = itemIndex; idx != m_pTree->GetTree()->GetRootItemIndex();
		idx = m_pTree->GetTree()->GetItemParent(idx))
	{
...
void CElementPropertiesTreeInternal::OnPaste_(bool reference)
{
	...
	{
		itemIndex =
			m_pTree->GetTree()->GetItemParent(itemIndex); // if we're an array element, start with the array itself
	}
...
void RemoveAllReferencesToElement(CDmElement *pElement)
{
	...
	for(DmElementHandle_t hElement = g_pDataModel->FirstAllocatedElement(); hElement != DMELEMENT_HANDLE_INVALID;
...
// For each owner/attribute have an entry and them for each arrayIndex into any array type, need to sort by arrayIndex
// so we can remove them in reverse order
...
struct OwnerAttribute_t
{
	...
	OwnerAttribute_t(const OwnerAttribute_t &src) : sortedData(0, 0, ArrayIndexLessFunc)
	{
		...
		for(int i = src.sortedData.FirstInorder(); i != src.sortedData.InvalidIndex();
			i = src.sortedData.NextInorder(i))
		{
...
bool CElementPropertiesTreeInternal::OnRemoveFromData(KeyValues *item)
{
	...
	if(!pAttribute->IsFlagSet(FATTRIB_EXTERNAL) && !pAttribute->IsFlagSet(FATTRIB_TOPOLOGICAL) &&
...
bool CElementPropertiesTreeInternal::OnRemoveFromData(CUtlVector<KeyValues *> &list)
{
	...
	for(i = sorted.m_Sorted.FirstInorder(); i != sorted.m_Sorted.InvalidIndex(); i = sorted.m_Sorted.NextInorder(i))
	{
		...
		for(int j = entry.sortedData.LastInorder(); j != entry.sortedData.InvalidIndex();
			j = entry.sortedData.PrevInorder(j))
		{
...
bool CElementPropertiesTreeInternal::BuildExpansionListToFindElement_R(CUtlRBTree<CDmElement *, int> &visited,
																	   int depth, SearchResult_t &sr, CDmElement *owner,
																	   CDmElement *element, const char *attributeName,
																	   int arrayIndex, CUtlVector<int> &expandIndices)
{
	if(!element)
		return true;
	...
	if(element == sr.handle.Get())
	{
		if(sr.attributeName.Length() > 0)
		{
			...
			for(CDmAttribute *attribute = element->FirstAttribute(); attribute;
				attribute = attribute->NextAttribute(), --idx)
			{
	...
	for(CDmAttribute *attribute = element->FirstAttribute(); attribute; attribute = attribute->NextAttribute(), --idx)
	{
		...
		{
			if(!BuildExpansionListToFindElement_R(visited, depth + 1, sr, element,
												  attribute->GetValueElement<CDmElement>(), attributeName, -1,
												  expandIndices))
			{
				expandIndices.AddToTail(idx);
				return false;
		...
		else if(attribute->GetType() == AT_ELEMENT_ARRAY)
		{
			...
			for(int i = 0; i < c; ++i)
			{
				if(!BuildExpansionListToFindElement_R(visited, depth + 1, sr, element, elementArray[i], attributeName,
													  i, expandIndices))
				{
...
static ConVar dme_properties_maxsearchresults("dme_properties_maxsearchresults", "50", 0,
											  "Max number of search results to track.");

void CElementPropertiesTreeInternal::FindMatchingElements_R(CUtlRBTree<CDmElement *, int> &visited,
															const char *searchstr, CDmElement *element,
															CUtlVector<SearchResult_t> &list)
{
...
void CElementPropertiesTreeInternal::NavigateToSearchResult()
{
	...
	BuildExpansionListToFindElement_R(visited, 0, m_SearchResults[m_nCurrentSearchResult], m_hObject.Get(),
									  m_hObject.Get(), "name", -1, expandIndices);
...
void CElementPropertiesTreeInternal::SpewHistory()
{
	...
	for(i = 0; i < c; ++i)
	{
		...
		Msg("%s:  [%02d] %s <%s>\n",
			((i < m_nCurrentHistoryPosition) ? "Fwd" : (i == m_nCurrentHistoryPosition ? "Current" : "Backward")), i,
			element->GetName(), element->GetTypeString());
	}
}
...
//-----------------------------------------------------------------------------
bool CElementPropertiesTreeInternal::ShowSetElementAttributeDialog(CDmElement *pOwner, const char *pAttributeName,
																   int nArrayIndex, const char *pElementType)
{
...
void CElementPropertiesTreeInternal::OnShowFileDialog(KeyValues *params)
{
	...
	pDialog->AddFilter("*.dmx", "Generic MovieObjects File (*.dmx)", true,
					   "movieobjects"); // read/write generic movieobjects files
...
void CElementPropertiesTreeInternal::OnExportElement(const char *pFullPath, KeyValues *pContext)
{
	...
	{
		// HACK - this is just a temporary hack - we should really force serialization to traverse past fileid changes
		// in this case
...
//-----------------------------------------------------------------------------
vgui::Panel *CElementPropertiesTreeInternal::CreateAttributeDataWidget(CDmElement *pElement, const char *pWidgetName,
																	   CDmElement *obj, CDmAttribute *pAttribute,
																	   int nArrayIndex)
{
...
void CElementPropertiesTreeInternal::GenerateContextMenu(int itemIndex, int x, int y)
{
	...
	// Add Attribute
	if((bIsElementAttribute && !bIsElementAttributeNull) || m_pTree->GetTree()->GetRootItemIndex() == itemIndex ||
	   bIsElementArrayItem)
	{
	...
	if(bIsElementAttribute || bIsElementArrayAttribute || bIsElementArrayItem)
	{
		...
		for(int i = g_pDataModel->GetFirstFactory(); g_pDataModel->IsValidFactory(i);
			i = g_pDataModel->GetNextFactory(i))
		{
	...
	// sharing
	if((bIsElementAttribute && !bIsElementAttributeNull) || m_pTree->GetTree()->GetRootItemIndex() == itemIndex ||
	   bIsElementArrayAttribute || bIsElementArrayItem)
	{
		CUtlVector<KeyValues *> selected;
		m_pTree->GetTree()->GetSelectedItemData(selected);
		int nElements = 0;
...
void CElementPropertiesTreeInternal::DropItemsIntoArray(CDmrElementArray<> &array, CUtlVector<KeyValues *> &msglist,
														CUtlVector<CDmElement *> &list, int nArrayIndex,
														DropOperation_t op)
{
	...
	else if(op == DO_MOVE)
	{
		...
		for(int i = 0; i < nMsgs; ++i)
		{
			...
			CDmAttribute *pAttribute =
				pOwner->GetAttribute(pAttributeName, isArrayElement ? AT_ELEMENT_ARRAY : AT_ELEMENT);
			if(!pAttribute)
				continue;
		...
		for(int i = arrayItemSorter.LastInorder(); i != arrayItemSorter.InvalidIndex();
			i = arrayItemSorter.PrevInorder(i))
		{
...
CElementPropertiesTreeInternal::DropOperation_t CElementPropertiesTreeInternal::GetDropOperation(
	int itemIndex, CUtlVector<KeyValues *> &msglist)
{
	...
	for(int i = 0; i < nMsgs; ++i)
	{
		...
		op = DO_MOVE; // basically, op will only stay DO_MOVE if *every* item is a non-element or is an array (or array
					  // item)
	}
...
void CElementPropertiesTreeInternal::OnItemDropped(int itemIndex, CUtlVector<KeyValues *> &msglist)
{
	...
	if(isArrayElement)
	{
		...
		char str[128];
		V_snprintf(str, sizeof(str), "%s %s element%s", bReplace ? "Replace with" : "Insert",
	...
	else
	{
		...
		else
		{
			...
			char str[128];
			V_snprintf(str, sizeof(str), "%s %s elements%s", bReplace ? "Replace array with" : "Insert",
					   op == DO_COPY ? "copied" : (op == DO_MOVE ? "moved" : "referenced"),
					   bHead   ? " at head"
					   : bTail ? " at tail"
							   : "");
...
// Gets tree view text
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::GetTreeViewText(CDmElement *obj, CDmAttribute *pAttribute, int nArrayIndex,
													 char *pBuffer, int nMaxLen, bool &editableText)
{
	pBuffer[0] = 0;
...
void CElementPropertiesTreeInternal::SpewOpenItems(int depth, OpenItemTree_t &tree, int nOpenTreeIndex, int nItemIndex)
{
	...
	if(nOpenTreeIndex != tree.InvalidIndex())
	{
		...
		{
			Msg("[%d] Marking %s <%s> %s array(%s) [expanded %i]\n", depth, info.m_Item.m_pElement->GetName(),
				info.m_Item.m_pElement->GetTypeString(), info.m_Item.m_pAttributeName.Get(),
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::ExpandOpenItems(OpenItemTree_t &tree, int nOpenTreeIndex, int nItemIndex,
													 bool makeVisible)
{
	int i = tree.FirstChild(nOpenTreeIndex);
	if(nOpenTreeIndex != tree.InvalidIndex())
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::BuildOpenItemList(OpenItemTree_t &tree, int nParent, int nItemIndex,
													   bool preservePrevSelectedItem)
{
	KeyValues *data = m_pTree->GetItemData(nItemIndex);
	if(!data)
		return;
	...
	if(selected)
	{
		...
		int preserve = preservePrevSelectedItem ? m_pTree->GetTree()->GetPrevChildItemIndex(nItemIndex)
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::SetTreeItemColor(int nItemID, CDmElement *pEntryElement, bool bIsElementArrayItem,
													  bool bEditableLabel)
{
...
// Adds a single entry into the tree
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::CreateTreeEntry(int parentNodeIndex, CDmElement *obj, CDmAttribute *pAttribute,
													 int nArrayIndex, AttributeWidgets_t &widgets)
{
	char pText[512];
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::SetupWidgetInfo(AttributeWidgetInfo_t *pInfo, CDmElement *obj,
													 CDmAttribute *pAttribute, int nArrayIndex)
{
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::InsertSingleAttribute(int parentNodeIndex, CDmElement *obj,
														   CDmAttribute *pAttribute, int nArrayIndex)
{
...
//-----------------------------------------------------------------------------
void CElementPropertiesTreeInternal::InsertAttributeArrayMembers(int parentNodeIndex, CDmElement *obj,
																 CDmAttribute *pAttribute)
{
	Assert(obj);
...
void CElementPropertiesTreeInternal::OnRefresh()
{
	...
	CElementTreeNotifyScopeGuard notify("CElementPropertiesTreeInternal::OnRefresh",
										NOTIFY_SETDIRTYFLAG | NOTIFY_CHANGE_TOPOLOGICAL, m_pNotify);
}
...
//
//-----------------------------------------------------------------------------
CElementPropertiesTree::CElementPropertiesTree(vgui::Panel *parent, IDmNotify *pNotify, CDmElement *pObject,
											   CDmeEditorTypeDictionary *pDict)
...
void CElementPropertiesTree::Refresh(CElementPropertiesTreeInternal::RefreshType_t rebuild /* = REFRESH_REBUILD */,
									 bool preservePrevSelectedItem /*= false*/)
{
...
CDmeElementPanel::CDmeElementPanel(vgui::Panel *pParent, const char *pPanelName) : BaseClass(pParent, this, NULL) {}
