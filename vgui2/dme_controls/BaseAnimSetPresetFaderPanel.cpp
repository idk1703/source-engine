...
void CAddPresetDialog::OnCommand(const char *command)
{
	if(!Q_stricmp(command, "OK"))
	{
		...
		CDmePresetGroup *pPresetGroup =
			pCurrentGroup ? GetElementKeyValue<CDmePresetGroup>(pCurrentGroup, "presetGroup") : NULL;
...
void CPresetSlider::UpdateProceduralValues()
{
	...
	switch(nPresetType)
	{
		...
		case PROCEDURAL_PRESET_STAGGER:
			// These are handled elsewhere right now... at some point we'll copy in values at head position in order to
			// do ctrl-key preview mode
			break;
...
// Constructor
//-----------------------------------------------------------------------------
CBaseAnimSetPresetFaderPanel::CBaseAnimSetPresetFaderPanel(vgui::Panel *parent, const char *className,
														   CBaseAnimationSetEditor *editor)
	: BaseClass(parent, className), m_flLastFrameTime(0.0f), m_pSliders(NULL), m_pWorkspace(NULL)
{
	m_hEditor = editor;
	...
	m_pSliders->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 24, 0, 0);
	...
	m_pFilter->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_RIGHT, 2, 2, 2, 22);
	m_pFilter->SetPos(0, 0);

	m_pWorkspace->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);
}
...
void CBaseAnimSetPresetFaderPanel::OnPresetNameSelected(KeyValues *pParams)
{
	...
	if(pPresetGroup->m_bIsReadOnly)
	{
		vgui::MessageBox *pError =
			new MessageBox("Add Preset Error", "Can't add preset to a read-only preset group!\n", this);
		pError->SetDeleteSelfOnClose(true);
		pError->DoModal();
		return;
	}
	...
	{
		vgui::MessageBox *pError =
			new MessageBox("Add Preset Error", "A preset with that name already exists!\n", this);
		pError->SetDeleteSelfOnClose(true);
		pError->DoModal();
		return;
	}
