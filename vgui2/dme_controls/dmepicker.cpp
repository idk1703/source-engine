...
void CDmePicker::RefreshDmeList()
{
	...
	int nTotalCount = 0;
	for(int nItemID = m_pDmeBrowser->FirstItem(); nItemID != m_pDmeBrowser->InvalidItemID();
	...
	Q_snprintf(pColumnTitle, sizeof(pColumnTitle), "%s (%d/%d)", "Dme Elements", nMatchingElements, nTotalCount);
...
//
//-----------------------------------------------------------------------------
CDmePickerFrame::CDmePickerFrame(vgui::Panel *pParent, const char *pTitle) : BaseClass(pParent, "DmePickerFrame")
{
