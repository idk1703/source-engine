...
// Constructor
//-----------------------------------------------------------------------------
static int __cdecl DestPresetNameSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
...
static int __cdecl SrcPresetNameSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
										 const vgui::ListPanelItem &item2)
{
...
void CDmePresetRemapPanel::RefreshPresetList()
{
	...
	bool bUseRemap =
		(pRemap && m_hSourceGroup.Get() && !Q_stricmp(pRemap->m_SourcePresetGroup, m_hSourceGroup->GetName()));
...
void CDmePresetRemapPanel::ApplyChangesToPresetRemap()
{
	...
	for(int nItemID = m_pPresetRemapList->FirstItem(); nItemID != m_pPresetRemapList->InvalidItemID();
...
//-----------------------------------------------------------------------------
static int __cdecl IndexSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
								 const vgui::ListPanelItem &item2)
{
...
void CDmePresetGroupEditorPanel::SetSelectedPreset(CDmePreset *pPreset)
{
	...
	for(int nItemID = m_pPresetList->FirstItem(); nItemID != m_pPresetList->InvalidItemID();
...
void CDmePresetGroupEditorPanel::SetSelectedPresetGroup(CDmePresetGroup *pPresetGroup)
{
	...
	for(int nItemID = m_pPresetGroupList->FirstItem(); nItemID != m_pPresetGroupList->InvalidItemID();
...
bool CDmePresetGroupEditorPanel::HasDuplicatePresetName(const char *pPresetName, CDmePreset *pIgnorePreset)
{
	...
	{
		vgui::MessageBox *pError = new vgui::MessageBox("#DmePresetGroupEditor_DuplicatePresetNameTitle",
...
//-----------------------------------------------------------------------------
static int BuildExportedControlList(CDmeAnimationSet *pAnimationSet, CDmePresetGroup *pPresetGroup,
...
// Fileopen state machine
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog, bool bOpenFile,
													 const char *pFileFormat, KeyValues *pContextKeyValues)
{
	if(bOpenFile)
	{
...
bool CDmePresetGroupEditorPanel::OnReadFileFromDisk(const char *pFileName, const char *pFileFormat,
...
bool CDmePresetGroupEditorPanel::OnWriteFileToDisk(const char *pFileName, const char *pFileFormat,
												   KeyValues *pContextKeyValues)
{
	...
	if(!Q_stricmp(pContextKeyValues->GetName(), "ExportPresetGroup"))
	{
		...
		bool bOk = g_pDataModel->SaveToFile(pFileName, NULL, g_pDataModel->GetDefaultEncoding(pFileFormat), pFileFormat,
											pPresetGroup);
		return bOk;
	}
	...
	bool bOk =
		g_pDataModel->SaveToFile(pFileName, NULL, g_pDataModel->GetDefaultEncoding(pFileFormat), pFileFormat, pRoot);
...
void CDmePresetGroupEditorPanel::OnOpenContextMenu(KeyValues *kv)
{
	...
	if(pPresetGroup)
	{
		...
		m_hContextMenu->AddSeparator();
		m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ToggleVisibility", new KeyValues("ToggleGroupVisibility"),
									this);
	...
	if(pPresetGroup)
	{
		...
		m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ExportPresetsToFaceposer",
									new KeyValues("ExportPresetGroupsToTXT"), this);
		m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ExportPresetsToExpression",
...
bool CDmePresetGroupEditorPanel::HasDuplicateGroupName(const char *pGroupName, CDmePresetGroup *pIgnorePreset)
{
	...
	if(pMatch && pMatch != pIgnorePreset)
	{
		vgui::MessageBox *pError = new vgui::MessageBox("#DmePresetGroupEditor_DuplicateNameTitle",
...
void CDmePresetGroupEditorPanel::OnAddPhonemeGroup()
{
	vgui::InputDialog *pInput =
		new vgui::InputDialog(this, "Add Phoneme Preset Group", "Enter name of new preset group", "phoneme");
	pInput->SetMultiline(false);
...
void CDmePresetGroupEditorPanel::OnRemoveGroup()
{
	...
	{
		vgui::MessageBox *pError = new vgui::MessageBox("#DmePresetGroupEditor_CannotRemovePresetGroupTitle",
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmePresetGroupListPanel::CDmePresetGroupListPanel(vgui::Panel *pParent, const char *pName,
		...
		if(m_pPresetGroupPanel)
		{
			...
			if(pPreset)
			{
				...
				CDmePresetGroup *pDropGroup =
					pKeyValues ? GetElementKeyValue<CDmePresetGroup>(pKeyValues, "presetGroup") : NULL;
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmePresetListPanel::CDmePresetListPanel(vgui::Panel *pParent, const char *pName,
										 CDmePresetGroupEditorPanel *pComboPanel)
	: BaseClass(pParent, pName), m_pPresetGroupPanel(pComboPanel)
{
}
