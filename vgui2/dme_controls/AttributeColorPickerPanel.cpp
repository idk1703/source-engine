...
void CAttributeColorPickerPanel::OnPreview(KeyValues *data)
{
	...
	if(IsAutoApply())
	{
		...
		CElementTreeNotifyScopeGuard guard("CAttributeColorPickerPanel::OnPreview", NOTIFY_CHANGE_ATTRIBUTE_VALUE,
										   GetNotify());
	}
	else
	{
...
void CAttributeColorPickerPanel::OnCancelled()
{
	...
	Refresh();
	CElementTreeNotifyScopeGuard notify("CAttributeColorPickerPanel::OnCancelled", NOTIFY_CHANGE_ATTRIBUTE_VALUE,
										GetNotify());
	SetDirty(false);
}
...
void CAttributeColorPickerPanel::OnPicked(KeyValues *data)
{
	...
	{
		CElementTreeUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, GetNotify(), "Set Attribute Value",
										 "Set Attribute Value");
		SetAttributeValue(c);
	}
