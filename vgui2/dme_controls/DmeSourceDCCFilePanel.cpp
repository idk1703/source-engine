...
//
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CDmeSourceDCCFilePanel, DmeSourceDCCFile, "DmeSourceDCCFileDefault",
						   "Maya/XSI Source File Editor", true);
...
bool CDmeSourceDCCFilePanel::CheckForDuplicateNames(const char *pDCCObjectName, int nDCCObjectSkipIndex)
{
	...
	if(pDCCObjectName[0])
	{
		...
		for(int i = 0; i < nCount; ++i)
		{
			...
			{
				vgui::MessageBox *pError = new vgui::MessageBox("#DmeSourceDCCFile_DuplicateSourceTitle",
																"#DmeSourceDCCFile_DuplicateSourceText", GetParent());
				pError->DoModal();
				return true;
			}
		}
