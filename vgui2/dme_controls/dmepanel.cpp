										   ...
										   const char *pEditorDisplayName, bool bIsDefault, bool bIsOverride)
{
		...
		pPrevFactory = pFactory, pFactory = pFactory->m_pNext)
	{
		   ...
		   !Q_stricmp(pFactory->m_pEditorDisplayName, pEditorDisplayName))
		{
			...
			if(!bIsOverride)
			{
				AssertMsg(pFactory->m_bIsOverride,
						  ("Two DmePanel factories have the same name (\"%s\") + type (\"%s\")!\n", pElementType,
						   pEditorName));
				return;
			}
...
//-----------------------------------------------------------------------------
bool CDmePanel::CreateDmePanel(vgui::Panel *pParent, const char *pPanelName, CDmElement *pElement,
