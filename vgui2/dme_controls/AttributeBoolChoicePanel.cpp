...
void CAttributeBoolChoicePanel::PopulateComboBox(vgui::ComboBox *pComboBox)
{
	...
	if(pInfo->HasChoiceType())
	{
		...
		if(ElementPropertiesChoices()->GetBoolChoiceList(pInfo->GetChoiceType(), GetPanelElement(), GetAttributeName(),
...
void CAttributeBoolChoicePanel::SetAttributeFromComboBox(vgui::ComboBox *pComboBox, KeyValues *pKeyValues)
{
	...
	CUndoScopeGuard guard(NOTIFY_SOURCE_PROPERTIES_TREE, NOTIFY_SETDIRTYFLAG, "Set Attribute Value",
						  "Set Attribute Value");
