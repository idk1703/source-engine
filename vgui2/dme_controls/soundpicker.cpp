...
void CSoundPicker::UpdateGameSoundColumnHeader(int nMatchCount, int nTotalCount)
{
	...
	Q_snprintf(pColumnTitle, sizeof(pColumnTitle), "%s (%d/%d)", "Game Sound", nMatchCount, nTotalCount);
...
void CSoundPicker::RefreshGameSoundList()
{
	...
	int nTotalCount = 0;
	for(int nItemID = m_pGameSoundList->FirstItem(); nItemID != m_pGameSoundList->InvalidItemID();
...
void CSoundPicker::PlayGameSound(const char *pSoundName)
{
	...
	{
		m_nPlayingSound =
			EngineTool()->StartSound(0, true, -1, CHAN_STATIC, params.soundname, params.volume, params.soundlevel,
									 vec3_origin, vec3_origin, 0, params.pitch, false, params.delay_msec / 1000.0f);
	}
}
...
{
	StopSoundPreview();
	m_nPlayingSound = EngineTool()->StartSound(0, true, -1, CHAN_STATIC, pSoundName, VOL_NORM, SNDLVL_NONE, vec3_origin,
...
//
//-----------------------------------------------------------------------------
CSoundPickerFrame::CSoundPickerFrame(vgui::Panel *pParent, const char *pTitle, int nFlags) : BaseClass(pParent)
{
...
//-----------------------------------------------------------------------------
void CSoundPickerFrame::DoModal(CSoundPicker::PickType_t initialType, const char *pInitialValue,
