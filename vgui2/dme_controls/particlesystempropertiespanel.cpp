...
public:
	...
	void DoModal(CDmeParticleSystemDefinition *pParticleSystem, ParticleFunctionType_t type,
...
static int __cdecl ParticleFunctionNameSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
...
//-----------------------------------------------------------------------------
void CParticleFunctionPickerFrame::RefreshParticleFunctions(CDmeParticleSystemDefinition *pParticleSystem,
...
// Sets the current scene + animation list
//-----------------------------------------------------------------------------
void CParticleFunctionPickerFrame::DoModal(CDmeParticleSystemDefinition *pParticleSystem, ParticleFunctionType_t type,
										   KeyValues *pContextKeyValues)
{
	CleanUpMessage();
...
static int __cdecl ParticleChildrenNameSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
												const vgui::ListPanelItem &item2)
{
	const char *string1 = item1.kv->GetString("name");
	const char *string2 = item2.kv->GetString("name");
...
CParticleChildrenPickerFrame::CParticleChildrenPickerFrame(vgui::Panel *pParent, const char *pTitle,
														   IParticleSystemPropertiesPanelQuery *pQuery)
...
void CParticleChildrenPickerFrame::OnCommand(const char *pCommand)
{
	if(!Q_stricmp(pCommand, "Ok"))
	{
		...
		CDmeParticleSystemDefinition *pParticleSystem =
			GetElementKeyValue<CDmeParticleSystemDefinition>(pKeyValues, "particleSystem");
...
public:
	...
	CParticleFunctionBrowser(vgui::Panel *pParent, const char *pName, ParticleFunctionType_t type,
							 IParticleSystemPropertiesPanelQuery *pQuery);
...
//-----------------------------------------------------------------------------
static int __cdecl ParticleFunctionSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
											const vgui::ListPanelItem &item2)
{
...
// constructor, destructor
//-----------------------------------------------------------------------------
CParticleFunctionBrowser::CParticleFunctionBrowser(vgui::Panel *pParent, const char *pName, ParticleFunctionType_t type,
												   IParticleSystemPropertiesPanelQuery *pQuery)
...
void CParticleFunctionBrowser::SelectParticleFunction(CDmeParticleFunction *pFind)
{
	...
	for(int nItemID = m_pFunctionList->FirstItem(); nItemID != m_pFunctionList->InvalidItemID();
		nItemID = m_pFunctionList->NextItem(nItemID))
	{
...
void CParticleFunctionBrowser::OnAdd()
{
	...
	{
		CParticleChildrenPickerFrame *pPicker =
			new CParticleChildrenPickerFrame(this, "Select Child Particle Systems", m_pQuery);
...
void CParticleFunctionBrowser::OnRename()
{
	...
	vgui::InputDialog *pInput =
		new vgui::InputDialog(this, "Rename Particle Function", "Enter new name of particle function");
...
// Constructor
//-----------------------------------------------------------------------------
CParticleSystemPropertiesPanel::CParticleSystemPropertiesPanel(IParticleSystemPropertiesPanelQuery *pQuery,
															   vgui::Panel *pParent)
	: BaseClass(pParent, "ParticleSystemPropertiesPanel"), m_pQuery(pQuery)
{
	...
	m_pFunctionTypeCombo =
		new vgui::ComboBox(pSplitterLeftSide, "FunctionTypeCombo", PARTICLE_FUNCTION_COUNT + 1, false);
	...
	m_pParticleFunctionProperties->SetAutoResize(vgui::Panel::PIN_TOPLEFT, vgui::Panel::AUTORESIZE_DOWNANDRIGHT, 6, 6,
												 -6, -6);
	...
	for(int i = 0; i < PARTICLE_FUNCTION_COUNT; ++i)
	{
		...
		m_pParticleFunctionBrowser[i] = new CParticleFunctionBrowser(m_pFunctionBrowserArea, "FunctionBrowser",
																	 (ParticleFunctionType_t)i, m_pQuery);
...
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CParticleSystemDmePanel, DmeParticleSystemDefinition, "DmeParticleSystemDefinitionEditor",
						   "Particle System Editor", true);

//-----------------------------------------------------------------------------
// constructor, destructor
//-----------------------------------------------------------------------------
CParticleSystemDmePanel::CParticleSystemDmePanel(vgui::Panel *pParent, const char *pName) : BaseClass(pParent, pName)
{
