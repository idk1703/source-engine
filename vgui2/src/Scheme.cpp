...
public:
	...
	VPANEL GetSizingPanel()
	{
		return m_SizingPanel;
	}
...
SchemeEntryTranslation_t g_SchemeTranslation[] = {
	...
	{"Border.Selection"
	 "BorderSelection",
	 "0 0 0 196"}, // the additional border color for displaying the default/selected button
...
void CScheme::LoadFonts()
{
	...
	if(IsPC())
	{
		bValid = vgui::g_pSystem->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Source\\Language", language,
													sizeof(language) - 1);
	}
	else
	{
	...
	for(KeyValues *kv = m_pData->FindKey("Fonts", true)->GetFirstSubKey(); kv != NULL; kv = kv->GetNextKey())
	{
		for(int i = 0; i < 2; i++)
		{
			...
			const char *fontName = GetMungedFontName(
				kv->GetName(), tag, proportionalFont); // first time it adds a normal font, and then a proportional one
...
void CScheme::ReloadFontGlyphs()
{
	...
	FOR_EACH_DICT_FAST(m_FontAliases, i)
	{
		...
		for(KeyValues *fontdata = kv->GetFirstSubKey(); fontdata != NULL; fontdata = fontdata->GetNextKey())
		{
			...
			{
				// add the new set
				g_pSurface->SetBitmapFontGlyphSet(m_FontAliases[i]._font,
												  g_pSurface->GetBitmapFontName(fontdata->GetString("name")), scalex,
												  scaley, flags);
			}
			else
			{
				...
				{
					// add the new set
					g_pSurface->SetFontGlyphSet(m_FontAliases[i]._font, fontdata->GetString("name"), tall,
												fontdata->GetInt("weight"), blur, scanlines, flags, nRangeMin,
												nRangeMax);
				}
				else
				{
...
void CScheme::SpewFonts(void)
{
	...
	FOR_EACH_DICT_FAST(m_FontAliases, i)
	{
		...
		Msg("  %2d: HFont:0x%8.8x, %s, %s, font:%s, tall:%d(%d). %s\n", i, Font, szTrueFontName ? szTrueFontName : "??",
			szFontAlias ? szFontAlias : "??", szFontName ? szFontName : "??", g_pSurface->GetFontTall(Font),
			g_pSurface->GetFontTallRequested(Font), szFontFamilyName ? szFontFamilyName : "");
	}
}
...
{
	if(!imageName || strlen(imageName) <= 0) // frame icons and the like are in the scheme file and may not be defined,
											 // so if this is null then fail silently
	{
		return NULL;
	}
...
const char *CScheme::GetMungedFontName(const char *fontName, const char *scheme, bool proportional)
{
	...
	else
	{
		Q_snprintf(mungeBuffer, sizeof(mungeBuffer), "%s-%s", fontName,
				   proportional ? "p" : "no"); // we don't want the "(null)" snprintf appends
	}
	return mungeBuffer;
}
...
int CScheme::GetMinimumFontHeightForCurrentLanguage()
{
	...
	if(bValid)
	{
		if(!stricmp(language, "korean") || !stricmp(language, "tchinese") || !stricmp(language, "schinese") ||
