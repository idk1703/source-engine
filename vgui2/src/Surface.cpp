...
public:
	...
	virtual void CreatePopup(VPANEL panel, bool minimised, bool showTaskbarIcon, bool disabled, bool mouseInput,
							 bool kbInput);
	...
	virtual bool SetFontGlyphSet(HFont font, const char *windowsFontName, int tall, int weight, int blur, int scanlines,
...
private:
	...
	virtual HRESULT STDMETHODCALLTYPE DragEnter(IDataObject *pDataObject, DWORD grfKeyState, POINTL pt,
												DWORD *pdwEffect)
	{
		if(_dragData)
		{
...
void CWin32Surface::DrawTexturedSubRect(int x0, int y0, int x1, int y1, float texs0, float text0, float texs1,
										float text1)
{
}
...
void CWin32Surface::SetEmbeddedPanel(VPANEL panel)
{
	...
	char language[64];
	if(g_pSystem->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Source\\Language", language,
									sizeof(language) - 1))
	{
...
void CWin32Surface::PushMakeCurrent(VPANEL panel, bool useInsets)
{
	...
	else
	{
		...
		::SetViewportOrgEx(PLAT(_currentContextPanel)->hdc, (absPanel[0] + inset[0]) - absThis[0],
						   (absPanel[1] + inset[1]) - absThis[1], null);
	}
...
//-----------------------------------------------------------------------------
void CWin32Surface::DrawSetTextureRGBA(int id, const unsigned char *rgba, int wide, int tall, int hardwareFilter,
									   bool forceReload)
{
...
void CWin32Surface::DrawSetTextureRGBAEx(int id, const unsigned char *rgba, int wide, int tall, ImageFormat imageFormat)
{
...

	{
		...
		texture->_icon = NULL;
		texture->_dib = NULL;
		texture->_bitmap = staticCreateBitmapHandle(texture->_wide, texture->_tall, PLAT(_currentContextPanel)->hdc, 32,
													&texture->_dib);
...
bool CWin32Surface::LoadBMP(Texture *texture, const char *filename)
{
	...
	if(bmfHeader.bfType == DIB_HEADER_MARKER)
	{
		...
		char *pDIB = (LPSTR)::GlobalLock((HGLOBAL)hDIB);
		{
			...
			texture->_bitmap = staticCreateBitmapHandle(texture->_wide, texture->_tall, PLAT(_currentContextPanel)->hdc,
														32, &texture->_dib);
...
bool CWin32Surface::LoadTGA(Texture *texture, const char *filename)
{
	...
	texture->_icon = NULL;
	texture->_bitmap = staticCreateBitmapHandle(tgaHeader.width, tgaHeader.height, PLAT(_currentContextPanel)->hdc, 32,
												&texture->_dib);
	texture->_bMask = bMask;
	if(bMask)
		texture->_maskBitmap = staticCreateBitmapHandle(tgaHeader.width, tgaHeader.height,
														PLAT(_currentContextPanel)->hdc, 32, &texture->_maskDib);
	else
...
void CWin32Surface::SetAsToolBar(VPANEL panel, bool state)
{
	...
	if(panel && PLAT(panel))
	{
		if(state)
		{
			::SetWindowLong(PLAT(panel)->hwnd, GWL_EXSTYLE,
								...
								bool kbInput)
{
	...
	plat->hwnd =
		CreateWindowEx(style_ex, "Surface", "", style, x, y, wide, tall, hwndParent, NULL, GetModuleHandle(NULL), NULL);
...
bool CWin32Surface::RecreateContext(VPANEL panel)
{
	if(panel && PLAT(panel))
	{
		...
		if((wide > plat->bitmapSize[0]) || (tall > plat->bitmapSize[1]) || (wide < (plat->bitmapSize[0] - 200)) ||
	...
	if(panel && PLAT(panel))
	{
		...
		if(focus && ((VPanel *)focus)->HasParent((VPanel *)panel))
		{
			...
			if(g_pInput->GetAppModalSurface() &&
			   !((VPanel *)focus)->HasParent((VPanel *)g_pInput->GetAppModalSurface()))
			{
...
//-----------------------------------------------------------------------------
bool CWin32Surface::SetFontGlyphSet(HFont font, const char *windowsFontName, int tall, int weight, int blur,
...
bool CWin32Surface::SetBitmapFontGlyphSet(HFont font, const char *windowsFontName, float scalex, float scaley,
										  int flags)
{
	Assert(0);
	return false;
...
void CWin32Surface::DrawFilledRectFastFade(int x0, int y0, int x1, int y1, int fadeStartPt, int fadeEndPt,
										   unsigned int alpha0, unsigned int alpha1, bool bHorizontal)
{
	Assert(0);
}

void CWin32Surface::DrawFilledRectFade(int x0, int y0, int x1, int y1, unsigned int alpha0, unsigned int alpha1,
									   bool bHorizontal)
{
	Assert(0);
}
...
static void staticNotifyIconProc(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	switch(lparam)
	{
		case WM_LBUTTONDOWN:
		{
			...
			{
				g_pIVgui->PostMessage(g_pSurface->GetNotifyPanel(),
...
static LRESULT CALLBACK staticProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	switch(msg)
	{
		...
		case WM_PAINT:
		{
			...
			//			debug timing code
			//			ivgui()->DPrintf2("Paint timings (%.3f %.3f)\n", (float)(solveTime - startTime), (float)(paintTime -
			//solveTime));
