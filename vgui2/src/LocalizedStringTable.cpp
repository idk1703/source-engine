...
public:
	...
	virtual void ConstructString(wchar_t *unicodeOutput, int unicodeBufferSizeInBytes,
								 StringIndex_t unlocalizedTextSymbol, KeyValues *localizationVariables);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR_WITH_NAMESPACE(CLocalizedStringTable, vgui::, ILocalize,
...
bool CLocalizedStringTable::AddFile(const char *szFileName, const char *pPathID, bool bIncludeFallbackSearchPaths)
{
	...
	if(langptr)
	{
		...
		if(IsPC())
		{
			bValid = vgui::g_pSystem->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Source\\Language",
	...
	{
		// We want to walk them in reverse order so newer files are "overrides" for older ones, so we add them to a list
		// in reverse order
	...
	for(int sp = 0; sp < searchList.Count(); ++sp)
	{
		...
		// this is an optimization so that the filename string doesn't have to get converted to a symbol for each
		// key/value
				...
				else
				{
					...
					if((bEnglishFile) || (!bEnglishFile && strnicmp(key, "[english]", 9)))
					{
						...
						if(!bQuoted && conditional[0] == L'[' &&
						   conditional[1] == L'$') // wcsstr( conditional, L"[$" ) )
						{
							...
							if(pszKey && pszKey[0])
							{
								...
								if(!V_stricmp(pszKey, "ENGLISH") || !V_stricmp(pszKey, "JAPANESE") ||
								   !V_stricmp(pszKey, "GERMAN") || !V_stricmp(pszKey, "FRENCH") ||
								   !V_stricmp(pszKey, "SPANISH") || !V_stricmp(pszKey, "ITALIAN") ||
								   !V_stricmp(pszKey, "KOREAN") || !V_stricmp(pszKey, "TCHINESE") ||
								   !V_stricmp(pszKey, "PORTUGUESE") || !V_stricmp(pszKey, "SCHINESE") ||
								   !V_stricmp(pszKey, "POLISH") || !V_stricmp(pszKey, "RUSSIAN"))
								{
...
void CLocalizedStringTable::ReloadLocalizationFiles()
{
	...
	for(int i = 0; i < m_LocalizationFiles.Count(); i++)
	{
		...
		AddFile(entry.symName.String(), entry.symPathID.String()[0] ? entry.symPathID.String() : NULL,
...
{
	const char *str1 =
		(i1.nameIndex == INVALID_LOCALIZE_STRING_INDEX) ? i1.pszValueString : &g_StringTable.m_Names[i1.nameIndex];
	const char *str2 =
		(i2.nameIndex == INVALID_LOCALIZE_STRING_INDEX) ? i2.pszValueString : &g_StringTable.m_Names[i2.nameIndex];
...
//-----------------------------------------------------------------------------
void CLocalizedStringTable::ConstructString(wchar_t *unicodeOutput, int unicodeBufferSizeInBytes, const char *tokenName,
											KeyValues *localizationVariables)
{
...
// Purpose: Constructs a string, inserting variables where necessary
//-----------------------------------------------------------------------------
void CLocalizedStringTable::ConstructString(wchar_t *unicodeOutput, int unicodeBufferSizeInBytes,
											StringIndex_t unlocalizedTextSymbol, KeyValues *localizationVariables)
{
