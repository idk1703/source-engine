...
void CMDLPanel::SetMDL(const char *pMDLName, void *pProxyData)
{
	...
	// FindMDL takes a reference and the the CMDL will also hold a reference for as long as it sticks around. Release
	// the FindMDL reference.
...
void CMDLPanel::OnPaint3D()
{
	...
	m_RootMDL.m_MDL.SetUpBones(m_RootMDL.m_MDLToWorld, studioHdr.numbones(), pBoneToWorld, m_PoseParameters,
							   m_SequenceLayers, m_nNumSequenceLayers);
	...
	for(int iMerge = 0; iMerge < nMergeCount; ++iMerge)
	{
		...
		if(pStudioHdr != NULL)
		{
			...
			m_aMergeMDLs[iMerge].m_MDL.SetupBonesWithBoneMerge(&mergeHdr, pMergeBoneToWorld, &studioHdr, pBoneToWorld,
															   m_RootMDL.m_MDLToWorld);
...
void CMDLPanel::Paint()
{
	...
	if(m_bThumbnailSafeZone)
	{
		...
		pRenderContext->DrawScreenSpaceRectangle(safezone, screenposx, screenposy, iWidth, iRemappedHeight, 0, 0,
...
void CMDLPanel::DoAnimationEvents()
{
	...
	for(int i = 0; i < m_nNumSequenceLayers; ++i)
	{
		...
		// Plat_DebugString( CFmtStr("Animation: time = %f, started = %f, delta =
		// %f\n",m_RootMDL.m_MDL.m_flTime,m_SequenceLayers[ i ].m_flCycleBeganAt,flTime ) );
		DoAnimationEvents(&studioHdr, m_SequenceLayers[i].m_nSequenceIndex, flTime, m_SequenceLayers[i].m_bNoLoop,
						  &m_SequenceLayerEventState[i]);
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CMDLPanel::DoAnimationEvents(CStudioHdr *pStudioHdr, int nSeqNum, float flTime, bool bNoLoop,
								  MDLAnimEventState_t *pEventState)
{
	...
	// m_RootMDL.m_MDL.m_flPlaybackRate, nFrameCount ) );
	if(bNoLoop)
	{
...
MDLHandle_t CMDLPanel::SetMergeMDL(const char *pMDLName, void *pProxyData, int nSkin /*= -1 */)
{
	...
	int nRef = vgui::MDLCache()->Release(hMDLFindResult);
	(void)nRef; // Avoid unreferenced variable warning
	AssertMsg(hMDL == MDLHANDLE_INVALID || nRef > 0,
			  "CMDLPanel::SetMergeMDL referenced a model that has a zero ref count.");
