...
// Constructor
//-----------------------------------------------------------------------------
CAssetTreeView::CAssetTreeView(Panel *pParent, const char *pName, const char *pRootFolderName, const char *pRootDir)
...
// Map sort func
//-----------------------------------------------------------------------------
bool CAssetCache::CachedAssetLessFunc(const CAssetCache::CachedAssetList_t &src1,
...
//-----------------------------------------------------------------------------
bool CAssetCache::AddFilesInDirectory(CachedAssetList_t &list, const char *pStartingFile, const char *pFilePath,
...
{
	CachedAssetList_t &list = m_CachedAssets[(uintp)hList];
...
CAssetTreeView *CAssetCache::GetFileTree(AssetList_t hList)
{
	if(hList == ASSET_LIST_INVALID)
		return NULL;
	return m_CachedAssets[(uintp)hList].m_pFileTree;
}
...
int CAssetCache::GetAssetCount(AssetList_t hList) const
{
	if(hList == ASSET_LIST_INVALID)
		return 0;
	return m_CachedAssets[(uintp)hList].m_AssetList.Count();
}
...
const CAssetCache::CachedAssetInfo_t &CAssetCache::GetAsset(AssetList_t hList, int nIndex) const
{
	...
	return m_CachedAssets[(uintp)hList].m_AssetList[nIndex];
}
...
static int __cdecl AssetBrowserModSortFunc(vgui::ListPanel *pPanel, const ListPanelItem &item1,
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CBaseAssetPicker::CBaseAssetPicker(vgui::Panel *pParent, const char *pAssetType, const char *pExt, const char *pSubDir,
...
void CBaseAssetPicker::GetUserConfigSettings(KeyValues *pUserConfig)
{
	...
	pUserConfig->SetString("mod",
						   (m_nCurrentModFilter >= 0) ? s_AssetCache.ModInfo(m_nCurrentModFilter).m_ModName : "");
}
...
void CBaseAssetPicker::SetInitialSelection(const char *pAssetName)
{
	...
	if(pAssetName)
	{
		// Sometimes we've already refreshed our list with a bunch of cached resources and the item is already in the
		// list, so in that case just select it here.
...
void CBaseAssetPicker::UpdateAssetColumnHeader()
{
	...
	Q_snprintf(pColumnTitle, sizeof(pColumnTitle), "%s (%d/%d)%s", m_pAssetType, m_nMatchingAssets, m_AssetList.Count(),
...
void CBaseAssetPicker::OnItemSelected(KeyValues *kv)
{
	...
	if(pPanel == m_pAssetBrowser)
	{
		...
		char pBuf[MAX_PATH];
		Q_snprintf(pBuf, sizeof(pBuf), "%s\\%s\\%s", s_AssetCache.ModInfo(nModIndex).m_Path.Get(), m_pAssetSubDir,
				   pSelectedAsset);
		Q_FixSlashes(pBuf);
...
//
//-----------------------------------------------------------------------------
CBaseAssetPickerFrame::CBaseAssetPickerFrame(vgui::Panel *pParent) : BaseClass(pParent, "AssetPickerFrame")
{
