...
void CBrowseButton::InitBrowseInfo(int x, int y, const char *pszName, const char *pszDir, const char *pszFilter,
								   const char *pszField)
{
	SetSize(24, 24);
	SetPos(x, y);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CQCGenerator::CQCGenerator(vgui::Panel *pParent, const char *pszPath, const char *pszScene)
...
{
	// TODO: clean this up.  Consider creating a datatype that includes the string to write out when the QC file is
	// created
	...
	for(int i = 0; i < m_QCInfo_t.LODs.Count(); i++)
	{
		...
		g_pFullFileSystem->FPrintf(pSaveFile, "$lod %d\n{\n\treplacemodel \"%s\" \"%s\"\n}\n\n", thisLOD.iLOD,
								   strrchr(m_QCInfo_t.pszSMDPath, '\\') + 1, thisLOD.pszFilename);
	}
...
void CQCGenerator::InitializeSMDPaths(const char *pszPath, const char *pszScene)
{
	...
	if(!bFoundReference)
	{
		...
		Q_snprintf(pszRefMessage, 1024,
				   ...
				   "default of %s. Model will not compile.\n\n",
