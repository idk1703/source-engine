...
void CPotteryWheelPanel::ParseLightsFromKV(KeyValues *pLightsKV)
{
	...
	FOR_EACH_SUBKEY(pLightsKV, pLocalLight)
	{
		...
		if(!Q_stricmp(pType, "spot"))
		{
			...
			Vector vecOrigin;
			StringToVector(vecOrigin.Base(), pLocalLight->GetString("origin"));
			pDesc->InitSpot(vecOrigin, vecColor, vec3_origin, pLocalLight->GetFloat("inner_cone_angle"),
...
void CPotteryWheelPanel::SetLightProbe(CDmxElement *pLightProbe)
{
	...
	const char *pCubemapHDR =
		pLightProbe->HasAttribute("cubemapHdr") ? pLightProbe->GetValueString("cubemapHdr") : pCubemap;
...
void CPotteryWheelPanel::EnterManipulationMode(ManipulationMode_t manipMode, bool bMouseCapture,
