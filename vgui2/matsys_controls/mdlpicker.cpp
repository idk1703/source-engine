...
static bool SaveTgaAndAddToP4(unsigned char *pImage, ImageFormat imageFormat, int Width, int Height,
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMDLPicker::CMDLPicker(vgui::Panel *pParent, int nFlags) : BaseClass(pParent, "MDL Files", "mdl", "models", "mdlName")
{
...
void CMDLPicker::OnCommand(const char *pCommand)
{
	...
	{
		CAssetPickerFrame *pPicker = new CAssetPickerFrame(this, "Select Light Probe (.prb) File", "Light Probe", "prb",
...
// have been set.
//-----------------------------------------------------------------------------
const char *CMDLPicker::CaptureModel(int nModIndex, const char *AssetName, const char *OutputPath, int Width,
									 int Height, Color BackgroundColor, bool bSelectedOnly)
{
	char pBuf[MAX_PATH];
...
void CMDLPicker::GenerateBackpackIcons(void)
{
	...
	int nModIndex = pItemKeyValues->GetInt("modIndex");
	const char *pLargeTGAName =
		CaptureModel(nModIndex, pLargeAssetName, pOutputPath, width, height, NewPanelColor, true);
	if(!pLargeTGAName)
		return;
	...
	if(!g_pFullFileSystem->ReadFile(pLargeTGAName, NULL, largeTGAFileData) ||
	   !TGALoader::GetInfo(largeTGAFileData, &nCheckWidth, &nCheckHeight, &largeFmt, &gamma) || nCheckWidth != width ||
	   nCheckHeight != height)
	{
	...
	// resampleInfo.m_nFlags |= ImageLoader::RESAMPLE_NICE_FILTER; // Turn this off.  It has some sort of edge
	// enhancement or something.  Causes edges to ring.
	...
	// Save it
	if(!SaveTgaAndAddToP4(resampleInfo.m_pDest, IMAGE_FORMAT_RGBA8888, resampleInfo.m_nDestWidth,
						  resampleInfo.m_nDestHeight, pSmallTGAName))
	{
		return;
	}
...
//
//-----------------------------------------------------------------------------
CMDLPickerFrame::CMDLPickerFrame(vgui::Panel *pParent, const char *pTitle, int nFlags) : BaseClass(pParent)
{
