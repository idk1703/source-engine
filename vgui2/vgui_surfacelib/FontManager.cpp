...
//-----------------------------------------------------------------------------
bool CFontManager::SetFontGlyphSet(HFont font, const char *windowsFontName, int tall, int weight, int blur,
								   ...
								   int scanlines, int flags, int nRangeMin, int nRangeMax)
{
	...
	if(IsX360())
	{
		...
		{
			if(!V_stricmp(m_szLanguage, "polish") || !V_stricmp(m_szLanguage, "russian") ||
			   !V_stricmp(m_szLanguage, "japanese") || !V_stricmp(m_szLanguage, "korean") ||
			   !V_stricmp(m_szLanguage, "portuguese") || !V_stricmp(m_szLanguage, "schinese") ||
...
//-----------------------------------------------------------------------------
font_t *CFontManager::CreateOrFindWin32Font(const char *windowsFontName, int tall, int weight, int blur, int scanlines,
											int flags)
{
	...
	if(!winFont)
	{
		...
		if(pchFontData)
		{
			...
			if(m_Win32Fonts[i]->CreateFromMemory(windowsFontName, pchFontData, memSize, tall, weight, blur, scanlines,
												 flags))
			{
				// add to the list
				winFont = m_Win32Fonts[i];
...
// list of how fonts fallback
FallbackFont_t g_FallbackFonts[] = {
	{"DejaVu Sans", NULL}, {NULL, "DejaVu Sans"}, // every other font falls back to this
};
#elif defined(_PS3)
// list of how fonts fallback
FallbackFont_t g_FallbackFonts[] = {
...
#if defined(_X360)
bool CFontManager::GetCachedXUIMetrics(const char *pFontName, int tall, int style, XUIFontMetrics *pFontMetrics,
									   XUICharMetrics charMetrics[256])
{
	...
	{
		if(m_XUIMetricCache[i].fontSymbol == fontSymbol && m_XUIMetricCache[i].tall == tall &&
...
#if defined(_X360)
void CFontManager::SetCachedXUIMetrics(const char *pFontName, int tall, int style, XUIFontMetrics *pFontMetrics,
									   XUICharMetrics charMetrics[256])
{
	MEM_ALLOC_CREDIT();
...
void CFontManager::GetKernedCharWidth(vgui::HFont font, wchar_t ch, wchar_t chBefore, wchar_t chAfter, float &wide,
									  float &flabcA, float &flabcC)
{
	wide = 0.0f;
	flabcA = 0.0f;
