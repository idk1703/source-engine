...
bool CBitmapFont::Create(const char *pFontFilename, float scalex, float scaley, int flags)
{
	...
	if(!pFontTable)
	{
		...
		if(IsX360())
		{
			...
			swap.SwapFieldsToTargetEndian((BitmapGlyph_t *)((char *)pBuf + sizeof(BitmapFont_t)),
										  ((BitmapFont_t *)pBuf)->m_NumGlyphs);
		}
		...
		memcpy(pFontTable->m_pBitmapGlyphs, (unsigned char *)pBuf + sizeof(BitmapFont_t),
...
void CBitmapFont::GetKernedCharWidth(wchar_t ch, wchar_t chBefore, wchar_t chAfter, float &wide, float &abcA,
									 float &abcC)
{
	Assert(IsValid() && ch >= 0 && ch <= 255);
...
void CBitmapFont::GetCharCoords(int ch, float *left, float *top, float *right, float *bottom)
{
	...
	*right =
		(float)(pFont->m_pBitmapGlyphs[ch].x + pFont->m_pBitmapGlyphs[ch].w) / (float)pFont->m_pBitmapFont->m_PageWidth;
	*bottom = (float)(pFont->m_pBitmapGlyphs[ch].y + pFont->m_pBitmapGlyphs[ch].h) /
...
BEGIN_BYTESWAP_DATADESC(BitmapGlyph_t)
	DEFINE_FIELD(x, FIELD_SHORT), DEFINE_FIELD(y, FIELD_SHORT), DEFINE_FIELD(w, FIELD_SHORT),
		DEFINE_FIELD(h, FIELD_SHORT), DEFINE_FIELD(a, FIELD_SHORT), DEFINE_FIELD(b, FIELD_SHORT),
		DEFINE_FIELD(c, FIELD_SHORT),
END_BYTESWAP_DATADESC()

BEGIN_BYTESWAP_DATADESC(BitmapFont_t)
	DEFINE_FIELD(m_id, FIELD_INTEGER), DEFINE_FIELD(m_Version, FIELD_INTEGER), DEFINE_FIELD(m_PageWidth, FIELD_SHORT),
		DEFINE_FIELD(m_PageHeight, FIELD_SHORT), DEFINE_FIELD(m_MaxCharWidth, FIELD_SHORT),
		DEFINE_FIELD(m_MaxCharHeight, FIELD_SHORT), DEFINE_FIELD(m_Flags, FIELD_SHORT),
		DEFINE_FIELD(m_Ascent, FIELD_SHORT), DEFINE_FIELD(m_NumGlyphs, FIELD_SHORT),
