...
CLocalizationDialog::CLocalizationDialog(const char *fileName) : Frame(NULL, "LocalizationDialog")
{
	...
	while(idx != INVALID_LOCALIZE_STRING_INDEX)
	{
		...
		m_pTokenList->AddItem(new KeyValues("LString", "Token", g_pVGuiLocalize->GetNameByIndex(idx)), idx, false,
							  false);
...
void CLocalizationDialog::OnTokenCreated(const char *tokenName)
{
	...
	int itemId =
		m_pTokenList->AddItem(new KeyValues("LString", "Token", g_pVGuiLocalize->GetNameByIndex(idx)), idx, true, true);
