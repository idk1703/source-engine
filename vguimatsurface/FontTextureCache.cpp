...
//-----------------------------------------------------------------------------
bool CFontTextureCache::GetTextureForChar(vgui::HFont font, vgui::FontDrawType_t type, wchar_t wch, int *textureID,
										  float **texCoords)
{
...
//-----------------------------------------------------------------------------
bool CFontTextureCache::GetTextureForChars(vgui::HFont font, vgui::FontDrawType_t type, const wchar_t *wch,
										   int *textureID, float **texCoords, int numChars)
{
		...
		if(numNewChars > 0)
		{
			...
			if(numNewChars > 1)
			{
				...
				for(int i = 0; i < numNewChars; i++)
				{
					...
					g_MatSystemSurface.DrawSetSubTextureRGBA(m_PageList[newEntry.page].textureID[typePage],
															 newEntry.drawX, newEntry.drawY, characterRGBA,
															 newChar.fontWide, newChar.fontTall);
				}
			}
			else
#endif
			{
				...
				for(int i = 0; i < numNewChars; i++)
				{
					...
					g_MatSystemSurface.DrawSetTexture(m_PageList[newEntry.page].textureID[typePage]);
					g_MatSystemSurface.DrawSetSubTextureRGBA(m_PageList[newEntry.page].textureID[typePage],
															 newEntry.drawX, newEntry.drawY, pRGBA, newChar.fontWide,
															 newChar.fontTall);
				}
			}
		}
...
void CFontTextureCache::CreateFontMaterials(Page_t &page, ITexture *pFontTexture, bool bitmapFont)
{
	...
	{
		g_MatSystemSurface.ReferenceProceduralMaterial(page.textureID[typePageAdditive],
													   page.textureID[typePageNonAdditive], pMaterial);
	}
	pMaterial->DecrementReferenceCount();
}
...
//-----------------------------------------------------------------------------
bool CFontTextureCache::AllocatePageForChar(int charWide, int charTall, int &pageIndex, int &drawX, int &drawY,
											int &twide, int &ttall)
{
	...
	if(bNeedsNewPage)
	{
		...
		ITexture *pTexture = g_pMaterialSystem->CreateProceduralTexture(
			pTextureName, TEXTURE_GROUP_VGUI, newPage.wide, newPage.tall, IMAGE_FORMAT_RGBA8888,
			TEXTUREFLAGS_POINTSAMPLE | TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_NOMIP |
		...
		if(IsPC() || !IsDebug())
		{
			...
			int typePageNonAdditive = (int)(vgui::FONT_DRAW_NONADDITIVE)-1;
			g_MatSystemSurface.DrawSetTextureRGBA(newPage.textureID[typePageNonAdditive], pRGBA, newPage.wide,
