...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CMatSystemSurface, ISurface, VGUI_SURFACE_INTERFACE_VERSION, g_MatSystemSurface);
...
InitReturnVal_t CMatSystemSurface::Init(void)
{
	...
	if(IsPC())
	{
		bValid = system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Source\\Language", language,
											 sizeof(language) - 1);
	}
	else
	{
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawPanelIn3DSpace(vgui::VPANEL pRootPanel, const VMatrix &panelCenterToWorld, int pw, int ph,
										   float sw, float sh)
{
	Assert(pRootPanel);
...
void CMatSystemSurface::StartDrawing(void)
{
	...
	//___stop___();
	pRenderContext->Ortho(g_flPixelOffsetX, g_flPixelOffsetY, width + g_flPixelOffsetX, height + g_flPixelOffsetY,
						  -1.0f, 1.0f);
...
void CMatSystemSurface::SetupPaintState(const PaintState_t &paintState)
{
	...
	SetScissorRect(paintState.m_iScissorLeft, paintState.m_iScissorTop, paintState.m_iScissorRight,
				   paintState.m_iScissorBottom);
}
...
void CMatSystemSurface::DrawPolyLine(int *px, int *py, int n)
{
	...
	for(int i = 0; i < n; i++)
	{
		...
		meshBuilder.Position3f(clippedVerts[0].m_Position.x + g_flPixelOffsetX,
							   clippedVerts[0].m_Position.y + g_flPixelOffsetY, m_flZPos);
		meshBuilder.Color4ubv(m_DrawColor);
		meshBuilder.TexCoord2fv(0, clippedVerts[0].m_TexCoord.Base());
		meshBuilder.AdvanceVertexF<VTX_HAVEPOS | VTX_HAVECOLOR, 1>();

		meshBuilder.Position3f(clippedVerts[1].m_Position.x + g_flPixelOffsetX,
							   clippedVerts[1].m_Position.y + g_flPixelOffsetY, m_flZPos);
		meshBuilder.Color4ubv(m_DrawColor);
		meshBuilder.TexCoord2fv(0, clippedVerts[1].m_TexCoord.Base());
		meshBuilder.AdvanceVertexF<VTX_HAVEPOS | VTX_HAVECOLOR, 1>();
	}
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawFilledRectFastFade(int x0, int y0, int x1, int y1, int fadeStartPt, int fadeEndPt,
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawFilledRectFade(int x0, int y0, int x1, int y1, unsigned int alpha0, unsigned int alpha1,
										   bool bHorizontal)
{
	MAT_FUNC;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawSetTextureFile(int id, const char *pFileName, int hardwareFilter,
...
// Draws a textured rectangle
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawTexturedSubRect(int x0, int y0, int x1, int y1, float texs0, float text0, float texs1,
											float text1)
{
	MAT_FUNC;
...
void CMatSystemSurface::DrawTexturedPolygon(int n, Vertex_t *pVertices, bool bClipVertices /*= true*/)
{
	...
	else
	{
		...
		for(int i = 0; i < n; ++i)
		{
			meshBuilder.Position3f(pVertices[i].m_Position.x + m_nTranslateX, pVertices[i].m_Position.y + m_nTranslateY,
								   m_flZPos);
			meshBuilder.Color4ubv(m_DrawColor);
...
//-----------------------------------------------------------------------------
bool CMatSystemSurface::SetFontGlyphSet(HFont font, const char *windowsFontName, int tall, int weight, int blur,
										int scanlines, int flags, int nRangeMin, int nRangeMax)
{
	return FontManager().SetFontGlyphSet(font, windowsFontName, tall, weight, blur, scanlines, flags, nRangeMin,
										 nRangeMax);
}
...
// Purpose: adds glyphs to a font created by CreateFont()
//-----------------------------------------------------------------------------
bool CMatSystemSurface::SetBitmapFontGlyphSet(HFont font, const char *windowsFontName, float scalex, float scaley,
											  int flags)
{
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetKernedCharWidth(HFont font, wchar_t ch, wchar_t chBefore, wchar_t chAfter, float &wide,
										   float &abcA) //, float &abcC )
{
	float abcC = 0.0f;
...
bool CMatSystemSurface::AddCustomFontFile(const char *fontName, const char *fontFileName)
{
	...
	if(gdiModule)
	{
		...
		AddFontResourceExProc pAddFontResourceEx =
			(AddFontResourceExProc)::GetProcAddress(gdiModule, "AddFontResourceExA");
	...
	if(err == noErr)
		err = ATSFontActivateFromFileReference(&ref, kATSFontContextLocal, kATSFontFormatUnspecified, NULL,
...
void *CMatSystemSurface::FontDataHelper(const char *pchFontName, int &size, const char *fontFileName)
{
	...
	if(fontFileName)
	{
		...
		const FT_Error error =
			FT_New_Memory_Face(FontManager().GetFontLibraryHandle(), (FT_Byte *)buf.Base(), buf.TellPut(), 0, &face);
...
bool CMatSystemSurface::AddBitmapFontFile(const char *fontFileName)
{
	...
	bFound = ((g_pFullFileSystem->GetDVDMode() == DVDMODE_STRICT) ||
			  g_pFullFileSystem->FileExists(fontFileName, IsX360() ? "GAME" : NULL));
	if(!bFound)
	{
...
void CMatSystemSurface::PrecacheFontCharacters(HFont font, const wchar_t *pCharacterString)
{
	...
	g_FontTextureCache.GetTextureForChars(m_hCurrentFont, FONT_DRAW_DEFAULT, pCharacterString, pTextureIDs_ignored,
...
void CMatSystemSurface::DrawSetTextColor(int r, int g, int b, int a)
{
	...
	if(r != m_DrawTextColor[0] || g != m_DrawTextColor[1] || b != m_DrawTextColor[2] ||
	   adjustedAlpha != m_DrawTextColor[3])
	{
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawPrintText(const wchar_t *text, int iTextLen,
...
void CMatSystemSurface::OnScreenSizeChanged(int nOldWidth, int nOldHeight)
{
	...
	ivgui()->PostMessage(panel, new KeyValues("OnScreenSizeChanged", "oldwide", nOldWidth, "oldtall", nOldHeight),
						 NULL);
...
void CMatSystemSurface::SetAsToolBar(VPANEL panel,
									 bool state) // removes the window's task bar entry (for context menu's, etc.)
{
}
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::CreatePopup(VPANEL panel, bool minimized, bool showTaskbarIcon, bool disabled, bool mouseInput,
									bool kbInput)
{
...
void CMatSystemSurface::RestrictPaintToSinglePanel(VPANEL panel)
{
	...
	{
		input()->SetAppModalSurface(panel); // if painting is restricted to this panel, it had better be modal, or else
											// you can get in some bad state...
	}
}
...
void CMatSystemSurface::Begin3DPaint(int iLeft, int iTop, int iRight, int iBottom, bool bRenderToTexture)
{
	...
	if(bRenderToTexture)
	{
		...
		pRenderContext->PushRenderTargetAndViewport(m_FullScreenBuffer, 0, 0, iRight - iLeft, iBottom - iTop);
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetFullScreenTexCoords(int x, int y, int w, int h, float *pMinU, float *pMinV, float *pMaxU,
											   float *pMaxV)
{
...
// Draws text with current font at position and wordwrapped to the rect using color values specified
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawColoredTextRect(vgui::HFont font, int x, int y, int w, int h, int r, int g, int b, int a,
											const char *fmt, ...)
{
	MAT_FUNC;
...
void CMatSystemSurface::DrawSetTextureRGBA(int id, const unsigned char *rgba, int wide, int tall, int hardwareFilter,
										   bool forceUpload)
{
...
void CMatSystemSurface::DrawSetSubTextureRGBA(int textureID, int drawX, int drawY, unsigned const char *rgba,
...
void CMatSystemSurface::DrawUpdateRegionTextureRGBA(int nTextureID, int x, int y, const unsigned char *pchData,
													int wide, int tall, ImageFormat imageFormat)
{
...
#if defined(WIN32) && !defined(_X360)
	SHFILEINFO info = {0};
	DWORD_PTR dwResult = SHGetFileInfo(pFullPath, 0, &info, sizeof(info),
...
void CMatSystemSurface::PushFullscreenViewport()
{
	...
	//___stop___();
	pRenderContext->Ortho(g_flPixelOffsetX, g_flPixelOffsetY, vw + g_flPixelOffsetX, vh + g_flPixelOffsetY, -1.0f,
						  1.0f);
