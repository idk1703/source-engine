...
void UTIL_ComputeBaseDir()
{
	...
	if(!g_szBasedir[0] && GetExecutableName(g_szBasedir, sizeof(g_szBasedir)))
	{
		...
		if(j > 0)
		{
			if((g_szBasedir[j - 1] == '\\') || (g_szBasedir[j - 1] == '/'))
			{
				g_szBasedir[j - 1] = 0;
			}
		}
...
CLogAllFiles::CLogAllFiles() : m_bActive(false), m_Logged(0, 0, AllLogLessFunc)
{
	MEM_ALLOC_CREDIT();
...
void CLogAllFiles::Init()
{
	...
	if(!CommandLine()->FindParm("-startmap") && !CommandLine()->FindParm("-startstage"))
	{
		...
		g_pFullFileSystem->RemoveFile(
			CFmtStr("%s\\%s\\%s", m_sFullGamePath.String(), m_sResListDir.String(), ALL_RESLIST_FILE), "GAME");
	}
...
void CLogAllFiles::LogToAllReslist(char const *line)
{
	...
	FileHandle_t fh = g_pFullFileSystem->Open(
		CFmtStr("%s\\%s\\%s", m_sFullGamePath.String(), m_sResListDir.String(), ALL_RESLIST_FILE), "at", "GAME");
...
static char const *Cmd_TranslateFileAssociation(char const *param)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		if(!Q_strcmp(extension, info.extension) && !CommandLine()->FindParm(va("+%s", info.command_to_issue)))
		{
...
//-----------------------------------------------------------------------------
#ifdef WIN32
extern "C" __declspec(dllexport) int LauncherMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine,
												  int nCmdShow)
#else
...
#endif
{
	...
	else
	{
		...
		if(!GrabSourceMutex())
		{
			...
			// We're going to hijack the existing session and load a new savegame into it. This will mainly occur when
			// users click on links in Bugzilla that will automatically copy saves and load them directly from the web
			// browser. The -hijack command prevents the launcher from objecting that there is already an instance of
			// the game.
			if(CommandLine()->CheckParm("-hijack"))
			{
				...
				{
					::MessageBox(NULL,
								 "The modified entity keyvalues could not be sent to the Source Engine because the "
				...
				else
				{
					...
					if(!::SendMessage(hwndEngine, WM_COPYDATA, 0, (LPARAM)&copyData))
					{
						::MessageBox(
							NULL,
							"The Source Engine was found running, but did not accept the request to load a savegame. "
							"It may be an old version of the engine that does not support this functionality.",
	...
	// Now that the mutex has been released, check HKEY_CURRENT_USER\Software\Valve\Source\Relaunch URL. If there is a
	// URL here, exec it. This supports the capability of immediately re-launching the the game via Steam in a different
	// audio language
	HKEY hKey;
