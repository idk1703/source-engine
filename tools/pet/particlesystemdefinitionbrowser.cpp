...
//-----------------------------------------------------------------------------
static int __cdecl ParticleSystemNameSortFunc(vgui::ListPanel *pPanel, const ListPanelItem &item1,
...
// Constructor
//-----------------------------------------------------------------------------
CParticleSystemDefinitionBrowser::CParticleSystemDefinitionBrowser(CPetDoc *pDoc, vgui::Panel *pParent,
																   const char *pName)
...
//-----------------------------------------------------------------------------
void CParticleSystemDefinitionBrowser::LoadKVSection(CDmeParticleSystemDefinition *pNew, KeyValues *pOverridesKv,
													 ParticleFunctionType_t eType)
{
	// Operator KV
	...
	FOR_EACH_TRUE_SUBKEY(pOperator, pFunctionBlock)
	{
			...
			FOR_EACH_SUBKEY(pFunctionBlock, pAttributeItem)
			{
				...
				if(!pAttribute)
				{
					Warning("Unable to Find Attribute [%s] in Function [%s] in Operator [%s] for Definition [%s]\n",
							pAttributeItem->GetName(), pFunctionBlock->GetName(), GetParticleFunctionTypeName(eType),
							pNew->GetName());
				}
				else
				{
		...
		{
			Warning("Function [%s] not found under Operator [%s] for Definition [%s]\n", pFunctionBlock->GetName(),
					GetParticleFunctionTypeName(eType), pNew->GetName());
		}
	}
}
...
{
	m_pParticleSystemsDefinitions->ClearSelectedItems();
	for(int nItemID = m_pParticleSystemsDefinitions->FirstItem();
		nItemID != m_pParticleSystemsDefinitions->InvalidItemID();
		nItemID = m_pParticleSystemsDefinitions->NextItem(nItemID))
	{
		KeyValues *kv = m_pParticleSystemsDefinitions->GetItem(nItemID);
		CDmeParticleSystemDefinition *pParticleSystem =
			GetElementKeyValue<CDmeParticleSystemDefinition>(kv, "particleSystem");
...
void CParticleSystemDefinitionBrowser::OnInputCompleted(KeyValues *pKeyValues)
{
	...
	if(m_pDoc->IsParticleSystemDefined(pText))
	{
		...
		vgui::MessageBox *pMessageBox =
			new vgui::MessageBox("Duplicate Particle System Name!\n", pBuf, g_pPetTool->GetRootPanel());
		pMessageBox->DoModal();
		return;
	}
	...
	else if(pKeyValues->FindKey("copy"))
	{
		...
		if(nCount)
		{
			CDmeParticleSystemDefinition *pParticleSystem = GetSelectedParticleSystem(0);
			CAppUndoScopeGuard guard(NOTIFY_SETDIRTYFLAG, "Copy Particle System", "Copy Particle System");
			CDmeParticleSystemDefinition *pNew = CastElement<CDmeParticleSystemDefinition>(pParticleSystem->Copy());
			pNew->SetName(pText);
...
void CParticleSystemDefinitionBrowser::ReplaceDef_r(CUndoScopeGuard &guard, CDmeParticleSystemDefinition *pDef)
{
	...
	{
		CDmeParticleChild *pChildFunction =
			static_cast<CDmeParticleChild *>(pDef->GetParticleFunction(FUNCTION_CHILDREN, i));
...
void CParticleSystemDefinitionBrowser::OnCommand(const char *pCommand)
{
	...
	{
		vgui::InputDialog *pInputDialog =
			new vgui::InputDialog(g_pPetTool->GetRootPanel(), "Enter Particle System Name", "Name:", "");
	...
	if(!Q_stricmp(pCommand, "Create From KeyValue"))
	{
		vgui::FileOpenDialog *pDialog =
			new vgui::FileOpenDialog(g_pPetTool->GetRootPanel(), "Select KV File", vgui::FOD_OPEN);
		...
		pDialog->SetStartDirectory(
			g_pFullFileSystem->RelativePathToFullPath("particles", "MOD", szParticlesDir, sizeof(szParticlesDir)));
