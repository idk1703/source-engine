...
public:
	...
	{
		return "ActBusy Script Editor";
	}
	virtual const char *GetBindingsContextFile()
	{
		return "cfg/ActBusy.kb";
	}
	virtual bool Init();
...
void CActBusyTool::InitEditorDict()
{
	...
	CDmeEditorStringChoicesInfo *pChoicesInfo =
		CreateElement<CDmeEditorStringChoicesInfo>("interrupts info", DMFILEID_INVALID);
...
public:
	CActBusyViewMenuButton(CActBusyTool *parent, const char *panelName, const char *text,
...
CActBusyViewMenuButton::CActBusyViewMenuButton(CActBusyTool *parent, const char *panelName, const char *text,
											   vgui::Panel *pActionSignalTarget)
	: BaseClass(parent, panelName, text, pActionSignalTarget)
{
	m_pTool = parent;

	AddCheckableMenuItem("properties", "#ActBusyProperties", new KeyValues("Command", "command", "OnToggleProperties"),
						 pActionSignalTarget);
	AddCheckableMenuItem("picker", "#ActBusyViewSequencePicker",
						 new KeyValues("Command", "command", "OnToggleSequencePicker"), pActionSignalTarget);

	AddSeparator();
	AddMenuItem("defaultlayout", "#ActBusyViewDefault", new KeyValues("Command", "command", "OnDefaultLayout"),
				pActionSignalTarget);
...
CActBusyMenuButton::CActBusyMenuButton(CActBusyTool *parent, const char *panelName, const char *text,
									   vgui::Panel *pActionSignalTarget)
	: BaseClass(parent, panelName, text, pActionSignalTarget)
{
	m_pTool = parent;

	AddMenuItem("NewActBusy", "#ActBusyNewActBusy", new KeyValues("Command", "command", "OnNewActBusy"),
				pActionSignalTarget);
	AddMenuItem("DeleteActBusy", "#ActBusyDeleteActBusy", new KeyValues("Command", "command", "OnDeleteActBusy"),
				pActionSignalTarget);
...
//-----------------------------------------------------------------------------
void CActBusyTool::OnFileOperationCompleted(const char *pFileType, bool bWroteFile,
											vgui::FileOpenStateMachine::CompletionState_t state,
											KeyValues *pContextKeyValues)
{
	if(bWroteFile)
...
//-----------------------------------------------------------------------------
void CActBusyTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog, bool bOpenFile, const char *pFileFormat,
									   KeyValues *pContextKeyValues)
{
...
void CActBusyTool::OnDefaultLayout()
{
	...
	ToolWindow *propertyWindow =
		m_ToolWindowFactory.InstanceToolWindow(GetClientArea(), false, properties, "#ActBusyProperties", false);
	ToolWindow *pickerWindow =
		m_ToolWindowFactory.InstanceToolWindow(GetClientArea(), false, picker, "#ActBusyViewSequencePicker", false);
