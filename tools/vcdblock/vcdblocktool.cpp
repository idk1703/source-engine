...
{
	return (materials != NULL) && (g_pMatSystemSurface != NULL) && (mdlcache != NULL) && (studiorender != NULL) &&
...
public:
	CVcdBlockViewMenuButton(CVcdBlockTool *parent, const char *panelName, const char *text,
...
CVcdBlockViewMenuButton::CVcdBlockViewMenuButton(CVcdBlockTool *parent, const char *panelName, const char *text,
												 vgui::Panel *pActionSignalTarget)
	: BaseClass(parent, panelName, text, pActionSignalTarget)
{
	...
	AddCheckableMenuItem("entityreport", "#VcdBlockEntityReport", new KeyValues("O|�glaEq��pyOeport"),
						 pActionSignalTarget);
...
public:
	CVcdBlockToolMenuButton(CVcdBlockTool *parent, const char *panelName, const char *text,
							vgui::Panel *pActionSignalTarget);
	virtual void OnShowMenu(vgui::Menu *menu);
...
CVcdBlockToolMenuButton::CVcdBlockToolMenuButton(CVcdBlockTool *parent, const char *panelName, const char *text,
												 vgui::Panel *pActionSignalTarget)
	: BaseClass(parent, panelName, text, pActionSignalTarget)
{
	m_pTool = parent;

	AddMenuItem("addnewnodes", "#VcdBlockAddNewNodes", new KeyValues("AddNewNodes"), pActionSignalTarget, NULL,
				"VcdBlockAddNewNodes");
	AddMenuItem("copyeditstovmf", "#VcdBlockCopyEditsToVMF", new KeyValues("CopyEditsToVMF"), pActionSignalTarget, NULL,
				"VcdBlockCopyEditsToVMF");

	AddSeparator();

	AddCheckableMenuItem("rememberposition", "#VcdBlockRememberPosition", new KeyValues("RememberPosition"),
						 pActionSignalTarget, NULL, "VcdBlockRememberPosition");
...
vgui::MenuBar *CVcdBlockTool::CreateMenuBar(CBaseToolSystem *pParent)
{
	...
	CVcdBlockToolMenuButton *pToolButton =
		new CVcdBlockToolMenuButton(this, "VcdBlock", "&VcdBlock", GetActionTarget());
...
void CVcdBlockTool::OnDefaultLayout()
{
	...
	ToolWindow *pPropertyWindow =
		m_ToolWindowFactory.InstanceToolWindow(GetClientArea(), false, properties, "#VcdBlockProperties", false);
	ToolWindow *pEntityReportWindow =
		m_ToolWindowFactory.InstanceToolWindow(GetClientArea(), false, pEntityReport, "#VcdBlockEntityReport", false);
...
void CVcdBlockTool::SaveAndTest()
{
	if(m_pDoc && m_pDoc->IsDirty())
	{
		SaveFile(m_pDoc->GetEditFileName(), "vle", FOSM_SHOW_PERFORCE_DIALOGS, new KeyValues("RestartLevel"));
	}
	else
	{
		OnRestartLevel();
...
//-----------------------------------------------------------------------------
void CVcdBlockTool::OnFileOperationCompleted(const char *pFileType, bool bWroteFile,
...
//-----------------------------------------------------------------------------
void CVcdBlockTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog, bool bOpenFile, const char *pFileFormat,
										KeyValues *pContextKeyValues)
{
