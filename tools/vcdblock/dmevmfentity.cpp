...
void CDmeVMFEntity::OnAttributeChanged(CDmAttribute *pAttribute)
{
	...
	if(pAttribute == m_TargetName.GetAttribute() || pAttribute == m_vecLocalAngles.GetAttribute())
	{
...
{
	m_vecLocalAngles.Set(Vector(
		angles.x, angles.y, angles.z)); // FIXME: angles is a vector due to the vmf "angles" having a problem parsing...
	clienttools->MarkClientRenderableDirty(this);
}
...
void CDmeVMFEntity::DrawFloorTarget(IMaterial *pMaterial)
{
	...
	CBaseTrace tr;
	ray.Init(m_vecLocalOrigin.Get() + Vector(0, 0, 10), m_vecLocalOrigin.Get() + Vector(0, 0, -128),
			 Vector(-13, -13, 0), Vector(13, 13, 10));
	...
	float block[4][2][3] = {{{-13, -13, 0}, {-13, -13, 10}},
							{{13, -13, 0}, {13, -13, 10}},
							{{13, 13, 0}, {13, 13, 10}},
							{{-13, 13, 0}, {-13, 13, 10}}};
