...
//-----------------------------------------------------------------------------
class CVcdBlockTool : public CBaseToolSystem,
					  public IFileMenuCallbacks,
					  public IVcdBlockDocCallback,
					  public IVcdBlockTool
{
...
public:
	...
	void SetRememberPlayerPosition(bool state = true)
	{
		m_bRememberPlayerPosition = state;
	};
	bool GetRememberPlayerPosition(void)
	{
		return m_bRememberPlayerPosition;
	};
	void QuickLoad();
	void QuickSave();

	bool IsInNodeDrag(void)
	{
		return m_bInNodeDragMode;
	};
...
public:
	...
	KEYBINDING_FUNC_NODECLARE(VcdBlockAddNewNodes, KEY_A, vgui::MODIFIER_CONTROL, OnAddNewNodes,
							  "#VcdBlockAddNewNodesHelp", 0);
	...
	CDmeHandle<CDmeVMFEntity> GetCurrentEntity(void)
	{
