	...
	: BaseClass(pParent, "CommentaryPropertiesPanel"), m_pDoc(pDoc)
{
	...
	m_pViewPositionPicker =
		new vgui::Button(m_pCommentaryNode, "ViewPositionPickerButton", "", this, "PickViewPosition");
...
{
	CSoundPickerFrame *pSoundPickerDialog = new CSoundPickerFrame(
		g_pCommEditTool->GetRootPanel(), "Select commentary audio file", CSoundPicker::PICK_WAVFILES);
...
void CCommentaryPropertiesPanel::PickInfoTarget(vgui::TextEntry *pControl)
{
	...
	CPickerFrame *pInfoTargetPickerDialog =
		new CPickerFrame(g_pCommEditTool->GetRootPanel(), "Select Target", "InfoTarget", NULL);
...
void CCommentaryPropertiesPanel::OnFileSelected(KeyValues *pKeyValues)
{
	...
	{
		char pBuf[1024];
		Q_snprintf(pBuf, sizeof(pBuf), "File %s already exists!\nRecording audio will not overwrite existing files.",
				   pFileName);
