...
CBasePropertiesContainer::CBasePropertiesContainer(vgui::Panel *parent, IDmNotify *pNotify,
...
bool CBasePropertiesContainer::IsDroppable(CUtlVector<KeyValues *> &msglist)
{
	...
	CDmElement *ptr = reinterpret_cast<CDmElement *>(
		g_pDataModel->GetElement(DmElementHandle_t(data->GetInt("dmeelement", DMELEMENT_HANDLE_INVALID))));
	if(!ptr)
		return false;
