...
//-----------------------------------------------------------------------------
CToolMenuButton *CreateToolFileMenuButton(vgui::Panel *parent, const char *panelName, const char *text,
										  vgui::Panel *pActionTarget, IFileMenuCallbacks *pCallbacks)
{
...
// Constructor
//-----------------------------------------------------------------------------
CToolFileMenuButton::CToolFileMenuButton(vgui::Panel *pParent, const char *panelName, const char *text,
										 vgui::Panel *pActionSignalTarget, IFileMenuCallbacks *pFileMenuCallback)
	: BaseClass(pParent, panelName, text, pActionSignalTarget), m_pFileMenuCallback(pFileMenuCallback)
{
	...
	if(p4)
	{
		...
		m_nPerforceAdd =
			m_pPerforce->AddMenuItem("perforce_add", "#ToolPerforceAdd", new KeyValues("OnPerforceAdd"), this);
		m_nPerforceOpen =
			m_pPerforce->AddMenuItem("perforce_open", "#ToolPerforceOpen", new KeyValues("OnPerforceOpen"), this);
		m_nPerforceRevert =
			m_pPerforce->AddMenuItem("perforce_revert", "#ToolPerforceRevert", new KeyValues("OnPerforceRevert"), this);
		m_nPerforceSubmit =
			m_pPerforce->AddMenuItem("perforce_submit", "#ToolPerforceSubmit", new KeyValues("OnPerforceSubmit"), this);
		m_pPerforce->AddSeparator();
		m_nPerforceP4Win =
			m_pPerforce->AddMenuItem("perforce_p4win", "#ToolPerforceP4Win", new KeyValues("OnPerforceP4Win"), this);
		m_nPerforceListOpenFiles = m_pPerforce->AddMenuItem("perforce_listopenfiles", "#ToolPerforceListOpenFiles",
...
void CToolFileMenuButton::OnPerforceAdd()
{
	...
	if(m_pFileMenuCallback->GetPerforceFileName(pPerforceFile, sizeof(pPerforceFile)))
	{
		CPerforceFileListFrame *pPerforceFrame = new CPerforceFileListFrame(
			m_pFileMenuCallback->GetRootPanel(), "Add Movie File to Perforce?", "Movie File", PERFORCE_ACTION_FILE_ADD);
...
void CToolFileMenuButton::OnPerforceOpen()
{
	...
	if(m_pFileMenuCallback->GetPerforceFileName(pPerforceFile, sizeof(pPerforceFile)))
	{
		CPerforceFileListFrame *pPerforceFrame =
			new CPerforceFileListFrame(m_pFileMenuCallback->GetRootPanel(), "Check Out Movie File from Perforce?",
...
void CToolFileMenuButton::OnPerforceRevert()
{
	...
	{
		CPerforceFileListFrame *pPerforceFrame =
			new CPerforceFileListFrame(m_pFileMenuCallback->GetRootPanel(), "Revert Movie File Changes from Perforce?",
...
void CToolFileMenuButton::OnPerforceSubmit()
{
	...
	{
		CPerforceFileListFrame *pPerforceFrame =
			new CPerforceFileListFrame(m_pFileMenuCallback->GetRootPanel(), "Submit Movie File Changes to Perforce?",
...
void CToolFileMenuButton::OnPerforceListOpenFiles()
{
	...
	COperationFileListFrame *pOpenedFiles = new COperationFileListFrame(
		m_pFileMenuCallback->GetRootPanel(), "Opened Files In Perforce", "File Name", false, true);
