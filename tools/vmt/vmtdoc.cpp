...
bool CVMTDoc::CreateRootElement()
{
	...
	for(int i = 0; g_pStandardParams[i].m_pParamName; ++i)
	{
		...
		{
			m_pCallback->AddToolParameter(g_pStandardParams[i].m_pParamName, g_pStandardParams[i].m_pWidgetType,
		...
		else if(g_pStandardParams[i].m_pWidgetType || g_pStandardParams[i].m_pTextType)
		{
			...
			m_pCallback->AddShaderParameter(g_pStandardParams[i].m_pParamName, g_pStandardParams[i].m_pWidgetType,
											g_pStandardParams[i].m_pTextType);
		}
	}
...
//-----------------------------------------------------------------------------
CDmAttribute *CVMTDoc::AddAttributeForShaderParameter(CDmElement *pMaterial, const char *pParamName,
...
bool CVMTDoc::SetVMatrixParamValue(CDmAttribute *pAttribute, const char *pValue)
{
	...
	VMatrix mat;
	int count =
		sscanf(pValue, " [ %f %f %f %f  %f %f %f %f  %f %f %f %f  %f %f %f %f ]", &mat.m[0][0], &mat.m[0][1],
	...
	count = sscanf(pValue, " center %f %f scale %f %f rotate %f translate %f %f", &center.x, &center.y, &scale.x,
...
//-----------------------------------------------------------------------------
void CVMTDoc::AddNewShaderParam(CDmElement *pMaterial, const char *pParamName, ShaderParamType_t paramType,
								const char *pValue)
{
	char temp[512];
