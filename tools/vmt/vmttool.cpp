...
void CVMTTool::AddShaderParameter(const char *pParam, const char *pWidget, const char *pTextType)
{
	...
	CDmeEditorAttributeInfo *pInfo =
		CreateElement<CDmeEditorAttributeInfo>("shader param info", m_pVMTType->GetFileId());
...
public:
	CVMTMaterialMenuButton(vgui::Panel *parent, const char *panelName, const char *text,
						   vgui::Panel *pActionSignalTarget);
	virtual void OnShowMenu(vgui::Menu *menu);
};

CVMTMaterialMenuButton::CVMTMaterialMenuButton(vgui::Panel *parent, const char *panelName, const char *text,
											   vgui::Panel *pActionSignalTarget)
...
CVMTViewMenuButton::CVMTViewMenuButton(vgui::Panel *parent, const char *panelName, const char *text,
									   vgui::Panel *pActionSignalTarget)
	: BaseClass(parent, panelName, text, pActionSignalTarget)
{
	...
	AddCheckableMenuItem("showtoolparams", "#VMTShowToolParams", new KeyValues("OnShowToolParams"),
						 pActionSignalTarget);
...
vgui::MenuBar *CVMTTool::CreateMenuBar(CBaseToolSystem *pParent)
{
	...
	CToolMenuButton *pMaterialButton =
		new CVMTMaterialMenuButton(m_pMenuBar, "Material", "&Material", GetActionTarget());
...
//-----------------------------------------------------------------------------
void CVMTTool::OnFileOperationCompleted(const char *pFileType, bool bWroteFile,
										vgui::FileOpenStateMachine::CompletionState_t state,
										KeyValues *pContextKeyValues)
{
	if(bWroteFile)
...
//-----------------------------------------------------------------------------
void CVMTTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog, bool bOpenFile, const char *pFileFormat,
								   KeyValues *pContextKeyValues)
{
...
void CVMTTool::OnDefaultLayout()
{
	...
	ToolWindow *pPropertyWindow =
		m_ToolWindowFactory.InstanceToolWindow(GetClientArea(), false, properties, "#VMTProperties", false);
	ToolWindow *pVMTPreviewWindow =
		m_ToolWindowFactory.InstanceToolWindow(GetClientArea(), false, pVMTPreview, "#VMTPreview", false);
