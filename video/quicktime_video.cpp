...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CQuickTimeVideoSubSystem, IVideoSubSystem, VIDEO_SUBSYSTEM_INTERFACE_VERSION,
								  g_QuickTimeSystem);
...
ConVar QuickTime_EncodeGamma("video_quicktime_encode_gamma", "3", FCVAR_ARCHIVE,
							 "QuickTime Video Encode Gamma Target- 0=no gamma adjust  1=platform default gamma  2 = "
							 "gamma 1.8  3 = gamma 2.2  4 = gamma 2.5",
							 true, 0.0f, true, 4.0f);
ConVar QuickTime_PlaybackGamma("video_quicktime_decode_gamma", "0", FCVAR_ARCHIVE,
							   "QuickTime Video Playback Gamma Target- 0=no gamma adjust  1=platform default gamma  2 "
							   "= gamma 1.8  3 = gamma 2.2  4 = gamma 2.5",
							   true, 0.0f, true, 4.0f);
...
const VideoSystemFeature_t CQuickTimeVideoSubSystem::DEFAULT_FEATURE_SET =
	VideoSystemFeature::FULL_PLAYBACK | VideoSystemFeature::FULL_ENCODE;
...
void *CQuickTimeVideoSubSystem::QueryInterface(const char *pInterfaceName)
{
	...
	{
		if(V_strncmp(pInterfaceName, VIDEO_SUBSYSTEM_INTERFACE_VERSION,
					 Q_strlen(VIDEO_SUBSYSTEM_INTERFACE_VERSION) + 1) == STRINGS_MATCH)
		{
...
{
	m_AvailableFeatures =
		DEFAULT_FEATURE_SET; // Put here because of issue with static const int, binary OR and DEBUG builds
	...
	if(!GetGWorldPixMap || !GetPixBaseAddr || !LockPixels || !UnlockPixels || !DisposeGWorld || !SetGWorld ||
	   !GetPixRowBytes)
		return false;
#endif
...
VideoResult_t CQuickTimeVideoSubSystem::VideoSoundDeviceCMD(VideoSoundDeviceOperation_t operation, void *pDevice,
															void *pData)
{
	switch(operation)
	{
...
{
	return (num < 0 || num >= s_QuickTimeExtensionCount) ? VideoSystemFeature::NO_FEATURES
														 : s_QuickTimeExtensions[num].m_VideoFeatures;
}

// ===========================================================================
// IVideoSubSystem Video Playback and Recording Services
// ===========================================================================
VideoResult_t CQuickTimeVideoSubSystem::PlayVideoFileFullScreen(const char *filename, void *mainWindow, int windowWidth,
																int windowHeight, int desktopWidth, int desktopHeight,
																...
																VideoPlaybackFlags_t playbackFlags)
{
	...
	status = QTNewDataReferenceFromFullPathCFString(imageStrRef, (QTPathStyle)kQTNativeDefaultPathStyle, 0,
													&MovieFileDataRef, &MovieFileDataRefType);
	...
	// on mac OSX, if we are fullscreen, quicktime is bypassing our targets and going to the display directly, so use
	// its dimensions
	...
	if(!m_pCommonServices->CalculateVideoDimensions(nNewWidth, nNewHeight, displayWidth, displayHeight, playbackFlags,
													&nBufferWidth, &nBufferHeight, &displayXOffset, &displayYOffset))
	{
#if defined(OSX)
		SetSystemUIMode(oldMode, oldOptions);
#endif
	...
	if(SetResult(m_pCommonServices->InitFullScreenPlaybackInputHandler(playbackFlags, forcedMinTime, windowed)) !=
	   VideoResult::SUCCESS)
	{
#if defined(OSX)
		SetSystemUIMode(oldMode, oldOptions);
	...
	while(true)
	{
		...
		if(now >= theQTMovieDuration)
		{
			...
			{
				Assert(ANY_BITFLAGS_SET(playbackFlags,
										VideoPlaybackFlags::ABORT_ON_ESC | VideoPlaybackFlags::ABORT_ON_RETURN |
											VideoPlaybackFlags::ABORT_ON_SPACE |
											VideoPlaybackFlags::ABORT_ON_ANY_KEY)); // Movie will loop forever otherwise
				StopMovie(theQTMovie);
...
// ===========================================================================
IVideoMaterial *CQuickTimeVideoSubSystem::CreateVideoMaterial(const char *pMaterialName, const char *pVideoFileName,
															  VideoPlaybackFlags_t flags)
{
	SetResult(VideoResult::BAD_INPUT_PARAMETERS);
	AssertExitN(m_CurrentStatus == VideoSystemStatus::OK && IS_NOT_EMPTY(pMaterialName) ||
				IS_NOT_EMPTY(pVideoFileName));
...
#pragma warning(disable : 4551)
	...
	if(!CompressImage)
	{
		m_AvailableFeatures = m_AvailableFeatures &
							  ~(VideoSystemFeature::ENCODE_VIDEO_TO_FILE | VideoSystemFeature::ENCODE_AUDIO_TO_FILE);
	}
...
bool MovieGetStaticFrameRate(Movie inMovie, VideoFrameRate_t &theFrameRate)
{
	...
	Track videoTrack = GetMovieIndTrackType(inMovie, 1, kCharacteristicHasVideoFrameRate,
											movieTrackCharacteristic | movieTrackEnabledOnly); // get first video track
	...
	if(isMPEG) // working with MPEG-1/MPEG-2 media
	{
		...
		if(MediaGetPublicInfo(movieMediaHandler, kMHInfoEncodedFrameRate, &encodedFrameRate, &encodedFrameRateSize) !=
		   noErr)
			goto error_out;
		...
		theFrameRate.SetRaw(MovieTimeScale,
							int((double)MovieTimeScale / Fix2X(encodedFrameRate.encodedFrameRate) + 0.5));
	}
...
bool MediaGetStaticFrameRate(Media inMovieMedia, VideoFrameRate_t &theFrameRate, bool AssumeConstantIntervals)
{
	...
	GetMediaNextInterestingDisplayTime(inMovieMedia, nextTimeMediaSample | nextTimeEdgeOK, (TimeValue64)0, fixed1,
	...
	GetMediaNextInterestingDisplayTime(inMovieMedia, nextTimeMediaSample | nextTimeEdgeOK,
									   sample_time + sample_duration, fixed1, &sample_time2, &sample_duration2);
...
//  correction
//
//   kQTUsePlatformDefaultGammaLevel = 0,  /* When decompressing into this PixMap, gamma-correct to the platform's
//   standard gamma. */ kQTUseSourceGammaLevel        = -1L,  /* When decompressing into this PixMap, don't perform
//   gamma-correction. */ kQTCCIR601VideoGammaLevel     = 0x00023333 /* 2.2, standard television video gamma.*/ Fixed
//   cGamma1_8 = 0x0001CCCC;		// Gamma 1.8 Fixed cGamma2_5 = 0x00028000;      // Gamma 2.5
// ============================================================================
...
// ============================================================================
bool CreateMovieAudioContext(bool enableAudio, Movie inMovie, QTAudioContextRef *pAudioContext, bool setVolume,
							 float *pCurrentVolume)
{
...
#if defined(WIN32)
		WCHAR strGUID[39];
		int numBytes = StringFromGUID2(DSDEVID_DefaultPlayback, (LPOLESTR)strGUID,
									   39); // CLSID_DirectSound is not what you want here

		// create the audio context
		CFStringRef deviceNameStrRef =
			CFStringCreateWithCharacters(kCFAllocatorDefault, (const UniChar *)strGUID, (CFIndex)(numBytes - 1));
