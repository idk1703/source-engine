...
#include <dlfcn.h>
typedef PixMapHandle (*PFNGetGWorldPixMap)(GWorldPtr offscreenGWorld);
typedef Ptr (*PFNGetPixBaseAddr)(PixMapHandle pm);
typedef Boolean (*PFNLockPixels)(PixMapHandle pm);
typedef void (*PFNUnlockPixels)(PixMapHandle pm);
typedef void (*PFNDisposeGWorld)(GWorldPtr offscreenGWorld);
typedef void (*PFNSetGWorld)(CGrafPtr port, GDHandle gdh);
typedef SInt32 (*PFNGetPixRowBytes)(PixMapHandle pm);
...
#define SAFE_RELEASE_AUDIOCONTEXT(_cxt) \
	if(_cxt != nullptr)                 \
	{                                   \
		QTAudioContextRelease(_cxt);    \
		_cxt = nullptr;                 \
	}
#define SAFE_RELEASE_CFREF(_ref) \
	if(_ref != nullptr)          \
	{                            \
		CFRelease(_ref);         \
		_ref = nullptr;          \
...
bool CreateMovieAudioContext(bool enableAudio, Movie inMovie, QTAudioContextRef *pAudioConectext,
							 bool setVolume = false, float *pCurrentVolume = nullptr);
