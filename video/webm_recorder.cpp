...
static WebMEncodingDataRateInfo_t s_WebMEncodeRates[] = {
	{320, 240, 1000, 1000, 128, 128}, {640, 960, 2000, 2000, 128, 128},	 {960, 640, 2000, 2000, 128, 128},
	{720, 480, 2500, 2500, 128, 128}, {1280, 720, 5000, 5000, 384, 384}, {1920, 1080, 8000, 8000, 384, 384},
};
...
bool CWebMVideoRecorder::SetMovieVideoParameters(VideoEncodeCodec_t theCodec, int videoQuality, int movieFrameWidth,
												 ...
												 VideoEncodeGamma_t gamma)
{
	...
	// add the video track
	m_vid_track =
		m_mkvMuxerSegment.AddVideoTrack(static_cast<int>(m_vpxConfig.g_w), static_cast<int>(m_vpxConfig.g_h), 1);

	mkvmuxer::VideoTrack *const video =
		static_cast<mkvmuxer::VideoTrack *>(m_mkvMuxerSegment.GetTrackByNumber(m_vid_track));
...
bool CWebMVideoRecorder::SetMovieSourceImageParameters(VideoEncodeSourceFormat_t srcImageFormat, int imgWidth,
													   int imgHeight)
{
#ifdef LOG_ENCODER_OPERATIONS
...
bool CWebMVideoRecorder::SetMovieSourceAudioParameters(AudioEncodeSourceFormat_t srcAudioFormat, int audioSampleRate,
													   AudioEncodeOptions_t audioOptions, int audioSampleGroupSize)
{
	...
	m_aud_track =
		m_mkvMuxerSegment.AddAudioTrack(static_cast<int>(m_audioSampleRate), static_cast<int>(m_audioChannels), 0);
	if(!m_aud_track)
	{
	...
	mkvmuxer::AudioTrack *const audio =
		static_cast<mkvmuxer::AudioTrack *>(m_mkvMuxerSegment.GetTrackByNumber(m_aud_track));
	if(!audio)
	{
		SetResult(VideoResult::BAD_INPUT_PARAMETERS);
		return false;
...
void CWebMVideoRecorder::ConvertBGRAToYV12(void *pFrameBuffer, int nStrideAdjustBytes,
										   vpx_image_t *m_SrcImageYV12Buffer, bool fIncludesAlpha)
{
...
// If you are trying to debug how the webm libraries work it's very difficult to use the full TF2 replay system as a
// testbed so this section of code here and in the AppendAudioSamples writes out the data TF2 would send to the video
// recorder to plain files on the disc.  You can then use those files with an extracted framework to work with the webm
// libraries
#ifdef NEVER
	{
		FILE *fp = NULL;
...
bool CWebMVideoRecorder::FlushAudioSamples()
{
	...
	while(vorbis_analysis_blockout(&m_vd, &m_vb) == 1)
	{
		...
		while((status_analysis = vorbis_bitrate_flushpacket(&m_vd, &op)) > 0)
		{
			...
			if(!m_mkvMuxerSegment.AddFrame(op.packet, op.bytes, m_aud_track, time_ns, true))
			{
				return false;
			}
		}
...
bool CWebMVideoRecorder::FinishMovie(bool SaveMovieToDisk)
{
	...
	Msg("Total frames written: %d Time for movie in nanoseconds: %lu\n", m_nFramesAdded,
		((unsigned long)m_nFramesAdded * m_FrameDuration));
#endif
...
bool CWebMVideoRecorder::EstimateMovieFileSize(size_t *pEstSize, int movieWidth, int movieHeight,
											   ...
											   AudioEncodeSourceFormat_t srcAudioFormat, int audioSampleRate)
{
	...
	*pEstSize = (size_t)((fVidRate * 1000 * movieDurationInSeconds / 8) +
						 (fAudRate * 1000 * movieDuration * movieDurationInSeconds / 8));
...
float CWebMVideoRecorder::GetVideoDataRate(int quality, int width, int height)
{
	...
	for(int i = 0; i < ARRAYSIZE(s_WebMEncodeRates); i++)
	{
		...
		{
			return s_WebMEncodeRates[i].m_MinDataRate +
				   (((s_WebMEncodeRates[i].m_MaxDataRate - s_WebMEncodeRates[i].m_MinDataRate) * (float)quality) /
					100.0);
		}
	}
...
float CWebMVideoRecorder::GetAudioDataRate(int quality, int width, int height)
{
	...
	for(int i = 0; i < ARRAYSIZE(s_WebMEncodeRates); i++)
	{
		if(s_WebMEncodeRates[i].m_XResolution == width && s_WebMEncodeRates[i].m_YResolution == height)
		{
			return s_WebMEncodeRates[i].m_MinAudioDataRate +
				   (((s_WebMEncodeRates[i].m_MaxAudioDataRate - s_WebMEncodeRates[i].m_MinAudioDataRate) *
					 (float)quality) /
					100.0);
		}
	}
