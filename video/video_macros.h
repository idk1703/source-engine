...
#define AssertExit(_exp) \
	if(!(_exp))          \
	return
#define AssertExitV(_exp, _rval) \
	if(!(_exp))                  \
	return _rval
#define AssertExitF(_exp) \
	if(!(_exp))           \
	return false
#define AssertExitN(_exp) \
	if(!(_exp))           \
	return nullptr
#define AssertExitFunc(_exp, _func) \
	if(!(_exp))                     \
	{                               \
		_func;                      \
		return;                     \
...
#define AssertPtrExit(_exp) \
	if((_exp) == nullptr)   \
	return
#define AssertPtrExitV(_exp, _rval) \
	if((_exp) == nullptr)           \
	return _rval
#define AssertPtrExitF(_exp) \
	if((_exp) == nullptr)    \
	return false
#define AssertPtrExitN(_exp) \
	if((_exp) == nullptr)    \
	return nullptr

#define AssertInRangeExit(_exp, low, high)  \
	if((_exp) <= (low) || (_exp) >= (high)) \
	return
#define AssertInRangeExitV(_exp, low, high, _rval) \
	if((_exp) <= (low) || (_exp) >= (high))        \
	return _rval
#define AssertInRangeExitF(_exp, low, high) \
	if((_exp) <= (low) || (_exp) >= (high)) \
	return false
#define AssertInRangeExitN(_exp, low, high) \
	if((_exp) <= (low) || (_exp) >= (high)) \
	return nullptr
