...
{
	ThreadProcInfo_t(ThreadFunc_t pfnThread, void *pParam) : pfnThread(pfnThread), pParam(pParam) {}
...
ThreadHandle_t CreateSimpleThread(ThreadFunc_t pfnThread, void *pParam, ThreadId_t *pID, unsigned stackSize)
{
	...
	if(!pID)
		pID = &idIgnored;
	HANDLE h = VCRHook_CreateThread(NULL, stackSize, (LPTHREAD_START_ROUTINE)ThreadProcConvert,
									new ThreadProcInfo_t(pfnThread, pParam), CREATE_SUSPENDED, pID);
	...
	if(ret)
	{
		// There are only PTHREAD_THREADS_MAX number of threads, and we're probably leaking handles if ret == EAGAIN
		// here?
		Error(
	...
	if(pID)
		*pID = (ThreadId_t)tid;
	Plat_ApplyHardwareDataBreakpointsToNewThread((ThreadId_t)tid);
...
#define MS_VC_EXCEPTION 0x406d1388
	...
	static pthread_setname_np_func *s_pthread_setname_np_func =
		(pthread_setname_np_func *)dlsym(RTLD_DEFAULT, "pthread_setname_np");
...
extern "C" unsigned char _InterlockedCompareExchange128(int64 volatile *Destination, int64 ExchangeHigh,
														int64 ExchangeLow, int64 *ComparandResult);
#endif
...
static CDynamicFunction<TryEnterCriticalSectionFunc_t> DynTryEnterCriticalSection("Kernel32.dll",
...
CThread::~CThread()
{
...
#endif
	{
		if(IsAlive())
		{
			Msg("Illegal termination of worker thread! Threads must negotiate an end to the thread before the CThread "
				"object is destroyed.\n");
#ifdef _WIN32

			DoNewAssertDialog(__FILE__, __LINE__,
							  "Illegal termination of worker thread! Threads must negotiate an end to the thread "
							  "before the CThread object is destroyed.\n");
#endif
			if(GetCurrentCThread() == this)
			{
				Stop(); // BUGBUG: Alfred - this doesn't make sense, this destructor fires from the hosting thread not
						// the thread itself!!
			}
		}
...
#elif defined(POSIX)
		_snprintf(m_szName, sizeof(m_szName) - 1, "Thread(0x%x/0x%x)", (uintp)this, (uint)m_threadId);
#endif
...
bool CThread::Start(unsigned nBytesStack)
{
...
#ifdef _WIN32
	HANDLE hThread;
	m_hThread = hThread = (HANDLE)VCRHook_CreateThread(NULL, nBytesStack, (LPTHREAD_START_ROUTINE)GetThreadProc(),
													   new ThreadInit_t(init), CREATE_SUSPENDED, &m_threadId);
	if(!hThread)
	{
	...
	//  A thread's stack size is fixed at the time of thread creation. Only the main thread can dynamically grow its
	//  stack.
...
#endif
		result =
			(*pfnWait)((sizeof(waits) / sizeof(waits[0])), waits, false, (timeout != TT_INFINITE) ? timeout : 30000);
