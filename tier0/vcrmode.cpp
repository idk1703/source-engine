...
public:
	...
	bool
		m_bEnabled; // By default, this is true, but it can be set to false to temporarily disable a thread's VCR usage.
};
CVCRThreadInfo *g_pVCRThreads =
	NULL; // This gets allocated to MAX_VCR_THREADS size if we're doing any VCR recording or playback.
int g_nVCRThreads = 0;
...
static long VCR_Hook_RegSetValueEx(void *hKey, tchar const *lpValueName, unsigned long Reserved, unsigned long dwType,
...
static long VCR_Hook_RegQueryValueEx(void *hKey, tchar const *lpValueName, unsigned long *lpReserved,
									...
									void *phkResult, unsigned long *lpdwDisposition)
{
	// Preamble.
	if(!IsVCRModeEnabledForThisThread())
		return RegCreateKeyEx((HKEY)hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired,
							  (LPSECURITY_ATTRIBUTES)lpSecurityAttributes, (HKEY *)phkResult, lpdwDisposition);
	...
	{
		ret = RegCreateKeyEx((HKEY)hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired,
							 (LPSECURITY_ATTRIBUTES)lpSecurityAttributes, (HKEY *)phkResult, lpdwDisposition);
...
int VCR_GenericPlaybackInternal(const char *pEventName, void *pOutData, int maxLen, bool bForceSameLen,
					   ...
					   unsigned long dwCreationFlags, unsigned long *lpThreadID)
{
	...
	if(!IsVCRModeEnabledForThisThread())
	{
		if(g_VCRMode == VCR_Disabled)
		{
			HANDLE hThread = (void *)_beginthreadex((LPSECURITY_ATTRIBUTES)lpThreadAttributes, dwStackSize,
...
unsigned long VCR_WaitForSingleObject(void *handle, unsigned long dwMilliseconds)
{
	...
	else
	{
		...
		if(val == 1)
		{
			...
			// If it wrote 1, then we know that this call has to signal the object, so just wait until it gets
			// signalled.
