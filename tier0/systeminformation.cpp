...
{
	CSysCallCacheEntry_LoadModule(_TCHAR *pszModule, char *pszFunction)
	{
		InitializeLoadModule(pszModule, pszFunction);
	}
};
struct CSysCallCacheEntry_FindModule : public CSysCallCacheEntry
{
	CSysCallCacheEntry_FindModule(_TCHAR *pszModule, char *pszFunction)
	{
		InitializeFindModule(pszModule, pszFunction);
	}
};
struct CSysCallCacheEntry_FindProc : public CSysCallCacheEntry
{
	CSysCallCacheEntry_FindProc(HMODULE hModule, char *pszFunction)
	{
...
CSysCallCacheEntry::CSysCallCacheEntry()
	: m_eResult(SYSCALL_SUCCESS), m_pfnSysCall(NULL), m_hModule(NULL), m_bInitialized(false), m_bFreeModule(false)
{
}
...
#define MAKEVER(high, low) DWORD(MAKELONG(low, high))
		...
		if(wow64.CallResult() == SYSCALL_SUCCESS)
		{
			...
			BOOL b64 = FALSE;
			if((wow64.GetFunction<PFNWOW64>())(GetCurrentProcess(), &b64) && b64)
			{
				qsi.SetFailed(SYSCALL_UNSUPPORTED);
			}
		}
	...
	PrivateType(NTSTATUS) lResult = (qsi.GetFunction<PrivateType(NtQuerySystemInformation)>())(
