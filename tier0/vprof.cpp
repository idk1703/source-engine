...
//-------------------------------------
static void DumpSorted(CVProfile::StreamOut_t outputStream, const tchar *pszHeading, double totalTime,
					   bool (*pfnSort)(const TimeSums_t &, const TimeSums_t &), int maxLen = 999999)
{
	...
	outputStream(_T("%s\n"), pszHeading);
	outputStream(_T("  Scope                                                      Calls Calls/Frame  Time+Child    ")
				 _T("Pct        Time    Pct   Avg/Frame    Avg/Call Avg-NoChild        Peak\n"));
	outputStream(_T("  ---------------------------------------------------- ----------- ----------- ----------- ")
				 _T("------ ----------- ------ ----------- ----------- ----------- -----------\n"));
	for(i = 0; i < sortedSums.size() && i < (unsigned)maxLen; i++)
	{
		...
		outputStream(_T("  %52.52s%12d%12.3f%12.3f%7.2f%12.3f%7.2f%12.3f%12.3f%12.3f%12.3f\n"),
					 ...
					 sortedSums[i].time / (float)g_TotalFrames, avg, avgLessChildren, sortedSums[i].peak);
	}
}
...
void CVProfile::OutputReport(int type, const tchar *pszStartNode, int budgetGroupID)
{
...
#ifdef _MSC_VER
#if(_MSC_VER < 1300)
	m_pOutputStream(_T("  (note: this report exceeds the output capacity of MSVC debug window. Use console window or ")
					_T("console log.) \n"));
#endif
#endif
	...
	else
	{
		...
		{
			DumpSorted(m_pOutputStream, _T("-- Profile scopes sorted by time (including children) --"),
					   GetTotalTimeSampled(), TimeCompare, maxLen);
			m_pOutputStream(_T("\n"));
		}
		if(type & VPRT_LIST_BY_TIME_LESS_CHILDREN)
		{
			DumpSorted(m_pOutputStream, _T("-- Profile scopes sorted by time (without children) --"),
					   GetTotalTimeSampled(), TimeLessChildrenCompare, maxLen);
			m_pOutputStream(_T("\n"));
		}
		if(type & VPRT_LIST_BY_AVG_TIME)
		{
			DumpSorted(m_pOutputStream, _T("-- Profile scopes sorted by average time (including children) --"),
					   GetTotalTimeSampled(), AverageTimeCompare, maxLen);
			m_pOutputStream(_T("\n"));
		}
		if(type & VPRT_LIST_BY_AVG_TIME_LESS_CHILDREN)
		{
			DumpSorted(m_pOutputStream, _T("-- Profile scopes sorted by average time (without children) --"),
					   GetTotalTimeSampled(), AverageTimeLessChildrenCompare, maxLen);
			m_pOutputStream(_T("\n"));
		}
		if(type & VPRT_LIST_BY_PEAK_TIME)
		{
			DumpSorted(m_pOutputStream, _T("-- Profile scopes sorted by peak --"), GetTotalTimeSampled(), PeakCompare,
					   maxLen);
			m_pOutputStream(_T("\n"));
		}
		if(type & VPRT_LIST_BY_PEAK_OVER_AVERAGE)
		{
			DumpSorted(m_pOutputStream, _T("-- Profile scopes sorted by peak over average (including children) --"),
...
static void UpdateTelemetryThreadNames()
{
	if(g_bThreadNameArrayChanged)
	{
		...
		for(ThreadNameInfo_t *pThreadNameInfo = g_ThreadNamesList.Pop(); pThreadNameInfo;
...
static bool TelemetryInitialize()
{
	...
	retVal =
		tmOpen(g_tmContext, pGameName, szBuildInfo, pServerAddress, tmType, TELEMETRY_DEFAULT_PORT, TmOpenFlags, 1000);
	if(retVal != TM_OK)
	{
...
PLATFORM_INTERFACE void TelemetryTick()
{
	...
	if(!g_Telemetry.Level && g_Telemetry.DemoTickStart &&
	   ((uint32)g_Telemetry.playbacktick > g_Telemetry.DemoTickStart))
	{
		TelemetrySetLevel(2);
