...
int WalkStack(void **ppAddresses, int nMaxAddresses, int nSkip = 0)
{
	...
	{
		if(!StackWalk64(IMAGE_FILE_MACHINE_I386, hProcess, hThread, &frame, NULL, NULL, SymFunctionTableAccess64,
...
bool GetCallerModule(char *pDest)
{
	...
	if(!bInit)
	{
		...
		psUserSearchPath =
			"u:\\data\\game\\bin\\;u:\\data\\game\\episodic\\bin\\;u:\\data\\game\\hl2\\bin\\;\\\\perforce\\symbols";
...
char *StackDescribe(void **ppAddresses, int nMaxAddresses)
{
	...
	for(int k = 0; k < nMaxAddresses; ++k)
	{
		...
		if(SymGetLineFromAddr64(GetCurrentProcess(), (DWORD64)ppAddresses[k], &hlpLineOffset, &hlpLine))
		{
			...
			sprintf(pchBuffer, hlpLineOffset ? "%s:%d+0x%I32X" : "%s:%d", pchFileName, hlpLine.LineNumber,
					hlpLineOffset);
		}
		else if(SymGetSymFromAddr64(GetCurrentProcess(), (DWORD64)ppAddresses[k], &hlpSymbolOffset, &hlpSymbol))
		{
			sprintf(pchBuffer, (hlpSymbolOffset > 0 && !(hlpSymbolOffset >> 63)) ? "%s+0x%I64X" : "%s", hlpSymbol.Name,
					hlpSymbolOffset);
		}
		else
		{
...
private:
	...
	typedef std::map<MemInfoKey_t, MemInfo_t, std::less<MemInfoKey_t>,
...
const char *CDbgMemAlloc::s_pCountHeader[CDbgMemAlloc::NUM_BYTE_COUNT_BUCKETS] = {
	"<=16 byte allocations", "17-32 byte allocations", "33-128 byte allocations", "129-1024 byte allocations",
...
extern "C" BOOL APIENTRY MemDbgDllMain(HMODULE hDll, DWORD dwReason, PVOID pvReserved)
{
	...
	if(dwReason == DLL_PROCESS_DETACH)
	{
		...
		//  so we can't actually operate on the STL object in a normal destructor here as its support libraries have
		//  been turned off already
...
// Updates stats
//-----------------------------------------------------------------------------
void CDbgMemAlloc::RegisterAllocation(const char *pFileName, int nLine, int nLogicalSize, int nActualSize,
									  unsigned nTime)
{
	HEAP_LOCK();
...
void CDbgMemAlloc::RegisterDeallocation(const char *pFileName, int nLine, int nLogicalSize, int nActualSize,
										unsigned nTime)
{
	HEAP_LOCK();
...
void CDbgMemAlloc::RegisterDeallocation(MemInfo_t &info, int nLogicalSize, int nActualSize, unsigned nTime)
{
	...
	// It is technically legal for code to request allocations of zero bytes, and there are a number of places in our
	// code that do. So only assert that nLogicalSize >= 0.
...
{
	m_OutputFunc("%s, line %i\t%.1f\t%.1f\t%.1f\t%.1f\t%.1f\t%d\t%d\t%d\t%d", pAllocationName, line,
				 ...
				 info.m_nCurrentCount, info.m_nPeakCount, info.m_nTotalCount);
...
void CDbgMemAlloc::DumpStatsFileBase(char const *pchFileBase)
{
	...
	if(m_OutputFunc == DefaultHeapReportFunc)
	{
		...
		_snprintf(szFileName, sizeof(szFileName), "%s%s_%2.2d%2.2d_%2.2d%2.2d%2.2d_%d.txt", pPath, s_szStatsMapName,
	...
	m_OutputFunc("Allocation type\tCurrent Size(k)\tPeak Size(k)\tTotal Allocations(k)\tOverhead Size(k)\tPeak "
				 "Overhead Size(k)\tTime(ms)\tCurrent Count\tPeak Count\tTotal Count");
