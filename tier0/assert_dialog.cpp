...
static bool IsDebugBreakEnabled()
{
	static bool bResult = (_tcsstr(Plat_GetCommandLine(), _T("-debugbreak")) != NULL) ||
						  (_tcsstr(Plat_GetCommandLine(), _T("-raiseonassert")) != NULL) || getenv("RAISE_ON_ASSERT");
	return bResult;
}
...
)
{
	switch(uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			SetWindowPos(hDlg, HWND_TOP, ((rcDesktop.right - rcDesktop.left) - (rcDlg.right - rcDlg.left)) / 2,
						 ((rcDesktop.bottom - rcDesktop.top) - (rcDlg.bottom - rcDlg.top)) / 2, 0, 0, SWP_NOSIZE);
		}
			return true;
...
#define COLOR_END	 "\033[0m"
		fprintf(stderr,
				COLOR_YELLOW "ASSERT:" COLOR_END " " COLOR_RED "%s" COLOR_GREEN ":%i:" COLOR_END " " COLOR_RED
							 "%s" COLOR_END "\n",
	...
	_snprintf(cmdString, sizeof(cmdString), "Assert() 0x%.8x File: %s\tLine: %d\t%s", &g_VXConsoleAssertReturnValue,
			  pFilename, line, pExpression);
