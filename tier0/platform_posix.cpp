...
size_t ApproximateProcessMemoryUsage(void)
{
	...
	*/
	...
	if(pFile)
	{
		...
		if(fscanf(pFile, "%zu %zu %zu %zu %zu %zu %zu", &nSize, &nResident, &nShare, &nText, &nLib_Unused,
				  &nDataPlusStack, &nDt_Unused) >= 2)
		{
			nRet = 4096 * nResident;
		}
		fclose(pFile);
...
static void InitTimeSystem(void)
{
	...
	// now, see if we can use rdtsc instead. If this is one of the chips with a separate constant clock for rdtsc, we
	// can
	...
	if(pCpuInfo)
	{
		...
		if(!bAnyBadCores)
		{
			// this system appears to have the proper cpu setup to use rdtsc from reliable timing. Let's either read the
			// cpu frequency from an environment variable, or time it ourselves
			...
			else
			{
				...
				s_flRDTSCScale = 1.0 / (1000.0 * 1000.0 * s_flRDTSCToMicroSeconds);
				printf("Finished RDTSC test. To prevent the startup delay from this benchmark, set the environment "
					   "variable RDTSC_FREQUENCY to %f on this system."
					   " This value is dependent upon the CPU clock speed and architecture and should be determined "
					   "separately for each server. The use of this mechanism"
...
static FORCEINLINE void TestTimeSystem(void)
{
	...
	for(int t = 0; t < 5; t++)
	{
		...
		printf(" plat_float time says %f elapsed. gettimeofday says %f\n", flElapsedT,
			   dtime.tv_sec + dtime.tv_usec / 1000000.0);
	}
#endif
}
...
void DumpMemoryLog(int nThresh)
{
	...
	for(int i = 0; i < memList.size(); i++)
	{
		...
		Msg("Context cursize=%d nallocs=%d maxsize=%d total_allocs=%d\n", p->m_nCurrentAllocSize, p->m_nNumAllocsInUse,
			p->m_nMaximumSize, p->m_TotalNumAllocs);
		Msg("  stack\n");
...
void DumpChangedMemory(int nThresh)
{
	...
	for(int i = 0; i < memList.size(); i++)
	{
		CLinuxMallocContext *p = memList[i];
		char **strings = backtrace_symbols(p->pStackTraceBack, MAX_STACK_TRACEBACK);
		Msg("Context cursize=%d lastsize=%d nallocs=%d maxsize=%d total_allocs=%d\n", p->m_nCurrentAllocSize,
			p->m_nLastAllocSize, p->m_nNumAllocsInUse, p->m_nMaximumSize, p->m_TotalNumAllocs);
		Msg("  stack\n");
		for(int n = 0; n < MAX_STACK_TRACEBACK; n++)
			if(p->pStackTraceBack[n])
				Msg("  %p %s\n", p->pStackTraceBack[n], strings[n]);
