...
// our global error callback function. Note that this is not initialized, but static space guarantees this is NULL at
// app start. If you initialize, it will set to zero again when the CPP runs its static initializers, which could stomp
// the value if another CPP sets this value while initializing its static space
...
void Plat_ExitProcess(int nCode)
{
	...
	{
		int *x = NULL;
		*x = 1; // cause a hard crash, GC is not allowed to exit voluntarily from gc.dll
	}
