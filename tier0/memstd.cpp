...
#error
	...
	virtual void DumpStats()
	{
		...
		Msg("%u outstanding allocations (%d delta).\n", (unsigned)m_nOutstandingAllocations,
			(int)(m_nOutstandingAllocations - m_nOldOutstandingAllocations));
		Msg("%u lifetime allocations (%u delta).\n", (unsigned)m_nLifetimeAllocations,
			(unsigned)(m_nLifetimeAllocations - m_nOldLifetimeAllocations));
	...
	virtual void RegisterDeallocation(const char *pFileName, int nLine, int nLogicalSize, int nActualSize,
									  unsigned nTime)
	{
	}
	...
	if(exeHandle)
	{
		...
		if(GetModuleFileNameA(exeHandle, appName, ARRAYSIZE(appName)))
		{
			...
			if(pFilePart)
			{
				...
				{
					OutputDebugStringA("Trailing space on executable name! This will cause Application Verifier and "
									   "ETW Heap tracing to fail!\n");
				...
				HKEY key;
				LONG regResult = RegOpenKeyA(HKEY_LOCAL_MACHINE, regPathName, &key);
...
int CSmallBlockPool::Compact()
{
	...
	if(m_FreeList.Count())
	{
		...
		FreeBlock_t **pSortArray =
			(FreeBlock_t **)malloc(nFree * sizeof(FreeBlock_t *)); // can't use new because will reenter
	...
	if(pFile)
	{
		for(int i = 0; i < NUM_POOLS; i++)
		{
			...
			fprintf(pFile, "Pool %i: Size: %llu Allocated: %i Free: %i Committed: %i CommittedSize: %i\n", i,
					(uint64)m_Pools[i].GetBlockSize(), m_Pools[i].CountAllocatedBlocks(), m_Pools[i].CountFreeBlocks(),
			...
			else
			{
				...
				if(pCurBlockEnd == m_pCurBlockEnd)
				{
					for(;;)
					{
						...
						if(ThreadInterlockedAssignPointerIf((void **)&gm_pPhysicalBlock,
															(void *)(pPhysicalBlock + PAGESIZE_X360_SBH),
															(void *)pPhysicalBlock))
						{
