...
static SpewRetval_t _SpewMessage(SpewType_t spewType, const char *pGroupName, int nLevel, const Color *pColor,
								 const tchar *pMsgFormat, va_list args)
{
...
void SpewActivate(const tchar *pGroupName, int level)
{
	...
	if(!FindSpewGroup(pGroupName, &ind))
	{
		...
		if(s_pSpewGroups)
		{
			s_pSpewGroups = (SpewGroup_t *)PvRealloc(s_pSpewGroups, s_GroupCount * sizeof(SpewGroup_t));
