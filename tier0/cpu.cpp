...
static bool cpuid(unsigned long function, unsigned long &out_eax, unsigned long &out_ebx, unsigned long &out_ecx,
...
// Returns non-zero if Hyper-Threading Technology is supported on the processors and zero if not.  This does not mean
// that Hyper-Threading Technology is necessarily enabled.
static bool HTSupported(void)
{
	...
	return true;
#else
	const unsigned int HT_BIT =
		0x10000000; // EDX[28] - Bit 28 set indicates Hyper-Threading Technology is supported in hardware.
	...
	unsigned long unused, reg_eax = 0, reg_edx = 0, vendor_id[3] = {0, 0, 0};

	// verify cpuid instruction is supported
	if(!cpuid(0, unused, vendor_id[0], vendor_id[2], vendor_id[1]) || !cpuid(1, reg_eax, unused, unused, reg_edx))
		return false;
		...
		while(fgets(buf, ARRAYSIZE(buf), fpCpuInfo))
		{
			if(char *value = strchr(buf, ':'))
			{
				...
				if(nProcId != -1 && nCoreId != -1) // as soon as we have a complete id, process it
				{
					...
					while(i < nKnownIdCount && anKnownIds[i] != nId)
					{
						++i;
					}
