...
JoltPhysicsFluidController::JoltPhysicsFluidController(JPH::PhysicsSystem *pPhysicsSystem,
													   JoltPhysicsObject *pFluidObject, const fluidparams_t *pParams)
...
{
public:
	SourceFluidCollector(JPH::PhysicsSystem *inSystem, std::vector<JoltPhysicsObject *> &objectsInShape,
						 const JPH::Plane &inSurface, float inDeltaTime, const fluidparams_t &params, float flDensity)
	...
	void AddHit(const ResultType &inResult) override
	{
		...
		if(body.IsActive())
		{
			...
			body.ApplyBuoyancyImpulse(point, normal, inBuoyancy, m_Params.damping, 0.1f,
									  SourceToJolt::Distance(m_Params.currentVelocity), m_pPhysicsSystem->GetGravity(),
									  m_DeltaTime);
		}
	}
...
void JoltPhysicsFluidController::OnPreSimulate(float deltaTime)
{
	...
	JPH::Plane surface =
		JPH::Plane(SourceToJolt::Unitless(surfacePlane.normal), SourceToJolt::Distance(-surfacePlane.dist));
	...
	m_pPhysicsSystem->GetNarrowPhaseQueryNoLock().CollideShape(
		...
		JPH::SpecifiedBroadPhaseLayerFilter(BroadPhaseLayers::MOVING), JPH::SpecifiedObjectLayerFilter(Layers::MOVING),
		body_filter);
