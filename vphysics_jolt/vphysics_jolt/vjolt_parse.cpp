...
public:
	...
	void ParseRagdollConstraint(constraint_ragdollparams_t *pConstraint,
	...
	void ParseRagdollAnimatedFriction(ragdollanimatedfriction_t *pFriction,
									  IVPhysicsKeyHandler *unknownKeyHandler) override_asw;
	...
	{"Suspension",
	 ...
	 {sizeof(vehicle_suspensionparams_t),
	  [](KeyValues *pProp, void *pPtr, size_t size)
	  {
		  ...
		  ParseJoltKVSchema(pProp, kVehicleAxleSuspensionDescs, ARRAYSIZE(kVehicleAxleSuspensionDescs),
							pSuspensionParams);
	  }}},
...
static const JoltKVSchemaProp_t kVehicleSteeringDescs[] = {
	...
	{"ThrottleSteeringRestRateFactor", KVSCHEMA_DESC(vehicle_steeringparams_t, throttleSteeringRestRateFactor),
	 FillFloatProp},
	{"BoostSteeringRestRateFactor", KVSCHEMA_DESC(vehicle_steeringparams_t, boostSteeringRestRateFactor),
	 FillFloatProp},
	...
	{"CollisionPair", KVSCHEMA_DESC(JoltPhysicsCollisionRulesHelper, CollisionPair), FillIntPairProp,
	 [](void *pBaseObject)
	 {
		 ...
		 if(pHelper->Rules.bSelfCollisions)
			 pHelper->Rules.pCollisionSet->EnableCollisions(pHelper->CollisionPair.Index0,
															pHelper->CollisionPair.Index1);
	 }},
};
...
JoltPhysicsParseKV::JoltPhysicsParseKV(KeyValues *pKV) : m_pKV(pKV), m_pCurrentBlock(m_pKV->GetFirstSubKey()) {}
...
void JoltPhysicsParseKV::ParseRagdollConstraint(constraint_ragdollparams_t *pConstraint,
												IVPhysicsKeyHandler *unknownKeyHandler)
{
	...
	ParseJoltKVSchema(m_pCurrentBlock, kRagdollDescs, ARRAYSIZE(kRagdollDescs), pConstraint, pConstraint,
					  unknownKeyHandler);
...
void JoltPhysicsParseKV::ParseCollisionRules(ragdollcollisionrules_t *pRules, IVPhysicsKeyHandler *unknownKeyHandler)
{
	...
	ParseJoltKVSchema(m_pCurrentBlock, kCollisionRulesDescs, ARRAYSIZE(kCollisionRulesDescs), &helper, pRules,
					  unknownKeyHandler);
...
void JoltPhysicsParseKV::ParseRagdollAnimatedFriction(ragdollanimatedfriction_t *pFriction,
													  IVPhysicsKeyHandler *unknownKeyHandler)
{
	if(unknownKeyHandler)
	...
	ParseJoltKVSchema(m_pCurrentBlock, kRagdollAnimatedFrictionDescs, ARRAYSIZE(kRagdollAnimatedFrictionDescs),
					  pFriction, pFriction, unknownKeyHandler);
...
{
	VJoltAssertMsg(!bIsPacked,
				   "Packed VPhysics KV not supported. You should not get here anyway as we do not emit it.");
	if(bIsPacked)
		return nullptr;
	...
	if(!pszKV)
	{
		Log_Warning(LOG_VJolt, "CreateVPhysicsKeyParser: Encountered invalid KV data. Falling back to a dummy KV. You "
							   "may notice a broken prop/vehicle.\n");
