...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(JoltPhysicsCollision, IPhysicsCollision, VPHYSICS_COLLISION_INTERFACE_VERSION,
								  JoltPhysicsCollision::GetInstance());
...
ShapeType *ShapeSettingsToShape(const T &settings)
{
	...
	if(!result.IsValid())
	{
		...
		const char *error = result.HasError() ? result.GetError().c_str() : "Unknown";
...
CPhysConvex *JoltPhysicsCollision::BBoxToConvex(const Vector &mins, const Vector &maxs)
{
	...
	JPH::RotatedTranslatedShapeSettings rotatedSettings(aabox.mMin + aabox.GetExtent(), JPH::Quat::sIdentity(),
														pBoxShape);
...
CPhysConvex *JoltPhysicsCollision::ConvexFromConvexPolyhedron(const CPolyhedron &ConvexPolyhedron)
{
	...
	JPH::ConvexHullShapeSettings settings(pPoints.get(), ConvexPolyhedron.iVertexCount, kMaxConvexRadius,
										  nullptr /* material */);
...
void JoltPhysicsCollision::ConvexesFromConvexPolygon(const Vector &vPolyNormal, const Vector *pPoints, int iPointCount,
													 CPhysConvex **pOutput)
{
	// Slart: Unused
	Log_Stub(LOG_VJolt);
...
void JoltPhysicsCollision::PolysoupAddTriangle(CPhysPolysoup *pSoup, const Vector &a, const Vector &b, const Vector &c,
											   int materialIndex7bits)
{
	// Add both windings to make this two-faced.
	pSoup->Triangles.push_back(JPH::Triangle(SourceToJolt::DistanceFloat3(c), SourceToJolt::DistanceFloat3(b),
											 SourceToJolt::DistanceFloat3(a)));
	pSoup->Triangles.push_back(JPH::Triangle(SourceToJolt::DistanceFloat3(a), SourceToJolt::DistanceFloat3(b),
											 SourceToJolt::DistanceFloat3(c)));
}
...
CPhysCollide *JoltPhysicsCollision::ConvertConvexToCollideParams(CPhysConvex **pConvex, int convexCount,
...
Vector JoltPhysicsCollision::CollideGetExtent(const CPhysCollide *pCollide, const Vector &collideOrigin,
											  const QAngle &collideAngles, const Vector &direction)
{
	...
	ActOnSubShapes<JPH::ConvexShape>(pShape,
									 [&](const JPH::ConvexShape *pConvexShape, JPH::Mat44Arg matSubShapeTransform)
									 {
										 JPH::Mat44 matTransform = matCollideTransform * matSubShapeTransform;
										 JPH::ConvexShape::SupportingFace supportingFace;
										 pConvexShape->GetSupportingFace(JPH::SubShapeID(), vecDirection,
																		 JPH::Vec3::sReplicate(1.0f), matTransform,
																		 supportingFace);
...
void JoltPhysicsCollision::CollideGetAABB(Vector *pMins, Vector *pMaxs, const CPhysCollide *pCollide,
...
int JoltPhysicsCollision::GetConvexesUsedInCollideable(const CPhysCollide *pCollideable, CPhysConvex **pOutputArray,
													   int iOutputArrayLimit)
{
	...
	{
		pOutputArray[0] =
			const_cast<CPhysConvex *>(CPhysConvex::FromConvexShape(static_cast<const JPH::ConvexShape *>(pShape)));
		return 1;
	}
	...
	for(uint i = 0; i < maxNumShapes; ++i)
		pOutputArray[i] = const_cast<CPhysConvex *>(
			CPhysConvex::FromConvexShape(static_cast<const JPH::ConvexShape *>(subShapes[i].mShape.GetPtr())));
...
bool JoltPhysicsCollision::IsBoxIntersectingCone(const Vector &boxAbsMins, const Vector &boxAbsMaxs,
...
namespace ivp_compat
{
	...
	static constexpr int IVP_COMPACT_SURFACE_SUPER_LEGACY =
		0; // Really old .phy files, don't have anything here, and were just serialized compact headers directly.
	...
	{
		const compactsurfaceheader_t *pSurfaceHeader =
			reinterpret_cast<const compactsurfaceheader_t *>(pCollideHeader + 1);
...
void JoltPhysicsCollision::VCollideLoad(vcollide_t *pOutput, int solidCount, const char *pBuffer, int size,
										bool swap /*= false*/)
{
	if(swap)
	{
	...
	for(int i = 0; i < solidCount; i++)
	{
		...
		const ivp_compat::collideheader_t *pCollideHeader =
			reinterpret_cast<const ivp_compat::collideheader_t *>(pCursor);
		...
		else
		{
			...
			const ivp_compat::compactsurface_t *pCompactSurface =
				reinterpret_cast<const ivp_compat::compactsurface_t *>(pCursor);
			...
			switch(legacyModelType)
			{
				...
				case ivp_compat::IVP_COMPACT_MOPP_ID:
					Log_Warning(LOG_VJolt,
								"Unsupported legacy solid type IVP_COMPACT_MOPP_ID on solid %d. Skipping...\n", i);
					break;
...
int JoltPhysicsCollision::CreateDebugMesh(CPhysCollide const *pCollisionModel, Vector **outVerts)
{
	...
	pShape->CollectTransformedShapes(JPH::AABox::sBiggest(), pShape->GetCenterOfMass() * JoltToSource::Factor,
									 JPH::Quat::sIdentity(), JPH::Vec3::sReplicate(JoltToSource::Factor),
									 JPH::SubShapeIDCreator(), collector, filter);
	...
	for(auto &shape : collector.mHits)
	{
		...
		for(;;)
		{
			int nSubShapeTriCount = shape.GetTrianglesNext(
				ctx, nRequestCount, reinterpret_cast<JPH::Float3 *>(&pVerts[nAccumTris * 3]), nullptr /* materials */);
