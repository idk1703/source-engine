...
static ConVar vjolt_vehicle_throttle_opposition_limit(
	"vjolt_vehicle_throttle_opposition_limit", "5", FCVAR_NONE,
...
static const char *VehicleTypeToName(unsigned int VehicleType)
{
	switch(VehicleType)
	{
		...
		case VEHICLE_TYPE_JETSKI_RAYCAST:
			return "Jetski Raycast";
		case VEHICLE_TYPE_AIRBOAT_RAYCAST:
			return "Airboat Raycast";
		default:
			return "Unknown";
	}
}
...
JoltPhysicsVehicleController::JoltPhysicsVehicleController(JoltPhysicsEnvironment *pEnvironment,
														   JPH::PhysicsSystem *pPhysicsSystem,
														   JoltPhysicsObject *pVehicleBodyObject,
...
void JoltPhysicsVehicleController::HandleBoostDecay()
{
	...
	{
		m_OperatingParams.boostTimeLeft =
			m_InternalState.BoostDelay
				? 100.0f - (100.0f * (m_InternalState.BoostDelay /
									  (m_VehicleParams.engine.boostDuration + m_VehicleParams.engine.boostDelay)))
				: 100.0f;
	}
}
...
void JoltPhysicsVehicleController::OnPreSimulate(float flDeltaTime)
{
	...
	if(m_ControlParams.steering != 0.0f || m_ControlParams.throttle != 0.0f || m_ControlParams.brake != 0.0f ||
	   m_ControlParams.handbrake)
	...
	if(m_InternalState.BoosterRemainingTime != 0.0f)
	{
		...
		const float flSpeedFactor =
			RemapValClamped(fabsf(m_OperatingParams.speed), 0, m_VehicleParams.engine.maxSpeed, 0.1f, 1.0f);
	...
	for(int i = 0; i < m_VehicleParams.axleCount; i++)
		GetWheeledVehicleController()->GetDifferentials()[i].mEngineTorqueRatio =
			flTotalTorqueMultiplier * m_VehicleParams.axles[i].torqueFactor;

	// Pass the input on to the constraint
	GetWheeledVehicleController()->SetDriverInput(flThrottle, m_ControlParams.steering, flBrake,
...
{
	// Draw our wheels (this needs to be done in the pre update since we draw the bodies too in the state before the
	// step)
	...
	for(int w = 0; w < GetWheelCount(); w++)
	{
		...
		JPH::Mat44 wheelTransform =
			m_VehicleConstraint->GetWheelWorldTransform(w, JPH::Vec3(1, 0, 0), JPH::Vec3(0, 0, 1));
		...
		if(vjolt_vehicle_wheel_debug.GetBool() && pDebugOverlay)
		{
			...
			const Vector vecWheelSize =
				JoltToSource::Distance(JPH::Vec3(settings->mWidth / 2.0f, settings->mRadius, settings->mRadius));

			pDebugOverlay->AddBoxOverlay(vecWheelPos, -vecWheelSize, vecWheelSize, newQuat, 255, 0, 255, 100, -1.0f);
		}
	}
...
void JoltPhysicsVehicleController::CreateWheel(JPH::VehicleConstraintSettings &vehicleSettings, matrix3x4_t &bodyMatrix,
											   int axleIdx, int wheelIdx)
{
	const vehicle_axleparams_t &axle = m_VehicleParams.axles[axleIdx];

	const Vector wheelPositionLocal = axle.offset + ((wheelIdx % 2 == 1) ? axle.wheelOffset : -axle.wheelOffset);
...

	{
		...
		IPhysicsObject *pWheelObject = m_pEnvironment->CreateSphereObject(
			wheelRadius, axle.wheels.materialIndex, wheelPositionWorld, QAngle(), &wheelParams, false);
	...
	const float steeringAngle =
		DEG2RAD(Max(m_VehicleParams.steering.degreesSlow, m_VehicleParams.steering.degreesFast));
	...
	wheelSettings->mMaxBrakeTorque = 0.5f * SourceToJolt::Distance(gravity.Length()) *
									 (m_pCarBodyObject->GetMass() + m_TotalWheelMass) * axle.brakeFactor *
...
JPH::WheeledVehicleControllerSettings *JoltPhysicsVehicleController::CreateVehicleController()
{
	...
	pController->mEngine.mMaxTorque = (HorsePowerToWatts * m_VehicleParams.engine.horsepower) /
									  (2.0f * M_PI * (m_VehicleParams.engine.maxRPM / 60.0f));
	// Josh: Fudge
	...
	pController->mTransmission.mMode =
		m_VehicleParams.engine.isAutoTransmission ? JPH::ETransmissionMode::Auto : JPH::ETransmissionMode::Manual;
