...
static ConVar vjolt_debugrender_color_mode("vjolt_debugrender_color_mode", "instance", 0,
										   "One of instance, shape_type, motion_type, sleep, island, material.");
#endif
...
void JoltPhysicsDebugRenderer::DrawTriangle(JPH::Vec3Arg inV1, JPH::Vec3Arg inV2, JPH::Vec3Arg inV3,
											JPH::ColorArg inColor)
{
	// DrawTriangle_Internal( JPH::Float3( inV1.GetX(), inV1.GetY(), inV1.GetZ() ), JPH::Float3( inV2.GetX(),
	// inV2.GetY(), inV2.GetZ() ), JPH::Float3( inV3.GetX(), inV3.GetY(), inV3.GetZ() ), inColor );
}

JoltPhysicsDebugRenderer::Batch JoltPhysicsDebugRenderer::CreateTriangleBatch(const Triangle *inTriangles,
																			  int inTriangleCount)
{
#ifndef VJOLT_USE_PHYSICS_DEBUG_OVERLAY
	...
	meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, inTriangleCount);
	{
		...
		{
			meshBuilder.Position3f(inVertices[i].mPosition.x * JoltToSource::Factor,
								   inVertices[i].mPosition.y * JoltToSource::Factor,
								   inVertices[i].mPosition.z * JoltToSource::Factor);
...
JoltPhysicsDebugRenderer::Batch JoltPhysicsDebugRenderer::CreateTriangleBatch(const Vertex *inVertices,
																			  int inVertexCount,
...
void JoltPhysicsDebugRenderer::DrawGeometry(JPH::Mat44Arg inModelMatrix, const JPH::AABox &inWorldSpaceBounds,
											...
											ECastShadow inCastShadow, EDrawMode inDrawMode)
{
	...
	DebugOverlayMeshDesc_t desc = {.pMesh = batch->GetMesh(),
...
void JoltPhysicsDebugRenderer::DrawText3D(JPH::Vec3Arg inPosition, const std::string_view &inString,
...
void JoltPhysicsDebugRenderer::RenderPhysicsSystem(JPH::PhysicsSystem &physicsSystem)
{
	...
	JPH::BodyManager::DrawSettings drawSettings {
		...
		.mDrawSupportDirection =
			false, ///< When drawing the support function, also draw which direction mapped to a specific support point
		...
		.mDrawShapeWireframe = wireframe,	 ///< When mDrawShape is true and this is true, the shapes will be drawn in
		...
		.mDrawWorldTransform =
			false, ///< Draw the world transform (which can be different than the center of mass) for each body
	...
	JPH::BodyManager::DrawSettings drawSettings {
		...
		.mDrawShapeWireframe = false,		 ///< When mDrawShape is true and this is true, the shapes will be drawn in
											 ///< wireframe instead of solid.
