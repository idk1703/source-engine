...
const JoltKVSchemaFunc_t FillBaseProp = {
	sizeof(surfacedata_t), [](KeyValues *pProp, void *pPtr, size_t size)
	{
		...
		if(nSurfaceIndex == -1)
		{
			Log_Warning(
				LOG_VJolt,
				"You must specify the base material %s before it can be used. Defaulting to 'default' as a base.\n",
				pProp->GetString());
...
const JoltKVSchemaFunc_t FillStringProp = {0, // Varies.
...
const JoltKVSchemaFunc_t FillIntProp = {sizeof(int), [](KeyValues *pProp, void *pPtr, size_t size)
										{
											int *pIntPtr = reinterpret_cast<int *>(pPtr);
											*pIntPtr = pProp->GetInt();
										}};

const JoltKVSchemaFunc_t FillFloatProp = {sizeof(float), [](KeyValues *pProp, void *pPtr, size_t size)
										  {
											  float *pFloatPtr = reinterpret_cast<float *>(pPtr);
											  *pFloatPtr = pProp->GetFloat();
										  }};

const JoltKVSchemaFunc_t FillUnsignedCharProp = {sizeof(unsigned char), [](KeyValues *pProp, void *pPtr, size_t size)
												 {
													 unsigned char *pCharPtr = reinterpret_cast<unsigned char *>(pPtr);
													 *pCharPtr = static_cast<unsigned char>(pProp->GetInt());
												 }};

const JoltKVSchemaFunc_t FillBoolProp = {sizeof(bool), [](KeyValues *pProp, void *pPtr, size_t size)
										 {
											 bool *pBoolPtr = reinterpret_cast<bool *>(pPtr);
											 *pBoolPtr = pProp->GetBool();
										 }};

const JoltKVSchemaFunc_t FillVectorProp = {sizeof(Vector), [](KeyValues *pProp, void *pPtr, size_t size)
										   {
											   Vector *pVectorPtr = reinterpret_cast<Vector *>(pPtr);
											   sscanf(pProp->GetString(), "%f %f %f", &pVectorPtr->x, &pVectorPtr->y,
													  &pVectorPtr->z);
										   }};

const JoltKVSchemaFunc_t FillVector4DProp = {sizeof(Vector4D), [](KeyValues *pProp, void *pPtr, size_t size)
											 {
												 Vector4D *pVector4DPtr = reinterpret_cast<Vector4D *>(pPtr);
												 sscanf(pProp->GetString(), "%f %f %f %f", &pVector4DPtr->x,
														&pVector4DPtr->y, &pVector4DPtr->z, &pVector4DPtr->w);
											 }};

const JoltKVSchemaFunc_t FillQAngleProp = {sizeof(QAngle), [](KeyValues *pProp, void *pPtr, size_t size)
										   {
											   QAngle *pQAnglePtr = reinterpret_cast<QAngle *>(pPtr);
											   sscanf(pProp->GetString(), "%f %f %f", &pQAnglePtr->x, &pQAnglePtr->y,
													  &pQAnglePtr->z);
										   }};

const JoltKVSchemaFunc_t FillIntPairProp = {
	sizeof(JoltPhysicsIntPair), [](KeyValues *pProp, void *pPtr, size_t size)
	{
...
const JoltKVSchemaFunc_t FillGameMaterialProp = {sizeof(unsigned short), [](KeyValues *pProp, void *pPtr, size_t size)
												 {
													 const char *pValue = pProp->GetString();
													 unsigned short *pShortPtr =
														 reinterpret_cast<unsigned short *>(pPtr);
...
const JoltKVSchemaFunc_t FillSoundProp = {sizeof(unsigned short), [](KeyValues *pProp, void *pPtr, size_t size)
										  {
											  const char *pValue = pProp->GetString();
...
const JoltKVSchemaFunc_t FillSurfaceProp = {sizeof(int), [](KeyValues *pProp, void *pPtr, size_t size)
											{
												const char *pValue = pProp->GetString();
												int *pIntPtr = reinterpret_cast<int *>(pPtr);

												*pIntPtr =
													JoltPhysicsSurfaceProps::GetInstance().GetSurfaceIndex(pValue);
											}};

//-------------------------------------------------------------------------------------------------

void ParseJoltKVSchema(KeyValues *pKV, const JoltKVSchemaProp_t *pDescs, uint count, void *pObj, void *pUnknownKeyObj,
					   IVPhysicsKeyHandler *pUnknownKeyHandler)
{
	...
	for(KeyValues *pProp = pKV->GetFirstSubKey(); pProp != nullptr; pProp = pProp->GetNextKey())
	{
		...
		for(uint i = 0; i < count; i++)
		{
			...
			{
				VJoltAssertMsg(desc.func.ptr_size == 0 || desc.size == desc.func.ptr_size,
