...
namespace VJoltTrace
{
	...
	//
	// Slart: Portal 2 probably passes in a bad winding order in the polyhedron or something, dunno if it affects Portal
	// 1
	...
	static void PrintTrace(const trace_t *tr, JoltPhysicsDebugRenderer &debugRenderer)
	{
		...
		pDebugOverlay->AddTextOverlayRGB(textPos, 0, dur, r, g, b, a, "startpos   : [ %g %g %g ]", tr->startpos.x,
										 tr->startpos.y, tr->startpos.z);
		pDebugOverlay->AddTextOverlayRGB(textPos, 1, dur, r, g, b, a, "endpos     : [ %g %g %g ]", tr->endpos.x,
		...
		pDebugOverlay->AddTextOverlayRGB(textPos, 6, dur, r, g, b, a, "normal     : [ %g %g %g ]", tr->plane.normal.x,
										 tr->plane.normal.y, tr->plane.normal.z);
	...
	public:
		...
		bool ShouldCollide(const JPH::Shape *inShape1, const JPH::SubShapeID &inSubShapeIDOfShape1,
	...
	//
	static void CastRay(const Ray_t &ray, uint32 contentsMask, IConvexInfo *pConvexInfo, const CPhysCollide *pCollide,
						const Vector &collideOrigin, const QAngle &collideAngles, trace_t *pTrace)
	{
		...
		JPH::Mat44 queryTransform =
			JPH::Mat44::sRotationTranslation(rotation, position + rotation * pShape->GetCenterOfMass());
		...
		// the BSP code (the gold standard) allows traces that start inside of solid objects to pass through backfaces
		// on the convex. BSP sets traces that start inside of objects as startsolid 1, IVP doesn't do this :( So in the
		// future, to mimic the BSP logic, we should make traces that begin inside of objects startsolid and only
		// allsolid if they never leave the object
		//
		...
		{
			JPH::Vec3 normal =
				queryTransform.GetRotation() *
				pShape->GetSurfaceNormal(collector.m_SubShapeID, joltRay.GetPointOnRay(collector.m_Fraction));
		...
		if(vjolt_trace_debug.GetBool() && vjolt_trace_debug_castray.GetBool() && pOverlay)
		{
			...
			if(collector.m_DidHit)
			{
				...
				debugRenderer.DrawArrow(
					queryTransform * (joltRay.mOrigin + (joltRay.mDirection * collector.GetEarlyOutFraction())),
					queryTransform * (joltRay.mOrigin + joltRay.mDirection * collector.GetEarlyOutFraction()) + normal,
					JPH::Color::sRed, 0.3f);
			}

			debugRenderer.DrawArrow(queryTransform * joltRay.mOrigin,
									queryTransform *
										(joltRay.mOrigin + (joltRay.mDirection * collector.GetEarlyOutFraction())),
									rayColor, 0.3f);
		}
	...
	//
	static void CollidePoint(const Ray_t &ray, uint32 contentsMask, IConvexInfo *pConvexInfo,
	...
	//
	static void CastBoxVsShape(const Ray_t &ray, uint32 contentsMask, IConvexInfo *pConvexInfo,
							   const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles,
							   trace_t *pTrace)
	{
		const JPH::Shape *pShape = pCollide->ToShape();
		...
		JPH::CollisionDispatch::sCastShapeVsShapeWorldSpace(shapeCast, settings, pShape, JPH::Vec3::sReplicate(1.0f),
															filter, queryTransform, JPH::SubShapeIDCreator(),
		...
		if(collector.m_DidHit)
		{
			...
			// Log_Msg( LOG_VJolt, "Depth: %g, InitialFraction = %g, NewFraction = %g\n", collector.m_PenetrationDepth,
			// flInitialFraction, pTrace->fraction );
	...
	//
	static void CollideBoxVsShape(const Ray_t &ray, uint32 contentsMask, IConvexInfo *pConvexInfo,
								  const CPhysCollide *pCollide, const Vector &collideOrigin,
								  const QAngle &collideAngles, trace_t *pTrace)
	{
		const JPH::Shape *pShape = pCollide->ToShape();
		...
		JPH::CollisionDispatch::sCollideShapeVsShape(
			&boxShape, pShape, JPH::Vec3::sReplicate(1.0f), JPH::Vec3::sReplicate(1.0f), JPH::Mat44::sIdentity(),
			queryTransform, JPH::SubShapeIDCreator(), JPH::SubShapeIDCreator(), settings, collector);
	...
	//
	// Slart: This sucks, it could be implemented better in the environment interface using a bool and two physics
	// objects instead... Basically all the code using this wants it to see if a shape is inside another shape.
	//
	static void CollideShapeVsShape(const Vector &start, const Vector &end, const CPhysCollide *pSweepCollide,
									...
									const Vector &collideOrigin, const QAngle &collideAngles, trace_t *pTrace)
	{
		...
		JPH::Mat44 sweepTransform = JPH::Mat44::sRotationTranslation(
			sweepJoltRotation, sweepJoltPosition + sweepJoltRotation * pSweepShape->GetCenterOfMass());
		...
		JPH::Mat44 collideTransform = JPH::Mat44::sRotationTranslation(
			collideJoltRotation, collideJoltPosition + collideJoltRotation * pCollideShape->GetCenterOfMass());
		...
		if(!isSweeping)
		{
			...
			JPH::CollisionDispatch::sCollideShapeVsShape(pSweepShape, pCollideShape, JPH::Vec3::sReplicate(1.0f),
														 JPH::Vec3::sReplicate(1.0f), sweepTransform, collideTransform,
	...
	static void TraceBase(const Ray_t &ray, uint32 contentsMask, IConvexInfo *pConvexInfo, const CPhysCollide *pCollide,
						  const Vector &collideOrigin, const QAngle &collideAngles, trace_t *pTrace)
	{
...
void JoltPhysicsCollision::TraceBox(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs,
									const CPhysCollide *pCollide, const Vector &collideOrigin,
...
void JoltPhysicsCollision::TraceBox(const Ray_t &ray, const CPhysCollide *pCollide, const Vector &collideOrigin,
									const QAngle &collideAngles, trace_t *ptr)
{
	VJoltTrace::TraceBase(ray, MASK_ALL, nullptr, pCollide, collideOrigin, collideAngles, ptr);
}

void JoltPhysicsCollision::TraceBox(const Ray_t &ray, unsigned int contentsMask, IConvexInfo *pConvexInfo,
									const CPhysCollide *pCollide, const Vector &collideOrigin,
									const QAngle &collideAngles, trace_t *ptr)
{
...
void JoltPhysicsCollision::TraceCollide(const Vector &start, const Vector &end, const CPhysCollide *pSweepCollide,
										const QAngle &sweepAngles, const CPhysCollide *pCollide,
										const Vector &collideOrigin, const QAngle &collideAngles, trace_t *pTrace)
{
	VJoltTrace::CollideShapeVsShape(start, end, pSweepCollide, sweepAngles, pCollide, collideOrigin, collideAngles,
									pTrace);
}
