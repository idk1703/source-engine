...
void JoltPhysicsPlayerController::Update(const Vector &position, const Vector &velocity, float secondsToArrival,
										 bool onground, IPhysicsObject *ground)
{
...
static void CheckCollision(JoltPhysicsObject *pObject, JPH::CollideShapeCollector &ioCollector,
						   JPH::BodyFilter &ioFilter)
{
	...
	JPH::DefaultBroadPhaseLayerFilter broadphase_layer_filter =
		pSystem->GetDefaultBroadPhaseLayerFilter(Layers::MOVING);
	...
	pSystem->GetNarrowPhaseQueryNoLock().CollideShape(pObject->GetBody()->GetShape(), JPH::Vec3::sReplicate(1.0f),
													  query_transform, settings, JPH::Vec3::sZero(), ioCollector,
													  broadphase_layer_filter, object_layer_filter, ioFilter);
}

// Slart: This is a version of CheckCollision that projects the player by their velocity, to attempt to push objects
// that we'll walk into soon
#if 0
...
public:
	SourceHitFilter(JPH::PhysicsSystem *pPhysicsSystem, JoltPhysicsObject *pSelfObject)
		: m_pPhysicsSystem(pPhysicsSystem), m_pSelfObject(pSelfObject)
	{
	}
...
{
public:
	NormalWeightedCollector(JPH::PhysicsSystem *pPhysicsSystem) : m_pPhysicsSystem(pPhysicsSystem) {}
...
{
	// This does not seem to affect much, we should aspire to have our physics be as 1:1 to brush collisions as possible
	// anyway
	...
	public:
		ContactStateCollector(JPH::PhysicsSystem *pPhysicsSystem, JoltPhysicsObject *pPlayerObject, uint16 nGameFlags)
			: m_pPhysicsSystem(pPhysicsSystem), m_pPlayerObject(pPlayerObject), m_nGameFlags(nGameFlags)
		{
		}
...
{
	VJoltAssertMsg(m_pObject->GetBody()->GetMotionType() == JPH::EMotionType::Kinematic,
				   "Shadow controllers must be kinematic!");
	...
	/*if ( m_pGround )
	{
		JPH::Mat44 matrix = JPH::Mat44::sRotationTranslation( m_pObject->GetBody()->GetRotation(),
	m_pObject->GetBody()->GetPosition() ).Transposed3x3(); JPH::Vec3 groundPos = -matrix.Multiply3x3( m_targetPosition
	); int g = 5; m_targetPosition = matrix * groundPos;
	}*/
#endif
	...
	/*if ( m_pGround && m_pGround->IsMoveable() )
	{
		...
		bodyInterface.AddImpulse( m_pGround->GetBodyID(), pPhysicsSystem->GetGravity() * m_pObject->GetMass() *
	flDeltaTime, m_pObject->GetBody()->GetPosition() );
	}*/
	...
	if(collector.HadHit() && collector.m_flLowestNormalZ < -0.7f)
	{
		...
		// bodyInterface.AddImpulse( otherID, m_pObject->GetMass() * m_targetVelocity * flDeltaTime,
		// m_pObject->GetBody()->GetPosition() );
		bodyInterface.AddImpulse(otherID, m_pObject->GetMass() * pPhysicsSystem->GetGravity() * flDeltaTime,
								 m_pObject->GetBody()->GetPosition());
	}

	if(m_secondsToArrival > 0.0f)
		bodyInterface.MoveKinematic(m_pObject->GetBodyID(), m_targetPosition, JPH::Quat::sIdentity(),
									m_secondsToArrival);
	else
	{
		bodyInterface.SetPositionAndRotation(m_pObject->GetBodyID(), m_targetPosition, JPH::Quat::sIdentity(),
