...
JoltPhysicsObject::JoltPhysicsObject(JPH::Body *pBody, JoltPhysicsEnvironment *pEnvironment, bool bStatic,
									 int nMaterialIndex, const objectparams_t *pParams)
	: m_pBody(pBody),
...
JoltPhysicsObject::JoltPhysicsObject(JPH::Body *pBody, JoltPhysicsEnvironment *pEnvironment, void *pGameData,
...
void JoltPhysicsObject::GetVelocityAtPoint(const Vector &worldPosition, Vector *pVelocity) const
{
	...
	*pVelocity = JoltToSource::Distance(m_pPhysicsSystem->GetBodyInterfaceNoLock().GetPointVelocity(
		m_pBody->GetID(), SourceToJolt::Distance(worldPosition)));
}
...
void JoltPhysicsObject::CalculateForceOffset(const Vector &forceVector, const Vector &worldPosition,
...
void JoltPhysicsObject::CalculateVelocityOffset(const Vector &forceVector, const Vector &worldPosition,
...
void JoltPhysicsObject::SetShadow(float maxSpeed, float maxAngularSpeed, bool allowPhysicsMovement,
								  bool allowPhysicsRotation)
{
	...
	else
	{
		...
		m_pShadowController = static_cast<JoltPhysicsShadowController *>(
			m_pEnvironment->CreateShadowController(this, allowPhysicsMovement, allowPhysicsRotation));
		m_pShadowController->MaxSpeed(maxSpeed, maxAngularSpeed);
	}
}

void JoltPhysicsObject::UpdateShadow(const Vector &targetPosition, const QAngle &targetAngles, bool tempDisableGravity,
									 float timeOffset)
{
	if(m_pShadowController)
	{
...
static void ComputeController(JPH::Vec3 &vecCurrentVelocity, const JPH::Vec3 &vecDeltaPos, float flMaxSpeed,
							  float flMaxDampSpeed, float flScaleDelta, float flDamping,
							  JPH::Vec3 *pOutImpulse = nullptr)
{
...
static float ComputeShadowController(JoltShadowControlParams &params, JPH::Vec3 &position, JPH::Quat &rotation,
									 JPH::Vec3 &linearVelocity, JPH::Vec3 &angularVelocity, float flSecondsToArrival,
									 float flDeltaTime)
{
	...
	ComputeController(linearVelocity, deltaPosition, params.MaxSpeed, params.MaxDampSpeed, flFractionTime,
	...
	ComputeController(angularVelocity, deltaAngles, params.MaxAngular, params.MaxDampAngular, flFractionTime,
					  params.DampFactor);

	return flSecondsToArrival;
}

float JoltPhysicsObject::ComputeShadowControl(const hlshadowcontrol_params_t &params, float flSecondsToArrival,
											  float flDeltaTime)
{
	...
	float flNewSecondsToArrival =
		ComputeShadowController(joltParams, scratchPosition, scratchRotation, scratchLinearVelocity,
								scratchAngularVelocity, flSecondsToArrival, flDeltaTime);

	if(scratchPosition != position || scratchRotation != rotation)
		bodyInterface.SetPositionAndRotation(m_pBody->GetID(), scratchPosition, scratchRotation,
...
void JoltPhysicsObject::RemoveTrigger()
{
	...
	if(pEventListener && IsTrigger())
	{
		...
		{
		public:
			SourceTriggerCollector(JPH::PhysicsSystem *pPhysicsSystem, IPhysicsCollisionEvent *pGameListener,
								   JoltPhysicsObject *pTrigger)
				: m_pPhysicsSystem(pPhysicsSystem), m_pGameListener(pGameListener), m_pTrigger(pTrigger)
			{
			}
		...
		m_pPhysicsSystem->GetNarrowPhaseQueryNoLock().CollideShape(
			...
			JPH::SpecifiedObjectLayerFilter(Layers::MOVING), body_filter);
	}
...
void JoltPhysicsObject::UpdateLayer()
{
	...
	if(!bStatic && !IsControlledByGame())
	{
		...
		bodyInterface.SetMotionType(m_pBody->GetID(),
									bStaticMotionType ? JPH::EMotionType::Static : JPH::EMotionType::Dynamic,
									JPH::EActivation::Activate);
	}
