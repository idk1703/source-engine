...
{
public:
	JoltPhysicsObject(JPH::Body *pBody, JoltPhysicsEnvironment *pEnvironment, bool bStatic, int nMaterialIndex,
					  const objectparams_t *pParams);
	JoltPhysicsObject(JPH::Body *pBody, JoltPhysicsEnvironment *pEnvironment, void *pGameData,
	...
	void SetShadow(float maxSpeed, float maxAngularSpeed, bool allowPhysicsMovement,
				   bool allowPhysicsRotation) override;
	void UpdateShadow(const Vector &targetPosition, const QAngle &targetAngles, bool tempDisableGravity,
					  float timeOffset) override;
...
public:
	JoltPhysicsEnvironment *GetEnvironment()
	{
	...
	void AddCallbackFlags(uint16 flags)
	{
		m_callbackFlags |= flags;
	}
	void RemoveCallbackFlags(uint16 flags)
	{
		m_callbackFlags &= ~flags;
	}
...
private:
	...
	uint16 m_callbackFlags = CALLBACK_GLOBAL_COLLISION | CALLBACK_GLOBAL_FRICTION | CALLBACK_FLUID_TOUCH |
							 CALLBACK_GLOBAL_TOUCH | CALLBACK_GLOBAL_COLLIDE_STATIC | CALLBACK_DO_FLUID_SIMULATION;
...
{
	return (pObject0->IsStatic() ? 0.0f : pObject0->GetInvMass()) +
		   (pObject1->IsStatic() ? 0.0f : pObject1->GetInvMass());
}
