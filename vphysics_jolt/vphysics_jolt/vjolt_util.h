...
namespace JoltToSource
{
	...
	loam_expr float Unitless(float value)
	{
		return value;
	}
	loam_expr Vector Unitless(JPH::Vec3Arg value)
	{
		return Vector(value[0], value[1], value[2]);
	}
	loam_expr Vector Unitless(JPH::Float3 value)
	{
		return Vector(value[0], value[1], value[2]);
	}
	...
	loam_expr float Distance(float value)
	{
		return value * Factor;
	}
	loam_expr Vector Distance(JPH::Vec3Arg value)
	{
		return Vector(Distance(value[0]), Distance(value[1]), Distance(value[2]));
	}
	loam_expr Vector Distance(JPH::Float3 value)
	{
		return Vector(Distance(value[0]), Distance(value[1]), Distance(value[2]));
	}

	// m^2 -> in^2
	loam_expr float Area(float value)
	{
		return value * Factor * Factor;
	}
	// m^3 -> in^3
	loam_expr float Volume(float value)
	{
	...
	loam_expr Quaternion Quat(JPH::QuatArg value)
	{
		return Quaternion(value.GetX(), value.GetY(), value.GetZ(), value.GetW());
	}
	loam_expr float Angle(float value)
	{
		return RAD2DEG(value);
	}
	loam_expr QAngle Angle(JPH::QuatArg value)
	{
		return ToQAngle(Quat(value));
	}

	loam_expr float Energy(float value)
	{
		return value / (InvFactor * InvFactor);
	}

	// Converts types and handles the angle (rad -> deg) unit conversion.
	loam_expr float AngularImpulse(float value)
	{
		return Angle(value);
	}
	loam_expr Vector AngularImpulse(JPH::Vec3Arg value)
	{
		return Vector(AngularImpulse(value[0]), AngularImpulse(value[1]), AngularImpulse(value[2]));
	}

	// Misc. AABB helpers.
	loam_expr Vector AABBCenter(const JPH::AABox &aabox)
	{
		return Distance(aabox.mMin + aabox.GetExtent());
	}
	loam_expr Vector AABBHalfExtent(const JPH::AABox &aabox)
	{
		return Distance(aabox.GetExtent());
	}
...
namespace SourceToJolt
{
	...
	loam_expr JPH::Vec3 Unitless(Vector value)
	{
		return JPH::Vec3(value[0], value[1], value[2]);
	}
	loam_expr JPH::Float3 UnitlessFloat3(Vector value)
	{
		return JPH::Float3(value[0], value[1], value[2]);
	}
	...
	loam_expr JPH::Vec3 Distance(Vector value)
	{
		return JPH::Vec3(Distance(value[0]), Distance(value[1]), Distance(value[2]));
	}
	loam_expr JPH::Float3 DistanceFloat3(Vector value)
	{
		return JPH::Float3(Distance(value[0]), Distance(value[1]), Distance(value[2]));
	}
	...
	loam_expr JPH::Quat Quat(Quaternion value)
	{
	...
	loam_expr JPH::Quat Angle(QAngle value)
	{
	...
	loam_expr JPH::Vec3 AngularImpulse(Vector value)
	{
		return JPH::Vec3(AngularImpulse(value[0]), AngularImpulse(value[1]), AngularImpulse(value[2]));
	}

	// Misc. AABB helpers.
	loam_expr JPH::Vec3 AABBCenter(Vector mins, Vector maxs)
	{
		return Distance(mins + VectorHalfExtent(mins, maxs));
	}
	loam_expr JPH::Vec3 AABBHalfExtent(Vector mins, Vector maxs)
	{
		return Distance(VectorHalfExtent(mins, maxs));
	}
	...
	{
		return JPH::Mat44 {
			JPH::Vec4(GetColumn(m, MatrixAxis::X).x, GetColumn(m, MatrixAxis::X).y, GetColumn(m, MatrixAxis::X).z,
					  0.0f),
			JPH::Vec4(GetColumn(m, MatrixAxis::Y).x, GetColumn(m, MatrixAxis::Y).y, GetColumn(m, MatrixAxis::Y).z,
					  0.0f),
			JPH::Vec4(GetColumn(m, MatrixAxis::Z).x, GetColumn(m, MatrixAxis::Z).y, GetColumn(m, MatrixAxis::Z).z,
					  0.0f),
...
inline const JPH::StaticCompoundShape *GetCompoundShape(const JPH::Shape *pShape)
{
	...
	return pShape->GetType() == JPH::EShapeType::Compound ? static_cast<const JPH::StaticCompoundShape *>(pShape)
														  : nullptr;
}
...
void ActOnSubShapes(const JPH::Shape *pShape, Func ShapeFunc)
{
	...
	if(pCompoundShape)
	{
		...
		{
			JPH::Mat44 matLocalTranslation =
				JPH::Mat44::sRotationTranslation(subShape.GetRotation(), subShape.GetPositionCOM());
