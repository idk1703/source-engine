...
static ConVar vjolt_ragdoll_hinge_optimization(
	"vjolt_ragdoll_hinge_optimization", "1", FCVAR_REPLICATED,
	"Optimizes ragdolls to use hinge constraints for joints with 1 degree of freedom. Additionally fixes legs going "
	"back on themselves. Currently breaks ragdolls of NPCs killed in a pose (they inherit the pose).");
...
JoltPhysicsConstraint::JoltPhysicsConstraint(JoltPhysicsEnvironment *pPhysicsEnvironment,
											 IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject,
											 constraintType_t Type, JPH::Constraint *pConstraint, void *pGameData)
...
void JoltPhysicsConstraint::UpdateRagdollTransforms(const matrix3x4_t &constraintToReference,
...
bool JoltPhysicsConstraint::GetConstraintTransform(matrix3x4_t *pConstraintToReference,
...
{
	return Vector(uDOF == 0 ? 1.0f : 0.0f, uDOF == 1 ? 1.0f : 0.0f, uDOF == 2 ? 1.0f : 0.0f);
}
...
bool JoltPhysicsConstraint::InitialiseHingeFromRagdoll(IPhysicsConstraintGroup *pGroup,
...
void JoltPhysicsConstraint::InitialiseRagdoll(IPhysicsConstraintGroup *pGroup,
											  const constraint_ragdollparams_t &ragdoll)
{
	// Josh:
	...
	for(int i = 0; i < 3; i++)
	{
		JPH::SixDOFConstraintSettings::EAxis positionalAxis =
			static_cast<JPH::SixDOFConstraintSettings::EAxis>(JPH::SixDOFConstraintSettings::EAxis::TranslationX + i);

		JPH::SixDOFConstraintSettings::EAxis rotationalAxis =
		...
		else
		{
			if(i == 0)
			{
				...
				settings.SetLimitedAxis(rotationalAxis, DEG2RAD(ragdoll.axes[i].minRotation),
										DEG2RAD(ragdoll.axes[i].maxRotation));
			}
			else
			{
				// Josh:
...
{
	return fabsf(dir.GetX()) < 0.57f ? JPH::Vec3::sAxisX().Cross(dir).Normalized()
...
void JoltPhysicsConstraint::InitialiseHinge(IPhysicsConstraintGroup *pGroup, const constraint_hingeparams_t &hinge)
{
	...
	settings.mHingeAxis1 =
		JPH::Vec3(hinge.worldAxisDirection.x, hinge.worldAxisDirection.y, hinge.worldAxisDirection.z);
	settings.mHingeAxis2 =
		JPH::Vec3(hinge.worldAxisDirection.x, hinge.worldAxisDirection.y, hinge.worldAxisDirection.z);
...
void JoltPhysicsConstraint::InitialiseSliding(IPhysicsConstraintGroup *pGroup,
...
void JoltPhysicsConstraint::InitialiseBallsocket(IPhysicsConstraintGroup *pGroup,
												 const constraint_ballsocketparams_t &ballsocket)
{
	...
	settings.mSpace = JPH::EConstraintSpace::LocalToBodyCOM;
	settings.mPoint1 =
		SourceToJolt::Distance(ballsocket.constraintPosition[0]) - refBody->GetShape()->GetCenterOfMass();
	settings.mPoint2 =
		SourceToJolt::Distance(ballsocket.constraintPosition[1]) - attBody->GetShape()->GetCenterOfMass();
