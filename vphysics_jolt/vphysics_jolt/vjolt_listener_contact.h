...
{
public:
	JoltPhysicsContactListener(JPH::PhysicsSystem &physicsSystem) : m_PhysicsSystem(physicsSystem) {}

	JPH::ValidateResult OnContactValidate(const JPH::Body &inBody1, const JPH::Body &inBody2, JPH::Vec3Arg inBaseOffset,
						...
						JPH::ContactSettings &ioSettings) override
	{
		...
		const bool bIsShadowCollision =
			bShouldCollide && JoltPhysicsCollisionEvent::IsShadowCollision(pObject1, pObject2);
		...
		if(bIsCollision || bIsShadowCollision)
		{
			...
			// (unlike the StartTouch objects which we can get away as long as the objects themselves aren't concurrent
			// it seems) To avoid this causing locks and therefore lagging with many objects, we can just know ahead of
			// time what is going to cause a sound to play, which is hardcoded at speed > 70.0f and deltaTime < 0.05
			// (the latter of which we don't track) So we can just avoid sending these PreCollision in this case.

			const Vector vecCollideNormal =
				Vector(inManifold.mWorldSpaceNormal.GetX(), inManifold.mWorldSpaceNormal.GetY(),
					   inManifold.mWorldSpaceNormal.GetZ());
			const float flCollisionSpeed =
				JoltPhysicsCollisionEvent::GetCollisionSpeed(pObject1, pObject2, vecCollideNormal);

			const bool bHasSound = flCollisionSpeed >= 70.0f && pObject1->GetGameMaterialAllowsSounds() &&
							...
							JPH::ContactSettings &ioSettings) override
	{
		...
		// m_pGameListener->Friction( pObject1, 15500.0f, pObject1->GetMaterialIndex(), pObject2->GetMaterialIndex(),
		// &data ); m_pGameListener->Friction( pObject2, 15500.0f, pObject2->GetMaterialIndex(),
		// pObject1->GetMaterialIndex(), &data );
	}
	...
	bool ShouldCollide(JoltPhysicsObject *pObject0, JoltPhysicsObject *pObject1)
	{
		...
		if((pObject0->GetCallbackFlags() & CALLBACK_ENABLING_COLLISION) &&
		   (pObject1->GetCallbackFlags() & CALLBACK_MARKED_FOR_DELETE))
			return false;

		if((pObject1->GetCallbackFlags() & CALLBACK_ENABLING_COLLISION) &&
		   (pObject0->GetCallbackFlags() & CALLBACK_MARKED_FOR_DELETE))
			return false;
	...
	bool PreEmptGameShouldCollide(JoltPhysicsObject *pObject0, JoltPhysicsObject *pObject1)
	{
		...
		if((pObject0->GetCallbackFlags() & CALLBACK_IS_VEHICLE_WHEEL) ||
		   (pObject1->GetCallbackFlags() & CALLBACK_IS_VEHICLE_WHEEL))
			return false;
	...
	void FlushCallbacks()
	{
		...
		m_CollisionEvents.ForEach<false>(
			[this](JoltPhysicsCollisionEvent &event)
			{
				...
				JPH::Vec3 object1Vel = event.m_Data.GetPair().pObject1->FakeJoltLinearVelocity(
					event.m_Data.GetObject1PreCollisionVelocity());
				JPH::Vec3 object2Vel = event.m_Data.GetPair().pObject2->FakeJoltLinearVelocity(
					event.m_Data.GetObject2PreCollisionVelocity());
		...
		m_StartTouchEvents.ForEach<true>(
			[this](JoltPhysicsCollisionData &event)
		...
		m_FluidStartTouchEvents.ForEach<true>(
			[this](JoltPhysicsContactPair &event)
		...
		m_FluidEndTouchEvents.ForEach<true>(
			[this](JoltPhysicsContactPair &event)
...
private:
	...
	public:
		...
		JoltPhysicsCollisionInfo(JoltPhysicsObject *pObject1, JoltPhysicsObject *pObject2,
			...
			: m_CollisionPair {pObject1, pObject2}
			  // Slart: Note this negated vector, it is important, Portal 2 bouncy paint needs it negated otherwise
			  // things fly into the surface they hit
			  ,
			  m_SurfaceNormal(-Vector(inManifold.mWorldSpaceNormal.GetX(), inManifold.mWorldSpaceNormal.GetY(),
									  inManifold.mWorldSpaceNormal.GetZ())),
	...
	{
	public:
		JoltPhysicsCollisionData(const JoltPhysicsCollisionInfo &info) : m_CollisionData {info} {}
	...
	{
	public:
		JoltPhysicsCollisionEvent(const JoltPhysicsCollisionInfo &info) : m_Data {info}
		{
		...
		JoltPhysicsCollisionEvent(const JoltPhysicsCollisionEvent &other) : m_Event(other.m_Event), m_Data(other.m_Data)
		{
		...
		JoltPhysicsCollisionEvent(JoltPhysicsCollisionEvent &&other)
			: m_Event(std::move(other.m_Event)), m_Data(std::move(other.m_Data))
		{
			// Re-target the event's internal data pointer to our own structure.
			m_Event.pInternalData = &m_Data;
		}
		...
		{
			bool bIsCollision =
				(pObject1->GetCallbackFlags() & pObject2->GetCallbackFlags()) & CALLBACK_GLOBAL_COLLISION;
