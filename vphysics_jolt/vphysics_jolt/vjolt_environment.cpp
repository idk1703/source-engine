...
static ConVar vjolt_linearcast(
	"vjolt_linearcast", "1", FCVAR_NONE,
	"Whether bodies will be created with linear cast motion quality (only takes effect after map restart).");
static ConVar vjolt_initial_simulation("vjolt_initial_simulation", "0", FCVAR_NONE,
									   "Whether to pre-settle physics objects on map load.");

static ConVar vjolt_substeps_collision("vjolt_substeps_collision", "1", FCVAR_NONE,
									   "Number of collision steps to perform.", true, 0.0f, true, 4.0f);

static ConVar vjolt_baumgarte_factor(
	"vjolt_baumgarte_factor", "0.2", FCVAR_NONE,
	"Baumgarte stabilization factor (how much of the position error to 'fix' in 1 update). Changing this may help with "
	"constraint stability. Requires a map restart to change.",
	true, 0.0f, true, 1.0f);
...
public:
	...
	bool ShouldCollide(JPH::ObjectLayer inObject1, JPH::ObjectLayer inObject2) const override
	{
		switch(inObject1)
		{
			...
			case Layers::MOVING:
				return inObject2 == Layers::MOVING || inObject2 == Layers::NON_MOVING_WORLD ||
...
public:
	...
	const char *GetBroadPhaseLayerName(JPH::BroadPhaseLayer inLayer) const override
	{
		...
		{
			case(JPH::BroadPhaseLayer::Type)BroadPhaseLayers::NON_MOVING_WORLD:
				return "NON_MOVING_WORLD";
			case(JPH::BroadPhaseLayer::Type)BroadPhaseLayers::NON_MOVING_OBJECT:
				return "NON_MOVING_OBJECT";
			case(JPH::BroadPhaseLayer::Type)BroadPhaseLayers::NO_COLLIDE:
				return "NO_COLLIDE";
...
public:
	...
	bool ShouldCollide(JPH::ObjectLayer inLayer1, JPH::BroadPhaseLayer inLayer2) const override
	{
		switch(inLayer1)
		{
			...
			// MOVING collides with moving and non-moving objects.
			case Layers::MOVING:
				return inLayer2 == BroadPhaseLayers::MOVING || inLayer2 == BroadPhaseLayers::NON_MOVING_WORLD ||
					   inLayer2 == BroadPhaseLayers::NON_MOVING_OBJECT;

			// DEBRIS only collides with non-moving objects.
			case Layers::DEBRIS:
				return inLayer2 == BroadPhaseLayers::NON_MOVING_WORLD ||
					   inLayer2 == BroadPhaseLayers::NON_MOVING_OBJECT;
...
JoltPhysicsEnvironment::JoltPhysicsEnvironment() : m_ContactListener(m_PhysicsSystem)
{
	m_PerformanceParams.Defaults();

	m_PhysicsSystem.Init(kMaxBodies, kNumBodyMutexes, kMaxBodyPairs, kMaxContactConstraints, s_BroadPhaseLayerInterface,
						 s_BroadPhaseFilter, s_LayerPairFilter);
	...
	m_PhysicsSystem.SetCombineFriction([](const JPH::Body &inBody1, const JPH::SubShapeID &inSubShapeID1,
										  const JPH::Body &inBody2, const JPH::SubShapeID &inSubShapeID2) -> float
	...
	m_PhysicsSystem.SetCombineRestitution(
		[](const JPH::Body &inBody1, const JPH::SubShapeID &inSubShapeID1, const JPH::Body &inBody2,
		   const JPH::SubShapeID &inSubShapeID2) -> float
...
IPhysicsObject *JoltPhysicsEnvironment::CreatePolyObject(const CPhysCollide *pCollisionModel, int materialIndex,
														 ...
														 objectparams_t *pParams)
{
	...
	JPH::BodyCreationSettings settings(pShape, SourceToJolt::Distance(position), SourceToJolt::Angle(angles),
	...
	settings.mOverrideMassProperties =
		JPH::EOverrideMassProperties::CalculateInertia; // JPH::EOverrideMassProperties::MassAndInertiaProvided;
...
IPhysicsObject *JoltPhysicsEnvironment::CreatePolyObjectStatic(const CPhysCollide *pCollisionModel, int materialIndex,
															   const Vector &position, const QAngle &angles,
															   objectparams_t *pParams)
{
	objectparams_t params = NormalizeObjectParams(pParams);

	JPH::BodyCreationSettings settings(pCollisionModel->ToShape(), SourceToJolt::Distance(position),
									   SourceToJolt::Angle(angles), JPH::EMotionType::Static, Layers::NON_MOVING_WORLD);
...
IPhysicsObject *JoltPhysicsEnvironment::CreateSphereObject(float radius, int materialIndex, const Vector &position,
														   const QAngle &angles, objectparams_t *pParams, bool isStatic)
{
	objectparams_t params = NormalizeObjectParams(pParams);
...
IPhysicsFluidController *JoltPhysicsEnvironment::CreateFluidController(IPhysicsObject *pFluidObject,
																	   fluidparams_t *pParams)
{
	JoltPhysicsObject *pJoltObject = static_cast<JoltPhysicsObject *>(pFluidObject);
	JoltPhysicsFluidController *pFluidController =
		new JoltPhysicsFluidController(&m_PhysicsSystem, pJoltObject, pParams);
...
{
public:
	JoltPhysicsSpring(JPH::PhysicsSystem *pPhysicsSystem, JoltPhysicsObject *pObjectStart,
					  JoltPhysicsObject *pObjectEnd, springparams_t *pParams);
...
JoltPhysicsSpring::JoltPhysicsSpring(JPH::PhysicsSystem *pPhysicsSystem, JoltPhysicsObject *pObjectStart,
									 JoltPhysicsObject *pObjectEnd, springparams_t *pParams)
	  ...
	  m_OnlyStretch(pParams->onlyStretch)
{
	...
	settings.mSpace =
		pParams->useLocalPositions ? JPH::EConstraintSpace::LocalToBodyCOM : JPH::EConstraintSpace::WorldSpace;
...
IPhysicsSpring *JoltPhysicsEnvironment::CreateSpring(IPhysicsObject *pObjectStart, IPhysicsObject *pObjectEnd,
...
IPhysicsConstraint *JoltPhysicsEnvironment::CreateRagdollConstraint(IPhysicsObject *pReferenceObject,
...
IPhysicsConstraint *JoltPhysicsEnvironment::CreateHingeConstraint(IPhysicsObject *pReferenceObject,
																  IPhysicsObject *pAttachedObject,
																  IPhysicsConstraintGroup *pGroup,
...
IPhysicsConstraint *JoltPhysicsEnvironment::CreateFixedConstraint(IPhysicsObject *pReferenceObject,
																  IPhysicsObject *pAttachedObject,
																  IPhysicsConstraintGroup *pGroup,
...
IPhysicsConstraint *JoltPhysicsEnvironment::CreateSlidingConstraint(IPhysicsObject *pReferenceObject,
																	IPhysicsObject *pAttachedObject,
																	IPhysicsConstraintGroup *pGroup,
...
IPhysicsConstraint *JoltPhysicsEnvironment::CreateBallsocketConstraint(IPhysicsObject *pReferenceObject,
																	   IPhysicsObject *pAttachedObject,
																	   IPhysicsConstraintGroup *pGroup,
...
IPhysicsConstraint *JoltPhysicsEnvironment::CreatePulleyConstraint(IPhysicsObject *pReferenceObject,
																   IPhysicsObject *pAttachedObject,
																   IPhysicsConstraintGroup *pGroup,
...
IPhysicsConstraint *JoltPhysicsEnvironment::CreateLengthConstraint(IPhysicsObject *pReferenceObject,
																   IPhysicsObject *pAttachedObject,
																   IPhysicsConstraintGroup *pGroup,
...
IPhysicsShadowController *JoltPhysicsEnvironment::CreateShadowController(IPhysicsObject *pObject, bool allowTranslation,
																		 bool allowRotation)
{
	JoltPhysicsShadowController *pController =
		new JoltPhysicsShadowController(static_cast<JoltPhysicsObject *>(pObject), allowTranslation, allowRotation);
...
{
	JoltPhysicsPlayerController *pController =
		new JoltPhysicsPlayerController(static_cast<JoltPhysicsObject *>(pObject));
	m_pPhysicsControllers.push_back(pController);
	return pController;
}
...
IPhysicsVehicleController *JoltPhysicsEnvironment::CreateVehicleController(IPhysicsObject *pVehicleBodyObject,
																		   ...
																		   IPhysicsGameTrace *pGameTrace)
{
	...
	JoltPhysicsVehicleController *pController =
		new JoltPhysicsVehicleController(this, &m_PhysicsSystem, pJoltCarBodyObject, params, nVehicleType, pGameTrace);
	m_pPhysicsControllers.push_back(pController);
	return pController;
}
...
{
public:
	JoltStateRecorderSave(ISave *pSave) : m_pSave(pSave) {}

	JoltStateRecorderSave(IRestore *pRestore) : m_pRestore(pRestore) {}
...
void JoltPhysicsEnvironment::PreRestore(const physprerestoreparams_t &params)
{
	...
	for(int i = 0; i < params.recreatedObjectCount; i++)
		AddPhysicsSaveRestorePointer(reinterpret_cast<uintp>(params.recreatedObjectList[i].pOldObject),
	...
	switch(params.type)
	{
		...
		case PIID_IPHYSICSCONSTRAINT:
		{
			...
			JPH::ConstraintSettings::ConstraintResult result =
				JPH::ConstraintSettings::sRestoreFromBinaryState(recorder);
			...
			const JPH::TwoBodyConstraintSettings *pSettings =
				static_cast<const JPH::TwoBodyConstraintSettings *>(result.Get().GetPtr());
			JPH::Constraint *pConstraint =
				bodyInterface.CreateConstraint(pSettings, pRefObject->GetBodyID(), pAttObject->GetBodyID());
			pConstraint->RestoreState(recorder);
			m_PhysicsSystem.AddConstraint(pConstraint);
			JoltPhysicsConstraint *pJoltConstraint =
				new JoltPhysicsConstraint(this, pRefObject, pAttObject, type, pConstraint, params.pGameData);
...
void JoltPhysicsEnvironment::TraceRay(const Ray_t &ray, unsigned int fMask, IPhysicsTraceFilter *pTraceFilter,
									  trace_t *pTrace)
{
	// Josh: This does nothing in regular vphysics.
}

void JoltPhysicsEnvironment::SweepCollideable(const CPhysCollide *pCollide, const Vector &vecAbsStart,
											  const Vector &vecAbsEnd, const QAngle &vecAngles, unsigned int fMask,
											  IPhysicsTraceFilter *pTraceFilter, trace_t *pTrace)
{
	// Josh: This does nothing in regular vphysics.
}
...
void JoltPhysicsEnvironment::SerializeObjectToBuffer(IPhysicsObject *pObject, unsigned char *pBuffer,
...
IPhysicsObject *JoltPhysicsEnvironment::UnserializeObjectFromBuffer(void *pGameData, unsigned char *pBuffer,
																	unsigned int bufferSize, bool enableCollisions)
{
...
void JoltPhysicsEnvironment::HandleDebugDumpingEnvironment(void *pReturnAddress)
{
	...
	VJoltAssertMsg(!StringHasPrefix(pszModuleFileName, "vphysics"),
				   "Should never get vphysics as the calling module, this only looks up 1 call in the stack.");
