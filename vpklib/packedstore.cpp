...
CFileHeaderFixedData *CPackedStore::FindFileEntry(char const *pDirname, char const *pBaseName, char const *pExtension,
												  uint8 **pExtBaseOut, uint8 **pNameBaseOut)
{
	if(pExtBaseOut)
		*pExtBaseOut = NULL;
	if(pNameBaseOut)
...
CPackedStore::CPackedStore(char const *pFileBasename, char *pszFName, IBaseFileSystem *pFS, bool bOpenForWrite)
	: m_PackedStoreReadCache(pFS)
{
	Init();
	m_pFileSystem = pFS;
	...
	if(pFileBasename)
	{
		...
		if(dirFile.IsOk())
		{
			...
			if((dirFile.Read(&dirHeader, sizeof(dirHeader)) == sizeof(dirHeader)) &&
			   (dirHeader.m_nHeaderMarker == VPK_HEADER_MARKER))
			{
				if(dirHeader.m_nVersion == VPK_PREVIOUS_VERSION)
				{
					...
					dirHeader.m_nEmbeddedChunkSize =
						dirFile.Size() - dirHeader.m_nDirectorySize - sizeof(VPKDirHeaderOld_t);
			...
			if(dirHeader.m_nSignatureSize != 0)
			{
				...
				{
					Error("Size mismatch determining size of signed data block (%d vs %d)", m_nSizeOfSignedData,
						  nExpectedSignedSize);
				}
...
void CPackedStore::Write(void)
{
	...
	if(m_SignaturePrivateKey.Count() > 0 && m_SignaturePublicKey.Count() > 0)
	{
		...
		headerOut.m_nSignatureSize =
			sizeof(uint32) + m_SignaturePublicKey.Count() + sizeof(uint32) + nExpectedSignatureSize;
#else
	...
	if(headerOut.m_nSignatureSize > 0)
	{
		...
		if(!CCrypto::RSASignSHA256((const uint8 *)bufDirFile.Base(), bufDirFile.TellPut(), (uint8 *)m_Signature.Base(),
								   &cubSignature, (const uint8 *)m_SignaturePrivateKey.Base(),
								   m_SignaturePrivateKey.Count()))
		{
		...
		{
			Error("VPK signing produced %d byte signature.  Expected size was %d bytes", cubSignature,
				  nExpectedSignatureSize);
		}
		...
		{
			Error("VPK signature verification failed immediately after signing.  The public key might be invalid, or "
				  "might not match the private key used to generate the signature.");
		}
...
void CPackedStore::SetKeysForSigning(int nPrivateKeySize, const void *pPrivateKeyData, int nPublicKeySize,
									 const void *pPublicKeyData)
{
...
CPackedStore::ESignatureCheckResult CPackedStore::CheckSignature(int nSignatureSize, const void *pSignature) const
{
	...
	{
		if(m_SignaturePublicKey.Count() != nSignatureSize ||
		   V_memcmp(pSignature, m_SignaturePublicKey.Base(), nSignatureSize) != 0)
		{
...
bool CPackedStoreReadCache::BCanSatisfyFromReadCache(uint8 *pOutData, CPackedStoreFileHandle &handle,
													 FileHandleTracker_t &fHandle, int nDesiredPos, int nNumBytes,
													 int &nRead)
{
	nRead = 0;
...
bool CPackedStoreReadCache::ReadCacheLine(FileHandleTracker_t &fHandle, CachedVPKRead_t &cachedVPKRead)
{
	...
	ReadFile(fHandle.m_hFileHandle, cachedVPKRead.m_pubBuffer, k_cubCacheBufferSize,
			 (LPDWORD)&cachedVPKRead.m_cubBuffer, NULL);
	...
	cachedVPKRead.m_cubBuffer =
		m_pFileSystem->Read(cachedVPKRead.m_pubBuffer, k_cubCacheBufferSize, fHandle.m_hFileHandle);
	...
	{
		cachedVPKRead.m_hMD5RequestHandle = m_pFileTracker->SubmitThreadedMD5Request(
			cachedVPKRead.m_pubBuffer, cachedVPKRead.m_cubBuffer, m_pPackedStore->m_PackFileID,
			cachedVPKRead.m_nPackFileNumber, cachedVPKRead.m_nFileFraction);
	}
...
bool CPackedStoreReadCache::CheckMd5Result(CachedVPKRead_t &cachedVPKRead)
{
	...
	if(!m_pPackedStore->FindFileHashFraction(cachedVPKRead.m_nPackFileNumber, cachedVPKRead.m_nFileFraction,
											 chunkHashFraction))
		return true;
	...
	if(Q_memcmp(&cachedVPKRead.m_md5Value, &chunkHashFraction.m_md5contents, sizeof(MD5Value_t)) != 0)
	{
		...
		Warning("Corruption detected in %s\n"
				"\n"
				"Try verifying the integrity of your game cache.\n"
				"https://support.steampowered.com/kb_article.php?ref=2037-QEUH-3335"
				"\n"
				"Offset %d, expected %s, got %s\n",
				szFilename, cachedVPKRead.m_nFileFraction, szExpected, szCalculated);
...
// manage the cache
bool CPackedStoreReadCache::BCanSatisfyFromReadCacheInternal(uint8 *pOutData, CPackedStoreFileHandle &handle,
															 FileHandleTracker_t &fHandle, int nDesiredPos,
															 int nNumBytes, int &nRead)
{
	...
	if(idxTrackedVPKFile == m_treeCachedVPKRead.InvalidIndex() ||
	   m_treeCachedVPKRead[idxTrackedVPKFile].m_pubBuffer == NULL)
	{
		m_rwlock.UnlockRead();
...
{
	ChunkHashFraction_t chunkHashFraction;
	m_pPackedStore->FindFileHashFraction(cachedVPKRead.m_nPackFileNumber, cachedVPKRead.m_nFileFraction,
										 chunkHashFraction);
...
void CPackedStore::GetPackFileLoadErrorSummary(CUtlString &sErrors)
{
	FOR_EACH_LL(m_PackedStoreReadCache.m_listCachedVPKReadsFailed, i)
	{
		...
		Q_binarytohex(m_PackedStoreReadCache.m_listCachedVPKReadsFailed[i].m_md5Value.bits, sizeof(MD5Value_t), hex,
					  sizeof(hex));

		ChunkHashFraction_t chunkHashFraction;
		FindFileHashFraction(m_PackedStoreReadCache.m_listCachedVPKReadsFailed[i].m_nPackFileNumber,
							 m_PackedStoreReadCache.m_listCachedVPKReadsFailed[i].m_nFileFraction, chunkHashFraction);
...
int CPackedStore::ReadData(CPackedStoreFileHandle &handle, void *pOutData, int nNumBytes)
{
	...
	if(nNumBytes > 0)
	{
		...
		{
			memcpy(pOutData, reinterpret_cast<uint8 const *>(handle.m_pMetaData) + handle.m_nCurrentFileOffset,
				   nNumMetaDataBytes);
		...
		if(nNumBytes > 0)
		{
			...
			if(m_PackedStoreReadCache.BCanSatisfyFromReadCache((uint8 *)pOutData, handle, fHandle, nDesiredPos,
															   nNumBytes, nRead))
			{
...
bool CPackedStore::HashEntirePackFile(CPackedStoreFileHandle &handle, int64 &nFileSize, int nFileFraction,
									  int nFractionSize, FileHash_t &fileHash)
{
...
void CPackedStore::HashChunkFile(int iChunkFileIndex)
{
	...
	while(1)
	{
		...
		Q_memcpy(fileHashFraction.m_md5contents.bits, filehash.m_md5contents.bits,
				 sizeof(fileHashFraction.m_md5contents));
...
void CPackedStore::ComputeChunkHash(MD5Value_t &md5ChunkHashes)
{
	...
	memset(&ctx, 0, sizeof(MD5Context_t));
	MD5Init(&ctx);
	MD5Update(&ctx, (uint8 *)m_vecChunkHashFraction.Base(),
			  m_vecChunkHashFraction.Count() * sizeof(m_vecChunkHashFraction[0]));
...
#ifdef IS_WINDOWS_PC
		m_FileHandles[nFileHandleIdx].m_hFileHandle = CreateFile(pszDataFileName,		// file to open
...
ePackedStoreAddResultCode CPackedStore::AddFile(char const *pFile, uint16 nMetaDataSize, const void *pFileData,
												uint32 nFileTotalSize, bool bMultiChunk, uint32 const *pCrcValue)
{
	...
	{
		CFileHeaderFixedData *pHeader = pData.m_pHeaderData;
		if((nFileTotalSize == pHeader->TotalDataSize()) && (pHeader->m_nFileCRC == nCRC) &&
		   (nMetaDataSize == pHeader->m_nMetaDataSize)) // file unchanged?
		{
	...
	if(bMultiChunk && nBytesInChunk > 0)
	{
		...
		else
		{
			...
			if((int)dirEntry.m_iOffsetInChunk <=
			   0) // technical wrong, but we shouldn't have 2GB chunks.  (Sort of defeats the whole purpose.)
			{
...
int CPackedStore::GetFileList(const char *pWildCard, CUtlStringList &outFilenames, bool bFormattedOutput,
							  bool bSortedOutput)
{
	...
	while(*pData)
	{
		...
		while(*pData)
		{
			...
			while(*pData) // until we're out of files to look at
			{
				...
				if(bFormattedOutput)
				{
					CFileHeaderFixedData const *pHeader =
						reinterpret_cast<CFileHeaderFixedData const *>(pData + 1 + strlen(pData));
					sprintf(pszFNameOut, "%s%s%s crc=0x%x metadatasz=%d", pszCurDir, pData, pszCurExtension,
							pHeader->m_nFileCRC, pHeader->m_nMetaDataSize);
					CFilePartDescr const *pPart = &(pHeader->m_PartDescriptors[0]);
					while(pPart->m_nFileNumber != PACKFILEINDEX_END)
					{
						sprintf(pszFNameOut + strlen(pszFNameOut), " fnumber=%d ofs=0x%x sz=%d", pPart->m_nFileNumber,
								pPart->m_nFileDataOffset, pPart->m_nFileDataSize);
						pPart++;
					}
				}
				else
				...
				if(pWildCard)
				{
					...
					matches = matches && (!V_strlen(szWildCardExt) || bNoExtWildcard
											  ? 0 == V_strnicmp(szFNameOutExt, szWildCardExt, strlen(szWildCardExt))
											  : 0 != V_stristr(szFNameOutExt, szWildCardExt));
					matches = matches && (!V_strlen(szWildCardBase) || bNoBaseWildcard
											  ? 0 == V_strnicmp(szFNameOutBase, szWildCardBase, strlen(szWildCardBase))
											  : 0 != V_stristr(szFNameOutBase, szWildCardBase));
				}
...
int CPackedStore::GetFileAndDirLists(const char *pWildCard, CUtlStringList &outDirnames, CUtlStringList &outFilenames,
									 bool bSortedOutput)
{
	...
	if(pWildCard)
	{
		...
		FOR_EACH_VEC(m_directoryList, i)
		{
			   ...
			   (!nLenWildcardPath && (0 == V_strlen(m_directoryList[i]))))
			{
				...
				if(bExtWildcard)
					bExtMatch =
						true; // The extension is the wildcard ("*"), so whatever we have as the extension matches
				else
					bExtMatch = (NULL == szSubDirExtension && '\0' == *szWildCardExt) ||
								((NULL != szSubDirExtension) &&
								 (0 == V_strnicmp(szSubDirExtension, szWildCardExt, nLenWildcardExt)));
		...
		FOR_EACH_VEC(m_directoryList, i)
		{
			...
			if(!V_strnicmp(szWildCardPath, m_directoryList[i], sizeof(szWildCardPath)))
			{
				...
				FOR_EACH_VEC(filesInDirectory, iFile)
				{
					...
					if(c > 0)
						continue;
					matches = ((nLenWildcardExt <= 0) || bBaseWildcard
								   ? 0 == V_strnicmp(szFNameOutExt, szWildCardExt, nLenWildcardExt)
								   : V_stristr(szFNameOutExt, szWildCardExt) != NULL);
