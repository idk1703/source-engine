// BE VERY VERY CAREFUL what you do in these function. They are extremely hot, and calling the wrong GL API's in here
// will crush perf. (especially on NVidia threaded drivers).
...
FORCEINLINE GLuint GLMContext::FindSamplerObject(const GLMTexSamplingParams &desiredParams)
{
	...
	while((m_samplerObjectHash[h].m_params.m_bits != desiredParams.m_bits) ||
		  (m_samplerObjectHash[h].m_params.m_borderColor != desiredParams.m_borderColor))
	{
...
FORCEINLINE void GLMContext::FlushDrawStates(uint nStartIndex, uint nEndIndex,
											 uint nBaseVertex) // shadersOn = true for draw calls, false for clear calls
{
	Assert(m_drawingLang == kGLMGLSL); // no support for ARB shaders right now (and NVidia reports that they aren't
									   // worth targeting under Windows/Linux for various reasons anyway)
	...
	if(m_caps.m_hasGammaWrites)
	{
		...
		if(draw_srgb)
		{
			...
			else
			{
				...
				GLMPRINTF(("-Z- srgb-enabled FBO conflict: attached tex %08x [%s] is not SRGB",
						   m_boundDrawFBO[0].m_attach[0].m_tex,
						   m_boundDrawFBO[0].m_attach[0].m_tex->m_layout->m_layoutSummary));
		...
		else
		{
			if(tex0_srgb)
			{
				...
				// GLMPRINTF(( "-Z- srgb-disabled FBO conflict: attached tex %08x [%s] is SRGB",
				// m_boundFBO[0].m_attach[0].m_tex, m_boundFBO[0].m_attach[0].m_tex->m_layout->m_layoutSummary ));
	...
	Assert((m_drawingProgram[kGLMVertexProgram]->m_type == kGLMVertexProgram) &&
		   (m_drawingProgram[kGLMFragmentProgram]->m_type == kGLMFragmentProgram));
	Assert(m_drawingProgram[kGLMVertexProgram]->m_bTranslatedProgram &&
		   m_drawingProgram[kGLMFragmentProgram]->m_bTranslatedProgram);
	...
	for(int nSamplerIndex = 0; nSamplerIndex < GLM_SAMPLER_COUNT; ++nSamplerIndex, nCurMask <<= 1)
	{
		...
		if(m_samplers[nSamplerIndex].m_pBoundTex->m_layout->m_mipCount == 1)
		{
			...
			{
				GLMDebugPrintf("Sampler %u has mipmap filtering enabled on a texture without mipmaps! (texture name: "
							   "%s, pixel shader: %s)!\n",
							   nSamplerIndex,
							   m_samplers[nSamplerIndex].m_pBoundTex->m_debugLabel
								   ? m_samplers[nSamplerIndex].m_pBoundTex->m_debugLabel
								   : "?",
		...
		if(m_samplers[nSamplerIndex].m_pBoundTex->m_layout->m_mipCount == 1)
		{
			if(m_samplers[nSamplerIndex].m_samp.m_packed.m_mipFilter == D3DTEXF_LINEAR)
			{
				// Note this is not always an error - shadow buffer debug visualization shaders purposely want to read
				// shadow depths (and not do the comparison)
				GLMDebugPrintf("Sampler %u has mipmap filtering enabled on a texture without mipmaps! (texture name: "
							   "%s, pixel shader: %s)!\n",
							   nSamplerIndex,
							   m_samplers[nSamplerIndex].m_pBoundTex->m_debugLabel
		...
		bool bSamplerIsDepth =
			(m_samplers[nSamplerIndex].m_pBoundTex->m_layout->m_key.m_texFlags & kGLMTexIsDepth) != 0;
		...
		if(bShaderShadow)
		{
			...
			{
				// FIXME: This occasionally occurs in L4D2 when CShaderAPIDx8::ExecuteCommandBuffer() sets the
				// TEXTURE_WHITE texture in the flashlight depth texture slot.
				GLMDebugPrintf("Sampler %u's compare mode (%u) or format (depth=%u) is not consistent with pixel "
							   "shader's compare mode (%u) (texture name: %s, pixel shader: %s)!\n",
		...
		else
		{
			...
			if(bSamplerIsDepth)
			{
				GLMDebugPrintf("Sampler %u is a depth texture but the pixel shader's shadow depth sampler mask does "
							   "not expect depth here (texture name: %s, pixel shader: %s)!\n",
							   nSamplerIndex,
							   m_samplers[nSamplerIndex].m_pBoundTex->m_debugLabel
								   ? m_samplers[nSamplerIndex].m_pBoundTex->m_debugLabel
								   : "?",
	...
	if(m_bDirtyPrograms)
	{
		...
		CGLMShaderPair *pNewPair = m_pairCache->SelectShaderPair(m_drawingProgram[kGLMVertexProgram],
																 m_drawingProgram[kGLMFragmentProgram], 0);
		...
		if(pNewPair != m_pBoundPair)
		{
			...
			// set the dirty levels appropriately since the program changed and has never seen any of the current
			// values.
	...
	if(m_bUseBoneUniformBuffers)
	{
		...
		if(m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone)
		{
			...
			int dirtySlotHighWater = MIN(m_drawingProgram[kGLMVertexProgram]->m_descs[kGLMGLSL].m_highWater,
										 m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone);
			...
			if((vconstLoc >= 0) && (dirtySlotHighWater > firstDirtySlot))
			{
				...
				// consts after the bones (c217 onwards), since we use the concatenated destination array vc[], upload
				// these consts starting from vc[58]
				if(numSlots > 0)
				{
					gGL->glUniform4fv(
						m_pBoundPair->m_UniformBufferParams[kGLMVertexProgram][DXABSTRACT_VS_FIRST_BONE_SLOT], numSlots,
						&m_programParamsF[kGLMVertexProgram].m_values[(DXABSTRACT_VS_LAST_BONE_SLOT + 1)][0]);
				...
				if(numSlots > 0)
				{
					gGL->glUniform4fv(m_pBoundPair->m_UniformBufferParams[kGLMVertexProgram][firstDirtySlot],
									  dirtySlotHighWater - firstDirtySlot,
									  &m_programParamsF[kGLMVertexProgram].m_values[firstDirtySlot][0]);
		...
		if(m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterBone)
		{
			...
			if(vconstBoneLoc >= 0)
			{
				...
				if((m_drawingProgram[kGLMVertexProgram]->m_descs[kGLMGLSL].m_VSHighWaterBone > 0) &&
				   (m_nMaxUsedVertexProgramConstantsHint > DXABSTRACT_VS_FIRST_BONE_SLOT))
				{
					shaderSlotsBone = MIN(m_drawingProgram[kGLMVertexProgram]->m_descs[kGLMGLSL].m_VSHighWaterBone,
										  m_nMaxUsedVertexProgramConstantsHint - DXABSTRACT_VS_FIRST_BONE_SLOT);
				}

				int dirtySlotHighWaterBone =
					MIN(shaderSlotsBone, m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterBone);
				if(dirtySlotHighWaterBone)
				{
					...
					gGL->glUniform4fv(vconstBoneLoc, nNumBoneRegs,
									  &m_programParamsF[kGLMVertexProgram].m_values[DXABSTRACT_VS_FIRST_BONE_SLOT][0]);
	...
	else
	{
		if(m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone)
		{
			...
			int firstDirtySlot = m_programParamsF[kGLMVertexProgram].m_firstDirtySlotNonBone;
			int dirtySlotHighWater =
				MIN(nMaxUsedShaderSlots, m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone);
	...
	{
		if(m_programParamsB[kGLMVertexProgram]
			   .m_dirtySlotCount) // optimize this later after the float param pushes are proven out
		{
			const uint nLimit =
				MIN(CGLMShaderPair::cMaxVertexShaderBoolUniforms, m_programParamsB[kGLMVertexProgram].m_dirtySlotCount);
		...
		if(m_programParamsB[kGLMFragmentProgram]
			   .m_dirtySlotCount) // optimize this later after the float param pushes are proven out
		{
			const uint nLimit = MIN(CGLMShaderPair::cMaxFragmentShaderBoolUniforms,
									m_programParamsB[kGLMFragmentProgram].m_dirtySlotCount);
			for(uint i = 0; i < nLimit; ++i)
			{
		...
		if(m_programParamsI[kGLMVertexProgram].m_dirtySlotCount)
		{
			...
			if(vconstInt0Loc >= 0)
			{
				gGL->glUniform1i(vconstInt0Loc,
								 m_programParamsI[kGLMVertexProgram].m_values[0][0]); // FIXME magic number
			}
			m_programParamsI[kGLMVertexProgram].m_dirtySlotCount = 0;
		}
	}

	Assert((m_pDevice->m_streams[0].m_vtxBuffer &&
			(m_pDevice->m_streams[0].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[0])) ||
		   ((!m_pDevice->m_streams[0].m_vtxBuffer) && (m_pDevice->m_vtx_buffers[0] == m_pDevice->m_pDummy_vtx_buffer)));
	Assert((m_pDevice->m_streams[1].m_vtxBuffer &&
			(m_pDevice->m_streams[1].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[1])) ||
		   ((!m_pDevice->m_streams[1].m_vtxBuffer) && (m_pDevice->m_vtx_buffers[1] == m_pDevice->m_pDummy_vtx_buffer)));
	Assert((m_pDevice->m_streams[2].m_vtxBuffer &&
			(m_pDevice->m_streams[2].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[2])) ||
		   ((!m_pDevice->m_streams[2].m_vtxBuffer) && (m_pDevice->m_vtx_buffers[2] == m_pDevice->m_pDummy_vtx_buffer)));
	Assert((m_pDevice->m_streams[3].m_vtxBuffer &&
			(m_pDevice->m_streams[3].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[3])) ||
		   ((!m_pDevice->m_streams[3].m_vtxBuffer) && (m_pDevice->m_vtx_buffers[3] == m_pDevice->m_pDummy_vtx_buffer)));

	uint nCurTotalBufferRevision;
	nCurTotalBufferRevision = m_pDevice->m_vtx_buffers[0]->m_nRevision + m_pDevice->m_vtx_buffers[1]->m_nRevision +
							  m_pDevice->m_vtx_buffers[2]->m_nRevision + m_pDevice->m_vtx_buffers[3]->m_nRevision;

	// If any of these inputs have changed, we need to enumerate through all of the expected GL vertex attribs and
	// modify anything in the GL layer that have changed. This is not always a win, but it is a net win on NVidia (by
	// 1-4.8% depending on whether driver threading is enabled).
	if((nCurTotalBufferRevision != m_CurAttribs.m_nTotalBufferRevision) ||
	   (m_CurAttribs.m_pVertDecl != m_pDevice->m_pVertDecl) ||
	   (m_CurAttribs.m_vtxAttribMap[0] !=
		reinterpret_cast<const uint64 *>(m_pDevice->m_vertexShader->m_vtxAttribMap)[0]) ||
	   (m_CurAttribs.m_vtxAttribMap[1] !=
		reinterpret_cast<const uint64 *>(m_pDevice->m_vertexShader->m_vtxAttribMap)[1]) ||
	   (memcmp(m_CurAttribs.m_streams, m_pDevice->m_streams, sizeof(m_pDevice->m_streams)) != 0))
	{
		...
		for(int nMask = 1, nIndex = 0; nIndex < nMaxVertexAttributesToCheck; ++nIndex, nMask <<= 1)
		{
			...
			SetBufAndVertexAttribPointer(
				nIndex, pBuf->GetHandle(), pStream->m_stride, pDeclElem->m_gldecl.m_datatype,
				pDeclElem->m_gldecl.m_normalized, pDeclElem->m_gldecl.m_nCompCount,
				reinterpret_cast<const GLvoid *>(reinterpret_cast<uintp>(pBuf->m_pPseudoBuf) + nBufOffset),
				pBuf->m_nRevision);
	...
	if(m_programParamsF[kGLMFragmentProgram].m_dirtySlotHighWaterNonBone)
	{
		...
		if(fconstLoc >= 0)
		{
			...
			int dirtySlotHighWater =
				MIN(nMaxUsedShaderSlots, m_programParamsF[kGLMFragmentProgram].m_dirtySlotHighWaterNonBone);
			...
			if(dirtySlotHighWater > firstDirtySlot)
			{
				...
				gGL->glUniform4fv(m_pBoundPair->m_UniformBufferParams[kGLMFragmentProgram][firstDirtySlot],
								  dirtySlotHighWater - firstDirtySlot,
								  &m_programParamsF[kGLMFragmentProgram].m_values[firstDirtySlot][0]);
