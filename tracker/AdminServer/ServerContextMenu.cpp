...
//-----------------------------------------------------------------------------
void CServerContextMenu::ShowMenu(Panel *target, unsigned int serverID, bool showConnect, bool showRefresh,
								  bool showAddToFavorites, bool manage)
{
	...
	{
		displayed = true;
		AddMenuItem("ConnectToServer", "&Connect to server", new KeyValues("ConnectToServer", "serverID", serverID),
					target);
	...
	{
		displayed = true;
		AddMenuItem("AddToFavorites", "&Add server to favorites", new KeyValues("AddToFavorites", "serverID", serverID),
					target);
	}
	...
	{
		displayed = true;
		AddMenuItem("Manage", "&Manage Server", new KeyValues("Manage", "serverID", serverID),
					CServerContextMenu::parent);
	}

	/*	if (parent->IsCursorOver() ) // if the cursor is over the tabs
		{
			displayed=false;
			if((int) parent->GetTabPanel()->GetActivePageNum()!=0) // don't let the first tab be deleted, its our
	   servers tab :)
			{
				char name[100];
				displayed=true;
				...
				AddMenuItem("Delete", name,new KeyValues("DeleteServer", "panelid",(int)
	   parent->GetTabPanel()->GetActivePageNum()), CServerContextMenu::parent);
			}
		} */
