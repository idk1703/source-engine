...
float CPhysics_Airboat::ComputeFrontPontoonWaveNoise(int nPontoonIndex, float flSpeedRatio)
{
	...
	IVP_FLOAT flNoiseScale =
		RemapValClamped(1.0 - flSpeedRatio, 0, 1, AIRBOAT_WATER_NOISE_MIN, AIRBOAT_WATER_NOISE_MAX);
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::pre_raycasts_gameside(int nRaycastCount, IVP_Ray_Solver_Template *pRays, Ray_t *pGameRays,
											 IVP_Raycast_Airboat_Impact *pImpacts)
{
	...
	if(nFrontPontoonsInWater == 2)
	{
		for(int i = 0; i < 2; i++)
		{
			...
			pRays[i].ray_length = RemapValClamped(flForwardSpeedRatio, 0, 1, AIRBOAT_RAYCAST_DIST_WATER_LOW,
												  AIRBOAT_RAYCAST_DIST_WATER_HIGH);
											...
											IVP_Raycast_Airboat_Impact *pImpacts)
{
	...
	for(int iRaycast = 0; iRaycast < nRaycastCount; ++iRaycast)
	{
		...
		ConvertPositionToIVP(gameRays[iRaycast].m_Start + gameRays[iRaycast].m_StartOffset,
							 m_CarSystemDebugData.wheelRaycasts[iRaycast][0]);
		ConvertPositionToIVP(gameRays[iRaycast].m_Start + gameRays[iRaycast].m_StartOffset + gameRays[iRaycast].m_Delta,
							 m_CarSystemDebugData.wheelRaycasts[iRaycast][1]);
...
//			pImpacts -
//-----------------------------------------------------------------------------
void CPhysics_Airboat::PreRaycasts(IVP_Ray_Solver_Template *pRaySolverTemplates, const IVP_U_Matrix *matWorldFromCore,
								   IVP_Raycast_Airboat_Impact *pImpacts)
{
									...
									IVP_Raycast_Airboat_Impact *pImpacts)
{
	...
	for(int iPoint = 0; iPoint < nPontoonPoints; ++iPoint)
	{
		...
		if(pImpact->bImpact)
		{
			...
			// Get the inverse portion of the surface normal in the direction of the ray cast (shock - used in the shock
			// simulation code for the sign and percentage of force applied to the shock).
			pImpact->inv_normal_dot_dir =
				1.1f /
				(IVP_Inline_Math::fabsd(pImpact->raycast_dir_ws.dot_product(&pImpact->vecImpactNormalWS)) + 0.1f);
		...
		// Set the new wheel position (the impact point or the full ray distance).  Make this from the wheel not the ray
		// trace position.
		pImpact->vecImpactPointWS.add_multiple(&pRaySolver->ray_start_point, &pImpact->raycast_dir_ws,
		...
		pImpact->projected_surface_speed_wheel_ws.set_orthogonal_part(&pImpact->surface_speed_wheel_ws,
																	  &pImpact->vecImpactNormalWS);

		matWorldFromCore->vmult3(&pPontoonPoint->axis_direction_cs, &pImpact->axis_direction_ws);
		pImpact->projected_axis_direction_ws.set_orthogonal_part(&pImpact->axis_direction_ws,
																 &pImpact->vecImpactNormalWS);
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysics_Airboat::PerformFrictionNotification(float flEliminatedEnergy, float dt, int nSurfaceProp,
												   IPhysicsCollisionData *pCollisionData)
{
	...
	{
		pEventHandler->Friction(pPhysAirboat, flEliminatedEnergy, pPhysAirboat->GetMaterialIndexInternal(),
								nSurfaceProp, pCollisionData);
	}
}
...
//-----------------------------------------------------------------------------
void CPhysics_Airboat::DoSimulationDrag(IVP_Raycast_Airboat_Impact *pImpacts, IVP_Event_Sim *pEventSim)
{
	const IVP_U_Matrix *matWorldFromCore = m_pCore->get_m_world_f_core_PSI();
	IVP_FLOAT flSpeed = (IVP_FLOAT)m_pCore->speed.real_length();
	...
	// Msg("(water=%d/land=%d) speed=%f (%f %f %f)\n", nPointsInWater, nPointsOnGround, flDebugSpeed,
	// vecAirboatDirLS.k[0], vecAirboatDirLS.k[1], vecAirboatDirLS.k[2]);
	...
	if(nPointsOnGround && (flSpeed > 0))
	{
		...
		IVP_FLOAT flFrictionDrag =
			m_pCore->get_mass() * AIRBOAT_GRAVITY * AIRBOAT_DRY_FRICTION_SCALE * flGroundFriction;
...
void CPhysics_Airboat::DoSimulationSteering(IVP_Event_Sim *pEventSim)
{
	...
	if(fabsf(m_SteeringAngle) > 0.01)
	{
		...
		if(!m_bAnalogSteering)
		{
			...
			flSteerScale = RemapValClamped(m_flSteerTime, 0, AIRBOAT_STEERING_INTERVAL, AIRBOAT_STEERING_RATE_MIN,
										   AIRBOAT_STEERING_RATE_MAX);
		}
		else // consoles
		{
			// Analog steering
			flSteerScale = RemapValClamped(fabs(m_SteeringAngle), 0, AIRBOAT_STEERING_INTERVAL,
										   AIRBOAT_STEERING_RATE_MIN, AIRBOAT_STEERING_RATE_MAX);
		}
	...
	IVP_FLOAT flRotationalDrag = AIRBOAT_ROT_DRAG * m_pCore->rot_speed.k[1] * m_pCore->rot_speed.k[1] *
								 m_pCore->get_mass() * pEventSim->i_delta_time;
	flRotationalDrag *= flRotSpeedSign;

	// Apply dampening proportional to angular velocity.
	IVP_FLOAT flRotationalDamping =
		AIRBOAT_ROT_DAMPING * fabs(m_pCore->rot_speed.k[1]) * m_pCore->get_mass() * pEventSim->i_delta_time;
...
void CPhysics_Airboat::DoSimulationKeepUprightPitch(IVP_Raycast_Airboat_Impact *pImpacts, IVP_Event_Sim *pEventSim)
{
	...
	// Msg("angle: %.2f, axis: (%.2f %.2f %.2f)\n", RAD2DEG(angle), vecRotAxisCS.k[0], vecRotAxisCS.k[1],
	// vecRotAxisCS.k[2]);
	...
	vecAngularImpulse.set_multiple(&vecRotAxisCS,
								   m_pCore->get_mass() *
									   (0.1f * angle + 0.04f * pEventSim->i_delta_time * (angle - m_flPitchErrorPrev)));
	...
	debugoverlay->AddLineOverlay(vecPosHL + Vector(0, 0, 10) * flDebugLen,
								 vecPosHL + Vector(0, 0, 10) * flDebugLen + vecGoalAxisHL * 10, 0, 255, 0, false,
								 100.0);
	debugoverlay->AddLineOverlay(vecPosHL + Vector(0, 0, 10) * flDebugLen,
								 vecPosHL + Vector(0, 0, 10) * flDebugLen + vecCurHL * 10, 255, 0, 0, false, 100.0);
...
void CPhysics_Airboat::DoSimulationKeepUprightRoll(IVP_Raycast_Airboat_Impact *pImpacts, IVP_Event_Sim *pEventSim)
{
	...
	vecAngularImpulse.set_multiple(&vecRotAxisCS,
								   m_pCore->get_mass() *
									   (0.2f * angle + 0.3f * pEventSim->i_delta_time * (angle - m_flRollErrorPrev)));
