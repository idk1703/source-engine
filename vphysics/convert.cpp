...
vphysics_units_t g_PhysicsUnits = {
	...
	DIST_EPSILON,			// float		collisionSweepEpsilon;		// collision sweep tests clip at this, must be the same as
							// engine's DIST_EPSILON
	1.0f / 256.0f, // float		collisionSweepIncrementalEpsilon;	// near-zero test for incremental steps in collision
				   // sweep tests
};
#else
// game is in meters
vphysics_units_t g_PhysicsUnits = {
	...
	0.01f, // float		collisionSweepEpsilon;		// collision sweep tests clip at this, must be the same as engine's
		   // DIST_EPSILON
	1e-4f, // float		collisionSweepIncrementalEpsilon;	// near-zero test for incremental steps in collision sweep
		   // tests
};
#endif
...
void TransformLocalToIVP(const IVP_U_Float_Point &pointIn, IVP_U_Point &pointOut, IVP_Real_Object *pObject,
						 bool translate)
{
#if USE_CACHE_OBJECT
	IVP_Cache_Object *cache = pObject->get_cache_object_no_lock();
...
void TransformLocalToIVP(const IVP_U_Float_Point &pointIn, IVP_U_Float_Point &pointOut, IVP_Real_Object *pObject,
						 bool translate)
{
	IVP_U_Point tmpOut;
