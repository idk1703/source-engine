...
public:
	...
	void ConvexesFromConvexPolygon(const Vector &vPolyNormal, const Vector *pPoints, int iPointCount,
								   CPhysConvex **pOutput);
	...
	CPhysCollide *ConvertConvexToCollideParams(CPhysConvex **pConvex, int convexCount,
	...
	int GetConvexesUsedInCollideable(const CPhysCollide *pCollideable, CPhysConvex **pOutputArray,
	...
	void PolysoupAddTriangle(CPhysPolysoup *pSoup, const Vector &a, const Vector &b, const Vector &c,
	...
	virtual void CollideGetAABB(Vector *pMins, Vector *pMaxs, const CPhysCollide *pCollide, const Vector &collideOrigin,
								const QAngle &collideAngles);
	virtual Vector CollideGetExtent(const CPhysCollide *pCollide, const Vector &collideOrigin,
									const QAngle &collideAngles, const Vector &direction);
	...
	void TraceBox(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs,
				  const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles, trace_t *ptr);
	void TraceBox(const Ray_t &ray, const CPhysCollide *pCollide, const Vector &collideOrigin,
				  const QAngle &collideAngles, trace_t *ptr);
	void TraceBox(const Ray_t &ray, unsigned int contentsMask, IConvexInfo *pConvexInfo, const CPhysCollide *pCollide,
				  const Vector &collideOrigin, const QAngle &collideAngles, trace_t *ptr);
	// Trace one collide against another
	void TraceCollide(const Vector &start, const Vector &end, const CPhysCollide *pSweepCollide,
...
private:
	...
	CPhysCollideCompactSurface *FastBboxCollide(const CPhysCollideCompactSurface *pCollide, const Vector &mins,
												const Vector &maxs);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CPhysicsCollision, IPhysicsCollision, VPHYSICS_COLLISION_INTERFACE_VERSION,
								  g_PhysicsCollision);
...
BEGIN_BYTESWAP_DATADESC(physcollideheader_t)
	DEFINE_FIELD(vphysicsID, FIELD_INTEGER), DEFINE_FIELD(version, FIELD_SHORT), DEFINE_FIELD(modelType, FIELD_SHORT),
...
public:
	...
	// IPhysCollide
	virtual int GetVCollideIndex() const
	{
		return 0;
	}
...
public:
	...
	virtual int GetVCollideIndex() const
	{
		return m_pCompactSurface->dummy[0];
	}
	virtual IVP_SurfaceManager *CreateSurfaceManager(short &) const;
	virtual void GetAllLedges(IVP_U_BigVector<IVP_Compact_Ledge> &ledges) const;
	virtual unsigned int GetSerializationSize() const;
	...
	const IVP_Compact_Surface *GetCompactSurface() const
	{
		return m_pCompactSurface;
	}
	virtual const collidemap_t *GetCollideMap() const
	{
		return m_pCollideMap;
	}
...
CPhysCollide *CPhysCollide::UnserializeFromBuffer(const char *pBuffer, unsigned int size, int index, bool swap)
{
	...
	if(pSurface->dummy[2] == IVP_COMPACT_SURFACE_ID || pSurface->dummy[2] == IVP_COMPACT_SURFACE_ID_SWAPPED ||
	   pSurface->dummy[2] == 0)
	{
...
CPhysConvex *CPhysicsCollision::RebuildConvexFromPlanes(CPhysConvex *pConvex, float mergeTolerance)
{
	...
	IVP_Compact_Ledge *pLedgeOut =
		IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(&halfspaces, mergeTolerance);
...
CPhysConvex *CPhysicsCollision::ConvexFromPlanes(float *pPlanes, int planeCount, float mergeDistance)
{
	...
	IVP_Compact_Ledge *pLedge =
		IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(&halfspaces, mergeDistance);
	return reinterpret_cast<CPhysConvex *>(pLedge);
}

CPhysConvex *CPhysicsCollision::ConvexFromConvexPolyhedron(const CPolyhedron &ConvexPolyhedron)
{
	IVP_Template_Polygon polyTemplate(ConvexPolyhedron.iVertexCount, ConvexPolyhedron.iLineCount,
									  ConvexPolyhedron.iPolygonCount);
	...
	for(int i = 0; i != ConvexPolyhedron.iLineCount; ++i)
		polyTemplate.lines[i].set(ConvexPolyhedron.pLines[i].iPointIndices[0],
								  ConvexPolyhedron.pLines[i].iPointIndices[1]);

	// copy polygons
	for(int i = 0; i != ConvexPolyhedron.iPolygonCount; ++i)
	{
		polyTemplate.surfaces[i].init_surface(
			ConvexPolyhedron.pPolygons[i].iIndexCount); // num vertices in a convex polygon == num lines
		...
		Polyhedron_IndexedLineReference_t *pLineReferences =
			&ConvexPolyhedron.pIndices[ConvexPolyhedron.pPolygons[i].iFirstIndex];
...
CPolyhedron *CPhysicsCollision::PolyhedronFromConvex(CPhysConvex *const pConvex, bool bUseTempPolyhedron)
{
	...
	PolyhedronMesh_Triangle *pTriangles =
		(PolyhedronMesh_Triangle *)stackalloc(iTriangles * sizeof(PolyhedronMesh_Triangle));
	...
	for(int i = 0; i != iTriangles; ++i)
	{
		...
		pTriangles[i].Edges[2].iPointIndices[1] = pTriangles[i].Edges[0].iPointIndices[0] =
			pTri->get_edge(2)->get_start_point_index();
		pTriangles[i].Edges[0].iPointIndices[1] = pTriangles[i].Edges[1].iPointIndices[0] =
			pTri->get_edge(1)->get_start_point_index();
		pTriangles[i].Edges[1].iPointIndices[1] = pTriangles[i].Edges[2].iPointIndices[0] =
			pTri->get_edge(0)->get_start_point_index();

		for(int j = 0; j != 3; ++j)
		{
			// get_n_points() has a whole bunch of ifdefs that apparently disable it in this case, detect number of
			// points
	...
	// apparently points might be shared between ledges and not all points will be used. So now we get to compress them
	// into a smaller set
	...
	int *pStartIndices =
		(int *)stackalloc(iNumPoints * sizeof(int)); // for quicker lookup of which edges to use in polygons

	pStartIndices[0] = 0; // the lowest index point drives links, so if the first point isn't the first link, then
						  // something is extremely messed up
...
int CPhysicsCollision::GetConvexesUsedInCollideable(const CPhysCollide *pCollideable, CPhysConvex **pOutputArray,
													int iOutputArrayLimit)
{
	IVP_U_BigVector<IVP_Compact_Ledge> ledges;
	...
	{
		IVP_Compact_Ledge *pLedge = ledges.element_at(
			i); // doing as a 2 step since a single convert seems more error prone (without compile error) in this case
...
void CPhysicsCollision::ConvexesFromConvexPolygon(const Vector &vPolyNormal, const Vector *pPoints, int iPointCount,
												  CPhysConvex **pOutput)
{
	...
	*pWrite = &pIVP_Points[iReverseCounter];
	...
	do
	{
		...
		*pWrite = &pIVP_Points[iForwardCounter];
		...
		pOutput[iOutputCount] = reinterpret_cast<CPhysConvex *>(
			IVP_SurfaceBuilder_Pointsoup::convert_triangle_to_compace_ledge(pRead[0], pRead[1], pRead[2]));
...
#define FAST_BBOX 1
CPhysCollideCompactSurface *CPhysicsCollision::FastBboxCollide(const CPhysCollideCompactSurface *pCollide,
...
void CPhysicsCollision::PolysoupAddTriangle(CPhysPolysoup *pSoup, const Vector &a, const Vector &b, const Vector &c,
											int materialIndex7bits)
{
...
void CPhysicsCollision::TraceBox(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs,
...
void CPhysicsCollision::TraceBox(const Ray_t &ray, const CPhysCollide *pCollide, const Vector &collideOrigin,
								 const QAngle &collideAngles, trace_t *ptr)
{
	TraceBox(ray, MASK_ALL, NULL, pCollide, collideOrigin, collideAngles, ptr);
}

void CPhysicsCollision::TraceBox(const Ray_t &ray, unsigned int contentsMask, IConvexInfo *pConvexInfo,
								 const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles,
								 trace_t *ptr)
{
...
// Trace one collide against another
void CPhysicsCollision::TraceCollide(const Vector &start, const Vector &end, const CPhysCollide *pSweepCollide,
...
Vector CPhysicsCollision::CollideGetExtent(const CPhysCollide *pCollide, const Vector &collideOrigin,
										   const QAngle &collideAngles, const Vector &direction)
{
	if(!pCollide)
...
bool CPhysicsCollision::IsBoxIntersectingCone(const Vector &boxAbsMins, const Vector &boxAbsMaxs,
