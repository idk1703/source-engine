...
public:
	...
	void Init(CPhysicsSurfaceProps *pProps)
	{
		m_props = pProps;
	}
	...
	virtual IVP_DOUBLE get_friction_factor(
		IVP_Contact_Situation *situation) // returns values >0, value of 1.0f means object stands on a 45 degres hill
	{
		...
		if(ShouldOverrideWheelContactFriction(&wheelFriction, situation->objects[0], situation->objects[1],
											  &situation->surf_normal))
		{
	...
	virtual IVP_DOUBLE get_elasticity(IVP_Contact_Situation *situation) // range [0, 1.0f[, the relative speed after a
																		// collision compared to the speed before
	{
...
public:
	...
	IVP_Material_Manager *GetIVPManager(void)
	{
		return &m_ivpManager;
	}
...
void CPhysicsSurfaceProps::GetPhysicsProperties(int materialIndex, float *density, float *thickness, float *friction,
												float *elasticity) const
{
...
IVP_Material *CIVPMaterialManager::get_material_by_index(IVP_Real_Object *pObject, const IVP_U_Point *world_position,
														 int index)
{
