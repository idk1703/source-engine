...
void ComputeController(IVP_U_Float_Point &currentSpeed, const IVP_U_Float_Point &delta, float maxSpeed,
					   float maxDampSpeed, float scaleDelta, float damping, IVP_U_Float_Point *pOutImpulse = NULL)
{
...
void ComputeController(IVP_U_Float_Point &currentSpeed, const IVP_U_Float_Point &delta,
...
public:
	...
	{
		return (IVP_CONTROLLER_PRIORITY)(IVP_CP_MOTION + 1);
	}
	...
	void Update(const Vector &position, const Vector &velocity, float secondsToArrival, bool onground,
...
public:
	bool IsFull()
	{
		return m_Normals.Count() == MAX_LIST_NORMALS;
	}
...
void CPlayerController::do_simulation_controller(IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> *)
{
	...
	delta_position.subtract(&m_targetPosition, cur_pos_ws);
	...
	{
		ComputeController(pCore->speed, delta_position, m_maxSpeed, fraction / es->delta_time, m_dampFactor,
						  &m_lastImpulse);
	}
...
void CPlayerController::Update(const Vector &position, const Vector &velocity, float secondsToArrival, bool onground,
							   IPhysicsObject *ground)
{
								 ...
								 float dt)
{
	...
	float invDt = 1.0f / dt;
	IVP_Core *pCore = pivp->get_core();
	ComputeController(pCore->speed, delta_position, params.maxSpeed, params.maxDampSpeed, fraction * invDt,
					  params.dampFactor, &params.lastImpulse);
	...
	ComputeController(pCore->rot_speed, deltaAngles, params.maxAngular, params.maxDampAngular, fraction * invDt,
					  params.dampFactor, NULL);
...
class CShadowController : public IVP_Controller_Independent,
						  public IPhysicsShadowController,
						  public CAlignedNewDelete<16>
{
public:
	CShadowController();
	...
	{
		return m_allowsTranslation;
	}
	virtual bool AllowsRotation()
	{
		return m_allowsRotation;
	}
...
BEGIN_SIMPLE_DATADESC(vphysics_save_cshadowcontroller_t)
	...
	DEFINE_FIELD(secondsToArrival, FIELD_FLOAT), DEFINE_ARRAY(saveRot.k, FIELD_FLOAT, 3),
		DEFINE_ARRAY(savedRI.k, FIELD_FLOAT, 3), DEFINE_ARRAY(currentSpeed.k, FIELD_FLOAT, 3),
		DEFINE_FIELD(savedMass, FIELD_FLOAT), DEFINE_FIELD(savedFlags, FIELD_INTEGER),
		DEFINE_CUSTOM_FIELD(savedMaterial, MaterialIndexDataOps()), DEFINE_FIELD(enable, FIELD_BOOLEAN),
		DEFINE_FIELD(allowPhysicsMovement, FIELD_BOOLEAN), DEFINE_FIELD(allowPhysicsRotation, FIELD_BOOLEAN),
...
void CShadowController::InitFromTemplate(const vphysics_save_cshadowcontroller_t &controllerTemplate)
{
	...
	m_pObject->GetObject()->get_environment()->get_controller_manager()->add_controller_to_core(
		this, m_pObject->GetObject()->get_core());
}
...
bool RestorePhysicsShadowControllerInternal(const physrestoreparams_t &params,
											IPhysicsShadowController **ppShadowController, CPhysicsObject *pObject)
{
	vphysics_save_cshadowcontroller_t controllerTemplate;
...
// HACKHACK: The physics object transfer system needs to temporarily detach a shadow controller from an object, then
// reattach without other repercussions
void ControlPhysicsShadowControllerAttachment_Silent(IPhysicsShadowController *pController, IVP_Real_Object *pivp,
													 bool bAttach)
{
	if(bAttach)
	{
		pivp->get_environment()->get_controller_manager()->add_controller_to_core((CShadowController *)pController,
																				  pivp->get_core());
	}
	else
	{
		pivp->get_environment()->get_controller_manager()->remove_controller_from_core((CShadowController *)pController,
																					   pivp->get_core());
	}
}

void ControlPhysicsPlayerControllerAttachment_Silent(IPhysicsPlayerController *pController, IVP_Real_Object *pivp,
													 bool bAttach)
{
	if(bAttach)
	{
		pivp->get_environment()->get_controller_manager()->add_controller_to_core((CPlayerController *)pController,
																				  pivp->get_core());
	}
	else
	{
		pivp->get_environment()->get_controller_manager()->remove_controller_from_core((CPlayerController *)pController,
																					   pivp->get_core());
	}
}