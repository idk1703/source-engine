...
public:
	...
	CBuoyancyAttacher(IVP_Template_Buoyancy &templ, IVP_U_Set_Active<IVP_Core> *set_of_cores_,
					  IVP_Liquid_Surface_Descriptor *liquid_surface_descriptor_);

	float m_density;
};

CPhysicsFluidController::CPhysicsFluidController(CBuoyancyAttacher *pBuoy, IVP_Liquid_Surface_Descriptor *pLiquid,
												 CPhysicsObject *pObject, int nContents)
{
	m_pBuoyancy = pBuoy;
...
void CPhysicsFluidController::GetSurfacePlane(Vector *pNormal, float *pDist) const
{
	...
	m_pLiquidSurface->calc_liquid_surface(GetIVPObject()->get_core()->environment, GetIVPObject()->get_core(), &surface,
										  &abs_speed_of_current);
...
CBuoyancyAttacher::CBuoyancyAttacher(IVP_Template_Buoyancy &templ, IVP_U_Set_Active<IVP_Core> *set_of_cores_,
									 IVP_Liquid_Surface_Descriptor *liquid_surface_descriptor_)
...
public:
	...
	virtual void calc_liquid_surface(IVP_Environment * /*environment*/, IVP_Core * /*core*/,
									 IVP_U_Float_Hesse *surface_normal_out, IVP_U_Float_Point *abs_speed_of_current_out)
	{
		cplane_t worldPlane;
		matrix3x4_t matObjectToWorld;
...
CPhysicsFluidController *CreateFluidController(IVP_Environment *pEnvironment, CPhysicsObject *pFluidObject,
											   fluidparams_t *pParams)
{
	...
	IVP_Liquid_Surface_Descriptor *lsd =
		new CLiquidSurfaceDescriptor(pFluidObject, pParams->surfacePlane, pParams->currentVelocity);
	...
	// -------------------------------------------------------------------------------
	CBuoyancyAttacher *attacher_to_cores_buoyancy =
		new CBuoyancyAttacher(buoyancy_input, pPhantom->get_intruding_cores(), lsd);

	CPhysicsFluidController *pFluid =
		new CPhysicsFluidController(attacher_to_cores_buoyancy, lsd, pFluidObject, pParams->contents);
