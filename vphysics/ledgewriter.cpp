...
// Builds a packedhull_t from a IVP_Compact_Ledge.  Assumes that the utlbuffer points at the memory following pHull
// (pHull is the header, utlbuffer is the body)
void PackLedgeIntoBuffer(packedhull_t *pHull, CUtlBuffer &buf, const IVP_Compact_Ledge *pLedge,
...
void CVPhysicsVirtualMeshWriter::UnpackCompactLedgeFromHull(IVP_Compact_Ledge *pLedge, int materialIndex,
															const IVP_Compact_Poly_Point *pPointList,
															const virtualmeshhull_t *pHullHeader, int hullIndex,
															bool isVirtualLedge)
{
	...
	// write the ledge
	pLedge->set_offset_ledge_points(
		(int)((char *)pPointList - (char *)pLedge)); // byte offset from 'this' to (ledge) point array
	pLedge->set_is_compact(IVP_TRUE);
	pLedge->set_size(sizeof(IVP_Compact_Ledge) +
					 sizeof(IVP_Compact_Triangle) * pHull->triangleCount); // <0 indicates a non compact compact ledge
	pLedge->n_triangles = pHull->triangleCount;
	pLedge->has_chilren_flag = isVirtualLedge ? IVP_TRUE : IVP_FALSE;

	// Make the offset -pLedge so the result is a NULL ledgetree node - we haven't needed to create one of these as of
	// yet
	pLedge->ledgetree_node_offset = -((int)pLedge);

	// keep track of which triangle edge referenced this edge (so the next one can swap the order and point to the first
	// one)
	...
	for(int i = 0; i < pHull->triangleCount; i++)
	{
		...
		for(int j = 0; j < 3; j++)
		{
			...
			{
				// this is the first triangle to use this edge, so it's forward (and the other triangle sharing
				// (opposite edge pointer) is unknown)
			...
			{
				// this is the second triangle to use this edge, so it's reversed (and the other triangle sharing is in
				// the forward edge table)
...
static void InitTriangle(IVP_Compact_Triangle *pTri, int index, int materialIndex, int v0, int v1, int v2, int opp0,
						 int opp1, int opp2)
{
...
void CVPhysicsVirtualMeshWriter::InitTwoSidedTriangleLege(triangleledge_t *pOut, const IVP_Compact_Poly_Point *pPoints,
														  int v0, int v1, int v2, int materialIndex)
{
	IVP_Compact_Ledge *pLedge = &pOut->ledge;
	pLedge->set_offset_ledge_points(
		(int)((char *)pPoints - (char *)pLedge)); // byte offset from 'this' to (ledge) point array
	pLedge->set_is_compact(IVP_TRUE);
...
virtualmeshhull_t *CVPhysicsVirtualMeshWriter::CreatePackedHullFromLedges(const virtualmeshlist_t &list,
																		  ...
																		  int ledgeCount)
{
	...
	unsigned int totalSize = sizeof(packedtriangle_t) * triCount + sizeof(packededge_t) * edgeCount +
							 sizeof(packedhull_t) * ledgeCount + sizeof(virtualmeshhull_t);
...
unsigned int CVPhysicsVirtualMeshWriter::UnpackLedgeListFromHull(byte *pOut, virtualmeshhull_t *pHull,
...
static bool DumpListToGLView( const char *pFilename, const virtualmeshlist_t &list )
{
	...
	{
		fprintf( fp, "3\n" );
		fprintf( fp, "%6.3f %6.3f %6.3f 1 0 0\n", list.pVerts[list.indices[i*3+0]].x,
list.pVerts[list.indices[i*3+0]].y, list.pVerts[list.indices[i*3+0]].z ); fprintf( fp, "%6.3f %6.3f %6.3f 0 1 0\n",
list.pVerts[list.indices[i*3+1]].x, list.pVerts[list.indices[i*3+1]].y, list.pVerts[list.indices[i*3+1]].z ); fprintf(
fp, "%6.3f %6.3f %6.3f 0 0 1\n", list.pVerts[list.indices[i*3+2]].x, list.pVerts[list.indices[i*3+2]].y,
list.pVerts[list.indices[i*3+2]].z );
	}
	fclose(fp);
#endif
	return true;
...
static bool DumpLedgeToGLView( const char *pFilename, const IVP_Compact_Ledge *pLedge, float r=1.0f, float g=1.0f, float
b=1.0f, float offset=0.0f )
{
#if DUMP_FILES
	FILE *fp = fopen( pFilename, "a+" );
	int ivpIndex;
...
bool CVPhysicsVirtualMeshWriter::CheckHulls( virtualmeshhull_t *pHull0, virtualmeshhull_t *pHull1, const
