...
public:
	...
	{
		return m_cone.origin;
	}
	virtual float Radius(void) const
	{
		return m_cone.h + m_radius;
	}
...
void GetStartVert(const leafmap_t *pLeafmap, const IVP_U_Float_Point &localDirection, int &triIndex, int &edgeIndex)
{
	...
	// map dir to index
	int cacheIndex =
		(localDirection.k[0] < 0 ? 1 : 0) + (localDirection.k[1] < 0 ? 2 : 0) + (localDirection.k[2] < 0 ? 4 : 0);
...
bool CTraceIVP::BuildLeafmapCacheRLE(const leafmap_t *RESTRICT pLeafmap)
{
	...
	for(int i = 0; i < m_cacheCount - 1; i++)
	{
		if(countThisSpan < 4)
		{
			...
			v3 = baseVert++;
			countThisSpan--;
			m_vertCache[i].LoadAndSwizzleAligned(pVerts[v0].Base(), pVerts[v1].Base(), pVerts[v2].Base(),
												 pVerts[v3].Base());
		}
		else
		{
			// we have four verts in this span, just grab the next four
			m_vertCache[i].LoadAndSwizzleAligned(pVerts[baseVert + 0].Base(), pVerts[baseVert + 1].Base(),
												 pVerts[baseVert + 2].Base(), pVerts[baseVert + 3].Base());
			baseVert += 4;
			countThisSpan -= 4;
		}
	}
	...
	m_vertCache[m_cacheCount - 1].LoadAndSwizzleAligned(pVerts[v[0]].Base(), pVerts[v[1]].Base(), pVerts[v[2]].Base(),
														pVerts[v[3]].Base());
...
bool CTraceIVP::BuildLeafmapCache(const leafmap_t *RESTRICT pLeafmap)
{
	...
	m_vertCache[m_cacheCount - 1].LoadAndSwizzleAligned(pVerts[x0].Base(), pVerts[x1].Base(), pVerts[x2].Base(),
														pVerts[x3].Base());
	FourVectors::RotateManyBy(&m_vertCache[0], m_cacheCount, *((const matrix3x4_t *)&m_ivpLocalToHLWorld));
	return true;
#endif
...
public:
	...
	Vector GetVertByIndex(int index) const
	{
		return (index) ? m_end : m_start;
	}
	float Radius(void) const
	{
		return m_length * 0.5f;
	}
...
{
public:
	CTraceSolverSweptObject(trace_t *ptr, ITraceObject *sweepobject, CTraceRay *ray, CTraceIVP *obstacle,
							const Vector &axis, unsigned int contentsMask, IConvexInfo *pConvexInfo);
...
CTraceSolverSweptObject::CTraceSolverSweptObject(trace_t *ptr, ITraceObject *sweepobject, CTraceRay *ray,
												 CTraceIVP *obstacle, const Vector &axis, unsigned int contentsMask,
...
void CPhysicsTrace::SweepBoxIVP(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs,
								const CPhysCollide *pCollide, const Vector &surfaceOrigin, const QAngle &surfaceAngles,
								trace_t *ptr)
{
	Ray_t ray;
...
void CPhysicsTrace::SweepBoxIVP(const Ray_t &raySrc, unsigned int contentsMask, IConvexInfo *pConvexInfo,
								const CPhysCollide *pCollide, const Vector &surfaceOrigin, const QAngle &surfaceAngles,
								trace_t *ptr)
{
	CM_ClearTrace(ptr);
...
void CPhysicsTrace::SweepIVP(const Vector &start, const Vector &end, const CPhysCollide *pSweptSurface,
							 const QAngle &sweptAngles, const CPhysCollide *pSurface, const Vector &surfaceOrigin,
							 const QAngle &surfaceAngles, trace_t *ptr)
{
	CM_ClearTrace(ptr);
...
static void CalculateSeparatingPlane(trace_t *ptr, ITraceObject *sweepObject, CTraceRay *ray, ITraceObject *obstacle,
									 simplex_t &simplex);
...
FORCEINLINE bool simplex_t::TriangleSimplex(const simplexvert_t &newPoint, int outIndex, const Vector &faceNormal,
											Vector *pOut)
{
	vertCount = 3;
...
void CalculateSeparatingPlane(trace_t *ptr, ITraceObject *sweepObject, CTraceRay *ray, ITraceObject *obstacle,
							  simplex_t &simplex)
{
...
void CPhysicsTrace::GetAABB(Vector *pMins, Vector *pMaxs, const CPhysCollide *pCollide, const Vector &collideOrigin,
...
void TraceGetExtent_r(const IVP_Compact_Ledgetree_Node *node, CTraceIVP &ivp, const Vector &dir, float &dot,
					  Vector &point)
{
	if(node->is_terminal() == IVP_TRUE)
	{
...
Vector CPhysicsTrace::GetExtent(const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles,
...
bool CPhysicsTrace::IsBoxIntersectingCone(const Vector &boxAbsMins, const Vector &boxAbsMaxs,
