...
public:
	...
	void get_radius_and_radius_dev_to_given_center(const IVP_U_Float_Point *center, IVP_FLOAT *radius,
...
public:
	...
	CMeshInstance *GetData()
	{
		return this;
	}
	const triangleledge_t *GetLedges()
	{
		return (triangleledge_t *)m_pMemory;
	}
	inline int HullCount()
	{
		return m_hullCount;
	}
	const IVP_Compact_Ledge *GetOuterHull()
	{
		return (m_hullCount == 1) ? (const IVP_Compact_Ledge *)(m_pMemory + m_hullOffset) : NULL;
	}
...
void CMeshInstance::Init(const virtualmeshlist_t &list)
{
	...
	for(i = 0; i < list.triangleCount; i++)
	{
		...
		CVPhysicsVirtualMeshWriter::InitTwoSidedTriangleLege(&pLedges[i], pPoints, list.indices[i * 3 + 0],
															 list.indices[i * 3 + 1], list.indices[i * 3 + 2], 0);
	}
...
public:
	...
	void GetAllLedgesWithinRadius(const IVP_U_Point *observer_os, IVP_DOUBLE radius,
								  IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges,
								  const IVP_Compact_Ledge *pRootLedge = NULL)
	{
	...
	CPhysCollideVirtualMesh(const virtualmeshparams_t &params)
		: m_params(params), m_hMemory(INVALID_MEMHANDLE), m_ledgeCount(0)
	{
		m_pHull = NULL;
	...
	static void DestroyMeshBoundingHull(virtualmeshhull_t *pHull)
	{
		CVPhysicsVirtualMeshWriter::DestroyPackedHull(pHull);
	}
	static IVP_Compact_Surface *CreateBoundingSurfaceFromRange(const virtualmeshlist_t &list, int firstIndex,
															   int indexCount);
	...
	if(list.triangleCount)
	{
		...
		if(pSurface)
		{
			...
			else
			{
				...
				IVP_Compact_Surface *pSurface1 =
					CreateBoundingSurfaceFromRange(list, list.indexCount / 2, list.indexCount / 2);

				const IVP_Compact_Ledge *pLedges[2] = {pSurface0->get_compact_ledge_tree_root()->get_compact_hull(),
													   pSurface1->get_compact_ledge_tree_root()->get_compact_hull()};
...
IVP_Compact_Surface *CPhysCollideVirtualMesh::CreateBoundingSurfaceFromRange(const virtualmeshlist_t &list,
																			 int firstIndex, int indexCount)
{
	...
	IVP_Compact_Ledge *pLedge = const_cast<IVP_Compact_Ledge *>(
		node->get_compact_hull()); // we're going to write into client data on each vert before we throw this away
...
void IVP_SurfaceManager_VirtualMesh::get_radius_and_radius_dev_to_given_center(const IVP_U_Float_Point *center,
																			   IVP_FLOAT *radius,
																			   IVP_FLOAT *radius_deviation) const
{
