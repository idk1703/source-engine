...
void CPhysicsObject::Init(const CPhysCollide *pCollisionModel, IVP_Real_Object *pObject, int materialIndex,
						  float volume, float drag, float angDrag)
{
	...
	m_callbacks = CALLBACK_GLOBAL_COLLISION | CALLBACK_GLOBAL_FRICTION | CALLBACK_FLUID_TOUCH | CALLBACK_GLOBAL_TOUCH |
...
void CPhysicsObject::RecomputeDragBases()
{
	...
	// Get an AABB for this object and use the area of each side as a basis for approximating cross-section area for
	// drag
	...
	m_angDragBasis.x = areaFractions.z * AngDragIntegral(pInvRI->k[0], delta.x, delta.y, delta.z) +
					   areaFractions.y * AngDragIntegral(pInvRI->k[0], delta.x, delta.z, delta.y);
	// rotation about the y axis
	m_angDragBasis.y = areaFractions.z * AngDragIntegral(pInvRI->k[1], delta.y, delta.x, delta.z) +
					   areaFractions.x * AngDragIntegral(pInvRI->k[1], delta.y, delta.z, delta.x);
	// rotation about the z axis
	m_angDragBasis.z = areaFractions.y * AngDragIntegral(pInvRI->k[2], delta.z, delta.x, delta.y) +
					   areaFractions.x * AngDragIntegral(pInvRI->k[2], delta.z, delta.y, delta.x);
}
...
void CPhysicsObject::CalculateForceOffset(const Vector &forceVector, const Vector &worldPosition, Vector *centerForce,
										  AngularImpulse *centerTorque) const
{
	IVP_U_Point pos;
	IVP_U_Float_Point force;
...
void CPhysicsObject::SetShadow(float maxSpeed, float maxAngularSpeed, bool allowPhysicsMovement,
...
void CPhysicsObject::UpdateShadow(const Vector &targetPosition, const QAngle &targetAngles, bool tempDisableGravity,
								  float timeOffset)
{
...
float CPhysicsObject::GetDragInDirection(const IVP_U_Float_Point &velocity) const
{
	...
	return m_dragCoefficient * IVP_Inline_Math::fabsd(local.k[0] * m_dragBasis.x) +
		   IVP_Inline_Math::fabsd(local.k[1] * m_dragBasis.y) + IVP_Inline_Math::fabsd(local.k[2] * m_dragBasis.z);
}
...
void CPhysicsObject::OutputDebugInfo() const
{
	...
	Msg("Inertia: %.2f, %.2f, %.2f (inv %.3f, %.3f, %.3f)\n", inertia.x, inertia.y, inertia.z, invInertia.x,
	...
	Msg("Linear Drag: %.2f, %.2f, %.2f (factor %.2f)\n", m_dragBasis.x, m_dragBasis.y, m_dragBasis.z,
		m_dragCoefficient);
	Msg("Angular Drag: %.2f, %.2f, %.2f (factor %.2f)\n", m_angDragBasis.x, m_angDragBasis.y, m_angDragBasis.z,
		m_angDragCoefficient);
	...
	Msg("State: %s, Collision %s, Motion %s, %sFlags %04X (game %04x, index %d)\n", IsAsleep() ? "Asleep" : "Awake",
		...
		(GetCallbackFlags() & CALLBACK_MARKED_FOR_TEST) ? "Debug! " : "", (int)GetCallbackFlags(), (int)GetGameFlags(),
		(int)GetGameIndex());
	...
	physprops->GetPhysicsProperties(GetMaterialIndexInternal(), &matDensity, &matThickness, &matFriction,
									&matElasticity);
	Msg("Material: %s : density(%.1f), thickness(%.2f), friction(%.2f), elasticity(%.2f)\n",
		physprops->GetPropName(GetMaterialIndexInternal()), matDensity, matThickness, matFriction, matElasticity);
	if(GetCollide())
	{
...
static void InitObjectTemplate(IVP_Template_Real_Object &objectTemplate, int materialIndex, objectparams_t *pParams,
							   bool isStatic)
{
...
CPhysicsObject *CreatePhysicsObject(CPhysicsEnvironment *pEnvironment, const CPhysCollide *pCollisionModel,
									int materialIndex, const Vector &position, const QAngle &angles,
									objectparams_t *pParams, bool isStatic)
{
	...
	BEGIN_IVP_ALLOCATION()
		...
		IVP_Polygon *realObject =
			pEnvironment->GetIVPEnvironment()->create_polygon(pSurman, &objectTemplate, &rotation, &pos);

		pObject->Init(pCollisionModel, realObject, materialIndex, pParams->volume, pParams->dragCoefficient,
					  pParams->dragCoefficient);
...
CPhysicsObject *CreatePhysicsSphere(CPhysicsEnvironment *pEnvironment, float radius, int materialIndex,
									...
									bool isStatic)
{
	...
	MEM_ALLOC_CREDIT();
	IVP_Ball *realObject =
		pEnvironment->GetIVPEnvironment()->create_ball(&ballTemplate, &objectTemplate, &rotation, &pos);
...
BEGIN_SIMPLE_DATADESC(vphysics_save_cphysicsobject_t)
	...
	DEFINE_FIELD(sphereRadius, FIELD_FLOAT), DEFINE_FIELD(isStatic, FIELD_BOOLEAN),
		DEFINE_FIELD(collisionEnabled, FIELD_BOOLEAN), DEFINE_FIELD(gravityEnabled, FIELD_BOOLEAN),
		DEFINE_FIELD(dragEnabled, FIELD_BOOLEAN), DEFINE_FIELD(motionEnabled, FIELD_BOOLEAN),
		DEFINE_FIELD(isAsleep, FIELD_BOOLEAN), DEFINE_FIELD(isTrigger, FIELD_BOOLEAN),
		DEFINE_FIELD(asleepSinceCreation, FIELD_BOOLEAN), DEFINE_FIELD(hasTouchedDynamic, FIELD_BOOLEAN),
		DEFINE_CUSTOM_FIELD(materialIndex, &g_MaterialIndexDataOps), DEFINE_FIELD(mass, FIELD_FLOAT),
		DEFINE_FIELD(rotInertia, FIELD_VECTOR), DEFINE_FIELD(speedDamping, FIELD_FLOAT),
		DEFINE_FIELD(rotSpeedDamping, FIELD_FLOAT), DEFINE_FIELD(massCenterOverride, FIELD_VECTOR),
		DEFINE_FIELD(callbacks, FIELD_INTEGER), DEFINE_FIELD(gameFlags, FIELD_INTEGER),
		DEFINE_FIELD(contentsMask, FIELD_INTEGER), DEFINE_FIELD(volume, FIELD_FLOAT),
		DEFINE_FIELD(dragCoefficient, FIELD_FLOAT), DEFINE_FIELD(angDragCoefficient, FIELD_FLOAT),
		DEFINE_FIELD(hasShadowController, FIELD_BOOLEAN),
		// DEFINE_VPHYSPTR( pShadow ),
		DEFINE_FIELD(origin, FIELD_POSITION_VECTOR), DEFINE_FIELD(angles, FIELD_VECTOR),
		DEFINE_FIELD(velocity, FIELD_VECTOR), DEFINE_FIELD(angVelocity, FIELD_VECTOR),
		DEFINE_FIELD(collideType, FIELD_SHORT), DEFINE_FIELD(gameIndex, FIELD_SHORT),
...
void CPhysicsObject::InitFromTemplate(CPhysicsEnvironment *pEnvironment, void *pGameData,
									  const vphysics_save_cphysicsobject_t &objectTemplate)
{
	...
	ivpObjectTemplate.rot_speed_damp_factor.set(objectTemplate.rotSpeedDamping, objectTemplate.rotSpeedDamping,
												objectTemplate.rotSpeedDamping);
	...
	Init(objectTemplate.pCollide, realObject, objectTemplate.materialIndex, objectTemplate.volume,
		 objectTemplate.dragCoefficient, objectTemplate.dragCoefficient);
...
bool RestorePhysicsObject(const physrestoreparams_t &params, CPhysicsObject **ppObject)
{
	...
	*ppObject = new CPhysicsObject();
	...
	(*ppObject)->InitFromTemplate(static_cast<CPhysicsEnvironment *>(params.pEnvironment), params.pGameData,
								  objectTemplate);
...
IPhysicsObject *CreateObjectFromBuffer(CPhysicsEnvironment *pEnvironment, void *pGameData, unsigned char *pBuffer,
									   unsigned int bufferSize, bool enableCollisions)
{
	...
	if(bufferSize >= sizeof(vphysics_save_cphysicsobject_t))
	{
		...
		pTemplate->hasShadowController =
			false; // this hasn't been saved separately so cannot be supported via this path
...
IPhysicsObject *CreateObjectFromBuffer_UseExistingMemory(CPhysicsEnvironment *pEnvironment, void *pGameData,
														 unsigned char *pBuffer, unsigned int bufferSize,
...
// regenerate the friction systems for these objects.  Because when it was saved it had them (came to rest with the
// contact points). So now we need to recreate them or some objects may not wake up when this object (or its neighbors)
// are deleted.
