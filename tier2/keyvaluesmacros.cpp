...
static KeyValues *HandleKeyValuesMacro_Insert(KeyValues *pkvInsert, KeyValues *pkvParent)
{
	...
	if(pkvInsert->GetDataType() != KeyValues::TYPE_STRING)
	{
		...
		Msg("Error: #insert on key without a data type of string, can only do #insert with simple key/value with "
			"string value\n");
		return nullptr;
	}
	...
	if(!pszInsert && *pszInsert == '\0')
	{
		...
		Msg("Error: #insert on key with empty string value, can only do #insert with simple key/value with string "
			"value\n");
		return nullptr;
	}
	...
	uint nBufSize =
		g_pFullFileSystem->GetOptimalReadSize(f, nFileSize + 2 /* null termination */ + 8 /* "\"x\"\n{\n}\n" */);
...
static void UpdateKeyValuesBlock(KeyValues *pkvDst, KeyValues *pkvUpdate)
{
	...
	for(KeyValues *pkvUpdateSubKey = pkvUpdate->GetFirstSubKey(); pkvUpdateSubKey;
		pkvUpdateSubKey = pkvUpdateSubKey->GetNextKey())
	{
...
static KeyValues *HandleKeyValuesMacro_Update(KeyValues *pkvUpdate, KeyValues *pkvParent, bool *pbDidUpdate)
{
	...
	for(KeyValues *pkvParentSubKey = pkvParent->GetFirstSubKey(); pkvParentSubKey;
		pkvParentSubKey = pkvParentSubKey->GetNextKey())
	{
