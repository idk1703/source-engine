...
void InitDefaultFileSystem(void)
{
	...
	if(!Sys_LoadInterface("filesystem_stdio", FILESYSTEM_INTERFACE_VERSION, &g_pFullFileSystemModule,
						  (void **)&g_pFullFileSystem))
	{
		if(!Sys_LoadInterface("filesystem_steam", FILESYSTEM_INTERFACE_VERSION, &g_pFullFileSystemModule,
							  (void **)&g_pFullFileSystem))
		{
			exit(0);
		}
