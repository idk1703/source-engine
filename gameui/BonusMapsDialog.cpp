...
void CBonusMapsDialog::BuildMapsList(void)
{
	...
	if(iMapCount <= 0)
	{
		vgui::Label *pNoSavesLabel =
			SETUP_PANEL(new Label(m_pGameList, "NoBonusMapsLabel", "#GameUI_NoBonusMapsToDisplay"));
	...
	for(int iMap = 0; iMap < m_pGameList->GetItemCount(); ++iMap)
	{
		...
		if(pBonusMapPanel)
			pBonusMapPanel->SetBonusMapInfo(
				...
				*(BonusMapsDatabase()->GetBonusData(pBonusMapPanel->GetBonusMapListItemID())));
	}
...
void CBonusMapsDialog::RefreshMedalDisplay(BonusMapDescription_t *pMap)
{
	...
	if(iChallenge < 0)
	{
		...
		for(int iChallengeIndex = 0; iChallengeIndex < m_pChallengeSelection->GetItemCount(); ++iChallengeIndex)
		{
			...
			if(iChallengeNum >= 0)
			{
				...
				CBitmapImagePanel *pBitmap =
					dynamic_cast<CBitmapImagePanel *>(GetChild(iFirstChildIndex + iChallengeNum));
	...
	if(iBest != -1)
	{
		...
		g_pVGuiLocalize->ConstructString(szWideBuff, sizeof(szWideBuff), g_pVGuiLocalize->Find("#GameUI_BonusMapsBest"),
										 1, szWideBuff2);
	...
	if(iNext != -1)
	{
		...
		g_pVGuiLocalize->ConvertANSIToUnicode(szBuff, szWideBuff2, sizeof(szWideBuff2));
		g_pVGuiLocalize->ConstructString(szWideBuff, sizeof(szWideBuff), g_pVGuiLocalize->Find("#GameUI_BonusMapsGoal"),
										 1, szWideBuff2);
		g_pVGuiLocalize->ConvertUnicodeToANSI(szWideBuff, szBuff, sizeof(szBuff));
...
void CBonusMapsDialog::RefreshCompletionPercentage(void)
{
	...
	if(fPercentage > 0.0f)
	{
		...
		Color cProgressBar = Color(static_cast<float>(m_PercentageBarBackgroundColor.r()) * (1.0f - fPercentage) +
									   static_cast<float>(m_PercentageBarColor.r()) * fPercentage,
								   static_cast<float>(m_PercentageBarBackgroundColor.g()) * (1.0f - fPercentage) +
									   static_cast<float>(m_PercentageBarColor.g()) * fPercentage,
								   static_cast<float>(m_PercentageBarBackgroundColor.b()) * (1.0f - fPercentage) +
									   static_cast<float>(m_PercentageBarColor.b()) * fPercentage,
								   static_cast<float>(m_PercentageBarBackgroundColor.a()) * (1.0f - fPercentage) +
				...
				else
				{
					...
					if(iChallenge > 0)
					{
						...
						BonusMapsDatabase()->SetCurrentChallengeObjectives(pChallengeDescription->iBronze,
																		   pChallengeDescription->iSilver,
																		   pChallengeDescription->iGold);
						BonusMapsDatabase()->SetCurrentChallengeNames(pBonusMap->szFileName, pBonusMap->szMapName,
					...
					else
					{
						const char *pchSubDir =
							Q_strnchr(BonusMapsDatabase()->GetPath(), '/', Q_strlen(BonusMapsDatabase()->GetPath()));
...
void CBonusMapsDialog::OnKeyCodePressed(vgui::KeyCode code)
{
	...
	else if(code == KEY_XSTICK1_RIGHT || code == KEY_XSTICK2_RIGHT || code == KEY_XBUTTON_RIGHT || code == KEY_RIGHT)
	{
	...
	else if(code == KEY_XSTICK1_LEFT || code == KEY_XSTICK2_LEFT || code == KEY_XBUTTON_LEFT || code == KEY_LEFT)
	{
		if(m_pGameList->GetItemCount())
		{
			Panel *pSelectedPanel = m_pGameList->GetSelectedPanel();
	...
	else if(code == KEY_XSTICK1_DOWN || code == KEY_XSTICK2_DOWN || code == KEY_XBUTTON_DOWN || code == KEY_DOWN)
	{
	...
	else if(code == KEY_XSTICK1_UP || code == KEY_XSTICK2_UP || code == KEY_XBUTTON_UP || code == KEY_UP)
	{
		int nOldActiveItem = m_pChallengeSelection->GetActiveItem();
...
void CBonusMapsDialog::OnPanelSelected()
{
	...
	if(pMap->m_pChallenges)
	{
		...
		for(iNumChallenges; iNumChallenges < pMap->m_pChallenges->Count(); ++iNumChallenges)
		{
			...
			// If the challenge type was specified then use that instead of (legacy) the order the challenges were
			// listed
