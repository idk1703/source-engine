...
ConVar x360_resolution_widescreen_mode("x360_resolution_widescreen_mode", "0", 0,
									   "This is only used for reference. Changing this value does nothing");
ConVar x360_resolution_width("x360_resolution_width", "640", 0,
							 "This is only used for reference. Changing this value does nothing");
ConVar x360_resolution_height("x360_resolution_height", "480", 0,
							  "This is only used for reference. Changing this value does nothing");
ConVar x360_resolution_interlaced("x360_resolution_interlaced", "0", 0,
								  "This is only used for reference. Changing this value does nothing");

ConVar x360_audio_english("x360_audio_english", "0", 0,
...
COptionsDialogXbox::COptionsDialogXbox(vgui::Panel *parent, bool bControllerOptions)
...
void COptionsDialogXbox::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	if(!m_bControllerOptions)
		vgui::GetAnimationController()->RunAnimationCommand(BasePanel(), "m_flBackgroundFillAlpha", 0.01f, 0.0f, 1.0f,
...
void COptionsDialogXbox::OnClose(void)
{
	...
	for(int iCode = 0; iCode < BUTTON_CODE_LAST; ++iCode)
	{
		...
		if(ActionsAreTheSame(pDuckKeyName, "+duck"))
		{
			if(varOption.GetBool())
			{
				...
				Q_snprintf(szCommand, sizeof(szCommand), "bind \"%s\" \"%s\"", g_pInputSystem->ButtonCodeToString(code),
						   "toggle_duck");
	...
	if(!m_bControllerOptions)
		vgui::GetAnimationController()->RunAnimationCommand(BasePanel(), "m_flBackgroundFillAlpha", 120.0f, 0.0f, 1.0f,
															AnimationController::INTERPOLATOR_LINEAR);
		...
		case STEAMCONTROLLER_A:
			if(!(*m_pOptions)[m_iSelection]->bUnchangeable)
			{
				...
				if(eOptionType == OPTION_TYPE_BIND &&
				   ((*m_pOptions)[m_iSelection]->iNumBinds < binds_per_command.GetInt() ||
...
void COptionsDialogXbox::HandleBindKeyCodePressed(vgui::KeyCode code)
{
	...
	if((code >= KEY_XSTICK1_RIGHT && code <= KEY_XSTICK1_UP) || (code >= KEY_XSTICK2_RIGHT && code <= KEY_XSTICK2_UP))
		return;
			...
			switch((*m_pOptions)[m_iSelection]->eOptionType)
			{
				...
				case OPTION_TYPE_CHOICE:
					m_pFooter->AddNewButtonLabel(
						((*m_pOptions)[m_iSelection]->m_Choices.Count() == 2) ? ("#GameUI_Toggle") : ("#GameUI_Modify"),
						"#GameUI_Icons_DPAD");
					break;

				case OPTION_TYPE_BIND:
				{
					if((*m_pOptions)[m_iSelection]->iNumBinds < binds_per_command.GetInt() ||
					   binds_per_command.GetInt() == 1)
		...
		case OPTION_TYPE_SLIDER:
		{
			if(system()->GetCurrentTime() > m_fNextChangeTime)
			{
				...
				if(m_pSelectedOption->fIncValue == 0.0f)
					...
					fIncValue = m_pSelectedOption->fIncValue *
								(m_pSelectedOption->fMaxValue - m_pSelectedOption->fMinValue) * (1.0f / fNumSegments);
		...
		case OPTION_TYPE_CHOICE:
		{
			...
			if(m_pSelectedOption->iCurrentChoice >= 0)
			{
				...
				else
				{
					...
					char szOptionValue[OPTION_STRING_LENGTH];
					Q_strncpy(szOptionValue, m_pSelectedOption->m_Choices[m_pSelectedOption->iCurrentChoice].szValue,
							  sizeof(szOptionValue));

					char *pchOptionValue2 = const_cast<char *>(Q_strnchr(szOptionValue, ';', sizeof(szOptionValue)));

					AssertMsg(pchOptionValue2,
							  "Option uses to convars but an option doesn't have 2 ; separated values!");
			...
			{
				DevWarning("ConVar \"%s\" set to value that's not a choice used by \"%s\" option.",
	...
	switch(pOption->eOptionType)
	{
		...
		case OPTION_TYPE_SLIDER:
		{
			...
			float fNormalizeHome =
				fabsf((pOption->fMinValue - pOption->fSliderHomeValue) / (pOption->fMaxValue - pOption->fMinValue));
			if(pOption->fIncValue < 0.0f)
				fNormalizeHome = 1.0f - fNormalizeHome;
			m_pValueBars[iLabel]->SetHomeValue(fNormalizeHome);

			float fNormalizeValue =
				fabsf((pOption->fMinValue - varOption.GetFloat()) / (pOption->fMaxValue - pOption->fMinValue));
			if(pOption->fIncValue < 0.0f)
			...
			else
			{
				if(pOption->bUnchangeable)
				{
					...
					else
					{
						...
						ConVarRef varOption2(pOption->szConvar2);
						char szBuff[256];
						Q_snprintf(szBuff, sizeof(szBuff), "%sx%s%s", varOption.GetString(), varOption2.GetString(),
...
void COptionsDialogXbox::UpdateBind(OptionData_t *pOption, int iLabel, ButtonCode_t codeIgnore, ButtonCode_t codeAdd)
{
	...
	for(int iCode = 0; iCode < BUTTON_CODE_LAST; ++iCode)
	{
		...
		Q_snprintf(szBuff, sizeof(szBuff), "#GameUI_Icons_%s",
			...
			case OPTION_TYPE_CHOICE:
			{
				...
				else
				{
					...
					if(pChoicesKey)
					{
						...
						{
							DevWarning("Option \"%s\" is type CHOICE but has less than 2 choices!",
