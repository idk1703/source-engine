	...
	if(!stricmp(command, "loadsave"))
	{
		...
		if(m_SaveGames.IsValidIndex(saveIndex))
		{
			...
			else
			{
				...
				QueryBox *box =
					new QueryBox("#GameUI_ConfirmOverwriteSaveGame_Title", "#GameUI_ConfirmOverwriteSaveGame_Info");
...
void CSaveGameDialog::OnScanningSaveGames()
{
	...
	SaveGameDescription_t save = {"NewSavedGame",		  "", "", "#GameUI_NewSaveGame", "", "", "Current",
