...
public:
	...
	virtual int AddMenuItem(const char *itemName, const char *itemText, const char *command, Panel *target,
	...
	virtual int AddMenuItem(const char *itemName, const char *itemText, KeyValues *command, Panel *target,
							KeyValues *userData = NULL)
	{
...
static int CC_GameMenuCompletionFunc(char const *partial,
...
static ConCommand gamemenucommand("gamemenucommand", CC_GameMenuCommand, "Issue game menu command.", 0,
...
void CBasePanel::UpdateBackgroundState()
{
	...
	if(m_bHaveDarkenedBackground != bNeedDarkenedBackground)
	{
		...
		vgui::GetAnimationController()->RunAnimationCommand(this, "m_flBackgroundFillAlpha", targetAlpha, 0.0f,
															duration, AnimationController::INTERPOLATOR_LINEAR);
	}
	...
	if(m_bHaveDarkenedTitleText != bNeedDarkenedTitleText || m_bForceTitleTextUpdate)
	{
		...
		{
			vgui::GetAnimationController()->RunAnimationCommand(m_pGameMenuButtons[i], "alpha", targetTitleAlpha, 0.0f,
																duration, AnimationController::INTERPOLATOR_LINEAR);
		}
		...
		if(IsX360() && m_ExitingFrameCount && engine->Time() >= m_flTransitionEndTime)
		{
			if(m_ExitingFrameCount > 1)
			{
				...
				if(m_ExitingFrameCount == 1)
				{
					...
					// If we kicked off this event from an invite, we need to properly setup the restart to account for
					// that
...
void CBasePanel::UpdateRichPresenceInfo()
{
	...
	if(!GameUI().IsInLevel())
	{
		...
		if(m_bSinglePlayer)
		{
			if(!xboxsystem->UserSetContext(XBX_GetPrimaryUserId(), X_CONTEXT_GAME_MODE, CONTEXT_GAME_MODE_SINGLEPLAYER,
										   true))
			{
				Warning("BasePanel: UserSetContext failed.\n");
			}
...
void CBasePanel::PerformLayout()
{
	...
	if(m_pGameLogo)
	{
		...
		m_pGameLogo->SetPos(m_iGameMenuPos.x + m_pGameLogo->GetOffsetX(),
...
void CBasePanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if(pClientScheme)
	{
		...
		for(i = 0; i < m_pGameMenuButtons.Count(); ++i)
		{
			...
			buttonColor.AddToTail(
				pClientScheme->GetColor(CFmtStr("Main.Title%d.Color", i + 1), Color(255, 255, 255, 255)));
		}
#ifdef CS_BETA
		if(!ModInfo().NoCrosshair()) // hack to not show the BETA for HL2 or HL1Port
		{
	...
	else
	{
		...
		V_snprintf(filename, sizeof(filename), "vgui/appchooser/background_%s%s", gameName,
	...
	if(IsX360())
	{
		...
		V_snprintf(filename, sizeof(filename), "vgui/appchooser/background_orange%s",
				   (bIsWidescreen ? "_widescreen" : ""));
...
void CBasePanel::RunMenuCommand(const char *command)
{
	...
	else if(!Q_stricmp(command, "OpenAchievementsDialog"))
	{
		if(IsPC())
		{
			...
			{
				vgui::MessageBox *pMessageBox = new vgui::MessageBox("#GameUI_Achievements_SteamRequired_Title",
		...
		if(IsPC())
		{
			if(!steamapicontext->SteamUser() || !steamapicontext->SteamUser()->BLoggedOn())
			{
				vgui::MessageBox *pMessageBox = new vgui::MessageBox(
					"#GameUI_Achievements_SteamRequired_Title", "#GameUI_Achievements_SteamRequired_Message", this);
				pMessageBox->DoModal();
				return;
			}
	...
	else if(!Q_stricmp(command, "RestartWithNewLanguage"))
	{
		if(!IsX360())
		{
			...
			// Construct Steam URL. Pattern is steam://run/<appid>/<language>. (e.g. Ep1 In French ==>
			// steam://run/380/french)
			V_snprintf(szSteamURL, sizeof(szSteamURL), "steam://run/%d/%s", engine->GetAppID(),
					   COptionsSubAudio::GetUpdatedAudioLanguage());

			// Set Steam URL for re-launch in registry. Launcher will check this registry key and exec it in order to
			// re-load the game in the proper language
#if defined(WIN32) && !defined(_X360)
			HKEY hKey;

			if(IsPC() &&
			   RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Valve\\Source", NULL, KEY_WRITE, &hKey) == ERROR_SUCCESS)
			{
...
//-----------------------------------------------------------------------------
// Purpose: Whether this command should cause us to prompt the user if they're not signed in and do not have a storage
// device
//-----------------------------------------------------------------------------
bool CBasePanel::IsPromptableCommand(const char *command)
{
	// Blech!
	if(!Q_stricmp(command, "OpenNewGameDialog") || !Q_stricmp(command, "OpenLoadGameDialog") ||
	   !Q_stricmp(command, "OpenSaveGameDialog") || !Q_stricmp(command, "OpenBonusMapsDialog") ||
	   !Q_stricmp(command, "OpenOptionsDialog") || !Q_stricmp(command, "OpenControllerDialog") ||
	   !Q_stricmp(command, "OpenLoadCommentaryDialog") || !Q_stricmp(command, "OpenLoadSingleplayerCommentaryDialog") ||
...
bool CBasePanel::CommandRequiresSignIn(const char *command)
{
	...
	if(!Q_stricmp(command, "OpenAchievementsDialog") ||
	   ...
	   !Q_stricmp(command, "OpenLoadGameDialog") || !Q_stricmp(command, "OpenSaveGameDialog") ||
...
bool CBasePanel::CommandRequiresStorageDevice(const char *command)
{
	...
	if(!Q_stricmp(command, "OpenSaveGameDialog") || !Q_stricmp(command, "OpenLoadGameDialog"))
		return true;
...
bool CBasePanel::CommandRespectsSignInDenied(const char *command)
{
	...
	if(!Q_stricmp(command, "OpenOptionsDialog") || !Q_stricmp(command, "OpenControllerDialog"))
		return true;
...
public:
	...
	uint GetContainerOpenResult(void)
	{
		return m_ContainerOpenResult;
	}
...
//-----------------------------------------------------------------------------
void CBasePanel::UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam, byte cVoiceState, int nPlayersNeeded,
								  bool bHost)
{
	CMatchmakingBasePanel *pBase = GetMatchmakingBasePanel();
	if(pBase)
...
void CFooterPanel::Paint(void)
{
	...
	else
	{
		...
		// need to loop through and figure out how wide our buttons and text are (with gaps between) so we can offset
		// from the center
...
void CMessageDialogHandler::ShowMessageDialog(int nType, vgui::Panel *pOwner)
{
	...
	switch(nType)
	{
		case MD_SEARCHING_FOR_GAMES:
			CreateMessageDialog(MD_CANCEL | MD_RESTRICTPAINT, NULL, "#TF_Dlg_SearchingForGames", NULL,
								"CancelOperation", pOwner, true);
			break;
		...
		case MD_SESSION_SEARCH_FAILED:
			CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, NULL, "#TF_Dlg_NoGamesFound", "ShowSessionOptionsDialog",
		...
		case MD_SAVE_BEFORE_QUIT:
			CreateMessageDialog(MD_YESNO | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_QuitConfirmationTitle",
								"#GameUI_Console_QuitWarning", "QuitNoConfirm", "CloseQuitDialog_OpenMainMenu", pOwner);
			break;

		case MD_QUIT_CONFIRMATION:
			CreateMessageDialog(MD_YESNO | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_QuitConfirmationTitle",
								"#GameUI_QuitConfirmationText", "QuitNoConfirm", "CloseQuitDialog_OpenMainMenu",
								pOwner);
			break;

		case MD_QUIT_CONFIRMATION_TF:
			CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, "#GameUI_QuitConfirmationTitle",
								"#GameUI_QuitConfirmationText", "QuitNoConfirm", "CloseQuitDialog_OpenMatchmakingMenu",
								pOwner);
			break;

		case MD_DISCONNECT_CONFIRMATION:
			CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, "", "#GameUI_DisconnectConfirmationText",
								"DisconnectNoConfirm", "close_dialog", pOwner);
			break;

		case MD_DISCONNECT_CONFIRMATION_HOST:
			CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, "", "#GameUI_DisconnectHostConfirmationText",
								"DisconnectNoConfirm", "close_dialog", pOwner);
			break;
		...
		case MD_STORAGE_DEVICES_NEEDED:
			CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT,
								"#GameUI_Console_StorageRemovedTitle", "#GameUI_Console_StorageNeededBody",
								"ShowDeviceSelector", "QuitNoConfirm", pOwner);
			break;

		case MD_STORAGE_DEVICES_CHANGED:
			CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT,
								"#GameUI_Console_StorageRemovedTitle", "#GameUI_Console_StorageRemovedBody",
								"ShowDeviceSelector", "clear_storage_deviceID", pOwner);
			break;

		case MD_STORAGE_DEVICES_TOO_FULL:
			CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT,
								"#GameUI_Console_StorageTooFullTitle", "#GameUI_Console_StorageTooFullBody",
		...
		case MD_PROMPT_STORAGE_DEVICE:
			CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT,
								...
								"#GameUI_Console_NoStorageDeviceSelectedBody", "ShowDeviceSelector",
								"StorageDeviceDenied", pOwner);
			break;
		...
		case MD_PROMPT_SIGNIN_REQUIRED:
			CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame,
								"#GameUI_Console_NoUserProfileSelectedTitle", "#GameUI_Console_UserProfileRequiredBody",
								"ShowSignInUI", "RequiredSignInDenied", pOwner);
			break;

		case MD_NOT_ONLINE_ENABLED:
			CreateMessageDialog(MD_YESNO | MD_WARNING, "", "#TF_Dlg_NotOnlineEnabled", "ShowSigninUI", "close_dialog",
								pOwner);
			break;

		case MD_NOT_ONLINE_SIGNEDIN:
			CreateMessageDialog(MD_YESNO | MD_WARNING, "", "#TF_Dlg_NotOnlineSignedIn", "ShowSigninUI", "close_dialog",
								pOwner);
			break;

		case MD_DEFAULT_CONTROLS_CONFIRM:
			CreateMessageDialog(MD_YESNO | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_RestoreDefaults",
								"#GameUI_ControllerSettingsText", "DefaultControls", "close_dialog", pOwner);
			break;

		case MD_AUTOSAVE_EXPLANATION:
			CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_ConfirmNewGame_Title",
								"#GameUI_AutoSave_Console_Explanation", "StartNewGameNoCommentaryExplanation", NULL,
								pOwner);
			break;

		case MD_COMMENTARY_EXPLANATION:
			CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_CommentaryDialogTitle",
								"#GAMEUI_Commentary_Console_Explanation", "StartNewGameNoCommentaryExplanation", NULL,
								pOwner);
			break;
		...
		case MD_COMMENTARY_CHAPTER_UNLOCK_EXPLANATION:
			CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_CommentaryDialogTitle",
								"#GameUI_CommentaryUnlock", "close_dialog", NULL, pOwner);
			break;
		...
		case MD_SAVE_BEFORE_LANGUAGE_CHANGE:
			CreateMessageDialog(MD_YESNO | MD_WARNING | MD_SIMPLEFRAME | MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT,
								...
								"AcceptVocalsLanguageChange", "CancelVocalsLanguageChange", pOwner);
		...
		case MD_SAVE_BEFORE_NEW_GAME:
			CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT,
								"#GameUI_ConfirmNewGame_Title", "#GameUI_NewGameWarning", "StartNewGame",
								"close_dialog", pOwner);
			break;
		...
		case MD_DELETE_SAVE_CONFIRM:
			CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE,
								"#GameUI_ConfirmDeleteSaveGame_Title", "#GameUI_ConfirmDeleteSaveGame_Info",
								"DeleteGame", "DeleteGameCancelled", pOwner);
			break;

		case MD_SAVE_OVERWRITE:
			CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE,
								"#GameUI_ConfirmOverwriteSaveGame_Title", "#GameUI_ConfirmOverwriteSaveGame_Info",
								"SaveGame", "OverwriteGameCancelled", pOwner);
			break;

		case MD_SAVING_WARNING:
			CreateMessageDialog(MD_WARNING | iSimpleFrame | MD_COMMANDONFORCECLOSE, "", "#GameUI_SavingWarning",
								"SaveSuccess", NULL, pOwner, true);
			break;

		case MD_SAVE_COMPLETE:
			CreateMessageDialog(MD_OK | iSimpleFrame | MD_COMMANDAFTERCLOSE, "#GameUI_ConfirmOverwriteSaveGame_Title",
								"#GameUI_GameSaved", "CloseAndSelectResume", NULL, pOwner);
			break;

		case MD_LOAD_FAILED_WARNING:
			CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame, "#GameUI_LoadFailed",
		...
		case MD_WARNING_SAMPLE:
			CreateMessageDialog(MD_OK | MD_WARNING, "#GameUI_Dialog_Warning", "This is a warning dialog",
								"close_dialog", NULL, pOwner);
			break;

		case MD_ERROR_SAMPLE:
			CreateMessageDialog(MD_OK | MD_ERROR, "Error Dialog", "This is an error dialog", "close_dialog", NULL,
								pOwner);
			break;

		case MD_STORAGE_DEVICES_CORRUPT:
			CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "", "#GameUI_Console_FileCorrupt",
								"close_dialog", NULL, pOwner);
			break;

		case MD_CHECKING_STORAGE_DEVICE:
			CreateMessageDialog(iSimpleFrame | MD_RESTRICTPAINT, NULL, "#GameUI_Dlg_CheckingStorageDevice", NULL, NULL,
								pOwner, true);
			break;
...
void CMessageDialogHandler::CreateMessageDialog(const uint nType, const char *pTitle, const char *pMsg,
												...
												bool bShowActivity /*= false*/)
{
	...
	// Show the new dialog
	m_hMessageDialogs[nStackIdx] =
		new CMessageDialog(BasePanel(), nType, pTitle, pMsg, pCmdA, pCmdB, pCreator, bShowActivity);
...
static ConCommand refresh_options_dialog("refresh_options_dialog", RefreshOptionsDialog, "Refresh the options dialog.",
										 0);