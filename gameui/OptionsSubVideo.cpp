...
public:
	COptionsSubVideoAdvancedDlg(vgui::Panel *parent) : BaseClass(parent, "OptionsSubVideoAdvancedDlg")
	{
		...
		for(int i = 0; i < ARRAYSIZE(g_DirectXLevels); i++)
		{
			...
			if(!CommandLine()->CheckParm("-dxlevel") && g_DirectXLevels[i] != config.dxSupportLevel &&
	...
	MESSAGE_FUNC_PTR(OnTextChanged, "TextChanged", panel)
	{
		if(panel == m_pDXLevel && RequiresRestart())
		{
			...
			QueryBox *box =
				new QueryBox("#GameUI_SettingRequiresDisconnect_Title", "#GameUI_SettingRequiresDisconnect_Info");
	...
	void MarkDefaultSettingsAsRecommended()
	{
		...
		// It doesn't make sense to retrieve this convar from dxsupport, because we'll then have materialsystem setting
		// this config at loadtime. (Also, it only has very minimal support for CPU related configuration.)
	...
	virtual void OnResetData()
	{
		...
		char szVersion[64];
		szVersion[0] = 0;
		system()->GetRegistryString("HKEY_LOCAL_MACHINE\\Software\\Microsoft\\DirectX\\Version", szVersion,
									sizeof(szVersion));
...
void COptionsSubVideo::PrepareResolutionList()
{
	...
	for(int i = 0; i < count; i++, plist++)
	{
		...
		else if(selectedItemID == -1 && plist->width == config.m_VideoMode.m_Width &&
...
void COptionsSubVideo::SetCurrentResolutionComboItem()
{
	...
	for(int i = 0; i < count; i++, plist++)
	{
		if(plist->width == config.m_VideoMode.m_Width && plist->height == config.m_VideoMode.m_Height)
		{
			resolution = i;
			break;
		}
...
void COptionsSubVideo::OpenAdvanced()
{
	if(!m_hOptionsSubVideoAdvancedDlg.Get())
	{
		m_hOptionsSubVideoAdvancedDlg = new COptionsSubVideoAdvancedDlg(
			BasePanel()->FindChildByName("OptionsDialog")); // we'll parent this to the OptionsDialog directly
	}
