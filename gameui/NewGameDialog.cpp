...
public:
	CGameChapterPanel(CNewGameDialog *parent, const char *name, const char *chapterName, int chapterIndex,
					  const char *chapterNumber, const char *chapterConfigFile, bool bCommentary)
...
CNewGameDialog::CNewGameDialog(vgui::Panel *parent, bool bCommentaryMode) : BaseClass(parent, "NewGameDialog")
{
	...
	else if(IsX360())
	{
		...
		while(bExists && chapterIndex < MAX_CHAPTERS)
		{
			...
			if(ChapterStringIndex == 10)
			{
				...
				if(fChap)
				{
					Q_strncpy(chapters[chapterIndex].filename, szFullFileName + 4,
	...
	for(int i = 0; i < chapterIndex; i++)
	{
		...
		CGameChapterPanel *chapterPanel = SETUP_PANEL(
			new CGameChapterPanel(this, NULL, chapterName, i, chapterID, szFullFileName, m_bCommentaryMode));
	...
	{
		m_PanelXPos[0] = m_PanelXPos[1] = m_PanelXPos[3] = m_PanelXPos[4] = m_PanelXPos[2];
	}
	...
	if(GameUI().IsConsoleUI())
	{
		...
		if(bHasBonus)
		{
			...
			for(int iBonus = 0; iBonus < BonusMapsDatabase()->BonusCount(); ++iBonus)
			{
				...
				if(Q_stricmp(m_pBonusMapDescription->szChapterName,
...
void CNewGameDialog::PostScroll(EScrollDirection dir)
{
	...
	if(index != INVALID_INDEX)
	{
		...
		panel->SetZPos(50);
		GetAnimationController()->RunAnimationCommand(panel, "alpha", 255, 0, m_ScrollSpeed,
...
void CNewGameDialog::AnimateSelectionPanels(void)
{
	...
	if(m_ScrollCt >= SCROLL_LEFT &&
	   (m_PanelIndex[SLOT_CENTER] < m_ChapterPanels.Count() - 1 || !GameUI().IsConsoleUI()))
	{
		idxOffset = -1;
	...
	for(int i = startIdx; i <= endIdx; ++i)
	{
		if(m_PanelIndex[i] != INVALID_INDEX)
		{
			...
			GetAnimationController()->RunAnimationCommand(panel, "xpos", m_PanelXPos[nextIdx], 0, m_ScrollSpeed,
														  vgui::AnimationController::INTERPOLATOR_LINEAR);
			GetAnimationController()->RunAnimationCommand(panel, "ypos", m_PanelYPos[nextIdx], 0, m_ScrollSpeed,
														  vgui::AnimationController::INTERPOLATOR_LINEAR);
			GetAnimationController()->RunAnimationCommand(panel, "alpha", m_PanelAlpha[nextIdx], 0, m_ScrollSpeed,
														  vgui::AnimationController::INTERPOLATOR_LINEAR);
		}
	}
	...
	if(GameUI().IsConsoleUI())
	{
		...
		GetAnimationController()->RunAnimationCommand(m_pChapterTitleLabels[(unsigned)m_ActiveTitleIdx], "alpha", 255,
													  0, m_ScrollSpeed, vgui::AnimationController::INTERPOLATOR_LINEAR);
		GetAnimationController()->RunAnimationCommand(m_pChapterTitleLabels[inactiveTitleIdx], "alpha", 0, 0,
													  m_ScrollSpeed, vgui::AnimationController::INTERPOLATOR_LINEAR);
			...
			if(m_ChapterPanels[m_iSelectedChapter]->HasBonus() && m_iBonusSelection > 0)
			{
				...
				else
				{
					...
					BonusMapsDatabase()->SetCurrentChallengeObjectives(
						pChallengeDescription->iBronze, pChallengeDescription->iSilver, pChallengeDescription->iGold);
					BonusMapsDatabase()->SetCurrentChallengeNames(m_pBonusMapDescription->szFileName,
																  m_pBonusMapDescription->szMapName,
	...
	else if(!stricmp(command, "StartNewGame"))
	{
		...
		else
		{
			...
			else
			{
				...
				PostMessage((vgui::Panel *)this,
							new KeyValues("command", "command", "StartNewGameWithAutosaveExplanation"), 0.2f);
			}
		}
	}
