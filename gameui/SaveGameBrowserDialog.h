...
public:
	...
	bool IsAutoSaveType(void)
	{
		return (Q_stristr(m_SaveInfo.szType, "autosave") != 0);
	}

	const SaveGameDescription_t *GetSaveInfo(void)
	{
		return (const SaveGameDescription_t *)&m_SaveInfo;
	}
...
public:
	...
	unsigned int GetNumPanels(void)
	{
		return m_SavePanels.Count();
	}
	bool HasActivePanels(void)
	{
		return (m_SavePanels.Count() != 0);
	}
	CGameSavePanel *GetActivePanel(void);
	int GetActivePanelIndex(void)
	{
		return m_iSelectedSave;
	}
	CFooterPanel *GetFooterPanel(void)
	{
		return m_pFooter;
	}
	const SaveGameDescription_t *GetPanelSaveDecription(int idx)
	{
		return (IsValidPanel(idx) ? m_SavePanels[idx]->GetSaveInfo() : NULL);
	}
	const SaveGameDescription_t *GetActivePanelSaveDescription(void)
	{
		return GetPanelSaveDecription(m_iSelectedSave);
	}

	uint GetStorageSpaceUsed(void)
	{
		return m_nUsedStorageSpace;
	}

	void SetSelectedSaveIndex(int index);
	void SetSelectedSave(const char *chapter);
	void AddPanel(CGameSavePanel *pPanel)
	{
		m_SavePanels.AddToHead(pPanel);
	}
	...
	void SetControlDisabled(bool bState)
	{
		m_bControlDisabled = bState;
	}
