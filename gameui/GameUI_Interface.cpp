...
{
	if(!pchFileName || pchFileName[0] == '\0' || !pchMapName || pchMapName[0] == '\0')
	{
...
void CGameUI::BonusMapChallengeUpdate(const char *pchFileName, const char *pchMapName, const char *pchChallengeName,
									  int iBest)
{
	if(!pchFileName || pchFileName[0] == '\0' || !pchMapName || pchMapName[0] == '\0' || !pchChallengeName ||
	...
	if(fileNames.Count() > 0)
	{
		...
		if(fileNames.IsValidIndex(index) && fileNames[index])
		{
			...
			if(pGameDir)
			{
				...
				bIsTF = nStrLen && nStrLen >= 2 && pGameDir[nStrLen - 2] == 't' && pGameDir[nStrLen - 1] == 'f';
			}
...
void CGameUI::UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam, byte cVoiceState, int nPlayersNeeded,
							   bool bHost)
{
