...
//			*pAchievement - source achievement to poll for progress. Non progress achievements will not show a percentage
//bar.
//-----------------------------------------------------------------------------
void UpdateProgressBar(vgui::EditablePanel *pPanel, IAchievement *pAchievement, Color clrProgressBar)
{
	if(pAchievement->GetGoal() > 1)
	{
		...
		// if this achievement gets saved with game and we're not in a level and have not achieved it, then we do not
		// have any state for this achievement, don't show progress
		...
		else if(bShowProgress)
		{
			...
			// In rare cases count can exceed goal and not be achieved (switch local storage on X360, take saved game
...
#ifdef WIN32
		V_snwprintf(wszProgressText, ARRAYSIZE(wszProgressText), L"%d%% %s", (m_nUnlocked * 100) / m_nTotalAchievements,
...
void CAchievementsDialog_XBox::ApplySettings(KeyValues *pResourceData)
{
	...
	for(int i = 0; i < iAllAchievements; ++i)
	{
		...
		CAchievementItem *pItem = m_Menu.AddAchievementItem(
			ACHIEVEMENT_LOCALIZED_NAME(pCurAchievement), ACHIEVEMENT_LOCALIZED_DESC(pCurAchievement),
			pCurAchievement->GetPointValue(), pCurAchievement->IsAchieved(), pCurAchievement);
...
CAchievementsDialog::CAchievementsDialog(vgui::Panel *parent) : BaseClass(parent, "AchievementsDialog")
{
	...
	m_pSelectionHighlight =
		new ImagePanel(m_pAchievementsList->FindChildByName("PanelListEmbedded"), "SelectionHighlight");
...
CAchievementsDialog::~CAchievementsDialog()
{
	if(achievementmgr)
	{
		achievementmgr->SaveGlobalStateIfDirty(
			false); // check for saving here to store achievements we want pinned to HUD
	}
		...
		if(achievementmgr)
		{
			...
			for(int i = 0; i < achievements.Count(); i++)
			{
				CAchievementDialogItemPanel *achievementItemPanel =
					new CAchievementDialogItemPanel(m_pAchievementsList, "AchievementDialogItemPanel", i);
...
void CAchievementsDialog::ApplySchemeSettings(IScheme *pScheme)
{
	...
	V_sprintf_safe(szPercentageText, "%d / %d ( %d%% )", m_nUnlocked, m_nTotalAchievements,
	...
	Color cProgressBar = Color(
		static_cast<float>(clrHighlight.r()) * (1.0f - flCompletion) + static_cast<float>(clrWhite.r()) * flCompletion,
...
void CAchievementsDialog::OnKeyCodePressed(vgui::KeyCode code)
{
	...
	{
		if(GameSupportsAchievementTracker() && m_nScrollItem >= 0 &&
		   m_nScrollItem < m_pAchievementsList->GetItemCount())
		{
			CAchievementDialogItemPanel *pItem =
				dynamic_cast<CAchievementDialogItemPanel *>(m_pAchievementsList->GetItemPanel(m_nScrollItem));
			...
			nButtonCode == STEAMCONTROLLER_DPAD_UP || nButtonCode == KEY_UP)
	{
		ScrollToItem(-1);
	}
...
void CAchievementsDialog::UpdateAchievementDialogInfo(void)
{
	...
	if(achievementmgr)
	{
		...
		V_sprintf_safe(szPercentageText, "%d / %d ( %d%% )", m_AchievementGroups[0].m_iNumUnlocked,
					   m_nTotalAchievements, (int)(flCompletion * 100.0f));
...
void CAchievementsDialog::CreateOrUpdateComboItems(bool bCreate)
{
	for(int i = 0; i < m_iNumAchievementGroups; i++)
	{
		...
		if(wzGroupName)
		{
			...
			V_snwprintf(wzNumAchievements, ARRAYSIZE(wzNumAchievements), L"%d",
						m_AchievementGroups[i].m_iNumAchievements);

			g_pVGuiLocalize->ConstructString(wzGroupTitle, sizeof(wzGroupTitle), wzGroupName, 2, wzNumUnlocked,
											 wzNumAchievements);
		}
	...
	if(bCreate && (m_iNumAchievementGroups > NUM_COMBO_BOX_LINES_DEFAULT))
	{
		...
		{
			m_pAchievementPackCombo->SetNumberOfEditLines((m_iNumAchievementGroups <= NUM_COMBO_BOX_LINES_MAX)
															  ? m_iNumAchievementGroups
															  : NUM_COMBO_BOX_LINES_MAX);
		}
	}
}
...
// Purpose: creates child panels, passes down name to pick up any settings from res files.
//-----------------------------------------------------------------------------
CAchievementDialogItemPanel::CAchievementDialogItemPanel(vgui::PanelListPanel *parent, const char *name,
														 int iListItemID)
...
void CAchievementDialogItemPanel::UpdateAchievementInfo(IScheme *pScheme)
{
	if(m_pSourceAchievement && m_pSchemeSettings)
	{
		...
		else
		{
			...
			{
				m_pShowOnHUDCheck->SetVisible(
					!m_pSourceAchievement->ShouldHideUntilAchieved()); // m_pSourceAchievement->GetGoal() > 1 &&
