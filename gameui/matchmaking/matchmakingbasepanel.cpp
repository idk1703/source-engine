...
void CMatchmakingBasePanel::OnCommand(const char *pCommand)
{
	...
	else if(Q_stristr(pCommand, "StartQuickMatchClient_"))
	{
		...
		{
			// Set the basic flags
			matchmaking->AddSessionProperty(SESSION_CONTEXT, "CONTEXT_GAME_MODE", "CONTEXT_GAME_MODE_MULTIPLAYER",
											NULL);
		...
		if(ret == ERROR_SUCCESS)
		{
			...
			if(info.dwInfoFlags & XUSER_INFO_FLAG_LIVE_ENABLED)
			{
				...
				if(state == eXUserSigninState_SignedInToLive)
				{
					...
					if(ERROR_SUCCESS != dwPrivCheck || !bPrivCheck)
					{
...
//-----------------------------------------------------------------------------
void CMatchmakingBasePanel::UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam, byte cVoiceState,
...
//-----------------------------------------------------------------------------
void CMatchmakingBasePanel::SessionSearchResult(int searchIdx, void *pHostData, XSESSION_SEARCHRESULT *pResult,
												int ping)
{
