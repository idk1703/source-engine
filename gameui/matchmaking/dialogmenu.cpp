...
//-----------------------------------------------------------------------
CBrowserItem::CBrowserItem(CDialogMenu *pParent, const char *pHost, const char *pPlayers, const char *pScenario,
						   const char *pPing)
...
//-----------------------------------------------------------------------
COptionsItem::COptionsItem(CDialogMenu *pParent, const char *pLabel) : BaseClass(pParent, pLabel, NULL)
{
...
void COptionsItem::PerformLayout()
{
	...
	int optionWide =
		max(m_nOptionsMinWide, GetWide() - m_nOptionsXPos - m_pRightArrow->GetWide() - m_nOptionsLeftMargin);
...
//-----------------------------------------------------------------------
CAchievementItem::CAchievementItem(CDialogMenu *pParent, const wchar_t *pName, const wchar_t *pDesc, uint points,
								   bool bUnlocked, IAchievement *pSourceAchievement)
	: BaseClass(pParent, "", "")
{
...
void CSectionedItem::PerformLayout()
{
	...
	for(int i = 0; i < m_Sections.Count(); ++i)
	{
		...
		pLabel->SetTextInset(
			10, m_bHeader ? 5 : m_pParent->GetColumnYPos(i)); // only use ypos for the y-inset if we're not a header
	}
}
...
CBrowserItem *CDialogMenu::AddBrowserItem(const char *pHost, const char *pPlayers, const char *pScenario,
										  const char *pPing)
{
...
CAchievementItem *CDialogMenu::AddAchievementItem(const wchar_t *pName, const wchar_t *pDesc, uint points,
												  bool bUnlocked, IAchievement *pSourceAchievement)
{
	return (CAchievementItem *)AddItemInternal(
		new CAchievementItem(this, pName, pDesc, points, bUnlocked, pSourceAchievement));
}
