...
CBaseGameStats *gamestats =
	&s_GameStats_Singleton; // start out pointing at the basic version which does nothing by default
extern ConVar skill;
...
void CBaseGameStats::Event_LevelShutdown(float flElapsed)
{
	...
	StatsLog("CBaseGameStats::Event_LevelShutdown [%s] %.2f elapsed %d total\n", CBGSDriver.m_PrevMapName.String(),
			 flElapsed, gamestats->m_BasicStats.m_Summary.m_nSeconds);
#endif // GAME_DLL
}
...
void CBaseGameStats::Event_PlayerTraveled(CBasePlayer *pBasePlayer, float distanceInInches, bool bInVehicle,
										  bool bSprinting)
{
}
...
{
	StatsLog("CBaseGameStats::Event_WeaponFired [%s] %s weapon [%s]\n", pShooter->GetPlayerName(),
			 bPrimary ? "primary" : "secondary", pchWeaponName);
}

void CBaseGameStats::Event_WeaponHit(CBasePlayer *pShooter, bool bPrimary, char const *pchWeaponName,
									 const CTakeDamageInfo &info)
{
	StatsLog("CBaseGameStats::Event_WeaponHit [%s] %s weapon [%s] damage [%f]\n", pShooter->GetPlayerName(),
			 bPrimary ? "primary" : "secondary", pchWeaponName, info.GetDamage());
}
...
void CBaseGameStats::Event_IncrementCountedStatistic(const Vector &vecAbsOrigin, char const *pchStatisticName,
													 float flIncrementAmount)
{
	StatsLog("Incrementing %s by %f at pos (%d, %d, %d)\n", pchStatisticName, flIncrementAmount, (int)vecAbsOrigin.x,
			 (int)vecAbsOrigin.y, (int)vecAbsOrigin.z);
}
...
bool CBaseGameStats::LoadFromFile(void)
{
	...
	{
		char fullpath[512];
		filesystem->RelativePathToFullPath(gamestats->GetStatSaveFileName(), GAMESTATS_PATHID, fullpath,
										   sizeof(fullpath));
	...
	if(filesystem->ReadFile(gamestats->GetStatSaveFileName(), GAMESTATS_PATHID, buf))
	{
		...
		if(s_szPseudoUniqueID[0] != 0)
		{
			...
			else
			{
				...
				{
					// the GAMESTATS_STANDARD_NOT_SAVED flag coincides with user completion time, rewind so the
					// gamestats parser can grab it
...
void CBaseGameStats_Driver::UpdatePerfStats(void)
{
	...
	if((m_flLastSampleTime == -1) || (flCurTime - m_flLastSampleTime >= STATS_RECORD_INTERVAL))
	{
...
void CBaseGameStats_Driver::LevelInitPreEntity()
{
	...
	if(Q_stricmp(s_szPseudoUniqueID, "unknown") == 0)
	{
		...
		// Change the unique ID to be a hash of IP & port.  We couldn't do this earlier because IP is not known until
		// level init time.
...
#ifdef GAME_DLL
	// for server, check with the engine to see if there already a gamestats data container registered.  (There will be
	// if there is a client running in the same process.)
...
{
#ifdef GAME_DLL
	// on the server, if there is a gamestats data container registered (by a client in the same process), they're in
	// charge of resetting it, nothing for us to do
...
static ConCommand resetGameStats("_resetgamestats", CC_ResetGameStats,
...
void CPointGamestatsCounter::InputIncrement(inputdata_t &inputdata)
{
	...
	{
		DevMsg(1,
			   "CPointGamestatsCounter::InputIncrement:  No stat name specified for point_gamestats_counter @%f, %f, "
			   "%f [ent index %d]\n",
