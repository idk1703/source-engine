...
CSheetSimulator::CSheetSimulator(TraceLineFunc_t traceline, TraceHullFunc_t traceHull)
...
void CSheetSimulator::ComputeForces()
{
	...
	for(i = 0; i < m_Springs.Size(); ++i)
	{
		...
		if(m_Springs[i].m_Particle2 < 0)
		{
			...
			dx = m_Particle[m_Springs[i].m_Particle1].m_Position - m_ControlPoints[-m_Springs[i].m_Particle2 - 1];
		...
		else
		{
			...
			dx = m_Particle[m_Springs[i].m_Particle1].m_Position - m_Particle[m_Springs[i].m_Particle2].m_Position;
			dv = m_Particle[m_Springs[i].m_Particle1].m_Velocity - m_Particle[m_Springs[i].m_Particle2].m_Velocity;
...
void CSheetSimulator::InitPosition(int i)
{
	...
	trace_t tr;
	m_TraceHull(m_Origin, m_ControlPoints[i], m_FrustumBoxMin, m_FrustumBoxMax, MASK_SOLID_BRUSHONLY, m_CollisionGroup,
				&tr);
...
void CSheetSimulator::EulerStep(float dt)
{
	...
	for(int i = 0; i < NumParticles(); ++i)
	{
		...
		assert(IsFinite(m_Particle[i].m_Velocity.x) && IsFinite(m_Particle[i].m_Velocity.y) &&
