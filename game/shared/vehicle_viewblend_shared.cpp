...
BEGIN_SIMPLE_DATADESC(ViewSmoothingData_t)
	DEFINE_FIELD(vecAnglesSaved, FIELD_VECTOR), DEFINE_FIELD(vecOriginSaved, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(vecAngleDiffSaved, FIELD_VECTOR), DEFINE_FIELD(vecAngleDiffMin, FIELD_VECTOR),
		DEFINE_FIELD(bRunningEnterExit, FIELD_BOOLEAN), DEFINE_FIELD(bWasRunningAnim, FIELD_BOOLEAN),
		DEFINE_FIELD(flEnterExitStartTime, FIELD_FLOAT), DEFINE_FIELD(flEnterExitDuration, FIELD_FLOAT),
...
//-----------------------------------------------------------------------------
float ApplyViewLocking(float flAngleRaw, float flAngleClamped, ViewLockData_t &lockData,
	...
	else
	{
		...
		if(!lockData.bLocked)
		{
			if(lockData.flUnlockTime != 0)
			{
				...
				float flBlend = RemapValClamped(gpGlobals->curtime - lockData.flUnlockTime, 0,
...
void RemapViewAngles(ViewSmoothingData_t *pData, QAngle &vehicleEyeAngles)
{
	...
	vecEyeAnglesRemapped.x =
		RemapAngleRange(pData->flPitchCurveZero, pData->flPitchCurveLinear, vehicleEyeAngles.x, &ePitchCurvePart);
	...
	float flRollDamped =
		RemapAngleRange(pData->flRollCurveZero, pData->flRollCurveLinear, vecEyeAnglesRemapped.z, &eRollCurvePart);
	vecEyeAnglesRemapped.z = Lerp(flBlendRoll, vecEyeAnglesRemapped.z, flRollDamped);

	// Msg("PITCH ");
	vehicleEyeAngles.x =
		ApplyViewLocking(vehicleEyeAngles.x, vecEyeAnglesRemapped.x, pData->pitchLockData, ePitchCurvePart);

	// Msg("ROLL ");
	vehicleEyeAngles.z =
		ApplyViewLocking(vehicleEyeAngles.z, vecEyeAnglesRemapped.z, pData->rollLockData, eRollCurvePart);
}

//-----------------------------------------------------------------------------
// Purpose: Vehicle dampening shared between server and client
//-----------------------------------------------------------------------------
void SharedVehicleViewSmoothing(CBasePlayer *pPlayer, Vector *pAbsOrigin, QAngle *pAbsAngles, bool bEnterAnimOn,
								bool bExitAnimOn, const Vector &vecEyeExitEndpoint, ViewSmoothingData_t *pData,
								float *pFOV)
{
	...
	*pAbsAngles = pPlayer->EyeAngles();
	...
	if(bRunningAnim || pData->bRunningEnterExit)
	{
		*pAbsAngles = vehicleEyeAngles;
		...
		if(frac < 1.0)
		{
			...
			// debugoverlay->AddBoxOverlay( vehicleEyeOrigin, -Vector(1,1,1), Vector(1,1,1), vec3_angle, 0,255,255, 64,
			// 10 );
		}
		else
		{
	...
	if(bRunningAnim || pData->bRunningEnterExit)
	{
		...
		*pAbsAngles -= pData->vecAngleDiffMin;
		...
		if(bExitAnimOn)
		{
			// debugoverlay->AddBoxOverlay( vecEyeExitEndpoint, -Vector(1,1,1), Vector(1,1,1), vec3_angle, 255,255,255,
			// 64, 10 );
