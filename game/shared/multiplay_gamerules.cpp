...
ConVar mp_chattime("mp_chattime", "10", FCVAR_REPLICATED, "amount of time players can chat after the game is over",
				   true, 1, true, 120);
...
ConVar mp_show_voice_icons("mp_show_voice_icons", "1", FCVAR_REPLICATED,
...
ConVar tv_delaymapchange_protect("tv_delaymapchange_protect", "1", FCVAR_NONE,
								 "Protect against doing a manual map change if HLTV is broadcasting and has not caught "
								 "up with a major game event such as round_end");

ConVar mp_restartgame("mp_restartgame", "0", FCVAR_GAMEDLL,
					  "If non-zero, game will restart in the specified number of seconds");
ConVar mp_restartgame_immediate("mp_restartgame_immediate", "0", FCVAR_GAMEDLL,
								"If non-zero, game will restart immediately");

ConVar mp_mapcycle_empty_timeout_seconds(
	"mp_mapcycle_empty_timeout_seconds", "0", FCVAR_REPLICATED,
...
ConCommand skip_next_map("skip_next_map", cc_SkipNextMapInCycle,
						 "Skips the next map in the map rotation for the server.");
ConCommand changelevel_next("changelevel_next", cc_GotoNextMapInCycle,
...
ConVar mp_waitingforplayers_time("mp_waitingforplayers_time", "0", FCVAR_GAMEDLL,
								 "WaitingForPlayers time length in seconds");
#endif

ConVar mp_waitingforplayers_restart("mp_waitingforplayers_restart", "0", FCVAR_GAMEDLL,
									"Set to 1 to start or restart the WaitingForPlayers period.");
ConVar mp_waitingforplayers_cancel("mp_waitingforplayers_cancel", "0", FCVAR_GAMEDLL,
...
ConVar mp_clan_ready_signal("mp_clan_ready_signal", "ready", FCVAR_GAMEDLL,
...
{
	int iDamage =
		(DMG_POISON | DMG_ACID | DMG_DROWN | DMG_BURN | DMG_SLOWBURN | DMG_NERVEGAS | DMG_RADIATION | DMG_SHOCK);
	return iDamage;
}
...
int CMultiplayRules::Damage_GetNoPhysicsForce(void)
{
	...
	int iDamage = (DMG_FALL | DMG_BURN | DMG_PLASMA | DMG_DROWN | iTimeBasedDamage | DMG_CRUSH | DMG_PHYSGUN |
...
bool CMultiplayRules::Damage_IsTimeBased(int iDmgType)
{
	...
	return ((iDmgType & (DMG_PARALYZE | DMG_NERVEGAS | DMG_POISON | DMG_RADIATION | DMG_DROWNRECOVER | DMG_ACID |
						 DMG_SLOWBURN)) != 0);
}
...
bool CMultiplayRules::Damage_ShowOnHUD(int iDmgType)
{
	...
	return ((iDmgType & (DMG_POISON | DMG_ACID | DMG_DROWN | DMG_BURN | DMG_SLOWBURN | DMG_NERVEGAS | DMG_RADIATION |
						 DMG_SHOCK)) != 0);
}
...
bool CMultiplayRules::Damage_NoPhysicsForce(int iDmgType)
{
	...
	int iTimeBasedDamage = Damage_GetTimeBased();
	return ((iDmgType & (DMG_FALL | DMG_BURN | DMG_PLASMA | DMG_DROWN | iTimeBasedDamage | DMG_CRUSH | DMG_PHYSGUN |
...
void CMultiplayRules::FrameUpdatePostEntityThink()
{
	...
	if(mp_mapcycle_empty_timeout_seconds.GetInt() > 0)
	{
		...
		if(iIdleSeconds >= mp_mapcycle_empty_timeout_seconds.GetInt())
		{
			...
			Log("Server has been empty for %d seconds on this map, cycling map as per "
...
//=========================================================
//=========================================================
bool CMultiplayRules::ClientConnected(edict_t *pEntity, const char *pszName, const char *pszAddress, char *reject,
									  int maxrejectlen)
{
...
CBaseEntity *CMultiplayRules::GetPlayerSpawnSpot(CBasePlayer *pPlayer)
{
	...
	/*
		...
		{
			FireTargets( STRING(pentSpawnSpot->m_target), pPlayer, pPlayer, USE_TOGGLE, 0 ); // dvsents2: what is this
	   code supposed to do?
		}
	*/
	...
	if(HLTVDirector()->IsActive() && (HLTVDirector()->GetDelay() >= HLTV_MIN_DIRECTOR_DELAY))
	{
		if(tv_delaymapchange.GetBool() && tv_delaymapchange_protect.GetBool())
		{
			...
			if(flLastEvent > -1)
			{
				if(flLastEvent >
				   (gpGlobals->curtime -
					(HLTVDirector()->GetDelay() + 3))) // +3 second delay to prevent instant change after a major event
				{
					*pszReason = "\n***WARNING*** Map change blocked. HLTV is broadcasting and has not caught up to "
								 "the last major game event yet.\nYou can disable this check by setting the value of "
								 "the server convar \"tv_delaymapchange_protect\" to 0.\n";
					return false;
				}
			}
		}
...
void CMultiplayRules::LoadMapCycleFile(void)
{
	...
	if(g_pStringTableServerMapCycle)
	{
		...
		g_pStringTableServerMapCycle->AddString(CBaseEntity::IsServer(), "ServerMapCycle", sFileList.Length() + 1,
												sFileList.String());
	}
	...
	if(g_pStringTableServerPopFiles)
	{
		...
		{
			g_pStringTableServerPopFiles->AddString(CBaseEntity::IsServer(), "ServerPopFiles", sFileList.Length() + 1,
													sFileList.String());
		}
	}
	...
	if(g_pStringTableServerMapCycleMvM)
	{
		...
		if(pKV->LoadFromFile(g_pFullFileSystem, tf_mvm_missioncyclefile.GetString(), "MOD"))
		{
			...
			if(mapList.Count())
			{
				...
				g_pStringTableServerMapCycleMvM->AddString(CBaseEntity::IsServer(), "ServerMapCycleMvM",
														   sFileList.Length() + 1, sFileList.String());
			}
	...
	if(nOldCycleIndex >= 0 && nOldCycleIndex < m_MapList.Count() && nOldPreviousMap >= 0 &&
	   nOldPreviousMap < m_MapList.Count() && V_strcmp(STRING(gpGlobals->mapname), m_MapList[nOldPreviousMap]) == 0)
	{
	...
	if(pKV->LoadFromFile(filesystem, "scripts/voicecommands.txt", "GAME"))
	{
		for(KeyValues *menu = pKV->GetFirstSubKey(); menu != NULL; menu = menu->GetNextKey())
		{
			...
			for(KeyValues *menuitem = menu->GetFirstSubKey(); menuitem != NULL; menuitem = menuitem->GetNextKey())
			{
				...
				{
					Warning("Voicecommand script attempting to use unknown concept. Need to define new concepts in "
							"code. ( %s )\n",
...
void CMultiplayRules::HaveAllPlayersSpeakConceptIfAllowed(int iConcept, int iTeam /* = TEAM_UNASSIGNED */,
...
void CMultiplayRules::RandomPlayersSpeakConceptIfAllowed(int iConcept, int iNumRandomPlayer /*= 1*/,
