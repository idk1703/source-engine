...
ConVar sv_pushaway_clientside_size("sv_pushaway_clientside_size", "15", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
								   "Minimum size of pushback objects");
ConVar props_break_max_pieces("props_break_max_pieces", "-1", 0,
							  "Maximum prop breakable piece count (-1 = model default)");
ConVar props_break_max_pieces_perframe("props_break_max_pieces_perframe", "-1", FCVAR_REPLICATED,
...
extern bool PropBreakableCapEdictsOnCreateAll(int modelindex, IPhysicsObject *pPhysics,
...
int CPropData::ParsePropFromKV(CBaseEntity *pProp, KeyValues *pSection, KeyValues *pInteractionSection)
{
	...
	pBreakableInterface->SetPhysicsMode(pSection->GetInt("physicsmode", pBreakableInterface->GetPhysicsMode()));
	...
	pBreakableInterface->SetDmgModExplosive(
		pSection->GetFloat("dmg.explosive", pBreakableInterface->GetDmgModExplosive()));
	...
	if(!FClassnameIs(pProp, "prop_physics_override") && !FClassnameIs(pProp, "prop_dynamic_override"))
	{
		...
		// Explosive?
		pBreakableInterface->SetExplosiveDamage(
			pSection->GetFloat("explosive_damage", pBreakableInterface->GetExplosiveDamage()));
		pBreakableInterface->SetExplosiveRadius(
			pSection->GetFloat("explosive_radius", pBreakableInterface->GetExplosiveRadius()));
	...
	pBreakableInterface->SetBreakableCount(
		pSection->GetInt("breakable_count", pBreakableInterface->GetBreakableCount()));
...
void BuildPropList(const char *pszBlockName, CUtlVector<breakmodel_t> &list, int modelindex, float defBurstScale,
...
void PropBreakableCreateAll(int modelindex, IPhysicsObject *pPhysics, const breakablepropparams_t &params,
							CBaseEntity *pEntity, int iPrecomputedBreakableCount, bool bIgnoreGibLimit,
...
#endif
			{
				pBreakable = BreakModelCreateSingle(pOwnerEntity, &list[i], position, angles, objectVelocity,
		   ...
		   pBreakableInterface->GetBreakableCount())
		{
			...
			for(int i = 0; i < pBreakableInterface->GetBreakableCount(); i++)
			{
				...
				Q_strncpy(breakModel.modelName,
						  g_PropDataSystem.GetRandomChunkModel(STRING(pBreakableInterface->GetBreakableModel()),
				...
				{
					pBreakable =
						BreakModelCreateSingle(pOwnerEntity, &breakModel, breakModel.offset, vecAngles, vecVelocity,
											   vec3_origin /*params.angularVelocity*/, iSkin, params);
					if(!pBreakable)
					{
...
void PropBreakableCreateAll(int modelindex, IPhysicsObject *pPhysics, const Vector &origin, const QAngle &angles,
							const Vector &velocity, const AngularImpulse &angularVelocity, float impactEnergyScale,
							float defBurstScale, int defCollisionGroup, CBaseEntity *pEntity, bool defaultLocation)
{
...
void PrecachePropsForModel(int iModel, const char *pszBlockName)
{
	...
	// The scale and group doesn't really matter at the moment, we are just using the parser to get the model name to
	// cache.
...
//			defaultLocation -
//-----------------------------------------------------------------------------
CBaseEntity *CreateGibsFromList(CUtlVector<breakmodel_t> &list, int modelindex, IPhysicsObject *pPhysics,
								const breakablepropparams_t &params, CBaseEntity *pEntity,
								int iPrecomputedBreakableCount, bool bIgnoreGibLimit, bool defaultLocation,
								CUtlVector<EHANDLE> *pGibList, bool bBurning)
{
	// Check for prop breakable count reset.
	int nPropCount = props_break_max_pieces_perframe.GetInt();
	if(nPropCount != -1)
