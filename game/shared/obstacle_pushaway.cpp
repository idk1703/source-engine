...
//-----------------------------------------------------------------------------------------------------
int GetPushawayEnts(CBaseCombatCharacter *pPushingEntity, CBaseEntity **ents, int nMaxEnts, float flPlayerExpand,
					int PartitionMask, CPushAwayEnumerator *enumerator)
{
...
void AvoidPushawayProps(CBaseCombatCharacter *pPlayer, CUserCmd *pCmd)
{
	...
	for(int i = 0; i < nEnts; i++)
	{
		...
		// We can get right up next to rotating doors before they start to move, so scale back our force so we don't go
		// flying
...
void PerformObstaclePushaway(CBaseCombatCharacter *pPushingEntity)
{
	...
	int nEnts =
		GetPushawayEnts(pPushingEntity, props, ARRAYSIZE(props), 3.0f, PARTITION_CLIENT_RESPONSIVE_EDICTS, NULL);
#else
	...
	for(int i = 0; i < nEnts; i++)
	{
		// If this entity uas PHYSICS_MULTIPLAYER_FULL set (ie: it's not just debris), and we're moving too slow, don't
		// push it away. Instead, let the client bounce off it. This allows players to get close to and duck behind
