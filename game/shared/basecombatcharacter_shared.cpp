...
bool CBaseCombatCharacter::IsAbleToSee(const CBaseEntity *pEntity, FieldOfViewCheckType checkFOV)
{
	...
	if(flDistToOther > NavObscureRange.GetFloat())
	{
		...
		TerrorNavArea *pTargetArea =
			static_cast<TerrorNavArea *>(TheNavMesh->GetNearestNavArea(vecTargetPosition, false, flMaxDistance));
...
bool CBaseCombatCharacter::IsAbleToSee(CBaseCombatCharacter *pBCC, FieldOfViewCheckType checkFOV)
{
	...
	if(nResult == VISCACHE_UNKNOWN)
	{
		...
		if(ComputeLOS(vecEyePosition, vecOtherEyePosition))
		{
			...
			{
				bThisCanSeeOther =
					!ComputeTargetIsInDarkness(vecEyePosition, pBCC->GetLastKnownArea(), vecOtherEyePosition);
...
bool CBaseCombatCharacter::ComputeLOS(const Vector &vecEyePosition, const Vector &vecTarget) const
{
	...
	UTIL_TraceLine(vecEyePosition, vecTarget, MASK_OPAQUE | CONTENTS_IGNORE_NODRAW_OPAQUE | CONTENTS_MONSTER,
				   &traceFilter, &result);
	return (result.fraction == 1.0f);
}

#if defined(GAME_DLL) && defined(TERROR)
bool CBaseCombatCharacter::ComputeTargetIsInDarkness(const Vector &vecEyePosition, CNavArea *pTargetNavArea,
													 const Vector &vecTargetPos) const
{
	...
	trace_t result;
	UTIL_TraceLine(vecTargetPos, vecTargetPos + vecSightDirection * 32768.0f, MASK_L4D_VISION, &lightingFilter,
				   &result);
...
{
	return IsLookingTowards(target->WorldSpaceCenter(), cosTolerance) ||
		   IsLookingTowards(target->EyePosition(), cosTolerance) ||
		   IsLookingTowards(target->GetAbsOrigin(), cosTolerance);
}
...
bool CBaseCombatCharacter::IsLineOfSightClear(const Vector &pos, LineOfSightCheckType checkType,
											  CBaseEntity *entityToIgnore) const
{
	...
	if(checkType == IGNORE_ACTORS)
	{
		...
		return IsLineOfSightBetweenTwoEntitiesClear(const_cast<CBaseCombatCharacter *>(this), EOFFSET_MODE_EYEPOSITION,
													entityToIgnore, EOFFSET_MODE_WORLDSPACE_CENTER, entityToIgnore,
													COLLISION_GROUP_NONE, MASK_L4D_VISION,
													TraceFilterNoCombatCharacters, 1.0);
#else
		trace_t trace;
		CTraceFilterNoCombatCharacters traceFilter(entityToIgnore, COLLISION_GROUP_NONE);
		UTIL_TraceLine(EyePosition(), pos, MASK_OPAQUE | CONTENTS_IGNORE_NODRAW_OPAQUE | CONTENTS_MONSTER, &traceFilter,
					   &trace);
