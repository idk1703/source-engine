...
public:
	...
	{
		CSchemaAttributeStats::RegisterAttributeType<ISchemaAttributeTypeBase<TAttribInMemoryType>,
													 TAttribInMemoryType>();

		// The implementation of the attributes-in-memory system is such that it may or may not behave according to
		// expectations. Rather than have to stare at all the details to answer questions about where memory is
		// allocated or managed, or when it will be freed, for all our current use cases it makes more sense to just
		// disable raw pointer types from being an attribute-in-memory type and instead steer people towards this
												  ...
												  const CEconGameAccount *pGameAccount) const OVERRIDE
	{
		...
		Assert(pAttrDef);
		AssertMsg(
			!staticAttrib.m_pKVCustomData,
			"Default implementation of LoadOrGenerateEconAttributeValue() doesn't support custom value generation!");
	...
	virtual void GenerateEconAttributeValue(const CEconItemAttributeDefinition *pAttrDef,
											...
											attribute_data_union_t *out_pValue) const OVERRIDE
	{
		Assert(pAttrDef);
		Assert(pGameAccount);
		Assert(out_pValue);
	...
	virtual void LoadEconAttributeValue(CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef,
										const union attribute_data_union_t &value) const OVERRIDE;
	...
	void ConvertTypedValueToEconAttributeValue(const TAttribInMemoryType &typedValue,
											   attribute_data_union_t *out_pValue) const
	{
		// If our type is smaller than an int, we don't know how to copy the memory into our flat structure. We could
		// write this code but we have no use case for it now so this is set up to fail so if someone does come up with
		// a use case they know where to fix.
		...
		// We're too big for our flat structure. We need to allocate space somewhere outside our attribute instance and
		// point to that.
		else
		{
	...
	void ConvertEconAttributeValueToTypedValue(const attribute_data_union_t &value,
											   TAttribInMemoryType *out_pTypedValue) const
	{
	...
	virtual bool OnIterateAttributeValue(IEconItemAttributeIterator *pIterator,
	...
	virtual void LoadByteStreamToEconAttributeValue(CEconItem *pTargetItem,
													const CEconItemAttributeDefinition *pAttrDef,
													const std::string &sBytes) const OVERRIDE;
	virtual void ConvertEconAttributeValueToByteStream(const attribute_data_union_t &value,
													   ::std::string *out_psBytes) const;

	virtual void ConvertTypedValueToByteStream(const TAttribInMemoryType &typedValue,
											   ::std::string *out_psBytes) const = 0;
	virtual void ConvertByteStreamToTypedValue(const ::std::string &sBytes,
											   TAttribInMemoryType *out_pTypedValue) const = 0;
...
public:
	...
	// called to determine if this item is tradable or not. This will return the time after which it can be traded. If 0
	// it can be traded. This is needed since the base implementation of this is protected
	RTime32 GetTradableAfterDateTime() const
	{
		return IEconItemInterface::GetTradableAfterDateTime();
	}

	// called to set a tradable after date/time value onto this item (this avoids a lot of potential inefficiencies
	// around this process)
...
public:
	...
	void SetDynamicAttributeValue(const CEconItemAttributeDefinition *pAttrDef, const T &value)
	{
		...
		AssertMsg4(
			!::FindAttribute(GetItemDefinition(), pAttrDef),
			"Item id %llu (%s) attempting to set dynamic attribute value for '%s' (%d) when static attribute exists!",
			GetItemID(), GetItemDefinition()->GetDefinitionName(), pAttrDef->GetDefinitionName(),
		...
		// T that make sense (ie., protobufs).
		{
			...
			AssertMsg1(sBytes == sReadBytes, "SetDynamicAttributeValue(): read/write mismatch for attribute '%s'.",
					   pAttrDef->GetDefinitionName());
		}
#endif // ENABLE_TYPED_ATTRIBUTE_PARANOIA
	}

	// Called to set a time stamp dynamic attribute on this item. But it will first check the current value assigned to
	// this item, and will only set it if this new time extends beyond the current one
...
public:
	...
	const CEconItem *YieldingGetInteriorItem() const
	{
		return const_cast<CEconItem *>(this)->YieldingGetInteriorItem();
	}

	void SetEquippedThisGameServerSession(bool bEquipped)
	{
		m_bEquippedThisGameServerSession = bEquipped;
	}
	bool EquippedThisGameServerSession() const
	{
		return m_bEquippedThisGameServerSession;
	}
#endif
...
protected:
	...
	attribute_t &AddDynamicAttributeInternal(); // add another chunk of data to our internal storage to store a new
												// attribute -- initialization is the responsibility of the caller
	attribute_t *FindDynamicAttributeInternal(
		const CEconItemAttributeDefinition
			*pAttrDef); // search for an instance of a dynamic attribute with this definition -- ignores static
						// properties, etc. and will return NULL if not found
	int GetDynamicAttributeCountInternal() const; // how many attributes are there attached to this instance?
	attribute_t &GetMutableDynamicAttributeInternal(
		int iAttrIndexIntoArray); // get a writable version of our attribute memory base chunk (added by
								  // AddDynamicAttributeInternal) for this index (same "array" as
								  // GetDynamicAttributeCountInternal)
	const attribute_t &GetDynamicAttributeInternal(
		int iAttrIndexIntoArray) const // read-only version of our attribute memory base chunk for this index (same
									   // "array" as GetDynamicAttributeCountInternal)
	{
...
public:
	...
	EquippedInstance_t m_EquipInstanceSingleton; // Where the item is equipped. Valid only if m_bHasEquipSingleton and
												 // there is no custom data
	attribute_t
		m_CustomAttribSingleton; // Custom attribute. Valid only if m_bHasAttribSingleton and there is no custom data
...
//-----------------------------------------------------------------------------
template<typename TAttribInMemoryType>
/*virtual*/ void ISchemaAttributeTypeBase<TAttribInMemoryType>::LoadByteStreamToEconAttributeValue(
	CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef, const std::string &sBytes) const
{
	Assert(pTargetItem);
	Assert(pAttrDef);
...
//-----------------------------------------------------------------------------
template<typename TAttribInMemoryType>
/*virtual*/ void ISchemaAttributeTypeBase<TAttribInMemoryType>::ConvertEconAttributeValueToByteStream(
	const attribute_data_union_t &value, ::std::string *out_psBytes) const
{
...
//-----------------------------------------------------------------------------
template<typename TAttribInMemoryType>
/*virtual*/ void ISchemaAttributeTypeBase<TAttribInMemoryType>::LoadEconAttributeValue(
	CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef,
	const union attribute_data_union_t &value) const
{
...
{
	static void AssignItemToSlot(CEconSharedObjectCache *pSOCache, CEconItem *pItem, equipped_class_t unClass,
								 equipped_slot_t unSlot, CEconUserSession *pOptionalSession = NULL);
};
#endif // GC_DLL
...
public:
		...
		: m_pLootList(NULL), m_unAuditDetailData(0), m_bIsDynamicallyAllocatedLootList(false)
	{
		...
		if(!BAttemptCrateSeriesInitialization(pEconItem) && !BAttemptLootListStringInitialization(pEconItem) &&
...
{
public:
	CEconItemHandle() : m_pItem(NULL), m_iItemID(INVALID_ITEM_ID) {}

	CEconItemHandle(CEconItem *pItem) : m_pItem(pItem)
	{
		SetItem(pItem);
	}
