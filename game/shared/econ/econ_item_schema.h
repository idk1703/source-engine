...
public:
	...
	bool BInitFromKV(KeyValues *pKVItem, KeyValues *pKVRarityWeights, CEconItemSchema &pschema,
...
public:
	...
	const char *GetLocalizeName() const
	{
		return m_pszLocalizedName;
	}
...
public:
	...
	const char *GetContractRewardLootlist(EContractRewardLootlist_t eType) const
	{
		return m_pszContractRewardLootlist[eType];
	}
	...
	uint8 GetNumSeededContracts() const
	{
		return m_unSeed;
	}
	uint16 GetNumMaxHeldDrops() const
	{
		return m_unMaxHeldDrops;
	}

	int GetNumMaxQueueCount() const
	{
		return m_nMaxQueueCount;
	}

	uint8 GetMaxDropPerThink() const
	{
		return m_unMaxDropPerThink;
	}
...
private:
	...
	item_definition_index_t m_unGatewayItemDefIndex; // Defindex of the item users need to acquire in order to get the
													 // required item.  Could be the required item itself.
...
#ifdef GC_DLL
	MUST_CHECK_RETURN virtual bool BGenerateSingleRollRandomItems(
		const CEconGameAccount *pGameAccount, bool bFreeAccount, CUtlVector<CEconItem *> *out_pvecItems,
		const CUtlVector<item_definition_index_t> *pVecAvoidItemDefs = NULL) const = 0;
#endif // GC_DLL
};
...
struct lootlist_attrib_t
{
	...
	bool BInitFromKV(const char *pszContext, KeyValues *pKVKey, CEconItemSchema &pschema,
...
public:
	...
	bool AddRandomAtrributes(KeyValues *pRandomAttributesKV, CEconItemSchema &pschema,
							 CUtlVector<CUtlString> *pVecErrors = NULL);
	bool AddRandomAttributesFromTemplates(KeyValues *pRandomAttributesKV, CEconItemSchema &pschema,
										  CUtlVector<CUtlString> *pVecErrors = NULL);

	// Generates a single roll for this loot list as well as each "additional drop" loot list specified. This will
	// return true if all items were created successfully or false if anything went wrong in any of the relevant
	// lootlists. All items created will be returned via out_pvecItems.
	MUST_CHECK_RETURN virtual bool BGenerateSingleRollRandomItems(
		const CEconGameAccount *pGameAccount, bool bFreeAccount, CUtlVector<CEconItem *> *out_pvecItems,
		const CUtlVector<item_definition_index_t> *pVecAvoidItemDefs = NULL) const OVERRIDE;

	void RollRandomAttributes(CUtlVector<static_attrib_t> &vecAttributes, const CEconGameAccount *pGameAccount) const;
	bool RollRandomItemsAndAdditionalItems(IUniformRandomStream *pRandomStream, bool bFreeAccount,
...
private:
	bool RollRandomItemDef(IUniformRandomStream *pRandomStream, bool bFreeAccount,
						   CUtlVector<rolled_item_defs_t> *out_pVecRolledItems,
						   const CUtlVector<item_definition_index_t> *pVecAvoidItemDefs = NULL) const;
	bool BIsInternalNoDupesLootList() const
	{
		return m_iNoDupesIterations >= 0;
	}
...
private:
	...
	int m_iNoDupesIterations; // if less than zero, "no dupes" functionality disabled; if greater than or equal to zero,
							  // the number of iterations we want to run through passing no-dupe sets
...
public:
...
#ifdef GC_DLL
	bool BIsCraftableByUnverifiedClients() const
	{
		return m_bIsCraftableByUnverifiedClient;
	}
#endif // GC_DLL
	...
	virtual bool ItemListMatchesInputs(CUtlVector<CEconItem *> *vecCraftingItems, KeyValues *out_pCraftParams = NULL,
									   bool bIgnoreSlop = false, CUtlVector<uint64> *vecChosenItems = NULL) const;
...
enum
{
	...
	ATTDESCFORM_VALUE_IS_INVERTED_PERCENTAGE, // Printed as:	((m_flValue*100)-100.0) if it's > 1.0, or
											  // ((1.0-m_flModifier)*100) if it's < 1.0
	...
	ATTDESCFORM_VALUE_IS_FROM_LOOKUP_TABLE, // Printed as a string from a lookup table, specified by the attribute
											// definition name
};
...
enum EAssetClassAttrExportRule_t
{
	...
	k_EAssetClassAttrExportRule_GCOnly =
		(1 << 2), // attribute only lives on GC and not exported to any external request
};
...
private:
	...
	// If this is true the attribute is counted as "instance" data for purposes of asset class in the Steam Economy.
	// Non-instance properties are considered things that can differentiate items at a fundamental level (ie.,
	// definition index, quality); instance properties are more things like additional customizations -- score for
	// strange items, paint color, etc.
	bool m_bInstanceData;
	EAssetClassAttrExportRule_t
		m_eAssetClassAttrExportRule; // if this is true the attribute will not be exported for asset class
...
enum wearableanimplayback_t
{
	...
	WAP_START_BUILDING, // Game code will start this anim whenever a player wearing this item deploys their builder
						// weapon.
	WAP_STOP_BUILDING,	// Game code will start this anim whenever a player wearing this item holsters their builder
						// weapon.
...
struct animation_on_wearable_t
{
	...
	int iReplacement; // Replacement activity to play. Might be set to one of
					  // kActivityLookup_Unknown/kActivityLookup_Missing.
...
public:
	...
	const CUtlVector<const char *> &GetAdditionalHideBodygroups() const
	{
...
enum item_capabilities_t
{
	...
	ITEM_CAP_CAN_BE_RESTORED =
		1 << 13, // can users remove properties (paint, nametag, etc.) from this item via the in-game UI?
...
enum
{
	ITEM_CAP_DEFAULT = ITEM_CAP_CAN_CRAFT_MARK | ITEM_CAP_CAN_BE_RESTORED | ITEM_CAP_CAN_USE_STRANGE_PARTS |
					   ITEM_CAP_CAN_CARD_UPGRADE | ITEM_CAP_CAN_STRANGIFY | ITEM_CAP_CAN_KILLSTREAKIFY |
					   ITEM_CAP_CAN_CONSUME | ITEM_CAP_CAN_GIFT_WRAP
...
public:
	IEconTool(const char *pszTypeName, const char *pszUseString, const char *pszUsageRestriction,
	...
	} // would like to disable on the client so we aren't tempted to check against it, but used for building a unique
	  // tool list
	...
	{
		Assert(
			!"IEconTool::ShouldShowContainedItemPanel(): we don't expect this to be called on anything besides gifts!");
		return false;
	}
	...
	virtual void OnClientApplyTool(class C_EconItemView *pTool, class C_EconItemView *pSubject,
	...
	virtual bool BGenerateDynamicAttributes(CEconItem *pItem, const CEconGameAccount *pGameAccount) const
	{
		return true;
	}
#endif // GC_DLL
...
public:
	...
	uint32 GetDefinitionIndex(void) const
	{
		return m_nDefIndex;
	}
	const char *GetDescriptionToken(void) const
	{
		return m_pszDescriptionToken;
	}
...
public:
	...
	} // what def index do we consider ourself for purposes of determining "is an item equipped that satisfies this set
	  // slot?" (ie., Festive Huntsman -> Huntsman); default is to point to itself
	...
	CEconItemPaintKitDefinition *GetCustomPainkKitDefinition(void) const
	{
		return m_pItemPaintKitDef;
	}
	void SetItemPaintKitDefinition(CEconItemPaintKitDefinition *pItemPaintKitDef)
	{
		Assert(!m_pItemPaintKitDef);
		m_pItemPaintKitDef = pItemPaintKitDef;
	}
...
public:
	...
	const CUtlVector<econ_tag_handle_t> &GetEconTags() const
	{
		return m_vecTags;
	} // meant for internal/debug use only, not for runtime iteration
	const CUtlVector<econ_item_payment_rule_t> &GetPaymentRules() const
	{
		return m_vecPaymentRules;
	}
...
private:
	...
	// This specifies an item quality that items from this definition must be set to. Used mostly to specify unique item
	// definitions.
	...
	// Alternatively, the image used for this item when it's displayed in inventory-style boxes. If specified, it's used
	// over the model.
	const char *m_pszInventoryImage;
	// An optional image that's overlayed over the top of the base inventory image. It'll be RGB colored by the tint
	// color of the item.
...
#ifdef TF_CLIENT_DLL
	uint32 m_unNumConcreteItems; // This is the number of items that will actually end up in a user's inventory - this
								 // can be 0 for some items (e.g. map stamps in TF), 1 for a "regular" item, or many for
								 // bundles, etc.
#endif							 // TF_CLIENT_DLL
	...
	// A pack item is an item which is not for sale individually and is only for sale as part of a pack bundle. A
	// 'regular' bundle can only include a pack bundle by explicitly including all of the pack bundle's items
	// individually. If this pointer is non-NULL, this item is considered to be a pack item (see
	// CEconItemDefinition::IsPackItem()).
	...
	equip_region_mask_t m_unEquipRegionConflictMask; // which equip regions does equipping this item prevent from having
													 // something in them

	item_definition_index_t m_unSetItemRemapDefIndex; // reference to the definition index we want to consider this item
													  // for set matching purposes; see GetSetItemRemap()
...
protected:
	...
	CUtlVector<const CEconItemDefinition *>
		m_vecContainingBundleItemDefs; // Item definition indices for any bundles which contain this item
...
// Purpose:
//-----------------------------------------------------------------------------
inline const char *CEconItemDefinition::GetCodeControlledBodyGroup(int iTeam, int i,
																   codecontrolledbodygroupdata_t &ccbgd) const
{
#ifndef CSTRIKE_DLL
	iTeam = GetBestVisualTeamData(iTeam);
	if(iTeam < 0 || iTeam >= TEAM_VISUAL_SECTIONS || !GetPerTeamVisual(iTeam))
...
inline int CEconItemDefinition::GetStyleSkin(style_index_t unStyle, int iTeam, bool bViewmodel) const
{
	...
	return pStyle ? pStyle->GetSkin(iTeam, bViewmodel) : GetDefaultSkin();
}
...
public:
	...
	CUtlVector<experiment_group_t> &GetGroups()
	{
		return m_vecGroups;
	}
	...
	uint32 GetID() const
	{
		return m_unExperimentID;
	}
	const char *GetName(void) const
	{
		return m_pKeyValues->GetString("name", "unknown");
	}
	const char *GetDescription(void) const
	{
		return m_pKeyValues->GetString("description", "unknown");
	}
	...
	bool IsFull() const
	{
		return m_unNumParticipants >= m_unMaxParticipants;
	}

	uint32 GetNumParticipants() const
	{
		return m_unNumParticipants;
	}
	void SetNumParticipants(uint32 unNumParticipants)
	{
		m_unNumParticipants = unNumParticipants;
	}
	uint32 GetMaxParticipants() const
	{
		return m_unMaxParticipants;
	}
...
struct AchievementAward_t
{
	AchievementAward_t(const AchievementAward_t &rhs)
		: m_sNativeName(rhs.m_sNativeName), m_unSourceAppId(rhs.m_unSourceAppId), m_unAuditData(rhs.m_unAuditData)
	{
...
struct kill_eater_score_type_t
{
	...
	bool m_bAllowBotVictims; // if true, we don't check for a valid Steam ID on the client before sending or a valid
							 // session on the GC before incrementing
#ifdef GC_DLL
	bool m_bGCUpdateOnly;
	bool m_AllowIncrementValues; // if true, clients are allowed to send up the amount to increment by (ie., "did 100
								 // damage") rather than implicitly assuming a value of 1
	bool m_bIsBaseKillType;		 // if true, when clients send up a notification of this type we'll also look for other
								 // relevant things on the GC, like whether the victim was a friend, etc.
#endif
};
...
//
//				class CSchemaAttributeType_StrangeScore : public CSchemaAttributeTypeProtobufBase< GC_SCH_REFERENCE(
//CSchItemAttributeStrangeScore ) CAttribute_StrangeScore >
//				{
//					virtual void ConvertEconAttributeValueToSch( itemid_t unItemId, const CEconItemAttributeDefinition
//*pAttrDef, const union attribute_data_union_t& value, GCSDK::CRecordBase *out_pSchRecord ) const OVERRIDE; 					virtual
//void LoadSchToEconAttributeValue( CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef, const
//GCSDK::CRecordBase *pSchRecord ) const OVERRIDE;
//				};
//
...
public:
	...
	virtual bool BAssetClassExportedAttributeValue(const CEconItemAttributeDefinition *pAttrDef,
												   const attribute_data_union_t &value) const
	{
		return true;
	}
	...
	virtual void ConvertEconAttributeValueToSch(itemid_t unItemId, const CEconItemAttributeDefinition *pAttrDef,
												...
												GCSDK::CRecordBase *out_pSchRecord) const = 0;
	...
	virtual void LoadSchToEconAttributeValue(CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef,
											 const GCSDK::CRecordBase *pSchRecord) const = 0;
	...
	virtual void LoadOrGenerateEconAttributeValue(CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef,
												  ...
												  const CEconGameAccount *pGameAccount) const = 0;
	...
	virtual void GenerateEconAttributeValue(const CEconItemAttributeDefinition *pAttrDef,
											...
											attribute_data_union_t *out_pValue) const = 0;
#endif // GC_DLL
	...
	virtual void LoadEconAttributeValue(CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef,
										const union attribute_data_union_t &value) const = 0;

	// ...
	virtual void ConvertEconAttributeValueToByteStream(const union attribute_data_union_t &value,
													   std::string *out_psBytes) const = 0;
	...
	virtual bool BConvertStringToEconAttributeValue(
		...
		bool bEnableTerribleBackwardsCompatibilitySchemaParsingCode = false) const = 0;

	// ...
	virtual void ConvertEconAttributeValueToString(const CEconItemAttributeDefinition *pAttrDef,
												   const attribute_data_union_t &value, std::string *out_ps) const = 0;
	...
	virtual void LoadByteStreamToEconAttributeValue(CEconItem *pTargetItem,
													const CEconItemAttributeDefinition *pAttrDef,
													const std::string &sBytes) const = 0;
	...
	// ...
	virtual bool OnIterateAttributeValue(class IEconItemAttributeIterator *pIterator,
										 const CEconItemAttributeDefinition *pAttrDef,
										 const attribute_data_union_t &value) const = 0;
...
protected:
	...
	bool InitializeSchemaInternal(CEconItemSchema *pItemSchema, CUtlBuffer &bufRawData, bool bInitAsBinary,
								  uint32 nExpectedVersion);
};
#endif // defined(CLIENT_DLL) || defined(GAME_DLL)
...
public:
...
#ifdef GC_DLL
	virtual bool DoPostPriceSheetLoadInit(
		CEconStorePriceSheet *pPriceSheet); // Called once the price sheet's been loaded
#endif
	...
	{
		return eType == EQUIP_TYPE_ACCOUNT
				   ? unSlot >= m_unFirstValidAccountItemSlot && unSlot <= m_unLastValidAccountItemSlot
	...
	{
		return m_mapDefaultBodygroupState;
	}
	...
	struct periodic_score_t
	{
		...
		bool m_bGCUpdateOnly; // if set, only code that runs on the GC can initiate a change of this stat (ie., counting
							  // gifts -> true; bots killed -> false)
	...
	{
		return m_mapItemCollections;
	}

	typedef CUtlVector<int> ItemCollectionCrateMap_t;
	const ItemCollectionCrateMap_t &GetItemCollectionCrates() const
	{
		return m_vecItemCollectionCrates;
	}

	typedef CUtlMap<const char *, CEconItemPaintKitDefinition *, int> ItemPaintKitMap_t;
	const ItemPaintKitMap_t &GetItemPaintKits() const
	{
		return m_mapItemPaintKits;
	}
	...
	{
		return m_mapOperationDefinitions;
	}
...
#elif defined(GC_DLL)
	CUtlVector<CExperimentDefinition> &GetExperiments()
	{
		return m_vecExperiments;
	}

	const CUtlVector<AppId_t> &GetForeignApps() const
	{
		return m_vecForeignApps;
	}
	...
	{
		return m_mapLootLists.IsValidIndex(iIdx) ? m_mapLootLists[iIdx] : NULL;
	}

	const CQuestObjectiveDefinition *GetQuestObjectiveByDefIndex(int iIdx) const;
	const CUtlMap<int, CQuestObjectiveDefinition *, int> &GetQuestObjectives() const
	{
		return m_mapQuestObjectives;
	}
	...
	{
		CUtlConstString m_sName;
		unsigned int m_unBitIndex;	  // which bit are we claiming ownership over? there might be multiple equip regions
									  // with the same bit if we're in a "shared" block
...
#ifdef GC_DLL
	const PeriodicScoreTypeList_t &GetPeriodicScoreTypeList() const
	{
		return m_vecPeriodicScoreTypes;
	}

	int GetPeriodicScoreTypeCount() const
	{
		return GetPeriodicScoreTypeList().Count();
	} // how many types of events are we tracking? the range goes from 0 through this return value
	const periodic_score_t &GetPeriodicScoreInfo(int iPeriodicScoreIndex)
		const; // get the full info block for this periodic score -- event type, time period, etc.

	// Only intended to be used for generating data for the WebAPI.
	const KillEaterScoreMap_t &GetKillEaterScoreTypes() const
	{
		return m_mapKillEaterScoreTypes;
	}
	const SchemaStringTableDict_t &GetStringTables() const
	{
		return m_dictStringTable;
	}
#endif // GC_DLL
...
#ifdef GC_DLL
	const EconTagDict_t &GetEconTagDict() const
	{
		return m_dictTags;
	}  // meant for internal/debug use only, not for runtime iteration
#endif // GC_DLL
...
public:
	...
	virtual IEconTool *CreateEconToolImpl(const char *pszToolType, const char *pszUseString,
										  const char *pszUsageRestriction, item_capabilities_t unCapabilities,
										  KeyValues *pUsageKV);

#ifdef GC_DLL
	virtual random_attrib_t *CreateRandomAttribute(const char *pszContext, KeyValues *pRandomAttributesKV,
												   CUtlVector<CUtlString> *pVecErrors = NULL);
#endif // GC_DLL

	virtual bool BCanStrangeFilterApplyToStrangeSlotInItem(uint32 /*strange_event_restriction_t*/ unRestrictionType,
														   uint32 unRestrictionValue, const IEconItemInterface *pItem,
														   int iStrangeSlot, uint32 *out_pOptionalScoreType) const;
	bool AddQuestObjective(const CQuestObjectiveDefinition **ppQuestObjective, KeyValues *pKVObjective,
						   CUtlVector<CUtlString> *pVecErrors);
...
#ifdef TF_CLIENT_DLL
	virtual int CalculateNumberOfConcreteItems(
		const CEconItemDefinition *pItemDef); // Let derived classes handle custom item types
#endif										  // TF_CLIENT_DLL
...
private:
	...
	bool BVerifyLootListItemDropDates(const CEconLootListDefinition *pLootList,
									  CUtlVector<CUtlString> *pVecErrors) const;
	bool BRecurseiveVerifyLootListItemDropDates(const CEconLootListDefinition *pLootList,
	...
	typedef CUtlMap<item_definition_index_t, item_definition_index_t, item_definition_index_t>
		CommunityMarketDefinitionRemapMap_t;
...
#ifdef GC_DLL
void PerformIncrementKillEaterAttributeScore(CEconUserSession *pLockedOwnerSession, CEconItem *pItem,
											 uint32 unEventType, uint32 unIncrementCount, bool bGCOrigination,
											 GCSDK::CSharedObjectTransactionEx *pTransaction);
#endif // GC_DLL
...
{
public:
	explicit CSchemaFieldHandle(const char *szName) : m_szName(szName)
	{
		m_pRef = GetTypedRef();
...
bool BYieldingGetChangedItemDefinitions(int iComparisonColumn,
										CUtlVector<item_definition_index_t> &out_vecChangedDefIndices);
bool BYieldingUpdateItemDefinitionStateHashValue(GCSDK::CSQLAccess &sqlAccess, item_definition_index_t unItemDef,
												 int iUpdatedColumn);
#endif // GC_DLL
...
public:
	CAttributeLineItemLootList(const IEconItemInterface *pEconItem) : m_pEconItem(pEconItem)
	{
		//
	}
