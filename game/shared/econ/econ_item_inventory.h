...
public:
	...
	void SteamRequestInventory(CPlayerInventory *pInventory, CSteamID pSteamID,
							   IInventoryUpdateListener *pListener = NULL);
...
public:
	...
	// We're generating a base item. We need to add the game-specific keys to the criteria so that it'll find the right
	// base item.
	...
	// Moves the item to the specified backpack position. If there's another item as that spot, it swaps positions with
	// it.
	...
	virtual bool SetItemBackpackPosition(CEconItemView *pItem, uint32 iPosition = 0, bool bForceUnequip = false,
	...
	virtual void UpdateInventoryEquippedState(CPlayerInventory *pInventory, uint64 ulItemID, equipped_class_t unClass,
	...
	virtual void AcknowledgeItem(
		CEconItemView *pItem,
		bool bMoveToBackpack = true);					 // Client Acknowledges an item and moves it in to the backpack
...
{
public:
	CEconItemViewHandle() : m_pItem(NULL), m_pInv(NULL), m_bPointerDirty(false) {}

	CEconItemViewHandle(CEconItemView *pItem) : m_pItem(pItem), m_pInv(NULL), m_bPointerDirty(false)
	{
		SetItem(pItem);
	}
