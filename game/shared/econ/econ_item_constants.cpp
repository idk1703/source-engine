...
const char *g_szQualityStrings[] = {
	...
	"rarity4", // Unusual
	"Unique",	 "community", "developer", "selfmade",	 "customized",
	"strange",	 "completed", "haunted",   "collectors", "paintkitWeapon",
...
const char *g_szQualityLocalizationStrings[] = {
	...
	"#Rarity_Default",	 "#Rarity_Common", "#Rarity_Uncommon", "#Rarity_Rare", "#Rarity_Mythical",
	"#Rarity_Legendary", "#Rarity_Ancient"};
...
//-----------------------------------------------------------------------------
const char *g_pchWearAmountStrings[] = {"#TFUI_InvTooltip_None",		"#TFUI_InvTooltip_FactoryNew",
										"#TFUI_InvTooltip_MinimalWear", "#TFUI_InvTooltip_FieldTested",
										"#TFUI_InvTooltip_WellWorn",	"#TFUI_InvTooltip_BattleScared"};
...
COMPILE_TIME_ASSERT(ARRAYSIZE(g_pszItemPickupMethodStrings) ==
					(UNACK_NUM_METHODS - 1)); // -1 because UNACK_ITEM_DROPPED is index 1, not 0
...
{
	strange_attr_set_t(const char *pScoreAttrName, const char *pTypeAttrName, const char *pRestrictionAttrName,
					   const char *pRestrictionValueAttrName, bool bIsUserCustomizable)
...
strange_attr_set_t g_KillEaterAttr[] = {
	strange_attr_set_t("kill eater", "kill eater score type", "strange restriction type 1",
					   "strange restriction value 1", false),
	strange_attr_set_t("kill eater 2", "kill eater score type 2", "strange restriction type 2",
					   "strange restriction value 2", false),
	strange_attr_set_t("kill eater 3", "kill eater score type 3", "strange restriction type 3",
					   "strange restriction value 3", false),

	// assumption: all of the user-customizable attributes will follow all of the schema-specified attributes
	strange_attr_set_t("kill eater user 1", "kill eater user score type 1", "strange restriction user type 1",
					   "strange restriction user value 1", true),
	strange_attr_set_t("kill eater user 2", "kill eater user score type 2", "strange restriction user type 2",
					   "strange restriction user value 2", true),
	strange_attr_set_t("kill eater user 3", "kill eater user score type 3", "strange restriction user type 3",
					   "strange restriction user value 3", true),
};
...
int GetKillEaterAttrCount()
{
	...
	for(int i = 0; i < ARRAYSIZE(g_KillEaterAttr); i++)
	{
		...
		{
			AssertMsg(g_KillEaterAttr[i].m_bIsUserCustomizable,
					  "Ordering assumption for g_KillEaterAttr violated! User-customizable attributes should all be at "
					  "the end of the list!");
		}
...
const CEconItemAttributeDefinition *GetKillEaterAttr_RestrictionValue(int i)
{
	...
	AssertMsg1(pAttrRes, "Missing Killeater attr restriction value %s",
			   g_KillEaterAttr[i].m_attrRestrictionValue.GetName());
...
bool GetKilleaterValueByEvent(const IEconItemInterface *pItem, const kill_eater_event_t &EEventType, uint32 &value)
{
	for(int i = 0; i < GetKillEaterAttrCount(); i++)
	{
		...
		if(FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pItem, pAttribKillEaterScoreType,
														   &fKillEaterScoreTypeAttrValue))
		{
...
//-----------------------------------------------------------------------------
const char *GetCollectionCraftingInvalidReason(const IEconItemInterface *pTestItem,
											   const IEconItemInterface *pSourceItem)
{
	if(!pTestItem)
	{
	...
	const CEconItemCollectionDefinition *pTestCollection =
		pTestItem->GetItemDefinition()->GetItemCollectionDefinition();
	if(!pTestCollection)
	{
...
//			another item to compare against, which causes extra consistency checks
//-----------------------------------------------------------------------------
const char *GetHalloweenOfferingInvalidReason(const IEconItemInterface *pTestItem,
											  const IEconItemInterface *pSourceItem)
{
	...
	if(pTestItem->GetItemDefinition()->GetLoadoutSlot(0) == LOADOUT_POSITION_MISC ||
	   pTestItem->GetItemDefinition()->GetLoadoutSlot(0) == LOADOUT_POSITION_TAUNT)
	{
	   ...
	   pTestItem->GetItemDefinition()->GetLoadoutSlot(0) == LOADOUT_POSITION_PDA2)
	{
		...
		if(eQuality == AE_RARITY1 || eQuality == AE_VINTAGE || eQuality == AE_HAUNTED || eQuality == AE_COLLECTORS ||
...
const CEconItemAttributeDefinition *GetCardUpgradeForIndex(const IEconItemInterface *pItem, int i)
{
	...
	{
	public:
		CGetNthUserGeneratedAttributeIterator(int iTargetIndex) : m_iCount(iTargetIndex), m_pAttrDef(NULL) {}
