...
// Purpose:	Constructor
//-----------------------------------------------------------------------------
CEconItemFactory::CEconItemFactory() : m_ulNextObjID(0), m_bIsInitialized(false) {}
...
bool CEconItemFactory::BYieldingInit()
{
	...
	static const char *pchMaxIDQuery =
		"SELECT MAX( ID ) FROM "
		"( select max(ID) AS ID FROM Item UNION SELECT MAX(ID) AS ID FROM ForeignItem ) as tbl";

	CSQLAccess sqlAccess;
	if(!sqlAccess.BYieldingExecuteSingleResult<uint64, uint64>(NULL, pchMaxIDQuery, k_EGCSQLType_int64, &m_ulNextObjID,
															   NULL))
	{
...
static const CEconItemQualityDefinition *GetQualityDefinitionForItemCreation(
	const CItemSelectionCriteria *pOptionalCriteria, const CEconItemDefinition *pItemDef)
{
	Assert(pItemDef);
...
//-----------------------------------------------------------------------------
CEconItem *CEconItemFactory::CreateRandomItem(const CEconGameAccount *pGameAccount,
											  const CItemSelectionCriteria &criteria)
{
	...
	if(NULL == pItemDef)
	{
		EmitWarning(SPEW_GC, 2,
					"CEconItemFactory::CreateRandomItem(): Item creation request with no matching definition\n");
		return NULL;
	}
	...
	pItem->SetQuantity(criteria.BInitialQuantitySet() ? criteria.GetInitialQuantity()
													  : pItemDef->GetDefaultDropQuantity());
...
// Output:	True if a matching item could be generated, false otherwise
//-----------------------------------------------------------------------------
CEconItem *CEconItemFactory::CreateSpecificItem(const CEconGameAccount *pGameAccount,
												item_definition_index_t unDefinitionIndex)
{
	...
	{
		EmitWarning(SPEW_GC, 2,
					"CEconItemFactory::CreateSpecificItem(): Item creation request with no matching definition (def "
					"index %u)\n",
					unDefinitionIndex);
		return NULL;
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CEconItemFactory::ApplyStaticAttributeToItem(CEconItem *pItem, const static_attrib_t &staticAttrib,
												  const CEconGameAccount *pGameAccount) const
{
	...
	{
		// AssertMsg( CEconItem::GetTypedAttributeType<CSchemaAttributeType_Default>( pAttrDef ), "Elevate quality
		// attribute doesn't have the right type!" );
		...
		{
			if(pItem->GetQuality() == AE_UNIQUE || pItem->GetQuality() == AE_PAINTKITWEAPON ||
			   pItem->GetQuality() == AE_NORMAL)
			{
	...
	{
		// Ensure the target item is a hat, if it is not bail, if it is setup a particle effect attr (Whole head items
		// are considered 'hats' for purposes of unusuals )

		if(!(pItem->GetItemDefinition()->GetEquipRegionMask() & GetItemSchema()->GetEquipRegionBitMaskByName("hat")) &&
		   !(pItem->GetItemDefinition()->GetEquipRegionMask() &
			 GetItemSchema()->GetEquipRegionBitMaskByName("whole_head")))
		{
			// does not match, bail
			return;
		}
		...
		pAttr_Particle->GetAttributeType()->LoadOrGenerateEconAttributeValue(pItem, pAttr_Particle, unusualAttr,
																			 pGameAccount);
		return;
	}
	else if(pAttrDef == pAttrDef_TauntUnusual)
	{
		...
		{
			// does not match, bail
			CFmtStr fmtStr("Attempted to put an unusual taunt effect onto item %s, but it's not a taunt!  Check which "
						   "lootlists it appears in and remove it from any that are trying to unusualize it!",
						   pItem->GetItemDefinition()->GetItemDefinitionName());
			EmitError(SPEW_GC, "%s\n", fmtStr.Get());
			return;
		}
		...
		// load the normal attach effect instead
		pAttrDef_TauntUnusualAttr->GetAttributeType()->LoadOrGenerateEconAttributeValue(
			pItem, pAttrDef_TauntUnusualAttr, unusualAttr, pGameAccount);
		return;
	}
