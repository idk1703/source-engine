...
#ifdef STAGING_ONLY
ConVar tf_paint_kit_force_wear(
	"tf_paint_kit_force_wear", "0", FCVAR_REPLICATED,
	"Set to force the wear level of paink kit weapons and ignore the GC dynamic attribute value.");
#endif
...
bool IEconItemInterface::GetCustomPaintKitWear(float &flWear) const
{
	...
	if(pAttrDef_DefaultWear &&
	   FindAttribute_UnsafeBitwiseCast<attrib_value_t>(this, pAttrDef_DefaultWear, &flPaintKitWear))
	{
		flWear = flPaintKitWear;
		return true;
	}
	// If you have no wear, you also should not have a paint kit
	AssertMsg(!GetCustomPainkKitDefinition(), "No Wear Found on Item [%llu - %s] that has a Paintkit!", GetID(),
...
bool IEconItemInterface::IsMarketable() const
{
...
#if defined(TF_DLL) || defined(TF_CLIENT_DLL) || defined(TF_GC_DLL)
	{
		...
		if(GetQuality() == AE_UNUSUAL && (GetItemDefinition()->GetLoadoutSlot(0) == LOADOUT_POSITION_MISC ||
										  GetItemDefinition()->GetLoadoutSlot(0) == LOADOUT_POSITION_TAUNT))
			return true;
