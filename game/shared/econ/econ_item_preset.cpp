...
#ifdef GC
static bool BYieldingAddPresetItemRowsForSpecificPreset(GCSDK::CSQLAccess &sqlAccess,
														CSchItemPresetInstance &schItemPresetInstance,
														const CUtlVector<PresetSlotItem_t> &vecPresetData)
{
...
bool CEconItemPerClassPresetData::BYieldingAddWriteToTransaction(GCSDK::CSQLAccess &sqlAccess,
																 const CUtlVector<int> &fields)
{
	...
	FOR_EACH_VEC(fields, i)
	{
		...
		if(iField == kPerClassPresetDataDirtyField_ActivePreset)
		{
			...
			if(!sqlAccess.BYieldingUpdateRecord(
				   schSelectedItemPreset, CSET_2_COL(CSchSelectedItemPreset, k_iField_unAccountID, k_iField_unClassID),
				   CSET_1_COL(CSchSelectedItemPreset, k_iField_unPresetID)))
				return false;
		}
		else if(iField >= kPerClassPresetDataDirtyField_PresetData_Base)
		{
			...
			if(!sqlAccess.BYieldingDeleteRecords(
				   schItemPresetInstance,
				   CSET_3_COL(CSchItemPresetInstance, k_iField_unAccountID, k_iField_unPresetID, k_iField_unClassID)))
				return false;
			...
			{
				// Add our new rows.
				if(!BYieldingAddPresetItemRowsForSpecificPreset(sqlAccess, schItemPresetInstance,
																m_PresetData[iDirtyPreset]))
					return false;
			}
		}
	}
...
bool CEconItemPerClassPresetData::BYieldingAddRemoveToTransaction(GCSDK::CSQLAccess &sqlAccess)
{
	...
	if(!sqlAccess.BYieldingDeleteRecords(schItemPresetInstance,
										 CSET_2_COL(CSchItemPresetInstance, k_iField_unAccountID, k_iField_unClassID)))
		return false;
	...
	if(!sqlAccess.BYieldingDeleteRecords(schSelectedItemPreset,
										 CSET_2_COL(CSchSelectedItemPreset, k_iField_unAccountID, k_iField_unClassID)))
		return false;
...
// Purpose:
//----------------------------------------------------------------------------
const CUtlVector<PresetSlotItem_t> *CEconItemPerClassPresetData::FindItemsForPresetIndex(
	equipped_preset_t unPreset) const
{
