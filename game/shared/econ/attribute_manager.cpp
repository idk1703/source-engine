...
BEGIN_DATADESC_NO_BASE(CAttributeManager)
	DEFINE_UTLVECTOR(m_Providers, FIELD_EHANDLE), DEFINE_UTLVECTOR(m_Receivers, FIELD_EHANDLE),
		DEFINE_FIELD(m_iReapplyProvisionParity, FIELD_INTEGER), DEFINE_FIELD(m_hOuter, FIELD_EHANDLE),
...
#ifndef CLIENT_DLL
	SendPropEHandle(SENDINFO(m_hOuter)), SendPropInt(SENDINFO(m_ProviderType), 4, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iReapplyProvisionParity), PROVIDER_PARITY_BITS, SPROP_UNSIGNED),
#else
	RecvPropEHandle(RECVINFO(m_hOuter)), RecvPropInt(RECVINFO(m_ProviderType)),
...
#else
	RecvPropEHandle(RECVINFO(m_hOuter)), RecvPropInt(RECVINFO(m_ProviderType)),
		RecvPropInt(RECVINFO(m_iReapplyProvisionParity)), RecvPropEHandle(RECVINFO(m_hPlayer)),
#endif
END_NETWORK_TABLE()
#endif
...
void CAttributeManager::RemoveProvider(CBaseEntity *pProvider)
{
	...
	Assert(pProviderAttrInterface->GetAttributeManager()->m_Receivers.Find(GetOuter()) !=
		   pProviderAttrInterface->GetAttributeManager()->m_Receivers.InvalidIndex());
...
//-----------------------------------------------------------------------------
float CAttributeManager::ApplyAttributeFloatWrapper(float flValue, CBaseEntity *pInitiator, string_t iszAttribHook,
													CUtlVector<CBaseEntity *> *pItemList)
{
...
// Purpose: Wrapper that checks to see if we've already got the result in our cache
//-----------------------------------------------------------------------------
string_t CAttributeManager::ApplyAttributeStringWrapper(string_t iszValue, CBaseEntity *pInitiator,
														string_t iszAttribHook,
														CUtlVector<CBaseEntity *> *pItemList /*= NULL*/)
{
	// Have we requested a global attribute cache flush?
	const int iGlobalCacheVersion = GetGlobalCacheVersion();
	if(m_iCacheVersion != iGlobalCacheVersion)
...
// Purpose:
//-----------------------------------------------------------------------------
float CAttributeManager::ApplyAttributeFloat(float flValue, CBaseEntity *pInitiator, string_t iszAttribHook,
											 CUtlVector<CBaseEntity *> *pItemList)
{
	...
	FOR_EACH_VEC(m_Providers, iHook)
	{
		...
		if(pInitiatorAttribInterface && pAttribInterface->GetAttributeManager()->GetProviderType() == PROVIDER_WEAPON &&
		...
		flValue =
			pAttribInterface->GetAttributeManager()->ApplyAttributeFloat(flValue, pInitiator, iszAttribHook, pItemList);
	}
	...
	if(pMyAttribInterface && pMyAttribInterface->GetAttributeOwner())
	{
		...
		if(pOwnerAttribInterface)
		{
			flValue = pOwnerAttribInterface->GetAttributeManager()->ApplyAttributeFloat(flValue, pInitiator,
																						iszAttribHook, pItemList);
		}
	}
...
string_t CAttributeManager::ApplyAttributeString(string_t iszValue, CBaseEntity *pInitiator,
												 string_t iszAttribHook /*= NULL_STRING*/,
												 CUtlVector<CBaseEntity *> *pItemList /*= NULL*/)
{
	...
	FOR_EACH_VEC(m_Providers, iHook)
	{
		...
		iszValue = pAttribInterface->GetAttributeManager()->ApplyAttributeString(iszValue, pInitiator, iszAttribHook,
																				 pItemList);
	}
	...
	if(pMyAttribInterface->GetAttributeOwner())
	{
		...
		if(pOwnerAttribInterface)
		{
			iszValue = pOwnerAttribInterface->GetAttributeManager()->ApplyAttributeString(iszValue, pInitiator,
																						  iszAttribHook, pItemList);
		}
	}
...
static void ApplyAttribute(const CEconItemAttributeDefinition *pAttributeDef, float &flValue,
						   const float flValueModifier)
{
	...
	AssertMsg1(pAttributeDef->GetAttributeType()->BSupportsGameplayModificationAndNetworking(),
			   "Attempt to hook the value of attribute '%s' which doesn't support hooking! Pull the value of the "
			   "attribute directly using FindAttribute()!",
	...
	switch(iAttrDescFormat)
	{
		...
		case ATTDESCFORM_VALUE_IS_DATE:
			Assert(
				!"Attempt to apply date attribute in ApplyAttribute()."); // No-one should be hooking date descriptions
			break;
							 ...
							 const CEconItemAttributeDefinition *pAttrDef2, const float flAttribValue2)
{
	...
	switch(iAttrDescFormat)
	{
		...
		case ATTDESCFORM_VALUE_IS_DATE:
			Assert(!"Attempt to apply date attribute in CollateAttributeValues()."); // No-one should be hooking date
																					 // descriptions
			break;
...
{
public:
	CEconItemAttributeIterator_ApplyAttributeFloat(CBaseEntity *pOuter, float flInitialValue, string_t iszAttribHook,
												   CUtlVector<CBaseEntity *> *pItemList)
...
// Purpose:
//-----------------------------------------------------------------------------
float CAttributeContainer::ApplyAttributeFloat(float flValue, CBaseEntity *pInitiator, string_t iszAttribHook,
											   CUtlVector<CBaseEntity *> *pItemList)
{
	if(m_bPreventLoopback || !GetOuter())
		return flValue;
...
// Purpose:
//-----------------------------------------------------------------------------
float CAttributeContainerPlayer::ApplyAttributeFloat(float flValue, CBaseEntity *pInitiator, string_t iszAttribHook,
													 CUtlVector<CBaseEntity *> *pItemList)
{
	if(m_bPreventLoopback || !GetOuter())
		return flValue;
...
{
public:
	CEconItemAttributeIterator_ApplyAttributeString(CBaseEntity *pOuter, string_t iszInitialValue,
													string_t iszAttribHook, CUtlVector<CBaseEntity *> *pItemList)
		: m_pOuter(pOuter),
...
private:
	bool FoundString()
	{
		...
		AssertMsg(!m_bFoundString, "Already found a string attribute with %s class, return the first attribute found.",
				  STRING(m_iszAttribHook));
...
// Purpose:
//-----------------------------------------------------------------------------
string_t CAttributeContainer::ApplyAttributeString(string_t iszValue, CBaseEntity *pInitiator,
												   string_t iszAttribHook /*= NULL_STRING*/,
												   CUtlVector<CBaseEntity *> *pItemList /*= NULL*/)
{
	if(m_bPreventLoopback || !GetOuter())
...
string_t CAttributeContainerPlayer::ApplyAttributeString(string_t iszValue, CBaseEntity *pInitiator,
														 string_t iszAttribHook /*= NULL_STRING*/,
														 CUtlVector<CBaseEntity *> *pItemList /*= NULL*/)
{
	if(m_bPreventLoopback || !GetOuter())
