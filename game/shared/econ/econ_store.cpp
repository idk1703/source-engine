...
ConVar store_version("store_version", "1", FCVAR_CLIENTDLL | FCVAR_HIDDEN | FCVAR_ARCHIVE,
					 "Which version of the store to display.");

ENUMSTRINGS_START(ECurrency) {k_ECurrencyUSD, "USD"}, {k_ECurrencyGBP, "GBP"}, {k_ECurrencyEUR, "EUR"},
	{k_ECurrencyRUB, "RUB"}, {k_ECurrencyBRL, "BRL"}, {k_ECurrencyJPY, "JPY"}, {k_ECurrencyNOK, "NOK"},
	{k_ECurrencyIDR, "IDR"}, {k_ECurrencyMYR, "MYR"}, {k_ECurrencyPHP, "PHP"}, {k_ECurrencySGD, "SGD"},
	{k_ECurrencyTHB, "THB"}, {k_ECurrencyVND, "VND"}, {k_ECurrencyKRW, "KRW"}, {k_ECurrencyTRY, "TRY"},
	{k_ECurrencyUAH, "UAH"}, {k_ECurrencyMXN, "MXN"}, {k_ECurrencyCAD, "CAD"}, {k_ECurrencyAUD, "AUD"},
	{k_ECurrencyNZD, "NZD"}, {k_ECurrencyPLN, "PLN"}, {k_ECurrencyCHF, "CHF"}, {k_ECurrencyCNY, "CNY"},
	{k_ECurrencyTWD, "TWD"}, {k_ECurrencyHKD, "HKD"}, {k_ECurrencyINR, "INR"}, {k_ECurrencyAED, "AED"},
	{k_ECurrencySAR, "SAR"}, {k_ECurrencyZAR, "ZAR"}, {k_ECurrencyCOP, "COP"}, {k_ECurrencyPEN, "PEN"},
	{k_ECurrencyCLP, "CLP"},
	...
	{k_EPurchaseStateInit, "Init"}, {k_EPurchaseStateWaitingForAuthorization, "WaitingForAuthorization"},
	{k_EPurchaseStatePending, "Pending"}, {k_EPurchaseStateComplete, "Complete"}, {k_EPurchaseStateFailed, "Failed"},
	{k_EPurchaseStateCanceled, "Canceled"}, {k_EPurchaseStateRefunded, "Refunded"},
	{k_EPurchaseStateChargeback, "Chargeback"}, {k_EPurchaseStateChargebackReversed, "Chargeback Reversed"},
	ENUMSTRINGS_END(EPurchaseState)

		ENUMSTRINGS_START(EPurchaseResult) {k_EPurchaseResultOK, "OK"},
	{k_EPurchaseResultFail, "Fail"}, {k_EPurchaseResultInvalidParam, "InvalidParam"},
	{k_EPurchaseResultInternalError, "InternalError"}, {k_EPurchaseResultNotApproved, "NotApproved"},
	{k_EPurchaseResultAlreadyCommitted, "AlreadyCommitted"}, {k_EPurchaseResultUserNotLoggedIn, "UserNotLoggedIn"},
	{k_EPurchaseResultWrongCurrency, "WrongCurrency"}, {k_EPurchaseResultAccountError, "AccountError"},
	{k_EPurchaseResultInsufficientFunds, "InsufficientFunds Reversed"}, {k_EPurchaseResultTimedOut, "TimedOut"},
	{k_EPurchaseResultAcctDisabled, "AcctDisabled"}, {k_EPurchaseResultAcctCannotPurchase, "AcctCannotPurchase"},
	{k_EMicroTxnResultFailedFraudChecks,
	 "PurchaseFailedSupport"}, // this string is mismatched so "fraud" doesn't appear in the client code
	{k_EPurchaseResultOldPriceSheet, "OldPriceSheet"}, {k_EPurchaseResultTxnNotFound, "TxnNotFound"},
	...
	{k_EGCTransactionAudit_GCTransactionInit, "Init"}, {k_EGCTransactionAudit_GCTransactionPostInit, "Post Init"},
...
//	Assert( ( vecTokens.Size() - 1 ) == nSepCount );
	...
	m_fRentalPriceScale =
		fMaxRentalPriceScale <= 0.0f || fMaxRentalPriceScale >= 100.0f ? 1.0f : fMaxRentalPriceScale * 0.01f;
...
bool econ_store_entry_t::IsListedInCategoryOrSubcategories(
	const CEconStoreCategoryManager::StoreCategory_t &Category) const
{
	return IsListedInCategory(Category.m_unID) || IsListedInSubcategories(Category);
}
...
{
	return m_fRentalPriceScale < 1.0f && m_fRentalPriceScale > 0.0f;
}
...
bool econ_store_entry_t::HasDiscount(ECurrency eCurrency, item_price_t *out_punOptionalBasePrice) const
{
	...
	{
		const econ_store_entry_t *pCurEntry =
			pBundleInfo->vecItemDefs[bundleIdx]
...
void econ_store_entry_t::ValidatePrice(ECurrency eCurrency, item_price_t unPrice)
{
	...
	// If the price is 0 and this is not a pack item (an item that is not individually for sale, but is sold as part of
	// a pack bundle), post an alert
...
// Purpose:
//-----------------------------------------------------------------------------
bool BInsertSinglePricePoint(CurrencyPricePointMap_t &out_mapPricePoints, const price_point_map_key_t &key,
							 item_price_t unValue)
{
...
bool CEconStorePriceSheet::InitFromKV(KeyValues *pKVRoot)
{
	...
	m_pStorePromotionFirstTimePurchaseItem =
		GetItemSchema()->GetItemDefinitionByName(pKVRoot->GetString("promotion_first_time_purchase_gift"));
	m_pStorePromotionFirstTimeWebPurchaseItem =
		GetItemSchema()->GetItemDefinitionByName(pKVRoot->GetString("promotion_first_time_web_purchase_gift"));
	...
	m_unBonusDiscountPeriod =
		pKVRoot->GetInt(eUniverse == k_EUniversePublic ? "bonus_discount_period" : "bonus_discount_period_nonpublic");
	...
	if(pPromotionsKV)
	{
...
#ifdef GC_DLL
				EmitError(SPEW_GC,
						  "Unable to Find Currency %s in Currency Map.  Likely missing from "
						  "inventoryvalvegcpricesheet.vdf  \n",
		...
		m_StorePromotionSpendForFreeItem.m_pItemDef =
			pSchema->GetItemDefinitionByName(pPromotionsKV->GetString("item_definition"));
...
// Purpose:
//-----------------------------------------------------------------------------
bool BInitializeStoreEntryPricePoints(econ_store_entry_t &out_entry,
									  const CurrencyPricePointMap_t &mapCurrencyPricePoints, KeyValues *pKVPrice,
									  int nSalePercent)
{
	if(!pKVPrice)
		return false;
...
#ifdef GC_DLL
			EmitError(SPEW_GC,
					  "Unable to Find Currency %s in init price points.  Currency is missing from "
					  "inventoryvalvegcpricesheet.vdf  \n",
					  PchNameFromECurrency(eCurrency));
#endif
			continue;
...
bool CEconStorePriceSheet::BInitEntryFromKV(KeyValues *pKVEntry)
{
	...
	if(!BInitializeStoreEntryPricePoints(entry, m_mapCurrencyPricePoints, pKVEntry->FindKey("base_price"),
										 nSalePercent))
		return false;
	...
	FOR_EACH_TRUE_SUBKEY(pKVSaleItems, pKVSaleItem)
	{
		...
		FOR_EACH_CURRENCY(eCurrency)
		{
			if(pStoreEntry->IsOnSale(eCurrency))
			{
				EG_ERROR(
					GCSDK::SPEW_GC,
					"Timed sale '%s' has item named '%s' for entry '%s' that hard-coded to be on sale (currency: %i)\n",
	...
	for(int i = 0; i < m_vecTimedSales.Count(); i++)
	{
		...
		for(int j = 0; j < BaseSale.m_vecSaleItems.Count(); j++)
		{
			...
			for(int k = i; k < m_vecTimedSales.Count(); k++)
			{
				...
				if(k == i || MAX(BaseSale.m_SaleStartTime, OtherSale.m_SaleStartTime) <=
								 MIN(BaseSale.m_SaleEndTime, OtherSale.m_SaleEndTime))
				{
					...
					for(int l = (k == i ? j + 1 : 0); l < OtherSale.m_vecSaleItems.Count(); l++)
					{
						...
						{
							EG_ERROR(GCSDK::SPEW_GC,
									 "Conflict detected for item index '%i' betweens timed sales '%s' / '%s'\n",
									 unSearchDefIndex, BaseSale.m_sIdentifier.Get(), OtherSale.m_sIdentifier.Get());
							return false;
						}
					}
				}
	...
	FOR_EACH_VEC(m_vecTimedSales, i)
	{
		...
		if(bSaleShouldBeActive != bSaleIsActive)
		{
			FOR_EACH_VEC(TimedSale.m_vecSaleItems, i)
			{
				...
				FOR_EACH_CURRENCY(eCurrency)
				{
					unSaleStoreEntry->SetSalePrice(eCurrency, bSaleIsActive
																  ? unSaleStoreEntry->GetBasePrice(eCurrency) *
...
void CEconStorePriceSheet::DumpTimeSaleState(const RTime32 curTime) const
{
	...
	FOR_EACH_VEC(m_vecTimedSales, i)
	{
		...
		else
		{
			...
			{
				EmitInfo(
					GCSDK::SPEW_GC, SPEW_ALWAYS, LOG_ALWAYS, "\t\t%s is %.0f%% off\n",
...
//-----------------------------------------------------------------------------
// Performs calculation of a discounted price given a base price, and will then handle ensuring the appropriate number
// of zeros at the end of the price via rounding
//-----------------------------------------------------------------------------
/*static*/ item_price_t econ_store_entry_t::GetDiscountedPrice(ECurrency eCurrency, item_price_t unBasePrice,
															   float fDiscountPercentage)
{
	...
	item_price_t unNewPrice =
		(item_price_t)((double)unBasePrice * (clamp(100.0 - fDiscountPercentage, 0.0, 100.0) / 100.0));

	// determine what the unit of granularity we want to use for pricing. For example if you use 1, we keep 1/100th
	// level precision, if you use 100 we'll round it to the nearest 100th (in USD this would be round to the nearest
	// dollar, etc)
...
//-----------------------------------------------------------------------------
/*static*/ item_price_t econ_store_entry_t::CalculateSalePrice(const econ_store_entry_t *pSaleStoreEntry,
															   ECurrency eCurrency, float fDiscountPercentage,
															   int32 *out_pAdjustedDiscountPercentage /*=NULL*/)
{
	...
	/*
		...
		{
			// For these currencies, we calculate the sale price based on the discount percentage times the *USD* base
	   price -- rather than the discount percentage
			// times the base price for the given currency.
			const item_price_t unSalePrice_USD = econ_store_entry_t::GetDiscountedPrice( k_ECurrencyUSD,
	   pSaleStoreEntry->GetBasePrice( k_ECurrencyUSD ), fDiscountPercentage ); unSalePrice = ( eCurrency ==
	   k_ECurrencyNXP ) ? ConvertUSDToNXP( unSalePrice_USD ) : ConvertUSDToRMB( unSalePrice_USD );
	...
	*/
	{
		unSalePrice = econ_store_entry_t::GetDiscountedPrice(eCurrency, pSaleStoreEntry->GetBasePrice(eCurrency),
															 fDiscountPercentage);
	}
	...
	const double fActualPercent =
		100.0 * (1.0 - (double)unSalePrice / (double)pSaleStoreEntry->GetBasePrice(eCurrency));

	const int32 nDiscountPercentageForCurrency =
		bUseDefaultDiscountPercentage ? RoundFloatToInt(fDiscountPercentage) : RoundFloatToInt(fActualPercent);
	AssertMsg(nDiscountPercentageForCurrency >= 0 && nDiscountPercentageForCurrency < 100,
			  "Invalid discount percentage of %u specified for item %u currency %u", nDiscountPercentageForCurrency,
			  pSaleStoreEntry->GetItemDefinitionIndex(), eCurrency);
				...
				if(g_pVGuiLocalize)
				{
					...
					if(wszItemNameA && wszItemNameB)
					{
						...
						return sortType == kEconStoreSortType_Name_ZToA
								   ? wcscoll(wszItemNameA, wszItemNameB) >
										 0 // only sort in reverse alphabetical order if asked to -- fall-through cases
										   // uses forward order
...
const char *GetLanguageCLocaleName(ELanguage eLang)
{
	...
	switch(eLang)
	{
		...
		case k_Lang_Brazilian:
			return "ptb"; // "portuguese-brazil" - that string fails even though it's in the MS lang table; ptb does
						  // work.
		default:
...
//-----------------------------------------------------------------------------
static void InitStreamLocale(std::wostringstream &stream, ELanguage eLang, uint32 nExpectedAmount,
							 ECurrency eCurrencyCode)
{
	...
	// hack hack - certain currencies should not show fractional symbol - see if we can wire this through from config at
	// some point
	if((eCurrencyCode == k_ECurrencyRUB || eCurrencyCode == k_ECurrencyJPY || eCurrencyCode == k_ECurrencyIDR ||
...
//
int MakeMoneyStringInternal(wchar_t *pchDest, uint32 nDest, item_price_t unPrice, ECurrency eCurrencyCode,
							ELanguage eLanguage)
{
	...
	// this code will be used by the client. An old client might encounter a currency code it doesn't know about. Handle
	// that.
	...
	bool bSpaceBetweenTokens =
		false; // Whether to render a space between tokens like "$5" has no space, but "5 pyb" has a space
	...
	return V_snwprintf(pchDest, nDest, L"%ls%ls%ls", (bFirstSymbolThenAmount ? wsSymbol : wszAmount),
					   (bSpaceBetweenTokens ? L" " : L""), (bFirstSymbolThenAmount ? wszAmount : wsSymbol));
}
...
bool CEconStorePriceSheet::BItemExistsInPriceSheet(item_definition_index_t unDefIndex) const
{
	...
	for(int i = 0; i < nCategoryCount; ++i)
	{
		...
		// Intentionally not using CUtlSortVector<>::Find(), since it calls Less(), which is slow as shit for
		// m_vecEntries.
...
#else
	return EconUI() && EconUI()->GetStorePanel() ? EconUI()->GetStorePanel()->GetPriceSheet() : NULL;
#endif
}