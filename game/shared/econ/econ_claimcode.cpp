...
bool BBuildRedemptionURL(CEconClaimCode *pClaimCode, CUtlString &redemptionURL)
{
	const CEconItemDefinition *pItemDef =
		GEconManager()->GetItemSchema()->GetItemDefinition(pClaimCode->Obj().code_type());
	if(pItemDef)
	{
...
bool CJobWG_GetPromoCodes::BYieldingRunJobFromRequest(KeyValues *pkvRequest, KeyValues *pkvResponse)
{
	...
	for(uint32 i = 0; i < pTypeCache->GetCount(); ++i)
	{
		...
		{
			SetErrorMessage(pkvResponse, CFmtStr("Unable to construct redemption url for: %s", pClaimCodeName),
							k_EResultFail);
			continue;
		}
		const char *code = pClaimCode->Obj().code().c_str();
...
END_DECLARE_GCWG_JOB
(CJobWG_GetPromoCodes);
