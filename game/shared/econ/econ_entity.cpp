...
#if defined(CLIENT_DLL)
bool ParseItemKeyvalue(void *pObject, typedescription_t *pFields, int iNumFields, const char *szKeyName,
					   const char *szValue);
#endif
...
#ifdef STAGING_ONLY
ConVar unusual_force_weapon_effect("unusual_force_weapon_effect", "-1", FCVAR_CHEAT,
								   "Set to force an Unusual effect on your weapon (Primary, Secondary, Melee)");
ConVar unusual_force_cosmetic_effect("unusual_force_cosmetic_effect", "-1", FCVAR_CHEAT,
									 "Set to force an Unusual effect on your equipped cosmetics");
#endif
#endif
...
// Purpose:
//-----------------------------------------------------------------------------
void DrawEconEntityAttachedModels(CBaseAnimating *pEnt, CEconEntity *pAttachedModelSource,
								  const ClientModelRenderInfo_t *pInfo, int iMatchDisplayFlags)
{
#ifndef DOTA_DLL
	...
	for(int i = 0; i < pAttachedModelSource->m_vecAttachedModels.Size(); i++)
	{
		...
		if(attachedModel.m_pModel && (attachedModel.m_iModelDisplayFlags & iMatchDisplayFlags))
		{
			...
			pEnt->DoInternalDrawModel(
				&infoAttached, (bMarkAsDrawn && (infoAttached.flags & STUDIO_RENDER)) ? &state : NULL, pBoneToWorld);
		}
	}
...
void CEconEntity::DebugDescribe(void)
{
	...
	Msg("%s \"%s\" (level %d)\n", pszQualityString ? pszQualityString : "[unknown]", tempstr,
		pScriptItem->GetItemLevel());
...
bool CEconEntity::ValidateEntityAttachedToPlayer(bool &bShouldRetry)
{
	...
	if((pOwner->m_Shared.InCond(TF_COND_DISGUISED) || pOwner->m_Shared.InCond(TF_COND_DISGUISING)) &&
	   iClass == TF_CLASS_SPY)
	{
	...
	*/
	...
	if(!pInv->GetInventoryItemByItemID(pScriptItem->GetItemID()) && !bSkipInventoryCheck)
	{
		...
		CEconItemView *pBaseItem =
			TFInventoryManager()->GetBaseItemForClass(iClass, pScriptItem->GetStaticData()->GetLoadoutSlot(iClass));
...
void CEconEntity::GetToolRecordingState(KeyValues *msg)
{
	...
	bool bUseOverride = (GetTeamNumber() >= 0 && GetTeamNumber() < TEAM_VISUAL_SECTIONS) &&
						m_MaterialOverrides[GetTeamNumber()].IsValid();
	if(bUseOverride)
	{
...
void C_ViewmodelAttachmentModel::StandardBlendingRules(CStudioHdr *hdr, Vector pos[], Quaternion q[], float currentTime,
													   int boneMask)
{
		...
		if(bShouldShowAttachment && AttachmentModelsShouldBeVisible())
		{
			if(!m_hViewmodelAttachment)
			{
				...
				if(vm)
				{
					...
					if(pEnt->InitializeAsClientEntity(pItem->GetPlayerDisplayModel(iClass, pOwner->GetTeamNumber()),
...
void CEconEntity::UpdateParticleSystems(void)
{
	...
	else if(GetOwnerEntity() && !GetOwnerEntity()->IsDormant() && GetOwnerEntity()->IsPlayer() &&
			GetOwnerEntity()->IsAlive())
	{
		nVisible = PARTICLE_SYSTEM_STATE_VISIBLE;
	}
	...
	if(nVisible == PARTICLE_SYSTEM_STATE_NOT_VISIBLE)
	{
		...
		if(pLocalPlayer && pLocalPlayer == GetOwnerEntity() && pLocalPlayer->GetViewModel() &&
		   pLocalPlayer->GetViewModel()->GetWeapon() == pWeapon && !C_BasePlayer::ShouldDrawLocalPlayer())
		{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEconEntity::OnFireEvent(C_BaseViewModel *pViewModel, const Vector &origin, const QAngle &angles, int event,
							  const char *options)
{
...
//-----------------------------------------------------------------------------
void CEconEntity::SetupWeights(const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights,
							   float *pFlexDelayedWeights)
{
	if(GetModelPtr() && GetModelPtr()->numflexcontrollers())
	{
		if(IsEffectActive(EF_BONEMERGE) && GetMoveParent())
		{
			...
			if(pParentFlex)
			{
				// BUGBUG: We have a bug with SetCustomModel that causes a disagreement between the studio header here
				// and the one used in l_studio.cpp CModelRender::DrawModelExecute So when we hit that case, let's not
				// do any work because otherwise we'd crash since the array sizes (m_flFlexDelayedWeight vs
				// pFlexWeights) don't match. Note that this check is duplicated in C_BaseFlex::SetupLocalWeights.
				...
				if(pParentFlex->SetupGlobalWeights(pBoneToWorld, nFlexWeightCount, pFlexWeights, pFlexDelayedWeights))
				{
					...
					{
						C_BaseFlex::RunFlexDelay(nFlexWeightCount, pFlexWeights, m_flFlexDelayedWeight,
												 m_flFlexDelayTime);
...
static ConCommand dump_particlemanifest("dump_particlemanifest", cc_dump_particlemanifest,
										"Dump the list of particles loaded.", FCVAR_CHEAT);
...
void CEconEntity::GetEconParticleSystems(CUtlVector<const attachedparticlesystem_t *> *out_pvecParticleSystems) const
{
	...
	if(pEconItemView)
	{
		...
		{
			out_pvecParticleSystems->AddToTail(
	...
	{
		out_pvecParticleSystems->AddToTail(
			GetItemSchema()->GetAttributeControlledParticleSystem(iStaticParticleEffect));
	}
	...
	{
		attachedparticlesystem_t *pSystem =
			GetItemSchema()->GetAttributeControlledParticleSystem(iDynamicParticleEffect);
	...
	{
		if(!(*out_pvecParticleSystems)[i] || !(*out_pvecParticleSystems)[i]->pszSystemName ||
	...
	if(bVisible && pEffectOwner)
	{
		...
		if(pEffect)
		{
			...
			for(int i = 1; i < ARRAYSIZE(pSystem->pszControlPoints); ++i)
			{
				...
				{
					pEffectOwner->ParticleProp()->AddControlPoint(pEffect, i, this, PATTACH_POINT_FOLLOW,
																  pszControlPointName);
				}
			}
...
bool CEconEntity::IsOverridingViewmodel(void)
{
	bool bUseOverride = (GetTeamNumber() >= 0 && GetTeamNumber() < TEAM_VISUAL_SECTIONS) &&
						m_MaterialOverrides[GetTeamNumber()].IsValid();
	bUseOverride = bUseOverride || (m_hViewmodelAttachment != NULL) ||
				   (m_AttributeManager.GetItem()->GetStaticData()->GetNumAttachedModels(GetTeamNumber()) > 0);
	return bUseOverride;
}
...
int CEconEntity::DrawOverriddenViewmodel(C_BaseViewModel *pViewmodel, int flags)
{
	...
	CEconItemView *pItem = GetAttributeContainer()->GetItem();
	bool bAttachesToHands = (pItem->IsValid() && (pItem->GetStaticData()->ShouldAttachToHands() ||
	...
	if(flags & STUDIO_RENDER)
	{
		...
		bUseOverride = !bIgnoreOverride && (GetTeamNumber() >= 0 && GetTeamNumber() < TEAM_VISUAL_SECTIONS) &&
					   m_MaterialOverrides[GetTeamNumber()].IsValid();
		if(bUseOverride)
		{
