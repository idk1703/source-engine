...
typedef uint32 item_price_t; // this is the type that is used to hold currency values for transactions! don't change
							 // this without changing the relevant code/databases/etc.
...
enum EItemAction
{
	...
	k_EItemActionManualOwnershipChange =
		49, // when we have bad bugs that corrupt item data and have to fix up rows in the DB by hand
	...
	k_EItemActionRemoveItemMakersMark_Add =
		153, // early versions of this will be in the database as 151 because I am a terrible person
	...
	k_EItemActionMvM_ChallengeCompleted_RemoveTicket =
		180, // we completed a challenge and consumed this ticket as the cost
	k_EItemActionMvM_ChallengeCompleted_GrantBadge =
		181, // we completed a challenge and granted the player a badge because they didn't have one
	k_EItemActionMvM_ChallengeCompleted_UpdateBadgeStamps_Remove =
		182, // we completed a challenge and we're crossing an entry off our badge checklist (this may also reset the
			 // badge back down to empty if this was the last line item)
...
enum EEconOwnershipAction
{
	...
	k_EEconOwnershipAction_TradeRollback =
		102, // cancelcommit and rollbackcommit step of a trade. Reference is trade ID
};
...
enum eEconItemFlags
{
	...
	kEconItemFlag_NonEconomy = 1 << 3, // used for items that are meant to not interact in the economy -- these can't be
									   // traded, gift-wrapped, crafted, etc.
	kEconItemFlag_PurchasedAfterStoreCraftabilityChanges2012 =
		1 << 4, // cosmetic items coming from the store are now usable in crafting; this flag is set on all items
				// purchased from the store after this change was made
	...
	kEconItemFlags_CheckFlags_AllGCFlags = kEconItemFlag_CannotTrade | kEconItemFlag_CannotBeUsedInCrafting |
										   kEconItemFlag_CanBeTradedByFreeAccounts | kEconItemFlag_NonEconomy |
										   kEconItemFlag_PurchasedAfterStoreCraftabilityChanges2012,
};
...
enum eEconItemOrigin
{
	kEconItemOrigin_Invalid =
		-1, // should never be stored in the DB! used to indicate "invalid" for in-memory objects only
	...
	kEconItemOrigin_MvMMissionCompletionReward, // includes loot from both "mission completed" and "tour completed"
												// events
...
#define MAX_ITEM_CUSTOM_NAME_DATABASE_SIZE \
	((4 * MAX_ITEM_CUSTOM_NAME_LENGTH) + 1) // Ensures we can store MAX_ITEM_CUSTOM_NAME_LENGTH
...
enum kill_eater_event_t
{
	...
	kKillEaterEvent_PlayerKillDuringFullMoon,	// (part) we killed a player during the full moon holiday event
												// (GC-updated)
	kKillEaterEvent_PlayerKillStartDomination,	// (part) we killed a player and this kill was enough to start our
												// domination of them
	kKillEaterEvent_PlayerKillAlreadyDominated, // (part) we killed a player with this weapon that we were already
												// dominating
	kKillEaterEvent_PlayerKillRevenge, // (part) we killed a player with this weapon when that player was dominating us
	kKillEaterEvent_PlayerKillPosthumous,	 // (part) we killed a player after we were already dead (afterburn, stray
											 // rocket, etc.)
	kKillEaterEvent_BurningAllyExtinguished, // (part) we used urine/milk/flamethrower/whatever to put out the fire on
											 // an ally that was burning
	...
	kKillEaterEvent_RobotsDestroyedAfterPenetration, // (part) we killed a robot with a shot that had already penetrated
													 // another robot
	...
	kKillEaterEvent_UniqueEvent__KilledAccountWithItem, // (part) (unique event) how many individual accounts have we
														// killed?
	//	kKillEaterEvent_UniqueEvent__PlayedWithAccountIDWhileWearingItem,	// (part) (unique event) how many individual
	//accounts have we played a round with?
	...
	kKillEaterEvent_FiresSurvived,	 // (cosmetic part) we were lit on fire wearing this item and then the fire went out
									 // and we were still alive
	kKillEaterEvent_AllyHealingDone, // (part) we have healed this much (directly, so doesn't count Mad Milk, etc.
									 // because we lose the item pointer at some point); also ignores self heal (ie.,
									 // Concheror buff, MvM upgrades)
...
void EconEntity_OnOwnerKillEaterEvent(class CEconEntity *pEconEntity, class CTFPlayer *pOwner, class CTFPlayer *pVictim,
									  kill_eater_event_t eEventType, int nIncrementValue = 1);
void EconItemInterface_OnOwnerKillEaterEvent(class IEconItemInterface *pEconEntity, class CTFPlayer *pOwner,
											 class CTFPlayer *pVictim, kill_eater_event_t eEventType,
											 int nIncrementValue = 1);
void EconEntity_OnOwnerKillEaterEventNoPartner(class CEconEntity *pEconEntity, class CTFPlayer *pOwner,
											   kill_eater_event_t eEventType, int nIncrementValue = 1);
void EconItemInterface_OnOwnerKillEaterEventNoPartner(class IEconItemInterface *pEconEntity, class CTFPlayer *pOwner,
													  kill_eater_event_t eEventType, int nIncrementValue = 1);
...
void EconEntity_NonEquippedItemKillTracking_NoPartner(class CTFPlayer *pOwner, item_definition_index_t iDefIndex,
													  kill_eater_event_t eEventType, int nIncrementValue = 1);
...
void EconEntity_OnOwnerKillEaterEvent_Batched(class CEconEntity *pEconEntity, class CTFPlayer *pOwner,
											  class CTFPlayer *pVictim, kill_eater_event_t eEventType,
											  int nIncrementValue = 1);
void EconItemInterface_OnOwnerKillEaterEvent_Batched(class IEconItemInterface *pEconEntity, class CTFPlayer *pOwner,
													 class CTFPlayer *pVictim, kill_eater_event_t eEventType,
													 int nIncrementValue = 1);
...
bool GetKilleaterValueByEvent(const class IEconItemInterface *pItem, const kill_eater_event_t &EEventType,
							  uint32 &value);
bool BIsItemStrange(const class IEconItemInterface *pItem);

const int COLLECTION_CRAFTING_ITEM_COUNT = 10;
const char *GetCollectionCraftingInvalidReason(const class IEconItemInterface *pTestItem,
											   const class IEconItemInterface *pSourceItem);

const int HALLOWEEN_OFFERING_ITEM_COUNT = 3;
const char *GetHalloweenOfferingInvalidReason(const class IEconItemInterface *pTestItem,
											  const class IEconItemInterface *pSourceItem);

const int CRAFT_COMMON_STATCLOCK_ITEM_COUNT = 5;
const char *GetCraftCommonStatClockInvalidReason(const class IEconItemInterface *pTestItem,
												 const class IEconItemInterface *pSourceItem);
...
extern const char
	*g_pszItemPickupMethodStrings[UNACK_NUM_METHODS - 1]; // -1 because UNACK_ITEM_DROPPED is index 1, not 0
...
{
	return eCartType == kCartItem_Rental_1Day || eCartType == kCartItem_Rental_3Day ||
