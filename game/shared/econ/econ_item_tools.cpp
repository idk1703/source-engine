...
// Purpose:
//---------------------------------------------------------------------------------------
CEconTool_WrappedGift::CEconTool_WrappedGift(const char *pszTypeName, const char *pszUseString,
...
// Purpose:
//---------------------------------------------------------------------------------------
CEconTool_GiftWrap::CEconTool_GiftWrap(const char *pszTypeName, const char *pszUseString,
									   item_capabilities_t unCapabilities, KeyValues *pUsageKV)
...
bool CEconTool_GiftWrap::CanApplyTo(const IEconItemInterface *pTool, const IEconItemInterface *pToolSubject) const
{
	...
	if(pToolSubject->GetQuality() == AE_SELFMADE || pToolSubject->GetQuality() == AE_COMMUNITY ||
	   pToolSubject->GetQuality() == AE_CUSTOMIZED || pToolSubject->GetQuality() == AE_NORMAL)
	{
		return false;
	}
...
CEconTool_StrangeCountTransfer::CEconTool_StrangeCountTransfer(const char *pszTypeName,
...
bool CEconTool_StrangeCountTransfer::AreItemsEligibleForStrangeCountTransfer(const IEconItemInterface *pItem1,
...
bool CEconTool_StrangePart::CanApplyTo(const IEconItemInterface *pTool, const IEconItemInterface *pToolSubject) const
{
	...
	{
		float flScoreType;
		if(FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pToolSubject, GetKillEaterAttr_Type(i),
														   &flScoreType) &&	  // if we have a counter in this slot...
...
CEconTool_StrangePartRestriction::CEconTool_StrangePartRestriction(const char *pszTypeName, const char *pszUseString,
																   item_capabilities_t unCapabilities,
																   KeyValues *pUsageKV)
	: IEconTool(pszTypeName, pszUseString, NULL, unCapabilities),
	  ...
	  m_unRestrictionValue((unsigned int)-1)
{
	...
	// check.
	const int iRestrictionType =
		StringFieldToInt(pUsageKV->GetString("restriction_type", ""), &s_pszStrangeRestrictionTypes[0],
						 ARRAYSIZE(s_pszStrangeRestrictionTypes));
...
bool CEconTool_StrangePartRestriction::BFinishInitialization()
{
	...
	switch(m_eRestrictionType)
	{
		...
		default:
			AssertMsg1(false, "CEconTool_StrangePartRestriction() doesn't understand how to parse restriction type %i.",
					   m_eRestrictionType);
			return false;
	}
	...
	return m_eRestrictionType != kStrangeEventRestriction_None && m_unRestrictionValue != (unsigned int)-1 &&
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEconTool_StrangePartRestriction::CanApplyTo(const IEconItemInterface *pTool,
												  const IEconItemInterface *pToolSubject) const
{
	Assert(pTool);
	...
	// scores/restrictions.
	const CEconTool_StrangePartRestriction *pToolRestriction =
		pTool->GetItemDefinition()->GetTypedEconTool<CEconTool_StrangePartRestriction>();
	if(!pToolRestriction)
		return false;
	...
	for(int i = 0; i < GetKillEaterAttrCount(); i++)
	{
		if(GetItemSchema()->BCanStrangeFilterApplyToStrangeSlotInItem(
			   pToolRestriction->GetRestrictionType(), pToolRestriction->GetRestrictionValue(), pToolSubject, i, NULL))
			return true;
	}
...
// Purpose:
//-----------------------------------------------------------------------------
CEconTool_ItemDynamicRecipe::CEconTool_ItemDynamicRecipe(const char *pszTypeName, const char *pszUseString,
														 item_capabilities_t unCapabilities, KeyValues *pUsageKV)
	: IEconTool(pszTypeName, pszUseString, NULL, unCapabilities)
{
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CanApplyTo(const IEconItemInterface *pTool,
											 const IEconItemInterface *pToolSubject) const
{
	Assert(pTool);
	Assert(pToolSubject);
	...
	const CUtlVector<const CEconItemAttributeDefinition *> &vecMatchingAttribs =
		matchingIterator.GetMatchingComponentInputs();
...
// Purpose:
//-----------------------------------------------------------------------------
CEconTool_ItemDynamicRecipe::CBaseRecipeComponent::CBaseRecipeComponent(bool bIsOutput,
																		const CBaseRecipeComponent *pParent)
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CBaseRecipeComponent::BFinishInitialization_Internal(
	CUtlVector<CUtlString> *pVecErrors, ComponentAttribVector_t *pAttribVec)
{
...
CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentDefinedItem::CDynamicRecipeComponentDefinedItem(
	bool bIsOutput, const CBaseRecipeComponent *pParent)
...
//			as their item name.
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentDefinedItem::BFinishInitialization_Internal(
	CUtlVector<CUtlString> *pVecErrors, ComponentAttribVector_t *pAttribVec)
{
	...
	else if(BStringsEqual(m_strName, m_pszUseParentNameIdentifier) ||
			GetItemSchema()->GetItemDefinitionByName(m_strName))
	{
		bAnyDataSet = true;
	}
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentDefinedItem::ParseKV(KeyValues *pKV,
																			  CUtlVector<CUtlString> *pVecErrors)
{
	...
	SCHEMA_INIT_CHECK(bNoItemDef != bItemName, "Both \"no_item_def\" and \"item_name\" specified in component.");

	// Make sure at least one of the above is set
	SCHEMA_INIT_CHECK(bNoItemDef || bItemName, "Neither \"no_item_def\" or \"item_name\" specified in component.");
...
//-----------------------------------------------------------------------------
#ifdef GC_DLL
bool CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentDefinedItem::AddRecipeComponentAsAttribute(
	CEconItem *pItem, const CEconGameAccount *pGameAccount) const
{
	...
	const CEconItemAttributeDefinition *pAttribDef =
		GetNextAvailableAttributeWithBaseName(GetAttributeName(), &attribVec);
	if(!pAttribDef)
		return false;
	...
	if(m_strName && m_strName[0])
	{
		if(BStringsEqual(m_pszUseParentNameIdentifier, pszItemDefName) && m_pParent)
		{
			...
			const CDynamicRecipeComponentDefinedItem *pParentDefinedItemComponent =
				dynamic_cast<const CDynamicRecipeComponentDefinedItem *>(m_pParent);
			AssertMsg(pParentDefinedItemComponent,
					  "Parent attribute passed into defined item component is not a defined item component");
	...
	// Write out attribute all attribute indexes and values, separated by what we expect to be an invalid character
	// sequence
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentLootList::BFinishInitialization_Internal(
	CUtlVector<CUtlString> *pVecErrors, ComponentAttribVector_t *pAttribVec)
{
	...
	SCHEMA_INIT_CHECK(!(m_eUniqueness == UNIQUE_AMONG_OUTPUTS && !GetIsOutput()),
					  "Input component marked to be unique among inputs.  Not supported!");
#endif
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentLootList::ParseKV(KeyValues *pKV,
																		   CUtlVector<CUtlString> *pVecErrors)
{
	const char *pszLootListName = pKV->GetString("lootlist_name");
	SCHEMA_INIT_CHECK(pszLootListName != NULL, "Missing lootlist name in lootlist recipe component");
...
//-----------------------------------------------------------------------------
#ifdef GC_DLL
bool CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentLootList::AddRecipeComponentAsAttribute(
	CEconItem *pItem, const CEconGameAccount *pGameAccount) const
{
	// Check if we should even apply
	if(!RollChanceOfApplying())
	...
	if(!RollLootlistItemAndAttributes(vecLootlistGeneratedAttributes, &pszItemDefName,
									  &inputIterator.GetMatchingComponentInputs(), pGameAccount))
	{
		return false;
	}
...
{
public:
	CAttributeToStringIterator(
		CUtlVector<CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentLootList::StringEncodedAttribute_t>
			&vecAdditionalAttribs,
		CEconItem *pItem)
		: m_vecAdditionalAttribs(vecAdditionalAttribs), m_pItem(pItem)
	{
	}
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CDynamicRecipeComponentLootList::RollLootlistItemAndAttributes(
	CUtlVector<StringEncodedAttribute_t> &vecAdditionalAttribs, const char **pszDefName,
	const CUtlVector<item_definition_index_t> *pVecAvoidItemDefs, const CEconGameAccount *pGameAccount) const
{
...
CEconTool_ItemDynamicRecipe::CRecipeComponentInputDefIndexIterator::CRecipeComponentInputDefIndexIterator(
	EItemDefUniqueness_t eUniqueness)
	: m_eUniqueness(eUniqueness)
{
}

bool CEconTool_ItemDynamicRecipe::CRecipeComponentInputDefIndexIterator::OnIterateAttributeValue(
	const CEconItemAttributeDefinition *pAttrDef, const CAttribute_DynamicRecipeComponent &value)
{
	// We dont care
	...
	if(m_eUniqueness == CEconTool_ItemDynamicRecipe::UNIQUE_AMONG_OUTPUTS &&
	   !(value.component_flags() & DYNAMIC_RECIPE_FLAG_IS_OUTPUT))
		return true;

	// Only check against inputs
	if(m_eUniqueness == CEconTool_ItemDynamicRecipe::UNIQUE_AMONG_INPUTS &&
	   value.component_flags() & DYNAMIC_RECIPE_FLAG_IS_OUTPUT)
		return true;
...
bool CEconTool_ItemDynamicRecipe::BInitFromKV(KeyValues *pKVDef, CUtlVector<CUtlString> *pVecErrors)
{
	...
	{
		static int SortRecipeComponentVector(CEconTool_ItemDynamicRecipe::CBaseRecipeComponent *const *pComponent1,
											 CEconTool_ItemDynamicRecipe::CBaseRecipeComponent *const *pComponent2)
		{
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CBaseRecipeComponent::ParseComponentsBlock(
	KeyValues *pKV, CUtlVector<CBaseRecipeComponent *> &vecComponents, CUtlVector<CUtlString> *pVecErrors,
	const CBaseRecipeComponent *pParent)
{
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::CBaseRecipeComponent::ParseComponents(
	KeyValues *pKV, CUtlVector<CBaseRecipeComponent *> &vecComponents, bool bIsOutput,
	CUtlVector<CUtlString> *pVecErrors, const CBaseRecipeComponent *pParent)
{
	...
	while(pEntry)
	{
		...
		{
			pComponent =
				vecComponents[vecComponents.AddToTail(new CDynamicRecipeComponentLootList(bIsOutput, pParent))];
		}
		else if(pEntry->FindKey("item_name") || pEntry->FindKey("no_item_def"))
		{
			pComponent =
				vecComponents[vecComponents.AddToTail(new CDynamicRecipeComponentDefinedItem(bIsOutput, pParent))];
		}
		else
		{
...
bool CEconTool_ItemDynamicRecipe::CBaseRecipeComponent::ParseKV(KeyValues *pKV, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(pszQuality)
	{
		...
		SCHEMA_INIT_CHECK(m_eQuality != AE_UNDEFINED, "Invalid item quality \"%s\" specified for component \"%s\"",
						  pszQuality, m_strName.Get());
	}
	...
	if(pKVAttribs)
	{
		FOR_EACH_SUBKEY(pKVAttribs, pKVAttribute)
		{
			...
			SCHEMA_INIT_CHECK(pAttrDef != NULL, "Attribute index %i not found when specifying dynamic recipe",
							  staticAttrib.iDefIndex);
	...
	SCHEMA_INIT_CHECK(flChance > 0.f && flChance <= 1.f, "Recipe component chance to apply out of bounds: %f",
					  flChance);
	...
	if(pKVCounts)
	{
		...
		FOR_EACH_SUBKEY(pKVCounts, pKVEntry)
		{
			...
			SCHEMA_INIT_CHECK(countChance.m_nMaxCount >= countChance.m_nMinCount,
							  "Recipe component count max is less than the min: %s", pszCount);
	...
	SCHEMA_INIT_CHECK(ParseComponentsBlock(pKV, m_vecAdditionalComponents, pVecErrors, this),
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemDynamicRecipe::BGenerateDynamicAttributes(CEconItem *pItem,
...
//			none available
//-----------------------------------------------------------------------------
CEconItemAttributeDefinition *CEconTool_ItemDynamicRecipe::CBaseRecipeComponent::GetNextAvailableAttributeWithBaseName(
	const char *pszBaseAttribName, ComponentAttribVector_t *pAttribVec)
{
	Assert(pAttribVec);
	if(!pAttribVec)
		return NULL;
...
bool CEconTool_Xifier::CanApplyTo(const IEconItemInterface *pTool, const IEconItemInterface *pToolSubject) const
{
	...
	if(m_ItemDefTargetRestrictions.Count() > 0)
	{
		...
		{
			const CEconItemDefinition *pTargetItemDef =
				GetItemSchema()->GetItemDefinition(m_ItemDefTargetRestrictions[i]);
...
//-----------------------------------------------------------------------------
bool CEconTool_Xifier::ItemDefMatch(const CEconItemDefinition *pTargetItemDef,
									const CEconItemDefinition *pSubjectItemDef) const
{
...
//-----------------------------------------------------------------------------
bool CEconTool_KillStreakifier::CanApplyTo(const IEconItemInterface *pTool,
										   const IEconItemInterface *pToolSubject) const
{
	Assert(pTool);
	Assert(pToolSubject);
...
//-----------------------------------------------------------------------------
bool CEconTool_ItemEaterRecharger::CanApplyTo(const IEconItemInterface *pTool,
											  const IEconItemInterface *pToolSubject) const
{
	Assert(pTool);
	Assert(pToolSubject);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEconTool_ClassTransmogrifier::CanApplyTo(const IEconItemInterface *pTool,
											   const IEconItemInterface *pToolSubject) const
{
	Assert(pTool);
	Assert(pToolSubject);
...
//---------------------------------------------------------------------------------------
/* static */ bool CEconSharedToolSupport::ToolCanApplyTo(const IEconItemInterface *pTool,
														 const IEconItemInterface *pToolSubject)
{
...
/* static */ bool CEconSharedToolSupport::ToolCanApplyToDefinition(const GameItemDefinition_t *pToolDef,
...
// WARNING
// DO NOT USE THIS CODE IF YOUR TOOL HAS Attribute restrictions like "tool_target_item" or similar restriction
// attributes
/* static */ bool CEconSharedToolSupport::ToolCanApplyToBaseItem(const GameItemDefinition_t *pToolDef,
																 const GameItemDefinition_t *pToolSubjectDef)
{
	if(!pToolSubjectDef)
		return false;
	...
	if(pToolSubjectDef->IsBaseItem() || pToolSubjectDef->IsHidden() || pToolSubjectDef->GetQuality() == AE_NORMAL ||
	   Q_strnicmp(pToolSubjectDef->GetDefinitionName(), "Upgradeable ", 12))
		return false;
