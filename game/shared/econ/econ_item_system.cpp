...
ConVar items_game_use_gc_copy("items_game_use_gc_copy", "1", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_ARCHIVE,
							  "If set, items_game.txt will be stomped by the GC.");
ConVar item_debug_validation("item_debug_validation", "1", FCVAR_REPLICATED | FCVAR_ARCHIVE,
							 "If set, CEconEntity::ValidateEntityAttachedToPlayer behaves as it would in release "
							 "builds and also allows bot players to take the same code path as real players.");
#endif

static ConVar item_quality_chance_unique("item_quality_chance_unique", "0.1", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
										 "Percentage chance that a random item is unique.");
static ConVar item_quality_chance_rare("item_quality_chance_rare", "0.5", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
									   "Percentage chance that a random item is a rare.");
static ConVar item_quality_chance_common("item_quality_chance_common", "1.0", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
...
#ifdef GAME_DLL
ConVar mp_tournament_whitelist("mp_tournament_whitelist", "item_whitelist.txt", FCVAR_NONE,
...
#ifdef GAME_DLL
CON_COMMAND_F(item_show_whitelistable_definitions,
			  ...
			  FCVAR_CHEAT)
{
	...
	const CEconItemSchema::SortedItemDefinitionMap_t &mapItemDefs =
		ItemSystem()->GetItemSchema()->GetSortedItemDefinitionMap();
...
item_definition_index_t CEconItemSystem::GenerateRandomItem(CItemSelectionCriteria *pCriteria,
															entityquality_t *outEntityQuality)
{
...
//-----------------------------------------------------------------------------
bool IDelayedSchemaData::InitializeSchemaInternal(CEconItemSchema *pItemSchema, CUtlBuffer &bufRawData,
												  bool bInitAsBinary, uint32 nExpectedVersion)
{
	Msg("Applying new item schema, version %08X\n", nExpectedVersion);

	CUtlVector<CUtlString> vecErrors;
	bool bSuccess = bInitAsBinary ? pItemSchema->BInitBinaryBuffer(bufRawData, &vecErrors)
...
{
public:
	DelayedSchemaData_HTTPResponseData(ISteamHTTP *pHTTP, HTTPRequestHandle handleHTTPRequest, uint32 unBodySize,
									   uint32 nExpectedVersion, const std::string &sSignature)
		: m_nExpectedVersion(nExpectedVersion)
	{
		...
		m_bValid =
			pHTTP->GetHTTPResponseBodyData(handleHTTPRequest, (uint8 *)m_bufRawData.Base(), m_bufRawData.TellPut());
		if(m_bValid)
			m_bValid = CheckValveSignature(m_bufRawData.Base(), m_bufRawData.TellPut(), sSignature.c_str(),
										   sSignature.length());
	}
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		// !TEST!
		// const char *szURL =
		// "http://cdn.beta.steampowered.com/apps/440/scripts/items/items_game.b8b7a85b4dd98b139957004b86ec0bc070a59d18.txt";
		if(msg.Body().has_items_game())
		{
			bool bDidInit = ItemSystem()->GetItemSchema()->MaybeInitFromBuffer(
				new DelayedSchemaData_GCDirectData(msg.Body().items_game()));
			Msg(bDidInit ? GC_ITEM_SCHEMA_UPDATE_APPLIED : GC_ITEM_SCHEMA_UPDATE_QUEUED,
	...
	void OnHTTPCompleted(HTTPRequestCompleted_t *arg, bool bFailed)
	{
		...
		else
		{
			...
			if(!bFailed)
			{
				...
				{
					bool bDidInit =
						ItemSystem()->GetItemSchema()->MaybeInitFromBuffer(new DelayedSchemaData_HTTPResponseData(
							pHTTP, arg->m_hRequest, unBodySize, m_nExpectedVersion, m_sSignature));
					Msg(bDidInit ? GC_ITEM_SCHEMA_UPDATE_APPLIED : GC_ITEM_SCHEMA_UPDATE_QUEUED, unBodySize,
						m_nExpectedVersion);
				}
			}
...
GC_REG_JOB(GCSDK::CGCClient, CGCUpdateItemSchema, "CGCUpdateItemSchema", k_EMsgGCUpdateItemSchema,
