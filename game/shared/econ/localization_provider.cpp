	...
	if(pchKey)
	{
		...
		if(!wszLocalized)
		{
			...
			return const_cast<locchar_t *>(LOCCHAR(
				"<NULL LOC STRING>")); // Super janky cast alert! This method should really return a const locchar_t*
									   // but making that change breaks all the callsites...fix later.
#else
			return const_cast<locchar_t *>(LOCCHAR(""));
#endif
		}
...
{
	AssertMsg(false, "Using global localization provider in GC - All strings will be in English.  For proper "
					 "localization, CLocalizationProvider instance should be created and passed in.");
...
int CGCLocalizationProvider::ConvertLoccharToUnicode(const locchar_t *loc, wchar_t *unicode,
													 int unicodeBufferSize) const
{
...
int CVGUILocalizationProvider::ConvertLoccharToUnicode(const locchar_t *loc, wchar_t *unicode,
													   int unicodeBufferSize) const
{
