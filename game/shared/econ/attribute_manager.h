...
#define CALL_ATTRIB_HOOK(vartype, retval, hookName, who, itemlist)                                                 \
	retval = CAttributeManager::AttribHookValue<vartype>(retval, #hookName, static_cast<const CBaseEntity *>(who), \
														 itemlist, true);
...
#define CALL_ATTRIB_HOOK_STRING_ON_OTHER(other, retval, hookName) \
	CALL_ATTRIB_HOOK(CAttribute_String, retval, hookName, other, NULL)
#define CALL_ATTRIB_HOOK_INT_ON_OTHER_WITH_ITEMS(other, retval, items_array, hookName) \
	CALL_ATTRIB_HOOK(int, retval, hookName, other, items_array)
#define CALL_ATTRIB_HOOK_FLOAT_ON_OTHER_WITH_ITEMS(other, retval, items_array, hookName) \
	CALL_ATTRIB_HOOK(float, retval, hookName, other, items_array)
#define CALL_ATTRIB_HOOK_STRING_ON_OTHER_WITH_ITEMS(other, retval, items_array, hookName) \
	CALL_ATTRIB_HOOK(CAttribute_String, retval, hookName, other, items_array)
...
public:
	...
	template<class T>
	static T AttribHookValue(T TValue, const char *pszAttribHook, const CBaseEntity *pEntity,
							 CUtlVector<CBaseEntity *> *pItemList = NULL, bool bIsGlobalConstString = false)
	{
		...
		AssertMsg(pAttribInterface, "If you hit this, you've probably got a hook incorrectly setup, because the entity "
									"it's hooking on doesn't know about attributes.");
		if(pAttribInterface == NULL)
			return TValue;

		// Hook base attribute.
		T Scratch;
		AttribHookValueInternal(Scratch, TValue, pszAttribHook, pEntity, pAttribInterface, pItemList,
								bIsGlobalConstString);
...
private:
	template<class T>
	static void TypedAttribHookValueInternal(T &out, T TValue, string_t iszAttribHook, const CBaseEntity *pEntity,
											 IHasAttributes *pAttribInterface, CUtlVector<CBaseEntity *> *pItemList)
	{
		float flValue = pAttribInterface->GetAttributeManager()->ApplyAttributeFloatWrapper(
			static_cast<float>(TValue), const_cast<CBaseEntity *>(pEntity), iszAttribHook, pItemList);

		out = AttributeConvertFromFloat<T>(flValue);
	}

	static void TypedAttribHookValueInternal(CAttribute_String &out, const CAttribute_String &TValue,
											 string_t iszAttribHook, const CBaseEntity *pEntity,
											 IHasAttributes *pAttribInterface, CUtlVector<CBaseEntity *> *pItemList)
	{
		string_t iszIn = AllocPooledString(TValue.value().c_str());
		string_t iszOut = pAttribInterface->GetAttributeManager()->ApplyAttributeStringWrapper(
			iszIn, const_cast<CBaseEntity *>(pEntity), iszAttribHook, pItemList);
	...
	template<class T>
	static void AttribHookValueInternal(T &out, T TValue, const char *pszAttribHook, const CBaseEntity *pEntity,
										IHasAttributes *pAttribInterface, CUtlVector<CBaseEntity *> *pItemList,
										bool bIsGlobalConstString)
	{
		...
		string_t iszAttribHook = bIsGlobalConstString ? AllocPooledString_StaticConstantStringPointer(pszAttribHook)
													  : AllocPooledString(pszAttribHook);
...
public:
	virtual float ApplyAttributeFloat(float flValue, CBaseEntity *pInitiator, string_t iszAttribHook = NULL_STRING,
									  CUtlVector<CBaseEntity *> *pItemList = NULL);
	virtual string_t ApplyAttributeString(string_t iszValue, CBaseEntity *pInitiator,
...
private:
	...
	virtual float ApplyAttributeFloatWrapper(float flValue, CBaseEntity *pInitiator, string_t iszAttribHook,
											 CUtlVector<CBaseEntity *> *pItemList = NULL);
	virtual string_t ApplyAttributeStringWrapper(string_t iszValue, CBaseEntity *pInitiator, string_t iszAttribHook,
												 CUtlVector<CBaseEntity *> *pItemList = NULL);
...
public:
	...
	// Attribute hook. Use the CALL_ATTRIB_HOOK macros above.
	virtual float ApplyAttributeFloat(float flValue, CBaseEntity *pInitiator, string_t iszAttribHook = NULL_STRING,
									  CUtlVector<CBaseEntity *> *pItemList = NULL) OVERRIDE;
	virtual string_t ApplyAttributeString(string_t iszValue, CBaseEntity *pInitiator,
										  string_t iszAttribHook = NULL_STRING,
										  CUtlVector<CBaseEntity *> *pItemList = NULL) OVERRIDE;
