	...
	if(attribValue.component_flags() & DYNAMIC_RECIPE_FLAG_PARAM_ATTRIBUTE_SET_ALL)
	{
		...
		FOR_EACH_VEC(vecAttribs, i)
		{
			const CEconItemAttributeDefinition *pAttr =
				GetItemSchema()->GetAttributeDefinition(vecAttribs[i].m_unDefinitionIndex);
			Assert(pAttr);
			uint32 itemAttributeValue;
			if(!pAttr || !pItem->FindAttribute(pAttr, &itemAttributeValue) ||
			   itemAttributeValue != vecAttribs[i].m_value.asUint32)
			{
				return false;
			}
		}
...
bool DecodeAttributeStringIntoAttributes(const CAttribute_DynamicRecipeComponent &attribValue,
										 CUtlVector<CEconItem::attribute_t> &vecAttribs)
{
...
bool DecodeItemFromEncodedAttributeString(const CAttribute_DynamicRecipeComponent &attribValue, CEconItem *pItem)
{
	...
	{
		// If the flag is not set, then we want the item name to be generic.  In english, we want to just call it
		// "item".
	...
	FOR_EACH_VEC(vecAttribs, j)
	{
		...
		const CEconItemAttributeDefinition *pAttrDef =
			GetItemSchema()->GetAttributeDefinition(vecAttribs[j].m_unDefinitionIndex);
		Assert(pAttrDef);
