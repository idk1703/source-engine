...
static bool BGetPaymentRule(KeyValues *pKVRule, EPaymentRuleType *out_pePaymentRuleType, double *out_pRevenueShare)
{
	...
	for(int i = 0; i < ARRAYSIZE(s_Lookup); i++)
	{
		...
		*out_pRevenueShare =
			atof(pKVKey->GetString()) *
			100.0; // KeyValues doesn't support parsing a string as a double-precision value, so we do it by hand
		return true;
	}
...
//-----------------------------------------------------------------------------
CEconItemSeriesDefinition::CEconItemSeriesDefinition(void) : m_nValue(INT_MAX) {}
...
bool CEconItemSeriesDefinition::BInitFromKV(KeyValues *pKVSeries, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	SCHEMA_INIT_CHECK(NULL != pKVSeries->FindKey("value"), "Quality definition %s: Missing required field \"value\"",
					  pKVSeries->GetName());
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CEconItemQualityDefinition::CEconItemQualityDefinition(void) : m_nValue(INT_MAX), m_bCanSupportSet(false) {}
...
bool CEconItemQualityDefinition::BInitFromKV(KeyValues *pKVQuality, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	// Check for required fields
	SCHEMA_INIT_CHECK(NULL != pKVQuality->FindKey("value"), "Quality definition %s: Missing required field \"value\"",
					  pKVQuality->GetName());
	...
	SCHEMA_INIT_CHECK(0 != Q_stricmp(GetName(), "any"),
					  "Quality definition any: The quality name \"any\" is a reserved keyword and cannot be used.");

	SCHEMA_INIT_CHECK(m_nValue != k_unItemQuality_Any,
					  "Quality definition %s: Invalid value (%d). It is reserved for Any", GetName(),
					  k_unItemQuality_Any);
...
//-----------------------------------------------------------------------------
CEconItemRarityDefinition::CEconItemRarityDefinition(void) : m_nValue(INT_MAX), m_nLootlistWeight(0) {}

//-----------------------------------------------------------------------------
// Purpose:	Initialize the rarity definition
//-----------------------------------------------------------------------------
bool CEconItemRarityDefinition::BInitFromKV(KeyValues *pKVRarity, KeyValues *pKVRarityWeights, CEconItemSchema &pschema,
											CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	// Check for required fields
	SCHEMA_INIT_CHECK(NULL != pKVRarity->FindKey("value"), "Rarity definition %s: Missing required field \"value\"",
					  pKVRarity->GetName());

	SCHEMA_INIT_CHECK(NULL != pKVRarity->FindKey("loc_key"), "Rarity definition %s: Missing required field \"loc_key\"",
					  pKVRarity->GetName());
...
bool CEconColorDefinition::BInitFromKV(KeyValues *pKVColor, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	SCHEMA_INIT_CHECK(!m_strColorName.IsEmpty(), "Quality definition %s: missing \"color_name\"", GetName());

#ifdef GC_DLL
	SCHEMA_INIT_CHECK(!m_strHexColor.IsEmpty(), "Quality definition %s: missing \"hex_color\"", GetName());
#endif // GC_DLL
...
CEconItemSetDefinition::CEconItemSetDefinition(void)
	: m_pszName(NULL), m_pszLocalizedName(NULL), m_iBundleItemDef(INVALID_ITEM_DEF_INDEX), m_bIsHiddenSet(false)
{
}
...
bool CEconItemSetDefinition::BInitFromKV(KeyValues *pKVItemSet, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(pszBundleName && pszBundleName[0])
	{
		...
		SCHEMA_INIT_CHECK(pDef != NULL, "Item set %s: Bundle definition \"%s\" was not found", m_pszName,
						  pszBundleName);
	}
	...
	if(pKVItems)
	{
		FOR_EACH_SUBKEY(pKVItems, pKVItem)
		{
			...
			SCHEMA_INIT_CHECK(pDef != NULL, "Item set %s: Item definition \"%s\" was not found", m_pszName, pszName);

			const item_definition_index_t unDefIndex = pDef->GetDefinitionIndex();

			SCHEMA_INIT_CHECK(!m_iItemDefs.IsValidIndex(m_iItemDefs.Find(unDefIndex)),
							  "Item set %s: item definition \"%s\" appears multiple times", m_pszName, pszName);
			SCHEMA_INIT_CHECK(!pDef->GetItemSetDefinition(),
							  "Item set %s: item definition \"%s\" specified in multiple item sets", m_pszName,
							  pszName);
	...
	if(pKVAttributes)
	{
		FOR_EACH_SUBKEY(pKVAttributes, pKVAttribute)
		{
			...
			SCHEMA_INIT_CHECK(pAttrDef != NULL, "Item set %s: Attribute definition \"%s\" was not found", m_pszName,
							  pszName);
			SCHEMA_INIT_CHECK(pAttrDef->BIsSetBonusAttribute(),
							  "Item set %s: Attribute definition \"%s\" is not a set bonus attribute", m_pszName,
							  pszName);
	...
	// Sanity check.
	SCHEMA_INIT_CHECK(m_pszLocalizedName != NULL, "Item set %s: Set contains no localized name", m_pszName);
	SCHEMA_INIT_CHECK(m_iItemDefs.Count() > 0, "Item set %s: Set contains no items", m_pszName);
...
void CEconItemSetDefinition::IterateAttributes(class IEconItemAttributeIterator *pIterator) const
{
	FOR_EACH_VEC(m_iAttributes, i)
	{
		...
		const CEconItemAttributeDefinition *pAttrDef =
			GetItemSchema()->GetAttributeDefinition(itemsetAttrib.m_iAttribDefIndex);
		if(!pAttrDef)
			continue;
...
static int SortCollectionByRarity(item_definition_index_t const *a, item_definition_index_t const *b)
{
	...
	if(pItemA->GetRarity() == pItemB->GetRarity() && pItemA->GetCustomPainkKitDefinition() &&
	   pItemB->GetCustomPainkKitDefinition())
	{
#ifdef CLIENT_DLL
...
bool CEconItemCollectionDefinition::BInitFromKV(KeyValues *pKVPItemCollection, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(pKVItems)
	{
		...
		FOR_EACH_TRUE_SUBKEY(pKVItems, pKVRarity)
		{
			...
			SCHEMA_INIT_CHECK(pRarity != NULL, "Item collection %s: Rarity type \"%s\" was not found", m_pszName,
							  pKVRarity->GetName());
			...
			if(!m_bIsReferenceCollection)
			{
				...
				SCHEMA_INIT_CHECK(GetItemSchema()->BInsertLootlist(pszName, pKVRarity, pVecErrors),
								  "Invalid collection lootlist %s", pszName);
			...
			FOR_EACH_VALUE(pKVRarity, pKVItem)
			{
				...
				SCHEMA_INIT_CHECK(pDef != NULL, "Item set %s: Item definition \"%s\" was not found", m_pszName,
								  pszName);

				const item_definition_index_t unDefIndex = pDef->GetDefinitionIndex();

				SCHEMA_INIT_CHECK(!m_iItemDefs.IsValidIndex(m_iItemDefs.Find(unDefIndex)),
								  "Item Collection %s: item definition \"%s\" appears multiple times", m_pszName,
								  pszName);
				...
				if(!m_bIsReferenceCollection)
				{
					SCHEMA_INIT_CHECK(!pDef->GetItemCollectionDefinition(),
	...
	if(!m_bIsReferenceCollection)
	{
		...
		FOR_EACH_VEC(m_iItemDefs, i)
		{
			...
			SCHEMA_INIT_CHECK(iRarityVerify - iNextRarity <= 1, "Items in Collection %s: Have a gap in rarity tiers",
							  m_pszName);
	...
	// Sanity check.
	SCHEMA_INIT_CHECK(m_pszLocalizedName != NULL, "Item Collection %s: Collection contains no localized name",
					  m_pszName);
	SCHEMA_INIT_CHECK(m_pszLocalizedDesc != NULL, "Item Collection %s: Collection contains no localized description",
					  m_pszName);
	SCHEMA_INIT_CHECK(m_iItemDefs.Count() > 0, "Item Collection %s: Collection contains no items", m_pszName);
...
//-----------------------------------------------------------------------------
CEconItemPaintKitDefinition::CEconItemPaintKitDefinition(void) : m_pszName(NULL) {}
...
bool VerifyPaintKitComposite(KeyValues *pKVWearInputItems, const char *pName, int iWearLevel,
							 CUtlVector<CUtlString> *pVecErrors)
{
	...
	SafeAssign(&pWeaponSkinBaseCompositor,
			   materials->NewTextureCompositor(w, h, pName, TF_TEAM_RED, seed, pKVWearInputItems,
											   TEX_COMPOSITE_CREATE_FLAGS_VERIFY_SCHEMA_ONLY));
	SCHEMA_INIT_CHECK(pWeaponSkinBaseCompositor != NULL,
					  "Could Not Create Weapon Skin Compositor for [%s][Wear %d][Team Red]", pName, iWearLevel);
	SafeRelease(&pWeaponSkinBaseCompositor);

	SafeAssign(&pWeaponSkinBaseCompositor,
			   materials->NewTextureCompositor(w, h, pName, TF_TEAM_BLUE, seed, pKVWearInputItems,
											   TEX_COMPOSITE_CREATE_FLAGS_VERIFY_SCHEMA_ONLY));
	SCHEMA_INIT_CHECK(pWeaponSkinBaseCompositor != NULL,
					  "Could Not Create Weapon Skin Compositor for [%s][Wear %d][Team BLUE]", pName, iWearLevel);
	SafeRelease(&pWeaponSkinBaseCompositor);
#endif // CLIENT_DLL
	return SCHEMA_INIT_SUCCESS();
}
...
bool CEconItemPaintKitDefinition::BInitFromKV(KeyValues *pKVPItemPaintKit, CUtlVector<CUtlString> *pVecErrors)
{
	...
	SCHEMA_INIT_CHECK(VerifyPaintKitComposite(pKVWearInputItems, m_pszName, 1, pVecErrors),
					  "Could Not Create Weapon Skin Compositor for [%s][Wear %d]", m_pszName, 1);
	...
	SCHEMA_INIT_CHECK(VerifyPaintKitComposite(pKVWearInputItems, m_pszName, 2, pVecErrors),
					  "Could Not Create Weapon Skin Compositor for [%s][Wear %d]", m_pszName, 2);
	m_vecPaintKitWearKVP.AddToTail(pKVWearInputItems->MakeCopy());

	pKVWearInputItems = pKVPItemPaintKit->FindKey("wear_level_3", false);
	SCHEMA_INIT_CHECK(VerifyPaintKitComposite(pKVWearInputItems, m_pszName, 3, pVecErrors),
					  "Could Not Create Weapon Skin Compositor for [%s][Wear %d]", m_pszName, 3);
	m_vecPaintKitWearKVP.AddToTail(pKVWearInputItems->MakeCopy());

	pKVWearInputItems = pKVPItemPaintKit->FindKey("wear_level_4", false);
	SCHEMA_INIT_CHECK(VerifyPaintKitComposite(pKVWearInputItems, m_pszName, 4, pVecErrors),
					  "Could Not Create Weapon Skin Compositor for [%s][Wear %d]", m_pszName, 4);
	m_vecPaintKitWearKVP.AddToTail(pKVWearInputItems->MakeCopy());

	pKVWearInputItems = pKVPItemPaintKit->FindKey("wear_level_5", false);
	SCHEMA_INIT_CHECK(VerifyPaintKitComposite(pKVWearInputItems, m_pszName, 5, pVecErrors),
					  "Could Not Create Weapon Skin Compositor for [%s][Wear %d]", m_pszName, 5);
	m_vecPaintKitWearKVP.AddToTail(pKVWearInputItems->MakeCopy());
...
bool CEconOperationDefinition::BInitFromKV(KeyValues *pKVPOperation, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(pszRequiredName)
	{
		...
		SCHEMA_INIT_CHECK(pDef != NULL, "OperationDefinition couldn't find item def from required name '%s'",
						  pszRequiredName);
	...
	if(pszGatewayItemName)
	{
		...
		SCHEMA_INIT_CHECK(pDef != NULL, "OperationDefinition couldn't find item def from gateway name '%s'",
						  pszGatewayItemName);
	...
	{
		SCHEMA_INIT_CHECK(m_unRequiredItemDefIndex != INVALID_ITEM_DEF_INDEX,
						  "If a gateway item is set, a required item must be set!  Mismatch in %d", m_unOperationID);
	}

	const char *pszOperationStartDate = pKVPOperation->GetString("operation_start_date", NULL);
	SCHEMA_INIT_CHECK(pszOperationStartDate != NULL, "OperationDefinition %s does not have 'operation_start_date'",
					  m_pszName);
	...
	SCHEMA_INIT_CHECK(pszDropEndDate != NULL, "OperationDefinition %s does not have 'stop_giving_to_player_date'",
					  m_pszName);
	...
	SCHEMA_INIT_CHECK(pszOperationEndDate != NULL, "OperationDefinition %s does not have 'stop_adding_to_queue_date'",
					  m_pszName);
	...
	SCHEMA_INIT_CHECK(m_pszOperationLootList != NULL, "OperationDefinition %s does not have 'operation_lootlist'",
					  m_pszName);
	...
	m_rtQueueFreqMin =
		pKVPOperation->GetFloat("queue_freq_min") * k_nSecondsPerHour; // converts specified hours to seconds
	SCHEMA_INIT_CHECK(m_rtQueueFreqMin != 0.f, "OperationDefinition %s does not have 'queue_freq_min'", m_pszName);

	m_rtQueueFreqMax =
		pKVPOperation->GetFloat("queue_freq_max") * k_nSecondsPerHour; // converts specified hours to seconds
	SCHEMA_INIT_CHECK(m_rtQueueFreqMax != 0.f, "OperationDefinition %s does not have 'queue_freq_max'", m_pszName);

	SCHEMA_INIT_CHECK(m_rtQueueFreqMin <= m_rtQueueFreqMax,
					  "OperationDefinition %s 'queue_freq_min' must be less than 'queue_freq_max'", m_pszName);

	m_rtDropFreqMin =
		pKVPOperation->GetFloat("drop_freq_min") * k_nSecondsPerHour; // converts specified hours to seconds
	SCHEMA_INIT_CHECK(m_rtDropFreqMin != 0.f, "OperationDefinition %s does not have 'drop_freq_min'", m_pszName);

	m_rtDropFreqMax =
		pKVPOperation->GetFloat("drop_freq_max") * k_nSecondsPerHour; // converts specified hours to seconds
	SCHEMA_INIT_CHECK(m_rtDropFreqMax != 0.f, "OperationDefinition %s does not have 'drop_freq_max'", m_pszName);

	SCHEMA_INIT_CHECK(m_rtDropFreqMin <= m_rtDropFreqMax,
					  "OperationDefinition %s 'drop_freq_min' must be less than 'drop_freq_max'", m_pszName);
...
// Purpose:
//-----------------------------------------------------------------------------
bool BCommonInitPropertyGeneratorsFromKV(const char *pszContext,
										 CUtlVector<const IEconItemPropertyGenerator *> *out_pvecGenerators,
										 KeyValues *pKV, CUtlVector<CUtlString> *pVecErrors)
{
...
#ifdef GC_DLL
bool CEconLootListDefinition::AddRandomAtrributes(KeyValues *pRandomAttributesKV, CEconItemSchema &pschema,
												  CUtlVector<CUtlString> *pVecErrors /*= NULL*/)
{
	...
	SCHEMA_INIT_CHECK(NULL != pRandomAttr,
...
bool CEconLootListDefinition::AddRandomAttributesFromTemplates(KeyValues *pRandomAttributesKV, CEconItemSchema &pschema,
															   CUtlVector<CUtlString> *pVecErrors /*= NULL*/)
{
	const char *pszAttrName = pRandomAttributesKV->GetName();
...
bool CEconLootListDefinition::BInitFromKV(KeyValues *pKVLootList, CEconItemSchema &pschema,
										  CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_SUBKEY(pKVLootList, pKVListItem)
	{
		...
		if(!Q_strcmp(pszName, "loot_list_header_desc"))
		{
			...
			SCHEMA_INIT_CHECK(g_pszDefaultRevolvingLootListHeader == m_pszLootListHeader,
							  "Loot list %s: Multiple header descriptions specified", m_pszName);

			m_pszLootListHeader = pKVListItem->GetString();

			SCHEMA_INIT_CHECK(NULL != m_pszLootListHeader, "Loot list %s: Invalid header description specified",
							  m_pszName);
		...
		{
			// Make sure we didn't specify multiple entries.
			SCHEMA_INIT_CHECK(NULL == m_pszLootListFooter, "Loot list %s: Multiple footer descriptions specified",
							  m_pszName);

			m_pszLootListFooter = pKVListItem->GetString();

			SCHEMA_INIT_CHECK(NULL != m_pszLootListFooter, "Loot list %s: Invalid header description specified",
							  m_pszName);
		...
		else if(!Q_strcmp(pszName, "attribute_templates"))
		{
			FOR_EACH_SUBKEY(pKVListItem, pKVAttributeTemplate)
			{
				...
				SCHEMA_INIT_CHECK(bAdded, "Loot list %s: Failed to attribute_templates '%s'", m_pszName,
								  pKVAttributeTemplate->GetName());
			}
		...
		else if(!Q_strcmp(pszName, "additional_drop"))
		{
			...
			int iRequiredHolidayIndex =
				pszRequiredHoliday ? EconHolidays_GetHolidayForString(pszRequiredHoliday) : kHoliday_None;
			...
			SCHEMA_INIT_CHECK(pszDropPerdiodStartDate == NULL ||
								  Q_strcmp(CRTime::RTime32ToString(dropStartDate, rtimeBuf), pszDropPerdiodStartDate) ==
									  0,
							  "Malformed start drop date \"%s\" for additional_drop in lootlist %s.  Must be of the "
							  "form \"YYYY-MM-DD hh:mm:ss\"",
							  pszDropPerdiodStartDate, m_pszName);
			...
			// Check that if we convert back to a string, we get the same value
			SCHEMA_INIT_CHECK(pszDropPerdiodEndDate == NULL ||
								  Q_strcmp(CRTime::RTime32ToString(dropEndDate, rtimeBuf), pszDropPerdiodEndDate) == 0,
							  "Malformed end drop date \"%s\" for additional_drop in lootlist %s.  Must be of the form "
							  "\"YYYY-MM-DD hh:mm:ss\"",
							  pszDropPerdiodEndDate, m_pszName);

			SCHEMA_INIT_CHECK(fChance > 0.0f && fChance <= 1.0f,
							  "Loot list %s: Invalid \"additional_drop\" chance %.2f", m_pszName, fChance);

			SCHEMA_INIT_CHECK(pszLootList && pszLootList[0], "Loot list %s: Missing \"additional_drop\" loot list name",
							  m_pszName);

			SCHEMA_INIT_CHECK((pszRequiredHoliday == NULL) == (iRequiredHolidayIndex == kHoliday_None),
							  "Loot list %s: Unknown or missing holiday \"%s\"", m_pszName,
			...
			if(pszLootList)
			{
				...
				SCHEMA_INIT_CHECK(pLootListDef, "Loot list %s: Invalid \"additional_drop\" loot list \"%s\"", m_pszName,
								  pszLootList);
				...
				if(pLootListDef)
				{
					...
					loot_list_additional_drop_t additionalDrop = {fChance, bPremiumOnly, pszLootList,
																  iRequiredHolidayIndex, dropPeriod};
		...
		{
			SCHEMA_INIT_SUBSTEP(
				BCommonInitPropertyGeneratorsFromKV(m_pszName, &m_PropertyGenerators, pKVListItem, pVecErrors));
			continue;
		}
#endif // GC_DLL
		...
		else
		{
			...
			else
			{
				...
				SCHEMA_INIT_CHECK(pDef != NULL, "Loot list %s: Item definition \"%s\" was not found", m_pszName,
								  pszName);
			}
		}
		...
		if(iDef > 0)
		{
			...
			SCHEMA_INIT_CHECK(pItemDef != NULL, "Loot list %s: Item definition index \"%s\" (%d) was not found",
							  m_pszName, pszName, iDef);
			...
			if(FindAttribute(pItemDef, pAttribDef_EndDropDate, &value))
			{
				...
				SCHEMA_INIT_CHECK(
					...
					pszEndDate, pItemDef->GetDefinitionName());
			}

#ifdef GC_DLL
			if(pItemDef)
			{
				SCHEMA_INIT_CHECK(true == pItemDef->BEnabled(),
								  "Loot list %s: Item definition \"%s\" (%d) isn't enabled, not allowed in loot lists",
								  m_pszName, pItemDef->GetDefinitionName(), iDef);
			}
#endif // GC_DLL
		}
		...
		SCHEMA_INIT_CHECK(fItemWeight > 0.0f, "Loot list %s: Item definition index \"%s\" (%d) has invalid weight %.2f",
						  m_pszName, pszName, iDef, fItemWeight);
#endif // GC_DLL
	...
	FOR_EACH_VEC(m_DropList, i)
	{
		...
		if(m_DropList[i].m_dropPeriod.m_DropStartDate != RTime32(0) ||
		   m_DropList[i].m_dropPeriod.m_DropEndDate != ~RTime32(0))
		{
	...
	SCHEMA_INIT_CHECK(
		m_DropList.Count() > nNumTimeLimitedItems,
		"Lootlist \"%s\" is made up entirely of limited-time items!  At least one must not be time-limited.",
		m_pszName);
#endif
...
// Purpose:
//-----------------------------------------------------------------------------
static bool BContainsDuplicateItemDefs(const CUtlVector<CEconLootListDefinition::rolled_item_defs_t> &vecItemDefsA,
									   const CUtlVector<CEconLootListDefinition::rolled_item_defs_t> &vecItemDefsB)
{
	...
	return BPopulateAndLookForDupes(vecItemDefsA) || BPopulateAndLookForDupes(vecItemDefsB);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CEconLootListDefinition::BGenerateSingleRollRandomItems(
	const CEconGameAccount *pGameAccount, bool bFreeAccount, CUtlVector<CEconItem *> *out_pvecItems,
	const CUtlVector<item_definition_index_t> *pVecAvoidItemDefs /*= NULL*/) const
{
	...
	// leave.
	std::unique_ptr<IUniformRandomStream> pRandomStream(
	...
	CUtlVector<rolled_item_defs_t>
		vecCumulativeItemDefs; // total list of everything we've generated so far in any number of result sets
	...
	enum
	{
		kUpperBoundIterationSanityCheck = 2000
	};
	...
	FOR_EACH_VEC(vecItemDefs, i)
	{
		...
		// Create the items
		CEconItem *pItem = GEconManager()->GetItemFactory().CreateSpecificItem(
			pGameAccount, rolledDef.m_pItemDef->GetDefinitionIndex());
...
public:
		...
		while(nRolls--)
		{
			...
			for(auto pItem : vecItems)
			{
				...
				const CEconItemRarityDefinition *pItemRarity =
					GetItemSchema()->GetRarityDefinition(pItem->GetItemDefinition()->GetRarity());
				if(pItemRarity)
				{
	...
	void PrintRarityBreakdwn()
	{
		...
		while(m_mapRarityCounts.Count())
		{
			...
			EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%9s %*d %.3f%%\n", m_mapRarityCounts.Key(maxIdx),
					 nMaxDigits, m_mapRarityCounts[maxIdx],
		...
		EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%-*s  %*s  %*s\n",
				 9 + 1 + NumDigits(m_DropsMaxes.m_nRollCount) + 6, "-- Item breakdown",
				 NumDigits(m_DropsMaxes.m_nStrangeCount) + 1, "S", NumDigits(m_DropsMaxes.m_nUnusualCount), "U");
	}
	...
	void PrintTotals()
	{
		while(m_mapCounts.Count())
		{
			...
			{
				AutoYield();
				if(maxIdx == m_mapCounts.InvalidIndex() ||
				   m_mapCounts[i].m_nRollCount > m_mapCounts[maxIdx].m_nRollCount)
				{
					maxIdx = i;
				}
			}
			...
			const char *pszRarity =
				pItemRarity ? GGCGameBase()->LocalizeToken(pItemRarity->GetLocKey(), k_Lang_English) : "";
			...
			const char *pszItemname = GGCGameBase()->LocalizeToken(
				pItemDef->GetCustomPainkKitDefinition() ? pItemDef->GetCustomPainkKitDefinition()->GetName()
														: pItemDef->GetItemBaseName(),
				k_Lang_English);

			EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%9s %s  %s  %s  %s\n", pszRarity, rollString.Get(),
					 strangeString.Get(), unusualString.Get(), pszItemname);

			m_mapCounts.RemoveAt(maxIdx);
		}

		EmitInfo(
			SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "-- Total Items: %d Strange: %d (%.3f%%) Unusual: %d (%.3f%%)\n",
			m_DropsTotals.m_nRollCount, m_DropsTotals.m_nStrangeCount,
			(100.f * (float)m_DropsTotals.m_nStrangeCount / m_DropsTotals.m_nRollCount), m_DropsTotals.m_nUnusualCount,
	...
	const DropResult_t &GetTotalDrops() const
	{
		return m_DropsTotals;
	}
	const DropResult_t &GetMaxesDrops() const
	{
		return m_DropsMaxes;
	}
...
private:
	int PrintUnusualsForType(CUtlMap<uint32, int> &mapUnusuals)
	{
		...
		while(mapUnusuals.Count())
		{
			...
			EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%*d %.3f%% %s (%d)\n", nMaxDigits, mapUnusuals[maxIdx],
					 100.f * (float)mapUnusuals[maxIdx] / nTotal, pszParticleName, mapUnusuals.Key(maxIdx));
...
GC_CON_COMMAND(simulate_lootlist_contents,
			   "<lootlist> <iterations> Check item distribution from a given lootlist n times")
{
...
{
public:
	CItemSourceFinder(const char *pszItemName) : m_pItemDef(GetItemSchema()->GetItemDefinitionByName(pszItemName))
	{
		Assert(m_pItemDef);
		if(!m_pItemDef)
		{
		...
		FOR_EACH_MAP_FAST(mapItemDefs, i)
		{
			...
			else // Crate with an item series?
			{
				int iCrateSeries;
				{
					...
					if(!FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pSourceItemDef, pAttr_CrateSeries,
																		&fCrateSeries) ||
					   fCrateSeries == 0.0f)
						continue;
...
private:
	struct DropSource_t
	{
		void PrintSources()
		{
			...
			EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%3.5f%% %s (%d - %s)\n", lootlistSource.m_flChance * 100.f,
					 m_pDroppingItem->GetDefinitionName(), m_pDroppingItem->GetDefinitionIndex(),
		...
		{
			LootListSource_t() : m_flChance(0.f) {}
			...
			void PrintSources(int nInset)
			{
				...
				if(nRevolvingIdx != mapRevolvingLootlists.InvalidIndex() && nRevolvingIdx > 0)
				{
					...
					EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%*.5f%% (%d) %s\n", 4 * nInset + 5,
							 m_flChance * 100.f, nRevolvingIdx, m_pDroppingLootlist->GetName());
				}
				else
				{
					// Not in the revolving lootlist
					EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%*.5f%% %s\n", 4 * nInset + 5, m_flChance * 100.f,
	...
	float ChanceForItemFromLootlist(const CEconItemDefinition *pItemDef, const CEconLootListDefinition *pLootlist,
									DropSource_t::LootListSource_t &lootlistSource)
	{
		...
		FOR_EACH_VEC(vecAdditionalDrops, i)
		{
			...
			auto &subSource = lootlistSource.m_vecLootlistSources[lootlistSource.m_vecLootlistSources.AddToTail()];
			lootlistSource.m_flChance +=
				ChanceForItemFromLootlist(pItemDef, pSubLootlist, subSource) * additionalDrop.m_fChance;
		}
...
GC_CON_COMMAND(list_keys, "Lists all the keys in the schema")
{
	...
	FOR_EACH_MAP_FAST(mapItemDefs, i)
	{
		...
		EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%llu - %s\n", pItemDef->GetDefinitionIndex(),
				 pItemDef->GetItemDefinitionName());
	}
}

ConVar case_behavior_rolls_per_lootlist("case_behavior_rolls_per_lootlist", "1000000", FCVAR_REPLICATED,
										"How many times to roll a lootlist");
class CJobCaseBehaviorCheck : public CGCGameBaseJob
{
public:
	CJobCaseBehaviorCheck() : CGCGameBaseJob(GGCGameBase()) {}
		...
		FOR_EACH_MAP(lootlists, i)
		{
			...
			if(pEconLootlist)
			{
				// Check if we have data for this lootlist on this hash already
				{
					...
					if(sqlReadAccess.BYieldingReadRecordsWithWhereClause(
						   &vecExistingResult, "SchemaSHA = ? and Series = ?", CSET_FULL(CSchCaseBehavior)))
					{
				...
				behavior.SetVarCharField(behavior.m_VarCharSchemaSHA, pchSHAHex, true,
										 CSchCaseBehavior::k_iField_VarCharSchemaSHA);
				behavior.m_RTime32Date = now;
				behavior.m_unSeries = i;
				behavior.SetVarCharField(behavior.m_VarCharLootListName, pEconLootlist->GetName(), true,
										 CSchCaseBehavior::k_iField_VarCharLootListName);
				behavior.m_fStrangeChance =
					100.f * (float)simulator.GetTotalDrops().m_nStrangeCount / simulator.GetTotalDrops().m_nRollCount;
				behavior.m_fUnusualChance =
					100.f * (float)simulator.GetTotalDrops().m_nUnusualCount / simulator.GetTotalDrops().m_nRollCount;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEconLootListDefinition::RollRandomItemsAndAdditionalItems(
	IUniformRandomStream *pRandomStream, bool bFreeAccount, CUtlVector<rolled_item_defs_t> *out_pVecRolledItems,
	const CUtlVector<item_definition_index_t> *pVecAvoidItemDefs) const
{
	...
	FOR_EACH_VEC(m_AdditionalDrops, i)
	{
		...
		if(m_AdditionalDrops[i].m_iRequiredHolidayIndex != kHoliday_None &&
		   !EconHolidays_IsHolidayActive(m_AdditionalDrops[i].m_iRequiredHolidayIndex, CRTime::RTime32TimeCur()))
			continue;
		...
		{
			AssertMsg2(false, "Loot list '%s' specifies unknown additional drop '%s'", GetName(),
					   pszAdditionalDropLootList);
			return false;
		}

		bCreatedItems &=
			pLootListDef->RollRandomItemsAndAdditionalItems(pRandomStream, bFreeAccount, out_pVecRolledItems);
	}

	// If we failed to create some items, we might still have chosen some item defs before those failures. In that case,
	// clear out any choices we've made so far
	if(!bCreatedItems)
	{
...
bool CEconLootListDefinition::RollRandomItemDef(IUniformRandomStream *pRandomStream, bool bFreeAccount,
												CUtlVector<rolled_item_defs_t> *out_pVecRolledItems,
												const CUtlVector<item_definition_index_t> *pVecAvoidItemDefs) const
{
	Assert(out_pVecRolledItems);
	...
	FOR_EACH_VEC(vecValidDrops, i)
	{
		...
		{
			const int iItemDef =
				vecValidDrops[i]->m_iItemOrLootlistDef; // not item_definition_index because it might also be a negative
														// value to indicate a sub lootlist
			...
			else
			{
				...
				if(!pNestedLootList->RollRandomItemsAndAdditionalItems(pRandomStream, bFreeAccount, &vecScratchDefs,
																	   pVecAvoidItemDefs))
					return false;
			}

			// Add ourselves to the list of affecting lootlists, so that we and all nested loot lists will affect this
			// item We intentionally don't do this in our calling function because we don't want to include additional
			// drops.
			...
			// We want to exit the loop here regardless of whether items were successfully so that we only perform a
			// single item-generating roll on this list.
			break;
		}
	}
...
//-----------------------------------------------------------------------------
void CEconLootListDefinition::GetRarityLootLists(
	CUtlVector<const CEconLootListDefinition *> *out_pVecRarityLootList) const
{
	...
	FOR_EACH_VEC(m_DropList, i)
	{
		const int iItemDef = m_DropList[i].m_iItemOrLootlistDef; // not item_definition_index because it might also be a
																 // negative value to indicate a sub lootlist
		if(iItemDef < 0)
		{
			// In the case where iItemDef is negative, it's a nested loot list. Ask that list to choose an item.
			// HACKY: Store loot list indices as negatives, starting from -1, because 0 is a valid item index
...
//			done with them!
//-----------------------------------------------------------------------------
void CEconLootListDefinition::RollRandomAttributes(CUtlVector<static_attrib_t> &vecAttributes,
...
bool CEconLootListDefinition::BAttachLootListAttributes(const CEconGameAccount *pGameAccount, CEconItem *pItem) const
{
	...
	FOR_EACH_VEC(vecAttributes, i)
	{
		...
		vecAttributes[i].GetAttributeDefinition()->GetAttributeType()->UnloadEconAttributeValue(
			&vecAttributes[i].m_value);
	}
...
// Purpose:
//-----------------------------------------------------------------------------
bool lootlist_attrib_t::BInitFromKV(const char *pszContext, KeyValues *pKVKey, CEconItemSchema &pschema,
									CUtlVector<CUtlString> *pVecErrors)
{
	SCHEMA_INIT_SUBSTEP(m_staticAttrib.BInitFromKV_MultiLine(pszContext, pKVKey, pVecErrors));

	SCHEMA_INIT_CHECK(pKVKey->FindKey("weight"), "Context '%s': Attribute \"%s\" missing required 'weight' field",
					  pszContext, pKVKey->GetName());
...
//-----------------------------------------------------------------------------
bool random_attrib_t::RollRandomAttributes(CUtlVector<static_attrib_t> &vecAttributes,
										   const CEconGameAccount *pGameAccount) const
{
	if(m_flChanceOfRandomAttribute && RandomFloat() <= m_flChanceOfRandomAttribute)
	{
		...
		for(int iAttrib = 0; iAttrib < m_RandomAttributes.Count(); ++iAttrib)
		{
			...
			if(flRand <= flAccum)
			{
				...
				pAttrDef->GetAttributeType()->GenerateEconAttributeValue(pAttrDef, staticAttrib, pGameAccount,
																		 &staticAttrib.m_value);
...
#ifdef GC_DLL
/*static*/ CSchemaAttributeDefHandle CAttributeLineItemLootList::s_pAttrDef_RandomDropLineItemUnusualChance(
	"random drop line item unusual chance"); // "one out of this many"
/*static*/ CSchemaAttributeDefHandle CAttributeLineItemLootList::s_pAttrDef_RandomDropLineItemUnusualList(
	"random drop line item unusual list");
#endif // GC_DLL
CSchemaAttributeDefHandle CAttributeLineItemLootList::s_pAttrDef_RandomDropLineItemFooterDesc(
...
const char *CAttributeLineItemLootList::GetLootListFooterLocalizationKey() const
{
	...
	if(FindAttribute_UnsafeBitwiseCast<CAttribute_String>(m_pEconItem, s_pAttrDef_RandomDropLineItemFooterDesc,
														  &pszFooter))
	{
		return pszFooter;
	}
...
bool CEconCraftingRecipeDefinition::BInitFromKV(KeyValues *pKVRecipe, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	// Check for required fields
	SCHEMA_INIT_CHECK(NULL != pKVRecipe->FindKey("input_items"),
					  "Recipe definition %d: Missing required field \"input_items\"", m_nDefIndex);

	SCHEMA_INIT_CHECK(NULL != pKVRecipe->FindKey("output_items"),
	...
	m_iCategory = (recipecategories_t)StringFieldToInt(pKVRecipe->GetString("category"), g_szRecipeCategoryStrings,
													   ARRAYSIZE(g_szRecipeCategoryStrings));
...
//
//-----------------------------------------------------------------------------
bool CEconCraftingRecipeDefinition::ItemListMatchesInputs(CUtlVector<CEconItem *> *vecCraftingItems,
														  KeyValues *out_pkvCraftParams, bool bIgnoreSlop,
														  CUtlVector<uint64> *vecChosenItems) const
{
	return false;
}
...
//-----------------------------------------------------------------------------
#ifdef GC_DLL
#define GC_SCH_REFERENCE(TAttribSchType) TAttribSchType,
#else
...
class CSchemaAttributeTypeProtobufBase
	: public CSchemaAttributeTypeBase<GC_SCH_REFERENCE(TAttribSchType) TProtobufValueType>
{
public:
	virtual void ConvertTypedValueToByteStream(const TProtobufValueType &typedValue,
											   ::std::string *out_psBytes) const OVERRIDE
	{
		DbgVerify(typedValue.SerializeToString(out_psBytes));
	}

	virtual void ConvertByteStreamToTypedValue(const ::std::string &sBytes,
											   TProtobufValueType *out_pTypedValue) const OVERRIDE
	{
		DbgVerify(out_pTypedValue->ParseFromString(sBytes));
	}

	virtual bool BConvertStringToEconAttributeValue(
		const CEconItemAttributeDefinition *pAttrDef, const char *pszValue, union attribute_data_union_t *out_pValue,
		bool bEnableTerribleBackwardsCompatibilitySchemaParsingCode) const OVERRIDE
	{
		Assert(pAttrDef);
		Assert(out_pValue);
	...
	virtual void ConvertEconAttributeValueToString(const CEconItemAttributeDefinition *pAttrDef,
												   const attribute_data_union_t &value,
												   std::string *out_ps) const OVERRIDE
	{
		Assert(pAttrDef);
		Assert(out_ps);
...
class CSchemaAttributeType_String
	: public CSchemaAttributeTypeProtobufBase<GC_SCH_REFERENCE(CSchItemAttributeString) CAttribute_String>
{
public:
#ifdef GC_DLL
	virtual void ConvertEconAttributeValueToSch(itemid_t unItemId, const CEconItemAttributeDefinition *pAttrDef,
												const attribute_data_union_t &value,
												GCSDK::CRecordBase *out_pSchRecord) const OVERRIDE
	{
	...
	virtual void LoadSchToEconAttributeValue(CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef,
											 const GCSDK::CRecordBase *pSchRecord) const OVERRIDE
	{
		Assert(pTargetItem);
		Assert(pAttrDef);
		Assert(pSchRecord);
	...
	// We intentionally override the convert-to-/convert-from-string functions for strings so that string literals can
	// be specified in the schema, etc. without worrying about the protobuf text format.
	virtual bool BConvertStringToEconAttributeValue(
		const CEconItemAttributeDefinition *pAttrDef, const char *pszValue, union attribute_data_union_t *out_pValue,
		bool bEnableTerribleBackwardsCompatibilitySchemaParsingCode) const OVERRIDE
	{
...
// Purpose:
//-----------------------------------------------------------------------------
class CSchemaAttributeType_DynamicRecipeComponentDefinedItem
	: public CSchemaAttributeTypeProtobufBase<GC_SCH_REFERENCE(CSchItemAttributeDynamicRecipeComponentDefinedItem)
												  CAttribute_DynamicRecipeComponent>
{
public:
#ifdef GC_DLL
	virtual void ConvertEconAttributeValueToSch(itemid_t unItemId, const CEconItemAttributeDefinition *pAttrDef,
												const union attribute_data_union_t &value,
												GCSDK::CRecordBase *out_pSchRecord) const OVERRIDE
	{
		Assert(out_pSchRecord);
		Assert(pAttrDef);
		...
		CSchItemAttributeDynamicRecipeComponentDefinedItem *out_psch =
			GetTypedSch<CSchItemAttributeDynamicRecipeComponentDefinedItem>(out_pSchRecord);
											 ...
											 const GCSDK::CRecordBase *pSchRecord) const OVERRIDE
	{
		...
		const CSchItemAttributeDynamicRecipeComponentDefinedItem *psch =
			GetTypedSch<const CSchItemAttributeDynamicRecipeComponentDefinedItem>(pSchRecord);
		...
		bool bEnableTerribleBackwardsCompatibilitySchemaParsingCode) const OVERRIDE
	{
		...
		for(int i = 0; i < pActualFields->field_count(); ++i)
		{
			...
			{
				EmitError(SPEW_GC,
						  "Missing field '%s' in CAttribute_DynamicRecipeComponent_COMPAT_NEVER_SERIALIZE_THIS_OUT\n",
...
// Purpose:
//-----------------------------------------------------------------------------
class CSchemaAttributeType_ItemSlotCriteria : public CSchemaAttributeTypeProtobufBase<GC_SCH_REFERENCE(
												  CSchItemAttributeItemSlotCriteria) CAttribute_ItemSlotCriteria>
{
public:
#ifdef GC_DLL
	virtual void ConvertEconAttributeValueToSch(itemid_t unItemId, const CEconItemAttributeDefinition *pAttrDef,
...
// Purpose:
//-----------------------------------------------------------------------------
class CSchemaAttributeType_WorldItemPlacement : public CSchemaAttributeTypeProtobufBase<GC_SCH_REFERENCE(
													CSchItemAttributeWorldItemPlacement) CAttribute_WorldItemPlacement>
{
public:
#ifdef GC_DLL
	virtual void ConvertEconAttributeValueToSch(itemid_t unItemId, const CEconItemAttributeDefinition *pAttrDef,
												...
												GCSDK::CRecordBase *out_pSchRecord) const OVERRIDE
	{
		...
		CSchItemAttributeWorldItemPlacement *out_psch =
			GetTypedSch<CSchItemAttributeWorldItemPlacement>(out_pSchRecord);
											 ...
											 const GCSDK::CRecordBase *pSchRecord) const OVERRIDE
	{
		...
		const CSchItemAttributeWorldItemPlacement *psch =
			GetTypedSch<const CSchItemAttributeWorldItemPlacement>(pSchRecord);
...
public:
												...
												GCSDK::CRecordBase *out_pSchRecord) const OVERRIDE
	{
		...
		// @note Tom Bui: we store the value as an unsigned integer in the DB, so just treat the field as a bunch of
		// bits
	...
	virtual void ConvertTypedValueToByteStream(const float &typedValue, ::std::string *out_psBytes) const OVERRIDE
	{
		...
		*reinterpret_cast<float *>(&((*out_psBytes)[0])) =
			typedValue; // overwrite string contents (sizeof( float ) bytes)
	}
...
public:
	...
	virtual void ConvertTypedValueToByteStream(const uint64 &typedValue, ::std::string *out_psBytes) const OVERRIDE
	{
		...
		*reinterpret_cast<uint64 *>(&((*out_psBytes)[0])) =
			typedValue; // overwrite string contents (sizeof( uint64 ) bytes)
	}
...
public:
#ifdef GC_DLL
	virtual bool BAssetClassExportedAttributeValue(const CEconItemAttributeDefinition *pAttrDef,
												   const attribute_data_union_t &value) const OVERRIDE
	{
		Assert(pAttrDef);
		...
		return CSchemaAttributeTypeBase<GC_SCH_REFERENCE(CSchItemAttribute)
											attrib_value_t>::BAssetClassExportedAttributeValue(pAttrDef, value);
	}
	...
	virtual void LoadOrGenerateEconAttributeValue(CEconItem *pTargetItem, const CEconItemAttributeDefinition *pAttrDef,
												  const static_attrib_t &staticAttrib,
												  const CEconGameAccount *pGameAccount) const OVERRIDE
	{
		...
		// do not apply an attribute if it already exists.  If the new and the old attribute value is different then we
		// assert (and use the latest value)
		...
		if(pTargetItem->FindAttribute(pAttrDef, &unValue) && pAttrDef != pAttrDef_PaintkitWear)
		{
			AssertMsg4(unValue == staticAttrib.m_value.asUint32,
					   "Item id %llu (%s) attempting to generate dynamic attribute value for '%s' (%d) when attribute "
					   "already exists with a different Value! This probably indicates some sort of code flow error "
					   "calling LoadOrGenerateEconAttributeValue() late.",
					   pTargetItem->GetItemID(), pTargetItem->GetItemDefinition()->GetDefinitionName(),
					   pAttrDef->GetDefinitionName(), pAttrDef->GetDefinitionIndex());

			if(unValue == staticAttrib.m_value.asUint32)
				return;
		}

		// Could be raw integer bits or raw floating-point bits depending on where in the union we stored the value. We
		// copy the bit pattern indiscriminately.
	...
	virtual void GenerateEconAttributeValue(const CEconItemAttributeDefinition *pAttrDef,
											const static_attrib_t &staticAttrib, const CEconGameAccount *pGameAccount,
											attribute_data_union_t *out_pValue) const
	{
		Assert(pAttrDef);
		AssertMsg(pGameAccount || !staticAttrib.m_pKVCustomData,
				  "Cannot run custom logic with no game account object! Passing in NULL for pGameAccount is only "
				  "supported when we know we won't be running custom value generation code!");
		Assert(out_pValue);
		...
		bool bEnableTerribleBackwardsCompatibilitySchemaParsingCode) const OVERRIDE
	{
		...
		{
			// Not having any value specified is valid -- we interpret this as "default", or 0 as both an in int and a
			// float.
			out_pValue->asFloat = pszValue ? atof(pszValue) : 0.0f;
		}
	...
	virtual void ConvertTypedValueToByteStream(const attrib_value_t &typedValue,
											   ::std::string *out_psBytes) const OVERRIDE
	{
		Assert(out_psBytes);
		Assert(out_psBytes->size() == 0);
		...
		*reinterpret_cast<attrib_value_t *>(&((*out_psBytes)[0])) =
			typedValue; // overwrite string contents (sizeof( attrib_value_t ) bytes)
	}

	virtual void ConvertByteStreamToTypedValue(const ::std::string &sBytes,
											   attrib_value_t *out_pTypedValue) const OVERRIDE
	{
		Assert(out_pTypedValue);
#ifdef GC_DLL
...
private:
#ifdef GC_DLL
	void Internal_RunCustomAttributeValueLogic(const static_attrib_t &staticAttrib,
											   const CEconGameAccount *pGameAccount,
											   attribute_data_union_t *out_pValue) const
	{
		...
		{
			AssertMsg1(0,
					   "Unknown storage type for CSchemaAttributeType_Default::Internal_RunCustomAttributeValueLogic() "
					   "for attribute %s",
					   staticAttrib.GetAttributeDefinition()->GetDefinitionName());
		}
	}
#endif
};
...
bool CEconItemAttributeDefinition::BInitFromKV(KeyValues *pKVAttribute, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	m_iEffectType = (attrib_effect_types_t)StringFieldToInt(m_pKVAttribute->GetString("effect_type"), g_EffectTypes,
															ARRAYSIZE(g_EffectTypes));
	m_iDescriptionFormat = StringFieldToInt(m_pKVAttribute->GetString("description_format"),
											g_AttributeDescriptionFormats, ARRAYSIZE(g_AttributeDescriptionFormats));
	m_pszDescriptionString = m_pKVAttribute->GetString("description_string", NULL);
	m_pszArmoryDesc = m_pKVAttribute->GetString("armory_desc", NULL);
	m_pszAttributeClass = m_pKVAttribute->GetString("attribute_class", NULL);
	...
	const char *pszAttrType =
		m_pKVAttribute->GetString("attribute_type", NULL); // NULL implies "default type" for backwards compatibility
	m_pAttrType = GetItemSchema()->GetAttributeType(pszAttrType);

	SCHEMA_INIT_CHECK(NULL != m_pKVAttribute->FindKey("name"),
					  "Attribute definition %s: Missing required field \"name\"", m_pKVAttribute->GetName());

	SCHEMA_INIT_CHECK(NULL != m_pAttrType, "Attribute definition %s: Unable to find attribute data type '%s'",
					  m_pszDefinitionName, pszAttrType ? pszAttrType : "(default)");

	if(m_bIsSetBonus)
	{
		SCHEMA_INIT_CHECK(m_pAttrType->BSupportsGameplayModificationAndNetworking(),
						  "Attribute definition %s: set as set bonus attribute but does not support gameplay "
	...
	if(char const *szRule = pKVAttribute->GetString("asset_class_export", NULL))
	{
		...
		{
			m_eAssetClassAttrExportRule =
				EAssetClassAttrExportRule_t(k_EAssetClassAttrExportRule_GCOnly | k_EAssetClassAttrExportRule_Skip);
		}
		else if(!V_stricmp(szRule, "bucketed"))
		{
			SCHEMA_INIT_CHECK(m_unAssetClassBucket,
							  "Attribute definition %s: Asset class export rule '%s' is incompatible",
							  m_pszDefinitionName, szRule);
		...
		{
			SCHEMA_INIT_CHECK(false, "Attribute definition %s: Invalid asset class export rule '%s'",
							  m_pszDefinitionName, szRule);
		}
	}

	// Check for misuse of asset class bucket
	SCHEMA_INIT_CHECK((!m_unAssetClassBucket || m_bInstanceData),
					  "Attribute definition %s: Cannot use \"asset_class_bucket\" on class-level attributes",
					  m_pKVAttribute->GetName());

	return SCHEMA_INIT_SUCCESS();
}

CQuestObjectiveDefinition::CQuestObjectiveDefinition(void) : m_pszDescriptionToken(NULL), m_nDefIndex(0), m_nPoints(0)
{
}
...
bool CEconItemDefinition::BInitFromTestItemKVs(int iNewDefIndex, KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(iPaintCanIndex)
	{
		...
		float flRGBVal;
		if(pCanDef && pAttrDef_PaintRGB &&
		   FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pCanDef, pAttrDef_PaintRGB, &flRGBVal))
		{
			...
			StaticAttrib.m_value.asFloat = flRGBVal; // this is bad! but we're in crazy hack code for UI customization
													 // of item definitions that don't exist so
		}
	}
	...
	if(iUnusualEffectIndex)
	{
		...
		const attachedparticlesystem_t *pSystem =
			GetItemSchema()->GetAttributeControlledParticleSystem(iUnusualEffectIndex);
		...
		if(pAttrDef_AttachParticleStatic && pSystem)
		{
			...
			StaticAttrib.m_value.asFloat = iUnusualEffectIndex; // this is bad! but we're in crazy hack code for UI
																// customization of item definitions that don't exist so
		}
	}
	...
	for(int team = 0; team < TEAM_VISUAL_SECTIONS; team++)
	{
		...
		if(pVisualsKV)
		{
			...
			while(pKVEntry)
			{
				...
				else if(!Q_stricmp(pszEntry, "attached_models"))
				{
					FOR_EACH_SUBKEY(pKVEntry, pKVAttachedModelData)
					{
						...
						pVisData->m_AttachedModels[iAtt].m_iModelDisplayFlags =
							pKVAttachedModelData->GetInt("model_display_flags", kAttachedModelDisplayFlag_MaskAll);
						pVisData->m_AttachedModels[iAtt].m_pszModelName =
							pKVAttachedModelData->GetString("model", NULL);
					}
				}
				else if(!Q_stricmp(pszEntry, "attached_models_festive"))
				{
					FOR_EACH_SUBKEY(pKVEntry, pKVAttachedModelData)
					{
						...
						pVisData->m_AttachedModelsFestive[iAtt].m_iModelDisplayFlags =
							pKVAttachedModelData->GetInt("model_display_flags", kAttachedModelDisplayFlag_MaskAll);
						pVisData->m_AttachedModelsFestive[iAtt].m_pszModelName =
							pKVAttachedModelData->GetString("model", NULL);
					}
				}
				else if(!Q_stricmp(pszEntry, "attached_particlesystems"))
				{
					FOR_EACH_SUBKEY(pKVEntry, pKVAttachedParticleSystemData)
					{
						...
						pVisData->m_AttachedParticles[iAtt].pszSystemName =
							pKVAttachedParticleSystemData->GetString("system", NULL);
						pVisData->m_AttachedParticles[iAtt].pszControlPoints[0] =
							pKVAttachedParticleSystemData->GetString("attachment", NULL);
						pVisData->m_AttachedParticles[iAtt].bFollowRootBone =
							pKVAttachedParticleSystemData->GetBool("attach_to_rootbone");
				...
				else if(!Q_stricmp(pszEntry, "playback_activity"))
				{
					...
					{
						int iPlaybackInt = StringFieldToInt(pKVSubKey->GetName(), g_WearableAnimTypeStrings,
															ARRAYSIZE(g_WearableAnimTypeStrings));
						if(iPlaybackInt >= 0)
						{
							activity_on_wearable_t *pEntry =
								GetOrCreatePlaybackActivity(pVisData, (wearableanimplayback_t)iPlaybackInt);
			...
			if(pStylesDataKV)
			{
				...
				{
					pVecErrors->AddToTail("Per-team styles blocks are no longer valid. Use \"skin_red\" and "
										  "\"skin_blu\" in a style entry instead.");
				}
			}
...
// Purpose:
//-----------------------------------------------------------------------------
void CEconItemDefinition::GeneratePrecacheModelStrings(bool bDynamicLoad,
													   CUtlVector<const char *> *out_pVecModelStrings) const
{
...
void CEconItemDefinition::GeneratePrecacheSoundStrings(bool bDynamicLoad,
													   CUtlVector<const char *> *out_pVecSoundStrings) const
{
...
//-----------------------------------------------------------------------------
void CEconItemDefinition::BInitStylesBlockFromKV(KeyValues *pKVStyles, perteamvisuals_t *pVisData,
												 CUtlVector<CUtlString> *pVecErrors)
{
...
static void RecursiveInheritKeyValues(KeyValues *out_pValues, KeyValues *pInstance)
{
	...
	for(KeyValues *pSubKey = pInstance->GetFirstSubKey(); pSubKey != NULL;
		pPrevSubKey = pSubKey, pSubKey = pSubKey->GetNextKey())
	{
		...
		AssertMsg2(!pPrevSubKey || pPrevSubKey->GetNameSymbol() != pSubKey->GetNameSymbol(),
				   ...
				   pInstance->GetName(), pSubKey->GetName());
...
ConVar tf_loadondemand_default("cl_loadondemand_default", "1", FCVAR_ARCHIVE | FCVAR_CLIENTDLL,
							   "The default value for whether items should be delay loaded (1) or loaded now (0).");
#elif defined(GAME_DLL)
// The server doesn't load on demand by default because it can crash sometimes when this is set. We need to run that
// down, but in the meantime we just have it load on demand.
ConVar tf_loadondemand_default("sv_loadondemand_default", "0", FCVAR_ARCHIVE | FCVAR_GAMEDLL,
							   "The default value for whether items should be delay loaded (1) or loaded now (0).");
#elif defined(GC_DLL)
GCConVar tf_loadondemand_default("gc_loadondemand_default", "1", FCVAR_ARCHIVE,
								 "The default value for whether items should be delay loaded (1) or loaded now (0).");
#else
...
bool CEconItemDefinition::BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	// This logging is useful for tracking down bugs that crop up because we've (possibly) swapped the default value for
	// loadondemand. But it can be removed once we're satisfied there aren't any bugs as a result of the change (when we
	// cleanup WITH_STREAMABLE_WEAPONS).
	if(bGotDefault)
	{
	...
	// Check for required fields
	SCHEMA_INIT_CHECK(NULL != m_pKVItem->FindKey("name"), "Item definition %s: Missing required field \"name\"",
					  m_pKVItem->GetName());

	SCHEMA_INIT_CHECK(NULL != m_pKVItem->FindKey("item_class"),
					  "Item definition %s: Missing required field \"item_class\"", m_pKVItem->GetName());

	// Check value ranges
	SCHEMA_INIT_CHECK(m_pKVItem->GetInt("min_ilevel") >= 0,
					  "Item definition %s: \"min_ilevel\" must be greater than or equal to 0", GetDefinitionName());

	SCHEMA_INIT_CHECK(m_pKVItem->GetInt("max_ilevel") >= 0,
	...
	SCHEMA_INIT_CHECK(m_unMinItemLevel >= GetItemSchema()->GetMinLevel(),
					  "Item definition %s: min_ilevel (%d) must be greater or equal to Minimum Item Level (%d)",
					  GetDefinitionName(), m_unMinItemLevel, GetItemSchema()->GetMinLevel());

	SCHEMA_INIT_CHECK(m_unMinItemLevel <= m_unMaxItemLevel,
					  "Item definition %s: min_ilevel (%d) must be greater or equal to min_ilevel (%d)",
					  GetDefinitionName(), m_unMaxItemLevel, m_unMinItemLevel);

	SCHEMA_INIT_CHECK(m_unMaxItemLevel <= GetItemSchema()->GetMaxLevel(),
					  "Item definition %s: max_ilevel (%d) must be less than or equal to Maximum Item Level (%d)",
					  GetDefinitionName(), m_unMaxItemLevel, GetItemSchema()->GetMaxLevel());
	...
	if(m_pKVItem->FindKey("item_quality"))
	{
		SCHEMA_INIT_CHECK(
			GetItemSchema()->BGetItemQualityFromName(m_pKVItem->GetString("item_quality"), &m_nItemQuality),
			"Item definition %s: Undefined item_quality \"%s\"", GetDefinitionName(),
			m_pKVItem->GetString("item_quality"));
	}
	if(m_pKVItem->FindKey("forced_item_quality"))
	{
		SCHEMA_INIT_CHECK(GetItemSchema()->BGetItemQualityFromName(m_pKVItem->GetString("forced_item_quality"),
																   &m_nForcedItemQuality),
						  "Item definition %s: Undefined item_quality \"%s\"", GetDefinitionName(),
						  m_pKVItem->GetString("forced_item_quality"));
	}
#endif
	...
	{
		SCHEMA_INIT_CHECK(GetItemSchema()->BGetItemRarityFromName(m_pKVItem->GetString("item_rarity"), &m_nItemRarity),
						  "Item definition %s: Undefined item_rarity \"%s\"", GetDefinitionName(),
	...
	if(m_pKVItem->FindKey("item_series"))
	{
		...
		SCHEMA_INIT_CHECK(GetItemSchema()->BGetItemSeries(m_pKVItem->GetString("item_series"), &m_unItemSeries),
						  "Item definition %s: Undefined item_series \"%s\"", GetDefinitionName(),
	...
	if(pchExpiration && pchExpiration[0])
	{
		if(pchExpiration[0] == '!')
		{
			...
			SCHEMA_INIT_CHECK(m_rtExpiration != k_RTime32Nil,
							  "Unknown/malformed expiration_date string \"%s\" in item %s.", pchExpiration,
							  m_pszDefinitionName);
		}
		else
		{
			...
			SCHEMA_INIT_CHECK(
				...
				"Malformed expiration_date \"%s\" for expiration_date in item %s.  Must be of the form \"YYYY-MM-DD "
				"hh:mm:ss\".  Input: %s Output: %s InputTime: %u LocalTime: %u Timezone: %lu",
				pchExpiration, m_pszDefinitionName, pchExpiration, rtimeBuf, m_rtExpiration, CRTime::RTime32TimeCur(),
				timezone);
#else
			// Check that if we convert back to a string, we get the same value.  Emit an error, but don't fail in the
			// game code
			char rtimeBuf[k_RTimeRenderBufferSize];
...
#define daylight _daylight
#endif
				Assert(false);
				Warning("Malformed expiration_date \"%s\" for expiration_date in item %s.  Must be of the form "
						"\"YYYY-MM-DD hh:mm:ss\".  Input: %s Output: %s InputTime: %u LocalTime: %u Timezone: %lu\n",
						pchExpiration, m_pszDefinitionName, pchExpiration, rtimeBuf, m_rtExpiration,
						CRTime::RTime32TimeCur(), timezone);
			}
#endif
		}
	}
	...
	m_pszWorldDisplayModel =
		m_pKVItem->GetString("model_world", NULL); // Not the ideal method. c_models are better, but this is to solve a
												   // retrofit problem with the sticky launcher.
	...
	m_iDropType =
		StringFieldToInt(m_pKVItem->GetString("drop_type"), g_szDropTypeStrings, ARRAYSIZE(g_szDropTypeStrings));
	...
	if(pToolDataKV)
	{
		...
		m_pTool = GetItemSchema()->CreateEconToolImpl(pszType, pszUseString, pszUsageRestriction, usageCapabilities,
													  pToolUsageKV);
		SCHEMA_INIT_CHECK(m_pTool != NULL, "Unable to create tool implementation for '%s', of type '%s'.",
						  m_pKVItem->GetName(), pszType);
	}
	...
	if(pBundleDataKV)
	{
		...
		FOR_EACH_SUBKEY(pBundleDataKV, pKVCurItem)
		{
			...
			SCHEMA_INIT_CHECK(pItemDef != NULL, "Unable to find item definition '%s' for bundle '%s'.",
							  pKVCurItem->GetName(), m_pszDefinitionName);

			m_BundleInfo->vecItemDefs.AddToTail(pItemDef);
		}

		// Only check for pack bundle if the item is actually a bundle - note that we could do this programatically by
		// checking that all items in the bundle are flagged as a "pack item" - but for now the bundle needs to be
		// explicitly flagged as a pack bundle.
	...
	// item_set
	SCHEMA_INIT_CHECK((!m_pKVItem->GetString("item_set", NULL)),
					  "Item definition '%s' specifies deprecated \"item_set\" field. Items sets are now specified only "
					  "in the set itself, not on the definition.",
					  GetDefinitionName());

	const char *pszSetItemRemapDefIndexName = m_pKVItem->GetString("set_item_remap", NULL);
	if(pszSetItemRemapDefIndexName)
	{
		const CEconItemDefinition *pRemapItemDef =
			GetItemSchema()->GetItemDefinitionByName(pszSetItemRemapDefIndexName);
		m_unSetItemRemapDefIndex = pRemapItemDef ? pRemapItemDef->GetDefinitionIndex() : INVALID_ITEM_DEF_INDEX;

		SCHEMA_INIT_CHECK(m_unSetItemRemapDefIndex != INVALID_ITEM_DEF_INDEX,
						  "Unable to find set item remap definition '%s' for '%s'.", pszSetItemRemapDefIndexName,
						  GetDefinitionName());
		SCHEMA_INIT_CHECK(m_unSetItemRemapDefIndex != GetDefinitionIndex(),
						  "Unable to set set item remap for definition '%s' to itself.", GetDefinitionName());
	}
	else
	{
	...
	// Calculate our equip region mask.
	{
		...
		{
			for(KeyValues *pKVRegion = pKVMultiEquipRegions->GetFirstSubKey(); pKVRegion;
				pKVRegion = pKVRegion->GetNextKey())
			{
		...
		FOR_EACH_VEC(vecEquipRegionNames, i)
		{
			...
			SCHEMA_INIT_CHECK(unThisRegionMask != 0,
							  "Item definition %s: Unable to find equip region mask for region named \"%s\"",
							  GetDefinitionName(), vecEquipRegionNames[i]);
	...
	// Single-line static attribute parsing.
	{
		...
		if(pKVStaticAttrsKey)
		{
			FOR_EACH_SUBKEY(pKVStaticAttrsKey, pKVKey)
			{
				...
				SCHEMA_INIT_SUBSTEP(
					staticAttrib.BInitFromKV_SingleLine(GetDefinitionName(), pKVKey, pVecErrors, false));
...
#ifdef GC_DLL
	SCHEMA_INIT_SUBSTEP(BCommonInitPropertyGeneratorsFromKV(GetDefinitionName(), &m_vecPropertyGenerators,
															m_pKVItem->FindKey("property_generators"), pVecErrors));
	...
	if(pKVPaymentRules)
	{
		FOR_EACH_TRUE_SUBKEY(pKVPaymentRules, pKVRule)
		{
			...
			SCHEMA_INIT_CHECK(bFoundPaymentRule,
							  "Item definition '%s': payment rule %s didn't specify a known payment rule type",
			...
			{
				SCHEMA_INIT_CHECK(rule.m_RevenueShare > 0.0,
								  "Item definition '%s': payment rule %s has invalid revenue share %0.2f",
								  GetDefinitionName(), pKVRule->GetName(), rule.m_RevenueShare);

				// Ordinarily, bundles can only specify the "bundle_revenue_share" payment rule type. However, for
				// backwards compatability with previous payment rules that pre-date the Workshop and had manual
				// percentages set offline, we allow people who know what they're doing to specify the
				// "sanity_check_override" key and then use custom rules.
				SCHEMA_INIT_CHECK((rule.m_eRuleType == kPaymentRule_Bundle) == IsBundle(),
								  "Item definition '%s': payment rule %s has invalid bundle rules", GetDefinitionName(),
								  pKVRule->GetName());
			}

			KeyValues *pPaymentRuleForItemdef = pKVRule->FindKey("payment_rule_for_itemdef");
			SCHEMA_INIT_CHECK(pPaymentRuleForItemdef && (pPaymentRuleForItemdef->GetInt() == m_nDefIndex),
							  "Item definition '%s': payment rule %s has invalid payment_rule_for_itemdef",
							  GetDefinitionName(), pKVRule->GetName());

			KeyValues *pKVMultiTargets = pKVRule->FindKey("targets");
			KeyValues *pKVSingleTarget = pKVRule->FindKey("target");
			SCHEMA_INIT_CHECK(pKVMultiTargets == NULL || pKVSingleTarget == NULL,
							  "Item definition '%s': payment rule %s specifies both single- and multi-targets",
							  GetDefinitionName(), pKVRule->GetName());
			...
			SCHEMA_INIT_CHECK((rule.m_eRuleType == kPaymentRule_Bundle) == (rule.m_vecValues.Count() == 0),
							  "Item definition '%s': payment rule %s has invalid number of target entries %i",
							  GetDefinitionName(), pKVRule->GetName(), rule.m_vecValues.Count());
			...
			{
				SCHEMA_INIT_CHECK(m_vecPaymentRules.Count() == 0,
								  "Item definition '%s': only the first payment rule can be specified as 'bundle'",
								  GetDefinitionName());
				...
				SCHEMA_INIT_CHECK(GetBundleInfo() && GetBundleInfo()->vecItemDefs.Count() > 0,
								  "Item definition '%s': payment rule %s is specified as a bundle but outer item "
								  "definition has no bundle contents.\n",
								  GetDefinitionName(), pKVRule->GetName());

				// Bundles rely on sub-items for figuring out payment so the revenue share for the
				// bundle itself is expected to be 100%.
				SCHEMA_INIT_CHECK(rule.m_RevenueShare == 100.0,
								  "Item definition '%s': payment rule %s has invalid bundle revenue share %0.2f",
								  GetDefinitionName(), pKVRule->GetName(), rule.m_RevenueShare);
			}

			const bool bWasNumberedCorrectly = (AddPaymentRule(rule) == atoi(pKVRule->GetName()));
			SCHEMA_INIT_CHECK(bWasNumberedCorrectly, "Item definition '%s': misnumbered payment rule %s",
							  GetDefinitionName(), pKVRule->GetName());
		}
	}
#endif // GC_DLL
...
bool static_attrib_t::BInitFromKV_MultiLine(const char *pszContext, KeyValues *pKVAttribute,
											CUtlVector<CUtlString> *pVecErrors)
{
	const CEconItemAttributeDefinition *pAttrDef =
		GetItemSchema()->GetAttributeDefinitionByName(pKVAttribute->GetName());

	SCHEMA_INIT_CHECK(NULL != pAttrDef,
					  "Context '%s': Attribute \"%s\" in \"attributes\" did not match any attribute definitions",
					  pszContext, pKVAttribute->GetName());
	...
	if(pAttrDef)
	{
		...
		const bool bSuccessfullyLoadedValue =
			pAttrType->BConvertStringToEconAttributeValue(pAttrDef, pszValue, &m_value, true);

		SCHEMA_INIT_CHECK(bSuccessfullyLoadedValue, "Context '%s': Attribute \"%s\" could not parse value \"%s\"!",
						  pszContext, pKVAttribute->GetName(), pszValue ? pszValue : "(null)");

		SCHEMA_INIT_CHECK(!pAttrDef->BIsSetBonusAttribute(),
						  "Context '%s': Attribute \"%s\" is a set bonus attribute and not supported here", pszContext,
		...
		SCHEMA_INIT_CHECK(m_pKVCustomData == NULL || bForceGCToGenerate,
						  "Context '%s': Attribute \"%s\" is set to have custom logic but is not GC-generated so that "
						  "logic will never get used!",
						  pszContext, pKVAttribute->GetName());

		SCHEMA_INIT_CHECK(m_pKVCustomData != NULL || pKVAttribute->FindKey("value"),
		...
		SCHEMA_INIT_CHECK(
			...
			"Context '%s': Attribute \"%s\" custom logic data is set, but custom logic method is not set!", pszContext,
			pKVAttribute->GetName());
#endif // GC_DLL
	}
...
bool static_attrib_t::BInitFromKV_SingleLine(const char *pszContext, KeyValues *pKVAttribute,
											 CUtlVector<CUtlString> *pVecErrors,
											 bool bEnableTerribleBackwardsCompatibilitySchemaParsingCode /* = true */)
{
	const CEconItemAttributeDefinition *pAttrDef =
		GetItemSchema()->GetAttributeDefinitionByName(pKVAttribute->GetName());
	...
	if(pAttrDef)
	{
		...
		const bool bSuccessfullyLoadedValue = pAttrType->BConvertStringToEconAttributeValue(
...
bool CEconItemDefinition::BInitItemMappings(CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(m_pszArmoryRemap && m_pszArmoryRemap[0])
	{
		...
		SCHEMA_INIT_CHECK(pDef != NULL, "Item %s: Armory remap definition \"%s\" was not found", m_pszItemBaseName,
						  m_pszArmoryRemap);
	}
	...
	if(m_pszStoreRemap && m_pszStoreRemap[0])
	{
		...
		SCHEMA_INIT_CHECK(pDef != NULL, "Item %s: Store remap definition \"%s\" was not found", m_pszItemBaseName,
						  m_pszStoreRemap);
	}
...
#ifdef STAGING_ONLY
ConVar item_enable_dynamic_loading("item_enable_dynamic_loading", "1", FCVAR_REPLICATED,
...
bool CTimedItemRewardDefinition::BInitFromKV(KeyValues *pKVTimedReward, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	if(pszLootList && pszLootList[0])
	{
		...
		SCHEMA_INIT_CHECK(NULL != m_pLootList, "Time Reward %s: loot_list (%s) does not exist",
						  pKVTimedReward->GetName(), pszLootList);
	}

	// Other integrity checks
	SCHEMA_INIT_CHECK(m_flChance >= 0.0f, "Time Reward %s: pctChance (%f) must be greater or equal to 0.0",
					  pKVTimedReward->GetName(), m_flChance);

	SCHEMA_INIT_CHECK(m_flChance <= 1.0f, "Time Reward %s: pctChance (%f) must be less than or equal to 1.0",
					  pKVTimedReward->GetName(), m_flChance);

	SCHEMA_INIT_CHECK(pKVTimedReward->GetInt("value_min") > 0, "Time Reward %s: value_min (%d) must be greater than 0",
					  pKVTimedReward->GetName(), m_unMinFreq);
	SCHEMA_INIT_CHECK(pKVTimedReward->GetInt("value_max") > 0, "Time Reward %s: value_max (%d) must be greater than 0",
					  pKVTimedReward->GetName(), m_unMaxFreq);
	SCHEMA_INIT_CHECK((m_unMaxFreq >= m_unMinFreq),
					  "Time Reward %s: value_max (%d) must be greater than or equal to value_min (%d)",
					  pKVTimedReward->GetName(), m_unMaxFreq, m_unMinFreq);
...
// Purpose:
//-----------------------------------------------------------------------------
IEconTool *CEconItemSchema::CreateEconToolImpl(const char *pszToolType, const char *pszUseString,
											   const char *pszUsageRestriction, item_capabilities_t unCapabilities,
											   KeyValues *pUsageKV)
{
	if(pszToolType)
	{
		if(!V_stricmp(pszToolType, "duel_minigame"))
		{
			...
			if(pszUsageRestriction)
				return NULL;
			if(unCapabilities != ITEM_CAP_NONE)
				return NULL;
			if(pUsageKV)
				return NULL;
...
#ifdef GC_DLL
random_attrib_t *CEconItemSchema::CreateRandomAttribute(const char *pszContext, KeyValues *pRandomAttributesKV,
														CUtlVector<CUtlString> *pVecErrors /*= NULL*/)
{
	// We've found the random attribute block. Parse it.
		...
		if(!Q_strcmp(pszName, "is_even_chance_attr"))
		{
			FOR_EACH_VALUE(pKVAttribute, pKVListItem)
			{
				...
				if(!lootListAttrib.m_staticAttrib.BInitFromKV_SingleLine(__FUNCTION__, pKVListItem, pVecErrors, false))
				{
					if(pVecErrors)
					{
						pVecErrors->AddToTail(__FUNCTION__ ": error initializing line-item attribute from lootlist "
														   "definition (possible attr template).\n");
					}
					return NULL;
				}
...
bool CheckValveSignature(const void *data, uint32 nDataSize, const void *signature, uint32 nSignatureSize)
{
	...
	static const unsigned char valvePublicKey[] = "\x30\x81\x9D\x30\x0D\x06\x09\x2A\x86\x48\x86\xF7\x0D\x01\x01\x01"
	...
	// code gets detoured
	g_bLastSignatureCheck = CCrypto::RSAVerifySignatureSHA256((const uint8 *)data, nDataSize, (const uint8 *)signature,
															  nSignatureSize, valvePublicKey, sizeof(valvePublicKey));
...
// Do we need to check the signature?
#if defined(TF_DLL) || defined(TF_CLIENT_DLL)
	{
		...
		CUtlString sSignatureFilename(fileName);
		...
		bool bSignatureValid = CheckValveSignature(bufRawData.Base(), bufRawData.TellPut(), bufSignatureBinary.Base(),
												   bufSignatureBinary.TellPut());
		...
		if(!bSignatureValid)
		{
			bSignatureValid = CheckValveSignature("", 0, bufSignatureBinary.Base(), bufSignatureBinary.TellPut());
		}

		SCHEMA_INIT_CHECK(bSignatureValid,
						  "'%s' is corrupt.  Please verify your local game files.  "
...
bool CEconItemSchema::DoPostPriceSheetLoadInit(CEconStorePriceSheet *pPriceSheet)
{
	FOR_EACH_MAP_FAST(m_mapItems, iItem)
	{
		...
		FOR_EACH_VEC(m_vecBundles, iBundle)
		{
			...
			const bool bBundleItemIsForSale =
				pPriceSheet->BItemExistsInPriceSheet(pBundleItemDef->GetDefinitionIndex()) !=
				NULL; // Only add bundles that are actually for sale

			bool bAddToContainingBundleItemDefs = false;
			if(pItemDef->IsPackBundle())
			{
				// If the current item is a pack bundle, look for the first pack item in the current bundle (pBundle).
				// We can safely assume that all pack items will be in pBundle if the first pack item is, since the GC
				// won't startup otherwise. Don't add self as a containing bundle.
				bAddToContainingBundleItemDefs =
					pItemDef->GetDefinitionIndex() != pBundleItemDef->GetDefinitionIndex() &&
...
bool CEconItemSchema::BInitSchema(KeyValues *pKVRawDefinition, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	SCHEMA_INIT_CHECK(m_unMinLevel <= m_unMaxLevel,
					  "Minimum Item Level must be less than or equal to Maximum Item Level");
#endif // GC_DLL
	...
	SCHEMA_INIT_CHECK(pPaintkitKV->LoadFromFile(g_pFullFileSystem, "scripts/items/paintkits_master.txt", "GAME"),
	...
	// Init Item Collections - Must be before lootlists since collections are lootlists themselves and are referenced by
	// lootlists
...
bool CEconItemSchema::BInitGameInfo(KeyValues *pKVGameInfo, CUtlVector<CUtlString> *pVecErrors)
{
	...
	SCHEMA_INIT_CHECK(m_unFirstValidClass <= m_unLastValidClass,
	...
	SCHEMA_INIT_CHECK(m_unAccoutClassIndex > m_unLastValidClass,
	...
	SCHEMA_INIT_CHECK(m_unFirstValidClassItemSlot <= m_unLastValidClassItemSlot,
	...
	SCHEMA_INIT_CHECK(m_unFirstValidAccountItemSlot <= m_unLastValidAccountItemSlot,
...
bool CEconItemSchema::BInitAttributeTypes(CUtlVector<CUtlString> *pVecErrors)
{
	...
	m_vecAttributeTypes.AddToTail(attr_type_t("dynamic_recipe_component_defined_item",
											  new CSchemaAttributeType_DynamicRecipeComponentDefinedItem));
	...
	FOR_EACH_VEC(m_vecAttributeTypes, i)
	{
		...
		SCHEMA_INIT_CHECK(Column0.GetType() == k_EGCSQLType_int64,
						  "BInitAttributeTypes(): '%s' column 0 has invalid data type %u.\n",
						  cs.GetRecordInfo()->GetName(), Column0.GetType());
		SCHEMA_INIT_CHECK(Column0.GetName() && !V_stricmp(Column0.GetName(), "ItemID"),
						  "BInitAttributeTypes(): '%s' has invalid name '%s'.\n", cs.GetRecordInfo()->GetName(),
						  Column0.GetName() ? Column0.GetName() : "[null]");
		SCHEMA_INIT_CHECK(Column0.BIsPrimaryKey(),
						  "BInitAttributeTypes(): '%s' has an item ID column that isn't in the PK.\n",
						  cs.GetRecordInfo()->GetName());

		const CColumnInfo &Column1 = cs.GetColumnInfo(1);
		SCHEMA_INIT_CHECK(Column1.GetType() == k_EGCSQLType_int16,
						  "BInitAttributeTypes(): '%s' column 1 has invalid data type %u.\n",
						  cs.GetRecordInfo()->GetName(), Column0.GetType());
		SCHEMA_INIT_CHECK(Column1.GetName() && !V_stricmp(Column1.GetName(), "AttrDefIndex"),
						  "BInitAttributeTypes(): '%s' has invalid name '%s'.\n", cs.GetRecordInfo()->GetName(),
						  Column1.GetName() ? Column1.GetName() : "[null]");
		...
		{
			SCHEMA_INIT_CHECK(cs.GetRecordInfo() !=
								  m_vecAttributeTypes[j].m_pAttrType->GetFullColumnSet().GetRecordInfo(),
...
//-----------------------------------------------------------------------------
struct periodic_score_event_lookup_entry_t
{
	const char *m_pszName;
	eEconPeriodicScoreEvents m_eValue;
...
struct periodic_score_duration_lookup_entry_t
{
	const char *m_pszName;
	uint32 m_unValue;
};
...
static bool LookupValueFromString(const search_entry_type (&searchArray)[search_entry_array_size],
								  const char *pszSearch, search_entry_type *out_pResult)
{
	Assert(out_pResult);
...
bool CEconItemSchema::BInitPeriodicScoring(KeyValues *pKVPeriodicScoring, CUtlVector<CUtlString> *pVecErrors)
{
	FOR_EACH_TRUE_SUBKEY(pKVPeriodicScoring, pKVScoreType)
	{
		...
		SCHEMA_INIT_CHECK(index == m_vecPeriodicScoreTypes.Count(), "Invalid or out-of-order periodic score type '%s'",
						  pKVScoreType->GetName());
		...
		PeriodicScore.m_pRewardItemDefinition =
			pszRewardItemDefName ? GetItemDefinitionByName(pszRewardItemDefName) : NULL;
		SCHEMA_INIT_CHECK(PeriodicScore.m_pRewardItemDefinition,
						  "Periodic score type '%s' missing reward item definition name", pKVScoreType->GetName());
		...
		const char *pszEventName = pKVScoreType->GetString("event", "");
		{
			...
			SCHEMA_INIT_CHECK(LookupValueFromString(sPeriodicScoreEvents, pszEventName, &EventEntry),
							  "Periodic score type '%s' could not find event name '%s'", pKVScoreType->GetName(),
							  pszEventName);
			PeriodicScore.m_eEventType = EventEntry.m_eValue;

			// Note: other parts of the code assume that the event type is associated with the GC-only updatability
			// flag.)
		...
		// Time period via string lookup.
		{
			...
			SCHEMA_INIT_CHECK(LookupValueFromString(sPeriodicScoreDurations, pszTimePeriodName, &DurationEntry),
							  "Periodic score type '%s' could not find time period name '%s'", pKVScoreType->GetName(),
							  pszEventName);
		...
		if(GGCHost()->GetUniverse() != k_EUniversePublic)
		{
			...
			if(pszInternalTimePeriodName)
			{
				...
				SCHEMA_INIT_CHECK(
					LookupValueFromString(sPeriodicScoreDurations, pszInternalTimePeriodName, &InternalDurationEntry),
					"Periodic score type '%s' could not find internal time period name '%s'", pKVScoreType->GetName(),
					pszEventName);
...
bool CEconItemSchema::BInitDefinitionPrefabs(KeyValues *pKVPrefabs, CUtlVector<CUtlString> *pVecErrors)
{
	FOR_EACH_TRUE_SUBKEY(pKVPrefabs, pKVPrefab)
	{
		...
		// Make sure the item index is correct because we use this index as a reference
		SCHEMA_INIT_CHECK(!m_mapDefinitionPrefabs.IsValidIndex(nMapIndex), "Duplicate prefab name (%s)", pszPrefabName);
...
bool CEconItemSchema::BInitItemSeries(KeyValues *pKVSeries, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVSeries)
	{
		FOR_EACH_TRUE_SUBKEY(pKVSeries, pKVItem)
		{
			...
			// Make sure the item index is correct because we use this index as a reference
			SCHEMA_INIT_CHECK(!m_mapItemSeries.IsValidIndex(nMapIndex), "Duplicate item series value (%d)",
							  nSeriesIndex);
...
//-----------------------------------------------------------------------------
bool CEconItemSchema::BInitRarities(KeyValues *pKVRarities, KeyValues *pKVRarityWeights,
									CUtlVector<CUtlString> *pVecErrors)
{
	// initialize the item definitions
	if(NULL != pKVRarities)
	{
		FOR_EACH_TRUE_SUBKEY(pKVRarities, pKVRarity)
		{
			...
			// Make sure the item index is correct because we use this index as a reference
			SCHEMA_INIT_CHECK(!m_mapRarities.IsValidIndex(nMapIndex), "Duplicate rarity value (%d)", nRarityIndex);
...
bool CEconItemSchema::BInitQualities(KeyValues *pKVQualities, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVQualities)
	{
		FOR_EACH_TRUE_SUBKEY(pKVQualities, pKVQuality)
		{
			...
			// Make sure the item index is correct because we use this index as a reference
			SCHEMA_INIT_CHECK(!m_mapQualities.IsValidIndex(nMapIndex), "Duplicate quality value (%d)", nQualityIndex);
	...
	FOR_EACH_MAP_FAST(m_mapQualities, i)
	{
		...
		SCHEMA_INIT_CHECK(!rbQualityNames.IsValidIndex(iIndex), "Quality definition %d: Duplicate quality name %s",
						  m_mapQualities[i].GetDBValue(), m_mapQualities[i].GetName());
...
void CEconItemSchema::AssignDefaultBodygroupState(const char *pszBodygroupName, int iValue)
{
	...
	if((m_mapDefaultBodygroupState.IsValidIndex(iIndex) && m_mapDefaultBodygroupState[iIndex] != iDefaultValue) ||
	   (iValue < 0 || iValue > 1))
	{
		EmitWarning(SPEW_GC, 4,
					"Unable to get accurate read on whether bodygroup '%s' is enabled or disabled by default. (The "
					"schema is fine, but the code is confused and could stand to be made smarter.)\n",
					pszBodygroupName);
	}
...
bool CEconItemSchema::BInitEquipRegions(KeyValues *pKVEquipRegions, CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_SUBKEY(pKVEquipRegions, pKVRegion)
	{
		...
		unsigned int unNewBitIndex =
			m_vecEquipRegionsList.Count() <= 0 ? 0 : m_vecEquipRegionsList.Tail().m_unBitIndex + 1;
...
bool CEconItemSchema::BInitEquipRegionConflicts(KeyValues *pKVEquipRegionConflicts, CUtlVector<CUtlString> *pVecErrors)
{
	FOR_EACH_TRUE_SUBKEY(pKVEquipRegionConflicts, pKVConflict)
	{
		...
		if(iRegionIdx < 0)
		{
			pVecErrors->AddToTail(
				CFmtStr("Unable to find base equip region named \"%s\" for conflicts.", pRegionName).Access());
			continue;
		}
		...
		FOR_EACH_SUBKEY(pKVConflict, pKVConflictOther)
		{
			...
			{
				pVecErrors->AddToTail(
					CFmtStr("Unable to find other equip region named \"%s\" for conflicts.", pOtherRegionName)
						.Access());
				continue;
			}
...
bool CEconItemSchema::BInitAttributes(KeyValues *pKVAttributes, CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_TRUE_SUBKEY(pKVAttributes, pKVAttribute)
	{
		...
		SCHEMA_INIT_CHECK(nAttrIndex >= 0, "Attribute definition index %d must be greater than or equal to zero",
						  nAttrIndex);

		// Make sure the attribute index is not repeated
		SCHEMA_INIT_CHECK(!m_mapAttributes.IsValidIndex(nMapIndex), "Duplicate attribute definition index (%d)",
						  nAttrIndex);
	...
	FOR_EACH_MAP_FAST(m_mapAttributes, i)
	{
		...
		SCHEMA_INIT_CHECK(!rbAttributeNames.IsValidIndex(iIndex), "Attribute definition %d: Duplicate name \"%s\"",
						  m_mapAttributes.Key(i), m_mapAttributes[i].GetDefinitionName());
...
bool CEconItemSchema::BInitItems(KeyValues *pKVItems, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVItems)
	{
		FOR_EACH_TRUE_SUBKEY(pKVItems, pKVItem)
		{
			...
			{
#if defined(CLIENT_DLL) || defined(GAME_DLL)
				SCHEMA_INIT_CHECK(m_pDefaultItemDefinition == NULL, "Duplicate 'default' item definition.");
			...
			else
			{
				...
				// Make sure the item index is correct because we use this index as a reference
				SCHEMA_INIT_CHECK(!m_mapItems.IsValidIndex(nMapIndex), "Duplicate item definition (%d)", nItemIndex);

				// Check to make sure the index is positive
				SCHEMA_INIT_CHECK(nItemIndex >= 0, "Item definition index %d must be greater than or equal to zero",
								  nItemIndex);
				...
				{
					// Cache off the item def for the bundle, since we'll need both the bundle info and the item def
					// index later.
					m_vecBundles.AddToTail(pItemDef);

					// If the bundle is a pack bundle, mark all the contained items as pack items / link to the owning
					// pack bundle
					if(pItemDef->IsPackBundle())
					{
						...
						FOR_EACH_VEC(pBundleInfo->vecItemDefs, iCurItem)
						{
							...
							SCHEMA_INIT_CHECK(NULL == pCurItemDef->m_pOwningPackBundle,
											  "Pack item \"%s\" included in more than one pack bundle - not allowed!",
											  pCurItemDef->GetDefinitionName());
	...
	FOR_EACH_MAP_FAST(m_mapItems, i)
	{
		...
		SCHEMA_INIT_CHECK(!rbItemNames.IsValidIndex(iIndex), "Item definition %s: Duplicate name on index %d",
						  pItemDef->GetDefinitionName(), m_mapItems.Key(i));
...
#ifdef DOTA
	// Go through all regular (ie non-pack) bundles and ensure that if any pack items are included, *all* pack items in
	// the owning pack bundle are included
	FOR_EACH_VEC(m_vecBundles, iBundle)
	{
		const CEconItemDefinition *pBundleItemDef = m_vecBundles[iBundle];
				...
				FOR_EACH_VEC(pOwningPackBundle->vecItemDefs, iCurPackBundleItem)
				{
					...
					if(!pBundle->vecItemDefs.HasElement(pCurPackBundleItem))
					{
						SCHEMA_INIT_CHECK(false,
										  "The bundle \"%s\" contains some, but not all pack items required specified "
										  "by pack bundle \"%s.\"",
...
bool CEconItemSchema::BInitItemSets(KeyValues *pKVItemSets, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVItemSets)
	{
		FOR_EACH_TRUE_SUBKEY(pKVItemSets, pKVItemSet)
		{
			...
			SCHEMA_INIT_CHECK(m_mapItemSets.Find(setName) == m_mapItemSets.InvalidIndex(),
							  "Duplicate itemset name (%s) found!", setName);
		...
		FOR_EACH_MAP_FAST(m_mapItems, i)
		{
			...
			AssertMsg(pRemappedSetItemDef,
					  "Somehow got through item and set initialization but have a broken set remap item!");
...
bool CEconItemSchema::BInitItemCollections(KeyValues *pKVItemCollections, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVItemCollections)
	{
		FOR_EACH_TRUE_SUBKEY(pKVItemCollections, pKVItemCollection)
		{
			...
			SCHEMA_INIT_CHECK(m_mapItemCollections.Find(setName) == m_mapItemCollections.InvalidIndex(),
							  "Duplicate item collection name (%s) found!", setName);
...
bool CEconItemSchema::BInitCollectionReferences(CUtlVector<CUtlString> *pVecErrors)
{
	FOR_EACH_MAP_FAST(m_mapItems, i)
	{
		...
		if(pszCollectionName)
		{
			...
			SCHEMA_INIT_CHECK(bFound == true, "Collection %s referenced by item %s not found", pszCollectionName,
...
bool CEconItemSchema::BInitItemPaintKitDefinitions(KeyValues *pKVItemPaintKits, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVItemPaintKits)
	{
		...
		SCHEMA_INIT_CHECK(materials->VerifyTextureCompositorTemplates(),
						  "Paintkit template post-init validation failed.");
#endif
		...
		FOR_EACH_TRUE_SUBKEY(pKVItemPaintKits, pKVPaintKit)
		{
			...
			if(V_stristr(keyField, "paintkit_template") == NULL)
			{
				SCHEMA_INIT_CHECK(m_mapItemPaintKits.Find(keyField) == m_mapItemPaintKits.InvalidIndex(),
								  "Duplicate paint kit definition name (%s) found!", keyField);
...
//-----------------------------------------------------------------------------
bool CEconItemSchema::BInitOperationDefinitions(KeyValues *pKVGameInfo, KeyValues *pKVOperationDefinitions,
												CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVOperationDefinitions)
	{
		FOR_EACH_TRUE_SUBKEY(pKVOperationDefinitions, pKVOperation)
		{
			...
			SCHEMA_INIT_CHECK(m_mapOperationDefinitions.Find(setName) == m_mapOperationDefinitions.InvalidIndex(),
							  "Duplicate operation definition name (%s) found!", setName);
...
//-----------------------------------------------------------------------------
bool CEconItemSchema::AddQuestObjective(const CQuestObjectiveDefinition **ppQuestObjective, KeyValues *pKVObjective,
										CUtlVector<CUtlString> *pVecErrors)
{
	...
	SCHEMA_INIT_CHECK(nMapIndex == m_mapQuestObjectives.InvalidIndex(), "Multiple quest objectives with defindex: %d",
					  nDefIndex);
...
//			in from a list.
//-----------------------------------------------------------------------------
bool CEconItemSchema::BVerifyLootListItemDropDates(const CEconLootListDefinition *pLootList,
												   CUtlVector<CUtlString> *pVecErrors) const
{
...
//-----------------------------------------------------------------------------
bool CEconItemSchema::BRecurseiveVerifyLootListItemDropDates(const CEconLootListDefinition *pLootList,
															 const CEconLootListDefinition *pRootLootList,
															 CUtlVector<CUtlString> *pVecErrors) const
{
	FOR_EACH_VEC(pLootList->GetLootListContents(), j)
	{
		...
		if(item.m_iItemOrLootlistDef >= 0)
		{
			...
			if(pItemDef)
			{
				...
				// Check for start drop date attribute on this item
				SCHEMA_INIT_CHECK(
					!FindAttribute(pItemDef, pAttribDef_StartDropDate, &value),
					"Lootlist \"%s\" contains lootlist \"%s\", which contains item \"%s\", which has start drop date.",
					pRootLootList->GetName(), pLootList->GetName(), pItemDef->GetDefinitionName());
				// Check for end drop date attribute on this item
				SCHEMA_INIT_CHECK(
					!FindAttribute(pItemDef, pAttribDef_EndDropDate, &value),
...
bool CEconItemSchema::BInitRecipes(KeyValues *pKVRecipes, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVRecipes)
	{
		FOR_EACH_TRUE_SUBKEY(pKVRecipes, pKVRecipe)
		{
			...
			SCHEMA_INIT_CHECK(!m_mapRecipes.IsValidIndex(nMapIndex), "Duplicate recipe definition (%d)", nRecipeIndex);

			// Check to make sure the index is positive
			SCHEMA_INIT_CHECK(nRecipeIndex >= 0, "Recipe definition index %d must be greater than or equal to zero",
							  nRecipeIndex);
		...
		FOR_EACH_SUBKEY(pKVAchievementRewards, pKVReward)
		{
			...
			if(pKVReward->GetDataType() == KeyValues::TYPE_NONE)
			{
				...
				else
				{
					...
					SCHEMA_INIT_CHECK(pkvItems != NULL,
			...
			FOR_EACH_VEC(award.m_vecDefIndex, nItem)
			{
				...
				SCHEMA_INIT_CHECK(pDefn != NULL, "Item definition index %d in achievement reward %s was not found",
								  award.m_vecDefIndex[nItem], pKVReward->GetName());
				if(!pDefn)
				{
			...
			SCHEMA_INIT_CHECK(award.m_vecDefIndex.Count() > 0, "Achievement reward %s has no items!",
							  pKVReward->GetName());
			...
			SCHEMA_INIT_CHECK(award.m_unSourceAppId >= 0, "Source App ID %d in achievement reward %s must be valid",
							  award.m_unSourceAppId, pKVReward->GetName());
			...
			if(!GGCGameBase()->BYieldingLoadStats(award.m_unSourceAppId))
			{
				...
				{
					SCHEMA_INIT_CHECK(false, "Unable to load stats schema for cross-game achievement %s for app %d",
									  pKVReward->GetName(), award.m_unSourceAppId);
				}
				continue;
			}
			...
			if(!pStatsSchema)
			{
				SCHEMA_INIT_CHECK(false, "Unable to retrieve stats schema for cross-game achievement %s for app %d",
								  pKVReward->GetName(), award.m_unSourceAppId);
				continue;
			}
			...
			if(award.m_unAuditData == 0)
			{
				...
				{
					SCHEMA_INIT_CHECK(false, "Unable to find achievement %s for app %d", award.m_sNativeName.Get(),
									  award.m_unSourceAppId);
					continue;
				}
			...
			*pAward = award;
			...
			m_dictAchievementRewards.Insert(ComputeAchievementName(pAward->m_unSourceAppId, pAward->m_sNativeName),
											pAward);
...
#ifdef GC_DLL
bool CEconItemSchema::BInitRandomAttributeTemplates(KeyValues *pKVRandomAttributeTemplates,
													CUtlVector<CUtlString> *pVecErrors)
{
	m_dictRandomAttributeTemplates.PurgeAndDeleteElements();
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEconItemSchema::BInitSteamPackageLocalizationToken(KeyValues *pKVSteamPackages,
														 CUtlVector<CUtlString> *pVecErrors)
{
	if(NULL != pKVSteamPackages)
	{
		FOR_EACH_TRUE_SUBKEY(pKVSteamPackages, pKVEntry)
		{
			...
			SCHEMA_INIT_CHECK(iRawPackageId > 0, "Invalid package ID %i for localization", iRawPackageId);
...
static const char *s_particle_controlpoint_names[] = {
	"attachment",	   "control_point_1", "control_point_2", "control_point_3",
	"control_point_4", "control_point_5", "control_point_6",
};

//-----------------------------------------------------------------------------
// Purpose:	Initializes the attribute-controlled-particle-systems section of the schema
//-----------------------------------------------------------------------------
bool CEconItemSchema::BInitAttributeControlledParticleSystems(KeyValues *pKVParticleSystems,
															  CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVParticleSystems)
	{
		FOR_EACH_TRUE_SUBKEY(pKVParticleSystems, pKVCategory)
		{
			...
			FOR_EACH_TRUE_SUBKEY(pKVCategory, pKVEntry)
			{
				...
				// Check to make sure the index is positive
				SCHEMA_INIT_CHECK(nItemIndex > 0, "Particle system index %d greater than zero", nItemIndex);
				if(nItemIndex <= 0)
					continue;
...
bool CEconItemSchema::BInitStringTables(KeyValues *pKVStringTables, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(NULL != pKVStringTables)
	{
		FOR_EACH_SUBKEY(pKVStringTables, pKVTable)
		{
			...
			SchemaStringTableDict_t::IndexType_t i =
				m_dictStringTable.Insert(pKVTable->GetName(), new CUtlVector<schema_string_table_entry_t>);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEconItemSchema::BInitCommunityMarketRemaps(KeyValues *pKVCommunityMarketRemaps,
												 CUtlVector<CUtlString> *pVecErrors)
{
	m_mapCommunityMarketDefinitionIndexRemap.Purge();
	...
	if(NULL != pKVCommunityMarketRemaps)
	{
		FOR_EACH_SUBKEY(pKVCommunityMarketRemaps, pKVRemapBase)
		{
			...
			FOR_EACH_SUBKEY(pKVRemapBase, pKVRemap)
			{
				...
				SCHEMA_INIT_CHECK(pItemDef != NULL, "Unknown Market remap definition '%s' (under '%s').", pszDefName,
								  pszBaseDefName);
				SCHEMA_INIT_CHECK(m_mapCommunityMarketDefinitionIndexRemap.Find(pItemDef->GetDefinitionIndex()) ==
									  m_mapCommunityMarketDefinitionIndexRemap.InvalidIndex(),
								  "Duplicate Market remap definition '%s'.\n", pszDefName);

				m_mapCommunityMarketDefinitionIndexRemap.Insert(pItemDef->GetDefinitionIndex(),
																pBaseItemDef->GetDefinitionIndex());
			}
		}
	}
...
// Purpose:
//-----------------------------------------------------------------------------
item_definition_index_t CEconItemSchema::GetCommunityMarketRemappedDefinitionIndex(
	item_definition_index_t unSearchItemDef) const
{
	CommunityMarketDefinitionRemapMap_t::IndexType_t index =
		m_mapCommunityMarketDefinitionIndexRemap.Find(unSearchItemDef);
...
//-----------------------------------------------------------------------------
const CItemLevelingDefinition *CEconItemSchema::GetItemLevelForScore(const char *pszLevelBlockName,
																	 uint32 unScore) const
{
...
bool CEconItemSchema::GetKillEaterScoreTypeAllowsBotVictims(uint32 unScoreType) const
{
	...
	return pScoreType ? pScoreType->m_bAllowBotVictims : false;
}
...
bool CEconItemSchema::GetKillEaterScoreTypeGCOnlyUpdate(uint32 unScoreType) const
{
	...
	return pScoreType ? pScoreType->m_bGCUpdateOnly : true; // default to being more restrictive
}
...
bool CEconItemSchema::GetKillEaterScoreTypeAllowsIncrementValues(uint32 unScoreType) const
{
	...
	return pScoreType ? pScoreType->m_AllowIncrementValues : true; // default to being more restrictive
}
...
bool CEconItemSchema::BCanStrangeFilterApplyToStrangeSlotInItem(
	uint32 /*strange_event_restriction_t*/ unRestrictionType, uint32 unRestrictionValue,
	const IEconItemInterface *pItem, int iStrangeSlot, uint32 *out_pOptionalScoreType) const
{
...
CExperimentDefinition::CExperimentDefinition(void)
	: m_bEnabled(false), m_unExperimentID(0), m_unNumParticipants(0), m_unMaxParticipants(0), m_pKeyValues(NULL)
{
}
...
static bool BTestToolApplicability(CUtlVector<CUtlString> *pVecErrors)
{
	static CSchemaItemDefHandle pItem_TeamPaint("Paint Can Team Color"), // tools
		pItem_Paint("Paint Can 14"), pItem_DescriptionTag("Description Tag"), pItem_NameTag("Name Tag"),
		pItem_Key("Decoder Ring"), pItem_SummerKey("Summer Key"), pItem_GiftWrap("Gift Wrap"),
		pItem_CustomTextureTool("Customize Texture Tool"),
		pItem_SupplyCrateGeneric("Supply Crate 2"), // tool targets
		pItem_SummerCrate("Summer Crate"), pItem_PaintableItem("Summer Hat"),
		// pItem_TeamPaintableItem	( "" ),
		pItem_UnpaintableItem("Big Steel Jaw of Summer Fun"),
		// pItem_UnnameableWeapon	( "" ),
		pItem_NameableWeapon("The Axtinguisher"), pItem_GiftWrappableItem("Supply Crate 3"),
		pItem_NonGiftWrappableItem("Wrapped Gift"), pItem_StampableObject("The Conscientious Objector"),
	...
	for(int i = 0; i < ARRAYSIZE(definitionTests); i++)
	{
		const GameItemDefinition_t *pToolDef = dynamic_cast<const GameItemDefinition_t *>(definitionTests[i].m_pTool),
								   *pTargetDef =
									   dynamic_cast<const GameItemDefinition_t *>(definitionTests[i].m_pTarget);
		...
		if(CEconSharedToolSupport::ToolCanApplyToDefinition(pToolDef, pTargetDef) !=
		   definitionTests[i].m_bExpectedValidity)
		{
			bAllSuccess = false;
			pVecErrors->AddToTail(CFmtStr("Tool validity test %i failed: %s %s have been able to apply to %s.", i,
										  pToolDef->GetDefinitionName(),
										  definitionTests[i].m_bExpectedValidity ? "should" : "shouldn't",
										  pTargetDef->GetDefinitionName())
									  .Access());
		}
	}
	...
	if(bAllSuccess)
	{
		for(int i = 0; i < ARRAYSIZE(interfaceTests); i++)
		{
			const GameItemDefinition_t *pToolDef =
										   dynamic_cast<const GameItemDefinition_t *>(interfaceTests[i].m_pTool),
									   *pTargetDef =
										   dynamic_cast<const GameItemDefinition_t *>(interfaceTests[i].m_pTarget);
			...
			CEconItem *pTool = GEconManager()->GetItemFactory().CreateSpecificItem((const CEconGameAccount *)NULL,
																				   pToolDef->GetDefinitionIndex());
			CEconItem *pTarget = GEconManager()->GetItemFactory().CreateSpecificItem((const CEconGameAccount *)NULL,
																					 pTargetDef->GetDefinitionIndex());
...
bool CEconItemSchema::BPostSchemaInit(CUtlVector<CUtlString> *pVecErrors) const
{
	...
	FOR_EACH_MAP_FAST(m_mapItems, i)
	{
		...
		if(pTool && !const_cast<IEconTool *>(pTool)->BFinishInitialization())
		{
			...
			bAllSuccess = false;
			pVecErrors->AddToTail(
				CFmtStr("BPostSchemaInit(): tool '%s' is invalid.", pItemDef->GetDefinitionName()).Get());
#endif // GC_DLL
		}
#if TF_GC_DLL
		else
			...
			if(pTFItemDef)
			{
				   ...
				   (nSlot == LOADOUT_POSITION_MISC2))
				{
					...
					FOR_EACH_VEC(pTFItemDef->GetStaticAttributes(), iIndex)
					{
						...
						const CEconItemAttributeDefinition *pAttrDef =
							GetItemSchema()->GetAttributeDefinition(staticAttrib.iDefIndex);
					...
					if(!bFoundScore || !bFoundKill)
					{
						bAllSuccess = false;
						pVecErrors->AddToTail(
							CFmtStr("BPostSchemaInit(): '%s' is missing the standard cosmetic killeater attributes.",
									pItemDef->GetDefinitionName())
								.Get());
					}
				}
	...
	else
	{
		...
		FOR_EACH_VEC(contents, i)
		{
			if(contents[i].m_iItemOrLootlistDef > 0)
			{
				...
				{
					bAllSuccess = false;
					pVecErrors->AddToTail(CFmtStr("Item \"%s\" is in all_particle_hats, but doesn't have equip region "
												  "hat or whole_head, meaning it can't become unusual.  REMOVE IT!",
												  pItemDef->GetDefinitionName())
											  .Get());
				}
			}
...
//
//-----------------------------------------------------------------------------
bool CItemLevelingDefinition::BInitFromKV(KeyValues *pKVItemLevel, const char *pszLevelBlockName,
										  CUtlVector<CUtlString> *pVecErrors)
{
	m_unLevel = Q_atoi(pKVItemLevel->GetName());
	m_unRequiredScore = pKVItemLevel->GetInt("score");
	m_pszLocalizedName_LocalStorage =
		strdup(pKVItemLevel->GetString("rank_name", CFmtStr("%s%i", pszLevelBlockName, m_unLevel).Access()));
