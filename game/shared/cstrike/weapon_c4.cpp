...
SendPropBool(SENDINFO(m_bBombTicking)), SendPropFloat(SENDINFO(m_flC4Blow), 0, SPROP_NOSCALE),
		...
		if(!m_pBombDefuser->IsDead())
		{
			...
			if(event)
			{
				...
				if((timeToDetonation > 0.0f) &&
				   (timeToDetonation <= AchievementConsts::BombDefuseCloseCall_MaxTimeRemaining))
				{
					// Give achievement for defusing with < 1 second before detonation
					m_pBombDefuser->AwardAchievement(CSBombDefuseCloseCall);
				}

				if((timeToDetonation > 0.0f) && (m_pBombDefuser->HasDefuser()) &&
				   (timeToDetonation < AchievementConsts::BombDefuseNeededKit_MaxTime))
				{
...
void CPlantedC4::Explode(trace_t *pTrace, int bitsDamageType)
{
...

	{
		...
		te->Explosion(filter, 0.0, &pos, g_sModelIndexFireball, 50.0, 25, TE_EXPLFLAG_NONE, flBombRadius * 3.5, 200);
	}
	...
	CSGameRules()->RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), flBombRadius, bitsDamageType), GetAbsOrigin(),
...
#ifdef CLIENT_DLL
	RecvPropBool(RECVINFO(m_bStartedArming)), RecvPropBool(RECVINFO(m_bBombPlacedAnimation)),
		RecvPropFloat(RECVINFO(m_fArmedTime))
#else
	SendPropBool(SENDINFO(m_bStartedArming)), SendPropBool(SENDINFO(m_bBombPlacedAnimation)),
...
bool CC4::OnFireEvent(C_BaseViewModel *pViewModel, const Vector &origin, const QAngle &angles, int event,
					  const char *options)
{
	if(event == 7001)
	{
...
void CC4::PrimaryAttack()
{
	...
	if(groundEntity)
	{
		...
		if(groundEntity->IsPlayer() || IsPushableEntity(groundEntity) ||
#ifndef CLIENT_DLL
	...
	if(bArmingTimeSatisfied && m_bStartedArming)
	{
		...
		if(pPlayer->m_bInBombZone)
		{
			...
			if(pC4)
			{
				...
				trace_t tr;
				UTIL_TraceEntity(pC4, GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -200), MASK_SOLID, this,
