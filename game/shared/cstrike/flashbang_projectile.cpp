...
float PercentageOfFlashForPlayer(CBaseEntity *player, Vector flashPos, CBaseEntity *pevInflictor)
{
	...
	UTIL_TraceLine(flashPos, pos, (CONTENTS_SOLID | CONTENTS_MOVEABLE | CONTENTS_DEBRIS | CONTENTS_MONSTER),
	...
	for(pSGren = gEntList.FindEntityByClassname(NULL, "env_particlesmokegrenade"); pSGren;
...
void RadiusFlash(Vector vecSrc, CBaseEntity *pevInflictor, CBaseEntity *pevAttacker, float flDamage, int iClassIgnore,
				 int bitsDamageType)
{
...
CFlashbangProjectile *CFlashbangProjectile::Create(const Vector &position, const QAngle &angles, const Vector &velocity,
												   const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner)
{
	CFlashbangProjectile *pGrenade =
		(CFlashbangProjectile *)CBaseEntity::Create("flashbang_projectile", position, angles, pOwner);
