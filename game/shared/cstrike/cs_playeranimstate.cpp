...
public:
	...
	friend ICSPlayerAnimState *CreatePlayerAnimState(CBaseAnimatingOverlay *pEntity,
													 ICSPlayerAnimStateHelpers *pHelpers, LegAnimType_t legAnimType,
	...
	void InitCS(CBaseAnimatingOverlay *pPlayer, ICSPlayerAnimStateHelpers *pHelpers, LegAnimType_t legAnimType,
				bool bUseAimSequences);
...
private:
	...
	float m_flFireCycle;
	PlayerAnimEvent_t
		m_delayedFire; // if we fire while reloading, delay the fire by one frame so we can cancel the reload first
	...
	CUtlDict<int, int> m_namedSequence; // Dictionary of sequences computed with CalcSequenceIndex.  This is because
										// LookupSequence is a performance hit - CS:S player models have 750+ sequences!
};

ICSPlayerAnimState *CreatePlayerAnimState(CBaseAnimatingOverlay *pEntity, ICSPlayerAnimStateHelpers *pHelpers,
...
//----------------------------------------------------------------------------------------------
ICSPlayerAnimState *CreateHostageAnimState(CBaseAnimatingOverlay *pEntity, ICSPlayerAnimStateHelpers *pHelpers,
										   LegAnimType_t legAnimType, bool bUseAimSequences)
{
...
void CCSPlayerAnimState::InitCS(CBaseAnimatingOverlay *pEntity, ICSPlayerAnimStateHelpers *pHelpers,
								LegAnimType_t legAnimType, bool bUseAimSequences)
{
...
void CCSPlayerAnimState::UpdateLayerSequenceGeneric(CStudioHdr *pStudioHdr, int iLayer, bool &bEnabled,
...
void CCSPlayerAnimState::ComputeGrenadeSequence(CStudioHdr *pStudioHdr)
{
	...
	if(m_bThrowingGrenade)
	{
		UpdateLayerSequenceGeneric(pStudioHdr, GRENADESEQUENCE_LAYER, m_bThrowingGrenade, m_flGrenadeCycle,
								   m_iGrenadeSequence, false);
	}
	else
	{
		if(m_pPlayer)
		...
		if(IsOuterGrenadePrimed() || bThrowPending)
		{
			...
			UpdateLayerSequenceGeneric(pStudioHdr, GRENADESEQUENCE_LAYER, m_bPrimingGrenade, m_flGrenadeCycle,
									   m_iGrenadeSequence, true);
...
void CCSPlayerAnimState::ComputeReloadSequence(CStudioHdr *pStudioHdr)
{
	...
	UpdateLayerSequenceGeneric(pStudioHdr, RELOADSEQUENCE_LAYER, m_bReloading, m_flReloadCycle, m_iReloadSequence,
							   hold);
	if(!m_bReloading)
	{
		m_flReloadHoldEndTime = 0.0f;
...
int CCSPlayerAnimState::CalcFireLayerSequence(PlayerAnimEvent_t event)
{
	...
	if(event == PLAYERANIMEVENT_THROW_GRENADE || pWeapon->GetWeaponID() == WEAPON_HEGRENADE ||
	   pWeapon->GetWeaponID() == WEAPON_SMOKEGRENADE || pWeapon->GetWeaponID() == WEAPON_FLASHBANG)
	{
		return -1;
	}
...
void CCSPlayerAnimState::DebugShowAnimState(int iStartLine)
{
	engine->Con_NPrintf(iStartLine++, "fire  : %s, cycle: %.2f\n",
						m_bFiring ? GetSequenceName(m_pOuter->GetModelPtr(), m_iFireSequence) : "[not firing]",
						m_flFireCycle);
	engine->Con_NPrintf(iStartLine++, "reload: %s, cycle: %.2f\n",
						m_bReloading ? GetSequenceName(m_pOuter->GetModelPtr(), m_iReloadSequence) : "[not reloading]",
						m_flReloadCycle);
...
void CCSPlayerAnimState::ClearAnimationLayers()
{
	...
	{
		// Client obeys Order of CBaseAnimatingOverlay::MAX_OVERLAYS (15), but server trusts only the ANIM_LAYER_ACTIVE
		// flag.
