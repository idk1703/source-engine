...
void CWeaponElite::PrimaryAttack()
{
	...
	FX_FireBullets(pPlayer->entindex(), pPlayer->Weapon_ShootPosition(),
				   ...
				   FiringLeft() ? Secondary_Mode : Primary_Mode, CBaseEntity::GetPredictionRandomSeed() & 255,
...
bool CWeaponElite::OnFireEvent(C_BaseViewModel *pViewModel, const Vector &origin, const QAngle &angles, int event,
							   const char *options)
{
	if(event == 5001)
	{
