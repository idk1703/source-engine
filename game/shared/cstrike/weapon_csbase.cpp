...
#if !defined(CLIENT_DLL)
	SendPropInt(SENDINFO(m_weaponMode), 1, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_fAccuracyPenalty)),
		// world weapon models have no aminations
		SendPropExclude("DT_AnimTimeMustBeFirst", "m_flAnimTime"), SendPropExclude("DT_BaseAnimating", "m_nSequence"),
//	SendPropExclude( "DT_LocalActiveWeaponData", "m_flTimeWeaponIdle" ),
#else
	RecvPropInt(RECVINFO(m_weaponMode)), RecvPropFloat(RECVINFO(m_fAccuracyPenalty)),
#endif
END_NETWORK_TABLE()
...
BEGIN_PREDICTION_DATA(CWeaponCSBase)
	DEFINE_PRED_FIELD(m_flTimeWeaponIdle, FIELD_FLOAT, FTYPEDESC_OVERRIDE | FTYPEDESC_NOERRORCHECK),
		...
		DEFINE_PRED_FIELD(m_bDelayFire, FIELD_BOOLEAN, 0), DEFINE_PRED_FIELD(m_flAccuracy, FIELD_FLOAT, 0),
...
ConVar cl_crosshaircolor("cl_crosshaircolor", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
						 "Set crosshair color: 0=green, 1=red, 2=blue, 3=yellow, 4=cyan, 5=custom");
ConVar cl_dynamiccrosshair("cl_dynamiccrosshair", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
						   "Enables dynamic crosshair; 0=off, 1=normal behavior (based on actual weapon accuracy), "
						   "2=legacy simulated dynamic behavior, 3=legacy simulated static behavior");
...
ConVar weapon_debug_spread_show(
	"weapon_debug_spread_show", "0", FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY,
	"Enables display of weapon accuracy; 1: show accuracy box, 2: show box with recoil offset");
...
ConVar cl_legacy_crosshair_recoil("cl_legacy_crosshair_recoil", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
								  "Enable legacy framerate dependent crosshair recoil");

// use old scaling behavior
ConVar cl_legacy_crosshair_scale("cl_legacy_crosshair_scale", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
								 "Enable legacy crosshair scaling");
...
float CWeaponCSBase::GetInaccuracy() const
{
	...
	return m_fAccuracyPenalty +
		   RemapValClamped(pPlayer->GetAbsVelocity().Length2D(), fMaxSpeed * CS_PLAYER_SPEED_DUCK_MODIFIER,
...
void CWeaponCSBase::DrawCrosshair()
{
	...
	switch(cl_dynamiccrosshair.GetInt())
	{
		...
		case 3:
		{
			float fCrosshairDistanceGoal =
				GetCSWpnData().m_iCrosshairMinDistance; // The minimum distance the crosshair can achieve...
			...
			// [jpaquin] changed to only bump up the crosshair size if the player is still shooting or is spectating
			// someone else
			int iDeltaDistance =
				GetCSWpnData()
					.m_iCrosshairDeltaDistance; // Amount by which the crosshair expands when shooting (per frame)
			...
			if(m_flCrosshairDistance > fCrosshairDistanceGoal)
			{
				...
				if(!cl_legacy_crosshair_recoil.GetBool())
				{
					...
					m_flCrosshairDistance =
						Lerp(expf(-gpGlobals->frametime / 0.44888f), fCrosshairDistanceGoal, m_flCrosshairDistance);
				}
				else
				{
...
bool CWeaponCSBase::OnFireEvent(C_BaseViewModel *pViewModel, const Vector &origin, const QAngle &angles, int event,
								const char *options)
{
	if(event == 5001)
	{
...
float CWeaponCSBase::CalcViewmodelBob(void)
{
	...
	if((!gpGlobals->frametime) || (player == NULL) || (cl_bobcycle.GetFloat() <= 0.0f) ||
	   (cl_bobup.GetFloat() <= 0.0f) || (cl_bobup.GetFloat() >= 1.0f))
	{
