...
bool CWeaponCSBaseGun::CSBaseGunFire(float flCycleTime, CSWeaponMode weaponMode)
{
	...
	else
	{
		...
		if(m_bFireOnEmpty)
		{
			...
			// NOTE[pmf]: we don't want to actually play the dry fire animations, as most seem to depict the weapon
			// actually firing. SendWeaponAnim( ACT_VM_DRYFIRE );
	...
	FX_FireBullets(pPlayer->entindex(), pPlayer->Weapon_ShootPosition(),
				   pPlayer->EyeAngles() + 2.0f * pPlayer->GetPunchAngle(), GetWeaponID(), weaponMode,
				   CBaseEntity::GetPredictionRandomSeed() & 255, GetInaccuracy(), GetSpread(), flCurAttack);
