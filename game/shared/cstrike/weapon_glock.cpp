...
private:
	...
	CNetworkVar(int,
				m_iBurstShotsRemaining); // used to keep track of the shots fired during the Glock18 burst fire mode.
...
#ifdef CLIENT_DLL
	RecvPropBool(RECVINFO(m_bBurstMode)), RecvPropInt(RECVINFO(m_iBurstShotsRemaining)),
#else
	SendPropBool(SENDINFO(m_bBurstMode)), SendPropInt(SENDINFO(m_iBurstShotsRemaining)),
#endif
END_NETWORK_TABLE()
...
void CWeaponGlock::FireRemaining(float fSpread)
{
	...
	FX_FireBullets(pPlayer->entindex(), pPlayer->Weapon_ShootPosition(),
				   pPlayer->EyeAngles() + 2.0f * pPlayer->GetPunchAngle(), GetWeaponID(), Secondary_Mode,
				   CBaseEntity::GetPredictionRandomSeed() &
					   255, // wrap it for network traffic so it's the same between client and server
				   fInaccuracy, GetSpread(), m_fNextBurstShot);
