...
public:
	...
	bool IsDifficulty(
		BotDifficultyType diff) const; ///< return true if this profile can be used for the given difficulty level
...
private:
	...
	void Inherit(const BotProfile *parent,
				 const BotProfile *baseline); ///< copy values from parent if they differ from baseline
...
public:
	...
	const BotProfile *GetRandomProfile(BotDifficultyType difficulty, int team, CSWeaponType weaponType)
		const; ///< return random unused profile that matches the given difficulty level
	...
	const VoiceBankList *GetVoiceBanks(void) const
	{
		return &m_voiceBanks;
	}
	int FindVoiceBankIndex(
		const char *filename); ///< return index of the (custom) bot phrase db, inserting it if needed
