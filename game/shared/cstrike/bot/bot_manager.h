...
public:
	...
	CBaseGrenade *GetEntity(void) const
	{
		return m_entity;
	}
	...
	bool IsSmoke(void) const
	{
		return m_isSmoke;
	}
	bool IsFlashbang(void) const
	{
		return m_isFlashbang;
	}
	CBaseGrenade *GetGrenade(void)
	{
		return m_entity;
	}
...
public:
	...
	CBasePlayer *AllocateAndBindBotEntity(
		edict_t *ed); ///< allocate the appropriate entity for the bot and bind it to the given edict
	...
	void SetGrenadeRadius(CBaseGrenade *grenade,
						  float radius); ///< the radius of the grenade entity (or associated smoke cloud)
	...
	bool IsLineBlockedBySmoke(const Vector &from, const Vector &to,
							  float grenadeBloat = 1.0f); ///< return true if line intersects smoke volume, with grenade
														  ///< radius increased by the grenadeBloat factor
	...
	const DebugMessage *GetDebugMessage(
		int which = 0) const; ///< return the debug message emitted by the bot (0 = most recent)
