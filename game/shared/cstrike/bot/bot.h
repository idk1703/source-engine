...
//--------------------------------------------------------------------------------------------------------
template<class T>
T *CreateBot(const BotProfile *profile, int team)
{
	if(!AreBotsAllowed())
		return NULL;
...
#define MUST_JUMP true
	...
	bool IsActiveWeaponRecoilHigh(
		void) const;		 ///< return true if active weapon's bullet spray has become large and inaccurate
	...
	bool IsPlayerLookingAtMe(CBasePlayer *enemy,
							 float cosTolerance = 0.9f) const; ///< returns true if other player is pointing right at us
	bool IsLookingAtPosition(
		const Vector &pos, float angleTolerance = 20.0f) const; ///< returns true if looking (roughly) at given position

	bool IsLocalPlayerWatchingMe(void) const; ///< return true if local player is observing this bot

	void PrintIfWatched(PRINTF_FORMAT_STRING const char *format,
						...) const; ///< output message to console if we are being watched by the local player
	...
	virtual bool ClientCommand(
		const CCommand &args);		  ///< Do a "client command" - useful for invoking menu choices, etc.
 ...
 */
template<class PlayerType>
inline void CBot<PlayerType>::BuildUserCmd(CUserCmd &cmd, const QAngle &viewangles, float forwardmove, float sidemove,
										   float upmove, int buttons, byte impulse)
{
...
inline void CBot<PlayerType>::PrintIfWatched(PRINTF_FORMAT_STRING const char *format, ...) const
{
	   ...
	   (cv_bot_debug.GetInt() == 2 || cv_bot_debug.GetInt() == 4))
	{
		...
		ClientPrint(UTIL_GetListenServerHost(), HUD_PRINTCONSOLE,
					UTIL_VarArgs("%s: %s", (name) ? name : "(NULL netname)", buffer));
...
extern const Vector *FindNearbyHidingSpot(CBaseEntity *me, const Vector &pos, float maxRange = 1000.0f,
										  bool isSniper = false, bool useNearest = false);
extern const Vector *FindRandomHidingSpot(CBaseEntity *me, Place place, bool isSniper = false);
extern const Vector *FindNearbyRetreatSpot(CBaseEntity *me, const Vector &start, float maxRange = 1000.0f,
										   int avoidTeam = 0);
