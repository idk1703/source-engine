	...
	while(true)
	{
		...
		if(isCustomSkin)
		{
			...
			if(m_nextSkin < NumCustomSkins && !skinExists)
			{
				...
				Q_snprintf(m_skinFilenames[m_nextSkin], sizeof(m_skinFilenames[m_nextSkin]), "models/player/%s/%s.mdl",
						   token, token);
				++m_nextSkin;
			}
 ...
 */
const BotProfile *BotProfileManager::GetRandomProfile(BotDifficultyType difficulty, int team,
													  CSWeaponType weaponType) const
{
