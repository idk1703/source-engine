...
void CNavPath::Draw(const Vector &color)
{
	...
	for(int i = 1; i < m_segmentCount; ++i)
	{
		DrawLine(m_path[i - 1].pos + Vector(0, 0, HalfHumanHeight), m_path[i].pos + Vector(0, 0, HalfHumanHeight), 2,
...
void CNavPathFollower::Update(float deltaT, bool avoidObstacles)
{
	...
	*/
	...
	if(!m_improv->IsUsingLadder())
	{
		...
		if(m_improv->GetLastKnownArea() && m_improv->GetLastKnownArea()->GetAttributes() & NAV_MESH_CROUCH &&
...
void CNavPathFollower::FeelerReflexAdjustment(Vector *goalPosition, float height)
{
	...
	const float feelerHeight =
		(height > 0.0f) ? height : StepHeight + 0.1f; // if obstacle is lower than StepHeight, we'll walk right over it
