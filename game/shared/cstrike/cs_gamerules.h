...
private:
	float GetExplosionDamageAdjustment(
		Vector &vecSrc, Vector &vecEnd,
		CBaseEntity *pEntityToIgnore); // returns multiplier between 0.0 and 1.0 that is the percentage of any damage
									   // done from vecSrc to vecEnd that actually makes it.
	float GetAmountOfEntityVisible(
		Vector &src,
		CBaseEntity *player); // returns a value from 0 to 1 that is the percentage of player visible from src.
...
public:
	bool IsBlackMarket(void)
	{
		return m_bBlackMarket;
	}
	int GetNumHostagesRemaining(void)
	{
		return m_iHostagesRemaining;
	}
	...
	{
		return (unsigned char *)"d7NSuLq2";
	} // both the client and server need this key
	...
	void HostageKilled()
	{
		m_hostageWasKilled = true;
	}
	void HostageInjured()
	{
		m_hostageWasInjured = true;
	}

	bool WasHostageKilled()
	{
		return m_hostageWasKilled;
	}
	bool WasHostageInjured()
	{
		return m_hostageWasInjured;
	}
	...
	bool IsCareer(void) const
	{
		return false;
	} // returns true if this is a CZ "career" game
...
public:
	...
	int m_iUnBalancedRounds; // keeps track of the # of consecutive rounds that have gone by where one team outnumbers
							 // the other team by more than 2
	...
	int m_iLoserBonus; // SupraFiend: the amount of money the losing team gets. This scales up as they lose more rounds
					   // in a row
	...
	bool GetCanDonateWeapon()
	{
		return m_bCanDonateWeapons;
	}
	...
	int m_iNumEscapers;
	int m_iNumEscapeRounds; // keeps track of the # of consecutive rounds of escape played.. Teams will be swapped after
							// 8 rounds
