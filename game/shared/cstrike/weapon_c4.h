...
public:
	...
	{
		return BaseClass::ObjectCaps() | (FCAP_CONTINUOUS_USE | FCAP_USE_IN_RADIUS);
	}

	void SetBombSiteIndex(int iIndex)
	{
		m_iBombSiteIndex = iIndex;
	}

	inline bool IsBombActive(void)
	{
		return m_bBombTicking;
	}
	...
	CCSPlayer *GetPlanter()
	{
		return m_pPlanter;
	}
	void SetPlanter(CCSPlayer *player)
	{
		m_pPlanter = player;
	}
	...
	{
		m_bPlantedAfterPickup = plantedAfterPickup;
	}
...
public:
	...
	void SetDroppedFromDeath(bool droppedFromDeath)
	{
		m_bDroppedFromDeath = droppedFromDeath;
	}
