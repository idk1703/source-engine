 ...
 */
static CViewVectors g_CSViewVectors(Vector(0, 0, 64), // eye position
...
#ifdef CLIENT_DLL
	RecvPropBool(RECVINFO(m_bFreezePeriod)), RecvPropInt(RECVINFO(m_iRoundTime)),
		RecvPropFloat(RECVINFO(m_fRoundStartTime)), RecvPropFloat(RECVINFO(m_flGameStartTime)),
		RecvPropInt(RECVINFO(m_iHostagesRemaining)), RecvPropBool(RECVINFO(m_bMapHasBombTarget)),
		RecvPropBool(RECVINFO(m_bMapHasRescueZone)), RecvPropBool(RECVINFO(m_bLogoMap)),
		RecvPropBool(RECVINFO(m_bBlackMarket))
#else
	SendPropBool(SENDINFO(m_bFreezePeriod)), SendPropInt(SENDINFO(m_iRoundTime), 16),
		SendPropFloat(SENDINFO(m_fRoundStartTime), 32, SPROP_NOSCALE),
		SendPropFloat(SENDINFO(m_flGameStartTime), 32, SPROP_NOSCALE), SendPropInt(SENDINFO(m_iHostagesRemaining), 4),
		SendPropBool(SENDINFO(m_bMapHasBombTarget)), SendPropBool(SENDINFO(m_bMapHasRescueZone)),
		SendPropBool(SENDINFO(m_bLogoMap)), SendPropBool(SENDINFO(m_bBlackMarket))
#endif
END_NETWORK_TABLE()
...
END_RECV_TABLE()
#else
void *SendProxy_CSGameRules(const SendProp *pProp, const void *pStructBase, const void *pData,
...
ConVar mp_buytime("mp_buytime", "1.5", FCVAR_REPLICATED,
				  "How many minutes after round start players can buy items for.", true, 0.25, false, 0);

ConVar mp_playerid("mp_playerid", "0", FCVAR_REPLICATED,
				   "Controls what information player see in the status bar: 0 all names; 1 team names; 2 no names",
				   true, 0, true, 2);

ConVar mp_playerid_delay("mp_playerid_delay", "0.5", FCVAR_REPLICATED,
						 "Number of seconds to delay showing information in the status bar", true, 0, true, 1);

ConVar mp_playerid_hold("mp_playerid_hold", "0.25", FCVAR_REPLICATED,
						"Number of seconds to keep showing old information in the status bar", true, 0, true, 1);

ConVar mp_round_restart_delay("mp_round_restart_delay", "5.0", FCVAR_REPLICATED,
							  "Number of seconds to delay before restarting a round after a win", true, 0.0f, true,
							  10.0f);

ConVar sv_allowminmodels("sv_allowminmodels", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
						 "Allow or disallow the use of cl_minmodels on this server.");

#ifdef CLIENT_DLL

ConVar cl_autowepswitch("cl_autowepswitch", "1", FCVAR_ARCHIVE | FCVAR_USERINFO,
...
#define CS_DAMAGE_FOR_FALL_SPEED \
	((float)100 / (CS_PLAYER_FATAL_FALL_SPEED - CS_PLAYER_MAX_SAFE_FALL_SPEED)) // damage per unit per second.
...
ConVar mp_startmoney("mp_startmoney", "800", FCVAR_REPLICATED | FCVAR_NOTIFY,
					 "amount of money each player gets when they reset", true, 800, true, 16000);

ConVar mp_roundtime("mp_roundtime", "2.5", FCVAR_REPLICATED | FCVAR_NOTIFY, "How many minutes each round takes.", true,
					1,		// min value
					true, 9 // max value
);

ConVar mp_freezetime("mp_freezetime", "6", FCVAR_REPLICATED | FCVAR_NOTIFY,
					 "how many seconds to keep players frozen when the round starts", true, 0, // min value
					 true, 60																   // max value
);

ConVar mp_c4timer("mp_c4timer", "45", FCVAR_REPLICATED | FCVAR_NOTIFY,
				  "how long from when the C4 is armed until it blows", true, 10, // min value
				  true, 90														 // max value
);

ConVar mp_limitteams("mp_limitteams", "2", FCVAR_REPLICATED | FCVAR_NOTIFY,
					 "Max # of players 1 team can have over another (0 disables check)", true, 0, // min value
					 true, 30																	  // max value
);
...
ConVar mp_spawnprotectiontime("mp_spawnprotectiontime", "5", FCVAR_REPLICATED,
							  "Kick players who team-kill within this many seconds of a round restart.");

ConVar mp_humanteam("mp_humanteam", "any", FCVAR_REPLICATED, "Restricts human players to a single team {any, CT, T}");

ConVar mp_ignore_round_win_conditions("mp_ignore_round_win_conditions", "0", FCVAR_REPLICATED,
...
void CCSGameRules::AddPricesToTable(weeklyprice_t prices)
{
	...
	{
		m_StringTableBlackMarket->AddString(CBaseEntity::IsServer(), "blackmarket_prices", sizeof(weeklyprice_t),
											&prices);
	}
	else
	{
		...
		if(pszCommand && pszCommand[0])
		{
			if(FStrEq(pszCommand, "ClanTagChanged"))
			{
				...
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"clantag\" (value \"%s\")\n", pPlayer->GetPlayerName(),
							   pPlayer->GetUserID(), pPlayer->GetNetworkIDString(), teamName,
...
float CCSGameRules::GetExplosionDamageAdjustment(Vector &vecSrc, Vector &vecEnd, CBaseEntity *pEntityToIgnore)
{
	...
	else if(!(tr.DidHitWorld()) && (tr.m_pEnt != NULL) && (tr.m_pEnt != pEntityToIgnore) &&
			(tr.m_pEnt->GetOwnerEntity() != pEntityToIgnore))
	{
		...
		if(tr.fraction == 1.0)
		{
			...
			{
				retval = 0.75; // we're blocked by something that isn't an entity with a physics module or world
							   // geometry, just cut damage in half for now.
			}
		}
	}
...
float CCSGameRules::GetAmountOfEntityVisible(Vector &vecSrc, CBaseEntity *entity)
{
	...
	chestHeightFromFeet =
		vecChest.z -
		vecFeet.z; // compute the distance from the chest to the feet. (this accounts for ducking and the like)
...
void CCSGameRules::RadiusDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore,
								...
								bool bIgnoreWorld)
{
	...
	for(CEntitySphereQuery sphere(vecSrc, flRadius); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity())
	{
		...
		if(pEntity->m_takedamage != DAMAGE_NO)
		{
			...
			if(bHit)
			{
				...
				// use a Gaussian function to describe the damage falloff over distance, with flRadius equal to 3 *
				// sigma this results in the following values:
				//
				...
				float fSigma =
					flRadius / 3.0f; // flRadius specifies 3rd standard deviation (0.0111 damage at this range)
				...
				if(flAdjustedDamage > 0)
				{
					...
					{
						CCS_GameStats.IncrementStat(pCSExplosionAttacker, CSSTAT_GRENADE_DAMAGE,
													static_cast<int>(adjustedInfo.GetDamage()));
					}
					//=============================================================================
					// HPE_END
					//=============================================================================
		...
		if(pCSExplosionVictim)
		{
			// If the bomb is exploding, set the attacker to the planter (we can't put this in the CTakeDamageInfo,
			// since players aren't supposed to get credit for bomb kills)
			if(isBomb)
			{
			...
			if(pCSExplosionAttacker && !pCSExplosionVictim->IsAlive() && wasAliveBeforeExplosion &&
	...
	if(pCSExplosionAttacker && isGrenade &&
	   numberOfEnemyPlayersKilledByThisExplosion >= AchievementConsts::GrenadeMultiKill_MinKills)
	{
	...
	if(pCSExplosionAttacker && isBomb &&
	   numberOfEnemyPlayersKilledByThisExplosion >= AchievementConsts::BombMultiKill_MinKills)
	{
...
void CCSGameRules::PlayerKilled(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
	...
	if(IPointsForKill(pScorer, pVictim) < 0)
	{
		...
		if(mp_autokick.GetBool())
		{
			...
			ClientPrint(pCSScorer, HUD_PRINTCONSOLE, "#Game_teammate_kills",
						strTeamKills); // this includes a " of 3" in it
		...
		{
			pCSScorer->m_iDisplayHistoryBits |=
DHF_FRIEND_KILLED
;
...
bool CCSGameRules::FShouldSwitchWeapon(CBasePlayer *pPlayer, CBaseCombatWeapon *pWeapon)
{
	...
	if(pPlayer->GetActiveWeapon() && pPlayer->IsNetClient() && !bIsBeingGivenItem)
	{
		...
		const char *cl_autowepswitch =
			engine->GetClientConVarValue(engine->IndexOfEdict(pPlayer->edict()), "cl_autowepswitch");
...
void CCSGameRules::InitializePlayerCounts(int &NumAliveTerrorist, int &NumAliveCT, int &NumDeadTerrorist,
										  int &NumDeadCT)
{
...
bool CCSGameRules::BombRoundEndCheck(bool bNeededPlayers)
{
	...
	else if((m_bBombDefused == true) && (m_bMapHasBombTarget == true))
	{
		m_iAccountCT += 3250;
...
void CCSGameRules::PickNextVIP()
{
	...
	else if (m_pVIP == NULL)  // There is no VIP and there is no one waiting to be the VIP.. therefore just pick the
	first CT player we can find.
	{
		CBaseEntity* pPlayer = NULL;
		CBasePlayer* player = NULL;
...
void CCSGameRules::RestartRound()
{
	...
	/*************** AUTO-BALANCE CODE *************/
	if(mp_autoteambalance.GetInt() != 0 && (m_iUnBalancedRounds >= 1))
	{
		if(GetHumanTeam() == TEAM_UNASSIGNED)
		{
			BalanceTeams();
	...
	if(((m_iNumSpawnableCT - m_iNumSpawnableTerrorist) >= 2) || ((m_iNumSpawnableTerrorist - m_iNumSpawnableCT) >= 2))
	{
		m_iUnBalancedRounds++;
	}
	else
	...
	if(gEntList.FindEntityByClassname(NULL, "func_escapezone"))
	{
		...
		m_iNumEscapeRounds++; // Increment the number of rounds played... After 8 rounds, the players will do a whole
							  // sale switch..
	}
	else
		m_bMapHasEscapeZone = false;
	...
	else if((m_iNumConsecutiveCTLoses > 1) && (m_iLoserBonus < 3000))
		m_iLoserBonus += 500; // help out the team in the losing streak
	...
	for(i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(pPlayer->GetTeamNumber() == TEAM_CT && pPlayer->PlayerClass() >= FIRST_CT_CLASS &&
		   pPlayer->PlayerClass() <= LAST_CT_CLASS)
		{
			//=============================================================================
			// HPE_BEGIN:
		...
		if(pPlayer->GetTeamNumber() == TEAM_TERRORIST && pPlayer->PlayerClass() >= FIRST_T_CLASS &&
		   pPlayer->PlayerClass() <= LAST_T_CLASS)
		{
			//=============================================================================
			// HPE_BEGIN:
	...
	for(i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(pPlayer->GetTeamNumber() == TEAM_CT && allCtsWearingSameUniform &&
		   numberOfCts >= AchievementConsts::SameUniform_MinPlayers)
		{
			pPlayer->AwardAchievement(CSSameUniform);
		}

		if(pPlayer->GetTeamNumber() == TEAM_TERRORIST && allTerroristsWearingSameUniform &&
		   numberOfTerrorists >= AchievementConsts::SameUniform_MinPlayers)
		{
			pPlayer->AwardAchievement(CSSameUniform);
		}
	}
...
void CCSGameRules::GiveC4()
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(pPlayer && pPlayer->IsAlive() && pPlayer->GetTeamNumber() == TEAM_TERRORIST &&
		   numAliveTs[ALL_TERRORISTS] < ABSOLUTE_PLAYER_LIMIT)
		{
			...
			if(team)
			{
				...
				switch(teamIndex)
				{
					case TEAM_TERRORIST:
						...
						winEvent->SetInt("t_objectives_done",
										 CCS_GameStats.GetTeamStats(teamIndex)[CSSTAT_OBJECTIVES_COMPLETED]);
					...
					case TEAM_CT:
						...
						winEvent->SetInt("ct_objectives_done",
										 CCS_GameStats.GetTeamStats(teamIndex)[CSSTAT_OBJECTIVES_COMPLETED]);
		...
		while((ent = gEntList.FindEntityByClassname(ent, "info_player_terrorist")) != NULL)
		{
			...
			else
			{
				Warning("Invalid terrorist spawnpoint at (%.1f,%.1f,%.1f)\n", ent->GetAbsOrigin()[0],
						ent->GetAbsOrigin()[2], ent->GetAbsOrigin()[2]);
			}
		}
...
void CCSGameRules::CheckRestartRound(void)
{
	...
	if(iRestartDelay > 0)
	{
		...
		UTIL_ClientPrintAll(HUD_PRINTCENTER, "#Game_will_restart_in", strRestartDelay,
							iRestartDelay == 1 ? "SECOND" : "SECONDS");
		UTIL_ClientPrintAll(HUD_PRINTCONSOLE, "#Game_will_restart_in", strRestartDelay,
							iRestartDelay == 1 ? "SECOND" : "SECONDS");
...
void CCSGameRules::BalanceTeams(void)
{
	...
	for(int i = 0; i < iNumToSwap; i++)
	{
		...
		for(int j = 1; j <= gpGlobals->maxClients; j++)
		{
			...
			if(pPlayer && (m_pVIP != pPlayer) && (pPlayer->GetTeamNumber() == iTeamToSwap) &&
...
void CCSGameRules::ProcessEndOfRoundAchievements(int iWinnerTeam, int iReason)
{
	if(iWinnerTeam == WINNER_CT || iWinnerTeam == WINNER_TER)
	{
		...
		if(((iReason == CTs_Win && m_bNoCTsKilled) || (iReason == Terrorists_Win && m_bNoTerroristsKilled)) &&
		   losingTeam &&
		   losingTeam->GetNumPlayers() - ignoreCount >= AchievementConsts::DefaultMinOpponentsForAchievement)
		{
		...
		if(((iWinnerTeam == TEAM_TERRORIST && m_bNoCTsKilled) ||
			(iWinnerTeam == Terrorists_Win && m_bNoTerroristsKilled)) &&
...
void CCSGameRules::UploadGameStats(void)
{
	...
	if(DataHasChanged() == true)
	{
		...
		{
			gamestatsuploader->UploadGameStats(STRING(gpGlobals->mapname), CS_STATS_BLOB_VERSION, uBlobSize,
											   pvBlobData);
		}
...
#define ENABLE_COMPETITIVE_CONVAR(convarName, ...)                                                                  \
	...
	{                                                                                                               \
	public:                                                                                                         \
		CCompetitiveMinspecConvar##convarName()                                                                     \
		{                                                                                                           \
			CCompetitiveCvarManager::AddConvarToList(this);                                                         \
		}                                                                                                           \
		...
		{                                                                                                           \
			...
			{                                                                                                       \
				CCompetitiveCvarManager::GetConVarBackupKV()->SetFloat(#convarName,                                 \
																	   ConVarRef(#convarName).GetFloat());          \
			}                                                                                                       \
		}                                                                                                           \
		virtual void BackupConvar()                                                                                 \
		{                                                                                                           \
			CCompetitiveCvarManager::GetConVarBackupKV()->SetFloat(#convarName, ConVarRef(#convarName).GetFloat()); \
		}                                                                                                           \
		virtual void EnforceRestrictions()                                                                          \
		{                                                                                                           \
			EnforceCompetitiveCVar(#convarName, __VA_ARGS__);                                                       \
		}                                                                                                           \
		virtual void RestoreOriginalValue()                                                                         \
		{                                                                                                           \
			ConVarRef(#convarName).SetValue(CCompetitiveCvarManager::GetConVarBackupKV()->GetFloat(#convarName));   \
		}                                                                                                           \
		virtual void InstallChangeCallback()                                                                        \
		{                                                                                                           \
			static_cast<ConVar *>(ConVarRef(#convarName).GetLinkedConVar())                                         \
				->InstallChangeCallback(CCompetitiveMinspecConvar##convarName::on_changed_##convarName);            \
		}                                                                                                           \
	};                                                                                                              \
	static CCompetitiveMinspecConvar##convarName *s_pCompetitiveConvar##convarName =                                \
		new CCompetitiveMinspecConvar##convarName();
...
static ConVar sv_competitive_minspec(
	"sv_competitive_minspec", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
