...
//=============================================================================
ConVar sv_nostats("sv_nostats", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
				  "Disable collecting statistics and awarding achievements.");
//=============================================================================
// HPE_END
//=============================================================================
#endif // CSTRIKE_DLL
...

{
	...
	if(bPersistToSteamCloud)
	{
		...
		ISteamRemoteStorage *pRemoteStorage =
			SteamClient()
				? (ISteamRemoteStorage *)SteamClient()->GetISteamGenericInterface(
...
{
public:
	CAchievementSaveThread() : m_pKV(NULL)
	{
...
void CAchievementMgr::PostInit()
{
	...
	while(pAchievementHelper)
	{
		...
		// (e.g. EP 1/2/... achievements are in shared binary but are game specific, they have a game filter for runtime
		// check.)
		...
		if(!pGameDirFilter || (0 == Q_strcmp(pGameDir, pGameDirFilter)))
		{
			...
			{
				m_mapMetaAchievement.Insert(pAchievement->GetAchievementID(),
											dynamic_cast<CAchievement_AchievedCount *>(pAchievement));
			}
		}
		else
		{
...
void CAchievementMgr::Update(float frametime)
{
	...
	for(int i = iCount - 1; i >= 0; i--)
	{
		if(m_vecThinkListeners[i].m_flThinkTime < gpGlobals->curtime)
		{
			...
			if(m_vecThinkListeners[i].pAchievement->IsAchieved() ||
			   m_vecThinkListeners[i].m_flThinkTime < gpGlobals->curtime)
			{
...
void CAchievementMgr::LevelInitPreEntity()
{
	...
	// load global state if we haven't already; X360 users may not have had a storage device available or selected at
	// boot time
	EnsureGlobalStateLoaded();

#ifdef GAME_DLL
	// For single-player games, achievement mgr must live on the server.  (Only the server has detailed knowledge of
	// game state.)
	Assert(!GameRules()->IsMultiplayer());
#else
	// For multiplayer games, achievement mgr must live on the client.  (Only the client can read/write player state
	// from Steam/XBox Live.)
	Assert(GameRules()->IsMultiplayer());
#endif
	...
	// client and server have map names available in different forms (full path on client, just file base name on
	// server), cache it in base file name form here so we don't have to have different code paths each time we access
	// it
#ifdef CLIENT_DLL
...
void CAchievementMgr::DownloadUserData()
{
	...
	else if(IsX360())
	{
		...
		{
			ret = xboxsystem->EnumerateAchievements(XBX_GetPrimaryUserId(), 0, 0, nTotalAchievements, pBuffer, bytes,
													false);
...
void CAchievementMgr::UploadUserData()
{
	if(IsPC())
	{
		...
		if(steamapicontext->SteamUserStats())
		{
			// Upload current Steam client achievements & stats state to Steam.  Will get called back at
			// OnUserStatsStored when complete. Only values previously set via SteamUserStats() get uploaded
...
bool CAchievementMgr::CheckAchievementsEnabled()
{
	...
	if(IsPC())
	{
		...
		{
#ifndef NO_STEAM
			// Cheats get turned on automatically if you run with -dev which many people do internally, so allow cheats
			// if developer is turned on and we're not running on Steam public
...
//-----------------------------------------------------------------------------
void CAchievementMgr::OnKillEvent(CBaseEntity *pVictim, CBaseEntity *pAttacker, CBaseEntity *pInflictor,
								  IGameEvent *event)
{
	...
	FOR_EACH_VEC(m_vecKillEventListeners, iAchievement)
	{
		...
		// if this achievement only looks for kills where attacker is player and that is not the case here, skip this
		// achievement
		if((pAchievement->GetFlags() & ACH_FILTER_ATTACKER_IS_PLAYER) && !bAttackerIsPlayer)
			continue;

		// if this achievement only looks for kills where victim is killer enemy and that is not the case here, skip
		// this achievement
		if((pAchievement->GetFlags() & ACH_FILTER_VICTIM_IS_PLAYER_ENEMY) && !bVictimIsPlayerEnemy)
			continue;

#if GAME_DLL
		// if this achievement only looks for a particular victim class name and this victim is a different class, skip
		// this achievement
		...
		// if this achievement only looks for a particular inflictor class name and this inflictor is a different class,
		...
		// if this achievement only looks for a particular attacker class name and this attacker is a different class,
		// skip this achievement
		...
		// if this achievement only looks for a particular inflictor entity name and this inflictor has a different
		// name, skip this achievement
...
//-----------------------------------------------------------------------------
// Purpose: Returns an achievement as it's abstract object. This interface is used by gameui.dll for getting achievement
// info. Input  : index - Output : IBaseAchievement*
//-----------------------------------------------------------------------------
...
void CAchievementMgr::Steam_OnUserStatsStored(UserStatsStored_t *pUserStatsStored)
{
	...
	{
		// We had some failure (like not connected or timeout) that means the stats were not stored. Redirty to try
		// again
		SetDirty(true);
	}
	else
	{
		if(k_EResultInvalidParam == pUserStatsStored->m_eResult)
		{
			...
			// In the case of k_EResultInvalidParam, some of the stats may have been stored, so we should still fall
			// through to fire events related to achievements being awarded.
		}
...
void CAchievementMgr::UpdateStateFromSteam_Internal()
{
	...
	FOR_EACH_MAP(m_mapAchievement, i)
	{
		...
		bool bRet = steamapicontext->SteamUserStats()->GetAchievementAndUnlockTime(pAchievement->GetName(), &bAchieved,
																				   &unlockTime);
...
CON_COMMAND_F(achievement_test_clan_count, "Determines if specified # of teammates belong to same clan w/local player",
			  FCVAR_CHEAT)
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
	if(!pAchievementMgr)
