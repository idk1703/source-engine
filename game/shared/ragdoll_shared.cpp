...
void CRagdollLowViolenceManager::SetLowViolence(const char *pMapName)
{
	...
	{
		if(Q_stricmp(pMapName, "ep1_citadel_02") == 0 || Q_stricmp(pMapName, "ep1_citadel_02b") == 0 ||
	...
	{
		if(Q_stricmp(pMapName, "d3_citadel_03") == 0 || Q_stricmp(pMapName, "d3_citadel_04") == 0 ||
		   Q_stricmp(pMapName, "d3_citadel_05") == 0 || Q_stricmp(pMapName, "d3_breen_01") == 0)
		{
			m_bLowViolence = false;
		}
	}
...
static void RagdollAddSolid(IPhysicsEnvironment *pPhysEnv, ragdoll_t &ragdoll, const ragdollparams_t &params,
							solid_t &solid)
{
	if(solid.index >= 0 && solid.index < params.pCollide->solidCount)
	{
		...
		if(boneIndex >= 0)
		{
			...
			ragdoll.list[ragdoll.listCount].pObject = pPhysEnv->CreatePolyObject(
				params.pCollide->solids[solid.index], surfaceData, vec3_origin, vec3_angle, &solid.params);
...
static void RagdollAddConstraint(IPhysicsEnvironment *pPhysEnv, ragdoll_t &ragdoll, const ragdollparams_t &params,
								 constraint_ragdollparams_t &constraint)
{
	...
	if(constraint.childIndex >= 0 && constraint.parentIndex >= 0)
	{
		...
		Studio_CalcBoneToBoneTransform(params.pStudioHdr, ragdoll.boneIndex[constraint.childIndex],
									   ragdoll.boneIndex[constraint.parentIndex], constraint.constraintToAttached);
		...
		if(params.fixedConstraints)
		{
			...
			childElement.pConstraint = pPhysEnv->CreateFixedConstraint(
				childElement.pObject, ragdoll.list[constraint.parentIndex].pObject, ragdoll.pGroup, fixed);
		}
		else
		{
			childElement.pConstraint = pPhysEnv->CreateRagdollConstraint(
				childElement.pObject, ragdoll.list[constraint.parentIndex].pObject, ragdoll.pGroup, constraint);
		}
	}
}
...
void RagdollSolveSeparation(ragdoll_t &ragdoll, CBaseEntity *pEntity)
{
	...
	for(int i = 0; i < ragdoll.listCount; i++)
	{
		...
		if(element.pConstraint && element.parentIndex >= 0)
		{
			...
			{
				// this fixes a bug in ep2 with antlion grubs, but causes problems in TF2 - revisit, but disable for TF
				// now
...
bool ShouldRemoveThisRagdoll(CBaseAnimating *pRagdoll)
{
	...
	*/
	...
	{
		if(g_debug_ragdoll_removal.GetBool())
			NDebugOverlay::Line(pRagdoll->GetAbsOrigin(), pRagdoll->GetAbsOrigin() + Vector(0, 0, 64), 0, 255, 0, true,
								5);
	...
	{
		if(g_debug_ragdoll_removal.GetBool())
			NDebugOverlay::Line(pRagdoll->GetAbsOrigin(), pRagdoll->GetAbsOrigin() + Vector(0, 0, 64), 0, 0, 255, true,
								5);
