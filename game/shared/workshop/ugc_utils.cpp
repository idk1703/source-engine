...
CUGCFileRequest::CUGCFileRequest(void)
	: m_hCloudID(k_UGCHandleInvalid), m_UGCStatus(UGCFILEREQUEST_READY), m_AsyncControl(NULL)
{
													  ...
													  const char *lpszTargetFilename /*= NULL*/)
{
	...
	if(hSteamAPICall != k_uAPICallInvalid)
	{
		...
		Msg("Started download of cloud file %s/%s (%08X%08X)\n", lpszTargetDirectory, lpszTargetFilename,
			(uint32)(hFileHandle >> 32), (uint32)hFileHandle);
...
void CUGCFileRequest::Steam_OnUGCDownload(RemoteStorageDownloadUGCResult_t *pResult, bool bError)
{
	...
	// Read in the data
	if(pRemoteStorage->UGCRead(m_hCloudID, m_bufContents.Base(), nFileSizeInBytes, 0,
...
#ifdef LOG_FILEREQUEST_PROGRESS
	Msg("Read file %s/%s (%08X%08X)\n", m_szTargetDirectory, m_szTargetFilename, (uint32)(m_hCloudID >> 32),
		(uint32)m_hCloudID);
#endif // LOG_FILEREQUEST_PROGRESS
	...
	if(g_pFullFileSystem->AsyncWrite(szLocalFilename, m_bufContents.Base(), m_bufContents.TellPut(), false, false,
									 &m_AsyncControl) < 0)
	{
...
bool UGC_SetFileTime(const char *pFileRelativePath, RTime32 uTimestamp)
{
	...
	char const *pchFullPath = g_pFullFileSystem->RelativePathToFullPath(
		pFileRelativePath, UGC_PATHID, chFullFilePathForTimestamp, sizeof(chFullFilePathForTimestamp));
	if(pchFullPath)
	{
