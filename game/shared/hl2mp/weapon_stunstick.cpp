...
#define NUM_BEAM_ATTACHMENTS 9
	...
	stunstickBeamInfo_t
		m_BeamAttachments[NUM_BEAM_ATTACHMENTS]; // Lookup for arc attachment points on the head of the stick
	...
	switch(pEvent->event)
	{
		case EVENT_WEAPON_MELEE_HIT:
		{
			...
			CBaseEntity *pHurt = pOperator->CheckTraceHullAttack(
				pOperator->Weapon_ShootPosition(), vecEnd, Vector(-16, -16, -40), Vector(16, 16, 16),
...
void C_WeaponStunStick::SetupAttachmentPoints(void)
{
	...
	if(ShouldDrawUsingViewModel())
	{
		const char *szBeamAttachNamesTop[NUM_BEAM_ATTACHMENTS] = {
			"spark1a", "spark2a", "spark3a", "spark4a", "spark5a", "spark6a", "spark7a", "spark8a", "spark9a",
		};

		const char *szBeamAttachNamesBottom[NUM_BEAM_ATTACHMENTS] = {
			"spark1b", "spark2b", "spark3b", "spark4b", "spark5b", "spark6b", "spark7b", "spark8b", "spark9b",
		};
...
bool C_WeaponStunStick::InSwing(void)
{
	...
	if(activity == GetPrimaryAttackActivity() || activity == GetSecondaryAttackActivity() ||
...
void StunstickImpactCallback(const CEffectData &data)
{
	...
	FX_AddQuad(data.m_vOrigin, data.m_vNormal, scale, scale * 2.0f, 1.0f, 1.0f, 0.0f, 0.0f, random->RandomInt(0, 360),
			   0, Vector(1.0f, 1.0f, 1.0f), 0.1f, "sprites/light_glow02_add", 0);
