...
#ifdef CLIENT_DLL
	RecvPropBool(RECVINFO(m_bInitialStateUpdate)), RecvPropBool(RECVINFO(m_bGuiding)),
		RecvPropBool(RECVINFO(m_bHideGuiding)), RecvPropEHandle(RECVINFO(m_hMissile), RecvProxy_MissileDied),
		RecvPropVector(RECVINFO(m_vecLaserDot)),
#else
	SendPropBool(SENDINFO(m_bInitialStateUpdate)), SendPropBool(SENDINFO(m_bGuiding)),
		SendPropBool(SENDINFO(m_bHideGuiding)), SendPropEHandle(SENDINFO(m_hMissile)),
...
void CWeaponRPG::DrawEffects(void)
{
	...
	// Get the beam's run
	trace_t tr;
	UTIL_TraceLine(vecAttachment, vecAttachment + (vecDir * RPG_LASER_BEAM_LENGTH), MASK_SHOT, GetOwner(),
...
int CLaserDot::DrawModel(int flags)
{
	...
	if(pOwner != NULL && pOwner->IsDormant() == false)
	{
		...
		trace_t tr;
		UTIL_TraceLine(vecAttachment, vecAttachment + (vecDir * MAX_TRACE_LENGTH), MASK_SHOT, pOwner,
					   COLLISION_GROUP_NONE, &tr);
