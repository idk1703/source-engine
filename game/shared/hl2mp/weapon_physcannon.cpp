...
#define NUM_PHYSCANNON_BEAMS 3
	...
	void GetEffectParameters(EffectType_t effectID, color32 &color, float &scale, IMaterial **pMaterial,
...
BEGIN_NETWORK_TABLE(CWeaponPhysCannon, DT_WeaponPhysCannon)
	...
	RecvPropBool(RECVINFO(m_bActive)), RecvPropEHandle(RECVINFO(m_hAttachedObject)),
		...
		RecvPropFloat(RECVINFO(m_attachedAnglesPlayerSpace[2])), RecvPropInt(RECVINFO(m_EffectState)),
	...
	SendPropBool(SENDINFO(m_bActive)), SendPropEHandle(SENDINFO(m_hAttachedObject)),
		...
		SendPropAngle(SENDINFO_VECTORELEM(m_attachedAnglesPlayerSpace, 2), 11), SendPropInt(SENDINFO(m_EffectState)),
	...
	/*	if( pOwner )
		{
			...
			float	maxSpeed = hl2_walkspeed.GetFloat() + ( ( hl2_normspeed.GetFloat() - hl2_walkspeed.GetFloat() ) *
	   loadWeight );
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError)
{
	...
	if(g_debug_physcannon.GetBool())
	{
		...
		debugoverlay->AddBoxOverlay(GetAttached()->WorldSpaceCenter(), -Vector(radius, radius, radius),
		...
		NDebugOverlay::Box(GetAttached()->WorldSpaceCenter(), -Vector(radius + 5, radius + 5, radius + 5),
						   Vector(radius + 5, radius + 5, radius + 5), 255, 0, 0, true, 0.0f);
#endif
	}
...
void CWeaponPhysCannon::CheckForTarget(void)
{
	...
	trace_t tr;
	UTIL_TraceHull(startPos, endPos, -Vector(4, 4, 4), Vector(4, 4, 4), MASK_SHOT | CONTENTS_GRATE, pOwner,
...
void CWeaponPhysCannon::StartEffects(void)
{
	...
	const char *attachNamesEndCap[NUM_ENDCAP_SPRITES] = {"fork1t", "fork2t", "fork3t"};
...
//			effect parameter
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::GetEffectParameters(EffectType_t effectID, color32 &color, float &scale, IMaterial **pMaterial,
											Vector &vecAttachment)
{
