...
static CGCClientSystem *s_pCGCGameSpecificClientSystem =
	NULL; // set this in the game specific derived version if needed
...
{
	AssertMsg(s_pCGCGameSpecificClientSystem,
			  "GCClientSystem is not initialized in the game specific client system constructor");
...
// Locate our steam client interface.
	...
	void *pGenericInterface =
		pSteamClient->GetISteamGenericInterface(hSteamUser, hSteamPipe, STEAMGAMECOORDINATOR_INTERFACE_VERSION);
...
// Currently logged on?
...
#else
	bool bLoggedOn = steamgameserverapicontext && steamgameserverapicontext->SteamGameServer() &&
	...
	if(bLoggedOn)
	{
		...
		if(!m_bLoggedOn)
		{
			...
			// server should automatically send us a HELLO pretty quickly after it detects us logon.  Give it some time
			// to send
		...
		if(!m_bConnectedToGC)
		{
			if(m_timeLastSendHello < Plat_FloatTime() - k_flClientHelloRetry)
			{
				...
				// CGCClientSharedObjectCache *pSOCache = m_GCClient.FindSOCache(
				// ClientSteamContext().GetLocalPlayerSteamID(), false ); if ( pSOCache != NULL &&
				// pSOCache->BIsInitialized() )
				//{
				...
				// CGCClientSharedObjectCache *pSOCache = m_GCClient.FindSOCache(
				// steamgameserverapicontext->SteamGameServer()->GetSteamID(), false ); if ( pSOCache != NULL &&
				// pSOCache->BIsInitialized() )
				//{
				//	msg.Body().set_socache_version( pSOCache->GetVersion() );
				// }
...
void CGCClientSystem::ReceivedClientGoodbye(const CMsgClientGoodbye &msg)
{
	switch(msg.reason())
	{
		...
		case GCGoodbyeReason_NO_SESSION:
			if(m_bConnectedToGC)
			{
				Warning("The connection to the item server has been interrupted.  Attempting to re-negotiate "
						"connection now.\n");
			}
			break;
...
GC_REG_JOB(GCSDK::CGCClient, CGCClientJobClientGoodbye, "CGCClientJobClientGoodbye", k_EMsgGCClientGoodbye,
		   k_EServerTypeGCClient);
...
void CGCClientSystem::ReceivedServerGoodbye(const CMsgServerGoodbye &msg)
{
	switch(msg.reason())
	{
		...
		case GCGoodbyeReason_NO_SESSION:
			if(m_bConnectedToGC)
			{
				Warning("The connection to the game coordinator has been interrupted.  Attempting to re-negotiate "
						"connection now.\n");
			}
			break;
...
GC_REG_JOB(GCSDK::CGCClient, CGCClientJobServerGoodbye, "CGCClientJobServerGoodbye", k_EMsgGCServerGoodbye,
		   k_EServerTypeGCClient);
