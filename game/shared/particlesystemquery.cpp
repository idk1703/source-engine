...
public:
	...
	virtual void TraceLine(const Vector &vecAbsStart, const Vector &vecAbsEnd, unsigned int mask,
						   const IHandleEntity *ignore, int collisionGroup, CBaseTrace *ptr);
...
//-----------------------------------------------------------------------------
#ifdef CLIENT_DLL
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CParticleSystemQuery, IParticleSystemQuery, PARTICLE_SYSTEM_QUERY_INTERFACE_VERSION,
								  s_ParticleSystemQuery);
#endif

static CThreadFastMutex s_LightMutex;

// This mutex exists because EntityToWorldTransform was not threadsafe, and could potentially have been called from
// multiple particle update threads. It has now been fixed to be threadsafe, so this mutex can safely just be a no-op
// (meaingful perf win for this). static CThreadFastMutex s_BoneMutex;
...
void CParticleSystemQuery::TraceLine(const Vector &vecAbsStart, const Vector &vecAbsEnd, unsigned int mask,
									 const IHandleEntity *ignore, int collisionGroup, CBaseTrace *ptr)
{
	bool bDoTrace = false;
#ifndef GAME_DLL
...
bool CParticleSystemQuery::MovePointInsideControllingObject(CParticleCollection *pParticles, void *pObject,
															Vector *pPnt)
{
#ifdef GAME_DLL
	return true;
	...
	int *pHitBoxIndexOut)
{
					...
					if(set)
					{
						...
						for(int i = 0; i < nNumPtsOut; i++)
						{
							...
							do
							{
								...
								vecLocalPosition.x =
									GetSurfaceCoord(flTryU, pBox->bbmin.x * pAnimating->GetModelScale(),
													pBox->bbmax.x * pAnimating->GetModelScale());
								vecLocalPosition.y =
									GetSurfaceCoord(flTryV, pBox->bbmin.y * pAnimating->GetModelScale(),
													pBox->bbmax.y * pAnimating->GetModelScale());
								vecLocalPosition.z =
									GetSurfaceCoord(flTryW, pBox->bbmin.z * pAnimating->GetModelScale(),
								...
								float flPointGoodness =
									pParticles->RandomFloat(0, 72) +
	...
	if(!bSucesss)
	{
		...
		{
			pPntsOut[i] =
				pParticles->m_ControlPoints[nControlPointNumber].m_Position; // fallback if anything goes wrong
...
bool CParticleSystemQuery::IsPointInControllingObjectHitBox(CParticleCollection *pParticles, int nControlPointNumber,
															Vector vecPos, bool bBBoxOnly)
{
	bool bSuccess = false;
#ifndef GAME_DLL
