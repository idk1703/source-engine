...
BEGIN_NETWORK_TABLE_NOBASE(CTeamRoundTimer, DT_TeamRoundTimer)
	...
	RecvPropInt(RECVINFO(m_bTimerPaused), 0, RecvProxy_TimerPaused), RecvPropTime(RECVINFO(m_flTimeRemaining)),
		RecvPropTime(RECVINFO(m_flTimerEndTime)), RecvPropInt(RECVINFO(m_nTimerMaxLength)),
		RecvPropBool(RECVINFO(m_bIsDisabled)), RecvPropBool(RECVINFO(m_bShowInHUD)),
		RecvPropInt(RECVINFO(m_nTimerLength)), RecvPropInt(RECVINFO(m_nTimerInitialLength)),
		RecvPropBool(RECVINFO(m_bAutoCountdown)), RecvPropInt(RECVINFO(m_nSetupTimeLength)),
		RecvPropInt(RECVINFO(m_nState)), RecvPropBool(RECVINFO(m_bStartPaused)),
		RecvPropBool(RECVINFO(m_bShowTimeRemaining)), RecvPropBool(RECVINFO(m_bInCaptureWatchState)),
		RecvPropBool(RECVINFO(m_bStopWatchTimer)), RecvPropTime(RECVINFO(m_flTotalTime)),

#else

	SendPropBool(SENDINFO(m_bTimerPaused)), SendPropTime(SENDINFO(m_flTimeRemaining)),
		SendPropTime(SENDINFO(m_flTimerEndTime)), SendPropInt(SENDINFO(m_nTimerMaxLength)),
		SendPropBool(SENDINFO(m_bIsDisabled)), SendPropBool(SENDINFO(m_bShowInHUD)),
		SendPropInt(SENDINFO(m_nTimerLength)), SendPropInt(SENDINFO(m_nTimerInitialLength)),
		SendPropBool(SENDINFO(m_bAutoCountdown)), SendPropInt(SENDINFO(m_nSetupTimeLength)),
		SendPropInt(SENDINFO(m_nState)), SendPropBool(SENDINFO(m_bStartPaused)),
		SendPropBool(SENDINFO(m_bShowTimeRemaining)), SendPropBool(SENDINFO(m_bStopWatchTimer)),
		SendPropBool(SENDINFO(m_bInCaptureWatchState)), SendPropTime(SENDINFO(m_flTotalTime)),
...
BEGIN_DATADESC(CTeamRoundTimer)
	DEFINE_KEYFIELD(m_nTimerInitialLength, FIELD_INTEGER, "timer_length"),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Pause", InputPause), DEFINE_INPUTFUNC(FIELD_VOID, "Resume", InputResume),
		DEFINE_INPUTFUNC(FIELD_INTEGER, "SetTime", InputSetTime),
		DEFINE_INPUTFUNC(FIELD_INTEGER, "AddTime", InputAddTime), DEFINE_INPUTFUNC(FIELD_VOID, "Restart", InputRestart),
		...
		DEFINE_OUTPUT(m_OnRoundStart, "OnRoundStart"), DEFINE_OUTPUT(m_OnFinished, "OnFinished"),
		DEFINE_OUTPUT(m_On5MinRemain, "On5MinRemain"), DEFINE_OUTPUT(m_On4MinRemain, "On4MinRemain"),
		DEFINE_OUTPUT(m_On3MinRemain, "On3MinRemain"), DEFINE_OUTPUT(m_On2MinRemain, "On2MinRemain"),
		DEFINE_OUTPUT(m_On1MinRemain, "On1MinRemain"), DEFINE_OUTPUT(m_On30SecRemain, "On30SecRemain"),
		DEFINE_OUTPUT(m_On10SecRemain, "On10SecRemain"), DEFINE_OUTPUT(m_On5SecRemain, "On5SecRemain"),
		DEFINE_OUTPUT(m_On4SecRemain, "On4SecRemain"), DEFINE_OUTPUT(m_On3SecRemain, "On3SecRemain"),
		DEFINE_OUTPUT(m_On2SecRemain, "On2SecRemain"), DEFINE_OUTPUT(m_On1SecRemain, "On1SecRemain"),
		DEFINE_OUTPUT(m_OnSetupStart, "OnSetupStart"), DEFINE_OUTPUT(m_OnSetupFinished, "OnSetupFinished"),
...
#ifndef GAME_DLL
	// We need to add a couple seconds to the time remaining because we've probably lost ~0.5 seconds from the timer
	// while waiting for the update to arrive from the server and we don't want to miss any critical countdown messages.
	// If the time remaining is over 10 seconds...adding 2 seconds to the total when calculating our output messages
	...
	if(!m_bTimerPaused && m_bAutoCountdown && !TeamplayRoundBasedRules()->IsInWaitingForPlayers())
	{
		...
		if(pPlayer)
		{
			if(ObjectiveResource())
			{
				...
				if(TeamplayRoundBasedRules()->IsInTournamentMode() == true &&
				   TeamplayRoundBasedRules()->IsInStopWatch() == true)
				{
					...
					if(IsStopWatchTimer() == true && IsWatchingTimeStamps() == false)
					{
						CTeamRoundTimer *pTimer =
							dynamic_cast<CTeamRoundTimer *>(ClientEntityList().GetEnt(iActiveTimer));

						if(pTimer && pTimer->IsTimerPaused() == false &&
						   pTimer->GetTimeRemaining() > GetTimeRemaining())
						{
					...
					else
					{
						CTeamRoundTimer *pStopWatch =
							dynamic_cast<CTeamRoundTimer *>(ClientEntityList().GetEnt(iStopWatchTimer));
						...
						if(ObjectiveResource()->GetTimerToShowInHUD() == entindex())
						{
							if(pStopWatch)
							{
								...
								if(pStopWatch->GetTimeRemaining() > GetTimeRemaining() &&
								   pStopWatch->IsWatchingTimeStamps() == false)
								{
									bShouldPlaySound = true;
								}
...
void CTeamRoundTimer::RoundTimerThink(void)
{
	...
	if(TeamplayRoundBasedRules()->RoundHasBeenWon() || TeamplayRoundBasedRules()->IsLoadingBugBaitReport())
	{
...
void CTeamRoundTimer::SetTimeRemaining(int iTimerSeconds)
{
	...
	if(TeamplayRoundBasedRules()->IsInTournamentMode() && TeamplayRoundBasedRules()->IsInStopWatch() &&
	   ObjectiveResource() && !IsStopWatchTimer() && !TeamplayRoundBasedRules()->InSetup())
	{
		...
		if(pStopWatch && !pStopWatch->IsWatchingTimeStamps() && TeamplayRoundBasedRules()->StopWatchShouldBeTimedWin())
		{
			...
			flTimerSeconds =
				flStopWatchRemainingTime > 0 ? MIN(flTimerSeconds, flStopWatchRemainingTime) : flTimerSeconds;
...
void CTeamRoundTimer::AddTimerSeconds(int iSecondsToAdd, int iTeamResponsible /* = TEAM_UNASSIGNED*/)
{
	...
	if(TeamplayRoundBasedRules()->State_Get() != GR_STATE_RND_RUNNING &&
	   TeamplayRoundBasedRules()->State_Get() != GR_STATE_TEAM_WIN)
		return;
	...
	if((ObjectiveResource() && ObjectiveResource()->GetTimerInHUD() == entindex()) ||
	   (TeamplayRoundBasedRules()->IsInKothMode()))
	{
