	...
	if(manifest->LoadFromFile(filesystem, szMapManifestFilename, "GAME"))
	{
		...
		for(KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey())
		{
			if(!Q_stricmp(sub->GetName(), "file"))
			{
				...
				{
					Warning("CParticleMgr::LevelInit:  Manifest '%s' contains a particle file '%s' that's not under "
...
void DispatchParticleEffect(const char *pszParticleName, ParticleAttachment_t iAttachType, CBaseEntity *pEntity,
							const char *pszAttachmentName, bool bResetAllParticlesOnEntity)
{
	int iAttachment = -1;
							...
							int iAttachmentPoint, bool bResetAllParticlesOnEntity)
{
	...
	if((data.m_fFlags & PARTICLE_DISPATCH_FROM_ENTITY) != 0 &&
	   (iAttachType == PATTACH_ABSORIGIN_FOLLOW || iAttachType == PATTACH_POINT_FOLLOW ||
		iAttachType == PATTACH_ROOTBONE_FOLLOW))
	{
...
//-----------------------------------------------------------------------------
void DispatchParticleEffect(const char *pszParticleName, ParticleAttachment_t iAttachType, CBaseEntity *pEntity,
							const char *pszAttachmentName, Vector vecColor1, Vector vecColor2, bool bUseColors,
...
// Purpose:
//-----------------------------------------------------------------------------
void DispatchParticleEffect(const char *pszParticleName, Vector vecOrigin, QAngle vecAngles, Vector vecColor1,
							Vector vecColor2, bool bUseColors, CBaseEntity *pEntity, int iAttachType)
{
...
//-----------------------------------------------------------------------------
void DispatchParticleEffect(const char *pszParticleName, Vector vecOrigin, Vector vecStart, QAngle vecAngles,
							CBaseEntity *pEntity)
{
	int iIndex = GetParticleSystemIndex(pszParticleName);
...
ConVar particle_test_attach_mode(
	"particle_test_attach_mode", "follow_attachment", FCVAR_CHEAT,
	"Possible Values: 'start_at_attachment', 'follow_attachment', 'start_at_origin', 'follow_origin'");
ConVar particle_test_attach_attachment("particle_test_attach_attachment", "0", FCVAR_CHEAT,
...
void Particle_Test_Start(CBasePlayer *pPlayer, const char *name, bool bStart)
{
	...
	while((pEntity = GetNextCommandEntity(pPlayer, name, pEntity)) != NULL)
	{
		...
		*/
		...
		DispatchParticleEffect(pszParticleFile, (ParticleAttachment_t)iAttachType, pEntity, iAttachmentIndex,
...
static ConCommand particle_test_start(
	"particle_test_start", CC_Particle_Test_Start,
...
static ConCommand particle_test_stop("particle_test_stop", CC_Particle_Test_Stop,
...
void CC_DispatchParticle(const CCommand &args)
{
	...
	trace_t tr;
	UTIL_TraceLine(pLocalPlayer->EyePosition(), pLocalPlayer->EyePosition() + vForward * 3000, MASK_SOLID_BRUSHONLY,
				   NULL, &tr);
...
static ConCommand dispatch_particle("dispatch_particle", CC_DispatchParticle,
									"Dispatch specified particle effect 50 units away from the lookat surface "
									"normal.\n\tArguments: {particle_name} {surface_offset_distance}",
									FCVAR_CHEAT);
