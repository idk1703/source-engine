...
CTFNailgunNail *CTFNailgunNail::CreateNail(bool fSendClientNail, Vector vecOrigin, QAngle vecAngles,
										   CBaseEntity *pOwner, CBaseEntity *pLauncher, bool fMakeClientNail)
{
...
CTFNailgunNail *CTFNailgunNail::CreateSuperNail(Vector vecOrigin, QAngle vecAngles, CBaseEntity *pOwner,
...
void CTFNailgunNail::NailTouch(CBaseEntity *pOther)
{
	...
	if(dynamic_cast<CTFNailgunNail *>(pOther) == NULL)
	{
		...
		trace_t tr;
		UTIL_TraceLine(GetAbsOrigin() - vVelNorm * 30, GetAbsOrigin() + vVelNorm * 50, MASK_SOLID_BRUSHONLY, pevOwner,
