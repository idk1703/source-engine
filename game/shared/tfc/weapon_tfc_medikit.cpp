	...
	*bDoEffects = false;
	...
	if(pPlayer->IsAlly(pTarget))
	{
		...
		if(pTarget->m_Shared.GetStateFlags() & TFSTATE_HALLUCINATING)
		{
			...
			// Tell everyone
			UTIL_ClientPrintAll(HUD_PRINTNOTIFY, "#Medic_curehalluc", pPlayer->GetPlayerName(),
		...
		if(pTarget->m_Shared.GetStateFlags() & TFSTATE_TRANQUILISED)
		{
			...
			// Tell everyone
			UTIL_ClientPrintAll(HUD_PRINTNOTIFY, "#Medic_curetranq", pPlayer->GetPlayerName(),
								pTarget->GetPlayerName());
		...
		if(pTarget->m_Shared.GetStateFlags() & TFSTATE_INFECTED)
		{
			...
			// Tell everyone
			UTIL_ClientPrintAll(HUD_PRINTNOTIFY, "#Medic_cureinfection", pPlayer->GetPlayerName(),
								pTarget->GetPlayerName());
	...
	else
	{
		...
		if((pTarget->m_Shared.GetStateFlags() & TFSTATE_INFECTED) || (pTarget->m_Shared.GetPlayerClass() == PC_MEDIC) ||
