...
#define MAIN_IDLE_SEQUENCE_LAYER \
	0 // For 8-way blended models, this layer blends an idle on top of the run/walk animation to simulate a 9-way blend.
	  // For 9-way blended models, we don't use this layer.

#define AIMSEQUENCE_LAYER \
	1 // Aim sequence uses layers 0 and 1 for the weapon idle animation (needs 2 layers so it can blend).
...
private:
	...
	void UpdateAimSequenceLayers(float flCycle, int iFirstLayer, bool bForceIdle, CSequenceTransitioner *pTransitioner,
								 float flWeightScale);
