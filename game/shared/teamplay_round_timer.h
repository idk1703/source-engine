...
public:
	...
	virtual void AddTimerSeconds(
		int iSecondsToAdd,
		int iTeamResponsible = TEAM_UNASSIGNED); // Add time to an already running ( or paused ) timer
...
private:
	...
	CNetworkVar(
		int, m_nSetupTimeLength); // current timer's setup time length (setup time is the time before the round begins)
	...
	CNetworkVar(bool, m_bShowTimeRemaining); // show how much time is left (default) instead of how much time has
											 // passed.
