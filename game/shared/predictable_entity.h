...
#define DECLARE_NETWORKCLASS_NOBASE() DECLARE_CLIENTCLASS_NOBASE()

#else

#define DECLARE_NETWORKCLASS() DECLARE_SERVERCLASS()

#define DECLARE_NETWORKCLASS_NOBASE() DECLARE_SERVERCLASS_NOBASE()
...
#define BEGIN_PREDICTION_DATA_GUTS(className)                                                          \
	...
	{                                                                                                  \
		datamap_t *g_PredMapHolder =                                                                   \
			PredMapInit((className *)NULL); /* This can/will be used for some clean up duties later */ \
	}                                                                                                  \
                                                                                                       \
	template<>                                                                                         \
	datamap_t *PredMapInit<className>(className *)                                                     \
...
#define BEGIN_PREDICTION_DATA(className)                  \
	...
	{                                                     \
		...
		{                                                 \
			typedef className classNameTypedef;           \
			typedescription_t predDesc[] = { {FIELD_VOID, 0, {0, 0}, 0, 0, 0, 0, 0, 0},
...
#define IMPLEMENT_NETWORKCLASS(className, dataTable)	IMPLEMENT_CLIENTCLASS(className, dataTable, className)
#define IMPLEMENT_NETWORKCLASS_DT(className, dataTable) IMPLEMENT_CLIENTCLASS_DT(className, dataTable, className)
...
#define IMPLEMENT_NETWORKCLASS(className, dataTable)		 IMPLEMENT_SERVERCLASS(className, dataTable)
#define IMPLEMENT_NETWORKCLASS_DT(className, dataTable)		 IMPLEMENT_SERVERCLASS_ST(className, dataTable)
