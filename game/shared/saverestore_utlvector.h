...
public:
	...
	virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave)
	{
		...
		typedescription_t dataDesc = {
			(fieldtype_t)FIELD_TYPE, "elems", {0, 0}, 1, FTYPEDESC_SAVE, NULL, NULL, NULL, pArrayTypeDatamap, -1,
		};
...
#define DEFINE_UTLVECTOR(name, fieldtype)                                                                \
	{                                                                                                    \
		FIELD_CUSTOM, #name, {offsetof(classNameTypedef, name), 0}, 1, FTYPEDESC_SAVE, NULL,             \
			CUtlVectorDataopsInstantiator<fieldtype>::GetDataOps(&(((classNameTypedef *)0)->name)), NULL \
	}

#define DEFINE_GLOBAL_UTLVECTOR(name, fieldtype)                                                                \
	{                                                                                                           \
		FIELD_CUSTOM, #name, {offsetof(classNameTypedef, name), 0}, 1, FTYPEDESC_SAVE | FTYPEDESC_GLOBAL, NULL, \
			CUtlVectorDataopsInstantiator<fieldtype>::GetDataOps(&(((classNameTypedef *)0)->name)), NULL        \
	}
