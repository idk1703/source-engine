...
BEGIN_DATADESC(CFailableAchievement)
	DEFINE_FIELD(m_bActivated, FIELD_BOOLEAN), DEFINE_FIELD(m_bFailed, FIELD_BOOLEAN),
END_DATADESC()
...
//			do special handling
//-----------------------------------------------------------------------------
void CBaseAchievement::Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker, CBaseEntity *pInflictor,
										  IGameEvent *event)
{
...
void CBaseAchievement::HandleProgressUpdate()
{
	...
	if((m_iProgressMsgIncrement > 0) && m_iCount >= m_iProgressMsgMinimum &&
	   (0 == (m_iCount % m_iProgressMsgIncrement)))
	{
...
{
	// save if we should get saved globally and have a non-zero count, or if we have been achieved, or if the player has
	// pinned this achievement to the HUD
