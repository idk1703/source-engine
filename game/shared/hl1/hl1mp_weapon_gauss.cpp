...
BEGIN_DATADESC(CWeaponGauss)
	DEFINE_FIELD(m_nAttackState, FIELD_INTEGER), DEFINE_FIELD(m_bPrimaryFire, FIELD_BOOLEAN),
	...
	if(m_nAttackState == 0)
	{
		...
		if(m_sndCharge == NULL)
		{
			...
			m_sndCharge = (CSoundEnvelopeController::GetController())
							  .SoundCreate(filter, entindex(), CHAN_WEAPON, "Weapon_Gauss.Spin", ATTN_NORM);
		}
		...
		if(pEntity->IsBSPModel() && !fShouldDamageEntity)
		{
...
#if !defined(CLIENT_DLL)
				// explode a bit
				RadiusDamage(CTakeDamageInfo(this, pPlayer, flDamage * n, DMG_BLAST), tr.endpos, flDamage * n * 2.5,
							 CLASS_NONE, NULL);
#endif
			...
			else
			{
				...
				if(!m_bPrimaryFire)
				{
					...
					UTIL_TraceLine(tr.endpos + vecDir * 8, vecDest, MASK_SHOT, pIgnore, COLLISION_GROUP_NONE,
								   &punch_tr);
					if(!punch_tr.allsolid)
					{
						...
						if(n < flDamage)
						{
							...
							RadiusDamage(CTakeDamageInfo(this, pPlayer, flDamage, DMG_BLAST),
										 exit_tr.endpos + vecDir * 8, flDamageRadius, CLASS_NONE, NULL);
