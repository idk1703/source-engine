...
#ifdef CLIENT_DLL
	RecvPropBool(RECVINFO(m_bZoomed)), RecvPropInt(RECVINFO(m_bDoViewAnim), 0, RecvProxy_AnimStart)
#else
	SendPropBool(SENDINFO(m_bZoomed)), SendPropBool(SENDINFO(m_bDoViewAnim))
#endif
END_NETWORK_TABLE()
...
void CDODSniperWeapon::ItemPostFrame(void)
{
	...
	if(m_flRezoomTime > 0)
	{
		...
		if((pPlayer->m_nButtons & IN_SPEED) > 0 && pPlayer->GetAbsVelocity().Length2D() > 20)
		{
			m_flRezoomTime = -1;
		}
...
{
	return (IsZoomed() == true && ((gpGlobals->curtime - m_flZoomChangeTime) > DOD_SNIPER_ZOOM_CHANGE_TIME));
}
...
Vector CDODSniperWeapon::GetDesiredViewModelOffset(C_DODPlayer *pOwner)
{
	...
	float flPercent = clamp((viewOffset.z - VEC_PRONE_VIEW_SCALED(pOwner).z) /
								(VEC_VIEW_SCALED(pOwner).z - VEC_PRONE_VIEW_SCALED(pOwner).z),
							0.0, 1.0);

	Vector offset =
		(flPercent * GetDODWpnData().m_vecViewNormalOffset + (1.0 - flPercent) * GetDODWpnData().m_vecViewProneOffset);

	// how long since we changed iron sight mode
	float flZoomAnimPercent =
		clamp(((m_flViewAnimTimer - gpGlobals->curtime) / (DOD_SNIPER_ZOOM_CHANGE_TIME)), 0.0, 1.0);
