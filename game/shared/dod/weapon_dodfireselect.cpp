...
Vector CDODFireSelectWeapon::GetDesiredViewModelOffset(C_DODPlayer *pOwner)
{
	...
	{
		return (flLastPercent * offset + (1.0 - flLastPercent) * GetDODWpnData().m_vecViewProneOffset);
	}
	...
	return (flZoomPercent * GetDODWpnData().m_vecIronSightOffset + (1.0 - flZoomPercent) * offset);
}
#endif