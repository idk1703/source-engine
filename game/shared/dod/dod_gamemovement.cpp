...
protected:
	...
	void TracePlayerBBoxWithStep(const Vector &vStart, const Vector &vEnd, unsigned int fMask, int collisionGroup,
								 trace_t &trace);
...
void CDODGameMovement::SetPlayerSpeed(void)
{
	...
	if(m_pDODPlayer->m_Shared.IsPlanting() || m_pDODPlayer->m_Shared.IsDefusing())
	{
		mv->m_flClientMaxSpeed = PLAYER_SPEED_FROZEN;
		return;
	}
	...
	else if(m_pDODPlayer->m_Shared.IsProne() && !m_pDODPlayer->m_Shared.IsGettingUpFromProne() &&
...
void CDODGameMovement::WalkMove(void)
{
	...
	if(bSprintButtonPressed && (mv->m_nButtons & IN_FORWARD) && !m_pDODPlayer->m_Shared.IsProne() &&
	   !m_pDODPlayer->m_Shared.IsDucking() && flSpeed > 80)
	{
...
void CDODGameMovement::CheckForLadders(bool wasOnGround)
{
	...
	{
		trace_t trace;
		TracePlayerBBox(mv->GetAbsOrigin(), mv->GetAbsOrigin() + Vector(0, 0, -5), MASK_PLAYERSOLID,
		...
		if(trace.fraction == 1.0f)
		{
			...
			vel *= 5.0f;
			Vector vecStandPos(mv->GetAbsOrigin().x - vel.x, mv->GetAbsOrigin().y - vel.y,
							   mv->GetAbsOrigin().z - (player->m_Local.m_flStepSize * 1.0f));
...
inline void CDODGameMovement::TracePlayerBBoxWithStep(const Vector &vStart, const Vector &vEnd, unsigned int fMask,
													  int collisionGroup, trace_t &trace)
{
...
void CDODGameMovement::ReduceTimers(void)
{
	...
	float fl2DVelocitySquared = vecPlayerVelocity.x * vecPlayerVelocity.x + vecPlayerVelocity.y * vecPlayerVelocity.y;
	...
	else
	{
		...
		else if((m_pDODPlayer->GetFlags() & FL_ONGROUND) && (mv->m_nButtons & IN_DUCK) &&
	...
	if(m_pDODPlayer->m_Shared.IsInMGDeploy() || flTimeSinceDeployChange < TIME_TO_DEPLOY)
	{
		if(m_pDODPlayer->m_Shared.IsInMGDeploy())
		{
			...
			if(m_pDODPlayer->m_Shared.GetLastViewAnimTime() < m_pDODPlayer->m_Shared.m_flDeployChangeTime.m_Value)
			{
				// Deployed height
				float flViewOffset =
					clamp(m_pDODPlayer->m_Shared.GetDeployedHeight(), CROUCHING_DEPLOY_HEIGHT, STANDING_DEPLOY_HEIGHT);
		...
		else
		{
			...
			if(m_pDODPlayer->m_Shared.GetLastViewAnimTime() < m_pDODPlayer->m_Shared.m_flDeployChangeTime.m_Value)
			{
				ViewOffsetAnimation(GetPlayerViewOffset(player->m_Local.m_bDucked), TIME_TO_DEPLOY,
...
void CDODGameMovement::Duck(void)
{
	...
	if(gpGlobals->curtime > m_pDODPlayer->m_Shared.m_flNextProneCheck)
	{
		...
		{
			if(m_pDODPlayer->m_Shared.IsProne() == false && m_pDODPlayer->m_Shared.IsGettingUpFromProne() == false)
			{
	...
	if(m_pDODPlayer->m_Shared.IsProne() && m_pDODPlayer->m_Shared.CanChangePosition() &&
	   !m_pDODPlayer->m_Shared.IsGettingUpFromProne() && (buttonsPressed & IN_DUCK) &&
	...
	if((mv->m_nButtons & IN_DUCK) || (player->m_Local.m_bDucking) || (player->GetFlags() & FL_DUCKING))
	{
		if(mv->m_nButtons & IN_DUCK)
		{
			...
			if(player->m_Local.m_bDucking)
			{
				...
				if((duckseconds > TIME_TO_DUCK) || (player->GetGroundEntity() == NULL) || alreadyDucked)
				{
					FinishDuck();
				}
				else
		...
		else
		{
			...
			if(player->m_Local.m_bAllowAutoMovement || player->GetGroundEntity() == NULL)
			{
				...
				if(CanUnduck())
				{
					...
					{
						// Finish ducking immediately if duck time is over or not on ground
						if((duckseconds > TIME_TO_UNDUCK) || (player->GetGroundEntity() == NULL))
						{
							FinishUnDuck();
						}
						else
