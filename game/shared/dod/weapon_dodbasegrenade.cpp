...
#if !defined(CLIENT_DLL)
	SendPropBool(SENDINFO(m_bPinPulled)), SendPropBool(SENDINFO(m_bArmed)),
#else
	RecvPropBool(RECVINFO(m_bPinPulled)), RecvPropBool(RECVINFO(m_bArmed)),
#endif
END_NETWORK_TABLE()
...
BEGIN_DATADESC(CWeaponDODBaseGrenade)
	DEFINE_FIELD(m_bRedraw, FIELD_BOOLEAN), DEFINE_INPUTFUNC(FIELD_FLOAT, "DetonateTime", InputSetDetonateTime),
END_DATADESC()
...
void CWeaponDODBaseGrenade::DropGrenade(void)
{
	...
	if(dod_bonusround.GetBool())
	{
		...
		if((team == TEAM_ALLIES && state == STATE_AXIS_WIN) || (team == TEAM_AXIS && state == STATE_ALLIES_WIN))
		{
			bThrow = false;
		}
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponDODBaseGrenade::EmitGrenade(Vector vecSrc, QAngle vecAngles, Vector vecVel, AngularImpulse angImpulse,
										CBasePlayer *pPlayer, float flLifeTime /* = GRENADE_FUSE_LENGTH */)
{
