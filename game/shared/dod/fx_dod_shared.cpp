...
void FX_WeaponSound(int iPlayerIndex, WeaponSound_t sound_type, const Vector &vOrigin, CDODWeaponInfo *pWeaponInfo)
{
					...
					float flSpread)
{
	...
	FireBulletsInfo_t info(1 /*shots*/, vOrigin, vecDirShooting, Vector(flSpread, flSpread, FLOAT32_NAN),
						   MAX_COORD_RANGE, pWeaponInfo->iAmmoType);
	...
	{
		trace_t tr;
		UTIL_TraceLine(vOrigin, vOrigin + vecDirShooting * MAX_COORD_RANGE, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE,
					   &tr);
		...
		else if(pLocalPlayer && pLocalPlayer->GetObserverTarget() == pPlayer &&
