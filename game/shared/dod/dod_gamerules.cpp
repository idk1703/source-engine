...
BEGIN_NETWORK_TABLE_NOBASE(CDODGameRules, DT_DODGameRules)
	...
	RecvPropInt(RECVINFO(m_iRoundState), 0, RecvProxy_RoundState), RecvPropBool(RECVINFO(m_bInWarmup)),
		RecvPropBool(RECVINFO(m_bAwaitingReadyRestart)), RecvPropTime(RECVINFO(m_flMapResetTime)),
		RecvPropTime(RECVINFO(m_flRestartRoundTime)), RecvPropBool(RECVINFO(m_bAlliesAreBombing)),
		...
		RecvPropInt(RECVINFO(m_iAlliesRespawnHead)), RecvPropInt(RECVINFO(m_iAlliesRespawnTail)),
		RecvPropInt(RECVINFO(m_iAxisRespawnHead)), RecvPropInt(RECVINFO(m_iAxisRespawnTail)),

#else

	SendPropInt(SENDINFO(m_iRoundState), 5), SendPropBool(SENDINFO(m_bInWarmup)),
		SendPropBool(SENDINFO(m_bAwaitingReadyRestart)), SendPropTime(SENDINFO(m_flMapResetTime)),
		SendPropTime(SENDINFO(m_flRestartRoundTime)), SendPropBool(SENDINFO(m_bAlliesAreBombing)),
...
#ifndef CLIENT_DLL
ConVar dod_flagrespawnbonus("dod_flagrespawnbonus", "1.0", FCVAR_GAMEDLL | FCVAR_CHEAT,
...
ConVar dod_bonusroundtime("dod_bonusroundtime", "15", FCVAR_REPLICATED, "Time after round win until round restarts",
						  true, 5, true, 15);
...
END_RECV_TABLE()
#else
void *SendProxy_DODGameRules(const SendProp *pProp, const void *pStructBase, const void *pData,
...
void CDODGameRules::UploadLevelStats(void)
{
	if(Q_strlen(STRING(gpGlobals->mapname)) > 0)
	{
		...
		for(j = 0; j < 7; j++)
		{
			stats.iMinutesPlayedPerClass_Allies[j] =
				clamp((short)(m_flSecondsPlayedPerClass_Allies[j] / 60), 0, MY_USHRT_MAX);
			stats.iMinutesPlayedPerClass_Axis[j] =
				clamp((short)(m_flSecondsPlayedPerClass_Axis[j] / 60), 0, MY_USHRT_MAX);
		}
		...
		for(i = 0; i < DOD_NUM_DISTANCE_STAT_WEAPONS; i++)
		{
			...
			for(int j = 0; j < DOD_NUM_WEAPON_DISTANCE_BUCKETS; j++)
			{
				stats.weaponStatsDistance[i].iDistanceBuckets[j] =
					clamp(m_iWeaponDistanceBuckets[weaponId][j], 0, MY_USHRT_MAX);
			}
		}
		...
		{
			gamestatsuploader->UploadGameStats(STRING(gpGlobals->mapname), DOD_STATS_BLOB_VERSION, uBlobSize,
											   pvBlobData);
		}
	}
}
...
float CDODGameRules::GetExplosionDamageAdjustment(Vector &vecSrc, Vector &vecEnd, CBaseEntity *pTarget,
...
void CDODGameRules::RadiusDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore,
...
void CDODGameRules::RadiusDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore,
								 CBaseEntity *pEntityIgnore, bool bIgnoreWorld /* = false */)
{
...
bool CDODGameRules::ShouldCollide(int collisionGroup0, int collisionGroup1)
{
	...
	if(collisionGroup0 == COLLISION_GROUP_PLAYER && collisionGroup1 == DOD_COLLISIONGROUP_SHELLS)
	{
		return false;
	}
...
CDODRoundStateInfo *CDODGameRules::State_LookupInfo(DODRoundState state)
{
	static CDODRoundStateInfo playerStateInfos[] = {
		...
		{STATE_PREGAME, "STATE_PREGAME", &CDODGameRules::State_Enter_PREGAME, NULL,
		 &CDODGameRules::State_Think_PREGAME},
		{STATE_STARTGAME, "STATE_STARTGAME", &CDODGameRules::State_Enter_STARTGAME, NULL,
		 &CDODGameRules::State_Think_STARTGAME},
		{STATE_PREROUND, "STATE_PREROUND", &CDODGameRules::State_Enter_PREROUND, NULL,
		 &CDODGameRules::State_Think_PREROUND},
		{STATE_RND_RUNNING, "STATE_RND_RUNNING", &CDODGameRules::State_Enter_RND_RUNNING, NULL,
		 &CDODGameRules::State_Think_RND_RUNNING},
		{STATE_ALLIES_WIN, "STATE_ALLIES_WIN", &CDODGameRules::State_Enter_ALLIES_WIN, NULL,
		 &CDODGameRules::State_Think_ALLIES_WIN},
		{STATE_AXIS_WIN, "STATE_AXIS_WIN", &CDODGameRules::State_Enter_AXIS_WIN, NULL,
		 &CDODGameRules::State_Think_AXIS_WIN},
		{STATE_RESTART, "STATE_RESTART", &CDODGameRules::State_Enter_RESTART, NULL,
		 &CDODGameRules::State_Think_RESTART},
...
void FillEventCategory(IGameEvent *event, int side, int category, CUtlVector<playerscore_t> &pList)
{
	...
	static const char *pCategoryNames[2][6] = {
		{"left_1", "left_score_1", "left_2", "left_score_2", "left_3", "left_score_3"},
		{"right_1", "right_score_1", "right_2", "right_score_2", "right_3", "right_score_3"}};
...
void CDODGameRules::SetWinningTeam(int team)
{
	...
	if(FStrEq(STRING(gpGlobals->mapname), "dod_colmar"))
	{
		CControlPointMaster *pMaster =
			dynamic_cast<CControlPointMaster *>(gEntList.FindEntityByClassname(NULL, "dod_control_point_master"));
...
void CDODGameRules::CreateStandardEntities()
{
	...
	g_pObjectiveResource =
		(CDODObjectiveResource *)CBaseEntity::Create("dod_objective_resource", vec3_origin, vec3_angle);
...
ConVar dod_waverespawnfactor("dod_waverespawnfactor", "1.0", FCVAR_REPLICATED | FCVAR_CHEAT,
...
void CDODGameRules::PlayerKilled(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
	...
	else if(pScorer)
	{
		...
		if(pDODVictim->m_signals.GetState() & SIGNAL_CAPTUREAREA)
		{
			...
			CAreaCapture *pArea =
				dynamic_cast<CAreaCapture *>(gEntList.FindEntityByClassname(NULL, "dod_capture_area"));
			while(pArea)
			{
...
#define DOD_DAMAGE_FOR_FALL_SPEED \
	((float)100 / (DOD_PLAYER_FATAL_FALL_SPEED - DOD_PLAYER_MAX_SAFE_FALL_SPEED)) // damage per unit per second.

/*
#define PLAYER_FALL_PUNCH_THRESHHOLD (float)350 // won't punch player's screen/make scrape noise unless player falling
at least this fast.
*/
	...
	switch(team)
	{
		case TEAM_ALLIES:
		{
			...
			DevMsg(1, "AddWaveTime ALLIES head %d tail %d numtotal %d time %.1f\n", m_iAlliesRespawnHead.Get(),
				   m_iAlliesRespawnTail.Get(), m_iNumAlliesRespawnWaves, gpGlobals->curtime + flTime);
		}
		break;
		case TEAM_AXIS:
		{
			...
			DevMsg(1, "AddWaveTime AXIS head %d tail %d numtotal %d time %.1f\n", m_iAxisRespawnHead.Get(),
				   m_iAxisRespawnTail.Get(), m_iNumAxisRespawnWaves, gpGlobals->curtime + flTime);
		}
		break;
		default:
			Assert(0);
	...
	switch(team)
	{
		case TEAM_ALLIES:
		{
			...
			DevMsg(1, "PopWaveTime ALLIES head %d tail %d numtotal %d time %.1f\n", m_iAlliesRespawnHead.Get(),
				   m_iAlliesRespawnTail.Get(), m_iNumAlliesRespawnWaves, gpGlobals->curtime);
		}
		break;
		case TEAM_AXIS:
		{
			...
			DevMsg(1, "PopWaveTime AXIS head %d tail %d numtotal %d time %.1f\n", m_iAxisRespawnHead.Get(),
				   m_iAxisRespawnTail.Get(), m_iNumAxisRespawnWaves, gpGlobals->curtime);
		}
		break;
		default:
			Assert(0);
...
const char *CDODGameRules::GetChatPrefix(bool bTeamOnly, CBasePlayer *pPlayer)
{
	...
	else
	{
		...
		bool bShowDeadPrefix = (pPlayer->IsAlive() == false) && !IsInBonusRound() && (State_Get() != STATE_GAME_OVER);
...
int CDODGameRules::DODPointsForKill(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
	...
	if(pVictim->GetTeamNumber() == pScorer->GetTeamNumber() && info.GetDamageType() & DMG_BOMB)
	{
		return 0;
	}
	...
	for(int i = 0; i < pPlayer->WeaponCount(); ++i)
	{
		...
		if(iWeight > -1 && iWeight == iCurrentWeight && pCheck != pCurrentWeapon)
		{
			...
			if(pPlayer->Weapon_CanSwitchTo(pCheck))
			{
				...
				pCheck != pCurrentWeapon) // don't reselect the weapon we're trying to get rid of
		{
...
char *szHitgroupNames[] = {"generic", "head", "chest", "stomach", "arm_left", "arm_right", "leg_left", "leg_right"};
			...
			for(j = 0; j < MAX_WEAPONS; j++)
			{
				if(pPlayer->m_WeaponStats[j].m_iNumShotsTaken > 0)
				{
					...
					for(k = 0; k < 8; k++)
					{
						if(pPlayer->m_WeaponStats[j].m_iBodygroupsHit[k] > 0)
						{
							filesystem->FPrintf(hFile, "\t\t\t\t<%s>%i</%s>\n", szHitgroupNames[k],
												pPlayer->m_WeaponStats[j].m_iBodygroupsHit[k], szHitgroupNames[k]);
						}
					}
					filesystem->FPrintf(hFile, "\t\t\t</hitgroups_hit>\n");
					...
					for(k = 0; k < 8; k++)
					{
						...
						{
							filesystem->FPrintf(hFile, "\t\t\t\t<%s>%i</%s>\n", szHitgroupNames[k],
												pPlayer->m_WeaponStats[j].m_iHitInBodygroups[k], szHitgroupNames[k]);
						}
					}
...
BEGIN_DATADESC(CFuncTeamWall)
	...
	DEFINE_KEYFIELD(m_iBlockTeam, FIELD_INTEGER, "blockteam"),
		...
		DEFINE_KEYFIELD(m_vecMaxs, FIELD_VECTOR, "maxs"), DEFINE_KEYFIELD(m_vecMins, FIELD_VECTOR, "mins"),
