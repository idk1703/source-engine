...
public:
	...
	inline CDODGamePlayRules *GetGamePlay()
	{
...
{
public:
	CDODViewVectors(Vector vView, Vector vHullMin, Vector vHullMax, Vector vDuckHullMin, Vector vDuckHullMax,
					Vector vDuckView, Vector vObsHullMin, Vector vObsHullMax, Vector vDeadViewHeight,
					Vector vProneHullMin, Vector vProneHullMax)
...
public:
	...
	inline DODRoundState State_Get(void)
	{
	...
	bool AwaitingReadyRestart(void)
	{
	...
	inline void RespawnAllPlayers(void)
	{
		RespawnPlayers(true);
	}

	// Respawn only one team, players that are ready to spawn - wave reset
	inline void RespawnTeam(int iTeam)
	{
	...
	bool AreAlliesReady(void)
	{
		return m_bHeardAlliesReady;
	}
	bool AreAxisReady(void)
	{
		return m_bHeardAxisReady;
	}
...
private:
	...
	float GetAmountOfEntityVisible(
		Vector &src, CBaseEntity *pTarget,
		CBaseEntity *pEntityToIgnore); // returns a value from 0 to 1 that is the percentage of player visible from src.
...
public:
	...
	int m_iWeaponDistanceBuckets[WEAPON_MAX]
								[DOD_NUM_WEAPON_DISTANCE_BUCKETS]; // distances of buckets are defined per-weapon
