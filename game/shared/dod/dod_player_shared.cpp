...
ConVar dod_bonusround("dod_bonusround", "1", FCVAR_REPLICATED,
					  "If true, the winners of the round can attack in the intermission.");
ConVar sv_showimpacts("sv_showimpacts", "0", FCVAR_REPLICATED | FCVAR_CHEAT,
...
static bool TraceToExit(const Vector &start, const Vector &dir, Vector &end, const float flStepSize,
...
void CDODPlayer::FireBullets(const FireBulletsInfo_t &info)
{
	...
	static int iTraceMask =
		((MASK_SOLID | CONTENTS_DEBRIS | CONTENTS_HITBOX | CONTENTS_PRONE_HELPER) & ~CONTENTS_GRATE);
	...
	while(flDamage > 0 && iPenetrations < iMaxPenetrations)
	{
		...
		UTIL_ClipTraceToPlayers(vecSrc, vecEnd + info.m_vecDirShooting * rayExtension, iTraceMask,
								&ignoreShooterAndPrevious, &tr);
		...
		if(sv_showimpacts.GetBool())
		{
			...
			debugoverlay->AddBoxOverlay(tr.endpos, Vector(-1, -1, -1), Vector(1, 1, 1), QAngle(0, 0, 0), 255, 0, 0, 127,
										4);
		...
		{
			trace_t waterTrace;
			UTIL_TraceLine(vecSrc, tr.endpos, (MASK_SHOT | CONTENTS_WATER | CONTENTS_SLIME), this, iCollisionGroup,
						   &waterTrace);
