...
static struct
{
	...
	int m_iFlagValue;
} g_AltWpnCritera[] = {{"ALTWPN_CRITERIA_FIRING", ALTWPN_CRITERIA_FIRING},
...
void CDODWeaponInfo::Parse(KeyValues *pKeyValuesData, const char *szWeaponName)
{
	...
	Q_strncpy(m_szDeployedReloadModel, pKeyValuesData->GetString("deployedreloadmodel"),
			  sizeof(m_szDeployedReloadModel));
	Q_strncpy(m_szProneDeployedReloadModel, pKeyValuesData->GetString("pronedeployedreloadmodel"),
			  sizeof(m_szProneDeployedReloadModel));
