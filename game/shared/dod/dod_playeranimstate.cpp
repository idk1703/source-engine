...
ConVar anim_showstate("anim_showstate", "-1", FCVAR_CHEAT | FCVAR_REPLICATED,
					  "Show the (client) animation state for the specified entity (-1 for none).");
ConVar anim_showstatelog("anim_showstatelog", "0", FCVAR_CHEAT | FCVAR_REPLICATED,
...
public:
	...
	virtual int CalcAimLayerSequence(float *flCyle, float *flAimSequenceWeight, bool bForceIdle)
	{
		return 0;
	}
...
protected:
	...
	void UpdateLayerSequenceGeneric(CStudioHdr *pStudioHdr, int iLayer, bool &bEnabled, float &flCurCycle,
									int &iSequence, bool bWaitAtEnd, float flWeight = 1.0);
...
void CDODPlayerAnimState::DoAnimationEvent(PlayerAnimEvent_t event, int nData)
{
	...
	else if(event == PLAYERANIMEVENT_HANDSIGNAL)
	{
		...
		if(pPlayer && !(pPlayer->m_Shared.IsBazookaDeployed() || pPlayer->m_Shared.IsProne() ||
						pPlayer->m_Shared.IsProneDeployed() || pPlayer->m_Shared.IsSniperZoomed() ||
						pPlayer->m_Shared.IsSandbagDeployed()))
		{
			CancelGestures();
...
Activity CDODPlayerAnimState::CalcMainActivity()
{
	...
	if(HandleJumping(&idealActivity) || HandleProne(&idealActivity) || HandleDucked(&idealActivity))
	{
		// intentionally blank
	}
	else
...
Activity CDODPlayerAnimState::TranslateActivity(Activity actDesired)
{
	...
	if(m_flFireIdleTime > gpGlobals->curtime)
	{
		...
		{
			case ACT_IDLE:
				idealActivity = ACT_DOD_STAND_AIM;
				break;
			case ACT_CROUCHIDLE:
				idealActivity = ACT_DOD_CROUCH_AIM;
				break;
			case ACT_RUN_CROUCH:
				idealActivity = ACT_DOD_CROUCHWALK_AIM;
				break;
			case ACT_WALK:
				idealActivity = ACT_DOD_WALK_AIM;
				break;
			case ACT_RUN:
				idealActivity = ACT_DOD_RUN_AIM;
				break;
			default:
				break;
		}
...
{
	if((GetCurrentMainSequenceActivity() == ACT_GET_UP_STAND) ||
	   (GetCurrentMainSequenceActivity() == ACT_GET_DOWN_STAND) ||
	   (GetCurrentMainSequenceActivity() == ACT_GET_UP_CROUCH) ||
	   (GetCurrentMainSequenceActivity() == ACT_GET_DOWN_CROUCH))
	{
...
void CDODPlayerAnimState::ComputeSequences(CStudioHdr *pStudioHdr)
{
	...
	if(!m_bGettingDown && !m_bGettingUp)
	{
		...
		if(pWeapon)
		{
			...
			if(!IsPlayingGesture(GESTURE_HAND_SIGNAL) && !IsPlayingGesture(GESTURE_FIDGET) && !m_bGettingDown &&
			   !m_bGettingUp)
			{
...
void CDODPlayerAnimState::ComputeFireSequence(void)
{
	...
	if(m_flFireIdleTime < gpGlobals->curtime && IsPlayingGesture(GESTURE_ATTACK1) &&
...
{
	UpdateLayerSequenceGeneric(pStudioHdr, GESTURE_LAYER, m_bPlayingGesture, m_flGestureCycle, m_iGestureSequence,
							   !m_bAutokillGesture);
}

void CDODPlayerAnimState::UpdateLayerSequenceGeneric(CStudioHdr *pStudioHdr, int iLayer, bool &bEnabled,
...
void CDODPlayerAnimState::ComputePoseParam_MoveYaw(CStudioHdr *pStudioHdr)
{
	...
	else
	{
		...
		if ( m_pOuter->entindex() == 2 )
		{
			...
			DevMsgRT("(%.2f) %.3f : (%.2f) %.3f\n", vecAnimVelocity.x, vecCurrentMoveYaw.x, vecAnimVelocity.y,
		vecCurrentMoveYaw.y );
		}
		*/
		...
		do
		{
			...
			retry = (vecCurrentMoveYaw.x < 1.0 && vecCurrentMoveYaw.x > -1.0) &&
					(dx < -DOD_MOVEMENT_ERROR_LIMIT || dx > DOD_MOVEMENT_ERROR_LIMIT);
			retry = retry || ((vecCurrentMoveYaw.y < 1.0 && vecCurrentMoveYaw.y > -1.0) &&
							  (dy < -DOD_MOVEMENT_ERROR_LIMIT || dy > DOD_MOVEMENT_ERROR_LIMIT));
...
void CDODPlayerAnimState::DebugShowAnimStateForPlayer(bool bIsServer)
{
	...
	for(int iAnim = 0; iAnim < GetOuter()->GetNumAnimOverlays(); ++iAnim)
	{
		...
		if(pLayer && (pLayer->m_nOrder != CBaseAnimatingOverlay::MAX_OVERLAYS))
		{
			Anim_StatePrintf(iLine++, "Layer %s: Weight: %.2f, Cycle: %.2f",
							 GetSequenceName(GetOuter()->GetModelPtr(), pLayer->m_nSequence), (float)pLayer->m_flWeight,
							 (float)pLayer->m_flCycle);
		}
	}
	...
	Anim_StatePrintf(iLine++, "EntityYaw: %.2f, AimYaw: %.2f, AimPitch: %.2f, MoveX: %.2f, MoveY: %.2f Body: %.2f",
					 m_angRender[YAW], m_flLastAimYaw, m_flLastAimPitch, m_vecLastMoveYaw.x, m_vecLastMoveYaw.y,
					 m_flLastBodyHeight);
