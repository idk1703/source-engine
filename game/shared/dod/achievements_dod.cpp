...
DECLARE_ACHIEVEMENT(CAchievementDODConsecutiveHeadshots, ACHIEVEMENT_DOD_CONSECUTIVE_HEADSHOTS,
					"DOD_CONSECUTIVE_HEADSHOTS", 1);
...
const char *pszOfficialMaps[] = {"dod_anzio", "dod_avalanche", "dod_argentan", "dod_colmar", "dod_donner",
								 "dod_flash", "dod_jagd",	   "dod_kalt",	   "dod_palermo"};
...
class CAchievementDODKillsWithGrenade : public CBaseAchievement
{
									...
									IGameEvent *event)
	{
		...
		if(pAttacker == pLocalPlayer)
		{
			...
			if(Q_strncmp(killedwith, "frag_", 5) && Q_strncmp(killedwith, "riflegren_", 10) &&
			   !FStrEq(killedwith, "pschreck") && !FStrEq(killedwith, "bazooka"))
			{
				m_flLastKillTime = 0;
				return;
			}
	...
	{                                                                               \
		...
		{                                                                           \
			...
			SetStoreProgressInSteam(true);                                          \
                                                                                    ...
                                                                                    \
		virtual const char *GetWeaponName(void)                                     \
		{                                                                           \
			return weaponName;                                                      \
		}                                                                           \
	};                                                                              \
...
DECLARE_ACHIEVEMENT(CAchievementDODBlockCapturesGrind, ACHIEVEMENT_DOD_BLOCK_CAPTURES_GRIND, "DOD_BLOCK_CAPTURES_GRIND",
					1);
...
DECLARE_ACHIEVEMENT(CAchievementDODBombsPlantedGrind, ACHIEVEMENT_DOD_BOMBS_PLANTED_GRIND, "DOD_BOMBS_PLANTED_GRIND",
					1);
...
DECLARE_ACHIEVEMENT(CAchievementDODBombsDefusedGrind, ACHIEVEMENT_DOD_BOMBS_DEFUSED_GRIND, "DOD_BOMBS_DEFUSED_GRIND",
					1);
...
public:
	...
	void Init()
	{
		...
		static const char *szComponents[] = {"dod_kalt", "dod_colmar"};
...
DECLARE_ACHIEVEMENT(CAchievementDODCollectHolidayGifts, ACHIEVEMENT_DOD_COLLECT_HOLIDAY_GIFTS,
					"DOD_COLLECT_HOLIDAY_GIFTS", 5);
