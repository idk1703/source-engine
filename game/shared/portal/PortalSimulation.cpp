...
static ConVar sv_portal_collision_sim_bounds_x("sv_portal_collision_sim_bounds_x", "200", FCVAR_REPLICATED,
											   "Size of box used to grab collision geometry around placed portals. "
											   "These should be at the default size or larger only!");
static ConVar sv_portal_collision_sim_bounds_y("sv_portal_collision_sim_bounds_y", "200", FCVAR_REPLICATED,
											   "Size of box used to grab collision geometry around placed portals. "
											   "These should be at the default size or larger only!");
static ConVar sv_portal_collision_sim_bounds_z("sv_portal_collision_sim_bounds_z", "252", FCVAR_REPLICATED,
											   "Size of box used to grab collision geometry around placed portals. "
											   "These should be at the default size or larger only!");
...
#define PORTAL_WORLD_WALL_HALF_SEPARATION_AMOUNT \
	0.1f // separating the world collision from wall collision by a small amount gets rid of extremely thin erroneous
		 // collision at the separating plane
...
static ConVar sv_dump_portalsimulator_collision(
	...
	FCVAR_REPLICATED | FCVAR_CHEAT); // whether to actually dump out the data now that the possibility exists
static void PortalSimulatorDumps_DumpCollideToGlView(CPhysCollide *pCollide, const Vector &origin, const QAngle &angles,
													 float fColorScale, const char *pFilename);
static void PortalSimulatorDumps_DumpBoxToGlView(const Vector &vMins, const Vector &vMaxs, float fRed, float fGreen,
												 float fBlue, const char *pszFileName);
#endif
...
static void ConvertBrushListToClippedPolyhedronList(const int *pBrushes, int iBrushCount,
													const float *pOutwardFacingClipPlanes, int iClipPlaneCount,
													float fClipEpsilon, CUtlVector<CPolyhedron *> *pPolyhedronList);
static void ClipPolyhedrons(CPolyhedron *const *pExistingPolyhedrons, int iPolyhedronCount,
							const float *pOutwardFacingClipPlanes, int iClipPlaneCount, float fClipEpsilon,
							CUtlVector<CPolyhedron *> *pPolyhedronList);
static inline CPolyhedron *TransformAndClipSinglePolyhedron(CPolyhedron *pExistingPolyhedron, const VMatrix &Transform,
															const float *pOutwardFacingClipPlanes, int iClipPlaneCount,
															float fCutEpsilon, bool bUseTempMemory);
static int GetEntityPhysicsObjects(IPhysicsEnvironment *pEnvironment, CBaseEntity *pEntity, IPhysicsObject **pRetList,
								   int iRetListArraySize);
static CPhysCollide *ConvertPolyhedronsToCollideable(CPolyhedron **pPolyhedrons, int iPolyhedronCount);

#ifndef CLIENT_DLL
static void UpdateShadowClonesPortalSimulationFlags(const CBaseEntity *pSourceEntity, unsigned int iFlags,
													int iSourceFlags);
	  ...
	  m_DataAccess(m_InternalData)
{
...
#ifndef CLIENT_DLL
	PS_SD_Static_World_StaticProps_ClippedProp_t::pTraceEntity = GetWorldEntity(); // will overinitialize, but it's
																				   // cheap

	m_InternalData.Simulation.pCollisionEntity =
		(CPSCollisionEntity *)CreateEntityByName("portalsimulator_collisionentity");
	...
	if(m_InternalData.Simulation.pCollisionEntity)
	{
		...
		m_InternalData.Simulation.Dynamic.EntFlags[m_InternalData.Simulation.pCollisionEntity->entindex()] |=
			PSEF_OWNS_PHYSICS;
...
void CPortalSimulator::MoveTo(const Vector &ptCenter, const QAngle &angles)
{
	if((m_InternalData.Placement.ptCenter == ptCenter) &&
	   (m_InternalData.Placement.qAngles == angles)) // not actually moving at all
		return;
...
#ifndef CLIENT_DLL
	// create a list of all entities that are actually within the portal hole, they will likely need to be moved out of
	// solid space when the portal moves
	CBaseEntity **pFixEntities =
		(CBaseEntity **)stackalloc(sizeof(CBaseEntity *) * m_InternalData.Simulation.Dynamic.OwnedEntities.Count());
	...
	// update geometric data
	{
		...
		AngleVectors(angles, &m_InternalData.Placement.vForward, &m_InternalData.Placement.vRight,
					 &m_InternalData.Placement.vUp);

		m_InternalData.Placement.PortalPlane.Init(
			m_InternalData.Placement.vForward,
			m_InternalData.Placement.vForward.Dot(m_InternalData.Placement.ptCenter));
	}
	...
	// update hole shape - used to detect if an entity is within the portal hole bounds
	{
		...
		// the remaining planes will always have the same ordering of normals, with different distances plugged in for
		// each convex we're creating normal order is up, down, left, right
		...
		fHolePlanes[(2 * 4) + 3] = m_InternalData.Placement.vUp.Dot(
			m_InternalData.Placement.ptCenter + (m_InternalData.Placement.vUp * (PORTAL_HALF_HEIGHT * 0.98f)));
		...
		fHolePlanes[(4 * 4) + 3] = -m_InternalData.Placement.vRight.Dot(
			m_InternalData.Placement.ptCenter - (m_InternalData.Placement.vRight * (PORTAL_HALF_WIDTH * 0.98f)));
	...
	DECREMENTTABSPACING();
	DEBUGTIMERONLY(DevMsg(2, "[PSDT:%d] %sCPortalSimulator::MoveTo() FINISH: %fms\n", GetPortalSimulatorGUID(),
						  TABSPACING, functionTimer.GetDuration().GetMillisecondsF()););
}
...
void CPortalSimulator::UpdateLinkMatrix(void)
{
	if(m_pLinkedPortal && m_pLinkedPortal->m_bLocalDataIsReady)
	{
		...
		VMatrix matRemoteToWorld(m_pLinkedPortal->m_InternalData.Placement.vForward, vRemoteLeft,
	...
	{
		Assert(m_bInCrossLinkedFunction ==
			   false); // I'm pretty sure switching to a stack would have negative repercussions
...
#ifdef DEBUG_PORTAL_COLLISION_ENVIRONMENTS
static ConVar sv_debug_dumpportalhole_nextcheck("sv_debug_dumpportalhole_nextcheck", "0",
...
bool CPortalSimulator::EntityIsInPortalHole(CBaseEntity *pEntity) const
{
	...
	if(sv_debug_dumpportalhole_nextcheck.GetBool())
	{
		...
		PortalSimulatorDumps_DumpCollideToGlView(m_InternalData.Placement.pHoleShapeCollideable, vec3_origin,
												 vec3_angle, 1.0f, szDumpFileName);
	}
#endif
	...
	switch(pEntity->GetSolid())
	{
		case SOLID_VPHYSICS:
		{
			...
			if(pVCollide != NULL)
			{
				...
				if(sv_debug_dumpportalhole_nextcheck.GetBool())
				{
					for(int i = 0; i != pVCollide->solidCount; ++i)
						PortalSimulatorDumps_DumpCollideToGlView(m_InternalData.Placement.pHoleShapeCollideable,
																 vec3_origin, vec3_angle, 0.4f, szDumpFileName);
				...
				for(int i = 0; i != pVCollide->solidCount; ++i)
				{
					physcollision->TraceCollide(ptEntityPosition, ptEntityPosition, pVCollide->solids[i], qEntityAngles,
		...
		case SOLID_BBOX:
		{
			...
			physcollision->TraceBox(ptEntityPosition, ptEntityPosition, pCollisionProp->OBBMins(),
									pCollisionProp->OBBMaxs(), m_InternalData.Placement.pHoleShapeCollideable,
...
bool CPortalSimulator::EntityHitBoxExtentIsInPortalHole(CBaseAnimating *pBaseAnimating) const
{
	...
	trace_t Trace;
	physcollision->TraceBox(ptCenter, ptCenter, vMinExtent, vMaxExtent, m_InternalData.Placement.pHoleShapeCollideable,
							vec3_origin, vec3_angle, &Trace);
...
void CPortalSimulator::AttachTo(CPortalSimulator *pLinkedPortalSimulator)
{
	...
	STARTDEBUGTIMER(functionTimer);
	DEBUGTIMERONLY(
		DevMsg(2, "[PSDT:%d] %sCPortalSimulator::AttachTo() START\n", GetPortalSimulatorGUID(), TABSPACING););
	INCREMENTTABSPACING();
...
void CPortalSimulator::TakeOwnershipOfEntity(CBaseEntity *pEntity)
{
	AssertMsg(m_bLocalDataIsReady,
			  "Tell the portal simulator where it is with MoveTo() before using it in any other way.");
	...
	if(pEntity->GetServerVehicle() != NULL) // we don't take kindly to vehicles in these here parts. Their physics
											// controllers currently don't migrate properly and cause a crash
		return;
	...
	UpdateShadowClonesPortalSimulationFlags(pEntity, PSEF_IS_IN_PORTAL_HOLE,
	...
	pEntity->CollisionRulesChanged(); // absolutely necessary in single-environment mode, possibly expendable in
									  // multi-environment moder
...
void CPortalSimulator::TakePhysicsOwnership(CBaseEntity *pEntity)
{
	...
	// physics cloning
	{
		...
		{
			int iDebugIndex;
			for(iDebugIndex = m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal.Count();
				--iDebugIndex >= 0;)
			{
				if(m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal[iDebugIndex]->GetClonedEntity() ==
				   pEntity)
					break;
			}
			...
			{
				for(iDebugIndex =
						m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal.Count();
					--iDebugIndex >= 0;)
				{
					if(m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal[iDebugIndex]
						   ->GetClonedEntity() == pEntity)
						break;
				}
				AssertMsg(iDebugIndex < 0,
						  "Trying to own an entity, when we're already exporting a clone to the linked portal");
			}
		...
		if(m_pLinkedPortal && m_pLinkedPortal->m_InternalData.Simulation.pPhysicsEnvironment)
		{
			...
			DBG_CODE(
				for(int i = m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal.Count();
					--i >= 0;)
					AssertMsg(m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal[i]
									  ->GetClonedEntity() != pEntity,
			...
			if(pClone)
			{
				...
				m_pLinkedPortal->m_InternalData.Simulation.Dynamic.EntFlags[pClone->entindex()] |=
					m_InternalData.Simulation.Dynamic.EntFlags[pEntity->entindex()] & PSEF_IS_IN_PORTAL_HOLE;
...
void CPortalSimulator::ReleaseAllEntityOwnership(void)
{
		  ...
		  iSkippedObjects) // the release function changes OwnedEntities
	{
		...
		RemoveEntityFromPortalHole(pEntity); // assume that whenever someone wants to release all entities, it's because
											 // the portal is going away
...
void CPortalSimulator::ReleasePhysicsOwnership(CBaseEntity *pEntity, bool bContinuePhysicsCloning /*= true*/,
											   bool bMovingToLinkedSimulator /*= false*/)
{
	...
	// physics cloning
	{
#ifdef _DEBUG
		{
			...
			AssertMsg(iDebugIndex < 0,
					  "Trying to release an entity, when a clone from the linked portal already exists.");
		}
#endif
		...
		// clear exported clones
		{
			...
			if(m_pLinkedPortal)
			{
				...
				for(int i = m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal.Count();
					--i >= 0;)
				{
					if(m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal[i]
						   ->GetClonedEntity() == pEntity)
					{
						CPhysicsShadowClone *pClone =
							m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal[i];
...
void CPortalSimulator::StopCloningEntity(CBaseEntity *pEntity)
{
	...
	m_InternalData.Simulation.Dynamic.ShadowClones.ShouldCloneFromMain.FastRemove(
		m_InternalData.Simulation.Dynamic.ShadowClones.ShouldCloneFromMain.Find(pEntity));
	m_InternalData.Simulation.Dynamic.EntFlags[pEntity->entindex()] &= ~PSEF_CLONES_ENTITY_FROM_MAIN;
}
...
void CPortalSimulator::CreateLocalPhysics(void)
{
	...
	// Any non-moving object can point to world safely-- Make sure we dont use 'params' for something other than that
	// beyond this point.
	if(m_InternalData.Simulation.pCollisionEntity)
	...
	// World
	{
		Assert(m_InternalData.Simulation.Static.World.Brushes.pPhysicsObject ==
			   NULL); // Be sure to find graceful fixes for asserts, performance is a big concern with portal simulation
		if(m_InternalData.Simulation.Static.World.Brushes.pCollideable != NULL)
		{
			m_InternalData.Simulation.Static.World.Brushes.pPhysicsObject =
				m_InternalData.Simulation.pPhysicsEnvironment->CreatePolyObjectStatic(
					...
					m_InternalData.Simulation.Static.SurfaceProperties.surface.surfaceProps, vec3_origin, vec3_angle,
					&params);

			if((m_InternalData.Simulation.pCollisionEntity != NULL) &&
			   (m_InternalData.Simulation.pCollisionEntity->VPhysicsGetObject() == NULL))
		...
		// Assert( m_InternalData.Simulation.Static.World.StaticProps.PhysicsObjects.Count() == 0 ); //Be sure to find
		// graceful fixes for asserts, performance is a big concern with portal simulation
#ifdef _DEBUG
		for(int i = m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations.Count(); --i >= 0;)
		{
			Assert(m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations[i].pPhysicsObject ==
				   NULL); // Be sure to find graceful fixes for asserts, performance is a big concern with portal
						  // simulation
		}
#endif
		...
		if(m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations.Count() != 0)
		{
			...
			for(int i = m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations.Count(); --i >= 0;)
			{
				PS_SD_Static_World_StaticProps_ClippedProp_t &Representation =
					m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations[i];
				Assert(Representation.pCollide != NULL);
				Assert(Representation.pPhysicsObject == NULL);

				Representation.pPhysicsObject = m_InternalData.Simulation.pPhysicsEnvironment->CreatePolyObjectStatic(
					Representation.pCollide, Representation.iTraceSurfaceProps, vec3_origin, vec3_angle, &params);
				Assert(Representation.pPhysicsObject != NULL);
...
void CPortalSimulator::CreateLinkedPhysics(void)
{
	...
	Assert(m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pPhysicsObject ==
		   NULL); // Be sure to find graceful fixes for asserts, performance is a big concern with portal simulation
	if(RemoteSimulationStaticWorld.Brushes.pCollideable != NULL)
	{
		m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pPhysicsObject =
			m_InternalData.Simulation.pPhysicsEnvironment->CreatePolyObjectStatic(
				RemoteSimulationStaticWorld.Brushes.pCollideable,
				m_pLinkedPortal->m_InternalData.Simulation.Static.SurfaceProperties.surface.surfaceProps,
				m_InternalData.Placement.ptaap_LinkedToThis.ptOriginTransform,
				m_InternalData.Placement.ptaap_LinkedToThis.qAngleTransform, &params);
		m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pPhysicsObject
			->RecheckCollisionFilter(); // some filters only work after the variable is stored in the class
	}

	Assert(m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.StaticProps.PhysicsObjects.Count() ==
		   0); // Be sure to find graceful fixes for asserts, performance is a big concern with portal simulation
	if(RemoteSimulationStaticWorld.StaticProps.ClippedRepresentations.Count() != 0)
	{
		for(int i = RemoteSimulationStaticWorld.StaticProps.ClippedRepresentations.Count(); --i >= 0;)
		{
			...
			IPhysicsObject *pPhysObject = m_InternalData.Simulation.pPhysicsEnvironment->CreatePolyObjectStatic(
				Representation.pCollide, Representation.iTraceSurfaceProps,
				m_InternalData.Placement.ptaap_LinkedToThis.ptOriginTransform,
				m_InternalData.Placement.ptaap_LinkedToThis.qAngleTransform, &params);
			if(pPhysObject)
			{
				m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.StaticProps.PhysicsObjects.AddToTail(
					pPhysObject);
				pPhysObject
					->RecheckCollisionFilter(); // some filters only work after the variable is stored in the class
			}
	...
	for(int i = RemoteOwnedEntities.Count(); --i >= 0;)
	{
		...
		{
			if(m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal[j]->GetClonedEntity() ==
		...
		CPhysicsShadowClone *pClone =
			CPhysicsShadowClone::CreateShadowClone(m_InternalData.Simulation.pPhysicsEnvironment, hEnt,
...
void CPortalSimulator::ClearLocalPhysics(void)
{
	...
	m_InternalData.Simulation.pPhysicsEnvironment->SetQuickDelete(
		true); // if we don't do this, things crash the next time we cleanup the delete list while checking mindists
...
void CPortalSimulator::ClearLinkedPhysics(void)
{
	...
	// static collideables
	{
		...
		{
			for(int i =
					m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.StaticProps.PhysicsObjects.Count();
				--i >= 0;)
				m_InternalData.Simulation.pPhysicsEnvironment->DestroyObject(
					m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.StaticProps.PhysicsObjects[i]);
	...
	Assert((m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal.Count() == 0) &&
		   ((m_pLinkedPortal == NULL) ||
			(m_pLinkedPortal->m_InternalData.Simulation.Dynamic.ShadowClones.FromLinkedPortal.Count() == 0)));
...
void CPortalSimulator::CreateLocalCollision(void)
{
	...
	if(m_InternalData.Simulation.Static.World.Brushes.Polyhedrons.Count() != 0)
		m_InternalData.Simulation.Static.World.Brushes.pCollideable =
			ConvertPolyhedronsToCollideable(m_InternalData.Simulation.Static.World.Brushes.Polyhedrons.Base(),
...
#endif
	Assert(m_InternalData.Simulation.Static.World.StaticProps.bCollisionExists ==
		   false); // Be sure to find graceful fixes for asserts, performance is a big concern with portal simulation
	if(m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations.Count() != 0)
	{
		...
		for(int i = m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations.Count(); --i >= 0;)
		{
			...
			Assert(Representation.pCollide == NULL);
			Representation.pCollide =
				ConvertPolyhedronsToCollideable(&pPolyhedronsBase[Representation.PolyhedronGroup.iStartIndex],
	...
	if(IsSimulatingVPhysics())
	{
		...
		if(m_InternalData.Simulation.Static.Wall.Local.Brushes.Polyhedrons.Count() != 0)
			m_InternalData.Simulation.Static.Wall.Local.Brushes.pCollideable = ConvertPolyhedronsToCollideable(
				m_InternalData.Simulation.Static.Wall.Local.Brushes.Polyhedrons.Base(),
	...
	if(m_InternalData.Simulation.Static.Wall.Local.Tube.Polyhedrons.Count() != 0)
		m_InternalData.Simulation.Static.Wall.Local.Tube.pCollideable =
			ConvertPolyhedronsToCollideable(m_InternalData.Simulation.Static.Wall.Local.Tube.Polyhedrons.Base(),
	...
	// grab surface properties to use for the portal environment
	{
		...
		trace_t Trace;
		UTIL_TraceLine(m_InternalData.Placement.ptCenter + m_InternalData.Placement.vForward,
					   m_InternalData.Placement.ptCenter - (m_InternalData.Placement.vForward * 500.0f),
...
void CPortalSimulator::CreateLinkedCollision(void)
{
	...
	// nothing to do for now, the current set of collision is just transformed from the linked simulator when needed.
	// It's really cheap to transform in traces and physics generation.
...
void CPortalSimulator::CreatePolyhedrons(void)
{
	...
	// forward reverse conventions signify whether the normal is the same direction as
	// m_InternalData.Placement.PortalPlane.m_Normal World and wall conventions signify whether it's been shifted in
	// front of the portal plane or behind it

	float fWorldClipPlane_Forward[4] = {
		m_InternalData.Placement.PortalPlane.m_Normal.x, m_InternalData.Placement.PortalPlane.m_Normal.y,
	...
	float fWorldClipPlane_Reverse[4] = {-fWorldClipPlane_Forward[0], -fWorldClipPlane_Forward[1],
	...
	// World
	{
		...
		vOBBRight *= flScaleY;
		vOBBUp *=
			flScaleZ; // default size for scale z (252) is player (height + portal half height) * 2. Any smaller than
					  // this will allow for players to reach unsimulated geometry before an end touch with teh portal.
		...
		// Brushes
		{
			...
			enginetrace->GetBrushesInAABB(vAABBMins, vAABBMaxs, &WorldBrushes,
			...
			// create locally clipped polyhedrons for the world
			{
				...
				ConvertBrushListToClippedPolyhedronList(pBrushList, iBrushCount, fWorldClipPlane_Reverse, 1,
		...
		// static props
		{
			...
			for(int i = StaticProps.Count(); --i >= 0;)
			{
				...
				int iPolyhedronCount =
					g_StaticCollisionPolyhedronCache.GetStaticPropPolyhedrons(pProp, PolyhedronArray, 1024);
				...
				for(int j = 0; j != iPolyhedronCount; ++j)
				{
					...
					{
						CPolyhedron *pClippedPropPolyhedron =
							ClipPolyhedron(pPropPolyhedronPiece, fWorldClipPlane_Reverse, 1, 0.01f, false);
						if(pClippedPropPolyhedron)
							m_InternalData.Simulation.Static.World.StaticProps.Polyhedrons.AddToTail(
								pClippedPropPolyhedron);
					}
				}

				indices.iNumPolyhedrons =
					m_InternalData.Simulation.Static.World.StaticProps.Polyhedrons.Count() - indices.iStartIndex;
				if(indices.iNumPolyhedrons != 0)
				{
					...
					PS_SD_Static_World_StaticProps_ClippedProp_t &NewEntry =
						m_InternalData.Simulation.Static.World.StaticProps.ClippedRepresentations[index];
	...
	//(Holy) Wall
	{
		...
		float fTubeDepthDist = vBackward.Dot(m_InternalData.Placement.ptCenter +
		...
		fPlanes[(2 * 4) + 3] = m_InternalData.Placement.vUp.Dot(
			m_InternalData.Placement.ptCenter + (m_InternalData.Placement.vUp * PORTAL_HOLE_HALF_HEIGHT));
		...
		fPlanes[(5 * 4) + 3] = m_InternalData.Placement.vRight.Dot(
			m_InternalData.Placement.ptCenter + (m_InternalData.Placement.vRight * PORTAL_HOLE_HALF_WIDTH));
		...
		float fFarRightPlaneDistance = m_InternalData.Placement.vRight.Dot(
			m_InternalData.Placement.ptCenter + m_InternalData.Placement.vRight * (PORTAL_WALL_FARDIST * 10.0f));
		float fFarLeftPlaneDistance =
			vLeft.Dot(m_InternalData.Placement.ptCenter + vLeft * (PORTAL_WALL_FARDIST * 10.0f));
		...
		if(IsSimulatingVPhysics()) // if not simulating vphysics, we skip making the entire wall, and just create the
								   // minimal tube instead
		{
			enginetrace->GetBrushesInAABB(vAABBMins, vAABBMaxs, &WallBrushes, MASK_SOLID_BRUSHONLY);

			if(WallBrushes.Count() != 0)
				ConvertBrushListToClippedPolyhedronList(WallBrushes.Base(), WallBrushes.Count(), fPlanes, 1,
														PORTAL_POLYHEDRON_CUT_EPSILON,
			...
			if(WallBrushPolyhedrons_ClippedToWall.Count() != 0)
			{
				...
				{
					CPolyhedron *pPolyhedron = ClipPolyhedron(WallBrushPolyhedrons_ClippedToWall[i], fSidePlanesOnly, 4,
					...
					else
					{
						...
						m_InternalData.Simulation.Static.Wall.Local.Brushes.Polyhedrons.AddToTail(
							WallBrushPolyhedrons_ClippedToWall[i]);
		...
		// upper wall
		{
			...
			fPlanes[(2 * 4) + 3] = m_InternalData.Placement.vUp.Dot(
				...
				m_InternalData.Placement.vUp * (PORTAL_HOLE_HALF_HEIGHT + PORTAL_WALL_MIN_THICKNESS));
			fPlanes[(3 * 4) + 3] =
				vDown.Dot(m_InternalData.Placement.ptCenter + m_InternalData.Placement.vUp * PORTAL_HOLE_HALF_HEIGHT);
			fPlanes[(4 * 4) + 3] = vLeft.Dot(m_InternalData.Placement.ptCenter +
											 vLeft * (PORTAL_HOLE_HALF_WIDTH + PORTAL_WALL_MIN_THICKNESS));
			fPlanes[(5 * 4) + 3] = m_InternalData.Placement.vRight.Dot(
				m_InternalData.Placement.ptCenter +
				m_InternalData.Placement.vRight * (PORTAL_HOLE_HALF_WIDTH + PORTAL_WALL_MIN_THICKNESS));
			...
			fPlanes[(2 * 4) + 3] = m_InternalData.Placement.vUp.Dot(
				m_InternalData.Placement.ptCenter + m_InternalData.Placement.vUp * (PORTAL_WALL_FARDIST * 10.0f));
			fPlanes[(3 * 4) + 3] =
			...
			ClipPolyhedrons(pWallClippedPolyhedrons, iWallClippedPolyhedronCount, fSidePlanesOnly, 4,
							PORTAL_POLYHEDRON_CUT_EPSILON,
		...
		// lower wall
		{
			...
			// fPlanes[(1*4) + 3] = fTubeDepthDist;
			fPlanes[(2 * 4) + 3] =
				m_InternalData.Placement.vUp.Dot(m_InternalData.Placement.ptCenter + (vDown * PORTAL_HOLE_HALF_HEIGHT));
			...
			// fPlanes[(1*4) + 3] += 2000.0f;
			fPlanes[(2 * 4) + 3] = m_InternalData.Placement.vUp.Dot(
				m_InternalData.Placement.ptCenter + (vDown * (PORTAL_HOLE_HALF_HEIGHT + PORTAL_WALL_MIN_THICKNESS)));
			fPlanes[(3 * 4) + 3] =
				vDown.Dot(m_InternalData.Placement.ptCenter + (vDown * (PORTAL_WALL_FARDIST * 10.0f)));
			fPlanes[(4 * 4) + 3] = fFarLeftPlaneDistance;
			fPlanes[(5 * 4) + 3] = fFarRightPlaneDistance;
		...
		// left wall
		{
			...
			fPlanes[(4 * 4) + 3] = vLeft.Dot(m_InternalData.Placement.ptCenter +
											 (vLeft * (PORTAL_HOLE_HALF_WIDTH + PORTAL_WALL_MIN_THICKNESS)));
			fPlanes[(5 * 4) + 3] = m_InternalData.Placement.vRight.Dot(m_InternalData.Placement.ptCenter +
			...
			fPlanes[(4 * 4) + 3] = fFarLeftPlaneDistance;
			fPlanes[(5 * 4) + 3] = m_InternalData.Placement.vRight.Dot(
				m_InternalData.Placement.ptCenter + (vLeft * (PORTAL_HOLE_HALF_WIDTH + PORTAL_WALL_MIN_THICKNESS)));
		...
		// right wall
		{
			...
			fPlanes[(3 * 4) + 3] = vDown.Dot(m_InternalData.Placement.ptCenter + (vDown * (PORTAL_HOLE_HALF_HEIGHT)));
			fPlanes[(4 * 4) + 3] =
				vLeft.Dot(m_InternalData.Placement.ptCenter + m_InternalData.Placement.vRight * PORTAL_HOLE_HALF_WIDTH);
			fPlanes[(5 * 4) + 3] = m_InternalData.Placement.vRight.Dot(
				m_InternalData.Placement.ptCenter +
				m_InternalData.Placement.vRight * (PORTAL_HOLE_HALF_WIDTH + PORTAL_WALL_MIN_THICKNESS));
...
void CPortalSimulator::DetachFromLinked(void)
{
	...
	// IMPORTANT: Physics objects must be destroyed before their associated collision data or a fairly cryptic crash
	// will ensue
#ifndef CLIENT_DLL
	ClearLinkedEntities();
	ClearLinkedPhysics();
...
{
	AssertMsg((m_pLinkedPortal == NULL) || (m_pLinkedPortal->m_bSimulateVPhysics == m_bSimulateVPhysics),
			  "Linked portals are in disagreement as to whether they would simulate VPhysics.");
		...
		for(int i = 0; i != iPortalSimulators; ++i)
		{
			...
			if(iOwnedEntities != 0)
			{
				...
				for(int j = 0; j != iOwnedEntities; ++j)
				{
					...
					if(((iExistingFlags ^ pSimulator->m_InternalData.Simulation.Dynamic.EntFlags[iEntIndex]) &
						PSEF_IS_IN_PORTAL_HOLE) != 0) // value changed
					{
						pEntity->CollisionRulesChanged(); // entity moved into or out of the portal hole, need to either
														  // add or remove collision with transformed geometry
...
void CPortalSimulator::PostPhysFrame(void)
{
	if(g_bPlayerIsInSimulator)
	{
		...
		if(pTouchedPortal && pSim &&
		   (pTouchedPortal->m_PortalSimulator.GetPortalSimulatorGUID() != pSim->GetPortalSimulatorGUID()))
		{
			Warning("Player is simulated in a physics environment but isn't touching a portal! Can't teleport, but can "
					"fall through portal hole. Returning player to main environment.\n");
			ADD_DEBUG_HISTORY(
				HISTORY_PLAYER_DAMAGE,
				UTIL_VarArgs("Player in PortalSimulator but not touching a portal, removing from sim at : %f\n",
							 gpGlobals->curtime));
...
CPortalSimulator *CPortalSimulator::GetSimulatorThatOwnsEntity(const CBaseEntity *pEntity)
{
	...
	AssertMsg(pOwningSimulatorCheck == s_OwnedEntityMap[iEntIndex],
			  "Owned entity mapping out of sync with individual simulator ownership flags.");
#endif

	return s_OwnedEntityMap[pEntity->entindex()];
}

CPortalSimulator *CPortalSimulator::GetSimulatorThatCreatedPhysicsObject(const IPhysicsObject *pObject,
...
bool CPortalSimulator::CreatedPhysicsObject(const IPhysicsObject *pObject,
											PS_PhysicsObjectSourceType_t *pOut_SourceType) const
{
	if((pObject == m_InternalData.Simulation.Static.World.Brushes.pPhysicsObject) ||
	   (pObject == m_InternalData.Simulation.Static.Wall.Local.Brushes.pPhysicsObject))
	{
		if(pOut_SourceType)
	...
	for(int i = m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.StaticProps.PhysicsObjects.Count();
		--i >= 0;)
	{
													...
													float fClipEpsilon, CUtlVector<CPolyhedron *> *pPolyhedronList)
{
	...
	{
		CPolyhedron *pPolyhedron = ClipPolyhedron(g_StaticCollisionPolyhedronCache.GetBrushPolyhedron(pBrushes[i]),
							...
							CUtlVector<CPolyhedron *> *pPolyhedronList)
{
	...
	for(int i = 0; i != iPolyhedronCount; ++i)
	{
		CPolyhedron *pPolyhedron =
			ClipPolyhedron(pExistingPolyhedrons[i], pOutwardFacingClipPlanes, iClipPlaneCount, fClipEpsilon);
		if(pPolyhedron)
			pPolyhedronList->AddToTail(pPolyhedron);
	}
}
...
static CPhysCollide *ConvertPolyhedronsToCollideable(CPolyhedron **pPolyhedrons, int iPolyhedronCount)
{
	...
	STARTDEBUGTIMER(functionTimer);
	DEBUGTIMERONLY(
		DevMsg(2, "[PSDT:%d] %sConvertPolyhedronsToCollideable() START\n", s_iPortalSimulatorGUID, TABSPACING););
	INCREMENTTABSPACING();
															...
															float fCutEpsilon, bool bUseTempMemory)
{
	...
	Polyhedron_IndexedPolygon_t *pTempPolygonArray = (Polyhedron_IndexedPolygon_t *)stackalloc(
	...
	CPolyhedron *pNewPolyhedron = ClipPolyhedron(pExistingPolyhedron, pOutwardFacingClipPlanes, iClipPlaneCount,
...
#ifndef CLIENT_DLL
// Move all entities back to the main environment for removal, and make sure the main environment is in control during
// the UTIL_Remove process
...
void UpdateShadowClonesPortalSimulationFlags(const CBaseEntity *pSourceEntity, unsigned int iFlags, int iSourceFlags)
{
	...
	while(pClones)
	{
		...
		unsigned int *pFlags =
			(unsigned int *)&pCloneSimulator->m_DataAccess.Simulation.Dynamic.EntFlags[pClone->entindex()];
		*pFlags &= ~iFlags;
		*pFlags |= iOrFlags;
...
int CPSCollisionEntity::ObjectCaps(void)
{
	return ((BaseClass::ObjectCaps() | FCAP_DONT_SAVE) &
			~(FCAP_FORCE_TRANSITION | FCAP_ACROSS_TRANSITION | FCAP_MUST_SPAWN | FCAP_SAVE_NON_NETWORKABLE));
}
...
IPhysicsObject *CPSCollisionEntity::VPhysicsGetObject(void)
{
	...
	if(m_pOwningSimulator->m_DataAccess.Simulation.Static.World.Brushes.pPhysicsObject != NULL)
		...
		return m_pOwningSimulator->m_DataAccess.Simulation.Static.Wall.Local.Brushes.pPhysicsObject;
	else if(m_pOwningSimulator->m_DataAccess.Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pPhysicsObject !=
			NULL)
...
int CPSCollisionEntity::VPhysicsGetObjectList(IPhysicsObject **pList, int listMax)
{
	...
	{
		pList[iRetVal] =
			m_pOwningSimulator->m_DataAccess.Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pPhysicsObject;
		++iRetVal;
		if(iRetVal == listMax)
			return iRetVal;
	}
...
static void PortalSimulatorDumps_DumpBoxToGlView(const Vector &vMins, const Vector &vMaxs, float fRed, float fGreen,
												 float fBlue, const char *pszFileName);
static void PortalSimulatorDumps_DumpOBBoxToGlView(const Vector &ptOrigin, const Vector &vExtent1,
												   const Vector &vExtent2, const Vector &vExtent3, float fRed,
												   float fGreen, float fBlue, const char *pszFileName);
...
void DumpActiveCollision(const CPortalSimulator *pPortalSimulator, const char *szFileName)
{
	...
	// color coding scheme, static prop collision is brighter than brush collision. Remote world stuff transformed to
	// the local wall is darker than completely local stuff
...
#define PSDAC_INTENSITY_REMOTEPROP	0.5f
	...
	if(pPortalSimulator->m_DataAccess.Simulation.Static.World.Brushes.pCollideable)
		PortalSimulatorDumps_DumpCollideToGlView(
			pPortalSimulator->m_DataAccess.Simulation.Static.World.Brushes.pCollideable, vec3_origin, vec3_angle,
			PSDAC_INTENSITY_LOCALBRUSH, szFileName);

	if(pPortalSimulator->m_DataAccess.Simulation.Static.World.StaticProps.bCollisionExists)
	{
		for(int i = pPortalSimulator->m_DataAccess.Simulation.Static.World.StaticProps.ClippedRepresentations.Count();
			--i >= 0;)
		{
			Assert(
				pPortalSimulator->m_DataAccess.Simulation.Static.World.StaticProps.ClippedRepresentations[i].pCollide);
			PortalSimulatorDumps_DumpCollideToGlView(
				pPortalSimulator->m_DataAccess.Simulation.Static.World.StaticProps.ClippedRepresentations[i].pCollide,
	...
	if(pPortalSimulator->m_DataAccess.Simulation.Static.Wall.Local.Brushes.pCollideable)
		PortalSimulatorDumps_DumpCollideToGlView(
			pPortalSimulator->m_DataAccess.Simulation.Static.Wall.Local.Brushes.pCollideable, vec3_origin, vec3_angle,
			PSDAC_INTENSITY_LOCALBRUSH, szFileName);

	if(pPortalSimulator->m_DataAccess.Simulation.Static.Wall.Local.Tube.pCollideable)
		PortalSimulatorDumps_DumpCollideToGlView(
			pPortalSimulator->m_DataAccess.Simulation.Static.Wall.Local.Tube.pCollideable, vec3_origin, vec3_angle,
			PSDAC_INTENSITY_LOCALBRUSH, szFileName);

	// if( pPortalSimulator->m_DataAccess.Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pCollideable )
	//	PortalSimulatorDumps_DumpCollideToGlView(
	//pPortalSimulator->m_DataAccess.Simulation.Static.Wall.RemoteTransformedToLocal.Brushes.pCollideable, vec3_origin,
	//vec3_angle, PSDAC_INTENSITY_REMOTEBRUSH, szFileName );
	...
	if(pLinkedPortal)
	{
		if(pLinkedPortal->m_DataAccess.Simulation.Static.World.Brushes.pCollideable)
			PortalSimulatorDumps_DumpCollideToGlView(
				pLinkedPortal->m_DataAccess.Simulation.Static.World.Brushes.pCollideable,
				pPortalSimulator->m_DataAccess.Placement.ptaap_LinkedToThis.ptOriginTransform,
				pPortalSimulator->m_DataAccess.Placement.ptaap_LinkedToThis.qAngleTransform,
		...
		{
			for(int i = pLinkedPortal->m_DataAccess.Simulation.Static.World.StaticProps.ClippedRepresentations.Count();
				--i >= 0;)
			{
				Assert(
					pLinkedPortal->m_DataAccess.Simulation.Static.World.StaticProps.ClippedRepresentations[i].pCollide);
				PortalSimulatorDumps_DumpCollideToGlView(
					pLinkedPortal->m_DataAccess.Simulation.Static.World.StaticProps.ClippedRepresentations[i].pCollide,
					pPortalSimulator->m_DataAccess.Placement.ptaap_LinkedToThis.ptOriginTransform,
					pPortalSimulator->m_DataAccess.Placement.ptaap_LinkedToThis.qAngleTransform,
	...
	DEBUGTIMERONLY(
		DevMsg(2, "[PSDT:%d] %sCPortalSimulator::DumpActiveCollision() Spent %fms generating a collision dump\n",
...
static void PortalSimulatorDumps_DumpPlanesToGlView(float *pPlanes, int iPlaneCount, const char *pszFileName)
{
	...
	for(int i = 0; i < iPlaneCount; ++i)
	{
		...
		filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[3].x, vPlaneVerts[3].y,
							vPlaneVerts[3].z, fRed, fGreen, fBlue);
		filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[2].x, vPlaneVerts[2].y,
							vPlaneVerts[2].z, fRed, fGreen, fBlue);
		filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[1].x, vPlaneVerts[1].y,
							vPlaneVerts[1].z, fRed, fGreen, fBlue);
		filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", vPlaneVerts[0].x, vPlaneVerts[0].y,
							vPlaneVerts[0].z, fRed, fGreen, fBlue);
	}
												   ...
												   float fGreen, float fBlue, const char *pszFileName)
{
	...
	for(counter = 0; counter != 8; ++counter)
	{
		...
		if(counter & (1 << 0))
			ptExtents[counter] += vExtent1;
		if(counter & (1 << 1))
			ptExtents[counter] += vExtent2;
		if(counter & (1 << 2))
			ptExtents[counter] += vExtent3;
	}

	// x min side
	filesystem->FPrintf(fp, "4\n");
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[0].x, ptExtents[0].y, ptExtents[0].z, fRed,
						fGreen, fBlue);
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[4].x, ptExtents[4].y, ptExtents[4].z, fRed,
						fGreen, fBlue);
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[6].x, ptExtents[6].y, ptExtents[6].z, fRed,
						fGreen, fBlue);
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[2].x, ptExtents[2].y, ptExtents[2].z, fRed,
						fGreen, fBlue);
	...
	// x max side
	filesystem->FPrintf(fp, "4\n");
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[1].x, ptExtents[1].y, ptExtents[1].z, fRed,
						fGreen, fBlue);
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[5].x, ptExtents[5].y, ptExtents[5].z, fRed,
						fGreen, fBlue);
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[7].x, ptExtents[7].y, ptExtents[7].z, fRed,
						fGreen, fBlue);
	filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", ptExtents[3].x, ptExtents[3].y, ptExtents[3].z, fRed,
						fGreen, fBlue);
