...
#define PORTAL_SIMULATORS_EMBED_GUID // define this to embed a unique integer with each portal simulator for debugging
									 // purposes

struct StaticPropPolyhedronGroups_t // each static prop is made up of a group of polyhedrons, these help us pull those
									// groups from an array
{
	int iStartIndex;
	int iNumPolyhedrons;
};
...
enum PortalSimulationEntityFlags_t
{
	...
	PSEF_CLONES_ENTITY_FROM_MAIN =
		(1 << 3), // entity is close enough to the portal to affect objects intersecting the portal
	// PSEF_HAS_LINKED_CLONE = (1 << 1), //this environment has a clone of the entity which is transformed from its
	// linked portal
};
...
struct PortalTransformAsAngledPosition_t // a matrix transformation from this portal to the linked portal, stored as
										 // vector and angle transforms
{
	Vector ptOriginTransform;
	QAngle qAngleTransform;
};

inline bool LessFunc_Integer(const int &a, const int &b)
{
	return a < b;
};
...
struct PS_PlacementData_t // stuff useful for geometric operations
{
	...
	CPhysCollide *pHoleShapeCollideable; // used to test if a collideable is in the hole, should NOT be collided against
										 // in general
...
struct PS_SD_Static_Wall_RemoteTransformedToLocal_t // things taken from the linked portal's "World" collision and
...
{
	CUtlVector<CBaseEntity *>
		ShouldCloneFromMain; // a list of entities that should be cloned from main if physics simulation is enabled
...
public:
	...
	void DetachFromLinked(
		void); // detach portals to sever the connection, saves work when planning on moving both portals
	...
	void SetCollisionGenerationEnabled(bool bEnabled); // enable/disable collision generation for the hole in the wall,
													   // needed for proper vphysics simulation
	bool IsCollisionGenerationEnabled(void) const;

	void SetVPhysicsSimulationEnabled(bool bEnabled); // enable/disable vphysics simulation. Will automatically update
													  // the linked portal to be the same
	bool IsSimulatingVPhysics(void) const; // this portal is setup to handle any physically simulated object, false
										   // means the portal is handling player movement only

	bool EntityIsInPortalHole(CBaseEntity *pEntity) const; // true if the entity is within the portal cutout bounds and
														   // crossing the plane. Not just *near* the portal
	bool EntityHitBoxExtentIsInPortalHole(
		CBaseAnimating *pBaseAnimating) const; // true if the entity is within the portal cutout bounds and crossing the
											   // plane. Not just *near* the portal
	void RemoveEntityFromPortalHole(
		CBaseEntity *pEntity); // if the entity is in the portal hole, this forcibly moves it out by any means possible

	bool RayIsInPortalHole(const Ray_t &ray)
		const; // traces a ray against the same detector for EntityIsInPortalHole(), bias is towards false positives

#ifndef CLIENT_DLL
	int GetMoveableOwnedEntities(CBaseEntity **pEntsOut,
								 int iEntOutLimit); // gets owned entities that aren't either world or static props.
													// Excludes fake portal ents such as physics clones

	static CPortalSimulator *GetSimulatorThatOwnsEntity(const CBaseEntity *pEntity); // fairly cheap to call
	static CPortalSimulator *GetSimulatorThatCreatedPhysicsObject(const IPhysicsObject *pObject,
																  PS_PhysicsObjectSourceType_t *pOut_SourceType = NULL);
	...
	void ReleaseOwnershipOfEntity(CBaseEntity *pEntity,
								  bool bMovingToLinkedSimulator =
									  false); // if bMovingToLinkedSimulator is true, the code skips some steps that are
											  // going to be repeated when the entity is added to the other simulator
	...
	bool CreatedPhysicsObject(const IPhysicsObject *pObject, PS_PhysicsObjectSourceType_t *pOut_SourceType = NULL)
		const; // true if the physics object was generated by this portal simulator
	...
	int GetPortalSimulatorGUID(void) const
	{
		return m_iPortalSimulatorGUID;
	};
#endif

protected:
	bool m_bLocalDataIsReady; // this side of the portal is properly setup, no guarantees as to linkage to another
							  // portal
	bool m_bSimulateVPhysics;
	bool m_bGenerateCollision;
	bool m_bSharedCollisionConfiguration; // when portals are in certain configurations, they need to cross-clip and
										  // share some collision data and things get nasty. For the love of all that is
										  // holy, pray that this is false.
	CPortalSimulator *m_pLinkedPortal;
	bool m_bInCrossLinkedFunction; // A flag to mark that we're already in a linked function and that the linked portal
								   // shouldn't call our side
	...
	void ReleasePhysicsOwnership(CBaseEntity *pEntity, bool bContinuePhysicsCloning = true,
