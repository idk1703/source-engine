...
ConVar sv_player_trace_through_portals("sv_player_trace_through_portals", "1", FCVAR_REPLICATED | FCVAR_CHEAT,
									   "Causes player movement traces to trace through portals.");
ConVar sv_player_funnel_into_portals("sv_player_funnel_into_portals", "1",
									 FCVAR_REPLICATED | FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
...
void CPortalGameMovement::FunnelIntoPortal(CProp_Portal *pPortal, Vector &wishdir)
{
	...
	// Make sure the player is trying to air control, they're falling downward and they are vertically close to the
	// portal
	if(fabsf(wishdir[0]) > 64.0f || fabsf(wishdir[1]) > 64.0f || mv->m_vecVelocity[2] > -165.0f ||
	   vPlayerToPortal.z < -512.0f)
		return;
...
void CPortalGameMovement::AirMove(void)
{
	...
	//
	if(mv->m_vecVelocity[0] * mv->m_vecVelocity[0] + mv->m_vecVelocity[1] * mv->m_vecVelocity[1] >
	   MIN_FLING_SPEED * MIN_FLING_SPEED)
	{
...
void CPortalGameMovement::PlayerRoughLandingEffects(float fvol)
{
	...
	if(fvol >= 1.0)
	{
		...
		if(CBaseEntity::GetParametersForSound("PortalPlayer.FallRecover", params, NULL))
		{
			...
			ep.m_flVolume =
				MIN(player->m_Local.m_flFallVelocity * 0.00075f - 0.38, 1.0f); // louder the harder they land
...
void TracePlayerBBoxForGround2(const Vector &start, const Vector &end, const Vector &minsSrc, const Vector &maxsSrc,
...
void CPortalGameMovement::CategorizePosition(void)
{
	...
	else
	{
		...
		if(pm.plane.normal[2] < 0.7)
		{
			...
			TracePlayerBBoxForGround2(bumpOrigin, point, GetPlayerMins(), GetPlayerMaxs(), mv->m_nPlayerHandle.Get(),
...
int CPortalGameMovement::CheckStuck(void)
{
	if(BaseClass::CheckStuck())
	{
		...
		if(pPortalPlayer->m_hPortalEnvironment)
		{
			...
			{
				pPortalPlayer->SetAbsOrigin(
					ptOldOrigin +
					(vIndecisive * 5.0f)); // this is an anti-bug hack, since this would have probably popped them out
										   // of the world, we're just going to move them forward a few units
			}
		}
...
void CPortalGameMovement::TracePlayerBBox(const Vector &start, const Vector &end, unsigned int fMask,
...
{
	TracePlayerBBox(pos, pos, MASK_PLAYERSOLID, collisionGroup,
					pm); // hook into the existing portal special trace functionality
	...
	{
#ifdef _DEBUG
		AssertMsgOnce(false, "The player got stuck on something. Break to investigate."); // happens enough to just
																						  // leave in a perma-debugger
		// this next trace is PURELY for tracking down how the player got stuck. Nothing new is discovered over the same
		// trace about 10 lines up
