...
int CSchGameAccount::GetITable() const
{
	return k_iTable;
}
CSchGameAccount::CSchGameAccount(const CSchGameAccount &that)
{
	*this = that;
}
void CSchGameAccount::operator=(const CSchGameAccount &that)
{
	CRecordBase::operator=(that);
}
...
int CSchGameAccountClient::GetITable() const
{
	return k_iTable;
}
CSchGameAccountClient::CSchGameAccountClient(const CSchGameAccountClient &that)
{
	*this = that;
}
void CSchGameAccountClient::operator=(const CSchGameAccountClient &that)
{
	CRecordBase::operator=(that);
}
...
namespace PORTAL_GCSCHEMA
{
	void GenerateIntrinsicSQLSchema(GCSDK::CSchemaFull &schemaFull)
	{
		...
		pSchema->CalcOffsets();
		schemaFull.CheckSchema(pSchema, CSchGameAccount::k_iFieldMax,
							   sizeof(CSchGameAccount) - sizeof(GCSDK::CRecordBase));
		...
		pSchema->SetBAllowWipeTableInProd(false);
		pSchema->CalcOffsets();
		schemaFull.CheckSchema(pSchema, CSchGameAccountClient::k_iFieldMax,
							   sizeof(CSchGameAccountClient) - sizeof(GCSDK::CRecordBase));
		pSchema->PrepareForUse();
