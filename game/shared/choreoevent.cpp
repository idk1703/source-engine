...
const char *CChoreoEvent::GetDescription(void)
{
	...
	{
		Assert(m_pChannel);
		Q_snprintf(description, sizeof(description), "%s : %s : %s -- %s \"%s\"", m_pActor->GetName(),
				   m_pChannel->GetName(), GetName(), NameForType(GetType()), GetParameters());
...
public:
	CCheckEventNames()
	{
		...
		{
			Error("g_NameMap contains %llu entries, CChoreoEvent::NUM_TYPES == %i!", (uint64)(ARRAYSIZE(g_NameMap)),
		...
		for(int i = 0; i < CChoreoEvent::NUM_TYPES; ++i)
		{
			...
			Error("g_NameMap:  Event type at %i has wrong value (%i)!", i, (int)g_NameMap[i].type);
		}
	}
};
...
public:
	CCheckCCNames()
	{
		...
		for(int i = 0; i < CChoreoEvent::NUM_CC_TYPES; ++i)
		{
			...
			Error("g_CCNameMap:  Event type at %i has wrong value (%i)!", i, (int)g_CCNameMap[i].type);
		}
	}
};
...
float CChoreoEvent::GetOriginalPercentageFromPlaybackPercentage(float t)
{
	...
	Vector vStart(GetBoundedAbsoluteTagPercentage(PLAYBACK, start), GetBoundedAbsoluteTagPercentage(ORIGINAL, start),
				  0);
...
float CChoreoEvent::GetPlaybackPercentageFromOriginalPercentage(float t)
{
	...
	Vector vStart(GetBoundedAbsoluteTagPercentage(ORIGINAL, start), GetBoundedAbsoluteTagPercentage(PLAYBACK, start),
				  0);
...
void CChoreoEvent::SaveToBuffer(CUtlBuffer &buf, CChoreoScene *pScene, IChoreoStringPool *pStringPool)
{
	...
	if(GetType() == CChoreoEvent::SPEAK)
	{
		...
		if(GetCloseCaptionType() != CChoreoEvent::CC_DISABLED && IsUsingCombinedFile())
		{
			flags |= (1 << 0);
		}
