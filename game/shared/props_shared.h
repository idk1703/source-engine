...
#define SF_PHYSPROP_MOTIONDISABLED \
	0x000008 // motion disabled at startup (flag only valid in spawn - motion can be enabled via input)
...
#define SF_PHYSPROP_ENABLE_PICKUP_OUTPUT \
	0x000100 // If set, allow the player to +USE this for the purposes of generating an output
...
#define SF_PHYSPROP_PREVENT_PLAYER_TOUCH_ENABLE \
	0x000400 // If set, the player will not cause the object to enable its motion when bumped into
...
#define SF_PHYSPROP_RADIUS_PICKUP \
	0x004000 // For Xbox, makes small objects easier to pick up by allowing them to be found
#define SF_PHYSPROP_ALWAYS_PICK_UP \
	0x100000 // Physcannon can always pick this up, no matter what mass or constraints may apply.
...
{
	breakablepropparams_t(const Vector &_origin, const QAngle &_angles, const Vector &_velocity,
...
void PropBreakableCreateAll(int modelindex, IPhysicsObject *pPhysics, const Vector &origin, const QAngle &angles,
							...
							float burstScale, int collisionGroup, CBaseEntity *pEntity = NULL,
							bool defaultLocation = true);
...
CBaseEntity *CreateGibsFromList(CUtlVector<breakmodel_t> &list, int modelindex, IPhysicsObject *pPhysics,
								...
								int iPrecomputedBreakableCount, bool bIgnoreGibLImit, bool defaultLocation = true,
								CUtlVector<EHANDLE> *pGibList = NULL, bool bBurning = false);
