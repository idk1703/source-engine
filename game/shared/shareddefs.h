...
public:
	...
	CViewVectors(Vector vView, Vector vHullMin, Vector vHullMax, Vector vDuckHullMin, Vector vDuckHullMax,
...
// The Source engine is really designed for 32 or less players.  If you raise this number above 32, you better know what
// you are doing
//  and have a good answer for a bunch of perf question related to player simulation, thinking logic, tracelines,
//  networking overhead, etc.
// But if you are brave or are doing something interesting, go for it...   ywb 9/22/03

// You might be wondering why these aren't multiple of 2. Well the reason is that if servers decide to have HLTV or
// Replay enabled we need the extra slot. This is ok since MAX_PLAYERS is used for code specific things like arrays and
// loops, but it doesn't really means that this is the max number of players allowed Since this is decided by the
// gamerules (and it can be whatever number as long as its less than MAX_PLAYERS).
...
#define PLAYER_FALL_PUNCH_THRESHOLD \
	303.0f // won't punch player's screen/make scrape noise unless player falling at least this fast - at least a 76"
		   // fall (sqrt( 2 * g * 76))
#else
...
#define DMG_CRUSH \
	(1 << 0) // crushed by falling or moving object.
			 // NOTE: It's assumed crush damage is occurring as a result of physics collision, so no extra physics force
			 // is generated by crush damage. DON'T use DMG_CRUSH when damaging entities unless it's the result of a
			 // physics collision. You probably want DMG_CLUB instead.
...
#define DMG_REMOVENORAGDOLL \
	(1 << 22) // with this bit OR'd in, no ragdoll will be created, and the target will be quietly removed.
...
enum
{
	...
	OBS_MODE_POI, // PASSTIME point of interest - game objective, big fight, anything interesting; added in the middle
				  // of the enum due to tons of hard-coded "<ROAMING" enum compares
...
{
	EFL_KILLME =
		(1 << 0), // This entity is marked for death -- This allows the game to actually delete ents at a safe time
	EFL_DORMANT = (1 << 1),			 // Entity is dormant, no updates to client
	EFL_NOCLIP_ACTIVE = (1 << 2),	 // Lets us know when the noclip command is active.
	EFL_SETTING_UP_BONES = (1 << 3), // Set while a model is setting up its bones.
	EFL_KEEP_ON_RECREATE_ENTITIES =
		(1 << 4), // This is a special entity that should not be deleted when we restart entities only
	...
	EFL_USE_PARTITION_WHEN_NOT_SOLID =
		(1 << 18),					// Entities with this flag set show up in the partition even when not solid
...
enum FireBulletsFlags_t
{
	...
	FIRE_BULLETS_TEMPORARY_DANGER_SOUND =
		0x8, // Danger sounds added from this impact can be stomped immediately if another is queued
};
...
struct FireBulletsInfo_t
{
	...
	FireBulletsInfo_t(int nShots, const Vector &vecSrc, const Vector &vecDir, const Vector &vecSpread, float flDistance,
...
struct EmitSound_t
{
	...
	float m_flSoundTime; ///< NOT DURATION, but rather, some absolute time in the future until which this sound should
						 ///< be delayed
	...
	mutable CUtlVector<Vector>
		m_UtlVecSoundOrigin; ///< Actual sound origin(s) (can be multiple if sound routed through speaker entity(ies) )
...
// YWB:  3/12/2007
// Changing the following #define for Prediction Error checking (See gamemovement.cpp for overview) will to 1 or 2
// enables the system, 0 turns it off Level 1 enables it, but doesn't force "full precision" networking, so you can
// still get lots of errors in position/velocity/etc. Level 2 enables it but also forces origins/angles to be sent full
// precision, so other fields can be error / tolerance checked NOTE:  This stuff only works on a listen server since it
// punches a hole from the client .dll to server .dll!!!
