...
BEGIN_DATADESC_NO_BASE(CCollisionProperty)
	...
	DEFINE_GLOBAL_FIELD(m_vecMinsPreScaled, FIELD_VECTOR), DEFINE_GLOBAL_FIELD(m_vecMaxsPreScaled, FIELD_VECTOR),
		DEFINE_GLOBAL_FIELD(m_vecMins, FIELD_VECTOR), DEFINE_GLOBAL_FIELD(m_vecMaxs, FIELD_VECTOR),
		DEFINE_KEYFIELD(m_nSolidType, FIELD_CHARACTER, "solid"), DEFINE_FIELD(m_usSolidFlags, FIELD_SHORT),
		DEFINE_FIELD(m_nSurroundType, FIELD_CHARACTER), DEFINE_FIELD(m_flRadius, FIELD_FLOAT),
		...
		DEFINE_FIELD(m_vecSpecifiedSurroundingMaxs, FIELD_VECTOR), DEFINE_FIELD(m_vecSurroundingMins, FIELD_VECTOR),
...
static void SendProxy_Solid(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
							int objectID)
{
	pOut->m_Int = ((CCollisionProperty *)pStruct)->GetSolid();
}

static void SendProxy_SolidFlags(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut,
...
BEGIN_NETWORK_TABLE_NOBASE(CCollisionProperty, DT_CollisionProperty)
	...
	RecvPropVector(RECVINFO(m_vecMinsPreScaled), 0, RecvProxy_OBBMinsPreScaled),
		...
		RecvPropVector(RECVINFO(m_vecMins), 0), RecvPropVector(RECVINFO(m_vecMaxs), 0),
	...
	SendPropVector(SENDINFO(m_vecMinsPreScaled), 0, SPROP_NOSCALE),
		...
		SendPropVector(SENDINFO(m_vecMins), 0, SPROP_NOSCALE), SendPropVector(SENDINFO(m_vecMaxs), 0, SPROP_NOSCALE),
...
void CCollisionProperty::RefreshScaledCollisionBounds(void)
{
	...
	{
		SetSurroundingBoundsType(nSurroundType, &(m_vecSpecifiedSurroundingMinsPreScaled.Get()),
...
//-----------------------------------------------------------------------------
void CCollisionProperty::RandomPointInBounds(const Vector &vecNormalizedMins, const Vector &vecNormalizedMaxs,
											 Vector *pPoint) const
{
...
//-----------------------------------------------------------------------------
void CCollisionProperty::CollisionAABBToWorldAABB(const Vector &entityMins, const Vector &entityMaxs,
												  Vector *pWorldMins, Vector *pWorldMaxs) const
{
...
/*
void CCollisionProperty::WorldAABBToCollisionAABB( const Vector &worldMins, const Vector &worldMaxs, Vector
*pEntityMins, Vector *pEntityMaxs ) const
{
	if ( !IsBoundsDefinedInEntitySpace() || (GetCollisionAngles() == vec3_angle) )
	{
...
void CCollisionProperty::ComputeVPhysicsSurroundingBox(Vector *pVecWorldMins, Vector *pVecWorldMaxs)
{
	...
	if(pPhysicsObject)
	{
		...
		{
			physcollision->CollideGetAABB(pVecWorldMins, pVecWorldMaxs, pPhysicsObject->GetCollide(),
										  GetCollisionOrigin(), GetCollisionAngles());
			bSetBounds = true;
		}
...
void CCollisionProperty::ComputeSurroundingBox(Vector *pVecWorldMins, Vector *pVecWorldMaxs)
{
	...
	switch(m_nSurroundType)
	{
		...
		Assert( vecTestMins.x >= pVecWorldMins->x && vecTestMins.y >= pVecWorldMins->y && vecTestMins.z >=
		pVecWorldMins->z ); Assert( vecTestMaxs.x <= pVecWorldMaxs->x && vecTestMaxs.y <= pVecWorldMaxs->y &&
...
//-----------------------------------------------------------------------------
void CCollisionProperty::SetSurroundingBoundsType(SurroundingBoundsType_t type, const Vector *pMins,
												  const Vector *pMaxs)
{
...
bool CCollisionProperty::DoesVPhysicsInvalidateSurroundingBox() const
{
	switch(m_nSurroundType)
	{
		...
		case USE_OBB_COLLISION_BOUNDS:
			return (GetSolid() == SOLID_VPHYSICS) && (GetOuter()->GetMoveType() == MOVETYPE_VPHYSICS) &&
				   GetOuter()->VPhysicsGetObject();
