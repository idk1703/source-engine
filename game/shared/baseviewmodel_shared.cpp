...
void CBaseViewModel::CalcViewModelView(CBasePlayer *owner, const Vector &eyePosition, const QAngle &eyeAngles)
{
...
#if !defined(CSTRIKE_DLL)
	// This was causing weapon jitter when rotating in updated CS:S; original Source had this in above InPrediction
	// block  07/14/10 Add lag
	CalcViewModelLag(vmorigin, vmangles, vmangoriginal);
#endif
	...
	if(UseVR())
	{
		g_ClientVirtualReality.OverrideViewModelTransform(vmorigin, vmangles,
														  pWeapon && pWeapon->ShouldUseLargeViewModelVROverride());
	}
#endif
...
void CBaseViewModel::CalcViewModelLag(Vector &origin, QAngle &angles, QAngle &original_angles)
{
	...
	if(gpGlobals->frametime != 0.0f)
	{
		...
		// If we start to lag too far behind, we'll increase the "catch up" speed.  Solves the problem with fast
		// cl_yawspeed, m_yaw or joysticks
		//  rotating quickly.  The old code would slam lastfacing with origin causing the viewmodel to pop to a new
		//  position
...
#if !defined(CLIENT_DLL)
	SendPropModelIndex(SENDINFO(m_nModelIndex)), SendPropInt(SENDINFO(m_nBody), 8), SendPropInt(SENDINFO(m_nSkin), 10),
		...
		SendPropInt(SENDINFO(m_nAnimationParity), 3, SPROP_UNSIGNED), SendPropEHandle(SENDINFO(m_hWeapon)),
...
#endif
#else
	RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropInt(RECVINFO(m_nSkin)), RecvPropInt(RECVINFO(m_nBody)),
		RecvPropInt(RECVINFO(m_nSequence), 0, RecvProxy_SequenceNum), RecvPropInt(RECVINFO(m_nViewModelIndex)),
		RecvPropFloat(RECVINFO(m_flPlaybackRate)), RecvPropInt(RECVINFO(m_fEffects), 0, RecvProxy_EffectFlags),
		RecvPropInt(RECVINFO(m_nAnimationParity)), RecvPropEHandle(RECVINFO(m_hWeapon), RecvProxy_Weapon),
...
BEGIN_PREDICTION_DATA(CBaseViewModel)
	...
	DEFINE_PRED_FIELD(m_nModelIndex, FIELD_SHORT, FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
		...
		DEFINE_FIELD(m_hOwner, FIELD_EHANDLE), DEFINE_FIELD(m_flTimeWeaponIdle, FIELD_FLOAT),
