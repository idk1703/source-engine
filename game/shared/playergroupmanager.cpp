...
CPlayerGroupManager::CPlayerGroupManager() : m_hashPlayerGroupIDLocks(k_nLocksRunInterval / k_cMicroSecPerShellFrame)
{
...
void CPlayerGroupManager::MemcachedUpdateAllMemberAssocation(IPlayerGroup *pPlayerGroup)
{
	...
	for(int i = 0; i < pPlayerGroup->GetNumMembers(); ++i)
	{
		...
		memberKey.Format("GC:%u:%sMEMBER:%llu", GGCBase()->GetAppID(), GetMemcachedIdentityKey(),
						 pPlayerGroup->GetMember(i).ConvertToUint64());
...
bool CPlayerGroupManager::BYldAddMemberToGroup(PlayerGroupID_t nPlayerGroupID, const CSteamID &steamIDNewMember)
{
	...
	if(pOldPlayerGroup)
	{
		EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
				 "BYldAddMemberToGroup not adding member: %s to player group: %016llx as he's already in a group\n",
	...
	{
		EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
				 "BYldAddMemberToGroup not adding member: %s to player group: %016llx as that group does not exist\n",
				 steamIDNewMember.Render(), nPlayerGroupID);
		return false;
	}
	...
	{
		EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
				 "BYldAddMemberToGroup not adding member: %s to player group: %016llx as he's already in the group\n",
				 steamIDNewMember.Render(), nPlayerGroupID);
	...
	{
		EmitInfo(
			SPEW_GC, 4, LOG_ALWAYS,
			"BYldAddMemberToGroup not adding member: %s to player group: %016llx as his cache could not be loaded\n",
			steamIDNewMember.Render(), nPlayerGroupID);
	...
	// If member is already in a group, abort (must do this again after yielding as another job could have put him in a
	// group while we yielded)
	pOldPlayerGroup = FindGroupByMemberID(steamIDNewMember);
	if(pOldPlayerGroup)
	{
		EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
				 "BYldAddMemberToGroup not adding member: %s to player group: %016llx as he's already in a group (2)\n",
				 steamIDNewMember.Render(), nPlayerGroupID);
		GGCBase()->UnlockSteamID(steamIDNewMember);
		UnlockGroupID(nPlayerGroupID);
		return false;
	...
	EmitInfo(SPEW_GC, 4, LOG_ALWAYS, "Steam id: %s added to player group: %016llx\n", steamIDNewMember.Render(),
			 nPlayerGroupID);

	return true;
}

void CPlayerGroupManager::YldRemoveMemberFromGroup(PlayerGroupID_t nPlayerGroupID,
												   const CSteamID &steamIDRemovingMember)
{
	// check group exists
	IPlayerGroup *pPlayerGroup = YldFindAndLockGroup(nPlayerGroupID);
	Assert(pPlayerGroup);
...
bool CPlayerGroupManager::BExpireLocks(CLimitTimer &limitTimer)
{
	...
	for(CLock *pLock = m_hashPlayerGroupIDLocks.PvRecordRun(); NULL != pLock;
		pLock = m_hashPlayerGroupIDLocks.PvRecordRun())
	{
...
void CPlayerGroupManager::YldDestroyGroup(PlayerGroupID_t nPlayerGroupID)
{
	...
	GGCBase()->BMemcachedDelete(
		CFmtStr("GC:%u:%s:%llu", GGCBase()->GetAppID(), GetMemcachedIdentityKey(), nPlayerGroupID).Access());
...
void CPlayerGroupManager::SendGroupStorageAndNetworkUpdate(IPlayerGroup *pPlayerGroup)
{
	...
	memcachedGroupKey.Format("GC:%u:%s:%llu", GGCBase()->GetAppID(), GetMemcachedIdentityKey(),
							 pPlayerGroup->GetGroupID());
...
void CPlayerGroupManager::YldDoFindGroupFromMemcached(const CSteamID &memberSteamID)
{
	...
	memcachedGroupResult.SetExternalBuffer(memcachedMemberResults[0].m_bufValue.Base(),
										   memcachedMemberResults[0].m_bufValue.Count(),
										   memcachedMemberResults[0].m_bufValue.Count());
	PlayerGroupID_t nPlayerGroupID = (uint64)memcachedGroupResult.GetInt64();

	if(!BYieldingLockGroupID(nPlayerGroupID))
	{
		EmitInfo(SPEW_GC, SPEW_ALWAYS, LOG_ALWAYS,
				 "YldFindGroupFromMemcached: Unable to create lock for PlayerGroup:%u\n", nPlayerGroupID);
		return;
	}
	...
	memcachedPartyResult.SetExternalBuffer(memcachedGroupResults[0].m_bufValue.Base(),
										   memcachedGroupResults[0].m_bufValue.Count(),
										   memcachedGroupResults[0].m_bufValue.Count());
	IPlayerGroup *pPlayerGroup = YldCreateAndLockPlayerGroupFromMemcached(memcachedPartyResult);

	// pPlayerGroup won't be NULL because creating it won't fail. Also, the group id is already locked, so only one
	// player will get to this point. So no checks needed before inserting this group id into the map
...
{
	if(!steamIDLeader.IsValid() || !steamIDLeader.BIndividualAccount() || !steamIDNewMember.IsValid() ||
	   !steamIDNewMember.BIndividualAccount())
		return;
...
void CPlayerGroupManager::YldGroupInviteResponse(const CSteamID &steamID, const PlayerGroupID_t nPlayerGroupID,
												 bool bAccepted)
{
	...
	{
		CProtoBufMsg<CMsgGCError> msg(k_EMsgGCError);
		msg.Body().set_error_text(
			CFmtStr("Couldn't respond to party invite, couldn't find a party with ID %llu.", nPlayerGroupID));
	...
	if(pPlayerGroup->GetPendingInviteIndexBySteamID(steamID) == -1)
	{
		CProtoBufMsg<CMsgGCError> msg(k_EMsgGCError);
		msg.Body().set_error_text(
			CFmtStr("Couldn't respond to party invite, that party (%llu) doesn't have an invite pending for you.",
					nPlayerGroupID));
		GGCBase()->BSendGCMsgToClient(steamID, msg);
		return;
	}
...
{
	if(!steamIDLeader.IsValid() || !steamIDLeader.BIndividualAccount() || !steamIDVictim.IsValid() ||
	   !steamIDVictim.BIndividualAccount())
		return;
...
bool CPlayerGroupManager::BYldRequestLeaveGroup(const CSteamID &steamID)
{
	...
	if(!pPlayerGroup)
	{
		CProtoBufMsg<CMsgGCError> msg(k_EMsgGCError);
		msg.Body().set_error_text(
			CFmtStr("%s: Failed to remove you from your group.  Couldn't find your group in the group map.",
...
void CPlayerGroupManager::YldOnGroupDestroyed(IPlayerGroup *pPlayerGroup)
{
	...
	EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
			 "CPlayerGroupManager::YldOnGroupDestroyed [%s] Removing all pending invites %016llx\n",
			 GetMemcachedIdentityKey(), nPlayerGroupID);
	for(int i = pPlayerGroup->GetNumPendingInvites() - 1; i >= 0;
		i--) // iterate backwards as pending invites will be removed from the list
	{
