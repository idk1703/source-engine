...
ConVar option_duck_method("option_duck_method", "1",
						  FCVAR_REPLICATED | FCVAR_ARCHIVE); // 0 = HOLD to duck, 1 = Duck is a toggle
...
// If you ever get stuck walking around, then you can run this code to find the code which would leave the player in a
// bad spot
...
{
public:
	CDiffManager() : m_Client(0, 0, CDiffInfo::Less), m_Server(0, 0, CDiffInfo::Less), m_flLastSpew(-1.0f)
	{
		g_pDiffMgr = this;
	}
	...
	virtual void Validate(bool bServer, int nCommandNumber)
	{
		...
		if(eMisMatched == DIFFCHECK_DIFFERS)
		{
			...
			do
			{
				...
				if(m_Server.IsValidIndex(prev) && ClientRecordExists(m_Server[prev].m_nCommandNumber))
				{
	...
	void SpewRecords(char const *prefix, int svidx)
	{
		...
		for(int i = 0; i < maxSlot; ++i)
		{
			...
			Msg("%s%s%d:  sv[%50.50s] cl[%50.50s]\n", prefix, bDiffers ? "+++" : "   ", serverslot->m_nCommandNumber,
				sv, cl);
		}
	}
...
int CGameMovement::GetCheckInterval(IntervalType_t type)
{
	...
	switch(type)
	{
		...
		case GROUND:
			...
			break;
		case STUCK:
			// If we are in the process of being "stuck", then try a new position every command tick until m_StuckLast
			// gets reset back down to zero
...
void CGameMovement::CategorizeGroundSurface(trace_t &pm)
{
	...
	MoveHelper()->GetSurfaceProps()->GetPhysicsProperties(player->m_surfaceProps, NULL, NULL,
...
void CGameMovement::CheckParameters(void)
{
	   ...
	   player->GetMoveType() != MOVETYPE_OBSERVER)
	{
		...
		spd = (mv->m_flForwardMove * mv->m_flForwardMove) + (mv->m_flSideMove * mv->m_flSideMove) +
	...
	if(!IsDead())
	{
		...
		{
			mv->m_vecAngles[ROLL] =
				CalcRoll(v_angle, mv->m_vecVelocity, sv_rollangle.GetFloat(), sv_rollspeed.GetFloat());
		}
		else
		{
...
void CGameMovement::DecayPunchAngle(void)
{
	if(player->m_Local.m_vecPunchAngle->LengthSqr() > 0.001 || player->m_Local.m_vecPunchAngleVel->LengthSqr() > 0.001)
	{
		...
		// don't wrap around
		player->m_Local.m_vecPunchAngle.Init(clamp(player->m_Local.m_vecPunchAngle->x, -89.f, 89.f),
...
bool CGameMovement::CheckJumpButton(void)
{
	...
	if(gpGlobals->maxClients == 1)
	{
		...
		// We give a certain percentage of the current forward movement as a bonus to the jump speed.  That bonus is
		// clipped to not accumulate over time.
...
int CGameMovement::CheckStuck(void)
{
	...
	if(developer.GetBool())
	{
		...
		engine->Con_NPrintf(isServer, "%s stuck on object %i/%s", isServer ? "server" : "client",
...
//-----------------------------------------------------------------------------
void TracePlayerBBoxForGround(const Vector &start, const Vector &end, const Vector &minsSrc, const Vector &maxsSrc,
							  IHandleEntity *player, unsigned int fMask, int collisionGroup, trace_t &pm)
{
...
// the original trace hit first.
//-----------------------------------------------------------------------------
void CGameMovement::TryTouchGroundInQuadrants(const Vector &start, const Vector &end, unsigned int fMask,
				...
				else
				{
					...
					if(!(mv->m_nButtons & IN_DUCK))
					{
						...
						if(CanUnDuckJump(trace))
						{
							...
							player->m_Local.m_flDuckJumpTime = (GAMEMOVEMENT_TIME_TO_UNDUCK * (1.0f - trace.fraction)) +
...
void CGameMovement::PlayerMove(void)
{
	...
	if(player->GetMoveType() != MOVETYPE_NOCLIP && player->GetMoveType() != MOVETYPE_NONE &&
	   player->GetMoveType() != MOVETYPE_ISOMETRIC && player->GetMoveType() != MOVETYPE_OBSERVER &&
	   !player->pl.deadflag)
	{
	...
	if(player->GetMoveType() != MOVETYPE_WALK || mv->m_bGameCodeMovedPlayer || !sv_optimizedmovement.GetBool())
	{
		CategorizePosition();
	}
	else
...
//-----------------------------------------------------------------------------
void CGameMovement::TracePlayerBBox(const Vector &start, const Vector &end, unsigned int fMask, int collisionGroup,
									trace_t &pm)
{
	VPROF("CGameMovement::TracePlayerBBox");
...
//-----------------------------------------------------------------------------
void CGameMovement::TryTouchGround(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs,
