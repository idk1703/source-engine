...
void CPredictionCopy::DescribeFields(difftype_t dt, const char *fmt, ...)
{
	...
	(*m_FieldCompareFunc)(m_pCurrentClassName, fieldname, g_FieldTypes[m_pCurrentField->fieldType], isnetworked,
						  isnoterrorchecked, dt != IDENTICAL ? true : false, dt == WITHINTOLERANCE ? true : false,
						  data);
...
void CPredictionCopy::DescribeVector(difftype_t dt, Vector &outValue, const Vector &inValue)
{
	...
	if(dt == DIFFERS)
	{
		...
		ReportFieldsDiffer("vec differs (net %f %f %f - pred %f %f %f) delta(%f %f %f)\n", inValue.x, inValue.y,
						   inValue.z, outValue.x, outValue.y, outValue.z, delta.x, delta.y, delta.z);
	}
...
void CPredictionCopy::DescribeVector(difftype_t dt, Vector *outValue, const Vector *inValue, int count)
{
	...
	if(dt == DIFFERS)
	{
		...
		ReportFieldsDiffer("vec[] differs (1st diff) (net %f %f %f - pred %f %f %f) delta(%f %f %f)\n", inValue[i].x,
						   inValue[i].y, inValue[i].z, outValue[i].x, outValue[i].y, outValue[i].z, delta.x, delta.y,
						   delta.z);
	}
...
void CPredictionCopy::DescribeQuaternion(difftype_t dt, Quaternion &outValue, const Quaternion &inValue)
{
	...
	if(dt == DIFFERS)
	{
		...
		ReportFieldsDiffer("quaternion differs (net %f %f %f %f - pred %f %f %f %f) delta(%f %f %f %f)\n", inValue[0],
						   inValue[1], inValue[2], inValue[3], outValue[0], outValue[1], outValue[2], outValue[3],
...
void CPredictionCopy::DescribeQuaternion(difftype_t dt, Quaternion *outValue, const Quaternion *inValue, int count)
{
	...
	DescribeFields(dt, "quaternion (%f %f %f %f)\n", (float)outValue[0][0], (float)outValue[0][1],
				   (float)outValue[0][2], (float)outValue[0][3]);
}
...
void CPredictionCopy::WatchQuaternion(difftype_t dt, Quaternion &outValue, const Quaternion &inValue)
{
	...
	WatchMsg("quaternion (%f %f %f %f)", (float)outValue[0], (float)outValue[1], (float)outValue[2],
			 (float)outValue[3]);
}
...
CPredictionCopy::difftype_t CPredictionCopy::CompareVector(Vector &outValue, const Vector &inValue)
{
	...
	if(CanCheck())
	{
		...
		if(outValue != inValue && (tolerance > 0.0f))
		{
			...
			if(fabs(delta.x) <= tolerance && fabs(delta.y) <= tolerance && fabs(delta.z) <= tolerance)
			{
...
void CPredictionDescribeData::Describe(const char *fmt, ...)
{
	...
	if(m_FieldDescFunc)
	{
		(*m_FieldDescFunc)(m_pCurrentClassName, m_pCurrentField->fieldName, g_FieldTypes[m_pCurrentField->fieldType],
						   isnetworked, data);
	}
	else
	{
...
void CPredictionDescribeData::DescribeFields_R(int chain_count, datamap_t *pRootMap, typedescription_t *pFields,
											   int fieldCount)
{
	int i;
	int flags;
...
#if defined(CLIENT_DLL)
CValueChangeTracker::CValueChangeTracker() : m_bActive(false), m_bTracking(false)
{
...
void CValueChangeTracker::GetValue(char *buf, size_t bufsize)
{
	...
	{
		td = m_FieldStack[i];
		Assert((i == (m_FieldStack.Count() - 1)) || (td->fieldType & FIELD_EMBEDDED));
...
BEGIN_PREDICTION_DATA_NO_BASE(CCopyTesterData)
	...
	DEFINE_FIELD(CCopyTesterData, m_CharValue, FIELD_CHARACTER),
		...
		DEFINE_FIELD(CCopyTesterData, m_szValue, FIELD_STRING), DEFINE_FIELD(CCopyTesterData, m_Vector, FIELD_VECTOR),
		DEFINE_FIELD(CCopyTesterData, m_Bool, FIELD_BOOLEAN), DEFINE_FIELD(CCopyTesterData, m_Clr, FIELD_COLOR32),
...
void CPredictionCopyTester::RunTests(void)
{
...

	{
		...
		int diff_count = 0;
		diff_count = tester.TransferData(foo3->GetPredDescMap(), foo1->GetPredDescMap()->dataDesc,
...

	{
		...
		int diff_count = 0;
		diff_count = tester.TransferData(foo2->GetPredDescMap(), foo1->GetPredDescMap()->dataDesc,
										 foo1->GetPredDescMap()->dataNumFields);
...

	{
		...
		describe.DumpDescription(foo1->GetPredDescMap(), foo1->GetPredDescMap()->dataDesc,
								 foo1->GetPredDescMap()->dataNumFields);
	}
