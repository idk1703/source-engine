...
bool IsStaticPointEntity(CBaseEntity *pEnt)
{
	...
	if(FClassnameIs(pEnt, "info_target") || FClassnameIs(pEnt, "info_landmark") || FClassnameIs(pEnt, "path_corner"))
		return true;
...
#if defined(CLIENT_DLL)
extern bool ComputeBeamEntPosition(CBaseEntity *pEnt, int nAttachment, bool bInterpretAttachmentIndexAsHitboxIndex,
								   Vector &pt);
...
BEGIN_NETWORK_TABLE_NOBASE(CBeam, DT_Beam)
	...
	SendPropInt(SENDINFO(m_nBeamType), Q_log2(NUM_BEAM_TYPES) + 1, SPROP_UNSIGNED),
		...
		SendPropArray3(SENDINFO_ARRAY3(m_hAttachEntity), SendPropEHandle(SENDINFO_ARRAY(m_hAttachEntity))),
...
#ifdef PORTAL
		SendPropBool(SENDINFO(m_bDrawInMainRender)), SendPropBool(SENDINFO(m_bDrawInPortalRender)),
#endif
...
#if !defined(NO_ENTITY_PREDICTION)
		SendPropDataTable("beampredictable_id", 0, &REFERENCE_SEND_TABLE(DT_BeamPredictableId),
						  SendProxy_SendPredictableId),
#endif

#else
	RecvPropInt(RECVINFO(m_nBeamType)), RecvPropInt(RECVINFO(m_nBeamFlags)), RecvPropInt(RECVINFO(m_nNumBeamEnts)),
		RecvPropArray3(RECVINFO_ARRAY(m_hAttachEntity), RecvPropEHandle(RECVINFO(m_hAttachEntity[0]))),
		RecvPropArray3(RECVINFO_ARRAY(m_nAttachIndex), RecvPropInt(RECVINFO(m_nAttachIndex[0]))),
		RecvPropInt(RECVINFO(m_nHaloIndex)), RecvPropFloat(RECVINFO(m_fHaloScale)), RecvPropFloat(RECVINFO(m_fWidth)),
		RecvPropFloat(RECVINFO(m_fEndWidth)), RecvPropFloat(RECVINFO(m_fFadeLength)),
		RecvPropFloat(RECVINFO(m_fAmplitude)), RecvPropFloat(RECVINFO(m_fStartFrame)),
		RecvPropFloat(RECVINFO(m_fSpeed), 0, RecvProxy_Beam_ScrollSpeed), RecvPropFloat(RECVINFO(m_flFrameRate)),
		RecvPropFloat(RECVINFO(m_flHDRColorScale)), RecvPropInt(RECVINFO(m_clrRender)),
		RecvPropInt(RECVINFO(m_nRenderFX)), RecvPropInt(RECVINFO(m_nRenderMode)), RecvPropFloat(RECVINFO(m_flFrame)),
		RecvPropVector(RECVINFO(m_vecEndPos)),
#ifdef PORTAL
		RecvPropBool(RECVINFO(m_bDrawInMainRender)), RecvPropBool(RECVINFO(m_bDrawInPortalRender)),
#endif
...
BEGIN_DATADESC(CBeam)
	DEFINE_FIELD(m_nHaloIndex, FIELD_MODELINDEX), DEFINE_FIELD(m_nBeamType, FIELD_INTEGER),
		DEFINE_FIELD(m_nBeamFlags, FIELD_INTEGER), DEFINE_FIELD(m_nNumBeamEnts, FIELD_INTEGER),
		DEFINE_ARRAY(m_hAttachEntity, FIELD_EHANDLE, MAX_BEAM_ENTS),
		DEFINE_ARRAY(m_nAttachIndex, FIELD_INTEGER, MAX_BEAM_ENTS), DEFINE_FIELD(m_nMinDXLevel, FIELD_INTEGER),

		DEFINE_FIELD(m_fWidth, FIELD_FLOAT), DEFINE_FIELD(m_fEndWidth, FIELD_FLOAT),
		DEFINE_FIELD(m_fFadeLength, FIELD_FLOAT), DEFINE_FIELD(m_fHaloScale, FIELD_FLOAT),
		DEFINE_FIELD(m_fAmplitude, FIELD_FLOAT), DEFINE_FIELD(m_fStartFrame, FIELD_FLOAT),
		...
		// Inputs
		DEFINE_INPUTFUNC(FIELD_FLOAT, "Width", InputWidth), DEFINE_INPUTFUNC(FIELD_FLOAT, "Noise", InputNoise),
...
const Vector &C_Beam::GetAbsEndPos(void) const
{
	...
	if(GetType() != BEAM_POINTS && GetType() != BEAM_HOSE)
	{
		if(ComputeBeamEntPosition(m_hAttachEntity[m_nNumBeamEnts - 1], m_nAttachIndex[m_nNumBeamEnts - 1], false,
								  vecEndAbsPosition))
...
// Output : CSprite
//-----------------------------------------------------------------------------
CBeam *CBeam::BeamCreatePredictable(const char *module, int line, bool persist, const char *pSpriteName, float width,
									CBasePlayer *pOwner)
{
#if !defined(NO_ENTITY_PREDICTION)
...
int CBeam::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		// Print state
		char tempstr[512];
		Q_snprintf(tempstr, sizeof(tempstr), "start: (%.2f,%.2f,%.2f)", GetAbsOrigin().x, GetAbsOrigin().y,
				   GetAbsOrigin().z);
		EntityText(text_offset, tempstr, 0);
		text_offset++;

		Q_snprintf(tempstr, sizeof(tempstr), "end  : (%.2f,%.2f,%.2f)", m_vecEndPos.GetX(), m_vecEndPos.GetY(),
				   m_vecEndPos.GetZ());
		EntityText(text_offset, tempstr, 0);
		text_offset++;
	}
...
void CBeam::ComputeBounds(Vector &mins, Vector &maxs)
{
	...
	bUseExtraPoints =
		UTIL_Portal_Trace_Beam(this, vecAbsStart, vecAbsEnd, vecAbsExtra1, vecAbsExtra2, &traceFilterChain);
#endif
		...
		case BEAM_ENTPOINT:
		{
			...
			for(int i = 0; i < m_nNumBeamEnts; ++i)
			{
				...
				if(pTestEnt)
				{
					...
					{
						// We do this so we don't have to calculate attachments (and do expensive bone-setup
						// calculations) on our attachments.
