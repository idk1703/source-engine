...
bool CTeamFortress::ClientCommand(CBaseEntity *pEdict, const CCommand &args)
{
	...
	{
		bool bTactical =
			args[1][0] == '!' ? !pPlayer->GetLocalData()->m_nInTacticalView : (atoi(args[1]) ? true : false);
	...
	else if(FStrEq(pcmd, "decaltest"))
	{
		...
		UTIL_TraceLine(pEdict->GetAbsOrigin(), pEdict->GetAbsOrigin() + vForward * 10000, MASK_SOLID_BRUSHONLY, pEdict,
		...
		te->BSPDecal(filter, 0.0, &trace.endpos, entityIndex, id);
...
//-----------------------------------------------------------------------------
float CTeamFortress::WeaponTraceEntity(CBaseEntity *pEntity, const Vector &src, const Vector &end, unsigned int mask,
									   trace_t *pTrace)
{
...
//-----------------------------------------------------------------------------
bool CTeamFortress::IsTraceBlockedByWorldOrShield(const Vector &src, const Vector &end, CBaseEntity *pShooter,
												  int damageType, trace_t *pTrace)
{
	// Iterate over all shields on the same team, disable them so
	// we don't intersect with them...
...
void CTeamFortress::RadiusDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore)
{
	...
	for(CEntitySphereQuery sphere(vecSrc, flRadius); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity())
	{
		if(pEntity->m_takedamage != DAMAGE_NO)
		{
			...
			WeaponTraceLine(vecSrc, vecSpot, MASK_SHOT & (~CONTENTS_HITBOX), subInfo.GetInflictor(),
							subInfo.GetDamageType(), &tr);
			...
			if(flAdjustedDamage > 0)
			{
				...
				if ( pEntity->IsPlayer() && flAdjustedDamage > 40 )
				{
					...
					((CBaseTFPlayer*)pEntity)->KnockDownPlayer( vecForce, flAdjustedDamage * 15.0f,
				tf_knockdowntime.GetFloat() );
				}
				*/
...
void CTeamFortress::DeathNotice(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
	...
	/*		CReliableBroadcastRecipientFilter filter;
			...
			if ( pVictim == pScorer )
			{
				UTIL_LogPrintf( "\"%s<%i>\" killed self with %s\n",  pVictim->GetPlayerName(), engine->GetPlayerUserId(
	   pVictim->edict() ), killer_weapon_name );
			}
			else if ( pScorer )
			{
	   ...
	   engine->GetPlayerUserId( pVictim->edict() ), killer_weapon_name );
			} */
...
void WeaponImpact(trace_t *tr, Vector vecDir, bool bHurt, CBaseEntity *pEntity, int iDamageType)
{
	...
	if(iDamageType & (DMG_PLASMA | DMG_ENERGYBEAM))
	{
		...
		{
			Assert(pEntity);
			bool bHitHandheldShield =
				(pEntity->IsPlayer() && ((CBaseTFPlayer *)pEntity)->IsHittingShield(vecDir, NULL));
...
void CTeamFortress::WeaponTraceLine(const Vector &src, const Vector &end, unsigned int mask, CBaseEntity *pShooter,
									int damageType, trace_t *pTrace)
{
	// Iterate over all shields on the same team, disable them so
	// we don't intersect with them...
...
//-----------------------------------------------------------------------------
void CTeamFortress::FireBullets(const CTakeDamageInfo &info, int cShots, const Vector &vecSrc,
								const Vector &vecDirShooting, const Vector &vecSpread, float flDistance, int iAmmoType,
								int iTracerFreq, int firingEntID, int attachmentID, const char *sCustomTracer)
{
	...
	for(int iShot = 0; iShot < cShots; iShot++)
	{
		...
		if(iTracerFreq != 0 && (tracerCount++ % iTracerFreq) == 0)
		{
			...
			if(sCustomTracer)
			{
				UTIL_Tracer(vecTracerSrc, tr.endpos, subInfo.GetInflictor()->entindex(), TRACER_DONT_USE_ATTACHMENT, 0,
							false, (char *)sCustomTracer);
			}
			else
			{
