...
// Buff ranges
ConVar weapon_arcwelder_target_range("weapon_arcwelder_target_range", "90", FCVAR_REPLICATED,
									 "The farthest away you can be for the arcwelder to initially lock onto a target.");
ConVar weapon_arcwelder_stick_range("weapon_arcwelder_stick_range", "100", FCVAR_REPLICATED,
									"How far away the arcwelder can stay locked onto someone.");
...
public:
	...
	virtual bool TargetsPlayers(void)
	{
		return false;
	}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CWeaponArcWelder::OnFireEvent(C_BaseViewModel *pViewModel, const Vector &origin, const QAngle &angles, int event,
								   const char *options)
{
...
void CWeaponArcWelder::ViewModelDrawn(C_BaseViewModel *pViewModel)
{
	...
	*/
	...
	// Add it
	CSmartPtr<CSimpleEmitter> pSimple = FX_Smoke(vecOrigin, vecVelocity, random->RandomFloat(4, 8),		// Scale
												 1, random->RandomFloat(0.5, 3.0),						// Dietime
												 color, random->RandomInt(64, 200),						// Alpha
												 "particle/smoke_arcwelder", random->RandomInt(0, 255), // Roll
												 0);													// Rolldelta
	...
	sParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
													   pSimple->GetPMaterial("effects/blueflare"), vecOrigin);
	if(sParticle == NULL)
		return;
	...
	// Dull Glow
	sParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
													   pSimple->GetPMaterial("effects/blueflare2"), vecOrigin);
	if(sParticle == NULL)
		return;
	sParticle->m_flLifetime = 0.0f;
