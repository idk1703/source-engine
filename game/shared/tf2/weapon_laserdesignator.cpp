...
// Damage CVars
ConVar weapon_laserdesignator_range("weapon_laserdesignator_range", "1024", FCVAR_NONE,
...
void CWeaponLaserDesignator::UpdateBeam(void)
{
	...
	if(pEntity)
	{
		...
		if(!ValidDesignationTarget(pEntity))
		{
			...
			int count =
				UTIL_EntitiesInBox(pList, 100, vecCenter - delta, vecCenter + delta, FL_CLIENT | FL_NPC | FL_OBJECT);
