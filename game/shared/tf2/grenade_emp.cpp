...
// Damage CVars
ConVar weapon_emp_grenade_duration("weapon_emp_grenade_duration", "5", FCVAR_REPLICATED,
								   "Duration of the EMP grenade's effect.");
ConVar weapon_emp_grenade_object_duration("weapon_emp_grenade_object_duration", "5", FCVAR_REPLICATED,
										  "Duration of the EMP grenade's effect on objects.");
...
void CGrenadeEMP::Explode(trace_t *pTrace, int bitsDamageType)
{
	...
	for(int r = 0; r < iEmpRings; r++, delay += fEmpDelay)
	{
		...
		te->BeamRingPoint(filter, delay, GetAbsOrigin() + Vector(0, 0, 32), // origin
						  64.0f,											// start radius
						  ...
						  50,	// spread
						  2,	// amplitude
						  50 + (1 - frac) * 200, 63, 63 + 127 * frac, 255 - frac * 127, 20);
	}
...
void CGrenadeEMP::ApplyRadiusEMPEffect(CBaseEntity *pOwner, const Vector &vecCenter)
{
	...
	for(CEntitySphereQuery sphere(vecCenter, weapon_emp_grenade_radius.GetFloat());
...
void CGrenadeEMP::ClientThink(void)
{
	...
	while(m_ParticleEvent.NextEvent(flCur))
	{
		...
		SimpleParticle *pParticle =
			(SimpleParticle *)pEmitter->AddParticle(sizeof(SimpleParticle), hSphereMaterial, vecOrigin);
		if(pParticle == NULL)
			return;
