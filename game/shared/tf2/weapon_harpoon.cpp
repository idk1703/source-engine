...
public:
	...
	CRopeKeyframe *GetRope(void)
	{
		return m_hRope;
	}
	void SetRope(CRopeKeyframe *pRope)
	{
		m_hRope = pRope;
	}
	CBaseEntity *GetImpaledTarget(void)
	{
		return m_hImpaledTarget;
	}
	void SetLinkedHarpoon(CHarpoon *pLinkedHarpoon)
	{
		m_hLinkedHarpoon = pLinkedHarpoon;
	}
...
SendPropVector(SENDINFO(m_vecOffset), -1, SPROP_COORD), SendPropVector(SENDINFO(m_angOffset), -1, SPROP_COORD),
END_SEND_TABLE
()
	...
	DEFINE_FUNCTION(FlyThink), DEFINE_FUNCTION(ConstrainThink),
END_DATADESC
()
	...
	if(m_hLinkedHarpoon)
	{
		...
		if(GetImpaledTarget() && m_hLinkedHarpoon->GetImpaledTarget())
		{
			...
			if(pPlayer)
			{
				...
				// For now, we have to be linked to a non-moving target. Eventually we could support linked moving
				// targets. pOtherTarget == NULL means the harpoon's buried in the world.
				if(pOtherTarget->IsBSPModel() || pOtherTarget->GetMoveType() == MOVETYPE_NONE)
				{
					...
					pPlayer->ActivateMovementConstraint(NULL, pNonMovingHarpoon->GetAbsOrigin(), m_flConstrainLength,
														150.0f, 0.1f);
...
void CWeaponHarpoon::ItemPostFrame(void)
{
	...
	else if(m_flCantThrowUntil && m_bActiveHarpoon && !(pOwner->m_nButtons & IN_ATTACK) &&
			(m_flNextPrimaryAttack <= gpGlobals->curtime) && (m_flCantThrowUntil <= gpGlobals->curtime))
	{
