...
// ConVars
//-----------------------------------------------------------------------------
ConVar shield_mobile_power("shield_mobile_power", "30", FCVAR_REPLICATED,
						   "Max power level of a escort's mobile projected shield.");
ConVar shield_mobile_recharge_delay("shield_mobile_recharge_delay", "0.1", FCVAR_REPLICATED,
									"Time after taking damage before mobile projected shields begin to recharge.");
ConVar shield_mobile_recharge_amount("shield_mobile_recharge_amount", "2", FCVAR_REPLICATED,
									 "Power recharged each recharge tick for mobile projected shields.");
ConVar shield_mobile_recharge_time("shield_mobile_recharge_time", "0.5", FCVAR_REPLICATED,
								   "Time between each recharge tick for mobile projected shields.");
...
private:
	...
	{
		SweepContext_t(const CBaseEntity *passentity, int collisionGroup) : m_Filter(passentity, collisionGroup) {}
...
BEGIN_NETWORK_TABLE(CShieldMobile, DT_ShieldMobile)
	...
	SendPropInt(SENDINFO(m_ShieldState), 2, SPROP_UNSIGNED),
		SendPropArray(SendPropInt(SENDINFO_ARRAY(m_pVertsActive), 8, SPROP_UNSIGNED), m_pVertsActive),
		...
		SendPropFloat(SENDINFO(m_flSpringConstant), 0, SPROP_NOSCALE), SendPropQAngles(SENDINFO(m_angLockedAngles), 9),
		...
		SendPropExclude("DT_BaseEntity", "m_vecOrigin"), SendPropExclude("DT_BaseEntity", "m_angAbsRotation[0]"),
	...
	RecvPropInt(RECVINFO(m_ShieldState)), RecvPropArray(RecvPropInt(RECVINFO(m_pVertsActive[0])), m_pVertsActive),
		RecvPropFloat(RECVINFO(m_flFrontDistance)), RecvPropFloat(RECVINFO(m_flShieldTheta)),
		RecvPropFloat(RECVINFO(m_flShieldPhi)), RecvPropFloat(RECVINFO(m_flSpringConstant)),
		RecvPropQAngles(RECVINFO(m_angLockedAngles)), RecvPropInt(RECVINFO(m_nAttachmentIndex)),
#endif
...
BEGIN_PREDICTION_DATA(CShieldMobile)
	...
	DEFINE_PRED_FIELD(m_ShieldState, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
		...
		DEFINE_PRED_TYPEDESCRIPTION(m_ShieldEffect, CShieldEffect), DEFINE_FIELD(m_nNextThinkTick, FIELD_INTEGER),
...
CShield *CreateMobileShield(CBaseEntity *owner, float flFrontDistance)
{
	...
	pShield->SetupRecharge(shield_mobile_power.GetFloat(), shield_mobile_recharge_delay.GetFloat(),
						   shield_mobile_recharge_amount.GetFloat(), shield_mobile_recharge_time.GetFloat());
	pShield->Spawn();
