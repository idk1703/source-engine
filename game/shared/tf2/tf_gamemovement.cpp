...
float CTFGameMovement::ComputeConstraintSpeedFactor(void)
{
	...
	float flFrac = (sqrt(flDistSq) - (pTFMove->m_flConstraintRadius - pTFMove->m_flConstraintWidth)) /
...
void CTFGameMovement::SetupSpeed(void)
{
	...
	float flSpeed = (mv->m_flForwardMove * mv->m_flForwardMove) + (mv->m_flSideMove * mv->m_flSideMove) +
	...
	if((mv->m_nButtons & IN_DUCK) || (player->m_Local.m_bDucking) || (player->GetFlags() & FL_DUCKING))
	{
		...
		if(mv->m_nButtons & IN_DUCK)
		{
			...
			if(player->m_Local.m_bDucking)
			{
				...
				if((duckseconds > TIME_TO_DUCK) || (player->GetGroundEntity() == NULL))
				{
					FinishDuck();
				}
				else
...
void CTFGameMovement::SetupViewAngles(void)
{
	...
	if((player->GetMoveType() != MOVETYPE_ISOMETRIC) && (player->GetMoveType() != MOVETYPE_NOCLIP))
	{
		mv->m_vecViewAngles[ROLL] =
			CalcRoll(v_angle, mv->m_vecVelocity, sv_rollangle.GetFloat(), sv_rollspeed.GetFloat()) * 4.0f;
	}
	else
	{
...
void CTFGameMovement::HandlePlayerMove(void)
{
	...
	switch(player->GetMoveType())
	{
		...
		case MOVETYPE_WALK:
		{
			...
			if((fabs(vecPlayerDelta.x) > 0.0001f) || (fabs(vecPlayerDelta.y) > 0.0001f) ||
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFGameMovement::CalcWishVelocityAndPosition(Vector &vWishPos, Vector &vWishDir, float &flWishSpeed)
{
	//
	...
	{
		vWishVel[axis] = (vForward[axis] * mv->m_flForwardMove) + (vRight[axis] * mv->m_flSideMove);
	}
	vWishVel.z = 0.0f;
...
// Purpose:
//-----------------------------------------------------------------------------
inline void CTFGameMovement::TracePlayerBBoxWithStep(const Vector &vStart, const Vector &vEnd, unsigned int fMask,
...
bool CTFGameMovement::RedirectAirVelocity(const trace_t &trace)
{
	...
	for(int iPlane = 0; iPlane < m_nImpactPlaneCount; iPlane++)
	{
		...
		{
			ClipVelocity(vecVelocity, m_aImpactPlaneNormals[iPlane], mv->m_vecVelocity,
			...
			for(i = 0; i < numplanes; i++)
			{
				...
				else
				{
					ClipVelocity(original_velocity, planes[i], new_velocity,
								 1.0 + sv_bounce.GetFloat() * (1 - m_surfaceFriction));
				}
			}
...
int CTFGameMovement::TryPlayerMove2(void)
{
	...
	for(int iBump = 0; iBump < BUMP_MAX_COUNT; iBump++)
	{
		...
		else
		{
			...
			TracePlayerBBoxWithStep(mv->m_vecAbsOrigin, vecWishPos, MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT,
									trace);
		}
...
void CTFGameMovement::ResolveStanding(void)
{
	...
	//
	Vector vecStandPos(mv->m_vecAbsOrigin.x, mv->m_vecAbsOrigin.y,
					   mv->m_vecAbsOrigin.z - (player->m_Local.m_flStepSize * 2.0f));
