...
#else
	RecvPropInt(RECVINFO(m_nMaxPassengers)), RecvPropArray(RecvPropEHandle(RECVINFO(m_hPassengers[0])), m_hPassengers),
...
ConVar vehicle_thirdperson("vehicle_thirdperson", "1", FCVAR_REPLICATED,
						   "Enable/disable thirdperson camera view in vehicles");

ConVar vehicle_attach_eye_angles("vehicle_attach_eye_angles", "0", FCVAR_REPLICATED,
								 "Attach player eye angles to vehicle attachments");
...
ConVar vehicle_deterioration_start_time("vehicle_deterioration_start_time", "90", 0,
										"Time it takes for a vehicle to start deteriorating after being left alone.");
...
{
#if !defined(CLIENT_DLL)
	SetContextThink(VehicleDeteriorationThink, gpGlobals->curtime + vehicle_deterioration_start_time.GetFloat(),
					DETERIORATION_THINK_CONTEXT);
#endif
}
...
void CBaseTFVehicle::VehicleDeteriorationThink(void)
{
	...
	SetContextThink(NULL, gpGlobals->curtime + vehicle_deterioration_start_time.GetFloat(),
					DETERIORATION_THINK_CONTEXT);
}
...
bool CBaseTFVehicle::GetRoleAbsViewPosition(int nRole, Vector *pAbsVehicleEyeOrigin, QAngle *pAbsVehicleEyeAngles)
{
	...
	if(ShouldUseThirdPersonVehicleView() && vehicle_thirdperson.GetInt() && nRole == VEHICLE_DRIVER && iAttachment > 0)
	{
		...
		*pAbsVehicleEyeOrigin = vAttachOrigin + vForward * vehicle_view_offset_forward.GetFloat() +
								vRight * vehicle_view_offset_right.GetFloat() + vUp * vehicle_view_offset_up.GetFloat();
...
void CBaseTFVehicle::GetVehicleViewPosition(int nRole, Vector *pAbsOrigin, QAngle *pAbsAngles, float *pFOV /*= NULL*/)
{
	...
	*pAbsAngles = vehicleEyeAngles;
	...
	else
	{
		...
		if( inv_demo.GetInt() )
		{
			vehicleEyeAngles.x = RemapAngleRange( PITCH_CURVE_ZERO * road_feel.GetFloat(), PITCH_CURVE_LINEAR,
	vehicleEyeAngles.x ); vehicleEyeAngles.z = RemapAngleRange( ROLL_CURVE_ZERO * road_feel.GetFloat(),
	ROLL_CURVE_LINEAR, vehicleEyeAngles.z );
		...
		// UNDONE: *pOrigin would already be correct in single player if the HandleView() on the server ran after
	vphysics MatrixGetColumn( newCameraToWorld, 3, *pAbsOrigin );
	}
	*/
}
...
#define HUD_IMAGE_LEFT XRES(568)
	...
	if(IsBoostable())
	{
		...
		if(pVehicleBoostLabel)
		{
			...
			gHUD.DrawProgressBar(HUD_IMAGE_LEFT, nScreenY + YRES(4), XRES(70), YRES(4), m_nBoostTimeLeft / 100.0f,
								 gHUD.m_clrNormal, CHud::HUDPB_HORIZONTAL_INV);
		}
	}
