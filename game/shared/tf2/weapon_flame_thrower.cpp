...
static inline void GenerateRandomFlameThrowerVelocity(Vector &vOut, const Vector &vForward, const Vector &vRight,
													  const Vector &vUp)
{
...
void CWeaponFlameThrower::ItemPostFrame()
{
	...
	if(pOwner->IsAlive() && (pOwner->m_nButtons & IN_ATTACK) && GetShieldState() == SS_DOWN && GetPrimaryAmmo() > 2)
	{
		PrimaryAttack();
		m_bFiring = true;
...
#define NUM_TEST_VECTORS 30
	for(int iTest = 0; iTest < NUM_TEST_VECTORS; iTest++)
	{
		...
		trace_t tr;
		UTIL_TraceLine(vOrigin, vOrigin + vVel * FLAMETHROWER_FLAME_DISTANCE, MASK_SHOT & (~CONTENTS_HITBOX), NULL,
					   COLLISION_GROUP_NONE, &tr);
		if(tr.m_pEnt)
		{
			if(TFGameRules()->IsTraceBlockedByWorldOrShield(vOrigin, vOrigin + vVel * FLAMETHROWER_FLAME_DISTANCE,
															GetOwner(), DMG_BURN | DMG_PROBE, &tr) == false)
			{
	...
	if(gpGlobals->curtime >= m_flNextPrimaryAttack)
	{
		...
		CGasolineBlob *pBlob =
			CGasolineBlob::Create(GetOwner(), vOrigin, vForward * FLAMETHROWER_FLAME_SPEED, false,
								  FLAMETHROWER_FLAME_DISTANCE / FLAMETHROWER_FLAME_SPEED, flLifetime);
		if(pBlob)
		{
			if(IsGasCanister())
			{
...
void CWeaponFlameThrower::ClientThink()
{
	...
	if(!IsDormant() && IsCarrierAlive() && m_bFiring && m_hFlameEmitter.IsValid())
	{
		...
		{
			SimpleParticle *p =
				m_hFlameEmitter->AddSimpleParticle(m_hFireMaterial, vOrigin + RandomVector(-3, 3),
...
void CWeaponFlameThrower::IgniteNearbyGasolineBlobs()
{
	...
	float dists[128];
	int nEnts = FindBurnableEntsInSphere(ents, dists, ARRAYSIZE(dists), vOrigin, 50, pOwner);
