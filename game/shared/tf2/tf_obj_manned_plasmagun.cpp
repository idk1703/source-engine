...
#if !defined(CLIENT_DLL)
	SendPropTime(SENDINFO(m_flNextIdleTime)), SendPropInt(SENDINFO(m_bFiringLeft), 1, SPROP_UNSIGNED),

		SendPropInt(SENDINFO(m_nNextThinkTick)),
#else
	RecvPropTime(RECVINFO(m_flNextIdleTime)), RecvPropInt(RECVINFO(m_bFiringLeft)),
...
ConVar obj_manned_plasmagun_range_def("obj_manned_plasmagun_range_def", "1000", FCVAR_REPLICATED,
									  "Defensive Manned Plasmagun range");
ConVar obj_manned_plasmagun_range_off("obj_manned_plasmagun_range_off", "1000", FCVAR_REPLICATED,
									  "Offensive Manned Plasmagun range");
ConVar obj_manned_plasmagun_damage("obj_manned_plasmagun_damage", "20", FCVAR_REPLICATED, "Manned Plasmagun damage");
ConVar obj_manned_plasmagun_radius("obj_manned_plasmagun_radius", "128", FCVAR_REPLICATED,
								   "Manned Plasmagun explosive radius");
...
void CObjectMannedPlasmagun::Fire()
{
	...
	float flNextRecharge = gpGlobals->curtime + (HasPowerup(POWERUP_EMP) ? MANNED_PLASMAGUN_RECHARGE_TIME * 1.5
																		 : MANNED_PLASMAGUN_RECHARGE_TIME);
	...
	CBasePlasmaProjectile *pPlasma =
		CBasePlasmaProjectile::CreatePredicted(vecSrc, vecAim, Vector(0, 0, 0), damageType, GetDriverPlayer());
	if(pPlasma)
	{
