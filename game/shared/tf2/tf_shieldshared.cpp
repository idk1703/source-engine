...
BEGIN_PREDICTION_DATA_NO_BASE(CShieldEffect)
	...
	DEFINE_FIELD(m_TestPoint, FIELD_INTEGER), DEFINE_FIELD(m_Position, FIELD_VECTOR),
		DEFINE_FIELD(m_Velocity, FIELD_VECTOR), DEFINE_FIELD(m_CurrentAngles, FIELD_VECTOR),
		DEFINE_FIELD(m_Theta, FIELD_FLOAT), DEFINE_FIELD(m_Phi, FIELD_FLOAT),
		DEFINE_FIELD(m_ThetaVelocity, FIELD_FLOAT), DEFINE_FIELD(m_PhiVelocity, FIELD_FLOAT),
		DEFINE_FIELD(m_vecDesiredOrigin, FIELD_VECTOR), DEFINE_FIELD(m_angDesiredAngles, FIELD_VECTOR),
		DEFINE_FIELD(m_ShieldTheta, FIELD_FLOAT), DEFINE_FIELD(m_ShieldPhi, FIELD_FLOAT),
...
void CShieldEffect::ComputeRestPositions()
{
	...
	for(i = 0; i < SHIELD_NUM_VERTICAL_POINTS; ++i)
	{
		...
		float phi = (M_PI - m_ShieldPhi) * 0.5f + m_ShieldPhi * (float)i / (float)(SHIELD_NUM_VERTICAL_POINTS - 1);

		for(int j = 0; j < SHIELD_NUM_HORIZONTAL_POINTS; ++j)
		{
			// Choose theta centered at pi/2 also (y, or forward axis)
			float theta =
				(M_PI - m_ShieldTheta) * 0.5f + m_ShieldTheta * (float)j / (float)(SHIELD_NUM_HORIZONTAL_POINTS - 1);
	...
	SetShieldPanelSize(Vector(-l * 0.25f, -l * 0.25f, -l * 0.25f), Vector(l * 0.25f, l * 0.25f, l * 0.25f));
}
...
{
	if((x < 0) || (y < 0) || (x >= SHIELD_NUM_HORIZONTAL_POINTS) || (y >= SHIELD_NUM_VERTICAL_POINTS))
	{
		return false;
	}
...
{
	if((x < 0) || (y < 0) || (x >= SHIELD_HORIZONTAL_PANEL_COUNT) || (y >= SHIELD_VERTICAL_PANEL_COUNT))
	{
		return false;
	}
...
void CShieldEffect::ComputePanelActivity()
{
	...
	for(int i = 0; i < SHIELD_NUM_HORIZONTAL_POINTS - 1; ++i)
	{
		for(int j = 0; j < SHIELD_NUM_VERTICAL_POINTS - 1; ++j)
		{
			...
			// Test the neighbors
			m_pActivePanels[idx] = IsVertexActive(i, j) || IsVertexActive(i + 1, j) || IsVertexActive(i, j + 1) ||
...
void CShieldEffect::ComputeVertexActivity()
{
	...
	for(i = 0; i < SHIELD_TESTS_PER_FRAME; ++i)
	{
		...
		UTIL_TraceHull(m_Position, m_pControlPoint[pt], m_PanelBoxMin, m_PanelBoxMax, MASK_SOLID_BRUSHONLY,
					   &traceFilter, &tr);
...
void CShieldEffect::SimulateRotation(float dt, const Vector &forward)
{
	...
	// rest condition...
	if((fabs(dTheta) < 1e-3) && (fabs(m_ThetaVelocity) < 1e-6) && (fabs(dPhi) < 1e-3) && (fabs(m_PhiVelocity) < 1e-6))
	{
		return;
	}
