...
BEGIN_NETWORK_TABLE_NOBASE(CPlasmaProjectileShared, DT_PlasmaProjectileShared)
	...
	SendPropVector(SENDINFO(m_vecSpawnPosition), 0, SPROP_COORD),
		...
		SendPropTime(SENDINFO(m_flSpawnTime)), SendPropTime(SENDINFO(m_flDeathTime)),
		SendPropFloat(SENDINFO(m_flSpawnSpeed), 0, SPROP_NOSCALE),
#else
	RecvPropVector(RECVINFO(m_vecSpawnPosition)), RecvPropVector(RECVINFO(m_vTracerDir)),
		RecvPropTime(RECVINFO(m_flSpawnTime)), RecvPropTime(RECVINFO(m_flDeathTime)),
...
BEGIN_PREDICTION_DATA_NO_BASE(PositionHistory_t)
	...
	DEFINE_FIELD(m_Position, FIELD_VECTOR), DEFINE_FIELD(m_Time, FIELD_FLOAT),
...
BEGIN_NETWORK_TABLE(CBasePlasmaProjectile, DT_BasePlasmaProjectile)
	...
	SendPropDataTable(SENDINFO_DT(m_Shared), &REFERENCE_SEND_TABLE(DT_PlasmaProjectileShared)),
		...
		SendPropExclude("DT_BaseEntity", "m_vecVelocity"), SendPropExclude("DT_BaseEntity", "m_vecAbsOrigin"),
...
//-----------------------------------------------------------------------------
void CBasePlasmaProjectile::PerformCustomPhysics(Vector *pNewPosition, Vector *pNewVelocity, QAngle *pNewAngles,
...
void CBasePlasmaProjectile::Detonate(void)
{
	...
	{
		RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), m_flDamage, m_DamageType | DMG_BLAST), GetAbsOrigin(),
					 m_flExplosiveRadius, CLASS_NONE, NULL);
	}
#endif
	Remove();
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CBasePlasmaProjectile::RecalculatePositions(Vector *pNewPosition, Vector *pNewVelocity, QAngle *pNewAngles,
												 QAngle *pNewAngVelocity)
{
...
void CBasePlasmaProjectile::RemapPosition(Vector &vecStart, float curtime, Vector &outpos)
{
	...
	float frac = (curtime - m_Shared.GetSpawnTime()) /
REMAP_BLEND_TIME
;
...
bool CBasePlasmaProjectile::SimulateAndRender(Particle *pInParticle, ParticleDraw *pDraw, float &sortKey)
{
	...
	{
		float frac = (gpGlobals->curtime - m_Shared.GetSpawnTime()) /
REMAP_BLEND_TIME
;
frac = 1.0f - clamp(frac, 0.0f, 1.0f);
Vector scaledOffset;
...
if(pInParticle == m_pHeadParticle)
{
	...
	RemapPosition(m_pPreviousPositions[MAX_HISTORY - 1].m_Position, m_pPreviousPositions[MAX_HISTORY - 1].m_Time,
				  vecOrigin);
	...
	// Render it
	RenderParticle_ColorSizeAngle(pDraw, tPos, UpdateColor(pParticle, timeDelta),
								  UpdateAlpha(pParticle, timeDelta) * GetAlphaDistanceFade(tPos, 16, 64),
								  UpdateScale(pParticle, timeDelta), UpdateRoll(pParticle, timeDelta));
...
//-----------------------------------------------------------------------------
void CBasePlasmaProjectile::SetupProjectile(const Vector &vecOrigin, const Vector &vecForward, int damageType,
											CBaseEntity *pOwner)
{
...
//-----------------------------------------------------------------------------
CBasePlasmaProjectile *CBasePlasmaProjectile::Create(const Vector &vecOrigin, const Vector &vecForward, int damageType,
...
CBasePlasmaProjectile *CBasePlasmaProjectile::CreatePredicted(const Vector &vecOrigin, const Vector &vecForward,
															  const Vector &gunOffset, int damageType,
															  CBasePlayer *pOwner)
{
...
//-----------------------------------------------------------------------------
CPowerPlasmaProjectile *CPowerPlasmaProjectile::Create(const Vector &vecOrigin, const Vector &vecForward,
...
CPowerPlasmaProjectile *CPowerPlasmaProjectile::CreatePredicted(const Vector &vecOrigin, const Vector &vecForward,
																const Vector &gunOffset, int damageType,
																CBasePlayer *pOwner)
{
