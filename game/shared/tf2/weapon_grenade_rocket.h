...
public:
	...
	void Spawn(void);
	void SetRealOwner(CBasePlayer *pOwner)
	{
		m_hOwner = pOwner;
	}
	...
	void SetDamageRadius(float flRadius)
	{
		m_flRadius = flRadius;
	}
	...
	static CWeaponGrenadeRocket *Create(const Vector &vecOrigin, const Vector &vecAngles, float flMaxRange,
										CBaseEntity *pRealOwner);
