...
CWeaponCombatShield *CBaseTFPlayer::GetCombatShield(void)
{
	if(!m_hWeaponCombatShield)
	{
		...
		{
			m_hWeaponCombatShield =
				static_cast<CWeaponCombatShield *>(Weapon_OwnsThisType("weapon_combat_shield_alien"));
#ifndef CLIENT_DLL
			if(!m_hWeaponCombatShield)
			{
...
static ConVar tf2_feetyawrunscale("tf2_feetyawrunscale", "2", FCVAR_REPLICATED,
...
CPlayerAnimState::CPlayerAnimState(CBaseTFPlayer *outer) : m_pOuter(outer)
{
	m_flGaitYaw = 0.0f;
	...
	switch(currentActivity)
	{
		...
		case ACT_IDLE:
		{
			...
			if(isMoving && (maxspeed > 0.0f))
			{
				...
				// HACK HACK:: Defender backward animation is animated at 0.6 times speed, so scale up animation for
				// this class
				//  if he's running backward.

				// Not sure if we're really going to do all classes this way.
				if(GetOuter()->IsClass(TFCLASS_DEFENDER) || GetOuter()->IsClass(TFCLASS_MEDIC))
				{
					Vector facing;
					Vector moving;
