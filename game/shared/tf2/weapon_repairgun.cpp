...
// Buff ranges
ConVar weapon_repairgun_target_range(
	"weapon_repairgun_target_range", "450", FCVAR_REPLICATED,
	"The farthest away you can be for the repair gun to initially lock onto a player.");
ConVar weapon_repairgun_stick_range("weapon_repairgun_stick_range", "512", FCVAR_REPLICATED,
									"How far away the repair gun can stay locked onto someone.");
ConVar weapon_repairgun_rate("weapon_repairgun_rate", "12", FCVAR_REPLICATED,
							 "Health healed per second by the repair gun.");
ConVar weapon_repairgun_damage_modifier("weapon_repairgun_damage_modifier", "1.5", FCVAR_REPLICATED,
										"Scales the damage a player does while being healed with the repair gun.");
ConVar weapon_repairgun_debug("weapon_repairgun_debug", "0", FCVAR_REPLICATED,
							  "Show debugging info for the repair gun.");
ConVar weapon_repairgun_construction_rate("weapon_repairgun_construction_rate", "10", FCVAR_REPLICATED,
										  "Constructing object health healed per second by the repair gun.");
...
#if !defined(CLIENT_DLL)
	SendPropInt(SENDINFO(m_bHealing), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_bAttacking), 1, SPROP_UNSIGNED),
		SendPropEHandle(SENDINFO(m_hHealingTarget)),
#else
	RecvPropInt(RECVINFO(m_bAttacking)), RecvPropInt(RECVINFO(m_bHealing)), RecvPropEHandle(RECVINFO(m_hHealingTarget)),
#endif
END_NETWORK_TABLE()
	...
	if(pTarget)
	{
...
#endif
		{
			...
			if(pTarget->IsAlive())
			{
				...
				if(!pTarget->AttemptToPowerup(POWERUP_BOOST, 1.0, flBoostAmount, pOwner,
											  AppliesModifier() ? &m_DamageModifier : NULL))
				{
					m_DamageModifier.RemoveModifier();
				}
