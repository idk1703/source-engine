...
#if !defined(CLIENT_DLL)
// Server Only
ConVar weapon_grenade_rocket_track_range_mod("weapon_grenade_rocket_track_range_mod", "1.5", FCVAR_NONE,
											 "Range multiplier when a rocket's tracking a designated target.");
...
//-----------------------------------------------------------------------------
CWeaponGrenadeRocket *CWeaponGrenadeRocket::Create(const Vector &vecOrigin, const Vector &vecForward, float flMaxRange,
												   CBaseEntity *pOwner)
{
#if !defined(CLIENT_DLL)
...
void CWeaponGrenadeRocket::RocketTouch(CBaseEntity *pOther)
{
	...
	RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), vec3_origin, GetAbsOrigin(), GetDamage(), GetDamageType(), 0,
								 &vecReported),
				 GetAbsOrigin(), GetDamageRadius(), CLASS_NONE, NULL);
...
void CWeaponGrenadeRocket::ClientThink(void)
{
	...
	for(int i = 0; i < iSmokeClouds; i++)
	{
		SimpleParticle *pParticle =
			(SimpleParticle *)pSmokeEmitter->AddParticle(sizeof(SimpleParticle), hSphereMaterial, GetAbsOrigin());
		if(!pParticle)
			return;
