...
// Buff ranges
static ConVar weapon_drainbeam_target_range(
	"weapon_drainbeam_target_range", "900", FCVAR_REPLICATED,
	"The farthest away you can be for the drain gun to initially lock onto a target.");
static ConVar weapon_drainbeam_stick_range("weapon_drainbeam_stick_range", "612", FCVAR_REPLICATED,
										   "How far away the drain gun can stay locked onto a target.");
...
#if !defined(CLIENT_DLL)
	SendPropInt(SENDINFO(m_bDraining), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_bAttacking), 1, SPROP_UNSIGNED),
		SendPropVector(SENDINFO(m_vFireTarget), 0, SPROP_COORD), SendPropEHandle(SENDINFO(m_hDrainTarget)),
#else
	RecvPropInt(RECVINFO(m_bAttacking)), RecvPropInt(RECVINFO(m_bDraining)), RecvPropVector(RECVINFO(m_vFireTarget)),
	...
	*/
	{
		...
		if(tr.fraction != 1.0)
		{
			...
			if(pEntity && (pEntity != pOwner) && pEntity->IsAlive())
			{
				...
				if(!pEntity->IsPlayer() && pEntity->CanBePoweredUp() && pEntity->GetTeamNumber() != 0 &&
				   !pEntity->InSameTeam(pOwner))
				{
	...
	if(pTarget)
	{
		...
		if(pTarget->CanBePoweredUp())
		{
			...
			if(pTarget->AttemptToPowerup(POWERUP_EMP, flPostDrainTime))
			{
				...
				float flDrainAmount =
					weapon_drainbeam_max_rate.GetFloat() * (flDrainTime / weapon_drainbeam_max_rate_time.GetFloat());
