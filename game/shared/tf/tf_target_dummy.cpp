...
ConVar tf_target_dummy_melee_mult("tf_target_dummy_melee_mult", "3.0f",
								  FCVAR_GAMEDLL | FCVAR_DEVELOPMENTONLY); // DEV ONLY
ConVar tf_target_dummy_bullet_mult("tf_target_dummy_bullet_mult", "0.1f",
								   FCVAR_GAMEDLL | FCVAR_DEVELOPMENTONLY); // DEV ONLY
ConVar tf_target_dummy_other_mult("tf_target_dummy_other_mult", "0.2f",
								  FCVAR_GAMEDLL | FCVAR_DEVELOPMENTONLY);									 // DEV ONLY
...
CTFTargetDummy *CTFTargetDummy::Create(const Vector &vPosition, const QAngle &qAngles, CTFPlayer *pOwner)
{
	...
	qProper.z = 0;
	qProper.x = 0;
	CTFTargetDummy *pDummy =
		assert_cast<CTFTargetDummy *>(CBaseEntity::Create("tf_target_dummy", vPosition, qProper, pOwner));
...
void CTFTargetDummy::Spawn()
{
	...
	SetContextThink(&CTFTargetDummy::DestroyThink, gpGlobals->curtime + tf_target_dummy_lifetime.GetFloat(),
					"DestroyThink");
}
