...
BEGIN_DATADESC(CWheelOfDoom)
	...
	DEFINE_KEYFIELD(m_flDuration, FIELD_FLOAT, "effect_duration"),
		...
		// Outputs
		DEFINE_OUTPUT(m_EffectApplied, "OnEffectApplied"), DEFINE_OUTPUT(m_EffectExpired, "OnEffectExpired"),
...
ConCommand cc_proc_wheel_effect("cc_proc_wheel_effect", ProcWheelEffect,
								"Force a Wheel of Doom entity to apply the specified effect");
#endif
...
CWheelOfDoom::CWheelOfDoom(void)
	: m_EffectManager(this), m_pChosenEffect(NULL), m_pSpiral(NULL), m_flFinishBroadcastingEffectTime(0.f)
{
	AddEffects(EF_NODRAW);
...
void CWheelOfDoom::SpeakMagicConceptToAllPlayers(const char *pszEffect)
{
	...
	else if((!V_stricmp(pszEffect, "Halloween.MerasmusWheelCrits")) ||
			(!V_stricmp(pszEffect, "Halloween.MerasmusWheelUber")) ||
			(!V_stricmp(pszEffect, "Halloween.MerasmusWheelSuperSpeed")))
	{
...
void CWheelOfDoom::WOD_Pee::UpdateEffect(EffectData_t &data)
{
	...
	Vector vRandomPos(RandomFloat(vBoxMin.x, vBoxMax.x), RandomFloat(vBoxMin.y, vBoxMax.y),
					  RandomFloat(vBoxMin.z, vBoxMax.z));

	// Drop some pee
	CTFProjectile_Jar *pGrenade = static_cast<CTFProjectile_Jar *>(
		CBaseEntity::CreateNoSpawn("tf_projectile_jar", vRandomPos, QAngle(0, 0, 0), NULL));
	DispatchSpawn(pGrenade);

	// Random angular impulse
	Vector angImpulse(RandomFloat(-300.f, 300.f), RandomFloat(-300.f, 300.f), RandomFloat(-300.f, 300.f));
...
void CWheelOfDoom::WOD_Dance::InitEffect(float /*flDefaultExpireTime*/)
{
	...
	m_vecMerasmusDancerCreateInfos.AddToTail(
		MerasmusCreateInfo_t(Vector(flMerasmusX, aMerasmusY[0], flMerasmusZ), QAngle(0.0f, 90.0f, 0.0f)));
	m_vecMerasmusDancerCreateInfos.AddToTail(
		MerasmusCreateInfo_t(Vector(flMerasmusX, aMerasmusY[1], flMerasmusZ), QAngle(0.0f, -90.0f, 0.0f)));
}
...
void CWheelOfDoom::WOD_Dance::UpdateEffect(EffectData_t & /*data*/)
{
	...
	if(bShouldTaunt)
	{
		...
		const MerasmusCreateInfo_t &info =
			m_vecMerasmusDancerCreateInfos[m_iCurrentMerasmusCreateInfo % m_vecMerasmusDancerCreateInfos.Count()];
