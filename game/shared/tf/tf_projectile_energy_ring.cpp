...
// Purpose:
//-----------------------------------------------------------------------------
CTFProjectile_EnergyRing *CTFProjectile_EnergyRing::Create(CTFWeaponBaseGun *pLauncher, const Vector &vecOrigin,
														   const QAngle &vecAngles, float fSpeed, float fGravity,
														   CBaseEntity *pOwner, CBaseEntity *pScorer, Vector vColor1,
														   Vector vColor2, bool bCritical)
{
	...
	pRing = static_cast<CTFProjectile_EnergyRing *>(
		CBaseEntity::Create("tf_projectile_energy_ring", vecOrigin, vecAngles, pOwner));
	if(!pRing)
		return NULL;
	...
	if(pRaygun && !pRaygun->UseNewProjectileCode())
	{
		...
		{
			pRing = static_cast<CTFProjectile_EnergyRing *>(CTFBaseProjectile::Create(
				"tf_projectile_energy_ring", vecOrigin, vecAngles, pOwner, 1200.f, g_sModelIndexRing,
...
void CTFProjectile_EnergyRing::ProjectileTouch(CBaseEntity *pOther)
{
	...
	if(!pOther->IsWorld() && pOther->GetSolid() == SOLID_VPHYSICS)
	{
		...
		for(int i = collideList.Count() - 1; i >= 0; --i)
		{
			...
			trace_t tr;
			physcollision->TraceCollide(pTrace->startpos, element.origin, element.pCollide, element.angles,
										pTriggerCollide, GetAbsOrigin(), GetAbsAngles(), &tr);
			if(!tr.DidHit())
				return;
		}
	}
	...
	bool bCombatEntity =
		pOther->IsPlayer() || pOther->IsBaseObject() || pOther->IsCombatCharacter() || pOther->IsCombatItem();

	if(!bCombatEntity)
	{
		// Couple more things that we collide with
		// HACK: these are the same checks we do in CTFProjectile_Arrow::ArrowTouch()...need to figure out a better way
		// to do this when we have time
	...
	if(bCombatEntity && (pOther != pOwner))
	{
		...
		UTIL_TraceLine(WorldSpaceCenter(), pOther->WorldSpaceCenter(), MASK_SOLID | CONTENTS_HITBOX, this,
					   COLLISION_GROUP_NONE, &traceAttack);
...
void CTFProjectile_EnergyRing::PlayImpactEffects(const Vector &vecPos, bool bHitFlesh)
{
	...
	if(pTFGun)
	{
		DispatchParticleEffect(g_pszEnergyProjectileImpactParticle, vecPos, GetAbsAngles(), pTFGun->GetParticleColor(1),
							   pTFGun->GetParticleColor(2), true, NULL, 0);
...
#ifdef CLIENT_DLL
void CreateClientSideEnergyRing(const char *pszStandardParticle, const char *pszCritParticle, const CEffectData &data,
								int nFlags)
{
...
{
	CreateClientSideEnergyRing(g_pszBisonTrailParticle, g_pszBisonTrailParticleCrit, data,
							   FTENT_COLLIDEKILL | FTENT_COLLIDEPROPS | FTENT_ATTACHTOTARGET | FTENT_ALIGNTOMOTION |
								   FTENT_CLIENTSIDEPARTICLES);
}
...
{
	CreateClientSideEnergyRing(g_pszPomsonTrailParticle, g_pszPomsonTrailParticleCrit, data,
							   FTENT_COLLIDEALL | FTENT_USEFASTCOLLISIONS | FTENT_ATTACHTOTARGET | FTENT_ALIGNTOMOTION |
								   FTENT_CLIENTSIDEPARTICLES);
}
