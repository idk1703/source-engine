...
// Client specific.
#ifdef CLIENT_DLL
	RecvPropInt(RECVINFO(m_iWeaponState)), RecvPropBool(RECVINFO(m_bCritShot))
// Server specific.
#else
	SendPropInt(SENDINFO(m_iWeaponState), 4, SPROP_UNSIGNED | SPROP_CHANGES_OFTEN), SendPropBool(SENDINFO(m_bCritShot))
#endif
END_NETWORK_TABLE()
...
#ifdef CLIENT_DLL
				int nAmmo = 0;
				if(prediction->IsFirstTimePredicted() && C_BasePlayer::GetLocalPlayer() == pPlayer)
				{
...
#ifdef CLIENT_DLL
				if(prediction->IsFirstTimePredicted() && C_BasePlayer::GetLocalPlayer() == pPlayer &&
				   nAmmo != pPlayer->GetAmmoCount(
								m_iPrimaryAmmoType)) // did PrimaryAttack() fire a shot? (checking our ammo to find out)
				{
					m_nShotsFired++;
					if(m_nShotsFired ==
					   1000) // == and not >= so we don't keep awarding this every shot after it's achieved
					{
...
void CTFMinigun::SetWeaponState(MinigunState_t nState)
{
	...
	{
		if(m_iWeaponState == AC_STATE_IDLE || m_iWeaponState == AC_STATE_STARTFIRING ||
		   m_iWeaponState == AC_STATE_DRYFIRE)
		{
			// Transitioning from non firing or non fully spinning states resets when our drain start point and when the
			// ring of fire can start
			m_flLastAmmoDrainTime = gpGlobals->curtime;
...
void CTFMinigun::RingOfFireAttack(int nDamage)
{
	...
	for(CEntitySphereQuery sphere(vOrigin, flFireRadius); (pEntity = sphere.GetCurrentEntity()) != NULL;
		sphere.NextEntity())
	{
		...
		if(!(vOrigin.z > pVictim->GetAbsOrigin().z + vMins.z - 32.0f &&
			 vOrigin.z < pVictim->GetAbsOrigin().z + vMaxs.z))
		{
			continue;
		}
		...
		pVictim->TakeDamage(
			CTakeDamageInfo(pPlayer, pPlayer, this, vec3_origin, vOrigin, nDamage, DMG_PLASMA, 0, &vOrigin));
	}
...
void CTFMinigun::StartBrassEffect()
{
	...
	// This caching will mess up if we go third person from first - we only do this in taunts and don't fire so we
	...
	{
		m_pEjectBrassEffect = m_hEjectBrassWeapon->ParticleProp()->Create("eject_minigunbrass", PATTACH_POINT_FOLLOW,
...
void CTFMinigun::StartMuzzleEffect()
{
	...
	{
		m_pMuzzleEffect = m_hMuzzleEffectWeapon->ParticleProp()->Create("muzzle_minigun_constant", PATTACH_POINT_FOLLOW,
																		m_iMuzzleAttachment);
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFMinigun::ViewModelAttachmentBlending(CStudioHdr *hdr, Vector pos[], Quaternion q[], float currentTime,
											 int boneMask)
{
