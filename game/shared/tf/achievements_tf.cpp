...
// Grace period that we allow a player to start after level init and still consider them to be participating for the
// full round.  This is fairly generous because it can in some cases take a client several minutes to connect with
// respect to when the server considers the game underway
...
bool CBaseTFAchievementSimple::LocalPlayerCanEarn(void)
{
	if(TFGameRules())
	{
		bool bMVMAchievement =
			(m_iAchievementID >= ACHIEVEMENT_TF_MVM_START_RANGE && m_iAchievementID <= ACHIEVEMENT_TF_MVM_END_RANGE);
...
DECLARE_ACHIEVEMENT(CAchievementTFPlayGameEveryClass, ACHIEVEMENT_TF_PLAY_GAME_EVERYCLASS, "TF_PLAY_GAME_EVERYCLASS",
					5);
...
class CAchievementTFPlayGameEveryMap : public CTFAchievementFullRound
{
	...
	void Init()
	{
		...
		static const char *szComponents[] = {"cp_dustbowl", "cp_granary", "cp_gravelpit",
											 "cp_well",		"ctf_2fort",  "tc_hydro"};
...
DECLARE_ACHIEVEMENT(CAchievementTFBurnPlayersInMinimumTime, ACHIEVEMENT_TF_BURN_PLAYERSINMINIMIMTIME,
					"TF_BURN_PLAYERSINMINIMUMTIME", 5);
...
class CAchievementTFGetHeadshots : public CBaseTFAchievementSimple
{
									...
									IGameEvent *event)
	{
		...
		{
			// Increment count.  Count will also get slammed whenever we get a stats update from server.  They should
			// generally agree, but server is authoritative.
			IncrementCount();
		}
	}
...
DECLARE_ACHIEVEMENT(CAchievementTFGetConsecutiveKillsNoDeaths, ACHIEVEMENT_TF_GET_CONSECUTIVEKILLS_NODEATHS,
					"TF_GET_CONSECUTIVEKILLS_NODEATHS", 10);
...
DECLARE_ACHIEVEMENT(CAchievementTFPlayGameFriendsOnly, ACHIEVEMENT_TF_PLAY_GAME_FRIENDSONLY, "TF_PLAY_GAME_FRIENDSONLY",
					10);
...
class CAchievementTFGetMultipleKills : public CBaseTFAchievementSimple
{
	...
	void OnPlayerStatsUpdate()
	{
		...
		for(int iClass = TF_FIRST_NORMAL_CLASS; iClass <= (TF_LAST_NORMAL_CLASS - 1);
			iClass++) //( TF_LAST_NORMAL_CLASS - 1 ) to exclude the new civilian class
		{
...
DECLARE_ACHIEVEMENT(CAchievementTFWin2FortNoEnemyCaps, ACHIEVEMENT_TF_WIN_2FORT_NOENEMYCAPS, "TF_WIN_2FORT_NOENEMYCAPS",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFWinWellMinimumTime, ACHIEVEMENT_TF_WIN_WELL_MINIMUMTIME, "TF_WIN_WELL_MINIMUMTIME",
					10);
...
DECLARE_ACHIEVEMENT(CAchievementTFWinHydroNoEnemyCaps, ACHIEVEMENT_TF_WIN_HYDRO_NOENEMYCAPS, "TF_WIN_HYDRO_NOENEMYCAPS",
					20);
...
class CAchievementTFWinDustbowlNoEnemyCaps : public CBaseTFAchievementSimple
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		// defending dustbowl with no enemy caps means there were no previous minirounds completed (that would be an
		// attacker capture), the player was on the defending team and they won with no enemy caps
...
DECLARE_ACHIEVEMENT(CAchievementTFWinDustbowlNoEnemyCaps, ACHIEVEMENT_TF_WIN_DUSTBOWL_NOENEMYCAPS,
					"TF_WIN_DUSTBOWL_NOENEMYCAPS", 10);
...
DECLARE_ACHIEVEMENT(CAchievementTFWinGravelPitNoEnemyCaps, ACHIEVEMENT_TF_WIN_GRAVELPIT_NOENEMYCAPS,
					"TF_WIN_GRAVELPIT_NOENEMYCAPS", 30);
...
DECLARE_ACHIEVEMENT(CAchievementTFKillEnemiesAfterTeleporting, ACHIEVEMENT_TF_GENERAL_KILL_ENEMIES_AFTER_TELEPORTING,
					"TF_GENERAL_KILL_ENEMIES_AFTER_TELEPORTING", 10);
...
bool GameRulesAllowsAchievements(void)
{
	...
	if((TFGameRules()->State_Get() < GR_STATE_TEAM_WIN) || (TFGameRules()->State_Get() == GR_STATE_STALEMATE))
	{
		bRetVal = true;
	}
...
DECLARE_ACHIEVEMENT(CAchievementTFChristmasCollectGifts, ACHIEVEMENT_TF_CHRISTMAS_COLLECT_GIFTS,
					"TF_CHRISTMAS_COLLECT_GIFTS", 5);
...
class CAchievementTF_KillBalloonicornOwners : public CBaseTFAchievement
{
									...
									IGameEvent *event)
	{
		...
		if(pLocalPlayer)
		{
			...
			if(pTFVictim && (pAttacker == pLocalPlayer) &&
			   (pTFVictim->GetTeamNumber() != pLocalPlayer->GetTeamNumber()))
			{
...
DECLARE_ACHIEVEMENT(CAchievementTF_KillBalloonicornOwners, ACHIEVEMENT_TF_KILL_BALLOONICORN_OWNERS,
					"TF_KILL_BALLOONICORN_OWNERS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTF_DominateForGoggles, ACHIEVEMENT_TF_DOMINATE_FOR_GOGGLES, "TF_DOMINATE_FOR_GOGGLES",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTF_ParachuteKillGrind, ACHIEVEMENT_TF_PARACHUTE_KILL_GRIND, "TF_PARACHUTE_KILL_GRIND",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTF_MeleeKillClassicRifleSniper, ACHIEVEMENT_TF_MELEE_KILL_CLASSIC_RIFLE_SNIPER,
					"TF_MELEE_KILL_CLASSIC_RIFLE_SNIPER", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFTaunt_DosidoMeleeKill, ACHIEVEMENT_TF_TAUNT_DOSIDO_MELLE_KILL,
					"TF_TAUNT_DOSIDO_MELLE_KILL", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFTaunt_TauntWhileCapping, ACHIEVEMENT_TF_TAUNT_WHILE_CAPPING, "TF_TAUNT_WHILE_CAPPING",
					5);
