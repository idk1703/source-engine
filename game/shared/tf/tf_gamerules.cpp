...
static int g_TauntCamAchievements[] = {
	...
	0,										// TF_CLASS_MEDIC,
	ACHIEVEMENT_TF_HEAVY_FREEZECAM_TAUNT, // TF_CLASS_HEAVYWEAPONS,  (there's an extra check on this one to see if we're
										  // also invuln)
...
// STAGING_SPY
ConVar tf_feign_death_activate_damage_scale("tf_feign_death_activate_damage_scale", "0.25",
...
ConVar training_can_build_sentry("training_can_build_sentry", "1", FCVAR_REPLICATED,
								 "Player can build sentry as engineer.");
ConVar training_can_build_dispenser("training_can_build_dispenser", "1", FCVAR_REPLICATED,
									"Player can build dispenser as engineer.");
ConVar training_can_build_tele_entrance("training_can_build_tele_entrance", "1", FCVAR_REPLICATED,
										"Player can build teleporter entrance as engineer.");
ConVar training_can_build_tele_exit("training_can_build_tele_exit", "1", FCVAR_REPLICATED,
									"Player can build teleporter exit as engineer.");
ConVar training_can_destroy_buildings("training_can_destroy_buildings", "1", FCVAR_REPLICATED,
									  "Player can destroy buildings as engineer.");
ConVar training_can_pickup_sentry("training_can_pickup_sentry", "1", FCVAR_REPLICATED,
								  "Player can pickup sentry gun as engineer.");
ConVar training_can_pickup_dispenser("training_can_pickup_dispenser", "1", FCVAR_REPLICATED,
									 "Player can pickup dispenser as engineer.");
ConVar training_can_pickup_tele_entrance("training_can_pickup_tele_entrance", "1", FCVAR_REPLICATED,
										 "Player can pickup teleporter entrance as engineer.");
ConVar training_can_pickup_tele_exit("training_can_pickup_tele_exit", "1", FCVAR_REPLICATED,
									 "Player can pickup teleporter entrance as engineer.");
ConVar training_can_select_weapon_primary("training_can_select_weapon_primary", "1", FCVAR_REPLICATED,
										  "In training player select primary weapon.");
ConVar training_can_select_weapon_secondary("training_can_select_weapon_secondary", "1", FCVAR_REPLICATED,
											"In training player select secondary weapon.");
ConVar training_can_select_weapon_melee("training_can_select_weapon_melee", "1", FCVAR_REPLICATED,
										"In training player select melee weapon.");
ConVar training_can_select_weapon_building("training_can_select_weapon_building", "1", FCVAR_REPLICATED,
										   "In training player select building tool.");
ConVar training_can_select_weapon_pda("training_can_select_weapon_pda", "1", FCVAR_REPLICATED,
									  "In training player select pda.");
ConVar training_can_select_weapon_item1("training_can_select_weapon_item1", "1", FCVAR_REPLICATED,
										"In training player select item 1.");
ConVar training_can_select_weapon_item2("training_can_select_weapon_item2", "1", FCVAR_REPLICATED,
										"In training player select item 2.");

ConVar tf_birthday_ball_chance("tf_birthday_ball_chance", "100", FCVAR_REPLICATED,
							   "Percent chance of a birthday beach ball spawning at each round start");

ConVar tf_halloween_boss_spawn_interval("tf_halloween_boss_spawn_interval", "480", FCVAR_CHEAT,
										"Average interval between boss spawns, in seconds");
ConVar tf_halloween_boss_spawn_interval_variation("tf_halloween_boss_spawn_interval_variation", "60", FCVAR_CHEAT,
												  "Variation of spawn interval +/-");

ConVar tf_halloween_eyeball_boss_spawn_interval("tf_halloween_eyeball_boss_spawn_interval", "180", FCVAR_CHEAT,
												"Average interval between boss spawns, in seconds");
ConVar tf_halloween_eyeball_boss_spawn_interval_variation("tf_halloween_eyeball_boss_spawn_interval_variation", "30",
														  FCVAR_CHEAT, "Variation of spawn interval +/-");

ConVar tf_merasmus_spawn_interval("tf_merasmus_spawn_interval", "180", FCVAR_CHEAT,
								  "Average interval between boss spawns, in seconds");
ConVar tf_merasmus_spawn_interval_variation("tf_merasmus_spawn_interval_variation", "30", FCVAR_CHEAT,
											"Variation of spawn interval +/-");

ConVar tf_halloween_zombie_mob_enabled("tf_halloween_zombie_mob_enabled", "0", FCVAR_CHEAT,
									   "If set to 1, spawn zombie mobs on non-Halloween Valve maps");
ConVar tf_halloween_zombie_mob_spawn_interval("tf_halloween_zombie_mob_spawn_interval", "180", FCVAR_CHEAT,
											  "Average interval between zombie mob spawns, in seconds");
ConVar tf_halloween_zombie_mob_spawn_count("tf_halloween_zombie_mob_spawn_count", "20", FCVAR_CHEAT,
										   "How many zombies to spawn");

ConVar tf_halloween_allow_truce_during_boss_event(
	"tf_halloween_allow_truce_during_boss_event", "0", FCVAR_NOTIFY,
...
ConVar tf_player_drop_bonus_ducks("tf_player_drop_bonus_ducks", "-1", FCVAR_REPLICATED,
...
ConVar tf_weapon_criticals_distance_falloff("tf_weapon_criticals_distance_falloff", "0", FCVAR_CHEAT,
											"Critical weapon damage will take distance into account.");
ConVar tf_weapon_minicrits_distance_falloff("tf_weapon_minicrits_distance_falloff", "0", FCVAR_CHEAT,
											"Mini-crit weapon damage will take distance into account.");

ConVar mp_spectators_restricted(
	"mp_spectators_restricted", "0", FCVAR_NONE,
...
ConVar tf_spec_xray("tf_spec_xray", "1", FCVAR_NOTIFY | FCVAR_REPLICATED,
					"Allows spectators to see player glows. 1 = same team, 2 = both teams");
ConVar tf_spawn_glows_duration("tf_spawn_glows_duration", "10", FCVAR_NOTIFY | FCVAR_REPLICATED,
...
ConVar tf_forced_holiday("tf_forced_holiday", "0", FCVAR_REPLICATED,
						 "Forced holiday, \n   Birthday = 1\n   Halloween = 2\n" //  Christmas = 3\n   Valentines = 4\n
																				 //  MeetThePyro = 5\n   FullMoon=6
#ifdef GAME_DLL
						 ,
...
ConVar tf_item_based_forced_holiday(
	"tf_item_based_forced_holiday", "0", FCVAR_REPLICATED,
	"" // like a clone of tf_forced_holiday, but controlled by client consumable item use
#ifdef GAME_DLL
	,
	cc_tf_forced_holiday_changed
#endif // GAME_DLL
...
ConVar tf_spells_enabled("tf_spells_enabled", "0", FCVAR_NOTIFY | FCVAR_REPLICATED,
						 "Enable to Allow Halloween Spells to be dropped and used by players");

ConVar tf_caplinear("tf_caplinear", "1", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
					"If set to 1, teams must capture control points linearly.");
ConVar tf_stalematechangeclasstime("tf_stalematechangeclasstime", "20", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
...
ConVar mp_tournament_stopwatch("mp_tournament_stopwatch", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
							   "Use Stopwatch mode while using Tournament mode (mp_tournament)"
#ifdef GAME_DLL
							   ,
							   cc_tf_stopwatch_changed
#endif
);
ConVar mp_tournament_readymode("mp_tournament_readymode", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
							   "Enable per-player ready status for tournament mode.");
ConVar mp_tournament_readymode_min(
	"mp_tournament_readymode_min", "2", FCVAR_REPLICATED | FCVAR_NOTIFY,
	"Minimum number of players required on the server before players can toggle ready status.");
ConVar mp_tournament_readymode_team_size(
	"mp_tournament_readymode_team_size", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
	"Minimum number of players required to be ready per-team before the game can begin.");
ConVar mp_tournament_readymode_countdown("mp_tournament_readymode_countdown", "10", FCVAR_REPLICATED | FCVAR_NOTIFY,
										 "The number of seconds before a match begins when both teams are ready.");
ConVar mp_windifference("mp_windifference", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
						"Score difference between teams before server changes maps", true, 0, false, 0);
ConVar mp_windifference_min("mp_windifference_min", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
							"Minimum score needed for mp_windifference to be applied", true, 0, false, 0);

ConVar tf_tournament_classlimit_scout("tf_tournament_classlimit_scout", "-1", FCVAR_REPLICATED,
									  "Tournament mode per-team class limit for Scouts.\n");
ConVar tf_tournament_classlimit_sniper("tf_tournament_classlimit_sniper", "-1", FCVAR_REPLICATED,
									   "Tournament mode per-team class limit for Snipers.\n");
ConVar tf_tournament_classlimit_soldier("tf_tournament_classlimit_soldier", "-1", FCVAR_REPLICATED,
										"Tournament mode per-team class limit for Soldiers.\n");
ConVar tf_tournament_classlimit_demoman("tf_tournament_classlimit_demoman", "-1", FCVAR_REPLICATED,
										"Tournament mode per-team class limit for Demomenz.\n");
ConVar tf_tournament_classlimit_medic("tf_tournament_classlimit_medic", "-1", FCVAR_REPLICATED,
									  "Tournament mode per-team class limit for Medics.\n");
ConVar tf_tournament_classlimit_heavy("tf_tournament_classlimit_heavy", "-1", FCVAR_REPLICATED,
									  "Tournament mode per-team class limit for Heavies.\n");
ConVar tf_tournament_classlimit_pyro("tf_tournament_classlimit_pyro", "-1", FCVAR_REPLICATED,
									 "Tournament mode per-team class limit for Pyros.\n");
ConVar tf_tournament_classlimit_spy("tf_tournament_classlimit_spy", "-1", FCVAR_REPLICATED,
									"Tournament mode per-team class limit for Spies.\n");
ConVar tf_tournament_classlimit_engineer("tf_tournament_classlimit_engineer", "-1", FCVAR_REPLICATED,
										 "Tournament mode per-team class limit for Engineers.\n");
ConVar tf_tournament_classchange_allowed("tf_tournament_classchange_allowed", "1", FCVAR_REPLICATED,
										 "Allow players to change class while the game is active?.\n");
ConVar tf_tournament_classchange_ready_allowed("tf_tournament_classchange_ready_allowed", "1", FCVAR_REPLICATED,
											   "Allow players to change class after they are READY?.\n");

ConVar tf_classlimit(
	"tf_classlimit", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
	"Limit on how many players can be any class (i.e. tf_class_limit 2 would limit 2 players per class).\n", true, 0.f,
	false, 0.f);
ConVar tf_player_movement_restart_freeze("tf_player_movement_restart_freeze", "1", FCVAR_REPLICATED,
...
ConVar mp_humans_must_join_team("mp_humans_must_join_team", "any", FCVAR_REPLICATED,
...
ConVar tf_medieval_autorp("tf_medieval_autorp", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
						  "Enable Medieval Mode auto-roleplaying.\n", true, 0, true, 1);

ConVar tf_sticky_radius_ramp_time("tf_sticky_radius_ramp_time", "2.0",
								  FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT | FCVAR_REPLICATED,
								  "Amount of time to get full radius after arming");
ConVar tf_sticky_airdet_radius("tf_sticky_airdet_radius", "0.85",
							   FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT | FCVAR_REPLICATED,
							   "Radius Scale if detonated in the air");

#ifdef STAGING_ONLY
ConVar tf_killstreak_alwayson("tf_killstreak_alwayson", "0", FCVAR_REPLICATED,
							  "enable to have killstreak effects for all players, bots included");

#ifdef GAME_DLL
// Bounty Mode
ConVar tf_bountymode_currency_starting("tf_bountymode_currency_starting", "1000", FCVAR_ARCHIVE,
									   "How much new players start with when playing Bounty Mode.\n");
ConVar tf_bountymode_currency_limit("tf_bountymode_currency_limit", "0", FCVAR_ARCHIVE,
									"The maximum amount a player can hold in Bounty Mode.\n");
ConVar tf_bountymode_currency_penalty_ondeath(
	"tf_bountymode_currency_penalty_ondeath", "0", FCVAR_ARCHIVE,
	"The percentage of unspent money players lose when they die in Bounty Mode.\n");
ConVar tf_bountymode_upgrades_wipeondeath("tf_bountymode_upgrades_wipeondeath", "0", FCVAR_ARCHIVE,
										  "If set to true, wipe player/item upgrades on death.\n");
...
ConVar tf_bountymode("tf_bountymode", "0", FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_ARCHIVE,
					 "Allow upgrades and award currency for mission objectives and killing enemy players.\n", true, 0,
					 true, 1
#ifdef GAME_DLL
					 ,
...
ConVar tf_particles_disable_weather("tf_particles_disable_weather", "0", FCVAR_ARCHIVE,
									"Disable particles related to weather effects.");
#endif

// arena mode cvars
ConVar tf_arena_force_class("tf_arena_force_class", "0",
							FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT,
							"Forces players to play a random class each time they spawn.");
ConVar tf_arena_change_limit(
	"tf_arena_change_limit", "1", FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT,
	"Number of times players can change their class when mp_force_random_class is being used.");
ConVar tf_arena_override_cap_enable_time("tf_arena_override_cap_enable_time", "-1", FCVAR_REPLICATED | FCVAR_NOTIFY,
										 "Overrides the time (in seconds) it takes for the capture point to become "
										 "enable, -1 uses the level designer specified time.");
ConVar tf_arena_override_team_size(
	"tf_arena_override_team_size", "0", FCVAR_REPLICATED,
	"Overrides the maximum team size in arena mode. Set to zero to keep the default behavior of 1/3 maxplayers.");
ConVar tf_arena_first_blood("tf_arena_first_blood", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
...
//=============================================================================
// Training mode cvars
ConVar tf_training_client_message("tf_training_client_message", "0", FCVAR_REPLICATED | FCVAR_CHEAT,
...
#ifdef TF_RAID_MODE
// Raid mode
ConVar tf_gamemode_raid("tf_gamemode_raid", "0",
						FCVAR_REPLICATED | FCVAR_NOTIFY); // client needs access to this for IsRaidMode()
ConVar tf_raid_enforce_unique_classes("tf_raid_enforce_unique_classes", "0", FCVAR_REPLICATED | FCVAR_NOTIFY);
ConVar tf_raid_respawn_time("tf_raid_respawn_time", "5", FCVAR_REPLICATED | FCVAR_NOTIFY /*| FCVAR_CHEAT*/,
							"How long it takes for a Raider to respawn with his team after death.");
...
enum
{
	kMVM_MaxConnectedPlayers = 10,
};

ConVar tf_mvm_min_players_to_start("tf_mvm_min_players_to_start", "3", FCVAR_REPLICATED | FCVAR_NOTIFY,
								   "Minimum number of players connected to start a countdown timer");
ConVar tf_mvm_respec_enabled("tf_mvm_respec_enabled", "1", FCVAR_CHEAT | FCVAR_REPLICATED,
							 "Allow players to refund credits spent on player and item upgrades.");
ConVar tf_mvm_respec_limit("tf_mvm_respec_limit", "0", FCVAR_CHEAT | FCVAR_REPLICATED,
						   "The total number of respecs a player can earn.  Default: 0 (no limit).", true, 0.f, true,
						   100.f);
ConVar tf_mvm_respec_credit_goal(
	"tf_mvm_respec_credit_goal", "2000", FCVAR_CHEAT | FCVAR_REPLICATED,
	"When tf_mvm_respec_limit is non-zero, the total amount of money the team must collect to earn a respec credit.");
#ifdef STAGING_ONLY
ConVar tf_mvm_buybacks_method(
	"tf_mvm_buybacks_method", "1", FCVAR_REPLICATED | FCVAR_HIDDEN,
	"When set to 0, use the traditional, currency-based system.  When set to 1, use finite, charge-based system.", true,
	0.0, true, 1.0);
#else
ConVar tf_mvm_buybacks_method(
	"tf_mvm_buybacks_method", "0", FCVAR_REPLICATED | FCVAR_HIDDEN,
	"When set to 0, use the traditional, currency-based system.  When set to 1, use finite, charge-based system.", true,
	0.0, true, 1.0);
#endif
ConVar tf_mvm_buybacks_per_wave("tf_mvm_buybacks_per_wave", "3", FCVAR_REPLICATED | FCVAR_HIDDEN,
...
static bool BIsCvarIndicatingHolidayIsActive(int iCvarValue, /*EHoliday*/ int eHoliday)
{
	...
	switch(eHoliday)
	{
		...
		case kHoliday_Halloween:
			return iCvarValue == kHoliday_Halloween || iCvarValue == kHoliday_HalloweenOrFullMoon ||
				   iCvarValue == kHoliday_HalloweenOrFullMoonOrValentines;
		...
		case kHoliday_FullMoon:
			return iCvarValue == kHoliday_FullMoon || iCvarValue == kHoliday_HalloweenOrFullMoon ||
				   iCvarValue == kHoliday_HalloweenOrFullMoonOrValentines;
		case kHoliday_HalloweenOrFullMoon:
			return iCvarValue == kHoliday_Halloween || iCvarValue == kHoliday_FullMoon ||
				   iCvarValue == kHoliday_HalloweenOrFullMoon || iCvarValue == kHoliday_HalloweenOrFullMoonOrValentines;
		case kHoliday_HalloweenOrFullMoonOrValentines:
			return iCvarValue == kHoliday_Halloween || iCvarValue == kHoliday_FullMoon ||
				   iCvarValue == kHoliday_Valentines || iCvarValue == kHoliday_HalloweenOrFullMoon ||
				   iCvarValue == kHoliday_HalloweenOrFullMoonOrValentines;
...
ConVar tf_creep_wave_player_respawn_time("tf_creep_wave_player_respawn_time", "10",
										 FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_CHEAT,
										 "How long it takes for a player to respawn with his team after death.");
#endif
...
ConVar mp_waitingforplayers_time("mp_waitingforplayers_time", (IsX360() ? "15" : "30"),
								 FCVAR_GAMEDLL | WAITING_FOR_PLAYERS_FLAGS, "WaitingForPlayers time length in seconds");
...
ConVar tf_gravetalk("tf_gravetalk", "1", FCVAR_NOTIFY,
					"Allows living players to hear dead players using text/voice chat.", true, 0, true, 1);
...
#ifdef STAGING_ONLY
ConVar mp_tournament_readymode_bots_allowed("mp_tournament_readymode_bots_allowed", "0", FCVAR_ARCHIVE,
											"Allow bot data to go through the system for debugging.");
#endif // STAGING_ONLY
...
ConVar tf_competitive_preround_duration("tf_competitive_preround_duration", "3", FCVAR_REPLICATED,
										"How long we stay in pre-round when in competitive games.");
ConVar tf_competitive_preround_countdown_duration("tf_competitive_preround_countdown_duration", "10.5", FCVAR_HIDDEN,
...
ConVar tf_competitive_required_late_join_timeout(
	"tf_competitive_required_late_join_timeout", "120", FCVAR_DEVELOPMENTONLY,
	"How long to wait for late joiners in matches requiring full player counts before canceling the match");
ConVar tf_competitive_required_late_join_confirm_timeout(
	"tf_competitive_required_late_join_confirm_timeout", "30", FCVAR_DEVELOPMENTONLY,
...
ConVar tf_powerup_mode("tf_powerup_mode", "0", FCVAR_NOTIFY,
					   "Enable/disable powerup mode. Not compatible with Mann Vs Machine mode", cc_powerup_mode);
ConVar tf_powerup_mode_imbalance_delta(
	"tf_powerup_mode_imbalance_delta", "24", FCVAR_CHEAT,
	"Powerup kill score lead one team must have before imbalance measures are initiated");

ConVar tf_skillrating_update_interval("tf_skillrating_update_interval", "180", FCVAR_ARCHIVE,
...
static bool PlayerHasDuckStreaks(CTFPlayer *pPlayer)
{
	...
	return FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pActionItem, pAttr_DuckStreaks, &iDuckStreaksActive) &&
		   iDuckStreaksActive > 0;
}
...
ConVar tf_flag_caps_per_round("tf_flag_caps_per_round", "3", FCVAR_REPLICATED,
							  "Number of captures per round on CTF and PASS Time maps. Set to 0 to disable.", true, 0,
							  false, 0
#ifdef GAME_DLL
							  ,
 ...
 */
static CViewVectors g_TFViewVectors(Vector(0, 0, 72), // VEC_VIEW (m_vView) eye position
...
BEGIN_NETWORK_TABLE_NOBASE(CTFGameRules, DT_TFGameRules)
	...
	RecvPropInt(RECVINFO(m_nGameType)), RecvPropInt(RECVINFO(m_nStopWatchState)),
		RecvPropString(RECVINFO(m_pszTeamGoalStringRed)), RecvPropString(RECVINFO(m_pszTeamGoalStringBlue)),
		...
		//=============================================================================
		RecvPropInt(RECVINFO(m_nHudType)), RecvPropBool(RECVINFO(m_bIsInTraining)),
		RecvPropBool(RECVINFO(m_bAllowTrainingAchievements)), RecvPropBool(RECVINFO(m_bIsWaitingForTrainingContinue)),
		//=============================================================================
		// HPE_END
		//=============================================================================
		RecvPropBool(RECVINFO(m_bIsTrainingHUDVisible)), RecvPropBool(RECVINFO(m_bIsInItemTestingMode)),

		RecvPropEHandle(RECVINFO(m_hBonusLogic)), RecvPropBool(RECVINFO(m_bPlayingKoth)),
		RecvPropBool(RECVINFO(m_bPlayingMedieval)), RecvPropBool(RECVINFO(m_bPlayingHybrid_CTF_CP)),
		RecvPropBool(RECVINFO(m_bPlayingSpecialDeliveryMode)), RecvPropBool(RECVINFO(m_bPlayingRobotDestructionMode)),
		RecvPropEHandle(RECVINFO(m_hRedKothTimer)), RecvPropEHandle(RECVINFO(m_hBlueKothTimer)),
		...
		RecvPropInt(RECVINFO(m_nBossHealth)), RecvPropInt(RECVINFO(m_nMaxBossHealth)),
		RecvPropInt(RECVINFO(m_fBossNormalizedTravelDistance)), RecvPropBool(RECVINFO(m_bMannVsMachineAlarmStatus)),
		RecvPropBool(RECVINFO(m_bHaveMinPlayersToEnableReady)),

		RecvPropBool(RECVINFO(m_bBountyModeEnabled)),

		RecvPropInt(RECVINFO(m_nHalloweenEffect)), RecvPropFloat(RECVINFO(m_fHalloweenEffectStartTime)),
		RecvPropFloat(RECVINFO(m_fHalloweenEffectDuration)), RecvPropInt(RECVINFO(m_halloweenScenario)),
		RecvPropBool(RECVINFO(m_bHelltowerPlayersInHell)), RecvPropBool(RECVINFO(m_bIsUsingSpells)),
		RecvPropBool(RECVINFO(m_bCompetitiveMode), 0, RecvProxy_CompetitiveMode),
		RecvPropInt(RECVINFO(m_nMatchGroupType)), RecvPropBool(RECVINFO(m_bMatchEnded)),
		RecvPropBool(RECVINFO(m_bPowerupMode)), RecvPropString(RECVINFO(m_pszCustomUpgradesFile)),
		RecvPropBool(RECVINFO(m_bTruceActive)), RecvPropBool(RECVINFO(m_bShowMatchSummary), 0, RecvProxy_MatchSummary),
		RecvPropBool(RECVINFO_NAME(m_bShowMatchSummary, "m_bShowCompetitiveMatchSummary"), 0,
					 RecvProxy_MatchSummary), // Renamed
		RecvPropBool(RECVINFO(m_bTeamsSwitched)), RecvPropBool(RECVINFO(m_bMapHasMatchSummaryStage)),
		RecvPropBool(RECVINFO(m_bPlayersAreOnMatchSummaryStage)), RecvPropBool(RECVINFO(m_bStopWatchWinner)),
		RecvPropArray3(RECVINFO_ARRAY(m_ePlayerWantsRematch),
					   RecvPropInt(RECVINFO(m_ePlayerWantsRematch[0]), 0, RecvProxy_PlayerVotedForMap)),
		RecvPropInt(RECVINFO(m_eRematchState)),
		RecvPropArray3(RECVINFO_ARRAY(m_nNextMapVoteOptions),
					   RecvPropInt(RECVINFO(m_nNextMapVoteOptions[0]), 0, RecvProxy_NewMapVoteStateChanged)),
#else

	SendPropInt(SENDINFO(m_nGameType), 4, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nStopWatchState), 3, SPROP_UNSIGNED),
		SendPropString(SENDINFO(m_pszTeamGoalStringRed)), SendPropString(SENDINFO(m_pszTeamGoalStringBlue)),
		...
		// [msmith]	Training status and hud type.
		//=============================================================================
		SendPropInt(SENDINFO(m_nHudType), 3, SPROP_UNSIGNED), SendPropBool(SENDINFO(m_bIsInTraining)),
		SendPropBool(SENDINFO(m_bAllowTrainingAchievements)), SendPropBool(SENDINFO(m_bIsWaitingForTrainingContinue)),
		//=============================================================================
		// HPE_END
		//=============================================================================
		SendPropBool(SENDINFO(m_bIsTrainingHUDVisible)), SendPropBool(SENDINFO(m_bIsInItemTestingMode)),

		SendPropEHandle(SENDINFO(m_hBonusLogic)), SendPropBool(SENDINFO(m_bPlayingKoth)),
		SendPropBool(SENDINFO(m_bPlayingMedieval)), SendPropBool(SENDINFO(m_bPlayingHybrid_CTF_CP)),
		SendPropBool(SENDINFO(m_bPlayingSpecialDeliveryMode)), SendPropBool(SENDINFO(m_bPlayingRobotDestructionMode)),
		SendPropEHandle(SENDINFO(m_hRedKothTimer)), SendPropEHandle(SENDINFO(m_hBlueKothTimer)),
		SendPropInt(SENDINFO(m_nMapHolidayType), 3, SPROP_UNSIGNED),

		SendPropEHandle(SENDINFO(m_itHandle)), SendPropBool(SENDINFO(m_bPlayingMannVsMachine)),
		SendPropEHandle(SENDINFO(m_hBirthdayPlayer)),

		SendPropInt(SENDINFO(m_nBossHealth)), SendPropInt(SENDINFO(m_nMaxBossHealth)),
		SendPropInt(SENDINFO(m_fBossNormalizedTravelDistance)), SendPropBool(SENDINFO(m_bMannVsMachineAlarmStatus)),
		SendPropBool(SENDINFO(m_bHaveMinPlayersToEnableReady)),

		SendPropBool(SENDINFO(m_bBountyModeEnabled)),

		SendPropInt(SENDINFO(m_nHalloweenEffect)), SendPropFloat(SENDINFO(m_fHalloweenEffectStartTime)),
		SendPropFloat(SENDINFO(m_fHalloweenEffectDuration)), SendPropInt(SENDINFO(m_halloweenScenario)),
		SendPropBool(SENDINFO(m_bHelltowerPlayersInHell)), SendPropBool(SENDINFO(m_bIsUsingSpells)),
		SendPropBool(SENDINFO(m_bCompetitiveMode)), SendPropBool(SENDINFO(m_bPowerupMode)),
		SendPropInt(SENDINFO(m_nMatchGroupType)), SendPropBool(SENDINFO(m_bMatchEnded)),
		SendPropString(SENDINFO(m_pszCustomUpgradesFile)), SendPropBool(SENDINFO(m_bTruceActive)),
		SendPropBool(SENDINFO(m_bShowMatchSummary)), SendPropBool(SENDINFO(m_bTeamsSwitched)),
		SendPropBool(SENDINFO(m_bMapHasMatchSummaryStage)), SendPropBool(SENDINFO(m_bPlayersAreOnMatchSummaryStage)),
		SendPropBool(SENDINFO(m_bStopWatchWinner)),
		SendPropArray3(SENDINFO_ARRAY3(m_ePlayerWantsRematch),
					   SendPropInt(SENDINFO_ARRAY(m_ePlayerWantsRematch), -1, SPROP_UNSIGNED | SPROP_VARINT)),
		SendPropInt(SENDINFO(m_eRematchState)),
		SendPropArray3(SENDINFO_ARRAY3(m_nNextMapVoteOptions),
					   SendPropInt(SENDINFO_ARRAY(m_nNextMapVoteOptions), -1, SPROP_UNSIGNED | SPROP_VARINT)),
#endif
END_NETWORK_TABLE()
...
END_RECV_TABLE()
#else
void *SendProxy_TFGameRules(const SendProp *pProp, const void *pStructBase, const void *pData,
...
BEGIN_DATADESC(CTFGameRulesProxy)
	...
	DEFINE_KEYFIELD(m_nHudType, FIELD_INTEGER, "hud_type"),
		...
		DEFINE_OUTPUT(m_OnWonByTeam1, "OnWonByTeam1"), DEFINE_OUTPUT(m_OnWonByTeam2, "OnWonByTeam2"),
		...
		DEFINE_OUTPUT(m_OnMatchSummaryStart, "OnMatchSummaryStart"), DEFINE_OUTPUT(m_OnTruceStart, "OnTruceStart"),
...
EMatchGroup CTFGameRules::GetCurrentMatchGroup() const
{
	...
	return GTFGCClientSystem()->BConnectedToMatchServer(false) ? (EMatchGroup)m_nMatchGroupType.Get()
...
void CTFGameRules::StopCompetitiveMatch(CMsgGC_Match_Result_Status nCode)
{
	...
	if(BAttemptMapVoteRollingMatch() && nCode == CMsgGC_Match_Result_Status_MATCH_SUCCEEDED && nActiveMatchPlayers > 0)
	{
	...
	else
	{
		...
		// TODO ROLLING MATCHES: If we bail between now and RequestNewMatchForLobby, we need to call this or we'll get
		// stuck.
			...
			if(pInfo->CalculatePlayerMatchRankData())
			{
				...
				for(int idx = 0; idx < nPlayers; idx++)
				{
					...
					if(pEvent)
					{
						...
						pEvent->SetInt("score_rank",
									   pMatchRankData ? pMatchRankData->nScoreMedal : 0); // medal won (if any)
	...
	else if(nCode == CMsgGC_Match_Result_Status_MATCH_FAILED_ABANDON)
	{
		...
		if(pEvent)
		{
			pEvent->SetBool("game_over",
							(tf_competitive_abandon_method.GetBool() || State_Get() == GR_STATE_BETWEEN_RNDS));
			gameeventmanager->FireEvent(pEvent);
		}
...
bool CTFGameRules::ReportMatchResultsToGC(CMsgGC_Match_Result_Status nCode)
{
	...
	GCSDK::CProtoBufMsg<CMsgGC_Match_Result> *pMsg =
		new GCSDK::CProtoBufMsg<CMsgGC_Match_Result>(k_EMsgGC_Match_Result);

	pMsg->Body().set_match_id(pMatch->m_nMatchID);
	pMsg->Body().set_match_group(pMatch->m_eMatchGroup);
	pMsg->Body().set_status(nCode);
	pMsg->Body().set_duration(CTF_GameStats.m_currentMap.m_Header.m_iTotalTime +
	...
	if(HasMultipleTrains())
	{
		...
		flBlueScoreRatio = RemapValClamped(
			pTFTeamBlue->GetTotalPLRTrackPercentTraveled(), 0.f,
			pTFTeamBlue->GetTotalPLRTrackPercentTraveled() + pTFTeamRed->GetTotalPLRTrackPercentTraveled(), 0.f, 1.f);
	}
	else if(!m_bPlayingKoth && !m_bPowerupMode &&
			(tf_gamemode_cp.GetInt() || tf_gamemode_sd.GetInt() || tf_gamemode_payload.GetInt()))
	{
		// Rounds Won
		...
		flBlueScoreRatio =
			RemapValClamped(pTFTeamBlue->GetScore(), 0.f, pTFTeamBlue->GetScore() + pTFTeamRed->GetScore(), 0.f, 1.f);
	}
	else if(tf_gamemode_ctf.GetInt() || m_bPowerupMode || tf_gamemode_passtime.GetInt())
	{
		...
		flBlueScoreRatio =
			RemapValClamped(pTFTeamBlue->GetTotalFlagCaptures(), 0.f,
							pTFTeamBlue->GetTotalFlagCaptures() + pTFTeamRed->GetTotalFlagCaptures(), 0.f, 1.f);
	}
	...
	else if(tf_gamemode_rd.GetInt() || tf_gamemode_pd.GetInt())
	{
		...
		flBlueScoreRatio =
			RemapValClamped(pRDLogic->GetScore(TF_TEAM_BLUE), 0.f,
							pRDLogic->GetScore(TF_TEAM_BLUE) + pRDLogic->GetScore(TF_TEAM_RED), 0.f, 1.f);
	}
	...
	for(int idxPlayer = 0; idxPlayer < pMatch->GetNumTotalMatchPlayers(); idxPlayer++)
	{
		...
		pMsgPlayer->set_rank(
			pMatchDesc->m_pProgressionDesc->GetLevelForExperience(pMatchPlayer->unMMSkillRating).m_nLevelNum);
		...
		uint32 unLeaveTime = (pMatchPlayer && (pMatchPlayer->bDropped || pMatchPlayer->BDropWasAbandon()))
								 ? pMatchPlayer->GetLastActiveEventTime()
								 : 0u;
		...
		if(nCode == CMsgGC_Match_Result_Status_MATCH_SUCCEEDED)
		{
			...
			int nPerformanceScore =
				RemapValClamped(pMsgPlayer->score(), 0, nTotalScore / 24, 0,
								nTeam == TF_TEAM_RED ? nRedTeamObjectiveBonus : nBlueTeamObjectiveBonus);
			pMatch->GiveXPRewardToPlayerForAction(pMatchPlayer->steamID, CMsgTFXPSource::SOURCE_OBJECTIVE_BONUS,
												  nPerformanceScore);

			// Everyone gets base completion points
			int nCompletionScore = RemapValClamped(pMsgPlayer->score(), 0, nTotalScore / 24, 0, nTotalScore);
			pMatch->GiveXPRewardToPlayerForAction(pMatchPlayer->steamID, CMsgTFXPSource::SOURCE_COMPLETED_MATCH,
												  nCompletionScore);
		}
...
bool CTFGameRules::MatchmakingShouldUseStopwatchMode()
{
	CTeamControlPointMaster *pMaster = g_hControlPointMasters.Count() ? g_hControlPointMasters[0] : NULL;
	bool bRetVal =
		!HasMultipleTrains() && (tf_gamemode_payload.GetBool() ||
								 (pMaster && (pMaster->PlayingMiniRounds() || pMaster->ShouldSwitchTeamsOnRoundWin())));
...
bool CTFGameRules::PlayerReadyStatus_HaveMinPlayersToEnable(void)
{
	...
	else if(IsMannVsMachineMode() && (engine->IsDedicatedServer() || (!engine->IsDedicatedServer() && nNumPlayers > 1)))
	{
	...
	// Should be renamed to m_bEnableReady, not sure why we encoded our criteria in the names of all associated
	...
	if(pMatch)
	{
		...
		for(int i = 0; i < nMatchPlayers; i++)
		{
			...
			if(!pPlayerData->bDropped && GetGameTeamForGCTeam(pPlayerData->eGCTeam) == iTeam)
			{
				...
				//             confusing
				//
				// AssertMsg( !pPlayer || ToTFPlayer( pPlayer )->GetTeamNumber() == GetGameTeamForGCTeam(
				// pPlayerData->eGCTeam ),
		...
		{
			int iTeamSize =
				IsMannVsMachineMode() ? pMatch->GetCanonicalMatchSize() : pMatch->GetCanonicalMatchSize() / 2;
...
bool CTFGameRules::PlayerReadyStatus_ShouldStartCountdown(void)
{
	...
	if(IsMannVsMachineMode())
	{
		if(!IsTeamReady(TF_TEAM_PVE_DEFENDERS) &&
		   m_flRestartRoundTime >= gpGlobals->curtime + mp_tournament_readymode_countdown.GetInt())
		{
	...
	else if(pMatch && PlayerReadyStatus_ArePlayersOnTeamReady(TF_TEAM_RED) &&
...
void CTFGameRules::PlayerReadyStatus_UpdatePlayerState(CTFPlayer *pTFPlayer, bool bState)
{
	...
	else
	{
		...
		if(!pMatch && !IsMannVsMachineMode())
		{
			...
			for(int iTeam = FIRST_GAME_TEAM; iTeam < TFTeamMgr()->GetTeamCount(); iTeam++)
			{
				...
				if(pTeam)
				{
					...
					for(int i = 0; i < pTeam->GetNumPlayers(); ++i)
					{
						...
						else if(pTeam->GetTeamNumber() == TF_TEAM_BLUE &&
								IsPlayerReady(pTeam->GetPlayer(i)->entindex()))
						{
							if(!nBlueCount)
							{
			...
			int nRedMin = (mp_tournament_readymode_team_size.GetInt() > 0) ? mp_tournament_readymode_team_size.GetInt()
																		   : Max(nRedCount, 1);
			int nBlueMin = (mp_tournament_readymode_team_size.GetInt() > 0) ? mp_tournament_readymode_team_size.GetInt()
																			: Max(nBlueCount, 1);
...
//-----------------------------------------------------------------------------
int CTFGameRules::GetCostForUpgrade(CMannVsMachineUpgrades *pUpgrade, int iItemSlot, int nClass,
									CTFPlayer *pPurchaser /*= NULL*/)
{
	Assert(pUpgrade);
...
void CTFGameRules::LevelInitPostEntity(void)
{
	...
	if(IsHolidayActive(kHoliday_Halloween))
	{
		...
		{
			CHalloweenGiftSpawnLocation *pGift =
				static_cast<CHalloweenGiftSpawnLocation *>(IHalloweenGiftSpawnAutoList::AutoList()[i]);
...
CTFGameRules::EUserNextMapVote CTFGameRules::GetWinningVote(int (&nVotes)[EUserNextMapVote::NUM_VOTE_STATES]) const
{
	...
	else
	{
		...
		for(int i = 0; i < NEXT_MAP_VOTE_OPTIONS; ++i)
		{
			...
			eWinningVote = nVotes[i] >= nVotes[eWinningVote] ? (EUserNextMapVote)i : eWinningVote;
		}
...
bool CTFGameRules::CanChangelevelBecauseOfTimeLimit(void)
{
	...
	// we're playing mini-rounds, and the master says we need to play all of them before changing (for maps like
	// Dustbowl)
...
void CTFGameRules::Activate()
{
	...
	CBossBattleLogic *pBossBattleLogic =
		dynamic_cast<CBossBattleLogic *>(gEntList.FindEntityByClassname(NULL, "tf_logic_boss_battle"));
	if(pBossBattleLogic)
	{
	...
	CMannVsMachineLogic *pMannVsMachineLogic =
		dynamic_cast<CMannVsMachineLogic *>(gEntList.FindEntityByClassname(NULL, "tf_logic_mann_vs_machine"));
	CTeamTrainWatcher *pTrainWatch =
		dynamic_cast<CTeamTrainWatcher *>(gEntList.FindEntityByClassname(NULL, "team_train_watcher"));
	...
	if(CTFRobotDestructionLogic::GetRobotDestructionLogic())
	{
		...
		if(CTFRobotDestructionLogic::GetRobotDestructionLogic()->GetType() ==
		   CTFRobotDestructionLogic::TYPE_ROBOT_DESTRUCTION)
		{
	...
	else if(pTrainWatch)
	{
		...
		CMultipleEscort *pMultipleEscort =
			dynamic_cast<CMultipleEscort *>(gEntList.FindEntityByClassname(NULL, "tf_logic_multiple_escort"));
		SetMultipleTrains(pMultipleEscort != NULL);
	}
	else if(g_hControlPointMasters.Count() &&
			m_nGameType != TF_GAMETYPE_ARENA) // We have cap points in arena but we're not CP
	{
	...
	m_hTrainingModeLogic =
		dynamic_cast<CTrainingModeLogic *>(gEntList.FindEntityByClassname(NULL, "tf_logic_training_mode"));
	...
	CMedievalLogic *pMedieval =
		dynamic_cast<CMedievalLogic *>(gEntList.FindEntityByClassname(NULL, "tf_logic_medieval"));
	if(pMedieval || tf_medieval.GetBool())
	{
		m_bPlayingMedieval.Set(true);
	}

	CCompetitiveLogic *pCompLogic =
		dynamic_cast<CCompetitiveLogic *>(gEntList.FindEntityByClassname(NULL, "tf_logic_competitive"));
	if(pCompLogic)
	{
		m_hCompetitiveLogicEntity = pCompLogic;
	}

	CHybridMap_CTF_CP *pHybridMap_CTF_CP =
		dynamic_cast<CHybridMap_CTF_CP *>(gEntList.FindEntityByClassname(NULL, "tf_logic_hybrid_ctf_cp"));
	if(pHybridMap_CTF_CP)
	{
		m_bPlayingHybrid_CTF_CP.Set(true);
	}

	CTFHolidayEntity *pHolidayEntity =
		dynamic_cast<CTFHolidayEntity *>(gEntList.FindEntityByClassname(NULL, "tf_logic_holiday"));
	if(pHolidayEntity)
	{
	...
	CHandle<CCPTimerLogic> hCPTimer =
		dynamic_cast<CCPTimerLogic *>(gEntList.FindEntityByClassname(NULL, "tf_logic_cp_timer"));
	while(hCPTimer != NULL)
	{
	...
	CLogicMannPower *pLogicMannPower =
		dynamic_cast<CLogicMannPower *>(gEntList.FindEntityByClassname(NULL, "tf_logic_mannpower"));
...
const char *CTFGameRules::GetStalemateSong(int nTeam)
{
	...
	{
		return (nTeam == TF_TEAM_RED) ? "Announcer.Helltower_Hell_Red_Stalemate"
...
const char *CTFGameRules::WinSongName(int nTeam)
{
	if(IsHalloweenScenario(HALLOWEEN_SCENARIO_HIGHTOWER))
	{
		return (nTeam == TF_TEAM_RED) ? "Announcer.Helltower_Hell_Red_Win" : "Announcer.Helltower_Hell_Blue_Win";
	}
...
const char *CTFGameRules::LoseSongName(int nTeam)
{
	if(IsHalloweenScenario(HALLOWEEN_SCENARIO_HIGHTOWER))
	{
		return (nTeam == TF_TEAM_RED) ? "Announcer.Helltower_Hell_Red_Lose" : "Announcer.Helltower_Hell_Blue_Lose";
	}
...
void SpawnRunes(void)
{
	...
	{
		Warning("Warning: Not enough valid info_powerup_spawn locations found. You need a minimum of %i valid "
				"locations to spawn all Powerups.\n",
				RUNE_TYPES_MAX);
	}
	...
	for(int nRuneTypes = 0; nRuneTypes < RUNE_TYPES_MAX && vecSpawnPoints.Count() > 0; nRuneTypes++)
	{
		...
		CTFRune *pNewRune =
			CTFRune::CreateRune(pSpawnPoint->GetAbsOrigin(), (RuneTypes_t)nRuneTypes, TEAM_ANY, false, false);
...
void CTFGameRules::RespawnPlayers(bool bForceRespawn, bool bTeam, int iTeam)
{
	...
	if(IsCompetitiveMode() && (GetRoundsPlayed() == 0) && bForceRespawn &&
	   (State_Get() == GR_STATE_BETWEEN_RNDS || State_Get() == GR_STATE_PREROUND))
	{
		CTeamControlPointMaster *pMaster = g_hControlPointMasters.Count() ? g_hControlPointMasters[0] : NULL;
...
void CTFGameRules::ManageStopwatchTimer(bool bInSetup)
{
	...
	if(IsInStopWatch() == true)
	{
		...
		{
			variant_t sVariant;
			CTeamRoundTimer *pStopWatch =
				(CTeamRoundTimer *)CBaseEntity::CreateNoSpawn("team_round_timer", vec3_origin, vec3_angle);
...
public:
	CTraceFilterIgnoreTeammatesWithException(const IHandleEntity *passentity, int collisionGroup, int iIgnoreTeam,
											 const IHandleEntity *pExceptionEntity)
...
void CTFGameRules::RadiusDamage(CTFRadiusDamageInfo &info)
{
	...
	if(info.flRadius > 0)
	{
		...
		for(CEntitySphereQuery sphere(info.vecSrc, info.flRadius); (pEntity = sphere.GetCurrentEntity()) != NULL;
			sphere.NextEntity())
		{
			...
			// CEntitySphereQuery actually does a box test. So we need to make sure the distance is less than the radius
			// first.
			Vector vecPos;
...
int CTFRadiusDamageInfo::ApplyToEntity(CBaseEntity *pEntity)
{
	...
	flAdjustedDamage =
		RemapValClamped(flDistanceToEntity, 0, flRadius, dmgInfo->GetDamage(), dmgInfo->GetDamage() * flFalloff);
...
//			*pEntityIgnore -
//-----------------------------------------------------------------------------
void CTFGameRules::RadiusDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore,
...
bool CTFGameRules::ApplyOnDamageModifyRules(CTakeDamageInfo &info, CBaseEntity *pVictimBaseEntity, bool bAllowDamage)
{
	...
	if(pVictim)
	{
		...
		if(iCritVsWet)
		{
			...
			if(pVictim->m_Shared.InCond(TF_COND_URINE) || pVictim->m_Shared.InCond(TF_COND_MAD_MILK) ||
			   (pVictim->GetWaterLevel() > WL_NotInWater) ||
			   ((flWaterExitTime > 0) &&
				(gpGlobals->curtime - flWaterExitTime < 5.0f))) // or they exited the water in the last few seconds
			{
				bitsDamage |= DMG_CRITICAL;
				info.AddDamageType(DMG_CRITICAL);
				info.SetCritType(CTakeDamageInfo::CRIT_FULL);
	...
	if(pAttacker != pVictimBaseEntity)
	{
		if(info.GetCritType() == CTakeDamageInfo::CRIT_NONE)
		{
			...
			if(pVictim &&
			   (pVictim->m_Shared.InCond(TF_COND_URINE) || pVictim->m_Shared.InCond(TF_COND_MARKEDFORDEATH) ||
			...
			else if(pTFAttacker && (bitsDamage & DMG_RADIUS_MAX) && pWeapon &&
					((pWeapon->GetWeaponID() == TF_WEAPON_SWORD) || (pWeapon->GetWeaponID() == TF_WEAPON_BOTTLE) ||
			...
			else if((pInflictor && pInflictor->IsPlayer() == false) &&
					((pBaseRocket && pBaseRocket->GetDeflected()) ||
					 (pBaseGrenade && pBaseGrenade->GetDeflected() && (pBaseGrenade->ShouldMiniCritOnReflect()))))
			{
			...
			else if((info.GetDamageType() & DMG_IGNITE) && pVictim && pVictim->m_Shared.InCond(TF_COND_BURNING) &&
					info.GetDamageCustom() == TF_DMG_CUSTOM_BURNING_FLARE)
			{
			...
			else if(pTFAttacker && pWeapon && pWeapon->GetWeaponID() == TF_WEAPON_CANNON &&
					(info.GetDamageType() & DMG_BLAST))
			{
				...
				if(pGrenadeLauncher->IsDoubleDonk(pVictim))
				{
					...
					EconEntity_OnOwnerKillEaterEvent(pGrenadeLauncher, pTFAttacker, pVictim,
													 kKillEaterEvent_DoubleDonks);
				}
			}
			else
			{
				...
				for(int i = 0; i < 1; ++i)
				{
					...
					if(iForceMiniCritOnBurning == 1 && pVictim && pVictim->m_Shared.InCond(TF_COND_BURNING) &&
			...
			// Don't do long range distance falloff when pAttacker has Rocket Specialist attrib and directly hits an
			// enemy
			...
			if(pTFAttacker && (pTFAttacker->m_Shared.GetCarryingRuneType() == RUNE_STRENGTH ||
							   pTFAttacker->m_Shared.GetCarryingRuneType() == RUNE_PRECISION))
			{
				bIgnoreLongRangeDmgEffects = true;
			}
		}
	...
	if(pVictim && pVictim->m_Shared.IsInvulnerable())
	{
		if(!bAllowDamage)
		{
			...
			// NOTE: Deliberately skip base player OnTakeDamage, because we don't want all the stuff it does re: suit
			// voice
	...
	if(pVictim && info.GetDamageCustom() != TF_DMG_CUSTOM_BACKSTAB && (info.GetDamageType() & DMG_CRUSH) == 0)
	{
		...
		if(!iPierceResists)
		{
			...
			else if(pVictim->m_Shared.InCond(TF_COND_DEFENSEBUFF) ||
					pVictim->m_Shared.InCond(TF_COND_DEFENSEBUFF_NO_CRIT_BLOCK))
			{
				// defense buffs gives 50% to sentry dmg and 35% from all other sources
				CObjectSentrygun *pSentry = (info.GetInflictor() && info.GetInflictor()->IsBaseObject())
												? dynamic_cast<CObjectSentrygun *>(info.GetInflictor())
												: NULL;
				if(pSentry)
				{
	...
	if(pAttacker != pVictimBaseEntity && !(bitsDamage & (DMG_DROWN | DMG_FALL)))
	{
		...
		bool bDoLongRangeDistanceDecrease =
			!bIgnoreLongRangeDmgEffects &&
			(bForceCritFalloff || (!bCrit && info.GetCritType() != CTakeDamageInfo::CRIT_MINI));
		...
		if(bitsDamage & DMG_USEDISTANCEMOD)
		{
			...
			// The base sniper rifle doesn't have DMG_USEDISTANCEMOD, so this isn't used. Unlockable rifle had it for a
			// bit.
		...
		if(tf_damage_disablespread.GetBool() ||
		   (pTFAttacker && pTFAttacker->m_Shared.GetCarryingRuneType() == RUNE_PRECISION))
		{
		...
		*/
		...
		{
			info.SetDamageBonus(
				flCritDamage + flBonusDamage,
				pProvider); // Order-of-operations sensitive, but fine as long as TF_COND_CRITBOOSTED is last
		}

		// Crit-A-Cola and Steak Sandwich - only increase normal damage
		if(pVictim && pVictim->m_Shared.InCond(TF_COND_ENERGY_BUFF) && !bCrit &&
		   info.GetCritType() != CTakeDamageInfo::CRIT_MINI)
		{
			float flDmgMult = 1.f;
...
// Purpose:
//-----------------------------------------------------------------------------
static bool CheckForDamageTypeImmunity(int nDamageType, CTFPlayer *pVictim, float &flDamageBase,
									   float &flCritBonusDamage)
{
	bool bImmune = false;
...
// Purpose:
//-----------------------------------------------------------------------------
static bool CheckMedicResist(ETFCond ePassiveCond, ETFCond eDeployedCond, CTFPlayer *pVictim, float flRawDamage,
							 float &flDamageBase, bool bCrit, float &flCritBonusDamage)
{
	...
	if(pTFProvider)
	{
		switch(eActiveCond)
		{
			case TF_COND_MEDIGUN_UBER_BULLET_RESIST:
				...
				//			if ( bCrit )
				//			{
				//				CALL_ATTRIB_HOOK_FLOAT_ON_OTHER( pTFProvider, flCritBarDeplete,
...
// Purpose:
//-----------------------------------------------------------------------------
static void PotentiallyFireDamageMitigatedEvent(const CTFPlayer *pMitigator, const CTFPlayer *pDamaged,
												const CEconEntity *pMitigationProvidingEconItem, float flBeforeDamage,
												float flAfterDamage)
{
	...
	if(pEvent)
	{
		...
		if(pMitigationProvidingEconItem && pMitigationProvidingEconItem->GetAttributeContainer() &&
		   pMitigationProvidingEconItem->GetAttributeContainer()->GetItem())
		{
			nDefIndex = pMitigationProvidingEconItem->GetAttributeContainer()
							->GetItem()
							->GetItemDefinition()
							->GetDefinitionIndex();
		}
...
// Purpose:
//-----------------------------------------------------------------------------
float CTFGameRules::ApplyOnDamageAliveModifyRules(const CTakeDamageInfo &info, CBaseEntity *pVictimBaseEntity,
												  DamageModifyExtras_t &outParams)
{
	...
	if(pVictimBaseEntity && pVictimBaseEntity->m_takedamage != DAMAGE_EVENTS_ONLY && pVictim)
	{
		...
		outParams.bPlayDamageReductionSound =
			CheckForDamageTypeImmunity(info.GetDamageType(), pVictim, flDamageBase, flDamageBonus);
		...
		if(!iPierceResists)
		{
			...
			// Apply general dmg type reductions. Should we only ever apply one of these? (Flaregun is
			// DMG_BULLET|DMG_IGNITE, for instance)
			if(info.GetDamageType() & (DMG_BURN | DMG_IGNITE))
			{
				CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(pVictim, flDamageBase, mult_dmgtaken_from_fire);
				CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(pVictim->GetActiveWeapon(), flDamageBase,
												mult_dmgtaken_from_fire_active);

				// Check for medic resist
				outParams.bPlayDamageReductionSound =
					CheckMedicResist(TF_COND_MEDIGUN_SMALL_FIRE_RESIST, TF_COND_MEDIGUN_UBER_FIRE_RESIST, pVictim,
									 flRawDamage, flDamageBase, bCrit, flDamageBonus);
			}
			...
			if((info.GetDamageType() & (DMG_BLAST)))
			{
				...
				if(bReduceBlast)
				{
					...
					// Check for medic resist
					outParams.bPlayDamageReductionSound =
						CheckMedicResist(TF_COND_MEDIGUN_SMALL_BLAST_RESIST, TF_COND_MEDIGUN_UBER_BLAST_RESIST, pVictim,
										 flRawDamage, flDamageBase, bCrit, flDamageBonus);
				}
			}
			...
			if(info.GetDamageType() & (DMG_BULLET | DMG_BUCKSHOT))
			{
				...
				// Check for medic resist
				outParams.bPlayDamageReductionSound =
					CheckMedicResist(TF_COND_MEDIGUN_SMALL_BULLET_RESIST, TF_COND_MEDIGUN_UBER_BULLET_RESIST, pVictim,
									 flRawDamage, flDamageBase, bCrit, flDamageBonus);
			}
			...
			if(pVictim->IsPlayerClass(TF_CLASS_HEAVYWEAPONS) && pVictim->m_Shared.InCond(TF_COND_AIMING) &&
			   ((pVictim->GetHealth() - flRealDamage) / pVictim->GetMaxHealth()) <= 0.5f)
			{
		...
		if((pVictim && info.GetDamageCustom() != TF_DMG_CUSTOM_BACKSTAB))
		{
			...
			if((!pVictim->m_Shared.InCond(TF_COND_PLAGUE) && info.GetDamageCustom() != TF_DMG_CUSTOM_BLEEDING))
			{
				...
				else if(pTFAttacker && (pVictim->m_Shared.GetCarryingRuneType() == RUNE_PLAGUE) &&
						pTFAttacker->m_Shared.InCond(TF_COND_PLAGUE))
				{
					flRealDamage *= 0.5f;
					outParams.bPlayDamageReductionSound = true;
				}
		...
		if(IsMannVsMachineMode())
		{
			...
			{
				flRealDamage *= g_pPopulationManager ? g_pPopulationManager->GetDamageMultiplier()
													 : tf_populator_damage_multiplier.GetFloat();
			}
		}
		...
		if(pVictim && pVictim->GetActiveTFWeapon())
		{
			...
			else if(info.GetDamageType() & (DMG_BLAST | DMG_BULLET | DMG_BUCKSHOT | DMG_IGNITE | DMG_SONIC))
			{
				...
				PotentiallyFireDamageMitigatedEvent(pVictim, pVictim, pVictim->GetActiveTFWeapon(), flBeforeDamage,
													flRealDamage);
			}
		}

		outParams.bSendPreFeignDamage = false;
		if(pVictim && pVictim->IsPlayerClass(TF_CLASS_SPY) && (info.GetDamageCustom() != TF_DMG_CUSTOM_TELEFRAG) &&
		   !pVictim->IsTaunting())
		{
			// STAGING_SPY
			...
			if(pVictim->m_Shared.InCond(TF_COND_FEIGN_DEATH) || pVictim->m_Shared.IsFeignDeathReady())
			{
				...
				float flDamageReduction =
					RemapValClamped(pVictim->m_Shared.GetSpyCloakMeter(), 50.0f, 0.0f,
									tf_feign_death_damage_scale.GetFloat(), tf_stealth_damage_reduction.GetFloat());
		...
		if(pAttacker == pVictimBaseEntity)
		{
			enum
			{
				kSelfBlastResponse_IgnoreProjectilesFromThisWeapon =
					1, // the sticky jumper doesn't disable damage from other explosive weapons
				kSelfBlastResponse_IgnoreProjectilesFromAllWeapons =
					2, // the rocket jumper doesn't have a special projectile type and so ignores all self-inflicted
					   // damage from explosive sources
			};
			...
			if(info.GetWeapon())
			{
				...
				const bool bIgnoreThisSelfDamage =
					(iNoSelfBlastDamage == kSelfBlastResponse_IgnoreProjectilesFromAllWeapons) ||
					((iNoSelfBlastDamage == kSelfBlastResponse_IgnoreProjectilesFromThisWeapon) &&
					 (info.GetDamageCustom() == TF_DMG_CUSTOM_PRACTICE_STICKY));
		...
		{
			// Vampire Powerup collects health based on damage received on victim. Does not apply to self damage. Do it
			// here to factor in victim resistance calculations
			if(pTFAttacker->m_Shared.GetCarryingRuneType() == RUNE_VAMPIRE)
			{
				if(flRealDamage > 0)
				{
					if(pTFAttacker->GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_MINIGUN ||
					   pTFAttacker->GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_FLAMETHROWER)
					{
...
void CTFGameRules::CheckTauntAchievement(CTFPlayer *pAchiever, int nGibs, int *pTauntCamAchievements)
{
	...
	if(pTauntCamAchievements[iClass])
	{
		...
		{
			if(!pAchiever->m_Shared.InCond(TF_COND_INVULNERABLE_WEARINGOFF) &&
			   !pAchiever->m_Shared.InCond(TF_COND_INVULNERABLE))
			{
		...
		if(iClass == TF_CLASS_SNIPER)
		{
			...
			else if(pTauntCamAchievements[iClass] == ACHIEVEMENT_TF_SNIPER_FREEZECAM_WAVE)
			{
				if(pAchiever->GetActiveTFWeapon() &&
				   WeaponID_IsSniperRifle(pAchiever->GetActiveTFWeapon()->GetWeaponID()))
				{
					bAwardAchievement = false;
				}
			}
		...
		if(iClass == TF_CLASS_SOLDIER)
		{
			...
			{
				if(pAchiever->GetActiveTFWeapon() &&
				   pAchiever->GetActiveTFWeapon()->GetWeaponID() != TF_WEAPON_SHOTGUN_SOLDIER)
				{
					bAwardAchievement = false;
				}
			}
		...
		if(iClass == TF_CLASS_DEMOMAN)
		{
			...
			{
				if(pAchiever->GetActiveTFWeapon() &&
				   pAchiever->GetActiveTFWeapon()->GetWeaponID() != TF_WEAPON_GRENADELAUNCHER)
				{
					bAwardAchievement = false;
				}
			}
			else if(pTauntCamAchievements[iClass] == ACHIEVEMENT_TF_DEMOMAN_FREEZECAM_RUMP)
			{
				if(pAchiever->GetActiveTFWeapon() && pAchiever->GetActiveTFWeapon()->GetAttributeContainer())
				{
					...
					if(pItem && pItem->IsValid() &&
					   pItem->GetItemDefIndex() != 130) // Scottish Defender is item index 130
					{
						bAwardAchievement = false;
					}
				}
		...
		{
			if(pAchiever->GetActiveTFWeapon() &&
			   pAchiever->GetActiveTFWeapon()->GetWeaponID() != TF_WEAPON_SENTRY_REVENGE)
			{
				bAwardAchievement = false;
			}
		}
...
bool CTFGameRules::TFVoiceManager(CBasePlayer *pListener, CBasePlayer *pTalker)
{
	...
	if(pTFListener->GetStudent() || pTFListener->GetCoach() || pTFTalker->GetStudent() || pTFTalker->GetCoach())
	{
...
void CTFGameRules::Think()
{
	...
	if(g_fGameOver)
	{
		if(UsePlayerReadyStatusMode() && !IsMannVsMachineMode())
		{
			...
			if(pMatchDesc && pMatchDesc->m_params.m_pszMatchEndKickWarning && nTimeLeft <= 50 && nTimeLeft % 10 == 0 &&
			   nTimeLeft != nLastTimeSent)
			{
				nLastTimeSent = nTimeLeft;
				CBroadcastRecipientFilter filter;
				UTIL_ClientPrintFilter(filter, HUD_PRINTTALK, pMatchDesc->m_params.m_pszMatchEndKickWarning,
									   CFmtStr("%d", nTimeLeft));
			}
	...
	else
	{
		...
		{
			if(State_Get() != GR_STATE_BONUS && State_Get() != GR_STATE_TEAM_WIN && State_Get() != GR_STATE_GAME_OVER &&
		...
		if(IsCompetitiveMode())
		{
			...
			if(pMatchDesc->m_params.m_bRequireCompleteMatch && !IsManagedMatchEnded() && nMissingPlayers)
			{
				...
				if(pMatchDesc->ShouldRequestLateJoin())
				{
					...
					else
					{
						...
						double flWaitLimit = bGotLateJoin
												 ? tf_competitive_required_late_join_timeout.GetFloat()
				...
				else
				{
					...
					if(iRedActive == 0 || iBlueActive == 0 ||
					   (iTeamSize - iRedActive) > tf_mm_abandoned_players_per_team_max.GetInt() ||
					   (iTeamSize - iBlueActive) > tf_mm_abandoned_players_per_team_max.GetInt())
					{
						Msg("Match type requires a complete match, but there are not enough active players left and we "
							"are not requesting late join.  Stopping match.\n");
						bEndMatch = true;
					}
				}
			}
			...
			else
			{
				...
				AssertMsg(
					!IsManagedMatchEnded() || (pMatch->BMatchTerminated() && bEveryoneSafeToLeave),
				...
				bool bDeadMatch =
					bGameRunning && IsManagedMatchEnded() && pMatch->BMatchTerminated() && bEveryoneSafeToLeave;
				if(bDeadMatch && (m_flSafeToLeaveTimer == -.1f || m_flSafeToLeaveTimer - gpGlobals->curtime <= 0.f))
				{
					// Periodic nag event
			...
			if((m_flCompModeRespawnPlayersAtMatchStart > 0) &&
	...
	if(tf_item_based_forced_holiday.GetInt() == kHoliday_Halloween && engine->Time() >= g_fEternaweenAutodisableTime)
	{
		if(GCClientSystem())
		{
			GCSDK::CProtoBufMsg<CMsgGC_GameServer_ServerModificationItemExpired> msg(
	...
	if(IsHalloweenScenario(HALLOWEEN_SCENARIO_HIGHTOWER))
	{
		if((State_Get() == GR_STATE_RND_RUNNING) && (!m_bHelltowerPlayersInHell) && (m_helltowerTimer.IsElapsed()))
		{
			...
			else
			{
				...
				{
					iRedLine =
						(iWinningTeam == TF_TEAM_RED) ? HELLTOWER_VO_RED_WINNING_RARE : HELLTOWER_VO_RED_LOSING_RARE;
					iBlueLine =
						(iWinningTeam == TF_TEAM_BLUE) ? HELLTOWER_VO_BLUE_WINNING_RARE : HELLTOWER_VO_BLUE_LOSING_RARE;
				}
			}
	...
	{
		if((State_Get() == GR_STATE_RND_RUNNING) && m_doomsdaySetupTimer.HasStarted() &&
		   m_doomsdaySetupTimer.IsElapsed())
		{
			m_doomsdaySetupTimer.Invalidate();
	...
	if(IsInTraining() == true)
	{
		if(m_flStateTransitionTime > gpGlobals->curtime)
		{
			...
			switch(client_message)
			{
				...
				{
					// Keep adding time to the restart while we are in the end screen menu so that we never restart the
					// round until the player presses the replay button (or next button to go to the next map).
...
#ifdef GAME_DLL
#ifdef STAGING_ONLY
ConVar tf_spawn_halloween_gift_test_enabled(
	"tf_spawn_halloween_gift_test_enabled", "0", 0,
	"enable to spawn a gift at the world origin.  You probably want to use ConCommand 'SpawnHalloweenGiftTest'");
...
void CTFGameRules::PeriodicHalloweenUpdate()
{
	...
	{
		Vector vLocation =
			m_halloweenGiftSpawnLocations.Element(RandomInt(0, m_halloweenGiftSpawnLocations.Count() - 1));
		CHalloweenGiftPickup *pGift = assert_cast<CHalloweenGiftPickup *>(
			CBaseEntity::CreateNoSpawn("tf_halloween_gift_pickup", vLocation, vec3_angle, NULL));
		if(pGift)
		{
...
void CTFGameRules::SpawnZombieMob(void)
{
	...
	if(m_zombieMobTimer.IsElapsed())
	{
		...
		for(int i = 0; i < playerVector.Count(); ++i)
		{
			...
			CollectSurroundingAreas(&areaVector, player->GetLastKnownArea(), maxSurroundTravelRange, StepHeight,
									StepHeight);
...
void CTFGameRules::PlayerHistory_AddPlayer(CTFPlayer *pTFPlayer)
{
	...
	PlayerHistoryInfo_t info = {steamID, (float)Plat_FloatTime(), pTFPlayer->GetTeamNumber()};
...
{
	return IsPasstimeMode() ? SetPasstimeWinningTeam() : SetCtfWinningTeam();
}
...
bool CTFGameRules::SetPasstimeWinningTeam()
{
	...
	int iWinnerTeam = (iBlueScore > iRedScore) ? TF_TEAM_BLUE : TF_TEAM_RED;
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFGameRules::SetWinningTeam(int team, int iWinReason, bool bForceMapReset /* = true */,
								  bool bSwitchTeams /* = false*/, bool bDontAddScore /* = false*/,
								  bool bFinal /*= false*/)
{
...
void CTFGameRules::RecalculateTruce(void)
{
	...
	if(IsHolidayActive(kHoliday_Halloween))
	{
		if((IMerasmusAutoList::AutoList().Count() > 0) || (IEyeballBossAutoList::AutoList().Count() > 0))
		{
			...
			if(bHaveActiveBoss && (IsValveMap() || tf_halloween_allow_truce_during_boss_event.GetBool() ||
								   IsMapForcedTruceDuringBossFight()))
			{
				bTruceActive = true;
			}
		}
...
bool CTFGameRules::FPlayerCanTakeDamage(CBasePlayer *pPlayer, CBaseEntity *pAttacker, const CTakeDamageInfo &info)
{
	...
	{
		if(((pAttacker->GetTeamNumber() == TF_TEAM_RED) && (pPlayer->GetTeamNumber() == TF_TEAM_BLUE)) ||
		   ((pAttacker->GetTeamNumber() == TF_TEAM_BLUE) && (pPlayer->GetTeamNumber() == TF_TEAM_RED)))
		{
			CBaseEntity *pInflictor = info.GetInflictor();
			if(pInflictor)
			{
	...
	if(info.GetDamageCustom() == TF_DMG_CUSTOM_SPELL_MONOCULUS &&
	   pAttacker->GetTeamNumber() == pPlayer->GetTeamNumber())
	{
		return false;
	}
	...
	if(IsPVEModeActive() && pPlayer->GetTeamNumber() == pAttacker->GetTeamNumber() && pPlayer != pAttacker &&
...
void TestSpawnPointType(const char *pEntClassName)
{
	...
	while(pSpot)
	{
		...
		if(UTIL_IsSpaceEmpty(pSpot, vTestMins, vTestMaxs))
		{
			...
			NDebugOverlay::HorzArrow(pSpot->GetAbsOrigin(), pSpot->GetAbsOrigin() + vecForward * 32, 10, 255, 0, 0, 255,
									 true, 60);
		}
		else
		{
...
ConCommand cc_TestSpawns("map_showspawnpoints", TestSpawns, "Dev - test the spawn points, draws for 60 seconds",
						 FCVAR_CHEAT);
...
void cc_ShowRespawnTimes()
{
	...
	if(pRules && pPlayer)
	{
		float flRedMin = (pRules->m_TeamRespawnWaveTimes[TF_TEAM_RED] >= 0 ? pRules->m_TeamRespawnWaveTimes[TF_TEAM_RED]
		...
		float flBlueMin =
			(pRules->m_TeamRespawnWaveTimes[TF_TEAM_BLUE] >= 0 ? pRules->m_TeamRespawnWaveTimes[TF_TEAM_BLUE]
															   : mp_respawnwavetime.GetFloat());
...
CBaseEntity *CTFGameRules::GetPlayerSpawnSpot(CBasePlayer *pPlayer)
{
	...
	// drop down to ground
	Vector GroundPos = DropToGround(pPlayer, pSpawnSpot->GetAbsOrigin(), VEC_HULL_MIN_SCALED(pTFPlayer),
									VEC_HULL_MAX_SCALED(pTFPlayer));
...
bool CTFGameRules::IsSpawnPointValid(CBaseEntity *pSpot, CBasePlayer *pPlayer, bool bIgnorePlayers,
									 PlayerTeamSpawnMode_t nSpawnMode /* = 0*/)
{
	...
	trace_t trace;
	UTIL_TraceHull(pSpot->GetAbsOrigin(), pSpot->GetAbsOrigin(), mins, maxs, MASK_PLAYERSOLID, pPlayer,
...
void CTFGameRules::ClientSettingsChanged(CBasePlayer *pPlayer)
{
	...
	pTFPlayer->SetRememberActiveWeapon(
		Q_atoi(engine->GetClientConVarValue(pPlayer->entindex(), "tf_remember_activeweapon")) > 0);
	pTFPlayer->SetRememberLastWeapon(
		Q_atoi(engine->GetClientConVarValue(pPlayer->entindex(), "tf_remember_lastswitched")) > 0);
	...
	pTFPlayer->m_bFlipViewModels =
		Q_strcmp(engine->GetClientConVarValue(pPlayer->entindex(), "cl_flipviewmodels"), "1") == 0;
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool EconEntity_KillEaterEventPassesRestrictionCheck(const IEconItemInterface *pEconInterface,
													 const CEconItemAttributeDefinition *pRestrictionAttribDef,
													 const CEconItemAttributeDefinition *pRestrictionValueAttribDef,
													 const CSteamID VictimSteamID)
{
...
{
public:
	CStrangeEventValidator(IEconItemInterface *pEconEntity, CTFPlayer *pOwner, CTFPlayer *pVictim,
						   kill_eater_event_t eEventType)
...
private:
	bool BInitEventParams(IEconItemInterface *pEconEntity, CTFPlayer *pOwner, CTFPlayer *pVictim)
	{
		...
		for(int i = 0; i < GetKillEaterAttrCount(); i++)
		{
			...
			kill_eater_event_t eMatchEvent =
				FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pEconEntity, pScoreTypeAttribDef, &fKillEaterEventType)
			...
			if(m_eEventType == eMatchEvent)
			{
				...
				if(EconEntity_KillEaterEventPassesRestrictionCheck(pEconEntity, GetKillEaterAttr_Restriction(i),
																   GetKillEaterAttr_RestrictionValue(i),
																   m_VictimSteamID))
					return true;
			}
		}
...
CTFWeaponBase *GetKilleaterWeaponFromDamageInfo(const CTakeDamageInfo *pInfo)
{
	...
	if(!pTFWeapon)
	{
		...
		if(!pSentrygun)
		{
			// If a sentry's rocket did the damage, then the rocket is the inflictor, and the sentry is the rocket's
			// owner
		...
		if(pSentrygun)
		{
			...
			if(pBuilder)
			{
				...
				CTFLaserPointer *pLaserPointer =
					dynamic_cast<CTFLaserPointer *>(pBuilder->GetEntityForLoadoutSlot(LOADOUT_POSITION_SECONDARY));
...
//-----------------------------------------------------------------------------
void EconEntity_ValidateAndSendStrangeMessageToGC(IEconItemInterface *pEconEntity, CTFPlayer *pOwner,
												  CTFPlayer *pVictim, kill_eater_event_t eEventType,
												  int nIncrementValue /*= 1*/)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void EconEntity_OnOwnerKillEaterEvent(CEconEntity *pEconEntity, CTFPlayer *pOwner, CTFPlayer *pVictim,
									  kill_eater_event_t eEventType, int nIncrementValue /*= 1*/)
{
	if(!pEconEntity)
		return;

	EconItemInterface_OnOwnerKillEaterEvent(pEconEntity->GetAttributeContainer()->GetItem(), pOwner, pVictim,
											eEventType, nIncrementValue);
}
//-----------------------------------------------------------------------------
void EconItemInterface_OnOwnerKillEaterEvent(IEconItemInterface *pEconEntity, CTFPlayer *pOwner, CTFPlayer *pVictim,
											 kill_eater_event_t eEventType, int nIncrementValue /*= 1*/)
{
	...
	for(int i = 0; i < pOwner->GetNumWearables(); ++i)
	{
		...
		EconEntity_ValidateAndSendStrangeMessageToGC(pWearableItem->GetAttributeContainer()->GetItem(), pOwner, pVictim,
													 eEventType, nIncrementValue);
...
// Purpose:
//-----------------------------------------------------------------------------
void EconEntity_OnOwnerKillEaterEventNoPartner(CEconEntity *pEconEntity, CTFPlayer *pOwner,
											   kill_eater_event_t eEventType, int nIncrementValue /*= 1*/)
{
	if(!pEconEntity)
		return;

	EconItemInterface_OnOwnerKillEaterEventNoPartner(pEconEntity->GetAttributeContainer()->GetItem(), pOwner,
													 eEventType, nIncrementValue);
}
//-----------------------------------------------------------------------------
void EconItemInterface_OnOwnerKillEaterEventNoPartner(IEconItemInterface *pEconEntity, CTFPlayer *pOwner,
													  kill_eater_event_t eEventType, int nIncrementValue /*= 1*/)
{
	...
	for(int iPass = 1; iPass <= 2; iPass++)
	{
		for(int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++)
		{
			...
			const bool bCandidatePasses =
				(iPass == 1 &&
				 GetSteamIDForKillEaterScoring(pOtherPlayer).IsValid()) // if we have a real Steam ID we're golden
				||
...
//-----------------------------------------------------------------------------
void EconEntity_ValidateAndSendStrangeMessageToGC_Batched(IEconItemInterface *pEconInterface, class CTFPlayer *pOwner,
														  class CTFPlayer *pVictim, kill_eater_event_t eEventType,
														  int nIncrementValue /*= 1*/)
{
	...
	{
		s_pmsgIncrementKillCountMessageBatch = new GCSDK::CProtoBufMsg<CMsgIncrementKillCountAttribute_Multiple>(
...
//-----------------------------------------------------------------------------
void EconEntity_OnOwnerKillEaterEvent_Batched(CEconEntity *pEconEntity, class CTFPlayer *pOwner,
											  class CTFPlayer *pVictim, kill_eater_event_t eEventType,
											  int nIncrementValue /*= 1*/)
{
	if(!pEconEntity)
	...
	EconItemInterface_OnOwnerKillEaterEvent_Batched(pEconEntity->GetAttributeContainer()->GetItem(), pOwner, pVictim,
													eEventType, nIncrementValue);
}
//-----------------------------------------------------------------------------
void EconItemInterface_OnOwnerKillEaterEvent_Batched(IEconItemInterface *pEconInterface, class CTFPlayer *pOwner,
													 class CTFPlayer *pVictim, kill_eater_event_t eEventType,
													 int nIncrementValue /*= 1*/)
{
	for(int i = 0; i < pOwner->GetNumWearables(); ++i)
	{
		...
		EconEntity_ValidateAndSendStrangeMessageToGC_Batched(pWearableItem->GetAttributeContainer()->GetItem(), pOwner,
															 pVictim, eEventType, nIncrementValue);
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void HatAndMiscEconEntities_OnOwnerKillEaterEventNoParter(class CTFPlayer *pOwner, kill_eater_event_t eEventType,
														  int nIncrementValue)
{
...
//-----------------------------------------------------------------------------
void HatAndMiscEconEntities_OnOwnerKillEaterEvent(class CTFPlayer *pOwner, class CTFPlayer *pVictim,
												  kill_eater_event_t eEventType, int nIncrementValue)
{
	EconItemInterface_OnOwnerKillEaterEvent(NULL, pOwner, pVictim, eEventType, nIncrementValue);
}

//-----------------------------------------------------------------------------
void EconEntity_OnOwnerUniqueEconEvent(IEconItemInterface *pEconEntity, CTFPlayer *pOwner, CTFPlayer *pVictim,
									   kill_eater_event_t eEventType)
{
	CStrangeEventValidator Validator(pEconEntity, pOwner, pVictim, eEventType);
...
//-----------------------------------------------------------------------------
void EconEntity_OnOwnerUniqueEconEvent(CEconEntity *pEconEntity, CTFPlayer *pOwner, CTFPlayer *pVictim,
									   kill_eater_event_t eEventType)
{
	if(!pEconEntity)
		return;
...
//-----------------------------------------------------------------------------
void EconEntity_NonEquippedItemKillTracking(CTFPlayer *pOwner, CTFPlayer *pVictim, item_definition_index_t iDefIndex,
											kill_eater_event_t eEventType, int nIncrementValue = 1)
{
	// Validate
...
//-----------------------------------------------------------------------------
void EconEntity_NonEquippedItemKillTracking_NoPartner(CTFPlayer *pOwner, item_definition_index_t iDefIndex,
													  kill_eater_event_t eEventType, int nIncrementValue)
{
...
void EconEntity_NonEquippedItemKillTracking_NoPartnerBatched(class CTFPlayer *pOwner, item_definition_index_t iDefIndex,
															 kill_eater_event_t eEventType, int nIncrementValue)
{
	// Find a partner or give up
	for(int iPass = 1; iPass <= 2; iPass++)
	{
		for(int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++)
		{
			...
			if(bCandidatePasses)
			{
				...
				CEconItemView *pNonEquippedItem =
					pOwner->Inventory()->GetFirstItemOfItemDef(iDefIndex, pOwner->Inventory());
				if(pNonEquippedItem)
				{
					uint32 unDeactiveDate = 0;
					if(!pNonEquippedItem->FindAttribute(pAttrDef_DeactiveDate, &unDeactiveDate) ||
					   unDeactiveDate > unCurrentDate)
					{
						EconEntity_ValidateAndSendStrangeMessageToGC_Batched(pNonEquippedItem, pOwner, pOtherPlayer,
																			 eEventType, nIncrementValue);
					}
				}
...
void EconEntity_NonEquippedItemKillTracking(CTFPlayer *pOwner, CTFPlayer *pVictim, int nIncrementValue)
{
	...
	FOR_EACH_MAP_FAST(mapOperations, i)
	{
		...
		{
			EconEntity_NonEquippedItemKillTracking(pOwner, pVictim, pOperation->GetRequiredItemDefIndex(),
												   kKillEaterEvent_CosmeticOperationKills, nIncrementValue);
		}
	}
}
...
void CTFGameRules::DropHalloweenSoulPackToTeam(int nAmount, const Vector &vecPosition, int nTeamNumber, int nSourceTeam)
{
	for(int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++)
	{
		...
		if(pTFPlayer->GetTeamNumber() != nTeamNumber || !pTFPlayer->IsAlive() ||
		   pTFPlayer->m_Shared.InCond(TF_COND_HALLOWEEN_GHOST_MODE))
			continue;
...
{
	QAngle angles(0, 0, 0);
	CHalloweenSoulPack *pSoulsPack =
		assert_cast<CHalloweenSoulPack *>(CBaseEntity::CreateNoSpawn("halloween_souls_pack", vecSource, angles, NULL));
...
void CTFGameRules::DropSpellPickup(const Vector &vPosition, int nTier /*= 0*/) const
{
	...
	CSpellPickup *pSpellPickup = assert_cast<CSpellPickup *>(
		CBaseEntity::CreateNoSpawn("tf_spell_pickup", vPosition + Vector(0, 0, 50), vec3_angle, NULL));
	if(pSpellPickup)
	{
...
//
#ifdef STAGING_ONLY
ConVar tf_duck_droprate_min("tf_duck_droprate_min", "-1", FCVAR_REPLICATED,
							"Set Minimum Number of Ducks to Spawn. No upgrade");
ConVar tf_duck_droprate_bias("tf_duck_droprate_bias", "0.2", FCVAR_REPLICATED, "Set Bias on Duck Spawns. No upgrade");
ConVar tf_duck_power_override("tf_duck_power_override", "0", FCVAR_REPLICATED,
							  "Override everyone's duck power and use this value when > 0");
ConVar tf_duck_random_extra("tf_duck_random_extra", "3.0f", FCVAR_REPLICATED,
							"Random Count of Ducks to be added used by bias function");
#endif
ConVar tf_duck_edict_limit("tf_duck_edict_limit", "1900", FCVAR_REPLICATED,
						   "Maximum number of edicts allowed before spawning a duck");
ConVar tf_duck_edict_warning("tf_duck_edict_warning", "1800", FCVAR_REPLICATED,
							 "Maximum number of edicts allowed before slowing duck spawn rate");
void CTFGameRules::DropBonusDuck(const Vector &vPosition, CTFPlayer *pTFCreator /*=NULL*/,
								 CTFPlayer *pAssister /*=NULL*/, CTFPlayer *pTFVictim /*=NULL*/, bool bCrit /*=false*/,
								 bool bObjective /*=false*/) const
{
	...
	if(pTFCreator)
	{
		for(int i = 0; i < pTFCreator->GetNumWearables(); ++i)
		{
			...
			// if ( pWearable->GetAttributeContainer() && pWearable->GetAttributeContainer()->GetItem() )
			{
				...
				// if ( pItem && FindAttribute_UnsafeBitwiseCast<attrib_value_t>( pItem, pAttr_DuckLevelBadge,
				// &iDuckBadgeLevel ) )
				{
					iDuckBadgeLevel++;
				}
	...
	if(iDuckCount > 0)
	{
		...
		for(int i = 0; i < iDuckCount; ++i)
		{
			...
			CBonusDuckPickup *pDuckPickup = assert_cast<CBonusDuckPickup *>(
				CBaseEntity::CreateNoSpawn("tf_bonus_duck_pickup", vecOrigin, vec3_angle, NULL));
			if(pDuckPickup)
			{
				...
				else
				{
					...
					{
						pDuckPickup->m_nSkin =
							GetDuckSkinForClass(iDuckTeam, (pTFVictim && pTFVictim->GetPlayerClass())
...
void CTFGameRules::PlayerKilled(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
	...
	if(pObject)
	{
		...
		if(pObject->ObjectType() == OBJ_SENTRYGUN)
		{
			...
			if(pOwner)
			{
				...
				// if we just got 10 kills with one sentry, tell the owner's client, which will award achievement if it
	...
	// if not killed by suicide or killed by world, see if the scorer had an assister, and if so give the assister
	// credit
	...
	if(pVictim)
	{
		...
		if(pTFScorer && pTFVictim)
		{
			...
			{
				DropBonusDuck(pTFVictim->GetAbsOrigin(), pTFScorer, pAssister, pTFVictim,
							  (info.GetDamageType() & DMG_CRITICAL) != 0);
			}
		}
	...
	if(IFuncPowerupVolumeAutoList::AutoList().Count() !=
	   0) // If there are no func_powerupvolumes in the map, there's no point in checking for imbalances
	{
		if(pTFPlayerScorer && pTFPlayerScorer != pTFPlayerVictim && pTFPlayerScorer->m_Shared.IsCarryingRune() &&
		   !pTFPlayerScorer->m_Shared.InCond(TF_COND_RUNE_IMBALANCE))
		{
			if(!m_bPowerupImbalanceMeasuresRunning &&
			   !PowerupModeFlagStandoffActive()) // Only count if imbalance measures aren't running and there is no flag
												 // standoff
			{
	...
	if(pTFPlayerScorer && pTFPlayerVictim && pTFPlayerScorer != pTFPlayerVictim)
	{
		...
		kill_eater_event_t eKillEaterEvent =
			pTFWeapon												  // if we have a weapon...
				? pTFWeapon->GetKillEaterKillEventType()			  // ...then ask it what type of event to report
				: info.GetDamageCustom() == TF_DMG_CUSTOM_BOOTS_STOMP // if we don't have a weapon, and we're hacking
																	  // for kill types from wearables (!)...
		...
		if(pAttackerEconWeapon)
		{
			if(!(IsPVEModeActive() && pTFPlayerVictim->GetTeamNumber() == TF_TEAM_PVE_INVADERS))
			{
				...
				EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
												 eKillEaterEvent);

				// Cosmetic any kill type tracking
				{
					HatAndMiscEconEntities_OnOwnerKillEaterEvent(pTFPlayerScorer, pTFPlayerVictim,
																 kKillEaterEvent_CosmeticKills);

					// Halloween silliness: overworld kills for Merasmus carnival.
					if(IsHalloweenScenario(HALLOWEEN_SCENARIO_DOOMSDAY) &&
					   !pTFPlayerScorer->m_Shared.InCond(TF_COND_HALLOWEEN_IN_HELL))
					{
						HatAndMiscEconEntities_OnOwnerKillEaterEvent(pTFPlayerScorer, pTFPlayerVictim,
				...
				EconEntity_OnOwnerUniqueEconEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
												  kKillEaterEvent_UniqueEvent__KilledAccountWithItem);

				// Optional Taunt Kill tracking
				if(IsTauntDmg(info.GetDamageCustom()))
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_TauntKill);
				}
				...
				{
					const kill_eater_event_t eClassKillType =
						g_eClassKillEvents[iVictimClassIndex - TF_FIRST_NORMAL_CLASS];
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 eClassKillType);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_VictoryTimeKill);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_AirborneEnemyKill);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_PlayerKillDuringFullMoon);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_PlayerKillPosthumous);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_PlayerKillCritical);
				}
				else
				{
				...
				// Optional: also track kills that were made while we were launched into the air from an explosion (ie.,
				// rocket-jumping).
				if(pTFPlayerScorer->InAirDueToExplosion())
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_PlayerKillWhileExplosiveJumping);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_InvisibleSpiesKilled);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_MedicsWithFullUberKilled);
				}

				// Optional: also track kills where the killer was at low health when they dealt the final damage. Don't
				// count kills with 0 or fewer health -- those would be post-mortem kills instead.
				if(((float)pTFPlayerScorer->GetHealth() / (float)pTFPlayerScorer->GetMaxHealth()) <= 0.1f &&
				   pTFPlayerScorer->GetHealth() > 0)
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_KillWhileLowHealth);
				}

				// Optional: also track kills that take place during the Halloween holiday. We intentionally *do* check
				// against the game rules logic here -- if folks want to enable Halloween mode on a server and play
				// around, I don't see any reason why we benefit from stopping their fun.
				if(TF_IsHolidayActive(kHoliday_Halloween))
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_HalloweenKill);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_UnderwaterKill);
				}

				// Optional: also track kills where we're under the effects of a medic's ubercharge.
				if(pTFPlayerScorer->m_Shared.InCond(TF_COND_INVULNERABLE) ||
				   pTFPlayerScorer->m_Shared.InCond(TF_COND_INVULNERABLE_WEARINGOFF))
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_KillWhileUbercharged);
				}

				// Optional: also track kills where the victim was in the process of carrying the intel, capturing a
				// point, or pushing the cart. The actual logic for "killed a flag carrier" is handled elsewhere because
				// by this point we've forgotten if we had a flag before we died.
				if(pTFPlayerVictim->IsCapturingPoint())
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_DefenderKill);
				}

				// Optional: also track kills where the victim was at least N units from the person dealing the damage,
				// where N is "however far you have to be to get the crowd chear noise". This also specifically checks
				// to make sure the damage dealer is alive to avoid casing where you spectate far away, etc.
				if(pTFPlayerScorer->IsAlive() &&
				   (pTFPlayerScorer->GetAbsOrigin() - pTFPlayerVictim->GetAbsOrigin()).Length() >= 2000.0f)
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_LongDistanceKill);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_PlayersWearingUnusualKill);
				}

				// Optional: also track kills where the victim was on fire at the time that they died. We can't check
				// the condition flag here because that may or may not be active after they die, but instead we test to
				// see whether they still had a burn queued up for the future as a proxy for "was on fire at this
				// point".
				if(pTFPlayerVictim->m_Shared.GetFlameBurnTime() >= gpGlobals->curtime)
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_BurningEnemyKill);
				}

				// Optional: also track kills where this kill ended someone else's killstreak, where "killstreak" starts
				// when the first announcement happens. If Mike gets to hard-code constants, I do, too.
				enum
				{
					kFirstKillStreakAnnouncement = 5
				};
				if(pTFPlayerVictim->m_Shared.GetStreak(CTFPlayerShared::kTFStreak_Kills) >=
				   kFirstKillStreakAnnouncement)
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_KillstreaksEnded);
				}

				// Optional: also track kills where the killer and the victim were basically right next to each other.
				// This is hard-coded to 1.5x default melee swing range.
				if(pTFPlayerScorer->IsAlive() &&
				   (pTFPlayerScorer->GetAbsOrigin() - pTFPlayerVictim->GetAbsOrigin()).Length() <= 72.0f)
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_PointBlankKills);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_FullHealthKills);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_TauntingPlayerKills);
				}
			}
			else
			{
				...
				EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
												 kKillEaterEvent_RobotsDestroyed);
				...
				{
					const kill_eater_event_t eClassKillType =
						g_eRobotClassKillEvents[iVictimClassIndex - TF_FIRST_NORMAL_CLASS];
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 eClassKillType);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_MinibossRobotsDestroyed);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_RobotsDestroyedAfterPenetration);
				}
				...
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_RobotHeadshotKills);
				}

				// Optional: also track kills that take place during the Halloween holiday. See note for
				// kKillEaterEvent_HalloweenKill regarding calling into game rules here.
				if(TF_IsHolidayActive(kHoliday_Halloween))
				{
					EconEntity_OnOwnerKillEaterEvent(pAttackerEconWeapon, pTFPlayerScorer, pTFPlayerVictim,
													 kKillEaterEvent_HalloweenKillRobot);
				}
			}
		}
	}
	...
	if(pAttacker->IsPlayerClass(TF_CLASS_HEAVYWEAPONS))
	{
		...
		for(i = 0; i < iNumHealers; i++)
		{
			...
			if(pMedic && pMedic->m_AchievementData.IsDamagerInHistory(pVictim, 3.0))
			{
				...
				break; // just award it once for each kill...even if the victim attacked more than one medic attached to
					   // the killer
			}
		}
		...
		for(i = 0; i < iNumHealers; i++)
		{
			if(pAttacker->m_Shared.HealerIsDispenser(i))
			{
				...
				break; // just award it once for each kill...even if the attacker is being healed by more than one
					   // dispenser
			}
		}
	}
...
//-----------------------------------------------------------------------------
void CTFGameRules::CalcDominationAndRevenge(CTFPlayer *pAttacker, CBaseEntity *pWeapon, CTFPlayer *pVictim,
											bool bIsAssist, int *piDeathFlags)
{
	//=============================================================================
	// HPE_BEGIN:
...
void CTFGameRules::CreateStandardEntities()
{
	...
	g_pObjectiveResource =
		(CTFObjectiveResource *)CBaseEntity::Create("tf_objective_resource", vec3_origin, vec3_angle);
	...
	*iWeaponID = TF_WEAPON_NONE;
		...
		if(pScorer)
		{
			...
			if(pTFScorer)
			{
				CTFWeaponBase *pWeapon =
					dynamic_cast<CTFWeaponBase *>(pTFScorer->Weapon_GetWeaponByType(TF_WPN_TYPE_PRIMARY));
	...
	else if(0 == Q_strcmp(killer_weapon_name, "obj_attachment_sapper"))
	{
		...
		if(pScorer)
		{
			...
			{
				CTFWeaponBase *pWeapon =
					dynamic_cast<CTFWeaponBase *>(pTFScorer->GetEntityForLoadoutSlot(LOADOUT_POSITION_BUILDING));
	...
	else if((info.GetDamageCustom() == TF_DMG_CUSTOM_STANDARD_STICKY) ||
			(info.GetDamageCustom() == TF_DMG_CUSTOM_AIR_STICKY_BURST))
	{
		...
		if(pScorer)
		{
			...
			{
				CTFWeaponBase *pWeapon =
					dynamic_cast<CTFWeaponBase *>(pTFScorer->Weapon_GetWeaponByType(TF_WPN_TYPE_SECONDARY));
...
CBasePlayer *CTFGameRules::GetAssister(CBasePlayer *pVictim, CBasePlayer *pScorer, CBaseEntity *pInflictor)
{
	...
	if(pTFScorer && pTFVictim)
	{
		...
		// Must be a medic to receive a healing assist, otherwise engineers get credit for assists from dispensers doing
		// healing. Also don't give an assist for healing if the inflictor was a sentry gun, otherwise medics healing
		// engineers get assists for the engineer's sentry kills.
		if(pHealer && (TF_CLASS_MEDIC == pHealer->GetPlayerClass()->GetClassIndex()) &&
		   (NULL == dynamic_cast<CObjectSentrygun *>(pInflictor)))
		{
			return pHealer;
		}
		...
		// See who has damaged the victim 2nd most recently (most recent is the killer), and if that is within a certain
		// time window. If so, give that player an assist.  (Only 1 assist granted, to single other most recent
		// damager.)
...
CBasePlayer *CTFGameRules::GetDeathScorer(CBaseEntity *pKiller, CBaseEntity *pInflictor, CBaseEntity *pVictim)
{
	...
	{
		// If this was an explicit suicide, see if there was a damager within a certain time window.  If so, award this
		// as a kill to the damager.
		CTFPlayer *pTFVictim = ToTFPlayer(pVictim);
		if(pTFVictim)
		{
...
void CTFGameRules::DeathNotice(CBasePlayer *pVictim, const CTakeDamageInfo &info, const char *eventName)
{
	...
	// Kill eater events.
	{
		...
		if(pSentrygun)
		{
			...
			if(pBuilder)
			{
				EconEntity_OnOwnerKillEaterEvent(
					dynamic_cast<CEconEntity *>(pBuilder->GetEntityForLoadoutSlot(LOADOUT_POSITION_MELEE)), pScorer,
					pTFPlayerVictim, kKillEaterEvent_PlayerKillsBySentry);
				// PDA's Also count Sentry kills
				EconEntity_OnOwnerKillEaterEvent(
					dynamic_cast<CEconEntity *>(pBuilder->GetEntityForLoadoutSlot(LOADOUT_POSITION_PDA)), pScorer,
					pTFPlayerVictim, kKillEaterEvent_PlayerKillsBySentry);
				...
				if(pLaserPointer && pLaserPointer->HasLaserDot())
				{
					EconEntity_OnOwnerKillEaterEvent(dynamic_cast<CEconEntity *>(pLaserPointer), pScorer,
													 pTFPlayerVictim,
		...
		if(pAssister)
		{
			EconEntity_OnOwnerKillEaterEvent(dynamic_cast<CEconEntity *>(pAssister->GetActiveWeapon()), pAssister,
											 pTFPlayerVictim, kKillEaterEvent_PlayerKillAssist);
	...
	if(IsFirstBloodAllowed())
	{
		...
		{
			if(!FStrEq(eventName, "fish_notice") && !FStrEq(eventName, "fish_notice__arm") &&
			   !FStrEq(eventName, "throwable_hit"))
			{
#ifndef _DEBUG
				if(GetGlobalTeam(pVictim->GetTeamNumber()) &&
				   GetGlobalTeam(pVictim->GetTeamNumber())->GetNumPlayers() > 1)
#endif // !DEBUG
				{
		...
		FOR_EACH_VEC(vecItems, i)
		{
			...
			if(pEconItemView->GetCustomName())
			{
				sAssisterOverrideDesc =
					CFmtStr("%c%s",
							iKillerHasPetItem == 2 ? kHorriblePyroVisionHack_KillAssisterType_CustomName_First
												   : kHorriblePyroVisionHack_KillAssisterType_CustomName,
							pEconItemView->GetCustomName());
			}
			else
			{
				sAssisterOverrideDesc =
					CFmtStr("%c%s",
							iKillerHasPetItem == 2 ? kHorriblePyroVisionHack_KillAssisterType_LocalizationString_First
												   : kHorriblePyroVisionHack_KillAssisterType_LocalizationString,
							pEconItemView->GetItemDefinition()->GetItemBaseName())
						.Get();
			}
			break;
		}
	...
	if(event)
	{
		...
		if(FStrEq(eventName, "player_death"))
		{
			...
			if(pTFPlayerVictim && pScorer && pTFPlayerVictim != pScorer)
			{
				...
				event->SetInt("duck_streak_victim",
							  pTFPlayerVictim->m_Shared.GetStreak(CTFPlayerShared::kTFStreak_Ducks));
				...
				if(!pAttribInterface)
				{
					...
					{
						pKillStreakTarget =
							dynamic_cast<CTFWeaponBase *>(pScorer->GetEntityForLoadoutSlot(LOADOUT_POSITION_MELEE));
					}
				}
				else
				{
				...
				if(pAssister)
				{
					event->SetInt("duck_streak_assist",
								  pAssister->m_Shared.GetStreak(CTFPlayerShared::kTFStreak_Ducks));
			...
			if(!pPlayer->IsBot() && State_Get() != GR_STATE_RND_RUNNING)
			{
				...
				else if(!IsTeamReady(pPlayer->GetTeamNumber()))
				{
					...
					else
					{
						...
						// If so, we want to cancel the ready_state so it doesn't start right away when this player
						// disconnects
...
void CTFGameRules::SendArenaWinPanelInfo(void)
{
	...
	if(winEvent)
	{
		...
		switch(m_iWinningTeam)
		{
			...
			{
				iBlueScorePrev =
					(iBlueScore - TEAMPLAY_ROUND_WIN_SCORE >= 0) ? (iBlueScore - TEAMPLAY_ROUND_WIN_SCORE) : 0;
			...
			{
				iRedScorePrev =
					(iRedScore - TEAMPLAY_ROUND_WIN_SCORE >= 0) ? (iRedScore - TEAMPLAY_ROUND_WIN_SCORE) : 0;
		...
		winEvent->SetString(
			"cappers", (m_iWinReason == WINREASON_ALL_POINTS_CAPTURED || m_iWinReason == WINREASON_FLAG_CAPTURE_LIMIT)
	...
	if(winEvent)
	{
		...
		winEvent->SetString(
			"cappers", (m_iWinReason == WINREASON_ALL_POINTS_CAPTURED || m_iWinReason == WINREASON_FLAG_CAPTURE_LIMIT)
						   ? m_szMostRecentCappers
						   : "");
		winEvent->SetInt("flagcaplimit",
						 IsPasstimeMode() ? tf_passtime_scores_per_round.GetInt() : tf_flag_caps_per_round.GetInt());
		winEvent->SetInt("blue_score", iBlueScore);
		winEvent->SetInt("red_score", iRedScore);
		winEvent->SetInt("blue_score_prev", iBlueScorePrev);
		winEvent->SetInt("red_score_prev", iRedScorePrev);
		...
		for(iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++)
		{
			...
			int iPlayerTeam = pTFPlayer->GetTeamNumber();
			if((iPlayerTeam < FIRST_GAME_TEAM) ||
			   (m_iWinningTeam != TEAM_UNASSIGNED && (m_iWinningTeam != iPlayerTeam)))
				continue;
...
#endif // TF_RAID_MODE
		{
			...
			{
				if(g_hControlPointMasters.Count() && g_hControlPointMasters[0] &&
				   g_hControlPointMasters[0]->PlayingMiniRounds())
				{
			...
			else
			{
				...
				if(g_hControlPointMasters.Count() && g_hControlPointMasters[0])
				{
					winEvent->SetInt("rounds_remaining",
									 g_hControlPointMasters[0]->CalcNumRoundsRemaining(m_iWinningTeam));
				}
			}
		}
...
//-----------------------------------------------------------------------------
int CTFGameRules::PlayerRoundScoreSortFunc(const PlayerRoundScore_t *pRoundScore1,
										   const PlayerRoundScore_t *pRoundScore2)
{
...
//-----------------------------------------------------------------------------
int CTFGameRules::PlayerArenaRoundScoreSortFunc(const PlayerArenaRoundScore_t *pRoundScore1,
...
void CTFGameRules::SetupSpawnPointsForRound(void)
{
	...
	// loop through the spawn points in the map and find which ones are associated with this round or the control points
	// in this round
	for(int i = 0; i < ITFTeamSpawnAutoList::AutoList().Count(); ++i)
	{
		CTFTeamSpawn *pTFSpawn = static_cast<CTFTeamSpawn *>(ITFTeamSpawnAutoList::AutoList()[i]);
	...
	if(AreTeamsUnbalanced(iHeavy, iLight) == true)
	{
		...
		for(int iPlayers = 0; iPlayers < iPlayersNeeded; iPlayers++)
		{
			...
			if(pPlayer && pPlayer->IsHLTV() == false && pPlayer->IsReplay() == false)
			{
				...
				UTIL_ClientPrintAll(HUD_PRINTTALK, "#TF_Arena_ClientDisconnect", pPlayer->GetPlayerName(),
									GetGlobalTeam(pPlayer->GetTeamNumber())->GetName(), playername);
...
void CTFGameRules::Arena_PrepareNewPlayerQueue(bool bResetAll)
{
	...
	for(int i = 1; i <= MAX_PLAYERS; i++)
	{
		...
		if(pPlayer && pPlayer->IsHLTV() == false && pPlayer->IsReplay() == false)
		{
			if((GetWinningTeam() > LAST_SHARED_TEAM && pPlayer->GetTeamNumber() != GetWinningTeam() &&
				pPlayer->IsReadyToPlay()) ||
			   bResetAll == true)
			{
...
int CTFGameRules::Arena_PlayersNeededForMatch(void)
{
	...
	//	Msg( "iPlayerNumber: %d - InQueue: %d - iPlayersInWinningTeam: %d - iDesiredTeamSize: %d - iBalancedTeamSize: %d
	//- iPlayersNeeded: %d - bRebalanceWinners %d\n", iPlayerNumber, m_hArenaPlayerQueue.Count(), iPlayersInWinningTeam,
	//iDesiredTeamSize, iBalancedTeamSize, iPlayersNeeded, bRebalanceWinners );
	...
	if(bRebalanceWinners == true)
	{
		while(iPlayersInWinningTeam > iDesiredTeamSize)
		{
			...
			for(int i = 1; i <= MAX_PLAYERS; i++)
			{
				...
				if(pPlayer && pPlayer->GetTeamNumber() == GetWinningTeam() && pPlayer->IsHLTV() == false &&
				   pPlayer->IsReplay() == false)
				{
					if(bRebalanceWinners == true)
					{
...
void CTFGameRules::Arena_RunTeamLogic(void)
{
	...
	{
		if(bGameNotReady == true || GetGlobalTFTeam(TF_TEAM_BLUE)->GetNumPlayers() == 0 ||
		   GetGlobalTFTeam(TF_TEAM_RED)->GetNumPlayers() == 0)
		{
	...
	if(AreTeamsUnbalanced(iHeavy, iLight) == true && iPlayersNeeded > 0)
	{
		...
		int iSwitch = floor(((GetGlobalTFTeam(GetWinningTeam())->GetNumPlayers() + iPlayersNeeded) * 0.5f) -
							GetGlobalTFTeam(GetWinningTeam())->GetNumPlayers());
...
void CTFGameRules::Arena_SendPlayerNotifications(void)
{
	...
	for(int i = 1; i <= MAX_PLAYERS; i++)
	{
		...
		if(pPlayer && pPlayer->IsHLTV() == false && pPlayer->IsReplay() == false &&
		   pPlayer->GetDesiredPlayerClassIndex() > TF_CLASS_UNDEFINED)
		{
			iNumPlayers++;
...
{
	// CUSTOM values are usually determined by CalculateCurrencyAmount_CustomPack() and set via
	// CCurrencyPack::SetValue()
...
//-----------------------------------------------------------------------------
int CTFGameRules::DistributeCurrencyAmount(int nAmount, CTFPlayer *pTFPlayer /* = NULL */, bool bShared /* = true */,
										   bool bCountAsDropped /*= false */, bool bIsBonus /*= false */)
{
...
void CTFGameRules::RoundRespawn(void)
{
	...
	{
		CBaseObject *pObj = static_cast<CBaseObject *>(IBaseObjectAutoList::AutoList()[i]);
		if(pObj->ObjectType() == OBJ_SENTRYGUN && pObj->IsEffectActive(EF_NODRAW) == false &&
		   pObj->GetTeamNumber() != m_iWinningTeam)
		{
				...
				if(g_hUpgradeEntity)
				{
					...
					while(pSubKey)
					{
						...
						if(iCount > 0)
						{
							int iItemSlot = pSubKey->GetInt("itemslot");
							int iUpgrade = pSubKey->GetInt("upgrade");
							bool bFree = (sv_cheats && sv_cheats->GetBool())
											 ? pSubKey->GetBool("free", false)
											 : false; // Never let a client set "free" without sv_cheats 1
...
{
	int paneltype = clamp(pKeyValues->GetInt("panel", DRAWING_PANEL_TYPE_NONE), DRAWING_PANEL_TYPE_NONE,
						  DRAWING_PANEL_TYPE_MAX - 1);
...
void CTFGameRules::CheckHelltowerCartAchievement(int iTeam)
{
	if(IsHalloweenScenario(HALLOWEEN_SCENARIO_HIGHTOWER))
	{
		...
		FOR_EACH_VEC(playerVector, i)
		{
			...
			if(pPlayer &&
			   (pPlayer->GetObserverMode() <= OBS_MODE_DEATHCAM)) // they might be killed by the explosion, so check if
																  // they are OBS_MODE_NONE OR OBS_MODE_DEATHCAM
			{
...
bool CTFGameRules::CanFlagBeCaptured(CBaseEntity *pOther)
{
	if(pOther && (tf_flag_return_on_touch.GetBool() || IsPowerupMode()))
	{
		for(int i = 0; i < ICaptureFlagAutoList::AutoList().Count(); ++i)
		{
			...
			if((pListFlag->GetType() == TF_FLAGTYPE_CTF) && !pListFlag->IsDisabled() &&
			   (pOther->GetTeamNumber() == pListFlag->GetTeamNumber()) && !pListFlag->IsHome())
				return false;
		}
	}
	return true;
...
bool CTFGameRules::PowerupModeFlagStandoffActive(void)
{
	if(IsPowerupMode())
	{
		int nQualifyingFlags = 0; // All flags need to be stolen and poisonous (poisonous time delay gives the flag
								  // carriers a chance to get out of the enemy base)
		int nEnabledFlags =
			0; // Some flags might be in the autolist but be out of play. We don't want them included in the count
		for(int i = 0; i < ICaptureFlagAutoList::AutoList().Count(); ++i)
		{
			CCaptureFlag *pFlag = static_cast<CCaptureFlag *>(ICaptureFlagAutoList::AutoList()[i]);
			if(!pFlag->IsDisabled())
...
void CTFGameRules::TeleportPlayersToTargetEntities(int iTeam, const char *pszEntTargetName,
...
//-----------------------------------------------------------------------------
int FindScoreDifferenceBetweenTeams(CUtlVector<CTFPlayer *> &vecSource, CTFPlayerResource *pPR, int &nRedScore,
									int &nBlueScore)
{
	if(!pPR)
		return false;
	...
	/*
		if ( IsCompetitiveMode() )
		{
			if ( State_Get() == GR_STATE_RND_RUNNING )
			{
				...
				{
					bool bPlayingMiniRounds = ( g_hControlPointMasters.Count() && g_hControlPointMasters[0] &&
	   g_hControlPointMasters[0]->PlayingMiniRounds() );
							...
							for ( int iTeam = TF_TEAM_RED; iTeam < TF_TEAM_COUNT; iTeam++ )
							{
								...
								if ( pTeam )
								{
									if ( ( tf_flag_caps_per_round.GetInt() - pTeam->GetFlagCaptures() ) <= 1 )
									{
										...
										{
											pFlag = static_cast< CCaptureFlag* >(
	   ICaptureFlagAutoList::AutoList()[iFlag] ); if ( !pFlag->IsDisabled() && ( pFlag->GetTeamNumber() == iTeam ) &&
	   !pFlag->IsHome() ) return true;
										}
									}
								}
							}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFGameRules::PlayerMayCapturePoint(CBasePlayer *pPlayer, int iPointIndex, char *pszReason /* = NULL */,
										 int iMaxReasonLength /* = 0 */)
{
	...
	if((pTFPlayer->m_Shared.IsInvulnerable() || pTFPlayer->m_Shared.InCond(TF_COND_MEGAHEAL)) &&
	   !pTFPlayer->m_bInPowerPlay && !IsMannVsMachineMode())
	{
		if(pszReason)
		{
	...
	if(pTFPlayer->m_Shared.InCond(TF_COND_DISGUISED) &&
	   pTFPlayer->m_Shared.GetDisguiseTeam() != pTFPlayer->GetTeamNumber())
	{
		if(pszReason)
		{
	...
	if(IsInTraining() && pTFPlayer->IsBotOfType(TF_BOT_TYPE))
	{
		switch(GetGameType())
		{
			...
			case TF_GAMETYPE_ESCORT:
			{
				...
				while((watcher = dynamic_cast<CTeamTrainWatcher *>(
						   gEntList.FindEntityByClassname(watcher, "team_train_watcher"))) != NULL)
				{
...
int CTFGameRules::CalcPlayerScore(RoundStats_t *pRoundStats, CTFPlayer *pPlayer)
{
	...
	int iScore =
		(pRoundStats->m_iStat[TFSTAT_KILLS] * TF_SCORE_KILL) +
		(pRoundStats->m_iStat[TFSTAT_KILLS_RUNECARRIER] *
		 TF_SCORE_KILL_RUNECARRIER) + // Kill someone who is carrying a rune
		...
		(iHealing /
		 ((bMvM) ? TF_SCORE_DAMAGE : TF_SCORE_HEAL_HEALTHUNITS_PER_POINT)) + // MvM values healing more than PvP
	...
	if(pPlayer)
	{
		...
		{
			// Increment Score
			iScore += (pRoundStats->m_iStat[TFSTAT_KILLS]) + (pRoundStats->m_iStat[TFSTAT_CAPTURES]) +
					  (pRoundStats->m_iStat[TFSTAT_DEFENSES]) + (pRoundStats->m_iStat[TFSTAT_BUILDINGSDESTROYED]);
...
int CTFGameRules::CalcPlayerSupportScore(RoundStats_t *pRoundStats, int iPlayerIdx)
{
	...
	return (pRoundStats->m_iStat[TFSTAT_DAMAGE_ASSIST] + pRoundStats->m_iStat[TFSTAT_HEALING_ASSIST] +
			pRoundStats->m_iStat[TFSTAT_DAMAGE_BLOCKED] + (pRoundStats->m_iStat[TFSTAT_BONUS_POINTS] * 25));
#else
	Assert(g_TF_PR);
	if(!g_TF_PR)
		return 0;

	return g_TF_PR->GetDamageAssist(iPlayerIdx) + g_TF_PR->GetHealingAssist(iPlayerIdx) +
		   g_TF_PR->GetDamageBlocked(iPlayerIdx) + (g_TF_PR->GetBonusPoints(iPlayerIdx) * 25);
#endif
}
...
void CTFGameRules::SetUpVisionFilterKeyValues(void)
{
	...
	pKVFlag->SetString("flamethrower_rainbow",
					   "flamethrower_rainbow"); // Rainblower defaults to rainbows and we want to ensure that we use it
	...
	pKVFlag = new KeyValues("1"); // TF_VISION_FILTER_PYRO
	pKVFlag->SetString(
		"weapons/rainblower/rainblower_start.wav",
		"weapons/rainblower/rainblower_start.wav"); // If we're in PyroVision, explicitly set to ensure this is saved
	...
	// No special vision
	pKVFlag = new KeyValues("0");
	pKVFlag->SetString("models/weapons/c_models/c_rainblower/c_rainblower.mdl",
	...
	pKVFlag->SetString("models/weapons/c_models/c_rainblower/c_rainblower.mdl",
					   "models/weapons/c_models/c_rainblower/c_rainblower.mdl"); // explicit set for pyrovision
	pKVFlag->SetString("models/weapons/c_models/c_lollichop/c_lollichop.mdl",
	...
	pKVFlag->SetString("models/player/items/pyro/xms_pyro_bells.mdl",
					   "models/player/items/all_class/mtp_bottle_pyro.mdl");
	pKVFlag->SetString("models/player/items/soldier/ds_can_grenades.mdl",
...
const char *CTFGameRules::TranslateEffectForVisionFilter(const char *pchEffectType, const char *pchEffectName)
{
	...
	if(pkvParticles)
	{
		for(KeyValues *pkvFlag = pkvParticles->GetFirstTrueSubKey(); pkvFlag != NULL;
			pkvFlag = pkvFlag->GetNextTrueSubKey())
		{
	...
	// Return the top item in the list.  Currently Halloween replacements would out prioritize Pyrovision if there is
	// any overlap
...
{
	// if there's a roster for this bot's team, then check to see if the class the bot has requested is allowed by the
	// roster
	...
	if(pMaster)
	{
		for(int i = 0; i < pMaster->GetNumPoints(); ++i)
		{
			...
			if(point && pMaster->IsInRound(point))
			{
				...
				{
					if(TeamplayGameRules()->TeamMayCapturePoint(GetEnemyTeam(player->GetTeamNumber()),
																point->GetPointIndex()))
					{
...
CObjectSentrygun *CTFGameRules::FindSentryGunWithMostKills(int team) const
{
	...
	{
		CBaseObject *pObj = static_cast<CBaseObject *>(IBaseObjectAutoList::AutoList()[i]);
		if(pObj->ObjectType() == OBJ_SENTRYGUN && pObj->GetTeamNumber() == team &&
		   pObj->GetKills() >= dangerousSentryKills)
		{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFGameRules::ClientConnected(edict_t *pEntity, const char *pszName, const char *pszAddress, char *reject,
								   int maxrejectlen)
{
	if(IsMannVsMachineMode())
	{
...
void CTFGameRules::Status(void (*print)(const char *fmt, ...))
{
	...
	for(int iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_LAST_NORMAL_CLASS; iClass++)
	{
		...
		print("%-8s %6d %6d %5d %6d %7d\n", g_aPlayerClassNames_NonLocalized[iClass], Stats.iSpawns, Stats.iScore,
			  Stats.iKills, Stats.iDeaths, Stats.iAssists);
	}
	print("\n");
}
...
CAmmoDef *GetAmmoDef()
{
	...
	if(!bInitted)
	{
		...
		for(int i = 1; i < TF_AMMO_COUNT; i++)
		{
			...
			def.AddAmmoType(pszAmmoName, DMG_BULLET | DMG_USEDISTANCEMOD | DMG_NOCLOSEDISTANCEMOD, TRACER_LINE, 0, 0,
							"ammo_max", 2400, 10, 14);
...
Vector MaybeDropToGround(CBaseEntity *pMainEnt, bool bDropToGround, const Vector &vPos, const Vector &vMins,
						 const Vector &vMaxs)
{
	if(bDropToGround)
	{
		trace_t trace;
		UTIL_TraceHull(vPos, vPos + Vector(0, 0, -500), vMins, vMaxs, MASK_SOLID, pMainEnt, COLLISION_GROUP_NONE,
					   &trace);
		return trace.endpos;
	}
	else
...
void CTFGameRules::InitCustomResponseRulesDicts()
{
	...
	for(int iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_CLASS_COUNT_ALL; ++iClass)
	{
		...
		for(int iConcept = 0; iConcept < MP_TF_CONCEPT_COUNT; ++iConcept)
		{
			...
			m_ResponseRules[iClass].m_ResponseSystems[iConcept] = BuildCustomResponseSystemGivenCriteria(
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFGameRules::SendHudNotification(IRecipientFilter &filter, const char *pszText, const char *pszIcon,
									   int iTeam /*= TEAM_UNASSIGNED*/)
{
	if(IsInWaitingForPlayers())
		return;
...
{
	CTFPlayer *pTFPlayer = ToTFPlayer(pPlayer);
	if(IsPVEModeActive() && pTFPlayer && pTFPlayer->GetTeamNumber() == TF_TEAM_PVE_DEFENDERS &&
	   pTFPlayer->GetPlayerClass()->GetClassIndex() == TF_CLASS_SCOUT)
	{
		return true;
	}
...
const char *CTFGameRules::FormatVideoName(const char *videoName, bool bWithExtension /*= true*/)
{
	...
	if(Q_strstr(videoName, "arena_"))
	{
		...
		if(!g_pVideo ||
		   g_pVideo->LocatePlayableVideoFile(strTempPath, "GAME", &vSystem, strFullpath, sizeof(strFullpath),
											 VideoSystemFeature::PLAY_VIDEO_FILE_IN_MATERIAL) != VideoResult::SUCCESS)
		{
	...
	if(bWithExtension)
	{
		Q_strncat(strFullpath, FILE_EXTENSION_ANY_MATCHING_VIDEO,
				  MAX_PATH); // Don't assume any specific video type, let the video services find it
	}
...
const char *GetMapDisplayName(const char *mapName)
{
	...
	if(pszFinal)
	{
		...
		if(pszNextChar)
		{
			if((*pszNextChar == '\0') || ((*pszNextChar == '1') && (*(pszNextChar + 1) == '\0')))
			{
				*pszFinal = '\0';
			}
		}
	...
	if(!Q_strncmp(pszSrc, "cp_", 3) || !Q_strncmp(pszSrc, "tc_", 3) || !Q_strncmp(pszSrc, "pl_", 3) ||
	   !Q_strncmp(pszSrc, "ad_", 3) || !Q_strncmp(pszSrc, "sd_", 3) || !Q_strncmp(pszSrc, "rd_", 3) ||
	   !Q_strncmp(pszSrc, "pd_", 3))
	{
		pszSrc += 3;
	}
	else if(!Q_strncmp(pszSrc, "ctf_", 4) || !Q_strncmp(pszSrc, "plr_", 4))
	{
		pszSrc += 4;
	}
	else if(!Q_strncmp(szTempName, "koth_", 5) || !Q_strncmp(szTempName, "pass_", 5))
	{
		pszSrc += 5;
	}
#ifdef TF_RAID_MODE
...
BEGIN_DATADESC(CArenaLogic)
	DEFINE_OUTPUT(m_OnArenaRoundStart, "OnArenaRoundStart"), DEFINE_OUTPUT(m_OnCapEnabled, "OnCapEnabled"),
		DEFINE_KEYFIELD(m_flTimeToEnableCapPoint, FIELD_FLOAT, "CapEnableDelay"), DEFINE_FUNCTION(ArenaLogicThink),
END_DATADESC()
...
BEGIN_DATADESC(CTrainingModeLogic)
	DEFINE_KEYFIELD(m_nextMapName, FIELD_STRING, "nextMap"),
		...
		DEFINE_OUTPUT(m_outputOnPlayerDied, "OnPlayerDied"), DEFINE_OUTPUT(m_outputOnBotDied, "OnBotDied"),
...
void CTFHolidayEntity::Teleport()
{
	...
	TFGameRules()->TeleportPlayersToTargetEntities(TF_TEAM_RED, CFmtStr("spawn_loot_%s", pszRedString),
												   &vecTeleportedPlayers);
	TFGameRules()->TeleportPlayersToTargetEntities(TF_TEAM_BLUE, CFmtStr("spawn_loot_%s", pszBlueString),
												   &vecTeleportedPlayers);
	...
	FOR_EACH_VEC(vecTeleportedPlayers, i)
	{
		...
		CTFSpellBook *pSpellBook =
			dynamic_cast<CTFSpellBook *>(pPlayer->GetEntityForLoadoutSlot(LOADOUT_POSITION_ACTION));
		if(pSpellBook)
		{
	...
	if(ShouldTauntInHell() || (TFGameRules()->GetHalloweenScenario() == CTFGameRules::HALLOWEEN_SCENARIO_HIGHTOWER))
	{
		...
		SetContextThink(&CTFHolidayEntity::HalloweenTeleportToHellDanceThink, gpGlobals->curtime + flDanceTime,
						"DanceThink1");
		SetContextThink(&CTFHolidayEntity::HalloweenTeleportToHellDanceThink,
						gpGlobals->curtime + flDanceTime + flDanceDuration, "DanceThink2");
	}
}
...
#ifdef GAME_DLL
	if(!Q_strcmp(eventName, "player_turned_to_ghost") || !Q_strcmp(eventName, "player_disconnect") ||
	...
	if(m_hControlPoint)
	{
		if(TeamplayGameRules()->TeamMayCapturePoint(TF_TEAM_BLUE, m_hControlPoint->GetPointIndex()))
		{
			if(!m_pointTimer.HasStarted())
			{
				...
				ObjectiveResource()->SetCPTimerTime(m_hControlPoint->GetPointIndex(),
													gpGlobals->curtime + m_nTimerLength);
			}
			else
			{
...
void CC_Spawn_SoccerBall(const CCommand &args)
{
	...
	if(pPlayer)
	{
		...
		UTIL_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH, MASK_NPCSOLID,
...
void CTFGameRules::OnPlayerSpawned(CTFPlayer *pPlayer)
{
	...
	if(pPlayer->GetSteamID(&steamIDForPlayer))
	{
		...
		if(m_mapCoachToStudentMap.IsValidIndex(idx))
		{
			...
			for(int i = 1; i <= gpGlobals->maxClients; i++)
			{
				...
				// is this the student?
				if(pPotentialStudent->GetSteamID(&steamIDForStudent) &&
				   steamIDForStudent.GetAccountID() == studentAccountID)
				{
	...
	if(!IsInTraining())
	{
		...
		if(IsBirthday() && !m_hasSpawnedToy &&
		   pPlayer->GetTeamNumber() ==
			   TF_TEAM_BLUE && // always give ball to first blue player, since they are often trapped during setup
...
GC_REG_JOB(GCSDK::CGCClient, CGCCoaching_CoachJoining, "CGCCoaching_CoachJoining", k_EMsgGCCoaching_CoachJoining,
...
GC_REG_JOB(GCSDK::CGCClient, CGCCoaching_RemoveCurrentCoach, "CGCCoaching_RemoveCurrentCoach",
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		{
			GCSDK::CProtoBufMsg<CMsgGC_GameServer_UseServerModificationItem_Response> msgResponse(
...
GC_REG_JOB(GCSDK::CGCClient, CGCUseServerModificationItemJob, "CGCUseServerModificationItemJob",
		   k_EMsgGC_GameServer_UseServerModificationItem, GCSDK::k_EServerTypeGCClient);
...
GC_REG_JOB(GCSDK::CGCClient, CGCUpdateServerModificationItemStateJob, "CGCUpdateServerModificationItemStateJob",
		   k_EMsgGC_GameServer_ModificationItemState, GCSDK::k_EServerTypeGCClient);
...
bool CTFGameRules::ShouldGoToBonusRound(void)
{
	...
	float flChance = RemapValClamped(flRoundTime, (3 * 60), (30 * 60), 0.0,
									 0.75); // 75% chance for > 30 min rounds, down to 0% at 3 min rounds
...
void CTFGameRules::BetweenRounds_Start(void)
{
	...
	if(pMatchDesc && pMatchDesc->m_params.m_bAutoReady)
	{
		for(int i = 1; i <= MAX_PLAYERS; i++)
		{
			...
			if(IsValidTFTeam(pPlayer->GetTeamNumber()) && pPlayer->GetPlayerClass() &&
			   IsValidTFPlayerClass(pPlayer->GetPlayerClass()->GetClassIndex()))
			{
...
void CTFGameRules::BetweenRounds_Think(void)
{
	if(UsePlayerReadyStatusMode())
	{
		...
		bool bStartFinalCountdown = (PlayerReadyStatus_ShouldStartCountdown() ||
									 (m_flRestartRoundTime > 0 && (int)(m_flRestartRoundTime - gpGlobals->curtime) ==
																	  mp_tournament_readymode_countdown.GetInt()));
	...
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(pPlayer && (pPlayer->GetTeamNumber() > TEAM_UNASSIGNED))
		{
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" position_report (position \"%d %d %d\")\n", pPlayer->GetPlayerName(),
						   ...
						   (int)pPlayer->GetAbsOrigin().x, (int)pPlayer->GetAbsOrigin().y,
...
void CTFGameRules::MatchSummaryTeleport()
{
	...
	if(bUseMatchSummaryStage && m_bMapHasMatchSummaryStage)
	{
		...
		CObserverPoint *pObserverPoint =
			dynamic_cast<CObserverPoint *>(gEntList.FindEntityByClassname(NULL, "info_observer_point"));
		while(pObserverPoint)
		{
			...
			pObserverPoint =
				dynamic_cast<CObserverPoint *>(gEntList.FindEntityByClassname(pObserverPoint, "info_observer_point"));
		}
		...
		for(int i = 1; i <= MAX_PLAYERS; i++)
		{
			...
			if(pPlayer->GetTeamNumber() >=
			   FIRST_GAME_TEAM) // spectators automatically get the RequiredObserverTarget that was set above
			{
				CTFPlayer *pTFPlayer = ToTFPlayer(pPlayer);
				if(pTFPlayer)
				{
					pTFPlayer->m_Shared.AddCond((pTFPlayer->GetTeamNumber() == GetWinningTeam())
...
int CTFGameRules::GetTeamAssignmentOverride(CTFPlayer *pTFPlayer, int iDesiredTeam, bool bAutoBalance /*= false*/)
{
	...
	CMatchInfo::PlayerMatchData_t *pMatchPlayer =
		(pMatch && steamID.IsValid()) ? pMatch->GetMatchDataForPlayer(steamID) : NULL;
	...
	if(IsMannVsMachineMode())
	{
		if(!pTFPlayer->IsBot() && iTeam != TEAM_SPECTATOR)
		{
			...
			{
				// no room
				Log("MVM assigned %s to spectator, all slots for defending team are in use, or reserved for lobby "
					"members\n",
					pTFPlayer->GetPlayerName());
...
void CTFGameRules::PushAllPlayersAway(const Vector &vFromThisPoint, float flRange, float flForce, int nTeam,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFGameRules::CanUpgradeWithAttrib(CTFPlayer *pPlayer, int iWeaponSlot, attrib_definition_index_t iAttribIndex,
										CMannVsMachineUpgrades *pUpgrade)
{
	...
	bool bHideDmgUpgrades = iWeaponID == TF_WEAPON_NONE || iWeaponID == TF_WEAPON_LASER_POINTER ||
	...
	// This is crappy, but it's hopefully more maintainable than the allowed attributes block for all current & future
	// items
	switch(iAttribIndex)
	{
		case 2: // "damage bonus"
		{
			...
			if(pPlayer->IsPlayerClass(TF_CLASS_DEMOMAN))
			{
				...
				{
					CTFWearableDemoShield *pWearableShield =
						dynamic_cast<CTFWearableDemoShield *>(pPlayer->GetWearable(i));
					if(pWearableShield)
					{
			...
			return ((iWeaponSlot == TF_WPN_TYPE_PRIMARY &&
					 (pPlayer->IsPlayerClass(TF_CLASS_SCOUT) || pPlayer->IsPlayerClass(TF_CLASS_SNIPER) ||
					  pPlayer->IsPlayerClass(TF_CLASS_SOLDIER) || pPlayer->IsPlayerClass(TF_CLASS_PYRO))) ||
		...
		case 6: // "fire rate bonus"
		{
			...
			// Non-melee version
			return (dynamic_cast<CTFWeaponBaseMelee *>(pEntity) == NULL && iWeaponID != TF_WEAPON_NONE &&
					!bHideDmgUpgrades && iWeaponID != TF_WEAPON_FLAMETHROWER &&
					!WeaponID_IsSniperRifleOrBow(iWeaponID) && !(pWeapon && pWeapon->HasEffectBarRegeneration()) &&
					!bMinigun);
		}
		break;
		...
		{
			return (
				iWeaponID == TF_WEAPON_FLAMETHROWER && pWeaponGun &&
				assert_cast<CTFFlameThrower *>(pWeaponGun)->SupportsAirBlastFunction(TF_FUNCTION_AIRBLAST_PUSHBACK));
		}
		break;
		case 266: // "projectile penetration"
		{
		...
		{
			return (pWeapon && pWeapon->HasEffectBarRegeneration() && iWeaponID != TF_WEAPON_BUILDER &&
		...
		{
			return ((pWeapon && pWeapon->ReloadsSingly()) || WeaponID_IsSniperRifleOrBow(iWeaponID) ||
					iWeaponID == TF_WEAPON_FLAREGUN);
		}
		break;
		...
		case 335: // "clip size bonus upgrade"
		{
			return (pWeapon && !pWeapon->IsBlastImpactWeapon() && pWeapon->UsesClipsForAmmo1() &&
					pWeapon->GetMaxClip1() > 1 && iWeaponID != TF_WEAPON_FLAREGUN_REVENGE &&
					iWeaponID != TF_WEAPON_SPELLBOOK);
		}
		break;
		...
		{
			return (pPlayer->IsPlayerClass(TF_CLASS_SNIPER) && iWeaponSlot == TF_WPN_TYPE_PRIMARY && pWeaponGun &&
					pWeaponGun->GetWeaponProjectileType() == TF_PROJECTILE_BULLET);
		}
		break;
