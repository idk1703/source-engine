...
//-----------------------------------------------------------------------------
#ifdef CLIENT_DLL
CTFViewModel::CTFViewModel() : m_LagAnglesHistory("CPredictedViewModel::m_LagAnglesHistory"), m_bBodygroupsDirty(true)
{
	m_vLagAngles.Init();
...
ConVar tf_viewmodels_offset_override("tf_viewmodels_offset_override", "", FCVAR_CHEAT,
									 "If set, this will override the position of all viewmodels. Usage 'x y z'");
#endif
...
void CTFViewModel::CalcViewModelView(CBasePlayer *owner, const Vector &eyePosition, const QAngle &eyeAngles)
{
	...
	m_vLoweredWeaponOffset.x =
		Approach(bLowered ? cl_gunlowerangle.GetFloat() : 0, m_vLoweredWeaponOffset.x, cl_gunlowerspeed.GetFloat());
