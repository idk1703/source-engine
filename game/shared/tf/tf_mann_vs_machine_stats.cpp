...
BEGIN_NETWORK_TABLE_NOBASE(CMannVsMachineWaveStats, DT_CMannVsMachineWaveStats)
	...
	SendPropInt(SENDINFO(nCreditsDropped), 16, SPROP_UNSIGNED),
		...
		SendPropInt(SENDINFO(nPlayerDeaths), 16, SPROP_UNSIGNED), SendPropInt(SENDINFO(nBuyBacks), 8, SPROP_UNSIGNED),
#else

	RecvPropInt(RECVINFO(nCreditsDropped)), RecvPropInt(RECVINFO(nCreditsAcquired)),
		RecvPropInt(RECVINFO(nCreditsBonus)), RecvPropInt(RECVINFO(nPlayerDeaths)), RecvPropInt(RECVINFO(nBuyBacks)),
#endif
END_NETWORK_TABLE()
...
#else
	RecvPropInt(RECVINFO(m_iCurrentWaveIdx)), RecvPropInt(RECVINFO(m_iServerWaveID)),
		...
		RecvPropInt(RECVINFO(m_iCurrencyCollectedForRespec)), RecvPropInt(RECVINFO(m_nRespecsAwardedInWave)),
#endif
END_NETWORK_TABLE()
...
// Purpose:
//-----------------------------------------------------------------------------
void CMannVsMachineStats::PlayerEvent_Upgraded(CTFPlayer *pTFPlayer, uint16 nItemDef, uint16 nAttributeDef,
											   uint8 nQuality, int16 nCost, bool bIsBottle)
{
	if(pTFPlayer->IsBot() || !TFGameRules() || !TFGameRules()->IsPVEModeActive())
		return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CMannVsMachineStats::NotifyPlayerEvent(CTFPlayer *pTFPlayer, uint32 idxWave, eMannVsMachineEvent eType, int nValue,
											int nParam)
{
	// Deprecated
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CMannVsMachineStats::NotifyTargetPlayerEvent(CTFPlayer *pTFPlayer, uint32 idxWave, eMannVsMachineEvent eType,
												  int nCost)
{
	CBroadcastRecipientFilter filter;
	filter.MakeReliable();
...
{
	// If the wave is different then the current, we're changing waves and need to save this stuff off if we're
	// advancing
	...
	if(idxWave != (int)m_iCurrentWaveIdx)
	{
		...
		FOR_EACH_MAP_FAST(m_currWaveSpendingStats, i)
		{
			...
			{
				allIndex =
					m_allPrevWaveSpendingStats.Insert(m_currWaveSpendingStats.Key(i), CPlayerWaveSpendingStats());
				m_allPrevWaveSpendingStats[allIndex] = m_currWaveSpendingStats[i];
			}
		}
	}
...
//-----------------------------------------------------------------------------
void CMannVsMachineStats::SW_ReportClientUpgradePurchase(uint8 waveIdx, uint16 nItemDef, uint16 nAttributeDef,
														 uint8 nQuality, int16 nCost)
{
	...
	// INT_FIELD( llServerSessionID, ServerSessionID, uint64 )			// Duplicated data from Session table due to game
	// client anomalies INT_FIELD( nServerWaveID, ServerWaveID, int32 )					// Incrementing WaveId
	// PerAttempt
	// INT_FIELD( ulAccountID, AccountID, uint64 )						// Duplicated data from Session table due to game client
	// anomalies INT_FIELD( nWaveNumber, WaveNumber, uint8 ) INT_FIELD( nItemCategoryID, MvMItemCategoryID, int16 )
	// // References TF2MvMItemCategoryReference INT_FIELD( nItemNameID, MvMItemNameID, int32 )					//
	// References TF2MvMItemNameReference INT_FIELD( nItemCost, ItemCost, int32 ) INT_FIELD( RTime32EventTime,
	// EventTime, RTime32 ) PRIMARY_KEY_CLUSTERED( 100, nID ) WIPE_TABLE_BETWEEN_TESTS( k_EWipePolicyWipeForAllTests )
	...
	//	const CEconItemAttributeDefinition *pAttributeDef = ItemSystem()->GetItemSchema()->GetAttributeDefinition(
	//nAttributeDef ); 	if ( !pAttributeDef ) 		return;
	//
	//	float flValue = 1.0f;
	...
	//	//SessionId, ServerSessionId, AccountID, ServerWaveID, WaveNumber, MvMItemCategoryID, MvMItemNameID, ItemCost,
	//EventTime 	KeyValues* pKVData = new KeyValues( "TF2ClientMvMWaveCreditSpend" ); 	pKVData->SetInt( "ID", ++m_nEventID
	//);
	//
	...
	//	else
	//	{
	//		pKVData->SetString( "MvMItemNameID", g_aPlayerClassNames_NonLocalized[
...
void CMannVsMachineStats::SW_ReportClientBuyBackPurchase(uint8 waveIdx, uint16 nCost)
{
	...
	//	// Create and Send the report
	//	//SessionId, ServerSessionId, AccountID, ServerWaveID, WaveNumber, MvMItemCategoryID, MvMItemNameID, ItemCost,
	//EventTime 	KeyValues* pKVData = new KeyValues( "TF2ClientMvMWaveCreditSpend" );
	//
	//	// ID - Auto
	//	// ServerSessionID - Auto
	//	// SessionID - Auto
	...
	//	g_pVGuiLocalize->ConvertUnicodeToANSI( g_pVGuiLocalize->Find( g_aPlayerClassNames[
	//pTFPlayer->GetPlayerClass()->GetClassIndex() ] ), szAnsi, sizeof(szAnsi) ); 	pKVData->SetString( "MvMItemNameID",
	//szAnsi );
	//
	//	// MvMItemCategoryID
...
void CMannVsMachineStats::SW_ReportClientWaveSummary(uint16 waveID, CMannVsMachinePlayerStats stats)
{
	...
	// INT_FIELD( nID, ID, int32 )                                           // Auto-increment fake key
	// INT_FIELD( nServerWaveID, ServerWaveID, int32 )                       // ID of ServerMvMWaveSummary Entry this is
	// linked with, How do we get this? INT_FIELD( llSessionID, SessionID, uint64 ) INT_FIELD( llServerSessionID,
	// ServerSessionID, uint64 )               // Duplicated data from Session table due to game client anomalies
	// INT_FIELD( ulAccountID, AccountID, uint64 )                           // Duplicated data from Session table due
	// to game client anomalies INT_FIELD( nDamageToBots, DamageToBots, int32 ) INT_FIELD( nDamageToTanks,
	// DamageToTanks, int32 ) INT_FIELD( nDamageToGiants, DamageToGiants, int32 ) INT_FIELD( nDeaths, Deaths, uint8 )
	...
	//	if ( pPrimaryDef )
	//	{
	//		g_pVGuiLocalize->ConvertUnicodeToANSI( g_pVGuiLocalize->Find ( pPrimaryDef->GetItemBaseName() ) , szAnsi,
	//sizeof(szAnsi) ); 		pKVData->SetString( "PrimaryWeapon", szAnsi );
	//	}
	//
	//	// Secondary
	//	CEconItemView *pSecondaryView = TFInventoryManager()->GetItemInLoadoutForClass( iClass,
	//LOADOUT_POSITION_SECONDARY ); 	const CEconItemDefinition *pSecondaryDef = pSecondaryView->GetItemDefinition();
	//
	//	if ( pSecondaryDef )
	//	{
	//		g_pVGuiLocalize->ConvertUnicodeToANSI( g_pVGuiLocalize->Find ( pSecondaryDef->GetItemBaseName() ) , szAnsi,
	//sizeof(szAnsi) ); 		pKVData->SetString( "SecondaryWeapon", szAnsi );
	//	}
	//
	//	// Melee
	...
	//	if ( pMeleeDef )
	//	{
	//		g_pVGuiLocalize->ConvertUnicodeToANSI( g_pVGuiLocalize->Find ( pMeleeDef->GetItemBaseName() ) , szAnsi,
	//sizeof(szAnsi) ); 		pKVData->SetString( "MeleeWeapon", szAnsi );
	//	}
	//
...
void MannVsMachineStats_PlayerEvent_Upgraded(CTFPlayer *pTFPlayer, uint16 nItemDef, uint16 nAttributeDef,
											 uint16 nQuality, int16 nCost, bool bIsBottle)
{
	if(g_pMVMStats)
	{
