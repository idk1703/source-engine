...
#ifdef CLIENT_DLL
	RecvPropFloat(RECVINFO(m_flChargeBeginTime)), RecvPropInt(RECVINFO(m_iChargeEffect))
#else
	SendPropFloat(SENDINFO(m_flChargeBeginTime)), SendPropInt(SENDINFO(m_iChargeEffect))
#endif
END_NETWORK_TABLE()
...
void CTFParticleCannon::CreateChargeEffect()
{
	...
	if(pPlayer)
	{
		DispatchParticleEffect("drg_cowmangler_muzzleflash_chargeup", PATTACH_POINT_FOLLOW,
							   GetAppropriateWorldOrViewModel(), "muzzle", GetParticleColor(1), GetParticleColor(2));
	}
}
#endif
