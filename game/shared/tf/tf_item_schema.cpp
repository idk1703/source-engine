...
// For a particular set of KeyValues, ensure that all of the one-level-deep subkeys are a subset of the values in
// testKeys. This ensures that there are no typos in the keynames.
static bool ValidateKeysAreSubset(KeyValues *kv, const CUtlVector<const char *> &testKeys,
								  CUtlVector<CUtlString> *pVecErrors)
{
	...
	// This currently is inefficient, it's O(len(_keyvalues) * len(_testKeys)). It could easily be made faster for large
	// N, but for small lengths cache dominates. It also has the nice property that it will show up on the profiler if
	// it's a problem.
...
//
//-----------------------------------------------------------------------------
bool CTFCraftingRecipeDefinition::ItemListMatchesInputs(CUtlVector<CEconItem *> *vecCraftingItems,
														...
														CUtlVector<uint64> *vecChosenItems) const
{
	...
	int hack_iForcedClass = -1, hack_iForcedSlot = LOADOUT_POSITION_INVALID;
	const CEconItemSetDefinition *hack_pForcedItemSetDef = NULL;

	int *iForcedClass = NULL, *iForcedSlot = NULL;
	...
	if(m_bRequiresAllSameClass)
	{
		...
		{
			const CTFItemDefinition *pDef =
				GetItemSchema()->GetTFItemDefinition(vecCraftingItems->Element(iVC)->GetDefinitionIndex());
			...
			if(m_InputItemsCriteria[i].BEvaluate(pItemDef))
			{
				...
				if(iForcedClass && m_iCacheClassUsageForOutputFromItem == i)
				{
					...
					else if(*iForcedClass == -1)
					{
						...
						// If we need to be the same class, but couldn't find a common class across the items, we're
						// done.
				...
				{
					// If the item def has a slot_token_id key, we use that. Otherwise, we find the first class that
					// uses it.
					...
					{
						// If we have a forced class, we find the slot that class uses. Otherwise, we find the first
						// slot used.
						if(iForcedClass)
						{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFCraftingRecipeDefinition::CanMatchAgainstBackpack(
	CUtlVector<CEconItem *> *vecAllItems, CUtlVector<CEconItem *> vecItemsByClass[LOADOUT_COUNT],
	CUtlVector<CEconItem *> vecItemsBySlot[CLASS_LOADOUT_POSITION_COUNT], CUtlVector<uint64> *vecChosenItems) const
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFCraftingRecipeDefinition::CheckSubItemListAgainstBackpack(CUtlVector<CEconItem *> *vecCraftingItems,
																  CUtlVector<uint64> *vecChosenItems) const
{
	CUtlVector<CEconItem *> vecTmp;
	vecTmp = *vecCraftingItems;
	...
	if(pPerClassData)
	{
		...
		for(int i = TF_FIRST_NORMAL_CLASS; i < TF_LAST_NORMAL_CLASS; i++)
		{
			...
			else if(pszBaseName)
			{
				...
				// So this is fun.  ClassUsabilityStrings refers to the "Demoman", but the vast majority of his models
				// are whatever_demo.mdl The RIGHT fix would be to either:
				//		1) change all the model and content files to whatever_demoman.mdl
				//		2) fixup the schema so every reference to "demoman" is changed to "demo" and update
				//GetClassUsabilityStrings 				and fix everything that breaks
				...
				{
					fmtStr.sprintf(pszBaseName, GetItemSchema()->GetClassUsabilityStrings()[i],
								   GetItemSchema()->GetClassUsabilityStrings()[i],
								   GetItemSchema()->GetClassUsabilityStrings()[i]);
				}
...
// Purpose:
//-----------------------------------------------------------------------------
static bool InitPerClassStringVectorArray(KeyValues *pPerClassData,
										  CUtlVector<const char *> (&outputArray)[LOADOUT_COUNT],
										  CUtlVector<CUtlString> *pVecErrors)
{
	if(pPerClassData)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
static bool InitPerClassRandomChanceStringArray(KeyValues *pPerClassData,
												CRandomChanceString (&outputArray)[LOADOUT_COUNT],
												CUtlVector<CUtlString> *pVecErrors)
{
	if(pPerClassData)
	{
...
{
	static const char *s_pszAllowedTauntInputButtonNames[] = {"IN_ATTACK", "IN_ATTACK2", "IN_FORWARD", "IN_BACK"};
	static int s_iAllowedTauntInputButtons[] = {IN_ATTACK, IN_ATTACK2, IN_FORWARD, IN_BACK};
	...
	FOR_EACH_SUBKEY(pKV, pButtonKey)
	{
		...
		if(pPressedKey || pReleasedKey)
		{
			...
			if(!InitPerClassStringVectorArray(pPressedKey, m_vecTauntInputRemap[iNew].m_vecButtonPressedScenes,
											  pVecErrors))
				return false;

			if(!InitPerClassStringVectorArray(pReleasedKey, m_vecTauntInputRemap[iNew].m_vecButtonReleasedScenes,
											  pVecErrors))
				return false;
		}
	}
...
bool CQuestThemeDefinition::BInitFromKV(KeyValues *pKV, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	m_eUnackPos =
		(unacknowledged_item_inventory_positions_t)m_pRawKVs->GetInt("unack_position", UNACK_ITEM_QUEST_OUTPUT);
...
bool CQuestDefinition::BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors)
{
	...
	SCHEMA_INIT_CHECK(m_nNumObjectivesToRoll <= m_vecObjectiveDefinitions.Count(),
...
void CQuestDefinition::GetRolledObjectivesForItem(QuestObjectiveDefVec_t &vecRolledObjectives,
												  const CEconItem *pItem) const
{
...
bool CTFItemDefinition::BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors)
{
	...
	SCHEMA_INIT_CHECK(m_eEquipType != EQUIP_TYPE_INVALID, "Item definition %i \"%s\" used uknown equip type: %s!",
					  GetDefinitionIndex(), GetItemBaseName(), pszEquipType);
	...
	if(*pszLoadoutSlot)
	{
		...
		m_iDefaultLoadoutSlot =
			StringFieldToInt(pszLoadoutSlot, GetItemSchema()->GetLoadoutStrings(m_eEquipType), true);
		SCHEMA_INIT_CHECK(m_iDefaultLoadoutSlot >= 0, "Item definition %i \"%s\" used unknown loadout slot: %s!",
						  GetDefinitionIndex(), GetItemBaseName(), pszLoadoutSlot);
	}
	...
	SCHEMA_INIT_CHECK(!(bHasAnyWearableSlots && bHasAnyNonwearableSlots),
					  "Item definition %i \"%s\" used in both wearable and not wearable slots!", GetDefinitionIndex(),
					  GetItemBaseName());
	...
	for(int i = 0; i < m_vbClassUsability.GetNumBits(); i++)
	{
		...
		{
			//	SCHEMA_INIT_CHECK( g_pFullFileSystem->FileExists( m_pszPlayerDisplayModel[ i ] ), "Missing model %s
			//specified in model_player_per_class in item %s", m_pszPlayerDisplayModel[ i ], GetItemBaseName() );
		}
	}
#endif
	...
	if(pTauntKV)
	{
		...
		SCHEMA_INIT_CHECK(m_pTauntData->BInitFromKV(pTauntKV, pVecErrors),
						  "Item definition %i \"%s\" failed to initialize taunt data!", GetDefinitionIndex(),
						  GetItemBaseName());
	}
	...
	if(pQuestKV)
	{
		...
		SCHEMA_INIT_CHECK(m_pQuestData->BInitFromKV(pQuestKV, pVecErrors),
						  "Item def %i \"%s\" failed to initialize quest data!", GetDefinitionIndex(),
						  GetItemBaseName());
	}
	...
	for(int i = 0; i < m_vbClassUsability.GetNumBits(); i++)
	{
		...
		{
			m_vecTags.AddToTail(GetItemSchema()->GetHandleForTag(
				CFmtStr("auto__class_%s", GetItemSchema()->GetClassUsabilityStrings()[i]).Get()));
		}
	}
	...
	if(pszPaintKit)
	{
		...
		SCHEMA_INIT_CHECK(GetItemSchema()->GetItemPaintKits().IsValidIndex(iPaintIndex),
						  "Item paintkit [%s] in definition %i \"%s\" does not exist", pszPaintKit,
						  GetDefinitionIndex(), GetItemBaseName());
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFItemDefinition::GeneratePrecacheModelStrings(bool bDynamicLoad,
...
bool CTFQuestObjectiveConditionsDefinition::BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors)
{
	...
	SCHEMA_INIT_CHECK(m_nDefIndex != INVALID_QUEST_OBJECTIVE_CONDITIONS_INDEX,
...
bool CTFQuestObjectiveDefinition::BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	SCHEMA_INIT_CHECK(GetItemSchema()->GetQuestObjectiveConditionByDefIndex(m_nConditionDefIndex) != NULL,
					  "Could not find quest objective conditions for defindex %d!", m_nConditionDefIndex);
...
KeyValues *CTFQuestObjectiveDefinition::GetConditionsKeyValues() const
{
	const CTFQuestObjectiveConditionsDefinition *pDef =
		GetItemSchema()->GetQuestObjectiveConditionByDefIndex(m_nConditionDefIndex);
	if(pDef)
	{
...
static PlayerClassInfo_t gs_PlayerClassData[] = {
	...
	{"Invalid", ""} // lots of code loops over these classes based on LOADOUT_COUNT, which is wrong, but this allows
...
COMPILE_TIME_ASSERT(
	ARRAYSIZE(g_szLoadoutStrings) <=
	CLASS_LOADOUT_POSITION_COUNT); // we don't support mapping directly to slots like "misc2", "taunt2-8", etc.
...
// Loadout positions
const char *g_szAccountLoadoutStrings[] = {"quest", ""
													""};
COMPILE_TIME_ASSERT(
	ARRAYSIZE(g_szAccountLoadoutStrings) <=
	ACCOUNT_LOADOUT_POSITION_COUNT); // we don't support mapping directly to slots like "misc2", "taunt2-8", etc.
...
const char *g_szWeaponTypeSubstrings[] = {
	// Weapons & Equipment
	"PRIMARY", "SECONDARY", "MELEE", "GRENADE",		   "BUILDING",	 "PDA",		"ITEM1",
	"ITEM2",   "HEAD",		"MISC",	 "MELEE_ALLCLASS", "SECONDARY2", "PRIMARY2"};
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFItemSchema::InitializeStringTable(const char **ppStringTable, unsigned int unStringCount,
										  CUtlVector<const char *> *out_pvecStringTable)
{
...
bool CTFItemSchema::BInitSchema(KeyValues *pKVRawDefinition, CUtlVector<CUtlString> *pVecErrors)
{
	...
	if(m_vecClassUsabilityStrings.Size() == 0)
	{
		...
		InitializeStringTable(&g_szLoadoutStringsForDisplay[0], ARRAYSIZE(g_szLoadoutStringsForDisplay),
							  &m_vecClassLoadoutStringsForDisplay);
		Assert(m_vecClassLoadoutStringsForDisplay.Size() == CLASS_LOADOUT_POSITION_COUNT);

		InitializeStringTable(&g_szAccountLoadoutStrings[0], ARRAYSIZE(g_szAccountLoadoutStrings),
							  &m_vecAccountLoadoutStrings);
		Assert(m_vecAccountLoadoutStrings.Size() <= ACCOUNT_LOADOUT_POSITION_COUNT);

		InitializeStringTable(&g_szAccountLoadoutStringsForDisplay[0], ARRAYSIZE(g_szAccountLoadoutStringsForDisplay),
							  &m_vecAccountLoadoutStringsForDisplay);
		Assert(m_vecAccountLoadoutStringsForDisplay.Size() == ACCOUNT_LOADOUT_POSITION_COUNT);

		InitializeStringTable(&g_szWeaponTypeSubstrings[0], ARRAYSIZE(g_szWeaponTypeSubstrings),
							  &m_vecWeaponTypeSubstrings);
...
bool CTFItemSchema::BInitMMCategories(KeyValues *pKVCategories, CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_TRUE_SUBKEY(pKVCategories, pKVCategory)
	{
		...
		SCHEMA_INIT_CHECK(pCat->m_nMaxExcludes != -1,
						  "BInitMMCategories: missing 'max_excludes' for mm category type '%s'",
						  pKVCategory->GetName());
		...
		if(pKVValidMatchGroups)
		{
			...
			{
				EMatchGroup eGroup = (EMatchGroup)StringFieldToInt(pKVGroup->GetName(), s_pszMatchGroups,
																   (int)k_nMatchGroup_Count, false);
...
bool CTFItemSchema::BInitGameModes(KeyValues *pKVMaps, CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_TRUE_SUBKEY(pKVMaps, pKVGameMode)
	{
		...
		SCHEMA_INIT_CHECK(iGameType != -1, "BInitMaps(): unknown game type '%s'", pKVGameMode->GetName());

		EGameCategory eGameType = (EGameCategory)iGameType;

		SchemaGameCategory_t *pGameMode =
			m_mapGameCategories[m_mapGameCategories.Insert(eGameType, new SchemaGameCategory_t())];
		...
		if(nMMType != (int)kMatchmakingType_None)
		{
			...
			SCHEMA_INIT_CHECK(idx != m_mapMMGroups.InvalidIndex(),
							  "mm_type '%s' does not have a matchmaking_categories entry",
							  pKVGameMode->GetString("mm_type"));
			SCHEMA_INIT_CHECK(pGameMode->m_pszLocalizedName != NULL, "game mode '%s' missing localized name!",
							  pKVGameMode->GetName());
			SCHEMA_INIT_CHECK(pGameMode->m_pszLocalizedDesc != NULL, "game mode '%s' missing localized desc!",
							  pKVGameMode->GetName());
			SCHEMA_INIT_CHECK(pGameMode->m_pszListImage != NULL, "game mode '%s' missing list image!",
							  pKVGameMode->GetName());
		...
		if(pKVMapList)
		{
			FOR_EACH_TRUE_SUBKEY(pKVMapList, pKVMap)
			{
				...
				SCHEMA_INIT_CHECK(pMap != NULL, "Map %s listed in game mode %s doesn't exist!",
								  pKVMap->GetString("name"), pGameMode->m_pszName);
		...
		SCHEMA_INIT_CHECK(
			pGameMode->m_vecEnabledMaps.Count(),
			"BInitMaps(): ERROR!! No valid maps for game type %s (at least one must be \"enabled\" in _maps.txt).",
			pKVGameMode->GetName());
	}
	return SCHEMA_INIT_SUCCESS();
}
...
bool CTFItemSchema::BInitMaps(KeyValues *pKVMaps, CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_TRUE_SUBKEY(pKVMaps, pKVMap)
	{
		...
		SCHEMA_INIT_CHECK(pMap->pszMapName != NULL, "BInitMaps(): missing map name for master map entry %s.",
						  pKVMap->GetName());
		...
		SCHEMA_INIT_CHECK((pMap->mapStampDef == NULL) == (pszMapStampname[0] == '\0'),
						  "BInitGameModes(): unable to find map stamp definition '%s' for map '%s'.", pszMapStampname,
						  pMap->pszMapName);
		...
		if(pKVTags)
		{
			FOR_EACH_SUBKEY(pKVTags, pKVTag)
			{
				pMap->m_vecRollingMatchTargets.AddToTail(
					{GetHandleForTag(pKVTag->GetName()), pKVTag->GetFloat("weight", 1.f)});
			}
		}
...
bool CTFItemSchema::BPostInitMaps(CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_VEC(m_vecMasterListOfMaps, i)
	{
		...
		{
			const SchemaGameCategory_t *pCategory =
				GetItemSchema()->GetGameCategory(pMapOuter->m_vecAssociatedGameCategories[j]);
		...
		FOR_EACH_VEC(pMapOuter->m_vecRollingMatchTargets, j)
		{
			...
			FOR_EACH_VEC(m_vecMasterListOfMaps, k)
			{
				...
				{
					const SchemaGameCategory_t *pCategory = GetItemSchema()->GetGameCategory(
						pCandidate->m_vecAssociatedGameCategories[idxCandidateCategory]);

					if(pCategory &&
					   pCategory->m_vecEnabledMaps.Find(pCandidate) != pCategory->m_vecEnabledMaps.InvalidIndex())
					{
						bEnabled = true;
						break;
					}
			...
			FOR_EACH_VEC(pMapOuter->m_vecRollingMatchMaps, k)
			{
				pMapOuter->m_vecRollingMatchMaps[k].m_flWeight =
					pMapOuter->m_vecRollingMatchMaps[k].m_flWeight / flMaxWeight;
			}
		}

		// We only have to roll one-less than the amount needed because the current map is always selected
		SCHEMA_INIT_CHECK(pMapOuter->m_vecRollingMatchMaps.Count() >= NEXT_MAP_VOTE_OPTIONS - 1,
						  "Not enough maps with matching tags for map %s", pMapOuter->pszMapName);
	}
...
bool CTFItemSchema::BInitMvmMissions(KeyValues *pKVMvmMaps, CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_TRUE_SUBKEY(pKVMvmMaps, pKVMap)
	{
			...
			FOR_EACH_TRUE_SUBKEY(pKVMissions, pKVMission)
			{
				...
				if(mission.m_eDifficulty == k_EMvMChallengeDifficulty_Invalid)
				{
					pVecErrors->AddToTail(
						CUtlString(CFmtStr("MvM mission '%s' on map %s has missing or invalid 'difficulty' value",
										   mission.m_sPop.Get(), map.m_sMap.Get())));
					bResult = false;
					continue;
				}
				...
				{
					pVecErrors->AddToTail(
						CUtlString(CFmtStr("MvM mission '%s' on map %s does not obey map prefix naming convention",
										   mission.m_sPop.Get(), map.m_sMap.Get())));
					bResult = false;
					continue;
				}
		...
		SCHEMA_INIT_CHECK(map.m_vecMissions.Count() > 0, "MvM map %s doesn't have any associated missions",
						  map.m_sMap.Get());
	}
...
bool CTFItemSchema::BInitMvmTours(KeyValues *pKVMvmTours, CUtlVector<CUtlString> *pVecErrors)
{
	...
	FOR_EACH_TRUE_SUBKEY(pKVMvmTours, pKVTour)
	{
		...
		SCHEMA_INIT_CHECK(FindMvmMissionByName(tour.m_sTourInternalName.Get()) < 0, "Duplicate MvM tour \"%s\"",
		...
		tour.m_pBadgeItemDef =
			pszBadgeItemDefName ? GetItemSchema()->GetItemDefinitionByName(pszBadgeItemDefName) : NULL;
		SCHEMA_INIT_CHECK((pszBadgeItemDefName == NULL) == (tour.m_pBadgeItemDef == NULL),
						  "MvM tour \"%s\" specified invalid badge definition name '%s'",
						  tour.m_sTourInternalName.Get(), pszBadgeItemDefName);
		...
		tour.m_pMissionCompleteLootList =
			pszMissionCompleteLootListName ? GetItemSchema()->GetLootListByName(pszMissionCompleteLootListName) : NULL;
		SCHEMA_INIT_CHECK((pszMissionCompleteLootListName == NULL) == (tour.m_pMissionCompleteLootList == NULL),
						  "MvM tour \"%s\" specified invalid mission completion loot list '%s'",
						  tour.m_sTourInternalName.Get(), pszMissionCompleteLootListName);

		const char *pszTourCompleteLootListName = pKVTour->GetString("tour_complete_loot_list");
		tour.m_pTourCompleteLootList =
			pszTourCompleteLootListName ? GetItemSchema()->GetLootListByName(pszTourCompleteLootListName) : NULL;
		SCHEMA_INIT_CHECK((pszTourCompleteLootListName == NULL) == (tour.m_pMissionCompleteLootList == NULL),
						  "MvM tour \"%s\" specified invalid tour completion loot list '%s'",
						  tour.m_sTourInternalName.Get(), pszTourCompleteLootListName);
#endif
		...
		if(pKVMissions)
		{
			...
			bool bContainsBitMissions =
				false; // does this contain any missions with bit set to anything besides -1 (non-practice)?
			...
			FOR_EACH_VALUE(pKVMissions, pKVMission)
			{
				...
				SCHEMA_INIT_CHECK(iMissionBit >= -1 && iMissionBit <= 31,
								  ...
								  tour.m_sTourInternalName.Get(), pszMissionName, iMissionBit);
				...
				SCHEMA_INIT_CHECK(m_vecMvMMissions.IsValidIndex(iMissionIndex),
								  "MvM tour \"%s\" unable to locate mission \"%s\"", tour.m_sTourInternalName.Get(),
								  pszMissionName);
				...
				SCHEMA_INIT_CHECK(
					!bContainsNonBitMissions || !bContainsBitMissions,
					"MvM tour \"%s\" mission \"%s\" contains both practice (-1 bit) and non-practice missions",
					tour.m_sTourInternalName.Get(), pszMissionName);
				...
				if(iMissionBit >= 0)
				{
					...
					SCHEMA_INIT_CHECK((tour.m_nAllChallengesBits & unMask) == 0,
									  "MvM tour \"%s\" mission \"%s\" re-uses bit %i", tour.m_sTourInternalName.Get(),
									  pszMissionName, iMissionBit);
		...
		SCHEMA_INIT_CHECK(tour.m_vecMissions.Count() > 0, "MvM tour \"%s\" has no missions specified",
						  tour.m_sTourInternalName.Get());
...
//-----------------------------------------------------------------------------
const CTFQuestObjectiveConditionsDefinition *CTFItemSchema::GetQuestObjectiveConditionByDefIndex(
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFItemSchema::BCanStrangeFilterApplyToStrangeSlotInItem(uint32 /*strange_event_restriction_t*/ unRestrictionType,
															  uint32 unRestrictionValue,
															  const IEconItemInterface *pItem, int iStrangeSlot,
															  uint32 *out_pOptionalScoreType) const
{
	uint32 unStrangeScoreType;
	if(!CEconItemSchema::BCanStrangeFilterApplyToStrangeSlotInItem(unRestrictionType, unRestrictionValue, pItem,
																   iStrangeSlot, &unStrangeScoreType))
		return false;
...
// Purpose:
//-----------------------------------------------------------------------------
IEconTool *CTFItemSchema::CreateEconToolImpl(const char *pszToolType, const char *pszUseString,
											 ...
											 KeyValues *pUsageKV)
{
	...
	return CEconItemSchema::CreateEconToolImpl(pszToolType, pszUseString, pszUsageRestriction, unCapabilities,
											   pUsageKV);
}

#if defined(STAGING_ONLY) && defined(CLIENT_DLL)
int PaintkitAutocomplete(char const *partial,
...
CON_COMMAND_F_COMPLETION(r_texcomp_debug, "Usage: r_texcomp_debug <paintkit_name> [wear level=1]", 0,
						 PaintkitAutocomplete)
{
	if(args.ArgC() < 2)
	{
	...
	ITextureCompositor *pWeaponSkinBaseCompositor =
		materials->NewTextureCompositor(1, 1, args[1], TF_TEAM_RED, 0, pKV, TEX_COMPOSITE_CREATE_FLAGS_LOG_NODES_ONLY);
	Assert(pWeaponSkinBaseCompositor == NULL);
	pWeaponSkinBaseCompositor;
}
