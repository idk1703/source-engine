...
struct MMRatingData_t
{
	...
	{
		return this->unRatingPrimary == b.unRatingPrimary && this->unRatingSecondary == b.unRatingSecondary &&
...
enum EMatchGroup
{
	...
	k_nMatchGroup_Count,
	// When adding a new matchgroup, add case handling to GetMatchSizeForMatchGroup(), GetMatchGroupName(),
	// GetServerPoolName(), GetMaxLobbySizeForMatchGroup(), YldWebAPIServersByDataCenter()
};
...
enum EMMRatingSource
{
	...
	k_nMMRatingSource_ImportedOldSystem =
		3, // For pre-history ratings, this source is used once for 'initial rating from old system'
...
COMPILE_TIME_ASSERT(k_nGameServerPool_Incomplete_Match_First + k_nMatchGroup_Count - 1 ==
					k_nGameServerPool_Incomplete_Match_Last);
...
{
	return nGameServerPool >= k_nGameServerPool_Incomplete_Match_First &&
		   nGameServerPool <= k_nGameServerPool_Incomplete_Match_Last;
}
