...
bool CTFInventoryManager::EquipItemInLoadout(int iClass, int iSlot, itemid_t iItemID)
{
	...
	if(!AreSlotsConsideredIdentical(pItem->GetStaticData()->GetEquipType(),
...
int CTFInventoryManager::GetNumItemPickedUpItems(void)
{
	...
	for(int i = 0; i < iCount; i++)
	{
		if(IsUnacknowledged(m_LocalInventory.GetItem(i)->GetInventoryPosition()) &&
		   !m_LocalInventory.GetItem(i)->GetStaticData()->IsHidden())
		{
			++iResult;
		}
	}
...
#ifdef STAGING_ONLY
	return ((iSlot < LOADOUT_POSITION_HEAD && iSlot != LOADOUT_POSITION_UTILITY &&
			 !IsTauntSlot(iSlot)) // Allow utility slots to be empty
...
//-----------------------------------------------------------------------------
void CTFInventoryManager::GetActiveSets(CUtlVector<const CEconItemSetDefinition *> *pItemSets,
										CSteamID steamIDForPlayer, int iClass)
{
	pItemSets->Purge();
	...
	for(int i = 0; i < CLASS_LOADOUT_POSITION_COUNT; i++)
	{
		...
		if(pData->GetHolidayRestriction())
		{
			...
			if(iHolidayRestriction != kHoliday_None &&
			   (!TFGameRules() || !TFGameRules()->IsHolidayActive(iHolidayRestriction)))
				continue;
		}
	...
	for(int inv = 0; inv < equippedSetItems.Count(); inv++)
	{
		...
		for(int i = 0; i < pItemSet->m_iItemDefs.Count(); i++)
		{
			...
			for(int j = 0; j < equippedSetItems.Count(); j++)
			{
				...
				const item_definition_index_t unEquippedItemSetDefIndex =
					pTestItem->GetItemDefinition()->GetSetItemRemap();
...
//-----------------------------------------------------------------------------
// Purpose: We're generating a base item. We need to add the game-specific keys to the criteria so that it'll find the
// right base item.
//-----------------------------------------------------------------------------
void CTFInventoryManager::AddBaseItemCriteria(baseitemcriteria_t *pCriteria, CItemSelectionCriteria *pSelectionCriteria)
{
	pSelectionCriteria->BAddCondition("used_by_classes", k_EOperator_Subkey_Contains,
									  ItemSystem()->GetItemSchema()->GetClassUsabilityStrings()[pCriteria->iClass],
									  true);
	pSelectionCriteria->BAddCondition(
		"item_slot", k_EOperator_String_EQ,
		ItemSystem()->GetItemSchema()->GetLoadoutStrings(EEquipType_t::EQUIP_TYPE_CLASS)[pCriteria->iSlot], true);
}
...
void CTFPlayerInventory::CheckSaxtonMaskAchievement(const CEconItem *pEconItem)
{
	if(pEconItem)
	{
		if(pEconItem->GetDefinitionIndex() == 277 &&
		   pEconItem->GetOrigin() == kEconItemOrigin_Crafted) // Saxton mask is item index 277
		{
...
{
	V_memcpy(m_CachedServerLoadoutItems, m_LoadoutItems,
			 sizeof(itemid_t) * ARRAYSIZE(m_CachedServerLoadoutItems) * ARRAYSIZE(m_CachedServerLoadoutItems[0]));
}
#endif // CLIENT_DLL
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayerInventory::SOUpdated(const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject,
...
void CTFPlayerInventory::ValidateInventoryPositions(void)
{
	...
	for(int i = iCount - 1; i >= 0; i--)
	{
		...
		if(i < (iCount - 1))
		{
			// We're not in an invalid slot yet. But if we're in the same position as another item, we should be moved
			// too.
			int iPos1 = TFInventoryManager()->GetBackpackPositionFromBackend(iPosition);
			int iPos2 =
				TFInventoryManager()->GetBackpackPositionFromBackend(m_aInventoryItems[i + 1].GetInventoryPosition());
			if(iPos1 == iPos2)
			{
		...
		if(bInvalidSlot)
		{
			...
			if(pEconItemView->GetItemDefinition() && !pEconItemView->GetItemDefinition()->IsHidden() &&
			   !TFInventoryManager()->SetItemBackpackPosition(pEconItemView, 0, true))
			{
				...
				TFInventoryManager()->UpdateInventoryPosition(this, pEconItemView->GetItemID(),
		...
		{
			if(!pEconItemView->GetStaticData()->CanBeUsedByClass(j) && pEconItemView->IsEquippedForClass(j))
			{
				// Unequip this item from this class.
				InventoryManager()->UpdateInventoryEquippedState(this, INVALID_ITEM_ID, j,
																 pEconItemView->GetEquippedPositionForClass(j));
			}
		}
	}
...
//-----------------------------------------------------------------------------
void CTFPlayerInventory::SOCreated(const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject,
								   GCSDK::ESOCacheEvent eEvent)
{
...
void CTFPlayerInventory::DumpInventoryToConsole(bool bRoot)
{
	...
	for(int i = 0; i < iCount; i++)
	{
		Msg("      %s (ID %llu) at backpack slot %d\n", m_aInventoryItems[i].GetStaticData()->GetDefinitionName(),
			...
			TFInventoryManager()->GetBackpackPositionFromBackend(m_aInventoryItems[i].GetInventoryPosition()));
...
//-----------------------------------------------------------------------------
CEconItemView *CTFPlayerInventory::GetFirstItemOfItemDef(item_definition_index_t nDefIndex,
...
CEconItemView *CTFPlayerInventory::GetItemInLoadout(int iClass, int iSlot)
{
	...
	else
	{
		...
		if(m_LoadoutItems[iClass][iSlot] != LOADOUT_SLOT_USE_BASE_ITEM)
		{
			...
			if(pItem && AreSlotsConsideredIdentical(pItem->GetStaticData()->GetEquipType(),
													pItem->GetStaticData()->GetLoadoutSlot(iClass), iSlot))
				return pItem;
		}
	}
...
void CTFPlayerInventory::ItemHasBeenUpdated(CEconItemView *pItem, bool bUpdateAckFile, bool bWriteAckFile)
{
	...
	for(equipped_class_t iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_LAST_NORMAL_CLASS; iClass++)
	{
		...
		bool bThisClassLoadoutChanged = UpdateEquipStateForClass(pItem->GetItemID(), unSlot, m_LoadoutItems[iClass],
																 ARRAYSIZE(m_LoadoutItems[iClass]));
	...
	equipped_slot_t unSlot =
		pEconItem ? pEconItem->GetEquippedPositionForClass(GEconItemSchema().GetAccountIndex()) : INVALID_EQUIPPED_SLOT;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPlayerInventory::UpdateEquipStateForClass(const itemid_t &itemID, equipped_slot_t nSlot, itemid_t *pLoadout,
												  int nCount)
{
...
void CTFPlayerInventory::VerifyLoadoutItemsAreValid(int iClass)
{
	...
	for(int i = 0; i < CLASS_LOADOUT_POSITION_COUNT; i++)
	{
		...
		if(unItemEquipMask & unCumulativeRegionMask)
		{
			...
			InventoryManager()->UpdateInventoryEquippedState(this, INVALID_ITEM_ID, iClass,
															 pEquippedItemView->GetEquippedPositionForClass(iClass));
		}
		else
		{
...
CON_COMMAND_F(item_dumpinv_other,
			  "Dumps the contents of a specified client inventory. Format: item_dumpinv_other <player index>",
			  FCVAR_NONE)
{
	if(args.ArgC() > 1)
	{
...
#else
CON_COMMAND_F(item_dumpinv_sv, "Dumps the contents of a specified server inventory. Format: item_dumpinv_sv",
			  FCVAR_CHEAT)
#endif
{
#if defined(CLIENT_DLL)
...
#ifdef _DEBUG
#if defined(CLIENT_DLL)
CON_COMMAND_F(item_deleteunknowns,
			  "Deletes all items in your inventory that we don't have static data for. Useful for removing items that "
