...
#ifdef GAME_DLL
	SendPropEHandle(SENDINFO(m_hOwner)), SendPropInt(SENDINFO(m_iHealth), -1, SPROP_VARINT | SPROP_CHANGES_OFTEN),
		SendPropInt(SENDINFO(m_iMaxHealth), -1, SPROP_VARINT),
		SendPropInt(SENDINFO(m_nRevives), -1, SPROP_VARINT | SPROP_UNSIGNED),
#else
	RecvPropEHandle(RECVINFO(m_hOwner)), RecvPropInt(RECVINFO(m_iHealth)), RecvPropInt(RECVINFO(m_iMaxHealth)),
...
CTFReviveMarker *CTFReviveMarker::Create(CTFPlayer *pOwner)
{
	if(pOwner)
	{
		CTFReviveMarker *pMarker = static_cast<CTFReviveMarker *>(CBaseEntity::Create(
			"entity_revive_marker", pOwner->GetAbsOrigin() + Vector(0, 0, 50), pOwner->GetAbsAngles()));
		if(pMarker)
		{
...
bool CTFReviveMarker::ReviveOwner(void)
{
	...
	UTIL_TraceHull(vecTeleportPos, vecTeleportPos, VEC_HULL_MIN_SCALED(m_hOwner), VEC_HULL_MAX_SCALED(m_hOwner),
				   (MASK_SOLID | CONTENTS_PLAYERCLIP), &filter, &tr);
