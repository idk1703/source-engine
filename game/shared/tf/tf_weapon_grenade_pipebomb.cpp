...
ConVar tf_grenadelauncher_chargescale("tf_grenadelauncher_chargescale", "1.0",
									  FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY);
ConVar tf_grenadelauncher_livetime("tf_grenadelauncher_livetime", "0.8",
								   FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY);
...
#ifdef CLIENT_DLL
	RecvPropInt(RECVINFO(m_bTouched)), RecvPropInt(RECVINFO(m_iType)), RecvPropEHandle(RECVINFO(m_hLauncher)),
		RecvPropBool(RECVINFO(m_bDefensiveBomb)),
#else
	SendPropBool(SENDINFO(m_bTouched)), SendPropInt(SENDINFO(m_iType), 3), SendPropEHandle(SENDINFO(m_hLauncher)),
...
// Purpose:
//-----------------------------------------------------------------------------
CTFGrenadePipebombProjectile *CTFGrenadePipebombProjectile::Create(
	const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity,
	CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo, int iPipeBombType, float flMultDmg)
{
	...
	CTFGrenadePipebombProjectile *pGrenade =
		static_cast<CTFGrenadePipebombProjectile *>(CBaseEntity::CreateNoSpawn(pszBombClass, position, angles, pOwner));
	if(pGrenade)
	{
...
void CTFGrenadePipebombProjectile::Spawn()
{
	if(HasStickyEffects())
	{
		...
		SetContextThink(&CTFGrenadePipebombProjectile::PreArmThink, gpGlobals->curtime + 0.001f,
						"PRE_ARM_THINK"); // Next frame.
		...
		if(m_iType == TF_GL_MODE_CANNONBALL)
		{
			...
			if(pAttacker && (pOther->IsPlayer() || pOther->IsBaseObject()))
			{
				...
				if(!m_penetratedEntities.HasElement(pOther))
				{
					...
					CTakeDamageInfo info(this, pAttacker, m_hLauncher, vec3_origin, vOrigin, flImpactDamage,
										 GetDamageType(), TF_DMG_CUSTOM_CANNONBALL_PUSH);
...
void CTFGrenadePipebombProjectile::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	...
	if(m_iType == TF_GL_MODE_REGULAR || m_iType == TF_GL_MODE_CANNONBALL)
	{
		...
		// Blow up if we hit an enemy we can damage
		else if(pHitEntity->GetTeamNumber() && pHitEntity->GetTeamNumber() != GetTeamNumber() &&
	...
	if(FStrEq(pHitEntity->m_iParent.ToCStr(), "sawmovelinear01") ||
	   FStrEq(pHitEntity->m_iParent.ToCStr(), "sawmovelinear02") || PropDynamic_CollidesWithGrenades(pHitEntity))
	{
...
ConVar tf_grenade_force_sleeptime(
	"tf_grenade_force_sleeptime", "1.0",
	FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY); // How long after being shot will we re-stick to the world.
...
int CTFGrenadePipebombProjectile::OnTakeDamage(const CTakeDamageInfo &info)
{
	...
	{
		if(m_bTouched && HasStickyEffects() &&
		   (info.GetDamageType() & (DMG_BULLET | DMG_BUCKSHOT | DMG_BLAST | DMG_SONIC | DMG_MELEE)))
		{
...
void CTFGrenadePipebombProjectile::ArmThink(void)
{
	...
	if(IsCritical() && HasStickyEffects() && TFGameRules() &&
	   (TFGameRules()->InSetup() || TFGameRules()->State_Get() == GR_STATE_BETWEEN_RNDS))
	{
		CTFPlayer *pOwner = ToTFPlayer(GetThrower());
		if(pOwner && pOwner->m_Shared.InCond(TF_COND_CRITBOOSTED) &&
		   !pOwner->m_Shared.InCond(TF_COND_CRITBOOSTED_USER_BUFF))
		{
			// Find the medic(s)
			for(int i = 0; i < pOwner->m_Shared.GetNumHealers(); i++)
			{
				...
				if(pMedigun && pMedigun->IsReleasingCharge() &&
				   pMedigun->GetChargeType() == MEDIGUN_CHARGE_CRITICALBOOST)
				{
...
void CTFGrenadePipebombProjectile::Deflected(CBaseEntity *pDeflectedBy, Vector &vecDir)
{
	...
	if(HasStickyEffects())
	{
		...
		if(pWeapon)
		{
			flForceMultiplier = RemapValClamped((gpGlobals->curtime - pWeapon->GetChargeBeginTime()), 0.0f,
												pWeapon->GetChargeMaxTime(), 1.0f, 2.0f);
		}
...
float CTFGrenadePipebombProjectile::GetDamageScaleOnWorldContact()
{
	...
	if(GetLauncher())
	{
		CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(GetLauncher(), flGrenadeDamageScaleOnWorldContact,
...
float CTFGrenadePipebombProjectile::GetDamageRadius()
{
	...
	if(m_iType == TF_GL_MODE_REMOTE_DETONATE)
	{
		if(m_bTouched == false)
		{
			...
			flRadiusMod *= RemapValClamped(gpGlobals->curtime - m_flCreationTime, flArmTime,
										   flArmTime + tf_sticky_radius_ramp_time.GetFloat(),
										   tf_sticky_airdet_radius.GetFloat(), 1.0);
		}
	}
#endif // GAME_DLL
