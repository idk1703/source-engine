...
ConVar tf_fortune_teller_warning_time("tf_fortune_teller_warning_time", "2", FCVAR_CHEAT,
									  "Warning time (in second) before fortune teller tells a fortune.");
ConVar tf_fortune_teller_interval_time("tf_fortune_teller_interval_time", "120", FCVAR_CHEAT,
									   "Time until the next fortune teller event (in second).");
ConVar tf_fortune_teller_fortune_duration("tf_fortune_teller_fortune_duration", "30", FCVAR_CHEAT,
...
BEGIN_DATADESC(CTFMiniGame)
	...
	DEFINE_KEYFIELD(m_iszYourTeamScoreSound, FIELD_STRING, "your_team_score_sound"),
		...
		DEFINE_OUTPUT(m_OnRedHitMaxScore, "OnRedHitMaxScore"), DEFINE_OUTPUT(m_OnBlueHitMaxScore, "OnBlueHitMaxScore"),
		DEFINE_OUTPUT(m_OnTeleportToMinigame, "OnTeleportToMinigame"),
		DEFINE_OUTPUT(m_OnReturnFromMinigame, "OnReturnFromMinigame"), DEFINE_OUTPUT(m_OnAllRedDead, "OnAllRedDead"),
		DEFINE_OUTPUT(m_OnAllBlueDead, "OnAllBlueDead"), DEFINE_OUTPUT(m_OnSuddenDeathStart, "OnSuddenDeathStart"),
...
BEGIN_NETWORK_TABLE_NOBASE(CTFMiniGame, DT_TFMinigame)
	...
	RecvPropArray3(RECVINFO_ARRAY(m_nMinigameTeamScore), RecvPropInt(RECVINFO(m_nMinigameTeamScore[0]))),
		RecvPropInt(RECVINFO(m_nMaxScoreForMiniGame)), RecvPropString(RECVINFO(m_pszHudResFile)),
		RecvPropInt(RECVINFO(m_eScoringType)),
#else
	SendPropArray3(SENDINFO_ARRAY3(m_nMinigameTeamScore),
				   SendPropInt(SENDINFO_ARRAY(m_nMinigameTeamScore), -1, SPROP_UNSIGNED | SPROP_VARINT)),
...
void CTFMiniGame::FireGameEvent(IGameEvent *pEvent)
{
	...
	if(!V_strcmp(pszEventName, "player_turned_to_ghost") || !V_strcmp(pszEventName, "player_disconnect") ||
	   !V_strcmp(pszEventName, "player_team") || !V_strcmp(pszEventName, "player_death") ||
...
void CTFMiniGame::TeleportAllPlayers()
{
	...
	TFGameRules()->TeleportPlayersToTargetEntities(TF_TEAM_RED, m_pszTeamSpawnPoint[TF_TEAM_RED],
												   &vecTeleportedPlayers);
	TFGameRules()->TeleportPlayersToTargetEntities(TF_TEAM_BLUE, m_pszTeamSpawnPoint[TF_TEAM_BLUE],
												   &vecTeleportedPlayers);
	...
	{
		SetContextThink(&CTFHalloweenMinigame::SuddenDeathTimeStartThink, gpGlobals->curtime + m_flSuddenDeathTime,
...
void CTFMiniGame::ScorePointsForTeam(int nTeamNum, int nPoints)
{
	...
	if(!m_bIsActive || (m_nMinigameTeamScore.Get(TF_TEAM_RED) == m_nMaxScoreForMiniGame) ||
	   (m_nMinigameTeamScore.Get(TF_TEAM_BLUE) == m_nMaxScoreForMiniGame))
	{
		return;
	}
	...
	if(m_iszYourTeamScoreSound.ToCStr() && *m_iszYourTeamScoreSound.ToCStr() && m_iszEnemyTeamScoreSound.ToCStr() &&
	   *m_iszEnemyTeamScoreSound.ToCStr())
	{
		// Get everyone
		...
		for(CTFPlayer *pPlayer : vecPlayer)
		{
			...
			params.m_pSoundName = pPlayer->GetTeamNumber() == nTeamNum ? m_iszYourTeamScoreSound.ToCStr()
																	   : m_iszEnemyTeamScoreSound.ToCStr();
			params.m_nPitch =
				RemapValClamped(nTeamPoints, m_nMaxScoreForMiniGame * 0.75, m_nMaxScoreForMiniGame, 100, 120);
...
void CTFHalloweenMinigame::FireGameEvent(IGameEvent *event)
{
	...
	if(FStrEq(event->GetName(), "pumpkin_lord_killed"))
	{
		...
		{
			m_hHalloweenBoss =
				CHalloweenBaseBoss::SpawnBossAtPos(HALLOWEEN_BOSS_HHH, m_hBossSpawnPoint->GetAbsOrigin());
		}
	}
}
...
static CConditionFortuneTellerEffect g_FortuneTellerEffect_BalloonHead = {
	"Announcer.SD_Event_BigHeadCurse", TF_COND_BALLOON_HEAD}; // FIXME: need 2014 sound link
static CConditionFortuneTellerEffect g_FortuneTellerEffect_MeleeOnly = {"Announcer.SD_Event_NoGunsCurse",
																		TF_COND_MELEE_ONLY};
static CConditionFortuneTellerEffect g_FortuneTellerEffect_SwimmingCurse = {
	"Announcer.SD_Event_SwimmingCurse", TF_COND_SWIMMING_CURSE}; // FIXME: need 2014 sound link
...
BEGIN_DATADESC(CTFHalloweenFortuneTeller)
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "EnableFortuneTelling", InputEnableFortuneTelling),
		...
		DEFINE_OUTPUT(m_OnFortuneWarning, "OnFortuneWarning"), DEFINE_OUTPUT(m_OnFortuneTold, "OnFortuneTold"),
		DEFINE_OUTPUT(m_OnFortuneCurse, "OnFortuneCurse"), DEFINE_OUTPUT(m_OnFortuneEnd, "OnFortuneEnd"),
...
void CTFHalloweenFortuneTeller::UpdateFortuneTellerTime()
{
	...
	if(flTimePast < flWarningTime)
	{
		...
		SetContextThink(&CTFHalloweenFortuneTeller::StartFortuneWarning, gpGlobals->curtime + flTimeBeforeEvent,
						"StartFortuneWarning");
	}
	else
	{
		float flTimeBeforeEvent = tf_fortune_teller_interval_time.GetFloat() - flTimePast;
		SetContextThink(&CTFHalloweenFortuneTeller::StartFortuneTell, gpGlobals->curtime + flTimeBeforeEvent,
						"StartFortuneTell");
	}
}
...
void CTFHalloweenFortuneTeller::StartFortuneWarning()
{
	...
	SetContextThink(&CTFHalloweenFortuneTeller::StartFortuneTell,
					gpGlobals->curtime + tf_fortune_teller_warning_time.GetFloat(), "StartFortuneTell");
}
...
void CTFHalloweenFortuneTeller::TellFortune()
{
	...
	SetContextThink(&CTFHalloweenFortuneTeller::DanceThink, gpGlobals->curtime + flDanceTime + flDanceDuration,
					"DanceThink2");
}
...
void CTFHalloweenFortuneTeller::ApplyFortuneEffect()
{
	...
	if(m_bUseTimer)
	{
		SetContextThink(&CTFHalloweenFortuneTeller::EndFortuneTell,
						gpGlobals->curtime + tf_fortune_teller_fortune_duration.GetFloat(), "EndFortuneTell");
	}
}
...
void CTFHalloweenFortuneTeller::SpeakThink()
{
	...
	SetContextThink(&CTFHalloweenFortuneTeller::StopTalkingAnim, gpGlobals->curtime + flSoundDuration,
					"StopTalkingAnim");
}
