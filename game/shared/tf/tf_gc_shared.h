...
template<typename RELIABLE_MSG_CLASS, typename MSG_TYPE, ETFGCMsg E_MSG_TYPE, typename REPLY_TYPE,
		 ETFGCMsg E_REPLY_TYPE>
...
public:
	...
	CJobReliableMessageBase() : GCSDK::CGCClientJob(GCClientSystem()->GetGCClient()), m_msg(E_MSG_TYPE), m_msgReply() {}
...
private:
	...
	{
		return static_cast<RELIABLE_MSG_CLASS *>(this)->MsgName();
	}
	...
	!defined(OSX) // (Don't have time to figure out what criteria the OS X toolchain has to not blow this)
		static_assert(std::is_base_of<decltype(*this), RELIABLE_MSG_CLASS>::value,
					  "RELIABLE_MSG_CLASS Must be an override of this base");
		static_assert(
			!std::is_same<decltype(&(decltype(*this)::InitDebugString)),
						  decltype(&RELIABLE_MSG_CLASS::InitDebugString)>::value &&
				!std::is_same<decltype(&(decltype(*this)::MsgName)), decltype(&RELIABLE_MSG_CLASS::MsgName)>::value,
