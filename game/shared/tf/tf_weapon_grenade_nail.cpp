...
BEGIN_DATADESC(CTFGrenadeNailProjectile)
	DEFINE_THINKFUNC(EmitNails), DEFINE_EMBEDDED(m_GrenadeController), DEFINE_PHYSPTR(m_pMotionController),
END_DATADESC()
														   ...
														   const CTFWeaponInfo &weaponInfo, float timer, int iFlags)
{
	...
	CTFGrenadeNailProjectile *pGrenade = static_cast<CTFGrenadeNailProjectile *>(
		CTFWeaponBaseGrenadeProj::Create("tf_weapon_grenade_nail_projectile", position, vecCustomAngles, velocity,
										 vecCustomAngVelocity, pOwner, weaponInfo, timer, iFlags));
	return pGrenade;
}
...
IMotionEvent::simresult_e CNailGrenadeController::Simulate(IPhysicsMotionController *pController,
														   ...
														   AngularImpulse &angular)
{
	...
	else
	{
		...
		angular = ComputeRotSpeedToAlignAxes(m_worldGoalAxis, currentLocalTargetAxis, angVel, 1.0,
											 invDeltaTime * invDeltaTime, m_angularLimit * invDeltaTime);
	}
