...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_TopScoreboard, ACHIEVEMENT_TF_MEDIC_TOP_SCOREBOARD, "TF_MEDIC_TOP_SCOREBOARD",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_ChargeBySetupEnd, ACHIEVEMENT_TF_MEDIC_SETUP_CHARGE, "TF_MEDIC_SETUP_CHARGE",
					1);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_HealTargetUnderFire, ACHIEVEMENT_TF_MEDIC_HEAL_UNDER_FIRE,
					"TF_MEDIC_HEAL_UNDER_FIRE", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_SimultaneousCharges, ACHIEVEMENT_TF_MEDIC_SIMUL_CHARGE, "TF_MEDIC_SIMUL_CHARGE",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_RapidUbercharges, ACHIEVEMENT_TF_MEDIC_RAPID_CHARGE, "TF_MEDIC_RAPID_CHARGE",
					1);
...
class CAchievementTFMedic_CounterUbercharges : public CBaseTFAchievement
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		if(Q_strcmp(event->GetName(), "player_chargedeployed") == 0)
		{
			...
			{
				// If we're the medic, get the achievement if an enemy medic in our PVS has deployed in the last 8
				// seconds
				if(pMedic == C_TFPlayer::GetLocalTFPlayer())
				{
					if(gpGlobals->curtime - m_flPreviousEnemyCharge < 8.0 && m_hEnemyMedic &&
					   !m_hEnemyMedic->IsDormant())
					{
						IncrementCount();
					}
				}
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_CounterUbercharges, ACHIEVEMENT_TF_MEDIC_COUNTER_CHARGE,
					"TF_MEDIC_COUNTER_CHARGE", 1);
...
class CAchievementTFMedic_SwitchToMedic : public CBaseTFAchievementSimple
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		else if(Q_strcmp(event->GetName(), "localplayer_changeclass") == 0 ||
				Q_strcmp(event->GetName(), "localplayer_becameobserver") == 0)
		{
			// See if we've switched to Medic right after someone called for medic.
			// We trigger off the localplayer_becameobserver as well, to catch the case where they've suicided to change
			// class, but don't respawn within the 10 second window (because we only get localplayer_changeclass on the
			// respawn as the new class).
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_SwitchToMedic, ACHIEVEMENT_TF_MEDIC_SWITCH_TO_MEDIC, "TF_MEDIC_SWITCH_TO_MEDIC",
					1);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_ChargeBlocker, ACHIEVEMENT_TF_MEDIC_CHARGE_BLOCKER, "TF_MEDIC_CHARGE_BLOCKER",
					5);
									...
									IGameEvent *event)
	{
		...
		if(pLocalPlayer)
		{
			...
			if(iAssisterIndex > 0)
			{
				...
				{
					if(pAssister && pAssister->IsPlayerClass(TF_CLASS_MEDIC) && pAtkPlayer &&
					   pAtkPlayer->IsPlayerClass(TF_CLASS_MEDIC))
					{
						m_iAssists++;
									...
									IGameEvent *event)
	{
		...
		if(pAttacker == pLocalPlayer)
		{
			...
			if(pTFVictim && pTFVictim->IsPlayerClass(TF_CLASS_SCOUT) &&
			   event->GetInt("weaponid") == TF_WEAPON_SYRINGEGUN_MEDIC)
			{
				IncrementCount();
			}
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_KillScoutsWithSyringe, ACHIEVEMENT_TF_MEDIC_SYRINGE_SCOUTS,
					"TF_MEDIC_SYRINGE_SCOUTS", 1);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_KillMedicsWithBonesaw, ACHIEVEMENT_TF_MEDIC_BONESAW_MEDICS,
					"TF_MEDIC_BONESAW_MEDICS", 1);
									...
									IGameEvent *event)
	{
		...
		if(pLocalPlayer)
		{
					...
					if(UTIL_PlayerByIndex(iAssisterIndex) == pLocalPlayer)
					{
						...
						if(pAtkPlayer && pAtkPlayer->IsPlayerClass(TF_CLASS_HEAVYWEAPONS))
						{
							...
							if(index == -1)
							{
								if(m_Partners.Count() >= MAX_PARTNERS)
								{
									...
									if(iLowestIndex >= 0)
									{
										...
										// g_PR->GetPlayerName(m_Partners[iLowestIndex].hPartner->entindex()) );
							...
							else
							{
								...
								// g_PR->GetPlayerName(m_Partners[index].hPartner->entindex()), index,
								// m_Partners[index].iAssists );
			...
			for ( int i = 0; i < m_Partners.Count(); i++ )
			{
				if ( m_Partners[i].hPartner )
				{
					Msg("   %d: %s with %d\n", i, g_PR->GetPlayerName(m_Partners[i].hPartner->entindex()),
			m_Partners[i].iAssists );
				}
				else
				{
					Msg("   %d: EMPTY\n", i );
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AssistHeavyLongStreak, ACHIEVEMENT_TF_MEDIC_ASSIST_HEAVY_LONG,
					"TF_MEDIC_ASSIST_HEAVY_LONG", 1);
...
public:
	...
	virtual void AssistedWhileCharged(CBaseEntity *pVictim, CBaseEntity *pAttacker, CBaseEntity *pInflictor,
									  IGameEvent *event) = 0;
};
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AssistChargeScout, ACHIEVEMENT_TF_MEDIC_ASSIST_SCOUT, "TF_MEDIC_ASSIST_SCOUT",
					1);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AssistChargeSoldier, ACHIEVEMENT_TF_MEDIC_ASSIST_SOLDIER,
					"TF_MEDIC_ASSIST_SOLDIER", 1);
			...
			if(iAssisterIndex > 0)
			{
				...
				if(pAssister == pTFPlayer && pTFPlayer && pTFPlayer->MedicIsReleasingCharge())
				{
					CBaseEntity *pAttacker =
						ClientEntityList().GetEnt(engine->GetPlayerForUserID(event->GetInt("attacker")));
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AssistChargeDemoman, ACHIEVEMENT_TF_MEDIC_ASSIST_DEMOMAN,
					"TF_MEDIC_ASSIST_DEMOMAN", 1);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AssistCapturer, ACHIEVEMENT_TF_MEDIC_ASSIST_CAPTURER,
					"TF_MEDIC_ASSIST_CAPTURER", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_ExtinguishTeammates, ACHIEVEMENT_TF_MEDIC_EXTINGUISH_TEAMMATES,
					"TF_MEDIC_EXTINGUISH_TEAMMATES", 5);
									...
									IGameEvent *event)
	{
		...
		if(iAssisterID > 0 && iAssisterID == pLocalPlayer->GetUserID())
		{
			if(event->GetInt("death_flags") & TF_DEATH_REVENGE ||
			   event->GetInt("death_flags") & TF_DEATH_ASSISTER_REVENGE)
			{
				IncrementCount();
			}
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AssistVsNemeses, ACHIEVEMENT_TF_MEDIC_ASSIST_VS_NEMESES,
					"TF_MEDIC_ASSIST_VS_NEMESES", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_KillWhileCharged, ACHIEVEMENT_TF_MEDIC_KILL_WHILE_CHARGED,
					"TF_MEDIC_KILL_WHILE_CHARGED", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_BonesawNoMisses, ACHIEVEMENT_TF_MEDIC_BONESAW_NOMISSES,
					"TF_MEDIC_BONESAW_NOMISSES", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_KillHealedSpy, ACHIEVEMENT_TF_MEDIC_KILL_HEALED_SPY, "TF_MEDIC_KILL_HEALED_SPY",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_SaveFallingTeammate, ACHIEVEMENT_TF_MEDIC_SAVE_FALLING_TEAMMATE,
					"TF_MEDIC_SAVE_FALLING_TEAMMATE", 5);
...
class CAchievementTFMedic_ChargeJuggle : public CBaseTFAchievement
{
	...
	void EvaluateInvulnJuggle(void)
	{
		...
		if(iTargetIndex > 0)
		{
			...
			if(pTarget)
			{
				if(pTarget->m_Shared.InCond(TF_COND_INVULNERABLE_WEARINGOFF) ||
				   pTarget->m_Shared.InCond(TF_COND_INVULNERABLE))
				{
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_FreezecamRagdoll, ACHIEVEMENT_TF_MEDIC_FREEZECAM_RAGDOLL,
					"TF_MEDIC_FREEZECAM_RAGDOLL", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_BonesawSpyCallers, ACHIEVEMENT_TF_MEDIC_BONESAW_SPY_CALLERS,
					"TF_MEDIC_BONESAW_SPY_CALLERS", 5);
			...
			if(event->GetInt("medic_userid") == pLocalPlayer->GetUserID() && iTargetIndex > 0)
			{
				...
				if(steamapicontext->SteamFriends()->HasFriend(steamID, k_EFriendFlagImmediate))
				{
					...
					if(!bFound)
					{
						...
						for(int i = 0; i < vecChargedFriends.Count(); i++)
						{
							Q_strncat(buf, VarArgs(":%d", (uint32)atoi(vecChargedFriends[i])), sizeof(buf),
									  COPY_ALL_CHARACTERS);
						}
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_ChargeFriends, ACHIEVEMENT_TF_MEDIC_CHARGE_FRIENDS, "TF_MEDIC_CHARGE_FRIENDS",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_InviteJoinCharge, ACHIEVEMENT_TF_MEDIC_INVITE_JOIN_CHARGE,
					"TF_MEDIC_INVITE_JOIN_CHARGE", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AchieveProgress1, ACHIEVEMENT_TF_MEDIC_ACHIEVE_PROGRESS1,
					"TF_MEDIC_ACHIEVE_PROGRESS1", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AchieveProgress2, ACHIEVEMENT_TF_MEDIC_ACHIEVE_PROGRESS2,
					"TF_MEDIC_ACHIEVE_PROGRESS2", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFMedic_AchieveProgress3, ACHIEVEMENT_TF_MEDIC_ACHIEVE_PROGRESS3,
					"TF_MEDIC_ACHIEVE_PROGRESS3", 5);
