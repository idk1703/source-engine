...
//			catapult trigger and the catapult target
//-----------------------------------------------------------------------------
Vector CTriggerCatapult::CalculateLaunchVectorPreserve(Vector vecInitialVelocity, CBaseEntity *pVictim,
													   CBaseEntity *pTarget, bool bForcePlayer)
{
#if defined(CLIENT_DLL)
	if(!GetPredictable() || !pVictim->GetPredictable())
		return vec3_origin;
	...
	float flRadical = flVelocity * flVelocity * flVelocity * flVelocity -
...
void CTriggerCatapult::LaunchByTarget(CBaseEntity *pVictim, CBaseEntity *pTarget)
{
	...
	Vector vecVelocity = m_bUseExactVelocity ? CalculateLaunchVectorPreserve(vecVictim, pVictim, pTarget)
	...
	if(pVictim->IsPlayer())
	{
		...
		if(pPlayer)
		{
			...
			// pPlayer->SetAirControlSupressionTime( flSupressionTimeInSeconds * 1000.0f ); // fix units, this method
...
void CTriggerCatapult::LaunchByDirection(CBaseEntity *pVictim)
{
	...
	if(pVictim->IsPlayer())
	{
		...
		if(m_bDirectionSuppressAirControl)
		{
			...
			// pTFPlayer->SetAirControlSupressionTime( flSupressionTimeInSeconds * 1000.0f ); // fix units, this method
			// expects milliseconds
		}
...
void CTriggerCatapult::StartTouch(CBaseEntity *pOther)
{
	...
	if(!GetPredictable() || !pOther->GetPredictable())
		...
		// Warning( "CTriggerCatapult::StartTouch( %i %s %f )\n", entindex(), gpGlobals->IsClient() ? "client" :
		// "server", gpGlobals->curtime );
	...
	{
		Warning("CTriggerCatapult::StartTouch Trying to store a refire index for an entity( %d ) outside the expected "
				"range ( < %d ).\n",
	...
	if(pLaunchTarget)
	{
		...
		if(m_bUseThresholdCheck)
		{
			...
			{
				//				DevMsg("Catapult fail!!  Object is not a player and has no physics object!  BUG
				//THIS\n");
			...
			if(flDot >= m_flEntryAngleTolerance)
			{
				...
				if(((flLaunchSpeed - (flLaunchSpeed * m_flLowerThreshold)) < flVictimSpeed) &&
				   ((flLaunchSpeed + (flLaunchSpeed * m_flUpperThreshold)) > flVictimSpeed))
				{
					...
					else
					{
						...
						//						DevMsg( 1, "Catapult \"%s\" is adjusting velocity of \"%s\" so it will hit the
						//target. (Object Velocity: %.1f -- Object needed to be between %.1f and %.1f \n",
						//STRING(GetEntityName()), pOther->GetClassname(), flVictimSpeed, flLaunchSpeed - (flLaunchSpeed
						//* m_flLowerThreshold ), flLaunchSpeed + (flLaunchSpeed * m_flUpperThreshold ) );
					}
				}
				else
				{
//					DevMsg( 1, "Catapult \"%s\" ignoring object \"%s\" because its velocity is outside of the threshold. (Object
//Velocity: %.1f -- Object needed to be between %.1f and %.1f \n", STRING(GetEntityName()), pOther->GetClassname(),
//flVictimSpeed, flLaunchSpeed - (flLaunchSpeed * m_flLowerThreshold ), flLaunchSpeed + (flLaunchSpeed *
//m_flUpperThreshold ) );
	...
	else
	{
		...
		if(m_hLaunchTarget.IsValid())
		{
			Warning(
				...
				"Catapult launch target not networked to client! This will make prediction fail! Fix this in the map.\n"
				"Catapult launch target not networked to client! This will make prediction fail! Fix this in the map.\n"
				"Catapult launch target not networked to client! This will make prediction fail! Fix this in the "
				"map.\n");
		}
#endif
