...
float CTFThrowable::GetChargeBeginTime(void)
{
	...
	if(iCanBePrimed)
	{
		...
		return gpGlobals->curtime -
			   Clamp(m_flChargeBeginTime + flDetonateTimeLength - gpGlobals->curtime, 0.f, flDetonateTimeLength);
	}
...
CTFProjectile_Throwable *CTFThrowable::FireProjectileInternal(void)
{
	...
	CTFProjectile_Throwable *pGrenade = static_cast<CTFProjectile_Throwable *>(CBaseEntity::CreateNoSpawn(
		attrProjectileEntityName.value().c_str(), trace.endpos, pPlayer->EyeAngles(), pPlayer));
	if(pGrenade)
	{
		...
		{
			flChargePercent =
				RemapVal(gpGlobals->curtime, m_flChargeBeginTime, m_flChargeBeginTime + flDetonateTime, 0.0f, 1.0f);
		}
...
Vector CTFProjectile_Throwable::GetVelocityVector(const Vector &vecForward, const Vector &vecRight, const Vector &vecUp,
												  float flCharge)
{
	...
	return ((flSpeed * vecForward) + ((random->RandomFloat(-10.0f, 10.0f) + 200.0f) * vecUp) +
...
void CTFProjectile_ThrowableBreadMonster::OnHit(CBaseEntity *pOther)
{
	...
	if(pVictim && pOwner && !pVictim->InSameTeam(pOwner))
	{
		...
		UTIL_TraceLine(WorldSpaceCenter(), vEndPos, CONTENTS_HITBOX | CONTENTS_MONSTER | CONTENTS_SOLID, &tracefilter,
					   &trace);
...
//		}
//
//		attachedparticlesystem_t *pParticleSystem = GetItemSchema()->GetAttributeControlledParticleSystem(
//iDynamicParticleEffect ); 		if ( pParticleSystem )
//		{
...
// THROWABLE KNIFE
//-----------------------------------------------------------------------------
Vector CTFProjectile_ThrowingKnife::GetVelocityVector(const Vector &vecForward, const Vector &vecRight,
													  const Vector &vecUp, float flCharge)
{
	// Scale the projectile speed up to a maximum of 3000?
	float flSpeed = RemapVal(flCharge, 0, 1.0f, GetProjectileSpeed(), GetProjectileMaxSpeed());

	return ((flSpeed * vecForward) + (flSpeed * 0.1 * vecUp));
}
//----------------------------------------------------------------------------------------------------------------------------------------------------------
