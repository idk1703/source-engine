...
static ConVar tf_demoman_charge_frametime_scaling(
	"tf_demoman_charge_frametime_scaling", "1", FCVAR_REPLICATED | FCVAR_CHEAT,
...
ConVar tf_spy_invis_time("tf_spy_invis_time", "1.0", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
						 "Transition time in and out of spy invisibility", true, 0.1, true, 5.0);
ConVar tf_spy_invis_unstealth_time("tf_spy_invis_unstealth_time", "2.0", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
								   "Transition time in and out of spy invisibility", true, 0.1, true, 5.0);
...
ConVar tf_max_health_boost("tf_max_health_boost", "1.5", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
						   "Max health factor that players can be boosted to by healers.", true, 1.0, false, 0);
ConVar tf_invuln_time("tf_invuln_time", "1.0", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
...
ConVar tf_mvm_bot_flag_carrier_movement_penalty("tf_mvm_bot_flag_carrier_movement_penalty", "0.5",
												FCVAR_REPLICATED | FCVAR_CHEAT);

// ConVar tf_scout_dodge_move_penalty_duration( "tf_scout_dodge_move_penalty_duration", "3.0", FCVAR_DEVELOPMENTONLY |
// FCVAR_REPLICATED ); ConVar tf_scout_dodge_move_penalty( "tf_scout_dodge_move_penalty", "0.5", FCVAR_DEVELOPMENTONLY |
// FCVAR_REPLICATED );

#ifdef GAME_DLL
ConVar tf_boost_drain_time("tf_boost_drain_time", "15.0", FCVAR_DEVELOPMENTONLY,
						   "Time is takes for a full health boost to drain away from a player.", true, 0.1, false, 0);
#ifdef _DEBUG
...
ConVar tf_allow_sliding_taunt("tf_allow_sliding_taunt", "0", FCVAR_NONE,
							  "1 - Allow player to slide for a bit after taunting");
...
ConVar tf_useparticletracers("tf_useparticletracers", "1", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
							 "Use particle tracers instead of old style ones.");
ConVar tf_spy_cloak_consume_rate("tf_spy_cloak_consume_rate", "10.0", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
								 "cloak to use per second while cloaked, from 100 max )"); // 10 seconds of invis
ConVar tf_spy_cloak_regen_rate("tf_spy_cloak_regen_rate", "3.3", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
							   "cloak to regen per second, up to 100 max"); // 30 seconds to full charge
ConVar tf_spy_cloak_no_attack_time("tf_spy_cloak_no_attack_time", "2.0", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
								   "time after uncloaking that the spy is prohibited from attacking");
ConVar tf_tournament_hide_domination_icons("tf_tournament_hide_domination_icons", "0", FCVAR_REPLICATED,
										   "Tournament mode server convar that forces clients to not display the "
										   "domination icons above players dominating them.");
ConVar tf_damage_disablespread("tf_damage_disablespread", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
							   "Toggles the random damage spread applied to all player damage.");

ConVar tf_scout_energydrink_regen_rate("tf_scout_energydrink_regen_rate", "3.3",
									   FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
									   "energy drink regen per second, up to 100 max");
ConVar tf_scout_energydrink_consume_rate("tf_scout_energydrink_consume_rate", "12.5",
										 FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
										 "energy drink to use per second while boosted, from 100 max");
ConVar tf_scout_energydrink_activation("tf_scout_energydrink_activation", "0.0",
									   FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
...
// STAGING_SPY
ConVar tf_feign_death_duration("tf_feign_death_duration", "3.0", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT,
							   "Time that feign death buffs last.");
ConVar tf_feign_death_speed_duration("tf_feign_death_speed_duration", "3.0",
									 FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT,
									 "Time that feign death speed boost last.");

ConVar tf_allow_taunt_switch(
	"tf_allow_taunt_switch", "0", FCVAR_REPLICATED,
	"0 - players are not allowed to switch weapons while taunting, 1 - players can switch weapons at the start of a "
	"taunt (old bug behavior), 2 - players can switch weapons at any time during a taunt.");

ConVar tf_allow_all_team_partner_taunt("tf_allow_all_team_partner_taunt", "1",
...
ConVar tf_killstreak_eyeglow(
	"tf_killstreak_eyeglow", "0", FCVAR_REPLICATED,
...
#ifdef CLIENT_DLL
ConVar tf_colorblindassist("tf_colorblindassist", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
						   "Setting this to 1 turns on colorblind mode.");
...
// ConVar tf_spy_stealth_blink_time( "tf_spy_stealth_blink_time", "0.3", FCVAR_DEVELOPMENTONLY, "time after being hit
// the spy blinks into view" ); ConVar tf_spy_stealth_blink_scale( "tf_spy_stealth_blink_scale", "0.85",
// FCVAR_DEVELOPMENTONLY, "percentage visible scalar after being hit the spy blinks into view" );
...
const char *g_pszBDayGibs[22] = {"models/effects/bday_gib01.mdl",		"models/effects/bday_gib02.mdl",
								 "models/effects/bday_gib03.mdl",		"models/effects/bday_gib04.mdl",
								 "models/player/gibs/gibs_balloon.mdl", "models/player/gibs/gibs_burger.mdl",
								 "models/player/gibs/gibs_boot.mdl",	"models/player/gibs/gibs_bolt.mdl",
								 "models/player/gibs/gibs_can.mdl",		"models/player/gibs/gibs_clock.mdl",
								 "models/player/gibs/gibs_fish.mdl",	"models/player/gibs/gibs_gear1.mdl",
								 "models/player/gibs/gibs_gear2.mdl",	"models/player/gibs/gibs_gear3.mdl",
								 "models/player/gibs/gibs_gear4.mdl",	"models/player/gibs/gibs_gear5.mdl",
								 "models/player/gibs/gibs_hubcap.mdl",	"models/player/gibs/gibs_licenseplate.mdl",
								 "models/player/gibs/gibs_spring1.mdl", "models/player/gibs/gibs_spring2.mdl",
								 "models/player/gibs/gibs_teeth.mdl",	"models/player/gibs/gibs_tire.mdl"};

ETFCond g_SoldierBuffAttributeIDToConditionMap[kSoldierBuffCount + 1] = {
	TF_COND_LAST, // dummy entry to deal with attribute value of "1" being the lowest value we store in the attribute
				  // itself
	TF_COND_OFFENSEBUFF,
	TF_COND_DEFENSEBUFF,
...
BEGIN_RECV_TABLE_NOBASE(localplayerscoring_t, DT_TFPlayerScoringDataExclusive)
	RecvPropInt(RECVINFO(m_iCaptures)), RecvPropInt(RECVINFO(m_iDefenses)), RecvPropInt(RECVINFO(m_iKills)),
		RecvPropInt(RECVINFO(m_iDeaths)), RecvPropInt(RECVINFO(m_iSuicides)), RecvPropInt(RECVINFO(m_iDominations)),
		RecvPropInt(RECVINFO(m_iRevenge)), RecvPropInt(RECVINFO(m_iBuildingsBuilt)),
		RecvPropInt(RECVINFO(m_iBuildingsDestroyed)), RecvPropInt(RECVINFO(m_iHeadshots)),
		RecvPropInt(RECVINFO(m_iBackstabs)), RecvPropInt(RECVINFO(m_iHealPoints)), RecvPropInt(RECVINFO(m_iInvulns)),
		RecvPropInt(RECVINFO(m_iTeleports)), RecvPropInt(RECVINFO(m_iResupplyPoints)),
		RecvPropInt(RECVINFO(m_iKillAssists)), RecvPropInt(RECVINFO(m_iPoints)), RecvPropInt(RECVINFO(m_iBonusPoints)),
		RecvPropInt(RECVINFO(m_iDamageDone)), RecvPropInt(RECVINFO(m_iCrits)),
END_RECV_TABLE()

EXTERN_RECV_TABLE(DT_TFPlayerConditionListExclusive);

BEGIN_RECV_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerSharedLocal)
	RecvPropInt(RECVINFO(m_nDesiredDisguiseTeam)), RecvPropInt(RECVINFO(m_nDesiredDisguiseClass)),
		RecvPropTime(RECVINFO(m_flStealthNoAttackExpire)), RecvPropTime(RECVINFO(m_flStealthNextChangeTime)),
		RecvPropBool(RECVINFO(m_bLastDisguisedAsOwnTeam)), RecvPropFloat(RECVINFO(m_flRageMeter)),
		RecvPropBool(RECVINFO(m_bRageDraining)), RecvPropTime(RECVINFO(m_flNextRageEarnTime)),
...
BEGIN_RECV_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerShared)
	RecvPropInt(RECVINFO(m_nPlayerCond)), RecvPropInt(RECVINFO(m_bJumping)), RecvPropInt(RECVINFO(m_nNumHealers)),
		RecvPropInt(RECVINFO(m_iCritMult)), RecvPropInt(RECVINFO(m_iAirDash)), RecvPropInt(RECVINFO(m_nAirDucked)),
		RecvPropFloat(RECVINFO(m_flDuckTimer)), RecvPropInt(RECVINFO(m_nPlayerState)),
		RecvPropInt(RECVINFO(m_iDesiredPlayerClass)), RecvPropFloat(RECVINFO(m_flMovementStunTime)),
		RecvPropInt(RECVINFO(m_iMovementStunAmount)), RecvPropInt(RECVINFO(m_iMovementStunParity)),
		RecvPropEHandle(RECVINFO(m_hStunner)), RecvPropInt(RECVINFO(m_iStunFlags)),
		RecvPropInt(RECVINFO(m_nArenaNumChanges)), RecvPropBool(RECVINFO(m_bArenaFirstBloodBoost)),
		RecvPropInt(RECVINFO(m_iWeaponKnockbackID)), RecvPropBool(RECVINFO(m_bLoadoutUnavailable)),
		RecvPropInt(RECVINFO(m_iItemFindBonus)), RecvPropBool(RECVINFO(m_bShieldEquipped)),
		RecvPropBool(RECVINFO(m_bParachuteEquipped)), RecvPropInt(RECVINFO(m_iNextMeleeCrit)),
		RecvPropInt(RECVINFO(m_iDecapitations)), RecvPropInt(RECVINFO(m_iRevengeCrits)),
		RecvPropInt(RECVINFO(m_iDisguiseBody)), RecvPropEHandle(RECVINFO(m_hCarriedObject)),
		RecvPropBool(RECVINFO(m_bCarryingObject)), RecvPropFloat(RECVINFO(m_flNextNoiseMakerTime)),
		RecvPropInt(RECVINFO(m_iSpawnRoomTouchCount)), RecvPropInt(RECVINFO(m_iKillCountSinceLastDeploy)),
		RecvPropFloat(RECVINFO(m_flFirstPrimaryAttack)),

		// Scout
		RecvPropFloat(RECVINFO(m_flEnergyDrinkMeter)), RecvPropFloat(RECVINFO(m_flHypeMeter)),

		// Demoman
		RecvPropFloat(RECVINFO(m_flChargeMeter)),

		// Spy.
		RecvPropTime(RECVINFO(m_flInvisChangeCompleteTime)), RecvPropInt(RECVINFO(m_nDisguiseTeam)),
		RecvPropInt(RECVINFO(m_nDisguiseClass)), RecvPropInt(RECVINFO(m_nDisguiseSkinOverride)),
		...
		RecvPropEHandle(RECVINFO(m_hDisguiseWeapon)), RecvPropInt(RECVINFO(m_nTeamTeleporterUsed)),
		RecvPropFloat(RECVINFO(m_flCloakMeter)), RecvPropFloat(RECVINFO(m_flSpyTranqBuffDuration)),
		...
		RecvPropInt(RECVINFO(m_iTauntIndex)), RecvPropInt(RECVINFO(m_iTauntConcept)),

		RecvPropInt(RECVINFO(m_nPlayerCondEx)), RecvPropInt(RECVINFO(m_iStunIndex)),

		RecvPropInt(RECVINFO(m_nHalloweenBombHeadStage)),

		RecvPropInt(RECVINFO(m_nPlayerCondEx2)), RecvPropInt(RECVINFO(m_nPlayerCondEx3)),
		RecvPropArray3(RECVINFO_ARRAY(m_nStreaks), RecvPropInt(RECVINFO(m_nStreaks[0]))),
		RecvPropInt(RECVINFO(m_unTauntSourceItemID_Low)), RecvPropInt(RECVINFO(m_unTauntSourceItemID_High)),
		RecvPropFloat(RECVINFO(m_flRuneCharge)),
#ifdef STAGING_ONLY
		RecvPropFloat(RECVINFO(m_flSpaceJumpCharge)),
#endif
		RecvPropBool(RECVINFO(m_bHasPasstimeBall)), RecvPropBool(RECVINFO(m_bIsTargetedForPasstimePass)),
		RecvPropEHandle(RECVINFO(m_hPasstimePassTarget)), RecvPropFloat(RECVINFO(m_askForBallTime)),
...
BEGIN_SEND_TABLE_NOBASE(localplayerscoring_t, DT_TFPlayerScoringDataExclusive)
	SendPropInt(SENDINFO(m_iCaptures), 10, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iDefenses), 10, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iKills), 10, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iDeaths), 10, SPROP_UNSIGNED),
		...
		SendPropInt(SENDINFO(m_iHealPoints), 20, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iInvulns), 10, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iTeleports), 10, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iDamageDone), 20, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iCrits), 10, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iResupplyPoints), 10, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iKillAssists), 12, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iBonusPoints), 10, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iPoints), 10, SPROP_UNSIGNED),
END_SEND_TABLE()
...
BEGIN_SEND_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerSharedLocal)
	SendPropInt(SENDINFO(m_nDesiredDisguiseTeam), 3, SPROP_UNSIGNED),
		...
		SendPropBool(SENDINFO(m_bLastDisguisedAsOwnTeam)), SendPropTime(SENDINFO(m_flStealthNoAttackExpire)),
		SendPropTime(SENDINFO(m_flStealthNextChangeTime)),
		SendPropFloat(SENDINFO(m_flRageMeter), 0, SPROP_NOSCALE, 0.0, 100.0), SendPropBool(SENDINFO(m_bRageDraining)),
		SendPropTime(SENDINFO(m_flNextRageEarnTime)), SendPropBool(SENDINFO(m_bInUpgradeZone)),
...
BEGIN_SEND_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerShared)
	SendPropInt(SENDINFO(m_nPlayerCond), -1, SPROP_VARINT | SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_bJumping), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nNumHealers), 5, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iCritMult), 8, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iAirDash), -1, SPROP_VARINT | SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_nAirDucked), 2, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_flDuckTimer)),
		...
		SendPropFloat(SENDINFO(m_flMovementStunTime)), SendPropInt(SENDINFO(m_iMovementStunAmount), 8, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iMovementStunParity), MOVEMENTSTUN_PARITY_BITS, SPROP_UNSIGNED),
		SendPropEHandle(SENDINFO(m_hStunner)), SendPropInt(SENDINFO(m_iStunFlags), 12, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_nArenaNumChanges), 5, SPROP_UNSIGNED), SendPropBool(SENDINFO(m_bArenaFirstBloodBoost)),
		SendPropInt(SENDINFO(m_iWeaponKnockbackID)), SendPropBool(SENDINFO(m_bLoadoutUnavailable)),
		SendPropInt(SENDINFO(m_iItemFindBonus)), SendPropBool(SENDINFO(m_bShieldEquipped)),
		SendPropBool(SENDINFO(m_bParachuteEquipped)), SendPropInt(SENDINFO(m_iNextMeleeCrit)),
		SendPropInt(SENDINFO(m_iDecapitations), 8, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_iRevengeCrits), 7, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iDisguiseBody)),
		SendPropEHandle(SENDINFO(m_hCarriedObject)), SendPropBool(SENDINFO(m_bCarryingObject)),
		SendPropFloat(SENDINFO(m_flNextNoiseMakerTime)), SendPropInt(SENDINFO(m_iSpawnRoomTouchCount)),
		SendPropInt(SENDINFO(m_iKillCountSinceLastDeploy)), SendPropFloat(SENDINFO(m_flFirstPrimaryAttack)),
		...
		// Spy
		SendPropTime(SENDINFO(m_flInvisChangeCompleteTime)), SendPropInt(SENDINFO(m_nDisguiseTeam), 3, SPROP_UNSIGNED),
		...
		SendPropInt(SENDINFO(m_iDisguiseHealth), -1, SPROP_VARINT), SendPropBool(SENDINFO(m_bFeignDeathReady)),
		SendPropEHandle(SENDINFO(m_hDisguiseWeapon)), SendPropInt(SENDINFO(m_nTeamTeleporterUsed), 3, SPROP_UNSIGNED),
		...
		// Local Data.
		SendPropDataTable("tfsharedlocaldata", 0, &REFERENCE_SEND_TABLE(DT_TFPlayerSharedLocal),
...
#endif
		SendPropBool(SENDINFO(m_bHasPasstimeBall)), SendPropBool(SENDINFO(m_bIsTargetedForPasstimePass)),
		SendPropEHandle(SENDINFO(m_hPasstimePassTarget)), SendPropFloat(SENDINFO(m_askForBallTime)),
...
bool CTFPlayer::IsAllowedToTaunt(void)
{
	...
	{
		if(m_Shared.IsStealthed() || m_Shared.InCond(TF_COND_STEALTHED_BLINK) || m_Shared.InCond(TF_COND_DISGUISED) ||
...
{
public:
	CConditionVars(tIntType &nPlayerCond, tIntType &nPlayerCondEx, tIntType &nPlayerCondEx2, tIntType &nPlayerCondEx3,
				   ETFCond eCond)
	{
		if(eCond >= 96)
		{
...
//-----------------------------------------------------------------------------
void CTFPlayerShared::AddCond(ETFCond eCond, float flDuration /* = PERMANENT_CONDITION */,
							  CBaseEntity *pProvider /*= NULL */)
{
	...
	CConditionVars<int> cPlayerCond(m_nPlayerCond.m_Value, m_nPlayerCondEx.m_Value, m_nPlayerCondEx2.m_Value,
									m_nPlayerCondEx3.m_Value, eCond);
...
void CTFPlayerShared::RemoveCond(ETFCond eCond, bool ignore_duration)
{
	...
	if(m_ConditionData[eCond].m_nPreventedDamageFromCondition)
	{
		...
		if(pEvent)
		{
			pEvent->SetInt("preventor", m_ConditionData[eCond].m_pProvider
											? m_ConditionData[eCond].m_pProvider->entindex()
											: m_pOuter->entindex());
...
bool CTFPlayerShared::InCond(ETFCond eCond) const
{
	...
	CConditionVars<const int> cPlayerCond(m_nPlayerCond.m_Value, m_nPlayerCondEx.m_Value, m_nPlayerCondEx2.m_Value,
										  m_nPlayerCondEx3.m_Value, eCond);
...
{
	// I don't know if this actually works for conditions < 32, because we definitely cannot peak into m_ConditionList
	// (back in time). But others think that m_ConditionList is propogated into m_nOldConditions, so just check if you
	// hit the assert. (And then remove the assert. And this comment).
	Assert(eCond >= 32 && eCond < TF_COND_LAST);

	CConditionVars<const int> cPlayerCond(m_nOldConditions, m_nOldConditionsEx, m_nOldConditionsEx2,
										  m_nOldConditionsEx3, eCond);
	return (cPlayerCond.CondVar() & cPlayerCond.CondBit()) != 0;
}
...
void CTFPlayerShared::ForceRecondNextSync(ETFCond eCond)
{
	...
	CConditionVars<int> playerCond(m_nForceConditions, m_nForceConditionsEx, m_nForceConditionsEx2,
								   m_nForceConditionsEx3, eCond);
...
CBaseEntity *CTFPlayerShared::GetConditionAssistFromAttacker(void)
{
	...
	static const ETFCond nTrackedConditions[] = {
		...
		TF_COND_DEFENSEBUFF, TF_COND_REGENONDAMAGEBUFF,
...
void CTFPlayerShared::OnDataChanged(void)
{
	...
	if((IsLoser() || InCond(TF_COND_COMPETITIVE_LOSER)) && GetActiveTFWeapon() &&
	   !GetActiveTFWeapon()->IsEffectActive(EF_NODRAW))
	{
...
//-----------------------------------------------------------------------------
void CTFPlayerShared::SyncConditions(int nPreviousConditions, int nNewConditions, int nForceConditions,
									 int nBaseCondBit)
{
...
// Purpose:
//-----------------------------------------------------------------------------
int CTFPlayerShared::GetDisguiseMaxBuffedHealth(bool bIgnoreAttributes /*= false*/,
												bool bIgnoreHealthOverMax /*= false*/)
{
	// Find the healer we have who's providing the most overheal
...
void CTFPlayerShared::ConditionGameRulesThink(void)
{
	...
	if(InCond(TF_COND_HEALTH_BUFF))
	{
		...
		float flCurDisguiseOverheal = (GetDisguiseMaxHealth() != 0)
										  ? ((float)GetDisguiseHealth() / (float)GetDisguiseMaxHealth())
										  : (flCurOverheal);
		...
		for(int i = 0; i < m_aHealers.Count(); i++)
		{
			...
			if(m_aHealers[i].bDispenserHeal)
			{
				...
				if(bHealActual)
				{
					...
					m_aHealers[i].flHealAccum +=
						clamp(flDispenserFraction, 0.f, (float)GetMaxBuffedHealth() - m_pOuter->GetHealth());
				}
				if(bHealDisguise)
				{
			...
			else // player heals are affected by the last damage time
			{
				if(bHealActual)
				{
					...
					m_flHealFraction += gpGlobals->frametime * m_aHealers[i].flAmount * flScale * flAttribModScale *
										flPerHealerAttribModScale;
				}
				if(bHealDisguise)
				{
					m_flDisguiseHealFraction += gpGlobals->frametime * m_aHealers[i].flAmount * flScale *
												flAttribModScale * flPerHealerAttribModScale;
				}
			}
		...
		if(nHealthToAdd > 0 || nDisguiseHealthToAdd > 0)
		{
			...
			m_pOuter->AdjustDrownDmg(-1.0 * nHealthToAdd); // subtract this from the drowndmg in case they're drowning
														   // and being healed at the same time
				...
				for(int i = 0; i < m_aHealers.Count(); i++)
				{
					...
					if(m_aHealers[i].pHealScorer.IsValid() && m_aHealers[i].pHealer.IsValid())
					{
						...
						if(pHealer && IsAlly(pHealer))
						{
							...
							if(pHealScorer)
							{
								// Don't report healing when we're close to the buff cap and haven't taken damage
								// recently. This avoids sending bogus heal stats while maintaining our max overheal.
								// Ideally we wouldn't decay in this scenario, but that would be a risky change.
								if(iBoostMax - nPrevHealth > 1 ||
								   gpGlobals->curtime - m_pOuter->GetLastDamageReceivedTime() <= 1.f)
								{
								...
								HandleRageGain(m_pOuter, kRageBuffFlag_OnMedicHealingReceived, flHealAmount / 2.f,
											   1.0f);

								float flRage = flHealAmount;
								if(TFGameRules() && TFGameRules()->IsMannVsMachineMode() && TFObjectiveResource() &&
								...
								// If it's been one second, or we know healing beyond this point will be overheal,
								// generate an event
								if((m_flHealedPerSecondTimer <= gpGlobals->curtime ||
									m_pOuter->GetHealth() >= m_pOuter->GetMaxHealth()) &&
								   m_aHealers[i].flHealedLastSecond > 1)
								{
									// Make sure this isn't pure overheal
									if(m_pOuter->GetHealth() - m_aHealers[i].flHealedLastSecond <
									   m_pOuter->GetMaxHealth())
									{
										...
										if(m_aHealers[i].flOverhealBonus <= 1.0f)
										{
											...
											CWeaponMedigun *pMedigun = static_cast<CWeaponMedigun *>(
												pHealScorer->Weapon_OwnsThisID(TF_WEAPON_MEDIGUN));
											if(pMedigun)
											{
												// On Mediguns, per frame, the amount of uber added is based on
												// Default heal rate is 24per second, we scale based on that and
												// frametime
												pMedigun->AddCharge((m_aHealers[i].flHealedLastSecond / 24.0f) *
																	gpGlobals->frametime * 0.33f);
											}
										}
										...
										if(pHealScorer)
										{
											// Can be Mediguns or anything that gives off 'heal' buff like amputator aoe
											// heal
											EconEntity_OnOwnerKillEaterEvent_Batched(
												pHealScorer->GetActiveTFWeapon(), pHealScorer, m_pOuter,
												kKillEaterEvent_AllyHealingDone, m_aHealers[i].flHealedLastSecond);
										}
									}
						...
						{
							CTF_GameStats.Event_PlayerLeachedHealth(m_pOuter, m_aHealers[i].bDispenserHeal,
																	flHealAmount);
						}
					}
				}
	...
	if(!InCond(TF_COND_HEALTH_OVERHEALED) &&
	   m_pOuter->GetHealth() > (m_pOuter->GetMaxHealth() - m_pOuter->GetRuneHealthBonus()))
	{
		AddCond(TF_COND_HEALTH_OVERHEALED, PERMANENT_CONDITION);
	}
	else if(InCond(TF_COND_HEALTH_OVERHEALED) &&
			m_pOuter->GetHealth() <= (m_pOuter->GetMaxHealth() - m_pOuter->GetRuneHealthBonus()))
	{
	...
	if(bDecayHealth)
	{
		...
		{
			// Items exist that get us over max health, without ever being healed, in which case our
			// m_flBestOverhealDecayMult will still be -1.
	...
	if(InCond(TF_COND_BURNING) && !m_pOuter->m_bInPowerPlay)
	{
				...
				for(int i = 0; i < m_aHealers.Count(); i++)
				{
					...
					if(m_aHealers[i].bDispenserHeal)
					{
						...
						if(pDispenser)
						{
							...
							{
								UTIL_LogPrintf(
									"\"%s<%i><%s><%s>\" triggered \"player_extinguished\" against \"%s<%i><%s><%s>\" "
									"with \"dispenser\" (attacker_position \"%d %d %d\") (victim_position \"%d %d "
									"%d\")\n",
									pTFPlayer->GetPlayerName(), pTFPlayer->GetUserID(), pTFPlayer->GetNetworkIDString(),
									pTFPlayer->GetTeam()->GetName(), m_pOuter->GetPlayerName(), m_pOuter->GetUserID(),
									m_pOuter->GetNetworkIDString(), m_pOuter->GetTeam()->GetName(),
									(int)m_aHealers[i].pHealer->GetAbsOrigin().x,
									(int)m_aHealers[i].pHealer->GetAbsOrigin().y,
									(int)m_aHealers[i].pHealer->GetAbsOrigin().z, (int)m_pOuter->GetAbsOrigin().x,
									(int)m_pOuter->GetAbsOrigin().y, (int)m_pOuter->GetAbsOrigin().z);
							}
						}
					...
					{
						UTIL_LogPrintf(
							"\"%s<%i><%s><%s>\" triggered \"player_extinguished\" against \"%s<%i><%s><%s>\" with "
							"\"%s\" (attacker_position \"%d %d %d\") (victim_position \"%d %d %d\")\n",
							pTFPlayer->GetPlayerName(), pTFPlayer->GetUserID(), pTFPlayer->GetNetworkIDString(),
							pTFPlayer->GetTeam()->GetName(), m_pOuter->GetPlayerName(), m_pOuter->GetUserID(),
							m_pOuter->GetNetworkIDString(), m_pOuter->GetTeam()->GetName(),
							(pTFPlayer->GetActiveTFWeapon()) ? pTFPlayer->GetActiveTFWeapon()->GetName()
															 : "tf_weapon_medigun",
							(int)pTFPlayer->GetAbsOrigin().x, (int)pTFPlayer->GetAbsOrigin().y,
							(int)pTFPlayer->GetAbsOrigin().z, (int)m_pOuter->GetAbsOrigin().x,
							(int)m_pOuter->GetAbsOrigin().y, (int)m_pOuter->GetAbsOrigin().z);
					}
				...
				(TF_CLASS_PYRO != m_pOuter->GetPlayerClass()->GetClassIndex()))
		{
			...
			CTakeDamageInfo info(m_hBurnAttacker, m_hBurnAttacker, m_hBurnWeapon, flBurnDamage,
								 DMG_BURN | DMG_PREVENT_PHYSICS_FORCE, nKillType);
	...
	if(InCond(TF_COND_BLEEDING))
	{
		FOR_EACH_VEC_BACK(m_PlayerBleeds, i)
		{
			...
			if(TFGameRules() && TFGameRules()->IsTruceActive() && bleed.hBleedingAttacker &&
			   (bleed.hBleedingAttacker != m_pOuter) && bleed.hBleedingAttacker->IsTruceValidForEnt())
			{
			...
			else if((gpGlobals->curtime >= bleed.flBleedingTime))
			{
				...
				CTakeDamageInfo info(bleed.hBleedingAttacker, bleed.hBleedingAttacker, bleed.hBleedingWeapon,
									 bleed.nBleedDmg, DMG_SLASH, TF_DMG_CUSTOM_BLEEDING);
				m_pOuter->TakeDamage(info);
...
void CTFPlayerShared::ConditionThink(void)
{
	...
	if(InCond(TF_COND_STUNNED))
	{
		...
		else if(IsControlStunned() && GetActiveStunInfo() &&
				(gpGlobals->curtime > GetActiveStunInfo()->flStartFadeTime))
		{
...
#ifdef CLIENT_DLL
	if(InCond(TF_COND_TAUNTING) && m_flTauntParticleRefireTime > 0.0f &&
...
void CTFPlayerShared::OnAddDisguising(void)
{
	...
	if(!m_pOuter->IsLocalPlayer() && (!IsStealthed() || !m_pOuter->IsEnemyPlayer()))
	{
		const char *pEffectName =
			(m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? "spy_start_disguise_red" : "spy_start_disguise_blue";
...
void CTFPlayerShared::OnRemoveInvulnerable(void)
{
	...
	if(m_pOuter->IsLocalPlayer())
	{
		...
		if(pMaterial && (FStrEq(pMaterial->GetName(), TF_SCREEN_OVERLAY_MATERIAL_INVULN_BLUE) ||
...
void CTFPlayerShared::OnAddUrine(void)
{
	...
	{
		m_pOuter->m_pUrineEffect =
			m_pOuter->ParticleProp()->Create("peejar_drips", PATTACH_ABSORIGIN_FOLLOW); // pEffect! Kek!
	}
	...
	{
		IMaterial *pMaterial =
			materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_URINE, TEXTURE_GROUP_CLIENT_EFFECTS, false);
		if(!IsErrorMaterial(pMaterial))
		{
			view->SetScreenOverlayMaterial(pMaterial);
		}
...
void CTFPlayerShared::OnAddMarkedForDeath(void)
{
	...
	{
		IMaterial *pMaterial = materials->FindMaterial( TF_SCREEN_OVERLAY_MATERIAL_URINE, TEXTURE_GROUP_CLIENT_EFFECTS,
	false ); if ( !IsErrorMaterial( pMaterial ) )
		{
			view->SetScreenOverlayMaterial( pMaterial );
		}
	}
...
void CTFPlayerShared::OnAddCondParachute(void)
{
	...
	if(InCond(TF_COND_HALLOWEEN_KART))
	{
		if(!m_hKartParachuteEntity)
		{
			...
			pBanner->m_nSkin = 0;
			pBanner->InitializeAsClientEntity(
...
// 	if ( m_pOuter->IsLocalPlayer() )
// 	{
// 		IMaterial *pMaterial = materials->FindMaterial( TF_SCREEN_OVERLAY_MATERIAL_MILK, TEXTURE_GROUP_CLIENT_EFFECTS,
// false ); 		if ( !IsErrorMaterial( pMaterial ) )
// 		{
// 			view->SetScreenOverlayMaterial( pMaterial );
// 		}
		...
		if(pTauntItem)
		{
			...
			if(iQualityParticleType > 0)
			{
				const attachedparticlesystem_t *pParticleSystem =
					GetItemSchema()->GetAttributeControlledParticleSystem(iQualityParticleType);
				if(pParticleSystem)
				{
			...
			if(pTauntItem->FindAttribute(pAttrDef_OnTauntAttachParticleIndex, &unUnusualEffectIndex) &&
			   unUnusualEffectIndex > 0)
			{
				const attachedparticlesystem_t *pParticleSystem =
				...
				if(pParticleSystem)
				{
					...
					if(m_pOuter->GetTeamNumber() == TF_TEAM_BLUE &&
					   V_stristr(pParticleSystem->pszSystemName, "_teamcolor_red"))
					{
					...
					else if(m_pOuter->GetTeamNumber() == TF_TEAM_RED &&
							V_stristr(pParticleSystem->pszSystemName, "_teamcolor_blue"))
					{
			...
			for(int i = 0; i < m_pOuter->GetNumWearables(); ++i)
			{
				...
				CEconItemView *pItem =
					pWearable && pWearable->GetAttributeContainer() && pWearable->GetAttributeContainer()->GetItem()
						? pWearable->GetAttributeContainer()->GetItem()
						: NULL;

				// check for Unusual Cap def index (1173)
				if(pItem && pItem->GetItemDefIndex() == 1173 &&
				   pItem->FindAttribute(pAttrDef_OnTauntAttachParticleIndex, &unUnusualEffectIndex) &&
				   unUnusualEffectIndex > 0)
				{
					const attachedparticlesystem_t *pParticleSystem =
						GetItemSchema()->GetAttributeControlledParticleSystem(unUnusualEffectIndex);
...
void CTFPlayerShared::FireClientTauntParticleEffects()
{
	...
	if(TauntParticleState.first)
	{
		...
		const char *pszAttachment =
			m_pOuter->GetTauntEconItemView()->GetStaticData()->GetTauntData()->GetParticleAttachment();
		int iAttachment = pszAttachment ? m_pOuter->LookupAttachment(pszAttachment) : INVALID_PARTICLE_ATTACHMENT;
		m_pOuter->m_pTauntEffect = m_pOuter->ParticleProp()->Create(
			TauntParticleState.first,
			iAttachment != INVALID_PARTICLE_ATTACHMENT ? PATTACH_POINT_FOLLOW : PATTACH_ABSORIGIN_FOLLOW, iAttachment,
			vec3_origin);
...
void CTFPlayerShared::OnAddTaunting(void)
{
	...
	if((m_pOuter->IsPlayerClass(TF_CLASS_HEAVYWEAPONS) || m_pOuter->IsPlayerClass(TF_CLASS_SCOUT)) &&
	   GetTauntIndex() == TAUNT_BASE_WEAPON)
	{
...
void CTFPlayerShared::OnAddBleeding(void)
{
	...
	{
		IMaterial *pMaterial =
			materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_BLEED, TEXTURE_GROUP_CLIENT_EFFECTS, false);
		if(!IsErrorMaterial(pMaterial))
		{
			view->SetScreenOverlayMaterial(pMaterial);
		}
...
void CTFPlayerShared::OnAddSoldierOffensiveBuff(void)
{
	...
	{
		m_pOuter->m_pSoldierOffensiveBuffEffect =
			m_pOuter->ParticleProp()->Create(strBuffName, PATTACH_ABSORIGIN_FOLLOW);
	}
#endif
}
...
void CTFPlayerShared::OnAddSoldierDefensiveBuff(void)
{
	...
	{
		m_pOuter->m_pSoldierDefensiveBuffEffect =
			m_pOuter->ParticleProp()->Create(strBuffName, PATTACH_ABSORIGIN_FOLLOW);
	}
#endif
}
...
void CTFPlayerShared::OnAddSoldierOffensiveHealthRegenBuff(void)
{
	...
	{
		m_pOuter->m_pSoldierOffensiveHealthRegenBuffEffect =
			m_pOuter->ParticleProp()->Create(strBuffName, PATTACH_ABSORIGIN_FOLLOW);
	}
#endif
}
...
COMPILE_TIME_ASSERT(ARRAYSIZE(s_pszRedResistOverheadEffectName) == MEDIGUN_NUM_RESISTS &&
					ARRAYSIZE(s_pszBlueResistOverheadEffectName) == MEDIGUN_NUM_RESISTS);

static void AddResistParticle(CTFPlayer *pPlayer, medigun_resist_types_t nResistType,
							  ETFCond eYeildToCond = TF_COND_LAST)
{
...
static void AddResistShield(C_LocalTempEntity **pShield, CTFPlayer *pPlayer, ETFCond eCond)
{
	...
	(*pShield) = tempents->SpawnTempModel(pModel, pPlayer->GetAbsOrigin(), pPlayer->GetAbsAngles(), Vector(0, 0, 0), 1,
										  FTENT_NEVERDIE | FTENT_PLYRATTACHMENT);
	if(*pShield)
	{
...
static void RemoveResistShield(C_LocalTempEntity **pShield, CTFPlayer *pPlayer)
{
	if(*pShield)
	{
		...
		eCond =
			pPlayer->m_Shared.InCond(TF_COND_MEDIGUN_UBER_BULLET_RESIST) ? TF_COND_MEDIGUN_UBER_BULLET_RESIST : eCond;
		...
		eCond = (pPlayer->m_Shared.InCond(TF_COND_RUNE_RESIST) && !pPlayer->m_Shared.IsStealthed())
					? TF_COND_RUNE_RESIST
					: eCond;
		...
		if(pLocalPlayer)
		{
			eCond = (pPlayer->IsEnemyPlayer() && pPlayer->m_Shared.InCond(TF_COND_RUNE_PLAGUE) &&
					 pLocalPlayer->m_Shared.InCond(TF_COND_PLAGUE))
						? TF_COND_RUNE_PLAGUE
						: eCond;
		}
...
void CTFPlayerShared::OnAddMVMBotRadiowave(void)
{
	...
	{
		m_pOuter->m_pMVMBotRadiowave =
			m_pOuter->ParticleProp()->Create("bot_radio_waves", PATTACH_POINT_FOLLOW, "head");
	}
#else
	if(!m_pOuter->IsBot())
		return;
...
void CTFPlayerShared::OnRemoveHalloweenTiny(void)
{
	...
	if(result.DidHit())
	{
		...
		static Vector vTest[] = {Vector(32, 32, flExtraHeight),
...
#else
	IMaterial *pMaterial =
		materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_STEALTH, TEXTURE_GROUP_CLIENT_EFFECTS, false);
	if(!IsErrorMaterial(pMaterial))
	{
		view->SetScreenOverlayMaterial(pMaterial);
	}
...
void CTFPlayerShared::OnRemoveStealthedPhase(void)
{
	...
	trace_t trace;
	UTIL_TraceHull(vecPos, vecPos, VEC_HULL_MIN_SCALED(m_pOuter), VEC_HULL_MAX_SCALED(m_pOuter),
				   (MASK_SOLID | CONTENTS_PLAYERCLIP), m_pOuter, COLLISION_GROUP_NONE, &trace);
...
void CTFPlayerShared::OnAddHalloweenKart(void)
{
	...
	CTFWeaponBase *pMeleeWeapon =
		dynamic_cast<CTFWeaponBase *>(m_pOuter->GetEntityForLoadoutSlot(LOADOUT_POSITION_MELEE));
	Assert(pMeleeWeapon);
	if(pMeleeWeapon)
	{
...
#else
	// When we have every taunt cam use this system, we should clean up after ourselves. But for now, this causes a bad
	// interaction with other systems. m_pOuter->SetTauntCameraTargets( 0.0f, 0.0f );
...
void CTFPlayerShared::OnAddSwimmingCurse(void)
{
	...
	{
		IMaterial *pMaterial =
			materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_SWIMMING_CURSE, TEXTURE_GROUP_CLIENT_EFFECTS, false);
		if(!IsErrorMaterial(pMaterial))
		{
			view->SetScreenOverlayMaterial(pMaterial);
		}
...
void CTFPlayerShared::OnAddHalloweenKartCage(void)
{
	...
	{
		m_pOuter->m_hHalloweenKartCage = C_PlayerAttachedModel::Create("models/props_halloween/bumpercar_cage.mdl",
																	   m_pOuter, 0, vec3_origin, PAM_PERMANENT, 0);
...
void CTFPlayerShared::OnAddPasstimeInterception(void)
{
	...
	{
		IMaterial *pMaterial =
			materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_PHASE, TEXTURE_GROUP_CLIENT_EFFECTS, false);
		if(!IsErrorMaterial(pMaterial))
		{
			view->SetScreenOverlayMaterial(pMaterial);
		}
...
void CTFPlayerShared::OnAddRunePlague(void)
{
	...
	m_pOuter->m_pRunePlagueEffect =
		m_pOuter->ParticleProp()->Create("powerup_plague_carrier", PATTACH_ABSORIGIN_FOLLOW);

	// show resist effect on enemy player that has plague rune if local player is in plague cond
	C_TFPlayer *pLocalPlayer = C_TFPlayer::GetLocalTFPlayer();
	if(pLocalPlayer && pLocalPlayer != m_pOuter && pLocalPlayer->m_Shared.InCond(TF_COND_PLAGUE) &&
...
void CTFPlayerShared::OnRemoveInPurgatory(void)
{
	...
	if(m_pOuter->IsAlive())
	{
		...
		if(m_pOuter->GetTeam())
		{
			...
			UTIL_LogPrintf("HALLOWEEN: \"%s<%i><%s><%s>\" %s\n", m_pOuter->GetPlayerName(), m_pOuter->GetUserID(),
						   m_pOuter->GetNetworkIDString(), m_pOuter->GetTeam()->GetName(), pszLogMessage);
		}
	}
#endif
}
...
float CTFPlayerShared::CalculateChargeCap(void) const
{
	...
	// Scale yaw cap based on frametime to prevent differences in turn effectiveness due to variable framerate (between
	// clients mainly)
	if(tf_demoman_charge_frametime_scaling.GetBool())
	{
		...
		float flMod = RemapValClamped(gpGlobals->frametime, (TICK_INTERVAL * YAW_CAP_SCALE_MIN),
									  (TICK_INTERVAL * YAW_CAP_SCALE_MAX), 0.25f, 2.f);
		flCap *= flMod;
	}
...
void CTFPlayerShared::OnAddRadiusHeal(void)
{
	...
	if(InCond(TF_COND_RADIUSHEAL))
	{
		...
		if(m_pOuter->IsPlayerClass(TF_CLASS_SPY) && InCond(TF_COND_DISGUISED) &&
		   (GetDisguiseTeam() == GetLocalPlayerTeam()))
		{
		...
		if(!m_pOuter->m_pRadiusHealEffect)
		{
			m_pOuter->m_pRadiusHealEffect =
				m_pOuter->ParticleProp()->Create(pszRadiusHealEffect, PATTACH_ABSORIGIN_FOLLOW);
		}
	}
#endif
}
...
void CTFPlayerShared::OnAddMegaHeal(void)
{
	...
	if(InCond(TF_COND_MEGAHEAL))
	{
		...
		{
			m_pOuter->m_pMegaHealEffect =
				m_pOuter->ParticleProp()->Create(pszMegaHealEffectName, PATTACH_ABSORIGIN_FOLLOW);
		}
	}
#endif
}
...
void CTFPlayerShared::ClientKillStreakBuffThink(void)
{
	int nLoadoutSlot =
		m_pOuter->GetActiveTFWeapon()
			? m_pOuter->GetActiveTFWeapon()->GetAttributeContainer()->GetItem()->GetStaticData()->GetLoadoutSlot(
...
void CTFPlayerShared::Burn(CTFPlayer *pAttacker, CTFWeaponBase *pWeapon, float flBurningTime /*=-1*/)
{
	...
	// pyros don't burn persistently or take persistent burning damage, but we show brief burn effect so attacker can
	// tell they hit
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayerShared::MakeBleed(CTFPlayer *pPlayer, CTFWeaponBase *pWeapon, float flBleedingTime,
								int nBleedDmg /* = TF_BLEEDING_DMG */, bool bPermanentBleeding /*= false*/)
{
#ifdef CLIENT_DLL
	...
	// New bleed source
	bleed_struct_t bleedinfo = {pPlayer,		// hBleedingAttacker
	...
	if(m_flCloakStartTime > 0)
	{
		...
		if(flCloaktime > 0)
		{
			EconEntity_OnOwnerKillEaterEventNoPartner(
				dynamic_cast<CEconEntity *>(m_pOuter->GetEntityForLoadoutSlot(LOADOUT_POSITION_PDA2)), m_pOuter,
				kKillEaterEvent_TimeCloaked, (int)flCloaktime);
		}
...
void CTFPlayerShared::OnAddBurning(void)
{
	...
	if(!m_pOuter->m_pBurningEffect)
	{
		...
		{
			const char *pEffectName =
				(m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? "burningplayer_red" : "burningplayer_blue";
	...
	{
		IMaterial *pMaterial =
			materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_BURNING, TEXTURE_GROUP_CLIENT_EFFECTS, false);
		if(!IsErrorMaterial(pMaterial))
		{
			view->SetScreenOverlayMaterial(pMaterial);
		}
		...
		if(GetActiveStunInfo())
		{
			if(!m_pOuter->m_pStunnedEffect && !(GetActiveStunInfo()->iStunFlags & TF_STUN_NO_EFFECTS))
			{
				if((GetActiveStunInfo()->iStunFlags & TF_STUN_BY_TRIGGER) || InCond(TF_COND_HALLOWEEN_BOMB_HEAD))
				{
					...
					m_pOuter->m_pStunnedEffect =
						m_pOuter->ParticleProp()->Create(pEffectName, PATTACH_POINT_FOLLOW, "head");
				}
				else
				{
...
{
#ifdef GAME_DLL
	return (m_PlayerStuns.IsValidIndex(m_iStunIndex)) ? const_cast<stun_struct_t *>(&m_PlayerStuns[m_iStunIndex])
													  : NULL;
#else
...
void CTFPlayerShared::UpdateCritBoostEffect(ECritBoostUpdateType eUpdateType)
{
	...
	if(eUpdateType == kCritBoost_ForceRefresh || !bShouldDisplayCritBoostEffect)
	{
		if(m_pOuter->m_pCritBoostEffect)
		{
			...
			{
				m_pOuter->m_pCritBoostEffect->GetOwner()->ParticleProp()->StopEmissionAndDestroyImmediately(
					m_pOuter->m_pCritBoostEffect);
			}
			else
			{
		...
		if(pWeapon)
		{
			if(!m_pOuter->m_pCritBoostEffect)
			{
				if(InCond(TF_COND_DISGUISED) && !m_pOuter->IsLocalPlayer() &&
				   m_pOuter->GetTeamNumber() != GetLocalPlayerTeam())
				{
					const char *pEffectName =
						(GetDisguiseTeam() == TF_TEAM_RED) ? "critgun_weaponmodel_red" : "critgun_weaponmodel_blu";
					m_pOuter->m_pCritBoostEffect =
						pWeapon->ParticleProp()->Create(pEffectName, PATTACH_ABSORIGIN_FOLLOW);
				}
				else
				{
					const char *pEffectName = (m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? "critgun_weaponmodel_red"
				...
				if(m_pOuter->IsLocalPlayer())
				{
					if(m_pOuter->m_pCritBoostEffect)
					{
						ClientLeafSystem()->SetRenderGroup(m_pOuter->m_pCritBoostEffect->RenderHandle(),
...
bool CTFPlayerShared::IsSpyDisguisedAsMyTeam(CTFPlayer *pPlayer)
{
	...
	if(pPlayer->IsPlayerClass(TF_CLASS_SPY) && pPlayer->GetTeamNumber() != m_pOuter->GetTeamNumber() &&
...
void CTFPlayerShared::FadeInvis(float fAdditionalRateScale)
{
	...
	else if((nExpiringCondition != TF_COND_STEALTHED_USER_BUFF) && !InCond(TF_COND_STEALTHED_USER_BUFF_FADING))
	{
		...
		m_flStealthNoAttackExpire =
			gpGlobals->curtime + (tf_spy_cloak_no_attack_time.GetFloat() * flDecloakRateScale * fAdditionalRateScale);
	}
...
void CTFPlayerShared::InvisibilityThink(void)
{
	...
	if(iSniperCloak)
	{
		...
		if(!bMoving && !bAiming && !IsCarryingObject())
		{
			...
			if(!InCond(TF_COND_STEALTHED_USER_BUFF) && !m_flInvisibility && m_flLastNoMovementTime > 0.f &&
			   gpGlobals->curtime - m_flLastNoMovementTime > flSniperCloakDelay)
			{
...
bool CTFPlayerShared::IsCritBoosted(void) const
{
	bool bAllWeaponCritActive =
		(InCond(TF_COND_CRITBOOSTED) || InCond(TF_COND_CRITBOOSTED_PUMPKIN) || InCond(TF_COND_CRITBOOSTED_USER_BUFF) ||
#ifdef CLIENT_DLL
		 InCond(TF_COND_CRITBOOSTED_DEMO_CHARGE) ||
#endif
		 InCond(TF_COND_CRITBOOSTED_FIRST_BLOOD) || InCond(TF_COND_CRITBOOSTED_BONUS_TIME) ||
		 InCond(TF_COND_CRITBOOSTED_CTF_CAPTURE) || InCond(TF_COND_CRITBOOSTED_ON_KILL) ||
		 InCond(TF_COND_CRITBOOSTED_CARD_EFFECT) || InCond(TF_COND_CRITBOOSTED_RUNE_TEMP));
...
{
	bool bInvuln = InCond(TF_COND_INVULNERABLE) || InCond(TF_COND_INVULNERABLE_USER_BUFF) ||
...
{
	return InCond(TF_COND_DISGUISED_AS_DISPENSER)
			   ? (int)((m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? TF_TEAM_BLUE : TF_TEAM_RED)
			   : m_nDisguiseTeam;
}
...
void CTFPlayerShared::DetermineDisguiseWeapon(bool bForcePrimary)
{
	...
	if(strDisguiseWeapon)
	{
		...
		m_hDisguiseWeapon.Set(
			dynamic_cast<CTFWeaponBase *>(m_pOuter->GiveNamedItem(strDisguiseWeapon, iSubType, pItem, true)));
		if(m_hDisguiseWeapon)
		{
			...
			m_hDisguiseWeapon->SetContextThink(&CTFWeaponBase::DisguiseWeaponThink, gpGlobals->curtime + 0.5,
											   "DisguiseWeaponThink");
			...
			if(!m_hDisguiseWeapon->IsMeleeWeapon())
			{
				...
				{
					int nMaxCount =
						m_hDisguiseWeapon->UsesClipsForAmmo1()
		...
		if(pWearable)
		{
			...
			if(pScriptItem && pScriptItem->IsValid() && pScriptItem->GetStaticData()->GetItemClass())
			{
				CEconEntity *pNewItem = dynamic_cast<CEconEntity *>(
					m_pOuter->GiveNamedItem(pScriptItem->GetStaticData()->GetItemClass(), 0, pScriptItem));
				...
				if(pNewWearable)
				{
					...
					CTFWearableLevelableItem *pNewLevelableItem =
						dynamic_cast<CTFWearableLevelableItem *>(pNewWearable);
...
void CTFPlayerShared::ProcessDisguiseImpulse(CTFPlayer *pPlayer)
{
	...
	if(pPlayer->GetImpulse() > 200)
	{
		...
		if(pPlayer->CanDisguise_OnKill() && pPlayer->m_Shared.InCond(TF_COND_DISGUISED))
		{
			...
			if(Q_atoi(szClass) == pPlayer->m_Shared.GetDisguiseClass() &&
			   Q_atoi(szTeam) == pPlayer->m_Shared.GetDisguiseTeam())
			{
...
bool CTFPlayerShared::CanRecieveMedigunChargeEffect(medigun_charge_types eType) const
{
	...
	if(pItem && pItem->GetItemID() == TF_ITEM_CAPTURE_FLAG)
	{
		...
		if((eType == MEDIGUN_CHARGE_MEGAHEAL) || (eType == MEDIGUN_CHARGE_BULLET_RESIST) ||
		   (eType == MEDIGUN_CHARGE_BLAST_RESIST) || (eType == MEDIGUN_CHARGE_FIRE_RESIST))
		{
			bCanRecieve = true;
		}
	}
...
//-----------------------------------------------------------------------------
void CTFPlayerShared::Heal(CBaseEntity *pHealer, float flAmount, float flOverhealBonus, float flOverhealDecayMult,
						   bool bDispenserHeal /* = false */, CTFPlayer *pHealScorer /* = NULL */)
{
...
void CTFPlayerShared::RecalculateChargeEffects(bool bInstantRemove)
{
	...
	SetChargeEffect(MEDIGUN_CHARGE_INVULN, aCharges[MEDIGUN_CHARGE_INVULN].bActive, bInstantRemove,
					g_MedigunEffects[MEDIGUN_CHARGE_INVULN], tf_invuln_time.GetFloat(),
					aCharges[MEDIGUN_CHARGE_INVULN].pProvider);
	SetChargeEffect(MEDIGUN_CHARGE_CRITICALBOOST, aCharges[MEDIGUN_CHARGE_CRITICALBOOST].bActive, bInstantRemove,
					g_MedigunEffects[MEDIGUN_CHARGE_CRITICALBOOST], 0.0f,
					aCharges[MEDIGUN_CHARGE_CRITICALBOOST].pProvider);
	SetChargeEffect(MEDIGUN_CHARGE_MEGAHEAL, aCharges[MEDIGUN_CHARGE_MEGAHEAL].bActive, bInstantRemove,
					g_MedigunEffects[MEDIGUN_CHARGE_MEGAHEAL], 0.0f, aCharges[MEDIGUN_CHARGE_MEGAHEAL].pProvider);
	SetChargeEffect(MEDIGUN_CHARGE_BULLET_RESIST, aCharges[MEDIGUN_CHARGE_BULLET_RESIST].bActive, bInstantRemove,
					g_MedigunEffects[MEDIGUN_CHARGE_BULLET_RESIST], 0.0f,
					aCharges[MEDIGUN_CHARGE_BULLET_RESIST].pProvider);
	SetChargeEffect(MEDIGUN_CHARGE_BLAST_RESIST, aCharges[MEDIGUN_CHARGE_BLAST_RESIST].bActive, bInstantRemove,
					g_MedigunEffects[MEDIGUN_CHARGE_BLAST_RESIST], 0.0f,
					aCharges[MEDIGUN_CHARGE_BLAST_RESIST].pProvider);
	SetChargeEffect(MEDIGUN_CHARGE_FIRE_RESIST, aCharges[MEDIGUN_CHARGE_FIRE_RESIST].bActive, bInstantRemove,
					g_MedigunEffects[MEDIGUN_CHARGE_FIRE_RESIST], 0.0f, aCharges[MEDIGUN_CHARGE_FIRE_RESIST].pProvider);
}
...
void CTFPlayerShared::TestAndExpireChargeEffect(medigun_charge_types iCharge)
{
	...
	if(InCond(effects.eCondition))
	{
		...
		FOR_EACH_VEC(m_aHealers, i)
		{
			...
			if(pTFProvider == pTFHealer &&
			   pTFHealer->GetTimeSinceLastUserCommand() > weapon_medigun_chargerelease_rate.GetFloat() + 1.f)
			{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayerShared::SetChargeEffect(medigun_charge_types iCharge, bool bState, bool bInstant,
									  const MedigunEffects_t &effects, float flWearOffTime,
									  CTFPlayer *pProvider /*= NULL*/)
{
	...
	float flMaxDuration = (pProvider && pProvider->IsBot())
							  ? PERMANENT_CONDITION
							  : weapon_medigun_chargerelease_rate.GetFloat() +
									((TFGameRules() && TFGameRules()->GameModeUsesUpgrades()) ? 8.f : 1.f);
...
void CTFPlayerShared::CheckForAchievement(int iAchievement)
{
	   ...
	   (iAchievement == ACHIEVEMENT_TF_MEDIC_CHARGE_BLOCKER && InCond(TF_COND_INVULNERABLE)))
	{
		...
		// ACHIEVEMENT_TF_MEDIC_CHARGE_BLOCKER: We just blocked a capture, and we're invuln. Whoever's invulning us gets
		// the achievement.
		...
		for(int i = 0; i < m_aHealers.Count(); i++)
		{
			...
			if(pMedigun && pMedigun->IsReleasingCharge())
			{
				...
				if(iAchievement != ACHIEVEMENT_TF_MEDIC_SAVE_TEAMMATE ||
				   (gpGlobals->curtime - pMedigun->GetReleaseStartedAt()) < 1.0)
				{
...
void CTFPlayerShared::RecordDamageEvent(const CTakeDamageInfo &info, bool bKill, int nVictimPrevHealth)
{
	...
	float flDamageCriticalScale = info.GetDamageType() & DMG_DONT_COUNT_DAMAGE_TOWARDS_CRIT_RATE ? 0.0f : 1.0f;
			...
			if(m_DamageEvents[iDamage].nDamageType & DMG_BLAST)
			{
				...
				if((gpGlobals->curtime - m_DamageEvents[iDamage].flTime) < CRIT_DAMAGE_TIME)
				{
					...
					if(flDamage > m_DamageEvents[iDamage].flDamage)
					{
						...
						//						Msg( "Update Damage Event: D:%f, T:%f\n", m_DamageEvents[iDamage].flDamage,
						//m_DamageEvents[iDamage].flTime );
					}
...
void CTFPlayerShared::AddTempCritBonus(float flAmount)
{
	...
	int iIndex = m_DamageEvents.AddToTail();
	m_DamageEvents[iIndex].flDamage =
		RemapValClamped(flAmount, 0, 1, 0, TF_DAMAGE_CRITMOD_DAMAGE) / (TF_DAMAGE_CRITMOD_MAXMULT - 1.0);
...
int CTFPlayerShared::GetNumKillsInTime(float flTime)
{
	...
	{
		Warning("Player asking for damage events for time %.0f, but tf_damage_events_track_for is only tracking events "
				"for %.0f\n",
...
void CTFPlayerShared::StunPlayer(float flTime, float flReductionAmount, int iStunFlags, CTFPlayer *pAttacker)
{
	...
	if(InCond(TF_COND_STUNNED))
	{
		if(GetActiveStunInfo())
		{
			...
			if(flRemapAmount > GetActiveStunInfo()->flStunAmount || iStunFlags & TF_STUN_CONTROLS ||
	...
	if((GetActiveStunInfo()->iStunFlags & TF_STUN_CONTROLS) == TF_STUN_CONTROLS ||
	   (GetActiveStunInfo()->iStunFlags & TF_STUN_LOSER_STATE) == TF_STUN_LOSER_STATE)
	{
		m_pOuter->StopTaunt();
		m_pOuter->ClearExpression();
...
float CTFPlayerShared::GetAmountStunned(int iStunFlags)
{
	if(GetActiveStunInfo())
	{
		if(InCond(TF_COND_STUNNED) && (iStunFlags & GetActiveStunInfo()->iStunFlags) &&
		   (GetActiveStunInfo()->flExpireTime > gpGlobals->curtime))
...
//-----------------------------------------------------------------------------
void CTFPlayer::MaybeDrawRailgunBeam(IRecipientFilter *pFilter, CTFWeaponBase *pWeapon, const Vector &vStartPos,
									 const Vector &vEndPos)
{
	...
	if(!iShouldFireTracer)
	{
		iShouldFireTracer = m_Shared.InCond(TF_COND_SNIPERCHARGE_RAGE_BUFF) && pWeapon &&
							WeaponID_IsSniperRifle(pWeapon->GetWeaponID());
	}
	...
	if(iShouldFireTracer)
	{
		...
		TE_TFParticleEffectComplex(*pFilter, 0.0f, pParticleSystemName, vStartPos, QAngle(0, 0, 0), NULL, &controlPoint,
								   pWeapon, PATTACH_CUSTOMORIGIN);
#else  // !GAME_DLL
		CSmartPtr<CNewParticleEffect> pEffect =
			pWeapon->ParticleProp()->Create(pParticleSystemName, PATTACH_CUSTOMORIGIN, 0);
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayer::FireBullet(CTFWeaponBase *pWpn, const FireBulletsInfo_t &info, bool bDoEffects, int nDamageType,
						   int nCustomDamageType /*= TF_DMG_CUSTOM_NONE*/)
{
	...
	bool bPenetratingShot = ((ePenetrateType == TF_DMG_CUSTOM_PENETRATE_ALL_PLAYERS) ||
							 (ePenetrateType == TF_DMG_CUSTOM_PENETRATE_MY_TEAM) ||
							 (ePenetrateType == TF_DMG_CUSTOM_PENETRATE_NONBURNING_TEAMMATE));
	if(bPenetratingShot && trace.m_pEnt)
	{
		if(trace.m_pEnt->IsCombatCharacter() || trace.m_pEnt->IsBaseObject())
		{
			...
			CBulletPenetrateEnum bulletpenetrate(&ray, this, ePenetrateType,
												 ePenetrateType == TF_DMG_CUSTOM_PENETRATE_MY_TEAM);
	...
	if(bPenetratingShot)
	{
		...
		FOR_EACH_VEC(vecTracedEntities, i)
		{
			...
			if(pTraceToUse->m_pEnt == pTarget)
			{
				...
				{
					if(pTargetPlayer && GetTeamNumber() == pTargetPlayer->GetTeamNumber() &&
					   pTargetPlayer->m_Shared.InCond(TF_COND_BURNING))
					{
						ExtinguishPlayer(GetActiveWeapon(), ToTFPlayer(GetActiveWeapon()->GetOwner()), pTargetPlayer,
										 GetActiveWeapon()->GetName());
					}
				}
				...
				pTarget->DispatchTraceAttack(dmgInfo, info.m_vecDirShooting, pTraceToUse,
											 GetActiveWeapon() ? GetActiveWeapon()->GetDmgAccumulator() : NULL);
	...
	if(trace.fraction < 1.0)
	{
		...
		if(bDoEffects)
		{
			...
			if(((info.m_iTracerFreq != 0) && (tracerCount++ % info.m_iTracerFreq) == 0) ||
			   (ePenetrateType == TF_DMG_CUSTOM_PENETRATE_ALL_PLAYERS))
			{
				...
				// If we're using a viewmodel, override vecStart with the muzzle of that - just for the visual effect,
				// not gameplay.
...
void CTFPlayer::ImpactWaterTrace(trace_t &trace, const Vector &vecStart)
{
	...
	trace_t traceWater;
	UTIL_TraceLine(vecStart, trace.endpos, (MASK_SHOT | CONTENTS_WATER | CONTENTS_SLIME), this, COLLISION_GROUP_NONE,
				   &traceWater);
...
{
	return (GetActiveTFWeapon() && GetActiveTFWeapon()->GetAttributeContainer() &&
...
float CTFPlayer::TeamFortress_CalculateMaxSpeed(bool bIgnoreSpecialAbility /*= false*/) const
{
	...
	if(playerclass == TF_CLASS_UNDEFINED || !CanPlayerMove())
		return 1.0f; // this can't return 0 because other parts of the code interpret that as "use default speed" during
					 // setup
	...
	if(!TFGameRules()->IsMannVsMachineMode() ||
	   !IsMiniBoss()) // No aiming slowdown penalties for MiniBoss players in MVM
	{
		...
		if(pWeapon)
		{
			...
			if(pMedigun)
			{
				...
				if(pHealTarget)
				{
					...
					bool bCharge = (pMedigun->GetMedigunType() == MEDIGUN_QUICKFIX &&
									pHealTarget->m_Shared.InCond(TF_COND_SHIELD_CHARGE));

					const float flHealTargetMaxSpeed =
						(bCharge) ? tf_max_charge_speed.GetFloat() : pHealTarget->TeamFortress_CalculateMaxSpeed(true);
...
bool CTFPlayerShared::IsAiming(void)
{
	...
	if(m_pOuter->IsPlayerClass(TF_CLASS_SNIPER) && m_pOuter->GetActiveTFWeapon() &&
	   (m_pOuter->GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_SNIPERRIFLE_CLASSIC))
	{
...
bool CTFPlayer::Weapon_Switch(CBaseCombatWeapon *pWeapon, int viewmodelindex)
{
	...
	// set last weapon before we switch to a new weapon to make sure that we can get the correct last weapon in
	// Deploy/Holster This should be done in CBasePlayer::Weapon_Switch, but we don't want to break other games
	...
	if(bSwitched)
	{
		...
		{
			// this will skip the logic to ignore first time block and allow weapon to check for honorbound attribute
			// right away
...
#ifdef GAME_DLL
	if(bSwitched && TFGameRules() && TFGameRules()->IsInTraining() && TFGameRules()->GetTrainingModeLogic() &&
	   IsFakeClient() == false)
	{
...
const char *CTFPlayer::GetOverrideStepSound(const char *pszBaseStepSoundName)
{
	...
	if(TFGameRules() && TFGameRules()->IsMannVsMachineMode() && GetTeamNumber() == TF_TEAM_PVE_INVADERS &&
	   !IsMiniBoss() && !m_Shared.InCond(TF_COND_DISGUISED))
	{
		return "MVM.BotStep";
	}
...
bool CTFPlayer::CanAttack(int iCanAttackFlags)
{
	...
	if((m_Shared.GetStealthNoAttackExpireTime() > gpGlobals->curtime &&
		!m_Shared.InCond(TF_COND_STEALTHED_USER_BUFF)) ||
		...
		case TF_CLASS_DEMOMAN:
			if(!m_Shared.HasPasstimeBall())
			{
				CTFPipebombLauncher *pPipebombLauncher =
					static_cast<CTFPipebombLauncher *>(Weapon_OwnsThisID(TF_WEAPON_PIPEBOMBLAUNCHER));
...
bool CTFPlayer::CanPickupBuilding(CBaseObject *pPickupObject)
{
	...
	if(TFGameRules()->IsInTraining())
	{
		...
		switch(pPickupObject->GetType())
		{
			...
			case OBJ_TELEPORTER:
				return pPickupObject->GetObjectMode() == MODE_TELEPORTER_ENTRANCE
						   ? training_can_pickup_tele_entrance.GetBool()
						   : training_can_pickup_tele_exit.GetBool();
			case OBJ_SENTRYGUN:
	...
	for(int i = 0; i < GetObjectCount(); i++)
	{
		...
		FOR_EACH_VEC(ePickupPenetrate.m_Targets, iTarget)
		{
			if(ePickupPenetrate.m_Targets[iTarget].pTarget == pObj)
			{
				...
				trace_t pTraceToUse;
				UTIL_TraceLine(EyePosition(), EyePosition() + vecForward * iPickUpRange, (MASK_SOLID | CONTENTS_HITBOX),
							   &penetrateFilter, &pTraceToUse);
	...
	if(pPickupObject)
	{
		...
		if(iIncreasedRangeCost)
		{
			...
			if(nSqrDist > TF_BUILDING_RESCUE_MIN_RANGE_SQ)
			{
				...
				const char *pRailParticleName =
					GetTeamNumber() == TF_TEAM_BLUE ? "dxhr_sniper_rail_blue" : "dxhr_sniper_rail_red";
				...
				te_tf_particle_effects_control_point_t controlPoint = {
					PATTACH_WORLDORIGIN, pPickupObject->GetAbsOrigin() + Vector(0, 0, 32)};
				TE_TFParticleEffectComplex(filter, 0.0f, pRailParticleName, GetAbsOrigin() + Vector(0, 0, 32),
										   QAngle(0, 0, 0), NULL, &controlPoint);
...
{
	int iMax = (iClassIndex == -1) ? m_PlayerClass.GetData()->m_aAmmoMax[iAmmoIndex]
...
int CTFPlayer::GetNumActivePipebombs(void)
{
	...
	{
		CTFPipebombLauncher *pWeapon =
			dynamic_cast<CTFPipebombLauncher *>(Weapon_OwnsThisID(TF_WEAPON_PIPEBOMBLAUNCHER));
...
bool CTFPlayer::CanMoveDuringTaunt()
{
	...
	if(TFGameRules() && TFGameRules()->IsCompetitiveMode())
	{
		if((TFGameRules()->GetRoundRestartTime() > -1.f) &&
		   ((int)(TFGameRules()->GetRoundRestartTime() - gpGlobals->curtime) <=
...
void CTFPlayerShared::UpdatePhaseEffects(void)
{
	...
	if(m_pOuter)
	{
		...
		{
			m_pOuter->m_pPhaseStandingEffect =
...
void CTFPlayerShared::AddPhaseEffects(void)
{
	...
	const char *pTrailTeamName =
		(m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? "effects/beam001_red.vmt" : "effects/beam001_blu.vmt";
...
ConVar tf_halloween_kart_slow_turn_accel_speed("tf_halloween_kart_slow_turn_accel_speed", "200",
											   FCVAR_CHEAT | FCVAR_REPLICATED);
ConVar tf_halloween_kart_fast_turn_accel_speed("tf_halloween_kart_fast_turn_accel_speed", "400",
											   FCVAR_CHEAT | FCVAR_REPLICATED);
ConVar tf_halloween_kart_return_turn_accell("tf_halloween_kart_return_turn_accell", "200",
											FCVAR_CHEAT | FCVAR_REPLICATED);
...
ConVar tf_halloween_kart_turning_curve_peak_position("tf_halloween_kart_turning_curve_peak_position", "0.5",
													 FCVAR_CHEAT | FCVAR_REPLICATED);
ConVar tf_halloween_kart_stationary_turn_speed("tf_halloween_kart_stationary_turn_speed", "50",
											   FCVAR_CHEAT | FCVAR_REPLICATED);
ConVar tf_halloween_kart_reverse_turn_speed("tf_halloween_kart_reverse_turn_speed", "50",
											FCVAR_CHEAT | FCVAR_REPLICATED);
...
void CTFPlayerShared::CreateVehicleMove(float flInputSampleTime, CUserCmd *pCmd)
{
	...
	{
		flTargetTurnSpeed =
			Sign(m_pOuter->GetCurrentTauntMoveSpeed()) * flSign * tf_halloween_kart_reverse_turn_speed.GetFloat();
	}
	else
	{
		const float flSmoothCurveVal =
			SmoothCurve_Tweak(flNormalizedSpeed, tf_halloween_kart_turning_curve_peak_position.GetFloat());
		flTargetTurnSpeed = Sign(m_pOuter->GetCurrentTauntMoveSpeed()) * flSign *
							RemapValClamped(flSmoothCurveVal, 0.f, 1.f, tf_halloween_kart_slow_turn_speed.GetFloat(),
											tf_halloween_kart_fast_turn_speed.GetFloat());
	}
	...
	{
		flTurnAccel = RemapValClamped(flNormalizedSpeed, 0.f, 1.f, tf_halloween_kart_slow_turn_accel_speed.GetFloat(),
									  tf_halloween_kart_fast_turn_accel_speed.GetFloat());
	}
	else
	...
	const float flMaxPossibleTurnSpeed =
		Max(tf_halloween_kart_slow_turn_speed.GetFloat(), tf_halloween_kart_fast_turn_speed.GetFloat());
	...
	const float flTargetPitch =
		tf_halloween_kart_pitch.GetFloat() + m_pOuter->m_PlayerAnimState->GetRenderAngles()[PITCH];
	const float flStepSpeed = fabs(flTargetPitch - tf_halloween_kart_pitch.GetFloat()) <
									  fabs(m_angVehicleMovePitchLast - tf_halloween_kart_pitch.GetFloat())
								  ? tf_halloween_kart_pitch_fast_follow_rate.GetFloat()
								  : tf_halloween_kart_pitch_slow_follow_rate.GetFloat();
	...
	else if(pCmd->buttons & IN_BACK) // Hitting the brakes
	{
		...
		else
		{
			...
			if((pCmd->buttons & IN_BACK) || m_pOuter->GetCurrentTauntMoveSpeed() < 0 ||
...
void CTFPlayerShared::VehicleThink(void)
{
	...
	static WheelEffect_t wheelEffects[] = {WheelEffect_t(20.f, "kart_dust_trail_red", "kart_dust_trail_blue")};
...
float CTFPlayer::GetKartSpeedBoost(void)
{
	...
	// Calculate time
	return RemapValClamped(gpGlobals->curtime, m_flKartNextAvailableBoost - tf_halloween_kart_boost_recharge.GetFloat(),
						   m_flKartNextAvailableBoost, 0.0f, 1.0f);
}
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
...
void CTFPlayerShared::UpdateRageBuffsAndRage(void)
{
	...
	if(m_bRageDraining)
	{
		...
		Assert(nBuffType >= 0 &&
			   nBuffType < ARRAYSIZE(g_RageBuffTypes)); // 0 is valid in the array, but an invalid buff
...
void CTFPlayerShared::SetRageMeter(float val)
{
	...
	{
		Assert(k_RageBuffType_Sniper > 0 &&
			   k_RageBuffType_Sniper < ARRAYSIZE(g_RageBuffTypes)); // 0 is valid in the array, but an invalid buff
...
void CTFPlayerShared::PulseRageBuff(ERageBuffSlot eBuffSlot)
{
	...
	for(int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; ++iPlayerIndex)
	{
		...
		if(pTFPlayer->m_Shared.InCond(TF_COND_DISGUISED) &&
		   (pTFPlayer->m_Shared.GetDisguiseTeam() != m_pOuter->GetTeamNumber()))
...
void CTFPlayerShared::UpdateEnergyDrinkMeter(void)
{
	...
	if(bIsLocalPlayer)
	{
		...
		else if(m_flEnergyDrinkMeter < 100.0f)
		{
			...
			CTFLunchBox_Drink *pDrink =
				static_cast<CTFLunchBox_Drink *>(m_pOuter->Weapon_OwnsThisID(TF_WEAPON_LUNCHBOX));

			if(pDrink)
			{
				// This is here in case something replenishes grenades
				if(m_flEnergyDrinkMeter < 100.0f &&
				   m_pOuter->GetAmmoCount(TF_AMMO_GRENADES2) == m_pOuter->GetMaxAmmo(TF_AMMO_GRENADES2))
				{
	...
	if(InCond(TF_COND_STEALTHED))
	{
		...
		for(int i = 0; g_aDebuffConditions[i] != TF_COND_LAST; i++)
		{
			if(InCond(g_aDebuffConditions[i]))
			{
				...
				{
					m_ConditionData[g_aDebuffConditions[i]].m_flExpireTime =
						MAX(m_ConditionData[g_aDebuffConditions[i]].m_flExpireTime - flReduction, 0);
				}
...
void CTFPlayerShared::DoRocketPack()
{
	...
	const float flVertPushScale =
		(m_pOuter->GetFlags() & FL_ONGROUND) ? 1.2f : 0.25f; // Less vertical force while airborne
...
void CTFPlayerShared::PulseMedicRadiusHeal(void)
{
	...
	if(gpGlobals->curtime >= m_flRadiusHealCheckTime)
	{
		for(int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; ++iPlayerIndex)
		{
			...
			if(vDist.LengthSqr() <= 450 * 450)
			{
				...
				trace_t trace;
				UTIL_TraceLine(pTFPlayer->WorldSpaceCenter(), m_pOuter->WorldSpaceCenter(), MASK_SOLID_BRUSHONLY, NULL,
	...
	if(!m_pOuter->m_pRadiusHealEffect)
	{
		...
		m_pOuter->m_pRadiusHealEffect =
			m_pOuter->ParticleProp()->Create(pszRadiusHealEffect, PATTACH_ABSORIGIN_FOLLOW, NULL, Vector(0, 0, 0));
	}
#endif // CLIENT_DLL
}
...
void CTFPlayerShared::PulseKingRuneBuff(void)
{
	...
	if(m_bKingRuneBuffActive && !InCond(TF_COND_PLAGUE))
	{
		if(!m_pOuter->m_pKingRuneRadiusEffect)
		{
			...
			m_pOuter->m_pKingRuneRadiusEffect =
				m_pOuter->ParticleProp()->Create(pszRadiusEffect, PATTACH_ABSORIGIN_FOLLOW, NULL, Vector(0, 0, 0));
		}
	}
	else
	{
...
// Purpose:
//-----------------------------------------------------------------------------
CEconItemView *CTFPlayerSharedUtils::GetEconItemViewByLoadoutSlot(CTFPlayer *pTFPlayer, int iSlot,
																  CEconEntity **pEntity)
{
...
bool CTFPlayer::CanPickupDroppedWeapon(const CTFDroppedWeapon *pWeapon)
{
	...
	return pOwnedWeaponToDrop && pWeapon->GetItem()->GetStaticData()->CanBeUsedByClass(iClass) &&
		   IsValidPickupWeaponSlot(iItemSlot);
}
...
CTFDroppedWeapon *CTFPlayer::GetDroppedWeaponInRange()
{
	...
	trace_t tr;
	UTIL_TraceLine(EyePosition(), EyePosition() + vecForward * TF_WEAPON_PICKUP_RANGE, MASK_SOLID | CONTENTS_DEBRIS,
