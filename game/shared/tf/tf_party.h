...
public:
	...
	virtual GCSDK::PlayerGroupID_t GetGroupID() const
	{
		return Obj().party_id();
	}
	...
	virtual int GetNumPendingInvites() const
	{
		return Obj().pending_invites_size();
	}
	...
	CSteamID GetSteamLobbyID() const
	{
		return CSteamID(Obj().steam_lobby_id());
	}
	...
	virtual bool AllowInvites() const
	{
		return false;
	} // we are using Steam invites
	...
	inline bool GetSearchLateJoinOK() const
	{
		return Obj().search_late_join_ok();
	}
	inline bool GetSearchPlayForBraggingRights() const
	{
		return Obj().search_play_for_bragging_rights();
	}
	TF_MatchmakingMode GetMatchmakingMode() const
	{
		return Obj().matchmaking_mode();
	}
	uint32 GetSearchQuickplayGameType() const
	{
		return Obj().search_quickplay_game_type();
	}
	EMatchGroup GetMatchGroup() const;
	void GetSearchChallenges(CMvMMissionSet &challenges) const;
	uint32 GetSearchLadderGameType() const
	{
		return Obj().search_ladder_game_type();
	}
	...
	uint32 GetCustomPingTolerance() const
	{
		return Obj().custom_ping_tolerance();
	}
	...
	void UpdatePreventMatchmakingDate(); // sets prevent_match variables to match the party member blocked from
										 // matchmaking the longest
...
class CTFPartyInvite : public GCSDK::CProtoBufSharedObject<CSOTFPartyInvite, k_EProtoObjectTFPartyInvite>,
