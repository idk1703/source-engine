...
ConVar tf_rd_max_points_override("tf_rd_max_points_override", "0", FCVAR_GAMEDLL,
								 "When changed, overrides the current max points", cc_tf_rd_max_points_override);
...
void cc_tf_rd_score_blue_points(const CCommand &args)
{
	...
	if(CTFRobotDestructionLogic::GetRobotDestructionLogic())
		CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints(TF_TEAM_BLUE, nPoints, SCORE_CORES_COLLECTED,
																		  NULL);
}
ConCommand tf_rd_score_blue_points("tf_rd_score_blue_points", cc_tf_rd_score_blue_points, "Give blue points.",
								   FCVAR_CHEAT);
...
void cc_tf_rd_score_red_points(const CCommand &args)
{
	...
	if(CTFRobotDestructionLogic::GetRobotDestructionLogic())
		CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints(TF_TEAM_RED, nPoints, SCORE_CORES_COLLECTED,
																		  NULL);
}
...
ConVar tf_rd_robot_attack_notification_cooldown("tf_rd_robot_attack_notification_cooldown", "10",
...
ConVar tf_rd_points_approach_interval("tf_rd_points_approach_interval", "0.1f",
									  FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY);
...
void CTFRobotDestruction_RobotSpawn::Activate()
{
	...
	if(pEnt != pGroup)
	{
		const char *pszMsg = CFmtStr("%s specified '%s' as its group, but %s is a %s", STRING(GetEntityName()),
									 m_spawnData.m_pszGroupName, m_spawnData.m_pszGroupName, pEnt->GetClassname());
	...
	if(pPath != pEnt)
	{
		const char *pszMsg = CFmtStr("%s specified '%s' as its first path, but %s is a %s", STRING(GetEntityName()),
									 m_spawnData.m_pszPathName, m_spawnData.m_pszPathName, pEnt->GetClassname());
	...
	else if(pEnt == NULL)
	{
		const char *pszMsg = CFmtStr("%s specified '%s' as its first path, but %s doesn't exist",
									 STRING(GetEntityName()), m_spawnData.m_pszPathName, m_spawnData.m_pszPathName);
		AssertMsg(0, "%s", pszMsg);
		Warning("%s", pszMsg);
	}
#endif
...
BEGIN_DATADESC(CTFRobotDestruction_RobotGroup)
	...
	DEFINE_KEYFIELD(m_iszHudIcon, FIELD_STRING, "hud_icon"),
		...
		DEFINE_OUTPUT(m_OnRobotsRespawn, "OnRobotsRespawn"), DEFINE_OUTPUT(m_OnAllRobotsDead, "OnAllRobotsDead"),
#endif
END_DATADESC()
...
#ifdef CLIENT_DLL
	RecvPropString(RECVINFO(m_pszHudIcon)), RecvPropInt(RECVINFO(m_iTeamNum)), RecvPropInt(RECVINFO(m_nGroupNumber)),
		RecvPropInt(RECVINFO(m_nState)), RecvPropFloat(RECVINFO(m_flRespawnStartTime)),
		RecvPropFloat(RECVINFO(m_flRespawnEndTime)), RecvPropFloat(RECVINFO(m_flLastAttackedTime)),
#else
	SendPropString(SENDINFO(m_pszHudIcon)), SendPropInt(SENDINFO(m_iTeamNum), -1, SPROP_VARINT | SPROP_UNSIGNED),
...
CTFRobotDestruction_RobotGroup::CTFRobotDestruction_RobotGroup() : m_flRespawnTime(0.f), m_nTeamNumber(0)
{
...
void CTFRobotDestruction_RobotGroup::StartRespawnTimerIfNeeded(CTFRobotDestruction_RobotGroup *pMasterGroup)
{
	...
	float flRespawnTime =
		bIsMaster ? gpGlobals->curtime + m_flRespawnTime : pMasterGroup->GetNextThink(GROUP_RESPAWN_CONTEXT);
...
void CTFRobotDestructionLogic::ApproachTargetScoresThink()
{
	...
	if(m_nBlueTargetPoints != m_nBlueScore.Get() || m_nRedTargetPoints != m_nRedScore.Get())
	{
		SetContextThink(&CTFRobotDestructionLogic::ApproachTargetScoresThink,
						gpGlobals->curtime + tf_rd_points_approach_interval.GetFloat(), APPROACH_POINTS_THINK);
	}
}
...
int CTFRobotDestructionLogic::ApproachTeamTargetScore(int nTeam, int nApproachScore, int nCurrentScore)
{
	if(nApproachScore != nCurrentScore)
	{
		...
		int nDelta = clamp(nApproachScore - nCurrentScore, -tf_rd_points_per_approach.GetInt(),
						   tf_rd_points_per_approach.GetInt());
...
void CTFRobotDestructionLogic::ScorePoints(int nTeam, int nPoints, RDScoreMethod_t eMethod, CTFPlayer *pPlayer)
{
	...
	FOR_EACH_VEC(vecAllPlayers, i)
	{
		...
		bool bPositive = (pSoundPlayer->GetTeamNumber() == nTeam && nPoints > 0) ||
						 (pSoundPlayer->GetTeamNumber() != nTeam && nPoints < 0);
	...
	if(eMethod == SCORE_CORES_COLLECTED && (nOldScore < nCloseToWinningThreshold) &&
	   (nNewScore >= nCloseToWinningThreshold) && GetType() == TYPE_ROBOT_DESTRUCTION)
	{
...
void CTFRobotDestructionLogic::PlaySoundInfoForScoreEvent(CTFPlayer *pPlayer, bool bPositive, int nNewScore, int nTeam,
														  RDScoreMethod_t eMethod)
{
	if(!pPlayer)
		return;
...
void CTFRobotDestructionLogic::ClientThink()
{
	...
	if(flSoonestFinale <= tf_rd_finale_beep_time.GetFloat() && flSoonestFinale > 0.f)
	{
		...
		if(pPlayer)
		{
			...
			float flExcitementScale = RemapValClamped(
				Bias(1.f - (flSoonestFinale / tf_rd_finale_beep_time.GetFloat()), 0.2f), 0.f, 1.f, 0.3f, 1.f);
			float soundlen = 0;
			EmitSound_t params;
			params.m_flSoundTime = 0;
...
void CTFRobotDestructionLogic::TeamWin(int nTeam)
{
	...
	if(TFGameRules())
	{
		TFGameRules()->SetWinningTeam(
			nTeam,
			(eMethod == SCORE_REACTOR_CAPTURED)
				? WINREASON_RD_REACTOR_CAPTURED
				: ((eMethod == SCORE_CORES_COLLECTED) ? WINREASON_RD_CORES_COLLECTED : WINREASON_RD_REACTOR_RETURNED));
	}
}
...
BEGIN_DATADESC(CTFRobotDestructionLogic)
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "RoundActivate", InputRoundActivate),
		...
		DEFINE_OUTPUT(m_OnRedHitZeroPoints, "OnRedHitZeroPoints"), DEFINE_OUTPUT(m_OnRedHasPoints, "OnRedHasPoints"),
...
BEGIN_NETWORK_TABLE_NOBASE(CTFRobotDestructionLogic, DT_TFRobotDestructionLogic)
#ifdef CLIENT_DLL
	RecvPropInt(RECVINFO(m_nMaxPoints)), RecvPropInt(RECVINFO(m_nBlueScore)), RecvPropInt(RECVINFO(m_nRedScore)),
		RecvPropInt(RECVINFO(m_nBlueTargetPoints)), RecvPropInt(RECVINFO(m_nRedTargetPoints)),
		RecvPropFloat(RECVINFO(m_flBlueTeamRespawnScale)), RecvPropFloat(RECVINFO(m_flRedTeamRespawnScale)),
		RecvPropFloat(RECVINFO(m_flBlueFinaleEndTime)), RecvPropFloat(RECVINFO(m_flRedFinaleEndTime)),
		RecvPropFloat(RECVINFO(m_flFinaleLength)), RecvPropString(RECVINFO(m_szResFile)),
	...
	SendPropInt(SENDINFO(m_nMaxPoints), -1, SPROP_VARINT | SPROP_UNSIGNED),
		...
		SendPropFloat(SENDINFO(m_flFinaleLength), -1, SPROP_NOSCALE), SendPropString(SENDINFO(m_szResFile)),
		SendPropArray3(SENDINFO_ARRAY3(m_eWinningMethod),
					   SendPropInt(SENDINFO_ARRAY(m_eWinningMethod), -1, SPROP_UNSIGNED | SPROP_VARINT)),
...
BEGIN_DATADESC(CRobotDestructionVaultTrigger)
	DEFINE_OUTPUT(m_OnPointsStolen, "OnPointsStolen"), DEFINE_OUTPUT(m_OnPointsStartStealing, "OnPointsStartStealing"),
...
{
	// Do it again!
	SetContextThink(&CRobotDestructionVaultTrigger::StealPointsThink, gpGlobals->curtime + tf_rd_steal_rate.GetFloat(),
					ADD_POINTS_CONTEXT);
...
int CRobotDestructionVaultTrigger::StealPoints(CTFPlayer *pPlayer)
{
	...
	if(pFlag && CTFRobotDestructionLogic::GetRobotDestructionLogic())
	{
		...
		if(nEnemyPoints)
		{
			...
			CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints(nEnemyTeamNumber, -nPointsToSteal,
																			  SCORE_REACTOR_STEAL, pPlayer);

			SetContextThink(&CRobotDestructionVaultTrigger::StealPointsThink,
							gpGlobals->curtime + tf_rd_steal_rate.GetFloat(), ADD_POINTS_CONTEXT);
