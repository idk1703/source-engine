...
//-----------------------------------------------------------------------------
void CTFBat_Wood::GetBallDynamics(Vector &vecLoc, QAngle &vecAngles, Vector &vecVelocity, AngularImpulse &angImpulse,
								  CTFPlayer *pPlayer)
{
...
void CTFBat_Wood::SecondaryAttack(void)
{
	...
	if((iBallCount > 0) && CanCreateBall(pPlayer))
	{
		...
		SetContextThink(&CTFBat_Wood::LaunchBallThink, gpGlobals->curtime + tf_scout_bat_launch_delay.GetFloat(),
						"LAUNCH_BALL_THINK");
...
bool CTFBat_Wood::CanCreateBall(CTFPlayer *pPlayer)
{
	...
	UTIL_TraceHull(vecBallStart, vecBallEnd, -Vector(8, 8, 8), Vector(8, 8, 8), MASK_SOLID_BRUSHONLY, &traceFilter,
				   &trace);
...
{
	CTFStunBall *pBall = static_cast<CTFStunBall *>(
		CBaseAnimating::CreateNoSpawn("tf_projectile_stun_ball", vecOrigin, vecAngles, pOwner));
	if(pBall)
	{
		DispatchSpawn(pBall);
	}
...
void CTFStunBall::Spawn(void)
{
	...
	if(!m_pBallTrail)
	{
		const char *pTrailTeamName =
			(GetTeamNumber() == TF_TEAM_RED) ? "effects/baseballtrail_red.vmt" : "effects/baseballtrail_blu.vmt";
...
void CTFStunBall::ApplyBallImpactEffectOnVictim(CBaseEntity *pOther)
{
	...
	if(flLifeTimeRatio > 0.1f)
	{
		...
		if(TFGameRules() && TFGameRules()->GameModeUsesMiniBosses() &&
		   (pPlayer->IsMiniBoss() || pPlayer->GetModelScale() > 1.0f))
		{
	...
	if(m_bTouched)
	{
		   ...
		   (pPlayer->GetAmmoCount(TF_AMMO_GRENADES1) < pPlayer->GetMaxAmmo(TF_AMMO_GRENADES1)))
		{
			...
			pPlayer->SpeakConceptIfAllowed(MP_CONCEPT_GRAB_BALL,
										   (pOther->GetTeamNumber() == GetTeamNumber()) ? "my_team:1" : "my_team:0");
		}
		return;
	}
...
{
	CTFBall_Ornament *pBall = static_cast<CTFBall_Ornament *>(
		CBaseAnimating::CreateNoSpawn("tf_projectile_ball_ornament", vecOrigin, vecAngles, pOwner));
	if(pBall)
	{
		DispatchSpawn(pBall);
	}
...
void CTFBall_Ornament::Explode(trace_t *pTrace, int bitsDamageType)
{
	...
	CTakeDamageInfo info(this, GetThrower(), m_hLauncher, vecBlastForce, GetAbsOrigin(), flExplodeDamage,
						 bitsDamageType, TF_DMG_CUSTOM_BASEBALL, &vecOrigin);
