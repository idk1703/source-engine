...
bool CTFMechanicalArm::ShockAttack(void)
{
	...
	int count = UTIL_EntitiesInBox(pObjects, maxCollectedEntities, vecCenter - vecSize, vecCenter + vecSize,
								   FL_GRENADE | FL_CLIENT | FL_FAKECLIENT);
...
void CTFMechanicalArm::SecondaryAttack(void)
{
	...
	{
		if((GetAmmoPerShot() > GetOwner()->GetAmmoCount(m_iPrimaryAmmoType)) || (pOwner->GetWaterLevel() == WL_Eyes))
		{
			WeaponSound(EMPTY);
	...
	CNewParticleEffect *pEffect =
		ParticleProp()->Create("dxhr_sniper_fizzle", PATTACH_POINT_FOLLOW, iParticleAttachment);
...
void CTFMechanicalArm::UpdateParticleBeam()
{
	...
	if(pFiringPlayer && pFiringPlayer->m_nButtons & IN_ATTACK && pFiringPlayer->GetActiveWeapon() == this &&
	   pFiringPlayer->GetWaterLevel() != WL_Eyes && pFiringPlayer->m_flNextAttack < gpGlobals->curtime)
	{
		trace_t tr;
		Vector vecAiming;
		pFiringPlayer->EyeVectors(&vecAiming);
		Vector vecEnd = pFiringPlayer->EyePosition() + vecAiming * 256.0f;
		UTIL_TraceLine(pFiringPlayer->EyePosition(), vecEnd, (MASK_SHOT & ~CONTENTS_HITBOX), pFiringPlayer, DMG_GENERIC,
					   &tr);
		...
		if(m_pParticleBeamEffect)
		{
			...
			pEffectOwner->ParticleProp()->AddControlPoint(m_pParticleBeamEffect, 1, pFiringPlayer,
														  PATTACH_ABSORIGIN_FOLLOW, NULL, vEndPos);
		}
		...
		{
			Vector vEndPos = tr.endpos - pFiringPlayer->GetAbsOrigin();
			pEffectOwner->ParticleProp()->AddControlPoint(m_pParticleBeamSpark, 1, pFiringPlayer,
														  PATTACH_ABSORIGIN_FOLLOW, NULL, vEndPos);
		}
	}
