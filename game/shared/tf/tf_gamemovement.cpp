...
ConVar tf_avoidteammates("tf_avoidteammates", "1", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
						 ...
						 "Teammates pass through each other, but push each other away (default)");
ConVar tf_avoidteammates_pushaway("tf_avoidteammates_pushaway", "1", FCVAR_REPLICATED,
...
ConVar tf_max_charge_speed("tf_max_charge_speed", "750",
						   FCVAR_NOTIFY | FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar tf_parachute_gravity("tf_parachute_gravity", "0.2f", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
							"Gravity while parachute is deployed");
ConVar tf_parachute_maxspeed_xy("tf_parachute_maxspeed_xy", "400.0f", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
								"Max XY Speed while Parachute is deployed");
ConVar tf_parachute_maxspeed_z("tf_parachute_maxspeed_z", "-100.0f", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
							   "Max Z Speed while Parachute is deployed");
ConVar tf_parachute_maxspeed_onfire_z("tf_parachute_maxspeed_onfire_z", "10.0f",
									  FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
									  "Max Z Speed when on Fire and Parachute is deployed");
ConVar tf_parachute_aircontrol("tf_parachute_aircontrol", "2.5f", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
							   "Multiplier for how much air control players have when Parachute is deployed");

ConVar tf_halloween_kart_aircontrol("tf_halloween_kart_aircontrol", "1.2f", FCVAR_CHEAT | FCVAR_REPLICATED,
									"Multiplier for how much air control players have when in Kart Mode");
ConVar tf_ghost_up_speed("tf_ghost_up_speed", "300.f", FCVAR_CHEAT | FCVAR_REPLICATED,
...
ConVar tf_grapplinghook_use_acceleration("tf_grapplinghook_use_acceleration", "0", FCVAR_REPLICATED,
...
ConVar tf_grapplinghook_prevent_fall_damage("tf_grapplinghook_prevent_fall_damage", "0",
											FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar tf_grapplinghook_medic_latch_speed_scale("tf_grapplinghook_medic_latch_speed_scale", "0.65",
												FCVAR_REPLICATED | FCVAR_CHEAT);

#ifdef STAGING_ONLY
ConVar tf_movement_doubletap_window("tf_movement_doubletap_window", "0.1f", FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar tf_space_gravity_jump_multipler("tf_space_gravity_jump_multipler", "1.05", FCVAR_CHEAT | FCVAR_REPLICATED,
									   "Multiplier for player jump velocity in space");
ConVar tf_space_aircontrol("tf_space_aircontrol", "1.0", FCVAR_CHEAT | FCVAR_REPLICATED,
						   "Multiplier for how much air control players have in space");
...
void CTFGameMovement::PlayerMove()
{
	...
	if(m_nOldWaterLevel != nNewWaterLevel)
	{
		if(WL_NotInWater == m_nOldWaterLevel)
		{
			...
			{
				// If the player has significant downward velocity, play a splash regardless of water depth.  (e.g.
				// Jumping hard into a puddle)
				bPlaySplash = true;
			}
			else
			{
				// Look at the water depth below the player.  If it's significantly deep, play a splash to accompany the
				// sinking that's about to happen.
...
bool CTFGameMovement::GrapplingHookMove()
{
	...
	else if(pPlayerToCheckForRune->GetPlayerClass()->GetClassIndex() == TF_CLASS_PYRO &&
			pPlayerToCheckForRune->m_Shared.InCond(TF_COND_GRAPPLED_TO_PLAYER))
	{
		mv->m_flMaxSpeed *= 0.7f;
	}

	// if the medic hook latched on to teammate, his movement should be slower to eventually detach from the healing
	// target this requires medic to do something instead of getting a free ride (except medic with AGILITY rune)
	if(m_pTFPlayer->GetPlayerClass()->GetClassIndex() == TF_CLASS_MEDIC && pHookTarget->IsPlayer() &&
	   pHookTarget->InSameTeam(m_pTFPlayer) && m_pTFPlayer->m_Shared.GetCarryingRuneType() != RUNE_AGILITY)
	{
	...
	if(tf_grapplinghook_use_acceleration.GetBool())
	{
		...
		mv->m_vecVelocity +=
			vDesiredMove.Normalized() * (tf_grapplinghook_acceleration.GetFloat() * gpGlobals->frametime);
	...
	if(flDistSqrToTarget < 10000)
	{
		...
		mv->m_vecVelocity =
			mv->m_vecVelocity.Normalized() * RemapValClamped(flDistSqrToTarget, 6400, 10000, 0.f, mv->m_flMaxSpeed);
	}
...
bool CTFGameMovement::StunMove()
{
	...
	else if(m_pTFPlayer->m_Shared.m_flLastMovementStunChange)
	{
		...
		float flCurStun = RemapValClamped((gpGlobals->curtime - m_pTFPlayer->m_Shared.m_flLastMovementStunChange), 0.2,
										  0.0, 0.0, 1.0);
		if(flCurStun)
		{
...
bool CTFGameMovement::TauntMove(void)
{
	...
	else if(m_pTFPlayer->m_Shared.InCond(TF_COND_TAUNTING) && m_pTFPlayer->CanMoveDuringTaunt())
	{
		...
		{
			m_pTFPlayer->SetCurrentTauntMoveSpeed(
				clamp(m_pTFPlayer->GetCurrentTauntMoveSpeed() +
						  flSign * (gpGlobals->frametime / flAcceleration) * flMaxMoveSpeed,
					  0.f, flMaxMoveSpeed));
		}
		else
		{
...
ConVar tf_halloween_kart_slowmoving_threshold("tf_halloween_kart_slowmoving_threshold", "300",
...
void CTFGameMovement::VehicleMove(void)
{
	...
	{
		// Grab normalized analog input (no need to check key input explicitly, since it's already baked into
		// m_flForwardMove
		float flNormalizedForwardInput =
			cl_forwardspeed.GetFloat() > 0.0f ? mv->m_flForwardMove / cl_forwardspeed.GetFloat() : 0.0f;
	...
	{
		// Grab normalized analog input (no need to check key input explicitly, since it's already baked into
		// m_flForwardMove. And flip the sign, since we're going backwards.
		float flNormalizedForwardInput =
			cl_backspeed.GetFloat() > 0.0f ? mv->m_flForwardMove / cl_backspeed.GetFloat() : 0.0f;
		if(flNormalizedForwardInput < -1.0f)
		{
			flNormalizedForwardInput = 1.0f;
		}
		...
		else
		{
			...
			if(mv->m_flOldForwardMove >= 0.0f || m_pTFPlayer->GetCurrentTauntMoveSpeed() < 0 ||
			   m_pTFPlayer->GetVehicleReverseTime() < gpGlobals->curtime)
			{
	...
	float flTargetMoveSpeed =
		Approach(flTargetSpeed, m_pTFPlayer->GetCurrentTauntMoveSpeed(), flAcceleration * gpGlobals->frametime);
	float flSmoothMoveSpeed = Bias(fabs(m_pTFPlayer->GetCurrentTauntMoveSpeed()) / flMaxMoveSpeed, 0.7f) *
							  flMaxMoveSpeed * Sign(flTargetMoveSpeed);
	...
	float flLeanAccel = flTargetSpeed > flSmoothMoveSpeed	? flAcceleration
						: flTargetSpeed < flSmoothMoveSpeed ? -flAcceleration
															: 0.f;
...
void CTFGameMovement::AirDash(void)
{
	...
	else
	{
		...
		{
			m_pTFPlayer->TakeDamage(CTakeDamageInfo(m_pTFPlayer, m_pTFPlayer, vec3_origin,
													m_pTFPlayer->WorldSpaceCenter(), 10.f, DMG_BULLET));
		}
#endif
	}
...
bool CTFGameMovement::CheckJumpButton()
{
	...
	if(m_pTFPlayer->GetGrapplingHookTarget())
	{
		...
		if(m_pTFPlayer->GetPlayerClass()->GetClassIndex() == TF_CLASS_HEAVYWEAPONS ||
		   (m_pTFPlayer->m_Shared.GetCarryingRuneType() != RUNE_NONE && m_pTFPlayer->HasTheFlag()))
		{
			mv->m_vecVelocity[2] *= 0.80f;
		}
		else if(m_pTFPlayer->m_Shared.GetCarryingRuneType() != RUNE_AGILITY &&
				m_pTFPlayer->m_Shared.GetCarryingRuneType() != RUNE_NONE && m_pTFPlayer->HasTheFlag())
		{
			mv->m_vecVelocity[2] *= 0.80f;
		}
	...
	if(tf_resolve_stuck_players.GetBool())
	{
...
#ifdef GAME_DLL
		if(TFGameRules() && TFGameRules()->IsMannVsMachineMode() && m_pTFPlayer &&
		   m_pTFPlayer->GetTeamNumber() == TF_TEAM_PVE_INVADERS)
		{
			if(traceresult.startsolid)
			{
				if(m_pTFPlayer->m_playerMovementStuckTimer.HasStarted() &&
				   m_pTFPlayer->m_playerMovementStuckTimer.IsElapsed())
				{
					DevMsg("%3.2f: A robot is interpenetrating a solid - killed!\n", gpGlobals->curtime);

					UTIL_LogPrintf("\"%s<%i><%s><%s>\" startsolid killed (position \"%3.2f %3.2f %3.2f\")\n",
								   m_pTFPlayer->GetPlayerName(), m_pTFPlayer->GetUserID(),
								   m_pTFPlayer->GetNetworkIDString(), m_pTFPlayer->GetTeam()->GetName(),
								   m_pTFPlayer->GetAbsOrigin().x, m_pTFPlayer->GetAbsOrigin().y,
								   m_pTFPlayer->GetAbsOrigin().z);

					m_pTFPlayer->TakeDamage(CTakeDamageInfo(m_pTFPlayer, m_pTFPlayer, vec3_origin,
															m_pTFPlayer->WorldSpaceCenter(), 999999.9f, DMG_CRUSH));
				}
				else
				{
			...
			{
				// Bot is *not* stuck right now. Continually restart timer, so if we become stuck it will count down and
				// expire.
		...
		if(traceresult.startsolid && traceresult.DidHitNonWorldEntity())
		{
			if(traceresult.m_pEnt->IsPlayer())
			{
				...
				TracePlayerBBox(originalPos, originalPos, PlayerSolidMask(), COLLISION_GROUP_PLAYER_MOVEMENT,
								traceresult);
			...
			else if(fabs(traceresult.m_pEnt->GetAbsVelocity().z) > 0.7071f &&
					FClassnameIs(traceresult.m_pEnt, "func_tracktrain"))
			{
...
void CTFGameMovement::CheckKartWallBumping()
{
	...
	if(fabs(flCurrentSpeed) > 100.f && (flCurrentSpeed - flClampedSpeed > 100.f))
	{
		...
		trace_t pWallTrace;
		UTIL_TraceLine(m_pTFPlayer->GetAbsOrigin(), m_pTFPlayer->GetAbsOrigin() + vAim * 64, MASK_SOLID, m_pTFPlayer,
					   COLLISION_GROUP_DEBRIS, &pWallTrace);

		// if we collide with a wall that is 90degrees or higher, bump backwards
		if(pWallTrace.fraction < 1.0 && !(pWallTrace.surface.flags & SURF_SKY) && pWallTrace.m_pEnt &&
		   !pWallTrace.m_pEnt->IsPlayer() && pWallTrace.plane.normal.z <= 0)
		{
#ifdef GAME_DLL
	...
	if(pEntity)
	{
		...
		else if(pEntity->IsPlayer())
		{
			if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
			{
				...
				if(bot &&
				   (bot->HasMission(CTFBot::MISSION_DESTROY_SENTRIES) || bot->HasMission(CTFBot::MISSION_REPROGRAMMED)))
				{
				...
				if(meBot && (meBot->HasMission(CTFBot::MISSION_DESTROY_SENTRIES) ||
							 meBot->HasMission(CTFBot::MISSION_REPROGRAMMED)))
				{
				...
				!pEntity->MyNextBotPointer()->GetLocomotionInterface()->ShouldCollideWith(pMe))
		{
			return false;
		}
#endif
...
//-----------------------------------------------------------------------------
void CTFGameMovement::TracePlayerBBox(const Vector &start, const Vector &end, unsigned int fMask, int collisionGroup,
									  trace_t &pm)
{
	if(tf_solidobjects.GetBool() == false)
...
void CTFGameMovement::CategorizePosition(void)
{
	...
	// NOTE YWB 7/5/07:  Since we're already doing a traceline here, we'll subsume the StayOnGround (stair debouncing)
	// check into the main traceline we do here to see what we're standing on
	bool bUnderwater = (player->GetWaterLevel() >= WL_Eyes);
	bool bMoveToEndPos = false;
	if(player->GetMoveType() == MOVETYPE_WALK && player->GetGroundEntity() != NULL && !bUnderwater)
	{
		// if walking and still think we're on ground, we'll extend trace down by stepsize so we don't bounce down
		// slopes
	...
	if(trace.plane.normal.z < 0.7f)
	{
		...
		TracePlayerBBoxForGround(vecStartPos, vecEndPos, GetPlayerMins(), GetPlayerMaxs(), mv->m_nPlayerHandle.Get(),
	...
	else
	{
		...
		if(bMoveToEndPos && !trace.startsolid && // not sure we need this check as fraction would == 0.0f?
...
void CTFGameMovement::FullWalkMove()
{
	...
	if(InWater() || (m_pTFPlayer && (m_pTFPlayer->m_Shared.InCond(TF_COND_HALLOWEEN_GHOST_MODE) ||
									 m_pTFPlayer->m_Shared.InCond(TF_COND_SWIMMING_NO_EFFECTS))))
	{
...
void CTFGameMovement::StepMove(Vector &vecDestination, trace_t &trace)
{
	...
	if(player->m_Local.m_bAllowAutoMovement)
	{
		...
		// If we are not on the standable ground any more or going the "high road" didn't move us at all, then we'll
		// also want to check the "low road"
		if((trace.fraction != 1.0f && trace.plane.normal[2] < 0.7) || VectorCompare(mv->GetAbsOrigin(), vecPos))
		{
			bLowRoad = true;
			bUpRoad = false;
		}
...
void CTFGameMovement::DuckOverrides()
{
	...
	if(((player->GetWaterLevel() >= WL_Feet) && !bOnGround) || player->GetWaterLevel() >= WL_Eyes)
	{
		...
		if((nButtonsReleased & IN_DUCK))
		{
			...
			else if(player->m_Local.m_bDucking && !player->m_Local.m_bDucked)
			{
				...
				player->m_Local.m_flDucktime =
					GAMEMOVEMENT_DUCK_TIME - unduckMilliseconds + remainingUnduckMilliseconds;
			}
		}
...
void CTFGameMovement::Duck(void)
{
	...
	// his view height is at the standing height.
			...
			!(TFGameRules() && TFGameRules()->ShowMatchSummary()))
	{
		...
		if((fabs(flOffsetDelta) > 0.1))
		{
			...
			DevMsg(1, "Restoring player view height at %i %0.3f.  Delta: %f.\n", gpGlobals->tickcount,
				   gpGlobals->curtime, flOffsetDelta);
	...
	if(tf_duck_debug_spew.GetBool())
	{
		...
		engine->Con_NPrintf(6, "viewoffset %3.2f, %3.2f, %3.2f", player->GetViewOffset().x, player->GetViewOffset().y,
							player->GetViewOffset().z);
		engine->Con_NPrintf(7, "IN_DUCK %d", mv->m_nButtons & IN_DUCK);
		engine->Con_NPrintf(8, "GetDuckTimer %3.2f",
							Max(0.f, m_pTFPlayer->m_Shared.GetDuckTimer() - gpGlobals->curtime));
#else
		...
		engine->Con_NPrintf(10 + 6, "viewoffset %3.2f, %3.2f, %3.2f", player->GetViewOffset().x,
							player->GetViewOffset().y, player->GetViewOffset().z);
		engine->Con_NPrintf(10 + 7, "IN_DUCK %d", mv->m_nButtons & IN_DUCK);
		engine->Con_NPrintf(10 + 8, "GetDuckTimer %3.2f",
							Max(0.f, m_pTFPlayer->m_Shared.GetDuckTimer() - gpGlobals->curtime));
#endif
	}
}
...
void CTFGameMovement::CheckForDoubleTap(void)
{
	...
	static const int aMoveType[4] = {
		IN_MOVELEFT, IN_MOVERIGHT, IN_FORWARD, IN_BACK,
		// Add movetypes here
	};
...
void CTFGameMovement::TeleportMove(Vector &vecDirection, float flDist)
{
	...
	UTIL_TraceHull(mv->GetAbsOrigin(), vecPos, VEC_HULL_MIN_SCALED(m_pTFPlayer), VEC_HULL_MAX_SCALED(m_pTFPlayer),
				   nMask, &traceFilter, &result);
	if(result.DidHit())
	{
		...
		// NDebugOverlay::SweptBox( mv->GetAbsOrigin(), vecPos, VEC_HULL_MIN_SCALED( m_pTFPlayer ), VEC_HULL_MAX_SCALED(
		// m_pTFPlayer ), m_pTFPlayer->GetAbsAngles(), 255, 0, 0, 40, 5.f );
	}
	...
	if(TFGameRules())
	{
		TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_PLAYER_SPELL_TELEPORT,
														   (m_pTFPlayer->GetTeamNumber() == TF_TEAM_RED) ? TF_TEAM_BLUE
																										 : TF_TEAM_RED);
	}
#endif // GAME_DLL
