...
const char *s_aPlayerClassFiles[] = {TF_CLASS_UNDEFINED_FILE,	 TF_CLASS_SCOUT_FILE,	TF_CLASS_SNIPER_FILE,
									 TF_CLASS_SOLDIER_FILE,		 TF_CLASS_DEMOMAN_FILE, TF_CLASS_MEDIC_FILE,
									 TF_CLASS_HEAVYWEAPONS_FILE, TF_CLASS_PYRO_FILE,	TF_CLASS_SPY_FILE,
									 TF_CLASS_ENGINEER_FILE,	 TF_CLASS_CIVILIAN_FILE};
...
void TFPlayerClassData_t::ParseData(KeyValues *pKeyValuesData)
{
	...
	// Death Sounds
	Q_strncpy(m_szDeathSound[DEATH_SOUND_GENERIC], pKeyValuesData->GetString("sound_death", "Player.Death"),
			  MAX_PLAYERCLASS_SOUND_LENGTH);
	Q_strncpy(m_szDeathSound[DEATH_SOUND_CRIT], pKeyValuesData->GetString("sound_crit_death", "TFPlayer.CritDeath"),
			  MAX_PLAYERCLASS_SOUND_LENGTH);
	Q_strncpy(m_szDeathSound[DEATH_SOUND_MELEE], pKeyValuesData->GetString("sound_melee_death", "Player.MeleeDeath"),
			  MAX_PLAYERCLASS_SOUND_LENGTH);
	Q_strncpy(m_szDeathSound[DEATH_SOUND_EXPLOSION],
			  pKeyValuesData->GetString("sound_explosion_death", "Player.ExplosionDeath"),
			  MAX_PLAYERCLASS_SOUND_LENGTH);
#endif
...
void TFPlayerClassData_t::AddAdditionalPlayerDeathSounds(void)
{
	...
	{
		CopySoundNameWithModifierToken(m_szDeathSound[i + DEATH_SOUND_MVM_FIRST], m_szDeathSound[i],
									   ARRAYSIZE(m_szDeathSound[0]), "MVM_");
		CopySoundNameWithModifierToken(m_szDeathSound[i + DEATH_SOUND_GIANT_MVM_FIRST], m_szDeathSound[i],
									   ARRAYSIZE(m_szDeathSound[0]), "M_MVM_");
	}
#endif
}
...
bool CTFPlayerClassDataMgr::Init(void)
{
	...
	Q_strncpy(pClassData->m_szModelName, "models/player/scout.mdl",
			  TF_NAME_LENGTH); // Undefined players still need a model
