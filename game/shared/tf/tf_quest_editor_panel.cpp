...
static const char *g_skQuestObjectivesConditionsDefFile =
	"tf/scripts/items/unencrypted/_items_quest_objective_definitions.txt";
...
static const char *g_skQuestPrefabs[] = {"quest_prefab_1st_operation_pauling", "quest_prefab_halloween_2015",
...
void WriteLocalizationData()
{
	...
	{
		CLocalizationEditorParam *pLocalizationEntry =
			static_cast<CLocalizationEditorParam *>(ILocalizationEditorParamAutoList::AutoList()[i]);
		...
		g_pVGuiLocalize->ConvertANSIToUnicode(pLocalizationEntry->GetLocalizationValue(), wszLocalizedValue,
											  sizeof(wszLocalizedValue));
...
// Purpose:
//-----------------------------------------------------------------------------
CEditorObjectNode::CEditorObjectNode(EditorObjectInitStruct init) : BaseClass(init)
{
...
const IEditableDataType *IEditorObject::GetOwningEditable() const
{
	...
	do
	{
		...
		pPanel = dynamic_cast<const IEditorObject *>(
			const_cast<IEditorObject *>(pPanel)->GetParent()); // Good lord, VGUI.  GetParent() isn't const?  Really?
...
IEditorObjectParameter::IEditorObjectParameter(EditorObjectInitStruct init, const char *pszLabelText) : BaseClass(init)
{
...
// Purpose:
//-----------------------------------------------------------------------------
CTextEntryEditorParam::CTextEntryEditorParam(EditorObjectInitStruct init, const char *pszLabelText,
											 const char *pszValue)
...
CLocalizationEditorParam::CLocalizationEditorParam(EditorObjectInitStruct init, const char *pszLabelText,
...
CComboBoxEditorParam::CComboBoxEditorParam(EditorObjectInitStruct init, const char *pszLabelText)
	: BaseClass(init, pszLabelText), m_pComboBox(NULL)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CComboBoxEditorParam::AddComboBoxEntry(const char *pszText, bool bSelected, const char *pszWriteValue,
...
void CNewQuestObjectiveParam::OnCommand(const char *command)
{
	if(FStrEq(command, "add"))
	{
		...
		if(pObjectiveParent)
		{
			...
			if(pNewCondition)
			{
				new CQuestObjectiveRestrictionNode(
					{GetParent(), CFmtStr("%d", pObjectiveParent->GetNextAvailableKeyNumber()), FLAGS_NONE},
					pNewCondition);
				InvalidateChain();
			}
		}
	  ...
	  m_pSelectObjectiveButton(NULL)
{
	...
	new CLocalizationEditorParam({this, "description_string", FLAGS_NONE},
								 "Objective Desc:", pKVObjective->GetString("description_string"));
	...
	CTextEntryEditorParam *pPointsNode = new CTextEntryEditorParam(
		{this, "points", FLAGS_NONE}, "Points:", CFmtStr("%d", pKVObjective->GetInt("points", 1)));
	pPointsNode->GetTextEntry()->SetAllowNumericInputOnly(true);

	// The condition def index this objective wants to use
	ObjectiveConditionDefIndex_t nConditionsDefIndex =
		pKVObjective->GetInt("conditions_def_index", INVALID_QUEST_OBJECTIVE_CONDITIONS_INDEX);
...
void CQuestObjectiveNode::PerformLayout()
{
	...
	m_pSelectObjectiveButton->SetPos(
		m_pDefIndexComboBox->GetComboBox()->GetXPos() + m_pDefIndexComboBox->GetComboBox()->GetWide(), 0);
...
void CQuestObjectiveNode::PopulateAndSelectConditionsCombobox(int nSelectedDefIndex)
{
	...
	FOR_EACH_VEC(vecEditableData, i)
	{
		...
		if(pEditable->GetType() == IEditableDataType::TYPE_OBJECTIVE_CONDITIONS &&
		   !g_pQuestEditor->IsOpenForEdit(pEditable))
		{
			KeyValues *pKVData = pEditable->GetLiveData();
			bool bSelected = atoi(pKVData->GetName()) == nSelectedDefIndex;
			m_pDefIndexComboBox->AddComboBoxEntry(CFmtStr("\t%s", pKVData->GetString("name", "ERROR")), bSelected,
												  pKVData->GetName(), NULL);
		}
	}
}
...
void CQuestDescriptionNode::CreateNewControl(KeyValues *pKV)
{
	...
	CLocalizationEditorParam *pLocParam =
		new CLocalizationEditorParam({pNode, "token", FLAGS_NONE}, "Description:", pKV->GetString("token"));
...
CComboBoxEditorParam *CreateWeaponComboBox(EditorObjectInitStruct init, const char *pszLabelText,
										   item_definition_index_t defIndexSelect)
{
	...
	FOR_EACH_MAP(mapItemDefs, i)
	{
		...
		if(!(iSlot == LOADOUT_POSITION_PRIMARY || iSlot == LOADOUT_POSITION_SECONDARY ||
			 iSlot == LOADOUT_POSITION_MELEE || iSlot == LOADOUT_POSITION_PDA || iSlot == LOADOUT_POSITION_PDA2 ||
...
void CRequiredItemsParam::CreateNewControl(KeyValues *pKV)
{
	...
	item_definition_index_t defIndexCurrent =
		(item_definition_index_t)pKV->GetInt("loaner_defindex", INVALID_ITEM_DEF_INDEX);
	CreateWeaponComboBox({pRoot, "loaner_defindex", FLAG_NOT_DELETABLE}, "Loaner Item:", defIndexCurrent);

	CQualifyingItemsParam *pQualifying =
		new CQualifyingItemsParam(EditorObjectInitStruct {pRoot, "qualifying_items", FLAG_NOT_DELETABLE});
...
// Purpose:
//-----------------------------------------------------------------------------
CQuestObjectiveRestrictionNode::CQuestObjectiveRestrictionNode(EditorObjectInitStruct init,
															   CTFQuestCondition *pCondition)
...
void CQuestObjectiveRestrictionNode::CreateControlsForCondition()
{
	...
	FOR_EACH_TRUE_SUBKEY(KVParams, params)
	{
		...
		{
			CComboBoxEditorParam *pNewParam =
				new CComboBoxEditorParam({this, params->GetName(), FLAGS_NONE}, pszLabelText);
			...
			FOR_EACH_TRUE_SUBKEY(params, pChoiceKey)
			{
				...
				pNewParam->AddComboBoxEntry(pszChoiceEnglishName ? pszChoiceEnglishName : pszChoiceName, bSelect,
											pszChoiceName, pszAction);
			}
	...
	: CEditorObjectNode(EditorObjectInitStruct {pParent, pKV->GetName(), FLAGS_NONE})
{
	...
	CQuestNameNode *pNameNode =
		new CQuestNameNode(EditorObjectInitStruct {pQuestNode, "names", FLAG_NOT_DELETABLE | FLAG_COLLAPSABLE});
	pNameNode->InitControls(pKVQuest->FindKey("names"));

	new CTextEntryEditorParam({pQuestNode, "max_standard_points", FLAGS_NONE},
							  "Max Standard Points:", pKVQuest->GetString("max_standard_points"));
	new CTextEntryEditorParam({pQuestNode, "max_bonus_points", FLAGS_NONE},
							  "Max Bonus Points:", pKVQuest->GetString("max_bonus_points"));
	new CTextEntryEditorParam({pQuestNode, "reward", FLAGS_NONE}, "Reward:", pKVQuest->GetString("reward"));
	new CTextEntryEditorParam({pQuestNode, "objectives_to_roll", FLAGS_NONE},
							  "Objectives to roll:", pKVQuest->GetString("objectives_to_roll"));
	...
	CComboBoxEditorParam *pCorrespondingOperation =
		new CComboBoxEditorParam({pQuestNode, "operation", FLAGS_NONE}, "Operation:");
	nChosenIndex = -1;
	{
		...
		FOR_EACH_MAP(mapOperations, i)
		{
			...
			bool bSelect = nChosenIndex == i || (nChosenIndex == -1 && i == 0);
			pCorrespondingOperation->AddComboBoxEntry(mapOperations[i]->GetName(), bSelect, mapOperations[i]->GetName(),
													  NULL);
		}
	}

	CQuestDescriptionNode *pDescNode = new CQuestDescriptionNode(
		EditorObjectInitStruct {pQuestNode, "descriptions", FLAG_NOT_DELETABLE | FLAG_COLLAPSABLE | FLAG_COLLAPSED});
	pDescNode->InitControls(pKVQuest->FindKey("descriptions"));

	CRequiredItemsParam *pRequiredExpandable = new CRequiredItemsParam(EditorObjectInitStruct {
		pQuestNode, "required_items", FLAG_NOT_DELETABLE | FLAG_COLLAPSABLE | FLAG_DONT_EXPORT});
	pRequiredExpandable->InitControls(pKVQuest->FindKey("required_items"));

	CObjectiveExpandable *pObjectiveExpandable = new CObjectiveExpandable(
		EditorObjectInitStruct {pQuestNode, "objectives", FLAG_NOT_DELETABLE | FLAG_COLLAPSABLE});
...
void IEditableDataType::UpdateButton()
{
	...
	if(pButton)
	{
		...
		{
			char szItemName[256];
			g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find(m_pKVLiveData->GetString("item_name")),
			...
			if(pKVObjectives)
			{
				FOR_EACH_TRUE_SUBKEY(pKVObjectives, pKVEntry)
				{
					...
					g_pVGuiLocalize->ConvertUnicodeToANSI(
						g_pVGuiLocalize->Find(pKVEntry->GetString("description_string")), szDesc, sizeof(szDesc));
					g_pFullFileSystem->FPrintf(fileHandle, "%d,%d,%d,%s,\n", pKVEntry->GetInt("DefIndex"),
											   pKVEntry->GetInt("points"), pKVEntry->GetInt("advanced"), szDesc);
				}
			}
		}
...
{
	CEditorObjectNode *pConditionDef =
		new CEditorObjectNode(EditorObjectInitStruct {pParent, m_pKVLiveData->GetName(), FLAG_HIDDEN});
	pConditionDef->SetOwningEditable(this);

	new CTextEntryEditorParam({pConditionDef, "name", FLAGS_NONE},
							  "Conditions name:", m_pKVLiveData->GetString("name"));

	CRequiredItemsParam *pRequiredExpandable = new CRequiredItemsParam(EditorObjectInitStruct {
		pConditionDef, "required_items", FLAG_NOT_DELETABLE | FLAG_COLLAPSABLE | FLAG_DONT_EXPORT});
...
CQuestEditorPanel::CQuestEditorPanel(Panel *pParent, const char *pszName)
	: Frame(pParent, pszName), m_pCurrentOpenEdit(NULL), m_eCurrentSelectionMode(SELECTION_MODE_NONE)
{
	...
	m_pButtonsContainers[IEditableDataType::TYPE_OBJECTIVE_CONDITIONS] =
		new CExScrollingEditablePanel(this, "QuestObjectiveConditionsContainer");
}
...
void CQuestEditorPanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	m_pButtonsFilterTextEntry[IEditableDataType::TYPE_OBJECTIVE_CONDITIONS] =
		FindControl<TextEntry>("ConditionsFilter");
...
void CQuestEditorPanel::OnCommand(const char *command)
{
	if(Q_strnicmp("select", command, 6) == 0)
	{
		...
		if(m_eCurrentSelectionMode == SELECTION_MODE_QUEST_OBJECTIVE &&
		   eType == IEditableDataType::TYPE_OBJECTIVE_CONDITIONS)
		{
	...
	else if(Q_stricmp("newquest", command) == 0)
	{
		...
		m_pCurrentOpenEdit = OpenForEdit(IEditableDataType::TYPE_QUEST,
										 CFmtStr("%s", pNewEditable->GetLiveData()->GetName()), m_pEditingPanel);
		ResetQuestSelectionState();
	}
	else if(Q_stricmp("newobjcond", command) == 0)
	{
		...
		m_pCurrentOpenEdit = OpenForEdit(IEditableDataType::TYPE_OBJECTIVE_CONDITIONS,
										 CFmtStr("%s", pNewEditable->GetLiveData()->GetName()), m_pEditingPanel);
		ResetQuestSelectionState();
	}
...
void CQuestEditorPanel::OnThink()
{
	FOR_EACH_VEC(m_vecOpenEdits, i)
	{
		engine->Con_NPrintf(i, CFmtStr("%s %s",
									   m_vecOpenEdits[i]->GetType() == IEditableDataType::TYPE_OBJECTIVE_CONDITIONS
										   ? "Condition"
										   : "Quest",
									   m_vecOpenEdits[i]->GetLiveData()->GetName()));
	}
	...
	FOR_EACH_VEC(m_vecEditableData, i)
	{
		if(m_vecEditableData[i]->MatchesCriteria(type, pszName))
		{
			...
			FOR_EACH_VEC(m_vecOpenEdits, j)
			{
				...
				{
					AssertMsg1(false, "Editable with name %s was already open for edit!",
							   m_vecEditableData[i]->GetLiveData()->GetName());
...
IEditableDataType *CQuestEditorPanel::AddNewEditableKVData(KeyValues *pKVData, const char *pszName)
{
	...
	Button *pButton =
		new Button(m_pButtonsContainers[eType], "objectivebutton", CFmtStr("%s: %s", pKVData->GetName(), pszName), this,
...
void CQuestEditorPanel::PopulateExistingQuests()
{
	...
	// Quest data
	{
		...
		{
			char szItemName[256];
			g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find(pKVQuest->GetString("item_name")), szItemName,
												  sizeof(szItemName));
...
IEditableDataType *CQuestEditorPanel::CreateNewQuest()
{
	...
	// Tool keys
	KeyValues *pKeyQuestBlock = pKVNewQuest->CreateNewKey();
	pKeyQuestBlock->SetName("quest");
	...
	KeyValues *pKeyObjectiveBlock = pKeyQuestBlock->CreateNewKey();
	pKeyObjectiveBlock->SetName("objectives");
