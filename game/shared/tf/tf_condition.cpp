...
void CTFConditionList::ServerThink()
{
	...
	{
		CTFCondition *cond = _conditions[i];
		if(cond->GetMaxDuration() > PERMANENT_CONDITION || cond->GetMinDuration() > PERMANENT_CONDITION)
		{
...
CTFCondition::CTFCondition(ETFCond type, float duration, CTFPlayer *outer, CBaseEntity *provider /*= NULL*/)
	: _type(type), _min_duration(0), _max_duration(duration), _outer(outer), _provider(provider)
{
}
...
void CTFCondition::Add(float duration)
{
	if(duration != PERMANENT_CONDITION)
	{
		...
		if(GetMaxDuration() == PERMANENT_CONDITION || duration < GetMaxDuration())
		{
...
// Crit Boost
//=============================================================================
CTFCondition_CritBoost::CTFCondition_CritBoost(ETFCond type, float duration, CTFPlayer *outer,
