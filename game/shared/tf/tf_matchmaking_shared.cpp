...
const char *s_pszMatchGroups[] = {
	"MatchGroup_MvM_Practice", "MatchGroup_MvM_MannUp",

	"MatchGroup_Ladder_6v6",   "MatchGroup_Ladder_9v9", "MatchGroup_Ladder_12v12",

	"MatchGroup_Casual_6v6",   "MatchGroup_Casual_9v9", "MatchGroup_Casual_12v12",
};
...
void On6v6MatchSizeChanged(IConVar *pConVar, const char *pOldString, float flOldValue)
{
	...
	{
		TFLobbyManager()->GetMatchmaker()->timeRemove6v6LadderGroupsExpire =
			CRTime::RTime32DateAdd(CRTime::RTime32TimeCur(), 10.f, k_ETimeUnitSecond);
	}
}
#endif
...
GCConVar tf_mm_match_size_ladder_12v12_minimum(
	"tf_mm_match_size_ladder_12v12_minimum", "12", FCVAR_MATCHSIZE_THING,
...
bool CCasualCriteriaHelper::IsMapInValidCategory(uint32 nMapDefIndex) const
{
	...
	FOR_EACH_VEC(pMap->m_vecAssociatedGameCategories, j)
	{
		const SchemaGameCategory_t *pCategory =
			GetItemSchema()->GetGameCategory(pMap->m_vecAssociatedGameCategories[j]);
		...
		if(pMMGroup->m_eMMGroup == kMatchmakingType_SpecialEvents || pMMGroup->m_eMMGroup == kMatchmakingType_Core ||
		   pMMGroup->m_eMMGroup == kMatchmakingType_Alternative)
		{
			return true;
		}
	}
