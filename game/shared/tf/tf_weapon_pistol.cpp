...
void CTFPistol_ScoutPrimary::Push(void)
{
	...
	for(int i = 0; i < enemyVector.Count(); ++i)
	{
		...
		if(bDebug)
		{
			NDebugOverlay::SweptBox(vecEyes, vecEyes + vecForward * flDist, -vHull, vHull, pOwner->EyeAngles(), 255, 0,
									0, 40, 5);
		}
		...
		if(trace.m_pEnt && trace.m_pEnt == pVictim && trace.fraction < 1.f)
		{
			...
			float flDamage = 1.f;
			CTakeDamageInfo info(pOwner, pVictim, this, flDamage, DMG_MELEE | DMG_NEVERGIB | DMG_CLUB,
								 TF_DMG_CUSTOM_NONE);
