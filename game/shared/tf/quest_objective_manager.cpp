...
CSOTrackerManager::SOTrackerMap_t::KeyType_t CQuestObjectiveManager::GetKeyForObjectTracker(const CSharedObject *pItem,
...
bool CQuestObjectiveManager::ShouldTrackObject(const CSteamID &steamIDOwner, const CSharedObject *pObject) const
{
	...
	if(pItemDef->GetQuestDef() == NULL)
	{
		SO_TRACKER_SPEW(CFmtStr("Not accepting item %llu with defindex %d.  It doesn't have a quest def.\n",
								pItem->GetID(), pItemDef->GetDefinitionIndex()),
						SO_TRACKER_SPEW_TRACKER_ACCEPTANCE);
		return false;
	}
...
int CQuestObjectiveManager::CompareRecords(const ::google::protobuf::Message *pNewProtoMsg,
										   const ::google::protobuf::Message *pExistingProtoMsg) const
{
	const CMsgGCQuestObjective_PointsChange *pNew =
		assert_cast<const CMsgGCQuestObjective_PointsChange *>(pNewProtoMsg);
	const CMsgGCQuestObjective_PointsChange *pExisting =
		assert_cast<const CMsgGCQuestObjective_PointsChange *>(pExistingProtoMsg);
...
void CQuestObjectiveManager::UpdateFromServer(itemid_t nID, uint32 nStandardPoints, uint32 nBonusPoints)
{
	...
	{
		SO_TRACKER_SPEW(CFmtStr("Got update from server, but itemID: %llu doesn't exist!", nID),
...
CFmtStr CQuestObjectiveManager::GetDebugObjectDescription(const CSharedObject *pSObject) const
{
	...
	return CFmtStr("%llu (%s)", pItem->GetItemID(),
				   pItem->GetItemDefinition()->GetQuestDef()->GetRolledNameForItem(pItem));
}

CBaseSOTracker *CQuestObjectiveManager::AllocateNewTracker(const CSharedObject *pItem, CSteamID steamIDOwner,
														   CSOTrackerManager *pManager) const
{
...
{
	const CMsgGCQuestObjective_PointsChange *pPointsChangeMsg =
		assert_cast<const CMsgGCQuestObjective_PointsChange *>(pProtoMsg);
	...
	{
		CQuestItemTracker *pItemTracker =
			assert_cast<CQuestItemTracker *>(GetTracker(pPointsChangeMsg->quest_item_id()));
		if(pItemTracker)
		{
...
CON_COMMAND(ensure_so_trackers_for_steamid,
			"Ensures a steamID has all the trackers it should have, with extra spew along the way")
{
	if(args.ArgC() != 2)
	{
...
GC_REG_JOB(GCSDK::CGCClient, CGCQuestObjective_PointsChangeResponse, "CGCQuestObjective_PointsChangeResponse",
		   k_EMsgGCQuestObjective_PointsChange, GCSDK::k_EServerTypeGCClient);
