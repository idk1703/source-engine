...
#define REGISTER_QUEST_CONDITION_SUB_CLASS(derivedClass, condName, nCondReqFields)  \
	QuestConditionEntry_t k_s##condName##RegisteredEntry(#condName, nCondReqFields, \
														 CreateCTFQuestConditionSubClass<derivedClass>);
...
bool IsValidServerForQuests(CSteamID steamIDQuestOwner)
{
	...
	bool bTrustedMatch =
		(eMatchGroup != k_nMatchGroup_Invalid) && GetMatchGroupDescription(eMatchGroup)->BIsTrustedServersOnly();
	if(!bTrustedMatch)
		return false;
...
void GetValidParamsKeyFromEvent(const char *pszKeyName, const char *pszRestrictionName, const char *pszEventName,
								KeyValues *pRequiredKeys)
{
	...
	if(pQuestEvents)
	{
		...
		if(pEvent)
		{
			...
			AssertMsg2(pRestriction, "Failed to find specified restriction name %s :: %s", pszEventName,
					   pszRestrictionName);
			if(pRestriction)
			{
				KeyValues *pParamsKey = pRestriction->FindKey(pszKeyName);
				AssertMsg3(pParamsKey, "Failed to find specified param key name %s :: %s :: %s", pszEventName,
						   pszRestrictionName, pszKeyName);
				if(pParamsKey)
				{
					if(pParamsKey->GetString("uses_method", NULL))
					{
						KeyValues *pKVMethod = pParamsKey->FindKey("method");
						AssertMsg3(pKVMethod, "Failed to find method block in %s :: %s :: %s", pszEventName,
								   pszRestrictionName, pszKeyName);
						if(pKVMethod)
						{
							const char *pszType = pKVMethod->GetString("type", NULL);
							AssertMsg3(pszType, "Missing method type in %s :: %s :: %s", pszEventName,
									   pszRestrictionName, pszKeyName);
							...
							else if(FStrEq(pszType, "weapon_name"))
							{
								...
								const CEconItemSchema::SortedItemDefinitionMap_t &mapItems =
									GetItemSchema()->GetSortedItemDefinitionMap();
								for(int it = mapItems.FirstInorder(); it != mapItems.InvalidIndex();
									it = mapItems.NextInorder(it))
								{
							...
							{
								AssertMsg5(false, "Type %s in %s :: %s :: %s didnt match any types defined in %s",
										   pszType, pszEventName, pszRestrictionName, pszKeyName, __FUNCTION__);
							}
						}
					}
					else
...
bool CTFQuestCondition::BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
	...
	// SCHEMA_INIT_CHECK( FStrEq( pszType, GetConditionName() ), "%s", CFmtStr( "Invalid quest restriction name '%s' for
	// '%s'", pszType, GetConditionName() ).Get() )
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		FOR_EACH_TRUE_SUBKEY(pKVItem, pSubKey)
		{
			...
			{
				SCHEMA_INIT_CHECK(nInputCount < GetMaxInputCount(), "%s",
								  CFmtStr("Too many input for operator '%s'. expected %d input(s)", GetConditionName(),
										  GetMaxInputCount())
									  .Get());
			}
			...
			SCHEMA_INIT_CHECK(
				pNewRestriction != NULL, "%s",
				CFmtStr("Failed to create quest restriction name '%s' for '%s'", pszType, GetConditionName()).Get());
		...
		SCHEMA_INIT_CHECK(
			nInputCount > 0 && nInputCount <= GetMaxInputCount(), "%s",
			CFmtStr("Invalid number of specified input. Expected from 0 to %d inputs.", GetMaxInputCount()).Get());
...
{
public:
	CTFGenericStringRestriction() : m_pszKeyName(NULL), m_pszValue(NULL) {}
...
{
public:
	CTFQuestBasePlayerRestriction() : m_pszPlayerKey(NULL), m_pszPlayerMethod(NULL) {}
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		static const char *s_pszValidGetMethod[] = {"by_id", "by_entindex", "by_cappers"};
...
protected:
	virtual bool BPlayerCheck(const CTFPlayer *pPlayer, IGameEvent *pEvent) const OVERRIDE
	{
		...
		if(pOwnerDisguiseTarget)
		{
			...
			if(m_eDisguiseState == DISGUISE_STATE_OWNER_IS_PLAYER && pOwnerDisguiseTarget == pPlayer &&
			   GetQuestOwner()->m_Shared.GetDisguiseClass() == pPlayer->GetPlayerClass()->GetClassIndex())
				return true;
		...
		{
			// must disguise as same class to look the same
			if(m_eDisguiseState == DISGUISE_STATE_PLAYER_IS_OWNER && pPlayerDisguiseTarget == GetQuestOwner() &&
			   pPlayer->m_Shared.GetDisguiseClass() == GetQuestOwner()->GetPlayerClass()->GetClassIndex())
				return true;
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		SCHEMA_INIT_CHECK(IsValidTFPlayerClass(m_iClass), "%s",
						  CFmtStr("Invalid owner class restriction '%s' for quest objective", pszClassName).Get());
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		SCHEMA_INIT_CHECK(
			m_eCondition != TF_COND_INVALID, "%s",
			CFmtStr("Invalid %s restriction '%s' for quest objective", GetConditionName(), pszConditionName).Get());
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		SCHEMA_INIT_CHECK(
			m_eObjectType != OBJ_LAST, "%s",
			CFmtStr("Invalid %s restriction '%s' for quest objective", GetConditionName(), pszObjectTypeName).Get());
	...
	virtual void GetRequiredParamKeys(KeyValues *pRequiredKeys) OVERRIDE
	{
		...
		GetValidParamsKeyFromEvent("object_key", CTFQuestPlayerObjectRestriction::GetConditionName(), m_pszEventName,
								   pRequiredKeys);
...
{
public:
	CTFQuestScorerRestriction() : m_pszScorerKey(NULL) {}
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		SCHEMA_INIT_CHECK(m_eWeaponType != TF_WEAPON_NONE, "%s",
						  CFmtStr("Invalid weapon restriction '%s' for quest objective", m_pszValue).Get());
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		SCHEMA_INIT_CHECK(m_eCustomDamageType != TF_DMG_CUSTOM_NONE, "%s",
						  CFmtStr("Invalid weapon restriction '%s' for quest objective", pszCustomDamageName).Get());
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		SCHEMA_INIT_CHECK(m_eEventType != TF_NUM_FLAG_EVENTS, "%s",
						  CFmtStr("Invalid CTF Event Type '%s' for quest objective", pszEventType).Get());
...
REGISTER_QUEST_CONDITION_SUB_CLASS(CTFQuestLoadoutPositionRestriction, loadout_position,
								   FIELD_PLAYER | FIELD_LOADOUT_POSITION);
...
{
public:
	CTFConditionQuestCondition() : m_pszKeyName(NULL) {}
...
public:
	...
	virtual bool BInitFromKV(KeyValues *pKVItem, CUtlVector<CUtlString> *pVecErrors /* = NULL */) OVERRIDE
	{
		...
		SCHEMA_INIT_CHECK(m_pszEventName != NULL, "%s",
						  CFmtStr("Invalid %s condition. Missing 'event_name'", GetConditionName()).Get());
		...
		FOR_EACH_TRUE_SUBKEY(pKVItem, pSubKey)
		{
			SCHEMA_INIT_CHECK(!m_pRestrictions, "%s",
							  CFmtStr("Too many input for operator '%s'.", GetConditionName()).Get());

			const char *pszType = pSubKey->GetString("type");
			m_pRestrictions = CreateRestrictionByName(pszType, this);
			SCHEMA_INIT_CHECK(
				m_pRestrictions != NULL, "%s",
				CFmtStr("Failed to create quest restriction name '%s' for '%s'", pszType, GetConditionName()).Get());
