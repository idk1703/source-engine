...
ConVar tf_flag_return_on_touch("tf_flag_return_on_touch", "0", FCVAR_REPLICATED,
							   "If this is set, your flag must be at base in order to capture the enemy flag. Remote "
							   "friendly flags return to your base instantly when you touch them");

#ifdef STAGING_ONLY
ConVar tf_flag_return_time_override(
	"tf_flag_return_time_override", "0", FCVAR_CHEAT | FCVAR_REPLICATED,
	"How long before a dropped flag will return (in seconds).  0 = Use map/default settings.  For internal-use only.",
	true, 0.f, false, 0.f);
#endif // STAGING_ONLY

ConVar tf_flag_return_time_credit_factor(
	"tf_flag_return_time_credit_factor", "1.0", FCVAR_REPLICATED,
	"Number of seconds the flag's return time will be credited for each second the flag is being carried.", true, 0.f,
	false, 0.f);
...
BEGIN_NETWORK_TABLE(CCaptureFlag, DT_CaptureFlag)
...
#ifdef GAME_DLL
	SendPropBool(SENDINFO(m_bDisabled)), SendPropBool(SENDINFO(m_bVisibleWhenDisabled)),
		SendPropInt(SENDINFO(m_nType), 5, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nFlagStatus), 3, SPROP_UNSIGNED),
		SendPropTime(SENDINFO(m_flResetTime)), SendPropTime(SENDINFO(m_flNeutralTime)),
		SendPropTime(SENDINFO(m_flMaxResetTime)), SendPropEHandle(SENDINFO(m_hPrevOwner)),
		SendPropString(SENDINFO(m_szModel)), SendPropString(SENDINFO(m_szHudIcon)),
		SendPropString(SENDINFO(m_szPaperEffect)), SendPropString(SENDINFO(m_szTrailEffect)),
		SendPropInt(SENDINFO(m_nUseTrailEffect), 3, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nPointValue)),
		SendPropFloat(SENDINFO(m_flAutoCapTime)), SendPropBool(SENDINFO(m_bGlowEnabled)),
	...
	RecvPropInt(RECVINFO(m_bDisabled), 0, RecvProxy_IsDisabled),
		...
		RecvPropInt(RECVINFO(m_nType)), RecvPropInt(RECVINFO(m_nFlagStatus), 0, RecvProxy_FlagStatus),
		RecvPropTime(RECVINFO(m_flResetTime)), RecvPropTime(RECVINFO(m_flNeutralTime)),
		RecvPropTime(RECVINFO(m_flMaxResetTime)), RecvPropEHandle(RECVINFO(m_hPrevOwner)),
		RecvPropString(RECVINFO(m_szModel)), RecvPropString(RECVINFO(m_szHudIcon)),
		RecvPropString(RECVINFO(m_szPaperEffect)), RecvPropString(RECVINFO(m_szTrailEffect)),
		RecvPropInt(RECVINFO(m_nUseTrailEffect)), RecvPropInt(RECVINFO(m_nPointValue)),
		RecvPropFloat(RECVINFO(m_flAutoCapTime)), RecvPropBool(RECVINFO(m_bGlowEnabled)),
...
BEGIN_DATADESC(CCaptureFlag)
	...
	// Keyfields.
	DEFINE_KEYFIELD(m_nType, FIELD_INTEGER, "GameType"), DEFINE_KEYFIELD(m_nReturnTime, FIELD_INTEGER, "ReturnTime"),
		...
		DEFINE_KEYFIELD(m_iszTrailEffect, FIELD_STRING, "flag_trail"), DEFINE_KEYFIELD(m_iszTags, FIELD_STRING, "tags"),
		...
		// Outputs.
		DEFINE_OUTPUT(m_outputOnReturn, "OnReturn"), DEFINE_OUTPUT(m_outputOnPickUp, "OnPickUp"),
		DEFINE_OUTPUT(m_outputOnPickUpTeam1, "OnPickupTeam1"), DEFINE_OUTPUT(m_outputOnPickUpTeam2, "OnPickupTeam2"),
		DEFINE_OUTPUT(m_outputOnDrop, "OnDrop"), DEFINE_OUTPUT(m_outputOnCapture, "OnCapture"),
		DEFINE_OUTPUT(m_OnCapTeam1, "OnCapTeam1"), DEFINE_OUTPUT(m_OnCapTeam2, "OnCapTeam2"),
...
{
	V_snprintf(pchName, nBuffSize, "%s_%s", ((m_szHudIcon[0] != '\0') ? (m_szHudIcon.Get()) : (TF_FLAG_ICON)),
...
bool CCaptureFlag::ShouldHideGlowEffect(void)
{
	...
	if(pLocalPlayer)
	{
		...
		else
		{
			...
			bIsHiddenTeam =
				(pLocalPlayer->GetTeamNumber() != TEAM_SPECTATOR && pLocalPlayer->GetTeamNumber() != GetTeamNumber());
		}
...
void CCaptureFlag::PlaySound(IRecipientFilter &filter, const char *pszString, int iTeam /*= TEAM_ANY */)
{
	...
	if(TFGameRules()->IsMannVsMachineMode())
	{
		...
		if(!(TFGameRules()->State_Get() == GR_STATE_RND_RUNNING &&
			 gpGlobals->curtime - TFGameRules()->GetLastRoundStateChangeTime() >= 5.0f))
		{
			return;
		}
	...
	else if(TFGameRules()->IsPlayingSpecialDeliveryMode() &&
			((V_strcmp(pszString, TF_RESOURCE_TEAM_DROPPED) == 0) ||
			 (V_strcmp(pszString, TF_RESOURCE_EVENT_TEAM_DROPPED) == 0)))
	{
...
int CCaptureFlag::GetReturnTimeShotClockMode(int nStartReturnTime)
{
	...
	if(IsFlagShotClockModePossible())
	{
		if(m_bUseShotClockMode)
		{
			...
			float flCreditTime =
				(gpGlobals->curtime - m_flLastPickupTime) * tf_flag_return_time_credit_factor.GetFloat() +
...
{
	CCaptureFlag *pFlag =
		static_cast<CCaptureFlag *>(CBaseEntity::CreateNoSpawn("item_teamflag", vecOrigin, vec3_angle, NULL));
...
void CCaptureFlag::Reset(void)
{
	...
	if(m_nType == TF_FLAGTYPE_ROBOT_DESTRUCTION && !IsDisabled())
	{
		if(m_nPointValue > 0)
		{
			...
			{
				CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints(GetTeamNumber(), m_nPointValue.Get(),
																				  SCORE_REACTOR_RETURNED, NULL);
		...
		SetDisabled(CTFRobotDestructionLogic::GetRobotDestructionLogic()->GetTargetScore(GetTeamNumber()) <
					tf_rd_min_points_to_steal.GetInt());
	}
...
void CCaptureFlag::FlagTouch(CBaseEntity *pOther)
{
	...
	if(pPlayer->m_Shared.IsStealthed() || pPlayer->m_Shared.InCond(TF_COND_STEALTHED_BLINK) ||
	   pPlayer->m_Shared.GetPercentInvisible() > 0.25f)
		return;
	...
	if(pPlayer->HasTheFlag() &&
	   !(m_nType == TF_FLAGTYPE_ROBOT_DESTRUCTION || m_nType == TF_FLAGTYPE_PLAYER_DESTRUCTION) &&
	...
	if(IsDropped() && (pOther->GetTeamNumber() == GetTeamNumber()) && (m_nType == TF_FLAGTYPE_CTF) &&
	   tf_flag_return_on_touch.GetBool())
	{
		Reset();
		ResetMessage();
...
void CCaptureFlag::PickUp(CTFPlayer *pPlayer, bool bInvisible)
{
	...
	else if(m_nType == TF_FLAGTYPE_RESOURCE_CONTROL)
	{
		...
		{
			bool bEventMap =
				TFGameRules() && TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_DOOMSDAY);
			if(bEventMap)
			{
	...
	if(TFGameRules()->IsMannVsMachineMode())
	{
		if(nOldFlagStatus == TF_FLAGINFO_HOME)
		{
			...
			{
				TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_GIANT_HAS_BOMB,
																   TF_TEAM_PVE_DEFENDERS);
			}
			else
			{
				TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_FIRST_BOMB_PICKUP,
																   TF_TEAM_PVE_DEFENDERS);
			}
		}
		else
...
void CCaptureFlag::Capture(CTFPlayer *pPlayer, int nCapturePoint)
{
	...
	if(m_nType == TF_FLAGTYPE_CTF)
	{
		...
		// don't play any sounds if this is going to win the round for one of the teams (victory sound will be played
		// instead)
	...
	else if(m_nType == TF_FLAGTYPE_RESOURCE_CONTROL)
	{
		if(TFGameRules() && TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_DOOMSDAY))
		{
			TFGameRules()->BroadcastSound(255, (pPlayer->GetTeamNumber() == TF_TEAM_RED)
												   ? TF_RESOURCE_EVENT_RED_CAPPED
	...
	else if(m_nType == TF_FLAGTYPE_ROBOT_DESTRUCTION)
	{
		...
		if(CTFRobotDestructionLogic::GetRobotDestructionLogic())
		{
			CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints(
				pPlayer->GetTeamNumber(), m_nPointValue.Get(), SCORE_REACTOR_CAPTURED, pPlayer);
			CTFRobotDestructionLogic::GetRobotDestructionLogic()->FlagDestroyed(GetTeamNumber());
			m_nPointValue = 0;
		}
	}
...
void CCaptureFlag::Drop(CTFPlayer *pPlayer, bool bVisible, bool bThrown /*= false*/, bool bMessage /*= true*/)
{
	...
	vecEnd.z -= 8000.0f;
	trace_t trace;
	UTIL_TraceHull(vecStart, vecEnd, WorldAlignMins(), WorldAlignMaxs(), MASK_SOLID, this, COLLISION_GROUP_DEBRIS,
				   &trace);
		...
		for(int z = 0; z < ARRAYSIZE(fMultipliers) && trace.startsolid; z++)
		{
			for(int y = 0; y < ARRAYSIZE(fMultipliers) && trace.startsolid; y++)
			{
				for(int x = 0; x < ARRAYSIZE(fMultipliers) && trace.startsolid; x++)
				{
					...
					vecEnd = vecStart;
					vecEnd.z -= 8000.0f;
					UTIL_TraceHull(vecStart, vecEnd, WorldAlignMins(), WorldAlignMaxs(), MASK_SOLID, this,
	...
	if(TFGameRules()->IsMannVsMachineMode())
	{
		...
		if(isBombInBadPlace)
		{
			...
			nearExtent.lo.z = MIN_COORD_FLOAT; // make sure we catch all areas under flag, even it is way up in the air
											   // for some reason
...
void CCaptureFlag::Think(void)
{
	...
	if(IsDropped())
	{
		...
		if(TFGameRules()->IsMannVsMachineMode() && m_bReturnBetweenWaves)
		{
			...
			else if(g_pPopulationManager && g_pPopulationManager->IsInEndlessWaves() &&
					g_pPopulationManager->EndlessShouldResetFlag())
			{
				Reset();
	...
	if(IsStolen() && TFGameRules() && TFGameRules()->IsPowerupMode() && IsPoisonous() &&
	   !pPlayer->m_Shared.InCond(TF_COND_MARKEDFORDEATH))
	{
...
void CCaptureFlag::CreateReturnIcon(void)
{
	...
	CBaseEntity *pReturnIcon = CBaseEntity::Create(
		"item_teamflag_return_icon", GetAbsOrigin() + Vector(0, 0, cl_flag_return_height.GetFloat()), vec3_angle, this);
	if(pReturnIcon)
	{
...
void CCaptureFlag::Simulate(void)
{
	...
	C_TFPlayer *pLocalPlayer = C_TFPlayer::GetLocalTFPlayer();
	if(m_hPrevOwner && m_hPrevOwner->IsPlayer() && pLocalPlayer && pLocalPlayer->m_Shared.IsFullyInvisible() &&
...
int CCaptureFlag::GetMaxReturnTime(void)
{
	...
	if(m_nType == TF_FLAGTYPE_ROBOT_DESTRUCTION)
	{
		...
		nReturnTime = RemapValClamped(m_nPointValue, 0.f, nMaxReturnTimePoints, tf_rd_return_min_time.GetFloat(),
									  tf_rd_return_max_time.GetFloat());
	}
	else if(m_nType == TF_FLAGTYPE_PLAYER_DESTRUCTION)
	{
		if(CTFPlayerDestructionLogic::GetPlayerDestructionLogic())
...
void CCaptureFlag::AddPointValue(int nPoints)
{
	...
	if(m_nType == TF_FLAGTYPE_ROBOT_DESTRUCTION || m_nType == TF_FLAGTYPE_PLAYER_DESTRUCTION)
	{
		...
		if(pEvent)
		{
			pEvent->SetInt("userid",
						   m_hPrevOwner && m_hPrevOwner->IsPlayer() ? ToBasePlayer(m_hPrevOwner)->GetUserID() : -1);
			pEvent->SetInt("entindex", entindex());
			gameeventmanager->FireEvent(pEvent);
		...
		if(nPoints > 0)
		{
			...
			if(pEvent)
			{
				pEvent->SetInt("player",
							   m_hPrevOwner && m_hPrevOwner->IsPlayer() ? ToBasePlayer(m_hPrevOwner)->entindex() : -1);
...
progress_segment_t Segments[8] = {
	{0.125, 0.5, 0.0, 1.0, 0.0, 1, 0},	{0.25, 1.0, 0.0, 1.0, 0.5, 0, 1},	{0.375, 1.0, 0.5, 1.0, 1.0, 0, 1},
	{0.50, 1.0, 1.0, 0.5, 1.0, -1, 0},	{0.625, 0.5, 1.0, 0.0, 1.0, -1, 0}, {0.75, 0.0, 1.0, 0.0, 0.5, 0, -1},
	...
	for(i = 0; i < 8; i++)
	{
		if(flProgress < Segments[i].maxProgress)
		{
			...
			float internalProgress =
				RemapVal(flProgress, Segments[i].maxProgress - 0.125, Segments[i].maxProgress, 0.0, 1.0);
			...
			meshBuilder_Full.Position3fv(
				(vOrigin + (vRight * (Segments[i].vert2x - 0.5)) + (vUp * (Segments[i].vert2y - 0.5))).Base());
			meshBuilder_Full.AdvanceVertex();
