...
#ifdef CLIENT_DLL
	RecvPropVector(RECVINFO(m_vInitialVelocity)), RecvPropEHandle(RECVINFO(m_hLauncher))
#else
...
// Purpose:
//-----------------------------------------------------------------------------
CTFBaseProjectile *CTFBaseProjectile::Create(const char *pszClassname, const Vector &vecOrigin, const QAngle &vecAngles,
											 CBaseEntity *pOwner, float flVelocity, short iProjModelIndex,
											 const char *pszDispatchEffect, CBaseEntity *pScorer, bool bCritical,
											 Vector vColor1, Vector vColor2)
{
	...
	if(pszDispatchEffect)
	{
		...
		CTraceFilterIgnoreFriendlyCombatItems traceFilterCombatItem(pOwner, COLLISION_GROUP_NONE,
		...
		UTIL_TraceLine(vecOrigin, vecOrigin + vecForward * MAX_COORD_RANGE,
					   (CONTENTS_SOLID | CONTENTS_MOVEABLE | CONTENTS_WINDOW | CONTENTS_GRATE), &traceFilterChain, &tr);
...
// Purpose:
//-----------------------------------------------------------------------------
C_LocalTempEntity *ClientsideProjectileCallback(const CEffectData &data, float flGravityBase,
												const char *pszParticleName)
{
	...
	if(pEnt && pEnt->IsPlayer())
	{
		...
		else
		{
			...
			if(pViewModel)
			{
				...
				trace_t trace;
				UTIL_TraceLine(vecMuzzleOrigin + vForward * -50, vecMuzzleOrigin, MASK_SOLID, pEnt,
	...
	return tempents->ClientProjectile(vecSrc, data.m_vStart, vecGravity, data.m_nMaterial, data.m_fFlags, pEnt,
									  "Impact", pszParticleName);
}
