...
#ifdef GAME_DLL
	: m_nClip(0), m_nAmmo(0), m_nDetonated(0), m_flEnergy(0.f), m_flEffectBarRegenTime(0.f)
#endif
{
#ifdef CLIENT_DLL
	m_pGlowEffect = NULL;
...
#ifdef GAME_DLL
	SetContextThink(&CTFDroppedWeapon::SUB_Remove, gpGlobals->curtime + tf_dropped_weapon_lifetime.GetFloat(),
					"RemoveThink");
#endif // GAME_DLL
}
		...
		if(m_Item.GetItemID() != INVALID_ITEM_ID)
		{
			...
			if(iStrangeType != -1)
			{
				...
				if(m_Item.FindAttribute(pAttr_module, &attrModule) && attrModule.has_value())
				{
					...
					if(pStatTrakEnt && pStatTrakEnt->InitializeAsClientEntity("models/weapons/c_models/stattrack.mdl",
																			  RENDER_GROUP_OPAQUE_ENTITY))
					{
						...
						//	//{
						//	//	pStatTrakEnt->SetBodygroup( 0, 1 ); // use a special mirror-image stattrak module that
						//appears correct for lefties
						//	//}
...
bool CTFDroppedWeapon::OnInternalDrawModel(ClientModelRenderInfo_t *pInfo)
{
	...
	for(int i = 0; i < m_vecAttachedModels.Size(); i++)
	{
		...
		if(attachedModel.m_pModel && (attachedModel.m_iModelDisplayFlags & kAttachedModelDisplayFlag_WorldModel))
		{
			...
			DoInternalDrawModel(&infoAttached, (bMarkAsDrawn && (infoAttached.flags & STUDIO_RENDER)) ? &state : NULL,
								pBoneToWorld);
		}
	}
...
void CTFDroppedWeapon::SetupParticleEffect()
{
	...
	if(m_effect)
	{
		for(int i = 0; i < ARRAYSIZE(pParticleSystem->pszControlPoints); ++i)
		{
			...
			{
				ParticleProp()->AddControlPoint(m_effect, i, this, PATTACH_POINT_FOLLOW,
												pParticleSystem->pszControlPoints[i]);
			}
		}
	}
}
...
void CTFDroppedWeapon::ClientThink()
{
	...
	bool bShouldGlowForLocalPlayer = pTFPlayer && pTFPlayer->IsAlive() && pTFPlayer->CanPickupDroppedWeapon(this) &&
									 pTFPlayer->IsLineOfSightClear(this);
...
//-----------------------------------------------------------------------------
CTFDroppedWeapon *CTFDroppedWeapon::Create(CTFPlayer *pLastOwner, const Vector &vecOrigin, const QAngle &vecAngles,
										   const char *pszModelName, const CEconItemView *pItem)
{
	...
	for(int i = 0; i < CTFDroppedWeapon::AutoList().Count(); ++i)
	{
		...
		if(pDroppedWeapon->m_Item.GetItemID() == pItem->GetItemID() &&
		   pDroppedWeapon->m_Item.GetItemDefIndex() == pItem->GetItemDefIndex() &&
		   pDroppedWeapon->m_hPlayer.Get() == pLastOwner)
		{
	...
	CTFDroppedWeapon *pDroppedWeapon =
		static_cast<CTFDroppedWeapon *>(CBaseAnimating::CreateNoSpawn("tf_dropped_weapon", vecOrigin, vecAngles));
	if(pDroppedWeapon)
	{
...
//-----------------------------------------------------------------------------
void CTFDroppedWeapon::InitDroppedWeapon(CTFPlayer *pPlayer, CTFWeaponBase *pWeapon, bool bSwap,
										 bool bIsSuicide /*= false*/)
{
	...
	else
	{
		...
		if(pMedigun)
		{
			...
			{
				SetContextThink(&CTFDroppedWeapon::ChargeLevelDegradeThink, gpGlobals->curtime + 0.1f,
								"ChargeLevelDegradeThink");
			}
		}
	}
