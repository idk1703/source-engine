...
public:
	float flDamage;
	float flDamageCritScaleMultiplier; // scale the damage by this amount when taking it into consideration for "should
									   // I crit?" calculations
	float flTime;
	int nDamageType;
	byte nKills;
...
public:
	...
	int GetDisguiseTargetIndex(void) const
	{
		return m_iDisguiseTargetIndex;
	}
	...
	CTFWeaponBase *GetDisguiseWeapon(void)
	{
	...
	void Heal(CBaseEntity *pHealer, float flAmount, float flOverhealBonus, float flOverhealDecayMult,
	...
	void MakeBleed(CTFPlayer *pPlayer, CTFWeaponBase *pWeapon, float flBleedingTime, int nBleedDmg = TF_BLEEDING_DMG,
				   bool bPermanentBleeding = false);
#ifdef GAME_DLL
	...
	{
		return GetPercentInvisible() == 1.f || InCond(TF_COND_STEALTHED_PHASE);
	}
#else
	bool IsFullyInvisible(void)
	{
	...
	float GetSpaceJumpChargeMeter() const
	{
		return m_flSpaceJumpCharge;
	}
	void SetSpaceJumpChargeMeter(float val)
	{
		m_flSpaceJumpCharge = Min(val, 100.0f);
	}
...
#ifdef GAME_DLL
	void StunPlayer(float flTime, float flReductionAmount, int iStunFlags = TF_STUN_MOVEMENT,
					CTFPlayer *pAttacker = NULL);
#endif // GAME_DLL
...
private:
	...
	void SetChargeEffect(medigun_charge_types iCharge, bool bState, bool bInstant,
						 const struct MedigunEffects_t &effects, float flWearOffTime, CTFPlayer *pProvider = NULL);
