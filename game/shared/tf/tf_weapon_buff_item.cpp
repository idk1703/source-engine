...
const char *BannerModels[] = {"models/weapons/c_models/c_buffbanner/c_buffbanner.mdl",
#ifdef STAGING_ONLY
...
void CTFBuffItem::BlowHorn(void)
{
	...
	if(!pPlayer->IsBot())
	{
		EconEntity_OnOwnerKillEaterEventNoPartner(dynamic_cast<CEconEntity *>(this), pPlayer,
												  kKillEaterEvent_BannersDeployed);
	}
#endif
...
void CTFBuffItem::FireGameEvent(IGameEvent *event)
{
	...
	{
		CTFPlayer *pPlayer =
			ToTFPlayer(ClientEntityList().GetEnt(engine->GetPlayerForUserID(event->GetInt("buff_owner"))));
		if(pPlayer == GetOwner())
		{
			CreateBanner();
		}
