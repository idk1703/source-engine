...
//-----------------------------------------------------------------------------
bool CTFRatingData::BYieldingUpdateRatingAndAddToTransaction(CSQLAccess &transaction, const MMRatingData_t &newData,
															 EMMRatingSource eAdjustmentSource,
															 uint64_t unSourceEventID, RTime32 rtTimestamp)
{
	// MMRatingData_t fields
	ASSERT_LAST_FIELD(unRatingTertiary);
	...
	// Add to transaction
	return transaction.BYieldingInsertOrUpdateOnPK(&schRatingData) && transaction.BYieldingInsertRecord(&schHistory);
}
