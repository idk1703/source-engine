...
void CTFPumpkinBomb::Spawn()
{
	...
	if(m_iTeam != TF_TEAM_HALLOWEEN)
	{
		...
		m_nSkin = m_iTeam == TF_TEAM_BLUE
					  ? 2
					  : 1; // This is actually opposite of what you'd think so Blue team can Hurt Blue Pumpkins
...
void CTFPumpkinBomb::RemovePumpkin()
{
	...
	TE_TFParticleEffect(filter, 0.0,
						m_iTeam == TF_TEAM_RED ? "spell_pumpkin_mirv_goop_red" : "spell_pumpkin_mirv_goop_blue",
						GetAbsOrigin(), vec3_angle);
	UTIL_Remove(this);
#endif // GAME_DLL
}
//-----------------------------------------------------------------------------
...
void CTFPumpkinBomb::Event_Killed(const CTakeDamageInfo &info)
{
	...
	CTakeDamageInfo damage_info(this, info.GetAttacker(), NULL, m_flDamage,
								DMG_BLAST | DMG_HALF_FALLOFF | DMG_NOCLOSEDISTANCEMOD);
