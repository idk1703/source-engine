...
void CMannVsMachineUpgradeManager::ParseUpgradeBlockForUIGroup(KeyValues *pKV, int iDefaultUIGroup)
{
	...
	for(KeyValues *pData = pKV->GetFirstSubKey(); pData != NULL; pData = pData->GetNextKey())
	{
		...
		if(pSchema)
		{
			...
			const CEconItemAttributeDefinition *pAttr =
				pSchema->GetAttributeDefinitionByName(m_Upgrades[index].szAttrib);
			if(!pAttr)
			{
...
void CMannVsMachineUpgradeManager::LoadUpgradesFileFromPath(const char *pszPath)
{
	...
	if(V_strstr(pszPath, "..") || V_strstr(pszPath, " ") || V_strstr(pszPath, "\r") || V_strstr(pszPath, "\n") ||
	   V_strstr(pszPath, ":") || V_strstr(pszPath, "\\\\") || V_IsAbsolutePath(pszPath) || pszExtension == NULL ||
	   V_strcmp(pszExtension, "txt") != 0)
	{
		return;
	}
...
int GetUpgradeStepData(CTFPlayer *pPlayer, int nWeaponSlot, int nUpgradeIndex, int &nCurrentStep, bool &bOverCap)
{
	...
	CEconItemAttributeDefinition *pAttribDef =
		ItemSystem()->GetStaticDataForAttributeByName(pMannVsMachineUpgrade->szAttrib);
	if(!pAttribDef)
		return 0;
	...
	if(pPowerupBottle)
	{
		...
		nCurrentStep = ::FindAttribute(pItemData, pAttribDef) ? pPowerupBottle->GetNumCharges() : 0;
	...
	bool bPercentage =
		nFormat == ATTDESCFORM_VALUE_IS_PERCENTAGE || nFormat == ATTDESCFORM_VALUE_IS_INVERTED_PERCENTAGE;
	...
	{
		::FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pPlayer->GetAttributeList(), pAttribDef,
														  &flCurrentAttribValue);
	}
	else
	{
