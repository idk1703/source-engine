		...
		if(pContainer)
		{
			...
			if(bIsEntryVisible)
			{
				...
				if(pProgressBar)
				{
					...
					pProgressBar->SetTooltip(
						m_pToolTip, CFmtStr("%d / %d", leaderboardEntry->m_nScore % DUCK_XP_SCALE, DUCK_XP_SCALE));
				}
...
void CDucksLeaderboardManager::ApplySettings(KeyValues *inResourceData)
{
	...
	if(pBackgroundPanel)
	{
		...
		CDucksLeaderboard *pLeaderboard =
			new CDucksLeaderboard(pBackgroundPanel, "DuckLeaderboard", g_szDuckLeaderboardNames[eShowLeaderboard]);
			...
			if(pScoresContainer)
			{
				...
				if(pScoreEntryKVs)
				{
					for(int i = 1; i < DUCK_NUM_LEADERBOARDS; ++i)
					{
						...
						pNewEntry->SetDialogVariable(
							"name", g_pVGuiLocalize->Find(CFmtStr("#%s", g_szDuckLeaderboardNames[i])));
						pNewEntry->SetTooltip(m_pToolTip, CFmtStr("#%s_Desc", g_szDuckLeaderboardNames[i]));
						pNewEntry->SetVisible(true);
						pNewEntry->SetPos(0, m_iScoreStep *
												 i); // This is off by 1, but that's what we want.  It starts at 1, but
													 // we want it to start lower so it matches the leaderboard entries
