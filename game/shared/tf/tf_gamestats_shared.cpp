...
//			flStartTime -
//-----------------------------------------------------------------------------
void TF_Gamestats_LevelStats_t::Init(const char *pszMapName, int nMapRevision, int nIPAddr, short nPort,
									 float flStartTime)
{
	Q_memset(&m_Header, 0,
			 sizeof(m_Header)); // TODO: This is correct for steamworks stats, but probably breaks old stats!!!
...
void TFReportedStats_t::AppendCustomDataToSaveBuffer(CUtlBuffer &SaveBuffer)
{
	...
	for(int iMap = m_dictMapStats.First(); iMap != m_dictMapStats.InvalidIndex(); iMap = m_dictMapStats.Next(iMap))
	{
		...
		// Write out the lumps.
		CBaseGameStats::AppendLump(MAX_LUMP_COUNT, SaveBuffer, TFSTATS_LUMP_MAPHEADER, 1,
								   sizeof(TF_Gamestats_LevelStats_t::LevelHeader_t),
								   static_cast<void *>(&pCurrentMap->m_Header));
		// CBaseGameStats::AppendLump( MAX_LUMP_COUNT, SaveBuffer, TFSTATS_LUMP_MAPDEATH,
		// pCurrentMap->m_aPlayerDeaths.Count(), sizeof( TF_Gamestats_LevelStats_t::PlayerDeathsLump_t ),
		// static_cast<void*>( pCurrentMap->m_aPlayerDeaths.Base() ) ); CBaseGameStats::AppendLump( MAX_LUMP_COUNT,
		// SaveBuffer, TFSTATS_LUMP_MAPDAMAGE, pCurrentMap->m_aPlayerDamage.Count(), sizeof(
		// TF_Gamestats_LevelStats_t::PlayerDamageLump_t ), static_cast<void*>( pCurrentMap->m_aPlayerDamage.Base() ) );
		CBaseGameStats::AppendLump(MAX_LUMP_COUNT, SaveBuffer, TFSTATS_LUMP_CLASS,
								   ARRAYSIZE(pCurrentMap->m_aClassStats), sizeof(pCurrentMap->m_aClassStats[0]),
								   static_cast<void *>(pCurrentMap->m_aClassStats));
		CBaseGameStats::AppendLump(MAX_LUMP_COUNT, SaveBuffer, TFSTATS_LUMP_WEAPON,
								   ARRAYSIZE(pCurrentMap->m_aWeaponStats), sizeof(pCurrentMap->m_aWeaponStats[0]),
								   static_cast<void *>(pCurrentMap->m_aWeaponStats));
	}

	// Append an end tag to verify we've reached end of file and data was sane.  (Sometimes we receive stat files that
	// start sane but become filled with garbage partway through.)
	...
	while(CBaseGameStats::GetLumpHeader(MAX_LUMP_COUNT, LoadBuffer, iLump, iLumpCount))
	{
		switch(iLump)
		{
			case TFSTATS_LUMP_MAPHEADER:
			{
				...
				CBaseGameStats::LoadLump(LoadBuffer, iLumpCount, sizeof(TF_Gamestats_LevelStats_t::LevelHeader_t),
										 &header);

				// quick sanity check on some data -- we get some stat files that start out OK but are corrupted later
				// in the file
			...
			case TFSTATS_LUMP_MAPDEATH:
			{
				...
				// CBaseGameStats::LoadLump( LoadBuffer, iLumpCount, sizeof(
				// TF_Gamestats_LevelStats_t::PlayerDeathsLump_t ), static_cast<void*>( playerDeaths.Base() ) ); if (
				// pCurrentGame )
				//{
			...
			case TFSTATS_LUMP_MAPDAMAGE:
			{
				...
				// CBaseGameStats::LoadLump( LoadBuffer, iLumpCount, sizeof(
				// TF_Gamestats_LevelStats_t::PlayerDamageLump_t ), static_cast<void*>( playerDamage.Base() ) ); if (
				// pCurrentGame )
				//{
			...
			case TFSTATS_LUMP_CLASS:
			{
				...
				{
					CBaseGameStats::LoadLump(LoadBuffer, ARRAYSIZE(pCurrentGame->m_aClassStats),
											 sizeof(pCurrentGame->m_aClassStats[0]), pCurrentGame->m_aClassStats);

					// quick sanity check on some data -- we get some stat files that start out OK but are corrupted
					// later in the file
					for(int i = 0; i < ARRAYSIZE(pCurrentGame->m_aClassStats); i++)
					{
						TF_Gamestats_ClassStats_t &classStats = pCurrentGame->m_aClassStats[i];
						if((classStats.iSpawns < 0) || (classStats.iSpawns > 10000) || (classStats.iTotalTime < 0) ||
						   (classStats.iTotalTime > 36000 * 20) || (classStats.iKills < 0) ||
						   (classStats.iKills > 10000))
						{
							return false;
						}
					}
				...
				{
					// mismatched lump size, possibly from different build, don't know how it interpret it, just skip
					// over it
					return false;
				}
				break;
			...
			case TFSTATS_LUMP_WEAPON:
			{
				...
				{
					CBaseGameStats::LoadLump(LoadBuffer, ARRAYSIZE(pCurrentGame->m_aWeaponStats),
											 sizeof(pCurrentGame->m_aWeaponStats[0]), pCurrentGame->m_aWeaponStats);

					// quick sanity check on some data -- we get some stat files that start out OK but are corrupted
					// later in the file
					if((pCurrentGame->m_aWeaponStats[TF_WEAPON_MEDIGUN].iShotsFired < 0) ||
					   (pCurrentGame->m_aWeaponStats[TF_WEAPON_MEDIGUN].iShotsFired > 100000) ||
					   (pCurrentGame->m_aWeaponStats[TF_WEAPON_FLAMETHROWER_ROCKET].iShotsFired !=
						0)) // check that unused weapon has 0 shots
					{
						return false;
					}
				}
...
const char *g_aRoundEndReasons[] = {
	"round_end",		   "client_disconnect",	 "client_quit",		  "server_map_change",
	"server_shutdown",	   "time_limit_reached", "win_limit_reached", "win_diff_limit_reached",
	"round_limit_reached", "next_level_cvar",
};
