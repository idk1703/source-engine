...
ConVar tf_grapplinghook_projectile_speed("tf_grapplinghook_projectile_speed", "1500", FCVAR_REPLICATED | FCVAR_CHEAT,
										 "How fast does the grappliing hook projectile travel");
ConVar tf_grapplinghook_max_distance("tf_grapplinghook_max_distance", "2000", FCVAR_REPLICATED | FCVAR_CHEAT,
...
void CTFGrapplingHook::PrimaryAttack(void)
{
	...
	trace_t tr;
	UTIL_TraceLine(vecSrc, vecSrc + tf_grapplinghook_max_distance.GetFloat() * vecForward, MASK_SOLID, pOwner,
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFGrapplingHook::GetProjectileFireSetup(CTFPlayer *pPlayer, Vector vecOffset, Vector *vecSrc, QAngle *angForward,
											  bool bHitTeammates /*= true*/, float flEndDist /*= 2000.f*/)
{
...
bool CTFGrapplingHook::SendWeaponAnim(int actBase)
{
	...
	else if(pPlayer->GetGrapplingHookTarget())
	{
		...
		else
		{
			...
			if(GetActivity() == ACT_GRAPPLE_PULL_START && m_startPullingTimer.HasStarted() &&
			   !m_startPullingTimer.IsElapsed())
				return true;
	...
	else if(actBase == ACT_VM_PRIMARYATTACK)
	{
		...
		else
		{
			...
			if(GetActivity() == ACT_GRAPPLE_FIRE_START && m_startFiringTimer.HasStarted() &&
			   !m_startFiringTimer.IsElapsed())
				return true;
	...
	if(pOwner && pOwner->m_Shared.IsRuneCharged())
	{
		...
		if(type == RUNE_SUPERNOVA)
		{
			...
			for(int i = 0; i < vecPlayers.Count(); ++i)
			{
				...
				if(flDistSqr <= flEffectRadiusSqr &&
				   pOwner->IsLineOfSightClear(pOther, CBaseCombatCharacter::IGNORE_ACTORS) &&
				   !PointInRespawnRoom(pOther, pOther->WorldSpaceCenter()))
				{
			...
			for(int i = 0; i < vecVictims.Count(); ++i)
			{
				...
				const char *pszEffect = pOwner->GetTeamNumber() == TF_TEAM_RED ? "powerup_supernova_strike_red"
				...
				TE_TFParticleEffectComplex(filter, 0.f, pszEffect, vStart, QAngle(0.f, 0.f, 0.f), NULL, &controlPoint,
										   pOther, PATTACH_CUSTOMORIGIN);
			...
			{
				// play effect
				const char *pszEffect = pOwner->GetTeamNumber() == TF_TEAM_RED ? "powerup_supernova_explode_red"
...
void CEquipGrapplingHookNotification::Accept()
{
	...
	if(iItemId == INVALID_ITEM_ID)
	{
		...
		CEconItemView *pDefaultGrapple = TFInventoryManager()->GetBaseItemForClass(
			pLocalPlayer->GetPlayerClass()->GetClassIndex(), LOADOUT_POSITION_ACTION);
		if(pDefaultGrapple)
		{
...
void CEquipGrapplingHookNotification::UpdateTick()
{
	...
	if(pLocalPlayer)
	{
		CTFGrapplingHook *pGrapplingHook =
			dynamic_cast<CTFGrapplingHook *>(pLocalPlayer->Weapon_OwnsThisID(TF_WEAPON_GRAPPLINGHOOK));
		if(pGrapplingHook)
		{
			MarkForDeletion();
		}
