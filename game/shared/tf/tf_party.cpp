...
GCConVar tf_mm_force_competitive_access_update(
	"tf_mm_force_competitive_access_update", "0", 0,
	"If set, force a update of party member's competitive status every time we update the party. Expensive, and would "
	"only wallpaper issues with the competitive acccess system.");
...
void CTFParty::SpewDebug()
{
	...
	Msg("  State: %d  Started matchmaking: %s (%d seconds ago, now is %s)\n", (int)GetState(), time.Render(time_buf),
		CRTime::RTime32TimeCur() - GetStartedMatchmakingTime(), now.Render(now_buf));
	// Msg( "  GameMode: %u\n", Obj().game_mode() );
...
void CTFParty::SetStateInternal(CSOTFParty_State newState)
{
	EmitInfo(SPEW_GC, MATCHMAKING_SPEWLEVEL4, LOG_ALWAYS, "Party %016llx SetState(%s)\n", Obj().party_id(),
			 CSOTFParty_State_Name(newState).c_str());
	...
	if(newState == CSOTFParty_State_UI)
	{
		if(!Obj().has_wizard_step() || Obj().wizard_step() == TF_Matchmaking_WizardStep_SEARCHING)
		{
			switch(GetMatchmakingMode())
			{
				...
				default:
					AssertMsg2(false, "Party %016llx invalid matchmaking mode %d", Obj().party_id(),
							   (int)GetMatchmakingMode());
			}
		}
	}
	else
...
void CTFParty::SetUIStateAndWizardStep(TF_Matchmaking_WizardStep eWizardStep)
{
	...
	{
		default:
			AssertMsg2(false, "Party %016llx in invalid matchmaking mode %d", Obj().party_id(),
					   (int)GetMatchmakingMode());
		case TF_Matchmaking_MVM:
#ifdef USE_MVM_TOUR
	...
	while(i < Obj().search_mvm_missions_size())
	{
	   ...
	   // check if mission is valid if we're in mannup
		...
		else if(Obj().search_mvm_missions_size() == 1)
		{
			...
			Obj().set_search_mvm_missions(0, "invalid"); // make sure we don't have a real mission name hanging around
														 // that might later actually get used.
		}
		else
		{
			bKeep = false;
...
void CTFParty::YldUpdateMemberData(int nMemberIndex)
{
	...
	if(!pSOCache)
	{
		AssertMsg2(false, "Party %016llx contains member %s, but we cannot load his SO cache?", GetGroupID(),
				   steamIDMember.Render());
		return;
	}
	...
	if(pUserSession && IsLadderGroup(GetMatchGroup()))
	{
		...
		bCompetitiveAccess =
			pSOCache->GetGameAccountClient() && pSOCache->GetGameAccountClient()->Obj().competitive_access();
	}
	else if(!pUserSession)
	{
		EmitInfo(SPEW_GC, MATCHMAKING_SPEWLEVEL4, LOG_ALWAYS,
				 "CTFParty::YldUpdateMemberData: Player %s is in party %016llx, but has no session",
				 steamIDMember.Render(), GetGroupID());
		// Assume ineligible
	}
	...
	{
		EmitInfo(SPEW_GC, MATCHMAKING_SPEWLEVEL4, LOG_ALWAYS,
				 "Party %016llx Member[%s].set_squad_surplus(0) --- no voucher in inventory\n", Obj().party_id(),
				 steamIDMember.Render());
	...
	if(idxTour >= 0)
	{
		...
		{
			EmitInfo(SPEW_GC, MATCHMAKING_SPEWLEVEL4, LOG_ALWAYS,
					 "Party %016llx Member[%s].set_completed_missions(0x%X)\n", Obj().party_id(),
					 steamIDMember.Render(), nMissionCompletedMask);
		...
		if(pGameAccountClient)
		{
			...
			switch(ePenaltyPool)
			{
				case eMMPenaltyPool_Casual:
					memberPreventDate =
						(bIgnoreAbandoned) ? 0 : pGameAccountClient->Obj().matchmaking_casual_ban_expiration();
					memberLowPriorityTime =
						bIgnoreAbandoned ? 0 : pGameAccountClient->Obj().matchmaking_casual_low_priority_expiration();
					break;

				case eMMPenaltyPool_Ranked:
					memberPreventDate =
						(bIgnoreAbandoned) ? 0 : pGameAccountClient->Obj().matchmaking_ranked_ban_expiration();
					memberLowPriorityTime =
						bIgnoreAbandoned ? 0 : pGameAccountClient->Obj().matchmaking_ranked_low_priority_expiration();
					break;
			...
			{
				EmitInfo(SPEW_GC, MATCHMAKING_SPEWLEVEL4, LOG_ALWAYS, "Party %016llx Member[%s].set_is_banned(%d)\n",
			...
			{
				EmitInfo(SPEW_GC, MATCHMAKING_SPEWLEVEL4, LOG_ALWAYS,
						 "Party %016llx Member[%s].set_is_low_priority(%d)\n", Obj().party_id(), GetMember(i).Render(),
						 bIsLowPriority ? 1 : 0);
...
void CTFPartyInvite::YldInitFromPlayerGroup(GCSDK::IPlayerGroup *pPlayerGroup)
{
	...
	for(int i = 0; i < nMembers; i++)
	{
		...
		//			CTFGameAccountClient *pPlayerSummary = static_cast<CTFGameAccountClient*>( pCache->FindSharedObject(
		//iSharedObject ) ); 			if ( pPlayerSummary )
		//			{
...
void CTFPartyInvite::AddMember(const CSteamID &steamID, const char *szPersonaName, uint16 unAvatar)
{
	EmitInfo(SPEW_GC, MATCHMAKING_SPEWLEVEL4, LOG_ALWAYS, "PartyInvite %016llx AddMember(%s, %s, %u)\n", GetGroupID(),
			 steamID.Render(), szPersonaName ? szPersonaName : "", unAvatar);
