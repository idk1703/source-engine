...
BEGIN_NETWORK_TABLE(CTFKnife, DT_TFWeaponKnife)
	...
	RecvPropBool(RECVINFO(m_bReadyToBackstab)), RecvPropBool(RECVINFO(m_bKnifeExists)),
		RecvPropFloat(RECVINFO(m_flKnifeRegenerateDuration)), RecvPropFloat(RECVINFO(m_flKnifeMeltTimestamp)),
#else
	SendPropBool(SENDINFO(m_bReadyToBackstab)), SendPropBool(SENDINFO(m_bKnifeExists)),
	...
	if(iMeltsInFire > 0 && info.GetDamageType() & DMG_BURN)
	{
		if(m_bKnifeExists)
		{
			...
			if(pPlayer)
			{
				...
				// Set flag to allow holstering during this forced switch (holstering might otherwise have been
				// inhibited by being blocked). This addresses a corner-case where a Spy stabbing a Razorback-equipped
				// sniper with the Spycicle and then immediately burned doesn't switch away and could backstab
				// immediately even though their knife should have melted.
