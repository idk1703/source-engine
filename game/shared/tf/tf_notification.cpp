...
CTFNotification::CTFNotification() : m_eLocalizedToLanguage(k_Lang_None)
{
...
bool CTFNotification::BLocalize(ELanguage eLang)
{
	...
	if(!pLocalized)
	{
		EmitError(
			SPEW_GC,
			"Notification has localized token \"%s\" that was not found in primary language %s *or* fallback to %s.\n",
			m_strUnlocalizedString.Get(), GetLanguageShortName(eLang), GetLanguageShortName(eFallback));
...
bool CTFNotification::BYieldingAddWriteToTransaction(GCSDK::CSQLAccess &sqlAccess, const CUtlVector<int> &fields)
{
	...
	FOR_EACH_VEC(fields, nField)
	{
		...
		{
			case CMsgGCNotification::kNotificationIdFieldNumber:
				csDatabaseDirty.BAddColumn(CSchNotification::k_iField_ulNotificationID);
				break;
			case CMsgGCNotification::kAccountIdFieldNumber:
				csDatabaseDirty.BAddColumn(CSchNotification::k_iField_unAccountID);
				break;
			case CMsgGCNotification::kExpirationTimeFieldNumber:
				csDatabaseDirty.BAddColumn(CSchNotification::k_iField_RTime32Expiration);
				break;
			case CMsgGCNotification::kTypeFieldNumber:
				csDatabaseDirty.BAddColumn(CSchNotification::k_iField_unNotificationType);
				break;
			case CMsgGCNotification::kNotificationStringFieldNumber:
				csDatabaseDirty.BAddColumn(CSchNotification::k_iField_VarCharData);
				break;
			default:
				Assert(false);
		}
