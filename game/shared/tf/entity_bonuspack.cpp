...
bool CBonusPack::MyTouch(CBasePlayer *pPlayer)
{
	if(ValidTouch(pPlayer) && gpGlobals->curtime >= m_flCanPickupTime)
	{
		...
		const char *pszParticleName =
			pPlayer->GetTeamNumber() == TF_TEAM_RED ? TF_POWERCORE_RED_PICKUP : TF_POWERCORE_BLUE_PICKUP;
		...
		if(CTFRobotDestructionLogic::GetRobotDestructionLogic())
		{
			CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints(
				GetTeamNumber(), tf_bonuspack_score.GetInt(), SCORE_CORES_COLLECTED, ToTFPlayer(pPlayer));
		}
