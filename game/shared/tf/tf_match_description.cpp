...
#ifdef GC_DLL
GCConVar tf_mm_ladder_force_map_by_name(
	"tf_mm_ladder_force_map_by_name", "",
	"If specified, force any matches that form for 6v6, 9v9, 12v12 to use the specified map (e.g. cp_sunshine).");
GCConVar tf_mm_casual_rejoin_cooldown_secs(
	"tf_mm_casual_rejoin_cooldown_secs", "180",
	"How many seconds must pass before anyone can re-match into a casual or MvM lobby they have left. "
	"Setting this too low may allow someone to rejoin a lobby while a vote-kick for them is still occuring, but before "
	"it has passed.");
GCConVar tf_mm_casual_rejoin_cooldown_votekick_secs(
...
static bool BCheckLeftMatchMembersAgainstParty(const MatchDescription_t *pMatch, const MatchParty_t *pParty,
...
static bool BThreadedFormerMemberMayRejoinJoinCasualOrMvMMatch(const CTFMemcachedLobbyFormerMember &member)
{
	...
	if(unRejoinTimeout && rtLeftLobby != 0 && rtLeftLobby + unRejoinTimeout > rtNow)
	{
	...
	if(unVoteKickTimeout && rtLeftMatch != 0 &&
	   (eLeftMatchReason == TFMatchLeaveReason_VOTE_KICK || eLeftMatchReason == TFMatchLeaveReason_ADMIN_KICK) &&
...
int IMatchGroupDescription::GetServerPoolIndex(EMatchGroup eGroup, EMMServerMode eMode) const
{
	...
	switch(eMode)
	{
		...
		{
			COMPILE_TIME_ASSERT((int)k_nMatchGroup_MvM_Practice + (int)k_nGameServerPool_Full_First ==
								(int)k_nGameServerPool_MvM_Practice_Full);
		...
		{
			COMPILE_TIME_ASSERT((int)k_nMatchGroup_MvM_Practice + (int)k_nGameServerPool_Incomplete_Match_First ==
								(int)k_nGameServerPool_MvM_Practice_Incomplete_Match);
...
ConVar tf_test_pvp_rank_xp_change("tf_test_pvp_rank_xp_change", "-1", 0, "Force your experience to a specific value",
								  cc_tf_test_pvp_rank_xp_change);

CON_COMMAND_F(tf_progression_set_xp_to_level, "Overrides your XP to be within the range the level specified",
			  FCVAR_CHEAT)
{
	if(args.ArgC() != 3)
	{
...
const XPSourceDef_t g_XPSourceDefs[] = {
	{"MatchMaking.XPChime", "TF_XPSource_PositiveFormat", "#TF_XPSource_Score",
	 flTargetXPPM *flScoreXPScale / flAvgPPMPP /* 6.5  */} // SOURCE_SCORE = 0;
	,
	{"MatchMaking.XPChime", "TF_XPSource_PositiveFormat", "#TF_XPSource_ObjectiveBonus",
	 flTargetXPPM *flObjectiveXPScale / flAvgPPMPM /* 0.0926  */} // SOURCE_OBJECTIVE_BONUS = 1;
	,
	{"MatchMaking.XPChime", "TF_XPSource_PositiveFormat", "#TF_XPSource_CompletedMatch",
	 flTargetXPPM *flMatchCompletionXPScale / flAvgPPMPM /* 0.185 */} // SOURCE_COMPLETED_MATCH = 2;
	,
	...
	,
	{"MatchMaking.XPChime", "TF_XPSource_PositiveFormat", "#TF_XPSource_Autobalance_Bonus",
	 1.f}}; // SOURCE_AUTOBALANCE_BONUS = 6;

IProgressionDesc::IProgressionDesc(EMatchGroup eMatchGroup, const char *pszBadgeName, const char *pszProgressionResFile,
...
public:
	CMvMMatchGroupDescription(EMatchGroup eMatchGroup, const char *pszConfig, bool bTrustedOnly)
		: IMatchGroupDescription(eMatchGroup, {eMatchMode_MatchMaker_LateJoinDropIn // m_eLateJoinMode;
											   ,
	...
	virtual void SelectModeSpecificParameters(const MatchDescription_t *pMatch, CTFLobby *pLobby) const OVERRIDE
	{
		...
		if(pMatch->m_eMatchGroup == k_nMatchGroup_MvM_MannUp)
		{
			...
			pLobby->SetMannUpTourName(
				GetItemSchema()->GetMvmTours()[pMatch->m_iMannUpTourOfDuty].m_sTourInternalName.Get());
		}
		else
		{
	...
	virtual bool BThreadedPartyCompatibleWithMatch(const MatchDescription_t *pMatch,
												   const MatchParty_t *pCandidateParty) const OVERRIDE
	{
		// Check for blacklisted former members that tank compatibility
		if(!BCheckLeftMatchMembersAgainstParty(pMatch, pCandidateParty,
											   &BThreadedFormerMemberMayRejoinJoinCasualOrMvMMatch))
		{
			return false;
		}
	...
	virtual bool BThreadedPartiesCompatible(const MatchParty_t *pLeftParty,
											const MatchParty_t *pRightParty) const OVERRIDE
	{
#ifdef USE_MVM_TOUR
	...
	virtual void GetServerDetails(const CMsgGameServerMatchmakingStatus &msg, int &nChallengeIndex,
								  const char *pszMap) const OVERRIDE
	{
		if(msg.matchmaking_state() == ServerMatchmakingState_EMPTY) // if we're empty, we can switch the challenge, so
	...
	virtual void Dump(const char *pszLeader, int nSpewLevel, int nLogLevel, const MatchParty_t *pMatch) const OVERRIDE
	{
		...
		EmitInfo(SPEW_GC, nSpewLevel, nLogLevel, "%s  MvM Type: %s    Search Pop: %s\n", pszLeader,
				 (m_eMatchGroup == k_nMatchGroup_MvM_MannUp
					  ? "Mann Up"
					  : (m_eMatchGroup == k_nMatchGroup_MvM_Practice ? "Bootcamp" : "UNKNOWN")),
				 sSelectedPops.String());
		EmitInfo(SPEW_GC, nSpewLevel, nLogLevel, "%s  Best valve data center ping: %.0fms\n", pszLeader,
				 pMatch->m_flPingClosestServer);
	}
#endif
		...
		switch(TFGameRules()->State_Get())
		{
			...
			case GR_STATE_RND_RUNNING:
				if(TFObjectiveResource() && !TFObjectiveResource()->GetMannVsMachineIsBetweenWaves() &&
				   TFObjectiveResource()->GetMannVsMachineWaveCount() ==
					   TFObjectiveResource()->GetMannVsMachineMaxWaveCount())
				{
	...
	bool BMatchIsSafeToLeaveForPlayer(const CMatchInfo *pMatchInfo,
									  const CMatchInfo::PlayerMatchData_t *pMatchPlayer) const
	{
		bool bSafe = false;
...
public:
	...
	public:
		CLadderProgressionDesc(EMatchGroup eMatchGroup)
			: IProgressionDesc(eMatchGroup, "models/vgui/competitive_badge.mdl", "resource/ui/PvPCompRankPanel.res",
							   "TF_Competitive_Rank")
		{
			// Bucket 1
			m_vecLevels.AddToTail({1, k_unDrilloRating_Ladder_Min, 11500, "competitive/competitive_badge_rank001",
								   "#TF_Competitive_Rank_1", "MatchMaking.RankOneAchieved",
								   "competitive/comp_background_tier001a"});
			m_vecLevels.AddToTail({2, m_vecLevels.Tail().m_nEndXP, 13000, "competitive/competitive_badge_rank002",
								   "#TF_Competitive_Rank_2", "MatchMaking.RankOneAchieved",
								   "competitive/comp_background_tier001a"});
			m_vecLevels.AddToTail({3, m_vecLevels.Tail().m_nEndXP, 14500, "competitive/competitive_badge_rank003",
								   "#TF_Competitive_Rank_3", "MatchMaking.RankOneAchieved",
								   "competitive/comp_background_tier001a"});
			m_vecLevels.AddToTail({4, m_vecLevels.Tail().m_nEndXP, 16000, "competitive/competitive_badge_rank004",
								   "#TF_Competitive_Rank_4", "MatchMaking.RankOneAchieved",
								   "competitive/comp_background_tier002a"});
			m_vecLevels.AddToTail({5, m_vecLevels.Tail().m_nEndXP, 17500, "competitive/competitive_badge_rank005",
								   "#TF_Competitive_Rank_5", "MatchMaking.RankOneAchieved",
								   "competitive/comp_background_tier002a"});
			m_vecLevels.AddToTail({6, m_vecLevels.Tail().m_nEndXP, 19500, "competitive/competitive_badge_rank006",
								   "#TF_Competitive_Rank_6", "MatchMaking.RankOneAchieved",
								   "competitive/comp_background_tier002a"});
			// Bucket 2
			m_vecLevels.AddToTail({7, m_vecLevels.Tail().m_nEndXP, 21500, "competitive/competitive_badge_rank007",
								   "#TF_Competitive_Rank_7", "MatchMaking.RankTwoAchieved",
								   "competitive/comp_background_tier003a"});
			m_vecLevels.AddToTail({8, m_vecLevels.Tail().m_nEndXP, 23500, "competitive/competitive_badge_rank008",
								   "#TF_Competitive_Rank_8", "MatchMaking.RankTwoAchieved",
								   "competitive/comp_background_tier003a"});
			m_vecLevels.AddToTail({9, m_vecLevels.Tail().m_nEndXP, 25500, "competitive/competitive_badge_rank009",
								   "#TF_Competitive_Rank_9", "MatchMaking.RankTwoAchieved",
								   "competitive/comp_background_tier003a"});
			m_vecLevels.AddToTail({10, m_vecLevels.Tail().m_nEndXP, 28000, "competitive/competitive_badge_rank010",
								   "#TF_Competitive_Rank_10", "MatchMaking.RankTwoAchieved",
								   "competitive/comp_background_tier004a"});
			m_vecLevels.AddToTail({11, m_vecLevels.Tail().m_nEndXP, 30500, "competitive/competitive_badge_rank011",
								   "#TF_Competitive_Rank_11", "MatchMaking.RankTwoAchieved",
								   "competitive/comp_background_tier004a"});
			m_vecLevels.AddToTail({12, m_vecLevels.Tail().m_nEndXP, 33000, "competitive/competitive_badge_rank012",
								   "#TF_Competitive_Rank_12", "MatchMaking.RankTwoAchieved",
								   "competitive/comp_background_tier004a"});
			// Bucket 3
			m_vecLevels.AddToTail({13, m_vecLevels.Tail().m_nEndXP, 35500, "competitive/competitive_badge_rank013",
								   "#TF_Competitive_Rank_13", "MatchMaking.RankThreeAchieved",
								   "competitive/comp_background_tier005a"});
			m_vecLevels.AddToTail({14, m_vecLevels.Tail().m_nEndXP, 38000, "competitive/competitive_badge_rank014",
								   "#TF_Competitive_Rank_14", "MatchMaking.RankThreeAchieved",
								   "competitive/comp_background_tier005a"});
			m_vecLevels.AddToTail({15, m_vecLevels.Tail().m_nEndXP, 40500, "competitive/competitive_badge_rank015",
								   "#TF_Competitive_Rank_15", "MatchMaking.RankThreeAchieved",
								   "competitive/comp_background_tier005a"});
			m_vecLevels.AddToTail({16, m_vecLevels.Tail().m_nEndXP, 43500, "competitive/competitive_badge_rank016",
								   "#TF_Competitive_Rank_16", "MatchMaking.RankThreeAchieved",
								   "competitive/comp_background_tier006a"});
			m_vecLevels.AddToTail({17, m_vecLevels.Tail().m_nEndXP, 46500, "competitive/competitive_badge_rank017",
								   "#TF_Competitive_Rank_17", "MatchMaking.RankThreeAchieved",
								   "competitive/comp_background_tier006a"});
			// Bucket 4
			m_vecLevels.AddToTail({18, m_vecLevels.Tail().m_nEndXP, 50000, "competitive/competitive_badge_rank018",
								   "#TF_Competitive_Rank_18", "MatchMaking.RankFourAchieved",
								   "competitive/comp_background_tier006a"});
		}
		...
		const LevelInfo_t &GetLevelForExperience(uint32 nExperience) const OVERRIDE
		{
			...
			return IProgressionDesc::GetLevelForExperience(nExperience == 0 ? k_unDrilloRating_Ladder_Min
																			: nExperience);
		}
...
#endif
			CTFRatingData *pRating = CTFRatingData::YieldingGetPlayerRatingDataBySteamID(
				steamapicontext->SteamUser()->GetSteamID(), k_nMMRating_6v6_DRILLO_PlayerAcknowledged);
...
#error Make this call a yielding call if you are removing it from client ifdefs
#endif
			CTFRatingData *pRating = CTFRatingData::YieldingGetPlayerRatingDataBySteamID(
				steamapicontext->SteamUser()->GetSteamID(), k_nMMRating_6v6_DRILLO);
														  ...
														  CTFSharedObjectCache *pLockedSOCache) const OVERRIDE
		{
			...
			MMRatingData_t ackedRatingData =
				pLockedSOCache->GetPlayerRatingData(k_nMMRating_6v6_DRILLO_PlayerAcknowledged);
			...
			return pLockedSOCache->BYieldingUpdatePlayerRating(transaction, k_nMMRating_6v6_DRILLO_PlayerAcknowledged,
															   k_nMMRatingSource_PlayerAcknowledge, 0, ratingData);
		}
		...
		virtual void DebugSpewLevels() const OVERRIDE
		{
			...
			{
				Msg("Level %d:\t%d - %d. (+%d)\n", m_vecLevels[i].m_nLevelNum, m_vecLevels[i].m_nStartXP,
					m_vecLevels[i].m_nEndXP, (m_vecLevels[i].m_nEndXP - m_vecLevels[i].m_nStartXP));
			}
		}
#endif
	};
	...
	CLadderMatchGroupDescription(EMatchGroup eMatchGroup, ConVar *pmm_match_group_size)
		: IMatchGroupDescription(eMatchGroup, {eMatchMode_MatchMaker_LateJoinMatchBased // m_eLateJoinMode;
											   ,
	...
	virtual void SelectModeSpecificParameters(const MatchDescription_t *pMatch, CTFLobby *pLobby) const OVERRIDE
	{
		...
		const char *pszMap = (*tf_mm_ladder_force_map_by_name.GetString()) ? tf_mm_ladder_force_map_by_name.GetString()
																		   : pCategory->GetRandomMap()->pszMapName;
	...
	virtual bool BThreadedPartyCompatibleWithMatch(const MatchDescription_t *pMatch,
												   const MatchParty_t *pCurrentParty) const OVERRIDE
	{
	...
	virtual void InitGameRulesSettingsPostEntity() const OVERRIDE
	{
		...
		bool bMultiStagePLR = (tf_gamemode_payload.GetBool() && pMaster && pMaster->PlayingMiniRounds() &&
							   TFGameRules()->HasMultipleTrains());
		bool bUseStopWatch = TFGameRules()->MatchmakingShouldUseStopwatchMode();
		bool bCTF = tf_gamemode_ctf.GetBool();
		bool bHighSkill = GTFGCClientSystem()->GetMatch() &&
						  GTFGCClientSystem()->GetMatch()->m_uAverageRank >= k_unDrilloRating_Ladder_HighSkill;

		// Exec our match settings
		const char *pszExecFile = (bHighSkill) ? "server_competitive_rounds_win_conditions_high_skill.cfg"
											   : "server_competitive_rounds_win_conditions.cfg";

		if(bUseStopWatch)
		{
			pszExecFile = (bHighSkill) ? "server_competitive_stopwatch_win_conditions_high_skill.cfg"
									   : "server_competitive_stopwatch_win_conditions.cfg";
		}
		else if(bMultiStagePLR || bCTF)
		{
			pszExecFile = (bHighSkill) ? "server_competitive_max_rounds_win_conditions_high_skill.cfg"
	...
	bool ShouldRequestLateJoin() const OVERRIDE
	{
		...
		return nMissingPlayers && nPlayers &&
			   (pTFGameRules->State_Get() == GR_STATE_BETWEEN_RNDS || pTFGameRules->State_Get() == GR_STATE_PREGAME ||
				pTFGameRules->State_Get() == GR_STATE_STARTGAME);
	}

	bool BMatchIsSafeToLeaveForPlayer(const CMatchInfo *pMatchInfo,
									  const CMatchInfo::PlayerMatchData_t *pMatchPlayer) const OVERRIDE
	{
	...
	virtual bool BPlayWinMusic(int nWinningTeam, bool bGameOver) const OVERRIDE
	{
		if(bGameOver)
		{
			TFGameRules()->BroadcastSound(255, (nWinningTeam == TF_TEAM_RED) ? "Announcer.CompMatchWinRed"
																			 : "Announcer.CompMatchWinBlu");
			TFGameRules()->BroadcastSound(255, (nWinningTeam == TF_TEAM_RED) ? "MatchMaking.MatchEndRedWinMusic"
...
public:
	...
	public:
		CCasualProgressionDesc(EMatchGroup eMatchGroup)
			: IProgressionDesc(eMatchGroup, "models/vgui/12v12_badge.mdl", "resource/ui/PvPCasualRankPanel.res",
			  ...
			  m_nAverageMinutesPerGame(30)
		{
			...
			const StepInfo_t stepInfo[] = {
				{1.5f, "MatchMaking.LevelOneAchieved"},	 {2.5f, "MatchMaking.LevelTwoAchieved"},
				{4.f, "MatchMaking.LevelThreeAchieved"}, {6.f, "MatchMaking.LevelFourAchieved"},
				{9.f, "MatchMaking.LevelFiveAchieved"},	 {14.f, "MatchMaking.LevelSixAchieved"}};
...
#error This function is only okay on the client due to calling yielding stuff
														  ...
														  CTFSharedObjectCache *pLockedSOCache) const OVERRIDE
		{
			...
			if(bRet)
			{
				transaction.AddCommitListener(
					[pLockedSOCache, nAck, pLadderData]()
					{
						Assert(GGCTF()->IsSteamIDLockedByCurJob(pLockedSOCache->GetOwner()));
		...
		virtual void DebugSpewLevels() const OVERRIDE
		{
			...
			for(int i = 0; i < m_vecLevels.Count(); ++i)
			{
				...
				Color color =
					pScheme->GetColor(GetColorNameForAttribColor(pRarity->GetAttribColor()), Color(255, 255, 255, 255));

				ConColorMsg(color, "Level %d:\t%d - %d.  Expected games required: %d  Expected hours required: %.2f\n",
							m_vecLevels[i].m_nLevelNum, m_vecLevels[i].m_nStartXP, m_vecLevels[i].m_nEndXP,
							nNumGamesToAchieve, (nExpectedMinutesToAchieve / 60.f));
#else
	...
	CCasualMatchGroupDescription(EMatchGroup eMatchGroup, ConVar *pmm_match_group_size,
	...
	virtual void InitGameRulesSettingsPostEntity() const OVERRIDE
	{
		...
		// Exec our match settings
		const char *pszExecFile = bUseStopWatch
									  ? "server_casual_stopwatch_win_conditions.cfg"
									  : ((bMultiStagePLR || bCTF) ? "server_casual_max_rounds_win_conditions.cfg"
	...
	virtual bool BPlayWinMusic(int nWinningTeam, bool bGameOver) const OVERRIDE
	{
		...
		if(bGameOver)
		{
			TFGameRules()->BroadcastSound(TF_TEAM_RED, nWinningTeam == TF_TEAM_RED
														   ? "MatchMaking.MatchEndWinMusicCasual"
														   : "MatchMaking.MatchEndLoseMusicCasual");
			TFGameRules()->BroadcastSound(TF_TEAM_BLUE, nWinningTeam == TF_TEAM_BLUE
															? "MatchMaking.MatchEndWinMusicCasual"
															: "MatchMaking.MatchEndLoseMusicCasual");
			return true;
		}
...
{
	static CMvMMatchGroupDescription descBootcamp(k_nMatchGroup_MvM_Practice, "server_bootcamp.cfg",
												  /* bTrustedOnly */ false);
