...
class CAchievementTFScout_FirstBloodKill : public CBaseTFAchievement
{
									...
									IGameEvent *event)
	{
		...
		if(pAttackerPlayer->m_Shared.IsFirstBloodBoosted() &&
		   pAttackerPlayer->m_Shared.InCond(TF_COND_CRITBOOSTED_FIRST_BLOOD) && !bFirstBlood)
		{
			IncrementCount();
		}
	}
};
DECLARE_ACHIEVEMENT(CAchievementTFScout_FirstBloodKill, ACHIEVEMENT_TF_SCOUT_FIRST_BLOOD_KILL,
					"TF_SCOUT_FIRST_BLOOD_KILL", 5);
...
class CAchievementTFScout_WellEarlyKill : public CBaseTFAchievement
{
									...
									IGameEvent *event)
	{
		...
		if(FStrEq(m_pAchievementMgr->GetMapName(), "cp_well") && TFGameRules() &&
		   (TFGameRules()->State_Get() == GR_STATE_RND_RUNNING) && TFGameRules()->InSetup())
		{
			IncrementCount();
		}
	}
};
DECLARE_ACHIEVEMENT(CAchievementTFScout_WellEarlyKill, ACHIEVEMENT_TF_SCOUT_WELL_EARLY_KILL, "TF_SCOUT_WELL_EARLY_KILL",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_LifetimeKills, ACHIEVEMENT_TF_SCOUT_LIFETIME_KILLS, "TF_SCOUT_LIFETIME_KILLS",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_IronManKills, ACHIEVEMENT_TF_SCOUT_IRON_MAN_KILLS, "TF_SCOUT_IRON_MAN_KILLS",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_DestroyTeleporters, ACHIEVEMENT_TF_SCOUT_DESTROY_TELEPORTERS,
					"TF_SCOUT_DESTROY_TELEPORTERS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_DestroyBuildingsBeingBuilt, ACHIEVEMENT_TF_SCOUT_DESTROY_BUILDINGS_BEING_BUILT,
					"TF_SCOUT_DESTROY_BUILDINGS_BEING_BUILT", 5);
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		if(Q_strcmp(event->GetName(), "object_destroyed") == 0)
		{
			...
			if(pDestroyer == pTFPlayer)
			{
				...
				if((iType == OBJ_SENTRYGUN) && (iWeaponID == TF_WEAPON_PISTOL_SCOUT) && !event->GetBool("was_building"))
				{
					IncrementCount();
				}
			}
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_DestroySentryWithPistol, ACHIEVEMENT_TF_SCOUT_DESTROY_SENTRY_WITH_PISTOL,
					"TF_SCOUT_DESTROY_SENTRY_WITH_PISTOL", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_AssistChargeMedic, ACHIEVEMENT_TF_SCOUT_ASSIST_MEDIC, "TF_SCOUT_ASSIST_MEDIC",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_StealSandwich, ACHIEVEMENT_TF_SCOUT_STEAL_SANDWICH, "TF_SCOUT_STEAL_SANDWICH",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_KillChargedMedic, ACHIEVEMENT_TF_SCOUT_KILL_CHARGED_MEDICS,
					"TF_SCOUT_KILL_CHARGED_MEDICS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_SurviveDamage, ACHIEVEMENT_TF_SCOUT_SURVIVE_DAMAGE, "TF_SCOUT_SURVIVE_DAMAGE",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_ThreeFlagCaptures, ACHIEVEMENT_TF_SCOUT_THREE_FLAGCAPS,
					"TF_SCOUT_THREE_FLAGCAPS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_DoubleJumpKill, ACHIEVEMENT_TF_SCOUT_DOUBLEJUMP_KILL,
					"TF_SCOUT_DOUBLEJUMP_KILL", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_FlagCapGrind, ACHIEVEMENT_TF_SCOUT_FLAG_CAP_GRIND, "TF_SCOUT_FLAG_CAP_GRIND",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_KnockIntoTrain, ACHIEVEMENT_TF_SCOUT_KNOCK_INTO_TRAIN,
					"TF_SCOUT_KNOCK_INTO_TRAIN", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_StunIntoTrain, ACHIEVEMENT_TF_SCOUT_STUN_INTO_TRAIN, "TF_SCOUT_STUN_INTO_TRAIN",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_StunUberEnemies, ACHIEVEMENT_TF_SCOUT_STUN_UBER_ENEMIES,
					"TF_SCOUT_STUN_UBER_ENEMIES", 5);
...
class CAchievementTFScout_StunCappingEnemies : public CBaseTFAchievement
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		if(FStrEq(pszEventName, "player_stunned"))
		{
			...
			if((pStunner == pLocalPlayer) && pVictim && event->GetBool("victim_capping"))
			{
				IncrementCount();
			}
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_StunCappingEnemies, ACHIEVEMENT_TF_SCOUT_STUN_CAPPING_ENEMIES,
					"TF_SCOUT_STUN_CAPPING_ENEMIES", 5);
...
class CAchievementTFScout_MaxStuns : public CBaseTFAchievement
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		if(FStrEq(pszEventName, "player_stunned"))
		{
			...
			if((pStunner == pLocalPlayer) && pVictim && event->GetBool("big_stun"))
			{
				IncrementCount();
			}
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_StunScoutWithTheirBall, ACHIEVEMENT_TF_SCOUT_STUN_SCOUT_WITH_THEIR_BALL,
					"TF_SCOUT_STUN_SCOUT_WITH_THEIR_BALL", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_KillInDodgeCooldown, ACHIEVEMENT_TF_SCOUT_KILL_IN_DODGE_COOLDOWN,
					"TF_SCOUT_KILL_IN_DODGE_COOLDOWN", 5);
...
class CAchievementTFScout_KillFromBehind : public CBaseTFAchievement
{
									...
									IGameEvent *event)
	{
		...
		{
			CTFScatterGun *pScattergun =
				(CTFScatterGun *)ToTFPlayer(C_TFPlayer::GetLocalPlayer())->Weapon_OwnsThisID(TF_WEAPON_SCATTERGUN);
			if(pScattergun && pScattergun->HasKnockback() && (DotProductToTarget(pAttacker, pVictim) > -0.1))
			{
				IncrementCount();
			}
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_KillFromBehind, ACHIEVEMENT_TF_SCOUT_KILL_FROM_BEHIND,
					"TF_SCOUT_KILL_FROM_BEHIND", 5);
...
class CAchievementTFScout_CaptureLastPoint : public CBaseTFAchievement
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		else if(FStrEq(pszEventName, "teamplay_round_win"))
		{
			...
			int iTeam = event->GetInt("team");
			if((iTeam == pLocalPlayer->GetTeamNumber()) && m_bRecentCapper &&
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_CaptureLastPoint, ACHIEVEMENT_TF_SCOUT_CAPTURE_LAST_POINT,
					"TF_SCOUT_CAPTURE_LAST_POINT", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_CaptureThreePoints, ACHIEVEMENT_TF_SCOUT_CAPTURE_THREE_POINTS,
					"TF_SCOUT_CAPTURE_THREE_POINTS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_StartAndFinishCap, ACHIEVEMENT_TF_SCOUT_START_AND_FINISH_CAP,
					"TF_SCOUT_START_AND_FINISH_CAP", 5);
...
class CAchievementTFScout_BlockCaps : public CBaseTFAchievement
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		if(FStrEq(pszEventName, "teamplay_capture_blocked"))
		{
			...
			if((iBlocker == pLocalPlayer->entindex()) && ((TFGameRules()->GetGameType() == TF_GAMETYPE_CP) ||
														  (TFGameRules()->GetGameType() == TF_GAMETYPE_ARENA)))
			{
				IncrementCount();
			}
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_CarrierKillCarrier, ACHIEVEMENT_TF_SCOUT_CARRIER_KILL_CARRIER,
					"TF_SCOUT_CARRIER_KILL_CARRIER", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_CapFlagWithoutAttacking, ACHIEVEMENT_TF_SCOUT_CAP_FLAG_WITHOUT_ATTACKING,
					"TF_SCOUT_CAP_FLAG_WITHOUT_ATTACKING", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_LongDistanceRunner, ACHIEVEMENT_TF_SCOUT_LONG_DISTANCE_RUNNER,
					"TF_SCOUT_LONG_DISTANCE_RUNNER", 5);
...
class CAchievementTFScout_TauntKill : public CBaseTFAchievement
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		else if(FStrEq(pszEventName, "scout_slamdoll_landed"))
		{
			...
			if(pPlayer && (m_iTargetID == pPlayer->GetUserID()) && (m_vSlamOrigin != Vector(0, 0, 0)))
			{
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_AchieveProgress1, ACHIEVEMENT_TF_SCOUT_ACHIEVE_PROGRESS1,
					"TF_SCOUT_ACHIEVE_PROGRESS1", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_AchieveProgress2, ACHIEVEMENT_TF_SCOUT_ACHIEVE_PROGRESS2,
					"TF_SCOUT_ACHIEVE_PROGRESS2", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_AchieveProgress3, ACHIEVEMENT_TF_SCOUT_ACHIEVE_PROGRESS3,
					"TF_SCOUT_ACHIEVE_PROGRESS3", 5);
				...
				if((event->GetInt("weaponid") == TF_WEAPON_SCATTERGUN) && (event->GetInt("damagebits") & DMG_CRITICAL))
				{
					...
					if(pTFAttacker)
					{
						...
						if(pWeapon)
						{
							...
							CALL_ATTRIB_HOOK_INT_ON_OTHER(pWeapon, iMiniCritBackAttack,
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_BackscatterKillSpyGrind, ACHIEVEMENT_TF_SCOUT_BACKSCATTER_KILL_SPY_GRIND,
					"TF_SCOUT_BACKSCATTER_KILL_SPY_GRIND", 5);
							...
							else
							{
								...
								if(pTFVictim->IsPlayerClass(TF_CLASS_HEAVYWEAPONS))
								{
									for(int i = 1; i <= gpGlobals->maxClients; i++)
									{
										...
										{
											// add all of his Medics to our list of targets (could be more than one
											// Medic)
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_BackscatterKillHeavyMedicPair,
					ACHIEVEMENT_TF_SCOUT_BACKSCATTER_KILL_HEAVY_MEDIC_PAIR,
					"TF_SCOUT_BACKSCATTER_KILL_HEAVY_MEDIC_PAIR", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFScout_BackscatterKillFriendsGrind,
					ACHIEVEMENT_TF_SCOUT_BACKSCATTER_KILL_FRIENDS_GRIND, "TF_SCOUT_BACKSCATTER_KILL_FRIENDS_GRIND", 5);
