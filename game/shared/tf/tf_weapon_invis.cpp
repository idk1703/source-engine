	...
	else
	{
		if(HasFeignDeath())
		{
			...
			else if(pOwner->m_Shared.GetSpyCloakMeter() == 100.f)
			{
				...
				(pOwner->m_Shared.GetSpyCloakMeter() > 8.0f)) // must have over 10% cloak to start
		{
			// Do standard cloak.
...
void CTFWeaponInvis::SetCloakRates(void)
{
	...
	CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(
		pOwner, fCloakConsumeFactor,
		mult_cloak_meter_consume_rate); // Ask the owner since this attr may come from another weapon
