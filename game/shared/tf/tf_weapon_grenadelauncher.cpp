...
ConVar tf_double_donk_window(
	"tf_double_donk_window", "0.5", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
	"How long after an impact from a cannonball that an explosion will count as a double-donk.");
...
const Vector cProceduralBarrelRotationAnimationPoints[] = {Vector(0, 0, 0), Vector(0.7519f, 63.546f, 0),
														   Vector(1.0f, 60, 0)};

static_assert(ARRAYSIZE(cProceduralBarrelRotationAnimationPoints) > 1,
			  "cProceduralBarrelRotationAnimationPoints must have at least two elements.");
...
#ifdef CLIENT_DLL
	RecvPropFloat(RECVINFO(m_flDetonateTime)), RecvPropInt(RECVINFO(m_iCurrentTube)),
		RecvPropInt(RECVINFO(m_iGoalTube)),
#else
	SendPropFloat(SENDINFO(m_flDetonateTime)), SendPropInt(SENDINFO(m_iCurrentTube)),
...
BEGIN_PREDICTION_DATA(CTFGrenadeLauncher)
	DEFINE_FIELD(m_flDetonateTime, FIELD_FLOAT), DEFINE_FIELD(m_iCurrentTube, FIELD_INTEGER),
...
void CTFGrenadeLauncher::FireProjectileInternal(CTFPlayer *pTFPlayer)
{
	...
	if(pProjectile)
	{
		...
		if(m_flDetonateTime > 0.f)
		{
			...
			{
				trace_t tr;
				UTIL_TraceLine(pProjectile->GetAbsOrigin(), pTFPlayer->EyePosition(), MASK_SOLID, pProjectile,
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFGrenadeLauncher::StandardBlendingRules(CStudioHdr *hdr, Vector pos[], Quaternion q[], float currentTime,
											   int boneMask)
{
...
void CTFGrenadeLauncher::UpdateBarrelMovement(void)
{
	if(m_iGoalTube != m_iCurrentTube)
	{
		...
		if(tVal < 1.0f)
		{
			...
			Assert(
				cProceduralBarrelRotationAnimationPoints[ARRAYSIZE(cProceduralBarrelRotationAnimationPoints) - 1].x ==
				1.0f);
			...
			for(int i = 1; i < ARRAYSIZE(cProceduralBarrelRotationAnimationPoints); ++i)
			{
				...
				Assert(cProceduralBarrelRotationAnimationPoints[i - 1].x <
					   cProceduralBarrelRotationAnimationPoints[i].x);
...
void CTFGrenadeLauncher::ViewModelAttachmentBlending(CStudioHdr *hdr, Vector pos[], Quaternion q[], float currentTime,
													 int boneMask)
{
