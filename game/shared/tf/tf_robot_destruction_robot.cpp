...
RobotData_t *g_RobotData[NUM_ROBOT_TYPES] = {
	// Model									// Busted model										// Pain			// Death		// Collide		// Idle				// Bar
	// offset
	new RobotData_t("models/bots/bot_worker/bot_worker_A.mdl", "models/bots/bot_worker/bot_worker_A.mdl", "Robot.Pain",
					"Robot.Death", "Robot.Collide", "Robot.Greeting", -35.f),
#ifdef STAGING_ONLY
	new RobotData_t("models/bots/bot_worker/bot_worker_b.mdl", "models/bots/bot_worker/bot_worker_b.mdl", "Robot.Pain",
					"Robot.Death", "Robot.Collide", "Robot.Greeting", -30.f),
#else
	new RobotData_t("models/bots/bot_worker/bot_worker2.mdl", "models/bots/bot_worker/bot_worker2.mdl", "Robot.Pain",
					"Robot.Death", "Robot.Collide", "Robot.Greeting", -30.f),
#endif
	new RobotData_t("models/bots/bot_worker/bot_worker3.mdl", "models/bots/bot_worker/bot_worker3_nohead.mdl",
					"Robot.Pain", "Robot.Death", "Robot.Collide", "Robot.Greeting", -10.f),
};
...
void RobotData_t::Precache()
{
	...
	if(GetStringData(DAMAGED_MODEL_KEY))
		CBaseEntity::PrecacheModel(GetStringData(DAMAGED_MODEL_KEY));
	if(GetStringData(HURT_SOUND_KEY))
		CBaseEntity::PrecacheScriptSound(GetStringData(HURT_SOUND_KEY));
	if(GetStringData(DEATH_SOUND_KEY))
		CBaseEntity::PrecacheScriptSound(GetStringData(DEATH_SOUND_KEY));
	if(GetStringData(COLLIDE_SOUND_KEY))
		CBaseEntity::PrecacheScriptSound(GetStringData(COLLIDE_SOUND_KEY));
	if(GetStringData(IDLE_SOUND_KEY))
		CBaseEntity::PrecacheScriptSound(GetStringData(IDLE_SOUND_KEY));
}
...
#ifdef CLIENT_DLL
	RecvPropInt(RECVINFO(m_iHealth)), RecvPropInt(RECVINFO(m_iMaxHealth)), RecvPropInt(RECVINFO(m_eType)),
#else
...
CTFRobotDestruction_Robot::CTFRobotDestruction_Robot() : m_animController(this)
{
#ifdef GAME_DLL
	m_nPointsSpewed = 0;
...
void CTFRobotDestruction_Robot::UpdateDamagedEffects()
{
	...
	{
		m_hDamagedParticleEffect = ParticleProp()->Create(DAMAGED_ROBOT_PARTICLE_EFFECT, PATTACH_ABSORIGIN_FOLLOW,
														  INVALID_PARTICLE_ATTACHMENT, Vector(0, 0, 50));
	}
...
void CTFRobotDestruction_Robot::FireGameEvent(IGameEvent *pEvent)
{
	...
	if(FStrEq(pszName, "rd_robot_impact"))
	{
		...
		if(index_ == entindex())
		{
			Vector vecImpulse(pEvent->GetFloat("impulse_x"), pEvent->GetFloat("impulse_y"),
							  pEvent->GetFloat("impulse_z"));
...
void CTFRobotDestruction_Robot::SpewGibs()
{
	for(int i = 0; i < m_aGibs.Count(); i++)
	{
		CTFAmmoPack *pAmmoPack =
			CTFAmmoPack::Create(GetAbsOrigin() + m_aGibs[i].offset, GetAbsAngles(), this, m_aGibs[i].modelName);
		Assert(pAmmoPack);
		if(pAmmoPack)
		{
			...
			Vector vecImpulse(random->RandomFloat(-0.5f, 0.5f), random->RandomFloat(-0.5f, 0.5f),
							  random->RandomFloat(0.75f, 1.25f));
			...
			if(bIsTheHead)
			{
				...
				vecImpulse[2] = 3.f;
				vecImpulse *=
					random->RandomFloat(tf_obj_gib_velocity_max.GetFloat() * 0.75, tf_obj_gib_velocity_max.GetFloat());
			}
			else
			{
				vecImpulse *=
					random->RandomFloat(tf_obj_gib_velocity_min.GetFloat(), tf_obj_gib_velocity_max.GetFloat());
			}
...
//-----------------------------------------------------------------------------
void CTFRobotDestruction_Robot::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr,
...
public:
	...
	virtual ActionResult<CTFRobotDestruction_Robot> OnStart(CTFRobotDestruction_Robot *pMe,
															Action<CTFRobotDestruction_Robot> *priorAction)
	{
	...
	virtual ActionResult<CTFRobotDestruction_Robot> OnResume(CTFRobotDestruction_Robot *pMe,
															 Action<CTFRobotDestruction_Robot> *interruptingAction)
	{
		PlayIdleActivity(pMe);
...
public:
	virtual ActionResult<CTFRobotDestruction_Robot> OnStart(CTFRobotDestruction_Robot *pMe,
															Action<CTFRobotDestruction_Robot> *priorAction);
	virtual ActionResult<CTFRobotDestruction_Robot> Update(CTFRobotDestruction_Robot *pMe, float interval);
	virtual EventDesiredResult<CTFRobotDestruction_Robot> OnInjured(CTFRobotDestruction_Robot *pMe,
...
//---------------------------------------------------------------------------------------------
ActionResult<CTFRobotDestruction_Robot> CRobotPanic::OnStart(CTFRobotDestruction_Robot *pMe,
															 Action<CTFRobotDestruction_Robot> *priorAction)
{
	...
	DispatchParticleEffect("rocketjump_smoke", PATTACH_POINT_FOLLOW, pMe,
						   "wheel");	// Smoke trails on our tire when panicked
...
ActionResult<CTFRobotDestruction_Robot> CRobotPanic::Update(CTFRobotDestruction_Robot *pMe, float interval)
{
	...
	UTIL_TraceLine(vArrivePosition, vArrivePosition + Vector(0, 0, -30), MASK_PLAYERSOLID, pMe,
...
EventDesiredResult<CTFRobotDestruction_Robot> CRobotPanic::OnInjured(CTFRobotDestruction_Robot *pMe,
																	 const CTakeDamageInfo &info)
{
...
ActionResult<CTFRobotDestruction_Robot> CRobotBehavior::OnStart(CTFRobotDestruction_Robot *pMe,
																Action<CTFRobotDestruction_Robot> *priorAction)
{
	return Continue();
}
...
EventDesiredResult<CTFRobotDestruction_Robot> CRobotBehavior::OnInjured(CTFRobotDestruction_Robot *pMe,
																		const CTakeDamageInfo &info)
{
	return TrySuspendFor(new CRobotEnterPanic, RESULT_TRY, "I've been attacked");
}

EventDesiredResult<CTFRobotDestruction_Robot> CRobotBehavior::OnContact(CTFRobotDestruction_Robot *pMe,
																		CBaseEntity *pOther, CGameTrace *result)
{
