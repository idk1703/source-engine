...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_RJEqualizerKill, ACHIEVEMENT_TF_SOLDIER_RJ_EQUALIZER_KILL,
					"TF_SOLDIER_RJ_EQUALIZER_KILL", 5);
...
class CAchievementTFSoldier_BuffTeammates : public CBaseTFAchievement
{
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		else if(FStrEq(pszEvent, "deploy_buff_banner"))
		{
			...
			{
				// the local player has deployed their banner, clear our list for the current banner so we can track who
				// we buff
				m_BuffedPlayers.RemoveAll();
			}
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_BuffTeammates, ACHIEVEMENT_TF_SOLDIER_BUFF_TEAMMATES,
					"TF_SOLDIER_BUFF_TEAMMATES", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillDemomanGrind, ACHIEVEMENT_TF_SOLDIER_KILL_DEMOMAN_GRIND,
					"TF_SOLDIER_KILL_DEMOMAN_GRIND", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_NemesisShovelKill, ACHIEVEMENT_TF_SOLDIER_NEMESIS_SHOVEL_KILL,
					"TF_SOLDIER_NEMESIS_SHOVEL_KILL", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_DestroyStickies, ACHIEVEMENT_TF_SOLDIER_DESTROY_STICKIES,
					"TF_SOLDIER_DESTROY_STICKIES", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_CrouchRocketJump, ACHIEVEMENT_TF_SOLDIER_CROUCH_ROCKET_JUMP,
					"TF_SOLDIER_CROUCH_ROCKET_JUMP", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_EqualizerStreak, ACHIEVEMENT_TF_SOLDIER_EQUALIZER_STREAK,
					"TF_SOLDIER_EQUALIZER_STREAK", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_BuffFriends, ACHIEVEMENT_TF_SOLDIER_BUFF_FRIENDS, "TF_SOLDIER_BUFF_FRIENDS",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillGroupWithCrocket, ACHIEVEMENT_TF_SOLDIER_KILL_GROUP_WITH_CROCKET,
					"TF_SOLDIER_KILL_GROUP_WITH_CROCKET", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillTwoDuringRocketJump, ACHIEVEMENT_TF_SOLDIER_KILL_TWO_DURING_ROCKET_JUMP,
					"TF_SOLDIER_KILL_TWO_DURING_ROCKET_JUMP", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_DefendMedic, ACHIEVEMENT_TF_SOLDIER_DEFEND_MEDIC, "TF_SOLDIER_DEFEND_MEDIC",
					5);
...
class CAchievementTFSoldier_KillWithEqualizerWhileHurt : public CBaseTFAchievement
{
									...
									IGameEvent *event)
	{
		...
		if(pTFAttacker && (pTFAttacker == C_BasePlayer::GetLocalPlayer()) &&
		   (event->GetInt("weaponid") == TF_WEAPON_SHOVEL))
		{
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillWithEqualizerWhileHurt,
					ACHIEVEMENT_TF_SOLDIER_KILL_WITH_EQUALIZER_WHILE_HURT, "TF_SOLDIER_KILL_WITH_EQUALIZER_WHILE_HURT",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillAirborneTargetWhileAirborne,
					ACHIEVEMENT_TF_SOLDIER_KILL_AIRBORNE_TARGET_WHILE_AIRBORNE,
					"TF_SOLDIER_KILL_AIRBORNE_TARGET_WHILE_AIRBORNE", 5);
									...
									IGameEvent *event)
	{
		if(pAttacker == C_BasePlayer::GetLocalPlayer())
		{
			...
			if(pTFVictim && !(pTFVictim->GetFlags() & FL_ONGROUND) &&
			   (event->GetInt("weaponid") == TF_WEAPON_SHOTGUN_SOLDIER))
			{
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_BounceThenShotgun, ACHIEVEMENT_TF_SOLDIER_BOUNCE_THEN_SHOTGUN,
					"TF_SOLDIER_BOUNCE_THEN_SHOTGUN", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillAirborneWithDirectHit,
					ACHIEVEMENT_TF_SOLDIER_KILL_AIRBORNE_WITH_DIRECT_HIT, "TF_SOLDIER_KILL_AIRBORNE_WITH_DIRECT_HIT",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillSniperWhileDead, ACHIEVEMENT_TF_SOLDIER_KILL_SNIPER_WHILE_DEAD,
					"TF_SOLDIER_KILL_SNIPER_WHILE_DEAD", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_DestroySentryOutOfRange, ACHIEVEMENT_TF_SOLDIER_DESTROY_SENTRY_OUT_OF_RANGE,
					"TF_SOLDIER_DESTROY_SENTRY_OUT_OF_RANGE", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_AssistMedicUbercharge, ACHIEVEMENT_TF_SOLDIER_ASSIST_MEDIC_UBER,
					"TF_SOLDIER_ASSIST_MEDIC_UBER", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_ShootMultCrits, ACHIEVEMENT_TF_SOLDIER_SHOOT_MULT_CRITS,
					"TF_SOLDIER_SHOOT_MULT_CRITS", 5);
...
class CAchievementTFSoldier_KillDefenseless : public CBaseTFAchievement
{
	...
	virtual bool LocalPlayerCanEarn(void)
	{
		if(TFGameRules())
		{
			bool bMVMAchievement = (m_iAchievementID >= ACHIEVEMENT_TF_MVM_START_RANGE &&
									m_iAchievementID <= ACHIEVEMENT_TF_MVM_END_RANGE);

			if((bMVMAchievement && !TFGameRules()->IsMannVsMachineMode()) ||
			   (!bMVMAchievement && TFGameRules()->IsMannVsMachineMode()))
			{
				return false;
			}
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillDefenseless, ACHIEVEMENT_TF_SOLDIER_KILL_DEFENSELESS,
					"TF_SOLDIER_KILL_DEFENSELESS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillWhileOnFire, ACHIEVEMENT_TF_SOLDIER_KILL_ON_FIRE,
					"TF_SOLDIER_KILL_ON_FIRE", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_FreezeTaunts, ACHIEVEMENT_TF_SOLDIER_FREEZECAM_TAUNT,
					"TF_SOLDIER_FREEZECAM_TAUNT", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_FreezeGibs, ACHIEVEMENT_TF_SOLDIER_FREEZECAM_GIBS,
					"TF_SOLDIER_FREEZECAM_GIBS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillSpyKiller, ACHIEVEMENT_TF_SOLDIER_KILL_SPY_KILLER,
					"TF_SOLDIER_KILL_SPY_KILLER", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_DuoSoldierKills, ACHIEVEMENT_TF_SOLDIER_DUO_SOLDIER_KILLS,
					"TF_SOLDIER_DUO_SOLDIER_KILLS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_ThreeDominations, ACHIEVEMENT_TF_SOLDIER_THREE_DOMINATIONS,
					"TF_SOLDIER_THREE_DOMINATIONS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_RideTheCart, ACHIEVEMENT_TF_SOLDIER_RIDE_THE_CART, "TF_SOLDIER_RIDE_THE_CART",
					5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillTwentyFromAbove, ACHIEVEMENT_TF_SOLDIER_KILL_TWENTY_FROM_ABOVE,
					"TF_SOLDIER_KILL_TWENTY_FROM_ABOVE", 5);
									...
									IGameEvent *event)
	{
		...
		if(pLocalPlayer)
		{
			if(pAttacker == pLocalPlayer)
			{
				...
				if((iStunFlags & TF_STUN_LOSER_STATE) != 0 || (iStunFlags & TF_STUN_CONTROLS) != 0)
				{
					IncrementCount();
				}
			}
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_KillFiveStunned, ACHIEVEMENT_TF_SOLDIER_KILL_FIVE_STUNNED,
					"TF_SOLDIER_KILL_FIVE_STUNNED", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_DefendCapThirtyTimes, ACHIEVEMENT_TF_SOLDIER_DEFEND_CAP_THIRTY_TIMES,
					"TF_SOLDIER_DEFEND_CAP_THIRTY_TIMES", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_AchieveProgress1, ACHIEVEMENT_TF_SOLDIER_ACHIEVE_PROGRESS1,
					"TF_SOLDIER_ACHIEVE_PROGRESS1", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_AchieveProgress2, ACHIEVEMENT_TF_SOLDIER_ACHIEVE_PROGRESS2,
					"TF_SOLDIER_ACHIEVE_PROGRESS2", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_AchieveProgress3, ACHIEVEMENT_TF_SOLDIER_ACHIEVE_PROGRESS3,
					"TF_SOLDIER_ACHIEVE_PROGRESS3", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_ParachuteKillGroup, ACHIEVEMENT_TF_SOLDIER_PARACHUTE_KILL_GROUP,
					"TF_SOLDIER_PARACHUTE_KILL_GROUP", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_ParachuteDistance, ACHIEVEMENT_TF_SOLDIER_PARACHUTE_DISTANCE,
					"TF_SOLDIER_PARACHUTE_DISTANCE", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_ParachuteKillParachute, ACHIEVEMENT_TF_SOLDIER_PARACHUTE_KILL_PARACHUTE,
					"TF_SOLDIER_PARACHUTE_KILL_PARACHUTE", 5);
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_AirstrikeMaxClip, ACHIEVEMENT_TF_SOLDIER_AIRSTRIKE_MAX_CLIP,
					"TF_SOLDIER_AIRSTRIKE_MAX_CLIP", 5);
...
public:
	...
	void FireGameEvent_Internal(IGameEvent *event)
	{
		...
		if(FStrEq(pszEventName, "teamplay_round_active") || FStrEq(pszEventName, "localplayer_respawn"))
		{
			ResetTracking();
		}
...
DECLARE_ACHIEVEMENT(CAchievementTFSoldier_AirstrikeGroupKill, ACHIEVEMENT_TF_SOLDIER_AIRSTRIKE_GROUP_KILL,
					"TF_SOLDIER_AIRSTRIKE_GROUP_KILL", 5);
