...
ConVar tf_obj_build_rotation_speed("tf_obj_build_rotation_speed", "250",
...
static ConVar sv_ignore_hitboxes("sv_ignore_hitboxes", "0", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
								 "Disable hitboxes");
...
bool CBaseObject::CalculatePlacementPos(void)
{
	...
	for(iIteration = 0; iIteration < nIterations; iIteration++)
	{
		UTIL_TraceHull(Vector(m_vecBuildOrigin.x, m_vecBuildOrigin.y, topZ),
					   ...
					   vHalfBuildDimsXY, MASK_PLAYERSOLID_BRUSHONLY, this, COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
		bottomZ = tr.endpos.z;
...
bool CBaseObject::VerifyCorner(const Vector &vBottomCenter, float xOffset, float yOffset)
{
	...
	trace_t tr;
	UTIL_TraceLine(vStart, vStart - Vector(0, 0, TF_OBJ_GROUND_CLEARANCE), MASK_PLAYERSOLID_BRUSHONLY, this,
...
bool CBaseObject::IsPlacementPosValid(void)
{
	...
	UTIL_TraceLine(pPlayer->EyePosition(), m_vecBuildOrigin + Vector(0, 0, m_vecBuildMaxs[2] * 0.5),
				   MASK_PLAYERSOLID_BRUSHONLY, pPlayer, COLLISION_GROUP_NONE, &tr);
	if(tr.fraction < 1.0)
	{
		return false;
	}
...
void CBaseObject::UpdateDesiredBuildRotation(float flFrameTime)
{
	...
	m_flCurrentBuildRotation =
		ApproachAngle(flBuildRotation, m_flCurrentBuildRotation, tf_obj_build_rotation_speed.GetFloat() * flFrameTime);
}