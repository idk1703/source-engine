...
BEGIN_PREDICTION_DATA(CTFRobotArm)
	...
	DEFINE_PRED_FIELD(m_iComboCount, FIELD_INTEGER, 0), DEFINE_PRED_FIELD(m_flLastComboHit, FIELD_FLOAT, 0),
END_PREDICTION_DATA()
#endif
...
// Purpose:
//-----------------------------------------------------------------------------
CTFWrench::CTFWrench() : m_bReloadDown(false) {}
...
void CTFWrench::ItemPostFrame()
{
	...
	if(pOwner->m_nButtons & IN_RELOAD && !m_bReloadDown)
	{
		...
		if(iAltFireTeleportToSpawn)
		{
			...
			CHudEurekaEffectTeleportMenu *pTeleportMenu =
				(CHudEurekaEffectTeleportMenu *)GET_HUDELEMENT(CHudEurekaEffectTeleportMenu);
			if(pTeleportMenu)
			{
...
// STAGING_ENGY
ConVar tf_construction_build_rate_multiplier("tf_construction_build_rate_multiplier", "1.5f",
