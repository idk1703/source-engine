...
class CPasstimeGun : public CTFWeaponBase, public ITFChargeUpWeapon
{
	...
	DECLARE_PREDICTABLE(); // this has to be here because the client's precache code uses it to get the classname of
						   // this entity...
...
protected:
	...
	{
		AttackInputState(int button) : iButton(button), eButtonState(BUTTONSTATE_UP), bLatchedUp(false) {}
		const int iButton;
