...
public:
	...
	void SetOwningEditable(const IEditableDataType *pEditable)
	{
		m_pOwningEditable = pEditable;
	}
	void SetFlag(int nFlag, bool bValue)
	{
		bValue ? m_Flags |= nFlag : m_Flags &= (~nFlag);
	}
	...
	virtual bool HasChanges(bool bCheckChildren) const
	{
...
public:
	...
	void DeleteEntry();
	void RevertChanges();
	void SetButton(Button *pButton)
	{
		m_hButton = pButton;
	}
	Panel *GetButton() const
	{
		return m_hButton.Get();
	}
	void CheckForChanges();

	KeyValues *GetLiveData() const
	{
		return m_pKVLiveData;
	}
...
private:
	...
	{
		return IEditableDataType::TYPE_QUEST;
	}
...
private:
	...
	virtual EType GetType() const
	{
		return IEditableDataType::TYPE_OBJECTIVE_CONDITIONS;
	}
	virtual IEditorObject *CreateEditableObject_Internal(Panel *pParent) const OVERRIDE;
};
...
public:
	...
	const CUtlVector<IEditorObject *> &GetChildren() const
	{
...
private:
	...
	virtual int GetMinCount() const OVERRIDE
	{
		return 1;
	}
};
...
public:
	...
	CTFQuestCondition *GetCondition()
	{
		return m_pCondition;
	}
...
public:
	...
	const CUtlVector<IEditableDataType *> &GetEditableData() const
	{
		return m_vecEditableData;
	}
