...
#ifdef GAME_DLL
		DEFINE_OUTPUT(m_OnRedPickup, "OnRedPickup"), DEFINE_OUTPUT(m_OnBluePickup, "OnBluePickup"),
#endif
END_DATADESC();
...
void CBonusDuckPickup::Spawn(void)
{
	...
	m_nBlinkCount = 0;
	SetContextThink(&CBonusDuckPickup::BlinkThink, gpGlobals->curtime + flLifeTime - DUCK_BLINK_TIME,
					"BonusDuckBlinkThink");
...
bool CBonusDuckPickup::MyTouch(CBasePlayer *pPlayer)
{
	...
	if(tf_duck_allow_team_pickup.GetBool() || ValidTouch(pPlayer))
	{
		...
		if(pPlayer && TFGameRules() && TFGameRules()->IsHolidayActive(kHoliday_EOTL) &&
		   !TFGameRules()->HaveCheatsBeenEnabledDuringLevel())
		{
			int iFlags = m_iFlags;
			if(m_bSpecial)
			{
...
//-----------------------------------------------------------------------------
void CBonusDuckPickup::DropSingleInstance(Vector &vecLaunchVel, CBaseCombatCharacter *pThrower,
										  float flThrowerTouchDelay, float flResetTime /*= 0.1f*/)
{
	...
	if(updateType == DATA_UPDATE_CREATED)
	{
		if(!IsDormant())
		{
			...
			if(m_bSpecial)
			{
				...
				ParticleProp()->Create((GetTeamNumber() == TF_TEAM_RED) ? BONUS_DUCK_TRAIL_SPECIAL_RED
																		: BONUS_DUCK_TRAIL_SPECIAL_BLUE,
									   PATTACH_ABSORIGIN_FOLLOW);
			}
			else
			{
				// these are fire and forget
				ParticleProp()->Create((GetTeamNumber() == TF_TEAM_RED) ? BONUS_DUCK_TRAIL_RED : BONUS_DUCK_TRAIL_BLUE,
									   PATTACH_ABSORIGIN_FOLLOW);
			}
...
#ifdef GAME_DLL
	// Set a timer
	SetContextThink(&CHalloweenGiftPickup::DespawnGift, gpGlobals->curtime + tf_halloween_gift_lifetime.GetInt(),
					"DespawnGift");
...
void CHalloweenGiftPickup::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if(updateType == DATA_UPDATE_DATATABLE_CHANGED)
	{
		...
		{
			// Gift Added
			if(m_hTargetPlayer.Get() != NULL && m_pPreviousTargetPlayer == NULL &&
			   m_hTargetPlayer.Get() == pLocalPlayer)
			{
				// Notification
			...
			// Gift Despawned
			if(m_hTargetPlayer.Get() == NULL && m_pPreviousTargetPlayer != NULL &&
			   m_pPreviousTargetPlayer == pLocalPlayer)
			{
				// Notification
				CEconNotification *pNotification = new CEconNotification();
