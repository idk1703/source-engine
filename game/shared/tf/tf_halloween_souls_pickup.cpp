...
#ifdef GAME_DLL
	SendPropEHandle(SENDINFO(m_hTarget)), SendPropVector(SENDINFO(m_vecPreCurvePos)),
		SendPropVector(SENDINFO(m_vecStartCurvePos)), SendPropFloat(SENDINFO(m_flDuration)),
#else
	RecvPropEHandle(RECVINFO(m_hTarget)), RecvPropVector(RECVINFO(m_vecPreCurvePos)),
		RecvPropVector(RECVINFO(m_vecStartCurvePos)), RecvPropFloat(RECVINFO(m_flDuration)),
#endif
END_NETWORK_TABLE()
...
void CHalloweenSoulPack::ItemTouch(CBaseEntity *pOther)
{
	...
	if(pPlayer)
	{
		...
		EconEntity_NonEquippedItemKillTracking_NoPartnerBatched(pPlayer, hItemDef->GetDefinitionIndex(), eEventType,
																m_nAmount);
...
void CHalloweenSoulPack::FlyTowardsTargetEntity(void)
{
	...
	Vector vecOutput;
	Catmull_Rom_Spline(m_vecPreCurvePos, m_vecStartCurvePos, pEntity->WorldSpaceCenter(), vecNextCuvePos, flT,
					   vecOutput);
