...
#if defined(GAME_DLL)
	SendPropBool(SENDINFO(m_bActive)), SendPropInt(SENDINFO(m_usNumCharges), -1, SPROP_UNSIGNED),
#else
	RecvPropBool(RECVINFO(m_bActive)), RecvPropInt(RECVINFO(m_usNumCharges)),
#endif // GAME_DLL
END_NETWORK_TABLE()
// -- Network Table
...
void CTFPowerupBottle::Reset(void)
{
	...
	{
	public:
		CAttributeIterator_ZeroRefundableCurrency(CAttributeList *pAttrList) : m_pAttrList(pAttrList)
		{
			Assert(m_pAttrList);
		}
...
bool CTFPowerupBottle::Use()
{
	if(!m_bActive && GetNumCharges() > 0)
	{
		...
		private:
			virtual bool OnIterateAttributeValueUntyped(const CEconItemAttributeDefinition *pAttrDef)
			{
				if(::FindAttribute(m_pAttrList, pAttrDef))
				{
					...
					{
						m_pAttrList->SetRuntimeAttributeRefundableCurrency(
							pAttrDef, nRefundableCurrency - (nRefundableCurrency / m_iNumCharges));
					}
				}
		...
		if(pOwner)
		{
			EconEntity_OnOwnerKillEaterEventNoPartner(dynamic_cast<CEconEntity *>(this), pOwner,
...
{
	if(TFGameRules() &&
	   !(TFGameRules()->State_Get() == GR_STATE_BETWEEN_RNDS || TFGameRules()->State_Get() == GR_STATE_RND_RUNNING))
		return false;
...
void CEquipMvMCanteenNotification::Accept()
{
	...
	for(int i = 0; i < pLocalInv->GetItemCount(); ++i)
	{
		...
		if(pItem->GetItemDefinition() == pItemDef_Robo || pItem->GetItemDefinition() == pItemDef_KritzOrTreat ||
		   pItem->GetItemDefinition() == pItemDef_Canteen || pItem->GetItemDefinition() == pItemDef_DefaultCanteen)
		{
			pCanteen = pItem;
			break;
		}
...
void CEquipMvMCanteenNotification::UpdateTick()
{
	...
	if(pLocalPlayer)
	{
		CTFPowerupBottle *pCanteen = dynamic_cast<CTFPowerupBottle *>(TFInventoryManager()->GetItemInLoadoutForClass(
