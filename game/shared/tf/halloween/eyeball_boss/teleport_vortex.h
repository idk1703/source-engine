...
protected:
	...
	float m_flScale;
	int m_iOldState; // This seems like it should not be necessary, but I'm getting OnDataChanged() called when m_iState
					 // has not actually changed, and updateType will be DATA_UPDATE_DATATABLE_CHANGED multiple times in
					 // a row.  Presumably this is because the position is being set explicitly on the server.
#else
