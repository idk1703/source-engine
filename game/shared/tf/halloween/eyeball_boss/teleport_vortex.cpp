...
void SendPlayerToTheUnderworld(CTFPlayer *teleportingPlayer, const char *where)
{
	...
	if(openSpawnVector.Count() == 0)
	{
		...
		for(int p = 0; p < enemyVector.Count(); ++p)
		{
			...
			{
				// telefrag!
				enemyVector[p]->TakeDamage(
					CTakeDamageInfo(teleportingPlayer, teleportingPlayer, 1000, DMG_CRUSH, TF_DMG_CUSTOM_TELEFRAG));
			}
		}
	}
	else
	...
	if(teleportDestination)
	{
		if(teleportingPlayer->GetTeam())
		{
			UTIL_LogPrintf("HALLOWEEN: \"%s<%i><%s><%s>\" purgatory_teleport \"%s\"\n",
						   teleportingPlayer->GetPlayerName(), teleportingPlayer->GetUserID(),
						   teleportingPlayer->GetNetworkIDString(), teleportingPlayer->GetTeam()->GetName(), where);
		}

		teleportingPlayer->Teleport(&teleportDestination->GetAbsOrigin(), &teleportDestination->GetAbsAngles(),
									&vec3_origin);
		...
		if(FStrEq(where, "spawn_loot"))
		{
			...
			UTIL_SayText2Filter(filter, teleportingPlayer, false, "#TF_Halloween_Loot_Island",
...
void CTeleportVortex::BuildTransformations(CStudioHdr *pStudioHdr, Vector *pos, Quaternion q[],
										   const matrix3x4_t &cameraTransform, int boneMask, CBoneBitList &boneComputed)
{
	// Translate root bone towards the player and oscillate a bit - if in the scale in/out period, translate up directly
	// from the underworld, or down towards it.
	const Vector vecBob(vortex_book_offset.GetFloat(), 0.0f,
						vortex_float_amp.GetFloat() * sinf(vortex_float_osc_speed.GetFloat() * gpGlobals->curtime));
...
public:
	...
	{
#ifdef GAME_DLL
		const char *pszTarget = CFmtStr("%s_%s", m_pszDestinationBaseName,
										(pOther->GetTeamNumber() == m_nWinningTeam) ? "winner" : "loser");
