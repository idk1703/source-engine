...
#ifdef GAME_DLL
	SendPropInt(SENDINFO(m_eThrowState)), SendPropFloat(SENDINFO(m_fChargeBeginTime))
#else
	RecvPropInt(RECVINFO(m_eThrowState)), RecvPropFloat(RECVINFO(m_fChargeBeginTime))
#endif
END_NETWORK_TABLE()

//-----------------------------------------------------------------------------
BEGIN_PREDICTION_DATA(CPasstimeGun)
END_PREDICTION_DATA() // this has to be here because the client's precache code uses it to get the classname of this
					  // entity...
...
//-----------------------------------------------------------------------------
CPasstimeGun::CPasstimeGun() : m_flTargetResetTime(0), m_attack(IN_ATTACK), m_attack2(IN_ATTACK2)
{
...
{
	return (tf_passtime_experiment_instapass.GetBool() && tf_passtime_experiment_instapass_charge.GetBool()) ? 3.0f
																											 : 0.0f;
}
...
acttable_t *CPasstimeGun::ActivityList(int &iActivityCount)
{
	...
	return GetTFPlayerOwner() ? s_acttablePasstime : BaseClass::ActivityList(iActivityCount);
}
...
void CPasstimeGun::ItemPostFrame()
{
	...
	if(pOwner->m_Shared.HasPasstimeBall())
	{
		...
		if(!bCanAttack2Cancel || !m_attack2.Is(BUTTONSTATE_DOWN)) // right click prevents pass lock
		{
			...
			for(int i = 1; i <= MAX_PLAYERS; i++)
			{
				...
				if(!bAutoPassing)
				{
					...
					flThisTargetDist = Vector(-vLocalTarget.y / vLocalTarget.x, -vLocalTarget.z / vLocalTarget.x, 0)
										   .Length(); // not aspect-correct
				}
		...
		// autopass
		if(tf_passtime_experiment_autopass.GetBool() && m_attack2.Is(EButtonState::BUTTONSTATE_DOWN) &&
...
void CPasstimeGun::UpdateThrowArch()
{
	...
	for(int iPoint = 0; iPoint < iMaxTraces; ++iPoint)
	{
		...
		UTIL_TraceHull(vecStart, vecEnd, -launchParams.traceHullSize, launchParams.traceHullSize, MASK_PLAYERSOLID,
					   &traceFilter, &tr);
...
//-----------------------------------------------------------------------------
// static
CPasstimeGun::LaunchParams CPasstimeGun::LaunchParams::Default(CTFPlayer *pPlayer)
{
	LaunchParams p;
...
static void GetThrowParams(CTFPlayer *pPlayer, float *speed, float *arc)
{
	...
	{
		if(speed)
			*speed = s_pThrowSpeedConvars[iClass]->GetFloat();
		if(arc)
			*arc = s_pThrowArcConvars[iClass]->GetFloat();
	}
}
