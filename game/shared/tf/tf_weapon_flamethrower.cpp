...
ConVar tf_flamethrower_velocity("tf_flamethrower_velocity", "2300.0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
								"Initial velocity of flame damage entities.");
ConVar tf_flamethrower_drag("tf_flamethrower_drag", "0.87", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							"Air drag of flame damage entities.");
ConVar tf_flamethrower_float("tf_flamethrower_float", "50.0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							 "Upward float velocity of flame damage entities.");
ConVar tf_flamethrower_vecrand("tf_flamethrower_vecrand", "0.05", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
...
ConVar tf_flamethrower_maxdamagedist("tf_flamethrower_maxdamagedist", "350.0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
									 "Maximum damage distance for flamethrower.");
ConVar tf_flamethrower_shortrangedamagemultiplier("tf_flamethrower_shortrangedamagemultiplier", "1.2",
												  FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
												  "Damage multiplier for close-in flamethrower damage.");
ConVar tf_flamethrower_velocityfadestart("tf_flamethrower_velocityfadestart", ".3", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
										 "Time at which attacker's velocity contribution starts to fade.");
ConVar tf_flamethrower_velocityfadeend("tf_flamethrower_velocityfadeend", ".5", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
...
ConVar tf_flamethrower_burstammo("tf_flamethrower_burstammo", "20",
								 FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
								 "How much ammo does the air burst uses per shot.");
ConVar tf_flamethrower_flametime("tf_flamethrower_flametime", "0.5",
								 FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
...
//-----------------------------------------------------------------------------
void *SendProxy_SendLocalFlameThrowerDataTable(const SendProp *pProp, const void *pStruct, const void *pVarData,
...
(SendProxy_SendLocalFlameThrowerDataTable);
#endif // CLIENT_DLL
...
#if defined(CLIENT_DLL)
	RecvPropInt(RECVINFO(m_iActiveFlames)), RecvPropInt(RECVINFO(m_iDamagingFlames)),
...
#if defined(CLIENT_DLL)
	RecvPropInt(RECVINFO(m_iWeaponState)), RecvPropBool(RECVINFO(m_bCritFire)), RecvPropBool(RECVINFO(m_bHitTarget)),
...
#else
	SendPropInt(SENDINFO(m_iWeaponState), 4, SPROP_UNSIGNED | SPROP_CHANGES_OFTEN), SendPropBool(SENDINFO(m_bCritFire)),
		SendPropBool(SENDINFO(m_bHitTarget)), SendPropFloat(SENDINFO(m_flChargeBeginTime)),
		SendPropDataTable("LocalFlameThrowerData", 0, &REFERENCE_SEND_TABLE(DT_LocalFlameThrower),
						  SendProxy_SendLocalFlameThrowerDataTable),
#endif
END_NETWORK_TABLE()
...
void CTFFlameThrower::ItemPostFrame()
{
	...
	if(!((pOwner->m_nButtons & IN_ATTACK) || (pOwner->m_nButtons & IN_RELOAD)) ||
	   (!(pOwner->m_nButtons & IN_ATTACK2) || !m_bFiredSecondary))
	{
...
void CTFFlameThrower::PrimaryAttack()
{
	...
	// Make sure the weapon can't fire in this condition by tracing a line between the eye point and the end of the
	// muzzle.
	trace_t trace;
	...
	{
		// there is something between the eye and the end of the muzzle, most likely a wall, don't fire, and stop firing
		// if we already are
	...
	// it would be better if all entities could opt-in to this, or a way for lagcompensation to handle non-players
	// automatically
...
#define NUM_TEST_VECTORS 30
...
#endif // STAGING_ONLY
		{
			CTFFlameEntity::Create(GetFlameOriginPos(), pOwner->EyeAngles(), this, tf_flamethrower_velocity.GetFloat(),
								   iDmgType, flDamage, iCritFromBehind == 1);
		}
...
#ifdef GAME_DLL
	// Figure how much ammo we're using per shot and add it to our remainder to subtract.  (We may be using less
	// than 1.0 ammo units per frame, depending on how constants are tuned, so keep an accumulator so we can expend
	// fractional amounts of ammo per shot.) Note we do this only on server and network it to client.  If we predict it
	// on client, it can get slightly out of sync w/server and cause ammo pickup indicators to appear
	...
	// take the integer portion of the ammo use accumulator and subtract it from player's ammo count; any fractional
	// amount of ammo use remains and will get used in the next shot
...
void ExtinguishPlayer(CEconEntity *pExtinguisher, CTFPlayer *pOwner, CTFPlayer *pTarget, const char *pExtinguisherName)
{
	...
	UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"player_extinguished\" against \"%s<%i><%s><%s>\" with \"%s\" "
				   "(attacker_position \"%d %d %d\") (victim_position \"%d %d %d\")\n",
				   pOwner->GetPlayerName(), pOwner->GetUserID(), pOwner->GetNetworkIDString(),
				   pOwner->GetTeam()->GetName(), pTarget->GetPlayerName(), pTarget->GetUserID(),
				   pTarget->GetNetworkIDString(), pTarget->GetTeam()->GetName(), pExtinguisherName,
				   (int)pOwner->GetAbsOrigin().x, (int)pOwner->GetAbsOrigin().y, (int)pOwner->GetAbsOrigin().z,
				   (int)pTarget->GetAbsOrigin().x, (int)pTarget->GetAbsOrigin().y, (int)pTarget->GetAbsOrigin().z);
}

bool CTFFlameThrower::DeflectPlayer(CTFPlayer *pTarget, CTFPlayer *pOwner, Vector &vecForward, Vector &vecCenter,
									Vector &vecSize)
{
	if(pTarget->GetTeamNumber() == pOwner->GetTeamNumber() && pTarget != pOwner)
	{
		if(pTarget->m_Shared.InCond(TF_COND_BURNING) &&
		   SupportsAirBlastFunction(TF_FUNCTION_AIRBLAST_PUT_OUT_TEAMMATES))
		{
	...
	if(SupportsAirBlastFunction(TF_FUNCTION_AIRBLAST_PUSHBACK))
	{
		...
		// Require our target be in a cone in front of us. Default threshold is the dot-product needs to be at least 0.8
		// = 1 - 0.2.
		...
		if(pTarget != pOwner)
		{
			...
			if(SupportsAirBlastFunction(TF_FUNCTION_AIRBLAST_PUSHBACK__STUN))
			{
				...
				{
					pTarget->m_Shared.StunPlayer(tf_player_movement_stun_time.GetFloat(), 1.f, TF_STUN_MOVEMENT,
												 pOwner);
				}
			}
		...
		*/
		...
		SendObjectDeflectedEvent(pOwner, pTarget, TF_WEAPON_NONE,
								 pTarget); // TF_WEAPON_NONE means the player got pushed
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFFlameThrower::DeflectEntity(CBaseEntity *pTarget, CTFPlayer *pOwner, Vector &vecForward, Vector &vecCenter,
									Vector &vecSize)
{
	Assert(pTarget);
	Assert(pOwner);
	...
	if(!pTFPlayerVictim)
	{
		// We can't use OwnerEntity for grenades, because then the owner can't shoot them with his hitscan weapons (due
		// to collide rules) Thrower is used to store the person who threw the grenade, for damage purposes.
...
void CTFFlameThrower::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if(IsCarrierAlive() && (WeaponState() == WEAPON_IS_ACTIVE) && bLocalPlayerAmmo == true)
	{
		if(m_iWeaponState > FT_STATE_IDLE)
		{
			if((m_iWeaponState == FT_STATE_SECONDARY && GetPlayerOwner() != C_BasePlayer::GetLocalPlayer()) ||
...
int CTFFlameThrower::GetWorldModelIndex(void)
{
	...
	if(iParticleEffectIndex == 3)
	{
		...
		if(pPlayer && pPlayer->m_Shared.InCond(TF_COND_TAUNTING) &&
		   pPlayer->m_Shared.GetTauntIndex() == TAUNT_BASE_WEAPON)
		{
...
const char *CTFFlameThrower::FlameEffectName(bool bIsFirstPersonView)
{
	...
	switch(iParticleEffectIndex)
	{
		...
		case 2:
			return "flamethrower_giant_mvm";
		case 3:
			return (bIsFirstPersonView ? "flamethrower_rainbow_FP" : "flamethrower_rainbow");
		default:
...
const char *CTFFlameThrower::FlameCritEffectName(bool bIsFirstPersonView)
{
	...
	switch(iParticleEffectIndex)
	{
		...
		case 2:
			return "flamethrower_crit_giant_mvm";
		case 3:
			return (bIsFirstPersonView ? "flamethrower_rainbow_FP" : "flamethrower_rainbow");
		default:
...
void CTFFlameThrower::ClientEffectsThink(void)
{
	...
	SetContextThink(&CTFFlameThrower::ClientEffectsThink,
					gpGlobals->curtime + 0.1f + RandomFloat(1.0f, 5.0f) * flRageInverse, "EFFECTS_THINK");
}
...
bool CTFFlameThrower::RocketPackLaunch(int nAmmoCost)
{
	...
	const float flVertPushScale =
		(pOwner->GetFlags() & FL_ONGROUND) ? 1.2f : 0.25f; // Less vertical force while airborne
...
//-----------------------------------------------------------------------------
CTFFlameEntity *CTFFlameEntity::Create(const Vector &vecOrigin, const QAngle &vecAngles, CBaseEntity *pOwner,
									   float flSpeed, int iDmgType, float flDmgAmount, bool bAlwaysCritFromBehind,
									   bool bRandomize)
{
	CTFFlameEntity *pFlame =
		static_cast<CTFFlameEntity *>(CBaseEntity::Create("tf_flame", vecOrigin, vecAngles, pOwner));
	if(!pFlame)
		return NULL;
	...
	if(bRandomize)
	{
		pFlame->m_vecBaseVelocity += RandomVector(-velocity * iFlameSizeMult * tf_flamethrower_vecrand.GetFloat(),
												  velocity * iFlameSizeMult * tf_flamethrower_vecrand.GetFloat());
	}
	if(pOwner->GetOwnerEntity())
	{
	...
	virtual bool EnumEntity(IHandleEntity *pHandleEntity)
	{
		...
		else if(pEnt->IsBaseObject() && m_pShooter->GetTeamNumber() != pEnt->GetTeamNumber())
		{
			...
			m_Targets.AddToTail(pEnt);
				...
				m_pShooter->GetTeamNumber() != pEnt->GetTeamNumber() &&
				FClassnameIs(pEnt, "tf_robot_destruction_robot"))
		{
			// only add enemy robots
			m_Targets.AddToTail(pEnt);
		}
		else if(FClassnameIs(pEnt, "func_breakable") || FClassnameIs(pEnt, "tf_pumpkin_bomb") ||
				FClassnameIs(pEnt, "tf_merasmus_trick_or_treat_prop"))
		{
			m_Targets.AddToTail(pEnt);
		}
...
void CTFFlameEntity::FlameThink(void)
{
	...
	if(GetAbsOrigin() != m_vecPrevPos)
	{
		...
		FOR_EACH_VEC(eFlameEnum.GetTargets(), i)
		{
			...
			// if we're removing the flame this frame from hitting world, check if we hit this ent before hitting the
			// world
			if(bHitWorld)
			{
				trace_t trEnt;
		...
		{
			NDebugOverlay::SweptBox(m_vecPrevPos, GetAbsOrigin(), WorldAlignMins(), WorldAlignMaxs(), vec3_angle, 255,
									255, 0, 100, 5.0);
		...
		if(bHitWorld)
		{
			...
			{
				NDebugOverlay::SweptBox(m_vecInitialPos, GetAbsOrigin(), WorldAlignMins(), WorldAlignMaxs(), vec3_angle,
										255, 0, 0, 100, 3.0);
			}
...
void CTFFlameEntity::OnCollide(CBaseEntity *pOther)
{
	...
	float flDamage = m_flDmgAmount * RemapValClamped(flDistance, tf_flamethrower_maxdamagedist.GetFloat() / 2,
													 tf_flamethrower_maxdamagedist.GetFloat(), 1.0f, 0.70f);
		...
		if(pAttacker->IsPlayer() && pVictim)
		{
			...
			if(pPlayerAttacker && pPlayerAttacker->IsPlayerClass(TF_CLASS_PYRO))
			{
				...
				if(pVictim->m_Shared.InCond(TF_COND_TAUNTING))
				{
					...
					if(!pVictim->IsTauntInitiator() && pVictim->GetTauntEconItemView() &&
					   pVictim->GetTauntEconItemView()->GetItemDefinition() == flipTaunt)
					{
	...
	trace_t pTrace;
	UTIL_TraceLine(WorldSpaceCenter(), pOther->WorldSpaceCenter(), MASK_SOLID | CONTENTS_HITBOX, this,
				   COLLISION_GROUP_NONE, &pTrace);
...
CTFProjectile_Napalm *CTFProjectile_Napalm::Create(CBaseCombatCharacter *pOwner, CTFFlameThrower *pLauncher)
{
	if(pOwner)
	{
		...
		CTFProjectile_Napalm *pProjectile = static_cast<CTFProjectile_Napalm *>(
			CBaseEntity::Create("tf_projectile_napalm", pLauncher->GetVisualMuzzlePos(), pOwner->EyeAngles()));
		if(pProjectile)
		{
			...
			pProjectile->m_vecBaseVelocity += RandomVector(-flVelocity * tf_flamethrower_vecrand.GetFloat(),
														   flVelocity * tf_flamethrower_vecrand.GetFloat());
		...
		if(pThrower)
		{
			...
			if(InitialExplodeEffects(pThrower, pTrace))
			{
				...
				if(GetImpactEffect())
				{
					...
					{
						// TE_TFParticleEffect( filter, 0.f, GetImpactEffect(), pEnt->GetAbsOrigin(), vec3_angle, pEnt,
				...
				for(int i = 0; i < iEntities; ++i)
				{
					...
					UTIL_TraceLine(GetAbsOrigin(), pBaseCombatCharacter->GetAbsOrigin(),
								   (MASK_SHOT & ~(CONTENTS_HITBOX)), &traceFilter, &pImpactTrace);
...
bool CTFProjectile_Napalm::InitialExplodeEffects(CTFPlayer *pThrower, const trace_t *pTrace)
{
	...
	// Particle
	CPVSFilter filter(vecOrigin);
	TE_TFExplosion(filter, 0.0f, vecOrigin, pTrace->plane.normal, TF_WEAPON_FLAMETHROWER, kInvalidEHandleExplosion, -1,
				   SPECIAL1, INVALID_STRING_INDEX);
...
// Purpose: Direct hit
//-----------------------------------------------------------------------------
void CTFProjectile_Napalm::ExplodeEffectOnTarget(CTFPlayer *pThrower, CTFPlayer *pTarget,
												 CBaseCombatCharacter *pBaseTarget)
{
