...
public:
	...
	{
		return Obj().mission_name().c_str();
	}
	EMatchGroup GetMatchGroup() const
	{
		return Obj().has_match_group() ? (EMatchGroup)Obj().match_group() : k_nMatchGroup_Invalid;
	}
	...
	{
		return Obj().map_name().c_str();
	}
	GCSDK::PlayerGroupID_t GetGroupID() const
	{
		return Obj().lobby_id();
	}
	bool GetLateJoinEligible() const
	{
		return Obj().late_join_eligible();
	}
	...
	{
		return Obj().connect().c_str();
	}
	uint32_t GetLobbyMMVersion() const
	{
		return Obj().lobby_mm_version();
	}
