...
ConVar tf_scout_hype_pep_min_damage("tf_scout_hype_pep_min_damage", "5.0",
...
#ifdef STAGING_ONLY
ConVar tf_weapon_force_allow_inspect("tf_weapon_force_allow_inspect", "0", FCVAR_REPLICATED | FCVAR_ARCHIVE,
									 "Allow the inspect animation on any weapon");
#endif
...
#ifdef GAME_DLL
void *SendProxy_SendActiveLocalWeaponDataTable(const SendProp *pProp, const void *pStruct, const void *pVarData,
											   CSendProxyRecipients *pRecipients, int objectID);
void *SendProxy_SendNonLocalWeaponDataTable(const SendProp *pProp, const void *pStruct, const void *pVarData,
											CSendProxyRecipients *pRecipients, int objectID);
#endif
...
#if defined(CLIENT_DLL)
	RecvPropTime(RECVINFO(m_flLastCritCheckTime)), RecvPropTime(RECVINFO(m_flReloadPriorNextFire)),
		RecvPropTime(RECVINFO(m_flLastFireTime)), RecvPropTime(RECVINFO(m_flEffectBarRegenTime)),
		RecvPropFloat(RECVINFO(m_flObservedCritChance)),
#else
	SendPropTime(SENDINFO(m_flLastCritCheckTime)), SendPropTime(SENDINFO(m_flReloadPriorNextFire)),
		SendPropTime(SENDINFO(m_flLastFireTime)), SendPropTime(SENDINFO(m_flEffectBarRegenTime)),
...
// Client specific.
#ifdef CLIENT_DLL
	RecvPropBool(RECVINFO(m_bLowered)), RecvPropInt(RECVINFO(m_iReloadMode)), RecvPropBool(RECVINFO(m_bResetParity)),
		RecvPropBool(RECVINFO(m_bReloadedThroughAnimEvent)), RecvPropBool(RECVINFO(m_bDisguiseWeapon)),
		...
		RecvPropFloat(RECVINFO(m_flEnergy)), RecvPropEHandle(RECVINFO(m_hExtraWearable)),
		RecvPropEHandle(RECVINFO(m_hExtraWearableViewModel)), RecvPropBool(RECVINFO(m_bBeingRepurposedForTaunt)),
		RecvPropInt(RECVINFO(m_nKillComboClass)), RecvPropInt(RECVINFO(m_nKillComboCount)),
		RecvPropFloat(RECVINFO(m_flInspectAnimTime)), RecvPropInt(RECVINFO(m_nInspectStage)),
#else
	// Server specific.
	SendPropBool(SENDINFO(m_bLowered)), SendPropBool(SENDINFO(m_bResetParity)),
		SendPropInt(SENDINFO(m_iReloadMode), 4, SPROP_UNSIGNED), SendPropBool(SENDINFO(m_bReloadedThroughAnimEvent)),
		SendPropBool(SENDINFO(m_bDisguiseWeapon)),
		SendPropDataTable("LocalActiveTFWeaponData", 0, &REFERENCE_SEND_TABLE(DT_LocalTFWeaponData),
						  SendProxy_SendActiveLocalWeaponDataTable),
		SendPropDataTable("NonLocalTFWeaponData", 0, &REFERENCE_SEND_TABLE(DT_TFWeaponDataNonLocal),
						  SendProxy_SendNonLocalWeaponDataTable),
		SendPropFloat(SENDINFO(m_flEnergy)), SendPropEHandle(SENDINFO(m_hExtraWearable)),
		SendPropEHandle(SENDINFO(m_hExtraWearableViewModel)), SendPropBool(SENDINFO(m_bBeingRepurposedForTaunt)),
		SendPropInt(SENDINFO(m_nKillComboClass), 4, SPROP_UNSIGNED),
		SendPropInt(SENDINFO(m_nKillComboCount), 2, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_flInspectAnimTime)),
...
ConVar tf_weapon_criticals("tf_weapon_criticals", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
	...
	if(flClip >= 0)
	{
		...
		if(pPlayer)
		{
			...
			if(IsBlastImpactWeapon())
			{
				...
				{
					nProjectiles +=
						Min(pPlayer->m_Shared.GetDecapitations(), iClipSizeOnKills); // max extra projectiles
				}
...
const char *CTFWeaponBase::GetViewModel(int iViewModel) const
{
	...
	if(pPlayer)
	{
		// CALL_ATTRIB_HOOK_FLOAT_ON_OTHER( pPlayer, iHandModelIndex, override_hand_model_index );		// this is a cleaner
		// way of doing it, but...
		CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(
			pPlayer, iHandModelIndex,
			wrench_builds_minisentry); // ...the gunslinger is the only thing that uses this attribute for now
	}
...
{
	return iActivity == GetInspectActivity(INSPECT_START) || iActivity == GetInspectActivity(INSPECT_IDLE) ||
		   iActivity == GetInspectActivity(INSPECT_END);
}
...
bool CTFWeaponBase::CanHolster(void) const
{
	...
	CTFPlayer *pPlayer = ToTFPlayer(GetOwner());
	if(pPlayer &&
	   (pPlayer->GetActiveWeapon() != this || gpGlobals->curtime >= pPlayer->m_Shared.m_flFirstPrimaryAttack))
	{
...
bool CTFWeaponBase::Holster(CBaseCombatWeapon *pSwitchingTo)
{
	...
	if(pPlayer &&
	   (pPlayer->GetActiveWeapon() != this || gpGlobals->curtime >= pPlayer->m_Shared.m_flFirstPrimaryAttack))
	{
		if(IsHonorBound() && pPlayer->m_Shared.m_iKillCountSinceLastDeploy == 0 && pPlayer->GetHealth() > 0 &&
		   pPlayer->IsAlive())
		{
			pPlayer->TakeDamage(CTakeDamageInfo(pPlayer, pPlayer, vec3_origin, pPlayer->WorldSpaceCenter(), 50.f,
												GetDamageType() | DMG_PREVENT_PHYSICS_FORCE));
		}
	}
#endif
...
bool CTFWeaponBase::Deploy(void)
{
	...
	if(bDeploy)
	{
		...
		// don't apply mult_switch_from_wep_deploy_time attribute if the last weapon hasn't been deployed for more than
		// 0.67 second to match to weapon script switch time unless the player latched to a hook target, then allow
		// switching right away
		CTFWeaponBase *pLastWeapon = dynamic_cast<CTFWeaponBase *>(pPlayer->GetLastWeapon());
		if(pPlayer->GetGrapplingHookTarget() != NULL ||
		   (pLastWeapon && gpGlobals->curtime - pLastWeapon->m_flLastDeployTime > flWeaponSwitchTime))
		{
		...
		float flPlaybackRate = Clamp((1.f / flDeployTimeMultiplier) * (0.67f / flWeaponSwitchTime), -4.f,
									 12.f); // clamp between the range that's defined in send table
...
ETFDmgCustom CTFWeaponBase::GetPenetrateType() const
{
	...
	return iMode >= 1 ? TF_DMG_CUSTOM_PENETRATE_ALL_PLAYERS : TF_DMG_CUSTOM_NONE;
}
...
bool CTFWeaponBase::CalcIsAttackCriticalHelper()
{
	...
	if(bRapidFire)
	{
		...
		// calculate the amount of time, on average, that we want to NOT fire crit shots for in order to achieve the
		// total crit chance we want
		float flNonCritDuration = (flCritDuration / flTotalCritChance) - flCritDuration;
		// calculate the chance per second of non-crit fire that we should transition into critting such that on average
		// we achieve the total crit chance we want
	...
	if(bCrit)
	{
		...
		if(bAntiCheat)
		{
			...
			{
				flDamage *=
					TF_DAMAGE_CRIT_DURATION_RAPID / m_pWeaponInfo->GetWeaponData(m_iWeaponMode).m_flTimeFireDelay;
...
float CTFWeaponBase::ApplyFireDelay(float flDelay) const
{
	...
	if(pPlayer && pPlayer->m_Shared.GetCarryingRuneType() == RUNE_HASTE)
	{
		flDelayMult *= 0.5f;
			...
			(pPlayer->m_Shared.GetCarryingRuneType() == RUNE_KING || pPlayer->m_Shared.InCond(TF_COND_KING_BUFFED)))
	{
		flDelayMult *= 0.75f;
	}
...
bool CTFWeaponBase::DefaultReload(int iClipSize1, int iClipSize2, int iActivity)
{
	...
	{
		// We consider the reload finished 0.2 sec before the anim is, so that players don't keep accidentally aborting
		// their reloads
...
void CTFWeaponBase::ItemBusyFrame(void)
{
	...
	if((pOwner->m_nButtons & IN_ATTACK) && Clip1() > 0)
	{
		...
		{
			// We don't let them abort before the next fire point, so they can't use reload to fire before they would
			// have fired if they hadn't reloaded
...
void CTFWeaponBase::ItemPostFrame(void)
{
	...
	if(!AutoFiresFullClip() && pOwner->ShouldAutoReload() && UsesClipsForAmmo1() &&
	   !(pOwner->m_nButtons & (IN_ATTACK | IN_ATTACK2)) && bNeedsReload)
	{
...
int CTFWeaponBase::GetInspectActivity(TFWeaponInspectStage inspectStage)
{
	static int s_inspectActivities[][INSPECT_STAGE_COUNT] = {
		...
		{ACT_PRIMARY_VM_INSPECT_START, ACT_PRIMARY_VM_INSPECT_IDLE, ACT_PRIMARY_VM_INSPECT_END},
		// LOADOUT_POSITION_SECONDARY
		{ACT_SECONDARY_VM_INSPECT_START, ACT_SECONDARY_VM_INSPECT_IDLE, ACT_SECONDARY_VM_INSPECT_END},
		// LOADOUT_POSITION_MELEE
		{ACT_MELEE_VM_INSPECT_START, ACT_MELEE_VM_INSPECT_IDLE, ACT_MELEE_VM_INSPECT_END},
	};
	...
	if(pVictim)
	{
		...
		if(fKillComboFireRateBoost != 0.0f)
		{
			...
			if(GetKillComboCount() == 1)
			{
				...
				if(pOwner)
				{
					pOwner->SpeakConceptIfAllowed(
						MP_CONCEPT_COMBO_KILLED,
						CFmtStr("victimclass:%s", g_aPlayerClassNames_NonLocalized[nClassIndex]).Access());
				}
			}
		}
	}
...
void TE_DynamicLight(IRecipientFilter &filter, float delay, const Vector *org, int r, int g, int b, int exponent,
					 float radius, float time, float decay, int nLightIndex = LIGHT_INDEX_TE_DYNAMIC);
...
void CTFWeaponBase::CreateMuzzleFlashEffects(C_BaseEntity *pAttachEnt, int nIndex)
{
	...
	// (this uses IsFirstPersonView() rather than UsingViewModel() because even when NOT using the viewmodel, in
	// 1st-person mode we still want the 1st-person muzzleflash effect)
	...
	if(iMuzzleFlashAttachment > 0 && (pszMuzzleFlashEffect || pszMuzzleFlashModel || pszMuzzleFlashParticleEffect))
	{
		...
		/*
				...
				TE_DynamicLight( filter, 0.0f, &vecOrigin, 255, 192, 64, 5, 70.0f, 0.05f, 70.0f / 0.05f,
		   LIGHT_INDEX_MUZZLEFLASH );
		*/
		...
		if(pszMuzzleFlashModel)
		{
			...
			{
				m_hMuzzleFlashModel[nIndex] = C_MuzzleFlashModel::CreateMuzzleFlashModel(
					pszMuzzleFlashModel, pAttachEnt, iMuzzleFlashAttachment, flEffectLifetime);
...
int CTFWeaponBase::InternalDrawModel(int flags)
{
	...
	bool bUseInvulnMaterial = (bNotViewModel && pOwner && pOwner->m_Shared.IsInvulnerable() &&
							   (!pOwner->m_Shared.InCond(TF_COND_INVULNERABLE_HIDE_UNLESS_DAMAGED) ||
								gpGlobals->curtime < pOwner->GetLastDamageTime() + 2.0f));
...
void CTFWeaponBase::OnDataChanged(DataUpdateType_t type)
{
	...
	if(pOwner && pOwner->IsAlive() && !pOwner->m_Shared.IsLoser() &&
	   !pOwner->m_Shared.InCond(TF_COND_COMPETITIVE_LOSER) && (pOwner->GetActiveWeapon() == this))
	{
...
int CTFWeaponBase::GetWorldModelIndex(void)
{
	...
	if(pPlayer)
	{
		...
		if(pPlayer->m_Shared.InCond(TF_COND_TAUNTING) &&
		   (pPlayer->m_Shared.GetTauntIndex() == TAUNT_MISC_ITEM || pPlayer->m_Shared.GetTauntIndex() == TAUNT_LONG))
		{
...
Activity CTFWeaponBase::TranslateViewmodelHandActivityInternal(Activity actBase)
{
	...
	if(pEconItemView && pEconItemView->IsValid() && GetOwnerEntity())
	{
		Activity translatedActivity =
			pEconItemView->GetStaticData()->GetActivityOverride(GetOwnerEntity()->GetTeamNumber(), actBase);
...
bool CTFWeaponBase::CanFireRandomCriticalShot(float flCritChance)
{
	...
	if(pPlayerStats)
	{
		...
		m_flObservedCritChance.Set(flNormalizedDamage /
								   (flNormalizedDamage + (nTotalDamage - nRandomRangedCritDamage)));
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFWeaponBase::OnFireEvent(C_BaseViewModel *pViewModel, const Vector &origin, const QAngle &angles, int event,
								const char *options)
{
	if(event == 6002 && ShouldEjectBrass())
	{
		...
		data.m_nDamageType =
			GetAttributeContainer()->GetItem() ? GetAttributeContainer()->GetItem()->GetItemDefIndex() : 0;
...
ConVar tf_dev_health_on_damage_recover_percentage("tf_dev_health_on_damage_recover_percentage", "0.35",
												  FCVAR_DEVELOPMENTONLY);

void CTFWeaponBase::ApplyOnHitAttributes(CBaseEntity *pVictimBaseEntity, CTFPlayer *pAttacker,
										 const CTakeDamageInfo &info)
{
	...
	if(iModAmmoOnHit > 0)
	{
		...
		if(pVictim && pVictim->IsPlayerClass(TF_CLASS_SPY) && pVictim->m_Shared.InCond(TF_COND_DISGUISED) &&
	...
	if(flChargeRefill > 0)
	{
		pAttacker->m_Shared.SetDemomanChargeMeter(pAttacker->m_Shared.GetDemomanChargeMeter() +
												  flChargeRefill * 100.0f);
	}
	...
	if(pVictim)
	{
		if(pVictim->m_Shared.InCond(TF_COND_MAD_MILK))
		{
			...
			if(pProvider)
			{
				...
				CWeaponMedigun *pMedigun =
					static_cast<CWeaponMedigun *>(pProvider->Weapon_OwnsThisID(TF_WEAPON_MEDIGUN));
				if(pMedigun)
				{
					int iHealedAmount =
						Max(Min((int)pAttacker->GetMaxHealth() - (int)pAttacker->GetHealth(), nAmount), 0);
	...
	{
		float fHype =
			MIN(tf_scout_hype_pep_max.GetFloat(), pAttacker->m_Shared.GetScoutHypeMeter() +
													  (MAX(tf_scout_hype_pep_min_damage.GetFloat(), info.GetDamage()) /
													   tf_scout_hype_pep_mod.GetFloat()));
	...
	if(pVictim)
	{
		...
		if((RandomFloat() < flProcChance))
		{
			...
			float flStunAmt = 1.f;
			pVictim->m_Shared.StunPlayer(flStunDuration, flStunAmt,
										 TF_STUN_MOVEMENT | TF_STUN_CONTROLS | TF_STUN_NO_EFFECTS, pAttacker);
			...
			for(int i = 0; i < nCount; i++)
			{
				...
				// Stun
				pTFPlayer->m_Shared.StunPlayer(flStunDuration, flStunAmt,
											   TF_STUN_MOVEMENT | TF_STUN_CONTROLS | TF_STUN_NO_EFFECTS, pAttacker);
				...
				TE_TFParticleEffectComplex(filter, 0.0f, "dxhr_arm_muzzleflash", vStart, QAngle(0, 0, 0), NULL,
										   &controlPoint, pTFPlayer, PATTACH_CUSTOMORIGIN);
	...
	*/
	...
	if(pVictim)
	{
		...
		if(!pVictim->m_Shared.InCond(TF_COND_HEALTH_BUFF))
		{
			...
			if(flSlowEnemy)
			{
				if(RandomFloat() < flSlowEnemy)
				{
					...
					float flStunAmount =
						RemapValClamped(vecDistance.LengthSqr(), (512.0f * 512.0f), (1536.0f * 1536.0f), 0.60f, 0.0f);
		...
		if(iMarkForDeath)
		{
			...
			if(pAttacker->m_pMarkedForDeathTarget != NULL &&
			   pAttacker->m_pMarkedForDeathTarget->m_Shared.InCond(TF_COND_MARKEDFORDEATH))
			{
				pAttacker->m_pMarkedForDeathTarget->m_Shared.RemoveCond(TF_COND_MARKEDFORDEATH);
			}

			float flDuration = pVictim->IsMiniBoss() ? tf_dev_marked_for_death_lifetime.GetFloat() / 2
													 : tf_dev_marked_for_death_lifetime.GetFloat();
		...
		if(iStunWaistHighAirborne > 0 && bIsVictimAirborne)
		{
			...
			{
				// right in the jimmy!
				pVictim->m_Shared.StunPlayer(iStunWaistHighAirborne, 0.5f, TF_STUN_LOSER_STATE | TF_STUN_BOTH,
											 pAttacker);
	...
	if(pVictim != m_hLastDrainVictim || m_lastDrainVictimTimer.IsElapsed())
	{
		...
		if(iSubtractVictimMedigunChargeOnHit > 0)
		{
			...
			if(pMedigun && !pMedigun->IsReleasingCharge())
			{
				...
				{
					iSubtractVictimMedigunChargeOnHit *=
						RemapValClamped(toEnt.LengthSqr(), (512.0f * 512.0f), (1536.0f * 1536.0f), 1.0f, 0.0f);
				}
		...
		if(!pVictim->m_Shared.InCond(TF_COND_DISGUISED) && !pVictim->m_Shared.IsStealthed())
		{
			if(bDidDrain)
			{
				DispatchParticleEffect("drg_pomson_impact_drain", PATTACH_POINT, pVictim, "head", GetParticleColor(1),
									   GetParticleColor(2));
				if(pAttacker)
				{
...
//-----------------------------------------------------------------------------
void CTFWeaponBase::GetProjectileFireSetup(CTFPlayer *pPlayer, Vector vecOffset, Vector *vecSrc, QAngle *angForward,
...
bool CTFWeaponBase::DeflectProjectiles()
{
	...
	int count = UTIL_EntitiesInBox(pObjects, maxCollectedEntities, vecCenter - vecSize, vecCenter + vecSize,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFWeaponBase::DeflectPlayer(CTFPlayer *pTarget, CTFPlayer *pOwner, Vector &vecForward, Vector &vecCenter,
								  Vector &vecSize)
{
	return true;
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFWeaponBase::DeflectEntity(CBaseEntity *pTarget, CTFPlayer *pOwner, Vector &vecForward, Vector &vecCenter,
								  Vector &vecSize)
{
	Assert(pTarget);
	Assert(pOwner);
...
// Purpose: Static deflection helper.
//-----------------------------------------------------------------------------
void CTFWeaponBase::SendObjectDeflectedEvent(CTFPlayer *pNewOwner, CTFPlayer *pPrevOwner, int iWeaponID,
											 CBaseAnimating *pObject)
{
...
void CTFWeaponBase::ApplyItemRegen(void)
{
	...
	if(m_flRegenTime > 1.0f)
	{
		...
		{
			pOwner->TakeDamage(
				CTakeDamageInfo(pOwner, pOwner, vec3_origin, WorldSpaceCenter(), (int)flRegenAmount * -1, DMG_GENERIC));
...
void CTFWeaponBase::UpdateWeaponBodyGroups(CTFPlayer *pPlayer, bool bHandleDeployedBodygroups)
{
	...
	for(int i = 0; i < pPlayer->WeaponCount(); i++)
	{
		...
		const bool bHideBodygroupsDeployedOnly =
			pScriptItem ? pScriptItem->GetStaticData()->GetHideBodyGroupsDeployedOnly() : false;
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		const CItemLevelingDefinition *pLevelDef =
			GetItemSchema()->GetItemLevelForScore(pszLevelBlockName, msg.Body().num_kills());
		if(!pLevelDef)
			return true;
		...
		{
			NotificationQueue_Add(new CTFKillEaterNotification(
				CSteamID(msg.Body().killer_account_id(), GetUniverse(), k_EAccountTypeIndividual), wszWeaponName,
				wszLevelName));
		}
		...
		if(pHUDChat)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszNotification,
												  g_pVGuiLocalize->Find("#TF_HUD_Event_KillEater_Leveled_Chat"), 3,
												  wszPlayerName, wszWeaponName, wszLevelName);
...
GC_REG_JOB(GCSDK::CGCClient, CGCPlayerKilledResponse, "CGCPlayerKilledResponse", k_EMsgGC_IncrementKillCountResponse,
...
{
#ifdef STAGING_ONLY
	if(iWeaponID == TF_WEAPON_SNIPERRIFLE || iWeaponID == TF_WEAPON_SNIPERRIFLE_DECAP ||
	   iWeaponID == TF_WEAPON_SNIPERRIFLE_CLASSIC || iWeaponID == TF_WEAPON_SNIPERRIFLE_REVOLVER)
#else
	if(iWeaponID == TF_WEAPON_SNIPERRIFLE || iWeaponID == TF_WEAPON_SNIPERRIFLE_DECAP ||
...
void CTFWeaponBase::EffectBarRegenFinished(void)
{
	...
	if(pPlayer && (pPlayer->GetAmmoCount(GetEffectBarAmmo()) < pPlayer->GetMaxAmmo(GetEffectBarAmmo())))
	{
		...
		if(pPlayer->GetAmmoCount(GetEffectBarAmmo()) < pPlayer->GetMaxAmmo(GetEffectBarAmmo()))
#else
		// On the client, we assume we'll get 1 more ammo as soon as the server updates us, so only restart if that
		// still won't make us full.
...
void CTFWeaponBase::AddStatTrakModel(CEconItemView *pItem, int nStatTrakType, AccountID_t holderAcctId)
{
	...
	if(GetViewmodelAttachment())
	{
		...
		if(!(m_viewmodelStatTrakAddon && m_viewmodelStatTrakAddon.Get() && m_viewmodelStatTrakAddon->GetMoveParent()))
		{
			...
			if(pStatTrakEnt)
			{
				...
				{
					pStatTrakEnt->SetBodygroup(
						1, 1);		  // use a special mirror-image stattrak module that appears correct for lefties
	...
	if(!(m_worldmodelStatTrakAddon && m_worldmodelStatTrakAddon.Get() && m_worldmodelStatTrakAddon->GetMoveParent()))
	{
		...
		if(pStatTrakEnt)
		{
			...
			//	//if ( !cl_flipviewmodels.GetBool() )
			//	//{
			//	//	pStatTrakEnt->SetBodygroup( 0, 1 ); // use a special mirror-image stattrak module that appears
			//correct for lefties
			//	//}
