...
extern const char
	*g_aRawPlayerClassNamesShort[TF_CLASS_MENU_BUTTONS]; // raw class names, useful for formatting resource/material
														 // filenames - "heavy" instead of "heavyweapons" and "demo"
														 // instead of "demoman"
extern const char *
	g_aRawPlayerClassNames[TF_CLASS_MENU_BUTTONS]; // raw class names, useful for formatting resource/material filenames
...
inline bool IsTauntDmg(int iType)
{
	return (iType == TF_DMG_CUSTOM_TAUNTATK_HADOUKEN || iType == TF_DMG_CUSTOM_TAUNTATK_HIGH_NOON ||
			...
			iType == TF_DMG_CUSTOM_TAUNTATK_ALLCLASS_GUITAR_RIFF || iType == TF_DMG_CUSTOM_TAUNTATK_ENGINEER_ARM_KILL);
}
inline bool IsDOTDmg(int iType)
{
	if(iType == TF_DMG_CUSTOM_BURNING || iType == TF_DMG_CUSTOM_BURNING_FLARE || iType == TF_DMG_CUSTOM_BURNING_ARROW ||
...
#define TF_SCORE_CAPTURE_POWERUPMODE \
	10 // With these CTF rules capturing flags is tougher, hence the higher scoring for flag events
...
{
	return AlmostEqual(flCurAttributeValue, flCap) || (flAttrDeltaValue > 0 && flCurAttributeValue >= flCap) ||
