...
void RegisterUserMessages()
{
	...
	usermessages->Register("MVMPlayerUpgradedEvent",
						   9); // PlayerIdx(1), WaveIdx(1), ItemDef(2), AttributeDef(2), Quality(1), cost(2)
	usermessages->Register("MVMVictory", 2); // IsKicking(1), time(1) (seconds)
	usermessages->Register("MVMWaveChange",
						   15); // ServerWaveID(2), deaths(1), damageBot(4), damageGiant(4), damageTank(4)
	...
	usermessages->Register(
		"EOTLDuckEvent",
		7); // IsCreated (vs IsPickedUp), ID of Creator, ID of Victim, ID of Toucher, iDuckTeam, Count, IsGolden

	usermessages->Register("PlayerPickupWeapon", -1);

	usermessages->Register("QuestObjectiveCompleted",
						   8 + 2 + 2 + 2); // ItemID(8) + StandardPoints(2) + BonusPoints(2) + ObjectiveDef(2)
