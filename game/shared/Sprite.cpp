...
BEGIN_DATADESC(CSprite)
	...
	DEFINE_FIELD(m_flLastTime, FIELD_TIME), DEFINE_FIELD(m_flMaxFrame, FIELD_FLOAT),
		DEFINE_FIELD(m_hAttachedToEntity, FIELD_EHANDLE), DEFINE_FIELD(m_nAttachment, FIELD_INTEGER),
		DEFINE_FIELD(m_flDieTime, FIELD_TIME),

		DEFINE_FIELD(m_nBrightness, FIELD_INTEGER), DEFINE_FIELD(m_flBrightnessTime, FIELD_FLOAT),
		...
		DEFINE_FIELD(m_flScaleTime, FIELD_FLOAT), DEFINE_FIELD(m_flStartScale, FIELD_FLOAT),
		DEFINE_FIELD(m_flDestScale, FIELD_FLOAT), DEFINE_FIELD(m_flScaleTimeStart, FIELD_TIME),
		DEFINE_FIELD(m_nStartBrightness, FIELD_INTEGER), DEFINE_FIELD(m_nDestBrightness, FIELD_INTEGER),
		DEFINE_FIELD(m_flBrightnessTimeStart, FIELD_TIME), DEFINE_FIELD(m_bWorldSpaceScale, FIELD_BOOLEAN),

		// Function Pointers
		DEFINE_FUNCTION(AnimateThink), DEFINE_FUNCTION(ExpandThink), DEFINE_FUNCTION(AnimateUntilDead),
...
BEGIN_PREDICTION_DATA(CSprite)
	...
	DEFINE_PRED_FIELD(m_hAttachedToEntity, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE),
		...
		DEFINE_FIELD(m_flLastTime, FIELD_FLOAT), DEFINE_FIELD(m_flMaxFrame, FIELD_FLOAT),
...
#if !defined(CLIENT_DLL)
	SendPropEHandle(SENDINFO(m_hAttachedToEntity)), SendPropInt(SENDINFO(m_nAttachment), 8),
		...
		SendPropInt(SENDINFO(m_nBrightness), 8, SPROP_UNSIGNED), SendPropBool(SENDINFO(m_bWorldSpaceScale)),
#else
	RecvPropEHandle(RECVINFO(m_hAttachedToEntity)), RecvPropInt(RECVINFO(m_nAttachment)),
		RecvPropFloat(RECVINFO(m_flScaleTime)), RecvPropFloat(RECVINFO(m_flSpriteScale), 0, RecvProxy_SpriteScale),
		RecvPropFloat(RECVINFO(m_flSpriteFramerate)), RecvPropFloat(RECVINFO(m_flGlowProxySize)),
...
#endif //#ifdef PORTAL
		RecvPropFloat(RECVINFO(m_flBrightnessTime)), RecvPropInt(RECVINFO(m_nBrightness)),
...
// Output : CSprite
//-----------------------------------------------------------------------------
CSprite *CSprite::SpriteCreatePredictable(const char *module, int line, const char *pSpriteName, const Vector &origin,
										  bool animate)
{
	...
	void CSpriteOriented::Spawn(void)
{
	...
	BaseClass::Spawn();
	// ORIENTED sprites "forward" vector points in the players "view" direction, not the direction "out" from the sprite
	// (gah)
