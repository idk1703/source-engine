...
BEGIN_DATADESC(CPoseController)
	DEFINE_AUTO_ARRAY(m_hProps, FIELD_EHANDLE), DEFINE_AUTO_ARRAY(m_chPoseIndex, FIELD_CHARACTER),
		DEFINE_FIELD(m_bDisablePropLookup, FIELD_BOOLEAN), DEFINE_FIELD(m_bPoseValueParity, FIELD_BOOLEAN),
		// Keys
...
SendPropArray3(SENDINFO_ARRAY3(m_hProps), SendPropEHandle(SENDINFO_ARRAY(m_hProps))),
	SendPropArray3(SENDINFO_ARRAY3(m_chPoseIndex),
				   SendPropInt(SENDINFO_ARRAY(m_chPoseIndex), 5,
							   SPROP_UNSIGNED)), // bits sent must be enough to represent MAXSTUDIOPOSEPARAM
	SendPropBool(SENDINFO(m_bPoseValueParity)), SendPropFloat(SENDINFO(m_fPoseValue), 11, 0, 0.0f, 1.0f),
...
void CPoseController::InputGetFMod(inputdata_t &inputdata)
{
	DevMsg("FMod values for pose controller %s\nTYPE: %i\nTIME OFFSET: %f\nRATE: %f\nAMPLITUDE: %f\n",
		   STRING(GetEntityName()), m_nFModType.Get(), m_fFModTimeOffset.Get(), m_fFModRate.Get(),
...
RecvPropArray3(RECVINFO_ARRAY(m_hProps), RecvPropEHandle(RECVINFO(m_hProps[0]))),
	...
	RecvPropBool(RECVINFO(m_bPoseValueParity)), RecvPropFloat(RECVINFO(m_fPoseValue)),
	RecvPropFloat(RECVINFO(m_fInterpolationTime)), RecvPropBool(RECVINFO(m_bInterpolationWrap)),
	RecvPropFloat(RECVINFO(m_fCycleFrequency)), RecvPropInt(RECVINFO(m_nFModType)),
	RecvPropFloat(RECVINFO(m_fFModTimeOffset)), RecvPropFloat(RECVINFO(m_fFModRate)),
...
void C_PoseController::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if(m_bOldPoseValueParity != m_bPoseValueParity)
	{
		...
		if(m_bInterpolationWrap)
		{
			...
			m_PoseTransitionValue.Init(
				((fabsf(fInterpForward) < fabsf(fInterpBackward)) ? (fInterpForward) : (fInterpBackward)), 0.0f,
...
void CPoseController::SetCurrentPose(float fCurrentPoseValue)
{
	for(int iPropNum = 0; iPropNum < MAX_POSE_CONTROLLED_PROPS; ++iPropNum)
	{
		...
		if(pProp)
		{
			...
			pProp->SetPoseParameter(m_chPoseIndex[iPropNum],
									fPoseValueMin + fCurrentPoseValue * (fPoseValueMax - fPoseValueMin));
		}
	}
}