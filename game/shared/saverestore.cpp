...
CSave::CSave(CSaveRestoreData *pdata) : m_pData(pdata), m_pGameInfo(pdata), m_bAsync(pdata->bAsync)
{
	...
	for(int iCount = 0; iCount < count; ++iCount)
	{
		switch(fieldType)
		{
			...
			case FIELD_COLOR32:
			{
				...
				Q_snprintf(szTempBuf, sizeof(szTempBuf), "(%d %d %d %d)", (int)pColor[0], (int)pColor[1],
						   (int)pColor[2], (int)pColor[3]);
				Q_strncat(szBuf, szTempBuf, sizeof(szTempBuf), COPY_ALL_CHARACTERS);
			}
			case FIELD_EMBEDDED:
			case FIELD_CUSTOM:
...
bool CSave::ShouldSaveField(const void *pData, typedescription_t *pField)
{
	...
	switch(pField->fieldType)
	{
		...
		case FIELD_CUSTOM:
		{
			...
			SaveRestoreFieldInfo_t fieldInfo = {const_cast<void *>(pData),
...
bool CSave::WriteBasicField(const char *pname, void *pData, datamap_t *pRootMap, typedescription_t *pField)
{
	switch(pField->fieldType)
	{
		...
		case FIELD_EMBEDDED:
		{
			AssertMsg(((pField->flags & FTYPEDESC_PTR) == 0) || (pField->fieldSize == 1),
					  "Arrays of embedded pointer types presently unsupported by save/restore");
		...
		case FIELD_CUSTOM:
		{
			...
			SaveRestoreFieldInfo_t fieldInfo = {pData, ((char *)pData) - pField->fieldOffset[TD_OFFSET_NORMAL], pField};
...
int CSave::WriteFields(const char *pname, const void *pBaseData, datamap_t *pRootMap, typedescription_t *pFields,
					   int fieldCount)
{
...
CRestore::CRestore(CSaveRestoreData *pdata) : m_pData(pdata), m_pGameInfo(pdata), m_global(0), m_precache(true)
{
...
void CRestore::ReadBasicField(const SaveRestoreRecordHeader_t &header, void *pDest, datamap_t *pRootMap,
							  typedescription_t *pField)
{
	switch(pField->fieldType)
	{
		...
		case FIELD_CUSTOM:
		{
			...
			SaveRestoreFieldInfo_t fieldInfo = {pDest, ((char *)pDest) - pField->fieldOffset[TD_OFFSET_NORMAL], pField};
...
void CRestore::ReadField(const SaveRestoreRecordHeader_t &header, void *pDest, datamap_t *pRootMap,
						 typedescription_t *pField)
{
	if(pField->fieldType <= FIELD_CUSTOM)
...
typedescription_t *CRestore::FindField(const char *pszFieldName, typedescription_t *pFields, int fieldCount,
									   int *pCookie)
{
...
int CRestore::ReadFields(const char *pname, void *pBaseData, datamap_t *pRootMap, typedescription_t *pFields,
						 int fieldCount)
{
	...
	for(i = 0; i < nFieldsSaved; i++)
	{
		...
		typedescription_t *pField =
			FindField(m_pData->StringFromSymbol(header.symbol), pFields, fieldCount, &searchCookie);
...
void CRestore::ReadGameField(const SaveRestoreRecordHeader_t &header, void *pDest, datamap_t *pRootMap,
							 typedescription_t *pField)
{
	switch(pField->fieldType)
	{
...
BEGIN_SIMPLE_DATADESC(entitytable_t)
	DEFINE_FIELD(id, FIELD_INTEGER), DEFINE_FIELD(edictindex, FIELD_INTEGER),
		...
		DEFINE_FIELD(location, FIELD_INTEGER), DEFINE_FIELD(size, FIELD_INTEGER), DEFINE_FIELD(flags, FIELD_INTEGER),
		DEFINE_FIELD(classname, FIELD_STRING), DEFINE_FIELD(globalname, FIELD_STRING),
		DEFINE_FIELD(landmarkModelSpace, FIELD_VECTOR), DEFINE_FIELD(modelname, FIELD_STRING),
END_DATADESC()
...
void CEntitySaveRestoreBlockHandler::Save(ISave *pSave)
{
	...
	for(int i = 0; i < pSaveData->NumEntities(); i++)
	{
		...
		if(pEnt && !(pEnt->ObjectCaps() & FCAP_DONT_SAVE))
		{
			MDLCACHE_CRITICAL_SECTION();
#if !defined(CLIENT_DLL)
			AssertMsg(!pEnt->edict() || (pEnt->m_iClassname != NULL_STRING && (STRING(pEnt->m_iClassname)[0] != 0) &&
			...
			pEntInfo->size =
				pSave->GetWritePos() - pEntInfo->location; // Size of entity block is data size written to block
...
void CEntitySaveRestoreBlockHandler::WriteSaveHeaders(ISave *pSave)
{
	...
	for(int i = 0; i < pSaveData->NumEntities(); i++)
		pSave->WriteFields("ETABLE", pSaveData->GetEntityInfo(i), NULL, entitytable_t::m_DataMap.dataDesc,
...
void CEntitySaveRestoreBlockHandler::ReadRestoreHeaders(IRestore *pRestore)
{
	...
	entitytable_t *pEntityTable =
		(entitytable_t *)engine->SaveAllocMemory((sizeof(entitytable_t) * nEntities), sizeof(char));
	if(!pEntityTable)
	{
		return;
	}
	...
	for(int i = 0; i < pSaveData->NumEntities(); i++)
		pRestore->ReadFields("ETABLE", pSaveData->GetEntityInfo(i), NULL, entitytable_t::m_DataMap.dataDesc,
							 entitytable_t::m_DataMap.dataNumFields);
}
...
void CEntitySaveRestoreBlockHandler::Restore(IRestore *pRestore, bool createPlayers)
{
	...
	for(i = 0; i < pSaveData->NumEntities(); i++)
	{
		...
		// BUGBUG: JAY: Disable ragdolls across transitions until PVS/solid check & client entity patch file are
		// implemented
...
#endif
	entitytable_t *pEntityTable =
		(entitytable_t *)engine->SaveAllocMemory((sizeof(entitytable_t) * number_of_entities), sizeof(char));
	if(!pEntityTable)
		return false;
	...
	if(pReturn)
	{
		if(!FClassnameIs(pReturn, STRING(classname)))
		{
			Warning("Global entity found %s, wrong class %s [expects class %s]\n", STRING(globalname),
					STRING(pReturn->m_iClassname), STRING(classname));
			pReturn = NULL;
		}
	}
...
int CEntitySaveRestoreBlockHandler::RestoreGlobalEntity(CBaseEntity *pEntity, CSaveRestoreData *pSaveData,
														entitytable_t *pEntInfo)
{
	Vector oldOffset;
...
CSaveRestoreData *SaveInit(int size)
{
	...
	void *pSaveMemory =
		engine->SaveAllocMemory(sizeof(CSaveRestoreData) + (sizeof(entitytable_t) * numentities) + size, sizeof(char));
	if(!pSaveMemory)
	{
		return NULL;
	}
...
public:
	...
	void CallBlockHandlerRestore(ISaveRestoreBlockHandler *pHandler, int baseFilePos, IRestore *pRestore,
								 bool fCreatePlayers)
	{
	...
	void AddBlockHandler(ISaveRestoreBlockHandler *pHandler)
	{
		...
		AssertMsg(pHandler == &g_EntitySaveRestoreBlockHandler ||
					  (m_Handlers.Count() >= 1 && m_Handlers[0] == &g_EntitySaveRestoreBlockHandler),
...
BEGIN_SIMPLE_DATADESC(SaveRestoreBlockHeader_t)
	DEFINE_ARRAY(szName, FIELD_CHARACTER, MAX_BLOCK_NAME_LEN + 1), DEFINE_FIELD(locHeader, FIELD_INTEGER),
...
int CreateEntityTransitionList(CSaveRestoreData *pSaveData, int levelMask)
{
	...
	for(i = 0; i < pSaveData->NumEntities(); i++)
	{
		...
		if(pent && (pEntInfo->flags & levelMask)) // Screen out the player if he's not to be spawned
		{
			...
			// Remove any entities that were removed using UTIL_Remove() as a result of the above calls to
