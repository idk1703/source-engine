...
static ConVar soundpatch_captionlength("soundpatch_captionlength", "2.0", FCVAR_REPLICATED,
...
BEGIN_SIMPLE_DATADESC(CSoundEnvelope)
	DEFINE_FIELD(m_current, FIELD_FLOAT), DEFINE_FIELD(m_target, FIELD_FLOAT), DEFINE_FIELD(m_rate, FIELD_FLOAT),
...
BEGIN_SIMPLE_DATADESC(CCopyRecipientFilter)
	...
	DEFINE_FIELD(m_Flags, FIELD_INTEGER), DEFINE_UTLVECTOR(m_Recipients, FIELD_INTEGER),
...
BEGIN_SIMPLE_DATADESC(CSoundPatch)
	...
	DEFINE_EMBEDDED(m_pitch), DEFINE_EMBEDDED(m_volume), DEFINE_FIELD(m_soundlevel, FIELD_INTEGER),
		DEFINE_FIELD(m_shutdownTime, FIELD_TIME), DEFINE_FIELD(m_flLastTime, FIELD_TIME),
		DEFINE_FIELD(m_iszSoundName, FIELD_STRING), DEFINE_FIELD(m_iszSoundScriptName, FIELD_STRING),
		DEFINE_FIELD(m_hEnt, FIELD_EHANDLE), DEFINE_FIELD(m_entityChannel, FIELD_INTEGER),
		DEFINE_FIELD(m_flags, FIELD_INTEGER), DEFINE_FIELD(m_baseFlags, FIELD_INTEGER),
		DEFINE_FIELD(m_isPlaying, FIELD_INTEGER), DEFINE_FIELD(m_flScriptVolume, FIELD_FLOAT),
		DEFINE_EMBEDDED(m_Filter), DEFINE_FIELD(m_flCloseCaptionDuration, FIELD_FLOAT),
...
bool CSoundPatch::Update(float time, float deltaTime)
{
	...
	{
		// SoundPatches take volumes between 0 & 1, and use that to multiply the sounds.txt specified volume.
		// Because of this, we need to always set the SND_CHANGE_VOL flag when we emit sound, or it'll use the
		// scriptfile's instead.
		m_flags |= SND_CHANGE_VOL;
...
void CSoundPatch::StartSound(float flStartTime)
{
	...
	if(m_Filter.IsActive())
	{
		...
		CBaseEntity::EmitSound(m_Filter, EntIndex(), ep);
		CBaseEntity::EmitCloseCaption(m_Filter, EntIndex(), STRING(m_iszSoundScriptName), ep.m_UtlVecSoundOrigin,
									  m_flCloseCaptionDuration, true);
	}
	m_isPlaying = true;
}
...
void CSoundPatch::ResumeSound(void)
{
	if(IsPlaying() && m_Filter.IsActive())
	{
		...
		else
		{
			...
			DevWarning("CSoundPatch::ResumeSound: Lost EHAndle on restore - destroy the sound patch in your entity's "
					   "StopLoopingSounds! (%s)\n",
					   STRING(m_iszSoundName));
		}
	}
...
BEGIN_SIMPLE_DATADESC(SoundCommand_t)
	...
	DEFINE_FIELD(m_time, FIELD_TIME), DEFINE_FIELD(m_deltaTime, FIELD_FLOAT), DEFINE_FIELD(m_command, FIELD_INTEGER),
...
public:
	...
	void CommandAdd(CSoundPatch *pSound, float executeDeltaTime, soundcommands_t command, float commandTime,
					float commandValue);
...
// Output : 	void
//-----------------------------------------------------------------------------
void CSoundControllerImp::CommandAdd(CSoundPatch *pSound, float executeDeltaTime, soundcommands_t command,
									 float commandTime, float commandValue)
{
	SoundCommand_t *pCommand =
		new SoundCommand_t(pSound, g_pEffects->Time() + executeDeltaTime, command, commandTime, commandValue);
...
//-----------------------------------------------------------------------------
float CSoundControllerImp::SoundPlayEnvelope(CSoundPatch *pSound, soundcommands_t soundCommand, envelopePoint_t *points,
											 int numPoints)
{
...
// Output : float - Returns the total duration of the envelope
//-----------------------------------------------------------------------------
float CSoundControllerImp::SoundPlayEnvelope(CSoundPatch *pSound, soundcommands_t soundCommand,
