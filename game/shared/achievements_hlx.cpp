...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithPhysicsObjects, ACHIEVEMENT_HLX_KILL_ENEMIES_WITHPHYSICS,
					"HLX_KILL_ENEMIES_WITHPHYSICS", 5);
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithHopper, ACHIEVEMENT_HLX_KILL_ENEMY_WITHHOPPERMINE,
					"HLX_KILL_ENEMY_WITHHOPPERMINE", 5);
...
class CAchievementHLXKillWithManhack : public CBaseAchievement
{
	...
	virtual void Event_EntityKilled(CBaseEntity *pVictim, CBaseEntity *pAttacker, CBaseEntity *pInflictor,
									IGameEvent *event)
	{
		// We've already filtered to only get called when a player enemy gets killed with a manhack.  Now just check for
		// the case of player smashing manhack into something, in which case the manhack is both the victim and
		// inflictor. If that's not the case, this is a player kill w/manhack.
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithManhack, ACHIEVEMENT_HLX_KILL_ENEMIES_WITHMANHACK,
					"HLX_KILL_ENEMIES_WITHMANHACK", 5);
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillSoldierWithOwnGrenade, ACHIEVEMENT_HLX_KILL_SOLDIER_WITHHISGRENADE,
					"HLX_KILL_SOLDIER_WITHHISGRENADE", 10);
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillWithOneEnergyBall, ACHIEVEMENT_HLX_KILL_ENEMIES_WITHONEENERGYBALL,
					"HLX_KILL_ENEMIES_WITHONEENERGYBALL", 5);
...
DECLARE_ACHIEVEMENT(CAchievementHLXKillEliteSoldierWithOwnEnergyBall,
					ACHIEVEMENT_HLX_KILL_ELITESOLDIER_WITHHISENERGYBALL, "HLX_KILL_ELITESOLDIER_WITHHISENERGYBALL", 10);
...
int CalcPlayerAttacks(bool bBulletOnly)
{
	...
	for(int i = 0; i < iWeapons; i++)
	{
		...
		if(pWeapon)
		{
			// add primary attacks if we were asked for all attacks, or only if it uses bullet ammo if we were asked to
			// count bullet attacks
			...
			// add secondary attacks if we were asked for all attacks, or only if it uses bullet ammo if we were asked
			// to count bullet attacks
