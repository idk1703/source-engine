...
#include "eventlist.h"
ConVar anim_showmainactivity("anim_showmainactivity", "0", FCVAR_CHEAT,
...
ConVar anim_showstate("anim_showstate", "-1", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
					  "Show the (client) animation state for the specified entity (-1 for none).");
ConVar anim_showstatelog("anim_showstatelog", "0", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
						 "1 to output anim_showstate to Msg(). 2 to store in AnimState.log. 3 for both.");
ConVar mp_showgestureslots(
	"mp_showgestureslots", "-1", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
...
void CMultiPlayerAnimState::RunGestureSlotAnimEventsToCompletion(GestureSlot_t *pGesture)
{
	...
	if(seqdesc.numevents > 0)
	{
		...
		for(int i = 0; i < (int)seqdesc.numevents; i++)
		{
			...
			{
				pPlayer->FireEvent(pPlayer->GetAbsOrigin(), pPlayer->GetAbsAngles(), pevent[i].event,
								   pevent[i].pszOptions());
			}
		}
	}
...
bool CMultiPlayerAnimState::VerifyAnimLayerInSlot(int iGestureSlot)
{
	...
	{
		AssertMsg2(false, "Player %d doesn't have gesture slot %d any more.", GetBasePlayer()->entindex(),
				   iGestureSlot);
	...
	{
		AssertMsg3(false, "Gesture slot %d pointing to wrong address %p. Updating to new address %p.", iGestureSlot,
...
void CMultiPlayerAnimState::AddToGestureSlot(int iGestureSlot, Activity iGestureActivity, bool bAutoKill)
{
	...
	m_aGestureSlots[iGestureSlot].m_pAnimLayer->m_bLooping =
		false; //( ( GetSequenceFlags( GetModelPtr(), iGestureSequence ) & STUDIO_LOOPING ) != 0);
	if(bAutoKill)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CMultiPlayerAnimState::AddVCDSequenceToGestureSlot(int iGestureSlot, int iGestureSequence, float flCycle,
														bool bAutoKill)
{
	// Sanity Check
	Assert(iGestureSlot >= 0 && iGestureSlot < GESTURE_SLOT_COUNT);
...
Activity CMultiPlayerAnimState::CalcMainActivity()
{
	...
	if(HandleJumping(idealActivity) || HandleDucking(idealActivity) || HandleSwimming(idealActivity) ||
...
void CMultiPlayerAnimState::ComputeMainSequence()
{
	...
	if((oldActivity == ACT_MP_CROUCH_IDLE || oldActivity == ACT_MP_STAND_IDLE || oldActivity == ACT_MP_DEPLOYED_IDLE ||
		oldActivity == ACT_MP_CROUCH_DEPLOYED_IDLE) &&
...
void CMultiPlayerAnimState::UpdateGestureLayer(CStudioHdr *pStudioHdr, GestureSlot_t *pGesture)
{
	...
	flCycle += pPlayer->GetSequenceCycleRate(pStudioHdr, pGesture->m_pAnimLayer->m_nSequence) * gpGlobals->frametime *
			   GetGesturePlaybackRate() * pGesture->m_pAnimLayer->m_flPlaybackRate;
...
void CMultiPlayerAnimState::DoMovementTest(CStudioHdr *pStudioHdr, float flX, float flY)
{
	...
	if(flTestSpeed < 10.0f)
	{
		Warning("%s : %s (X %.0f Y %.0f) missing movement\n", pStudioHdr->pszName(),
				GetBasePlayer()->GetSequenceName(m_nMovementSequence), flX, flY);
	}
#endif
	...
	GetBasePlayer()->GetSequenceName( m_nMovementSequence ), flX, flY, flDuration, flForward );
	}

	if ( flDuration > flCenter * 1.1f || flDuration < flCenter * 0.9f )
	{
		Warning( "%s : %s (X %.0f Y %.0f) mismatched duration with center  %.1f vs %.1f\n", pStudioHdr->pszName(),
	GetBasePlayer()->GetSequenceName( m_nMovementSequence ), flX, flY, flDuration, flCenter );
	}
	*/
}
...
void CMultiPlayerAnimState::GetMovementFlags(CStudioHdr *pStudioHdr)
{
	...
	if(m_nMovementSequence < 0 || !(GetBasePlayer()->GetFlags() & FL_ONGROUND) ||
	   pStudioHdr->pSeqdesc(m_nMovementSequence).groupsize[0] == 1)
	{
		return;
	}
...
void CMultiPlayerAnimState::DebugShowAnimStateForPlayer(bool bIsServer)
{
	...
	Anim_StatePrintf(iLine++, "Main: %s, Cycle: %.2f\n",
					 GetSequenceName(GetBasePlayer()->GetModelPtr(), GetBasePlayer()->GetSequence()),
					 GetBasePlayer()->GetCycle());
	...
	for(int iAnim = 0; iAnim < GetBasePlayer()->GetNumAnimOverlays(); ++iAnim)
	{
		...
		{
			Anim_StatePrintf(iLine++, "Layer %s: Weight: %.2f, Cycle: %.2f",
							 GetSequenceName(GetBasePlayer()->GetModelPtr(), pLayer->m_nSequence),
	...
	Anim_StatePrintf(iLine++, "Time: %.2f, Speed: %.2f, MaxSpeed: %.2f", gpGlobals->curtime, vecVelocity.Length2D(),
					 GetCurrentMaxGroundSpeed());

	// Write out the 9-way blend data.
	Anim_StatePrintf(iLine++, "EntityYaw: %.2f, AimYaw: %.2f, AimPitch: %.2f, MoveX: %.2f, MoveY: %.2f",
					 m_angRender[YAW], m_DebugAnimData.m_flAimYaw, m_DebugAnimData.m_flAimPitch,
					 m_DebugAnimData.m_vecMoveYaw.x, m_DebugAnimData.m_vecMoveYaw.y);
...
void CMultiPlayerAnimState::DebugShowAnimState(int iStartLine)
{
	...
	for(int i = 0; i < GetBasePlayer()->GetNumAnimOverlays() - 1; i++)
	{
		...
		Anim_StatePrintf(iLine++, "%s, weight: %.2f, cycle: %.2f, aim (%d)",
						 ...
						 pLayer->m_nOrder == CBaseAnimatingOverlay::MAX_OVERLAYS ? -1 : (float)pLayer->m_flCycle, i);
	}
#endif

	Anim_StatePrintf(iLine++, "vel: %.2f, time: %.2f, max: %.2f", vOuterVel.Length2D(), gpGlobals->curtime,
	...
	if(debugoverlay)
	{
		...
		debugoverlay->AddTriangleOverlay(vBasePos + vRight * flBaseSize / 2, vBasePos - vRight * flBaseSize / 2,
...
const char *s_aGestureSlotNames[GESTURE_SLOT_COUNT] = {
	"Attack and Reload", "Grenade", "Jump", "Swim", "Flinch", "VCD", "Custom"};
		...
		if(pGesture)
		{
			if(pGesture->m_bActive)
			{
				Anim_StatePrintf(iLine++, "Gesture Slot %d(%s): %s %s(A:%s, C:%f P:%f)\n", iGesture,
								 s_aGestureSlotNames[iGesture], ActivityList_NameForIndex(pGesture->m_iActivity),
								 GetSequenceName(pPlayer->GetModelPtr(), pGesture->m_pAnimLayer->m_nSequence),
								 (pGesture->m_bAutoKill ? "true" : "false"), (float)pGesture->m_pAnimLayer->m_flCycle,
								 (float)pGesture->m_pAnimLayer->m_flPlaybackRate);
			}
			else
			{
