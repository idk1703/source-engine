...
void SetActivityForSequence(CStudioHdr *pstudiohdr, int i)
{
	...
	if(pszActivityName[0] != '\0')
	{
		...
		if(iActivityIndex == -1)
		{
			// Allow this now.  Animators can create custom activities that are referenced only on the client or by
			// scripts, etc.
			// Warning( "***\nModel %s tried to reference unregistered activity: %s \n***\n", pstudiohdr->name,
			// pszActivityName ); Assert(0);
			// HACK: the client and server don't share the private activity list so registering it on the client would
			// hose the server
#ifdef CLIENT_DLL
			seqdesc.flags &= ~STUDIO_ACTIVITY;
#else
...
int CStudioHdr::CActivityToSequenceMapping::SelectWeightedSequence(CStudioHdr *pstudiohdr, int activity,
																   int curSequence)
{
	if(!ValidateAgainst(pstudiohdr))
	{
		AssertMsg1(false,
				   "CStudioHdr %s has changed its vmodel pointer without reinitializing its activity mapping! Now "
				   "performing emergency reinitialization.",
				   pstudiohdr->pszName());
	...
	const SequenceTuple *const stopHere =
		sequenceInfo + actData->count; // this is a backup
									   // in case the weights are somehow miscalculated -- we don't read or write
									   // through it (because it aliases the restricted pointer above); it's only here
									   // for the comparison.
...
int CStudioHdr::CActivityToSequenceMapping::SelectWeightedSequenceFromModifiers(CStudioHdr *pstudiohdr, int activity,
																				...
																				int iModifierCount)
{
	...
	// randomly pick between the highest scoring sequences ( NOTE: this method of selecting a sequence ignores activity
	// weights )
	if(IsInPrediction())
	{
...
{
	if(!pstudiohdr || !pstudiohdr->SequencesAvailable() || sequence < 0 || sequence >= pstudiohdr->GetNumSeq())
	{
		return 0;
	}
...
int GetAnimationEvent(CStudioHdr *pstudiohdr, int sequence, animevent_t *pNPCEvent, float flStart, float flEnd,
					  int index)
{
...
bool GotoSequence(CStudioHdr *pstudiohdr, int iCurrentSequence, float flCurrentCycle, float flCurrentRate,
				  int iGoalSequence, int &nNextSequence, float &flNextCycle, int &iNextDir)
{
	if(!pstudiohdr)
		return false;
