...
private:
	...
	void Start();
	void AddParticle(float flInitialDeltaTime, const Vector &vecMins, const Vector &vecMaxs,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_EntityParticleTrail::AddParticle(float flInitialDeltaTime, const Vector &vecMins, const Vector &vecMaxs,
										const matrix3x4_t &boxToWorld)
{
...
trailNoHitboxes:
	while(m_teParticleSpawn.NextEvent(tempDelta))
	{
		AddParticle(tempDelta, pMoveParent->CollisionProp()->OBBMins(), pMoveParent->CollisionProp()->OBBMaxs(),
					pMoveParent->EntityToWorldTransform());
	}
}
...
inline void C_EntityParticleTrail::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		Vector color = Vector(pParticle->m_uchColor[0] / 255.0f, pParticle->m_uchColor[1] / 255.0f,
