...
#ifdef CSTRIKE_DLL
				// [jpaquin] this allows counter strike to display default avatars for bots.  It can't be a virtual
				// function on C_BasePlayer because there would be no way to get a game specific default image if the
				// player is null.
				extern vgui::IImage *GetDefaultAvatarImage(C_BasePlayer * pPlayer);
				activeSpeaker.pAvatar->SetDefaultImage(
					GetDefaultAvatarImage(UTIL_PlayerByIndex(activeSpeaker.playerId)));
#endif
...
void CHudVoiceStatus::Paint()
{
	...
	FOR_EACH_LL(m_SpeakingList, i)
	{
				...
				if(asciiLocation && *asciiLocation)
				{
					...
					if(unicodeLocation && *unicodeLocation)
					{
						...
						if(formatStr)
						{
							...
							g_pVGuiLocalize->ConstructString_safe(szconverted, formatStr, 2, unicodeName,
																  unicodeLocation);
		...
		if(show_dead_icon && bIsAlive == false && m_iDeadImageID != -1)
		{
			...
			vert[2].Init(Vector2D(dead_icon_xpos + dead_icon_wide, ypos + dead_icon_ypos + dead_icon_tall),
						 Vector2D(uv2, uv2));
		...
		// Draw the voice icon
		if(show_voice_icon)
			m_pVoiceIcon->DrawSelf(voice_icon_xpos, ypos + voice_icon_ypos, voice_icon_wide, voice_icon_tall,
								   m_clrIcon);
