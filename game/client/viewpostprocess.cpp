							   ...
							   ClipBox *dest_coords_out = 0)  // receives dest coords of last blit
{
	...
	while(pass_list->material_name)
	{
		...
		if(do_it)
		{
			...
			{
				ITexture *src_rt = materials->FindTexture(pass_list->src_rendering_target, TEXTURE_GROUP_RENDER_TARGET);
			...
			else
			{
				...
				if((pass_list->dest_rendering_target) && (debug_postproc.GetInt() == 1))
				{
					...
					pRenderContext->GetRenderTargetDimensions(dest_width, dest_height);
					pRenderContext->Viewport(0, 0, dest_width, dest_height);
					DrawClippedScreenSpaceRectangle(src_mat, 10 + col * 220, 10 + row * 220, 200, 200, 0, 0, 1, 1, 1, 1,
													cb);
				}
			}
			if(dest_coords_out)
...
PostProcessingPass HDRFinal_Float_NoBloom[] = {PPP_PROCESS_SRCTEXTURE("dev/copyfullframefb", "_rt_FullFrameFB", NULL),
											   PPP_END};
	...
	while(n_queries_issued_this_frame < MAX_QUERIES_PER_FRAME)
	{
		...
		for(int i = 0; i < nNumRanges; i++)
			if((CurHistogram[i].m_state == HESTATE_QUERY_DONE) &&
			   ((oldest_so_far == -1) || (CurHistogram[i].m_frame_queued < CurHistogram[oldest_so_far].m_frame_queued)))
				oldest_so_far = i;
	...
	if(mat_tonemap_algorithm.GetInt() == 1) // New algorithm
	{
		...
		for(int i = N_LUMINANCE_RANGES_NEW - 2; i >= 0; i--) // Start at the bright end
		{
			...
			if(flThisBinPercentOfTotalPixels >= flPixelPercentNeeded) // We found the bin needed
			{
				...
				flPercentLocationOfBorder =
					MAX(CurHistogram[i].m_min_lum,
					...
					else
					{
						engine->Con_NPrintf(
							...
							(g_HDR_HistogramSystem.FindLocationOfPercentBrightPixels(
								 mat_tonemap_percent_bright_pixels.GetFloat(), mat_tonemap_percent_target.GetFloat()) *
							 100.0f),
							 ...
							 void *pClientRenderable)
{
	...
	pRenderContext->DrawScreenSpaceRectangle(pVignetteBorder, 0, 0, nScreenWidth, nScreenHeight, 0, 0, nScreenWidth - 1,
											 nScreenHeight - 1, nScreenWidth, nScreenHeight, pClientRenderable);
			...
			for(int nIndex = 0; nIndex < MAX_PYRO_SIDES; nIndex++, pSide++)
			{
				if(pSide->m_bAlive)
				{
					for(int i = 0; i < 6; i++)
					{
						...
						meshBuilder.TexCoord2fv(0, pSide->m_bHorizontal ? PryoVignetteSTHorizontal[i]
																		: PryoVignetteSTVertical[i]);
						 ...
						 void *pClientRenderable)					  // Used to pass to the bind proxies
{
	...
	// Bottom left (y+1)
	meshBuilder.Position3f(flLeftX + (float)(xSegments - 1) * flWidth, flTopY - (float)(ySegments - 1) * flHeight,
						   0.0f);
		...
		case HDR_TYPE_INTEGER:
		{
			...
			if(bPerformBloom || bPerformSoftwareAA || bPerformColCorrect)
			{
				...
				if(debug_postproc.GetInt() == 2)
				{
					...
					Vector2D uvScalePost(1.0f - ((w / 2) / (float)(w - 1)), 1.0f - ((h / 2) / (float)(h - 1)));
...
void DoImageSpaceMotionBlur(const CViewSetup &viewBlur, int x, int y, int w, int h)
{
	...
	if(true)
	{
		...
		else
		{
			...
			float flVerticalFov =
				(viewBlur.m_flAspectRatio <= 0.0f) ? (viewBlur.fov) : (viewBlur.fov / viewBlur.m_flAspectRatio);
			...
			g_vMotionBlurValues[0] =
				flUndampenedYaw * (1.0f - (fabs(flCurrentPitch) / 90.0f)); // Dampen horizontal yaw blur based on pitch
	...
	if(true)
	{
		...
		{
			pRenderContext->DrawScreenSpaceRectangle(pMatMotionBlur, 0, 0, dest_width, dest_height, 0, 0, nSrcWidth - 1,
