...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CClientVirtualReality, IClientVirtualReality, CLIENTVIRTUALREALITY_INTERFACE_VERSION,
								  g_ClientVirtualReality);

// --------------------------------------------------------------------
// A huge pile of VR convars
// --------------------------------------------------------------------
ConVar vr_activate_default("vr_activate_default", "0", FCVAR_ARCHIVE,
						   "If this is true the game will switch to VR mode once startup is complete.");

ConVar vr_moveaim_mode(
	"vr_moveaim_mode", "3", FCVAR_ARCHIVE,
...
ConVar vr_moveaim_mode_zoom(
	"vr_moveaim_mode_zoom", "3", FCVAR_ARCHIVE,
	"0=move+shoot from face. 1=move with torso. 2,3,4=shoot with face+mouse cursor. 5+ are probably not that useful.");

ConVar vr_moveaim_reticle_yaw_limit("vr_moveaim_reticle_yaw_limit", "10", FCVAR_ARCHIVE,
									"Beyond this number of degrees, the mouse drags the torso");
ConVar vr_moveaim_reticle_pitch_limit("vr_moveaim_reticle_pitch_limit", "30", FCVAR_ARCHIVE,
									  "Beyond this number of degrees, the mouse clamps");
// Note these are scaled by the zoom factor.
ConVar vr_moveaim_reticle_yaw_limit_zoom("vr_moveaim_reticle_yaw_limit_zoom", "0", FCVAR_ARCHIVE,
										 "Beyond this number of degrees, the mouse drags the torso");
ConVar vr_moveaim_reticle_pitch_limit_zoom("vr_moveaim_reticle_pitch_limit_zoom", "-1", FCVAR_ARCHIVE,
										   "Beyond this number of degrees, the mouse clamps");

// This are somewhat obsolete.
ConVar vr_aim_yaw_offset("vr_aim_yaw_offset", "90", 0,
...
ConVar vr_translation_limit("vr_translation_limit", "10.0", 0,
...
ConVar vr_hud_axis_lock_to_world("vr_hud_axis_lock_to_world", "0", FCVAR_ARCHIVE,
								 "Bitfield - locks HUD axes to the world - 0=pitch, 1=yaw, 2=roll");

// Default distance clips through rocketlauncher, heavy's body, etc.
ConVar vr_projection_znear_multiplier("vr_projection_znear_multiplier", "0.3", 0,
									  "Allows moving the ZNear plane to deal with body clipping");

// Should the viewmodel (weapon) translate with the HMD, or remain fixed to the in-world body (but still rotate with the
// head)? Purely a graphics effect - no effect on actual bullet aiming. Has no effect in aim modes where aiming is not
// controlled by the head.
ConVar vr_viewmodel_translate_with_head("vr_viewmodel_translate_with_head", "0", 0,
...
ConVar vr_zoom_scope_scale(
	"vr_zoom_scope_scale", "6.0", 0,
	"Something to do with the default scope HUD overlay size."); // Horrible hack - should work out the math properly,
																 // but we need to ship.
...
ConVar vr_first_person_uses_world_model("vr_first_person_uses_world_model", "1", 0,
...
static ConCommand vr_cycle_aim_move_mode("vr_cycle_aim_move_mode", CC_VR_Cycle_Aim_Move_Mode,
...
void CClientVirtualReality::DrawMainMenu()
{
	...
	g_ClientVirtualReality.OverrideStereoView(&viewEye[STEREO_EYE_MONO], &viewEye[STEREO_EYE_LEFT],
											  &viewEye[STEREO_EYE_RIGHT]);
	...
	for(int nView = STEREO_EYE_LEFT; nView <= STEREO_EYE_RIGHT; nView++)
	{
		...
		ITexture *pColor =
			g_pSourceVR->GetRenderTarget((ISourceVirtualReality::VREye)(nView - 1), ISourceVirtualReality::RT_Color);
		ITexture *pDepth =
			g_pSourceVR->GetRenderTarget((ISourceVirtualReality::VREye)(nView - 1), ISourceVirtualReality::RT_Depth);
...
// --------------------------------------------------------------------
bool CClientVirtualReality::OverrideView(CViewSetup *pViewMiddle, Vector *pViewModelOrigin, QAngle *pViewModelAngles,
										 HeadtrackMovementMode_t hmmMovementOverride)
{
	if(!UseVR())
	{
		return false;
	...
	// Ignore what was passed in - it's just the direction the weapon is pointing, which was determined by last frame's
	// HMD orientation! Instead use our cached value.
	...
	*pViewModelAngles = pViewMiddle->angles;
	...
	{
		Vector vOffset(vr_debug_remote_cam_pos_x.GetFloat(), vr_debug_remote_cam_pos_y.GetFloat(),
					   vr_debug_remote_cam_pos_z.GetFloat());
		Vector vLookat(vr_debug_remote_cam_target_x.GetFloat(), vr_debug_remote_cam_target_y.GetFloat(),
					   vr_debug_remote_cam_target_z.GetFloat());
...
bool CClientVirtualReality::OverrideStereoView(CViewSetup *pViewMiddle, CViewSetup *pViewLeft, CViewSetup *pViewRight)
{
	...
	g_pSourceVR->GetEyeProjectionMatrix(&pViewLeft->m_ViewToProjection, ISourceVirtualReality::VREye_Left,
										pViewMiddle->zNear, pViewMiddle->zFar, 1.0f / headtrackFovScale);
	g_pSourceVR->GetEyeProjectionMatrix(&pViewRight->m_ViewToProjection, ISourceVirtualReality::VREye_Right,
										pViewMiddle->zNear, pViewMiddle->zFar, 1.0f / headtrackFovScale);
	...
	for(int i = 0; i < 4; i++)
	{
		for(int j = 0; j < 4; j++)
		{
			pViewMiddle->m_ViewToProjection.m[i][j] =
				(pViewLeft->m_ViewToProjection.m[i][j] + pViewRight->m_ViewToProjection.m[i][j]) * 0.5f;
		}
	}
	...
	// The aspect ratio of the HMD may be something bizarre (e.g. Rift is 640x800), and the pixels may not be square, so
	// don't use that!
...
//			to HMD status.
// --------------------------------------------------------------------
bool CClientVirtualReality::OverridePlayerMotion(float flInputSampleFrametime, const QAngle &oldAngles,
												 const QAngle &curAngles, const Vector &curMotion, QAngle *pNewAngles,
												 Vector *pNewMotion)
{
	...
	*pNewMotion = curMotion;
	...
	// originalAngles tells us what the weapon angles were before whatever mouse, joystick, etc thing changed them -
	// called "old" curAngles holds the new weapon angles after mouse, joystick, etc. applied. We need to compute what
	// weapon angles WE want and return them in *pNewAngles - called "new"
	...
	switch(m_hmmMovementActual)
	{
		...
		case HMM_SHOOTBOUNDEDMOUSE_LOOKFACE_MOVEMOUSE:
		{
			...
			*pNewMotion = curMotion;
			...
			if(fReticlePitchLimit >= 0.0f)
			{
				...
				(*pNewAngles)[PITCH] = Clamp(curAngles[PITCH], m_PlayerViewAngle[PITCH] - fReticlePitchLimit,
											 m_PlayerViewAngle[PITCH] + fReticlePitchLimit);
			}
		...
		{
			(*pNewAngles)[PITCH] =
				clamp((*pNewAngles)[PITCH], m_PlayerViewAngle[PITCH] - 15.f, m_PlayerViewAngle[PITCH] + 15.f);
	...
	switch(m_hmmMovementActual)
	{
		case HMM_SHOOTBOUNDEDMOUSE_LOOKFACE_MOVEFACE:
		{
			// The motion passed in is meant to be relative to the face, so jimmy it to be relative to the new weapon
			// aim.
		...
		case HMM_SHOOTFACE_MOVETORSO:
		{
			// The motion passed in is meant to be relative to the torso, so jimmy it to be relative to the new weapon
			// aim.
...
bool CClientVirtualReality::ProcessCurrentTrackingState(float fGameFOV)
{
	...
	if(fGameFOV != 0.0f)
	{
		...
		// The game has overridden the FOV, e.g. because of a sniper scope. So we need to match this view with whatever
		// actual FOV the HUD has.
		float wantedGameTanfov = tanf(DEG2RAD(fGameFOV * 0.5f));
		// OK, so now in stereo mode, we're going to also draw an overlay, but that overlay usually covers more of the
		// screen (because in a good HMD usually our actual FOV is much wider)
...
void CClientVirtualReality::OverrideViewModelTransform(Vector &vmorigin, QAngle &vmangles, bool bUseLargeOverride)
{
	...
	float fForward =
		bUseLargeOverride ? vr_viewmodel_offset_forward_large.GetFloat() : vr_viewmodel_offset_forward.GetFloat();
...
void CClientVirtualReality::PostProcessFrame(StereoEye_t eEye)
{
	...
	g_pSourceVR->DoDistortionProcessing(eEye == STEREO_EYE_LEFT ? ISourceVirtualReality::VREye_Left
...
// --------------------------------------------------------------------
void CClientVirtualReality::OverlayHUDQuadWithUndistort(const CViewSetup &eyeView, bool bDoUndistort, bool bBlackout,
														bool bTranslucent)
{
	if(!UseVR())
		return;
	...
	// Get the position of the HUD quad in world space as used by RenderHUDQuad().  Then convert to a rectangle in
	...
	ISourceVirtualReality::VREye sourceVrEye = (eyeView.m_eStereoEye == STEREO_EYE_LEFT)
...
void CClientVirtualReality::Activate()
{
	...
	if(!ShouldForceVRActive())
	{
		...
		if(nVRModeAdapter == -1)
		{
			Warning("Unable to get VRMode adapter from OpenVR. VR mode cannot be enabled. Try restarting and then "
					"enabling VR again.\n");
			return;
		}
		...
		{
			Warning("VR Mode expects adapter %d which is different from %d which we are currently using. Try "
					"restarting and enabling VR mode again.\n",
	...
	g_pMatSystemSurface->SetFullscreenViewportAndRenderTarget(
		...
		g_pSourceVR->GetRenderTarget(ISourceVirtualReality::VREye_Left, ISourceVirtualReality::RT_Color));
...
void CClientVirtualReality::Deactivate()
{
	...
	// set mode
	char szCmd[256];
	Q_snprintf(szCmd, sizeof(szCmd), "mat_setvideomode %i %i %i\n", m_nNonVRWidth, m_nNonVRHeight,
			   m_bNonVRWindowed ? 1 : 0);
	engine->ClientCmd_Unrestricted(szCmd);
}
