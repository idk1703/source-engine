...
//   press_command_str is the concommand executed on start of the action (ie button press) and release_command_str on
//   stop (ie button release) release_command_str can be the empty string if no stop action is desired if the
//   press_command_str begins with "+", an equivalent "-" is set to the release_command automatically
void SixenseGestureBindings::AddBinding(CUtlString hand_str, CUtlString action_str, CUtlString argument_str,
										CUtlString press_command_str, CUtlString release_command_str)
{
	...
	FOR_EACH_LL(m_GestureBindingList, it)
	{
		...
		if(binding.m_Action == existing_binding.m_Action && binding.m_iArgument == existing_binding.m_iArgument &&
...
void SixenseGestureBindings::ListBindings()
{
	...
	FOR_EACH_LL(m_GestureBindingList, it)
	{
		...
		if(binding.m_pDeactivateCommand &&
		   !binding.m_bAutoMirrored) // only print deactivated command if we didn't generate it.
		{
...
// Erase all the bindings. Right now this will cause code in in_sixense.cpp to detect the lack of bindings and
// immediately create defaults. I think that's good.
...
{
	if(num < 0 || num > m_GestureBindingList.Count() - 1)
		return;
...
// Called each frame with the latest controller info. sixenseUtils::IButtonStates are sixenseUtils classes that look for
// various transitions of controller state, whether it's 'buttonJustPressed', or analog joysticks
// 'joystickJustPushed(up)'. It now has support for motion triggers like 'controllerJustTilted(left)'
void SixenseGestureBindings::UpdateBindings(sixenseUtils::IButtonStates *pLeftButtonStates,
											sixenseUtils::IButtonStates *pRightButtonStates, bool bIsMenuVisible)
{
	...
	FOR_EACH_LL(m_GestureBindingList, it)
	{
		...
		if(binding.m_iHand == 0)
		{
			...
			// just started?
			if(pLeftButtonStates->justStarted((sixenseUtils::IButtonStates::ActionType)binding.m_Action,
											  binding.m_iArgument))
			{
				if(binding.m_pActivateCommand)
				{
			...
			// just stopped?
			if(pLeftButtonStates->justStopped((sixenseUtils::IButtonStates::ActionType)binding.m_Action,
											  binding.m_iArgument))
			{
				if(binding.m_pDeactivateCommand)
				{
		...
		else
		{
			...
			// just started?
			if(pRightButtonStates->justStarted((sixenseUtils::IButtonStates::ActionType)binding.m_Action,
											   binding.m_iArgument))
			{
				if(binding.m_pActivateCommand)
				{
			...
			// just stopped?
			if(pRightButtonStates->justStopped((sixenseUtils::IButtonStates::ActionType)binding.m_Action,
											   binding.m_iArgument))
			{
				if(binding.m_pDeactivateCommand)
				{
