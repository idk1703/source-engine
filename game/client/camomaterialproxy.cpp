...
private:
	...
	void GetColors(Vector &lighting, Vector &base, int index, const Vector &boxMin, const Vector &boxExtents,
				   const Vector &forward, const Vector &right, const Vector &up, const Vector &entityPosition);
...
void CCamoMaterialProxy::GetColors(Vector &diffuseColor, Vector &baseColor, int index, const Vector &boxMin,
								   const Vector &boxExtents, const Vector &forward, const Vector &right,
								   const Vector &up, const Vector &entityPosition)
{
...
void CCamoMaterialProxy::LoadCamoPattern(void)
{
...
#ifndef _XBOX
	float dummyGamma;
	if(!TGALoader::GetInfo(m_pCamoPatternTextureVar->GetStringValue(), &m_CamoPatternWidth, &m_CamoPatternHeight,
	...
	{
		//	Warning( "Camo material texture hl2/materials/models/combine_elite/camo7paletted.tga must be 8-bit
		//greyscale\n" );
		m_pCamoTextureVar = NULL;
		return;
	}
...
#ifndef _XBOX
	if(!TGALoader::Load(m_pCamoPatternImage, m_pCamoPatternTextureVar->GetStringValue(), m_CamoPatternWidth,
						m_CamoPatternHeight, IMAGE_FORMAT_I8, dummyGamma, false))
	{
