...
static ConVar youtube_http_proxy("youtube_http_proxy", "", FCVAR_ARCHIVE,
								 "HTTP proxy.  Specify if you have have problems uploading to YouTube.");
...
namespace
{
	...
	public:
		...
		void LoginCancel();
		YouTubeUploadHandle_t Upload(const char *pFilePath, const char *pMimeType, const char *pTitle,
		...
		bool GetUploadResults(YouTubeUploadHandle_t handle, bool &bSuccess, CUtlString &strURLToVideo,
		...
		void SetUploadFinished(YouTubeUploadHandle_t handle, bool bSuccess, const char *pURLToVideo,
...
protected:
	void OnHTTPRequestCompleted(HTTPRequestCompleted_t *pParam, bool bIOFailure)
	{
		...
		if((!m_bAllowRequestFailure && pParam->m_eStatusCode != k_EHTTPStatusCode200OK) || bIOFailure ||
		   !pParam->m_bRequestSuccessful)
		{
...
{
public:
	CYouTubeRetrieveUserProfile(CYouTubeSystem *pSystem) : CYouTubeJob(pSystem) {}
...
private:
	void OnHTTPRequestCompleted(HTTPRequestCompleted_t *pParam) OVERRIDE
	{
		...
		else
		{
			...
			if(GetISteamHTTP()->GetHTTPResponseBodyData(pParam->m_hRequest, (uint8 *)m_strResponse.String(),
														unBodySize))
			{
	...
	virtual JobStatus_t DoExecute()
	{
		HTTPRequestHandle hRequest =
			GetISteamHTTP()->CreateHTTPRequest(k_EHTTPMethodGET, "http://gdata.youtube.com/feeds/api/users/default");
		GetISteamHTTP()->SetHTTPRequestNetworkActivityTimeout(hRequest, 30);
		GetISteamHTTP()->SetHTTPRequestHeaderValue(hRequest, "Authorization",
												   CFmtStr1024("GoogleLogin auth=%s", gYouTube.GetAuthToken()));
...
private:
	...
	virtual JobStatus_t DoExecute()
	{
		...
		HTTPRequestHandle hRequest =
			GetISteamHTTP()->CreateHTTPRequest(k_EHTTPMethodPOST, "https://www.google.com/accounts/ClientLogin");
		GetISteamHTTP()->SetHTTPRequestNetworkActivityTimeout(hRequest, 30);

		//	GetISteamHTTP()->SetHTTPRequestHeaderValue( hRequest, "Content-Type", "application/x-www-form-urlencoded" );
		GetISteamHTTP()->SetHTTPRequestHeaderValue(hRequest, "X-GData-Key",
												   CFmtStr1024("key=%s", m_strDeveloperKey.Get()));
		...
		CFmtStr1024 data("Email=%s&Passwd=%s&service=youtube&source=%s", szUserNameEncoded, szPasswordEncoded,
						 szSourceEncoded);

		GetISteamHTTP()->SetHTTPRequestRawPostBody(hRequest, "application/x-www-form-urlencoded",
												   (uint8 *)data.Access(), data.Length());
...
{
public:
	CYouTubeUploadJob(CYouTubeSystem *pSystem, const char *pFilePath, const char *pMimeType, const char *pTitle,
					  const char *pDescription, const char *pCategory, const char *pKeywords,
					  eYouTubeAccessControl access)
		: CYouTubeJob(pSystem),
...
private:
	virtual JobStatus_t DoExecute()
	{
		...
		HTTPRequestHandle hRequest = GetISteamHTTP()->CreateHTTPRequest(
			k_EHTTPMethodPUT, "http://uploads.gdata.youtube.com/feeds/api/users/default/uploads");
		GetISteamHTTP()->SetHTTPRequestNetworkActivityTimeout(hRequest, 30);
		...
		CFmtStr1024 strAPIRequest("<?xml version=\"1.0\"?>"
								  ...
								  "%s"
								  "</entry>",
								  m_strTitle.Get(), m_strDesc.Get(), m_strCategory.Get(), pPrivateString,
								  m_strKeywords.Get(), m_strDeveloperTag.Get(), pAccessControlString);
		...
		if(bReadFileOK)
		{
			...
			GetISteamHTTP()->SetHTTPRequestRawPostBody(hRequest, "multipart/form-data;boundary=-x",
													   (uint8 *)postDataRaw.Base(), postDataRaw.TellPut());
	...
	void OnHTTPRequestCompleted(HTTPRequestCompleted_t *pParam) OVERRIDE
	{
			...
			else
			{
															...
															unBodySize))
				{
					// @note Tom Bui: wish I had an xml parser...
					{
						...
						// href='http://www.youtube.com/watch?v=7D4pb3irM_0&feature=youtube_gdata' /> ";
		...
		gYouTube.SetUploadFinished((YouTubeUploadHandle_t)this, bSuccess, strURLToVideo.Get(),
								   strURLToVideoStats.Get());
	}
...
YouTubeUploadHandle_t CYouTubeSystem::Upload(const char *pFilePath, const char *pMimeType, const char *pTitle,
											 const char *pDescription, const char *pCategory, const char *pKeywords,
											 eYouTubeAccessControl access)
{
	...
	CYouTubeUploadJob *pJob =
		new CYouTubeUploadJob(this, pFilePath, pMimeType, pTitle, pDescription, pCategory, pKeywords, access);
	m_pThreadPool->AddJob(pJob);
...
bool CYouTubeSystem::GetUploadResults(YouTubeUploadHandle_t handle, bool &bSuccess, CUtlString &strURLToVideo,
									  CUtlString &strURLToVideoStats)
{
	AUTO_LOCK(m_Mutex);
	uploadstatus_t *pStatus = GetStatus(handle);
...
void CYouTubeSystem::SetUploadFinished(YouTubeUploadHandle_t handle, bool bSuccess, const char *pURLToVideo,
									   const char *pURLToVideoStats)
{
	AUTO_LOCK(m_Mutex);
	uploadstatus_t *pStatus = GetStatus(handle);
