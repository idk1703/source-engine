...
static ConVar cl_showbattery("cl_showbattery", "0", 0,
...
void CFPSPanel::Paint()
{
	...
	if(g_bDisplayParticlePerformance)
	{
		...
		if(nPerf)
		{
			...
			g_pMatSystemSurface->DrawColoredText(m_hFont, x, 42, ucColor[0], ucColor[1], ucColor[2], 255,
	...
	if(cl_showfps.GetInt() && realFrameTime > 0.0)
	{
		if(m_lastRealTime != -1.0f)
		{
			...
			if(frequency.m_GHz > 0 && frequency.m_timeStamp + displayTime > currentTime)
			{
				...
				g_pMatSystemSurface->DrawColoredText(m_hFont, x, lineHeight + 2, ucColor[0], ucColor[1], ucColor[2],
													 255, "CPU frequency percent: %3.1f%%   Min percent: %3.1f%%",
													 frequency.m_percentage, frequency.m_lowestPercentage);
			}
		}
	}
	...
	if(nShowPosMode > 0)
	{
		...
		g_pMatSystemSurface->DrawColoredText(m_hFont, x, 2 + i * (vgui::surface()->GetFontTall(m_hFont) + 2), 255, 255,
											 255, 255, "pos:  %.02f %.02f %.02f", vecOrigin.x, vecOrigin.y,
											 vecOrigin.z);
		i++;

		g_pMatSystemSurface->DrawColoredText(m_hFont, x, 2 + i * (vgui::surface()->GetFontTall(m_hFont) + 2), 255, 255,
											 255, 255, "ang:  %.02f %.02f %.02f", angles.x, angles.y, angles.z);
		i++;
...
static ConVar cl_blocking_threshold(
	"cl_blocking_threshold", "0.000", 0,
	"If file ops take more than this amount of time, add to 'spewblocking' history list");
...
static ConVar cl_blocking_recentsize("cl_blocking_recentsize", "40", 0,
...
void CBlockingFileIOPanel::Paint()
{
	...
	if(bval > 0)
	{
		...
		if(list)
		{
			...
			y += 2 * (itemHeight + 2);
			DrawIOTime(x, y, w, itemHeight, FILESYSTEM_BLOCKING_ASYNCHRONOUS_BLOCK, "Async Block",
					   Color(255, 100, 0, 255));
			y += 2 * (itemHeight + 2);
			...
			for(i = 0; i < c; ++i)
			{
				...
				else
				{
					...
					{
						m_History[i].m_flHistory = m_History[i].m_flHistory * IO_DECAY_FRAC +
...
void CBlockingFileIOPanel::SpewItem(const RecentPeaks_t &item)
{
	switch(item.reason)
	{
		...
		case FILESYSTEM_BLOCKING_ASYNCHRONOUS:
		case FILESYSTEM_BLOCKING_CALLBACKTIMING:
			Msg("%8.3f %16.16s i/o [%6.6s] took %8.3f msec:  %33.33s\n", item.time, GetBlockReason(item.reason),
				GetIOType(item.ioType), item.elapsed * 1000.0f, item.fileName.String());
			break;
		case FILESYSTEM_BLOCKING_SYNCHRONOUS:
		case FILESYSTEM_BLOCKING_ASYNCHRONOUS_BLOCK:
...
void CBlockingFileIOPanel::DrawIOTime(int x, int y, int w, int h, int slot, char const *label, const Color &clr)
{
	...
	if(m_History[slot].m_LastFile.IsValid() && (gpGlobals->realtime < latchedtime + 10.0f))
	{
		bDrawHistorySpike = true;
		g_pMatSystemSurface->DrawColoredText(m_hFont, x + w + 5, y + 1, 255, 255, 255, 200, "[%8.3f ms]",
											 m_History[slot].m_flHistorySpike * 1000.0f);
		g_pMatSystemSurface->DrawColoredText(m_hFont, x, y + h + 1, 255, 255, 255, 200, "%s",
