...
ConVar cl_show_num_particle_systems("cl_show_num_particle_systems", "0", FCVAR_CLIENTDLL,
...
ConVar voice_vox("voice_vox", "0", FCVAR_ARCHIVE, "Voice chat uses a vox-style always on", true, 0, true, 1,
				 VoxCallback);
...
void ClientModeShared::Update()
{
	...
	if(cl_show_num_particle_systems.GetBool())
	{
		...
		for(int i = 0; i < g_pParticleSystemMgr->GetParticleSystemCount(); i++)
		{
			...
			for(CParticleCollection *pCurCollection = pParticleSystem->FirstCollection(); pCurCollection != NULL;
...
int ClientModeShared::KeyInput(int down, ButtonCode_t keynum, const char *pszCurrentBinding)
{
	...
	if(pszCurrentBinding &&
	   (Q_strcmp(pszCurrentBinding, "messagemode") == 0 || Q_strcmp(pszCurrentBinding, "say") == 0))
	{
		if(down)
		{
		...
		return 0;
			...
			(Q_strcmp(pszCurrentBinding, "messagemode2") == 0 || Q_strcmp(pszCurrentBinding, "say_team") == 0))
	{
		if(down)
		{
...
void ClientModeShared::FireGameEvent(IGameEvent *event)
{
	...
	if(Q_strcmp("player_connect_client", eventname) == 0)
	{
		...
		if(!IsInCommentaryMode())
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#game_player_joined_game"), 1,
												  wszPlayerName);
	...
	else if(Q_strcmp("player_disconnect", eventname) == 0)
	{
		...
		if(!IsInCommentaryMode())
		{
			...
			if(IsPC())
			{
				g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#game_player_left_game"), 2,
													  wszPlayerName, wszReason);
			}
			else
			{
				g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#game_player_left_game"), 1,
													  wszPlayerName);
			}
			...
			if(!IsInCommentaryMode())
			{
				...
				if(bAutoTeamed)
				{
					g_pVGuiLocalize->ConstructString_safe(
						wszLocalized,
						bUsingCustomTeamName ? g_pVGuiLocalize->Find("#game_player_joined_autoteam_party_leader")
				...
				else
				{
					g_pVGuiLocalize->ConstructString_safe(
						wszLocalized,
						bUsingCustomTeamName ? g_pVGuiLocalize->Find("#game_player_joined_team_party_leader")
											 : g_pVGuiLocalize->Find("#game_player_joined_team"),
						2, wszPlayerName, wszTeam);
				}
	...
	else if(Q_strcmp("player_changename", eventname) == 0)
	{
		...
		wchar_t wszLocalized[100];
		g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#game_player_changed_name"), 2,
											  wszOldName, wszNewName);
	...
	else if(Q_strcmp("server_cvar", eventname) == 0)
	{
		if(!IsInCommentaryMode())
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#game_server_cvar_changed"), 2,
			...
			if(pAchievement)
			{
				...
				if(g_PR)
				{
					...
					if(pchLocalizedAchievement)
					{
						...
						g_pVGuiLocalize->ConstructString_safe(wszLocalizedString,
	...
	else if(Q_strcmp("item_found", eventname) == 0)
	{
		...
		const GameItemDefinition_t *pItemDefinition =
			dynamic_cast<GameItemDefinition_t *>(GetItemSchema()->GetItemDefinition(iItemDef));
		...
		if(g_PR)
		{
			...
			if(pszLocString)
			{
				...
				const CEconItemRarityDefinition *pItemRarity =
					GetItemSchema()->GetRarityDefinition(pItemDefinition->GetRarity());
				...
				if(colorMarker)
				{
					if(pItemRarity)
					{
						...
						Color color =
							pScheme->GetColor(GetColorNameForAttribColor(colorRarity), Color(255, 255, 255, 255));
				...
				if(pItemRarity)
				{
					...
					{
						loc_scpy_safe(szWear, CConstructLocalizedString(
												  g_pVGuiLocalize->Find("TFUI_InvTooltip_ItemFound_Wear"),
												  g_pVGuiLocalize->Find(GetWearLocalizationString(flWear))));
					}

					// Rarity / grade
					loc_scpy_safe(szRarity,
								  CConstructLocalizedString(g_pVGuiLocalize->Find("TFUI_InvTooltip_ItemFound_Rarity"),
															g_pVGuiLocalize->Find(pItemRarity->GetLocKey())));
				}

				if(bIsUnusual)
				{
					loc_scpy_safe(szUnusual,
								  CConstructLocalizedString(g_pVGuiLocalize->Find("TFUI_InvTooltip_ItemFound_Unusual"),
															g_pVGuiLocalize->Find("rarity4")));
				}

				if(bIsStrange)
				{
					loc_scpy_safe(szStrange,
								  CConstructLocalizedString(g_pVGuiLocalize->Find("TFUI_InvTooltip_ItemFound_Strange"),
															g_pVGuiLocalize->Find("strange")));
				}

				// // Strange Unusual Item Grade
				loc_scpy_safe(wszLocalizedString,
							  CConstructLocalizedString(g_pVGuiLocalize->Find("TFUI_InvTooltip_ItemFound"), szStrange,
														szUnusual, szItemname, szRarity, szWear));

				loc_scpy_safe(tempName, wszLocalizedString);
				g_pVGuiLocalize->ConstructString_safe(wszLocalizedString, wszItemFound, 3, wszPlayerName, tempName,
													  L"");
	...
	{
		DisplayReplayMessage(event->GetString("error", "#Replay_DefaultServerError"),
	...
	{
		DisplayReplayMessage("#Replay_ReplaysAvailable", replay_msgduration_replaysavailable.GetFloat(), false, NULL,
							 false);
	}

	else if(!V_strcmp("game_newmap", eventname))
	{
		// Make sure the instance count is reset to 0.  Sometimes the count stay in sync and we get replay messages
		// displaying lower than they should.
...
void ClientModeShared::UpdateReplayMessages()
{
	...
	{
		DisplayReplayMessage("#Replay_StartRecord", replay_msgduration_startrecord.GetFloat(), true,
	...
	{
		DisplayReplayMessage("#Replay_EndRecord", replay_msgduration_stoprecord.GetFloat(), true,
...
void ClientModeShared::DisplayReplayReminder()
{
	...
	if(m_pReplayReminderPanel && g_pReplay->IsRecording() && !::input->IsSteamControllerActive())
	{
		...
		CReplay *pCurLifeReplay =
			static_cast<CReplay *>(g_pClientReplayContext->GetReplayManager()->GetReplayForCurrentLife());
