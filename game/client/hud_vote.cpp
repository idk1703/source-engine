...
public:
	...
	virtual void Trigger()
	{
		CTFGenericConfirmDialog *pDialog =
			ShowConfirmDialog("#GameUI_Vote_Notification_Title", "#GameUI_Vote_Notification_Text",
							  "#GameUI_Vote_Notification_View", "#cancel", &ConfirmShowVoteSetup);
...
void VoteBarPanel::Paint(void)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		vgui::surface()->DrawFilledRect(x + m_iBoxInset, m_iBoxInset, x + m_iBoxSize - m_iBoxInset,
										m_iBoxSize - m_iBoxInset);
	...
	vgui::surface()->DrawTexturedRect(x + iImageInset, iImageInset, x + m_iBoxSize - iImageInset,
									  m_iBoxSize - iImageInset);
...
void CVoteSetupDialog::InitializeIssueList(void)
{
	...
	for(int index = 0; index < m_VoteIssues.Count(); index++)
	{
		...
		g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find(m_VoteIssues[index].szNameString), szIssueLocalized,
											  sizeof(szIssueLocalized));

		if(!bActive)
		{
			char szDisabled[k_MAX_VOTE_NAME_LENGTH] = {0};
			g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find("#GameUI_Vote_Disabled"), szDisabled,
												  sizeof(szDisabled));
	...
	else
	{
		// No active issues
		char szIssueLocalized[k_MAX_VOTE_NAME_LENGTH] = {0};
		g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find("#GameUI_Vote_System_Disabled"), szIssueLocalized,
											  sizeof(szIssueLocalized));
...
void CVoteSetupDialog::Activate()
{
	...
	m_pVoteSetupList->AddColumnToSection(0, "Issue", "#TF_Vote_Column_Issue", SectionedListPanel::COLUMN_CENTER,
										 m_iIssueWidth);
	...
	m_pVoteParameterList->AddColumnToSection(0, "Avatar", "",
											 SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_RIGHT, 55);
	...
	m_pVoteParameterList->AddColumnToSection(0, "Properties", "#TF_Vote_Column_Properties",
											 SectionedListPanel::COLUMN_CENTER, m_iParameterWidth * 0.3);
			...
			else if(!V_stricmp("Kick", szIssueRaw))
			{
				...
				if(iSelectedParam >= 0)
				{
					...
					if(pKeyValues)
					{
						...
						const char *pReasonString = m_pComboBox->GetActiveItemUserData()
														? m_pComboBox->GetActiveItemUserData()->GetName()
														: "other";
						...
						if(engine->GetPlayerInfo(playerIndex, &playerInfo))
						{
							...
							Q_snprintf(szVoteCommand, sizeof(szVoteCommand), "callvote %s \"%d %s\"\n;", szIssueRaw,
									   pPlayer->GetUserID(), pReasonString);
							engine->ClientCmd(szVoteCommand);
#ifdef TF_CLIENT_DLL
							CSteamID steamID;
		...
		if(iSelectedItem >= 0)
		{
			...
			else if(!V_stricmp("ChangeLevel", pszIssueRaw) || !V_stricmp("NextLevel", pszIssueRaw))
			{
				...
				for(int index = 0; index < m_VoteIssuesMapCycle.Count(); index++)
				{
					...
					if(V_strncmp(m_VoteIssuesMapCycle[index], m_szCurrentMap,
								 (V_strlen(m_VoteIssuesMapCycle[index]) - 1)) == 0)
						continue;
			...
			else if(!V_stricmp("Kick", pszIssueRaw))
			{
				...
				for(int playerIndex = 1; playerIndex <= nMaxClients; playerIndex++)
				{
					...
					if(TFGameRules() && TFGameRules()->IsMannVsMachineMode() && g_PR->IsConnected(playerIndex) &&
			...
			else if(!V_stricmp("ChangeMission", pszIssueRaw))
			{
				...
				for(int index = 0; index < m_VoteIssuesPopFiles.Count(); index++)
				{
					...
					if(!pszPopFileName || !pszPopFileName[0])
					{
						...
						if(V_strncmp(m_VoteIssuesPopFiles[index], V_GetFileName(szShortMapName),
									 (V_strlen(m_VoteIssuesPopFiles[index]) - 1)) == 0)
							continue;
					}
					else
					{
						// Use the specified pop file
						if(V_strncmp(m_VoteIssuesPopFiles[index], TFObjectiveResource()->GetMvMPopFileName(),
									 (V_strlen(m_VoteIssuesPopFiles[index]) - 1)) == 0)
							continue;
					}
...
void CHudVote::MsgFunc_CallVoteFailed(bf_read &msg)
{
	...
	switch(nReason)
	{
		...
		{
			const char *pszTimeString =
				(bMinutes) ? ((nTime < 2) ? "#GameUI_vote_failed_vote_spam_min" : "#GameUI_vote_failed_vote_spam_mins")
						   : "#GameUI_vote_failed_vote_spam";
		...
		{
			const char *pszTimeString =
				(bMinutes) ? ((nTime < 2) ? "#GameUI_vote_failed_recently_min" : "#GameUI_vote_failed_recently_mins")
						   : "#GameUI_vote_failed_recently";
			g_pVGuiLocalize->ConstructString_safe(wszHeaderString, g_pVGuiLocalize->Find(pszTimeString), 1, wszTime);
			m_pCallVoteFailed->SetDialogVariable("FailedReason", wszHeaderString);
			break;
		}
		...
		{
			const char *pszTimeString = (bMinutes) ? ((nTime < 2) ? "#GameUI_vote_failed_cannot_kick_min"
																  : "#GameUI_vote_failed_cannot_kick_mins")
												   : "#GameUI_vote_failed_cannot_kick";
			g_pVGuiLocalize->ConstructString_safe(wszHeaderString, g_pVGuiLocalize->Find(pszTimeString), 1, wszTime);
			m_pCallVoteFailed->SetDialogVariable("FailedReason", wszHeaderString);
			break;
		}
...
void CHudVote::MsgFunc_VoteStart(bf_read &msg)
{
	...
	// String
	g_pVGuiLocalize->ConstructString_safe(wszHeaderString, g_pVGuiLocalize->Find("#GameUI_vote_header"), 1,
										  wszCallerName);
	...
	if(m_bIsYesNoVote)
	{
		...
		wchar_t *pszText = g_pVGuiLocalize->Find(
			::input->IsSteamControllerActive() ? "#GameUI_vote_yes_sc_instruction" : "#GameUI_vote_yes_pc_instruction");
		if(pszText)
		{
		...
		pszText = g_pVGuiLocalize->Find(::input->IsSteamControllerActive() ? "#GameUI_vote_no_sc_instruction"
																		   : "#GameUI_vote_no_pc_instruction");
		if(pszText)
		{
			UTIL_ReplaceKeyBindings(pszText, 0, wzFinal, sizeof(wzFinal), GAME_ACTION_SET_FPSCONTROLS);
			if(m_pVoteActive)
...
void CHudVote::MsgFunc_VoteSetup(bf_read &msg)
{
	...
	{
		int index =
			bMvM ? pStringTable->FindStringIndex("ServerMapCycleMvM") : pStringTable->FindStringIndex("ServerMapCycle");
...
void CHudVote::OnThink()
{
	...
	if(pLocalPlayer)
	{
		...
		if(m_flVoteResultCycleTime > 0 && gpGlobals->curtime > m_flVoteResultCycleTime)
		{
			...
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(m_pVoteActive,
																					"HideVoteBackgrounds");
