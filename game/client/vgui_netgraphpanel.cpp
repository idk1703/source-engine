...
static ConVar net_graphmsecs("net_graphmsecs", "400", FCVAR_ARCHIVE,
...
static ConVar net_graph("net_graph", "0", 0,
						"Draw the network usage graph, = 2 draws data on payload, = 3 draws payload legend.",
						NetgraphFontChangeCallback);
static ConVar net_graphheight("net_graphheight", "64", FCVAR_ARCHIVE, "Height of netgraph panel",
							  NetgraphFontChangeCallback);
static ConVar net_graphproportionalfont("net_graphproportionalfont", "1", FCVAR_ARCHIVE,
...
public:
	...
	inline void DrawLine2(vrect_t *rect, unsigned char *color, unsigned char *color2, unsigned char alpha,
...
// Input  : *parent -
//-----------------------------------------------------------------------------
CNetGraphPanel::CNetGraphPanel(VPANEL parent) : BaseClass(NULL, "CNetGraphPanel")
{
	int w, h;
...
//			1 -
//-----------------------------------------------------------------------------
void CNetGraphPanel::GetFrameData(INetChannelInfo *netchannel, int *biggest_message, float *avg_message,
...
void CNetGraphPanel::DrawTextFields(int graphvalue, int x, int y, int w, netbandwidthgraph_t *graph, cmdinfo_t *cmdinfo)
{
	...
	g_pMatSystemSurface->DrawColoredText(font, x + textWidth, y, interpcolor[0], interpcolor[1], interpcolor[2], 255,
										 "%s", sz);

	Q_snprintf(sz, sizeof(sz), "%3.1f/s", m_AvgPacketIn);
	textWidth = g_pMatSystemSurface->DrawTextLen(font, "%s", sz);

	g_pMatSystemSurface->DrawColoredText(font, x + w - textWidth - 1, y, GRAPH_RED, GRAPH_GREEN, GRAPH_BLUE, 255, "%s",
										 sz);
	...
	if(graphvalue > 2)
	{
		Q_snprintf(sz, sizeof(sz), "loss:%3i    choke: %2i ", (int)(m_AvgPacketLoss * 100.0f),
		...
		if(graphvalue > 3)
		{
			Q_snprintf(sz, sizeof(sz), "sv  : %5.1f   var: %4.2f msec", m_flServerFramerate,
			...
			g_pMatSystemSurface->DrawColoredText(font, x, y, servercolor[0], servercolor[1], servercolor[2], 255, "%s",
												 sz);
	...
	else
	{
		...
		if(frequency.m_GHz > 0 && frequency.m_timeStamp + displayTime > currentTime)
		{
			...
			g_pMatSystemSurface->DrawColoredText(font, x, y, cpuColor[0], cpuColor[1], cpuColor[2], cpuColor[3],
												 "CPU freq: %3.1f%%   Min: %3.1f%%", frequency.m_percentage,
												 frequency.m_lowestPercentage);
		}
	}
}
...
void CNetGraphPanel::DrawHatches(int x, int y, int maxmsgbytes)
{
	...
	for(starty = rcHatch.y; rcHatch.y > 0 && ((starty - rcHatch.y) * net_scale.GetFloat() < (maxmsgbytes + 50));
		rcHatch.y -= ystep)
	{
...
void CNetGraphPanel::DrawUpdateRate(int xright, int y)
{
	...
	g_pMatSystemSurface->DrawColoredText(GetNetgraphFont(), xright - textWide - 1, y, GRAPH_RED, GRAPH_GREEN,
										 GRAPH_BLUE, 255, "%s", sz);
}
...
void CNetGraphPanel::DrawLargePacketSizes(int x, int w, int graphtype, float warning_threshold)
{
	...
	for(a = 0; a < w; a++)
	{
		...
		if(warning_threshold != 0.0f && nTotalBytes > MAX(300, warning_threshold))
		{
			char sz[32];
...
//			alpha -
//-----------------------------------------------------------------------------
void CNetGraphPanel::DrawLine2(vrect_t *rect, unsigned char *color, unsigned char *color2, unsigned char alpha,
							   unsigned char alpha2)
{
