...
//			flags -
//-----------------------------------------------------------------------------
void FX_BloodSpray(const Vector &origin, const Vector &normal, float scale, unsigned char r, unsigned char g,
				   unsigned char b, int flags)
{
	...
	// C_BaseEntity::EmitSound( filter, SOUND_FROM_WORLD, CHAN_VOICE, "Physics.WaterSplash", 1.0, ATTN_NORM, 0, 100,
	// &origin );
}
...
//-----------------------------------------------------------------------------
void FX_BloodBulletImpact(const Vector &origin, const Vector &normal, float scale /*NOTE: Unused!*/, unsigned char r,
...
void HunterDamageCallback(const CEffectData &data)
{
	...
	pGlassEmitter->m_ParticleCollision.Setup(data.m_vOrigin, &data.m_vNormal, flAngularSpray, flMinSpeed, flMaxSpeed,
											 600.0f, 0.2f);
	...
	for(int i = 0; i < numFlecks; i++)
	{
		...
		pFleckParticle = (Particle3D *)pGlassEmitter->AddParticle(
