...
CPDumpPanel::CPDumpPanel(const char *pElementName) : CHudElement(pElementName), BaseClass(NULL, "HudPredictionDump")
{
	g_pPDumpPanel = this;
...
void CPDumpPanel::DumpComparision(const char *classname, const char *fieldname, const char *fieldtype, bool networked,
								  bool noterrorchecked, bool differs, bool withintolerance, const char *value)
{
	if(fieldname == NULL)
		return;
...
// Output : static void
//-----------------------------------------------------------------------------
static void DumpComparision(const char *classname, const char *fieldname, const char *fieldtype, bool networked,
							bool noterrorchecked, bool differs, bool withintolerance, const char *value)
{
	if(!g_pPDumpPanel)
		return;

	g_pPDumpPanel->DumpComparision(classname, fieldname, fieldtype, networked, noterrorchecked, differs,
...
// Output : static void
//-----------------------------------------------------------------------------
void CPDumpPanel::PredictionDumpColor(bool networked, bool errorchecked, bool differs, bool withintolerance, int &r,
									  int &g, int &b, int &a)
{
	r = 255;
	g = 255;
...
void CPDumpPanel::DumpEntity(C_BaseEntity *ent, int commands_acknowledged)
{
	...
	CPredictionCopy datacompare(PC_EVERYTHING, original_state_data, data_type_original, predicted_state_data,
								data_type_predicted,
								true,  // counterrors
								true,  // reporterrors
								false, // copy data
