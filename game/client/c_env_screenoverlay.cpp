...
RecvPropArray(RecvPropString(RECVINFO(m_iszOverlayNames[0])), m_iszOverlayNames),
	...
	RecvPropFloat(RECVINFO(m_flStartTime)), RecvPropInt(RECVINFO(m_iDesiredOverlay)),
...
void C_EnvScreenOverlay::PostDataUpdate(DataUpdateType_t updateType)
{
	...
	if(updateType == DATA_UPDATE_CREATED && CommandLine()->FindParm("-makereslists"))
	{
...
{
	if(m_iCurrentOverlay == MAX_SCREEN_OVERLAYS || !m_iszOverlayNames[m_iCurrentOverlay] ||
	   !m_iszOverlayNames[m_iCurrentOverlay][0])
	{
	...
	IMaterial *pMaterial =
		materials->FindMaterial(m_iszOverlayNames[m_iCurrentOverlay], TEXTURE_GROUP_CLIENT_EFFECTS, false);
...
RecvPropFloat(RECVINFO(m_flDuration)), RecvPropInt(RECVINFO(m_nType)),
END_RECV_TABLE
()
