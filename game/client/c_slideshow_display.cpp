...
RecvPropBool(RECVINFO(m_bEnabled)), RecvPropString(RECVINFO(m_szDisplayText)),
	...
	RecvPropFloat(RECVINFO(m_fMinSlideTime)), RecvPropFloat(RECVINFO(m_fMaxSlideTime)),
	RecvPropInt(RECVINFO(m_iCycleType)), RecvPropBool(RECVINFO(m_bNoListRepeats)),
END_RECV_TABLE
()
...
void C_SlideshowDisplay::BuildSlideShowImagesList(void)
{
	...
	while(szMatFileName[0])
	{
		...
		Q_snprintf(szFullFileName, sizeof(szFullFileName), "materials/vgui/%s/%s", m_szSlideshowDirectory,
				   szMatFileName);
