	...
	switch(iType)
	{
		case PERFDATA_LEVEL:
		{
			...
			V_snprintf(s_cPerfString, sizeof(s_cPerfString),
					   ...
					   flAverageFrameRate, flMinFrameRate, flMaxFrameRate, cpu.m_szProcessorID,
					   cpu.m_Speed * (1.0 / 1.0e9), cpu.m_nPhysicalProcessors, SafeString(gpu.m_pDriverName),
					   gpu.m_VendorID, gpu.m_DeviceID, gpu.m_nDriverVersionHigh, gpu.m_nDriverVersionLow,
					   g_pMaterialSystemHardwareConfig->GetDXSupportLevel(), dest_width, dest_height, szMap);
...
void CStatsRecorder::UploadPerfData(int iType)
{
	if(g_pClientGameStatsUploader)
	{
		...
		if(pPerfData)
		{
			g_pClientGameStatsUploader->UploadGameStats("", 1, 1 + strlen(pPerfData), pPerfData);
		}
	}
}