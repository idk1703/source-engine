...
RecvPropFloat(RECVINFO(m_flFadeStartDist)), RecvPropFloat(RECVINFO(m_flFadeDist)),
	RecvPropFloat(RECVINFO(m_flTranslucencyLimit)), RecvPropInt(RECVINFO(m_iBackgroundModelIndex))
END_RECV_TABLE
()
...
int C_FuncAreaPortalWindow::DrawModel(int flags)
{
	...
	if(m_iBackgroundModelIndex >= 0)
	{
		...
		{
			render->DrawBrushModelEx(this, pBackground, GetAbsOrigin(), GetAbsAngles(), mode);
		}
	}
