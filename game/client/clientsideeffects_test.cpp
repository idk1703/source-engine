...
{
	CFXDiscreetLine *t =
		new CFXDiscreetLine("Line", start, direction, velocity, length, clipLength, scale, life, shader);
	assert(t);
...
//-----------------------------------------------------------------------------
void FX_AddQuad(const Vector &origin, const Vector &normal, float startSize, float endSize, float sizeBias,
				float startAlpha, float endAlpha, float alphaBias, float yaw, float deltaYaw, const Vector &color,
...
void FX_TracerSound(const Vector &start, const Vector &end, int iTracerType)
{
	...
	if(C_BaseEntity::GetParametersForSound(pszSoundName, params, NULL))
	{
		...
		enginesound->EmitSound(filter, SOUND_FROM_WORLD, CHAN_STATIC, params.soundname, params.volume,
							   SNDLVL_TO_ATTN(params.soundlevel), 0, params.pitch, 0, &start, &shotDir, NULL);
	}
