...
void TE_BSPDecal(IRecipientFilter &filter, float delay, const Vector *pos, int entity, int index)
{
	...
	if(r_decals.GetInt())
	{
		effects->DecalShoot(index, entity, ent->GetModel(), ent->GetAbsOrigin(), ent->GetAbsAngles(), *pos, 0,
							FDECAL_PERMANENT);
	}
}
...
void C_TEBSPDecal::PostDataUpdate(DataUpdateType_t updateType)
{
	...
	if(r_decals.GetInt())
	{
		effects->DecalShoot(m_nIndex, m_nEntity, ent->GetModel(), ent->GetAbsOrigin(), ent->GetAbsAngles(), m_vecOrigin,
							0, FDECAL_PERMANENT);
	}
}

IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEBSPDecal, DT_TEBSPDecal, CTEBSPDecal)
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropInt(RECVINFO(m_nEntity)), RecvPropInt(RECVINFO(m_nIndex)),
END_RECV_TABLE
()