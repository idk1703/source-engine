...
void CTextureAllocator::Init()
{
	...
	m_TexturePage.InitRenderTarget(TEXTURE_PAGE_SIZE, TEXTURE_PAGE_SIZE, RT_SIZE_NO_CHANGE, IMAGE_FORMAT_ARGB8888,
	...
	m_TexturePage.InitRenderTargetTexture(TEXTURE_PAGE_SIZE, TEXTURE_PAGE_SIZE, RT_SIZE_NO_CHANGE,
...
void CTextureAllocator::DebugPrintCache(void)
{
	...
	for(int f = 0; f < nNumFragments; f++)
	{
		if((m_Fragments[f].m_FrameUsed != 0) && (m_Fragments[f].m_Texture != INVALID_TEXTURE_HANDLE))
			Warning("Fragment %d, Block: %d, Index: %d, Texture: %d Frame Used: %d\n", f, m_Fragments[f].m_Block,
					m_Fragments[f].m_Index, m_Fragments[f].m_Texture, m_Fragments[f].m_FrameUsed);
		else
...
public:
	...
	void ComputeShadowBBox(IClientRenderable *pRenderable, const Vector &vecAbsCenter, float flRadius, Vector *pAbsMins,
						   Vector *pAbsMaxs);
...
private:
	...
	void BuildGeneralWorldToShadowMatrix(VMatrix &matWorldToShadow, const Vector &origin, const Vector &dir,
	...
	float ComputeLocalShadowOrigin(IClientRenderable *pRenderable, const Vector &mins, const Vector &maxs,
	...
	void BuildOrthoShadow(IClientRenderable *pRenderable, ClientShadowHandle_t handle, const Vector &mins,
						  const Vector &maxs);

	// Builds a more complex shadow...
	void BuildRenderToTextureShadow(IClientRenderable *pRenderable, ClientShadowHandle_t handle, const Vector &mins,
									const Vector &maxs);
	...
	void ComputeExtraClipPlanes(IClientRenderable *pRenderable, ClientShadowHandle_t handle, const Vector *vec,
	...
	int BuildActiveShadowDepthList(const CViewSetup &viewSetup, int nMaxDepthShadows,
...
private:
	...
	// If either changes in a frame, PreRender() will catch it and do the appropriate allocation, deallocation or
	// reallocation
...
CClientShadowMgr::CClientShadowMgr()
	: m_DirtyShadows(0, 0, ShadowHandleCompareFunc), m_RenderToTextureActive(false), m_bDepthTextureActive(false)
{
...
void CClientShadowMgr::InitDepthTextureShadows()
{
	...
	if(!m_bDepthTextureActive)
	{
		...
		ImageFormat nullFormat =
			materials
				->GetNullTextureFormat(); // Vendor-dependent null texture format (takes as little memory as possible)
#endif
		...
		m_DummyColorTexture.InitRenderTargetTexture(r_flashlightdepthres.GetInt(), r_flashlightdepthres.GetInt(),
													RT_SIZE_OFFSCREEN, IMAGE_FORMAT_BGR565, MATERIAL_RT_DEPTH_SHARED,
													false, "_rt_ShadowDummy");
		m_DummyColorTexture.InitRenderTargetSurface(r_flashlightdepthres.GetInt(), r_flashlightdepthres.GetInt(),
													IMAGE_FORMAT_BGR565, true);
#else
		m_DummyColorTexture.InitRenderTarget(r_flashlightdepthres.GetInt(), r_flashlightdepthres.GetInt(),
		...
		for(int i = 0; i < m_nMaxDepthTextureShadows; i++)
		{
			...
			depthTex.InitRenderTargetTexture(m_nDepthTextureResolution, m_nDepthTextureResolution, RT_SIZE_OFFSCREEN,
											 dstFormat, MATERIAL_RT_DEPTH_NONE, false, strRTName);
			depthTex.InitRenderTargetSurface(1, 1, dstFormat, false);
#else
			depthTex.InitRenderTarget(m_nDepthTextureResolution, m_nDepthTextureResolution, RT_SIZE_OFFSCREEN,
									  dstFormat, MATERIAL_RT_DEPTH_NONE, false, strRTName);
#endif
	...
	if(!m_RenderToTextureActive)
	{
		...
		for(ClientShadowHandle_t i = m_Shadows.Head(); i != m_Shadows.InvalidIndex(); i = m_Shadows.Next(i))
		{
			...
			if(shadow.m_Flags & SHADOW_FLAGS_USE_RENDER_TO_TEXTURE)
			{
				...
				shadowmgr->SetShadowMaterial(shadow.m_ShadowHandle, m_RenderShadow, m_RenderModelShadow,
											 (void *)(uintp)i);
			}
		}
	}
...
void CClientShadowMgr::ShutdownRenderToTextureShadows()
{
	if(m_RenderToTextureActive)
	{
		...
		for(ClientShadowHandle_t i = m_Shadows.Head(); i != m_Shadows.InvalidIndex(); i = m_Shadows.Next(i))
		{
			...
			ClientShadow_t &shadow = m_Shadows[i];
			shadowmgr->SetShadowMaterial(shadow.m_ShadowHandle, m_SimpleShadow, m_SimpleShadow,
...
//-----------------------------------------------------------------------------
void CClientShadowMgr::UpdateFlashlightState(ClientShadowHandle_t shadowHandle,
...
//-----------------------------------------------------------------------------
void CClientShadowMgr::BuildGeneralWorldToShadowMatrix(VMatrix &matWorldToShadow, const Vector &origin,
													   const Vector &dir, const Vector &xvec, const Vector &yvec)
{
...
void CClientShadowMgr::BuildWorldToShadowMatrix(VMatrix &matWorldToShadow, const Vector &origin,
...
void CClientShadowMgr::BuildPerspectiveWorldToFlashlightMatrix(VMatrix &matWorldToShadow,
															   const FlashlightState_t &flashlightState)
{
	...
	MatrixBuildPerspective(matPerspective, flashlightState.m_fHorizontalFOVDegrees,
						   flashlightState.m_fVerticalFOVDegrees, flashlightState.m_NearZ, flashlightState.m_FarZ);
...
// Compute the shadow origin and attenuation start distance
//-----------------------------------------------------------------------------
float CClientShadowMgr::ComputeLocalShadowOrigin(IClientRenderable *pRenderable, const Vector &mins, const Vector &maxs,
												 const Vector &localShadowDir, float backupFactor, Vector &origin)
{
...
static void BuildOrthoWorldToShadowMatrix(VMatrix &worldToShadow, const Vector &origin, const Vector &dir,
										  const Vector &xvec, const Vector &yvec)
{
...
// Compute the extra shadow planes
//-----------------------------------------------------------------------------
void CClientShadowMgr::ComputeExtraClipPlanes(IClientRenderable *pRenderable, ClientShadowHandle_t handle,
											  const Vector *vec, const Vector &mins, const Vector &maxs,
...
//-----------------------------------------------------------------------------
static void BuildShadowLeafList(CShadowLeafEnum *pEnum, const Vector &origin, const Vector &dir, const Vector2D &size,
								float maxDist)
{
	Ray_t ray;
...
// Builds a simple blobby shadow
//-----------------------------------------------------------------------------
void CClientShadowMgr::BuildOrthoShadow(IClientRenderable *pRenderable, ClientShadowHandle_t handle, const Vector &mins,
										const Vector &maxs)
{
	...
	shadowmgr->ProjectShadow(m_Shadows[handle].m_ShadowHandle, worldOrigin, vecShadowDir, matWorldToTexture, size,
							 nCount, pLeafList, maxHeight, falloffStart, MAX_FALLOFF_AMOUNT,
...
// Visualization....
//-----------------------------------------------------------------------------
void CClientShadowMgr::DrawRenderToTextureDebugInfo(IClientRenderable *pRenderable, const Vector &mins,
													const Vector &maxs)
{
	if(!debugoverlay)
		return;
...
// Builds a more complex shadow...
//-----------------------------------------------------------------------------
void CClientShadowMgr::BuildRenderToTextureShadow(IClientRenderable *pRenderable, ClientShadowHandle_t handle,
												  const Vector &mins, const Vector &maxs)
{
	...
	// to the projection direction, then add in the projection of the perp direction
	Vector2D size;
	size.x = boxSize.x * fabs(DotProduct(vec[0], xvec)) + boxSize.y * fabs(DotProduct(vec[1], xvec)) +
			 boxSize.z * fabs(DotProduct(vec[2], xvec));
	size.y = boxSize.x * fabs(DotProduct(vec[0], yvec)) + boxSize.y * fabs(DotProduct(vec[1], yvec)) +
...
static void LineDrawHelper(const Vector &startShadowSpace, const Vector &endShadowSpace, const VMatrix &shadowToWorld,
...
// Adds the child bounds to the bounding box
//-----------------------------------------------------------------------------
void CClientShadowMgr::AddChildBounds(matrix3x4_t &matWorldToBBox, IClientRenderable *pParent, Vector &vecMins,
									  Vector &vecMaxs)
{
...
void CClientShadowMgr::AddToDirtyShadowList(IClientRenderable *pRenderable, bool bForce)
{
	...
	if(handle != CLIENTSHADOW_INVALID_HANDLE)
	{
		IClientRenderable *pShadowRenderable =
			ClientEntityList().GetClientRenderableFromHandle(m_Shadows[handle].m_Entity);
...
//-----------------------------------------------------------------------------
void CClientShadowMgr::ComputeShadowBBox(IClientRenderable *pRenderable, const Vector &vecAbsCenter, float flRadius,
	...
	if(foundSeparatingPlane)
	{
		...
		if(shadowDot > 0.0f)
		{
			if(receiverDot <= sourceDot)
			{
				...
				//				debugoverlay->AddLineOverlay( pSourceRenderable->GetRenderOrigin(), dest, 255, 255, 0,
				//true, 1.0f );
				return true;
			}
			else
			{
				//				Vector dest;
				//				VectorMA( pSourceRenderable->GetRenderOrigin(), 50, plane.normal, dest );
				//				debugoverlay->AddLineOverlay( pSourceRenderable->GetRenderOrigin(), dest, 255, 0, 0,
				//true, 1.0f );
			}
		}
		else
...
// deals with shadows being added to shadow receivers
//-----------------------------------------------------------------------------
void CClientShadowMgr::AddShadowToReceiver(ClientShadowHandle_t handle, IClientRenderable *pRenderable,
										   ShadowReceiver_t type)
{
	...
	switch(type)
	{
		case SHADOW_RECEIVER_BRUSH_MODEL:
			...
			{
				shadowmgr->AddShadowToBrushModel(shadow.m_ShadowHandle, const_cast<model_t *>(pRenderable->GetModel()),
												 pRenderable->GetRenderOrigin(), pRenderable->GetRenderAngles());
			}
			break;
...
// deals with shadows being added to shadow receivers
//-----------------------------------------------------------------------------
void CClientShadowMgr::RemoveAllShadowsFromReceiver(IClientRenderable *pRenderable, ShadowReceiver_t type)
{
...
//-----------------------------------------------------------------------------
bool CClientShadowMgr::BuildSetupShadowHierarchy(IClientRenderable *pRenderable, const ClientShadow_t &shadow,
												 bool bChild)
{
	...
	if(!pRenderable || shadowType != SHADOWS_NONE)
	{
		...
		if(bDrawModelShadow)
		{
			...
			matrix3x4_t *pBoneToWorld =
				modelrender->DrawModelShadowSetup(pRenderable, pRenderable->GetBody(), pRenderable->GetSkin(), &info);
			if(pBoneToWorld)
			{
...
void CClientShadowMgr::DrawRenderToTextureShadowLOD(unsigned short clientShadowHandle)
{
	...
	if((shadow.m_Flags & SHADOW_FLAGS_USING_LOD_SHADOW) == 0)
	{
		...
		ClearExtraClipPlanes(
			clientShadowHandle); // this was ClearExtraClipPlanes( shadow.m_ShadowHandle ), fix is from Joe Demers
		shadow.m_Flags |= SHADOW_FLAGS_USING_LOD_SHADOW;
	}
}
...
//-----------------------------------------------------------------------------
int CClientShadowMgr::BuildActiveShadowDepthList(const CViewSetup &viewSetup, int nMaxDepthShadows,
												 ClientShadowHandle_t *pActiveDepthShadows)
{
	...
	for(ClientShadowHandle_t i = m_Shadows.Head(); i != m_Shadows.InvalidIndex(); i = m_Shadows.Next(i))
	{
		...
		GeneratePerspectiveFrustum(viewSetup.origin, viewSetup.angles, viewSetup.zNear, viewSetup.zFar, viewSetup.fov,
...
void CClientShadowMgr::ComputeShadowDepthTextures(const CViewSetup &viewSetup)
{
	...
	int nActiveDepthShadowCount =
		BuildActiveShadowDepthList(viewSetup, ARRAYSIZE(pActiveDepthShadows), pActiveDepthShadows);
	...
	for(int j = 0; j < nActiveDepthShadowCount; ++j)
	{
		...
		pRenderContextMat->SetShadowDepthBiasFactors(flashlightState.m_flShadowSlopeScaleDepthBias,
...
void CClientShadowMgr::ComputeShadowTextures(const CViewSetup &viewShadow, int leafCount, LeafIndex_t *pLeafList)
{
	...
	if(m_bThreaded && g_pThreadPool->NumIdleThreads())
	{
		...
		ParallelProcess("NPCShadowBoneSetups", s_NPCShadowBoneSetups.Base(), s_NPCShadowBoneSetups.Count(),
						&SetupBonesOnBaseAnimating);
		ParallelProcess("NonNPCShadowBoneSetups", s_NonNPCShadowBoneSetups.Base(), s_NonNPCShadowBoneSetups.Count(),
						&SetupBonesOnBaseAnimating);
