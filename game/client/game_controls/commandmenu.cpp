...
void CommandMenu::StartNewSubMenu(KeyValues *params)
{
	...
	menuitem.itemnr = m_CurrentMenu->AddCascadingMenuItem(params->GetString("label"), this, menu,
														  params); // add to current menu as item
...
void CommandMenu::AddMenuCommandItem(KeyValues *params)
{
	...
	menuitem.itemnr =
		m_CurrentMenu->AddMenuItem(params->GetString("label"), params->MakeCopy(), this, params); // add it
	m_MenuItems.AddToTail(menuitem);															  // add to global list
}
...
void CommandMenu::AddMenuToggleItem(KeyValues *params)
{
	...
	menuitem.menu = m_CurrentMenu; // save the current menu context
	menuitem.itemnr =
		m_CurrentMenu->AddCheckableMenuItem(params->GetString("label"), params->MakeCopy(), this, params); // add it
	m_MenuItems.AddToTail(menuitem); // add to global list
}
