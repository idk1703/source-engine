...
public:
	CNavButton(Panel *parent, const char *name, const char *text = "", Panel *pActionSignalTarget = NULL,
			   const char *cmd = NULL)
		: CExImageButton(parent, name, text, pActionSignalTarget, cmd), m_iUserData(-1)
	{
	}

	CNavButton(Panel *parent, const char *name, const wchar_t *wszText = L"", Panel *pActionSignalTarget = NULL,
			   const char *cmd = NULL)
...
void CNavigationPanel::ApplySettings(KeyValues *pInResourceData)
{
	...
	*/
	...
	{
		CNavButton *pNewButton =
			new CNavButton(this, pKVCurButton->GetString("FieldName", pKVCurButton->GetName()), L"");
...
public:
	CNavPanelTest(vgui::Panel *pParent) : Frame(pParent, "NavPanelTest")
	{
