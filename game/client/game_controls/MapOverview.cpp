...
ConVar overview_health("overview_health", "1", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
					   "Show player's health in map overview.\n");
ConVar overview_names("overview_names", "1", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
					  "Show player's names in map overview.\n");
ConVar overview_tracks("overview_tracks", "1", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
					   "Show player's tracks in map overview.\n");
ConVar overview_locked("overview_locked", "1", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
					   "Locks map angle, doesn't follow view angle.\n");
ConVar overview_alpha("overview_alpha", "1.0", FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
...
CON_COMMAND(overview_zoom, "Sets overview map zoom: <zoom> [<time>] [rel]")
{
	...
	// We are going to store their zoom pick as the resultant overview size that it sees.  This way, the value will
	// remain correct even on a different map that has a different intrinsic zoom.
	...
	g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(
		g_pMapOverview->GetAsPanel(), "zoom", zoom, 0.0, time, vgui::AnimationController::INTERPOLATOR_LINEAR);
}
...
void CMapOverview::Update(void)
{
	...
	m_bFollowAngle = (GetMode() != MAP_MODE_RADAR && !overview_locked.GetBool()) ||
...
void CMapOverview::DrawMapTexture()
{
	...
	Vertex_t points[4] = {Vertex_t(MapToPanel(Vector2D(0, 0)), Vector2D(0, 0)),
...
bool CMapOverview::DrawIcon(MapObject_t *obj)
{
	...
	Vertex_t points[4] = {Vertex_t(MapToPanel(pos1), Vector2D(0, 0)), Vertex_t(MapToPanel(pos2), Vector2D(1, 0)),
						  Vertex_t(MapToPanel(pos3), Vector2D(1, 1)), Vertex_t(MapToPanel(pos4), Vector2D(0, 1))};
...
void CMapOverview::SetMap(const char *levelname)
{
	...
	m_fNextTrailUpdate =
		0;			  // Set to 0 for immediate update.  Our WorldTime var hasn't been updated to 0 for the new map yet
	m_fWorldTime = 0; // In release, we occasionally race and get this bug again if we gt a paint before an update.
					  // Reset this before the old value gets in to the timer.
