...
void CSpectatorMenu::Update(void)
{
	...
	for(iPlayerIndex = 1; iPlayerIndex <= gpGlobals->maxClients; iPlayerIndex++)
	{
		...
		if(teamname)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(playerText, g_pVGuiLocalize->Find("#Spec_PlayerItem_Team"), 2,
												  playerName, team);
		}
		else
		{
	...
	if(engine->IsHLTV())
	{
		...
		else
		{
			...
			else
			{
				...
				else if(mode == OBS_MODE_POI) // PASSTIME skip POI mode since hltv doesn't have the entity data required
											  // to make it work
