...
void CModelPanel::ParseModelInfo(KeyValues *inResourceData)
{
	...
	m_pModelInfo->m_vecAbsAngles.Init(inResourceData->GetFloat("angles_x", 0.0),
									  inResourceData->GetFloat("angles_y", 0.0),
									  inResourceData->GetFloat("angles_z", 0.0));
	m_pModelInfo->m_vecOriginOffset.Init(inResourceData->GetFloat("origin_x", 110.0),
										 inResourceData->GetFloat("origin_y", 5.0),
										 inResourceData->GetFloat("origin_z", 5.0));
	m_pModelInfo->m_vecFramedOriginOffset.Init(inResourceData->GetFloat("frame_origin_x", 110.0),
											   inResourceData->GetFloat("frame_origin_y", 5.0),
...
void CModelPanel::SetupModel(void)
{
	...
	if(m_pModelInfo->m_Animations.Count() > 0 && m_pModelInfo->m_Animations.IsValidIndex(m_iDefaultAnimation))
	{
		...
		if(sequence != ACT_INVALID)
		{
			...
			{
				for(KeyValues *pData = pAnim->m_pPoseParameters->GetFirstSubKey(); pData != NULL;
...
void CModelPanel::Paint()
{
	...
	m_hModel->SetAbsAngles(
		QAngle(m_pModelInfo->m_vecAbsAngles.x, m_pModelInfo->m_vecAbsAngles.y, m_pModelInfo->m_vecAbsAngles.z));
	...
	static Vector white[6] = {
		Vector(0.4, 0.4, 0.4), Vector(0.4, 0.4, 0.4), Vector(0.4, 0.4, 0.4),
		Vector(0.4, 0.4, 0.4), Vector(0.4, 0.4, 0.4), Vector(0.4, 0.4, 0.4),
	};
	...
	if(m_pModelInfo->m_bUseSpotlight)
	{
		...
		LightDesc_t spotLight(vec3_origin + Vector(0, 0, 200), Vector(1, 1, 1),
							  m_hModel->GetAbsOrigin() + Vector(0, 0, (vecMaxs.z - vecMins.z) * 0.75), 0.035, 0.873);
...
void CModelPanel::CalculateFrameDistanceInternal(const model_t *pModel)
{
	...
	QAngle angPanelAngles(m_pModelInfo->m_vecAbsAngles.x, m_pModelInfo->m_vecAbsAngles.y,
						  m_pModelInfo->m_vecAbsAngles.z);
