...
void CLitSmokeEmitter::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		Vector color01 =
			Vector(pParticle->m_uchColor[0], pParticle->m_uchColor[1], pParticle->m_uchColor[2]) * (tLifetime / 255.0f);

		m_Renderer.RenderParticle_AddColor(pIterator->GetParticleDraw(), pParticle->m_Pos, tPos, alpha255,
