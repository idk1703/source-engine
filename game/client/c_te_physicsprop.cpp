...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropFloat(RECVINFO(m_angRotation[0])),
	...
	RecvPropVector(RECVINFO(m_vecVelocity)), RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropInt(RECVINFO(m_nFlags)),
	RecvPropInt(RECVINFO(m_nSkin)), RecvPropInt(RECVINFO(m_nEffects)),
END_RECV_TABLE
()
...
// Recording
//-----------------------------------------------------------------------------
static inline void RecordPhysicsProp(const Vector &start, const QAngle &angles, const Vector &vel, int nModelIndex,
									 bool bBreakModel, int nSkin, int nEffects)
{
	if(!ToolsEnabled())
		return;
...
void TE_PhysicsProp(IRecipientFilter &filter, float delay, int modelindex, int skin, const Vector &pos,
					const QAngle &angles, const Vector &vel, bool breakmodel, int fEffects)
{
