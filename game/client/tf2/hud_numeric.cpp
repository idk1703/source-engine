...
{
	if(gpGlobals->curtime >= m_flRotaryStartTime && gpGlobals->curtime <= (m_flRotaryStartTime + m_flRotaryTime))
	{
		return true;
	}
...
//			subfrac -
//-----------------------------------------------------------------------------
void CHudNumeric::GetRotatedChar(float frac, char startchar, char endchar, char &prevchar, char &nextchar,
								 float &subfrac)
{
	...
	// No delta
	if(diff == 0.0f || startidx == m_Printables.InvalidIndex() || endidx == m_Printables.InvalidIndex())
	{
		prevchar = startchar;
		nextchar = startchar;
		subfrac = 0.0f;
	...
	if(!m_Printables.IsValidIndex(outindex) || !m_Printables.IsValidIndex(nextindex))
	{
		prevchar = startchar;
		nextchar = startchar;
		subfrac = 0.0f;
...
void CHudNumeric::PaintRotatedCharacterSpeedomter(int x, int y, vgui::HFont &font, int prevchar, int nextchar,
												  float frac)
{
	frac = 3 * frac * frac - 2 * frac * frac * frac;
...
{
	if(m_nRotaryEffect == ROTARY_EFFECT_NONE || m_nRotaryEffect == ROTARY_EFFECT_SPEEDOMETER)
		return;
...
{
	if(m_nRotaryEffect != ROTARY_EFFECT_NONE && IsRotating() && m_flRotaryTime > 0.0f)
	{
...
void CHudNumeric::PaintValue(const char *value, int textlen, int wide, int tall, Color &clr)
{
	...
	if(m_nRotaryEffect != ROTARY_EFFECT_NONE && (bool)m_bDrawCharacterBackground)
	{
	...
	if(m_nRotaryEffect != ROTARY_EFFECT_NONE && (bool)m_bDrawCharacterForeground)
	{
