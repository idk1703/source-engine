...
{
public:
	void CreateMeteor(int nID, int iType, const Vector &vecPosition, const Vector &vecDirection, float flSpeed,
					  float flStartTime, float flDamageRadius, const Vector &vecTriggerMins,
...
public:
	...
	void SetSmokeEmission(bool bEmit)
	{
		m_bEmitSmoke = bEmit;
	}
	bool EmitSmoke(void)
	...
	void SetParticleScale(float flScale)
	{
		m_flParticleScale = flScale;
	}
...
public:
	...
	void DrawFragment(ParticleDraw *pDraw, const Vector &vecStart, const Vector &vecDelta,
					  const Vector4D &vecStartColor, const Vector4D &vecEndColor, float flStartV, float flEndV);
...
public:
	...
	C_EnvMeteor();
	~C_EnvMeteor();
	static C_EnvMeteor *Create(int nID, int iMeteorType, const Vector &vecOrigin, const Vector &vecDirection,
	...
	void SetTravelDirection(const Vector &vecDir)
	{
		m_vecTravelDir = vecDir;
	}
