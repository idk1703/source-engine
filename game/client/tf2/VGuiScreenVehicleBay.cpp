...
int g_ValidVehicles[] = {
	OBJ_WAGON,			OBJ_BATTERING_RAM,		OBJ_VEHICLE_TANK, OBJ_VEHICLE_TELEPORT_STATION,
	OBJ_WALKER_STRIDER, OBJ_WALKER_MINI_STRIDER
...
void CVehicleBayVGuiScreen::OnTick()
{
	...
	for(int i = 0; i < NUM_VEHICLES; i++)
	{
		...
		char buf[128];
		int iCost = CalculateObjectCost(g_ValidVehicles[i], pLocalPlayer->GetNumObjects(g_ValidVehicles[i]),
										pLocalPlayer->GetTeamNumber());
