...
public:
	...
	// It is looking down negative Z, and goes from [vCenter.x-xSize, vCenter.y-ySize] to [vCenter.x+xSize,
	// vCenter.y+ySize].
	...
	void CreatePickingRay(int mousex, int mousey, int screenwidth, int screenheight, const Vector &vecRenderOrigin,
						  const QAngle &vecRenderAngles, Vector &rayOrigin, Vector &rayDirection);

	Vector &TacticalOrigin()
	{
		return m_vecTacticalOrigin;
	}
	QAngle &TacticalAngles()
	{
		return m_vecTacticalAngles;
	}
