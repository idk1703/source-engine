...
public:
	CMinimapPlayerPanel(vgui::Panel *parent, const char *panelName) : BaseClass(parent, "CMinimapPlayerPanel") {}
...
bool CMinimapPlayerPanel::InitClassImages(KeyValues *pKeyValues, MinimapInitData_t *pInitData)
{
	...
	for(int i = 1; i < TFCLASS_CLASS_COUNT; ++i)
	{
		...
		if(!InitializeTeamImage(pKeyValues, GetTFClassInfo(i)->m_pClassName, this, pInitData->m_pEntity,
								&m_pClassAliveImages[i]))
			return false;
	}
