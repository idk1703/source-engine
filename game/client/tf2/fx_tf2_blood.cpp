...
//			scale -
//-----------------------------------------------------------------------------
void FX_TF2_BloodSpray(const Vector &origin, const Vector &normal, float scale, unsigned char r, unsigned char g,
					   unsigned char b, int flags)
{
	...
	if(flags & FX_BLOODSPRAY_DROPS && (flDistance < 1500))
	{
		...
		for(i = 0; i < 7; i++)
		{
			...
			FloatToColor32(tParticle->m_color, MIN(1.0, color[0] * colorRamp), MIN(1.0, color[1] * colorRamp),
						   MIN(1.0, color[2] * colorRamp), 1.0f);
		}
