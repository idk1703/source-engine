...
C_PlayerClass *C_PlayerClass::Create(C_BaseTFPlayer *pPlayer, int iClassType)
{
	...
	switch(iClassType)
	{
		...
		{
			return (new C_PlayerClassCommando(pPlayer));
		}
		case TFCLASS_DEFENDER:
		{
			return (new C_PlayerClassDefender(pPlayer));
		}
		case TFCLASS_ESCORT:
		{
			return (new C_PlayerClassEscort(pPlayer));
		}
		case TFCLASS_INFILTRATOR:
		{
			return (new C_PlayerClassInfiltrator(pPlayer));
		}
		case TFCLASS_MEDIC:
		{
			return (new C_PlayerClassMedic(pPlayer));
		}
		case TFCLASS_RECON:
		{
			return (new C_PlayerClassRecon(pPlayer));
		}
		case TFCLASS_SNIPER:
		{
			return (new C_PlayerClassSniper(pPlayer));
		}
		case TFCLASS_SUPPORT:
		{
			return (new C_PlayerClassSupport(pPlayer));
		}
		case TFCLASS_SAPPER:
		{
			return (new C_PlayerClassSapper(pPlayer));
		}
		default:
		{
			return NULL;
