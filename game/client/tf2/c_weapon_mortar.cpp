...
// Purpose:
//-----------------------------------------------------------------------------
CHudMortar::CHudMortar(const char *pElementName) : CHudElement(pElementName), vgui::Panel(NULL, pElementName)
{
	m_flPower = 0;
	m_flFiringPower = 0;
...
static ConVar g_CVMortarGroundLineUpdateInterval("mortar_groundlineupdateinterval", "0.1", 0,
												 "Number of seconds, mininum, between ground line position updates.");
...
RecvPropInt(RECVINFO(m_bCarried)), RecvPropInt(RECVINFO(m_bMortarReloading)),
	RecvPropVector(RECVINFO(m_vecMortarOrigin)), RecvPropVector(RECVINFO(m_vecMortarAngles)),
END_RECV_TABLE
()
...
void C_WeaponMortar::Redraw()
{
	...
	if(m_bMortarReloading)
	{
		...
		messagechars->DrawString(m_hFontText, (ScreenWidth() - width) / 2, YRES(350), 192, 192, 192, 255,
								 "Mortar is reloading...", IMessageChars::MESSAGESTRINGID_NONE);
		return;
	}
	...
	switch(m_iFiringState)
	{
		case MORTAR_IDLE:
			...
			break;
		case MORTAR_CHARGING_POWER:
			m_pPowerBar->m_flPower =
				MIN(m_pPowerBar->m_flPower + ((1.0 / MORTAR_CHARGE_POWER_RATE) * gpGlobals->curtimeDelta), 1.0f);
		...
		case MORTAR_CHARGING_ACCURACY:
			...
			m_pPowerBar->m_flPower =
				MAX(m_pPowerBar->m_flPower - (m_flAccuracySpeed * gpGlobals->curtimeDelta), -0.25f);
