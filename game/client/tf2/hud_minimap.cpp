...
void MinimapCreateTempTrace(const char *pMetaClassName, int sortOrder, const Vector &vecPosition)
{
	...
	PanelMetaClassMgr()->CreatePanelMetaClass(pMetaClassName, sortOrder, &initData, CMinimapPanel::MinimapRootPanel());
}
...
public:
	CMinimapRootPanel(Panel *pParent = NULL) : BaseClass(pParent, "CMinimapRootPanel")
	{
...
// Purpose:
//-----------------------------------------------------------------------------
CTextHelpPanel::CTextHelpPanel() : BaseClass(NULL, "HudMinimapTextHelpPanel")
{
...
//-----------------------------------------------------------------------------
CMinimapPanel::CMinimapPanel(const char *pElementName) : CHudElement(pElementName), BaseClass(NULL, "HudMinimap")
{
	Panel *pParent = g_pClientMode->GetViewport();
	SetParent(pParent);
...
//-----------------------------------------------------------------------------
bool CMinimapPanel::InternalWorldToMinimap(MinimapPosType_t posType, const Vector &pos, const Vector &origin,
										   float zoomscale, float &outx, float &outy)
{
	int wide, tall;
...
void CMinimapPanel::SetBackgroundMaterials(const char *pMaterialName)
{
	...
	if(pMaterialName[0])
	{
		for(i = 0; i < MAX_ACT_TEAMS; i++)
		{
			...
			Q_snprintf(teammaterial, sizeof(teammaterial), "%s_team%i", pMaterialName, i + 1);
...
void CMinimapPanel::SetBackgroundViewport(float minx, float miny, float maxx, float maxy, bool includedetails)
{
	...
	if(minx < 0.0f || maxx > 1.0f || miny < 0.0f || maxy > 1.0f)
	{
		float x0 = 0.0f;
		float y0 = 0.0f;
		float x1 = 1.0f;
...
void CMinimapPanel::Paint()
{
	...
	if(m_pBackground[teamIndex])
	{
		...
		InternalWorldToMinimap(MINIMAP_NOCLIP, m_vecCurrentOrigin, -m_vecMapCenter, 1, offsetx, offsety);
	...
	if(!shouldDrawDetails && CurrentActIsAWaitingAct())
	{
		...
		messagechars->DrawString(g_hFontTrebuchet24, XRES(16), ScreenHeight() - (height * 6), 255, 255, 245, 255, cmsg,
								 IMessageChars::MESSAGESTRINGID_NONE);
	}
...
void CMinimapPanel::ComputeMapOrigin(Vector &origin)
{
	...
	// Map center pointer is biased toward player origin as we become zoomed in to 1.0x to 2.5x and toward true world
	// center as we zoom all the way out
	...
	if(m_flAspectAdjustment > 1.0f)
	{
		...
		actualworldunitsperpixel = zooomedoutworldunitsperpixel +
								   m_flCenterOnPlayer * (zoomedworldunitsperpixel - zooomedoutworldunitsperpixel);
	...
	m_flWorldSpaceInsets[0] =
		MIN(m_vecMapCenter.x, worldmins.x + m_flCenterOnPlayer * (viewport_width_world_units)*0.5f);
	m_flWorldSpaceInsets[1] =
		MIN(m_vecMapCenter.y, worldmins.y + m_flCenterOnPlayer * (viewport_height_world_units)*0.5f);
	m_flWorldSpaceInsets[2] =
		MAX(m_vecMapCenter.x, worldmaxs.x - m_flCenterOnPlayer * (viewport_width_world_units)*0.5f);
	m_flWorldSpaceInsets[3] =
		MAX(m_vecMapCenter.y, worldmaxs.y - m_flCenterOnPlayer * (viewport_height_world_units)*0.5f);
...
void CMinimapPanel::InitOverlays(const char *materialrootname)
{
	...
	for(t = 0; t < MAX_ACT_TEAMS; t++)
	{
		...
		for(i = 0; i < MAX_ACTS; i++)
		{
			...
			Q_snprintf(filename, sizeof(filename), "%s_act%s_overlay_team%s", materialrootname, actnum, teamnum);
			...
			else
			{
				...
				Q_snprintf(filename, sizeof(filename), "%s_act%s_overlay", materialrootname, actnum);
			...
			Q_snprintf(filename, sizeof(filename), "%s_act%s_text_team%s", materialrootname, actnum, teamnum);
			...
			{
				// Try it without the team number
				Q_snprintf(filename, sizeof(filename), "%s_act%s_text", materialrootname, actnum);
...
void CMinimapPanel::ZoomIn(void)
{
	...
	else
	{
		...
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			m_nZoomLevel == 0 ? "MinimapZoomLevel0" : "MinimapZoomLevel1");
	}
}
...
void CMinimapPanel::SetMinimapZoom(bool bZoom)
{
	...
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			m_bMinimapZoomed ? "MinimapZoomToFullScreen" : "MinimapZoomToCorner");

		CLocalPlayerFilter filter;
		C_BaseEntity::EmitSound(filter, SOUND_FROM_LOCAL_PLAYER,
								m_bMinimapZoomed ? "Minimap.ZoomIn" : "Minimap.ZoomOut");
	}
}
