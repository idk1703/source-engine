...
RecvPropFloat(RECVINFO(m_flTurretYaw)), RecvPropFloat(RECVINFO(m_flTurretPitch))
END_RECV_TABLE
()
...
void C_VehicleTank::ClientThink()
{
	if(GetPassenger(VEHICLE_ROLE_DRIVER) == C_BasePlayer::GetLocalPlayer())
	{
		...
		trace_t trace;
		UTIL_TraceLine(MainViewOrigin(), MainViewOrigin() + MainViewForward() * 100000, MASK_OPAQUE, NULL,
