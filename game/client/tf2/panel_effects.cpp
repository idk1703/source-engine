...
//			output -
//-----------------------------------------------------------------------------
static void FindConnectingLines_Straight(int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2,
...
//			output -
//-----------------------------------------------------------------------------
static void FindConnectingLines_Axial(int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2,
									  CUtlVector<CConnectingLine> &output)
{
	// Reset output
	output.RemoveAll();
	...
	if(edge1 == TOPCENTER || edge1 == BOTTOMCENTER)
	{
		...
		if(dx < (sizefrac * size1[0]) && dx < (sizefrac * size2[0]))
		{
	...
	else
	{
		...
		if(dy < (sizefrac * size1[1]) && dy < (sizefrac * size2[1]))
		{
			// Gap is small, just use midpoint to align both
			line.m_ptStart[0] = pt1[0];
			line.m_ptStart[1] = mid[1];
...
// Input  : *owner -
//-----------------------------------------------------------------------------
CFlashBorderPanelEffect::CFlashBorderPanelEffect(ITFHintItem *owner) : CPanelEffect(owner)
{
	// Mark type field
	SetType(FLASHBORDER);
}
...
// Input  : *owner -
//-----------------------------------------------------------------------------
CArrowPanelEffect::CArrowPanelEffect(ITFHintItem *owner) : CPanelEffect(owner)
{
	SetType(ARROW);
...
void CArrowPanelEffect::doPaint(vgui::Panel *panel)
{
		...
		for(i = 0; i < lines.Size(); i++)
		{
			...
			for(int x = -1; x <= 1 + hstep; x++)
			{
				for(int y = -1; y <= 1 + vstep; y++)
				{
					...
					if(i == lines.Size() - 1)
					{
						DrawArrow(l->m_ptStart[0] + x, l->m_ptStart[1] + y, l->m_ptEnd[0] + x, l->m_ptEnd[1] + y, 0, 0,
								  0, m_a);
					}
					else
					{
						DrawLine(l->m_ptStart[0] + x, l->m_ptStart[1] + y, l->m_ptEnd[0] + x, l->m_ptEnd[1] + y, 0, 0,
								 0, m_a);
					}
				}
			}
	...
	for(i = 0; i < lines.Size(); i++)
	{
		...
		if(i == lines.Size() - 1)
		{
			...
			DrawArrow(l->m_ptStart[0] + hstep, l->m_ptStart[1] + vstep, l->m_ptEnd[0] + hstep, l->m_ptEnd[1] + vstep, r,
					  g, b, m_a);
		}
		else
		{
			...
			// Draw a second time, but thicker
			DrawLine(l->m_ptStart[0] + hstep, l->m_ptStart[1] + vstep, l->m_ptEnd[0] + hstep, l->m_ptEnd[1] + vstep, r,
					 g, b, m_a);
		}
	}
}
...
// Input  : *owner -
//-----------------------------------------------------------------------------
CAxialLinePanelEffect::CAxialLinePanelEffect(ITFHintItem *owner) : CArrowPanelEffect(owner) {}
		...
		for(i = 0; i < lines.Size(); i++)
		{
			...
			for(int x = -1; x <= 1; x++)
			{
				for(int y = -1; y <= 1; y++)
				{
					...
					DrawLine(l->m_ptStart[0] + x, l->m_ptStart[1] + y, l->m_ptEnd[0] + x, l->m_ptEnd[1] + y, 0, 0, 0,
							 m_a);
				}
			}
		}
