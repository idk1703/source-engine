...
RecvPropFloat(RECVINFO(m_flClientResources)), RecvPropInt(RECVINFO(m_nResourcesLeft)),
END_RECV_TABLE
()
...
void C_ResourceSpawner::SpawnEffect(bool bSpawningChunk)
{
	...
	fleckEmitter->m_ParticleCollision.Setup(offset, &normal, CHUNK_FLECK_ANGULAR_SPRAY, CHUNK_FLECK_MIN_SPEED,
											CHUNK_FLECK_MAX_SPEED, CHUNK_FLECK_GRAVITY, CHUNK_FLECK_DAMPEN);
	...
	for(i = 0; i < numFlecks; i++)
	{
		...
		if(bSpawningChunk)
		{
			...
			pParticle->m_vecVelocity =
				dir * (random->RandomFloat(CHUNK_FLECK_MIN_SPEED, CHUNK_FLECK_MAX_SPEED) * (9 - pParticle->m_uchSize));
		}
		else
		{
