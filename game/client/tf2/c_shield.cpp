...
#define SHIELD_DAMAGE_CHANGE_TOTAL_TIME \
	(SHIELD_DAMAGE_CHANGE_TRANSITION_START_TIME + SHIELD_DAMAGE_CHANGE_TRANSITION_TIME)
...
RecvPropInt(RECVINFO(m_nOwningPlayerIndex)), RecvPropFloat(RECVINFO(m_flPowerLevel)), RecvPropInt(RECVINFO(m_bIsEMPed)),
END_RECV_TABLE
()
...
{
	return m_bCollisionsActive &&
		   ((collisionGroup == TFCOLLISION_GROUP_WEAPON) || (collisionGroup == TFCOLLISION_GROUP_GRENADE));
}
...
bool C_Shield::TestCollision(const Ray_t &ray, unsigned int mask, trace_t &trace)
{
	...
	for(int i = 0; i < h - 1; ++i)
	{
		for(int j = 0; j < w - 1; ++j)
		{
			...
			t = IntersectRayWithTriangle(ray, GetPoint(j, i + 1), GetPoint(j + 1, i + 1), GetPoint(j, i), true);
			if((t >= 0.0f) && (t < mint))
			{
				mint = t;
				v1[0] = j;
			...
			t = IntersectRayWithTriangle(ray, GetPoint(j + 1, i), GetPoint(j, i), GetPoint(j + 1, i + 1), true);
			if((t >= 0.0f) && (t < mint))
			{
				mint = t;
				v1[0] = j + 1;
	...
	float u, v;
	bool ok = ComputeIntersectionBarycentricCoordinates(ray, v1vec, v2vec, v3vec, u, v);
	Assert(ok);
	if(!ok)
	{
		return false;
...
void C_Shield::ComputePoint(float s, float t, Vector &pt, Vector &normal, float &opacity)
{
	...
	{
		if((s == 0.0f) || (t == 0.0f) || (s == (Width() - 1.0f)) || (t == (Height() - 1.0f)))
		{
