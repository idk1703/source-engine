...
private:
	...
	void DrawBox(bool isbuildmenu, int x, int y, int wide, int tall, bool isactive, bool isvalid, float normalizedAlpha,
				 int number);
	int GetTeamIndex();
	void OnWeaponSelectionDrawn(CHudWeaponSelection *selection, C_WeaponObjectSelection *weapon,
...
private:
	...
	void DrawBox(bool isbuildmenu, int x, int y, int wide, int tall, bool current, bool isvalid, float normalizedAlpha,
				 int number);
...
void CHudWeaponSelection::TranslateColor(float percent, Color &clr)
{
	...
	else
	{
		...
		clr = Color(clr[0] * frac, clr[1] * frac, clr[2] * frac, clr[3] * frac);
	}
}
...
{
	int panelPosition = 0;
	for(int item = menu.items.FirstInorder();
		item != menu.items.InvalidIndex() && panelPosition < MAX_WEAPON_MENU_ITEMS; item = menu.items.NextInorder(item))
	{
...
{
	int panelPosition = 0;
	for(int item = menu.items.FirstInorder(); item != menu.items.InvalidIndex() && panelPosition < MAX_BUILD_MENU_ITEMS;
		item = menu.items.NextInorder(item))
	{
		CWeaponMenuItem *w = &menu.items[item];
		Assert(w);
...
void CHudWeaponSelection::ApplySettings(KeyValues *resourceData)
{
	...
	for(KeyValues *controlKeys = resourceData->GetFirstSubKey(); controlKeys != NULL;
		controlKeys = controlKeys->GetNextKey())
	{
...
//			*panelName -
//-----------------------------------------------------------------------------
CHudWeaponItemPanel::CHudWeaponItemPanel(vgui::Panel *parent, const char *panelName) : BaseClass(parent, panelName)
{
	m_bInUse = false;
...
void CHudWeaponItemPanel::Paint()
{
	...
	// Draw box
	DrawBox(buildMenu, 0, 0, wide, tall, isactive, isvalid, m_flGrowFraction * 255,
			m_bDrawNumbers ? menuItem.hotkey : -1);
	...
	if(info.drawAmmo)
	{
		...
		gHUD.DrawProgressBar(sizefracw * ammoBarX, sizefrach * iconYPos, sizefracw * ammoBarWide, sizefrach * iconTall,
							 info.ammoPerc, ammoColor, CHud::HUDPB_VERTICAL);
	}
												 ...
												 bool bCurrentlySelected, int wide, int tall, Color &clr)
{
	...
	int iCost =
		CalculateObjectCost(weapon->GetSubType(), pOwner->GetNumObjects(weapon->GetSubType()), pOwner->GetTeamNumber());
...
//-----------------------------------------------------------------------------
void CHudWeaponItemPanel::DrawBox(bool isbuildmenu, int x, int y, int wide, int tall, bool isactive, bool isvalid,
...
void CHudWeaponItemPanel::SetupIcons()
{
	...
	for(int team = 0; team < NUM_SLOT_TEAMS; team++)
	{
		...
		for(int type = 0; type < NUM_MENU_TYPES; type++)
		{
			...
			char sz[256];
			Q_snprintf(sz, sizeof(sz), "%s_weapon_%s_%s_%i", teamtype, menutypestr, slottype, m_nSlotNumber + 1);
