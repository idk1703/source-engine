...
RecvPropFloat(RECVINFO(m_flMortarYaw)), RecvPropFloat(RECVINFO(m_flMortarPitch)),
...
// Purpose:
//-----------------------------------------------------------------------------
void C_VehicleMortar::GetMortarData(float *flClientMortarYaw, bool *bAllowedToFire, float *flPower,
									float *flFiringPower, float *flFiringAccuracy, int *iFiringState)
{
...
// CMortarMinimapPanel
//-----------------------------------------------------------------------------
CMortarMinimapPanel::CMortarMinimapPanel(vgui::Panel *pParent, const char *pElementName) : CMinimapPanel(pElementName)
{
	SetParent(pParent);
...
void CMortarMinimapPanel::Paint()
{
	...
	if(pMortar && pMortarInterface)
	{
		...
		int iFiringState;
		bool bAllowedToFire;
		pMortarInterface->GetMortarData(&flClientMortarYaw, &bAllowedToFire, &flPower, &flFiringPower,
										&flFiringAccuracy, &iFiringState);
		...
		if(WorldToMinimap(MINIMAP_CLAMP, pMortar->GetAbsOrigin(), x, y))
		{
			...
			pSurface->DrawSetColor(255, 255, 255, 255);
			vgui::Vertex_t pts[4] = {vgui::Vertex_t(vStart2D - vStartPerp), vgui::Vertex_t(vStart2D + vStartPerp),
									 vgui::Vertex_t(vEnd2D + vEndPerp), vgui::Vertex_t(vEnd2D - vEndPerp)};
...
void CMortarMinimapPanel::OnMousePressed(vgui::MouseCode code)
{
	...
	int iFiringState;
	bool bAllowedToFire;
	pMortarInterface->GetMortarData(&flClientMortarYaw, &bAllowedToFire, &flPower, &flFiringPower, &flFiringAccuracy,
									&iFiringState);
...
void CVehicleMortarControlPanel::OnTickMainPanel()
{
	...
	Q_snprintf(buf, sizeof(buf), "Passengers %d/%d", nPassengerCount >= 1 ? nPassengerCount - 1 : 0,
			   nMaxPassengerCount - 1);
