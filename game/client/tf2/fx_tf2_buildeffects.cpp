...
//-----------------------------------------------------------------------------
void FX_BuildImpact(const Vector &origin, const QAngle &vecAngles, const Vector &vecNormal, float flScale,
					bool bGround = false, CBaseEntity *pIgnore = NULL)
{
	Vector offset;
	float spread = 0.1f;
...
public:
	...
	virtual float UpdateScale(const SimpleParticle *pParticle)
	{
		...
		if(tLifeTime < WARPEMMITER_MIDPOINT)
		{
			...
			return (float)pParticle->m_uchStartSize +
				   ((float)pParticle->m_uchEndSize - (float)pParticle->m_uchStartSize) * tLifeTime;
		}

		tLifeTime -= WARPEMMITER_MIDPOINT;
		tLifeTime = RemapVal(tLifeTime, 0, 1 - WARPEMMITER_MIDPOINT, 0, 1.0);
		return (float)pParticle->m_uchEndSize -
			   ((float)pParticle->m_uchEndSize - (float)pParticle->m_uchStartSize) * tLifeTime;
	}
	...
	virtual float UpdateAlpha(const SimpleParticle *pParticle)
	{
		...
		{
			tLifeTime = RemapVal(tLifeTime, 0, WARPEMMITER_MIDPOINT, 0, 1.0);
			flAlpha = (float)pParticle->m_uchStartAlpha +
					  ((float)pParticle->m_uchEndAlpha - (float)pParticle->m_uchStartAlpha) * tLifeTime;
		}
		else
		{
			tLifeTime -= WARPEMMITER_MIDPOINT;
			tLifeTime = RemapVal(tLifeTime, 0, 1 - WARPEMMITER_MIDPOINT, 0, 1.0);
			flAlpha = (float)pParticle->m_uchEndAlpha -
					  ((float)pParticle->m_uchEndAlpha - (float)pParticle->m_uchStartAlpha) * tLifeTime;
		}
...
void FX_BuildWarp(Vector &vecOrigin, QAngle &vecAngles, float flScale)
{
	...
	pParticle = (SimpleParticle *)pWarpEmitter->AddParticle(
		sizeof(SimpleParticle), pWarpEmitter->GetPMaterial("effects/human_build_warp"), vecOrigin);
	if(pParticle != NULL)
	{
		pParticle->m_flLifetime = 0.0f;
...
void FX_BuildWarpSuck(Vector &vecOrigin, QAngle &vecAngles, float flScale)
{
	...
	pEmitter->Setup((Vector &)vecOrigin, NULL, 0.0, 0, 64, 0, 0,
	...
	for(int i = 0; i < iNumParticles; i++)
	{
		...
		TrailParticle *pParticle =
			(TrailParticle *)pEmitter->AddParticle(sizeof(TrailParticle), hParticleMaterial, vPos);
		if(pParticle)
		{
