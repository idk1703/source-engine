...
//-----------------------------------------------------------------------------
void DrawQuadMaterial(IMaterial *pMaterial, int iX, int iY, int iWidth, int iHeight, unsigned char r = 255,
					  unsigned char g = 255, unsigned char b = 255, unsigned char a = 255, float flTextureLeft = 0.0,
					  float flTextureRight = 1.0, bool bRotated = false)
{
...
RecvPropInt(RECVINFO(m_iBuildState)), RecvPropInt(RECVINFO(m_iCurrentObject)),
	RecvPropInt(RECVINFO(m_iCurrentObjectState)), RecvPropEHandle(RECVINFO(m_hObjectBeingBuilt)),
	RecvPropTime(RECVINFO(m_flStartTime)), RecvPropTime(RECVINFO(m_flTotalTime)),
	RecvPropArray(RecvPropInt(RECVINFO(m_bObjectValidity[0])), m_bObjectValidity),
	RecvPropArray(RecvPropInt(RECVINFO(m_bObjectBuildability[0])), m_bObjectBuildability),
END_RECV_TABLE
()
...
//-----------------------------------------------------------------------------
static void DrawTextIcon(IMaterial *pMaterial, int parentWidth, int parentHeight, float r = 1.0f, float g = 1.0f,
						 float b = 1.0f)
{
	if(!pMaterial)
		return;
...
CHumanPDAPanel::CHumanPDAPanel(vgui::Panel *parent, const char *panelName)
	: BaseClass(parent, "CHumanPDAPanel",
...
void CHumanPDAPanel::OnTick()
{
	...
	if(m_nLastObjectID == objectType && m_nLastObjectCount == numOwned && m_nLastObjectCost == iCost)
		return;
