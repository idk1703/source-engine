...
RecvPropFloat(RECVINFO(m_fResources)), RecvPropFloat(RECVINFO(m_fPotentialResources)),
	RecvPropInt(RECVINFO(m_bHaveZone)),

	RecvPropArray2(RecvProxyArrayLength_TeamObjects,
				   RecvPropInt("object_array_element", 0, SIZEOF_IGNORE, 0, RecvProxy_ObjectList), MAX_OBJECTS_PER_TEAM,
				   0, "object_array"),
END_RECV_TABLE
()
...
void C_TFTeam::ReceiveMessage(int classID, bf_read &msg)
{
	...
	messagechars->DrawStringForTime(5.0, g_hFontTrebuchet24, (ScreenWidth() - width) / 2, iYPos, 192, 192, 192, 255,
									sResourceString, MESSAGESTRINGID_RESOURCESHARVESTED);
}