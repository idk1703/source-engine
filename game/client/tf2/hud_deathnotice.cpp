...
void CHudDeathNotice::MsgFunc_DeathMsg(bf_read &msg)
{
	...
	else
	{
		if(assist)
		{
			Q_snprintf(sDeathMsg, sizeof(sDeathMsg), "%s and %s killed %s", deathMsg.Killer.szName,
					   deathMsg.Assist.szName, deathMsg.Victim.szName);
		}
		else
		{
	...
	if(fullkilledwith && *fullkilledwith && (*fullkilledwith > 13) && strcmp(fullkilledwith, "d_world") &&
	   !deathMsg.iTeamKill)
	{
