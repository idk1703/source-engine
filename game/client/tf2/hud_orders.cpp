...
// Purpose:
//-----------------------------------------------------------------------------
CHudOrderList::CHudOrderList(const char *pElementName) : CHudElement(pElementName), vgui::Panel(NULL, "HudOrderList")
{
...
void CHudOrderList::LevelInit(void)
{
	...
	for(int i = 0; i < MAX_HUD_ORDERS; i++)
	{
		m_pOrderPanels[i] = new CHudOrder(ORDERS_ELEMENT_LEFT - ORDERS_LEFT, ORDERS_ELEMENT_HEIGHT * i,
										  ORDERS_ELEMENT_WIDTH, ORDERS_ELEMENT_HEIGHT);
		m_pOrderPanels[i]->Init();

		m_pOrderLabels[i] = new CHudLabel(NULL, "orderlabels", " ");
		m_pOrderLabels[i]->SetBounds(0, ORDERS_ELEMENT_HEIGHT * i, ORDERS_ELEMENT_LEFT - ORDERS_LEFT,
									 ORDERS_ELEMENT_HEIGHT);
