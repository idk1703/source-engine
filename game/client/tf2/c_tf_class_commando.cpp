...
BEGIN_RECV_TABLE_NOBASE(C_PlayerClassCommando, DT_PlayerClassCommandoData)
	RecvPropInt(RECVINFO(m_ClassData.m_bCanBullRush)), RecvPropInt(RECVINFO(m_ClassData.m_bBullRush)),
...
// Purpose:
//-----------------------------------------------------------------------------
C_PlayerClassCommando::C_PlayerClassCommando(C_BaseTFPlayer *pPlayer) : C_PlayerClass(pPlayer)
{
...
void C_PlayerClassCommando::InterpolateBullRushViewAngles(void)
{
	...
	float flFraction =
		1.0f - ((m_ClassData.m_flBullRushTime - COMMANDO_BULLRUSH_VIEWDELTA_TEST) / COMMANDO_BULLRUSH_VIEWDELTA_TIME);
