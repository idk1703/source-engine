...
{
public:
	static CPersonalShieldEffect *Create(C_BaseTFPlayer *pEnt, const Vector &vOffsetFromEnt,
										 const Vector &vIncomingDirection, int iDamage);
...
CPersonalShieldEffect *CPersonalShieldEffect::Create(C_BaseTFPlayer *pEnt, const Vector &vOffsetFromEnt,
													 const Vector &vIncomingDirection, int iDamage)
{
...
BEGIN_RECV_TABLE_NOBASE(CTFPlayerLocalData, DT_TFLocal)
	RecvPropInt(RECVINFO(m_nInTacticalView)), RecvPropInt(RECVINFO(m_bKnockedDown)),
		RecvPropVector(RECVINFO(m_vecKnockDownDir)), RecvPropInt(RECVINFO(m_bThermalVision)),
		RecvPropInt(RECVINFO(m_iIDEntIndex)), RecvPropArray(RecvPropInt(RECVINFO(m_iResourceAmmo[0])), m_iResourceAmmo),
		...
		RecvPropInt(RECVINFO(m_bAttachingSapper)), RecvPropFloat(RECVINFO(m_flSapperAttachmentFrac)),
...
RecvPropDataTable(RECVINFO_DT(m_TFLocal), 0, &REFERENCE_RECV_TABLE(DT_TFLocal), DataTableRecvProxy_StaticDataTable),
	...
	RecvPropDataTable(RECVINFO_DT(m_PlayerClasses), 0, &REFERENCE_RECV_TABLE(DT_AllPlayerClasses),
					  DataTableRecvProxy_StaticDataTable),

	RecvPropEHandle(RECVINFO(m_hSelectedMCV)), RecvPropInt(RECVINFO(m_iCurrentZoneState)),
	RecvPropInt(RECVINFO(m_iMaxHealth)), RecvPropInt(RECVINFO(m_TFPlayerFlags)), RecvPropInt(RECVINFO(m_bUnderAttack)),
	RecvPropInt(RECVINFO(m_bIsBlocking)),

	// Sniper - will get moved to a class data table
	RecvPropVector(RECVINFO(m_vecDeployedAngles)), RecvPropInt(RECVINFO(m_bDeployed)),
	RecvPropInt(RECVINFO(m_bDeploying)), RecvPropInt(RECVINFO(m_bUnDeploying)),

	// Infiltrator - will get moved to a class data table
	RecvPropFloat(RECVINFO(m_flCamouflageAmount)), RecvPropEHandle(RECVINFO(m_hSpawnPoint)),
END_RECV_TABLE
()
...
#include "c_tf_class_pyro.h"
	...
	DEFINE_FIELD(m_vecPosDelta, FIELD_VECTOR), DEFINE_ARRAY(m_aMomentum, FIELD_FLOAT, C_BaseTFPlayer::MOMENTUM_MAXSIZE),
...
ConVar cl_TargetInfoFadeDist("cl_TargetInfoFadeDist", "800", 0,
							 "Distance at which TF player targetting info fades out.");
...
void C_BaseTFPlayer::ReceiveMessage(int classID, bf_read &msg)
{
	...
	switch(messageType)
	{
		...
		case PLAYER_MSG_PERSONAL_SHIELD:
		{
			...
			// Show the effect.
			CPersonalShieldEffect *pEffect =
				CPersonalShieldEffect::Create(this, vOffsetFromEnt, vIncomingDirection, iDamage);
			if(pEffect)
	...
	if(m_flNextUseCheck < gpGlobals->curtime)
	{
		...
		if(IsAlive() && !GetVehicle() && (gpGlobals->curtime > m_flNextAttack))
		{
			...
			if(pUseEntity)
			{
				...
				if(pVehicle && InSameTeam(pVehicle) && !pVehicle->IsPlacing() && !pVehicle->IsBuilding() &&
				   pVehicle->GetMaxPassengerCount() >= 2)
				{
...
void C_BaseTFPlayer::CheckKnockdownState(void)
{
	...
	if(m_flStartKnockdown == 0.0f && m_flEndKnockdown == 0.0f && !IsKnockedDown())
	{
...
int C_BaseTFPlayer::DrawOverriddenViewmodel(C_BaseViewModel *pViewmodel, int flags)
{
	...
	if(IsDamageBoosted() || HasPowerup(POWERUP_EMP))
	{
		...
		if(!inv_demo.GetInt())
		{
			if(IsDamageBoosted() && pViewmodel->ViewModelIndex() != 0)
			{
				return ret;
			}
			...
			for(int i = 0; i < 3; i++)
			{
				...
				m_BoostModelAngles[i] += RandomFloat(damageboost_modelphasespeed_min.GetFloat(),
													 damageboost_modelphasespeed_max.GetFloat()) *
										 gpGlobals->frametime;
...
void C_BaseTFPlayer::CheckMovementCamoSuppression(void)
{
	...
	{
		float frac =
			(vel - CAMO_DAMPENINGVELOCITY_CUTOFF) / (CAMO_DAMPENINGVELOCITY_MAX - CAMO_DAMPENINGVELOCITY_CUTOFF);
		frac = MIN(1.0f, frac);
...
IMaterial *C_BaseTFPlayer::GetCamoMaterial(void)
{
	if(!m_pCamoEffectMaterial)
	{
		m_pCamoEffectMaterial =
			materials->FindMaterial("player/infiltratorcamo/infiltratorcamo", TEXTURE_GROUP_CLIENT_EFFECTS);
		if(m_pCamoEffectMaterial)
		{
		...
		if(local && GetTeamNumber() != local->GetTeamNumber())
		{
			...
			if(m_iPlayerClass == TFCLASS_SNIPER)
			{
				...
				if(dt > SNIPER_STATIONARY_FADESTART)
				{
					...
					{
						float frac = (dt - SNIPER_STATIONARY_FADESTART) /
									 (SNIPER_STATIONARY_FADEFINISH - SNIPER_STATIONARY_FADESTART);
						alpha *= (1.0f - frac);
					}
				}
			}
...
void C_BaseTFPlayer::PerformClientSideObstacleAvoidance(float flFrameTime, CUserCmd *pCmd)
{
	...
	for(i = 0; i < c; i++)
	{
		...
		if(!IntersectInfiniteRayWithSphere(GetAbsOrigin(), currentdir, obj->GetAbsOrigin(), objectradius, &flHit1,
										   &flHit2))
			continue;
