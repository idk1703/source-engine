...
public:
	...
	void SetDoPulses(bool dopulses)
	{
		m_bSendPulses = dopulses;
	}
	void ForcePulse(void);

	void SetRotaryEffect(int rotary)
	{
		m_nRotaryEffect = rotary;
	}
	void SetRotaryTimeMax(float maxTime)
	{
		m_flRotaryTimeMax = maxTime;
	}
	void SetRotaryCharsPerSecond(float cps)
	{
		m_flDesiredCharactersPerSecond = cps;
	}
...
protected:
	...
	void GetRotatedChar(float frac, char startchar, char endchar, char &prevchar, char &nextchar, float &subfrac);
