...
// Purpose:
//-----------------------------------------------------------------------------
CCommanderStatusPanel::CCommanderStatusPanel(void) : BaseClass(NULL, "CCommanderStatusPanel")
{
...
void CCommanderStatusPanel::Paint()
{
	...
	if(m_Type == TYPE_INFO && m_nTitlePos != -1)
	{
		...
		// Start after title
		g_pMatSystemSurface->DrawColoredTextRect(m_hFontText, x, y, wide, tall, 255, 255, 255, alpha, "%s",
												 &m_szText[m_nTitlePos + 1]);
		...
		if(m_bShowTechnology && m_pTechnology)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						if(fResourceLevel)
							Q_snprintf(szCostString, sizeof(szCostString), "A: %i (%i)", (int)fResourceLevel,
									   (int)fResourceCost);
						else
							Q_snprintf(szCostString, sizeof(szCostString), "A: %i", (int)fResourceCost);
						g_pMatSystemSurface->DrawColoredText(m_hFontText, x, y, sResourceColor.r, sResourceColor.g,
															 sResourceColor.b, alpha, "%s", szCostString);
	...
	{
		g_pMatSystemSurface->DrawColoredTextRect(m_hFont, x, y + tall - vgui::surface()->GetFontTall(m_hFont) - 4, wide,
												 vgui::surface()->GetFontTall(m_hFont) + 2, 220, 220, 255, alpha, "%s",
												 m_szText);
	}
}
