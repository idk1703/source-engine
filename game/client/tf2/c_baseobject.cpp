...
RecvPropInt(RECVINFO(m_iHealth)), RecvPropInt(RECVINFO(m_iMaxHealth)), RecvPropInt(RECVINFO(m_bHasSapper)),
	RecvPropInt(RECVINFO(m_iObjectType)), RecvPropInt(RECVINFO(m_bBuilding)), RecvPropInt(RECVINFO(m_bPlacing)),
	RecvPropFloat(RECVINFO(m_flPercentageConstructed)), RecvPropInt(RECVINFO(m_fObjectFlags)),
	RecvPropInt(RECVINFO(m_bDeteriorating)), RecvPropEHandle(RECVINFO(m_hBuiltOnEntity)),
	RecvPropInt(RECVINFO(m_takedamage)), RecvPropInt(RECVINFO(m_bDisabled)), RecvPropEHandle(RECVINFO(m_hBuilder)),
END_RECV_TABLE
()
...
void C_BaseObject::RecalculateIDString(void)
{
	...
	if(m_iHealth < m_iMaxHealth)
	{
		...
		if(IsDeteriorating())
		{
			Q_snprintf(szHealth, sizeof(szHealth), "\nBUILDER LOST, DETERIORATING... %.0f percent",
	...
	if(m_bHasSapper)
	{
		Q_strncat(m_szIDString, "\nUse it to remove the attached enemy object", sizeof(m_szIDString),
				  COPY_ALL_CHARACTERS);
	}
	...
	if(IsDeteriorating() && pLocalPlayer && ClassCanBuild(pLocalPlayer->PlayerClass(), GetType()))
	{
		...
		int iCost =
			CalculateObjectCost(GetType(), pLocalPlayer->GetNumObjects(GetType()), pLocalPlayer->GetTeamNumber());
...
void C_BaseObject::DrawRunningEffects(void)
{
	...
	if(GetAttachment(iSmokeAttachment, vecSmoke, angSmoke))
	{
		...
		ObjectSmokeParticle *pParticle =
			(ObjectSmokeParticle *)pSmokeEmitter->AddParticle(sizeof(ObjectSmokeParticle), g_Mat_DustPuff[1], vecSmoke);
		if(pParticle)
		{
...
void C_BaseObject::DrawDamageEffects(void)
{
	...
	SimpleParticle *pParticle =
		(SimpleParticle *)pSmokeEmitter->AddParticle(sizeof(SimpleParticle), g_Mat_DustPuff[1], vecSmoke);
	if(pParticle)
	{
		pParticle->m_flLifetime = 0.0f;
		pParticle->m_flDieTime = random->RandomFloat(2.0f, 3.0f);
	...
	if(flDamaged > 0.25)
	{
		...
		ObjectFireParticle *pParticle =
			(ObjectFireParticle *)pFireEmitter->AddParticle(sizeof(ObjectFireParticle), hSphereMaterial, vecFire);
		if(pParticle)
		{
