...
ConVar cl_phys_timescale("cl_phys_timescale", "1.0", FCVAR_CHEAT,
...
public:
	...
	void Friction(IPhysicsObject *pObject, float energy, int surfaceProps, int surfacePropsHit,
	...
	int ShouldSolvePenetration(IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0, void *pGameData1,
							   float dt);
...
private:
	...
	void AddTouchEvent(C_BaseEntity *pEntity0, C_BaseEntity *pEntity1, int touchType, const Vector &point,
					   const Vector &normal);
...
void PhysicsLevelInit(void)
{
	...
	g_PhysWorldObject =
		PhysCreateWorld_Shared(GetClientWorldEntity(), modelinfo->GetVCollide(1), g_PhysDefaultObjectParams);
...
public:
	...
	void AddImpactSound(void *pGameData, IPhysicsObject *pObject, int surfaceProps, int surfacePropsHit, float volume,
						float speed);
...
void CPhysicsSystem::AddImpactSound(void *pGameData, IPhysicsObject *pObject, int surfaceProps, int surfacePropsHit,
									float volume, float speed)
{
	physicssound::AddImpactSound(m_impactSounds, pGameData, SOUND_FROM_WORLD, CHAN_STATIC, pObject, surfaceProps,
...
void CCollisionEvent::ObjectSound(int index, vcollisionevent_t *pEvent)
{
	...
	if(pGameData)
	{
		...
		if(surfaceProps >= 0)
		{
			g_PhysicsSystem.AddImpactSound(pGameData, pObject, surfaceProps, pEvent->surfaceProps[!index], volume,
										   speed);
		}
	}
}
...
//			touchType -
//-----------------------------------------------------------------------------
void CCollisionEvent::AddTouchEvent(C_BaseEntity *pEntity0, C_BaseEntity *pEntity1, int touchType, const Vector &point,
									const Vector &normal)
{
	if(!pEntity0 || !pEntity1)
		return;
...
//			*pEntity1 -
//-----------------------------------------------------------------------------
void CCollisionEvent::DispatchStartTouch(C_BaseEntity *pEntity0, C_BaseEntity *pEntity1, const Vector &point,
										 const Vector &normal)
{
	trace_t trace;
...
void PhysicsSplash(IPhysicsFluidController *pFluid, IPhysicsObject *pObject, CBaseEntity *pEntity)
{
	...
	tanPts[0] = physcollision->CollideGetExtent(pObject->GetCollide(), pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
												-tangent);
	tanPts[1] = physcollision->CollideGetExtent(pObject->GetCollide(), pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
												tangent);
	binPts[0] = physcollision->CollideGetExtent(pObject->GetCollide(), pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
												-binormal);
	binPts[1] = physcollision->CollideGetExtent(pObject->GetCollide(), pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
												binormal);
	...
	*/
	...
	for(int i = 0; i < splashes; i++)
	{
		...
		{
			FX_WaterRipple(point, random->RandomFloat(0.25f, 0.5f), &color, luminosity,
					   ...
					   HSOUNDSCRIPTHANDLE &handle, float flVolume)
{
	...
	if(flVolume > (1.0f / 128.0f))
	{
		...
		if(!pFriction->pObject)
		{
			...
			pFriction->patch = CSoundEnvelopeController::GetController().SoundCreate(filter, entindex, CHAN_BODY,
																					 pSoundName, params.soundlevel);
