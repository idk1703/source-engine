...
protected:
	...
	//	SendPropArray3( SENDINFO_ARRAY3(m_iControlPointParents), SendPropInt( SENDINFO_ARRAY(m_iControlPointParents), 3,
	//SPROP_UNSIGNED ) ),
...
BEGIN_RECV_TABLE_NOBASE(C_ParticleSystem, DT_ParticleSystem)
	RecvPropVector(RECVINFO_NAME(m_vecNetworkOrigin, m_vecOrigin)), RecvPropEHandle(RECVINFO(m_hOwnerEntity)),
		RecvPropInt(RECVINFO_NAME(m_hNetworkMoveParent, moveparent), 0, RecvProxy_IntToMoveParent),
		RecvPropInt(RECVINFO(m_iParentAttachment)), RecvPropQAngles(RECVINFO_NAME(m_angNetworkAngles, m_angRotation)),

		RecvPropInt(RECVINFO(m_iEffectIndex)), RecvPropBool(RECVINFO(m_bActive)),
	...
	if(m_bActive)
	{
		...
		if(pszName && pszName[0])
		{
			...
			if(pEffect)
			{
				...
				// NOTE: What we really want here is to compare our lifetime and that of our children and see if this
				// delta is
				//		 already past the end of it, denoting that we're finished.  In that case, just destroy us and be
				//done. -- jdw
...
void ParticleEffectCallback(const CEffectData &data)
{
	if(SuppressingParticleEffects())
		return; // this needs to be before using data.m_nHitBox, since that may be a serialized value that's past the
				// end of the current particle system string table
	...
	if(data.m_fFlags & PARTICLE_DISPATCH_FROM_ENTITY)
	{
		if(data.m_hEntity.Get())
		{
			...
			if(pEnt && !pEnt->IsDormant())
			{
				...
				if(iAttachType == PATTACH_ABSORIGIN_FOLLOW || iAttachType == PATTACH_POINT_FOLLOW ||
