...
static ConVar cam_ideallag("cam_ideallag", "4.0", FCVAR_ARCHIVE | FCVAR_CHEAT,
						   "Amount of lag used when matching offset to ideal angles in thirdperson view");
static ConVar cam_idealdelta("cam_idealdelta", "4.0", FCVAR_ARCHIVE | FCVAR_CHEAT,
...
static ConVar cam_collision("cam_collision", "1", FCVAR_ARCHIVE | FCVAR_CHEAT,
							"When in thirdperson and cam_collision is set to 1, an attempt is made to keep the camera "
							"from passing though walls.");
static ConVar cam_showangles("cam_showangles", "0", FCVAR_CHEAT,
...
void CAM_ToThirdPerson(void)
{
	...
	{
		g_ThirdPersonManager.SetDesiredCameraOffset(
...
void CInput::CAM_Think(void)
{
	...
	if(cam_showangles.GetInt())
	{
		...
		engine->Con_NPrintf(6, "Pitch: %6.1f   Yaw: %6.1f   Dist: %6.1f %19s", cam_idealpitch.GetFloat(),
							cam_idealyaw.GetFloat(), cam_idealdist.GetFloat(), "ideal angles");
		engine->Con_NPrintf(8, "Pitch: %6.1f   Yaw: %6.1f   Dist: %6.1f %16s",
							g_ThirdPersonManager.GetCameraOffsetAngles()[PITCH],
							g_ThirdPersonManager.GetCameraOffsetAngles()[YAW],
							g_ThirdPersonManager.GetCameraOffsetAngles()[DIST], "camera offset");
	}
...
static ConCommand thirdperson_mayamode("thirdperson_mayamode", ::CAM_ToThirdPerson_MayaMode,
