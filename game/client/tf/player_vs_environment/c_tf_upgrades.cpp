...
ConVar tf_mvm_tabs_discovered("tf_mvm_tabs_discovered", "0", FCVAR_ARCHIVE,
							  "Remember how many times players have clicked tabs.");
...
void CHudUpgradePanel::PerformLayout(void)
{
	...
	FOR_EACH_VEC(m_pItemPanels, i)
	{
		...
		vgui::Panel *pInactiveTabPanel =
			m_pSelectWeaponPanel->FindChildByName(VarArgs("InactiveTabPanel%i", iVisiblePanel + 2));
	...
	for(int nSlotIndex = 0; nSlotIndex < ARRAYSIZE(m_ItemSlotBuyPanels); ++nSlotIndex)
	{
		...
		if(pItemSlotBuyPanel->upgradeBuyPanels.Count() > 0)
		{
			...
			FOR_EACH_VEC(pItemSlotBuyPanel->upgradeBuyPanels, i)
			{
				...
				int iXPos = m_iUpgradeBuyPanelXPos + (pUpgradeBuyPanel->GetWide() + m_iUpgradeBuyPanelDelta) *
														 pUpgradeBuyPanel->m_nGridPositionX;
				int iYPos = m_iUpgradeBuyPanelYPos + (pUpgradeBuyPanel->GetTall() + m_iUpgradeBuyPanelDelta) *
														 pUpgradeBuyPanel->m_nGridPositionY;
...
void CHudUpgradePanel::CreateItemModelPanel(int iLoadoutSlot)
{
	...
	if(iLoadoutSlot == LOADOUT_POSITION_ACTION &&
	   (!bVisible ||
		!dynamic_cast<CTFPowerupBottle *>(m_hPlayer->GetEquippedWearableForLoadoutSlot(LOADOUT_POSITION_ACTION))))
	{
...
void CHudUpgradePanel::OnItemPanelMousePressed(vgui::Panel *panel)
{
	...
	{
		int iLoadoutSlot =
			pItemPanel->GetItem()->GetStaticData()->GetLoadoutSlot(m_hPlayer->GetPlayerClass()->GetClassIndex());
...
void CHudUpgradePanel::UpgradeItemInSlot(int iSlot)
{
	...
	if(m_pMouseOverUpgradePanel)
	{
		for(int nSlotIndex = 0; nSlotIndex < ARRAYSIZE(m_ItemSlotBuyPanels); ++nSlotIndex)
		{
			...
			{
				m_pActiveUpgradeBuyPanel =
					pItemSlotBuyPanel->upgradeBuyPanels.Count() > 0 ? pItemSlotBuyPanel->upgradeBuyPanels[0] : NULL;
	...
	else
	{
		CEconItemView *pCurItemData =
			CTFPlayerSharedUtils::GetEconItemViewByLoadoutSlot(m_hPlayer, m_iWeaponSlotBeingUpgraded);
		if(pCurItemData)
		{
	...
	for(int nSlotIndex = 0; nSlotIndex < ARRAYSIZE(m_ItemSlotBuyPanels); ++nSlotIndex)
	{
		...
		if(pItemSlotBuyPanel->nSlot != -2)
		{
			...
			FOR_EACH_VEC(g_MannVsMachineUpgrades.m_Upgrades, i)
			{
				...
				if((pItemSlotBuyPanel->nSlot == -1 &&
					(nUIGroup == UIGROUP_UPGRADE_ATTACHED_TO_ITEM || nUIGroup == UIGROUP_POWERUPBOTTLE)) ||
				...
				CEconItemAttributeDefinition *pAttribDef =
					ItemSystem()->GetStaticDataForAttributeByName(pUpgrade->szAttrib);
				if(!pAttribDef)
				{
				...
				if(!TFGameRules()->CanUpgradeWithAttrib(m_hPlayer, pItemSlotBuyPanel->nSlot,
														pAttribDef->GetDefinitionIndex(), pUpgrade))
				{
					continue;
				}

				int nCost = pItemSlotBuyPanel->nSlot >= 0
								? TFGameRules()->GetCostForUpgrade(
									  &(g_MannVsMachineUpgrades.m_Upgrades[i]), pItemSlotBuyPanel->nSlot,
									  m_hPlayer->GetPlayerClass()->GetClassIndex(), m_hPlayer)
				...
				CEconItemView *pCurItemData =
					CTFPlayerSharedUtils::GetEconItemViewByLoadoutSlot(m_hPlayer, pItemSlotBuyPanel->nSlot);
				if(pCurItemData)
				{
				...
				if(!m_bInspectMode)
				{
					pUpgradeBuyPanel->m_pIncrementButton->SetCommand(
						VarArgs("mvm_upgrade%03d_%i", pUpgradeBuyPanel->m_nUpgradeIndex, pUpgradeBuyPanel->m_nPrice));
					pUpgradeBuyPanel->m_pDecrementButton->SetCommand(
						VarArgs("mvm_downgrade%03d_%i", pUpgradeBuyPanel->m_nUpgradeIndex, pUpgradeBuyPanel->m_nPrice));
				}
...
void CHudUpgradePanel::UpdateJoystickControls(void)
{
	...
	bool bUp = ::input->Joystick_GetForward() < 0.0f || ::input->Joystick_GetPitch() < 0.0f ||
			   vgui::input()->IsKeyDown(KEY_XBUTTON_UP) || vgui::input()->IsKeyDown(KEY_UP);
	bool bDown = ::input->Joystick_GetForward() > 0.0f || ::input->Joystick_GetPitch() > 0.0f ||
				 vgui::input()->IsKeyDown(KEY_XBUTTON_DOWN) || vgui::input()->IsKeyDown(KEY_DOWN);
	bool bNavUpDownPressed = bUp || bDown;

	bool bLeft = ::input->Joystick_GetSide() < 0.0f || ::input->Joystick_GetYaw() < 0.0f ||
				 vgui::input()->IsKeyDown(KEY_XBUTTON_LEFT) || vgui::input()->IsKeyDown(KEY_LEFT);
	bool bRight = ::input->Joystick_GetSide() > 0.0f || ::input->Joystick_GetYaw() > 0.0f ||
				  vgui::input()->IsKeyDown(KEY_XBUTTON_RIGHT) || vgui::input()->IsKeyDown(KEY_RIGHT);
	bool bNavLeftRightPressed = bLeft || bRight;

	bool bAccept = vgui::input()->IsKeyDown(KEY_XBUTTON_A) || vgui::input()->IsKeyDown(KEY_ENTER) ||
				   vgui::input()->IsKeyDown(STEAMCONTROLLER_A);
	bool bBack = vgui::input()->IsKeyDown(KEY_XBUTTON_X) || vgui::input()->IsKeyDown(KEY_BACKSPACE) ||
				 vgui::input()->IsKeyDown(STEAMCONTROLLER_X);
	bool bDone = vgui::input()->IsKeyDown(KEY_XBUTTON_B) || vgui::input()->IsKeyDown(KEY_ESCAPE) ||
				 vgui::input()->IsKeyDown(STEAMCONTROLLER_B);
			...
			if(m_iWeaponSlotBeingUpgraded == pItemSlotBuyPanel->nSlot)
			{
				FOR_EACH_VEC(pItemSlotBuyPanel->upgradeBuyPanels, i)
				{
					...
					if(pUpgradeBuyPanel && pUpgradeBuyPanel->IsVisible() &&
					   pUpgradeBuyPanel->m_nGridPositionX == nGridPosX &&
					   pUpgradeBuyPanel->m_nGridPositionY == nGridPosY)
					{
	...
	else if(bNavButtonPressed)
	{
		...
		if(bAccept)
		{
			if(m_pActiveUpgradeBuyPanel && m_pActiveUpgradeBuyPanel->m_pIncrementButton->IsEnabled() &&
			   m_pActiveUpgradeBuyPanel->m_pIncrementButton->IsVisible())
			{
				OnCommand(VarArgs("mvm_upgrade%03d_%i", m_pActiveUpgradeBuyPanel->m_nUpgradeIndex,
								  m_pActiveUpgradeBuyPanel->m_nPrice));
			}
			else if(m_iWeaponSlotBeingUpgraded == LOADOUT_POSITION_ACTION &&
					GetLocalPlayerBottleFromInventory() != NULL)
			{
		...
		{
			if(m_pActiveUpgradeBuyPanel && m_pActiveUpgradeBuyPanel->m_pDecrementButton->IsEnabled() &&
			   m_pActiveUpgradeBuyPanel->m_pDecrementButton->IsVisible())
			{
				OnCommand(VarArgs("mvm_downgrade%03d_%i", m_pActiveUpgradeBuyPanel->m_nUpgradeIndex,
								  m_pActiveUpgradeBuyPanel->m_nPrice));
			}
		}
		else if(bDone)
...
void CHudUpgradePanel::UpdateHighlights(void)
{
	...
	else
	{
		FOR_EACH_VEC(m_pItemPanels, i)
		{
			...
			if(pItemPanel)
			{
				int iLoadoutSlot = pItemPanel->GetItem()->GetStaticData()->GetLoadoutSlot(
					m_hPlayer->GetPlayerClass()->GetClassIndex());
				if(m_iWeaponSlotBeingUpgraded == iLoadoutSlot)
				{
					int x, y;
	...
	if(!m_bUsingController)
	{
		if(pMouseOverPanel)
		{
			...
			{
				// Set the text displaying which item the upgrades are for ("Shovel", etc.)
				m_pSelectWeaponPanel->SetDialogVariable(
					"upgrade_description",
					g_pVGuiLocalize->Find(g_aPlayerClassNames[m_hPlayer->GetPlayerClass()->GetClassIndex()]));
			}
			...
			if(!(bIsUpgradeButton || bIsPlayerUpgradeButton))
			{
				...
				if(!m_bHighlightedTab)
				{
					...
					if(bBlink)
					{
						...
						vgui::Panel *pInactiveTabPanel =
							m_pSelectWeaponPanel->FindChildByName(VarArgs("InactiveTabPanel%i", nPanel + 1));
		...
		for(int nSlotIndex = 0; nSlotIndex < ARRAYSIZE(m_ItemSlotBuyPanels); ++nSlotIndex)
		{
			...
			FOR_EACH_VEC(pItemSlotBuyPanel->upgradeBuyPanels, i)
			{
				...
				if(pMouseOverPanel &&
				   (pMouseOverPanel == pUpgradeBuyPanel || pMouseOverPanel->GetParent() == pUpgradeBuyPanel))
				{
					m_pActiveUpgradeBuyPanel = pUpgradeBuyPanel;
...
void CHudUpgradePanel::UpdateButtonStates(int nCurrentMoney, int nUpgrade /*= 0*/, int nNumPurchased /*= 0*/)
{
	for(int nSlotIndex = 0; nSlotIndex < ARRAYSIZE(m_ItemSlotBuyPanels); ++nSlotIndex)
	{
		...
		if(m_iWeaponSlotBeingUpgraded == LOADOUT_POSITION_ACTION ||
		   (TFGameRules() && TFGameRules()->GetUpgradeTier(nUpgrade)))
		{
			if(nNumPurchased > 0)
			{
			...
			if(pItemSlotBuyPanel->upgradeBuyPanels.Count() <= 0 && !m_bInspectMode &&
			   m_iWeaponSlotBeingUpgraded == LOADOUT_POSITION_ACTION)
			{
				...
				const wchar_t *pLocalized = g_pVGuiLocalize->Find(bHasBottle ? "#TF_PVE_Unequipped_Powerup_Bottle"
																			 : "#TF_PVE_No_Powerup_Bottle");
				if(pLocalized)
				{
		...
		FOR_EACH_VEC(pItemSlotBuyPanel->upgradeBuyPanels, i)
		{
			...
			if(pUpgradeBuyPanel->m_nUpgradeIndex == nUpgrade && nNumPurchased != 0)
			{
				...
				if(m_iWeaponSlotBeingUpgraded == LOADOUT_POSITION_ACTION && !bSellAllowed)
				{
					...
					else if(nNumPurchased < 0 && pUpgradeBuyPanel->m_nPurchases == 0 &&
							pUpgradeBuyPanel->m_nCurrentStep == 0)
					{
			...
			bool bMax = false;
			if(pUpgradeBuyPanel->m_nPurchases + pUpgradeBuyPanel->m_nCurrentStep ==
			   pUpgradeBuyPanel->m_SkillTreeImages.Count())
			{
				nUpgradesAtMax++;
				bMax = true;
			}
			...
			if(bMax && m_iWeaponSlotBeingUpgraded == -1)
			{
				...
				{
					CEconItemAttributeDefinition *pAttribDef =
						ItemSystem()->GetStaticDataForAttributeByName(pUpgradeBuyPanel->m_szAttribName);
					if(pAttribDef)
					{
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudUpgradePanel::AddItemStatText(const locchar_t *loc_AttrDescText, attrib_colors_t eColor,
									   wchar_t *out_wszAttribDesc, int iAttribDescSize)
{
	...
	else if(!Q_stricmp(command, "next"))
	{
		...
		if(m_hPlayer)
		{
			...
			for(i = 0; i < m_pItemPanels.Count(); i++)
			{
				...
				nLoadoutSlot = pItemPanel->GetItem()->GetStaticData()->GetLoadoutSlot(
					m_hPlayer->GetPlayerClass()->GetClassIndex());
...
bool MannVsMachine_GetUpgradeInfo(int iAttribute, int iQuality, float &flValue)
{
	FOR_EACH_VEC(g_MannVsMachineUpgrades.m_Upgrades, i)
	{
		CEconItemAttributeDefinition *pAttrib =
			ItemSystem()->GetItemSchema()->GetAttributeDefinitionByName(g_MannVsMachineUpgrades.m_Upgrades[i].szAttrib);
		if(!pAttrib)
			continue;
