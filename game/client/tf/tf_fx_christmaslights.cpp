...
void CreateHolidayLight(const HolidayLightData_t &holidayLight)
{
	...
	const model_t *pModel = (nHolidayLightStyle == 0 ? engine->LoadModel("effects/christmas_bulb.vmt")
													 : engine->LoadModel("effects/mtp_fluff.vmt"));
	if(!pModel)
		return;
	...
	if(!pTemp)
	{
		...
		QAngle angOrientation =
			(nHolidayLightStyle == 0 ? QAngle(0.0f, 0.0f, RandomFloat(-180.0f, 180.0f)) : vec3_angle);
		pTemp =
			tempents->SpawnTempModel(pModel, holidayLight.vOrigin, angOrientation, vec3_origin, 2.0f, FTENT_NEVERDIE);
		if(!pTemp)
		{
			return;
		}
		...
		// Set the color
		pTemp->SetRenderColor(
			rgbaHolidayLightColors[g_nHolidayLightColor]->r(), rgbaHolidayLightColors[g_nHolidayLightColor]->g(),
			rgbaHolidayLightColors[g_nHolidayLightColor]->b(), rgbaHolidayLightColors[g_nHolidayLightColor]->a());
	...
	else
	{
		...
		if(pTemp->m_nSkin % 5 == 0)
		{
			...
			int nCycle = (pTemp->hitSound + static_cast<int>(gpGlobals->curtime * 2.0f)) %
						 (pTemp->m_nHitboxSet + ARRAYSIZE(rgbaHolidayLightColors) + 1);
...
ConCommand r_ropes_holiday_light_color("r_ropes_holiday_light_color", RopesHolidayLightColor,
									   "Set each light's color: [light0-3] [r0-255] [g0-255] [b0-255]", FCVAR_NONE);