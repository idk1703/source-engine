...
private:
	...
	// keeps the two rounded ends a certain distance apart from each other so the rounded line continues to look good
	// even at low values
...
private:
	...
	void AddClassIconBeingUsed(CUtlVector<const char *> &vector,
							   const char *pchIcon) const; // used temporarily to track the icons we're showing
	bool IsClassIconBeingUsed(CUtlVector<const char *> &vector,
							  const char *pchIcon) const; // used temporarily to track the icons we're showing
...
private:
	...
	CPanelAnimationVarAliasType(int, m_nWaveCountYPos, "count_ypos", "32",
								"proportional_ypos"); // ypos for the icon/count pairs
	CPanelAnimationVarAliasType(int, m_nSupportLabelYOffset, "support_label_yOffset", "20", "proportional_ypos");
	CPanelAnimationVarAliasType(int, m_nWaveCountOffset, "count_offset", "5",
								"proportional_xpos"); // space between the icon/count pairs
	CPanelAnimationVarAliasType(int, m_nWaveCountBGMinWidth, "count_bg_min_width", "200",
								"proportional_xpos"); // min background width
...
public:
	...
	{
		return IsVictoryPanelVisible() ? GAME_ACTION_SET_MENUCONTROLS : CHudElement::GetPreferredActionSet();
	}
