...
void CTFPVEWinPanel::FireGameEvent(IGameEvent *event)
{
	...
	if(Q_strcmp("teamplay_round_start", pEventName) == 0 || Q_strcmp("teamplay_game_over", pEventName) == 0 ||
	   Q_strcmp("tf_game_over", pEventName) == 0 || Q_strcmp("training_complete", pEventName) == 0)
	{
		m_bShouldBeVisible = false;
	}
...
void CTFPVEWinPanel::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	{
		m_pRespecTextLabel =
			dynamic_cast<vgui::Label *>(m_pRespecContainerPanel->FindChildByName("RespecTextLabelLoss"));
