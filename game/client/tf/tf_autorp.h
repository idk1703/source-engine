...
private:
	struct wordreplacement_t
	{
		...
		CUtlVector<const char *> a_pszPluralReplacements; // If the match was a plural match, use these replacements
														  // instead, if they exist. Otherwise, use a_pszReplacements.
		CUtlVector<CUtlSymbol> m_Words;					  // Word that matches this replacement
		CUtlVector<CUtlSymbol> m_Plurals;	// Word that must come before to match this replacement, for double word
											// replacements (i.e. "it is" -> "'tis")
		CUtlVector<CUtlSymbol> m_PrevWords; // Word that must come before to match this replacement, for double word
											// replacements (i.e. "it is" -> "'tis")
	};
...
private:
	...
	bool PerformReplacement(const char *pszReplacement, replacementcheck_t *pRepCheck, char *szStoredWord,
