...
static ConVar hud_low_ammo_warning_threshold("hud_lowammowarning_threshold", "0.40",
											 FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY,
											 "Percentage threshold at which the low ammo warning will become visible.");
static ConVar hud_low_ammo_warning_max_pos_adjust("hud_lowammowarning_maxposadjust", "5",
												  FCVAR_CLIENTDLL | FCVAR_DEVELOPMENTONLY,
												  "Maximum pixel amount to increase the low ammo warning image.");
...
void CTFHudWeaponAmmo::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if(m_pLowAmmoImage)
	{
		m_pLowAmmoImage->GetBounds(m_nLowAmmoImageOrigX, m_nLowAmmoImageOrigY, m_nLowAmmoImageOrigW,
								   m_nLowAmmoImageOrigH);
	}
...
void CTFHudWeaponAmmo::ShowLowAmmoIndicator(void)
{
	...
	{
		m_pLowAmmoImage->SetBounds(m_nLowAmmoImageOrigX, m_nLowAmmoImageOrigY, m_nLowAmmoImageOrigW,
								   m_nLowAmmoImageOrigH);
...
void CTFHudWeaponAmmo::SizeLowAmmoIndicator(float flCurrentAmount, float flMaxAmount)
{
	if(m_pLowAmmoImage && m_pLowAmmoImage->IsVisible() == true)
	{
		...
		m_pLowAmmoImage->SetBounds(m_nLowAmmoImageOrigX - nPosAdj, m_nLowAmmoImageOrigY - nPosAdj,
								   m_nLowAmmoImageOrigW + nSizeAdj, m_nLowAmmoImageOrigH + nSizeAdj);
	}
}
