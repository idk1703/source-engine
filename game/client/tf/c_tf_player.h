...
{
	BonusEffect_t(const char *pszSoundName, const char *pszParticleName, EBonusEffectFilter_t eParticleFilter,
				  EBonusEffectFilter_t eSoundFilter, bool bPlaySoundInAttackersEars)
...
class C_TFPlayer : public C_BasePlayer,
				   ...
				   public C_TFMvMBossProgressUser
{
public:
	...
	CNewParticleEffect *SpawnHalloweenSpellFootsteps(ParticleAttachment_t eParticleAttachment,
	...
	bool StartGestureSceneEvent(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event, CChoreoActor *actor,
								CBaseEntity *pTarget);
	...
	void CreatePlayerGibs(const Vector &vecOrigin, const Vector &vecVelocity, float flImpactScale, bool bBurning,
						  bool bWearableGibs = false, bool bOnlyHead = false, bool bDisguiseGibs = false);
	...
	void GetTargetIDDataString(bool bIsDisguised, OUT_Z_BYTECAP(iMaxLenInBytes) wchar_t *sDataString,
	...
	RTime32 GetSpottedInPVSTime() const
	{
		return m_rtSpottedInPVSTime;
	}
	RTime32 GetJoinedSpectatorTeamTime() const
	{
		return m_rtJoinedSpectatorTeam;
	}
	RTime32 GetJoinedNormalTeamTime() const
	{
		return m_rtJoinedNormalTeam;
	}
