...
void TFPaintBombSplashCallback(const CEffectData &data)
{
	...
	{
		cColor.SetColor(data.m_CustomColors.m_vecColor1.x * 255, data.m_CustomColors.m_vecColor1.y * 255,
						data.m_CustomColors.m_vecColor1.z * 255, 255);
	}
	else
	{
	...
	if((pEntity->index == 0) && (data.m_nHitBox != 0))
	{
		...
		staticpropmgr->AddColorDecalToStaticProp(data.m_vStart, data.m_vOrigin, data.m_nHitBox - 1, nMaterial, false,
												 tr, true, cColor);
	}
	else
	{
		...
		pEntity->AddColoredDecal(data.m_vStart, data.m_vOrigin, data.m_vOrigin, data.m_nHitBox, nMaterial, false, tr,
								 cColor);
	}
}
...
void TFEnergyShieldImpactCallback(const CEffectData &data)
{
	...
	FX_AddQuad(vecPos,						   // Origin
			   ...
			   "effects/circle_nocull", (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));
	...
	for(int j = 0; j < 6; j++)
	{
		...
		sParticle->m_vecVelocity =
			Vector(random->RandomFloat(-8.f, 8.f), random->RandomFloat(-8.f, 8.f), random->RandomFloat(16.f, 64.f));
