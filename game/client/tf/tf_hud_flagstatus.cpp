...
ConVar tf_rd_flag_ui_mode("tf_rd_flag_ui_mode", "3", FCVAR_DEVELOPMENTONLY,
						  "When flags are stolen and not visible: 0 = Show outlines (glows), 1 = Show most valuable "
						  "enemy flag (icons), 2 = Show all enemy flags (icons), 3 = Show all flags (icons).");
...
void CTFHudFlagObjectives::OnTick()
{
	...
	for(int i = 0; i < ICaptureFlagAutoList::AutoList().Count(); i++)
	{
		...
		if(CTFRobotDestructionLogic::GetRobotDestructionLogic() &&
		   CTFRobotDestructionLogic::GetRobotDestructionLogic()->GetType() ==
	...
	{
		m_pPoisonTimeLabel->SetVisible(pPoisonFlag && pPoisonFlag->GetPoisonTime() > 0.f &&
									   !pPoisonFlag->IsPoisonous());
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFFlagCalloutPanel::GetCalloutPosition(const Vector &vecDelta, float flRadius, float *xpos, float *ypos,
											 float *flRotation)
{
	// Player Data
...
void CTFFlagCalloutPanel::OnTick(void)
{
	...
	if(!bOnScreen || iX < nHalfWidth || iX > ScreenWidth() - nHalfWidth)
	{
		...
		if(TFGameRules() && TFGameRules()->IsPlayingRobotDestructionMode() &&
		   gpGlobals->curtime > m_flFirstDisplayTime + 5.f)
		{
			m_iDrawArrow = DRAW_ARROW_UP;
			SetAlpha(0);
		}
	...
	m_pFlagCalloutPanel->SetImage(m_hFlag->GetTeamNumber() == TF_TEAM_BLUE ? "../hud/obj_briefcase_blue"
																		   : "../hud/obj_briefcase_red");
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFlagCalloutPanel *CTFFlagCalloutPanel::AddFlagCalloutIfNotFound(CCaptureFlag *pFlag, float flDuration,
																   Vector &vecLocation)
{
...
bool CTFFlagCalloutPanel::ShouldShowFlagIconToLocalPlayer(void)
{
	...
	if(m_hFlag->IsStolen() && m_hFlag->InSameTeam(pLocalTFPlayer) && nDisplayMode == FLAG_CALLER_DISPLAY_ALL)
		return false;

	// In all other modes, don't show flags on the local player's team
	if(m_hFlag->InSameTeam(pLocalTFPlayer) && nDisplayMode < FLAG_CALLER_DISPLAY_ALL)
		return false;
...
void CTFFlagCalloutPanel::ScaleAndPositionCallout(float flScale /*= 1.f*/)
{
	...
	m_pFlagValueLabel->SetPos((m_pFlagCalloutPanel->GetWide() - m_pFlagValueLabel->GetWide()) * 0.5f,
							  (m_pFlagCalloutPanel->GetWide() - m_pFlagValueLabel->GetTall()) * 0.65f);

	// Icon - lower-right
	m_pFlagStatusIcon->SetSize((m_nIconWideOrig * flScale), (m_nIconTallOrig * flScale));
	m_pFlagStatusIcon->SetPos((m_pFlagCalloutPanel->GetWide() - m_pFlagStatusIcon->GetWide()) * 1.05f,
							  (m_pFlagCalloutPanel->GetWide() - m_pFlagStatusIcon->GetTall()) * 0.85f);
