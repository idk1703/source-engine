...
static const char *GetPlayerProgressPortrait(C_TFPlayer *pPlayer)
{
	...
	switch(iClass)
	{
		case TF_CLASS_SOLDIER:
			return (iTeam == TF_TEAM_RED) ? "../passtime/hud/portrait_soldier_red"
		...
		case TF_CLASS_SNIPER:
			return (iTeam == TF_TEAM_RED) ? "../passtime/hud/portrait_sniper_red"
...
static const char *GetProgressGoalImage(const C_FuncPasstimeGoal *pGoal)
{
	...
	if(pGoal->GetGoalType() == C_FuncPasstimeGoal::TYPE_TOWER)
	{
		...
		{
			return bRedIcon ? "../passtime/hud/passtime_goal_red_locked" : "../passtime/hud/passtime_goal_blue_locked";
		}
		else
		{
			return bRedIcon ? "../passtime/hud/passtime_goal_red_unlocked"
							: "../passtime/hud/passtime_goal_blue_unlocked";
		}
	}
	else if(pGoal->GetGoalType() == C_FuncPasstimeGoal::TYPE_ENDZONE)
	{
		return bRedIcon ? "../passtime/hud/passtime_endzone_red_icon" : "../passtime/hud/passtime_endzone_blue_icon";
	}
	else
	{
		return bRedIcon ? "../passtime/hud/passtime_goal_red_icon" : "../passtime/hud/passtime_goal_blue_icon";
	}
}
...
void CTFHudTeamScore::OnTick()
{
	...
	if(m_pPlayingToCluster)
	{
		m_pPlayingToCluster->SetDialogVariable("rounds", tf_passtime_scores_per_round.GetInt());
	}
...
int CTFHudTeamScore::GetTeamScore(int iTeam)
{
	...
	return pTeam ? pTeam->Get_Score() : 0;
}
...
void CTFHudPasstimePassNotify::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	Assert(m_pTextInPassRange && m_pTextLockedOn && m_pTextPassIncoming && m_pTextPlayerName && m_pSpeechIndicator &&
		   m_pPassLockIndicator);
}
...
void CTFHudPasstimePassNotify::OnTick()
{
	...
	if(!pBallCarrier)
	{
		if(pBall && pBall->GetHomingTarget() == pLocalPlayer)
		{
			...
			if(m_pTextBox)
			{
				m_pTextBox->SetBorder((pLocalPlayer->GetTeamNumber() == TF_TEAM_RED) ? m_pTextBoxBorderIncomingRed
	...
	if(!pBallCarrier || pLocalPlayer->IsObserver() || (pBallCarrier == pLocalPlayer) ||
...
//-----------------------------------------------------------------------------
CTFHudPasstimeEventText::CTFHudPasstimeEventText() : m_localizeKeys("")
{
	m_bValid = false;
	m_pTitleLabel = 0;
	m_pDetailLabel = 0;
...
//-----------------------------------------------------------------------------
void CTFHudPasstimeEventText::Enqueue(C_TFPlayer *pSource, C_TFPlayer *pSubject, const char *pTitle,
									  const char *pDetail, const char *pBonus)
{
	...
	auto bShowBonus =
		(pSubject == pLocalPlayer) || (pLocalPlayer->IsObserver() && pLocalPlayer->GetObserverTarget() == pLocalPlayer);
...
{
	Enqueue(pVictim, pStealer, "#Msg_PasstimeEventStealTitle", "#Msg_PasstimeEventStealDetail",
...
{
	Enqueue(pThrower, pCatcher, "#Msg_PasstimeEventPassTitle", "#Msg_PasstimeEventPassDetail",
			"#Msg_PasstimeEventPassBonus");
}

//-----------------------------------------------------------------------------
void CTFHudPasstimeEventText::EnqueueInterception(C_TFPlayer *pThrower, C_TFPlayer *pCatcher)
{
	Enqueue(pThrower, pCatcher, "#Msg_PasstimeEventInterceptTitle", "#Msg_PasstimeEventInterceptDetail",
...
{
	if(pAssister)
		Enqueue(pAssister, pThrower, "#Msg_PasstimeEventScoreTitle", "#Msg_PasstimeEventScoreDetail_Assist",
				"#Msg_PasstimeEventScoreBonus");
	else
		Enqueue(pAssister, pThrower, "#Msg_PasstimeEventScoreTitle", "#Msg_PasstimeEventScoreDetail_NoAssist",
				"#Msg_PasstimeEventScoreBonus");
}
...
void CTFHudPasstimeBallStatus::ApplySchemeSettings(IScheme *pScheme)
{
	...
	m_pEventText->SetControls(FindControl<Label>("EventTitleLabel"), FindControl<Label>("EventDetailLabel"),
							  FindControl<Label>("EventBonusLabel"));

	m_bInitialized = m_pProgressBall && m_pProgressBallCarrierName && m_pProgressLevelBar && m_pSelfPlayerIcon &&
					 m_pBallPowerMeterFillContainer && m_pBallPowerMeterFill && m_pBallPowerMeterFinalSection &&
	...
	if(m_pBallPowerMeterFinalSection)
	{
		...
		m_pBallPowerMeterFinalSection->SetPos(m_pBallPowerMeterFinalSection->GetXPos() +
												  (m_iBallPowerMeterFillWidth - iFinalSectionWidth),
											  m_pBallPowerMeterFinalSection->GetYPos());
	}
...
static float CalcBallProgressFrac()
{
	...
	return CalcProgressFrac(pCarrier ? pCarrier->GetNetworkOrigin() : pBall->GetNetworkOrigin());
}
...
void CTFHudPasstimeBallStatus::UpdateGoalIcon(vgui::ImagePanel *pIcon, C_FuncPasstimeGoal *pGoal)
{
	...
	const int iActualBarHalfHeight =
		m_pProgressLevelBar ? m_pProgressLevelBar->GetTall() / 6 : 0; // magic number because NPOT waste in image
...
void CTFHudPasstimeBallStatus::OnTickVisible(C_TFPlayer *pLocalPlayer, C_PasstimeBall *pBall)
{
	...
	int iX_Ball = m_pProgressBall
					  ? Lerp(flBallProgressFrac, m_iXBlueProgress, m_iXRedProgress) - (m_pProgressBall->GetWide() / 2)
					  : 0;
	int iY_Ball = m_pProgressBall
					  ? Lerp(flBallProgressFrac, m_iYBlueProgress, m_iYRedProgress) - (m_pProgressBall->GetTall() / 2)
					  : 0;
	...
	CTFPlayer *pCarrier = pBall->GetCarrier();
	int iTeam = pCarrier ? pCarrier->GetTeamNumber() : pBall->GetTeamNumber();
	if(m_pProgressBall)
	{
	...
	}
	{
		...
		int iAlpha = (iCurPower > iThreshold) ? FLerp(150, 255, (1 + sin(gpGlobals->curtime * 5)) / 2.0f) : 222;
		...
		if(bWasAboveThreshold && !bIsAboveThreshold)
		{
			...
			m_pEventText->EnqueueGeneric("#Msg_PasstimeEventPowerDownTitle", "#Msg_PasstimeEventPowerDownDetail",
										 "#Msg_PasstimeEventPowerDownBonus");
		}
		else if(!bWasAboveThreshold && bIsAboveThreshold)
		{
			m_pEventText->Clear(); // push everything else out of the way
			m_pEventText->EnqueueGeneric("#Msg_PasstimeEventPowerUpTitle", "#Msg_PasstimeEventPowerUpDetail",
	...
	const int iActualBarHalfHeight =
		m_pProgressLevelBar ? m_pProgressLevelBar->GetTall() / 7 : 0; // magic number because NPOT waste in image
	for(int iEntIndex = 1; iEntIndex <= MAX_PLAYERS; iEntIndex++)
	{
		...
		// Player pips
		{
			...
			if(iEntTeam == TF_TEAM_RED)
			{
				int iX = Lerp(flProgressFrac, m_iXBlueProgress, m_iXRedProgress) - (pIcon->GetWide() / 2);
				int iY =
					Lerp(flProgressFrac, m_iYBlueProgress, m_iYRedProgress) - pIcon->GetTall() - iActualBarHalfHeight;
				pIcon->SetPos(iX, iY);
		...
		if(g_TF_PR->IsLocalPlayer(iEntIndex) || (pLocalPlayer->IsObserver() && (pPlayer == pSpecTarget)))
		{
			if(m_pSelfPlayerIcon)
			{
				...
				int iY = Lerp(flProgressFrac, m_iYBlueProgress, m_iYRedProgress) - m_pSelfPlayerIcon->GetTall() -
						 iActualBarHalfHeight;
	...
	if(!m_bGoalsFound && bReadyToFindGoals)
	{
		...
		if((iNumGoals > 1) && (iNumGoals < iMaxSortedGoals))
		{
			...
			std::sort(sortedgoals, sortedgoals + iNumGoals,
					  [](C_FuncPasstimeGoal *a, C_FuncPasstimeGoal *b)
					  {
...
//-----------------------------------------------------------------------------
void CTFHudPasstimeBallStatus::OnBallFreeOther(C_TFPlayer *pOwner, C_TFPlayer *pAttacker)
{
	Assert(m_bInitialized);
	if(m_pProgressBallCarrierName)
	{
...
//-----------------------------------------------------------------------------
void CTFHudPasstimeBallStatus::OnBallFreeSelf(C_TFPlayer *pOwner, C_TFPlayer *pAttacker)
{
	Assert(m_bInitialized);
	if(m_pProgressBallCarrierName)
	{
...
void CTFHudPasstimeBallStatus::OnBallGetOther(int iPlayer)
{
	...
	g_pVGuiLocalize->ConvertANSIToUnicode((iPlayer > 0) ? g_PR->GetPlayerName(iPlayer) : "", wszPlayerName,
