...
bool C_PasstimeGoalReticle::Update()
{
	...
	if(!g_pPasstimeLogic || !g_pPasstimeLogic->GetBall() || IsLocalPlayerSpectator() || !pGoal ||
	   pGoal->BGoalTriggerDisabled() || (pGoal->GetTeamNumber() != pEnt->GetTeamNumber()))
	{
		return false;
	}
...
bool C_PasstimePlayerReticle::Update()
{
	...
	if(!pLocalPlayer->m_Shared.HasPasstimeBall() &&
	   ((bIsFriend && (iFriendsDetail == 1)) || (!bIsFriend && (iEnemiesDetail == 1))))
	{
	...
	if(!pLocalPlayer->InSameTeam(pPlayer) &&
	   bIsFriend) // they're not on my team but they're showing as a friend, they must be a spy so use my team color
	{
