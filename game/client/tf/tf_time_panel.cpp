...
// Purpose:
//-----------------------------------------------------------------------------
CTFProgressBar::CTFProgressBar(Panel *parent, const char *name) : ImagePanel(parent, name)
{
	m_flPercent = 0.0f;
...
void CTFHudTimeStatus::SetExtraTimePanels()
{
	...
	if(m_pSuddenDeathBG && m_pSuddenDeathLabel)
	{
		...
		if(bInSD != m_pSuddenDeathLabel->IsVisible())
		{
			...
			{
				g_pClientMode->GetViewportAnimationController()->StopAnimationSequence(this,
	...
	if(m_flNextThink < gpGlobals->curtime)
	{
		...
		if(pTimer)
		{
			...
			if(m_pServerTimeLabel && m_pServerTimeLabelBG)
			{
				...
				bool bDisplayServerTimerEnabled =
					tf_hud_show_servertimelimit.GetInt() && TFGameRules() && !TFGameRules()->InSetup() &&
					!TFGameRules()->IsInWaitingForPlayers() && pTimer->IsRoundMaxTimerSet() && nServerTimeLimit;
				...
				if(bDisplayServerTimerEnabled)
				{
					...
					if(!nServerTimeLimit)
					{
						g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft,
															  g_pVGuiLocalize->Find("#TF_HUD_ServerNoTimeLimit"), 0);
					...
					if(iTimeLeft == 0)
					{
						g_pVGuiLocalize->ConstructString_safe(
							wzServerTimeLeft, g_pVGuiLocalize->Find("#TF_HUD_ServerChangeOnRoundEnd"), 0);
						SetDialogVariable("servertimeleft", wzServerTimeLeft);
						return;
					}
					...
					{
						g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft,
															  g_pVGuiLocalize->Find("#TF_HUD_ServerTimeLeftNoHours"), 2,
					...
					g_pVGuiLocalize->ConstructString_safe(
						wzServerTimeLeft, g_pVGuiLocalize->Find("#TF_HUD_ServerTimeLeft"), 3, wzServerTimeHrsLeft,
						wzServerTimeMinLeft, wzServerTimeSecLeft);
					SetDialogVariable("servertimeleft", wzServerTimeLeft);
				}
			}
...
void CTFHudTimeStatus::Paint(void)
{
	...
	for(int i = 0; i < NUM_TIMER_DELTA_ITEMS; i++)
	{
		...
		if(m_TimerDeltaItems[i].m_flDieTime > gpGlobals->curtime)
		{
			...
			int nClockTime = (m_TimerDeltaItems[i].m_nAmount > 0) ? m_TimerDeltaItems[i].m_nAmount
																  : (m_TimerDeltaItems[i].m_nAmount * -1);
...
void CTFHudKothTimeStatus::UpdateActiveTeam(void)
{
	if(ShouldUseMatchHUD())
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			m_pRedPanel, m_nActiveTeam == TF_TEAM_RED ? "ActiveTimerHighlight" : "ActiveTimerDim", false);
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			m_pBluePanel, m_nActiveTeam == TF_TEAM_BLUE ? "ActiveTimerHighlight" : "ActiveTimerDim", false);
	}
