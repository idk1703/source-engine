...
CSaxxyAwardsPanel::CSaxxyAwardsPanel(Panel *pParent, const char *pName)
	: EditablePanel(pParent, pName), m_pCameraFlashKv(NULL)
{
...
bool CSaxxyAwardsPanel::CreateFlash()
{
	...
	const int iUnusedSlot = GetUnusedFlashSlot();
	Assert(iUnusedSlot >= 0 && iUnusedSlot < MAX_FLASHES);
...
{
	pFlashInfo->m_pPanel->SetBounds(pFlashInfo->m_nCenterX - pFlashInfo->m_nCurW / 2,
									pFlashInfo->m_nCenterY - pFlashInfo->m_nCurH / 2, pFlashInfo->m_nCurW,
									pFlashInfo->m_nCurH);
}
...
bool CSaxxyAwardsPanel::AreOtherPanelsOpen(float flCurTime)
{
	...
	bool bOtherPanelsOpen = false;
	{
		...
		bOtherPanelsOpen = bOtherPanelsOpen || AreNonMainMenuPanelsOpen(enginevgui->GetPanel(PANEL_GAMEUIDLL),
																		pCarePanels, nNumGameUICarePanels);
	}
...
void CSaxxyAwardsPanel::RotateModel(float flElapsed)
{
	if(m_pSaxxyModelPanel)
	{
		...
		const Vector vCurPos(Lerp(m_flGlowFade, m_vSaxxyDefaultPos.x, m_vSaxxyDefaultPos.x - 8.0f),
...
CSaxxyAwardsSubmitForm::CSaxxyAwardsSubmitForm(Panel *pParent)
	: BaseClass(pParent, "SaxxySubmitForm"), m_pURLInput(NULL), m_pCategoryCombo(NULL)
{
}
	...
	else if(FStrEq(pCommand, "rules"))
	{
		if(steamapicontext && steamapicontext->SteamFriends())
		{
			steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(
				"http://www.teamfortress.com/saxxyawards/#rules");
		}
	}
	else
	{
...
GC_REG_JOB(GCSDK::CGCClient, CGC_Replay_SubmitContestEntryResponse_Status,
		   "CGC_Replay_SubmitContestEntryResponse_Status", k_EMsgGCReplay_SubmitContestEntryResponse,
