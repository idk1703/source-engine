...
RecvPropInt(RECVINFO(m_iState)), RecvPropTime(RECVINFO(m_flRechargeTime)),
	RecvPropTime(RECVINFO(m_flCurrentRechargeDuration)), RecvPropInt(RECVINFO(m_iTimesUsed)),
	RecvPropFloat(RECVINFO(m_flYawToExit)), RecvPropBool(RECVINFO(m_bMatchBuilding)),
END_RECV_TABLE
()
...
void C_ObjectTeleporter::StartBuildingEffects()
{
	...
	// arm glow effects
	Q_snprintf(szEffect, sizeof(szEffect), "teleporter_arms_circle_%s_blink",
...
void C_ObjectTeleporter::StartActiveEffects()
{
	...
	Q_snprintf(szEffect, sizeof(szEffect), "teleporter_%s_%s_level%d",
			   (GetTeamNumber() == TF_TEAM_RED) ? "red" : "blue",
			   GetObjectMode() == MODE_TELEPORTER_ENTRANCE ? "entrance" : "exit", GetUpgradeLevel());
...
void C_ObjectTeleporter::UpdateTeleporterEffects(void)
{
	...
	else if((m_iState <= TELEPORTER_STATE_IDLE || m_iState == TELEPORTER_STATE_UPGRADING) &&
			m_iOldState > TELEPORTER_STATE_IDLE)
	{
		StopActiveEffects();
	}
}
...
void C_ObjectTeleporter::GetTargetIDDataString(OUT_Z_BYTECAP(iMaxLenInBytes) wchar_t *sDataString, int iMaxLenInBytes)
{
	...
	if(m_iState == TELEPORTER_STATE_RECHARGING && gpGlobals->curtime < m_flRechargeTime)
	{
		...
		g_pVGuiLocalize->ConstructString(sDataString, iMaxLenInBytes, g_pVGuiLocalize->Find(printFormatString), 1,
										 wszRecharging);
	}
	else if(m_iState == TELEPORTER_STATE_IDLE)
	{
		g_pVGuiLocalize->ConstructString(sDataString, iMaxLenInBytes,
										 g_pVGuiLocalize->Find("#TF_playerid_teleporter_nomatch"), 0);
	}
