...
bool CDisguiseStatus::ShouldDraw(void)
{
	...
	if(bShow && (!m_bDisguised || (m_pDisguiseWeapon != pPlayer->m_Shared.GetDisguiseWeapon()) ||
...
void CDisguiseStatus::CheckHealth(void)
{
	...
	m_pDisguiseHealth->SetHealth(pPlayer->m_Shared.GetDisguiseHealth(), pPlayer->m_Shared.GetDisguiseMaxHealth(),
								 pPlayer->m_Shared.GetDisguiseMaxBuffedHealth());
}
...
void CDisguiseStatus::PerformLayout(void)
{
	...
	if ( m_pTargetNameLabel && m_pTargetDataLabel )
	{
		...
		int nOffset = m_bArenaPanelVisible ? YRES (120) : 0; // HACK: move the targetID up a bit so it won't overlap the
	panel
