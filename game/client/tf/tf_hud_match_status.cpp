...
void CRoundCounterPanel::PerformLayout()
{
	...
	LayoutPanels(m_vecBlueRoundIndicators, EAlignment::ALIGN_WEST, (GetWide() / 2) - m_nIndicatorStartOffset,
				 m_nIndicatorPanelStep);
	VisibleCondition(m_vecBlueRoundIndicators, mp_winlimit.GetInt());
	LayoutPanels(m_vecRedRoundIndicators, EAlignment::ALIGN_EAST, (GetWide() / 2) + m_nIndicatorStartOffset,
				 m_nIndicatorPanelStep);
	VisibleCondition(m_vecRedRoundIndicators, mp_winlimit.GetInt());
	// Layout the win indicators
	LayoutPanels(m_vecBlueWinIndicators, EAlignment::ALIGN_WEST, (GetWide() / 2) - m_nIndicatorStartOffset,
				 m_nIndicatorPanelStep);
	VisibleCondition(m_vecBlueWinIndicators, Min(mp_winlimit.GetInt(), pTeams[TF_TEAM_BLUE]->m_iScore));
	LayoutPanels(m_vecRedWinIndicators, EAlignment::ALIGN_EAST, (GetWide() / 2) + m_nIndicatorStartOffset,
				 m_nIndicatorPanelStep);
...
void CRoundCounterPanel::FireGameEvent(IGameEvent *event)
{
	if(FStrEq(event->GetName(), "winlimit_changed")) // Resize if the win limit changes
	{
		m_bCountDirty = true;
			...
			|| FStrEq(event->GetName(),
					  "stop_watch_changed") // Match the timing of the win panel "Ding!" when the scores update
			|| FStrEq(event->GetName(), "teamplay_round_start")) // Make sure we're accurate when the round starts in
																 // case the hud event didnt happen
	{
...
void CTFHudMatchStatus::FireGameEvent(IGameEvent *event)
{
	...
	else if(FStrEq("show_match_summary", event->GetName()))
	{
		...
		{
			if(TFGameRules() && TFGameRules()->MapHasMatchSummaryStage() &&
			   (bForceDoors || pMatchDesc->m_params.m_bUseMatchSummaryStage))
			{
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
					this, "HudMatchStatus_ShowMatchWinDoors", false);
			}
			else
			{
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
					this, "HudMatchStatus_ShowMatchWinDoors_NoOpen", false);
			}
		}
	}
}
...
void CTFHudMatchStatus::HandleCountdown(int nTime)
{
	...
	switch(nTime)
	{
		...
		case 10:
			...
			{
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
					this, "HudMatchStatus_ShowCountdown", false);
			}
			break;
	}
}
...
#ifdef STAGING_ONLY
ConVar tf_comp_door_skin_override(
	"tf_comp_door_skin_override", "-1", 0,
	"Skin override for the competitive doors.  Set to -1 to not override calculated skin");
ConVar tf_comp_door_bodygroup_override(
	"tf_comp_door_bodygroup_override", "-1", 0,
	"Bodygroup override for the competitive doors.  Set to -1 to not override calculated skin");
#endif
...
void CTFHudMatchStatus::ShowMatchStartDoors()
{
	...
	if(pMatchDesc->BGetRoundDoorParameters(nSkin, nSubModel))
	{
		...
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			this, "HudMatchStatus_ShowMatchStartDoors", false);
...
void CTFHudMatchStatus::ShowRoundSign(int nRoundNumber)
{
	...
	int nSkin = 0;
	int nBodyGroup = 0;
	if(GetMatchGroupDescription(TFGameRules()->GetCurrentMatchGroup())
		   ->BGetRoundStartBannerParameters(nSkin, nBodyGroup))
	{
		...
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(this, "HudTournament_ShowRoundSign",
																				false);
	}
}
...
void CTFHudMatchStatus::InitPlayerList(SectionedListPanel *pPlayerList, int nTeam)
{
	...
	int nExtraSpace =
		pPlayerList->GetWide() - m_iAvatarWidth - m_iSpacerWidth - m_iNameWidth -
