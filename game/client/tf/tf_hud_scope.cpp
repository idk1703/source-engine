...
void CHudScopeCharge::Paint(void)
{
	...
	if(UseVR())
	{
		...
		// The origin is wherever the property file put it, so (0,0) means "the right place, if the scope is in the
		// middle of the screen" So offset from there. But additional fun - the rendering coordinates are in the UI
		// space, not the actual screen space.
...
void CHudScope::ApplySchemeSettings(vgui::IScheme *scheme)
{
	...
	if(UseVR())
	{
		...
		g_pSourceVR->GetViewportBounds(ISourceVirtualReality::VREye_Left, NULL, NULL, &iViewportWidth,
									   &iViewportHeight);
