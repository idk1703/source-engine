...
RecvPropEHandle(RECVINFO(m_laserTarget)), RecvPropBool(RECVINFO(m_isNuking)),
...
void C_BotNPC::ClientThink(void)
{
	if(m_laserTarget)
	{
		...
		{
			m_laserBeamEffect =
				ParticleProp()->Create("laser_sight_beam", PATTACH_POINT_FOLLOW, LookupAttachment("head"));
		}
