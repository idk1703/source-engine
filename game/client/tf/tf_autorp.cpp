...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFAutoRP::PerformReplacement(const char *pszReplacement, replacementcheck_t *pRepCheck, char *szStoredWord,
								   int iStoredWordSize, char *pszOutText, int iOutLen)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFAutoRP::ModifySpeech(const char *pszInText, char *pszOutText, int iOutLen, bool bGeneratePreAndPost,
							 bool bInPrePost)
{
	...
	while(1)
	{
		...
		{
			bool bChanged =
				bModifyWord ? ReplaceWord(&repCheck, szCurrentWord, sizeof(szCurrentWord), false, bInPrePost) : false;

			// If the character that broke the last two words apart was an apostrophe, see if we can replace the whole
			// word
			...
			if(szStoredWord[0] != '\0')
			{
				if(PerformReplacement(szCurrentWord, &repCheck, szStoredWord, sizeof(szStoredWord), pszOutText,
									  iOutLen))
				{
