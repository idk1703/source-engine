	...
	if(bVisible && g_TF_PR)
	{
		...
		if(m_pClassImage)
		{
			if(m_bPrevAlive != bAlive)
			{
				...
				if(!bAlive)
				{
					m_pClassImage->SetDrawColor((m_iTeam == TF_TEAM_RED) ? m_ColorPortraitBlendDeadRed
																		 : m_ColorPortraitBlendDeadBlue);
				}

				m_pDeathFlag->SetImage((m_iTeam == TF_TEAM_RED) ? "../HUD/comp_player_status"
																: "../HUD/comp_player_status_blue");

				if(bAlive)
				{
					g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
						this, "TeamStatus_PlayerAlive", false);
				}
				else
				{
					g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
						this, "TeamStatus_PlayerDead", false);
				}
			}
			...
			if((m_iPrevClass != iClass) || bChanged)
			{
				...
				else if(iClass == TF_CLASS_UNDEFINED)
				{
					...
					if(!bAlive && !bSameTeamAsLocalPlayer && (m_iTeam >= FIRST_GAME_TEAM) &&
					   (iDeadClass > TF_CLASS_UNDEFINED))
					{
						m_pClassImage->SetImage(VarArgs("%s_alpha", (m_iTeam == TF_TEAM_RED)
																		? g_pszItemClassImagesRed[iDeadClass + 9]
																		: g_pszItemClassImagesBlue[iDeadClass + 9]));
					}
					else
					{
				...
				else
				{
					...
					{
						m_pClassImage->SetImage(VarArgs("%s_alpha", (m_iTeam == TF_TEAM_RED)
																		? g_pszItemClassImagesRed[iClass]
																		: g_pszItemClassImagesBlue[iClass]));
					}
					else
					{
						m_pClassImage->SetImage(VarArgs("%s_alpha", (m_iTeam == TF_TEAM_RED)
																		? g_pszItemClassImagesRed[iClass + 9]
																		: g_pszItemClassImagesBlue[iClass + 9]));
					}
				}
			}
		}
...
void CTFTeamStatus::PerformLayout(void)
{
	...
	for(int i = 0; i < m_PlayerPanels.Count(); i++)
	{
		...
		int nTeamPanelIndex = bIsLocalPlayerPanel ? 0 : iTeam == nLocalPlayerTeam ? iProcessed + 1 : iProcessed;
		...
		iXPos =
			eGrowDir == GROW_EAST
				? iBaseX + iXOffset // Align the left edge of the left-most panel on the specified point
			: eGrowDir == GROW_WEST
				? iBaseX - iXOffset - nNewWide // Align the right edge of the right-most panel on the specified point
				: 0;
