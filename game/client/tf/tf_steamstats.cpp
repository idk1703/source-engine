	...
	else if(0 == Q_strcmp(pEventName, "user_data_downloaded"))
	{
		...
		for(int iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_LAST_NORMAL_CLASS; iClass++)
		{
			...
			for(int iStat = 0; iStat < ARRAYSIZE(g_SteamStats); iStat++)
			{
				...
				// MVM Stats
				Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.mvm.accum.%s",
						   g_aPlayerClassNames_NonLocalized[iClass], g_SteamStats[iStat].pszName);
				if(steamapicontext->SteamUserStats()->GetStat(szStatName, &iData))
				{
					classStats.accumulatedMVM.m_iStat[g_SteamStats[iStat].iStat] = iData;
				}
				Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.mvm.max.%s", g_aPlayerClassNames_NonLocalized[iClass],
						   g_SteamStats[iStat].pszName);
				if(steamapicontext->SteamUserStats()->GetStat(szStatName, &iData))
				{
			...
			if(pClassStatMap)
			{
				...
				do
				{
					...
					Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.accum.%s",
							   g_aPlayerClassNames_NonLocalized[iClass], pClassStatMap[iStat].pszName);
					if(steamapicontext->SteamUserStats()->GetStat(szStatName, &iData))
					{
						classStats.accumulated.m_iStat[pClassStatMap[iStat].iStat] = iData;
					}
					Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.max.%s", g_aPlayerClassNames_NonLocalized[iClass],
							   pClassStatMap[iStat].pszName);
					if(steamapicontext->SteamUserStats()->GetStat(szStatName, &iData))
					{
					...
					Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.mvm.max.%s",
							   g_aPlayerClassNames_NonLocalized[iClass], pClassStatMap[iStat].pszName);
					if(steamapicontext->SteamUserStats()->GetStat(szStatName, &iData))
					{
		...
		for(int i = 0; i < GetItemSchema()->GetMapCount(); i++)
		{
			...
			for(int iStat = 0; iStat < ARRAYSIZE(g_SteamMapStats); iStat++)
			{
				...
				Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.accum.%s", pMap->pszMapName,
						   g_SteamMapStats[iStat].pszName);
				if(steamapicontext->SteamUserStats()->GetStat(szStatName, &iData))
				{
	...
	for(int iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_LAST_NORMAL_CLASS; iClass++)
	{
		...
		for(int iStat = 0; iStat < ARRAYSIZE(g_SteamStats); iStat++)
		{
			...
			steamapicontext->SteamUserStats()->SetStat(szStatName,
													   classStats.accumulated.m_iStat[g_SteamStats[iStat].iStat]);
			...
			// MVM Stats
			Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.mvm.accum.%s", g_aPlayerClassNames_NonLocalized[iClass],
					   g_SteamStats[iStat].pszName);
			steamapicontext->SteamUserStats()->SetStat(szStatName,
													   classStats.accumulatedMVM.m_iStat[g_SteamStats[iStat].iStat]);

			Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.mvm.max.%s", g_aPlayerClassNames_NonLocalized[iClass],
					   g_SteamStats[iStat].pszName);
			steamapicontext->SteamUserStats()->SetStat(szStatName,
													   classStats.maxMVM.m_iStat[g_SteamStats[iStat].iStat]);
		}
		...
		if(pClassStatMap)
		{
			...
			do
			{
				...
				Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.accum.%s", g_aPlayerClassNames_NonLocalized[iClass],
						   pClassStatMap[iStat].pszName);
				steamapicontext->SteamUserStats()->SetStat(szStatName,
														   classStats.accumulated.m_iStat[pClassStatMap[iStat].iStat]);

				Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.max.%s", g_aPlayerClassNames_NonLocalized[iClass],
						   pClassStatMap[iStat].pszName);
				steamapicontext->SteamUserStats()->SetStat(szStatName,
														   classStats.max.m_iStat[pClassStatMap[iStat].iStat]);

				// MVM Stats
				Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.mvm.accum.%s",
						   g_aPlayerClassNames_NonLocalized[iClass], pClassStatMap[iStat].pszName);
				steamapicontext->SteamUserStats()->SetStat(
					szStatName, classStats.accumulatedMVM.m_iStat[pClassStatMap[iStat].iStat]);

				Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.mvm.max.%s", g_aPlayerClassNames_NonLocalized[iClass],
						   pClassStatMap[iStat].pszName);
				steamapicontext->SteamUserStats()->SetStat(szStatName,
														   classStats.maxMVM.m_iStat[pClassStatMap[iStat].iStat]);
	...
	for(int i = 0; i < GetItemSchema()->GetMapCount(); i++)
	{
		...
		for(int iStat = 0; iStat < ARRAYSIZE(g_SteamMapStats); iStat++)
		{
			...
			Q_snprintf(szStatName, ARRAYSIZE(szStatName), "%s.accum.%s", pMap->pszMapName,
					   g_SteamMapStats[iStat].pszName);
			steamapicontext->SteamUserStats()->SetStat(szStatName,
													   mapStats.accumulated.m_iStat[g_SteamMapStats[iStat].iStat]);
		}
	}
