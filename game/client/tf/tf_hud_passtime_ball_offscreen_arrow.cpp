...
void CTFHudPasstimeOffscreenArrow::PaintBackground()
{
	...
	Vector vecEntPos = pEnt->WorldSpaceCenter();
	{
		...
		const bool bCenterOfScreen = (vecActualProjection.x < 1.0f) && (vecActualProjection.x > -1.0f) &&
	...
	float flArrowAngle;
	{
		const Vector vecHudDir =
			Vector(-vecLocalTarget.y / engine->GetScreenAspectRatio(), -vecLocalTarget.z, 0).Normalized();
...
//-----------------------------------------------------------------------------
C_BaseEntity *CTFHudPasstimeBallOffscreenArrow::PreparePaint(vgui::ImagePanel *pImage, C_TFPlayer *pLocalPlayer)
{
	if(!g_pPasstimeLogic)
	{
		return NULL;
...
//-----------------------------------------------------------------------------
C_BaseEntity *CTFHudPasstimePlayerOffscreenArrow::PreparePaint(vgui::ImagePanel *pImage, C_TFPlayer *pLocalPlayer)
{
