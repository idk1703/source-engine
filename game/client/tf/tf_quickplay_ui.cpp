...
ConVar tf_quickplay_pref_community_servers("tf_quickplay_pref_community_servers", "0", FCVAR_ARCHIVE,
...
static int FindRecentlyMatchedServer(uint32 ip, uint16 port)
{
	...
	while(i < s_vecRecentlyMatchedServers.Count())
	{
		...
		{
			DevLog(" Expiring quickplay recently joined server entry %08X:%d\n", s_vecRecentlyMatchedServers[i].m_ip,
				   (int)s_vecRecentlyMatchedServers[i].m_port);
...
ConVar tf_quickplay_pref_increased_maxplayers("tf_quickplay_pref_increased_maxplayers", "0", FCVAR_ARCHIVE,
											  "0=Default only, 1=Yes, 2=Don't care");
ConVar tf_quickplay_pref_disable_random_crits("tf_quickplay_pref_disable_random_crits", "0", FCVAR_ARCHIVE,
											  "0=Random crits enabled, 1=Random crits disabled, 2=Don't care");
ConVar tf_quickplay_pref_enable_damage_spread("tf_quickplay_pref_enable_damage_spread", "0", FCVAR_ARCHIVE,
											  "0=Damage spread disabled, 1=Damage spread enabled, 2=Don't care");
ConVar tf_quickplay_pref_respawn_times(
	"tf_quickplay_pref_respawn_times", "0", FCVAR_ARCHIVE,
	"0=Default respawn times only, 1=Instant respawn times ('norespawn' tag), 2=Don't care");
ConVar tf_quickplay_pref_advanced_view("tf_quickplay_pref_advanced_view", "0", FCVAR_NONE,
									   "0=Default to simplified view, 1=Default to more detailed options view");
ConVar tf_quickplay_pref_beta_content("tf_quickplay_pref_beta_content", "0", FCVAR_ARCHIVE,
...
static void GetQuickplayTags(const QuickplaySearchOptions &opt, CUtlStringList &requiredTags,
...
class CQuickplayWaitDialog : public CGenericWaitingDialog,
...
public:
		  ...
		  m_ScoringNumbers(steamapicontext ? steamapicontext->SteamUser()->GetSteamID() : CSteamID())
	{
		...
		{
			m_fValveBonus =
				lerp(m_fValveBonusHrsA, m_fValveBonusPtsA, m_fValveBonusHrsB, m_fValveBonusPtsB, m_fHoursPlayed);
		}
		else
		{
		...
		m_hServerListRequest = steamapicontext->SteamMatchmakingServers()->RequestInternetServerList(
			GetTfMatchmakingAppID(), &pFilters, vecServerFilters.Count(), this);
	}
	...
	void OnReceivedGCScores(CMsgTFQuickplay_ScoreServersResponse &msg)
	{
		...
		if(msg.servers_size() > 0)
		{
			...
			for(int i = 0; i < msg.servers_size(); ++i)
			{
				...
				int nOptionsScore = RemapValClamped(info.options_score(), 0.f, 30000.f, tf_mm_options_bonus.GetFloat(),
													tf_mm_options_penalty.GetFloat());
	...
	void ShowSelectServerUI()
	{
		...
		if(m_pServerListPanel)
		{
			FOR_EACH_VEC(m_vecServerJoinQueue, idxServer)
			{
				...
				const char *pszFriendlyGameTypeName = GetServerBrowser()->GetMapFriendlyNameAndGameType(
					s.server.m_szMap, szFriendlyName, sizeof(szFriendlyName));
	...
	void ApplySchemeSettings(vgui::IScheme *pScheme)
	{
		...
		Assert(pTitle);
		if(pTitle)
			pTitle->SetText(m_bFeelingLucky ? "#TF_MM_WaitDialog_Title_FeelingLucky"
											: "#TF_MM_WaitDialog_Title_ShowServers");
...
protected:
	...
	virtual void ServerResponded(gameserveritem_t &server)
	{
		...
		// Bogus steam ID?
		if(!server.m_steamID.IsValid() || !server.m_steamID.BGameServerAccount() ||
		   server.m_steamID.GetAccountID() == 0)
		{
			TF_MATCHMAKING_SPEW(2, "Quickplay ignoring gameserver at %s with invalid Steam ID %s",
								server.m_NetAdr.GetQueryAddressString(), server.m_steamID.Render());
			return;
		}
		...
		int failureCodes = 0;
		if(!PassesFilter(item.server))
			failureCodes |= (1 << 15);
		if(!HasAppropriateTags(TagList))
			failureCodes |= (1 << 14);
		if(pMapInfo == NULL)
		...
		else
		{
			...
			{
				case kGameCategory_EventMix:
					if(pMapInfo->eGameCategory != kGameCategory_EventMix &&
					   pMapInfo->eGameCategory != kGameCategory_Event247)
						failureCodes |= (1 << 12);
					break;
		...
		if(server.m_nPlayers >= server.m_nMaxPlayers)
			failureCodes |= (1 << 11);
		if(m_blackList.IsServerBlacklisted(server))
			failureCodes |= (1 << 10);
		...
		else
		{
			...
			item.serverScore = QuickplayCalculateServerScore(nHumans, item.server.m_nBotPlayers,
															 item.server.m_nMaxPlayers, nNumInSearchParty);
			item.serverScore += 6.0f; // !KLUDGE! Add offset to keep score ranges comparable with historical norms.  (We
									  // used to give bonuses for some things we now simply require.)
		...
		else if(m_eCurrentStep == k_EStep_PingCheckNotFull || m_eCurrentStep == k_EStep_SelectServerUI)
		{
			...
			if(netAdr.GetIPNetworkByteOrder() != m_adrCurrentPing.GetIPNetworkByteOrder() ||
			   netAdr.GetPort() != m_adrCurrentPing.GetPort())
			{
	...
	static void AddFilter(CUtlVector<MatchMakingKeyValuePair_t> &vecServerFilters, const char *pchKey,
						  const char *pchValue)
	{
		// @note Tom Bui: this is to get around the linker error where we don't like strncpy in
		// MatchMakingKeyValuePair_t's constructor!
	...
	void CalculateRecentMatchPenalty(sortable_gameserveritem_t &item)
	{
		// Hysteresis
		int iRecentIndex =
			FindRecentlyMatchedServer(item.server.m_NetAdr.GetIP(), item.server.m_NetAdr.GetConnectionPort());
		...
		if(iRecentIndex >= 0)
		{
			...
			else
			{
				...
				if(fCooldownTime > 0.0f) // 0 can be used to turn this off, but then all the entries should immediately
										 // expire and we shouldn't be here...
				{
	...
	virtual void GenericFailure()
	{
		...
		ShowMessageBox("#TF_MM_GenericFailure_Title", "#TF_MM_GenericFailure", "#GameUI_OK",
					   &OpenQuickplayDialogCallback);
	}

	virtual void NoServersFoundFailure()
	{
		CloseMe();
		ShowMessageBox("#TF_MM_ResultsDialog_Title", "#TF_MM_ResultsDialog_ServerNotFound", "#GameUI_OK",
					   &OpenQuickplayDialogCallback);
	}
	...
	void CheckSendScoresToGC()
	{
		...
		if(m_hServerListRequest)
		{
			...
			float fScoreLo = Lerp(fSearchTimePct, tf_matchmaking_goodenough_score_start.GetFloat(),
								  tf_matchmaking_goodenough_score_end.GetFloat());
			float fCountLo = Lerp(fSearchTimePct, tf_matchmaking_goodenough_count_start.GetFloat(),
								  tf_matchmaking_goodenough_count_end.GetFloat());
			// float fScoreHi = Lerp( fSearchTimePct, tf_matchmaking_goodenough_hi_score_start.GetFloat(),
			// tf_matchmaking_goodenough_hi_score_end.GetFloat() ); float fCountHi = Lerp( fSearchTimePct,
			// tf_matchmaking_goodenough_hi_count_start.GetFloat(), tf_matchmaking_goodenough_hi_count_end.GetFloat() );
			...
			while(iEligibleCount < vecGameServers.Count() &&
				  vecGameServers[iEligibleCount]->m_eStatus > TF_Gamestats_QuickPlay_t::k_Server_Ineligible)
			{
			...
			if(m_pBusyContainer)
			{
				...
				g_pVGuiLocalize->ConstructString_safe(wszText, g_pVGuiLocalize->Find("#TF_Quickplay_NumGames"), 1,
													  wszValue);
			...
			{
				TF_MATCHMAKING_SPEW(1,
									"Quickplay initial search: %d servers with scores %.2f or better.  Good "
									"enough...proceeding to backend scoring.\n",
		...
		TF_MATCHMAKING_SPEW(
			1, "Found %d total servers, %d met minimum score threshold, %d will be sent to GC for scoring\n",
		...
		if(vecGameServersToPrint.Count() > 0)
		{
			...
			Msg("Name, address, numplayers, maxplayers, ping, new user friendly, registered, valve, recent match "
				"penalty, user score, server score, total score, steamID, map, tags\n");
			...
			for(int i = vecGameServersToPrint.Count() - 1; i >= 0; --i)
			{
				...
				Msg("\"%s\",\"%s\",%d,%d,%d,%d,%d,%d,%.2f,%7.4f,%7.4f,%7.4f,\"%s\",\"%s\",\"%s\"\n",
					...
					item.m_bNewUserFriendly ? 1 : 0, item.m_bRegistered ? 1 : 0, item.m_bValve ? 1 : 0,
					item.m_fRecentMatchPenalty, item.userScore, item.serverScore, item.serverScore + item.userScore,
					item.server.m_steamID.Render(), item.server.m_szMap, item.server.m_szGameTags);
			}
		}
		...
		if(iNumServersToScore <= 0)
		{
			...
			SendStats(m_mapServers.Count() > 0 ? TF_Gamestats_QuickPlay_t::k_Result_NoServersMetCrtieria
	...
	void PingServer()
	{
		...
		m_hServerQueryRequest = steamapicontext->SteamMatchmakingServers()->PingServer(
			m_adrCurrentPing.GetIPHostByteOrder(), m_adrCurrentPing.GetPort(), this);
		Assert(m_hServerQueryRequest != HSERVERQUERY_INVALID);
...
public:
	CStandaloneQuickplayMenu(bool bFeelingLucky, const QuickplaySearchOptions &opt) : BaseClass(bFeelingLucky, opt) {}
...
void CQuickplayPanelBase::ShowItemByIndex(int iItem)
{
	...
	if(m_pGameModeInfoContainer)
	{
		...
		vgui::ImagePanel *pImage =
			dynamic_cast<vgui::ImagePanel *>(m_pGameModeInfoContainer->FindChildByName("ModeImage"));
		if(pImage)
		{
...
void CQuickplayPanelBase::AddItem(EGameCategory gameType, const char *pTitle, const char *pDescription,
								  const char *pMoreInfo, const char *pComplexity, const char *pImage,
...
void CQuickplayPanelBase::ApplySettings(KeyValues *pInResourceData)
{
	...
	V_strcpy_safe(m_szCommunityUpdateImage,
				  pInResourceData->GetString("community_update_image", "illustrations/gamemode_cp"));
}
...
void CQuickplayPanelBase::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pSimplifiedOptionsContainer =
		dynamic_cast<vgui::EditablePanel *>(FindChildByName("SimplifiedOptionsContainer", true));
	Assert(m_pSimplifiedOptionsContainer);
	if(m_pSimplifiedOptionsContainer)
	{
		m_pGameModeInfoContainer = dynamic_cast<vgui::EditablePanel *>(
			m_pSimplifiedOptionsContainer->FindChildByName("ModeInfoContainer", true));
		Assert(m_pGameModeInfoContainer);
	}
	if(m_pGameModeInfoContainer)
	{
		m_pMoreInfoContainer =
			dynamic_cast<vgui::EditablePanel *>(m_pGameModeInfoContainer->FindChildByName("MoreInfoContainer", true));
	...
	if(m_pAdvOptionsContainer)
	{
		...
		{
			m_pGameModeCombo =
				dynamic_cast<vgui::ComboBox *>(pGameModeOptionContainer->FindChildByName("OptionCombo", true));
		...
		if(m_pContainer)
		{
			m_pOptionsSummaryLabel =
				dynamic_cast<vgui::Label *>(m_pContainer->FindChildByName("OptionsSummaryLabel", true));
			Assert(m_pOptionsSummaryLabel);
		}
	}

	// m_pFavoritesCheckButton = dynamic_cast< vgui::CheckButton *>( m_pContainer->FindChildByName(
	// "FavoritesCheckButton" ) ); m_pFavoritesCheckButton->AddActionSignalTarget( this );
	//
	...
	if(bHalloween)
	{
		AddItem(kGameCategory_Event247, "#Gametype_Halloween247", "#TF_GameModeDesc_Halloween247",
				"#TF_GameModeDetail_Halloween247", "#TF_Quickplay_Complexity1", m_szEvent247Image, NULL);
		AddItem(kGameCategory_EventMix, "#Gametype_HalloweenMix", "#TF_GameModeDesc_HalloweenMix",
				"#TF_GameModeDetail_HalloweenMix", "#TF_Quickplay_Complexity1", "illustrations/gamemode_halloween",
				NULL);
	}
	//	AddItem( kGameType_Community_Update,"#GameType_Community_Update",	"#TF_GameModeDesc_Community_Update",
	//"#TF_GameModeDetail_Community_Update",	"#TF_Quickplay_Complexity1",	m_szCommunityUpdateImage,						NULL
	//); 	AddItem( kGameType_Featured,		"#GameType_Featured",			"#TF_GameModeDesc_Featured",
	//"#TF_GameModeDetail_Featured",			"#TF_Quickplay_Complexity1",	"illustrations/gamemode_operation_tough_break",
	//NULL );
	AddItem(kGameCategory_Escort, "#Gametype_Escort", "#TF_GameModeDesc_Escort", "#TF_GameModeDetail_Escort",
			"#TF_Quickplay_Complexity1", "illustrations/gamemode_payload", "illustrations/gamemode_payload_beta");
	AddItem(kGameCategory_Koth, "#Gametype_Koth", "#TF_GameModeDesc_Koth", "#TF_GameModeDetail_Koth",
			"#TF_Quickplay_Complexity1", "illustrations/gamemode_koth", NULL);
	AddItem(kGameCategory_AttackDefense, "#Gametype_AttackDefense", "#TF_GameModeDesc_AttackDefense",
			"#TF_GameModeDetail_AttackDefense", "#TF_Quickplay_Complexity1", "illustrations/gamemode_attackdefend",
			NULL);
	AddItem(kGameCategory_EscortRace, "#Gametype_EscortRace", "#TF_GameModeDesc_EscortRace",
			"#TF_GameModeDetail_EscortRace", "#TF_Quickplay_Complexity2", "illustrations/gamemode_payloadrace", NULL);
	AddItem(kGameCategory_CP, "#Gametype_CP", "#TF_GameModeDesc_CP", "#TF_GameModeDetail_CP",
			"#TF_Quickplay_Complexity2", "illustrations/gamemode_cp", NULL);
	AddItem(kGameCategory_CTF, "#Gametype_CTF", "#TF_GameModeDesc_CTF", "#TF_GameModeDetail_CTF",
			"#TF_Quickplay_Complexity2", "illustrations/gamemode_ctf", NULL);
	AddItem(kGameCategory_Misc, "#Gametype_Misc", "#TF_GameModeDesc_Misc", "#TF_GameModeDetail_Misc",
			"#TF_Quickplay_Complexity2", "illustrations/gamemode_sd", NULL);
	AddItem(kGameCategory_Powerup, "#GameType_Powerup", "#TF_GameModeDesc_Powerup", "#TF_GameModeDetail_Powerup",
			"#TF_Quickplay_Complexity3", "illustrations/gamemode_powerup",
			"illustrations/gamemode_powerup_beta"); // Fix beta image once Heather has the image
	AddItem(kGameCategory_Passtime, "#GameType_Passtime", "#TF_GameModeDesc_Passtime", "#TF_GameModeDetail_Passtime",
			"#TF_Quickplay_Complexity2", "illustrations/gamemode_passtime", "illustrations/gamemode_passtime_beta");
	AddItem(kGameCategory_RobotDestruction, "#Gametype_RobotDestruction", "#TF_GameModeDesc_RobotDestruction",
			"#TF_GameModeDetail_RobotDestruction", "#TF_Quickplay_Complexity2", "illustrations/gamemode_sd",
			"illustrations/gamemode_robotdestruction_beta");
	if(m_bShowRandomOption)
		AddItem(kGameCategory_Quickplay, "#Gametype_Quickplay", "#TF_GameModeDesc_Quickplay",
				"#TF_GameModeDetail_Quickplay", "#TF_Quickplay_Complexity2", "illustrations/quickplay",
				"illustrations/quickplay_beta");

	// AddItem( kQuickplayGameType_Arena, 		"#Gametype_Arena", 			"#TF_GameModeDesc_Arena",
	// "#TF_GameModeDetail_Arena", "#TF_Quickplay_Complexity3", "maps/menu_photos_cp_granary" ); AddItem(
	// kQuickplayGameType_Specialty, 	"#Gametype_Specialty", 		"#TF_GameModeDesc_Specialty",
	// "#TF_GameModeDetail_Specialty", "#TF_Quickplay_Complexity3", "maps/menu_photos_cp_granary" );
...
void CQuickplayPanelBase::SetupMoreOptions()
{
	...
	FOR_EACH_VEC(m_vecAdvOptions, idxAdvOpt)
	{
		...
		FOR_EACH_VEC(pOpt->m_vecOptionNames, idx)
		{
			...
			{
				pRadioButton =
					dynamic_cast<vgui::RadioButton *>(pContainer->FindChildByName(VarArgs("RadioButton%d", idx)));
				Assert(pRadioButton);
			}
			if(pRadioButton)
			{
...
public:
	CQuickplayDialog(vgui::Panel *parent) : CQuickplayPanelBase(parent, "QuickPlayDialog")
	{
	...
	virtual void OnCommand(const char *pCommand)
	{
		...
		if(FStrEq(pCommand, "playnow") || FStrEq(pCommand, "show_servers"))
		{
			...
			opt.m_eServers =
				(QuickplaySearchOptions::EServers)(bBetaContent ?
#ifdef STAGING_ONLY
																2
																:
#else
			...
			opt.m_eRandomCrits = (QuickplaySearchOptions::ERandomCrits)(
				bBetaContent ? 2 : tf_quickplay_pref_disable_random_crits.GetInt());
			opt.m_eDamageSpread = (QuickplaySearchOptions::EDamageSpread)(
				bBetaContent ? 2 : tf_quickplay_pref_enable_damage_spread.GetInt());
			opt.m_eRespawnTimes =
				(QuickplaySearchOptions::ERespawnTimes)(bBetaContent ? 2 : tf_quickplay_pref_respawn_times.GetInt());
			opt.m_eMaxPlayers = (QuickplaySearchOptions::EMaxPlayers)(
				bBetaContent ? 2 : tf_quickplay_pref_increased_maxplayers.GetInt());
	...
	virtual void OnKeyCodePressed(vgui::KeyCode code)
	{
		...
		else if(nButtonCode == KEY_XBUTTON_X || nButtonCode == STEAMCONTROLLER_X)
		{
			OnCommand("more_info");
				...
				nButtonCode == STEAMCONTROLLER_DPAD_LEFT || code == KEY_LEFT)
		{
			OnCommand("prevpage");
		}
		else if(nButtonCode == KEY_XBUTTON_RIGHT || nButtonCode == KEY_XSTICK1_RIGHT ||
				nButtonCode == KEY_XSTICK2_RIGHT || nButtonCode == STEAMCONTROLLER_DPAD_RIGHT || code == KEY_RIGHT)
		{
			OnCommand("nextpage");
		}
		else
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		{
			Warning(" Received CGCTFQuickplay_ScoreServers_Response, but no quick play query in progress dialog to "
					"receive them?\n");
		}
...
GC_REG_JOB(GCSDK::CGCClient, CGCTFQuickplay_ScoreServers_Response, "CGCTFQuickplay_ScoreServers_Response",
		   k_EMsgGC_QP_ScoreServersResponse, GCSDK::k_EServerTypeGCClient);
...
ConVar tf_quickplay_beta_preference(
	"tf_quickplay_beta_preference", "-1", FCVAR_NONE,
	"Preference to participate in beta quickplay: -1 = no preference, 0 = opt out, 1 = opt in");
ConVar tf_quickplay_beta_ask_percentage("tf_quickplay_beta_ask_percentage", "0", FCVAR_NONE,
										"Percentage of people who will be prompted to participate in beta quickplay.");
...
// Check for opting into beta quickplay
	...
	if(args.ArgC() < 2 && tf_quickplay_beta_preference.GetInt() != 0)
	{
		...
		if(steamapicontext && steamapicontext->SteamUser() &&
		   (steamapicontext->SteamUser()->GetSteamID().GetAccountID() % 100U) <
			   (uint32)tf_quickplay_beta_ask_percentage.GetInt())
		{
			ShowConfirmDialog("#TF_QuickplayBeta_OptIn_Title", "#TF_QuickplayBeta_OptIn_Message",
							  "#TF_QuickplayBeta_OptIn_YesButton", "#TF_QuickplayBeta_OptIn_NoButton",
							  QuickplayBetaConfirmCallback);
			return;
		}
	}
#endif
...
static ConCommand OpenQuickplayWaitDialogForMap("OpenQuickplayWaitDialogForMap", &CL_OpenQuickplayDialogForMap,
												"Instantly starts a quickplay query for a map.");