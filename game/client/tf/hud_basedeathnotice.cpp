...
void CHudBaseDeathNotice::Paint()
{
	...
	for(int i = 0; i < iCount; i++)
	{
		...
		int iDeathInfoTextWide =
			msg.wzInfoText[0] ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzInfoText) + xSpacing : 0;
		int iDeathInfoEndTextWide =
			msg.wzInfoTextEnd[0] ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzInfoTextEnd) + xSpacing : 0;
		...
		int iPreKillerTextWide =
			msg.wzPreKillerText[0] ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzPreKillerText) - xSpacing : 0;
		...
		int iTotalWide =
			iKillerTextWide + iconWide + iVictimTextWide + iDeathInfoTextWide + iDeathInfoEndTextWide + (xMargin * 2);
		...
		// Draw victims name
		DrawText(x + iVictimTextOffset, yText, m_hTextFont, GetTeamColor(msg.Victim.iTeam, msg.bLocalPlayerInvolved),
				 victim);
		x += iVictimTextWide;
...
void CHudBaseDeathNotice::RetireExpiredDeathNotices()
{
	...
	if(m_DeathNotices.Count() > 0 && m_DeathNotices.Count() > (int)m_flMaxDeathNotices)
	{
		...
		// new non-local-player-involved messages would not appear if the queue was full of messages involving the local
		// player
...
void CHudBaseDeathNotice::FireGameEvent(IGameEvent *event)
{
	...
	if(bPlayerDeath)
	{
		...
		if(bIsFeignDeath)
		{
			...
			if(pVictim && pLocalPlayer && !BAreTeamsEnemies(pLocalPlayer->GetTeamNumber(), pVictim->GetTeamNumber()))
			{
				return;
			}
	...
	if(bPlayerDeath || bObjectDeath)
	{
		...
		if(event->GetInt("death_flags") & TF_DEATH_AUSTRALIUM)
		{
			...
			m_DeathNotices[iMsg].iconCritDeath =
				GetIcon("d_australium", bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted : kDeathNoticeIcon_Standard);
		}
		else if(event->GetInt("damagebits") & DMG_CRITICAL)
		{
			m_DeathNotices[iMsg].bCrit = true;
			m_DeathNotices[iMsg].iconCritDeath =
				GetIcon("d_crit", bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted : kDeathNoticeIcon_Standard);
		}
		else
		{
		...
		if(!killer || killer == victim)
		{
			...
			else if(event->GetInt("damagebits") & DMG_FALL)
			{
				...
				V_wcsncpy(m_DeathNotices[iMsg].wzInfoText, g_pVGuiLocalize->Find("#DeathMsg_Fall"),
						  sizeof(m_DeathNotices[iMsg].wzInfoText));
			}
			else if((event->GetInt("damagebits") & DMG_VEHICLE) ||
					(0 == Q_stricmp(m_DeathNotices[iMsg].szIcon, "d_tracktrain")))
			{
		...
		{
			Q_snprintf(sDeathMsg, sizeof(sDeathMsg), "%s killed %s", m_DeathNotices[iMsg].Killer.szName,
					   m_DeathNotices[iMsg].Victim.szName);
			...
			{
				Q_strncat(sDeathMsg, VarArgs(" with %s.", &m_DeathNotices[iMsg].szIcon[2]), sizeof(sDeathMsg),
						  COPY_ALL_CHARACTERS);
			}
		}
	...
	{
		GetLocalizedControlPointName(event, m_DeathNotices[iMsg].Victim.szName,
									 ARRAYSIZE(m_DeathNotices[iMsg].Victim.szName));
		...
		V_wcsncpy(m_DeathNotices[iMsg].wzInfoText,
				  g_pVGuiLocalize->Find(len > 1 ? "#Msg_Captured_Multiple" : "#Msg_Captured"),
				  sizeof(m_DeathNotices[iMsg].wzInfoText));

		// print a log message
		Msg("%s captured %s for team #%d\n", m_DeathNotices[iMsg].Killer.szName, m_DeathNotices[iMsg].Victim.szName,
			m_DeathNotices[iMsg].Killer.iTeam);
	}
	else if(FStrEq("teamplay_capture_blocked", pszEventName))
	{
		GetLocalizedControlPointName(event, m_DeathNotices[iMsg].Victim.szName,
									 ARRAYSIZE(m_DeathNotices[iMsg].Victim.szName));
		V_wcsncpy(m_DeathNotices[iMsg].wzInfoText, g_pVGuiLocalize->Find("#Msg_Defended"),
				  sizeof(m_DeathNotices[iMsg].wzInfoText));
	...
	if(!bSpecialScore && !bTeamLeaderKilled)
	{
		if(!m_DeathNotices[iMsg].iconDeath && m_DeathNotices[iMsg].szIcon)
		{
			...
			m_DeathNotices[iMsg].iconDeath =
				GetIcon(m_DeathNotices[iMsg].szIcon, bInverted ? kDeathNoticeIcon_Inverted : kDeathNoticeIcon_Standard);
			if(!m_DeathNotices[iMsg].iconDeath)
			{
				// Can't find it, so use the default skull & crossbones icon
				m_DeathNotices[iMsg].iconDeath =
					GetIcon("d_skull_tf", m_DeathNotices[iMsg].bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted
...
void CHudBaseDeathNotice::GetBackgroundPolygonVerts(int x0, int y0, int x1, int y1, int iVerts, vgui::Vertex_t vert[])
{
	...
	// use the offsets we generated for one corner and apply those to the passed-in dimensions to create verts for the
	// poly
