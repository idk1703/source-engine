...
extern void ShowWaitingDialog(CGenericWaitingDialog *pWaitingDialog, const char *pUpdateText, bool bAnimate,
							  bool bShowCancel, float flMaxDuration);
...
{
public:
	CUseItemConfirmContext(CEconItemView *pEconItemView, EServerPlayersGCSend eSendServerPlayers,
						   const char *pszConfirmUseSound = NULL)
...
void CEconTool_WeddingRing::OnClientUseConsumable(CEconItemView *pItem, vgui::Panel *pParent) const
{
	...
	CTFGenericConfirmDialog *pDialog =
		ShowConfirmDialog("#TF_UseWeddingRing_Title", "#TF_UseWeddingRing_Text", "#TF_WeddingRing_AcceptProposal",
						  "#TF_WeddingRing_RejectProposal", &UseItemConfirm);

	pDialog->AddStringToken("item_name", pItem->GetItemName());

	CUtlConstWideString sProposerPersonaName;
	GLocalizationProvider()->ConvertUTF8ToLocchar(TFInventoryManager()->PersonaName_Get(unAccountID),
												  &sProposerPersonaName);
...
void CEconTool_BackpackExpander::OnClientUseConsumable(CEconItemView *pItem, vgui::Panel *pParent) const
{
	...
	if(IsFreeTrialAccount() && unNewNumSlots > MAX_NUM_BACKPACK_SLOTS - (DEFAULT_NUM_BACKPACK_SLOTS -
																		 DEFAULT_NUM_BACKPACK_SLOTS_FREE_TRIAL_ACCOUNT))
	{
		ShowMessageBox("#TF_UseBackpackExpanderFail_Title", "#TF_UseBackpackExpanderFail_Text", "#GameUI_OK");
		return;
	}
	...
	CTFGenericConfirmDialog *pDialog = ShowConfirmDialog(
		"#TF_UseBackpackExpander_Title", "#TF_UseBackpackExpander_Text", "#GameUI_OK", "#Cancel", &UseItemConfirm);
	pDialog->AddStringToken("item_name", pItem->GetItemName());
...
void CEconTool_AccountUpgradeToPremium::OnClientUseConsumable(CEconItemView *pItem, vgui::Panel *pParent) const
{
	...
	{
		ShowMessageBox("#TF_UseAccountUpgradeToPremiumFail_Title", "#TF_UseAccountUpgradeToPremiumFail_Text",
					   "#GameUI_OK");
		return;
	}
	...
	CTFGenericConfirmDialog *pDialog =
		ShowConfirmDialog("#TF_UseAccountUpgradeToPremium_Title", "#TF_UseAccountUpgradeToPremium_Text", "#GameUI_OK",
						  "#Cancel", &UseItemConfirm);
	pDialog->AddStringToken("item_name", pItem->GetItemName());
	wchar_t wszUsesLeft[32];
	_snwprintf(wszUsesLeft, ARRAYSIZE(wszUsesLeft), L"%d", pItem->GetItemQuantity());
	pDialog->AddStringToken("uses_left", wszUsesLeft);
...
void CEconTool_ClaimCode::OnClientUseConsumable(CEconItemView *pItem, vgui::Panel *pParent) const
{
	...
	CTFGenericConfirmDialog *pDialog =
		ShowConfirmDialog("#TF_UseClaimCode_Title", "#TF_UseClaimCode_Text", "#GameUI_OK", "#Cancel", &UseItemConfirm);
	pDialog->AddStringToken("item_name", pItem->GetItemName());
...
static void ClientConsumableTool_Generic(CEconItemView *pItem, vgui::Panel *pParent)
{
	...
	CTFGenericConfirmDialog *pDialog =
		ShowConfirmDialog("#TF_UseItem_Title", "#TF_UseItem_Text", "#GameUI_OK", "#Cancel", &UseItemConfirm);
	pDialog->AddStringToken("item_name", pItem->GetItemName());
	wchar_t wszUsesLeft[32];
	_snwprintf(wszUsesLeft, ARRAYSIZE(wszUsesLeft), L"%d", pItem->GetItemQuantity());
	pDialog->AddStringToken("uses_left", wszUsesLeft);
	pDialog->SetContext(new CUseItemConfirmContext(pItem, pItem->GetItemDefinition()->GetTypedEconTool<CEconTool_Gift>()
															  ? kServerPlayers_Send
															  : kServerPlayers_DontSend));
...
{
	Assert(pItem);
	const CEconTool_GrantOperationPass *pEconToolOperationPass =
		pItem->GetItemDefinition()->GetTypedEconTool<CEconTool_GrantOperationPass>();
	...
	for(int i = 0; i < pLocalInv->GetItemCount(); ++i)
	{
		...
		{
			ShowMessageBox("#TF_UseOperationPassAlreadyActive_Title", "#TF_UseOperationPassAlreadyActive_Text",
						   "#GameUI_OK");
			return;
		}
	}
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		switch(msg.Body().response_code())
		{
			...
			case CMsgGC_Client_UseServerModificationItem_Response::
				kServerModificationItemResponse_NotOnAuthenticatedServer:
				ShowMessageBox("#TF_ServerEnchantmentType", "#TF_Eternaween__AuthenticatedServerRequired",
							   (KeyValues *)NULL);
				break;
...
GC_REG_JOB(GCSDK::CGCClient, CGCEventEnableResponse, "CGCEventEnableResponse",
		   k_EMsgGC_Client_UseServerModificationItem_Response, GCSDK::k_EServerTypeGCClient);
...
void UseGift(CEconItemView *pItem, CSteamID targetID)
{
	...
	C_CTF_GameStats.Event_Trading(IE_TRADING_ITEM_GIFTED, pItem, true, steamID.ConvertToUint64(),
								  targetID.ConvertToUint64());
}
...
CDeliverGiftSelectDialog::CDeliverGiftSelectDialog(vgui::Panel *parent) : CSelectPlayerDialog(parent) {}
...
static void StartUseActionSlotItem(const CCommand &args)
{
	...
	if(TFGameRules() && TFGameRules()->IsUsingGrapplingHook())
	{
		CTFGrapplingHook *pGrapplingHook =
			dynamic_cast<CTFGrapplingHook *>(pLocalPlayer->GetEntityForLoadoutSlot(LOADOUT_POSITION_ACTION));
		if(pGrapplingHook)
		{
...
static ConCommand start_use_action_slot_item("+use_action_slot_item", StartUseActionSlotItem,
											 "Use the item in the action slot.");
...
static void StartContextAction(const CCommand &args)
{
	...
	if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
	{
		...
		if(pLocalPlayer)
		{
			CTFPowerupBottle *pPowerupBottle = dynamic_cast<CTFPowerupBottle *>(
				pLocalPlayer->GetEquippedWearableForLoadoutSlot(LOADOUT_POSITION_ACTION));
			...
			if(pLocalPlayer->IsPlayerClass(TF_CLASS_HEAVYWEAPONS) && pLocalPlayer->GetActiveTFWeapon() &&
			   pLocalPlayer->GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_MINIGUN)
			{
				int iRage = 0;
...
{
public:
	CTFGiftNotification(GCSDK::CProtoBufMsg<CMsgGCGiftedItems> &msg) : CEconNotification()
	{
		...
		else
		{
			...
			{
				const CSteamID recipientSteamID(msg.Body().recipient_account_ids(i), eUniverse,
	...
	void PrintToChatLog()
	{
		...
		if(pHUDChat)
		{
			...
			FOR_EACH_VEC(m_vecSteamIDRecipients, i)
			{
				...
				g_pVGuiLocalize->ConstructString_safe(wszNotification, pFormat, 2, m_wszPlayerName, wszRecipientName);
...
{
public:
	CTFUseItemNotification(EGCMsgUseItemResponse eResponse) : CEconNotification()
	{
		switch(eResponse)
		{
...
GC_REG_JOB(GCSDK::CGCClient, CGCUseItemResponse, "CGCUseItemResponse", k_EMsgGCUseItemResponse,
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		char szGifterName[MAX_PLAYER_NAME_LENGTH];
		szGifterName[0] = '\0';
		GetPlayerNameBySteamID(CSteamID(msg.Body().gifter_steam_id(), GetUniverse(), k_EAccountTypeIndividual),
							   szGifterName, sizeof(szGifterName));
		if(szGifterName[0] == '\0')
		{
			return true;
		}
...
GC_REG_JOB(GCSDK::CGCClient, CGCUsedClaimCodeItem, "CGCUsedClaimCodeItem", k_EMsgGCUsedClaimCodeItem,
		   GCSDK::k_EServerTypeGCClient);
...
{
	return gDuelMiniGameLocalData.m_steamIDOpponent != CSteamID() ? gDuelMiniGameLocalData.m_iRequiredPlayerClass
																  : TF_CLASS_UNDEFINED;
}
...
{
public:
	CTFDuelRequestNotification(const char *pInitiatorName, const CSteamID &steamIDInitiator,
							   const CSteamID &steamIDTarget, const int iRequiredPlayerClass)
		: CEconNotification(),
	...
	{
		CTFGenericConfirmDialog *pDialog =
			ShowConfirmDialog("#TF_Duel_Title", "#TF_Duel_Request", "#GameUI_OK", "#TF_Duel_JoinCancel", &ConfirmDuel);
	...
	virtual void FireGameEvent(IGameEvent *event)
	{
		...
		else if(Q_strcmp("duel_status", pEventName) == 0)
		{
			...
			if(pLocalPlayer->entindex() == iInitiatorID || pLocalPlayer->entindex() == iTargetID)
			{
				...
				pNotification->SetText(iScoreType == kDuelScoreType_Kill ? "TF_Duel_StatusKill"
																		 : "TF_Duel_StatusAssist");
			...
			// print to chat log
			PrintTextToChat(iScoreType == kDuelScoreType_Kill ? "TF_Duel_StatusForChat_Kill"
															  : "TF_Duel_StatusForChat_Assist",
							pKeyValues);
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		g_pVGuiLocalize->ConvertANSIToUnicode(szPlayerName_Initiator, wszPlayerName_Initiator,
											  sizeof(wszPlayerName_Initiator));
		...
		{
			CTFDuelRequestNotification *pNotification = new CTFDuelRequestNotification(
				szPlayerName_Initiator, steamIDInitiator, steamIDTarget, msg.Body().m_usAsPlayerClass);
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		bool bIsClassDuel = msg.Body().m_usAsPlayerClass >= TF_FIRST_NORMAL_CLASS &&
							msg.Body().m_usAsPlayerClass < TF_LAST_NORMAL_CLASS;

		// add notification
		const char *pText = "TF_Duel_Accept";
		const char *pSoundFilename =
			bIsClassDuel ? "ui/duel_challenge_accepted_with_restriction.wav" : "ui/duel_challenge_accepted.wav";
		if(msg.Body().m_bAccepted == false)
		{
			const char *kDeclineStrings[] = {"TF_Duel_Decline", "TF_Duel_Decline2", "TF_Duel_Decline3"};
			pText = kDeclineStrings[RandomInt(0, ARRAYSIZE(kDeclineStrings) - 1)];
			pSoundFilename =
				bIsClassDuel ? "ui/duel_challenge_rejected_with_restriction.wav" : "ui/duel_challenge_rejected.wav";
		}
...
GC_REG_JOB(GCSDK::CGCClient, CGCClient_Duel_Response, "CGCClient_Duel_Response", k_EMsgGC_Duel_Response,
		   GCSDK::k_EServerTypeGCClient);
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		if(bTie == false)
		{
			...
			pKeyValues->SetWString("loser",
								   steamIDWinner == steamIDInitiator ? wszPlayerName_Target : wszPlayerName_Initiator);
			...
			pKeyValues->SetWString("winner_score",
								   steamIDWinner == steamIDInitiator ? wszScoreInitiator : wszScoreTarget);
			pKeyValues->SetWString("loser_score",
		...
		if(localSteamID == steamIDInitiator || localSteamID == steamIDTarget)
		{
			...
			pNotification->SetText(pText);
			pNotification->SetKeyValues(pKeyValues);
			pNotification->SetSteamID(
				bTie == false ? steamIDWinner : (localSteamID == steamIDInitiator ? steamIDTarget : steamIDInitiator));
			pNotification->SetSoundFilename("ui/duel_event.wav");
			NotificationQueue_Add(pNotification);
...
void CSelectPlayerForDuelDialog::OnShowClassIconMouseover(KeyValues *data)
{
	if(m_pClassIconMouseoverLabel)
	{
		...
		if(iClass >= TF_FIRST_NORMAL_CLASS && iClass < TF_LAST_NORMAL_CLASS)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wzLocalized, g_pVGuiLocalize->Find(pszLocString), 1,
...
void CSelectPlayerForDuelDialog::SetSelectedClass(int iClass)
{
	...
	g_pVGuiLocalize->ConstructString_safe(wszText, g_pVGuiLocalize->Find("#TF_SelectPlayer_DuelClass"), 1,
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		if(pHUDChat)
		{
			...
			const GameItemDefinition_t *pItemDefinition =
				dynamic_cast<GameItemDefinition_t *>(GetItemSchema()->GetItemDefinition(nDefIndex));
			if(!pItemDefinition)
				return true;
			...
			g_pVGuiLocalize->ConvertANSIToUnicode(msg.Body().item_name_custom().c_str(), wszCustomName,
												  sizeof(wszCustomName));
			...
			g_pVGuiLocalize->ConstructString_safe(
				wszLocalizedString, wszItemRenamed, 3, wszPlayerName,
				CEconItemLocalizedFullNameGenerator(GLocalizationProvider(), pItemDefinition, iItemQuality)
					.GetFullName(),
				wszCustomName);
...
GC_REG_JOB(GCSDK::CGCClient, CTFNameItemNotification, "CTFNameItemNotification", k_EMsgGCNameItemNotification,
		   GCSDK::k_EServerTypeGCClient);
...
{
public:
	CClientDisplayNotification(GCSDK::IMsgNetPacket *pNetPacket) : m_msg(pNetPacket)
	{
...
GC_REG_JOB(GCSDK::CGCClient, CTFClientDisplayNotification, "CTFClientDisplayNotification",
