...
#ifdef STAGING_ONLY
ConVar cl_mvm_log_playerstats("cl_mvm_log_playerstats", "0", FCVAR_ARCHIVE,
							  "Internal dump of verbose player stats from mvm missions.");
#endif // STAGING_ONLY
...
void CTFHudMannVsMachineScoreboard::ApplySchemeSettings(IScheme *pScheme)
{
	...
	{
		m_pPreviousWaveCreditsInfo =
			dynamic_cast<CCreditDisplayPanel *>(m_pCreditStatsPanel->FindChildByName("PreviousWaveCreditInfoPanel"));
		m_pTotalCreditsInfo =
			dynamic_cast<CCreditDisplayPanel *>(m_pCreditStatsPanel->FindChildByName("TotalGameCreditInfoPanel"));
		m_pPreviousWaveCreditsSpend =
			dynamic_cast<CCreditSpendPanel *>(m_pCreditStatsPanel->FindChildByName("PreviousWaveCreditSpendPanel"));
		m_pTotalCreditsSpend =
			dynamic_cast<CCreditSpendPanel *>(m_pCreditStatsPanel->FindChildByName("TotalGameCreditSpendPanel"));
	...
	m_bInitialized = true;
	m_popfile[0] =
		'\0'; // clear the popfile name so everything will be reset again in the UpdatePopFile() call in OnTick()
	OnTick();
}
...
void CTFHudMannVsMachineScoreboard::InitPlayerList(IScheme *pScheme)
{
	...
	m_pPlayerList->AddColumnToSection(
		0, "class", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_CENTER, m_iClassWidth);
	m_pPlayerList->AddColumnToSection(0, "tour_no", "#TF_MvMScoreboard_Tour", SectionedListPanel::COLUMN_RIGHT,
									  m_iClassWidth);
	m_pPlayerList->AddColumnToSection(0, "score", "#TF_Scoreboard_Score", SectionedListPanel::COLUMN_RIGHT,
									  m_iStatWidth);
	m_pPlayerList->AddColumnToSection(0, "damage", "#TF_MvMScoreboard_Damage", SectionedListPanel::COLUMN_RIGHT,
									  m_iStatWidth);
	m_pPlayerList->AddColumnToSection(0, "tank", "#TF_MvMScoreboard_Tank", SectionedListPanel::COLUMN_RIGHT,
									  m_iStatWidth);
	m_pPlayerList->AddColumnToSection(0, "healing", "#TF_MvMScoreboard_Healing", SectionedListPanel::COLUMN_RIGHT,
									  m_iStatWidth);
	m_pPlayerList->AddColumnToSection(0, "support", "#TF_MvMScoreboard_Support", SectionedListPanel::COLUMN_RIGHT,
									  m_iStatWidth);
	...
	m_pPlayerList->AddColumnToSection(0, "credits", "#TF_MvMScoreboard_Money", SectionedListPanel::COLUMN_RIGHT,
									  m_iStatWidth);
	m_pPlayerList->AddColumnToSection(
		0, "squad_surplus", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_CENTER, m_iStatWidth);
...
char *ConvertScoreboardValueToString(int iValue)
{
	...
	if(iValue >= 1000000)
	{
		V_sprintf_safe(szConversion, "%d%s%d%d%d%s%d%d%d", iValue / 1000000, ",", (iValue % 1000000) / 100000,
					   (iValue % 100000) / 10000, (iValue % 10000) / 1000, ",", (iValue % 1000) / 100,
					   (iValue % 100) / 10, iValue % 10);
	}
	else if(iValue >= 1000)
	{
		V_sprintf_safe(szConversion, "%d%s%d%d%d", iValue / 1000, ",", (iValue % 1000) / 100, (iValue % 100) / 10,
					   iValue % 10);
	}
	else if(iValue >= 0)
	{
...
void CTFHudMannVsMachineScoreboard::UpdatePlayerList()
{
	...
	for(int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++)
	{
		...
		if(g_PR->IsConnected(playerIndex))
		{
			...
			{
				// If this is local player and he is dead, show desired class (which he will spawn as) rather than
				// current class.
				C_TFPlayer *pPlayer = C_TFPlayer::GetLocalTFPlayer();
				int iDesiredClass = pPlayer->m_Shared.GetDesiredPlayerClassIndex();
				// use desired class unless it's random -- if random, his future class is not decided until moment of
				// spawn
		...
		// check for bots first, so malicious server operators can't fake a ping and stuff their server with bots that
		// look like players
		...
		Color fgClr;
		Color bgClr;
		// change color based off alive or dead status. Also slightly different if its local player since the name is
		// highlighted.
		if(bAlive)
		{
		...
		if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
		{
			...
			// ) ) ); pKeyValues->SetString( "bonus", ConvertScoreboardValueToString( g_TF_PR->GetBonusPoints(
			// playerIndex ) ) );
			pKeyValues->SetString("credits",
								  ConvertScoreboardValueToString(g_TF_PR->GetCurrencyCollected(playerIndex)));
		}
...
void CTFHudMannVsMachineScoreboard::UpdateCreditStats()
{
	...
	g_pVGuiLocalize->ConstructString_safe(wszLocalizedWave, g_pVGuiLocalize->Find("#TF_PVE_WaveCount"), 1,
										  wszWaveNumber);
	...
	if((TFObjectiveResource()->GetMannVsMachineIsBetweenWaves() == true) &&
	   (TFObjectiveResource()->GetMannVsMachineWaveCount() > 1))
	{
	...
	if(m_pRespecStatusLabel)
	{
		...
		bool bShowRespec =
			TFGameRules() && TFGameRules()->IsMannVsMachineRespecEnabled() && tf_mvm_respec_limit.GetInt();
		...
		if(bShowRespec)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wzRespecProg, g_pVGuiLocalize->Find("#TF_PVE_RespecsProgress"), 2,
												  wzCredits, wzGoal);
...
//-----------------------------------------------------------------------------
void CTFHudMannVsMachineScoreboard::UpdateCreditPanel(CCreditDisplayPanel *panel, int nAcquired, int nMissed,
													  int nBonus)
{
...
//-----------------------------------------------------------------------------
void CTFHudMannVsMachineScoreboard::UpdateCreditSpend(CCreditSpendPanel *panel, int nUpgrades, int nBuybacks,
													  int nBottles)
{
	if(panel)
	{
		...
		if(Q_strcmp(m_popfile, TFObjectiveResource()->GetMvMPopFileName()) != 0)
		{
			...
			if(GetItemSchema()->GetMvmMissions().IsValidIndex(iChallengeIndex))
			{
				...
				g_pVGuiLocalize->ConstructString_safe(wszChallengeName, L"%s1 (%s2)", 2,
													  g_pVGuiLocalize->Find(mission.m_sDisplayName.Get()),
													  g_pVGuiLocalize->Find(mission.m_sMode.Get()));

				SetDialogVariable("popfile", wszChallengeName);

				m_pDifficultyContainer->SetVisible(true);
				m_pDifficultyContainer->SetDialogVariable(
					"difficultyvalue", g_pVGuiLocalize->Find(GetMvMChallengeDifficultyLocName(mission.m_eDifficulty)));
			}
			else
			{
...
void CTFHudMannVsMachineScoreboard::WritePlayerScoreStats(void)
{
	if(g_TF_PR)
	{
		...
		if(hFile != FILESYSTEM_INVALID_HANDLE)
		{
			...
			buf.Printf("%s - Wave %d - %s", TFObjectiveResource()->GetMvMPopFileName(),
					   TFObjectiveResource()->GetMannVsMachineWaveCount(), asctime(&newtime));
			buf.Printf("----\n");
			...
			for(int i = 1; i <= MAX_PLAYERS; i++)
			{
				...
				buf.Printf("%10.10s %8.8s\tScore: %-6d Damage: %-6d DmgAssist: %-6d Tank: %-6d Healing: %-6d "
						   "HealAssist: %-6d Blocked: %-6d Money: %-6d Bonus: %-6d\n",
						   pPlayer->GetPlayerName(), pszClassName ? pszClassName : "Unknown", g_TF_PR->GetTotalScore(i),
						   g_TF_PR->GetDamage(i), g_TF_PR->GetDamageAssist(i), g_TF_PR->GetDamageBoss(i),
						   g_TF_PR->GetHealing(i), g_TF_PR->GetHealingAssist(i), g_TF_PR->GetDamageBlocked(i),
