...
const char *szLocalizedObjectNames[OBJ_LAST] = {"#TF_Object_Dispenser", "#TF_Object_Tele", "#TF_Object_Sentry",
												"#TF_object_Sapper"};

ConVar cl_hud_killstreak_display_time(
	"cl_hud_killstreak_display_time", "3", FCVAR_ARCHIVE,
	"How long a killstreak notice stays on the screen (in seconds).  Range is from 0 to 100.");
ConVar cl_hud_killstreak_display_fontsize(
	"cl_hud_killstreak_display_fontsize", "0", FCVAR_ARCHIVE,
	"Adjusts font size of killstreak notices.  Range is from 0 to 2 (default is 1).");
ConVar cl_hud_killstreak_display_alpha(
	"cl_hud_killstreak_display_alpha", "120", FCVAR_ARCHIVE,
	"Adjusts font alpha value of killstreak notices.  Range is from 0 to 255 (default is 200).");
...
void CTFStreakNotice::Paint(void)
{
	...
	C_TFPlayer *pPlayer = CTFPlayer::GetLocalTFPlayer();
	CHudTexture *pIcon = (m_nCurrStreakType == CTFPlayerShared::kTFStreak_Ducks ||
						  m_nCurrStreakType == CTFPlayerShared::kTFStreak_Duck_levelup)
							 ? m_iconDuckStreak
							 : m_iconKillStreak;
	if(pPlayer && pIcon)
	{
		...
		pIcon->DrawSelf(XRES(320) - (iWide / 2) + iTextWide, m_nLabelYPos + nYOffset, iTall, iTall,
						Color(235, 226, 202, GetAlpha()));
	}
...
void CTFStreakNotice::StreakEnded(CTFPlayerShared::ETFStreak eStreakType, int iKillerID, int iVictimID, int iStreak)
{
	...
	{
		wzMsg = g_pVGuiLocalize->Find((eStreakType == CTFPlayerShared::kTFStreak_Ducks) ? "#Msg_DuckStreakEndSelf"
																						: "#Msg_KillStreakEndSelf");
		bSelfKill = true;
	}
	else
	{
		wzMsg = g_pVGuiLocalize->Find((eStreakType == CTFPlayerShared::kTFStreak_Ducks) ? "#Msg_DuckStreakEnd"
																						: "#Msg_KillStreakEnd");
	}
	...
	while(txt && *txt)
	{
		switch(*txt)
		{
			case 0x01: // Normal color
				m_pLabel->GetTextImage()->AddColorChange(Color(235, 226, 202, cl_hud_killstreak_display_alpha.GetInt()),
														 iWChars);
				break;
			case 0x02: // Team color
				m_pLabel->GetTextImage()->AddColorChange(Color(cKillerColor.r(), cKillerColor.g(), cKillerColor.b(),
															   cl_hud_killstreak_display_alpha.GetInt()),
														 iWChars);
				break;
			case 0x03: // Item 2 color
				m_pLabel->GetTextImage()->AddColorChange(Color(cVictimColor.r(), cVictimColor.g(), cVictimColor.b(),
															   cl_hud_killstreak_display_alpha.GetInt()),
														 iWChars);
				break;
			default:
...
//-----------------------------------------------------------------------------
void CTFStreakNotice::StreakUpdated(CTFPlayerShared::ETFStreak eStreakType, int iKillerID, int iStreak,
									int iStreakIncrement)
{
	// Temp override all messages
	// Add New message
	...
	if(eStreakType == CTFPlayerShared::kTFStreak_Ducks)
	{
		// Notices at 15, 30, then increments of 50. We may increment by multiple ducks per kill, so check if we passed
		// over a milestone.
	...
	while(txt && *txt)
	{
		switch(*txt)
		{
			case 0x01: // Normal color
				...
				break;
			case 0x02: // Team color
				m_pLabel->GetTextImage()->AddColorChange(
					Color(cTeamColor.r(), cTeamColor.g(), cTeamColor.b(), cl_hud_killstreak_display_alpha.GetInt()),
					iWChars);
				break;
			case 0x03: // Item 2 color
				m_pLabel->GetTextImage()->AddColorChange(Color(cCustomColor.r(), cCustomColor.g(), cCustomColor.b(),
															   cl_hud_killstreak_display_alpha.GetInt()),
														 iWChars);
				break;
			default:
...
private:
	...
	void AddStreakMsg(CTFPlayerShared::ETFStreak eStreakType, int iKillerID, int iKillerStreak, int iStreakIncrement,
					  int iVictimID, int iDeathNoticeMsg);
	void AddStreakEndedMsg(CTFPlayerShared::ETFStreak eStreakType, int iKillerID, int iVictimID, int iVictimStreak,
						   int iDeathNoticeMsg);
...
void CTFHudDeathNotice::OnGameEvent(IGameEvent *event, int iDeathNoticeMsg)
{
	...
	if(FStrEq(pszEventName, "player_death") || FStrEq(pszEventName, "object_destroyed"))
	{
				...
				if(pszMaybeFallbackAssisterName && pszMaybeFallbackAssisterName[0])
				{
					...
					switch(ePyroVisionHack)
					{
						...
						case kHorriblePyroVisionHack_KillAssisterType_LocalizationString_First:
						{
							...
							g_pVGuiLocalize->ConvertUnicodeToANSI(wszLocalizedItemName, szANSIConvertedItemName,
		...
		if(assister_name)
		{
			...
			// Check to see if we're swapping the killer and the assister. We use this so the brain slug can get the
			// kill credit for the HUD death notices, with the player being the assister.
			if(pszAssisterName &&
			   (ePyroVisionHack == kHorriblePyroVisionHack_KillAssisterType_CustomName_First ||
		...
		const int iPlayerPenetrationCount =
			!event->IsEmpty("playerpenetratecount") ? event->GetInt("playerpenetratecount") : 0;
		...
		{
			// if this death involved a player dominating another player or getting revenge on another player, add an
			// additional message mentioning that
			...
			{
				AddAdditionalMsg(iAssisterID, iVictimID,
								 bIsSillyPyroVision ? "#Msg_Dominating_What" : "#Msg_Dominating");
		...
		else
		{
			...
			if(iObjectType >= 0 && iObjectType < OBJ_LAST)
			{
				...
				{
					g_pVGuiLocalize->ConvertUnicodeToANSI(wszLocalizedObjectName, szLocalizedObjectName,
														  ARRAYSIZE(szLocalizedObjectName));
				}
				else
				{
					Warning("Couldn't find localized object name for '%s'\n", szLocalizedObjectNames[iObjectType]);
					Q_strncpy(szLocalizedObjectName, szLocalizedObjectNames[iObjectType],
							  sizeof(szLocalizedObjectName));
				}
				...
				if(msg.Victim.szName[0])
				{
					...
					Q_snprintf(szVictimBuf, ARRAYSIZE(szVictimBuf), "%s (%s)", szLocalizedObjectName,
							   msg.Victim.szName);
		...
		switch(iCustomDamage)
		{
			...
			{
				// display a different message if this was suicide, or assisted suicide (suicide w/recent damage, kill
				// awarded to damager)
				bool bAssistedSuicide = event->GetInt("userid") != event->GetInt("attacker");
				pMsg =
					g_pVGuiLocalize->Find((bAssistedSuicide) ? (bMultipleKillers ? "#DeathMsg_AssistedSuicide_Multiple"
																				 : "#DeathMsg_AssistedSuicide")
															 : ("#DeathMsg_Suicide"));
				if(pMsg)
				{
			...
			case TF_DMG_CUSTOM_EYEBALL_ROCKET:
			{
				if(msg.Killer.iTeam == TEAM_UNASSIGNED)
				{
					...
					if(wszLocalizedName)
					{
						g_pVGuiLocalize->ConvertUnicodeToANSI(wszLocalizedName, szLocalizedName,
															  ARRAYSIZE(szLocalizedName));
		...
		{
			msg.iconPreKillerName =
				GetMannPowerIcon(pKiller->m_Shared.GetCarryingRuneType(), pKiller->GetTeamNumber() == TF_TEAM_RED);
		}

		if(pVictim && pVictim->m_Shared.IsCarryingRune())
		{
			msg.iconPostVictimName =
				GetMannPowerIcon(pVictim->m_Shared.GetCarryingRuneType(), pVictim->GetTeamNumber() == TF_TEAM_RED);
		}
		...
		if(iKillStreakWep > 0)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(msg.wzPreKillerText, g_pVGuiLocalize->Find("#Kill_Streak"), 1,
												  wzCount);
		...
		else if(iDuckStreakTotal > 0 && iDucksThisKill)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(msg.wzPreKillerText, g_pVGuiLocalize->Find("#Duck_Streak"), 1,
												  wzCount);
		...
		{
			AddStreakMsg(CTFPlayerShared::kTFStreak_Kills, iAssisterID, iKillStreakAssist, 1, iVictimID,
						 iDeathNoticeMsg);
		}

		if(pVictim && iKillStreakVictim > 2)
		{
			AddStreakEndedMsg(CTFPlayerShared::kTFStreak_Kills, iKillerID, iVictimID, iKillStreakVictim,
							  iDeathNoticeMsg);
		}
		...
		AddStreakMsg(CTFPlayerShared::kTFStreak_Ducks, iKillerID, iDuckStreakTotal, iDuckStreakIncrement, iVictimID,
					 iDeathNoticeMsg);
		if(pAssister && iDuckStreakAssist > 0 && iDucksThisKill)
		{
			AddStreakMsg(CTFPlayerShared::kTFStreak_Ducks, iAssisterID, iDuckStreakAssist, iDuckStreakIncrement,
						 iVictimID, iDeathNoticeMsg);
		}

		if(pVictim && iDuckStreakVictim > 2)
		{
			AddStreakEndedMsg(CTFPlayerShared::kTFStreak_Ducks, iKillerID, iVictimID, iDuckStreakVictim,
							  iDeathNoticeMsg);
		}
		...
		//); 		NotificationQueue_Add( pNotification ); 		m_bShowItemOnKill = false;
		//	}
		// }
			...
			FStrEq("teamplay_flag_event", pszEventName))
	{
		bool bDefense =
			(FStrEq("teamplay_capture_blocked", pszEventName) ||
			 (FStrEq("teamplay_flag_event", pszEventName) && TF_FLAGEVENT_DEFEND == event->GetInt("eventtype")));
	...
	else if(FStrEq("fish_notice", pszEventName) || FStrEq("fish_notice__arm", pszEventName))
	{
		...
		{
			g_pVGuiLocalize->ConstructString_safe(msg.wzInfoText,
												  FStrEq("fish_notice", pszEventName)
													  ? g_pVGuiLocalize->Find("#Humiliation_Kill")
													  : g_pVGuiLocalize->Find("#Humiliation_Kill_Arm"),
												  0);
		}
		else
		{
			...
			g_pVGuiLocalize->ConstructString_safe(msg.wzInfoText, g_pVGuiLocalize->Find("#Humiliation_Count"), 1,
												  wzCount);
		}
	...
	//	m_DeathNotices[iDeathNoticeMsg].iconDeath = GetIcon( m_DeathNotices[ iDeathNoticeMsg ].szIcon,
	//m_DeathNotices[iDeathNoticeMsg].bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted : kDeathNoticeIcon_Standard );
	...
	else if(FStrEq("rd_robot_killed", pszEventName))
	{
		...
		Q_strncpy(msg.Victim.szName, g_PR->GetTeam(killer) == TF_TEAM_RED ? "BLUE ROBOT" : "RED ROBOT",
				  ARRAYSIZE(msg.Victim.szName));
	...
	else if(FStrEq(PasstimeGameEvents::Score::s_eventName, pszEventName)) // passtime score
	{
		...
		if(ev.numPoints > 1)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(msg.wzInfoText, g_pVGuiLocalize->Find("#Msg_PasstimeScoreCount"), 1,
												  wzCount);
		}
		else
		{
		...
		// icon
		const char *const icon = (msg.Killer.iTeam == TF_TEAM_RED) ? "d_passtime_score_red" : "d_passtime_score_blue";
		Q_strncpy(msg.szIcon, icon, ARRAYSIZE(msg.szIcon));
	}
...
//-----------------------------------------------------------------------------
void CTFHudDeathNotice::AddStreakMsg(CTFPlayerShared::ETFStreak eStreakType, int iKillerID, int iKillerStreak,
									 int iStreakIncrement, int iVictimID, int iDeathNoticeMsg)
{
...
//-----------------------------------------------------------------------------
void CTFHudDeathNotice::AddStreakEndedMsg(CTFPlayerShared::ETFStreak eStreakType, int iKillerID, int iVictimID,
										  int iVictimStreak, int iDeathNoticeMsg)
{
	int nMinStreak = MinStreakForType(eStreakType);
