...
{
	CHudMainMenuOverride *pMMOverride =
...
ConVar tf_training_has_prompted_for_training("tf_training_has_prompted_for_training", "0", FCVAR_ARCHIVE,
											 "Whether the user has been prompted for training");
ConVar tf_training_has_prompted_for_offline_practice("tf_training_has_prompted_for_offline_practice", "0",
													 FCVAR_ARCHIVE,
													 "Whether the user has been prompted to try offline practice.");
ConVar tf_training_has_prompted_for_forums("tf_training_has_prompted_for_forums", "0", FCVAR_ARCHIVE,
										   "Whether the user has been prompted to view the new user forums.");
ConVar tf_training_has_prompted_for_options("tf_training_has_prompted_for_options", "0", FCVAR_ARCHIVE,
...
ConVar tf_training_has_prompted_for_loadout("tf_training_has_prompted_for_loadout", "0", FCVAR_ARCHIVE,
											"Whether the user has been prompted to equip something in their loadout.");
ConVar cl_ask_bigpicture_controller_opt_out(
	"cl_ask_bigpicture_controller_opt_out", "0", FCVAR_ARCHIVE,
	"Whether the user has opted out of being prompted for controller support in Big Picture.");
...
ConVar cl_promotional_codes_button_show("cl_promotional_codes_button_show", "1", FCVAR_ARCHIVE,
										"Toggles the 'View Promotional Codes' button in the main menu for players that "
										"have used the 'RIFT Well Spun Hat Claim Code'.",
										cc_promotional_codes_button_changed);
...
static ConCommand openmatchmakinglobby_command("OpenMatchmakingLobby", &CL_OpenMatchmakingLobby,
											   "Activates the matchmaking lobby.");
...
public:
	CCompetitiveAccessInfoPanel(Panel *pParent, const char *pszName) : EditablePanel(pParent, pszName)
	{
		m_pPhoneButton = NULL;
	...
	virtual void OnCommand(const char *command) OVERRIDE
	{
		...
		else if(FStrEq(command, "addphone"))
		{
			if(steamapicontext && steamapicontext->SteamFriends())
			{
				steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(
					"https://support.steampowered.com/kb_article.php?ref=8625-WRAH-9030#addphone");
			}
			return;
		}
		else if(FStrEq(command, "addpremium"))
		{
			...
			{
				steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(
					"https://steamcommunity.com/sharedfiles/filedetails/?id=143430756");
			}
			return;
		}
...
public:
	...
	CMainMenuPlayListEntry(Panel *pParent, const char *pszName) : EditablePanel(pParent, pszName)
	{
		m_pToolTip = NULL;
	}
	...
	{
		static const DisabledStateDesc_t s_DisabledStates[] = {
			{NULL, NULL, NULL} // NOT_DISABLED
			,
...
CHudMainMenuOverride::CHudMainMenuOverride(IViewPort *pViewPort) : BaseClass(NULL, PANEL_MAINMENUOVERRIDE)
{
	...
	m_pNotificationsScroller =
		new vgui::ScrollableEditablePanel(m_pNotificationsPanel, m_pNotificationsControl, "Notifications_Scroller");
...
// Purpose:
//-----------------------------------------------------------------------------
ConVar tf_last_store_pricesheet_version("tf_last_store_pricesheet_version", "0",
										FCVAR_CLIENTDLL | FCVAR_ARCHIVE | FCVAR_DONTRECORD | FCVAR_HIDDEN);
...
void CHudMainMenuOverride::FireGameEvent(IGameEvent *event)
{
	...
	if(FStrEq(type, "gameui_activated"))
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			this, "MMenu_PlayList_Collapse_Immediate", false);
		m_bPlayListExpanded = false;
		return;
	}
	...
	else if(Q_strcmp(type, "store_pricesheet_updated") == 0)
	{
		...
		if(EconUI() && EconUI()->GetStorePanel() && EconUI()->GetStorePanel()->GetPriceSheet())
		{
...
{
	// We need to re-hook ourselves up to the TF client scheme, because the GameUI will try to change us their its
	// scheme
	vgui::HScheme pScheme = vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL),
	...
	vgui::EditablePanel *pHeaderContainer =
		dynamic_cast<vgui::EditablePanel *>(m_pMOTDPanel->FindChildByName("MOTD_HeaderContainer"));
	if(pHeaderContainer)
	{
	...
	m_pMOTDTitleImageContainer =
		dynamic_cast<vgui::EditablePanel *>(m_pMOTDPanel->FindChildByName("MOTD_TitleImageContainer"));
	if(m_pMOTDTitleImageContainer)
	{
		m_pMOTDTitleImage =
			dynamic_cast<vgui::ImagePanel *>(m_pMOTDTitleImageContainer->FindChildByName("MOTD_TitleImage"));
	}
...
void CHudMainMenuOverride::PerformLayout(void)
{
	...
	{
		bool bIsVisible = (m_pMMButtonEntries[i].pPanel ? m_pMMButtonEntries[i].pPanel->IsVisible()
														: m_pMMButtonEntries[i].bIsVisible);
		if(!bIsVisible)
			continue;
	...
	if(m_pEventPromoContainer && m_pSafeModeContainer)
	{
		...
		{
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(m_pSafeModeContainer,
	...
	if(m_pEventPromoContainer && cl_mainmenu_updateglow.GetInt())
	{
		...
		{
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(pUpdateBackground,
	...
	if(m_pQuitButton && m_pDisconnectButton && m_pBackToReplaysButton)
	{
		...
		if(bShowDisconnect)
		{
			...
			if(m_bIsDisconnectText != bIsDisconnectText)
			{
				...
				m_pDisconnectButton->SetText(m_bIsDisconnectText ? "#GameUI_GameMenu_Disconnect"
																 : "#TF_MM_Rejoin_Abandon");
			}
		}
	}
...
void CHudMainMenuOverride::UpdateMOTD(bool bNewMOTDs)
{
	...
	if(pMOTD)
	{
		...
		m_pMOTDTextScroller->GetScrollbar()->SetVisible(m_pMOTDTextPanel->GetTall() >
														m_pMOTDTextScroller->GetScrollbar()->GetTall());
...
void CHudMainMenuOverride::SetMOTDButtonVisible(bool bVisible)
{
	...
	if(m_pMOTDShowPanel)
	{
		...
		{
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(m_pMOTDShowPanel,
																					"HasMOTDBlinkStop");
		}
	}
}
...
void CHudMainMenuOverride::UpdateNotifications()
{
	...
	if(m_pNotificationsShowPanel)
	{
		...
		{
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(m_pNotificationsShowPanel,
	...
	if(m_pNotificationsScroller)
	{
		   ...
		   m_pNotificationsScroller->GetScrollbar()->GetSlider()->IsSliderVisible())
		{
			m_pNotificationsPanel->SetWide(m_iNotiPanelWide +
										   m_pNotificationsScroller->GetScrollbar()->GetSlider()->GetWide());
...
void CHudMainMenuOverride::StopUpdateGlow()
{
	...
	if(m_pEventPromoContainer)
	{
		...
		if(pUpdateBackground)
		{
			g_pClientMode->GetViewportAnimationController()->StopAnimationSequence(pUpdateBackground,
																				   "MMenu_UpdateButton_StartGlow");
...
void CHudMainMenuOverride::OnCommand(const char *command)
{
	...
	{
		GetClientModeTFNormal()->GameUI()->SendMainMenuCommand(VarArgs(
			"engine open_store %d 1", m_pFeaturedItemPanel ? m_pFeaturedItemPanel->GetItem()->GetItemDefIndex() : 0));
	}
	else if(!Q_stricmp(command, "armory_open"))
	{
		GetClientModeTFNormal()->GameUI()->SendMainMenuCommand("engine open_charinfo_armory");
	}
	else if(!Q_stricmp(command, "engine disconnect") && engine->IsInGame() && TFGameRules() &&
		...
		if(steamapicontext && steamapicontext->SteamFriends() && steamapicontext->SteamUtils())
		{
			...
			switch(GetUniverse())
			{
				case k_EUniversePublic:
					steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(CFmtStr1024(
						"http://steamcommunity.com/profiles/%llu/promocodes/tf2", steamID.ConvertToUint64()));
					break;
				case k_EUniverseBeta:
					steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(CFmtStr1024(
						"http://beta.steamcommunity.com/profiles/%llu/promocodes/tf2", steamID.ConvertToUint64()));
					break;
				case k_EUniverseDev:
					steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(CFmtStr1024(
						"http://localhost/community/profiles/%llu/promocodes/tf2", steamID.ConvertToUint64()));
					break;
			}
		}
	}
	...
	else if(FStrEq("view_update_page", command))
	{
		...
		if(steamapicontext && steamapicontext->SteamFriends() && steamapicontext->SteamUtils() &&
		   steamapicontext->SteamUtils()->IsOverlayEnabled())
		{
			CSteamID steamID = steamapicontext->SteamUser()->GetSteamID();
			switch(GetUniverse())
			{
				...
				case k_EUniverseDev:
					steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(
						"http://csham.valvesoftware.com/tf.com/meetyourmatch");
					break;
			}
		}
		else
...
void CHudMainMenuOverride::CheckTrainingStatus(void)
{
	...
	else if(bNeedsTraining && m_bPlayListExpanded)
	{
		...
		if(m_pHighlightAnims[MMHA_TUTORIAL])
		{
			...
			{
				m_pHighlightAnims[MMHA_TUTORIAL]->SetDialogVariable(
					"highlighttext", g_pVGuiLocalize->Find("#MMenu_TutorialHighlight_Title2"));
			}
			else
			{
				m_pHighlightAnims[MMHA_TUTORIAL]->SetDialogVariable(
					"highlighttext", g_pVGuiLocalize->Find("#MMenu_TutorialHighlight_Title"));
			}
		}

		StartHighlightAnimation(MMHA_TUTORIAL);
	}
	else if(bWasInTraining && Training_IsComplete() == false && tf_training_has_prompted_for_training.GetInt() < 2 &&
			m_bPlayListExpanded)
	{
		...
		{
			m_pHighlightAnims[MMHA_TUTORIAL]->SetDialogVariable(
				"highlighttext", g_pVGuiLocalize->Find("#MMenu_TutorialHighlight_Title3"));
		}
...
void CHudMainMenuOverride::UpdatePlaylistEntries(void)
{
	...
	if((pParty && pParty->BOffline()) || !GTFGCClientSystem()->BConnectedtoGC() ||
	   GTFGCClientSystem()->BHasOutstandingMatchmakingPartyMessage())
	{
	...
	if(pEntry)
	{
		...
		{
			eDisabledState = !GTFGCClientSystem()->BHasCompetitiveAccess()
								 ? CMainMenuPlayListEntry::DISABLED_NO_COMP_ACCESS
								 : eDisabledState;
		}
		pEntry->SetDisabledReason(eDisabledState);
	}
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		pEntriesKV->SetString("last_request_time",
							  CRTime::RTime32ToString(pMMPanel->GetLastMOTDRequestTime(), rtime_buf));
...
GC_REG_JOB(GCSDK::CGCClient, CGCMOTDRequestResponse, "CGCMOTDRequestResponse", k_EMsgGCMOTDRequestResponse,
...
CON_COMMAND(create_icons,
			"Generate 512 x 512 Paint Kit Item Icons for SteamMarket, Specify min and max itemdef ranges if desired")
{
...
CON_COMMAND(create_icons_large,
			"Generate 1024 x 1024 Paint Kit Item Icons for Testing, Specify min and max itemdef ranges if desired")
{
	tf_icon_festive.SetValue(false);
	CHudMainMenuOverride *pMMPanel =
		(CHudMainMenuOverride *)gViewPortInterface->FindPanelByName(PANEL_MAINMENUOVERRIDE);
...
//-----------------------------------------------------------------------------
void BuildPaintkitItemInventoryImagePath(char *pchOutfile, int nMaxPath, const CTFItemDefinition *pItemDef, int iWear,
										 bool bLargeTestIcons)
{
	...
	if(bIsPaintkitItem)
	{
		pchOutputFolder = bLargeTestIcons ? "resource/econ/generated_icons/LargeTest/"
										  : "scripts/items/unencrypted/icons/generated_paintkit_icons/";
	}
	else
	{
	...
	{
		V_snprintf(pchOutfile, nMaxPath, "%s%s_festive.png", pchOutputFolder, fname);
	}
	else
	{
		V_snprintf(pchOutfile, nMaxPath, "%s%s.png", pchOutputFolder, fname);
	}
}
//-----------------------------------------------------------------------------
		...
		if((width > 0) && (height > 0))
		{
			...
			if(pixelValue)
			{
				...
				pRenderContext->ReadPixels(0, 0, width, height, (unsigned char *)pixelValue,
										   pInputTexture->GetImageFormat());

				CUtlBuffer outBuffer;
				ImgUtl_WriteRGBAAsPNGToBuffer(reinterpret_cast<const unsigned char *>(pixelValue), width, height,
											  outBuffer);
...
void StartNextImage(CEconItemView *pItemData, CEmbeddedItemModelPanel *pItemModelPanel,
					const CUtlVector<item_definition_index_t> &vecItemDef, float flCurrentWear)
{
	// Init the item
...
{
	CEmbeddedItemModelPanel *pItemModelPanel =
		dynamic_cast<CEmbeddedItemModelPanel *>(FindChildByName("icon_generator"));
	if(!pItemModelPanel)
		return;
	...
	BuildPaintkitItemInventoryImagePath(outfile, sizeof(outfile), m_pIconData->GetItemDefinition(),
										tf_paint_kit_force_wear.GetInt(), m_bGeneratingLargeTestIcons);
...
void CHudMainMenuOverride::GenerateIcons(bool bLarge, int min /*= -1*/, int max /*= -1*/)
{
	...
	const char *pchOutputFolder = bLarge ? "resource/econ/generated_icons/LargeTest/"
										 : "scripts/items/unencrypted/icons/generated_paintkit_icons";
	...
	materials->CreateNamedRenderTargetTextureEx2("offline_icon_generation", rtSize, rtSize, RT_SIZE_DEFAULT,
