...
ConVar cl_streams_request_accept("cl_streams_request_accept", "application/vnd.twitchtv.v3+json", FCVAR_DEVELOPMENTONLY,
								 "Header for api request");
ConVar cl_streams_image_sfurl("cl_streams_image_sfurl", "img://loadjpeg:(320x200):", FCVAR_DEVELOPMENTONLY,
							  "Format of Scaleform image representing the stream");

ConVar cl_streams_request_count("cl_streams_request_count", "6", FCVAR_DEVELOPMENTONLY,
								"How many streams are displayed in main menu");

ConVar cl_streams_refresh_interval("cl_streams_refresh_interval", "60", FCVAR_DEVELOPMENTONLY,
								   "How often to refresh streams list");
ConVar cl_streams_write_response_file("cl_streams_write_response_file", "", FCVAR_DEVELOPMENTONLY,
									  "When set will save streams info file for diagnostics");
ConVar cl_streams_override_global_version("cl_streams_override_global_version", "", FCVAR_DEVELOPMENTONLY,
										  "When set will override global API version");

ConVar cl_streams_mytwitchtv_nolink("cl_streams_mytwitchtv_nolink", "https://www.twitch.tv/settings/connections",
									FCVAR_DEVELOPMENTONLY, "Twitch.tv account linking URL");
ConVar cl_streams_mytwitchtv_channel("cl_streams_mytwitchtv_channel", "https://www.twitch.tv/", FCVAR_DEVELOPMENTONLY,
...
{
public:
	CHelperStreamDownloadUrlToLocalFile(char const *szUrlGet, char const *szLocalFile, long lTimeStampLocal,
										CTFStreamPanel *pStreamPanel)
	{
		m_sUrlGet = szUrlGet;
		...
		{
			m_CallbackOnHTTPRequestCompleted.Set(hCall, this,
												 &CHelperStreamDownloadUrlToLocalFile::Steam_OnHTTPRequestCompleted);
		}
		else
		{
...
private:
	...
	void Steam_OnHTTPRequestCompleted(HTTPRequestCompleted_t *p, bool bError)
	{
		...
		uint32 unBytes = 0;
		if(!bError && p && steamapicontext->SteamHTTP()->GetHTTPResponseBodySize(p->m_hRequest, &unBytes) &&
		   unBytes != 0)
		{
			DevMsg("Request for '%s' succeeded (code: %u, size: %u)...\n", m_sUrlGet.Get(), p ? p->m_eStatusCode : 0,
				   unBytes);
	...
	if(chPreviewImageLocal[0])
	{
		...
		if(!s_bCreateDirHierarchyDone)
		{
			...
			if(long lDirectoryTime = g_pFullFileSystem->GetFileTime(s_pszCacheImagePath, "GAME"))
			{
				...
				{
					if(!Q_strcmp(".", szFileName) || !Q_strcmp("..", szFileName))
						continue;
		...
		if(lFileTime != lUpdatedAtStamp)
		{
			...
			DevMsg(2, "%s -- Requesting download of preview image (%ld != %ld): %s -> %s\n", info.m_sGlobalName.Get(),
				   lFileTime, lUpdatedAtStamp, info.m_sPreviewImage.Get(), info.m_sPreviewImageLocalFile.Get());
			new CHelperStreamDownloadUrlToLocalFile(info.m_sPreviewImage.Get(), info.m_sPreviewImageLocalFile.Get(),
													lUpdatedAtStamp, pStreamPanel);
		}
		else
		{
			DevMsg(2, "%s -- Preview image is up to date (%ld): %s -> %s\n", info.m_sGlobalName.Get(), lUpdatedAtStamp,
				   info.m_sPreviewImage.Get(), info.m_sPreviewImageLocalFile.Get());
...
void CTFStreamManager::RequestTopStreams()
{
	...
	if(!m_dblTimeStampLastUpdate ||
	   (Plat_FloatTime() - m_dblTimeStampLastUpdate > cl_streams_refresh_interval.GetFloat()))
	{
		...
		if(!m_hHTTPRequestHandle && steamapicontext && steamapicontext->SteamHTTP())
		{
			...
			m_hHTTPRequestHandle =
				steamapicontext->SteamHTTP()->CreateHTTPRequest(k_EHTTPMethodGET, cl_streams_request_url.GetString());
			steamapicontext->SteamHTTP()->SetHTTPRequestHeaderValue(m_hHTTPRequestHandle, "Accept",
																	cl_streams_request_accept.GetString());
			steamapicontext->SteamHTTP()->SetHTTPRequestHeaderValue(m_hHTTPRequestHandle, "Client-ID",
																	"b7816vx0i8sng8bwy9es0dirdcsy3im");
			DevMsg("Requesting twitch.tv streams update...\n");

			SteamAPICall_t hCall = NULL;
			if(m_hHTTPRequestHandle && steamapicontext->SteamHTTP()->SendHTTPRequest(m_hHTTPRequestHandle, &hCall) &&
			   hCall)
			{
				m_CallbackOnHTTPRequestCompleted.Set(hCall, this,
													 &CTFStreamManager::Steam_OnHTTPRequestCompletedStreams);
			}
			else
			{
				if(m_hHTTPRequestHandle)
	...
	if(!bError && p && steamapicontext->SteamHTTP()->GetHTTPResponseBodySize(p->m_hRequest, &unBytes) && unBytes != 0)
	{
		...
		if(steamapicontext->SteamHTTP()->GetHTTPResponseBodyData(p->m_hRequest, (uint8 *)bufFile.Base(), unBytes))
		{
			...
			if(pValues)
			{
				...
				{
					for(GCSDK::CWebAPIValues *pvStream = pvStreams->GetFirstChild(); pvStream;
						pvStream = pvStream->GetNextChild())
					{
						...
						if((info.m_numViewers > 0) && !info.m_sGlobalName.IsEmpty() && !info.m_sDisplayName.IsEmpty() &&
						   !info.m_sTextDescription.IsEmpty() && !info.m_sVideoFeedUrl.IsEmpty())
						{
							// DevMsg( 2, "Channel: %s (%s, %u viewers) -- %s [[%s]]\n", info.m_sGlobalName.Get(),
							// info.m_sDisplayName.Get(), info.m_numViewers, info.m_sTextDescription.Get(),
							// info.m_sVideoFeedUrl.Get() );
...
void CTFStreamManager::UpdateTwitchTvAccounts()
{
	...
	// Create HTTP download job
	//
	m_hHTTPRequestHandleTwitchTv = steamapicontext->SteamHTTP()->CreateHTTPRequest(
		k_EHTTPMethodGET, CFmtStr("http://api.twitch.tv/api/steam/%llu", m_pLoadingAccount->m_uiSteamID));
	steamapicontext->SteamHTTP()->SetHTTPRequestHeaderValue(m_hHTTPRequestHandleTwitchTv, "Accept",
															cl_streams_request_accept.GetString());
	DevMsg("Requesting twitch.tv account link...\n");

	SteamAPICall_t hCall = NULL;
	if(m_hHTTPRequestHandleTwitchTv &&
	   steamapicontext->SteamHTTP()->SendHTTPRequest(m_hHTTPRequestHandleTwitchTv, &hCall) && hCall)
	{
		m_CallbackOnHTTPRequestCompletedTwitchTv.Set(hCall, this,
													 &CTFStreamManager::Steam_OnHTTPRequestCompletedMyTwitchTv);
	}
	else
	{
		if(m_hHTTPRequestHandleTwitchTv)
...
void CTFStreamManager::Steam_OnHTTPRequestCompletedMyTwitchTv(HTTPRequestCompleted_t *p, bool bError)
{
	...
	if(!bError && p && steamapicontext->SteamHTTP()->GetHTTPResponseBodySize(p->m_hRequest, &unBytes) && unBytes != 0)
	{
		...
		if(steamapicontext->SteamHTTP()->GetHTTPResponseBodyData(p->m_hRequest, (uint8 *)bufFile.Base(), unBytes))
		{
			...
			if(pValues)
			{
				...
				{
					m_pLoadingAccount->m_sTwitchTvChannel = CFmtStr("%s%s", cl_streams_mytwitchtv_channel.GetString(),
																	m_pLoadingAccount->m_sTwitchTvChannel.Get());
	...
	steamapicontext->SteamHTTP()->ReleaseHTTPRequest(p->m_hRequest);
	m_hHTTPRequestHandleTwitchTv = NULL;
	m_pLoadingAccount->m_dblTimeStampTwitchTvUpdate =
		Plat_FloatTime(); // push the update counter to not update for a little bit
	...
	if(FStrEq(command, "stream"))
	{
		...
		if(pInfo)
		{
			vgui::system()->ShellExecute(
				"open", CFmtStr("%s%s", cl_streams_mytwitchtv_channel.GetString(), pInfo->m_sDisplayName.Get()));
		}
	}
	else
	{
...
void CTFStreamPanel::SetPreviewImage(const char *pszPreviewImageFile)
{
	...
	pBitmapImage->SetRenderSize(flSubRectWidthScale * m_pPreviewImage->GetWide(),
								flSubRectWidthScale * flPanelWidthScale * tall);
