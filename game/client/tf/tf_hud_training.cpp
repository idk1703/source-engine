...
//-----------------------------------------------------------------------------
// Purpose: Localize text for training messages.  Used in annotations and in the training hud. Assumes the output string
// size is greater than or equal to MAX_TRAINING_MSG_LENGTH
//-----------------------------------------------------------------------------
...
void CTFHudTraining::Reset(void)
{
	...
	if(m_pMsgLabel)
		m_pMsgLabel->SetText(emptyText);
	if(m_pPressSpacebarToContinueLabel)
		m_pPressSpacebarToContinueLabel->SetVisible(false);
}
	...
	while(bContinue)
	{
		...
		switch(wszText[endIdx])
		{
			...
			case COLOR_NORMAL:
				...
				break;
			case COLOR_HINT:
				newColor = m_pMsgLabel->GetSchemeColor("HudTrainingHint", Color(255, 255, 255, 255),
													   scheme()->GetIScheme(m_pMsgLabel->GetScheme()));
				bSetText = true;
				break;
		}
		if(bSetText)
...
void CTFHudTraining::OnTick()
{
	...
	if(m_pPressSpacebarToContinueLabel && bShouldBeVisible != m_pPressSpacebarToContinueLabel->IsVisible())
	{
		...
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			bShouldBeVisible ? "TrainingPressSpacebarBlink" : "TrainingPressSpacebarBlinkStop");
	}
}