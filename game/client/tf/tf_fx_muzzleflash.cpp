...
void TF_3rdPersonMuzzleFlashCallback(const CEffectData &data)
{
	...
	CSmartPtr<CLocalSpaceEmitter> pSimple =
		CLocalSpaceEmitter::Create("MuzzleFlash", data.m_hEntity, attachmentIndex, 0);
	...
	if(data.m_nHitBox > 0) // >0 is a mg flash in script based muzzleflashes ..
	{
		...
		for(int i = 1; i < 9; i++)
		{
			...
			SimpleParticle *pParticle = (SimpleParticle *)pSimple->AddParticle(
				...
				pSimple->GetPMaterial(VarArgs("effects/muzzleflash%d", random->RandomInt(1, 4))), offset);
	...
	for(i = 0; i < 3; i++)
	{
		SimpleParticle *pParticle =
			(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), g_Mat_DustPuff[0], offset);
...
void TF_3rdPersonMuzzleFlashCallback_SentryGun(const CEffectData &data)
{
	...
	if(pEnt && !pEnt->IsDormant())
	{
		...
		// C_MuzzleFlashModel::CreateMuzzleFlashModel( "models/effects/sentry1_muzzle/sentry1_muzzle.mdl", pEnt,
		// iMuzzleFlashAttachment );
...
//-----------------------------------------------------------------------------
C_MuzzleFlashModel *C_MuzzleFlashModel::CreateMuzzleFlashModel(const char *pszModelName, C_BaseEntity *pParent,
															   int iAttachment, float flLifetime)
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_MuzzleFlashModel::InitializeMuzzleFlash(const char *pszModelName, C_BaseEntity *pParent, int iAttachment,
											   float flLifetime)
{
