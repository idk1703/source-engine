...
CTFAnnotationsPanelCallout *CTFAnnotationsPanel::TestAndAddCallout(int id, Vector &origin, const char *text)
{
	...
	CTFAnnotationsPanelCallout *pCallout =
		new CTFAnnotationsPanelCallout(g_pClientMode->GetViewport(), "AnnotationsPanelCallout", id, origin, text);
	...
	if(pCallout)
	{
		...
		if(pCallout->IsVisible())
		{
			...
			if(bShowEffect)
			{
				...
				{
					Vector vecNormal(event->GetFloat("worldNormalX"), event->GetFloat("worldNormalY"),
									 event->GetFloat("worldNormalZ"));
...
// Purpose:
//-----------------------------------------------------------------------------
CTFAnnotationsPanelCallout::CTFAnnotationsPanelCallout(Panel *parent, const char *name, int id, Vector &location,
													   const char *text)
...
void CTFAnnotationsPanelCallout::PerformLayout(void)
{
	...
	else
	{
		...
		// const float flDot = DotProduct( vDelta, vView );	// As the player looks away the target to the target, this
		// will go from -1 to 1 m_flAlpha[1] = clamp( -255 * flDot, 255, 255 );	// Set target.
	...
	if(bOffscreen)
	{
		...
		iX = flPerpDot <= 0.0f ? nHorizontalBuffer
							   : (ScreenWidth() - nHorizontalBuffer - m_pBackground->GetWide() - m_pArrow->GetWide());
	...
	else
	{
		...
		if(tr.fraction < 1.0f)
		{
			...
			if(m_FollowEntity.Get() && m_FollowEntity->GetTeamNumber() != TEAM_UNASSIGNED &&
			   m_FollowEntity->GetTeamNumber() != TEAM_INVALID &&
	...
	if(m_bShowDistance)
	{
		...
		if(m_FollowEntity.Get())
		{
			...
			if(pBuilding && pBuilding->GetType() < OBJ_LAST)
			{
				...
				const char *szLocalizedObjectNames[OBJ_LAST] = {"#TF_Object_Dispenser", "#TF_Object_Tele",
																"#TF_Object_Sentry", "#TF_object_Sapper"};
		...
		{
			g_pVGuiLocalize->ConstructString_safe(wzText, g_pVGuiLocalize->Find("#TR_DistanceToObject"), 2,
												  wzFollowEntityName, wzValue);
		}
