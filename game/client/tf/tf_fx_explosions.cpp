...
void TFExplosionCallback(const Vector &vecOrigin, const Vector &vecNormal, int iWeaponID, ClientEntityHandle_t hEntity,
						 int nDefID, int nSound, int iCustomParticleIndex)
{
	// Get the weapon information.
		...
		if(Q_strlen(pWeaponInfo->m_szExplosionSound) > 0)
		{
			...
			if(nDefID >= 0)
			{
				...
				if(pLocalPlayer)
				{
					...
					if(pItemDef)
					{
						pszSound = (char *)pItemDef->GetWeaponReplacementSound(pLocalPlayer->GetTeamNumber(),
																			   (WeaponSound_t)nSound);
...
static void RecvProxy_ExplosionEntIndex(const CRecvProxyData *pData, void *pStruct, void *pOut)
{
	...
	((C_TETFExplosion *)pStruct)->m_hEntity = (nEntIndex == kInvalidEHandleExplosion || nEntIndex == -1)
...
RecvPropFloat(RECVINFO(m_vecOrigin[0])), RecvPropFloat(RECVINFO(m_vecOrigin[1])),
	RecvPropFloat(RECVINFO(m_vecOrigin[2])), RecvPropVector(RECVINFO(m_vecNormal)), RecvPropInt(RECVINFO(m_iWeaponID)),
	RecvPropInt("entindex", 0, SIZEOF_IGNORE, 0, RecvProxy_ExplosionEntIndex), RecvPropInt(RECVINFO(m_nDefID)),
	RecvPropInt(RECVINFO(m_nSound)), RecvPropInt(RECVINFO(m_iCustomParticleIndex)),
END_RECV_TABLE
()