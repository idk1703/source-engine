...
ConVar tf_weapon_select_demo_start_delay("tf_weapon_select_demo_start_delay", "1.0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
										 "Delay after spawning to start the weapon bucket demo.");
ConVar tf_weapon_select_demo_time("tf_weapon_select_demo_time", "0.5", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
								  "Time to pulse each weapon bucket upon spawning as a new class. 0 to turn off.");
...
private:
	...
	void DrawWeaponTexture(C_TFPlayer *pPlayer, C_BaseCombatWeapon *pWeapon, int xpos, int ypos, float flLargeBoxWide,
						   float flLargeBoxTall);
...
bool CHudWeaponSelection::ShouldDrawInternal()
{
	...
	if(hud_fastswitch.GetInt() == 0 && !::input->IsSteamControllerActive() &&
	   (m_iDemoModeSlot >= 0 || m_flDemoStartTime > 0))
	{
		return true;
	}
			...
			for(int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS; slotpos++)
			{
				...
				if(pWeapon == pSelectedWeapon || (m_iDemoModeSlot == i))
				{
					...
					if(IsPC() && shortcut >= 0 && nFastswitchMode != HUDTYPE_PLUS)
					{
						...
						surface()->DrawSetTextPos(xpos + wide - XRES(5) - m_flSelectionNumberXPos,
												  ypos + YRES(5) + m_flSelectionNumberYPos);
		...
		else
		{
			...
			if(GetFirstPos(i))
			{
				...
				if(IsPC() && nFastswitchMode != HUDTYPE_PLUS)
				{
					...
					surface()->DrawSetTextFont(m_hNumberFont);
					wchar_t wch = '0' + i + 1;
					surface()->DrawSetTextPos(x + m_flSmallBoxWide - XRES(10) - m_flSelectionNumberXPos,
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudWeaponSelection::DrawWeaponTexture(C_TFPlayer *pPlayer, C_BaseCombatWeapon *pWeapon, int xpos, int ypos,
											float flLargeBoxWide, float flLargeBoxTall)
{
	// draw icon
...
void CHudWeaponSelection::DrawString(wchar_t *text, int xpos, int ypos, Color col, bool bCenter)
{
	...
	for(wchar_t *pch = text; charCount > 0; pch++)
	{
		if(*pch == '\n')
		{
			...
			surface()->DrawSetTextPos(x + ((m_flLargeBoxWide - slen) / 2),
									  ypos + (surface()->GetFontTall(m_hTextFont) * 1.1f));
		}
		else if(*pch == '\r')
		{
			// do nothing
