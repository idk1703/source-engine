...
CON_COMMAND_F(cl_gameserver_create_identity,
			  "Creates a new game server account associated with the currently logged in steam account",
			  FCVAR_CLIENTDLL)
{
...
CON_COMMAND_F(cl_gameserver_list, "List all the game server accounts owned by the currently logged in steam account",
			  FCVAR_CLIENTDLL)
{
	if(steamapicontext && steamapicontext->SteamUser())
	{
...
GC_REG_JOB(GCSDK::CGCClient, CGC_GameServer_CreateIdentityResponse, "CGC_GameServer_CreateIdentityResponse",
		   k_EMsgGC_GameServer_CreateIdentityResponse, GCSDK::k_EServerTypeGCClient);
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		for(int i = 0; i < msg.Body().owned_game_servers_size(); ++i)
		{
			...
			const char *pStanding =
				GameServerAccount_GetStandingString((eGameServerScoreStanding)identity.game_server_standing());
			const char *pStandingTrend = GameServerAccount_GetStandingTrendString(
				(eGameServerScoreStandingTrend)identity.game_server_standing_trend());
			Msg("%d - Account ID: %u  Identity Token: %s   Standing: %s  Trend: %s\n", i + 1,
				identity.game_server_account_id(), identity.game_server_identity_token().c_str(), pStanding,
				pStandingTrend);
		}
		return true;
	}
};
GC_REG_JOB(GCSDK::CGCClient, CGC_GameServer_ListResponse, "CGC_GameServer_ListResponse",
		   k_EMsgGC_GameServer_ListResponse, GCSDK::k_EServerTypeGCClient);
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		{
			Msg("Failed to reset game server account identity--check to make sure the game server account id is "
				"correct!\n");
		}
...
GC_REG_JOB(GCSDK::CGCClient, CGC_GameServer_ResetIdentityResponse, "CGC_GameServer_ResetIdentityResponse",
		   k_EMsgGC_GameServer_ResetIdentityResponse, GCSDK::k_EServerTypeGCClient);