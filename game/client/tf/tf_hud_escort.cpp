...
void CEscortHillPanel::OnTick(void)
{
	...
	if(m_bOnHill)
	{
		if(m_bFadingOut)
		{
			GetAnimationController()->RunAnimationCommand(this, "alpha", 32, 0.0,
														  (float)TF_ESCORT_HILL_ALPHA_CYCLE_TIME / 1000,
														  vgui::AnimationController::INTERPOLATOR_LINEAR);
			m_bFadingOut = false;
		}
		else
		{
			GetAnimationController()->RunAnimationCommand(this, "alpha", 96, 0.0,
														  (float)TF_ESCORT_HILL_ALPHA_CYCLE_TIME / 1000,
														  vgui::AnimationController::INTERPOLATOR_LINEAR);
			m_bFadingOut = true;
		}
	...
	if(m_bMultipleTrains)
	{
		if(m_pEscortItemImageAlert)
		{
			if(m_bAlarm)
			{
				...
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(m_pEscortItemPanel,
																						"HudCartAlarmPulse");
			}
			else
			{
...
void CTFHudEscort::OnTick()
{
	...
	if(TFGameRules() && (TFGameRules()->GetGameType() != TF_GAMETYPE_ESCORT) &&
	   (TFGameRules()->GetHUDType() != TF_HUDTYPE_ESCORT))
		return;

	if(!BaseClass::IsVisible()) // intentionally skipping our version of IsVisible() to bypass the
	...
	// flSecondsToRecede ); 	SetDialogVariable( "progress", value );
...
void CTFHudEscort::UpdateStatusTeardropFor(int iIndex)
{
	...
	if(m_bMultipleTrains)
	{
		// don't draw teardrops for points that aren't in the local group the player cares about (group numbers map to
		// the teams)
		if(iIndex != -1 &&
		   (ObjectiveResource()->GetCPGroup(iIndex) != (m_nTeam - 2))) // -2 to offset the team numbers to 0
			return;

		// don't draw teardrops if you're not the same team as the local player (top bar is the only one to use the
		// teamdrop)
		C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
		if(pPlayer)
		{
...
void CTFHudEscort::UpdateCPImages(void)
{
	...
	for(int i = 0; i < iPoints; i++)
	{
		...
		if(!ObjectiveResource()->IsInMiniRound(i) || !ObjectiveResource()->IsCPVisible(i) ||
		   (m_bMultipleTrains &&
			(ObjectiveResource()->GetCPGroup(i) != (m_nTeam - 2)))) // -2 to offset the team numbers to 0
		{
			bValid = false;
		}
