...
// Purpose:
//-----------------------------------------------------------------------------
ITexture *CTFRenderTargets::CreateItemModelPanelTexture(const char *pszName, IMaterialSystem *pMaterialSystem,
														int iSize)
{
	return pMaterialSystem->CreateNamedRenderTargetTextureEx2(
		pszName, iSize, iSize, RT_SIZE_DEFAULT, pMaterialSystem->GetBackBufferFormat(), MATERIAL_RT_DEPTH_SHARED,
...
void CTFRenderTargets::InitClientRenderTargets(IMaterialSystem *pMaterialSystem,
											   IMaterialSystemHardwareConfig *pHardwareConfig)
{
	...
	for(int i = 0; i < ITEM_MODEL_IMAGE_CACHE_SIZE; i++)
	{
		...
		m_tfRenderTargets[index].Init(
			CreateItemModelPanelTexture(g_ItemModelPanelRenderTargetNames[i], pMaterialSystem, 256));
	}
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CTFRenderTargets, IClientRenderTargets, CLIENTRENDERTARGETS_INTERFACE_VERSION,
								  g_TFRenderTargets);
