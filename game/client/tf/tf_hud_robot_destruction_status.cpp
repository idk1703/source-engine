...
// Purpose:
//-----------------------------------------------------------------------------
CTFHudRobotDestruction_StateImage::CTFHudRobotDestruction_StateImage(Panel *parent, const char *name,
...
void CTFHudRobotDestruction_StateImage::ApplySettings(KeyValues *inResourceData)
{
	...
	const CTFHudRobotDestruction_RobotIndicator *pRobotImageParent =
		dynamic_cast<const CTFHudRobotDestruction_RobotIndicator *>(GetParent()->GetParent());
...
// Purpose:
//-----------------------------------------------------------------------------
CTFHudRobotDestruction_DeadImage::CTFHudRobotDestruction_DeadImage(Panel *parent, const char *name,
																   const char *pszResFile)
...
// Purpose:
//-----------------------------------------------------------------------------
CTFHudRobotDestruction_ActiveImage::CTFHudRobotDestruction_ActiveImage(Panel *parent, const char *name,
																	   const char *pszResFile)
...
// Purpose:
//-----------------------------------------------------------------------------
CTFHudRobotDestruction_RobotIndicator::CTFHudRobotDestruction_RobotIndicator(vgui::Panel *pParent, const char *pszName,
																			 CTFRobotDestruction_RobotGroup *pGroup)
	: EditablePanel(pParent, pszName), m_hGroup(pGroup), m_pNextRobotIndicator(NULL), m_pPrevRobotIndicator(NULL)
{
	...
	m_pDeadPanel = new CTFHudRobotDestruction_DeadImage(m_pRobotStateContainer, "DeadState",
														"resource/UI/TFHudRobotDestruction_DeadState.res");
	m_pActivePanel = new CTFHudRobotDestruction_ActiveImage(m_pRobotStateContainer, "ActiveState",
															"resource/UI/TFHudRobotDestruction_ActiveState.res");
	m_pShieldedPanel = new CTFHudRobotDestruction_StateImage(m_pRobotStateContainer, "ShieldedState",
...
void CTFHudRobotDestruction_RobotIndicator::DoUnderAttackBlink()
{
	...
	if(gpGlobals->curtime < (m_hGroup->GetLastAttackedTime() + ATTACK_BLINK_TIME) &&
	   (GetLocalPlayerTeam() == m_hGroup->GetTeamNumber()))
	{
		// Pulse red
		ImagePanel *pGlow = dynamic_cast<ImagePanel *>(FindChildByName("GlowImage", true));
		if(pGlow)
...
void CTFHUDRobotDestruction::ApplySchemeSettings(IScheme *pScheme)
{
	...
	{
		CTFRobotDestruction_RobotGroup *pGroup =
			static_cast<CTFRobotDestruction_RobotGroup *>(IRobotDestructionGroupAutoList::AutoList()[i]);
		...
		robotVec[robotVec.AddToTail()] = new CTFHudRobotDestruction_RobotIndicator(
			this, CFmtStr("%s_group_%d", pszPanelName, robotVec.Count()), pGroup);
	}
...
void CTFHUDRobotDestruction::PerformLayout()
{
	...
	m_nStealRightEdge = nXPos + m_pProgressBarsContainer->GetWide() - m_nStealRightEdgeOffset +
						(m_pRedStolenContainer->GetWide() / 2.f);
}
...
void CTFHUDRobotDestruction::PerformRobotLayout(RobotVector_t &vecRobots, int nTeam)
{
	...
	FOR_EACH_VEC_BACK(vecRobots, i)
	{
		...
		if(pRobot)
		{
			...
			CTFHudRobotDestruction_RobotIndicator *pPrevRobot =
				dynamic_cast<CTFHudRobotDestruction_RobotIndicator *>(pPrevPanel);
			if(pPrevRobot)
			{
...
#endif
	{
		...
		if(pRoboLogic->GetType() == CTFRobotDestructionLogic::TYPE_PLAYER_DESTRUCTION)
		{
			for(int i = 0; i < ICaptureFlagAutoList::AutoList().Count(); ++i)
			{
				...
				{
					int &nFriendScore =
						pFlag->GetPrevOwner()->GetTeamNumber() == TF_TEAM_RED ? nRedEscrow : nBlueEscrow;
			...
			if(m_pCountdownContainer)
			{
				...
				if(flTimeRemaining > -1)
				{
					...
					ImagePanel *pCountdownImage =
						dynamic_cast<ImagePanel *>(m_pCountdownContainer->FindChildByName("CountdownImage", true));
					if(pCountdownImage)
					{
						bool bVisible = true;
					...
					CExLabel *pCountdownTime =
						dynamic_cast<CExLabel *>(m_pCountdownContainer->FindChildByName("CountdownTime", true));
					CExLabel *pCountdownTimeShadow =
						dynamic_cast<CExLabel *>(m_pCountdownContainer->FindChildByName("CountdownTimeShadow", true));
					if(pCountdownTime)
					{
					...
					m_pCountdownContainer->SetDialogVariable("countdowntime",
															 (nCountdownTime < 0) ? 0 : nCountdownTime);
				}
				else
				{
					if(m_pCountdownContainer->IsVisible())
		...
		float flFinaleProgress =
			clamp(pRoboLogic->GetFinaleWinTime(TF_TEAM_RED) - gpGlobals->curtime, 0.f, pRoboLogic->GetFinaleLength());
		...
		flFinaleProgress =
			clamp(pRoboLogic->GetFinaleWinTime(TF_TEAM_BLUE) - gpGlobals->curtime, 0.f, pRoboLogic->GetFinaleLength());
...
void CTFHUDRobotDestruction::UpdateStolenPoints(int nTeam, EditablePanel *pContainer)
{
	...
	if(pStolenFlag && pStartCaptureZone && pEndCaptureZone)
	{
		...
		Vector vecFlagPos =
			pStolenFlag->GetMoveParent() ? pStolenFlag->GetMoveParent()->GetAbsOrigin() : pStolenFlag->GetAbsOrigin();
		// Get the distance of the flag between the cap points
		const float flTotalDist =
			(pEndCaptureZone->WorldSpaceCenter() - pStartCaptureZone->WorldSpaceCenter()).Length() -
			pEndCaptureZone->BoundingRadius() - pStartCaptureZone->BoundingRadius();
		const float flFlagDist =
			(pEndCaptureZone->WorldSpaceCenter() - vecFlagPos).Length() - pEndCaptureZone->BoundingRadius();
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFHUDRobotDestruction::UpdateCarriedFlagStatus(C_BasePlayer *pNewOwner /*= NULL*/,
													 C_BaseEntity *pFlagEntity /*= NULL*/)
{
	...
	if(pPlayerFlag && !pPlayerFlag->IsMarkedForDeletion() && !pPlayerFlag->IsDormant())
	{
		...
		if(m_pCarriedImage && !m_pCarriedImage->IsVisible())
		{
			...
			if(pPlayerFlag->GetType() == TF_FLAGTYPE_ATTACK_DEFEND ||
			   ...
			   pPlayerFlag->GetType() == TF_FLAGTYPE_INVADE || pPlayerFlag->GetType() == TF_FLAGTYPE_RESOURCE_CONTROL)
			{
			...
			m_pCarriedFlagProgressBar->SetColor(pLocalPlayer->GetTeamNumber() == TF_TEAM_RED ? m_ColorRed
																							 : m_ColorBlue);
		}
...
void CTFHUDRobotDestruction::FireGameEvent(IGameEvent *pEvent)
{
	...
	else if(FStrEq(pszEventName, "rd_team_points_changed"))
	{
		...
		bool bPositive =
			(nTeam == GetLocalPlayerTeam() && nPoints > 0) || (nTeam != GetLocalPlayerTeam() && nPoints < 0);
	...
	else if(FStrEq(pszEventName, "teamplay_round_start"))
	{
		...
		float flApproachSpeed = (tf_rd_points_per_approach.GetInt() / tf_rd_points_approach_interval.GetFloat()) /
								pRoboLogic->GetMaxPoints();
...
void CTFHUDRobotDestruction::CProgressPanel::PaintBackground()
{
	...
	float flLerp = bPastBlink ? (m_flCurrentProgress >= m_flBlinkThreshold
	...
	Color drawColor(flInverseLerp * m_BrightColor.r() + flLerp * m_StandardColor.r(),
					...
					flInverseLerp * m_BrightColor.b() + flLerp * m_StandardColor.b(), 255);
...
void CTFHUDRobotDestruction::CProgressPanel::CalculateSize()
{
	...
	m_flXpos = m_bLeftToRight ? m_nXOrg : (1.f - m_flCurrentProgress) * nProgressWidth + m_nXOrg;
