...
void CHudTournament::PlaySounds(int nTime)
{
	...
	switch(nTime)
	{
		...
		case 10:
		{
			...
			{
				if(TFObjectiveResource()->GetMannVsMachineWaveCount() >=
				...
				else if(TFObjectiveResource()->GetMannVsMachineWaveCount() <= 1)
				{
					if(GTFGCClientSystem()->GetLobby() &&
					   IsMannUpGroup(GTFGCClientSystem()->GetLobby()->GetMatchGroup()))
					{
			...
			{
				pLocalPlayer->EmitSound(bCompetitiveMode ? "Announcer.CompGame1Begins10Seconds"
		...
		case 5:
		{
			pLocalPlayer->EmitSound(bCompetitiveMode ? "Announcer.CompGameBegins05Seconds"
													 : "Announcer.RoundBegins5Seconds");
			break;
		}
		case 4:
		{
			pLocalPlayer->EmitSound(bCompetitiveMode ? "Announcer.CompGameBegins04Seconds"
													 : "Announcer.RoundBegins4Seconds");
			break;
		}
		case 3:
		{
			pLocalPlayer->EmitSound(bCompetitiveMode ? "Announcer.CompGameBegins03Seconds"
													 : "Announcer.RoundBegins3Seconds");
			break;
		}
		case 2:
		{
			pLocalPlayer->EmitSound(bCompetitiveMode ? "Announcer.CompGameBegins02Seconds"
													 : "Announcer.RoundBegins2Seconds");
			break;
		}
		case 1:
		{
			pLocalPlayer->EmitSound(bCompetitiveMode ? "Announcer.CompGameBegins01Seconds"
				...
				if(m_bReadyStatusMode)
				{
					if(bSteamController)
					{
						SetDialogVariable("readylabel",
										  g_pVGuiLocalize->Find("Tournament_Instructions_Ready_NoKeyHintText"));
						bShowReadyHintIcon = true;
					}
					else
					{
		...
		if(m_bCountDownVisible != bCountdownVisible)
		{
			...
			if(m_bCountDownVisible)
			{
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
					this, m_bCompetitiveMode ? "HudTournament_ShowTimerCompetitive" : "HudTournament_ShowTimerDefault",
					false);
			}
			else
			{
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(this, "HudTournament_HideTimer",
																						false);
			}
		}
	}
...
#define STRING_FMT L"%S"
	...
	SetDialogVariable("bluestate", TFGameRules()->IsTeamReady(TF_TEAM_BLUE)
									   ? g_pVGuiLocalize->Find("Tournament_TeamReady")
									   : g_pVGuiLocalize->Find("Tournament_TeamNotReady"));
	SetDialogVariable("redstate", TFGameRules()->IsTeamReady(TF_TEAM_RED)
									  ? g_pVGuiLocalize->Find("Tournament_TeamReady")
									  : g_pVGuiLocalize->Find("Tournament_TeamNotReady"));

	if(m_bTeamReady[TF_TEAM_BLUE] != TFGameRules()->IsTeamReady(TF_TEAM_BLUE) ||
	...
	_snwprintf(szWindConditions, ARRAYSIZE(szWindConditions), STRING_FMT,
			   g_pVGuiLocalize->Find("Tournament_WinConditions"));
	...
	if(mp_timelimit.GetInt() > 0 || mp_winlimit.GetInt() > 0 || mp_maxrounds.GetInt())
	{
		...
		{
			_snwprintf(szWindConditions, ARRAYSIZE(szWindConditions), STRING_FMT L"%d " STRING_FMT, szWindConditions,
					   mp_timelimit.GetInt(),
					   mp_timelimit.GetInt() == 1 ? g_pVGuiLocalize->Find("Tournament_WinConditionsMinute")
		...
		if(mp_winlimit.GetInt() > 0)
		{
			...
			_snwprintf(szWindConditions, ARRAYSIZE(szWindConditions), STRING_FMT L"%d " STRING_FMT, szWindConditions,
					   mp_winlimit.GetInt(),
					   mp_winlimit.GetInt() == 1 ? g_pVGuiLocalize->Find("Tournament_WinConditionsWin")
												 : g_pVGuiLocalize->Find("Tournament_WinConditionsWins"));
			bPrev = true;
		}
		...
		if(mp_maxrounds.GetInt() > 0)
		{
			...
			_snwprintf(szWindConditions, ARRAYSIZE(szWindConditions), STRING_FMT L"%d " STRING_FMT, szWindConditions,
					   mp_maxrounds.GetInt(),
					   mp_maxrounds.GetInt() == 1 ? g_pVGuiLocalize->Find("Tournament_WinConditionsRound")
												  : g_pVGuiLocalize->Find("Tournament_WinConditionsRounds"));
		}
	}
	else
	{
		_snwprintf(szWindConditions, ARRAYSIZE(szWindConditions), STRING_FMT STRING_FMT, szWindConditions,
				   g_pVGuiLocalize->Find("Tournament_WinConditionsNone"));
	}
...
void CHudTournament::FireGameEvent(IGameEvent *event)
{
	...
	if(Q_strcmp("tournament_stateupdate", pEventName) == 0)
	{
		...
		if(bNameChange)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#Tournament_TeamName_Change"), 2,
												  wszPlayerName, wszTeam);
		}
		else
		{
			g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#Tournament_ReadyState_Change"),
												  2, wszPlayerName,
												  iTeamState == 1 ? g_pVGuiLocalize->Find("#Tournament_TeamReady")
																  : g_pVGuiLocalize->Find("#Tournament_TeamNotReady"));
		}
	...
	else if(FStrEq("restart_timer_time", pEventName))
	{
		...
		if(TFGameRules()->GetRoundsPlayed() == 0 && m_bCompetitiveMode)
		{
			...
			{
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
					this, "HudTournament_MoveTimerDown", false);
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence("HudTournament_MoveChatWindow",
																						false);
			}
		}
	}
	...
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
			this, "HudTournament_DoorsCloseEndRound", false);
	}
}
...
void CHudTournament::RecalculatePlayerPanels(void)
{
	...
	if(pLobby)
	{
		for(int i = 0; i < pLobby->GetNumMembers(); ++i)
		{
			...
			{
				CTFPlayerPanel *pPanel = GetOrAddPanel(iPanel);
				pPanel->Setup(0, steamID, pLobby->GetMemberDetails(i)->name().c_str(),
							  pLobby->GetMemberDetails(steamID)->team());
				++iPanel;
			}
		}
	}
...
void CHudTournament::UpdatePlayerPanels(void)
{
	...
	if(TFGameRules()->GetRoundRestartTime() >= 0.f &&
	   TFGameRules()->GetRoundRestartTime() - gpGlobals->curtime <= TOURNAMENT_NOCANCEL_TIME)
	{
		SetPlayerPanelsVisible(false);
		m_pModeImage->SetVisible(false);
	...
	if((iLocalTeam == TF_TEAM_RED || iLocalTeam == TF_TEAM_BLUE) &&
	   !TFGameRules()->IsCompetitiveMode()) // Blue always on left in comp
	{
		iTeam1 = iLocalTeam;
	...
	if(GTFGCClientSystem()->GetLobby())
	{
		...
		iTeam1Count = TFGameRules()->IsMannVsMachineMode() ? GTFGCClientSystem()->GetLobby()->GetNumMembers()
...
void CHudTournamentSetup::OnTick(void)
{
	...
	if(m_flNextThink <= gpGlobals->curtime)
	{
		if(!IsMouseInputEnabled())
		{
			m_pNameEntry->SetText((iLocalTeam == TF_TEAM_BLUE) ? mp_tournament_blueteamname.GetString()
															   : mp_tournament_redteamname.GetString());
		}

		SetDialogVariable("tournamentstatelabel", TFGameRules()->IsTeamReady(iLocalTeam)
													  ? g_pVGuiLocalize->Find("Tournament_TeamSetupReady")
													  : g_pVGuiLocalize->Find("Tournament_TeamSetupNotReady"));
...
void CHudStopWatch::OnTick(void)
{
	...
	if(!bProperMatch || TFGameRules()->IsInPreMatch() || !TFGameRules()->IsInStopWatch() || bInFreezeCam ||
	...
	if(m_pTimePanel && ObjectiveResource())
	{
		...
		else if(TFGameRules()->GetStopWatchState() == STOPWATCH_RUNNING)
		{
			...
			{
				g_pVGuiLocalize->ConstructString_safe(wzHelp, g_pVGuiLocalize->Find("Tournament_StopWatch_TimeVictory"),
													  1, pDefender->Get_Localized_Name());
			}
			else
			{
				g_pVGuiLocalize->ConstructString_safe(wzHelp,
													  g_pVGuiLocalize->Find("Tournament_StopWatch_TimeVictoryDefender"),
													  1, pDefender->Get_Localized_Name());
			}
		...
		else if(TFGameRules()->GetStopWatchState() == STOPWATCH_OVERTIME)
		{
			...
			if(pPlayer->GetTeam() == pAttacker)
			{
				g_pVGuiLocalize->ConstructString_safe(
					wzScoreVal, g_pVGuiLocalize->Find("Tournament_StopWatchPointCaptureAttacker"), 2, wzVal,
			...
			{
				g_pVGuiLocalize->ConstructString_safe(
					wzScoreVal, g_pVGuiLocalize->Find("Tournament_StopWatchPointCaptureDefender"), 2, wzVal,
					iPoints == 1 ? g_pVGuiLocalize->Find("#Tournament_StopWatch_Point")
								 : g_pVGuiLocalize->Find("#Tournament_StopWatch_Points"));
			}
			else
			{
				g_pVGuiLocalize->ConstructString_safe(
					wzScoreVal, g_pVGuiLocalize->Find("Tournament_StopWatchPointCaptureSpectator"), 2, wzVal,
					iPoints == 1 ? g_pVGuiLocalize->Find("#Tournament_StopWatch_Point")
								 : g_pVGuiLocalize->Find("#Tournament_StopWatch_Points"));
			}
...
CON_COMMAND(player_ready_toggle, "Toggle player ready state")
{
	...
	if(pPlayer)
	{
		CHudTournamentSetup *pTournamentPanel =
			dynamic_cast<CHudTournamentSetup *>(GET_HUDELEMENT(CHudTournamentSetup));
		if(pTournamentPanel)
		{
