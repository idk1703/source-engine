...
void CHudArenaNotification::FireGameEvent(IGameEvent *event)
{
	...
	else if(Q_strcmp("arena_match_maxstreak", pEventName) == 0)
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#TF_Arena_MaxStreak"), 2,
											  pTeam->Get_Localized_Name(), wStreak);
...
void CHudArenaNotification::OnTick(void)
{
	if(IsVisible() == true)
	{
		...
		if((m_flHideAt && m_flHideAt < gpGlobals->curtime) ||
		   (m_pTeamSwitchPanel && m_pTeamSwitchPanel->ShouldDraw() == true) || TFGameRules()->InStalemate() == true ||
