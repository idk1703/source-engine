...
static const char *kBodygroupArray[] = {
	"backpack", "dogtags", "grenades", "hands", "hat", "head", "headphones", "shoes", "shoes_socks",
};

extern IFileSystem *g_pFullFileSystem;

// This should be a constant since it's in the game directory hierarchy now
// static ConVar tf_steam_workshop_dir( "tf_steam_workshop_dir", "workshop", FCVAR_CLIENTDLL | FCVAR_ARCHIVE, "Directory
// used to hold import sessions for the Steam Workshop." );
...
static ConVar tf_steam_workshop_import_icon_path("tf_steam_workshop_import_icon_path", "",
												 FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
												 "Default location to load backpack icons from");
static ConVar tf_steam_workshop_import_model_path("tf_steam_workshop_import_model_path", "",
												  FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
												  "Default location to load models from");
static ConVar tf_steam_workshop_import_material_path("tf_steam_workshop_import_material_path", "",
													 FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
													 "Default location to load materials from");
...
static const char *kLODLevels[] = {"HIGH LOD", "MEDIUM LOD", "LOW LOD"};
COMPILE_TIME_ASSERT(ARRAYSIZE(kLODLevels) == NUM_IMPORT_LODS);

static const char *kPrefabs[] = {"hat", "misc", "taunt"};
...
static const char *s_pszMaterialFilePrefixes[] = {"base", "normal", "phongexponent", "selfillum"};
...
static EnvmapInfo_t kEnvmaps[] = {{"none", ""},
...
static EnvmapMaskInfo_t kEnvmapMasks[] = {
	{"none", ""}, {"base alpha", "$basealphaenvmapmask"}, {"normal map alpha", "$normalmapalphaenvmapmask"}};
...
static const char *GetWorkshopSoundScriptFile()
{
	...
	if(GenerateFullPath(kWorkshopSoundScriptFile, "MOD", szItemWorkshopSoundScriptAbsPath,
						ARRAYSIZE(szItemWorkshopSoundScriptAbsPath)))
	{
...
public:
	...
	{
		static const char *pszIgnoredMessages[] = {"CDynamicFunction: "};
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportTextEditDialog::CTFFileImportTextEditDialog(vgui::Panel *parent, const char *pszTitle,
														 const char *pszCommand)
	: Frame(parent, "ImportFileTextEditDialog"),
...
// Purpose:
//-----------------------------------------------------------------------------
CTFImportMaterialEditDialog::CTFImportMaterialEditDialog(vgui::Panel *parent, int nSkinIndex, int nMaterialIndex,
														 KeyValues *pItemValues)
	: Frame(parent, "ImportMaterialEditDialog"),
	  ...
	  m_pItemValues(pItemValues)
{
	...
	{
		KeyValues *pMaterialKey = m_pItemValues->FindKey(
			CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(nSkin), m_nMaterialIndex));
		if(pMaterialKey)
		{
			m_strBaseTextureFile[nSkin] = pMaterialKey->GetString(
				CFmtStr("%s_texture_file", s_pszMaterialFilePrefixes[MATERIAL_FILE_BASETEXTURE]));
			if(!bSetSharedTextures)
			{
				m_strNormalTextureFile = pMaterialKey->GetString(
					CFmtStr("%s_texture_file", s_pszMaterialFilePrefixes[MATERIAL_FILE_NORMAL]));
				m_strPhongExponentTextureFile = pMaterialKey->GetString(
					CFmtStr("%s_texture_file", s_pszMaterialFilePrefixes[MATERIAL_FILE_PHONGEXPONENT]));
				m_strSelfIllumTextureFile = pMaterialKey->GetString(
					CFmtStr("%s_texture_file", s_pszMaterialFilePrefixes[MATERIAL_FILE_SELFILLUM]));
...
void CTFImportMaterialEditDialog::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	if(m_pItemValues->GetBool(pszPaintable))
	{
		...
		static const char *paintableUILabels[] = {"BlendTintByBaseAlphaLabel", "BlendTintColorOverBaseLabel",
												  "ColorTintBaseLabel"};
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::BUILD_RESULT CheckImageSize(const char *fullpath, MATERIAL_FILE_TYPE materialFileType,
												 KeyValues *pMessageVariables)
{
	CAssetTF asset;
...
void CTFImportMaterialEditDialog::OnTextChanged(KeyValues *data)
{
	...
	if(pComboBox)
	{
		...
		else if(pComboBox == m_pEnvmapAlphaMaskComboBox)
		{
			...
			if(FStrEq(pszEnvmapMaskVarName, "$basealphaenvmapmask"))
			{
				...
				{
					bWarned = true;
					ShowMessageBox("#TF_ImportFile_Warning",
								   kWarningMessages[CTFFileImportDialog::WARNING_BASEALPHAMASK]);
				}
			}
			else
			{
	...
	if(pTextEntry)
	{
		...
		else if(pTextEntry == m_pColorTintBaseRedTextEntry || pTextEntry == m_pColorTintBaseGreenTextEntry ||
				pTextEntry == m_pColorTintBaseBlueTextEntry)
		{
			const char *pszColor = CFmtStr("{ %d %d %d }", clamp(m_pColorTintBaseRedTextEntry->GetValueAsInt(), 0, 255),
		...
		else if(pTextEntry == m_pCubemapTintRedTextEntry || pTextEntry == m_pCubemapTintGreenTextEntry ||
		...
		else if(pTextEntry == m_pSelfIllumTintRedTextEntry || pTextEntry == m_pSelfIllumTintGreenTextEntry ||
...
void CTFImportMaterialEditDialog::SetVMTKeyValues(const char *pszRedVMTText, const char *pszBlueVMTText)
{
	...
	if(m_VMTKeyValues->LoadFromBuffer("VMT", pszVMTText))
	{
		...
		if(m_pRimLightExponentTextEntry)
		{
			m_pRimLightExponentTextEntry->SetText(
				CFmtStr(pszFloatFormat, m_VMTKeyValues->GetFloat("$rimlightexponent")));
		}
		if(m_pRimLightBoostTextEntry)
		{
		...
		if(m_pBlendTintColorOverBaseTextEntry)
		{
			m_pBlendTintColorOverBaseTextEntry->SetText(
				CFmtStr(pszFloatFormat, m_VMTKeyValues->GetFloat("$blendtintcoloroverbase")));
		}
...
void CTFImportMaterialEditDialog::UpdateBlendTintColorOverBaseDisplay(bool bEnable)
{
	if(m_pBlendTintColorOverBaseTextEntry)
	{
		...
		m_pBlendTintColorOverBaseTextEntry->SetText(
			CFmtStr("%.3f", m_VMTKeyValues->GetFloat("$blendtintcoloroverbase")));
...
void CTFImportMaterialEditDialog::UpdateSelfIllumDisplay(bool bEnable)
{
	...
	static const char *pszSelfIllumLabels[] = {"SelfIllumTintLabel", "SelfIllumTextureLabel"};
	...
	static const char *pszSelfIllumButtons[] = {"SelfIllumTextureBrowse", "SelfIllumTextureClear"};
...
{
	static const char *pszEnvmapLabels[] = {"EnvmapAlphaMaskLabel", "EnvmapTintLabel"};
...
void CTFImportMaterialEditDialog::UpdateUniqueLabels()
{
	...
	static const char *pszLabelNames[] = {"BaseTextureLabel", "ColorTintBaseLabel", "SelfIllumTintLabel"};
...
protected:
	...
	bool ClassHasModels(int nClassIndex)
	{
		...
		{
			if(V_stricmp(m_pItemValues->GetString(CFmtStr(kClassLODNFile, kClassFolders[nClassIndex], nModelIndex)),
						 "") != 0)
				return true;
		}
		return false;
...
{
	// We want to skip CTFStorePreviewItemPanel2::OnThink() because it closes when the mouse is clicked outside the
	// dialog.
...
void CImportPreviewItemPanel::OnCommand(const char *command)
{
	...
	else if(V_strcasecmp(command, "BuildPreview") == 0 || V_strcasecmp(command, "EditQC") == 0 ||
			V_strcasecmp(command, "EditQCI") == 0 ||
			V_strncasecmp(command, "EditMaterial", V_strlen("EditMaterial")) == 0)
	{
		// Dispatch directly to our parent because the base class tries to run the command through the console
		// interpreter
	...
	if(m_pLoadoutComboBox)
	{
		...
		if(m_iState == PS_PLAYER)
		{
			...
			for(int iLoadoutPosition = 0; iLoadoutPosition < CLASS_LOADOUT_POSITION_COUNT; ++iLoadoutPosition)
			{
				...
				{
					const char *pszLabel = ItemSystem()->GetItemSchema()->GetLoadoutStringsForDisplay(
						pItem->GetItemDefinition()->GetEquipType())[iLoadoutPosition];
	...
	if(m_pPoseComboBox)
	{
		...
		if(m_iState == PS_PLAYER)
		{
			...
			for(int iPose = 0; iPose < ARRAYSIZE(kModelPoses); ++iPose)
			{
				...
				int nRow =
					m_pPoseComboBox->AddItem(CFmtStr("#TF_ImportPreview_Pose%s", kModelPoses[iPose]), pKeyValues);
		...
		if(m_iState == PS_PLAYER)
		{
			...
			if(m_bIsTaunt)
			{
				...
				if(pFileImportDialog)
				{
					...
					const CUtlVector<const char *> &vecUsabilityStrings =
						ItemSystem()->GetItemSchema()->GetClassUsabilityStrings();
					...
					if(pTauntKey)
					{
						...
						if(pCustomTauntPerClass)
						{
							const char *pSceneName =
								pCustomTauntPerClass->GetString(vecUsabilityStrings[m_nSelectedClass]);
...
void CImportPreviewItemPanel::UpdateActivity()
{
	...
	{
		// FIXME: Once we support weapons we'll need to check to see if we want to display the preview item here
		// instead.
...
CTFFileImportDialog::CTFFileImportDialog(vgui::Panel *parent)
	: Frame(parent, "ImportFileDialog"), m_tempQC(0, 0, CUtlBuffer::TEXT_BUFFER)
{
	vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL),
																"resource/ClientScheme.res", "ClientScheme");
	SetScheme(scheme);
...
void CTFFileImportDialog::OnCommandLoad()
{
	...
	if(g_pFullFileSystem->RelativePathToFullPath_safe(CFmtStr(kSteamWorkshopDir, GetWorkshopFolder()), "GAME",
													  pszStartPath))
	{
		pDialog->SetStartDirectory(pszStartPath);
	}
...
void CTFFileImportDialog::OnCommandBrowseAnimationSource()
{
	vgui::FileOpenDialog *pDialog =
		new vgui::FileOpenDialog(NULL, "#TF_ImportFile_SelectAnimationSource", vgui::FOD_OPEN);
...
void CTFFileImportDialog::OnCommandSwapVMT()
{
	...
	{
		KeyValues *pVMT0 =
			GetItemValues()->FindKey(CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(nSkin), 0));
		KeyValues *pVMT1 =
			GetItemValues()->FindKey(CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(nSkin), 1));
		if(pVMT0 && pVMT1)
		{
...
{
	CTFImportMaterialEditDialog *pDialog =
		new CTFImportMaterialEditDialog(this, nSkinIndex, nMaterialIndex, GetItemValues());
	pDialog->InvalidateLayout(true, true);

	CUtlBuffer sRedMaterialText, sBlueMaterialText;
	pDialog->SetVMTKeyValues(GetMaterialText(0, nMaterialIndex, sRedMaterialText),
							 GetMaterialText(1, nMaterialIndex, sBlueMaterialText));
	pDialog->DoModal();
	pDialog->Activate();
...
void CTFFileImportDialog::OnCommandEditMaterialDone(int nSkinIndex, int nMaterialIndex)
{
	...
	for(int nSkin = 0; nSkin < nNumSkin; ++nSkin)
	{
		...
		{
			ShowMessageBoxWithFile("#TF_SteamWorkshop_Error", kBuildResultMessages[nResult],
								   CFmtStr("VMT%d", 1 + nMaterialIndex));
			return;
		}
		...
		oldPhongExponentTextures.CopyAndAddToTail(
			GetMaterialTextureFile(nSkin, nMaterialIndex, MATERIAL_FILE_PHONGEXPONENT));
		...
		SetMaterial(nSkin, nMaterialIndex, m_pMaterialEditDialog->GetPhongExponentTextureFile(),
...
void CTFFileImportDialog::OnCommandEditQCI()
{
	...
	CTFFileImportTextEditDialog *pDialog =
		new CTFFileImportTextEditDialog(this, "#TF_ImportFile_EditQCI", "EditQCIDone");
	...
	if(nResult == BUILD_OKAY)
	{
		switch(buildStage)
		{
			...
			case BUILD_FINAL:
			{
				...
				ShowMessageBoxWithFile("#TF_ImportFile_ImportCompleteTitle", "#TF_ImportFile_ImportComplete",
									   pszBuildOutput);
				SetWorkshopData();
...
void CTFFileImportDialog::OnClose()
{
	...
	{
		char pszSessionPath[MAX_PATH];
		V_ComposeFileName(CFmtStr(kSteamWorkshopDir, GetWorkshopFolder()), CFmtStr("%s.txt", sName.Get()),
						  pszSessionPath, sizeof(pszSessionPath));
...
void CTFFileImportDialog::SetItemIcon(const char *pszFilePath)
{
	if(pszFilePath && *pszFilePath && m_pIconImagePanel)
	{
		...
		{
			ShowMessageBoxWithFile("#TF_ImportFile_LoadFailed", "#TF_ImportFile_LoadFailedBadIconResolution",
								   pszFilePath);
			return;
		}
		...
		for(int y = 0; y < 512; ++y)
		{
			...
			for(int x = 0; x < 512; ++x)
			{
				...
				else
				{
					...
					{
						ShowMessageBox("#TF_ImportFile_LoadFailed",
									   "#TF_ImportFile_LoadFailedImageDoesNotFitInsideSafeZone");
						return;
					}
				}
			}
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::LOAD_RESULT CTFFileImportDialog::SetLOD(int selectedClass, int nModelIndex,
															 const char *pszFilePath, KeyValues *pKV /*= NULL*/)
{
	...
	if(*pszFilePath)
	{
		...
		{
			KeyValues *pKey =
				GetItemValues()->FindKey(CFmtStr(kClassLODN, kClassFolders[selectedClass], nModelIndex - 1));
			Assert(pKey);
			if(pKey)
			{
		...
		KeyValues *pKey =
			GetItemValues()->FindKey(CFmtStr(kClassLODN, kClassFolders[selectedClass], nModelIndex), true);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFFileImportDialog::SetMaterial(int selectedSkin, int nMaterialIndex, const char *pszFilePath,
									  MATERIAL_FILE_TYPE fileType)
{
	if(*pszFilePath)
	{
		CAssetTF asset;
		...
		KeyValues *pKey = GetItemValues()->FindKey(
			CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(selectedSkin), nMaterialIndex), true);
	...
	{
		static const char *s_removeKeyNames[] = {"%s_texture_file", "%s_texture_width", "%s_texture_height",
												 "%s_texture_channels", "%s_texture_alpha"};

		KeyValues *pKey = GetItemValues()->FindKey(
			CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(selectedSkin), nMaterialIndex));
		if(pKey)
		{
			const char *pszMaterialFilePrefix = s_pszMaterialFilePrefixes[fileType];
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CTFFileImportDialog::GetMaterialTextureFile(int selectedSkin, int nMaterialIndex,
														MATERIAL_FILE_TYPE fileType)
{
	KeyValues *pMaterialKey = GetItemValues()->FindKey(
		CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(selectedSkin), nMaterialIndex));
	if(pMaterialKey)
	{
		const char *pszMaterialFilePrefix = s_pszMaterialFilePrefixes[fileType];
...
{
	KeyValues *pKey = GetItemValues()->FindKey(
		CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(nSkinIndex), nMaterialIndex));
	if(pKey)
	{
		pKey->Clear();
	}
...
void CTFFileImportDialog::UpdateMaterialDisplay(int nSkinIndex, int nMaterialIndex)
{
	...
	if(pLabel)
	{
		...
		pLabel->SetText(
			CFmtStr("%s: \"%s\"", pszInputName, GetItemValues()->GetString(CFmtStr(kMaterialN, nMaterialIndex))));
	}
			...
			if(pVMTKey->LoadFromBuffer("VMT", pKey->GetString("vmt")))
			{
				...
				else
				{
					...
					if(pCurrentProxies)
					{
						...
						if(pKVTemplate->LoadFromFile(g_pFullFileSystem,
													 "materials/models/player/items/templates/standard.vmt", "GAME"))
						{
...
{
	KeyValues *pKey = GetItemValues()->FindKey(
		CFmtStr(kMaterialSkinN, CItemUpload::Manifest()->GetMaterialSkin(nSkinIndex), nMaterialIndex), true);
	if(pKey)
	{
...
CTFFileImportDialog::BUILD_RESULT CTFFileImportDialog::ValidateMaterialValues(KeyValues *pKV, int nMaterialIndex)
{
	...
	static struct MaterialCheck
	{
		...
		{BUILD_FAILED_MATERIALMISSINGBURNING,
		 0,
		 {"$detail", "$detailscale", "$detailblendfactor", "$detailblendmode", "Proxies/AnimatedTexture",
		  "Proxies/BurnLevel"}},
		{BUILD_FAILED_MATERIALMISSINGJARATE,
		 CONDITION_NOTPAINTABLE,
...
{
	static const char *vecLightingKeys[] = {"$basemapalphaphongmask", "$halflambert"};
...
void CTFFileImportDialog::RemovePaintParameters(KeyValues *pKV, int nMaterialIndex)
{
	if(IsPaintable(nMaterialIndex))
	{
		static const char *vecNonPaintableProxyKeys[] = {"Equals"};
	...
	static const char *vecPaintableKeys[] = {"$blendtintbybasealpha", "$blendtintcoloroverbase", "$colortint_base",
											 "$colortint_tmp"};
	static const char *vecPaintableProxyKeys[] = {"ItemTintColor", "SelectFirstIfNonZero", "Multiply"};
...
{
	static const char *vecEnvmapKeys[] = {"$envmap", "$basealphaenvmapmask", "$normalmapalphaenvmapmask",
										  "$envmaptint"};
...
{
	static const char *vecSelfIllumKeys[] = {"$selfillum", "$selfillumtint", "$selfillummask"};
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::LOAD_RESULT CTFFileImportDialog::SetAnimationSource(int selectedClass, const char *pszFilePath,
																		 KeyValues *pKV /*= NULL*/)
{
	if(selectedClass == TF_CLASS_UNDEFINED)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::LOAD_RESULT CTFFileImportDialog::SetAnimationVCD(int selectedClass, const char *pszFilePath,
																	  KeyValues *pKV /*= NULL*/)
{
	if(selectedClass == TF_CLASS_UNDEFINED)
	{
...
void CTFFileImportDialog::UpdateAnimationSourceDisplay()
{
	...
	{
		const char *pszFilePath =
			GetItemValues()->GetString(CFmtStr(kClassAnimationSourceFile, kClassFolders[m_nSelectedClass]));
		if(pszFilePath && *pszFilePath)
		{
			char file[MAX_PATH];
			V_FileBase(pszFilePath, file, sizeof(file));
...
void CTFFileImportDialog::UpdateAnimationVCDDisplay()
{
	...
	{
		const char *pszFilePath =
			GetItemValues()->GetString(CFmtStr(kClassAnimationVCDFile, kClassFolders[m_nSelectedClass]));
		if(pszFilePath && *pszFilePath)
		{
			char file[MAX_PATH];
			V_FileBase(pszFilePath, file, sizeof(file));
...
void CTFFileImportDialog::UpdateAnimDurationDisplay()
{
	if(m_pAnimationDurationLabel)
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wszTemp, g_pVGuiLocalize->Find("#TF_ImportFile_AnimationDuration"), 1,
											  wszCount);
...
{
	static const char *vecTextureExtensions[] = {".tga", ".psd"};
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::BUILD_RESULT CTFFileImportDialog::AddTauntToAsset(CAssetTF &asset, int nClassIndex,
																	   bool bIsMulticlass, BUILD_STAGE buildStage,
																	   KeyValues *pItemData,
																	   KeyValues *pBuildMessageVariables)
{
	const uint nPathFlags =
		(CTargetBase::PATH_FLAG_ALL & ~CTargetBase::PATH_FLAG_ABSOLUTE) | CTargetBase::PATH_FLAG_ZIP;
	...
	const char *pszCustomRelativeDir =
		CFmtStr("%s/player/%s/low", GetWorkshopFolder(), vecUsabilityStrings[nClassIndex]);
	...
	const char *pszAnimMDLMiddleName =
		bPerforce ? "workshop"
				  : "user"; // use class_user_animations as temp MDL and class_workshop_animations as shipping MDL
	...
	// Dir for QC
	pszCustomRelativeDir =
		bPerforce ? CFmtStr("%s/player/animations", GetWorkshopFolder()) : CFmtStr("player/%s", pszClassFolder);
	const char *pszAnimQCMiddleName =
		bPerforce ? "workshop"
				  : "preview"; // use class_preview_animations as temp QC and class_workshop_animations as shipping QC
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::BUILD_RESULT CTFFileImportDialog::AddModelToAsset(CAssetTF &asset, int nClassIndex,
																	   bool bIsMulticlass, BUILD_STAGE buildStage,
																	   KeyValues *pItemData,
																	   KeyValues *pBuildMessageVariables)
{
	...
	if(!bPreview && nLastSpecifiedTriCount > GetModelTriangleBudget(nClassIndex, NUM_IMPORT_LODS - 1))
	{
		...
		const char *pszFilePath =
			GetItemValues()->GetString(CFmtStr(kClassLODNFile, pszClassFolder, nLastSpecifiedLOD));
		SetMessageFileVariable(pBuildMessageVariables, pszFilePath);
...
CTFFileImportDialog::BUILD_RESULT CTFFileImportDialog::AddMaterialsToAsset(CAssetTF &asset, KeyValues *pItemData,
																		   KeyValues *pBuildMessageVariables)
{
	const uint nPathFlags =
		(CTargetBase::PATH_FLAG_ALL & ~CTargetBase::PATH_FLAG_ABSOLUTE) | CTargetBase::PATH_FLAG_ZIP;
		...
		if(pVMT)
		{
			...
			for(int nSkinIndex = 0; nSkinIndex < CItemUpload::Manifest()->GetNumMaterialSkins(); ++nSkinIndex)
			{
				...
				for(int nTextureType = 0; nTextureType < CItemUpload::Manifest()->GetNumTextureTypes(); ++nTextureType)
				{
					...
					BUILD_RESULT checkImageResult =
						CheckImageSize(pszTexturePath, materialFileType, pBuildMessageVariables);
					...
					if(bBaseAlphaMask && FStrEq(pszTextureType, "_normal"))
					{
						...
						// Base alpha mask will not work in materials using a normal map, use normal map alpha mask
						// instead
						continue;
					}
				...
				for(int iVTF = 0; iVTF < pVMT->GetNumTargetVTFS(nSkinIndex); ++iVTF)
				{
					...
					if(pVTF)
					{
						...
						sOutputPath = asset.CheckRedundantOutputFilePath(pVTF->GetTargetTGA()->GetInputFile().String(),
																		 "", sOutputPath.String());
						pItemData->SetString(CFmtStr(kMaterialSkinNFile,
													 CItemUpload::Manifest()->GetMaterialSkin(nSkinIndex),
													 nValidVMTIndex, s_pszMaterialFilePrefixes[iVTF]),
											 sOutputPath);
					}
				}
			}
...
CTFFileImportDialog::BUILD_RESULT CTFFileImportDialog::Build(BUILD_STAGE buildStage, KeyValues *pBuildMessageVariables)
{
	...
	for(int nClassIndex = TF_FIRST_NORMAL_CLASS; nClassIndex < TF_LAST_NORMAL_CLASS; ++nClassIndex)
	{
		...
		{
			buildResult =
				AddTauntToAsset(asset, nClassIndex, bIsMulticlass, buildStage, pItemData, pBuildMessageVariables);
			...
			{
				buildResult =
					AddModelToAsset(asset, nClassIndex, bIsMulticlass, buildStage, pItemData, pBuildMessageVariables);
				if(buildResult != BUILD_OKAY)
					return buildResult;
			}
		}
...
KeyValues *CTFFileImportDialog::BuildItemSchema(const char *pszItemName)
{
	...
	pData->SetString(
		"name", pszTFEnglishName); // TODO: Change this when we add a ship panel to fill in name from tf_english.txt
	...
	if(m_nPrefab != PREFAB_TAUNT)
	{
		...
		for(nClassIndex = TF_FIRST_NORMAL_CLASS; nClassIndex < TF_LAST_NORMAL_CLASS; ++nClassIndex)
		{
			...
			const char *pszFilePath =
				GetItemValues()->GetString(CFmtStr(kClassLODNFile, kClassFolders[nClassIndex], 0));
			if(*pszFilePath)
			{
				int nLOD = asset.AddTargetDMX(pszFilePath);
				if(nLOD < 0)
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::LOAD_RESULT CTFFileImportDialog::Load(const char *pszFilePath, const char *pathID,
														   CUtlString &sFailedPath)
{
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::LOAD_RESULT CTFFileImportDialog::LoadTxt(const char *pszFilePath, const char *pathID,
															  CUtlString &sFailedPath)
{
	...
	for(int nClassIndex = TF_FIRST_NORMAL_CLASS; nClassIndex < TF_LAST_NORMAL_CLASS; ++nClassIndex)
	{
		...
		for(int nModelIndex = 0; nModelIndex < NUM_IMPORT_LODS; ++nModelIndex)
		{
			const char *pszModelFile =
				pItemData->GetString(CFmtStr(kClassLODNFile, kClassFolders[nClassIndex], nModelIndex));
			if(*pszModelFile)
			{
	...
	for(int nClassIndex = TF_FIRST_NORMAL_CLASS; nClassIndex < TF_LAST_NORMAL_CLASS; ++nClassIndex)
	{
		...
		const char *pszSourceFile =
			pItemData->GetString(CFmtStr(kClassAnimationSourceFile, kClassFolders[nClassIndex]));
		if(*pszSourceFile)
		{
	...
	for(int nSkinIndex = 0; nSkinIndex < CItemUpload::Manifest()->GetNumMaterialSkins(); ++nSkinIndex)
	{
		for(int nMaterialIndex = 0; nMaterialIndex < NUM_IMPORT_MATERIALS_PER_TEAM; ++nMaterialIndex)
		{
			...
			const char *pszMaterialVMT =
				pItemData->GetString(CFmtStr(kMaterialSkinNVMT, pszMaterialSkin, nMaterialIndex));
			const char *pszMaterialFile =
				pItemData->GetString(CFmtStr(kMaterialSkinNFile, pszMaterialSkin, nMaterialIndex,
											 s_pszMaterialFilePrefixes[MATERIAL_FILE_BASETEXTURE]));
			...
			for(int nTextureType = 0; nTextureType < CItemUpload::Manifest()->GetNumTextureTypes(); ++nTextureType)
			{
				const char *pszMaterialTextureFile = pItemData->GetString(CFmtStr(
					kMaterialSkinNFile, pszMaterialSkin, nMaterialIndex, s_pszMaterialFilePrefixes[nTextureType]));
...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportDialog::LOAD_RESULT CTFFileImportDialog::LoadZip(const char *pszFilePath, const char *pathID,
															  CUtlString &sFailedPath)
{
...
bool CTFFileImportDialog::ClassHasModels(int nClassIndex)
{
	for(int nModelIndex = 0; nModelIndex < NUM_IMPORT_LODS; ++nModelIndex)
	{
		if(V_stricmp(GetItemValues()->GetString(CFmtStr(kClassLODNFile, kClassFolders[nClassIndex], nModelIndex)),
					 "") != 0)
			return true;
	}
	return false;
	...
	for(int nClassIndex = TF_FIRST_NORMAL_CLASS; nClassIndex < TF_LAST_NORMAL_CLASS; ++nClassIndex)
	{
		...
		if(pKey)
		{
			...
			for(int nSkinIndex = 0; nSkinIndex <= nSkinType; ++nSkinIndex)
			{
				...
				{
					const char *pszMaterialFile = GetItemValues()->GetString(
						CFmtStr(kMaterialSkinNFile, CItemUpload::Manifest()->GetMaterialSkin(nSkinIndex),
								nMaterialIndex, s_pszMaterialFilePrefixes[MATERIAL_FILE_BASETEXTURE]));
...
void CTFFileImportDialog::SetWorkshopData()
{
	...
	if(pClassUsagePanel)
	{
		for(int nClassIndex = TF_FIRST_NORMAL_CLASS; nClassIndex < TF_LAST_NORMAL_CLASS; nClassIndex++)
		{
			SetChildButtonSelected(pClassUsagePanel, VarArgs("ClassCheckBox%d", nClassIndex),
								   ClassHasModels(nClassIndex));
		}
	}
}
	...
	if(m_pEquipRegionComboBox)
	{
		...
		for(int i = 0; i < m_pEquipRegionComboBox->GetItemCount(); ++i)
		{
			KeyValues *pKeyValues =
				m_pEquipRegionComboBox->GetItemUserData(m_pEquipRegionComboBox->GetItemIDFromRow(i));
...
bool CTFFileImportDialog::IsTFEnglishNameValid(item_definition_index_t defIndex)
{
	...
	if(m_pTFEnglishNameTextEntry->IsVisible())
	{
		...
		FOR_EACH_MAP_FAST(mapItemDefs, i)
		{
			...
			if(existingItemDefIndex != 0 && existingItemDefIndex != defIndex &&
			   FStrEq(pszExistingItemName, pszTFEnglishName))
			{
				return false;
			}
		}
