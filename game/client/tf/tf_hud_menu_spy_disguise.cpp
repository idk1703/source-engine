...
ConVar tf_simple_disguise_menu("tf_simple_disguise_menu", NULL, FCVAR_ARCHIVE,
							   "Use a more concise disguise selection menu.");
...
ConVar tf_disguise_menu_controller_mode("tf_disguise_menu_controller_mode", "0", FCVAR_ARCHIVE,
										"Use console controller disguise menus. 1 = ON, 0 = OFF.");
...
int CHudMenuSpyDisguise::HudElementKeyInput(int down, ButtonCode_t keynum, const char *pszCurrentBinding)
{
	...
	static int iRemapKeyToClass[9] = {TF_CLASS_SCOUT,	TF_CLASS_SOLDIER,	   TF_CLASS_PYRO,
...
void CHudMenuSpyDisguise::SetVisible(bool state)
{
	if(state == true)
	{
		...
		bool bConsoleMode =
			(IsConsole() || tf_disguise_menu_controller_mode.GetBool() || ::input->IsSteamControllerActive());
