...
void TF_EjectBrassCallback(const CEffectData &data)
{
	...
	Vector vecVelocity = random->RandomFloat(130, 180) * vForward + random->RandomFloat(-30, 30) * vRight +
	...
	if(data.m_nHitBox == TF_WEAPON_MINIGUN)
	{
		...
		vecVelocity = random->RandomFloat(130, 250) * vForward + random->RandomFloat(-100, 100) * vRight +
	...
	C_LocalTempEntity *pTemp =
		tempents->SpawnTempModel(pModel, data.m_vOrigin, vecShellAngles, vecVelocity, flLifeTime, FTENT_NEVERDIE);
	if(pTemp == NULL)
		return;
