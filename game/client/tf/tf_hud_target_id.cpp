...
ConVar tf_spectator_target_location("tf_spectator_target_location", "0", FCVAR_ARCHIVE,
									"Determines the location of the spectator targetID panel.", true, 0, true, 3,
									SpectatorTargetLocationCallback);
...
ConVar tf_hud_target_id_alpha("tf_hud_target_id_alpha", "100", FCVAR_ARCHIVE,
							  "Alpha value of target id background, default 100");
ConVar tf_hud_target_id_offset("tf_hud_target_id_offset", "0", FCVAR_ARCHIVE, "RES file Y offset for target id");
ConVar tf_hud_target_id_show_avatars(
	"tf_hud_target_id_show_avatars", "2", FCVAR_ARCHIVE,
	"Display Steam avatars on TargetID when using floating health icons.  1 = everyone, 2 = friends only.");

#ifdef STAGING_ONLY
ConVar tf_bountymode_showhealth(
	"tf_bountymode_showhealth", "0", FCVAR_ARCHIVE,
	"Show floating health icon over enemy players.  1 = show health, 2 = show health and level", true, 0, true, 2);
#endif // STAGING_ONLY
...
// Purpose:
//-----------------------------------------------------------------------------
CTargetID::CTargetID(const char *pElementName) : CHudElement(pElementName), BaseClass(NULL, pElementName)
{
...
C_TFPlayer *CTargetID::GetTargetForSteamAvatar(C_TFPlayer *pTFPlayer)
{
	...
	if(pTFPlayer->IsPlayerClass(TF_CLASS_SPY) && pTFPlayer->m_Shared.InCond(TF_COND_DISGUISED))
	{
		...
		if(pDisguiseTarget && (pTFLocalPlayer->InSameTeam(pDisguiseTarget) || pDisguiseTarget == pTFLocalPlayer))
		{
			...
			if(tf_hud_target_id_show_avatars.GetInt() == 2 &&
			   !pTFLocalPlayer->IsPlayerOnSteamFriendsList(pDisguiseTarget))
				return NULL;
...
void CTargetID::ApplySchemeSettings(vgui::IScheme *scheme)
{
	...
	if(m_pMoveableSubPanel)
	{
		...
		m_pMoveableSymbolIcon =
			dynamic_cast<vgui::ImagePanel *>(m_pMoveableSubPanel->FindChildByName("MoveableSymbolIcon"));
	...
	if(nEntIndex)
	{
		...
		if(pEnt)
		{
			...
			if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
			{
				...
				if(pLocalTFPlayer->m_Shared.InCond(TF_COND_DISGUISED) &&
				   pLocalTFPlayer->m_Shared.GetDisguiseTeam() != pLocalTFPlayer->GetTeamNumber())
				{
			...
			if(pPlayer)
			{
				...
				if(bHealthBarVisible)
				{
...
#ifdef STAGING_ONLY
					bShow |= TFGameRules() && TFGameRules()->IsBountyMode() && tf_bountymode_showhealth.GetInt() &&
							 bEnemyPlayer;
#endif // STAGING_ONLY
					...
					if(bShow)
					{
						...
						// Minibosses keep the health indicator up within a small FOV until a different valid target is
						// selected The FOV needs to grow exponentially when a target is getting near
						if(bEnemyMiniBoss)
						{
							bMaintainInFOV = true;
						}
				...
				bReturn = (bSpectator || pLocalTFPlayer->InSameTeam(pEnt) ||
						   ((bInSameTeam || bSpy || iSeeEnemyHealth) && !bStealthed));
			}
...
void CTargetID::PerformLayout(void)
{
	...
	if(m_pMoveableSubPanel && m_pMoveableSubPanel->IsVisible())
	{
		if(m_pMoveableKeyLabel && m_pMoveableIcon && m_pMoveableSymbolIcon && m_pMoveableIconBG)
		{
			...
			int iIndent = XRES(4);
			int iMoveWide = MAX(XRES(16) + m_pMoveableKeyLabel->GetWide() + iIndent,
								(m_pMoveableIcon->GetWide()) + iIndent + XRES(8));
			...
			m_pMoveableSymbolIcon->SetPos((iMoveWide - m_pMoveableSymbolIcon->GetWide()) * 0.5,
										  y - m_pMoveableSymbolIcon->GetTall());
	...
	int nOffset =
		m_bArenaPanelVisible ? YRES(120) : 0; // HACK: move the targetID up a bit so it won't overlap the panel
	if(UseVR())
	{
	...
	if(m_iTargetEntIndex)
	{
		...
		if(IsPlayerIndex(m_iTargetEntIndex))
		{
			...
			if(pPlayer->m_Shared.InCond(TF_COND_DISGUISED) && // they're disguised
															  ...
															  //! they're not in the process of disguising
			...
			if(bDisguisedTarget)
			{
				...
				if(pPlayer->IsEnemyPlayer())
				{
					if(pDisguiseTarget)
					{
						...
						g_pVGuiLocalize->ConvertANSIToUnicode(pDisguiseTarget->GetPlayerName(), wszPlayerName,
			...
			pPlayer->GetTargetIDDataString(bDisguisedTarget, sDataString, sizeof(sDataString), bIsAmmoData,
										   bIsKillStreakData);
			if(pLocalTFPlayer->GetTeamNumber() == TEAM_SPECTATOR || bInSameTeam || bSpy || bDisguisedEnemy || bMedic ||
			   bHeavy)
			{
			...
			if(printFormatString)
			{
				...
				g_pVGuiLocalize->ConstructString_safe(sIDString, g_pVGuiLocalize->Find(printFormatString), 2,
													  pszPrepend, wszPlayerName);
			}

			// Show target's clip state to attached medics
			bool bShowClipInfo =
				bIsAmmoData && sDataString[0] && ToTFPlayer(pLocalTFPlayer->MedicGetHealTarget()) == pPlayer;
		...
		else
		{
				...
				if(pObj->GetBuilder() == pLocalTFPlayer)
				{
					...
					if(iObj >= OBJ_DISPENSER && iObj <= OBJ_SENTRYGUN)
					{
						...
						switch(iObj)
						{
							...
							case OBJ_TELEPORTER:
							{
								pszActionIcon = (pObj->GetObjectMode() == MODE_TELEPORTER_ENTRANCE)
			...
			else if(pEnt->IsVisibleToTargetID())
			{
				...
				{
					bShowHealth = false;
					g_pVGuiLocalize->ConvertANSIToUnicode(CFmtStr("%d Points", pFlag->GetPointValue()), sIDString,
														  sizeof(sIDString));
				}
				else
				{
					...
					if(pDroppedWeapon)
					{
						...
						if(FStrEq(pDroppedEconItem->GetStaticData()->GetItemClass(), "tf_weapon_medigun"))
						{
							...
							_snwprintf(wszChargeLevel, ARRAYSIZE(wszChargeLevel) - 1, L"%.0f",
									   pDroppedWeapon->GetChargeLevel() * 100);
							wszChargeLevel[ARRAYSIZE(wszChargeLevel) - 1] = '\0';

							g_pVGuiLocalize->ConstructString_safe(
								sIDString, L"%s1 (%s2%)", 2,
								CEconItemLocalizedFullNameGenerator(GLocalizationProvider(),
																	pDroppedEconItem->GetItemDefinition(),
																	pDroppedEconItem->GetItemQuality())
									.GetFullName(),
								wszChargeLevel);
						}
						else
						...
						if(pOwner)
						{
							g_pVGuiLocalize->ConvertANSIToUnicode(pOwner->GetPlayerName(), wszPlayerName,
																  sizeof(wszPlayerName));
							g_pVGuiLocalize->ConstructString_safe(sDataString, g_pVGuiLocalize->Find("#TF_WhoDropped"),
																  1, wszPlayerName);
							...
							if(pScheme)
							{
								const char *pszColorName =
									GetItemSchema()->GetRarityColor(pDroppedEconItem->GetItemDefinition()->GetRarity());
					...
					else if(pLocalTFPlayer->InSameTeam(pEnt))
					{
						...
						if(!pEnt->IsPlayer())
						{
							...
							{
								float flRespawn =
									TFGameRules()->GetNextRespawnWave(pMarker->GetTeamNumber(), pMarker->GetOwner()) -
									gpGlobals->curtime;
								m_pTargetHealth->SetLevel((int)flRespawn);

								g_pVGuiLocalize->ConvertANSIToUnicode(pMarker->GetOwner()->GetPlayerName(), sIDString,
																	  sizeof(sIDString));
							}
						}
					}
...
bool CSpectatorTargetID::ShouldDraw(void)
{
	...
	if(pLocalTFPlayer->GetObserverMode() <= OBS_MODE_NONE || pLocalTFPlayer->GetObserverMode() == OBS_MODE_FREEZECAM)
		return false;
...
int CSpectatorTargetID::CalculateTargetIndex(C_TFPlayer *pLocalTFPlayer)
{
	...
	if((!g_pEngineClientReplay || !g_pEngineClientReplay->IsPlayingReplayDemo()) &&
	   pLocalTFPlayer->GetObserverMode() == OBS_MODE_IN_EYE && pLocalTFPlayer->GetObserverTarget())
#else
...
// Purpose:
//-----------------------------------------------------------------------------
ConVar tf_healthicon_height_offset("tf_healthicon_height_offset", "10", FCVAR_ARCHIVE,
								   "Offset of the health icon away from the top of the target.");
...
bool CFloatingHealthIcon::CalculatePosition()
{
	...
	vecTarget.z += VEC_HULL_MAX_SCALED(m_hEntity->GetBaseAnimating()).z + tf_healthicon_height_offset.GetInt() +
				   m_hEntity->GetHealthBarHeightOffset();
