...
#ifdef STAGING_ONLY
ConVar tf_tips_show_captain_canteen("tf_tips_show_captain_canteen", 0, FCVAR_NONE,
									"Show Captain Canteen when displaying tips");
#endif
	...
	if(pMVMStats != NULL)
	{
		if(m_pCollectionContainer)
		{
			m_pCollectionContainer->SetDialogVariable("creditscollected",
													  (int)pMVMStats->GetAcquiredCredits(-1, false));
...
void CMvMWaveLossPanel::ShowPanel()
{
	...
	if(TFObjectiveResource())
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wszLocalizedWave, g_pVGuiLocalize->Find("#TF_PVE_WaveCountFail"), 1,
											  wszWaveNumber);
		...
		if(GetItemSchema()->GetMvmMissions().IsValidIndex(iChallengeIndex))
		{
			...
			g_pVGuiLocalize->ConstructString_safe(
				wszLocalizedSummary, g_pVGuiLocalize->Find("#TF_PVE_MissionSummaryScheme"), 2,
		...
		else
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszLocalizedSummary,
												  g_pVGuiLocalize->Find("#TF_PVE_MissionSummaryDefault"), 1, wszName);
		}
