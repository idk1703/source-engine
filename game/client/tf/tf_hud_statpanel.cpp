...
TFStatType_t g_statPriority[] = {
	TFSTAT_HEADSHOTS,	  TFSTAT_BACKSTABS,	   TFSTAT_MAXSENTRYKILLS, TFSTAT_HEALING,	  TFSTAT_KILLS,
	TFSTAT_KILLASSISTS,	  TFSTAT_DAMAGE,	   TFSTAT_DOMINATIONS,	  TFSTAT_INVULNS,	  TFSTAT_BUILDINGSDESTROYED,
	TFSTAT_CAPTURES,	  TFSTAT_DEFENSES,	   TFSTAT_REVENGE,		  TFSTAT_TELEPORTS,	  TFSTAT_BUILDINGSBUILT,
	TFSTAT_HEALTHLEACHED, TFSTAT_POINTSSCORED, TFSTAT_PLAYTIME,		  TFSTAT_BONUS_POINTS};
// stat types that we don't display records for, kept in this list just so we can assert all stats appear in one list or
// the other
TFStatType_t g_statUnused[] = {TFSTAT_DEATHS,
							   TFSTAT_UNDEFINED,
							   TFSTAT_SHOTS_FIRED,
							   TFSTAT_SHOTS_HIT,
							   TFSTAT_FIREDAMAGE,
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTFStatPanel::CTFStatPanel(const char *pElementName) : EditablePanel(NULL, "StatPanel"), CHudElement(pElementName)
{
...
int CTFStatPanel::CalcCRC(int iSteamID)
{
	...
	for(int iClass = TF_FIRST_NORMAL_CLASS; iClass <= TF_LAST_NORMAL_CLASS; iClass++)
	{
		...
		// since the class data structure is highly guessable from the file, add one other thing to make the CRC hard to
		// hack w/o code disassembly
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFStatPanel::ShowStatPanel(int iClass, int iTeam, int iCurStatValue, TFStatType_t statType,
								 RecordBreakType_t recordBreakType, bool bAlive)
{
	bool bMVM = TFGameRules() && TFGameRules()->IsMannVsMachineMode();
	...
	if(recordBreakType == RECORDBREAK_CLOSE)
	{
		...
		GetStatValueAsString(bMVM ? classStats.maxMVM.m_iStat[statType] : classStats.max.m_iStat[statType], statType,
							 szBest, ARRAYSIZE(szBest));
	...
	SetDialogVariable("statdesc",
					  g_pVGuiLocalize->Find(CFmtStr(
						  "%s_%s", bMVM ? g_szLocalizedMVMRecordText[statType] : g_szLocalizedRecordText[statType],
...
void CTFStatPanel::ResetStats(void)
{
	...
	// Otherwise, our stat panel stats, stat file, and local steam context stats will be out of sync because UploadStats
	// will fail.
...
float CTFStatPanel::GetTotalHoursPlayed(void)
{
	...
	for(int iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_LAST_NORMAL_CLASS; iClass++)
	{
		totalTimePlayed += GetClassStats(iClass).accumulated.m_iStat[TFSTAT_PLAYTIME] +
						   GetClassStats(iClass).accumulatedMVM.m_iStat[TFSTAT_PLAYTIME];
	}
...
void TestStatPanel(const CCommand &args)
{
	...
	if(args.ArgC() < 2)
	{
		...
		ConMsg("Usable panel types are %d to %d. Record types are %d to %d\n", TFSTAT_FIRST, TFSTAT_LAST,
			   RECORDBREAK_NONE + 1, RECORDBREAK_MAX - 1);
		return;
	}
	...
	if(statPanel)
	{
		...
		if((iPanelType <= TFSTAT_UNDEFINED) || (iPanelType >= TFSTAT_TOTAL) || (iRecordType <= RECORDBREAK_NONE) ||
		   (iRecordType >= RECORDBREAK_MAX))
		{
			ConMsg("Usage:  teststatpanel < panel type > < optional record type >\n");
			ConMsg("Usable panel types are %d to %d. Record types are %d to %d\n", TFSTAT_FIRST, TFSTAT_LAST,
				   RECORDBREAK_NONE + 1, RECORDBREAK_MAX - 1);
