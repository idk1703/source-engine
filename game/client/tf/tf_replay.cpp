...
CTFReplay::CTFReplay() : m_flNextMedicUpdateTime(0.0f) {}
...
void CTFReplay::FireGameEvent(IGameEvent *pEvent)
{
	...
	if(FStrEq(pEvent->GetName(), "player_death"))
	{
		...
		if(nKillerID == pLocalPlayer->GetUserID() && !bSuicide && !bSentry)
		{
			// Domination?
		...
		// Player death?
		else if(pKiller && nVictimID == pLocalPlayer->GetUserID())
		{
			...
			params.m_flDelay =
				replay_playerdeathscreenshotdelay.IsValid() ? replay_playerdeathscreenshotdelay.GetFloat() : 0.0f;
			params.m_nEntity = pLocalPlayer->entindex();
			params.m_posCamera.Init(
				0, 0, replay_deathcammaxverticaloffset.IsValid() ? replay_deathcammaxverticaloffset.GetFloat() : 150);
		...
		// Killer is invuln/crit boosted and healer is local player?
				...
				(pKiller->m_Shared.IsCritBoosted() || pKiller->m_Shared.InCond(TF_COND_INVULNERABLE) ||
		...
		else if(pLocalPlayer->IsAlive() && bSentry && pSentry && pSentry->GetOwner() == pLocalPlayer && pVictim)
		{
			...
			static float s_aSentryEyeLevels[3] = {SENTRYGUN_EYE_OFFSET_LEVEL_1[2], SENTRYGUN_EYE_OFFSET_LEVEL_2[2],
												  SENTRYGUN_EYE_OFFSET_LEVEL_3[2]};
			...
			Vector vecSentryAimDir; // Since it seems the sentry's *actual* aim direction is only available on the
									// server, use the victim's location to calculate a general idea of one
			Vector vecVictimUp = pVictim->WorldSpaceCenter() -
								 pVictim->GetAbsOrigin(); // WorldSpaceCenter() seems to return player's eye level
			...
			Vector vecOffset;
			if(replay_sentrycamoffset_frontback.IsValid() && replay_sentrycamoffset_leftright.IsValid() &&
			   replay_sentrycamoffset_updown.IsValid())
			{
				vecOffset.Init(replay_sentrycamoffset_frontback.GetFloat(),
							   -replay_sentrycamoffset_leftright.GetFloat(), replay_sentrycamoffset_updown.GetFloat());
			}
			else
			{
			...
			out += Vector(0, 0,
						  s_aSentryEyeLevels[iSentryUpgrade] + (replay_sentrycammaxverticaloffset.IsValid()
																	? replay_sentrycammaxverticaloffset.GetFloat()
																	: 5));
