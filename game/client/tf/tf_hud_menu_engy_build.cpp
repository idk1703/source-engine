...
ConVar tf_build_menu_controller_mode("tf_build_menu_controller_mode", "0", FCVAR_ARCHIVE,
									 "Use console controller build menus. 1 = ON, 0 = OFF.");

const EngyConstructBuilding_t g_kEngyBuildings[NUM_ENGY_BUILDINGS] = {
	// Sentry gun
	EngyConstructBuilding_t(true, OBJ_SENTRYGUN, 0, "sentry_active.res", "sentry_already_built.res",
							"sentry_cant_afford.res", "sentry_unavailable.res", "sentry_active.res",
							"sentry_inactive.res", "sentry_inactive.res"),

	// Dispenser
	EngyConstructBuilding_t(true, OBJ_DISPENSER, 0, "dispenser_active.res", "dispenser_already_built.res",
							"dispenser_cant_afford.res", "dispenser_unavailable.res", "dispenser_active.res",
							"dispenser_inactive.res", "dispenser_inactive.res"),

	// Teleporter entrance
	EngyConstructBuilding_t(true, OBJ_TELEPORTER, MODE_TELEPORTER_ENTRANCE, "tele_entrance_active.res",
							"tele_entrance_already_built.res", "tele_entrance_cant_afford.res",
							"tele_entrance_unavailable.res", "tele_entrance_active.res", "tele_entrance_inactive.res",
							"tele_entrance_inactive.res"),

	// Teleporter exit
	EngyConstructBuilding_t(true, OBJ_TELEPORTER, MODE_TELEPORTER_EXIT, "tele_exit_active.res",
							"tele_exit_already_built.res", "tele_exit_cant_afford.res", "tele_exit_unavailable.res",
							"tele_exit_active.res", "tele_exit_inactive.res", "tele_exit_inactive.res")};

static int flagSlots[NUM_ENGY_BUILDINGS] = {0x01, 0x02, 0x04, 0x08};

#ifdef STAGING_ONLY
const EngyBuildingReplacement_t s_alternateEngineerBuildings[] = {
	// Catapult
	EngyBuildingReplacement_t(OBJ_CATAPULT, 0, "catapult_active.res", "catapult_already_built.res",
							  "catapult_cant_afford.res", "catapult_unavailable.res", "catapult_active.res",
							  "catapult_inactive.res", "catapult_inactive.res", flagSlots[2], flagSlots[3]),
	// Speed 1
	EngyBuildingReplacement_t(OBJ_TELEPORTER, MODE_TELEPORTER_SPEED, "speedpad_active.res",
							  "speedpad_already_built.res", "speedpad_cant_afford.res", "speedpad_unavailable.res",
							  "speedpad_active.res", "speedpad_inactive.res", "speedpad_inactive.res", flagSlots[2], 0),
	// Speed 2
	EngyBuildingReplacement_t(OBJ_TELEPORTER, MODE_TELEPORTER_SPEED2, "speedpad_active.res",
							  "speedpad_already_built.res", "speedpad_cant_afford.res", "speedpad_unavailable.res",
							  "speedpad_active.res", "speedpad_inactive.res", "speedpad_inactive.res", flagSlots[3], 0),
...
// Purpose:
//-----------------------------------------------------------------------------
CHudMenuEngyBuild::CHudMenuEngyBuild(const char *pElementName) : CHudBaseBuildMenu(pElementName, "HudMenuEngyBuild")
{
...
void CHudMenuEngyBuild::ApplySchemeSettings(IScheme *pScheme)
{
	...
	else
	{
		...
		for(int i = 0; i < NUM_ENGY_BUILDINGS; ++i)
		{
			...
			m_pAvailableObjects[i]->LoadControlSettings(
				VarArgs("%s/%s", pszCustomDir, m_Buildings[i].m_pszConstructAvailableObjectRes));
			m_pAlreadyBuiltObjects[i]->LoadControlSettings(
				VarArgs("%s/%s", pszCustomDir, m_Buildings[i].m_pszConstructAlreadyBuiltObjectRes));
			m_pCantAffordObjects[i]->LoadControlSettings(
				VarArgs("%s/%s", pszCustomDir, m_Buildings[i].m_pszConstructCantAffordObjectRes));
			m_pUnavailableObjects[i]->LoadControlSettings(
				VarArgs("%s/%s", pszCustomDir, m_Buildings[i].m_pszConstructUnavailableObjectRes));
	...
	for(int i = 0; i < NUM_ENGY_BUILDINGS; i++)
	{
		...
		int iCost = (pLocalPlayer) ? pLocalPlayer->m_Shared.CalculateObjectCost(pLocalPlayer, iBuilding)
								   : GetObjectInfo(iBuilding)->m_Cost;
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudMenuEngyBuild::GetBuildingIDAndModeFromSlot(int iSlot, int &iBuilding, int &iMode,
...
int CHudMenuEngyBuild::HudElementKeyInput(int down, ButtonCode_t keynum, const char *pszCurrentBinding)
{
	...
	else
	{
		...
		if(pszCurrentBinding && (!Q_strncmp(pszCurrentBinding, "slot", NUM_ENGY_BUILDINGS) &&
								 Q_strlen(pszCurrentBinding) > NUM_ENGY_BUILDINGS))
		{
