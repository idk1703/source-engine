...
void CHudMenuEngyDestroy::ApplySchemeSettings(IScheme *pScheme)
{
	...
	for(int i = 0; i < NUM_ENGY_BUILDINGS; ++i)
	{
		...
		{
			m_pActiveItems[i]->LoadControlSettings(
				VarArgs("%s/%s", pszCustomDir, m_Buildings[i].m_pszDestroyActiveObjectRes));
		}

		if(m_Buildings[i].m_pszDestroyInactiveObjectRes)
		{
			m_pInactiveItems[i]->LoadControlSettings(
				VarArgs("%s/%s", pszCustomDir, m_Buildings[i].m_pszDestroyInactiveObjectRes));
		}

		if(m_Buildings[i].m_pszDestroyUnavailableObjectRes)
		{
			m_pUnavailableItems[i]->LoadControlSettings(
				VarArgs("%s/%s", pszCustomDir, m_Buildings[i].m_pszDestroyUnavailableObjectRes));
		}
