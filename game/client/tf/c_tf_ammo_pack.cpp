...
RecvPropVector(RECVINFO(m_vecInitialVelocity)), RecvPropFloat(RECVINFO_NAME(m_angNetworkAngles[0], m_angRotation[0])),
...
int C_TFAmmoPack::DrawModel(int flags)
{
	...
	if(tf_debug_weapontrail.GetBool())
	{
		Msg("Ammo Pack:: Position: (%f %f %f), Velocity (%f %f %f)\n", GetAbsOrigin().x, GetAbsOrigin().y,
			GetAbsOrigin().z, GetAbsVelocity().x, GetAbsVelocity().y, GetAbsVelocity().z);
		if(debugoverlay)
		{
			debugoverlay->AddBoxOverlay(GetAbsOrigin(), Vector(-2, -2, -2), Vector(2, 2, 2), QAngle(0, 0, 0), 255, 255,
										0, 32, 5.0);
		}
	}
#endif // _DEBUG
