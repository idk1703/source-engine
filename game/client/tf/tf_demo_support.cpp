...
ConVar ds_enable("ds_enable", "0", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
				 "Demo support - enable automatic .dem file recording and features. 0 - Manual, 1 - Auto-record "
				 "competitive matches, 2 - Auto-record all matches, 3 - Auto-record tournament (mp_tournament) matches",
				 true, 0, true, 3);
ConVar ds_dir("ds_dir", "demos", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
			  "Demo support - will put all files into this folder under the gamedir. 24 characters max.");
ConVar ds_prefix("ds_prefix", "", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
				 "Demo support - will prefix files with this string. 24 characters max.");
ConVar ds_min_streak("ds_min_streak", "4", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
					 "Demo support - minimum kill streak count before being recorded.", true, 2, false, 0);
ConVar ds_kill_delay("ds_kill_delay", "15", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
					 "Demo support - maximum time between kills for tracking kill streaks.", true, 5, false, 0);
ConVar ds_log("ds_log", "1", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
			  "Demo support - log kill streak and bookmark events to an associated .txt file.", true, 0, true, 1);
ConVar ds_sound("ds_sound", "1", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
				"Demo support - play start/stop sound for demo recording.", true, 0, true, 1);
ConVar ds_notify("ds_notify", "0", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
				 "Demo support - text output when recording start/stop/bookmark events : 0 - console, 1 - console and "
				 "chat, 2 - console and HUD.",
				 true, 0, true, 2);
ConVar ds_screens("ds_screens", "1", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
				  "Demo support - take screenshot of the scoreboard for non-competitive matches or the match summary "
				  "stats for competitive matches. For competitive matches, it will not capture the screenshot if you "
				  "disconnect from the server before the medal awards have completed.",
				  true, 0, true, 1);
ConVar ds_autodelete(
	"ds_autodelete", "0", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
	"Demo support - automatically delete .dem files with no associated bookmark or kill streak events.", true, 0, true,
	1);

CON_COMMAND_F(ds_mark,
			  "Demo support - bookmark (with optional single-word description) the current tick count for the demo "
			  "being recorded.",
			...
			if((m_flNextRecordStartCheckTime < 0) || (m_flNextRecordStartCheckTime < gpGlobals->curtime))
			{
				...
				if(pLocalPlayer)
				{
					...
					if((pLocalPlayer->GetTeamNumber() == TEAM_SPECTATOR) ||
					   ((pLocalPlayer->GetTeamNumber() >= FIRST_GAME_TEAM) && pLocalPlayer->GetPlayerClass() &&
						(pLocalPlayer->GetPlayerClass()->GetClassIndex() >= TF_FIRST_NORMAL_CLASS) &&
						(pLocalPlayer->GetPlayerClass()->GetClassIndex() < TF_LAST_NORMAL_CLASS)))
					{
						if(!StartRecording())
						{
...
void CTFDemoSupport::LogEvent(EDemoEventType eType, int nValue /* = 0 */, const char *pszValue /* = NULL */)
{
	...
	if(m_hGlobalEventList != FILESYSTEM_INVALID_HANDLE)
	{
		...
		g_pFullFileSystem->FPrintf(m_hGlobalEventList, "[%04u/%02u/%02u %02u:%02u] %s %s (\"%s\" at %d)\n",
								   ...
								   g_aDemoEventNames[eType], szArg, m_szFilename, nTickCount);
	}
...
bool CTFDemoSupport::IsValidPath(const char *pszFolder)
{
	...
	if(Q_strlen(pszFolder) <= 0 || Q_strstr(pszFolder, "\\\\") || // to protect network paths
...
bool CTFDemoSupport::StartRecording(void)
{
	...
	V_sprintf_safe(szTime, "%04u-%02u-%02u_%02u-%02u-%02u", ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday,
