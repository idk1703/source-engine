...
enum EMatchmakingUIState
{
	...
	eMatchmakingUIState_InQueue,	//< In matchmaking queue, awaiting to be matched with compatible players and a
									//gameserver.  Game could start at any moment
	eMatchmakingUIState_Connecting, //< Matched with other players and assigned a gameerver, trying to connect to a game
									//server
...
enum EAbandonGameStatus
{
	...
	k_EAbandonGameStatus_AbandonWithoutPenalty, //< Leaving right now would be considered "abandoning", but there will
												//be no penalty right now
	k_EAbandonGameStatus_AbandonWithPenalty,	//< Leaving right now would be considered "abandoning", and you will be
												//penalized
};
...
public:
	...
	TF_Matchmaking_WizardStep GetWizardStep() const
	{
		return m_eLocalWizardStep;
	}
	...
	//	void SetAutoSpectateCheckTime( float flAutoSpectateCheckTime ) { m_flAutoSpectateCheckTime =
	...
	const CUtlDict<float> &GetDataCenterPopulationRatioDict(EMatchGroup eMatchGroup)
	{
	...
	bool BUserWantsToBeInMatchmaking() const
	{
		return m_bUserWantsToBeInMatchmaking;
	}
