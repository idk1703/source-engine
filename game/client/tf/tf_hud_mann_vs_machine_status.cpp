...
ConVar cl_mvm_wave_status_visible_during_wave("cl_mvm_wave_status_visible_during_wave", "0", FCVAR_ARCHIVE,
											  "Display full wave contents while a wave is active in MvM.");
...
void CMvMBossProgressBar::OnTick(void)
{
	...
	if(m_flOldPercentage != m_flPercentage)
	{
		...
		if(m_pProgressBar)
		{
			m_pProgressBar->SetBounds(m_nBarOrgX, m_nBarOrgY, (int)(m_nBarOrgW * m_flPercentage) + m_nWidthSpacer,
									  m_nBarOrgT);
		}
...
void CMvMBossStatusPanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if(m_pBackground)
	{
		m_pBackground->GetBounds(m_nBackgroundOriginalX, m_nBackgroundOriginalY, m_nBackgroundOriginalW,
								 m_nBackgroundOriginalT);
	}
...
void CMvMBossStatusPanel::OnTick(void)
{
	...
	{
		C_TFMvMBossProgressUser *pBossProgressUser =
			static_cast<C_TFMvMBossProgressUser *>(ITFMvMBossProgressUserAutoList::AutoList()[i]);
	...
	int nTotalPanelHeight =
		(activeBosses.Count() * nHeightPerPanel); // + ( ( nNumBosses - 1 ) * m_nSpaceBetweenPanels );
	if(m_pBackground && bBackgroundVisible)
	{
		...
		if((m_nBackGroundTall != nBackgroundTall) || m_bPanelDirty)
		{
			...
			m_pBackground->SetBounds(m_nBackgroundOriginalX, m_nBackgroundOriginalY, m_nBackgroundOriginalW,
									 m_nBackGroundTall);
		}
	}
...
void CWaveStatusPanel::OnTick(void)
{
	...
	if(pLocalPlayer)
	{
		bool bVisible = m_bVerbose || pLocalPlayer->IsAlive() || (pLocalPlayer->GetTeamNumber() == TEAM_SPECTATOR) ||
						(!m_bVerbose && !pLocalPlayer->IsAlive());
...
void CWaveStatusPanel::UpdateEnemyCounts(void)
{
		...
		if(pchClassIconName[0] != '\0')
		{
			...
			else if(iFlags & MVM_CLASS_FLAG_MISSION)
			{
				if(((TFGameRules()->State_Get() != GR_STATE_RND_RUNNING) &&
					!(FStrEq("sentry_buster", pchClassIconName) || FStrEq("teleporter", pchClassIconName))) ||
				   (nClassCount > 0))
				{
	...
	if(nMaxEnemyCountNoSupport > 0)
	{
		if(m_nEnemyRemainingNoSupport != nNumEnemyRemaining)
		{
			...
			if(m_pProgressBar)
			{
				...
				m_pProgressBar->SetBounds(m_nBarOrgX, m_nBarOrgY, (int)(m_nBarOrgW * flPercent) + m_nWidthSpacer,
										  m_nBarOrgT);
			}
	...
	bool bVerbose = (m_bVerbose || (TFGameRules()->State_Get() != GR_STATE_RND_RUNNING) ||
	...
	if(m_pBackground)
	{
		...
		int nNeededBgWidth =
			nTotalEnemyWidth + (m_nWaveCountOffset * 3); // add little buffer to both ends of the background
	...
	if(bVerbose)
	{
		...
		for(int i = 0; i < miniboss.Count() && iPanelIndex < m_EnemyCountPanels.Count(); i++, iPanelIndex++)
		{
			...
			{
				pPanel->m_pEnemyCountImage->SetImage(
					VarArgs("../hud/leaderboard_class_%s", miniboss[i].pchClassIconName));
	...
	for(int i = 0; i < normal.Count() && iPanelIndex < m_EnemyCountPanels.Count(); i++)
	{
		...
		if(bVerbose || bNonVerboseSpy)
		{
			...
			{
				pPanel->m_pEnemyCountImage->SetImage(
					VarArgs("../hud/leaderboard_class_%s", normal[i].pchClassIconName));
				pPanel->m_pEnemyCountImage->SetVisible(true);
			}
	...
	for(int i = 0; i < support.Count() && iPanelIndex < m_EnemyCountPanels.Count(); i++)
	{
		...
		if(bVerbose || bActive)
		{
			if(!IsClassIconBeingUsed(classIconsBeingUsed, support[i].pchClassIconName))
			{
				...
				{
					pPanel->m_pEnemyCountImage->SetImage(
						VarArgs("../hud/leaderboard_class_%s", support[i].pchClassIconName));
					pPanel->m_pEnemyCountImage->SetVisible(true);
				}
	...
	for(int i = 0; i < mission.Count() && iPanelIndex < m_EnemyCountPanels.Count(); i++, iPanelIndex++)
	{
		if(!IsClassIconBeingUsed(classIconsBeingUsed, mission[i].pchClassIconName))
		{
			...
			if(pPanel->m_pEnemyCountImage)
			{
				...
				if(!bBetweenWaves &&
				   (FStrEq("spy", pchMissionClassIconName) || FStrEq("sentry_buster", pchMissionClassIconName) ||
					FStrEq("engineer", pchMissionClassIconName)))
				{
					if(pPanel->IsFlashing())
					{
						// we're already flashing so don't reset the background
...
void CCurrencyStatusPanel::OnTick(void)
{
	...
	if(!pLocalPlayer || (pLocalPlayer->GetTeamNumber() != TF_TEAM_PVE_DEFENDERS) ||
...
void CInWorldCurrencyStatus::OnTick(void)
{
	...
	if(m_pGood && m_pBad)
	{
		...
		if((pStats && nMissed > nWorldMoney) ||
		   (TFObjectiveResource()->GetMannVsMachineIsBetweenWaves() && nWorldMoney == 0))
		{
...
void CWaveCompleteSummaryPanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	{
		m_pCreditBonusTextLabel =
			dynamic_cast<vgui::Label *>(m_pCreditContainerPanel->FindChildByName("CreditBonusTextLabel"));
		m_pCreditBonusCountLabel =
			dynamic_cast<vgui::Label *>(m_pCreditContainerPanel->FindChildByName("CreditBonusCountLabel"));
	}
	...
	{
		m_pRespecTextLabel =
			dynamic_cast<vgui::Label *>(m_pRespecContainerPanel->FindChildByName("RespecTextLabelWin"));
...
{
	if(TFGameRules()->State_Get() != GR_STATE_BETWEEN_RNDS && TFGameRules()->State_Get() != GR_STATE_GAME_OVER &&
	...
	switch(m_eState)
	{
		case CREDITS_COLLECT:
			StateUpdateValue(m_pCreditContainerPanel, CREDITS_COLLECTED_STR, CREDITS_COLLECTED_TIME,
							 m_fStateRunningTime, CREDITS_MISSED, m_nCreditsCollected);
			break;
		case CREDITS_MISSED:
			StateUpdateValue(m_pCreditContainerPanel, CREDITS_MISSED_STR, CREDITS_MISSED_TIME, m_fStateRunningTime,
							 RATING_LABEL, m_nCreditsMissed);
			break;
		case RATING_LABEL:
			RatingLabelUpdate();
...
//-----------------------------------------------------------------------------
bool CWaveCompleteSummaryPanel::StateUpdateValue(vgui::EditablePanel *parent, char *field, float targetTime,
												 float currentTime, int nextState, int endValue)
{
	...
	if(TFGameRules()->IsMannVsMachineRespecEnabled())
	{
		...
		if(pStats)
		{
			...
			if(bVisible)
			{
				...
				{
					if(!m_pRespecBackground->IsVisible() && !m_pRespecCountLabel->IsVisible() &&
					   !m_pRespecTextLabel->IsVisible())
					{
...
void CTFHudMannVsMachineStatus::ApplySchemeSettings(IScheme *pScheme)
{
	...
	CMvMBombCarrierProgress *parent =
		dynamic_cast<CMvMBombCarrierProgress *>(m_pUpgradeLevelContainer->FindChildByName("UpgradeProgressTrack"));
	if(parent)
	{
...
void CTFHudMannVsMachineStatus::OnTick(void)
{
	...
	if(TFGameRules()->InSetup() && ObjectiveResource() && !TFGameRules()->IsInTournamentMode())
	{
		CTeamRoundTimer *pTimer =
			dynamic_cast<CTeamRoundTimer *>(ClientEntityList().GetEnt(ObjectiveResource()->GetTimerToShowInHUD()));
		if(pTimer)
		{
	...
	if(TFGameRules()->State_Get() == GR_STATE_GAME_OVER && !m_bInVictorySplash)
	{
		...
		if(!m_bIsServerKicking)
		{
			if(seconds > 1)
			{
				g_pVGuiLocalize->ConstructString_safe(
					wszLocalizedMessage, g_pVGuiLocalize->Find("#TF_PVE_Server_Message_Reset"), 1, wszTime);
			}
			else
			{
				g_pVGuiLocalize->ConstructString_safe(
					wszLocalizedMessage, g_pVGuiLocalize->Find("#TF_PVE_Server_Message_ResetNoS"), 1, wszTime);
			}
		...
		else
		{
			if(seconds > 1)
			{
				g_pVGuiLocalize->ConstructString_safe(wszLocalizedMessage,
													  g_pVGuiLocalize->Find("#TF_PVE_Server_Message_Kick"), 1, wszTime);
			}
			else
			{
				g_pVGuiLocalize->ConstructString_safe(
					wszLocalizedMessage, g_pVGuiLocalize->Find("#TF_PVE_Server_Message_KickNoS"), 1, wszTime);
			}
			m_pServerChangeMessage->SetDialogVariable("servermessage", wszLocalizedMessage);
		}
	}
...
void CTFHudMannVsMachineStatus::UpdateBombCarrierProgress(void)
{
	...
	// Width of Meter
	float flBase =
		TFObjectiveResource()->GetNextMvMBombUpgradeTime() - TFObjectiveResource()->GetBaseMvMBombUpgradeTime();
	...
	m_pBombUpgradeMeterMask->SetWide((m_nUpgradeMaskBaseWidth * (nFlagLevel + 1)) +
									 flPercent * m_nUpgradeMaskBaseWidth);
