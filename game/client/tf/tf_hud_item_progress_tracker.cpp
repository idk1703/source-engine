...
ConVar tf_contract_progress_show(
	"tf_contract_progress_show", "1", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
	"Settings for the contract HUD element: 0 show nothing, 1 show everything, 2 show only active contracts.",
	cc_contract_progress_show_update);
ConVar tf_contract_competitive_show("tf_contract_competitive_show", "2",
									FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
									"Settings for the contract HUD element during competitive matches: 0 show nothing, "
									"1 show everything, 2 show only active contracts.",
									cc_contract_progress_show_update);
...
// Purpose:
//-----------------------------------------------------------------------------
CItemAttributeProgressPanel::CItemAttributeProgressPanel(Panel *pParent, const char *pElementName,
														 const CQuestObjectiveDefinition *pObjectiveDef,
														 const char *pszResFileName)
...
void CItemAttributeProgressPanel::ApplySettings(KeyValues *inResourceData)
{
	...
	if(pObjectiveDef)
	{
		...
		if(!pLocalizedObjectiveName || !pLocalizedObjectiveName[0])
		{
			...
			GLocalizationProvider()->ConvertUTF8ToLocchar(pszDescriptionToken, loc_ItemDescription,
														  ARRAYSIZE(loc_ItemDescription));
		}
		else
		{
			...
			locchar_t *pszLocString = GLocalizationProvider()->Find(
				pObjectiveDef->IsAdvanced() ? m_strAdvancedLocToken : m_strNormalPointLocToken);
...
void CItemAttributeProgressPanel::OnThink()
{
	...
	{
		float flGlowAlpha =
			RemapValClamped(flGlowTime, ATTRIB_TRACK_GLOW_HOLD_TIME, ATTRIB_TRACK_GLOW_HOLD_TIME + 0.25f, 1.f, 0.f);
...
// Purpose:
//-----------------------------------------------------------------------------
CItemTrackerPanel::CItemTrackerPanel(Panel *pParent, const char *pElementName, const CEconItem *pItem,
									 const char *pszItemTrackerResFile)
	: EditablePanel(pParent, pElementName),
	  m_pItem(NULL),
...
void CItemTrackerPanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if(m_pItem)
	{
		m_pItem->GetItemDefinition()->GetQuestDef()->GetRolledObjectivesForItem(vecChosenObjectives,
																				m_pItem->GetSOCData());
	}
...
void CItemTrackerPanel::PerformLayout()
{
	...
	const wchar_t *pszLocToken =
		g_pVGuiLocalize->Find(m_pItem->GetItemDefinition()->GetQuestDef()->GetRolledNameForItem(m_pItem->GetSOCData()));
	...
	const CQuestItemTracker *pItemTracker =
		QuestObjectiveManager()->GetTypedTracker<CQuestItemTracker *>(m_pItem->GetItemID());

	if(pItemTracker)
	{
		bool bStandardPointsCompleteAndBonusPossible = m_flStandardCurrentProgress == m_flStandardTargetProgress &&
													   m_flStandardTargetProgress == 1.f && m_nMaxBonusPoints > 0;
		uint32 nCurrentPoints = bStandardPointsCompleteAndBonusPossible ? pItemTracker->GetEarnedBonusPoints()
																		: pItemTracker->GetEarnedStandardPoints();
		uint32 nTargetPoints = bStandardPointsCompleteAndBonusPossible ? m_nMaxBonusPoints : m_nMaxStandardPoints;

		locchar_t locValue[MAX_ITEM_NAME_LENGTH];
		const locchar_t *pPointsToken =
			GLocalizationProvider()->Find(bStandardPointsCompleteAndBonusPossible ? m_strProgressBarAdvancedLocToken
																				  : m_strProgressBarStandardLocToken);
	...
	FOR_EACH_VEC(m_vecAttribPanels, i)
	{
		...
		// Hide all objectives if everything is completed, or just hide standard objectives is standard points re
		// completed
		...
		else
		{
			...
			if(pItemTracker && pLocalPlayer)
			{
				...
				const CBaseQuestObjectiveTracker *pObjectiveTracker =
					pItemTracker->FindTrackerForDefIndex(pPanel->m_nDefIndex);
...
void CItemTrackerPanel::OnThink()
{
	...
	if(IsDoneProgressing() && IsStandardCompleted())
	{
		if(m_pCompletedDescGlow && m_pCompletedNameGlow)
		{
			...
			float flGlowAlpha = RemapValClamped(flPeriod, ATTRIB_TRACK_COMPLETE_PULSE_DIM_HOLD,
												ATTRIB_TRACK_COMPLETE_PULSE_GLOW_HOLD, 0.f, 1.f);
	...
	float flGlowTime = Plat_FloatTime() - m_flUpdateTime;
	if(flGlowTime > ATTRIB_TRACK_GLOW_HOLD_TIME && (m_flStandardCurrentProgress != m_flStandardTargetProgress ||
													m_flBonusCurrentProgress != m_flBonusTargetProgress))
	{
		float flDelta = Plat_FloatTime() - m_flLastThink;
		m_flStandardCurrentProgress =
			Approach(m_flStandardTargetProgress, m_flStandardCurrentProgress, flDelta * ATTRIB_TRACK_BAR_GROW_RATE);
		m_flBonusCurrentProgress =
			Approach(m_flBonusTargetProgress, m_flBonusCurrentProgress, flDelta * ATTRIB_TRACK_BAR_GROW_RATE);
...
void CItemTrackerPanel::FireGameEvent(IGameEvent *pEvent)
{
	if(FStrEq(pEvent->GetName(), "quest_objective_completed"))
	{
		...
		if(pItemTracker)
		{
			const Color &glowColor = pItemTracker->GetEarnedStandardPoints() < m_nMaxStandardPoints
										 ? m_clrStandardHighlight
										 : m_clrBonusHighlight;
			FOR_EACH_VEC(m_vecAttribPanels, i)
			{
			...
			if(m_flStandardCurrentProgress < m_flStandardTargetProgress &&
			   pItemTracker->GetEarnedStandardPoints() == m_nMaxStandardPoints)
			{
				eNewSound = SOUND_QUEST_STANDARD_COMPLETE;
			}
			else if(m_flBonusCurrentProgress < m_flBonusTargetProgress &&
					pItemTracker->GetEarnedBonusPoints() == m_nMaxBonusPoints)
			{
	...
	else if(FStrEq(pEvent->GetName(), "player_spawn") || FStrEq(pEvent->GetName(), "inventory_updated") ||
...
bool CItemTrackerPanel::IsEverythingCompleted() const
{
	...
	if(pItemTracker)
	{
		return (pItemTracker->GetEarnedBonusPoints() + pItemTracker->GetEarnedStandardPoints()) >=
			   (m_nMaxBonusPoints + m_nMaxStandardPoints);
	}
...
CItemAttributeProgressPanel *CItemTrackerPanel::GetPanelForObjective(const CQuestObjectiveDefinition *pObjective)
{
	...
	CItemAttributeProgressPanel *pPanel =
		new CItemAttributeProgressPanel(this, "ItemAttributeProgressPanel", pObjective, m_strItemAttributeResFile);
	SETUP_PANEL(pPanel);
...
void CHudItemAttributeTracker::PerformLayout()
{
	...
	FOR_EACH_VEC(vecQuestItems, i)
	{
		...
		{
			const CQuestItemTracker *pItemTracker =
				QuestObjectiveManager()->GetTypedTracker<CQuestItemTracker *>(pItem->GetItemID());
			InvalidReasonsContainer_t invalidReasons;
			if(pItemTracker)
			{
	...
	if(bShowExtras)
	{
		...
		if(pLocalizedString && pLocalizedString[0])
		{
			...
			loc_scpy_safe(locNumNewQuests, CConstructLocalizedString(pLocalizedString, wszCounter));
		}
	...
	if(m_pStatusContainer)
	{
		...
		if(m_pStatusContainer->IsVisible())
		{
			...
			m_pStatusHeaderLabel->SetPos(m_pStatusContainer->GetWide() - m_pStatusHeaderLabel->GetWide(),
										 m_pStatusHeaderLabel->GetYPos());
			m_pCallToActionLabel->SetPos(m_pStatusContainer->GetWide() - m_pCallToActionLabel->GetWide(),
										 m_pCallToActionLabel->GetYPos());
			m_pStatusContainer->SetPos(m_pStatusContainer->GetParent()->GetWide() - m_pStatusContainer->GetWide(),
									   m_pStatusContainer->GetYPos());
		}
	}
...
{
	if(FStrEq(pEvent->GetName(), "player_spawn") || FStrEq(pEvent->GetName(), "inventory_updated") ||
	   FStrEq(pEvent->GetName(), "client_disconnect") || FStrEq(pEvent->GetName(), "localplayer_changeclass") ||
...
// Purpose:
//-----------------------------------------------------------------------------
bool CHudItemAttributeTracker::FindTrackerForItem(const CEconItem *pItem, CItemTrackerPanel **ppTracker,
												  bool bCreateIfNotFound)
{
	...
	{
		(*ppTracker) = new CItemTrackerPanel(
			this, "ItemTrackerPanel", pItem,
