...
BEGIN_NETWORK_TABLE(C_TFProjectile_Arrow, DT_TFProjectile_Arrow)
	RecvPropBool(RECVINFO(m_bArrowAlight)), RecvPropBool(RECVINFO(m_bCritical)),
...
void C_TFProjectile_Arrow::CheckNearMiss(void)
{
	...
	trace_t tr;
	UTIL_TraceLine(vecArrowPos, vecArrowPos + forward * 400.f, CONTENTS_HITBOX | CONTENTS_MONSTER | CONTENTS_SOLID,
...
void C_TFProjectile_GrapplingHook::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if(updateType == DATA_UPDATE_CREATED)
	{
		...
		if(pTFPlayer && pTFPlayer->IsPlayerClass(TF_CLASS_SPY) && pTFPlayer->m_Shared.InCond(TF_COND_DISGUISED) &&
