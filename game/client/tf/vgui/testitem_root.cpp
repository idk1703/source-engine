...
void CTestItemRoot::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	if(m_pTestingPanel)
	{
		...
		{
			m_pItemTestButtons[i] =
				dynamic_cast<CExButton *>(m_pTestingPanel->FindChildByName(VarArgs("TestItemButton%d", i)));
			m_pItemTestButtons[i]->AddActionSignalTarget(this);
			m_pItemRemoveButtons[i] =
				dynamic_cast<CExButton *>(m_pTestingPanel->FindChildByName(VarArgs("RemoveItemButton%d", i)));
			m_pItemRemoveButtons[i]->AddActionSignalTarget(this);
			m_pItemTestLabels[i] =
				dynamic_cast<CExLabel *>(m_pTestingPanel->FindChildByName(VarArgs("TestItemEntry%d", i)));
		}
	}
	...
	if(m_pClassUsagePanel)
	{
		...
		{
			m_pClassCheckButtons[i] =
				dynamic_cast<vgui::CheckButton *>(m_pClassUsagePanel->FindChildByName(VarArgs("ClassCheckBox%d", i)));
	...
	CExButton *pKickAllBotsButton =
		dynamic_cast<CExButton *>(m_pBotAdditionPanel->FindChildByName("KickAllBotsButton"));
...
int CTestItemRoot::FindReplaceableItemsForSelectedClass(CUtlVector<item_definition_index_t> *pItemDefs, bool bWeapons)
{
	...
	FOR_EACH_MAP(mapItemDefs, i)
	{
		...
		if(bWeapons)
		{
			bValidSlot = (iDefSlot == LOADOUT_POSITION_PRIMARY || iDefSlot == LOADOUT_POSITION_SECONDARY ||
						  iDefSlot == LOADOUT_POSITION_MELEE);
			if(!bValidSlot)
			{
				bValidSlot = pDef->CanBePlacedInSlot(LOADOUT_POSITION_PRIMARY) ||
							 pDef->CanBePlacedInSlot(LOADOUT_POSITION_SECONDARY) ||
							 pDef->CanBePlacedInSlot(LOADOUT_POSITION_MELEE);
			}
		}
		else
		{
			...
			{
				bValidSlot =
					pDef->CanBePlacedInSlot(LOADOUT_POSITION_HEAD) || pDef->CanBePlacedInSlot(LOADOUT_POSITION_MISC);
			}
		}
		if(!bValidSlot)
			continue;
...
void CTestItemRoot::UpdateTestItems(void)
{
	for(int i = 0; i < TI_TYPE_COUNT; i++)
	{
		...
		if(m_pItemTestKVs[i])
		{
			...
			item_definition_index_t iExistingDef =
				m_pItemTestKVs[i]->GetInt("existing_itemdef", INVALID_ITEM_DEF_INDEX);
...
void CTestItemRoot::OnCommand(const char *command)
{
	...
	else if(!Q_strnicmp(command, "item_test", 9))
	{
		...
		if(iItemType >= 0 && iItemType < TI_TYPE_COUNT)
		{
			...
			{
				m_hEditItemDialog = vgui::SETUP_PANEL(new CTestItemDialog(this, (testitem_itemtypes_t)iItemType,
																		  m_iClassUsage, m_pItemTestKVs[iItemType]));
			}
	...
	else if(!Q_stricmp(command, "export") || !Q_stricmp(command, "import"))
	{
		...
		{
			m_hImportExportDialog = new vgui::FileOpenDialog(NULL, "#ToolCustomizeTextureTitle",
															 m_bExporting ? vgui::FOD_SAVE : vgui::FOD_OPEN, NULL);
			...
			m_hImportExportDialog->SetStartDirectory(
				g_pFullFileSystem->RelativePathToFullPath("cfg", "MOD", szModelsDir, sizeof(szModelsDir)));
		}
...
ConCommand testitem_botcontrols("itemtest_botcontrols", OpenTestItemBotControls,
								"Open the item testing bot control panel.", FCVAR_NONE);