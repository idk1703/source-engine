...
void CTFAdvancedOptionsDialog::OnKeyCodePressed(KeyCode code)
{
	...
	if(GetBaseButtonCode(code) == KEY_XBUTTON_B || GetBaseButtonCode(code) == STEAMCONTROLLER_B ||
	   GetBaseButtonCode(code) == STEAMCONTROLLER_START)
	{
		OnClose();
	}
	else
...
static ConCommand opentf2options("opentf2options", &CL_OpenTFAdvancedOptionsDialog,
								 "Displays the TF2 Advanced Options dialog.");
...
public:
	CExScrollBar(Panel *parent, const char *name, bool bVertical) : ScrollBar(parent, name, bVertical) {}
