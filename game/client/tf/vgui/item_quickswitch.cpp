	...
	: CHudElement(pElementName), BaseClass(NULL, "ItemQuickSwitchPanel")
{
	...
	m_pItemContainerScroller =
		vgui::SETUP_PANEL(new vgui::ScrollableEditablePanel(this, m_pItemContainer, "itemcontainerscroller"));
	m_pItemKV = NULL;
	m_pWeaponLabel = NULL;
...
void CItemQuickSwitchPanel::UpdateEquippedItem(void)
{
	...
	if(pCurItemData)
	{
		...
		else if(!TFInventoryManager()->SlotContainsBaseItems(GEconItemSchema().GetEquipTypeFromClassIndex(m_iClass),
															 m_iSlot))
		{
			for(int i = 0; i < m_pItemPanels.Count(); i++)
			{
...
void CItemQuickSwitchPanel::UpdateModelPanels(void)
{
	...
	const equip_region_mask_t unUsedEquipRegionMask =
		GenerateEquipRegionConflictMask(m_iClass, m_iSlot, LOADOUT_POSITION_INVALID);

	CEquippableItemsForSlotGenerator equippableItems(m_iClass, m_iSlot, unUsedEquipRegionMask,
