...
bool BInEndOfMatch()
{
	const bool bInEndOfMatch = TFGameRules() && TFGameRules()->State_Get() == GR_STATE_GAME_OVER &&
...
public:
	...
	CMatchMakingHUDPopupContainer() : Panel(g_pClientMode->GetViewport(), "MMDashboardPopupContainer")
	{
	...
	virtual void OnTick()
	{
		...
		for(int i = 0; i < nCount && !bChildrenVisible; ++i)
		{
			...
			bChildrenVisible = bChildrenVisible || pChild->BIsExpanded() ||
							   (!pChild->BIsExpanded() && pChild->GetPercentAnimated() != 1.f);
		}
...
CMMDashboardParentManager::CMMDashboardParentManager() : m_bAttachedToGameUI(false)
{
...
CTFMatchmakingPopup::CTFMatchmakingPopup(const char *pszName, const char *pszResFile)
	: CExpandablePanel(NULL, pszName), m_pszResFile(pszResFile), m_bActive(false)
{
...
CTFMatchmakingDashboard::CTFMatchmakingDashboard() : CExpandablePanel(NULL, "MMDashboard")
{
	SetKeyBoardInputEnabled(false);
	ivgui()->AddTickSignal(GetVPanel(), 100);
...
void CTFMatchmakingDashboard::OnTick()
{
	...
	const IMatchGroupDescription *pMatchDesc =
		GetMatchGroupDescription(TFGameRules() ? TFGameRules()->GetCurrentMatchGroup() : k_nMatchGroup_Invalid);
	bool bShouldBeVisible = GTFGCClientSystem()->BConnectedToMatchServer(false) && bInEndOfMatch && pMatchDesc &&
