...
public:
	CQuestTooltip(vgui::Panel *parent, const char *text = NULL) : BaseClass(parent, text) {}
...
public:
	...
	CQuestItemPanel(Panel *parent, const char *pszPanelName, CEconItemView *pQuestItem,
	...
	const CEconItemView *GetItem()
	{
		return m_hQuestItem;
	}
	void SetItem(CEconItemView *pItem);
	void QuestCompletedResponse();
	EItemPanelState_t GetState() const
	{
		return m_eState;
	}
...
protected:
	bool HasAllControls() const
	{
		return m_bHasAllControls;
	}
	...
	CPanelAnimationVarAliasType(int, m_iFrontPaperHideHeight, "front_paper_hide_height", "1000",
								"proportional_int"); // Default to a large value so it wont be visible
	...
	// CPanelAnimationVarAliasType( int, m_iScrollingContainerHeight, "scrolling_container_height", "200",
	// "proportional_int" );
