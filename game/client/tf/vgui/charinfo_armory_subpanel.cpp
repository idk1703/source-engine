...
ConVar tf_explanations_charinfo_armory_panel("tf_explanations_charinfo_armory_panel", "0", FCVAR_ARCHIVE,
		...
		if(steamapicontext && steamapicontext->SteamFriends())
		{
			if(IsVisible() && m_SelectedItem.IsValid())
			{
				...
				Q_snprintf(szURL, sizeof(szURL), "http://wiki.teamfortress.com/scripts/itemredirect.php?id=%d&lang=%s",
						   m_SelectedItem.GetItemDefIndex(), GetLanguageICUName(iLang));
...
void CArmoryPanel::SetFilterTo(int iItemDef, armory_filters_t nFilter)
{
	...
	else
	{
		...
		FOR_EACH_MAP(mapItemDefs, i)
		{
			...
			if(pDef->IsHidden() || pDef->GetQuality() == k_unItemQuality_Any || pDef->GetQuality() == AE_NORMAL ||
			   !pDef->ShouldShowInArmory())
				continue;
...
bool CArmoryPanel::DefPassesFilter(const CTFItemDefinition *pDef, armory_filters_t iFilter)
{
	...
	switch(iFilter)
	{
		...
		case ARMFILT_WEAPONS:
		{
			...
			bInList = (iSlot == LOADOUT_POSITION_PRIMARY || iSlot == LOADOUT_POSITION_SECONDARY ||
		...
		{
			bInList = pDef->GetItemClass() &&
					  (!V_strcmp(pDef->GetItemClass(), "craft_item") ||
					   !V_strcmp(pDef->GetItemClass(), "class_token") || !V_strcmp(pDef->GetItemClass(), "slot_token"));
			break;
		}

		case ARMFILT_TOOLS:
		{
			// For now, put the supply crates into the tool list, since it's the only item that shows up in no other
			// lists
			bInList = pDef->GetItemClass() &&
					  (!V_strcmp(pDef->GetItemClass(), "tool") || !V_strcmp(pDef->GetItemClass(), "supply_crate"));
			break;
		}
...
void CArmoryPanel::UpdateSelectedItem(void)
{
	...
	if(m_pSelectedItemModelPanel)
	{
		...
		m_pDataTextRichText->SetBounds(iX, iRichTextYPosInDataPanel, m_pDataTextRichText->GetWide(),
									   m_pDataPanel->GetTall() - iRichTextYPosInDataPanel);
	}
	...
	if(m_pStoreButton)
	{
		bool bShowStoreButton = m_bAllowGotoStore && EconUI()->GetStorePanel() &&
								EconUI()->GetStorePanel()->GetPriceSheet() &&
								EconUI()->GetStorePanel()->GetPriceSheet()->GetEntry(m_SelectedItem.GetItemDefIndex());
