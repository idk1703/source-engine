...
//-----------------------------------------------------------------------------
bool CDynamicRecipePanel::CRecipeComponentAttributeCounter::OnIterateAttributeValue(
	const CEconItemAttributeDefinition *pAttrDef, const CAttribute_DynamicRecipeComponent &value)
{
	...
	if(value.component_flags() & DYNAMIC_RECIPE_FLAG_IS_OUTPUT)
	{
		...
		for(unsigned i = 0; i < nCount; ++i)
		{
			...
			{
				item.GetAttributeList()->SetRuntimeAttributeValue(
					pAttrib_CannotTrade, 0.0f); // value doesn't matter -- we only check for presence/absence
			}
		}
	}
	else
...
//-----------------------------------------------------------------------------
int CDynamicRecipePanel::CRecipeComponentAttributeCounter::LeastCommonInputSortFunc(
	const CCopyableUtlVector<InputComponent_t> *p1, const CCopyableUtlVector<InputComponent_t> *p2)
{
...
//-----------------------------------------------------------------------------
CDynamicRecipePanel::CRecipeComponentAttributeCounter::InputComponent_t *CDynamicRecipePanel::
	CRecipeComponentAttributeCounter::GetInputComponent(int i)
{
	int nAccum = 0;
...
//-----------------------------------------------------------------------------
bool CDynamicRecipePanel::CDynamicRecipeItemMatchFind::OnIterateAttributeValue(
	const CEconItemAttributeDefinition *pAttrDef, const CAttribute_DynamicRecipeComponent &value)
{
	// Can't match ourself
...
void CInputPanelItemModelPanel::SetBlankState()
{
	...
	if(pDefaultItem)
	{
		...
		if(FStrEq(attrItemNameTextOverride.value().c_str(), "#TF_ItemName_Item"))
		{
			...
			if(V_wcscmp(pszItemname, g_pVGuiLocalize->Find("#TF_ItemName_Item")) == 0 &&
			   pDefaultItem->GetQuality() == AE_PAINTKITWEAPON)
			{
...

{
	...
	m_pShowUntradableItemsCheckbox =
		new vgui::CheckButton(m_pInventoryContainer, "untradablecheckbox", "#Dynamic_Recipe_Untradable_Checkbox");
...
void CDynamicRecipePanel::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pDevGiveInputsButton =
		new CExButton(m_pInventoryContainer, "dev_giveinputsbutton", "[Debug] Give Inputs", this, "dev_giveinputs");
...
void CDynamicRecipePanel::OnCommand(const char *command)
{
	...
	else if(!Q_strnicmp(command, "deleteitem", 10) || !Q_strnicmp(command, "useitem", 7))
	{
...
bool CDynamicRecipePanel::CheckForUntradableItems(void)
{
	...
	for(int i = 0; i < CRAFTING_SLOTS_INPUTPANELS; ++i)
	{
		...
		for(int j = 0; j < GetNumInputPages(); ++j)
		{
			...
			{
				CEconItemView *pItemData =
					TFInventoryManager()->GetLocalTFInventory()->GetInventoryItemByItemID(nRecipeItemID);
	...
	if(bHasUntradable)
	{
		...
		CTFGenericConfirmDialog *pDialog = new CTFGenericConfirmDialog(
			"#Craft_Untradable_Title", wszWarning, "#GameUI_OK", "#Cancel", &ConfirmUntradableCraft, NULL);
...
bool CDynamicRecipePanel::WarnAboutPartialCompletion(void)
{
	if(!AllRecipePanelsFilled())
	{
		...
		loc_scpy_safe(wszWarning, CConstructLocalizedString(
									  GLocalizationProvider()->Find("Dynamic_Recipe_Partial_Completion_Warning"),
									  m_pDynamicRecipeItem->GetItemName()));

		CTFGenericConfirmDialog *pDialog = new CTFGenericConfirmDialog(
			"#Craft_Untradable_Title", wszWarning, "#GameUI_OK", "#Cancel", &ConfirmDestroyItems, NULL);
		if(pDialog)
		{
			pDialog->SetContext(this);
			pDialog->Show();
...
void CDynamicRecipePanel::AddNewItemPanel(int iPanelIndex)
{
	if(IsInputPanel(iPanelIndex))
	{
		...
		CInputPanelItemModelPanel *pPanel = vgui::SETUP_PANEL(
			new CInputPanelItemModelPanel(this, VarArgs("modelpanel%d", iPanelIndex), m_pDynamicRecipeItem));
	...
	else
	{
		...
		CRecipeComponentItemModelPanel *pPanel =
			vgui::SETUP_PANEL(new CRecipeComponentItemModelPanel(this, VarArgs("modelpanel%d", iPanelIndex)));
...
{
	return (iPanelIndex >= (DYNAMIC_RECIPE_INPUT_COUNT + DYNAMIC_RECIPE_OUTPUT_COUNT) && !IsInputPanel(iPanelIndex) &&
...
void CDynamicRecipePanel::PositionItemPanel(CItemModelPanel *pPanel, int iIndex)
{
	...
	if(IsInputPanel(iIndex))
	{
		...
		iXPos =
			(iCenter + m_iItemCraftingOffcenterX) + (iButtonX * pPanel->GetWide()) + (m_iItemBackpackXDelta * iButtonX);
...
void CDynamicRecipePanel::UpdateModelPanels(void)
{
	...
	m_pInputsLabel->SetText(bPartialCompletionAllowed ? "#Craft_Recipe_Inputs"
	...
	if(m_pOutputsLabel)
	{
		m_pOutputsLabel->SetText(AllRecipePanelsFilled() ? "#Craft_Recipe_Outputs"
...
void CDynamicRecipePanel::OnItemPanelMouseDoublePressed(vgui::Panel *panel)
{
	...
	else if(IsBackpackPanel(iIndex))
	{
		...
		FOR_EACH_VEC(m_vecRecipeInputModelPanels, i)
		{
			...
			if(pDstPanel->GetRecipeItem(pDstPanel->GetPageNumber()) == NULL &&
			   pDstPanel->MatchesAttribCriteria(nSrcRecipeIndex))
			{
				// Next check if we just want to move the item
				SetRecipeComponentIntoPanel(nSrcRecipeIndex, pSrcPanel, pSrcPanel->GetPageNumber(), pDstPanel,
											pDstPanel->GetPageNumber());
				break;
			}
		}
	}
...
void CDynamicRecipePanel::SetBorderForItem(CItemModelPanel *pItemPanel, bool bMouseOver)
{
	...
	if(IsInputPanel(iIndex))
	{
		...
		if(pInputPanel->GetRecipeItem(pInputPanel->GetPageNumber()) == NULL)
		{
			...
			bool bValidHighlightSourcePanel =
				pPanel == m_pMouseDragItemPanel ||
				m_vecBackpackModelPanels.Find(static_cast<CRecipeComponentItemModelPanel *>(pPanel)) !=
					m_vecBackpackModelPanels.InvalidIndex();
...
//-----------------------------------------------------------------------------
void CDynamicRecipePanel::SetRecipeComponentIntoPanel(itemid_t nSrcRecipeIndex,
													  CRecipeComponentItemModelPanel *pSrcPanel, int nSrcPage,
													  CRecipeComponentItemModelPanel *pDstPanel, int nDstPage)
{
...
bool CDynamicRecipePanel::CanDragTo(CItemModelPanel *pItemPanel, int iPanelIndex)
{
	...
	itemid_t itemID = m_pMouseDragItemPanel->GetItem() ? m_pMouseDragItemPanel->GetItem()->GetItemID() : 0;
...
void CDynamicRecipePanel::HandleDragTo(CItemModelPanel * /*pItemPanel*/, int iPanelIndex)
{
	...
	{
		CRecipeComponentItemModelPanel *pSrcPanel =
			dynamic_cast<CRecipeComponentItemModelPanel *>(m_pItemModelPanels[iDraggedFromPos]);
		CInputPanelItemModelPanel *pDstPanel =
			dynamic_cast<CInputPanelItemModelPanel *>(m_pItemModelPanels[iPanelIndex]);
		...
		if(pSrcPanel && pDstPanel)
		{
			...
			{
				SetRecipeComponentIntoPanel(nSrcRecipeIndex, pSrcPanel, GetCurrentPage(), pDstPanel,
											pDstPanel->GetPageNumber());
			}
		}
	}
	else if(IsInputPanel(iDraggedFromPos) && IsBackpackPanel(iPanelIndex))
	{
		CRecipeComponentItemModelPanel *pSrcPanel =
			dynamic_cast<CRecipeComponentItemModelPanel *>(m_pItemModelPanels[iDraggedFromPos]);
		CRecipeComponentItemModelPanel *pDstPanel =
			dynamic_cast<CRecipeComponentItemModelPanel *>(m_pItemModelPanels[iPanelIndex]);
		...
		if(pSrcPanel && pDstPanel)
		{
			...
			else if(pSrcRecipeIndex)
			{
				...
				SetRecipeComponentIntoPanel(pSrcRecipeIndex, pSrcPanel, pSrcPanel->GetPageNumber(), pDstPanel,
											GetCurrentPage());
			}
		}
	}
	...
	{
		CInputPanelItemModelPanel *pSrcPanel =
			dynamic_cast<CInputPanelItemModelPanel *>(m_pItemModelPanels[iDraggedFromPos]);
		CInputPanelItemModelPanel *pDstPanel =
			dynamic_cast<CInputPanelItemModelPanel *>(m_pItemModelPanels[iPanelIndex]);
...
//-----------------------------------------------------------------------------
void CDynamicRecipePanel::ReturnRecipeItemToBackpack(itemid_t nItemID, CRecipeComponentItemModelPanel *pSrcPanel,
													 int nSrcPage)
{
	...
	switch(eResponse)
	{
		case k_EGCMsgResponseOK:
		{
			...
			if(nNewToolID != 0)
			{
				...
				const char *pszSoundFilename = m_pDynamicRecipeItem->GetDefinitionString(
					"recipe_partial_complete_sound", "ui/chem_set_add_element.wav");
				vgui::surface()->PlaySound(pszSoundFilename);

				CEconItemView *pNewRecipe =
					TFInventoryManager()->GetLocalTFInventory()->GetInventoryItemByItemID(nNewToolID);
				Assert(pNewRecipe);
			...
			else
			{
				...
				const char *pszSoundFilename =
					m_pDynamicRecipeItem->GetDefinitionString("recipe_complete_sound", "ui/chem_set_creation.wav");
				vgui::surface()->PlaySound(pszSoundFilename);
...
GC_REG_JOB(GCSDK::CGCClient, CGCCompleteDynamicRecipeResponse, "CGCCompleteDynamicRecipeResponse",
		   k_EMsgGCFulfillDynamicRecipeComponentResponse, GCSDK::k_EServerTypeGCClient);
...
void CDynamicRecipePanel::Debug_GiveRequiredInputs() const
{
	...
	FOR_EACH_VEC(m_vecRecipeInputModelPanels, i)
	{
		...
		int nPage = 0;
		for(const CEconItemAttributeDefinition *pAttrDef = pInputPanel->GetAttrib(nPage); pAttrDef != NULL;
			pAttrDef = pInputPanel->GetAttrib(++nPage))
		{
			CAttribute_DynamicRecipeComponent attribValue;
			if(m_pDynamicRecipeItem->FindAttribute<CAttribute_DynamicRecipeComponent>(pAttrDef, &attribValue))
			{
				...
				{
					criteria.BAddCondition(
						"name", k_EOperator_String_EQ,
						GetItemSchema()->GetItemDefinition(attribValue.def_index())->GetDefinitionName(), true);
				}
