...
CPvPRankPanel::XPState_t::XPState_t(EMatchGroup eMatchGroup)
	: m_nStartXP(0u), m_nTargetXP(0u), m_nActualXP(0u), m_bCurrentDeltaViewed(true), m_eMatchGroup(eMatchGroup)
{
...
void CPvPRankPanel::XPState_t::UpdateXP(bool bInitial)
{
	...
	if(pMatchDesc && pMatchDesc->m_pProgressionDesc)
	{
		...
		if(steamapicontext && steamapicontext->SteamUser())
		{
			...
			nNewXP = pMatchDesc->m_pProgressionDesc->GetPlayerExperienceBySteamID(
...
void CPvPRankPanel::XPState_t::SOCreated(const CSteamID &steamIDOwner, const CSharedObject *pObject,
...
void CPvPRankPanel::XPState_t::SOUpdated(const CSteamID &steamIDOwner, const CSharedObject *pObject,
										 ESOCacheEvent eEvent)
{
	if(pObject->GetTypeID() == CSOTFLadderData::k_nTypeID)
	{
...
public:
	...
	CXPSourcePanel(Panel *pParent, const char *panelName, const CMsgTFXPSource &source)
		: BaseClass(pParent, panelName), m_source(source)
	{
	}
	...
	virtual void PerformLayout() OVERRIDE
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wszOutString, wpszFormat, 2,
											  g_pVGuiLocalize->Find(g_XPSourceDefs[m_source.type()].m_pszTypeLocToken),
											  wszCount);
...
public:
	...
	CScrollingXPSourcePanel(Panel *pParent, const char *panelName, const CMsgTFXPSource &source, float flStartTime)
		: BaseClass(pParent, panelName, source), m_flStartTime(flStartTime), m_bStarted(false)
	{
	...
	virtual void OnTick() OVERRIDE
	{
		...
		if(Plat_FloatTime() > m_flStartTime && !m_bStarted)
		{
			...
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
				this, m_source.amount() >= 0 ? "XPSourceShow_Positive" : "XPSourceShow_Negative", false);
...
void CPvPRankPanel::ApplySettings(KeyValues *inResourceData)
{
	...
	SetMatchGroup((EMatchGroup)StringFieldToInt(inResourceData->GetString("matchgroup"), s_pszMatchGroups,
...
void CPvPRankPanel::OnThink()
{
	...
	if(m_pContinuousProgressBar && m_pXPBar && m_pModelPanel && m_pProgressionDesc && m_pBGPanel)
	{
		...
		if(m_nLastLerpXP != nCurrentXP)
		{
			...
			// We only want to do level up effects if we are thinking (visible) when the current XP passes the level
			// boundary
		...
		else
		{
			...
			// our current level actually is.  This can happen if we haven't been thinking, but we did level up
			// somewhere else.  In this case, we need to update our badge.
			const LevelInfo_t &levelCur = m_pProgressionDesc->GetLevelForExperience(nCurrentXP);
...
void CPvPRankPanel::UpdateControls(uint32 nPreviousXP, uint32 nCurrentXP, const LevelInfo_t &levelCurrent)
{
	if(m_pContinuousProgressBar)
	{
		...
		float flPrevBarProgress =
			RemapValClamped((float)nPreviousXP, (float)levelCurrent.m_nStartXP, (float)levelCurrent.m_nEndXP, 0.f, 1.f);
		float flCurrentBarProgress =
			RemapValClamped((float)nCurrentXP, (float)levelCurrent.m_nStartXP, (float)levelCurrent.m_nEndXP, 0.f, 1.f);
	...
	if(m_pXPBar)
	{
		...
		if(levelCurrent.m_nLevelNum < m_pProgressionDesc->GetNumLevels())
		{
			m_pXPBar->SetDialogVariable("next_level_xp",
										LocalizeNumberWithToken("TF_Competitive_XP", levelCurrent.m_nEndXP));
		}
		...
		g_pVGuiLocalize->ConstructString_safe(wszOutString, wpszFormat, 2, wszCount,
											  g_pVGuiLocalize->Find(levelCurrent.m_pszLevelTitle));
...
void CPvPRankPanel::OnCommand(const char *command)
{
	if(FStrEq("medal_clicked", command))
	{
		...
		if(pModelContainer)
		{
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(pModelContainer,
																					"PvPRankModelClicked", false);
		}
...
void CPvPRankPanel::BeginXPLerp()
{
	...
	if(steamapicontext && steamapicontext->SteamUser())
	{
		GCSDK::CGCClientSharedObjectCache *pSOCache =
			GCClientSystem()->GetSOCache(steamapicontext->SteamUser()->GetSteamID());
		...
		if(pSOCache)
		{
			...
			if(pTypeCache)
			{
				...
				FOR_EACH_VEC(vecSources, i)
				{
					...
					CScrollingXPSourcePanel *pSourcePanel =
						new CScrollingXPSourcePanel(this, "XPSourcePanel", pXPSource->Obj(),
													Plat_FloatTime() + (i * tf_xp_breakdown_interval.GetFloat()));
					pSourcePanel->MakeReadyForUse();

					// Offset from the BGPanel.
					pSourcePanel->SetPos(
						m_pBGPanel->GetXPos() + m_iXPSourceNotificationCenterX - (pSourcePanel->GetWide() * 0.5f),
						m_pBGPanel->GetYPos() + m_pXPBar->GetYPos() + YRES(22) - pSourcePanel->GetTall());
				}
			}
		}
	}
...
void CPvPRankPanel::PlayLevelUpEffects(const LevelInfo_t &level) const
{
	...
	if(pModelContainer)
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(pModelContainer, "PvPRankLevelUpModel",
																				false);
	}
}
