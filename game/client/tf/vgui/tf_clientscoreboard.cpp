...
{
	CTFClientScoreBoardDialog *pScoreboard =
		dynamic_cast<CTFClientScoreBoardDialog *>(gViewPortInterface->FindPanelByName(PANEL_SCOREBOARD));
	if(pScoreboard)
	{
		pScoreboard->Reset();
	}
}
ConVar tf_scoreboard_ping_as_text("tf_scoreboard_ping_as_text", "0", FCVAR_ARCHIVE,
								  "Show ping values as text in the scoreboard.", cc_scoreboard_convar_changed);
ConVar tf_scoreboard_alt_class_icons("tf_scoreboard_alt_class_icons", "0", FCVAR_ARCHIVE,
									 "Show alternate class icons in the scoreboard.");
...
static const char *pszPingIcons[SCOREBOARD_PING_ICONS] = {
	"../hud/scoreboard_ping_low",		"../hud/scoreboard_ping_med",	  "../hud/scoreboard_ping_high",
	"../hud/scoreboard_ping_very_high", "../hud/scoreboard_ping_bot_red", "../hud/scoreboard_ping_bot_blue",
};
static const char *pszPingIconsDead[SCOREBOARD_PING_ICONS] = {
	"../hud/scoreboard_ping_low_d",		  "../hud/scoreboard_ping_med_d",	  "../hud/scoreboard_ping_high_d",
	"../hud/scoreboard_ping_very_high_d", "../hud/scoreboard_ping_bot_red_d", "../hud/scoreboard_ping_bot_blue_d",
};
...
void CTFClientScoreBoardDialog::UpdatePlayerModel()
{
	...
	int nItemSlot =
		(pPlayer->IsAlive() && pPlayer->GetActiveTFWeapon())
			? pPlayer->GetActiveTFWeapon()->GetAttributeContainer()->GetItem()->GetStaticData()->GetLoadoutSlot(nClass)
...
void CTFClientScoreBoardDialog::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_duelPanelLocalPlayer.m_pAvatar =
		dynamic_cast<CAvatarImagePanel *>(m_duelPanelLocalPlayer.m_pPanel->FindChildByName("AvatarImage"));
	m_duelPanelLocalPlayer.m_pPlayerNameLabel =
		dynamic_cast<CExLabel *>(m_duelPanelLocalPlayer.m_pPanel->FindChildByName("AvatarTextLabel"));
	m_duelPanelOpponent.m_pAvatar =
		dynamic_cast<CAvatarImagePanel *>(m_duelPanelOpponent.m_pPanel->FindChildByName("AvatarImage"));
	m_duelPanelOpponent.m_pPlayerNameLabel =
		dynamic_cast<CExLabel *>(m_duelPanelOpponent.m_pPanel->FindChildByName("AvatarTextLabel"));
			...
			if(iSelectedItem >= 0)
			{
				...
				if(playerIndex > 0 && playerIndex <= MAX_PLAYERS && GetClientVoiceMgr())
				{
					...
					GetClientVoiceMgr()->SetPlayerBlockedState(
						playerIndex, (GetClientVoiceMgr()->IsPlayerBlocked(playerIndex) ? false : true));
				}
			}
		}
	}
...
void CTFClientScoreBoardDialog::OnItemSelected(vgui::Panel *panel)
{
	if(panel == m_pPlayerListBlue || panel == m_pPlayerListRed)
	{
		...
		if(panel == m_pPlayerListBlue && m_pPlayerListBlue->GetSelectedItem() >= 0 &&
		   m_pPlayerListRed->GetSelectedItem() >= 0)
		{
			m_pPlayerListRed->ClearSelection();
		}
		else if(panel == m_pPlayerListRed && m_pPlayerListRed->GetSelectedItem() >= 0 &&
				m_pPlayerListBlue->GetSelectedItem() >= 0)
		{
...
void CTFClientScoreBoardDialog::OnScoreBoardMouseRightRelease(void)
{
	...
	bool bTournamentGame =
		(g_TF_PR->GetTeam(playerIndex) >= FIRST_GAME_TEAM && TFGameRules() &&
		 TFGameRules()->UsePlayerReadyStatusMode() && TFGameRules()->State_Get() == GR_STATE_RND_RUNNING);
	...
	if(!bFakeClient && engine->GetLocalPlayer() != playerIndex)
	{
		...
		for(int iReason = CMsgGC_ReportPlayer_EReason_kReason_INVALID + 1;
			iReason < CMsgGC_ReportPlayer_EReason_kReason_COUNT; ++iReason)
		{
	...
	{
		const char *pszString = GetClientVoiceMgr()->IsPlayerBlocked(playerIndex) ? "#TF_ScoreBoard_Context_UnMute"
	...
	if(g_TF_PR->IsAlive(playerIndex) &&
	   (!pLocalTFPlayer->IsAlive() || pLocalTFPlayer->GetTeamNumber() == TEAM_SPECTATOR))
	{
	...
	if(!bFakeClient)
	{
		...
		if(pTFTarget && pTFTarget->GetSteamID(&steamID))
		{
			if(steamapicontext && steamapicontext->SteamFriends() &&
			   steamapicontext->SteamFriends()->GetFriendRelationship(steamID) == k_EFriendRelationshipNone)
			{
...
void CTFClientScoreBoardDialog::InitializeInputScheme(void)
{
	...
	if(m_pPlayerListBlue->GetScrollBar() && m_pPlayerListBlue->GetScrollBar()->GetButton(0) &&
	   m_pPlayerListBlue->GetScrollBar()->GetButton(1) && m_pPlayerListBlue->GetScrollBar()->GetSlider())
	{
	...
	if(m_pPlayerListRed->GetScrollBar() && m_pPlayerListRed->GetScrollBar()->GetButton(0) &&
	   m_pPlayerListRed->GetScrollBar()->GetButton(1) && m_pPlayerListRed->GetScrollBar()->GetSlider())
	{
...
void CTFClientScoreBoardDialog::InitPlayerList(SectionedListPanel *pPlayerList)
{
	...
	pPlayerList->AddColumnToSection(
		0, "medal", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_CENTER, m_iMedalWidth);

	// Avatars are always displayed at 32x32 regardless of resolution
	if(ShowAvatars())
	{
		pPlayerList->AddColumnToSection(
			0, "avatar", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_RIGHT, m_iAvatarWidth);
	...
	// to adjust the total width of all the columns
	m_nExtraSpace =
		pPlayerList->GetWide() - m_iMedalWidth - m_iAvatarWidth - m_iSpacerWidth - m_iNameWidth - m_iKillstreakWidth -
		m_iKillstreakImageWidth - m_iNemesisWidth - m_iNemesisWidth - m_iScoreWidth - m_iClassWidth - m_iPingWidth -
		m_iSpacerWidth -
		(2 * SectionedListPanel::COLUMN_DATA_INDENT); // the SectionedListPanel will indent the columns on either end by
													  // SectionedListPanel::COLUMN_DATA_INDENT
	...
	pPlayerList->AddColumnToSection(0, "killstreak_image", "", SectionedListPanel::COLUMN_IMAGE,
									m_iKillstreakImageWidth);
	pPlayerList->AddColumnToSection(
		0, "dominating", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_CENTER, m_iNemesisWidth);
	pPlayerList->AddColumnToSection(
		0, "nemesis", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_CENTER, m_iNemesisWidth);
	pPlayerList->AddColumnToSection(0, "score", "#TF_Scoreboard_Score", SectionedListPanel::COLUMN_RIGHT,
									m_iScoreWidth);
	pPlayerList->AddColumnToSection(0, "class", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_RIGHT,
									m_iClassWidth);

	if(tf_scoreboard_ping_as_text.GetBool())
	{
		pPlayerList->AddColumnToSection(0, "ping", "#TF_Scoreboard_Ping", SectionedListPanel::COLUMN_RIGHT,
										m_iPingWidth);
	}
	else
	{
		pPlayerList->AddColumnToSection(
			0, "ping", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_RIGHT, m_iPingWidth);
	}
#ifdef STAGING_ONLY
// 	if ( m_bDisplayLevel )
// 	{
//		pPlayerList->AddColumnToSection( 0, "level", "#TF_ScoreBoard_LevelLabel", SectionedListPanel::COLUMN_RIGHT,
//m_iPingWidth );
// 	}
#endif // STAGING_ONLY
}
...
void CTFClientScoreBoardDialog::Update()
{
	...
	if(UseMouseMode())
	{
		...
		{
			// we need a much faster update rate for MouseMode() so the MedalList can update when the scrollbar is
			// quickly moved we're listening for the ScrollBarSliderMoved message but it still doesn't keep up with
...
void CTFClientScoreBoardDialog::UpdateTeamInfo()
{
	...
	for(int teamIndex = TF_TEAM_RED; teamIndex <= TF_TEAM_BLUE; teamIndex++)
	{
		...
		if(team)
		{
			...
			{
				g_pVGuiLocalize->ConstructString_safe(string1, g_pVGuiLocalize->Find("#TF_ScoreBoard_Player"), 1,
													  wNumPlayers);
			}
			else
			{
				g_pVGuiLocalize->ConstructString_safe(string1, g_pVGuiLocalize->Find("#TF_ScoreBoard_Players"), 1,
													  wNumPlayers);
			}
	...
	if(bShowAvatars)
	{
		m_pRedLeaderAvatarImage->SetPlayer(GetSteamIDForPlayerIndex(g_TF_PR->GetPartyLeaderRedTeamIndex()),
										   k_EAvatarSize64x64);
		m_pRedLeaderAvatarImage->SetShouldDrawFriendIcon(false);
		m_pBlueLeaderAvatarImage->SetPlayer(GetSteamIDForPlayerIndex(g_TF_PR->GetPartyLeaderBlueTeamIndex()),
											k_EAvatarSize64x64);
...
{
	if(m_pPlayerListRed && m_pPlayerListRed->GetScrollBar() &&
	...
	if(m_pPlayerListBlue && m_pPlayerListBlue->GetScrollBar() &&
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFClientScoreBoardDialog::UpdateBadgePanels(CUtlVector<CTFBadgePanel *> &pBadgePanels,
												  vgui::SectionedListPanel *pPlayerList)
{
	int iNumPanels = 0;

	const IMatchGroupDescription *pMatchDesc =
		TFGameRules() ? GetMatchGroupDescription(TFGameRules()->GetCurrentMatchGroup()) : NULL;
		...
		if(TFGameRules()->IsMatchTypeCasual())
		{
			...
			for(int i = 0; i < pPlayerList->GetItemCount(); i++)
			{
				...
				if(steamID.IsValid())
				{
					...
					if((nPanelXPos != xParent + x) || (nPanelYPos != yParent + y) || (nPanelWide != wide) ||
					   (nPanelTall != tall))
					{
	...
	for(int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++)
	{
		if(g_PR->IsConnected(playerIndex) || g_PR->IsValid(playerIndex))
		{
			...
			if(pwszFormat)
			{
				...
				pPlayerList->SetItemFgColor(itemID, (eConnectionState == MM_DISCONNECTED) ? Color(208, 147, 7, 255)
																						  : Color(76, 107, 34, 255));
			...
			// check for bots first, so malicious server operators can't fake a ping and stuff their server with bots
			...
			if(!AreEnemyTeams(g_PR->GetTeam(playerIndex), localteam) && !(localteam == TEAM_UNASSIGNED))
			{
				...
				if(g_PR->IsConnected(playerIndex))
				{
					...
					{
						// If this is local player and he is dead, show desired class (which he will spawn as) rather
						// than current class.
						int iDesiredClass = pLocalTFPlayer->m_Shared.GetDesiredPlayerClassIndex();
						// use desired class unless it's random -- if random, his future class is not decided until
						// moment of spawn
					...
					if(iClass >= TF_FIRST_NORMAL_CLASS && iClass <= TF_LAST_NORMAL_CLASS)
					{
						if(bAlive)
						{
							pKeyValues->SetInt("class", tf_scoreboard_alt_class_icons.GetBool()
															? m_iImageClassAlt[iClass]
															: m_iImageClass[iClass]);
						}
						else
						{
							pKeyValues->SetInt(
								"class",
								tf_scoreboard_alt_class_icons.GetBool()
									? m_iImageClassAlt[iClass + 9]
									: m_iImageClass[iClass + 9]); // +9 is to jump ahead to the darker dead icons
						}
					}
					else
					{
...
// 				}
			...
			// change color based off alive or dead status. Also slightly different if its local player since the name
			// is highlighted.
			if(!bAlive)
			{
...
void CTFClientScoreBoardDialog::UpdateSpectatorList()
{
	...
	if(nSpectators > 0)
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wzSpectatorText, g_pVGuiLocalize->Find(pchFormat), 2, wzSpectatorCount,
											  wzSpectatorList);
		if(nCoaches > 0)
		{
	...
	if(TFGameRules() && TFGameRules()->IsInArenaMode() == true)
	{
		...
		if(nSpectators > 0)
		{
			const char *pchFormat =
				(1 == nSpectators ? "#TF_Arena_ScoreBoard_Spectator" : "#TF_Arena_ScoreBoard_Spectators");

			wchar_t wzSpectatorCount[16];
			wchar_t wzSpectatorList[1024];
			_snwprintf(wzSpectatorCount, ARRAYSIZE(wzSpectatorCount), L"%i", nSpectators);
			g_pVGuiLocalize->ConvertANSIToUnicode(szSpectatorList, wzSpectatorList, sizeof(wzSpectatorList));
			g_pVGuiLocalize->ConstructString_safe(wzSpectators, g_pVGuiLocalize->Find(pchFormat), 2, wzSpectatorCount,
												  wzSpectatorList);
		}
	}
...
void CTFClientScoreBoardDialog::UpdatePlayerDetails()
{
	...
	else
	{
		...
		m_pLocalPlayerStatsPanel->SetDialogVariable("destruction",
													pSelectedPlayer->m_Shared.GetBuildingsDestroyed(playerIndex));
		if(m_pDestructionLabel)
		{
			m_pDestructionLabel->SetFgColor(pSelectedPlayer->m_Shared.GetBuildingsDestroyed(playerIndex) ? cGreen
																										 : cWhite);
		}
		...
		m_pLocalPlayerStatsPanel->SetDialogVariable("dominations",
													pSelectedPlayer->m_Shared.GetDominations(playerIndex));
...
void CTFClientScoreBoardDialog::UpdateServerTimeLeft()
{
	...
	if(iServerTimeLimit == 0)
	{
		...
		if(m_pServerTimeLeftValue && m_pServerTimeLeftValue->IsVisible() &&
		   (m_pFontTimeLeftString != vgui::INVALID_FONT))
		{
	...
	if(iTimeLeft == 0)
	{
		g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft, g_pVGuiLocalize->Find("#Scoreboard_ChangeOnRoundEnd"),
											  0);
		SetDialogVariable("servertimeleft", wzServerTimeLeft);

		g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft,
											  g_pVGuiLocalize->Find("#Scoreboard_ChangeOnRoundEndNew"), 0);
		SetDialogVariable("servertime", wzServerTimeLeft);
	...
	if(iHours == 0)
	{
		g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft, g_pVGuiLocalize->Find("#Scoreboard_TimeLeftNoHours"), 2,
											  wzServerTimeMinLeft, wzServerTimeSecLeft);
		SetDialogVariable("servertimeleft", wzServerTimeLeft);

		g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft, g_pVGuiLocalize->Find("#Scoreboard_TimeLeftNoHoursNew"),
											  2, wzServerTimeMinLeft, wzServerTimeSecLeft);
		SetDialogVariable("servertime", wzServerTimeLeft);

		return;
	}

	g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft, g_pVGuiLocalize->Find("#Scoreboard_TimeLeft"), 3,
										  wzServerTimeHrsLeft, wzServerTimeMinLeft, wzServerTimeSecLeft);
	SetDialogVariable("servertimeleft", wzServerTimeLeft);

	g_pVGuiLocalize->ConstructString_safe(wzServerTimeLeft, g_pVGuiLocalize->Find("#Scoreboard_TimeLeftNew"), 3,
										  wzServerTimeHrsLeft, wzServerTimeMinLeft, wzServerTimeSecLeft);
	SetDialogVariable("servertime", wzServerTimeLeft);
}
...
bool CTFClientScoreBoardDialog::TFPlayerSortFunc(vgui::SectionedListPanel *list, int itemID1, int itemID2)
{
	...
	// sort by connected status next ( 0 is looking for player, 1 is disconnected/connecting/loading, 2 is fully
	// connected )
...
void CTFClientScoreBoardDialog::FireGameEvent(IGameEvent *event)
{
	...
	if(FStrEq(type, "server_spawn"))
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wzServerLabel, g_pVGuiLocalize->Find("#Scoreboard_Server"), 1,
											  wzHostName);
