...
private:
	struct XPState_t : public CGameEventListener, public CLocalSteamSharedObjectListener
	{
		...
		uint32 GetStartXP() const
		{
			return m_nStartXP;
		}
		// Get the target XP that the delta lerp is headed to
		uint32 GetTargetXP() const
		{
			return m_nTargetXP;
		}
		uint32 GetActualXP() const
		{
			return m_nActualXP;
		}
		...
		virtual void SOCreated(const CSteamID &steamIDOwner, const CSharedObject *pObject,
							   ESOCacheEvent eEvent) OVERRIDE;
		virtual void SOUpdated(const CSteamID &steamIDOwner, const CSharedObject *pObject,
							   ESOCacheEvent eEvent) OVERRIDE;
	...
	CPanelAnimationVarAliasType(int, m_iXPSourceNotificationCenterX, "xp_source_notification_center_x", "19",
								"proportional_int");
};
