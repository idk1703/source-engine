...
// Purpose:
//-----------------------------------------------------------------------------
CTFStorePreviewItemPanelBase::CTFStorePreviewItemPanelBase(vgui::Panel *pParent, const char *pResFile,
														   const char *pPanelName, CStorePage *pOwner)
...
void CTFStorePreviewItemPanelBase::PerformLayout(void)
{
	...
	if(iNumItemIcons || iNumClassIcons)
	{
		...
		int interval = XRES(2);
		int totalWidth = (iNumItemIcons * m_pItemIcons[0]->GetWide()) + (iNumClassIcons * m_pClassIcons[0]->GetWide()) +
...
static void UpdatePaintColorsForTeam(CTFPlayerModelPanel *pPlayerModelPanel, uint32 unRGB0, uint32 unRGB1)
{
	...
	for(int i = 0; i < items.Count(); ++i)
	{
		...
		{
			items[i]->GetAttributeList()->SetRuntimeAttributeValue(
				pAttrDef_ItemTintRGB, pPlayerModelPanel->GetTeam() == TF_TEAM_RED ? unRGB0 : unRGB1);
...
{
	return m_pPlayerModelPanel ? m_pPlayerModelPanel->GetTeam() : TF_TEAM_RED;
}
	...
	// Add paint options sub menu
	{
		...
		FOR_EACH_VEC(m_vecPaintCans, i)
		{
			...
			GameItemDefinition_t *pPaintCanDef =
				dynamic_cast<GameItemDefinition_t *>(GEconItemSchema().GetItemDefinition(paintItemDefIndex));
			if(!CEconSharedToolSupport::ToolCanApplyToDefinition(
				   dynamic_cast<const GameItemDefinition_t *>(pPaintCanDef), m_item.GetStaticData()))
				continue;
			...
			int nIndex = pPaintSubMenu->AddMenuItem(
				"", new KeyValues("Command", "command", CFmtStr("SetPaint%d", paintItemDefIndex)), this);
			...
			CItemMaterialCustomizationIconPanel *pCustomPanel =
				new CItemMaterialCustomizationIconPanel(pMenuItem, "paint");
	...
	// Add style
	{
		...
		if(m_item.GetStaticData()->GetNumSelectableStyles() > 1)
		{
			for(style_index_t unStyle = 0; unStyle < m_item.GetStaticData()->GetNumStyles(); ++unStyle)
			{
				...
				int nIndex = pStyleSubMenu->AddMenuItem(
					"", new KeyValues("Command", "command", CFmtStr("SetStyle%d", unStyle)), this);
	...
	// Add unusual
	{
		...
		if(pUnusualList)
		{
			...
			for(int i = 0; i < pUnusualList->Count(); ++i)
			{
				...
				int nIndex = pUnusualSubMenu->AddMenuItem(
					"", new KeyValues("Command", "command", CFmtStr("SetUnusual%d", iParticleIndex)), this);
...
void CTFStorePreviewItemPanelBase::UpdateNextWeaponButton(void)
{
	...
	bShowNextWeaponsButton |= iNumItemsArray[LOADOUT_POSITION_PRIMARY] + iNumItemsArray[LOADOUT_POSITION_SECONDARY] +
								  iNumItemsArray[LOADOUT_POSITION_MELEE] >
							  1;
...
void CTFStorePreviewItemPanelBase::OnShowClassIconMouseover(KeyValues *data)
{
	if(m_pClassIconMouseoverLabel)
	{
		...
		{
			const char *pszLocString =
				bIsABundle ? "#Store_ClassImageMouseoverAllBundle" : "#Store_ClassImageMouseoverAll";
...
const CUtlVector<int> *CTFStorePreviewItemPanelBase::GetUnusualList() const
{
	...
	else if(m_item.GetItemDefinition()->GetEquipRegionMask() & GetItemSchema()->GetEquipRegionBitMaskByName("hat") ||
			m_item.GetItemDefinition()->GetEquipRegionMask() &
