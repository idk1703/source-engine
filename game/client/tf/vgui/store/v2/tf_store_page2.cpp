...
{
public:
	CClassFilterTooltip(CTFStorePage2 *pStorePage) : BaseTooltip(pStorePage), m_pStorePage(pStorePage) {}
	...
	virtual void ShowTooltip(Panel *currentPanel)
	{
		...
		// The tooltip wants to be centered around the given panel, but it's constrained by the left and right
		// boundaries of the navigation panel.
		if(m_pStorePage->m_pClassFilterButtons)
		{
			...
			x = clamp(x + (currentPanel->GetWide() - nTipWide) / 2, aClassFilterNavPos[0],
...
// Purpose:
//-----------------------------------------------------------------------------
CTFStorePage2::CTFStorePage2(Panel *parent, const CEconStoreCategoryManager::StoreCategory_t *pPageData,
	...
	: BaseClass(parent, pPageData, pPreviewItemResFile),
	  ...
	  m_iCurrentSubcategory(
		  2), // Switch to Featured items tab on home page when store launches; BRETT SAID I COULD DO THIS
	  ...
	  m_flFilterItemTime(0.0f)
{
	...
	if(IsHomePage())
	{
		...
		FOR_EACH_VEC(m_pPageData->m_vecSubcategories, i)
		{
			...
			bool bIsOnSaleCategory =
				m_pPageData->m_vecSubcategories[i]->m_unID == CEconStoreCategoryManager::k_CategoryID_OnSale;

			// Skip over the "Top Sellers" tab as we're replacing it with the 'Starter Packs' tab for now
			bool bIsPopularCategory =
				m_pPageData->m_vecSubcategories[i]->m_unID == CEconStoreCategoryManager::k_CategoryID_Popular;
			...
			// We include all of the sale items in the Featured tab currently, so we don't need to add these categories
...
void CTFStorePage2::OnNavButtonSelected(KeyValues *pData)
{
	...
	else if(pPanel == m_pHomeCategoryTabs)
	{
		...
		C_CTFGameStats::ImmediateWriteInterfaceEvent("store_page_2_nav(category)",
													 CFmtStr("%i", iSelectedTab).Access());
	}
}
...
void CTFStorePage2::OnTextChanged(KeyValues *data)
{
	...
	if(pComboBox)
	{
		if(pComboBox == m_pSubcategoriesFilterCombo)
		{
			...
			C_CTFGameStats::ImmediateWriteInterfaceEvent("store_page_2_nav(subcategories)",
														 CFmtStr("%i", m_iCurrentSubcategory).Access());
		}
		...
		else if(pComboBox == m_pSortByCombo)
		{
			...
			if(iSortTypeSelectionIndex >= 0)
			{
				...
				C_CTFGameStats::ImmediateWriteInterfaceEvent("store_page_2_nav(sort_by)",
															 CFmtStr("%i", iSortType).Access());
			}
		}
	}
}
...
bool CTFStorePage2::FindAndSelectEntry(const econ_store_entry_t *pEntry)
{
	...
	{
		ivgui()->PostMessage(GetVPanel(),
							 new KeyValues("ItemDefDetails", "ItemDefIndex", pEntry->GetItemDefinitionIndex()),
							 GetVPanel());
		return true;
	}
	return false;
...
void CTFStorePage2::UpdateFilteredItems()
{
	if(m_bFilterDirty)
	{
		...
		if(m_pSortByCombo)
		{
			...
			if(pUserData)
			{
				...
				if(iSortTypeSelectionIndex >= 0)
				{
					eEconStoreSortType iSortType =
						(eEconStoreSortType)g_StoreSortTypes[iSortTypeSelectionIndex].iSortType;
	...
	{
		m_vecItemPanels[idx].m_pItemModelPanel->SetShowQuantity(unSubcategoryID !=
																CEconStoreCategoryManager::k_CategoryID_Popular);
	}
	...
	{
		const CEconStorePriceSheet::FeaturedItems_t &vecFeaturedItems =
			pStorePanel->GetPriceSheet()->GetFeaturedItems();
	...
	else if(unSubcategoryID == CEconStoreCategoryManager::k_CategoryID_ClassBundles)
	{
		...
		// extern int ItemNameSortComparator( const econ_store_entry_t *const *ppEntryA, const econ_store_entry_t *const
		// *ppEntryB );
		extern int FirstSaleDateSortComparator(const econ_store_entry_t *const *ppItemA,
											   const econ_store_entry_t *const *ppItemB);
	...
	// by sale date.
	if(unSubcategoryID == CEconStoreCategoryManager::k_CategoryID_New ||
	   unSubcategoryID == CEconStoreCategoryManager::k_CategoryID_OnSale ||
	   unSubcategoryID == CEconStoreCategoryManager::k_CategoryID_Taunts)
	{
		extern int FirstSaleDateSortComparator(const econ_store_entry_t *const *ppItemA,
											   const econ_store_entry_t *const *ppItemB);
...
{
	if(m_pPageData && m_pPageData->m_bIsHome && HasSubcategories() &&
	   m_pPageData->m_vecSubcategories.IsValidIndex(m_iCurrentSubcategory) &&
	   m_pPageData->m_vecSubcategories[m_iCurrentSubcategory]->m_unID ==
		   CEconStoreCategoryManager::k_CategoryID_Popular)
	{
