...
void CFullscreenStorePreviewItem::GoFullscreen(CTFPlayerModelPanel *pPlayerModelPanel)
{
	...
	m_pPlayerModelPanel->GetBounds(
		m_OldModelState.m_aPlayerModelPanelBounds[0], m_OldModelState.m_aPlayerModelPanelBounds[1],
		m_OldModelState.m_aPlayerModelPanelBounds[2], m_OldModelState.m_aPlayerModelPanelBounds[3]);
...
void CFullscreenStorePreviewItem::ExitFullscreen()
{
	...
	if(m_pPlayerModelPanel.Get())
	{
		...
		m_pPlayerModelPanel->SetBounds(
			m_OldModelState.m_aPlayerModelPanelBounds[0], m_OldModelState.m_aPlayerModelPanelBounds[1],
			m_OldModelState.m_aPlayerModelPanelBounds[2], m_OldModelState.m_aPlayerModelPanelBounds[3]);

		// Reset the player position to it's pre-fullscreen location, but add on any zoom delta if needed.
		const Vector vecZoomOffset = m_OldModelState.m_bZoomed != m_pPlayerModelPanel->IsZoomed()
										 ? m_pPlayerModelPanel->GetZoomOffset()
										 : vec3_origin;
...
void CFullscreenStorePreviewItem::OnNavButtonSelected(KeyValues *pData)
{
	...
	C_CTFGameStats::ImmediateWriteInterfaceEvent("team_switch_%s(store_preview_item_panel_fullscreen)",
...
void CFullscreenStorePreviewItem::OnThink()
{
	...
	if(m_pOverlayPanel)
	{
		const float flButtonBlockerFade =
			SCurve(LerpScale(gpGlobals->realtime, m_flLastMouseMoveTime + m_flUiFadeoutTime,
							 m_flLastMouseMoveTime + m_flUiFadeoutTime + m_flUiFadeoutDuration, 0.0f, 1.0f));
	...
	const float flFade = SCurve(LerpScale(gpGlobals->realtime, m_flGoFullscreenStartTime,
										  m_flGoFullscreenStartTime + m_flFullscreenFadeToBlackDuration, 0.0f, 1.0f));
	...
	const int aBounds[4] = {Lerp(flFade, m_OldModelState.m_aPlayerModelPanelBounds[0], aDstBounds[0]),
	...
	if(flFade < 0.999f)
	{
		const Vector vecZoomOffset =
			m_pPlayerModelPanel->IsZoomed() ? m_pPlayerModelPanel->GetZoomOffset() : vec3_origin;
		const Vector vecFullscreenOrigin(m_flModelPanelOriginX, m_flModelPanelOriginY, m_flModelPanelOriginZ);
		m_pPlayerModelPanel->m_vecPlayerPos =
			Lerp(flFade, m_OldModelState.m_vecPlayerPos, vecFullscreenOrigin + vecZoomOffset);
	}
...
// Purpose:
//-----------------------------------------------------------------------------
CTFStorePreviewItemPanel2::CTFStorePreviewItemPanel2(vgui::Panel *pParent, const char *pResFile, const char *pPanelName,
													 CStorePage *pOwner)
...
void CTFStorePreviewItemPanel2::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	if(m_pDialogFrame)
	{
		...
		if(m_pDetailsView)
		{
			...
			m_pItemCollectionHighlight =
				dynamic_cast<EditablePanel *>(m_pDetailsViewChild->FindChildByName("collectionhighlight"));
			if(m_pItemCollectionHighlight)
			{
				m_pItemCollectionHighlight->InvalidateLayout(true, true);
			}
			m_pItemWikiPageButton =
				dynamic_cast<CExButton *>(m_pDetailsViewChild->FindChildByName("ItemWikiPageButton"));
...
//-----------------------------------------------------------------------------
int CTFStorePreviewItemPanel2::PlaceControl(Panel *pParent, const char *pControlNameA, const char *pControlNameB,
											int nOffset, bool bVertical, bool bSizeAToContents /*=true*/,
											bool bUseContentSize /*=true*/)
{
...
void CTFStorePreviewItemPanel2::PerformLayout(void)
{
	...
	if(m_pDialogFrame && (iNumItemIcons || iNumClassIcons))
	{
		...
		int interval = XRES(2);
		int totalWidth = (iNumItemIcons > 0 ? iNumItemIcons * m_pItemIcons[0]->GetWide() : 0) +
						 (iNumClassIcons * m_pClassIcons[0]->GetWide()) +
						 (interval * (iNumItemIcons + iNumClassIcons - 1));
	...
	if(m_pItemNameLabel && m_item.IsValid())
	{
		const CEconItemRarityDefinition *pItemRarity =
			GetItemSchema()->GetRarityDefinition(m_item.GetItemDefinition()->GetRarity());
	...
	if(nNumButtonsNeeded)
	{
		...
		for(int i = 0; i < nNumButtonsNeeded; ++i)
		{
			...
			pCurButton->SetPos(aDialogFramePos[0] + aViewportPos[0] +
								   (i + 1) * m_pPreviewViewportBg->GetWide() / (nNumButtonsNeeded + 1) -
								   m_iControlButtonWidth / 2,
							   m_iControlButtonY);
	...
	PlaceControl(m_pDetailsViewChild, m_nNumAttribLinesAdded == 0 ? "PriceLabel" : "AttributesLabel",
				 "ItemWikiPageButton", m_iBigVerticalBreakSize, true);

	PlaceControl(m_pDetailsViewChild, "ItemWikiPageButton", "TradableLabel", m_iBigVerticalBreakSize, true, false,
				 false);
...
void CTFStorePreviewItemPanel2::OnCommand(const char *command)
{
	...
	{
#ifdef ENABLE_STORE_RENTAL_BACKEND
		ECartItemType eCartItemType = !V_stricmp(command, "addrentaltocart_1day")	? kCartItem_Rental_1Day
									  : !V_stricmp(command, "addrentaltocart_3day") ? kCartItem_Rental_3Day
									  : !V_stricmp(command, "addrentaltocart_7day") ? kCartItem_Rental_7Day
																					: kCartItem_Purchase;
#else
	...
	else if(!V_strnicmp(command, "viewwikipage", 12))
	{
		if(steamapicontext && steamapicontext->SteamFriends() && m_pItemFullImage)
		{
			...
			if(pItem->IsValid())
			{
				...
				char szURL[512];
				Q_snprintf(szURL, sizeof(szURL), "http://wiki.teamfortress.com/scripts/itemredirect.php?id=%d&lang=%s",
						   pItem->GetItemDefIndex(), GetLanguageICUName(iLang));
...
void CTFStorePreviewItemPanel2::PreviewItem(int iClass, CEconItemView *pItem, const econ_store_entry_t *pEntry)
{
	...
	C_CTFGameStats::ImmediateWriteInterfaceEvent("store_preview_item_panel(preview_item)",
												 CFmtStr("%i", m_item.GetItemDefIndex()).Access());
	...
	if(m_pDialogFrame && m_pDetailsView && m_pItemFullImage && m_pItemFullImage->GetItem() && m_pAttributesLabel)
	{
		...
		if(pFullItem && pDef)
		{
			...
			if(pSchema)
			{
				...
				if(bAllClasses)
				{
					m_pDetailsViewChild->SetDialogVariable("used_by_classes",
														   g_pVGuiLocalize->Find("#Store_ItemDesc_AllClasses"));
				}
				else
				{
					for(int i = 0; i < LOADOUT_COUNT; ++i)
					{
						if(pbvClassUsability->IsBitSet(i))
						{
							V_wcscat_safe(wszClassNames,
										  nClassAdded++ == 0
											  ? L""
											  : L", "); // add empty lines everywhere except before the first line
				...
				m_pDetailsViewChild->SetDialogVariable(
					"slot",
					g_pVGuiLocalize->Find(bSlotValid ? CFmtStr("#LoadoutSlot_%s", vecLoadoutStrings[iSlot]).Access()
													 : "#Store_ItemDesc_Slot_None"));
				...
				const wchar_t *pYesNo[2] = {g_pVGuiLocalize->Find("#Store_ItemDesc_Yes"),
				...
				m_pDetailsViewChild->SetDialogVariable(
					"nameable", (pDef->GetCapabilities() & ITEM_CAP_NAMEABLE) != 0 ? pYesNo[0] : pYesNo[1]);

				m_pDetailsViewChild->SetDialogVariable("tradable", bIsTradeable);

				// No store-bought items are craftable, but items that were going to be tradable would show as craftable
				// because they weren't real items with a real origin that would prevent them from being crafted. In the
				// short term it makes more sense to just force this to always display "false" because at least it will
				// never be wrong.
				m_pDetailsViewChild->SetDialogVariable(
					"craftable", pDef->IsBundle() ? g_pVGuiLocalize->Find("#Attrib_CannotCraftWeapons")
								 : pDef->GetCapabilities() & ITEM_CAP_CAN_BE_CRAFTED_IF_PURCHASED ? pYesNo[0]
																								  : pYesNo[1]);
				...
				if(pEntry)
				{
					...
					if(pEntry->IsRentable() && pwsRentalPriceFormat)
					{
						...
						MakeMoneyString(wzRentalLocalizedPrice, ARRAYSIZE(wzRentalLocalizedPrice),
										pEntry->GetRentalPriceScale() * iTotalPrice, eCurrency)

							wchar_t wzLocalizedPriceString[96];
						::ILocalize::ConstructString_safe(wzLocalizedPriceString, pwsRentalPriceFormat, 2,
														  wzLocalizedPrice, wzRentalLocalizedPrice);
...
#endif
					{
						if(pEntry->m_bIsMarketItem)
						{
							if(iTotalPrice != 0)
							{
								...
								g_pVGuiLocalize->ConstructString_safe(wzMarketString, LOCCHAR("%s1 %s2"), 2,
		...
		if(pDescription)
		{
			...
			for(uint32 i = 0; i < pDescription->GetLineCount(); ++i)
			{
				...
				else if((line.unMetaType & kDescLineFlagSet_DisplayInAttributeBlock) != 0 && m_pAttributesLabel)
				{
					...
					Color col = bIsCurrentCollectionItem ? Color(0, 0, 0, 255)
														 : pScheme->GetColor(GetColorNameForAttribColor(line.eColor),
					...
					if(line.unDefIndex != INVALID_ITEM_DEF_INDEX)
					{
						...
						CItemModelPanel *pItemModelPanel = new CItemModelPanel(
							m_pAttributesLabel, CFmtStr("reference_item_%d", m_vecReferenceItemPanels.Count()));
		...
		// Get PerformLayout() called, now that we have text in our controls - without this, SizeToContents() sizes all
		// the labels as tall and narrow.
...
void CTFStorePreviewItemPanel2::OnNavButtonSelected(KeyValues *pData)
{
	...
	C_CTFGameStats::ImmediateWriteInterfaceEvent("team_switch(store_preview_item_panel)",
												 iTeam == TF_TEAM_RED ? "red" : "blu");
}
