...
const char *g_szClassFilterStrings[] = {"", // Undefined
										"#Store_Items_Scout",
										"#Store_Items_Sniper",
										"#Store_Items_Soldier",
										"#Store_Items_Demoman",
...
ConVar tf_explanations_store("tf_explanations_store", "0", FCVAR_ARCHIVE,
							 "Whether the user has seen explanations for this panel.");

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
CTFStorePageBase::CTFStorePageBase(Panel *parent, const CEconStoreCategoryManager::StoreCategory_t *pPageData,
								   const char *pPreviewItemResFile)
	: CStorePage(parent, pPageData, pPreviewItemResFile)
{
	m_flStartExplanationsAt = 0;

	// TF has an option for each class, all class items, all items, and an unowned item option. Let's make sure they all
	// fit.
...
void CTFStorePageBase::OnItemDetails(vgui::Panel *panel)
{
	...
	if(pControlsPanel)
	{
		...
		if(pEntry)
		{
			...
			PostMessage(EconUI()->GetStorePanel(),
						new KeyValues("ArmoryOpened", "itemdef", pEntry->GetItemDefinitionIndex()));
		}
	}
}
...
void CTFStorePageBase::UpdateFilterComboBox(void)
{
	...
	if(nCount)
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wzLocalized, g_pVGuiLocalize->Find("#Store_ClassFilter_AllClasses"), 1,
											  wszCount);
	...
	for(int iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_LAST_NORMAL_CLASS; iClass++)
	{
		...
		_snwprintf(wszCount, ARRAYSIZE(wszCount), L"%d", nCount);
		g_pVGuiLocalize->ConstructString_safe(wzLocalized, g_pVGuiLocalize->Find(g_szClassFilterStrings[iClass]), 1,
											  wszCount);
		m_pFilterComboBox->AddItem(wzLocalized, pKeyValues);
	}
