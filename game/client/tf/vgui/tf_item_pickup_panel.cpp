...
ConVar tf_explanations_discardpanel("tf_explanations_discardpanel", "0", FCVAR_ARCHIVE,
...
void CTFItemPickupPanel::OnCommand(const char *command)
{
	if(!Q_stricmp(command, "changeloadout"))
	{
		...
		if(m_iSelectedItem >= 0 && m_iSelectedItem < m_aItems.Count())
		{
			...
			{
				// Open the loadout panel with the first class that can use this item (or the base loadout screen if
				// it's an all-class item)
		...
		if(m_aItems[m_iSelectedItem].pItem.IsValid() && !bDiscarded)
		{
			...
			else
			{
				...
				if(iClass != TF_CLASS_UNDEFINED)
				{
					...
					g_pVGuiLocalize->ConstructString_safe(wzLocalized, g_pVGuiLocalize->Find("#OpenSpecificLoadout"), 1,
...
ConCommand test_itempickuppanel("test_itempickuppanel", Test_ItemPickupPanel,
...
ConCommand test_itemdiscardpanel("test_itemdiscardpanel", Test_ItemDiscardPanel,
								 "Debugging tool to test the item discard panel. Usage: test_itemdiscardpanel <weapon "
