...
CON_COMMAND(showmapinfo, "Show map info panel")
{
	...
	// don't let the player open the team menu themselves until they're a spectator or they're on a regular team and
	// have picked a class
	if(pPlayer)
	{
	...
	if(pPlayer && pPlayer->CanShowClassMenu())
	{
		if(TFGameRules() && TFGameRules()->IsInArenaMode() && pPlayer->IsAlive() == true)
		{
			if(pPlayer->GetTeamNumber() > LAST_SHARED_TEAM &&
			   (tf_arena_force_class.GetBool() == true || TFGameRules()->InStalemate() == true))
			{
				...
				if(pHUDChat)
				{
					...
					g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find("#TF_Arena_NoClassChange"), szLocalized,
														  sizeof(szLocalized));
		...
		if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
		{
			if(!TFGameRules()->InSetup())
			{
				...
				{
					char szLocalized[100];
					g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find("#TF_MVM_NoClassChangeAfterSetup"),
	...
	if(pPlayer)
	{
		...
		if(pPlayer->GetTeamNumber() == TEAM_UNASSIGNED)
		{
			engine->ClientCmd("show_motd");
				...
				(pPlayer->m_Shared.GetDesiredPlayerClassIndex() == TF_CLASS_UNDEFINED))
		{
