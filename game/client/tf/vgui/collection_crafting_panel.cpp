...
void CCollectionCraftingPanel::SetItemPanelCount()
{
	...
	EditablePanel *pPaperContainer =
		dynamic_cast<vgui::EditablePanel *>(m_pTradeUpContainer->FindChildByName("PaperContainer"));
	if(pPaperContainer)
	{
...
void CCollectionCraftingPanel::ApplySettings(KeyValues *inResourceData)
{
	...
	if(pLocalizedPanelNames)
	{
		...
		{
			m_vecLocalizedPanels.AddToTail(
				{pValue->GetString("panelname"), pValue->GetBool("show_for_english", false)});
		}
	}
...
void CCollectionCraftingPanel::UpdateOKButton()
{
	...
	if(bOKEnabled)
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(m_pOKButton->GetParent(),
...
// Purpose:
//-----------------------------------------------------------------------------
void CCollectionCraftingPanel::SOCreated(const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject,
										 GCSDK::ESOCacheEvent eEvent)
{
	...
	if(m_bWaitingForGCResponse)
	{
		...
		if(IsUnacknowledged(pItem->GetInventoryToken()) && (pItem->GetOrigin() == m_eEconItemOrigin))
		{
			...
			CEconItemView *pNewEconItemView =
				InventoryManager()->GetLocalInventory()->GetInventoryItemByItemID(pItem->GetItemID());
			if(pNewEconItemView)
			{
...
void CCollectionCraftingPanel::SetItem(const CEconItemView *pItem, int nIndex)
{
	if(nIndex != m_vecItemPanels.InvalidIndex())
	{
		...
		if(pItem && m_vecBoxTopNames.Count())
		{
			...
			m_vecImagePanels[nIndex]->SetImage(
				m_vecBoxTopNames[randomStream.RandomInt(0, m_vecBoxTopNames.Count() - 1)]);
		}
	}
		...
		else if(m_timerResponse.GetElapsedTime() > flSoonestAirDropTime || m_bShowImmediately)
		{
			...
			if(m_nFoundItemID.Count() > 0)
			{
				...
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
					this, "CollectionCrafting_HideWaiting");

				// Setup the item in the panel
				CEconItemView *pNewEconItemView =
					InventoryManager()->GetLocalInventory()->GetInventoryItemByItemID(m_nFoundItemID[0]);
				if(pNewEconItemView)
				{
			...
			else
			{
				...
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
					this, "CollectionCrafting_ShowWaiting");
			}
		}
	}
