...
ConVar tf_explanations_charinfopanel("tf_explanations_charinfopanel", "0", FCVAR_ARCHIVE,
...
int g_nLoadoutClassOrder[] = {TF_CLASS_SCOUT,	TF_CLASS_SOLDIER,	   TF_CLASS_PYRO,
							  TF_CLASS_DEMOMAN, TF_CLASS_HEAVYWEAPONS, TF_CLASS_ENGINEER,
							  TF_CLASS_MEDIC,	TF_CLASS_SNIPER,	   TF_CLASS_SPY};
...
void CCharInfoLoadoutSubPanel::UpdateLabelFromClass(int nClass)
{
	...
	Assert(iPos != 0);
	int iXLeft = (GetWide() - ((m_iClassWideMin * NUM_CLASSES_IN_LOADOUT_PANEL) +
							   (m_iClassXDelta * (NUM_CLASSES_IN_LOADOUT_PANEL - 1)))) *
				 0.5;
	...
	switch(nButton)
	{
		...
		case CHSB_BACKPACK:
		{
			...
			else
			{
				...
				_snwprintf(wzCount, ARRAYSIZE(wzCount), L"%d", iNumItems);
				wchar_t wTemp[32];
				g_pVGuiLocalize->ConstructString_safe(wTemp, g_pVGuiLocalize->Find("Loadout_OpenBackpackDesc"), 1,
													  wzCount);
				m_pItemsLabel->SetText(wTemp);
			}
		}
...
void CCharInfoLoadoutSubPanel::OnKeyCodePressed(vgui::KeyCode code)
{
	   ...
	   nButtonCode == STEAMCONTROLLER_DPAD_LEFT || code == KEY_LEFT)
	{
		...
		return;
			...
			nButtonCode == STEAMCONTROLLER_DPAD_RIGHT || code == KEY_RIGHT)
	{
		if(m_iLabelSetToClass != -1)
		{
			MoveCharacterSelection(1);
