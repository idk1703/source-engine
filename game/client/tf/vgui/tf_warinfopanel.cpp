...
void CWarStandingPanel::OnThink()
{
	...
	if(m_bNeedsLerp)
	{
		...
		for(int i = 0; i < 2; ++i)
		{
			...
			float flCurrentScore =
				RemapValClamped(flPercent, 0.f, 1.f, (float)m_Scores[i].m_nLastScore, (float)m_Scores[i].m_nNewScore);
			...
			m_Scores[i].m_pContainerPanel->SetDialogVariable(CFmtStr("team%dscore", i),
															 CFmtStr("%.0f%%", flProgressPercent * 100.f));

			int nScoreXpos =
				RemapValClamped(flProgressPercent, 0.f, 1.f, m_Scores[i].m_pTeamProgressBar->GetXPos(),
...
void CWarStandingPanel::PerformLayout()
{
	...
	{
		uint64 nScore = nScore =
			GetWarData().GetGlobalSideScore(pWarDef->GetDefIndex(), pWarDef->GetSide(i)->m_nSideIndex);
		...
		if(!m_bNeedsLerp)
		{
			m_Scores[i].m_pContainerPanel->SetDialogVariable(
				CFmtStr("team%dscore", i), CFmtStr("%.0f%%", m_Scores[i].m_pTeamProgressBar->GetProgress() * 100.f));
		}
...
void CWarLandingPanel::SetVisible(bool bVisible)
{
	...
	if(pSceneContainer)
	{
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(pSceneContainer, m_strSceneAnimName,
																				false);
	}
}
...
void CWarLandingPanel::UpdateUIState()
{
	...
	if(pMainContainer)
	{
		...
		pMainContainer->SetControlVisible("AffiliatedContainer",
										  bWarActive && !bNeedsToJoin && m_eJoiningState == NO_ACTION);
		pMainContainer->SetControlVisible("UnaffiliatedContainer",
										  bWarActive && bNeedsToJoin && m_eJoiningState == NO_ACTION);
	...
	const wchar_t *wpszEndDateFormat = g_pVGuiLocalize->Find(
		IsWarActive(PYRO_VS_HEAVY_WAR_DEF_INDEX) ? "#TF_War_EndFutureDate" : "#TF_War_EndPastDate");
	...
	switch(m_eJoiningState)
	{
		...
		case CONFIRM_SIDE_SELECTION:
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszOutString, wpszFormat, 1,
												  g_pVGuiLocalize->Find(pChosenSide->m_pszLocalizedName));
	...
	pJoiningPopup->SetControlVisible("JoiningContainer", m_eJoiningState == ATTEMPTING_TO_JOIN_AND_WAITING_FOR_RESPONSE,
									 true);
	pJoiningPopup->SetControlVisible("TeamJoinedContainer",
									 m_eJoiningState == SUCCESS_RESPONSE_RECIEVED_WAITING_FOR_USER_CONFIRMATION, true);
	pJoiningPopup->SetControlVisible("FailedToJoinContainer",
									 m_eJoiningState == FAILED_RESPONSE_RECIEVED_WAITING_FOR_USER_CONFIRMATION, true);
}
