...
ConVar tf_matchmaking_join_in_progress("tf_matchmaking_join_in_progress", "0", FCVAR_DONTRECORD | FCVAR_ARCHIVE,
									   "Saved preference for if the player wants to join games in progress.");
...
CBaseLobbyPanel::CBaseLobbyPanel(vgui::Panel *pParent, CBaseLobbyContainerFrame *pContainer)
	: vgui::PropertySheet(pParent, "LobbyPanel"),
	  m_sPersonaStateChangedCallback(this, &CBaseLobbyPanel::OnPersonaStateChanged),
...
void CBaseLobbyPanel::OnClickedOnPlayer()
{
	...
	menu->AddMenuItem(szLocalized,
					  new KeyValues("TradeWithUser", "steamid", CFmtStr("%llu", steamID.ConvertToUint64()).Access()),
					  this);
...
void CBaseLobbyPanel::SetMatchmakingModeBackground()
{
	...
	if(pMatchDesc && pMatchDesc->m_pProgressionDesc)
	{
		...
		const LevelInfo_t &level =
			pMatchDesc->m_pProgressionDesc->YieldingGetLevelForSteamID(steamapicontext->SteamUser()->GetSteamID());
...
void CBaseLobbyPanel::FireGameEvent(IGameEvent *event)
{
				...
				switch(pParty->GetState())
				{
					case CSOTFParty_State_UI:
						...
						g_pVGuiLocalize->ConstructString_safe(
							wszLocalized, g_pVGuiLocalize->Find("#TF_Matchmaking_RemovedFromQueue"), 0);
					...
					case CSOTFParty_State_FINDING_MATCH:
						...
						g_pVGuiLocalize->ConstructString_safe(wszLocalized,
															  g_pVGuiLocalize->Find("#TF_Matchmaking_AddedToQueue"), 0);
						m_pChatLog->InsertString(wszLocalized);
						break;
	...
	if(!Q_stricmp(pszEventName, "mm_lobby_member_join"))
	{
		...
		if(bSolo)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszLocalized,
												  g_pVGuiLocalize->Find("#TF_Matchmaking_StartSearchChat"), 0);
			m_pChatLog->InsertString(wszLocalized);

			g_pVGuiLocalize->ConstructString_safe(wszLocalized,
												  g_pVGuiLocalize->Find("#TF_Matchmaking_InviteFriendsChat"), 0);
			m_pChatLog->InsertString(wszLocalized);
		}
		else
		{
			...
			g_pVGuiLocalize->ConstructString_safe(
				wszLocalized, g_pVGuiLocalize->Find("#TF_Matchmaking_PlayerJoinedPartyChat"), 1, wCharPlayerName);
			m_pChatLog->InsertString(wszLocalized);
		}
	...
	if(!Q_stricmp(pszEventName, "mm_lobby_member_leave"))
	{
		...
		wchar_t wszLocalized[512];
		g_pVGuiLocalize->ConstructString_safe(
			wszLocalized, g_pVGuiLocalize->Find("#TF_Matchmaking_PlayerLeftPartyChat"), 1, wCharPlayerName);
		m_pChatLog->InsertString(wszLocalized);
...
#define DO_FIELD(protobufname, labelname, bNearby)                                              \
	...
	{                                                                                           \
		...
		if(bNearby)                                                                             \
			vecNearbyFields.AddToTail(p##labelname);                                            \
	}
		...
		if(bShowTimer)
		{
			...
			{
				m_pPartyHasLowPriority->SetDialogVariable(
					"penaltytimer", CFmtStr("Expires: %s", ((rtimeExpire > 0) ? timeExpire.Render(time_buf) : "")));
			}
		...
		wchar_t wszLocalized[512];
		g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#TF_Matchmaking_Searching"), 1,
											  pwszEllipses);
...
void CBaseLobbyPanel::WriteGameSettingsControls()
{
	...
	m_pJoinLateCheckButton->ToggleButton::SetSelected(
		GTFGCClientSystem()->GetSearchJoinLate()); // !KLUDGE! call base to avoid firing the signal

	bool bShowLateJoin = ShouldShowLateJoin();
	m_pJoinLateCheckButton->SetVisible(bShowLateJoin && bLeader);
	m_pJoinLateValueLabel->SetText(GTFGCClientSystem()->GetSearchJoinLate() ? "#TF_Matchmaking_SearchForAll"
...
void CBaseLobbyPanel::UpdatePlayerList()
{
	...
	if(pParty == NULL)
	{
		...
		int idxTour = GTFGCClientSystem()->GetSearchMannUpTourIndex();
		if(idxTour < 0 ||
		   !GTFGCClientSystem()->BGetLocalPlayerBadgeInfoForTour(idxTour, &p.m_nBadgeLevel, &p.m_nCompletedChallenges))
		{
			p.m_nBadgeLevel = 0;
	...
	for(int i = 0; i < m_vecPlayers.Count(); ++i)
	{
		...
		pKeyValues->SetInt("is_banned",
						   (m_vecPlayers[i].m_bIsBanned || m_vecPlayers[i].m_bIsLowPriority) ? m_iImageIsBanned : 0);
		...
		m_pChatPlayerList->SetItemFgColor(itemID, (m_vecPlayers[i].m_bIsBanned || m_vecPlayers[i].m_bIsLowPriority)
													  ? s_colorBannedPlayerListItem
													  : s_colorPlayerListItem);
	...
	if((GetMatchGroup() == k_nMatchGroup_Ladder_6v6) || (GetMatchGroup() == k_nMatchGroup_Casual_12v12))
	{
		...
		for(int nRow = 0; nRow < m_pChatPlayerList->GetItemCount(); nRow++)
		{
			...
			if(iPanelCount >= m_vecChatBadges.Count())
			{
				...
				m_vecChatBadges[iPanelCount].m_pBadgeModel =
					vgui::SETUP_PANEL(new CTFBadgePanel(m_pChatPlayerList, "Model"));
			   ...
			   m_vecChatBadges[iPanelCount].m_nShownLevel != unLevel)
			{
				...
				g_pVGuiLocalize->ConstructString_safe(wszOutString, wpszFormat, 2, wszCount,
													  g_pVGuiLocalize->Find(level.m_pszLevelTitle));
...
static ConCommand connect_lobby_command("connect_lobby", &CL_ConnectLobby,
										"<64-bit lobby ID> Accept friend invite, connecting to specified Steam lobby "
										"and joining the corresponding search party");
