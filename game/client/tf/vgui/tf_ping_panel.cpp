...
ConVar tf_custom_ping_enabled("tf_custom_ping_enabled", "0", FCVAR_ARCHIVE, "", false, 0.f, false, 0.f,
							  OnConVarChangeCustomPingTolerance);
ConVar tf_custom_ping("tf_custom_ping", "100", FCVAR_ARCHIVE, "", true, (float)CUSTOM_PING_TOLERANCE_MIN, true,
					  (float)CUSTOM_PING_TOLERANCE_MAX, OnConVarChangeCustomPingTolerance);
...
CTFPingPanel::CTFPingPanel(Panel *pPanel, const char *pszName, EMatchGroup eMatchGroup)
	: EditablePanel(pPanel, pszName), m_eMatchGroup(eMatchGroup)
{
...
void CTFPingPanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if(pDataCenterList && GTFGCClientSystem()->BHavePingData())
	{
		...
		{
			bTesting = true;
			const char *pszDataCenterNames[] = {"eat", "lax", "iad", "atl", "gru", "scl", "lim",
												"lux", "vie", "sto", "mad", "sgp", "hkg", "tyo",
												"syd", "dxb", "bom", "maa", "ord", "waw", "jhb"};
		...
		for(int iPing = 0; iPing < pingData.pingdata_size(); ++iPing)
		{
			...
			if(dictIndex != dictDataCenterPopulations.InvalidIndex() || bTesting)
			{
				...
				EditablePanel *pDataCenterPopulationPanel =
					new EditablePanel(pDataCenterList, "DataCenterPopulationPanel");
				...
				wchar_t *pwszLocalizedDataCenterName =
					g_pVGuiLocalize->Find(CFmtStr("#TF_DataCenter_%s", pszPingFromDataCenterName));
				...
				else
				{
					...
					Assert(false);
					g_pVGuiLocalize->ConvertANSIToUnicode(pszPingFromDataCenterName, wszDataCenterName,
														  sizeof(wszDataCenterName));
				}
...
void CTFPingPanel::UpdateCurrentPing()
{
	...
	FOR_EACH_VEC(m_vecDataCenterPingPanels, i)
	{
		...
		if(bHighLight)
		{
			g_pClientMode->GetViewportAnimationController()->StopAnimationSequence(info.m_pPanel,
																				   "HealthProgressBar_NotSelected");
			g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(info.m_pPanel,
