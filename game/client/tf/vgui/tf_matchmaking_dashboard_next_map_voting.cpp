...
public:
	CNextMapVotingDashboardState(const char *pszName, const char *pszResFile)
		: CTFMatchmakingPopup(pszName, pszResFile), m_pTimerProgressBar(NULL)
	{
	...
	virtual void ApplySchemeSettings(IScheme *pScheme)
	{
		...
		if(m_pTimerProgressBar)
		{
			...
			m_pTimerProgressBar->SetFgImage(GetLocalPlayerTeam() == TF_TEAM_RED ? "progress_bar_red"
																				: "progress_bar_blu");
		}
	...
	virtual void OnUpdate() OVERRIDE
	{
		...
		if(TFGameRules())
		{
			...
			flVoteEndTime = (tf_mm_next_map_vote_time.GetInt() -
							 (gpGlobals->curtime - TFGameRules()->GetLastRoundStateChangeTime())) /
							tf_mm_next_map_vote_time.GetFloat();
		}
	...
	virtual bool ShouldBeActve() const OVERRIDE
	{
		...
		if(BInEndOfMatch() && TFGameRules() &&
		   TFGameRules()->GetCurrentNextMapVotingState() ==
			   CTFGameRules::NEXT_MAP_VOTE_STATE_WAITING_FOR_USERS_TO_VOTE &&
				...
				if(pButton)
				{
					...
					if(GetPlayerVoteState() == nIndex)
					{
						...
						pButton->SetArmedColor(
							pButton->GetButtonArmedFgColor(),
							scheme()->GetIScheme(GetScheme())->GetColor("CreditsGreen", Color(94, 150, 49, 255)));
					}
				}
			}
		}
			...
			if(pMapChoicePanel)
			{
				...
				if(g_pClientMode && g_pClientMode->GetViewport())
				{
					g_pClientMode->GetViewportAnimationController()->StopAnimationSequence(
						pMapChoicePanel, i == eWinningVote ? "LosingNextMapVote" : "WinningNextMapVote");
					g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
						pMapChoicePanel, i == eWinningVote ? "WinningNextMapVote" : "LosingNextMapVote");
				}
			}
		}
	}
...
REG_MM_POPUP_FACTORY(CNextMapVotingDashboardState, "NextMapVoting",
