...
ConVar tf_quest_turn_in_confirm_opt_out(
	"tf_quest_turn_in_confirm_opt_out", "0", FCVAR_ARCHIVE,
	"If nonzero, don't confirm submitting a contract that does not have all of the bonus points");
...
static int GetLoanerListFromQuest(const CEconItemView *pQuest, CUtlVector<item_definition_index_t> &vecLoanerItems)
{
	...
	const CUtlVector<CTFRequiredQuestItemsSet> &vecQuestRequiredItems =
		pQuest->GetItemDefinition()->GetQuestDef()->GetRequiredItemSets();
	...
	//	{
	//		const CUtlVector< CTFRequiredQuestItemsSet >& vecObjectiveRequiredItems = vecChosenObjectives[ i
	//]->GetConditions()->GetRequiredItemSets(); 		FOR_EACH_VEC( vecObjectiveRequiredItems, iRequired )
	//		{
	//			// don't add dups
	//			if ( vecLoanerItems.Find( vecObjectiveRequiredItems[ iRequired ].GetLoanerItemDef() ) ==
	//vecLoanerItems.InvalidIndex() )
	//			{
...
//-----------------------------------------------------------------------------
static int GetLoanersFromLocalInventory(const itemid_t &questID,
										const CUtlVector<item_definition_index_t> &vecLoanerItems,
...
CInputProxyPanel::CInputProxyPanel(Panel *parent, const char *pszPanelName) : BaseClass(parent, pszPanelName) {}
...
void CInputProxyPanel::OnCursorMoved(int x, int y)
{
	...
	{
		PostMessage(m_vecRedirectPanels[INPUT_MOUSE_MOVE][i].m_pPanel,
					new KeyValues(m_vecRedirectPanels[INPUT_MOUSE_MOVE][i].m_pszCommand, "x", x, "y", y));
	}
}
...
void CInputProxyPanel::OnCursorEntered()
{
	...
	{
		PostMessage(m_vecRedirectPanels[INPUT_MOUSE_ENTER][i].m_pPanel,
					new KeyValues(m_vecRedirectPanels[INPUT_MOUSE_ENTER][i].m_pszCommand));
	}
}
...
void CInputProxyPanel::OnCursorExited()
{
	...
	{
		PostMessage(m_vecRedirectPanels[INPUT_MOUSE_EXIT][i].m_pPanel,
					new KeyValues(m_vecRedirectPanels[INPUT_MOUSE_EXIT][i].m_pszCommand));
	}
}
...
void CInputProxyPanel::OnMousePressed(MouseCode code)
{
	...
	{
		PostMessage(m_vecRedirectPanels[INPUT_MOUSE_PRESS][i].m_pPanel,
					new KeyValues(m_vecRedirectPanels[INPUT_MOUSE_PRESS][i].m_pszCommand, "code", code));
	}
}
...
void CInputProxyPanel::OnMouseDoublePressed(MouseCode code)
{
	...
	{
		PostMessage(m_vecRedirectPanels[INPUT_MOUSE_DOUBLE_PRESS][i].m_pPanel,
					new KeyValues(m_vecRedirectPanels[INPUT_MOUSE_DOUBLE_PRESS][i].m_pszCommand, "code", code));
	}
}
...
void CInputProxyPanel::OnMouseReleased(MouseCode code)
{
	...
	{
		PostMessage(m_vecRedirectPanels[INPUT_MOUSE_RELEASED][i].m_pPanel,
					new KeyValues(m_vecRedirectPanels[INPUT_MOUSE_RELEASED][i].m_pszCommand, "code", code));
	}
}
...
void CInputProxyPanel::OnMouseWheeled(int delta)
{
	...
	{
		PostMessage(m_vecRedirectPanels[INPUT_MOUSE_WHEEL][i].m_pPanel,
					new KeyValues(m_vecRedirectPanels[INPUT_MOUSE_WHEEL][i].m_pszCommand, "delta", delta));
	}
}
...
CQuestStatusPanel::CQuestStatusPanel(Panel *parent, const char *pszPanelName)
	: EditablePanel(parent, pszPanelName), m_pMovingContainer(NULL), m_bShouldBeVisible(false)
{
...
void CQuestStatusPanel::OnThink()
{
	...
	{
		flProgress = Bias(RemapValClamped(m_transitionTimer.GetElapsedTime(), 0.f,
										  m_transitionTimer.GetCountdownDuration(), 0.f, 1.f),
						  0.7f);
	}
...
// Purpose:
//-----------------------------------------------------------------------------
CQuestItemPanel::CQuestItemPanel(Panel *parent, const char *pszPanelName, CEconItemView *pQuestItem,
								 CScrollableQuestList *pQuestList)
...
void CQuestItemPanel::LoadResFileForCurrentItem()
{
	...
	SetMouseInputEnabled(true); // Slam this to true.  When panels get created, they'll inherit their parents' mouse
								// enabled state and if we've been fiddling with it, we might accidently create all
								// child panels with mouse input disabled. Setting this to true just before the controls
								// are made gives them a chance to be mouse enabled if they want.
	...
	if(m_pQuestPaperContainer)
	{
		...
		{
			m_pRequestLoanerItemsButton =
				m_pLoanerContainerPanel->FindControl<CExButton>("RequestLoanerItemsButton", true);
			...
			{
				m_pLoanerItemModelPanel[i] = m_pLoanerContainerPanel->FindControl<CItemModelPanel>(
					CFmtStr("Loaner%dItemModelPanel", i + 1), true);
			}
		}
		...
		m_pFlavorScrollingContainer =
			m_pQuestPaperContainer->FindControl<CExScrollingEditablePanel>("ScrollableBottomContainer", true);
		Assert(m_pFlavorScrollingContainer);

		if(m_pFlavorScrollingContainer)
		{
			m_pObjectiveExplanationLabel =
				m_pFlavorScrollingContainer->FindControl<Label>("QuestObjectiveExplanation", true);
		}

		CInputProxyPanel *pInputProxy =
			m_pQuestPaperContainer->FindControl<CInputProxyPanel>("PaperInputProxyPanel", true);
		if(pInputProxy)
		{
			// Make the scroller scroll
			pInputProxy->AddPanelForCommand(CInputProxyPanel::INPUT_MOUSE_WHEEL, m_pFlavorScrollingContainer,
											"MouseWheeled");
	...
	{
		CInputProxyPanel *pInputProxy =
			m_pFrontFolderContainer->FindControl<CInputProxyPanel>("FrontInputProxyPanel", true);
		if(pInputProxy)
		{
	...
	m_bHasAllControls = m_pQuestPaperContainer && m_pFrontFolderContainer && m_pFrontFolderImage &&
						m_pBackFolderContainer && m_pBackFolderImage && m_pEncodedStatus && m_pInactiveStatus &&
						m_pReadyToTurnInStatus && m_pFlavorText && m_pObjectiveExplanationLabel && m_pExpirationLabel &&
						m_pTurnInContainer && m_pTurnInDimmer && m_pTurnInButton && m_pIdentifyButton &&
						m_pTurnInSpinnerContainer && m_pTitleButton && m_pIdentifyDimmer && m_pIdentifyContainer &&
						m_pPhotoStatic && m_pAcceptedImage && m_pTurningInLabel && m_pFlavorScrollingContainer &&
...
void CQuestItemPanel::ApplySettings(KeyValues *inResourceData)
{
	...
	if(m_hQuestItem)
	{
		...
		else
		{
			...
			if(pTheme)
			{
				Warning("%s %s is missing 'folders' data\n", pItemDef->GetQuestDef()->GetCorrespondingOperationName(),
						pTheme->GetQuestItemResFile());
			}
		}
	}
...
void CQuestItemPanel::PerformLayout(void)
{
	...
	m_pTurnInContainer->SetVisible(m_eState == STATE_COMPLETED || m_eState == STATE_TURNING_IN__GC_RESPONDED ||
								   m_eState == STATE_TURNING_IN__WAITING_FOR_GC);
	m_pTurnInButton->SetVisible(m_eState == STATE_COMPLETED);
	m_pTurnInSpinnerContainer->SetVisible(m_eState == STATE_TURNING_IN__GC_RESPONDED ||
										  m_eState == STATE_TURNING_IN__WAITING_FOR_GC);
	...
	m_pReadyToTurnInStatus->SetShow(m_eState == STATE_COMPLETED || m_eState == STATE_TURNING_IN__GC_RESPONDED ||
									m_eState == STATE_TURNING_IN__WAITING_FOR_GC);
	...
	if(m_hQuestItem)
	{
		...
		// Check if the quest is going to expire soon (within a week).  If so, show a "This is going to be destroyed"
		// message.
		...
		m_pObjectiveExplanationLabel->InvalidateLayout(
			true); // So we get the right height when we do SizeToContents below
		...
		{
			RandomSeed(m_hQuestItem->GetSOCData() ? m_hQuestItem->GetSOCData()->GetOriginalID()
												  : m_hQuestItem->GetItemDefIndex());
...
void CQuestItemPanel::SetupObjectivesPanels(bool bRecreate)
{
	...
	if(!m_pItemTrackerPanel)
	{
		m_pItemTrackerPanel = new CItemTrackerPanel(m_pFlavorScrollingContainer, "ItemTrackerPanel",
													m_hQuestItem->GetSOCData(), m_strItemTrackerResFile);
...
void CQuestItemPanel::CaptureAndEncodeStrings()
{
...

	{
		...
		const char *pszLocToken =
			m_hQuestItem->GetItemDefinition()->GetQuestDef()->GetRolledDescriptionForItem(m_hQuestItem->GetSOCData());
	...
	const char *pszLocToken =
		m_hQuestItem->GetItemDefinition()->GetQuestDef()->GetRolledNameForItem(m_hQuestItem->GetSOCData());
...
void CQuestItemPanel::OnCommand(const char *command)
{
	...
	else if(FStrEq(command, "turnin"))
	{
		...
		{
			if(!tf_quest_turn_in_confirm_opt_out.GetBool() &&
			   (GetEarnedBonusPoints(m_hQuestItem) !=
				m_hQuestItem->GetItemDefinition()->GetQuestDef()->GetMaxBonusPoints()))
			{
				CTFGenericConfirmOptOutDialog *pPanel = ShowConfirmOptOutDialog(
					"#TF_Quest_TurnIn_Title", "#TF_Quest_TurnIn_Text", "#TF_Quest_TurnIn_Yes", "#TF_Quest_TurnIn_No",
					"#TF_Quest_TurnIn_Ask_Opt_Out", "tf_quest_turn_in_confirm_opt_out", ConfirmTurnInQuest);
				if(pPanel)
				{
	...
	else if(FStrEq("mm_casual_open", command))
	{
		...
		{
			if((m_strMatchmakingGroupName != 0) || (m_strMatchmakingCategoryName != 0) ||
			   (m_strMatchmakingMapName != 0))
			{
				...
				{
					int iGroupType =
						StringFieldToInt(m_strMatchmakingGroupName.Get(), s_pszMMTypes, ARRAYSIZE(s_pszMMTypes));
					if(iGroupType > -1)
					{
				...
				{
					int iCategoryType =
						StringFieldToInt(m_strMatchmakingCategoryName.Get(), s_pszGameModes, ARRAYSIZE(s_pszGameModes));
					if(iCategoryType > -1)
					{
	...
	switch(m_eState)
	{
		...
		case STATE_TURNING_IN__WAITING_FOR_GC:
		{
			...
			if(m_StateTimer.HasStarted() && m_StateTimer.IsElapsed())
			{
				...
				CTFGenericConfirmDialog *pDialog = new CTFGenericConfirmDialog(
					"#TF_Trading_Timeout_Title", "#TF_Trading_Timeout_Text", "#TF_OK", NULL, NULL, NULL);
		...
		case STATE_TURNING_IN__GC_RESPONDED:
		{
			...
			if(m_pTurningInLabel)
			{
				...
				m_pTurningInLabel->GetTextImage()->GetUnlocalizedText(szTurningInLocToken,
																	  ARRAYSIZE(szTurningInLocToken));
				V_snwprintf(wszTurningInText, ARRAYSIZE(wszTurningInText), L"%ls",
							g_pVGuiLocalize->Find(szTurningInLocToken));
...
void CQuestItemPanel::FireGameEvent(IGameEvent *event)
{
	...
	else if(FStrEq(event->GetName(), "player_spawn") || FStrEq(event->GetName(), "client_disconnect"))
	{
		InvalidateLayout();
	}
...
void CQuestItemPanel::UpdateInvalidReasons()
{
	...
	if(pLocalPlayer && m_hQuestItem)
	{
		...
		const CQuestItemTracker *pItemTracker =
			QuestObjectiveManager()->GetTypedTracker<CQuestItemTracker *>(m_hQuestItem->GetItemID());
		// Get invalid reasons
		if(pItemTracker)
		{
...
void CQuestItemPanel::OnDiscardQuest(void)
{
	...
	if(m_pQuestList->GetCompletingPanel() == NULL)
	{
		...
		CTFGenericConfirmDialog *pDialog =
			new CTFGenericConfirmDialog("#QuestConfirmDiscard_Title", "#QuestConfirmDiscard_Body", "#X_DiscardItem",
										"#Cancel", &ConfirmDiscardQuest, NULL);
		if(pDialog)
		{
			pDialog->SetContext(this);
			pDialog->Show();
...
void CQuestItemPanel::OnEquipLoaners(void)
{
	...
	if(m_pQuestList->GetCompletingPanel() == NULL)
	{
		// Bring up confirm dialog
		CTFGenericConfirmDialog *pDialog =
			new CTFGenericConfirmDialog("#QuestConfirmEquipLoaners_Title", "#QuestConfirmEquipLoaners_Body", "#Equip",
										"#Cancel", &ConfirmEquipLoaners, NULL);
		if(pDialog)
		{
			pDialog->SetContext(this);
			pDialog->Show();
...
void CQuestItemPanel::OnIdentify()
{
	if(IsUnacknowledged())
	{
		...
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(m_pQuestPaperContainer,
...
GC_REG_JOB(GCSDK::CGCClient, CGCLoanerRequestResponse, "CGCLoanerRequestResponse",
		   k_EMsgGCQuestObjective_RequestLoanerResponse, GCSDK::k_EServerTypeGCClient);