...
ConVar tf_explanations_craftingpanel("tf_explanations_craftingpanel", "0", FCVAR_ARCHIVE,
...
recipefilter_data_t g_RecipeFilters[NUM_RECIPE_CATEGORIES] = {
	{"#RecipeFilter_Crafting", "crafticon_crafting_items",
	 "crafticon_crafting_items_over"}, // RECIPE_CATEGORY_CRAFTINGITEMS,
	{"#RecipeFilter_CommonItems", "crafticon_common_items",
	 "crafticon_common_items_over"},												  // RECIPE_CATEGORY_COMMONITEMS,
	{"#RecipeFilter_RareItems", "crafticon_rare_items", "crafticon_rare_items_over"}, // RECIPE_CATEGORY_RAREITEMS,
	{"#RecipeFilter_Special", "crafticon_special_blueprints",
	 "crafticon_special_blueprints_over"} // RECIPE_CATEGORY_SPECIAL,
};
...
void SetItemPanelToRecipe(CItemModelPanel *pPanel, const CEconCraftingRecipeDefinition *pRecipeDef, bool bShowName)
{
	...
	else
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wcTmpDesc, g_pVGuiLocalize->Find(pRecipeDef->GetDescInputs()), 3, pInp_A,
											  pInp_B, pInp_C);
		iNegAttribsBegin = Q_wcslen(wcTmpDesc);
		...
		g_pVGuiLocalize->ConstructString_safe(wcTmp, g_pVGuiLocalize->Find(pRecipeDef->GetDescOutputs()), 3, pOut_A,
											  pOut_B, pOut_C);
...
// Purpose:
//-----------------------------------------------------------------------------
void PositionMouseOverPanelForRecipe(vgui::Panel *pScissorPanel, vgui::Panel *pRecipePanel,
									 vgui::ScrollableEditablePanel *pRecipeScroller,
									 CItemModelPanel *pMouseOverItemPanel)
{
	int x, y;
...
CCraftingPanel::CCraftingPanel(vgui::Panel *parent, const char *panelName) : CBaseLoadoutPanel(parent, panelName)
{
	...
	m_pRecipeListContainerScroller =
		new vgui::ScrollableEditablePanel(this, m_pRecipeListContainer, "recipecontainerscroller");
...
void CCraftingPanel::PositionItemPanel(CItemModelPanel *pPanel, int iIndex)
{
	...
	if(IsInputItemPanel(iIndex))
	{
		...
		iXPos = (iCenter + m_iItemCraftingOffcenterX) + (iButtonX * m_pItemModelPanels[iIndex]->GetWide()) +
	...
	else
	{
		...
		iXPos = (iCenter + m_iItemCraftingOffcenterX) + (iButtonX * m_pItemModelPanels[iIndex]->GetWide()) +
				(m_iItemBackpackXDelta * iButtonX);
		iYPos =
			m_iOutputItemYPos + (iButtonY * m_pItemModelPanels[iIndex]->GetTall()) + (m_iItemBackpackYDelta * iButtonY);
	}
...
void CCraftingPanel::UpdateRecipeItems(bool bClearInputItems)
{
	...
	*/
	...
	{
		const CTFCraftingRecipeDefinition *pRecipeDef =
...
void CCraftingPanel::UpdateCraftButton(void)
{
	...
	if(m_iCurrentlySelectedRecipe != RECIPE_CUSTOM)
	{
		pRecipeDef = (CTFCraftingRecipeDefinition *)TFInventoryManager()->GetLocalTFInventory()->GetRecipeDefByDefIndex(
			m_iCurrentlySelectedRecipe);
		if(!pRecipeDef)
			return;
	...
	if(m_iCurrentlySelectedRecipe == RECIPE_CUSTOM)
	{
		...
		for(int i = 0; i < CRAFTING_SLOTS_INPUTPANELS; i++)
		{
			CEconItemView *pItemData =
				TFInventoryManager()->GetLocalTFInventory()->GetInventoryItemByItemID(m_InputItems[i]);
			if(pItemData)
			{
...
const char *CCraftingPanel::GetItemTextForCriteria(const CItemSelectionCriteria *pCriteria)
{
	...
	if(pszVal && pszVal[0])
	{
		...
		int iSlot = StringFieldToInt(
			pszVal, ItemSystem()->GetItemSchema()->GetLoadoutStrings(EEquipType_t::EQUIP_TYPE_CLASS), true);
		if(iSlot != -1)
...
void CCraftingPanel::UpdateSelectedRecipe(bool bClearInputItems)
{
	for(int i = 0; i < m_pRecipeButtons.Count(); i++)
	{
		...
		if(bSelected)
		{
			...
			if(m_iCurrentlySelectedRecipe == RECIPE_CUSTOM)
			{
				m_pSelectedRecipeContainer->SetDialogVariable("recipeinputstring",
															  g_pVGuiLocalize->Find("#Craft_Recipe_CustomDesc"));
			}
			else
			{
				const CTFCraftingRecipeDefinition *pRecipeDef =
				...
				if(pRecipeDef)
				{
					...
					wchar_t *pInp_B = LocalizeRecipeStringPiece(pRecipeDef->GetDescI_B(), wcTmpB, sizeof(wcTmpB));
					wchar_t *pInp_C = LocalizeRecipeStringPiece(pRecipeDef->GetDescI_C(), wcTmpC, sizeof(wcTmpC));
					g_pVGuiLocalize->ConstructString_safe(wcTmpDesc, g_pVGuiLocalize->Find(pRecipeDef->GetDescInputs()),
	...
	if(pItemPanel && IsVisible() && !pItemPanel->IsGreyedOut())
	{
		...
		if(IsInputItemPanel(iPos))
		{
			...
			else
			{
				...
				m_pSelectionPanel->UpdateOnShow(m_ItemPanelCriteria[iPos], false, m_InputItems,
												ARRAYSIZE(m_InputItems));
			}
...
bool CCraftingPanel::CheckForUntradableItems(void)
{
	...
	if(bHasUntradable)
	{
		CTFGenericConfirmDialog *pDialog = ShowConfirmDialog("#Craft_Untradable_Title", "#Craft_Untradable_Text",
															 "#GameUI_OK", "#Cancel", &ConfirmCraft);
...
void CCraftingPanel::OnTick(void)
{
	...
	if(IsVisible())
	{
		...
		if(m_bWaitingForCraftItems)
		{
			...
			{
				CEconItemView *pNewItem =
					InventoryManager()->GetLocalInventory()->GetInventoryItemByItemID(m_vecNewlyCraftedItems[i]);
				if(pNewItem == NULL)
					return;
...
CCraftingStatusDialog *OpenCraftingStatusDialog(vgui::Panel *pParent, const char *pszText, bool bAnimateEllipses,
												bool bAllowClose, bool bShowOnExit)
{
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		if(EconUI()->GetCraftingPanel())
		{
			EconUI()->GetCraftingPanel()->OnCraftResponse((EGCMsgResponse)msg.Body().m_eResponse, &vecCraftedIndices,
														  msg.Body().m_nResponseIndex);
		}
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		if(pNotifyPanel)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(
				szNotification,
				g_pVGuiLocalize->Find(bDeleted ? "#TF_HUD_Event_GoldenWrench_D" : "#TF_HUD_Event_GoldenWrench_C"), 2,
				szPlayerName, szWrenchNumber);
...
GC_REG_JOB(GCSDK::CGCClient, CGCGoldenWrenchBroadcast, "CGCGoldenWrenchBroadcast", k_EMsgGCGoldenWrenchBroadcast,
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
...

		{
			...
			g_pVGuiLocalize->ConvertANSIToUnicode(msg.Body().has_user_name() ? msg.Body().user_name().c_str() : NULL,
...
GC_REG_JOB(GCSDK::CGCClient, CGSaxxyBroadcast, "CGSaxxyBroadcast", k_EMsgGCSaxxyBroadcast,
		   GCSDK::k_EServerTypeGCClient);
...
GC_REG_JOB(GCSDK::CGCClient, CClientItemBroadcastNotificationJob, "CClientItemBroadcastNotificationJob",
		   k_EMsgGCTFSpecificItemBroadcast, GCSDK::k_EServerTypeGCClient);
...
GC_REG_JOB(GCSDK::CGCClient, CGSaxxyAwardedBroadcast, "CGSaxxyAwardedBroadcast", k_EMsgGCSaxxy_Awarded,
		   GCSDK::k_EServerTypeGCClient);
...
GC_REG_JOB(GCSDK::CGCClient, CGCSystemMessageBroadcast, "CGCSystemMessageBroadcast", k_EMsgGCSystemMessage,
		   GCSDK::k_EServerTypeGCClient);