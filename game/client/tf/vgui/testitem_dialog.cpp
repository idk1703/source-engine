...
// Purpose:
//-----------------------------------------------------------------------------
CTestItemDialog::CTestItemDialog(vgui::Panel *parent, testitem_itemtypes_t iItemType, int iClassUsage,
								 KeyValues *pExistingKVs)
	: vgui::EditablePanel(parent, "TestItemDialog")
{
	...
	{
		m_pBodygroupCheckButtons[i] =
			new vgui::CheckButton(m_pBodygroupPanel, VarArgs("HideBodygroupCheckBox%d", i), "");
...
void CTestItemDialog::InitializeFromExistingKVs(KeyValues *pExistingKVs)
{
	...
	else
	{
		...
		if(pszModel && pszModel[0])
		{
			...
			{
				item_definition_index_t iItemDefToReplace =
					pExistingKVs->GetInt("item_replace", INVALID_ITEM_DEF_INDEX);
				...
				if(pkvVisuals)
				{
					...
					while(pKVEntry)
					{
						if(!Q_stricmp(pKVEntry->GetName(), "player_bodygroups"))
						{
							...
							{
								int iBG = StringFieldToInt(pKVSubEntry->GetName(), g_pszTestItemHideBodygroup,
...
void CTestItemDialog::CloseAndUpdateItem(void)
{
	...
	item_definition_index_t iExistingItemDef =
		pExistingUserData ? pExistingUserData->GetInt("item", INVALID_ITEM_DEF_INDEX) : INVALID_ITEM_DEF_INDEX;
	if(iExistingItemDef != INVALID_ITEM_DEF_INDEX)
	{
	...
	else
	{
		...
		switch(m_iItemType)
		{
			...
			case TI_TYPE_HEADGEAR:
				iItemDef =
					ItemSystem()->GetItemSchema()->GetItemDefinitionByName("Football Helmet")->GetDefinitionIndex();
				break;
			case TI_TYPE_MISC1:
				iItemDef =
					ItemSystem()->GetItemSchema()->GetItemDefinitionByName("Employee Badge A")->GetDefinitionIndex();
				break;
			case TI_TYPE_MISC2:
				iItemDef =
					ItemSystem()->GetItemSchema()->GetItemDefinitionByName("High Five Taunt")->GetDefinitionIndex();
				break;
		}
	}
...
void CTestItemDialog::OpenSelectModelDialog(void)
{
	...
	switch(m_iItemType)
	{
		...
		case TI_TYPE_WEAPON:
			m_hImportModelDialog->SetStartDirectory(g_pFullFileSystem->RelativePathToFullPath(
				"models/weapons/c_models", "MOD", szModelsDir, sizeof(szModelsDir)));
			break;
		...
		case TI_TYPE_MISC2:
		{
			...
			{
				m_hImportModelDialog->SetStartDirectory(g_pFullFileSystem->RelativePathToFullPath(
					VarArgs("models/player/items/%s", pszSubDir), "MOD", szModelsDir, sizeof(szModelsDir)));
			}
			else
			{
				m_hImportModelDialog->SetStartDirectory(g_pFullFileSystem->RelativePathToFullPath(
					"models/player/items", "MOD", szModelsDir, sizeof(szModelsDir)));
			}
		}
		break;
	}
...
void CTestItemDialog::SetupItemComboBox(vgui::ComboBox *pComboBox)
{
	...
	int iReplacements =
		((CTestItemRoot *)GetParent())->FindReplaceableItemsForSelectedClass(&vecDefs, m_iItemType == TI_TYPE_WEAPON);
	if(iReplacements)
	{
...
void CTestItemDialog::SetupUnusualEffectComboBox(void)
{
	...
	const CEconItemSchema::ParticleDefinitionMap_t &mapParticleDefs =
		ItemSystem()->GetItemSchema()->GetAttributeControlledParticleSystems();
...
void CTestItemDialog::SetEntryStep(testitem_entrysteps_t iStep)
{
	...
	m_pBodygroupPanel->SetVisible(iStep >= TI_STEP_NONWPN_BODYGROUPS && m_iItemType != TI_TYPE_WEAPON &&
								  !bTestingExistingItem);
	m_pExistingItemToTestPanel->SetVisible(iStep == TI_STEP_MODELNAME || bTestingExistingItem);
	m_pItemReplacedPanel->SetVisible(iStep >= TI_STEP_WPN_ITEMREPLACED && m_iItemType == TI_TYPE_WEAPON &&
									 !bTestingExistingItem);
	if(m_pNoItemsToReplaceLabel)
	{
