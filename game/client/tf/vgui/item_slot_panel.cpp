...
// Purpose:
//-----------------------------------------------------------------------------
CItemSlotPanel::CItemSlotPanel(vgui::Panel *parent) : CBaseLoadoutPanel(parent, "item_slot_panel")
{
	m_pItem = NULL;
...
void CItemSlotPanel::PerformLayout(void)
{
	...
	for(int i = 0; i < m_pItemModelPanels.Count(); i++)
	{
		...
		int iXPos = (iCenter + m_iItemBackpackOffcenterX) + (iButtonX * m_pItemModelPanels[i]->GetWide()) +
	...
	if(data)
	{
		...
		if(ulIndex != INVALID_ITEM_ID)
		{
			...
			if(pItemData)
			{
				...
				// "applied_upgrade_card", m_pToolModelPanel->GetItem()->GetItemDefIndex() );
	...
	else if(V_stristr(command, "change"))
	{
		...
		if(pszNum && pszNum[0])
		{
			...
			if(iSlot >= 0 && iSlot < m_itemSlots.Count())
			{
				...
				m_pSelectionPanel->InvalidateLayout(
					false,
					true); // need to ApplySchemeSettings now so it doesn't override our SetDialogVariable below later
				m_pSelectionPanel->ShowPanel(0, true);
				m_pSelectionPanel->SetDialogVariable("loadoutclass",
													 g_pVGuiLocalize->Find("#EditSlots_SelectItemPanel"));
...
void CItemSlotPanel::UpdateModelPanels(void)
{
	...
	for(int i = 0; i < m_pItemModelPanels.Count(); i++)
	{
		CEconItemView *pItemData =
			TFInventoryManager()->GetLocalTFInventory()->GetInventoryItemByOriginalID(m_itemSlots[i].m_ulOriginalID);
