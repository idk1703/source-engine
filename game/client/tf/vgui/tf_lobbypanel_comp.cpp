...
ConVar tf_mm_ladder_ui_last_rating_change("tf_mm_ladder_ui_last_rating_change", "0", FCVAR_HIDDEN | FCVAR_ARCHIVE,
										  "Track last match skillrating change for UI.");
ConVar tf_mm_ladder_ui_last_rating_time("tf_mm_ladder_ui_last_rating_time", "-1", FCVAR_HIDDEN | FCVAR_ARCHIVE,
										"Track last match skillrating change time for UI.");
...
public:
	...
	CMatchHistoryEntryPanel(Panel *pParent, const char *pszPanelname) : BaseClass(pParent, pszPanelname) {}
	...
	void SetMatchData(const CSOTFMatchResultPlayerStats &stats)
	{
		...
		if(pStatsContainer)
		{
			pStatsContainer->SetDialogVariable("stat_kills",
											   LocalizeNumberWithToken("TF_Competitive_Kills", stats.kills()));
			pStatsContainer->SetDialogVariable("stat_deaths",
											   LocalizeNumberWithToken("TF_Competitive_Deaths", stats.deaths()));
			pStatsContainer->SetDialogVariable("stat_damage",
											   LocalizeNumberWithToken("TF_Competitive_Damage", stats.damage()));
			pStatsContainer->SetDialogVariable("stat_healing",
											   LocalizeNumberWithToken("TF_Competitive_Healing", stats.healing()));
			pStatsContainer->SetDialogVariable("stat_support",
											   LocalizeNumberWithToken("TF_Competitive_Support", stats.support()));
			pStatsContainer->SetDialogVariable("stat_score",
											   LocalizeNumberWithToken("TF_Competitive_Score", stats.score()));
...
private:
	...
	void SetupClassIcon(EditablePanel *pParent, const char *pszClassName, int nBit,
						const CSOTFMatchResultPlayerStats &stats)
	{
		ScalableImagePanel *pClassIcon =
			pParent->FindControl<ScalableImagePanel>(CFmtStr("%sIcon", pszClassName), true);
		if(pClassIcon)
		{
			pClassIcon->SetImage(stats.classes_played() & (1 << nBit)
									 ? CFmtStr("class_icons/filter_%s_on", pszClassName)
									 : CFmtStr("class_icons/filter_%s", pszClassName));
		}
	}
};
...
bool CLadderLobbyLeaderboard::UpdateLeaderboards()
{
	...
	if(m_pScoreListScroller->GetScrollbar()->GetButton(0) && m_pScoreListScroller->GetScrollbar()->GetButton(1))
	{
	...
	FOR_EACH_VEC(m_vecLeaderboardEntries, i)
	{
		...
		if(bIsEntryVisible)
		{
			...
			bool bIsLocalPlayer = steamapicontext && steamapicontext->SteamUser() &&
								  steamapicontext->SteamUser()->GetSteamID() == steamID;
			pContainer->SetDialogVariable("position",
										  m_bGlobal ? CFmtStr("%d.", pLeaderboardEntry->m_nGlobalRank) : "");
			...
			if(pRankImage)
			{
				...
				const LevelInfo_t &levelInfo =
					pMatchDesc->m_pProgressionDesc->GetLevelForExperience(pLeaderboardEntry->m_nScore);
				...
				g_pVGuiLocalize->ConstructString_safe(wszOutString, wpszFormat, 2, wszCount,
													  g_pVGuiLocalize->Find(levelInfo.m_pszLevelTitle));
			...
			CExImageButton *pStreamImage =
				dynamic_cast<CExImageButton *>(pContainer->FindChildByName("StreamImageButton"));
			if(pStreamImage)
			{
#ifdef TWITCH_LEADERBOARD
...
void CLobbyPanel_Comp::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	int nExtraWidth =
		(m_pChatPlayerList->GetWide() - (2 * nAvatarWidth) - m_iPlayerNameWidth - m_iBannedWidth - m_iHasPassWidth);

	m_pChatPlayerList->AddColumnToSection(0, "avatar", "#TF_Players", vgui::SectionedListPanel::COLUMN_IMAGE,
										  nAvatarWidth);
	...
	m_pChatPlayerList->AddColumnToSection(
		...
		m_iBannedWidth);
	m_pChatPlayerList->AddColumnToSection(
		0, "has_competitive_access", "",
		vgui::SectionedListPanel::COLUMN_IMAGE | vgui::SectionedListPanel::COLUMN_CENTER, m_iHasPassWidth);
	m_pChatPlayerList->AddColumnToSection(
		0, "rank", "", vgui::SectionedListPanel::COLUMN_IMAGE | vgui::SectionedListPanel::COLUMN_CENTER, nAvatarWidth);
...
void CLobbyPanel_Comp::UpdateMatchDataForLocalPlayer()
{
	...
	if(m_pMatchHistoryScroller)
	{
		...
		SortMethodData_t sortMethods[NUM_SORT_METHODS] = {
			{&SortResult, pPlaylistBGPanel->FindControl<CExButton>("ResultButton", true)},
		...
		if(m_bDescendingMatchHistorySort)
		{
			...
			{
				CMatchHistoryEntryPanel *pMatchEntryPanel =
					new CMatchHistoryEntryPanel(m_pMatchHistoryScroller, "MatchEntry");
