...
void cc_toggle_debug_inspect(IConVar *pConVar, const char *pOldString, float flOldValue)
{
	...
	if(var.IsValid())
	{
		CTFItemInspectionPanel *pInspectionPanel =
			dynamic_cast<CTFItemInspectionPanel *>(EconUI()->GetBackpackPanel()->FindChildByName("InspectionPanel"));
		if(pInspectionPanel)
		{
...
ConVar tf_use_debug_inspection_panel("tf_use_debug_inspection_panel", "0", FCVAR_ARCHIVE,
									 "Toggles developer controls for the item inspection panel",
									 cc_toggle_debug_inspect);
...
static void Helper_LaunchPreviewWithPreviewDataBlock(CEconItemPreviewDataBlock const &protoData)
{
	...
	itemView.Init(econItem.GetDefinitionIndex(), econItem.GetQuality(), econItem.GetItemLevel(),
				  econItem.GetAccountID());
...
GC_REG_JOB(GCSDK::CGCClient, CGCClient2GCEconPreviewDataBlockResponse, "CGCClient2GCEconPreviewDataBlockResponse",
		   k_EMsgGC_Client2GCEconPreviewDataBlockResponse, GCSDK::k_EServerTypeGCClient);

// ********************************************************************************************************************************
CON_COMMAND_F(tf_econ_item_preview, "Preview an economy item",
...
CTFItemInspectionPanel::CTFItemInspectionPanel(Panel *pPanel, const char *pszName)
	: BaseClass(pPanel, pszName), m_pModelInspectPanel(NULL), m_pItemViewData(NULL), m_pSOEconItemData(NULL)
{
...
void CTFItemInspectionPanel::PerformLayout()
{
	...
	if(pItem && pItem->IsValid())
	{
		...
		if(m_pTeamColorNavPanel)
		{
			...
			bool bShowTeamButton = FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pItem, pAttrTeamColoredPaintkit,
																				   &iHasTeamColoredPaintkit) &&
								   iHasTeamColoredPaintkit > 0;
