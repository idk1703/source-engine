...
SoftLine::SoftLine(vgui::Panel *parent, const char *panelName, Color col) : vgui::Panel(parent, panelName)
{
	m_Color = col;
	m_iCornerType = 0;
}
...
void SoftLine::DrawPolygonLine(float x, float y, float x2, float y2, float width)
{
	...
	short_edge.y = newy;
	short_edge *= width;
	vgui::Vertex_t points[4] = {vgui::Vertex_t(Vector2D(x, y) - short_edge * 0.5f, Vector2D(0, 0)),
