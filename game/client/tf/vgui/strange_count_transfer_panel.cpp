...
public:
		  ...
		  m_mapXifierClassCount(CaselessStringLessThan)
	{
		...
		for(int i = 0; i < nCount; ++i)
		{
			...
			const char *pItemXifier =
				InventoryManager()->GetLocalInventory()->GetItem(i)->GetItemDefinition()->GetXifierRemapClass();
...
void CStrangeCountTransferPanel::OnCommand(const char *command)
{
	if(FStrEq("apply", command))
	{
		...
		EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolItem, "applied_strangecounttransfer",
											m_pToolItem->GetItemDefIndex());
