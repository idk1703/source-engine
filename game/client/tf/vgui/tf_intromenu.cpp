	...
	else if(m_flActionThink > 0 && m_flActionThink < gpGlobals->curtime)
	{
		...
		else if(m_iAction == INTRO_CONTINUE)
		{
			...
			else
			{
				...
				if(pPlayer && (GetLocalPlayerTeam() != TEAM_SPECTATOR) &&
...
void CTFIntroMenu::ShowPanel(bool bShow)
{
	...
	if(TFGameRules() && TFGameRules()->IsInTraining())
	{
		...
		if(PendingInGameVideo() == false)
		{
			...
			if(g_pVideo != NULL &&
			   g_pVideo->LocatePlayableVideoFile(GetVideoFileName(), "GAME", &playbackSystem, resolvedFile,
	...
	if(m_pVideo)
	{
		...
		if(LoadCaptions())
		{
			if(m_pCaptionLabel && !m_pCaptionLabel->IsVisible())
			{
				...
				// It could have been faded out from a prior animation event form an animation effect in a previous
				// video instance.
