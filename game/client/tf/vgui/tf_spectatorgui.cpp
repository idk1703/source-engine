...
// Purpose:
//-----------------------------------------------------------------------------
ConVar cl_spec_carrieditems("cl_spec_carrieditems", "1", FCVAR_ARCHIVE,
...
ConVar cl_use_tournament_specgui("cl_use_tournament_specgui", "0", FCVAR_ARCHIVE,
								 "When in tournament mode, use the advanced tournament spectator UI.",
								 HUDTournamentSpecChangedCallBack);
...
void CTFSpectatorGUI::UpdateReinforcements(void)
{
	...
	else
	{
		...
		bool bQuickSpawn =
			TFGameRules() && TFGameRules()->IsMannVsMachineMode() && pPlayer->IsPlayerClass(TF_CLASS_SCOUT);
		...
		else
		{
			...
			if(TFGameRules()->IsMannVsMachineMode())
			{
				...
				if(bBuyBackVisible)
				{
					// When using the new system, we display "Hit '%use_action_slot_item%' to RESPAWN INSTANTLY! (%s1
					// remaining this wave)" When using the old system, we display "Hit '%use_action_slot_item%' to pay
					// %s1 credits and RESPAWN INSTANTLY!"
					int nCost = (bNewMethod) ? g_TF_PR->GetNumBuybackCredits(pPlayer->entindex())
											 : iRespawnWait * MVM_BUYBACK_COST_PER_SEC;
...
void CTFSpectatorGUI::UpdateKeyLabels(void)
{
		...
		if(m_bCoaching == true || (cl_hud_minmode.IsValid() && (cl_hud_minmode.GetBool() == false)))
		{
			if(m_pSwitchCamModeKeyLabel)
			{
				if((pPlayer && pPlayer->GetTeamNumber() > TEAM_SPECTATOR) &&
				   ((mp_forcecamera.GetInt() == OBS_ALLOW_TEAM) || (mp_forcecamera.GetInt() == OBS_ALLOW_NONE) ||
					mp_fadetoblack.GetBool()))
				{
			...
			{
				if((pPlayer && pPlayer->GetTeamNumber() > TEAM_SPECTATOR) &&
				   (mp_fadetoblack.GetBool() || (mp_forcecamera.GetInt() == OBS_ALLOW_NONE)))
				{
	...
	if(pPlayer && pPlayer->m_hStudent)
	{
		...
		C_TFPlayer *pStudent = pPlayer->m_hStudent;
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wLabel, g_pVGuiLocalize->Find("#TF_Coach_Student_Prefix"), 1,
												  wPlayerName);
...
void CTFSpectatorGUI::ShowPanel(bool bShow)
{
	if(bShow != IsVisible())
	{
		...
		CHudBuildingStatusContainer_Engineer *pEngBuilds =
			GET_NAMED_HUDELEMENT(CHudBuildingStatusContainer_Engineer, BuildingStatus_Engineer);
		CHudBuildingStatusContainer_Spy *pSpyBuilds =
			GET_NAMED_HUDELEMENT(CHudBuildingStatusContainer_Spy, BuildingStatus_Spy);
		...
		if(bShow)
		{
			...
			// when the class menu is shown.  We use the player spawn cache here - which is a terrible name for
			// something very useful - which will nuke m_nDisplaySaveReplay every time a new map is loaded, which is
			...
			if(g_pReplay->IsRecording() && !engine->IsPlayingDemo() && !::input->IsSteamControllerActive() &&
			   nDisplaySaveReplay &&
			   (pCurLifeReplay && !pCurLifeReplay->m_bRequestedByUser && !pCurLifeReplay->m_bSaved))
			{
				...
				g_pVGuiLocalize->ConstructString_safe(wText, g_pVGuiLocalize->Find("#Replay_SaveThisLifeMsg"), 1,
													  wKeyBind);
...
void CTFSpectatorGUI::UpdateItemPanel(bool bForce)
{
	...
	if((pStatPanel && pStatPanel->IsVisible()) || (TFGameRules() && TFGameRules()->State_Get() == GR_STATE_TEAM_WIN) ||
	   (!cl_spec_carrieditems.GetBool() && !bForce))
	{
		bVisible = false;
	}
	else
	{
		...
		if(pLocalPlayer)
		{
			...
			{
				if(m_flNextItemPanelUpdate && m_flNextItemPanelUpdate > gpGlobals->curtime &&
				   m_hPrevItemPlayer == pPlayer)
					return;

				if(TFGameRules() && TFGameRules()->IsMannVsMachineMode() &&
				   (pPlayer->GetTeamNumber() == TF_TEAM_PVE_INVADERS))
					return;
				...
				if(!m_bShownItems || bForce)
				{
					...
					if(pItemToShow)
					{
						...
						if(pOriginalOwner)
						{
							m_pItemPanel->SetDialogVariable("ownername",
															g_TF_PR->GetPlayerName(pOriginalOwner->entindex()));
						}
						...
						m_pItemPanel->SetPos(ScreenWidth() - XRES(10) - m_pItemPanel->GetWide(),
											 ScreenHeight() - YRES(12) - m_pItemPanel->GetTall());
					}
				}
			}
		}
...
bool CTFSpectatorGUI::InTournamentGUI(void)
{
	...
	return (TFGameRules()->IsInTournamentMode() && !TFGameRules()->IsCompetitiveMode() &&
			(cl_use_tournament_specgui.GetBool() || bOverride));
}
...
void CTFSpectatorGUI::RecalculatePlayerPanels(void)
{
	...
	// Calculate the number of players that must be shown. Spectators see all players (except in MvM), team members only
	// see their team.
	int iPanel = 0;
