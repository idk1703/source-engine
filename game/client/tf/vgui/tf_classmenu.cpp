...
#if defined(REPLAY_ENABLED)
ConVar replay_replaywelcomedlgcount("replay_replaywelcomedlgcount", "0",
									FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE | FCVAR_HIDDEN,
									"The number of times the replay help dialog has displayed.");
#endif

ConVar tf_mvm_classupgradehelpcount("tf_mvm_classupgradehelpcount", "0",
									FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE | FCVAR_HIDDEN,
									"The number of times the player upgrade help dialog has displayed.");
...
public:
	CTFClassTipsListPanel(Panel *parent, const char *panelName) : PanelListPanel(parent, panelName)
	{
	...
	void ApplySchemeSettings(IScheme *pScheme)
	{
		...
		if(m_pScrollBar)
		{
			...
			if(m_pScrollBar->IsVisible())
			{
				...
				if(m_pLine)
				{
					m_pLine->SetBounds(wide - nScrollbarWide, nScrollbarWide, nScrollbarWide,
									   tall - (2 * nScrollbarWide));
				}
				...
				if(m_pDownArrow)
				{
					m_pDownArrow->SetBounds(wide - nScrollbarWide, tall - nScrollbarWide, nScrollbarWide,
											nScrollbarWide);
...
public:
	CTFClassTipsPanel(Panel *parent, const char *panelName) : EditablePanel(parent, panelName)
	{
	...
	void SetClass(int iClass)
	{
		...
		if(m_pClassTipsListPanel)
		{
			...
			for(int iTip = 1; iTip < nTipCount + 1; ++iTip)
			{
				...
				{
					CTFClassTipsItemPanel *pClassTipsItemPanel =
						new CTFClassTipsItemPanel(this, "ClassTipsItemPanel", iTip);
					if(pwszText)
					{
...
void CTFClassMenu::ApplySchemeSettings(IScheme *pScheme)
{
	...
	for(int i = 0; i < ARRAYSIZE(m_pClassButtons); ++i)
	{
		...
		m_pClassButtons[i]->SetImageSelected(
			CFmtStr("class_sel_sm_%s_%s", g_aRawPlayerClassNamesShort[i], pTeamExtension).Access());
...
void CTFClassMenu::ShowPanel(bool bShow)
{
	if(bShow)
	{
		...
		if((TFGameRules()->State_Get() == GR_STATE_TEAM_WIN && C_TFPlayer::GetLocalTFPlayer() &&
		   ...
		   (TFGameRules()->IsInTraining() && C_TFPlayer::GetLocalTFPlayer() &&
			(C_TFPlayer::GetLocalTFPlayer()->GetPlayerClass() == NULL ||
			 C_TFPlayer::GetLocalTFPlayer()->GetPlayerClass()->GetClassIndex() != TF_CLASS_UNDEFINED)))
		{
			SetVisible(false);
			...
			if(pNotifyPanel)
			{
				...
				{
					pNotifyPanel->SetupNotifyCustom("#TF_CantChangeClassNow", "ico_notify_flag_moving",
													C_TFPlayer::GetLocalTFPlayer()->GetTeamNumber());
				}
			}
...
void CTFClassMenu::LoadItems()
{
	...
	for(int i = 0; i < CLASS_LOADOUT_POSITION_COUNT; i++)
	{
		...
		if(pItemData && pItemData->IsValid())
		{
			...
			bCanUseFancyClassSelectAnimation =
				bCanUseFancyClassSelectAnimation && !pItemData->FindAttribute(pAttrDef_DisableFancyLoadoutAnim);
	...
	m_pTFPlayerModelPanel->PlayVCD(bCanUseFancyClassSelectAnimation ? pszVCD : NULL,
								   g_pszLegacyClassSelectVCDWeapons[iClass]);
...
void CTFClassMenu::OnKeyCodePressed(KeyCode code)
{
	...
	else if(code == KEY_ENTER || code == KEY_SPACE || code == KEY_XBUTTON_A || code == KEY_XBUTTON_RTRIGGER ||
			code == STEAMCONTROLLER_A)
	{
		Go();
	}
	else if((m_iClassMenuKey != BUTTON_CODE_INVALID && m_iClassMenuKey == code) || code == KEY_XBUTTON_BACK ||
			code == KEY_XBUTTON_B || code == STEAMCONTROLLER_B || code == KEY_0 || code == KEY_PAD_0)
	{
		C_TFPlayer *pLocalPlayer = C_TFPlayer::GetLocalTFPlayer();
...
void CTFClassMenu::Go()
{
	...
	if(g_pReplay->IsReplayEnabled() &&
	   !g_pEngineClientReplay->IsPlayingReplayDemo() && // FIXME: We shouldn't need this here but for some reason the
														// engine thinks a replay is recording during demo playback,
														// even though replay_recording has a FCVAR_DONTRECORD flag
	   !nDisplayedConnectedRecording && replay_replaywelcomedlgcount.GetInt() <= MAX_TIMES_TO_SHOW_REPLAY_WELCOME_DLG)
	{
		wchar_t wText[256];
		wchar wKeyBind[80];
		char szText[256];
...
static const char *g_sDialogVariables[] = {
	"",			  "numScout",  "numSoldier",  "numPyro",

	"numDemoman", "numHeavy",  "numEngineer",

	"numMedic",	  "numSniper", "numSpy",	  "",
};
	...
	for(int i = TF_FIRST_NORMAL_CLASS; i <= TF_LAST_NORMAL_CLASS; i++)
	{
		...
		if(iClassLimit != NO_CLASS_LIMIT)
		{
			...
			else
			{
				...
				g_pVGuiLocalize->ConstructString_safe(wTemp, g_pVGuiLocalize->Find("TF_ClassLimitUnder"), 2, wzCount,
													  wzMax);
				SetDialogVariable(g_sDialogVariables[i], wTemp);
			}
		}
