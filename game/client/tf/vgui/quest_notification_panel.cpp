...
// Purpose:
//-----------------------------------------------------------------------------
CQuestNotification_Speaking::CQuestNotification_Speaking(CEconItem *pItem) : CQuestNotification(pItem)
{
...
float CQuestNotification_Speaking::Present(CQuestNotificationPanel *pNotificationPanel)
{
	...
	if(m_hItem)
	{
		...
		if(pTheme)
		{
			...
			{
				flPresentTime =
					enginesound->GetSoundDuration(m_pszSoundToSpeak) + m_timerDialog.GetCountdownDuration() + 1.f;
				m_timerShow.Start(enginesound->GetSoundDuration(m_pszSoundToSpeak) +
								  m_timerDialog.GetCountdownDuration() + 1.f);
			}
...
bool CQuestNotificationPanel::AddNotificationForItem(const CEconItemView *pItem, CQuestNotification *pNotification)
{
	...
	if(Plat_FloatTime() < m_mapNotifiedItemIDs[idx][pNotification->GetType()] ||
	   m_mapNotifiedItemIDs[idx][pNotification->GetType()] == NEVER_REPEAT)
	{
		return false;
	}
	...
	if(!bTypeAlreadyInQueue &&
	   (m_bInitialized || pNotification->GetType() == CQuestNotification::NOTIFICATION_TYPE_NEW_QUEST))
	{
		// Add notification
	...
	m_mapNotifiedItemIDs[idx][pNotification->GetType()] = pNotification->GetReplayTime() == NEVER_REPEAT
															  ? NEVER_REPEAT
															  : Plat_FloatTime() + pNotification->GetReplayTime();
