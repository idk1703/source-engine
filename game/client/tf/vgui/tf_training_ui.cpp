...
ConVar cl_training_completed_with_classes("cl_training_completed_with_classes", "0", FCVAR_ARCHIVE,
...
int Training_GetNumCourses()
{
	...
	AssertMsg(s_nTotal == 5,
			  "Number of total courses is incorrect - should be soldier (2) + demo (1) + spy (1) + engy (1)");
...
public:
	CTrainingBasePanel(Panel *pParent, const char *pName) : EditablePanel(pParent, pName), m_pPrevPagePanel(NULL) {}
	...
	virtual void OnKeyCodePressed(KeyCode nCode)
	{
				...
				nButtonCode == STEAMCONTROLLER_DPAD_LEFT || nButtonCode == KEY_LEFT)
		{
			GoPrev();
				...
				nButtonCode == KEY_XSTICK2_RIGHT || nButtonCode == STEAMCONTROLLER_DPAD_RIGHT ||
...
public:
	CTrainingBaseCarouselPanel(Panel *pParent, const char *pName) : CTrainingBasePanel(pParent, pName), m_iPage(0) {}
	...
	virtual void OnKeyCodePressed(KeyCode nCode)
	{
		   ...
		   nButtonCode == KEY_LEFT)
		{
			GoPrev();
				...
				nButtonCode == KEY_XSTICK2_RIGHT || nButtonCode == KEY_RIGHT)
		{
			GoNext();
		}
		else
...
public:
	CModePanel(Panel *pParent, const char *pName) : EditablePanel(pParent, pName)
	{
		HScheme hScheme = vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL),
...
public:
	CModeSelectionPanel(Panel *pParent, const char *pName) : CTrainingBasePanel(pParent, pName)
	{
		SetProportional(true);
	}
...
public:
	CBasicTraining_ClassPanel(Panel *pParent, const char *pName)
		: EditablePanel(pParent, pName), m_pImagePanel(NULL), m_pSelectButton(NULL)
	{
		SetProportional(true);
	}
	...
	virtual void ApplySchemeSettings(IScheme *pScheme)
	{
		...
		if(m_pSelectButton)
		{
			m_pSelectButton->SetDefaultBorder(
				pScheme->GetBorder(m_pSelectButton->IsEnabled() ? "MainMenuButtonDefault" : "MainMenuButtonDisabled"));
		}
		...
		if(m_pProgressLabel)
		{
			...
			if(nPercent > 0)
			{
				if(nPercent < 100)
				{
					...
					g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#TR_Progress"), 1,
														  wszNum);
				}
				else
				{
...
public:
	CBasicTraining_ClassSelectionPanel(Panel *pParent, const char *pName) : CTrainingBasePanel(pParent, pName)
	{
		SetProportional(true);
	...
	virtual void PerformLayout()
	{
		...
		const int aTrainingClasses[NUM_CLASS_PANELS] = {TF_CLASS_SOLDIER, TF_CLASS_DEMOMAN, TF_CLASS_SPY,
														TF_CLASS_ENGINEER};
		...
		for(int i = 0; i < NUM_CLASS_PANELS; ++i)
		{
			...
			pCurClassPanel->SetDialogVariable("selectbuttontext",
											  g_pVGuiLocalize->Find(m_PanelInfos[i].m_strSelectButtonToken.Get()));
...
public:
	CBasicTraining_ClassDetailsPanel(Panel *pParent, const char *pName)
		: CTrainingBasePanel(pParent, pName), m_iClass(TF_CLASS_UNDEFINED), m_pStartTrainingButton(NULL)
	{
		SetProportional(true);
	}
	...
	virtual void ApplySchemeSettings(IScheme *pScheme)
	{
		...
		if(pOverlayPanel && m_iClass >= TF_FIRST_NORMAL_CLASS && m_iClass < TF_LAST_NORMAL_CLASS)
		{
			...
			for(int i = 0; i < 3; ++i)
			{
				...
				ImagePanel *pCurImage =
					dynamic_cast<ImagePanel *>(pOverlayPanel->FindChildByName(fmtWeaponImageName.Access()));
...
public:
	COfflinePractice_ModeSelectionPanel(Panel *pParent, const char *pName)
		: CTrainingBaseCarouselPanel(pParent, pName), m_pGameModeImagePanel(NULL)
	{
		SetProportional(true);
	}
...
public:
	...
	virtual void ApplySchemeSettings(IScheme *pScheme)
	{
		...
		if(pSuggestedPlayerCountLabel)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#TF_OfflinePractice_NumPlayers"),
												  2, wszNum1, wszNum2);
	...
	virtual void OnKeyCodePressed(KeyCode nCode)
	{
		...
		if(nButtonCode == KEY_XBUTTON_X)
		{
			if(m_pDifficultyComboBox)
			{
				m_pDifficultyComboBox->SilentActivateItemByRow((m_pDifficultyComboBox->GetActiveItem() + 1) %
															   ARRAYSIZE(g_pDifficultyModes));
			}
		}
		else if(nButtonCode == KEY_XBUTTON_UP || nButtonCode == KEY_XSTICK1_UP || nButtonCode == KEY_XSTICK2_UP ||
				nButtonCode == KEY_UP)
		{
...
private:
	virtual KeyValues *GetTitleFormatData() const
	{
		...
		if(pResult)
		{
			const char *pGameModeToken =
				(m_iGameMode >= 0 && m_iGameMode < NUM_GAME_MODES) ? gs_pGameModeTokens[m_iGameMode] : "";
...
public:
	CTrainingDialog(Panel *parent)
		: EditablePanel(parent, TRAINING_DIALOG_NAME),
		  ...
		  m_bContinue(false)
	{
		HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL),
															  "resource/ClientScheme.res", "ClientScheme");
		SetScheme(scheme);
		SetProportional(true);
	...
	{
		ShowConfirmDialog("#TR_ContinueTitle", "#TR_ContinueMsg", "#TR_Continue", "#TR_StartOver",
						  &ConfirmDialogCallback, NULL, this);
	}
	...
	virtual void ApplySchemeSettings(IScheme *pScheme)
	{
		...
		m_pModeSelectionPanel =
			dynamic_cast<CModeSelectionPanel *>(m_pContainer->FindChildByName("ModeSelectionPanel"));
		Assert(m_pModeSelectionPanel);
		m_pBasicTraining_ClassSelectionPanel = dynamic_cast<CBasicTraining_ClassSelectionPanel *>(
			m_pContainer->FindChildByName("BasicTraining_ClassSelectionPanel"));
		Assert(m_pBasicTraining_ClassSelectionPanel);
		m_pBasicTraining_ClassDetailsPanel = dynamic_cast<CBasicTraining_ClassDetailsPanel *>(
			m_pContainer->FindChildByName("BasicTraining_ClassDetailsPanel"));
		Assert(m_pBasicTraining_ClassDetailsPanel);
		m_pOfflinePractice_ModeSelectionPanel = dynamic_cast<COfflinePractice_ModeSelectionPanel *>(
			m_pContainer->FindChildByName("OfflinePractice_ModeSelectionPanel"));
		Assert(m_pOfflinePractice_ModeSelectionPanel);
		m_pOfflinePractice_MapSelectionPanel = dynamic_cast<COfflinePractice_MapSelectionPanel *>(
			m_pContainer->FindChildByName("OfflinePractice_MapSelectionPanel"));
	...
	virtual void OnKeyCodePressed(KeyCode code)
	{
		...
		else if(code == KEY_ENTER || code == KEY_SPACE || nButtonCode == KEY_XBUTTON_A ||
				nButtonCode == STEAMCONTROLLER_A)
		{
			if(m_pCurrentPagePanel)
			{
