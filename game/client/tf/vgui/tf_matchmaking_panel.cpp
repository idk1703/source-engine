...
// Purpose:
//-----------------------------------------------------------------------------
CMatchMakingPanel::CMatchMakingPanel(IViewPort *pViewPort)
	: EditablePanel(NULL, PANEL_MATCHMAKING), m_iMMPanelKey(BUTTON_CODE_INVALID)
{
...
void CMatchMakingPanel::ShowPanel(bool bShow)
{
	...
	if(bShow && (!steamapicontext || !steamapicontext->SteamUtils() || !steamapicontext->SteamMatchmakingServers() ||
				 !steamapicontext->SteamUser() || !steamapicontext->SteamUser()->BLoggedOn()))
	{
...
void CMatchMakingPanel::StartSearch(void)
{
	...
	if(!pParty || pParty->GetNumMembers() <= 1)
	{
		...
		{
			ShowEconRequirementDialog("#TF_Competitive_RequiresPass_Title", "#TF_Competitive_RequiresPass",
									  CTFItemSchema::k_rchLadderPassItemDefName);
			return;
		}
		else if(bCompetitive && !CheckCompetitiveConvars())
		{
			ShowMessageBox("#TF_Competitive_Convars_CantProceed_Title", "#TF_Competitive_Convars_CantProceed",
						   "#GameUI_OK");
			return;
		}
	}
			...
			if(bCompetitive)
			{
				if(!pParty->Obj().members(i).competitive_access())
				{
					...
					g_pVGuiLocalize->ConstructString_safe(
						wszLocalized, g_pVGuiLocalize->Find("#TF_Matchmaking_MissingPass"), 1, wszCharPlayerName);
