...
ConVar tf_matchmaking_ticket_help("tf_matchmaking_ticket_help", "0",
								  FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE | FCVAR_HIDDEN,
								  "Saved if the player has see the ticket help screen.");
...
{
	return GTFGCClientSystem()->GetSearchPlayForBraggingRights() ? k_nMatchGroup_MvM_MannUp
		...
		if(BIsPartyInUIState() && GTFGCClientSystem()->GetSearchPlayForBraggingRights())
		{
			if(m_pSquadSurplusCheckButton->IsSelected())
			{
				...
				else
				{
					...
					ShowEconRequirementDialog("#TF_MvM_RequiresSquadSurplusVoucher_Title",
											  "#TF_MvM_RequiresSquadSurplusVoucher",
...
void CLobbyPanel_MvM::WriteGameSettingsControls()
{
	...
	if(!tf_matchmaking_ticket_help.GetBool() && bShowBottomPanel &&
	   GTFGCClientSystem()->GetSearchPlayForBraggingRights())
	{
		OnCommand("open_help");
	...
	m_pSquadSurplusImage->SetImage(GTFGCClientSystem()->GetLocalPlayerSquadSurplus() ? "pve/mvm_voucher_active"
																					 : "pve/mvm_voucher_inactive");
...
bool CLobbyPanel_MvM::ShouldShowLateJoin() const
{
	...
	return
		...
		((eWizardStep == TF_Matchmaking_WizardStep_SEARCHING) &&
		 (GTFGCClientSystem()->GetSearchMode() == TF_Matchmaking_MVM));
}
...
void CLobbyPanel_MvM::WriteTourList()
{
	...
	m_pTourList->AddColumnToSection(0, "check_box", "", vgui::SectionedListPanel::COLUMN_IMAGE,
									m_iChallengeCheckBoxWidth);
	...
	if(idxSelectedTour >= 0)
	{
		...
		if(tour.m_eDifficulty >= k_EMvMChallengeDifficulty_Expert)
		{
			...
			if(!GTFGCClientSystem()->BGetLocalPlayerBadgeInfoForTour(idxSelectedTour, &unBadgeLevel,
																	 &unCompletedChallengeMask) ||
			   unBadgeLevel == 0)
			{
...
void CLobbyPanel_MvM::WriteChallengeList()
{
	...
	else
	{
		m_pMvMSelectChallengeGroupPanel->SetDialogVariable(
			"tour_name",
			g_pVGuiLocalize->Find(GetItemSchema()->GetMvmTours()[idxTour].m_sTourNameLocalizationToken.Get()));
	}
	m_pMvMSelectChallengeGroupPanel->SetDialogVariable(
		"complete_heading", g_pVGuiLocalize->Find(bForBraggingRights ? "#TF_MvM_Complete" : "#TF_MvM_Difficulty"));
	...
	m_pChallengeList->AddColumnToSection(nCurrentSection, "check_box", "", vgui::SectionedListPanel::COLUMN_IMAGE,
										 m_iChallengeCheckBoxWidth);
	...
	FOR_EACH_VEC(GetItemSchema()->GetMvmMissions(), iMissionIndex)
	{
...
#ifdef USE_MVM_TOUR
		if(bForBraggingRights && GetItemSchema()->FindMvmMissionInTour(idxTour, iMissionIndex) <
									 0) // !KLUDGE! This is sort of crappy, we probably should iterate the tour's
										// mission list rather than iterating the larger list with filtering
#else									// new mm
		...
		if(mission.m_iDisplayMapIndex != nCurrentMap)
		{
			...
			m_pChallengeList->SetSectionDividerColor(nCurrentSection, Color(0, 0, 0, 0));
#ifdef USE_MVM_TOUR
			m_pChallengeList->AddColumnToSection(nCurrentSection, "check_box", "",
	   ...
	   // for mannup, don't show check box
...
#endif // USE_MVM_TOUR
			m_pChallengeList->AddColumnToSection(nCurrentSection, "spacer", "", 0, m_iChallengeSpacer);
			m_pChallengeList->AddColumnToSection(nCurrentSection, "display_name", map.m_sDisplayName.Get(), 0,
												 m_iChallengeNameWidth);
		...
		//								  g_pVGuiLocalize->Find( mission.m_sDisplayName.Get() ), g_pVGuiLocalize->Find(
		//mission.m_sMode.Get() ) ); kvItem->SetWString( "display_name", wszChallengeName );
...
void CLobbyPanel_MvM::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pOpenStoreButton = dynamic_cast<vgui::Button *>(FindChildByName("OpenStoreButton", true));
	Assert(m_pOpenStoreButton);
	m_pOpenStoreButton2 = dynamic_cast<vgui::Button *>(FindChildByName("OpenStoreButton2", true));
	Assert(m_pOpenStoreButton2);
	m_pOpenHelpButton = dynamic_cast<vgui::Button *>(FindChildByName("OpenHelpButton", true));
	Assert(m_pOpenHelpButton);
	...
	int nExtraWidth = m_pChatPlayerList->GetWide() - nAvatarWidth - m_iPlayerNameWidth - m_iBannedWidth -
					  m_iHasTicketWidth - m_iSquadSurplusWidth - m_iBadgeLevelWidth;
	...
	m_pChatPlayerList->AddColumnToSection(
		...
		m_iBannedWidth);
	m_pChatPlayerList->AddColumnToSection(
		0, "has_ticket", "", vgui::SectionedListPanel::COLUMN_IMAGE | vgui::SectionedListPanel::COLUMN_CENTER,
		m_iHasTicketWidth);
	m_pChatPlayerList->AddColumnToSection(
		0, "squad_surplus", "", vgui::SectionedListPanel::COLUMN_IMAGE | vgui::SectionedListPanel::COLUMN_CENTER,
		m_iSquadSurplusWidth);
	m_pChatPlayerList->AddColumnToSection(0, "badge_level", "#TF_MvM_Tours", vgui::SectionedListPanel::COLUMN_CENTER,
										  m_iBadgeLevelWidth);
}
...
void CLobbyPanel_MvM::ChallengeList::Paint()
{
	...
	if(idxTour >= 0)
	{
		...
		int nCompletedX0 = m_pLobbyPanel->m_iChallengeCheckBoxWidth + m_pLobbyPanel->m_iChallengeNameWidth +
						   m_pLobbyPanel->m_iChallengeSkillWidth / 4;

		uint32 nTours, nCompletedChallenge;
		bool bFoundPlayerCompletedChallenges =
			GTFGCClientSystem()->BGetLocalPlayerBadgeInfoForTour(idxTour, &nTours, &nCompletedChallenge);
