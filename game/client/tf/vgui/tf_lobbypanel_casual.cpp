...
ConVar tf_show_maps_details_explanation_count("tf_show_maps_details_explanation_count", "2",
...
public:
	...
	{
		if(bIsExpanded && !tf_show_maps_details_explanation_session.GetBool() &&
		   tf_show_maps_details_explanation_count.GetInt() > 0)
		{
			...
			// I dont care anymore
			GetParent()->GetParent()->GetParent()->GetParent()->GetParent()->GetParent()->OnCommand(
				"show_maps_details_explanation");
		}
	...
	void SetCheckButtonState(uint32 nMapDefIndex, bool bSelected, bool bClickable)
	{
		...
		if(idx != m_mapMapPanels.InvalidIndex())
		{
			...
			if(g_pClientMode && g_pClientMode->GetViewport())
			{
				if(bSelected)
				{
					g_pClientMode->GetViewportAnimationController()->StopAnimationSequence(
						pMapEntry, "HealthProgressBar_NotSelected");
					g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
						pMapEntry, "HealthProgressBar_Selected");
				}
				else
				{
					g_pClientMode->GetViewportAnimationController()->StopAnimationSequence(
...
void CLobbyPanel_Casual::WriteCategories(void)
{
	...
	FOR_EACH_MAP(mapMMGroups, i)
	{
		...
		FOR_EACH_VEC(pCat->m_vecModes, j)
		{
			...
			{
				pListEntry =
					new CCasualCategory(pScrollableList, "MatchmakingCategoryPanel", pCategory->m_eGameCategory, this);
			...
			{
				bool bMapSelected =
					GTFGCClientSystem()->IsCasualMapSelected(pCategory->m_vecEnabledMaps[k]->m_nDefIndex);
			...
			if(bCatSelected)
			{
				g_pClientMode->GetViewportAnimationController()->StopAnimationSequence(pListEntry,
																					   "CasualCategory_NotSelected");
				g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(pListEntry,
	...
	if(pPlayListPanel)
	{
		...
		const char *pszToken =
			vecSelectedMaps.Count() == 1 ? "TF_Casual_SelectedMaps_Singular" : "TF_Casual_SelectedMaps_Plural";
		pPlayListPanel->SetDialogVariable("selected_maps_count",
										  LocalizeNumberWithToken(pszToken, vecSelectedMaps.Count()));
	}
