...
// Purpose:
//-----------------------------------------------------------------------------
CTFImmediateAutoJoinHandler::CTFImmediateAutoJoinHandler() : m_flNextAutoJoinTime(0.f) {}
...
CTFMatchMakingPopupPrompJoinHandler::CTFMatchMakingPopupPrompJoinHandler() : m_flNextRejoinThinkTime(0.f) {}
...
void CTFMatchMakingPopupPrompJoinHandler::UpdatePromptState()
{
	...
	if(!m_pRejoinPrompt && bHasMatch && !bInLiveMatch)
	{
		...
		m_pRejoinPrompt = vgui::SETUP_PANEL(
			new CTFRejoinConfirmDialog(pszTitle, pszBody, pszConfirm, pszCancel, &OnJoinLobbyInProgressCallback, NULL));
