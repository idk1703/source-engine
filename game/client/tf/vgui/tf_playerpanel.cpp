...
bool CTFPlayerPanel::Update(void)
{
	...
	int iRespawnWait = -1;
	m_bPlayerReadyModeActive = (!bObserver && TFGameRules()->UsePlayerReadyStatusMode() &&
	...
	if(bVisible)
	{
		if(g_TF_PR)
		{
			...
			if(m_iPlayerIndex > 0)
			{
				...
				if(!bObserver && TFGameRules()->IsCompetitiveMode() &&
			...
			if(m_pClassImage)
			{
				...
				if(m_iPrevClass != iClass)
				{
					...
					{
						m_pClassImage->SetImage(VarArgs("%s_alpha", (GetTeam() == TF_TEAM_RED)
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayerPanel::Setup(int iPlayerIndex, CSteamID steamID, const char *pszPlayerName,
						   int nLobbyTeam /*= TEAM_INVALID*/)
{
