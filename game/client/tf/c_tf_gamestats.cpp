...
ConVar tf_matchmaking_ogs_odds("tf_matchmaking_ogs_odds", "0.05", FCVAR_HIDDEN,
							   "Percentage (0...1) of quickplay queries that will report to OGS");

const char *g_ItemEventNames[] = {
	// STORE EVENTS
	"store_entered", "store_exited", "store_tab_changed", "store_item_selected", "store_item_previewed",
	"store_item_added_to_cart", "store_item_removed_from_cart", "store_checkout_attempt", "store_checkout_failure",
	"store_checkout_success", "store_checkout_item",

	// LOADOUT EVENTS
	"loadout_entered", "loadout_exited",

	// TRADE EVENTS
	"trading_entered", "trading_exited", "trading_went_to_armory", "trading_returned_from_armory",
	"trading_request_sent", "trading_request_received", "trading_request_rejected", "trading_request_accepted",
	"trading_trade_negotiated", "trading_trade_success", "trading_trade_failure", "trading_item_given",
	"trading_item_received", "trading_item_gifted",

	// CRAFTING EVENTS
	"crafting_entered", "crafting_exited", "crafting_went_to_armory", "crafting_returned_from_armory",
	"crafting_view_blueprints", "crafting_timeout", "crafting_failure", "crafting_success", "crafting_no_recipe_match",
	"crafting_attempt", "crafting_recipe_found",

	// ARMORY EVENTS
	"armory_entered", "armory_exited", "armory_select_item", "armory_browse_wiki", "armory_change_filter",

	// GENERAL EVENTS
	"item_received", "item_discarded", "item_deleted", "item_used_tool", "item_used_consumable", "item_removed_attrib",
	"item_changed_style"
...
bool C_CTFGameStats::Init(void)
{
	...
	{
		// We want to track the country code to help with localization. The countrycode is empty when testing on
		// SteamBeta, so we won't get data until users in the wild play
		...
		// Now lets get the text language that Steam is in (If the game supports the language, then the UI is changed to
		// that language).
...
{
	m_currentMap.Init(engine->GetLevelName(), engine->GetLevelVersion(), 0, 0,
					  GetSteamWorksSGameStatsUploader().GetTimeSinceEpoch());
...
void C_CTFGameStats::SW_GameStats_WriteClientSessionSummary()
{
	...
	if(sb_firstopentime.GetInt() > 0 && m_currentSession.m_FirstConnect > 0)
	{
...
//-----------------------------------------------------------------------------
TF_Gamestats_CraftingEvent::TF_Gamestats_CraftingEvent(int in_eventNum, CEconItemView *in_item, int in_numAttempts,
													   int in_recipe)
	: TF_Gamestats_ItemEvent(in_eventNum, in_item)
{
...
void C_CTFGameStats::Event_Store(int eventID, CEconItemView *item, const char *panelName, int classId,
								 const cart_item_t *cartItem, int checkoutAttempts, const char *storeError,
								 int totalPrice, int currencyCode)
{
	TF_Gamestats_StoreEvent event(eventID, item, panelName, classId, cartItem, checkoutAttempts, storeError, totalPrice,
								  currencyCode);
...
//-----------------------------------------------------------------------------
TF_Gamestats_StoreEvent::TF_Gamestats_StoreEvent(int in_eventNum, CEconItemView *in_item, const char *in_panelName,
												 int in_classId, const cart_item_t *in_cartItem,
												 int in_checkoutAttempts, const char *in_storeError, int in_totalPrice,
												 int in_currencyCode)
	: TF_Gamestats_ItemEvent(in_eventNum, in_item)
{
	classId = in_classId;
	...
	if(in_cartItem)
	{
		CEconItemDefinition *pItemDef =
			ItemSystem()->GetStaticDataForItemByDefIndex(in_cartItem->pEntry->GetItemDefinitionIndex());
		if(pItemDef)
		{
...
//-----------------------------------------------------------------------------
TF_Gamestats_ItemTransactionEvent::TF_Gamestats_ItemTransactionEvent(int in_eventNum, CEconItemView *in_item,
																	 const char *in_reason, int in_quality)
	: TF_Gamestats_ItemEvent(in_eventNum, in_item)
{
	static CSchemaAttributeDefHandle pAttrDef_SupplyCrateSeries("set supply crate series");
...
//-----------------------------------------------------------------------------
void C_CTFGameStats::Event_Trading(int eventID, CEconItemView *item, bool localPlayerIsPartyA, uint64 steamIDPartyA,
								   uint64 steamIDPartyB, int iTradeRequests, int iTradeAttempts)
{
	TF_Gamestats_TradeEvent event(eventID, item, localPlayerIsPartyA, steamIDPartyA, steamIDPartyB, iTradeRequests,
								  iTradeAttempts);
...
TF_Gamestats_TradeEvent::TF_Gamestats_TradeEvent(int in_eventNum, CEconItemView *in_item, bool in_localPlayerIsPartyA,
												 uint64 in_steamIDPartyA, uint64 in_steamIDPartyB, int in_tradeRequests,
												 int in_tradeAttempts)
	: TF_Gamestats_ItemEvent(in_eventNum, in_item)
{
...
TF_Gamestats_TradeEvent::TF_Gamestats_TradeEvent(int in_eventNum, uint64 in_steamIDRequested, int in_tradeRequests,
												 int in_tradeAttempts)
...
TF_Gamestats_TradeEvent::TF_Gamestats_TradeEvent(int in_eventNum, int in_tradeRequests, const char *in_reason,
												 int in_tradeAttempts)
	: TF_Gamestats_ItemEvent(in_eventNum, NULL)
{
	localPlayerPartyMatters = false;
...
void C_CTFGameStats::QuickplayResults(const TF_Gamestats_QuickPlay_t &info)
{
	...
	float r = RandomFloat();
	int iDetailLevel =
		(r < odds) ? TF_Gamestats_QuickPlay_t::k_Server_Ineligible : TF_Gamestats_QuickPlay_t::k_Server_Pinged;
