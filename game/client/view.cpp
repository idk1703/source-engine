...
ConVar zoom_sensitivity_ratio("zoom_sensitivity_ratio", "1.0", FCVAR_ARCHIVE,
...
ConVar gl_clear_randomcolor("gl_clear_randomcolor", "0", FCVAR_CHEAT,
							"Clear the back buffer to random colors every frame. Helps spot open seams in geometry.");

static ConVar r_farz("r_farz", "-1", FCVAR_CHEAT,
...
//-----------------------------------------------------------------------------
void ComputeCameraVariables(const Vector &vecOrigin, const QAngle &vecAngles, Vector *pVecForward, Vector *pVecRight,
							Vector *pVecUp, VMatrix *pMatCamInverse)
{
	// Compute view bases
...
bool R_CullSphere(VPlane const *pPlanes, int nPlanes, Vector const *pCenter, float radius)
{
	...
	if(player)
	{
...
#endif
		{
			...
			else
			{
				...
				gHUD.m_flFOVSensitivityAdjust = ((float)localFOV / (float)iDefaultFOV) * // linear fov downscale
												zoom_sensitivity_ratio.GetFloat();		 // sensitivity scale factor
#ifndef _XBOX
				gHUD.m_flMouseSensitivity =
					gHUD.m_flFOVSensitivityAdjust * sensitivity.GetFloat(); // regular sensitivity
#endif
			}
		}
	}
...
void CViewRender::SetUpViews()
{
	...
	C_BaseAnimating::PopBoneAccess(
		"OnRenderStart->CViewRender::SetUpView"); // pops the (true, false) bone access set in OnRenderStart
												...
												bool bCreatePowerOf2Padded /*=false*/, bool bWriteVTF /*=false*/)
{
	...
	else
	{
		...
		bWriteResult = TGAWriter::WriteToBuffer(pSrcImage, buffer, nSrcWidth, nSrcHeight, IMAGE_FORMAT_RGB888,
												IMAGE_FORMAT_RGB888);
	}
...
void CViewRender::SetUpOverView()
{
	...
	if(newCRC != oldCRC)
	{
		Msg("Overview: scale %.2f, pos_x %.0f, pos_y %.0f\n", cl_leveloverview.GetFloat(), viewEye.origin.x,
			viewEye.origin.y);
		oldCRC = newCRC;
	}
...
void CViewRender::Render(vrect_t *rect)
{
	...
	for(StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye(); eEye = (StereoEye_t)(eEye + 1))
	{
		...
		{
			limitedAspectRatio = MIN(
				aspectRatio,
				1.85f *
					0.75f); // cap out the FOV advantage at a 1.85:1 ratio (about the widest any legit user should be)
		}
		...
		switch(eEye)
		{
			case STEREO_EYE_MONO:
			{
				...
				viewEye.m_flAspectRatio =
					(engineAspectRatio > 0.0f) ? engineAspectRatio : ((float)viewEye.width / (float)viewEye.height);
			}
			break;
		...
		if(UseVR())
		{
			...
			if(!bInFreezeCam && g_ClientVirtualReality.ShouldRenderHUDInWorld())
			{
				...
				g_ClientVirtualReality.OverlayHUDQuadWithUndistort(
