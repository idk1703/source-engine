...
void CCSModeManager::LevelInit(const char *newmap)
{
	...
	if(!cl_detail_max_sway.GetFloat() && !cl_detail_avoid_radius.GetFloat() && !cl_detail_avoid_force.GetFloat() &&
...
void ClientModeCSNormal::FireGameEvent(IGameEvent *event)
{
	...
	else if(Q_strcmp("player_team", eventname) == 0)
	{
		...
		if(iTeam == TEAM_SPECTATOR)
			pHudChat->Printf(CHAT_FILTER_NONE, hudtextmessage->LookupString("#Game_join_spectators"),
							 pPlayer->GetPlayerName());
		else if(iTeam == TEAM_TERRORIST)
			pHudChat->Printf(CHAT_FILTER_NONE, hudtextmessage->LookupString("#Game_join_terrorist"),
							 pPlayer->GetPlayerName());
	...
	else if(Q_strcmp("achievement_earned", eventname) == 0)
	{
		...
		if(pAchievement)
		{
			if(!pPlayer->IsDormant() && pPlayer->ShouldAnnounceAchievement())
			{
				...
				// Do something for the player - Actually we should probably do this client-side when the achievement is
				// first earned.
				if(pPlayer->IsLocalPlayer())
				{
				}
			...
			if(g_PR)
			{
				...
				if(constAchievementName)
				{
					...
					g_pVGuiLocalize->ConstructString(wszLocalizedString, sizeof(wszLocalizedString),
													 g_pVGuiLocalize->Find("#Achievement_Earned"), 2, wszPlayerName,
													 constAchievementName /*wszAchievementString*/);
					...
					if (pPlayer->IsLocalPlayer())
					{
						...
						constAchievementDescription = pUserStats->GetAchievementDisplayAttribute(
					pAchievement->GetName(), "desc" ); hudChat->ChatPrintf( iPlayerIndex, CHAT_FILTER_ACHIEVEMENT,
					"(%s)", constAchievementDescription );
					}
					*/
				}
			}
...
void UpdateClassImageEntity(const char *pModelName, int x, int y, int width, int height)
{
	...
	pPlayerModel->m_SequenceTransitioner.CheckForSequenceChange(
		pPlayerModel->GetModelPtr(), pPlayerModel->LookupSequence("walk_lower"), false, true);
	pPlayerModel->m_SequenceTransitioner.UpdateCurrent(
		pPlayerModel->GetModelPtr(), pPlayerModel->LookupSequence("walk_lower"), pPlayerModel->GetCycle(),
