...
static const char *InterestingPoseParameters[NumInterestingPoseParameters] = {"body_yaw",	"spine_yaw", "neck_trans",
...
RecvPropInt(RECVINFO(m_isRescued), 0, C_CHostage::RecvProxy_Rescued), RecvPropInt(RECVINFO(m_iHealth)),
	RecvPropInt(RECVINFO(m_iMaxHealth)), RecvPropInt(RECVINFO(m_lifeState)),
...
void C_CHostage::UpdateLookAt(CStudioHdr *pStudioHdr)
{
	...
	bodyAngles[YAW] =
		GetRenderAngles()[YAW] + RemapVal(poseParams[m_bodyYawPoseParam], 0, 1, m_bodyYawMin, m_bodyYawMax);
...
void C_CHostage::UpdateClientSideAnimation()
{
	...
	char *setToZero[] = {"spine_yaw", "head_roll"};
	CStudioHdr *pStudioHdr = GetModelPtr();
