...
ConVar cl_disablefreezecam("cl_disablefreezecam", "0", FCVAR_ARCHIVE, "Turn on/off freezecam on client");
...
public:
	...
	HorizontalGauge(Panel *parent, const char *name) : vgui::Panel(parent, name), m_fPercent(0.0f) {}
...
bool CCSFreezePanel::ShouldDraw(void)
{
	...
	return (m_bShouldBeVisible && !cl_disablefreezecam.GetBool() && !sv_disablefreezecam.GetBool() &&
...
void CCSFreezePanel::SetActive(bool bActive)
{
	...
	if(bActive)
	{
		...
		g_pVGuiLocalize->ConstructString(wLabel, sizeof(wLabel), g_pVGuiLocalize->Find("#FreezePanel_SaveReplay"), 1,
										 wKey);
