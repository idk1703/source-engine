...
void InterpolateRamp(const CHitEffectRamp &a, const CHitEffectRamp &b, CHitEffectRamp &out, int iDamage)
{
...
void FX_HitEffectSmoke(CSmartPtr<CBloodSprayEmitter> pEmitter, int iDamage, const Vector &vEntryPoint,
					   const Vector &vDirection, float flScale)
{
	...
	InterpolateRamp(ramps[0], ramps[1], interpolatedRamp, iDamage);
	...
	for(int i = 0; i < 2; i++)
	{
		...
		newParticle.m_uchStartSize =
			random->RandomInt(interpolatedRamp.m_flMinSize, interpolatedRamp.m_flMaxSize) * flScale;
		newParticle.m_uchEndSize = newParticle.m_uchStartSize * 4;

		newParticle.m_vecVelocity = Vector(0, 0, 5) + RandomVector(-2, 2);
		newParticle.m_uchStartAlpha = random->RandomInt(interpolatedRamp.m_flMinSize, interpolatedRamp.m_flMaxSize);
...
void FX_HitEffectBloodSpray(CSmartPtr<CBloodSprayEmitter> pEmitter, int iDamage, const Vector &vEntryPoint,
							const Vector &vSprayNormal, const char *pMaterialName, float flLODDistance,
							float flDistanceScale, float flScale, float flSpeed)
{
	...
	for(int i = 0; i < 6; i++)
	{
		...
		if(pParticle != NULL)
		{
			...
			pParticle->m_vecVelocity +=
				vSprayNormal * random->RandomInt(interpolatedRamp.m_flMinVelocity, interpolatedRamp.m_flMaxVelocity) *
				flSpeed * speedFactor;
			...
			pParticle->m_uchStartSize =
				random->RandomFloat(interpolatedRamp.m_flMinSize, interpolatedRamp.m_flMaxSize) * flDistanceScale;
			pParticle->m_uchEndSize = pParticle->m_uchStartSize * 4 * flDistanceScale;

			pParticle->m_uchStartAlpha =
				random->RandomInt(interpolatedRamp.m_flMinAlpha, interpolatedRamp.m_flMaxAlpha);
...
void FX_HitEffectBloodSplatter(CSmartPtr<CBloodSprayEmitter> pTrailEmitter, int iDamage, const Vector &vExitPoint,
							   const Vector &vSplatterNormal, float flLODDistance)
{
	float flScale = 4;
	...
	for(int i = 0; i < 20; i++)
	{
		...
		SimpleParticle *tParticle = (SimpleParticle *)pTrailEmitter->AddSimpleParticle(
			hMaterial, vExitPoint, random->RandomFloat(0.225f, 0.35f),
		...
		tParticle->m_vecVelocity =
			offDir * random->RandomFloat(interpolatedRamp.m_flMinVelocity, interpolatedRamp.m_flMaxVelocity);
...
void FX_CS_BloodSpray(const Vector &origin, const Vector &normal, float flDamage)
{
	...
	Vector color = Vector((float)(worldLight[0] * r) / 255.0f, (float)(worldLight[1] * g) / 255.0f,
						  (float)(worldLight[2] * b) / 255.0f);
	float colorRamp;
