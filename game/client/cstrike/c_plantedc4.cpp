...
RecvPropBool(RECVINFO(m_bBombTicking)), RecvPropFloat(RECVINFO(m_flC4Blow)), RecvPropFloat(RECVINFO(m_flTimerLength)),
	RecvPropFloat(RECVINFO(m_flDefuseLength)), RecvPropFloat(RECVINFO(m_flDefuseCountDown)),
END_RECV_TABLE
()
...
void C_PlantedC4::ClientThink(void)
{
	...
	if(gpGlobals->curtime > m_flNextGlow)
	{
		...
		if(enginetrace->GetPointContents(vPos) & (CONTENTS_WATER | CONTENTS_SLIME))
		{
			...
			if(player)
			{
				...
				if((enginetrace->GetPointContents(eyes) & (CONTENTS_WATER | CONTENTS_SLIME)) == 0)
				{
					...
					trace_t waterTrace;
					UTIL_TraceLine(eyes, vPos, (CONTENTS_WATER | CONTENTS_SLIME), player, COLLISION_GROUP_NONE,
								   &waterTrace);

					if(waterTrace.allsolid != 1)
					{
						// now trace from the C4 to the edge of the water (in case there was something solid in the
						// water)
						trace_t solidTrace;
		...
		tempents->TempSprite(vPos, vec3_origin, scale, modelindex, kRenderTransAdd, 0, 1.0, 0.05,
