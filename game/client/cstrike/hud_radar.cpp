...
void CHudRadar::MsgFunc_UpdateRadar(bf_read &msg)
{
	...
	while(iPlayerEntity > 0)
	{
		...
		bool bOppositeTeams = (pLocalPlayer->GetTeamNumber() != TEAM_UNASSIGNED &&
							   pCSPR->GetTeam(pPlayer->entindex()) != pLocalPlayer->GetTeamNumber());
...
void CHudRadar::WorldToRadar(const Vector location, const Vector origin, const QAngle angles, float &x, float &y,
							 float &z_delta)
{
...
void CHudRadar::DrawPlayerOnRadar(int iPlayer, C_CSPlayer *pLocalPlayer)
{
	...
	bool bOppositeTeams =
		(pLocalPlayer->GetTeamNumber() != TEAM_UNASSIGNED && pCSPR->GetTeam(iPlayer) != pLocalPlayer->GetTeamNumber());
...
bool CHudLocation::ShouldDraw()
{
	...
	if(g_pMapOverview && g_pMapOverview->GetMode() == CMapOverview::MAP_MODE_RADAR && pCSMapOverview &&
	   pCSMapOverview->ShouldDraw() == true)
		return true;
