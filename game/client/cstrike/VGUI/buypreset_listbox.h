...
public:
	...
	virtual int AddItem(vgui::Panel *panel,
						void *userData);	///< Adds an item to the end of the listbox.  UserData is assumed to be a
											///< pointer that can be freed by the listbox if non-NULL.
	...
	virtual void RemoveItem(int index); ///< Removes an item from the table (changing the indices of all following
										///< items), deleting the panel and userData
