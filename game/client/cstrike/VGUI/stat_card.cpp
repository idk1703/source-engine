...
void StatCard::UpdateInfo()
{
	...
	if(deaths > 0)
	{
		...
		g_pVGuiLocalize->ConstructString(buf, sizeof(buf), g_pVGuiLocalize->Find("#GameUI_Stats_LastMatch_KDRatio"), 1,
										 numBuf);
	...
	g_pVGuiLocalize->ConstructString(buf, sizeof(buf), g_pVGuiLocalize->Find("#GameUI_Stats_LastMatch_MVPS"), 1,
									 numBuf);
