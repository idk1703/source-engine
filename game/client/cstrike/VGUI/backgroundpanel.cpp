...
static void FixupBackgroundPanels(EditablePanel *pWindow, int offsetX, int offsetY)
{
	...
	{
		pPanel->SetZPos(-20);
		pPanel->SetBounds(screenWide - offsetX - inset - cornerSize, screenTall - offsetY - inset - cornerSize,
	...
	{
		pPanel->SetZPos(-20);
		pPanel->SetBounds(offsetX + inset, offsetY + inset + cornerSize, screenWide - 2 * offsetX - 2 * inset,
						  titleHeight);
	}
	...
	{
		pPanel->SetZPos(-20);
		pPanel->SetBounds(offsetX + inset + cornerSize, screenTall - offsetY - inset - cornerSize,
						  screenWide - 2 * offsetX - 2 * inset - 2 * cornerSize, cornerSize);
	}
	...
	{
		pPanel->SetZPos(-20);
		pPanel->SetBounds(offsetX + inset, screenTall - offsetY - inset - cornerSize - mainHeight,
	...
	{
		pPanel->SetZPos(-20);
		pPanel->SetBounds(screenWide - offsetX - inset, offsetY + inset, offsetX + inset,
	...
	{
		pPanel->SetZPos(-19); // higher than the background
		pPanel->SetBounds(offsetX + captionInset /*inset + 2*logoInset + logoSize*/, offsetY + inset + logoInset,
						  screenWide, logoSize);
	}
}
	...
	for(int i = 0; i < panelList->Size(); ++i)
	{
		...
		if(panel != pWindow)
		{
			...
			DevMsg("Repositioning '%s' from (%d,%d) to (%d,%d) -- a distance of (%d,%d)\n", panel->GetName(), x, y,
				   x + offsetX, y + offsetY, offsetX, offsetY);
#endif
		}
	}
}
