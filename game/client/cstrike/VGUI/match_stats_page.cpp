...
void CMatchStatsPage::RepopulateStats()
{
	...
	if(m_detailedWeaponStatsGroupPanel->IsGroupActive() || m_allStatsGroupPanel->IsGroupActive())
	{
		...
		for(CSStatType_t killStat = CSSTAT_KILLS_DEAGLE; killStat <= CSSTAT_KILLS_HEGRENADE;
			killStat = (CSStatType_t)(killStat + 1))
		{
...
int CMatchStatsPage::AddSimpleStat(int desiredStat, const StatsCollection_t &personalMatchStats,
								   RoundStatsDirectAverage_t *tStats, RoundStatsDirectAverage_t *ctStats,
								   RoundStatsDirectAverage_t *serverStats)
{
	...
	else
	{
		...
		pKeyValues->SetString("ctValue",
							  (ctStats->m_fStat[stat.iStatId] || personalMatchStats[stat.iStatId]) ? buf : "");
	}

	Q_snprintf(buf, sizeof(buf), "%.1f", serverStats->m_fStat[stat.iStatId]);
	pKeyValues->SetString("serverValue",
						  (serverStats->m_fStat[stat.iStatId] || personalMatchStats[stat.iStatId]) ? buf : "");
...
int CMatchStatsPage::AddKillToDeathStat(const StatsCollection_t &personalMatchStats, RoundStatsDirectAverage_t *tStats,
										RoundStatsDirectAverage_t *ctStats, RoundStatsDirectAverage_t *serverStats)
{
...
int CMatchStatsPage::AddAccuracyStat(const StatsCollection_t &personalMatchStats, RoundStatsDirectAverage_t *tStats,
									 RoundStatsDirectAverage_t *ctStats, RoundStatsDirectAverage_t *serverStats)
{
