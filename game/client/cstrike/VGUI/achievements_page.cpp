...
//-----------------------------------------------------------------------------
bool CSLoadAchievementIconForPage(vgui::ImagePanel *pIconPanel, CCSBaseAchievement *pAchievement,
...
CAchievementsPage::CAchievementsPage(vgui::Panel *parent, const char *name) : BaseClass(parent, "CSAchievementsDialog")
{
	...
	for(int i = 0; i < iCount; ++i)
	{
		CCSBaseAchievement *pAchievement =
			dynamic_cast<CCSBaseAchievement *>(g_AchievementMgrCS.GetAchievementByIndex(i));
...
{
	g_AchievementMgrCS.SaveGlobalStateIfDirty(
...
void CAchievementsPage::UpdateTotalProgressDisplay()
{
	...
	V_sprintf_safe(szPercentageText, "%d / %d", nUnlocked, g_AchievementMgrCS.GetAchievementCount());
...
void CAchievementsPage::UpdateAchievementList(int minID, int maxID)
{
	...
	FOR_EACH_VEC(sortedAchivementList, i)
	{
		...
		CAchievementsPageItemPanel *pAchievementItemPanel =
			new CAchievementsPageItemPanel(m_pAchievementsList, "AchievementDialogItemPanel");
		pAchievementItemPanel->SetAchievementInfo(pAchievement);

		// force all our new panel to have the correct internal layout and size so that our parent container can layout
		// properly
...
void CAchievementsPage::CreateOrUpdateComboItems(bool bCreate)
{
	...
	for(int i = 0; i < m_iNumAchievementGroups; i++)
	{
		...
		if(bCreate)
		{
			...
			CAchievementsPageGroupPanel *achievementGroupPanel =
				new CAchievementsPageGroupPanel(m_pGroupsList, this, "AchievementDialogGroupPanel", i);
			achievementGroupPanel->SetGroupInfo(wzGroupTitle, m_AchievementGroups[i].m_iMinRange,
												m_AchievementGroups[i].m_iMaxRange);
...
// Purpose: creates child panels, passes down name to pick up any settings from res files.
//-----------------------------------------------------------------------------
CAchievementsPageItemPanel::CAchievementsPageItemPanel(vgui::PanelListPanel *parent, const char *name)
...
void CAchievementsPageItemPanel::PreloadResourceFile(void)
{
	...
	/*
		...
		{
			CCSBaseAchievement* pAchievement =
	   dynamic_cast<CCSBaseAchievement*>(g_AchievementMgrCS.GetAchievementByIndex( i )); char imagePath[_MAX_PATH];
...
// Purpose: creates child panels, passes down name to pick up any settings from res files.
//-----------------------------------------------------------------------------
CAchievementsPageGroupPanel::CAchievementsPageGroupPanel(vgui::PanelListPanel *parent, CAchievementsPage *owner,
														 const char *name, int iListItemID)
	: BaseClass(parent, name)
{
	m_pParent = parent;
	m_pOwner = owner;
...
void CAchievementsPage::OnThink()
{
	...
	else if(m_bStatsDirty)
	{
		...
		{
			CAchievementsPageItemPanel *pAchievementItem =
				dynamic_cast<CAchievementsPageItemPanel *>(m_pAchievementsList->GetItemPanel(itemId));
