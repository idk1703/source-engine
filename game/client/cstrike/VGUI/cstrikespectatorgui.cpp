...
ConVar overview_preferred_mode("overview_preferred_mode", "1", FCVAR_ARCHIVE, "Preferred overview mode",
							   PreferredOverviewModeChanged);

ConVar overview_preferred_view_size("overview_preferred_view_size", "600", FCVAR_ARCHIVE,
...
void CCSSpectatorGUI::StoreWidths(void)
{
	...
	if(!m_modifiedWidths)
	{
		...
		{
			m_pTerScore =
				NULL; // We're working with a modified res file.  Don't muck things up playing with positioning.
		}
	}
}
...
{
	return (m_pCTLabel != NULL && m_pCTScore != NULL && m_pTerLabel != NULL && m_pTerScore != NULL &&
			m_pTimer != NULL && m_pTimerLabel != NULL && m_pDivider != NULL && m_pExtraInfo != NULL);
}
...
bool CCSMapOverview::CanPlayerBeSeen(MapPlayer_t *player)
{
	...
	if(GetMode() == MAP_MODE_RADAR)
	{
		...
		if(csPlayer->timeLastSeen != -1 && (now - csPlayer->timeLastSeen < TIME_SPOTS_STAY_SEEN) &&
...
void CCSMapOverview::UpdatePlayers()
{
	...
	if(GetMode() == MAP_MODE_RADAR)
	{
		...
		for(int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			// Now that everyone has had a say on people they can see for us, go through and handle baddies that can no
			// longer be seen.
			if(playerCS->timeLastSeen != now && player->health > 0)
			{
				...
				if(timeSinceLastSeen < 0.5f && (playerCS->timeLastSeen != -1))
				{
					if(now - playerCS->timeFirstSeen > TIME_UNTIL_ENEMY_SEEN)
					{
						...
						;
						playerCS->overrideIconOffscreen =
							m_TeamIconsOffscreen[GetIconNumberFromTeamNumber(player->team)];
...
void CCSMapOverview::UpdateHostages()
{
	...
	for(int i = 0; i < MAX_HOSTAGES; i++)
	{
		if(pCSPR->IsHostageAlive(i))
		{
			...
			if(hostage == NULL)
				hostage =
					&m_Hostages[i]; // Don't have entry yet, so need one.  This'll only happen once, at start of map
...
void CCSMapOverview::UpdateBomb()
{
	...
	else if(pCSPR->HasC4(0))
	{
		...
		else
		{
			...
			// Not a bomb map.  Man, what a weird system instead of IsBombMap.  If nobody has it, and it isn't on the
			// ground, then it isn't a bomb map.
		}
	}
	else
	{
		for(int i = 1; i <= gpGlobals->maxClients; i++)
		{
			if(pCSPR->HasC4(i))
			{
				...
				{
					// Dormant or no player means we are relying on RadarUpdate messages so we can trust the MapOverview
					// position.
					MapPlayer_t *player = &m_Players[i - 1];
				...
				{
					// Update players is about to put this Real Data in the player sturct, and we don't want the bomb
					// pos lagged one update behind
	...
	{
		m_bomb.currentRingRadius +=
			(m_bomb.maxRingRadius - m_flIconSize) * gpGlobals->frametime / m_bomb.ringTravelTime;
		m_bomb.currentRingRadius = MIN(m_bomb.currentRingRadius, m_bomb.maxRingRadius);
		m_bomb.currentRingAlpha =
			(alpha - 55) * ((m_bomb.maxRingRadius - m_bomb.currentRingRadius) / (m_bomb.maxRingRadius - m_flIconSize)) +
			55;
	}
}
...
bool CCSMapOverview::AdjustPointToPanel(Vector2D *pos)
{
	...
	if(mapInset != 0)
		mapInset +=
			BORDER_WIDTH; // And this gives us the border inside the map edge to give us room for offscreen icons.
...
void CCSMapOverview::DrawMapTexture()
{
	...
	if(textureIDToUse > 0)
	{
		...
		Vertex_t points[4] = {// To draw a textured polygon, the first column is where you want to draw (to), and the
							  // second is what you want to draw (from). We want to draw to the panel (pulled in for a
							  // border), and we want to draw the part of the map texture that should be seen. First
							  // column is in panel coords, second column is in 0-1 texture coords
							  Vertex_t(panelTL, textureTL), Vertex_t(panelTR, textureTR), Vertex_t(panelBR, textureBR),
...
void CCSMapOverview::DrawBomb()
{
	...
	if(localMapPlayer->team == TEAM_CT)
	{
		...
		else if((m_bomb.timeLastSeen == -1) || (now - m_bomb.timeLastSeen >= TIME_SPOTS_STAY_SEEN) ||
...
bool CCSMapOverview::DrawIconCS(int textureID, int offscreenTextureID, Vector pos, float scale, float angle, int alpha,
								bool allowRotation, const char *text, Color *textColor, float status,
								Color *statusColor)
{
	...
	Vertex_t points[4] = {Vertex_t(pos1Panel, Vector2D(0, 0)), Vertex_t(pos2Panel, Vector2D(1, 0)),
						  Vertex_t(pos3Panel, Vector2D(1, 1)), Vertex_t(pos4Panel, Vector2D(0, 1))};
	...
	for(int i = 0; i < MAX_PLAYERS; i++)
	{
		...
		if(playerCS->overrideExpirationTime > gpGlobals->curtime) // If dead, an X, if alive, an alpha'd normal icon
		{
			...
			if(playerCS->overrideFadeTime != -1 && playerCS->overrideFadeTime <= gpGlobals->curtime)
			{
				...
				alphaToUse *= 1 - (float)(gpGlobals->curtime - playerCS->overrideFadeTime) /
									  (float)(playerCS->overrideExpirationTime - playerCS->overrideFadeTime);
			}

			DrawIconCS(playerCS->overrideIcon, playerCS->overrideIconOffscreen, playerCS->overridePosition,
					   m_flIconSize * 1.1f, GetViewAngle(), player->health > 0 ? alphaToUse / 2 : alphaToUse, true,
					   name, &player->color, -1, &colorGreen);
			if(player->health > 0)
				DrawIconCS(m_playerFacing, -1, playerCS->overridePosition, m_flIconSize * 1.1f,
						   playerCS->overrideAngle[YAW], player->health > 0 ? alphaToUse / 2 : alphaToUse, true, name,
						   &player->color, status, &colorGreen);
		}
		else
		{
			float zDifference = 0;
			...
			float sizeForPlayer = m_flIconSize * 1.1f; // The 1.1 is because the player dots are shrunken a little, so
													   // their facing pip can have some space to live
			...
			bool showTalkRing = localPlayer && (localPlayer->GetTeamNumber() == player->team ||
												localPlayer->GetTeamNumber() == TEAM_SPECTATOR);

			if(showTalkRing && playerCS->currentFlashAlpha > 0) // Flash type
			{
				// Make them flash a halo
				DrawIconCS(m_radioFlash, m_radioFlashOffscreen, player->position, sizeForRing, player->angle[YAW],
						   playerCS->currentFlashAlpha);
			}
			else if(showTalkRing && pCSPR->IsAlive(i + 1) &&
					GetClientVoiceMgr()->IsPlayerSpeaking(i + 1)) // Or solid on type
			{
			...
			DrawIconCS(player->icon, offscreenIcon, player->position, sizeForPlayer, angleForPlayer, alpha, true, name,
					   &player->color, status, &colorGreen);
			if(!doingLocalPlayer)
			{
				// Draw the facing for everyone but the local player.
				if(player->health > 0)
					DrawIconCS(m_playerFacing, -1, player->position, sizeForPlayer, player->angle[YAW], alpha, true,
							   name, &player->color, status, &colorGreen);
			}
		}
	}
...
void CCSMapOverview::DrawHostages()
{
	...
	for(int i = 0; i < MAX_HOSTAGES; i++)
	{
		...
		if(hostageCS->overrideExpirationTime > gpGlobals->curtime) // If dead, an X, if alive, an alpha'd normal icon
		{
			...
			{
				// Fade linearly from fade start to disappear
				alphaToUse *= 1 - (float)(gpGlobals->curtime - hostageCS->overrideFadeTime) /
									  (float)(hostageCS->overrideExpirationTime - hostageCS->overrideFadeTime);
			}

			DrawIconCS(hostageCS->overrideIcon, hostageCS->overrideIconOffscreen, hostageCS->overridePosition,
					   m_flIconSize, hostageCS->overrideAngle[YAW], hostage->health > 0 ? alphaToUse / 2 : alphaToUse,
					   true, name, &hostage->color, status, &colorGreen);
		}
		else
		{
			...
			{
				// Make them flash a halo
				DrawIconCS(m_radioFlash, m_radioFlashOffscreen, hostage->position, m_flIconSize * 1.4f,
						   hostage->angle[YAW], hostageCS->currentFlashAlpha);
			}
			...
			DrawIconCS(normalIcon, offscreenIcon, hostage->position, sizeForHostage, GetViewAngle(), alpha, true, name,
					   &hostage->color, status, &colorGreen);

			if(pCSPR->IsHostageFollowingSomeone(i))
			{
				// If they are following a CT, then give them a little extra symbol to show it.
				DrawIconCS(m_hostageFollowing, m_hostageFollowingOffscreen, hostage->position, sizeForHostage,
						   hostage->angle[YAW], alpha);
			}
		}
	}
}
...
bool CCSMapOverview::CreateRadarImage(const char *mapName, const char *radarFileName)
{
	...
	buf.SeekGet(CUtlBuffer::SEEK_HEAD,
				0); // Need to set these explicitly since ->Read goes straight to memory and skips them.
...
void CCSMapOverview::DrawCamera()
{
	...
	if(localPlayer->GetObserverMode() == OBS_MODE_ROAMING)
	{
		...
		int alpha = 255;
		DrawIconCS(m_cameraIconFree, m_cameraIconFree, localPlayer->GetAbsOrigin(), m_flIconSize * 3.0f,
				   localPlayer->EyeAngles()[YAW], alpha);
	}
	else if(localPlayer->GetObserverMode() == OBS_MODE_IN_EYE)
	{
		if(localPlayer->GetObserverTarget())
		{
			...
			int alpha = 255 * 0.5f;
			DrawIconCS(m_cameraIconFirst, m_cameraIconFirst, localPlayer->GetObserverTarget()->GetAbsOrigin(),
					   m_flIconSize * 1.5f, GetViewAngle(), alpha);
		}
	}
	else if(localPlayer->GetObserverMode() == OBS_MODE_CHASE)
	{
		if(localPlayer->GetObserverTarget())
		{
			...
			int alpha = 255;
			DrawIconCS(m_cameraIconThird, m_cameraIconThird, localPlayer->GetObserverTarget()->GetAbsOrigin(),
					   m_flIconSize * 3.0f, localPlayer->EyeAngles()[YAW], alpha);
		}
	}
}
...
void CCSMapOverview::SetMode(int mode)
{
	if(mode == MAP_MODE_RADAR)
	{
		...
		g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(
			this, "zoom", desiredZoom, 0.0, 0, vgui::AnimationController::INTERPOLATOR_LINEAR);
	...
	else if(mode == MAP_MODE_INSET)
	{
		...
		g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(
			this, "zoom", desiredZoom, 0.0f, 0.2f, vgui::AnimationController::INTERPOLATOR_LINEAR);
	}
	else
	{
...
void CCSMapOverview::UpdateSizeAndPosition()
{
	...
	switch(m_nMode)
	{
		case MAP_MODE_RADAR:
		{
			...
			m_vSize.x = w;
			m_vSize.y = w; // Intentionally not 't'.  We need to enforce square-ness to prevent people from seeing more
						   // of the map by fiddling their HudLayout
			break;
		}
	...
	for(int i = 0; i < MAX_PLAYERS; i++)
	{
		...
		else
		{
			...
			else
			{
				// Just fade away
				playerCS->currentFlashAlpha -=
					((playerCS->currentFlashAlpha * gpGlobals->frametime) / (playerCS->nextFlashPeakTime - now));
...
void CCSMapOverview::DrawGoalIcons()
{
	for(int iconIndex = 0; iconIndex < m_goalIcons.Count(); iconIndex++)
	{
		...
		int alpha = 255;
		DrawIconCS(currentIcon->iconToUse, currentIcon->iconToUse, currentIcon->position, m_flIconSize, GetViewAngle(),
				   alpha, false);
	}
}
