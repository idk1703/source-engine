		...
		if(isVar)
		{
			...
			if(IsX360())
			{
				...
				for(int iCode = 0; iCode < BUTTON_CODE_LAST; ++iCode)
				{
					...
					if(!bUseThisKey && (Q_stricmp(pBinding, "+duck") == 0 || Q_stricmp(pBinding, "toggle_duck") == 0) &&
					...
					if(!bUseThisKey && (Q_stricmp(pBinding, "+zoom") == 0 || Q_stricmp(pBinding, "toggle_zoom") == 0) &&
			...
			else
			{
				...
				else
				{
					...
					// keyboard bindings have never been localized in the past, they probably won't in the future
					// either.
					bIsBitmap = true;
...
#define TEST_KEYHINT_DISPLAY 0
	...
	const char *sampleText[] = {"This is a test", "of the hint system\nwith a multi-line hint", "that\ngoes\non\nfor",
								"several", "lines"};
