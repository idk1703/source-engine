...
void CControlPointIcon::UpdateCapImage(void)
{
	...
	if(m_pCapImage)
	{
		...
		if(iCappingTeam != TEAM_UNASSIGNED && iCappingTeam != iOwningTeam)
		{
			...
			if(m_pCapHighlightImage && pPlayer->GetTeamNumber() != iCappingTeam &&
...
{
	if(m_bCachedLockedState != IsPointLocked() || m_bCachedCountdownState != IsPointUnlockCountdownRunning())
	{
		UpdateImage();
	}
...
void CControlPointIcon::PerformLayout(void)
{
	...
	if(m_pCapHighlightImage->IsVisible())
	{
		...
		m_pCapHighlightImage->SetBounds(iBaseXPos + CAP_BOX_INDENT_X, iBaseYPos - iHeight,
										GetWide() - (CAP_BOX_INDENT_X * 2), iHeight + GetTall() - CAP_BOX_INDENT_Y);
	}
...
void CControlPointIcon::SetTimerTime(float flTime)
{
	...
	if(m_pCPTimerBG && m_pCPTimerLabel)
	{
		...
		else
		{
			...
			m_pCPTimerLabel->SetPos(xPos + GetWide() - XRES(1),
									yPos + (GetTall() / 2) - (m_pCPTimerLabel->GetTall() / 2));
...
void CControlPointIconCapArrow::Paint(void)
{
	...
	{
		flYa = RemapVal(flCapPercentage, 0.0, 1.0, -flMovementInTextureSpace - flIndent,
						flArrowSizeInTextureSpace - flIndent);
	...
	else
	{
		...
		{
			flXa = RemapVal(flCapPercentage, 0.0, 1.0, flStart + flMovementInTextureSpace,
							flEnd - flArrowSizeInTextureSpace);
			flXb = RemapVal(flCapPercentage, 0.0, 1.0, flStart, 0.0);
		}
		else
		{
			flXa = RemapVal(flCapPercentage, 0.0, 1.0, flStart, 0.0);
			flXb = RemapVal(flCapPercentage, 0.0, 1.0, flStart + flMovementInTextureSpace,
							flEnd - flArrowSizeInTextureSpace);
		}
	}
