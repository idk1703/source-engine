...
void C_BaseExplosionEffect::Create(const Vector &position, float force, float scale, int flags)
{
	...
	if(debugoverlay)
	{
		debugoverlay->AddBoxOverlay(m_vecOrigin, -Vector(32, 32, 32), Vector(32, 32, 32), vec3_angle, 255, 0, 0, 64,
									5.0f);
		debugoverlay->AddLineOverlay(m_vecOrigin, m_vecOrigin + (m_vecDirection * force * m_flForce), 0, 0, 255, false,
									 3);
	}
#endif
...
void C_BaseExplosionEffect::CreateCore(void)
{
	...
	if((m_fFlags & TE_EXPLFLAG_NOFIREBALLSMOKE) == 0)
	{
		...
		for(i = 0; i < 4; i++)
		{
			...
			if(pParticle != NULL)
			{
				...
				if(debugoverlay)
				{
					debugoverlay->AddLineOverlay(m_vecOrigin, m_vecOrigin + pParticle->m_vecVelocity, 255, 0, 0, false,
												 3);
				}
#endif
	...
	for(i = 0; i < 16; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
														   m_Material_Embers[random->RandomInt(0, 1)], offset);
...
void C_BaseExplosionEffect::CreateDebris(void)
{
	...
	{
		tParticle =
			(TrailParticle *)pSparkEmitter->AddParticle(sizeof(TrailParticle), m_Material_FireCloud, m_vecOrigin);
	...
	CSmartPtr<CFleckParticles> fleckEmitter =
		CFleckParticles::Create("CreateDebris 2", m_vecOrigin, Vector(128, 128, 128));
	if(!fleckEmitter)
		return;
	...
	for(i = 0; i < numFlecks; i++)
	{
		...
		FleckParticle *pParticle = (FleckParticle *)fleckEmitter->AddParticle(
			sizeof(FleckParticle), g_Mat_Fleck_Cement[random->RandomInt(0, 1)], offset);
...
float C_BaseExplosionEffect::Probe(const Vector &origin, Vector *vecDirection, float strength)
{
	...
	if(debugoverlay)
	{
		debugoverlay->AddLineOverlay(m_vecOrigin, endpos, (255 * (1.0f - tr.fraction)), (255 * tr.fraction), 0, false,
									 3);
	}
#endif
...
//			&m_flForce -
//-----------------------------------------------------------------------------
void C_BaseExplosionEffect::GetForceDirection(const Vector &origin, float magnitude, Vector *resultDirection,
											  float *resultForce)
{
	Vector d[6];
...
void C_WaterExplosionEffect::CreateCore(void)
{
	...
	// Ground splash
	FX_AddQuad(m_vecWaterSurface + Vector(0, 0, 2), Vector(0, 0, 1), 64, 64 * 4.0f, 0.85f, luminosity, 0.0f, 0.25f,
			   random->RandomInt(0, 360), random->RandomFloat(-4, 4), color, 2.0f, "effects/splashwake1",
...
void C_WaterExplosionEffect::CreateDebris(void)
{
	...
	for(i = 0; i < 16; i++)
	{
		...
		pParticle =
			(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), pMaterial[random->RandomInt(0, 1)], offset);
...
void C_WaterExplosionEffect::CreateMisc(void)
{
	...
	for(i = 0; i < numDrops; i++)
	{
		...
		FloatToColor32(tParticle->m_color, MIN(1.0f, m_vecColor[0] * colorRamp), MIN(1.0f, m_vecColor[1] * colorRamp),
					   MIN(1.0f, m_vecColor[2] * colorRamp), m_flLuminosity);
	}
	...
	pSimple->GetBinding().SetBBox(m_vecWaterSurface - (Vector(32.0f, 32.0f, 32.0f) * flScale),
