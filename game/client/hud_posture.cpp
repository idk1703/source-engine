	...
	if(PlayerIsDucking(pPlayer))
	{
		...
		if(GetAlpha() < 255)
		{
			...
			if(m_kIsFading != FADING_UP)
			{
				...
				GetAnimationController()->RunAnimationCommand(this, "alpha", 255, 0, HUD_POSTURE_FADE_TIME,
	...
	else // player is not ducking
	{
		if(GetAlpha() > 0)
		{
			...
			if(m_kIsFading != FADING_DOWN)
			{
				...
				GetAnimationController()->RunAnimationCommand(this, "alpha", 0, 0, HUD_POSTURE_FADE_TIME,
															  vgui::AnimationController::INTERPOLATOR_SIMPLESPLINE);
			}
		}
		else
