...
RecvPropString(RECVINFO(m_szMaterialName)), RecvPropString(RECVINFO(m_szMaterialVar)),
	RecvPropString(RECVINFO(m_szMaterialVarValue)), RecvPropInt(RECVINFO(m_iFrameStart)),
	RecvPropInt(RECVINFO(m_iFrameEnd)), RecvPropInt(RECVINFO(m_bWrap)), RecvPropFloat(RECVINFO(m_flFramerate)),
	RecvPropInt(RECVINFO(m_bNewAnimCommandsSemaphore)), RecvPropFloat(RECVINFO(m_flFloatLerpStartValue)),
	RecvPropFloat(RECVINFO(m_flFloatLerpEndValue)), RecvPropFloat(RECVINFO(m_flFloatLerpTransitionTime)),
	RecvPropInt(RECVINFO(m_bFloatLerpWrap)), RecvPropInt(RECVINFO(m_nModifyMode)),
END_RECV_TABLE
()
...
void CMaterialModifyProxy::OnBindSetVar(C_MaterialModifyControl *pControl)
{
	...
	if(Q_strcmp(pControl->GetMaterialVariableValue(), ""))
	{
		...
		//				m_pMaterial->GetName(), pMaterialVar->GetName(), pControl->GetMaterialVariableValue(), count++,
		//pControl );
		//		}
...
void CMaterialModifyProxy::OnBindFloatLerp(C_MaterialModifyControl *pControl)
{
	...
	if(bFound)
	{
		...
		{
			currentValue =
				m_flStartValue + (m_flEndValue - m_flStartValue) *
									 clamp(((gpGlobals->curtime - m_flStartTime) / m_flTransitionTime), 0.0f, 1.0f);
		}
		else
		{
		...
		if(debug_materialmodifycontrol_client.GetBool() && Q_stristr(m_pMaterial->GetName(), "faceandhair") &&
		   Q_stristr(m_pMaterialVar->GetName(), "warp"))
		{
			static int count = 0;
			DevMsg(1, "CMaterialFloatLerpProxy::OnBind \"%s\" %s=%f %d\n", m_pMaterial->GetName(),
				   m_pMaterialVar->GetName(), currentValue, count++);
		}
...
EXPOSE_INTERFACE(CMaterialModifyAnimatedProxy, IMaterialProxy,
				 "MaterialModifyAnimated" IMATERIAL_PROXY_INTERFACE_VERSION);