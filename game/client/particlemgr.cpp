...
static ConVar particle_simulateoverflow(
	"particle_simulateoverflow", "0", FCVAR_CHEAT,
	"Used for stress-testing particle systems. Randomly denies creation of particles.");
ConVar cl_particleeffect_aabb_buffer("cl_particleeffect_aabb_buffer", "2", FCVAR_CHEAT,
									 "Add this amount to a particle effect's bbox in the leaf system so if it's "
									 "growing slowly, it won't have to be reinserted as often.");
ConVar cl_particle_show_bbox("cl_particle_show_bbox", "0", FCVAR_CHEAT);
ConVar cl_particle_show_bbox_cost(
	"cl_particle_show_bbox_cost", "0", FCVAR_CHEAT,
...
static ConCommand cl_particle_stats_start("cl_particle_stats_start", StatsParticlesStart,
										  "Start or restart particle stats - also dumps to particle_stats.csv");
static ConCommand cl_particle_stats_stop(
	"cl_particle_stats_stop", StatsParticlesStop,
	"Stop particle stats, or snapshot this frame - also dumps to particle_stats.csv");
static ConVar cl_particle_stats_trigger_count("cl_particle_stats_trigger_count", "0", 0,
...
inline void CParticleEffectBinding::StartDrawMaterialParticles(CEffectMaterial *pMaterial, float flTimeDelta,
															   IMesh *&pMesh, CMeshBuilder &builder,
															   ParticleDraw &particleDraw, bool bWireframe)
{
	...
	if(bWireframe)
	{
		IMaterial *pMaterialWire =
			m_pParticleMgr->m_pMaterialSystem->FindMaterial("debug/debugparticlewireframe", TEXTURE_GROUP_OTHER);
...
int CParticleEffectBinding::DrawModel(int flags)
{
	...
	// At the very least, we absolutely should not do refraction updates below. So if this gets removed, be sure to wrap
	// the refract/screen texture updates.
	...
	FOR_EACH_LL(m_Materials, iMaterial)
	{
		...
		if(pMaterial->m_pGroup->m_pPageMaterial &&
		   pMaterial->m_pGroup->m_pPageMaterial->NeedsPowerOfTwoFrameBufferTexture())
		{
		...
		if(g_cl_particle_show_bbox || (g_cl_particle_show_bbox_cost != 0))
		{
			...
			if(bDraw)
			{
				if(debugoverlay)
				{
					...
					debugoverlay->AddTextOverlayRGB(center, 0, 0, r, g, b, 64, "%s:(%d)", m_pSim->GetEffectName(),
													m_nActiveParticles);
				}
			}
		}
...
{
	m_pParticleMgr->RepairPMaterial(
		hMaterial); // HACKHACK: Remove this when we can stop leaking handles from level to level.
...
void CParticleEffectBinding::DetectChanges()
{
	...
	if(m_Min.x < m_LastMin.x || m_Min.y < m_LastMin.y || m_Min.z < m_LastMin.z ||
	   ...
	   m_Min.x > (m_LastMin.x + flExtraBuffer) || m_Min.y > (m_LastMin.y + flExtraBuffer) ||
	   m_Min.z > (m_LastMin.z + flExtraBuffer) ||

	   m_Max.x > m_LastMax.x || m_Max.y > m_LastMax.y || m_Max.z > m_LastMax.z ||

	   m_Max.x < (m_LastMax.x - flExtraBuffer) || m_Max.y < (m_LastMax.y - flExtraBuffer) ||
...
void CParticleEffectBinding::GrowBBoxFromParticlePositions(CEffectMaterial *pMaterial, bool &bboxSet, Vector &bbMin,
														   Vector &bbMax)
{
...
{
	// NOTE (2012/11/27, TomF) - this whole system seems to be deprecated - nothing ever checks these flags, and
	// CParticleMgr::DrawBeforeViewModelEffects is never called by anything!
...
int CParticleEffectBinding::DrawMaterialParticles(bool bBucketSort, CEffectMaterial *pMaterial, float flTimeDelta,
												  bool bWireframe)
{
	// Setup everything.
	...
	if(bBucketSort)
	{
		DoBucketSort(pMaterial, renderIterator.m_zCoords, renderIterator.m_nZCoords, renderIterator.m_MinZ,
					 renderIterator.m_MaxZ);
	}
...
void CParticleEffectBinding::DoBucketSort(CEffectMaterial *pMaterial, float *zCoords, int nZCoords, float minZ,
										  float maxZ)
{
...
//-----------------------------------------------------------------------------
bool CParticleMgr::Init(unsigned int count, IMaterialSystem *pMaterials)
{
	Term();
...
void CParticleMgr::RemoveAllEffects()
{
	...
	// HACKHACK: commented out because we need to keep leaking handles until every piece of code that grabs one ditches
	// it at level end m_SubTextures.PurgeAndDeleteElements();
	...
	// HACKHACK: commented out because we need to keep leaking handles until every piece of code that grabs one ditches
	// it at level end m_SubTextureGroups.PurgeAndDeleteElements();
}
...
int CParticleMgr::ComputeParticleDefScreenArea(int nInfoCount, RetireInfo_t *pInfo, float *pTotalArea,
											   CParticleSystemDefinition *pDef, const CViewSetup &viewParticle,
											   const VMatrix &worldToPixels, float flFocalDist)
{
	...
	*pTotalArea = 0.0f;
	...
	for(; pCollection; pCollection = pCollection->GetNextCollectionUsingSameDef())
	{
		...
		float flProjRadius =
			(lSqr > flCullRadiusSqr) ? 0.5f * flFocalDist * flCullRadius / sqrt(lSqr - flCullRadiusSqr) : 1.0f;
...
bool CParticleMgr::RetireParticleCollections(CParticleSystemDefinition *pDef, int nCount, RetireInfo_t *pInfo,
											 float flScreenArea, float flMaxTotalArea)
{
...
bool CParticleMgr::EarlyRetireParticleSystems(int nCount, ParticleSimListEntry_t *ppEffects)
{
	...
	int nDefCount = 0;
	CParticleSystemDefinition **ppDefs =
		(CParticleSystemDefinition **)stackalloc(nCount * sizeof(CParticleSystemDefinition *));
	for(int i = 0; i < nCount; ++i)
	{
	...
	for(int i = 0; i < nDefCount; ++i)
	{
		...
		int nActualCount =
			ComputeParticleDefScreenArea(nCount, pInfo, &flScreenArea, pDef, *pViewSetup, worldToScreen, flFocalDist);
...
void CParticleMgr::UpdateNewEffects(float flTimeDelta)
{
	...
	// This is done on all particles, because it updates control point locations which we need to determine whether or
	// not we should do full simulation later.
	for(CNewParticleEffect *pNewEffect = m_NewEffects.m_pHead; pNewEffect; pNewEffect = pNewEffect->m_pNext)
	{
	...
	if((nParticleStatsTriggerCount > 0 && (nParticleActiveParticlesCount >= nParticleStatsTriggerCount)) ||
	   bFrameWarningNeeded)
	{
		if(m_nStatsFramesSinceLastAlert >=
		   (300 * 3)) // 3 seconds at the clamp of 300 fps (or 15 secs at 60fps). Just stop it spamming too much.
		{
			m_nStatsFramesSinceLastAlert = 0;
			if(m_bStatsRunning)
			{
			...
			// This single-frame capture doesn't work that well because the "actual drawn" numbers will just be zero. Ah
...
PMaterialHandle CParticleMgr::GetPMaterial(const char *pMaterialName)
{
	...
	{
		RepairPMaterial(
			m_SubTextures[hMat]); // HACKHACK: Remove this when we can stop leaking handles from level to level.

		return m_SubTextures[hMat];
	}
}

// HACKHACK: The old system would leak handles and materials until shutdown. The new system still needs to leak handles
// until every piece of code that grabs one ditches it at level end. This function takes a leaked handle from a previous
// level and reacquires necessary materials.
...
struct ParticleInfo_t
{
	ParticleInfo_t()
		: m_nCount(0),
		  m_nChildCount(0),
		  m_nTotalActiveParticles(0),
		  m_nTotalDrawnParticles(0),
		  m_nCountMax(0),
		  m_nChildCountMax(0),
		  m_nTotalActiveParticlesMax(0),
		  m_nTotalDrawnParticlesMax(0),
		  pDef(NULL)
	{
	}
	int m_nCount;
...
void CParticleMgr::StatsAccumulateActiveParticleSystems()
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		pGlobalInfo->m_nTotalActiveParticlesMax =
			Max(pGlobalInfo->m_nTotalActiveParticlesMax, pSingleInfo->m_nTotalActiveParticles);
		pGlobalInfo->m_nTotalDrawnParticlesMax =
			Max(pGlobalInfo->m_nTotalDrawnParticlesMax, pSingleInfo->m_nTotalDrawnParticles);
...
void CParticleMgr::StatsSpewResults()
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		if(pParticleInfo->m_nTotalActiveParticles > 0)
		{
			Msg("%38s\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t%d\t%d\t%d\t\t%.1f\n", ProfilingHistogram.String(i),
				pParticleInfo->m_nCount / Profiling_nFrames, pParticleInfo->m_nTotalActiveParticles / Profiling_nFrames,
				pParticleInfo->m_nTotalDrawnParticles / Profiling_nFrames,
				pParticleInfo->m_nChildCount / pParticleInfo->m_nCount, pParticleInfo->m_nCountMax,
				pParticleInfo->m_nTotalActiveParticlesMax, pParticleInfo->m_nTotalDrawnParticlesMax,
	...
	g_pFullFileSystem->FPrintf(fh, "Name, Systems, Particles active, Particles drawn, Av Children, Max systems, Max "
								   "particles active, Max particles drawn, Max children, Max draw distance\n");
	for(int i = 0; i < nCount; ++i)
	{
		ParticleInfo_t *pParticleInfo = &(ProfilingHistogram[i]);
		if(pParticleInfo->m_nTotalActiveParticles > 0)
