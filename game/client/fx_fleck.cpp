...
//{
// public:
//	bool MergeParticleSystems( CFleckParticles *pSystem, const char *pEffectName, const Vector &center, const Vector
//&extents ) 	{ merge; return true; }
//};
...
template<class PARTICLE_EFFECT, class PARTICLE_MERGE>
PARTICLE_EFFECT *CParticleMergeList<PARTICLE_EFFECT, PARTICLE_MERGE>::FindAndMergeParticleSystem(
...
void CFleckParticles::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		// Render it
		RenderParticle_ColorSizeAngle(pIterator->GetParticleDraw(), tPos, color,
									  1.0f - (pParticle->m_flLifetime / pParticle->m_flDieTime), pParticle->m_uchSize,
									  pParticle->m_flRoll);
