...
CBorderVisualizerPanel::CBorderVisualizerPanel(Panel *pParent, const char *pName, IBorder *pBorder)
	: Panel(pParent, pName), m_pBorder(pBorder)
{
...
CColorVisualizerPanel::CColorVisualizerPanel(Panel *pParent, const char *pName, const Color &color)
	: Panel(pParent, pName), m_Color(color)
{
}
...
void CSchemeVisualizer::PerformLayout()
{
	...
	m_pListDataTypeCombo->SetBounds(GetWide() - nComboWidth - XRES(10), YRES(2), nComboWidth, YRES(8));
...
void CSchemeVisualizer::AddFontsToList()
{
	...
	CFmtStr fmtText("ABCDEFGHIJKLMN%sPQRSTUVWXYZ%sabcdefhijklmnopqrstuvwxyz0123456789!@#$%%^&*()-_=+", strOAccent,
					strSkull);
...
CON_COMMAND(showschemevisualizer,
			"Show borders, fonts and colors for a particular scheme.  The default is ClientScheme.res")
{
	...
	g_pSchemeVisualizer->SetBounds((ScreenWidth() - nWidth) / 2, (ScreenHeight() - nHeight) / 2, nWidth, nHeight);
