...
void CBoneMergeCache::UpdateCache()
{
	...
	if(pTestFollow != m_pFollow || pTestHdr != m_pFollowHdr || pTestStudioHDR != m_pFollowRenderHdr ||
	   pOwnerHdr != m_pOwnerHdr)
	{
		m_MergedBones.Purge();
		m_BoneMergeBits.Purge();
		...
		if(pTestFollow && pTestHdr && pOwnerHdr)
		{
			...
			for(int i = 0; i < m_pOwnerHdr->numbones(); i++)
			{
				...
				if((m_pFollowHdr->boneFlags(parentBoneIndex) & BONE_USED_BY_BONE_MERGE) == 0)
				{
					...
					//					Warning("Performance warning: Merge with '%s'. Mark bone '%s' in model '%s' as being
					//used by bone merge in the .qc!\n", 						pOwnerHdr->pszName(), m_pFollowHdr->pBone( parentBoneIndex
					//)->pszName(), m_pFollowHdr->pszName() );
				}
			}
...
void CBoneMergeCache::MergeMatchingBones(int boneMask)
{
	...
	if(!bWorked)
	{
		...
		{
			// We actually want to see when Captain Canteen happened, and make it really obvious that (a) he was here
			// and (b) this code would have fixed him.
...
void CBoneMergeCache::CopyParentToChild(const Vector parentPos[], const Quaternion parentQ[], Vector childPos[],
										Quaternion childQ[], int boneMask)
{
	UpdateCache();
...
void CBoneMergeCache::CopyChildToParent(const Vector childPos[], const Quaternion childQ[], Vector parentPos[],
										Quaternion parentQ[], int boneMask)
{
	UpdateCache();
