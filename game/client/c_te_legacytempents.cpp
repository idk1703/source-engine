...
int C_LocalTempEntity::DrawStudioModel(int modelFlags)
{
	...
	{
		drawn = modelrender->DrawModel(modelFlags, this, MODEL_INSTANCE_INVALID, index, GetModel(), GetAbsOrigin(),
									   GetAbsAngles(), m_nSkin, m_nBody, m_nHitboxSet);
	}
	return drawn;
}
...
bool C_LocalTempEntity::Frame(float frametime, int framenumber)
{
	...
	else if(flags & FTENT_SINEWAVE)
	{
		...
		SetLocalOrigin(Vector(x + sin(m_vecTempEntVelocity[2] + gpGlobals->curtime /* * anim.prevframe */) *
									  (10 * m_flSpriteScale),
	...
	else if(flags & FTENT_SPIRAL)
	{
		...
		SetLocalOrigin(GetLocalOrigin() + Vector(m_vecTempEntVelocity[0] * frametime + 8 * sin(gpGlobals->curtime * 20),
	...
	if(flags & (FTENT_COLLIDEALL | FTENT_COLLIDEWORLD | FTENT_COLLIDEPROPS))
	{
		...
		if(flags & (FTENT_COLLIDEALL | FTENT_COLLIDEPROPS))
		{
			...
			if(bShouldCollide == true)
			{
				...
				if((trace.fraction != 1) &&
				   ((trace.DidHitWorld()) || (trace.m_pEnt != ClientEntityList().GetEnt(clientIndex))))
				{
		...
		if(traceFraction != 1) // Decent collision now, and damping works
		{
			...
			if(flags & (FTENT_GRAVITY | FTENT_SLOWGRAVITY))
			{
				...
				if(traceNormal[2] > 0.9) // Hit floor?
				{
					if(m_vecTempEntVelocity[2] <= 0 && m_vecTempEntVelocity[2] >= gravity * 3)
					{
						damp = 0; // Stop
						flags &=
							~(FTENT_ROTATE | FTENT_GRAVITY | FTENT_SLOWGRAVITY | FTENT_COLLIDEWORLD | FTENT_SMOKETRAIL);
			...
			{
				// Stop the emission of particles on collision - removed from the ClientEntityList on removal from the
				// tempent pool.
...
//			speed -
//-----------------------------------------------------------------------------
void CTempEnts::BubbleTrail(const Vector &start, const Vector &end, float flWaterZ, int modelIndex, int count,
							float speed)
{
	C_LocalTempEntity *pTemp;
	const model_t *model;
...
void CTempEnts::PhysicsProp(int modelindex, int skin, const Vector &pos, const QAngle &angles, const Vector &vel,
							int physFlags, int physEffects)
{
...
//			*pOwner -
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::ClientProjectile(const Vector &vecOrigin, const Vector &vecVelocity,
											   const Vector &vecAcceleration, int modelIndex, int lifetime,
											   CBaseEntity *pOwner, const char *pszImpactEffect,
...
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::TempSprite(const Vector &pos, const Vector &dir, float scale, int modelIndex,
										 int rendermode, int renderfx, float a, float life, int flags,
										 const Vector &normal)
{
	C_LocalTempEntity *pTemp;
	const model_t *model;
...
void CTempEnts::Sprite_Trail(const Vector &vecStart, const Vector &vecEnd, int modelIndex, int nCount, float flLife,
							 float flSize, float flAmplitude, int nRenderamt, float flSpeed)
{
	C_LocalTempEntity *pTemp;
	const model_t *pModel;
	int flFrameCount;
...
void CTempEnts::EjectBrass(const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type)
{
	...
	pTemp->SetVelocity(Vector(dir[0] + random->RandomFloat(-64, 64), dir[1] + random->RandomFloat(-64, 64),
...
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::SpawnTempModel(const model_t *pModel, const Vector &vecOrigin, const QAngle &vecAngles,
											 const Vector &vecVelocity, float flLifeTime, int iFlags)
{
	Assert(pModel);
...
//			type -
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash(const Vector &pos1, const QAngle &angles, int type, ClientEntityHandle_t hEntity,
							bool firstPerson)
{
#ifdef CSTRIKE_DLL
...
C_LocalTempEntity *CTempEnts::TempEntAlloc(const Vector &org, const model_t *model)
{
	...
	if(CommandLine()->CheckParm("-tools") != NULL)
	{
		...
		if(first)
		{
			Msg("Currently not recording tempents, since recording them as entites causes them to be deleted as "
				"entities, even though they were allocated through the tempent pool. (crash)\n");
			first = false;
		}
#endif
		//		ClientEntityList().AddNonNetworkableEntity(	pTemp );
...
inline void CTempEnts::CacheMuzzleFlashes(void)
{
	...
	for(i = 0; i < 2; i++)
	{
		...
		{
			m_Material_Combine_MuzzleFlash_Player[i] =
				ParticleMgr()->GetPMaterial(VarArgs("effects/combinemuzzle%d_noz", i + 1));
		}
	}
	...
	for(i = 0; i < 2; i++)
	{
		...
		{
			m_Material_Combine_MuzzleFlash_NPC[i] =
				ParticleMgr()->GetPMaterial(VarArgs("effects/combinemuzzle%d", i + 1));
		}
	}
}
...
void CTempEnts::MuzzleFlash_Combine_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	CSmartPtr<CLocalSpaceEmitter> pSimple =
		CLocalSpaceEmitter::Create("MuzzleFlash", hEntity, attachmentIndex, FLE_VIEWMODEL);
	...
	for(int i = 1; i < 6; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(
			sizeof(SimpleParticle), m_Material_Combine_MuzzleFlash_Player[random->RandomInt(0, 1)], offset);
	...
	// Tack on the smoke
	pParticle = (SimpleParticle *)pSimple->AddParticle(
		sizeof(SimpleParticle), m_Material_Combine_MuzzleFlash_Player[random->RandomInt(0, 1)], vec3_origin);
...
void CTempEnts::MuzzleFlash_Combine_NPC(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	CSmartPtr<CLocalSpaceEmitter> pSimple =
		CLocalSpaceEmitter::Create("MuzzleFlash_Combine_NPC", hEntity, attachmentIndex);
...
#define FRONT_LENGTH 6
	...
	for(int i = 1; i < FRONT_LENGTH; i++)
	{
		...
		pParticle->m_uchStartSize =
			((random->RandomFloat(6.0f, 8.0f) * (FRONT_LENGTH * 1.25f - (i)) / (FRONT_LENGTH)) * flScale);
		pParticle->m_uchEndSize = pParticle->m_uchStartSize;
		pParticle->m_flRoll = random->RandomInt(0, 360);
		pParticle->m_flRollDelta = 0.0f;
	}
...
#define SIDE_LENGTH 6
	...
	for(int i = 1; i < SIDE_LENGTH; i++)
	{
		...
		pParticle->m_uchStartSize =
			((random->RandomFloat(2.0f, 4.0f) * (SIDE_LENGTH - (i)) / (SIDE_LENGTH * 0.5f)) * flScale);
		pParticle->m_uchEndSize = pParticle->m_uchStartSize;
		pParticle->m_flRoll = random->RandomInt(0, 360);
		pParticle->m_flRollDelta = 0.0f;
	}
	...
	pParticle =
		(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), g_Mat_Combine_Muzzleflash[2], vec3_origin);
	if(pParticle == NULL)
		return;
...
void CTempEnts::MuzzleFlash_SMG1_Player(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	CSmartPtr<CLocalSpaceEmitter> pSimple =
		CLocalSpaceEmitter::Create("MuzzleFlash_SMG1_Player", hEntity, attachmentIndex, FLE_VIEWMODEL);
	...
	for(int i = 1; i < 6; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(
			sizeof(SimpleParticle), m_Material_MuzzleFlash_Player[random->RandomInt(0, 3)], offset);
...
void CTempEnts::MuzzleFlash_Shotgun_NPC(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	if(random->RandomInt(0, 2) == 0)
	{
		...
		{
			pParticle =
				(SimpleParticle *)pEmbers->AddParticle(sizeof(SimpleParticle), g_Mat_SMG_Muzzleflash[0], origin);
...
void CTempEnts::CSEjectBrass(const Vector &vecPosition, const QAngle &angVelocity, int nVelocity, int shellType,
							 CBasePlayer *pShooter)
{
	...
	velocity = forward * nVelocity * random->RandomFloat(1.2, 2.8) + up * random->RandomFloat(-10, 10) +
	...
	pTemp->m_nBody = 0;
	pTemp->flags =
		FTENT_FADEOUT | FTENT_GRAVITY | FTENT_COLLIDEALL | FTENT_HITSOUND | FTENT_ROTATE | FTENT_CHANGERENDERONCOLLIDE;
