...
BEGIN_RECV_TABLE_NOBASE(C_TestTraceline, DT_TestTraceline)
	RecvPropInt(RECVINFO(m_clrRender)), RecvPropVector(RECVINFO_NAME(m_vecNetworkOrigin, m_vecOrigin)),
...
void C_TestTraceline::DrawCube(Vector &center, unsigned char *pColor)
{
	...
	for(int nFace = 0; nFace < 6; nFace++)
	{
		...
		meshBuilder.DrawQuad(pMesh, facePoints[nP1].Base(), facePoints[nP2].Base(), facePoints[nP3].Base(),
							 facePoints[nP4].Base(), pColor, true);
	}
}
