...
C_Hairball::C_Hairball()
{
	...
	for(int i = 0; i < m_HairPositions.Count(); i++)
	{
		...
		m_HairPositions[i].Init(cos(theta) * cosPhi * m_flSphereRadius, sin(theta) * cosPhi * m_flSphereRadius,
	...
	if(m_flSitStillTime > 0)
	{
		...
		if(m_flSitStillTime <= 0)
		{
			...
			for(int i = 0; i < 15; i++)
			{
				...
				trace_t trace;
				UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vDir * 10000, MASK_SOLID, NULL, COLLISION_GROUP_NONE,
							   &trace);
