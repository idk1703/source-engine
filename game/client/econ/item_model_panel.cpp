...
ConVar tf_time_loading_item_panels("tf_time_loading_item_panels", "0.0005", FCVAR_ARCHIVE,
								   "The time to spend per frame loading data for item panels");
#ifdef STAGING_ONLY
...
const char *g_ItemModelPanelRenderTargetNames[] = {"_rt_ItemModelPanel0", "_rt_ItemModelPanel1", "_rt_ItemModelPanel2"};
...
void CItemMaterialCustomizationIconPanel::DrawQuad(int iSubtileIndex, int iSubtileCount)
{
	...
	float fXScale = 1.0f / (float)iSubtileCount, fXOffsetL = (float)iSubtileIndex * fXScale,
		  fXOffsetR = (float)(iSubtileIndex + 1) * fXScale, fXUpperLowerOffset = fXScale * 0.65f;
...
void CEmbeddedItemModelPanel::SetItem(CEconItemView *pItem)
{
	...
	if(FindAttribute_UnsafeBitwiseCast<attrib_value_t>(m_pItem, pAttrib_UseModelCacheIcon, &flUseCacheIcon) &&
	   flUseCacheIcon != 0.f)
	{
		...
		if(!m_pItem->GetInventoryImage() || IsForcingModelUsage() || m_bWeaponAllowInspect || UseRenderTargetAsIcon())
		{
			...
			if(pszModelName)
			{
				...
				// Attach the models for the item
				{
					...
					{
						const int iNumAttachedModels =
							m_pItem->GetItemDefinition()->GetNumAttachedModelsFestivized(iTeam);
						for(int i = 0; i < iNumAttachedModels; ++i)
						{
							attachedmodel_t *pModel =
								m_pItem->GetItemDefinition()->GetAttachedModelDataFestivized(iTeam, i);
							LoadAttachedModel(pModel);
						}
					}
				}
				...
				if(m_pItem->FindAttribute(pAttr_module, &attrModule) && attrModule.has_value())
				{
					...
					if(!bIsStrange)
					{
						// Go over the attributes of the item, if it has any strange attributes the item is strange and
						// don't apply
				...
				int iTeam = GetLocalPlayerTeam(), iSkin = iTeam;
...
bool CEmbeddedItemModelPanel::IsLoadingWeaponSkin(void) const
{
	...
	if(m_pItem && m_pItem->IsValid())
	{
		...
		{
			return m_pItem->GetWeaponSkinBaseCompositor() != NULL || !m_pCachedWeaponIcon ||
				   !m_pCachedWeaponIcon->GetTexture();
		}
...
IMaterial *GetMaterialForImage(CEmbeddedItemModelPanel::InventoryImageType_t eImageType, const char *pszBaseName)
{
	...
	switch(eImageType)
	{
		...
		case CEmbeddedItemModelPanel::IMAGETYPE_DETAILED:
			pMaterial =
				g_pMaterialSystem->FindMaterial(CFmtStr("%s_detail", pszBaseName).Access(), TEXTURE_GROUP_VGUI, false);
			break;
...
void CEmbeddedItemModelPanel::Paint(void)
{
	...
	if(!bIsLoadingWeaponSkin && !m_bForceUseModel)
	{
		...
		if(m_pCachedWeaponIcon && m_pCachedWeaponIcon->GetTexture())
		{
			...
			if(!m_pCachedWeaponMaterial && g_pMaterialSystem)
			{
				...
				IMaterial *pMaterial =
					g_pMaterialSystem->FindProceduralMaterial(pszTextureName, TEXTURE_GROUP_VGUI, pVMTKeyValues);
			...
			if(m_iCachedTextureID == -1)
			{
				// m_pCachedWeaponIcon->GetTexture()->SaveToFile( CFmtStr( "%d_weapon_skin_cache.tga",
				// m_pItem->GetItemDefIndex() ) );
		...
		else if(pszInventoryImage)
		{
			...
			IMaterial *pMaterial = GetMaterialForImage(
				(CEmbeddedItemModelPanel::InventoryImageType_t)m_iInventoryImageType, pszInventoryImage);
	...
	if(iTexture != -1)
	{
		...
		for(int i = 0; i < m_pItem->GetInventoryOverlayImageCount(); i++)
		{
			...
			IMaterial *pOverlayMaterial = GetMaterialForImage(
				(CEmbeddedItemModelPanel::InventoryImageType_t)m_iInventoryImageType, pszInventoryOverlayImage);
			...
			Color col;
			col.SetColor(clamp((iRGB & 0xFF0000) >> 16, 0, 255), clamp((iRGB & 0xFF00) >> 8, 0, 255),
						 clamp((iRGB & 0xFF), 0, 255), 255);
			...
			surface()->DrawTexturedSubRect(x, y, x + iWidth, y + iHeight, flTexX, flTexY, flTexX + flTexW,
										   flTexY + flTexH);
		}
		...
		{
			IMaterial *pToolTargetItemMaterial = GetMaterialForImage(
				(CEmbeddedItemModelPanel::InventoryImageType_t)m_iInventoryImageType, m_pszToolTargetItemImage);
			...
			if(m_iToolTargetItemTextureID == -1)
			{
				...
				if(m_pItem->FindAttribute(pAttrDef_ToolTargetItemIconOffset, &attrToolTargetItemIconOffset) &&
				   attrToolTargetItemIconOffset.has_value())
				{
					UTIL_StringToVector(m_vecToolTargetItemImageOffset.Base(),
										attrToolTargetItemIconOffset.value().c_str());
				}
			}
			...
			surface()->DrawTexturedSubRect(iStrangeX, iStrangeY, iStrangeX + (iWidth * flScale),
										   iStrangeY + (iHeight * flScale), flTexX, flTexY, flTexX + (flTexW),
										   flTexY + (flTexH));
		}
	...
	for(int i = 0; i < ITEM_MODEL_IMAGE_CACHE_SIZE; ++i)
	{
		...
		if(pLockPanel == NULL)
		{
			pszCacheRenderTargetName =
				m_bOfflineIconGeneration ? "offline_icon_generation" : g_ItemModelPanelRenderTargetNames[i];
	...
	m_pItem->SetWeaponSkinBaseCreateFlags(TEX_COMPOSITE_CREATE_FLAGS_NO_COMPRESSION |
										  TEX_COMPOSITE_CREATE_FLAGS_NO_MIPMAPS);
	...
	if(!m_bForceUseModel && (UseRenderTargetAsIcon() || bDrawWeaponWithSkin) && !m_pCachedWeaponIcon)
	{
		...
		V_sprintf_safe(buffer, "proc/icon/item%d_id%lld_w%d_h%d", m_pItem->GetItemDefIndex(), m_pItem->GetID(), iWidth,
					   iHeight);
		...
		else
		{
			...
			ITexture *pRenderTarget =
				g_pMaterialSystem->FindTexture(pszCacheRenderTargetName, TEXTURE_GROUP_RENDER_TARGET);
			if(pRenderTarget)
			{
				pRenderContext->AsyncCreateTextureFromRenderTarget(pRenderTarget, buffer, IMAGE_FORMAT_RGBA8888, false,
																   0, m_pCachedWeaponIcon, NULL);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEmbeddedItemModelPanel::UpdateParticle(IMatRenderContext *pRenderContext, CStudioHdr *pStudioHdr,
...
bool CEmbeddedItemModelPanel::RenderAttachedModels(CStudioHdr *pStudioHdr, matrix3x4_t *pWorldMatrix)
{
	...
	FOR_EACH_VEC(m_AttachedModels, iModel)
	{
		...
		if(pAttachedStudioHdr != NULL)
		{
			...
			m_AttachedModels[iModel].m_MDL.SetupBonesWithBoneMerge(&mergeHdr, pMergeBoneToWorld, pStudioHdr,
																   pWorldMatrix, m_AttachedModels[iModel].m_MDLToWorld);
...
// Purpose:
//-----------------------------------------------------------------------------
void CEmbeddedItemModelPanel::RenderingRootModel(IMatRenderContext *pRenderContext, CStudioHdr *pStudioHdr,
												 MDLHandle_t mdlHandle, matrix3x4_t *pWorldMatrix)
{
	// No model?  Bail
...
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
	bool bCollectionMouseover =
		(m_bIsMouseOverPanel && GetItem() && GetItem()->GetItemDefinition()->GetItemCollectionDefinition());
	if(bCollectionMouseover)
	{
		float flInspect = 0;
		static CSchemaAttributeDefHandle pAttrib_WeaponAllowInspect("weapon_allow_inspect");
		if(FindAttribute_UnsafeBitwiseCast<attrib_value_t>(GetItem(), pAttrib_WeaponAllowInspect, &flInspect) &&
		   flInspect != 0.f)
		{
...
void CItemModelPanel::PerformLayout(void)
{
	...
	else if(m_pItemNameLabel && m_pItemAttribLabel && !m_bModelOnly)
	{
		...
		if(m_bTextCenterX)
		{
			...
			m_pItemAttribLabel->SetPos((w - m_pItemAttribLabel->GetWide()) * 0.5f,
									   m_iTextYPos + iNameTall + iLabelOffset);
		}
		else if(m_iTextYPos)
		{
		...
		if(m_bResizeToText)
		{
			if(m_bIsMouseOverPanel && GetItem() && GetItem()->GetItemDefinition()->GetItemCollectionDefinition() &&
			   !m_bHideCollectionPanel)
			{
...
void CItemModelPanel::ResizeLabels(void)
{
	...
	else
	{
		...
		if(!m_bResizeToText)
		{
			...
			if(m_pItemNameLabel->GetWide() > iTextW || (!m_bNameOnly && m_pItemAttribLabel->GetWide() > iTextW) ||
			   (iTotalH > h))
			{
				m_pItemNameLabel->SetFont(m_pFontNameSmall);
				m_pItemAttribLabel->SetFont(m_pFontAttribSmall);
	...
	if(m_pItemEquippedLabel)
	{
		m_pItemEquippedLabel->SetPos(GetWide() - m_pItemEquippedLabel->GetWide() - m_iEquippedInsetX,
									 GetTall() - m_pItemEquippedLabel->GetTall() - m_iEquippedInsetY);
	}
}
	...
	if(pItem && pItem->IsValid())
	{
		if(m_ItemData.IsValid())
		{
			...
			else if(pItem->GetItemID() == INVALID_ITEM_ID)
			{
				...
				// Our current item is a base item. Our new item needs to be base too, and match item indices and
				// quality
				bMatch &= (m_ItemData.GetItemDefIndex() == pItem->GetItemDefIndex()) &&
		...
		if(bMatch)
		{
			...
			for(int i = 0; i < GetKillEaterAttrCount(); i++)
			{
				...
				float flOldType = 0.f;
				bNewFoundAttr =
					FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pItem, pKillEaterAltScoreTypeAttrDef, &flNewType);
				bOldFoundAttr = FindAttribute_UnsafeBitwiseCast<attrib_value_t>(
					&m_ItemData, pKillEaterAltScoreTypeAttrDef, &flOldType);
...
void CItemModelPanel::ShowContainedItemPanel(const CEconItemView *pItem)
{
	...
	if(pItem->GetSOCData() && pItem->GetSOCData()->GetInteriorItem() && m_pContainedItemPanel)
	{
		...
		const IEconTool *pEconTool = pItem->GetItemDefinition() ? pItem->GetItemDefinition()->GetEconTool() : NULL;
		if(!pEconTool)
			return;
...
{
	// hijacking m_bShowQuantity here to indicate "show things where the quantity counter goes", in this case meaning
	// "show crate series indicator"
	if(m_pSeriesLabel && m_bShowQuantity)
	{
		...
		float fCrateSeries;
		if(pAttrDef_CrateSeries &&
		   FindAttribute_UnsafeBitwiseCast<attrib_value_t>(&m_ItemData, pAttrDef_CrateSeries, &fCrateSeries) &&
		   pAttrDef_HideSeries && !m_ItemData.FindAttribute(pAttrDef_HideSeries))
		{
...
bool CItemModelPanel::CheckRecipeMatches()
{
	...
	if(!pTool || V_stricmp(m_ItemData.GetStaticData()->GetEconTool()->GetTypeName(), "dynamic_recipe") ||
	...
	if(m_pMatchesLabel && m_bShowQuantity)
	{
		...
		if(se_CurrentLoadingTask == LOADING_RECIPE_MATCHES)
		{
			...
			while(m_nRecipeMatchingIndex < pLocalInv->GetItemCount() &&
				  sm_flLoadingTimeThisFrame < tf_time_loading_item_panels.GetFloat())
			{
				// Mark this time
				...
				const CUtlVector<const CEconItemAttributeDefinition *> &matchingAttribs =
...
void CItemModelPanel::UpdateDescription()
{
	...
	if(!m_bNameOnly)
	{
		...
		if(pDescription)
		{
			...
			for(unsigned int i = 0; i < pDescription->GetLineCount(); i++)
			{
				...
				{
					V_wcscat_safe(wszCollectionListBuffer,
								  unWrittenCollectionLines++ == 0
									  ? L""
									  : L"\n"); // add empty lines everywhere except before the first line
	...
	if(m_pItemAttribLabel && !m_bNameOnly && m_pItemAttribLabel->GetTextImage() && m_ItemData.GetDescription())
	{
		...
		vgui::TextImage *pCollectionNameTextImage =
			m_pItemCollectionNameLabel ? m_pItemCollectionNameLabel->GetTextImage() : NULL;
		if(pCollectionNameTextImage)
			pCollectionNameTextImage->ClearColorChangeStream();

		vgui::TextImage *pCollectionListTextImage =
			m_pItemCollectionListLabel ? m_pItemCollectionListLabel->GetTextImage() : NULL;
		...
		for(unsigned int i = 0; i < pDescription->GetLineCount(); i++)
		{
			...
			if((line.unMetaType &
				(kDescLineFlag_Collection | kDescLineFlag_CollectionName | kDescLineFlag_CollectionCurrentItem)) != 0 &&
			   pCollectionNameTextImage && pCollectionListTextImage)
			{
				...
				unsigned int &unCurrentCollectionTextStreamIndex =
					bIsCollectionName ? unCurrentCollectionNameTextStreamIndex : unCurrentCollectionListTextStreamIndex;
				...
				Color col = bIsCurrentItem
								? Color(0, 0, 0, 255)
								: pScheme->GetColor(GetColorNameForAttribColor(line.eColor), Color(255, 255, 255, 255));
				...
				unCurrentCollectionTextStreamIndex +=
					StringFuncs<locchar_t>::Length(line.sText.Get()) + 1; // add one character to deal with newlines
	...
	if(pwszAttribs)
	{
		...
		if(m_pItemAttribLabel && m_pItemAttribLabel->GetTextImage())
		{
			...
			if(iNegAttribsBegin)
			{
				...
				{
					Color col =
						pScheme->GetColor(pColorDef_DescAttribPositive->GetColorName(), Color(255, 255, 255, 255));
				...
				{
					Color col =
						pScheme->GetColor(pColorDef_DescAttribNegative->GetColorName(), Color(255, 255, 255, 255));
...
void CItemModelPanel::UpdatePanels(void)
{
	...
	if(m_pPaintIcon)
	{
		...
		if(!m_bHideModel && !m_bHidePaintIcon)
		{
			...
			int iRGB0 = m_ItemData.GetModifiedRGBValue(false), iRGB1 = m_ItemData.GetModifiedRGBValue(true);

			if(!bIsEconTool && (iRGB0 != 0 || iRGB1 != 0))
			{
				m_pPaintIcon->SetVisible(true);
				m_pPaintIcon->m_colPaintColors.AddToTail(Color(clamp((iRGB0 & 0xFF0000) >> 16, 0, 255),
															   clamp((iRGB0 & 0xFF00) >> 8, 0, 255),
															   clamp((iRGB0 & 0xFF), 0, 255), 255));
				if(iRGB0 != iRGB1)
				{
					m_pPaintIcon->m_colPaintColors.AddToTail(Color(clamp((iRGB1 & 0xFF0000) >> 16, 0, 255),
																   clamp((iRGB1 & 0xFF00) >> 8, 0, 255),
																   clamp((iRGB1 & 0xFF), 0, 255), 255));
				}
			}
		}
	}
		...
		switch(nVisionFilterFlags)
		{
			...
			case 2:
				...
				if(TFGameRules() ? TFGameRules()->IsHolidayActive(kHoliday_HalloweenOrFullMoon)
								 : TF_IsHolidayActive(kHoliday_HalloweenOrFullMoon))
				{
	...
	if(m_pIsStrangeImage)
	{
		...
		if(!m_bIsMouseOverPanel)
		{
			...
			if(!bIsStrange)
			{
				// Go over the attributes of the item, if it has any strange attributes the item is strange and don't
				// apply
				for(int i = 0; i < GetKillEaterAttrCount(); i++)
				{
...
bool CItemModelPanel::LoadData()
{
	...
	if(!m_bHideModel && m_pModelPanel)
	{
		...
		if(bLoadingWeaponSkin || bLoadingBackpackIcon)
		{
			...
			if(sm_flLoadingTimeThisFrame < tf_time_loading_item_panels.GetFloat() &&
			   se_CurrentLoadingTask == LOADING_ICONS)
			{
				float flTime = Plat_FloatTime();
	...
	if(m_bDescriptionDirty && !IsContainedItem())
	{
		...
		if(sm_flLoadingTimeThisFrame < tf_time_loading_item_panels.GetFloat() &&
		   se_CurrentLoadingTask == LOADING_DESCRIPTIONS)
		{
			float flTime = Plat_FloatTime();
			// Update!
			UpdateDescription();
...
void CItemModelPanel::OnCommand(const char *command)
{
	if(FStrEq(command, "sellitem"))
	{
		if(HasItem() && steamapicontext && steamapicontext->SteamFriends() && steamapicontext->SteamUtils())
		{
			...
			char szURL[512];
			V_snprintf(szURL, sizeof(szURL), "http://%ssteamcommunity.com/my/inventory/?sellOnLoad=1#%d_%d_%llu",
					   pszPrefix, engine->GetAppID(), nAssetContext, GetItem()->GetItemID());
...
// Purpose:
//-----------------------------------------------------------------------------
void CItemModelPanelToolTip::GetPosition(itempanel_tooltippos_t iTooltipPosition, CItemModelPanel *pItemPanel,
										 int iItemX, int iItemY, int *iXPos, int *iYPos)
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CItemModelPanelToolTip::ValidatePosition(CItemModelPanel *pItemPanel, int iItemX, int iItemY, int *iXPos,
											  int *iYPos)
{
	...
	if(bSucceeded)
	{
		...
		bSucceeded = !(vecToolTipMin.x < vecItemMax.x && vecToolTipMax.x > vecItemMin.x &&
					   vecToolTipMin.y < vecItemMax.y && vecToolTipMax.y > vecItemMin.y);
	}
