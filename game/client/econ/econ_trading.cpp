...
const char *g_FriendRelationship[] = {"none"
									  "blocked",
									  "request_recipient",
									  "friend",
									  "request_initiator",
...
const char *g_RejectedReasons[] = {
	"accepted",			"declined",			 "vac_banned_initiator", "vac_banned_target", "target_already_trading",
	"trading_disabled", "user_not_logged_in"};

const char *g_ClosedReasons[] = {"traded",			 "canceled", "error",			"does_not_own_items",
								 "untradable_items", "no_items", "trading_disabled"};
...
ConVar cl_trading_show_requests_from("cl_trading_show_requests_from", "3", FCVAR_ARCHIVE,
									 "View trade requests from a certain group only.");
...
public:
	...
	virtual void Trigger()
	{
		CTFGenericConfirmDialog *pDialog =
			ShowConfirmDialog("#TF_Trading_JoinTitle", "#TF_Trading_JoinText", "#GameUI_OK", "#TF_Trading_JoinCancel",
							  &ConfirmJoinTradeSession);
	...
	static void ConfirmJoinTradeSession(bool bConfirmed, void *pContext)
	{
		...
		msg.Body().m_eResponse =
			bConfirmed ? k_EGCMsgInitiateTradeResponse_Accepted : k_EGCMsgInitiateTradeResponse_Declined;
		msg.Body().m_unTradeRequestID = pNotification->m_unTradeRequestID;
		GCClientSystem()->BSendMessage(msg);
		if(bConfirmed)
		{
			ShowWaitingDialog(new CTradingWaitDialog(), "#TF_Trading_WaitingForServer", true, false,
							  kTradeRequestLifetime);
		}
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		NotificationQueue_Add(new CTFTradeRequestNotification(msg.Body().m_ulOtherSteamID,
															  msg.Body().m_unTradeRequestID, playerName.Get()));
		return true;
	}

protected:
};
GC_REG_JOB(GCSDK::CGCClient, CGCTrading_InitiateTradeRequest, "CGCTrading_InitiateTradeRequest",
		   k_EMsgGCTrading_InitiateTradeRequest, GCSDK::k_EServerTypeGCClient);
...
CON_COMMAND(cl_trading_test, "Tests the trade ui notification.")
{
	...
	NotificationQueue_Add(
		new CTFTradeRequestNotification(steamID.ConvertToUint64(), steamID.ConvertToUint64(), "Biff"));
}
#endif
#endif // _DEBUG
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		switch(eResponse)
		{
...
#ifdef TF_CLIENT_DLL
				C_CTF_GameStats.Event_Trading(IE_TRADING_REQUEST_ACCEPTED, iTradeRequests,
											  g_RejectedReasons[msg.Body().m_eResponse]);
#endif
			...
			case k_EGCMsgInitiateTradeResponse_SteamGuardDuration:
			{
				...
				kvTokens->SetWString(
					"days", L"15"); // Ideally this would come from the GC, which would get the value from Steam
				ShowMessageBox("#TF_Trading_StatusTitle", g_pszTradeResponseDescLocKeys[eResponse], kvTokens,
							   "#GameUI_OK");
				break;
			}
...
#ifdef TF_CLIENT_DLL
		C_CTF_GameStats.Event_Trading(IE_TRADING_REQUEST_REJECTED, iTradeRequests,
									  g_RejectedReasons[msg.Body().m_eResponse]);
#endif
...
GC_REG_JOB(GCSDK::CGCClient, CGCTrading_InitiateTradeResponse, "CGCTrading_InitiateTradeResponse",
		   k_EMsgGCTrading_InitiateTradeResponse, GCSDK::k_EServerTypeGCClient);
...
GC_REG_JOB(GCSDK::CGCClient, CGCTrading_StartSession, "CGCTrading_StartSession", k_EMsgGCTrading_StartSession,
	...
	if(bSent)
	{
		...
		if(pPlayerName != NULL && FStrEq(pPlayerName, "") == false)
		{
			...
			g_pVGuiLocalize->ConstructString_safe(
				wszConstructedString, g_pVGuiLocalize->Find("#TF_Trading_WaitingForPartyB_Named"), 1, wszPlayerName);
...
void Trading_SendGift(const CSteamID &steamID, const CEconItemView &giftItem)
{
	...
	steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(
		CFmtStrMax("%s/trade/1/sendgift/?appid=%d&contextid=%d&assetid=%llu&steamid_target=%llu", GetCommunityURL(),
				   engine->GetAppID(),
				   2, // k_EEconContextBackpack
				   giftItem.GetItemID(), steamID.ConvertToUint64()));
}
