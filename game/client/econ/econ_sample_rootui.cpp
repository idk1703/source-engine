...
void CEconSampleRootUI::OnCommand(const char *command)
{
	if(!Q_stricmp(command, "close"))
	{
		...
		if(engine->IsInGame())
		{
			...
			{
				// Check to make sure the player has room for all his items. If not, bring up the discard panel.
				// Otherwise, go away. We need to do this to catch players who used the "Change Loadout" button in the
				// pickup panel, and may be out of room.
