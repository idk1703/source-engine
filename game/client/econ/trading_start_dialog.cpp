...
void CTradingStartDialog::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pURLFailLabel =
		dynamic_cast<vgui::Label *>(m_pStatePanels[TDS_SELECTING_FROM_PROFILE]->FindChildByName("URLFailLabel"));
	m_pURLSearchingLabel =
		dynamic_cast<vgui::Label *>(m_pStatePanels[TDS_SELECTING_FROM_PROFILE]->FindChildByName("URLSearchingLabel"));
...
void CTradingStartDialog::OnCommand(const char *command)
{
	...
	{
		vgui::TextEntry *pEntry =
			dynamic_cast<vgui::TextEntry *>(m_pStatePanels[m_iCurrentState]->FindChildByName("URLEntry"));
		if(pEntry)
		{
...
bool CTradingStartDialog::ExtractSteamIDFromURL(char *inputURL)
{
	...
	const char *pszProfilePrepend = (localUniverse == k_EUniversePublic) ? "http://steamcommunity.com/profiles/"
	...
	else
	{
		...
		const char *pszIDPrepend = (localUniverse == k_EUniversePublic) ? "http://steamcommunity.com/id/"
																		: "http://beta.steamcommunity.com/id/";
...
GC_REG_JOB(GCSDK::CGCClient, CGCLookupAccountResponse, "CGCLookupAccountResponse", k_EMsgGCLookupAccountResponse,
