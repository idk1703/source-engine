...
// Purpose:
//-----------------------------------------------------------------------------
CStoreItemControlsPanel::CStoreItemControlsPanel(vgui::Panel *pParent, const char *pPanelName,
												 CItemModelPanel *pItemModelPanel)
...
void CStoreItemControlsPanel::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	LoadControlSettings(ShouldUseNewStore() ? "Resource/UI/econ/store/v2/StoreItemControls.res"
...
void CStorePricePanel::PerformLayout()
{
	...
	if(m_pPrice && m_pDiscount && m_pOGPrice)
	{
		...
		m_pCrossout->SetBounds(aOGPricePos[0] + m_pOGPrice->GetWide() - contentWidth,
	...
	{
		int iPosX, iPosY;
		Panel *pRefPanel = (m_pSale && m_pSale->IsVisible()) ? m_pSale
						   : (m_pNew && m_pNew->IsVisible()) ? m_pNew
															 : NULL;
...
void CStorePricePanel::SetPriceText(int iPrice, const char *pVariable, const econ_store_entry_t *pEntry)
{
	...
	if(pEntry->m_bIsMarketItem)
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wzMarketString, LOCCHAR("%s1 %s2"), 2,
											  g_pVGuiLocalize->Find("#Store_Market"), wzLocalizedPrice);
...
void AddItemToCartHelper(const char *pszContext, const econ_store_entry_t *pEntry, ECartItemType eSelectedCartItemType)
{
	...
	// get a discount.
	ECartItemType eCartItemType = eSelectedCartItemType == kCartItem_Purchase && IsItemPreviewed(pEntry, eCurrency) &&
										  !pCart->ContainsItemDefinition(pEntry->GetItemDefinitionIndex())
...
// Purpose:
//-----------------------------------------------------------------------------
CStorePage::CStorePage(Panel *parent, const CEconStoreCategoryManager::StoreCategory_t *pPageData,
...
void CStorePage::PerformLayout(void)
{
	...
	FOR_EACH_VEC(m_vecItemPanels, i)
	{
		...
		pItemControlsPanel->SetPos(iX + m_iItemControlsXOffset,
								   iY + iH - pItemControlsPanel->GetTall() - m_iItemControlsYOffset);
	}
	...
	if(m_pItemBackdropPanel && m_vecItemPanels.Count() >= 2)
	{
		...
		if(pTopLeftPanel && pBottomRightPanel)
		{
			...
			aItemBackdropBounds[2] =
				nX + pBottomRightPanel->GetWide() + m_iItemBackdropRightMargin - aItemBackdropBounds[0];
			aItemBackdropBounds[3] =
				nY + pBottomRightPanel->GetTall() + m_iItemBackdropBottomMargin - aItemBackdropBounds[1];

			m_pItemBackdropPanel->SetBounds(aItemBackdropBounds[0], aItemBackdropBounds[1], aItemBackdropBounds[2],
											aItemBackdropBounds[3]);
	...
	if(IsHomePage())
	{
		const store_promotion_spend_for_free_item_t *pPromotion =
			EconUI()->GetStorePanel()->GetPriceSheet()->GetStorePromotion_SpendForFreeItem();
		...
		MakeMoneyString(wszPriceThreshold, ARRAYSIZE(wszPriceThreshold), iPriceThreshold,
...
void CStorePage::PositionItemPanel(CItemModelPanel *pPanel, int iIndex)
{
	...
	int iXPos =
		m_iItemXPos + (iCenter + m_iItemOffcenterX) + (iButtonX * pRealPanel->GetWide()) + (m_iItemXDelta * iButtonX);
...
void CStorePage::OnPageShow(void)
{
	...
	if(m_pPageData && !ShouldUseNewStore())
	{
		...
		CEconStoreCategoryManager::StoreCategory_t *pPageData =
			const_cast<CEconStoreCategoryManager::StoreCategory_t *>(m_pPageData);
...
void CStorePage::CreateItemPanels(void)
{
	...
	if(m_pPageData && m_vecItemPanels.Count() < iNumPanels)
	{
		for(int i = m_vecItemPanels.Count(); i < iNumPanels; i++)
		{
			...
			// and controls
			CStoreItemControlsPanel *pControlsPanel =
				vgui::SETUP_PANEL(new CStoreItemControlsPanel(this, "StoreItemControls", pPanel));
	...
	else if(!Q_strnicmp(command, "marketplace", 8))
	{
		if(steamapicontext && steamapicontext->SteamFriends())
		{
			steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(
				"http://steamcommunity.com/market/search?appid=440");
		}
		return;
	}
...
// Purpose:
//-----------------------------------------------------------------------------
/* static */ int CStorePage::ItemDisplayOrderSort_UseSortOverride(const econ_store_entry_t *const *ppA,
																  const econ_store_entry_t *const *ppB)
{
	...
	float flValue;
	int unSortKeyA = (pAttribDef_StoreSortOverride &&
					  FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pDefA, pAttribDef_StoreSortOverride, &flValue))
						 ? (int)flValue
						 : pDefA->GetDefinitionIndex(),
		unSortKeyB = (pAttribDef_StoreSortOverride &&
					  FindAttribute_UnsafeBitwiseCast<attrib_value_t>(pDefB, pAttribDef_StoreSortOverride, &flValue))
						 ? (int)flValue
...
void CStorePage::UpdateFilteredItems(void)
{
	...
	for(int i = 0; i < m_pPageData->m_vecEntries.Count(); i++)
	{
		const econ_store_entry_t *pEntry =
			EconUI()->GetStorePanel()->GetPriceSheet()->GetEntry(m_pPageData->m_vecEntries[i]);
...
void CStorePage::SetBorderForItem(CItemModelPanel *pItemPanel, bool bMouseOver)
{
	...
	if(pItemPanel->GetItem() && pPriceSheet)
	{
		...
		{
			pItemPanel->SetBorder(
				vgui::scheme()->GetIScheme(GetScheme())->GetBorder("StoreHighlightedBackgroundBorder"));
	...
	if(m_pSelectedPanel)
	{
		...
		if(pItem && pEntry)
		{
			...
			if(pItemData)
			{
				...
				MakeMoneyString(wzLocalizedPrice, ARRAYSIZE(wzLocalizedPrice), iPrice,
								EconUI()->GetStorePanel()->GetCurrency());
...
void CStorePage::UpdateBackpackLabel(void)
{
	...
	{
		g_pVGuiLocalize->ConstructString_safe(wszLocalized, g_pVGuiLocalize->Find("#Store_FreeBackpackSpace"), 1,
											  wszBackpackSlotCount);
	}
	else
	{
		...
		if(IsFreeTrialAccount())
		{
			...
			_snwprintf(wszUpgradeSlotCount, ARRAYSIZE(wszUpgradeSlotCount), L"%d",
					   DEFAULT_NUM_BACKPACK_SLOTS - DEFAULT_NUM_BACKPACK_SLOTS_FREE_TRIAL_ACCOUNT);
			...
			g_pVGuiLocalize->ConstructString_safe(
				wszLocalized, g_pVGuiLocalize->Find("#Store_FreeBackpackSpace_WithCartItems_WithUpgrade"), 3,
				wszBackpackSlotCount, wszCartCount, wszUpgradeSlotCount);
		}
		else
#endif // defined( TF_CLIENT_DLL )
		{
			...
			g_pVGuiLocalize->ConstructString_safe(wszLocalized,
												  g_pVGuiLocalize->Find("#Store_FreeBackpackSpace_WithCartItems"), 2,
												  wszBackpackSlotCount, wszCartCount);
		}
	}
	...
	if(m_pBackpackLabel)
	{
		...
		m_pBackpackLabel->SetFgColor(
			InventoryManager()->GetLocalInventory()->CanPurchaseItems(iItemsInCart) ? m_colBackpackOrg : clrTooMany);
	}
}
