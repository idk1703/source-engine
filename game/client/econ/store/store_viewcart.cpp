...
CStoreViewCartPanel::CStoreViewCartPanel(Panel *parent) : Frame(parent, "store_viewcart_panel")
{
	...
	m_pItemListContainerScroller =
		new vgui::ScrollableEditablePanel(m_pClientArea, m_pItemListContainer, "ItemListContainerScroller");
...
void CStoreViewCartPanel::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	LoadControlSettings(ShouldUseNewStore() ? "Resource/UI/econ/store/v2/StoreViewCartPanel.res"
	...
	m_pFeaturedItemImage =
		dynamic_cast<vgui::ImagePanel *>(m_pItemListContainer->FindChildByName("FeaturedItemSymbol"));
...
void CStoreViewCartPanel::PerformLayout(void)
{
	...
	if(m_pItemEntries.Count())
	{
		...
		m_pItemListContainer->SetSize(m_pItemListContainer->GetWide(),
									  (iTall * m_pItemEntries.Count()) + m_pPurchaseFooter->GetTall());
...
void CStoreViewCartPanel::UpdateCartItemList(void)
{
	...
	if(m_pItemEntries.Count() < iNumEntriesInCart)
	{
		...
		{
			CCartViewItemEntry *pPanel =
				vgui::SETUP_PANEL(new CCartViewItemEntry(m_pItemListContainer, VarArgs("itementry%d", i)));
	...
	MakeMoneyString(wzLocalizedPrice, ARRAYSIZE(wzLocalizedPrice), unTotalPrice,
