...
public:
	CStorePage(Panel *parent, const CEconStoreCategoryManager::StoreCategory_t *pPageData,
...
protected:
	...
	static int ItemDisplayOrderSort_UseSortOverride(const econ_store_entry_t *const *ppA,
...
protected:
	...
	bool m_bShouldDeletePreviewPanel; // Set to true by derived classes if the preview panel's panel should be deleted,
									  // which is necessary if its parent is NULL
	...
	CPanelAnimationVarAliasType(int, m_iItemBackdropRightMargin, "item_backdrop_right_margin", "20",
								"proportional_xpos");
	CPanelAnimationVarAliasType(int, m_iItemBackdropTopMargin, "item_backdrop_top_margin", "20", "proportional_ypos");
	CPanelAnimationVarAliasType(int, m_iItemBackdropBottomMargin, "item_backdrop_bottom_margin", "20",
								"proportional_ypos");
...
void AddItemToCartHelper(const char *pszContext, item_definition_index_t unItemDef,
						 ECartItemType eSelectedCartItemType);
