...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		if(msg.Body().m_eResponse == k_EGCMsgResponseOK)
		{
			...
			CConfirmItemPreviewDialog *dialog = vgui::SETUP_PANEL(
				new CConfirmItemPreviewDialog(pStorePanel->GetPropertySheet()->GetActivePage(), pPreviewItem));
			MakeModalAndBringToFront(dialog);
		}
		else
		{
#ifdef TF_CLIENT_DLL
			C_CTFGameStats::ImmediateWriteInterfaceEvent("store_preview_item_denied",
														 CFmtStr("%i", msg.Body().m_unItemDefIndex).Access());
#endif

			// We aren't allowed to preview an item right now.
			CTFMessageBoxDialog *pDialog = ShowMessageBox("#ItemPreview_PreviewStartFailedTitle",
														  "#ItemPreview_PreviewStartFailedText", "#GameUI_OK");
			RTime32 nextTime =
				msg.Body().m_timePreviewTime + EconUI()->GetStorePanel()->GetPriceSheet()->GetPreviewPeriod();
...
GC_REG_JOB(GCSDK::CGCClient, CGCItemPreviewStatusResponse, "CGCItemPreviewStatusResponse",
		   k_EMsgGCItemPreviewStatusResponse, GCSDK::k_EServerTypeGCClient);
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		if(msg.Body().m_eResponse == k_EGCMsgResponseOK)
		{
			...
			ShowMessageBox("#ItemPreview_PreviewStartedTitle", "#ItemPreview_PreviewStartedText", "#GameUI_OK",
						   OnPreviewItemConfirm);
		}
		else
		{
...
GC_REG_JOB(GCSDK::CGCClient, CGCItemPreviewRequestResponse, "CGCItemPreviewRequestResponse",
		   k_EMsgGCItemPreviewRequestResponse, GCSDK::k_EServerTypeGCClient);
...
CEconPreviewNotification::CEconPreviewNotification(uint64 ulSteamID, uint32 iItemDef) : CEconNotification()
{
...
void CEconPreviewExpiredNotification::Trigger()
{
	CTFGenericConfirmDialog *pDialog =
		ShowConfirmDialog("#TF_PreviewItem_Expired_Title", "#TF_PreviewItem_Expired_Text", "#TF_PreviewItem_BuyIt",
						  "#TF_PreviewItem_NotNow", &OpenStoreToItem);
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		CEconPreviewExpiredNotification *pNotification =
			new CEconPreviewExpiredNotification(msg.Hdr().m_ulSteamID, msg.Body().m_unItemDefIndex);
...
GC_REG_JOB(GCSDK::CGCClient, CGCItemPreviewExpireNotification, "CGCItemPreviewExpireNotification",
		   k_EMsgGCItemPreviewExpireNotification, GCSDK::k_EServerTypeGCClient);
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		CEconPreviewItemBoughtNotification *pNotification =
			new CEconPreviewItemBoughtNotification(msg.Hdr().client_steam_id(), msg.Body().item_def_index());
...
GC_REG_JOB(GCSDK::CGCClient, CGCItemPreviewItemBoughtNotification, "CGCItemPreviewItemBoughtNotification",
		   k_EMsgGCItemPreviewItemBoughtNotification, GCSDK::k_EServerTypeGCClient);