...
public:
	...
	virtual void Gamestats_ItemTransaction(int eventID, CEconItemView *item, const char *pszReason = NULL,
										   int iQuality = 0) = 0;
	virtual void Gamestats_Store(int eventID, CEconItemView *item = NULL, const char *panelName = NULL, int classId = 0,
								 const cart_item_t *in_cartItem = NULL, int in_checkoutAttempts = 0,
								 const char *storeError = NULL, int in_totalPrice = 0, int in_currencyCode = 0) = 0;
	...
	// The panel at which we want back to actually close the UI - defaults to the root panel - a negative value can be
	// passed in for class loadout panels
