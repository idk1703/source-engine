...
// Purpose:
//-----------------------------------------------------------------------------
CBaseToolUsageDialog::CBaseToolUsageDialog(vgui::Panel *parent, const char *panelName, CEconItemView *pTool,
...
void CBaseToolUsageDialog::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	if(m_pTitleLabel)
	{
		...
		{
			wchar_t wTemp[256];
			g_pVGuiLocalize->ConstructString_safe(wTemp, pszBaseString, 2, m_pToolModelPanel->GetItem()->GetItemName(),
												  m_pSubjectModelPanel->GetItem()->GetItemName());
	...
	// @note Tom Bui: because the children have already applied their scheme settings and/or performed their layout
	// before this dialog has had a chance to load its res file, we need to manually invalidate the layout of these
	// children to make sure that the settings are valid with respect to what the parent wants
...
void CBaseToolUsageDialog::PerformLayout()
{
	...
	// properly. 	if ( m_pItemPanelBeingMousedOver )
	// 	{
...
#ifdef TF_CLIENT_DLL
	const char *pSubjectBaseName =
		m_pSubjectModelPanel && m_pSubjectModelPanel->GetItem() && m_pSubjectModelPanel->GetItem()->GetItemDefinition()
...
void CBaseToolUsageDialog::OnKeyCodePressed(vgui::KeyCode code)
{
	...
	if(nButtonCode == KEY_XBUTTON_LEFT || nButtonCode == KEY_XSTICK1_LEFT || nButtonCode == KEY_XSTICK2_LEFT ||
	   nButtonCode == STEAMCONTROLLER_DPAD_LEFT || code == KEY_LEFT || nButtonCode == KEY_XBUTTON_RIGHT ||
	   nButtonCode == KEY_XSTICK1_RIGHT || nButtonCode == KEY_XSTICK2_RIGHT ||
	   nButtonCode == STEAMCONTROLLER_DPAD_RIGHT || code == KEY_RIGHT || nButtonCode == KEY_XBUTTON_UP ||
	   nButtonCode == KEY_XSTICK1_UP || nButtonCode == KEY_XSTICK2_UP || nButtonCode == STEAMCONTROLLER_DPAD_UP ||
	   code == KEY_UP || nButtonCode == KEY_XBUTTON_DOWN || nButtonCode == KEY_XSTICK1_DOWN ||
	   nButtonCode == KEY_XSTICK2_DOWN || nButtonCode == STEAMCONTROLLER_DPAD_DOWN || code == KEY_DOWN)
	{
...
public:
	CConfirmStrangeCountTransferApplicationDialog(vgui::Panel *pParent, CEconItemView *pTool,
												  CEconItemView *pToolSubject)
	...
	virtual void Apply(void)
	{
		...
		EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolModelPanel->GetItem(),
											"applied_strangecounttransfer",
											m_pToolModelPanel->GetItem()->GetItemDefIndex());
...
// Purpose:
//-----------------------------------------------------------------------------
void CEconTool_StrangeCountTransfer::OnClientApplyTool(CEconItemView *pTool, CEconItemView *pSubject,
													   vgui::Panel *pParent) const
{
	CConfirmStrangeCountTransferApplicationDialog *pDialog =
		vgui::SETUP_PANEL(new CConfirmStrangeCountTransferApplicationDialog(pParent, pTool, pSubject));
...
// Purpose:
//-----------------------------------------------------------------------------
CConfirmStrangePartApplicationDialog::CConfirmStrangePartApplicationDialog(vgui::Panel *pParent, CEconItemView *pTool,
																		   CEconItemView *pToolSubject)
...
void CConfirmStrangePartApplicationDialog::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	SetDialogVariable(
		...
		GLocalizationProvider()->Find(m_pSubjectModelPanel->GetItem()->GetItemDefinition()->GetItemBaseName()));
	SetDialogVariable("slot_singular_plural",
					  GLocalizationProvider()->Find(iRemainingStrangePartSlots == 1 ? "#Econ_FreeSlot_Singular"
...
void CConfirmStrangePartApplicationDialog::Apply()
{
	...
	EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolModelPanel->GetItem(), "applied_strange_part",
										m_pToolModelPanel->GetItem()->GetItemDefIndex());
...
{
	CConfirmStrangePartApplicationDialog *pDialog =
		vgui::SETUP_PANEL(new CConfirmStrangePartApplicationDialog(pParent, pTool, pSubject));
	MakeModalAndBringToFront(pDialog);
}
...
public:
	CConfirmStrangeRestrictionApplicationDialog(vgui::Panel *pParent, CEconItemView *pTool, CEconItemView *pToolSubject,
												int iStrangeSlot, const char *pszStatLocalizationToken);
...
// Purpose:
//-----------------------------------------------------------------------------
CConfirmStrangeRestrictionApplicationDialog::CConfirmStrangeRestrictionApplicationDialog(
	vgui::Panel *pParent, CEconItemView *pTool, CEconItemView *pToolSubject, int iStrangeSlot,
	const char *pszStatLocalizationToken)
...
void CConfirmStrangeRestrictionApplicationDialog::Apply()
{
	...
	EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolModelPanel->GetItem(), "applied_strange_restriction",
										m_pToolModelPanel->GetItem()->GetItemDefIndex());
...
private:
	virtual void PopulateComboBoxOptions()
	{
		...
		const CEconTool_StrangePartRestriction *pToolRestriction =
			m_ToolItem.GetItemDefinition()->GetTypedEconTool<CEconTool_StrangePartRestriction>();
		...
		{
			uint32 unScoreType;
			if(!GetItemSchema()->BCanStrangeFilterApplyToStrangeSlotInItem(pToolRestriction->GetRestrictionType(),
																		   pToolRestriction->GetRestrictionValue(),
																		   &m_SubjectItem, i, &unScoreType))
				continue;
			...
			GetComboBox()->AddItem(CConstructLocalizedString(pLocBase, GLocalizationProvider()->Find(pszTypeLocKey)),
								   pKeyValues);
		}
	...
	virtual void OnComboBoxApplication()
	{
		...
		CConfirmStrangeRestrictionApplicationDialog *pDialog =
			vgui::SETUP_PANEL(new CConfirmStrangeRestrictionApplicationDialog(
				GetParent(), &m_ToolItem, &m_SubjectItem, iIndex,
				pKVActiveUserData ? pKVActiveUserData->GetString("token", NULL) : NULL));
		MakeModalAndBringToFront(pDialog);
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CEconTool_StrangePartRestriction::OnClientApplyTool(CEconItemView *pTool, CEconItemView *pSubject,
														 vgui::Panel *pParent) const
{
	...
	CSelectStrangePartToRestrictDialog *pDialog =
		vgui::SETUP_PANEL(new CSelectStrangePartToRestrictDialog(pParent, pTool, pSubject));
	MakeModalAndBringToFront(pDialog);
}
...
public:
	CConfirmApplyStrangifierDialog(vgui::Panel *pParent, CEconItemView *pTool, CEconItemView *pToolSubject,
								   const char *pszPromptLocToken, const char *pszTransactionReason,
								   const char *pszUpdatingText = "")
	...
	virtual void ApplySchemeSettings(vgui::IScheme *pScheme)
	{
		...
		if(pTextLabel && m_pToolModelPanel)
		{
			...
			if(m_pToolModelPanel->GetItem() && !m_pToolModelPanel->GetItem()->IsTradable() && m_pSubjectModelPanel &&
			   m_pSubjectModelPanel->GetItem())
			{
				...
				g_pVGuiLocalize->ConstructString_safe(wTempUntradable, pszUntradableString, 2,
													  m_pToolModelPanel->GetItem()->GetItemName(),
													  m_pSubjectModelPanel->GetItem()->GetItemName());
...
{
	CConfirmApplyStrangifierDialog *pDialog = vgui::SETUP_PANEL(
		new CConfirmApplyStrangifierDialog(pParent, pTool, pSubject, "ToolStrangifierConfirm", "strangified_item"));
	MakeModalAndBringToFront(pDialog);
}
//-----------------------------------------------------------------------------
void CEconTool_KillStreakifier::OnClientApplyTool(CEconItemView *pTool, CEconItemView *pSubject,
												  vgui::Panel *pParent) const
{
	CConfirmApplyStrangifierDialog *pDialog = vgui::SETUP_PANEL(new CConfirmApplyStrangifierDialog(
		pParent, pTool, pSubject, "ToolKillStreakifierConfirm", "killstreakified_item"));
	MakeModalAndBringToFront(pDialog);
}
//-----------------------------------------------------------------------------
...
{
	CConfirmApplyStrangifierDialog *pDialog = vgui::SETUP_PANEL(new CConfirmApplyStrangifierDialog(
		pParent, pTool, pSubject, "ToolFestivizerConfirm", "festivized_item", "#ToolFestivizerInProgress"));
	MakeModalAndBringToFront(pDialog);
}
//-----------------------------------------------------------------------------
void CEconTool_Unusualifier::OnClientApplyTool(CEconItemView *pTool, CEconItemView *pSubject,
											   vgui::Panel *pParent) const
{
	CConfirmApplyStrangifierDialog *pDialog = vgui::SETUP_PANEL(new CConfirmApplyStrangifierDialog(
		pParent, pTool, pSubject, "ToolUnusualifierConfirm", "unusualified_item", "#ToolUnusualifierInProgress"));
	MakeModalAndBringToFront(pDialog);
}
...
public:
	...
	virtual void ApplySchemeSettings(vgui::IScheme *pScheme)
	{
		...
		const CEconTool_ItemEaterRecharger *pTool =
			m_pToolModelPanel->GetItem()->GetItemDefinition()->GetTypedEconTool<CEconTool_ItemEaterRecharger>();
		if(pTool)
		{
...
// Purpose:
//-----------------------------------------------------------------------------
void CEconTool_ItemEaterRecharger::OnClientApplyTool(CEconItemView *pTool, CEconItemView *pSubject,
													 vgui::Panel *pParent) const
{
	CConfirmUseItemEaterRechargerDialog *pDialog =
		vgui::SETUP_PANEL(new CConfirmUseItemEaterRechargerDialog(pParent, pTool, pSubject));
	MakeModalAndBringToFront(pDialog);
}
...
public:
	...
	virtual void Apply(void)
	{
		...
		EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolModelPanel->GetItem(), "applied_upgrade_card",
											m_pToolModelPanel->GetItem()->GetItemDefIndex());
...
{
	CConfirmCardUpgradeApplicationDialog *pDialog =
		vgui::SETUP_PANEL(new CConfirmCardUpgradeApplicationDialog(pParent, pTool, pSubject));
	MakeModalAndBringToFront(pDialog);
}
...
public:
	...
	virtual void ApplySchemeSettings(vgui::IScheme *pScheme)
	{
		...
		const CEconTool_ClassTransmogrifier *pTool =
			m_pToolModelPanel->GetItem()->GetItemDefinition()->GetTypedEconTool<CEconTool_ClassTransmogrifier>();
		Assert(pTool);
	...
	virtual void Apply(void)
	{
		...
		EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolModelPanel->GetItem(), "applied_transmogrifier",
											m_pToolModelPanel->GetItem()->GetItemDefIndex());
...
// Purpose:
//-----------------------------------------------------------------------------
void CEconTool_ClassTransmogrifier::OnClientApplyTool(CEconItemView *pTool, CEconItemView *pSubject,
													  vgui::Panel *pParent) const
{
	CConfirmTransmogrifyApplicationDialog *pDialog =
		vgui::SETUP_PANEL(new CConfirmTransmogrifyApplicationDialog(pParent, pTool, pSubject));
	MakeModalAndBringToFront(pDialog);
}
...
public:
	...
	virtual void Apply(void)
	{
		...
		EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolModelPanel->GetItem(), "applied_spellbook_page",
											m_pToolModelPanel->GetItem()->GetItemDefIndex());
...
// Purpose:
//-----------------------------------------------------------------------------
void CEconTool_TFSpellbookPage::OnClientApplyTool(CEconItemView *pTool, CEconItemView *pSubject,
												  vgui::Panel *pParent) const
{
	CConfirmSpellbookPageApplicationDialog *pDialog =
		vgui::SETUP_PANEL(new CConfirmSpellbookPageApplicationDialog(pParent, pTool, pSubject));
	MakeModalAndBringToFront(pDialog);
}
...
public:
	...
	virtual void Apply(void)
	{
		...
		EconUI()->Gamestats_ItemTransaction(IE_ITEM_USED_TOOL, m_pToolModelPanel->GetItem(), "applied_ducktoken",
											m_pToolModelPanel->GetItem()->GetItemDefIndex());
...
{
	CConfirmDuckTokenApplicationDialog *pDialog =
		vgui::SETUP_PANEL(new CConfirmDuckTokenApplicationDialog(pParent, pTool, pSubject));
	MakeModalAndBringToFront(pDialog);
}
#endif // TF_CLIENT_DLL