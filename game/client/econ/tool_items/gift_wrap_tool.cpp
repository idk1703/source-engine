		...
		switch(msg.Body().response_code())
		{
			case k_EGCMsgResponseOK:
				if(msg.Body().has_receiver_account_name())
				{
					...
					ShowMessageBox("#TF_DeliverGiftResultDialog_Title",
								   "#TF_DeliverGiftResultDialog_Success_WithAccount", pkv, "#GameUI_OK");
				}
				else
				{
					ShowMessageBox("#TF_DeliverGiftResultDialog_Title", "#TF_DeliverGiftResultDialog_Success",
								   "#GameUI_OK");
				}
				break;
...
GC_REG_JOB(GCSDK::CGCClient, CGCGiftGivenResponse, "CGCGiftGivenResponse", k_EMsgGCDeliverGiftResponseGiver,
...
GC_REG_JOB(GCSDK::CGCClient, CGCGiftReceivedResponse, "CGCGiftReceivedResponse", k_EMsgGCDeliverGiftResponseReceiver,
		   GCSDK::k_EServerTypeGCClient);
...
void PerformToolAction_UnwrapGift(vgui::Panel *pParent, CEconItemView *pGiftItem)
{
	CTFGenericConfirmDialog *pDialog =
		ShowConfirmDialog("#TF_UnwrapGift_Title", "#TF_UnwrapGift_Text", "#GameUI_OK", "#Cancel", &UnwrapGiftConfirm);
