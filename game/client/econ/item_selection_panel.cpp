...
ConVar tf_item_selection_panel_sort_type("tf_item_selection_panel_sort_type", 0, FCVAR_NONE,
										 "0 - Sort is off, 1 - Sort is Alphabet (Pub)");
...
bool CEquipSlotItemSelectionPanel::ShouldItemPanelBeVisible(CItemModelPanel *pPanel, int iPanelIndex)
{
	...
	return (pPanel->HasItem() ||
			(iPanelIndex == 0 && !TFInventoryManager()->SlotContainsBaseItems(
									 GEconItemSchema().GetEquipTypeFromClassIndex(m_iClass), m_iSlot)));
}
...
struct RarityEconIdKey
{
	...
	RarityEconIdKey() : m_iQualitySort(-1), m_defIndex(INVALID_ITEM_DEF_INDEX), m_unKillEaterScore(0)
	{
		//
	}
	...
	{
		return m_defIndex < rhs.m_defIndex || m_iQualitySort < rhs.m_iQualitySort ||
...
static int SortRarityEconIdKeysBackpack(CEconItemView *const *a, CEconItemView *const *b)
{
	...
	return ExtractBackpackPositionFromBackend((*a)->GetInventoryPosition()) <
				   ExtractBackpackPositionFromBackend((*b)->GetInventoryPosition())
			   ? -1
			   : 1;
}
...
static int SortRarityEconIdKeysAlphabetical_Views(CEconItemView *const *a, CEconItemView *const *b)
{
	...
	uint32 unKillEaterScoreA = 0, unKillEaterScoreB = 0;
	...
	if(EconQuality_GetRarityScore((EEconItemQuality)(*a)->GetItemQuality()) <
		   EconQuality_GetRarityScore((EEconItemQuality)(*b)->GetItemQuality()) ||
	   unKillEaterScoreA < unKillEaterScoreB || (*a)->GetItemLevel() < (*b)->GetItemLevel())
	{
		return -1;
	}
...
// Purpose:
//-----------------------------------------------------------------------------
static int SortRarityEconIdKeysAlphabetical(const CEquippableItemsForSlotGenerator::CEquippableResult *a,
											const CEquippableItemsForSlotGenerator::CEquippableResult *b)
{
	return SortRarityEconIdKeysAlphabetical_Views(&a->m_pEconItemView, &b->m_pEconItemView);
}

//-----------------------------------------------------------------------------
static int SortRarityEconIdKeysDate(const CEquippableItemsForSlotGenerator::CEquippableResult *a,
									const CEquippableItemsForSlotGenerator::CEquippableResult *b)
{
	return (a->m_pEconItemView->GetID() > b->m_pEconItemView->GetID())	 ? -1
		   : (a->m_pEconItemView->GetID() < b->m_pEconItemView->GetID()) ? 1
																		 : 0;
}
...
//-----------------------------------------------------------------------------
CEquippableItemsForSlotGenerator::CEquippableItemsForSlotGenerator(int iClass, int iSlot,
																   equip_region_mask_t unUsedEquipRegionMask,
	...
	for(int i = 0; i < iNumItems; i++)
	{
		...
		// (ie., a higher-level item will never be equippable where a lower-level item is not)
		{
			...
			if(iIndex == mapHighestLevel.InvalidIndex() ||
			   pItem->GetItemLevel() > mapHighestLevel[iIndex].m_pEconItemView->GetItemLevel())
			{
				mapHighestLevel.InsertOrReplace(
					keyEconId, CEquippableItemsForSlotGenerator::CEquippableResult(pItem, eDisplayType));
			}
		}
	}
...
void CEquipSlotItemSelectionPanel::UpdateModelPanelsForSelection(void)
{
	...
	const equip_region_mask_t unUsedEquipRegionMask =
		GenerateEquipRegionConflictMask(m_iClass, CLASS_LOADOUT_POSITION_COUNT, m_iSlot);

	CEquippableItemsForSlotGenerator equippableItems(
		m_iClass, m_iSlot, unUsedEquipRegionMask,
		bShowEquippedItemFirst ? CEquippableItemsForSlotGenerator::kSlotGenerator_EquippedSpecialHandling
							   : CEquippableItemsForSlotGenerator::kSlotGenerator_None);
	...
	if(m_bShowingEntireBackpack)
	{
		...
		for(int i = 1; i <= iNumItems; i++)
		{
			...
			if(pItemData && pItemData->IsValid())
			{
				...
				CEquippableItemsForSlotGenerator::CEquippableResult &result =
					vecDisplayItems[vecDisplayItems.AddToTail(pItemData)];
				result.m_eDisplayType = GetItemNotSelectableReason(pItemData)
											? CEquippableItemsForSlotGenerator::kSlotDisplay_Invalid
											: CEquippableItemsForSlotGenerator::kSlotDisplay_Normal;
			}
		}
	}
	else
	{
		...
		const CEquippableItemsForSlotGenerator::EquippableResultsVec_t &allDisplayItems =
			equippableItems.GetDisplayItems();
	...
	for(int i = 0; i < m_pItemModelPanels.Count(); i++)
	{
		...
		if(vecDisplayItems.Count() > iItemIndex)
		{
			...
			{
				pszGreyOutReason =
					vecDisplayItems[iItemIndex].m_eDisplayType == CEquippableItemsForSlotGenerator::kSlotDisplay_Normal
						? NULL
						: "#Econ_GreyOutReason_EquipRegionConflict";
			}
...
const char *CEquipSlotItemSelectionPanel::GetItemNotSelectableReason(const CEconItemView *pItem) const
{
	...
	if(!AreSlotsConsideredIdentical(pItem->GetStaticData()->GetEquipType(), pItemData->GetLoadoutSlot(m_iClass),
									m_iSlot))
...
// Purpose:
//-----------------------------------------------------------------------------
CItemCriteriaSelectionPanel::CItemCriteriaSelectionPanel(Panel *parent, const CItemSelectionCriteria *pCriteria,
														 itemid_t pExceptions[], int iNumExceptions)
	: CItemSelectionPanel(parent)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CCraftingItemSelectionPanel::UpdateOnShow(const CItemSelectionCriteria *pCriteria, bool bForceBackpack,
											   itemid_t pExceptions[], int iNumExceptions)
{
	m_pCriteria = pCriteria;
	UpdateExceptions(pExceptions, iNumExceptions);
