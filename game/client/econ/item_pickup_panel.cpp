...
void CItemPickupPanel::PerformLayout(void)
{
	...
	for(int i = 0; i < ITEMPICKUP_NUM_MODELPANELS; i++)
	{
		...
		int iXPos =
			(GetWide() * 0.5) + (iPos * m_iModelPanelSpacing) + (iPos * m_iModelPanelW) - (m_iModelPanelW * 0.5);
...
void CItemPickupPanel::ShowPanel(bool bShow)
{
	if(bShow)
	{
		...
		m_iSelectedItem = 0;
		UpdateModelPanels();
		MoveToFront(); // Any open dialogs from the main menu (options, servers, etc.) showing up sometimes - this
					   // should fix.
	}
	else
	{
...
void CItemPickupPanel::OnKeyCodePressed(vgui::KeyCode code)
{
	...
	if(code == KEY_ENTER || nButtonCode == KEY_XBUTTON_A || nButtonCode == KEY_XBUTTON_B ||
	   nButtonCode == STEAMCONTROLLER_A || nButtonCode == STEAMCONTROLLER_B)
	{
		OnCommand("vguicancel");
	}
...
void CItemDiscardPanel::OnCommand(const char *command)
{
	if(!Q_stricmp(command, "vguicancel"))
	{
		...
		// Check to make sure the player has room for all his items. If not, bring up the discard panel. Otherwise, go
		// away.
		if(!InventoryManager()->CheckForRoomAndForceDiscard())
		{
			// If we're connected to a game server, we also close the game UI.
