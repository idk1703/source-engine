...
public:
	...
	virtual void CollateViewModelRenderables(CUtlVector<IClientRenderable *> &opaque,
...
private:
	...
	void SortEntities(const Vector &vecRenderOrigin, const Vector &vecRenderForward,
	...
	void InsertTranslucentRenderable(IClientRenderable *pRenderable, int &count, IClientRenderable **pList,
									 float *pDist);
...
private:
	...
	CBidirectionalSet<ClientRenderHandle_t, ClientLeafShadowHandle_t, unsigned short, unsigned int>
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CClientLeafSystem, IClientLeafSystem, CLIENTLEAFSYSTEM_INTERFACE_VERSION,
...
{
	// Tracker 37433:  This fixes a bug where if the stunstick is being wielded by a combine soldier, the fact that the
	// stick was
...
inline void CalcRenderableWorldSpaceAABB(IClientRenderable *pRenderable, Vector &absMins, Vector &absMaxs)
{
...
void CClientLeafSystem::PreRender()
{
	...
	while(m_DirtyRenderables.Count())
	{
		...
		bool bThreaded =
			false; //( nDirty > 5 && cl_threaded_client_leaf_system.GetBool() && g_pThreadPool->NumThreads() );
		...
		else
		{
			...
			ParallelProcess("CClientLeafSystem::PreRender", pDirtyRenderables, nDirty, this,
							&CClientLeafSystem::InsertIntoTree, &CClientLeafSystem::FrameLock,
							&CClientLeafSystem::FrameUnlock);
		}
...
int CClientLeafSystem::GetRenderableLeaves(ClientRenderHandle_t handle, int leaves[128])
{
	...
	int nLeaves = 0;
	for(int i = m_RenderablesInLeaf.FirstBucket(handle); i != m_RenderablesInLeaf.InvalidIndex();
...
//-----------------------------------------------------------------------------
bool CClientLeafSystem::GetRenderableLeaf(ClientRenderHandle_t handle, int *pOutLeaf, const int *pInIterator /* = 0 */,
										  int *pOutIterator /* = 0  */)
{
...
// Adds a shadow to a leaf/removes shadow from renderable
//-----------------------------------------------------------------------------
void CClientLeafSystem::AddShadowToRenderable(ClientRenderHandle_t renderHandle, ClientLeafShadowHandle_t shadowHandle)
{
	...
	if(m_Renderables[renderHandle].m_Flags & RENDER_FLAGS_BRUSH_MODEL)
	{
		...
		g_pClientShadowMgr->AddShadowToReceiver(m_Shadows[shadowHandle].m_Shadow, pRenderable,
			...
			for(int j = 0; j < m_RenderablesInLeaf.NumAllocated(); j++)
			{
				...
				else
				{
					...
					if(pEffect)
					{
						...
						Q_snprintf(pTemp, sizeof(pTemp), "ps: %s %.2f,%.2f", pEffect->GetEffectName(), maxs.x - mins.x,
								   maxs.y - mins.y);
						pClassName = pTemp;
					}
				...
				Msg(" %d: %p group:%d %s %d %d TransCalc:%d renderframe:%d\n", j, info.m_pRenderable,
					info.m_RenderGroup, pClassName, info.m_LeafList, info.m_RenderLeaf, info.m_TranslucencyCalculated,
					info.m_RenderFrame);
			}
...
void CClientLeafSystem::RemoveFromTree(ClientRenderHandle_t handle)
{
	...
	{
		g_pClientShadowMgr->RemoveAllShadowsFromReceiver(m_Renderables[handle].m_pRenderable,
														 SHADOW_RECEIVER_BRUSH_MODEL);
	}
...
// Detail system marks
//-----------------------------------------------------------------------------
void CClientLeafSystem::DrawDetailObjectsInLeaf(int leaf, int nFrameNumber, int &nFirstDetailObject,
...
//-----------------------------------------------------------------------------
void CClientLeafSystem::ComputeTranslucentRenderLeaf(int count, const LeafIndex_t *pLeafList,
...
#define MarkerToLeaf(p)	   (int)((reinterpret_cast<size_t>(p)) >> 1)
	...
	static CUtlVector<RenderableInfo_t *>
		orderedList; // @MULTICORE (toml 8/30/2006): will need to make non-static if thread this function
	...
	if(bThreaded)
	{
		ParallelProcess("CClientLeafSystem::ComputeTranslucentRenderLeaf", renderablesToUpdate.Base(),
						renderablesToUpdate.Count(), &CallComputeFXBlend, &::FrameLock, &::FrameUnlock);
...
//-----------------------------------------------------------------------------
inline void AddRenderableToRenderList(CClientRenderablesList &renderList, IClientRenderable *pRenderable, int iLeaf,
									  RenderGroup_t group, ClientRenderHandle_t renderHandle, bool bTwoPass = false)
{
#ifdef _DEBUG
...
//			renderGroup -
//-----------------------------------------------------------------------------
void CClientLeafSystem::CollateViewModelRenderables(CUtlVector<IClientRenderable *> &opaque,
													CUtlVector<IClientRenderable *> &translucent)
{
...
static RenderGroup_t DetectBucketedRenderGroup(RenderGroup_t group, float fDimension)
{
	...
	if(RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS <= 2 || fDimension >= arrThresholds[1])
	{
		if(RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS <= 1 || fDimension >= arrThresholds[0])
	...
	{
		if(RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS <= 3 || fDimension >= arrThresholds[2])
	...
	RenderGroup_t bucketedGroup =
		RenderGroup_t(group - ((RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS - 1) - bucketedGroupIndex) * 2);
...
void CClientLeafSystem::CollateRenderablesInLeaf(int leaf, int worldListLeafIndex, const SetupRenderInfo_t &info)
{
	...
	for(; idx != m_RenderablesInLeaf.InvalidIndex(); idx = m_RenderablesInLeaf.NextElement(idx))
	{
		...
		if(renderable.m_RenderGroup != RENDER_GROUP_TRANSLUCENT_ENTITY)
		{
			...
			if(RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS > 1 && group >= RENDER_GROUP_OPAQUE_STATIC &&
		...
		else
		{
			...
			if(info.m_bDrawTranslucentObjects)
			{
				AddRenderableToRenderList(*info.m_pRenderList, renderable.m_pRenderable, worldListLeafIndex,
	...
	if(info.m_bDrawDetailObjects && ShouldDrawDetailObjectsInLeaf(leaf, info.m_nDetailBuildFrame))
	{
		...
		while(--count >= 0)
		{
			...
			// FIXME: This if check here is necessary because the detail object system also maintains lists of
			// sprites...
			if(pRenderable)
			{
				if(pRenderable->IsTransparent())
				{
					if(info.m_bDrawTranslucentObjects) // Don't draw translucent objects into shadow depth maps
					{
						...
						{
							AddRenderableToRenderList(*info.m_pRenderList, pRenderable, worldListLeafIndex,
