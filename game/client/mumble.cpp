...
ConVar sv_mumble_positionalaudio("sv_mumble_positionalaudio", "1", FCVAR_REPLICATED,
...
void CMumbleSystem::LevelInitPostEntity()
{
	...
	g_pMumbleMemory =
		(MumbleSharedMemory_t *)MapViewOfFile(g_hMapObject, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(MumbleSharedMemory_t));
	...
	g_pMumbleMemory = (MumbleSharedMemory_t *)(mmap(NULL, sizeof(struct MumbleSharedMemory_t), PROT_READ | PROT_WRITE,
													MAP_SHARED, shmfd, 0));
...
void CMumbleSystem::PostRender()
{
	...
	if(!m_bHasSetPlayerUniqueId && steamapicontext && steamapicontext->SteamUser())
	{
		...
		if(steamid.IsValid())
		{
			...
			char szSteamId[256];
			V_sprintf_safe(szSteamId, "universe:%u;account_type:%u;id:%u;instance:%u;team:%d", steamid.GetEUniverse(),
						   steamid.GetEAccountType(), steamid.GetAccountID(), steamid.GetUnAccountInstance(), unTeam);
