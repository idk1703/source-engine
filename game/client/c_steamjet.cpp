...
RecvPropFloat(RECVINFO(m_SpreadSpeed), 0), RecvPropFloat(RECVINFO(m_Speed), 0), RecvPropFloat(RECVINFO(m_StartSize), 0),
	...
	RecvPropInt(RECVINFO(m_bEmit), 0), RecvPropInt(RECVINFO(m_bFaceLeft), 0), RecvPropInt(RECVINFO(m_nType), 0),
	RecvPropInt(RECVINFO(m_spawnflags)), RecvPropFloat(RECVINFO(m_flRollSpeed), 0),
END_RECV_TABLE
()
	...
	if(m_bEmit)
	{
		...
		if(nToEmit > 0)
		{
			...
			if(m_ParticleEffect.WasDrawnPrevFrame())
			{
				...
				{
					// Make a new particle.
					if(SteamJetParticle *pParticle =
						   (SteamJetParticle *)m_ParticleEffect.AddParticle(sizeof(SteamJetParticle), m_MaterialHandle))
					{
						pParticle->m_Pos = GetAbsOrigin();

						pParticle->m_Velocity = FRand(-m_SpreadSpeed, m_SpreadSpeed) * right +
												FRand(-m_SpreadSpeed, m_SpreadSpeed) * up + m_Speed * forward;
...
inline void RenderParticle_ColorSizePerturbNormal(ParticleDraw *pDraw, const Vector &pos, const Vector &color,
...
void C_SteamJet::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		{
			RenderParticle_ColorSizePerturbNormal(pIterator->GetParticleDraw(), tPos, vRampColor,
		...
		else
		{
			RenderParticle_ColorSizeAngle(
				pIterator->GetParticleDraw(), tPos, vRampColor, sinLifetime * (m_clrRender->a / 255.0f),
				FLerp(pParticle->m_uchStartSize, pParticle->m_uchEndSize, pParticle->m_Lifetime), pParticle->m_flRoll);
		}
