...
void UTIL_GetNormalizedColorTintAndLuminosity(const Vector &color, Vector *tint, float *luminosity)
{
	...
	if(luminosity != NULL)
	{
		...
		*luminosity = (color.x * RED_CHANNEL_CONTRIBUTION) + (color.y * GREEN_CHANNEL_CONTRIBUTION) +
...
void FX_WaterRipple(const Vector &origin, float scale, Vector *pColor, float flLifetime, float flAlpha)
{
	...
	if(tr.fraction < 1.0f)
	{
		...
		FX_AddQuad(tr.endpos + (tr.plane.normal * 0.5f), tr.plane.normal, 16.0f * scale, 128.0f * scale, 0.7f,
				   flAlpha, // start alpha
				   0.0f,	// end alpha
				   0.25f, random->RandomFloat(0, 360), random->RandomFloat(-16.0f, 16.0f), color, flLifetime,
...
void FX_GunshotSplash(const Vector &origin, const Vector &normal, float scale)
{
	...
	for(int i = 0; i < 16; i++)
	{
		...
		tParticle->m_vecVelocity =
			offDir * random->RandomFloat(SPLASH_MIN_SPEED * flScale * 3.0f, SPLASH_MAX_SPEED * flScale * 3.0f);
...
float CSplashParticle::UpdateAlpha(const SimpleParticle *pParticle)
{
	if(m_bUseClipHeight)
	{
		...
		return flAlpha * RemapValClamped(pParticle->m_Pos.z, m_flClipHeight,
										 m_flClipHeight - (UpdateScale(pParticle) * 0.5f), 1.0f, 0.0f);
	}
