...
FireRamp g_FireRamps[] = {FireRamp(Vector(1, 0, 0), Vector(1, 1, 0)), FireRamp(Vector(0.5, 0.5, 0), Vector(0, 0, 0))};
...
RecvPropVector(RECVINFO(m_vOrigin)), RecvPropVector(RECVINFO(m_vDirection)),
END_RECV_TABLE
()
...
void C_ParticleFire::Update(float fTimeDelta)
{
	...
	if(m_nEmitters < MAX_FIRE_EMITTERS)
	{
		...
		while(m_EmitterSpawn.NextEvent(tempDelta))
		{
			...
			if(m_nEmitters == 0)
			{
				...
				trace_t trace;
				UTIL_TraceLine(m_vOrigin, m_vOrigin + m_vDirection * 1000, MASK_SOLID_BRUSHONLY, NULL,
	...
	for(int i = 0; i < m_nEmitters; i++)
	{
		...
		{
			FireParticle *pParticle =
				(FireParticle *)m_ParticleEffect.AddParticle(sizeof(FireParticle), m_MaterialHandle);
			if(pParticle)
			{
