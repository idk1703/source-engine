...
public:
	virtual bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask)
	{
		...
		if(pEntity &&
		   ((dynamic_cast<C_BaseViewModel *>(pEntity) != NULL) || (dynamic_cast<C_BasePlayer *>(pEntity) != NULL)))
		{
			return false;
		}
		else
			...
			if(pWpn)
			{
				...
				for(int iShot = 0; iShot < m_iShots; iShot++)
				{
					...
					if(m_iShots > 1 && iShot % 2)
					{
						...
						UTIL_TraceHull(m_vecOrigin, vecEnd, Vector(-3, -3, -3), Vector(3, 3, 3), MASK_SHOT,
									   &traceFilter, &tr);
					}
					else
					{
...
BEGIN_RECV_TABLE_NOBASE(C_TEHL2MPFireBullets, DT_TEHL2MPFireBullets)
	RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropVector(RECVINFO(m_vecDir)), RecvPropInt(RECVINFO(m_iAmmoID)),
		RecvPropInt(RECVINFO(m_iSeed)), RecvPropInt(RECVINFO(m_iShots)), RecvPropInt(RECVINFO(m_iPlayer)),
		RecvPropInt(RECVINFO(m_iWeaponIndex)), RecvPropFloat(RECVINFO(m_flSpread)),
		RecvPropBool(RECVINFO(m_bDoImpacts)), RecvPropBool(RECVINFO(m_bDoTracers)),
END_RECV_TABLE()