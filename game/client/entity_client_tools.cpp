...
private:
	struct HToolEntry_t
	{
		...
		HToolEntry_t(int handle, C_BaseEntity *pEntity = NULL) : m_Handle(handle), m_hEntity(pEntity)
		{
			if(pEntity)
			{
...
bool CClientTools::DrawSprite(IClientRenderable *pRenderable, float scale, float frame, int rendermode, int renderfx,
							  const Color &color, float flProxyRadius, int *pVisHandle)
{
	...
	C_SpriteRenderer::GetSpriteAxes((C_SpriteRenderer::SPRITETYPE)psprite->GetOrientation(), origin, angles, forward,
									right, up);
	...
	if(rendermode != kRenderNormal)
	{
		...
		if((rendermode == kRenderGlow) || (rendermode == kRenderWorldGlow))
		{
			...
			float blend = oldBlend * StandardGlowBlend(params, (pixelvis_handle_t *)pVisHandle, rendermode, renderfx,
													   color.a(), &scale);
	...
	DrawSpriteModel((IClientEntity *)pRenderable, psprite, origin, scale, frame, rendermode, r, g, b, color.a(),
					forward, right, up);
