...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropInt(RECVINFO(m_nIndex)),
END_RECV_TABLE
()
...
void C_TEWorldDecal::PostDataUpdate(DataUpdateType_t updateType)
{
	...
	if(r_decals.GetInt())
	{
		...
		if(ent)
		{
			...
			{
				effects->DecalShoot(m_nIndex, 0, ent->GetModel(), ent->GetAbsOrigin(), ent->GetAbsAngles(), m_vecOrigin,
									0, 0);
			}
		}
	}
