...
// Purpose:
//-----------------------------------------------------------------------------
void FX_MuzzleEffect(const Vector &origin, const QAngle &angles, float scale, ClientEntityHandle_t hEntity,
					 unsigned char *pFlashColor, bool bOneFrame)
{
	...
	for ( i = 0; i < 4; i++ )
	{
		pParticle = (SimpleParticle *) pSimple->AddParticle( sizeof( SimpleParticle ), pSimple->GetPMaterial(
	"particle/particle_smokegrenade" ), origin );
...
//			bOneFrame -
//-----------------------------------------------------------------------------
void FX_MuzzleEffectAttached(float scale, ClientEntityHandle_t hEntity, int attachmentIndex, unsigned char *pFlashColor,
							 bool bOneFrame)
{
	VPROF_BUDGET("FX_MuzzleEffect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	for(i = 1; i < 9; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
														   g_Mat_SMG_Muzzleflash[random->RandomInt(0, 3)], offset);
	...
	pEmitter->SetString("material", "effects/muzzleflash2"); // FIXME - create DmeMultiMaterialSpriteEmitter to support
															 // the 4 materials of muzzleflash
	...
	Color color(pFlashColor ? pFlashColor[0] : 255, pFlashColor ? pFlashColor[1] : 255,
...
void MuzzleFlashCallback(const CEffectData &data)
{
	...
	tempents->MuzzleFlash(vecOrigin, vecAngles, data.m_fFlags & (~MUZZLEFLASH_FIRSTPERSON), data.m_hEntity,
						  (data.m_fFlags & MUZZLEFLASH_FIRSTPERSON) != 0);
}
...
//			flRollDelta -
//-----------------------------------------------------------------------------
CSmartPtr<CSimpleEmitter> FX_Smoke(const Vector &origin, const Vector &velocity, float scale, int numParticles,
								   float flDietime, unsigned char *pColor, int iAlpha, const char *pMaterial,
...
// Purpose: Smoke puffs
//-----------------------------------------------------------------------------
void FX_Smoke(const Vector &origin, const QAngle &angles, float scale, int numParticles, unsigned char *pColor,
			  int iAlpha)
{
	VPROF_BUDGET("FX_Smoke", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	Vector vecVelocity;
	...
	for(int i = 0; i < numParticles; i++)
	{
		...
		FX_Smoke(origin, vecVelocity, iSize, 1, random->RandomFloat(0.5f, 1.0f), particlecolor, alpha,
				 "particle/particle_smokegrenade", flRoll, flRollDelta);
	}
}
...
//-----------------------------------------------------------------------------
void FX_BuildSmoke(Vector &vecOrigin, QAngle &vecAngles, ClientEntityHandle_t hEntity, int nAttachment,
...
void CommandPointerCallback(const CEffectData &data)
{
	...
	for(int i = 0; i < size; i++)
	{
		...
		{
			FX_GunshipImpact(data.m_vOrigin, Vector(0, 0, 1), commandercolors[i].r, commandercolors[i].g,
							 commandercolors[i].b);
			return;
		}
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void FX_GunshipMuzzleEffect(const Vector &origin, const QAngle &angles, float scale, ClientEntityHandle_t hEntity,
							unsigned char *pFlashColor)
{
	...
	pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
													   pSimple->GetPMaterial("effects/gunshipmuzzle"), offset);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void FX_StriderMuzzleEffect(const Vector &origin, const QAngle &angles, float scale, ClientEntityHandle_t hEntity,
							unsigned char *pFlashColor)
{
	Vector vecDir;
...
void FX_GaussTracer(Vector &start, Vector &end, int velocity, bool makeWhiz)
{
	...
	// Add it
	FX_AddDiscreetLine(start, shotDir, velocity, length, totalDist, random->RandomFloat(5.0f, 8.0f), life,
					   "effects/spark");
}
...
//-----------------------------------------------------------------------------
void FX_BuildTesla(C_BaseEntity *pEntity, const Vector &vecOrigin, const Vector &vecEnd, const char *pModelName,
				   float flBeamWidth, const Vector &vColor, int nFlags, float flTimeVisible)
{
	BeamInfo_t beamInfo;
...
void FX_Tesla(const CTeslaInfo &teslaInfo)
{
	...
	for(int i = 0; i < iTotalBeams; i++)
	{
		...
		do
		{
			...
			UTIL_TraceLine(teslaInfo.m_vPos, teslaInfo.m_vPos + (vecForward * teslaInfo.m_flRadius), MASK_SHOT, pEntity,
		...
		if(tr.fraction < 1.0)
		{
			if(!EffectOccluded(tr.endpos, 0))
			{
				...
				SimpleParticle *pParticle;
				pParticle = (SimpleParticle *)pSimple->AddParticle(
					sizeof(SimpleParticle), pSimple->GetPMaterial("effects/tesla_glow_noz"), vecFlash);
				if(pParticle != NULL)
				{
					pParticle->m_flLifetime = 0.0f;
		...
		// Build the tesla
		FX_BuildTesla(pEntity, teslaInfo.m_vPos, tr.endpos, teslaInfo.m_pszSpriteName, teslaInfo.m_flBeamWidth,
					  teslaInfo.m_vColor, FBEAM_ONLYNOISEONCE, teslaInfo.m_flTimeVisible);
	}
}
...
//-----------------------------------------------------------------------------
void FX_BuildTeslaHitbox(C_BaseEntity *pEntity, int nStartAttachment, int nEndAttachment, float flBeamWidth,
						 const Vector &vColor, float flTimeVisible)
{
	...
	UTIL_TraceLine(pEntity->WorldSpaceCenter(), pEntity->WorldSpaceCenter() + (randomDir * 100), MASK_SOLID_BRUSHONLY,
...
void FX_BuildTeslaHitbox(const CEffectData &data)
{
	...
	for(int i = 0; i < nBeamCount; ++i)
	{
		...
		FX_BuildTeslaHitbox(pEntity, nStartHitBox, nEndHitBox, data.m_flScale, vColor,
							random->RandomFloat(0.05f, 0.2f));
	}
}
