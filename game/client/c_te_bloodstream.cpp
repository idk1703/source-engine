...
RecvPropVector(RECVINFO(m_vecDirection)), RecvPropInt(RECVINFO(r)), RecvPropInt(RECVINFO(g)), RecvPropInt(RECVINFO(b)),
...
// Recording
//-----------------------------------------------------------------------------
static inline void RecordBloodStream(const Vector &start, const Vector &direction, int r, int g, int b, int a,
									 int amount)
{
	if(!ToolsEnabled())
		return;
...
void TE_BloodStream(IRecipientFilter &filter, float delay, const Vector *org, const Vector *direction, int r, int g,
		...
		if(p)
		{
			...
			for(count2 = 0; count2 < 2; count2++)
			{
				...
				if(pChunky)
				{
					...
					pChunky->SetAlpha(a);
					pChunky->m_Pos.Init((*org)[0] + random->RandomFloat(-1, 1), (*org)[1] + random->RandomFloat(-1, 1),
