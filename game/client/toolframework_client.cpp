	...
	for(int i = 0; i < nParams; ++i)
	{
		...
		switch(vartype)
		{
			...
			case MATERIAL_VAR_TYPE_MATRIX:
			{
				...
				V_snprintf(str, sizeof(str), "matrix: %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f", pVal[0],
						   pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6], pVal[7], pVal[8], pVal[9], pVal[10],
						   pVal[11], pVal[12], pVal[13], pVal[14], pVal[15]);
				msg->SetString(pVarName, str);
			}
			break;
			...
			case MATERIAL_VAR_TYPE_UNDEFINED:
				//			Assert( 0 ); // these appear to be (mostly? all?) textures, although I don't know why they're
				//not caught by the texture case above...
				break; // JDTODO
