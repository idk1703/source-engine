...
static ConVar joy_response_move("joy_response_move", "1", FCVAR_ARCHIVE,
								"'Movement' stick response mode: 0=Linear, 1=quadratic, 2=cubic, 3=quadratic extreme, "
								"4=power function(i.e., pow(x,1/sensitivity)), 5=two-stage");
ConVar joy_response_move_vehicle("joy_response_move_vehicle", "6");
static ConVar joy_response_look("joy_response_look", "0", FCVAR_ARCHIVE,
...
static ConVar joy_inverty("joy_inverty", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
...
static ConVar joy_movement_stick_default("joy_movement_stick_default", "0",
...
static ConVar joy_movement_stick("joy_movement_stick", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
								 "Which stick controls movement (0 is left stick)", joy_movement_stick_Callback);
...
ConVar joy_accel_filter("joy_accel_filter",
						"0.2"); // If the non-accelerated axis is pushed farther than this, then accelerate it, too.
...
void CInput::JoyStickMove(float frametime, CUserCmd *cmd)
{
	...
	m_flPreviousJoystickForward =
		ScaleAxisValue(gameAxes[GAME_AXIS_FORWARD].value, MAX_BUTTONSAMPLE * joy_forwardthreshold.GetFloat());
	m_flPreviousJoystickSide =
		ScaleAxisValue(gameAxes[GAME_AXIS_SIDE].value, MAX_BUTTONSAMPLE * joy_sidethreshold.GetFloat());
	m_flPreviousJoystickPitch =
		ScaleAxisValue(gameAxes[GAME_AXIS_PITCH].value, MAX_BUTTONSAMPLE * joy_pitchthreshold.GetFloat());
	m_flPreviousJoystickYaw =
		ScaleAxisValue(gameAxes[GAME_AXIS_YAW].value, MAX_BUTTONSAMPLE * joy_yawthreshold.GetFloat());
	...
	if(CAM_IsThirdPerson() && thirdperson_platformer.GetInt())
	{
		if(m_flPreviousJoystickForward || m_flPreviousJoystickSide)
		{
			...
			viewangles[YAW] = RAD2DEG(atan2(-m_flPreviousJoystickSide, -m_flPreviousJoystickForward)) +
							  g_ThirdPersonManager.GetCameraOffsetAngles()[YAW];
		...
		if(m_flPreviousJoystickForward || m_flPreviousJoystickSide || m_flPreviousJoystickPitch ||
		   m_flPreviousJoystickYaw)
		{
	...
	{
		iResponseCurve = pLocalPlayer->GetVehicle() ? pLocalPlayer->GetVehicle()->GetJoystickResponseCurve()
	...
	{
		float fAxisValue = ResponseCurveLook(joy_response_look.GetInt(), m_flPreviousJoystickYaw, YAW,
											 m_flPreviousJoystickPitch, dist, frametime);
	...
	if(IsX360() || in_jlook.state & 1)
	{
		...
		{
			float fAxisValue = ResponseCurveLook(joy_response_look.GetInt(), m_flPreviousJoystickPitch, PITCH,
												 m_flPreviousJoystickYaw, dist, frametime);
