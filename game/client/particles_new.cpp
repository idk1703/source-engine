...
void CNewParticleEffect::SetControlPointOrientation(int nWhichPoint, const Vector &forward, const Vector &right,
													const Vector &up)
{
...
CNewParticleEffect *CNewParticleEffect::ReplaceWith(const char *pParticleSystemName)
{
	...
	CSmartPtr<CNewParticleEffect> pNewEffect =
		CNewParticleEffect::Create(GetOwner(), pParticleSystemName, pParticleSystemName);
...
void CNewParticleEffect::DetectChanges()
{
	...
	if(m_MinBounds.x < m_LastMin.x || m_MinBounds.y < m_LastMin.y || m_MinBounds.z < m_LastMin.z ||
	   ...
	   m_MinBounds.x > (m_LastMin.x + flExtraBuffer) || m_MinBounds.y > (m_LastMin.y + flExtraBuffer) ||
	   m_MinBounds.z > (m_LastMin.z + flExtraBuffer) ||

	   m_MaxBounds.x > m_LastMax.x || m_MaxBounds.y > m_LastMax.y || m_MaxBounds.z > m_LastMax.z ||

	   m_MaxBounds.x < (m_LastMax.x - flExtraBuffer) || m_MaxBounds.y < (m_LastMax.y - flExtraBuffer) ||
				...
				if(!input->CAM_IsThirdPerson())
				{
					...
					if(pPlayer == pCameraObject)
					{
						...
						if(pPlayer->GetObserverMode() == OBS_MODE_IN_EYE &&
						   (pObTarget == pEntity || pRootMove == pObTarget))
							return 0;
					}
				}
			}
...
static ConCommand cl_dump_particle_stats("cl_dump_particle_stats", DumpParticleStats_f,
