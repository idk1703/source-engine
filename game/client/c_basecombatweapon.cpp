...
void C_BaseCombatWeapon::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if(bIsLocalPlayer &&
	   ShouldDrawLocalPlayerViewModel()) // TODO: figure out the purpose of the ShouldDrawLocalPlayer() test.
	{
...
void C_BaseCombatWeapon::DrawCrosshair()
{
	...
	/*
		...
		trace_t tr;
		traceline->TraceLine( vShootPos, vShootPos + vForward * 10000, COLLISION_GROUP_NONE, MASK_SHOT, &tr, true, ~0,
	   &filter );
...
int C_BaseCombatWeapon::DrawModel(int flags)
{
	...
	if(localplayer && localplayer->IsObserver() && GetOwner())
	{
		...
		if(localplayer->GetObserverMode() == OBS_MODE_IN_EYE && localplayer->GetObserverTarget() == GetOwner())
			return false;
	}
...
{
	C_BasePlayer *localplayer = C_BasePlayer::GetLocalPlayer();
	if(localplayer && localplayer == GetOwner() && ShouldDrawLocalPlayerViewModel())
	{
