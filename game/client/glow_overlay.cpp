...
void CGlowOverlay::UpdateSkyGlowObstruction(float zFar, bool bCacheFullSceneState)
{
	...
	// Trace a ray at the object.
	trace_t trace;
	UTIL_TraceLine(CurrentViewOrigin(), CurrentViewOrigin() + (m_vDirection * MAX_TRACE_LENGTH), CONTENTS_SOLID, NULL,
...
void CGlowOverlay::UpdateGlowObstruction(const Vector &vToGlow, bool bCacheFullSceneState)
{
	...
	if(m_bInSky)
	{
		// Trace a ray at the object.
		trace_t trace;
		UTIL_TraceLine(CurrentViewOrigin(), CurrentViewOrigin() + (vToGlow * MAX_TRACE_LENGTH), CONTENTS_SOLID, NULL,
					   COLLISION_GROUP_NONE, &trace);
...
void CGlowOverlay::CalcSpriteColorAndSize(float flDot, CGlowSprite *pSprite, float *flHorzSize, float *flVertSize,
										  Vector *vColor)
{
	...
	*flHorzSize = RemapValClamped(flDot, g_flOverlayRange, 1, flSizeAtOverlayRangeMul * pSprite->m_flHorzSize,
								  flSizeAtOneMul * pSprite->m_flHorzSize);

	*flVertSize = RemapValClamped(flDot, g_flOverlayRange, 1, flSizeAtOverlayRangeMul * pSprite->m_flVertSize,
								  flSizeAtOneMul * pSprite->m_flVertSize);

	float flOpacity = RemapValClamped(flDot, g_flOverlayRange, 1, flOpacityAtOverlayRange, flOpacityAtOne);

	flOpacity = flOpacity * m_flGlowObstructionScale;
	*vColor = pSprite->m_vColor * flOpacity;
}

void CGlowOverlay::CalcBasis(const Vector &vToGlow, float flHorzSize, float flVertSize, Vector &vBasePt, Vector &vUp,
							 Vector &vRight)
{
...
void CGlowOverlay::Draw(bool bCacheFullSceneState)
{
	...
	for(int iSprite = 0; iSprite < m_nSprites; iSprite++)
	{
		...
		{
			m_Sprites[iSprite].m_pMaterial =
				materials->FindMaterial("sprites/light_glow02_add_noz", TEXTURE_GROUP_CLIENT_EFFECTS);
		}

		Assert(m_Sprites[iSprite].m_pMaterial);
		static unsigned int nHDRColorScaleCache = 0;
		IMaterialVar *pHDRColorScaleVar =
			m_Sprites[iSprite].m_pMaterial->FindVarFast("$hdrcolorscale", &nHDRColorScaleCache);
		...
		if(bWireframe)
		{
			IMaterial *pWireframeMaterial =
				materials->FindMaterial("debug/debugwireframevertexcolor", TEXTURE_GROUP_OTHER);
...
void CGlowOverlay::DrawOverlays(bool bCacheFullSceneState)
{
	...
	for(unsigned short i = g_GlowOverlaySystem.m_GlowOverlays.Head();
		i != g_GlowOverlaySystem.m_GlowOverlays.InvalidIndex(); i = iNext)
	{
		...
		if(pOverlay->Update())
		{
			pRenderContext->EnableClipping(
				((pOverlay->m_bInSky)
					 ? (false)
					 : (bClippingEnabled))); // disable clipping in skybox, restore clipping to pre-existing state when
											 // not in skybox (it may be off as well)
