	...
	for(int i = 0; i < HL1_MAX_WEAPON_SLOTS; i++)
	{
		...
		else
		{
			...
			for(int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++)
			{
				...
				vgui::surface()->DrawFilledRect(xpos, ypos, xpos + icon_buckets[i]->Width(),
												ypos + icon_buckets[i]->Height());
...
void CHudWeaponSelection::DrawAmmoBar(C_BaseCombatWeapon *pWeapon, int x, int y, int nWidth, int nHeight)
{
	if(pWeapon->GetPrimaryAmmoType() != -1)
	{
		...
		{
			flPct = (float)pPlayer->GetAmmoCount(pWeapon->GetSecondaryAmmoType()) /
					(float)GetAmmoDef()->MaxCarry(pWeapon->GetSecondaryAmmoType());
