...
{
public:
	ConVar_m_pitch() : ConVar_ServerBounded("m_pitch", "0.022", FCVAR_ARCHIVE, "Mouse pitch factor.") {}
...
static ConVar m_mousespeed("m_mousespeed", "1", FCVAR_ARCHIVE,
						   "Windows mouse acceleration (0 to disable, 1 to enable [Windows 2000: enable initial "
						   "threshold], 2 to enable secondary threshold [Windows 2000 only]).",
						   true, 0, true, 2);
...
void CInput::CheckMouseAcclerationVars()
{
	...
	if(!m_fMouseActive || !m_fMouseInitialized || !m_fMouseParmsValid || !m_fRestoreSPI)
	{
		return;
	}
		...
		if(m_fMouseParmsValid)
		{
			if(CommandLine()->FindParm("-noforcemspd"))
			{
				...
				/*
								...
								// Now check registry and close/re-open Control Panel > Mouse and see 'Enhance pointer
				   precision' is OFF mouseAccel[2] = 1; ok = SystemParametersInfo(SPI_SETMOUSE, 0, &mouseAccel,
				   SPIF_UPDATEINIFILE);
...
//  these results are accumulated each time. This function gets the accumulated mouse changes and resets the
//  accumulators
// Input  : *mx -
//			*my -
//-----------------------------------------------------------------------------
