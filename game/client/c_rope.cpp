...
RecvPropInt(RECVINFO(m_iRopeMaterialModelIndex)), RecvPropEHandle(RECVINFO(m_hStartPoint)),
	RecvPropEHandle(RECVINFO(m_hEndPoint)), RecvPropInt(RECVINFO(m_iStartAttachment)),
	RecvPropInt(RECVINFO(m_iEndAttachment)),

	RecvPropInt(RECVINFO(m_fLockedPoints)), RecvPropInt(RECVINFO(m_Slack), 0, RecvProxy_RecomputeSprings),
	RecvPropInt(RECVINFO(m_RopeLength), 0, RecvProxy_RecomputeSprings), RecvPropInt(RECVINFO(m_RopeFlags)),
	RecvPropFloat(RECVINFO(m_TextureScale)), RecvPropInt(RECVINFO(m_nSegments)),
	RecvPropBool(RECVINFO(m_bConstrainBetweenEndpoints)), RecvPropInt(RECVINFO(m_Subdiv)),

	RecvPropFloat(RECVINFO(m_Width)), RecvPropFloat(RECVINFO(m_flScrollSpeed)),
...
static ConVar rope_smooth_enlarge("rope_smooth_enlarge", "1.4", 0,
								  "How much to enlarge ropes in screen space for antialiasing effect");

static ConVar rope_smooth_minwidth("rope_smooth_minwidth", "0.3", 0,
...
static ConVar rope_averagelight("rope_averagelight", "1", 0,
	...
	inline void *Alloc(size_t bytes)
	{
		...
		if(pReturn == NULL)
		{
			...
			Warning("Overflowed rope queued rendering memory stack. Needed %llu, have %d/%d\n", (uint64)bytes,
					iMaxSize - m_QueuedRopeMemory[m_nCurrentStack].GetUsed(), iMaxSize);
...
public:
	void DrawRenderCache_NonQueued(bool bShadowDepth, RopeRenderData_t *pRenderCache, int nRenderCacheCount,
...
private:
	...
	void RenderSolidRopes(IMatRenderContext *pRenderContext, IMaterial *pMaterial, int nVertCount, int nIndexCount,
...
{
	return pBackMaterial && rope_smooth.GetInt() && engine->GetDXSupportLevel() > 70 &&
...
void CRopeManager::DrawRenderCache_NonQueued(bool bShadowDepth, RopeRenderData_t *pRenderCache, int nRenderCacheCount,
											 const Vector &vCurrentViewForward, const Vector &vCurrentViewOrigin,
											 C_RopeKeyframe::BuildRopeQueuedData_t *pBuildRopeQueuedData)
{
	VPROF_BUDGET("CRopeManager::DrawRenderCache", VPROF_BUDGETGROUP_ROPES);
	AUTO_LOCK(
		m_RenderCacheMutex); // contention cases: Toggling from queued mode on to off. Rope deletion from the cache.
	...
	for(int iRenderCache = 0; iRenderCache < nRenderCacheCount; ++iRenderCache)
	{
		...
		for(int iCache = 0; iCache < nCacheCount; ++iCache)
		{
			...
			else
			{
				if(pBuildRopeQueuedData)
				{
					// we should only be here if a rope was in the queue and then deleted. We still have it's relevant
					// data (and need to skip over it).
					++pBuildRopeQueuedData;
				}
			}
		}
		...
		if(rope_rendersolid.GetInt())
		{
			if(bShadowDepth)
				...
				RenderSolidRopes(pRenderContext, pRenderCache[iRenderCache].m_pSolidMaterial, nVertCount, nIndexCount,
								 bRenderNonSolid);
		}
	}
	ResetSegmentCache(0);
...
void CRopeManager::DrawRenderCache(bool bShadowDepth)
{
	...
	if(r_queued_ropes.GetBool() && (pCallQueue = materials->GetRenderContext()->GetCallQueue()) != NULL)
	{
		...
		C_RopeKeyframe::BuildRopeQueuedData_t *pBuildRopeQueuedDataStart =
			(C_RopeKeyframe::BuildRopeQueuedData_t *)(pRenderCachesStart + iRenderCacheCount);
		...
		// Setup the rest of our data. This writes to two separate areas of memory at the same time. One area for the
		// C_RopeKeyframe::BuildRopeQueuedData_t array, the other for mini-arrays of vector data
		for(int i = 0; i != iRenderCacheCount; ++i)
		{
			...
			for(int j = 0; j != iCacheCount; ++j)
			{
				...
				pVectorWrite +=
					iNodes; // so we don't overwrite the light values with the next rope's predicted positions
			}
		}
		Assert(((void *)pVectorWrite == (void *)(((uint8 *)pMemory) + iMemoryNeeded)) &&
			   ((void *)pWriteRopeQueuedData == (void *)pVectorDataStart));
		pCallQueue->QueueCall(this, &CRopeManager::DrawRenderCache_NonQueued, bShadowDepth, pRenderCachesStart,
							  iRenderCacheCount, vForward, vOrigin, pBuildRopeQueuedDataStart);
...
// Purpose:
//-----------------------------------------------------------------------------
void CRopeManager::RenderSolidRopes(IMatRenderContext *pRenderContext, IMaterial *pMaterial, int nVertCount,
	...
	if(bRenderNonSolid)
	{
		...
		for(int iSegmentCache = 0; iSegmentCache < m_nSegmentCacheCount; ++iSegmentCache)
		{
			...
			for(int iSegment = 0; iSegment < nSegmentCount; ++iSegment)
			{
				...
				pSeg->m_flAlpha =
					RemapVal(pSeg->m_flWidth, rope_solid_minwidth.GetFloat(), rope_solid_maxwidth.GetFloat(),
							 rope_solid_minalpha.GetFloat(), rope_solid_maxalpha.GetFloat());
...
void LockNodeDirection(CSimplePhysics::CNode *pNodes, int parity, int nFalloffNodes, float flLockAmount,
...
void C_RopeKeyframe::CPhysicsDelegate::ApplyConstraints(CSimplePhysics::CNode *pNodes, int nNodes)
{
	...
	if(((m_pKeyframe->m_RopeFlags & ROPE_COLLIDE) && rope_collide.GetInt()) || (rope_collide.GetInt() == 2))
	{
		...
		for(int i = 0; i < nNodes; i++)
		{
			...
			{
				trace_t trace;
				UTIL_TraceHull(pNode->m_vPrevPos, pNode->m_vPos, Vector(-2, -2, -2), Vector(2, 2, 2),
...
C_RopeKeyframe *C_RopeKeyframe::Create(C_BaseEntity *pStartEnt, C_BaseEntity *pEndEnt, int iStartAttachment,
									   int iEndAttachment, float ropeWidth, const char *pMaterialName, int numSegments,
									   int ropeFlags)
{
...
C_RopeKeyframe *C_RopeKeyframe::CreateFromKeyValues(C_BaseAnimating *pEnt, KeyValues *pValues)
{
	C_RopeKeyframe *pRope = C_RopeKeyframe::Create(
		...
		pEnt->LookupAttachment(pValues->GetString("EndAttachment")), pValues->GetFloat("Width", 0.5),
		pValues->GetString("Material"), pValues->GetInt("NumSegments"), 0);
...
int C_RopeKeyframe::GetRopesIntersectingAABB(C_RopeKeyframe **pRopes, int nMaxRopes, const Vector &vAbsMin,
...
{
	m_RopePhysics.ResetSpringLength((m_RopeLength + m_Slack + ROPESLACK_FUDGEFACTOR) / (m_RopePhysics.NumNodes() - 1));
}
...
Vector C_RopeKeyframe::ConstrainNode(const Vector &vNormal, const Vector &vNodePosition, const Vector &vMidpiont,
									 float fNormalLength)
{
...
void C_RopeKeyframe::ConstrainNodesBetweenEndpoints(void)
{
	...
	for(int i = 1; i < m_RopePhysics.NumNodes() - 1; ++i)
	{
		...
		m_RopePhysics.GetNode(i)->m_vPos =
			ConstrainNode(vNormal, m_RopePhysics.GetNode(i)->m_vPos, vMidpiont, fNormalLength);

		// Fix the predicted position
		m_RopePhysics.GetNode(i)->m_vPredicted =
			ConstrainNode(vNormal, m_RopePhysics.GetNode(i)->m_vPredicted, vMidpiont, fNormalLength);
	}
}
...
bool C_RopeKeyframe::GetAttachment(int number, Vector &origin, QAngle &angles)
{
	...
	if((number == ROPE_ATTACHMENT_START_POINT || number == ROPE_ATTACHMENT_END_POINT) && nNodes >= 2)
	{
		...
		else
		{
			origin = m_RopePhysics.GetNode(nNodes - 1)->m_vPredicted;
			vForward =
				m_RopePhysics.GetNode(nNodes - 1)->m_vPredicted - m_RopePhysics.GetNode(nNodes - 2)->m_vPredicted;
		}
...
void Catmull_Rom_Spline_Matrix(const Vector &vecP1, const Vector &vecP2, const Vector &vecP3, const Vector &vecP4,
							   catmull_t &output)
{
	output.t3 =
		0.5f * ((-1 * vecP1) + (3 * vecP2) + (-3 * vecP3) + vecP4); // 0.5 t^3 * [ (-1*p1) + ( 3*p2) + (-3*p3) + p4 ]
	output.t2 =
		0.5f * ((2 * vecP1) + (-5 * vecP2) + (4 * vecP3) - vecP4); // 0.5 t^2 * [ ( 2*p1) + (-5*p2) + ( 4*p3) - p4 ]
							   ...
							   bool bQueued)
{
	...
	float flTotalTexCoord =
		flPixelsPerInch * (pQueuedData->m_RopeLength + pQueuedData->m_Slack + ROPESLACK_FUDGEFACTOR);
	...
	if(ShouldUseFakeAA(m_pBackMaterial))
	{
		...
		for(int iSegment = 0; iSegment < nSegmentCount; ++iSegment)
		{
			...
			else
			{
				...
				{
					pSegmentData->m_Segments[iSegment].m_flAlpha = RemapVal(
						flScreenSpaceWidth, flMinScreenSpaceWidth, flMaxAlphaScreenSpaceWidth, flMinAlpha, flMaxAlpha);
				}
				...
				else
				{
					pSegmentData->m_flMaxBackWidth =
						MAX(pSegmentData->m_flMaxBackWidth, pSegmentData->m_BackWidths[iSegment]);
				}
			}
...
bool C_RopeKeyframe::GetEndPointAttachment(int iPt, Vector &vPos, QAngle &angle)
{
	...
	{
		CalculateEndPointAttachment(m_hStartPoint, m_iStartAttachment, m_vCachedEndPointAttachmentPos[0],
									&m_vCachedEndPointAttachmentAngle[0]);
		CalculateEndPointAttachment(m_hEndPoint, m_iEndAttachment, m_vCachedEndPointAttachmentPos[1],
...
void C_RopeKeyframe::CalcLightValues()
{
	...
	for(int i = 0; i < m_RopePhysics.NumNodes(); i++)
	{
		...
		if(!rope_averagelight.GetInt())
		{
			...
			//
			// So here, we take the average of all the incoming light, and scale it to use the max intensity of all the
			// box sides.
