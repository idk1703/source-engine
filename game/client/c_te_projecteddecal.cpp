...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropQAngles(RECVINFO(m_angRotation)), RecvPropFloat(RECVINFO(m_flDistance)),
...
void TE_ProjectDecal(IRecipientFilter &filter, float delay, KeyValues *pKeyValues)
{
	...
	TE_ProjectDecal(filter, 0.0f, &vecOrigin, &angles, flDistance,
					effects->Draw_DecalIndexFromName((char *)pDecalName));
}