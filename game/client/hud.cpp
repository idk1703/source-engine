	...
	if(pKeyValuesData)
	{
		...
		if(pTextureSection)
		{
			...
			while(pTemp)
			{
				...
				{
					int iTexLeft = pTemp->GetInt("x", 0), iTexTop = pTemp->GetInt("y", 0),
					...
					for(int i = 0; i < hudTextureFileRefs.Size(); i++)
					{
						...
						if(cszFilename)
						{
							...
							Q_strncpy(tex->szShortName, hudTextureFileRefs[i].m_cszHudTexturePrefix,
									  sizeof(tex->szShortName));
							Q_strncpy(tex->szShortName + hudTextureFileRefs[i].m_uiPrefixLength, pTemp->GetName(),
									  sizeof(tex->szShortName) - hudTextureFileRefs[i].m_uiPrefixLength);
	...
	if(kv)
	{
		if(kv->LoadFromFile(filesystem, "scripts/HudLayout.res"))
		{
			...
			for(int i = 0; i < numelements; i++)
			{
				...
				if(!element->IsParentedToClientDLLRootPanel() && !pPanel->GetParent())
				{
...
void CHudTexture::DrawSelf(int x, int y, int w, int h, const Color &clr) const
{
	...
	else
	{
		...
		vgui::surface()->DrawTexturedSubRect(x, y, x + w, y + h, texCoords[0], texCoords[1], texCoords[2],
											 texCoords[3]);
	}
}

void CHudTexture::DrawSelfCropped(int x, int y, int cropx, int cropy, int cropw, int croph, int finalWidth,
								  int finalHeight, Color clr) const
{
	if(bRenderUsingFont)
	{
	...
	else
	{
		...
		vgui::surface()->DrawSetTexture(textureId);
		vgui::surface()->DrawSetColor(clr);
		vgui::surface()->DrawTexturedSubRect(x, y, x + finalWidth, y + finalHeight, tCoords[0], tCoords[1], tCoords[2],
											 tCoords[3]);
	}
}
...
CHudTexture *CHud::AddUnsearchableHudIconToList(CHudTexture &texture)
{
	...
	{
		Q_snprintf(composedName, sizeof(composedName), "%s_%i_%i_%i_%i", texture.szTextureFile, texture.rc.left,
				   texture.rc.top, texture.rc.right, texture.rc.bottom);
	}
