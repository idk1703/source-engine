...
#ifdef PORTAL
	float m_skyObstructionScaleBackups[MAX_PORTAL_RECURSIVE_VIEWS]; // used in portal mod during stencil rendering to
																	// maintain obstructions while rendering recursive
																	// views
#endif
...
protected:
	...
	virtual void CalcBasis(const Vector &vToGlow, float flHorzSize, float flVertSize, Vector &vBasePt, Vector &vUp,
						   Vector &vRight);
