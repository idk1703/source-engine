...
CReplayReminderPanel::CReplayReminderPanel(const char *pElementName)
	: EditablePanel(g_pClientMode->GetViewport(), "ReplayReminder"), CHudElement(pElementName)
{
...
void CReplayReminderPanel::OnThink()
{
	...
	if(m_flShowTime)
	{
		...
		float flShowLength =
			replay_postwinreminderduration.IsValid() ? replay_postwinreminderduration.GetFloat() : 5.0f;
