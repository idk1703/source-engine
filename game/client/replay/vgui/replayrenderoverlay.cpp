...
ConVar replay_enablerenderpreview("replay_enablerenderpreview", "1", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_ARCHIVE,
...
void CReplayRenderOverlay::PerformLayout()
{
	...
	if(pReplay)
	{
		...
		const int nServerRecordStartTick =
			g_pClientReplayContext->GetRecordingSessionManager()->GetServerStartTickForSession(
				pReplay->m_hSession); // NOTE: Returns -1 on fail
	...
	if(m_pPreviewCheckButton)
	{
		...
		m_pPreviewCheckButton->SetPos((nProgX - m_pPreviewCheckButton->GetWide()) / 2,
									  nTitleLabelY + m_pTitleLabel->GetTall() + YRES(3));
	}
	...
	m_pCancelButton->SetPos(nProgRightX + (m_pBottom->GetWide() - nProgRightX - m_pCancelButton->GetWide()) / 2,
...
void CReplayRenderOverlay::OnTick()
{
	...
	if(m_pRenderProgress)
	{
		...
		if(pReplay && m_pRenderProgress->IsVisible())
		{
			...
			Assert(
				flTotalTime >
				0.0f); // NOTE: Progress bar will always be invisible if total time is 0, but check anyway to be safe.
			m_pRenderProgress->SetProgress(MAX(m_pRenderProgress->GetProgress(),
											   flProgress)); // The MAX() here keeps the progress bar from thrashing

			if(m_pProgressLabel)
			{
				// @note Tom Bui: this is a horribly ugly hack, but the first couple of frames take a really freaking
				// long time, so that really blows out the estimate
				...
				else if(m_unNumFrames > kNumFramesToWait)
				{
					...
					{
						m_flPreviousTimeLeft =
							kSmoothingFactor * flEstimatedTimeLeft + (1 - kSmoothingFactor) * m_flPreviousTimeLeft;
					}
				}
				...
				wchar_t wszText[256];
				g_pVGuiLocalize->ConstructString_safe(wszText, g_pVGuiLocalize->Find("#Replay_RenderOverlay_TimeLeft"),
													  2, wszTime, wszTimeLeft);
...
void CReplayRenderOverlay::OnCommand(const char *pCommand)
{
	...
	{
		ShowConfirmDialog("#Replay_CancelRenderTitle", "#Replay_ConfirmCancelRender", "#Replay_YesCancel", "#Replay_No",
						  OnRenderCancelDialogButtonPressed, this, NULL, "replay\\replaydialog_warn.wav");
		return;
	}
...
void ReplayUI_OpenReplayRenderOverlay()
{
	...
	s_pRenderOverlay = SETUP_PANEL(new CReplayRenderOverlay(
		NULL)); // Parenting to NULL allows us to turn off world rendering in engine/view.cpp (V_RenderView())
