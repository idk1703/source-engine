...
CReplayScreenshotSlideshowPanel::CReplayScreenshotSlideshowPanel(Panel *pParent, const char *pName,
																 ReplayHandle_t hReplay)
	: CSlideshowPanel(pParent, pName), m_hReplay(hReplay)
{
...
CReplayBrowserThumbnail::CReplayBrowserThumbnail(Panel *pParent, const char *pName,
												 QueryableReplayItemHandle_t hReplayItem,
...
void CReplayBrowserThumbnail::PerformLayout()
{
	...
	float flScale = pReplay->GetScreenshotCount() == 0
						? 1.0f
						: ((float)m_pScreenshotThumb->GetWide() / (.95f * pReplay->GetScreenshot(0)->m_nWidth));
	m_pScreenshotThumb->SetScaleAmount(flScale);
	m_pScreenshotThumb->SetShouldScaleImage(true);
	if(bShowScreenshotThumb)
	{
		// We don't want to actually hide it (via SetVisible()), since we need to get mouse click events from the image
		// panel
	...
	m_pDownloadProgress->SetPos((GetWide() - m_pDownloadProgress->GetWide()) / 2,
	...
	m_pErrorLabel->SetPos((GetWide() - m_pErrorLabel->GetWide()) / 2,
...
CReplayBrowserThumbnailRow::CReplayBrowserThumbnailRow(Panel *pParent, const char *pName,
													   IReplayItemManager *pReplayItemManager)
	: BaseClass(pParent, pName), m_pReplayItemManager(pReplayItemManager)
{
	SetProportional(true);
}
...
{
	CReplayBrowserThumbnail *pThumbnail =
		new CReplayBrowserThumbnail(this, "ListThumbnail", pItem->GetItemHandle(), m_pReplayItemManager);
...
{
	CReplayBrowserThumbnail *pThumbnail =
		new CReplayBrowserThumbnail(this, "ListThumbnail", hReplayItem, m_pReplayItemManager);
	m_vecThumbnails.AddToTail(pThumbnail);
}
...
CBaseThumbnailCollection::CBaseThumbnailCollection(CReplayListPanel *pParent, const char *pName,
												   IReplayItemManager *pReplayItemManager)
...
int CBaseThumbnailCollection::GetRowStartY()
{
	...
	Panel *pLowestPanel =
		m_pReplayItemManager->GetItemCount() == 0 ? m_pNoReplayItemsLabel : GetLowestPanel(nVerticalBuffer);

	int x, y;
	pLowestPanel->GetPos(x, y);

	bool bMakeRoomForNextPrev =
		(m_pShowPrevButton && m_pShowNextButton && (m_pShowPrevButton->IsVisible() || m_pShowNextButton->IsVisible()));
...
CReplayBrowserThumbnailRow *CBaseThumbnailCollection::FindReplayItemThumbnailRow(
	const IQueryableReplayItem *pReplayItem)
{
	AssertValidReadPtr(pReplayItem);
...
void CBaseThumbnailCollection::OnCommand(const char *pCommand)
{
	...
	else if(FStrEq(pCommand, "show_next"))
	{
		...
		m_iViewingPage =
			clamp(m_iViewingPage + 1, 0, Ceil2Int((float)m_vecReplays.Count() / (float)iThumbnailsPerPage));
		UpdateViewingPage();
		return;
	}
...
void CBaseThumbnailCollection::UpdateViewingPage(void)
{
	...
	for(int i = 0; i < iThumbnailsPerPage; i++)
	{
		...
		if(iRow >= m_vecRows.Count())
		{
			...
			CReplayBrowserThumbnailRow *pNewRow =
				new CReplayBrowserThumbnailRow(this, "ThumbnailRow", m_pReplayItemManager);
...
void CBaseThumbnailCollection::PerformLayout()
{
	...
	if(m_pShowNextButton)
	{
		...
		bool bMakeRoomForNextPrev = (m_pShowPrevButton && m_pShowNextButton &&
									 (m_pShowPrevButton->IsVisible() || m_pShowNextButton->IsVisible()));
		if(bMakeRoomForNextPrev)
		{
...
CReplayThumbnailCollection::CReplayThumbnailCollection(CReplayListPanel *pParent, const char *pName,
													   IReplayItemManager *pReplayItemManager)
	: BaseClass(pParent, pName, pReplayItemManager), m_pWarningLabel(NULL), m_pUnconvertedBg(NULL)
{
...
void CReplayThumbnailCollection::LayoutUpperPanels(int nStartY, int nBgWidth)
{
	...
	m_pWarningLabel->SetBounds(m_nStartX, nWarningStartY, 2 * GetWide() / 3,
							   cy); // For when "convert all" button is shown
...
void CReplayThumbnailCollection::LayoutBackgroundPanel(int nWide, int nTall)
{
	...
	if(m_pUnconvertedBg)
	{
		m_pUnconvertedBg->SetBounds(0, 0, nWide, nTall);
...
CMovieThumbnailCollection::CMovieThumbnailCollection(CReplayListPanel *pParent, const char *pName,
													 IReplayItemManager *pReplayItemManager, int nDay, int nMonth,
													 int nYear, bool bShowSavedMoviesLabel)
...
CMovieThumbnailCollection::CMovieThumbnailCollection(CReplayListPanel *pParent, const char *pName,
													 IReplayItemManager *pReplayItemManager, bool bShowSavedMoviesLabel)
	: BaseClass(pParent, pName, pReplayItemManager)
{
...
void CMovieThumbnailCollection::Init(int nDay, int nMonth, int nYear, bool bShowSavedMoviesLabel)
{
	...
	{
		m_pDateLabel = m_nDay >= 0 ? new CExLabel(this, "DateLabel",
												  CReplayTime::GetLocalizedDate(g_pVGuiLocalize, nDay, nMonth, nYear))
								   : NULL;
	}
...
void CMovieThumbnailCollection::PerformLayout()
{
	...
	// Movies have multiple collections under a single header. So we use the total movies, not the amount in this
	// collection.
...
void CMovieThumbnailCollection::LayoutUpperPanels(int nStartY, int nBgWidth)
{
	if(m_pTitleLabel && m_pTitleLabel->IsVisible())
	{
		...
		m_pCaratLabel->SizeToContents();
		m_pCaratLabel->SetPos(m_nStartX - m_pCaratLabel->GetWide() - XRES(3),
							  nStartY + (m_pCaratLabel->GetTall() - m_pCaratLabel->GetTall()) / 2);
