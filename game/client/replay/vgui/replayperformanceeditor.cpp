...
ConVar replay_perftip_count_enter("replay_perftip_count_enter", "0", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_HIDDEN,
								  "", true, 0, false, 0);
ConVar replay_perftip_count_exit("replay_perftip_count_exit", "0", FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_HIDDEN,
								 "", true, 0, false, 0);
ConVar replay_perftip_count_freecam_enter("replay_perftip_count_freecam_enter", "0",
										  FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_HIDDEN, "", true, 0, false, 0);
ConVar replay_perftip_count_freecam_exit("replay_perftip_count_freecam_exit", "0",
										 FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_HIDDEN, "", true, 0, false, 0);
ConVar replay_perftip_count_freecam_exit2("replay_perftip_count_freecam_exit2", "0",
										  FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_HIDDEN, "", true, 0, false, 0);

ConVar replay_editor_fov_mousewheel_multiplier(
	"replay_editor_fov_mousewheel_multiplier", "5", FCVAR_ARCHIVE | FCVAR_CLIENTDLL | FCVAR_DONTRECORD,
	"The multiplier on mousewheel input for adjusting camera FOV in the replay editor.");
ConVar replay_editor_fov_mousewheel_invert("replay_editor_fov_mousewheel_invert", "0",
										   FCVAR_ARCHIVE | FCVAR_CLIENTDLL | FCVAR_DONTRECORD,
										   "Invert FOV zoom/unzoom on mousewheel in the replay editor.");

ConVar replay_replayeditor_rewindmsgcounter("replay_replayeditor_rewindmsgcounter", "0",
											FCVAR_ARCHIVE | FCVAR_CLIENTDLL | FCVAR_DONTRECORD | FCVAR_HIDDEN, "");
...
public:
	CSavingDialog(CReplayPerformanceEditorPanel *pEditorPanel) : CGenericWaitingDialog(pEditorPanel)
	{
...
public:
	CReplayTipLabel(Panel *pParent, const char *pName, const char *pText) : BaseClass(pParent, pName, pText) {}
...
public:
	...
	virtual void ApplySchemeSettings(IScheme *pScheme)
	{
		...
		SetBounds(0, 3 * nScreenH / 4 - nLabelHeight / 2, nScreenW, nLabelHeight + 2 * m_nTopBottomMargin);
		m_pTextLabel->SetBounds(m_nLeftRightMarginWidth, m_nTopBottomMargin, nScreenW - 2 * m_nLeftRightMarginWidth,
								nLabelHeight);
	}
...
public:
	CPlayerCell(Panel *pParent, const char *pName, int *pCurTargetPlayerIndex)
		: CExImageButton(pParent, pName, ""), m_iPlayerIndex(-1), m_pCurTargetPlayerIndex(pCurTargetPlayerIndex)
	{
	}
...
public:
	CCameraOptionsPanel(Panel *pParent, const char *pName, const char *pTitle)
		: EditablePanel(pParent, pName), m_bControlsAdded(false)
	{
	...
	void AddSliderToLayout(int nId, Slider *pSlider, const char *pLabelText, float flMinValue, float flMaxValue,
						   float &flOut)
	{
...
public:
	...
	virtual void AddControls()
	{
		...
		AddSliderToLayout(SLIDER_TIMESCALE, m_pTimeScaleSlider, "#Replay_Scale", TIMESCALE_MIN, TIMESCALE_MAX,
						  *m_pTimeScaleProxy);
	}
...
public:
	...
	virtual void AddControls()
	{
		...
		AddSliderToLayout(SLIDER_ACCEL, m_pAccelSlider, "#Replay_Accel", FREE_CAM_ACCEL_MIN, FREE_CAM_ACCEL_MAX,
						  ReplayCamera()->m_flRoamingAccel);
		AddSliderToLayout(SLIDER_SPEED, m_pSpeedSlider, "#Replay_Speed", FREE_CAM_SPEED_MIN, FREE_CAM_SPEED_MAX,
						  ReplayCamera()->m_flRoamingSpeed);
		AddSliderToLayout(SLIDER_FOV, m_pFovSlider, "#Replay_Fov", FREE_CAM_FOV_MIN, FREE_CAM_FOV_MAX,
						  ReplayCamera()->m_flRoamingFov[1]);
		AddSliderToLayout(SLIDER_ROTFILTER, m_pRotFilterSlider, "#Replay_RotFilter", FREE_CAM_ROT_FILTER_MIN,
						  FREE_CAM_ROT_FILTER_MAX, ReplayCamera()->m_flRoamingRotFilterFactor);
		AddSliderToLayout(SLIDER_SHAKE_SPEED, m_pShakeSpeedSlider, "#Replay_ShakeSpeed", FREE_CAM_SHAKE_SPEED_MIN,
						  FREE_CAM_SHAKE_SPEED_MAX, ReplayCamera()->m_flRoamingShakeSpeed);
		AddSliderToLayout(SLIDER_SHAKE_AMOUNT, m_pShakeAmountSlider, "#Replay_ShakeAmount", FREE_CAM_SHAKE_AMOUNT_MIN,
						  FREE_CAM_SHAKE_AMOUNT_MAX, ReplayCamera()->m_flRoamingShakeAmount);
		AddSliderToLayout(SLIDER_SHAKE_DIR, m_pShakeDirSlider, "#Replay_ShakeDir", FREE_CAM_SHAKE_DIR_MIN,
						  FREE_CAM_SHAKE_DIR_MAX, ReplayCamera()->m_flRoamingShakeDir);
	}
...
public:
	CReplayButton(Panel *pParent, const char *pName, const char *pText)
		: BaseClass(pParent, pName, pText), m_pTipText(NULL)
	{
	}
...
public:
	CReplayEditorFastForwardButton(Panel *pParent, const char *pName, const char *pText)
		: BaseClass(pParent, pName, pText), m_flPressTime(0.0f)
	{
...
public:
	CRecLightPanel(Panel *pParent)
		: EditablePanel(pParent, "RecLightPanel"), m_flPlayPauseTime(0.0f), m_bPaused(false), m_bPerforming(false)
	{
		m_pRecLights[0] = NULL;
		m_pRecLights[1] = NULL;
		m_pPlayPause[0] = NULL;
	...
	virtual void PerformLayout()
	{
		...
		if(m_pCameraCrosshair)
		{
			...
			m_pCameraCrosshair->SetBounds(nStartY + (nWidth - aImageSize[0]) / 2,
										  nStartY + (nHeight - aImageSize[1]) / 2, aImageSize[0] - 2 * nStartY,
	...
	virtual void OnThink()
	{
		...
		bool bPauseAnimating =
			m_flPlayPauseTime > 0.0f && flTime >= m_flPlayPauseTime && flTime < (m_flPlayPauseTime + m_flAnimTime);
		...
		if(bPauseAnimating)
		{
			...
			m_pPlayPause[iPlayPauseActive]->SetBounds(nScreenXCenter - nSize / 2, nScreenYCenter - nSize / 2, nSize,
													  nSize);
	  ...
	  m_pSavingDlg(NULL)
{
	...
	HScheme hScheme = scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/ClientScheme.res",
													 "ClientScheme");
	SetScheme(hScheme);
...
void CReplayPerformanceEditorPanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	static const char *s_pControlButtonNames[NUM_CTRLBUTTONS] = {"InButton",   "GotoBeginningButton", "RewindButton",
																 "PlayButton", "FastForwardButton",	  "GotoEndButton",
																 "OutButton"};
...
void CReplayPerformanceEditorPanel::HandleMouseWheel(int nDelta)
{
	if(ReplayCamera()->GetMode() == OBS_MODE_ROAMING)
	{
		...
		flFov = clamp(flFov - nDelta * replay_editor_fov_mousewheel_multiplier.GetFloat(), FREE_CAM_FOV_MIN,
					  FREE_CAM_FOV_MAX);

		// Update FOV slider in free camera settings
		CCameraOptionsPanel_Free *pFreeCamOptions =
			static_cast<CCameraOptionsPanel_Free *>(m_pCameraOptionsPanels[CAM_FREE]);
...
void CReplayPerformanceEditorPanel::OnTick()
{
	...
	if(IsVisible() && m_bMousePressed)
	{
		...
		else if(bInAControllableCameraMode && !bMouseCursorOverPerfEditor && !bMouseOverMenuButton && !bMouseOverMenu &&
				bNoDialogsUp)
		{
			if(bRecording)
			{
				...
				{
					DisplayPerformanceTip("#Replay_PerfTip_ExitFreeCam", &replay_perftip_count_freecam_exit,
										  MAX_TIP_DISPLAYS);
					surface()->PlaySound("replay\\cameracontrolmodeentered.wav");
				}
				else
				{
					DisplayPerformanceTip("#Replay_PerfTip_EnterFreeCam", &replay_perftip_count_freecam_enter,
										  MAX_TIP_DISPLAYS);
	...
	for(int iPlayer = 1; iPlayer <= MAX_PLAYERS; ++iPlayer)
	{
		...
		char const *pClassName =
			iPlayerClass == TF_CLASS_DEMOMAN ? "demo" : g_aPlayerClassNames_NonLocalized[iPlayerClass];
	...
	if(bRecording)
	{
		for(int i = 0; i < NCAMS; ++i)
		{
			...
			bool bInCameraModeThatMouseIsOver =
				ReplayCamera()->GetMode() == GetCameraModeFromButtonIndex((CameraMode_t)i);
			bool bDontCareAboutCameraMode = i == COMPONENT_TIMESCALE;
			bool bActivate = (i == m_nMouseClickedOverCameraSettingsPanel) ||
							 (((bInCameraModeThatMouseIsOver || bDontCareAboutCameraMode) && bMouseOverButton) ||
							  (bMouseOverOptionsPanel && pCurOptionsPanel->IsVisible()));
	...
	if(m_pPlayerNameLabel && pGamePlayerResource && pMouseOverCell)
	{
		...
		int nLabelX = MAX(nCellPos[0], m_nRedBlueLabelRightX);
...
{
	CCameraOptionsPanel_Free *pSettingsPanel =
		dynamic_cast<CCameraOptionsPanel_Free *>(m_pCameraOptionsPanels[CAM_FREE]);
	if(!pSettingsPanel)
		return;
...
{
	CTimeScaleOptionsPanel *pSettingsPanel =
		dynamic_cast<CTimeScaleOptionsPanel *>(m_pCameraOptionsPanels[COMPONENT_TIMESCALE]);
	if(!pSettingsPanel)
		return;
...
void CReplayPerformanceEditorPanel::PerformLayout()
{
	...
	for(int i = 0; i < NCAMS; ++i)
	{
		...
		// Position it
		pCurOptionsPanel->SetPos(aBottomPos[0] + aCameraButtonPos[0] + pCurCameraButton->GetWide() -
									 pCurOptionsPanel->GetWide() - XRES(3),
	...
	CExLabel *pRedBlueLabels[2] = {dynamic_cast<CExLabel *>(m_pPlayerCellsPanel->FindChildByName("RedLabel")),
	...
	for(int i = 0; i < 2; ++i)
	{
		...
		const int nY = m_pPlayerCellsPanel->GetTall() / 2 + m_nRedBlueSigns[i] * m_pPlayerCellsPanel->GetTall() / 4 -
					   pRedBlueLabels[i]->GetTall() / 2;
...
bool CReplayPerformanceEditorPanel::OnStateChangeRequested(const char *pEventStr)
{
	...
	ShowConfirmDialog("#Replay_Warning", "#Replay_NukePerformanceChanges", "#GameUI_Confirm", "#GameUI_CancelBold",
					  OnConfirmDestroyChanges, this, this, REPLAY_SOUND_DIALOG_POPUP);
...
void CReplayPerformanceEditorPanel::SetButtonTip(wchar_t *pTipText, Panel *pContextPanel)
{
	...
	const int nX =
		clamp(aPos[0] - m_pButtonTip->GetWide() / 2, 0, ScreenWidth() - m_pButtonTip->GetWide() - (int)XRES(40));
...
void CReplayPerformanceEditorPanel::ShowPanel(bool bShow)
{
	...
	// Keep controller informed about pause state so that it can throw away unimportant events during pause if it's
	// recording.
...
void CReplayPerformanceEditorPanel::UpdateCameraButtonImages(bool bForceUnselected /*=false*/)
{
	...
	for(int i = 0; i < NCAMS; ++i)
	{
		CFmtStr fmtFile(gs_pBaseComponentNames[i], gs_pCamNames[i],
						(!bForceUnselected && (!pPerformance || g_pReplayPerformanceController->IsRecording()) &&
						 i == m_iCameraSelection)
							? "_selected"
							: "");
...
{
	ShowMessageBox("#Replay_RewindWarningTitle", "#Replay_RewindWarningMsg", "#GameUI_OK", OnConfirmRewind, NULL,
				   (void *)this);
...
void CReplayPerformanceEditorPanel::Exit_ShowDialogs()
{
	...
	{
		ShowConfirmDialog("#Replay_DiscardTitle", "#Replay_DiscardChanges", "#Replay_Discard", "#Replay_Cancel",
						  OnConfirmDiscard, NULL, this, REPLAY_SOUND_DIALOG_POPUP);
	}
	else
	{
		ShowConfirmDialog("#Replay_ExitEditorTitle", "#Replay_BackToReplays", "#GameUI_Confirm", "#Replay_Cancel",
						  OnConfirmExit, NULL, this, REPLAY_SOUND_DIALOG_POPUP);
	}
}
...
void CReplayPerformanceEditorPanel::OnCommand(const char *command)
{
	...
	else if(!V_strnicmp(command, "goto_", 5))
	{
		...
		if(pReplay)
		{
			...
			const int nStartTick =
				MAX(0, (pPerformance && pPerformance->HasInTick()) ? pPerformance->m_nTickIn : pReplay->m_nSpawnTick);
			const int nEndTick =
				MAX( // The MAX() here will keep us from going back in time if we're already past the "end" tick
					nCurTick,
					((pPerformance && pPerformance->HasOutTick()) ? pPerformance->m_nTickOut
