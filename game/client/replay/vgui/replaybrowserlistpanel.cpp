...
CReplayListPanel::CReplayListPanel(Panel *pParent, const char *pName)
	: BaseClass(pParent, pName), m_pPrevHoverPanel(NULL), m_pPreviewPanel(NULL)
{
	...
	m_pMoviesCollection =
		new CMovieThumbnailCollection(this, "MovieThumbnailCollection", GetReplayMovieItemManager(), true);
...
void CReplayListPanel::OnItemPanelEntered(vgui::Panel *pPanel)
{
	...
	if(IsVisible() && pThumbnail && pThumbnail->IsVisible())
	{
		...
		int nOffset = XRES(1);
		nXPos =
			(x > GetWide() / 2) ? (x - m_pPreviewPanel->GetWide() - nOffset) : (x + pThumbnail->GetWide() + nOffset);
		...
		nArrowYPos = MIN(nYPos + m_pPreviewPanel->GetTall() - m_pBorderArrowImg->GetTall() * 2,
						 y + (pThumbnail->m_pScreenshotThumb->GetTall() - m_aBorderArrowDims[1]) / 2);
...
CBaseThumbnailCollection *CReplayListPanel::FindOrAddReplayThumbnailCollection(const IQueryableReplayItem *pItem,
																			   IReplayItemManager *pItemManager)
{
	Assert(pItem);
