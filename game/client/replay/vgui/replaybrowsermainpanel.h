...
class CReplayBrowserPanel : public vgui::PropertyDialog, public CGameEventListener
{
...
public:
	...
	virtual void CleanupUIForReplayItem(
		ReplayItemHandle_t hReplay); // After a replay has been deleted - deletes all UI (thumbnail, but maybe also row
									 // and/or collection as well)
	...
	void AttemptToDeleteReplayItem(Panel *pHandler, ReplayItemHandle_t hReplayItem, IReplayItemManager *pItemManager,
								   int iPerformance);
...
private:
	...
	void ConfirmReplayItemDelete(Panel *pHandler, ReplayItemHandle_t hReplayItem, IReplayItemManager *pItemManager,
								 int iPerformance);
