...
CReplayBrowserBasePage::CReplayBrowserBasePage(Panel *pParent) : BaseClass(pParent, "BasePage")
{
...
void CReplayBrowserBasePage::OnTick()
{
	...
	if(input()->IsMouseDown(MOUSE_LEFT) && !m_pSearchTextEntry->IsWithin(nCursorX, nCursorY) &&
...
void CReplayBrowserBasePage::OnCommand(const char *pCommand)
{
	...
	if(!V_strnicmp(pCommand, "details", 7))
	{
		...
		if(pReplayItem)
		{
			...
			m_hReplayDetailsPanel =
				vgui::SETUP_PANEL(new CReplayDetailsPanel(this, hReplayItem, iPerformance, pItemManager));
