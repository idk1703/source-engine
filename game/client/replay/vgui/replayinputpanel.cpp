...
CReplayInputPanel::CReplayInputPanel(Panel *pParent, const char *pName, ReplayHandle_t hReplay)
	: BaseClass(pParent, pName), m_hReplay(hReplay), m_pDlg(NULL), m_pTitleEntry(NULL)
{
...
void CReplayInputPanel::OnCommand(const char *command)
{
	...
	if(!Q_strnicmp(command, "save", 4))
	{
		if(m_hReplay != REPLAY_HANDLE_INVALID)
		{
			...
			if(pLocalPlayer)
			{
				g_pClientMode->DisplayReplayMessage(pLocalPlayer->IsAlive() ? "#Replay_ReplaySavedAlive"
																			: "#Replay_ReplaySavedDead",
													-1.0f, false, "replay\\saved.wav", false);
