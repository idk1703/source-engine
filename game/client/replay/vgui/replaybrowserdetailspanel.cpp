...
ConVar replay_movie_reveal_warning("replay_movie_reveal_warning", "1",
								   FCVAR_ARCHIVE | FCVAR_CLIENTDLL | FCVAR_HIDDEN | FCVAR_DONTRECORD);
ConVar replay_movie_export_last_dir("replay_movie_export_last_dir", "",
									FCVAR_ARCHIVE | FCVAR_CLIENTDLL | FCVAR_HIDDEN | FCVAR_DONTRECORD);
ConVar replay_renderqueue_first_add("replay_renderqueue_first_add", "0",
									FCVAR_ARCHIVE | FCVAR_CLIENTDLL | FCVAR_HIDDEN | FCVAR_DONTRECORD);
...
public:
	CConfirmDisconnectFromServerDialog(Panel *pParent) : BaseClass(pParent)
	{
...
CBaseDetailsPanel::CBaseDetailsPanel(Panel *pParent, const char *pName, ReplayHandle_t hReplay)
	: EditablePanel(pParent, pName), m_hReplay(hReplay), m_bShouldShow(true)
{
	SetScheme("ClientScheme");
...
void CRecordsPanel::PerformLayout()
{
	...
	V_snprintf(szImage, sizeof(szImage), "class_sel_sm_%s_%s", pReplay->GetMaterialFriendlyPlayerClass(),
			   pReplay->GetPlayerTeam()); // Cause default image to display
	...
	if(pImage)
	{
		...
		int nX, nY;
		GetPos(nX, nY);
		m_pClassImage->SetBounds(nX + nMargin, nY - flScale * nImageHeight + GetTall() - nMargin, nImageWidth * flScale,
								 nImageHeight * flScale);
...
CStatsPanel::CStatsPanel(Panel *pParent, ReplayHandle_t hReplay) : CBaseDetailsPanel(pParent, "StatsPanel", hReplay)
{
	...
	for(int i = 0; i < REPLAY_MAX_DISPLAY_GAMESTATS; ++i)
	{
		...
		m_paStatLabels[i] =
			new CKeyValueLabelPanel(GetInset(), g_pReplayDisplayGameStats[i].m_pStatLocalizationToken, szValue);
...
CKillsPanel::CKillsPanel(Panel *pParent, ReplayHandle_t hReplay) : CBaseDetailsPanel(pParent, "KillsPanel", hReplay)
{
	...
	: CBaseDetailsPanel(pParent, "BasicLifeInfo", hReplay)
{
	// Create labels
	CGenericClassBasedReplay *pReplay = GetGenericClassBasedReplay(hReplay);
	m_pKilledByLabels = new CKeyValueLabelPanel(GetInset(), "#Replay_StatKilledBy",
												pReplay->WasKilled() ? pReplay->GetKillerName() : "#Replay_None");
	m_pMapLabels = new CKeyValueLabelPanel(GetInset(), "#Replay_OnMap", GetMapDisplayName(pReplay->m_szMapName));
	m_pLifeLabels =
		new CKeyValueLabelPanel(GetInset(), "#Replay_Life", CReplayTime::FormatTimeString((int)pReplay->m_flLength));
}
...
CYouTubeInfoPanel::CYouTubeInfoPanel(Panel *pParent) : CBaseDetailsPanel(pParent, "YouTubeInfo", NULL), m_pLabels(NULL)
{
...
CTitleEditPanel::CTitleEditPanel(Panel *pParent, QueryableReplayItemHandle_t hReplayItem,
...
void CTitleEditPanel::PerformLayout()
{
	...
	m_pClickToEditLabel->SetBounds(nTitleEntryX + GetParent()->GetWide() - nClickToEditW * 1.4f,
								   nTitleEntryY + (nTitleEntryTall - nClickToEditH) / 2, nClickToEditW, nClickToEditH);
...
void CTitleEditPanel::PaintBackground()
{
	...
	bool bDrawExtraStuff = !vgui::input()->GetAppModalSurface() &&
						   (m_bMouseOver || bEditing); // Don't draw extra stuff when render dialog is up
...
CPlaybackPanel::CPlaybackPanel(Panel *pParent) : EditablePanel(pParent, "PlaybackPanel") {}
...
CPlaybackPanelSlideshow::CPlaybackPanelSlideshow(Panel *pParent, ReplayHandle_t hReplay)
	: CPlaybackPanel(pParent), m_hReplay(hReplay)
{
...
void CPlaybackPanelSlideshow::PerformLayout()
{
	...
	if(m_pScreenshotImage)
	{
		...
		// Setup screenshot scale based on width of first screenshot (if there are any screenshots at all) - otherwise
		// don't scale
		float flScale = pReplay->GetScreenshotCount() == 0
							? 1.0f
							: ((float)nScreenshotWidth / (.95f * pReplay->GetScreenshot(0)->m_nWidth));
...
CPlaybackPanelMovie::CPlaybackPanelMovie(Panel *pParent, ReplayHandle_t hMovie) : CPlaybackPanel(pParent)
{
...
CCutImagePanel::CCutImagePanel(Panel *pParent, const char *pName)
	: BaseClass(pParent, pName, ""), m_pSelectedBorder(NULL)
{
}
...
void CCutsPanel::OnTick()
{
	...
	if(m_hDetailsPanel.Get())
	{
		...
		UpdateNameLabel(iHoverPerformance >= 0		? iHoverPerformance
						: iSelectedPerformance >= 0 ? iSelectedPerformance
													: -1);
	}
}
...
void CCutsPanel::UpdateNameLabel(int iPerformance)
{
	...
	const int iPerformanceButton =
		PerformanceToButton(iPerformance); // Not necessarily the selected button - can be hover button
	...
	if(m_pNameLabel)
	{
		...
		m_pNameLabel->SetBounds(nNameLabelX, nNameLabelY, GetWide() - nNameLabelX, GetTall() - nNameLabelY);
	}
}
...
{
public:
	CYouTubeGetStatsHandler(CReplayDetailsPanel *pPanel) : m_pPanel(pPanel), m_handle(NULL) {}
	...
	virtual void HandleResponse(long responseCode, const char *pResponse)
	{
		...
		g_pVGuiLocalize->ConstructString_safe(wszStats, g_pVGuiLocalize->Find("#YouTube_Stats"), 3, wszFavorited,
											  wszViews, wszLikes);
...
CReplayDetailsPanel::CReplayDetailsPanel(Panel *pParent, QueryableReplayItemHandle_t hReplayItem, int iPerformance,
										 IReplayItemManager *pItemManager)
	...
	if(m_pItemManager->AreItemsMovies())
	{
		...
		if(m_pYouTubeResponseHandler->m_handle == NULL)
		{
			...
			if(pReplayItem && pReplayItem->IsItemAMovie())
			{
				...
				{
					m_pYouTubeResponseHandler->m_handle =
						YouTube_GetVideoInfo(pMovie->GetUploadURL(), *m_pYouTubeResponseHandler);
...
void CReplayDetailsPanel::OnCommand(const char *pCommand)
{
	...
	{
		ReplayUI_GetBrowserPanel()->AttemptToDeleteReplayItem(this, m_hReplayItem, m_pItemManager,
															  m_iSelectedPerformance);
		return;
	}
	...
	else if(FStrEq(pCommand, "exportmovie"))
	{
		...
		{
#ifdef USE_WEBM_FOR_REPLAY
			CTFMessageBoxDialog *pDialog =
				ShowMessageBox("#Replay_Tip", "#Replay_UseVLCPlayer", "#Replay_ThanksIWill", OnPlayerWarningDlgConfirm);
#else
			CTFMessageBoxDialog *pDialog = ShowMessageBox("#Replay_Tip", "#Replay_UseQuickTimePlayer",
														  "#Replay_ThanksIWill", OnPlayerWarningDlgConfirm);
#endif
		...
		if(pReplayItem && pReplayItem->IsItemAMovie())
		{
			...
			else if(pMovie->IsUploaded())
			{
				CTFGenericConfirmDialog *pDialog =
					ShowConfirmDialog("#YouTube_Upload_Title", "#YouTube_FileAlreadyUploaded", "#GameUI_OK",
									  "#GameuI_CancelBold", &ConfirmUploadMovie, this);
				pDialog->SetContext(this);
			}
			else
			{
	...
	else if(FStrEq(pCommand, "viewyoutube"))
	{
		if(steamapicontext && steamapicontext->SteamFriends() &&
		   m_pYouTubeResponseHandler->m_strVideoURL.IsEmpty() == false)
		{
			steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage(
				m_pYouTubeResponseHandler->m_strVideoURL.Get());
		}
	}

	else if(FStrEq(pCommand, "shareyoutubeurl"))
	{
		system()->SetClipboardText(m_pYouTubeResponseHandler->m_strVideoURL.Get(),
								   m_pYouTubeResponseHandler->m_strVideoURL.Length());
...
void CReplayDetailsPanel::ShowRenderInfo()
{
	...
	wchar_t wszStr[1024];
	g_pVGuiLocalize->ConstructString_safe(wszStr, "#Replay_MovieRenderInfo", kvParams);
...
void CReplayDetailsPanel::OnMessage(const KeyValues *pParams, VPANEL hFromPanel)
{
	if(FStrEq(pParams->GetName(), "ReplayItemDeleted"))
	{
		...
		if(iPerformance >= 0)
		{
			CReplayPerformance *pPerformance =
				GetGenericClassBasedReplay(m_hReplay)->GetPerformance(m_iSelectedPerformance);
