...
static ConVar replay_ragdoll_dbg("replay_ragdoll_dbg", "0", FCVAR_CLIENTDLL,
								 "Display replay ragdoll debugging information.");
static ConVar replay_cache_client_ragdolls("replay_cache_client_ragdolls", "0", FCVAR_CLIENTDLL,
										   "Record ragdolls on the client during.",
										   OnReplayCacheClientRagdollsCvarChange);

//--------------------------------------------------------------------------------

void DrawBones(matrix3x4_t const *pBones, int nNumBones, ragdoll_t const *pRagdoll, int nRed, int nGreen, int nBlue,
			   C_BaseAnimating *pBaseAnimating)
{
	Assert(pBones);
	Assert(pRagdoll);
...
RagdollSimulationData_t::RagdollSimulationData_t(C_BaseAnimating *pEntity, int nStartTick, int nNumBones)
	: m_pEntity(pEntity), m_nEntityIndex(-1), m_nStartTick(nStartTick), m_nNumBones(nNumBones), m_nDuration(-1)
{
	if(pEntity)
	{
...
bool _ComputeRagdollBones(const ragdoll_t *pRagdoll, matrix3x4_t &parentTransform, matrix3x4_t *pBones,
						  Vector *pPositions, QAngle *pAngles)
{
...
void RagdollSimulationData_t::Record()
{
	...
	_ComputeRagdollBones(m_pEntity->m_pRagdoll->GetRagdoll(), parentTransform, pBones, pNewFrame->pPositions,
						 pNewFrame->pAngles);

	// Draw bones
	if(replay_ragdoll_dbg.GetBool())
	{
		DrawBones(pBones, m_pEntity->m_pRagdoll->RagdollBoneCount(), m_pEntity->m_pRagdoll->GetRagdoll(), 255, 0, 0,
				  m_pEntity);
	}
}
...
void CReplayRagdollRecorder::PrintDebug()
{
	...
	{
		engine->Con_NPrintf(nLine++, "entity %d: start time=%d  duration=%d  num bones=%d",
							m_lstRagdolls[i]->m_nEntityIndex, m_lstRagdolls[i]->m_nStartTick,
							m_lstRagdolls[i]->m_nDuration, m_lstRagdolls[i]->m_nNumBones);
	}
}
...
bool CReplayRagdollCache::GetFrame(C_BaseAnimating *pEntity, int nTick, bool *pBoneSimulated,
								   CBoneAccessor *pBoneAccessor) const
{
	...
	if(pNextFrame)
	{
		AngleMatrix((const QAngle &)Lerp(flInterpAmount, pFrame->angRootAngles,
										 pNextFrame->angRootAngles), // Actually does a slerp
					Lerp(flInterpAmount, pFrame->vRootPosition, pNextFrame->vRootPosition), rootTransform);
	}
	else
	{
	...
	for(int k = 0; k < pRagdoll->listCount; ++k)
	{
		...
		{
			// Get blended Eular angles - NOTE: The Lerp() here actually calls Lerp<QAngle>() which converts to quats
			// and back
...
RagdollSimulationData_t *CReplayRagdollCache::FindRagdollEntry(C_BaseAnimating *pEntity, int nTick)
{
	...
	FOR_EACH_LL(m_lstRagdolls, i)
	{
		...
		if(nTick < pRagdollData->m_nStartTick || nTick > pRagdollData->m_nStartTick + pRagdollData->m_nDuration)
			return NULL;
									...
									const RagdollSimulationData_t *pRagdollEntry, int nTick)
{
	...
	FOR_EACH_LL(pRagdollEntry->m_lstFrames, j)
	{
		...
		// Use this frame?
		if(nTick >= pFrame->nTick && ((pNextFrame && nTick <= pNextFrame->nTick) ||
									  !pNextFrame)) // Use the last frame if the tick is past the range of frames -
...
void CReplayRagdollCache::Think()
{
	...
	FOR_EACH_LL( m_lstRagdolls, i )
	{
		engine->Con_NPrintf( 10 + i, "entity %d: start time=%d  duration=%d  num bones=%d",
	m_lstRagdolls[i]->m_nEntityIndex, m_lstRagdolls[i]->m_nStartTick, m_lstRagdolls[i]->m_nDuration,
	m_lstRagdolls[i]->m_nNumBones );
	}
	*/
}
