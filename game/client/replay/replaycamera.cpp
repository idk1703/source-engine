...
ConVar replay_editor_camera_length(
	"replay_editor_camera_length", "15", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
	"This is the camera length used to simulate camera shake in the replay editor.  The larger this number, the more "
	"the actual position will change.  It can also be set to negative values.");

// ConVar spec_autodirector( "spec_autodirector", "1", FCVAR_CLIENTDLL | FCVAR_CLIENTCMD_CAN_EXECUTE, "Auto-director
// chooses best view modes while spectating" );
...
void C_ReplayCamera::CalcRoamingView(Vector &eyeOrigin, QAngle &eyeAngles, float &fov, float flDelta)
{
	...
	if(!ShouldUseDefaultRoamingSettings() && m_flRoamingShakeAmount > 0.0f)
	{
		...
		// Vertical shake
		const float flAmplitudeX =
			m_flRoamingShakeAmount * (m_flRoamingShakeDir < 0.0f ? (1.0f + m_flRoamingShakeDir) : 1.0f);
		angShake.x = flAmplitudeX * flNoiseX;

		// Lateral shake
		const float flAmplitudeY =
			m_flRoamingShakeAmount * (m_flRoamingShakeDir > 0.0f ? (1.0f - m_flRoamingShakeDir) : 1.0f);
		angShake.y = flAmplitudeY * flNoiseY;

		// The math below simulates a camera with length "replay_editor_camera_length," so that the camera position
		// bounces around as if it were on someone's shoulder.  If we were to just use angShake at this point with no
		// translation, we would get a camera that looks around but is anchored and doesn't feel quite right.  With the
		// code below, the camera will translate, but be centered around the same point as when camera shake is off
		// completely, rather than actually offsetting that point by the camera length.
...
{
	m_flRoamingFov[0] = clamp(Lerp(7 * flDelta, m_flRoamingFov[0], m_flRoamingFov[1]),
							  //		Approach( m_flRoamingFov[1], m_flRoamingFov[0], 40 * m_flFrameTime ),
							  FREE_CAM_FOV_MIN, FREE_CAM_FOV_MAX);
}
...
void C_ReplayCamera::FixupMovmentParents()
{
	...
	for(ClientEntityHandle_t e = ClientEntityList().FirstHandle(); e != ClientEntityList().InvalidHandle();
