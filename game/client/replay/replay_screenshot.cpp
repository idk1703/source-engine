...
CReplayScreenshotTaker::CReplayScreenshotTaker(IViewRender *pViewRender, CViewSetup &viewSetup)
	: m_pViewRender(pViewRender), m_View(viewSetup)
{
...
void CReplayScreenshotTaker::TakeScreenshot(WriteReplayScreenshotParams_t &params)
{
	...
	timer.End();
	if(bDbg)
		Warning("Screenshot RenderView(): %.4f s\n", timer.GetDuration().GetSeconds());
	...
	timer.End();
	if(bDbg)
		Warning("Screenshot ReadPixels(): %.4f s\n", timer.GetDuration().GetSeconds());
	...
	timer.End();
	if(bDbg)
		Warning("Screenshot copy image: %.4f s\n", timer.GetDuration().GetSeconds());
	...
	timer.End();
	if(bDbg)
		Warning("Screenshot VTF->Serialize(): %.4f s\n", timer.GetDuration().GetSeconds());
	...
	timer.End();
	if(bDbg)
		Warning("Screenshot AsyncWrite(): %.4f s\n", timer.GetDuration().GetSeconds());
...
{
	m_pScreenshotTarget = pMaterialSystem->CreateNamedRenderTargetTextureEx2(
		"rt_ReplayScreenshot", 0, 0, RT_SIZE_REPLAY_SCREENSHOT, IMAGE_FORMAT_RGB888, MATERIAL_RT_DEPTH_SEPARATE);
	m_pScreenshotTarget
		->AddRef(); // we will leak this ref, but only at shutdown of the app, which will be cleaned up then
}
