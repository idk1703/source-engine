...
static Vector2D g_vJitterTable4[4] = {Vector2D(0.5318f, -0.6902f), Vector2D(-0.5123f, 0.8362f),
									  Vector2D(-0.5193f, -0.2195f), Vector2D(0.4749f, 0.3478f)};

// 8-entry table of values in 2D -1 to +1 range using Poisson disk distribution
static Vector2D g_vJitterTable8[8] = {Vector2D(0.3475f, 0.0042f),	Vector2D(0.8806f, 0.3430f),
									  Vector2D(-0.0041f, -0.6197f), Vector2D(0.0472f, 0.4964f),
									  Vector2D(-0.3730f, 0.0874f),	Vector2D(-0.9217f, -0.3177f),
									  Vector2D(-0.6289f, 0.7388f),	Vector2D(0.5744f, -0.7741f)};

// 16-entry table of values in 2D -1 to +1 range using Poisson disk distribution (disk size 0.38f)
static Vector2D g_vJitterTable16[16] = {
	Vector2D(0.0747f, -0.8341f),  Vector2D(-0.9138f, 0.3251f),	Vector2D(0.8667f, -0.3029f),
	Vector2D(-0.4642f, 0.2187f),  Vector2D(-0.1505f, 0.7320f),	Vector2D(0.7310f, -0.6786f),
	Vector2D(0.2859f, -0.3254f),  Vector2D(-0.1311f, -0.2292f), Vector2D(0.3518f, 0.6470f),
	Vector2D(-0.7485f, -0.6307f), Vector2D(0.1687f, 0.1873f),	Vector2D(-0.3604f, -0.7483f),
	Vector2D(-0.5658f, -0.1521f), Vector2D(0.7102f, 0.0536f),	Vector2D(-0.6056f, 0.7747f),
	Vector2D(0.7793f, 0.6194f)};

// 32-entry table of values in 2D -1 to +1 range using Poisson disk distribution (disk size 0.28f)
static Vector2D g_vJitterTable32[32] = {
	Vector2D(0.0854f, -0.0644f),  Vector2D(0.8744f, 0.1665f),	Vector2D(0.2329f, 0.3995f),
	Vector2D(-0.7804f, 0.5482f),  Vector2D(-0.4577f, 0.7647f),	Vector2D(-0.1936f, 0.5564f),
	Vector2D(0.4205f, -0.5768f),  Vector2D(-0.0304f, -0.9050f), Vector2D(-0.5215f, 0.1854f),
	Vector2D(0.3161f, -0.2954f),  Vector2D(0.0666f, -0.5564f),	Vector2D(-0.2137f, -0.0072f),
	Vector2D(-0.4112f, -0.3311f), Vector2D(0.6438f, -0.2484f),	Vector2D(-0.9055f, -0.0360f),
	Vector2D(0.8323f, 0.5268f),	  Vector2D(0.5592f, 0.3459f),	Vector2D(-0.6797f, -0.5201f),
	Vector2D(-0.4325f, -0.8857f), Vector2D(0.8768f, -0.4197f),	Vector2D(0.3090f, -0.8646f),
	Vector2D(0.5034f, 0.8603f),	  Vector2D(0.3752f, 0.0627f),	Vector2D(-0.0161f, 0.2627f),
	Vector2D(0.0969f, 0.7054f),	  Vector2D(-0.2291f, -0.6595f), Vector2D(-0.5887f, -0.1100f),
	Vector2D(0.7048f, -0.6528f),  Vector2D(-0.8438f, 0.2706f),	Vector2D(-0.5061f, 0.4653f),
	Vector2D(-0.1245f, -0.3302f), Vector2D(-0.1801f, 0.8486f)};
...
void CAccumBuffProxy::OnBind(void *pC_BaseEntity)
{
	...
	{
		m_pAccumBuffWeights->SetVecValue(pAccumParams->m_fSampleWeight, 1.0f - pAccumParams->m_fSampleWeight, 0.0f,
										 0.0f);
	}
}
...
bool CReplayRenderer::SetupRenderer(RenderMovieParams_t &params, IReplayMovie *pMovie)
{
	...
	else
	{
		...
		if(m_pMovieMaker->CreateNewMovieFile(fmtMovieFullFilename.Access(), true))
		{
			...
			VideoEncodeGamma_t encodeGamma =
				(QTEncodeGamma.IsValid()) ? (VideoEncodeGamma_t)QTEncodeGamma.GetInt() : VideoEncodeGamma::GAMMA_2_2;
#else
			VideoEncodeGamma_t encodeGamma = VideoEncodeGamma::GAMMA_2_2;
#endif

			if(m_pMovieMaker->SetMovieVideoParameters(Settings.m_Codec, Settings.m_nEncodingQuality,
													  (int)Settings.m_nWidth, (int)Settings.m_nHeight, Settings.m_FPS,
													  encodeGamma))
			{
				if(m_pMovieMaker->SetMovieSourceImageParameters(VideoEncodeSourceFormat::BGRA_32BIT,
																(int)Settings.m_nWidth, (int)Settings.m_nHeight))
				{
					AudioEncodeOptions_t audioOptions = AudioEncodeOptions::USE_AUDIO_ENCODE_GROUP_SIZE |
														AudioEncodeOptions::GROUP_SIZE_IS_VIDEO_FRAME |
														AudioEncodeOptions::LIMIT_AUDIO_TRACK_TO_VIDEO_DURATION |
														AudioEncodeOptions::PAD_AUDIO_WITH_SILENCE;

					if(m_pMovieMaker->SetMovieSourceAudioParameters(AudioEncodeSourceFormat::AUDIO_16BIT_PCMStereo,
																	44100, audioOptions))
					{
						bSuccess = true;
					}
...
void CReplayRenderer::InitBuffers(const RenderMovieParams_t &params)
{
	...
	ImageFormat AccumSampleFormat = (g_pMaterialSystemHardwareConfig->GetHDRType() == HDR_TYPE_FLOAT)
										? IMAGE_FORMAT_RGBA16161616F
										: g_pMaterialSystem->GetBackBufferFormat();
	m_AccumBuffSample.Init(g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
		"_rt_Replay_Accum_Sample", Settings.m_nWidth, Settings.m_nHeight, RT_SIZE_OFFSCREEN, AccumSampleFormat,
		MATERIAL_RT_DEPTH_SHARED, TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_POINTSAMPLE));
	...
	m_AccumBuffPingPong[0].Init(g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
		"_rt_Replay_Ping", Settings.m_nWidth, Settings.m_nHeight, RT_SIZE_OFFSCREEN, PingPongFormat,
	...
	m_AccumBuffPingPong[1].Init(g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
		"_rt_Replay_Pong", Settings.m_nWidth, Settings.m_nHeight, RT_SIZE_OFFSCREEN, PingPongFormat,
		MATERIAL_RT_DEPTH_NONE, TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_POINTSAMPLE));
	...
	m_LayoffResult.Init(g_pMaterialSystem->CreateNamedRenderTargetTextureEx2(
		...
		TEXTUREFLAGS_BORDER | TEXTUREFLAGS_POINTSAMPLE));
...
void CReplayRenderer::ResolveSamples(int nSample, DmeTime_t frametime, int x, int y, int nWidth, int nHeight,
									 bool bLayoffResult, float flBloomScale)
{
	CMatRenderContextPtr pRenderContext(g_pMaterialSystem);

	// Render resolving quad to current ping-pong buffer
	AccumParams_t accParms = {m_AccumBuffSample, m_AccumBuffPingPong[(m_nCurrentPingPong + 1) % 2],
							  1.0f / (float)(nSample + 1), false};
...
void CReplayRenderer::SetupSampleView(int x, int y, int w, int h, int nSample, CViewSetup &viewSetup)
{
	...
	SetupDOFMatrixSkewView(viewSetup.origin, viewSetup.angles, nSample,
						   viewSetup); // Sheared matrix method more comparable to image-space DoF approximation
...
void CReplayRenderer::SetupDOFMatrixSkewView(const Vector &pos, const QAngle &angles, int nSample,
...
void CReplayRenderer::LayoffFrame(int nFrame)
{
	...
	pRenderContext->ReadPixels(0, 0, (int)m_RenderParams.m_Settings.m_nWidth, (int)m_RenderParams.m_Settings.m_nHeight,
							   (unsigned char *)m_pLayoffBuf, IMAGE_FORMAT_BGRA8888);
...
void CReplayRenderer::GetViewSetup(CViewSetup &viewsetup)
{
	...
	viewsetup.fovViewmodel =
		ScaleFOVByWidthRatio(v_viewmodel_fov.GetFloat(), viewsetup.m_flAspectRatio / (4.0f / 3.0f));
}
