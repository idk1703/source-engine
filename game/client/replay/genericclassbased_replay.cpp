...
CGenericClassBasedReplay::CGenericClassBasedReplay() : m_nPlayerTeam(0)
{
	m_szKillerName[0] = 0;
...
bool CGenericClassBasedReplay::Read(KeyValues *pIn)
{
	...
	if(pKills)
	{
		FOR_EACH_TRUE_SUBKEY(pKills, pKill)
		{
			...
			AddKill(pKill->GetString("victim_name"), pKill->GetInt("victim_class"));
		}
	}
...
void CGenericClassBasedReplay::AddKillStats(CUtlVector<GenericStatInfo_t *> &vecKillStats, KeyValues *pIn,
											const char *pSubKeyName, int iStatIndex)
{
...
void CGenericClassBasedReplay::WriteKillStatVector(CUtlVector<CGenericClassBasedReplay::GenericStatInfo_t *> const &vec,
												   const char *pSubKeyName, const char *pElementKeyName,
...
void CGenericClassBasedReplay::AddKill(const char *pPlayerName, int nPlayerClass)
{
	...
	if(replay_debug.IsValid() && replay_debug.GetBool())
	{
		DevMsg("\n\nRecorded kill: name=%s, class=%s (this=%i)\n\n", pPlayerName, GetPlayerClass(nPlayerClass),
			   (int)this);
	}
...
void CGenericClassBasedReplay::AddKillStatFromUserIds(CUtlVector<GenericStatInfo_t *> &vec, int nVictimId,
													  int nAssisterId /*=0*/)
{
...
void CGenericClassBasedReplay::AddKillStatFromFriendIds(CUtlVector<GenericStatInfo_t *> &vec, uint32 nVictimFriendId,
														uint32 nAssisterFriendId /*=0*/)
{
	GenericStatInfo_t *pNewKillStat = new GenericStatInfo_t;
...
{
	player_info_t pi;
	if(!steamapicontext->SteamFriends() || !steamapicontext->SteamUtils() || !engine->GetPlayerInfo(nPlayerIndex, &pi))
	{
