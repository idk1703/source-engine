...
C_EntityFlame *FireEffect(C_BaseAnimating *pTarget, C_BaseEntity *pServerFire, float *flScaleEnd, float *flTimeStart,
						  float *flTimeEnd);
...
ConVar vcollide_wireframe("vcollide_wireframe", "0", FCVAR_CHEAT, "Render physics collision models in wireframe",
...
RecvPropInt(RECVINFO(m_nSequence), 0, RecvProxy_Sequence), RecvPropInt(RECVINFO(m_nForceBone)),
	RecvPropVector(RECVINFO(m_vecForce)), RecvPropInt(RECVINFO(m_nSkin)), RecvPropInt(RECVINFO(m_nBody)),
	...
	RecvPropInt(RECVINFO(m_bClientSideAnimation)), RecvPropInt(RECVINFO(m_bClientSideFrameReset)),

	RecvPropInt(RECVINFO(m_nNewSequenceParity)), RecvPropInt(RECVINFO(m_nResetEventsParity)),
	RecvPropInt(RECVINFO(m_nMuzzleFlashParity)),

	RecvPropEHandle(RECVINFO(m_hLightingOrigin)), RecvPropEHandle(RECVINFO(m_hLightingOriginRelative)),

	RecvPropDataTable("serveranimdata", 0, 0, &REFERENCE_RECV_TABLE(DT_ServerAnimationData)),

	RecvPropFloat(RECVINFO(m_fadeMinDist)), RecvPropFloat(RECVINFO(m_fadeMaxDist)),
...
BEGIN_DATADESC(C_ClientRagdoll)
	DEFINE_FIELD(m_bFadeOut, FIELD_BOOLEAN), DEFINE_FIELD(m_bImportant, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iCurrentFriction, FIELD_INTEGER), DEFINE_FIELD(m_iMinFriction, FIELD_INTEGER),
		DEFINE_FIELD(m_iMaxFriction, FIELD_INTEGER), DEFINE_FIELD(m_flFrictionModTime, FIELD_FLOAT),
		DEFINE_FIELD(m_flFrictionTime, FIELD_TIME), DEFINE_FIELD(m_iFrictionAnimState, FIELD_INTEGER),
		DEFINE_FIELD(m_bReleaseRagdoll, FIELD_BOOLEAN), DEFINE_FIELD(m_nBody, FIELD_INTEGER),
		DEFINE_FIELD(m_nSkin, FIELD_INTEGER), DEFINE_FIELD(m_nRenderFX, FIELD_CHARACTER),
		DEFINE_FIELD(m_nRenderMode, FIELD_CHARACTER), DEFINE_FIELD(m_clrRender, FIELD_COLOR32),
		DEFINE_FIELD(m_flEffectTime, FIELD_TIME), DEFINE_FIELD(m_bFadingOut, FIELD_BOOLEAN),

		DEFINE_AUTO_ARRAY(m_flScaleEnd, FIELD_FLOAT), DEFINE_AUTO_ARRAY(m_flScaleTimeStart, FIELD_FLOAT),
		DEFINE_AUTO_ARRAY(m_flScaleTimeEnd, FIELD_FLOAT), DEFINE_EMBEDDEDBYREF(m_pRagdoll),
...
void C_ClientRagdoll::FadeOut(void)
{
	...
	int iFadeSpeed =
		(g_RagdollLVManager.IsLowViolence()) ? g_ragdoll_lvfadespeed.GetInt() : g_ragdoll_fadespeed.GetInt();
...
// Purpose: clear out any face/eye values stored in the material system
//-----------------------------------------------------------------------------
void C_ClientRagdoll::SetupWeights(const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights,
...
void C_ClientRagdoll::Release(void)
{
	...
	{
		::partition->Remove(PARTITION_CLIENT_SOLID_EDICTS | PARTITION_CLIENT_RESPONSIVE_EDICTS |
...
void C_BaseAnimating::UpdateRelevantInterpolatedVars()
{
	...
	if(!IsMarkedForDeletion() && !GetPredictable() && !IsClientCreated() && GetModelPtr() &&
...
void C_BaseAnimating::RemoveBaseAnimatingInterpolatedVars()
{
	...
	//  and so needs to continue to be interpolated for smooth rendering of the lower body of the local player in third
	//  person, etc.
	if(!GetPredictable())
#endif
	{
...
void C_BaseAnimating::LockStudioHdr()
{
	...
	{
		Assert(m_pStudioHdr ? m_pStudioHdr->GetRenderHdr() == mdlcache->GetStudioHdr(m_hStudioHdr)
							: m_hStudioHdr == MDLHANDLE_INVALID);
		return;
	}
...
CStudioHdr *C_BaseAnimating::OnNewModel()
{
	...
	for(i = 0; i < hdr->GetNumPoseParameters(); i++)
	{
		...
		// Note:  We can't do this since if we get a DATA_UPDATE_CREATED (i.e., new entity) with both a new model and
		// some valid pose parameters this will slam the
		//  pose parameters to zero and if the model goes dormant the pose parameter field will never be set to the true
		//  value.  We shouldn't have to zero these out as they are under the control of the server and should be
		//  properly set
		if(!IsServerEntity())
		{
...
void C_BaseAnimating::DelayedInitModelEffects(void)
{
	...
	if(modelKeyValues->LoadFromBuffer(modelinfo->GetModelName(GetModel()), modelinfo->GetModelKeyValueText(GetModel())))
	{
		...
		if(pkvAllCables)
		{
			...
			for(KeyValues *pSingleCable = pkvAllCables->GetFirstSubKey(); pSingleCable;
				pSingleCable = pSingleCable->GetNextKey())
			{
		...
		if(!m_bNoModelParticles)
		{
			...
			{
				// Start grabbing the sounds and slotting them in
				for(KeyValues *pSingleEffect = pkvAllParticleEffects->GetFirstSubKey(); pSingleEffect;
					pSingleEffect = pSingleEffect->GetNextKey())
				{
					...
					if(iAttachType == -1)
					{
						Warning("Invalid attach type specified for particle effect in model '%s' keyvalues section. "
					...
					if(pszAttachment[0] != '0' && iAttachment == 0)
					{
						...
						if(iAttachment <= 0)
						{
							Warning("Failed to find attachment point specified for particle effect in model '%s' "
									"keyvalues section. Trying to spawn effect '%s' on attachment named '%s'\n",
					...
					if(!V_strcmp("sentry_rocket", pszParticleEffect))
					{
						...
						if(TF_IsHolidayActive(kHoliday_HalloweenOrFullMoon))
						{
							...
							if(GetOwnerEntity() && GetOwnerEntity()->IsBaseObject())
							{
								...
								CALL_ATTRIB_HOOK_INT_ON_OTHER(pSentry->GetOwner(), iHalloweenSpell,
															  halloween_pumpkin_explosions);
							}
							else
							{
								CALL_ATTRIB_HOOK_INT_ON_OTHER(GetOwnerEntity(), iHalloweenSpell,
															  halloween_pumpkin_explosions);
							}
						}
...
//-----------------------------------------------------------------------------
void C_BaseAnimating::BuildTransformations(CStudioHdr *hdr, Vector *pos, Quaternion *q,
										   const matrix3x4_t &cameraTransform, int boneMask, CBoneBitList &boneComputed)
{
	...
	if(m_pRagdoll)
	{
		...
		// If we're playing back a demo, override the ragdoll bones with cached version if available - otherwise,
		// simulate.
		if((!engine->IsPlayingDemo() && !engine->IsPlayingTimeDemo()) ||
		   !CReplayRagdollCache::Instance().IsInitialized() ||
		   !CReplayRagdollCache::Instance().GetFrame(this, engine->GetDemoPlaybackTick(), boneSimulated,
													 &m_BoneAccessor))
#endif
		{
	...
	for(int i = 0; i < hdr->numbones(); i++)
	{
		...
		else
		{
			...
			if((hdr->boneFlags(i) & BONE_ALWAYS_PROCEDURAL) && (hdr->pBone(i)->proctype & STUDIO_PROC_JIGGLE))
			{
				...
				// do jiggle physics
				m_pJiggleBones->BuildJiggleTransformations(i, gpGlobals->realtime, jiggleInfo, goalMX,
														   GetBoneForWrite(i));
			}
...
void C_BaseAnimating::MaintainSequenceTransitions(IBoneSetup &boneSetup, float flCycle, Vector pos[], Quaternion q[])
{
	...
	m_SequenceTransitioner.CheckForSequenceChange(boneSetup.GetStudioHdr(), GetSequence(),
	...
	m_SequenceTransitioner.UpdateCurrent(boneSetup.GetStudioHdr(), GetSequence(), flCycle, m_flPlaybackRate,
										 gpGlobals->curtime);
...
#if 1 // _DEBUG
		if(/*Q_stristr( hdr->pszName(), r_sequence_debug.GetString()) != NULL || */ r_sequence_debug.GetInt() ==
		   entindex())
		{
			DevMsgRT("%8.4f : %30s : %5.3f : %4.2f  +\n", gpGlobals->curtime,
					 boneSetup.GetStudioHdr()->pSeqdesc(blend->m_nSequence).pszLabel(), flCycle,
...
void C_BaseAnimating::ChildLayerBlend(Vector pos[], Quaternion q[], float currentTime, int boneMask)
{
	...
	for(C_BaseEntity *pChild = FirstMoveChild(); pChild; pChild = pChild->NextMovePeer())
	{
		...
		if(pChildAnimating)
		{
			...
			if(pChildHdr && pChild->IsEffectActive(EF_BONEMERGE) && pChildHdr->SequencesAvailable() &&
			   pChildAnimating->m_pBoneMergeCache)
			{
...
//-----------------------------------------------------------------------------
void C_BaseAnimating::StandardBlendingRules(CStudioHdr *hdr, Vector pos[], Quaternion q[], float currentTime,
											int boneMask)
{
	...
	// debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), 0, 0, "%30s %6.2f : %6.2f", hdr->pSeqdesc(
	// GetSequence() )->pszLabel( ), fCycle, 1.0 );
	...
	for(int i = 0; i < targetCount; i++)
	{
		...
		switch(pTarget->type)
		{
			case IK_GROUND:
			{
				...
				{
					pTarget->SetOwner(trace.m_pEnt->entindex(), trace.m_pEnt->GetAbsOrigin(),
				...
				debugoverlay->AddBoxOverlay( pTarget->est.pos, Vector( -r, -r, -1 ), Vector( r, r, 1), QAngle( 0, 0, 0
				), 255, 0, 0, 0, 0 );
				*/
				// debugoverlay->AddBoxOverlay( pTarget->latched.pos, Vector( -2, -2, 2 ), Vector( 2, 2, 6), QAngle( 0,
				// 0, 0 ), 0, 255, 0, 0, 0 );
			}
			break;
			...
			case IK_ATTACHMENT:
			{
				...
				for(CEntitySphereQuery sphere(pTarget->est.pos, 64); (pEntity = sphere.GetCurrentEntity()) != NULL;
					sphere.NextEntity())
				{
					...
					// debugoverlay->AddBoxOverlay( origin, Vector( -1, -1, -1 ), Vector( 1, 1, 1 ), QAngle( 0, 0, 0 ),
					// 255, 0, 0, 0, 0 );
					...
					// debugoverlay->AddBoxOverlay( pTarget->est.pos, Vector( -pTarget->est.radius,
					// -pTarget->est.radius, -pTarget->est.radius ), Vector( pTarget->est.radius, pTarget->est.radius,
				...
				if(flDist >= pTarget->est.radius)
				{
					// debugoverlay->AddBoxOverlay( pTarget->est.pos, Vector( -pTarget->est.radius,
					// -pTarget->est.radius, -pTarget->est.radius ), Vector( pTarget->est.radius, pTarget->est.radius,
					// pTarget->est.radius), QAngle( 0, 0, 0 ), 0, 0, 255, 0, 0 ); no solution, disable ik rule
					pTarget->IKFailed();
				}
			}
			break;
...
void C_BaseAnimating::ThreadedBoneSetup()
{
	...
	if(g_bDoThreadedBoneSetup)
	{
		...
		if(nCount > 1)
		{
			...
			ParallelProcess("C_BaseAnimating::ThreadedBoneSetup", g_PreviousBoneSetups.Base(), nCount,
							&SetupBonesOnBaseAnimating, &PreThreadedBoneSetup, &PostThreadedBoneSetup);
...
bool C_BaseAnimating::SetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime)
{
	...
	// boneMask = BONE_USED_BY_ANYTHING; // HACK HACK - this is a temp fix until we have accessors for bones to find out
	// where problems are.
	...
	if(g_bDoThreadedBoneSetup && !g_bInThreadedBoneSetup && (nBoneCount >= 16) && !GetMoveParent() &&
	...
	if((m_BoneAccessor.GetReadableBones() & boneMask) != boneMask)
	{
		...
		else
		{
			...
			// NOTE: For model scaling, we need to opt out of IK because it will mark the bones as already being
			// calculated
			if(!IsModelScaled())
			{
			...
			int bonesMaskNeedRecalc =
				boneMask | oldReadableBones; // Hack to always recalc bones, to fix the arm jitter in the new CS player
			...
			if(m_pIk)
			{
				...
				m_pIk->Init(hdr, GetRenderAngles(), GetRenderOrigin(), currentTime, gpGlobals->framecount,
							bonesMaskNeedRecalc);
			}
...
C_BaseAnimating *C_BaseAnimating::FindFollowedEntity()
{
	...
	if(modelinfo->GetModelType(follow->GetModel()) != mod_studio)
	{
		Warning("Attached %s (mod_studio) to %s (%d)\n", modelinfo->GetModelName(GetModel()),
				modelinfo->GetModelName(follow->GetModel()), modelinfo->GetModelType(follow->GetModel()));
		return NULL;
	}
...
void C_BaseAnimating::PushAllowBoneAccess(bool bAllowForNormalModels, bool bAllowForViewModels, char const *tagPush)
{
	...
	Assert(g_BoneAccessStack.Count() < 32); // Most likely we are leaking "PushAllowBoneAccess" calls if PopBoneAccess
											// is never called. Consider using AutoAllowBoneAccess.
...
void C_BaseAnimating::PopBoneAccess(char const *tagPop)
{
	...
	Assert((g_BoneAcessBase.tag == tagPop) ||
		   (g_BoneAcessBase.tag && g_BoneAcessBase.tag != (char const *)1 && tagPop && tagPop != (char const *)1 &&
...
int C_BaseAnimating::DrawModel(int flags)
{
	...
	if(r_drawothermodels.GetInt())
	{
		...
		if((flags & (STUDIO_SSAODEPTHTEXTURE | STUDIO_SHADOWDEPTHTEXTURE)) == 0 && g_pStudioStatsEntity != NULL &&
		   g_pStudioStatsEntity == GetClientRenderable())
		{
...
//
//-----------------------------------------------------------------------------
void C_BaseAnimating::DoInternalDrawModel(ClientModelRenderInfo_t *pInfo, DrawModelState_t *pState,
...
{
public:
	MaterialOverrideRestore() : m_bRestore(false), m_pOverrideMaterial(NULL), m_nOverrideType(OVERRIDE_NORMAL) {}
...
int C_BaseAnimating::InternalDrawModel(int flags)
{
	...
	// This should never happen, but if the server class hierarchy has bmodel entities derived from CBaseAnimating or
	// does a
...
void C_BaseAnimating::DoAnimationEvents(CStudioHdr *pStudioHdr)
{
	...
	if(m_nEventSequence != GetSequence() || resetEvents)
	{
		if(watch)
		{
			Msg("new seq: %i - old seq: %i - reset: %s - m_flCycle %f - Model Name: %s - (time %.3f)\n", GetSequence(),
				m_nEventSequence, resetEvents ? "true" : "false", GetCycle(), pStudioHdr->pszName(),
				gpGlobals->curtime);
		}
	...
	if(watch)
	{
		Msg("%i (seq %d cycle %.3f ) evcycle %.3f prevevcycle %.3f (time %.3f)\n", gpGlobals->tickcount, GetSequence(),
			GetCycle(), flEventCycle, m_flPrevEventCycle, gpGlobals->curtime);
	}
	...
	if(bLooped)
	{
		for(int i = 0; i < (int)seqdesc.numevents; i++)
		{
			...
			if(watch)
			{
				Msg("%i FE %i Looped cycle %f, prev %f ev %f (time %.3f)\n", gpGlobals->tickcount, pevent[i].event,
					pevent[i].cycle, m_flPrevEventCycle, flEventCycle, gpGlobals->curtime);
			}
		...
		if((pevent[i].cycle > m_flPrevEventCycle && pevent[i].cycle <= flEventCycle))
		{
			if(watch)
			{
				Msg("%i (seq: %d) FE %i Normal cycle %f, prev %f ev %f (time %.3f)\n", gpGlobals->tickcount,
					GetSequence(), pevent[i].event, pevent[i].cycle, m_flPrevEventCycle, flEventCycle,
					gpGlobals->curtime);
			}
...
void C_BaseAnimating::FireEvent(const Vector &origin, const QAngle &angles, int event, const char *options)
{
	switch(event)
	{
		case AE_CL_CREATE_PARTICLE_EFFECT:
		{
			...
			{
				Warning("Invalid options specified for AE_CL_CREATE_PARTICLE_EFFECT event. Must specify \"<particle "
						"effect name> <attachment type> <attachment point>\"\n");
				return;
			}
			...
			if(iAttachType == -1)
			{
				Warning("Invalid attach type specified for AE_CL_CREATE_PARTICLE_EFFECT event. Trying to spawn effect "
			...
			if(splitString.Count() > 2)
			{
				...
				if(pszAttachment[0] != '0' && iAttachment == 0)
				{
					...
					if(iAttachment <= 0)
					{
						Warning("Failed to find attachment point specified for AE_CL_CREATE_PARTICLE_EFFECT event. "
								"Trying to spawn effect '%s' on attachment named '%s'\n",
		...
		case AE_CL_REMOVE_PARTICLE_EFFECT:
		{
			...
			{
				Warning("Invalid options specified for AE_CL_REMOVE_PARTICLE_EFFECT event. Must specify \"<particle "
						"effect name> <attachment point>\"\n");
				return;
			}
			...
			if(splitString.Count() > 1)
			{
				...
				if(pszAttachment[0] != '0' && iAttachment == 0)
				{
					...
					if(iAttachment <= 0)
					{
						Warning("Failed to find attachment point specified for AE_CL_REMOVE_PARTICLE_EFFECT event. "
								"Trying to spawn effect '%s' on attachment named '%s'\n",
								szParticleEffect, pszAttachment);
						return;
					}
...
void C_BaseAnimating::FireObsoleteEvent(const Vector &origin, const QAngle &angles, int event, const char *options)
{
	...
	switch(event)
	{
		// Obsolete. Use the AE_CL_CREATE_PARTICLE_EFFECT event instead, which uses the artist driven particle system &
		// editor.
		case AE_CLIENT_EFFECT_ATTACH:
		{
			int iAttachment;
...
bool C_BaseAnimating::IsSelfAnimating()
{
	...
	if(iMoveType != MOVETYPE_STEP && iMoveType != MOVETYPE_NONE && iMoveType != MOVETYPE_WALK &&
	   iMoveType != MOVETYPE_FLY && iMoveType != MOVETYPE_FLYGRAVITY)
	{
		return true;
	}
...
bool C_BaseAnimating::GetRagdollInitBoneArrays(matrix3x4_t *pDeltaBones0, matrix3x4_t *pDeltaBones1,
											   matrix3x4_t *pCurrentBones, float boneDt)
{
	bool bSuccess = true;
...
bool C_BaseAnimating::InitAsClientRagdoll(const matrix3x4_t *pDeltaBones0, const matrix3x4_t *pDeltaBones1,
										  const matrix3x4_t *pCurrentBonePosition, float boneDt, bool bFixedConstraints)
{
	CStudioHdr *hdr = GetModelPtr();
	...
	m_pRagdoll = CreateRagdoll(this, hdr, m_vecForce, m_nForceBone, pDeltaBones0, pDeltaBones1, pCurrentBonePosition,
							   boneDt, bFixedConstraints);
	...
	if(m_pRagdoll && pReplayEnable && pReplayEnable->GetInt() && !engine->IsPlayingDemo() &&
...
void C_BaseAnimating::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if(IsNoInterpolationFrame() && m_pRagdollInfo && m_pRagdollInfo->m_bActive)
	{
...
void C_BaseAnimating::ClientSideAnimationChanged()
{
	...
	m_SequenceTransitioner.CheckForSequenceChange(
		GetModelPtr(), GetSequence(), m_nNewSequenceParity != m_nPrevNewSequenceParity, !IsNoInterpolationFrame());
}
...
bool C_BaseAnimating::TestHitboxes(const Ray_t &ray, unsigned int fContentsMask, trace_t &tr)
{
	...
	if(TraceToStudio(physprops, ray, pStudioHdr, set, hitboxbones, fContentsMask, GetRenderOrigin(), GetModelScale(),
					 tr))
	{
...
float C_BaseAnimating::FrameAdvance(float flInterval)
{
	...
	if(GetServerIntendedCycle() != -1.0f)
	{
		...
		bool adjustOkay =
			serverAdvance > 0.0f; // only want to go forward. backing up looks really jarring, even when slight
		...
		if(adjustOkay)
		{
			...
			DevMsg(2, "(%d): Cycle latch used to correct %.2f in to %.2f instead of %.2f.\n", entindex(), GetCycle(),
				   GetCycle() + addcycle, GetCycle() + originalAdvance);
		}
	...
	if(bWatch)
	{
		Msg("%i CLIENT Time: %6.3f : (Interval %f) : cycle %f rate %f add %f\n", gpGlobals->tickcount,
			gpGlobals->curtime, flInterval, flNewCycle, cyclerate, addcycle);
	}
...
static Vector hullcolor[8] = {Vector(1.0, 1.0, 1.0), Vector(1.0, 0.5, 0.5), Vector(0.5, 1.0, 0.5),
...
int C_BaseAnimating::SelectWeightedSequenceFromModifiers(Activity activity, CUtlSymbol *pActivityModifiers,
														 int iModifierCount)
{
	Assert(activity != ACT_INVALID);
	Assert(GetModelPtr());
...
CBoneList *C_BaseAnimating::RecordBones(CStudioHdr *hdr, matrix3x4_t *pBoneState)
{
	...
	// Possible optimization: Instead of inverting everything while recording, record the pos/q stuff into a structure
	// instead?
	Assert(hdr);
