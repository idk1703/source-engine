...
void CAchievementNotificationPanel::PerformLayout(void)
{
	...
	// Set background color of various elements.  Need to do this in code, if we do it in res file it gets slammed by
	// the scheme.  (Incl. label background: some products don't have label background colors set in their scheme and
...
// Purpose:
//-----------------------------------------------------------------------------
void CAchievementNotificationPanel::AddNotification(const char *szIconBaseName, const wchar_t *pHeading,
...
CON_COMMAND_F(achievement_notification_test, "Test the hud notification UI", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY)
{
	...
	if(pPanel)
	{
		pPanel->AddNotification("HL2_KILL_ODESSAGUNSHIP", L"Achievement Progress",
								(0 == (iCount % 2) ? L"Test Notification Message A (1/10)" : L"Test Message B"));
	}
