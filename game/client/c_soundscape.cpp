...
ConVar soundscape_fadetime("soundscape_fadetime", "3.0", FCVAR_CHEAT,
...
public:
	...
	void PrintDebugInfo()
	{
		...
		{
			Msg("- PLAYING DEBUG SOUNDSCAPE: %d [%s]\n", m_forcedSoundscapeIndex,
				SoundscapeNameByIndex(m_forcedSoundscapeIndex));
		}
		Msg("- CURRENT SOUNDSCAPE: %d [%s]\n", m_params.soundscapeIndex.Get(),
			SoundscapeNameByIndex(m_params.soundscapeIndex));
		Msg("----------------------------------\n\n");
	}
	...
	int AddLoopingSound(const char *pSoundName, bool isAmbient, float volume, soundlevel_t soundLevel, int pitch,
...
static int SoundscapeCompletion(const char *partial,
...
Vector C_SoundscapeSystem::GenerateRandomSoundPosition()
{
	...
	else
	{
		return CurrentViewOrigin() +
			   DEFAULT_SOUND_RADIUS * (cosAngle * CurrentViewRight() + sinAngle * CurrentViewForward());
	}
}
...
//		this prevents pops
int C_SoundscapeSystem::AddLoopingSound(const char *pSoundName, bool isAmbient, float volume, soundlevel_t soundlevel,
										int pitch, const Vector &position)
{
	...
	while(soundSlot >= 0)
	{
		...
		if(sound.id != m_loopingSoundId && sound.pitch == pitch && !Q_strcasecmp(pSoundName, sound.pWaveName))
		{
			...
			if(isAmbient == true && sound.isAmbient == true)
			{
				// reuse this sound
				pSoundSlot = &sound;
				break;
			...
			else if(isAmbient == sound.isAmbient)
			{
				...
				else
				{
					...
					// Because this will occasionally cause the sound to vanish entirely, we stop the old sound
					// immediately.
					StopLoopingSound(sound);
					pSoundSlot = &sound;
...
void C_SoundscapeSystem::PlayRandomSound(randomsound_t &sound)
{
	...
	if(sound.isAmbient)
	{
		enginesound->EmitAmbientSound(pWaveName, sound.masterVolume * RandomInterval(sound.volume),
									  (int)RandomInterval(sound.pitch));
	}
	else
	{
		CLocalPlayerFilter filter;
