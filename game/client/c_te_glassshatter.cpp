...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropVector(RECVINFO(m_vecAngles)), RecvPropVector(RECVINFO(m_vecForce)),
	RecvPropVector(RECVINFO(m_vecForcePos)), RecvPropFloat(RECVINFO(m_flWidth)), RecvPropFloat(RECVINFO(m_flHeight)),
	RecvPropFloat(RECVINFO(m_flShardSize)), RecvPropInt(RECVINFO(m_nSurfaceType)),
	RecvPropInt(RECVINFO(m_uchFrontColor[0])), RecvPropInt(RECVINFO(m_uchFrontColor[1])),
	RecvPropInt(RECVINFO(m_uchFrontColor[2])), RecvPropInt(RECVINFO(m_uchBackColor[0])),
	RecvPropInt(RECVINFO(m_uchBackColor[1])), RecvPropInt(RECVINFO(m_uchBackColor[2])),
END_RECV_TABLE
()
...
void C_TEShatterSurface::PostDataUpdate(DataUpdateType_t updateType)
{
	...
	for(int width = 0; width < nNumWide; width++)
	{
		...
		{
			pParticle = (Particle3D *)pGlassEmitter->AddParticle(sizeof(Particle3D), hMaterial[random->RandomInt(0, 1)],
																 vCurPos);
	...
	pGlassEmitter->m_ParticleCollision.Setup(m_vecOrigin, &vMoveDir, GLASS_SHARD_NOISE, flMinSpeed, flMaxSpeed,
											 GLASS_SHARD_GRAVITY, GLASS_SHARD_DAMPING);
}
...
void TE_ShatterSurface(IRecipientFilter &filter, float delay, KeyValues *pKeyValues)
{
	...
	TE_ShatterSurface(filter, 0.0f, &vecOrigin, &angles, &vecForce, &vecForcePos, flWidth, flHeight, flSize,
					  nSurfaceType, front.r(), front.g(), front.b(), back.r(), back.g(), back.b());
}