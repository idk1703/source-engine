...
RecvPropFloat(RECVINFO(m_flStartScale)), RecvPropFloat(RECVINFO(m_flScale), 0, RecvProxy_PlasmaScale),
	RecvPropFloat(RECVINFO(m_flScaleTime), 0, RecvProxy_PlasmaScaleTime), RecvPropInt(RECVINFO(m_nFlags)),
	RecvPropInt(RECVINFO(m_nPlasmaModelIndex)), RecvPropInt(RECVINFO(m_nPlasmaModelIndex2)),
	...
	for(int i = 0; i < NUM_CHILD_FLAMES; i++)
	{
		...
		if(i != 0)
		{
			m_entFlames[i].SetLocalOrigin(offset +
										  (m_entFlames[i].m_vecMoveDir * ((m_entFlames[i].GetScale()) * CHILD_SPREAD)));
		}
	...
	if(m_flScaleRegister > 0.1)
	{
		...
		while(m_tDecalSpawn.NextEvent(tempDelta))
		{
			...
			if(ent)
			{
				...
				if(iDecal >= 0)
				{
					effects->DecalShoot(iDecal, 0, ent->GetModel(), ent->GetAbsOrigin(), ent->GetAbsAngles(),
										GetAbsOrigin(), 0, 0);
				}
			}
		}
