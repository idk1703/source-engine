...
void UTIL_EmitAmbientSound(C_BaseEntity *entity, const Vector &vecOrigin, const char *samp, float vol,
...
void UTIL_MakeSafeName(const char *oldName, OUT_Z_CAP(newNameBufSize) char *newName,
					   int newNameBufSize); ///< Cleans up player names for putting in vgui controls (cleaned names can
											///< be up to original*2+1 in length)
...
void UTIL_ReplaceKeyBindings(const wchar_t *inbuf, int inbufsizebytes, OUT_Z_BYTECAP(outbufsizebytes) wchar_t *outbuf,
							 int outbufsizebytes, GameActionSet_t action_set = GAME_ACTION_SET_NONE);
...
void UTIL_SetTrace(trace_t &tr, const Ray_t &ray, C_BaseEntity *edict, float fraction, int hitgroup,
...
int UTIL_EntitiesInSphere(C_BaseEntity **pList, int listMax, const Vector &center, float radius, int flagMask,
						  int partitionMask = PARTITION_CLIENT_NON_STATIC_EDICTS);
int UTIL_EntitiesAlongRay(C_BaseEntity **pList, int listMax, const Ray_t &ray, int flagMask,
						  int partitionMask = PARTITION_CLIENT_NON_STATIC_EDICTS);
