...
RecvPropArray3(RECVINFO_ARRAY(m_flexWeight), RecvPropFloat(RECVINFO(m_flexWeight[0]))),
	RecvPropInt(RECVINFO(m_blinktoggle)), RecvPropVector(RECVINFO(m_viewtarget)),
...
//-----------------------------------------------------------------------------
// Tracker 14992:  We used to load 18K of .vfes for every C_BaseFlex who lipsynced, but now we only load those files
// once globally. Note, we could wipe these between levels, but they don't ever load more than the weak/normal/strong
// phoneme classes that I can tell
...
// Output : 	void
//-----------------------------------------------------------------------------
void CFlexSceneFileManager::EnsureTranslations(IHasLocalToGlobalFlexSettings *instance,
...
// Purpose:
//-----------------------------------------------------------------------------
void *CFlexSceneFileManager::FindSceneFile(IHasLocalToGlobalFlexSettings *instance, const char *filename,
...
Vector C_BaseFlex::SetViewTarget(CStudioHdr *pStudioHdr)
{
	...
	if(m_iEyeAttachment > 0)
	{
		...
		if(local.x < flMaxEyeDeflection)
		{
			...
			// debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), 1, 0, "%5.3f %5.3f %5.3f", local.x,
			// local.y, local.z );
			local.x = 0;
...
void C_BaseFlex::AddVisemesForSentence(Emphasized_Phoneme *classes, float emphasis_intensity, CSentence *sentence,
...
//-----------------------------------------------------------------------------
void C_BaseFlex::BuildTransformations(CStudioHdr *pStudioHdr, Vector *pos, Quaternion q[],
									  const matrix3x4_t &cameraTransform, int boneMask, CBoneBitList &boneComputed)
{
	...
	for(int i = 0; i < nBoneFlexDriverCount; ++i)
	{
		...
		for(int j = 0; j < nControllerCount; ++j)
		{
			...
			SetFlexWeight(static_cast<LocalFlexController_t>(pController->m_nFlexControllerIndex),
						  RemapValClamped(position[pController->m_nBoneComponent], pController->m_flMin,
...
//-----------------------------------------------------------------------------
// Purpose: process the entities networked state, vcd playback, wav file visemes, and blinks into a global shared flex
// controller array
//-----------------------------------------------------------------------------
bool C_BaseFlex::SetupGlobalWeights(const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights,
									float *pFlexDelayedWeights)
{
	CStudioHdr *hdr = GetModelPtr();
	if(!hdr)
...
void C_BaseFlex::RunFlexDelay(int nFlexWeightCount, float *pFlexWeights, float *pFlexDelayedWeights,
...
//-----------------------------------------------------------------------------
void C_BaseFlex::SetupLocalWeights(const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights,
								   float *pFlexDelayedWeights)
{
	CStudioHdr *hdr = GetModelPtr();
	if(!hdr)
	...
	// BUGBUG: We have a bug with SetCustomModel that causes a disagreement between the studio header here and the one
	// used in l_studio.cpp CModelRender::DrawModelExecute So when we hit that case, let's not do any work because
	// otherwise we'd crash since the array sizes (m_flFlexDelayedWeight vs pFlexWeights) don't match. Note that this
	// check is duplicated in CEconEntity::SetupWeights.
	AssertMsg(nFlexWeightCount == m_cFlexDelayedWeight,
			  "Disagreement between the number of flex weights. Do the studio headers match?");
	...
	{
		debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), i-hdr->numflexcontrollers, 0, "%2d:%s :
	%3.2f", i, hdr->pFlexdesc( i )->pszFACS(), pFlexWeights[i] );
	}
	*/
	...
	for (i = 0; i < g_numflexcontrollers; i++)
	{
		...
		debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), -i, 0, "%s %3.2f", g_flexcontroller[i],
	g_flexweight[j] );
	}
	*/
}
