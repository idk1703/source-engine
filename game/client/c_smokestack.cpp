...
RecvPropFloat(RECVINFO(m_SpreadSpeed), 0), RecvPropFloat(RECVINFO(m_Speed), 0), RecvPropFloat(RECVINFO(m_StartSize), 0),
	RecvPropFloat(RECVINFO(m_EndSize), 0), RecvPropFloat(RECVINFO(m_Rate), 0), RecvPropFloat(RECVINFO(m_JetLength), 0),
	RecvPropInt(RECVINFO(m_bEmit), 0), RecvPropFloat(RECVINFO(m_flBaseSpread)), RecvPropFloat(RECVINFO(m_flTwist)),
	RecvPropFloat(RECVINFO(m_flRollSpeed)), RecvPropIntWithMinusOneFlag(RECVINFO(m_iMaterialModel)),

	RecvPropVector(RECVINFO(m_AmbientLight.m_vPos)), RecvPropVector(RECVINFO(m_AmbientLight.m_vColor)),
	RecvPropFloat(RECVINFO(m_AmbientLight.m_flIntensity)),

	RecvPropVector(RECVINFO(m_DirLight.m_vPos)), RecvPropVector(RECVINFO(m_DirLight.m_vColor)),
...
void C_SmokeStack::Update(float fTimeDelta)
{
	...
	if(m_bEmit && (m_ParticleEffect.WasDrawnPrevFrame() || m_ParticleEffect.GetAlwaysSimulate()))
	{
		...
		while(m_ParticleSpawn.NextEvent(tempDelta))
		{
			...
			// Make a new particle.
			if(SmokeStackParticle *pParticle = (SmokeStackParticle *)m_ParticleEffect.AddParticle(
				   sizeof(SmokeStackParticle), m_MaterialHandle[iRandomFrame]))
			{
				float angle = FRand(0, 2.0f * M_PI_F);

				pParticle->m_Pos =
					GetAbsOrigin() + right * (cos(angle) * m_flBaseSpread) + forward * (sin(angle) * m_flBaseSpread);

				pParticle->m_Velocity = FRand(-m_SpreadSpeed, m_SpreadSpeed) * right +
										FRand(-m_SpreadSpeed, m_SpreadSpeed) * forward + m_Speed * up;
...
void C_SmokeStack::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		m_Renderer.RenderParticle(pIterator->GetParticleDraw(), pParticle->m_Pos, tPos, alpha * m_flAlphaScale,
								  FLerp(m_StartSize, m_EndSize, tLifetime), DEG2RAD(pParticle->m_flAngle));
...
void C_SmokeStack::SimulateParticles(CParticleSimulateIterator *pIterator)
{
	...
	if(bQuickTest == false && m_bEmit &&
	   (!m_ParticleEffect.WasDrawnPrevFrame() && !m_ParticleEffect.GetAlwaysSimulate()))
		return;
	...
	while(pParticle)
	{
		...
		else
		{
			...
			if(m_bTwist)
			{
				Vector vTwist(pParticle->m_Pos.x - GetAbsOrigin().x, pParticle->m_Pos.y - GetAbsOrigin().y, 0);
...
#else
			pParticle->m_Pos = pParticle->m_Pos + pParticle->m_Velocity * pIterator->GetTimeDelta() +
							   pParticle->m_vAccel * pIterator->GetTimeDelta();
#endif
