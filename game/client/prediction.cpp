...
ConVar cl_predictweapons("cl_predictweapons", "1", FCVAR_USERINFO | FCVAR_NOT_CONNECTED,
						 "Perform client side prediction of weapon effects.");
ConVar cl_lagcompensation("cl_lagcompensation", "1", FCVAR_USERINFO | FCVAR_NOT_CONNECTED,
...
static ConVar cl_pred_optimize("cl_pred_optimize", "2", 0,
							   "Optimize for not copying data if didn't receive a network update (1), and also for not "
							   "repredicting if there were no errors (2).");

#ifdef STAGING_ONLY
// Do not ship this - testing a fix
static ConVar cl_pred_optimize_prefer_server_data("cl_pred_optimize_prefer_server_data", "0", 0,
												  "In the case where we have both server data and predicted data up to "
												  "the same tick, choose server data over predicted data.");
//
#endif // STAGING_ONLY
		...
		for(i = 0; i < c; i++)
		{
			...
			if(showlist)
			{
				...
				if(showlist >= 2)
				{
					...
					engine->Con_NXPrintf(&np, "%15s %30s (%5i / %5i bytes): %15s", sz, ent->GetClassname(), size,
										 intermediate_size, ent->GetPredictable() ? "predicted" : "client created");
				...
				{
					engine->Con_NXPrintf(&np, "%15s %30s: %15s", sz, ent->GetClassname(),
...
//  as appropriate
//-----------------------------------------------------------------------------
void CPrediction::SetIdealPitch(C_BasePlayer *player, const Vector &origin, const QAngle &angles,
			...
			if(!ent->IsEFlagSet(EFL_KILLME))
			{
				if(cl_showerror.GetInt() != 0)
				{
					...
					Msg("Removing unack'ed predicted entity:  %s created %s(%i) id == %s : %p\n", ent->GetClassname(),
						ctx->m_pszCreationModule, ctx->m_nCreationLineNumber, ent->m_PredictableID.Describe(), ent);
				}
			}
...
// Output : int
//-----------------------------------------------------------------------------
int CPrediction::ComputeFirstCommandToExecute(bool received_new_world_update, int incoming_acknowledged,
											  int outgoing_command)
{
	...
	if(!received_new_world_update || !m_nServerCommandsAcknowledged)
	{
		...
		// Always restore since otherwise we might start prediction using an "interpolated" value instead of a purely
		// predicted value
	...
	else
	{
#ifdef STAGING_ONLY
		int nPredictedLimit =
			cl_pred_optimize_prefer_server_data.GetBool() ? m_nCommandsPredicted - 1 : m_nCommandsPredicted;
#else
	   ...
	   //  only predict the usercmd from the latest render frame.
		if(cl_pred_optimize.GetInt() >= 2 && !m_bPreviousAckHadErrors && m_nCommandsPredicted > 0 &&
...
void CPrediction::Update(int startframe, bool validframe, int incoming_acknowledged, int outgoing_command)
{
	...
	if((m_nPreviousStartFrame == startframe) && cl_pred_optimize.GetBool() && cl_predict->GetInt())
	{
...
//-----------------------------------------------------------------------------
void CPrediction::_Update(bool received_new_world_update, bool validframe, int incoming_acknowledged,
						  int outgoing_command)
{
#if !defined(NO_ENTITY_PREDICTION)
	...
	SetIdealPitch(localPlayer, localPlayer->GetLocalOrigin(), localPlayer->GetLocalAngles(),
				  localPlayer->m_vecViewOffset);
#endif
}
