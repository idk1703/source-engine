...
RecvPropInt(RECVINFO(m_Flags)), RecvPropInt(RECVINFO(m_LightStyle)), RecvPropFloat(RECVINFO(m_Radius)),
	RecvPropInt(RECVINFO(m_Exponent)), RecvPropFloat(RECVINFO(m_InnerAngle)), RecvPropFloat(RECVINFO(m_OuterAngle)),
...
void C_DynamicLight::ClientThink(void)
{
	...
	if((m_Flags & DLIGHT_NO_MODEL_ILLUMINATION) == 0)
	{
		...
		if(!m_pDynamicLight || (m_pDynamicLight->key != index))
		{
			...
			m_pDynamicLight = ShouldBeElight() != 0 ? effects->CL_AllocElight(index) : effects->CL_AllocDlight(index);
#else
