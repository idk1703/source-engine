...
//-----------------------------------------------------------------------------
void CTrailParticles::Setup(const Vector &origin, const Vector *direction, float angularSpread, float minSpeed,
							float maxSpeed, float gravity, float dampen, int flags, bool bNotCollideable)
{
	// Take the flags
	if(!bNotCollideable)
	{
...
void FX_ElectricSpark(const Vector &pos, int nMagnitude, int nTrailLength, const Vector *vecDir)
{
	...
	pSparkEmitter->Setup((Vector &)pos, NULL, SPARK_ELECTRIC_SPREAD, SPARK_ELECTRIC_MINSPEED, SPARK_ELECTRIC_MAXSPEED,
						 SPARK_ELECTRIC_GRAVITY, SPARK_ELECTRIC_DAMPEN, bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
	...
	// Caps
	//
	CSmartPtr<CSimpleGlowEmitter> pSimple =
		CSimpleGlowEmitter::Create("FX_ElectricSpark 3", pos, gpGlobals->curtime + 0.2);
	...
	sParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
													   pSimple->GetPMaterial("effects/yellowflare_noz"), pos);
...
void FX_MetalScrape(Vector &position, Vector &normal)
{
	...
	// Setup our collision information
	sparkEmitter->Setup(offset, &normal, METAL_SCRAPE_SPREAD, METAL_SCRAPE_MINSPEED, METAL_SCRAPE_MAXSPEED,
						METAL_SCRAPE_GRAVITY, METAL_SCRAPE_DAMPEN, bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
	...
	for(int i = 0; i < numSparks; i++)
	{
		...
		pParticle->m_vecVelocity = dir * random->RandomFloat((METAL_SCRAPE_MINSPEED * (2.0f - spreadOfs)),
															 (METAL_SCRAPE_MAXSPEED * (2.0f - spreadOfs)));
...
void FX_MetalSpark(const Vector &position, const Vector &direction, const Vector &surfaceNormal, int iScale)
{
	...
	for(int i = 0; i < numSparks; i++)
	{
		...
		dir[0] =
			direction[0] + random->RandomFloat(-(METAL_SPARK_SPREAD * spreadOfs), (METAL_SPARK_SPREAD * spreadOfs));
		dir[1] =
			direction[1] + random->RandomFloat(-(METAL_SPARK_SPREAD * spreadOfs), (METAL_SPARK_SPREAD * spreadOfs));
		dir[2] =
			direction[2] + random->RandomFloat(-(METAL_SPARK_SPREAD * spreadOfs), (METAL_SPARK_SPREAD * spreadOfs));
		...
		pParticle->m_vecVelocity = dir * random->RandomFloat((METAL_SPARK_MINSPEED * (2.0f - spreadOfs)),
...
void FX_Sparks(const Vector &pos, int nMagnitude, int nTrailLength, const Vector &vecDir, float flWidth,
			   float flMinSpeed, float flMaxSpeed, char *pSparkMaterial)
{
	...
	// Setup our collision information
	pSparkEmitter->Setup((Vector &)pos, NULL, SPARK_SPREAD, flMinSpeed, flMaxSpeed, SPARK_GRAVITY, SPARK_DAMPEN,
	...
	for(i = 0; i < numSparks; i++)
	{
		...
		pParticle->m_vecVelocity =
			dir * random->RandomFloat((flMinSpeed * (2.0f - spreadOfs)), (flMaxSpeed * (2.0f - spreadOfs)));
...
void FX_EnergySplash(const Vector &pos, const Vector &normal, int nFlags)
{
	...
	// Quick flash
	FX_AddQuad(pos, normal, 64.0f, 0, 0.75f, 1.0f, 0.0f, 0.4f, random->RandomInt(0, 360), 0, Vector(1.0f, 1.0f, 1.0f),
			   0.25f, "effects/combinemuzzle1_nocull", (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));

	// Lingering burn
	FX_AddQuad(pos, normal, 16, 32, 0.75f, 1.0f, 0.0f, 0.4f, random->RandomInt(0, 360), 0, Vector(1.0f, 1.0f, 1.0f),
			   0.5f, "effects/combinemuzzle2_nocull", (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));
	...
	for(int j = 0; j < 8; j++)
	{
		...
		sParticle->m_vecVelocity =
			Vector(Helper_RandomFloat(-4.0f, 4.0f), Helper_RandomFloat(-4.0f, 4.0f), Helper_RandomFloat(16.0f, 64.0f));
...
void FX_MicroExplosion(Vector &position, Vector &normal)
{
	...
	// Setup our collision information
	sparkEmitter->Setup(offset, &normal, MICRO_EXPLOSION_SPREAD, MICRO_EXPLOSION_MINSPEED, MICRO_EXPLOSION_MAXSPEED,
						MICRO_EXPLOSION_GRAVITY, MICRO_EXPLOSION_DAMPEN, bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
	...
	for(int i = 0; i < numSparks; i++)
	{
		...
		if(pParticle)
		{
			...
			pParticle->m_vecVelocity = dir * random->RandomFloat(MICRO_EXPLOSION_MINSPEED * (1.5f - ramp),
																 MICRO_EXPLOSION_MAXSPEED * (1.5f - ramp));
	...
	SimpleParticle *sParticle =
		(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), pSimple->GetPMaterial("sprites/rico1"), offset);
...
void FX_Explosion(Vector &origin, Vector &normal, char materialType)
{
	...
	// Setup our collision information
	pSparkEmitter->Setup(offset, &normal, EXPLOSION_SPREAD, EXPLOSION_MINSPEED, EXPLOSION_MAXSPEED, EXPLOSION_GRAVITY,
						 EXPLOSION_DAMPEN, bitsPARTICLE_TRAIL_VELOCITY_DAMPEN);
	...
	{
		TrailParticle *pParticle =
			(TrailParticle *)pSparkEmitter->AddParticle(sizeof(TrailParticle), g_Material_Spark, offset);
	...
	if(materialType == 'C' || materialType == 'W')
	{
		...
		// Setup our collision information
		fleckEmitter->m_ParticleCollision.Setup(offset, &normal, EXPLOSION_FLECK_ANGULAR_SPRAY,
												EXPLOSION_FLECK_MIN_SPEED, EXPLOSION_FLECK_MAX_SPEED,
												EXPLOSION_FLECK_GRAVITY, EXPLOSION_FLECK_DAMPEN);
		...
		for(i = 0; i < numFlecks; i++)
		{
			FleckParticle *pParticle = (FleckParticle *)fleckEmitter->AddParticle(
				sizeof(FleckParticle), hMaterialArray[random->RandomInt(0, 1)], offset);
			...
			pParticle->m_vecVelocity =
				dir * (random->RandomFloat(EXPLOSION_FLECK_MIN_SPEED, EXPLOSION_FLECK_MAX_SPEED) *
					   (7 - pParticle->m_uchSize));
	...
	SimpleParticle *pSphereParticle =
		(SimpleParticle *)pSimpleEmitter->AddParticle(sizeof(SimpleParticle), hSphereMaterial, vecBurstOrigin);
	if(pSphereParticle)
	{
	...
	{
		SimpleParticle *pParticle =
			(SimpleParticle *)pSmokeEmitter->AddParticle(sizeof(SimpleParticle), g_Mat_DustPuff[1], offset);
		if(pParticle == NULL)
			break;
...
void FX_ConcussiveExplosion(Vector &origin, Vector &normal)
{
	...
	SimpleParticle *pParticle = (SimpleParticle *)pSimple->AddParticle(
		sizeof(SimpleParticle), pSimple->GetPMaterial("effects/blueflare1"), offset);
	...
	pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
													   pSimple->GetPMaterial("effects/blueflare1"), offset);
	...
	{
		// Setup our collision information
		pCollisionEmitter->Setup((Vector &)offset, NULL, SPARK_ELECTRIC_SPREAD, SPARK_ELECTRIC_MINSPEED * 6,
								 SPARK_ELECTRIC_MAXSPEED * 6, -400, SPARK_ELECTRIC_DAMPEN, bitsPARTICLE_TRAIL_FADE);
