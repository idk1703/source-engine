...
bool IsLocalPlayerUsingVisionFilterFlags(int nFlags, bool bWeaponsCheck /* = false */)
{
	...
	if(!bWeaponsCheck)
	{
		// We can only modify the RJ flags with normal checks that won't take the forced kill cam flags that can happen
		// in weapon checks
...
void UTIL_Tracer(const Vector &vecStart, const Vector &vecEnd, int iEntIndex, int iAttachment, float flVelocity,
				 bool bWhiz, char *pCustomTracerName)
{
	CEffectData data;
...
// Purpose:
//-----------------------------------------------------------------------------
void UTIL_SetTrace(trace_t &trace, const Ray_t &ray, C_BaseEntity *ent, float fraction, int hitgroup,
				   unsigned int contents, const Vector &normal, float intercept)
{
...
//			*param4 -
//-----------------------------------------------------------------------------
void ClientPrint(C_BasePlayer *player, int msg_dest, const char *msg_name, const char *param1 /*= NULL*/,
...
//-----------------------------------------------------------------------------
void UTIL_ReplaceKeyBindings(const wchar_t *inbuf, int inbufsizebytes, OUT_Z_BYTECAP(outbufsizebytes) wchar_t *outbuf,
							 int outbufsizebytes, GameActionSet_t actionset)
{
	...
	while(inbuf != inbufend && *inbuf != 0)
	{
		...
		if(*inbuf == '%')
		{
			...
			if(end &&
			   (end != inbuf)) // make sure we handle %% in the string, which should be treated in the output as %
			{
				wchar_t token[64];
				...
				{
					auto origin = g_pInputSystem->GetSteamControllerActionOrigin(
						*binding == '+' ? binding + 1 : binding, actionset);
				...
				// If we got a Steam Controller key description, use that, otherwise use the (possibly localized) key
				// name
				if(sc_origin)
				{
					locName = sc_origin;
...
unsigned char UTIL_ComputeEntityFade(C_BaseEntity *pEntity, float flMinDist, float flMaxDist, float flFadeScale)
{
...
#endif
	{
		...
		unsigned char nGlobalAlpha =
			IsXbox() ? 255 : modelinfo->ComputeLevelScreenFade(vecAbsCenter, flRadius, flFadeScale);
