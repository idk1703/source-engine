...
void CBaseSimpleCollision::TraceLine(const Vector &start, const Vector &end, trace_t *pTrace, bool coarse)
{
	...
	for(int i = 0; i < m_nActivePlanes; i++)
	{
		...
		if(debugoverlay)
		{
			debugoverlay->AddBoxOverlay(vIntersection, Vector(-1, -1, -1), Vector(1, 1, 1), QAngle(0, 0, 0), 0, 255, 0,
										16, __DEBUG_PARTICLE_COLLISION_OVERLAY_LIFETIME);
		}
...
void CBaseSimpleCollision::TestForPlane(const Vector &start, const Vector &dir, float speed, float gravity)
{
	...
	for(int i = 1; i <= NUM_DISCREET_STEPS; i++)
	{
		...
		if(tr.fraction != 1.0f)
		{
			...
			if(debugoverlay)
			{
				debugoverlay->AddLineOverlay(testStart, tr.endpos, 255, 0, 0, true,
											 __DEBUG_PARTICLE_COLLISION_OVERLAY_LIFETIME);
				...
				debugoverlay->AddBoxOverlay(tr.endpos, Vector(-64, -64, 0), Vector(64, 64, 0), angles, 255, 0, 0, 16,
											__DEBUG_PARTICLE_COLLISION_OVERLAY_LIFETIME);
			}
#endif //__DEBUG_PARTICLE_COLLISION_OVERLAY
...
// We missed
		...
		if(debugoverlay)
		{
			debugoverlay->AddLineOverlay(testStart, tr.endpos, 0,
										 128.0f + (128.0f * ((float)i / (float)NUM_DISCREET_STEPS)), 0, true,
										 __DEBUG_PARTICLE_COLLISION_OVERLAY_LIFETIME);
		}
#endif //__DEBUG_PARTICLE_COLLISION_OVERLAY
...
//-----------------------------------------------------------------------------
void CParticleCollision::Setup(const Vector &origin, const Vector *dir, float angularSpread, float minSpeed,
							   float maxSpeed, float gravity, float dampen)
{
...
//			timeDelta - time step
//-----------------------------------------------------------------------------
bool CParticleCollision::MoveParticle(Vector &origin, Vector &velocity, float *rollDelta, float timeDelta,
									  trace_t *pTrace)
{
	...
	if(m_nActivePlanes > 0)
	{
		...
		if(pTrace->fraction != 1.0f)
		{
			...
			else
			{
				...
				if(debugoverlay)
				{
					debugoverlay->AddBoxOverlay(pTrace->endpos, Vector(-1, -1, -1), Vector(1, 1, 1), QAngle(0, 0, 0),
												255, 0, 0, 16, __DEBUG_PARTICLE_COLLISION_OVERLAY_LIFETIME);
				}
#endif //__DEBUG_PARTICLE_COLLISION_OVERLAY
			}
		}
