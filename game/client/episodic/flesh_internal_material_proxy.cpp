...
RecvPropFloat(RECVINFO(m_flRadius), 0, RecvProxy_FleshEffect_Radius), RecvPropFloat(RECVINFO(m_flScaleTime)),
END_RECV_TABLE
()
...
{
	IMaterialVar *pParams[] = {m_pMaterialParamFleshEffectCenterRadius1, m_pMaterialParamFleshEffectCenterRadius2,
							   m_pMaterialParamFleshEffectCenterRadius3, m_pMaterialParamFleshEffectCenterRadius4};
	...
	if(m_pMaterialParamFleshSubsurfaceTint != NULL)
	{
		...
		// vSubsurfaceTintColor[0] = vSubsurfaceTintColor[1] = vSubsurfaceTintColor[2] = sinf( gpGlobals->curtime * 3.0f
		// ) + 1.0f;  // * 0.5f + 0.5f;
