...
RecvPropTime(RECVINFO(m_flBlueEndFadeTime)), RecvPropBool(RECVINFO(m_bIsBlue)), RecvPropBool(RECVINFO(m_bIsBlack)),
END_RECV_TABLE
()
...
void C_NPC_Vortigaunt::ReceiveMessage(int classID, bf_read &msg)
{
	...
	switch(messageType)
	{
		...
		case VORTFX_ARMBEAM:
		{
			...
			C_BaseEntity *pEnt =
				ClientEntityList().GetBaseEntityFromHandle(ClientEntityList().EntIndexToHandle(nIndex));
			...
			if(pEnt)
			{
				...
				CNewParticleEffect *pEffect =
					pEnt->ParticleProp()->Create("vortigaunt_beam_charge", PATTACH_POINT_FOLLOW, nAttachment);
				if(pEffect)
				{
...
void DispelCallback(const CEffectData &data)
{
	...
	if(tr.fraction < 1.0f)
	{
		...
		FX_AddQuad(tr.endpos + (tr.plane.normal * 8.0f), Vector(0, 0, 1), 64.0f, 600.0f, 0.8f,
				   1.0f, // start alpha
				   0.0f, // end alpha
				   0.3f, random->RandomFloat(0, 360), 0.0f, Vector(0.5f, 1.0f, 0.5f), 0.75f, "effects/ar2_altfire1b",
				   (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA | FXQUAD_COLOR_FADE));

		// Add a ripple quad to the surface
		FX_AddQuad(tr.endpos + (tr.plane.normal * 8.0f), Vector(0, 0, 1), 16.0f, 300.0f, 0.9f,
				   1.0f, // start alpha
				   0.0f, // end alpha
				   0.9f, random->RandomFloat(0, 360), 0.0f, Vector(0.5f, 1.0f, 0.5f), 1.25f, "effects/rollerglow",
