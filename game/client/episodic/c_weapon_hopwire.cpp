...
public:
	C_HopwireExplosion(void) : m_hOwner(NULL)
	{
...
void C_HopwireExplosion::AddParticles(void)
{
	...
	while(m_ParticleTimer.NextEvent(tempDelta))
	{
		...
		sParticle =
			(SimpleParticle *)m_pAttractorEmitter->AddParticle(sizeof(SimpleParticle), g_Mat_Fleck_Cement[0], offset);
		...
		sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
			sizeof(SimpleParticle), m_pSimpleEmitter->GetPMaterial("effects/strider_muzzle"), GetRenderOrigin());
		...
		for(int i = 0; i < numRingSprites; i++)
		{
			...
			UTIL_TraceLine(GetRenderOrigin(), GetRenderOrigin() + (Vector(0, 0, -1024)), MASK_SOLID_BRUSHONLY, NULL,
						   COLLISION_GROUP_NONE, &tr);

			offset = (RandomVector(-4.0f, 4.0f) + tr.endpos) + (forward * 512.0f);

			sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
				sizeof(SimpleParticle), g_Mat_DustPuff[random->RandomInt(0, 1)], offset);
