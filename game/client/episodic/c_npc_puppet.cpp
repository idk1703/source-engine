...
RecvPropEHandle(RECVINFO(m_hAnimationTarget)), RecvPropInt(RECVINFO(m_nTargetAttachment)),
END_RECV_TABLE
()
...
//-----------------------------------------------------------------------------
void C_NPC_Puppet::BuildTransformations(CStudioHdr *pStudioHdr, Vector *pos, Quaternion q[],
										const matrix3x4_t &cameraTransform, int boneMask, CBoneBitList &boneComputed)
{
				...
				if(nSequence >= 0)
				{
					...
					if(Q_stristr(boneSetup.GetStudioHdr()->pszName(), r_sequence_debug.GetString()) != NULL)
					{
						DevMsgRT("%6.2f : %30s : %5.3f : %4.2f : %1d\n", currentTime,
								 boneSetup.GetStudioHdr()->pSeqdesc(nSequence).pszLabel(), fCycle, fWeight, i);
					}
#endif
				}
			}
