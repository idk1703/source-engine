...
void CStunEffect::Render(int x, int y, int w, int h)
{
	...
	if(g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80)
	{
		...
		pRenderContext->DrawScreenSpaceRectangle(
			m_EffectMaterial, 0, 0, w, h, vX, 0, (m_StunTexture->GetActualWidth() - 1) + vX,
			(m_StunTexture->GetActualHeight() - 1), m_StunTexture->GetActualWidth(), m_StunTexture->GetActualHeight());
	}
...
void CEP1IntroEffect::Render(int x, int y, int w, int h)
{
	...
	pRenderContext->DrawScreenSpaceRectangle(m_EffectMaterial, vX, vY, w, h, flU1, flV1, flU2, flV2,
