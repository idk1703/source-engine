...
#define MAX_ID_STRING 256
	...
	if(iEntIndex)
	{
		...
		if(IsPlayerIndex(iEntIndex) && pPlayer->InSameTeam(pLocalPlayer))
		{
			...
			g_pVGuiLocalize->ConstructString(sIDString, sizeof(sIDString), L"%s1 (%s2)", 2, wszPlayerName,
											 wszHealthText);
		...
		else if(IsPlayerIndex(iEntIndex) && pPlayer->GetTeamNumber() != TEAM_SPECTATOR &&
				pPlayer->GetTeamNumber() != TEAM_UNASSIGNED && pPlayer->IsAlive())
		{
