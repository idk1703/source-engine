...
ConVar cl_voicesubtitles("cl_voicesubtitles", "1", 0,
						 "Enable/disable subtitle printing on voice commands and hand signals.");
...
void CHudChat::MsgFunc_VoiceSubtitle(bf_read &msg)
{
	...
	ChatPrintf(client, CHAT_FILTER_NONE, "%c(%s) %s%c: %s", COLOR_PLAYERNAME, szPrefix, g_PR->GetPlayerName(client),
			   COLOR_NORMAL, ConvertCRtoNL(szString));
}
...
void CHudChat::MsgFunc_HandSignalSubtitle(bf_read &msg)
{
	...
	ChatPrintf(client, CHAT_FILTER_NONE, "%c(%s) %s%c: %s", COLOR_USEOLDCOLORS, szPrefix, g_PR->GetPlayerName(client),
			   COLOR_NORMAL, ConvertCRtoNL(szString));
}
