...
void CDoDHudAmmo::PaintMGAmmo(CWeaponDODBase *pWpn)
{
	...
	if(pFullClip)
	{
		...
		DrawText(buf, xpos + nIconWidth - ((float)nIconWidth / 3.0), ypos + nIconHeight - ((float)nIconHeight / 3.0),
				 m_clrTextColor);
	}
...
void CDoDHudAmmo::PaintGunAmmo(CWeaponDODBase *pWpn)
{
	...
	if(pFullClip && pEmptyClip)
	{
		...
		pEmptyClip->DrawSelfCropped(xpos, ypos + yPosOffset, 0, nOffset, pEmptyClip->Width(),
									pEmptyClip->Height() - nOffset, nIconWidth, nIconHeight - yPosOffset, m_clrIcon);
	}
