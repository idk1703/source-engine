...
private:
	void DrawSplotch(float x, float y, float width, float height, float u, float v, float uWidth, float vHeight,
					 int orientation, int alpha = 255);
...
void CFilmGrainEffect::SetParameters(KeyValues *params)
{
	if(params->FindKey("split_screen"))
	{
		...
		EnableHUDFilmDemo(m_bSplitScreen, "#Valve_FilmDemo_FilmGrain_LeftTitle",
...
//-----------------------------------------------------------------------------
void CFilmGrainEffect::DrawSplotch(float x, float y, float flWidth, float flHeight, float u, float v, float uWidth,
								   float vWidth, int orientation, int alpha)
{
...
void CFilmGrainEffect::Render(int x, int y, int w, int h)
{
		...
		if(!Q_stricmp(pVar->GetName(), "$noisescale"))
		{
			if(g_pMaterialSystemHardwareConfig->GetDXSupportLevel() <= 70)
			{
				pVar->SetVecValue(m_NoiseScale.x * g_flDX7NoiseScale * m_flEffectAlpha,
								  ...
								  m_NoiseScale.z * g_flDX7NoiseScale * m_flEffectAlpha, alpha);
			}
			else
			{
				pVar->SetVecValue(m_NoiseScale.x * m_flEffectAlpha, m_NoiseScale.y * m_flEffectAlpha,
								  m_NoiseScale.z * m_flEffectAlpha, alpha);
			}
		}
	}
	...
	for(int i = 0; i < m_CachedParticles.Count(); i++)
	{
		...
		DrawSplotch(particle->m_flPositionX, particle->m_flPositionY, particle->m_flWidth * m_flEffectAlpha,
					particle->m_flHeight * m_flEffectAlpha, flUOffset, flVOffset, 0.25f, 0.25f,
					particle->m_nOrientation);
	}
}
...
static ConVar mat_filmgrain("mat_filmgrain", "0", FCVAR_CLIENTDLL, "Enable/disable film grain post effect",
							EnableFilmGrain);