...
int CHudDeathNotice::DrawDefenseItem(DeathNoticeItem *pItem, int xRight, int y)
{
	...
	DrawBackgroundBox(x - boxBorder, y - boxBorder, boxWidth + 2 * boxBorder, boxHeight + 2 * boxBorder,
					  pItem->bLocalPlayerInvolved);
...
{
	Panel::DrawBox(x, y, w, h, bLocalPlayerInvolved ? m_ActiveBackgroundColor : m_BackgroundColor,
...
void CHudDeathNotice::FireGameEvent(IGameEvent *event)
{
	...
	if(Q_strcmp("dod_point_captured", pEventName) == 0)
	{
		...
		Q_snprintf(szLogMsg, sizeof(szLogMsg), "%s captured %s for the %s\n", capMsg.Killer.szName,
				   capMsg.Victim.szName, capMsg.Killer.iEntIndex == TEAM_ALLIES ? "U.S. Army" : "Wermacht");
