...
// Purpose:
//-----------------------------------------------------------------------------
CDODFreezePanelCallout *CDODFreezePanel::TestAndAddCallout(Vector &origin, Vector &vMins, Vector &vMaxs,
				   ...
				   !QuickBoxIntersectTest(vecCalloutTL, vecCalloutBR, vecStatTL, vecStatBR))
				{
					...
					if(bClear)
					{
						CDODFreezePanelCallout *pCallout =
							new CDODFreezePanelCallout(g_pClientMode->GetViewport(), "FreezePanelCallout");
...
void CDODFreezePanel::UpdateCallout(void)
{
	...
	if(pRagdoll)
	{
		...
		TestAndAddCallout(origin, vMins, vMaxs, &vecCalloutsTL, &vecCalloutsBR, vecFreezeTL, vecFreezeBR, vecStatTL,
						  vecStatBR, &iX, &iY);
	}
}
...
void CDODFreezePanel::ShowSnapshotPanel(bool bShow)
{
	...
	if(bShow)
	{
		...
		g_pVGuiLocalize->ConstructString(wLabel, sizeof(wLabel), g_pVGuiLocalize->Find("#TF_freezecam_snapshot"), 1,
										 wKey);
				...
				if(m_iKillerIndex <= MAX_PLAYERS)
				{
					...
					if(pszKillerName)
					{
						...
						if(cl_screenshotname.IsValid())
						{
							...
							Q_snprintf(szScreenShotName, sizeof(szScreenShotName), "%s %s", pszKillerName,
									   FREEZECAM_SCREENSHOT_STRING);
