...
void CDoDCutEditablePanel::ApplySettings(KeyValues *inResourceData)
{
	...
	Q_strncpy(m_szBackgroundTexture, inResourceData->GetString("BackgroundTexture", "vgui/white"),
			  sizeof(m_szBackgroundTexture));
	Q_strncpy(m_szBackgroudColor, inResourceData->GetString("BackgroundColor", "HudPanelForeground"),
			  sizeof(m_szBackgroudColor));
	Q_strncpy(m_szBorderColor, inResourceData->GetString("BackgroundBorder", "HudPanelBorder"),
			  sizeof(m_szBorderColor));
...
void CDoDCutEditablePanel::PaintBackground()
{
	...
	int wide = nwide - 1; // -1 because we can't draw all the way out to the width of our panel (it gets clipped), we
						  // can only draw to width - 1
