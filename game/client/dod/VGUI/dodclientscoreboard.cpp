...
void CDODClientScoreBoardDialog::InitPlayerList(SectionedListPanel *pPlayerList, int teamNumber)
{
	...
	pPlayerList->AddColumnToSection(0, "avatar", "",
									...
									m_iProportionalAvatarWidth);
	pPlayerList->AddColumnToSection(0, "name", "", 0, m_iNameWidth);
	pPlayerList->AddColumnToSection(
		0, "status", "", SectionedListPanel::COLUMN_IMAGE | SectionedListPanel::COLUMN_CENTER, m_iStatusWidth);
...
void CDODClientScoreBoardDialog::UpdateTeamInfo()
{
	...
	for(int teamIndex = TEAM_ALLIES; teamIndex <= TEAM_AXIS; teamIndex++)
	{
		...
		if(team)
		{
			...
			{
				g_pVGuiLocalize->ConstructString(string1, sizeof(string1), g_pVGuiLocalize->Find("#scoreboard_Player"),
												 2, teamName, wNumPlayers);
			}
			else
			{
				g_pVGuiLocalize->ConstructString(string1, sizeof(string1), g_pVGuiLocalize->Find("#scoreboard_Players"),
												 2, teamName, wNumPlayers);
			}
			...
			g_pVGuiLocalize->ConstructString(wTeamScore, sizeof(wTeamScore),
											 g_pVGuiLocalize->Find("#scoreboard_teamscore"), 2, wRoundsWon, wTickScore);
...
void CDODClientScoreBoardDialog::UpdateSpectatorList()
{
	...
	if(nSpectators > 0)
	{
		...
		g_pVGuiLocalize->ConstructString(wzSpectators, sizeof(wzSpectators), g_pVGuiLocalize->Find(pchFormat), 2,
										 wzSpectatorCount, wzSpectatorList);
	}
	...
	if((localteam == TEAM_SPECTATOR || localteam == TEAM_UNASSIGNED || team == localteam))
	{
		...
		if(g_PR->IsConnected(playerIndex))
		{
			...
			if(cls != PLAYERCLASS_UNDEFINED)
			{
				...
				g_pVGuiLocalize->ConvertUnicodeToANSI(g_pVGuiLocalize->Find(info.m_szPrintName), szClassName,
													  sizeof(szClassName));
			}
