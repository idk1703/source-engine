...
ConVar hud_capturepanel("hud_capturepanel", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
						"Set to 0 to not draw the HUD capture panel");

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
CDoDCapturePanelProgressBar::CDoDCapturePanelProgressBar(vgui::Panel *parent, const char *name)
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDoDHudCapturePanel::CDoDHudCapturePanel(const char *pElementName)
...
void CDoDHudCapturePanel::OnThink()
{
	...
	if(pPlayer)
	{
		...
		if(!bInCapZone || !hud_capturepanel.GetBool())
		{
			...
			if(pPlayer->m_iProgressBarDuration > 0 && pPlayer)
			{
				...
				if(m_pProgressBar)
				{
					...
					float flPercent = (pPlayer->m_flSimulationTime - pPlayer->m_flProgressBarStartTime) /
									  (float)pPlayer->m_iProgressBarDuration;
		...
		int nNumTeammates =
			g_pObjectiveResource->GetNumPlayersInArea(iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_ALLIES : TEAM_AXIS);
		int nRequiredTeammates =
			g_pObjectiveResource->GetRequiredCappers(iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_ALLIES : TEAM_AXIS);

		int nNumEnemies =
			g_pObjectiveResource->GetNumPlayersInArea(iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_AXIS : TEAM_ALLIES);
		int nRequiredEnemies =
			g_pObjectiveResource->GetRequiredCappers(iCPIndex, nPlayerTeam == TEAM_ALLIES ? TEAM_AXIS : TEAM_ALLIES);
		...
		if(iCappingTeam == TEAM_UNASSIGNED || iCappingTeam != nPlayerTeam)
		{
			...
			if(m_pMessage)
			{
				...
				if(m_pBackground)
				{
					...
					if(bgW < textW)
					{
						m_pBackground->SetBounds(bgX + (bgW / 2.0) - ((textW + XRES(3)) / 2.0), bgY, textW + XRES(3),
												 bgH);
					}
				}
			}
