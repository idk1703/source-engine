...
#ifdef WIN32
		_snwprintf(wText, sizeof(wText) / sizeof(wchar_t), L"%s %d:%02d", bNegative ? L"-" : L"+", iSecondsToDraw / 60,
				   iSecondsToDraw % 60);
#else
		_snwprintf(wText, sizeof(wText) / sizeof(wchar_t), L"%S %d:%02d", bNegative ? L"-" : L"+", iSecondsToDraw / 60,
				   iSecondsToDraw % 60);
...
void CHudObjectiveIcons::Paint()
{
	...
	if(bShowTimer)
	{
		if(m_pTimer)
		{
			...
			int boxHeight =
				2 * m_nBackgroundOverlap + m_nIconSize + (int)((float)m_nTimeAddedHeight * m_flTimeAddedExpandPercent);

			DrawBackgroundBox(xpos - m_nBackgroundOverlap, ypos - m_nBackgroundOverlap,
							  m_pTimer->GetWide() + 2 * m_nBackgroundOverlap, boxHeight, (num <= 0) ? true : false);
	...
	for(index = 0; index < num; index++)
	{
		if(g_pObjectiveResource->IsCPVisible(index))
		{
			...
			{
				DrawBackgroundBox(
					flXPos - m_nBackgroundOverlap, flYPos - m_nBackgroundOverlap, flIconSize + 2 * m_nBackgroundOverlap,
					flIconSize + 2 * m_nBackgroundOverlap + m_nTimeAddedHeight, (index != iLastVisible) ? false : true);
			}
			else
			{
				DrawBackgroundBox(flXPos - m_nBackgroundOverlap, flYPos - m_nBackgroundOverlap,
								  flIconSize + 2 * m_nBackgroundOverlap, flIconSize + 2 * m_nBackgroundOverlap,
				...
				for(j = 0; j <= 7; j++)
				{
					...
					else
					{
						...
						uv1.x = quadrants[j].vecTrailing.x -
								xdir * abs(quadrants[j].vecLeading.x - quadrants[j].vecTrailing.x) * tan(flAngle);
						uv1.y = quadrants[j].vecTrailing.y -
								ydir * abs(quadrants[j].vecLeading.y - quadrants[j].vecTrailing.y) * tan(flAngle);
						...
						switch(iBombsRemaining)
						{
							...
							case 1:
								if(bBombPlanted)
								{
									...
									m_pC4PlantedBG->DrawSelf(xMid - iIconWidth, yIcon - iIconHalfWidth, iIconWidth * 2,
						...
						switch(iBombsRemaining)
						{
							...
							case 1:
								if(bBombPlanted)
								{
									// draw the background behind 1
									int alpha = (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
									m_pC4PlantedBG->DrawSelf(xMid1 - iIconWidth, yIcon - iIconHalfWidth, iIconWidth * 2,
															 iIconWidth * 2, Color(255, 255, 255, alpha));
								}
							...
							case 2:
								if(bBombPlanted)
								{
									...
									int alpha = (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
									m_pC4PlantedBG->DrawSelf(xMid2 - iIconWidth, yIcon - iIconHalfWidth, iIconWidth * 2,
															 iIconWidth * 2, Color(255, 255, 255, alpha));
								}
								m_pC4Icon->DrawSelf(xMid1 - iIconHalfWidth, yIcon, iIconWidth, iIconWidth, c);
					...
					default:
					{
						...
						if(iBombsRemaining > 0)
						{
							...
							if(bBombPlanted)
							{
								// draw the background behind 2
								int alpha = (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
								m_pC4PlantedBG->DrawSelf(xpos - iIconHalfWidth, yIcon - iIconHalfWidth, iIconWidth * 2,
														 iIconWidth * 2, Color(255, 255, 255, alpha));
							}
			...
			if(requiredPlayers > 1)
			{
				...
				// set pos centered under icon
				vgui::surface()->DrawSetTextPos(xpos + (m_nIconSize / 2.0f) - (len / 2),
