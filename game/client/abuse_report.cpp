...
bool AbuseIncidentData_t::Poll()
{
	...
	if(!m_bitmapScreenshot.IsValid() && m_nScreenShotWaitFrames > 0)
	{
		...
		if(!g_pFullFileSystem->RelativePathToFullPath(CAbuseReportManager::k_rchScreenShotFilename, NULL, szFullPath,
		...
		if(g_pFullFileSystem->FileExists(szFullPath))
		{
			...
			if(!g_pFullFileSystem->ReadFile(CAbuseReportManager::k_rchScreenShotFilename, NULL,
...
void CAbuseReportManager::FireGameEvent(IGameEvent *event)
{
	...
	if(!Q_strcmp("teamplay_round_win", eventname) || !Q_strcmp("tf_game_over", eventname))
	{
...
bool CAbuseReportManager::PopulateIncident()
{
	...
	if(m_adrCurrentServer.IsValid() && !m_adrCurrentServer.IsLocalhost() && m_steamIDCurrentServer.IsValid() &&
	...
	for(int i = 1; i <= gpGlobals->maxClients; ++i)
	{
		...
		for(int j = 0; j < 8; ++j)
		{
			...
			Vector4D modelCorner((j & 1) ? p->m_vecRenderBoundsMax.x : p->m_vecRenderBoundsMin.x,
								 (j & 2) ? p->m_vecRenderBoundsMax.y : p->m_vecRenderBoundsMin.y,
								 (j & 4) ? p->m_vecRenderBoundsMax.z : p->m_vecRenderBoundsMin.z, 1.0f);
		...
		if(p->m_bRenderBoundsValid)
		{
			...
			p->m_bRenderBoundsValid = p->m_screenBoundsMin.x + .01f < p->m_screenBoundsMax.x &&
...
void CAbuseReportManager::DestroyIncidentData()
{
	...
	if(steamapicontext && steamapicontext->SteamRemoteStorage() &&
	   steamapicontext->SteamRemoteStorage()->FileExists(k_rchScreenShotFilename))
	{
...
void CAbuseReportManager::CheckCreateReportReadyNotification(float flMinSecondsSinceLastNotification, bool bInGame,
															 float flLifetime)
{
	...
	if(m_timeLastReportReadyNotification != 0.0 &&
	   Plat_FloatTime() < m_timeLastReportReadyNotification + flMinSecondsSinceLastNotification)
	{
		return;
	}
...
CON_COMMAND_F(abuse_report_queue,
			  "Capture data for abuse report and queue for submission.  Use abose_report_submit to activate UI to "
			  "submit the report",
			  FCVAR_DONTRECORD)
{
	if(!g_AbuseReportMgr)
...
CON_COMMAND_F(abuse_report_submit,
