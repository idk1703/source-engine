...
static ConVar r_forcecheapwater(
	"r_forcecheapwater", "0", FCVAR_CLIENTDLL | FCVAR_CHEAT,
	"Force all water to be cheap water, will show old renders if enabled after water has been seen");

ConVar r_portal_use_stencils(
	"r_portal_use_stencils", "1", FCVAR_CLIENTDLL,
	"Render portal views using stencils (if available)"); // draw portal views using stencil rendering
ConVar r_portal_stencil_depth("r_portal_stencil_depth", "2", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
...
void RemovePortalViewIDChildLinkIndex(PortalViewIDNode_t *pNode, int iRemoveIndex)
{
	...
	// I know the current behavior for CUtlVector::FastRemove() is to move the tail into the removed index. But I need
	// that behavior to be true in the future as well so I'm doing it explicitly
...
//
//-----------------------------------------------------------------------------
CPortalRenderable::CPortalRenderable(void) : m_bIsPlaybackPortal(false)
{
...
CPortalRenderable::~CPortalRenderable(void)
{
	...
	RemovePortalViewIDChildLinkIndex(
		&s_PortalRender.m_HeadPortalViewIDNode,
		m_iPortalViewIDNodeIndex); // does the same transplant operation as above to all portal view id nodes
}
...
void CPortalRenderable::BeginPortalPixelVisibilityQuery(void)
{
	...
	if(g_pPortalRender->ShouldUseStencilsToRenderPortals()) // this function exists because we require help in texture
															// mode, we need no assistance in stencil mode. Moreover,
															// doing the query twice will probably fubar the results
		return;

	PortalViewIDNode_t *pCurrentPortalViewNode =
		g_pPortalRender->m_PortalViewIDNodeChain[g_pPortalRender->m_iViewRecursionLevel]
			->ChildNodes[m_iPortalViewIDNodeIndex];
...
// Constructor
//-----------------------------------------------------------------------------
CPortalRender::CPortalRender() : m_MaterialsAccess(m_Materials)
{
	m_iRemainingPortalViewDepth = 1; // let's portals know that they should do "end of the line" kludges to cover up
...
int CPortalRender::ShouldForceCheaperWaterLevel() const
{
	...
	if(m_iViewRecursionLevel > 0)
	{
		...
		PortalViewIDNode_t *pPixelVisNode = m_PortalViewIDNodeChain[m_iViewRecursionLevel - 1]
												->ChildNodes[m_pRenderingViewForPortal->m_iPortalViewIDNodeIndex];
...
void Recursive_UpdatePortalPixelVisibility(PortalViewIDNode_t *pNode, IMatRenderContext *pRenderContext)
{
	if(pNode->iWindowPixelsAtQueryTime > 0)
	{
		...
		{
			pNode->iOcclusionQueryPixelsRendered =
				pRenderContext->OcclusionQuery_GetNumPixelsRendered(pNode->occlusionQueryHandle);
			pNode->fScreenFilledByPortalSurfaceLastFrame_Normalized =
				((float)pNode->iOcclusionQueryPixelsRendered) / ((float)pNode->iWindowPixelsAtQueryTime);
		}
	}
	else
	{
...
void CPortalRender::EnteredPortal(CPortalRenderable *pEnteredPortal)
{
	...
	// Create a new node that will preserve main's visibility. This new node will be linked to the new head node at the
	// exit portal's index (imagine entering a portal walking backwards)
	...
	if(pNewHead) // it's possible we entered a portal we couldn't see through
	{
		...
		Assert(pNewHead->ChildNodes[pExitPortal->m_iPortalViewIDNodeIndex] ==
			   NULL); // seeing out of an exit portal back into itself should be impossible

		for(int i = 0; i != iNodeLinkCount; ++i)
		{
			m_HeadPortalViewIDNode.ChildNodes[i] = pNewHead->ChildNodes[i];
			pNewHead->ChildNodes[i] =
				NULL; // going to be freeing the node in a minute, don't want to kill transplanted children
		}
	...
	Assert(m_HeadPortalViewIDNode.ChildNodes[pExitPortal->m_iPortalViewIDNodeIndex] ==
		   NULL); // asserted above in pNewHead code, but call me paranoid
...
bool CPortalRender::DrawPortalsUsingStencils(CViewRender *pViewRender)
{
	...
	for(int i = 0; i < iNumRenderablePortals; ++i)
	{
		...
		bool bIsVisible = (pPairedEntity == NULL) ||
						  (pPairedEntity->IsVisible() &&
						   pPairedEntity->ShouldDraw()); // either unknown visibility or definitely visible.
		...
		if(!pPortalRenderable->m_bIsPlaybackPortal)
		{
			if(!bIsVisible)
			{
				...
				if(m_PortalViewIDNodeChain[m_iViewRecursionLevel]
					   ->ChildNodes[pPortalRenderable->m_iPortalViewIDNodeIndex] != NULL)
				{
					FreePortalViewIDNode(m_PortalViewIDNodeChain[m_iViewRecursionLevel]
											 ->ChildNodes[pPortalRenderable->m_iPortalViewIDNodeIndex]);
					m_PortalViewIDNodeChain[m_iViewRecursionLevel]
						->ChildNodes[pPortalRenderable->m_iPortalViewIDNodeIndex] = NULL;
				}
	...
	const int iMaxDepth = MIN(r_portal_stencil_depth.GetInt(),
							  MIN(MAX_PORTAL_RECURSIVE_VIEWS, (1 << materials->StencilBufferBits())) - 1);
	...
	// queued mode makes us pass the barrier of just noticeable difference when using a previous frame's occlusion as a
	// draw skip check
	...
	if(m_iViewRecursionLevel == 0) // first entry into the stencil drawing
	{
		...
		m_RecursiveViewComplexFrustums[0]
			.RemoveAll(); // clear any garbage leftover in the complex frustums from last frame
	}
	...
	if(m_RecursiveViewComplexFrustums[m_iViewRecursionLevel].Count() == 0)
	{
		...
		m_RecursiveViewComplexFrustums[m_iViewRecursionLevel].AddMultipleToTail(FRUSTUM_NUMPLANES,
																				pViewRender->GetFrustum());
	}
	...
	for(int i = 0; i != iNumRenderablePortals; ++i)
	{
		...
		m_RecursiveViewComplexFrustums[m_iViewRecursionLevel + 1]
			.RemoveAll(); // clear any previously stored complex frustum

		if((pCurrentPortal->GetLinkedPortal() == NULL) || (pCurrentPortal == m_pRenderingViewExitPortal) ||
		   (pCurrentPortal->ShouldUpdatePortalView_BasedOnView(
				*pViewSetup, m_RecursiveViewComplexFrustums[m_iViewRecursionLevel]) == false))
		{
			// can't see through the portal, free up it's view id node for use elsewhere
			if(m_PortalViewIDNodeChain[m_iViewRecursionLevel]->ChildNodes[pCurrentPortal->m_iPortalViewIDNodeIndex] !=
			   NULL)
			{
				FreePortalViewIDNode(m_PortalViewIDNodeChain[m_iViewRecursionLevel]
										 ->ChildNodes[pCurrentPortal->m_iPortalViewIDNodeIndex]);
				m_PortalViewIDNodeChain[m_iViewRecursionLevel]->ChildNodes[pCurrentPortal->m_iPortalViewIDNodeIndex] =
					NULL;
			}
			continue;
		}

		Assert(m_PortalViewIDNodeChain[m_iViewRecursionLevel]->ChildNodes.Count() >
			   pCurrentPortal->m_iPortalViewIDNodeIndex);

		if(m_PortalViewIDNodeChain[m_iViewRecursionLevel]->ChildNodes[pCurrentPortal->m_iPortalViewIDNodeIndex] == NULL)
			m_PortalViewIDNodeChain[m_iViewRecursionLevel]->ChildNodes[pCurrentPortal->m_iPortalViewIDNodeIndex] =
				AllocPortalViewIDNode(m_HeadPortalViewIDNode.ChildNodes.Count());
		...
		if(bIsQueuedMode || // don't use pixel visibly as a skip check in queued mode, the data is simply too old.
		   pCurrentPortal->ShouldUpdatePortalView_BasedOnPixelVisibility(
			   pCurrentPortalViewNode->fScreenFilledByPortalSurfaceLastFrame_Normalized))
		{
			...
			// step 3, fill in stencil views (remember that in multiple depth situations that any subportals will run
			// through this function again before this section completes, thereby screwing with stencil settings)
			{
				...
				m_PortalViewIDNodeChain[m_iViewRecursionLevel + 1] =
					m_PortalViewIDNodeChain[m_iViewRecursionLevel]
						->ChildNodes[pCurrentPortal->m_iPortalViewIDNodeIndex];
	...
	{
		memcpy((void *)pViewSetup, &ViewBackup,
			   sizeof(CViewSetup)); // if we don't restore this, the view is permanently altered (in mid render of an
									// existing scene)
	}
...
void CPortalRender::DrawPortalsToTextures(CViewRender *pViewRender, const CViewSetup &cameraView)
{
	...
	for(int i = 0; i != iNumRenderablePortals; ++i)
	{
		...
		if((pCurrentPortal->ShouldUpdatePortalView_BasedOnView(
				cameraView, m_RecursiveViewComplexFrustums[m_iViewRecursionLevel]) == false) ||
...
//-----------------------------------------------------------------------------
// normalized for how many of the screen's possible pixels it takes up, less than zero indicates a lack of data from
// last frame
//-----------------------------------------------------------------------------
float CPortalRender::GetPixelVisilityForPortalSurface(const CPortalRenderable *pPortal) const
{
	PortalViewIDNode_t *pNode =
		m_PortalViewIDNodeChain[m_iViewRecursionLevel]->ChildNodes[pPortal->m_iPortalViewIDNodeIndex];
	if(pNode)
...
int CPortalRender::GetCurrentViewId() const
{
	...
	{
		Assert(m_PortalViewIDNodeChain[i]->iPrimaryViewID !=
			   m_PortalViewIDNodeChain[m_iViewRecursionLevel]->iPrimaryViewID);
	}
#endif
...
void OverlayCameraRenderTarget(const char *pszMaterialName, float flX, float flY, float w,
							   float h); // implemented in view_scene.cpp
...
void CPortalRender::HandlePortalPlaybackMessage(KeyValues *pKeyValues)
{
	...
	for(KeyValues *pCurr = pKeyValues->GetFirstTrueSubKey(); pCurr; pCurr = pCurr->GetNextTrueSubKey())
	{
		...
		if(nIndex < 0)
		{
			...
			const char *szType =
				pCurr->GetString("portalType", "flatBasic"); //"flatBasic" being the type commonly found in "Portal" mod
			...
			if(pPortal == NULL)
			{
				...
				Warning("CPortalRender::HandlePortalPlaybackMessage() unable to find creation function for portal "
						"type: %s\n",
						szType);
			}
			else
...
void CPortalRender::AddPortalCreationFunc(const char *szPortalType, PortalRenderableCreationFunc creationFunc)
{
	...
	for(int i = m_PortalRenderableCreators.Count(); --i >= 0;)
	{
		AssertMsg(FStrEq(m_PortalRenderableCreators[i].portalType.String(), szPortalType) == false,
				  "Multiple portal renderable creation functions for same type of portal renderable.");
	}
#endif
