...
public:
	...
	void LevelInitPreEntity()
	{
		...
		m_Materials.m_PortalRenderFixMaterials[0].Init("models/portals/portal_1_renderfix_dynamicmesh",
													   TEXTURE_GROUP_CLIENT_EFFECTS);
		m_Materials.m_PortalRenderFixMaterials[1].Init("models/portals/portal_2_renderfix_dynamicmesh",
													   TEXTURE_GROUP_CLIENT_EFFECTS);
		...
		// m_Materials.m_PortalLightTransfer_ShadowTexture.Init( "effects/flashlight001", TEXTURE_GROUP_OTHER ); //light
		// transfers disabled indefinitely

		m_Materials.m_pDepthDoubleViewMatrixVar =
			m_Materials.m_PortalDepthDoubler->FindVar("$alternateviewmatrix", NULL, false);
...
const FlatBasicPortalRenderingMaterials_t &CPortalRenderable_FlatBasic::m_Materials =
	s_FlatBasicPortalDrawingMaterials.m_Materials;
	  ...
	  m_bIsPortal2(false)
{
	...
	m_InternallyMaintainedData.m_ptCorners[0] = m_InternallyMaintainedData.m_ptCorners[1] =
		m_InternallyMaintainedData.m_ptCorners[2] = m_InternallyMaintainedData.m_ptCorners[3] =
...
void CPortalRenderable_FlatBasic::PortalMoved(void)
{
	...
	// Update the points on the portal which we add to PVS
	{
		...
		m_InternallyMaintainedData.m_ptCorners[0] =
			(m_InternallyMaintainedData.m_ptForwardOrigin + vScaledRight) + vScaledUp;
		...
		m_InternallyMaintainedData.m_VisData.m_fDistToAreaPortalTolerance = 64.0f;
		m_InternallyMaintainedData.m_iViewLeaf =
			enginetrace->GetLeafContainingPoint(m_InternallyMaintainedData.m_ptForwardOrigin);
	}

	m_InternallyMaintainedData.m_nSkyboxVisibleFromCorners =
		engine->IsSkyboxVisibleFromPoint(m_InternallyMaintainedData.m_ptForwardOrigin);
	for(int i = 0; i < 4 && (m_InternallyMaintainedData.m_nSkyboxVisibleFromCorners != SKYBOX_3DSKYBOX_VISIBLE); ++i)
	{
		...
		if((m_InternallyMaintainedData.m_nSkyboxVisibleFromCorners == SKYBOX_NOT_VISIBLE) ||
		   (nCornerVis != SKYBOX_NOT_VISIBLE))
		{
	...
	// render fix bounding planes
	{
		for(int i = 0; i != PORTALRENDERFIXMESH_OUTERBOUNDPLANES; ++i)
		{
			...
			Vector ptOnPlane = m_ptOrigin + (m_vUp * (fUpBlend * PORTAL_HALF_HEIGHT * 1.1f)) +
							   (m_vRight * (fRightBlend * PORTAL_HALF_WIDTH * 1.1f));

			m_InternallyMaintainedData.m_BoundingPlanes[i].Init(vNormal, vNormal.Dot(ptOnPlane));
		}

		m_InternallyMaintainedData.m_BoundingPlanes[PORTALRENDERFIXMESH_OUTERBOUNDPLANES].Init(
			-m_vForward, (-m_vForward).Dot(m_ptOrigin));
		m_InternallyMaintainedData.m_BoundingPlanes[PORTALRENDERFIXMESH_OUTERBOUNDPLANES + 1].Init(
			m_vForward, m_vForward.Dot(m_ptOrigin - (m_vForward * 5.0f)));
	}
	...
	m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration =
		(m_pLinkedPortal && // linked to another portal
		 (m_vForward.Dot(m_pLinkedPortal->m_ptOrigin - m_ptOrigin) >
		  0.0f) && // this portal looking in the general direction of the other portal
		 (m_vForward.Dot(m_pLinkedPortal->m_vForward) <
		  -0.7071f)); // within 45 degrees of facing directly at each other
	...
	if(m_pLinkedPortal != NULL)
	{
		...
		matrix3x4_t remoteToWorld(m_pLinkedPortal->m_vForward, -m_pLinkedPortal->m_vRight, m_pLinkedPortal->m_vUp,
								  m_pLinkedPortal->m_ptOrigin);
...
bool CPortalRenderable_FlatBasic::WillUseDepthDoublerThisDraw(void) const
{
	return g_pPortalRender->ShouldUseStencilsToRenderPortals() &&
		   m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration &&
		   (g_pPortalRender->GetRemainingPortalViewDepth() == 0) && (g_pPortalRender->GetViewRecursionLevel() > 1) &&
		   (g_pPortalRender->GetCurrentViewExitPortal() != this);
}

ConVar r_portal_use_complex_frustums("r_portal_use_complex_frustums", "1", FCVAR_CLIENTDLL,
									 "View optimization, turn this off if you get odd visual bugs.");
...
bool CPortalRenderable_FlatBasic::CalcFrustumThroughPortal(const Vector &ptCurrentViewOrigin, Frustum OutputFrustum)
{
	...
	if((iViewRecursionLevel == 0) &&
	   ((ptCurrentViewOrigin - m_ptOrigin).LengthSqr() <
		(PORTAL_HALF_HEIGHT * PORTAL_HALF_HEIGHT))) // FIXME: Player closeness check might need reimplementation
	{
	...
	// VPlane *pInputFrustum = view->GetFrustum();
	// //g_pPortalRender->m_RecursiveViewComplexFrustums[iViewRecursionLevel].Base(); int iInputFrustumPlaneCount = 6;
	// //g_pPortalRender->m_RecursiveViewComplexFrustums[iViewRecursionLevel].Count();
	...
	Vector *pInVerts =
		(Vector *)stackalloc(sizeof(Vector) * iAllocSize *
							 2); // possible to add 1 point per cut, 4 starting points, iInputFrustumPlaneCount cuts
	Vector *pOutVerts = pInVerts + iAllocSize;
	Vector *pTempVerts;

	// clip by first plane and put output into pInVerts
	int iVertCount = ClipPolyToPlane(m_InternallyMaintainedData.m_ptCorners, 4, pInVerts, pInputFrustum[0].m_Normal,
									 pInputFrustum[0].m_Dist, 0.01f);
	...
	for(i = 1; i != iInputFrustumPlaneCount; ++i)
	{
		...
		iVertCount =
			ClipPolyToPlane(pInVerts, iVertCount, pOutVerts, pInputFrustum[i].m_Normal, pInputFrustum[i].m_Dist, 0.01f);
		pTempVerts = pInVerts;
		pInVerts = pOutVerts;
		pOutVerts = pTempVerts; // swap vertex pointers
	}

	if(iVertCount < 3)
		return false; // nothing left in the frustum

	g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].SetCount(iVertCount +
																					  2); //+2 for near and far z planes
	...
	for(i = 0; i != iVertCount; ++i)
	{
		...
		g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(i).Init(
			vNormal, vNormal.Dot(ptTransformedCamera));
	}

	// Near Z
	g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(i).Init(
		m_pLinkedPortal->m_vForward, m_pLinkedPortal->m_InternallyMaintainedData.m_fPlaneDist);
	...
	++i;
	{
		...
		Vector ptOnPlane =
			pInputFrustum[iInputFrustumPlaneCount - 1].m_Dist * pInputFrustum[iInputFrustumPlaneCount - 1].m_Normal;
		g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(i).Init(
			vNormal, vNormal.Dot(m_matrixThisToLinked * ptOnPlane));
	}
	...
	if(iVertCount > 4)
	{
		...
		memcpy(Planes, g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Base(),
			   sizeof(VPlane) * iVertCount);
		...
		while(iVertCount > 4)
		{
			...
			// now we know the two points that we have to merge to one, project and make a merged point from the
			// surrounding lines
			if(fMinSide >= 0.1f) // only worth doing the math if it's actually going to be accurate and make a
								 // difference
			{
				Vector vLine1 = *p2 - *p1;
				Vector vLine2 = *p3 - *p4;
				...
				if(fNormalDot == 0.0f)
				{
					return false; // something went horribly wrong, bail and just suffer a slight framerate penalty for
								  // now
				}
	...
	OutputFrustum[FRUSTUM_NEARZ] =
		g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(iComplexCount - 2);
	OutputFrustum[FRUSTUM_FARZ] =
		g_pPortalRender->m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(iComplexCount - 1);
...
void CPortalRenderable_FlatBasic::RenderPortalViewToBackBuffer(CViewRender *pViewRender, const CViewSetup &cameraView)
{
...

	{
		...
		fCustomClipPlane[3] = vRemotePortalForward.Dot(
			ptRemotePortalPosition -
			(vRemotePortalForward * 0.5f)); // moving it back a smidge to eliminate visual artifacts for half-in objects

		pRenderContext->PushCustomClipPlane(
			fCustomClipPlane); // this is technically the same plane within recursive views, but pushing it anyway
							   // in-case something else has been added to the stack
	}

	{
		...
		render->Push3DView(portalView, 0, NULL, pViewRender->GetFrustum());
		{
			...
			if(m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration &&
			   (g_pPortalRender->GetRemainingPortalViewDepth() == 1))
			{
				// save the view matrix for usage with the depth doubler.
				// It's important that we do this AFTER using the depth doubler this frame to compensate for the fact
				// that the front buffer is 1 frame behind the current view matrix otherwise we get a lag effect when
				// the player changes their viewing angles
...
void CPortalRenderable_FlatBasic::RenderPortalViewToTexture(CViewRender *pViewRender, const CViewSetup &cameraView)
{
	...
	portalView.fov = cameraView.fov;
	portalView.m_bOrtho = false;
	portalView.m_flAspectRatio =
		(float)cameraView.width /
		(float)cameraView.height; // use the screen aspect ratio, 0.0f doesn't work as advertised
	...
	fCustomClipPlane[3] = m_pLinkedPortal->m_vForward.Dot(
		m_pLinkedPortal->m_ptOrigin -
		(m_pLinkedPortal->m_vForward *
		 0.5f)); // moving it back a smidge to eliminate visual artifacts for half-in objects
...

	{
...

		{
			...
			int nClearFlags = 0;
			Draw3dSkyboxworld_Portal(pViewRender, portalView, nClearFlags, bDrew3dSkybox, nSkyboxVisible,
									 pRenderTarget);
...
static void DrawComplexPortalMesh_SubQuad(Vector &ptBottomLeft, Vector &vUp, Vector &vRight, float *fSubQuadRect,
										  void *pBindEnt, const IMaterial *pMaterial,
										  const VMatrix *pReplacementViewMatrixForTexCoords = NULL)
{
...
void CPortalRenderable_FlatBasic::DrawComplexPortalMesh(
	const IMaterial *pMaterialOverride, float fForwardOffsetModifier) // generates and draws the portal mesh (Needed for
																	  // compatibility with fixed function rendering)
{
	PortalMeshPoint_t BaseVertices[4];

	Vector ptBottomLeft = m_ptOrigin + (m_vForward * (fForwardOffsetModifier)) - (m_vRight * PORTAL_HALF_WIDTH) -
						  (m_vUp * PORTAL_HALF_HEIGHT);
	...
	for(int i = 0; i != PORTAL_PROJECTION_MESH_SUBDIVIDE_HEIGHTCHUNKS; ++i)
	{
		...
		for(int j = 0; j != PORTAL_PROJECTION_MESH_SUBDIVIDE_WIDTHCHUNKS; ++j)
		{
			...
			DrawComplexPortalMesh_SubQuad(ptBottomLeft, vScaledUp, vScaledRight, fSubQuadRect, GetClientRenderable(),
										  pMaterial);
...
{
	IMaterialVar *pDepthDoubleViewMatrixVar =
		s_FlatBasicPortalDrawingMaterials.m_Materials.m_PortalDepthDoubler->FindVar("$alternateviewmatrix", NULL,
																					false);
...
void CPortalRenderable_FlatBasic::DrawSimplePortalMesh(const IMaterial *pMaterialOverride, float fForwardOffsetModifier)
{
	...
	{
		pRenderContext->Bind((IMaterial *)(const IMaterial *)g_pPortalRender->m_MaterialsAccess.m_Wireframe,
							 (CPortalRenderable *)this);
...
void CPortalRenderable_FlatBasic::DrawRenderFixMesh(const IMaterial *pMaterialOverride, float fFrontClipDistance)
{
	...
	if(vPortalCenterToCamera.LengthSqr() <
	   (PORTAL_HALF_HEIGHT * PORTAL_HALF_HEIGHT)) // FIXME: Player closeness check might need reimplementation
	{
		...
		if(fFrontClipDistance > fCameraDist) // never clip further out than the camera, we can see into the garbage
											 // space of the portal view's texture
...
{
	PortalMeshPoint_t *pInVerts = (PortalMeshPoint_t *)stackalloc(
		4 * (PORTALRENDERFIXMESH_OUTERBOUNDPLANES + 2) * 16 *
		sizeof(PortalMeshPoint_t)); // really only should need 4x points, but I'm paranoid
	PortalMeshPoint_t *pOutVerts = (PortalMeshPoint_t *)stackalloc(4 * (PORTALRENDERFIXMESH_OUTERBOUNDPLANES + 2) * 16 *
																   sizeof(PortalMeshPoint_t));
	...
	// clip by bounding area planes
	{
		for(int i = 0; i != (PORTALRENDERFIXMESH_OUTERBOUNDPLANES + 1); ++i)
		{
			...
			iVertCount = ClipPolyToPlane_LerpTexCoords(pInVerts, iVertCount, pOutVerts,
													   m_InternallyMaintainedData.m_BoundingPlanes[i].m_Normal,
													   m_InternallyMaintainedData.m_BoundingPlanes[i].m_Dist, 0.01f);
			pTempVerts = pInVerts;
			pInVerts = pOutVerts;
			pOutVerts = pTempVerts; // swap vertex pointers
		}
	...
	// clip by the viewing frustum
	{
		...
		for(int i = 0; i != FRUSTUM_NUMPLANES; ++i)
		{
			...
			iVertCount = ClipPolyToPlane_LerpTexCoords(pInVerts, iVertCount, pOutVerts, pFrustum[i].m_Normal,
													   pFrustum[i].m_Dist, 0.01f);
			pTempVerts = pInVerts;
			pInVerts = pOutVerts;
			pOutVerts = pTempVerts; // swap vertex pointers
		}
	...
	// project the points so we can fudge the numbers a bit and move them to exactly 0.0f depth
	{
		...
		for(int i = 0; i != iVertCount; ++i)
		{
			...
			pInVerts[i].vWorldSpacePosition.z = 0.00001f; // the primary reason we're projecting on the CPU to begin
														  // with
		}
	}
	...
	// render with identity transforms and clipping disabled
	{
		...
		if(mat_wireframe.GetBool())
			RenderPortalMeshConvexPolygon(
				pInVerts, iVertCount,
				materials->FindMaterial("shadertest/wireframe", TEXTURE_GROUP_CLIENT_EFFECTS, false), this);
...
//-----------------------------------------------------------------------------
// renders a quad that simulates fog as an overlay for something else (most notably the hole we create for stencil mode
// portals)
//-----------------------------------------------------------------------------
...
void CPortalRenderable_FlatBasic::DrawPortal(void)
{
	...
	else
	{
		...
		else if(g_pPortalRender->GetCurrentViewExitPortal() !=
				this) // don't render portals that our view is exiting from
		{
			if((m_fOpenAmount > 0.0f) && (m_fOpenAmount < 1.0f))
			{
				DrawSimplePortalMesh(m_Materials.m_Portal_Refract[((m_bIsPortal2) ? (1) : (0))]);
			}
			DrawSimplePortalMesh(m_Materials.m_PortalStaticOverlay[(
				(m_bIsPortal2) ? (1) : (0))]); // FIXME: find out why the projection mesh screws up at the second level
											   // of rendering in -nouserclip situations
		}
...
bool CPortalRenderable_FlatBasic::ShouldUpdatePortalView_BasedOnView(const CViewSetup &currentView,
																	 CUtlVector<VPlane> &currentComplexFrustum)
{
	if(m_pLinkedPortal == NULL)
		return false;
	...
	if((g_pPortalRender->GetViewRecursionLevel() == 0) &&
	   ((m_ptOrigin - vCameraPos).LengthSqr() <
		(PORTAL_HALF_HEIGHT * PORTAL_HALF_HEIGHT))) // FIXME: Player closeness check might need reimplementation
	{
		return true; // fudgery time. The player might not be able to see the surface, but they can probably see the
					 // render fix
	}
	...
	int allocSize = (6 + currentComplexFrustum.Count()); // possible to add 1 point per cut, 4 starting points, N plane
														 // cuts, 2 extra because I'm paranoid
	Vector *pInVerts = (Vector *)stackalloc(sizeof(Vector) * allocSize * 2);
	Vector *pOutVerts = pInVerts + allocSize;
	Vector *pTempVerts;

	// clip by first plane and put output into pInVerts
	int iVertCount = ClipPolyToPlane(m_InternallyMaintainedData.m_ptCorners, 4, pInVerts, currentFrustum[0].m_Normal,
									 currentFrustum[0].m_Dist, 0.01f);
	...
	for(int i = 1; i != iCurrentFrustmPlanes; ++i)
	{
		...
		iVertCount = ClipPolyToPlane(pInVerts, iVertCount, pOutVerts, currentFrustum[i].m_Normal,
									 currentFrustum[i].m_Dist, 0.01f);
		pTempVerts = pInVerts;
		pInVerts = pOutVerts;
		pOutVerts = pTempVerts; // swap vertex pointers
	}
...
bool CPortalRenderable_FlatBasic::ShouldUpdatePortalView_BasedOnPixelVisibility(
	float fScreenFilledByStencilMaskLastFrame_Normalized)
{
