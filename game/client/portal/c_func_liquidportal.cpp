...
RecvPropEHandle(RECVINFO(m_hLinkedPortal)), RecvPropFloat(RECVINFO(m_fFillStartTime)),
...
void CPortalRenderable_Func_LiquidPortal::RenderPortalViewToBackBuffer(CViewRender *pViewRender,
																	   const CViewSetup &cameraView)
{
	...
	if(g_pPortalRender->GetViewRecursionLevel() == 0)
	{
		bUseSeeThroughFrustum = CalcFrustumThroughPortal(cameraView.origin, seeThroughFrustum,
														 pViewRender->GetFrustum(), FRUSTUM_NUMPLANES);
	}
	else
	{
		bUseSeeThroughFrustum = CalcFrustumThroughPortal(cameraView.origin, seeThroughFrustum);
	...
	portalView.m_flAspectRatio =
		cameraView.m_flAspectRatio; // use the screen aspect ratio, 0.0f doesn't work as advertised
...
void CPortalRenderable_Func_LiquidPortal::RenderPortalViewToTexture(CViewRender *pViewRender,
																	const CViewSetup &cameraView)
{
}
...
bool CPortalRenderable_Func_LiquidPortal::CalcFrustumThroughPortal(const Vector &ptCurrentViewOrigin,
																   Frustum OutputFrustum, const VPlane *pInputFrustum,
																   int iInputFrustumPlaneCount)
{
	return false;
}
...
bool CPortalRenderable_Func_LiquidPortal::ShouldUpdatePortalView_BasedOnView(const CViewSetup &currentView,
...
void CPortalRenderable_Func_LiquidPortal::DrawInnerLiquid(
	bool bClipToBounds, float fOpacity,
	const IMaterial *pMaterial) // quads in front of camera clipped to box dimensions
{
	...
	// vOrigin = CurrentViewOrigin() + vForward * (view->GetViewSetup()->zNear + 0.011f); //experimentation has shown
	// this to be the optimal distance on the Nvidia 6800 cards we develop on
	...
	PortalMeshPoint_t *pInVerts = (PortalMeshPoint_t *)stackalloc(
		4 * (6) * 2 * sizeof(PortalMeshPoint_t)); // really only should need 2x points, but I'm paranoid
	...
	if(bClipToBounds)
	{
		...
		iVertCount = ClipPolyToPlane_LerpTexCoords(
			WorkVertices, 4, pInVerts, Vector(0.0f, 0.0f, -1.0f),
			-(m_vAABBMins.z + ((m_vAABBMaxs.z - m_vAABBMins.z) * GetFillInterpolationAmount())), 0.01f);
		...
		for(int i = 0; i != 5; ++i)
		{
			...
			iVertCount = ClipPolyToPlane_LerpTexCoords(pInVerts, iVertCount, pOutVerts, *(Vector *)m_fBoundingPlanes[i],
													   m_fBoundingPlanes[i][3], 0.01f);
			pTempVerts = pInVerts;
			pInVerts = pOutVerts;
...
CLiquidPortal_InnerLiquidEffect::CLiquidPortal_InnerLiquidEffect(void)
	: m_bEnable(true), m_pImmersionPortal(NULL), m_bFadeBackToReality(false), m_fFadeBackTimeLeft(0.0f)
{
}
