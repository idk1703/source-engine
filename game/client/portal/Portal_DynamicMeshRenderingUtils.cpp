...
int ClipPolyToPlane_LerpTexCoords(PortalMeshPoint_t *inVerts, int vertCount, PortalMeshPoint_t *outVerts,
...
void RenderPortalMeshConvexPolygon(PortalMeshPoint_t *pVerts, int iVertCount, const IMaterial *pMaterial, void *pBind)
{
	...
	// any convex polygon can be rendered with a triangle strip by starting at a vertex and alternating vertices from
	// each side
...
void Clip_And_Render_Convex_Polygon(PortalMeshPoint_t *pVerts, int iVertCount, const IMaterial *pMaterial, void *pBind)
{
	PortalMeshPoint_t *pInVerts = (PortalMeshPoint_t *)stackalloc(
		iVertCount * 4 * sizeof(PortalMeshPoint_t)); // really only should need 2x points, but I'm paranoid
	...
	// clip by the viewing frustum
	{
		...
		iVertCount = ClipPolyToPlane_LerpTexCoords(pVerts, iVertCount, pInVerts, pFrustum[0].m_Normal,
												   pFrustum[0].m_Dist, 0.01f);
	...
	if(mat_wireframe.GetBool())
		RenderPortalMeshConvexPolygon(
			pOutVerts, iVertCount, materials->FindMaterial("shadertest/wireframe", TEXTURE_GROUP_CLIENT_EFFECTS, false),
			pBind);
