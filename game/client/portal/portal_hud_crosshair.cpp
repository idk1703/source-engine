...
{
	// NOTE: Portal crosshair should no longer be in use, but I'm leaving the code here until X360 lock down... we don't
	// want to draw this ever. -Jeep
	return false;
	...
	bool bNeedsDraw = m_pCrosshair && crosshair.GetInt() && !engine->IsDrawingLoadingImage() && !engine->IsPaused() &&
