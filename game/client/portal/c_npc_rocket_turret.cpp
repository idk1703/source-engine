...
RecvPropInt(RECVINFO(m_iLaserState)), RecvPropInt(RECVINFO(m_nSiteHalo)), RecvPropVector(RECVINFO(m_vecCurrentAngles)),
...
void C_NPC_RocketTurret::LaserOn(void)
{
	...
	if(UTIL_Portal_TraceRay_Beam(rayPath, MASK_SHOT, &m_filterBeams, &fEndFraction))
		vEndPoint =
			vecMuzzle + vecAimDir * ROCKET_TURRET_LASER_RANGE; // Trace went through portal and endpoint is unknown
	else
...
{
	return (MAX(0.0f, sinf(gpGlobals->curtime * M_PI + m_fPulseOffset))) *
ROCKET_TURRET_END_POINT_PULSE_SCALE
+ 1.0f;
}