...
public:
	...
	{
		return (m_StaticOutput) ? m_StaticOutput->GetOwningMaterial() : NULL;
	}
...
float CPortalStaticProxy::ComputeStaticAmount(CPortalRenderable_FlatBasic *pFlatBasic)
{
	...
	else if((g_pPortalRender->GetRemainingPortalViewDepth() == 1) &&
			(pFlatBasic->m_fSecondaryStaticAmount >
			 flStaticAmount)) // fading in from no views to another view (player just walked through it)
	{
