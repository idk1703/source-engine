...
RecvPropString(RECVINFO(m_szSoundName)), RecvPropFloat(RECVINFO(m_flOuterRadius)),
	RecvPropFloat(RECVINFO(m_flInnerRadius)), RecvPropInt(RECVINFO(m_nSignalID)),
END_RECV_TABLE
()
...
RecvPropEHandle(RECVINFO(m_hDinosaur_Signal)), RecvPropBool(RECVINFO(m_bAlreadyDiscovered)),
END_RECV_TABLE
()
...
void C_Portal_Dinosaur::SetupSounds()
{
	...
	{
		m_pNormalSound =
			CSoundEnvelopeController::GetController().SoundCreate(filter, entindex(), "Portal.room1_radio");
	...
	{
		m_pSignalSound = CSoundEnvelopeController::GetController().SoundCreate(filter, entindex(),
																			   m_hDinosaur_Signal->m_szSoundName);
		// m_pSignalSound = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(),
		// "UpdateItem.Signal" );
...
void C_Portal_Dinosaur::ScanForSounds()
{
	...
	float flOuterBlend =
		RemapValClamped(flDist, m_hDinosaur_Signal.Get()->m_flOuterRadius,
						m_hDinosaur_Signal.Get()->m_flOuterRadius - (flRadiusDelta * 0.5f), 1.0f, 0.0f);
	float flInnerBlend = RemapValClamped(flDist, m_hDinosaur_Signal.Get()->m_flOuterRadius - (flRadiusDelta * 0.5f),
										 m_hDinosaur_Signal.Get()->m_flInnerRadius, 0.0f, 1.0f);
	...
	if(m_pStaticSound)
	{
		...
		{
			flMidBlend = RemapValClamped(flDist, m_hDinosaur_Signal.Get()->m_flOuterRadius - (flRadiusDelta * 0.5f),
										 m_hDinosaur_Signal.Get()->m_flInnerRadius, 1.0f, 0.0f);
		}
		else
		{
			flMidBlend =
				RemapValClamped(flDist, m_hDinosaur_Signal.Get()->m_flOuterRadius,
								m_hDinosaur_Signal.Get()->m_flOuterRadius - (flRadiusDelta * 0.5f), 0.0f, 1.0f);
