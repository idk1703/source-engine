...
C_PortalGhostRenderable::C_PortalGhostRenderable(C_Prop_Portal *pOwningPortal, C_BaseEntity *pGhostSource,
												 RenderGroup_t sourceRenderGroup, const VMatrix &matGhostTransform,
												 float *pSharedRenderClipPlane, bool bLocalPlayer)
...
QAngle const &C_PortalGhostRenderable::GetRenderAngles(void)
{
	...
	m_ReferencedReturns.qRenderAngle =
		TransformAnglesToWorldSpace(m_pGhostedRenderable->GetRenderAngles(), m_matGhostTransform.As3x4());
	return m_ReferencedReturns.qRenderAngle;
}
...
bool C_PortalGhostRenderable::SetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime)
{
	...
	if(m_pGhostedRenderable->SetupBones(pBoneToWorldOut, nMaxBones, boneMask, currentTime))
	{
		if(pBoneToWorldOut)
		{
			for(int i = 0; i != nMaxBones; ++i) // FIXME: nMaxBones is most definitely greater than the actual number of
												// bone transforms actually used, find the subset somehow
			{
...
const matrix3x4_t &C_PortalGhostRenderable::RenderableToWorldTransform()
{
	...
	ConcatTransforms(m_matGhostTransform.As3x4(), m_pGhostedRenderable->RenderableToWorldTransform(),
					 m_ReferencedReturns.matRenderableToWorldTransform);
	return m_ReferencedReturns.matRenderableToWorldTransform;
}
...
int C_PortalGhostRenderable::DrawModel(int flags)
{
	...
	else
	{
		...
		render->DrawBrushModelEx(m_pGhostedRenderable, (model_t *)m_pGhostedRenderable->GetModel(), GetRenderOrigin(),
								 GetRenderAngles(), mode);
