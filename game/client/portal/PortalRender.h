...
#define MAX_PORTAL_RECURSIVE_VIEWS \
	11 // maximum number of recursions we allow when drawing views through portals. Seeing as how 5 is extremely choppy
	   // under best conditions and is barely visible, 10 is a safe limit. Adding one because 0 tends to be the primary
	   // view in most arrays of this size
...
public:
	...
	virtual void DrawStencilMask(void) {}; // Draw to wherever you should see through the portal. The mask will later be
										   // filled with the portal view.
	virtual void DrawPostStencilFixes(
		void) {}; // After done drawing to the portal mask, we need to fix the depth buffer as well as fog. So draw your
				  // mesh again, writing to z and with the fog color alpha'd in by distance
	...
	//-----------------------------------------------------------------------------
	// Based on view, will the camera be able to see through the portal this frame? This will allow the stencil mask to
	// start being tested for pixel visibility.
	virtual bool ShouldUpdatePortalView_BasedOnView(const CViewSetup &currentView,
													CUtlVector<VPlane> &currentComplexFrustum)
	{
		return false;
	};

	// Stencil mode only: You stated the portal was visible based on view, and this is how much of the screen your
	// stencil mask took up last frame. Still want to draw this frame? Values less than zero indicate a lack of data
	// from last frame
	virtual bool ShouldUpdatePortalView_BasedOnPixelVisibility(float fScreenFilledByStencilMaskLastFrame_Normalized)
	{
		return (fScreenFilledByStencilMaskLastFrame_Normalized != 0.0f);
	}; // < 0 is unknown visibility, > 0 is known to be partially visible
	...
	virtual void DrawPortal(void) {}; // sort of like what you'd expect to happen in C_BaseAnimating::DrawModel() if
									  // portals were fully compatible with models

	virtual C_BaseEntity *PortalRenderable_GetPairedEntity(void)
	{
		return NULL;
	}; // Pairing a portal with an entity is common but not required. Accessing that entity allows the CPortalRender
...
protected:
	//-----------------------------------------------------------------------------
	// Wrap the draw of the surface that makes use of your portal render targets with these. Only required for texture
	// mode, but won't hurt stencil mode.
	...
	void ViewDrawScene_PortalStencil(CViewRender *pViewRender, const CViewSetup &viewIn,
									 ViewCustomVisibility_t *pCustomVisibility);
	void Draw3dSkyboxworld_Portal(CViewRender *pViewRender, const CViewSetup &viewIn, int &nClearFlags,
								  bool &bDrew3dSkybox, SkyboxVisibility_t &nSkyboxVisible,
								  ITexture *pRenderTarget = NULL);
	void ViewDrawScene(CViewRender *pViewRender, bool bDrew3dSkybox, SkyboxVisibility_t nSkyboxVisible,
					   const CViewSetup &viewIn, int nClearFlags, view_id_t viewID, bool bDrawViewModel = false,
					   int baseDrawFlags = 0, ViewCustomVisibility_t *pCustomVisibility = NULL);
...
private:
	int m_iPortalViewIDNodeIndex; // each PortalViewIDNode_t has a child node link for each CPortalRenderable in
								  // CPortalRender::m_ActivePortals. This portal follows the same indexed link from each
								  // node
...
{
	CUtlVector<PortalViewIDNode_t *> ChildNodes; // links will only be non-null if they're useful (can see through the
												 // portal at that depth and view setup)
	int iPrimaryViewID;
	// skybox view id is always primary + 1

	// In stencil mode this wraps CPortalRenderable::DrawStencilMask() and gives previous frames' results to
	// CPortalRenderable::RenderPortalViewToBackBuffer() In texture mode there's no good spot to auto-wrap occlusion
	// tests. So you'll need to wrap it yourself for that.
...
public:
	...
	// Returns the current View IDs. Portal View IDs will change often (especially with recursive views) and should not
	// be cached
	...
	float GetPixelVisilityForPortalSurface(
		const CPortalRenderable *pPortal) const; // normalized for how many of the screen's possible pixels it takes up,
												 // less than zero indicates a lack of data from last frame

	// Returns the remaining number of portals to render within other portals
	// lets portals know that they should do "end of the line" kludges to cover up that portals don't go infinitely
	// recursive
	int GetRemainingPortalViewDepth() const;

	inline CPortalRenderable *GetCurrentViewEntryPortal(void) const
	{
		return m_pRenderingViewForPortal;
	}; // if rendering a portal view, this is the portal the current view enters into
	inline CPortalRenderable *GetCurrentViewExitPortal(void) const
	{
		return m_pRenderingViewExitPortal;
	}; // if rendering a portal view, this is the portal the current view exits from
	...
	void UpdateDepthDoublerTexture(const CViewSetup &viewSetup); // our chance to update all depth doubler texture
																 // before the view model is added to the back buffer

	void EnteredPortal(CPortalRenderable *pEnteredPortal); // does a bit of internal maintenance whenever the
														   // player/camera has logically passed the portal threshold
	...
	CViewSetup m_RecursiveViewSetups[MAX_PORTAL_RECURSIVE_VIEWS]; // before we recurse into a view, we backup the view
																  // setup here for reference
...
private:
	...
	PortalViewIDNode_t m_HeadPortalViewIDNode; // pseudo node. Primary view id will be VIEW_MAIN. The child links are
											   // what we really care about
	PortalViewIDNode_t
		*m_PortalViewIDNodeChain[MAX_PORTAL_RECURSIVE_VIEWS]; // the view id node chain we're following, 0 always being
															  // &m_HeadPortalViewIDNode (offsetting by 1 seems like
															  // it'd cause bugs in the long run)
...
private:
	...
	int m_iRemainingPortalViewDepth; // let's portals know that they should do "end of the line" kludges to cover up
									 // that portals don't go infinitely recursive
...
public:
	// frustums with more (or less) than 6 planes. Store each recursion level's custom frustum here so further
	// recursions can be better optimized. When going into further recursions, if you've failed to fill in a complex
	// frustum, the standard frustum will be copied in. So all parent levels are guaranteed to contain valid data
...
inline void CPortalRenderable::ViewDrawScene_PortalStencil(CViewRender *pViewRender, const CViewSetup &viewIn,
														   ViewCustomVisibility_t *pCustomVisibility)
{
	pViewRender->ViewDrawScene_PortalStencil(viewIn, pCustomVisibility);
}

inline void CPortalRenderable::Draw3dSkyboxworld_Portal(CViewRender *pViewRender, const CViewSetup &viewIn,
														int &nClearFlags, bool &bDrew3dSkybox,
...
inline void CPortalRenderable::ViewDrawScene(CViewRender *pViewRender, bool bDrew3dSkybox,
											 SkyboxVisibility_t nSkyboxVisible, const CViewSetup &viewIn,
											 int nClearFlags, view_id_t viewID, bool bDrawViewModel, int baseDrawFlags,
											 ViewCustomVisibility_t *pCustomVisibility)
{
	pViewRender->ViewDrawScene(bDrew3dSkybox, nSkyboxVisible, viewIn, nClearFlags, viewID, bDrawViewModel,
							   baseDrawFlags, pCustomVisibility);
}
...
inline void CPortalRenderable::SetViewEntranceAndExitPortals(CPortalRenderable *pEntryPortal,
															 CPortalRenderable *pExitPortal)
{
