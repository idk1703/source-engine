...
public:
	...
	C_PortalGhostRenderable(C_Prop_Portal *pOwningPortal, C_BaseEntity *pGhostSource, RenderGroup_t sourceRenderGroup,
							const VMatrix &matGhostTransform, float *pSharedRenderClipPlane, bool bLocalPlayer);
	...
	// virtual void					DoAnimationEvents( void ) { NULL; }; //TODO: find out if there's something we should be doing
	// with this virtual IPVSNotify*				GetPVSNotifyInterface() { return NULL; }; virtual bool
	// ShouldReceiveProjectedTextures( int flags ) { return false; };//{ return
	// m_pGhostedRenderable->ShouldReceiveProjectedTextures( flags ); };
	...
	// virtual ModelInstanceHandle_t	GetModelInstance() { return m_pGhostedRenderable->GetModelInstance(); }; //TODO:
	// find out if sharing an instance causes bugs
