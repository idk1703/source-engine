...
public:
	...
	// generates a 8x6 tiled set of quads, each clipped to the view frustum. Helps vs11/ps11 portal shaders interpolate
	// correctly. Not necessary for vs20/ps20 portal shaders or stencil mode.
	...
	// When we're in a configuration that sees through recursive portal views to a depth of 2, we should be able to
	// cheaply approximate even further depth using pixels from previous frames
	virtual void DrawDepthDoublerMesh(float fForwardOffsetModifier = 0.25f);

	virtual void DrawPortal(void);

	virtual void DrawPreStencilMask(
		void); // Draw to wherever you can see through the portal. The mask will later be filled with the portal view.
	...
	virtual bool ShouldUpdatePortalView_BasedOnView(
		const CViewSetup &currentView,
		CUtlVector<VPlane>
			&currentComplexFrustum); // portal is both visible, and will display at least some portion of a remote view
	...
	{
		return m_InternallyMaintainedData.m_nSkyboxVisibleFromCorners;
	};
	virtual bool DoesExitViewIntersectWaterPlane(float waterZ, int leafWaterDataID) const;

	bool WillUseDepthDoublerThisDraw(void)
		const; // returns true if the DrawPortal() would draw a depth doubler mesh if you were to call it right now
...
protected:
	...
	// renders a quad that simulates fog as an overlay for something else (most notably the hole we create for stencil
	// mode portals)
	...
	struct FlatBasicPortal_InternalData_t
	{
		...
		VMatrix m_DepthDoublerTextureView; // cached version of view matrix at depth 1 for use when drawing the depth
										   // doubler mesh
		bool m_bUsableDepthDoublerConfiguration; // every time a portal moves we re-evaluate whether the depth doubler
												 // will reasonably approximate more views
