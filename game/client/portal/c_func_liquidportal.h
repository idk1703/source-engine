...
public:
	...
	bool CalcFrustumThroughPortal(const Vector &ptCurrentViewOrigin, Frustum OutputFrustum,
								  const VPlane *pInputFrustum = NULL, int iInputFrustumPlaneCount = 0);
	...
	virtual bool ShouldUpdatePortalView_BasedOnView(
		...
		Frustum currentFrustum); // portal is both visible, and will display at least some portion of a remote view
	...
	void DrawInnerLiquid(bool bClipToBounds = true, float fOpacity = 1.0f,
						 const IMaterial *pMaterial = NULL); // quads in front of camera clipped to box dimensions
