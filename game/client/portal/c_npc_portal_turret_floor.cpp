...
RecvPropBool(RECVINFO(m_bOutOfAmmo)), RecvPropBool(RECVINFO(m_bLaserOn)), RecvPropInt(RECVINFO(m_sLaserHaloSprite)),
...
void C_NPC_Portal_FloorTurret::LaserOn(void)
{
	...
	if(!m_pBeam)
	{
		...
		m_pBeam->SetNoise(0);
		m_pBeam->SetWidth(IsX360() ? (1.5f)
								   : (0.75f)); // On low end TVs these lasers are very hard to see at a distance
	...
	if(UTIL_Portal_TraceRay_Beam(rayPath, MASK_SHOT, &traceFilter, &fEndFraction))
		vEndPoint = vecMuzzle +
					vecMuzzleDir * FLOOR_TURRET_PORTAL_LASER_RANGE; // Trace went through portal and endpoint is unknown
	else
...
{
	return ((MAX(0.0f, sinf(gpGlobals->curtime * M_PI + m_fPulseOffset))) * FLOOR_TURRET_PORTAL_END_POINT_PULSE_SCALE +
			3.0f) *
		   (IsX360() ? (3.0f) : (1.5f));
}