...
void __MsgFunc_EntityPortalled(bf_read &msg)
{
	...
	bool bAlreadyTeleported = ((ptCurrentPosition - ptNewPosition).LengthSqr() <
							   (ptInvCurrentPosition - ptNewPosition)
								   .LengthSqr()); // newest network update closer to destination than transformed to
												  // destination indicates it's already been transformed
...
static ConVar portal_demohack(
	"portal_demohack", "0", FCVAR_ARCHIVE,
	"Do the demo_legacy_rollback setting to help during demo playback of going through portals.");
...
class C_PortalInitHelper : public CAutoGameSystem
{
	virtual bool Init()
	{
		...
		if(portal_demohack.GetBool())
		{
			...
			demo_legacy_rollback_ref.SetValue(
				false); // Portal demos are wrong if the eyes rollback as far as regular demos
		}
...
void C_Prop_Portal::Simulate()
{
	...
	int iEntsNearPortal = UTIL_EntitiesInSphere(pEntsNearPortal, 1024, GetNetworkOrigin(), PORTAL_HALF_HEIGHT, 0,
	...
	if(iEntsNearPortal != 0)
	{
		...
		for(int i = 0; i != iEntsNearPortal; ++i)
		{
			...
			Assert(dynamic_cast<C_Prop_Portal *>(pEntity) ==
				   NULL); // should have been killed with (pEntity->GetMoveType() == MOVETYPE_NONE) check. Infinite
						  // recursion is infinitely bad.
	...
	{
		bool *bStillInUse =
			(bool *)stackalloc(sizeof(bool) * (m_GhostRenderables.Count() + m_hGhostingEntities.Count()));
		...
		for(int i = m_hGhostingEntities.Count(); --i >= 0;)
		{
			...
			C_PortalGhostRenderable *pNewGhost =
				new C_PortalGhostRenderable(this, pRenderable, pEntity->GetRenderGroup(), m_matrixThisToLinked,
											m_fGhostRenderablesClip, (pEntity == pLocalPlayer || bIsHeldWeapon));
			Assert(pNewGhost);
...
void C_Prop_Portal::OnDataChanged(DataUpdateType_t updateType)
{
	...
	bool bPortalMoved = ((PreDataChanged.m_vOrigin != m_ptOrigin) || (PreDataChanged.m_qAngles != GetNetworkAngles()) ||
						 (PreDataChanged.m_bActivated == false) || (PreDataChanged.m_bIsPortal2 != m_bIsPortal2));
	...
	if(m_bActivated)
	{
		...
		if(bPortalMoved)
		{
			...
			if(pRemote)
			{
				...
				AssertMsg((ShadowHandle == CLIENTSHADOW_INVALID_HANDLE) ||
							  (TransformedLighting.m_LightShadowHandle == CLIENTSHADOW_INVALID_HANDLE),
						  "Two shadow handles found, should only have one shared handle");
				AssertMsg((pFakeLight == NULL) || (TransformedLighting.m_pEntityLight == NULL),
						  "two lights found, should only have one shared light");
