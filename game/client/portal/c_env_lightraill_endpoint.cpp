...
RecvPropFloat(RECVINFO(m_flSmallScale)), RecvPropFloat(RECVINFO(m_flLargeScale)), RecvPropInt(RECVINFO(m_nState)),
	RecvPropFloat(RECVINFO(m_flDuration)), RecvPropFloat(RECVINFO(m_flStartTime)), RecvPropInt(RECVINFO(m_spawnflags)),
END_RECV_TABLE
()
...
void C_Env_Lightrail_Endpoint::UpdateIdle(float percentage)
{
	...
	for(int i = 0; i < numParticles; i++)
	{
		...
		sParticle = (SimpleParticle *)m_pAttractorEmitter->AddParticle(
			sizeof(SimpleParticle), m_pAttractorEmitter->GetPMaterial("effects/strider_muzzle"), offset);
...
void C_Env_Lightrail_Endpoint::UpdateCharging(float percentage)
{
	...
	for(int i = 0; i < 2; i++)
	{
		sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
			sizeof(SimpleParticle), m_pSimpleEmitter->GetPMaterial("effects/strider_muzzle"), GetAbsOrigin());
...
void C_Env_Lightrail_Endpoint::UpdateLargeFX(void)
{
	...
	{
		sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
			sizeof(SimpleParticle), m_pSimpleEmitter->GetPMaterial("effects/combinemuzzle2"), GetAbsOrigin());
	...
	for(int i = 0; i < 4; i++)
	{
		...
		sParticle = (SimpleParticle *)m_pAttractorEmitter->AddParticle(
			sizeof(SimpleParticle), m_pAttractorEmitter->GetPMaterial("effects/combinemuzzle2_dark"), offset);
