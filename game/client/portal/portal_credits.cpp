...
void CHudPortalCredits::ReadLyrics(KeyValues *pKeyValue)
{
	...
	while(pKVNames)
	{
		...
		{
			g_pVGuiLocalize->ConvertANSIToUnicode(Credits.szCreditName, Credits.szLyricLine,
	...
	if(gpGlobals->curtime - m_flLastPaintTime > 0.05)
	{
		...
		if(iCounter > (int)m_flScrollTime && m_iYOffset < (m_CreditsList.Count() - 1))
		{
			   ...
			   m_iYOffset < (m_CreditsList.Count() - 1))
			{
				...
				vgui::HScheme scheme = vgui::scheme()->GetScheme("ClientScheme");
				vgui::HFont m_hTFont =
					vgui::scheme()->GetIScheme(scheme)->GetFont(m_CreditsList[m_iYOffset].szFontName, true);
...
void CHudPortalCredits::DrawPortalAsciiArt(void)
{
	...
	for(int i = 0; i < 20; i++)
	{
		...
		surface()->DrawSetTextPos(20 + (iWidth / 2) + m_iAAScreenXOffset,
...
void CHudPortalCredits::DrawPortalOutroCreditsLyrics(void)
{
	...
	if(m_flLyricsStartTime == -1)
		m_flLyricsStartTime = gpGlobals->curtime;
	...
	for(int i = 0; i < m_LyricsList.Count(); i++)
	{
		...
		if(pCredit->flTimeStart > flCurTime)
			break;
		if(pCredit->flTimeStart <= flCurTime && pCredit->flTimeEnd > flCurTime)
		{
			...
			m_iXOffset =
				(int)(((flCurTime - pCredit->flTimeStart) / pCredit->flTimeInit) * Q_wcslen(pCredit->szLyricLine)) + 1;
			if(m_iXOffset < 1)
				m_iXOffset = 1;
		...
		if(!bBorders)
		{
			...
			while(iBorderY < (iTall - (iFontTall * 3)))
			{
				...
				if(iBorderY < (iTall / 2))
				{
					...
					surface()->DrawSetTextPos((iWidth - iBorderPipeWidth) + m_iScreenXOffset,
	...
	for(int i = iCreditsListCount - 1; i >= 0; i--)
	{
		...
		if(pCredit->flTimeStart <= flCurTime && pCredit->flTimeEnd > flCurTime)
		{
			...
			m_iYOffsetNames = i;
			m_iXOffsetNames =
				(int)(((flCurTime - pCredit->flTimeStart) / pCredit->flTimeInit) * Q_strlen(pCredit->szCreditName)) + 1;
...
void CHudPortalCredits::PreparePortalOutroCredits(void)
{
	...
	for(int i = 0; i < m_CreditsList.Count(); i++)
	{
		...
		if(iHeight == -1)
			iHeight = (iTall / 2) - (surface()->GetFontTall(m_hTFont) + m_flSeparation);
		else
			iHeight += surface()->GetFontTall(m_hTFont) + m_flSeparation;
