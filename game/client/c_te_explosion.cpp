...
RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropFloat(RECVINFO(m_fScale)), RecvPropInt(RECVINFO(m_nFrameRate)),
	RecvPropInt(RECVINFO(m_nFlags)), RecvPropVector(RECVINFO(m_vecNormal)), RecvPropInt(RECVINFO(m_chMaterialType)),
	RecvPropInt(RECVINFO(m_nRadius)), RecvPropInt(RECVINFO(m_nMagnitude)),
END_RECV_TABLE
()
...
void C_TEExplosion::AffectRagdolls(void)
{
	...
	::partition->EnumerateElementsInSphere(PARTITION_CLIENT_RESPONSIVE_EDICTS, m_vecOrigin, m_nRadius, false,
										   &ragdollEnum);
}
...
void TE_Explosion(IRecipientFilter &filter, float delay, KeyValues *pKeyValues)
{
	...
	TE_Explosion(filter, 0.0f, &vecOrigin, nModelIndex, flScale, nFrameRate, nFlags, nRadius, nMagnitude, &vecNormal,
				 (unsigned char)nMaterialType, false);
}