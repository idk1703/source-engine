...
void CHudRadar::Paint()
{
	...
	for(int i = 0; i < m_iNumRadarContacts; i++)
	{
		...
		if(RADAR_USE_ICONS)
		{
			...
			DrawIconOnRadar(pContact->m_vecOrigin, pLocalPlayer, pContact->m_iType, RADAR_IGNORE_Z, 255, 255, 255,
							alpha + flicker);
		}
		else
		{
			DrawPositionOnRadar(pContact->m_vecOrigin, pLocalPlayer, pContact->m_iType, RADAR_IGNORE_Z, 255, 255, 255,
								alpha);
		}
	}
...
//		-etc-
//---------------------------------------------------------
bool CHudRadar::WorldToRadar(const Vector location, const Vector origin, const QAngle angles, float &x, float &y,
							 float &z_delta, float &scale)
{
	bool bInRange = true;
...
void CHudRadar::DrawPositionOnRadar(Vector vecPos, C_BasePlayer *pLocalPlayer, int type, int flags, int r, int g, int b,
									int a)
{
	float x, y, z_delta;
	int iBaseDotSize = 3;
...
void CHudRadar::DrawIconOnRadar(Vector vecPos, C_BasePlayer *pLocalPlayer, int type, int flags, int r, int g, int b,
								int a)
{
	float x, y, z_delta;
	int wide, tall;
