...
{
	int drawn = 0;
	if(m_nReferencePlayer <= 0 || m_nReferencePlayer > gpGlobals->maxClients)
	{
		return drawn;
	};
	...
	if(player)
	{
		...
		drawn = modelrender->DrawModel(flags, this, MODEL_INSTANCE_INVALID, m_nReferencePlayer, GetModel(),
									   GetAbsOrigin(), GetAbsAngles(), m_nSkin, m_nBody, m_nHitboxSet);
	}
