...
RecvPropInt(RECVINFO(m_nState)), RecvPropFloat(RECVINFO(m_flDuration)), RecvPropFloat(RECVINFO(m_flStartTime)),
END_RECV_TABLE
()
...
void C_AlyxEmpEffect::UpdateCharging(float percentage)
{
	...
	while(m_tParticleSpawn.NextEvent(dTime))
	{
		...
		sParticle = (SimpleParticle *)m_pSimpleEmitter->AddParticle(
			sizeof(SimpleParticle), m_pSimpleEmitter->GetPMaterial(EMP_PARTICLES), GetAbsOrigin());
