...
RecvPropFloat(RECVINFO(m_flTimeBurnOut)), RecvPropFloat(RECVINFO(m_flScale)), RecvPropInt(RECVINFO(m_bLight)),
	RecvPropInt(RECVINFO(m_bSmoke)), RecvPropInt(RECVINFO(m_bPropFlare)),
END_RECV_TABLE
()
...
void C_Flare::RestoreResources(void)
{
	...
	{
		m_pParticle[0] =
			(SimpleParticle *)AddParticle(sizeof(SimpleParticle), GetPMaterial("effects/redflare"), GetAbsOrigin());
	...
	{
		m_pParticle[1] = (SimpleParticle *)AddParticle(sizeof(SimpleParticle), GetPMaterial("effects/yellowflare_noz"),
													   GetAbsOrigin());
	...
	if(m_bSmoke)
	{
		while(m_teSmokeSpawn.NextEvent(dt))
		{
			...
			SimpleParticle *sParticle =
				(SimpleParticle *)AddParticle(sizeof(SimpleParticle), g_Mat_DustPuff[1], smokeOrg);
			...
			sParticle->m_vecVelocity = Vector(random->RandomFloat(-16.0f, 16.0f), random->RandomFloat(-16.0f, 16.0f),
											  random->RandomFloat(8.0f, 16.0f) + 32.0f);
