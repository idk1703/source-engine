...
RecvPropVector(RECVINFO(m_vecHitPos)), RecvPropVector(RECVINFO(m_vecIKTarget[0])),
	RecvPropVector(RECVINFO(m_vecIKTarget[1])), RecvPropVector(RECVINFO(m_vecIKTarget[2])),
	RecvPropVector(RECVINFO(m_vecIKTarget[3])), RecvPropVector(RECVINFO(m_vecIKTarget[4])),
...
//-----------------------------------------------------------------------------
C_Strider::C_Strider() : m_iv_vecHitPos("C_Strider::m_iv_vecHitPos"), m_iv_vecIKTarget("C_Strider::m_iv_vecIKTarget")
{
...
#define FRONT_LENGTH 12
	...
	for(int i = 1; i < FRONT_LENGTH; i++)
	{
		...
		pParticle->m_uchStartSize =
			((random->RandomFloat(6.0f, 8.0f) * (FRONT_LENGTH - (i)) / (FRONT_LENGTH * 0.75f)) * flScale);
...
#define SIDE_LENGTH 8
	...
	pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
													   pSimple->GetPMaterial("effects/strider_muzzle"), vec3_origin);
...
void StriderBlood(const Vector &origin, const Vector &normal, float scale)
{
	...
	for(i = 0; i < 64; i++)
	{
		...
		tParticle->m_vecVelocity =
			offDir * random->RandomFloat(BLOOD_MIN_SPEED * flScale * 2.0f, BLOOD_MAX_SPEED * flScale * 2.0f);
