...
RecvPropFloat(RECVINFO(m_flLifespan)), RecvPropFloat(RECVINFO(m_flRadius)),
...
void C_MortarShell::AddRisingParticles(float flPerc)
{
	...
	while(m_ParticleEvent.NextEvent(flCur))
	{
		...
		sParticle = (SimpleParticle *)m_pEmitter->AddParticle(sizeof(SimpleParticle),
															  m_pEmitter->GetPMaterial("effects/spark"), offset);

		if(sParticle == NULL)
			return;

		sParticle->m_vecVelocity = Vector(Helper_RandomFloat(-4.0f, 4.0f), Helper_RandomFloat(-4.0f, 4.0f),
										  Helper_RandomFloat(32.0f, 256.0f) * Bias(val, 0.25f));
...
{
	float val = RemapValClamped(gpGlobals->curtime, m_flStarttime + m_flLifespan, m_flStarttime + m_flLifespan + 1.0f,
								1.0f, 0.0f);
...
int C_MortarShell::DrawModel(int flags)
{
	...
	FX_AddQuad(GetAbsOrigin() + (m_vecSurfaceNormal * 2.0f), m_vecSurfaceNormal, flScale, flScale, 1.0f, flAlpha,
			   flAlpha, 1.0f, 0, 0, Vector(1.0f, 1.0f, 1.0f), 0.0001f, "effects/combinemuzzle2_nocull", 0);
