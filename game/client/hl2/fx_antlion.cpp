...
public:
	static C_AntlionGib *CreateClientsideGib(const char *pszModelName, Vector vecOrigin, Vector vecForceDir,
											 AngularImpulse vecAngularImp, float m_flLifetime = DEFAULT_GIB_LIFETIME)
	{
	...
	virtual void HitSurface(C_BaseEntity *pOther)
	{
		...
		if (index >= 0 )
		{
			effects->DecalShoot( index, pOther->entindex(), pOther->GetModel(), pOther->GetAbsOrigin(),
		pOther->GetAbsAngles(), GetAbsOrigin(), 0, 0 );
		}
		*/
	}
};
...
void FX_AntlionGib(const Vector &origin, const Vector &direction, float scale)
{
	...
	for(int i = 0; i < NUM_ANTLION_GIBS; i++)
	{
		...
		C_AntlionGib::CreateClientsideGib(pszAntlionGibs[i], offset,
										  (direction + RandomVector(-0.8f, 0.8f)) * (250 * scale),
										  RandomAngularImpulse(-32, 32), 1.0f);
	}
	...
	for(int i = 0; i < numGibs; i++)
	{
		...
		C_AntlionGib::CreateClientsideGib(pszAntlionGibs_Unique[i], offset,
										  (direction + RandomVector(-0.8f, 0.8f)) * (150 * scale),
										  RandomAngularImpulse(-32, 32), 2.0f);
	}
	...
	for(int i = 0; i < numGibs; i++)
	{
		...
		C_AntlionGib::CreateClientsideGib(pszAntlionGibs_Medium[i], offset,
										  (direction + RandomVector(-0.8f, 0.8f)) * (250 * scale),
										  RandomAngularImpulse(-200, 200), 1.0f);
	}
	...
	for(int i = 0; i < NUM_ANTLION_GIBS_SMALL; i++)
	{
		...
		C_AntlionGib::CreateClientsideGib(pszAntlionGibs_Small[i], offset,
										  (direction + RandomVector(-0.8f, 0.8f)) * (400 * scale),
										  RandomAngularImpulse(-300, 300), 0.5f);
	}
	...
	for(int i = 0; i < 4; i++)
	{
		SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), g_Material_Blood[0],
																		   origin + RandomVector(-16, 16));
		if(sParticle == NULL)
			return;
	...
	SimpleParticle *sParticle =
		(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), g_Material_Blood[1], origin);
	if(sParticle == NULL)
		return;
	...
	{
		SimpleParticle *sParticle =
			(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), g_Mat_BloodPuff[0], origin);
	...
	{
		SimpleParticle *sParticle =
			(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), g_Mat_BloodPuff[1], origin);
