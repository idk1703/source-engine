...
ConVar hud_showemptyweaponslots("hud_showemptyweaponslots", "1", FCVAR_ARCHIVE,
...
private:
	...
	void DrawLargeWeaponBox(C_BaseCombatWeapon *pWeapon, bool bSelected, int x, int y, int wide, int tall, Color color,
							float alpha, int number);
...
bool CHudWeaponSelection::ShouldDraw()
{
	...
	if(hud_fastswitch.GetBool() &&
	   (gpGlobals->curtime - m_flSelectionTime) < (FASTSWITCH_DISPLAY_TIMEOUT + FASTSWITCH_FADEOUT_TIME))
		return true;
...
void CHudWeaponSelection::ActivateFastswitchWeaponDisplay(C_BaseCombatWeapon *pSelectedWeapon)
{
	...
	g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(this, "WeaponBoxOffset", flStop, 0, flTime,
																		 AnimationController::INTERPOLATOR_LINEAR);

	// start the highlight after the scroll completes
	m_flBlur = 7.f;
	g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(this, "Blur", 0, flTime, 0.75f,
			...
			else if(m_WeaponBoxes.Count() < MAX_CAROUSEL_SLOTS)
			{
				...
				for(int i = 0; i < m_WeaponBoxes.Count(); i++)
				{
					C_BaseCombatWeapon *pWeapon =
						GetWeaponInSlot(m_WeaponBoxes[i].m_iSlot, m_WeaponBoxes[i].m_iSlotPos);
					if(!pWeapon)
						break;
					...
					if(i == m_iSelectedWeaponBox)
					{
						...
						DrawLargeWeaponBox(pWeapon, true, xpos, ypos, m_flLargeBoxWide, m_flLargeBoxTall, selectedColor,
										   alpha, -1);
					}
					else
					{
						DrawLargeWeaponBox(pWeapon, false, xpos, ypos, m_flLargeBoxWide, m_flLargeBoxTall / 1.5f,
										   m_BoxColor, alpha, -1);
					}
			...
			else
			{
				...
				while(1)
				{
					...
					if(i == m_iSelectedWeaponBox && !m_flHorizWeaponSelectOffsetPoint)
					{
						...
						DrawLargeWeaponBox(pWeapon, true, xpos, ypos, largeBoxWide, largeBoxTall, selectedColor, alpha,
										   -1);
					}
					else
					{
						alpha = GetWeaponBoxAlpha(false);
						DrawLargeWeaponBox(pWeapon, false, xpos, ypos, largeBoxWide, largeBoxTall / 1.5f, m_BoxColor,
										   alpha, -1);
					}
			...
			for(int i = 0; i < MAX_WEAPON_SLOTS; ++i)
			{
				...
				for(int slotPos = 0; slotPos <= lastSlotPos; ++slotPos)
				{
					...
					DrawLargeWeaponBox(pWeapon, selectedWeapon, x, y, boxWide, boxTall,
									   selectedWeapon ? selectedColor : m_BoxColor, GetWeaponBoxAlpha(selectedWeapon),
									   -1);
				}
			}
		}
					...
					for(int slotpos = 0; slotpos <= iLastPos; slotpos++)
					{
						...
						if(!pWeapon)
						{
							if(!hud_showemptyweaponslots.GetBool())
								continue;
							DrawBox(xpos, ypos, largeBoxWide, largeBoxTall, m_EmptyBoxColor, m_flAlphaOverride,
									bDrawBucketNumber ? i + 1 : -1);
						}
						else
						{
							bool bSelected = (pWeapon == pSelectedWeapon);
							DrawLargeWeaponBox(pWeapon, bSelected, xpos, ypos, largeBoxWide, largeBoxTall,
											   bSelected ? selectedColor : m_BoxColor, GetWeaponBoxAlpha(bSelected),
											   bDrawBucketNumber ? i + 1 : -1);
						}
...
//-----------------------------------------------------------------------------
void CHudWeaponSelection::DrawLargeWeaponBox(C_BaseCombatWeapon *pWeapon, bool bSelected, int xpos, int ypos,
											 int boxWide, int boxTall, Color selectedColor, float alpha, int number)
{
		...
		if(CanBeSelectedInHUD(pWeapon))
		{
			...
			if(weaponSlot > iCurrentSlot || (weaponSlot == iCurrentSlot && weaponPosition > iCurrentPosition))
			{
				...
				if(weaponSlot < iLowestNextSlot ||
				   (weaponSlot == iLowestNextSlot && weaponPosition < iLowestNextPosition))
				{
		...
		if(CanBeSelectedInHUD(pWeapon))
		{
			...
			if(weaponSlot < iCurrentSlot || (weaponSlot == iCurrentSlot && weaponPosition < iCurrentPosition))
			{
				...
				if(weaponSlot > iLowestPrevSlot ||
				   (weaponSlot == iLowestPrevSlot && weaponPosition > iLowestPrevPosition))
				{
