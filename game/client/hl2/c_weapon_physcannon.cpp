...
RecvPropBool(RECVINFO(m_bIsCurrentlyUpgrading)), RecvPropFloat(RECVINFO(m_flTimeForceView)),
END_RECV_TABLE
()
...
//-----------------------------------------------------------------------------
void ComputeRenderInfo(mstudiobbox_t *pHitBox, const matrix3x4_t &hitboxToWorld, Vector *pVecAbsOrigin, Vector *pXVec,
					   Vector *pYVec)
{
...
int C_WeaponPhysCannon::DrawModel(int flags)
{
			...
			for(i = 0; i < set->numhitboxes; ++i)
			{
				...
				for(int j = 0; j < numParticles; j++)
				{
					...
					sParticle = (SimpleParticle *)m_pEmitter->AddParticle(
						sizeof(SimpleParticle), m_pEmitter->GetPMaterial("effects/combinemuzzle1"),
						vecAbsOrigin + offset);
	...
	for(int i = 0; i < 4; i++)
	{
		sParticle = (SimpleParticle *)m_pLocalEmitter->AddParticle(
			sizeof(SimpleParticle), m_pLocalEmitter->GetPMaterial("effects/strider_muzzle"), vec3_origin);
	...
	if(m_bWasUpgraded && m_bIsCurrentlyUpgrading)
	{
		...
		for(int i = 0; i < 4; i++)
		{
			...
			sParticle = (SimpleParticle *)m_pAttractor->AddParticle(
				sizeof(SimpleParticle), m_pAttractor->GetPMaterial("effects/strider_muzzle"), offset);
