...
()
	...
	static int IntersectSegmentWithSurfInfos(const Vector &vStart, const Vector &vEnd, SurfInfo *pSurfInfos,
											 ...
											 Vector *pIntersectionPositions, int nMaxIntersections)
{
	...
	for(int i = 0; i < nSurfInfos; i++)
	{
		...
		if((dot1 > 0) != (dot2 > 0))
		{
			...
			{
				VPlane edgePlane;
				edgePlane.m_Normal = pSurf->m_Plane.m_Normal.Cross(pSurf->m_Verts[iEdge] -
...
#define NUM_DUSTEMITTER_SURFINFOS 128
...
#else
	{
		...
		if(ent)
		{
			nSurfInfos = engine->GetIntersectingSurfaces(ent->GetModel(), vCenter, AR2_DUST_RADIUS, true, surfInfos,
...
#define MAX_SURFINFO_INTERSECTIONS 4
				...
				int nIntersections;
				nIntersections = IntersectSegmentWithSurfInfos(startPos, endPos, surfInfos, nSurfInfos, pIntersected,
				...
				if(nIntersections)
				{
					...
					Vector velocity;
					// velocity.Init(-1.0f + ((float)rand()/VALVE_RAND_MAX) * 2.0f, -1.0f +
					// ((float)rand()/VALVE_RAND_MAX) * 2.0f, -1.0f + ((float)rand()/VALVE_RAND_MAX) * 2.0f); velocity =
					// velocity * FRand(m_MinSpeed, m_MaxSpeed);
					...
					Vector reflection =
						direction - 2 * DotProduct(direction, pIntersected[iIntersection]->m_Plane.m_Normal) *
					...
					*/
#if 1
					AR2ExplosionParticle *pParticle = (AR2ExplosionParticle *)m_ParticleEffect.AddParticle(
...
void C_AR2Explosion::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		if(pParticle->m_Lifetime >= 0.0f)
		{
			...
			RenderParticle_ColorSizeAngle(pIterator->GetParticleDraw(), tPos, color, alpha,
