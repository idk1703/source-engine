...
void FX_PlayerAR2Tracer(const Vector &start, const Vector &end)
{
	...
	// Create the line
	CFXStaticLine *tracerLine =
		new CFXStaticLine("Tracer", dStart, dEnd, random->RandomFloat(6.0f, 12.0f), 0.01f, "effects/gunshiptracer", 0);
	assert(tracerLine);
...
void FX_AR2Tracer(Vector &start, Vector &end, int velocity, bool makeWhiz)
{
	...
	// Add it
	FX_AddDiscreetLine(start, dir, velocity, length, dist, random->RandomFloat(0.5f, 1.5f), life,
...
void AR2ExplosionCallback(const CEffectData &data)
{
	...
	// Ground splash
	FX_AddQuad(data.m_vOrigin, data.m_vNormal, data.m_flRadius, data.m_flRadius * 4.0f, 0.85f, 1.0f, 0.0f, 0.25f,
			   random->RandomInt(0, 360), random->RandomFloat(-4, 4), Vector(1.0f, 1.0f, 1.0f), lifetime,
			   "effects/combinemuzzle1", (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));
...
{
	FX_AddQuad(data.m_vOrigin, data.m_vNormal, random->RandomFloat(24, 32), 0, 0.75f, 1.0f, 0.0f, 0.4f,
			   random->RandomInt(0, 360), 0, Vector(1.0f, 1.0f, 1.0f), 0.25f, "effects/combinemuzzle2_nocull",
...
//-----------------------------------------------------------------------------
void CreateMuzzleflashELight(const Vector &origin, int exponent, int nMinRadius, int nMaxRadius,
...
void MuzzleFlash_Airboat(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	for(int i = 1; i < 7; i++)
	{
		...
		pParticle =
			(SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle), pMuzzle[random->RandomInt(0, 1)], offset);
	...
	pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
													   pSimple->GetPMaterial("sprites/ar2_muzzle1"), vec3_origin);
...
void MuzzleFlash_Chopper(ClientEntityHandle_t hEntity, int attachmentIndex)
{
	...
	for(int i = 1; i < 7; i++)
	{
		...
		pParticle = (SimpleParticle *)pSimple->AddParticle(
			sizeof(SimpleParticle), pSimple->GetPMaterial(VarArgs("effects/combinemuzzle%d", random->RandomInt(1, 2))),
			offset);
