...
RecvPropInt(RECVINFO(m_bOn)), RecvPropFloat(RECVINFO(m_flDensity)),
END_RECV_TABLE
()
...
void C_EnvStarfield::OnDataChanged(DataUpdateType_t updateType)
{
	if(updateType == DATA_UPDATE_CREATED)
	{
		...
		m_pEmitter->Setup((Vector &)vecCenter, NULL, 0.0, 0, 64, 0, 0,
...
void C_EnvStarfield::ClientThink(void)
{
	...
	for(int i = 0; i < iNumParticles; i++)
	{
		...
		Vector vecEnd = vecViewOrigin + (MainViewRight() * RandomFloat(-flDiameter, flDiameter)) +
		...
		TrailParticle *pParticle =
			(TrailParticle *)m_pEmitter->AddParticle(sizeof(TrailParticle), hParticleMaterial, vecStart);
		if(pParticle)
		{
