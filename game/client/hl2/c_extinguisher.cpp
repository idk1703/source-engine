...
RecvPropInt(RECVINFO(m_bEmit), 0), RecvPropInt(RECVINFO(m_bUseMuzzlePoint), 0), RecvPropInt(RECVINFO(m_nLength), 0),
...
void C_ExtinguisherJet::Update(float fTimeDelta)
{
	...
	if(m_pEmitter.IsValid())
	{
		...
		{
			pParticle =
				(SimpleParticle *)m_pEmitter->AddParticle(sizeof(SimpleParticle), m_MaterialHandle, GetAbsOrigin());
			...
			if(hitWall)
			{
				...
				pParticle = (SimpleParticle *)m_pEmitter->AddParticle(sizeof(SimpleParticle), m_MaterialHandle,
																	  (tr.endpos + (tr.plane.normal * 8.0f)) + offDir);
			...
			if(random->RandomInt(0, 1) == 0)
			{
				...
				pParticle = (SimpleParticle *)m_pEmberEmitter->AddParticle(sizeof(SimpleParticle), g_Mat_DustPuff[0],
																		   GetAbsOrigin());
