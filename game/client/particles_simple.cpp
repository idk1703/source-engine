...
SimpleParticle *CSimpleEmitter::AddSimpleParticle(PMaterialHandle hMaterial, const Vector &vOrigin, float flDieTime,
...
{
	return (float)pParticle->m_uchStartSize + ((float)pParticle->m_uchEndSize - (float)pParticle->m_uchStartSize) *
												  (pParticle->m_flLifetime / pParticle->m_flDieTime);
}
...
void CSimpleEmitter::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		// Render it
		RenderParticle_ColorSizeAngle(pIterator->GetParticleDraw(), tPos, UpdateColor(pParticle),
									  UpdateAlpha(pParticle) *
										  GetAlphaDistanceFade(tPos, m_flNearClipMin, m_flNearClipMax),
									  UpdateScale(pParticle), pParticle->m_flRoll);
