...
CHudBaseAccount::CHudBaseAccount(const char *pName) : CHudNumericDisplay(NULL, pName), CHudElement(pName)
{
...
void CHudBaseAccount::Paint()
{
	...
	if(m_iPreviousAccount != account)
	{
		...
		m_flLastAnimationEnd =
			gpGlobals->curtime + GetAnimationController()->GetAnimationSequenceLength(m_pszLastAnimationName);
