...
RecvPropInt(RECVINFO(m_Color)), RecvPropInt(RECVINFO(m_SpawnRate)), RecvPropFloat(RECVINFO(m_flSizeMin)),
	RecvPropFloat(RECVINFO(m_flSizeMax)), RecvPropInt(RECVINFO(m_LifetimeMin)), RecvPropInt(RECVINFO(m_LifetimeMax)),
	RecvPropInt(RECVINFO(m_DustFlags)), RecvPropInt(RECVINFO(m_SpeedMax)), RecvPropInt(RECVINFO(m_DistMax)),
	RecvPropInt(RECVINFO(m_nModelIndex)), RecvPropFloat(RECVINFO(m_FallSpeed)),
	...
	void CDustEffect::RenderParticles(CParticleRenderIterator *pIterator)
{
	...
	while(pParticle)
	{
		...
		if(-tPos.z <= m_pDust->m_DistMax)
		{
			...
			RenderParticle_Color255Size(pIterator->GetParticleDraw(), tPos,
										Vector(m_pDust->m_Color.r, m_pDust->m_Color.g, m_pDust->m_Color.b),
										flAlpha * m_pDust->m_Color.a, flSize);
		}
		...
		if(contents & CONTENTS_SOLID)
		{
			...
			if(pParticle)
			{
				...
				if(m_DustFlags & DUSTFLAGS_SCALEMOTES)
					pParticle->m_flSize =
						RemapVal(rand(), 0, VALVE_RAND_MAX, m_flSizeMin / 10000.0f, m_flSizeMax / 10000.0f);
				else
...
RecvPropFloat(RECVINFO(m_flSize)), RecvPropFloat(RECVINFO(m_flSpeed)), RecvPropVector(RECVINFO(m_vecDirection)),
END_RECV_TABLE
()
