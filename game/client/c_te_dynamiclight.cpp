...
RecvPropVector(RECVINFO(m_vecOrigin)), RecvPropInt(RECVINFO(r)), RecvPropInt(RECVINFO(g)), RecvPropInt(RECVINFO(b)),
	RecvPropInt(RECVINFO(exponent)), RecvPropFloat(RECVINFO(m_fRadius)), RecvPropFloat(RECVINFO(m_fTime)),
...
void TE_DynamicLight(IRecipientFilter &filter, float delay, const Vector *org, int r, int g, int b, int exponent,
					 float radius, float time, float decay, int nLightIndex)
{
...
void TE_DynamicLight(IRecipientFilter &filter, float delay, KeyValues *pKeyValues)
{
	...
	TE_DynamicLight(filter, 0.0f, &vecOrigin, c.r(), c.g(), c.b(), nExponent, flRadius, flDuration, flDecay,
					nLightIndex);
}