...
static const char *s_NetworkMessageNames[MAX_NETMESSAGE] = {NETWORK_MESSAGE1, NETWORK_MESSAGE2, NETWORK_MESSAGE3,
...
// Purpose:
//-----------------------------------------------------------------------------
CHudMessage::CHudMessage(const char *pElementName) : CHudElement(pElementName), BaseClass(NULL, "HudMessage")
{
...
void CHudMessage::MessageDrawScan(client_textmessage_t *pMessage, float time)
{
	...
	if(pMessage->bRoundedRectBackdropBox)
	{
		...
		Color boxColor(pMessage->boxcolor[0], pMessage->boxcolor[1], pMessage->boxcolor[2],
					   pMessage->boxcolor[3] * flAlphaScale);

		DrawBox(boxx, boxy, m_parms.totalWidth + 2.0f * flBoxPixels, m_parms.totalHeight + 2.0f * flBoxPixels * 0.5f,
				boxColor, 1.0f);
	}
...
{
	return (CHudElement::ShouldDraw() && (m_bHaveMessage || m_Messages.Count()));
}
...
void CHudMessage::Paint()
{
	...
	if(m_gameTitleTime > 0)
	{
		...
		else
		{
			...
			m_iconTitleHalf->DrawSelf(x, y,
									  Color(m_pGameTitle->r1, m_pGameTitle->g1, m_pGameTitle->b1, brightness * 255));
			m_iconTitleLife->DrawSelf(x + halfWidth, y,
									  Color(m_pGameTitle->r1, m_pGameTitle->g1, m_pGameTitle->b1, brightness * 255));
			drawn = 1;
		}
	}
	...
	for(i = 0; i < maxHUDMessages; i++)
	{
		...
		if(m_pMessages[i])
		{
			...
			if(m_startTime[i] > gpGlobals->curtime)
				m_startTime[i] = gpGlobals->curtime + m_parms.time - m_startTime[i] +
