...
static ConVar cl_extrapolate("cl_extrapolate", "1", FCVAR_CHEAT,
...
static ConVar cl_interp_all("cl_interp_all", "0", 0, "Disable interpolation list optimizations.", 0, 0, 0, 0,
							cc_cl_interp_all_changed);
...
BEGIN_RECV_TABLE_NOBASE(C_BaseEntity, DT_PredictableId)
	RecvPropPredictableId(RECVINFO(m_PredictableID)), RecvPropInt(RECVINFO(m_bIsPlayerSimulated)),
END_RECV_TABLE()
#endif
...
BEGIN_RECV_TABLE_NOBASE(C_BaseEntity, DT_BaseEntity)
	RecvPropDataTable("AnimTimeMustBeFirst", 0, 0, &REFERENCE_RECV_TABLE(DT_AnimTimeMustBeFirst)),
		...
		RecvPropInt(RECVINFO(m_fEffects), 0, RecvProxy_EffectFlags), RecvPropInt(RECVINFO(m_nRenderMode)),
		RecvPropInt(RECVINFO(m_nRenderFX)), RecvPropInt(RECVINFO(m_clrRender)), RecvPropInt(RECVINFO(m_iTeamNum)),
		RecvPropInt(RECVINFO(m_CollisionGroup)), RecvPropFloat(RECVINFO(m_flElasticity)),
		RecvPropFloat(RECVINFO(m_flShadowCastDistance)), RecvPropEHandle(RECVINFO(m_hOwnerEntity)),
...
BEGIN_PREDICTION_DATA_NO_BASE(C_BaseEntity)
	...
	DEFINE_PRED_TYPEDESCRIPTION(m_Collision, CCollisionProperty),
		...
		DEFINE_FIELD(m_vecAbsOrigin, FIELD_VECTOR), DEFINE_FIELD(m_angAbsRotation, FIELD_VECTOR),
		DEFINE_FIELD(m_vecOrigin, FIELD_VECTOR), DEFINE_FIELD(m_angRotation, FIELD_VECTOR),

		//	DEFINE_FIELD( m_hGroundEntity, FIELD_EHANDLE ),
		DEFINE_FIELD(m_nWaterLevel, FIELD_CHARACTER), DEFINE_FIELD(m_nWaterType, FIELD_CHARACTER),
		...
		DEFINE_FIELD(m_vecBaseVelocity, FIELD_VECTOR), DEFINE_FIELD(m_iEFlags, FIELD_INTEGER),
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				//				Assert(0);
				IRotateAABB(EntityToWorldTransform(), CollisionProp()->OBBMins(), CollisionProp()->OBBMaxs(), theMins,
							theMaxs);
			}
		}
	}
...
int C_BaseEntity::DrawModel(int flags)
{
	...
	{
		case mod_brush:
			drawn = DrawBrushModel(flags & STUDIO_TRANSPARENCY ? true : false, flags,
...
//-----------------------------------------------------------------------------
void C_BaseEntity::SetupWeights(const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights,
...
void C_BaseEntity::UnlinkFromHierarchy()
{
	...
	// Adrian: This was causing problems with the local network backdoor with entities coming in and out of the PVS at
	// certain times. This would work fine if a full entity update was coming (caused by certain factors like too many
	// entities entering the pvs at once). but otherwise it would not detect the change on the client (since the server
	// and client shouldn't be out of sync) and the var would not be updated like it should. m_iParentAttachment = 0;
...
void C_BaseEntity::CheckInitPredictable(const char *context)
{
	...
	{
		if(m_PredictableID.IsActive() && (player->index - 1) == m_PredictableID.GetPlayer())
		{
...
void C_BaseEntity::OnStoreLastNetworkedValue()
{
	...
	// Kind of a hack, but we want to latch the actual networked value for origin/angles, not what's sitting in
	// m_vecOrigin in the
...
int CBaseEntity::BaseInterpolatePart1(float &currentTime, Vector &oldOrigin, QAngle &oldAngles, Vector &oldVel,
									  int &bNoMoreChanges)
{
...
{
	if(model && modelinfo->GetModelType(model) == mod_brush && modelinfo->GetModelName(model)[0] == '*')
	{
		return true;
	}
...
//-----------------------------------------------------------------------------
void C_BaseEntity::AddStudioDecal(const Ray_t &ray, int hitbox, int decalIndex, bool doTrace, trace_t &tr,
								  int maxLODToDecal)
{
	if(doTrace)
	{
...
//-----------------------------------------------------------------------------
void C_BaseEntity::AddColoredStudioDecal(const Ray_t &ray, int hitbox, int decalIndex, bool doTrace, trace_t &tr,
										 Color cColor, int maxLODToDecal)
{
	...
	if(doTrace && (GetSolid() == SOLID_VPHYSICS) && !tr.startsolid && !tr.allsolid)
	{
		...
		Ray_t betterRay;
		betterRay.Init(tr.endpos, temp);
		modelrender->AddColoredDecal(m_ModelInstance, betterRay, up, decalIndex, GetStudioBody(), cColor, true,
									 maxLODToDecal);
	}
	else
	{
		modelrender->AddColoredDecal(m_ModelInstance, ray, up, decalIndex, GetStudioBody(), cColor, false,
									 maxLODToDecal);
	}
}
...
//-----------------------------------------------------------------------------
void C_BaseEntity::AddBrushModelDecal(const Ray_t &ray, const Vector &decalCenter, int decalIndex, bool doTrace,
									  trace_t &tr)
{
	if(doTrace)
	{
...
//-----------------------------------------------------------------------------
void C_BaseEntity::AddDecal(const Vector &rayStart, const Vector &rayEnd, const Vector &decalCenter, int hitbox,
...
//-----------------------------------------------------------------------------
void C_BaseEntity::AddColoredDecal(const Vector &rayStart, const Vector &rayEnd, const Vector &decalCenter, int hitbox,
								   int decalIndex, bool doTrace, trace_t &tr, Color cColor, int maxLODToDecal)
{
	...
	switch(modelType)
	{
		...
		case mod_brush:
		{
			...
			effects->DecalColorShoot(decalIndex, index, model, GetAbsOrigin(), GetAbsAngles(), decalCenter, 0, 0,
									 cColor32);
		}
		break;
...
void C_BaseEntity::BoneMergeFastCullBloat(Vector &localMins, Vector &localMaxs, const Vector &thisEntityMins,
...
void C_BaseEntity::CalcAbsolutePosition()
{
	...
	if((m_iEFlags & EFL_DIRTY_ABSTRANSFORM) ==
	   0) // need second check in event another thread grabbed mutex and did the calculation
	{
		return;
	}
	...
	// (ie: this entity) to call GetAbsOrigin(), and they'll use the moveparent's OLD bone transforms to get their
	// attachments since the moveparent is right in the middle of setting up new transforms.
	//
	// So here, we keep our absorigin invalidated. It means we're returning an origin that is a frame old to
	// CalculateIKLocks, but we'll still render with the right origin.
...
bool C_BaseEntity::PostNetworkDataReceived(int commands_acknowledged)
{
	...
	if(errorcheck)
	{
		...
		CPredictionCopy errorCheckHelper(PC_NETWORKED_ONLY, predicted_state_data, PC_DATA_PACKED, original_state_data,
										 PC_DATA_PACKED, counterrors, reporterrors, copydata);
...
// Output : C_BaseEntity
//-----------------------------------------------------------------------------
C_BaseEntity *C_BaseEntity::CreatePredictedEntityByName(const char *classname, const char *module, int line,
														bool persist /*= false */)
{
#if !defined(NO_ENTITY_PREDICTION)
...
static int g_FieldSizes[FIELD_TYPECOUNT] = {
	...
	sizeof(matrix3x4_t), // FIELD_MATRIX3X4_WORLDSPACE	// NOTE: Use array(FIELD_FLOAT, 12) for matrix3x4_t NOT in
						 // worldspace
...
CBaseEntity *FindEntityInFrontOfLocalPlayer()
{
	...
	if(pPlayer)
	{
		...
		UTIL_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_COORD_RANGE, MASK_SOLID, pPlayer,
...
static ConCommand cl_removedecals("cl_removedecals", RemoveDecals_f,
...
void C_BaseEntity::DrawBBoxVisualizations(void)
{
	if(m_fBBoxVisFlags & VISUALIZE_COLLISION_BOUNDS)
	{
		if(debugoverlay)
		{
			debugoverlay->AddBoxOverlay(CollisionProp()->GetCollisionOrigin(), CollisionProp()->OBBMins(),
										CollisionProp()->OBBMaxs(), CollisionProp()->GetCollisionAngles(), 190, 190, 0,
										0, 0.01);
		}
	}
	...
	if(m_fBBoxVisFlags & VISUALIZE_SURROUNDING_BOUNDS)
	{
		...
		if(debugoverlay)
		{
			debugoverlay->AddBoxOverlay(vec3_origin, vecSurroundMins, vecSurroundMaxs, vec3_angle, 0, 255, 255, 0,
										0.01);
		}
	}
	...
	if(m_fBBoxVisFlags & VISUALIZE_RENDER_BOUNDS || r_drawrenderboxes.GetInt())
	{
		...
		debugoverlay->AddBoxOverlay(GetRenderOrigin(), vecRenderMins, vecRenderMaxs, GetRenderAngles(), 255, 0, 255, 0,
									0.01);
	}
}
...
int C_BaseEntity::RestoreData(const char *context, int slot, int type)
{
	...
	// In that case, we'd be comparing "old" data from this "unused" slot with the networked data and reporting all
	// kinds of prediction errors possibly.
...
BEGIN_DATADESC_NO_BASE(C_BaseEntity)
	DEFINE_FIELD(m_ModelName, FIELD_STRING), DEFINE_FIELD(m_vecAbsOrigin, FIELD_POSITION_VECTOR),
...
//-----------------------------------------------------------------------------
// Purpose: For each client (only can be local client in client .dll ) checks the client has disabled CC and if so,
// removes them from
//  the recipient list.
// Input  : filter -
//-----------------------------------------------------------------------------
...
void C_BaseEntity::CheckCLInterpChanged()
{
	...
	if(flLastValue_Interp != flCurValue_Interp || flLastValue_InterpNPCs != flCurValue_InterpNPCs)
	{
...
static ConCommand cl_find_ent("cl_find_ent", CC_CL_Find_Ent,
							  "Find and list all client entities with classnames that contain the specified "
