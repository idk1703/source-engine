...
private:
	...
	{
		return WorldAlignMins() + Vector(x * m_SpacingRadius * 2 + m_SpacingRadius,
...
RecvPropInt(RECVINFO(m_Color1), 0, RecvProxy_IntToColor32), RecvPropInt(RECVINFO(m_Color2), 0, RecvProxy_IntToColor32),
	RecvPropString(RECVINFO(m_MaterialName)), RecvPropFloat(RECVINFO(m_ParticleDrawWidth)),
	RecvPropFloat(RECVINFO(m_ParticleSpacingDistance)), RecvPropFloat(RECVINFO(m_DensityRampSpeed)),
	RecvPropFloat(RECVINFO(m_RotationSpeed)), RecvPropFloat(RECVINFO(m_MovementSpeed)),
	RecvPropFloat(RECVINFO(m_Density)), RecvPropInt(RECVINFO(m_spawnflags)),
...
void C_FuncSmokeVolume::Update(float fTimeDelta)
{
	...
	if(!VectorsAreEqual(curOrigin, m_vLastOrigin, 0.1) || fabs(curAngles.x - m_vLastAngles.x) > 0.1 ||
	   fabs(curAngles.y - m_vLastAngles.y) > 0.1 || fabs(curAngles.z - m_vLastAngles.z) > 0.1 || m_bFirstUpdate)
	{
					...
					for(int zCount = 0; zCount < 3; zCount++)
					{
						...
						if(IsValidXYZCoords(testX, testY, testZ))
						{
							...
							if(pOther->m_pParticle && pOther->m_TradeIndex == -1)
							{
								...
								pOther->m_TradeDuration = pInfo->m_TradeDuration =
									FRand(tradeDurationMin, tradeDurationMax);
...
void C_FuncSmokeVolume::SimulateParticles(CParticleSimulateIterator *pIterator)
{
	...
	while(pParticle)
	{
		pParticle->m_CurRotation +=
			pParticle->m_RotationFactor * (M_PI / 180.0f) * m_RotationSpeed * pIterator->GetTimeDelta();
				...
				if(SmokeParticleInfo *pInfo = GetSmokeParticleInfo(x, y, z))
				{
					...
					else
					{
						SmokeGrenadeParticle *pParticle = (SmokeGrenadeParticle *)m_ParticleEffect.AddParticle(
							sizeof(SmokeGrenadeParticle), m_MaterialHandle);
