...
float PixelVisibility_DrawProxy(IMatRenderContext *pRenderContext, OcclusionQueryObjectHandle_t queryHandle,
								Vector origin, float scale, float proxyAspect, IMaterial *pMaterial, bool screenspace)
{
	Vector point;
	...
	if(screenspace)
	{
		...
		float pixelsPerUnit2 = pRenderContext->ComputePixelDiameterOfSphere(
			CurrentViewOrigin() + CurrentViewForward() * 1.0f, scale * 0.5f);
	...
	verts[2] = origin + CurrentViewUp() * scale45y +
			   CurrentViewRight() * scale45x; // the pyramid is a sprite with a point that
	verts[3] = origin - CurrentViewUp() * scale45y +
			   CurrentViewRight() * scale45x; // pokes back toward the camera through any nearby
...
public:
	...
	void IssueQuery(IMatRenderContext *pRenderContext, float proxySize, float proxyAspect, IMaterial *pMaterial,
					bool sizeIsScreenSpace);
	void IssueCountingQuery(IMatRenderContext *pRenderContext, float proxySize, float proxyAspect, IMaterial *pMaterial,
							bool sizeIsScreenSpace);
...
private:
	...
	friend void PixelVisibility_ShiftVisibilityViews(
		int iSourceViewID, int iDestViewID); // need direct access to private data to make shifting smooth
};
...
void CPixelVisibilityQuery::IssueQuery(IMatRenderContext *pRenderContext, float proxySize, float proxyAspect,
									   IMaterial *pMaterial, bool sizeIsScreenSpace)
{
	if(!m_failed)
	{
		Assert(IsValid());
...
void CPixelVisibilityQuery::IssueCountingQuery(IMatRenderContext *pRenderContext, float proxySize, float proxyAspect,
											   IMaterial *pMaterial, bool sizeIsScreenSpace)
{
	if(!m_failed)
	{
...
#endif
		PixelVisibility_DrawProxy(pRenderContext, m_queryHandleCount, m_origin, proxySize, proxyAspect, pMaterial,
								  sizeIsScreenSpace);
	}
}
...
public:
	...
	void DebugInfo()
	{
		Msg("Pixel vis system using %d sets total (%d in free list), %d queries total (%d in free list)\n",
			m_setList.TotalCount(), m_setList.Count(m_freeSetsList), m_queryList.TotalCount(),
...
void CPixelVisibilitySystem::EndView()
{
	...
	if(r_pixelvisibility_partial.GetBool() && !m_drawQueries)
	{
		...
		while(node != m_setList.InvalidIndex())
		{
			...
			{
				m_queryList[queryNode].IssueCountingQuery(pRenderContext, pSet->proxySize, pSet->proxyAspect, pProxy,
														  pSet->sizeIsScreenSpace);
			}
...

	{
		...
		while(node != m_setList.InvalidIndex())
		{
			...
			if(queryNode != m_queryList.InvalidIndex())
			{
				m_queryList[queryNode].IssueQuery(pRenderContext, pSet->proxySize, pSet->proxyAspect, pProxy,
												  pSet->sizeIsScreenSpace);
			}
			node = m_setList.Next(node);
		}
...
CPixelVisSet *CPixelVisibilitySystem::FindOrCreatePixelVisSet(const pixelvis_queryparams_t &params,
															  pixelvis_handle_t *queryHandle)
{
	if(queryHandle[0])
	{
...
// this originally called a class function of CPixelVisibiltySystem to keep the work clean, but that function needed
// friend access to CPixelVisibilityQuery and I didn't want to make the whole class a friend or shift all the functions
// and class declarations around in this file
void PixelVisibility_ShiftVisibilityViews(int iSourceViewID, int iDestViewID)
{
	...
	while(node != g_PixelVisibilitySystem.m_setList.InvalidIndex())
	{
		...
		if(iDestQueryNode != g_PixelVisibilitySystem.m_queryList.InvalidIndex())
		{
			...
			if(g_PixelVisibilitySystem.m_queryList.Head(pSet->queryList) ==
			   g_PixelVisibilitySystem.m_queryList.InvalidIndex())
			{
