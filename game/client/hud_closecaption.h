...
{
	AsyncCaption_t() : m_DataBaseFile(UTL_INVAL_SYMBOL), m_RequestedBlocks(0, 0, BlockInfo_t::Less)
	{
	...
	AsyncCaption_t &operator=(const AsyncCaption_t &rhs)
	{
		...
		for(int i = rhs.m_RequestedBlocks.FirstInorder(); i != rhs.m_RequestedBlocks.InvalidIndex();
			i = rhs.m_RequestedBlocks.NextInorder(i))
		{
...
public:
	...
	{
		CaptionRepeat() : m_nTokenIndex(0), m_flLastEmitTime(0), m_flInterval(0), m_nLastEmitTick(0) {}
		int m_nTokenIndex;
		int m_nLastEmitTick;
...
private:
	...
	void _ProcessCaption(const wchar_t *caption, char const *tokenname, float duration, bool fromplayer,
						 bool direct = false);
