...
static ConVar cc_captiontrace("cc_captiontrace", "1", 0,
...
static ConVar cc_smallfontlength("cc_smallfontlength", "300", 0,
...
{
public:
	CCloseCaptionItem(const wchar_t *stream, float timetolive, float addedtime, float predisplay, bool valid,
					  bool fromplayer)
		: m_flTimeToLive(0.0f),
...
//  the handle is a pointer to a AsyncCaptionData_t defined above.  If the resource manager uncaches the handle, we
//  reload the .vcd from disk.  Precaching a .vcd calls into FindOrAddBlock which moves the .vcd to the head of the LRU
//  if it's in memory or it reloads it from disk otherwise.
//-----------------------------------------------------------------------------
class CAsyncCaptionResourceManager : public CAutoGameSystem,
...
void CHudCloseCaption::Paint(void)
{
	...
	if(!visibleitems.Count() && m_nGoalHeight == m_nCurrentHeight && m_flGoalAlpha == m_flCurrentAlpha)
	{
		m_flGoalHeightStartTime = 0;
		m_flGoalHeightFinishTime = 0;
		return;
	...
	// Continue growth?
	if(m_flGoalHeightFinishTime && m_flGoalHeightStartTime && m_flGoalHeightFinishTime > m_flGoalHeightStartTime)
	{
	...
	DrawBox(rcText.left, MAX(rcText.top, 0), rcText.right - rcText.left, rcText.bottom - MAX(rcText.top, 0), bgColor,
			m_flCurrentAlpha);
...
void CHudCloseCaption::Process(const wchar_t *stream, float duration, const char *tokenstream, bool fromplayer,
							   bool direct)
{
	if(!direct)
	{
		...
		if(SplitCommand(&curpos, cmd, args))
		{
			if(!wcscmp(cmd, L"delay"))
			{
				...
				*out = L'\0';
				...
				{
					CCloseCaptionItem *item =
						new CCloseCaptionItem(phrase, lifespan, addedlife, delay, valid, fromplayer);
...
void CHudCloseCaption::AddWorkUnit(CCloseCaptionItem *item, WorkUnitParams &params)
{
...
void CHudCloseCaption::DrawStream(wrect_t &rcText, wrect_t &rcWindow, CCloseCaptionItem *item, int iFadeLine,
...
{
public:
	CAsyncCaption() : m_flDuration(0.0f), m_bIsStream(false), m_bFromPlayer(false) {}
	...
	void OnDataArrived(CUtlVector<AsyncCaption_t> &directories, int nFileIndex, int nBlockNum,
					   AsyncCaptionData_t *pData)
	{
		...
		for(int i = 0; i < c; ++i)
		{
			...
			V_UCS2ToUnicode((ucs2 *)&pData->m_pBlockData[entry.offset], caption->stream,
							entry.length * sizeof(wchar_t));
#endif
		}
	}
...
bool CHudCloseCaption::AddAsyncWork(const char *tokenstream, bool bIsStream, float duration, bool fromplayer,
									bool direct /* = false */)
{
	bool bret = true;
...
void CHudCloseCaption::_ProcessSentenceCaptionStream(int wordCount, const char *tokenstream,
													 const wchar_t *caption_full)
{
...
bool CHudCloseCaption::ProcessCaption(const char *tokenname, float duration, bool fromplayer /* = false */,
									  bool direct /* = false */)
{
	return AddAsyncWork(tokenname, false, duration, fromplayer, direct);
}

void CHudCloseCaption::_ProcessCaption(const wchar_t *caption, const char *tokenname, float duration, bool fromplayer,
									   bool direct)
{
...
void CHudCloseCaption::MsgFunc_CloseCaption(bf_read &msg)
{
	...
	if(bIsMale || bIsFemale)
	{
		...
		// If the gender-ified version exists, use it, otherwise fall through and pass the non-gender string to the cc
...
void CHudCloseCaption::InitCaptionDictionary(const char *dbfile)
{
	...
	for(char *path = strtok(searchPaths, ";"); path; path = strtok(NULL, ";"))
	{
		...
		if(FILESYSTEM_INVALID_HANDLE != fh)
		{
			...
			entry.m_CaptionDirectory.CopyArray((const CaptionLookup_t *)dirbuffer.PeekGet(),
...
static int EmitCaptionCompletion(const char *partial,
								 char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH])
{
	...
	while(i != INVALID_LOCALIZE_STRING_INDEX && current < COMMAND_COMPLETION_MAXITEMS)
	{
...
ConVar cc_lang("cc_lang", "", FCVAR_ARCHIVE, "Current close caption language (emtpy = use game UI language)",
	...
	for(int i = 0; i < m_AsyncCaptions.Count(); ++i)
	{
		...
		for(int j = 0; j < dict.Count(); ++j)
		{
			...
			if(Q_stristr(streamANSI, pchANSI))
			{
				...
				for(int k = soundemitterbase->First(); k != soundemitterbase->InvalidIndex();
				...
				if(IsPC())
				{
					for(int r = g_pVGuiLocalize->GetFirstStringIndex(); r != INVALID_LOCALIZE_STRING_INDEX;
