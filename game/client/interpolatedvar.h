...
inline Vector ExtrapolateInterpolatedVarType(const Vector &oldVal, const Vector &newVal, float divisor,
											 float flExtrapolationAmount)
{
	return Lerp(1.0f + flExtrapolationAmount * divisor, oldVal, newVal);
}

inline float ExtrapolateInterpolatedVarType(const float &oldVal, const float &newVal, float divisor,
											float flExtrapolationAmount)
{
	return Lerp(1.0f + flExtrapolationAmount * divisor, oldVal, newVal);
}

inline QAngle ExtrapolateInterpolatedVarType(const QAngle &oldVal, const QAngle &newVal, float divisor,
											 float flExtrapolationAmount)
{
...
public:
	...
	void GetDerivative_SmoothVelocity(Type *pOut,
									  float currentTime); // See notes on ::Derivative_HermiteLinearVelocity for info.
...
protected:
	...
	bool GetInterpolationInfo(CInterpolationInfo *pInfo, float currentTime, float interpolation_amount,
							  int *pNoMoreChanges);

	void TimeFixup_Hermite(CInterpolatedVarEntry &fixup, CInterpolatedVarEntry *&prev, CInterpolatedVarEntry *&start,
						   CInterpolatedVarEntry *&end);

	// Force the time between prev and start to be dt (and extend prev out farther if necessary).
	void TimeFixup2_Hermite(CInterpolatedVarEntry &fixup, CInterpolatedVarEntry *&prev, CInterpolatedVarEntry *&start,
							float dt);

	void _Extrapolate(Type *pOut, CInterpolatedVarEntry *pOld, CInterpolatedVarEntry *pNew, float flDestinationTime,
	...
	void _Interpolate_Hermite(Type *out, float frac, CInterpolatedVarEntry *pOriginalPrev, CInterpolatedVarEntry *start,
							  CInterpolatedVarEntry *end, bool looping = false);

	void _Derivative_Hermite(Type *out, float frac, CInterpolatedVarEntry *pOriginalPrev, CInterpolatedVarEntry *start,
							 CInterpolatedVarEntry *end);
	void _Derivative_Hermite_SmoothVelocity(Type *out, float frac, CInterpolatedVarEntry *b, CInterpolatedVarEntry *c,
...
template<typename Type, bool IS_ARRAY>
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::NoteChanged(float changetime, float interpolation_amount,
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetInterpolationInfo(
	typename CInterpolatedVarArrayBase<Type, IS_ARRAY>::CInterpolationInfo *pInfo, float currentTime,
...
template<typename Type, bool IS_ARRAY>
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetInterpolationInfo(float currentTime, int *pNewer, int *pOlder,
																			int *pOldest)
{
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::DebugInterpolate(Type *pOut, float currentTime)
{
	...
	else if(info.newer == info.older)
	{
		...
		if(CInterpolationContext::IsExtrapolationAllowed() && IsValidIndex(realOlder) &&
		   history[realOlder].changetime != 0.0 && interpolation_amount > 0.000001f &&
		   CInterpolationContext::GetLastTimeStamp() <= m_LastNetworkedTime)
		{
			...
			_Extrapolate(pOut, &history[realOlder], &history[info.newer], currentTime - interpolation_amount,
...
inline int CInterpolatedVarArrayBase<Type, IS_ARRAY>::Interpolate(float currentTime, float interpolation_amount)
{
	...
	if(m_bDebug)
	{
		// "value will hold" means we are either extrapolating, or the samples in GetInterpolationInfo are all the
		// same... In either case there are no more "changes" until we latch a new
	...
	else if(info.newer == info.older)
	{
		   ...
		   CInterpolationContext::GetLastTimeStamp() <= m_LastNetworkedTime)
		{
			...
			// Use the velocity here (extrapolate up to 1/4 of a second).
			_Extrapolate(m_pValue, &history[realOlder], &history[info.newer], currentTime - interpolation_amount,
						 cl_extrapolate_amount.GetFloat());
		}
		else
		{
...
void CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetDerivative(Type *pOut, float currentTime)
{
	...
	if(info.m_bHermite)
	{
		_Derivative_Hermite(pOut, info.frac, &m_VarHistory[info.oldest], &m_VarHistory[info.older],
...
void CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetDerivative_SmoothVelocity(Type *pOut, float currentTime)
{
	...
	if(info.m_bHermite)
	{
		_Derivative_Hermite_SmoothVelocity(pOut, info.frac, &history[info.oldest], &history[info.older],
										   &history[info.newer]);
		return;
	}
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::Copy(IInterpolatedVar *pInSrc)
{
	...
	if(!pSrc || pSrc->m_nMaxCount != m_nMaxCount)
	{
		if(pSrc)
		{
			AssertMsg3(false, "pSrc->m_nMaxCount (%i) != m_nMaxCount (%i) for %s.", pSrc->m_nMaxCount, m_nMaxCount,
					   m_pDebugName);
		}
		else
		{
...
template<typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate(Type *out, float frac, CInterpolatedVarEntry *start,
																	CInterpolatedVarEntry *end)
{
	Assert(start);
	Assert(end);
...
template<typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Extrapolate(Type *pOut, CInterpolatedVarEntry *pOld,
																	...
																	float flMaxExtrapolationAmount)
{
	...
	else
	{
		...
		for(int i = 0; i < m_nMaxCount; i++)
		{
			pOut[i] = ExtrapolateInterpolatedVarType(pOld->GetValue()[i], pNew->GetValue()[i], divisor,
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::TimeFixup2_Hermite(
	...
	typename CInterpolatedVarArrayBase<Type, IS_ARRAY>::CInterpolatedVarEntry *&start, float dt1)
{
	float dt2 = start->changetime - prev->changetime;

	// If times are not of the same interval renormalize the earlier sample to allow for uniform hermite spline
	// interpolation
...
template<typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate_Hermite(Type *out, float frac,
																			CInterpolatedVarEntry *prev,
																			CInterpolatedVarEntry *start,
																			CInterpolatedVarEntry *end, bool looping)
{
	Assert(start);
	Assert(end);
...
template<typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Hermite(Type *out, float frac,
																		   CInterpolatedVarEntry *prev,
																		   CInterpolatedVarEntry *start,
																		   CInterpolatedVarEntry *end)
{
...
template<typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Hermite_SmoothVelocity(Type *out, float frac,
...
template<typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Linear(Type *out, CInterpolatedVarEntry *start,
																		  CInterpolatedVarEntry *end)
{
...
{
public:
	CInterpolatedVarArray(const char *pDebugName = "no debug name") : CInterpolatedVarArrayBase<Type, true>(pDebugName)
	{
