...
bool CAI_BaseNPC::SetSchedule(int localScheduleID)
{
	...
	if(pNewSchedule)
	{
		// ken: I'm don't know of any remaining cases, but if you find one, hunt it down as to why the schedule is
		// getting slammed while they're in the middle of script
		if(m_hCine != NULL)
		{
			if(!(localScheduleID == SCHED_SLEEP || localScheduleID == SCHED_WAIT_FOR_SCRIPT ||
				 localScheduleID == SCHED_SCRIPTED_WALK || localScheduleID == SCHED_SCRIPTED_RUN ||
				 localScheduleID == SCHED_SCRIPTED_CUSTOM_MOVE || localScheduleID == SCHED_SCRIPTED_WAIT ||
...
void CAI_BaseNPC::MaintainSchedule(void)
{
		...
		if(!IsScheduleValid() || m_NPCState != m_IdealNPCState)
		{
			...
			if(HasCondition(COND_TASK_FAILED) && m_NPCState == m_IdealNPCState)
			{
				...
				DevWarning(2, "(%s) Schedule (%s) Failed at %d!\n", STRING(GetEntityName()),
						   GetCurSchedule() ? GetCurSchedule()->GetName() : "GetCurSchedule() == NULL",
	...
	if(CAI_BaseNPC::m_nDebugBits & bits_debugStepAI)
	{
		if(!GetNavigator()->IsGoalActive() && m_nDebugCurIndex >= CAI_BaseNPC::m_nDebugPauseIndex)
		{
			m_flPlaybackRate = 0;
		}
	}
...
bool CAI_BaseNPC::FindCoverPos(CBaseEntity *pEntity, Vector *pResult)
{
	...
	{
		if(!GetTacticalServices()->FindCoverPos(pEntity->GetAbsOrigin(), pEntity->EyePosition(), 0, CoverRadius(),
												pResult))
		{
			return false;
		}
...
bool CAI_BaseNPC::FindCoverPosInRadius(CBaseEntity *pEntity, const Vector &goalPos, float coverRadius, Vector *pResult)
{
	...
	if((!GetSquad() || GetSquad()->GetFirstMember() == this) &&
	   IsCoverPosition(enemyEyePos, goalPos + GetViewOffset()) && IsValidCover(goalPos, NULL))
	{
		coverPos = goalPos;
	}
	else if(!pTacticalServices->FindCoverPos(goalPos, enemyPos, enemyEyePos, 0, coverRadius * 0.5, &coverPos))
	{
		...
		{
			if(!pTacticalServices->FindCoverPos(goalPos, enemyPos, enemyEyePos, coverRadius * 0.5 - 0.1, coverRadius,
												&coverPos))
			{
...
{
	if(!GetTacticalServices()->FindCoverPos(pSound->GetSoundReactOrigin(), pSound->GetSoundReactOrigin(),
											MIN(pSound->Volume(), 120.0), CoverRadius(), pResult))
	{
		return GetTacticalServices()->FindLateralCover(pSound->GetSoundReactOrigin(), MIN(pSound->Volume(), 60.0),
													   pResult);
	}
...
bool CAI_BaseNPC::FindCoverFromEnemy(bool bNodesOnly, float flMinDistance, float flMaxDistance)
{
	...
	if(bNodesOnly)
	{
		...
		if(!GetTacticalServices()->FindCoverPos(pEntity->GetAbsOrigin(), pEntity->EyePosition(), flMinDistance,
...
float CAI_BaseNPC::CalcReasonableFacing(bool bIgnoreOriginalFacing)
{
	...
	else
	{
		...
		float zEye =
			GetAbsOrigin().z + m_vDefaultEyeOffset.z; // always use standing eye so as to not screw with crouch cover
		...
		else
		{
			...
			else
			{
				...
				if(!GetNavigator()->SetGoal(goal, AIN_DISCARD_IF_FAIL))
				{
					...
					// FIXME: scripted sequences don't actually know how to handle failure, but we're failing.  This is
					// serious
	...
	switch(pTask->iTask)
	{
		...
		case TASK_FACE_HINTNODE:
			...
			GetMotor()->SetIdealYaw(
				CalcReasonableFacing(true)); // CalcReasonableFacing() is based on previously set ideal yaw
			if(FacingIdeal())
				TaskComplete();
			else
				SetTurnActivity();
		...
		case TASK_FACE_PATH:
		{
			...
			else
			{
				const float NPC_TRIVIAL_TURN =
					15; // (Degrees). Turns this small or smaller, don't bother with a transition.
		...
		case TASK_GET_PATH_TO_RANGE_ENEMY_LKP_LOS:
		{
			if(GetEnemy())
			{
				...
				if(dist <= flRange ||
				   GetNavigator()->SetVectorGoalFromTarget(GetEnemy()->GetAbsOrigin(), dist - flRange))
				{
					TaskComplete();
					break;
				}
		...
		case TASK_GET_PATH_TO_ENEMY_LKP_LOS:
		{
			...
			if(!found)
			{
				...
				if(GetTacticalServices()->FindLos(vecEnemy, vecEnemyEye, flMinRange, flMaxRange, 1.0, eFlankType,
		...
		{
			AI_NavGoal_t goal(m_nStoredPathType, AIN_DEF_ACTIVITY, AIN_HULL_TOLERANCE, AIN_DEF_FLAGS,
							  m_hStoredPathTarget);
			...
			switch((int)pTask->flTaskData)
			{
				...
				case PATH_LOS: // A path to get LOS to our goal
				{
					...
					Vector eyePosition =
						(m_hStoredPathTarget != NULL) ? m_hStoredPathTarget->EyePosition() : m_vecStoredPathGoal;

					Vector posLos;

					// See if we've found it
					if(GetTacticalServices()->FindLos(m_vecStoredPathGoal, eyePosition, flMinRange, flMaxRange, 1.0f,
													  &posLos))
					{
						goal.dest = posLos;
						foundPath = GetNavigator()->SetGoal(goal);
				...
				case PATH_COVER: // Get a path to cover FROM our goal
				{
					...
					{
						// Try any cover
						if(GetTacticalServices()->FindCoverPos(pEntity->GetAbsOrigin(), pEntity->EyePosition(), 0,
															   CoverRadius(), &coverPos))
						{
							// If we've found it, find a safe route there
							AI_NavGoal_t coverGoal(GOALTYPE_COVER, coverPos, ACT_RUN, AIN_HULL_TOLERANCE, AIN_DEF_FLAGS,
												   m_hStoredPathTarget);
		...
		case TASK_ENABLE_SCRIPT:
		{
			...
			{
				// FIXME: too many ss assume its safe to leave the npc is whatever sequence they were in before, so only
				// slam their activity
				//		  if they're playing a recognizable movement animation
				//
#ifdef HL2_EPISODIC
				// dvs: Check current activity rather than ideal activity. Since scripted NPCs early out in
				// MaintainActivity,
				//      they'll never reach their ideal activity if it's different from their current activity.
				if(GetActivity() == ACT_WALK || GetActivity() == ACT_RUN || GetActivity() == ACT_WALK_AIM ||
				...
				if(GetIdealActivity() == ACT_WALK || GetIdealActivity() == ACT_RUN ||
				   GetIdealActivity() == ACT_WALK_AIM || GetIdealActivity() == ACT_RUN_AIM)
				{
					SetActivity(ACT_IDLE);
				}
#endif // HL2_EPISODIC
...
void CAI_BaseNPC::RunDieTask()
{
	...
	if(IsActivityFinished() && GetCycle() >= 1.0f)
	{
		...
		if(!BBoxFlat())
		{
			// a bit of a hack. If a corpses' bbox is positioned such that being left solid so that it can be attacked
			// will block the player on a slope or stairs, the corpse is made nonsolid.
			//					SetSolid( SOLID_NOT );
			...
			UTIL_SetSize(this, WorldAlignMins(),
						 Vector(WorldAlignMaxs().x, WorldAlignMaxs().y, WorldAlignMins().z + 1));
	}
}
...
void CAI_BaseNPC::RunAttackTask(int task)
{
	...
	{
		if((task == TASK_RANGE_ATTACK1 || task == TASK_RELOAD) && (CapabilitiesGet() & bits_CAP_AIM_GUN) &&
	...
	switch(pTask->iTask)
	{
			...
			else
			{
				...
				if(GetNavType() == NAV_JUMP)
				{
					if(GetFlags() & FL_ONGROUND)
					{
						...
						SetNavType(NAV_GROUND); // this assumes that NAV_JUMP only happens with npcs that use NAV_GROUND
												// as base movement
					}
		...
		case TASK_MOVE_TO_GOAL_RANGE:
		{
			...
			else
			{
				...
				if(GetNavigator()->GetNavType() != NAV_JUMP)
				{
					...
					if((distance < pTask->flTaskData) ||
					   (vecGoalPos - pTarget->GetAbsOrigin()).Length() > pTask->flTaskData * 0.5)
					{
		...
		case TASK_GET_PATH_TO_ENEMY_LKP_LOS:
		{
			...
			if(GetTaskInterrupt() > 0)
			{
				...
				Vector vecEnemy =
					(pTask->iTask == TASK_GET_PATH_TO_ENEMY_LOS) ? GetEnemy()->GetAbsOrigin() : GetEnemyLKP();
				AI_NavGoal_t goal(m_vInterruptSavePosition, ACT_RUN, AIN_HULL_TOLERANCE);
			...
			switch(GetTaskInterrupt())
			{
				case 0:
				{
					if(IsPlayerAlly())
					{
						...
						hintCriteria.AddExcludePosition(GetAbsOrigin(),
					...
					AngleVectors(ang, &move);
#endif // HL2_EPISODIC
					if(GetNavigator()->SetVectorGoal(move, (float)pTask->flTaskData, MIN(36, pTask->flTaskData),
													 true) &&
					...
					else
					{
						...
						if(GetNavigator()->SetVectorGoal(move, (float)pTask->flTaskData, MIN(24, pTask->flTaskData),
														 true) &&
						   IsValidMoveAwayDest(GetNavigator()->GetGoalPos()))
						{
							TaskComplete();
				...
				case 1:
				{
					...
					else
					{
						...
						if(GetNavigator()->SetVectorGoal(move, (float)pTask->flTaskData, MIN(6, pTask->flTaskData),
														 false) &&
						   IsValidMoveAwayDest(GetNavigator()->GetGoalPos()))
						{
							TaskComplete();
				...
				case 2:
				{
					...
					if(GetTacticalServices()->FindCoverPos(GetLocalOrigin(), EyePosition(), 0, CoverRadius(),
														   &coverPos) &&
					   IsValidMoveAwayDest(GetNavigator()->GetGoalPos()))
					{
		...
		{
			bool fTimeExpired =
				(pTask->flTaskData != 0 && pTask->flTaskData < gpGlobals->curtime - GetTimeTaskStarted());
		...
		case TASK_WALK_PATH_FOR_UNITS:
		{
			...
			if(distance > pTask->flTaskData || GetNavigator()->GetGoalType() == GOALTYPE_NONE)
			{
				TaskComplete();
			}
			break;
		...
		case TASK_FALL_TO_GROUND:
			...
			else
			{
				if(IsWaitFinished())
				{
					...
					// isn't actually falling, and make an attempt to slam the ground entity to whatever's under the
					// NPC.
	...
	else if(m_iszSceneCustomMoveSeq != NULL_STRING)
	{
		...
		if(iSequence == ACTIVITY_NOT_AVAILABLE)
		{
			Warning("SCRIPT_CUSTOM_MOVE: %s failed scripted custom move. Has no sequence called: %s\n", GetClassname(),
					STRING(m_iszSceneCustomMoveSeq));
		}
	}
...
int CAI_BaseNPC::SelectIdleSchedule()
{
	...
	if(HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_COMBAT) || HasCondition(COND_HEAR_WORLD) ||
	   HasCondition(COND_HEAR_BULLET_IMPACT) || HasCondition(COND_HEAR_PLAYER))
	{
...
int CAI_BaseNPC::SelectAlertSchedule()
{
	...
	if(HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_PLAYER) || HasCondition(COND_HEAR_WORLD) ||
	   HasCondition(COND_HEAR_BULLET_IMPACT) || HasCondition(COND_HEAR_COMBAT))
	{
		return SCHED_ALERT_FACE_BESTSOUND;
	}
...
int CAI_BaseNPC::SelectCombatSchedule()
{
	...
	{
		if(HasCondition(COND_SEE_ENEMY) || HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE))
		{
			FearSound();
			// ClearCommandGoal();
	...
	if(HasCondition(COND_WEAPON_PLAYER_IN_SPREAD) || HasCondition(COND_WEAPON_BLOCKED_BY_FRIEND) ||
	...
	if(!HasCondition(COND_CAN_RANGE_ATTACK1) && !HasCondition(COND_CAN_MELEE_ATTACK1))
	{
		...
		if(HasCondition(COND_TOO_FAR_TO_ATTACK) &&
		   (CapabilitiesGet() & (bits_CAP_INNATE_RANGE_ATTACK1 | bits_CAP_INNATE_RANGE_ATTACK2)))
			return SCHED_MOVE_TO_WEAPON_RANGE;
