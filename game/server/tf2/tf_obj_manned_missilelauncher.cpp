...
// CVars
ConVar obj_manned_missilelauncher_health("obj_manned_missilelauncher_health", "100", FCVAR_NONE,
										 "Manned Missile Launcher health");
ConVar obj_manned_missilelauncher_range_def("obj_manned_missilelauncher_range_def", "1100", FCVAR_NONE,
											"Defensive Manned Missile Launcher range");
ConVar obj_manned_missilelauncher_range_off("obj_manned_missilelauncher_range_off", "900", FCVAR_NONE,
											"Offensive Manned Missile Launcher range");
ConVar obj_manned_missilelauncher_damage("obj_manned_missilelauncher_damage", "150", FCVAR_NONE,
										 "Manned Missile Launcher damage");
ConVar obj_manned_missilelauncher_radius("obj_manned_missilelauncher_radius", "128", FCVAR_NONE,
										 "Manned Missile Launcher explosive radius");
...
void CObjectMannedMissileLauncher::MissileRechargeThink(void)
{
	...
	SetNextThink(gpGlobals->curtime + (HasPowerup(POWERUP_EMP) ? (MANNED_MISSILELAUNCHER_RECHARGE_TIME * 1.5)
															   : MANNED_MISSILELAUNCHER_RECHARGE_TIME));
...
void CObjectMannedMissileLauncher::Fire()
{
	...
	m_flNextAttack = gpGlobals->curtime + (HasPowerup(POWERUP_EMP) ? MANNED_MISSILELAUNCHER_REFIRE_TIME * 2
																   : MANNED_MISSILELAUNCHER_REFIRE_TIME);
}
...
void CObjectMannedMissileLauncher::PostDataUpdate(DataUpdateType_t updateType)
{
	...
	if(teamchanged || updateType == DATA_UPDATE_CREATED)
	{
