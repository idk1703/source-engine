...
void CFireDamageMgr::FrameUpdatePostEntityThink()
{
	...
	for(int iCur = m_DamageEnts.Head(); iCur != m_DamageEnts.InvalidIndex(); iCur = iNext)
	{
		...
		for(i = 0; i < pEnt->m_nAttackers; i++)
		{
			...
			if(!bEntsValid || pEnt->m_Attackers[i].m_flVelocity <= 0.001)
			{
				if(bEntsValid)
					ApplyCollectedDamage(pEnt, i); // Apply the last-remaining damage from this guy.

				Q_memmove(&pEnt->m_Attackers[i], &pEnt->m_Attackers[i + 1],
						  sizeof(pEnt->m_Attackers[0]) * (pEnt->m_nAttackers - i - 1));
				Q_memmove(&flContributionPercent[i], &flContributionPercent[i + 1],
						  sizeof(flContributionPercent[0]) * (pEnt->m_nAttackers - i - 1));
...
void CFireDamageMgr::ApplyCollectedDamage(CFireDamageMgr::CDamageEnt *pEnt, int iAttacker)
{
	...
	CTakeDamageInfo info(NULL, pAttacker->m_hAttacker, pAttacker->m_flDamageSum * GetFireDamageScale(pEnt->m_hEnt),
						 DMG_BURN);
...
bool IsBurnableEnt(CBaseEntity *pEntity, int iIgnoreTeam)
{
	...
	if(dynamic_cast<CBasePlayer *>(pEntity) || dynamic_cast<CAI_BaseNPC *>(pEntity) ||
...
int FindBurnableEntsInSphere(CBaseEntity **ents, float *dists, int nMaxEnts, const Vector &vecCenter,
							 float flSearchRadius, CBaseEntity *pOwner)
{
	Assert(nMaxEnts > 0);
	int nOutEnts = 0;

	CBaseEntity *pEntity;
	for(CEntitySphereQuery sphere(vecCenter, flSearchRadius); (pEntity = sphere.GetCurrentEntity()) != NULL;
		sphere.NextEntity())
	{
