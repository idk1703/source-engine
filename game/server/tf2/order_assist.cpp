...
bool COrderAssist::CreateOrder(CPlayerClass *pClass)
{
	...
	int sorted[512];
	int nSorted = BuildSortedActiveList(sorted, ARRAYSIZE(sorted), SortFn_TeamPlayersByDistance,
										IsValidFn_PlayersWantingAssist, &info, pClass->GetTeam()->GetNumPlayers());
	...
	if(nSorted)
	{
		...
		pClass->GetTeam()->AddOrder(ORDER_ASSIST, pPlayerToAssist, info.m_pPlayer, COMMAND_ASSIST_DISTANCE, 25, pOrder);
		...
		int nSortedEnemies =
			BuildSortedActiveList(sortedEnemies, ARRAYSIZE(sortedEnemies), SortFn_PlayerEntitiesByDistance,
								  IsValidFn_OnEnemyTeam, &info, gpGlobals->maxClients);
