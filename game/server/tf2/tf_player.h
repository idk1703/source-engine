...
#define SetClassTouch(_player, a)                 \
	_player->SetTouch(CBaseTFPlayer::ClassTouch); \
	_player->m_pfnClassTouch = static_cast<void (CPlayerClass::*)(CBaseEntity *)>(a)
...
public:
	...
	void PickShieldAnimation(Activity &activity, int &overlayindex, bool moving, bool ducked, Activity overlay,
							 Activity crouch, Activity normal, bool autokill = true, float blendin = 0.0f,
							 float blendout = 0.0f);
	...
	bool IsSameClass(CBaseTFPlayer *pPlayer)
	{
		Assert(pPlayer);
		return IsClass((TFClass)pPlayer->PlayerClass());
	}
	...
	tfplayertech_t &AvailableTech(int i)
	{
		return m_rgClientTechAvail[i];
	}
	...
	CTFPlayerLocalData *GetLocalData()
	{
		return &m_TFLocal;
	}
	...
	// Control zone
	CControlZone *GetCurrentZone()
	{
		return m_pCurrentZone;
	}
	void SetCurrentZone(CControlZone *pZone)
	{
		m_pCurrentZone = pZone;
	}

	// Camouflage
	float GetCamouflageAmount(void)
	{
	...
	double LastTimeDamagedByEnemy() const
	{
		return m_flLastTimeDamagedByEnemy;
	}

	// Orders
	COrder *GetOrder(void)
	{
		return m_hSelectedOrder;
	}
	...
	virtual CBaseCombatWeapon *GetLastWeaponBeforeObject(void)
	{
...
private:
	...
	bool IsClientRagdoll() const
	{
		return m_hRagdollShadow.Get() != 0;
	}
	...
	IMPLEMENT_NETWORK_VAR_FOR_DERIVED(
		m_iMaxHealth); // Make sure this ent is marked as changed when m_iMaxHealth changes.
	...
	friend void *SendProxy_RideVehicle(const void *pStruct, const void *pData, CSendProxyRecipients *pRecipients,
									   int objectID);
