...
{
	SpawnWalker(WALKER_MINI_STRIDER_MODEL,					  // Model name.
	...
	if(m_flLargeGunCountdown <= 0)
	{
		...
		if(trace.fraction < 1)
		{
			...
			if(pDriver)
			{
				...
				te->Explosion(filter, 0.0, &vHitPos, g_sModelIndexFireball, 2.0, 15, TE_EXPLFLAG_NONE, flDamageRadius,
							  flDamage);
...
void CWalkerMiniStrider::FireMachineGun()
{
	...
	if(pDriver)
	{
		...
		UTIL_TraceLine(vEyePos, vEyePos + vEyeForward * flMaxRange, MASK_SOLID, this, COLLISION_GROUP_NONE, &trace);
		...
		// Shoot!
		TFGameRules()->FireBullets(
			CTakeDamageInfo(this, pDriver, tf_skirmisher_machinegun_damage.GetFloat(), DMG_BULLET),
			1, // Num shots
			vAttachmentPos, vecDir,
...
void CWalkerMiniStrider::WalkerThink()
{
	...
	if(m_flOriginToLowestLegHeight != -1)
	{
		...
		trace_t trace;
		UTIL_TraceLine(vCenter, vCenter - Vector(0, 0, 2000), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &trace);
...
Vector CWalkerMiniStrider::GetWalkerLocalMovement()
{
	...
	Vector vMovement = vForward * (GetSteerVelocity().x * flSpeed) + vRight * (GetSteerVelocity().y * flSpeed);
