...
// Damage CVars
ConVar weapon_mortar_shell_damage("weapon_mortar_shell_damage", "0", FCVAR_NONE,
								  "Mortar's standard shell maximum damage");
ConVar weapon_mortar_shell_radius("weapon_mortar_shell_radius", "0", FCVAR_NONE,
								  "Mortar's standard shell splash radius");
ConVar weapon_mortar_starburst_damage("weapon_mortar_starburst_damage", "0", FCVAR_NONE,
									  "Mortar's starburst maximum damage");
ConVar weapon_mortar_starburst_radius("weapon_mortar_starburst_radius", "0", FCVAR_NONE,
									  "Mortar's starburst splash radius");
ConVar weapon_mortar_cluster_shells("weapon_mortar_cluster_shells", "0", FCVAR_NONE,
									"Number of shells a mortar cluster round bursts into");
...
BEGIN_DATADESC(CMortarRound)
	...
	// Function Pointers
	DEFINE_FUNCTION(MissileTouch), DEFINE_FUNCTION(FallThink),
...
void CMortarRound::MissileTouch(CBaseEntity *pOther)
{
	...
	if(m_iRoundType == MA_STARBURST)
	{
		...
		RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), weapon_mortar_starburst_damage.GetFloat(), DMG_BLAST),
					 GetAbsOrigin(), weapon_mortar_starburst_radius.GetFloat(), CLASS_NONE, NULL);
	...
	else
	{
		...
		RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), weapon_mortar_shell_damage.GetFloat(), DMG_BLAST),
					 GetAbsOrigin(), weapon_mortar_shell_radius.GetFloat(), CLASS_NONE, NULL);
		UTIL_Remove(this);
	}
}
	...
	if(m_pLauncher)
	{
		...
		if(m_iRoundType == MA_CLUSTER)
		{
			...
			for(int i = 0; i < weapon_mortar_cluster_shells.GetInt(); i++)
			{
				...
				CMortarRound *pRound = CMortarRound::Create(GetAbsOrigin(), vecVelocity,
															GetOwnerEntity() ? GetOwnerEntity()->edict() : NULL);
