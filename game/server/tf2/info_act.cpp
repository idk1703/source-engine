...
BEGIN_DATADESC(CInfoAct)
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "Start", InputStart),
		...
		// outputs
		DEFINE_OUTPUT(m_OnStarted, "OnStarted"), DEFINE_OUTPUT(m_OnFinishedTeamNone, "OnFinishedWinNone"),
		DEFINE_OUTPUT(m_OnFinishedTeam1, "OnFinishedWin1"), DEFINE_OUTPUT(m_OnFinishedTeam2, "OnFinishedWin2"),
		...
		// functions
		DEFINE_FUNCTION(ActThink), DEFINE_FUNCTION(ActThinkEndActOverlayTime), DEFINE_FUNCTION(RespawnTimerThink),
		DEFINE_FUNCTION(Team1RespawnDelayThink), DEFINE_FUNCTION(Team2RespawnDelayThink),
...
SendPropInt(SENDINFO(m_iActNumber), 5), SendPropInt(SENDINFO(m_spawnflags), SF_ACT_BITS, SPROP_UNSIGNED),
	SendPropFloat(SENDINFO(m_flActTimeLimit), 12), SendPropInt(SENDINFO(m_nRespawn1Team1Time), 8),
	SendPropInt(SENDINFO(m_nRespawn2Team1Time), 8), SendPropInt(SENDINFO(m_nRespawn1Team2Time), 8),
	SendPropInt(SENDINFO(m_nRespawn2Team2Time), 8), SendPropInt(SENDINFO(m_nRespawnTeam1Delay), 8),
...
void CInfoAct::SetUpRespawnTimers()
{
	...
	{
		SetContextThink(Team1RespawnDelayThink, gpGlobals->curtime + m_nRespawnTeam1Delay,
						RESPAWN_TEAM_1_DELAY_CONTEXT);
	}
	else
	{
	...
	{
		SetContextThink(Team2RespawnDelayThink, gpGlobals->curtime + m_nRespawnTeam2Delay,
						RESPAWN_TEAM_2_DELAY_CONTEXT);
	}
	else
	{
