...
public:
	...
	{
		return (m_iSalvoLeft <= 0);
	};

	float LastBlastTime()
	{
		return m_flLastBlastTime;
	}
	void SetBlastTime(float time)
	{
		m_flLastBlastTime = time;
	}

	const Vector &LastBlastPosition()
	{
		return m_vLastBlastPos;
	}
	void SetBlastPosition(const Vector &pos)
	{
		m_vLastBlastPos = pos;
	}
