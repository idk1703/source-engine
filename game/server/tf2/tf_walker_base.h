...
public:
	...
	void SpawnWalker(const char *pModelName, int objectType, const Vector &vPlacementMins, const Vector &vPlacementMaxs,
					 int iHealth, int nMaxPassengers,
...
public:
	// This is a hack to prevent the strider from playing a lot of footstep sounds, until we figure out 9-way anim
	// events
...
private:
	...
	// Note that it is completely virtual - the speed that the walker moves at is gotten from the animations ground
	// speed. It maps this velocity a 9-way blend for movement. +X = forward, +Y = left.
