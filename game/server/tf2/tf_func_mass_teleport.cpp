...
BEGIN_DATADESC(CFuncMassTeleport)
	...
	DEFINE_FIELD(m_hTargetEntity, FIELD_EHANDLE),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnTeleport, "OnTeleport"), DEFINE_OUTPUT(m_OnActive, "OnActive"),
...
// Purpose:
//-----------------------------------------------------------------------------
void CFuncMassTeleport::MassTeleport(CTFTeam *pTeam, Vector vSourceMins, Vector vSourceMaxs, Vector vDest, bool bSwap,
									 bool bPlayersOnly, EHANDLE hMCV, bool bTelefragDestination)
{
	...
	*/
	...
	for(int i = 0; i < count; i++)
	{
		...
		Vector vEntTarget =
			vDest + vDelta +
			Vector(0, 0, 3); // Teleport them a little above the ground, to allow for the suspension drop.
	...
	if(bSwap)
	{
		MassTeleport(pTeam, vSourceMins - vSource + vDest, vSourceMaxs - vSource + vDest, vSource, false, bPlayersOnly,
					 hMCV, false);
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CFuncMassTeleport::DoTeleport(CTFTeam *pTeam, Vector vSourceMins, Vector vSourceMaxs, Vector vDest, bool bSwap,
								   bool bPlayersOnly, EHANDLE hMCV)
{
...
void CFuncMassTeleport::InputDoTeleport(inputdata_t &inputdata)
{
	...
	if(m_hTargetEntity)
	{
		...
		DoTeleport(((CTFTeam *)GetTeam()), vecAbsMins, vecAbsMaxs, m_hTargetEntity->GetAbsOrigin(), true, false,
				   EHANDLE());
	}
}
