	...
	if(!tunnel->exitstarted)
	{
		if(gpGlobals->curtime > tunnel->fadeintime)
		{
			...
			if(m_hTunnelExit != NULL)
			{
				...
				// Do a placement test to prevent the player from teleporting inside another player, the ground, or just
				// to help prevent badly placed tunnels from causing stuck situations.
				...
				tunnel->player->Teleport(&vTarget /*m_hTunnelExit->GetAbsOrigin()*/, &m_hTunnelExit->GetAbsAngles(),
										 NULL);
...
//  he is hidden
	...
	if(tunnel->ilastremaining != tunnel->iremaining && tunnel->needremainigcounter && tunnel->iremaining >= 1 &&
	   tunnel->player != NULL)
	{
		// Counter
		ClientPrint(tunnel->player, HUD_PRINTCENTER,
					UTIL_VarArgs("\nExiting tunnel in %d %s\n", tunnel->iremaining,
								 tunnel->iremaining > 1 ? "seconds" : "second"));
	}
