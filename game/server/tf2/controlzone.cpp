...
void CControlZone::ReevaluateControllingTeam(void)
{
	...
	// Find out if the zone isn't owned by anyone at all (hasn't been touched since the map started, and it started
	// un-owned)
	...
	if(m_iTryingToChangeToTeam != ZONE_CONTESTED)
	{
		...
		else
		{
			DevMsg(1, "trigger_controlzone: (%s) changing team to %d in %.2f seconds\n", GetDebugName(),
				   m_iTryingToChangeToTeam, m_flTimeTillCaptured);
