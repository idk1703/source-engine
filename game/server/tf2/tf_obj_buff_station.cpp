...
// Console Variables
//
static ConVar obj_buff_station_damage_modifier("obj_buff_station_damage_modifier", "1.5", 0,
											   "Scales the damage a player does while connected to the buff station.");
...
void CObjectBuffStation::AttachPlayer(CBaseTFPlayer *pPlayer)
{
	...
	// Update think.
	if(GetNextThink(BUFF_STATION_BOOST_PLAYER_THINK_CONTEXT) >
	   gpGlobals->curtime + BUFF_STATION_BOOST_PLAYER_THINK_INTERVAL)
	{
...
void CObjectBuffStation::AttachObject(CBaseObject *pObject, bool bPlacing)
{
	...
	m_aObjectAttachInfo[m_nObjectCount].m_hRope =
		CreateRope(pObject, m_aObjectAttachInfo[m_nObjectCount].m_iAttachPoint, nObjectAttachPoint);
	m_nObjectCount += 1;
	...
	// Update think.
	if(GetNextThink(BUFF_STATION_BOOST_OBJECT_THINK_CONTEXT) >
	   gpGlobals->curtime + BUFF_STATION_BOOST_OBJECT_THINK_INTERVAL)
	{
...
void CObjectBuffStation::BoostPlayerThink(void)
{
	...
	for(int iPlayer = 0; iPlayer < BUFF_STATION_MAX_PLAYERS; iPlayer++)
	{
		...
		if(!bIsEmped)
		{
			...
			bBoosted = pPlayer->AttemptToPowerup(POWERUP_BOOST, 0, flHealAmount, this,
												 &m_aPlayerAttachInfo[iPlayer].m_DamageModifier);
		}
...
void CObjectBuffStation::BoostObjectThink(void)
{
	...
	for(int iObject = m_nObjectCount; --iObject >= 0;)
	{
		...
		// Boost it
		if(!pObject->AttemptToPowerup(POWERUP_BOOST, BUFF_STATION_BOOST_OBJECT_THINK_INTERVAL, 0, this,
									  &m_aObjectAttachInfo[iObject].m_DamageModifier))
		{
