...
BEGIN_SEND_TABLE_NOBASE(CObjectResourcePump, DT_ResourcePumpTeamOnlyVars)
	SendPropInt(SENDINFO(m_iPumpLevel), 4), SendPropEHandle(SENDINFO(m_hResourceZone)),
END_SEND_TABLE()
...
ConVar obj_resourcepump_rate("obj_resourcepump_rate", "0", FCVAR_NONE,
							 "Base rate at which resource pumps give resources to their team.");
ConVar obj_resourcepump_amount("obj_resourcepump_amount", "0", FCVAR_NONE,
							   "Base amount of resources that resource pumps give to their team.");
...
void CObjectResourcePump::Spawn(void)
{
	...
	m_fObjectFlags |= OF_DONT_PREVENT_BUILD_NEAR_OBJ | OF_DOESNT_NEED_POWER | OF_MUST_BE_BUILT_IN_RESOURCE_ZONE |
