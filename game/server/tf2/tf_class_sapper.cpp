...
ConVar class_sapper_boost_amount("class_sapper_boost_amount", "35", FCVAR_NONE,
								 "Amount of energy drained for the Sapper to get a boost.");
ConVar class_sapper_boost_time("class_sapper_boost_time", "15", FCVAR_NONE,
							   "Sapper's boost time after draining a full charge of boost.");
ConVar class_sapper_boost_rate("class_sapper_boost_rate", "1", FCVAR_NONE, "Sapper's boost rate on his self-boost.");
ConVar class_sapper_damage_modifier("class_sapper_damage_modifier", "1.5", 0,
									"Scales the damage a Sapper does while he's self-boosted.");
...
void CPlayerClassSapper::ClassThink(void)
{
	...
	{
		m_pPlayer->AttemptToPowerup(POWERUP_BOOST, class_sapper_boost_time.GetFloat(),
									class_sapper_boost_rate.GetFloat() * 0.1, m_pPlayer, &m_DamageModifier);
	}
	else
	{
		m_DamageModifier.RemoveModifier();
