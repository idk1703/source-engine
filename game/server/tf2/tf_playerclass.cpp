...
ConVar tf2_object_hard_limits("tf2_object_hard_limits", "0", FCVAR_NONE,
							  "If true, use hard object limits instead of resource costs");
...
void PrintPlayerClassStats()
{
	...
	for(int i = TFCLASS_UNDECIDED + 1; i < STATS_NUM_GROUPS; i++)
	{
		...
		double flAvgNormalizedEngagementDist =
			nTotalNormalizedEngagements ? (flTotalNormalizedEngagementDist / nTotalNormalizedEngagements) : 0;
	...
	for(i = TFCLASS_UNDECIDED + 1; i < STATS_NUM_GROUPS; i++)
	{
		...
		for(int j = TFCLASS_UNDECIDED + 1; j < STATS_NUM_GROUPS; j++)
		{
			...
			pFileSys->FPrintf(hFile, "\t%d",
							  (int)(pInter->m_nNormalizedEngagements
										? (pInter->m_flTotalNormalizedEngagementDist / pInter->m_nNormalizedEngagements)
										: 0));
			pFileSys->FPrintf(
				hFile, "\t%d",
				(int)(pInter->m_nEngagements ? (pInter->m_flTotalEngagementDist / pInter->m_nEngagements) : 0));
			pFileSys->FPrintf(hFile, "\n");
		}
	}
...
//=====================================================================
// Base PlayerClass
CPlayerClass::CPlayerClass(CBaseTFPlayer *pPlayer, TFClass iClass) : m_TFClass(iClass)
{
	m_pPlayer = pPlayer;
...
void CPlayerClass::CreateClass(void)
{
	...
	if(GetTFClassInfo(m_TFClass)->m_pClassObjects[0] != OBJ_LAST)
	{
		...
		for(int i = 0; i < OBJ_LAST; i++)
		{
			...
			CWeaponObjectSelection *pSelection =
				(CWeaponObjectSelection *)m_pPlayer->GiveNamedItem("weapon_objectselection", iClassObject);
			if(pSelection)
			{
...
int CPlayerClass::GetNumObjects(int iObjectType)
{
	...
	{
		return (m_pPlayer->GetNumObjects(OBJ_SENTRYGUN_PLASMA) +
				m_pPlayer->GetNumObjects(OBJ_SENTRYGUN_ROCKET_LAUNCHER));
	}
...
void CPlayerClass::PickupObject(CBaseObject *pObject)
{
	...
	if(!tf2_object_hard_limits.GetBool())
	{
		int iCost = CalculateObjectCost(pObject->GetType(), GetNumObjects(pObject->GetType()),
										m_pPlayer->GetTeamNumber(), true);
		m_pPlayer->AddBankResources(iCost);
	}
}
...
{
	return m_pPlayer->GetNumResourceZoneOrders() == 0 && m_pPlayer->GetOrder();
}
...
void CPlayerClass::InitVCollision(void)
{
	...
	m_pPlayer->m_pShadowStand = PhysModelCreateCustom(m_pPlayer, pStandModel, m_pPlayer->GetLocalOrigin(),
													  m_pPlayer->GetLocalAngles(), "tfplayer_generic", false, &solid);
	m_pPlayer->m_pShadowStand->SetCallbackFlags(CALLBACK_SHADOW_COLLISION);

	// create crouchig hull
	m_pPlayer->m_pShadowCrouch = PhysModelCreateCustom(m_pPlayer, pCrouchModel, m_pPlayer->GetLocalOrigin(),
													   m_pPlayer->GetLocalAngles(), "tfplayer_generic", false, &solid);
