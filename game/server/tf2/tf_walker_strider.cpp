...
char *g_StriderFeet[] = {"left foot", "right foot", "back foot"};
...
{
	SpawnWalker(WALKER_STRIDER_MODEL,	  // Model name.
...
void CWalkerStrider::WalkerThink()
{
		...
		else
		{
			...
			if(gpGlobals->curtime >= m_flNextShootTime)
			{
				...
				if(pDriver)
				{
					...
					RadiusDamage(CTakeDamageInfo(this, pDriver, flDamage, DMG_BLAST), vHitPos, flDamageRadius,
								 CLASS_NONE, NULL);
				}
	...
	else
	{
		...
		{
			trace_t trace;
			UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 2000), MASK_SOLID_BRUSHONLY, this,
