...
#define ACC_RICOCHET_MULTIPLE \
	0.25 // The effect of ricochets on accuracy is multiplied by this by the number of ricochets nearby
...
void CBaseTFPlayer::CalculateAccuracy(void)
{
	...
	if(gpGlobals->curtime <= flFarTime)
		m_flTargetAccuracy *= 1 + (m_flNumberOfRicochets * ACC_RICOCHET_MULTIPLE) *
									  (ACCMULT_RICOCHET * ((flFarTime - gpGlobals->curtime) / ACC_RICOCHET_TIME));
