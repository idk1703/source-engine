...
ConVar tf_knockdowntime("tf_knockdowntime", "3", FCVAR_NONE,
						"Length of time knocked-down players remain on the ground.");

// Adrenalin
ConVar class_commando_speed("class_commando_speed", "200", FCVAR_NONE, "Commando movement speed.");
ConVar class_commando_rush_length("class_commando_rush_length", "10", FCVAR_NONE,
								  "Commando's adrenalin rush length in seconds.");
ConVar class_commando_rush_recharge("class_commando_rush_recharge", "60", FCVAR_NONE,
									"Commando's adrenalin rush recharge time in seconds.");

ConVar class_commando_battlecry_radius("class_commando_battlecry_radius", "512", FCVAR_NONE,
									   "Commando's battlecry radius.");
ConVar class_commando_battlecry_length("class_commando_battlecry_length", "10", FCVAR_NONE,
									   "Length of adrenalin rush given by the Commando's battlecry in seconds.");
...
void CPlayerClassCommando::BullRushTouch(CBaseEntity *pTouched)
{
	if(pTouched->IsPlayer() && !pTouched->InSameTeam(m_pPlayer))
	{
		...
		// float flDamage = 1.0f - ( ( COMMANDO_BULLRUSH_TIME - m_ClassData.m_flBullRushTime ) * ( 1.0f /
		// COMMANDO_BULLRUSH_TIME ) ); flDamage *= 115.0f; // max bullrush damage
	...
	if(m_bCanBoot && m_pPlayer->IsAlive() && m_flNextBootCheck < gpGlobals->curtime)
	{
		...
		for(CEntitySphereQuery sphere(vecTarget, 16); pEntity = sphere.GetCurrentEntity(); sphere.NextEntity())
		{
			if(pEntity->IsPlayer() && (pEntity != m_pPlayer))
			{
				...
				if(!pPlayer->IsClass(TFCLASS_UNDECIDED) && pPlayer->IsAlive() &&
				   pPlayer->InSameTeam(m_pPlayer) == false)
				{
					Boot(pPlayer);
...
void CPlayerClassCommando::StartAdrenalinRush(void)
{
	...
	if(m_bHasBattlecry)
	{
		...
		for(int i = 0; i < m_pPlayer->GetTFTeam()->GetNumPlayers(); i++)
		{
			...
			// Is it within range?
			if(pPlayer != m_pPlayer && (pPlayer->GetAbsOrigin() - m_pPlayer->GetAbsOrigin()).Length() <
										   class_commando_battlecry_radius.GetFloat())
			{
				// Can I see it?
				trace_t tr;
				UTIL_TraceLine(m_pPlayer->EyePosition(), pPlayer->EyePosition(), MASK_SOLID_BRUSHONLY, m_pPlayer,
