...
void CVehicleTeleportStation::OnFinishedDeploy(void)
{
	...
	while((pTeleporter = (CFuncMassTeleport *)gEntList.FindEntityByClassname(pTeleporter, "func_mass_teleport")) !=
		  NULL)
	{
...
bool CVehicleTeleportStation::ValidDeployPosition(void)
{
	...
	QAngle vecAngles;
	if(!GetAttachment("teleport_corner1", m_vecTeleporterMins, vecAngles) ||
	   !GetAttachment("teleport_corner4", m_vecTeleporterMaxs, vecAngles))
		return false;
	...
	int iNumberOfNearbyEntities =
		UTIL_EntitiesInBox(pListOfNearbyEntities, 100, m_vecTeleporterMins, m_vecTeleporterMaxs, MASK_ALL);
