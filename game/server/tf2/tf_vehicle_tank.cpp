...
SendPropAngle(SENDINFO(m_flTurretYaw), 11), SendPropAngle(SENDINFO(m_flTurretPitch), 11),
END_SEND_TABLE
()
...
void CVehicleTank::Fire()
{
	...
	CWeaponGrenadeRocket *pRocket =
		CWeaponGrenadeRocket::Create(vFireOrigin, vForward, vehicle_tank_range.GetFloat(), this);
	if(pRocket)
	{
