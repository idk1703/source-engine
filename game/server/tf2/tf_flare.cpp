...
BEGIN_DATADESC(CSignalFlare)
	...
	// Function Pointers
	DEFINE_FUNCTION(FlareTouch), DEFINE_FUNCTION(FlareThink),

END_DATADESC()

// Data Table
IMPLEMENT_SERVERCLASS_ST(CSignalFlare, DT_SignalFlare)
SendPropFloat(SENDINFO(m_flDuration), 0, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_flScale), 0, SPROP_NOSCALE),
