...
public:
	...
	virtual bool ResupplyAmmoType(
		float flAmount,
		const char *pAmmoType); // Purpose: Supply the player with Ammo. Return true if some ammo was given.
	...
	// Resources
	virtual int ClassCostAdjustment(ResupplyBuyType_t nType)
	{
		return 0;
	}
	...
	void SetClassModel(string_t sModelName, int nTeam)
	{
		m_sClassModel[nTeam] = sModelName;
	}
	...
	// Accessors.
	inline CBaseTFPlayer *GetPlayer()
	{
		return m_pPlayer;
	}
	CTFTeam *GetTeam();
