...
ConVar obj_child_damage_factor("obj_child_damage_factor", "0.25", FCVAR_NONE,
...
SendPropInt(SENDINFO(m_iHealth), 13), SendPropInt(SENDINFO(m_iMaxHealth), 13),
	SendPropInt(SENDINFO(m_bHasSapper), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iObjectType), 6, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_bBuilding), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_bPlacing), 1, SPROP_UNSIGNED),
	...
	SendPropInt(SENDINFO(m_bDeteriorating), 1, SPROP_UNSIGNED), SendPropEHandle(SENDINFO(m_hBuiltOnEntity)),
	SendPropInt(SENDINFO(m_takedamage), 2, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_bDisabled), 1, SPROP_UNSIGNED),
...
void CBaseObject::SetBuilder(CBaseTFPlayer *pBuilder, bool moveobjects)
{
	TRACE_OBJECT(UTIL_VarArgs("%0.2f CBaseObject::SetBuilder builder %s, moveobjects == %s\n", gpGlobals->curtime,
							  pBuilder ? pBuilder->GetPlayerName() : "NULL", moveobjects ? "true" : "false"));
...
void CBaseObject::DeterioratingThink(void)
{
	...
	flDamage = 0.1 * (GetMaxHealth() / object_deterioration_time.GetFloat()) *
			   ceil(flDeteriorationTime / object_deterioration_time.GetFloat());
	...
	// Apply the damage
	OnTakeDamage(
		CTakeDamageInfo(GetContainingEntity(INDEXENT(0)), GetContainingEntity(INDEXENT(0)), flDamage, DMG_GENERIC));
}
...
//-----------------------------------------------------------------------------
bool CBaseObject::FindNearestBuildPoint(CBaseEntity *pEntity, Vector vecBuildOrigin, float &flNearestPoint,
...
bool CBaseObject::VerifyCorner(const Vector &vBottomCenter, float xOffset, float yOffset)
{
	...
	trace_t tr;
	UTIL_TraceLine(vStart, vStart - Vector(0, 0, tf_obj_ground_clearance.GetFloat()), MASK_SOLID, this,
...
bool CBaseObject::CheckBuildOrigin(CBaseTFPlayer *pPlayer, const Vector &vecInitialBuildOrigin, bool bSnappedToPoint)
{
	...
	if(!bSnappedToPoint)
	{
		...
		{
			UTIL_TraceHull(Vector(m_vecBuildOrigin.x, m_vecBuildOrigin.y, topZ),
						   Vector(m_vecBuildOrigin.x, m_vecBuildOrigin.y, flBoxBottomZ), -vHalfBuildDimsXY,
						   vHalfBuildDimsXY, MASK_SOLID, this, COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
			bottomZ = tr.endpos.z;
	...
	if(!bSnappedToPoint || IsAVehicle())
	{
		if(!(m_fObjectFlags & OF_DONT_PREVENT_BUILD_NEAR_OBJ))
		{
			...
			int iNumberOfNearbyEntities =
				UTIL_EntitiesInSphere(pListOfNearbyEntities, 100, m_vecBuildOrigin, GetNearbyObjectCheckRadius(), 0);
...
void CBaseObject::AlignToGround(Vector vecOrigin)
{
	...
	UTIL_TraceLine(vecOrigin, vecOrigin + Vector(0, 0, -flHeight), MASK_SOLID, this, COLLISION_GROUP_PLAYER_MOVEMENT,
				   &tr);
	if(tr.fraction != 1.0)
	{
...
bool CBaseObject::StartBuilding(CBaseEntity *pBuilder)
{
	...
	if(VPhysicsGetObject() && !IsBuiltOnAttachment())
	{
		...
		physcollision->CollideGetAABB(&absmins, &absmaxs, VPhysicsGetObject()->GetCollide(), GetAbsOrigin(),
									  GetAbsAngles());
...
void CBaseObject::BuildingThink(void)
{
	...
	Repair((GetMaxHealth() - OBJECT_CONSTRUCTION_STARTINGHEALTH) / m_flTotalConstructionTime *
...
void CBaseObject::AttemptToActivateBuffStation(void)
{
	...
	if(GetBuffStation()->IsPlacing() || GetBuffStation()->IsBuilding() || !GetBuffStation()->IsPowered())
		return;
...
void CBaseObject::SetHealth(float flHealth)
{
	if(IsDisabled())
	{
		if((m_flMinDisableHealth != 0.0f && flHealth > m_flMinDisableHealth) || (flHealth > 1))
		{
	...
	switch(iPowerup)
	{
		case POWERUP_BOOST:
		{
			...
			if(GetHealth() < GetMaxHealth())
			{
				...
				{
					Msg("TOTAL REPAIR: %.2f in %.2f\n\n", m_flRepairedSinceLastTime, (gpGlobals->curtime -
				m_flLastRepairTime) );
				}
				...
				if ( pAttacker->IsPlayer() )
				{
					...
					Msg("(%.2f) %s repaired %s for %.2f health.\n", gpGlobals->curtime, pPlayer->GetPlayerName(),
				GetClassname(), flAmount );
				}
				*/
...
void CBaseObject::Killed(void)
{
	...
	te->Explosion(filter, 0.0, &GetAbsOrigin(), g_sModelIndexFireball,
				  5.4, // radius
				  15, TE_EXPLFLAG_NODLIGHTS, 256, 200);
	...
	if(bDropResources && iCost)
	{
		...
		TFStats()->IncrementTeamStat(GetTeamNumber(), TF_TEAM_STAT_RESOURCE_CHUNKS_DROPPED,
									 (resource_chunk_processed_value.GetFloat() * nProcessedChunks) +
										 (resource_chunk_value.GetFloat() * nNormalChunks));
	}
...
void CBaseObject::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if(pActivator->IsPlayer())
	{
		...
		else
		{
			...
			{
				// Don't abort if we just started placing it. This is to catch people who'd like to +use toggle instead
				// of hold down
...
void CBaseObject::ChangeBuilder(CBaseTFPlayer *pNewBuilder, bool moveobjects)
{
	...
	TRACE_OBJECT(UTIL_VarArgs("%0.2f CBaseObject::ChangeBuilder old %s, new %s, moveobjects %s\n", gpGlobals->curtime,
							  ...
							  pNewBuilder ? pNewBuilder->GetPlayerName() : "NULL", moveobjects ? "true" : "false"));
	...
	TRACE_OBJECT(
		UTIL_VarArgs("%0.2f CBaseObject::ChangeBuilder moving associated objects %s\n", gpGlobals->curtime, classname));
...
void CBaseObject::AddVulnerablePoint(const char *szName, float fMultiplier)
{
	...
	if(obj_debug_vulnerable.GetBool())
	{
		Msg("Vulnerable point %s on model %s added with a damage multiplier of %f. (%i, %i)\n", szName,
			STRING(GetModelName()), fMultiplier, nSet, nBox);
	}
...
void CBaseObject::SetBuffStation(CObjectBuffStation *pBuffStation, bool bPlacing)
{
	...
	if(GetBuffStation() && (pBuffStation == GetBuffStation()) && !m_bBuffActivated && !bPlacing && !IsBuilding() &&
	   IsPowered())
	{
		BuffStationActivate();
	}
...
bool CBaseObject::IsHookedAndBuffed(void)
{
	...
	{
		if(GetBuffStation()->IsPowered() && !GetBuffStation()->IsPlacing() && !GetBuffStation()->IsBuilding())
			return true;
	}
