...
bool COrderRepair::CreateOrder_RepairFriendlyObjects(CPlayerClassDefender *pClass)
{
	...
	int nSorted = BuildSortedActiveList(sorted, MAX_TEAM_OBJECTS, SortFn_Defender, IsValidFn_RepairFriendlyObjects,
										&info, pTeam->GetNumObjects());
	...
	if(nSorted)
	{
		...
		if(pPlayer->GetAbsOrigin().DistTo(pObjToHeal->GetAbsOrigin()) < flClosestDist)
		{
			...
			pTeam->AddOrder(ORDER_REPAIR, pObjToHeal, pPlayer, 1e24, 60, pOrder);
...
bool COrderRepair::CreateOrder_RepairOwnObjects(CPlayerClass *pClass)
{
	...
	int sorted[16];
	int nSorted = BuildSortedActiveList(sorted, sizeof(sorted) / sizeof(sorted[0]), SortFn_PlayerObjectsByDistance,
										IsValidFn_RepairOwnObjects, &info, info.m_pPlayer->GetObjectCount());
	...
	if(nSorted)
	{
		...
		info.m_pPlayer->GetTFTeam()->AddOrder(ORDER_REPAIR, pObj, info.m_pPlayer, 1e24, 60, pOrder);
