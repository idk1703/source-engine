...
bool CHintCriteria::MatchesSingleHintType() const
{
	...
	if(m_iFirstHintType != HINT_ANY && m_iLastHintType == HINT_NONE)
	{
		return true;
	}
...
CAI_Hint *CAI_HintManager::AddFoundHint(CAI_Hint *hint)
{
	if(hint)
	{
		CAI_HintManager::gm_nFoundHintIndex =
			(CAI_HintManager::gm_nFoundHintIndex + 1) & CAI_HintManager::HINT_HISTORY_MASK;
...
//-----------------------------------------------------------------------------
int CAI_HintManager::FindAllHints(CAI_BaseNPC *pNPC, const Vector &position, const CHintCriteria &hintCriteria,
...
CAI_Hint *CAI_HintManager::FindHint(CAI_BaseNPC *pNPC, const Vector &position, const CHintCriteria &hintCriteria)
{
	...
	if(!pBestHint)
	{
		Msg("%i search failed for [%d] at pos %.3f %.3f %.3f [%.4f msec ~ %.4f msec per node]\n", gpGlobals->tickcount,
			pNPC ? pNPC->entindex() : -1, position.x, position.y, position.z, timer.GetDuration().GetMillisecondsF(),
...
//-----------------------------------------------------------------------------
CAI_Hint *CAI_HintManager::FindHint(CAI_BaseNPC *pNPC, Hint_e nHintType, int nFlags, float flMaxDist,
...
int CAI_HintManager::GetFlags(const char *token)
{
	...
	if((bits & bits_HINT_NODE_NEAREST) && (bits & bits_HINT_NODE_RANDOM))
	{
		// Remove random
		bits &= ~bits_HINT_NODE_RANDOM;
...
void CAI_HintManager::DumpHints()
{
	...
	while(pCurHint)
	{
		...
		Msg("(%.1f, %.1f, %.1f) -- Node ID: %d; WC id %d; type %d\n", v.x, v.y, v.z, pCurHint->GetNodeId(),
			pCurHint->GetWCId(), pCurHint->HintType());
...
void CAI_HintManager::DrawHintOverlays(float flDrawDuration)
{
	...
	for(int i = 0; i < c; ++i)
	{
		...
		{
			vHintPos = g_pBigAINet->GetNode(pHint->m_NodeData.nNodeID)
		...
		if(pHint->GetNodeId() != NO_NODE)
			NDebugOverlay::Text(vHintPos + Vector(0, 6, 8),
								CFmtStr("(%d), (%d)", pHint->HintType(), pHint->GetNodeId()), true, flDrawDuration);
		else
...
BEGIN_DATADESC(CAI_Hint)
	...
	DEFINE_EMBEDDED(m_NodeData),
		...
		DEFINE_FIELD(m_hHintOwner, FIELD_EHANDLE), DEFINE_FIELD(m_flNextUseTime, FIELD_TIME),
		DEFINE_FIELD(m_vecForward, FIELD_VECTOR), DEFINE_KEYFIELD(m_nodeFOV, FIELD_FLOAT, "nodeFOV"),
...
//-----------------------------------------------------------------------------
bool CAI_Hint::HintMatchesCriteria(CAI_BaseNPC *pNPC, const CHintCriteria &hintCriteria, const Vector &position,
								   float *flNearestDistance, bool bIgnoreLock, bool bIgnoreHintType)
{
	// Cannot be locked
		...
		if(hintCriteria.HasFlag(bits_HINT_NODE_VISIBLE))
		{
			...
			else
			{
				...
				else
				{
					...
					AI_TraceLine(pNPC->EyePosition(), vHintPos + pNPC->GetViewOffset(), MASK_NPCSOLID_BRUSHONLY, pNPC,
	...
	if(hintCriteria.HasFlag(bits_HINT_NODE_CLEAR))
	{
		...
		else
		{
			...
			AI_TraceHull(GetAbsOrigin(), GetAbsOrigin(), pNPC->WorldAlignMins(), pNPC->WorldAlignMaxs(), MASK_SOLID,
	...
	if(hintCriteria.HasFlag(bits_HINT_NODE_NOT_VISIBLE_TO_PLAYER | bits_HINT_NODE_VISIBLE_TO_PLAYER))
	{
		...
		for(int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if(pPlayer)
			{
				...
				{
					trace_t tr;
					UTIL_TraceLine(pPlayer->EyePosition(), GetAbsOrigin(), MASK_SOLID_BRUSHONLY, pPlayer,
								   COLLISION_GROUP_NONE, &tr);
...
ConCommand ai_drop_hint("ai_drop_hint", CC_ai_drop_hint, "Drop an ai_hint at the player's current eye position.",
						FCVAR_CHEAT);