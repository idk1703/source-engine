...
BEGIN_DATADESC(CEntityParticleTrail)
	...
	DEFINE_FIELD(m_iMaterialName, FIELD_MATERIALINDEX), DEFINE_EMBEDDED(m_Info),
...
// Input  : pTarget -
//-----------------------------------------------------------------------------
CEntityParticleTrail *CEntityParticleTrail::Create(CBaseEntity *pTarget, const EntityParticleTrailInfo_t &info,
...
//-----------------------------------------------------------------------------
void CEntityParticleTrail::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
											 const notify_system_event_params_t &params)
{
	...
	if(eventType == NOTIFY_EVENT_DESTROY)
	{
		...
		{
			m_nRefCount = 0;
			SetContextThink(&CEntityParticleTrail::SUB_Remove, gpGlobals->curtime + m_Info.m_flLifetime,
							s_pRetireContext);
		}
	}
}
