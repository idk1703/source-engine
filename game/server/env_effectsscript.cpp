...
void CEnvEffectsScript::TrailEffectEvent(CEffectScriptElement *pEffect)
{
	if(pEffect->IsActive() == false)
	{
		...
		if(pEffect->m_pTrail == NULL)
		{
			...
			pEffect->m_pTrail->SetTransparency(pEffect->m_iRenderType, pEffect->m_iR, pEffect->m_iG, pEffect->m_iB,
...
void CEnvEffectsScript::SpriteEffectEvent(CEffectScriptElement *pEffect)
{
	if(pEffect->IsActive() == false)
	{
		...
		if(pEffect->m_pSprite == NULL)
		{
			...
			pEffect->m_pSprite->SetTransparency(pEffect->m_iRenderType, pEffect->m_iR, pEffect->m_iG, pEffect->m_iB,
												pEffect->m_iA, kRenderFxNone);
	...
	while(1)
	{
		...
		if(!Q_stricmp(token, "{"))
		{
			while(1)
			{
				...
				{
					ParseToken();
					sscanf(token, "%i %i %i %i", &NewElement.m_iR, &NewElement.m_iG, &NewElement.m_iB,
						   &NewElement.m_iA);
