...
BEGIN_DATADESC(CTriggerAreaCapture)
	...
	DEFINE_FUNCTION(CTriggerAreaCaptureShim::Touch),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnStartTeam1, "OnStartTeam1"), DEFINE_OUTPUT(m_OnStartTeam2, "OnStartTeam2"),
		DEFINE_OUTPUT(m_OnBreakTeam1, "OnBreakTeam1"), DEFINE_OUTPUT(m_OnBreakTeam2, "OnBreakTeam2"),
		DEFINE_OUTPUT(m_OnCapTeam1, "OnCapTeam1"), DEFINE_OUTPUT(m_OnCapTeam2, "OnCapTeam2"),

		DEFINE_OUTPUT(m_StartOutput, "OnStartCap"), DEFINE_OUTPUT(m_BreakOutput, "OnBreakCap"),
...
void CTriggerAreaCapture::CaptureThink(void)
{
	...
	if(m_bCapturing)
	{
			...
			if((flPercentToGo <= 0.5 || TeamplayGameRules()->PointsMayAlwaysBeBlocked()) && m_hPoint)
			{
				...
				if(pBlockingPlayer)
				{
					...
					for(int i = m_Blockers.Count() - 1; i >= 0; i--)
					{
						...
						if(m_Blockers[i].iCapAttemptNumber != m_iCapAttemptNumber ||
						   !IsTouching(m_Blockers[i].hPlayer) ||
						   (TeamplayGameRules()->PointsMayAlwaysBeBlocked() &&
							m_Blockers[i].flNextBlockTime < gpGlobals->curtime && m_bStartTouch))
						{
							m_Blockers.Remove(i);
							continue;
						}
		...
		else
		{
			...
			if(TeamplayRoundBasedRules() && m_hPoint &&
			   TeamplayRoundBasedRules()->TeamMayCapturePoint(m_nCapturingTeam, m_hPoint->GetPointIndex()))
			{
				float flDecreaseScale =
					CaptureModeScalesWithPlayers() ? mp_capdeteriorate_time.GetFloat() : flTotalTimeToCap;
		...
		if ( (iTeamMembersHere == 0 ) || (mp_capstyle.GetInt() == 0 && iTeamMembersHere <
		m_TeamData[m_nCapturingTeam].iNumRequiredToCap) )
		{
			BreakCapture( true );
			return;
		}
		*/
		...
		{
			// We may get several simultaneous CaptureThink calls from StartTouch if there are several players on the
			// trigger when it is enabled (like in Raid mode). We haven't started reducing m_fTimeRemaining yet but the
			// second call to CaptureThink from StartTouch has m_bCapturing set to true and we hit this condition and
			// call BreakCapture right away. We put this check here to prevent calling BreakCapture from the StartTouch
			// call to CaptureThink. If the capture should really be broken it will happen the next time the trigger
			// thinks on its own.
			if(!m_bStartTouch)
			{
		...
		if(ent)
		{
			...
			if(IsTouching(player) &&
			   (player->GetTeamNumber() ==
				team)) // need to make sure disguised spies aren't included in the list of capping players
			{
	...
	if(!m_hPoint)
	{
		...
		if(m_hPoint)
		{
			...
			for(int i = FIRST_GAME_TEAM; i < GetNumberOfTeams(); i++)
			{
				...
				ObjectiveResource()->SetCPRequiredCappers(m_hPoint->GetPointIndex(), i,
														  m_TeamData[i].iNumRequiredToCap);
				ObjectiveResource()->SetTeamCanCap(m_hPoint->GetPointIndex(), i, m_TeamData[i].bCanCap);

				if(CaptureModeScalesWithPlayers())
				{
					ObjectiveResource()->SetCPCapTime(m_hPoint->GetPointIndex(), i,
													  (m_flCapTime * 2) * m_TeamData[i].iNumRequiredToCap);
				}
				else
				{
				...
				ObjectiveResource()->SetCPCapTimeScalesWithPlayers(m_hPoint->GetPointIndex(),
																   CaptureModeScalesWithPlayers());
			}
		}
	}
}
...
bool CTriggerAreaCapture::CheckIfDeathCausesBlock(CBaseMultiplayerPlayer *pVictim, CBaseMultiplayerPlayer *pKiller)
{
	...
	{
		bBreakCap =
			(m_TeamData[m_nCapturingTeam].iBlockedTouching - 1 < m_TeamData[m_nCapturingTeam].iNumRequiredToCap);
	}
