...
SendPropInt(SENDINFO(m_clrRender), 32, SPROP_UNSIGNED, SendProxy_Color32ToInt),
	...
	SendPropInt(SENDINFO(m_nVerticalSize), 16, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nMinDist), 16, SPROP_UNSIGNED),
	...
	SendPropInt(SENDINFO(m_spawnflags), 8, SPROP_UNSIGNED), SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD),
...
BEGIN_DATADESC(CLightGlow)
	...
	DEFINE_KEYFIELD(m_nVerticalSize, FIELD_INTEGER, "VerticalGlowSize"),
		...
		DEFINE_KEYFIELD(m_nMinDist, FIELD_INTEGER, "MinDist"), DEFINE_KEYFIELD(m_nMaxDist, FIELD_INTEGER, "MaxDist"),
...
void CLightGlow::Activate()
{
	...
	if(m_nOuterMaxDist > LIGHTGLOW_OUTERMAXDIST_MAX_VALUE)
	{
		Warning("env_lightglow outermaxdist too large (%d should be %d).\n", m_nOuterMaxDist.Get(),
				LIGHTGLOW_OUTERMAXDIST_MAX_VALUE);
