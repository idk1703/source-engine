...
#define AI_LOAD_SCHEDULE(classname, name)                                                         \
	...
	{                                                                                             \
		...
		{                                                                                         \
			classname::gm_SchedLoadStatus.fValid = g_AI_SchedulesManager.LoadSchedulesFromBuffer( \
				#classname, (char *)g_psz##name, &classname::gm_ClassScheduleIdSpace);            \
		}                                                                                         \
	} while(false)
...
#define AI_LOAD_DEF_SCHEDULE(classname, name)                                                   \
	...
	{                                                                                           \
		extern const char *g_psz##name;                                                         \
		if(!g_AI_SchedulesManager.LoadSchedulesFromBuffer(#classname, (char *)g_psz##name,      \
														  &classname::gm_ClassScheduleIdSpace)) \
			return false;                                                                       \
	} while(false)
