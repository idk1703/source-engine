...
BEGIN_DATADESC(CControlPoint)
	...
	DEFINE_KEYFIELD(m_iszPrintName, FIELD_STRING, "point_printname"),
		...
		DEFINE_OUTPUT(m_AxisCapOutput, "OnAxisCap"), DEFINE_OUTPUT(m_PointResetOutput, "OnCapReset"),

		DEFINE_OUTPUT(m_OwnerChangedToAllies,
					  "AlliesCapturePoint"), // these are fired when a team does the work to change the owner
...
bool CControlPoint::KeyValue(const char *szKeyName, const char *szValue)
{
	if(FStrEq(szKeyName, "point_default_owner"))
	{
		...
		if(m_iDefaultOwner != TEAM_ALLIES && m_iDefaultOwner != TEAM_AXIS && m_iDefaultOwner != TEAM_UNASSIGNED)
		{
