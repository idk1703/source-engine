...
ConVar mp_logdetail("mp_logdetail", "0", FCVAR_NONE, "Logs attacks.  Values are: 0=off, 1=enemy, 2=teammate, 3=both)",
					true, 0.0f, true, 3.0f);
	...
	bool PrintDodEvent(IGameEvent *event) // print Mod specific logs
	{
		...
		if(FStrEq(eventName, "player_death"))
		{
			...
			else if(pAttacker)
			{
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\" with \"%s\"\n", pAttacker->GetPlayerName(),
							   attackerid, pAttacker->GetNetworkIDString(), pAttacker->GetTeam()->GetName(),
							   pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
							   pPlayer->GetTeam()->GetName(), weapon);
			}
			else
			{
				// killed by the world
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" committed suicide with \"world\"\n", pPlayer->GetPlayerName(),
							   userid, pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName());
			}
		...
		else if(FStrEq(eventName, "player_hurt"))
		{
			   ...
			   (!isTeamAttack && (detail & LOG_DETAIL_ENEMY_ATTACKS)))
			{
				...
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" attacked \"%s<%i><%s><%s>\" with \"%s\" (damage \"%d\") (health "
							   "\"%d\") (hitgroup \"%s\")\n",
							   pAttacker->GetPlayerName(), attackerid, pAttacker->GetNetworkIDString(),
							   pAttacker->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid,
							   pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName(), weapon,
							   event->GetInt("damage"), event->GetInt("health"), hitgroupStr);
			}
			return true;
		}
		else if(FStrEq(eventName, "player_changeclass"))
		{
			...
			if(iClass == PLAYERCLASS_RANDOM)
			{
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" changed role to \"Random\"\n", pPlayer->GetPlayerName(), userid,
							   pPlayer->GetNetworkIDString(), pTeam->GetName());
			}
			else if(iClass < GetGlobalDODTeam(iTeam)->GetNumPlayerClasses())
			{
				...
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" changed role to \"%s\"\n", pPlayer->GetPlayerName(), userid,
							   pPlayer->GetNetworkIDString(), pTeam->GetName(), pInfo.m_szPrintName);
			}
			return true;
		}
		...
		else if(FStrEq(eventName, "dod_team_scores"))
		{
			...
			UTIL_LogPrintf("Team \"Allies\" triggered \"team_scores\" (roundswon \"%d\") (tickpoints \"%d\") "
						   "(numplayers \"%d\")\n",
		...
		else if(FStrEq(eventName, "dod_round_win"))
		{
			...
			UTIL_LogPrintf("Team \"%s\" triggered \"round_win\" (rounds_won \"%d\") (numplayers \"%d\")\n",
						   pTeam->GetName(), pTeam->GetRoundsWon(), pTeam->GetNumPlayers());
		...
		else if(FStrEq(eventName, "dod_tick_points"))
		{
			...
			UTIL_LogPrintf(
				...
				pTeam->GetName(), iScore, iTotalScore, pTeam->GetNumPlayers());
			...
			for(int i = 0; i < iNumCappers; i++)
			{
				...
				if(i == 0)
				{
					Q_snprintf(buf, sizeof(buf),
							   "Team \"%s\" triggered \"captured_loc\" (flagindex \"%d\") (flagname \"%s\") "
							   "(numplayers \"%d\") ",
							   pPlayer->GetTeam()->GetName(), iPointIndex, szPointName, iNumCappers);
				}

				char playerBuf[256];
				Q_snprintf(playerBuf, sizeof(playerBuf), "(player \"%s<%i><%s><%s>\") ", pPlayer->GetPlayerName(),
		...
		else if(FStrEq(eventName, "dod_capture_blocked"))
		{
			...
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"capblock\" (flagindex \"%d\") (flagname \"%s\")\n",
						   ...
						   pPlayer->GetTeam()->GetName(), iPointIndex, szPointName);
		}
		...
		else if(FStrEq(eventName, "dod_kill_planter"))
		{
			...
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"kill_planter\" against \"%s<%i><%s><%s>\"\n",
						   pPlayer->GetPlayerName(), pPlayer->GetUserID(), pPlayer->GetNetworkIDString(),
						   pPlayer->GetTeam()->GetName(), pVictim->GetPlayerName(), pVictim->GetUserID(),
						   pVictim->GetNetworkIDString(), pVictim->GetTeam()->GetName());
