...
ConVar dod_playerstatetransitions(
	"dod_playerstatetransitions", "-2", FCVAR_CHEAT,
	"dod_playerstatetransitions <ent index or -1 for all>. Show player state transitions.");
ConVar dod_debugdamage("dod_debugdamage", "0", FCVAR_CHEAT);

ConVar mp_bandage_heal_amount("mp_bandage_heal_amount", "40", FCVAR_GAMEDLL,
							  "How much health to give after a successful bandage\n", true, 0, false, 0);

ConVar dod_freezecam("dod_freezecam", "1", FCVAR_REPLICATED | FCVAR_ARCHIVE,
					 "show players a freezecam shot of their killer on death");
...
BEGIN_SEND_TABLE_NOBASE(CDODPlayerShared, DT_DODSharedLocalPlayerExclusive)
	SendPropInt(SENDINFO(m_iPlayerClass), 4), SendPropInt(SENDINFO(m_iDesiredPlayerClass), 4),
		SendPropFloat(SENDINFO(m_flDeployedYawLimitLeft)), SendPropFloat(SENDINFO(m_flDeployedYawLimitRight)),
...
BEGIN_SEND_TABLE_NOBASE(CDODPlayerShared, DT_DODPlayerShared)
	SendPropFloat(SENDINFO(m_flStamina), 0, SPROP_NOSCALE | SPROP_CHANGES_OFTEN),
		SendPropTime(SENDINFO(m_flSlowedUntilTime)), SendPropBool(SENDINFO(m_bProne)),
		SendPropBool(SENDINFO(m_bIsSprinting)), SendPropTime(SENDINFO(m_flGoProneTime)),
		SendPropTime(SENDINFO(m_flUnProneTime)), SendPropTime(SENDINFO(m_flDeployChangeTime)),
		SendPropTime(SENDINFO(m_flDeployedHeight)), SendPropBool(SENDINFO(m_bPlanting)),
		SendPropBool(SENDINFO(m_bDefusing)),
		SendPropDataTable("dodsharedlocaldata", 0, &REFERENCE_SEND_TABLE(DT_DODSharedLocalPlayerExclusive),
...
SendPropExclude("DT_BaseAnimating", "m_flPoseParameter"), SendPropExclude("DT_BaseAnimating", "m_flPlaybackRate"),
	...
	// Data that only gets sent to the local player.
	SendPropDataTable("dodlocaldata", 0, &REFERENCE_SEND_TABLE(DT_DODLocalPlayerExclusive),
					  SendProxy_SendLocalDataTable),
	SendPropDataTable("dodnonlocaldata", 0, &REFERENCE_SEND_TABLE(DT_DODNonLocalPlayerExclusive),
	...
	SendPropEHandle(SENDINFO(m_hRagdoll)), SendPropBool(SENDINFO(m_bSpawnInterpCounter)),
	SendPropInt(SENDINFO(m_iAchievementAwardsMask), NUM_ACHIEVEMENT_AWARDS, SPROP_UNSIGNED),
END_SEND_TABLE
()

	BEGIN_DATADESC(CDODPlayer) DEFINE_THINKFUNC(PushawayThink),
...
//-----------------------------------------------------------------------------
void CDODPlayer::RunPlayerMove(const QAngle &viewangles, float forwardmove, float sidemove, float upmove,
...
void CDODPlayer::Spawn()
{
	...
	SetContextThink(&CDODPlayer::PushawayThink, gpGlobals->curtime + PUSHAWAY_THINK_INTERVAL,
					DOD_PUSHAWAY_THINK_CONTEXT);
...
ConVar dod_friendlyfiresafezone("dod_friendlyfiresafezone", "100", FCVAR_ARCHIVE,
								"Units around a player where they will not damage teammates, even if FF is on", true, 0,
								false, 0);
	...
	if(bTakeDamage)
	{
		if(dod_debugdamage.GetInt())
		{
			...
			Q_snprintf(buf, sizeof(buf),
					   "%s hit %s in the %s hitgroup for %f damage ( %f base damage ) ( %s now has %d health )\n",
					   pAttacker->GetPlayerName(), GetPlayerName(), szHitbox, flDamage, flOriginalDmg, GetPlayerName(),
...
CDODPlayerStateInfo *CDODPlayer::State_LookupInfo(DODPlayerState state)
{
	...
	static CDODPlayerStateInfo playerStateInfos[] = {
		...
		{STATE_PICKINGTEAM, "STATE_PICKINGTEAM", &CDODPlayer::State_Enter_PICKINGTEAM, NULL,
		 &CDODPlayer::State_PreThink_PICKING},
		{STATE_PICKINGCLASS, "STATE_PICKINGCLASS", &CDODPlayer::State_Enter_PICKINGCLASS, NULL,
		 &CDODPlayer::State_PreThink_PICKING},
		{STATE_DEATH_ANIM, "STATE_DEATH_ANIM", &CDODPlayer::State_Enter_DEATH_ANIM, NULL,
		 &CDODPlayer::State_PreThink_DEATH_ANIM},
		{STATE_OBSERVER_MODE, "STATE_OBSERVER_MODE", &CDODPlayer::State_Enter_OBSERVER_MODE, NULL,
		 &CDODPlayer::State_PreThink_OBSERVER_MODE}};
	...
	if(dod_freezecam.GetBool())
	{
		...
		if(!m_bPlayedFreezeCamSound && GetObserverTarget() && GetObserverTarget() != this)
		{
			...
			float flFreezeSoundTime =
				m_flDeathTime + DOD_DEATH_ANIMATION_TIME - flFreezeSoundLength + 0.4f /* travel time */;
		...
		if(gpGlobals->curtime >=
		   (m_flDeathTime + DOD_DEATH_ANIMATION_TIME)) // allow 2 seconds death animation / death cam
		{
...
void CDODPlayer::AttemptToExitFreezeCam(void)
{
	float flFreezeTravelTime =
		(m_flDeathTime + DOD_DEATH_ANIMATION_TIME) + 0.4 /*spec_freeze_traveltime.GetFloat()*/ + 0.5;
...
CBaseEntity *CDODPlayer::SelectSpawnSpot(CUtlVector<EHANDLE> *pSpawnPoints, int &iLastSpawnIndex)
{
	...
	for(int i = 0; i < iNumSpawns; i++)
	{
		...
		for(int i = 0; i < 4; i++)
		{
			...
			{
				case 0:
					origin += vecForward * flTestDist;
					break;
				case 1:
					origin += vecRight * flTestDist;
					break;
				case 2:
					origin -= vecForward * flTestDist;
					break;
...
int CDODPlayer::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	if(event)
	{
		...
		if(attacker->IsPlayer())
		{
			...
			if(pDODPlayer->GetTeamNumber() == GetTeamNumber() && pDODPlayer != this &&
			   !FBitSet(info.GetDamageType(), DMG_BOMB))
			{
...
int CDODPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
	...
	if(pInflictor->GetTeamNumber() != TEAM_UNASSIGNED && info.GetAttacker() != NULL &&
	...
	if(bFriendlyFire || info.GetAttacker()->GetTeamNumber() != GetTeamNumber() || pInflictor == this ||
	   info.GetAttacker() == this || info.GetDamageType() & DMG_BOMB)
	{
...
void CDODPlayer::OutputDamageTaken(void)
{
	...
	FOR_EACH_LL(m_DamageTakenList, i)
	{
		...
		if(pRecord)
		{
			Q_snprintf(buf, sizeof(buf), "( life ID %i ) - %d in %d %s", pRecord->GetLifeID(), pRecord->GetDamage(),
					   pRecord->GetNumHits(), (pRecord->GetNumHits() == 1) ? "hit" : "hits");
...
void CDODPlayer::SetupBones(matrix3x4_t *pBoneToWorld, int boneMask)
{
	...
	if(pParent)
	{
		...
		if(pParentCache)
		{
			BuildMatricesWithBoneMerge(pStudioHdr, m_PlayerAnimState->GetRenderAngles(), adjOrigin, pos, q,
	...
	Studio_BuildMatrices(pStudioHdr, m_PlayerAnimState->GetRenderAngles(), adjOrigin, pos, q, -1,
...
void CDODPlayer::PrintLifetimeStats(void)
{
	...
	Msg("                  (shots) (hits) (damage) (avg.dist) (kills) (hits taken) (dmg taken) (times killed) "
		"(accuracy)\n");
	...
	for(i = 0; i < WEAPON_MAX; i++)
	{
		if(m_WeaponStats[i].m_iNumShotsTaken > 0)
		{
			Msg("* %10s (%2i) %6i %6i %8i  %9.1f %7i    %9i   %9i      %9i   %9.1f\n", WeaponIDToAlias(i), i,
				m_WeaponStats[i].m_iNumShotsTaken, m_WeaponStats[i].m_iNumShotsHit,
				m_WeaponStats[i].m_iTotalDamageGiven, m_WeaponStats[i].m_flAverageHitDistance,
				m_WeaponStats[i].m_iNumKills, m_WeaponStats[i].m_iNumHitsTaken, m_WeaponStats[i].m_iTotalDamageTaken,
...
void CDODPlayer::Stats_WeaponHit(CDODPlayer *pVictim, int weaponID, int iDamage, int iDamageGiven, int hitgroup,
								 float flHitDistance)
{
	// distance
...
bool CDODPlayer::WantsLagCompensationOnEntity(const CBasePlayer *pPlayer, const CUserCmd *pCmd,
...
void CDODPlayer::OnGSStatsReceived(GSStatsReceived_t *pResponse)
{
	...
	if(pResponse->m_steamIDUser == GetSteamIDAsUInt64())
	{
		...
		{
			bool bUnlocked;
			if(steamgameserverapicontext->SteamGameServerStats()->GetUserAchievement(
				   pResponse->m_steamIDUser, g_pszAchievementAwards[i], &bUnlocked))
			{
				if(bUnlocked)
				{
...
{
	m_bRecordingStats =
		((iTeam == TEAM_ALLIES || iTeam == TEAM_AXIS) && (iPlayerClass >= 0 && iPlayerClass <= NUM_DOD_PLAYERCLASSES));
