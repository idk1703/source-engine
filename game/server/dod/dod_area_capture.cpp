...
BEGIN_DATADESC(CAreaCapture)
	...
	DEFINE_FUNCTION(AreaTouch),
		...
		// Outputs
		DEFINE_OUTPUT(m_AlliesStartOutput, "OnAlliesStartCap"), DEFINE_OUTPUT(m_AlliesBreakOutput, "OnAlliesBreakCap"),
		DEFINE_OUTPUT(m_AlliesCapOutput, "OnAlliesEndCap"),

		DEFINE_OUTPUT(m_AxisStartOutput, "OnAxisStartCap"), DEFINE_OUTPUT(m_AxisBreakOutput, "OnAxisBreakCap"),
...
void CAreaCapture::Think(void)
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(ent)
		{
			...
			if((pPlayer->m_signals.GetState() & SIGNAL_CAPTUREAREA) && pPlayer->GetCapAreaIndex() == m_iAreaIndex &&
	...
	if(m_bCapturing)
	{
		...
		if(iNumAllies > 0 && iNumAxis > 0)
		{
			...
			if(flPercentToGo <= 0.5 && m_pPoint)
			{
				...
				CDODPlayer *pBlockingPlayer =
					(m_nCapturingTeam == TEAM_ALLIES) ? pFirstAxisTouching : pFirstAlliedTouching;
