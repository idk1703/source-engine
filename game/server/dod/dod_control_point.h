...
public:
	...
	{
		return !(FBitSet(m_spawnflags, CAP_POINT_HIDEFLAG));
	}
	...
	} // the mapper set index
	void SetPointIndex(int index)
	{
		m_iPointIndex = index;
	}
	int GetAlliesIcon(void)
	{
		return m_iAlliesIcon;
	}
	int GetAxisIcon(void)
	{
		return m_iAxisIcon;
	}
	int GetNeutralIcon(void)
	{
		return m_iNeutralIcon;
	}
	...
	int GetBombsRemaining(void)
	{
		return m_iBombsRemaining;
	} // total bombs required
	int GetBombsRequired(void)
	{
		return m_iBombsRequired;
