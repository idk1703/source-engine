...
ConVar mp_tickpointinterval("mp_tickpointinterval", "30", FCVAR_GAMEDLL, "Delay between point gives.", true, 1, false,
							0);
...
bool CControlPointMaster::FindControlPoints(void)
{
	...
	for(i = 0; i < m_ControlPoints.Count(); i++)
	{
		...
		g_pObjectiveResource->SetCPIcons(
			iPointIndex, pPoint->GetHudIconIndexForTeam(TEAM_ALLIES), pPoint->GetHudIconIndexForTeam(TEAM_AXIS),
			pPoint->GetHudIconIndexForTeam(TEAM_UNASSIGNED), pPoint->GetTimerCapHudIcon(), pPoint->GetBombedHudIcon());
	}
...
int CControlPointMaster::TeamOwnsAllPoints(CControlPoint *pOverridePoint /* = NULL */,
