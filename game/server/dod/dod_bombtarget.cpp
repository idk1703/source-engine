...
BEGIN_DATADESC(CDODBombTarget)
	...
	DEFINE_KEYFIELD(m_iszCapPointName, FIELD_STRING, "target_control_point"),
		...
		DEFINE_OUTPUT(m_OnBecomeInactive, "OnBombTargetDeactivated"), DEFINE_OUTPUT(m_OnBombPlanted, "OnBombPlanted"),
		DEFINE_OUTPUT(m_OnBombExploded, "OnBombExploded"), DEFINE_OUTPUT(m_OnBombDisarmed, "OnBombDefused"),

		DEFINE_KEYFIELD(m_bStartDisabled, FIELD_INTEGER, "StartDisabled"),

		DEFINE_USEFUNC(State_Use), DEFINE_THINKFUNC(State_Think),

END_DATADESC();

IMPLEMENT_SERVERCLASS_ST(CDODBombTarget, DT_DODBombTarget)
SendPropInt(SENDINFO(m_iState), 3), SendPropInt(SENDINFO(m_iBombingTeam), 3),

	SendPropModelIndex(SENDINFO(m_iTargetModel)), SendPropModelIndex(SENDINFO(m_iUnavailableModel)),
END_SEND_TABLE
()
...
CDODBombTargetStateInfo *CDODBombTarget::State_LookupInfo(BombTargetState state)
{
	static CDODBombTargetStateInfo bombTargetStateInfos[] = {
		...
		{BOMB_TARGET_ACTIVE, "BOMB_TARGET_ACTIVE", &CDODBombTarget::State_Enter_ACTIVE, NULL, NULL,
		 &CDODBombTarget::State_Use_ACTIVE},
		{BOMB_TARGET_ARMED, "BOMB_TARGET_ARMED", &CDODBombTarget::State_Enter_ARMED, &CDODBombTarget::State_Leave_Armed,
		 &CDODBombTarget::State_Think_ARMED, &CDODBombTarget::State_Use_ARMED}};
...
void CDODBombTarget::Explode(void)
{
	...
	te->Explosion(filter, -1.0, // don't apply cl_interp delay
				  &origin, g_sModelIndexFireball,
				  20, // scale
				  25, TE_EXPLFLAG_NONE, 0, 0);
