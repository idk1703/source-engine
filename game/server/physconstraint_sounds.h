...
public:
	...
	*/
	...
	int HasReversed(const Vector &relativeVelocity, const float thresholdAcceleration[],
...
/** This class encapsulates the data and behavior necessary for a constraint to play sounds.
	...
	DEFINE_KEYFIELD( m_soundInfo.m_soundProfile.m_keyPoints[SimpleConstraintSoundProfile::kMIN_THRESHOLD] , FIELD_FLOAT,
   "minSoundThreshold" ), DEFINE_KEYFIELD(
   m_soundInfo.m_soundProfile.m_keyPoints[SimpleConstraintSoundProfile::kMIN_FULL] , FIELD_FLOAT, "maxSoundThreshold" ),
...
public:
	...
	string_t m_iszReversalSounds[SimpleConstraintSoundProfile::kREVERSAL_SOUND_ARRAY_SIZE]; // Path/filename of WAV
																							// files to play -- one per
																							// entry in threshold.
