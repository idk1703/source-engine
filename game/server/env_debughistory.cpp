...
void CDebugHistory::AddDebugHistoryLine(int iCategory, const char *szLine)
{
	...
	while(iCharsToWrite)
	{
		int iCharsLeftBeforeLoop =
			sizeof(m_DebugLines[iCategory]) - (m_DebugLineEnd[iCategory] - m_DebugLines[iCategory]);
...
void CDebugHistory::DumpDebugHistory(int iCategory)
{
	...
	while((!(pszLine == m_DebugLines[iCategory] &&
			 *(m_DebugLines[iCategory] + sizeof(m_DebugLines[iCategory]) - 1) == '\0') &&
...
static ConCommand dbghist_addline("dbghist_addline", CC_DebugHistory_AddLine,
								  "Add a line to the debug history. Format: <category id> <line>", FCVAR_NONE);
