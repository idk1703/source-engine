...
public:
	...
	void SetNumTrainCappers(int iNumCappers,
							CBaseEntity *pTrigger); // only used for train watchers that control the train movement

	virtual void FireGameEvent(IGameEvent *event);

	int GetCapturerCount(
		void) const; // return the number of players who are "capturing" the payload, or -1 if the payload is blocked

	void ProjectPointOntoPath(const Vector &pos, Vector *posOnPath,
							  float *distanceAlongPath) const; // project the given position onto the track and return
															   // the point and how far along that projected position is
	bool IsAheadOfTrain(
		const Vector &pos) const; // return true if the given position is farther down the track than the train is
...
private:
	...
	float m_flTrainDistanceFromStart; // actual distance along path of train, for comparing against
