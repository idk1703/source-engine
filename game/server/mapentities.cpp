...
#ifdef _WIN32
	qsort(&pSpawnList[0], nEntities, sizeof(pSpawnList[0]),
		  (int(__cdecl *)(const void *, const void *))CompareSpawnOrder);
#elif POSIX
...
void MapEntity_ParseAllEntities(const char *pMapData, IMapEntityFilter *pFilter, bool bActivateEntities)
{
	...
	// Now loop through all our point_template entities and tell them to make templates of everything they're pointing
	// to
		...
		for(int iTemplateNum = 0; iTemplateNum < iNumTemplates; iTemplateNum++)
		{
			...
			for(int iEntNum = 0; iEntNum < nEntities; iEntNum++)
			{
				if(pSpawnList[iEntNum].m_hEntity == pEntity)
				{
					...
					pPointTemplate->AddTemplate(pEntity, pSpawnMapData[iEntNum].m_pMapData,
												pSpawnMapData[iEntNum].m_iMapDataLength);
