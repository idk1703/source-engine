...
BEGIN_DATADESC(CAI_ScriptedSequence)
	...
	DEFINE_KEYFIELD(m_iszEntry, FIELD_STRING, "m_iszEntry"), DEFINE_KEYFIELD(m_iszPreIdle, FIELD_STRING, "m_iszIdle"),
		...
		DEFINE_KEYFIELD(m_fMoveTo, FIELD_INTEGER, "m_fMoveTo"), DEFINE_KEYFIELD(m_flRadius, FIELD_FLOAT, "m_flRadius"),
		...
		DEFINE_FIELD(m_iDelay, FIELD_INTEGER), DEFINE_FIELD(m_bDelayed, FIELD_BOOLEAN),
		DEFINE_FIELD(m_startTime, FIELD_TIME), DEFINE_FIELD(m_bWaitForBeginSequence, FIELD_BOOLEAN),

		DEFINE_FIELD(m_saved_effects, FIELD_INTEGER), DEFINE_FIELD(m_savedFlags, FIELD_INTEGER),
		DEFINE_FIELD(m_savedCollisionGroup, FIELD_INTEGER),

		DEFINE_FIELD(m_interruptable, FIELD_BOOLEAN), DEFINE_FIELD(m_sequenceStarted, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hTargetEnt, FIELD_EHANDLE), DEFINE_FIELD(m_hNextCine, FIELD_EHANDLE),
		DEFINE_FIELD(m_hLastFoundEntity, FIELD_EHANDLE), DEFINE_FIELD(m_hForcedTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_bDontCancelOtherSequences, FIELD_BOOLEAN), DEFINE_FIELD(m_bForceSynch, FIELD_BOOLEAN),

		DEFINE_FIELD(m_bThinking, FIELD_BOOLEAN), DEFINE_FIELD(m_bInitiatedSelfDelete, FIELD_BOOLEAN),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnBeginSequence, "OnBeginSequence"), DEFINE_OUTPUT(m_OnEndSequence, "OnEndSequence"),
		...
		DEFINE_OUTPUT(m_OnScriptEvent[0], "OnScriptEvent01"), DEFINE_OUTPUT(m_OnScriptEvent[1], "OnScriptEvent02"),
		DEFINE_OUTPUT(m_OnScriptEvent[2], "OnScriptEvent03"), DEFINE_OUTPUT(m_OnScriptEvent[3], "OnScriptEvent04"),
		DEFINE_OUTPUT(m_OnScriptEvent[4], "OnScriptEvent05"), DEFINE_OUTPUT(m_OnScriptEvent[5], "OnScriptEvent06"),
		DEFINE_OUTPUT(m_OnScriptEvent[6], "OnScriptEvent07"), DEFINE_OUTPUT(m_OnScriptEvent[7], "OnScriptEvent08"),
...
CAI_BaseNPC *CAI_ScriptedSequence::FindScriptEntity()
{
	...
	else
	{
		...
		if(!pEntity)
		{
			pEntity = gEntList.FindEntityByClassnameWithin(m_hLastFoundEntity, STRING(m_iszEntity), GetAbsOrigin(),
														   m_flRadius);
	...
	while(pEntity != NULL)
	{
		...
		else
		{
			if(interrupt == SS_INTERRUPT_BY_NAME)
				...
				pEntity =
					gEntList.FindEntityByClassnameWithin(pEntity, STRING(m_iszEntity), GetAbsOrigin(), m_flRadius);
		}
	}
...
void CAI_ScriptedSequence::StartScript(void)
{
	...
	if(pTarget)
	{
		...
		// DevMsg( 2,  "\"%s\" found and used (INT: %s)\n", STRING( pTarget->m_iName ), FBitSet(m_spawnflags,
		...
		if(pTarget->IsEFlagSet(EFL_NO_THINK_FUNCTION) && pTarget->GetNextThink() != TICK_NEVER_THINK)
		{
			DevWarning("scripted_sequence %d:%s - restarting dormant entity %d:%s : %.1f:%.1f\n", entindex(),
					   GetDebugName(), pTarget->entindex(), pTarget->GetDebugName(), gpGlobals->curtime,
					   pTarget->GetNextThink());
			pTarget->SetNextThink(gpGlobals->curtime);
		}
	}
}
...
void CAI_ScriptedSequence::ScriptThink(void)
{
	...
	{
		StartScript();
		DevMsg(2, "scripted_sequence %d:\"%s\" using NPC %d:\"%s\"(%s)\n", entindex(), GetDebugName(),
			   GetTarget()->entindex(), GetTarget()->GetEntityName().ToCStr(), STRING(m_iszEntity));
	}
	else
	{
		CancelScript();
...
bool CAI_ScriptedSequence::StartSequence(CAI_BaseNPC *pTarget, string_t iszSeq, bool completeOnEmpty)
{
	...
	// If the target was hidden even though we woke it up, only make it drawable if we're not still on the preidle
	// seq...
	...
	// DevMsg( 2, "%s (%s): started \"%s\":INT:%s\n", STRING( pTarget->m_iName ), pTarget->GetClassname(), STRING(
	// iszSeq), (m_spawnflags & SF_SCRIPT_NOINTERRUPT) ? "No" : "Yes" );
...
void CAI_ScriptedSequence::SynchronizeSequence(CAI_BaseNPC *pNPC)
{
	...
	// Msg("%.2f \"%s\"  %s : %f (%f): interval %f\n", gpGlobals->curtime, GetEntityName().ToCStr(),
	// pNPC->GetClassname(), pNPC->m_flAnimTime.Get(), m_startTime, flInterval );
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ScriptedSequence::SynchNewSequence(CAI_BaseNPC::SCRIPTSTATE newState, string_t iszSequence,
											bool bSynchOtherScenes)
{
	...
	// Msg("%s (for %s) starting %s seq at %0.2f\n", pNPC->GetDebugName(), GetDebugName(), iszSequence,
	// gpGlobals->curtime);
...
void CAI_ScriptedSequence::FixFlyFlag(CAI_BaseNPC *pNPC, int iSavedCineFlags)
{
	...
	if(pNPC->GetTask() &&
	   (pNPC->GetTask()->iTask == TASK_PLAY_SCRIPT || pNPC->GetTask()->iTask == TASK_PLAY_SCRIPT_POST_IDLE))
	{
...
void CAI_ScriptedSequence::ModifyScriptedAutoMovement(Vector *vecNewPos)
{
	if(m_hInteractionRelativeEntity)
	{
		...
		if(bDebug)
		{
			Msg("--\n%s current org: %f %f\n", m_hTargetEnt->GetDebugName(), m_hTargetEnt->GetAbsOrigin().x,
				m_hTargetEnt->GetAbsOrigin().y);
			Msg("%s current org: %f %f", m_hInteractionRelativeEntity->GetDebugName(), vecRelativeOrigin.x,
				vecRelativeOrigin.y);
		}
		...
		if(pAnimating)
		{
			...
			pAnimating->GetSequenceMovement(pAnimating->GetSequence(), 0.0f, pAnimating->GetCycle(), vecDeltaPos,
											vecDeltaAngles);
...
void CAI_ScriptedSequence::CancelScript(void)
{
	...
	bool bDontCancelOther =
		((m_bDontCancelOtherSequences || HasSpawnFlags(SF_SCRIPT_ALLOW_DEATH)) && (m_startTime != 0));
...
BEGIN_DATADESC(CAI_ScriptedSchedule)
	...
	DEFINE_FIELD(m_hLastFoundEntity, FIELD_EHANDLE), DEFINE_KEYFIELD(m_flRadius, FIELD_FLOAT, "m_flRadius"),

		DEFINE_KEYFIELD(m_iszEntity, FIELD_STRING, "m_iszEntity"),
		DEFINE_KEYFIELD(m_nSchedule, FIELD_INTEGER, "schedule"),
		DEFINE_KEYFIELD(m_nForceState, FIELD_INTEGER, "forcestate"),
		DEFINE_KEYFIELD(m_sGoalEnt, FIELD_STRING, "goalent"), DEFINE_KEYFIELD(m_bGrabAll, FIELD_BOOLEAN, "graball"),
...
void CAI_ScriptedSchedule::ScriptThink(void)
{
	...
	else
	{
		...
		{
			DevMsg(2, "scripted_schedule \"%s\" using NPC \"%s\"(%s)\n", GetDebugName(),
				   pTarget->GetEntityName().ToCStr(), STRING(m_iszEntity));
			StartSchedule(pTarget);
			success = true;
		}
	}
...
{
	CBaseEntity *pEntity = gEntList.FindEntityGenericWithin(m_hLastFoundEntity, STRING(m_iszEntity), GetAbsOrigin(),
															m_flRadius, this, m_hActivator);
	...
	while(pEntity != NULL)
	{
		...
		pEntity =
			gEntList.FindEntityGenericWithin(pEntity, STRING(m_iszEntity), GetAbsOrigin(), m_flRadius, this, NULL);
	}
...
void CAI_ScriptedSchedule::StartSchedule(CAI_BaseNPC *pTarget)
{
	...
	static NPC_STATE forcedStatesMap[] = {NPC_STATE_NONE, NPC_STATE_IDLE, NPC_STATE_ALERT, NPC_STATE_COMBAT};
	...
	switch(m_nSchedule)
	{
		...
		case SCHED_SCRIPT_ENEMY_IS_GOAL_AND_RUN_TO_GOAL:
		{
			...
			bool bIsFlying =
				(pTarget->GetMoveType() == MOVETYPE_FLY) || (pTarget->GetMoveType() == MOVETYPE_FLYGRAVITY);
			if(bIsFlying)
			{
...
BEGIN_DATADESC(CAI_ScriptedSentence)
	...
	DEFINE_KEYFIELD(m_iszSentence, FIELD_STRING, "sentence"), DEFINE_KEYFIELD(m_iszEntity, FIELD_STRING, "entity"),
		DEFINE_KEYFIELD(m_flRadius, FIELD_FLOAT, "radius"), DEFINE_KEYFIELD(m_flDelay, FIELD_FLOAT, "delay"),
		DEFINE_KEYFIELD(m_flRepeat, FIELD_FLOAT, "refire"), DEFINE_KEYFIELD(m_iszListener, FIELD_STRING, "listener"),

		DEFINE_KEYFIELD(m_TempAttenuation, FIELD_INTEGER, "attenuation"),

		DEFINE_FIELD(m_iSoundLevel, FIELD_INTEGER), DEFINE_FIELD(m_flVolume, FIELD_FLOAT),
		DEFINE_FIELD(m_active, FIELD_BOOLEAN), DEFINE_FIELD(m_pActivator, FIELD_EHANDLE),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnBeginSentence, "OnBeginSentence"), DEFINE_OUTPUT(m_OnEndSentence, "OnEndSentence"),
...
CAI_BaseNPC *CAI_ScriptedSentence::FindEntity(void)
{
	...
	for(CEntitySphereQuery sphere(GetAbsOrigin(), m_flRadius, FL_NPC); (pEntity = sphere.GetCurrentEntity()) != NULL;
		sphere.NextEntity())
	{
...
int CAI_ScriptedSentence::StartSentence(CAI_BaseNPC *pTarget)
{
	...
	else if(m_iszListener != NULL_STRING)
	{
		...
		pListener =
			gEntList.FindEntityGenericNearest(STRING(m_iszListener), pTarget->GetAbsOrigin(), radius, this, NULL);
	}

	int sentenceIndex = pTarget->PlayScriptedSentence(STRING(m_iszSentence), m_flDelay, m_flVolume, m_iSoundLevel,
													  bConcurrent, pListener);
