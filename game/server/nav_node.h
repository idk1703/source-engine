...
public:
	...
	CNavNode *GetConnectedNode(
		NavDirType dir) const; ///< get navigation node connected in given direction, or NULL if cant go that way
	...
	void ConnectTo(
		CNavNode *node, NavDirType dir, float obstacleHeight, float flObstacleStartDist,
		float flObstacleEndDist); ///< create a connection FROM this node TO the given node, in the given direction
	...
	float GetGroundHeightAboveNode(NavCornerType cornerType)
		const; ///< return ground height above node in given corner direction (NUM_CORNERS for highest in any direction)
...
private:
	...
	bool TestForCrouchArea(NavCornerType cornerNum, const Vector &mins, const Vector &maxs,
	...
	float m_obstacleHeight[NUM_DIRECTIONS]; ///< obstacle height (delta from nav node z position) that must be climbed
											///< to reach next node in this direction
	...
	unsigned char m_visited; ///< flags for automatic node generation. If direction bit is clear, that direction hasn't
							 ///< been explored yet.
	CNavNode *m_parent; ///< the node prior to this in the search, which we pop back to when this node's search is done
						///< (a stack)
