...
BEGIN_DATADESC(CAI_TrackPather)
	DEFINE_FIELD(m_vecDesiredPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecGoalOrientation, FIELD_VECTOR),

		DEFINE_FIELD(m_pCurrentPathTarget, FIELD_CLASSPTR), DEFINE_FIELD(m_pDestPathTarget, FIELD_CLASSPTR),
		DEFINE_FIELD(m_pLastPathTarget, FIELD_CLASSPTR), DEFINE_FIELD(m_pTargetNearestPath, FIELD_CLASSPTR),

		DEFINE_FIELD(m_strCurrentPathName, FIELD_STRING), DEFINE_FIELD(m_strDestPathName, FIELD_STRING),
		DEFINE_FIELD(m_strLastPathName, FIELD_STRING), DEFINE_FIELD(m_strTargetNearestPathName, FIELD_STRING),

		DEFINE_FIELD(m_vecLastGoalCheckPosition, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_flEnemyPathUpdateTime, FIELD_TIME), DEFINE_FIELD(m_bForcedMove, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bPatrolling, FIELD_BOOLEAN), DEFINE_FIELD(m_bPatrolBreakable, FIELD_BOOLEAN),
		...
		DEFINE_FIELD(m_flTargetTolerance, FIELD_FLOAT), DEFINE_FIELD(m_vecSegmentStartPoint, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vecSegmentStartSplinePoint, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_bMovingForward, FIELD_BOOLEAN), DEFINE_FIELD(m_bChooseFarthestPoint, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flFarthestPathDist, FIELD_FLOAT), DEFINE_FIELD(m_flPathMaxSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_flTargetDistFromPath, FIELD_FLOAT), DEFINE_FIELD(m_flLeadDistance, FIELD_FLOAT),
		DEFINE_FIELD(m_vecTargetPathDir, FIELD_VECTOR), DEFINE_FIELD(m_vecTargetPathPoint, FIELD_POSITION_VECTOR),
...
{
	trace_t tr;
	AI_TraceHull(vecViewPoint, vecTargetPos, -Vector(4, 4, 4), Vector(4, 4, 4), MASK_SHOT, this, COLLISION_GROUP_NONE,
				 &tr);
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CPathTrack *CAI_TrackPather::BestPointOnPath(CPathTrack *pPath, const Vector &targetPos, float flAvoidRadius,
											 bool visible, bool bFarthestPoint)
{
...
//-----------------------------------------------------------------------------
CPathTrack *CAI_TrackPather::ComputeLeadingPointAlongPath(const Vector &vecStartPoint, CPathTrack *pFirstTrack,
...
//-----------------------------------------------------------------------------
float CAI_TrackPather::ComputeDistanceAlongPathToPoint(CPathTrack *pStartTrack, CPathTrack *pDestTrack,
													   const Vector &vecDestPosition, bool bMovingForward)
{
	...
	BEGIN_PATH_TRACK_ITERATION()
		...
		for(; CPathTrack::ValidPath(pTravPath); pTravPath = pNextPath)
		{
			...
			// Find the distance between this test point and our goal point
			flTotalDist +=
				(bMovingForward ? 1.0f : -1.0f) * vecPoint.AsVector2D().DistTo(pTestPath->GetAbsOrigin().AsVector2D());
...
void CAI_TrackPather::VisualizeDebugInfo(const Vector &vecNearestPoint, const Vector &vecTarget)
{
	if(g_debug_trackpather.GetInt() == TRACKPATHER_DEBUG_PATH)
	{
		...
		NDebugOverlay::Cross3D(m_pCurrentPathTarget->GetAbsOrigin(), -Vector(16, 16, 16), Vector(16, 16, 16), 0, 255, 0,
							   true, 0.1f);
		NDebugOverlay::Cross3D(m_vecDesiredPosition, -Vector(16, 16, 16), Vector(16, 16, 16), 0, 0, 255, true, 0.1f);
		NDebugOverlay::Cross3D(m_pDestPathTarget->GetAbsOrigin(), -Vector(16, 16, 16), Vector(16, 16, 16), 255, 255,
							   255, true, 0.1f);

		if(m_pTargetNearestPath)
		{
			NDebugOverlay::Cross3D(m_pTargetNearestPath->GetAbsOrigin(), -Vector(24, 24, 24), Vector(24, 24, 24), 255,
								   0, 255, true, 0.1f);
		}
	}
	...
	if(g_debug_trackpather.GetInt() == TRACKPATHER_DEBUG_TRACKS)
	{
		if(m_pCurrentPathTarget)
		{
			...
			for(; CPathTrack::ValidPath(pPathTrack); pPathTrack = pPathTrack->GetNext())
			{
				...
				{
					NDebugOverlay::Line(pPathTrack->GetAbsOrigin(), pPathTrack->GetNext()->GetAbsOrigin(), 0, 255, 0,
										true, 0.1);
				}
...
{
	return ComputeDistanceAlongPathToPoint(m_pCurrentPathTarget, m_pDestPathTarget, GetDesiredPosition(),
										   m_bMovingForward);
}
...
//-----------------------------------------------------------------------------
float CAI_TrackPather::ComputePerpDistanceFromPath(const Vector &vecPointOnPath, const Vector &vecPathDir,
...
void CAI_TrackPather::ComputePointFromPerpDistance(const Vector &vecPointOnPath, const Vector &vecPathDir,
...
//-----------------------------------------------------------------------------
CPathTrack *CAI_TrackPather::FindClosestPointOnPath(CPathTrack *pPath, const Vector &targetPos,
													Vector *pVecClosestPoint, Vector *pVecPathDir,
	...
	for(int i = 0; i < 2; ++i)
	{
		...
		BEGIN_PATH_TRACK_ITERATION()
			...
			for(; CPathTrack::ValidPath(pTravPath); pTravPath = pNextPath, loopCheck++)
			{
				...
				Vector vecClosest;
				CalcClosestPointOnLineSegment(targetPos, pTravPath->GetAbsOrigin(), pNextPath->GetAbsOrigin(),
											  vecClosest);
...
void CAI_TrackPather::UpdateTargetPositionLeading(void)
{
	...
	CPathTrack *pNextPath = FindClosestPointOnPath(m_pCurrentPathTarget, targetPos, &vecClosestPoint, &vecPathDir,
	...
	//	NDebugOverlay::Cross3D( m_pCurrentPathTarget->GetAbsOrigin(), -Vector(36,36,36), Vector(36,36,36), 255, 0, 0,
	//true, 0.1f ); 	NDebugOverlay::Cross3D( m_pDestPathTarget->GetAbsOrigin(), -Vector(48,48,48), Vector(48,48,48), 0,
	//255, 0, true, 0.1f ); 	NDebugOverlay::Cross3D( targetPos, -Vector(36,36,36), Vector(36,36,36), 0, 0, 255, true,
	//0.1f );
}
...
void CAI_TrackPather::UpdateTargetPosition(void)
{
	...
	// Find the best position to be on our path
	CPathTrack *pDest =
		BestPointOnPath(m_pCurrentPathTarget, targetPos, m_flAvoidDistance, true, m_bChooseFarthestPoint);
...
float CAI_TrackPather::MaxDistanceFromCurrentPath() const
{
	...
	float t;
	Vector vecTemp;
	CalcClosestPointOnLine(GetAbsOrigin(), pPrevPath->GetAbsOrigin(), m_pCurrentPathTarget->GetAbsOrigin(), vecTemp,
						   &t);
	t = clamp(t, 0.0f, 1.0f);
...
float CAI_TrackPather::ClosestPointToCurrentPath(Vector *pVecPoint) const
{
	...
	float t;
	CalcClosestPointOnLine(GetAbsOrigin(), m_vecSegmentStartPoint, m_pCurrentPathTarget->GetAbsOrigin(), *pVecPoint,
						   &t);
	return t;
}
