...
public:
	...
	virtual void ValidateCurrentObserverTarget(
		void); // Checks the current observer target, and moves on if it's not valid anymore
	...
	float GetAutoaimScore(const Vector &eyePosition, const Vector &viewDir, const Vector &vecTarget,
...
public:
	...
	void SetupVPhysicsShadow(const Vector &vecAbsOrigin, const Vector &vecAbsVelocity, CPhysCollide *pStandModel,
	...
	{
		if(Hints())
			Hints()->HintMessage(pMessage);
	}
	void StartHintTimer(int iHintID)
	{
		if(Hints())
			Hints()->StartHintTimer(iHintID);
	}
	void StopHintTimer(int iHintID)
	{
		if(Hints())
			Hints()->StopHintTimer(iHintID);
	}
	void RemoveHintTimer(int iHintID)
	{
		if(Hints())
			Hints()->RemoveHintTimer(iHintID);
	}
	...
	virtual bool IsBot() const; // IMPORTANT: This returns true for ANY type of bot. If your game uses different,
								// incompatible types of bots check your specific bot type before casting
	virtual bool IsBotOfType(
		int botType) const;				// return true if this player is a bot of the specific type (zero is invalid)
	...
	bool SetFOV(CBaseEntity *pRequester, int FOV, float zoomRate = 0.0f,
				int iZoomStart = 0); // Alters the base FOV of the player (must have a valid requester)
	...
	void ActivateMovementConstraint(CBaseEntity *pEntity, const Vector &vecCenter, float flRadius,
...
protected:
	...
	void CalcVehicleView(IServerVehicle *pVehicle, Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar,
						 float &fov);
	...
	CNetworkHandle(
		CBaseEntity,
		m_hUseEntity); // the player is currently controlling this entity because of +USE latched, NULL if no entity
	...
	float m_flDeathAnimTime; // the time at which the player finished their death anim (used in PlayerDeathThink() and
							 // ShouldTransmit())
	...
	CNetworkHandle(CBaseEntity, m_hZoomOwner); // This is a pointer to the entity currently controlling the player's
											   // zoom Only this entity can change the zoom state once it has ownership
...
protected:
	...
	// These are generated while running usercmds, then given to UpdateVPhysicsPosition after running all queued
	// commands.
	...
	Vector m_vecPreviouslyPredictedOrigin; // Used to determine if non-gamemovement game code has teleported, or tweaked
										   // the player's origin
...
public:
	virtual unsigned int PlayerSolidMask(bool brushOnly = false)
		const; // returns the solid mask for the given player, so bots can have a more-restrictive set
};
...
template<typename T>
int CollectHumanPlayers(CUtlVector<T *> *playerVector, int team = TEAM_ANY, bool isAlive = false,
