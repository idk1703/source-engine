...
static ConVar scene_showfaceto("scene_showfaceto", "0", FCVAR_ARCHIVE,
							   "When playing back, show the directions of faceto events.");

BEGIN_DATADESC(CAI_BaseActor)

	DEFINE_FIELD(m_fLatchedPositions, FIELD_INTEGER), DEFINE_FIELD(m_latchedEyeOrigin, FIELD_VECTOR),
		DEFINE_FIELD(m_latchedEyeDirection, FIELD_VECTOR), DEFINE_FIELD(m_latchedHeadDirection, FIELD_VECTOR),
		DEFINE_FIELD(m_goalHeadDirection, FIELD_VECTOR), DEFINE_FIELD(m_goalHeadInfluence, FIELD_FLOAT),
		DEFINE_FIELD(m_goalSpineYaw, FIELD_FLOAT), DEFINE_FIELD(m_goalBodyYaw, FIELD_FLOAT),
		DEFINE_FIELD(m_goalHeadCorrection, FIELD_VECTOR), DEFINE_FIELD(m_flBlinktime, FIELD_TIME),
		DEFINE_FIELD(m_hLookTarget, FIELD_EHANDLE), DEFINE_UTLVECTOR(m_lookQueue, FIELD_EMBEDDED),
		DEFINE_UTLVECTOR(m_randomLookQueue, FIELD_EMBEDDED), DEFINE_UTLVECTOR(m_syntheticLookQueue, FIELD_EMBEDDED),
		DEFINE_FIELD(m_flNextRandomLookTime, FIELD_TIME), DEFINE_FIELD(m_iszExpressionScene, FIELD_STRING),
		DEFINE_FIELD(m_hExpressionSceneEnt, FIELD_EHANDLE), DEFINE_FIELD(m_flNextRandomExpressionTime, FIELD_TIME),
		DEFINE_FIELD(m_iszIdleExpression, FIELD_STRING), DEFINE_FIELD(m_iszAlertExpression, FIELD_STRING),
		DEFINE_FIELD(m_iszCombatExpression, FIELD_STRING), DEFINE_FIELD(m_iszDeathExpression, FIELD_STRING),
		...
		DEFINE_FIELD(m_ParameterNeckTrans, FIELD_INTEGER), DEFINE_FIELD(m_ParameterHeadYaw, FIELD_INTEGER),
		DEFINE_FIELD(m_ParameterHeadPitch, FIELD_INTEGER), DEFINE_FIELD(m_ParameterHeadRoll, FIELD_INTEGER),
		...
		DEFINE_FIELD(m_FlexweightHeadRightLeft, FIELD_INTEGER), DEFINE_FIELD(m_FlexweightHeadUpDown, FIELD_INTEGER),
		DEFINE_FIELD(m_FlexweightHeadTilt, FIELD_INTEGER),

		DEFINE_FIELD(m_ParameterGestureHeight, FIELD_INTEGER), DEFINE_FIELD(m_ParameterGestureWidth, FIELD_INTEGER),
		DEFINE_FIELD(m_FlexweightGestureUpDown, FIELD_INTEGER),
		DEFINE_FIELD(m_FlexweightGestureRightLeft, FIELD_INTEGER), DEFINE_FIELD(m_flAccumYawDelta, FIELD_FLOAT),
...
BEGIN_SIMPLE_DATADESC(CAI_InterestTarget_t)
	DEFINE_FIELD(m_eType, FIELD_INTEGER), DEFINE_FIELD(m_hTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_vecPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_flStartTime, FIELD_TIME),
		DEFINE_FIELD(m_flEndTime, FIELD_TIME), DEFINE_FIELD(m_flRamp, FIELD_FLOAT),
...
static char const *g_ServerSideFlexControllers[] = {"body_rightleft",
													//"body_updown",
													//"body_tilt",
													"chest_rightleft",
													//"chest_updown",
...
bool CAI_BaseActor::StartSceneEvent(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event,
...
bool CAI_BaseActor::ProcessSceneEvent(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event)
{
	...
	switch(info->m_pEvent->GetType())
	{
		case CChoreoEvent::FACE:
		{
			...
			NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 8 ), GetLocalAngles().y + m_goalBodyYaw, 64, 16,
			255, 128, 128, 0, true, 0.1 ); NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 16 ),
			GetLocalAngles().y + m_goalSpineYaw, 64, 16, 128, 255, 128, 0, true, 0.1 ); NDebugOverlay::YawArrow(
			GetAbsOrigin() + Vector( 0, 0, 24 ), info->m_flTargetYaw, 64, 16, 128, 128, 255, 0, true, 0.1 );
			*/
			...
			{
				NDebugOverlay::YawArrow(GetAbsOrigin() + Vector(0, 0, 1), goalYaw, 8 + 32 * intensity, 8, 255, 255, 255,
										0, true, 0.12);
			}
					...
					if(info->m_flNext < gpGlobals->curtime)
					{
						...
						if(m_syntheticLookQueue.Count() > 0)
						{
							...
							m_syntheticLookQueue[i].m_flEndTime =
								MIN(m_syntheticLookQueue[i].m_flEndTime, gpGlobals->curtime + flDuration);
...
bool CAI_BaseActor::CheckSceneEventCompletion(CSceneEventInfo *info, float currenttime, CChoreoScene *scene,
											  CChoreoEvent *event)
{
	Assert(info);
	Assert(info->m_pScene);
		...
		case CChoreoEvent::GENERIC:
		{
			switch(info->m_nType)
			{
				...
				case SCENE_AI_UNHOLSTER:
				{
					...
					float t =
						(1.0 - GetLayerCycle(info->m_iLayer)) * SequenceDuration(GetLayerSequence(info->m_iLayer));
...
static ConVar scene_clamplookat("scene_clamplookat", "1", FCVAR_NONE,
...
void CAI_BaseActor::UpdateHeadControl(const Vector &vHeadTarget, float flHeadInfluence)
{
	...
	m_goalHeadCorrection.x =
		UTIL_Approach(m_goalHeadCorrection.x * s0 + vTargetAngles.x * s1, m_goalHeadCorrection.x, 10.0);
	m_goalHeadCorrection.y =
		UTIL_Approach(m_goalHeadCorrection.y * s0 + vTargetAngles.y * s1, m_goalHeadCorrection.y, 30.0);
	m_goalHeadCorrection.z =
		UTIL_Approach(m_goalHeadCorrection.z * s0 + vTargetAngles.z * s1, m_goalHeadCorrection.z, 10.0);

	/*
	if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
	{
		// Msg( "yaw %.1f (%f) pitch %.1f (%.1f)\n", m_goalHeadCorrection.y, vTargetAngles.y, vTargetAngles.x,
	m_goalHeadCorrection.x );
		// Msg( "yaw %.2f (goal %.2f) (influence %.2f) (flex %.2f)\n", flLimit, m_goalHeadCorrection.y, flHeadInfluence,
	Get( m_FlexweightHeadRightLeft ) );
	}
	*/
	...
	if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
	{
		Msg( "yaw  %5.1f : (%5.1f : %5.1f) %5.1f (%5.1f)\n", flLimit, m_goalHeadCorrection.y, Get(
	m_FlexweightHeadRightLeft ), angBias.y, Get( m_ParameterHeadYaw ) );
	}
	*/
	...
	if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
	{
		Msg( "pitch %5.1f : (%5.1f : %5.1f) %5.1f (%5.1f)\n", flLimit, m_goalHeadCorrection.x, Get(
	m_FlexweightHeadUpDown ), angBias.x, Get( m_ParameterHeadPitch ) );
	}
	*/
	...
	if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
	{
		Msg( "roll  %5.1f : (%5.1f : %5.1f) %5.1f (%5.1f)\n", flLimit, m_goalHeadCorrection.z, Get( m_FlexweightHeadTilt
	), angBias.z, Get( m_ParameterHeadRoll ) );
	}
	*/
...
bool CAI_BaseActor::PickRandomLookTarget(AILookTargetArgs_t *pArgs)
{
	...
	int iConsidered = 0;
	for(CEntitySphereQuery sphere(GetAbsOrigin(), 30 * 12, 0); (pEntity = sphere.GetCurrentEntity()) != NULL;
		sphere.NextEntity())
	{
		if(pEntity == this)
		{
...
void CAI_BaseActor::MakeRandomLookTarget(AILookTargetArgs_t *pArgs, float minTime, float maxTime)
{
	...
	{
		pArgs->vTarget =
			EyePosition() + forward * 2048 + right * random->RandomFloat(-650, 650) + up * random->RandomFloat(-32, 32);
	}
	else
#endif // HL2_EPISODIC
	{
		pArgs->vTarget =
			EyePosition() + forward * 128 + right * random->RandomFloat(-32, 32) + up * random->RandomFloat(-16, 16);
	}
...
void CAI_BaseActor::MaintainLookTargets(float flInterval)
{
	...
	for(i = 0; i < active.Count(); i++)
	{
		...
		if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
		{
			DevMsg( "head (%d) %.2f : %s : %.1f %.1f %.1f\n", i, flInterest, active[i]->m_hTarget->GetClassname(),
		active[i]->GetPosition().x, active[i]->GetPosition().y, active[i]->GetPosition().z );
		}
		*/
	...
	if(m_hLookTarget != hTarget)
	{
		...
		if((m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) && ai_debug_looktargets.GetInt() == 2 && m_hLookTarget.Get())
		{
			if(m_hLookTarget != this)
			{
				...
				NDebugOverlay::Text(
					vecEyePos, UTIL_VarArgs("%s (%s)", m_hLookTarget->GetClassname(), m_hLookTarget->GetDebugName()),
					false, 20);
			}
		}
	...
	if(!bFoundTarget && !ValidEyeTarget(GetViewtarget()))
	{
		...
		// DevMsg("random view\n");
		SetViewtarget(EyePosition() + HeadDirection3D() * 128 + right * random->RandomFloat(-32, 32) +
