...
void CBaseProp::Spawn(void)
{
	...
	if(!OverridePropdata())
	{
		...
		else if(iResult == PARSE_FAILED_NO_DATA)
		{
			...
			{
				DevWarning("%s at %.0f %.0f %0.f uses model %s, which has no propdata which means it must be used on a "
						   "prop_static. DELETED.\n",
						   GetClassname(), GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z, szModel);
				UTIL_Remove(this);
				return;
		...
		else if(iResult == PARSE_SUCCEEDED)
		{
			...
			{
				DevWarning("%s at %.0f %.0f %0.f uses model %s, which has propdata which means that it be used on a "
...
void CPhysicsProp::HandleAnyCollisionInteractions(int index, gamevcollisionevent_t *pEvent)
{
	...
	if(HasInteraction(PROPINTER_PHYSGUN_FIRST_IMPALE))
	{
		...
		else if(pHitEntity->MyNPCPointer())
		{
			...
			trace_t tr;
			UTIL_TraceLine(vecPos, vecPos + pEvent->preVelocity[index] * 1.5, MASK_SHOT, this, COLLISION_GROUP_NONE,
						   &tr);
			...
			CBaseEntity *pRagdoll =
				CreateServerRagdollAttached(pNPC, vec3_origin, -1, COLLISION_GROUP_INTERACTIVE_DEBRIS, pObj, this, 0,
											vecImpalePos, iBone, vec3_origin);
			if(pRagdoll)
			{
...
()
	...
	DEFINE_KEYFIELD(m_iMinHealthDmg, FIELD_INTEGER, "minhealthdmg"), DEFINE_FIELD(m_createTick, FIELD_INTEGER),
	DEFINE_FIELD(m_hBreaker, FIELD_EHANDLE), DEFINE_KEYFIELD(m_PerformanceMode, FIELD_INTEGER, "PerformanceMode"),

	DEFINE_KEYFIELD(m_iszBreakModelMessage, FIELD_STRING, "BreakModelMessage"),

	DEFINE_FIELD(m_flDmgModBullet, FIELD_FLOAT), DEFINE_FIELD(m_flDmgModClub, FIELD_FLOAT),
	DEFINE_FIELD(m_flDmgModExplosive, FIELD_FLOAT), DEFINE_FIELD(m_iszPhysicsDamageTableName, FIELD_STRING),
	DEFINE_FIELD(m_iszBreakableModel, FIELD_STRING), DEFINE_FIELD(m_iBreakableSkin, FIELD_INTEGER),
	DEFINE_FIELD(m_iBreakableCount, FIELD_INTEGER), DEFINE_FIELD(m_iMaxBreakableSize, FIELD_INTEGER),
	DEFINE_FIELD(m_iszBasePropData, FIELD_STRING), DEFINE_FIELD(m_iInteractions, FIELD_INTEGER),
	DEFINE_FIELD(m_iNumBreakableChunks, FIELD_INTEGER), DEFINE_FIELD(m_nPhysgunState, FIELD_CHARACTER),
	DEFINE_KEYFIELD(m_iszPuntSound, FIELD_STRING, "puntsound"),

	DEFINE_KEYFIELD(m_flPressureDelay, FIELD_FLOAT, "PressureDelay"),
	DEFINE_FIELD(m_preferredCarryAngles, FIELD_VECTOR), DEFINE_FIELD(m_flDefaultFadeScale, FIELD_FLOAT),
	DEFINE_FIELD(m_bUsePuntSound, FIELD_BOOLEAN),
	// DEFINE_FIELD( m_mpBreakMode, mp_break_t ),

	// Inputs
	DEFINE_INPUTFUNC(FIELD_VOID, "Break", InputBreak), DEFINE_INPUTFUNC(FIELD_INTEGER, "SetHealth", InputSetHealth),
	...
	// Outputs
	DEFINE_OUTPUT(m_OnBreak, "OnBreak"), DEFINE_OUTPUT(m_OnHealthChanged, "OnHealthChanged"),
	DEFINE_OUTPUT(m_OnTakeDamage, "OnTakeDamage"), DEFINE_OUTPUT(m_OnPhysCannonDetach, "OnPhysCannonDetach"),
	...
	// Function Pointers
	DEFINE_THINKFUNC(BreakThink), DEFINE_THINKFUNC(AnimateThink), DEFINE_THINKFUNC(RampToDefaultFadeScale),
	...
	// Damage
	DEFINE_FIELD(m_hLastAttacker, FIELD_EHANDLE), DEFINE_FIELD(m_hFlareEnt, FIELD_EHANDLE),
...
void CBreakableProp::Spawn()
{
	...
	if((m_iHealth == 0) ||
	   (!m_iNumBreakableChunks && !HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE) &&
		!HasInteraction(PROPINTER_PHYSGUN_FIRST_BREAK) && !HasInteraction(PROPINTER_FIRE_FLAMMABLE) &&
		!HasInteraction(PROPINTER_FIRE_IGNITE_HALFHEALTH) && !HasInteraction(PROPINTER_FIRE_EXPLOSIVE_RESIST)))
	{
		m_iHealth = 0;
	...
	else
	{
		...
		{
			if(HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE) || HasInteraction(PROPINTER_FIRE_IGNITE_HALFHEALTH))
			{
...
int CBreakableProp::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
	...
	if(bDeadly && (info.GetDamageType() & DMG_BLAST) && HasInteraction(PROPINTER_FIRE_EXPLOSIVE_RESIST) &&
	   info.GetInflictor())
	{
...
void CBreakableProp::Break(CBaseEntity *pBreaker, const CTakeDamageInfo &info)
{
	...
	if(gpGlobals->maxClients > 1 && breakable_multiplayer.GetBool())
	{
		...
		switch(GetMultiplayerBreakMode())
		{
			...
			case MULTIPLAYER_BREAK_CLIENTSIDE:
				te->PhysicsProp(filter, -1, GetModelIndex(), m_nSkin, GetAbsOrigin(), GetAbsAngles(), velocity, true,
								GetEffects());
				break;
	...
	if(HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE))
	{
		...
		for(CEntitySphereQuery sphere(origin, m_explodeRadius); (pEntity = sphere.GetCurrentEntity()) != NULL;
			sphere.NextEntity())
		{
...
BEGIN_DATADESC(CDynamicProp)
	...
	// Fields
	DEFINE_KEYFIELD(m_iszDefaultAnim, FIELD_STRING, "DefaultAnim"), DEFINE_FIELD(m_iGoalSequence, FIELD_INTEGER),
		DEFINE_FIELD(m_iTransitionDirection, FIELD_INTEGER),
		DEFINE_KEYFIELD(m_bRandomAnimator, FIELD_BOOLEAN, "RandomAnimation"),
		DEFINE_FIELD(m_flNextRandAnim, FIELD_TIME), DEFINE_KEYFIELD(m_flMinRandAnimTime, FIELD_FLOAT, "MinAnimTime"),
		...
		DEFINE_FIELD(m_bUseHitboxesForRenderBox, FIELD_BOOLEAN), DEFINE_FIELD(m_nPendingSequence, FIELD_SHORT),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputTurnOn), DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputTurnOff),
		...
		// Outputs
		DEFINE_OUTPUT(m_pOutputAnimBegun, "OnAnimationBegun"), DEFINE_OUTPUT(m_pOutputAnimOver, "OnAnimationDone"),
...
void CDynamicProp::Spawn()
{
	...
	if(pszModelName && pszModelName[0])
	{
		if(FStrEq(pszModelName, "models/bots/boss_bot/carrier_parts.mdl"))
		{
			...
			SetModelIndexOverride(VISION_MODE_ROME,
								  modelinfo->GetModelIndex("models/bots/tw2/boss_bot/twcarrier_addon.mdl"));
		}
	}
#endif
}
...
// Call this when creating bone followers or changing hierarchy to make sure the bone followers get updated when
// hierarchy changes
...
void CDynamicProp::AnimThink(void)
{
	...
	if(((m_iTransitionDirection > 0 && GetCycle() >= 0.999f) || (m_iTransitionDirection < 0 && GetCycle() <= 0.0f)) &&
	   !SequenceLoops())
	{
...
void CDynamicProp::PropSetSequence(int nSequence)
{
	...
	// Msg("%.2f CDynamicProp::PropSetSequence( %d (%d:%.1f:%.3f)\n", gpGlobals->curtime, nSequence, GetSequence(),
	// GetPlaybackRate(), GetCycle() );

	int nNextSequence;
	float nextCycle;
	float flInterval = 0.1f;

	if(GotoSequence(GetSequence(), GetCycle(), GetPlaybackRate(), m_iGoalSequence, nNextSequence, nextCycle,
...
BEGIN_DATADESC(CPhysicsProp)
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "EnableMotion", InputEnableMotion),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Wake", InputWake), DEFINE_INPUTFUNC(FIELD_VOID, "Sleep", InputSleep),
		...
		DEFINE_OUTPUT(m_OnAwakened, "OnAwakened"), DEFINE_OUTPUT(m_MotionEnabled, "OnMotionEnabled"),
		DEFINE_OUTPUT(m_OnPhysGunPickup, "OnPhysGunPickup"),
		DEFINE_OUTPUT(m_OnPhysGunOnlyPickup, "OnPhysGunOnlyPickup"), DEFINE_OUTPUT(m_OnPhysGunPunt, "OnPhysGunPunt"),
		DEFINE_OUTPUT(m_OnPhysGunDrop, "OnPhysGunDrop"), DEFINE_OUTPUT(m_OnPlayerUse, "OnPlayerUse"),
		DEFINE_OUTPUT(m_OnPlayerPickup, "OnPlayerPickup"), DEFINE_OUTPUT(m_OnOutOfWorld, "OnOutOfWorld"),

		DEFINE_FIELD(m_bThrownByPlayer, FIELD_BOOLEAN), DEFINE_FIELD(m_bFirstCollisionAfterLaunch, FIELD_BOOLEAN),
...
void CPhysicsProp::Spawn()
{
	...
	{
		SetCollisionGroup(HasSpawnFlags(SF_PHYSPROP_FORCE_TOUCH_TRIGGERS) ? COLLISION_GROUP_DEBRIS_TRIGGER
	...
	// Episode 1 change:
	// Hi, since we're trying to ship this game we'll just go ahead and make all these doors not fade out instead of
	// changing all the levels.
	...
	if(!HasSpawnFlags(SF_PHYSPROP_DONT_TAKE_PHYSICS_DAMAGE))
	{
		...
		if(damage > 0)
		{
			...
			PhysCallbackDamage(this,
							   CTakeDamageInfo(pHitEntity, pHitEntity, damageForce, damagePos, damage, damageType),
							   *pEvent, index);
		}
	}
	...
	if(IsOnFire())
	{
		if((info.GetDamageType() & DMG_BURN) && (info.GetDamageType() & DMG_DIRECT))
		{
			...
			if(VPhysicsGetObject())
			{
				...
				trace_t tr;
				UTIL_TraceLine(WorldSpaceCenter(), WorldSpaceCenter() + vel, MASK_SHOT, this, COLLISION_GROUP_NONE,
							   &tr);
...
int CPhysicsProp::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		{
			char tempstr[512];
			Q_snprintf(tempstr, sizeof(tempstr), "Mass: %.2f kg / %.2f lb (%s)", VPhysicsGetObject()->GetMass(),
					   kg2lbs(VPhysicsGetObject()->GetMass()), GetMassEquivalent(VPhysicsGetObject()->GetMass()));
...

			{
				...
				Q_snprintf(tempstr, sizeof(tempstr), "Stress: %.2f (%.2f / %.2f)", stress, stressOut.exertedStress,
			...
			{
				IBreakableWithPropData *pBreakableInterface = assert_cast<IBreakableWithPropData *>(this);
				Q_snprintf(tempstr, sizeof(tempstr), "Breakable Chunks: %d (Max Size %d)", m_iNumBreakableChunks,
...
static CBreakableProp *BreakModelCreate_Prop(CBaseEntity *pOwner, breakmodel_t *pModel, const Vector &position,
...
static CBaseAnimating *BreakModelCreate_Ragdoll(CBaseEntity *pOwner, breakmodel_t *pModel, const Vector &position,
												const QAngle &angles)
{
	CBaseAnimating *pAnimating = CreateServerRagdollSubmodel(dynamic_cast<CBaseAnimating *>(pOwner), pModel->modelName,
															 position, angles, pModel->collisionGroup);
	return pAnimating;
}
...
bool PropBreakableCapEdictsOnCreateAll(int modelindex, IPhysicsObject *pPhysics, const breakablepropparams_t &params,
									   CBaseEntity *pEntity, int iPrecomputedBreakableCount = -1)
{
	...
	else
	{
		...
		else if(pEntity)
		{
			...
			if(pBreakableInterface && pBreakableInterface->GetBreakableModel() != NULL_STRING &&
...
BEGIN_DATADESC_NO_BASE(locksound_t)
	...
	DEFINE_FIELD(sLockedSound, FIELD_STRING), DEFINE_FIELD(sLockedSentence, FIELD_STRING),
		DEFINE_FIELD(sUnlockedSound, FIELD_STRING), DEFINE_FIELD(sUnlockedSentence, FIELD_STRING),
		DEFINE_FIELD(iLockedSentence, FIELD_INTEGER), DEFINE_FIELD(iUnlockedSentence, FIELD_INTEGER),
		DEFINE_FIELD(flwaitSound, FIELD_FLOAT), DEFINE_FIELD(flwaitSentence, FIELD_FLOAT),
		DEFINE_FIELD(bEOFLocked, FIELD_CHARACTER), DEFINE_FIELD(bEOFUnlocked, FIELD_CHARACTER),
...
BEGIN_DATADESC(CBasePropDoor)
	...
	DEFINE_KEYFIELD(m_nHardwareType, FIELD_INTEGER, "hardware"),
		DEFINE_KEYFIELD(m_flAutoReturnDelay, FIELD_FLOAT, "returndelay"), DEFINE_FIELD(m_hActivator, FIELD_EHANDLE),
		...
		DEFINE_KEYFIELD(m_SlaveName, FIELD_STRING, "slavename"), DEFINE_FIELD(m_bLocked, FIELD_BOOLEAN),
		// DEFINE_KEYFIELD(m_flBlockDamage, FIELD_FLOAT, "dmg"),
		DEFINE_KEYFIELD(m_bForceClosed, FIELD_BOOLEAN, "forceclosed"), DEFINE_FIELD(m_eDoorState, FIELD_INTEGER),
		DEFINE_FIELD(m_hMaster, FIELD_EHANDLE), DEFINE_FIELD(m_hBlocker, FIELD_EHANDLE),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Close", InputClose), DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle),
		DEFINE_INPUTFUNC(FIELD_VOID, "Lock", InputLock), DEFINE_INPUTFUNC(FIELD_VOID, "Unlock", InputUnlock),

		DEFINE_OUTPUT(m_OnBlockedOpening, "OnBlockedOpening"), DEFINE_OUTPUT(m_OnBlockedClosing, "OnBlockedClosing"),
		DEFINE_OUTPUT(m_OnUnblockedOpening, "OnUnblockedOpening"),
		DEFINE_OUTPUT(m_OnUnblockedClosing, "OnUnblockedClosing"), DEFINE_OUTPUT(m_OnFullyClosed, "OnFullyClosed"),
		DEFINE_OUTPUT(m_OnFullyOpen, "OnFullyOpen"), DEFINE_OUTPUT(m_OnClose, "OnClose"),
		DEFINE_OUTPUT(m_OnOpen, "OnOpen"), DEFINE_OUTPUT(m_OnLockedUse, "OnLockedUse"), DEFINE_EMBEDDED(m_ls),

		// Function Pointers
		DEFINE_THINKFUNC(DoorOpenMoveDone), DEFINE_THINKFUNC(DoorCloseMoveDone), DEFINE_THINKFUNC(DoorAutoCloseThink),
END_DATADESC()
...
void CBasePropDoor::Spawn()
{
	...
	if((m_nHardwareType == 0) && (!HasSpawnFlags(SF_DOOR_LOCKED)))
	{
		...
		DevWarning(
			1, "Unlocked prop_door '%s' at (%.0f %.0f %.0f) has no hardware. All openable doors must have hardware!\n",
...
void CBasePropDoor::CalcDoorSounds()
{
	...
	{
		Warning("%s has Door model (%s) with no door_options! Verify that SKIN is valid, and has a corresponding "
				"options block in the model QC file\n",
...
void CBasePropDoor::InputOpenAwayFrom(inputdata_t &inputdata)
{
	CBaseEntity *pOpenAwayFrom =
		gEntList.FindEntityByName(NULL, inputdata.value.String(), NULL, inputdata.pActivator, inputdata.pCaller);
...
void CBasePropDoor::Blocked(CBaseEntity *pOther)
{
	...
	//					{
	//						if (pDoor->GetAbsVelocity() == GetAbsVelocity() && pDoor->GetLocalAngularVelocity() ==
...
inline void TraceHull_Door(const CBasePropDoor *pDoor, const Vector &vecAbsStart, const Vector &vecAbsEnd,
						   const Vector &hullMin, const Vector &hullMax, unsigned int mask, const CBaseEntity *ignore,
...
BEGIN_DATADESC(CPropDoorRotating)
	DEFINE_KEYFIELD(m_eSpawnPosition, FIELD_INTEGER, "spawnpos"),
		DEFINE_KEYFIELD(m_eOpenDirection, FIELD_INTEGER, "opendir"), DEFINE_KEYFIELD(m_vecAxis, FIELD_VECTOR, "axis"),
		DEFINE_KEYFIELD(m_flDistance, FIELD_FLOAT, "distance"),
		DEFINE_KEYFIELD(m_angRotationAjar, FIELD_VECTOR, "ajarangles"), DEFINE_FIELD(m_angRotationClosed, FIELD_VECTOR),
		DEFINE_FIELD(m_angRotationOpenForward, FIELD_VECTOR), DEFINE_FIELD(m_angRotationOpenBack, FIELD_VECTOR),
		DEFINE_FIELD(m_angGoal, FIELD_VECTOR), DEFINE_FIELD(m_hDoorBlocker, FIELD_EHANDLE),
...
//			*destMaxs -
//-----------------------------------------------------------------------------
void UTIL_ComputeAABBForBounds(const Vector &mins1, const Vector &maxs1, const Vector &mins2, const Vector &maxs2,
...
void CPropDoorRotating::OnDoorOpened(void)
{
	if(m_hDoorBlocker != NULL)
	{
		...
		{
			NDebugOverlay::Box(GetAbsOrigin(), m_hDoorBlocker->CollisionProp()->OBBMins(),
...
bool CPropDoorRotating::CheckDoorClear(doorCheck_e state)
{
	...
	switch(state)
	{
		...
		case DOOR_CHECK_FULL:
			UTIL_ComputeAABBForBounds(m_vecForwardBoundsMin, m_vecForwardBoundsMax, m_vecBackBoundsMin,
									  m_vecBackBoundsMax, &moveMins, &moveMaxs);
			break;
	}

	// Look for blocking entities, ignoring ourselves and the entity that opened us.
	trace_t tr;
	TraceHull_Door(this, GetAbsOrigin(), GetAbsOrigin(), moveMins, moveMaxs, MASK_SOLID, GetActivator(),
	...
	if(tr.allsolid || tr.startsolid)
	{
		if(g_debug_doors.GetBool())
		{
			...
			if(tr.m_pEnt)
			{
				NDebugOverlay::Box(tr.m_pEnt->GetAbsOrigin(), tr.m_pEnt->CollisionProp()->OBBMins(),
...
void CPropDoorRotating::BeginOpening(CBaseEntity *pOpenAwayFrom)
{
	...
	if(g_debug_doors.GetBool())
	{
		NDebugOverlay::Cross3D(volumeCenter, -Vector(volumeRadius, volumeRadius, volumeRadius),
							   Vector(volumeRadius, volumeRadius, volumeRadius), 255, 0, 0, true, 1.0f);
	}
...
int CPropDoorRotating::DrawDebugTextOverlays(void)
{
	...
	{
		char tempstr[512];
		Q_snprintf(tempstr, sizeof(tempstr), "Avelocity: %.2f %.2f %.2f", GetLocalAngularVelocity().x,
				   GetLocalAngularVelocity().y, GetLocalAngularVelocity().z);
		EntityText(text_offset, tempstr, 0);
		text_offset++;
...
public:
	...
	bool CreateVPhysics()
	{
		...
		IPhysicsObject *pPhysicsObject =
			physenv->CreateSphereObject(12, 0, GetAbsOrigin(), GetAbsAngles(), &params, false);
		if(pPhysicsObject)
		{
...
SendPropInt(SENDINFO(m_iPhysicsMode), 1, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_fMass), 0, SPROP_NOSCALE),
END_SEND_TABLE
()
									...
									public IMultiplayerPhysics
{
	...
	virtual void Spawn(void)
	{
		...
		if(m_iPhysicsMode == PHYSICS_MULTIPLAYER_AUTODETECT)
		{
			...
			{
				m_iPhysicsMode =
					GetAutoMultiplayerPhysicsMode(CollisionProp()->OBBSize(), VPhysicsGetObject()->GetMass());
			}
			else
			{
				UTIL_Remove(this);
		...
		if((GetSolid() == SOLID_VPHYSICS) && (GetMoveType() == MOVETYPE_VPHYSICS))
		{
			...
			{
				physcollision->CollideGetAABB(&m_collisionMins.GetForModify(), &m_collisionMaxs.GetForModify(),
...
BEGIN_DATADESC(CPhysicsPropMultiplayer)
	DEFINE_KEYFIELD(m_iPhysicsMode, FIELD_INTEGER, "physicsmode"), DEFINE_FIELD(m_fMass, FIELD_FLOAT),
		DEFINE_FIELD(m_usingCustomCollisionBounds, FIELD_BOOLEAN), DEFINE_FIELD(m_collisionMins, FIELD_VECTOR),
		DEFINE_FIELD(m_collisionMaxs, FIELD_VECTOR),
END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CPhysicsPropMultiplayer, DT_PhysicsPropMultiplayer)
SendPropInt(SENDINFO(m_iPhysicsMode), 2, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_fMass), 0, SPROP_NOSCALE),
...
BEGIN_DATADESC(CPhysicsPropRespawnable)
	DEFINE_THINKFUNC(Materialize), DEFINE_KEYFIELD(m_flRespawnTime, FIELD_FLOAT, "RespawnTime"),
		DEFINE_FIELD(m_vOriginalSpawnOrigin, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vOriginalSpawnAngles, FIELD_VECTOR),
		DEFINE_FIELD(m_vOriginalMins, FIELD_VECTOR), DEFINE_FIELD(m_vOriginalMaxs, FIELD_VECTOR),
END_DATADESC()
...
{
	trace_t tr;
	UTIL_TraceHull(m_vOriginalSpawnOrigin, m_vOriginalSpawnOrigin, m_vOriginalMins, m_vOriginalMaxs, MASK_SOLID, this,
				   COLLISION_GROUP_NONE, &tr);
...
void CC_Prop_Dynamic_Create(const CCommand &args)
{
	...
	trace_t tr;
	UTIL_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH, MASK_NPCSOLID, pPlayer,
				   COLLISION_GROUP_NONE, &tr);
...
static ConCommand prop_dynamic_create("prop_dynamic_create", CC_Prop_Dynamic_Create,
									  "Creates a dynamic prop with a specific .mdl aimed away from where the player is "
									  "looking.\n\tArguments: {.mdl name}",
									  FCVAR_CHEAT);
...
void CC_Prop_Physics_Create(const CCommand &args)
{
	...
	CreatePhysicsProp(pModelName, pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH, pPlayer,
					  true);
}

static ConCommand prop_physics_create("prop_physics_create", CC_Prop_Physics_Create,
									  "Creates a physics prop with a specific .mdl aimed away from where the player is "
									  "looking.\n\tArguments: {.mdl name}",
									  FCVAR_CHEAT);

CPhysicsProp *CreatePhysicsProp(const char *pModelName, const Vector &vTraceStart, const Vector &vTraceEnd,
								const IHandleEntity *pTraceIgnore, bool bRequireVCollide, const char *pClassName)
{
	MDLCACHE_CRITICAL_SECTION();
	...
	trace_t tr;
	UTIL_TraceHull(vTraceStart, vTraceEnd, vecSweepMins, vecSweepMaxs, MASK_NPCSOLID, pTraceIgnore,
				   COLLISION_GROUP_NONE, &tr);
...
bool UTIL_CreateScaledPhysObject(CBaseAnimating *pInstance, float flScale)
{
	...
	if(flScale != 1.0f)
	{
		...
		ICollisionQuery *pQuery =
			physcollision->CreateQueryModel(pCollide->solids[0]); // FIXME: This should iterate over all solids!
		if(pQuery == NULL)
			return false;
	...
	if(bWasStatic)
	{
		pNewObject = physenv->CreatePolyObjectStatic(pNewCollide, surfaceProp, pInstance->GetAbsOrigin(),
													 pInstance->GetAbsAngles(), &tmpSolid.params);
	}
	else
	{
		pNewObject = physenv->CreatePolyObject(pNewCollide, surfaceProp, pInstance->GetAbsOrigin(),
											   pInstance->GetAbsAngles(), &tmpSolid.params);
	}
	Assert(pNewObject);
