...
BEGIN_DATADESC(CEnvExplosion)
	...
	DEFINE_KEYFIELD(m_iMagnitude, FIELD_INTEGER, "iMagnitude"),
		...
		DEFINE_FIELD(m_spriteScale, FIELD_INTEGER), DEFINE_KEYFIELD(m_flDamageForce, FIELD_FLOAT, "DamageForce"),
		DEFINE_FIELD(m_iszFireballSprite, FIELD_STRING), DEFINE_FIELD(m_sFireballSprite, FIELD_SHORT),
		DEFINE_FIELD(m_hInflictor, FIELD_EHANDLE), DEFINE_FIELD(m_iCustomDamageType, FIELD_INTEGER),

		DEFINE_FIELD(m_iClassIgnore, FIELD_INTEGER), DEFINE_KEYFIELD(m_hEntityIgnore, FIELD_EHANDLE, "ignoredEntity"),
...
void CEnvExplosion::InputExplode(inputdata_t &inputdata)
{
	...
	UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -40), (MASK_SOLID_BRUSHONLY | MASK_WATER), this,
				   COLLISION_GROUP_NONE, &tr);

	// Pull out of the wall a bit. We used to move the explosion origin itself, but that seems unnecessary, not to
	// mention a little weird when you consider that it might be in hierarchy. Instead we just calculate a new virtual
	// position at which to place the explosion. We don't use that new position to calculate radius damage because
	// according to Steve's comment, that adversely affects the force vector imparted on explosion victims when they
	// ragdoll.
	...
	te->Explosion(filter, 0.0, &vecExplodeOrigin, (m_sFireballSprite < 1) ? g_sModelIndexFireball : m_sFireballSprite,
				  !(m_spawnflags & SF_ENVEXPLOSION_NOFIREBALL) ? (m_spriteScale / 10.0) : 0.0, 15, nFlags, iRadius,
				  m_iMagnitude);
...
void ExplosionCreate(const Vector &center, const QAngle &angles, CBaseEntity *pOwner, int magnitude, int radius,
					 int nSpawnFlags, float flExplosionForce, CBaseEntity *pInflictor, int iCustomDamageType,
...
void ExplosionCreate(const Vector &center, const QAngle &angles, CBaseEntity *pOwner, int magnitude, int radius,
					 bool doDamage, float flExplosionForce, bool bSurfaceOnly, bool bSilent, int iCustomDamageType)
{
	// For E3, no sparks
...
void ExplosionCreate(const Vector &center, const QAngle &angles, CBaseEntity *pOwner, int magnitude, int radius,
					 bool doDamage, const EHANDLE *ignoredEntity, Class_T ignoredClass, float flExplosionForce,
					 bool bSurfaceOnly, bool bSilent, int iCustomDamageType)
{
	// For E3, no sparks
	int nFlags = SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE;
	if(!doDamage)
