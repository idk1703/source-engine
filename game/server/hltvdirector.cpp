...
static ConVar tv_allow_static_shots("tv_allow_static_shots", "1", 0,
									"Auto director uses fixed level cameras for shots");
static ConVar tv_allow_camera_man("tv_allow_camera_man", "1", 0,
...
{
	static const char *s_modevents[] = {"hltv_status", "hltv_chat",	  "player_connect", "player_disconnect",
...
void CHLTVDirector::FrameUpdatePostEntityThink(void)
{
	...
	if((m_nNextAnalyzeTick < gpGlobals->tickcount) && (m_fDelay >= HLTV_MIN_DIRECTOR_DELAY))
	{
...
void CHLTVDirector::CreateShotFromEvent(CHLTVGameEvent *event)
{
	...
	if(bPlayerHurt || bPlayerKilled)
	{
		...
		else // attacker != NULL
		{
			...
			// view over shoulder, randomly left or right
			StartChaseCameraShot(victim->entindex(), attacker->entindex(), 96, -20, (RandomFloat() > 0.5) ? 30 : -30,
								 bInEye);
		}
...
CHLTVGameEvent *CHLTVDirector::FindBestGameEvent()
{
	...
	if(bestEventPrio[1] >= bestEventPrio[0] && bestEventPrio[1] >= bestEventPrio[2] &&
	   bestEventPrio[1] >= bestEventPrio[3])
	{
		return &m_EventHistory[bestEvent[1]]; // best case
	}
	else if(bestEventPrio[0] > bestEventPrio[1] && bestEventPrio[0] > bestEventPrio[2])
	{
