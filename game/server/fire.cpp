...
bool FireSystem_IsFireInWall(Vector &position, fireType_e type)
{
	...
	trace_t tr;
	UTIL_TraceHull(position, position + Vector(0, 0, 0.1), FIRE_MINS, FIRE_MAXS, MASK_SOLID, NULL, COLLISION_GROUP_NONE,
				   &tr);
...
//-----------------------------------------------------------------------------
bool FireSystem_StartFire(const Vector &position, float fireHeight, float attack, float fuel, int flags,
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool FireSystem_StartFire(CBaseAnimating *pEntity, float fireHeight, float attack, float fuel, int flags,
						  CBaseEntity *owner, fireType_e type)
{
...
BEGIN_DATADESC(CFire)
	...
	DEFINE_FIELD(m_hEffect, FIELD_EHANDLE), DEFINE_FIELD(m_hOwner, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_flFuel, FIELD_FLOAT), DEFINE_FIELD(m_flDamageTime, FIELD_TIME),
		DEFINE_FIELD(m_lastDamage, FIELD_TIME), DEFINE_KEYFIELD(m_flFireSize, FIELD_FLOAT, "firesize"),

		DEFINE_KEYFIELD(m_flHeatLevel, FIELD_FLOAT, "ignitionpoint"), DEFINE_FIELD(m_flHeatAbsorb, FIELD_FLOAT),
		...
		DEFINE_KEYFIELD(m_flAttackTime, FIELD_FLOAT, "fireattack"), DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
		DEFINE_KEYFIELD(m_bStartDisabled, FIELD_BOOLEAN, "StartDisabled"), DEFINE_FIELD(m_bDidActivate, FIELD_BOOLEAN),
		...
		DEFINE_OUTPUT(m_OnIgnited, "OnIgnited"), DEFINE_OUTPUT(m_OnExtinguished, "OnExtinguished"),
...
void CFire::StartFire(void)
{
	...
	{
		UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 1024), MASK_FIRE_SOLID, this, COLLISION_GROUP_NONE,
					   &tr);
		vFirePos = tr.endpos;
	}
...
void CFire::Update(float simTime)
{
	...
	for(i = 0; i < nearbyCount; i++)
	{
		...
		else if(damage)
		{
			...
			if(FIRE_SPREAD_DAMAGE_MULTIPLIER != 1.0 &&
			   !pOther->IsPlayer()) // if set to 1.0, optimizer will remove this code
			{
			...
			if(bDoDamage)
			{
				...
				trace_t tr;
				UTIL_TraceLine(this->WorldSpaceCenter(), pOther->WorldSpaceCenter(), MASK_FIRE_SOLID, pOther,
...
BEGIN_DATADESC(CEnvFireSource)
	...
	DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_radius, FIELD_FLOAT, "fireradius"),
...
BEGIN_DATADESC(CEnvFireSensor)
	...
	DEFINE_KEYFIELD(m_radius, FIELD_FLOAT, "fireradius"), DEFINE_KEYFIELD(m_targetLevel, FIELD_FLOAT, "heatlevel"),
		DEFINE_KEYFIELD(m_targetTime, FIELD_FLOAT, "heattime"),

		DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN), DEFINE_FIELD(m_bHeatAtLevel, FIELD_BOOLEAN),
		DEFINE_FIELD(m_levelTime, FIELD_FLOAT),

		DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable), DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),

		DEFINE_OUTPUT(m_OnHeatLevelStart, "OnHeatLevelStart"), DEFINE_OUTPUT(m_OnHeatLevelEnd, "OnHeatLevelEnd"),
