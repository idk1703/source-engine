...
void CAI_BlendedMotor::ResetGoalSequence(void)
{
	...
	// Msg("Start %s end %s\n", GetOuter()->GetSequenceName( m_nPrimarySequence ), GetOuter()->GetSequenceName(
	// m_nGoalSequence ) );
	...
	if(m_nSecondarySequence != ACT_INVALID)
	{
		...
		if(m_flSecondaryWeight < 1.0)
		{
			SetLayerWeight(m_iPrimaryLayer,
						   (flWeight - m_flSecondaryWeight * flWeight) / (1.0f - m_flSecondaryWeight * flWeight));
...
int CAI_BlendedMotor::GetInteriorSequence(int fromSequence)
{
	...
	if(seqKeyValues)
	{
		...
		if(pkvInterior)
		{
			...
			{
				m_nInteriorSequence =
					GetOuter()->SelectWeightedSequence(GetOuter()->TranslateActivity(activity), fromSequence);
			}
			else
			{
...
AIMotorMoveResult_t CAI_BlendedMotor::MoveGroundExecute(const AILocalMoveGoal_t &move, AIMoveTrace_t *pTraceResult)
{
	...
	{
		// DevMsg( "%6.2f : Speed %.1f : %.1f (%.1f) :  %d\n", gpGlobals->curtime, flNewSpeed, move.maxDist,
	move.transitionDist, GetOuter()->m_pHintNode != NULL );
	...
	*/
	...
	{
		DevMsg( "%6.2f : Speed %.1f : %.1f : %.2f\n", gpGlobals->curtime, flNewSpeed, GetIdealSpeed(), flNewSpeed /
	GetIdealSpeed() );
	}
	*/
...
AIMotorMoveResult_t CAI_BlendedMotor::MoveFlyExecute(const AILocalMoveGoal_t &move, AIMoveTrace_t *pTraceResult)
{
	...
	// FIXME: the above is redundant with MoveGroundExecute, and the below is a mix of MoveGroundExecuteWalk and
	// MoveFlyExecute
	...
	/*
		if (GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)
		{
			...
			for (i = 1; i < m_scriptMove.Count(); i++)
			{
				...
				NDebugOverlay::Box( m_scriptMove[i].vecLocation, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), 0,255,255, 0,
	0.1 );

				//NDebugOverlay::Line( m_scriptMove[i].vecLocation, m_scriptMove[i].vecLocation + Vector(
	0,0,m_scriptMove[i].flMaxVelocity), 0,255,255, true, 0.1 );

				Vector vecMidway = m_scriptMove[i].vecLocation + ((m_scriptMove[i-1].vecLocation -
	m_scriptMove[i].vecLocation) * 0.5); NDebugOverlay::Text( vecMidway, UTIL_VarArgs( "%d", i ), false, 0.1 );
			}
	#endif
	#if 0
			for (i = 1; i < m_scriptTurn.Count(); i++)
			{
				...
				NDebugOverlay::Box( m_scriptTurn[i].vecLocation, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), 255,0,0, 0,
	0.1 );

				NDebugOverlay::Line( m_scriptTurn[i].vecLocation + Vector( 0,0,1), m_scriptTurn[i].vecLocation + Vector(
	0,0,1) + UTIL_YawToVector( m_scriptTurn[i].flYaw ) * 32, 255,0,0, true, 0.1 );
			}
	#endif
		}
	*/
		...
		if(pCurWaypoint)
		{
			...
			if(pCurWaypoint->GetNext())
			{
				...
				if(deltaYaw > 0.1)
				{
					...
					m_scriptTurn.AddToTail(script);
					// DevMsg("turn waypoint %.1f %.1f %.1f\n", script.vecLocation.x, script.vecLocation.y,
					// script.vecLocation.z );
					prev++;
				}
			}
			else
...
void CAI_BlendedMotor::BuildVelocityScript(const AILocalMoveGoal_t &move)
{
	...
	if(npc_height_adjust.GetBool() && move.bHasTraced && move.directTrace.flTotalDist != move.thinkTrace.flTotalDist)
	{
		...
		if ((GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
		{
			...
			NDebugOverlay::Box( move.directTrace.vEndPosition, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), 0,255,255, 0,
		0.12 );
		}
		*/
	}
	if(npc_height_adjust.GetBool())
	{
		...
		if ((GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
		{
			...
			NDebugOverlay::Box( move.thinkTrace.vEndPosition, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), 255,0,255, 0,
		0.12 ); Msg("m_flReactiveSpeedAdjust %.3f  %.1f %.1f\n", m_flReactiveSpeedAdjust, flHeight, flDist );
		}
		*/
	}
	...
	while(pCurWaypoint &&
		  (pCurWaypoint->NavType() == NAV_GROUND ||
		   pCurWaypoint->NavType() == NAV_FLY) /*&& flTotalDist / idealVelocity < 3.0*/) // limit lookahead to 3 seconds
	{
		script.Init();
				...
				case NAV_JUMP:
...

					{
						...
						Vector rawJumpVel = GetMoveProbe()->CalcJumpLaunchVelocity(
							script.vecLocation, pNext->vecLocation, gravity, &minJumpHeight, maxHorzVel, &vecApex);
	...
	{
		flTotalDist += m_scriptMove[i].flDist =
			(m_scriptMove[i + 1].vecLocation - m_scriptMove[i].vecLocation).Length2D();
	}
	...
	{
		float flNeededAccel =
			DeltaV(m_scriptMove[0].flMaxVelocity, m_scriptMove[m_scriptMove.Count() - 1].flMaxVelocity, flTotalDist);
	...
	if(ai_path_adjust_speed_on_immediate_turns.GetBool() && m_scriptMove.Count() > 1)
	{
		...
		if ((GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
		{
			...
			VectorNormalize( tmp );
			NDebugOverlay::Line( m_scriptMove[0].vecLocation + Vector( 0, 0, 10 ), m_scriptMove[0].vecLocation + tmp *
		32 + Vector( 0, 0, 10 ), 255,255,255, true, 0.1 );

			NDebugOverlay::Line( m_scriptMove[0].vecLocation + Vector( 0, 0, 10 ), m_scriptMove[1].vecLocation + Vector(
		0, 0, 10 ), 255,0,0, true, 0.1 );

			tmp = GetCurVel();
			VectorNormalize( tmp );
			NDebugOverlay::Line( m_scriptMove[0].vecLocation + Vector( 0, 0, 10 ), m_scriptMove[0].vecLocation + tmp *
		32 + Vector( 0, 0, 10 ), 0,0,255, true, 0.1 );
		}
		*/
	...
	for (i = 0; i < m_scriptMove.Count(); i++)
	{
		NDebugOverlay::Text( m_scriptMove[i].vecLocation, (const char *)CFmtStr( "%.2f ", m_scriptMove[i].flMaxVelocity
	), false, 0.1 );
	...
	*/
	...
	for(i = 0; i < m_scriptMove.Count() - 1;)
	{
		...
		else
		{
			...
			float flNeededAccel =
				fabs(DeltaV(m_scriptMove[i].flMaxVelocity, m_scriptMove[i + 1].flMaxVelocity, m_scriptMove[i].flDist));
			if(flNeededAccel < idealAccel)
			{
				...
				if(t1 > 0.0 && t2 > 0.0)
				{
					...
					*/
					...
					{
						// DevMsg("insert %.2f %.2f %.2f\n", m_scriptMove[i].flMaxVelocity, script.flMaxVelocity,
...
void CAI_BlendedMotor::InsertSlowdown(float distToObstruction, float idealAccel, bool bAlwaysSlowdown)
{
	...
	for(i = 0; i < m_scriptMove.Count() - 1; i++)
	{
		if(m_scriptMove[i].flDist > 0 && distToObstruction - m_scriptMove[i].flDist < 0)
		{
			...
			// NDebugOverlay::Line( m_scriptMove[i].vecLocation + Vector( 0, 0, 5 ), script.vecLocation + Vector( 0, 0,
			// 5 ), 0,255,0, true, 0.1 ); NDebugOverlay::Line( script.vecLocation + Vector( 0, 0, 5 ),
			...
			// clamp the next velocity to the possible accel in the given distance
			if(!bAlwaysSlowdown &&
			   SolveQuadratic(-0.5 * idealAccel, m_scriptMove[0].flMaxVelocity, -distToObstruction, r1, r2))
			{
...
bool CAI_BlendedMotor::AddTurnGesture(float flYD)
{
	...
	if(seq != ACTIVITY_NOT_AVAILABLE)
	{
		...
		if(iLayer != -1)
		{
			...
			if ( GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT )
			{
				Msg( "%.1f : %.2f %.2f : %.2f (%.2f)\n", flYD, weight, speed, actualDuration, turnCompletion *
			actualDuration );
			}
			*/
			return true;
