...
#define SF_RELATIONSHIP_NOTIFY_SUBJECT \
	(1 << 0) // Alert the subject of the change and give them a memory of the target entity
#define SF_RELATIONSHIP_NOTIFY_TARGET \
	(1 << 1) // Alert the target of the change and give them a memory of the subject entity
...
public:
	...
	void ChangeRelationships(int disposition, int iReverting, CBaseEntity *pActivator = NULL,
...
BEGIN_DATADESC(CAI_Relationship)
	DEFINE_THINKFUNC(ApplyRelationshipThink),
		...
		DEFINE_KEYFIELD(m_iDisposition, FIELD_INTEGER, "disposition"), DEFINE_KEYFIELD(m_iRank, FIELD_INTEGER, "rank"),
		DEFINE_KEYFIELD(m_fStartActive, FIELD_BOOLEAN, "StartActive"), DEFINE_FIELD(m_bIsActive, FIELD_BOOLEAN),
		DEFINE_KEYFIELD(m_flRadius, FIELD_FLOAT, "radius"), DEFINE_FIELD(m_iPreviousDisposition, FIELD_INTEGER),
		DEFINE_FIELD(m_iPreviousRank, FIELD_INTEGER), DEFINE_KEYFIELD(m_bReciprocal, FIELD_BOOLEAN, "reciprocal"),
...
CBaseEntity *CAI_Relationship::FindEntityForProceduralName(string_t iszName, CBaseEntity *pActivator,
														   CBaseEntity *pCaller)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::ChangeRelationships(int disposition, int iReverting, CBaseEntity *pActivator,
										   CBaseEntity *pCaller)
{
		...
		for(int j = 0; j < targetList.Count(); j++)
		{
			...
			else if(iReverting == REVERTING_TO_DEFAULT)
			{
				...
				if(m_bReciprocal)
				{
					...
					pSubject->IRelationPriority(pTarget) != m_iRank || HasSpawnFlags(SF_RELATIONSHIP_NOTIFY_SUBJECT) ||
