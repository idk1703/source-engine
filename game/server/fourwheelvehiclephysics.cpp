...
BEGIN_DATADESC_NO_BASE(CFourWheelVehiclePhysics)
	...
	// Controls
	DEFINE_FIELD(m_controls.throttle, FIELD_FLOAT), DEFINE_FIELD(m_controls.steering, FIELD_FLOAT),
		DEFINE_FIELD(m_controls.brake, FIELD_FLOAT), DEFINE_FIELD(m_controls.boost, FIELD_FLOAT),
		DEFINE_FIELD(m_controls.handbrake, FIELD_BOOLEAN), DEFINE_FIELD(m_controls.handbrakeLeft, FIELD_BOOLEAN),
		DEFINE_FIELD(m_controls.handbrakeRight, FIELD_BOOLEAN), DEFINE_FIELD(m_controls.brakepedal, FIELD_BOOLEAN),
		...
		DEFINE_FIELD(m_nSpeed, FIELD_INTEGER), DEFINE_FIELD(m_nLastSpeed, FIELD_INTEGER),
		DEFINE_FIELD(m_nRPM, FIELD_INTEGER), DEFINE_FIELD(m_fLastBoost, FIELD_FLOAT),
		DEFINE_FIELD(m_nBoostTimeLeft, FIELD_INTEGER), DEFINE_FIELD(m_nHasBoost, FIELD_INTEGER),

		DEFINE_FIELD(m_maxThrottle, FIELD_FLOAT), DEFINE_FIELD(m_flMaxRevThrottle, FIELD_FLOAT),
		DEFINE_FIELD(m_flMaxSpeed, FIELD_FLOAT), DEFINE_FIELD(m_actionSpeed, FIELD_FLOAT),
		...
		DEFINE_ARRAY(m_wheelPosition, FIELD_VECTOR, 4), DEFINE_ARRAY(m_wheelRotation, FIELD_VECTOR, 4),
		DEFINE_ARRAY(m_wheelBaseHeight, FIELD_FLOAT, 4), DEFINE_ARRAY(m_wheelTotalHeight, FIELD_FLOAT, 4),
		DEFINE_ARRAY(m_poseParameters, FIELD_INTEGER, 12), DEFINE_FIELD(m_actionValue, FIELD_FLOAT),
		DEFINE_KEYFIELD(m_actionScale, FIELD_FLOAT, "actionScale"), DEFINE_FIELD(m_debugRadius, FIELD_FLOAT),
		DEFINE_FIELD(m_throttleRate, FIELD_FLOAT), DEFINE_FIELD(m_throttleStartTime, FIELD_FLOAT),
		DEFINE_FIELD(m_throttleActiveTime, FIELD_FLOAT), DEFINE_FIELD(m_turboTimer, FIELD_FLOAT),

		DEFINE_FIELD(m_flVehicleVolume, FIELD_FLOAT), DEFINE_FIELD(m_bIsOn, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bLastThrottle, FIELD_BOOLEAN), DEFINE_FIELD(m_bLastBoost, FIELD_BOOLEAN),
...
void CFourWheelVehiclePhysics::DrawDebugGeometryOverlays()
{
	...
	if(pBody)
	{
		...
		NDebugOverlay::BoxAngles(vecBodyPosition, Vector(-5, -5, -5), Vector(5, 5, 5), angBodyDirection, 255, 0, 0, 0,
								 0);
		...
		NDebugOverlay::BoxAngles(vecAxlePositionsHL[0], Vector(-3, -3, -3), Vector(3, 3, 3), angBodyDirection, 0, 255,
								 0, 0, 0);
		NDebugOverlay::BoxAngles(vecAxlePositionsHL[1], Vector(-3, -3, -3), Vector(3, 3, 3), angBodyDirection, 0, 255,
								 0, 0, 0);
		...
		for(int iWheel = 0; iWheel < 4; ++iWheel)
		{
			...
			NDebugOverlay::BoxAngles(vecImpact, Vector(-0.5f, -0.5f, -0.5f), Vector(0.5f, 0.5f, 0.5f), angBodyDirection,
									 0, 0, 255, 0, 0);
...
int CFourWheelVehiclePhysics::DrawDebugTextOverlays(int nOffset)
{
	...
	char tempstr[512];
	Q_snprintf(tempstr, sizeof(tempstr), "Speed %.1f  T/S/B (%.0f/%.0f/%.1f)", params.speed, m_controls.throttle,
			   m_controls.steering, m_controls.brake);
...
bool CFourWheelVehiclePhysics::Think()
{
	...
	if(m_pOuter->VPhysicsGetObject() && !m_pOuter->VPhysicsGetObject()->GetShadowController())
	{
		...
		// we have to slide at least 15% of our speed at higher speeds to make the skid sound (otherwise it can be too
		// frequent)
		...
		if(skidThreshold < DEFAULT_SKID_THRESHOLD)
		{
			...
			skidThreshold =
				RemapValClamped(fabs(carState.speed), 0, minSpeed, DEFAULT_SKID_THRESHOLD * 8, DEFAULT_SKID_THRESHOLD);
		}
...
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::GetVehicleViewPosition(const char *pViewAttachment, float flPitchFactor,
...
void CFourWheelVehiclePhysics::SteeringRest(float carSpeed, const vehicleparams_t &vehicleData)
{
	float flSteeringRate =
		RemapValClamped(carSpeed, vehicleData.steering.speedSlow, vehicleData.steering.speedFast,
											...
											bool bBrake, bool bThrottle)
{
	...
	{
		flSteeringRestRate = RemapValClamped(carSpeedIns, vehicleData.engine.maxSpeed, vehicleData.engine.boostMaxSpeed,
...
#else
	// This is tested with gamepads with analog sticks.  It gives full analog control allowing the player to hold
	// shallow turns.
...
void CFourWheelVehiclePhysics::UpdateDriverControls(CUserCmd *cmd, float flFrameTime)
{
	...
	if(pDriver && pDriver->IsPlayer())
	{
		...
		else if(cmd->forwardmove > 0.0f)
		{
			if(pPlayerDriver->GetVehicleAnalogControlBias() == VEHICLE_ANALOG_BIAS_REVERSE)
			{
				...
				// Must pass a threshold to be accepted as forward input. Otherwise we just spoof a reduced reverse
				// input to keep the car moving in the direction the player probably expects.
	...
	if(cmd->forwardmove > 0.0f)
	{
		...
		if(m_controls.steering != 0)
		{
			...
			{
				flThrottleReduce = RemapValClamped(carSpeed, 0, vehicleData.steering.speedSlow, 0,
												   vehicleData.steering.turnThrottleReduceSlow);
			}
			else
			{
				flThrottleReduce = RemapValClamped(
		...
		m_controls.throttle =
			Approach(flMaxThrottle * flAnalogThrottle, m_controls.throttle, flFrameTime * m_throttleRate);

		// Apply the brake.
		if((flCarSign < 0.0f) && m_controls.bHasBrakePedal)
		{
			m_controls.brake = Approach(BRAKE_MAX_VALUE, m_controls.brake,
										flFrameTime * r_vehicleBrakeRate.GetFloat() * BRAKE_BACK_FORWARD_SCALAR);
	...
	else if(cmd->forwardmove < 0.0f)
	{
		...
		m_controls.throttle =
			Approach(flMaxThrottle * flAnalogBrake, m_controls.throttle, flFrameTime * m_throttleRate);
