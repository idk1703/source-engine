...
{
	// this is derived from IActionResult to ensure that ActionResult and EventDesiredResult cannot be silently
	// converted
...
{
	EventDesiredResult(ActionResultType type = CONTINUE, Action<Actor> *action = NULL,
					   EventResultPriorityType priority = RESULT_TRY, const char *reason = NULL)
		: IActionResult<Actor>(type, action, reason)
	{
		m_priority = priority;
	}
...
public:
	...
	virtual ~Behavior()
	{
		...
		for(bottomAction = m_action; bottomAction && bottomAction->m_buriedUnderMe;
	...
	virtual QueryResultType ShouldPickUp(const INextBot *me, CBaseEntity *item)
		const // if the desired item was available right now, should we pick it up?
	{
	...
	virtual QueryResultType IsHindrance(const INextBot *me, CBaseEntity *blocker)
		const // return true if we should wait for 'blocker' that is across our path somewhere up ahead.
	{
		QueryResultType result = ANSWER_UNDEFINED;
	...
	virtual Vector SelectTargetPoint(const INextBot *me, const CBaseCombatCharacter *subject)
		const // given a subject, return the world space position we should aim at
	{
	...
	virtual const CKnownEntity *SelectMoreDangerousThreat(const INextBot *me, const CBaseCombatCharacter *subject,
														  const CKnownEntity *threat1, const CKnownEntity *threat2)
		const // return the more dangerous of the two threats, or NULL if we have no opinion
	{
...
public:
	...
	virtual const char *GetFullName(
		void) const;			 // return a temporary string showing the full lineage of this one action
	...
	virtual EventDesiredResult<Actor> OnOtherKilled(Actor *me, CBaseCombatCharacter *victim,
	...
	virtual EventDesiredResult<Actor> OnSpokeConcept(Actor *me, CBaseCombatCharacter *who, AIConcept_t concept,
													 AI_Response *response)
	{
		return TryContinue();
	}
	virtual EventDesiredResult<Actor> OnWeaponFired(Actor *me, CBaseCombatCharacter *whoFired,
	...
	EventDesiredResult<Actor> TryChangeTo(Action<Actor> *action, EventResultPriorityType priority = RESULT_TRY,
										  const char *reason = NULL) const;
	EventDesiredResult<Actor> TrySuspendFor(Action<Actor> *action, EventResultPriorityType priority = RESULT_TRY,
											const char *reason = NULL) const;
	EventDesiredResult<Actor> TryDone(EventResultPriorityType priority = RESULT_TRY, const char *reason = NULL) const;
	EventDesiredResult<Actor> TryToSustain(EventResultPriorityType priority = RESULT_TRY,
										   const char *reason = NULL) const;
...
#define PROCESS_EVENT(METHOD)                                                                                          \
	{                                                                                                                  \
		...
		{                                                                                                              \
			...
			{                                                                                                          \
				m_actor->DebugConColorMsg(NEXTBOT_EVENTS, Color(100, 100, 100, 255),                                   \
										  "%3.2f: %s:%s: %s received EVENT %s\n", gpGlobals->curtime,                  \
										  m_actor->GetDebugIdentifier(), m_behavior->GetName(),                        \
										  _action->GetFullName(), #METHOD);                                            \
			}                                                                                                          \
		...
		if(_action)                                                                                                    \
		{                                                                                                              \
			if(m_actor && _result.IsRequestingChange() &&                                                              \
			   (m_actor->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool()))                              \
			{                                                                                                          \
				m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 0, 255),                                   \
										  "%3.2f: %s:%s: ", gpGlobals->curtime, m_actor->GetDebugIdentifier(),         \
										  m_behavior->GetName());                                                      \
				m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 255, 255), "%s ", _action->GetFullName()); \
				m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 0, 255), "reponded to EVENT %s with ",     \
										  #METHOD);                                                                    \
				m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 0, 0, 255), "%s %s ", _result.GetTypeName(),    \
										  _result.m_action ? _result.m_action->GetName() : "");                        \
...
#define PROCESS_EVENT_WITH_1_ARG(METHOD, ARG1)                                                                         \
	{                                                                                                                  \
		...
		if(_action)                                                                                                    \
		{                                                                                                              \
			if(m_actor && (m_actor->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool()) &&                 \
			   _result.IsRequestingChange() && _action)                                                                \
			{                                                                                                          \
				m_actor->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 0, 255),                                   \
										  "%3.2f: %s:%s: ", gpGlobals->curtime, m_actor->GetDebugIdentifier(),         \
										  m_behavior->GetName());                                                      \
...
#define PROCESS_EVENT_WITH_3_ARGS(METHOD, ARG1, ARG2, ARG3)                                                            \
	...
	Action<Actor> *m_child;	 // the ACTIVE Action we contain, top of the stack. Use m_buriedUnderMe, m_coveringMe on the
							 // child to traverse to other suspended children
	...
	Action<Actor> *GetActionBuriedUnderMe(
		void) const // return Action just "under" us that we will resume to when we finish
	{
		return m_buriedUnderMe;
	}

	Action<Actor> *GetActionCoveringMe(
		void) const // return Action just "above" us that will resume to us when it finishes
	{
		return m_coveringMe;
	}
	...
	bool IsOutOfScope(void) const
	{
		...
		{
			if(under->m_eventResult.m_type == CHANGE_TO || under->m_eventResult.m_type == DONE)
			{
				return true;
			}
		}
	...
	ActionResult<Actor> ProcessPendingEvents(void) const
	{
		...
		while(under)
		{
			if(under->m_eventResult.m_type == SUSPEND_FOR)
			{
				...
				ActionResult<Actor> result(under->m_eventResult.m_type, under->m_eventResult.m_action,
										   under->m_eventResult.m_reason);
	 ...
	 */
	ActionResult<Actor> InvokeOnStart(Actor *me, Behavior<Actor> *behavior, Action<Actor> *priorAction,
									  Action<Actor> *buriedUnderMeAction);
...
template<typename Actor>
EventDesiredResult<Actor> Action<Actor>::TryChangeTo(Action<Actor> *action, EventResultPriorityType priority,
...
template<typename Actor>
EventDesiredResult<Actor> Action<Actor>::TrySuspendFor(Action<Actor> *action, EventResultPriorityType priority,
													   const char *reason) const
{
...
template<typename Actor>
EventDesiredResult<Actor> Action<Actor>::TryToSustain(EventResultPriorityType priority,
													  const char *reason /*= NULL*/) const
{
 ...
 */
template<typename Actor>
ActionResult<Actor> Action<Actor>::InvokeOnStart(Actor *me, Behavior<Actor> *behavior, Action<Actor> *priorAction,
												 Action<Actor> *buriedUnderMeAction)
{
	// debug display
	if((me->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool()))
	{
		me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 150, 255), "%3.2f: %s:%s: ", gpGlobals->curtime,
							 me->GetDebugIdentifier(), behavior->GetName());
...
//-------------------------------------------------------------------------------------------
template<typename Actor>
ActionResult<Actor> Action<Actor>::InvokeOnResume(Actor *me, Behavior<Actor> *behavior,
												  Action<Actor> *interruptingAction)
{
	if(me->IsDebugging(NEXTBOT_BEHAVIOR) || NextBotDebugHistory.GetBool())
	{
		me->DebugConColorMsg(NEXTBOT_BEHAVIOR, Color(255, 255, 150, 255), "%3.2f: %s:%s: ", gpGlobals->curtime,
...
const char *Action<Actor>::GetFullName(void) const
{
	...
	for(const Action<Actor> *action = this; stackIndex < maxStack && action; action = action->m_parent)
	{
