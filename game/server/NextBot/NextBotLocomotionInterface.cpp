...
void ILocomotion::Update(void)
{
	...
	{
		// show motion vector
		NDebugOverlay::HorzArrow(GetFeet(), GetFeet() + 25.0f * m_groundMotionVector, 3.0f, 100, 255, 0, 255, true,
								 0.1f);
 ...
 */
bool ILocomotion::IsPotentiallyTraversable(const Vector &from, const Vector &to, TraverseWhenType when,
										   float *fraction) const
{
	...
	const float probeSize =
		0.25f * GetBot()
					->GetBodyInterface()
					->GetHullWidth(); // Cant be TOO small, or open stairwells/grates/etc will cause problems
...
bool ILocomotion::IsGap(const Vector &pos, const Vector &forward) const
{
	...
	TraceHull(pos + Vector(0, 0, GetStepHeight()), // start up a bit to handle rough terrain
			  pos + Vector(0, 0, -GetMaxJumpHeight()), Vector(-halfWidth, -halfWidth, 0),
			  Vector(halfWidth, halfWidth, hullHeight), mask, &filter, &ground);
		...
		else
		{
			...
			if(m_stillStuckTimer.IsElapsed())
			{
				...
				if(GetBot()->IsDebugging(NEXTBOT_LOCOMOTION))
				{
					...
					NDebugOverlay::Circle(m_stuckPos + Vector(0, 0, 5.0f), QAngle(-90.0f, 0, 0), 5.0f, 255, 0, 0, 255,
										  true, 1.0f);
				}
			...
			if(m_stuckTimer.IsGreaterThen(escapeTime))
			{
				...
				if(GetBot()->IsDebugging(NEXTBOT_ERRORS))
				{
					DevMsg("%3.2f: %s STUCK at position( %3.2f, %3.2f, %3.2f )\n", gpGlobals->curtime,
						   GetBot()->GetDebugIdentifier(), m_stuckPos.x, m_stuckPos.y, m_stuckPos.z);

					NDebugOverlay::Circle(m_stuckPos + Vector(0, 0, 15.0f), QAngle(-90.0f, 0, 0), 3.0f, 255, 255, 0,
										  255, true, 1.0f);
					NDebugOverlay::Circle(m_stuckPos + Vector(0, 0, 5.0f), QAngle(-90.0f, 0, 0), 5.0f, 255, 0, 0, 255,
										  true, 9999999.9f);
				}
