...
//---------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------
static const char *debugTypeName[] = {"BEHAVIOR", "LOOK_AT", "PATH",   "ANIMATION", "LOCOMOTION",
									  "VISION",	  "HEARING", "EVENTS", "ERRORS",	NULL};
...
static ConCommand SetDebug(
	"nb_debug", CC_SetDebug,
...
static ConCommand SetDebugFilter(
	"nb_debug_filter", CC_SetDebugFilter,
...
static void CC_SelectBot(const CCommand &args)
{
	...
	if(player)
	{
		...
		if(select.m_pick)
		{
			NDebugOverlay::Circle(select.m_pick->GetLocomotionInterface()->GetFeet() + Vector(0, 0, 5), Vector(1, 0, 0),
								  Vector(0, -1, 0), 25.0f, 0, 255, 0, 255, false, 1.0f);
		}
	}
}
static ConCommand SelectBot("nb_select", CC_SelectBot, "Select the bot you are aiming at for further debug operations.",
							FCVAR_CHEAT);
...
static ConCommand ForceLookAt("nb_force_look_at", CC_ForceLookAt,
...
void CC_WarpSelectedHere(const CCommand &args)
{
	...
	trace_t result;
	UTIL_TraceLine(me->EyePosition(), me->EyePosition() + 999999.9f * forward,
				   MASK_BLOCKLOS_AND_NPCS | CONTENTS_IGNORE_NODRAW_OPAQUE, me, COLLISION_GROUP_NONE, &result);
	if(result.DidHit())
	{
...
static ConCommand WarpSelectedHere("nb_warp_selected_here", CC_WarpSelectedHere,
...
void NextBotManager::Reset(void)
{
	...
	while(i != m_botList.InvalidIndex())
	{
		...
		if(m_botList[i]->IsRemovedOnReset())
		{
			...
			// Assert( !m_botList.IsInList( i ) );	// UTIL_Remove() calls UpdateOnRemove, adds EFL_KILLME, but doesn't
			// delete until the end of the frame
		}
		i = iNext;
	}
	...
	if(m_botList.Count())
	{
		...
		if(m_iUpdateTickrate != tickRate)
		{
			Msg("NextBot tickrate changed from %d (%.3fms) to %d (%.3fms)\n", m_iUpdateTickrate,
				TICKS_TO_TIME(m_iUpdateTickrate), tickRate, TICKS_TO_TIME(tickRate));
		...
		if(nb_update_debug.GetBool())
		{
			...
			Msg("Frame %8d/tick %8d: %3d run of %3d, %3d sliders, %3d blocked slides, scheduled %3d for next tick, %3d "
				"intentional sliders, %d nonresponsive, %d dead\n",
				gpGlobals->framecount - 1, gpGlobals->tickcount - 1, g_nRun, m_botList.Count() - nDead, g_nSlid,
				g_nBlockedSlides, nScheduled, nIntentionalSliders, nNonResponsive, nDead);
		...
		if(frameLimit > 0.0f)
		{
			...
			else if(nb_update_debug.GetBool())
			{
				Msg("Frame %8d/tick %8d: frame out of budget (%.2fms > %.2fms)\n", gpGlobals->framecount,
					gpGlobals->tickcount, sumFrameTime, frameLimit);
			}
		}
	}
...
void NextBotManager::OnSpokeConcept(CBaseCombatCharacter *who, AIConcept_t concept, AI_Response *response)
{
	...
	if(IsDebugging(NEXTBOT_HEARING))
	{
		...
		DevMsg("%3.2f: OnSpokeConcept( %s, %s )\n", gpGlobals->curtime, who->GetDebugName(),
