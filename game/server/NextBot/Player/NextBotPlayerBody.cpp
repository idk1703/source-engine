...
ConVar nb_head_aim_resettle_angle(
	"nb_head_aim_resettle_angle", "100", FCVAR_CHEAT,
...
void PlayerBody::Upkeep(void)
{
	...
	if(subject)
	{
		...
		{
			// update subject tracking by periodically estimating linear aim velocity, allowing for "slop" between
			// updates
			...
			{
				desiredLookAtPos =
					GetBot()->GetIntentionInterface()->SelectTargetPoint(GetBot(), subject->MyCombatCharacterPointer());
			}
			else
			{
 ...
 */
void PlayerBody::AimHeadTowards(const Vector &lookAtPos, LookAtPriorityType priority, float duration,
								INextBotReply *replyWhenAimed, const char *reason)
{
	...
	// do NOT clear this here, or continuous calls to AimHeadTowards will keep IsHeadAimingOnTarget returning false all
	// of the time m_isSightedIn = false;
 ...
 * Aim the bot's head towards the given goal
 */
void PlayerBody::AimHeadTowards(CBaseEntity *subject, LookAtPriorityType priority, float duration,
								INextBotReply *replyWhenAimed, const char *reason)
{
	if(duration <= 0.0f)
	{
	...
	if(GetBot()->IsDebugging(NEXTBOT_LOOK_AT))
	{
		...
		ConColorMsg(Color(255, 100, 0, 255), "%3.2f: %s Look At subject %s for %3.2f s, Pri = %s, Reason = %s\n",
					gpGlobals->curtime, m_player->GetPlayerName(), subject->GetClassname(), duration, priName,
					(reason) ? reason : "");
	}
}
...
//-----------------------------------------------------------------------------------------------
/**
 * Return the bot's collision mask (hack until we get a general hull trace abstraction here or in the locomotion
 * interface)
 */
