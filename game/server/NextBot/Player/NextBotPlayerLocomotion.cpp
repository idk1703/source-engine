...
PlayerLocomotion::LadderState PlayerLocomotion::ApproachAscendingLadder(void)
{
	...
	{
		NDebugOverlay::EntityText(GetBot()->GetEntity()->entindex(), 0, "Approach ascending ladder", 0.1f, 255, 255,
								  255, 255);
	}
...
PlayerLocomotion::LadderState PlayerLocomotion::ApproachDescendingLadder(void)
{
	...
	Vector mountPoint =
		m_ladderInfo->m_top + 0.25f * GetBot()->GetBodyInterface()->GetHullWidth() * m_ladderInfo->GetNormal();
	...
	if(GetBot()->IsDebugging(NEXTBOT_LOCOMOTION))
	{
		NDebugOverlay::EntityText(GetBot()->GetEntity()->entindex(), 0, "Approach descending ladder", 0.1f, 255, 255,
								  255, 255);
	}
...
void PlayerLocomotion::Update(void)
{
	...
	if(m_isJumpingAcrossGap || m_isClimbingUpToLedge)
	{
		...
		if(m_hasLeftTheGround)
		{
			...
			GetBot()->GetBodyInterface()->AimHeadTowards(GetBot()->GetEntity()->EyePosition() + 100.0 * toLanding,
														 IBody::MANDATORY, 0.25f, NULL, "Facing impending jump/climb");
...
void PlayerLocomotion::Approach(const Vector &pos, float goalWeight)
{
	...
	if(m_player->IsOnLadder() && IsUsingLadder() &&
	   (m_ladderState == ASCENDING_LADDER || m_ladderState == DESCENDING_LADDER))
	{
		// we are on a ladder and WANT to be on a ladder.
		playerButtons->PressForwardButton();

		// Stay in center of ladder.  The gamemovement will autocenter us in most cases, but this is needed in case it
		// doesn't.
		if(m_ladderInfo)
		{
			Vector posOnLadder;
	...
	else
	{
		...
		if(ahead > epsilon)
		{
			...
			if(GetBot()->IsDebugging(NEXTBOT_LOCOMOTION))
			{
				NDebugOverlay::HorzArrow(m_player->GetAbsOrigin(),
										 m_player->GetAbsOrigin() + 50.0f * Vector(forward.x, forward.y, 0.0f), 15.0f,
										 0, 255, 0, 255, true, 0.1f);
			}
		}
		...
		if(side <= -epsilon)
		{
			...
			{
				NDebugOverlay::HorzArrow(m_player->GetAbsOrigin(),
										 m_player->GetAbsOrigin() - 50.0f * Vector(right.x, right.y, 0.0f), 15.0f, 255,
										 0, 255, 255, true, 0.1f);
			}
		}
		else if(side >= epsilon)
		{
			...
			{
				NDebugOverlay::HorzArrow(m_player->GetAbsOrigin(),
										 m_player->GetAbsOrigin() + 50.0f * Vector(right.x, right.y, 0.0f), 15.0f, 0,
...
//----------------------------------------------------------------------------------------------------
bool PlayerLocomotion::ClimbUpToLedge(const Vector &landingGoal, const Vector &landingForward,
...
void PlayerLocomotion::JumpAcrossGap(const Vector &landingGoal, const Vector &landingForward)
{
	...
	// face forward
	GetBot()->GetBodyInterface()->AimHeadTowards(landingGoal, IBody::MANDATORY, 1.0f, NULL,
