 ...
 */
bool NextBotGroundLocomotion::DetectCollision(trace_t *pTrace, int &recursionLimit, const Vector &from,
											  const Vector &to, const Vector &vecMins, const Vector &vecMaxs)
{
	IBody *body = GetBot()->GetBodyInterface();
	...
	while(true)
	{
		...
		if(trace.startsolid)
		{
			...
			if(trace.m_pEnt && !trace.m_pEnt->IsWorld())
			{
				...
				if(dynamic_cast<CPhysicsProp *>(trace.m_pEnt) != NULL &&
				   dynamic_cast<CBasePropDoor *>(trace.m_pEnt) == NULL)
				{
					IPhysicsObject *physics = trace.m_pEnt->VPhysicsGetObject();
					if(physics && physics->IsMoveable())
					{
						// we've intersected a (likely moving) physics prop - ignore it for awhile so we can move out of
						// it
		...
		if(!body->HasActivityType(IBody::MOTION_CONTROLLED_Z) && trace.plane.normal.z < GetTraversableSlopeLimit() &&
		   fullMove.z > 0.0f)
		{
			fullMove.z = 0.0f;
			trace.plane.normal.z = 0.0f;
		...
		if(GetBot()->IsDebugging(NEXTBOT_LOCOMOTION))
		{
			NDebugOverlay::Line(trace.endpos, trace.endpos + 20.0f * trace.plane.normal, 255, 0, 150, true, 15.0f);
		}
...
Vector NextBotGroundLocomotion::ResolveZombieCollisions(const Vector &pos)
{
	...
	if(me != NULL && !IsUsingLadder() && !IsClimbingOrJumping() && IsOnGround() && m_nextBot->IsAlive() &&
	   IsAttemptingToMove() /*&& GetBot()->GetBodyInterface()->IsArousal( IBody::INTENSE )*/)
	{
...
void NextBotGroundLocomotion::UpdateGroundConstraint(void)
{
	...
	TraceHull(m_nextBot->GetPosition() + Vector(0, 0, GetStepHeight() + 0.001f),
			  m_nextBot->GetPosition() + Vector(0, 0, -stickToGroundTolerance), Vector(-halfWidth, -halfWidth, 0),
			  Vector(halfWidth, halfWidth, hullHeight), body->GetSolidMask(), &filter, &ground);
	...
	if(ground.startsolid)
	{
		...
		{
			DevMsg("%3.2f: Inside ground, ( %.0f, %.0f, %.0f )\n", gpGlobals->curtime, m_nextBot->GetPosition().x,
				   m_nextBot->GetPosition().y, m_nextBot->GetPosition().z);
		}
		return;
	}
...
void NextBotGroundLocomotion::StandUp( void )
{
	...
	TraceHull( GetFeet(), GetFeet(), standHullMin, standHullMax, MASK_NPCSOLID, m_nextBot, MASK_DEFAULTPLAYERSOLID,
&result );
 ...
 */
bool NextBotGroundLocomotion::ClimbUpToLedge(const Vector &landingGoal, const Vector &landingForward,
											 const CBaseEntity *obstacle)
{
	return false;
}
