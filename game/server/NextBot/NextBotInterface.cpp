...
ConVar NextBotDebugHistory("nb_debug_history", IsX360() ? "0" : "1", FCVAR_CHEAT,
...
const char *INextBot::GetDebugIdentifier(void) const
{
	...
	Q_snprintf(name, nameSize, "%s(#%d)", const_cast<INextBot *>(this)->GetEntity()->GetClassname(),
			   const_cast<INextBot *>(this)->GetEntity()->entindex());
...
bool INextBot::IsAbleToClimbOnto(const CBaseEntity *object) const
{
	...
	if(FClassnameIs(const_cast<CBaseEntity *>(object), "prop_door*") ||
	   FClassnameIs(const_cast<CBaseEntity *>(object), "func_door*"))
	{
		return false;
	}
...
bool INextBot::IsAbleToBreak(const CBaseEntity *object) const
{
	...
	{
		if(FClassnameIs(const_cast<CBaseEntity *>(object), "func_breakable") && object->GetHealth())
		{
			return true;
		}
