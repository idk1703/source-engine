...
public:
	...
	//
	virtual void Approach(const Vector &goalPos,
						  float goalWeight = 1.0f); // (EXTEND) move directly towards the given position
	...
	virtual bool IsScrambling(void) const; // is in the middle of a complex action (climbing a ladder, climbing a ledge,
										   // jumping, etc) that shouldn't be interrupted
	...
	virtual const Vector &GetGroundMotionVector(void) const; // return unit vector in XY plane describing our direction
															 // of motion - even if we are currently not moving
	...
	virtual bool IsUsingLadder(
		void) const; // we are moving to get on, ascending/descending, and/or dismounting a ladder
	virtual bool IsAscendingOrDescendingLadder(
		void) const; // we are actually on the ladder right now, either climbing up or down
	...
	virtual const Vector &GetFeet(
		void) const; // return position of "feet" - the driving point where the bot contacts the ground
	...
	virtual const Vector &GetMotionVector(
		void) const; // return unit vector describing our direction of motion - even if we are currently not moving

	virtual bool IsAreaTraversable(
		const CNavArea *baseArea) const; // return true if given area can be used for navigation

	virtual float GetTraversableSlopeLimit(
		void) const; // return Z component of unit normal of steepest traversable slope
	 ...
	 */
	virtual bool IsPotentiallyTraversable(const Vector &from, const Vector &to, TraverseWhenType when = EVENTUALLY,
										  float *fraction = NULL) const;
	...
	virtual bool IsAttemptingToMove(
		void) const; // return true if we have tried to Approach() or DriveTo() very recently

	void TraceHull(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs, unsigned int fMask,
				   ITraceFilter *pFilter, trace_t *pTrace) const;
...
inline void ILocomotion::TraceHull(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs,
								   unsigned int fMask, ITraceFilter *pFilter, trace_t *pTrace) const
{
