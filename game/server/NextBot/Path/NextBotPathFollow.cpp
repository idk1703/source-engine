...
void PathFollower::AdjustSpeed(INextBot *bot)
{
	...
	mover->SetDesiredSpeed(mover->GetRunSpeed() +
						   fabs(data.curvature) * (mover->GetWalkSpeed() - mover->GetRunSpeed()));
}
...
bool PathFollower::IsAtGoal(INextBot *bot) const
{
	...
	else if(m_goal->type == CLIMB_UP)
	{
		...
		/* This breaks infected climbing up holes in the ceiling - they can get within 2D range of m_goal before finding
		a ledge to climb up to else if ( mover->IsOnGround() )
		{
			// proximity check
	...
	else
	{
		...
		if(next)
		{
			...
			if(DotProduct2D(toGoal.AsVector2D(), dividingPlane) < 0.0001f && abs(toGoal.z) < body->GetStandHullHeight())
			{
				...
				// move *through*
				if(toGoal.z < mover->GetStepHeight() && (mover->IsPotentiallyTraversable(mover->GetFeet(), next->pos) &&
														 !mover->HasPotentialGap(mover->GetFeet(), next->pos)))
				{
					// passed goal
					return true;
				}
...
bool PathFollower::LadderUpdate(INextBot *bot)
{
	...
	{
		// Check if we have somehow ended up on a ladder, if so, and its a tall down-ladder we are expecting, jump the
		// path ahead. This happens for players, who run off ledges and the gamemovement sticks them onto ladders.  We
		// only care about tall down-ladders, because up ladders work without this, and short ladders aren't dangerous
		// to miss and drop down instead of climbing down.
		if(bot->GetEntity()->GetMoveType() == MOVETYPE_LADDER)
		{
			...
			// Start with current, the segment we are currently traversing.  Skip the distance check for that segment,
			// because the pos is (hopefully) behind us.  And if it's a long path segment, it's already outside the
			// climbLookAheadRange, and thus it would prevent us looking at m_goal and further for imminent planned
			// climbs. 'current' is the segment we are on/just passed over
			...
			for(const Segment *s = current; s; s = NextSegment(s))
			{
				...
				// Only consider reasonably tall down ladders - if we don't grab onto a short ladder, it hopefully won't
				// be a bad fall.
	...
	if(m_goal->how == GO_LADDER_UP)
	{
		...
		body->AimHeadTowards(m_goal->ladder->m_top - 50.0f * m_goal->ladder->GetNormal() +
								 Vector(0, 0, body->GetCrouchHullHeight()),
		...
		{
			// approach the base of the ladder - use normal path following in case there are jumps/climbs on the way to
			// the ladder
			return false;
		}
	}
	else // go down ladder
	{
		...
		else
		{
			...
			body->AimHeadTowards(m_goal->ladder->m_bottom + 50.0f * m_goal->ladder->GetNormal() +
									 Vector(0, 0, body->GetCrouchHullHeight()),
								 IBody::CRITICAL, 1.0f, NULL, "Mounting downward ladder");
			...
			{
				// approach the top of the ladder - use normal path following in case there are jumps/climbs on the way
				// to the ladder
				return false;
			}
		}
...
void PathFollower::Update(INextBot *bot)
{
	...
	if(!m_goal->ladder && !mover->IsClimbingOrJumping() && !isOnStairs &&
	   m_goal->pos.z > mover->GetFeet().z + tooHighDistance)
	{
		...
		if(mover->IsStuck() || to.IsLengthLessThan(closeRange))
		{
			...
			if(mover->IsStuck() || !next || (next->pos.z - mover->GetFeet().z > mover->GetMaxJumpHeight()) ||
			   !mover->IsPotentiallyTraversable(mover->GetFeet(), next->pos))
			{
				...
				// reset stuck status since we're (likely) repathing anyways. otherwise, we could be stuck in a loop
				// here and not move
	...
	if(true || m_goal != LastSegment()) // think more about this - we often need to avoid to reach the final goal pos,
										// too (MSB 5/15/09)
	{
...
CBaseEntity *PathFollower::FindBlocker(INextBot *bot)
{
	...
	for(const Segment *s = GetCursorData().segmentPrior; s && range < maxHindranceRangeAlong; s = NextSegment(s))
	{
		...
		mover->TraceHull(from, from + traceRange * traceForward, blockerMins, blockerMaxs, body->GetSolidMask(),
						 &filter, &result);
		...
		if(result.DidHitNonWorldEntity())
		{
			...
			if(DotProduct(toBlocker, alongPath) > 0.0f)
			{
				...
				if(think->IsHindrance(bot, result.m_pEnt) == ANSWER_YES)
				{
					if(bot->IsDebugging(NEXTBOT_PATH))
					{
						NDebugOverlay::Circle(bot->GetLocomotionInterface()->GetFeet(), QAngle(-90.0f, 0, 0), 10.0f,
											  255, 0, 0, 255, true, 1.0f);
						NDebugOverlay::HorzArrow(bot->GetLocomotionInterface()->GetFeet(),
												 result.m_pEnt->GetAbsOrigin(), 1.0f, 255, 0, 0, 255, true, 1.0f);
					}
...
bool PathFollower::FindClimbLedge(INextBot *bot, Vector startTracePos, Vector ledgeRegionMins, Vector ledgeRegionMaxs)
{
	...
	mover->TraceHull(startTracePos, startTracePos, ledgeRegionMins, ledgeRegionMaxs,
	...
	else
	{
		...
		mover->TraceHull(startTracePos, startTracePos + Vector(0, 0, -100.0f), ledgeRegionMins, ledgeRegionMaxs,
						 bot->GetBodyInterface()->GetSolidMask(), &filter, &result);
	}
}
#endif // _DEBUG
 ...
 * Climb up ledges
 */
bool PathFollower::Climbing(INextBot *bot, const Path::Segment *goal, const Vector &forward, const Vector &right,
							float goalRange)
{
	...
	// This is required to try to avoid accidentally climbing onto valid high ledges when we really want to run UNDER
	// them to our destination. We need to check "immediate" traversability to pay attention to breakable objects in our
	// way that we should climb over. We also need to check traversability out to 2 * ledgeLookAheadRange in case our
	// goal is just before a tricky ledge climb and once we pass the goal it will be too late. When we're in a CLIMB_UP
	// segment, allow us to look for ledges - we know the destination ledge height, and will only grab the correct
	// ledge.
	Vector toGoal = m_goal->pos - mover->GetFeet();
	toGoal.NormalizeInPlace();

	if(toGoal.z < mover->GetTraversableSlopeLimit() && !mover->IsStuck() && m_goal->type != CLIMB_UP &&
	   mover->IsPotentiallyTraversable(mover->GetFeet(), mover->GetFeet() + 2.0f * ledgeLookAheadRange * toGoal,
	...
	//	mover->IsPotentiallyTraversable( mover->GetFeet(), mover->GetFeet() + Vector( 0, 0, mover->GetMaxJumpHeight() ),
	//ILocomotion::IMMEDIATELY, &ceilingFraction );
	...
	{
		trace_t backupTrace;
		const float backupDistance =
			hullWidth * 0.25f; // The IsPotentiallyTraversable check this replaces uses a 1/4 hull width trace
		Vector backupFeet(feet - climbDirection * backupDistance);
		Vector backupCeiling(backupFeet + Vector(0, 0, mover->GetMaxJumpHeight()));
		mover->TraceHull(backupFeet, backupCeiling, skipStepHeightHullMin, skipStepHeightHullMax, mask, &filter,
						 &backupTrace);
		if(!backupTrace.startsolid && backupTrace.fraction > ceilingFraction)
		{
			bot->DebugConColorMsg(NEXTBOT_PATH, Color(255, 255, 255, 255),
	...
	mover->TraceHull(feet, feet + climbDirection * ledgeLookAheadRange, skipStepHeightHullMin, climbHullMax, mask,
					 &filter, &result);

	if(bot->IsDebugging(NEXTBOT_PATH) && NextBotDebugClimbing.GetBool())
	{
		// show ledge-finding hull as we move
		NDebugOverlay::SweptBox(feet, feet + climbDirection * ledgeLookAheadRange, skipStepHeightHullMin, climbHullMax,
								vec3_angle, 255, 100, 0, 255, 0.1f);
	}

	bool wasPotentialLedgeFound = result.DidHit() && !result.startsolid;
	// To test climbing up past small lips on walls, we can force the bot to run past the overhang and use the backup
	// trace: wasPotentialLedgeFound = wasPotentialLedgeFound && (result.fraction == 0 || isBackupTraceUsed);
	if(wasPotentialLedgeFound)
	{
		...
		if(bot->IsDebugging(NEXTBOT_PATH) && NextBotDebugClimbing.GetBool())
		{
			...
			NDebugOverlay::SweptBox(feet, feet + climbDirection * ledgeLookAheadRange, skipStepHeightHullMin,
									climbHullMax, vec3_angle, 255, 100, 0, 100, 999.9f);
		...
		if(!result.DidHitNonWorldEntity() || bot->IsAbleToClimbOnto(obstacle))
		{
			...
			if(m_goal->type == CLIMB_UP)
			{
				...
				if(afterClimb && afterClimb->area)
				{
					...
					{
						depthVector.x =
							(depthVector.x > 0) ? afterClimb->area->GetSizeX() : -afterClimb->area->GetSizeX();
					}
					if(fabs(depthVector.y) > afterClimb->area->GetSizeY())
					{
						depthVector.y =
							(depthVector.y > 0) ? afterClimb->area->GetSizeY() : -afterClimb->area->GetSizeY();
					}
			...
			// once we have found the ledge's front wall, we must look at least minLedgeDepth farther in to verify it is
			// a ledge NOTE: This *must* be ledgeLookAheadRange since ledges are compared against the initial trace
			// which was ledgeLookAheadRange deep
			float ledgeTopLookAheadRange = ledgeLookAheadRange;

			// TODO: we also must look far enough ahead in case the ledge we actually find is "deeper" than the initial
			// wall at the base
				...
				if(!result.startsolid)
				{
					...
					if(foundWall)
					{
						...
						if((traceDepth - ledgeFrontWallDepth) > minLedgeDepth)
						{
							...
							mover->TraceHull(ledgePos, ledgePos + Vector(0, 0, -ledgeHeightIncrement), climbHullMin,
											 climbHullMax, mask, &filter, &result);
							...
							else
							{
								...
								{
									// if we're climbing to a specific ledge via a CLIMB_UP link, only climb to that
									// ledge. Do this only for the world (which includes static props) so we can still
									// opportunistically climb up onto breakable railings and physics props.
							...
							if(isUsable)
							{
								...
								while(backUpSoFar < maxBackUp)
								{
									...
									mover->TraceHull(testPos, testPos + Vector(0, 0, -ledgeHeightIncrement),
													 climbHullMin, climbHullMax, mask, &filter, &result);
									...
									if(bot->IsDebugging(NEXTBOT_PATH) && NextBotDebugClimbing.GetBool())
									{
										...
										NDebugOverlay::SweptBox(
											testPos, testPos + Vector(0, 0, -mover->GetStepHeight()), climbHullMin,
											climbHullMax, vec3_angle, 255, 0, 0, 255, 999.9f);
									}
								...
								// Make sure this doesn't embed us in the far wall if the ledge is narrow, since we
								// would have backed up less than halfSize.
								Vector climbHullMinStep(climbHullMin); // skip StepHeight for sloped ledges
								mover->TraceHull(validLedgePos, ledgePos, climbHullMinStep, climbHullMax, mask, &filter,
												 &result);

								ledgePos = result.endpos;

								// Now since ledgePos + StepHeight is valid, trace down to find ground on sloped ledges.
								mover->TraceHull(ledgePos + Vector(0, 0, StepHeight), ledgePos, climbHullMin,
												 climbHullMax, mask, &filter, &result);
								if(!result.startsolid)
								{
									ledgePos = result.endpos;
							...
							/*** NOTE: While this saves us from climbing into a window below the window we want to get
							 in,
							 *** it also causes us to climb in midair high over crates sitting against walls we need to
							 climb over. if ( isUsable && m_goal->type == CLIMB_UP )
														{
															// we can only accept ledges at least as high as our current
							 CLIMB_UP destination
															// NOTE: Can't use plannedClimbZ here, since that could be 2
							 or 3 short climbs ahead const Segment *ledge = NextSegment( m_goal );

															if ( !ledge || ledgeHeight < ledge->pos.z - feet.z -
							 mover->GetStepHeight() )
															{
																// this ledge is below the CLIMB_UP destination - can't
							 use it isUsable = false;
															}
														}
							*/
					...
					else if(result.DidHit())
					{
						...
						if(bot->IsDebugging(NEXTBOT_PATH))
						{
							...
							{
								NDebugOverlay::HorzArrow(result.endpos, result.endpos + 20.0f * result.plane.normal,
														 5.0f, 255, 100, 0, 255, true, 9999.9f);
							}
			...
			if(foundLedge)
			{
				if(bot->IsDebugging(NEXTBOT_PATH))
				{
					...
					if(NextBotDebugClimbing.GetBool())
					{
						...
						NDebugOverlay::Triangle(wallBottomRight, wallBottomLeft, wallTopLeft, 255, 100, 0, 100, true,
												9999.9f);
						NDebugOverlay::Triangle(wallBottomRight, wallTopLeft, wallTopRight, 255, 100, 0, 100, true,
												9999.9f);
 ...
 * Jump over gaps
 */
bool PathFollower::JumpOverGaps(INextBot *bot, const Path::Segment *goal, const Vector &forward, const Vector &right,
								float goalRange)
{
...
void PathFollower::Draw(const Path::Segment *start) const
{
	...
	if(m_goal)
	{
		...
		switch(m_goal->how)
		{
			case GO_NORTH:
			case GO_SOUTH:
				NDebugOverlay::Line(m_goal->m_portalCenter - Vector(m_goal->m_portalHalfWidth, 0, 0),
									m_goal->m_portalCenter + Vector(m_goal->m_portalHalfWidth, 0, 0), 255, 0, 255, true,
									0.1f);
				break;

			default:
				NDebugOverlay::Line(m_goal->m_portalCenter - Vector(0, m_goal->m_portalHalfWidth, 0),
									m_goal->m_portalCenter + Vector(0, m_goal->m_portalHalfWidth, 0), 255, 0, 255, true,
									0.1f);
				break;
		}
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return true if there is a the given discontinuity ahead in the path within the given range (-1 = entire remaining
 * path)
 */
