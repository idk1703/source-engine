...
bool Path::ComputePathDetails(INextBot *bot, const Vector &start)
{
		...
		if(to->how <= GO_WEST) // walk along the floor to the next area
		{
			...
			if(expectedHeightDrop > mover->GetStepHeight())
			{
				...
				for(pushDist = 0.0f; pushDist <= maxPushDist; pushDist += inc)
				{
					...
					UTIL_TraceHull(pos, lowerPos, Vector(-halfWidth, -halfWidth, stepHeight),
								   Vector(halfWidth, halfWidth, hullHeight), bot->GetBodyInterface()->GetSolidMask(),
								   &filter, &result);
	...
	for(int i = 0; i < m_segmentCount - 1; ++i)
	{
		...
		else if((closeTo.z - closeFrom.z) > stepHeight)
		{
			...
			// use center of climb-up destination area to make sure bot moves onto actual ground once they finish their
			// climb
			to->pos = to->area->GetCenter();
						...
						if(overlap < NextBotPathSegmentInfluenceRadius.GetFloat())
						{
							if(length - overlap < NextBotPathSegmentInfluenceRadius.GetFloat())
							{
								...
								float startCurvature =
									segment->curvature *
									(1.0f - (overlap / NextBotPathSegmentInfluenceRadius.GetFloat()));
								float endCurvature =
									next->curvature *
									(1.0f - ((length - overlap) / NextBotPathSegmentInfluenceRadius.GetFloat()));
 ...
 */
void Path::ComputeAreaCrossing(INextBot *bot, const CNavArea *from, const Vector &fromPos, const CNavArea *to,
							   NavDirType dir, Vector *crossPos) const
{
	from->ComputeClosestPointInPortal(to, dir, fromPos, crossPos);

	// move goal position into the goal area a bit to avoid running directly along the edge of an area against a wall,
	// etc don't do this unless area is against a wall - and what if our hull is wider than the area?
