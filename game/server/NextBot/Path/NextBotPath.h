...
{
public:
	virtual float operator()(CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder,
							 const CFuncElevator *elevator, float length) const = 0;
};
...
public:
	...
	struct Segment
	{
		...
		float
			curvature; // how much the path 'curves' at this point in the XY plane (0 = none, 1 = 180 degree doubleback)
	...
	virtual const Vector &GetPosition(float distanceFromStart, const Segment *start = NULL)
		const; // return a position on the path at the given distance from the path start
	virtual const Vector &GetClosestPosition(const Vector &pos, const Segment *start = NULL, float alongLimit = 0.0f)
		const; // return the closest point on the path to the given position
	...
	virtual CBaseCombatCharacter *GetSubject(
		void) const; // return the actor this path leads to, or NULL if there is no subject

	virtual const Path::Segment *GetCurrentGoal(
		void) const; // return current goal along the path we are trying to reach
	...
	virtual void MoveCursorToClosestPosition(
		const Vector &pos, SeekType type = SEEK_ENTIRE_PATH,
		float alongLimit = 0.0f) const; // Set cursor position to closest point on path to given position
	...
	virtual const Segment *NextSegment(
		const Segment *currentSegment) const; // return next segment of path, given current one
	virtual const Segment *PriorSegment(
		const Segment *currentSegment) const;		// return previous segment of path, given current one
	...
	virtual void OnPathChanged(INextBot *bot, ResultType result) {
	...
	bool Compute(INextBot *bot, CBaseCombatCharacter *subject, CostFunctor &costFunc, float maxPathLength = 0.0f,
				 bool includeGoalIfPathFails = true)
	{
		...
		bool pathResult = NavAreaBuildPath(startArea, subjectArea, &subjectPos, costFunc, &closestArea, maxPathLength,
										   bot->GetEntity()->GetTeamNumber());
	 ...
	 */
	template<typename CostFunctor>
	bool Compute(INextBot *bot, const Vector &goal, CostFunctor &costFunc, float maxPathLength = 0.0f,
				 bool includeGoalIfPathFails = true)
	{
		...
		//
		CNavArea *closestArea = NULL;
		bool pathResult = NavAreaBuildPath(startArea, goalArea, &goal, costFunc, &closestArea, maxPathLength,
										   bot->GetEntity()->GetTeamNumber());
	 ...
	 * goal criteria.
	 */
	virtual bool ComputeWithOpenGoal(INextBot *bot, const IPathCost &costFunc,
									 const IPathOpenGoalSelector &goalSelector, float maxSearchRadius = 0.0f)
	{
		...
		while(!CNavArea::IsOpenListEmpty())
		{
			...
			for(int i = 0; i < m_adjAreaIndex; ++i)
			{
				...
				if(maxSearchRadius > 0.0f &&
				   (newArea->GetCenter() - bot->GetEntity()->GetAbsOrigin()).IsLengthGreaterThan(maxSearchRadius))
					continue;
	 ...
	 */
	virtual void ComputeAreaCrossing(INextBot *bot, const CNavArea *from, const Vector &fromPos, const CNavArea *to,
									 NavDirType dir, Vector *crossPos) const;
