	...
	if(threat != m_pathThreat)
	{
		if(bot->IsDebugging(INextBot::PATH))
		{
			DevMsg("%3.2f: bot(#%d) Chase path threat changed (from %X to %X).\n", gpGlobals->curtime,
				   bot->GetEntity()->entindex(), m_pathThreat.Get(), threat);
		}
...
public:
	...
	float Cost(CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder)
	{
		...
		if(fromArea == NULL)
		{
			...
			else
			{
				...
				if(rangeToThreat < maxThreatRange)
				{
					...
					if(m_me->IsDebugging(INextBot::PATH))
					{
						NDebugOverlay::Line(m_me->GetEntity()->GetAbsOrigin(), m_threat->GetAbsOrigin(), 255, 0, 0,
											true, debugDeltaT);
					}
				}
			}
		...
		else
		{
			...
			Vector close;
			float t;
			CalcClosestPointOnLineSegment(m_threat->GetAbsOrigin(), area->GetCenter(), fromArea->GetCenter(), close,
										  &t);
			if(t < 0.0f)
			{
			...
			if(rangeToThreat < maxThreatRange)
			{
				...
				if(m_me->IsDebugging(INextBot::PATH))
				{
					NDebugOverlay::HorzArrow(fromArea->GetCenter(), area->GetCenter(), 5, 255 * dangerFactor, 0, 0, 255,
											 true, debugDeltaT);
