...
BEGIN_DATADESC(CAI_RappelBehavior)
	DEFINE_FIELD(m_bWaitingToRappel, FIELD_BOOLEAN), DEFINE_FIELD(m_bOnGround, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hLine, FIELD_EHANDLE), DEFINE_FIELD(m_vecRopeAnchor, FIELD_POSITION_VECTOR),
END_DATADESC();
...
void CRopeAnchor::Spawn()
{
	...
	m_hRope =
		CRopeKeyframe::CreateWithSecondPointDetached(this, -1, flDist, RAPPEL_ROPE_WIDTH, "cable/cable.vmt", 5, true);
...
void CAI_RappelBehavior::SetDescentSpeed()
{
	...
	trace_t tr;
	AI_TraceLine(GetOuter()->GetAbsOrigin(), GetOuter()->GetAbsOrigin() - Vector(0, 0, 8192), MASK_SHOT, GetOuter(),
...
void CAI_RappelBehavior::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_HIT_GROUND:
			...
			if(GetOuter()->GetGroundEntity() != NULL && GetOuter()->GetGroundEntity()->IsNPC() &&
			   GetOuter()->GetGroundEntity()->m_iClassname == GetOuter()->m_iClassname)
			{
				// Although I tried to get NPC's out from under me, I landed on one. Kill it, so long as it's the same
				// type of character as me.
				variant_t val;
				val.SetFloat(0);
...
void CAI_RappelBehavior::BeginRappel()
{
	...
	UTIL_TraceEntity(GetOuter(), GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 4096), MASK_SHOT, GetOuter(),
					 COLLISION_GROUP_NONE, &tr);
