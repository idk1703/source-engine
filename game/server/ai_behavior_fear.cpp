...
BEGIN_DATADESC(CAI_FearBehavior)
	DEFINE_FIELD(m_flTimeToSafety, FIELD_TIME), DEFINE_FIELD(m_flTimePlayerLastVisible, FIELD_TIME),
		DEFINE_FIELD(m_hSafePlaceHint, FIELD_EHANDLE), DEFINE_FIELD(m_hMovingToHint, FIELD_EHANDLE),
		DEFINE_EMBEDDED(m_SafePlaceMoveMonitor), DEFINE_FIELD(m_flDeferUntil, FIELD_TIME),
END_DATADESC();
			...
			switch(GetOuter()->GetTaskInterrupt())
			{
				case 0: // Find the hint node
				{
					...
					if(pHint == NULL)
					{
						...
						m_flDeferUntil =
							gpGlobals->curtime + 3.0f; // Don't bang the hell out of this behavior. If we don't find a
													   // node, take a short break and run regular AI.
					}
					else
					{
				...
				case 1: // Do the pathfinding.
				{
					...
					if(GetNavigator()->SetGoal(goal) == false)
					{
						...
						AI_NavGoal_t nearGoal( GOALTYPE_LOCATION_NEAREST_NODE, m_hRallyPoint->GetAbsOrigin(),
...
void CAI_FearBehavior::GatherConditions()
{
	...
	if(pPlayer != NULL &&
	   GetAbsOrigin().DistToSqr(pPlayer->GetAbsOrigin()) >= Square(ai_fear_player_dist.GetFloat() * 1.5f))
	{
...
void CAI_FearBehavior::BuildScheduleTestBits()
{
	...
	if(GetOuter()->GetState() != NPC_STATE_SCRIPT)
	{
		...
		if(!IsCurSchedule(SCHED_FEAR_MOVE_TO_SAFE_PLACE_RETRY, false) &&
		   !IsCurSchedule(SCHED_FEAR_MOVE_TO_SAFE_PLACE, false))
		{
			GetOuter()->SetCustomInterruptCondition(
				GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_FEAR_SEPARATED_FROM_PLAYER));
		}
	}
}
...
CAI_Hint *CAI_FearBehavior::FindFearWithdrawalDest()
{
	...
	hintCriteria.SetFlag(bits_HINT_NODE_VISIBLE_TO_PLAYER |
						 bits_HINT_NOT_CLOSE_TO_ENEMY /*| bits_HINT_NODE_IN_VIEWCONE | bits_HINT_NPC_IN_NODE_FOV*/);
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_FearBehavior)
	...
	//===============================================
	//===============================================
	DEFINE_SCHEDULE(SCHED_FEAR_MOVE_TO_SAFE_PLACE,
	...
	DEFINE_SCHEDULE(SCHED_FEAR_MOVE_TO_SAFE_PLACE_RETRY,
	...
	//===============================================
	//===============================================
	DEFINE_SCHEDULE(SCHED_FEAR_STAY_IN_SAFE_PLACE,
