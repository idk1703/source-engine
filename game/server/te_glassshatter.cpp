...
// Input  : *name -
//-----------------------------------------------------------------------------
CTEShatterSurface::CTEShatterSurface(const char *name) : CBaseTempEntity(name)
{
	m_vecOrigin.Init();
	m_vecAngles.Init();
	m_vecForce.Init();
...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD), SendPropVector(SENDINFO(m_vecAngles), -1, SPROP_COORD),
	SendPropVector(SENDINFO(m_vecForce), -1, SPROP_COORD), SendPropVector(SENDINFO(m_vecForcePos), -1, SPROP_COORD),
	SendPropFloat(SENDINFO(m_flWidth), 0, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_flHeight), 0, SPROP_NOSCALE),
	SendPropFloat(SENDINFO(m_flShardSize), 0, SPROP_NOSCALE), SendPropInt(SENDINFO(m_nSurfaceType), 2, SPROP_UNSIGNED),
...
void TE_ShatterSurface(IRecipientFilter &filter, float delay, const Vector *pos, const QAngle *angle,
					   const Vector *force, const Vector *forcepos, float width, float height, float shardsize,
