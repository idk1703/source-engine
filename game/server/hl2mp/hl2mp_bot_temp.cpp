...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove(CHL2MP_Player *fakeclient, const QAngle &viewangles, float forwardmove, float sidemove,
	...
	if(pBot->IsAlive() && (pBot->GetSolid() == SOLID_BBOX))
	{
		...
		if(!pBot->IsEFlagSet(EFL_BOT_FROZEN) && pBot->m_iHealth == 100)
		{
			...
			while(--maxtries >= 0)
			{
				...
				UTIL_TraceHull(vecSrc, vecEnd, VEC_HULL_MIN_SCALED(pBot), VEC_HULL_MAX_SCALED(pBot), MASK_PLAYERSOLID,
