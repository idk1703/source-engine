	...
	for(node = 0; node < m_pNetwork->m_iNumNodes; node++)
	{
		...
		if(iPreviousNodeBinding != wcIDs.InvalidIndex())
		{
			...
			DevWarning("   AI node %d is associated with Hammer node %d, but %d is already bound to node %d\n", node,
					   GetEditOps()->m_pNodeIndexTable[node], GetEditOps()->m_pNodeIndexTable[node],
					   wcIDs[iPreviousNodeBinding]);
		}
		else
		{
...
void CAI_NetworkManager::SaveNetworkGraph( void )
{
	...
	{
		filesystem->FPrintf ( file, "Location      %4f,%4f,%4f\n",m_pAInode[node]->GetOrigin().x,
m_pAInode[node]->GetOrigin().y, m_pAInode[node]->GetOrigin().z ); for (int hull =0;hull<NUM_HULLS;hull++)
		{
...
bool CAI_NetworkManager::IsAIFileCurrent(const char *szMapName)
{
...

	{
		...
		if(!V_stricmp(szLoweredGameDir, "hl2") || !V_stricmp(szLoweredGameDir, "episodic") ||
		   !V_stricmp(szLoweredGameDir, "ep2") || !V_stricmp(szLoweredGameDir, "portal") ||
		   !V_stricmp(szLoweredGameDir, "lostcoast") || !V_stricmp(szLoweredGameDir, "hl1"))
		{
...
void CAI_NetworkManager::DelayedInit(void)
{
	...
	if(!g_ai_norebuildgraph.GetInt())
	{
		...
		else if(!m_bDontSaveGraph && (!CAI_NetworkManager::NetworksLoaded() || engine->IsInEditMode()))
		{
			...
			DevMsg("Node Graph out of Date. Rebuilding... (%d, %d, %d)\n", (int)m_bDontSaveGraph,
				   (int)!CAI_NetworkManager::NetworksLoaded(), (int)engine->IsInEditMode());
...
CAI_Node *CAI_NetworkEditTools::FindAINodeNearestFacing(const Vector &origin, const Vector &facing, float threshold,
														int nNodeType)
{
	...
	for(int node = 0; node < aiNet->NumNodes(); node++)
	{
		if(aiNet->GetNode(node)->GetType() != NODE_DELETED)
		{
			...
			if(nNodeType == NODE_ANY || nNodeType == aiNet->GetNode(node)->GetType())
			{
				// Make vector to node
				...
				if(dot > bestDot)
				{
					...
					trace_t tr;
					AI_TraceLine(origin, aiNet->GetNode(node)->GetPosition(m_iHullDrawNum), MASK_BLOCKLOS, NULL,
			...
			for(int node = startDrawNode; node < endDrawNode; node++)
			{
				for(int link = 0; link < pAINode[node]->NumLinks(); link++)
				{
					...
					if(pAINode[node]->GetLinkByIndex(link)->DestNodeID(node) < node)
					{
						...
						Vector srcPos =
							pAINode[pAINode[node]->GetLinkByIndex(link)->m_iSrcID]->GetPosition(m_iHullDrawNum);
						Vector desPos =
							pAINode[pAINode[node]->GetLinkByIndex(link)->m_iDestID]->GetPosition(m_iHullDrawNum);
						...
						if(pAINode[node]->GetLinkByIndex(link)->m_iAcceptedMoveTypes[m_iHullDrawNum] &
						   bits_CAP_MOVE_FLY)
						{
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 100, 255, 255, 20,
														flDrawDuration);
						}
						   ...
						   bits_CAP_MOVE_CLIMB)
						{
							...
							if(pAINode[pAINode[node]->GetLinkByIndex(link)->m_iSrcID]->GetOrigin() !=
							   pAINode[pAINode[node]->GetLinkByIndex(link)->m_iDestID]->GetOrigin())
							{
							...
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 255, 0, 255, 20,
														flDrawDuration);
						}
						...
						{
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 0, 255, 50, 20,
														flDrawDuration);
						}

						else if(pAINode[node]->GetLinkByIndex(link)->m_iAcceptedMoveTypes[m_iHullDrawNum] &
								bits_CAP_MOVE_JUMP)
						{
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 0, 0, 255, 20,
														flDrawDuration);
						}
					}
				}
		...
		for(int node = startDrawNode; node < endDrawNode; node++)
		{
			...
			if(pAINode[node]->GetType() != NODE_DELETED)
			{
					...
					else
					{
						for(int link = 0; link < pAINode[node]->NumLinks(); link++)
						{
							...
							if(pAINode[node]->GetLinkByIndex(link)->DestNodeID(node) < node)
							{
								...
								int moveTypes =
									pAINode[node]->GetLinkByIndex(link)->m_iAcceptedMoveTypes[m_iHullDrawNum];
								...
								else if((m_debugNetOverlays & bits_debugOverlayFlyConnections) &&
								...
								else if((m_debugNetOverlays & bits_debugOverlayJumpConnections) &&
								...
								{ // Dark red if this hull can't use
									...
									{
										if(pAINode[node]->GetLinkByIndex(link)->m_iAcceptedMoveTypes[i] &
										   ~bits_CAP_MOVE_JUMP)
										{
											isJump = false;
											break;
				...
				if(m_debugNetOverlays & bits_debugOverlayNodes)
				{
					...
					if(m_debugNetOverlays & bits_debugOverlayVisibility && m_iVisibilityNode != NO_NODE)
					{
						// ---------------------------------------------------
						//  If network needs rebuilding do so before display
						// --------------------------------------------------
					...
					if(m_debugNetOverlays & bits_debugOverlayGraphConnect && m_iGConnectivityNode != NO_NODE)
					{
						// ---------------------------------------------------
						//  If network needs rebuilding do so before display
						// --------------------------------------------------
					...
					{
						Vector offsetDir = 12.0 * Vector(cos(DEG2RAD(pAINode[node]->GetYaw())),
														 sin(DEG2RAD(pAINode[node]->GetYaw())), flDrawDuration);
						NDebugOverlay::Line(nodePos, nodePos + offsetDir, r, g, b, false, flDrawDuration);
					}

					if(pAINode[node]->GetHint())
					{
						NDebugOverlay::Box(nodePos, Vector(-7, -7, -7), Vector(7, 7, 7), 255, 255, 0, 0,
										   flDrawDuration);
					}
					...
					if(m_debugNetOverlays & bits_debugOverlayNodesLev2)
					{
						...
						if(m_pNodeIndexTable)
							msg.sprintf("%i (wc:%i; z:%i)", node, m_pNodeIndexTable[pAINode[node]->GetId()],
...
void CAI_NetworkBuilder::FloodFillZone(CAI_Node **ppNodes, CAI_Node *pNode, int zone)
{
	...
	{
		CAI_Link *pLink = pNode->GetLinkByIndex(link);
		CAI_Node *pLinkedNode =
			(pLink->m_iDestID == pNode->GetId()) ? ppNodes[pLink->m_iSrcID] : ppNodes[pLink->m_iDestID];
...
void CAI_NetworkBuilder::InitClimbNodePosition(CAI_Network *pNetwork, CAI_Node *pNode)
{
	...
	AI_TraceHull(posOnLadder, posOnLadder + Vector(0, 0, -37), NAI_Hull::Mins(HULL_SMALL_CENTERED),
				 NAI_Hull::Maxs(HULL_SMALL_CENTERED), MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &trace);
	...
	for(int i = 0; i < 3; i++)
	{
		...
		//  Check outward
		// ----------------
		AI_TraceLine(posOnLadder, origin, MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &trace);
...
void CAI_NetworkBuilder::InitGroundNodePosition(CAI_Network *pNetwork, CAI_Node *pNode)
{
	...
	for(int hull = 0; hull < NUM_HULLS; hull++)
	{
		...
		AI_TraceHull(origin, origin + Vector(0, 0, -384), mins, maxs, MASK_NPCSOLID_BRUSHONLY, NULL,
					 COLLISION_GROUP_NONE, &tr);
...
void CAI_NetworkBuilder::InitNodePosition(CAI_Network *pNetwork, CAI_Node *pNode)
{
	...
	else if(pNode->m_eNodeType == NODE_GROUND)
	{
		...
		if(pNode->m_flVOffset[HULL_SMALL_CENTERED] < -100)
		{
			...
			DevWarning("ERROR: Node %.0f %.0f %.0f, WC ID# %i, is either too low (fell through floor) or too high "
					   "(>100 units above floor)\n",
...
void CAI_NetworkBuilder::InitVisibility(CAI_Network *pNetwork, CAI_Node *pNode)
{
	...
	for(int testnode = 0; testnode < pNetwork->NumNodes(); testnode++)
	{
		...
		if(!isVisible)
		{
			AI_TraceLine(srcPos + Vector(0, 0, 70), destPos + Vector(0, 0, 70), MASK_NPCWORLDSTATIC, NULL,
						 COLLISION_GROUP_NONE, &tr);
			if(!tr.startsolid && tr.fraction == 1.0)
			{
				isVisible = true;
		...
		if ( tr.fraction != 1.0 )
		{
			...
			// there is a solid_bsp ent in the way of these two nodes, so we must record several things about in order
		to keep
			// track of it in the pathfinding code, as well as through save and restore of the node graph. ANY data that
		is manipulated
			// as part of the process of adding a LINKENT to a connection here must also be done in
		CGraph::SetGraphPointers, where reloaded
...
void CAI_NetworkBuilder::InitNeighbors(CAI_Network *pNetwork, CAI_Node *pNode)
{
	...
	for(int checknode = 0; checknode < pNetwork->NumNodes(); checknode++)
	{
		...
		for(int testnode = 0; testnode < pNetwork->NumNodes(); testnode++)
		{
			...
			// ----------------------------------------------------------
			if((pCheckNode->GetOrigin() == pNode->GetOrigin() && pNode->GetType() == NODE_CLIMB &&
				pCheckNode->GetType() == NODE_CLIMB) ||
			   (pTestNode->GetOrigin() == pNode->GetOrigin() && pNode->GetType() == NODE_CLIMB &&
				pTestNode->GetType() == NODE_CLIMB) ||
			   (pTestNode->GetOrigin() == pCheckNode->GetOrigin() && pCheckNode->GetType() == NODE_CLIMB &&
				pTestNode->GetType() == NODE_CLIMB))
			{
				continue;
			}
			...
			if(DotProduct(vec2DirToCheckNode, vec2DirToTestNode) >= tolerance)
			{
				...
				{
					DebugConnectMsg(pNode->m_iID, checknode,
									"      Revoking neighbor status to to closer redundant link %d\n", testnode);
					m_NeighborsTable[pNode->m_iID].Clear(checknode);
				}
				else
				{
					DebugConnectMsg(pNode->m_iID, testnode,
									"      Revoking neighbor status to to closer redundant link %d\n", checknode);
...
static bool IsInLineForClimb(const Vector &srcPos, const Vector &srcFacing, const Vector &destPos,
							 const Vector &destFacing)
{
#ifdef DEBUG
	...
	if(CalcDistanceToLine2D(destPos.AsVector2D(), srcPos.AsVector2D(), srcPos.AsVector2D() + srcFacing.AsVector2D()) >
	   0.01)
		return false;
...
int CAI_NetworkBuilder::ComputeConnection(CAI_Node *pSrcNode, CAI_Node *pDestNode, Hull_t hull)
{
	...
	if(pSrcNode->m_eNodeType == NODE_AIR || pDestNode->GetType() == NODE_AIR)
	{
		...
		if(pSrcNode->m_eNodeType == NODE_AIR && pDestNode->GetType() == NODE_AIR)
		{
			AI_TraceHull(pSrcNode->GetOrigin(), pDestNode->GetOrigin(), NAI_Hull::Mins(hull), NAI_Hull::Maxs(hull),
	...
	else if((pSrcNode->m_eNodeType == NODE_CLIMB) && (pDestNode->GetType() == NODE_CLIMB))
	{
		...
		{
			AI_TraceHull(srcPos, destPos, NAI_Hull::Mins(hull), NAI_Hull::Maxs(hull), MASK_NPCWORLDSTATIC, m_pTestHull,
						 COLLISION_GROUP_NONE, &tr);
			if(!tr.startsolid && tr.fraction == 1.0)
			{
		...
		{
			if(!IsInLineForClimb(srcPos, UTIL_YawToVector(pSrcNode->m_flYaw), destPos,
								 UTIL_YawToVector(pDestNode->m_flYaw)))
			{
				Assert(!IsInLineForClimb(destPos, UTIL_YawToVector(pDestNode->m_flYaw), srcPos,
										 UTIL_YawToVector(pSrcNode->m_flYaw)));
	...
	else if((pSrcNode->m_eNodeType == NODE_GROUND) || (pDestNode->GetType() == NODE_GROUND))
	{
		...
		// if (hull == 0)
		//	DevMsg("from %.1f %.1f %.1f to %.1f %.1f %.1f\n", srcPos.x, srcPos.y, srcPos.z, destPos.x, destPos.y,
		//destPos.z );

		if(!fStandFailed)
		{
			fWalkFailed = !m_pTestHull->GetMoveProbe()->TestGroundMove(srcPos, destPos, MASK_NPCWORLDSTATIC,
																	   AITGM_IGNORE_INITIAL_STAND_POS, NULL);
			if(fWalkFailed)
		...
		// =============================================================================
		if(!fStandFailed && fWalkFailed && (pSrcNode->m_eNodeType == NODE_GROUND) &&
