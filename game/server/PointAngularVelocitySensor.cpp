...
BEGIN_DATADESC(CPointAngularVelocitySensor)
	...
	// Fields
	DEFINE_FIELD(m_hTargetEntity, FIELD_EHANDLE), DEFINE_KEYFIELD(m_flThreshold, FIELD_FLOAT, "threshold"),
		DEFINE_FIELD(m_nLastCompareResult, FIELD_INTEGER), DEFINE_FIELD(m_nLastFireResult, FIELD_INTEGER),
		DEFINE_FIELD(m_flFireTime, FIELD_TIME), DEFINE_KEYFIELD(m_flFireInterval, FIELD_FLOAT, "fireinterval"),
		DEFINE_FIELD(m_flLastAngVelocity, FIELD_FLOAT), DEFINE_FIELD(m_lastOrientation, FIELD_VECTOR),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnLessThan, "OnLessThan"), DEFINE_OUTPUT(m_OnLessThanOrEqualTo, "OnLessThanOrEqualTo"),
		DEFINE_OUTPUT(m_OnGreaterThan, "OnGreaterThan"),
		DEFINE_OUTPUT(m_OnGreaterThanOrEqualTo, "OnGreaterThanOrEqualTo"), DEFINE_OUTPUT(m_OnEqualTo, "OnEqualTo"),
		DEFINE_OUTPUT(m_AngularVelocity, "AngularVelocity"),

		DEFINE_KEYFIELD(m_vecAxis, FIELD_VECTOR, "axis"), DEFINE_KEYFIELD(m_bUseHelper, FIELD_BOOLEAN, "usehelper"),
...
float CPointAngularVelocitySensor::SampleAngularVelocity(CBaseEntity *pEntity)
{
	if(pEntity->GetMoveType() == MOVETYPE_VPHYSICS)
	{
		...
		if(pPhys != NULL)
		{
			...
			// HACKHACK: We don't expect a real 'delta' orientation here, just enough of an error estimate to tell if
			// this thing is trying to move, but failing.
...
BEGIN_DATADESC(CPointVelocitySensor)
	...
	// Fields
	DEFINE_FIELD(m_hTargetEntity, FIELD_EHANDLE), DEFINE_KEYFIELD(m_vecAxis, FIELD_VECTOR, "axis"),
		DEFINE_KEYFIELD(m_bEnabled, FIELD_BOOLEAN, "enabled"), DEFINE_FIELD(m_fPrevVelocity, FIELD_FLOAT),
