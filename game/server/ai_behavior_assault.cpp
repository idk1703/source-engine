...
BEGIN_DATADESC(CRallyPoint)
	DEFINE_KEYFIELD(m_AssaultPointName, FIELD_STRING, "assaultpoint"),
		...
		DEFINE_KEYFIELD(m_bIsUrgent, FIELD_BOOLEAN, "urgent"), DEFINE_FIELD(m_hLockedBy, FIELD_EHANDLE),
...
BEGIN_DATADESC(CAssaultPoint)
	DEFINE_KEYFIELD(m_AssaultHintGroup, FIELD_STRING, "assaultgroup"),
		...
		DEFINE_KEYFIELD(m_iStrictness, FIELD_INTEGER, "strict"),
		DEFINE_KEYFIELD(m_bForceCrouch, FIELD_BOOLEAN, "forcecrouch"),
		DEFINE_KEYFIELD(m_bIsUrgent, FIELD_BOOLEAN, "urgent"), DEFINE_FIELD(m_bInputForcedClear, FIELD_BOOLEAN),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnArrival, "OnArrival"), DEFINE_OUTPUT(m_OnAssaultClear, "OnAssaultClear"),
END_DATADESC();
...
BEGIN_DATADESC(CAI_AssaultBehavior)
	DEFINE_FIELD(m_hAssaultPoint, FIELD_EHANDLE), DEFINE_FIELD(m_hRallyPoint, FIELD_EHANDLE),
		DEFINE_FIELD(m_AssaultCue, FIELD_INTEGER), DEFINE_FIELD(m_ReceivedAssaultCue, FIELD_INTEGER),
		DEFINE_FIELD(m_bHitRallyPoint, FIELD_BOOLEAN), DEFINE_FIELD(m_bHitAssaultPoint, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bDiverting, FIELD_BOOLEAN), DEFINE_FIELD(m_flLastSawAnEnemyAt, FIELD_FLOAT),
		DEFINE_FIELD(m_flTimeDeferScheduleSelection, FIELD_TIME), DEFINE_FIELD(m_AssaultPointName, FIELD_STRING)
END_DATADESC();
...
int CAI_AssaultBehavior::DrawDebugTextOverlays(int text_offset)
{
	...
	{
		Q_snprintf(tempstr, sizeof(tempstr), "Assault Point: %s %s", STRING(m_AssaultPointName),
				   VecToString(m_hAssaultPoint->GetAbsOrigin()));
...
CAssaultPoint *CAI_AssaultBehavior::FindAssaultPoint(string_t iszAssaultPointName)
{
	...
	for(int i = 0; i < pAssaultPoints.Count(); i++)
	{
		...
		AI_TraceHull(pAssaultPoint->GetAbsOrigin(), pAssaultPoint->GetAbsOrigin(), pNPC->WorldAlignMins(),
					 pNPC->WorldAlignMaxs(), MASK_SOLID, pNPC, COLLISION_GROUP_NONE, &tr);
...
void CAI_AssaultBehavior::GatherConditions(void)
{
	...
	{
		if(m_hAssaultPoint && m_hAssaultPoint->HasSpawnFlags(SF_ASSAULTPOINT_CLEARONARRIVAL) &&
...
void CAI_AssaultBehavior::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		{
			// If we're at an assault point that can never be cleared, keep waiting forever (if it's the last point in
			// the assault)
			if(m_hAssaultPoint && !m_hAssaultPoint->HasSpawnFlags(SF_ASSAULTPOINT_CLEARONARRIVAL) &&
			   m_hAssaultPoint->m_bNeverTimeout && m_hAssaultPoint->m_NextAssaultPointName == NULL_STRING)
			{
				TaskComplete();
				return;
			}
		...
		case TASK_GET_PATH_TO_RALLY_POINT:
		{
			...
			if(GetNavigator()->SetGoal(goal) == false)
			{
				...
				AI_NavGoal_t nearGoal(GOALTYPE_LOCATION_NEAREST_NODE, m_hRallyPoint->GetAbsOrigin(), AIN_DEF_ACTIVITY,
									  256);
		...
		case TASK_GET_PATH_TO_ASSAULT_POINT:
		{
			...
			{
				// Try and get as close as possible otherwise
				AI_NavGoal_t nearGoal(GOALTYPE_LOCATION_NEAREST_NODE, m_hAssaultPoint->GetAbsOrigin(), AIN_DEF_ACTIVITY,
									  256);
				if(GetNavigator()->SetGoal(nearGoal, AIN_CLEAR_PREVIOUS_STATE))
				{
					// FIXME: HACK! The internal pathfinding is setting this without our consent, so override it!
...
void CAI_AssaultBehavior::RunTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_AWAIT_ASSAULT_TIMEOUT:
			if(m_hAssaultPoint)
			{
				if(m_hAssaultPoint->m_bInputForcedClear ||
				   (m_hAssaultPoint->m_bClearOnContact && HasCondition(COND_SEE_ENEMY)))
				{
			...
			if(GetOuter()->IsWaitFinished() &&
			   (pTask->iTask == TASK_WAIT_ASSAULT_DELAY || !m_hAssaultPoint->m_bNeverTimeout))
			{
				TaskComplete();
			}
			break;
		...
		case TASK_WAIT_FOR_MOVEMENT:
			if(ai_debug_assault.GetBool())
			{
				if(IsCurSchedule(SCHED_MOVE_TO_ASSAULT_POINT))
				{
					...
					NDebugOverlay::Box(GetNavigator()->GetGoalPos(), -Vector(10, 10, 10), Vector(10, 10, 10), 255, 0, 0,
									   8, 0.1);
				}
				else if(IsCurSchedule(SCHED_MOVE_TO_RALLY_POINT))
				{
					...
					NDebugOverlay::Box(GetNavigator()->GetGoalPos(), -Vector(10, 10, 10), Vector(10, 10, 10), 0, 255, 0,
									   8, 0.1);
				}
			}

			if(m_hAssaultPoint && (m_hAssaultPoint->m_bInputForcedClear ||
								   (m_hAssaultPoint->m_bClearOnContact && HasCondition(COND_SEE_ENEMY))))
			{
			...
			if(((!GetOuter()->DidChooseEnemy() && gpGlobals->curtime - GetOuter()->GetTimeEnemyAcquired() > 1) ||
				!GetOuter()->GetEnemy()))
			{
...
bool CAI_AssaultBehavior::UpdateForceCrouch(void)
{
	if(IsForcingCrouch())
	{
		...
		float flDistanceToTargetSqr = GetOuter()->GetAbsOrigin().DistToSqr(
			AssaultHasBegun() ? m_hAssaultPoint->GetAbsOrigin() : m_hRallyPoint->GetAbsOrigin());
...
bool CAI_AssaultBehavior::IsAllowedToDivert(void)
{
	...
	{
		if(m_hAssaultPoint->m_flAllowDiversionRadius == 0.0f ||
		   (m_bHitAssaultPoint && GetEnemy() != NULL &&
			GetEnemy()->GetAbsOrigin().DistToSqr(m_hAssaultPoint->GetAbsOrigin()) <=
				Square(m_hAssaultPoint->m_flAllowDiversionRadius)))
		{
...
void CAI_AssaultBehavior::BuildScheduleTestBits()
{
	...
	if(IsAllowedToDivert())
	{
		if(IsCurSchedule(SCHED_MOVE_TO_ASSAULT_POINT) || IsCurSchedule(SCHED_MOVE_TO_RALLY_POINT) ||
...
int CAI_AssaultBehavior::SelectSchedule()
{
	...
	if(!m_bHitAssaultPoint)
	{
		   ...
		   m_AssaultCue == CUE_DONT_WAIT)
		{
			...
			if(m_hRallyPoint && !m_hRallyPoint->IsExclusive())
			{
				...
				// Otherwise, we do not unlock this rally point until we are FINISHED or DEAD. It's exclusively our
				// chain of assault
		...
		else if(GetOuter()->GetEnemy() && !HasCondition(COND_CAN_RANGE_ATTACK1) &&
				!HasCondition(COND_CAN_RANGE_ATTACK2) && !HasCondition(COND_ENEMY_OCCLUDED))
		{
			return SCHED_COMBAT_FACE;
		}
		else
	...
	if((!GetEnemy() || HasCondition(COND_ENEMY_OCCLUDED)) &&
	   !GetOuter()->HasConditionsToInterruptSchedule(SCHED_WAIT_AND_CLEAR))
	{
	...
	if(OnStrictAssault())
	{
		...
		if(!HasCondition(COND_CAN_RANGE_ATTACK1) && !HasCondition(COND_CAN_RANGE_ATTACK2) &&
		   !HasCondition(COND_CAN_MELEE_ATTACK1) && !HasCondition(COND_CAN_MELEE_ATTACK2) &&
		   !HasCondition(COND_TOO_CLOSE_TO_ATTACK) && !HasCondition(COND_NOT_FACING_ATTACK))
		{
			return SCHED_WAIT_AND_CLEAR;
		}
	}
...
#ifdef HL2_EPISODIC
	// This ugly patch fixes a bug where Combine Soldiers on an assault would not shoot through glass, because of the
	// way that shooting through glass is implemented in their AI. (sjb)
