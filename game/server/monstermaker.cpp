...
BEGIN_DATADESC(CNPCSpawnDestination)
	DEFINE_KEYFIELD(m_ReuseDelay, FIELD_FLOAT, "ReuseDelay"), DEFINE_KEYFIELD(m_RenameNPC, FIELD_STRING, "RenameNPC"),
...
BEGIN_DATADESC(CBaseNPCMaker)
	...
	DEFINE_KEYFIELD(m_nMaxNumNPCs, FIELD_INTEGER, "MaxNPCCount"),
		...
		// Inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "Spawn", InputSpawnNPC), DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnAllSpawned, "OnAllSpawned"), DEFINE_OUTPUT(m_OnAllSpawnedDead, "OnAllSpawnedDead"),
		DEFINE_OUTPUT(m_OnAllLiveChildrenDead, "OnAllLiveChildrenDead"), DEFINE_OUTPUT(m_OnSpawnNPC, "OnSpawnNPC"),

		// Function Pointers
		DEFINE_THINKFUNC(MakerThink),

		DEFINE_FIELD(m_hIgnoreEntity, FIELD_EHANDLE), DEFINE_KEYFIELD(m_iszIngoreEnt, FIELD_STRING, "IgnoreEntity"),
END_DATADESC()
...
{
	trace_t tr;
	UTIL_TraceHull(vecLocation, vecLocation + Vector(0, 0, 1), NAI_Hull::Mins(HULL_HUMAN), NAI_Hull::Maxs(HULL_HUMAN),
		...
		if(count)
		{
			...
			for(int i = 0; i < count; i++)
			{
				...
				if((pList[i]->GetSolidFlags() & FSOLID_NOT_SOLID) == false)
				{
					...
					trace_t tr;
					UTIL_TraceHull(GetAbsOrigin() + Vector(0, 0, 2), GetAbsOrigin() - Vector(0, 0, 8192),
								   NAI_Hull::Mins(HULL_HUMAN), NAI_Hull::Maxs(HULL_HUMAN), MASK_NPCSOLID,
								   m_hIgnoreEntity, COLLISION_GROUP_NONE, &tr);
...
void CBaseNPCMaker::ChildPostSpawn(CAI_BaseNPC *pChild)
{
	...
	{
		trace_t tr;
		UTIL_TraceHull(pChild->GetAbsOrigin(), pChild->GetAbsOrigin(), pChild->WorldAlignMins(),
					   pChild->WorldAlignMaxs(), MASK_NPCSOLID, pChild, COLLISION_GROUP_NONE, &tr);
		// NDebugOverlay::Box( pChild->GetAbsOrigin(), pChild->WorldAlignMins(), pChild->WorldAlignMaxs(), 0, 255, 0,
		// 32, 5.0 );
...
BEGIN_DATADESC(CTemplateNPCMaker)
	...
	DEFINE_KEYFIELD(m_iszTemplateName, FIELD_STRING, "TemplateName"),
		DEFINE_KEYFIELD(m_flRadius, FIELD_FLOAT, "radius"), DEFINE_FIELD(m_iszTemplateData, FIELD_STRING),
	...
	if(m_iszDestinationGroup != NULL_STRING)
	{
		...
		if(!pDestination)
		{
			DevMsg(2, "%s '%s' failed to find a valid spawnpoint in destination group: '%s'\n", GetClassname(),
				   STRING(GetEntityName()), STRING(m_iszDestinationGroup));
			return;
		}
	}
...
bool CTemplateNPCMaker::PlaceNPCInLine(CAI_BaseNPC *pNPC)
{
	...
	for(int i = 0; i < 10; i++)
	{
		UTIL_TraceHull(vecPlace, vecPlace + Vector(0, 0, 10), pNPC->GetHullMins(), pNPC->GetHullMaxs(), MASK_SHOT, pNPC,
