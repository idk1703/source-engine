...
public:
	...
	static CSNavNode *GetFirst(void)
	{
		return m_list;
	}
	...
	void ConnectTo(CSNavNode *node,
				   NavDirType dir); ///< create a connection FROM this node TO the given node, in the given direction
...
private:
	...
	unsigned char m_visited; ///< flags for automatic node generation. If direction bit is clear, that direction hasn't
							 ///< been explored yet.
	CSNavNode *m_parent; ///< the node prior to this in the search, which we pop back to when this node's search is done
						 ///< (a stack)
