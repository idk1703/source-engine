...
ConVar sv_debugroundstats("sv_debugroundstats", "0", 0,
						  "A temporary variable that will print extra information about stats upload which may be "
						  "useful in debugging any problems.");
...
{
	CFastTimer totalTimer, purchasesAndDeathsStatTimer, weaponsStatBuildTimer, weaponsStatSubmitTimer, submitTimer,
		cleanupTimer;
	...
	for(int iWeapon = 0; iWeapon < WEAPON_MAX; ++iWeapon)
	{
		...
		if(!pWeaponName || !pWeaponName[0] ||
		   (m_weaponStats[iWeapon][0].shots == 0 && m_weaponStats[iWeapon][1].shots == 0))
			continue;
	...
	if(sv_debugroundstats.GetBool())
	{
		...
		Msg("UploadRoundStats completed. %.3f msec. Breakdown:\n a: %.3f msec\n b: %.3f msec\n c: %.3f msec\n d: %.3f "
			"msec\n e: %.3f msec\n objects: %d %d %d %d\n commit: %.3fms\n write: %.3fms.\n\n",
			totalTimer.GetDuration().GetMillisecondsF(), purchasesAndDeathsStatTimer.GetDuration().GetMillisecondsF(),
			weaponsStatBuildTimer.GetDuration().GetMillisecondsF(),
			weaponsStatSubmitTimer.GetDuration().GetMillisecondsF(), submitTimer.GetDuration().GetMillisecondsF(),
			cleanupTimer.GetDuration().GetMillisecondsF(), iPurchases, iDeathData, iWeaponData, listCount,
...
void CCSGameStats::Event_PlayerKilled_PreWeaponDrop(CBasePlayer *pPlayer, const CTakeDamageInfo &info)
{
	...
	if(pAttacker && pCSPlayer && pAttacker == info.GetInflictor() &&
	   pAttacker->GetTeamNumber() != pCSPlayer->GetTeamNumber())
	{
...
{
	// The most stats we can send at once is the max message size minus the header, divided by the total size of each
	// stat.
...
void CCSGameStats::DumpMatchWeaponMetrics()
{
	...
	// filesystem->FPrintf(hLogFile, "%s\n", "WeaponId, Mode, Cost, Bullets, CycleTime, TotalShots, TotalHits,
...
void CCSGameStats::Event_PlayerKilledOther(CBasePlayer *pAttacker, CBaseEntity *pVictim, const CTakeDamageInfo &info)
{
	...
	if(pPlayerVictim && pPlayerAttacker && pPlayerAttacker->GetTeamNumber() == TEAM_TERRORIST &&
	   CSGameRules()->m_bBombPlanted)
	{
		float bombCheckDistSq =
			AchievementConsts::KillEnemyNearBomb_MaxDistance * AchievementConsts::KillEnemyNearBomb_MaxDistance;
	...
	// if they were damaged by more than one person that must mean that someone else did damage before the killer
	// finished them off.
	...
	if(pPlayerAttacker && !pPlayerAttacker->IsBot() && weaponId != WEAPON_NONE)
	{
		...
		bool bUsingSniper = (weaponId == WEAPON_AWP || weaponId == WEAPON_SCOUT || weaponId == WEAPON_SG550 ||
	...
	if(pPlayer)
	{
		...
		for(int i = 0; i < ARRAYSIZE(ServerStatBasedAchievements); ++i)
		{
			if(ServerStatBasedAchievements[i].statId == statId)
			{
				...
				if(ServerStatBasedAchievements[i].mapFilter != NULL &&
				   V_strcmp(gpGlobals->mapname.ToCStr(), ServerStatBasedAchievements[i].mapFilter) != 0)
					continue;

				bool bWasMet = ServerStatBasedAchievements[i].IsMet(stats.statsCurrentRound[statId] - iDelta,
																	stats.statsCurrentMatch[statId] - iDelta);
				bool bIsMet = ServerStatBasedAchievements[i].IsMet(stats.statsCurrentRound[statId],
																   stats.statsCurrentMatch[statId]);
				if(!bWasMet && bIsMet)
				{
