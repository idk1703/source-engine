	...
	bool PrintCStrikeEvent(IGameEvent *event) // print Mod specific logs
	{
		...
		if(FStrEq(eventName, "player_hurt"))
		{
			   ...
			   (!isTeamAttack && (detail & LOG_DETAIL_ENEMY_ATTACKS)))
			{
				...
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" attacked \"%s<%i><%s><%s>\" with \"%s\" (damage \"%d\") "
							   "(damage_armor \"%d\") (health \"%d\") (armor \"%d\") (hitgroup \"%s\")\n",
							   ...
							   event->GetInt("dmg_health"), event->GetInt("dmg_armor"), event->GetInt("health"),
							   event->GetInt("armor"), hitgroupStr);
			}
			return true;
		}
