...
public:
	...
	bool GetPointAlongPath(float distAlong,
						   Vector *pointOnPath) const; ///< return point a given distance along the path - if distance
...
private:
	...
	bool BuildTrivialPath(const Vector &start,
						  const Vector &goal); ///< utility function for when start and goal are in the same area
...
public:
	...
	void SetImprov(CImprovLocomotor *improv)
	{
		m_improv = improv;
	}
	void SetPath(CCSNavPath *path)
	{
		m_path = path;
	}
...
#define DONT_AVOID_OBSTACLES false
	...
	void FeelerReflexAdjustment(Vector *goalPosition,
								float height = -1.0f); ///< adjust goal position if "feelers" are touched
...
private:
	...
	int FindOurPositionOnPath(Vector *close,
							  bool local) const; ///< return the closest point to our current position on current path
	int FindPathPoint(float aheadRange, Vector *point,
					  int *prevIndex); ///< compute a point a fixed distance ahead along our path.
