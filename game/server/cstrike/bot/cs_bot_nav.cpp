...
//--------------------------------------------------------------------------------------------------------------
#if DEBUG_BREAKABLES
static void DrawOutlinedQuad(const Vector &p1, const Vector &p2, const Vector &p3, const Vector &p4, int r, int g,
							 int b, float duration)
{
...
//--------------------------------------------------------------------------------------------------------------
CBaseEntity *CheckForEntitiesAlongSegment(const Vector &start, const Vector &end, const Vector &mins,
										  const Vector &maxs, CPushAwayEnumerator *enumerator)
{
	...
	if(entity)
	{
		DrawOutlinedQuad(start + mins, start + maxs, end + maxs, end + mins, 255, 0, 0,
						 bot_debug_breakable_duration.GetFloat());
	}
	else
	{
...
CBaseEntity *CCSBot::FindEntitiesOnPath(float distance, CPushAwayEnumerator *enumerator, bool checkStuck)
{
	...
	if(isDegeneratePath)
	{
		entity = CheckForEntitiesAlongSegment(WorldSpaceCenter(), m_goalPosition + Vector(0, 0, HalfHumanHeight), mins,
											  maxs, enumerator);
#if DEBUG_BREAKABLES
		if(entity)
		{
			NDebugOverlay::HorzArrow(WorldSpaceCenter(), m_goalPosition, 6, 0, 0, 255, 255, true,
									 bot_debug_breakable_duration.GetFloat());
		}
#endif // DEBUG_BREAKABLES
	}
	else
	{
		...
		for(int i = startIndex - 1; i < m_pathLength - 1; ++i)
		{
			...
			entity = CheckForEntitiesAlongSegment(start + Vector(0, 0, HalfHumanHeight),
												  end + Vector(0, 0, HalfHumanHeight), mins, maxs, enumerator);
			if(entity)
			{
#if DEBUG_BREAKABLES
			...
			if(m_path[i].ladder && !IsOnLadder() &&
			   distance > NearBreakableCheckDist) // don't try to break breakables on the other end of a ladder
				break;
			...
			if(i != startIndex - 1 && m_path[i + 1].ladder)
			{
				...
				start = (m_path[i + 1].how == GO_LADDER_DOWN) ? m_path[i + 1].ladder->m_bottom
															  : m_path[i + 1].ladder->m_top;
				end = m_path[i + 1].pos;
				...
				{
#if DEBUG_BREAKABLES
					NDebugOverlay::HorzArrow(start, end, 4, 0, 255, 0, 255, true,
											 bot_debug_breakable_duration.GetFloat());
#endif // DEBUG_BREAKABLES
					break;
				}
	...
	*/
	...
	if(IsAttacking())
	{
		...
		if(IsUsingKnife() && m_enemy != NULL)
		{
			...
			breakable =
				CheckForEntitiesAlongSegment(WorldSpaceCenter(), m_enemy->WorldSpaceCenter(), mins, maxs, &enumerator);
			if(breakable)
			{
#if DEBUG_BREAKABLES
				NDebugOverlay::HorzArrow(WorldSpaceCenter(), m_enemy->WorldSpaceCenter(), 6, 0, 0, 255, 255, true,
										 bot_debug_breakable_duration.GetFloat());
#endif // DEBUG_BREAKABLES
				...
				// break it (again, don't wait: we don't have ammo, since we're using the knife, and we're looking
				// mostly at it anyway)
				PrimaryAttack();
			}
		}
	...
	if(IsLookingAtSpot(PRIORITY_HIGH) && m_lookAtSpotAttack)
	{
		...
		else if(GetActiveWeapon() && GetActiveWeapon()->m_flNextPrimaryAttack <= gpGlobals->curtime)
		{
			...
			if(!shouldShoot)
			{
				...
				trace_t result;
				UTIL_TraceLine(EyePosition(), EyePosition() + FarBreakableCheckDist * aimDir, MASK_PLAYERSOLID, this,
...
void CCSBot::MoveTowardsPosition(const Vector &pos)
{
	...
	//
	if((m_lastKnownArea == NULL || !(m_lastKnownArea->GetAttributes() & NAV_MESH_NO_JUMP)) && !IsOnLadder())
	{
		float ground;
...
void CCSBot::DrawApproachPoints(void) const
{
	for(int i = 0; i < m_approachPointCount; ++i)
	{
		if(TheCSBots()->GetElapsedRoundTime() >=
		   m_approachPoint[i].m_area->GetEarliestOccupyTime(OtherTeam(GetTeamNumber())))
...
bool CCSBot::IsAtEnemySpawn(void) const
{
	...
	const char *spawnName =
		(GetTeamNumber() == TEAM_TERRORIST) ? "info_player_counterterrorist" : "info_player_terrorist";

	// check if we are at any of the enemy's spawn points
	for(spot = gEntList.FindEntityByClassname(NULL, spawnName); spot;
		spot = gEntList.FindEntityByClassname(spot, spawnName))
	{
