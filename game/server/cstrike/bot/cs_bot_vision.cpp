 ...
 */
void CCSBot::SetLookAt(const char *desc, const Vector &pos, PriorityType pri, float duration, bool clearIfClose,
					   float angleTolerance, bool attack)
{
	if(IsBlind())
		return;
...
#define MAX_DANGER_SPOTS 16
			...
			FOR_EACH_VEC(m_spotEncounter->spots, it)
			{
				...
				if(spotOrder->spot->GetArea())
				{
					if(TheCSBots()->GetElapsedRoundTime() <
					   spotOrder->spot->GetArea()->GetEarliestOccupyTime(OtherTeam(GetTeamNumber())))
					{
						continue;
					}
				}
			...
			if(dangerSpotCount)
			{
				...
				SetLookAt("Encounter Spot", dangerSpot[which]->GetPosition() + Vector(0, 0, HalfHumanHeight),
						  PRIORITY_LOW, 0.2f, true, 10.0f);
...
bool CCSBot::BendLineOfSight(const Vector &eye, const Vector &target, Vector *bend, float angleLimit) const
{
	...
	for(float angle = angleInc; angle <= angleLimit; angle += angleInc)
	{
		...
		for(int side = 0; side < 2; ++side)
		{
			...
			if(doDebug && cv_bot_debug.GetBool() && IsLocalPlayerWatchingMe())
			{
				NDebugOverlay::Line(eye, eye + visibleLength * ray, colorSet[color].r, colorSet[color].g,
									colorSet[color].b, true, debugDuration);
			}
			...
			for(float bendLength = startLength; bendLength <= visibleLength; bendLength += bendStepSize)
			{
				...
				if(doDebug && cv_bot_debug.GetBool() && IsLocalPlayerWatchingMe())
				{
					NDebugOverlay::Line(bendPoint, result.endpos, colorSet[color].r / 2, colorSet[color].g / 2,
										colorSet[color].b / 2, true, debugDuration);
				}
