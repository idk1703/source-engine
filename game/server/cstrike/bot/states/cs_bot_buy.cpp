...
ConVar bot_randombuy("bot_randombuy", "0", FCVAR_CHEAT,
...
void BuyState::OnEnter(CCSBot *me)
{
	...
	if(me->GetTeamNumber() == TEAM_CT)
	{
		...
		{
			// CT's sometimes buy defuse kits in the bomb scenario (except in career mode, where the player should
			// defuse)
	...
	if(TheCSBots()->AllowPistols())
	{
		...
		if(me->Weapon_GetSlot(WEAPON_SLOT_PISTOL))
		{
			...
			if(HasDefaultPistol(me))
			{
				...
				if(TheCSBots()->AllowShotguns() == false && TheCSBots()->AllowSubMachineGuns() == false &&
				   TheCSBots()->AllowRifles() == false && TheCSBots()->AllowMachineGuns() == false &&
				   TheCSBots()->AllowTacticalShield() == false && TheCSBots()->AllowSnipers() == false)
				{
...
void BuyState::OnUpdate(CCSBot *me)
{
	...
	if(!me->IsInBuyZone())
	{
		...
		CONSOLE_ECHO("%s bot spawned outside of a buy zone (%d, %d, %d)\n",
					 (me->GetTeamNumber() == TEAM_CT) ? "CT" : "Terrorist", (int)me->GetAbsOrigin().x,
					 (int)me->GetAbsOrigin().y, (int)me->GetAbsOrigin().z);
		return;
	}
	...
	if(gpGlobals->curtime - me->GetStateTimestamp() > buyInterval)
	{
		...
		if(m_prefIndex < me->GetProfile()->GetWeaponPreferenceCount() && bot_randombuy.GetBool() == false)
		{
			...
			Q_snprintf(weaponPreferenceName, sizeof(weaponPreferenceName), "weapon_%s",
					   me->GetProfile()->GetWeaponPreferenceAsString(m_prefIndex));
			if(me->Weapon_OwnsThisType(
				   weaponPreferenceName)) // Prefs and buyalias use the short version, this uses the long
			{
		...
		if(!me->HasPrimaryWeapon() && (isPreferredAllDisallowed || !me->GetProfile()->HasPrimaryPreference()))
		{
			...
			else
			{
				...
				BuyInfo *masterPrimary =
					(me->GetTeamNumber() == TEAM_TERRORIST) ? primaryWeaponBuyInfoT : primaryWeaponBuyInfoCT;
