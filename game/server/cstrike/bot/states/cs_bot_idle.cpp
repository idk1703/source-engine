...
void IdleState::OnUpdate(CCSBot *me)
{
	...
	if(me->GetGameState()->IsRoundOver())
	{
		...
		{
			const CCSBotManager::Zone *zone =
				TheCSBots()->GetClosestZone(me->GetLastKnownArea(), PathCost(me, FASTEST_ROUTE));
	...
	switch(TheCSBots()->GetScenario())
	{
			...
			if(me->GetTeamNumber() == TEAM_TERRORIST)
			{
				if(me->GetGameState()->IsBombPlanted())
				{
					if(me->GetGameState()->GetPlantedBombsite() != CSGameState::UNKNOWN)
					{
						...
						const CCSBotManager::Zone *zone =
							TheCSBots()->GetZone(me->GetGameState()->GetPlantedBombsite());
						if(zone)
						{
					...
					else
					{
						...
						// move to bombsite - if we reach it, we'll update its cleared status, causing us to select
						// another
				...
				else if(me->HasC4())
				{
					...
					else if(TheCSBots()->IsTimeToPlantBomb())
					{
						...
						const CCSBotManager::Zone *zone =
							TheCSBots()->GetClosestZone(me->GetLastKnownArea(), PathCost(me));
						if(zone)
						{
				...
				if(me->GetGameState()->IsBombPlanted())
				{
					...
					else
					{
						...
						for(int z = 0; z < TheCSBots()->GetZoneCount(); ++z)
						{
							...
							float dist = NavAreaTravelDistance(
								me->GetLastKnownArea(),
								TheNavMesh->GetNearestNavArea(TheCSBots()->GetZone(z)->m_center), cost);
		...
		case CCSBotManager::SCENARIO_RESCUE_HOSTAGES:
		{
			if(me->GetTeamNumber() == TEAM_TERRORIST)
			{
				...
				else if(me->GetGameState()->HaveSomeHostagesBeenTaken() ||
						me->GetGameState()->AreAllHostagesBeingRescued())
				{
					campHostages = false;
				}
				else
				{
					...
					const float campZoneChance =
						100.0f * (TheCSBots()->GetElapsedRoundTime() - me->GetSafeTime()) / 120.0f;
			...
			else // CT ---------------------------------------------------------------------------------
			{
				...
				if(hostage)
				{
					...
					if(zone)
					{
						...
						float toHostage = NavAreaTravelDistance(
							me->GetLastKnownArea(), TheNavMesh->GetNearestNavArea(GetCentroid(hostage)), cost);
