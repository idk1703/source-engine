		...
		else if(gpGlobals->curtime >= m_nextDodgeStateTimestamp)
		{
			...
			else
			{
				...
				do
				{
					...
					if(m_firstDodge && me->GetProfile()->GetSkill() < 0.5f && RandomFloat(0, 100) < jumpChance &&
					   !me->IsNotMoving())
...
void AttackState::OnUpdate(CCSBot *me)
{
	...
	if(weapon)
	{
		if(weapon->GetWeaponID() == WEAPON_C4 || weapon->GetWeaponID() == WEAPON_HEGRENADE ||
		   weapon->GetWeaponID() == WEAPON_FLASHBANG || weapon->GetWeaponID() == WEAPON_SMOKEGRENADE)
		{
			me->EquipBestWeapon();
		}
	}
	...
	if(m_retreatTimer.IsElapsed())
	{
		...
		if(isPinnedDown || (me->CanSeeSniper() && !me->IsSniper()) || (me->IsOutnumbered() && m_isCoward) ||
	...
	if(!me->IsEnemyVisible() && me->GetTimeSinceAttacked() < hurtRecentlyTime && me->GetAttacker() &&
