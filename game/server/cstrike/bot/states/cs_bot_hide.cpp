...
void HideState::OnUpdate(CCSBot *me)
{
	...
	if(!me->IsReloading())
	{
		...
		switch(TheCSBots()->GetScenario())
		{
			case CCSBotManager::SCENARIO_DEFUSE_BOMB:
			{
				...
				{
					// if we are just holding position (due to a radio order) and the bomb has just planted, go defuse
					// it
					if(me->GetTask() == CCSBot::HOLD_POSITION && TheCSBots()->IsBombPlanted() &&
					...
					if(me->IsDoingScenario() && me->GetTask() != CCSBot::GUARD_BOMB_DEFUSER &&
				...
				else // TERRORIST
				{
					...
					if(TheCSBots()->GetBombDefuser())
					{
						...
						{
							// if we are nearby, attack, otherwise move to the bomb (which will cause us to attack when
							// we see defuser)
		...
		if(!me->IsReloading() && !isSettledInSniper && me->GetDisposition() == CCSBot::ENGAGE_AND_INVESTIGATE)
		{
	...
	else
	{
		...
		if(range < atDist)
		{
			...
			{
				UTIL_TraceLine(me->EyePosition(),
							   me->EyePosition() + 1000.0f * Vector(BotCOS(angle), BotSIN(angle), 0.0f),
							   MASK_PLAYERSOLID, me, COLLISION_GROUP_NONE, &result);
