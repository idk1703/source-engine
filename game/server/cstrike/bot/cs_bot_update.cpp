	...
	if(IsAimingAtEnemy())
	{
		...
		if(m_enemy != NULL && m_enemy->IsAlive())
		{
			...
			else
			{
				...
				// if we put this back, make sure you only bend the ray ONCE and keep the bent spot - dont continually
				// recompute
...
void CCSBot::Update(void)
{
	...
	if(m_enemy != NULL)
	{
		...
		{
			// check LOS to current enemy (chest & head), in case he's dead (GetNearestEnemy() only returns live
			// players) note we're not checking FOV - once we've acquired an enemy (which does check FOV), assume we
	...
	/// @todo This doesn't work if we are restricted to just knives and sniper rifles because we cant use the rifle at
	/// close range
	...
	if(cv_bot_auto_follow.GetBool() && TheCSBots()->GetElapsedRoundTime() > earliestAutoFollowTime &&
	   GetProfile()->GetTeamwork() > minAutoFollowTeamwork && CanAutoFollow() && !IsBusy() && !IsFollowing() &&
	   !IsBlind() && !GetGameState()->IsAtPlantedBombsite())
	{
	...
	if(!IsAttacking() && m_inhibitWaitingForHostageTimer.IsElapsed())
	{
		...
		if((GetTask() == COLLECT_HOSTAGES || GetTask() == RESCUE_HOSTAGES) &&
		   GetRangeToFarthestEscortedHostage() > waitForHostageRange)
		{
...
public:
	...
	bool operator()(CBasePlayer *player)
	{
		...
		{
			CFmtStr msg;
			player->EntityText(0, msg.sprintf("%3.0f", m_me->GetTravelDistanceToPlayer((CCSPlayer *)player)), 0.1f);
...
void CCSBot::DebugDisplay(void) const
{
	...
	NDebugOverlay::ScreenText(0.5f, 0.34f, msg.sprintf("Skill: %d%%", (int)(100.0f * GetProfile()->GetSkill())), 255,
							  255, 255, 150, duration);

	if(m_pathLadder)
	{
		NDebugOverlay::ScreenText(0.5f, 0.36f, msg.sprintf("Ladder: %d", m_pathLadder->GetID()), 255, 255, 255, 150,
								  duration);
	}
	...
	if(m_isAttacking)
	{
		NDebugOverlay::ScreenText(0.5f, 0.4f, msg.sprintf("ATTACKING: %s", GetBotEnemy()->GetPlayerName()), 255, 0, 0,
								  255, duration);
	}
	else
	{
		...
		NDebugOverlay::ScreenText(0.5f, 0.44f, msg.sprintf("Disposition: %s", GetDispositionName()), 100, 100, 255, 255,
								  duration);
	...
	{
		const char *string =
			msg.sprintf("LookAt: %s (%s)", m_lookAtDesc,
						(m_lookAtSpotState == LOOK_TOWARDS_SPOT) ? "LOOK_TOWARDS_SPOT" : "LOOK_AT_SPOT");

		NDebugOverlay::ScreenText(0.5f, 0.60f, string, 255, 255, 0, 150, duration);
	}

	NDebugOverlay::ScreenText(0.5f, 0.62f, msg.sprintf("Steady view = %s", HasViewBeenSteady(0.2f) ? "YES" : "NO"), 255,
							  255, 0, 150, duration);

	// show friend/foes I know of
	NDebugOverlay::ScreenText(0.5f, 0.64f, msg.sprintf("Nearby friends = %d", m_nearbyFriendCount), 100, 255, 100, 150,
							  duration);
	NDebugOverlay::ScreenText(0.5f, 0.66f, msg.sprintf("Nearby enemies = %d", m_nearbyEnemyCount), 255, 100, 100, 150,
							  duration);

	if(m_lastNavArea)
	{
		NDebugOverlay::ScreenText(0.5f, 0.68f,
								  msg.sprintf("Nav Area: %d (%s)", m_lastNavArea->GetID(), m_szLastPlaceName.Get()),
								  255, 255, 255, 150, duration);
		/*
	...
	if(noisePos)
	{
		...
		NDebugOverlay::VertArrow(*noisePos + Vector(0, 0, size), *noisePos, size / 4.0f, 255, 255, 0, 0, true,
								 duration);
	}
			...
			FOR_EACH_VEC(m_spotEncounter->spots, it)
			{
				...
				if(order->spot->GetArea())
				{
					if(TheCSBots()->GetElapsedRoundTime() <
					   order->spot->GetArea()->GetEarliestOccupyTime(OtherTeam(GetTeamNumber())))
					{
						continue;
					}
				}
	...
	NDebugOverlay::ScreenText( 0.3f, 0.66f, msg.sprintf( "Punch angle pitch = %3.2f", angles.x ), 255, 255, 0, 150,
	duration );
	*/
	...
	/*
	   ...
	   ); Vector arrow( size * (float)cos( m_lookAheadAngle * M_PI/180.0f ), size * (float)sin( m_lookAheadAngle *
	   M_PI/180.0f ), 0.0f );
