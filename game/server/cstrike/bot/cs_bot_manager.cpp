 ...
 */
void BotArgumentsFromArgv(const CCommand &args, const char **name, CSWeaponType *weaponType,
						  BotDifficultyType *difficulty, int *team = NULL, bool *all = NULL)
{
...
CON_COMMAND_F(bot_kill,
			  "bot_kill <all> <t|ct> <type> <difficulty> <name> - Kills a specific bot, or all bots, matching the "
			  "given criteria.",
			  FCVAR_GAMEDLL)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
...
CON_COMMAND_F(bot_kick,
			  "bot_kick <all> <t|ct> <type> <difficulty> <name> - Kicks a specific bot, or all bots, matching the "
			  "given criteria.",
			  FCVAR_GAMEDLL)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
 ...
 */
bool CCSBotManager::BotAddCommand(int team, bool isFromConsole, const char *profileName, CSWeaponType weaponType,
								  BotDifficultyType difficulty)
{
	...
	if(CSGameRules()->TeamStacked(team, TEAM_UNASSIGNED))
	{
		if(isFromConsole)
		{
			Msg("Could not add bot to the game: Team is stacked (to disable this check, set mp_autoteambalance to "
				"zero, increase mp_limitteams, and restart the round).\n");
		}
		return false;
	}
...
void CCSBotManager::MaintainBotQuota(void)
{
	...
	bool isRoundInProgress = CSGameRules()->m_bFirstConnected && !TheCSBots()->IsRoundOver() &&
	...
	if(botsInGame > 0 && desiredBotCount == botsInGame && CSGameRules()->m_bFirstConnected)
	{
		if(CSGameRules()->GetRoundElapsedTime() < 20.0f) // new bots can still spawn during this time
		{
			if(mp_autoteambalance.GetBool())
			{
				...
				if(!FStrEq(cv_bot_join_team.GetString(), "T") && !FStrEq(cv_bot_join_team.GetString(), "CT"))
				{
...
void CCSBotManager::ExtractScenarioData(void)
{
	...
	{
		for(entity = gEntList.FindEntityByClassname(NULL, "info_player_start"); entity && !FNullEnt(entity->edict());
		...
		if(cv_bot_debug.GetInt() == 5)
		{
			if(m_zone[i].m_isBlocked)
				DevMsg("%.1f: Zone %d, area %d (%.0f %.0f %.0f) is blocked from spawn area %d (%.0f %.0f %.0f)\n",
					   gpGlobals->curtime, i, m_zone[i].m_area[0]->GetID(), m_zone[i].m_area[0]->GetCenter().x,
					   m_zone[i].m_area[0]->GetCenter().y, m_zone[i].m_area[0]->GetCenter().z, spawnArea->GetID(),
...
void CCSBotManager::OnRoundFreezeEnd(IGameEvent *event)
{
	...
	m_NavBlockedEvent.Enable(
		false); // don't listen to nav_blocked events - there could be several, and we don't have bots pathing
	CUtlVector<CNavArea *> &transientAreas = TheNavMesh->GetTransientAreas();
	for(int i = 0; i < transientAreas.Count(); ++i)
	{
		CNavArea *area = transientAreas[i];
...
CBaseEntity *CCSBotManager::GetRandomSpawn(int team) const
{
	...
	{
		// collect T spawns
		for(spot = gEntList.FindEntityByClassname(NULL, "info_player_terrorist"); spot;
	...
	{
		// collect CT spawns
		for(spot = gEntList.FindEntityByClassname(NULL, "info_player_counterterrorist"); spot;
...
static bool CheckAreaAgainstAllZoneAreas(CNavArea *queryArea)
{
	...
	for(int zoneIndex = 0; zoneIndex < goalZoneCount; zoneIndex++)
	{
		...
		for(int areaIndex = 0; areaIndex < zoneAreaCount; areaIndex++)
		{
			...
			// We need to be connected to every area in the zone, since we don't know what other code might pick for an
			// area
...
CON_COMMAND_F(nav_check_connectivity,
			  "Checks to be sure every (or just the marked) nav area can get to every goal area for the map (hostages "
			  "or bomb site).",
			  FCVAR_CHEAT)
{
