...
#define DECLARE_CSBOTMANAGER_EVENT_LISTENER(EventClass, EventName) \
	DECLARE_BOTMANAGER_EVENT_LISTENER(TheCSBots, EventClass, EventName)
...
public:
	...
	bool IsServerActive(void) const
	{
	...
	virtual bool IsImportantPlayer(
		CCSPlayer *player) const; ///< return true if player is important to scenario (VIP, bomb carrier, etc)

	void ExtractScenarioData(
		void); ///< search the map entities to determine the game scenario and define important zones
	...
	GameScenarioType GetScenario(void) const
	{
	...
	float GetBombPlantTimestamp(void) const
	{
	...
	CCSPlayer *GetBombDefuser(void) const
	{
		return m_bombDefuser;
	}								   ///< return the player currently defusing the bomb, or NULL
	...
	float GetRadioMessageTimestamp(
		RadioType event, int teamID) const; ///< return the last time the given radio message was sent for given team
	float GetRadioMessageInterval(RadioType event,
	...
	{
		m_lastSeenEnemyTimestamp = gpGlobals->curtime;
	}
	...
	{
		return gpGlobals->curtime - m_roundStartTimestamp;
	} ///< return the elapsed time since the current round began

	bool AllowRogues(void) const
	{
		return cv_bot_allow_rogues.GetBool();
	}
	bool AllowPistols(void) const
	{
		return cv_bot_allow_pistols.GetBool();
	}
	bool AllowShotguns(void) const
	{
		return cv_bot_allow_shotguns.GetBool();
	}
	bool AllowSubMachineGuns(void) const
	{
		return cv_bot_allow_sub_machine_guns.GetBool();
	}
	bool AllowRifles(void) const
	{
		return cv_bot_allow_rifles.GetBool();
	}
	bool AllowMachineGuns(void) const
	{
		return cv_bot_allow_machine_guns.GetBool();
	}
	bool AllowGrenades(void) const
	{
		return cv_bot_allow_grenades.GetBool();
	}
	bool AllowSnipers(void) const
	{
		return cv_bot_allow_snipers.GetBool();
	}
#ifdef CS_SHIELD_ENABLED
	bool AllowTacticalShield(void) const
	{
		return cv_bot_allow_shield.GetBool();
	}
#else
	bool AllowTacticalShield(void) const
	{
	...
	}												 ///< returns true if defense team has "decided" to rush this round
...
#define FROM_CONSOLE true
	bool BotAddCommand(int team, bool isFromConsole = false, const char *profileName = NULL,
					   CSWeaponType weaponType = WEAPONTYPE_UNKNOWN,
					   BotDifficultyType difficulty = NUM_DIFFICULTY_LEVELS); ///< process the "bot_add" console command
...
private:
	...
	CUtlVector<BotEventInterface *> m_commonEventListeners; // These event listeners fire often, and can be disabled for
															// performance gains when no bots are present.
