...
public:
	...
	CHostage *GetNearestFreeHostage(
		Vector *knowPos = NULL) const; ///< return the closest free hostage, and where we think it is (knowPos)
	...
	unsigned char ValidateHostagePositions(
		void); ///< update our knowledge with what we currently see - returns bitflag events
...
private:
	...
	BombState GetBombState(void) const
	{
		return m_bombState;
	}
