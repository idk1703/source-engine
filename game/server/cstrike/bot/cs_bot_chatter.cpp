...
void BotFollowMeme::Interpret(CCSBot *sender, CCSBot *receiver) const
{
	...
	float travelDistance =
		NavAreaTravelDistance(receiver->GetLastKnownArea(), TheNavMesh->GetNearestNavArea(senderOrigin), cost);
...
{
	// special cases
	if(GetType() == REPORT_MY_PLAN || GetType() == REPORT_REQUEST_HELP || GetType() == REPORT_CRITICAL_EVENT ||
	...
	if(gpGlobals->curtime > m_nextTime)
	{
		...
		else
		{
			...
			switch(m_statement[m_index].context)
			{
				...
				{
					static const char *speak[] = {"NoEnemiesLeft", "OneEnemyLeft", "TwoEnemiesLeft",
												  "ThreeEnemiesLeft"};
		...
		if(phrase)
		{
			...
			else
			{
				...
				if(sayIt)
				{
					...
					else if ( g_engfuncs.pfnPlayClientVoice && TheBotPhrases->GetOutputType(
					me->GetProfile()->GetVoiceBank() ) == BOT_CHATTER_VOICE )
					{
						me->GetChatter()->ResetRadioSilenceDuration();
...
void BotChatterInterface::OnDeath(void)
{
	if(IsTalking())
	{
		   ...
		   m_me->GetChatter()->GetVerbosity() == BotChatterInterface::NORMAL)
		{
			...
			{
				/*
				if ( g_engfuncs.pfnPlayClientVoice && TheBotPhrases->GetOutputType( m_me->GetProfile()->GetVoiceBank() )
				== BOT_CHATTER_VOICE )
				{
					g_engfuncs.pfnPlayClientVoice( m_me->entindex() - 1,
				pain->GetSpeakable(m_me->GetProfile()->GetVoiceBank()) );
					m_me->GetChatter()->ResetRadioSilenceDuration();
				}
				else
				*/
				{
					m_me->SpeakAudio(pain->GetSpeakable(m_me->GetProfile()->GetVoiceBank()), 0.0f,
									 m_me->GetProfile()->GetVoicePitch());
				}
			}
		}
	}
...
void BotChatterInterface::ReportingIn(void)
{
	...
	else
	{
		...
		if(m_me->GetEnemyDeathTimestamp() < recentTime &&
		   m_me->GetEnemyDeathTimestamp() >= m_me->GetTimeSinceLastSawEnemy() + 0.5f)
		{
