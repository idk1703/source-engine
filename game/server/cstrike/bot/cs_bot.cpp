...
bool CCSBot::Jump(bool mustJump)
{
	...
	bool inCrouchJumpArea = (m_lastKnownArea && (m_lastKnownArea->GetAttributes() & NAV_MESH_CROUCH) &&
...
void CCSBot::Touch(CBaseEntity *other)
{
	...
	if(other->IsPlayer())
	{
		...
		if(m_avoid != NULL)
		{
			unsigned int avoidPri =
				TheCSBots()->GetPlayerPriority(static_cast<CBasePlayer *>(static_cast<CBaseEntity *>(m_avoid)));
...
bool CCSBot::IsHurrying(void) const
{
	...
	if(ctrl->GetScenario() == CCSBotManager::SCENARIO_RESCUE_HOSTAGES && GetTeamNumber() == TEAM_TERRORIST &&
...
public:
	...
	bool operator()(CNavArea *area)
	{
		...
		FOR_EACH_VEC((*pSpots), it)
		{
			...
			if(UTIL_IsVisibleToTeam(spot->GetPosition() + Vector(0, 0, HalfHumanHeight),
									OtherTeam(m_me->GetTeamNumber())))
				continue;
...
{
	static const char *name[NUM_DISPOSITIONS] = {"ENGAGE_AND_INVESTIGATE", "OPPORTUNITY_FIRE", "SELF_DEFENSE",
												 "IGNORE_ENEMIES"};
...
{
	static const char *name[EXCELLENT - TERRIBLE + 1] = {"TERRIBLE", "BAD",	 "NEGATIVE", "NEUTRAL",
 ...
 */
void CCSBot::BuildUserCmd(CUserCmd &cmd, const QAngle &viewangles, float forwardmove, float sidemove, float upmove,
