	...
	for(int i = 1; i < m_pathLength; ++i)
	{
		...
		else if(to->how == GO_LADDER_UP) // to get to next area, must go up a ladder
		{
			...
			for(it = 0; it < pLadders->Count(); ++it)
			{
				...
				if(ladder->m_topForwardArea == to->area || ladder->m_topLeftArea == to->area ||
...
bool CCSBot::UpdateLadderMovement(void)
{
	...
	else if(m_pathLadderState == APPROACH_ASCENDING_LADDER || m_pathLadderState == APPROACH_DESCENDING_LADDER ||
			m_pathLadderState == ASCEND_LADDER || m_pathLadderState == DESCEND_LADDER ||
			m_pathLadderState == DISMOUNT_ASCENDING_LADDER || m_pathLadderState == MOVE_TO_DESTINATION)
	{
		if(m_isStuck)
		{
	...
	switch(m_pathLadderState)
	{
		...
		case DESCEND_LADDER:
		{
			...
			const Vector &ladderPos =
				(m_pathLadderState == MOUNT_ASCENDING_LADDER || m_pathLadderState == ASCEND_LADDER)
...
case MOVE_TO_DESTINATION:
	...
	{
		// this is causing hang-up problems when crawling thru ducts/windows that drop off into rooms (they fail the
		// "falling" check)
		return true;
	...
	int CCSBot::FindPathPoint(float aheadRange, Vector *point, int *prevIndex)
	{
		...
		for(i = startIndex; i < m_pathLength; ++i)
		{
			...
			if(i < m_pathLength && (m_path[i].ladder || m_path[i].area->GetAttributes() & NAV_MESH_JUMP))
				break;
		...
		// if position found is too close to us, or behind us, force it farther down the path so we don't stop and
		// wiggle
		if(!isCorner)
		{
			...
			if(DotProduct2D(toPoint, initDir.AsVector2D()) < 0.0f || toPoint.IsLengthLessThan(epsilon))
			{
				...
				for(i = startIndex; i < m_pathLength; ++i)
				{
					...
					if(m_path[i].ladder || m_path[i].area->GetAttributes() & NAV_MESH_JUMP ||
	...
	void CCSBot::SetPathIndex(int newIndex)
	{
		...
		else
		{
			...
			if(m_pathIndex < m_pathLength && m_pathIndex >= 2)
				m_spotEncounter = m_path[m_pathIndex - 1].area->GetSpotEncounter(m_path[m_pathIndex - 2].area,
																				 m_path[m_pathIndex].area);
			else
				m_spotEncounter = NULL;
		...
		if(m_pathIndex >= m_pathLength - 1)
		{
			...
			if(d.IsLengthLessThan(walkRange))
			{
				...
				if(d.IsLengthLessThan(closeEpsilon))
				{
					...
					/// @todo We should push and pop walk state here, in case we want to continue walking after reaching
					/// goal
					if(allowSpeedChange)
						Run();
	...
	else
	{
		...
		else if(m_pathIndex < m_pathLength)
		{
			...
			if(!m_path[m_pathIndex].ladder && !IsNearJump() && toGoal.AsVector2D().IsLengthLessThan(lookAheadRange))
			{
			...
			if(m_pathIndex < m_pathLength - 1)
			{
				if(m_path[m_pathIndex + 1].pos.z - GetFeetZ() > JumpCrouchHeight)
				{
					...
					for(int i = m_pathIndex; i <= m_pathIndex + 1; ++i)
					{
						...
						{
							// if we're going up a ladder, and we're within reach of the ladder bottom, we haven't
							// fallen
