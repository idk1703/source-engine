...
{
public:
	char *GetSpeakable(int bankIndex, float *duration = NULL)
		const; ///< return a random speakable and its duration in seconds that meets the current criteria
	...
	const unsigned int GetPlace(void) const
	{
		return m_place;
	}
...
public:
	...
	bool Initialize(const char *filename, int bankIndex); ///< initialize phrase system from database file for a
														  ///< specific voice bank (0 is the default voice bank)
	...
	float GetPlaceStatementInterval(
		Place where) const; ///< return time last statement of given type was emitted by a teammate for the given place
	void ResetPlaceStatementInterval(
		Place where); ///< set time of last statement of given type was emitted by a teammate for the given place
...
public:
	...
	BotStatement *GetActiveStatement(
		void); ///< returns the statement that is being spoken, or is next to be spoken if no-one is speaking now
...
#define IS_PLAN true
	...
	bool SeesAtLeastOneEnemy(void) const
	{
		return m_seeAtLeastOneEnemy;
	}
...
private:
	...
	static CountdownTimer
		m_encourageTimer; ///< timer to know when we can "encourage" the human player again - shared by all bots
};
