...
void CCSHLTVDirector::CreateShotFromEvent(CHLTVGameEvent *event)
{
	...
	if(!Q_strcmp("hostage_rescued", name) || !Q_strcmp("hostage_hurt", name) || !Q_strcmp("hostage_follows", name) ||
	...
	else if(!Q_strcmp("bomb_pickup", name) || !Q_strcmp("bomb_dropped", name) || !Q_strcmp("bomb_planted", name) ||
...
const char **CCSHLTVDirector::GetModEvents()
{
	...
	static const char *s_modevents[] = {"hltv_status", "hltv_chat", "player_connect", "player_disconnect",
										"player_team", "player_info", "server_cvar", "player_death", "player_chat",
										"round_start", "round_end",
										// additional CS:S events:
										"bomb_planted", "bomb_defused", "hostage_killed", "hostage_hurt", NULL};
