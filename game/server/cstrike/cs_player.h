...
enum
{
	...
	NAME_CHANGE_HISTORY_INTERVAL =
		600, // no more than NAME_CHANGE_HISTORY_SIZE name changes can be made in this many seconds
};
...
#define DHM_CONNECT_CLEAR                                                                                      \
	(DHF_HOSTAGE_SEEN_FAR | DHF_HOSTAGE_SEEN_NEAR | DHF_HOSTAGE_USED | DHF_HOSTAGE_INJURED | DHF_FRIEND_SEEN | \
	 DHF_ENEMY_SEEN | DHF_FRIEND_INJURED | DHF_ENEMY_KILLED | DHF_AMMO_EXHAUSTED | DHF_IN_TARGET_ZONE |        \
	 DHF_IN_RESCUE_ZONE | DHF_IN_ESCAPE_ZONE | DHF_IN_VIPSAFETY_ZONE | DHF_HOSTAGE_CTMOVE | DHF_SPEC_DUCK)
...
public:
	...
	virtual bool IsBeingGivenItem() const
	{
		return m_bIsBeingGivenItem;
	}
...
public:
	...
	void FireBullet(Vector vecSrc, const QAngle &shootAngles, float flDistance, int iPenetration, int iBulletType,
					int iDamage, float flRangeModifier, CBaseEntity *pevAttacker, bool bDoEffects, float xSpread,
					float ySpread);
	...
	ICSPlayerAnimState *GetPlayerAnimState()
	{
...
public:
	...
	void RunPlayerMove(const QAngle &viewangles, float forwardmove, float sidemove, float upmove,
	...
	void HintMessage(const char *pMessage, bool bDisplayIfDead,
					 bool bOverrideClientSettings = false); // Displays a hint message to the player
	...
	bool PickedUpDefuser()
	{
		return m_bPickedUpDefuser;
	}
	void SetDefusedWithPickedUpKit(bool bDefusedWithPickedUpKit)
	{
		m_bDefusedWithPickedUpKit = bDefusedWithPickedUpKit;
	}
	bool GetDefusedWithPickedUpKit()
	{
		return m_bDefusedWithPickedUpKit;
	}
	...
	bool IsBlindForAchievement(); // more stringent than IsBlind; more accurately represents when the player can see
								  // again
...
public:
	...
	CNetworkVar(int, m_iLastZoom); // after firing a shot, set the FOV to 90, and after showing the animation, bring the
								   // FOV back to last zoom level.
	...
	Vector m_vLastHitLocationObjectSpace; // position where last hit occured in space of the bone associated with the
										  // hitbox
	...
	void WieldingKnifeAndKilledByGun(bool bState)
	{
		m_bWieldingKnifeAndKilledByGun = bState;
	}
	bool WasWieldingKnifeAndKilledByGun()
	{
		return m_bWieldingKnifeAndKilledByGun;
	}

	// [dwenger] adding tracking for weapon used fun fact
	void PlayerUsedFirearm(CBaseCombatWeapon *pBaseWeapon);
	int GetNumFirearmsUsed()
	{
		return m_WeaponTypesUsed.Count();
	}
	...
	int m_iOldTeam;	  // Keep what team they were last on so we can allow joining spec and switching back to their real
					  // team
...
private:
	...
	// AutoBuy functions.
public:
	void AutoBuy(); // this should take into account what the player can afford and should buy the best equipment for
					// them.

	bool IsInAutoBuy(void)
	{
		return m_bIsInAutoBuy;
	}
	bool IsInReBuy(void)
	{
		return m_bIsInRebuy;
...
private:
	...
	void PrioritizeAutoBuyString(char *autobuyString,
								 const char *priorityString); // reorders the tokens in autobuyString based on the order
															  // of tokens in the priorityString.
...
public:
	...
	CUtlLinkedList<CDamageRecord *, int> &GetDamageGivenList()
	{
		return m_DamageGivenList;
	}
	CUtlLinkedList<CDamageRecord *, int> &GetDamageTakenList()
	{
		return m_DamageTakenList;
	}
...
private:
	...
	CNetworkVar(int, m_cycleLatch); // Every so often, we are going to transmit our cycle to the client to correct
									// divergence caused by PVS changes
...
public:
	...
	float m_firstKillBlindStartTime; // This is the start time of the blind effect during which we got our most recent
									 // kill.
	...
	void IncrementNumFollowers()
	{
		m_iNumFollowers++;
	}
	void DecrementNumFollowers()
	{
		m_iNumFollowers--;
		if(m_iNumFollowers < 0)
			m_iNumFollowers = 0;
	}
	int GetNumFollowers()
	{
		return m_iNumFollowers;
	}
	void SetIsRescuing(bool in_bRescuing)
	{
		m_bIsRescuing = in_bRescuing;
	}
	bool IsRescuing()
	{
		return m_bIsRescuing;
	}
	void SetInjuredAHostage(bool in_bInjured)
	{
		m_bInjuredAHostage = in_bInjured;
	}
	bool InjuredAHostage()
	{
		return m_bInjuredAHostage;
	}
	float GetBombPickuptime()
	{
		return m_bombPickupTime;
	}
	void SetBombPickupTime(float time)
	{
		m_bombPickupTime = time;
	}
	CCSPlayer *GetLastFlashbangAttacker()
	{
		return m_lastFlashBangAttacker;
	}
	void SetLastFlashbangAttacker(CCSPlayer *attacker)
	{
		m_lastFlashBangAttacker = attacker;
	}
	...
	bool GetKilledDefuser()
	{
		return m_bKilledDefuser;
	}
	bool GetKilledRescuer()
	{
		return m_bKilledRescuer;
	}
	int GetMaxGrenadeKills()
	{
		return m_maxGrenadeKills;
	}
	...
	bool WasNotKilledNaturally()
	{
		return m_wasNotKilledNaturally;
	}
...
private:
	CNetworkArray(bool, m_bPlayerDominated,
				  MAX_PLAYERS + 1); // array of state per other player whether player is dominating other players
	CNetworkArray(bool, m_bPlayerDominatingMe,
				  MAX_PLAYERS + 1); // array of state per other player whether other players are dominating this player
