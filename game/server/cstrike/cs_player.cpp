...
ConVar sv_max_usercmd_future_ticks(
	"sv_max_usercmd_future_ticks", "8", 0,
...
ConVar sv_legacy_grenade_damage(
	"sv_legacy_grenade_damage", "0", FCVAR_REPLICATED,
...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD | SPROP_CHANGES_OFTEN, 0.0f, HIGH_DEFAULT, SendProxy_Origin),
	SendPropVector(SENDINFO(m_vecRagdollOrigin), -1, SPROP_COORD), SendPropEHandle(SENDINFO(m_hPlayer)),
	SendPropModelIndex(SENDINFO(m_nModelIndex)), SendPropInt(SENDINFO(m_nForceBone), 8, 0),
	SendPropVector(SENDINFO(m_vecForce), -1, SPROP_NOSCALE), SendPropVector(SENDINFO(m_vecRagdollVelocity)),
	SendPropInt(SENDINFO(m_iDeathPose), ANIMATION_SEQUENCE_BITS, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_iDeathFrame), 5), SendPropInt(SENDINFO(m_iTeamNum), TEAMNUM_NUM_BITS, 0),
...
BEGIN_SEND_TABLE_NOBASE(CCSPlayer, DT_CSLocalPlayerExclusive)
	SendPropFloat(SENDINFO(m_flStamina), 14, 0, 0, 1400), SendPropInt(SENDINFO(m_iDirection), 1, SPROP_UNSIGNED),
...
SendPropExclude("DT_BaseAnimating", "m_flPoseParameter"), SendPropExclude("DT_BaseAnimating", "m_flPlaybackRate"),
	SendPropExclude("DT_BaseAnimating", "m_nSequence"), SendPropExclude("DT_BaseAnimating", "m_nNewSequenceParity"),
	SendPropExclude("DT_BaseAnimating", "m_nResetEventsParity"),
	SendPropExclude("DT_BaseAnimating", "m_nMuzzleFlashParity"), SendPropExclude("DT_BaseEntity", "m_angRotation"),
	...
	SendPropDataTable("csnonlocaldata", 0, &REFERENCE_SEND_TABLE(DT_CSNonLocalPlayerExclusive),
					  SendProxy_SendNonLocalDataTable),
	...
	SendPropInt(SENDINFO(m_iAccount), 16, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_bInBombZone), 1, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_bInBuyZone), 1, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_iClass), Q_log2(CS_NUM_CLASSES) + 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_ArmorValue), 8),
	...
	SendPropBool(SENDINFO(m_bHasShield)), SendPropBool(SENDINFO(m_bShieldDrawn)),
#endif
	SendPropBool(SENDINFO(m_bHasHelmet)), SendPropFloat(SENDINFO(m_flFlashDuration), 0, SPROP_NOSCALE),
	SendPropFloat(SENDINFO(m_flFlashMaxAlpha), 0, SPROP_NOSCALE),
	SendPropInt(SENDINFO(m_iProgressBarDuration), 4, SPROP_UNSIGNED),
	SendPropFloat(SENDINFO(m_flProgressBarStartTime), 0, SPROP_NOSCALE), SendPropEHandle(SENDINFO(m_hRagdoll)),
...
//-----------------------------------------------------------------------------
void CCSPlayer::RunPlayerMove(const QAngle &viewangles, float forwardmove, float sidemove, float upmove,
							  unsigned short buttons, byte impulse, float frametime)
{
	CUserCmd cmd;
...
void CCSPlayer::Spawn()
{
	...
	if(!engine->IsDedicatedServer() && CSGameRules()->m_iTotalRoundsPlayed < 2 && TheNavMesh->IsOutOfDate() &&
	...
	if(GetActiveWeapon() && !IsObserver())
	{
		...
		m_flNextAttack = gpGlobals->curtime; // Allow reloads to finish, since we're playing the deploy anim instead.
											 // This mimics goldsrc behavior, anyway.
	}
...
void CCSPlayer::UpdateAddonBits()
{
	...
	if(GetAmmoCount(GetAmmoDef()->Index(AMMO_TYPE_HEGRENADE)) && !dynamic_cast<CHEGrenade *>(GetActiveWeapon()))
	{
		iNewBits |= ADDON_HE_GRENADE;
	}

	if(GetAmmoCount(GetAmmoDef()->Index(AMMO_TYPE_SMOKEGRENADE)) && !dynamic_cast<CSmokeGrenade *>(GetActiveWeapon()))
	{
...
case CLASS_PLAYER_ALLY:
	...
	void CCSPlayer::PostThink()
	{
		...
		if(IsAlive() && m_cycleLatchTimer.IsElapsed())
		{
			...
			// Cycle is a float from 0 to 1.  We don't need to transmit a whole float for that.  Compress it in to a
			// small fixed point
	...
	int CCSPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
	{
		...
		if(bFriendlyFire && pInflictor->GetTeamNumber() == GetTeamNumber() && pInflictor != this &&
		   info.GetAttacker() != this)
		{
			...
			if(pCSAttacker)
			{
				if(!(pCSAttacker->m_iDisplayHistoryBits & DHF_FRIEND_INJURED))
				{
					...
					pCSAttacker->m_iDisplayHistoryBits |=
	DHF_FRIEND_INJURED
	;
	}
	   ...
	   info.GetAttacker() == this)
	{
		...
		if(m_iHealth - info.GetDamage() <= 0 && m_iHealth <= AchievementConsts::KillLowDamage_MaxHealthLeft)
		{
			...
			if(pAttacker && pAttacker->GetTeamNumber() != GetTeamNumber())
			{
				...
				{
					if(Q_strncmp(pAttacker->GetPlayerName(), m_DamageTakenList[i]->GetPlayerName(),
		...
		Q_snprintf(outputString, sizeof(outputString),
				   "%f: Player %s incoming %f damage from %s, type %s; applied %d health and %d armor\n",
				   gpGlobals->curtime, GetPlayerName(), inputInfo.GetDamage(), info.GetInflictor()->GetDebugName(),
				   dmgtype, m_lastDamageHealth, m_lastDamageArmor);
...
#ifndef _XBOX
		// CS would like their players to continue to update their LastArea since it is displayed in the hud voice chat
		// UI But we won't do the population tracking while dead.
	...
	void CCSPlayer::DropShield(void)
	{
		...
		CBaseAnimating *pShield =
			(CBaseAnimating *)CBaseEntity::Create("item_shield", WorldSpaceCenter(), GetLocalAngles());
	...
	void UTIL_CSRadioMessage(IRecipientFilter &filter, int iClient, int msg_dest, const char *msg_name,
							 const char *param1 = NULL, const char *param2 = NULL, const char *param3 = NULL,
	...
	void CCSPlayer::Radio(const char *pszRadioSound, const char *pszRadioText)
	{
		...
		if(pszRadioText)
		{
			...
			{
				UTIL_CSRadioMessage(filter, entindex(), HUD_PRINTTALK, "#Game_radio_location", GetPlayerName(),
	...
	void CCSPlayer::ApplyDeafnessEffect()
	{
		...
		//  effect: the total effect of combining all the sections, the whole of what the player hears from start to
		//  finish.
	...
	bool CCSPlayer::WantsLagCompensationOnEntity(const CBasePlayer *pPlayer, const CUserCmd *pCmd,
		...
		else if(FStrEq(pcmd, "drop"))
		{
			...
			if(pWeapon)
			{
				...
				if(!pWeapon->IsAPriorOwner(this))
				{
					...
					CCS_GameStats.IncrementStat(this, CSTAT_ITEMS_DROPPED_VALUE,
												pWeapon->GetCSWpnData().GetWeaponPrice());
				}
	...
	bool CCSPlayer::HandleCommand_JoinTeam(int team)
	{
		...
		// : a) There is another TEAM_CT player who is in the queue to be a VIP b) This player is dead
		...
		*/
		...
		if(mp->TeamStacked(
			   team,
			   GetTeamNumber())) // players are allowed to change to their own team so they can just change their model
		{
			// attempt to kick a bot to make room for this player
			bool madeRoom = false;
			if(cv_bot_auto_vacate.GetBool() && !IsBot())
	...
	*/
	...
	void CCSPlayer::GetIntoGame()
	{
		...
		if(!MPRules->FPlayerCanRespawn(this))
		{
			...
			// pev->flags		   &= ( FL_PROXY | FL_FAKECLIENT );	// clear flags, but keep proxy and bot flags that might
			// already be set pev->flags		   |= FL_CLIENT | FL_SPECTATOR; SetThink(PlayerDeathThink);
			if(!(m_iDisplayHistoryBits & DHF_SPEC_DUCK))
			{
				m_iDisplayHistoryBits |= DHF_SPEC_DUCK;
				HintMessage("#Spec_Duck", true, true);
	...
	CCSPlayerStateInfo *CCSPlayer::State_LookupInfo(CSPlayerState state)
	{
		...
		static CCSPlayerStateInfo playerStateInfos[] = {
			...
			{STATE_PICKINGTEAM, "STATE_PICKINGTEAM", &CCSPlayer::State_Enter_PICKINGTEAM, NULL,
			 &CCSPlayer::State_PreThink_OBSERVER_MODE},
			{STATE_PICKINGCLASS, "STATE_PICKINGCLASS", &CCSPlayer::State_Enter_PICKINGCLASS, NULL,
			 &CCSPlayer::State_PreThink_OBSERVER_MODE},
			{STATE_DEATH_ANIM, "STATE_DEATH_ANIM", &CCSPlayer::State_Enter_DEATH_ANIM, NULL,
			 &CCSPlayer::State_PreThink_DEATH_ANIM},
			{STATE_DEATH_WAIT_FOR_KEY, "STATE_DEATH_WAIT_FOR_KEY", &CCSPlayer::State_Enter_DEATH_WAIT_FOR_KEY, NULL,
			 &CCSPlayer::State_PreThink_DEATH_WAIT_FOR_KEY},
			{STATE_OBSERVER_MODE, "STATE_OBSERVER_MODE", &CCSPlayer::State_Enter_OBSERVER_MODE, NULL,
			 &CCSPlayer::State_PreThink_OBSERVER_MODE}};
	...
	void CCSPlayer::State_Enter_WELCOME()
	{
		...
		// Important to set MOVETYPE_NONE or our physics object will fall while we're sitting at one of the intro
		// cameras.
	...
	void CCSPlayer::State_PreThink_DEATH_ANIM()
	{
		...
		{
			if(GetObserverTarget() && GetObserverTarget() != this && !m_bAbortFreezeCam &&
			   gpGlobals->curtime < fFreezeEnd && GetObserverMode() != OBS_MODE_FREEZECAM)
			{
	...
	void CCSPlayer::State_PreThink_DEATH_WAIT_FOR_KEY()
	{
		...
		// won't do collisions and stuff anymore this prevents a bug where the dead body would go to a player's head if
		// he walked over it while the dead player was clicking their button to respawn
	...
	void CCSPlayer::Weapon_Equip(CBaseCombatWeapon *pWeapon)
	{
		...
		if(pCSWeapon)
		{
			...
			if(pCSWeapon->GetSlot() == WEAPON_SLOT_RIFLE || pCSWeapon->GetSlot() == WEAPON_SLOT_PISTOL ||
	...
	//==============================================
	void CCSPlayer::PostAutoBuyCommandProcessing(const AutoBuyInfoStruct *commandInfo, bool &boughtPrimary,
												 bool &boughtSecondary)
	{
		...
		else if((pPrimary == NULL) && ((commandInfo->m_class & AUTOBUYCLASS_SHIELD) == AUTOBUYCLASS_SHIELD) &&
				HasShield())
		{
	...
	bool CCSPlayer::ShouldExecuteAutoBuyCommand(const AutoBuyInfoStruct *commandInfo, bool boughtPrimary,
												bool boughtSecondary)
	{
		if(commandInfo == NULL)
		{
		...
		if((boughtSecondary) && ((commandInfo->m_class & AUTOBUYCLASS_SECONDARY) != 0) &&
		   ((commandInfo->m_class & AUTOBUYCLASS_AMMO) == 0))
		{
	...
	void CCSPlayer::BuildRebuyStruct()
	{
		...
		if(primary == NULL)
		{
			...
			else
			{
				...
				m_rebuyStruct.m_szPrimaryWeapon[0] =
					0; // if we don't have a shield and we don't have a primary weapon, we got nuthin.
	...
	BuyResult_e CCSPlayer::RebuyPrimaryWeapon()
	{
		...
		if(primary != NULL)
		{
			return BUY_ALREADY_HAVE; // don't drop primary weapons via rebuy - if the player picked up a different
									 // weapon, he wants to keep it.
		}
	...
	CBaseEntity *CCSPlayer::FindUseEntity()
	{
		...
		if(CSGameRules()->IsBombDefuseMap() && GetTeamNumber() == TEAM_CT)
		{
			...
			if(bomb != NULL)
			{
				...
				if(vecLOS.LengthSqr() <
				   (96 * 96)) // 64 is the distance in Goldsrc.  However since Goldsrc did distance from the player's
							  // origin and we're doing distance from the player's eye, make the radius a bit bigger.
				{
	...
	void CCSPlayer::DropWeapons(bool fromDeath, bool friendlyFire)
	{
		...
		if(HasDefuser())
		{
			...
			CBaseAnimating *pDefuser =
				(CBaseAnimating *)CBaseEntity::Create("item_defuser", WorldSpaceCenter(), GetLocalAngles(), this);
	 ...
	 * 		info -
	 */
	void CCSPlayer::ProcessPlayerDeathAchievements(CCSPlayer *pAttacker, CCSPlayer *pVictim,
												   const CTakeDamageInfo &info)
	{
		...
		if(pAttacker != NULL && pVictim != NULL && pVictim->GetTeamNumber() != pAttacker->GetTeamNumber())
		{
			...
			{
				if(pVictim->m_DamageTakenList[i]->GetDamage() >=
					   pVictim->GetMaxHealth() - AchievementConsts::DamageNoKill_MaxHealthLeftOnKill &&
				   Q_strncmp(pAttacker->GetPlayerName(), pVictim->m_DamageTakenList[i]->GetPlayerName(),
							 MAX_PLAYER_NAME_LENGTH) != 0)
				{
					...
					for(int j = 1; j <= MAX_PLAYERS; j++)
					{
						...
						if(pPlayerIter &&
						   Q_strncmp(pPlayerIter->GetPlayerName(), pVictim->m_DamageTakenList[i]->GetPlayerName(),
									 MAX_PLAYER_NAME_LENGTH) == 0 &&
			...
			for(int avengedIndex = 0; avengedIndex < pVictim->m_enemyPlayersKilledThisRound.Count(); avengedIndex++)
			{
				...
				if(avengedPlayer)
				{
					// Make sure you are avenging someone on your own team (This is the expected flow. Just here to
					// avoid edge cases like team-switching).
					if(pAttacker->GetTeamNumber() == avengedPlayer->GetTeamNumber())
					{
						CCS_GameStats.Event_PlayerAvengedTeammate(pAttacker,
																  pVictim->m_enemyPlayersKilledThisRound[avengedIndex]);
					}
				}
			}

			// remove elements older than a certain time
			while(pAttacker->m_killTimes.Count() > 0 &&
				  pAttacker->m_killTimes[0] + AchievementConsts::KillingSpree_WindowTime < gpGlobals->curtime)
			{
			...
			if(pVictim->m_KillingSpreeStartTime >= 0 &&
			   pVictim->m_KillingSpreeStartTime - gpGlobals->curtime <= AchievementConsts::KillingSpreeEnder_TimeWindow)
			{
			...
			if(pAttacker->GetActiveWeapon() && pAttacker->GetActiveWeapon()->Clip1() == 0 && pAttackerWeapon &&
			   pAttackerWeapon->GetCSWpnData().m_WeaponType != WEAPONTYPE_SNIPER_RIFLE)
			{
			...
			if(pVictimWeapon && pVictimWeapon->GetCSWpnData().m_WeaponType == WEAPONTYPE_KNIFE && pAttackerWeapon &&
			   pAttackerWeapon->GetCSWpnData().m_WeaponType != WEAPONTYPE_KNIFE &&
			   pAttackerWeapon->GetCSWpnData().m_WeaponType != WEAPONTYPE_C4 &&
			   pAttackerWeapon->GetCSWpnData().m_WeaponType != WEAPONTYPE_GRENADE)
			{
			...
			bool attackerUsedSniperRifle = pAttackerWeapon &&
										   pAttackerWeapon->GetCSWpnData().m_WeaponType == WEAPONTYPE_SNIPER_RIFLE &&
			...
			bool attackerInAir = pAttacker->GetMoveType() != MOVETYPE_LADDER &&
								 pAttacker->GetNearestSurfaceBelow(AchievementConsts::KillInAir_MinimumHeight) == NULL;
			bool victimInAir = pVictim->GetMoveType() != MOVETYPE_LADDER &&
							   pVictim->GetNearestSurfaceBelow(AchievementConsts::KillInAir_MinimumHeight) == NULL;
			...
			if(pVictim->HasC4() &&
			   pVictim->GetBombPickuptime() + AchievementConsts::KillBombPickup_MaxTime > gpGlobals->curtime)
			{
				pAttacker->AwardAchievement(CSKillBombPickup);
			}
		}

		// If you kill a friendly player while blind (from an enemy player), give the guy that blinded you an
		// achievement
		if(pAttacker != NULL && pVictim != NULL && pVictim->GetTeamNumber() == pAttacker->GetTeamNumber() &&
		   pAttacker->IsBlind())
		{
		...
		if(pAlivePlayer)
		{
			...
			if(livePlayerCount == 1 && CSGameRules()->m_iRoundWinStatus == WINNER_NONE &&
			   teamCount[alivePlayerTeam] - teamIgnoreCount[alivePlayerTeam] >=
				   AchievementConsts::LastPlayerAlive_MinPlayersOnTeam &&
			   teamCount[alivePlayerOpposingTeam] - teamIgnoreCount[alivePlayerOpposingTeam] >=
				   AchievementConsts::DefaultMinOpponentsForAchievement &&
	...
	void CCSPlayer::OnRoundEnd(int winningTeam, int reason)
	{
		if(winningTeam == WINNER_CT || winningTeam == WINNER_TER)
		{
			...
			if(IsAlive() && reason == Target_Bombed && m_gooseChaseStep == GC_STOPPED_AFTER_GETTING_SHOT &&
	...
	int CCSPlayer::GetNumEnemyDamagers()
	{
		...
		FOR_EACH_LL(m_DamageTakenList, i)
		{
			for(int j = 1; j <= MAX_PLAYERS; j++)
			{
				...
				if(pPlayer &&
				   V_strncmp(pPlayer->GetPlayerName(), m_DamageTakenList[i]->GetPlayerName(), MAX_PLAYER_NAME_LENGTH) ==
					   0 &&
	...
	int CCSPlayer::GetNumEnemiesDamaged()
	{
		...
		FOR_EACH_LL(m_DamageGivenList, i)
		{
			for(int j = 1; j <= MAX_PLAYERS; j++)
			{
				...
				if(pPlayer &&
				   V_strncmp(pPlayer->GetPlayerName(), m_DamageGivenList[i]->GetPlayerName(), MAX_PLAYER_NAME_LENGTH) ==
					   0 &&
				   pPlayer->GetTeamNumber() != GetTeamNumber())
				{
