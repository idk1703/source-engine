...
ConVar sv_pushaway_hostage_force(
	"sv_pushaway_hostage_force", "20000", FCVAR_REPLICATED | FCVAR_CHEAT,
	"How hard the hostage is pushed away from physics objects (falls off with inverse square of distance).");
ConVar sv_pushaway_max_hostage_force("sv_pushaway_max_hostage_force", "1000", FCVAR_REPLICATED | FCVAR_CHEAT,
...
BEGIN_DATADESC(CHostage)
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "OnRescueZoneTouch", HostageRescueZoneTouch),
		...
		DEFINE_USEFUNC(HostageUse), DEFINE_THINKFUNC(HostageThink),
...
SendPropExclude("DT_BaseAnimating", "m_flPoseParameter"), SendPropExclude("DT_BaseAnimating", "m_flPlaybackRate"),
	...
	SendPropBool(SENDINFO(m_isRescued)), SendPropInt(SENDINFO(m_iHealth), 10), SendPropInt(SENDINFO(m_iMaxHealth), 10),
...
void CHostage::Spawn(void)
{
	...
	SetContextThink(&CHostage::PushawayThink, gpGlobals->curtime + PUSHAWAY_THINK_INTERVAL,
					HOSTAGE_PUSHAWAY_THINK_CONTEXT);
		...
		else
		{
			...
			if(CSGameRules()->m_iNumRescuers == 1 && !CSGameRules()->WasHostageKilled())
			{
				...
				if(allHostagesRescued)
				{
					...
					//[tj] fast version
					if(gpGlobals->curtime - CSGameRules()->GetRoundStartTime() <
					   AchievementConsts::FastHostageRescue_Time)
					{
						if(player)
						{
...
void CHostage::Touch(CBaseEntity *other)
{
	...
	else if(m_inhibitDoorTimer.IsElapsed() && (other->ClassMatches("func_door*") || other->ClassMatches("prop_door*")))
	{
 ...
 */
void CHostage::StartLadder(const CNavLadder *ladder, NavTraverseType how, const Vector &approachPos,
						   const Vector &departPos)
{
}
 ...
 */
bool CHostage::TraverseLadder(const CNavLadder *ladder, NavTraverseType how, const Vector &approachPos,
							  const Vector &departPos, float deltaT)
{
	return false;
}
