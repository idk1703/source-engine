...
ConVar nav_show_area_info("nav_show_area_info", "0.5", FCVAR_CHEAT,
						  "Duration in seconds to show nav area ID and attributes while editing");
ConVar nav_snap_to_grid("nav_snap_to_grid", "0", FCVAR_CHEAT,
						"Snap to the nav generation grid when creating new nav areas");
ConVar nav_create_place_on_ground("nav_create_place_on_ground", "0", FCVAR_CHEAT,
...
ConVar nav_create_area_at_feet("nav_create_area_at_feet", "0", FCVAR_CHEAT,
							   "Anchor nav_begin_area Z to editing player's feet");

ConVar nav_drag_selection_volume_zmax_offset("nav_drag_selection_volume_zmax_offset", "32", FCVAR_REPLICATED,
											 "The offset of the nav drag volume top from center");
ConVar nav_drag_selection_volume_zmin_offset("nav_drag_selection_volume_zmin_offset", "32", FCVAR_REPLICATED,
...
//--------------------------------------------------------------------------------------------------------------
bool CNavMesh::FindNavAreaOrLadderAlongRay(const Vector &start, const Vector &end, CNavArea **bestArea,
										   CNavLadder **bestLadder, CNavArea *ignore)
{
	if(!m_grid.Count())
		return false;
...
bool CNavMesh::FindActiveNavArea(void)
{
	...
	if(result.fraction != 1.0f)
	{
		...
		if(!m_climbableSurface && !m_selectedLadder)
		{
			...
			FindNavAreaOrLadderAlongRay(result.startpos, result.endpos + 100.0f * dir, &m_selectedArea,
										&m_selectedLadder); // extend a few units into the ground
...
public:
	...
	bool operator()(CNavArea *area)
	{
		...
		if((TheNavMesh->IsInSelectedSet(area) == bShouldBeInSelectedSet) && area->IsOverlapping(m_dragArea) &&
		   area->GetCenter().z >= m_zMin && area->GetCenter().z <= m_zMax)
		{
...
void CNavMesh::UpdateDragSelectionSet(void)
{
	...
	AddToDragSet add(dragArea, m_anchor.z - m_nDragSelectionVolumeZMin, m_anchor.z + m_nDragSelectionVolumeZMax,
					 m_bIsDragDeselecting);
	ForAllAreas(add);
}
...
void CNavMesh::DrawEditMode(void)
{
	...
	*/
	...
	if(FindActiveNavArea() || m_markedArea || m_markedLadder || !IsSelectedSetEmpty() || IsEditMode(CREATING_AREA) ||
	   IsEditMode(CREATING_LADDER))
	{
		...
		else
		{
			...
			NavDrawLine(m_editCursorPos + Vector(cursorSize, 0, 0), m_editCursorPos + Vector(-cursorSize, 0, 0),
						NavCursorColor);
			NavDrawLine(m_editCursorPos + Vector(0, cursorSize, 0), m_editCursorPos + Vector(0, -cursorSize, 0),
						NavCursorColor);
		...
		if(IsEditMode(CREATING_AREA))
		{
			...
			NavDrawLine(Vector(m_editCursorPos.x, m_editCursorPos.y, z), Vector(m_anchor.x, m_editCursorPos.y, z),
						NavCreationColor);
			...
			NavDrawLine(Vector(m_editCursorPos.x, m_editCursorPos.y, z), Vector(m_editCursorPos.x, m_anchor.y, z),
						NavCreationColor);
		}
		else if(IsEditMode(DRAG_SELECTING))
		{
			...
			Color dragSelectionColor =
				m_bIsDragDeselecting ? s_dragSelectionSetDeleteColor : s_dragSelectionSetAddColor;
			...
			if(m_selectedLadder != m_lastSelectedLadder || nav_show_area_info.GetBool())
			{
				...
				if(ladderEntity)
				{
					V_snprintf(buffer, sizeof(buffer), "Ladder #%d (Team %s)\n", m_selectedLadder->GetID(),
							   GetGlobalTeam(ladderEntity->GetTeamNumber())->GetName());
				}
				else
				{
			...
			if(m_showAreaInfoTimer.HasStarted() && !m_showAreaInfoTimer.IsElapsed())
			{
				...
				else
				{
					...
					if(attributes & NAV_MESH_CROUCH)
						Q_strncat(attrib, "CROUCH ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_JUMP)
						Q_strncat(attrib, "JUMP ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_PRECISE)
						Q_strncat(attrib, "PRECISE ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_NO_JUMP)
						Q_strncat(attrib, "NO_JUMP ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_STOP)
						Q_strncat(attrib, "STOP ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_RUN)
						Q_strncat(attrib, "RUN ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_WALK)
						Q_strncat(attrib, "WALK ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_AVOID)
						Q_strncat(attrib, "AVOID ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_TRANSIENT)
						Q_strncat(attrib, "TRANSIENT ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_DONT_HIDE)
						Q_strncat(attrib, "DONT_HIDE ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_STAND)
						Q_strncat(attrib, "STAND ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_NO_HOSTAGES)
						Q_strncat(attrib, "NO HOSTAGES ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_STAIRS)
						Q_strncat(attrib, "STAIRS ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_OBSTACLE_TOP)
						Q_strncat(attrib, "OBSTACLE ", sizeof(attrib), -1);
					if(attributes & NAV_MESH_CLIFF)
						Q_strncat(attrib, "CLIFF ", sizeof(attrib), -1);
#ifdef TERROR
					if(attributes & TerrorNavArea::NAV_PLAYERCLIP)
						Q_strncat(attrib, "PLAYERCLIP ", sizeof(attrib), -1);
					if(attributes & TerrorNavArea::NAV_BREAKABLEWALL)
						Q_strncat(attrib, "BREAKABLEWALL ", sizeof(attrib), -1);
					if(m_selectedArea->IsBlocked(TEAM_SURVIVOR))
						Q_strncat(attrib, "BLOCKED_SURVIVOR ", sizeof(attrib), -1);
					if(m_selectedArea->IsBlocked(TEAM_ZOMBIE))
						Q_strncat(attrib, "BLOCKED_ZOMBIE ", sizeof(attrib), -1);
#else
					if(m_selectedArea->IsBlocked(TEAM_ANY))
						Q_strncat(attrib, "BLOCKED ", sizeof(attrib), -1);
#endif
					if(m_selectedArea->HasAvoidanceObstacle())
						Q_strncat(attrib, "OBSTRUCTED ", sizeof(attrib), -1);
					if(m_selectedArea->IsDamaging())
						Q_strncat(attrib, "DAMAGING ", sizeof(attrib), -1);
					if(m_selectedArea->IsUnderwater())
						Q_strncat(attrib, "UNDERWATER ", sizeof(attrib), -1);
...
ConCommand nav_world_center("nav_world_center", CommandNavCenterInWorld, "Centers the nav mesh in the world",
							FCVAR_CHEAT);
		...
		if(area)
		{
			...
			for(float x = areaExtent.lo.x; x + GenerationStepSize <= areaExtent.hi.x; x += GenerationStepSize)
			{
				for(float y = areaExtent.lo.y; y + GenerationStepSize <= areaExtent.hi.y; y += GenerationStepSize)
				{
					...
					if(!IsHeightDifferenceValid(nw, ne, sw, se) || !IsHeightDifferenceValid(ne, nw, sw, se) ||
					   !IsHeightDifferenceValid(sw, ne, nw, se) || !IsHeightDifferenceValid(se, ne, sw, nw))
					{
						AddToSelectedSet(area);
					}
				}
...
void CNavMesh::CommandNavEndArea(void)
{
	...
	if(IsEditMode(CREATING_AREA))
	{
		...
		{
			nearby =
				TheNavMesh->GetNearestNavArea(m_editCursorPos + Vector(0, 0, HalfHumanHeight), false, 10000.0f, true);
		}
		if(!nearby)
		{
...
void CNavMesh::CommandNavConnect(void)
{
	...
	else if(m_selectedArea)
	{
		...
		else
		{
			...
			{
				Msg("To connect areas, mark an area, highlight a second area, then invoke the connect command. Make "
					"sure the cursor is directly north, south, east, or west of the marked area.");
				player->EmitSound("EDIT_CONNECT.AllDirections");
			}
		}
	}
	else if(m_selectedLadder)
	{
		...
		{
			Msg("To connect areas, mark an area, highlight a second area, then invoke the connect command. Make sure "
				"the cursor is directly north, south, east, or west of the marked area.");
			player->EmitSound("EDIT_CONNECT.AllDirections");
		}
	}
...
void CNavMesh::CommandNavDisconnect(void)
{
	...
	else if(m_selectedArea)
	{
		...
		else
		{
			...
			{
				Msg("To disconnect areas, mark an area, highlight a second area, then invoke the disconnect command. "
	...
	else if(m_selectedLadder)
	{
		...
		{
			Msg("To disconnect areas, mark an area, highlight a second area, then invoke the disconnect command. This "
				"will remove all connections between the two areas.");
			player->EmitSound("EDIT_DISCONNECT.NoMarkedArea");
		}
	}
...
void CNavMesh::CommandNavSplice(void)
{
	...
	if(m_selectedArea)
	{
		...
		{
			Msg("To splice, mark an area, highlight a second area, then invoke the splice command to create an area "
				"between them");
			player->EmitSound("EDIT_SPLICE.NoMarkedArea");
		}
	}
