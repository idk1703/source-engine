...
BEGIN_DATADESC(CPointSpotlight)
	DEFINE_FIELD(m_flSpotlightCurLength, FIELD_FLOAT),
		...
		DEFINE_FIELD(m_bSpotlightOn, FIELD_BOOLEAN), DEFINE_FIELD(m_bEfficientSpotlight, FIELD_BOOLEAN),
...
void CPointSpotlight::CreateEfficientSpotlight()
{
	...
	VectorSubtract(m_hSpotlightTarget->GetAbsOrigin(), m_hSpotlightTarget->m_vSpotlightOrg,
...
void CPointSpotlight::SpotlightCreate(void)
{
	...
	{
		trace_t tr;
		UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + m_vSpotlightDir * m_flSpotlightMaxLength, MASK_SOLID_BRUSHONLY,
