...
ConVar nav_split_place_on_ground("nav_split_place_on_ground", "0", FCVAR_CHEAT,
...
ConVar nav_corner_adjust_adjacent("nav_corner_adjust_adjacent", "18", FCVAR_CHEAT,
...
ConVar nav_max_view_distance("nav_max_view_distance", "6000", FCVAR_CHEAT,
							 "Maximum range for precomputed nav mesh visibility (0 = default 1500 units)");
ConVar nav_update_visibility_on_edit("nav_update_visibility_on_edit", "0", FCVAR_CHEAT,
...
ConVar nav_show_potentially_visible("nav_show_potentially_visible", "0", FCVAR_CHEAT,
...
ConVar nav_selected_set_color("nav_selected_set_color", "255 255 200 96", FCVAR_CHEAT,
							  "Color used to draw the selected set background while editing.", false, 0.0f, false, 0.0f,
							  SelectedSetColorChaged);
ConVar nav_selected_set_border_color("nav_selected_set_border_color", "100 100 0 255", FCVAR_CHEAT,
									 "Color used to draw the selected set borders while editing.", false, 0.0f, false,
									 0.0f, SelectedSetColorChaged);
...
void CNavArea::GetNodes(NavDirType dir, CUtlVector<CNavNode *> *nodes) const
{
	...
	CNavNode *node;
	for(node = m_node[startCorner]; node && node != m_node[endCorner];
		node = node->GetConnectedNode(traversalDirection))
	{
				...
				if(center.z <= floor->height + tolerance && center.z >= floor->height - tolerance)
				{
					...
					for(int of = 0; of < elevator->GetNumFloors(); ++of)
					{
						...
						FOR_EACH_VEC(TheNavAreas, it)
						{
							...
							{
								if(area->GetCenter().z <= otherFloor->height + tolerance &&
								   area->GetCenter().z >= otherFloor->height - tolerance)
								{
						...
						{
							Warning("Floor %d ('%s') of elevator at ( %3.2f, %3.2f, %3.2f ) has no matching navigation "
									"areas\n",
									of, otherFloor->name.ToCStr(), elevator->GetAbsOrigin().x,
									elevator->GetAbsOrigin().y, elevator->GetAbsOrigin().z);
						}
					}
...
bool CNavArea::IsConnected(const CNavArea *area, NavDirType dir) const
{
	...
	if(dir == NUM_DIRECTIONS)
	{
		...
		FOR_EACH_VEC(m_ladder[CNavLadder::LADDER_UP], it)
		{
			...
			if(ladder->m_topBehindArea == area || ladder->m_topForwardArea == area || ladder->m_topLeftArea == area ||
...
void CNavArea::CalcDebugID()
{
	if(m_debugid == 0)
	{
		...
		int coord[6] = {(int)m_nwCorner.x, (int)m_nwCorner.x, (int)m_nwCorner.z,
						(int)m_seCorner.x, (int)m_seCorner.y, (int)m_seCorner.z};
...
bool CNavArea::MergeEdit(CNavArea *adj)
{
	...
	bool merge = false;
	if(fabs(m_nwCorner.x - adj->m_nwCorner.x) < tolerance && fabs(m_seCorner.x - adj->m_seCorner.x) < tolerance)
		merge = true;

	if(fabs(m_nwCorner.y - adj->m_nwCorner.y) < tolerance && fabs(m_seCorner.y - adj->m_seCorner.y) < tolerance)
		merge = true;
...
{
	if(pos.x + tolerance >= m_nwCorner.x && pos.x - tolerance <= m_seCorner.x && pos.y + tolerance >= m_nwCorner.y &&
...
{
	if(area->m_nwCorner.x < m_seCorner.x && area->m_seCorner.x > m_nwCorner.x && area->m_nwCorner.y < m_seCorner.y &&
...
{
	return (extent.lo.x < m_seCorner.x && extent.hi.x > m_nwCorner.x && extent.lo.y < m_seCorner.y &&
			extent.hi.y > m_nwCorner.y);
}
 ...
 */
void CNavArea::ComputeClosestPointInPortal(const CNavArea *to, NavDirType dir, const Vector &fromPos,
...
void CNavArea::Draw(void) const
{
	...
	if(nav_show_light_intensity.GetBool())
	{
		for(int i = 0; i < NUM_CORNERS; ++i)
		{
			...
			NDebugOverlay::Line(end, pos, lightIntensity, lightIntensity, MAX(192, lightIntensity), true,
								DebugDuration);
		}
	}
	...
	int bgcolor[4];
	if(4 ==
	   sscanf(nav_area_bgcolor.GetString(), "%d %d %d %d", &(bgcolor[0]), &(bgcolor[1]), &(bgcolor[2]), &(bgcolor[3])))
	{
		for(int i = 0; i < 4; ++i)
		...
		if(bgcolor[3] > 0)
		{
			...
			NDebugOverlay::Triangle(nw + offset, se + offset, ne + offset, bgcolor[0], bgcolor[1], bgcolor[2],
									bgcolor[3], true, DebugDuration);
			NDebugOverlay::Triangle(se + offset, nw + offset, sw + offset, bgcolor[0], bgcolor[1], bgcolor[2],
									bgcolor[3], true, DebugDuration);
		}
	}
	...
	if(this != TheNavMesh->GetMarkedArea() && this == TheNavMesh->GetSelectedArea() &&
	...
	if(GetAttributes() & NAV_MESH_AVOID)
	{
		...
		NavDrawTriangle(m_center + Vector(0, -bottomHeight, 0), m_center + Vector(-bottomWidth, -bottomHeight * 2, 0),
						m_center + Vector(bottomWidth, -bottomHeight * 2, 0), color);
	}

	if(IsBlocked(TEAM_ANY) || HasAvoidanceObstacle() || IsDamaging())
	{
		NavEditColor color = (IsBlocked(TEAM_ANY) && (m_attributeFlags & NAV_MESH_NAV_BLOCKER))
 ...
 * @todo "bubbling" does unnecessary work, since the order of all other nodes will be unchanged - only this node is
 * altered
 */
	...
	if(!area->IsOverlapping(pos))
	{
		...
		if(!area->IsOverlapping(pos))
		{
			...
			if(!area->IsOverlapping(pos))
			{
				...
				if(!area->IsOverlapping(pos))
				{
					AssertMsg(false, "A Hiding Spot can't be placed on its area at (%.0f %.0f %.0f)", cornerPos.x,
							  cornerPos.y, cornerPos.z);
...
void ClassifySniperSpot(HidingSpot *spot)
{
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		...
		// scan this area
		for(walkable.y = areaExtent.lo.y + GenerationStepSize / 2.0f; walkable.y < areaExtent.hi.y;
			walkable.y += GenerationStepSize)
		{
			for(walkable.x = areaExtent.lo.x + GenerationStepSize / 2.0f; walkable.x < areaExtent.hi.x;
				walkable.x += GenerationStepSize)
			{
				walkable.z = area->GetZ(walkable) + HalfHumanHeight;

				// check line of sight
				UTIL_TraceLine(eye, walkable, CONTENTS_SOLID | CONTENTS_MOVEABLE | CONTENTS_PLAYERCLIP, NULL,
...
void CNavArea::AddSpotEncounters(const CNavArea *from, NavDirType fromDir, const CNavArea *to, NavDirType toDir)
{
	...
	for(float along = 0.0f; !done; along += stepSize)
	{
		...
		FOR_EACH_VEC(TheHidingSpots, it)
		{
			...
			UTIL_TraceLine(eye, Vector(spotPos.x, spotPos.y, spotPos.z + HalfHumanHeight), MASK_NPCSOLID_BRUSHONLY,
...
bool CNavArea::ComputeLighting(void)
{
	...
	for(int i = 0; i < NUM_CORNERS; ++i)
	{
		...
		pos.z = GetZ(pos) + HalfHumanHeight - StepHeight; // players light from their centers, and we light from
														  // slightly below that, to allow for low ceilings
		float height;
		if(TheNavMesh->GetGroundHeight(pos, &height))
		{
			pos.z = height + HalfHumanHeight - StepHeight; // players light from their centers, and we light from
														   // slightly below that, to allow for low ceilings
		}
		...
		lightIntensity = clamp(lightIntensity, 0.f, 1.f); // sum can go well over 1.0, but it's the lower region we care
														  // about.  if it's bright, we don't need to know *how* bright.
...
static ConCommand nav_update_blocked("nav_update_blocked", CommandNavUpdateBlocked,
...
void CNavArea::UpdateBlocked(bool force, int teamID)
{
	...
	CTraceFilterWalkableEntities filter(NULL, COLLISION_GROUP_PLAYER_MOVEMENT,
										WALK_THRU_PROP_DOORS | WALK_THRU_BREAKABLES);
#else
	...
	trace_t tr;
	{
		...
		UTIL_TraceHull(origin, origin, bounds.lo, bounds.hi, MASK_NPCSOLID_BRUSHONLY, &filter, &tr);
	}
...
void CNavArea::CheckFloor(CBaseEntity *ignore)
{
	...
	// See if spot is valid
	trace_t tr;
	UTIL_TraceHull(origin, origin, mins, maxs, MASK_NPCSOLID_BRUSHONLY, ignore, COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
...
static ConCommand nav_check_floor("nav_check_floor", CommandNavCheckFloor,
								  "Updates the blocked/unblocked status for every nav area.", FCVAR_GAMEDLL);
...
static ConCommand nav_select_overlapping("nav_select_overlapping", CommandNavSelectOverlapping,
 ...
 */
CNavArea::VisibilityType CNavArea::ComputeVisibility(const CNavArea *area, bool isPVSValid, bool bCheckPVS,
													 bool *pOutsidePVS) const
{
	...
	if(tr.fraction == 1.0 || (tr.endpos.x > vOtherMins.x && tr.endpos.x < vOtherMaxs.x && tr.endpos.y > vOtherMins.y &&
							  tr.endpos.y < vOtherMaxs.y))
	{
		return COMPLETELY_VISIBLE; // Counter-intuitive: the way this function was written, "COMPLETELY_VISIBLE"
								   // actually means "I am completely visible to the other"
	}
	...
	float angleTolerance =
		nav_potentially_visible_dot_tolerance.GetFloat(); // if corner-to-eye angles are this close to center-to-eye
														  // angles, assume the same result and skip the trace
...
void CNavArea::ComputeVisToArea(CNavArea *&pOtherArea)
{
	...
	if(area != g_pCurVisArea)
	{
		...
		visOtherToThis = g_pCurVisArea->ComputeVisibility(
			area, true, true, &bOutsidePVS); // TODO: Hacky right now. Compute visibility for the "complete" case
											 // actually returns how completely visible the area is to the other. Should
											 // fix it to be more clear [1/30/2009 tom]

		if(!bOutsidePVS && (visOtherToThis || (g_pCurVisArea->GetCenter() - area->GetCenter()).LengthSqr() <
...
void CNavArea::ComputeVisibilityToMesh(void)
{
	...
	g_pCurVisArea = this;
	ParallelProcess("CNavArea::ComputeVisibilityToMesh", collector.m_area.Base(), collector.m_area.Count(),
					&ComputeVisToArea);
