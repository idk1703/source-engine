...
ConVar mp_time_between_capscoring("mp_time_between_capscoring", "30", FCVAR_GAMEDLL,
								  "Delay between scoring of owned capture points.", true, 1, false, 0);
...
bool CTeamControlPointMaster::FindControlPoints(void)
{
	...
	for(unsigned int i = 0; i < m_ControlPoints.Count(); i++)
	{
		...
		for(int team = 0; team < GetNumberOfTeams(); team++)
		{
			...
			{
				ObjectiveResource()->SetPreviousPoint(iPointIndex, team, prevpoint,
													  pPoint->GetPreviousPointForTeam(team, prevpoint));
			}
		}
	}
...
bool CTeamControlPointMaster::SelectSpecificRound(void)
{
	...
	if(pRules)
	{
		if(pRules->GetRoundToPlayNext() != NULL_STRING)
		{
			...
			pRound = dynamic_cast<CTeamControlPointRound *>(
				gEntList.FindEntityByName(NULL, STRING(pRules->GetRoundToPlayNext())));
...
void CTeamControlPointMaster::CheckWinConditions(void)
{
	...
	else
	{
		...
		if((m_iInvalidCapWinner != 1) && (iWinners >= FIRST_GAME_TEAM) && (iWinners != m_iInvalidCapWinner))
		{
			bool bWinner = true;
		...
		/*		if ( m_bFirstRoundAfterRestart )
				{
					...
					if ( pRules && ( pRules->GetRoundToPlayNext() == NULL_STRING ) )
					{
						...
						// (prevents points being randomized again after "waiting for players" has finished and we're
		   going to play the same round) HandleRandomOwnerControlPoints();
					}
				}
		*/
...
//			win the game.
//-----------------------------------------------------------------------------
int CTeamControlPointMaster::TeamOwnsAllPoints(CTeamControlPoint *pOverridePoint /* = NULL */,
											   int iOverrideNewTeam /* = TEAM_UNASSIGNED */)
{
	unsigned int i;
...
int CTeamControlPointMaster::GetBaseControlPoint(int iTeam)
{
	...
	if(pLowestPoint && pHighestPoint)
	{
		...
		if((pLowestPoint->GetDefaultOwner() == iTeam &&
			pHighestPoint->GetDefaultOwner() ==
				iTeam) || // if the same team owns both, take the highest value to be the last point
	...
	while(vecControlPointsOwned.Count() < (int)m_ControlPoints.Count())
	{
		...
		for(int i = 0; i < m_ControlPointRounds.Count(); ++i)
		{
			...
			for(iControlPoint = 0; iControlPoint < pRound->m_ControlPoints.Count(); iControlPoint++)
			{
				...
				{
					// for all other control points, if the control point ownership does not match other control points,
					// reset round ownership to no team
			...
			// this round is playable if all control points are not owned by one team (or owned by a team that can't win
			// by capping them)
			bool bPlayable = ((iRoundOwningTeam < FIRST_GAME_TEAM) || (pRound->GetInvalidCapWinner() == 1) ||
							  (iRoundOwningTeam == pRound->GetInvalidCapWinner()));
			if(!bPlayable)
				continue;

			// Pretend this team played and won this round.  It now owns all control points from this round.  Add all
			// the control points from this round that are not already own the owned list to the owned list
			...
			// sanity check: team being simulated should be owning at least one more new control point per round, or
			...
			// repeat until all control points are owned.  The number of iterations it takes is the # of rounds
			// remaining for this team to win.
			break;
		}
	}
	...
	if(args.ArgC() > 1)
	{
		...
		if(pRules && pMaster)
		{
			if(pMaster->PlayingMiniRounds())
			{
				...
				CTeamControlPointRound *pRound =
					dynamic_cast<CTeamControlPointRound *>(gEntList.FindEntityByName(NULL, args[1]));
...
static ConCommand playround("playround", cc_PlayRound,
							"Play the selected round\n\tArgument: {round name given by \"listrounds\" command}",
							FCVAR_CHEAT);
#endif