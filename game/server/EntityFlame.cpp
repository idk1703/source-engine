...
BEGIN_DATADESC(CEntityFlame)
	...
	DEFINE_KEYFIELD(m_flLifetime, FIELD_FLOAT, "lifetime"),
		...
		DEFINE_FIELD(m_flSize, FIELD_FLOAT), DEFINE_FIELD(m_hEntAttached, FIELD_EHANDLE),
		DEFINE_FIELD(m_bUseHitboxes, FIELD_BOOLEAN), DEFINE_FIELD(m_iNumHitboxFires, FIELD_INTEGER),
...
void CEntityFlame::FlameThink(void)
{
	...
	if(m_hEntAttached)
	{
		...
		if(!m_hEntAttached->IsNPC() && hl2_episodic.GetBool())
		{
			...
			CSoundEnt::InsertSound(SOUND_MOVE_AWAY, GetAbsOrigin(), ENTITYFLAME_MOVE_AWAY_DIST, 0.1f, this,
								   SOUNDENT_CHANNEL_REPEATED_DANGER);
			CSoundEnt::InsertSound(SOUND_MOVE_AWAY, GetAbsOrigin() + Vector(0, 0, 48.0f), ENTITYFLAME_MOVE_AWAY_DIST,
								   0.1f, this, SOUNDENT_CHANNEL_REPEATING);
		}
	}
	else
	{
		RadiusDamage(CTakeDamageInfo(this, this, FLAME_RADIUS_DAMAGE, DMG_BURN), GetAbsOrigin(), m_flSize / 2,
					 CLASS_NONE, NULL);
	}
