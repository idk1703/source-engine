...
ConVar nav_slope_tolerance(
	"nav_slope_tolerance", "0.1", FCVAR_CHEAT,
...
ConVar nav_generate_fixup_jump_areas("nav_generate_fixup_jump_areas", "1", FCVAR_CHEAT,
									 "Convert obsolete jump areas into 2-way connections");
ConVar nav_generate_jump_connections("nav_generate_jump_connections", "1", FCVAR_CHEAT,
									 "If disabled, don't generate jump connections from jump areas");
ConVar nav_generate_incremental_range("nav_generate_incremental_range", "2000", FCVAR_CHEAT);
ConVar nav_generate_incremental_tolerance("nav_generate_incremental_tolerance", "0", FCVAR_CHEAT,
...
bool FindGroundForNode(Vector *pos,
					   Vector *normal); // find a ground Z for pos that is clear for NavTraceMins -> NavTraceMaxs

const float MaxTraversableHeight =
	StepHeight; // max internal obstacle height that can occur between nav nodes and safely disregarded
										  ...
										  CBaseEntity *traceIgnore = NULL, Vector *closePos = NULL)
{
	...
	for(int i = 1; i <= end; i++)
	{
		...
		UTIL_TraceHull(*start, pos, NavTraceMins, NavTraceMaxs, TheNavMesh->GetGenerationTraceMask(), traceIgnore,
 ...
 * Create a navigation representation of a ladder.
 */
void CNavMesh::CreateLadder(const Vector &top, const Vector &bottom, float width, const Vector2D &ladderDir,
...
private:
	...
	void TryToConnect(CNavArea *jumpArea, const NavConnectVector *source, const NavConnectVector *dest,
	...
	void TryToConnect(CNavArea *jumpArea, CNavArea *sourceArea, const NavConnectVector *dest, NavDirType outgoingDir)
	{
		FOR_EACH_VEC((*dest), destIt)
		{
			...
			{
				// Don't connect areas across 2 jump areas.  This means we'll have some missing links due to sampling
				// errors. This is preferable to generating incorrect links across multiple jump areas, which is far
				// more common.
				continue;
			}
...
void CNavMesh::MarkPlayerClipAreas(void)
{
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		...
		// Trace upward a bit from our center point just colliding wtih PLAYERCLIP to see if we're in one, if we are,
		// mark us as accordingly.
		trace_t trace;
...
void AdjustObstacleDistances(float *pObstacleStartDist, float *pObstacleEndDist, float maxAllowedDist)
{
	...
	if(obstacleWidth < MinObstacleAreaWidth)
	{
		...
		*pObstacleEndDist = *pObstacleStartDist + MinObstacleAreaWidth;
		...
		// if this pushes the end past max allowed distance, pull start and end back so that end is within allowed
		// distance
...
void CNavMesh::RaiseAreasWithInternalObstacles()
{
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		...
		// Look at all 4 directions and determine if there are obstacles in that direction.  Find the direction with the
		// highest obstacle, if any.
		for(int i = 0; i < NUM_DIRECTIONS; i++)
		{
			NavDirType dir = (NavDirType)i;

			// For this direction, look at the left and right edges of the nav area relative to this direction and
			// determined if they are both blocked by obstacles.  We only consider this area obstructed if both edges
			// are blocked (e.g. fence runs all the way through it).
			...
			float obstacleZThisDir[2] = {
				-FLT_MAX, -FLT_MAX}; // absolute Z pos of obstacle for left and right edge in this direction
			...
			// are both edged blocked in this direction, and is the obstacle height in this direction the tallest we've
			// seen?
		...
		if(obstacleZMax > -FLT_MAX)
		{
			...
			switch(obstacleDir)
			{
				...
				case SOUTH:
					...
					::V_swap(
						obstacleZ[0],
						obstacleZ[1]); // swap left and right Z heights for obstacle so we can run common code below
					break;
				case EAST:
			...
			// clear out the nodes associated with this area's corners -- corners don't match the node positions any
			// more
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		...
		for(int i = NORTH; i < NUM_DIRECTIONS; i++)
		{
			...
			for(int j = 0; j < iConnections; j++)
			{
				...
				// create an obstacle top if there is a one-node separation between the areas and there is an intra-node
				// obstacle within that separation
				if(!CreateObstacleTopAreaIfNecessary(area, areaOther, dir, false))
				{
					// if not, create an obstacle top if there is a two-node separation between the areas and the
					// intermediate node is significantly higher than the two areas, which means there's some geometry
					// there that causes the middle node to be higher
...
bool CNavMesh::CreateObstacleTopAreaIfNecessary(CNavArea *area, CNavArea *areaOther, NavDirType dir, bool bMultiNode)
{
	...
	if(halfPortalWidth > 0)
	{
		...
		// along the edge of this area that faces the other area, look at every node that's in the portal between the
		// two
		while(node)
		{
					...
					if(!areaOther->Contains(*node->GetPosition()))
					{
						...
						if(nodeTowardOtherArea)
						{
							...
							if(deltaZ > MaxTraversableHeight)
							{
								...
								{
									float z = areaOther->GetZ(nodeTowardOtherArea->GetPosition()->x,
								...
								// if we have not arrived in the other area yet, take one more step in the same
								// direction
								if(!bInOtherArea)
								{
									...
									{
										float areaDeltaZ =
											node->GetPosition()->z - nodeTowardOtherArea2->GetPosition()->z;
										if(fabs(areaDeltaZ) <= MaxTraversableHeight)
										{
											// if we arrived in the other area, the obstacle height to get here was the
											// peak deltaZ of the node above to get here
											obstacleHeight = deltaZ;
											// make a nav area MinObstacleAreaWidth wide centered on the peak node,
											// which is GenerationStepSize away from where we started
		...
		// if we had some obstacle height from EVERY node along the portal, then getting from this area to the other
		// requires scaling an obstacle, need to generate a nav area on top of it
		if((obstacleHeightMax > MaxTraversableHeight) && (obstacleHeightMin > MaxTraversableHeight))
		{
			// If the maximum obstacle height was greater than both the height at start and end of the edge, then the
			// obstacle is highest somewhere in the middle.  Use that as the height of both ends.
			...
			switch(dir)
			{
				case NORTH:
				case SOUTH:
					cornerNW.Init(center.x - halfPortalWidth, center.y - (newAreaWidth / 2),
					...
					cornerSW.Init(center.x - halfPortalWidth, center.y + (newAreaWidth / 2),
								  zStart + obstacleHeightStart);
					break;
				case EAST:
				case WEST:
					cornerNW.Init(center.x - (newAreaWidth / 2), center.y - halfPortalWidth,
								  zStart + obstacleHeightStart);
					...
					cornerSW.Init(center.x - (newAreaWidth / 2), center.y + halfPortalWidth,
								  zStart + obstacleHeightStart);
					break;
			}
			...
			Msg("Created new fencetop area %d(%x) between %d(%x) and %d(%x)\n", areaNew->GetID(), areaNew->GetDebugID(),
				area->GetID(), area->GetDebugID(), areaOther->GetID(), areaOther->GetDebugID());
...
{
	// What we really want is the union of all obstacle top areas that get generated.  That would be hard to compute
	// exactly, so instead we'll just remove any that overlap.  The obstacle top areas don't have to be exact, we just
	// need enough of them so there is generally a path to get over any obstacle.
			...
			if(area->IsOverlapping(areaOther))
			{
				...
				else
				{
					...
					CNavArea *areaToRemove =
						(area->GetSizeX() * area->GetSizeY() > areaOther->GetSizeX() * areaOther->GetSizeY() ? areaOther
																											 : area);
...
static void splitX(CNavArea *area)
{
	...
	if(fabs(split - area->GetCorner(NORTH_WEST).x) < epsilon || fabs(split - area->GetCorner(SOUTH_EAST).x) < epsilon)
	{
...
static void splitY(CNavArea *area)
{
	...
	const float epsilon = 0.1f;
	if(fabs(split - area->GetCorner(NORTH_WEST).y) < epsilon || fabs(split - area->GetCorner(SOUTH_EAST).y) < epsilon)
	{
		// too small to subdivide
		return;
	}
	...
	else
	{
		...
		for(float height = StepHeight; height <= ClimbUpHeight; height += 1.0f)
		{
			...
			if(!tr.startsolid && tr.fraction == 1.0f)
			{
				...
				UTIL_TraceHull(start, end, NavTraceMins, NavTraceMaxs, TheNavMesh->GetGenerationTraceMask(), &filter,
							   &tr);
				if(tr.fraction < 1.0f)
				{
					break;
...
inline bool testJumpDown(const Vector *fromPos, const Vector *toPos)
{
	...
	for(up = 1.0f; up <= ClimbUpHeight; up += 1.0f)
	{
		...
		UTIL_TraceHull(from, to, NavTraceMins, NavTraceMaxs, TheNavMesh->GetGenerationTraceMask(), NULL,
					   COLLISION_GROUP_NONE, &result);
...
{
	// cliffs are half-baked, not used by any existing AI, and create poorly behaved nav areas (ie: long, thin, strips)
	// (MSB 8/7/09)
	return false;
	...
	if(TraceAdjacentNode(0, *fromPos, toPos, &trace, DeathDrop * 10) && !trace.allsolid && !trace.startsolid)
	{
		...
		// if we're looking to south or east, and the first node we found was approximately flat, and this is the
		// top-level call, recurse one level to check one more step in this direction
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		...
		for(int dir = 0; dir < NUM_DIRECTIONS; ++dir)
		{
			...
			for(int i = 0; i < connectedAreas->Count(); ++i)
			{
				...
				// Also, if the adjacent area has to jump up higher than StepHeight above the stair area to reach the
				// stairs, there's an obstruction close to the adjacent area that could prevent walking from the stairs
				// down.
...
void CNavMesh::FixCornerOnCornerAreas(void)
{
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		...
		for(int iCorner = NORTH_WEST; iCorner < NUM_CORNERS; iCorner++)
		{
			...
			// if we have any connections on cardinal compass directions on edge on either side of corner we're OK, skip
			// this nav area
			...
			for(int iDir = 0; iDir < ARRAYSIZE(dirsAlongOtherEdge); iDir++)
			{
				...
				// we are touching corner-to-corner with the other nav area and don't have connections in cardinal
				// directions around the corner that touches, this is a candidate to generate new small helper nav
				// areas.

				// calculate the corners of the 0.5 x 0.5 nav area we would consider building between us and the other
				// nav area whose corner we touch
				...
				vecCorner[1] =
					cornerPos +
					vecDeltaOtherEdge;	  // intersection of far edge of new nav area with other nav area we touch
				vecCorner[2] = cornerPos; // common corner of this nav area, nav area we touch, and new nav area
				vecCorner[3] =
					cornerPos + vecDeltaOurEdge; // intersection of far edge of new nav area with this nav area
				...
				if(!TraceAdjacentNode(0, vecCorner[1], vecCorner[0], &result,
				   ...
				   !TraceAdjacentNode(0, vecCorner[3], vecCorner[0], &result,
									  MaxDrop)) // can we move from edge of this area to far corner of new node
			...
			for(int dir = NORTH; dir < NUM_DIRECTIONS && !bRestartProcessing; dir++)
			{
				...
				for(int iConnection = 0; iConnection < pConnections->Count() && !bRestartProcessing; iConnection++)
				{
					...
					if(area->IsOverlapping(otherArea))
					{
						...
						float splitCoord; // absolute world coordinate along which we will split lower nav area (X or Y,
										  // depending on axis we split on)
						float
							splitLen; // length of the segment of lower nav area that is in shadow of the upper nav area
						...
						// if we split the lower nav area right where it's in shadow of the upper nav area, will it
						// create a really tiny strip?
						if(splitLen < GenerationStepSize)
						{
							// if the "in shadow" part of the lower nav area is really small or the lower nav area is
							// really small to begin with, don't split it, we're better off as is
							if((splitLen < GenerationStepSize * 0.3) || (splitEdgeSize <= GenerationStepSize * 2))
								continue;

							// Move our split point so we don't create a really tiny strip on the lower nav area.  Move
							// the split point away from the upper nav area so the "in shadow" area expands to be
							// GenerationStepSize.  The checks above ensure we have room to do this.
							float splitDelta = GenerationStepSize - splitLen;
							splitCoord += splitDelta *
										  (((dirFromAboveToBelow == NORTH) || (dirFromAboveToBelow == WEST)) ? -1 : 1);
						}

						// remove any connections between the two areas (so they don't get inherited by the new areas
						// when we split the lower area), but remember what the connections were.
						...
						{
							//							Msg( "Split area %d into %d and %d\n", idBelow, pNewAlpha->GetID(),
							//pNewBeta->GetID() );

							// determine which of the two new lower areas is the one *not* in shadow of the upper nav
							// area.  This is the one we want to reconnect to
							CNavArea *pNewNonoverlappedArea =
								((dirFromAboveToBelow == NORTH) || (dirFromAboveToBelow == WEST)) ? pNewAlpha
																								  : pNewBeta;

							// restore the previous connections from the upper nav area to the new lower nav area that
							// is not in shadow of the upper
							...
							// Now we need to just start the whole process over.  We've just perturbed the list we're
							// iterating on (removed a nav area, added two new ones, when we did the split), and it's
							// possible we may have to subdivide a lower nav area twice if the upper nav area overhangs
							// a corner of the lower area.  We just start all over again each time we do a split until
							// no more overhangs occur.
...
bool TestForValidJumpArea(CNavNode *node)
{
	...
	if(!IsHeightDifferenceValid(node->GetPosition()->z, south->GetPosition()->z, southEast->GetPosition()->z,
								east->GetPosition()->z))
		return false;

	if(!IsHeightDifferenceValid(south->GetPosition()->z, node->GetPosition()->z, southEast->GetPosition()->z,
								east->GetPosition()->z))
		return false;

	if(!IsHeightDifferenceValid(southEast->GetPosition()->z, south->GetPosition()->z, node->GetPosition()->z,
								east->GetPosition()->z))
		return false;

	if(!IsHeightDifferenceValid(east->GetPosition()->z, south->GetPosition()->z, southEast->GetPosition()->z,
								node->GetPosition()->z))
		return false;
...
bool CNavMesh::TestArea(CNavNode *node, int width, int height)
{
	...
	for(y = 0; y < height; y++)
	{
		...
		for(x = 0; x < width; x++)
		{
			...
			// crouch attributes and blocked state need to be checked to the south and east of the southEdge and
			// eastEdge nodes.
...
int CNavMesh::BuildArea(CNavNode *node, int width, int height)
{
	...
	// If any of the corners have an obstacle in the direction of another corner, then there's an internal obstruction
	// of this nav node. Mark it as not mergable so it doesn't become a part of anything else and we will fix it up
	// later.
	if(nwNode->m_obstacleHeight[SOUTH] > MaxTraversableHeight ||
	   nwNode->m_obstacleHeight[EAST] > MaxTraversableHeight || neNode->m_obstacleHeight[WEST] > MaxTraversableHeight ||
	   neNode->m_obstacleHeight[SOUTH] > MaxTraversableHeight ||
	   seNode->m_obstacleHeight[NORTH] > MaxTraversableHeight ||
	   seNode->m_obstacleHeight[WEST] > MaxTraversableHeight || swNode->m_obstacleHeight[EAST] > MaxTraversableHeight ||
	   swNode->m_obstacleHeight[NORTH] > MaxTraversableHeight)
	{
		Assert(width == 1); // We should only ever try to build a 1x1 area out of any two nodes that have an obstruction
							// between them
		Assert(height == 1);
		...
		case FIND_LIGHT_INTENSITY:
		{
			...
			if(s_unlitAreas.Count())
			{
				if(s_unlitSeedAreas.Count())
				{
					...
					// s_unlitAreas.Count(), TheNavAreas.Count() );

					Vector eyePos = moveArea->GetCenter();
					float height;
					if(GetGroundHeight(eyePos, &height))
					{
						eyePos.z = height + HalfHumanHeight -
								   StepHeight; // players light from their centers, and we light from slightly below
											   // that, to allow for low ceilings
					}
					else
					{
						eyePos.z += HalfHumanHeight - StepHeight; // players light from their centers, and we light from
																  // slightly below that, to allow for low ceilings
					}
					host->SetAbsOrigin(eyePos);
					AnalysisProgress("Finding light intensity...", 100,
									 100 * (TheNavAreas.Count() - s_unlitAreas.Count()) / TheNavAreas.Count());
...
CNavNode *CNavMesh::AddNode(const Vector &destPos, const Vector &normal, NavDirType dir, CNavNode *source,
							bool isOnDisplacement, float obstacleHeight, float obstacleStartDist, float obstacleEndDist)
{
	...
	if(fabs(deltaZ) < zTolerance)
	{
		...
		node->ConnectTo(source, OppositeDirection(dir), obstacleHeight, GenerationStepSize - obstacleEndDist,
...
inline CNavNode *LadderEndSearch(const Vector *pos, NavDirType mountDir)
{
	...
	for(int d = (-1); d < 2 * NUM_DIRECTIONS; ++d)
	{
		...
		trace_t result;
		UTIL_TraceHull(center + Vector(0, 0, fudge), tryPos + Vector(0, 0, fudge), NavTraceMins, NavTraceMaxs,
					   TheNavMesh->GetGenerationTraceMask(), NULL, COLLISION_GROUP_NONE, &result);
		if(result.fraction != 1.0f || result.startsolid)
			continue;
...
void DrawTrace(const trace_t *trace)
{
	...
	{
		NDebugOverlay::SweptBox( trace->startpos, trace->endpos, NavTraceMins, NavTraceMaxs, vec3_angle, 0, 255, 0, 45,
	100 );
	}
	else
	{
		NDebugOverlay::SweptBox( trace->startpos, trace->endpos, NavTraceMins, NavTraceMaxs, vec3_angle, 255, 0, 0, 45,
	100 );
	}
	*/
}
...
//--------------------------------------------------------------------------------------------------------------
bool TraceAdjacentNode(int depth, const Vector &start, const Vector &end, trace_t *trace,
					   float zLimit /* = DeathDrop */)
{
	...
	UTIL_TraceHull(testStart, testEnd, NavTraceMins, NavTraceMaxs, TheNavMesh->GetGenerationTraceMask(), &filter,
				   trace);
	DrawTrace(trace);
	...
	while(true)
	{
		...
		for(int dir = NORTH; dir < NUM_DIRECTIONS; dir++)
		{
			if(!m_currentNode->HasVisited((NavDirType)dir))
			{
				...
				else
				{
					...
					for(float height = StepHeight; height <= ClimbUpHeight; height += 1.0f)
					{
						...
						if(!tr.startsolid && tr.fraction == 1.0f)
						{
							...
							UTIL_TraceHull(start, end, NavTraceMins, NavTraceMaxs, GetGenerationTraceMask(), &filter,
										   &tr);
							if(tr.fraction < 1.0f)
							{
								break;
						...
						else
						{
							...
							{
								UTIL_TraceHull(end, start, NavTraceMins, NavTraceMaxs, GetGenerationTraceMask(),
											   &filter, &tr);
								if(!tr.startsolid && tr.fraction < 1.0)
								{
									// We hit something going the other direction.  There is some obstacle between the
									// two nodes.
									...
									{
										// Remember the distances to start and end of the obstacle (with respect to the
										// "from" node). Keep track of the last distances to obstacle as we keep
										// increasing the height we do a trace for. If we do eventually clear the
										// obstacle, these values will be the start and end distance to the very tip of
										// the obstacle.
				...
				// If there's an obstacle in the way and it's traversable, or the obstacle is not higher than the
				// destination node itself minus a small epsilon (meaning the obstacle was just the height change to get
				// to the destination node, no extra obstacle between the two), clear obstacle height and distances
				...
				AddNode(to, toNormal, m_generationDir, m_currentNode, isOnDisplacement, obstacleHeight,
		...
		for(NavDirType dir = NORTH; dir < NUM_DIRECTIONS; dir = (NavDirType)(((int)dir) + 1))
		{
			...
			for(int iConnect = 0; iConnect < pOutgoing->Count(); iConnect++)
			{
				...
				if(!bIsTwoWay)
				{
					...
					{
						Msg("Area %d has one-way connect to area %d but does not appear on the latter's incoming "
							"list\n",
			...
			for(int iConnect = 0; iConnect < pIncoming->Count(); iConnect++)
			{
				...
				{
					Msg("Area %d has incoming connection from area %d but does not appear on latter's outgoing "
						"connection list\n",
						area->GetID(), areaOther->GetID());
					Assert(false);
				}
