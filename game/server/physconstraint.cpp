...
ConVar g_debug_constraint_sounds("g_debug_constraint_sounds", "0", FCVAR_CHEAT,
...
BEGIN_DATADESC(CPhysConstraintSystem)
	DEFINE_PHYSPTR(m_pMachine), DEFINE_KEYFIELD(m_additionalIterations, FIELD_INTEGER, "additionaliterations"),
...
void PhysTeleportConstrainedEntity(CBaseEntity *pTeleportSource, IPhysicsObject *pObject0, IPhysicsObject *pObject1,
								   const Vector &prevPosition, const QAngle &prevAngles, bool physicsRotate)
{
...
int CPhysConstraint::DrawDebugTextOverlays()
{
	...
	if(m_pConstraint && (m_debugOverlays & OVERLAY_TEXT_BIT))
	{
		...
		if((params.bodyMassScale[0] != 1.0f && params.bodyMassScale[0] != 0.0f) ||
		   (params.bodyMassScale[1] != 1.0f && params.bodyMassScale[1] != 0.0f))
		{
...
void CPhysConstraint::GetConstraintObjects(hl_constraint_info_t &info)
{
	...
	else if(info.pObjects[0] && !info.pObjects[1])
	{
		...
		info.pObjects[0] =
			g_PhysWorldObject; // Try to make the world object consistently object0 for ease of implementation
		info.massScale[0] = info.massScale[1] = 1.0f; // no mass scale on world constraint
		info.swapped = true;
	}
...
bool CPhysConstraint::ActivateConstraint(void)
{
	...
	if(info.pObjects[0]->IsStatic() && info.pObjects[1]->IsStatic())
	{
		Warning("Constraint (%s) attached to two static objects (%s and %s)!!!\n", STRING(GetEntityName()),
				STRING(m_nameAttach1), m_nameAttach2 == NULL_STRING ? "world" : STRING(m_nameAttach2));
		return false;
	}
...
void CPhysConstraint::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
										const notify_system_event_params_t &params)
{
	...
	PhysTeleportConstrainedEntity(pNotify, m_pConstraint->GetReferenceObject(), m_pConstraint->GetAttachedObject(),
								  params.pTeleport->prevOrigin, params.pTeleport->prevAngles,
								  params.pTeleport->physicsRotate);
}
...
public:
	...
	void DrawDebugGeometryOverlays()
	{
		if(m_debugOverlays & (OVERLAY_BBOX_BIT | OVERLAY_PIVOT_BIT | OVERLAY_ABSBOX_BIT))
		{
			NDebugOverlay::Line(m_hinge.worldPosition, m_hinge.worldPosition + 48 * m_hinge.worldAxisDirection, 0, 255,
								0, false, 0);
		}
		BaseClass::DrawDebugGeometryOverlays();
	}
	...
	void NotifyVPhysicsStateChanged(IPhysicsObject *pPhysics, CBaseEntity *pEntity, bool bAwake)
	{
		...
		if(bAwake)
		{
			...
			if(GetNextThink() == TICK_NEVER_THINK)
			{
				m_soundInfo.StartThinking(this,
										  VelocitySampler::GetRelativeAngularVelocity(
											  m_pConstraint->GetAttachedObject(), m_pConstraint->GetReferenceObject()),
		...
		else
		{
			...
			if(GetNextThink() != TICK_NEVER_THINK && m_pConstraint->GetAttachedObject()->IsAsleep() &&
...
BEGIN_DATADESC(CPhysHinge)
	...
	DEFINE_KEYFIELD(m_hingeFriction, FIELD_FLOAT, "hingefriction"),
...
#if HINGE_NOTIFY
		DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_keyPoints[SimpleConstraintSoundProfile::kMIN_THRESHOLD],
						FIELD_FLOAT, "minSoundThreshold"),
		DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_keyPoints[SimpleConstraintSoundProfile::kMIN_FULL], FIELD_FLOAT,
						"maxSoundThreshold"),
		...
		DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_reversalSoundThresholds[0], FIELD_FLOAT,
						"reversalsoundthresholdSmall"),
		DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_reversalSoundThresholds[1], FIELD_FLOAT,
						"reversalsoundthresholdMedium"),
		DEFINE_KEYFIELD(m_soundInfo.m_soundProfile.m_reversalSoundThresholds[2], FIELD_FLOAT,
...
void CPhysHinge::Activate(void)
{
	...
	{
		m_soundInfo.StartThinking(this,
								  VelocitySampler::GetRelativeAngularVelocity(m_pConstraint->GetAttachedObject(),
																			  m_pConstraint->GetReferenceObject()),
								  m_hinge.worldAxisDirection);
	...
	void NotifyVPhysicsStateChanged(IPhysicsObject *pPhysics, CBaseEntity *pEntity, bool bAwake)
	{
		...
		if(bAwake)
		{
			...
			if(GetNextThink() == TICK_NEVER_THINK)
			{
				...
				m_soundInfo.StartThinking(this,
										  VelocitySampler::GetRelativeVelocity(m_pConstraint->GetAttachedObject(),
																			   m_pConstraint->GetReferenceObject()),
										  axisDirection);
...
IPhysicsConstraint *CPhysSlideConstraint::CreateConstraint(IPhysicsConstraintGroup *pGroup,
...
BEGIN_DATADESC(CPhysPulley)
	...
	DEFINE_KEYFIELD(m_position2, FIELD_POSITION_VECTOR, "position2"), DEFINE_AUTO_ARRAY(m_offset, FIELD_VECTOR),
		DEFINE_KEYFIELD(m_addLength, FIELD_FLOAT, "addlength"), DEFINE_KEYFIELD(m_gearRatio, FIELD_FLOAT, "gearratio"),
...
IPhysicsConstraint *CPhysPulley::CreateConstraint(IPhysicsConstraintGroup *pGroup, const hl_constraint_info_t &info)
{
	...
	pulley.totalLength = m_addLength + (world[0] - pulley.pulleyPosition[0]).Length() +
...
BEGIN_DATADESC(CPhysLength)
	...
	DEFINE_AUTO_ARRAY(m_offset, FIELD_VECTOR), DEFINE_KEYFIELD(m_addLength, FIELD_FLOAT, "addlength"),
		DEFINE_KEYFIELD(m_minLength, FIELD_FLOAT, "minlength"),
		DEFINE_KEYFIELD(m_vecAttach, FIELD_POSITION_VECTOR, "attachpoint"), DEFINE_FIELD(m_totalLength, FIELD_FLOAT),
END_DATADESC()
...
BEGIN_DATADESC(CRagdollConstraint)
	...
	DEFINE_KEYFIELD(m_xmin, FIELD_FLOAT, "xmin"), DEFINE_KEYFIELD(m_xmax, FIELD_FLOAT, "xmax"),
		DEFINE_KEYFIELD(m_ymin, FIELD_FLOAT, "ymin"), DEFINE_KEYFIELD(m_ymax, FIELD_FLOAT, "ymax"),
		DEFINE_KEYFIELD(m_zmin, FIELD_FLOAT, "zmin"), DEFINE_KEYFIELD(m_zmax, FIELD_FLOAT, "zmax"),
		DEFINE_KEYFIELD(m_xfriction, FIELD_FLOAT, "xfriction"), DEFINE_KEYFIELD(m_yfriction, FIELD_FLOAT, "yfriction"),
...
// Purpose: Activate/create the constraint
//-----------------------------------------------------------------------------
IPhysicsConstraint *CRagdollConstraint::CreateConstraint(IPhysicsConstraintGroup *pGroup,
														 const hl_constraint_info_t &info)
{
...
int VelocitySampler::HasReversed(const Vector &relativeVelocity, const float thresholdAcceleration[],
...
void ConstraintSoundInfo::OnThink(CPhysConstraint *pOuter, const Vector &relativeVelocity)
{
	...
	int playReversal = m_vSampler.HasReversed(relativeVelocity, m_soundProfile.m_reversalSoundThresholds,
											  SimpleConstraintSoundProfile::kREVERSAL_SOUND_ARRAY_SIZE);
	float relativeVelMag = relativeVelocity.Length(); //< magnitude of relative velocity

	CBaseEntity *pChildEntity =
		static_cast<CBaseEntity *>(pOuter->GetPhysConstraint()->GetAttachedObject()->GetGameData());
	...
	if(m_bPlayTravelSound)
	{
		if(soundVol > 0)
		{
			...
			{ // if a sound does not exist, create it
				...
				m_pTravelSound =
					controller.SoundCreate(filter, pChildEntity->entindex(),
...
void ConstraintSoundInfo::StartThinking(CPhysConstraint *pOuter, const Vector &relativeVelocity,
