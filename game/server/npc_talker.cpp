...
BEGIN_SIMPLE_DATADESC(CNPCSimpleTalkerExpresser)
	...
	DEFINE_AUTO_ARRAY(m_szMonologSentence, FIELD_CHARACTER), DEFINE_FIELD(m_iMonologIndex, FIELD_INTEGER),
		DEFINE_FIELD(m_fMonologSuspended, FIELD_BOOLEAN), DEFINE_FIELD(m_hMonologTalkTarget, FIELD_EHANDLE),
END_DATADESC()

BEGIN_DATADESC(CNPCSimpleTalker)
	DEFINE_FIELD(m_useTime, FIELD_TIME), DEFINE_FIELD(m_flNextIdleSpeechTime, FIELD_TIME),
		DEFINE_FIELD(m_nSpeak, FIELD_INTEGER), DEFINE_FIELD(m_iszUse, FIELD_STRING),
...
bool CNPCSimpleTalker::ShouldSuspendMonolog(void)
{
	...
	flDist = ((assert_cast<CNPCSimpleTalkerExpresser *>(GetExpresser()))->GetMonologueTarget()->GetAbsOrigin() -
			  GetAbsOrigin())
				 .Length();
...
Activity CNPCSimpleTalker::NPC_TranslateActivity(Activity eNewActivity)
{
	...
	else if((eNewActivity == ACT_SIGNAL3) && (SelectWeightedSequence(ACT_SIGNAL3) == ACTIVITY_NOT_AVAILABLE))
	{
		return ACT_IDLE;
	}
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPCSimpleTalker::PlayScriptedSentence(const char *pszSentence, float delay, float volume, soundlevel_t soundlevel,
...
int CNPCSimpleTalker::SelectNonCombatSpeechSchedule()
{
	...
	if(AI_IsSinglePlayer() && GetExpresser()->CanSpeak() && HasCondition(COND_SEE_PLAYER) &&
	   random->RandomInt(0, 6) == 0)
	{
		CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
		Assert(pPlayer);
...
int CNPCSimpleTalkerExpresser::SpeakRawSentence(const char *pszSentence, float delay, float volume,
...
bool CNPCSimpleTalker::OnBehaviorChangeStatus(CAI_BehaviorBase *pBehavior, bool fCanFinishSchedule)
{
	...
	if(!interrupt)
	{
		interrupt = (dynamic_cast<CAI_FollowBehavior *>(pBehavior) != NULL &&
