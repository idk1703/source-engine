...
BEGIN_DATADESC(CAmbientGeneric)
	...
	DEFINE_KEYFIELD(m_iszSound, FIELD_SOUNDNAME, "message"), DEFINE_KEYFIELD(m_radius, FIELD_FLOAT, "radius"),
		...
		DEFINE_FIELD(m_flMaxRadius, FIELD_FLOAT), DEFINE_FIELD(m_fActive, FIELD_BOOLEAN),
		DEFINE_FIELD(m_fLooping, FIELD_BOOLEAN), DEFINE_FIELD(m_iSoundLevel, FIELD_INTEGER),
		...
		DEFINE_INPUTFUNC(FIELD_FLOAT, "Pitch", InputPitch), DEFINE_INPUTFUNC(FIELD_FLOAT, "Volume", InputVolume),
		DEFINE_INPUTFUNC(FIELD_FLOAT, "FadeIn", InputFadeIn), DEFINE_INPUTFUNC(FIELD_FLOAT, "FadeOut", InputFadeOut),
...
void CAmbientGeneric::Spawn(void)
{
	...
	{
		Warning("Empty %s (%s) at %.2f, %.2f, %.2f\n", GetClassname(), GetDebugName(), GetAbsOrigin().x,
...
void CAmbientGeneric::Activate(void)
{
	...
	if(m_fActive)
	{
		...
		if(gpGlobals->eLoadType == MapLoad_Transition || gpGlobals->eLoadType == MapLoad_LoadGame ||
...
void CAmbientGeneric::SetTransmit(CCheckTransmitInfo *pInfo, bool bAlways)
{
	...
	if((m_flMaxRadius < 0) ||
	   (pClient->GetAbsOrigin().DistToSqr(m_hSoundSource->GetAbsOrigin()) <= m_flMaxRadius * m_flMaxRadius))
	{
...
void CAmbientGeneric::SendSound(SoundFlags_t flags)
{
	...
	if(pSoundSource)
	{
		if(flags == SND_STOP)
		{
			UTIL_EmitAmbientSound(pSoundSource->GetSoundSourceIndex(), pSoundSource->GetAbsOrigin(), m_szSoundFile, 0,
								  SNDLVL_NONE, flags, 0);
		}
		else
		{
...
int SENTENCEG_PlayRndI(edict_t *entity, int isentenceg, float volume, soundlevel_t soundlevel, int flags, int pitch)
{
	...
	if((ipick > 0) && name[0])
	{
		...
		CBaseEntity::EmitSentenceByIndex(filter, ENTINDEX(entity), CHAN_VOICE, sentenceIndex, volume, soundlevel, flags,
										 pitch);
								 ...
								 int pitch)
{
	...
	{
		CPASAttenuationFilter filter(GetContainingEntity(entity), soundlevel);
		CBaseEntity::EmitSentenceByIndex(filter, ENTINDEX(entity), CHAN_VOICE, iSentenceIndex, volume, soundlevel,
										 flags, pitch);
	}
}

int SENTENCEG_PlayRndSz(edict_t *entity, const char *szgroupname, float volume, soundlevel_t soundlevel, int flags,
						int pitch)
{
	char name[64];
	int ipick;
...
int SENTENCEG_PlaySequentialSz(edict_t *entity, const char *szgroupname, float volume, soundlevel_t soundlevel,
