...
ConVar sv_netvisdist("sv_netvisdist", "10000", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
					 "Test networking visibility distance");

// This table encodes edict data.
void SendProxy_AnimTime(const SendProp *pProp, const void *pStruct, const void *pVarData, DVariant *pOut, int iElement,
						int objectID)
{
...
// This table encodes edict data.
void SendProxy_SimulationTime(const SendProp *pProp, const void *pStruct, const void *pVarData, DVariant *pOut,
...
BEGIN_SEND_TABLE_NOBASE(CBaseEntity, DT_AnimTimeMustBeFirst)
	...
	SendPropInt(SENDINFO(m_flAnimTime), 8, SPROP_UNSIGNED | SPROP_CHANGES_OFTEN | SPROP_ENCODED_AGAINST_TICKCOUNT,
				SendProxy_AnimTime),
END_SEND_TABLE()

#if !defined(NO_ENTITY_PREDICTION)
BEGIN_SEND_TABLE_NOBASE(CBaseEntity, DT_PredictableId)
	SendPropPredictableId(SENDINFO(m_PredictableID)), SendPropInt(SENDINFO(m_bIsPlayerSimulated), 1, SPROP_UNSIGNED),
END_SEND_TABLE()

static void *SendProxy_SendPredictableId(const SendProp *pProp, const void *pStruct, const void *pVarData,
										 CSendProxyRecipients *pRecipients, int objectID)
{
	CBaseEntity *pEntity = (CBaseEntity *)pStruct;
...
void SendProxy_Origin(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
					  int objectID)
{
...
//--------------------------------------------------------------------------------------------------------
void SendProxy_OriginXY(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
						int objectID)
{
	CBaseEntity *entity = (CBaseEntity *)pStruct;
	Assert(entity);
...
// Used when breaking up origin, note we still have to deal with StepSimulation
//--------------------------------------------------------------------------------------------------------
void SendProxy_OriginZ(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
					   int objectID)
{
	CBaseEntity *entity = (CBaseEntity *)pStruct;
	Assert(entity);
...
void SendProxy_Angles(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
					  int objectID)
{
	CBaseEntity *entity = (CBaseEntity *)pStruct;
	Assert(entity);
...
SendPropDataTable("AnimTimeMustBeFirst", 0, &REFERENCE_SEND_TABLE(DT_AnimTimeMustBeFirst),
	...
	SendPropInt(SENDINFO(m_flSimulationTime), SIMULATION_TIME_WINDOW_BITS,
				SPROP_UNSIGNED | SPROP_CHANGES_OFTEN | SPROP_ENCODED_AGAINST_TICKCOUNT, SendProxy_SimulationTime),

#if PREDICTION_ERROR_CHECK_LEVEL > 1
	SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_NOSCALE | SPROP_CHANGES_OFTEN, 0.0f, HIGH_DEFAULT,
				   SendProxy_Origin),
#else
	...
	SendPropInt(SENDINFO(m_nRenderFX), 8, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nRenderMode), 8, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_fEffects), EF_MAX_BITS, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_clrRender), 32, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iTeamNum), TEAMNUM_NUM_BITS, 0),
	SendPropInt(SENDINFO(m_CollisionGroup), 5, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_flElasticity), 0, SPROP_COORD),
	SendPropFloat(SENDINFO(m_flShadowCastDistance), 12, SPROP_UNSIGNED), SendPropEHandle(SENDINFO(m_hOwnerEntity)),
	SendPropEHandle(SENDINFO(m_hEffectEntity)), SendPropEHandle(SENDINFO_NAME(m_hMoveParent, moveparent)),
	...
	SendPropInt(SENDINFO(m_bAnimatedEveryTick), 1, SPROP_UNSIGNED), SendPropBool(SENDINFO(m_bAlternateSorting)),

#ifdef TF_DLL
	SendPropArray3(SENDINFO_ARRAY3(m_nModelIndexOverrides),
				   SendPropInt(SENDINFO_ARRAY(m_nModelIndexOverrides), SP_MODEL_INDEX_BITS, 0)),
#endif
...
static CUtlHashtable<CBaseEntityModelLoadProxy, empty_t, PointerHashFunctor, PointerEqualFunctor, CBaseEntity *>
...
void CBaseEntity::DrawTimedOverlays(void)
{
	...
	while(pTO)
	{
		...
		if((!CBaseEntity::Debug_IsPaused() && gpGlobals->curtime > pTO->msgEndTime) || (nCount > 10))
		{
			if(pLastTO)
			{
	...
	if(m_debugOverlays & (OVERLAY_BBOX_BIT | OVERLAY_PIVOT_BIT))
	{
		...
		if(VPhysicsGetObject())
		{
			...
			if(GetMoveType() != MOVETYPE_VPHYSICS)
			{
				...
				{
					Vector mins, maxs;
					physcollision->CollideGetAABB(&mins, &maxs, VPhysicsGetObject()->GetCollide(), vec3_origin,
												  vec3_angle);
...
void CBaseEntity::ValidateEntityConnections()
{
	...
	if(ClassMatches("scripted_*") || ClassMatches("trigger_relay") || ClassMatches("trigger_auto") ||
	   ClassMatches("path_*") || ClassMatches("monster_*") || ClassMatches("trigger_teleport") ||
	   ClassMatches("func_train") || ClassMatches("func_tracktrain") || ClassMatches("func_plat*") ||
	   ClassMatches("npc_*") || ClassMatches("info_big*") || ClassMatches("env_texturetoggle") ||
	   ClassMatches("env_render") || ClassMatches("func_areaportalwindow") || ClassMatches("point_view*") ||
	   ClassMatches("func_traincontrols") || ClassMatches("multisource") || ClassMatches("xen_plant*"))
		return;
	...
	while(dmap)
	{
		...
		for(int i = 0; i < fields; i++)
		{
			...
			{
				CBaseEntityOutput *pOutput = (CBaseEntityOutput *)((uintp)this + (int)dataDesc->fieldOffset[0]);
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseEntity::FireNamedOutput(const char *pszOutput, variant_t variant, CBaseEntity *pActivator,
								  CBaseEntity *pCaller, float flDelay)
{
	if(pszOutput == NULL)
		return;
	...
	if(!IsEFlagSet(EFL_NO_DAMAGE_FORCES))
	{
		...
		else
		{
			   ...
			   !info.GetAttacker()->IsSolidFlagSet(FSOLID_TRIGGER))
			{
				...
				float flForce = info.GetDamage() *
								((32 * 32 * 72.0) / (WorldAlignSize().x * WorldAlignSize().y * WorldAlignSize().z)) * 5;
...
int CBaseEntity::TakeDamage(const CTakeDamageInfo &inputInfo)
{
	...
	if(bHasPhysicsForceDamage && inputInfo.GetDamageType() != DMG_GENERIC)
	{
		...
		// damage type to DMG_GENERIC, or | DMG_CRUSH if you need to preserve the damage type for purposes of HUD
		// display.
...
BEGIN_SIMPLE_DATADESC(thinkfunc_t)
	...
	DEFINE_FIELD(m_iszContext, FIELD_STRING),
		...
		DEFINE_FIELD(m_nNextThinkTick, FIELD_TICK), DEFINE_FIELD(m_nLastThinkTick, FIELD_TICK),
...
BEGIN_DATADESC_NO_BASE(CBaseEntity)
	...
	DEFINE_KEYFIELD(m_iClassname, FIELD_STRING, "classname"),
		...
		DEFINE_KEYFIELD(m_iHammerID, FIELD_INTEGER,
						"hammerid"), // save ID numbers so that entities can be tracked between save/restore and vmf

		DEFINE_KEYFIELD(m_flSpeed, FIELD_FLOAT, "speed"), DEFINE_KEYFIELD(m_nRenderFX, FIELD_CHARACTER, "renderfx"),
		DEFINE_KEYFIELD(m_nRenderMode, FIELD_CHARACTER, "rendermode"),

		// Consider moving to CBaseAnimating?
		DEFINE_FIELD(m_flPrevAnimTime, FIELD_TIME), DEFINE_FIELD(m_flAnimTime, FIELD_TIME),
		DEFINE_FIELD(m_flSimulationTime, FIELD_TIME), DEFINE_FIELD(m_nLastThinkTick, FIELD_TICK),
	...
	// DEFINE_FIELD( m_PredictableID, CPredictableId ),
#endif
		DEFINE_FIELD(touchStamp, FIELD_INTEGER), DEFINE_CUSTOM_FIELD(m_aThinkFunctions, thinkcontextFuncs),
		//								m_iCurrentThinkContext (not saved, debug field only, and think transient to
		//boot)

		DEFINE_UTLVECTOR(m_ResponseContexts, FIELD_EMBEDDED),
		DEFINE_KEYFIELD(m_iszResponseContext, FIELD_STRING, "ResponseContext"),

		DEFINE_FIELD(m_pfnThink, FIELD_FUNCTION), DEFINE_FIELD(m_pfnTouch, FIELD_FUNCTION),
		DEFINE_FIELD(m_pfnUse, FIELD_FUNCTION), DEFINE_FIELD(m_pfnBlocked, FIELD_FUNCTION),
		DEFINE_FIELD(m_pfnMoveDone, FIELD_FUNCTION),

		DEFINE_FIELD(m_lifeState, FIELD_CHARACTER), DEFINE_FIELD(m_takedamage, FIELD_CHARACTER),
		DEFINE_KEYFIELD(m_iMaxHealth, FIELD_INTEGER, "max_health"), DEFINE_KEYFIELD(m_iHealth, FIELD_INTEGER, "health"),
		...
		DEFINE_GLOBAL_FIELD(m_pParent, FIELD_EHANDLE), DEFINE_FIELD(m_iParentAttachment, FIELD_CHARACTER),
		DEFINE_GLOBAL_FIELD(m_hMoveParent, FIELD_EHANDLE), DEFINE_GLOBAL_FIELD(m_hMoveChild, FIELD_EHANDLE),
		DEFINE_GLOBAL_FIELD(m_hMovePeer, FIELD_EHANDLE),

		DEFINE_FIELD(m_iEFlags, FIELD_INTEGER),

		DEFINE_FIELD(m_iName, FIELD_STRING), DEFINE_EMBEDDED(m_Collision), DEFINE_EMBEDDED(m_Network),

		DEFINE_FIELD(m_MoveType, FIELD_CHARACTER), DEFINE_FIELD(m_MoveCollide, FIELD_CHARACTER),
		DEFINE_FIELD(m_hOwnerEntity, FIELD_EHANDLE), DEFINE_FIELD(m_CollisionGroup, FIELD_INTEGER),
		DEFINE_PHYSPTR(m_pPhysicsObject), DEFINE_FIELD(m_flElasticity, FIELD_FLOAT),
		DEFINE_KEYFIELD(m_flShadowCastDistance, FIELD_FLOAT, "shadowcastdist"),
		DEFINE_FIELD(m_flDesiredShadowCastDistance, FIELD_FLOAT),

		DEFINE_INPUT(m_iInitialTeamNum, FIELD_INTEGER, "TeamNum"), DEFINE_FIELD(m_iTeamNum, FIELD_INTEGER),

		//	DEFINE_FIELD( m_bSentLastFrame, FIELD_INTEGER ),

		DEFINE_FIELD(m_hGroundEntity, FIELD_EHANDLE), DEFINE_FIELD(m_flGroundChangeTime, FIELD_TIME),
		DEFINE_GLOBAL_KEYFIELD(m_ModelName, FIELD_MODELNAME, "model"),

		DEFINE_KEYFIELD(m_vecBaseVelocity, FIELD_VECTOR, "basevelocity"), DEFINE_FIELD(m_vecAbsVelocity, FIELD_VECTOR),
		...
		DEFINE_ARRAY(m_rgflCoordinateFrame, FIELD_FLOAT,
					 12), // NOTE: MUST BE IN LOCAL SPACE, NOT POSITION_VECTOR!!! (see CBaseEntity::Restore)

		DEFINE_KEYFIELD(m_nWaterLevel, FIELD_CHARACTER, "waterlevel"), DEFINE_FIELD(m_nWaterType, FIELD_CHARACTER),
		DEFINE_FIELD(m_pBlocker, FIELD_EHANDLE),

		DEFINE_KEYFIELD(m_flGravity, FIELD_FLOAT, "gravity"), DEFINE_KEYFIELD(m_flFriction, FIELD_FLOAT, "friction"),
		...
		DEFINE_KEYFIELD(m_flLocalTime, FIELD_FLOAT, "ltime"), DEFINE_FIELD(m_flVPhysicsUpdateLocalTime, FIELD_FLOAT),
		DEFINE_FIELD(m_flMoveDoneTime, FIELD_FLOAT),

		//	DEFINE_FIELD( m_nPushEnumCount, FIELD_INTEGER ),

		DEFINE_FIELD(m_vecAbsOrigin, FIELD_POSITION_VECTOR), DEFINE_KEYFIELD(m_vecVelocity, FIELD_VECTOR, "velocity"),
		DEFINE_KEYFIELD(m_iTextureFrameIndex, FIELD_CHARACTER, "texframeindex"),
		DEFINE_FIELD(m_bSimulatedEveryTick, FIELD_BOOLEAN), DEFINE_FIELD(m_bAnimatedEveryTick, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bAlternateSorting, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_spawnflags, FIELD_INTEGER, "spawnflags"),
		DEFINE_FIELD(m_nTransmitStateOwnedCounter, FIELD_CHARACTER), DEFINE_FIELD(m_angAbsRotation, FIELD_VECTOR),
		DEFINE_FIELD(m_vecOrigin,
					 FIELD_VECTOR), // NOTE: MUST BE IN LOCAL SPACE, NOT POSITION_VECTOR!!! (see CBaseEntity::Restore)
...
//	DEFINE_FIELD( m_hPlayerSimulationOwner, FIELD_EHANDLE ),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Use", InputUse), DEFINE_INPUTFUNC(FIELD_INTEGER, "Alpha", InputAlpha),
		...
		DEFINE_OUTPUT(m_OnUser1, "OnUser1"), DEFINE_OUTPUT(m_OnUser2, "OnUser2"), DEFINE_OUTPUT(m_OnUser3, "OnUser3"),
		DEFINE_OUTPUT(m_OnUser4, "OnUser4"),

		// Function Pointers
		DEFINE_FUNCTION(SUB_Remove), DEFINE_FUNCTION(SUB_DoNothing), DEFINE_FUNCTION(SUB_StartFadeOut),
		DEFINE_FUNCTION(SUB_StartFadeOutInstant), DEFINE_FUNCTION(SUB_FadeOut), DEFINE_FUNCTION(SUB_Vanish),
		DEFINE_FUNCTION(SUB_CallUseToggle), DEFINE_THINKFUNC(ShadowCastDistThink),
...
int CBaseEntity::ObjectCaps(void)
{
	...
	if(pParent)
	{
		...
		if(!bIsBrush)
			caps &= (FCAP_ACROSS_TRANSITION | FCAP_IMPULSE_USE | FCAP_CONTINUOUS_USE | FCAP_ONOFF_USE |
...
static void ComputePushStartMatrix(matrix3x4_t &start, CBaseEntity *pEntity, const pushblock_t &params)
{
	...
	else
	{
		...
		localAngles =
			params.pRootParent->GetAbsAngles() - params.pRootParent->GetLocalAngularVelocity() * params.movetime;
	}
			...
			if(dist > 1)
			{
				...
				if(pEntity == params.pRootParent)
				{
					if(params.pList)
					{
						...
						movementAmount =
							targetAmount + DotProduct(params.pList->localOrigin, localVel) - localTargetAmt;
					}
				}
				else
				{
...
void CBaseEntity::VPhysicsUpdatePusher(IPhysicsObject *pPhysics)
{
	...
	if(GetSolid() != SOLID_BSP && pPhysics->GetShadowPosition(&origin, &angles))
	{
		...
		// NDebugOverlay::BoxAngles( origin, CollisionProp()->OBBMins(), CollisionProp()->OBBMaxs(), angles, 255,0,0,0,
		// gpGlobals->frametime);
				...
				if(physLocalTime <= (m_flVPhysicsUpdateLocalTime + movetime * 0.99f))
				{
					...
					if(pList)
					{
						...
						CBaseEntity *pPhysicsBlocker =
							FindPhysicsBlocker(VPhysicsGetObject(), *pList, pList->pushVec[0]);
						if(pPhysicsBlocker)
						{
...
void CBaseEntity::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	...
	if(pHitEntity == this)
	{
		PhysCollisionSound(this, pEvent->pObjects[index], CHAN_BODY, pEvent->surfaceProps[index],
						   pEvent->surfaceProps[otherIndex], pEvent->deltaCollisionTime, pEvent->collisionSpeed);
	}
	else
	{
		PhysCollisionSound(this, pEvent->pObjects[index], CHAN_STATIC, pEvent->surfaceProps[index],
						   pEvent->surfaceProps[otherIndex], pEvent->deltaCollisionTime, pEvent->collisionSpeed);
	}
...
bool CBaseEntity::VPhysicsIsFlesh(void)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		if(pSurfaceData->game.material == CHAR_TEX_ANTLION || pSurfaceData->game.material == CHAR_TEX_FLESH ||
		   pSurfaceData->game.material == CHAR_TEX_BLOODYFLESH || pSurfaceData->game.material == CHAR_TEX_ALIENFLESH)
			return true;
	}
	return false;
}
...
bool CBaseEntity::Intersects(CBaseEntity *pOther)
{
	...
	return IsOBBIntersectingOBB(pMyProp->GetCollisionOrigin(), pMyProp->GetCollisionAngles(), pMyProp->OBBMins(),
								pMyProp->OBBMaxs(), pOtherProp->GetCollisionOrigin(), pOtherProp->GetCollisionAngles(),
								pOtherProp->OBBMins(), pOtherProp->OBBMaxs());
}
...
ConVar ai_debug_los("ai_debug_los", "0", FCVAR_CHEAT,
					"NPC Line-Of-Sight debug mode. If 1, solid entities that block NPC LOC will be highlighted with "
					"white bounding boxes. If 2, it'll show non-solid entities that would do it if they were solid.",
					CC_AI_LOS_Debug);
...
bool CBaseEntity::IsInWorld(void) const
{
	...
	if(GetAbsOrigin().z <= MIN_COORD_INTEGER)
		return false;
	// speed
	if(GetAbsVelocity().x >= 2000)
		return false;
	if(GetAbsVelocity().y >= 2000)
		return false;
	if(GetAbsVelocity().z >= 2000)
		return false;
	if(GetAbsVelocity().x <= -2000)
		return false;
	if(GetAbsVelocity().y <= -2000)
...
CBaseEntity *CBaseEntity::Create(const char *szName, const Vector &vecOrigin, const QAngle &vecAngles,
								 CBaseEntity *pOwner)
{
...
// NOTE: szName must be a pointer to constant memory, e.g. "NPC_class" because the entity
// will keep a pointer to it after this call.
CBaseEntity *CBaseEntity::CreateNoSpawn(const char *szName, const Vector &vecOrigin, const QAngle &vecAngles,
										CBaseEntity *pOwner)
{
...
#endif
			// We only need to be back in the parent's list because we're already in the right place and with the right
			// data
			LinkChild(m_pParent, this);
		}
	}
...
void CBaseEntity::DrawInputOverlay(const char *szInputName, CBaseEntity *pCaller, variant_t Value)
{
	...
	if(Value.FieldType() == FIELD_INTEGER)
	{
		Q_snprintf(bigstring, sizeof(bigstring), "%3.1f  (%s,%d) <-- (%s)\n", gpGlobals->curtime, szInputName,
				   Value.Int(), pCaller ? pCaller->GetDebugName() : NULL);
	}
	else if(Value.FieldType() == FIELD_STRING)
	{
		Q_snprintf(bigstring, sizeof(bigstring), "%3.1f  (%s,%s) <-- (%s)\n", gpGlobals->curtime, szInputName,
				   Value.String(), pCaller ? pCaller->GetDebugName() : NULL);
	}
	else
	{
	...
	if(Value.FieldType() == FIELD_INTEGER)
	{
		DevMsg(2, "input: (%s,%d) -> (%s,%s), from (%s)\n", szInputName, Value.Int(), STRING(m_iClassname),
			   GetDebugName(), pCaller ? pCaller->GetDebugName() : NULL);
	}
	else if(Value.FieldType() == FIELD_STRING)
	{
		DevMsg(2, "input: (%s,%s) -> (%s,%s), from (%s)\n", szInputName, Value.String(), STRING(m_iClassname),
			   GetDebugName(), pCaller ? pCaller->GetDebugName() : NULL);
	}
	else
...
void CBaseEntity::DrawOutputOverlay(CEventAction *ev)
{
	...
	if(ev->m_flDelay)
	{
		Q_snprintf(bigstring, sizeof(bigstring), "%3.1f  (%s) --> (%s),%.1f) \n", gpGlobals->curtime,
				   STRING(ev->m_iTargetInput), STRING(ev->m_iTarget), ev->m_flDelay);
	}
	else
	{
		Q_snprintf(bigstring, sizeof(bigstring), "%3.1f  (%s) --> (%s)\n", gpGlobals->curtime,
				   STRING(ev->m_iTargetInput), STRING(ev->m_iTarget));
	}
	AddTimedOverlay(bigstring, 10.0);
	...
	if(ev->m_flDelay)
	{
		DevMsg(2, "output: (%s,%s) -> (%s,%s,%.1f)\n", STRING(m_iClassname), GetDebugName(), STRING(ev->m_iTarget),
			   STRING(ev->m_iTargetInput), ev->m_flDelay);
	}
	else
	{
...
void CBaseEntity::OnEntityEvent(EntityEvent_t event, void *pEventData)
{
	switch(event)
	{
		...
		{
			int nContents = (uintp)pEventData;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseEntity::AcceptInput(const char *szInputName, CBaseEntity *pActivator, CBaseEntity *pCaller, variant_t Value,
							  int outputID)
{
				...
				if(!Q_stricmp(dmap->dataDesc[i].externalName, szInputName))
				{
					...
					if(Value.FieldType() != dmap->dataDesc[i].fieldType)
					{
						if(!(Value.FieldType() == FIELD_VOID &&
							 dmap->dataDesc[i].fieldType == FIELD_STRING)) // allow empty strings
						{
	...
	DevMsg(2, "unhandled input: (%s) -> (%s,%s)\n", szInputName, STRING(m_iClassname),
		   GetDebugName() /*,", from (%s,%s)" STRING(pCaller->m_iClassname), STRING(pCaller->m_iName)*/);
	return false;
}
	...
	for(datamap_t *dmap = GetDataDescMap(); dmap != NULL; dmap = dmap->baseMap)
	{
		...
		for(int i = 0; i < dmap->dataNumFields; i++)
		{
			if(dmap->dataDesc[i].flags & (FTYPEDESC_OUTPUT | FTYPEDESC_KEY))
			{
				...
				{
					var->Set(dmap->dataDesc[i].fieldType,
...
void CBaseEntity::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
...
static void TeleportEntity(CBaseEntity *pSourceEntity, TeleportListEntry_t &entry, const Vector *newPosition,
						   const QAngle *newAngles, const Vector *newVelocity)
{
...
static CUtlCachedFileData<CModelSoundsCache> g_ModelSoundsCache("modelsounds.cache", MODELSOUNDSCACHE_VERSION, 0,
	...
	// particles
	{
		...
		if(pModelKeyValues->LoadFromBuffer(modelinfo->GetModelName(pModel), modelinfo->GetModelKeyValueText(pModel)))
		{
			...
			if(pParticleEffects)
			{
				...
				for(KeyValues *pSingleEffect = pParticleEffects->GetFirstSubKey(); pSingleEffect;
					pSingleEffect = pSingleEffect->GetNextKey())
				{
		...
		if(studioHdr.IsValid())
		{
			...
			for(int i = 0; i < nSeqCount; ++i)
			{
				...
				for(int j = 0; j < nEventCount; ++j)
				{
					...
					// The disk based solution was not needed. Now at runtime partly due to already crawling the
					// sequences for the particles and the expensive part was redundant PrecacheScriptSound(), which is
					// now prevented by a local symbol table.
					if(IsX360())
					{
						switch(pEvent->event)
						{
...
int CBaseEntity::PrecacheModel(const char *name, bool bPreload)
{
	...
	if(!CBaseEntity::IsPrecacheAllowed())
	{
		...
		{
			DevMsg("Late precache of %s -- not necessarily a bug now that we allow ~everything to be dynamically "
				   "loaded.\n",
				   name);
		}
	}
...
static ConCommand ent_text(
	...
	"Displays text debugging information about the given entity(ies) on top of the entity (See Overlay "
...
static ConCommand ent_bbox("ent_bbox", CC_Ent_BBox,
						   "Displays the movement bounding box for the given entity(ies) in orange.  Some entites will "
						   "also display entity specific overlays.\n\tArguments:   	{entity_name} / {class_name} / no "
...
static ConCommand ent_absbox(
	...
	"Displays the total bounding box for the given entity(s) in green.  Some entites will also display entity specific "
	"overlays.\n\tArguments:   	{entity_name} / {class_name} / no argument picks what player is looking at ",
	FCVAR_CHEAT);
...
static ConCommand ent_attachments("ent_attachments", CC_Ent_AttachmentPoints,
...
static ConCommand ent_viewoffset("ent_viewoffset", CC_Ent_ViewOffset,
...
static ConCommand ent_remove("ent_remove", CC_Ent_Remove,
							 "Removes the given entity(s)\n\tArguments:   	{entity_name} / {class_name} / no argument "
...
static ConCommand ent_setname("ent_setname", CC_Ent_SetName,
							  "Sets the targetname of the given entity(s)\n\tArguments:   	{new entity name} "
							  "{entity_name} / {class_name} / no argument picks what player is looking at ",
							  FCVAR_CHEAT);
...
static ConCommand find_ent("find_ent", CC_Find_Ent,
						   "Find and list all entities with classnames or targetnames that contain the specified "
...
public:
	virtual void CommandCallback(const CCommand &command)
	{
		...
		else
		{
			...
			// Don't allow them to run anything on a point_servercommand unless they're the host player. Otherwise they
			// can ent_fire and run any command on the server. Admittedly, they can only do the ent_fire if sv_cheats is
			// on, but people complained about users resetting the rcon password if the server briefly turned on cheats
			// like this:
			...
			// So, I'm removing the ability for anyone to execute ent_fires on dedicated servers (we can't check to see
			// if this command is going to connect with a point_servercommand entity here, because they could delay the
			// event and create it later).
...
static ConCommand ent_fire("ent_fire", &g_EntFireAutoComplete,
						   "Usage:\n   ent_fire <target> [action] [value] [delay]\n", FCVAR_CHEAT,
...
static ConCommand ent_messages(
	...
	"Toggles input/output message display for the selected entity(ies).  The name of the entity will be displayed as "
	"well as any messages that it sends or receives.\n\tArguments:   	{entity_name} / {class_name} / no argument "
	"picks what player is looking at",
	FCVAR_CHEAT);
...
static ConCommand ent_pause(
	...
	"Toggles pausing of input/output message processing for entities.  When turned on processing of all message will "
	"stop.  Any messages displayed with 'ent_messages' will stop fading and be displayed indefinitely. To step through "
	"the messages one by one use 'ent_step'.",
	FCVAR_CHEAT);
...
static ConCommand picker(
	...
	"Toggles 'picker' mode.  When picker is on, the bounding box, pivot and debugging text is displayed for whatever "
...
static ConCommand ent_step(
	"ent_step", CC_Ent_Step,
	"When 'ent_pause' is set this will step through one waiting input / output message at a time.", FCVAR_CHEAT);
...
void CBaseEntity::CalcAbsolutePosition(void)
{
	...
	// Do this callback *after* we have updated the position, and (importantly) after we clear the dirty flag, because
	// this callback can potentially end up recursively calling back in here, so the dirty flag must be cleared to break
...
void CBaseEntity::SetLocalVelocity(const Vector &inVecVelocity)
{
	...
	switch(CheckEntityVelocity(vecVelocity))
	{
		...
		case 0:
			if(CheckEmitReasonablePhysicsSpew())
			{
				Warning("Clamping SetLocalVelocity(%f,%f,%f) on %s\n", inVecVelocity.x, inVecVelocity.y,
						inVecVelocity.z, GetDebugName());
			}
			break;
	}
...
void CBaseEntity::SetLocalAngularVelocity(const QAngle &vecAngVelocity)
{
	...
	if(!IsEntityQAngleVelReasonable(vecAngVelocity))
	{
		if(CheckEmitReasonablePhysicsSpew())
		{
			Warning("Bad SetLocalAngularVelocity(%f,%f,%f) on %s\n", vecAngVelocity.x, vecAngVelocity.y,
					vecAngVelocity.z, GetDebugName());
		}
		Assert(false);
		return;
	}
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CBaseEntity *CBaseEntity::CreatePredictedEntityByName(const char *classname, const char *module, int line,
													  bool persist /* = false */)
{
#if !defined(NO_ENTITY_PREDICTION)
...
void CBaseEntity::InputAddOutput(inputdata_t &inputdata)
{
	...
	{
		Warning("AddOutput input fired with bad string. Format: <output name> "
				"<targetname>,<inputname>,<parameter>,<delay>,<max times to fire (-1 == infinite)>\n");
	}
}
...
void CBaseEntity::DispatchResponse(const char *conceptName)
{
	...
	switch(result.GetType())
	{
		...
		case RESPONSE_SENTENCE:
		{
			...
			CBaseEntity::EmitSentenceByIndex(filter, entindex(), CHAN_VOICE, sentenceIndex, 1, result.GetSoundLevel(),
											 0, PITCH_NORM);
		}
		break;
...
static ConCommand ent_autoaim("ent_autoaim", CC_Ent_Autoaim,
...
void CBaseEntity::ComputeStepSimulationNetwork(StepSimulationData *step)
{
	...
	if(step->m_bOriginActive)
	{
		...
		else
		{
			...
			{
				Hermite_Spline(step->m_Previous2.vecOrigin, step->m_Previous.vecOrigin, step->m_Next.vecOrigin, frac,
							   step->m_vecNetworkOrigin);
			}
		}
	}
	...
	if(step->m_bAnglesActive)
	{
		...
		else
		{
			...
			else
			{
				...
				Quaternion outangles;
				Hermite_Spline(step->m_Previous2.qRotation, step->m_Previous.qRotation, step->m_Next.qRotation, frac,
							   outangles);
				QuaternionAngles(outangles, step->m_angNetworkAngles);
			}
		}
...
bool CBaseEntity::UseStepSimulationNetworkOrigin(const Vector **out_v)
{
	...
	if(g_bTestMoveTypeStepSimulation && GetMoveType() == MOVETYPE_STEP && HasDataObjectType(STEPSIMULATION))
	{
		StepSimulationData *step = (StepSimulationData *)GetDataObject(STEPSIMULATION);
...
//-----------------------------------------------------------------------------
// Purpose: For each client who appears to be a valid recipient, checks the client has disabled CC and if so, removes
// them from
//  the recipient list.
// Input  : filter -
//-----------------------------------------------------------------------------
									  ...
									  float soundtime /*=0.0f*/)
{
	...
	enginesound->EmitSentenceByIndex(filter, iEntIndex, iChannel, iSentenceIndex, flVolume, iSoundlevel, iFlags, iPitch,
									 0, pOrigin, pDirection, &dummy, bUpdatePositions, soundtime);
}
...
void CBaseEntity::SetRefEHandle(const CBaseHandle &handle)
{
	...
	if(edict())
	{
		...
		edict()->m_NetworkSerialNumber =
			m_RefEHandle.GetSerialNumber() & ((1 << NUM_NETWORKED_EHANDLE_SERIAL_NUMBER_BITS) - 1);
	}
}
...
static ConCommand ent_create(
	...
	"Creates an entity of the given type where the player is looking.  Additional parameters can be passed in in the "
	"form: ent_create <entity name> <param 1 name> <param 1> <param 2 name> <param 2>...<param N name> <param N>",
...
static ConCommand ent_orient(
	...
	"Orient the specified entity to match the player's angles. By default, only orients target entity's YAW. Use the "
	"'allangles' option to orient on all axis.\n\tFormat: ent_orient <entity name> <optional: allangles>",
	FCVAR_CHEAT);