...
BEGIN_DATADESC(CFuncNavCost)
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable), DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),
		DEFINE_KEYFIELD(m_iszTags, FIELD_STRING, "tags"), DEFINE_KEYFIELD(m_team, FIELD_INTEGER, "team"),
...
int CFuncNavBlocker::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		for(int i = 0; i < collector.m_area.Count(); ++i)
		{
			...
			if(debugoverlay)
			{
				debugoverlay->AddBoxOverlay(vec3_origin, areaExtent.lo, areaExtent.hi, vec3_angle, 0, 255, 0, 10,
	...
	FOR_EACH_LL(gm_NavBlockers, iBlocker)
	{
		...
		for(i = 0; i < MAX_NAV_TEAMS; ++i)
		{
			if(pBlocker->m_isBlockingNav[i])
			{
				...
				{
					if(bIsIntersecting ||
					   (bIsIntersecting = IsBoxIntersectingBox(pBlocker->m_CachedMins, pBlocker->m_CachedMaxs, vecMins,
															   vecMaxs)) != false)
					{
						bBlocked = true;
