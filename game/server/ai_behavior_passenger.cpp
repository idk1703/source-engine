...
BEGIN_DATADESC(CAI_PassengerBehavior)
	...
	DEFINE_EMBEDDED(m_vehicleState), DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
		DEFINE_FIELD(m_PassengerIntent, FIELD_INTEGER), DEFINE_FIELD(m_PassengerState, FIELD_INTEGER),
		DEFINE_FIELD(m_hVehicle, FIELD_EHANDLE), DEFINE_FIELD(m_hBlocker, FIELD_EHANDLE),
		DEFINE_FIELD(m_vecTargetPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecTargetAngles, FIELD_VECTOR),
		DEFINE_FIELD(m_flOriginStartFrame, FIELD_FLOAT), DEFINE_FIELD(m_flOriginEndFrame, FIELD_FLOAT),
		DEFINE_FIELD(m_flAnglesStartFrame, FIELD_FLOAT), DEFINE_FIELD(m_flAnglesEndFrame, FIELD_FLOAT),
...
BEGIN_SIMPLE_DATADESC(passengerVehicleState_t)
	...
	DEFINE_FIELD(m_bWasBoosting, FIELD_BOOLEAN), DEFINE_FIELD(m_bWasOverturned, FIELD_BOOLEAN),
		DEFINE_FIELD(m_vecLastLocalVelocity, FIELD_VECTOR), DEFINE_FIELD(m_vecDeltaVelocity, FIELD_VECTOR),
		DEFINE_FIELD(m_vecLastAngles, FIELD_VECTOR), DEFINE_FIELD(m_flNextWarningTime, FIELD_TIME),
		DEFINE_FIELD(m_flLastSpeedSqr, FIELD_FLOAT), DEFINE_FIELD(m_bPlayerInVehicle, FIELD_BOOLEAN),
...
void CAI_PassengerBehavior::InitVehicleState(void)
{
	...
	m_vehicleState.m_bPlayerInVehicle =
		(pPlayer && pPlayer->IsInAVehicle() && pPlayer->GetServerVehicle() == m_hVehicle->GetServerVehicle());
...
void CAI_PassengerBehavior::BuildScheduleTestBits(void)
{
	...
	if(GetPassengerState() == PASSENGER_STATE_OUTSIDE || GetPassengerState() == PASSENGER_STATE_INSIDE)
	{
		GetOuter()->SetCustomInterruptCondition(
			GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_ENTERING));
		GetOuter()->SetCustomInterruptCondition(
			GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_EXITING));
		GetOuter()->SetCustomInterruptCondition(
			GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_CANCEL_ENTER));
	}
...
int CAI_PassengerBehavior::SelectTransitionSchedule(void)
{
	...
	else if(GetPassengerState() == PASSENGER_STATE_ENTERING || GetPassengerState() == PASSENGER_STATE_EXITING)
	{
		...
		ADD_DEBUG_HISTORY(HISTORY_AI_DECISIONS,
						  UTIL_VarArgs("%s(%d):  SelectSchedule() called on transitioning passenger!\n",
		...
		if(GetPassengerState() == PASSENGER_STATE_EXITING)
		{
			...
			// The teleport function is overridden for passengers, meaning that they will clean up properly when called
			// to do so
...
bool CAI_PassengerBehavior::PointIsNavigable(const Vector &vecTargetPos)
{
	...
	if(iNearestNode != NO_NODE)
	{
		   ...
		   GetOuter()->GetMoveProbe()->CheckStandPosition(moveTrace.vEndPosition, MASK_SOLID_BRUSHONLY))
		{
			...
			// NDebugOverlay::HorzArrow( vecTargetPos, g_pBigAINet->GetNodePosition(GetOuter()->GetHullType(),
			// iNearestNode ), 8.0f, 255, 255, 0, 16, true, 4.0f );
			return true;
		}
	}
...
bool CAI_PassengerBehavior::GetExitPoint(int nSequence, Vector *vecExitPoint, QAngle *vecExitAngles)
{
	...
	if(vecExitPoint != NULL)
	{
		...
		bool bFoundGround =
			FindGroundAtPosition(GetOuter()->GetAbsOrigin() + vecDeltaPos, flUpDelta, flDownDelta, &vecGroundPos);
		if(bFoundGround)
		{
...
int CAI_PassengerBehavior::FindExitSequence(void)
{
	...
	const PassengerSeatAnims_t *pExitAnims =
		m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatAnims(GetOuter(), PASSENGER_SEAT_EXIT);
	if(pExitAnims == NULL)
		return -1;
...
bool CAI_PassengerBehavior::GetEntryPoint(int nSequence, Vector *vecEntryPoint, QAngle *vecEntryAngles)
{
	...
	if(vecEntryPoint != NULL)
	{
		...
		if(FindGroundAtPosition((*vecEntryPoint), 16.0f, 64.0f, &vecGroundPos) == false)
		{
			...
			if(passenger_debug_transition.GetBool())
			{
				NDebugOverlay::SweptBox((*vecEntryPoint), vecGroundPos, GetOuter()->GetHullMins(),
...
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::GetTransitionAnimationIdeal(float flCycle, const Vector &vecTargetPos,
														const QAngle &vecTargetAngles, Vector *idealOrigin,
														QAngle *idealAngles)
{
...
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::LocalIntervalMovement(float flInterval, bool &bMoveSeqFinished, Vector &newPosition,
												  QAngle &newAngles)
{
	...
	{
		flInterval =
			GetOuter()->GetCycle() / (GetOuter()->GetSequenceCycleRate(GetSequence()) * GetOuter()->GetPlaybackRate());
		flNextCycle = 1.0f;
	...
	if(Studio_SeqMovement(pstudiohdr, GetOuter()->GetSequence(), GetOuter()->GetCycle(), flNextCycle,
						  GetOuter()->GetPoseParameterArray(), deltaPos, deltaAngles))
	{
...
float CAI_PassengerBehavior::GetNextCycleForInterval(int nSequence, float flInterval)
{
	return GetOuter()->GetCycle() +
		   flInterval * GetOuter()->GetSequenceCycleRate(GetSequence()) * GetOuter()->GetPlaybackRate();
}

//-----------------------------------------------------------------------------
// Purpose: Draw debug information for the transitional movement
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::DrawDebugTransitionInfo(const Vector &vecIdealPos, const QAngle &vecIdealAngles,
...
void CAI_PassengerBehavior::PrescheduleThink(void)
{
	...
	if(GetPassengerState() == PASSENGER_STATE_INSIDE)
	{
		...
		if(m_hVehicle && m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatPositionLocal(GetOuter(), &vecSeatOrigin,
																						   &vecSeatAngles))
		{
...
void CAI_PassengerBehavior::CacheBlendTargets(void)
{
	...
	int nMaxFrames =
		Studio_MaxFrame(GetOuter()->GetModelPtr(), m_nTransitionSequence, GetOuter()->GetPoseParameterArray());
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::SpeakIfAllowed(AIConcept_t concept, const char *modifiers /*= NULL*/,
...
{
	// Cannot do this while we're transitioning, but it's also a bug because the code that called it was probably
