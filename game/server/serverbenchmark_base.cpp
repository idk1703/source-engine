...
static ConVar sv_benchmark_numticks("sv_benchmark_numticks", "3300", 0,
									"If > 0, then it only runs the benchmark for this # of ticks.");
static ConVar sv_benchmark_autovprofrecord("sv_benchmark_autovprofrecord", "0", 0,
										   "If running a benchmark and this is set, it will record a vprof file over "
										   "the duration of the benchmark with filename benchmark.vprof.");

static float s_flBenchmarkStartWaitSeconds =
	3; // Wait this many seconds after level load before starting the benchmark.
...
public:
	...
	virtual void EndBenchmark(void)
	{
		...
		if(m_nBenchmarkMode == 2)
		{
			...
			// If this file doesn't get written out, then the build script will generate an email that there's a problem
			// somewhere.
			if(fh)
			{
			...
			if(m_PhysicsObjects.Count() < s_nBenchmarkPhysicsObjects)
			{
				...
				if(curPlayers.Count() > 0 && m_PhysicsModelNames.Count() > 0)
				{
					...
					for(int i = 0; i < 15; i++)
					{
						...
						CPhysicsProp *pProp = CreatePhysicsProp(pModelName, vSpawnPos, vSpawnPos + vOffset,
																curPlayers[iPlayer], false, "prop_physics_multiplayer");
						if(pProp)
						{
							m_PhysicsObjects.AddToTail(pProp);
							pProp->SetAbsVelocity(Vector(this->RandomFloat(-500, 500), this->RandomFloat(-500, 500),
														 this->RandomFloat(-500, 500)));
							break;
						}
					}
				}
			...
			for(int i = 0; i < m_PhysicsObjects.Count(); i++)
			{
				...
				if(pEnt)
				{
					...
					if(pPhysicsObject)
					{
						...
						AngularImpulse vAngularImpulse(this->RandomFloat(-flAngImpulse, flAngImpulse),
													   this->RandomFloat(-flAngImpulse, flAngImpulse),
													   this->RandomFloat(flAngImpulse, flAngImpulse));
						pPhysicsObject->ApplyForceCenter(Vector(this->RandomFloat(-flForce, flForce),
																this->RandomFloat(-flForce, flForce),
																this->RandomFloat(0, flForce)));
					}
				}
			}
		}
...
CON_COMMAND(sv_benchmark_force_start, "Force start the benchmark. This is only for debugging. It's better to set "
