...
#define TLK_DENY_COMMAND \
	"TLK_DENY_COMMAND" // designer has asked this NPC to politely deny player commands to move the squad
...
#define TLK_CANT_INTERACT_NOW \
	"TLK_CANT_INTERACT_NOW" // to busy to interact with an object the player is holding up to me
...
public:
	...
	bool SelectSpeechResponse(AIConcept_t concept, const char *pszModifiers, CBaseEntity *pTarget,
	...
	int PlayScriptedSentence(const char *pszSentence, float delay, float volume, soundlevel_t soundlevel,
	...
	virtual bool SpeakIfAllowed(AIConcept_t concept, const char *modifiers = NULL, bool bRespondingToPlayer = false,
...
private:
	...
	EHANDLE m_hPotentialSpeechTarget; // NPC to tell the response rules about when trying to find a response to talk to
									  // them with
	...
	float m_flHealthAccumulator; // Counterpart to the damage accumulator in CBaseCombatCharacter. So ally health
