...
SendPropInt(SENDINFO(m_clrRender), 32, SPROP_UNSIGNED, SendProxy_Color32ToInt),
	...
	SendPropVector(SENDINFO(m_vDirection), 0, SPROP_NORMAL), SendPropInt(SENDINFO(m_bOn), 1, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_nSize), 10, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nOverlaySize), 10, SPROP_UNSIGNED),
...
BEGIN_DATADESC(CSun)
	...
	DEFINE_FIELD(m_vDirection, FIELD_VECTOR),
		...
		DEFINE_KEYFIELD(m_bUseAngles, FIELD_INTEGER, "use_angles"), DEFINE_KEYFIELD(m_flPitch, FIELD_FLOAT, "pitch"),
		DEFINE_KEYFIELD(m_flYaw, FIELD_FLOAT, "angle"), DEFINE_KEYFIELD(m_nSize, FIELD_INTEGER, "size"),
