...
// Input  : *name -
//-----------------------------------------------------------------------------
CTEClientProjectile::CTEClientProjectile(const char *name) : CBaseTempEntity(name)
{
	m_vecOrigin.Init();
	m_vecVelocity.Init();
	m_nModelIndex = 0;
...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD), SendPropVector(SENDINFO(m_vecVelocity), -1, SPROP_COORD),
	SendPropModelIndex(SENDINFO(m_nModelIndex)), SendPropInt(SENDINFO(m_nLifeTime), 6, SPROP_UNSIGNED),
