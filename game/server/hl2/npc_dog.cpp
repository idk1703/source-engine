...
BEGIN_DATADESC(CNPC_Dog)
	DEFINE_EMBEDDED(m_BoneFollowerManager),
		//	m_FollowBehavior
		DEFINE_FIELD(m_flNextSwat, FIELD_TIME), DEFINE_FIELD(m_flTimeToCatch, FIELD_TIME),
		DEFINE_FIELD(m_flTimeToPull, FIELD_TIME), DEFINE_FIELD(m_hPhysicsEnt, FIELD_EHANDLE),
		DEFINE_FIELD(m_hThrowTarget, FIELD_EHANDLE), DEFINE_FIELD(m_iPhysGunAttachment, FIELD_INTEGER),
		DEFINE_FIELD(m_bDoCatchThrowBehavior, FIELD_BOOLEAN), DEFINE_FIELD(m_bDoWaitforObjectBehavior, FIELD_BOOLEAN),
		DEFINE_FIELD(m_sObjectName, FIELD_STRING), DEFINE_FIELD(m_flThrowArcModifier, FIELD_FLOAT),
		DEFINE_FIELD(m_flNextRouteTime, FIELD_TIME), DEFINE_FIELD(m_bHasObject, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iContainerMoveType, FIELD_INTEGER), DEFINE_FIELD(m_bBeamEffects, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bBoneFollowersActive, FIELD_BOOLEAN), DEFINE_UTLVECTOR(m_hUnreachableObjects, FIELD_EHANDLE),
		DEFINE_AUTO_ARRAY(m_hGlowSprites, FIELD_EHANDLE), DEFINE_AUTO_ARRAY(m_hBeams, FIELD_EHANDLE),
		...
		DEFINE_OUTPUT(m_OnThrow, "OnDogThrow"), DEFINE_OUTPUT(m_OnCatch, "OnDogCatch"),
...
void CNPC_Dog::SetPlayerAvoidState(void)
{
	...
	if(m_bPerformAvoidance || (ShouldPlayerAvoid() && bIsMoving))
	{
		...
		if(pLocalPlayer)
		{
			...
			bIntersectingNPCBox = IsBoxIntersectingBox(GetAbsOrigin() + vWorldMins, GetAbsOrigin() + vWorldMaxs,
													   vPlayerMins, vPlayerMaxs);
		...
		if(pPhysObj)
		{
			...
			if(m_hThrowTarget)
			{
				...
				if(m_hThrowTarget->IsPlayer())
					vThrowOrigin =
						vThrowOrigin + Vector(random->RandomFloat(-128, 128), random->RandomFloat(-128, 128), 0);

				Vector vecToss = VecCheckToss(this, vGunPos, vThrowOrigin, m_flThrowArcModifier, 1.0f, true);

				if(vecToss == vec3_origin)
				{
					// Fix up an impossible throw so dog will at least toss the box in the target's general direction
					// instead of dropping it. Also toss it up in the air so it will fall down and break. (Just throw
					// the box up at a 45 degree angle)
					Vector forward, up;
			...
			if(m_bBeamEffects == true)
			{
				...
				te->GaussExplosion(filter, 0.0f, m_hPhysicsEnt->WorldSpaceCenter() - (shotDir * 4.0f),
...
bool CNPC_Dog::FindPhysicsObject(const char *pPickupName, CBaseEntity *pIgnore)
{
	...
	{
		if(pNearest)
			NDebugOverlay::Box(pNearest->WorldSpaceCenter(), pNearest->CollisionProp()->OBBMins(),
							   pNearest->CollisionProp()->OBBMaxs(), 255, 0, 255, true, 3);
	}
	...
	switch(pTask->iTask)
	{
		...
		case TASK_DOG_WAIT_FOR_OBJECT:
		{
			if(m_hPhysicsEnt != NULL)
			{
				...
				if(pPhysObj && !(pPhysObj->GetGameFlags() & FVPHYSICS_PLAYER_HELD) &&
				   flDistanceToPlayer > (flDistance * 2))
				{
				...
				if(m_flTimeToCatch < gpGlobals->curtime && m_bDoWaitforObjectBehavior == false)
				{
					...
					TaskFail("Done waiting!");
						...
						(flDistance <= DOG_CATCH_DISTANCE && !(pPhysObj->GetGameFlags() & FVPHYSICS_PLAYER_HELD)))
				{
...
AI_BEGIN_CUSTOM_NPC(npc_dog, CNPC_Dog)
	...
	DEFINE_SCHEDULE(SCHED_DOG_FIND_OBJECT,
	...
	DEFINE_SCHEDULE(SCHED_DOG_WAIT_THROW_OBJECT, "	Tasks"
												 "		TASK_DOG_SETUP_THROW_TARGET			0"
												 "		TASK_FACE_TARGET					0.5"
												 "		TASK_DOG_WAIT_FOR_TARGET_TO_FACE	0"
												 "		TASK_DOG_LAUNCH_ITEM				0"
	...
	DEFINE_SCHEDULE(SCHED_DOG_CATCH_OBJECT,
