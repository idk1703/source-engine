...
BEGIN_DATADESC(CZombie)
	...
	DEFINE_FIELD(m_hBlockingDoor, FIELD_EHANDLE), DEFINE_FIELD(m_flDoorBashYaw, FIELD_FLOAT),
		DEFINE_EMBEDDED(m_DurationDoorBash), DEFINE_EMBEDDED(m_NextTimeToStartDoorBash),
...
void CZombie::GatherConditions(void)
{
	...
	if(m_hBlockingDoor == NULL ||
	   (m_hBlockingDoor->m_toggle_state == TS_AT_TOP || m_hBlockingDoor->m_toggle_state == TS_GOING_UP))
	{
...
bool CZombie::OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor, float distClear,
								AIMoveResult_t *pResult)
{
...
bool CZombie::IsSquashed(const CTakeDamageInfo &info)
{
	...
	if(info.GetDamageType() & DMG_CRUSH)
	{
		...
		if(pCrusher && pCrusher->GetMass() >= ZOMBIE_SQUASH_MASS &&
		   info.GetInflictor()->WorldSpaceCenter().z > EyePosition().z)
		{
			...
			// The zombie must take physics damage from a 300+kg object that is centered above its eyes (comes from
			// above)
			return true;
		}
	}
...
AI_BEGIN_CUSTOM_NPC(npc_zombie, CZombie)
	...
	DEFINE_SCHEDULE(
		...
		"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_ZOMBIE_TANTRUM" /* placeholder until frustration/rage/fence
