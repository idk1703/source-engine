...
BEGIN_DATADESC(CTripwireGrenade)
	...
	DEFINE_FIELD(m_flPowerUp, FIELD_TIME), DEFINE_FIELD(m_nMissileCount, FIELD_INTEGER),
		DEFINE_FIELD(m_vecDir, FIELD_VECTOR), DEFINE_FIELD(m_vTargetPos, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vTargetOffset, FIELD_VECTOR), DEFINE_FIELD(m_pRope, FIELD_CLASSPTR),
		DEFINE_FIELD(m_pHook, FIELD_CLASSPTR),

		// Function Pointers
		DEFINE_FUNCTION(WarningThink), DEFINE_FUNCTION(PowerupThink), DEFINE_FUNCTION(RopeBreakThink),
...
void CTripwireGrenade::FireMissile(const Vector &vTargetPos)
{
	...
	CGrenadeHomer *pGrenade =
		CGrenadeHomer::CreateGrenadeHomer(MAKE_STRING(GRENADETRIPWIRE_MISSILEMDL),
										  MAKE_STRING("TripwireGrenade.FlySound"), GetAbsOrigin(), vec3_angle, edict());
...
BEGIN_DATADESC(CTripwireHook)
	...
	DEFINE_FIELD(m_hGrenade, FIELD_EHANDLE), DEFINE_FIELD(m_bAttached, FIELD_BOOLEAN),
