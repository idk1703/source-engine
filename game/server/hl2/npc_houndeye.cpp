...
BEGIN_DATADESC(CNPC_Houndeye)
	...
	DEFINE_FIELD(m_fAsleep, FIELD_BOOLEAN), DEFINE_FIELD(m_fDontBlink, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flNextSecondaryAttack, FIELD_TIME), DEFINE_FIELD(m_bLoopClockwise, FIELD_BOOLEAN),
		DEFINE_FIELD(m_pEnergyWave, FIELD_CLASSPTR), DEFINE_FIELD(m_flEndEnergyWaveTime, FIELD_TIME),
...
int CNPC_Houndeye::RangeAttack1Conditions(float flDot, float flDist)
{
	...
	trace_t tr;
	AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 0.1), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this,
...
void CNPC_Houndeye::NPCThink(void)
{
	...
	else
	{
		...
		trace_t tr;
		AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 1), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID,
...
void CNPC_Houndeye::SonicAttack(void)
{
	...
	for(CEntitySphereQuery sphere(GetAbsOrigin(), HOUNDEYE_MAX_ATTACK_RADIUS); pEntity = sphere.GetCurrentEntity();
		sphere.NextEntity())
	{
		if(pEntity->Classify() == CLASS_HOUNDEYE)
		{
		...
		if(pEntity->m_takedamage != DAMAGE_NO || pPhysicsObject)
		{
			...
			{
				CTakeDamageInfo info(this, this, flDamageAdjuster * sk_Houndeye_dmg_blast.GetFloat(),
									 DMG_SONIC | DMG_ALWAYSGIB);
				CalculateExplosiveDamageForce(&info, (pEntity->GetAbsOrigin() - GetAbsOrigin()),
...
void CNPC_Houndeye::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_GET_PATH_TO_ENEMY_LOS:
		{
			...
			if(GetEnemy() != NULL)
			{
				foundLos = GetTacticalServices()->FindLos(GetEnemyLKP(), GetEnemy()->EyePosition(), flMinRange,
														  flMaxRange, 0.0, &posLos);
			}
			else
			{
	...
	switch(m_NPCState)
	{
		...
		case NPC_STATE_COMBAT:
		{
			...
			if(HasCondition(COND_HOUND_GROUP_ATTACK))
			{
				...
				// before attacking
				trace_t tr;
				AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 1), GetHullMins(), GetHullMaxs(),
			...
			if(HasCondition(COND_LIGHT_DAMAGE) | HasCondition(COND_HEAVY_DAMAGE))
			{
				if(random->RandomFloat(0, 1) <= 0.4)
				{
					...
					Vector forward;
					AngleVectors(GetAbsAngles(), &forward);
					AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + forward * -128, GetHullMins(), GetHullMaxs(),
								 MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);
...
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_ATTACK_STRAFE,
...
//  Run a cirle around my enemy
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_ATTACK_STRAFE_REVERSE,
...
// SCHED_HOUND_CHASE_ENEMY
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_CHASE_ENEMY,
...
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_GROUP_ATTACK,
...
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_GROUP_RETREAT,
...
//		Run to rally hound!
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_GROUP_RALLEY,
...
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_COVER_WAIT,
...
// > SCHED_HOUND_RANGE_ATTACK1
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_RANGE_ATTACK1,
...
// > SCHED_HOUND_HOP_RETREAT
//=========================================================
AI_DEFINE_SCHEDULE(SCHED_HOUND_HOP_RETREAT,
