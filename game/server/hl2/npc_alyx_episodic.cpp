...
#define ALYX_MIN_CONSIDER_DIST \
	Square(1200) // Only enemies within this range are counted and considered to generate AI speech
...
BEGIN_DATADESC(CNPC_Alyx)
	...
	DEFINE_FIELD(m_hEmpTool, FIELD_EHANDLE), DEFINE_FIELD(m_hHackTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_hStealthLookTarget, FIELD_EHANDLE), DEFINE_FIELD(m_bInteractionAllowed, FIELD_BOOLEAN),
		DEFINE_FIELD(m_fTimeNextSearchForInteractTargets, FIELD_TIME),
		DEFINE_FIELD(m_bDarknessSpeechAllowed, FIELD_BOOLEAN), DEFINE_FIELD(m_bIsEMPHolstered, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bIsFlashlightBlind, FIELD_BOOLEAN), DEFINE_FIELD(m_fStayBlindUntil, FIELD_TIME),
		DEFINE_FIELD(m_flDontBlindUntil, FIELD_TIME), DEFINE_FIELD(m_bSpokeLostPlayerInDarkness, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bPlayerFlashlightState, FIELD_BOOLEAN), DEFINE_FIELD(m_bHadCondSeeEnemy, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iszCurrentBlindScene, FIELD_STRING),
		DEFINE_FIELD(m_fTimeUntilNextDarknessFoundPlayer, FIELD_TIME), DEFINE_FIELD(m_fCombatStartTime, FIELD_TIME),
		DEFINE_FIELD(m_fCombatEndTime, FIELD_TIME), DEFINE_FIELD(m_flNextCrouchTime, FIELD_TIME),
		DEFINE_FIELD(m_WeaponType, FIELD_INTEGER), DEFINE_KEYFIELD(m_bShouldHaveEMP, FIELD_BOOLEAN, "ShouldHaveEMP"),

		DEFINE_SOUNDPATCH(m_sndDarknessBreathing),

		DEFINE_EMBEDDED(m_SpeechWatch_LostPlayer), DEFINE_EMBEDDED(m_SpeechTimer_HeardSound),
		DEFINE_EMBEDDED(m_SpeechWatch_SoundDelay), DEFINE_EMBEDDED(m_SpeechWatch_BreathingRamp),
...
void CNPC_Alyx::Activate(void)
{
	...
	if(pSpeechManager)
	{
		ConceptInfo_t conceptRequestItem = {CONCEPT_ALYX_REQUEST_ITEM, SPEECH_IMPORTANT, -1, -1, -1, -1, -1, -1,
											AICF_TARGET_PLAYER};
...
void CNPC_Alyx::ReadinessLevelChanged(int iPriorLevel)
{
	...
	if(npc_alyx_readiness_transitions.GetBool())
	{
		...
		for(int i = 0; i < ARRAYSIZE(readinessTransitions); i++)
		{
			...
			Activity translatedTransitionActivity =
				Weapon_TranslateActivity(readinessTransitions[i].transitionActivity);

			if(translatedTransitionActivity == ACT_INVALID ||
			   translatedTransitionActivity == readinessTransitions[i].transitionActivity)
				continue;

			Activity finalActivity = TranslateActivityReadiness(translatedTransitionActivity);

			if(iPriorLevel == readinessTransitions[i].iPreviousLevel &&
			   GetReadinessLevel() == readinessTransitions[i].iCurrentLevel)
			{
...
void CNPC_Alyx::PrescheduleThink(void)
{
	...
	{
		// we used to have a bug where if we tried to remove alyx from the blind scene before it got loaded
		// asynchronously, she would get stuck in the animation with m_bIsFlashlightBlind set to false.  that should be
		// fixed, but just to be sure, we wait a bit to prevent this from happening.
		if(m_fStayBlindUntil < gpGlobals->curtime)
		{
			CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
			if(pPlayer &&
			   (!CanBeBlindedByFlashlight(true) || !pPlayer->IsIlluminatedByFlashlight(this, NULL) ||
	...
	if(HasCondition(COND_HEAR_COMBAT))
	{
		...
		if(IsInAVehicle() == false) // For now, don't do these animations while in the vehicle
		{
			if((pSound->SoundTypeNoContext() & SOUND_COMBAT) && (pSound->SoundContext() & SOUND_CONTEXT_EXPLOSION))
			{
				if(HasShotgun())
				{
					if(!IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_SHOTGUN) &&
					   !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED_SHOTGUN))
					{
						RestartGesture(ACT_GESTURE_FLINCH_BLAST_SHOTGUN);
						GetShotRegulator()->FireNoEarlierThan(
							gpGlobals->curtime + SequenceDuration(ACT_GESTURE_FLINCH_BLAST_SHOTGUN) +
							0.5f); // Allow another second for Alyx to bring her weapon to bear after the flinch.
					}
				}
				else
				{
					if(!IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) &&
					   !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED))
					{
						RestartGesture(ACT_GESTURE_FLINCH_BLAST);
						GetShotRegulator()->FireNoEarlierThan(
							gpGlobals->curtime + SequenceDuration(ACT_GESTURE_FLINCH_BLAST) +
							0.5f); // Allow another second for Alyx to bring her weapon to bear after the flinch.
					}
				}
			}
...
void CNPC_Alyx::AnalyzeGunfireSound(CSound *pSound)
{
	...
	{
		// The shooter is firing at the player. Assume Alyx can deduce the origin if the player COULD see the origin,
		// and Alyx COULD see the player.
...
void CNPC_Alyx::DoMobbedCombatAI(void)
{
		...
		pEMemory = GetEnemies()->GetNext(&iter))
	{
		if(IRelationType(pEMemory->hEnemy) != D_NU && IRelationType(pEMemory->hEnemy) != D_LI &&
		   pEMemory->hEnemy->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= ALYX_MIN_CONSIDER_DIST)
		{
			if(pEMemory->hEnemy && pEMemory->hEnemy->IsAlive() && gpGlobals->curtime - pEMemory->timeLastSeen <= 0.5f &&
			   pEMemory->hEnemy->Classify() != CLASS_BULLSEYE)
			{
	...
	if(closeEnemiesScore > 2)
	{
			...
			pEMemory = GetEnemies()->GetNext(&iter))
		{
			...
			if(IRelationType(pEMemory->hEnemy) != D_NU && IRelationType(pEMemory->hEnemy) != D_LI &&
			   pEMemory->hEnemy->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= ALYX_MIN_CONSIDER_DIST)
			{
				if(pEMemory->hEnemy && pEMemory->hEnemy->IsAlive() &&
				   gpGlobals->curtime - pEMemory->timeLastSeen <= 0.5f &&
				   pEMemory->hEnemy->Classify() != CLASS_BULLSEYE)
				{
					if(pEMemory->hEnemy->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= ALYX_MIN_MOB_DIST_SQR)
					{
...
void CNPC_Alyx::DoCustomSpeechAI(void)
{
	...
	if(HasCondition(COND_NEW_ENEMY) && GetEnemy())
	{
		if(GetEnemy()->Classify() == CLASS_HEADCRAB)
		{
			...
			if((GetEnemy()->GetEnemy() == this) && pHC->IsJumping() &&
			   gpGlobals->curtime - GetEnemies()->FirstTimeSeen(GetEnemy()) < 0.5)
			{
				SpeakIfAllowed("TLK_SPOTTED_INCOMING_HEADCRAB");
			}
			// If we see a headcrab leaving a zombie that just died, mention it
			else if(pHC->GetOwnerEntity() && (pHC->GetOwnerEntity()->Classify() == CLASS_ZOMBIE) &&
					!pHC->GetOwnerEntity()->IsAlive())
			{
	...
	if(HL2GameRules()->IsAlyxInDarknessMode())
	{
		...
		if(!bNearbyFlare)
		{
			...
			if(HasCondition(COND_ALYX_PLAYER_TURNED_OFF_FLASHLIGHT) ||
			   HasCondition(COND_ALYX_PLAYER_FLASHLIGHT_EXPIRED))
			{
				...
				if(!m_sndDarknessBreathing)
				{
					...
					m_sndDarknessBreathing = CSoundEnvelopeController::GetController().SoundCreate(
						filter, entindex(), CHAN_STATIC, "ep_01.al_dark_breathing01", SNDLVL_TALKING);
				...
				if(m_sndDarknessBreathing)
				{
					CSoundEnvelopeController::GetController().SoundChangeVolume(
						m_sndDarknessBreathing, ALYX_BREATHING_VOLUME_MAX, RandomFloat(10, 20));
		...
		{
			if(m_bDarknessSpeechAllowed && HasCondition(COND_ALYX_PLAYER_TURNED_OFF_FLASHLIGHT) && GetEnemy() &&
			   (GetEnemy()->Classify() != CLASS_BULLSEYE))
			{
				SpeakIfAllowed("TLK_DARKNESS_LOSTENEMY_BY_FLASHLIGHT");
			}
			else if(m_bDarknessSpeechAllowed && HasCondition(COND_ALYX_PLAYER_FLASHLIGHT_EXPIRED) && GetEnemy() &&
					(GetEnemy()->Classify() != CLASS_BULLSEYE))
			{
				SpeakIfAllowed("TLK_DARKNESS_LOSTENEMY_BY_FLASHLIGHT_EXPIRED");
			}
			else if(m_bDarknessSpeechAllowed && GetEnemy() && (GetEnemy()->Classify() != CLASS_BULLSEYE) && pPlayer &&
					pPlayer->FlashlightIsOn() && !pPlayer->IsIlluminatedByFlashlight(GetEnemy(), NULL) &&
					FVisible(GetEnemy()))
			{
	...
	if(m_bSpokeLostPlayerInDarkness)
	{
		...
		if(!m_bIsFlashlightBlind && HL2GameRules()->IsAlyxInDarknessMode() && m_bDarknessSpeechAllowed)
		{
			if(HasCondition(COND_SEE_PLAYER) && !HasCondition(COND_TALKER_PLAYER_DEAD))
			{
				if((m_fTimeUntilNextDarknessFoundPlayer == AI_INVALID_TIME) ||
				   (gpGlobals->curtime < m_fTimeUntilNextDarknessFoundPlayer))
				{
	...
	if(m_SpeechWatch_BreathingRamp.IsRunning())
	{
		...
		{
			CSoundEnvelopeController::GetController().SoundChangeVolume(m_sndDarknessBreathing,
																		ALYX_BREATHING_VOLUME_MAX, RandomFloat(5, 10));
			m_SpeechWatch_BreathingRamp.Stop();
		}
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_Alyx::SpeakIfAllowed(AIConcept_t concept, const char *modifiers /*= NULL*/,
							   ...
							   size_t bufsize /* = 0 */)
{
	if(BaseClass::SpeakIfAllowed(concept, modifiers, bRespondingToPlayer, pszOutResponseChosen, bufsize))
	{
		...
		if(m_sndDarknessBreathing &&
		   CSoundEnvelopeController::GetController().SoundGetVolume(m_sndDarknessBreathing) > 0.0)
		{
...
bool CNPC_Alyx::FInViewCone(CBaseEntity *pEntity)
{
	...
	if((pEntity->IsNPC() || pEntity->IsPlayer()) &&
	   pEntity->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= ALYX_360_VIEW_DIST_SQR)
	{
...
int CNPC_Alyx::SelectSchedule(void)
{
	...
	if(HL2GameRules()->IsAlyxInDarknessMode() && m_NPCState == NPC_STATE_ALERT)
	{
		if(HasCondition(COND_HEAR_COMBAT) && !HasCondition(COND_SEE_PLAYER))
		{
			...
			{
				if(pBestSound->m_hOwner->Classify() == CLASS_ZOMBIE &&
				   pBestSound->SoundChannel() == SOUNDENT_CHANNEL_NPC_FOOTSTEP)
	...
	switch(scheduleType)
	{
			...
			if(GetEnemy())
			{
				...
				if(!IsCrouching())
				{
					...
					if(pEnemy->GetHealth() > ALYX_MIN_ENEMY_HEALTH_TO_CROUCH)
					{
						...
						if(EnemyDistance(pEnemy) > (ALYX_MIN_ENEMY_DIST_TO_CROUCH * 1.5) &&
						   (pEnemy->GetFlags() & FL_ONGROUND))
						{
		...
		case SCHED_HIDE_AND_RELOAD:
		{
				...
				pEMemory = GetEnemies()->GetNext(&iter))
			{
				...
				if(pEnemy->CapabilitiesGet() & (bits_CAP_WEAPON_RANGE_ATTACK1 | bits_CAP_WEAPON_RANGE_ATTACK2 |
	...
	if(taken && (info.GetDamageType() & DMG_BLAST))
	{
		if(HasShotgun())
		{
			   ...
			   !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED_SHOTGUN))
			{
				...
				GetShotRegulator()->FireNoEarlierThan(
					gpGlobals->curtime + SequenceDuration(ACT_GESTURE_FLINCH_BLAST_DAMAGED_SHOTGUN) + 0.5f);
			}
		}
		else
		{
			if(!IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST) && !IsPlayingGesture(ACT_GESTURE_FLINCH_BLAST_DAMAGED))
			{
				...
				GetShotRegulator()->FireNoEarlierThan(gpGlobals->curtime +
													  SequenceDuration(ACT_GESTURE_FLINCH_BLAST_DAMAGED) + 0.5f);
			}
		}
	}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_Alyx::PlayerInSpread(const Vector &sourcePos, const Vector &targetPos, float flSpread, float maxDistOffCenter,
...
AI_BEGIN_CUSTOM_NPC(npc_alyx, CNPC_Alyx)
	...
	DEFINE_SCHEDULE(SCHED_ALYX_PREPARE_TO_INTERACT_WITH_TARGET,
	...
	DEFINE_SCHEDULE(SCHED_ALYX_WAIT_TO_INTERACT_WITH_TARGET,
					"	Tasks"
					"		TASK_STOP_MOVING						0"
	...
	DEFINE_SCHEDULE(SCHED_ALYX_INTERACT_WITH_TARGET,
	...
	DEFINE_SCHEDULE(SCHED_ALYX_FINISH_INTERACTING_WITH_TARGET,
	...
	DEFINE_SCHEDULE(SCHED_ALYX_INTERACTION_INTERRUPTED,
					"	Tasks"
					"		TASK_STOP_MOVING						0"
	...
	DEFINE_SCHEDULE(SCHED_ALYX_ALERT_FACE_AWAYFROM_BESTSOUND,
					"	Tasks"
	...
	//	> RangeAttack1
	//===============================================
	DEFINE_SCHEDULE(SCHED_ALYX_RANGE_ATTACK1,
	...
	//===============================================
	DEFINE_SCHEDULE(SCHED_ALYX_ALERT_REACT_TO_COMBAT_SOUND,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_ALYX_COMBAT_FACE,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_ALYX_WAKE_ANGRY,
	...
	//	> NewWeapon
	//===============================================
	DEFINE_SCHEDULE(SCHED_ALYX_NEW_WEAPON,
	...
	//	> Alyx_Idle_Stand
	//===============================================
	DEFINE_SCHEDULE(SCHED_ALYX_IDLE_STAND,
	...
	//===============================================
	DEFINE_SCHEDULE(SCHED_ALYX_FALL_TO_GROUND,
	...
	DEFINE_SCHEDULE(SCHED_ALYX_ALERT_FACE_BESTSOUND,
