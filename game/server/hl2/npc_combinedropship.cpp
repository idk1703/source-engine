...
protected:
	...
	// again.
	static int m_poseBody_Accel, m_poseBody_Sway, m_poseCargo_Body_Accel, m_poseCargo_Body_Sway, m_poseWeapon_Pitch,
		m_poseWeapon_Yaw;
...
BEGIN_DATADESC(CCombineDropshipContainer)
	...
	DEFINE_FIELD(m_nSmokeTrailCount, FIELD_INTEGER), DEFINE_FIELD(m_hLastInflictor, FIELD_EHANDLE),
...
void CCombineDropshipContainer::CreateCorpse()
{
	...
	te->Explosion(filter, 0.0f, &vecAbsPoint, g_sModelIndexFireball, random->RandomInt(4, 10), random->RandomInt(8, 15),
				  TE_EXPLFLAG_NOPARTICLES, 100, 0);

	// Break into chunks
	Vector angVelocity;
	QAngleToAngularImpulse(GetLocalAngularVelocity(), angVelocity);
	PropBreakableCreateAll(GetModelIndex(), VPhysicsGetObject(), GetAbsOrigin(), GetAbsAngles(), GetAbsVelocity(),
						   angVelocity, 1.0, 250, COLLISION_GROUP_NPC, this);
...
void CCombineDropshipContainer::AddSmokeTrail(const Vector &vecPos)
{
	...
	ExplosionCreate(
		vecPos, vec3_angle, this, 1000, 500.0f,
...
BEGIN_DATADESC(CNPC_CombineDropship)
	...
	DEFINE_FIELD(m_flTimeTakeOff, FIELD_TIME), DEFINE_FIELD(m_flNextTroopSpawnAttempt, FIELD_TIME),
		DEFINE_FIELD(m_flDropDelay, FIELD_TIME), DEFINE_FIELD(m_flTimeNextAttack, FIELD_TIME),
		DEFINE_FIELD(m_flLastTime, FIELD_TIME), DEFINE_FIELD(m_iMineCount, FIELD_INTEGER),
		DEFINE_FIELD(m_totalMinesToDrop, FIELD_INTEGER), DEFINE_FIELD(m_soldiersToDrop, FIELD_INTEGER),
		DEFINE_FIELD(m_iDropState, FIELD_INTEGER), DEFINE_FIELD(m_bDropMines, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iLandState, FIELD_INTEGER), DEFINE_FIELD(m_engineThrust, FIELD_FLOAT),
		DEFINE_FIELD(m_bIsFiring, FIELD_BOOLEAN), DEFINE_FIELD(m_iBurstRounds, FIELD_INTEGER),
		DEFINE_FIELD(m_existPitch, FIELD_FLOAT), DEFINE_FIELD(m_existRoll, FIELD_FLOAT),
		DEFINE_FIELD(m_leaveCrate, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_iCrateType, FIELD_INTEGER, "CrateType"),
		DEFINE_FIELD(m_flLandingSpeed, FIELD_FLOAT), DEFINE_KEYFIELD(m_flGunRange, FIELD_FLOAT, "GunRange"),
		DEFINE_FIELD(m_vecAngAcceleration, FIELD_VECTOR), DEFINE_FIELD(m_hContainer, FIELD_EHANDLE),
		DEFINE_FIELD(m_hPickupTarget, FIELD_EHANDLE), DEFINE_FIELD(m_iContainerMoveType, FIELD_INTEGER),
		DEFINE_FIELD(m_bWaitForDropoffInput, FIELD_BOOLEAN), DEFINE_FIELD(m_hLandTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_bHasDroppedOff, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_bInvulnerable, FIELD_BOOLEAN, "Invulnerable"),
		DEFINE_KEYFIELD(m_iszLandTarget, FIELD_STRING, "LandTarget"), DEFINE_SOUNDPATCH(m_pRotorOnGroundSound),
		DEFINE_SOUNDPATCH(m_pDescendingWarningSound), DEFINE_SOUNDPATCH(m_pNearRotorSound),
		...
		DEFINE_FIELD(m_iCurrentTroopExiting, FIELD_INTEGER), DEFINE_FIELD(m_hLastTroopToLeave, FIELD_EHANDLE),

		DEFINE_FIELD(m_iMuzzleAttachment, FIELD_INTEGER), DEFINE_FIELD(m_iMachineGunBaseAttachment, FIELD_INTEGER),
		DEFINE_FIELD(m_iMachineGunRefAttachment, FIELD_INTEGER), DEFINE_FIELD(m_iAttachmentTroopDeploy, FIELD_INTEGER),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "DropAPC", InputDropAPC), DEFINE_INPUTFUNC(FIELD_STRING, "Pickup", InputPickup),
		...
		DEFINE_OUTPUT(m_OnFinishedDropoff, "OnFinishedDropoff"), DEFINE_OUTPUT(m_OnFinishedPickup, "OnFinishedPickup"),
...
void CNPC_CombineDropship::Flight(void)
{
	...
	bool bRunFlight = (GetLandingState() == LANDING_NO || GetLandingState() == LANDING_LEVEL_OUT ||
					   GetLandingState() == LANDING_LIFTOFF || GetLandingState() == LANDING_SWOOPING ||
					   GetLandingState() == LANDING_DESCEND || GetLandingState() == LANDING_HOVER_LEVEL_OUT ||
	...
	if(bRunFlight)
	{
		...
		QAngle goalAngAccel;
		goalAngAccel.x = 2.0 *
						 (AngleDiff(goalPitch, AngleNormalize(GetLocalAngles().x)) - GetLocalAngularVelocity().x * dt) /
						 (dt * dt);
		goalAngAccel.y = 2.0 *
						 (AngleDiff(goalYaw, AngleNormalize(GetLocalAngles().y)) - GetLocalAngularVelocity().y * dt) /
						 (dt * dt);
		goalAngAccel.z = 2.0 *
						 (AngleDiff(goalRoll, AngleNormalize(GetLocalAngles().z)) - GetLocalAngularVelocity().z * dt) /
						 (dt * dt);
...
void CNPC_CombineDropship::DropSoldierContainer()
{
	...
	{
		CTakeDamageInfo dmgInfo(this, this, vec3_origin, m_hContainer->GetAbsOrigin(),
...
void CNPC_CombineDropship::SetLandingState(LandingState_t landingState)
{
	...
	{
		CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
		if((landingState == LANDING_DESCEND) || (landingState == LANDING_TOUCHDOWN) ||
		   (landingState == LANDING_UNLOADING) || (landingState == LANDING_UNLOADED) ||
...
{
	float flVolume =
		RemapValClamped(flAltitude, DROPSHIP_GROUND_WASH_MIN_ALTITUDE, DROPSHIP_GROUND_WASH_MAX_ALTITUDE, 1.0f, 0.0f);
	...
	switch(GetLandingState())
	{
		case LANDING_NO:
		{
			if(IsActivityFinished() &&
			   (GetActivity() != ACT_DROPSHIP_FLY_IDLE_EXAGG && GetActivity() != ACT_DROPSHIP_FLY_IDLE_CARGO))
			{
				if(m_hContainer)
				{
					SetIdealActivity((Activity)ACT_DROPSHIP_FLY_IDLE_CARGO);
...
void CNPC_CombineDropship::SpawnTroop(void)
{
	...
	pSequence->KeyValue("OnEndSequence", UTIL_VarArgs("%s,NPCFinishDustoff,%s,0,-1", STRING(GetEntityName()),
...
// Input  : vecOrigin -
//-----------------------------------------------------------------------------
Vector CNPC_CombineDropship::GetDropoffFinishPosition(Vector vecOrigin, CAI_BaseNPC *pNPC, Vector vecMins,
													  Vector vecMaxs)
{
	...
	trace_t tr;
	AI_TraceHull(vecOrigin + Vector(0, 0, 32), vecOrigin, vecMins, vecMaxs, MASK_SOLID, pNPC, COLLISION_GROUP_NONE,
				 &tr);
	if(tr.fraction < 1.0)
	{
...
{
	CBaseEntity *pEnt =
		gEntList.FindEntityByName(NULL, inputdata.value.StringID(), NULL, inputdata.pActivator, inputdata.pCaller);
	if(!pEnt)
		return;
...
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::UpdateContainerGunFacing(Vector &vecMuzzle, Vector &vecToTarget, Vector &vecAimDir,
													float *flTargetRange)
{
	Assert(m_hContainer);
	...
	if(quadTarget > vecBarrelPos.LengthSqr())
	{
		...
		float flNewAngle = AngleNormalize(
			UTIL_ApproachAngle(angles.x, m_hContainer->GetPoseParameter(m_poseWeapon_Pitch), DROPSHIP_GUN_SPEED));
		m_hContainer->SetPoseParameter(m_poseWeapon_Pitch, flNewAngle);

		flNewAngle = AngleNormalize(
			UTIL_ApproachAngle(angles.y, m_hContainer->GetPoseParameter(m_poseWeapon_Yaw), DROPSHIP_GUN_SPEED));
