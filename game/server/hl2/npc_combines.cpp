...
void CNPC_CombineS::Spawn(void)
{
	...
	{
		Msg("Soldier %s is set to use march anim, but is not an efficient AI. The blended march anim can only be used "
...
void CNPC_CombineS::Event_Killed(const CTakeDamageInfo &info)
{
	...
	if(pPlayer != NULL)
	{
		...
		if(IsElite())
		{
			...
			{
				CBaseEntity *pItem =
					DropItem("item_ammo_ar2_altfire", WorldSpaceCenter() + RandomVector(-4, 4), RandomAngle(0, 360));
