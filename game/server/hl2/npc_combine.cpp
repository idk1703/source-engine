...
int g_fCombineQuestion; // true if an idle grunt asked a question. Cleared when someone answers. YUCK old global from
						// grunt code
...
#define COMBINE_GRENADE_FLUSH_DIST \
	256.0 // Don't try to flush an enemy who has moved farther than this distance from the last place I saw him.
...
enum TacticalVariant_T
{
	...
	TACTICAL_VARIANT_PRESSURE_ENEMY_UNTIL_CLOSE, // Act like VARIANT_PRESSURE_ENEMY, but go to VARIANT_DEFAULT once
												 // within 30 feet
};
...
BEGIN_DATADESC(CNPC_Combine)
	...
	DEFINE_FIELD(m_nKickDamage, FIELD_INTEGER), DEFINE_FIELD(m_vecTossVelocity, FIELD_VECTOR),
		DEFINE_FIELD(m_hForcedGrenadeTarget, FIELD_EHANDLE), DEFINE_FIELD(m_bShouldPatrol, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bFirstEncounter, FIELD_BOOLEAN), DEFINE_FIELD(m_flNextPainSoundTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextAlertSoundTime, FIELD_TIME), DEFINE_FIELD(m_flNextGrenadeCheck, FIELD_TIME),
		DEFINE_FIELD(m_flNextLostSoundTime, FIELD_TIME), DEFINE_FIELD(m_flAlertPatrolTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextAltFireTime, FIELD_TIME), DEFINE_FIELD(m_nShots, FIELD_INTEGER),
		DEFINE_FIELD(m_flShotDelay, FIELD_FLOAT), DEFINE_FIELD(m_flStopMoveShootTime, FIELD_TIME),
		DEFINE_KEYFIELD(m_iNumGrenades, FIELD_INTEGER, "NumGrenades"), DEFINE_EMBEDDED(m_Sentences),
		...
		DEFINE_FIELD(m_iLastAnimEventHandled, FIELD_INTEGER), DEFINE_FIELD(m_fIsElite, FIELD_BOOLEAN),
...
{
	return SOUND_WORLD | SOUND_COMBAT | SOUND_PLAYER | SOUND_DANGER | SOUND_PHYSICS_DANGER | SOUND_BULLET_IMPACT |
		   SOUND_MOVE_AWAY;
}
...
int CNPC_Combine::SelectCombatSchedule()
{
	...
	if(HasCondition(COND_NEW_ENEMY))
	{
		...
		if(m_pSquad && pEnemy)
		{
			...
			else
			{
				...
				{
					JustMadeSound(
						SENTENCE_PRIORITY_MEDIUM); // squelch anything that isn't high priority so the leader can speak
				}
	...
	{
		if(HasCondition(COND_SEE_ENEMY) || HasCondition(COND_SEE_FEAR) || HasCondition(COND_LIGHT_DAMAGE) ||
			...
			if(HasCondition(COND_HEAR_DANGER))
			{
				...
				if(pSound)
				{
					if(pSound->m_iType & SOUND_DANGER)
					{
						...
						if(!GetEnemy() && pSound->IsSoundType(SOUND_CONTEXT_DANGER_APPROACH) && pSound->m_hOwner &&
	...
	switch(m_NPCState)
	{
		...
		case NPC_STATE_ALERT:
		{
			...
			if(HasCondition(COND_HEAR_COMBAT))
			{
				...
				{
					if(m_pSquad && m_pSquad->GetSquadMemberNearestTo(pSound->GetSoundReactOrigin()) == this &&
...
int CNPC_Combine::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	...
	{
		if(IsInSquad() && IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2) &&
		   HasCondition(COND_SEE_ENEMY))
		{
			// This eases the effects of an unfortunate bug that usually plagues shotgunners. Since their rate of fire
			// is low, they spend relatively long periods of time without an attack squad slot. If you corner a
			// shotgunner, usually the other memebers of the squad will hog all of the attack slots and pick schedules
			// to move to establish line of fire. During this time, the shotgunner is prevented from attacking. If he
			// also cannot find cover (the fallback case) he will stand around like an idiot, right in front of you.
			// Instead of this, we have him run up to you for a melee attack.
...
#define COMBINE_MEGA_PHYSCANNON_ATTACK_DISTANCE_SQ \
	(COMBINE_MEGA_PHYSCANNON_ATTACK_DISTANCE * COMBINE_MEGA_PHYSCANNON_ATTACK_DISTANCE)
	...
	switch(scheduleType)
	{
		case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if(m_pSquad)
			{
				...
				if(g_pGameRules->IsSkillLevel(SKILL_HARD) && HasCondition(COND_CAN_RANGE_ATTACK2) &&
				   OccupyStrategySlot(SQUAD_SLOT_GRENADE1))
				{
		...
		case SCHED_HIDE_AND_RELOAD:
		{
			...
			{
				// If I COULD throw a grenade and I need to reload, 20% chance I'll throw a grenade before I hide to
				// reload.
...
// BUGBUG: It looks like this is never played because combine don't do SCHED_WAKE_ANGRY or anything else that does a
// TASK_SOUND_WAKE
...
bool CNPC_Combine::CanThrowGrenade(const Vector &vecTarget)
{
	...
	if(m_pSquad)
	{
		if(m_pSquad->SquadMemberInRange(vecTarget, COMBINE_MIN_GRENADE_CLEAR_DIST))
		{
			...
			CSoundEnt::InsertSound(SOUND_MOVE_AWAY | SOUND_CONTEXT_COMBINE_ONLY, vecTarget,
								   COMBINE_MIN_GRENADE_CLEAR_DIST, 0.1);
			return false;
		}
	}
...
bool CNPC_Combine::CanAltFireEnemy(bool bUseFreeKnowledge)
{
	...
	{
		vecTarget =
			GetEnemies()->LastKnownPosition(pEnemy) + (pEnemy->GetViewOffset() * 0.75); // approximates the chest
	}
	else
	{
...
AI_BEGIN_CUSTOM_NPC(npc_combine, CNPC_Combine)
	...
	DEFINE_SCHEDULE(
		...
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_RANGE_ATTACK2"
		"		TASK_COMBINE_DEFER_SQUAD_GRENADES	0"
		"		TASK_SET_SCHEDULE					SCHEDULE:SCHED_COMBINE_WAIT_IN_COVER" // don't run immediately after
																						  // throwing grenade.
		""
		"	Interrupts")
	...
	DEFINE_SCHEDULE(
		...
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_RANGE_ATTACK2"
		"		TASK_COMBINE_DEFER_SQUAD_GRENADES	0"
		"		TASK_SET_SCHEDULE					SCHEDULE:SCHED_HIDE_AND_RELOAD" // don't run immediately after
																					// throwing grenade.
		""
		"	Interrupts")
