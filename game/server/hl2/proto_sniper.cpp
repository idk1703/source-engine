...
#define SF_SNIPER_FAST \
	(1 << 20) ///< This is faster-shooting sniper. Paint time is decreased 25%. Bullet speed increases 150%.
...
#define SNIPER_PROTECTION_MINDIST \
	(1024.0 * 1024.0) // Distance around protect target that sniper does priority modification in
#define SNIPER_PROTECTION_PRIORITYCAP \
	100.0 // Max addition to priority of an enemy right next to the protect target, falls to 0 at
		  // SNIPER_PROTECTION_MINDIST.
...
BEGIN_DATADESC(CProtoSniper)
	...
	DEFINE_FIELD(m_fWeaponLoaded, FIELD_BOOLEAN), DEFINE_FIELD(m_fEnabled, FIELD_BOOLEAN),
		DEFINE_FIELD(m_fIsPatient, FIELD_BOOLEAN), DEFINE_FIELD(m_flPatience, FIELD_FLOAT),
		DEFINE_FIELD(m_iMisses, FIELD_INTEGER), DEFINE_FIELD(m_hDecoyObject, FIELD_EHANDLE),
		DEFINE_FIELD(m_hSweepTarget, FIELD_EHANDLE), DEFINE_FIELD(m_vecDecoyObjectTarget, FIELD_VECTOR),
		DEFINE_FIELD(m_vecFrustratedTarget, FIELD_VECTOR), DEFINE_FIELD(m_vecPaintStart, FIELD_VECTOR),
		DEFINE_FIELD(m_flPaintTime, FIELD_TIME), DEFINE_FIELD(m_vecPaintCursor, FIELD_VECTOR),
		DEFINE_FIELD(m_flFrustration, FIELD_TIME), DEFINE_FIELD(m_flThinkInterval, FIELD_FLOAT),
		DEFINE_FIELD(m_flDecoyRadius, FIELD_FLOAT), DEFINE_FIELD(m_pBeam, FIELD_CLASSPTR),
		DEFINE_FIELD(m_fSnapShot, FIELD_BOOLEAN), DEFINE_FIELD(m_iNumGroupTargets, FIELD_INTEGER),
		...
		DEFINE_FIELD(m_flTimeLastAttackedPlayer, FIELD_TIME), DEFINE_FIELD(m_hProtectTarget, FIELD_EHANDLE),
...
BEGIN_DATADESC(CSniperBullet)
	...
	DEFINE_FIELD(m_SoundTime, FIELD_TIME), DEFINE_FIELD(m_AmmoType, FIELD_INTEGER),
		DEFINE_FIELD(m_PenetratedAmmoType, FIELD_INTEGER), DEFINE_FIELD(m_fActive, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iImpacts, FIELD_INTEGER), DEFINE_FIELD(m_vecOrigin, FIELD_VECTOR),
		DEFINE_FIELD(m_vecDir, FIELD_VECTOR), DEFINE_FIELD(m_flLastThink, FIELD_TIME),
		DEFINE_FIELD(m_Speed, FIELD_FLOAT), DEFINE_FIELD(m_bDirectShot, FIELD_BOOLEAN),
...
bool CProtoSniper::FInViewCone(CBaseEntity *pEntity)
{
	if(pEntity->GetFlags() & FL_CLIENT)
	{
		...
		if(m_spawnflags & SF_SNIPER_VIEWCONE)
		{
			...
			vecNearestPoint =
				PointOnLineNearestPoint(EyePosition(), EyePosition() + vecLOS * 8192, pPlayer->EyePosition());
...
void CProtoSniper::PaintTarget(const Vector &vecTarget, float flPaintTime)
{
	...
	if(P > 0.25f && GetEnemy() && GetEnemy()->IsNPC() && HasCondition(COND_SEE_ENEMY) && !m_bWarnedTargetEntity)
	{
		...
		{
			CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE, GetEnemy()->EarPosition(), 16, 1.0f,
								   this);
		}
	}
		...
		if(flDistance <= SNIPER_PROTECTION_MINDIST)
		{
			...
			if(m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)
			{
				NDebugOverlay::Text(pTarget->GetAbsOrigin() + Vector(0, 0, 16),
									UTIL_VarArgs("P: %d (b %f)!", priority, flBonus), false, 0.1);
			}
		}
	}
...
void CProtoSniper::Event_Killed(const CTakeDamageInfo &info)
{
	if(!(m_spawnflags & SF_SNIPER_NOCORPSE))
	{
		...
		pGib = CreateRagGib("models/combine_soldier.mdl", GetLocalOrigin(), GetLocalAngles(),
...
#define SEARCH_DEPTH 50
	...
	for(i = 0; i < count; i++)
	{
		...
		if(FClassnameIs(pCurrent, "func_breakable") || FClassnameIs(pCurrent, "prop_physics") ||
	...
	for(i = 0; i < 4; i++)
	{
		...
		UTIL_TraceLine(vecBulletOrigin, vecDecoyTarget + vecDirToDecoy * 32, MASK_BLOCKLOS, this, COLLISION_GROUP_NONE,
					   &tr);
		...
		if(tr.m_pEnt == pProspect || tr.fraction == 1.0)
		{
			...
			Vector vecNoise;
			pProspect->CollisionProp()->RandomPointInBounds(Vector(0.25, 0.25, 0.25), Vector(0.75, 0.75, 0.75),
															&vecNoise);
...
int CProtoSniper::DrawDebugTextOverlays()
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		Q_snprintf(tempstr, sizeof(tempstr), "Sweep group (count): %s (%d)",
				   pTarget != NULL ? STRING(pTarget->m_iszGroupName) : "<None>", m_iNumGroupTargets);
	...
	if(gpGlobals->curtime >= m_SoundTime)
	{
		...
		if(GetOwnerEntity())
		{
			...
			if(pSniper && pSniper->GetEnemy())
			{
				...
				{
					CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_FROM_SNIPER, pSniper->GetEnemy()->EarPosition(),
										   16, 1.0f, GetOwnerEntity());
				}
			}
		}
...
bool CSniperBullet::Start(const Vector &vecOrigin, const Vector &vecTarget, CBaseEntity *pOwner, bool bDirectShot)
{
	...
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + m_vecDir * 8192, MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
				   &tr);
