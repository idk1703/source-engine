...
BEGIN_DATADESC(CPropCrane)
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "Lock", InputLock), DEFINE_INPUTFUNC(FIELD_VOID, "Unlock", InputUnlock),
		...
		DEFINE_FIELD(m_hPlayer, FIELD_EHANDLE), DEFINE_FIELD(m_bMagnetOn, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hNPCDriver, FIELD_EHANDLE), DEFINE_FIELD(m_nNPCButtons, FIELD_INTEGER),
		DEFINE_FIELD(m_bLocked, FIELD_BOOLEAN), DEFINE_FIELD(m_bEnterAnimOn, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bExitAnimOn, FIELD_BOOLEAN), DEFINE_FIELD(m_vecEyeExitEndpoint, FIELD_POSITION_VECTOR),
		DEFINE_OUTPUT(m_playerOn, "PlayerOn"), DEFINE_OUTPUT(m_playerOff, "PlayerOff"),
		DEFINE_FIELD(m_iTurning, FIELD_INTEGER), DEFINE_FIELD(m_bStartSoundAtCrossover, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flTurn, FIELD_FLOAT), DEFINE_FIELD(m_bExtending, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flExtension, FIELD_FLOAT), DEFINE_FIELD(m_flExtensionRate, FIELD_FLOAT),
		...
		DEFINE_FIELD(m_flNextDropAllowedTime, FIELD_TIME), DEFINE_FIELD(m_flSlowRaiseTime, FIELD_TIME),
		DEFINE_FIELD(m_flMaxExtensionSpeed, FIELD_FLOAT), DEFINE_FIELD(m_flMaxTurnSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_flExtensionAccel, FIELD_FLOAT), DEFINE_FIELD(m_flExtensionDecel, FIELD_FLOAT),
		DEFINE_FIELD(m_flTurnAccel, FIELD_FLOAT), DEFINE_FIELD(m_flTurnDecel, FIELD_FLOAT),
		DEFINE_KEYFIELD(m_iszMagnetName, FIELD_STRING, "magnetname"), DEFINE_FIELD(m_hCraneMagnet, FIELD_EHANDLE),
		DEFINE_FIELD(m_hCraneTip, FIELD_EHANDLE), DEFINE_FIELD(m_hRope, FIELD_EHANDLE),
		DEFINE_PHYSPTR(m_pConstraintGroup), DEFINE_KEYFIELD(m_vehicleScript, FIELD_STRING, "vehiclescript"),

END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CPropCrane, DT_PropCrane)
SendPropEHandle(SENDINFO(m_hPlayer)), SendPropBool(SENDINFO(m_bMagnetOn)), SendPropBool(SENDINFO(m_bEnterAnimOn)),
	SendPropBool(SENDINFO(m_bExitAnimOn)), SendPropVector(SENDINFO(m_vecEyeExitEndpoint), -1, SPROP_COORD),
END_SEND_TABLE
();
...
bool CPropCrane::CanExitVehicle(CBaseEntity *pEntity)
{
	...
	return (!m_bLocked && (GetLocalAngularVelocity() == vec3_angle) && m_bExitAnimOn == false &&
...
void CPropCrane::DriveCrane(int iDriverButtons, int iButtonsPressed, float flNPCSteering)
{
	...
	{
		m_flExtensionRate =
			UTIL_Approach(m_flMaxExtensionSpeed, m_flExtensionRate, m_flExtensionAccel * gpGlobals->frametime);
		m_bExtending = true;
	}
...
void CPropCrane::RunCraneMovement(float flTime)
{
	...
	else if((m_flSlowRaiseTime + CRANE_SLOWRAISE_TIME) > gpGlobals->curtime)
	{
		...
		float flCurrentSpringConstant = RemapVal(flDelta, 0, CRANE_SLOWRAISE_TIME,
												 CRANE_SPRING_CONSTANT_INITIAL_RAISING, CRANE_SPRING_CONSTANT_HANGING);
...
void CCraneServerVehicle::NPC_DriveVehicle(void)
{
	if(g_debug_vehicledriver.GetInt())
	{
		if(m_nNPCButtons)
		{
			...
			{
				NDebugOverlay::Line(GetCrane()->GetAbsOrigin(), GetCrane()->GetAbsOrigin() + vecForward * 200, 0, 255,
									0, true, 0.1);
			}
			...
			{
				NDebugOverlay::Line(GetCrane()->GetAbsOrigin(), GetCrane()->GetAbsOrigin() - vecRight * 200, 0, 255, 0,
									true, 0.1);
			}
			...
			{
				NDebugOverlay::Box(GetCrane()->GetAbsOrigin(), -Vector(20, 20, 20), Vector(20, 20, 20), 0, 255, 0, true,
								   0.1);
			}
		}
	}
...
//-----------------------------------------------------------------------------
CCraneTip *CCraneTip::Create(CBaseAnimating *pCraneMagnet, IPhysicsConstraintGroup *pGroup, const Vector &vecOrigin,
