...
BEGIN_DATADESC(CPropJetski)
	...
	DEFINE_AUTO_ARRAY(m_flSpringLengthApproach, FIELD_FLOAT), DEFINE_AUTO_ARRAY(m_flFrictionWheels, FIELD_FLOAT),
		DEFINE_FIELD(m_flHandbrakeTime, FIELD_TIME), DEFINE_FIELD(m_bInitialHandbrake, FIELD_BOOLEAN),
		DEFINE_AUTO_ARRAY(m_springLen, FIELD_FLOAT), DEFINE_PHYSPTR(m_pVehicle),
...
void CPropJetski::OnTurn(CUserCmd *ucmd)
{
	...
	{
		float flLengthRight =
			JETSKI_SHOCK_LENGTH_SHORT + (JETSKI_SHOCK_LENGTH_REST - JETSKI_SHOCK_LENGTH_SHORT) * flInvAbsSteering;
		float flLengthLeft =
			JETSKI_SHOCK_LENGTH_REST + (JETSKI_SHOCK_LENGTH_LONG - JETSKI_SHOCK_LENGTH_REST) * flAbsSteering;
