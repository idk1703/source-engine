...
BEGIN_DATADESC(CNPC_PoisonZombie)
	...
	DEFINE_SOUNDPATCH(m_pFastBreathSound), DEFINE_SOUNDPATCH(m_pSlowBreathSound),
		DEFINE_KEYFIELD(m_nCrabCount, FIELD_INTEGER, "crabcount"), DEFINE_ARRAY(m_bCrabs, FIELD_BOOLEAN, MAX_CRABS),
...
void CNPC_PoisonZombie::Spawn(void)
{
	...
	m_pFastBreathSound =
		ENVELOPE_CONTROLLER.SoundCreate(filter, entindex(), CHAN_ITEM, "NPC_PoisonZombie.FastBreath", ATTN_IDLE);
	ENVELOPE_CONTROLLER.Play(m_pFastBreathSound, 0.0f, 100);

	CPASAttenuationFilter filter2(this);
	m_pSlowBreathSound =
		ENVELOPE_CONTROLLER.SoundCreate(filter2, entindex(), CHAN_ITEM, "NPC_PoisonZombie.Moan1", ATTN_NORM);
...
void CNPC_PoisonZombie::BreatheOffShort(void)
{
	if(m_bNearEnemy)
	{
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pFastBreathSound, SOUNDCTRL_CHANGE_VOLUME,
											  envPoisonZombieBreatheVolumeOffShort,
											  ARRAYSIZE(envPoisonZombieBreatheVolumeOffShort));
	}
	else
	{
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pSlowBreathSound, SOUNDCTRL_CHANGE_VOLUME,
											  envPoisonZombieBreatheVolumeOffShort,
											  ARRAYSIZE(envPoisonZombieBreatheVolumeOffShort));
	}
}
...
void CNPC_PoisonZombie::HandleAnimEvent(animevent_t *pEvent)
{
	...
	if(pEvent->event == AE_ZOMBIE_POISON_THROW_CRAB)
	{
		...
		CBlackHeadcrab *pCrab =
			(CBlackHeadcrab *)CreateNoSpawn(GetHeadcrabClassname(), EyePosition(), vec3_angle, this);
...
void CNPC_PoisonZombie::EvacuateNest(bool bExplosion, float flDamage, CBaseEntity *pAttacker)
{
	...
	for(int i = 0; i < MAX_CRABS; i++)
	{
		if(m_bCrabs[i])
		{
			...
			CBlackHeadcrab *pCrab =
				(CBlackHeadcrab *)CreateNoSpawn(GetHeadcrabClassname(), vecPosition, vecAngles, this);
			pCrab->Spawn();
...
void CNPC_PoisonZombie::PrescheduleThink(void)
{
	if(HasCondition(COND_NEW_ENEMY))
	{
		m_flNextCrabThrowTime =
			gpGlobals->curtime + random->RandomInt(ZOMBIE_THROW_FIRST_MIN_DELAY, ZOMBIE_THROW_FIRST_MAX_DELAY);
	}
	...
	if(bNearEnemy)
	{
		if(!m_bNearEnemy)
		{
			...
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pFastBreathSound, random->RandomInt(100, 120),
...
AI_BEGIN_CUSTOM_NPC(npc_poisonzombie, CNPC_PoisonZombie)
	...
	DEFINE_SCHEDULE(SCHED_ZOMBIE_POISON_RANGE_ATTACK1,
