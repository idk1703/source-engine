...
BEGIN_DATADESC(CFuncTank)
	DEFINE_KEYFIELD(m_yawRate, FIELD_FLOAT, "yawrate"), DEFINE_KEYFIELD(m_yawRange, FIELD_FLOAT, "yawrange"),
		...
		DEFINE_KEYFIELD(m_fireRate, FIELD_FLOAT, "firerate"), DEFINE_FIELD(m_fireTime, FIELD_TIME),
		DEFINE_KEYFIELD(m_persist, FIELD_FLOAT, "persistence"),
		DEFINE_KEYFIELD(m_persist2, FIELD_FLOAT, "persistence2"), DEFINE_KEYFIELD(m_minRange, FIELD_FLOAT, "minRange"),
		DEFINE_KEYFIELD(m_maxRange, FIELD_FLOAT, "maxRange"), DEFINE_FIELD(m_flMinRange2, FIELD_FLOAT),
		DEFINE_FIELD(m_flMaxRange2, FIELD_FLOAT), DEFINE_KEYFIELD(m_iAmmoCount, FIELD_INTEGER, "ammo_count"),
		...
		DEFINE_KEYFIELD(m_bulletType, FIELD_INTEGER, "bullet"), DEFINE_FIELD(m_nBulletCount, FIELD_INTEGER),
...
#ifdef HL2_EPISODIC
		DEFINE_KEYFIELD(m_iszAmmoType, FIELD_STRING, "ammotype"), DEFINE_FIELD(m_iAmmoType, FIELD_INTEGER),
#else
		DEFINE_FIELD(m_iSmallAmmoType, FIELD_INTEGER), DEFINE_FIELD(m_iMediumAmmoType, FIELD_INTEGER),
		...
		DEFINE_FIELD(m_yawCenter, FIELD_FLOAT), DEFINE_FIELD(m_yawCenterWorld, FIELD_FLOAT),
		DEFINE_FIELD(m_pitchCenter, FIELD_FLOAT), DEFINE_FIELD(m_pitchCenterWorld, FIELD_FLOAT),
		DEFINE_FIELD(m_fireLast, FIELD_TIME), DEFINE_FIELD(m_lastSightTime, FIELD_TIME),
		DEFINE_FIELD(m_barrelPos, FIELD_VECTOR), DEFINE_FIELD(m_sightOrigin, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_hFuncTankTarget, FIELD_EHANDLE), DEFINE_FIELD(m_hController, FIELD_EHANDLE),
		DEFINE_FIELD(m_vecControllerUsePos, FIELD_VECTOR), DEFINE_FIELD(m_flNextAttack, FIELD_TIME),
		DEFINE_FIELD(m_targetEntityName, FIELD_STRING), DEFINE_FIELD(m_hTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_vTargetPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecNPCIdleTarget, FIELD_POSITION_VECTOR),
		...
		DEFINE_FIELD(m_flNextControllerSearch, FIELD_TIME), DEFINE_FIELD(m_bShouldFindNPCs, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bNPCInRoute, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_iszNPCManPoint, FIELD_STRING, "npc_man_point"),
		DEFINE_FIELD(m_bReadyToFire, FIELD_BOOLEAN),

		DEFINE_KEYFIELD(m_bPerformLeading, FIELD_BOOLEAN, "LeadTarget"), DEFINE_FIELD(m_flStartLeadFactor, FIELD_FLOAT),
		DEFINE_FIELD(m_flStartLeadFactorTime, FIELD_TIME), DEFINE_FIELD(m_flNextLeadFactor, FIELD_FLOAT),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnFire, "OnFire"), DEFINE_OUTPUT(m_OnLoseTarget, "OnLoseTarget"),
		DEFINE_OUTPUT(m_OnAquireTarget, "OnAquireTarget"), DEFINE_OUTPUT(m_OnAmmoDepleted, "OnAmmoDepleted"),
		DEFINE_OUTPUT(m_OnGotController, "OnGotController"), DEFINE_OUTPUT(m_OnLostController, "OnLostController"),
...
int CFuncTank::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		{
			Q_snprintf(tempstr, sizeof(tempstr), "Aim Pos: %3.0f %3.0f %3.0f", m_vTargetPosition.x, m_vTargetPosition.y,
					   m_vTargetPosition.z);
		}
		else
		{
...
//			bitsDamageType -
//-----------------------------------------------------------------------------
void CFuncTank::TraceAttack(CBaseEntity *pAttacker, float flDamage, const Vector &vecDir, trace_t *ptr,
							int bitsDamageType)
{
...
void CFuncTank::ControllerPostFrame(void)
{
	...
	if(HasSpawnFlags(SF_TANK_AIM_ASSISTANCE))
	{
		...
		UTIL_TraceHull(start, start + forward * 8192, -Vector(8, 8, 8), Vector(8, 8, 8), MASK_SHOT, this,
...
void CFuncTank::NPC_Fire(void)
{
	...
	//		m_fireLast = gpGlobals->curtime - (1/m_fireRate) - 0.01;  // to make sure the gun doesn't fire too many
	//bullets
	...
	if(IsControllable() && !HasController() && (m_flNextControllerSearch <= gpGlobals->curtime))
	{
		...
		if(pPlayer)
		{
			if(!m_hControlVolume)
			{
				...
				{
					m_hControlVolume =
						dynamic_cast<CBaseTrigger *>(gEntList.FindEntityByName(NULL, m_iszControlVolume));
				}
			...
			if(m_hControlVolume)
			{
				...
				{
					// If my control volume is touching a player that's facing the mounted gun, automatically use the
					// gun.
					// !!!BUGBUG - this only works in cases where the player can see the gun whilst standing in the
					// control volume. (This works just fine for all func_tanks mounted on combine walls and small
					// barriers)
...
void CFuncTank::CalcPlayerCrosshairTarget(Vector *pVecTarget)
{
	...
	UTIL_TraceLine(vecStart + vecDir * 24, vecStart + vecDir * 8192, MASK_BLOCKLOS_AND_NPCS, this, COLLISION_GROUP_NONE,
				   &tr);
...
bool CFuncTank::RotateTankToAngles(const QAngle &angles, float *pDistX, float *pDistY)
{
	...
	if((fabs(offsetY) > m_yawRange + m_yawTolerance) || (fabs(offsetX) > m_pitchRange + m_pitchTolerance))
	{
...
void CFuncTank::ComputeLeadingPosition(const Vector &vecShootPosition, CBaseEntity *pTarget, Vector *pLeadPosition)
{
	...
	float flFactor =
		(gpGlobals->curtime - m_flStartLeadFactorTime) / (m_flNextLeadFactorTime - m_flStartLeadFactorTime);
...
void CFuncTank::AimFuncTankAtTarget(void)
{
	...
	if(CanFire() &&
	   (((fabs(distX) <= m_pitchTolerance) && (fabs(distY) <= m_yawTolerance)) || (m_spawnflags & SF_TANK_LINEOFSIGHT)))
	{
		bool fire = false;
		Vector forward;
					 ...
					 bool bIgnoreSpread)
{
	...
	else
	{
		if(m_iszSpriteSmoke != NULL_STRING)
		{
			...
			pSprite->SetTransparency(kRenderTransAlpha, m_clrRender->r, m_clrRender->g, m_clrRender->b, 255,
									 kRenderFxNone);
	...
	if(pAttacker && pAttacker->IsPlayer())
	{
		...
		{
			CSoundEnt::InsertSound(SOUND_MOVE_AWAY, barrelEnd + forward * 32.0f, 32.0f, 0.2f, pAttacker,
...
bool CFuncTank::IsEntityInViewCone(CBaseEntity *pEntity)
{
	...
	if(m_flPlayerGracePeriod && pEntity->IsPlayer() &&
	   (gpGlobals->curtime - m_flLastSawNonPlayer) < m_flPlayerGracePeriod)
	{
...
// Output  :
//------------------------------------------------------------------------------
void CFuncTankPulseLaser::Fire(int bulletCount, const Vector &barrelEnd, const Vector &vecForward,
							   CBaseEntity *pAttacker, bool bIgnoreSpread)
{
	// --------------------------------------------------
...
BEGIN_DATADESC(CFuncTankAirboatGun)
	...
	DEFINE_SOUNDPATCH(m_pGunFiringSound), DEFINE_FIELD(m_flNextHeavyShotTime, FIELD_TIME),
...
BEGIN_DATADESC(CFuncTankAPCRocket)
	...
	DEFINE_KEYFIELD(m_flRocketSpeed, FIELD_FLOAT, "rocketspeed"), DEFINE_FIELD(m_hLaserDot, FIELD_EHANDLE),
		DEFINE_FIELD(m_nSide, FIELD_INTEGER), DEFINE_KEYFIELD(m_nBurstCount, FIELD_INTEGER, "burstcount"),
...
public:
	...
	static CMortarShell *Create(const Vector &vecStart, const Vector &vecTarget, const Vector &vecShotDir,
								float flImpactDelay, float flWarnDelay, string_t warnSound);
...
BEGIN_DATADESC(CMortarShell)
	DEFINE_FIELD(m_flImpactTime, FIELD_TIME), DEFINE_FIELD(m_flFadeTime, FIELD_TIME),
		DEFINE_FIELD(m_flWarnTime, FIELD_TIME), DEFINE_FIELD(m_flNPCWarnTime, FIELD_TIME),
		DEFINE_FIELD(m_warnSound, FIELD_STRING), DEFINE_FIELD(m_iSpriteTexture, FIELD_INTEGER),
		DEFINE_FIELD(m_bHasWarned, FIELD_BOOLEAN), DEFINE_FIELD(m_flLifespan, FIELD_FLOAT),
		DEFINE_FIELD(m_vecFiredFrom, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecFlyDir, FIELD_VECTOR),
		DEFINE_FIELD(m_flSpawnedTime, FIELD_TIME), DEFINE_AUTO_ARRAY(m_pBeamEffect, FIELD_EHANDLE),
		DEFINE_FIELD(m_flRadius, FIELD_FLOAT), DEFINE_FIELD(m_vecSurfaceNormal, FIELD_VECTOR),

		DEFINE_FUNCTION(FlyThink), DEFINE_FUNCTION(FadeThink),
END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CMortarShell, DT_MortarShell)
SendPropFloat(SENDINFO(m_flLifespan), -1, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_flRadius), -1, SPROP_NOSCALE),
...
#define MORTAR_TEST_RADIUS 16.0f
	...
	//			*endNormal -
	//-----------------------------------------------------------------------------
	void CMortarShell::FixUpImpactPoint(const Vector &initialPos, const Vector &initialNormal, Vector *endPos,
										Vector *endNormal)
{
...
CMortarShell *CMortarShell::Create(const Vector &vecStart, const Vector &vecTarget, const Vector &vecShotDir,
								   float flImpactDelay, float flWarnDelay, string_t warnSound)
{
	...
	trace_t tr;
	UTIL_TraceLine(vecTarget, vecTarget + (vecShotDir * 128.0f), MASK_SOLID_BRUSHONLY, pShell, COLLISION_GROUP_NONE,
				   &tr);
	...
	pShell->m_pBeamEffect[0]->PointsInit(vecStart,
										 vecStart + Vector(vecStartSkew[0], vecStartSkew[1], MORTAR_BLAST_HEIGHT));
	...
	pShell->m_pBeamEffect[1]->PointsInit(vecStart,
										 vecStart + Vector(vecStartSkew[0], vecStartSkew[1], MORTAR_BLAST_HEIGHT));
	...
	trace_t skyTrace;
	UTIL_TraceLine(targetPos, targetPos + Vector(vecEndSkew[0], vecEndSkew[1], MORTAR_BLAST_HEIGHT),
				   MASK_SOLID_BRUSHONLY, pShell, COLLISION_GROUP_NONE, &skyTrace);
	...
	if(skyTrace.fraction <= 1.0f && skyTrace.surface.flags & SURF_SKY)
	{
		...
		pShell->m_pBeamEffect[2]->PointsInit(targetPos,
											 targetPos + Vector(vecEndSkew[0], vecEndSkew[1], MORTAR_BLAST_HEIGHT));
		...
		pShell->m_pBeamEffect[3]->PointsInit(targetPos,
											 targetPos + Vector(vecEndSkew[0], vecEndSkew[1], MORTAR_BLAST_HEIGHT));
...
void UTIL_VisualizeCurve(int type, int steps, float bias)
{
	...
	for(int i = 0; i < steps; i++)
	{
		...
		renderOffs = (vRight * (-steps * 0.5f) * renderScale) + (vUp * (renderScale * -(steps * 0.5f))) +
					 (vRight * i * renderScale) + (vUp * perc * (renderScale * steps));
...
void CMortarShell::FlyThink()
{
	...
	{
		// Warn the AI. Make this radius a little larger than the explosion will be, and make the sound last a little
		// longer.
		CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_MORTAR, GetAbsOrigin(), MORTAR_BLAST_RADIUS * 1.25,
							   (m_flImpactTime - m_flNPCWarnTime) + 0.15);
...
void CMortarShell::Impact(void)
{
	...
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 128), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
				   &tr);
	...
	RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), MORTAR_BLAST_DAMAGE, (DMG_BLAST | DMG_DISSOLVE)),
				 GetAbsOrigin(), MORTAR_BLAST_RADIUS, CLASS_NONE, NULL);
...
BEGIN_DATADESC(CFuncTankMortar)
	...
	DEFINE_KEYFIELD(m_Magnitude, FIELD_INTEGER, "iMagnitude"), DEFINE_KEYFIELD(m_fireDelay, FIELD_FLOAT, "firedelay"),
...
// Purpose:
//-----------------------------------------------------------------------------
void CFuncTankPhysCannister::Fire(int bulletCount, const Vector &barrelEnd, const Vector &forward,
								  CBaseEntity *pAttacker, bool bIgnoreSpread)
{
...
private:
	...
	bool m_bLastTargetWasNPC; // Tells whether the last entity we fired a shot at was an NPC (otherwise it was the
							  // player)
};

BEGIN_DATADESC(CFuncTankCombineCannon)
	DEFINE_FIELD(m_originalFireRate, FIELD_FLOAT), DEFINE_THINKFUNC(UpdateBeamThink),
		DEFINE_FIELD(m_flTimeNextSweep, FIELD_TIME), DEFINE_FIELD(m_flTimeBeamOn, FIELD_TIME),
		DEFINE_FIELD(m_hBeam, FIELD_EHANDLE), DEFINE_FIELD(m_vecTrueForward, FIELD_VECTOR),
		DEFINE_FIELD(m_bShouldHarrass, FIELD_BOOLEAN), DEFINE_FIELD(m_bLastTargetWasNPC, FIELD_BOOLEAN),
...
void CFuncTankCombineCannon::UpdateBeamThink()
{
	...
	AI_TraceLine(vecBarrel, vecBarrel + vecAim * COMBINE_CANNON_BEAM_MAX_DIST, MASK_SHOT, this, COLLISION_GROUP_NONE,
				 &trBeam);
...
// adjusted angle.
//---------------------------------------------------------
void CFuncTankCombineCannon::Fire(int bulletCount, const Vector &barrelEnd, const Vector &forward,
								  CBaseEntity *pAttacker, bool bIgnoreSpread)
{
...
void CFuncTankCombineCannon::MakeTracer(const Vector &vecTracerSrc, const trace_t &tr, int iTracerType)
{
	...
	if(AI_IsSinglePlayer())
	{
		...
		{
			// Don't shake the screen if we're hit (within 10 inches), but do shake if a shot otherwise comes within 10
			// feet.
