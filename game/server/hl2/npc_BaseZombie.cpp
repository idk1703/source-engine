...
BEGIN_DATADESC(CNPC_BaseZombie)
	...
	DEFINE_SOUNDPATCH(m_pMoanSound), DEFINE_FIELD(m_fIsTorso, FIELD_BOOLEAN),
		DEFINE_FIELD(m_fIsHeadless, FIELD_BOOLEAN), DEFINE_FIELD(m_flNextFlinch, FIELD_TIME),
		DEFINE_FIELD(m_bHeadShot, FIELD_BOOLEAN), DEFINE_FIELD(m_flBurnDamage, FIELD_FLOAT),
		DEFINE_FIELD(m_flBurnDamageResetTime, FIELD_TIME), DEFINE_FIELD(m_hPhysicsEnt, FIELD_EHANDLE),
		DEFINE_FIELD(m_flNextMoanSound, FIELD_TIME), DEFINE_FIELD(m_flNextSwat, FIELD_TIME),
		DEFINE_FIELD(m_flNextSwatScan, FIELD_TIME), DEFINE_FIELD(m_crabHealth, FIELD_FLOAT),
		DEFINE_FIELD(m_flMoanPitch, FIELD_FLOAT), DEFINE_FIELD(m_iMoanSound, FIELD_INTEGER),
		DEFINE_FIELD(m_hObstructor, FIELD_EHANDLE), DEFINE_FIELD(m_bIsSlumped, FIELD_BOOLEAN),
...
bool CNPC_BaseZombie::FindNearestPhysicsObject(int iMaxMass)
{
	...
	public:
		...
		virtual IterationRetval_t EnumElement(IHandleEntity *pHandleEntity)
		{
			...
			if(pEntity && pEntity->VPhysicsGetObject() && pEntity->VPhysicsGetObject()->GetMass() <= m_iMaxMass &&
			   pEntity->VPhysicsGetObject()->IsAsleep() && pEntity->VPhysicsGetObject()->IsMoveable())
			{
	...
	for(i = 0; i < count; i++)
	{
		...
		physcollision->CollideGetAABB(&objMins, &objMaxs, pCollide->solids[0], pList[i]->GetAbsOrigin(),
...
int CNPC_BaseZombie::MeleeAttack1Conditions(float flDot, float flDist)
{
	...
	AI_TraceHull(WorldSpaceCenter(), WorldSpaceCenter() + forward * GetClawAttackRange(), vecMins, vecMaxs,
	...
	if(GetEnemy() && GetEnemy()->MyCombatCharacterPointer() &&
	   tr.m_pEnt == static_cast<CBaseCombatCharacter *>(GetEnemy())->GetVehicleEntity())
	{
								  ...
								  CDmgAccumulator *pAccumulator)
{
	...
	if(infoCopy.GetDamageType() & DMG_BUCKSHOT)
	{
		...
		// make the shotgun much more powerful, and returns the zombies to a level that has been playtested
		// extensively.(sjb) This normalizes the buckshot damage to what it used to be on normal (5 dmg per pellet. Now
		// it's 8 dmg per pellet).
...
int CNPC_BaseZombie::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
	...
	{
		//!!!HACKHACK- Stuff a light_damage condition if an actbusying zombie takes direct burn damage. This will cause
		//!an
...
void CNPC_BaseZombie::MoanSound(envelopePoint_t *pEnvelope, int iEnvelopeSize)
{
	...
	float duration =
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, pEnvelope, iEnvelopeSize);

	float flPitch =
		random->RandomInt(m_flMoanPitch + zombie_changemin.GetInt(), m_flMoanPitch + zombie_changemax.GetInt());
...
void CNPC_BaseZombie::DieChopped(const CTakeDamageInfo &info)
{
	...
	CBaseEntity *pLegGib =
		CreateRagGib(GetLegsModel(), GetAbsOrigin(), GetAbsAngles(), vecLegsForce, flFadeTime, ShouldIgniteZombieGib());
	if(pLegGib)
	{
	...
	TorsoAngles.x -= 90.0f;
	CBaseEntity *pTorsoGib = CreateRagGib(GetTorsoModel(), GetAbsOrigin() + Vector(0, 0, 64), TorsoAngles, forceVector,
...
void CNPC_BaseZombie::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner)
{
	...
	SetHealth(
		MIN(m_iHealth, FLAME_DIRECT_DAMAGE_PER_SEC *
						   (ZOMBIE_BURN_TIME + random->RandomFloat(-ZOMBIE_BURN_TIME_NOISE, ZOMBIE_BURN_TIME_NOISE))));
										 ...
										 int BloodOrigin)
{
	...
	if(GetEnemy())
	{
		...
		// CheckTraceHullAttack() can damage player in vehicle as side effect of melee attack damaging physics objects,
		// which the car forwards to the player need to detect this to get correct damage effects
...
void CNPC_BaseZombie::HandleAnimEvent(animevent_t *pEvent)
{
	...
	if(pEvent->event == AE_ZOMBIE_SWATITEM)
	{
		...
		if(pEnemy)
		{
			...
			PhysicsImpactSound(pEnemy, pPhysObj, CHAN_BODY, pPhysObj->GetMaterialIndex(),
	...
	if(pEvent->event == AE_ZOMBIE_POPHEADCRAB)
	{
		...
		if(boneIndex == -1)
		{
			Warning("AE_ZOMBIE_POPHEADCRAB event using invalid bone name! Usage: event AE_ZOMBIE_POPHEADCRAB "
					"\"<BoneName> <Speed>\" \n");
			return;
		}
		...
		if(!token)
		{
			Warning("AE_ZOMBIE_POPHEADCRAB event format missing velocity parameter! Usage: event AE_ZOMBIE_POPHEADCRAB "
					"\"<BoneName> <Speed>\" \n");
			return;
		}
...
int CNPC_BaseZombie::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	...
	{
		if(!m_fIsTorso && IsPathTaskFailure(taskFailCode) && m_hObstructor != NULL &&
		   m_hObstructor->VPhysicsGetObject() && m_hObstructor->VPhysicsGetObject()->GetMass() < 100)
		{
...
void CNPC_BaseZombie::BecomeTorso(const Vector &vecTorsoForce, const Vector &vecLegsForce)
{
	...
	if(m_fIsTorso == true)
	{
		...
		CBaseEntity *pGib =
			CreateRagGib(GetLegsModel(), GetAbsOrigin() - Vector(0, 0, 40), GetAbsAngles(), vecLegsForce, flFadeTime);
...
bool CNPC_BaseZombie::HeadcrabFits(CBaseAnimating *pCrab)
{
	...
	trace_t tr;
	AI_TraceHull(vecSpawnLoc, vecSpawnLoc - Vector(0, 0, 1), NAI_Hull::Mins(HULL_TINY) * CRAB_HULL_EXPAND,
				 NAI_Hull::Maxs(HULL_TINY) * CRAB_HULL_EXPAND, MASK_NPCSOLID, &traceFilter, &tr);

	if(tr.fraction != 1.0)
	{
		// NDebugOverlay::Box( vecSpawnLoc, NAI_Hull::Mins(HULL_TINY) * CRAB_HULL_EXPAND, NAI_Hull::Maxs(HULL_TINY) *
		// CRAB_HULL_EXPAND, 255, 0, 0, 100, 10.0 );
		return false;
	}

	// NDebugOverlay::Box( vecSpawnLoc, NAI_Hull::Mins(HULL_TINY) * CRAB_HULL_EXPAND, NAI_Hull::Maxs(HULL_TINY) *
	// CRAB_HULL_EXPAND, 0, 255, 0, 100, 10.0 );
	return true;
}
...
//			fRagdollBody -
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::ReleaseHeadcrab(const Vector &vecOrigin, const Vector &vecVelocity, bool fRemoveHead,
									  bool fRagdollBody, bool fRagdollCrab)
{
	CAI_BaseNPC *pCrab;
	...
	if(fRagdollCrab)
	{
		...
		CBaseEntity *pGib =
			CreateRagGib(GetHeadcrabModel(), vecOrigin, GetLocalAngles(), vecVelocity, 15, ShouldIgniteZombieGib());
