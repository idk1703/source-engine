...
BEGIN_SIMPLE_DATADESC(CRollerController)
	...
	DEFINE_FIELD(m_vecAngular, FIELD_VECTOR), DEFINE_FIELD(m_vecLinear, FIELD_VECTOR),
		DEFINE_FIELD(m_fIsStopped, FIELD_BOOLEAN),

END_DATADESC()

//-----------------------------------------------------------------------------
IMotionEvent::simresult_e CRollerController::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject,
...
#define ROLLERMINE_SEE_VEHICLESONLY_BEYOND_IDLE \
	300 // See every other than vehicles upto this distance (i.e. old idle see dist)
#define ROLLERMINE_SEE_VEHICLESONLY_BEYOND_NORMAL \
	800 // See every other than vehicles upto this distance (i.e. old normal see dist)
...
BEGIN_DATADESC(CNPC_RollerMine)
	...
	DEFINE_SOUNDPATCH(m_pRollSound), DEFINE_SOUNDPATCH(m_pPingSound), DEFINE_EMBEDDED(m_RollerController),
		DEFINE_PHYSPTR(m_pMotionController),

		DEFINE_FIELD(m_flSeeVehiclesOnlyBeyond, FIELD_FLOAT), DEFINE_FIELD(m_flActiveTime, FIELD_TIME),
		DEFINE_FIELD(m_flChargeTime, FIELD_TIME), DEFINE_FIELD(m_flGoIdleTime, FIELD_TIME),
		DEFINE_FIELD(m_flShockTime, FIELD_TIME), DEFINE_FIELD(m_flForwardSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_bIsOpen, FIELD_BOOLEAN), DEFINE_FIELD(m_bHeld, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hVehicleStuckTo, FIELD_EHANDLE), DEFINE_FIELD(m_flPreventUnstickUntil, FIELD_TIME),
		DEFINE_FIELD(m_flNextHop, FIELD_FLOAT), DEFINE_FIELD(m_bIsPrimed, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iSoundEventFlags, FIELD_INTEGER), DEFINE_FIELD(m_rollingSoundState, FIELD_INTEGER),

		DEFINE_KEYFIELD(m_bStartBuried, FIELD_BOOLEAN, "StartBuried"), DEFINE_FIELD(m_bBuried, FIELD_BOOLEAN),
		DEFINE_FIELD(m_wakeUp, FIELD_BOOLEAN), DEFINE_FIELD(m_bEmbedOnGroundImpact, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bHackedByAlyx, FIELD_BOOLEAN),

		DEFINE_FIELD(m_bPowerDown, FIELD_BOOLEAN), DEFINE_FIELD(m_flPowerDownTime, FIELD_TIME),
		...
		// Function Pointers
		DEFINE_ENTITYFUNC(SpikeTouch), DEFINE_ENTITYFUNC(ShockTouch), DEFINE_ENTITYFUNC(CloseTouch),
		DEFINE_ENTITYFUNC(EmbedTouch), DEFINE_THINKFUNC(Explode), DEFINE_THINKFUNC(PreDetonate),
...
SendPropInt(SENDINFO(m_bIsOpen), 1, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_flActiveTime), 0, SPROP_NOSCALE),
	SendPropInt(SENDINFO(m_bHackedByAlyx), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_bPowerDown), 1, SPROP_UNSIGNED),
END_SEND_TABLE
()
...
{
	AI_TraceHull(GetAbsOrigin() + Vector(0, 0, 64), GetAbsOrigin() - Vector(0, 0, MAX_TRACE_LENGTH),
				 Vector(-16, -16, -16), Vector(16, 16, 16), MASK_NPCSOLID, this, GetCollisionGroup(), tr);
...
void CNPC_RollerMine::WakeNeighbors()
{
	...
	{
		AISquadIter_t iter;
		for(CAI_BaseNPC *pSquadMember = m_pSquad->GetFirstMember(&iter); pSquadMember;
	...
	int boxCount =
		UTIL_EntitiesInBox(entityList, ARRAYSIZE(entityList), GetAbsOrigin() - range, GetAbsOrigin() + range, FL_NPC);
...
void CNPC_RollerMine::RunAI()
{
	if(m_bTurnedOn)
	{
		...
		if(!m_bHeld && vecVelocity.Length() > 64.0)
		{
			...
			{
				// Scare combine
				CSoundEnt::InsertSound((SOUND_DANGER | SOUND_CONTEXT_COMBINE_ONLY | SOUND_CONTEXT_REACT_TO_SOURCE |
										SOUND_CONTEXT_DANGER_APPROACH),
									   WorldSpaceCenter() + Vector(0, 0, 32) + vecVelocity * 0.5f, 120.0f, 0.2f, this,
									   SOUNDENT_CHANNEL_REPEATED_DANGER);
			}
			else
			{
				// Scare player allies
				CSoundEnt::InsertSound((SOUND_DANGER | SOUND_CONTEXT_EXCLUDE_COMBINE | SOUND_CONTEXT_REACT_TO_SOURCE |
										SOUND_CONTEXT_DANGER_APPROACH),
									   WorldSpaceCenter() + Vector(0, 0, 32) + vecVelocity * 0.5f, 120.0f, 0.2f, this,
									   SOUNDENT_CHANNEL_REPEATED_DANGER);
			}
...
void CNPC_RollerMine::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
...

		{
			...
			trace_t tr;
			AI_TraceLine(GetAbsOrigin() + Vector(0, 0, 1), GetAbsOrigin() - Vector(0, 0, 64), MASK_SOLID_BRUSHONLY,
		...
		case TASK_ROLLERMINE_NUDGE_TOWARDS_NODES:
		{
			...
			if(pPhysicsObject)
			{
				...
				for(int i = 0; i < 4; i++)
				{
					...
					UTIL_TraceLine(WorldSpaceCenter(), WorldSpaceCenter() + vecNudge * 24.0f, MASK_SHOT, this,
...
void CNPC_RollerMine::RunTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		case TASK_ROLLERMINE_UNBURROW:
		{
			...
			trace_t tr;
			AI_TraceHull(vCenter, vCenter, Vector(-16, -16, -16), Vector(16, 16, 16), MASK_NPCSOLID, this,
		...
		case TASK_WALK_PATH:
			...
			if((GetLocalOrigin() - GetNavigator()->GetCurWaypointPos()).Length() <= 64)
			{
				if(GetNavigator()->CurWaypointIsGoal())
				{
					...
					m_RollerController.m_vecAngular +=
						WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()), vecRight, -m_flForwardSpeed * 5);
...

			{
				...
				if(flDot > 0.25 && flDot < 0.7)
				{
					...
					m_RollerController.m_vecAngular = WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()),
				...
				if(m_bHackedByAlyx)
				{
					// Move faster.
					m_RollerController.m_vecAngular +=
						WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()), vecRight, m_flForwardSpeed * 2.0f);
				}
				else
				{
					m_RollerController.m_vecAngular +=
						WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()), vecRight, m_flForwardSpeed);
				}
			}
			break;
		...
		case TASK_ROLLERMINE_CHARGE_ENEMY:
		{
			...
			// NDebugOverlay::Line( GetLocalOrigin(), GetLocalOrigin() + (GetEnemy()->GetLocalOrigin() -
			// GetLocalOrigin()), 0,255,0, true, 0.1 );
			...
			if((m_bIsOpen == false) &&
			   ((UTIL_DistApprox(GetAbsOrigin(), GetEnemy()->GetAbsOrigin()) <= flThreshold) || !IsActive()))
			{
				Open();
			}
			else if(m_bIsOpen)
...
void CNPC_RollerMine::ShockTouch(CBaseEntity *pOther)
{
	...
	if(FClassnameIs(pOther, "npc_combine_s"))
	{
		if(pOther->GetHealth() <= (pOther->GetMaxHealth() / 2))
		{
			...
			pOther->MyCombatCharacterPointer()->BecomeRagdollBoogie(this, vecDamageForce, 5.0f,
...
void CNPC_RollerMine::OnPhysGunDrop(CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason)
{
	...
	if(Reason == LAUNCHED_BY_CANNON)
	{
		if(m_bIsOpen)
		{
			...
			VPhysicsGetObject()->SetCallbackFlags(VPhysicsGetObject()->GetCallbackFlags() | CALLBACK_GLOBAL_TOUCH |
...
void CNPC_RollerMine::UpdateRollingSound()
{
	...
	if(m_pRollSound)
	{
		...
		// );
		controller.SoundChangePitch(m_pRollSound, params.pitchlow + (params.pitchhigh - params.pitchlow) * rollingSpeed,
									0.1);
...
void CNPC_RollerMine::UpdatePingSound()
{
	...
	if(pingSpeed > 0)
	{
		...
		if(!m_pPingSound)
		{
			CPASAttenuationFilter filter(this);
			m_pPingSound =
				controller.SoundCreate(filter, entindex(), params.channel, params.soundname, params.soundlevel);
			controller.Play(m_pPingSound, params.volume, 101);
		}

		controller.SoundChangePitch(m_pPingSound, params.pitchlow + (params.pitchhigh - params.pitchlow) * pingSpeed,
									0.1);
...
AI_BEGIN_CUSTOM_NPC(npc_rollermine, CNPC_RollerMine)
	...
	DEFINE_SCHEDULE(SCHED_ROLLERMINE_CHASE_ENEMY,
