...
BEGIN_DATADESC(CNPC_CraneDriver)
	...
	DEFINE_INPUTFUNC(FIELD_STRING, "ForcePickup", InputForcePickup),
		...
		DEFINE_FIELD(m_hPickupTarget, FIELD_EHANDLE), DEFINE_FIELD(m_flDistanceToTarget, FIELD_FLOAT),
		DEFINE_UTLVECTOR(m_PreviouslyPickedUpObjects, FIELD_EHANDLE), DEFINE_FIELD(m_bForcedPickup, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bForcedDropoff, FIELD_BOOLEAN), DEFINE_FIELD(m_flDropWait, FIELD_FLOAT),
		DEFINE_KEYFIELD(m_flReleasePause, FIELD_FLOAT, "releasepause"), DEFINE_FIELD(m_flReleaseAt, FIELD_FLOAT),

		// Outputs
		DEFINE_OUTPUT(m_OnPickedUpObject, "OnPickedUpObject"), DEFINE_OUTPUT(m_OnDroppedObject, "OnDroppedObject"),
...
void CNPC_CraneDriver::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_CRANE_FIND_OBJECT_TO_PICKUP:
		{
			...
			int count =
				UTIL_EntitiesInBox(pList, 1024, m_hCrane->GetAbsOrigin() - delta, m_hCrane->GetAbsOrigin() + delta, 0);
...
void CNPC_CraneDriver::InputForceDrop(inputdata_t &inputdata)
{
	...
	{
		CBaseEntity *pEntity =
			gEntList.FindEntityByName(NULL, iszDropName, NULL, inputdata.pActivator, inputdata.pCaller);
		if(!pEntity)
		{
