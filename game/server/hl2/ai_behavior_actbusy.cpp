...
#define ACTBUSY_COMBAT_PLAYER_MAX_DIST \
	720.0f // NPCs in combat actbusy should try to stay within this distance of the player.
...
BEGIN_DATADESC(CAI_ActBusyBehavior)
	DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN), DEFINE_FIELD(m_bForceActBusy, FIELD_BOOLEAN),
		DEFINE_CUSTOM_FIELD(m_ForcedActivity, ActivityDataOps()), DEFINE_FIELD(m_bTeleportToBusy, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bUseNearestBusy, FIELD_BOOLEAN), DEFINE_FIELD(m_bLeaving, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bVisibleOnly, FIELD_BOOLEAN), DEFINE_FIELD(m_bUseRenderBoundsForCollision, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flForcedMaxTime, FIELD_FLOAT), DEFINE_FIELD(m_bBusy, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bMovingToBusy, FIELD_BOOLEAN), DEFINE_FIELD(m_bNeedsToPlayExitAnim, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flNextBusySearchTime, FIELD_TIME), DEFINE_FIELD(m_flEndBusyAt, FIELD_TIME),
		DEFINE_FIELD(m_flBusySearchRange, FIELD_FLOAT), DEFINE_FIELD(m_bInQueue, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iCurrentBusyAnim, FIELD_INTEGER), DEFINE_FIELD(m_hActBusyGoal, FIELD_EHANDLE),
		DEFINE_FIELD(m_bNeedToSetBounds, FIELD_BOOLEAN), DEFINE_FIELD(m_hSeeEntity, FIELD_EHANDLE),
		DEFINE_FIELD(m_fTimeLastSawSeeEntity, FIELD_TIME), DEFINE_FIELD(m_bExitedBusyToDueLostSeeEntity, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bExitedBusyToDueSeeEnemy, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iNumConsecutivePathFailures, FIELD_INTEGER), DEFINE_FIELD(m_bAutoFireWeapon, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flDeferUntil, FIELD_TIME), DEFINE_FIELD(m_iNumEnemiesInSafeZone, FIELD_INTEGER),
END_DATADESC();
...
bool CActBusyAnimData::ParseActBusyFromKV(busyanim_t *pAnim, KeyValues *pSection)
{
	...
	pAnim->iActivities[BA_ENTRY] =
		(Activity)CAI_BaseNPC::GetActivityID(pSection->GetString("entry_anim", "ACT_INVALID"));
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::ForceActBusy(CAI_ActBusyGoal *pGoal, CAI_Hint *pHintNode, float flMaxTime, bool bVisibleOnly,
									   bool bTeleportToBusy, bool bUseNearestBusy, CBaseEntity *pSeeEntity,
									   Activity activity)
{
	Assert(!m_bLeaving);
...
bool CAI_ActBusyBehavior::FValidateHintType(CAI_Hint *pHint)
{
	...
	// Check for clearance
	trace_t tr;
	AI_TraceHull(pHint->GetAbsOrigin(), pHint->GetAbsOrigin(), GetOuter()->WorldAlignMins(),
				 GetOuter()->WorldAlignMaxs(), MASK_SOLID, GetOuter(), COLLISION_GROUP_NONE, &tr);
	if(tr.fraction == 1.0)
		return true;
	...
	if(ai_debug_actbusy.GetInt() == 3 && GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)
	{
		...
		NDebugOverlay::Box(pHint->GetAbsOrigin(), GetOuter()->WorldAlignMins(), GetOuter()->WorldAlignMaxs(), 255, 0, 0,
						   8, 2.0);
	}
...
bool CAI_ActBusyBehavior::ShouldIgnoreSound(CSound *pSound)
{
	...
	if(m_bBusy)
	{
		...
		if(pBusyAnim &&
		   ((pBusyAnim->iBusyInterruptType == BA_INT_AMBUSH) || (pBusyAnim->iBusyInterruptType == BA_INT_COMBAT)))
		{
			/*
...
void CAI_ActBusyBehavior::GatherConditions(void)
{
	...
	{
		if((!bCheckFOV || GetOuter()->FInViewCone(m_hSeeEntity)) && GetOuter()->QuerySeeEntity(m_hSeeEntity) &&
	...
	if(IsCurSchedule(SCHED_ACTBUSY_START_BUSYING))
	{
		...
		if(IsCombatActBusy())
		{
			GetOuter()->SetCustomInterruptCondition(
				GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_ACTBUSY_ENEMY_TOO_CLOSE));
		}
	}
	...
	if(pBusyAnim)
	{
		switch(pBusyAnim->iBusyInterruptType)
		{
			...
			case BA_INT_SIEGE_DEFENSE:
			{
				...
				GetOuter()->SetCustomInterruptCondition(
					GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_ACTBUSY_AWARE_OF_ENEMY_IN_SAFE_ZONE));
				GetOuter()->SetCustomInterruptCondition(
					GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_ACTBUSY_ENEMY_TOO_CLOSE));
			}
			break;
...
int CAI_ActBusyBehavior::SelectScheduleWhileNotBusy(int iBase)
{
	...
	if(m_bForceActBusy || m_flNextBusySearchTime < gpGlobals->curtime)
	{
		...
		{
			m_flNextBusySearchTime = gpGlobals->curtime + RandomFloat(ai_actbusy_search_time.GetFloat(),
		...
		if(!pNode)
		{
			if(IsCombatActBusy())
			{
				if(m_hActBusyGoal->IsCombatActBusyTeleportAllowed() && m_iNumConsecutivePathFailures >= 2 &&
			...
			if(HasAnimForActBusy(iBusyAnim, BA_BUSY))
			{
				...
				if(ai_debug_actbusy.GetInt() == 4)
				{
					Msg("ACTBUSY: NPC %s (%s) found Actbusy node %s \n", GetOuter()->GetClassname(),
						GetOuter()->GetDebugName(), pNode->GetDebugName());
				}
				...
				if(GetHintNode() && GetHintNode()->Lock(GetOuter()))
				{
					if(ai_debug_actbusy.GetInt() == 2)
					{
						...
						NDebugOverlay::Line(GetOuter()->WorldSpaceCenter(), pNode->GetAbsOrigin(), 0, 255, 0, true,
											5.0);
						NDebugOverlay::Box(pNode->GetAbsOrigin(), GetOuter()->WorldAlignMins(),
...
int CAI_ActBusyBehavior::SelectScheduleWhileBusy(void)
{
	...
	if(IsCombatActBusy() &&
	   (HasCondition(COND_ACTBUSY_AWARE_OF_ENEMY_IN_SAFE_ZONE) || HasCondition(COND_ACTBUSY_ENEMY_TOO_CLOSE)))
	{
		return SCHED_ACTBUSY_STOP_BUSYING;
	}
...
int CAI_ActBusyBehavior::SelectSchedule()
{
	...
	bool bShouldNotBeBusy =
		(!m_bEnabled || HasCondition(COND_PLAYER_ADDED_TO_SQUAD) || HasCondition(COND_RECEIVED_ORDERS));
	if(bShouldNotBeBusy)
	{
...
bool CAI_ActBusyBehavior::IsInSafeZone(CBaseEntity *pEntity)
{
	...
	for(int i = 0; i < m_SafeZones.Count(); i++)
	{
		...
		if(vecLocation.x > pSafeZone->vecMins.x && vecLocation.y > pSafeZone->vecMins.y &&
		   vecLocation.z > pSafeZone->vecMins.z &&

		   vecLocation.x < pSafeZone->vecMaxs.x && vecLocation.y < pSafeZone->vecMaxs.y &&
...
bool CAI_ActBusyBehavior::HasAnimForActBusy(int iActBusy, busyanimparts_t AnimPart)
{
	...
	if(pBusyAnim->iszSequences[AnimPart] != NULL_STRING)
		return (GetOuter()->LookupSequence((char *)STRING(pBusyAnim->iszSequences[AnimPart])) !=
...
void CAI_ActBusyBehavior::PlaySoundForActBusy(busyanimparts_t AnimPart)
{
	...
	if(pBusyAnim->iszSounds[AnimPart] != NULL_STRING)
	{
		...
		if(GetOuter()->GetParametersForSound(STRING(pBusyAnim->iszSounds[AnimPart]), params,
		...
		case TASK_ACTBUSY_GET_PATH_TO_ACTBUSY:
		{
			...
			if(!HasCondition(COND_TASK_FAILED))
			{
				   ...
				   pBusyAnim->iActivities[BA_ENTRY] == ACT_INVALID)
				{
					...
					{
						GetNavigator()->SetArrivalSequence(
							GetOuter()->LookupSequence(STRING(pBusyAnim->iszSequences[BA_BUSY])));
					}
				...
				else
				{
					...
					{
						GetNavigator()->SetArrivalSequence(
							GetOuter()->LookupSequence(STRING(pBusyAnim->iszSequences[BA_ENTRY])));
					}
			...
			else
			{
				...
				if(ai_debug_actbusy.GetInt() == 1)
				{
					...
					{
						// Show which actbusy we're moving towards
						NDebugOverlay::Line(GetOuter()->WorldSpaceCenter(), GetHintNode()->GetAbsOrigin(), 255, 0, 0,
											true, 1.0);
					}
				}
			}
	...
	switch(pTask->iTask)
	{
		case TASK_WAIT_FOR_MOVEMENT:
		{
			...
			if(ai_debug_actbusy.GetInt() == 1)
			{
				...
				{
					// Show which actbusy we're moving towards
					NDebugOverlay::Line(GetOuter()->WorldSpaceCenter(), GetHintNode()->GetAbsOrigin(), 0, 255, 0, true,
										0.2);
				}
			}
		...
		case TASK_ACTBUSY_VERIFY_EXIT:
		{
			if(GetOuter()->IsWaitFinished())
			{
				...
				UTIL_TraceHull(GetOuter()->GetAbsOrigin(), GetOuter()->GetAbsOrigin(), NAI_Hull::Mins(HULL_HUMAN),
							   NAI_Hull::Maxs(HULL_HUMAN), MASK_NPCSOLID, &filter, &tr);
				...
				else
				{
					...
					CBaseEntity *pBlocker =
						CEntityBlocker::Create(GetOuter()->GetAbsOrigin(), NAI_Hull::Mins(HULL_HUMAN),
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_ActBusyBehavior)
	...
	DEFINE_SCHEDULE(SCHED_ACTBUSY_START_BUSYING,
	...
	DEFINE_SCHEDULE(SCHED_ACTBUSY_LEAVE,
...
BEGIN_DATADESC(CAI_ActBusyGoal)
	DEFINE_KEYFIELD(m_flBusySearchRange, FIELD_FLOAT, "busysearchrange"),
		DEFINE_KEYFIELD(m_bVisibleOnly, FIELD_BOOLEAN, "visibleonly"), DEFINE_KEYFIELD(m_iType, FIELD_INTEGER, "type"),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnNPCStartedBusy, "OnNPCStartedBusy"), DEFINE_OUTPUT(m_OnNPCFinishedBusy, "OnNPCFinishedBusy"),
		DEFINE_OUTPUT(m_OnNPCLeft, "OnNPCLeft"), DEFINE_OUTPUT(m_OnNPCLostSeeEntity, "OnNPCLostSeeEntity"),
...
// Purpose:
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CAI_ActBusyGoal::GetBusyBehaviorForNPC(const char *pszActorName, CBaseEntity *pActivator,
															CBaseEntity *pCaller, const char *sInputName)
{
...
void CAI_ActBusyGoal::InputForceNPCToActBusy(inputdata_t &inputdata)
{
	...
	CAI_ActBusyBehavior *pBehavior =
		GetBusyBehaviorForNPC(pszParam, inputdata.pActivator, inputdata.pCaller, "InputForceNPCToActBusy");
	if(!pBehavior)
		return;
	...
	pBehavior->SetBusySearchRange(m_flBusySearchRange);
	pBehavior->ForceActBusy(this, pHintNode, flMaxTime, m_bVisibleOnly, bTeleport, bUseNearestBusy, pSeeEntity,
							activity);
}
...
BEGIN_DATADESC(CAI_ActBusyQueueGoal)
	// Keys
	DEFINE_FIELD(m_iCurrentQueueCount, FIELD_INTEGER), DEFINE_ARRAY(m_hNodes, FIELD_EHANDLE, MAX_QUEUE_NODES),
		DEFINE_ARRAY(m_bPlayerBlockedNodes, FIELD_BOOLEAN, MAX_QUEUE_NODES), DEFINE_FIELD(m_hExitNode, FIELD_EHANDLE),
		...
		DEFINE_KEYFIELD(m_iszNodes[0], FIELD_STRING, "node01"), DEFINE_KEYFIELD(m_iszNodes[1], FIELD_STRING, "node02"),
		DEFINE_KEYFIELD(m_iszNodes[2], FIELD_STRING, "node03"), DEFINE_KEYFIELD(m_iszNodes[3], FIELD_STRING, "node04"),
		DEFINE_KEYFIELD(m_iszNodes[4], FIELD_STRING, "node05"), DEFINE_KEYFIELD(m_iszNodes[5], FIELD_STRING, "node06"),
		DEFINE_KEYFIELD(m_iszNodes[6], FIELD_STRING, "node07"), DEFINE_KEYFIELD(m_iszNodes[7], FIELD_STRING, "node08"),
		DEFINE_KEYFIELD(m_iszNodes[8], FIELD_STRING, "node09"), DEFINE_KEYFIELD(m_iszNodes[9], FIELD_STRING, "node10"),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnQueueMoved, "OnQueueMoved"), DEFINE_OUTPUT(m_OnNPCLeftQueue, "OnNPCLeftQueue"),
