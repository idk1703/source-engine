...
static ConVar airboat_fatal_stress("airboat_fatal_stress", "5000", FCVAR_NONE,
...
SendPropBool(SENDINFO(m_bHeadlightIsOn)), SendPropInt(SENDINFO(m_nAmmoCount), 9),
	SendPropInt(SENDINFO(m_nExactWaterLevel)), SendPropInt(SENDINFO(m_nWaterLevel)),
...
BEGIN_DATADESC(CPropAirboat)
	DEFINE_FIELD(m_vecLastEyePos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecLastEyeTarget, FIELD_POSITION_VECTOR),
		...
		DEFINE_FIELD(m_flChargeRemainder, FIELD_FLOAT), DEFINE_FIELD(m_flDrainRemainder, FIELD_FLOAT),
		DEFINE_FIELD(m_nGunState, FIELD_INTEGER), DEFINE_FIELD(m_flNextHeavyShotTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextGunShakeTime, FIELD_TIME), DEFINE_FIELD(m_nAmmoCount, FIELD_INTEGER),
		DEFINE_FIELD(m_bHeadlightIsOn, FIELD_BOOLEAN), DEFINE_FIELD(m_hAvoidSphere, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_flPrevThrottle, FIELD_FLOAT), DEFINE_FIELD(m_flSpinRate, FIELD_FLOAT),
		DEFINE_FIELD(m_flTargetSpinRate, FIELD_FLOAT), DEFINE_FIELD(m_flPropTime, FIELD_TIME),
		DEFINE_FIELD(m_flBlurTime, FIELD_TIME), DEFINE_FIELD(m_bForcedExit, FIELD_BOOLEAN),

		DEFINE_SOUNDPATCH(m_pFanSound), DEFINE_SOUNDPATCH(m_pFanMaxSpeedSound), DEFINE_SOUNDPATCH(m_pEngineSound),
		DEFINE_SOUNDPATCH(m_pWaterFastSound), DEFINE_SOUNDPATCH(m_pWaterStoppedSound),
		DEFINE_SOUNDPATCH(m_pGunFiringSound),

		DEFINE_PHYSPTR(m_pAntiFlipConstraint),

		DEFINE_FIELD(m_flEngineIdleTime, FIELD_TIME), DEFINE_FIELD(m_flEngineDuckTime, FIELD_TIME),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "ExitVehicle", InputExitVehicle), DEFINE_INPUTFUNC(FIELD_VOID, "Wake", InputWake),
...
void CPropAirboat::Think(void)
{
	...
	trace_t tr;
	UTIL_TraceHull(startPos, endPos, vecMins, vecMaxs, (CONTENTS_WATER | CONTENTS_SLIME), this, COLLISION_GROUP_NONE,
				   &tr);
...
#define ENGINE_DUCK_TIME  4.0
	...
	else if(gpGlobals->curtime > m_flEngineDuckTime)
	{
		controller.SoundChangeVolume(m_pEngineSound,
									 RemapValClamped(speedRatio, 0, 1.0, ENGINE_MIN_VOLUME, ENGINE_MAX_VOLUME), 0.0);
...
#define FAN_DUCK_TIME		   2.0 // seconds over which to duck the fan sound
	...
	if(speedRatio >= 0.8)
	{
		...
		controller.SoundChangeVolume(m_pFanSound, RemapValClamped(speedRatio, 0.8, 1.0, FAN_MAX_VOLUME, FAN_MIN_VOLUME),
									 FAN_CHANGE_VOLUME_TIME);
		controller.SoundChangeVolume(m_pFanMaxSpeedSound,
	...
	{
		m_bFadeOutFan = false;
		controller.SoundChangeVolume(m_pFanSound,
									 RemapValClamped(fabs(m_flThrottle), 0, 1.0, FAN_MIN_VOLUME, FAN_MAX_VOLUME), 0.25);
		controller.SoundChangeVolume(m_pFanMaxSpeedSound, 0.0, 0.0);
	}
...
void CPropAirboat::UpdateWaterSound(CSoundEnvelopeController &controller, float speedRatio)
{
	...
	else
	{
		...
		if(speedRatio <= 0.1)
		{
			...
			{
				controller.SoundChangeVolume(m_pWaterStoppedSound, random->RandomFloat(0.2, 1.0),
											 random->RandomFloat(1.0, 3.0));
				controller.SoundChangePitch(m_pWaterStoppedSound, random->RandomFloat(90, 110),
											random->RandomFloat(1.0, 3.0));
...
void CPropAirboat::ProcessMovement(CBasePlayer *pPlayer, CMoveData *pMoveData)
{
	...
	if(gpGlobals->frametime != 0)
	{
		...
		{
			CSoundEnt::InsertSound(SOUND_PLAYER_VEHICLE, pPlayer->GetAbsOrigin(), 3500, 0.1f, pPlayer,
...
void CPropAirboat::CreateDangerSounds(void)
{
	...
	if(fabs(speed) > 120)
	{
		...
		CSoundEnt::InsertSound(SOUND_PHYSICS_DANGER, vecSpot, radius, soundDuration, this,
							   SOUNDENT_CHANNEL_REPEATED_PHYSICS_DANGER);
...
// * flDeltaTime;
//-----------------------------------------------------------------------------
void CPropAirboat::ComputePDControllerCoefficients(float *pCoefficientsOut, float flFrequency, float flDampening,
												   float flDeltaTime)
{
...
void CPropAirboat::DampenForwardMotion(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime)
{
	...
	ComputePDControllerCoefficients(flCoefficients, r_AirboatViewDampenFreq.GetFloat(),
