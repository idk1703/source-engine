...
ConVar g_antlionguard_hemorrhage("g_antlionguard_hemorrhage", "1", FCVAR_NONE,
...
#if HL2_EPISODIC
#define ANTLIONGUARD_POISON_TO \
	12 // we only poison Gordon down to twelve to give him a chance to regen up to 20 by the next charge
#endif
...
int g_interactionAntlionGuardFoundPhysicsObject =
	0; // We're moving to a physics object to shove it, don't all choose the same object
...
public:
	...
	public:
		CNavigator(CNPC_AntlionGuard *pOuter) : BaseClass(pOuter) {}
...
private:
	...
	void UpdatePhysicsTarget(bool bPreferObjectsAlongTargetVector,
							 float flRadius = ANTLIONGUARD_FARTHEST_PHYSICS_OBJECT);
	...
	Vector GetPhysicsHitPosition(CBaseEntity *pObject, CBaseEntity *pTarget, Vector *vecTrajectory,
...
BEGIN_DATADESC(CNPC_AntlionGuard)
	...
	DEFINE_FIELD(m_nFlinchActivity, FIELD_INTEGER), DEFINE_FIELD(m_bStopped, FIELD_BOOLEAN),
		DEFINE_KEYFIELD(m_bIsBurrowed, FIELD_BOOLEAN, "startburrowed"),
		DEFINE_KEYFIELD(m_bBarkEnabled, FIELD_BOOLEAN, "allowbark"), DEFINE_FIELD(m_flNextSummonTime, FIELD_TIME),
		DEFINE_FIELD(m_iNumLiveAntlions, FIELD_INTEGER),

		DEFINE_FIELD(m_flSearchNoiseTime, FIELD_TIME), DEFINE_FIELD(m_flAngerNoiseTime, FIELD_TIME),
		DEFINE_FIELD(m_flBreathTime, FIELD_TIME), DEFINE_FIELD(m_flChargeTime, FIELD_TIME),

		DEFINE_FIELD(m_hShoveTarget, FIELD_EHANDLE), DEFINE_FIELD(m_hChargeTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_hChargeTargetPosition, FIELD_EHANDLE), DEFINE_FIELD(m_hOldTarget, FIELD_EHANDLE),
		// m_FailedPhysicsTargets	// We do not save/load these

		DEFINE_FIELD(m_hPhysicsTarget, FIELD_EHANDLE), DEFINE_FIELD(m_vecPhysicsTargetStartPos, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vecPhysicsHitPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_flPhysicsCheckTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextHeavyFlinchTime, FIELD_TIME), DEFINE_FIELD(m_flNextRoarTime, FIELD_TIME),
		DEFINE_FIELD(m_iChargeMisses, FIELD_INTEGER), DEFINE_FIELD(m_bDecidedNotToStop, FIELD_BOOLEAN),
		...
		DEFINE_SOUNDPATCH(m_pGrowlHighSound), DEFINE_SOUNDPATCH(m_pGrowlLowSound), DEFINE_SOUNDPATCH(m_pGrowlIdleSound),
		DEFINE_SOUNDPATCH(m_pBreathSound), DEFINE_SOUNDPATCH(m_pConfusedSound),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Unburrow", InputUnburrow), DEFINE_INPUTFUNC(FIELD_VOID, "Ragdoll", InputRagdoll),
...
static impactentry_t antlionGuardLinearTable[] = {
	{100 * 100, 10}, {250 * 250, 25}, {350 * 350, 50}, {500 * 500, 75}, {1000 * 1000, 100},
};
...
SendPropBool(SENDINFO(m_bCavernBreed)), SendPropBool(SENDINFO(m_bInCavern)),
...
void CNPC_AntlionGuard::Activate(void)
{
	...
	while((pObject = gEntList.FindEntityInSphere(pObject, WorldSpaceCenter(), 2500)) != NULL)
	{
		...
		if((pPhysObj == NULL) || (pPhysObj->GetMass() > ANTLIONGUARD_MAX_OBJECT_MASS) ||
		   (pPhysObj->GetMass() < ANTLIONGUARD_MIN_OBJECT_MASS))
			continue;
...
int CNPC_AntlionGuard::SelectUnreachableSchedule(void)
{
	...
	if(pTarget)
	{
		...
		if(GetSquad() != NULL)
		{
			GetSquad()->BroadcastInteraction(g_interactionAntlionGuardFoundPhysicsObject,
											 (void *)((CBaseEntity *)m_hPhysicsTarget), this);
		}
...
bool CNPC_AntlionGuard::ShouldCharge(const Vector &startPos, const Vector &endPos, bool useTime, bool bCheckForCancel)
{
	...
	if(GetSquad())
	{
		// If someone in our squad is closer to the enemy, then don't charge (we end up hitting them more often than
		// not!)
...
int CNPC_AntlionGuard::MeleeAttack1Conditions(float flDot, float flDist)
{
	...
	trace_t tr;
	TraceHull_SkipPhysics(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), Vector(-10, -10, -10), Vector(10, 10, 10),
						  MASK_SHOT_HULL, this, COLLISION_GROUP_NONE, &tr, VPhysicsGetObject()->GetMass() * 0.5);
...
float CNPC_AntlionGuard::MaxYawSpeed(void)
{
	...
	// Stay still
	if((eActivity == ACT_ANTLIONGUARD_SEARCH) || (eActivity == ACT_ANTLIONGUARD_PEEK_ENTER) ||
	   (eActivity == ACT_ANTLIONGUARD_PEEK_EXIT) || (eActivity == ACT_ANTLIONGUARD_PEEK1) ||
	   (eActivity == ACT_ANTLIONGUARD_BARK) || (eActivity == ACT_ANTLIONGUARD_PEEK_SIGHTED) ||
...
bool CNPC_AntlionGuard::OverrideMove(float flInterval)
{
	...
	// TODO: This code increases the guard's ability to successfully hit a target, but adds a new dimension of
	// navigation
	//		 trouble to do with him not being able to "close the distance" between himself and the object he wants to
	//hit. 		 Fixing this will require some thought on how he picks the correct distances to his targets and when he's
	//"close enough". -- jdw
...
void CNPC_AntlionGuard::Footstep(bool bHeavy)
{
	...
	float flNearVolume =
		RemapValClamped(flDistanceToPlayerSqr, Square(10 * 12.0f), MIN_FOOTSTEP_NEAR_DIST, VOL_NORM, 0.0f);
...
void CNPC_AntlionGuard::HandleAnimEvent(animevent_t *pEvent)
{
	...
	if(pEvent->event == AE_ANTLIONGUARD_SHOVE_PHYSOBJECT)
	{
		...
		if(pEnemy != NULL)
		{
			...
			if(hl2_episodic.GetBool())
			{
				...
				if(HasCondition(COND_ENEMY_UNREACHABLE) && HasCondition(COND_ENEMY_FACING_ME) == false)
				{
					...
					float flOffsetRadius = (m_hPhysicsTarget->CollisionProp()->BoundingRadius() +
											GetEnemy()->CollisionProp()->BoundingRadius()) *
										   1.5f;
		...
		if(GetSquad() != NULL)
		{
			GetSquad()->BroadcastInteraction(g_interactionAntlionGuardShovedPhysicsObject,
											 (void *)((CBaseEntity *)m_hPhysicsTarget), this);
		}
	...
	if(pEvent->event == AE_ANTLIONGUARD_VOICE_GROWL)
	{
		...
		{
			duration =
				ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME,
													  envAntlionGuardFastGrowl, ARRAYSIZE(envAntlionGuardFastGrowl));
		}
		else
		{
			duration = 1.0f;
	...
	if(pEvent->event == AE_ANTLIONGUARD_VOICE_BARK)
	{
		...
		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME,
															   envAntlionGuardBark1, ARRAYSIZE(envAntlionGuardBark1));
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardBark2,
											  ARRAYSIZE(envAntlionGuardBark2));
	...
	if(pEvent->event == AE_ANTLIONGUARD_VOICE_ROAR)
	{
		...
		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
			m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardFastGrowl, ARRAYSIZE(envAntlionGuardFastGrowl));
	...
	if(pEvent->event == AE_ANTLIONGUARD_VOICE_PAIN)
	{
		...
		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME,
															   envAntlionGuardPain1, ARRAYSIZE(envAntlionGuardPain1));
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardBark2,
											  ARRAYSIZE(envAntlionGuardBark2));
	...
	if(pEvent->event == AE_ANTLIONGUARD_VOICE_SQUEEZE)
	{
		...
		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
			m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardSqueeze, ARRAYSIZE(envAntlionGuardSqueeze));

		ENVELOPE_CONTROLLER.SoundChangeVolume(m_pBreathSound, 0.6f, random->RandomFloat(2.0f, 4.0f));
		ENVELOPE_CONTROLLER.SoundChangePitch(m_pBreathSound, random->RandomInt(60, 80),
	...
	if(pEvent->event == AE_ANTLIONGUARD_VOICE_GRUNT)
	{
		...
		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME,
															   envAntlionGuardGrunt, ARRAYSIZE(envAntlionGuardGrunt));
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardGrunt2,
											  ARRAYSIZE(envAntlionGuardGrunt2));
		...
		case TASK_ANTLIONGUARD_GET_PATH_TO_PHYSOBJECT:
		{
			...
			if(GetNavigator()->SetGoal(goal))
			{
				if(g_debug_antlionguard.GetInt() == 1)
				{
					...
					NDebugOverlay::Line(m_hPhysicsTarget->WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), 0, 255, 0,
										true, 2.0f);
				}
			...
			else
			{
				if(g_debug_antlionguard.GetInt() == 1)
				{
					...
					NDebugOverlay::Line(m_hPhysicsTarget->WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), 255, 0, 0,
										true, 2.0f);
				}
			...
			// It's too scary to figure out why this particular antlion guard can't get to its object, but we're
			// shipping like, tomorrow. (sjb) 8/22/2007
...
void ApplyChargeDamage(CBaseEntity *pAntlionGuard, CBaseEntity *pTarget, float flDamage)
{
	...
	if(static_cast<CNPC_AntlionGuard *>(pAntlionGuard)->IsInCavern() && pTarget->IsPlayer() && pTarget->IsAlive() &&
	   pTarget->m_iHealth > ANTLIONGUARD_POISON_TO)
	{
		// That didn't finish them. Take them down to one point with poison damage. It'll heal.
		pTarget->TakeDamage(
			CTakeDamageInfo(pAntlionGuard, pAntlionGuard, pTarget->m_iHealth - ANTLIONGUARD_POISON_TO, DMG_POISON));
	}
#endif
}
...
void CNPC_AntlionGuard::ChargeLookAhead(void)
{
	...
	TraceHull_SkipPhysics(GetAbsOrigin(), vecTestPos, testHullMins, GetHullMaxs(), MASK_SHOT_HULL, this,
						  COLLISION_GROUP_NONE, &tr, VPhysicsGetObject()->GetMass() * 0.5);
...
float CNPC_AntlionGuard::ChargeSteer(void)
{
	...
	// Probe
	TraceHull_SkipPhysics(GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(), MASK_SOLID_BRUSHONLY, this,
...
void CNPC_AntlionGuard::SummonAntlions(void)
{
	...
	spawnpos_t sAntlionSpawnPositions[] = {
		{0, 200}, {0, -200}, {128, 128}, {128, -128}, {-128, 128}, {-128, -128}, {200, 0}, {-200, 0},
	};

	// Only spawn up to our max count
	int iSpawnPoint = 0;
	for(int i = 0;
		(m_iNumLiveAntlions < ANTLIONGUARD_SUMMON_COUNT) && (iSpawnPoint < ARRAYSIZE(sAntlionSpawnPositions)); i++)
	{
		// Determine spawn position for the antlion
		Vector vecSpawn = GetAbsOrigin() + (sAntlionSpawnPositions[iSpawnPoint].flForward * vecForward) +
						  (sAntlionSpawnPositions[iSpawnPoint].flRight * vecRight);
		iSpawnPoint++;
		...
		trace_t tr;
		UTIL_TraceHull(vecSpawn, vecSpawn - Vector(0, 0, 128), NAI_Hull::Mins(HULL_MEDIUM), NAI_Hull::Maxs(HULL_MEDIUM),
					   MASK_NPCSOLID, NULL, COLLISION_GROUP_NONE, &tr);
		if(tr.startsolid || tr.allsolid || tr.fraction == 1.0)
		{
			if(g_debug_antlionguard.GetInt() == 2)
			{
				NDebugOverlay::Box(tr.endpos, NAI_Hull::Mins(HULL_MEDIUM), NAI_Hull::Maxs(HULL_MEDIUM), 255, 0, 0, true,
								   5.0f);
			}
			continue;
		}
		...
		if((pdata->game.material != CHAR_TEX_DIRT) && (pdata->game.material != CHAR_TEX_SAND))
		{
			if(g_debug_antlionguard.GetInt() == 2)
			{
				NDebugOverlay::Box(tr.endpos, NAI_Hull::Mins(HULL_MEDIUM), NAI_Hull::Maxs(HULL_MEDIUM), 255, 128, 128,
								   true, 5.0f);
			}
			continue;
		}
		...
		if(g_debug_antlionguard.GetInt() == 2)
		{
			NDebugOverlay::Box(tr.endpos, NAI_Hull::Mins(HULL_MEDIUM), NAI_Hull::Maxs(HULL_MEDIUM), 0, 255, 0, true,
							   5.0f);
...
bool CNPC_AntlionGuard::ShouldWatchEnemy(void)
{
	...
	if((nActivity == ACT_ANTLIONGUARD_SEARCH) || (nActivity == ACT_ANTLIONGUARD_PEEK_ENTER) ||
	   (nActivity == ACT_ANTLIONGUARD_PEEK_EXIT) || (nActivity == ACT_ANTLIONGUARD_PEEK1) ||
	   (nActivity == ACT_ANTLIONGUARD_PEEK_SIGHTED) || (nActivity == ACT_ANTLIONGUARD_SHOVE_PHYSOBJECT) ||
	   (nActivity == ACT_ANTLIONGUARD_PHYSHIT_FR) || (nActivity == ACT_ANTLIONGUARD_PHYSHIT_FL) ||
	   (nActivity == ACT_ANTLIONGUARD_PHYSHIT_RR) || (nActivity == ACT_ANTLIONGUARD_PHYSHIT_RL) ||
	   (nActivity == ACT_ANTLIONGUARD_CHARGE_CRASH) || (nActivity == ACT_ANTLIONGUARD_CHARGE_HIT) ||
...
void CNPC_AntlionGuard::MaintainPhysicsTarget(void)
{
	...
	{
		NDebugOverlay::Cross3D(m_hPhysicsTarget->WorldSpaceCenter(), -Vector(32, 32, 32), Vector(32, 32, 32), 255, 255,
							   255, true, 1.0f);
	}
}
...
void CNPC_AntlionGuard::PrescheduleThink(void)
{
	...
	if(IsCurSchedule(SCHED_ANTLIONGUARD_CHASE_ENEMY) || IsCurSchedule(SCHED_ANTLIONGUARD_PATROL_RUN) ||
	   IsCurSchedule(SCHED_ANTLIONGUARD_CANT_ATTACK) || IsCurSchedule(SCHED_ANTLIONGUARD_CHASE_ENEMY_TOLERANCE))
	{
		bool bCheckAlongLine =
			(IsCurSchedule(SCHED_ANTLIONGUARD_CHASE_ENEMY) || IsCurSchedule(SCHED_ANTLIONGUARD_CHASE_ENEMY_TOLERANCE));
	...
	if((m_flGroundSpeed <= 0.0f))
	{
		...
		if(m_flBreathTime < gpGlobals->curtime)
		{
			...
			ENVELOPE_CONTROLLER.SoundChangeVolume(m_pGrowlIdleSound, random->RandomFloat(0.2f, 0.3f),
												  random->RandomFloat(0.5f, 1.0f));
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pGrowlIdleSound, random->RandomInt(80, 120),
												 random->RandomFloat(0.5f, 1.0f));
	...
	else
	{
		if(m_bStopped)
		{
			...
			ENVELOPE_CONTROLLER.SoundChangeVolume(m_pBreathSound, 0.6f, random->RandomFloat(2.0f, 4.0f));
			ENVELOPE_CONTROLLER.SoundChangePitch(m_pBreathSound, random->RandomInt(140, 160),
												 random->RandomFloat(2.0f, 4.0f));
...
//-----------------------------------------------------------------------------
Vector CNPC_AntlionGuard::GetPhysicsHitPosition(CBaseEntity *pObject, CBaseEntity *pTarget, Vector *vecTrajectory,
												float *flClearDistance)
{
	...
	Vector extent = physcollision->CollideGetExtent(pPhys->GetCollide(), pObject->GetAbsOrigin(),
													pObject->GetAbsAngles(), -vecToTarget);
...
inline bool CNPC_AntlionGuard::CanStandAtPoint(const Vector &vecPos, Vector *pOut)
{
	...
	if(tr.startsolid && !tr.allsolid)
	{
		...
		UTIL_TraceHull(tr.endpos, tr.endpos, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this, COLLISION_GROUP_NONE,
					   &tr);
...
bool CNPC_AntlionGuard::CanStandAtShoveTarget(CBaseEntity *pShoveObject, CBaseEntity *pTarget, Vector *pOut)
{
	...
	if(g_debug_antlionguard.GetInt() == 3)
	{
		NDebugOverlay::HorzArrow(pShoveObject->WorldSpaceCenter(),
								 pShoveObject->WorldSpaceCenter() + vecTrajectory * 64.0f, 16.0f, 255, 255, 0, 16, true,
								 15.0f);
	}
	...
	if(CanStandAtPoint(vecHitPosition, &vecStandPosition) == false)
	{
		...
		// Try to the side
		if(CanStandAtPoint((pShoveObject->WorldSpaceCenter() + (vecSkewTrajectory * flClearDistance)),
						   &vecStandPosition) == false)
		{
			// Try the other side
			SinCos(flRad - flRadOffset, &vecSkewTrajectory.y, &vecSkewTrajectory.x);
			vecSkewTrajectory.z = 0.0f;
...
CBaseEntity *CNPC_AntlionGuard::FindPhysicsObjectTarget(const PhysicsObjectCriteria_t &criteria)
{
	...
	// Use the distance to the player as the base cost for throwing an object (avoids pushing things too close to the
	// player)
	float flLeastCost = (criteria.bPreferObjectsAlongTargetVector)
							? (criteria.pTarget->GetAbsOrigin() - GetAbsOrigin()).LengthSqr()
							: Square(criteria.flRadius);
	float flCost;
	...
	while((pObject = GetNextShoveTarget(pObject, iter)) != NULL)
	{
		...
		if(criteria.bPreferObjectsAlongTargetVector)
		{
			...
			if((GetAbsOrigin() - vecObjCenter).LengthSqr() >
			   (GetAbsOrigin() - criteria.pTarget->GetAbsOrigin()).LengthSqr())
				continue;
		...
		trace_t tr;
		UTIL_TraceLine(vecObjCenter, criteria.pTarget->BodyTarget(vecObjCenter), MASK_SOLID_BRUSHONLY, this,
...
void CNPC_AntlionGuard::ImpactShock(const Vector &origin, float radius, float magnitude, CBaseEntity *pIgnored)
{
	...
	while((pEntity = gEntList.FindEntityInSphere(pEntity, origin, radius)) != NULL)
	{
		...
		if(pEntity->GetMoveType() == MOVETYPE_VPHYSICS ||
		   (pEntity->VPhysicsGetObject() && pEntity->IsPlayer() == false))
		{
...
void CNPC_AntlionGuard::StartSounds(void)
{
	...
	{
		m_pGrowlHighSound =
			ENVELOPE_CONTROLLER.SoundCreate(filter, entindex(), CHAN_VOICE, "NPC_AntlionGuard.GrowlHigh", ATTN_NORM);
	...
	{
		m_pGrowlIdleSound =
			ENVELOPE_CONTROLLER.SoundCreate(filter, entindex(), CHAN_STATIC, "NPC_AntlionGuard.GrowlIdle", ATTN_NORM);
	...
	{
		m_pBreathSound =
			ENVELOPE_CONTROLLER.SoundCreate(filter, entindex(), CHAN_ITEM, "NPC_AntlionGuard.BreathSound", ATTN_NORM);
	...
	{
		m_pConfusedSound =
			ENVELOPE_CONTROLLER.SoundCreate(filter, entindex(), CHAN_WEAPON, "NPC_AntlionGuard.Confused", ATTN_NORM);
...
bool CNPC_AntlionGuard::QueryHearSound(CSound *pSound)
{
	...
	if(pSound->SoundType() == SOUND_DANGER &&
	   (pSound->m_hOwner != NULL && pSound->m_hOwner->Classify() == CLASS_ANTLION))
		return false;
