...
SendPropBool(SENDINFO(m_bIsCurrentlyUpgrading)), SendPropFloat(SENDINFO(m_flTimeForceView)),
END_SEND_TABLE
()
...
BEGIN_DATADESC(CWeaponPhysCannon)
	...
	DEFINE_FIELD(m_bOpen, FIELD_BOOLEAN), DEFINE_FIELD(m_bActive, FIELD_BOOLEAN),
		...
		DEFINE_FIELD(m_bIsCurrentlyUpgrading, FIELD_BOOLEAN), DEFINE_FIELD(m_flTimeForceView, FIELD_TIME),
...
bool CWeaponPhysCannon::Holster(CBaseCombatWeapon *pSwitchingTo)
{
	...
	if(m_bActive)
	{
		if(!pSwitchingTo || (m_grabController.GetAttached() == pSwitchingTo &&
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError)
{
	...
	Vector radial =
		physcollision->CollideGetExtent(pPhys->GetCollide(), vec3_origin, pEntity->GetAbsAngles(), -forward);
