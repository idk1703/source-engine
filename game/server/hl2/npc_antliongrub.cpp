...
BEGIN_DATADESC(CAntlionGrub)
	...
	DEFINE_FIELD(m_hGlowSprite, FIELD_EHANDLE), DEFINE_FIELD(m_flFlinchTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextIdleSoundTime, FIELD_TIME), DEFINE_FIELD(m_flNextSquealSoundTime, FIELD_TIME),
		DEFINE_FIELD(m_State, FIELD_INTEGER),

		DEFINE_INPUTFUNC(FIELD_FLOAT, "Agitate", InputAgitate),

		DEFINE_OUTPUT(m_OnAgitated, "OnAgitated"), DEFINE_OUTPUT(m_OnDeath, "OnDeath"),
		DEFINE_OUTPUT(m_OnDeathByPlayer, "OnDeathByPlayer"),

		// Functions
		DEFINE_ENTITYFUNC(GrubTouch), DEFINE_ENTITYFUNC(IdleThink), DEFINE_ENTITYFUNC(FlinchThink),
...
int CAntlionGrub::GetNuggetDenomination(void)
{
	...
	// flDesiredHealthPerc, flSeed, flRandomPerc, nDenomination );
...
{
	return (UTIL_FindClientInPVS(edict()) != NULL) ||
...
// Output : inline bool
//-----------------------------------------------------------------------------
inline bool CAntlionGrub::ProbeSurface(const Vector &vecTestPos, const Vector &vecDir, Vector *vecResult,
									   Vector *vecNormal)
{
	// Trace down to find a surface
	trace_t tr;
	UTIL_TraceLine(vecTestPos, vecTestPos + (vecDir * 256.0f), MASK_NPCSOLID & (~CONTENTS_MONSTER), this,
...
void CAntlionGrub::AttachToSurface(void)
{
	...
	if(ProbeSurface(WorldSpaceCenter() - (vecForward * 12.0f) + vecOffset, vecDown, &vecPivot, &vecPivotNormal) ==
	   false)
	{
...
void CAntlionGrub::GrubTouch(CBaseEntity *pOther)
{
	...
	IPhysicsObject *pPhysOther =
		pOther->VPhysicsGetObject(); // bool bThrown = ( pTarget->VPhysicsGetObject()->GetGameFlags() &
									 // FVPHYSICS_WAS_THROWN ) != 0;
	if(pOther->IsPlayer() || FClassnameIs(pOther, "npc_vortigaunt") ||
	   (pPhysOther && (pPhysOther->GetGameFlags() & FVPHYSICS_WAS_THROWN)))
	{
...
void CAntlionGrub::MakeSquashDecals(const Vector &vecOrigin)
{
	...
	for(int i = 0; i < 8; i++)
	{
		...
		UTIL_TraceLine(vecStart, vecStart + (vecTraceDir * (5 * 12)), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
					   &tr);
