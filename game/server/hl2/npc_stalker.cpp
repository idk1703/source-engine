...
BEGIN_DATADESC(CNPC_Stalker)
	...
	DEFINE_KEYFIELD(m_eBeamPower, FIELD_INTEGER, "BeamPower"), DEFINE_FIELD(m_vLaserDir, FIELD_VECTOR),
		DEFINE_FIELD(m_vLaserTargetPos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_fBeamEndTime, FIELD_FLOAT),
		DEFINE_FIELD(m_fBeamRechargeTime, FIELD_FLOAT), DEFINE_FIELD(m_fNextDamageTime, FIELD_FLOAT),
		DEFINE_FIELD(m_bPlayingHitWall, FIELD_FLOAT), DEFINE_FIELD(m_bPlayingHitFlesh, FIELD_FLOAT),
		DEFINE_FIELD(m_pBeam, FIELD_CLASSPTR), DEFINE_FIELD(m_pLightGlow, FIELD_CLASSPTR),
		DEFINE_FIELD(m_flNextNPCThink, FIELD_FLOAT), DEFINE_FIELD(m_vLaserCurPos, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_flNextAttackSoundTime, FIELD_TIME), DEFINE_FIELD(m_flNextBreatheSoundTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextScrambleSoundTime, FIELD_TIME), DEFINE_FIELD(m_nextSmokeTime, FIELD_TIME),
...
bool CNPC_Stalker::IsValidEnemy(CBaseEntity *pEnemy)
{
	...
	if(IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2) &&
	   !HasStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2))
	{
		return false;
	}
...
void CNPC_Stalker::UpdateAttackBeam(void)
{
	...
	if(pEnemy)
	{
		...
		else
		{
			...
			trace_t tr;
			AI_TraceLine(vecSrc, vecSrc + m_vLaserDir * MAX_STALKER_FIRE_RANGE, MASK_SHOT, this, COLLISION_GROUP_NONE,
						 &tr);
			// ---------------------------------------------
...
void CNPC_Stalker::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_ANNOUNCE_ATTACK:
		{
			...
			if(pBCC && (!pBCC->FInViewCone(this)) && (gpGlobals->curtime - m_flLastAttackTime > 1.0))
			{
		...
		case TASK_RANGE_ATTACK1:
		{
			...
			if(pEnemy)
			{
				...
				if(pBCC)
				{
					...
					if((DotProduct(vBCCFacing, targetToMe) < 0) && (pBCC->GetSmoothedVelocity().Length() < 50))
					{
						missPos.z -= 150;
					}
					// --------------------------------------------------------
		...
		case TASK_GET_PATH_TO_ENEMY_LOS:
		{
			...
			if(GetTacticalServices()->FindLos(m_vLaserCurPos, m_vLaserCurPos, MIN_STALKER_FIRE_RANGE,
											  MAX_STALKER_FIRE_RANGE, 1.0, &posLos))
			{
...
void CNPC_Stalker::HandleAnimEvent(animevent_t *pEvent)
{
	switch(pEvent->event)
	{
		...
		case STALKER_AE_MELEE_HIT:
		{
			...
			pHurt = CheckTraceHullAttack(32, Vector(-16, -16, -16), Vector(16, 16, 16), sk_stalker_melee_dmg.GetFloat(),
										 DMG_SLASH);
			...
			if(pHurt)
			{
				...
				if(pBCC)
				{
					SpawnBlood(pBCC->EyePosition(), g_vecAttackDir, pBCC->BloodColor(),
							   sk_stalker_melee_dmg.GetFloat());
				}
...
AI_BEGIN_CUSTOM_NPC(npc_stalker, CNPC_Stalker)
	...
	DEFINE_SCHEDULE(SCHED_STALKER_PATROL,
					...
					"	Tasks"
					"		TASK_STOP_MOVING				0"
					"		TASK_WAIT						0.5" // This makes them look a bit more vigilant, instead of
																 // INSTANTLY patrolling after some other action.
					"		TASK_WAIT_RANDOM				0.5"
