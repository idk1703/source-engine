...
BEGIN_DATADESC(CNPC_Zombine)
	DEFINE_FIELD(m_flSprintTime, FIELD_TIME), DEFINE_FIELD(m_flSprintRestTime, FIELD_TIME),
		DEFINE_FIELD(m_flSuperFastAttackTime, FIELD_TIME), DEFINE_FIELD(m_hGrenade, FIELD_EHANDLE),
		DEFINE_FIELD(m_flGrenadePullTime, FIELD_TIME), DEFINE_FIELD(m_iGrenadeCount, FIELD_INTEGER),
...
void CNPC_Zombine::PrescheduleThink(void)
{
	...
	if(HasGrenade())
	{
		CSoundEnt::InsertSound(SOUND_DANGER, GetAbsOrigin() + GetSmoothedVelocity() * 0.5f, 256, 0.1, this,
							   SOUNDENT_CHANNEL_ZOMBINE_GRENADE);

		if(IsSprinting() && GetEnemy() && GetEnemy()->Classify() == CLASS_PLAYER_ALLY_VITAL &&
...
void CNPC_Zombine::HandleAnimEvent(animevent_t *pEvent)
{
	if(pEvent->event == AE_ZOMBINE_PULLPIN)
	{
		...
		CBaseGrenade *pGrenade =
			Fraggrenade_Create(vecStart, vec3_angle, vec3_origin, AngularImpulse(0, 0, 0), this, 3.5f, true);
			...
			if(pPhysicsObject)
			{
				...
				for(int i = 0; i < g_AI_Manager.NumAIs(); i++)
				{
					...
					if(pNPC->Classify() == CLASS_PLAYER_ALLY ||
					   (pNPC->Classify() == CLASS_PLAYER_ALLY_VITAL && pNPC->FVisible(this)))
					{
						int priority;
