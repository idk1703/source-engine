...
BEGIN_DATADESC_NO_BASE(washentity_t)
	DEFINE_FIELD(hEntity, FIELD_EHANDLE), DEFINE_FIELD(flWashStartTime, FIELD_TIME),
END_DATADESC()

BEGIN_DATADESC(CBaseHelicopter)

	DEFINE_THINKFUNC(HelicopterThink), DEFINE_THINKFUNC(RotorWashThink), DEFINE_THINKFUNC(CallDyingThink),
		DEFINE_THINKFUNC(DelayedKillThink), DEFINE_ENTITYFUNC(CrashTouch), DEFINE_ENTITYFUNC(FlyTouch),

		DEFINE_SOUNDPATCH(m_pRotorSound), DEFINE_SOUNDPATCH(m_pRotorBlast), DEFINE_FIELD(m_flForce, FIELD_FLOAT),
		DEFINE_FIELD(m_fHelicopterFlags, FIELD_INTEGER), DEFINE_FIELD(m_vecDesiredFaceDir, FIELD_VECTOR),
		...
		DEFINE_FIELD(m_flRandomOffsetTime, FIELD_TIME), DEFINE_FIELD(m_vecRandomOffset, FIELD_VECTOR),
		DEFINE_FIELD(m_flRotorWashEntitySearchTime, FIELD_TIME), DEFINE_FIELD(m_bSuppressSound, FIELD_BOOLEAN),
		...
		// Inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "Activate", InputActivate), DEFINE_INPUTFUNC(FIELD_VOID, "GunOn", InputGunOn),
		DEFINE_INPUTFUNC(FIELD_VOID, "GunOff", InputGunOff), DEFINE_INPUTFUNC(FIELD_VOID, "MissileOn", InputMissileOn),
...
void CBaseHelicopter::HelicopterThink(void)
{
	...
	if(GetEnemy() != NULL &&
	   (!GetEnemy()->IsAlive() || GetEnemy()->GetFlags() & FL_NOTARGET || IRelationType(GetEnemy()) == D_NU))
	{
		SetEnemy(NULL);
	}
...
void CBaseHelicopter::RotorWashThink(void)
{
	if(m_lifeState == LIFE_ALIVE || m_lifeState == LIFE_DYING)
	{
		...
		SetContextThink(&CBaseHelicopter::RotorWashThink, gpGlobals->curtime + HELICOPTER_ROTORWASH_THINK_INTERVAL,
...
void CBaseHelicopter::DoWashPushOnAirboat(CBaseEntity *pAirboat, const Vector &vecWashToAirboat, float flWashAmount)
{
...
bool CBaseHelicopter::DoWashPush(washentity_t *pWash, const Vector &vecWashOrigin)
{
	...
	float flWashAmount =
		RemapVal(flPushTime, 0, BASECHOPPER_WASH_RAMP_TIME, BASECHOPPER_WASH_PUSH_MIN, BASECHOPPER_WASH_PUSH_MAX);
	...
	if(g_debug_basehelicopter.GetInt() == BASECHOPPER_DEBUG_WASH)
	{
		...
		Msg("Pushed %s (index %d) (mass %f) with force %f (min %.2f max %.2f) at time %.2f\n", pEntity->GetClassname(),
			pEntity->entindex(), pPhysObject->GetMass(), flWashAmount, BASECHOPPER_WASH_PUSH_MIN * flMass,
...
void CBaseHelicopter::DoRotorPhysicsPush(const Vector &vecRotorOrigin, float flAltitude)
{
	...
	UTIL_TraceLine(vecRotorOrigin, vecRotorOrigin + Vector(0, 0, -flAltitude), (MASK_SOLID_BRUSHONLY | CONTENTS_WATER),
