...
BEGIN_DATADESC(CEnvHeadcrabCanister)
	...
	DEFINE_FIELD(m_bLanded, FIELD_BOOLEAN), DEFINE_EMBEDDED(m_Shared), DEFINE_FIELD(m_hTrail, FIELD_EHANDLE),
		DEFINE_FIELD(m_hSmokeTrail, FIELD_EHANDLE), DEFINE_KEYFIELD(m_nHeadcrabType, FIELD_INTEGER, "HeadcrabType"),
		...
		DEFINE_FIELD(m_vecImpactPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_bIncomingSoundStarted, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bHasDetonated, FIELD_BOOLEAN), DEFINE_FIELD(m_bLaunched, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bOpened, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_flMinRefireTime, FIELD_FLOAT, "MinSkyboxRefireTime"),
		...
		// Function Pointers.
		DEFINE_FUNCTION(HeadcrabCanisterSkyboxThink), DEFINE_FUNCTION(HeadcrabCanisterWorldThink),
		DEFINE_FUNCTION(HeadcrabCanisterSpawnHeadcrabThink), DEFINE_FUNCTION(WaitForOpenSequenceThink),
		DEFINE_FUNCTION(HeadcrabCanisterSkyboxOnlyThink), DEFINE_FUNCTION(HeadcrabCanisterSkyboxRestartThink),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnLaunched, "OnLaunched"), DEFINE_OUTPUT(m_OnImpacted, "OnImpacted"),
...
void CEnvHeadcrabCanister::Spawn(void)
{
	...
	else if(!HasSpawnFlags(SF_START_IMPACTED))
	{
		...
		if(!HasSpawnFlags(SF_LAND_AT_INITIAL_POSITION))
		{
			...
			trace_t trace;
			UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecForward * 10000, MASK_NPCWORLDSTATIC, this,
...
void CEnvHeadcrabCanister::ComputeWorldEntryPoint(Vector *pStartPosition, QAngle *pStartAngles, Vector *pStartDirection)
{
	...
	UTIL_TraceLine(GetAbsOrigin() + vecForward * 100, GetAbsOrigin() + vecForward * 10000, CONTENTS_SOLID, &filter,
				   &tr);
...
CSkyCamera *CEnvHeadcrabCanister::PlaceCanisterInWorld()
{
	...
	if(m_iszLaunchPositionName != NULL_STRING)
	{
		...
		if(!pLaunchPos)
		{
			Warning("%s (%s) could not find an entity matching LaunchPositionName of '%s'\n", GetEntityName().ToCStr(),
					GetDebugName(), STRING(m_iszLaunchPositionName));
			SUB_Remove();
		}
		else
		{
			...
			m_Shared.InitInWorld(gpGlobals->curtime, pLaunchPos->GetAbsOrigin(), GetAbsAngles(), vecImpactDirection,
								 m_vecImpactPosition, true);
	...
	else if(DetectInSkybox())
	{
		...
		m_Shared.InitInSkybox(gpGlobals->curtime, m_vecImpactPosition, GetAbsAngles(), vecForward, m_vecImpactPosition,
							  pCamera->m_skyboxData.origin, pCamera->m_skyboxData.scale);
	...
	else
	{
		...
		{
			m_Shared.InitInWorld(gpGlobals->curtime, vecStartPosition, vecStartAngles, vecDirection,
								 m_vecImpactPosition);
			SetThink(&CEnvHeadcrabCanister::HeadcrabCanisterWorldThink);
			SetNextThink(gpGlobals->curtime);
		}
...
void CEnvHeadcrabCanister::TestForCollisionsAgainstWorld(const Vector &vecEndPosition)
{
	...
	CBaseEntity *pEntity;
	for(CEntitySphereQuery sphere(vecEndPosition, flDamageRadius); (pEntity = sphere.GetCurrentEntity()) != NULL;
		sphere.NextEntity())
	{
		if(pEntity == this)
			continue;
...
void CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink()
{
	...
	if(m_nHeadcrabCount != 0)
	{
		...
		SetContextThink(&CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink, gpGlobals->curtime + flWaitTime,
						s_pHeadcrabThinkContext);
	}
	else
	{
...
void CEnvHeadcrabCanister::StartSpawningHeadcrabs(float flDelay)
{
	...
	if(m_nHeadcrabCount != 0)
	{
		SetContextThink(&CEnvHeadcrabCanister::HeadcrabCanisterSpawnHeadcrabThink, gpGlobals->curtime + flDelay,
						s_pHeadcrabThinkContext);
	}
}
...
void CEnvHeadcrabCanister::WaitForOpenSequenceThink()
{
	...
	{
		SetContextThink(&CEnvHeadcrabCanister::WaitForOpenSequenceThink, gpGlobals->curtime + 0.01f,
						s_pOpenThinkContext);
	}
}
...
void CEnvHeadcrabCanister::Detonate()
{
	...
	if(!HasSpawnFlags(SF_NO_SHAKE))
	{
		...
		float shakeRadius = (pPlayer && pPlayer->IsInAVehicle())
								? sk_env_headcrabcanister_shake_radius_vehicle.GetFloat()
								: sk_env_headcrabcanister_shake_radius.GetFloat();

		UTIL_ScreenShake(m_vecImpactPosition, sk_env_headcrabcanister_shake_amplitude.GetFloat(), 150.0, 1.0,
						 shakeRadius, SHAKE_START);
	}
	...
	if(!HasSpawnFlags(SF_NO_IMPACT_EFFECTS))
	{
		...
		ExplosionCreate(m_vecImpactPosition, GetAbsAngles(), this, 50.0f, 500.0f,
						SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODAMAGE |
