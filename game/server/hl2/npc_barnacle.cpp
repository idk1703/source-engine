...
const char *CNPC_Barnacle::m_szGibNames[NUM_BARNACLE_GIBS] = {"models/gibs/hgibs.mdl", "models/gibs/hgibs_scapula.mdl",
...
BEGIN_DATADESC(CNPC_Barnacle)
	...
	DEFINE_FIELD(m_flAltitude, FIELD_FLOAT),
		...
		DEFINE_FIELD(m_bLiftingPrey, FIELD_BOOLEAN), DEFINE_FIELD(m_bSwallowingPrey, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flDigestFinish, FIELD_TIME), DEFINE_FIELD(m_bPlayedPullSound, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bPlayerWasStanding, FIELD_BOOLEAN), DEFINE_FIELD(m_flVictimHeight, FIELD_FLOAT),
		DEFINE_FIELD(m_iGrabbedBoneIndex, FIELD_INTEGER),

		DEFINE_FIELD(m_vecRoot, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecTip, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_hTongueRoot, FIELD_EHANDLE), DEFINE_FIELD(m_hTongueTip, FIELD_EHANDLE),
		DEFINE_FIELD(m_hRagdoll, FIELD_EHANDLE), DEFINE_AUTO_ARRAY(m_pRagdollBones, FIELD_MATRIX3X4_WORLDSPACE),
		DEFINE_PHYSPTR(m_pConstraint), DEFINE_KEYFIELD(m_flRestUnitsAboveGround, FIELD_FLOAT, "RestDist"),
		DEFINE_FIELD(m_nSpitAttachment, FIELD_INTEGER), DEFINE_FIELD(m_hLastSpitEnemy, FIELD_EHANDLE),
		DEFINE_FIELD(m_nShakeCount, FIELD_INTEGER), DEFINE_FIELD(m_flNextBloodTime, FIELD_TIME),
#ifndef _XBOX
		DEFINE_FIELD(m_nBloodColor, FIELD_INTEGER),
#endif
		DEFINE_FIELD(m_vecBloodPos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_flBarnaclePullSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_flLocalTimer, FIELD_TIME), DEFINE_FIELD(m_vLastEnemyPos, FIELD_POSITION_VECTOR),
...
#ifdef HL2_EPISODIC
		DEFINE_INPUTFUNC(FIELD_VOID, "LetGo", InputLetGo), DEFINE_OUTPUT(m_OnGrab, "OnGrab"),
...
SendPropFloat(SENDINFO(m_flAltitude), 0, SPROP_NOSCALE), SendPropVector(SENDINFO(m_vecRoot), 0, SPROP_COORD),
	SendPropVector(SENDINFO(m_vecTip), 0, SPROP_COORD), SendPropVector(SENDINFO(m_vecTipDrawOffset), 0, SPROP_NOSCALE),
END_SEND_TABLE
()
...
void CNPC_Barnacle::PlayerHasIlluminatedNPC(CBasePlayer *pPlayer, float flDot)
{
	...
	if(IsAlive())
	{
		CSoundEnt::InsertSound(SOUND_MOVE_AWAY | SOUND_CONTEXT_ALLIES_ONLY, m_vecTip, 60.0f,
							   FLASHLIGHT_NPC_CHECK_INTERVAL);
	}
}
...
void CNPC_Barnacle::BarnacleThink(void)
{
	...
	else
	{
		...
		if(!UTIL_FindClientInPVS(vecSurroundMins, vecSurroundMaxs))
		{
			SetNextThink(
				gpGlobals->curtime +
				random->RandomFloat(1, 1.5)); // Stagger a bit to keep barnacles from thinking on the same frame
		}
	...
	// NDebugOverlay::Box( GetAbsOrigin() - Vector( 0, 0, m_flAltitude ), Vector( -2, -2, -2 ), Vector( 2, 2, 2 ),
	// 255,255,255, 0, 0.1 );
...
void CNPC_Barnacle::LiftRagdoll(float flBiteZOffset)
{
	...
	else
	{
		...
		if(GetEnemy()->IsAlive())
		{
			...
			CalcPoseSingle( pStudioHdr, pos, q, pAnimating->GetSequence(), pAnimating->m_flCycle,
			pAnimating->GetPoseParameterArray(), BONE_USED_BY_ANYTHING ); Studio_BuildMatrices( pStudioHdr, vec3_angle,
...
CRagdollProp *CNPC_Barnacle::AttachRagdollToTongue(CBaseAnimating *pAnimating)
{
	...
	CRagdollProp *pRagdoll =
		CreateServerRagdollAttached(pAnimating, vec3_origin, -1, COLLISION_GROUP_NONE, pTonguePhysObject, m_hTongueTip,
									0, vecBonePos, m_iGrabbedBoneIndex, vec3_origin);
	if(pRagdoll)
	{
#if HL2_EPISODIC
...
void CNPC_Barnacle::AttachTongueToTarget(CBaseEntity *pTouchEnt, Vector vecGrabPos)
{
	...
	if(/* pTouchEnt->IsPlayer() || */ pTouchEnt->MyNPCPointer())
	{
		...
		UTIL_TraceHull(origin, origin, pTouchEnt->WorldAlignMins(), pTouchEnt->WorldAlignMaxs(), MASK_NPCSOLID,
					   &traceFilter, &placementTrace);
		if(placementTrace.startsolid)
		{
			UTIL_TraceHull(origin + Vector(0, 0, 24), origin, pTouchEnt->WorldAlignMins(), pTouchEnt->WorldAlignMaxs(),
	...
	if(IsEnemyAPlayer() || IsEnemyAPhysicsObject())
	{
		...
		else
		{
			...
			vecGrabPos = physcollision->CollideGetExtent(pPlayerPhys->GetCollide(), pTouchEnt->GetAbsOrigin(),
		...
		/*
You can use this stanza to try to counterplace the constraint on the player's head so he gets hauled sideways to the
right place on the barnacle, but it is better to just move the tongue before attachment. if ( IsEnemyAPlayer() )
		{
...
#ifndef _XBOX
	UTIL_BloodSpray(m_vecBloodPos + jitterPos, Vector(0, 0, -1), m_nBloodColor, RandomInt(4, 8),
					RandomInt(0, 2) == 0 ? FX_BLOODSPRAY_ALL : FX_BLOODSPRAY_CLOUD);
#else
	UTIL_BloodSpray(m_vecBloodPos + jitterPos, Vector(0, 0, -1), BLOOD_COLOR_YELLOW, RandomInt(4, 8),
					RandomInt(0, 2) == 0 ? FX_BLOODSPRAY_ALL : FX_BLOODSPRAY_CLOUD);
#endif
}
...
void CNPC_Barnacle::Event_Killed(const CTakeDamageInfo &info)
{
	...
	trace_t bloodTrace;
	AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 256), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
				 &bloodTrace);
...
void CNPC_Barnacle::WaitTillDead(void)
{
	...
	trace_t tr;
	AI_TraceLine(m_vecRoot.Get(), m_vecRoot.Get() - Vector(0, 0, 256), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
				 &tr);
...
bool CNPC_Barnacle::IsPoisonous(CBaseEntity *pVictim)
{
	...
	if(FClassnameIs(pVictim, "npc_antlion") && static_cast<CNPC_Antlion *>(pVictim)->IsWorker())
		return true;
...
static impactentry_t barnacleLinearTable[] = {
	{150 * 150, 5}, {250 * 250, 10}, {350 * 350, 50}, {500 * 500, 100}, {1000 * 1000, 500},
};
	...
	for(int i = 0; i < nCount; i++)
	{
		...
		if(pTest->GetServerVehicle())
		{
			...
			if(pDriver)
			{
				...
				Ray_t sweptDriver;
				sweptDriver.Init(vecPrevDriverPos, pDriver->GetAbsOrigin(), pDriver->WorldAlignMins(),
								 pDriver->WorldAlignMaxs());
		...
		if(pTest != this && IRelationType(pTest) == D_HT && pVictim->m_lifeState != LIFE_DEAD &&
		   pVictim->m_lifeState != LIFE_DYING && !(pVictim->GetFlags() & FL_NOTARGET))
		{
...
//-----------------------------------------------------------------------------
CBarnacleTongueTip *CBarnacleTongueTip::CreateTongueTip(CNPC_Barnacle *pBarnacle, CBaseAnimating *pTongueRoot,
														const Vector &vecOrigin, const QAngle &vecAngles)
{
	CBarnacleTongueTip *pTip =
		(CBarnacleTongueTip *)CBaseEntity::Create("npc_barnacle_tongue_tip", vecOrigin, vecAngles);
	if(!pTip)
		return NULL;
