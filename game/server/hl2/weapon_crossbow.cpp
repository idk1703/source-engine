...
void CCrossbowBolt::BoltTouch(CBaseEntity *pOther)
{
	...
	if(pOther->m_takedamage != DAMAGE_NO)
	{
		...
		//!she's crouched in front of the player
		// (the player thinks they have clear line of sight because Alyx is crouching, but her BBOx is still full-height
		// and blocks crossbow bolts.
		if(GetOwnerEntity() && GetOwnerEntity()->IsPlayer() && pOther->Classify() == CLASS_PLAYER_ALLY_VITAL &&
		   FStrEq(STRING(gpGlobals->mapname), "ep2_outland_10"))
		{
...
void CCrossbowBolt::BubbleThink(void)
{
	...
	CSoundEnt::InsertSound(SOUND_DANGER_SNIPERONLY, GetAbsOrigin() + GetAbsVelocity() * 0.2, 120.0f, 0.5f, this,
...
()
	...
	DEFINE_FIELD(m_bMustReload, FIELD_BOOLEAN), DEFINE_FIELD(m_nChargeState, FIELD_INTEGER),
...
void CWeaponCrossbow::FireBolt(void)
{
	...
	if(FStrEq(STRING(gpGlobals->mapname), "ep2_outland_10"))
	{
		...
		{
			// If Alyx is right in front of the player, make sure the bolt starts outside of the player's BBOX, or the
			// bolt will instantly collide with the player after the owner of the bolt is switched to Alyx in
			// ::BoltTouch(). We avoid this altogether by making it impossible for the bolt to collide with the player.
...
bool CWeaponCrossbow::Deploy(void)
{
	if(m_iClip1 <= 0)
	{
		return DefaultDeploy((char *)GetViewModel(), (char *)GetWorldModel(), ACT_CROSSBOW_DRAW_UNLOADED,
							 (char *)GetAnimPrefix());
	}
