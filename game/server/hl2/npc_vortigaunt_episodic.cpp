...
BEGIN_DATADESC(CNPC_Vortigaunt)
	...
	DEFINE_FIELD(m_eHealState, FIELD_INTEGER), DEFINE_FIELD(m_flNextHealTokenTime, FIELD_TIME),
		DEFINE_ARRAY(m_hHandEffect, FIELD_EHANDLE, 2), DEFINE_FIELD(m_flNextHealTime, FIELD_TIME),
		DEFINE_FIELD(m_bPlayerRequestedHeal, FIELD_BOOLEAN), DEFINE_FIELD(m_flDispelTestTime, FIELD_TIME),
		DEFINE_FIELD(m_flHealHinderedTime, FIELD_FLOAT), DEFINE_FIELD(m_nLightningSprite, FIELD_INTEGER),
		DEFINE_FIELD(m_fGlowAge, FIELD_FLOAT), DEFINE_FIELD(m_fGlowChangeTime, FIELD_TIME),
		DEFINE_FIELD(m_bGlowTurningOn, FIELD_BOOLEAN), DEFINE_FIELD(m_nCurGlowIndex, FIELD_INTEGER),
		DEFINE_FIELD(m_flNextHealTime, FIELD_TIME), DEFINE_FIELD(m_flPainTime, FIELD_TIME),
		DEFINE_FIELD(m_nextLineFireTime, FIELD_TIME),
		DEFINE_KEYFIELD(m_bArmorRechargeEnabled, FIELD_BOOLEAN, "ArmorRechargeEnabled"),
		DEFINE_FIELD(m_bForceArmorRecharge, FIELD_BOOLEAN), DEFINE_FIELD(m_bExtractingBugbait, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iLeftHandAttachment, FIELD_INTEGER), DEFINE_FIELD(m_iRightHandAttachment, FIELD_INTEGER),
		DEFINE_FIELD(m_hHealTarget, FIELD_EHANDLE), DEFINE_FIELD(m_flBlueEndFadeTime, FIELD_TIME),
		DEFINE_FIELD(m_bIsBlue, FIELD_BOOLEAN), DEFINE_FIELD(m_bIsBlack, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flAimDelay, FIELD_TIME), DEFINE_FIELD(m_bCarryingNPC, FIELD_BOOLEAN),
...
void CNPC_Vortigaunt::RunTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		case TASK_RANGE_ATTACK1:
		{
			...
			if((HasCondition(COND_ENEMY_OCCLUDED) || GetEnemy() == NULL || GetEnemy()->IsAlive() == false) &&
...
bool CNPC_Vortigaunt::InnateWeaponLOSCondition(const Vector &ownerPos, const Vector &targetPos, bool bSetConditions)
{
	...
	NDebugOverlay::HorzArrow( GetEnemy()->BodyTarget( vecNewOwnerPos ), vecFinalTargetPos, 12.0f, 255, 0, 0, 16.0f,
	true, 3.0f ); NDebugOverlay::HorzArrow( vecNewOwnerPos, GetEnemy()->BodyTarget( vecNewOwnerPos ), 8.0f, 255, 255,
	0, 32.0f, true, 3.0f );
	*/
...
int CNPC_Vortigaunt::RangeAttack1Conditions(float flDot, float flDist)
{
	...
	if(IsAntlionWorker(GetEnemy()))
	{
		...
		CBasePlayer *pPlayer = AI_GetSinglePlayer();
		if(pPlayer &&
		   (pPlayer->GetAbsOrigin() - GetEnemy()->GetAbsOrigin()).LengthSqr() < Square(AntlionWorkerBurstRadius()))
		{
...
int CNPC_Vortigaunt::NumAntlionsInRadius(float flRadius)
{
	...
	int nNumAntlions = 0;
	int nNumEnemies =
		UTIL_EntitiesInBox(sEnemySearch, ARRAYSIZE(sEnemySearch), GetAbsOrigin() - Vector(flRadius, flRadius, flRadius),
			...
			if(pWeapon)
			{
				...
				for(; i < iNumAttempts; i++)
				{
					...
					if(tr.fraction == 1.0 && !tr.m_pEnt)
					{
						...
						AI_TraceHull(vecSpawnOrigin, vecSpawnOrigin, -Vector(16, 16, 16), Vector(16, 16, 48), MASK_SHOT,
									 &traceFilter, &tr);
						if(tr.fraction == 1.0 && !tr.m_pEnt)
							break;
					}

					// NDebugOverlay::Box( vecSpawnOrigin, pWeapon->WorldAlignMins(), pWeapon->WorldAlignMins(),
					// 255,0,0, 64, 100 );
...
Activity CNPC_Vortigaunt::NPC_TranslateActivity(Activity eNewActivity)
{
	...
	if(IsCarryingNPC())
	{
		...
		if(eNewActivity == ACT_WALK || eNewActivity == ACT_WALK_AIM || eNewActivity == ACT_RUN ||
...
int CNPC_Vortigaunt::TranslateSchedule(int scheduleType)
{
	switch(scheduleType)
	{
		...
		case SCHED_RANGE_ATTACK1:
			...
			// If we're told to fire when we're already firing, just face our target.  If we don't do this, we get a
			// bizarre double-shot
			if(IsPlayingGesture(ACT_GESTURE_RANGE_ATTACK1))
				return SCHED_COMBAT_FACE;
...
int CNPC_Vortigaunt::SelectHealSchedule(void)
{
	...
	if(m_hHealTarget != NULL)
	{
		...
		if(PlayerBelowHealthPercentage(pPlayer, PLAYER_CRITICAL_HEALTH_PERC) == false &&
		...
		if(m_eHealState != HEAL_STATE_HEALING && m_eHealState != HEAL_STATE_WARMUP &&
		   HasCondition(COND_VORTIGAUNT_HEAL_VALID))
			return SCHED_VORTIGAUNT_HEAL;

		// If the player is too far away or blocked, give chase
		if(HasCondition(COND_VORTIGAUNT_HEAL_TARGET_TOO_FAR) || HasCondition(COND_VORTIGAUNT_HEAL_TARGET_BLOCKED))
...
bool CNPC_Vortigaunt::FInViewCone(CBaseEntity *pEntity)
{
	...
	if((pEntity->IsNPC() || pEntity->IsPlayer()) &&
	   pEntity->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= VORT_360_VIEW_DIST_SQR)
		return true;
		...
		if(HasCondition(COND_VORTIGAUNT_HEAL_VALID))
		{
			if(m_flNextHealTokenTime < gpGlobals->curtime)
			{
				...
				if(m_nNumTokensToSpawn <= 0 ||
				   (m_bForceArmorRecharge == false &&
					(pPlayer && pPlayer->ArmorValue() >= sk_vortigaunt_armor_charge.GetInt())))
				{
					m_flHealHinderedTime = 0.0f;
					m_nNumTokensToSpawn = 0;
...
bool CNPC_Vortigaunt::OverrideMoveFacing(const AILocalMoveGoal_t &move, float flInterval)
{
	...
	if(IsPlayingGesture(nActivity) || IsCurSchedule(SCHED_PC_MOVE_TOWARDS_COVER_FROM_BEST_SOUND) ||
	   IsCurSchedule(SCHED_VORT_FLEE_FROM_BEST_SOUND) || IsCurSchedule(SCHED_TAKE_COVER_FROM_BEST_SOUND))
	{
...
void CNPC_Vortigaunt::ArmBeam(int beamType, int nHand)
{
	...
	for(int i = 0; i < 3; i++)
	{
		Vector vecAim = forward * random->RandomFloat(-1, 1) + right * side * random->RandomFloat(0, 1) +
...
void CNPC_Vortigaunt::StartHandGlow(int beamType, int nHand)
{
	...
	// turned back on in the same frame due to how animations are applied and anim events are executed after the AI
	// frame.
	if(m_fGlowChangeTime > gpGlobals->curtime)
		return;
	...
	switch(beamType)
	{
		...
		case VORTIGAUNT_BEAM_ZAP:
		{
			...
			// Stomp our settings
			m_hHandEffect[nHand]->SetParent(this,
...
void CNPC_Vortigaunt::GatherHealConditions(void)
{
	...
	if(m_bArmorRechargeEnabled == false || HasCondition(COND_NEW_ENEMY) || HasCondition(COND_HEAR_DANGER) ||
	   HasCondition(COND_HEAVY_DAMAGE))
	{
...
void CNPC_Vortigaunt::DispelAntlions(const Vector &vecOrigin, float flRadius, bool bDispel /*= true*/)
{
	...
	int nNumEnemies =
		UTIL_EntitiesInBox(pEnemySearch, ARRAYSIZE(pEnemySearch), vecOrigin - Vector(flRadius, flRadius, flRadius),
	...
	for(int i = 0; i < nNumEnemies; i++)
	{
		...
		if(pAntlion->IsWorker() == false)
		{
			...
			UTIL_TraceLine(vecOrigin, pAntlion->BodyTarget(vecOrigin), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
						   &tr);
...
int CNPC_Vortigaunt::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	if(Classify() == CLASS_PLAYER_ALLY_VITAL)
	{
		...
		if(info.GetAttacker()->Classify() == CLASS_ANTLION && (info.GetDamageType() & (DMG_ACID | DMG_POISON)) != 0)
		{
...
AI_BEGIN_CUSTOM_NPC(npc_vortigaunt, CNPC_Vortigaunt)
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_VORTIGAUNT_HEAL,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_VORTIGAUNT_STAND,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_VORTIGAUNT_RUN_TO_PLAYER,
	...
	//
	//=========================================================
	DEFINE_SCHEDULE(SCHED_VORT_FLEE_FROM_BEST_SOUND,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_VORT_ALERT_FACE_BESTSOUND,
...
BEGIN_DATADESC(CVortigauntChargeToken)
	DEFINE_FIELD(m_hTarget, FIELD_EHANDLE), DEFINE_FIELD(m_flLifetime, FIELD_TIME),
...
//-----------------------------------------------------------------------------
CVortigauntChargeToken *CVortigauntChargeToken::CreateChargeToken(const Vector &vecOrigin, CBaseEntity *pOwner,
																  CBaseEntity *pTarget)
{
...
void CVortigauntChargeToken::SeekTouch(CBaseEntity *pOther)
{
	...
	{
		pPlayer->IncrementArmorValue(sk_vortigaunt_armor_charge_per_token.GetInt() + random->RandomInt(-1, 1),
...
//			*pTarget - Who we're seeking towards
//-----------------------------------------------------------------------------
CVortigauntEffectDispel *CVortigauntEffectDispel::CreateEffectDispel(const Vector &vecOrigin, CBaseEntity *pOwner,
																	 CBaseEntity *pTarget)
{
...
BEGIN_DATADESC(CFleshEffectTarget)
	...
	DEFINE_FIELD(m_flScaleTime, FIELD_TIME), DEFINE_KEYFIELD(m_flRadius, FIELD_FLOAT, "radius"),
...
SendPropFloat(SENDINFO(m_flRadius), 0, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_flScaleTime), 0, SPROP_NOSCALE),
END_SEND_TABLE
()
