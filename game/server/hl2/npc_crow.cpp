...
BEGIN_DATADESC(CNPC_Crow)
	...
	DEFINE_FIELD(m_flGroundIdleMoveTime, FIELD_TIME), DEFINE_FIELD(m_bOnJeep, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flEnemyDist, FIELD_FLOAT), DEFINE_FIELD(m_nMorale, FIELD_INTEGER),
		DEFINE_FIELD(m_bReachedMoveGoal, FIELD_BOOLEAN), DEFINE_FIELD(m_flHopStartZ, FIELD_FLOAT),
		DEFINE_FIELD(m_vDesiredTarget, FIELD_VECTOR), DEFINE_FIELD(m_vCurrentTarget, FIELD_VECTOR),
		DEFINE_FIELD(m_flSoarTime, FIELD_TIME), DEFINE_FIELD(m_bSoar, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bPlayedLoopingSound, FIELD_BOOLEAN), DEFINE_FIELD(m_iBirdType, FIELD_INTEGER),
		DEFINE_FIELD(m_vLastStoredOrigin, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_flLastStuckCheck, FIELD_TIME),
		DEFINE_FIELD(m_flDangerSoundTime, FIELD_TIME), DEFINE_KEYFIELD(m_bIsDeaf, FIELD_BOOLEAN, "deaf"),
...
void CNPC_Crow::MoveCrowFly(float flInterval)
{
	...
	if(GetHintNode())
	{
		...
		GetMoveProbe()->MoveLimit(NAV_FLY, GetAbsOrigin(), GetNavigator()->GetCurWaypointPos(), MASK_NPCSOLID,
...
bool CNPC_Crow::Probe(const Vector &vecMoveDir, float flSpeed, Vector &vecDeflect)
{
	...
	//
	trace_t tr;
	AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + vecMoveDir * flSpeed, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID,
				 this, HL2COLLISION_GROUP_CROW, &tr);
	if(tr.fraction < 1.0f)
	{
		//
...
void CNPC_Crow::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_FIND_HINTNODE:
		{
			...
			if(!GetHintNode())
			{
				SetHintNode(CAI_HintManager::FindHint(this, HINT_CROW_FLYTO_POINT,
		...
		case TASK_GET_PATH_TO_HINTNODE:
		{
			...
			if(GetHintNode())
			{
				...
				SetNavType(NAV_FLY);
				CapabilitiesAdd(bits_CAP_MOVE_FLY);
				// @HACKHACK: Force allow triangulation. Too many HL2 maps were relying on this feature WRT fly nodes
				// (toml 8/1/2007)
		...
		{
			m_vSavePosition =
				GetLocalOrigin() + Vector(random->RandomFloat(-48.0f, 48.0f), random->RandomFloat(-48.0f, 48.0f), 0);
			TaskComplete();
			break;
		}
...
void CNPC_Crow::RunTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		case TASK_CROW_TAKEOFF:
		{
			if(GetNavigator()->IsGoalActive())
			{
				GetMotor()->SetIdealYawToTargetAndUpdate(GetAbsOrigin() + GetNavigator()->GetCurWaypointPos(),
														 AI_KEEP_YAW_SPEED);
			}
			else
				TaskFail(FAIL_NO_ROUTE);
		...
		case TASK_CROW_HOP:
		{
			...
			if((GetAbsOrigin().z < m_flHopStartZ) && (!(GetFlags() & FL_ONGROUND)))
			{
				...
				//
				trace_t tr;
				AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -32), MASK_SOLID, this,
...
int CNPC_Crow::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		if(GetEnemy() != NULL)
		{
			Q_snprintf(tempstr, sizeof(tempstr), "enemy (dist): %s (%g)", GetEnemy()->GetClassname(),
					   (double)m_flEnemyDist);
			EntityText(nOffset, tempstr, 0);
			nOffset++;
		}
	}
...
AI_BEGIN_CUSTOM_NPC(npc_crow, CNPC_Crow)
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_CROW_IDLE_WALK,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_CROW_WALK_AWAY,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_CROW_RUN_AWAY,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_CROW_IDLE_FLY,
