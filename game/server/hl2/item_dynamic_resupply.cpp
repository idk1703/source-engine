...
ConVar g_debug_dynamicresupplies("g_debug_dynamicresupplies", "0", FCVAR_NONE,
								 "Debug item_dynamic_resupply spawning. Set to 1 to see text printouts of the "
								 "spawning. Set to 2 to see lines drawn to other items factored into the spawning.");
...
private:
	...
	bool SpawnItemFromRatio(int nCount, DynamicResupplyItems_t *pItems, int iDebug, SpawnInfo_t *pSpawnInfo,
...
BEGIN_DATADESC(CItem_DynamicResupply)
	...
	DEFINE_THINKFUNC(CheckPVSThink),
		...
		DEFINE_FIELD(m_version, FIELD_INTEGER), DEFINE_FIELD(m_bIsMaster, FIELD_BOOLEAN),
...
void CItem_DynamicResupply::Activate(void)
{
	...
	if(!HasSpawnFlags(SF_DYNAMICRESUPPLY_ALTERNATE_MASTER) && HasSpawnFlags(SF_DYNAMICRESUPPLY_USE_MASTER) &&
...
// Purpose:
//-----------------------------------------------------------------------------
void CItem_DynamicResupply::FindPotentialItems(int nCount, DynamicResupplyItems_t *pItems, int iDebug,
...
// Purpose:
//-----------------------------------------------------------------------------
void CItem_DynamicResupply::ComputeHealthRatios(CItem_DynamicResupply *pMaster, CBasePlayer *pPlayer, int iDebug,
												SpawnInfo_t *pSpawnInfo)
{
		...
		else if(i == DS_ARMOR_INDEX)
		{
			...
			else
			{
				...
				float flCurrentArmor =
					pPlayer->ArmorValue() + (pSpawnInfo[i].m_iPotentialItems * sk_battery.GetFloat());
	...
	if(iDebug)
	{
		...
		for(int i = 0; i < NUM_HEALTH_ITEMS; i++)
		{
			Msg("   %s Desired Ratio: %.2f, Current Ratio: %.2f = Delta of %.2f\n",
				g_DynamicResupplyHealthItems[i].sEntityName, pSpawnInfo[i].m_flDesiredRatio,
				pSpawnInfo[i].m_flCurrentRatio, pSpawnInfo[i].m_flDelta);
		}
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CItem_DynamicResupply::ComputeAmmoRatios(CItem_DynamicResupply *pMaster, CBasePlayer *pPlayer, int iDebug,
											  SpawnInfo_t *pSpawnInfo)
{
	...
	if(iDebug)
	{
		...
		{
			Msg("   %s Desired Ratio: %.2f, Current Ratio: %.2f = Delta of %.2f\n",
				g_DynamicResupplyAmmoItems[i].sEntityName, pSpawnInfo[i].m_flDesiredRatio,
				pSpawnInfo[i].m_flCurrentRatio, pSpawnInfo[i].m_flDelta);
		}
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CItem_DynamicResupply::SpawnItemFromRatio(int nCount, DynamicResupplyItems_t *pItems, int iDebug,
											   SpawnInfo_t *pSpawnInfo, Vector *pVecSpawnOrigin)
{
...
void CItem_DynamicResupply::SpawnDynamicItem(CBasePlayer *pPlayer)
{
	...
	bool bHealthSpawned =
		SpawnItemFromRatio(NUM_HEALTH_ITEMS, g_DynamicResupplyHealthItems, iDebug, pHealthInfo, &vecSpawnOrigin);
	bool bAmmoSpawned =
		SpawnItemFromRatio(NUM_AMMO_ITEMS, g_DynamicResupplyAmmoItems, iDebug, pAmmoInfo, &vecSpawnOrigin);
