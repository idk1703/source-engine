...
BEGIN_DATADESC(CAntlionTemplateMaker)
	...
	DEFINE_KEYFIELD(m_strSpawnGroup, FIELD_STRING, "spawngroup"),
		...
		DEFINE_FIELD(m_nChildMoveState, FIELD_INTEGER), DEFINE_FIELD(m_hFightTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_hProxyTarget, FIELD_EHANDLE), DEFINE_FIELD(m_hFollowTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_iSkinCount, FIELD_INTEGER), DEFINE_FIELD(m_flBlockedBumpTime, FIELD_TIME),
...
void CAntlionTemplateMaker::Activate(void)
{
	...
	if(m_iMaxPool)
	{
		...
		if(m_bDisabled == false && gpGlobals->eLoadType != MapLoad_LoadGame)
		{
			...
			SetContextThink(&CAntlionTemplateMaker::PoolRegenThink, gpGlobals->curtime + m_flPoolRegenTime,
							s_pPoolThinkContext);

			// Start our blocked effects cycle
			if(hl2_episodic.GetBool() == true && HasSpawnFlags(SF_ANTLIONMAKER_DO_BLOCKEDEFFECTS))
			{
				SetContextThink(&CAntlionTemplateMaker::FindNodesCloseToPlayer, gpGlobals->curtime + 1.0f,
...
bool CAntlionTemplateMaker::FindPositionOnVehicle(Vector &origin, float radius, CBaseEntity *pTarget)
{
	...
	while(iMaxTries > 0)
	{
		...
		float flSpeed =
			(pTarget->GetSmoothedVelocity().Length() * m_flVehicleSpawnDistance) * random->RandomFloat(1.0f, 1.5f);
...
{
	trace_t tr;
	UTIL_TraceLine(vOrigin, vOrigin - Vector(0, 0, 1024), MASK_BLOCKLOS | CONTENTS_WATER, NULL, COLLISION_GROUP_NONE,
				   &tr);
	...
	if(tr.fraction != 1.0)
	{
		...
		trace_t trCheck;
		UTIL_TraceHull(tr.endpos, tr.endpos + Vector(0, 0, 5), NAI_Hull::Mins(HULL_MEDIUM), NAI_Hull::Maxs(HULL_MEDIUM),
					   MASK_NPCSOLID, NULL, COLLISION_GROUP_NONE, &trCheck);

		if(trCheck.DidHit() == false)
		{
			if(g_debug_antlionmaker.GetInt() == 1)
				NDebugOverlay::Box(tr.endpos + Vector(0, 0, 5), NAI_Hull::Mins(HULL_MEDIUM),
			...
			if(pTarget)
			{
				if(pTarget->IsPlayer())
				{
					...
					UTIL_TraceLine(vVerifyOrigin, tr.endpos + Vector(0, 0, flZOffset), MASK_BLOCKLOS | CONTENTS_WATER,
								   &traceFilter, &trVerify);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAntlionTemplateMaker::FindHintSpawnPosition(const Vector &origin, float radius, string_t hintGroupName,
												  CAI_Hint **pHint, bool bRandom)
{
	...
	if(pChosenHint != NULL)
	{
		...
		if(AllHintsFromClusterBlocked(pChosenHint, bChosenHintBlocked))
		{
			...
			{
				SetContextThink(&CAntlionTemplateMaker::BlockedCheckFunc, gpGlobals->curtime + 2.0f,
		...
		for(int i = 0; i < hintList.Count(); i++)
		{
			...
			if(pTestHint)
			{
				if(pTestHint->NameMatches(pNode->GetEntityName()))
				{
					...
					int count = UTIL_EntitiesInBox(pList, 20, spawnOrigin + NAI_Hull::Mins(HULL_MEDIUM),
					...
					if(g_debug_antlionmaker.GetInt() == 1)
					{
						if(bBlocked)
						{
							NDebugOverlay::Box(spawnOrigin + Vector(0, 0, 5), NAI_Hull::Mins(HULL_MEDIUM),
...
void CAntlionTemplateMaker::FindNodesCloseToPlayer(void)
{
	SetContextThink(&CAntlionTemplateMaker::FindNodesCloseToPlayer,
					gpGlobals->curtime + random->RandomFloat(0.75, 1.75), s_pBlockedEffectsThinkContext);
	...
	for(int i = 0; i < hintList.Count(); i++)
	{
		...
		if(pNode && pNode->HintMatchesCriteria(NULL, hintCriteria, pPlayer->GetAbsOrigin(), &flRadius))
		{
			...
			// Have one of the nodes from this cluster been checked for blockage? If so then there's no need to do block
			// checks again for this cluster.
	...
	if(CAI_HintManager::FindAllHints(vec3_origin, hintCriteria, &hintList) > 0)
	{
		for(int i = 0; i < hintList.Count(); i++)
		{
			...
			if(pNode)
			{
				...
				CBaseEntity *pList[20];
				int count = UTIL_EntitiesInBox(pList, 20, vHintPos + NAI_Hull::Mins(HULL_MEDIUM),
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		if(m_strSpawnGroup != NULL_STRING)
		{
			...
			while(pHint != NULL)
			{
				...
				NDebugOverlay::VertArrow(GetAbsOrigin(), pHint->GetAbsOrigin() + Vector(0, 0, 32), 8.0f, r, g, b, 0,
										 true, 0.05f);

				// Draw a box to represent where it's sitting
				Vector vecForward;
				AngleVectors(pHint->GetAbsAngles(), &vecForward);
				NDebugOverlay::BoxDirection(pHint->GetAbsOrigin(), -Vector(32, 32, 0), Vector(32, 32, 16), vecForward,
											r, g, b, true, 0.05f);
		...
		if(m_strSpawnTarget != NULL_STRING)
		{
			...
			if(pTarget != NULL)
			{
				NDebugOverlay::VertArrow(GetAbsOrigin(), pTarget->WorldSpaceCenter(), 4.0f, 255, 255, 255, 0, true,
										 0.05f);
			}
		}
		...
		if(m_strFollowTarget != NULL_STRING)
		{
			...
			if(pTarget != NULL)
			{
				NDebugOverlay::VertArrow(GetAbsOrigin(), pTarget->WorldSpaceCenter(), 4.0f, 255, 255, 0, 0, true,
										 0.05f);
			}
		}
