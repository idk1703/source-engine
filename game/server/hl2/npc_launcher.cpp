...
BEGIN_DATADESC(CNPC_Launcher)
	...
	DEFINE_KEYFIELD(m_nStartOn, FIELD_INTEGER, "StartOn"),
		...
		DEFINE_KEYFIELD(m_flSpinSpeed, FIELD_FLOAT, "SpinSpeed"), DEFINE_KEYFIELD(m_flDamage, FIELD_FLOAT, "Damage"),
		DEFINE_KEYFIELD(m_flDamageRadius, FIELD_FLOAT, "DamageRadius"), DEFINE_FIELD(m_flMissileGravity, FIELD_FLOAT),
...
void CNPC_Launcher::LaunchGrenade(CBaseEntity *pEnemy)
{
	...
	if(m_sPathCornerName != NULL_STRING)
	{
		CGrenadePathfollower *pGrenade = CGrenadePathfollower::CreateGrenadePathfollower(
	...
	else
	{
		...
		CGrenadeHomer *pGrenade =
			CGrenadeHomer::CreateGrenadeHomer(m_sMissileModel, m_sFlySound, GetAbsOrigin(), vec3_angle, edict());
		pGrenade->Spawn();
		pGrenade->SetSpin(m_flSpinMagnitude, m_flSpinSpeed);
		pGrenade->SetHoming((0.01 * m_nHomingStrength), m_flHomingDelay, m_flHomingRampUp, m_flHomingDuration,
							m_flHomingRampDown);
		pGrenade->SetDamage(m_flDamage);
		pGrenade->SetDamageRadius(m_flDamageRadius);
