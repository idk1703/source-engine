...
private:
	bool SteerAvoidObstacles(Vector &Steer, const Vector &Velocity, const Vector &Forward, const Vector &Right,
							 const Vector &Up);
	bool Beached(void);
...
BEGIN_DATADESC(CNPC_Ichthyosaur)
	...
	DEFINE_FIELD(m_pVictim, FIELD_CLASSPTR), DEFINE_FIELD(m_vecLastMoveTarget, FIELD_VECTOR),
		DEFINE_FIELD(m_flNextBiteTime, FIELD_FLOAT), DEFINE_FIELD(m_flHoldTime, FIELD_FLOAT),
		DEFINE_FIELD(m_flSwimSpeed, FIELD_FLOAT), DEFINE_FIELD(m_flTailYaw, FIELD_FLOAT),
		DEFINE_FIELD(m_flTailPitch, FIELD_FLOAT), DEFINE_FIELD(m_flNextPingTime, FIELD_FLOAT),
		DEFINE_FIELD(m_flNextGrowlTime, FIELD_FLOAT), DEFINE_FIELD(m_bHasMoveTarget, FIELD_BOOLEAN),
...
void CNPC_Ichthyosaur::Spawn(void)
{
	...
	// m_pSwimSound	= ENVELOPE_CONTROLLER.SoundCreate( edict(), CHAN_BODY,
	// "xxxCONVERTTOGAMESOUNDS!!!npc/ichthyosaur/ich_amb1wav", ATTN_NORM ); m_pVoiceSound	=
	// ENVELOPE_CONTROLLER.SoundCreate( edict(), CHAN_VOICE,	"xxxCONVERTTOGAMESOUNDS!!!npc/ichthyosaur/water_breathwav",
	// ATTN_IDLE );
...
//-----------------------------------------------------------------------------
bool CNPC_Ichthyosaur::SteerAvoidObstacles(Vector &Steer, const Vector &Velocity, const Vector &Forward,
										   const Vector &Right, const Vector &Up)
{
	...
	AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + (dir * speed), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this,
	...
	AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -ICH_HEIGHT_PREFERENCE), MASK_NPCSOLID_BRUSHONLY, this,
				 COLLISION_GROUP_NONE, &tr);
	...
	{
		float waterLevel = (UTIL_WaterLevel(GetAbsOrigin(), GetAbsOrigin().z, GetAbsOrigin().z + ICH_DEPTH_PREFERENCE) -
							GetAbsOrigin().z) /
						   ICH_DEPTH_PREFERENCE;
...
void CNPC_Ichthyosaur::MoveFlyExecute(CBaseEntity *pTargetEnt, const Vector &vecDir, float flDistance, float flInterval)
{
	...
	if((GetEnemy() != NULL) && (GetNavigator()->GetGoalTarget() == (CBaseEntity *)GetEnemy()))
	{
		...
		AI_TraceHull(GetAbsOrigin(), goalPos, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, GetEnemy(),
					 COLLISION_GROUP_NONE, &tr);
...
void CNPC_Ichthyosaur::Bite(void)
{
	...
	if(pHurt != NULL)
	{
		...
		if(pHurt->IsPlayer())
		{
			...
			if(pPlayer)
			{
				if(((m_flHoldTime < gpGlobals->curtime) && (pPlayer->m_iHealth < (pPlayer->m_iMaxHealth * 0.5f))) ||
				   (pPlayer->GetWaterLevel() < 1))
				{
					// EnsnareVictim( pHurt );
				}
				else
		...
		// Bubbles!
		UTIL_Bubbles(pHurt->GetAbsOrigin() + Vector(-32.0f, -32.0f, -32.0f),
					 pHurt->GetAbsOrigin() + Vector(32.0f, 32.0f, 0.0f), random->RandomInt(16, 32));
...
int CNPC_Ichthyosaur::TranslateSchedule(int type)
{
	...
	if(type == SCHED_PATROL_RUN)
		return SCHED_ICH_PATROL_RUN;
	if(type == SCHED_PATROL_WALK)
		return SCHED_ICH_PATROL_WALK;
	if(type == SCHED_MELEE_ATTACK1)
		return SCHED_ICH_MELEE_ATTACK1;
...
float CNPC_Ichthyosaur::MaxYawSpeed(void)
{
	...
	return ICH_MIN_TURN_SPEED +
		   ((ICH_MAX_TURN_SPEED - ICH_MIN_TURN_SPEED) * (fabs(GetAbsVelocity().Length()) / ICH_SWIM_SPEED_RUN));
}
...
AI_DEFINE_SCHEDULE(SCHED_ICH_CHASE_ENEMY,
...
AI_DEFINE_SCHEDULE(SCHED_ICH_PATROL_RUN,
...
AI_DEFINE_SCHEDULE(SCHED_ICH_PATROL_WALK,
...
AI_DEFINE_SCHEDULE(SCHED_ICH_DROWN_VICTIM,
...
AI_DEFINE_SCHEDULE(SCHED_ICH_MELEE_ATTACK1,
...
AI_DEFINE_SCHEDULE(SCHED_ICH_THRASH,
