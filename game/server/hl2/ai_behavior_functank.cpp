...
BEGIN_DATADESC(CAI_FuncTankBehavior)
	DEFINE_FIELD(m_hFuncTank, FIELD_EHANDLE), DEFINE_FIELD(m_bMounted, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flBusyTime, FIELD_TIME), DEFINE_FIELD(m_bSpottedPlayerOutOfCover, FIELD_BOOLEAN),
END_DATADESC();
...
CBaseEntity *CAI_FuncTankBehavior::BestEnemy(void)
{
		...
		pEMemory = GetEnemies()->GetNext(&iter))
	{
		...
		if((pEnemy->GetFlags() & FL_NOTARGET) ||
		   (pNPC->IRelationType(pEnemy) != D_HT && pNPC->IRelationType(pEnemy) != D_FR) || !IsValidEnemy(pEnemy))
			continue;
		...
		if(bBestUnreachable && !bUnreachable)
		{
			bBestSeen = (pNPC->GetSenses()->DidSeeEntity(pEnemy) ||
						 pNPC->FVisible(pEnemy)); // @TODO (toml 04-02-03): Need to optimize CanSeeEntity() so multiple
												  // calls in frame do not recalculate, rather cache
		...
		else if(pNPC->IRelationPriority(pEnemy) == iBestPriority)
		{
			...
			{
				// @TODO (toml 04-02-03): Need to optimize FVisible() so multiple calls in frame do not recalculate,
				// rather cache
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_FuncTankBehavior)
	...
	//=========================================================
	//=========================================================
	DEFINE_SCHEDULE(SCHED_MOVE_TO_FUNCTANK,
	...
	//=========================================================
	//=========================================================
	DEFINE_SCHEDULE(SCHED_FIRE_FUNCTANK,
	...
	DEFINE_SCHEDULE(SCHED_SCAN_WITH_FUNCTANK,
