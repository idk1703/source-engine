...
BEGIN_DATADESC(CNPC_BaseScanner)
	DEFINE_EMBEDDED(m_KilledInfo), DEFINE_SOUNDPATCH(m_pEngineSound),
		...
		DEFINE_FIELD(m_flFlyNoiseBase, FIELD_FLOAT), DEFINE_FIELD(m_flEngineStallTime, FIELD_TIME),
		DEFINE_FIELD(m_fNextFlySoundTime, FIELD_TIME), DEFINE_FIELD(m_nFlyMode, FIELD_INTEGER),
		...
		DEFINE_FIELD(m_flAttackNearDist, FIELD_FLOAT), DEFINE_FIELD(m_flAttackFarDist, FIELD_FLOAT),
		DEFINE_FIELD(m_flAttackRange, FIELD_FLOAT),

		DEFINE_FIELD(m_nPoseTail, FIELD_INTEGER), DEFINE_FIELD(m_nPoseDynamo, FIELD_INTEGER),
		DEFINE_FIELD(m_nPoseFlare, FIELD_INTEGER), DEFINE_FIELD(m_nPoseFaceVert, FIELD_INTEGER),
	...
	switch(pTask->iTask)
	{
		...
		case TASK_SCANNER_SET_FLY_DIVE:
		{
			...
			else
			{
				...
				m_vecDiveBombDirection = forward + Vector(random->RandomFloat(-10, 10), random->RandomFloat(-10, 10),
...
#define SCANNER_SMASH_TIME \
	0.75 // How long after being thrown from a physcannon that a manhack is eligible to die from impact
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_BaseScanner::IdealGoalForMovement(const Vector &goalPos, const Vector &startPos, float idealRange,
											  float idealHeightDiff)
{
	...
	trace_t tr;
	AI_TraceHull(vIdealPos, vIdealPos - Vector(0, 0, MinGroundDist()), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID,
	...
	if(g_debug_basescanner.GetBool())
	{
		...
		NDebugOverlay::Cross3D(goalPos + (vMoveDir * -idealRange), -Vector(8, 8, 8), Vector(8, 8, 8), 255, 255, 255,
							   true, 0.1f);
...
void CNPC_BaseScanner::DiveBombSoundThink()
{
	...
	if(pPlayer)
	{
		...
		if(DotProduct(vecDelta, vecVelocity) > 0.5f)
		{
			...
			if(flDist < 200.0f)
			{
				...
				SetContextThink(&CNPC_BaseScanner::DiveBombSoundThink, gpGlobals->curtime + 0.5f,
	...
	SetContextThink(&CNPC_BaseScanner::DiveBombSoundThink, gpGlobals->curtime + 2.0f * TICK_INTERVAL,
					s_pDiveBombSoundThinkContext);
}
