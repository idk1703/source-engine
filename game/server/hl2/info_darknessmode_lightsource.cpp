...
public:
	...
	void DebugThink(void)
	{
		...
		int textoffset = 0;
		EntityText(textoffset,
				   UTIL_VarArgs("Org: %.2f %.2f %.2f", GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z), 0.1);
		textoffset++;
...
BEGIN_DATADESC(CInfoDarknessLightSource)
	DEFINE_KEYFIELD(m_flLightRadius, FIELD_FLOAT, "LightRadius"), DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
		DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),
		DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"), DEFINE_FIELD(m_bIgnoreLOS, FIELD_BOOLEAN),
	...
	for(int i = m_LightSources.Count() - 1; i >= 0; i--)
	{
		...
		if(flDistanceSqr < m_LightSources[i].flLightRadiusSqr)
		{
			if(pLightSource->ShouldIgnoreLOS())
			{
				if(bDebug)
				{
					NDebugOverlay::Line(pTarget->WorldSpaceCenter(), pLightSource->GetAbsOrigin(), 0, 255, 0, true,
										0.1);
				}
				return true;
			}
		...
		if(flDot > 0)
		{
			...
			if(flDistToTarget < flDistToSource)
			{
				if(bDebug)
				{
					NDebugOverlay::Line(pLooker->WorldSpaceCenter(),
										pLooker->WorldSpaceCenter() + (vecLookerToLight * 128), 255, 255, 255, true,
										0.1);
					NDebugOverlay::Line(pLooker->WorldSpaceCenter(),
										pLooker->WorldSpaceCenter() + (vecLookerToTarget * 128), 255, 0, 0, true, 0.1);
				}
...
bool CDarknessLightSourcesSystem::AreThereLightSourcesWithinRadius(CBaseEntity *pLooker, float flRadius)
{
	...
	for(int i = m_LightSources.Count() - 1; i >= 0; i--)
	{
		...
		{
			trace_t tr;
			AI_TraceLine(pLooker->EyePosition(), pLightSource->GetAbsOrigin(), MASK_SOLID_BRUSHONLY, pLooker,
...
void CDarknessLightSourcesSystem::SetDebug(bool bDebug)
{
	for(int i = m_LightSources.Count() - 1; i >= 0; i--)
	{
		CInfoDarknessLightSource *pLightSource =
			dynamic_cast<CInfoDarknessLightSource *>(m_LightSources[i].hEntity.Get());
		if(pLightSource)
		{
			if(bDebug)
			{
...
void AddEntityToDarknessCheck(CBaseEntity *pEntity, float flLightRadius /*=DARKNESS_LIGHTSOURCE_SIZE*/)
{
	...
	CInfoDarknessLightSource *pLightSource =
		(CInfoDarknessLightSource *)CreateEntityByName("info_darknessmode_lightsource");
	if(pLightSource)
	{
