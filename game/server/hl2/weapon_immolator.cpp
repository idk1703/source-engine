...
BEGIN_DATADESC(CWeaponImmolator)
	...
	DEFINE_FIELD(m_beamIndex, FIELD_INTEGER), DEFINE_FIELD(m_flBurnRadius, FIELD_FLOAT),
		DEFINE_FIELD(m_flTimeLastUpdatedRadius, FIELD_TIME), DEFINE_FIELD(m_vecImmolatorTarget, FIELD_VECTOR),
...
void CWeaponImmolator::Update()
{
	...
	UTIL_Beam(vecSrc, tr.endpos, m_beamIndex,
			  0,	// halo index
			  0,	// frame start
			  2.0f, // framerate
			  0.1f, // life
	...
	if(tr.DidHitWorld())
	{
		...
		for(beams = 0; beams < 5; beams++)
		{
			...
			UTIL_Beam(tr.endpos, vecDest, m_beamIndex,
					  0,	 // halo index
					  0,	 // frame start
					  2.0f,	 // framerate
					  0.15f, // life
...
void CWeaponImmolator::ImmolationDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius,
										int iClassIgnore)
{
