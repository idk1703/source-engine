...
void CGrenadeBugBait::BugBaitTouch(CBaseEntity *pOther)
{
	...
	if(ActivateBugbaitTargets(GetThrower(), GetAbsOrigin(), false) == false)
	{
		...
		CSoundEnt::InsertSound(SOUND_BUGBAIT, GetAbsOrigin(), bugbait_hear_radius.GetInt(),
							   bugbait_distract_time.GetFloat(), GetThrower());
	}
...
bool CGrenadeBugBait::ActivateBugbaitTargets(CBaseEntity *pOwner, Vector vecOrigin, bool bSqueezed)
{
	...
	Vector delta(bugbait_grenade_radius.GetFloat(), bugbait_grenade_radius.GetFloat(),
				 bugbait_grenade_radius.GetFloat());
...
// Output : CBaseGrenade
//-----------------------------------------------------------------------------
CGrenadeBugBait *BugBaitGrenade_Create(const Vector &position, const QAngle &angles, const Vector &velocity,
