...
public:
	...
	void Disable(void);
	void ForceActBusy(CAI_ActBusyGoal *pGoal, CAI_Hint *pHintNode = NULL, float flMaxTime = NO_MAX_TIME,
					  bool bVisibleOnly = false, bool bTeleportToBusy = false, bool bUseNearestBusy = false,
					  CBaseEntity *pSeeEntity = NULL, Activity activity = ACT_INVALID);
...
private:
	...
	int m_iNumConsecutivePathFailures; // Count how many times we failed to find a path to a node, so we can consider
									   // teleporting.
...
protected:
	CAI_ActBusyBehavior *GetBusyBehaviorForNPC(const char *pszActorName, CBaseEntity *pActivator, CBaseEntity *pCaller,
