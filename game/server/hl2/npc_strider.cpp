...
extern void CreateConcussiveBlast(const Vector &origin, const Vector &surfaceNormal, CBaseEntity *pOwner,
								  float magnitude);
...
#define STRIDER_SHOOT_ON_TARGET_TIME \
	0.5 // How much of DEFAULT_SHOOT_DURATION is spent on-target (vs. stitching up to a target)
...
BEGIN_DATADESC(CNPC_Strider)
	...
	DEFINE_UTLVECTOR(m_hAttachedBusters, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_miniGunAmmo, FIELD_INTEGER), DEFINE_FIELD(m_miniGunDirectAmmo, FIELD_INTEGER),
		DEFINE_FIELD(m_nextStompTime, FIELD_TIME), DEFINE_FIELD(m_nextShootTime, FIELD_TIME),
		DEFINE_FIELD(m_ragdollTime, FIELD_TIME), DEFINE_FIELD(m_miniGunShootDuration, FIELD_FLOAT),
		DEFINE_FIELD(m_aimYaw, FIELD_FLOAT), DEFINE_FIELD(m_aimPitch, FIELD_FLOAT),
		DEFINE_FIELD(m_blastHit, FIELD_VECTOR), DEFINE_FIELD(m_blastNormal, FIELD_VECTOR),
		DEFINE_FIELD(m_vecHitPos, FIELD_POSITION_VECTOR), DEFINE_AUTO_ARRAY(m_vecIKTarget, FIELD_POSITION_VECTOR),
		...
		DEFINE_FIELD(m_hRagdoll, FIELD_EHANDLE), DEFINE_FIELD(m_hCannonTarget, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_iVisibleEnemies, FIELD_INTEGER), DEFINE_FIELD(m_flTargetAcquiredTime, FIELD_FLOAT),
		DEFINE_FIELD(m_bCrouchLocked, FIELD_BOOLEAN), DEFINE_FIELD(m_bNoCrouchWalk, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bDontCrouch, FIELD_BOOLEAN), DEFINE_FIELD(m_bNoMoveToLOS, FIELD_BOOLEAN),
		...
		DEFINE_FIELD(m_prevYaw, FIELD_FLOAT), DEFINE_FIELD(m_doTurn, FIELD_FLOAT), DEFINE_FIELD(m_doLeft, FIELD_FLOAT),
		DEFINE_FIELD(m_doRight, FIELD_FLOAT), DEFINE_FIELD(m_flNextTurnAct, FIELD_TIME),

		DEFINE_FIELD(m_strTrackName, FIELD_STRING), DEFINE_FIELD(m_hFocus, FIELD_EHANDLE),
		DEFINE_FIELD(m_hSmoke, FIELD_EHANDLE),

		DEFINE_FIELD(m_flTimeLastAlertSound, FIELD_TIME), DEFINE_FIELD(m_flTimeNextHuntSound, FIELD_TIME),
		DEFINE_FIELD(m_flTimePlayerMissileDetected, FIELD_TIME), DEFINE_FIELD(m_hPlayersMissile, FIELD_EHANDLE),
		DEFINE_FIELD(m_bMinigunUseDirectFire, FIELD_BOOLEAN),

		DEFINE_FIELD(m_bUseAggressiveBehavior, FIELD_BOOLEAN),

		DEFINE_FIELD(m_bFastCrouch, FIELD_BOOLEAN), DEFINE_FIELD(m_bMinigunEnabled, FIELD_BOOLEAN),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Stand", InputStand), DEFINE_INPUTFUNC(FIELD_FLOAT, "SetHeight", InputSetHeight),
...
void CNPC_Strider::Spawn()
{
	...
	CapabilitiesAdd(bits_CAP_MOVE_FLY | bits_CAP_INNATE_RANGE_ATTACK2 | bits_CAP_INNATE_MELEE_ATTACK1 |
...
void CNPC_Strider::GatherConditions()
{
	if(AIGetNumFollowers(this, m_iszHunterClassname) == 0)
	{
		...
		if(m_PlayerFreePass.HasPass() ||
		   (!m_pMinigun->IsShooting() ||
			GetEnemy() != m_PlayerFreePass.GetPassTarget())) // no free pass when in midst of shooting at target
			m_PlayerFreePass.Update();
	}
	else
	{
		m_PlayerFreePass.Reset(strider_free_pass_after_escorts_dead.GetFloat(),
	...
	if(GetCannonTarget())
	{
		...
		{
			trace_t tr;
			UTIL_TraceLine(CannonPosition(), GetCannonTarget()->WorldSpaceCenter(), MASK_SHOT, this,
	...
	if(!m_bCrouchLocked && !m_bDontCrouch)
	{
		...
		else if(GetEnemy())
		{
			if(strider_distributed_fire.GetBool() && !IsUsingAggressiveBehavior())
			{
				...
				// and this function may be called several times per strider think, don't call this code anymore in the
				// same think when a new enemy is chosen.
				//
					...
					if(m_pMinigun->IsOnTarget(3) && !FClassnameIs(GetEnemy(), "npc_bullseye"))
					{
						if(m_iVisibleEnemies > 1)
						{
							...
							GetEnemies()->SetTimeValidEnemy(
								GetEnemy(), gpGlobals->curtime + (STRIDER_IGNORE_TARGET_DURATION * m_iVisibleEnemies));
							SetEnemy(NULL, false);
							ChooseEnemy();
						}
	...
	if(FInViewCone(pEntity) && GetWeaponLosZ(vTestPos, minZ, maxZ, GetHeightRange() * .1, pEntity, &newHeight))
	{
		...
		if(m_LowZCorrectionTimer.Expired())
		{
			...
			if(pEntity && fabs(newHeight - GetHeight()) < 12 && newHeight < GetMinHeight() + GetHeightRange() * .33)
			{
				...
				{
					if(!GetWeaponLosZ(vTestPos, minZ + GetHeightRange() * .33, maxZ, GetHeightRange() * .1, pEntity,
									  &newHeight))
					{
						return;
					}
...
void CNPC_Strider::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_STRIDER_REFRESH_HUNT_PATH:
			...
			if(GetGoalEnt())
			{
				AI_NavGoal_t goal(GOALTYPE_PATHCORNER, GetGoalEnt()->GetLocalOrigin(), ACT_WALK, AIN_DEF_TOLERANCE,
								  AIN_YAW_TO_DEST);
...
void CNPC_Strider::InputDisableCollisionWith( inputdata_t &inputdata )
{
	...
	for (idx = 0 ; pFol = GetBoneFollowerByIndex(idx) ; ++idx) // stop when the function starts returning null (idx is
no longer good)
	{
...
void CNPC_Strider::InputDisableCollisionWith(inputdata_t &inputdata)
{
	...
	for(int idx = m_BoneFollowerManager.GetNumBoneFollowers() - 1; idx >= 0;
		--idx) // stop when the function starts returning null (idx is no longer good)
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::GetWeaponLosZ(const Vector &vOrigin, float minZ, float maxZ, float increment, CBaseEntity *pTarget,
								 float *pResult)
{
	Vector vTestPos;
...
bool CNPC_Strider::ShouldExplodeFromDamage(const CTakeDamageInfo &info)
{
	...
	if(pAttacker != NULL &&
	   (FClassnameIs(pAttacker, "weapon_striderbuster") || FClassnameIs(pAttacker, "npc_grenade_magna")))
		return true;
		...
		if(pPlayer && mat_dxlevel.GetInt() > 0)
		{
			...
			if(maxRagdolls > 0)
			{
				...
				if(striderRagdolls.Count() >= maxRagdolls)
				{
					...
					{
						float distSqrCur =
							CalcSqrDistanceToAABB(striderRagdolls[i]->WorldAlignMins(),
			...
			pRagdoll =
				assert_cast<CRagdollProp *>(CreateServerRagdoll(this, m_nForceBone, info, HL2COLLISION_GROUP_STRIDER));
...
bool CNPC_Strider::OverrideMove(float flInterval)
{
	...
	else if(GetEnemy())
	{
		...
		if(!m_bCrouchLocked && !m_hCannonTarget && GetIdealHeight() < GetMaxHeight() &&
	...
	// If we're not supposed to crouch walk and we're under the threshold for what we consider crouch walking, stand
	// back up!
	if(!m_bCrouchLocked && m_bNoCrouchWalk && IsMoving() &&
	   (GetIdealHeight() <
		(GetMinHeight() + GetHeightRange() * (strider_pct_height_no_crouch_move.GetFloat() / 100.0))))
	{
...
{
	if(GetGoalEnt() && (IsCurSchedule(SCHED_IDLE_WALK) || IsCurSchedule(SCHED_ALERT_WALK) ||
...
void CNPC_Strider::VPhysicsShadowCollision(int index, gamevcollisionevent_t *pEvent)
{
	...
	if(pOther && UTIL_IsCombineBall(pOther))
	{
		...
		CTakeDamageInfo dmgInfo(pOther, pOther, vec3_origin, damagePos, (m_iMaxHealth / 3) + 1,
								DMG_BLAST | DMG_PREVENT_PHYSICS_FORCE);

		// FIXME: is there a better way for physics objects to keep track of what root entity responsible for them
		// moving?
...
bool CNPC_Strider::TestCollision(const Ray_t &ray, unsigned int mask, trace_t &trace)
{
	...
	if(IntersectRayWithBox(ray, WorldAlignMins() + GetAbsOrigin(), WorldAlignMaxs() + GetAbsOrigin(), DIST_EPSILON,
						   &trace))
	{
		trace.hitbox = 0;
...
//---------------------------------------------------------
//---------------------------------------------------------
static Vector GetAttachmentPositionInSpaceOfBone(CStudioHdr *pStudioHdr, const char *pAttachmentName,
												 int outputBoneIndex)
{
...
void CNPC_Strider::StompHit(int followerBoneIndex)
{
	...
	CBaseEntity *pRagdoll = CreateServerRagdollAttached(pNPC, vec3_origin, -1, COLLISION_GROUP_DEBRIS, pLegPhys, this,
...
{
	trace_t tr;
	AI_TraceLine(origin + Vector(0, 0, 48), origin - Vector(0, 0, 100), MASK_SOLID_BRUSHONLY, this,
				 COLLISION_GROUP_NONE, &tr);
	...
	{
		CRopeKeyframe::ShakeRopes(tr.endpos, npc_strider_shake_ropes_radius.GetFloat(),
...
bool CNPC_Strider::CNavigator::DoFindPathToPos()
{
	...
	if(BaseClass::DoFindPathToPos())
	{
		...
		if(pFirstWaypoint->IsReducible() && pFirstWaypoint->GetNext() &&
		   pFirstWaypoint->GetNext()->iNodeID != NO_NODE && pFirstWaypoint->GetNext()->NavType() == GetNavType())
		{
			// Find nearest point on the line segment of our path
			Vector vOrigin = GetOuter()->GetAbsOrigin();
			Vector vClosest;
			CalcClosestPointOnLineSegment(vOrigin, pFirstWaypoint->GetPos(), pFirstWaypoint->GetNext()->GetPos(),
										  vClosest);
			...
			// If we're seemingly beyond the waypoint and our hull is near enough to the path segment, advance to the
			// next node
				...
				if(pLastNodeWaypoint->iNodeID != NO_NODE)
				{
					...
					for(int link = 0; link < pLastNode->NumLinks(); link++)
					{
						...
						if(!(pLink->m_LinkInfo & bits_LINK_OFF) &&
						   (pLink->m_iAcceptedMoveTypes[GetHullType()] & bits_CAP_MOVE_FLY))
						{
							...
							{
								Vector vClosest;
								CalcClosestPointOnLineSegment(pLast->vecLocation, pLastNodeWaypoint->vecLocation,
					...
					if(vNewEnd == vec3_invalid)
					{
						...
						delete pLast;
							...
							pLastNodeWaypoint->GetPrev()->iNodeID != segmentDestClosest)
					{
...
bool CNPC_Strider::IsLegBoneFollower(CBoneFollower *pFollower)
{
	...
	// See if we're a leg
	if(nFollowerIndex >= STRIDER_LEFT_LEG_FOLLOWER_INDEX && nFollowerIndex <= STRIDER_BACK_UPPERLEG_FOLLOWER_INDEX)
		return true;
...
BEGIN_DATADESC_NO_BASE(CStriderMinigun)
	...
	DEFINE_FIELD(m_enable, FIELD_BOOLEAN), DEFINE_FIELD(m_minigunState, FIELD_INTEGER),
		DEFINE_FIELD(m_nextBulletTime, FIELD_TIME), DEFINE_FIELD(m_burstTime, FIELD_TIME),
		DEFINE_FIELD(m_nextTwitchTime, FIELD_TIME), DEFINE_FIELD(m_randomState, FIELD_INTEGER),
		DEFINE_FIELD(m_hTarget, FIELD_EHANDLE), DEFINE_FIELD(m_bWarnedAI, FIELD_BOOLEAN),
		DEFINE_FIELD(m_shootDuration, FIELD_TIME), DEFINE_FIELD(m_vecAnchor, FIELD_VECTOR),
		DEFINE_FIELD(m_bOverrideEnemy, FIELD_BOOLEAN), DEFINE_FIELD(m_vecLastTargetPos, FIELD_VECTOR),
		...
		DEFINE_FIELD(m_yaw.current, FIELD_FLOAT), DEFINE_FIELD(m_yaw.target, FIELD_FLOAT),
		DEFINE_FIELD(m_yaw.rate, FIELD_FLOAT), DEFINE_FIELD(m_pitch.current, FIELD_FLOAT),
		DEFINE_FIELD(m_pitch.target, FIELD_FLOAT), DEFINE_FIELD(m_pitch.rate, FIELD_FLOAT),
...
void CStriderMinigun::Think(IStriderMinigunHost *pHost, float dt)
{
	...
	else
	{
		...
		{
			StartShooting(pHost, pTargetEnt,
						  pHost->GetMinigunShootDuration() + random->RandomFloat(0, pHost->GetMinigunShootVariation()));
		}
	}
	...
	if(m_nextBulletTime <= gpGlobals->curtime && !IsPegged())
	{
		if(pTargetEnt && pTargetEnt == pHost->GetEntity()->GetEnemy())
		{
			...
			Vector vecTarget =
				pTargetEnt->BodyTarget(assert_cast<CNPC_Strider *>(pHost->GetEntity())->GetAdjustedOrigin());
			...
			if(GetTarget())
			{
				...
				if(flFactor <= 0.5 && !m_bWarnedAI)
				{
					...
					CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE,
										   pTargetEnt->EarPosition() + Vector(0, 0, 1), 120,
										   MAX(1.0, flRemainingShootTime), pHost->GetEntity());
				}
			}
		}
		else
...
AI_BEGIN_CUSTOM_NPC(npc_strider, CNPC_Strider)
	...
	DEFINE_SCHEDULE(SCHED_STRIDER_FLICKL,
					...
					"	"
					"	Interrupts")
	DEFINE_SCHEDULE(SCHED_STRIDER_FLICKR,

					"	Tasks"
					"		TASK_PLAY_SEQUENCE		ACTIVITY:ACT_STRIDER_FLICKR"
					"	"
					"	Interrupts")
	DEFINE_SCHEDULE(SCHED_STRIDER_ATTACK_CANNON_TARGET,
