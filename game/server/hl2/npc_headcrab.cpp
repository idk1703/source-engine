...
BEGIN_DATADESC(CBaseHeadcrab)
	...
	DEFINE_FIELD(m_bHidden, FIELD_BOOLEAN), DEFINE_FIELD(m_flTimeDrown, FIELD_TIME),
		DEFINE_FIELD(m_bCommittedToJump, FIELD_BOOLEAN), DEFINE_FIELD(m_vecCommittedJumpPos, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_flNextNPCThink, FIELD_TIME), DEFINE_FIELD(m_flIgnoreWorldCollisionTime, FIELD_TIME),

		DEFINE_KEYFIELD(m_bStartBurrowed, FIELD_BOOLEAN, "startburrowed"), DEFINE_FIELD(m_bBurrowed, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flBurrowTime, FIELD_TIME), DEFINE_FIELD(m_nContext, FIELD_INTEGER),
		DEFINE_FIELD(m_bCrawlFromCanister, FIELD_BOOLEAN), DEFINE_FIELD(m_bMidJump, FIELD_BOOLEAN),
...
bool CBaseHeadcrab::IsFirmlyOnGround()
{
	...
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, HEADCRAB_MAX_LEDGE_HEIGHT), MASK_NPCSOLID, this,
...
void CBaseHeadcrab::RunTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_HEADCRAB_CEILING_FALL:
		{
			...
			UTIL_TraceHull(vecPrPos, vecPrPos, GetHullMins(), GetHullMaxs(), MASK_SOLID, this, COLLISION_GROUP_NONE,
						   &tr);
...
{
	trace_t tr;
	UTIL_TraceEntity(this, GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 1), MASK_NPCSOLID, this, GetCollisionGroup(),
					 &tr);
...
void CBaseHeadcrab::GatherConditions(void)
{
	...
	if(m_bHidden)
	{
		...
		trace_t tr;
		AI_TraceHull(GetAbsOrigin(), GetAbsOrigin(), GetHullMins(), GetHullMaxs(), MASK_SHOT, this, GetCollisionGroup(),
					 &tr);
		if(tr.fraction == 1.0)
		{
		...
		if(IsHangingFromCeiling() == false)
		{
			if((ground->IsNPC() || ground->IsPlayer()))
			{
				SetCondition(COND_HEADCRAB_ILLEGAL_GROUNDENT);
					...
					(ground->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD))
			{
				SetCondition(COND_HEADCRAB_ILLEGAL_GROUNDENT);
			}
		}
...
void CBaseHeadcrab::PrescheduleThink(void)
{
	...
	if(m_bBurrowed && (eActivity != ACT_HEADCRAB_BURROW_IDLE) && (eActivity != ACT_HEADCRAB_BURROW_OUT) &&
...
void CBaseHeadcrab::BeginClimbFromCanister()
{
	...
	CAI_Hint *pHint = CAI_HintManager::FindHint(this, HINT_HEADCRAB_BURROW_POINT, 0,
												HEADCRAB_BURROW_POINT_SEARCH_RADIUS, &vecSearchCenter);
...
void CBaseHeadcrab::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		{
			trace_t tr;
			UTIL_TraceHull(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 512), NAI_Hull::Mins(GetHullType()),
						   NAI_Hull::Maxs(GetHullType()), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
		...
		case TASK_RANGE_ATTACK1:
		{
			...
			EmitSound(filter, entindex(), CHAN_WEAPON, pAttackSounds[0], GetSoundVolume(), ATTN_IDLE, 0,
					  GetVoicePitch());
#endif
			SetIdealActivity(ACT_RANGE_ATTACK1);
			break;
...
int CBaseHeadcrab::RangeAttack1Conditions(float flDot, float flDist)
{
	...
	if(pEnemy)
	{
		...
		if(GetEnemy()->EyePosition().z - 36.0f > GetAbsOrigin().z)
		{
			...
			AI_TraceHull(vStartHullTrace, vEndHullTrace, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this,
						 GetCollisionGroup(), &tr);
...
void CBaseHeadcrab::Touch(CBaseEntity *pOther)
{
	...
	if(m_NPCState == NPC_STATE_DEAD)
	{
		if(GetAbsVelocity().Length() > 250)
		{
			...
			AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecDir * 100, MASK_SOLID_BRUSHONLY, this,
...
void CBaseHeadcrab::Event_Killed(const CTakeDamageInfo &info)
{
	...
	{
		trace_t tr;
		AI_TraceLine(GetAbsOrigin() + Vector(0, 0, 1), GetAbsOrigin() - Vector(0, 0, 64), MASK_SOLID_BRUSHONLY, this,
					 COLLISION_GROUP_NONE, &tr);
...
int CBaseHeadcrab::SelectSchedule(void)
{
	...
	switch(m_NPCState)
	{
		case NPC_STATE_ALERT:
		{
			...
			else if(HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_PLAYER) || HasCondition(COND_HEAR_WORLD) ||
...
int CBaseHeadcrab::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	...
	if(failedSchedule == SCHED_BACK_AWAY_FROM_ENEMY || failedSchedule == SCHED_PATROL_WALK ||
...
bool CBaseHeadcrab::ValidBurrowPoint(const Vector &point)
{
	...
	AI_TraceHull(point, point + Vector(0, 0, 1), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this, GetCollisionGroup(),
				 &tr);
...
BEGIN_DATADESC(CFastHeadcrab)
	...
	DEFINE_FIELD(m_iRunMode, FIELD_INTEGER), DEFINE_FIELD(m_flRealGroundSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_flSlowRunTime, FIELD_TIME), DEFINE_FIELD(m_flPauseTime, FIELD_TIME),
...
void CFastHeadcrab::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		case TASK_HEADCRAB_HOP_ASIDE:
		{
			...
			AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + vecDir, GetHullMins(), GetHullMaxs(), MASK_SHOT, this,
						 GetCollisionGroup(), &tr);
...
BEGIN_DATADESC(CBlackHeadcrab)
	...
	DEFINE_FIELD(m_bPanicState, FIELD_BOOLEAN), DEFINE_FIELD(m_flPanicStopTime, FIELD_TIME),
...
void CBlackHeadcrab::BuildScheduleTestBits(void)
{
	...
	if(IsCurSchedule(SCHED_HEADCRAB_CRAWL_FROM_CANISTER) || IsCurSchedule(SCHED_RANGE_ATTACK1) ||
	   (IsCurSchedule(SCHED_TAKE_COVER_FROM_ENEMY) && HasMemory(bits_MEMORY_FLINCHED)))
	{
		...
		if(pOther->IsAlive() && pOther->m_iHealth > 1)
		{
			...
			if(hl2_episodic.GetBool())
			{
				...
				else
				{
					...
					pOther->TakeDamage(
						CTakeDamageInfo(this, this, sk_headcrab_poison_npc_damage.GetFloat(), DMG_SLASH));
				}
			}
			else
			{
...
void CBlackHeadcrab::Eject(const QAngle &vecAngles, float flVelocityScale, CBaseEntity *pEnemy)
{
	...
	SetAbsVelocity(
		flVelocityScale * random->RandomInt(20, 50) *
...
bool CBlackHeadcrab::FInViewCone(CBaseEntity *pEntity)
{
	if(IsCurSchedule(SCHED_HEADCRAB_AMBUSH) &&
	   ((pEntity->IsNPC() || pEntity->IsPlayer()) &&
		pEntity->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= CRAB_360_VIEW_DIST_SQR))
	{
...
void CBlackHeadcrab::JumpFlinch(const Vector *pvecDir)
{
	...
	{
		SetAbsVelocity(Vector(random->RandomFloat(-4, 4), random->RandomFloat(-4, 4), up.z) *
...
AI_BEGIN_CUSTOM_NPC(npc_headcrab, CBaseHeadcrab)
	...
	DEFINE_SCHEDULE(SCHED_HEADCRAB_CRAWL_FROM_CANISTER, "	Tasks"
