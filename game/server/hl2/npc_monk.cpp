...
BEGIN_DATADESC(CNPC_Monk)
	//					m_AssaultBehavior
	//					m_LeadBehavior
	DEFINE_FIELD(m_iNumZombies, FIELD_INTEGER), DEFINE_FIELD(m_iDangerousZombies, FIELD_INTEGER),
		DEFINE_FIELD(m_bPerfectAccuracy, FIELD_BOOLEAN), DEFINE_FIELD(m_bMournedPlayer, FIELD_BOOLEAN),
...
Activity CNPC_Monk::NPC_TranslateActivity(Activity eNewActivity)
{
	...
	if((m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT))
	{
		...
		if(bGunUp)
		{
			...
			// keep aiming a little longer than normal since the shot takes so long and there's no good way to do a
...
void CNPC_Monk::GatherConditions()
{
	...
	if(GetActiveWeapon())
	{
		if(GetActiveWeapon()->UsesPrimaryAmmo())
		{
			...
			else if(m_NPCState != NPC_STATE_COMBAT && GetActiveWeapon()->UsesClipsForAmmo1() &&
					GetActiveWeapon()->Clip1() < 2)
			{
...
{
	if(info.GetAttacker()->ClassMatches("npc_headcrab_black") ||
	   info.GetAttacker()->ClassMatches("npc_headcrab_poison"))
		return false;
