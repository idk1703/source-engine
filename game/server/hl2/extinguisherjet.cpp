...
SendPropInt(SENDINFO(m_bEmit), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_bUseMuzzlePoint), 1, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_nLength), 32, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nSize), 32, SPROP_UNSIGNED),
END_SEND_TABLE
()
	...
	DEFINE_KEYFIELD(m_bEnabled, FIELD_BOOLEAN, "enabled"), DEFINE_KEYFIELD(m_nLength, FIELD_INTEGER, "length"),
	DEFINE_KEYFIELD(m_nSize, FIELD_INTEGER, "size"), DEFINE_KEYFIELD(m_nRadius, FIELD_INTEGER, "radius"),
	DEFINE_KEYFIELD(m_flStrength, FIELD_FLOAT, "strength"),

	DEFINE_FIELD(m_bAutoExtinguish, FIELD_BOOLEAN), DEFINE_FIELD(m_bUseMuzzlePoint, FIELD_BOOLEAN),
...
void CExtinguisherJet::ExtinguishThink(void)
{
	...
	if(fire_extinguisher_debug.GetInt())
	{
		...
		NDebugOverlay::Box(tr.endpos, Vector(-radius, -radius, -radius), Vector(radius, radius, radius), 0, 0, 255,
						   false, 0.1f);
	}
}