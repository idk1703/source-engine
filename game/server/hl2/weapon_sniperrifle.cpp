...
BEGIN_DATADESC(CWeaponSniperRifle)
	...
	DEFINE_FIELD(m_fNextZoom, FIELD_FLOAT), DEFINE_FIELD(m_nZoomLevel, FIELD_INTEGER),
...
void CWeaponSniperRifle::PrimaryAttack(void)
{
	...
	if(gpGlobals->curtime >= m_flNextPrimaryAttack)
	{
		...
		// MUST call sound before removing a round from the clip of a CMachineGun dvs: does this apply to the sniper
		// rifle? I don't know.
		WeaponSound(SINGLE);
		...
		// Fire the bullets
		pPlayer->FireBullets(SNIPER_BULLET_COUNT_PLAYER, vecSrc, vecAiming, GetBulletSpread(), MAX_TRACE_LENGTH,
							 m_iPrimaryAmmoType, SNIPER_TRACER_FREQUENCY_PLAYER);
...
void CWeaponSniperRifle::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	switch(pEvent->event)
	{
		case EVENT_WEAPON_SNIPER_RIFLE_FIRE:
		{
			...
			pOperator->FireBullets(SNIPER_BULLET_COUNT_NPC, vecShootOrigin, vecShootDir, vecSpread, MAX_TRACE_LENGTH,
								   m_iPrimaryAmmoType, SNIPER_TRACER_FREQUENCY_NPC);
