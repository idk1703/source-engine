...
char *pszMineStateNames[] = {
	"Dormant", "Deploy", "Captive", "Armed", "Triggered", "Launched",
};
...
BEGIN_DATADESC(CBounceBomb)
	DEFINE_THINKFUNC(ExplodeThink), DEFINE_ENTITYFUNC(ExplodeTouch), DEFINE_THINKFUNC(SearchThink),
		DEFINE_THINKFUNC(BounceThink), DEFINE_THINKFUNC(SettleThink), DEFINE_THINKFUNC(CaptiveThink),
		...
		DEFINE_FIELD(m_bAwake, FIELD_BOOLEAN), DEFINE_FIELD(m_hNearestNPC, FIELD_EHANDLE),
		DEFINE_FIELD(m_hSprite, FIELD_EHANDLE), DEFINE_FIELD(m_LastSpriteColor, FIELD_COLOR32),

		DEFINE_FIELD(m_flHookPositions, FIELD_FLOAT), DEFINE_FIELD(m_iHookN, FIELD_INTEGER),
		DEFINE_FIELD(m_iHookE, FIELD_INTEGER), DEFINE_FIELD(m_iHookS, FIELD_INTEGER),
		DEFINE_FIELD(m_iAllHooks, FIELD_INTEGER),

		DEFINE_KEYFIELD(m_bLockSilently, FIELD_BOOLEAN, "LockSilently"), DEFINE_FIELD(m_bFoeNearest, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flIgnoreWorldTime, FIELD_TIME), DEFINE_KEYFIELD(m_bDisarmed, FIELD_BOOLEAN, "StartDisarmed"),
...
void CBounceBomb::BounceThink()
{
	...
	if(pPhysicsObject != NULL)
	{
		...
		trace_t tr;
		UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, MINE_MAX_JUMP_HEIGHT), MASK_SHOT, this,
...
void CBounceBomb::CavernBounceThink()
{
	...
	if(pPhysicsObject != NULL)
	{
		...
		pPhysicsObject->ApplyTorqueCenter(
			AngularImpulse(random->RandomFloat(15, 40), random->RandomFloat(15, 40), random->RandomFloat(30, 60)));
...
void CBounceBomb::SettleThink()
{
	...
	if(!m_bDisarmed)
	{
		if(VPhysicsGetObject()->IsAsleep() && !(VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD))
		{
			...
			trace_t tr;
			UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 1024), MASK_SHOT | CONTENTS_GRATE, this,
			...
			m_pConstraint =
				physenv->CreateBallsocketConstraint(g_PhysWorldObject, VPhysicsGetObject(), NULL, ballsocket);
			CloseHooks();
...
bool CBounceBomb::IsFriend(CBaseEntity *pEntity)
{
	...
	if(classify == CLASS_METROPOLICE || classify == CLASS_COMBINE || classify == CLASS_MILITARY ||
	   classify == CLASS_COMBINE_HUNTER || classify == CLASS_SCANNER)
	{
		bIsCombine = true;
	}
...
void CBounceBomb::ExplodeThink()
{
	...
	if(m_iModification == MINE_MODIFICATION_CAVERN)
	{
		ExplosionCreate(GetAbsOrigin(), GetAbsAngles(), (pThrower) ? pThrower : this, BOUNCEBOMB_EXPLODE_DAMAGE,
						BOUNCEBOMB_EXPLODE_RADIUS, true, NULL, CLASS_PLAYER_ALLY);
	}
	else
	{
		ExplosionCreate(GetAbsOrigin(), GetAbsAngles(), (pThrower) ? pThrower : this, BOUNCEBOMB_EXPLODE_DAMAGE,
						BOUNCEBOMB_EXPLODE_RADIUS, true);
	}
	UTIL_Remove(this);
}
...
void CBounceBomb::OpenHooks(bool bSilent)
{
	...
	if(VPhysicsGetObject())
	{
		// It's possible to not have a valid physics object here, since this function doubles as an initialization
		// function.
