...
#define MIN_GROUND_ATTACK_DIST \
	500.0f // Minimum distance a target has to be for the gunship to consider using the ground attack weapon
#define MIN_GROUND_ATTACK_HEIGHT_DIFF \
	128.0f // Target's position and hit position must be within this threshold vertically
...
()
	...
	DEFINE_FIELD(m_flNextGroundAttack, FIELD_TIME), DEFINE_FIELD(m_bIsGroundAttacking, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bCanGroundAttack, FIELD_BOOLEAN), DEFINE_FIELD(m_flGroundAttackTime, FIELD_TIME),
	DEFINE_FIELD(m_pRotorWashModel, FIELD_CLASSPTR), DEFINE_FIELD(m_pSmokeTrail, FIELD_EHANDLE),
	DEFINE_FIELD(m_hGroundAttackTarget, FIELD_EHANDLE), DEFINE_SOUNDPATCH(m_pAirExhaustSound),
	DEFINE_SOUNDPATCH(m_pAirBlastSound), DEFINE_SOUNDPATCH(m_pCannonSound),
	DEFINE_FIELD(m_vecAngAcceleration, FIELD_VECTOR), DEFINE_FIELD(m_flDeltaT, FIELD_FLOAT),
	DEFINE_FIELD(m_flTimeNextAttack, FIELD_TIME), DEFINE_FIELD(m_flNextSeeEnemySound, FIELD_TIME),
	DEFINE_FIELD(m_flEndDestructTime, FIELD_TIME), DEFINE_FIELD(m_flNextRocket, FIELD_TIME),
	DEFINE_FIELD(m_iDoSmokePuff, FIELD_INTEGER), DEFINE_FIELD(m_iAmmoType, FIELD_INTEGER),
	DEFINE_FIELD(m_iBurstSize, FIELD_INTEGER), DEFINE_FIELD(m_flBurstDelay, FIELD_FLOAT),
	DEFINE_FIELD(m_fBlindfire, FIELD_BOOLEAN), DEFINE_FIELD(m_fOmniscient, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bIsFiring, FIELD_BOOLEAN), DEFINE_FIELD(m_iBurstHits, FIELD_INTEGER),
	DEFINE_FIELD(m_flTimeNextPing, FIELD_TIME), DEFINE_FIELD(m_flPenetrationDepth, FIELD_FLOAT),
	DEFINE_FIELD(m_vecAttackPosition, FIELD_VECTOR), DEFINE_FIELD(m_vecAttackVelocity, FIELD_VECTOR),
	DEFINE_FIELD(m_angGun, FIELD_VECTOR), DEFINE_PHYSPTR(m_pCrashingController), DEFINE_EMBEDDED(m_crashCallback),
	DEFINE_FIELD(m_hRagdoll, FIELD_EHANDLE), DEFINE_FIELD(m_hCrashTarget, FIELD_EHANDLE),
	DEFINE_FIELD(m_vecHitPos, FIELD_VECTOR), DEFINE_FIELD(m_fPatrolLoopPlaying, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bPreFire, FIELD_BOOLEAN), DEFINE_FIELD(m_bInvulnerable, FIELD_BOOLEAN),
	...
	DEFINE_OUTPUT(m_OnFireCannon, "OnFireCannon"), DEFINE_OUTPUT(m_OnFirstDamage, "OnFirstDamage"),
	DEFINE_OUTPUT(m_OnSecondDamage, "OnSecondDamage"), DEFINE_OUTPUT(m_OnThirdDamage, "OnThirdDamage"),
	DEFINE_OUTPUT(m_OnFourthDamage, "OnFourthDamage"), DEFINE_OUTPUT(m_OnCrashed, "OnCrashed"),
...
Vector CNPC_CombineGunship::GetGroundAttackHitPosition(void)
{
	...
	AI_TraceLine(vecShootPos, vecShootPos + Vector(0, 0, -MAX_TRACE_LENGTH), MASK_SHOT, this, COLLISION_GROUP_NONE,
				 &tr);
	...
	while(tr.endpos != m_vecHitPos)
	{
		...
		if(tr.fraction != 1.0)
		{
			...
			if(tr.startsolid)
			{
				...
				g_pEffects->Sparks(tr2.endpos, 3.0f - (m_flGroundAttackTime - gpGlobals->curtime),
								   3.5f - (m_flGroundAttackTime - gpGlobals->curtime), &tr2.plane.normal);
			}
		}
	}
}
...
void CNPC_CombineGunship::DoBellyBlastDamage(trace_t &tr, Vector vMins, Vector vMaxs)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		trace_t groundTrace;
		UTIL_TraceLine(pEntity->GetAbsOrigin(), pEntity->GetAbsOrigin() - Vector(0, 0, 256), MASK_SOLID, pEntity,
...
void CNPC_CombineGunship::PrescheduleThink(void)
{
	...
	else if(m_lifeState == LIFE_DYING)
	{
		...
		if(bCreateExplosion)
		{
			...
			if(m_hRagdoll)
			{
				m_hRagdoll->CollisionProp()->RandomPointInBounds(Vector(0.25, 0.25, 0.25), Vector(0.75, 0.75, 0.75),
																 &explodePoint);
			}
			else
			{
...
bool CNPC_CombineGunship::FindNearestGunshipCrash(void)
{
	...
	while((pEnt = gEntList.FindEntityByClassname(pEnt, "info_target_gunshipcrash")) != NULL)
	{
		...
		{
			trace_t tr;
			UTIL_TraceLine(WorldSpaceCenter(), pEnt->WorldSpaceCenter(), MASK_SOLID_BRUSHONLY, NULL,
	...
	if(g_debug_gunship.GetInt())
	{
		...
		NDebugOverlay::Box(m_hCrashTarget->GetAbsOrigin(), -Vector(200, 200, 200), Vector(200, 200, 200), 0, 255, 0,
						   128, 0.5);
	}
...
void CNPC_CombineGunship::Flight(void)
{
	...
	QAngle goalAngAccel;
	goalAngAccel.x =
		2.0 * (AngleDiff(goalPitch, AngleNormalize(GetLocalAngles().x)) - GetLocalAngularVelocity().x * dt) / (dt * dt);
	goalAngAccel.y =
		2.0 * (AngleDiff(goalYaw, AngleNormalize(GetLocalAngles().y)) - GetLocalAngularVelocity().y * dt) / (dt * dt);
	goalAngAccel.z =
		2.0 * (AngleDiff(goalRoll, AngleNormalize(GetLocalAngles().z)) - GetLocalAngularVelocity().z * dt) / (dt * dt);
	...
	if(!m_hCrashTarget && m_lifeState == LIFE_DYING && !hl2_episodic.GetBool())
	{
		...
		// EXCEPT In episodic, where forcing it to the ground means it crashes where the player can't see (attic
		// showdown) (sjb)
		vecImpulse.z = -10;
	}
	else
...
void CNPC_CombineGunship::SelfDestruct(void)
{
	...
	if(!m_hCrashTarget)
	{
		...
		PropBreakableCreateAll(pBreakEnt->GetModelIndex(), pBreakEnt->VPhysicsGetObject(), pBreakEnt->GetAbsOrigin(),
							   pBreakEnt->GetAbsAngles(), pBreakEnt->GetAbsVelocity(), angVelocity, 1.0, 800,
		...
		for(int i = 0; i < 20; i++)
		{
			...
			te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, Vector(40, 40, 40), gibVelocity, iModelIndex, 400,
						   1, 2.5, BREAK_METAL);
		}
...
void CNPC_CombineGunship::UpdateEnemyTarget(void)
{
	...
	if(bMaxHits || (m_vecAttackPosition - enemyPos).LengthSqr() > (64 * 64))
	{
		...
		if(g_debug_gunship.GetInt() == GUNSHIP_DEBUG_STITCHING)
		{
			NDebugOverlay::Line(m_vecAttackPosition, m_vecAttackPosition + (m_vecAttackVelocity * 0.1), 255, 0, 0, true,
								4.0f);
		}
