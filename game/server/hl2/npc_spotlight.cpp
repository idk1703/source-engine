...
BEGIN_DATADESC(CNPC_Spotlight)
	DEFINE_FIELD(m_vInspectPos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_flYaw, FIELD_FLOAT),
		DEFINE_FIELD(m_flYawCenter, FIELD_FLOAT), DEFINE_FIELD(m_flYawSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_flYawDir, FIELD_FLOAT), DEFINE_FIELD(m_flPitch, FIELD_FLOAT),
		DEFINE_FIELD(m_flPitchCenter, FIELD_FLOAT), DEFINE_FIELD(m_flPitchSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_flPitchDir, FIELD_FLOAT), DEFINE_FIELD(m_flSpotlightCurLength, FIELD_FLOAT),

		DEFINE_FIELD(m_fSpotlightFlags, FIELD_INTEGER), DEFINE_FIELD(m_flInspectEndTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextEntitySearchTime, FIELD_TIME), DEFINE_FIELD(m_flNextHintSearchTime, FIELD_TIME),
		DEFINE_FIELD(m_bHadEnemy, FIELD_BOOLEAN), DEFINE_FIELD(m_vSpotlightTargetPos, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vSpotlightCurrentPos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_pSpotlight, FIELD_CLASSPTR),
		DEFINE_FIELD(m_pSpotlightTarget, FIELD_CLASSPTR), DEFINE_FIELD(m_vSpotlightDir, FIELD_VECTOR),
		DEFINE_FIELD(m_nHaloSprite, FIELD_INTEGER), DEFINE_FIELD(m_pScriptedTarget, FIELD_CLASSPTR),

		DEFINE_KEYFIELD(m_flYawRange, FIELD_FLOAT, "YawRange"), DEFINE_KEYFIELD(m_flPitchMin, FIELD_FLOAT, "PitchMin"),
		...
		// Inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "LightOn", InputLightOn), DEFINE_INPUTFUNC(FIELD_VOID, "LightOff", InputLightOff),
		DEFINE_INPUTFUNC(FIELD_VOID, "TrackOn", InputTrackOn), DEFINE_INPUTFUNC(FIELD_VOID, "TrackOff", InputTrackOff),

		// Outputs
		DEFINE_OUTPUT(m_pOutputAlert, "OnAlert"), DEFINE_OUTPUT(m_pOutputDetect, "DetectedEnemy"),
		DEFINE_OUTPUT(m_pOutputLost, "LostEnemy"), DEFINE_OUTPUT(m_pOutputSquadDetect, "SquadDetectedEnemy"),
		DEFINE_OUTPUT(m_pOutputSquadLost, "SquadLostEnemy"), DEFINE_OUTPUT(m_pOutputPosition, "LightPosition"),
...
CBaseEntity *CNPC_Spotlight::BestInspectTarget(void)
{
	...
	if(pBestEntity)
	{
		...
		if(!FVisible(pBestEntity))
		{
			...
			if(!GetEnemies()->HasEludedMe(pBestEntity) && flTimeLastSeen + 0.5 < gpGlobals->curtime)
			{
	...
	for(CEntitySphereQuery sphere(vSearchOrigin, SPOTLIGHT_ENTITY_SEARCH_DIST); pEntity = sphere.GetCurrentEntity();
		sphere.NextEntity())
	{
		if(pEntity->GetFlags() & (FL_CLIENT | FL_NPC))
		{
			...
			if((pEntity->Classify() == CLASS_MILITARY) || (pEntity->Classify() == CLASS_BULLSEYE))
			{
				continue;
			}
			...
			if(!GetEnemies()->HasMemory(pEntity) || GetEnemies()->HasEludedMe(pEntity))
			{
...
void CNPC_Spotlight::UpdateTargets(void)
{
	if(m_fSpotlightFlags & BITS_SPOTLIGHT_TRACK_ON)
	{
		...
		if(GetEnemy() == NULL)
		{
			...
			// -----------------------------------------------------------
			if(HaveInspectTarget() && gpGlobals->curtime > m_flInspectEndTime)
			{
			...
			// --------------------------------------
			if(gpGlobals->curtime > m_flNextHintSearchTime && !HaveInspectTarget())
			{
		...
		// ------------------------------------------
		if(!HaveInspectTarget() && m_pScriptedTarget && m_pSpotlightTarget != NULL)
		{
...
void CNPC_Spotlight::SpotlightCreate(void)
{
	...
	trace_t tr;
	AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() + m_vSpotlightDir * m_flSpotlightMaxLength, MASK_OPAQUE, this,
...
Vector CNPC_Spotlight::SpotlightCurrentPos(void)
{
	...
	// ---------------------------------------------
	if((HaveInspectTarget() && GetEnemy() == NULL) || (!HaveInspectTarget() && !m_pScriptedTarget))
	{
		bool bInRange = true;
...
void CNPC_Spotlight::SpotlightUpdate(void)
{
	...
	// ---------------------------------------------------
	if(m_NPCState == NPC_STATE_ALERT && m_flLastStateChangeTime + 30 < gpGlobals->curtime)
	{
	...
	// ---------------------------------------------------
	if(!m_pSpotlight && m_fSpotlightFlags & BITS_SPOTLIGHT_LIGHT_ON)
	{
		SpotlightCreate();
	}
	if(!m_pSpotlight)
	...
	{
		m_pSpotlightTarget->SetRenderColorA(
			(1 - ((m_flSpotlightCurLength - m_flSpotlightMaxLength) / m_flSpotlightMaxLength)));
		m_pSpotlight->SetFadeLength(m_flSpotlightMaxLength);
	}
	else
	{
