...
#define CHOPPER_MIN_CHASE_DIST_DIFF \
	128.0f // Distance threshold used to determine when a target has moved enough to update our navigation to it
...
ConVar sk_helicopter_firingcone("sk_helicopter_firingcone", "20.0", 0,
...
ConVar sk_helicopter_grenadedamage("sk_helicopter_grenadedamage", "25.0", 0,
								   "The amount of damage the helicopter grenade deals.");
ConVar sk_helicopter_grenaderadius("sk_helicopter_grenaderadius", "275.0", 0,
								   "The damage radius of the helicopter grenade.");
ConVar sk_helicopter_grenadeforce("sk_helicopter_grenadeforce", "55000.0", 0,
								  "The physics force that the helicopter grenade exerts.");
ConVar sk_helicopter_grenade_puntscale("sk_helicopter_grenade_puntscale", "1.5", 0,
...
ConVar sk_npc_dmg_helicopter_to_plr("sk_npc_dmg_helicopter_to_plr", "3", 0,
									"Damage helicopter shots deal to the player");
ConVar sk_npc_dmg_helicopter("sk_npc_dmg_helicopter", "6", 0,
...
ConVar g_helicopter_chargetime("g_helicopter_chargetime", "2.0", 0,
							   "How much time we have to wait (on average) between the time we start hearing the "
							   "charging sound + the chopper fires");
...
ConVar g_helicopter_bullrush_shoot_height(
	...
	"The maximum distance the player can be from the chopper before it stops firing");
ConVar g_helicopter_bullrush_mega_bomb_health("g_helicopter_bullrush_mega_bomb_health", "0.25", 0,
...
// Purpose :
//------------------------------------------------------------------------------
static inline float ClampSplineRemapVal(float flValue, float flMinValue, float flMaxValue, float flOutMin,
										float flOutMax)
{
...
public:
	...
	static CHelicopterChunk *CreateHelicopterChunk(const Vector &vecPos, const QAngle &vecAngles,
												   const Vector &vecVelocity, const char *pszModelName, int chunkID);
...
private:
	...
	CGrenadeHelicopter *SpawnBombEntity(const Vector &vecPos,
										const Vector &vecVelocity); // Spawns the bomb entity and sets it up

	// Deliberately aims as close as possible w/o hitting
	void AimCloseToTargetButMiss(CBaseEntity *pTarget, float flMinDist, float flMaxDist, const Vector &shootOrigin,
								 Vector *pResult);
	...
	void ComputeVelocity(const Vector &deltaPos, float flAdditionalHeight, float flMinDistFromSegment,
						 float flMaxDistFromSegment, Vector *pVecAccel);
...
BEGIN_DATADESC(CNPC_AttackHelicopter)
	...
	DEFINE_ENTITYFUNC(FlyTouch),
...
#endif
		DEFINE_FIELD(m_angGun, FIELD_VECTOR), DEFINE_FIELD(m_vecAngAcceleration, FIELD_VECTOR),
		DEFINE_FIELD(m_iAmmoType, FIELD_INTEGER), DEFINE_FIELD(m_flLastCorpseFall, FIELD_TIME),
		DEFINE_FIELD(m_nGunState, FIELD_INTEGER), DEFINE_FIELD(m_flChargeTime, FIELD_TIME),
		DEFINE_FIELD(m_flIdleTimeDelay, FIELD_FLOAT), DEFINE_FIELD(m_nRemainingBursts, FIELD_INTEGER),
		DEFINE_FIELD(m_nGrenadeCount, FIELD_INTEGER), DEFINE_FIELD(m_flPathOffset, FIELD_FLOAT),
		DEFINE_FIELD(m_flAcrossTime, FIELD_TIME), DEFINE_FIELD(m_flCurrPathOffset, FIELD_FLOAT),
		DEFINE_FIELD(m_nBurstHits, FIELD_INTEGER), DEFINE_FIELD(m_nMaxBurstHits, FIELD_INTEGER),
		DEFINE_FIELD(m_flCircleOfDeathRadius, FIELD_FLOAT), DEFINE_FIELD(m_nAttackMode, FIELD_INTEGER),
		DEFINE_FIELD(m_flInputDropBombTime, FIELD_TIME), DEFINE_FIELD(m_hSensor, FIELD_EHANDLE),
		DEFINE_FIELD(m_flAvoidMetric, FIELD_FLOAT), DEFINE_FIELD(m_vecLastAngVelocity, FIELD_VECTOR),
		DEFINE_AUTO_ARRAY(m_hSmokeTrail, FIELD_EHANDLE), DEFINE_FIELD(m_nSmokeTrailCount, FIELD_INTEGER),
		DEFINE_FIELD(m_nNearShots, FIELD_INTEGER), DEFINE_FIELD(m_nMaxNearShots, FIELD_INTEGER),
		...
		DEFINE_FIELD(m_flLastFastTime, FIELD_TIME), DEFINE_FIELD(m_nSecondaryMode, FIELD_INTEGER),
		DEFINE_FIELD(m_flSecondaryModeStartTime, FIELD_TIME), DEFINE_FIELD(m_bRushForward, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flBullrushAdditionalHeight, FIELD_FLOAT), DEFINE_FIELD(m_nBullrushBombMode, FIELD_INTEGER),
		DEFINE_FIELD(m_flNextBullrushBombTime, FIELD_TIME), DEFINE_FIELD(m_flNextMegaBombHealth, FIELD_FLOAT),
		DEFINE_FIELD(m_nShootingMode, FIELD_INTEGER), DEFINE_FIELD(m_bDeadlyShooting, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bBombingSuppressed, FIELD_BOOLEAN), DEFINE_SOUNDPATCH(m_pGunFiringSound),
		DEFINE_AUTO_ARRAY(m_hLights, FIELD_EHANDLE), DEFINE_FIELD(m_bIgnorePathVisibilityTests, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bShortBlink, FIELD_BOOLEAN), DEFINE_FIELD(m_bIndestructible, FIELD_BOOLEAN),
...
CNPC_AttackHelicopter::CNPC_AttackHelicopter()
	: m_bNonCombat(false), m_flGracePeriod(2.0f), m_bBombsExplodeOnContact(false)
{
	m_flMaxSpeed = 0;
}
...
void CNPC_AttackHelicopter::Spawn(void)
{
	...
	float flChaseDist =
		HasSpawnFlags(SF_HELICOPTER_AGGRESSIVE) ? CHOPPER_MIN_AGGRESSIVE_CHASE_DIST_DIFF : CHOPPER_MIN_CHASE_DIST_DIFF;
...
void CNPC_AttackHelicopter::Startup()
{
	...
	if(HasSpawnFlags(SF_HELICOPTER_LIGHTS))
	{
		...
		SetContextThink(&CNPC_AttackHelicopter::BlinkLightsThink, gpGlobals->curtime + CHOPPER_LIGHT_BLINK_TIME_SHORT,
...
void CNPC_AttackHelicopter::SpotlightThink()
{
	...
	m_Spotlight.Update();
	SetContextThink(&CNPC_AttackHelicopter::SpotlightThink, gpGlobals->curtime + TICK_INTERVAL,
					s_pSpotlightThinkContext);
}
...
bool CTraceFilterChopper::ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask)
{
	...
	if(pEnt)
	{
		if(FClassnameIs(pEnt, "func_breakable") || FClassnameIs(pEnt, "func_physbox") ||
		   FClassnameIs(pEnt, "prop_physics") || FClassnameIs(pEnt, "physics_prop"))
		{
			return false;
		}
	}
...
int CNPC_AttackHelicopter::BuildMissTargetList(int nCount, CBaseEntity **ppMissCandidates)
{
	...
	for(int i = 0; i < numEnts; i++)
	{
		...
		if(FClassnameIs(pEnts[i], "prop_dynamic") || FClassnameIs(pEnts[i], "prop_physics") ||
...
// Chooses a point within the circle of death to fire in
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::PickDirectionToCircleOfDeath(const Vector &vBasePos, const Vector &vecFireAtPosition,
														 Vector *pResult)
{
...
// Deliberately aims as close as possible w/o hitting
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::AimCloseToTargetButMiss(CBaseEntity *pTarget, float flMinDist, float flMaxDist,
													const Vector &shootOrigin, Vector *pResult)
{
	Vector vecDirection;
...
void CNPC_AttackHelicopter::ShootAtVehicle(const Vector &vBasePos, const Vector &vecFireAtPosition)
{
	...
	for(i = nShotsRemaining; --i >= 0;)
	{
		...
		{
			Vector vecFireDirection;
			ppNearbyTargets[i]->CollisionProp()->RandomPointInBounds(Vector(.25, .25, .25), Vector(.75, .75, .75),
																	 &vecFireDirection);
			...
			// I put in all the default arguments simply so I could guarantee the first shot of one of the bursts always
			// hits
			FireBulletsInfo_t info(1, vBasePos, vecFireDirection, VECTOR_CONE_PRECALCULATED, MAX_COORD_RANGE,
								   m_iAmmoType);
			info.m_iTracerFreq = 1;
			FireBullets(info);
		}
...
bool CNPC_AttackHelicopter::DoGunCharging()
{
	...
	else
	{
		...
		m_flCircleOfDeathRadius = SimpleSplineRemapVal(flTotal, 0.0f, 1.0f, CHOPPER_MIN_CIRCLE_OF_DEATH_RADIUS,
...
// Shoot where we're facing
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::ShootAtFacingDirection(const Vector &vBasePos, const Vector &vGunDir,
												   bool bFirstShotAccurate)
{
	...
	if(GetEnemy() != NULL)
	{
		CSoundEnt::InsertSound(SOUND_DANGER, GetEnemy()->WorldSpaceCenter(), 180.0f, 0.5f, this,
...
bool CNPC_AttackHelicopter::IsValidZapTarget(CBaseEntity *pTarget)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		if(pSurfaceData->game.material == CHAR_TEX_METAL || pSurfaceData->game.material == CHAR_TEX_FLESH ||
		   pSurfaceData->game.material == CHAR_TEX_VENT || pSurfaceData->game.material == CHAR_TEX_GRATE ||
		   pSurfaceData->game.material == CHAR_TEX_COMPUTER || pSurfaceData->game.material == CHAR_TEX_BLOODYFLESH ||
...
void CNPC_AttackHelicopter::FireElectricityGun()
{
	...
	{
		Vector vecDelta;
		Vector2DSubtract(GetEnemy()->WorldSpaceCenter().AsVector2D(), WorldSpaceCenter().AsVector2D(),
						 vecDelta.AsVector2D());
		vecDelta.z = 0.0f;
	...
	for(i = 0; i < nBoltCount; ++i)
	{
		if((nCandidateCount > 0) && random->RandomFloat(0.0f, 1.0f) < 0.6f)
		{
			...
			Vector vecTarget;
			ppCandidates[nCandidateCount]->CollisionProp()->RandomPointInBounds(vec3_origin, Vector(1, 1, 1),
																				&vecTarget);
...
bool CNPC_AttackHelicopter::DoGunFiring(const Vector &vBasePos, const Vector &vGunDir, const Vector &vecFireAtPosition)
{
	...
	m_flNextAttack =
		gpGlobals->curtime + m_flIdleTimeDelay + random->RandomFloat(flIdleTime - flVariance, flIdleTime + flVariance);
	m_nGunState = GUN_STATE_IDLE;
	SetPauseState(PAUSE_NO_PAUSE);
	return true;
}
...
void CNPC_AttackHelicopter::DropBombs()
{
	...
	else
	{
		...
		if(GetEnemy())
		{
			if(GetEnemy()->GetAbsOrigin().AsVector2D().DistToSqr(GetAbsOrigin().AsVector2D()) >
			   MAX_BULLRUSH_BOMB_DISTANCE_SQR)
				return;
		}
	}
...
// Input  : &vecChunkPos -
//-----------------------------------------------------------------------------
void Chopper_CreateChunk(CBaseEntity *pChopper, const Vector &vecChunkPos, const QAngle &vecChunkAngles,
						 const char *pszChunkName, bool bSmall)
{
	...
	if(random->RandomInt(0, 4))
	{
		for(int i = 0; i < 2; i++)
		{
			Chopper_CreateChunk(this, vecExplosionPos, RandomAngle(0, 360),
								g_PropDataSystem.GetRandomChunkModel("MetalChunks"), true);
		}
	}
	else
	{
		Chopper_CreateChunk(this, vecExplosionPos, RandomAngle(0, 360),
							s_pChunkModelName[random->RandomInt(0, CHOPPER_MAX_SMALL_CHUNKS - 1)], false);
	}
}
...
int CNPC_AttackHelicopter::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	if((info.GetInflictor() != NULL) && (info.GetInflictor()->GetOwnerEntity() != NULL) &&
...
void Chopper_BecomeChunks(CBaseEntity *pChopper)
{
	...
	CHelicopterChunk *pBodyChunk = CHelicopterChunk::CreateHelicopterChunk(
		vecChunkPos, vecChunkAngles, pChopper->GetAbsVelocity(), HELICOPTER_CHUNK_BODY, CHUNK_BODY);
	Chopper_CreateChunk(pChopper, vecChunkPos, RandomAngle(0, 360),
	...
	CHelicopterChunk *pCockpitChunk = CHelicopterChunk::CreateHelicopterChunk(
		vecChunkPos, vecChunkAngles, pChopper->GetAbsVelocity() + vecRight * -800.0f, HELICOPTER_CHUNK_COCKPIT,
		CHUNK_COCKPIT);
	Chopper_CreateChunk(pChopper, vecChunkPos, RandomAngle(0, 360),
						s_pChunkModelName[random->RandomInt(0, CHOPPER_MAX_CHUNKS - 1)], false);
	...
	CHelicopterChunk *pTailChunk = CHelicopterChunk::CreateHelicopterChunk(
		vecChunkPos, vecChunkAngles, pChopper->GetAbsVelocity() + vecRight * 800.0f, HELICOPTER_CHUNK_TAIL, CHUNK_TAIL);
	Chopper_CreateChunk(pChopper, vecChunkPos, RandomAngle(0, 360),
						s_pChunkModelName[random->RandomInt(0, CHOPPER_MAX_CHUNKS - 1)], false);
...
void CNPC_AttackHelicopter::ComputeVelocity(const Vector &vecTargetPosition, float flAdditionalHeight,
											float flMinDistFromSegment, float flMaxDistFromSegment, Vector *pVecAccel)
{
	Vector deltaPos;
	...
	flNewHorizLiftFactor = SimpleSplineRemapVal(flNewHorizLiftFactor, HELICOPTER_MIN_DZ_DAMP, HELICOPTER_MAX_DZ_DAMP,
	...
	float flForceBlend = GetEnemyVehicle() ?
HELICOPTER_FORCE_BLEND_VEHICLE : HELICOPTER_FORCE_BLEND;
...
void CNPC_AttackHelicopter::ComputeAngularVelocity(const Vector &vecGoalUp, const Vector &vecFacingDirection)
{
	...
	if(m_lifeState != LIFE_DYING || (m_lifeState == LIFE_DYING && GetCrashPoint() != NULL))
	{
		...
		float dt = 0.6;
		goalAngAccel.x = 2.0 *
						 (AngleDiff(goalPitch, AngleNormalize(GetAbsAngles().x)) - GetLocalAngularVelocity().x * dt) /
						 (dt * dt);
		goalAngAccel.y =
			2.0 * (AngleDiff(goalYaw, AngleNormalize(GetAbsAngles().y)) - GetLocalAngularVelocity().y * dt) / (dt * dt);
		goalAngAccel.z = 2.0 *
						 (AngleDiff(goalRoll, AngleNormalize(GetAbsAngles().z)) - GetLocalAngularVelocity().z * dt) /
						 (dt * dt);
...
float CNPC_AttackHelicopter::CreepTowardEnemy(float flSpeed, float flMinSpeed, float flMaxSpeed, float flMinDist,
											  float flMaxDist)
{
...
bool CNPC_AttackHelicopter::ShouldShootIdlePlayerInBullrush()
{
	...
	float flSpeedSqr = IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER) ? CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_2_SQ
...
void CNPC_AttackHelicopter::UpdateBullrushState(void)
{
	...
	switch(m_nSecondaryMode)
	{
		...
		case BULLRUSH_MODE_SHOOT_GUN:
		{
			...
			if(m_bRushForward)
			{
				m_flBullrushAdditionalHeight = ClampSplineRemapVal(
					flDistanceToGoal, flSwitchToBombDist, flSwitchToBombDist + flDropDownDist, 0.0f, flShootingHeight);
			...
			else
			{
				m_flBullrushAdditionalHeight =
					ClampSplineRemapVal(flDistanceToGoal, -flSwitchToBombDist - flDropDownDist, -flSwitchToBombDist,
										flShootingHeight, 0.0f);
				if(flDistanceToGoal < -flSwitchToBombDist)
					break;
			}
		...
		case BULLRUSH_MODE_SHOOT_IDLE_PLAYER:
		{
			...
			//			m_nBurstHits = 0;
			m_flCircleOfDeathRadius = ClampSplineRemapVal(SecondaryModeTime(), BULLRUSH_IDLE_PLAYER_FIRE_TIME,
...
BEGIN_DATADESC(CGrenadeHelicopter)
	...
	DEFINE_FIELD(m_bActivated, FIELD_BOOLEAN), DEFINE_FIELD(m_bExplodeOnContact, FIELD_BOOLEAN),
		DEFINE_SOUNDPATCH(m_pWarnSound),

		DEFINE_FIELD(m_hWarningSprite, FIELD_EHANDLE), DEFINE_FIELD(m_bBlinkerAtTop, FIELD_BOOLEAN),

#ifdef HL2_EPISODIC
		DEFINE_FIELD(m_flLifetime, FIELD_FLOAT), DEFINE_FIELD(m_hCollisionObject, FIELD_EHANDLE),
		DEFINE_FIELD(m_bPickedUp, FIELD_BOOLEAN), DEFINE_FIELD(m_flBlinkFastTime, FIELD_TIME),
		...
		DEFINE_THINKFUNC(ExplodeThink), DEFINE_THINKFUNC(AnimateThink), DEFINE_THINKFUNC(RampSoundThink),
		DEFINE_THINKFUNC(WarningBlinkerThink), DEFINE_ENTITYFUNC(ExplodeConcussion),
...
void CGrenadeHelicopter::BecomeActive()
{
	...
	if(!bMegaBomb)
	{
		SetContextThink(&CGrenadeHelicopter::RampSoundThink,
						gpGlobals->curtime + GetBombLifetime() - BOMB_RAMP_SOUND_TIME, s_pRampSoundContext);
	...
	SetContextThink(&CGrenadeHelicopter::WarningBlinkerThink, gpGlobals->curtime + (GetBombLifetime() - 2.0f),
...
void CGrenadeHelicopter::WarningBlinkerThink()
{
	...
	*/
	...
	// Frighten people
	CSoundEnt::InsertSound(SOUND_DANGER, WorldSpaceCenter(), g_helicopter_bomb_danger_radius.GetFloat(), 0.2f, this,
						   SOUNDENT_CHANNEL_REPEATED_DANGER);
...
{
	ExplosionCreate(GetAbsOrigin(), GetAbsAngles(), GetOwnerEntity() ? GetOwnerEntity() : this,
					sk_helicopter_grenadedamage.GetFloat(), sk_helicopter_grenaderadius.GetFloat(),
					(SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NODECAL |
					 SF_ENVEXPLOSION_NOFIREBALL | SF_ENVEXPLOSION_NOPARTICLES),
...
void CGrenadeHelicopter::ExplodeThink(void)
{
	...
	if(bIsThrownByPlayer && pOwner && (iHealthBefore > 0))
	{
		...
		{
			// The player threw us, we exploded due to timer, and we did not damage the helicopter that fired us.  Send
			// a miss event
			SendMissEvent();
		}
	}
...
// Where are how should we avoid?
//-----------------------------------------------------------------------------
void CAvoidSphere::ComputeAvoidanceForces(CBaseEntity *pEntity, float flEntityRadius, float flAvoidTime,
...
// Where are how should we avoid?
//-----------------------------------------------------------------------------
void CAvoidBox::ComputeAvoidanceForces(CBaseEntity *pEntity, float flEntityRadius, float flAvoidTime,
									   Vector *pVecAvoidForce)
{
	pVecAvoidForce->Init();
...
BEGIN_DATADESC(CHelicopterChunk)
	...
	DEFINE_THINKFUNC(FallThink),
		...
		DEFINE_FIELD(m_bLanded, FIELD_BOOLEAN), DEFINE_FIELD(m_hMaster, FIELD_EHANDLE),
		DEFINE_FIELD(m_nChunkID, FIELD_INTEGER), DEFINE_PHYSPTR(m_pTailConstraint),
...
//-----------------------------------------------------------------------------
CHelicopterChunk *CHelicopterChunk::CreateHelicopterChunk(const Vector &vecPos, const QAngle &vecAngles,
														  const Vector &vecVelocity, const char *pszModelName,
														  int chunkID)
{
	// Drop a flaming, smoking chunk.
