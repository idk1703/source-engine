...
public:
	CHLPlayerMove() : m_bWasInVehicle(false), m_bVehicleFlipped(false), m_bInGodMode(false), m_bInNoClip(false)
	{
...
void CHLPlayerMove::FinishMove(CBasePlayer *player, CUserCmd *ucmd, CMoveData *move)
{
	...
	if(gpGlobals->frametime != 0)
	{
		...
		if(distance > 0)
		{
			gamestats->Event_PlayerTraveled(player, distance, pVehicle ? true : false,
											!pVehicle && static_cast<CHL2_Player *>(player)->IsSprinting());
		}
	}
