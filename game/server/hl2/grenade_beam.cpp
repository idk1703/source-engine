...
BEGIN_DATADESC(CGrenadeBeam)
	...
	DEFINE_FIELD(m_vLaunchPos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_flBeamWidth, FIELD_FLOAT),
		DEFINE_FIELD(m_flBeamSpeed, FIELD_FLOAT), DEFINE_FIELD(m_flBeamLag, FIELD_FLOAT),
		DEFINE_FIELD(m_flLaunchTime, FIELD_TIME), DEFINE_FIELD(m_flLastTouchTime, FIELD_TIME),
		DEFINE_FIELD(m_hBeamChaser, FIELD_EHANDLE), DEFINE_FIELD(m_nNumHits, FIELD_INTEGER),
...
void CGrenadeBeam::GrenadeBeamTouch(CBaseEntity *pOther)
{
	...
	UTIL_TraceLine(GetAbsOrigin() - vDirection, GetAbsOrigin() + vDirection, MASK_SOLID, NULL, COLLISION_GROUP_NONE,
				   &tr);
