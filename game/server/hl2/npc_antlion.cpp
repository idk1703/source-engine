...
ConVar sk_antlion_worker_burst_damage("sk_antlion_worker_burst_damage", "50", FCVAR_NONE,
									  "How much damage is inflicted by an antlion worker's death explosion.");
ConVar sk_antlion_worker_health("sk_antlion_worker_health", "0", FCVAR_NONE,
								"Hitpoints of an antlion worker. If 0, will use base antlion hitpoints.");
ConVar sk_antlion_worker_spit_speed("sk_antlion_worker_spit_speed", "0", FCVAR_NONE,
									"Speed at which an antlion spit grenade travels.");

// This must agree with the AntlionWorkerBurstRadius() function!
ConVar sk_antlion_worker_burst_radius("sk_antlion_worker_burst_radius", "160", FCVAR_NONE,
...
BEGIN_DATADESC(CNPC_Antlion)
	...
	DEFINE_KEYFIELD(m_bStartBurrowed, FIELD_BOOLEAN, "startburrowed"),
		...
		DEFINE_FIELD(m_vecSaveSpitVelocity, FIELD_VECTOR), DEFINE_FIELD(m_flIdleDelay, FIELD_TIME),
		DEFINE_FIELD(m_flBurrowTime, FIELD_TIME), DEFINE_FIELD(m_flJumpTime, FIELD_TIME),
		DEFINE_FIELD(m_flPounceTime, FIELD_TIME), DEFINE_FIELD(m_iUnBurrowAttempts, FIELD_INTEGER),
		DEFINE_FIELD(m_iContext, FIELD_INTEGER), DEFINE_FIELD(m_vecSavedJump, FIELD_VECTOR),
		DEFINE_FIELD(m_vecLastJumpAttempt, FIELD_VECTOR), DEFINE_FIELD(m_flIgnoreSoundTime, FIELD_TIME),
		DEFINE_FIELD(m_vecHeardSound, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_bHasHeardSound, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bAgitatedSound, FIELD_BOOLEAN), DEFINE_FIELD(m_bWingsOpen, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flNextAcknowledgeTime, FIELD_TIME), DEFINE_FIELD(m_hFollowTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_hFightGoalTarget, FIELD_EHANDLE), DEFINE_FIELD(m_strParentSpawner, FIELD_STRING),
		DEFINE_FIELD(m_flSuppressFollowTime, FIELD_FLOAT), DEFINE_FIELD(m_MoveState, FIELD_INTEGER),
		DEFINE_FIELD(m_flObeyFollowTime, FIELD_TIME), DEFINE_FIELD(m_bLeapAttack, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bDisableJump, FIELD_BOOLEAN), DEFINE_FIELD(m_flTimeDrown, FIELD_TIME),
		DEFINE_FIELD(m_flTimeDrownSplash, FIELD_TIME), DEFINE_FIELD(m_bDontExplode, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flNextJumpPushTime, FIELD_TIME), DEFINE_FIELD(m_bForcedStuckJump, FIELD_BOOLEAN),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Unburrow", InputUnburrow), DEFINE_INPUTFUNC(FIELD_VOID, "Burrow", InputBurrow),
		...
		// Function Pointers
		DEFINE_ENTITYFUNC(Touch), DEFINE_USEFUNC(BurrowUse), DEFINE_THINKFUNC(ZapThink),
...
const char *pszAntlionGibs_Unique[NUM_ANTLION_GIBS_UNIQUE] = {"models/gibs/antlion_gib_large_1.mdl",
															  "models/gibs/antlion_gib_large_2.mdl",
															  "models/gibs/antlion_gib_large_3.mdl"};

#define NUM_ANTLION_GIBS_MEDIUM 3
const char *pszAntlionGibs_Medium[NUM_ANTLION_GIBS_MEDIUM] = {"models/gibs/antlion_gib_medium_1.mdl",
...
const char *pszAntlionGibs_Small[NUM_ANTLION_GIBS_SMALL] = {"models/gibs/antlion_gib_small_1.mdl",
...
void CNPC_Antlion::Event_Killed(const CTakeDamageInfo &info)
{
	...
	{
		trace_t tr;
		UTIL_TraceLine(GetAbsOrigin() + Vector(0, 0, 64), pAttacker->GetAbsOrigin(), MASK_SOLID, this,
...
void CNPC_Antlion::MeleeAttack(float distance, float damage, QAngle &viewPunch, Vector &shove)
{
	...
	CBaseEntity *pHurt =
		CheckTraceHullAttack(distance, -Vector(16, 16, 32), Vector(16, 16, 32), damage, DMG_SLASH, 5.0f);
...
bool CNPC_Antlion::GetGroundPosition(const Vector &testPos, Vector &result)
{
	...
	trace_t tr;
	AI_TraceHull(testPos, testPos + Vector(0, 0, 64), NAI_Hull::Mins(GetHullType()), NAI_Hull::Maxs(GetHullType()),
	...
	if(tr.allsolid)
	{
		...
		{
			NDebugOverlay::BoxDirection(testPos, NAI_Hull::Mins(GetHullType()),
										NAI_Hull::Maxs(GetHullType()) + Vector(0, 0, 128), Vector(0, 0, 1), 255, 0, 0,
										true, 2.0f);
		}
	...
	{
		NDebugOverlay::BoxDirection(testPos, NAI_Hull::Mins(GetHullType()),
									NAI_Hull::Maxs(GetHullType()) + Vector(0, 0, 128), Vector(0, 0, 1), 0, 255, 0, true,
									2.0f);
	}

	// Trace down to find the ground
	AI_TraceHull(tr.endpos, tr.endpos - Vector(0, 0, 128), NAI_Hull::Mins(GetHullType()), NAI_Hull::Maxs(GetHullType()),
				 MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);

	if(g_debug_antlion.GetInt() == 3)
	{
		NDebugOverlay::BoxDirection(tr.endpos, NAI_Hull::Mins(GetHullType()) - Vector(0, 0, 256),
									NAI_Hull::Maxs(GetHullType()), Vector(0, 0, 1), 255, 255, 0, true, 2.0f);
	}
	...
	if(tr.fraction < 1.0f)
	{
		if(g_debug_antlion.GetInt() == 3)
		{
			NDebugOverlay::Cross3D(tr.endpos, NAI_Hull::Mins(GetHullType()), NAI_Hull::Maxs(GetHullType()), 255, 0, 0,
								   true, 2.0f);
		}
...
Vector VecCheckThrowTolerance(CBaseEntity *pEdict, const Vector &vecSpot1, Vector vecSpot2, float flSpeed,
							  float flTolerance)
{
...
bool CNPC_Antlion::GetSpitVector(const Vector &vecStartPos, const Vector &vecTarget, Vector *vecOut)
{
	...
	Vector vecToss =
		VecCheckThrowTolerance(this, vecStartPos, vecTarget, sk_antlion_worker_spit_speed.GetFloat(), (10.0f * 12.0f));

	// If this failed then try a little faster (flattens the arc)
	if(vecToss == vec3_origin)
	{
		vecToss = VecCheckThrowTolerance(this, vecStartPos, vecTarget, sk_antlion_worker_spit_speed.GetFloat() * 1.5f,
										 (10.0f * 12.0f));
		if(vecToss == vec3_origin)
			return false;
	}
...
void CNPC_Antlion::HandleAnimEvent(animevent_t *pEvent)
{
	...
	if(pEvent->event == AE_ANTLION_WORKER_SPIT)
	{
		if(GetEnemy())
		{
			...
			if(HasCondition(COND_ENEMY_FACING_ME) &&
			   UTIL_DistApprox(GetAbsOrigin(), GetEnemy()->GetAbsOrigin()) > (40 * 12))
			{
			...
			// Tell any squadmates not to fire for some portion of the time this volley will be in the air (except on
			// hard)
			...
			for(int i = 0; i < 6; i++)
			{
				...
				pGrenade->SetLocalAngularVelocity(QAngle(
					random->RandomFloat(-250, -500), random->RandomFloat(-250, -500), random->RandomFloat(-250, -500)));
			}

			for(int i = 0; i < 8; i++)
			{
				DispatchParticleEffect("blood_impact_yellow_01", vSpitPos + RandomVector(-12.0f, 12.0f),
									   RandomAngle(0, 360));
			}
	...
	{
		CSoundEnt::InsertSound(SOUND_PHYSICS_DANGER, GetAbsOrigin(), sk_antlion_worker_burst_radius.GetFloat(), 0.5f,
							   this);
		return;
	}
	...
	{
		CTakeDamageInfo info(this, this, sk_antlion_worker_burst_damage.GetFloat(),
							 DMG_BLAST_SURFACE | (ANTLION_WORKER_BURST_IS_POISONOUS() ? DMG_POISON : DMG_ACID));
		Event_Gibbed(info);
		return;
	}
#endif
...
void CNPC_Antlion::LockJumpNode(void)
{
	...
	while(pWaypoint)
	{
		...
		if(pNextWaypoint && pNextWaypoint->NavType() == NAV_JUMP && pWaypoint->iNodeID != NO_NODE)
		{
			...
			if(pNode)
			{
				// NDebugOverlay::Box( pNode->GetOrigin(), Vector( -16, -16, -16 ), Vector( 16, 16, 16 ), 255, 0, 0, 0,
				// 2 );
				pNode->Lock(0.5f);
				break;
			}
	...
	switch(pTask->iTask)
	{
		...
		case TASK_ANTLION_DISMOUNT_NPC:
		{
			...
			if(pGroundEnt != NULL)
			{
				...
				AI_TraceHull(GetAbsOrigin(), GetAbsOrigin(), WorldAlignMins(), WorldAlignMaxs(), MASK_SOLID,
							 &traceFilter, &trace);
		...
		case TASK_ANTLION_GET_PATH_TO_BUGBAIT:
		{
			...
			if(FindChasePosition(m_vecHeardSound, goalPos))
			{
				...
				if(GetNavigator()->SetGoal(goal, AIN_DISCARD_IF_FAIL) == false)
				{
					...
					AI_NavGoal_t nearGoal(GOALTYPE_LOCATION_NEAREST_NODE, goalPos, (Activity)ACT_ANTLION_RUN_AGITATED,
...
void CNPC_Antlion::RunTask(const Task_t *pTask)
{
	...
	switch(pTask->iTask)
	{
		...
		case TASK_ANTLION_DROWN:
		{
			...
			{
				float flWaterZ = UTIL_FindWaterSurface(GetAbsOrigin(), GetAbsOrigin().z,
		...
		case TASK_ANTLION_DISMOUNT_NPC:
			...
			if(GetFlags() & FL_ONGROUND)
			{
				...
				if((pGroundEnt != NULL) &&
				   ((pGroundEnt->MyNPCPointer() != NULL) || pGroundEnt->GetSolidFlags() & FSOLID_NOT_STANDABLE))
				{
					// Jump behind the other NPC so I don't block their path.
					Vector vecJumpDir;
...
{
	if(IsCurSchedule(SCHED_ANTLION_BURROW_WAIT) || IsCurSchedule(SCHED_ANTLION_BURROW_IN) ||
	   IsCurSchedule(SCHED_ANTLION_BURROW_OUT) || IsCurSchedule(SCHED_ANTLION_BURROW_AWAY) ||
	...
	if(IsMoving() == false && IsCurSchedule(SCHED_ANTLION_FLIP) == false && GetNavType() != NAV_JUMP &&
	   m_flNextJumpPushTime <= gpGlobals->curtime)
	{
		return true;
	}
...
int CNPC_Antlion::SelectSchedule(void)
{
	...
	if((m_spawnflags & SF_ANTLION_BURROW_ON_ELUDED) &&
	   (HasCondition(COND_ENEMY_UNREACHABLE) || HasCondition(COND_ENEMY_TOO_FAR)))
		return SCHED_ANTLION_BURROW_AWAY;
...
void CNPC_Antlion::CascadePush(const Vector &vecForce)
{
	...
	int nNumEnemies = UTIL_EntitiesInBox(pEnemySearch, ARRAYSIZE(pEnemySearch), vecPushBack - Vector(48, 48, 0),
...
int CNPC_Antlion::MeleeAttack1Conditions(float flDot, float flDist)
{
	...
	trace_t tr;
	AI_TraceHull(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), -Vector(8, 8, 8), Vector(8, 8, 8), MASK_NPCSOLID,
...
bool CNPC_Antlion::CheckLanding(void)
{
	...
	// Look below
	AI_TraceHull(GetAbsOrigin(), testPos, NAI_Hull::Mins(GetHullType()), NAI_Hull::Maxs(GetHullType()), MASK_NPCSOLID,
				 this, COLLISION_GROUP_NONE, &tr);
...
{
	trace_t tr;
	AI_TraceLine(GetAbsOrigin() + Vector(0, 0, 1), GetAbsOrigin() - Vector(0, 0, 64),
				 MASK_SOLID_BRUSHONLY | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP, this, COLLISION_GROUP_NONE, &tr);
	...
	if(tr.fraction < 1.0f)
	{
		...
		if(hl2_episodic.GetBool() == true || (pdata->game.material == CHAR_TEX_CONCRETE) ||
		   (pdata->game.material == CHAR_TEX_DIRT) || (pdata->game.material == CHAR_TEX_SAND))
		{
...
void CNPC_Antlion::BuildScheduleTestBits(void)
{
	...
	*/
	...
	if(!IsCurSchedule(SCHED_ANTLION_FLEE_THUMPER) && !IsCurSchedule(SCHED_ANTLION_FLEE_PHYSICS_DANGER) &&
	   !IsCurSchedule(SCHED_ANTLION_BURROW_IN) && !IsCurSchedule(SCHED_ANTLION_WAIT_UNBORROW) &&
	   !IsCurSchedule(SCHED_ANTLION_BURROW_OUT) && !IsCurSchedule(SCHED_ANTLION_BURROW_WAIT) &&
	   !IsCurSchedule(SCHED_ANTLION_WAIT_FOR_UNBORROW_TRIGGER) &&
	   !IsCurSchedule(SCHED_ANTLION_WAIT_FOR_CLEAR_UNBORROW) && !IsCurSchedule(SCHED_ANTLION_WAIT_UNBORROW) &&
	   !IsCurSchedule(SCHED_ANTLION_JUMP) && !IsCurSchedule(SCHED_ANTLION_FLIP) &&
	   !IsCurSchedule(SCHED_ANTLION_DISMOUNT_NPC) && (GetFlags() & FL_ONGROUND))
	{
...
bool CNPC_Antlion::IsValidEnemy(CBaseEntity *pEnemy)
{
	...
	if(IsCurSchedule(GetGlobalScheduleId(SCHED_ANTLION_CHASE_BUGBAIT)) && (GetNavigator() != NULL))
	{
		...
		if((GetNavigator()->IsGoalActive()) &&
		   (GetNavigator()->GetGoalPos() - pEnemy->GetAbsOrigin()).Length() > bugbait_radius.GetFloat())
			return false;
	}
...
void CNPC_Antlion::GatherConditions(void)
{
	...
	if(IsCurSchedule(SCHED_ANTLION_BURROW_WAIT) == false && IsCurSchedule(SCHED_ANTLION_BURROW_IN) == false &&
	   IsCurSchedule(SCHED_ANTLION_BURROW_OUT) == false && IsCurSchedule(SCHED_FALL_TO_GROUND) == false &&
...
void CNPC_Antlion::PrescheduleThink(void)
{
	...
	if((m_bWingsOpen) && (eActivity != ACT_ANTLION_JUMP_START) && (eActivity != ACT_JUMP) && (eActivity != ACT_GLIDE) &&
	...
	if(IsEffectActive(EF_NODRAW) && (eActivity != ACT_ANTLION_BURROW_IDLE) && (eActivity != ACT_ANTLION_BURROW_OUT) &&
...
void CNPC_Antlion::Flip(bool bZapped /*= false*/)
{
	...
	{
		m_flZapDuration =
			gpGlobals->curtime + SequenceDuration(SelectWeightedSequence((Activity)ACT_ANTLION_ZAP_FLIP)) + 0.1f;
...
void CNPC_Antlion::InputJumpAtTarget(inputdata_t &inputdata)
{
	...
	Vector rawJumpVel = GetMoveProbe()->CalcJumpLaunchVelocity(
		GetAbsOrigin(), targetPos, GetCurrentGravity() * GetJumpGravity(), &minJumpHeight, maxHorzVel, &vecApex);
...
BEGIN_DATADESC(CAntlionRepellant)
	DEFINE_KEYFIELD(m_flRepelRadius, FIELD_FLOAT, "repelradius"), DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
...
AI_BEGIN_CUSTOM_NPC(npc_antlion, CNPC_Antlion)
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_JUMP,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_BURROW_WAIT,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_BURROW_IN,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_RUN_TO_BURROW_IN,
	...
	//==================================================
	DEFINE_SCHEDULE(SCHED_ANTLION_FLEE_THUMPER,
	...
	//==================================================
	DEFINE_SCHEDULE(SCHED_ANTLION_CHASE_BUGBAIT,
	...
	//==================================================
	DEFINE_SCHEDULE(SCHED_ANTLION_ZAP_FLIP,
	...
	// SCHED_ANTLION_FLIP
	//==================================================
	DEFINE_SCHEDULE(SCHED_ANTLION_FLIP,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_RUN_TO_FIGHT_GOAL,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_RUN_TO_FOLLOW_GOAL,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_BURROW_AWAY,
	...
	//==================================================
	DEFINE_SCHEDULE(SCHED_ANTLION_FLEE_PHYSICS_DANGER,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_POUNCE,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_WORKER_RANGE_ATTACK1,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_WORKER_RUN_RANDOM,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_TAKE_COVER_FROM_ENEMY,
	...
	DEFINE_SCHEDULE(SCHED_ANTLION_TAKE_COVER_FROM_SAVEPOSITION,
...
bool IsAntlionWorker(CBaseEntity *pEntity)
{
	...
	return (pEntity != NULL && pEntity->Classify() == CLASS_ANTLION && pEntity->HasSpawnFlags(SF_ANTLION_WORKER) &&
