...
BEGIN_DATADESC(CBaseTurret)
	...
	DEFINE_FIELD(m_pEyeGlow, FIELD_CLASSPTR), DEFINE_FIELD(m_eyeBrightness, FIELD_INTEGER),
		...
		DEFINE_FIELD(m_iBaseTurnRate, FIELD_INTEGER), DEFINE_FIELD(m_fTurnRate, FIELD_FLOAT),
		DEFINE_FIELD(m_iOn, FIELD_INTEGER), DEFINE_FIELD(m_fBeserk, FIELD_INTEGER),
		...
		DEFINE_THINKFUNC(TurretDeath), DEFINE_THINKFUNC(Deploy), DEFINE_THINKFUNC(Retire), DEFINE_THINKFUNC(Initialize),
...
bool CBaseTurret::KeyValue(const char *szKeyName, const char *szValue)
{
	...
	else if(FStrEq(szKeyName, "style") || FStrEq(szKeyName, "height") || FStrEq(szKeyName, "value1") ||
