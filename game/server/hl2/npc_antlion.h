...
public:
	CAI_AntlionFollowBehavior() : BaseClass(AIF_ANTLION) {}

	bool FarFromFollowTarget(void)
	{
		return (GetFollowTarget() &&
				(GetAbsOrigin() - GetFollowTarget()->GetAbsOrigin()).LengthSqr() > ANTLION_FOLLOW_DISTANCE_SQR);
	}
...
private:
	...
	Vector m_vecSaveSpitVelocity; // Saved when we start to attack and used if we failed to get a clear shot once we
								  // release
