...
()
	...
	DEFINE_FIELD(m_activeChain, FIELD_INTEGER), DEFINE_FIELD(m_bHasStuckSegments, FIELD_BOOLEAN),
	DEFINE_FIELD(m_flCurrentLength, FIELD_FLOAT), DEFINE_FIELD(m_vecHeadGoal, FIELD_POSITION_VECTOR),
	DEFINE_FIELD(m_flHeadGoalInfluence, FIELD_FLOAT), DEFINE_FIELD(m_vecHeadDir, FIELD_VECTOR),
	DEFINE_FIELD(m_flRelaxedLength, FIELD_FLOAT), DEFINE_FIELD(m_vecOutward, FIELD_VECTOR),
	DEFINE_UTLVECTOR(m_body, FIELD_EMBEDDED), DEFINE_FIELD(m_idealLength, FIELD_FLOAT),
	DEFINE_FIELD(m_idealSegmentLength, FIELD_FLOAT), DEFINE_FIELD(m_bExtendSoundActive, FIELD_BOOLEAN),
	DEFINE_SOUNDPATCH(m_pExtendTentacleSound), DEFINE_FIELD(m_seed, FIELD_FLOAT),
	DEFINE_FIELD(m_vecTarget, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecTargetDir, FIELD_VECTOR),
	DEFINE_FIELD(m_flLastAdjustmentTime, FIELD_TIME), DEFINE_FIELD(m_flTaskStartTime, FIELD_TIME),
	DEFINE_FIELD(m_flTaskEndTime, FIELD_TIME), DEFINE_FIELD(m_flLengthTime, FIELD_TIME),
...
()
	...
	BEGIN_SIMPLE_DATADESC(HydraBone) DEFINE_FIELD(vecPos, FIELD_POSITION_VECTOR),
	DEFINE_FIELD(vecDelta, FIELD_VECTOR), DEFINE_FIELD(flIdealLength, FIELD_FLOAT),
	DEFINE_FIELD(flActualLength, FIELD_FLOAT), DEFINE_FIELD(bStuck, FIELD_BOOLEAN),
	DEFINE_FIELD(bOnFire, FIELD_BOOLEAN), DEFINE_FIELD(vecGoalPos, FIELD_POSITION_VECTOR),
...
{
	// return GetAbsOrigin( ) + Vector( sin( (m_seed + t) * 2.3 ) * 15, cos( (m_seed + t) * 2.4 ) * 150, sin( ( m_seed +
	// t ) * 1.8 ) * 50 ) + m_vecOutward * sv_hydraLength.GetFloat();;
	t = (int)(t * 0.2);
#if 1
	...
	return tmp;
#endif
	// m_vecHeadGoal = GetAbsOrigin( ) + Vector( sin( gpGlobals->curtime * 0.3 ) * 15, cos( gpGlobals->curtime * 0.4 ) *
	// 150, sin( gpGlobals->curtime * 0.2 ) * 50 + dt );
}
...
void CNPC_Hydra::CalcGoalForces()
{
	...
	for(i = iFirst; i <= iLast; i++)
	{
		...
		if(flInfluence > 0)
		{
			...
			// NDebugOverlay::Box(m_body[i].vecGoalPos, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), 255, 255, 0,
			// flInfluence * 255, .1);
		}
	}
#endif
	...
	for(i = iFirst - 1; i <= iLast - 1; i++)
	{
		...
		// NDebugOverlay::Line( m_body[i].vecPos + v3 * flGoalSegmentLength, m_body[i].vecPos - v3 *
		// flGoalSegmentLength, 255, 0, 0, true, .1);

		if(i + 1 <= iLast)
		{
			// towards head
			delta =
				(m_body[i].vecPos + v3 * flGoalSegmentLength - m_body[i + 1].vecPos) * sv_hydraBendTension.GetFloat();
			length = delta.Length();
...
void CNPC_Hydra::MoveBody()
{
	...
	for(i = iFirst; i <= iLast; i++)
	{
		...
		AI_TraceHull(m_body[i].vecPos, m_body[i].vecPos + m_body[i].vecDelta, Vector(-2, -2, -2), Vector(2, 2, 2),
		...
		if(tr.fraction != 1.0)
		{
			...
			AI_TraceHull(direct, direct + slide * (1 - tr.fraction), Vector(-2, -2, -2), Vector(2, 2, 2), MASK_NPCSOLID,
		...
		AI_TraceHull(direct, m_body[i - 1].vecPos, Vector(-2, -2, -2), Vector(2, 2, 2), MASK_NPCSOLID, this,
...
void CNPC_Hydra::AdjustLength()
{
	...
	// DevMsg( "actual %.0f ideal %.0f relaxed %.0f\n", actualLength, m_idealLength, m_idealSegmentLength *
	// (m_body.Count() - 3) );
...
bool CNPC_Hydra::IsValidConnection(int iNode0, int iNode1)
{
	...
	AI_TraceHull(m_body[iNode0].vecPos, m_body[iNode1].vecPos, Vector(-2, -2, -2), Vector(2, 2, 2), MASK_NPCSOLID, this,
				 COLLISION_GROUP_NONE, &tr);
		...
		case TASK_HYDRA_PREP_STAB:
		{
			...
			for(i = i - 1; i > 1 && distBackFromHead < distToTarget; i--)
			{
				...
				Vector p0 = m_vecHeadGoal - m_vecHeadDir * distBackFromHead * 0.5 +
							cos(r) * m_body[i].flActualLength * right + sin(r) * m_body[i].flActualLength * up;
...
public:
	...
	IPhysicsConstraint *CreateConstraint(CNPC_Hydra *pHydra, IPhysicsObject *pTargetPhys,
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *CHydraImpale::CreateConstraint(CNPC_Hydra *pHydra, IPhysicsObject *pTargetPhys,
												   IPhysicsConstraintGroup *pGroup)
{
	m_hHydra = pHydra;
...
AI_BEGIN_CUSTOM_NPC(npc_hydra, CNPC_Hydra)
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HYDRA_DEPLOY,
	...
	// > SCHED_HYDRA_COWER
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HYDRA_RETRACT,
	...
	DEFINE_SCHEDULE(SCHED_HYDRA_IDLE,
	...
	DEFINE_SCHEDULE(SCHED_HYDRA_STAB,
	...
	DEFINE_SCHEDULE(SCHED_HYDRA_PULLBACK,
	...
	DEFINE_SCHEDULE(SCHED_HYDRA_THROW,
	...
	DEFINE_SCHEDULE(SCHED_HYDRA_RANGE_ATTACK,
