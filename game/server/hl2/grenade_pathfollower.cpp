...
BEGIN_DATADESC(CGrenadePathfollower)
	...
	DEFINE_FIELD(m_pPathTarget, FIELD_CLASSPTR), DEFINE_FIELD(m_flFlySpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_sFlySound, FIELD_SOUNDNAME), DEFINE_FIELD(m_flNextFlySoundTime, FIELD_TIME),
...
void CGrenadePathfollower::Detonate(void)
{
	...
	te->Explosion(filter, 0.0, &GetAbsOrigin(), g_sModelIndexFireball, 0.5, 15, TE_EXPLFLAG_NONE, m_DmgRadius,
				  m_flDamage);
	...
	RadiusDamage(CTakeDamageInfo(this, GetThrower(), m_flDamage, DMG_BLAST), GetAbsOrigin(), m_DmgRadius, CLASS_NONE,
				 NULL);
