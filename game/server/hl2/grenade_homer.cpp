...
BEGIN_DATADESC(CGrenadeHomer)
	...
	DEFINE_ARRAY(m_hRocketTrail, FIELD_EHANDLE, 3), DEFINE_FIELD(m_sFlySound, FIELD_STRING),
		DEFINE_FIELD(m_flNextFlySoundTime, FIELD_TIME),

		DEFINE_FIELD(m_flHomingStrength, FIELD_FLOAT), DEFINE_FIELD(m_flHomingDelay, FIELD_FLOAT),
		DEFINE_FIELD(m_flHomingRampUp, FIELD_FLOAT), DEFINE_FIELD(m_flHomingDuration, FIELD_FLOAT),
		DEFINE_FIELD(m_flHomingRampDown, FIELD_FLOAT), DEFINE_FIELD(m_flHomingSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_flSpinMagnitude, FIELD_FLOAT), DEFINE_FIELD(m_flSpinSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_nRocketTrailType, FIELD_INTEGER),

		//	DEFINE_FIELD( m_spriteTexture,				FIELD_INTEGER),

		DEFINE_FIELD(m_flHomingLaunchTime, FIELD_TIME), DEFINE_FIELD(m_flHomingStartTime, FIELD_TIME),
		DEFINE_FIELD(m_flHomingEndTime, FIELD_TIME), DEFINE_FIELD(m_flSpinOffset, FIELD_FLOAT),
...
// Output  :
//------------------------------------------------------------------------------
CGrenadeHomer *CGrenadeHomer::CreateGrenadeHomer(string_t sModelName, string_t sFlySound, const Vector &vecOrigin,
...
// Output  :
//------------------------------------------------------------------------------
void CGrenadeHomer::Launch(CBaseEntity *pOwner, CBaseEntity *pTarget, const Vector &vInitVelocity, float flHomingSpeed,
...
void CGrenadeHomer::GrenadeHomerTouch(CBaseEntity *pOther)
{
	...
	// ----------------------------------
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + GetAbsVelocity(), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
				   &tr);
...
void CGrenadeHomer::Detonate(void)
{
	...
	te->Explosion(filter, 0.0, &GetAbsOrigin(), g_sModelIndexFireball, 2.0, 15, TE_EXPLFLAG_NONE, m_DmgRadius,
				  m_flDamage);
	...
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + 60 * vecForward, MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
				   &tr);
	...
	RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), m_flDamage, DMG_BLAST), GetAbsOrigin(), m_DmgRadius,
				 CLASS_NONE, NULL);
...
void CGrenadeHomer::AimThink(void)
{
	...
	if(m_hTarget != NULL)
	{
		...
		if((vTargetPos - GetAbsOrigin()).Length() > 200)
		{
			...
			trace_t tr;
			UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vTravelDir, MASK_SHOT, m_hTarget, COLLISION_GROUP_NONE,
						   &tr);
			if(tr.fraction != 1.0)
			{
				// Head off in normal
