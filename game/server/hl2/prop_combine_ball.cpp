...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CBaseEntity *CreateCombineBall(const Vector &origin, const Vector &velocity, float radius, float mass, float lifetime,
							   CBaseEntity *pOwner)
{
...
BEGIN_DATADESC(CPropCombineBall)
	...
	DEFINE_FIELD(m_flLastBounceTime, FIELD_TIME), DEFINE_FIELD(m_flRadius, FIELD_FLOAT),
		DEFINE_FIELD(m_nState, FIELD_CHARACTER), DEFINE_FIELD(m_pGlowTrail, FIELD_CLASSPTR),
		DEFINE_SOUNDPATCH(m_pHoldingSound), DEFINE_FIELD(m_bFiredGrabbedOutput, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bEmit, FIELD_BOOLEAN), DEFINE_FIELD(m_bHeld, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bLaunched, FIELD_BOOLEAN), DEFINE_FIELD(m_bStruckEntity, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bWeaponLaunched, FIELD_BOOLEAN), DEFINE_FIELD(m_bForward, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flSpeed, FIELD_FLOAT),

		DEFINE_FIELD(m_flNextDamageTime, FIELD_TIME), DEFINE_FIELD(m_flLastCaptureTime, FIELD_TIME),
		DEFINE_FIELD(m_bCaptureInProgress, FIELD_BOOLEAN), DEFINE_FIELD(m_nBounceCount, FIELD_INTEGER),
		DEFINE_FIELD(m_nMaxBounces, FIELD_INTEGER), DEFINE_FIELD(m_bBounceDie, FIELD_BOOLEAN),

		DEFINE_FIELD(m_hSpawner, FIELD_EHANDLE),

		DEFINE_THINKFUNC(ExplodeThink), DEFINE_THINKFUNC(WhizSoundThink), DEFINE_THINKFUNC(DieThink),
		DEFINE_THINKFUNC(DissolveThink), DEFINE_THINKFUNC(DissolveRampSoundThink), DEFINE_THINKFUNC(AnimThink),
		DEFINE_THINKFUNC(CaptureBySpawner),

		DEFINE_INPUTFUNC(FIELD_VOID, "Explode", InputExplode),
		DEFINE_INPUTFUNC(FIELD_VOID, "FadeAndRespawn", InputFadeAndRespawn),
		DEFINE_INPUTFUNC(FIELD_VOID, "Kill", InputKill), DEFINE_INPUTFUNC(FIELD_VOID, "Socketed", InputSocketed),

END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CPropCombineBall, DT_PropCombineBall)
SendPropBool(SENDINFO(m_bEmit)), SendPropFloat(SENDINFO(m_flRadius), 0, SPROP_NOSCALE), SendPropBool(SENDINFO(m_bHeld)),
...
bool CPropCombineBall::CreateVPhysics()
{
	...
	IPhysicsObject *pPhysicsObject =
		physenv->CreateSphereObject(flSize, nMaterialIndex, GetAbsOrigin(), GetAbsAngles(), &params, false);
	if(!pPhysicsObject)
		return false;
...
void CPropCombineBall::StartAnimating(void)
{
	...
	SetContextThink(&CPropCombineBall::AnimThink, gpGlobals->curtime + random->RandomFloat(0.0f, 0.1f),
					s_pAnimThinkContext);
...
void CPropCombineBall::WhizSoundThink()
{
	...
	if(pPhysicsObject == NULL)
	{
		...
		Assert(0);
		SetContextThink(&CPropCombineBall::WhizSoundThink, gpGlobals->curtime + 2.0f * TICK_INTERVAL,
						s_pWhizThinkContext);
		return;
	}
...
void CPropCombineBall::OnPhysGunPickup(CBasePlayer *pPhysGunUser, PhysGunPickup_t reason)
{
	...
	if(reason != PUNTED_BY_CANNON)
	{
		...
		SetContextThink(&CPropCombineBall::DissolveRampSoundThink, gpGlobals->curtime + GetBallHoldSoundRampTime(),
...
void CPropCombineBall::OnHitEntity(CBaseEntity *pHitEntity, float flSpeed, int index, gamevcollisionevent_t *pEvent)
{
	...
	CTakeDamageInfo info(this, GetOwnerEntity(), GetAbsVelocity(), GetAbsOrigin(), sk_npc_dmg_combineball.GetFloat(),
						 DMG_DISSOLVE);
	...
	// One more check
	// Combine soldiers are not allowed to hurt their friends with combine balls (they can still shoot and hurt each
	// other with grenades).
				...
				if(gpGlobals->curtime >= m_flNextDamageTime)
				{
					   ...
					   hl2_episodic.GetBool() == true)
					{
						if(pHitEntity->Classify() != CLASS_PLAYER_ALLY ||
						   (pHitEntity->Classify() == CLASS_PLAYER_ALLY && m_bStruckEntity == false))
						{
...
void CPropCombineBall::DoImpactEffect(const Vector &preVelocity, int index, gamevcollisionevent_t *pEvent)
{
	...
	UTIL_TraceLine(tr.startpos - preVelocity * 2.0f, tr.startpos + preVelocity * 2.0f, MASK_SOLID, pTraceEntity,
...
bool CPropCombineBall::IsAttractiveTarget(CBaseEntity *pEntity)
{
	...
	else
	{
		...
		trace_t tr;
		UTIL_TraceLine(WorldSpaceCenter(), pEntity->BodyTarget(WorldSpaceCenter()), MASK_SOLID, this,
					   COLLISION_GROUP_NONE, &tr);
...
void CPropCombineBall::DeflectTowardEnemy(float flSpeed, int index, gamevcollisionevent_t *pEvent)
{
	...
	if(bSeekKill)
	{
		int nCount = UTIL_EntitiesInSphere(list, 1024, GetAbsOrigin(), sk_combine_ball_search_radius.GetFloat(),
										   FL_NPC | FL_CLIENT);
...
BEGIN_DATADESC(CFuncCombineBallSpawner)
	...
	DEFINE_KEYFIELD(m_nBallCount, FIELD_INTEGER, "ballcount"), DEFINE_KEYFIELD(m_flMinSpeed, FIELD_FLOAT, "minspeed"),
		DEFINE_KEYFIELD(m_flMaxSpeed, FIELD_FLOAT, "maxspeed"),
		DEFINE_KEYFIELD(m_flBallRadius, FIELD_FLOAT, "ballradius"),
		DEFINE_KEYFIELD(m_flBallRespawnTime, FIELD_FLOAT, "ballrespawntime"), DEFINE_FIELD(m_flRadius, FIELD_FLOAT),
		DEFINE_FIELD(m_nBallsRemainingInField, FIELD_INTEGER), DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
		DEFINE_UTLVECTOR(m_BallRespawnTime, FIELD_TIME), DEFINE_FIELD(m_flDisableTime, FIELD_TIME),

		DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable), DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),

		DEFINE_OUTPUT(m_OnBallGrabbed, "OnBallGrabbed"), DEFINE_OUTPUT(m_OnBallReinserted, "OnBallReinserted"),
...
void CFuncCombineBallSpawner::ChoosePointInBox(Vector *pVecPoint)
{
	...
	CollisionProp()->RandomPointInBounds(Vector(flXBoundary, flYBoundary, 0.0f),
...
void CFuncCombineBallSpawner::ChoosePointInCylinder(Vector *pVecPoint)
{
	...
	do
	{
		CollisionProp()->RandomPointInBounds(Vector(0.5f - flXRange, 0.5f - flYRange, 0.0f),
...
void CFuncCombineBallSpawner::GetTargetEndpoint(bool bForward, Vector *pVecEndPoint)
{
	...
	CollisionProp()->RandomPointInBounds(Vector(0.0f, 0.0f, flZValue), Vector(1.0f, 1.0f, flZValue), pVecEndPoint);
}
...
void CPointCombineBallLauncher::SpawnBall()
{
	...
	qAngle = qAngle + QAngle(random->RandomFloat(-m_flConeDegrees, m_flConeDegrees),
