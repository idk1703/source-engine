...
BEGIN_DATADESC(CNPC_Assassin)
	DEFINE_FIELD(m_nNumFlips, FIELD_INTEGER), DEFINE_FIELD(m_nLastFlipType, FIELD_INTEGER),
		DEFINE_FIELD(m_flNextFlipTime, FIELD_TIME), DEFINE_FIELD(m_flNextLungeTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextShotTime, FIELD_TIME), DEFINE_FIELD(m_bEvade, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bAggressive, FIELD_BOOLEAN), DEFINE_FIELD(m_bBlinkState, FIELD_BOOLEAN),
		DEFINE_FIELD(m_pEyeSprite, FIELD_CLASSPTR), DEFINE_FIELD(m_pEyeTrail, FIELD_CLASSPTR),
END_DATADESC()
...
void CNPC_Assassin::Spawn(void)
{
	...
	CapabilitiesAdd(bits_CAP_SQUAD | bits_CAP_USE_WEAPONS | bits_CAP_AIM_GUN | bits_CAP_INNATE_RANGE_ATTACK1 |
					bits_CAP_INNATE_RANGE_ATTACK2 | bits_CAP_INNATE_MELEE_ATTACK1);
...
int CNPC_Assassin::RangeAttack2Conditions(float flDot, float flDist)
{
	...
	trace_t tr;
	UTIL_TraceHull(GetAbsOrigin(), GetEnemy()->GetAbsOrigin(), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this,
...
void CNPC_Assassin::HandleAnimEvent(animevent_t *pEvent)
{
	...
	if(pEvent->event == AE_ASSASIN_KICK_HIT)
	{
		...
		trace_t tr;
		UTIL_TraceHull(WorldSpaceCenter(), attackPos, -Vector(8, 8, 8), Vector(8, 8, 8), MASK_SHOT_HULL, this,
					   COLLISION_GROUP_NONE, &tr);
...
int CNPC_Assassin::SelectSchedule(void)
{
	switch(m_NPCState)
	{
		...
		case NPC_STATE_COMBAT:
		{
			...
			// Need to move
			if(/*(	HasCondition( COND_SEE_ENEMY ) && HasCondition( COND_ASSASSIN_ENEMY_TARGETTING_ME ) &&
				  random->RandomInt( 0, 32 ) == 0 && m_flNextFlipTime < gpGlobals->curtime ) )*/
			   (m_nNumFlips > 0) || ((HasCondition(COND_LIGHT_DAMAGE) && random->RandomInt(0, 2) == 0)) ||
...
bool CNPC_Assassin::CanFlip(int flipType, Activity &activity, const Vector *avoidPosition)
{
	...
	/*
	UTIL_TraceHull( GetAbsOrigin(), endPos, NAI_Hull::Mins(m_eHull) + Vector( 0, 0, StepHeight() ),
NAI_Hull::Maxs(m_eHull), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );
	...
	if ( tr.fraction < 1.0f )
	{
		...
		{
			NDebugOverlay::BoxDirection( GetAbsOrigin(), NAI_Hull::Mins(m_eHull) + Vector( 0, 0, StepHeight() ),
NAI_Hull::Maxs(m_eHull) + Vector( testDist, 0, StepHeight() ), testDir, 255, 0, 0, true, 2.0f );
		}
...
#define NUM_STEPS 2
	...
	for ( int i = 1; i <= NUM_STEPS; i++ )
	{
		...
		UTIL_TraceHull( endPos, endPos - Vector( 0, 0, StepHeight() * 4.0f ), NAI_Hull::Mins(m_eHull) + Vector( 0, 0,
StepHeight() ), NAI_Hull::Maxs(m_eHull), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );

		if ( tr.fraction == 1.0f )
		{
			if ( g_debug_assassin.GetBool() )
			{
				NDebugOverlay::BoxDirection( endPos, NAI_Hull::Mins(m_eHull) + Vector( 0, 0, StepHeight() ),
NAI_Hull::Maxs(m_eHull) + Vector( StepHeight() * 4.0f, 0, StepHeight() ), Vector(0,0,-1), 255, 0, 0, true, 2.0f );
			}
	...
	{
		NDebugOverlay::BoxDirection( GetAbsOrigin(), NAI_Hull::Mins(m_eHull) + Vector( 0, 0, StepHeight() ),
NAI_Hull::Maxs(m_eHull) + Vector( testDist, 0, StepHeight() ), testDir, 0, 255, 0, true, 2.0f );
	}
	*/
...
const Vector &CNPC_Assassin::GetViewOffset(void)
{
	...
	if((GetActivity() == ACT_ASSASSIN_PERCH) || (GetActivity() == ACT_RANGE_ATTACK1))
	{
...
AI_BEGIN_CUSTOM_NPC(npc_assassin, CNPC_Assassin)
	...
	DEFINE_SCHEDULE(SCHED_ASSASSIN_STALK_ENEMY,
	...
	DEFINE_SCHEDULE(SCHED_ASSASSIN_FIND_VANTAGE_POINT,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_ASSASSIN_EVADE,
	...
	// Assassin needs to avoid the player
	//=========================================================
	DEFINE_SCHEDULE(SCHED_ASSASSIN_LUNGE,
