...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD), SendPropVector(SENDINFO(m_vecDirection), -1, SPROP_COORD),
	SendPropInt(SENDINFO(m_nEntity), 11, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nIndex), 8, SPROP_UNSIGNED),
...
CTEFootprintDecal::CTEFootprintDecal(const char *name) : CBaseTempEntity(name)
{
	m_vecOrigin.Init();
	m_nEntity = 0;
	m_nIndex = 0;
