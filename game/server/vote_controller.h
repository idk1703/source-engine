...
class CBaseIssue // Base class concept for vote issues (i.e. Kick Player).  Created per level-load and destroyed by
				 // CVoteController's dtor.
{
public:
	...
	virtual void SetIssueDetails(
		const char *pszDetails);					   // We need to know the details part of the con command for later
	virtual void OnVoteFailed(int iEntityHoldingVote); // The moment the vote fails, also has some time for feedback
	...
	virtual bool CanCallVote(int nEntIndex, const char *pszDetails, vote_create_failed_t &nFailCode,
	...
	virtual int GetNumberVoteOptions(
		void); // How many choices this vote will have.  i.e. Returns 2 on a Yes/No issue (the default).
	...
	virtual bool NeedsPermissionFromGC(void)
	{
		return false;
	} // Per-issue decision to ask a GC if this vote is permitted (see TF's MvM kick vote for an example)
	virtual void GCResponseReceived(
		bool bApproved); // How to handle the response/verdict from the GC (VoteController also receives a response)
...
public:
	...
	bool CreateVote(int iEntIndex, const char *pszTypeString,
	...
	{
		return (m_iActiveIssueIndex != INVALID_ISSUE || m_pendingVoteParams.m_iIssueIndex != INVALID_ISSUE);
	}
	int GetNumVotesCast(void);

	void AddPlayerToKickWatchList(CSteamID steamID,
								  float flDuration); // Band-aid until we figure out how player's avoid kick votes
...
protected:
	...
	bool SubmitPendingVote(VoteParams_t params); // When an issue requires permission from the GC, this is how the
												 // pending issue moves forward
	...
	CountdownTimer
		m_waitingForGCResponseTimer;   // Some votes require approval from the GC - wait this long for a response
