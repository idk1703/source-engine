...
BEGIN_DATADESC(CScriptedTarget)
	...
	DEFINE_FIELD(m_vLastPosition, FIELD_POSITION_VECTOR),
		...
		DEFINE_KEYFIELD(m_flPauseDuration, FIELD_FLOAT, "PauseDuration"), DEFINE_FIELD(m_flPauseDoneTime, FIELD_TIME),
		...
		// Outputs
		DEFINE_OUTPUT(m_AtTarget, "AtTarget"), DEFINE_OUTPUT(m_LeaveTarget, "LeaveTarget"),
...
CBaseEntity *CScriptedTarget::FindEntity(void)
{
	...
	for(CEntitySphereQuery sphere(GetAbsOrigin(), m_flRadius); (pTestEnt = sphere.GetCurrentEntity()) != NULL;
		sphere.NextEntity())
	{
