...
BEGIN_DATADESC(CTankTrainAI)
	...
	DEFINE_FIELD(m_hTrain, FIELD_EHANDLE), DEFINE_FIELD(m_hTargetEntity, FIELD_EHANDLE),
		DEFINE_FIELD(m_soundPlaying, FIELD_INTEGER), DEFINE_SOUNDPATCH(m_soundTreads), DEFINE_SOUNDPATCH(m_soundEngine),
...
void CTankTrainAI::Activate(void)
{
	...
	if(pTrain)
	{
		...
		if(m_movementSoundName != NULL_STRING)
		{
			...
			m_soundTreads = controller.SoundCreate(filter, pTrain->entindex(), CHAN_STATIC, STRING(m_movementSoundName),
												   ATTN_NORM * 0.5);
		}
		if(m_engineSoundName != NULL_STRING)
		{
			...
			m_soundEngine =
				controller.SoundCreate(filter, pTrain->entindex(), CHAN_STATIC, STRING(m_engineSoundName), ATTN_NORM);
		}
	}
}
