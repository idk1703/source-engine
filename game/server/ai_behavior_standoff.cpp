...
static AI_StandoffParams_t AI_DEFAULT_STANDOFF_PARAMS = {
	AIHCR_MOVE_ON_COVER, true, true, 2.5, 1., 3, 25, 0, false, 0.f};
// Suspected originally intended values:
//
// static AI_StandoffParams_t AI_DEFAULT_STANDOFF_PARAMS = { AIHCR_MOVE_ON_COVER, true, true(?), 1.5, 2.5, 1, 3, 25,
// false(?), 0.(?) };
...
BEGIN_DATADESC(CAI_BattleLine)
	DEFINE_KEYFIELD(m_iszActor, FIELD_STRING, "Actor"), DEFINE_KEYFIELD(m_fActive, FIELD_BOOLEAN, "Active"),
		DEFINE_KEYFIELD(m_fStrict, FIELD_BOOLEAN, "Strict"), DEFINE_EMBEDDED(m_SelfMoveMonitor),
...
BEGIN_SIMPLE_DATADESC(AI_StandoffParams_t)
	DEFINE_FIELD(hintChangeReaction, FIELD_INTEGER), DEFINE_FIELD(fPlayerIsBattleline, FIELD_BOOLEAN),
		DEFINE_FIELD(fCoverOnReload, FIELD_BOOLEAN), DEFINE_FIELD(minTimeShots, FIELD_FLOAT),
		DEFINE_FIELD(maxTimeShots, FIELD_FLOAT), DEFINE_FIELD(minShots, FIELD_INTEGER),
		DEFINE_FIELD(maxShots, FIELD_INTEGER), DEFINE_FIELD(oddsCover, FIELD_INTEGER),
		DEFINE_FIELD(fStayAtCover, FIELD_BOOLEAN), DEFINE_FIELD(flAbandonTimeLimit, FIELD_FLOAT),
END_DATADESC();

BEGIN_DATADESC(CAI_StandoffBehavior)
	DEFINE_FIELD(m_fActive, FIELD_BOOLEAN), DEFINE_FIELD(m_fTestNoDamage, FIELD_BOOLEAN),
		DEFINE_FIELD(m_vecStandoffGoalPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_posture, FIELD_INTEGER),
		DEFINE_EMBEDDED(m_params), DEFINE_FIELD(m_hStandoffGoal, FIELD_EHANDLE),
		DEFINE_FIELD(m_fTakeCover, FIELD_BOOLEAN), DEFINE_FIELD(m_SavedDistTooFar, FIELD_FLOAT),
		DEFINE_FIELD(m_fForceNewEnemy, FIELD_BOOLEAN), DEFINE_EMBEDDED(m_PlayerMoveMonitor),
		DEFINE_EMBEDDED(m_TimeForceCoverHint), DEFINE_EMBEDDED(m_TimePreventForceNewEnemy),
		...
		DEFINE_FIELD(m_nSavedMinShots, FIELD_INTEGER), DEFINE_FIELD(m_nSavedMaxShots, FIELD_INTEGER),
		DEFINE_FIELD(m_flSavedMinRest, FIELD_FLOAT), DEFINE_FIELD(m_flSavedMaxRest, FIELD_FLOAT),

END_DATADESC();

//-------------------------------------

CAI_StandoffBehavior::CAI_StandoffBehavior(CAI_BaseNPC *pOuter) : CAI_MappedActivityBehavior_Temporary(pOuter)
{
	m_fActive = false;
	...
	if(DrawBattleLines.GetInt() != 0)
	{
		...
		while((pEntity = gEntList.FindEntityByClassname(pEntity, "ai_battle_line")) != NULL)
		{
			...
			if(pLine->m_fActive)
			{
				...
				NDebugOverlay::Line(pLine->GetAbsOrigin() - Vector(0, 0, 64), pLine->GetAbsOrigin() + Vector(0, 0, 64),
									0, 255, 0, false, 0.1);
			}
		}
	}
...
void CAI_StandoffBehavior::GatherConditions()
{
	...
	else if(GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)
	{
		if(DrawBattleLines.GetInt() != 0)
		{
			...
			{
				NDebugOverlay::Box(GetOuter()->GetAbsOrigin(), -Vector(48, 48, 4), Vector(48, 48, 4), 255, 0, 0, 8,
								   0.1);
			}
			else
			{
				NDebugOverlay::Box(GetOuter()->GetAbsOrigin(), -Vector(48, 48, 4), Vector(48, 48, 4), 0, 255, 0, 8,
								   0.1);
			}
		}
	}
	...
	if(GetPosture() == AIP_PEEKING || GetPosture() == AIP_STANDING)
	{
		   ...
		   HasCondition(COND_TOO_FAR_TO_ATTACK))
		{
			if(GetOuter()->GetActiveWeapon() &&
			   (GetOuter()->GetActiveWeapon()->CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK1))
			{
...
void CAI_StandoffBehavior::UpdateBattleLines()
{
	...
	{
		// @TODO (toml 06-19-03): This is the quick to code thing. Could use some optimization/caching to not recalc
		// everything (up to) each think
		...
		for(;;)
		{
			...
			if(pBattleLine->m_fActive && (pBattleLine->m_fStrict || !bHaveGoalPosition) &&
	...
	switch(pTask->iTask)
	{
		...
		case TASK_FIND_COVER_FROM_ENEMY:
		{
			...
			if(!m_params.fStayAtCover && (!m_TimeForceCoverHint.Expired() || m_RandomCoverChangeTimer.Expired()))
			{
				...
				// @TODO (toml 03-24-03):  clean this up be tool-izing base tasks. Right now, this is here to force to
				// not use lateral cover search
				...
				if(bTryGoalPosFirst)
				{
					...
					if(coverPos == vec3_origin)
						pTacticalServices->FindCoverPos(goalPos, enemyPos, enemyEyePos, 15 * 12 - 0.1, 40 * 12,
														&coverPos);
				...
				if(coverPos == vec3_origin)
				{
					...
					if(!GetTacticalServices()->FindCoverPos(enemyPos, enemyEyePos, 0, coverRadius, &coverPos))
					{
						...
						{
							// At this point, try again ignoring front lines. Any cover probably better than hanging out
							// in the open
...
void CAI_MappedActivityBehavior_Temporary::UpdateTranslateActivityMap()
{
	AI_ActivityMapping_t
		mappings[] = // This array cannot be static, as some activity values are set on a per-map-load basis
		{
			{
				AIP_CROUCHING,
				ACT_IDLE,
	...
	for(int i = 0; i < ARRAYSIZE(mappings); i++)
	{
		if(!mappings[i].pszWeapon || stricmp(mappings[i].pszWeapon, pszWeaponClass) == 0)
		{
			if(HaveSequenceForActivity(mappings[i].translation) ||
			   HaveSequenceForActivity(GetOuter()->Weapon_TranslateActivity(mappings[i].translation)))
			{
				Assert(m_ActivityMap.Find(MAKE_ACTMAP_KEY(mappings[i].posture, mappings[i].activity)) ==
					   m_ActivityMap.InvalidIndex());
				m_ActivityMap.Insert(MAKE_ACTMAP_KEY(mappings[i].posture, mappings[i].activity),
...
void CAI_StandoffBehavior::UpdateTranslateActivityMap()
{
	...
	m_bHasLowCoverActivity =
		((CapabilitiesGet() & bits_CAP_DUCK) && (GetOuter()->TranslateActivity(lowCoverActivity) != ACT_INVALID));
...
void CAI_MappedActivityBehavior_Temporary::OnChangeActiveWeapon(CBaseCombatWeapon *pOldWeapon,
...
AI_StandoffParams_t g_StandoffParamsByAgression[] = {
	//	hintChangeReaction,		fCoverOnReload, PlayerBtlLn,	minTimeShots,	maxTimeShots, 	minShots, 	maxShots,
	//oddsCover			flAbandonTimeLimit
