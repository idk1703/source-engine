...
void CAI_LocalNavigator::Init(IAI_MovementSink *pMovementServices)
{
	...
	m_pMoveProbe = GetOuter()->GetMoveProbe(); // @TODO (toml 03-30-03): this is a "bad" way to grab this pointer.
											   // Components should have an explcit "init" phase.
}
...
bool CAI_LocalNavigator::MoveCalcDirect(AILocalMoveGoal_t *pMoveGoal, bool bOnlyCurThink, float *pDistClear,
										AIMoveResult_t *pResult)
{
	...
	if(pMoveGoal->speed)
	{
		...
		float probeDist =
			m_pPlaneSolver->CalcProbeDist(pMoveGoal->speed); // having this match steering allows one fewer traces
		...
		if(!bExpectingArrival)
		{
			...
			bTraceClear = GetMoveProbe()->MoveLimit(
				pMoveGoal->navType, GetLocalOrigin(), testPos, MASK_NPCSOLID, pMoveGoal->pMoveTarget, 100.0,
				(pMoveGoal->navType == NAV_GROUND) ? AIMLF_2D : AIMLF_DEFAULT, &pMoveGoal->directTrace);

			if(!bTraceClear)
			{
				// Adjust probe top match expected probe dist (relied on later in process)
				pMoveGoal->directTrace.flDistObstructed =
					(checkDist - moveThisInterval) + pMoveGoal->directTrace.flDistObstructed;
			}
			...
			if(!IsRetail() && ai_debug_directnavprobe.GetBool())
			{
				if(!bTraceClear)
				{
					...
					NDebugOverlay::Line(WorldSpaceCenter(), Vector(testPos.x, testPos.y, WorldSpaceCenter().z), 255, 0,
										0, false, 0.1);
					if(pMoveGoal->directTrace.pObstruction)
						NDebugOverlay::Line(WorldSpaceCenter(), pMoveGoal->directTrace.pObstruction->WorldSpaceCenter(),
											255, 0, 255, false, 0.1);
				}
				else
				{
					NDebugOverlay::Line(WorldSpaceCenter(), Vector(testPos.x, testPos.y, WorldSpaceCenter().z), 0, 255,
										0, false, 0.1);
				}
			}
		...
		if(bTraceClear)
		{
			if(m_FullDirectTimer.Expired())
			{
				...
				bTraceClear = GetMoveProbe()->MoveLimit(
					pMoveGoal->navType, GetLocalOrigin(), testPos, MASK_NPCSOLID, pMoveGoal->pMoveTarget, checkStepPct,
					(pMoveGoal->navType == NAV_GROUND) ? AIMLF_2D : AIMLF_DEFAULT, &pMoveGoal->directTrace);
				if(bExpectingArrival)
					pMoveGoal->thinkTrace = pMoveGoal->directTrace;
				...
				if(ai_debug_directnavprobe.GetBool())
				{
					if(!bTraceClear)
					{
						NDebugOverlay::Line(GetOuter()->EyePosition(),
											Vector(testPos.x, testPos.y, GetOuter()->EyePosition().z), 255, 0, 0, false,
											0.1);
					...
					{
						NDebugOverlay::Line(GetOuter()->EyePosition(),
											Vector(testPos.x, testPos.y, GetOuter()->EyePosition().z), 0, 255, 0, false,
											0.1);
