...
// Input  : *name -
//-----------------------------------------------------------------------------
CTEEnergySplash::CTEEnergySplash(const char *name) : CBaseTempEntity(name)
{
	m_vecPos.Init();
	m_vecDir.Init();
	m_bExplosive = false;
...
SendPropVector(SENDINFO(m_vecPos), -1, SPROP_COORD), SendPropVector(SENDINFO(m_vecDir), -1, SPROP_COORD),
