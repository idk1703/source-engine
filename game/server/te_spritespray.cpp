...
// Input  : *name -
//-----------------------------------------------------------------------------
CTESpriteSpray::CTESpriteSpray(const char *name) : CBaseTempEntity(name)
{
	m_vecOrigin.Init();
	m_vecDirection.Init();
	m_nModelIndex = 0;
...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD), SendPropVector(SENDINFO(m_vecDirection), -1, SPROP_COORD),
	SendPropModelIndex(SENDINFO(m_nModelIndex)), SendPropFloat(SENDINFO(m_fNoise), 8, SPROP_ROUNDDOWN, 0.0, 2.56),
	SendPropInt(SENDINFO(m_nSpeed), 8, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nCount), 8, SPROP_UNSIGNED),
END_SEND_TABLE
()
