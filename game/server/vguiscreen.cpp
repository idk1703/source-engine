...
BEGIN_DATADESC(CVGuiScreen)
	...
	DEFINE_CUSTOM_FIELD(m_nPanelName, &g_VguiScreenStringOps), DEFINE_FIELD(m_nAttachmentIndex, FIELD_INTEGER),
		//	DEFINE_FIELD( m_nOverlayMaterial, FIELD_INTEGER ),
		DEFINE_FIELD(m_fScreenFlags, FIELD_INTEGER), DEFINE_KEYFIELD(m_flWidth, FIELD_FLOAT, "width"),
...
int CVGuiScreen::UpdateTransmitState()
{
	...
	{
		// Let the parent object trigger the send. This is more efficient than having it call
		// CBaseEntity::ShouldTransmit for all the vgui screens in the map.
...
//-----------------------------------------------------------------------------
CVGuiScreen *CreateVGuiScreen(const char *pScreenClassname, const char *pScreenType, CBaseEntity *pAttachedTo,
