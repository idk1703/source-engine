...
BEGIN_DATADESC(CBreakable)
	...
	DEFINE_FIELD(m_Material, FIELD_INTEGER), DEFINE_KEYFIELD(m_Explosion, FIELD_INTEGER, "explosion"),
		DEFINE_KEYFIELD(m_GibDir, FIELD_VECTOR, "gibdir"), DEFINE_FIELD(m_hBreaker, FIELD_EHANDLE),

		// Don't need to save/restore these because we precache after restore
		// DEFINE_FIELD( m_idShard, FIELD_INTEGER ),
		DEFINE_FIELD(m_angle, FIELD_FLOAT), DEFINE_FIELD(m_iszGibModel, FIELD_STRING),
		...
		DEFINE_FIELD(m_bTookPhysicsDamage, FIELD_BOOLEAN), DEFINE_FIELD(m_iszPropData, FIELD_STRING),
		...
		DEFINE_FIELD(m_explodeRadius, FIELD_FLOAT), DEFINE_FIELD(m_iszModelName, FIELD_STRING),
...
void CBreakable::Die(void)
{
	...
	if(m_iszModelName != NULL_STRING)
	{
		for(int i = 0; i < iCount; i++)
		{
			...
			te->BreakModel(filter2, 0.0, vecSpot, pCollisionProp->GetCollisionAngles(), vSize, vecVelocity, iModelIndex,
						   100, 1, 2.5, cFlag | slaveFlag);
		}
	}
	...
	if(Explodable())
	{
		ExplosionCreate(vecSpot, pCollisionProp->GetCollisionAngles(), this, GetExplosiveDamage(), GetExplosiveRadius(),
						true);
	}
}
