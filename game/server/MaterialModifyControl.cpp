...
BEGIN_DATADESC(CMaterialModifyControl)
	// Variables.
	DEFINE_AUTO_ARRAY(m_szMaterialName, FIELD_CHARACTER), DEFINE_AUTO_ARRAY(m_szMaterialVar, FIELD_CHARACTER),
		DEFINE_AUTO_ARRAY(m_szMaterialVarValue, FIELD_CHARACTER), DEFINE_FIELD(m_iFrameStart, FIELD_INTEGER),
		DEFINE_FIELD(m_iFrameEnd, FIELD_INTEGER), DEFINE_FIELD(m_bWrap, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flFramerate, FIELD_FLOAT), DEFINE_FIELD(m_bNewAnimCommandsSemaphore, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flFloatLerpStartValue, FIELD_FLOAT), DEFINE_FIELD(m_flFloatLerpEndValue, FIELD_FLOAT),
		DEFINE_FIELD(m_flFloatLerpTransitionTime, FIELD_FLOAT), DEFINE_FIELD(m_nModifyMode, FIELD_INTEGER),
		// Inputs.
...
SendPropString(SENDINFO(m_szMaterialName)), SendPropString(SENDINFO(m_szMaterialVar)),
	SendPropString(SENDINFO(m_szMaterialVarValue)), SendPropInt(SENDINFO(m_iFrameStart), 8),
	SendPropInt(SENDINFO(m_iFrameEnd), 8), SendPropInt(SENDINFO(m_bWrap), 1, SPROP_UNSIGNED),
...
void CMaterialModifyControl::InputStartAnimSequence(inputdata_t &inputdata)
{
	...
	Warning(
		"%s (%s) received StartAnimSequence input without correct parameters. Syntax: <Frame Start> <Frame End> <Frame "
...
void CMaterialModifyControl::InputStartFloatLerp(inputdata_t &inputdata)
{
	...
	Warning("%s (%s) received StartFloatLerp input without correct parameters. Syntax: <Start Value> <End Value> "
