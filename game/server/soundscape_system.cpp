...
void CSoundscapeSystem::PrintDebugInfo()
{
	...
	for(int entityIndex = 0; entityIndex < m_soundscapeEntities.Size(); ++entityIndex)
	{
		...
		Msg("- %d: %s x:%.4f y:%.4f z:%.4f\n", entityIndex, STRING(currentSoundscape->GetSoundscapeName()),
			currentSoundscape->GetAbsOrigin().x, currentSoundscape->GetAbsOrigin().y,
...
void CSoundscapeSystem::LevelInitPostEntity()
{
	...
	for(int i = 0; i < soundscapeIndexList.Count(); i++)
	{
		...
		int outIndex =
			m_soundscapesInCluster[cluster].soundscapeCount + m_soundscapesInCluster[cluster].firstSoundscape;
