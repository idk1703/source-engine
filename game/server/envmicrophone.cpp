...
BEGIN_DATADESC(CEnvMicrophone)
	...
	DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"), DEFINE_FIELD(m_hMeasureTarget, FIELD_EHANDLE),
		...
		DEFINE_KEYFIELD(m_iszListenFilter, FIELD_STRING, "ListenFilter"), DEFINE_FIELD(m_hListenFilter, FIELD_EHANDLE),
		...
		DEFINE_KEYFIELD(m_flMaxRange, FIELD_FLOAT, "MaxRange"), DEFINE_AUTO_ARRAY(m_szLastSound, FIELD_CHARACTER),
		...
		DEFINE_OUTPUT(m_SoundLevel, "SoundLevel"), DEFINE_OUTPUT(m_OnRoutedSound, "OnRoutedSound"),
...
//-----------------------------------------------------------------------------
MicrophoneResult_t CEnvMicrophone::SoundPlayed(int entindex, const char *soundname, soundlevel_t soundlevel,
...
//-----------------------------------------------------------------------------
bool CEnvMicrophone::OnSoundPlayed(int entindex, const char *soundname, soundlevel_t soundlevel, float flVolume,
								   int iFlags, int iPitch, const Vector *pOrigin, float soundtime,
		...
		for(int i = iCount - 1; i >= 0; i--)
		{
			if(s_Microphones[i])
			{
				MicrophoneResult_t eResult = s_Microphones[i]->SoundPlayed(
					entindex, soundname, soundlevel, flVolume, iFlags, iPitch, pOrigin, soundtime, soundorigins);
