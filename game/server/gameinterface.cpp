...
ConVar sv_force_transmit_ents("sv_force_transmit_ents", "0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							  "Will transmit all entities to client, regardless of PVS conditions (will still skip "
							  "based on transmit flags, however).");
...
#ifdef _DEBUG
static ConVar sv_showhitboxes("sv_showhitboxes", "-1", FCVAR_CHEAT,
							  "Send server-side hitboxes for specified entity to client (NOTE:  this uses lots of "
							  "bandwidth, use on listen server only).");
#endif
...
void DrawAllDebugOverlays(void)
{
	...
	if(engine->IsInEditMode())
	{
		...
		if(pPlayer)
		{
			if(g_pAINetworkManager->GetEditOps()->m_bLinkEditMode)
			{
				...
				{
					// For now just using one big AI network
					Vector startPos = g_pBigAINet->GetNode(pAILink->m_iSrcID)
										  ->GetPosition(g_pAINetworkManager->GetEditOps()->m_iHullDrawNum);
					Vector endPos = g_pBigAINet->GetNode(pAILink->m_iDestID)
										->GetPosition(g_pAINetworkManager->GetEditOps()->m_iHullDrawNum);
					...
					{
						NDebugOverlay::BoxDirection(startPos, Vector(-4, -4, -4), Vector(-linkLen, 4, 4), linkDir, 0,
													255, 0, 40, 0);
					}
					else
					{
						NDebugOverlay::BoxDirection(startPos, Vector(-4, -4, -4), Vector(-linkLen, 4, 4), linkDir, 255,
													0, 0, 40, 0);
					}
				}
			}
	...
	if(sv_massreport.GetInt())
	{
		...
		for(; pInfo; pInfo = pInfo->m_pNext)
		{
			...
			char tempstr[512];
			Q_snprintf(tempstr, sizeof(tempstr), "%s: Mass: %.2f kg / %.2f lb (%s)", STRING(ent->GetModelName()),
					   ent->VPhysicsGetObject()->GetMass(), kg2lbs(ent->VPhysicsGetObject()->GetMass()),
...
// INTERFACEVERSION_SERVERGAMEDLL_VERSION_8 is compatible with the latest since we're only adding things to the end, so
// expose that as well.
// EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerGameDLL, IServerGameDLL008, INTERFACEVERSION_SERVERGAMEDLL_VERSION_8,
// g_ServerGameDLL );
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerGameDLL, IServerGameDLL, INTERFACEVERSION_SERVERGAMEDLL, g_ServerGameDLL);

// When bumping the version to this interface, check that our assumption is still valid and expose the older version in
// the same way
							 ...
							 CreateInterfaceFn fileSystemFactory, CGlobalVars *pGlobals)
{
	...
	if((networkstringtable =
			(INetworkStringTableContainer *)appSystemFactory(INTERFACENAME_NETWORKSTRINGTABLESERVER, NULL)) == NULL)
		return false;
	...
	if((datacache = (IDataCache *)appSystemFactory(DATACACHE_INTERFACE_VERSION, NULL)) == NULL)
		return false;
	if((soundemitterbase = (ISoundEmitterSystemBase *)appSystemFactory(SOUNDEMITTERSYSTEM_INTERFACE_VERSION, NULL)) ==
	   NULL)
		return false;
#ifndef _XBOX
	...
	if(!mdlcache)
		return false;
	if((serverpluginhelpers = (IServerPluginHelpers *)appSystemFactory(INTERFACEVERSION_ISERVERPLUGINHELPERS, NULL)) ==
	   NULL)
		return false;
...
//-----------------------------------------------------------------------------
// Purpose: See shareddefs.h for redefining this.  Don't even think about it, though, for HL2.  Or you will pay.  ywb
// 9/22/03 Output : float
//-----------------------------------------------------------------------------
...
bool CServerGameDLL::LevelInit(const char *pMapName, char const *pMapEntities, char const *pOldLevel,
...
void CServerGameDLL::PreClientUpdate(bool simulating)
{
	...
	*/
	...
	CBaseAnimating *anim =
		dynamic_cast<CBaseAnimating *>(CBaseEntity::Instance(engine->PEntityOfEntIndex(sv_showhitboxes.GetInt())));
	if(!anim)
		return;
...
void CServerGameDLL::OnQueryCvarValueFinished(QueryCvarCookie_t iCookie, edict_t *pPlayerEntity,
...
void CServerGameDLL::CreateNetworkStringTables(void)
{
	...
	g_pStringTableParticleEffectNames =
		networkstringtable->CreateStringTable("ParticleEffectNames", MAX_PARTICLESYSTEMS_STRINGS);
	...
	Assert(g_pStringTableParticleEffectNames && g_pStringTableEffectDispatch && g_pStringTableVguiScreen &&
		   g_pStringTableMaterials && g_pStringTableInfoPanel && g_pStringTableClientSideChoreoScenes &&
...
//-----------------------------------------------------------------------------
void CServerGameDLL::SaveWriteFields(CSaveRestoreData *pSaveData, const char *pname, void *pBaseData, datamap_t *pMap,
...
void CServerGameDLL::SaveReadFields(CSaveRestoreData *pSaveData, const char *pname, void *pBaseData, datamap_t *pMap,
									typedescription_t *pFields, int fieldCount)
{
...
int CServerGameDLL::CreateEntityTransitionList(CSaveRestoreData *s, int a)
{
	...
	if(movedCount)
	{
		g_pGameSaveRestoreBlockSet->CallBlockHandlerRestore(GetPhysSaveRestoreBlockHandler(), base, &restoreHelper,
															false);
		g_pGameSaveRestoreBlockSet->CallBlockHandlerRestore(GetAISaveRestoreBlockHandler(), base, &restoreHelper,
															false);
	}
...
static TITLECOMMENT gTitleComments[] = {
	...
	{"c2a4d", "#HL1_Chapter11_Title"}, // These must appear before "C2A4" so all other map names starting with C2A4 get
									   // that title
...
//-----------------------------------------------------------------------------
IServerGameDLL::ePrepareLevelResourcesResult CServerGameDLL::AsyncPrepareLevelResources(/* in/out */ char *pszMapName,
																						size_t nMapNameSize,
...
static bool IsValidPath(const char *pszFilename)
{
	...
	if(Q_strlen(pszFilename) <= 0 || Q_IsAbsolutePath(pszFilename) || // to protect absolute paths
...
static ConVar motdfile_text("motdfile_text", "motd_text.txt", 0,
...
void CServerGameEnts::CheckTransmit(CCheckTransmitInfo *pInfo, const unsigned short *pEdictIndices, int nEdicts)
{
	...
	for(int i = 0; i < nEdicts; i++)
	{
		...
		int nFlags =
			pEdict->m_fStateFlags & (FL_EDICT_DONTSEND | FL_EDICT_ALWAYS | FL_EDICT_PVSCHECK | FL_EDICT_FULLCHECK);
		...
		//  for any parent which is also in the PVS.  If none are found, then we don't need to worry about sending
		//  ourself
		CBaseEntity *orig = pEnt;
		CServerNetworkProperty *check = netProp->GetNetworkParent();

		// BUG BUG:  I think it might be better to build up a list of edict indices which "depend" on other answers and
		// then resolve them in a second pass.  Not sure what happens if an entity has two parents who both request PVS
		// check?
		while(check)
		{
			...
			int checkFlags = checkEdict->m_fStateFlags &
							 (FL_EDICT_DONTSEND | FL_EDICT_ALWAYS | FL_EDICT_PVSCHECK | FL_EDICT_FULLCHECK);
...
// INTERFACEVERSION_SERVERGAMECLIENTS_VERSION_3 is compatible with the latest since we're only adding things to the end,
// so expose that as well.
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerGameClients, IServerGameClients003,
								  INTERFACEVERSION_SERVERGAMECLIENTS_VERSION_3, g_ServerGameClients);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerGameClients, IServerGameClients, INTERFACEVERSION_SERVERGAMECLIENTS,
								  g_ServerGameClients);
...
//-----------------------------------------------------------------------------
bool CServerGameClients::ClientConnect(edict_t *pEdict, const char *pszName, const char *pszAddress, char *reject,
									   int maxrejectlen)
{
	if(!g_pGameRules)
		return false;
...
//-----------------------------------------------------------------------------
int TestAreaPortalVisibilityThroughPortals(CFuncAreaPortalBase *pAreaPortal, edict_t *pViewEntity, unsigned char *pvs,
										   int pvssize)
{
	...
	for(int i = 0; i != iPortalCount; ++i)
	{
		...
		if(pLocalPortal && pLocalPortal->m_bActivated)
		{
			   ...
			   pRemotePortal->NetworkProp()->IsInPVS(pViewEntity, pvs, pvssize))
			{
				...
				int iPortalNeedsThisPortalOpen =
					pAreaPortal->UpdateVisibility(portalOrg, fovDistanceAdjustFactor, bIsOpenOnClient);
...
// From the eye position, we set up the PAS and PVS to use for filtering network messages to the client.  At this point,
// we could
//  override the actual PAS or PVS values, or use a different origin.
// NOTE:  Do not cache the values of pas and pvs, as they depend on reusable memory in the engine, they are only good
// for this one frame Input  : *pViewEntity -
//			*pClient -
//			**pvs -
//			**pas -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Anything this game .dll wants to add to the bug reporter text (e.g., the entity/model under the picker
// crosshair)
//  can be added here
// Input  : *buf -
//			buflen -
