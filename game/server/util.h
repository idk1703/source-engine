...
#ifdef _WIN32
#define SETUP_EXTERNC(mapClassName) extern "C" _declspec(dllexport) IServerNetworkable *mapClassName(void);
#else
...
void UTIL_SetTrace(trace_t &tr, const Ray_t &ray, edict_t *edict, float fraction, int hitgroup, unsigned int contents,
...
void UTIL_ClientPrintFilter(IRecipientFilter &filter, int msg_dest, const char *msg_name, const char *param1 = NULL,
...
int SENTENCEG_PlayRndSz(edict_t *entity, const char *szrootname, float volume, soundlevel_t soundlevel, int flags,
						int pitch);
int SENTENCEG_PlaySequentialSz(edict_t *entity, const char *szrootname, float volume, soundlevel_t soundlevel,
							   int flags, int pitch, int ipick, int freset);
void SENTENCEG_PlaySentenceIndex(edict_t *entity, int iSentenceIndex, float volume, soundlevel_t soundlevel, int flags,
								 int pitch);
...
bool UTIL_LoadAndSpawnEntitiesFromScript(CUtlVector<CBaseEntity *> &entities, const char *pScriptFile,
										 const char *pBlock, bool bActivate = true);
