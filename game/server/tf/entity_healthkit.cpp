...
bool CHealthKit::MyTouch(CBasePlayer *pPlayer)
{
	...
	if(ValidTouch(pPlayer))
	{
		...
		const bool bIsAnyHeavyWithSandvichEquippedPickingUp =
			pTFPlayer->Weapon_OwnsThisID(TF_WEAPON_LUNCHBOX) && pTFPlayer->IsPlayerClass(TF_CLASS_HEAVYWEAPONS);
		...
		{
			float flHealth =
				ceil((pPlayer->GetMaxHealth() - pTFPlayer->GetRuneHealthBonus()) * PackRatios[GetPowerupSize()]);
			...
			if(pTFPlayer->m_Shared.InCond(TF_COND_DISGUISED) &&
			   pTFPlayer->m_Shared.GetCarryingRuneType() != RUNE_PLAGUE)
			{
			...
			if(nHealthGiven > 0 || pTFPlayer->m_Shared.InCond(TF_COND_BLEEDING) ||
			   pTFPlayer->m_Shared.InCond(TF_COND_BURNING) || pTFPlayer->m_Shared.InCond(TF_COND_PLAGUE))
			{
				bPerformPickup = true;
				bSuccess = true;
				...
				if(pOwner && (pOwner != pTFPlayer))
				{
					if(pOwner->GetTeamNumber() == pTFPlayer->GetTeamNumber())
					{
						...
						if(pOwner->Weapon_OwnsThisID(TF_WEAPON_LUNCHBOX) &&
						   pOwner->IsPlayerClass(TF_CLASS_HEAVYWEAPONS))
						{
							CEconEntity *pEconItem = dynamic_cast<CEconEntity *>(
								pOwner->GetEntityForLoadoutSlot(LOADOUT_POSITION_SECONDARY));
							if(pEconItem)
							{
								EconEntity_OnOwnerKillEaterEvent(pEconItem, pOwner, pTFPlayer,
																 kKillEaterEvent_AllyHealingDone, nHealthGiven);

								if(pTFPlayer->m_Shared.InCond(TF_COND_BURNING))
								{
									EconEntity_OnOwnerKillEaterEvent(pEconItem, pOwner, pTFPlayer,
																	 kKillEaterEvent_BurningAllyExtinguished);
								}
							}
						}
					}
		...
		if(pTFPlayer)
		{
			...
			if(!bIsAnyHeavyWithSandvichEquippedPickingUp || GetOwnerEntity() != pPlayer)
			{
				...
				if(pTFPlayer->IsPlayerClass(TF_CLASS_ENGINEER))
				{
					...
					if(pTFPlayer->GiveAmmo(ceil(iMaxGrenades1 * flPackRatio), TF_AMMO_GRENADES1, true,
										   kAmmoSource_Pickup))
					{
						bAmmoSuccess = true;
					}
