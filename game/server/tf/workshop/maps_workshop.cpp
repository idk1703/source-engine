...
static_assert(sizeof(PublishedFileId_t) == 8,
			  "Various printfs in this file assuming PublishedFileId_t is a 64bit type (e.g. %llu)");
	...
	if(pUGC && !bInitUGC)
	{
		...
		if(engine->IsDedicatedServer() && i)
		{
			...
			if(pUGCPath)
			{
				...
				if(*szFullPath)
				{
					// NOTE we use our own AppID here as the workshop depot id, but this should match the
					// workshopdepotid in our steam config
...
CTFWorkshopMap::CTFWorkshopMap(PublishedFileId_t fileID)
	: m_nFileID(fileID), m_rtimeUpdated(0), m_nFileSize(0), m_eState(eState_Refreshing), m_bHighPriority(false)
{
...
void CTFWorkshopMap::Steam_OnQueryUGCDetails(SteamUGCQueryCompleted_t *pResult, bool bError)
{
	...
	else if(engine->IsDedicatedServer() && (state & k_EItemStateInstalled) && !(state & k_EItemStateDownloading) &&
...
bool CTFWorkshopMap::Downloaded(float *flProgress)
{
	...
	if((state & k_EItemStateInstalled) &&
	   !(state & (k_EItemStateNeedsUpdate | k_EItemStateDownloadPending | k_EItemStateDownloading)))
	{
		if(flProgress)
		{
			*flProgress = 1.f;
...
//-----------------------------------------------------------------------------
IServerGameDLL::ePrepareLevelResourcesResult CTFMapsWorkshop::AsyncPrepareLevelResources(
	/* in/out */ char *pMapName, size_t nMaxMapNameLen,
	/* in/out */ char *pMapFileToUse, size_t nMaxMapFileLen, float *flProgress /* = NULL */)
{
	...
	if(pMap->State() == CTFWorkshopMap::eState_Downloaded)
	{
		...
		{
			// Tell engine we're done so it can go on and fail. It should be using maps/workshop/foo.ugc1234.bsp as a
			// fallback...
...
//-----------------------------------------------------------------------------
IServerGameDLL::eCanProvideLevelResult CTFMapsWorkshop::OnCanProvideLevel(/* in/out */ char *pMapName, int nMapNameMax)
{
	// Prepare the map if necessary
...
//-----------------------------------------------------------------------------
bool CTFMapsWorkshop::CanonicalNameForMap(PublishedFileId_t fileID, const CUtlString &originalFileName,
										  /* out */ CUtlString &strCanonName)
{
