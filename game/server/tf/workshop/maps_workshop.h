...
public:
	...
	//
	// NOTE This does not validate the friendly name of the map: workshop/cp_bogus_name.ugc12345 will return 12345 just
	// the same.
	...
	IServerGameDLL::ePrepareLevelResourcesResult AsyncPrepareLevelResources(/* in/out */ char *pszMapName,
																			size_t nMapNameSize,
...
inline bool CTFMapsWorkshop::IsValidOriginalFileNameForMap(const CUtlString &originalName)
{
	...
	const unsigned int nMaxFileName =
		MAX_DISPLAY_MAP_NAME + 4; // Map minus extension must be within MAX_DISPLAY_MAP_NAME
...
inline bool CTFMapsWorkshop::IsValidDisplayNameForMap(const CUtlString &originalName)
{
	...
	for(int i = 0; i < len; i++)
	{
		...
		{
			TFWorkshopWarning(
				"Invalid map name: _ cannot appear consecutively nor at the beginning/end of a map name\n");
			return false;
		}
	}
