...
BEGIN_DATADESC(CCaptureZone)
	...
	DEFINE_KEYFIELD(m_nCapturePoint, FIELD_INTEGER, "CapturePoint"),
		...
		// Outputs.
		DEFINE_OUTPUT(m_outputOnCapture, "OnCapture"), DEFINE_OUTPUT(m_OnCapTeam1, "OnCapTeam1"),
		DEFINE_OUTPUT(m_OnCapTeam2, "OnCapTeam2"), DEFINE_OUTPUT(m_OnCapTeam1_PD, "OnCapTeam1_PD"),
...
void CCaptureZone::PlayerDestructionThink(void)
{
	...
	if(!IsDisabled())
	{
		...
		// safety check, but this should have already been caught by the ( m_hTouchingEntities.Count() == 0 ) check
		// above
			...
			if(pEnt && pEnt->IsPlayer())
			{
				...
				if(pTFPlayer && pTFPlayer->IsAlive())
				{
					...
					if(pTFPlayer->HasTheFlag() && pTFPlayer->CanScorePointForPD())
					{
						...
						{
							CTFPlayerDestructionLogic::GetPlayerDestructionLogic()->CalcTeamLeader(
...
void CCaptureZone::Capture(CBaseEntity *pOther)
{
	...
	if(pPlayer)
	{
		...
		if(pPlayer->HasItem() && (pPlayer->GetItem()->GetItemID() == TF_ITEM_CAPTURE_FLAG) &&
		   TFGameRules()->CanFlagBeCaptured(pOther))
		{
			CCaptureFlag *pFlag = dynamic_cast<CCaptureFlag *>(pPlayer->GetItem());
			if(!pFlag)
				return;
			...
			if(!pFlag->IsCaptured())
			{
				...
				if(TFGameRules())
				{
					...
					{
						TFGameRules()->DropHalloweenSoulPackToTeam(5, GetAbsOrigin(), pPlayer->GetTeamNumber(),
																   TEAM_SPECTATOR);
					}
				}
			}
...
BEGIN_DATADESC(CFlagDetectionZone)
	...
	DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"),
		...
		DEFINE_OUTPUT(m_outputOnEndTouchFlag, "OnEndTouchFlag"), DEFINE_OUTPUT(m_outputOnDroppedFlag, "OnDroppedFlag"),
...
void CFlagDetectionZone::InputTest(inputdata_t &inputdata)
{
	...
	for(int i = iSize - 1; i >= 0; i--)
	{
		...
		else if(hOther->IsPlayer() && !hOther->IsAlive())
		{
			...
			AssertMsg(false, "Dead player [%s] is still touching this trigger at [%f %f %f]",
					  hOther->GetEntityName().ToCStr(), XYZ(hOther->GetAbsOrigin()));
			Warning("Dead player [%s] is still touching this trigger at [%f %f %f]", hOther->GetEntityName().ToCStr(),
					XYZ(hOther->GetAbsOrigin()));
#endif
			m_hTouchingEntities.Remove(i);
		}
	}
