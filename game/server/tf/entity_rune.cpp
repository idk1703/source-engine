...
bool CTFRune::MyTouch(CBasePlayer *pPlayer)
{
	...
	if(bSuccess)
	{
		...
		// don't allow different team player to pick up team based rune unless they are a spy disguised as the correct
		// team
		...
		if(pTFPlayer->m_Shared.IsStealthed() || pTFPlayer->m_Shared.InCond(TF_COND_STEALTHED_BLINK) ||
		...
		switch(m_nRuneType)
		{
			...
			case RUNE_KNOCKOUT:
			{
				...
				CTFWeaponBase *pMeleeWeapon =
					dynamic_cast<CTFWeaponBase *>(pTFPlayer->GetEntityForLoadoutSlot(LOADOUT_POSITION_MELEE));
				Assert(pMeleeWeapon);
				if(pMeleeWeapon)
				{
					if(pTFPlayer->GetActiveTFWeapon() &&
					   pTFPlayer->GetActiveTFWeapon()->GetWeaponID() != TF_WEAPON_GRAPPLINGHOOK)
					{
...
//-----------------------------------------------------------------------------
CTFRune *CTFRune::CreateRune(const Vector &vecOrigin, RuneTypes_t nType, int nTeam, bool bShouldReposition,
							 bool bApplyForce, Vector vecSpawnDirection /*= vec3_angle*/)
{
	// Make sure we're passing in a valid rune type
	Assert(nType >= 0 && nType < RUNE_TYPES_MAX);

	CTFRune *pRune = NULL;
	pRune = static_cast<CTFRune *>(
		CBaseEntity::CreateNoSpawn("item_powerup_rune", vecOrigin + Vector(0.f, 0.f, 48.f), vec3_angle, NULL));
...
{
	// Powerups will pick a spawn point based on what team color they were when they repositioned. First try your own
	// team color, then neutral, then enemy
	...
	if(nTeamNumber != TEAM_ANY)
	{
		...
		{
			CTFInfoPowerupSpawn *pSpawnPoint =
				static_cast<CTFInfoPowerupSpawn *>(IInfoPowerupSpawnAutoList::AutoList()[n]);
...
//-----------------------------------------------------------------------------
// Purpose: MyTouch function for the temporary powerup rune - needed because these respawn, and don't set the condition
// TF_COND_RUNE
//-----------------------------------------------------------------------------
bool CTFRuneTemp::MyTouch(CBasePlayer *pPlayer)
{
	...
	if(bSuccess)
	{
		...
		if(pTFPlayer->m_Shared.InCond(TF_COND_INVULNERABLE_USER_BUFF) ||
		   pTFPlayer->m_Shared.InCond(TF_COND_CRITBOOSTED_RUNE_TEMP))
		{
			return false;
		}
		...
		if(m_nRuneTempType == RUNETYPE_TEMP_UBER)
		{
			if(pTFPlayer->HasTheFlag() ||
			   pTFPlayer->m_Shared.IsInvulnerable()) // Disallow players who have the flag or are already ubercharged
			{
				return false;
			}
			else
