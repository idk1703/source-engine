...
static ConVar tf_stats_nogameplaycheck("tf_stats_nogameplaycheck", "0", FCVAR_NONE,
...
const char *g_aClassNames[] = {
	"TF_CLASS_UNDEFINED", "TF_CLASS_SCOUT",	   "TF_CLASS_SNIPER",		"TF_CLASS_SOLDIER",
	"TF_CLASS_DEMOMAN",	  "TF_CLASS_MEDIC",	   "TF_CLASS_HEAVYWEAPONS", "TF_CLASS_PYRO",
	"TF_CLASS_SPY",		  "TF_CLASS_ENGINEER", "TF_CLASS_CIVILIAN",
};
...
bool CTFGameStats::AddDataForSend(KeyValues *pKV, StatSendType_t sendType)
{
	...
	for(int iMap = dictMapStats.First(); iMap != dictMapStats.InvalidIndex(); iMap = dictMapStats.Next(iMap))
	{
		...
		for(int i = 0; i <= MAX_CONTROL_POINTS; i++)
		{
			...
			{
				pKVData->SetInt(UTIL_VarArgs("RoundsEndingOnCP%d", i),
								pCurrentMap->m_Header.m_iLastCapChangedInRound[i]);
			}
		}
...
void CTFGameStats::Event_LevelInit(void)
{
	...
	m_reportedStats.m_pCurrentGame->Init(STRING(gpGlobals->mapname), gpGlobals->mapversion, nIPAddr, nPort,
										 gpGlobals->curtime);
...
void CTFGameStats::SendStatsToPlayer(CTFPlayer *pPlayer, bool bIsAlive)
{
	...
	stats.statsCurrentLife.m_iStat[TFSTAT_POINTSSCORED] =
		TFGameRules()->CalcPlayerScore(&stats.statsCurrentLife, pPlayer);
...
void CTFGameStats::Event_PlayerDisconnectedTF(CTFPlayer *pTFPlayer)
{
	...
	if(pTFPlayer->IsAlive())
	{
		...
		if(m_reportedStats.m_pCurrentGame != NULL)
		{
			m_reportedStats.m_pCurrentGame->m_aClassStats[iClass].iTotalTime +=
...
void CTFGameStats::Event_PlayerForceRespawn(CTFPlayer *pPlayer)
{
	if(pPlayer->IsAlive() && !TFGameRules()->PrevRoundWasWaitingForPlayers())
	{
		...
		{
			m_reportedStats.m_pCurrentGame->m_aClassStats[iClass].iTotalTime +=
				(int)(gpGlobals->curtime - pPlayer->GetSpawnTime());
		}
	}
...
{
	// defensive guard against insanely huge damage values that apparently get into the stats system once in a while --
...
{
	// This also gets called when the victim is a building.  That gets tracked separately as building destruction, don't
	// count it here
	if(!pVictim->IsPlayer())
		return;
	...
	if((iClass == TF_CLASS_SCOUT) && pPlayerVictim &&
	   (pPlayerVictim->GetPlayerClass()->GetClassIndex() == TF_CLASS_MEDIC))
	{
...
void CTFGameStats::Event_KillDetail(CTFPlayer *pKiller, CTFPlayer *pVictim, CTFPlayer *pAssister,
									IGameEvent *event /*player_death*/, const CTakeDamageInfo &info)
{
...
void CTFGameStats::Event_RoundEnd(int iWinningTeam, bool bFullRound, float flRoundTime, bool bWasSuddenDeathWin)
{
	...
	if(TFGameRules() &&
	   (TFGameRules()->GetGameType() == TF_GAMETYPE_CP || TFGameRules()->GetGameType() == TF_GAMETYPE_ESCORT ||
...
void CTFGameStats::Event_PlayerKilled(CBasePlayer *pPlayer, const CTakeDamageInfo &info)
{
	...
	if(map)
	{
		// const int MAX_REPORTED_DEATH_COORDS = 2000;	// limit list of death coords reported so it doesn't grow
		// unbounded. if ( map->m_aPlayerDeaths.Count() < MAX_REPORTED_DEATH_COORDS )
		//{
...
void CTFGameStats::FireGameEvent(IGameEvent *event)
{
	...
	else if(Q_strcmp("teamplay_point_startcapture", pEventName) == 0)
	{
		...
		// Should probably see what the community needs are or what our needs are after we release the initial set of
		// stats.
		/*
...
bool CTFGameStats::GetVoteData(const char *szIssueName, int nNumOptions, CUtlVector<const char *> &vecNames)
{
	...
	if(Q_strcmp(szIssueName, "NextLevel") == 0)
	{
		...
		FOR_EACH_MAP_FAST(m_MapsPlaytime, iIndex)
		{
			...
			if(Q_strcmp(szItemName, m_szNextMap) != 0 && Q_strcmp(szItemName, STRING(gpGlobals->mapname)) != 0)
			{
...
CON_COMMAND_F(tf_dumpplayerstats, "Dumps current player stats", FCVAR_DEVELOPMENTONLY)
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(pPlayer && pPlayer->IsConnected())
		{
			...
			{
				Msg("   Stat %d = %d (round), %d (map)\n", iStat, stats.statsCurrentRound.m_iStat[iStat],
					stats.statsAccumulated.m_iStat[iStat]);
			}
		}
	}
}
...
//-----------------------------------------------------------------------------
void CTFGameStats::SW_GameStats_WriteKill(CTFPlayer *pKiller, CTFPlayer *pVictim, CTFPlayer *pAssister,
										  IGameEvent *event /*player_death*/, const CTakeDamageInfo &info)
{
	if(!pKiller || !pVictim)
	...
	pKVData->SetString("VictimClass", ClampedArrayElement(g_aPlayerClassNames_NonLocalized,
														  pVictim->GetPlayerClass()->GetClassIndex()));
	...
	pKVData->SetString("KillerClass", ClampedArrayElement(g_aPlayerClassNames_NonLocalized,
														  pKiller->GetPlayerClass()->GetClassIndex()));
	...
	if(pAssister)
	{
		...
		pKVData->SetString("AssisterClass", ClampedArrayElement(g_aPlayerClassNames_NonLocalized,
																pAssister->GetPlayerClass()->GetClassIndex()));
...
// Purpose: Halloween!
//-----------------------------------------------------------------------------
void CTFGameStats::Event_HalloweenBossEvent(uint8 unBossType, uint16 unBossLevel, uint8 unEventType,
											uint8 unPlayersInvolved, float fElapsedTime)
{
	// if ( !GCClientSystem() )
	//	return;
...
static PasstimeHistogramStats Passtime_HistogramStats(const uint32 (&hist)[TBinCount], uint32 iHistSum,
													  uint32 iSampleCount)
{
	PasstimeHistogramStats result;
	if(iSampleCount <= 1)
...
void CTFGameStats::SW_PasstimeRoundEnded()
{
	...
	pKVData->SetInt("RoundElapsedSec",
					m_passtimeStats.summary.nRoundMaxSec - m_passtimeStats.summary.nRoundRemainingSec);
	...
	auto ballFracStats =
		Passtime_HistogramStats(m_passtimeStats.summary.arrBallFracHist, m_passtimeStats.summary.nBallFracHistSum,
	...
	pKVData->SetInt("BallFracHistSampleCount",
					(int)m_passtimeStats.summary.nBallFracSampleCount); // for approx global average

	auto passTravelDistStats = Passtime_SampleStats(m_passtimeStats.summary.arrPassTravelDistSamples,
													m_passtimeStats.summary.nPassTravelDistSampleCount);
	...
	pKVData->SetInt("PassTravelDistSampleCount",
					(int)m_passtimeStats.summary.nPassTravelDistSampleCount); // for approx global average
...
void CTFGameStats::Event_PowerUpModeDeath(CTFPlayer *pKiller, CTFPlayer *pVictim)
{
	...
	CEconItemView *pItem =
		pKiller->GetLoadoutItem(pKiller->GetPlayerClass()->GetClassIndex(), LOADOUT_POSITION_PRIMARY);
...
void CTFGameStats::Event_PowerUpRuneDuration(CTFPlayer *pPlayer, int iDuration, int nRune)
{
	...
	CEconItemView *pItem =
		pPlayer->GetLoadoutItem(pPlayer->GetPlayerClass()->GetClassIndex(), LOADOUT_POSITION_PRIMARY);
	item_definition_index_t iItemDef = pItem ? pItem->GetItemDefIndex() : 0;
