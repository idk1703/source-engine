...
BEGIN_DATADESC(CTriggerBotTag)
	DEFINE_KEYFIELD(m_iszTags, FIELD_STRING, "tags"), DEFINE_KEYFIELD(m_bAdd, FIELD_BOOLEAN, "add"),
END_DATADESC()
...
BEGIN_DATADESC(CTriggerAddTFPlayerCondition)
	DEFINE_KEYFIELD(m_nCondition, FIELD_INTEGER, "condition"), DEFINE_KEYFIELD(m_flDuration, FIELD_FLOAT, "duration"),
END_DATADESC()
		...
		if(pEnt && pEnt->IsPlayer())
		{
			...
			if(pTFPlayer)
			{
				...
				if(!pTFPlayer->m_Shared.IsInvulnerable() &&
				   !pTFPlayer->m_Shared.InCond(
					   TF_COND_BURNING)) // if player enters trigger invuln, we need to ignite them when it wears off.
										 // We also don't want to ignite an already burning player
				{
