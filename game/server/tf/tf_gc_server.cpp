...
////////////////////////////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
	...
	UpdateConnectedPlayersAndServerInfo()
		...
		- m_pMatchInfo (via GetMatch()) - this represents our match in progress, and should generally mirror the GC, but
			*MIGHT NOT*.  For instance, when the GC is unavailable this object is locked, and when the GC returns we may
be desync'd. This function is in charge of managing that.  Outside code should simply look at the MatchInfo object and
trust that it is the state of the match.

		- m_vecReservationExpiryTime - this should be merged into MatchInfo eventually, but is an array of active
			reservations and when they expire.  This isn't in MatchInfo because in some modes we operate with
reservations but without running a proper Match.  When we're running a match, anyone in this vector should be in the
MatchInfo

		- CTFGSLobby - This is the shared object from the server that represents the match we are hosting.  However, it
is *NOT* the article of record on the match.  This is due to matches being designed to be resilient to GC connection
			loss.  Essentially, only this function should be looking at CTFGSLobby and negotiating the state of the
actual match it believes itself to have in MatchInfo.
	...
	== Gameserver / GC Authority
			...
			- If more players are needed
				- Gameserver requests GC attention with appropriate flag (6v6: Stalled, waiting on complete match,
12v12: Non-full match)
			...
			- If GC is unavailable
				- Game server still carries out duties, may decide to make changes like end match instead of request
late joins if it decides GC wont be able to provide them.
	...
	== Adding Players
			...
			- UpdateConnectedPlayersAndServerInfo ensures that makes sense (it should, though, we no longer have legacy
match types where the GC adds players we shouldn't accept)
	...
	== Dropping Players
			...
			- Case 2: Player is dropped from GC lobby
				...
				- TODO: Ideally there'd be a KickThisGuy GC message, and we'd respond with PlayerLeftMatch, rather than
the GC unilaterally dropping people like this.
			- Case 3: Votekicked
				- PlayerLeftMatch is sent, from server
			- All cases:
				- A reliable GC message player-abandoned (or was kicked or never joined) message queued to reconcile
this with the lobby state, but if GC is unavailable it will be informed when it returns.
				- Player is marked dropped in MatchInfo

	== Team Assignments
			- The GC delivers an initial team assignment for each player added to the match.  This team assignment does
not change when game teams change sides, see TFGameRules::GameTeamToLobbyTeam and its inverse to map these to game logic
teams (vs TF_GC_TEAM objects)

			- All other team changes have to be initiated by a game server message, in modes that allow it, to prevent
				race-conditions.

				- The NewMatchForLobby message expects the GC to shuffle our teams.  We prevent races by not issuing
other team change messages while this message is pending.  If we time out waiting for the GC, some modes may start a
					speculative server-created match (expecting the GC to come back and respond to that message
positively).  In this case, we queue a ChangeMatchPlayerTeams message to stomp any assignments back to our known state,
					allowing us to ignore the temporary de-sync (queued messages always get processed in sequence)

				- The ChangeMatchPlayerTeams message allows the gameserver to change match player teams mid-game in
match modes that allow it.  The game server is in charge of not queuing this message in parallel with NewMatchForLobby
					above, or handling the potential race.

				- When processing either of these messages, the GC cancels any players that are awaiting acceptance by
the game-server, and re-tries if necessary.  This prevents team changes from racing with player-joins which may have
been predicated on differing team layouts.
					- The game server does not accept pending players or send any heartbeats until any queued messages
have been responded to.  See Queued Messages below.
	...
	== Match End
			...
			- GameServerKickingLobby message dissolves live match if GC is available/tracking it. Queued similarly.
				...
				- In MvM, we send potentially multiple victory messages per match -- they can cycle missions and keep
winning.
			...
			- Ended matches have queued a message to dissolve their lobby, though, so further GC interaction with the
match is not possible, and players are allowed to leave (since they're now allowed to be put in a new match by the GC)
	...
	== Queued Messages And Match State And Race Conditions
			- Since queued messages are sent in order until confirmed, the GC will always see (eventually) a coherent
				...
				- MatchResult (minus the two players who left) - GC finishes match accounting, marks match complete,
missing players are already noted as leavers so their absence from the result is expected.
			- While messages are queued, we do not run the UpdateConnectedPlayers() think. This prevents having to worry
about a fractal of potential edge cases -- we don't look at updated lobby data or send heartbeats while anything we're
				trying to tell the GC hasn't been confirmed.  This also means we won't send a heartbeat until all such
actions have been confirmed.
				- GC message handlers for queued messages do have to handle possible races -- if the GC sends us players
while we're sending a "Reassign Player Team" message, this behavior means we'll stubbornly wait for a response to the
team message before acknowledging any players, allowing the GC to easily resolve the race (in this case, by canceling or
retrying any attempted add-player-match actions)

	== Gameserver Crashes
			- If GC is available, it handles it, otherwise, match is lost.  Gameservers don't currently try to persist
this state.
	...
	== Match empties out
			...
			- Once the match is marked ended, and the GC concurs and deletes the lobby, we delete MatchInfo
				- If the GC is not around, we hang out on the completed match state until it is.  We can't exactly take
new matches in the mean time. (but, see k_InvalidState_Timeout_With_Match)
...
// ConVar dota_force_upload_match_stats( "dota_force_upload_match_stats", "0", FCVAR_CHEAT, "If enabled, server will
// upload match stats even when there aren't human players on each side" ); extern ConVar dota_force_bot_cycle;
extern CServerGameDLL g_ServerGameDLL;

// How long a player can be missing from a MM match before they are dropped and given an abandon. Set to -1 to disable.
ConVar tf_mm_player_disconnect_time_before_abandon("tf_mm_player_disconnect_time_before_abandon", "180",
...
ConVar tf_mm_player_disconnect_time_forgive_ratio("tf_mm_player_disconnect_time_forgive_ratio", "10",
												  FCVAR_DEVELOPMENTONLY);
...
ConVar tf_mm_player_disconnect_time_minimum_penalty("tf_mm_player_disconnect_time_minimum_penalty", "90",
													FCVAR_DEVELOPMENTONLY);
...
ConVar tf_allow_server_hibernation("tf_allow_server_hibernation", "1", FCVAR_NONE,
...
class ReliableMsgNewMatchForLobby
	: public CJobReliableMessageBase<ReliableMsgNewMatchForLobby, CMsgGCNewMatchForLobbyRequest,
									 k_EMsgGC_NewMatchForLobbyRequest, CMsgGCNewMatchForLobbyResponse,
...
public:
	...
	{
		dbgStr.Format("Match %llx, Lobby %llx, Next Map %d", Msg().Body().current_match_id(), Msg().Body().lobby_id(),
...
class ReliableMsgChangeMatchPlayerTeams
	: public CJobReliableMessageBase<ReliableMsgChangeMatchPlayerTeams, CMsgGCChangeMatchPlayerTeamsRequest,
									 k_EMsgGC_ChangeMatchPlayerTeamsRequest, CMsgGCChangeMatchPlayerTeamsResponse,
...
public:
	...
	void InitDebugString(CUtlString &dbgStr)
	{
		dbgStr.Format("Match %llx, Lobby %llx, %d members", Msg().Body().match_id(), Msg().Body().lobby_id(),
					  Msg().Body().member_size());
	}
};

//-----------------------------------------------------------------------------
class ReliableMsgMvMVictory : public CJobReliableMessageBase<ReliableMsgMvMVictory, CMsgMvMVictory, k_EMsgGCMvMVictory,
...
public:
	...
	void InitDebugString(CUtlString &dbgStr)
	{
		dbgStr.Format("Lobby %016llx", Msg().Body().lobby_id());
	}
};

//-----------------------------------------------------------------------------
class ReliableMsgGameServerKickingLobby
	: public CJobReliableMessageBase<ReliableMsgGameServerKickingLobby, CMsgGameServerKickingLobby,
									 k_EMsgGCGameServerKickingLobby, CMsgGameServerKickingLobbyResponse,
...
public:
	...
	void InitDebugString(CUtlString &dbgStr)
	{
		dbgStr.Format("Match %llx, Lobby %llx", Msg().Body().match_id(), Msg().Body().lobby_id());
	}
};

//-----------------------------------------------------------------------------
class ReliableMsgPlayerLeftMatch
	: public CJobReliableMessageBase<ReliableMsgPlayerLeftMatch, CMsgPlayerLeftMatch, k_EMsgGCPlayerLeftMatch,
...
public:
	...
	void InitDebugString(CUtlString &dbgStr)
	{
		dbgStr.Format("Player %s, Match %llx, Lobby %llx", CSteamID(Msg().Body().steam_id()).Render(),
...
public:
	...
	const char *MsgName()
	{
		return "PlayerVoteKickedAfterLeavingMatch";
	}
	void InitDebugString(CUtlString &dbgStr)
	{
		dbgStr.Format("Player %s, Match %llx, Lobby %llx", CSteamID(Msg().Body().steam_id()).Render(),
...
class ReliableMsgMatchResult
	: public CJobReliableMessageBase<ReliableMsgMatchResult, CMsgGC_Match_Result, k_EMsgGC_Match_Result,
...
public:
	...
	void InitDebugString(CUtlString &dbgStr)
	{
		dbgStr.Format("Match %016llx", Msg().Body().match_id());
	}
};
...
void CMatchInfo::PlayerMatchData_t::OnConnected(int nEntindex)
{
	...
	MMLog("Match player %s reconnected into slot %d, last active %u seconds ago.\n", steamID.Render(),
		  nConnectingButNotActiveIndex, now - rtLastActiveEvent);
}
...
//-----------------------------------------------------------------------------
void CMatchInfo::AddPlayer(CSteamID steamID, const CTFLobbyMember *pMemberData, bool bIsLateJoin, int nEntIndex,
						   bool bActive)
{
...
//-----------------------------------------------------------------------------
void CMatchInfo::GiveXPDirectly(CSteamID steamID, CMsgTFXPSource::XPSourceType eType, int nAmount,
								bool bCanAwardBonusXP)
{
	...
	if(pMatchPlayer && !pMatchPlayer->bDropped)
	{
		...
		if(tf_debug_xp_changes.GetBool() && nAccum != pSource->amount())
		{
			...
			if(pPlayer)
			{
				Msg("%s received %d %s xp\n", pPlayer->GetPlayerName(), nAccum - pSource->amount(),
...
//-----------------------------------------------------------------------------
void CMatchInfo::GiveXPBonus(CSteamID steamID, CMsgTFXPSource_XPSourceType eType, float flMultipler, int nBonusPool)
{
	const IMatchGroupDescription *pMatchDesc = GetMatchGroupDescription(m_eMatchGroup);
...
#ifdef STAGING_ONLY
CON_COMMAND(give_xp_bonus, "Gives the player with the specified name an xp boost.  Usage: give_xp_bonus <name> <type> "
						   "<multiplier> <bonus_pool>")
{
	if(args.ArgC() != 5)
	{
	...
	if(nType < CMsgTFXPSource_XPSourceType_XPSourceType_MIN || nType >= CMsgTFXPSource_XPSourceType_NUM_SOURCE_TYPES)
	{
	...
	GTFGCClientSystem()->GetMatch()->GiveXPBonus(steamID, nType, atof(args.Arg(3)), atoi(args.Arg(4)));
}
#endif
...
bool CMatchInfo::CalculatePlayerMatchRankData(void)
{
	...
	if(!pMatchDesc || !pMatchDesc->m_pProgressionDesc || !pMatchDesc->m_params.m_bDistributePerformanceMedals)
	{
		return false;
	}
	...
	FOR_EACH_VEC(vecPlayers, i)
	{
		...
		uint32 unScoreMedal =
			GetRankForStat(RankStat_Score, nRankIndex, pTFResource->GetTotalScore(vecPlayers[i]->entindex()));
		uint32 unKillsMedal =
			GetRankForStat(RankStat_Kills, nRankIndex, pStats->statsAccumulated.m_iStat[TFSTAT_KILLS]);
		uint32 unDamageMedal =
			GetRankForStat(RankStat_Damage, nRankIndex, pStats->statsAccumulated.m_iStat[TFSTAT_DAMAGE]);
		uint32 unHealingMedal =
			GetRankForStat(RankStat_Healing, nRankIndex, pStats->statsAccumulated.m_iStat[TFSTAT_HEALING]);
		uint32 unSupportMedal =
			GetRankForStat(RankStat_Support, nRankIndex,
						   TFGameRules()->CalcPlayerSupportScore(&pStats->statsAccumulated, vecPlayers[i]->entindex()));
...
bool CMatchInfo::CalculateMatchSkillRatingAdjustments(int iWinningTeam)
{
	...
	if(pMatchDesc->m_params.m_bRequireCompleteMatch && (unWinningPlayers + unLosingPlayers != GetCanonicalMatchSize()))
	{
		...
		Log("CalculateMatchSkillRatingAdjustments(): Match %llu has invalid team size(s): %d vs %d\n", m_nMatchID,
			unWinningPlayers, unLosingPlayers);
	}

	int nTeamSize = (pMatch->GetCanonicalMatchSize() % 2) ? (pMatch->GetCanonicalMatchSize() / 2 + 1)
														  : (pMatch->GetCanonicalMatchSize() / 2);
	...
	const int nChange = RemapValClamped(
		nRatingDiff, /* from */ -(float)k_unDrilloRating_MaxDifference, (float)k_unDrilloRating_MaxDifference,
		/* to   */ (float)k_nDrilloRating_MinRatingAdjust, (float)k_nDrilloRating_Ladder_MaxRatingAdjust);

	// Cap loss for low-rated teams, but not low-rated winners.  This breaks the loose "sort-of-zero-sum" system we
	// have, but that's ok in the lower range.
	const int nLoserChange = (nLosingTeamAverage <= k_unDrilloRating_Ladder_LowSkill)
								 ? Min(nChange, k_nDrilloRating_Ladder_MaxLossAdjust_LowRank)
								 : nChange;
	...
	for(int i = 0; i < GetNumTotalMatchPlayers(); i++)
	{
		...
		if(pPlayerInfo->BDropWasAbandon())
		{
			...
			{
				GiveXPDirectly(pPlayerInfo->steamID,
							   CMsgTFXPSource_XPSourceType::CMsgTFXPSource_XPSourceType_SOURCE_COMPETITIVE_ABANDON,
							   nAmount);
			}
		}
...
int CMatchInfo::GetRankForStat(RankStatType_t statType, int nRankIndex, uint32 nValue)
{
	...
	uint16 nMatchDuration =
		CTF_GameStats.m_currentMap.m_Header.m_iTotalTime + (gpGlobals->curtime - TFGameRules()->GetRoundStart());
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		for(int i = 0; i < msg.Body().rankdata_size(); i++)
		{
			CMatchInfo::DailyStatsRankBucket_t rankBucket = {
				msg.Body().rankdata(i).rank(),		  msg.Body().rankdata(i).records(),
				msg.Body().rankdata(i).avg_score(),	  msg.Body().rankdata(i).stdev_score(),
				msg.Body().rankdata(i).avg_kills(),	  msg.Body().rankdata(i).stdev_kills(),
				msg.Body().rankdata(i).avg_damage(),  msg.Body().rankdata(i).stdev_damage(),
				msg.Body().rankdata(i).avg_healing(), msg.Body().rankdata(i).stdev_healing(),
				msg.Body().rankdata(i).avg_support(), msg.Body().rankdata(i).stdev_support()};
...
GC_REG_JOB(GCSDK::CGCClient, CGCCompetitiveDailyStatsRollupJob, "CGCCompetitiveDailyStatsRollupJob",
		   k_EMsgGC_DailyCompetitiveStatsRollup_Response, k_EServerTypeGCClient);
...
GC_REG_JOB(GCSDK::CGCClient, CGCVoteSystemVoteKickResponse, "CGCVoteSystemVoteKickResponse",
		   k_EMsgGCVoteKickPlayerRequestResponse, k_EServerTypeGCClient);
...
GC_REG_JOB(GCSDK::CGCClient, CGCKickPlayerFromLobbyJob, "CGCKickPlayerFromLobbyJob", k_EMsgGC_KickPlayerFromLobby,
...
void CTFGCServerSystem::PreClientUpdate()
{
	...
	if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
	{
		...
		{
			if(!spectatorVector[iIndex]->IsBot() && !spectatorVector[iIndex]->IsReplay() &&
			   !spectatorVector[iIndex]->IsHLTV())
			{
				spectatorCount++;
			}
		}
...
void CTFGCServerSystem::MatchPlayerAbandonThink()
{
	...
	for(int idx = 0; idx < nPlayers; idx++)
	{
		...
		{
			const CSteamID *pIndexSteamID =
				engine->GetClientSteamIDByPlayerIndex(pPlayer->nConnectingButNotActiveIndex);
		...
		if(!pPlayer->bConnected && !pPlayer->bDropped)
		{
			...
			if(nTimeGone > nAbandonSeconds)
			{
				...
				SetMatchPlayerDropped(pPlayer->steamID,
									  pPlayer->bEverConnected ? TFMatchLeaveReason_AWOL : TFMatchLeaveReason_NO_SHOW);
...
void CTFGCServerSystem::NewMatchForLobbyResponse(bool bSuccess)
{
	...
	if(pMatch && pMatch->m_bServerCreated)
	{
		...
		else
		{
			...
			MMLog("!! NewMatchForLobby responded negatively, this match will likely not be acknowledged by the "
				  "system.\n");
	...
	else
	{
		...
		if(!bSuccess)
		{
			MMLog("!! NewMatchForLobby responded negatively.  We haven't launched the match yet, so just shutting "
				  "down.\n");
			if(TFGameRules())
			{
...
void CTFGCServerSystem::SetMatchPlayerDropped(CSteamID steamID, TFMatchLeaveReason eReason)
{
	...
	switch(eReason)
	{
		case TFMatchLeaveReason_VOTE_KICK:
			// Vote kicks don't penalize you currently.  We need to revisit how these tie in with e.g. abuse
			// reports/etc..
			bAbandon = false;
			break;
...
void CTFGCServerSystem::FireGameEvent(IGameEvent *event)
{
	...
	else if(FStrEq(event->GetName(), "player_score_changed"))
	{
		...
		pMatch->GiveXPRewardToPlayerForAction(steamId, CMsgTFXPSource_XPSourceType_SOURCE_SCORE,
											  event->GetInt("delta", 0));
	}
}
...
void CTFGCServerSystem::SOCreated(const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject,
								  GCSDK::ESOCacheEvent eEvent)
{
	...
	if(pObject->GetTypeID() == CTFGSLobby::k_nTypeID)
	{
		...
		if(m_bMMServerMode && bNewLobby)
		{
			...
			//engine->GetServerVersion() ); 			engine->ServerCommand( "quit\n" );
			//		}
		}
		else
		{
...
void CTFGCServerSystem::SOUpdated(const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject,
								  GCSDK::ESOCacheEvent eEvent)
{
	...
	if(pObject->GetTypeID() == CTFGSLobby::k_nTypeID)
	{
		...
		bool bNewMatchID =
			m_pMatchInfo && !m_pMatchInfo->BMatchTerminated() && (m_pMatchInfo->m_nMatchID != pLobby->GetMatchID());
		if(bNewMatchID)
		{
...
void CTFGCServerSystem::SODestroyed(const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject,
									GCSDK::ESOCacheEvent eEvent)
{
	// Lobby handling
	if(pObject->GetTypeID() == CTFGSLobby::k_nTypeID)
...
const CTFGSLobby *CTFGCServerSystem::GetLobby() const
{
	...
	{
		AssertMsg1(pTypeCache->GetCount() == 1, "Server has %d lobby objects in his cache!  He should only have 1.",
				   pTypeCache->GetCount());
...
ConVar dbg_spew_connected_players_level("dbg_spew_connected_players_level", "0", FCVAR_NONE,
										"If enabled, server will spew connected player GC updates\n");

// Inform the GC of any change in the connected players
void CTFGCServerSystem::UpdateConnectedPlayersAndServerInfo(CMsgGameServerMatchmakingStatus_Event event,
															bool bForceSendMessages)
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		CMatchInfo::PlayerMatchData_t *pMatchPlayer =
			(pMatch && playerSteamID.IsValid()) ? pMatch->GetMatchDataForPlayer(playerSteamID) : NULL;
		...
		if(pLobby)
		{
			...
			CMsgGameServerMatchmakingStatus_PlayerConnectState sendPlayerConnectState =
		...
		for(int i = 0; i < pLobby->GetNumMembers(); i++)
		{
			...
			switch(eLobbyState)
			{
				case CTFLobbyMember_ConnectState_RESERVATION_PENDING:
				{
					...
					CMatchInfo::PlayerMatchData_t *pMatchPlayer =
						GetMatch() ? GetMatch()->GetMatchDataForPlayer(steamID) : NULL;
	...
	bool bRequestMatchLateJoin = bLiveMatch && nHumans < nMaxMatchPlayers && nClients < gpGlobals->maxClients &&
	...
	//
	if(pLobby && vecReservationRequests.Count() && (bNewMatch || bRequestMatchLateJoin) &&
	   ...
	   nUnconnectedPlayerReservationRequests + nClients <= gpGlobals->maxClients)
	{
		...
		bRequestMatchLateJoin = bRequestMatchLateJoin && nHumans < nMaxMatchPlayers &&
								nClients < gpGlobals->maxClients && pMatchDesc && pMatchDesc->ShouldRequestLateJoin();
	}
	...
	// state.
	if(vecConnectedPlayers.Count() == 0 && m_pMatchInfo && !pLobby && m_pMatchInfo->m_bMatchEnded)
	{
	...
	switch(TFGameRules()->State_Get())
	{
		...
		case GR_STATE_GAME_OVER:
			gcState = TF_GC_GAMESTATE_GAME_IN_PROGRESS;
			if(TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsCompetitiveMode()) // right?
			{
	...
	if(m_bMMServerMode)
	{
		...
		if(TFGameRules()->IsMannVsMachineMode())
		{
			const char *pszFilenameShort =
				g_pPopulationManager ? g_pPopulationManager->GetPopulationFilenameShort() : NULL;
		...
		if(!m_pMatchInfo && !pLobby)
		{
			...
			if(nClients >= gpGlobals->maxClients || nMaxHumans < 1 || (nHumans && tf_mm_strict.GetInt() == 1))
			{
				eGameServerInfoState = ServerMatchmakingState_NOT_PARTICIPATING;
				if(m_eLastGameServerUpdateState != eGameServerInfoState)
				{
	...
	if(event == CMsgGameServerMatchmakingStatus_Event_None && !bForceSendMessages &&
	   ...
	   Q_stricmp(m_sLastGameServerUpdateMap, sGameServerInfoMap) == 0)
	{
		...
		if((Q_stricmp(m_sLastGameServerUpdateTags, sGameServerInfoTags) != 0) ||
		   (nMaxHumans != m_nLastGameServerUpdateMaxHumans) || (nBotCountToSend != m_nLastGameServerUpdateBotCount))
		{
	...
	else
	{
		   ...
		   (event != CMsgGameServerMatchmakingStatus_Event_None && dbg_spew_connected_players_level.GetInt() >= 1))
		{
			MMLog("Sending CMsgGameServerMatchmakingStatus (state=%s, slots_free=%d, event=%s, %s)\n",
				  ServerMatchmakingState_Name(msg.Body().matchmaking_state()).c_str(), msg.Body().slots_free(),
...
//	}
// };
// GC_REG_JOB( GCSDK::CGCClient, CGCClientJobServerWelcome, "CGCClientJobServerWelcome", k_EMsgGCServerWelcome,
...
// **************************************************************************************************
void CTFGCServerSystem::AcceptGCReservation(CSteamID steamID, const CTFLobbyMember *pMemberData, bool bIsLateJoin,
											int nEntindex, bool bActive)
{
	if(m_pMatchInfo)
	{
...
void CTFGCServerSystem::LaunchNewMatchForLobby()
{
	...
	if(!pLobby || m_flWaitingForNewMatchTime == 0.f || !m_pMatchInfo || m_pMatchInfo->BMatchTerminated() ||
	   m_pMatchInfo->m_bServerCreated)
	{
...
ConVar tf_mm_servermode("tf_mm_servermode", "0", FCVAR_NOTIFY,
						...
						true, 0.f, true, 1.f, OnMMServerModeChanged);
...
ConVar tf_mm_trusted("tf_mm_trusted", "0", FCVAR_NOTIFY | FCVAR_HIDDEN,
					 "Set to 1 on Valve servers to requested trusted status.  (Yes, it is authenticated on the "
					 "backend, and attempts by non-valve servers are logged.)\n",
