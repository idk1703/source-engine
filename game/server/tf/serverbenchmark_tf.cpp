...
static ConVar sv_benchmark_freeroam("sv_benchmark_freeroam", "0", 0,
									"Allow the local player to move freely in the benchmark. Only used for debugging. "
									"Don't use for real benchmarks because it will make the timing inconsistent.");
...
public:
			...
			if(nSentries < 2)
			{
				...
				for(int i = 1; i <= gpGlobals->maxClients; i++)
				{
					...
					if(pPlayer && pPlayer->GetTeamNumber() == iTeam &&
					   pPlayer->GetPlayerClass()->GetClassIndex() == TF_CLASS_ENGINEER)
					{
	...
	virtual CBasePlayer *CreateBot()
	{
		...
		int iClass = g_pServerBenchmark->RandomInt(
			TF_FIRST_NORMAL_CLASS,
