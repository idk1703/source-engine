...
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_restart_game_allowed("sv_vote_issue_restart_game_allowed", "0", FCVAR_NONE,
										  "Can players call votes to restart the game?");
ConVar sv_vote_issue_restart_game_allowed_mvm("sv_vote_issue_restart_game_allowed_mvm", "1", FCVAR_NONE,
											  "Can players call votes to restart the game in Mann-Vs-Machine?");
ConVar sv_vote_issue_restart_game_cooldown("sv_vote_issue_restart_game_cooldown", "300", FCVAR_NONE,
...
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_kick_allowed("sv_vote_issue_kick_allowed", "0", FCVAR_NONE,
								  "Can players call votes to kick players from the server?");
ConVar sv_vote_issue_kick_allowed_mvm("sv_vote_issue_kick_allowed_mvm", "1", FCVAR_NONE,
									  "Can players call votes to kick players from the server in MvM?");
ConVar sv_vote_kick_ban_duration("sv_vote_kick_ban_duration", "20", FCVAR_NONE,
								 "The number of minutes a vote ban should last. (0 = Disabled)");
ConVar sv_vote_issue_kick_min_connect_time_mvm(
	"sv_vote_issue_kick_min_connect_time_mvm", "300", FCVAR_NONE,
	"How long a player must be connected before they can be kicked (in seconds).");
ConVar sv_vote_issue_kick_spectators_mvm("sv_vote_issue_kick_spectators_mvm", "1", FCVAR_NONE,
										 "Allow players to kick spectators in MvM.");
ConVar sv_vote_issue_kick_namelock_duration("sv_vote_issue_kick_namelock_duration", "120", FCVAR_NONE,
											"How long to prevent kick targets from changing their name (in seconds).");
ConVar sv_vote_issue_kick_limit_mvm(
	"sv_vote_issue_kick_limit_mvm", "0", FCVAR_HIDDEN,
	"The maximum number of kick votes a player can call during an MvM mission started by matchmaking. (0 = disabled)");
ConVar sv_vote_issue_kick_limit_gc("sv_vote_issue_kick_limit_gc", "0", FCVAR_HIDDEN,
								   "Ask the GC if a kick vote can be called by this player.  Official servers only.");
...
void CKickIssue::ExecuteCommand(void)
{
	...
	// Technically the GC might send them back here for *new* match if they get kicked as ad-hoc, the current match
	// ends, etc.
...
bool CKickIssue::CanCallVote(int iEntIndex, const char *pszDetails, vote_create_failed_t &nFailCode, int &nTime)
{
	...
	if((!engine->IsDedicatedServer() && m_hPlayerTarget->entindex() == 1) || m_hPlayerTarget->IsAutoKickDisabled())
	{
	...
	if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
	{
		...
		if(pTFVoteTarget)
		{
			...
			if(!bFakeClient)
			{
				CMatchInfo::PlayerMatchData_t *pMatchPlayerTarget =
					GTFGCClientSystem()->GetLiveMatchPlayer(m_steamIDVoteTarget);
				CMatchInfo::PlayerMatchData_t *pMatchPlayerCaller =
					GTFGCClientSystem()->GetLiveMatchPlayer(m_steamIDVoteCaller);
				if(pMatchPlayerTarget)
				{
					...
					if(sv_vote_issue_kick_limit_mvm.GetInt())
					{
						if(pMatchPlayerCaller &&
						   pMatchPlayerCaller->nVoteKickAttempts > (uint32)sv_vote_issue_kick_limit_mvm.GetInt())
						{
		...
		// Allow kicking of spectators when this is set, except when it's a bot (invader bots are spectators between
		// rounds)
		if(sv_vote_issue_kick_spectators_mvm.GetBool() && !m_hPlayerTarget->IsBot() &&
		   m_hPlayerTarget->GetTeamNumber() == TEAM_SPECTATOR)
			return true;
	}
	...
	if(m_steamIDVoteTarget.IsValid() && m_steamIDVoteTarget.BIndividualAccount())
	{
		...
		if(sv_vote_issue_kick_namelock_duration.GetFloat() > 0)
		{
			g_voteController->AddPlayerToNameLockedList(
				m_steamIDVoteTarget, sv_vote_issue_kick_namelock_duration.GetFloat(), m_hPlayerTarget->GetUserID());
		}
...
const char *CKickIssue::GetDisplayString(void)
{
	...
	{
		case kVoteKickBanPlayerReason_Other:
			return "#TF_vote_kick_player_other";
		case kVoteKickBanPlayerReason_Cheating:
			return "#TF_vote_kick_player_cheating";
		case kVoteKickBanPlayerReason_Idle:
			return "#TF_vote_kick_player_idle";
		case kVoteKickBanPlayerReason_Scamming:
...
{
	bool bFakeClient = m_hPlayerTarget &&
					   (m_hPlayerTarget->IsFakeClient() || m_hPlayerTarget->IsHLTV() || m_hPlayerTarget->IsReplay());

	UTIL_LogPrintf("Kick Vote details:  VoteInitiatorSteamID: %s  VoteTargetSteamID: %s  Valid: %i  BIndividual: %i  "
				   "Name: %s  Proxy: %i\n",
				   m_steamIDVoteCaller.IsValid() ? m_steamIDVoteCaller.Render() : "[unknown]",
				   m_steamIDVoteTarget.Render(), m_steamIDVoteTarget.IsValid(),
				   m_steamIDVoteTarget.BIndividualAccount(), m_hPlayerTarget ? m_szTargetPlayerName : "Disconnected",
				   bFakeClient);
}
...
// Purpose: Changelevel
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_changelevel_allowed("sv_vote_issue_changelevel_allowed", "0", FCVAR_NONE,
										 "Can players call votes to change levels?");
ConVar sv_vote_issue_changelevel_allowed_mvm("sv_vote_issue_changelevel_allowed_mvm", "0", FCVAR_NONE,
...
// Purpose: Nextlevel
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_nextlevel_allowed("sv_vote_issue_nextlevel_allowed", "1", FCVAR_NONE,
									   "Can players call votes to set the next level?");
ConVar sv_vote_issue_nextlevel_choicesmode("sv_vote_issue_nextlevel_choicesmode", "0", FCVAR_NONE,
										   "Present players with a list of lowest playtime maps to choose from?");
ConVar sv_vote_issue_nextlevel_allowextend("sv_vote_issue_nextlevel_allowextend", "1", FCVAR_NONE,
										   "Allow players to extend the current map?");
ConVar sv_vote_issue_nextlevel_prevent_change("sv_vote_issue_nextlevel_prevent_change", "1", FCVAR_NONE,
...
bool CNextLevelIssue::GetVoteOptions(CUtlVector<const char *> &vecNames)
{
	...
	int nNumOptions =
		sv_vote_issue_nextlevel_allowextend.GetBool() ? GetNumberVoteOptions() - 1 : GetNumberVoteOptions();
...
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_extendlevel_allowed("sv_vote_issue_extendlevel_allowed", "1", FCVAR_NONE,
										 "Can players call votes to set the next level?");
ConVar sv_vote_issue_extendlevel_quorum("sv_vote_issue_extendlevel_quorum", "0.6", FCVAR_NONE,
...
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_scramble_teams_allowed("sv_vote_issue_scramble_teams_allowed", "1", FCVAR_NONE,
											"Can players call votes to scramble the teams?");
ConVar sv_vote_issue_scramble_teams_cooldown("sv_vote_issue_scramble_teams_cooldown", "1200", FCVAR_NONE,
...
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_mvm_challenge_allowed("sv_vote_issue_mvm_challenge_allowed", "1", FCVAR_NONE,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMannVsMachineChangeChallengeIssue::CanCallVote(int iEntIndex, const char *pszDetails,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEnableTemporaryHalloweenIssue::CanCallVote(int iEntIndex, const char *pszDetails, vote_create_failed_t &nFailCode,
												 int &nTime)
{
	if(!CBaseTFIssue::CanCallVote(iEntIndex, pszDetails, nFailCode, nTime))
		return false;
...
static void SendVoteResponseToGC(bool bVoteResponse)
{
	if(GTFGCClientSystem())
	{
		...
		msgResponse.Body().set_server_response_code(
			bVoteResponse
				? CMsgGC_GameServer_UseServerModificationItem_Response::kServerModificationItemServerResponse_Accepted
		...
		if(bVoteResponse)
		{
			TFGameRules()->BroadcastSound(255, RandomInt(0, 100) <= 10 ? "Halloween.MerasmusHalloweenModeRare"
...
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_autobalance_allowed("sv_vote_issue_autobalance_allowed", "0", FCVAR_NONE,
										 "Can players call votes to enable or disable auto team balance?");
ConVar sv_vote_issue_autobalance_cooldown("sv_vote_issue_autobalance_cooldown", "300", FCVAR_NONE,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamAutoBalanceIssue::CanCallVote(int iEntIndex, const char *pszDetails, vote_create_failed_t &nFailCode,
										int &nTime)
{
	if(!CBaseTFIssue::CanCallVote(iEntIndex, pszDetails, nFailCode, nTime))
		return false;
...
//-----------------------------------------------------------------------------
#ifdef STAGING_ONLY
ConVar sv_vote_issue_classlimits_allowed("sv_vote_issue_classlimits_allowed", "1", FCVAR_NONE,
										 "Can players call votes to enable or disable per-class limits?");
#else
ConVar sv_vote_issue_classlimits_allowed("sv_vote_issue_classlimits_allowed", "0", FCVAR_NONE,
										 "Can players call votes to enable or disable per-class limits?");
#endif
ConVar sv_vote_issue_classlimits_allowed_mvm(
	"sv_vote_issue_classlimits_allowed_mvm", "0", FCVAR_NONE,
	"Can players call votes in Mann-Vs-Machine to enable or disable per-class limits?");
ConVar sv_vote_issue_classlimits_max("sv_vote_issue_classlimits_max", "4", FCVAR_NONE,
									 "Maximum number of players (per-team) that can be any one class.", true, 1.f,
									 false, 16.f);
ConVar sv_vote_issue_classlimits_max_mvm("sv_vote_issue_classlimits_max_mvm", "2", FCVAR_NONE,
										 "Maximum number of players (per-team) that can be any one class.", true, 1.f,
										 false, 16.f);
ConVar sv_vote_issue_classlimits_cooldown("sv_vote_issue_classlimits_cooldown", "300", FCVAR_NONE,
...
void CClassLimitsIssue::ExecuteCommand(void)
{
	...
	{
		int nLimit = (TFGameRules() && TFGameRules()->IsMannVsMachineMode())
						 ? sv_vote_issue_classlimits_max_mvm.GetInt()
						 : sv_vote_issue_classlimits_max.GetInt();
...
{
	int nLimit = (TFGameRules() && TFGameRules()->IsMannVsMachineMode()) ? sv_vote_issue_classlimits_max_mvm.GetInt()
																		 : sv_vote_issue_classlimits_max.GetInt();
...
// Purpose: Pause Game
//-----------------------------------------------------------------------------
ConVar sv_vote_issue_pause_game_allowed("sv_vote_issue_pause_game_allowed", "0", FCVAR_HIDDEN,
										"Can players call votes to pause the game?");
ConVar sv_vote_issue_pause_game_timer("sv_vote_issue_pause_game_timer", "120", FCVAR_HIDDEN,
									  "How long to pause the game for when this vote passes (in seconds).");
ConVar sv_vote_issue_pause_game_cooldown("sv_vote_issue_pause_game_cooldown", "1200", FCVAR_HIDDEN,
