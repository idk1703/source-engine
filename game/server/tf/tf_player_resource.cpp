...
SendPropArray3(SENDINFO_ARRAY3(m_iTotalScore),
			   SendPropInt(SENDINFO_ARRAY(m_iTotalScore), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iMaxHealth),
				   SendPropInt(SENDINFO_ARRAY(m_iMaxHealth), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iMaxBuffedHealth),
				   SendPropInt(SENDINFO_ARRAY(m_iMaxBuffedHealth), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	...
	SendPropArray3(SENDINFO_ARRAY3(m_iActiveDominations),
				   SendPropInt(SENDINFO_ARRAY(m_iActiveDominations), 6, SPROP_UNSIGNED)),
	...
	SendPropArray3(SENDINFO_ARRAY3(m_iDamage),
				   SendPropInt(SENDINFO_ARRAY(m_iDamage), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iDamageAssist),
				   SendPropInt(SENDINFO_ARRAY(m_iDamageAssist), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iDamageBoss),
				   SendPropInt(SENDINFO_ARRAY(m_iDamageBoss), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iHealing),
				   SendPropInt(SENDINFO_ARRAY(m_iHealing), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iHealingAssist),
				   SendPropInt(SENDINFO_ARRAY(m_iHealingAssist), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iDamageBlocked),
				   SendPropInt(SENDINFO_ARRAY(m_iDamageBlocked), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iCurrencyCollected),
				   SendPropInt(SENDINFO_ARRAY(m_iCurrencyCollected), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iBonusPoints),
				   SendPropInt(SENDINFO_ARRAY(m_iBonusPoints), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iPlayerLevel),
				   SendPropInt(SENDINFO_ARRAY(m_iPlayerLevel), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iStreaks),
				   SendPropInt(SENDINFO_ARRAY(m_iStreaks), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iUpgradeRefundCredits),
				   SendPropInt(SENDINFO_ARRAY(m_iUpgradeRefundCredits), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	SendPropArray3(SENDINFO_ARRAY3(m_iBuybackCredits),
				   SendPropInt(SENDINFO_ARRAY(m_iBuybackCredits), -1, SPROP_UNSIGNED | SPROP_VARINT)),
	...
	SendPropArray3(SENDINFO_ARRAY3(m_iPlayerClassWhenKilled),
				   SendPropInt(SENDINFO_ARRAY(m_iPlayerClassWhenKilled), 5, SPROP_UNSIGNED)),
	SendPropArray3(SENDINFO_ARRAY3(m_iConnectionState),
				   SendPropInt(SENDINFO_ARRAY(m_iConnectionState), 3, SPROP_UNSIGNED)),
END_SEND_TABLE
()
...
void CTFPlayerResource::UpdatePlayerData(void)
{
	...
	if(pMatch && TFGameRules())
	{
		for(int i = 0; i < pMatch->GetNumTotalMatchPlayers(); ++i)
		{
			...
			if(!pData->bDropped && pVecPlayers->Find(unAccountID) == pVecPlayers->InvalidIndex() &&
			   m_vecFreeSlots.Count() > 0)
			{
	...
	{
		m_flNextDamageAndHealingSend = gpGlobals->curtime +
STATS_SEND_FREQUENCY
;
}
...
void CTFPlayerResource::UpdateConnectedPlayer(int iIndex, CBasePlayer *pPlayer)
{
	...
	// m_iMaxBuffedHealth is misnamed -- it should be m_iMaxHealthForBuffing, but we don't want to change it now due to
	// demos.
	...
	float flRespawnTime =
		pTFPlayer->IsAlive() ? 0 : TFGameRules()->GetNextRespawnWave(pTFPlayer->GetTeamNumber(), pTFPlayer);
	...
	{
		m_iStreaks.Set(iIndex * CTFPlayerShared::kTFStreak_COUNT + streak_type,
					   pTFPlayer->m_Shared.GetStreak((CTFPlayerShared::ETFStreak)streak_type));
	}
	...
	CUtlVector<uint32> *pVecPlayers =
		(iTeam == TF_TEAM_RED) ? &m_vecRedPlayers : ((iTeam == TF_TEAM_BLUE) ? &m_vecBluePlayers : NULL);
	if(pVecPlayers)
	{
