...
ConVar bot_nav_useoffsetpaths(
	"bot_nav_useoffsetpaths", "1", FCVAR_CHEAT,
	"If set, bots will generate waypoints on both sides of portals between waypoints when building paths.");
ConVar bot_nav_offsetpathinset(
	"bot_nav_offsetpathinset", "20", FCVAR_CHEAT,
...
void GetBotsFromCommand(const CCommand &args, int iArgsRequired, const char *pszUsage,
...
//-----------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_kick, "Remove a bot by name, or an entire team (\"red\" or \"blue\"), or all bots (\"all\").",
			  FCVAR_CHEAT)
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; ++i)
	{
		...
		{
			if(FStrEq(args.Arg(1), "all") || FStrEq(args.Arg(1), player->GetPlayerName()) ||
	...
	while(--count >= 0)
	{
		...
		if(bTeleport && pTemp)
		{
			...
			if(pPlayer && pBot)
			{
				...
				UTIL_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH,
				...
				if(tr.fraction != 1.0)
				{
					...
					const char *pszClassname = (pBotData->m_WantedClass == TF_CLASS_RANDOM)
												   ? "random"
												   : GetPlayerClassData(pBotData->m_WantedClass)->m_szClassName;
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove(CTFPlayer *fakeclient, const QAngle &viewangles, float forwardmove, float sidemove,
						  float upmove, unsigned short buttons, byte impulse, float frametime)
{
	if(!fakeclient)
		return;
...
void Bot_Think(CTFPlayer *pBot)
{
	...
	else if(pBot->GetTeamNumber() != TEAM_UNASSIGNED && pBot->GetPlayerClass()->IsClass(TF_CLASS_UNDEFINED))
	{
		...
		const char *pszClassname = (botdata->m_WantedClass == TF_CLASS_RANDOM)
									   ? "random"
...
void botdata_t::Bot_AliveMovementThink(QAngle *vecAngles, Vector *vecMove)
{
	...
	if(HasPath())
	{
		...
		else if(bot_nav_simplifypaths.GetBool())
		{
			...
			while(m_iPathLength >= (m_iPathIndex + 1) && m_iPathIndex < (m_iPathLength - 1) &&
		...
		if(bot_debug.GetInt() == 1)
		{
			for(int i = 0; i < m_iPathLength - 1; i++)
			{
				...
				NDebugOverlay::Box(m_path[i].pos, -Vector(3, 3, 3), Vector(3, 3, 3), 0, i == m_iPathIndex ? 255 : 64, 0,
								   0, 0.1);
			}
		}
		...
		{
			NDebugOverlay::HorzArrow(m_hBot->GetAbsOrigin(), m_hBot->GetAbsOrigin() + vecDesiredVelocity, 3, 255, 0, 0,
									 0, true, 0.1);
		}
...
static ConCommand bot_equip("bot_equip", cc_bot_equip,
...
static ConCommand bot_disguise(
	"bot_disguise", cc_bot_disguise,
	"Force the specified bot to disguise themselves.\n\tFormat: bot_disguise <bot name> <team> <class>", FCVAR_CHEAT);
...
static ConCommand bot_taunt(
	"bot_taunt", cc_bot_taunt,
	"Force the specified bot to taunt with specific taunt name.\n\tFormat: bot_taunt <bot name> <taunt_name>",
	FCVAR_CHEAT);
...
//------------------------------------------------------------------------------
CON_COMMAND_F(cc_bot_selectweapon,
...
bool botdata_t::FindPathTo(Vector vecTarget)
{
	...
	if(NavAreaBuildPath(pCurrentArea, pTargetArea, &vecTarget, cost) == false)
	{
		Warning("Bot '%s' couldn't find a path from nav mesh %d to nav mesh %d.\n", m_hBot->GetPlayerName(),
				pCurrentArea->GetID(), pTargetArea->GetID());
		return false;
	}
...
bool botdata_t::ValidTargetPlayer(CTFPlayer *pPlayer, const Vector &vecStart, const Vector &vecEnd)
{
	...
	if(pPlayer->m_Shared.InCond(TF_COND_DISGUISED) && pPlayer->m_Shared.GetDisguiseTeam() == m_hBot->GetTeamNumber() &&
	   pPlayer != m_hEnemy)
		return false;

	// Not across water boundary.
	if((m_hBot->GetWaterLevel() == 0 && pPlayer->GetWaterLevel() >= 3) ||
	   (m_hBot->GetWaterLevel() == 3 && pPlayer->GetWaterLevel() <= 0))
		return false;
...
bool botdata_t::ValidTargetObject(CBaseObject *pObject, const Vector &vecStart, const Vector &vecEnd)
{
	...
	// Not across water boundary.
	if((m_hBot->GetWaterLevel() == 0 && pObject->GetWaterLevel() >= 3) ||
	   (m_hBot->GetWaterLevel() == 3 && pObject->GetWaterLevel() <= 0))
		return false;
...
bool botdata_t::ShouldFinishCommandOnArrival(void)
{
	return (m_Commands.Count() &&
			(m_Commands[0].iCommand == BC_MOVETO_ENTITY || m_Commands[0].iCommand == BC_MOVETO_POINT));
}
...
BEGIN_DATADESC(CTFBotController)
	DEFINE_KEYFIELD(m_iszBotName, FIELD_STRING, "bot_name"), DEFINE_KEYFIELD(m_iBotClass, FIELD_INTEGER, "bot_class"),
...
{
	m_hBot = ToTFPlayer(BotPutInServer(false, false, GetTeamNumber(), m_iBotClass ? m_iBotClass : TF_CLASS_RANDOM,
									   STRING(m_iszBotName)));
	if(m_hBot)
	{
