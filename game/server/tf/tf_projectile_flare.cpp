...
// Purpose:
//-----------------------------------------------------------------------------
CTFProjectile_Flare *CTFProjectile_Flare::Create(CBaseEntity *pLauncher, const Vector &vecOrigin,
												 const QAngle &vecAngles, CBaseEntity *pOwner, CBaseEntity *pScorer)
{
	CTFProjectile_Flare *pFlare = static_cast<CTFProjectile_Flare *>(
		CBaseEntity::CreateNoSpawn("tf_projectile_flare", vecOrigin, vecAngles, pOwner));
	if(!pFlare)
		return NULL;
	...
	if(pFlareGun)
	{
		if(pFlareGun->GetFlareGunType() == FLAREGUN_SCORCHSHOT)
		{
			...
			if(pTFVictim)
			{
				...
				CTakeDamageInfo info(this, pAttacker, m_hLauncher, vec3_origin, vecOrigin, GetDamage(), iDamageType,
									 m_bIsFromTaunt ? TF_DMG_CUSTOM_FLARE_PELLET : 0);
...
#endif
		CTakeDamageInfo info(this, pAttacker, m_hLauncher, vec3_origin, vecOrigin, GetDamage(),
							 bitsDamageType | DMG_HALF_FALLOFF, TF_DMG_CUSTOM_FLARE_EXPLOSION);
...
void CTFProjectile_Flare::PerformCustomPhysics(Vector *pNewPosition, Vector *pNewVelocity, QAngle *pNewAngles,
											   QAngle *pNewAngVelocity)
{
	if(m_flNextSeekUpdate < gpGlobals->curtime)
	{
		...
		for(i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if(!bBurning || pPlayer->InSameTeam(this) ||
			   (pPlayer->m_Shared.GetDisguiseTeam() == GetTeamNumber() && !bBurning) ||
			   (pPlayer->m_Shared.IsStealthed() && !bBurning) || pPlayer->GetTeamNumber() == TEAM_SPECTATOR ||
			   !pPlayer->IsAlive())
			{
				continue;
			}
			...
			{
				trace_t tr;
				UTIL_TraceLine(pPlayer->WorldSpaceCenter(), GetAbsOrigin(), MASK_SOLID_BRUSHONLY, pPlayer,
		...
		{
			pNewAngVelocity->x = Approach(UTIL_AngleDiff(angToTarget.x, pNewAngles->x) * flUpdatesPerSecond,
										  pNewAngVelocity->x, flHeatSeekPower);
			pNewAngVelocity->y = Approach(UTIL_AngleDiff(angToTarget.y, pNewAngles->y) * flUpdatesPerSecond,
										  pNewAngVelocity->y, flHeatSeekPower);
			pNewAngVelocity->z = Approach(UTIL_AngleDiff(angToTarget.z, pNewAngles->z) * flUpdatesPerSecond,
										  pNewAngVelocity->z, flHeatSeekPower);
