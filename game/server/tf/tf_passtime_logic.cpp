...
SendPropEHandle(SENDINFO(m_hBall)),
	...
	SendPropInt(SENDINFO(m_iNumSections)), SendPropInt(SENDINFO(m_iCurrentSection)),
	SendPropFloat(SENDINFO(m_flMaxPassRange)), SendPropInt(SENDINFO(m_iBallPower), 8),
	SendPropFloat(SENDINFO(m_flPackSpeed)),
	SendPropArray3(SENDINFO_ARRAY3(m_bPlayerIsPackMember),
				   SendPropInt(SENDINFO_ARRAY(m_bPlayerIsPackMember), 1, SPROP_UNSIGNED)),
END_SEND_TABLE
()

	//-----------------------------------------------------------------------------
	BEGIN_DATADESC(CTFPasstimeLogic) DEFINE_KEYFIELD(m_iNumSections, FIELD_INTEGER, "num_sections"),
	...
	DEFINE_INPUTFUNC(FIELD_STRING, "SetSection", InputSetSection), DEFINE_INPUTFUNC(FIELD_VOID, "TimeUp", InputTimeUp),
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "statica",
					 statica), // SecretRoom_InputStartTouchPlayerSlot NOTE: intentionally not in FGD
	DEFINE_INPUTFUNC(FIELD_VOID, "staticb",
					 staticb), // SecretRoom_InputEndTouchPlayerSlot NOTE: intentionally not in FGD
	...
	DEFINE_OUTPUT(m_onBallFree, "OnBallFree"), DEFINE_OUTPUT(m_onBallGetBlu, "OnBallGetBlu"),
	DEFINE_OUTPUT(m_onBallGetRed, "OnBallGetRed"), DEFINE_OUTPUT(m_onBallGetAny, "OnBallGetAny"),
	DEFINE_OUTPUT(m_onBallRemoved, "OnBallRemoved"), DEFINE_OUTPUT(m_onScoreBlu, "OnScoreBlu"),
	DEFINE_OUTPUT(m_onScoreRed, "OnScoreRed"), DEFINE_OUTPUT(m_onScoreAny, "OnScoreAny"),
	DEFINE_OUTPUT(m_onBallPowerUp, "OnBallPowerUp"), DEFINE_OUTPUT(m_onBallPowerDown, "OnBallPowerDown"),
END_DATADESC
()
...
static const CCountdownAnnouncer::TimeSounds sCountdownSoundsRoundBeginMerasmus = {
	"Announcer.RoundBegins60seconds", "Announcer.RoundBegins30seconds", "Announcer.RoundBegins10seconds",
	"Merasmus.RoundBegins5seconds",	  "Merasmus.RoundBegins4seconds",	"Merasmus.RoundBegins3seconds",
	"Merasmus.RoundBegins2seconds",	  "Merasmus.RoundBegins1seconds",
};
...
// Would belong in BaseEnity, but this this hacky so I'm just hiding it here.
//------------------------------------------------------------------------------
static void HookOutput(const char *pSourceName, string_t pTargetName, const char *pOutputName, const char *pInputName,
					   const char *pParameter = nullptr, int nTimesToFire = EVENT_FIRE_ALWAYS)
{
	...
	pAction->m_iParameter = pParameter ? AllocPooledString(pParameter) : string_t();
...
bool CTFPasstimeLogic::AddBallPower(int iPower)
{
	...
	else if(!bWasAboveThreshold && bIsAboveThreshold)
	{
		...
		SetContextThink(&CTFPasstimeLogic::BallPower_PowerThink,
						gpGlobals->curtime + tf_passtime_powerball_decay_delay.GetFloat(), "BallPower_PowerThink");
...
void CTFPasstimeLogic::BallPower_PowerThink()
{
	...
	{
		SetContextThink(&CTFPasstimeLogic::BallPower_PowerThink, gpGlobals->curtime, "BallPower_PowerThink");
		return;
	}

	float flTickTime = (pBall->GetTeamNumber() == TEAM_UNASSIGNED) ? tf_passtime_powerball_decaysec_neutral.GetFloat()
																   : tf_passtime_powerball_decaysec.GetFloat();

	SetContextThink(&CTFPasstimeLogic::BallPower_PowerThink, gpGlobals->curtime + flTickTime, "BallPower_PowerThink");
...
static uint64 CalcPackMemberBits(CTFPlayer *pBallCarrier)
{
	...
	for(int i = 1; i <= MAX_PLAYERS; ++i, nMask <<= 1)
	{
		...
		if(!pPlayer || !pPlayer->IsAlive() || (pPlayer->GetTeamNumber() != iCarrierTeam) ||
...
{
	SetContextThink(&CTFPasstimeLogic::BallPower_PackThink, gpGlobals->curtime, "BallPower_PackThink");
...
void CTFPasstimeLogic::FireGameEvent(IGameEvent *pEvent)
{
	...
	if(!V_strcmp(pEventName, "teamplay_round_stalemate"))
	{
		...
		CTF_GameStats.m_passtimeStats.summary.nRoundMaxSec = TFGameRules()->GetActiveRoundTimer()->GetTimeRemaining();
		RespawnBall();
	}
...
float CTFPasstimeLogic::CalcProgressFrac() const
{
	...
	Vector vecOrigin;
	{
		...
		vecOrigin = pCarrier ? pCarrier->GetAbsOrigin() : pBall->GetAbsOrigin();
	}
...
void CTFPasstimeLogic::OneSecStatsUpdateThink()
{
	...
	CTF_GameStats.m_passtimeStats.summary.nPlayersBlueMax =
		MAX(CTF_GameStats.m_passtimeStats.summary.nPlayersBlueMax, pBlue->GetNumPlayers());
	CTF_GameStats.m_passtimeStats.summary.nPlayersRedMax =
		MAX(CTF_GameStats.m_passtimeStats.summary.nPlayersRedMax, pRed->GetNumPlayers());
...
{
	MapEventStat(input.pActivator, GetBall(), &CTF_GameStats.m_passtimeStats.summary.nTotalSpeedBoosts,
...
{
	MapEventStat(input.pActivator, GetBall(), &CTF_GameStats.m_passtimeStats.summary.nTotalJumpPads,
...
{
	if(IsGamestatePlayable() && pGoal->BEnablePlayerScore() && pPlayer->m_Shared.HasPasstimeBall() &&
...
bool CTFPasstimeLogic::BCanPlayerPickUpBall(CTFPlayer *pPlayer, HudNotification_t *pReason) const
{
	...
	if(pPlayer->m_Shared.InCond(TF_COND_INVULNERABLE) || pPlayer->m_Shared.InCond(TF_COND_PHASE) ||
	   pPlayer->m_Shared.InCond(TF_COND_INVULNERABLE_WEARINGOFF))
	{
		if(pReason)
			*pReason = HUD_NOTIFY_PASSTIME_NO_INVULN;
		return false;
	}
	...
	{
		if(pReason)
			*pReason = HUD_NOTIFY_PASSTIME_NO_CLOAK;
		return false;
	}

	// let disguised spies pick up enemy ball, which amounts to interception and fake passes
	auto bEnemyBall =
		(pBall->GetTeamNumber() != TEAM_UNASSIGNED) && (pBall->GetTeamNumber() != pPlayer->GetTeamNumber());
	...
	{
		if(pReason)
			*pReason = HUD_NOTIFY_PASSTIME_NO_CARRY;
		return false;
	}
	...
	{
		if(pReason)
			*pReason = HUD_NOTIFY_PASSTIME_NO_TELE;
		return false;
	}
	...
	{
		if(pReason)
			*pReason = HUD_NOTIFY_PASSTIME_NO_TAUNT;
		return false;
	}

	if(!pPlayer->IsAllowedToPickUpFlag() ||
	   !pPlayer->IsAlive() // NOTE: it's possible to be !alive and !dead at the same time
	   || pPlayer->IsAwayFromKeyboard() || pPlayer->m_Shared.InCond(TF_COND_HALLOWEEN_GHOST_MODE) ||
	...
	{
		bool bCanHolster =
			pActiveWeapon->CanHolster() && !(pActiveWeapon->IsReloading() && pActiveWeapon->ReloadsSingly() &&
											 pActiveWeapon->CanOverload()); // semihack to fix beggars bazooka problems
		if(pActiveWeapon && (pActiveWeapon->GetWeaponID() != TF_WEAPON_PASSTIME_GUN) && !bCanHolster)
		{
			if(pReason)
				*pReason = HUD_NOTIFY_PASSTIME_NO_HOLSTER;
			return false;
		}
	}
	...
	{
		if(pReason)
			*pReason = HUD_NOTIFY_PASSTIME_NO_OOB;
		return false;
	}
...
void CTFPasstimeLogic::Score(CTFPlayer *pPlayer, int iTeam, int iPoints, bool bForceWin)
{
	...
	//
	{
		...
		for(unsigned short i = 0; i < m_ballLastHeldTimes.Count(); i++)
		{
			...
			if((pPossibleAssister->GetTeamNumber() == pPlayer->GetTeamNumber()) && (pPossibleAssister != pPlayer) &&
			   (flSecAgo < 10.0f) && (flSecAgo < flAssisterTime))
			{
	...
	{
		const char *pszSound =
			(iTeam == TF_TEAM_RED) ? "sf14.Merasmus.Soccer.GoalRed" : "sf14.Merasmus.Soccer.GoalBlue";
...
void CTFPasstimeLogic::OnPlayerTouchBall(CTFPlayer *pCatcher, CPasstimeBall *pBall)
{
	...
	{
		PasstimeGameEvents::PassCaught(pThrower->entindex(), pCatcher->entindex(), flFeet, pBall->GetAirtimeSec())
			.Fire();
		...
		if(pThrower->GetTeamNumber() == iCatcherTeam)
		{
				...
				if(iExperiment != EPasstimeExperiment_Telepass::None)
				{
					...
					switch(pThrower->GetTeamNumber())
					{
						case TF_TEAM_RED:
							...
							TE_TFParticleEffect(filterThrower, 0.0, "player_sparkles_red", throwerOrigin, vec3_angle,
												pThrower, PATTACH_ABSORIGIN);
							break;
						case TF_TEAM_BLUE:
							TE_TFParticleEffect(filterThrower, 0.0, "teleported_blue", throwerOrigin, vec3_angle);
							TE_TFParticleEffect(filterThrower, 0.0, "player_sparkles_blue", throwerOrigin, vec3_angle,
												pThrower, PATTACH_ABSORIGIN);
							break;
						default:
							break;
					...
					if(iExperiment == EPasstimeExperiment_Telepass::SwapWithCatcher)
					{
						...
						switch(pCatcher->GetTeamNumber())
						{
							case TF_TEAM_RED:
								...
								TE_TFParticleEffect(filterCatcher, 0.0, "player_sparkles_red", catcherOrigin,
													vec3_angle, pCatcher, PATTACH_ABSORIGIN);
								break;
							case TF_TEAM_BLUE:
								TE_TFParticleEffect(filterCatcher, 0.0, "teleported_blue", catcherOrigin, vec3_angle);
								TE_TFParticleEffect(filterCatcher, 0.0, "player_sparkles_blue", catcherOrigin,
													vec3_angle, pCatcher, PATTACH_ABSORIGIN);
								break;
							default:
								break;
			...
			for(int i = 0; i < m_ballLastPassTimes.Count(); i++)
			{
				if(m_ballLastPassTimes[i].first == pThrower)
				{
					m_ballLastPassTimes[i].second =
						toAdd.second; // replace old time rather than add a new pair to the vector
					skipTheRest = true;
					break;
				}
			}
		...
		else
		{
			...
			pCatcher->m_Shared.AddCond(TF_COND_PASSTIME_INTERCEPTION,
									   tf_passtime_speedboost_on_get_ball_time.GetFloat());
			pCatcher->m_Shared.AddCond(TF_COND_SPEED_BOOST, tf_passtime_speedboost_on_get_ball_time.GetFloat());
...
bool CTFPasstimeLogic::ShouldEndOvertime() const
{
	...
	int iWinningTeam = (iRedScore > iBluScore) ? TF_TEAM_RED : TF_TEAM_BLUE;
...
void CTFPasstimeLogic::InputSetSection(inputdata_t &input)
{
	...
	for(CPathTrack *pTrack = params.pSectionStart; pTrack; pTrack = pTrack->GetNext(), ++iTrackPoint)
	{
		...
		{
			Warning("Too many track_path in section (%i max, easily changed but must be fixed).",
					m_trackPoints.Count());
			return;
		}
...
void CTFPasstimeLogic::SecretRoom_UpdateTv(int iNumSlotsFilled)
{
	if(iNumSlotsFilled == 9)
	{
		...
		SecretRoom_PlayTvSound(&m_SecretRoom_pTvSound, m_SecretRoom_pTv->entindex(), "Passtime.Tv3", 1.0f);
	}
	else
	{
		// sound
		float volume = (float)(iNumSlotsFilled + 1) / 10.0f;
		const char *pSoundName = (iNumSlotsFilled >= 4) ? "Passtime.Tv2" : "Passtime.Tv1";

		SECRETROOM_LOG("  @@  SECRET ROOM: Update TV %i slots filled\n", iNumSlotsFilled);

		SecretRoom_PlayTvSound(&m_SecretRoom_pTvSound, m_SecretRoom_pTv->entindex(), pSoundName, volume);
...
struct SecretRoom_TriggerInfo
{
	...
	{0, "comillow", TF_CLASS_SCOUT, TF_TEAM_BLUE},	   {1, "unissubs", TF_CLASS_SOLDIER, TF_TEAM_RED},
	{2, "amment", TF_CLASS_PYRO, TF_TEAM_RED},		   {3, "memagold", TF_CLASS_DEMOMAN, TF_TEAM_BLUE},
	{4, "subcla", TF_CLASS_HEAVYWEAPONS, TF_TEAM_RED}, {5, "enempose", TF_CLASS_ENGINEER, TF_TEAM_BLUE},
	{6, "irlenous", TF_CLASS_MEDIC, TF_TEAM_RED},	   {7, "donked", TF_CLASS_SNIPER, TF_TEAM_RED},
	{8, "finear", TF_CLASS_SPY, TF_TEAM_BLUE},
};

//-----------------------------------------------------------------------------
static const SecretRoom_TriggerInfo &SecretRoom_GetSlotInfoForTrigger(const char *pTriggerName)
{
