...
ConVar sv_motd_unload_on_dismissal("sv_motd_unload_on_dismissal", "0", 0,
...
ConVar tf_playerstatetransitions("tf_playerstatetransitions", "-2", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
								 "tf_playerstatetransitions <ent index or -1 for all>. Show player state transitions.");
ConVar tf_playergib("tf_playergib", "1", FCVAR_NOTIFY, "Allow player gibbing. 0: never, 1: normal, 2: always", true, 0,
					true, 2);

ConVar tf_damageforcescale_other("tf_damageforcescale_other", "6.0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar tf_damageforcescale_self_soldier_rj("tf_damageforcescale_self_soldier_rj", "10.0",
										   FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar tf_damageforcescale_self_soldier_badrj("tf_damageforcescale_self_soldier_badrj", "5.0",
											  FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
...
ConVar tf_damage_multiplier_blue("tf_damage_multiplier_blue", "1.0", FCVAR_CHEAT,
								 "All incoming damage to a blue player is multiplied by this value");
ConVar tf_damage_multiplier_red("tf_damage_multiplier_red", "1.0", FCVAR_CHEAT,
								"All incoming damage to a red player is multiplied by this value");

ConVar tf_max_voice_speak_delay("tf_max_voice_speak_delay", "1.5", FCVAR_DEVELOPMENTONLY,
...
ConVar tf_deploying_bomb_time("tf_deploying_bomb_time", "1.90", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							  "Time to deploy bomb before the point of no return.");
ConVar tf_deploying_bomb_delay_time("tf_deploying_bomb_delay_time", "0.0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
									"Time to delay before deploying bomb.");

#ifdef TF_RAID_MODE
ConVar tf_raid_team_size("tf_raid_team_size", "5", FCVAR_NOTIFY, "Max number of Raiders");
ConVar tf_raid_respawn_safety_time("tf_raid_respawn_safety_time", "1.5", FCVAR_NOTIFY,
								   "Number of seconds of invulnerability after respawning");
ConVar tf_raid_allow_class_change("tf_raid_allow_class_change", "1", FCVAR_NOTIFY,
								  "If nonzero, allow invaders to change their class after leaving the safe room");
...
ConVar tf_boss_battle_team_size("tf_boss_battle_team_size", "5", FCVAR_NOTIFY,
								"Max number of players in Boss Battle mode");
ConVar tf_boss_battle_respawn_safety_time("tf_boss_battle_respawn_safety_time", "3", FCVAR_NOTIFY,
										  "Number of seconds of invulnerability after respawning");
ConVar tf_boss_battle_respawn_on_friends("tf_boss_battle_respawn_on_friends", "1", FCVAR_NOTIFY);
#endif

ConVar tf_mvm_death_penalty("tf_mvm_death_penalty", "0", FCVAR_NOTIFY | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
...
ConVar tf_space_thrust_scout("tf_space_thrust_scout", "40.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							 "How much thrust is added while holding jump");
ConVar tf_space_thrust_sniper("tf_space_thrust_sniper", "34.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							  "How much thrust is added while holding jump");
ConVar tf_space_thrust_spy("tf_space_thrust_spy", "35.0", FCVAR_CHEAT | FCVAR_REPLICATED,
						   "How much thrust is added while holding jump");
ConVar tf_space_thrust_pyro("tf_space_thrust_pyro", "35.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							"How much thrust is added while holding jump");
ConVar tf_space_thrust_soldier("tf_space_thrust_soldier", "33.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							   "How much thrust is added while holding jump");
ConVar tf_space_thrust_engy("tf_space_thrust_engy", "35.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							"How much thrust is added while holding jump");
ConVar tf_space_thrust_medic("tf_space_thrust_medic", "37.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							 "How much thrust is added while holding jump");
ConVar tf_space_thrust_heavy("tf_space_thrust_heavy", "33.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							 "How much thrust is added while holding jump");
ConVar tf_space_thrust_demo("tf_space_thrust_demo", "33.0", FCVAR_CHEAT | FCVAR_REPLICATED,
							"How much thrust is added while holding jump");
ConVar tf_space_thrust_use_rate("tf_space_thrust_use_rate", "2.0", FCVAR_CHEAT | FCVAR_REPLICATED,
								"How much fuel is used per tick");
ConVar tf_space_thrust_recharge_rate("tf_space_thrust_recharge_rate", "0.5", FCVAR_CHEAT | FCVAR_REPLICATED,
									 "How much fuel is recharged per tick");
ConVar tf_skip_intro_and_spectate("tf_skip_intro_and_spectate", "0", FCVAR_REPLICATED,
								  "Skip intro panels and start spectating.");
#endif

#ifdef STAGING_ONLY
ConVar tf_highfive_separation_forward("tf_highfive_separation_forward", "0", FCVAR_CHEAT,
									  "Forward distance between high five partners");
ConVar tf_highfive_separation_right("tf_highfive_separation_right", "0", FCVAR_CHEAT,
									"Right distance between high five partners");
#else
ConVar tf_highfive_separation_forward("tf_highfive_separation_forward", "0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
									  "Forward distance between high five partners");
ConVar tf_highfive_separation_right("tf_highfive_separation_right", "0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
									"Right distance between high five partners");
#endif

ConVar tf_highfive_max_range("tf_highfive_max_range", "150", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							 "The farthest away a high five partner can be");
ConVar tf_highfive_height_tolerance("tf_highfive_height_tolerance", "12", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
									"The maximum height difference allowed for two high-fivers.");
ConVar tf_highfive_debug("tf_highfive_debug", "0", FCVAR_NONE,
...
void CC_tf_debug_ballistic_targeting_mark_target(const CCommand &args)
{
	...
	trace_t result;
	UTIL_TraceLine(player->EyePosition(), player->EyePosition() + 2000.0f * forward, MASK_SHOT, player,
				   COLLISION_GROUP_NONE, &result);

	tf_debug_ballistic_target = result.endpos;
}
static ConCommand tf_debug_ballistic_targeting_mark_target("tf_debug_ballistic_targeting_mark_target",
														   CC_tf_debug_ballistic_targeting_mark_target,
														   "Mark a spot for testing ballistic targeting.", FCVAR_CHEAT);
...
ConVar tf_halloween_kart_impact_rate("tf_halloween_kart_impact_rate", "0.5f", FCVAR_CHEAT,
									 "rate of allowing impact damage");
ConVar tf_halloween_kart_boost_impact_force("tf_halloween_kart_boost_impact_force", "0.75f", FCVAR_CHEAT,
...
static const char *s_pszTauntRPSParticleNames[] = {"rps_rock_red",		"rps_paper_red",	  "rps_scissors_red",
												   "rps_rock_red_win",	"rps_paper_red_win",  "rps_scissors_red_win",
												   "rps_rock_blue",		"rps_paper_blue",	  "rps_scissors_blue",
												   "rps_rock_blue_win", "rps_paper_blue_win", "rps_scissors_blue_win"};
...
SendPropInt(SENDINFO(m_iPlayerIndex), 7, SPROP_UNSIGNED),
	...
	// BUGBUG:  ywb  we assume this is either 0 or an animation sequence #, but it could also be an activity, which
	// should fit within this limit, but we're not guaranteed.
...
void TE_PlayerAnimEvent(CBasePlayer *pPlayer, PlayerAnimEvent_t event, int nData)
{
	...
	if(!IsCustomPlayerAnimEvent(event) && (event != PLAYERANIMEVENT_SNAP_YAW) &&
	   (event != PLAYERANIMEVENT_VOICE_COMMAND_GESTURE))
	{
		// if prediction is off, alway send jump
		if(!((event == PLAYERANIMEVENT_JUMP) &&
			 (FStrEq(engine->GetClientConVarValue(pPlayer->entindex(), "cl_predict"), "0"))))
		{
...
SendPropVector(SENDINFO(m_vecRagdollOrigin), -1, SPROP_COORD), SendPropInt(SENDINFO(m_iPlayerIndex), 7, SPROP_UNSIGNED),
	SendPropVector(SENDINFO(m_vecForce), -1, SPROP_NOSCALE),
	SendPropVector(SENDINFO(m_vecRagdollVelocity), 13, SPROP_ROUNDDOWN, -2048.0f, 2048.0f),
	SendPropInt(SENDINFO(m_nForceBone)), SendPropBool(SENDINFO(m_bGib)), SendPropBool(SENDINFO(m_bBurning)),
	SendPropBool(SENDINFO(m_bElectrocuted)), SendPropBool(SENDINFO(m_bFeignDeath)),
	SendPropBool(SENDINFO(m_bWasDisguised)), SendPropBool(SENDINFO(m_bBecomeAsh)), SendPropBool(SENDINFO(m_bOnGround)),
	SendPropBool(SENDINFO(m_bCloaked)), SendPropInt(SENDINFO(m_iDamageCustom)),
	SendPropInt(SENDINFO(m_iTeam), 3, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_iClass), 4, SPROP_UNSIGNED),
	SendPropUtlVector(SENDINFO_UTLVECTOR(m_hRagWearables), 8, SendPropEHandle(NULL, 0)),
	SendPropBool(SENDINFO(m_bGoldRagdoll)), SendPropBool(SENDINFO(m_bIceRagdoll)),
	SendPropBool(SENDINFO(m_bCritOnHardHit)), SendPropFloat(SENDINFO(m_flHeadScale)),
	SendPropFloat(SENDINFO(m_flTorsoScale)), SendPropFloat(SENDINFO(m_flHandScale)),
END_SEND_TABLE
()
	...
	void *SendProxy_SendNonLocalDataTable(const SendProp *pProp, const void *pStruct, const void *pVarData,
...
//-----------------------------------------------------------------------------
void SendProxy_PlayerObjectList(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut,
...
//-----------------------------------------------------------------------------
void *SendProxy_SendHealersDataTable(const SendProp *pProp, const void *pStruct, const void *pVarData,
									 CSendProxyRecipients *pRecipients, int objectID)
{
	CTFPlayer *pPlayer = (CTFPlayer *)pStruct;
	if(pPlayer)
...
BEGIN_SEND_TABLE_NOBASE(CTFPlayer, DT_TFLocalPlayerExclusive)
	...
	SendPropVectorXY(SENDINFO(m_vecOrigin), -1, SPROP_NOSCALE | SPROP_CHANGES_OFTEN, 0.0f, HIGH_DEFAULT,
					 SendProxy_OriginXY),
		SendPropFloat(SENDINFO_VECTORELEM(m_vecOrigin, 2), -1, SPROP_NOSCALE | SPROP_CHANGES_OFTEN, 0.0f, HIGH_DEFAULT,
					  SendProxy_OriginZ),
		SendPropArray2(SendProxyArrayLength_PlayerObjects,
					   SendPropInt("player_object_array_element", 0, SIZEOF_IGNORE, NUM_NETWORKED_EHANDLE_BITS,
								   SPROP_UNSIGNED, SendProxy_PlayerObjectList),
					   MAX_OBJECTS_PER_PLAYER, 0, "player_object_array"),

		SendPropFloat(
			SENDINFO_VECTORELEM(m_angEyeAngles, 0), 8, SPROP_CHANGES_OFTEN, -90.0f,
			90.0f), // No longer used by the local player, could be omitted. Preserved for backwards-compat for now.
					//	SendPropAngle( SENDINFO_VECTORELEM(m_angEyeAngles, 1), 10, SPROP_CHANGES_OFTEN ),

		SendPropBool(SENDINFO(m_bIsCoaching)), SendPropEHandle(SENDINFO(m_hCoach)),
...
BEGIN_SEND_TABLE_NOBASE(CTFPlayer, DT_TFNonLocalPlayerExclusive)
	...
	SendPropVectorXY(SENDINFO(m_vecOrigin), -1, SPROP_COORD_MP_LOWPRECISION | SPROP_CHANGES_OFTEN, 0.0f, HIGH_DEFAULT,
					 SendProxy_OriginXY),
		SendPropFloat(SENDINFO_VECTORELEM(m_vecOrigin, 2), -1, SPROP_COORD_MP_LOWPRECISION | SPROP_CHANGES_OFTEN, 0.0f,
					  HIGH_DEFAULT, SendProxy_OriginZ),
...
SendPropExclude("DT_BaseAnimating", "m_flPoseParameter"), SendPropExclude("DT_BaseAnimating", "m_flPlaybackRate"),
	SendPropExclude("DT_BaseAnimating", "m_nSequence"), SendPropExclude("DT_BaseAnimating", "m_nBody"),
	SendPropExclude("DT_BaseEntity", "m_angRotation"), SendPropExclude("DT_BaseAnimatingOverlay", "overlay_vars"),
	SendPropExclude("DT_BaseEntity", "m_nModelIndex"), SendPropExclude("DT_BaseEntity", "m_vecOrigin"),

	// cs_playeranimstate and clientside animation takes care of these on the client
	SendPropExclude("DT_ServerAnimationData", "m_flCycle"), SendPropExclude("DT_AnimTimeMustBeFirst", "m_flAnimTime"),

	SendPropExclude("DT_BaseFlex", "m_flexWeight"), SendPropExclude("DT_BaseFlex", "m_blinktoggle"),
	SendPropExclude("DT_BaseFlex", "m_viewtarget"),

	SendPropBool(SENDINFO(m_bSaveMeParity)), SendPropBool(SENDINFO(m_bIsMiniBoss)), SendPropBool(SENDINFO(m_bIsABot)),
	...
	SendPropDataTable("tfnonlocaldata", 0, &REFERENCE_SEND_TABLE(DT_TFNonLocalPlayerExclusive),
					  SendProxy_SendNonLocalDataTable),

	SendPropBool(SENDINFO(m_bAllowMoveDuringTaunt)), SendPropBool(SENDINFO(m_bIsReadyToHighFive)),
	SendPropEHandle(SENDINFO(m_hHighFivePartner)), SendPropInt(SENDINFO(m_nForceTauntCam), 2, SPROP_UNSIGNED),
	SendPropFloat(SENDINFO(m_flTauntYaw), 0, SPROP_NOSCALE), SendPropInt(SENDINFO(m_nActiveTauntSlot)),
	SendPropInt(SENDINFO(m_iTauntItemDefIndex)), SendPropFloat(SENDINFO(m_flCurrentTauntMoveSpeed)),
	...
	SendPropInt(SENDINFO(m_iSpawnCounter)), SendPropBool(SENDINFO(m_bArenaSpectator)),
	SendPropFloat(SENDINFO(m_flHeadScale)), SendPropFloat(SENDINFO(m_flTorsoScale)),
	...
	SendPropBool(SENDINFO(m_bForcedSkin)), SendPropInt(SENDINFO(m_nForcedSkin), ANIMATION_SKIN_BITS),

	SendPropDataTable(SENDINFO_DT(m_AttributeManager), &REFERENCE_SEND_TABLE(DT_AttributeManager)),

	SendPropDataTable("TFSendHealersDataTable", 0, &REFERENCE_SEND_TABLE(DT_TFSendHealersDataTable),
					  SendProxy_SendHealersDataTable),

	SendPropFloat(SENDINFO(m_flKartNextAvailableBoost)), SendPropInt(SENDINFO(m_iKartHealth)),
	SendPropInt(SENDINFO(m_iKartState)), SendPropEHandle(SENDINFO(m_hGrapplingHookTarget)),
	SendPropEHandle(SENDINFO(m_hSecondaryLastWeapon)), SendPropBool(SENDINFO(m_bUsingActionSlot)),
	SendPropFloat(SENDINFO(m_flInspectTime)), SendPropInt(SENDINFO(m_iCampaignMedals)),
...
ConCommand cc_CreatePredictionError("CreatePredictionError", cc_CreatePredictionError_f, "Create a prediction error",
									FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
...
static void HandleCoachCommand(CTFPlayer *pPlayer, eCoachCommand command)
{
	if(pPlayer && pPlayer->IsCoaching() && pPlayer->GetStudent() && command < kNumCoachCommands)
	{
		...
		if(pEvent)
		{
			...
			UTIL_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + vForward * MAX_TRACE_LENGTH, MASK_SOLID,
						   &filter, &trace);

			CBaseEntity *pHitEntity =
				trace.m_pEnt && trace.m_pEnt->IsWorld() == false && trace.m_pEnt != pPlayer->GetStudent() ? trace.m_pEnt
																										  : NULL;
...
void CTFPlayer::SetGrapplingHookTarget(CBaseEntity *pTarget, bool bShouldBleed /*= false*/)
{
	...
	if(pTarget)
	{
		if(pTarget->IsPlayer())
		{
			...
			if(bShouldBleed)
			{
				CTFGrapplingHook *pGrapplingHook =
					dynamic_cast<CTFGrapplingHook *>(GetEntityForLoadoutSlot(LOADOUT_POSITION_ACTION));
				if(pGrapplingHook)
			...
			if(!pTargetPlayer->m_Shared.InCond(TF_COND_GRAPPLINGHOOK_BLEEDING) &&
			   pTargetPlayer->m_nHookAttachedPlayers > 0)
			{
...
void CTFPlayer::TFPlayerThink()
{
	...
	if(m_Shared.InCond(TF_COND_HALLOWEEN_KART))
	{
		...
		for(int i = 0; i < ghostVector.Count(); ++i)
		{
			...
			if(pGhost->GetTeamNumber() == GetTeamNumber())
			{
				...
				trace_t trace;
				Ray_t ray;
				ray.Init(pGhost->GetAbsOrigin(), pGhost->GetAbsOrigin(), pGhost->GetPlayerMins(),
						 pGhost->GetPlayerMaxs());
				...
				if(trace.fraction == 1.0f)
				{
					...
					pGhost->m_Shared.AddCond(
						TF_COND_HALLOWEEN_IN_HELL); // keep you in hell to be able to respawn as ghost
			...
			else if(tf_halloween_allow_ghost_hit_by_kart_delay.GetFloat() > 0 &&
					gpGlobals->curtime - pGhost->m_flGhostLastHitByKartTime >
						tf_halloween_allow_ghost_hit_by_kart_delay.GetFloat())
			{
				...
				vPuntDir.z = 0.5f;
				pGhost->ApplyAirBlastImpulse(tf_halloween_kart_punting_ghost_force_scale.GetFloat() * flImpactForce *
											 vPuntDir);
	...
	if(IsAlive() && !IsReadyToTauntWithPartner() && (m_flNextSpeakWeaponFire < gpGlobals->curtime) &&
	   m_flNextRandomExpressionTime >= 0 && gpGlobals->curtime > m_flNextRandomExpressionTime)
	{
	...
	if(IsTaunting())
	{
		...
		if(!m_strTauntSoundLoopName.IsEmpty() && m_flTauntSoundLoopTime > 0 &&
		   m_flTauntSoundLoopTime <= gpGlobals->curtime)
		{
	...
	if(IsTaunting())
	{
		...
		if(!CanMoveDuringTaunt())
		{
			...
			if(m_TauntEconItemView.IsValid() &&
			   m_TauntEconItemView.GetStaticData()->GetTauntData()->ShouldStopTauntIfMoved())
	...
	if(m_bIsTeleportingUsingEurekaEffect)
	{
		...
		if(!IsTaunting())
		{
			...
			CObjectTeleporter *pTeleExit =
				assert_cast<CObjectTeleporter *>(GetObjectOfType(OBJ_TELEPORTER, MODE_TELEPORTER_EXIT));

			// Check if they wanted to go to their teleporter AND their teleporter can accept them
			if(m_eEurekaTeleportTarget == EUREKA_TELEPORT_TELEPORTER_EXIT && pTeleExit &&
			   (pTeleExit->GetState() != TELEPORTER_STATE_BUILDING))
			{
	...
	bool bSendClipInfo = gpGlobals->curtime > m_flNextClipSendTime && m_Shared.GetNumHealers() && IsAlive();
	if(bSendClipInfo)
	{
		...
		if(pTFWeapon)
		{
			...
			{
				nClip =
					pTFWeapon->UsesClipsForAmmo1() ? pTFWeapon->Clip1() : GetAmmoCount(pTFWeapon->GetPrimaryAmmoType());
			}
	...
	if(GetGrapplingHookTarget() && GetGrapplingHookTarget()->IsPlayer() &&
	   m_Shared.GetCarryingRuneType() == RUNE_PLAGUE)
	{
		...
		if(flDistSqrToTarget < 8100 && !pHookedPlayer->m_Shared.InCond(TF_COND_PLAGUE) &&
		   !m_Shared.IsAlly(pHookedPlayer) && !pHookedPlayer->m_Shared.IsInvulnerable() &&
	...
	/*
		...
		{
			// JetPack testing
			if ( m_nButtons & IN_JUMP && !( GetFlags() & FL_ONGROUND ) && m_Shared.GetSpaceJumpChargeMeter() >
	tf_space_thrust_use_rate.GetFloat() )
			{
				...
				m_Shared.SetSpaceJumpChargeMeter( m_Shared.GetSpaceJumpChargeMeter() -
	tf_space_thrust_use_rate.GetFloat() );
			}
			else
			{
				...
				{
					m_Shared.SetSpaceJumpChargeMeter( m_Shared.GetSpaceJumpChargeMeter() +
	tf_space_thrust_recharge_rate.GetFloat() );
				}
			}
		}
	#endif
...
void CTFPlayer::PreThink()
{
	...
	if(tf_debug_ballistic_targeting.GetBool())
	{
		...
		else
		{
			...
			{
				// too far
				NDebugOverlay::Cross3D(tf_debug_ballistic_target, 10.0f, 0, 0, 255, true,
									   NDEBUG_PERSIST_TILL_NEXT_SERVER);
				NDebugOverlay::VertArrow(tf_debug_ballistic_target, tf_debug_ballistic_target - Vector(0, 0, error),
										 10.0f, 0, 0, 255, 255, true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
			}
			else
			{
				// too near
				NDebugOverlay::Cross3D(tf_debug_ballistic_target, 10.0f, 255, 0, 0, true,
									   NDEBUG_PERSIST_TILL_NEXT_SERVER);
				NDebugOverlay::VertArrow(tf_debug_ballistic_target, tf_debug_ballistic_target + Vector(0, 0, error),
										 10.0f, 255, 0, 0, 255, true, NDEBUG_PERSIST_TILL_NEXT_SERVER);
			}
		}
	}
#endif // STAGING_ONLY
...
ConVar mp_idledealmethod("mp_idledealmethod", "1", FCVAR_GAMEDLL,
						 "Deals with Idle Players. 1 = Sends them into Spectator mode then kicks them if they're still "
						 "idle, 2 = Kicks them out of the game;");
	...
	if(mp_idledealmethod.GetInt())
	{
		...
		if(m_bIsAFK == true)
		{
			...
			if((mp_allowspectators.IsValid() && mp_allowspectators.GetBool() == false) ||
			   (TFGameRules()->IsInArenaMode() && tf_arena_use_queue.GetBool()))
			{
...
void CTFPlayer::UpdateHalloween(void)
{
	...
	if(!m_vHalloweenKartPush.IsZero())
	{
		if(!m_Shared.InCond(TF_COND_INVULNERABLE_USER_BUFF))
		{
			...
			{
				TE_TFParticleEffect(filter, 0.0, "kartimpacttrail", GetAbsOrigin(), vec3_angle, this,
			...
			{
				m_Shared.StunPlayer(flStunDuration * tf_halloween_kart_stun_duration_scale.GetFloat(),
									tf_halloween_kart_stun_amount.GetFloat(), TF_STUN_BOTH | TF_STUN_NO_EFFECTS);
			}
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayer::AddHalloweenKartPushEvent(CTFPlayer *pOther, CBaseEntity *pInflictor, CBaseEntity *pWeapon,
										  Vector vForce, int iDamage, int iDamageType /* = 0 */)
{
	...
	if(pOther && iDamage > 10 && CTFMinigameLogic::GetMinigameLogic() &&
	   CTFMinigameLogic::GetMinigameLogic()->GetActiveMinigame() &&
	   CTFMinigameLogic::GetMinigameLogic()->GetActiveMinigame()->GetMinigameType() ==
		   CTFMiniGame::EMinigameType::MINIGAME_HALLOWEEN2014_COLLECTION)
	{
	...
	//	vForce.x, vForce.y, vForce.z, vForce.Length(), flNewKartKnockbackMultiplier, vForce.Length() *
	//flNewKartKnockbackMultiplier, iDamage, GetPlayerClass()->GetClassIndex() );
...
void CTFPlayer::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper)
{
	...
	if(IsTaunting() && m_flNextAllowTauntRemapInputTime >= 0.f &&
	   m_flNextAllowTauntRemapInputTime <= gpGlobals->curtime)
	{
	...
	if(GetTeamNumber() > LAST_SHARED_TEAM)
	{
		...
		else
		{
			if(TFGameRules() && TFGameRules()->IsInArenaMode() && tf_arena_force_class.GetBool() == true &&
			   tf_arena_use_queue.GetBool() == true)
			{
				bRetVal = true;
			}
		}
...
void CTFPlayer::Spawn()
{
	...
	if(m_Shared.InState(TF_STATE_ACTIVE) || bMatchSummary)
	{
		...
		// If this is true it means I respawned without dying (changing class inside the spawn room) but doesn't
		// necessarily mean that my healers have stopped healing me This means that medics can still be linked to me but
		// my health would not be affected since this condition is not set. So instead of going and forcing every healer
		// on me to stop healing we just set this condition back on. If the game decides I shouldn't be healed by
		// someone (LOS, Distance, etc) they will break the link themselves like usual.
	...
	if(TFGameRules())
	{
		...
		if(TFGameRules()->ArePlayersInHell())
		{
			...
			else if(TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_DOOMSDAY) &&
...
void CTFPlayer::ManageBuilderWeapons(TFPlayerClassData_t *pData)
{
	...
	for(int i = 0; i < OBJ_LAST; ++i)
	{
...
#ifdef STAGING_ONLY
		int nLoadoutPos =
			(GetObjectInfo(i)->m_bRequiresOwnBuilder) ? LOADOUT_POSITION_BUILDING2 : LOADOUT_POSITION_BUILDING;
#else
		...
		if(pBuilder)
		{
			...
			if(pCurrentBuilderItemView == NULL || pLoadoutBuilderItemView == NULL ||
			   !ItemsMatch(pData, pCurrentBuilderItemView, pLoadoutBuilderItemView, pBuilder))
			{
		...
		if(!pBuilder ||
		   (GetObjectInfo(i)->m_bRequiresOwnBuilder && !(CTFPlayerSharedUtils::GetBuilderForObjectType(this, i))))
		{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPlayer::ItemsMatch(TFPlayerClassData_t *pData, CEconItemView *pCurWeaponItem, CEconItemView *pNewWeaponItem,
						   CTFWeaponBase *pWpnEntity)
{
	...
	if(bHasNonBaseWeapon)
	{
		...
		if(pWpnEntity)
		{
			const char *pszCurWeaponClass = pWpnEntity->GetClassname(),
					   *pszNewWeaponTransClass = TranslateWeaponEntForClass(
						   pNewWeaponItem->GetStaticData()->GetItemClass(), GetPlayerClass()->GetClassIndex());
	...
	{
		if(pCurWeaponItem->GetItemQuality() != AE_NORMAL ||
		   (pCurWeaponItem->GetItemDefIndex() != pNewWeaponItem->GetItemDefIndex()))
		{
...
void CTFPlayer::ManageRegularWeapons(TFPlayerClassData_t *pData)
{
	...
	else
	{
		...
		if(iClass > TF_CLASS_UNDEFINED && iClass < TF_CLASS_COUNT)
		{
			...
			for(int i = 0; i < CLASS_LOADOUT_POSITION_COUNT; i++)
			{
				...
				if((i == LOADOUT_POSITION_ACTION) && IsBot() && TFGameRules() && TFGameRules()->IsMannVsMachineMode() &&
				...
				if(IsTauntSlot(i))
				{
					...
					// This has to be done before the continue for "no_entity", because we're trying to precache taunts
					// which explicitly bail out there.
				...
				if(pItem->GetStaticData()->GetItemClass() &&
				   FStrEq(pItem->GetStaticData()->GetItemClass(), "no_entity"))
					continue;
				...
				{
					CEconEntity *pNewItem =
						dynamic_cast<CEconEntity *>(GiveNamedItem(pItem->GetStaticData()->GetItemClass(), 0, pItem));
					Assert(pNewItem);
					if(pNewItem)
					{
	...
	if((pCurr && pCurr->GetAttributeContainer()->GetItem()->GetEquippedPositionForClass(
					 GetPlayerClass()->GetClassIndex()) == LOADOUT_POSITION_ACTION) ||
	   (pPrev && pPrev->GetAttributeContainer()->GetItem()->GetEquippedPositionForClass(
					 GetPlayerClass()->GetClassIndex()) == LOADOUT_POSITION_ACTION) ||
	...
	if(m_bRegenerating == false)
	{
		...
		if(m_bRememberActiveWeapon && m_iActiveWeaponTypePriorToDeath)
		{
			...
			if(pWeapon && pWeapon->GetAttributeContainer()->GetItem()->GetEquippedPositionForClass(
							  GetPlayerClass()->GetClassIndex()) != LOADOUT_POSITION_ACTION)
			{
	...
	{
		if(m_Inventory.ClassLoadoutHasChanged(GetPlayerClass()->GetClassIndex()) || (m_bSwitchedClass) ||
...
CEconItemView *CTFPlayer::GetLoadoutItem(int iClass, int iSlot, bool bReportWhitelistFails)
{
	...
	if((pItem && pItem->IsValid()) && (pItem->GetItemQuality() != AE_NORMAL) &&
	   !pItem->GetStaticData()->IsAllowedInMatch() && TFGameRules()->IsInTournamentMode())
	{
...
void CTFPlayer::ValidateWeapons(TFPlayerClassData_t *pData, bool bResetWeapons)
{
	...
	for(int i = 0; i < MAX_WEAPONS; i++)
	{
		...
		int iLoadoutSlot = pWeapon->GetAttributeContainer()->GetItem()->GetStaticData()->GetLoadoutSlot(
		...
		if(bForceRemoved || !ItemsMatch(pData, pWeapon->GetAttributeContainer()->GetItem(), pItem, pWeapon))
		{
			...
			if(!bForceRemoved && m_bRegenerating)
			{
				...
				if(pDroppedItem->GetAccountID() != steamID.GetAccountID())
				{
					...
					CTFDroppedWeapon *pDroppedWeapon = CTFDroppedWeapon::Create(this, vecPackOrigin, vecPackAngles,
																				pWeapon->GetWorldModel(), pDroppedItem);
					if(pDroppedWeapon)
					{
	...
	if((IsPlayerClass(TF_CLASS_SOLDIER) || IsPlayerClass(TF_CLASS_PYRO) || IsPlayerClass(TF_CLASS_SNIPER)) &&
	   !bFoundBuffItem)
	{
		m_Shared.SetRageMeter(0.0f);
	}
...
void CTFPlayer::ValidateWearables(TFPlayerClassData_t *pData)
{
	...
	for(int wbl = m_hMyWearables.Count() - 1; wbl >= 0; wbl--)
	{
		...
		if(pEntity)
		{
			...
			if(iLoadoutSlot >= 0)
			{
				CEconItemView *pItem = TFInventoryManager()->GetItemInLoadoutForClass(GetPlayerClass()->GetClassIndex(),
																					  iLoadoutSlot, &steamIDForPlayer);
		...
		{
			// Regular Wearable
			int iLoadoutSlot = pWearable->GetAttributeContainer()->GetItem()->GetStaticData()->GetLoadoutSlot(
				GetPlayerClass()->GetClassIndex());
			if(iLoadoutSlot >= 0)
			{
				CEconItemView *pItem = TFInventoryManager()->GetItemInLoadoutForClass(GetPlayerClass()->GetClassIndex(),
				...
				// Item says what slot it wants to be in, but Misc's and Taunts can be in multiple places, check against
				// all
				...
				if(bLoadoutMisc || bLoadoutTaunt)
				{
					for(int i = LOADOUT_POSITION_INVALID + 1; i < CLASS_LOADOUT_POSITION_COUNT; i++)
					{
						...
						{
							pItem = TFInventoryManager()->GetItemInLoadoutForClass(GetPlayerClass()->GetClassIndex(), i,
																				   &steamIDForPlayer);
							itemMatch |= ItemsMatch(pData, pWearable->GetAttributeContainer()->GetItem(), pItem);
						}
					}
		...
		if(!itemMatch || pWearable->GetTeamNumber() != GetTeamNumber() || m_bForceItemRemovalOnRespawn ||
		   m_bSwitchedClass)
		{
...
void CTFPlayer::ManageRegularWeaponsLegacy(TFPlayerClassData_t *pData)
{
	...
	for(int iWeapon = 0; iWeapon < TF_PLAYER_WEAPON_COUNT; ++iWeapon)
	{
		if(pData->m_aWeapons[iWeapon] != TF_WEAPON_NONE)
		{
			...
			if(pWeapon)
			{
				...
				if(pItem)
				{
					...
					if(pWeapon->GetAttributeContainer()->GetItem()->GetItemID() != pItem->GetItemID())
					{
						...
						/*
						Msg("Removing %s because its global index (%d) doesn't match the loadout's (%d)\n",
						pWeapon->GetDebugName(), pWeapon->GetAttributeContainer()->GetItem()->GetItemID(),
						pItem->GetItemID() );
						*/
					}
				}
			...
			if(pWeapon)
			{
				Assert(pWeapon->GetAttributeContainer()->GetItem()->GetItemID() ==
					   (pItem ? pItem->GetItemID() : INVALID_ITEM_ID));
				...
				// char tempstr[1024];
				// g_pVGuiLocalize->ConvertUnicodeToANSI( pWeapon->GetAttributeContainer()->GetItem()->GetItemName(),
				// tempstr, sizeof(tempstr) ); Msg("Updated %s for %s\n", tempstr, GetPlayerName() );
			}
			else
			{
				...
				if(pNewItem)
				{
					...
					// pWeapon->GetAttributeContainer()->GetItem()->GetItemName(), tempstr, sizeof(tempstr) );
					// Msg("Created %s for %s\n", tempstr, GetPlayerName() );
...
void CTFPlayer::RemovePlayerAttributes(bool bSetBonuses)
{
	...
	for(int i = iAttribs - 1; i >= 0; i--)
	{
		...
		const CEconItemAttributeDefinition *pAttrDef =
			GetItemSchema()->GetAttributeDefinition(pAttrib->GetAttribIndex());
...
void CTFPlayer::ApplySetBonuses(void)
{
	...
	FOR_EACH_VEC(pActiveSets, set)
	{
		...
		{
			const CEconItemAttributeDefinition *pAttrDef =
				GetItemSchema()->GetAttributeDefinition(pActiveSets[set]->m_iAttributes[i].m_iAttribDefIndex);
			if(pAttrDef)
			{
				Assert(pAttrDef->GetAttributeType());
				Assert(pAttrDef->GetAttributeType()
						   ->BSupportsGameplayModificationAndNetworking()); // is an assert instead of a check because
																			// we're in client code here -- this means
																			// someone set up a set with bad data
				...
				else if(tf_boss_battle_respawn_on_friends.GetBool())
				{
					...
					else
					{
						...
						for(int i = 0; i < raidingTeam->GetNumPlayers(); ++i)
						{
							...
							if(buddy != this && buddy->m_Shared.InState(TF_STATE_ACTIVE) &&
							   IsValidRaidRespawnTarget(buddy))
							{
					...
					if(respawnEntity)
					{
						...
						TE_TFParticleEffect(filter, 0.0, "player_sparkles_blue", respawnEntity->GetAbsOrigin(),
											vec3_angle, this, PATTACH_POINT);
	...
	switch(GetTeamNumber())
	{
		...
		case TF_TEAM_BLUE:
		{
			...
			else if(pSpot)
			{
				...
				{
					Warning("EntSelectSpawnPoint(): No valid spawns for class %s on team %i found, even though at "
							"least one spawn entity exists.\n",
							g_aPlayerClassNames[iClass], GetTeamNumber());
				}
			}

			// need to save this for later so we can apply and modifiers to the armor and grenades...after the call to
			// InitClass()
...
bool CTFPlayer::SelectSpawnSpotByType(const char *pEntClassName, CBaseEntity *&pSpot)
{
	...
	do
	{
		if(pSpot)
		{
			...
			if(TFGameRules()->IsSpawnPointValid(pSpot, this, bIgnorePlayers))
			{
				...
				// SpawnFlags were only recently added to the .fgd (Feb 2016), which means older maps won't have any
				// flags at all (they default to on). So this means we only look for restrictions when we find flags,
				// which a map compiled after this change would/should have.
	...
	next_spawn_point:;
		...
		if(bMatchSummary && !pMatchSummaryFallback)
		{
			...
			if(pCTFSpawn)
			{
				if((pCTFSpawn->GetTeamNumber() == pCTFSpawn->GetTeamNumber()) &&
				   (pCTFSpawn->GetMatchSummaryType() == PlayerTeamSpawn_MatchSummary_None))
				{
...
int CTFPlayer::GetAutoTeam(int nPreferedTeam /*= TF_TEAM_AUTOASSIGN*/)
{
	...
	if(pBlue && pRed)
	{
				...
				if(TFGameRules()->IsMannVsMachineMode() && g_pPopulationManager)
				{
					...
					if(g_pPopulationManager)
					{
						...
						if(TFGameRules()->IsMannVsMachineRespecEnabled() &&
						   !g_pPopulationManager->GetNumRespecsAvailableForPlayer(this))
						{
		...
		if(FStrEq(tf_bot_quota_mode.GetString(), "fill") && (tf_bot_quota.GetInt() > 0) &&
		   !(pPlayerBot && pPlayerBot->HasAttribute(CTFBot::QUOTA_MANANGED)))
		{
			// We're using 'tf_bot_quota_mode fill' to keep the teams even so balance based on the human players on each
			// team
...
void CTFPlayer::HandleCommand_JoinTeam(const char *pTeamName)
{
	...
	if(iTeam == TEAM_SPECTATOR ||
	   (TFGameRules()->IsInArenaMode() && tf_arena_use_queue.GetBool() && GetTeamNumber() <= LAST_SHARED_TEAM))
	{
		// Prevent this is the cvar is set
		if((mp_allowspectators.GetBool() == false) && !IsHLTV() && !IsReplay() &&
		   TFGameRules()->IsInArenaMode() == false)
		{
		...
		for(int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if(pTemp && pTemp != this)
			{
				if((pTemp->m_Shared.GetDisguiseTarget() == this) || // they were disguising as me and I've changed teams
				   (!pTemp->m_Shared.GetDisguiseTarget() &&
					pTemp->m_Shared.GetDisguiseTeam() ==
						iTeamNum)) // they don't have a disguise and I'm joining the team they're disguising as
				{
					// choose someone else...
...
void CTFPlayer::HandleCommand_JoinClass(const char *pClassName, bool bAllowSpawn /* = true */)
{
	...
	{
		if(!tf_tournament_classchange_allowed.GetBool() && TFGameRules()->State_Get() == GR_STATE_RND_RUNNING)
		{
			ClientPrint(this, HUD_PRINTCENTER, "#TF_Ladder_NoClassChangeRound");
			return;
		}

		if(!tf_tournament_classchange_ready_allowed.GetBool() && TFGameRules()->State_Get() == GR_STATE_BETWEEN_RNDS &&
	...
	if(TFGameRules()->IsMannVsMachineMode() && GetTeamNumber() == TF_TEAM_PVE_DEFENDERS)
	{
		...
		if(IsReadyToPlay() && !TFGameRules()->InSetup() && g_pPopulationManager &&
		   !g_pPopulationManager->IsInEndlessWaves())
		{
	...
	if(!bAllowSpawn ||
	   ((TFGameRules()->State_Get() == GR_STATE_TEAM_WIN) && (TFGameRules()->GetWinningTeam() != GetTeamNumber())))
	{
		m_bAllowInstantSpawn = false;
	...
	else
	{
		...
		do
		{
			...
			iClass = random->RandomInt(TF_FIRST_NORMAL_CLASS,
									   TF_LAST_NORMAL_CLASS - 1); // -1 to remove the civilian from the randomness
			iTries--;
		} while(iClass == GetPlayerClass()->GetClassIndex() ||
				(iTries > 0 && !TFGameRules()->CanPlayerChooseClass(this, iClass)));
	...
	// are they TF_CLASS_RANDOM and trying to select the class they're currently playing as (so they can stay this
	// class)?
	...
	if(bShouldNotRespawn == false &&
	   (m_bAllowInstantSpawn || bDeadInstantSpawn || bInRespawnRoom || bInStalemateClassChangeTime))
	{
		ForceRespawn();
...
void CTFPlayer::CheckInstantLoadoutRespawn(void)
{
	...
	if(iClass >= TF_FIRST_NORMAL_CLASS && iClass < TF_LAST_NORMAL_CLASS)
	{
		if(m_Inventory.ClassLoadoutHasChanged(iClass))
		{
			...
			{
				// If we are in condition TF_COND_AIMING it will be removed during the ForceRespawn() so we need to
				// reset the weapon (which is normally skipped while regenerating)...this only affects the Minigun and
				// the Sniper Rifle.
				CTFWeaponBase *pWeapon = GetActiveTFWeapon();
				if(pWeapon)
				{
...
GC_REG_JOB(GCSDK::CGCClient, CGC_RespawnPostLoadoutChange, "CGC_RespawnPostLoadoutChange",
		   k_EMsgGCRespawnPostLoadoutChange, GCSDK::k_EServerTypeGCClient);
...
bool CTFPlayer::ClientCommand(const CCommand &args)
{
	...
	else if(FStrEq(pcmd, "show_motd"))
	{
		if(ShouldRunRateLimitedCommand(args))
		{
			if(ShouldForceAutoTeam())
			{
				...
				PlayerHistoryInfo_t *pPlayerInfo =
					(TFGameRules()) ? TFGameRules()->PlayerHistory_GetPlayerInfo(this) : NULL;
			...
			data->SetString(
				"msg_fallback",
				"motd_text"); // use this stringtable entry if the base is HTML, and client has disabled HTML motds
	...
	else if(FStrEq(pcmd, "showroundinfo"))
	{
		if(ShouldRunRateLimitedCommand(args))
		{
			// don't let the player open the round info menu until they're a spectator or they're on a regular team and
			// have picked a class
			if((GetTeamNumber() == TEAM_SPECTATOR) ||
			   ((GetTeamNumber() != TEAM_UNASSIGNED) && (GetPlayerClass()->GetClassIndex() != TF_CLASS_UNDEFINED)))
			{
				if(TFGameRules())
				{
					TFGameRules()->ShowRoundInfoPanel(this);
...
void CTFPlayer::MerasmusPlayerBombExplode(bool bExcludeMe /*= true */)
{
	...
	if(TFGameRules() && TFGameRules()->GetActiveBoss() &&
	   (TFGameRules()->GetActiveBoss()->GetBossType() == HALLOWEEN_BOSS_MERASMUS))
	{
		flDamage = 100.0f;
	}
	...
	CTakeDamageInfo info(this, this, NULL, vecExplosion, vecExplosion, flDamage, iDmgType,
						 TF_DMG_CUSTOM_MERASMUS_PLAYER_BOMB, &vecExplosion);
...
void CTFPlayer::DetonateObjectOfType(int iType, int iMode, bool bIgnoreSapperState)
{
	...
	if(pInfo)
	{
		UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"killedobject\" (object \"%s\") (weapon \"%s\") (objectowner "
					   "\"%s<%i><%s><%s>\") (attacker_position \"%d %d %d\")\n",
					   GetPlayerName(), GetUserID(), GetNetworkIDString(), GetTeam()->GetName(), pInfo->m_pObjectName,
					   "pda_engineer", GetPlayerName(), GetUserID(), GetNetworkIDString(), GetTeam()->GetName(),
...
void CTFPlayer::DropRune(bool bApplyForce /* = true */, int nTeam /* = TEAM_ANY */)
{
	if(m_Shared.IsCarryingRune())
	{
		...
		CTFRune::CreateRune(GetAbsOrigin(), nRuneType, nTeam, true, bApplyForce,
							forward); // Manually dropped powerups are always neutral
	}
}
...
void HandleRageGain(CTFPlayer *pPlayer, unsigned int iRequiredBuffFlags, float flDamage, float fInverseRageGainScale)
{
	...
	if(pPlayer->IsPlayerClass(TF_CLASS_SOLDIER))
	{
		...
		if(iBuffId < ARRAYSIZE(g_RageBuffTypes))
		{
			...
			{
				pPlayer->m_Shared.ModifyRage(g_RageBuffTypes[iBuffId].m_fRageScale *
											 (flDamage / fInverseRageGainScale));
			}
		}
	}
	else if(pPlayer->IsPlayerClass(TF_CLASS_PYRO))
	{
		CTFFlameThrower *pFlameThrower =
			dynamic_cast<CTFFlameThrower *>(pPlayer->Weapon_OwnsThisID(TF_WEAPON_FLAMETHROWER));
		...
		if(iBuffId < ARRAYSIZE(g_RageBuffTypes))
		{
			if(g_RageBuffTypes[iBuffId].m_iBuffFlags & iRequiredBuffFlags)
			{
				if(TFGameRules() && TFGameRules()->IsPowerupMode() &&
				   pPlayer->m_Shared.GetCarryingRuneType() != RUNE_NONE)
				{
					pPlayer->m_Shared.ModifyRage(g_RageBuffTypes[iBuffId].m_fRageScale *
												 ((flDamage / 10) / fInverseRageGainScale));
				}
				else
				{
					pPlayer->m_Shared.ModifyRage(g_RageBuffTypes[iBuffId].m_fRageScale *
...
#ifdef STAGING_ONLY
	if(TFGameRules()->GameModeUsesUpgrades() && pPlayer->IsPlayerClass(TF_CLASS_SPY) &&
	   (kRageBuffFlag_OnDamageDealt & iRequiredBuffFlags))
	{
...
int CTFPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
		...
		if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
		{
			...
			if(bot)
			{
				...
				if(pTFAttacker && pTFAttacker->IsBot())
				{
					...
					if(pTFAttackerBot && (pTFAttackerBot != this) &&
					   pTFAttackerBot->GetPrevMission() == CTFBot::MISSION_DESTROY_SENTRIES &&
					   info.IsForceFriendlyFire() && InSameTeam(pTFAttackerBot) && IsMiniBoss())
					{
	...
	bool bIsSoldierRocketJumping = (IsPlayerClass(TF_CLASS_SOLDIER) && (pAttacker == this) &&
									!(GetFlags() & FL_ONGROUND) && !(GetFlags() & FL_INWATER)) &&
								   (inputInfo.GetDamageType() & DMG_BLAST);
	bool bIsDemomanPipeJumping = (IsPlayerClass(TF_CLASS_DEMOMAN) && (pAttacker == this) &&
								  !(GetFlags() & FL_ONGROUND) && !(GetFlags() & FL_INWATER)) &&
								 (inputInfo.GetDamageType() & DMG_BLAST);
	...
	if(bDebug)
	{
		Warning("%s taking damage from %s, via %s. Damage: %.2f\n", GetDebugName(),
				info.GetInflictor() ? info.GetInflictor()->GetDebugName() : "Unknown Inflictor",
				pAttacker ? pAttacker->GetDebugName() : "Unknown Attacker", info.GetDamage());
	}
	...
	if(pTFAttacker)
	{
		...
		if(myWeapon && attackerWeapon)
		{
			...
			CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(attackerWeapon, iStunEnemyWithSameWeapon,
	...
	if((info.GetDamageType() & DMG_FALL))
	{
		   ...
		   GetGroundEntity() && GetGroundEntity()->IsPlayer())
		{
			...
			if(pOther && pOther->GetTeamNumber() != GetTeamNumber())
			{
				...
				CTakeDamageInfo infoInner(this, this, GetEquippedWearableForLoadoutSlot(LOADOUT_POSITION_SECONDARY),
										  flStompDamage, DMG_FALL, TF_DMG_CUSTOM_BOOTS_STOMP);
	...
	if(pAttacker && pAttacker->GetTeam() != GetTeam())
	{
		...
		if(pAttacker->IsPlayer())
		{
			...
			// add to list of damagers via sentry so that later we can check for achievement:
			// ACHIEVEMENT_TF_ENGINEER_SHOTGUN_KILL_PREV_SENTRY_TARGET
	...
	// We do this here, after damage modify rules to ensure distance falloff calculations have already been made before
	// we pass that damage back to the attacker
	if(pTFAttacker && m_Shared.GetCarryingRuneType() == RUNE_REFLECT && pTFAttacker != this &&
	   !pTFAttacker->m_Shared.IsInvulnerable() && pTFAttacker->IsAlive())
	{
		...
		if(gpGlobals->curtime >
		   m_flNextReflectZap) // don't spam the effect for fast weapons like flamethrower and minigun
		{
			...
			else
			{
				...
				{
					float flPushForce = RemapValClamped(flDamage, 0.1f, 150.f, 300.f,
														500.f); // Scale the push force according to damage
			...
			TE_TFParticleEffectComplex(filter, 0.f, "dxhr_arm_muzzleflash", vStart, QAngle(0.f, 0.f, 0.f), NULL,
		...
		else
		{
			...
			else
			{
				...
				if(pTFAttacker->m_Shared.GetCarryingRuneType() != RUNE_RESIST &&
				   pTFAttacker->m_Shared.GetCarryingRuneType() != RUNE_VAMPIRE)
				{
	...
	bool bMedicBulletResist =
		m_Shared.InCond(TF_COND_MEDIGUN_UBER_BULLET_RESIST) || m_Shared.InCond(TF_COND_MEDIGUN_SMALL_BULLET_RESIST);
	bool bMedicExplosiveResist =
		m_Shared.InCond(TF_COND_MEDIGUN_UBER_BLAST_RESIST) || m_Shared.InCond(TF_COND_MEDIGUN_SMALL_BLAST_RESIST);
	bool bMedicFireResist =
		m_Shared.InCond(TF_COND_MEDIGUN_UBER_FIRE_RESIST) || m_Shared.InCond(TF_COND_MEDIGUN_SMALL_FIRE_RESIST);
	...
	if(IsPlayerClass(TF_CLASS_DEMOMAN))
	{
		...
		if(iDemoChargeDamagePenalty && m_Shared.InCond(TF_COND_SHIELD_CHARGE) && !(info.GetDamageType() & DMG_FALL) &&
		   (pAttacker != this))
		{
			iDemoChargeDamagePenalty *= info.GetDamage();
			m_Shared.SetDemomanChargeMeter(
				Max(m_Shared.GetDemomanChargeMeter() - (float)iDemoChargeDamagePenalty, 0.0f));
		}
	}
	...
	if(bTookDamage && pTFAttacker != this)
	{
		...
		if((info.GetDamageType() & DMG_BURN) != 0 || (info.GetDamageType() & DMG_PLASMA) != 0)
		{
			...
			HandleRageGain(pTFAttacker, kRageBuffFlag_OnBurnDamageDealt, info.GetDamage() * flRageScale,
						   flInverseRageGainScale);
		}
	}
	...
	{
		bool bDisguised =
			m_Shared.InCond(TF_COND_DISGUISED) && (m_Shared.GetDisguiseTeam() == pTFAttacker->GetTeamNumber());
	...
	//{
	//	bool bDisguised = m_Shared.InCond( TF_COND_DISGUISED ) && (m_Shared.GetDisguiseTeam() ==
	//pTFAttacker->GetTeamNumber());
	...
	bool bIsPyroDetonateJumping = (IsPlayerClass(TF_CLASS_PYRO) && pAttacker == this && !(GetFlags() & FL_ONGROUND) &&
								   !(GetFlags() & FL_INWATER));
	if(bIsDemomanPipeJumping || bIsSoldierRocketJumping || bIsPyroDetonateJumping)
	{
		...
		for(int i = 0; i < m_Shared.m_nNumHealers; i++)
		{
			...
			if(pMedigun && pMedigun->GetMedigunType() == MEDIGUN_QUICKFIX)
			{
				...
				// 				if ( info.GetInflictor() )
				// 				{
				// 					vecDir = info.GetInflictor()->WorldSpaceCenter() - Vector ( 0.0f, 0.0f, 10.0f ) -
				// WorldSpaceCenter(); 					info.GetInflictor()->AdjustDamageDirection( info, vecDir, this );
				// 					VectorNormalize( vecDir );
				// 				}
	...
	if(!pTFWeapon)
	{
		...
		if(pWearable)
		{
			EconEntity_OnOwnerKillEaterEvent_Batched(pWearable, pTFAttacker, this, kKillEaterEvent_DamageDealt,
													 info.GetDamage());
	...
	{
		EconEntity_OnOwnerKillEaterEvent_Batched(pTFWeapon, pTFAttacker, this, kKillEaterEvent_DamageDealt,
												 info.GetDamage());
...
bool CTFPlayer::CheckBlockBackstab(CTFPlayer *pTFAttacker)
{
	...
	if(iBackStabShield)
	{
		...
		if(pEntity)
		{
			...
			{
				// Yay stats.
				EconEntity_OnOwnerKillEaterEvent(dynamic_cast<CEconEntity *>(pEntity), this, pTFAttacker,
												 kKillEaterEvent_BackstabAbsorbed);
...
void CTFPlayer::CommitSuicide(bool bExplode /* = false */, bool bForce /*= false*/)
{
	...
	if(!bForce && TFGameRules()->State_Get() == GR_STATE_TEAM_WIN && GetTeamNumber() != TFGameRules()->GetWinningTeam())
	{
		return;
	}
...
// Output : int
//-----------------------------------------------------------------------------
ConVar tf_preround_push_from_damage_enable(
	"tf_preround_push_from_damage_enable", "0", FCVAR_NONE,
...
void CTFPlayer::ApplyPushFromDamage(const CTakeDamageInfo &info, Vector vecDir)
{
	...
	if(info.GetAttacker() == this)
	{
		...
		{
			// Use the original hull for damage force calculation to ensure our RJ height doesn't change due to crouch
			// hull increase
			// ^^ Comment above is an ancient lie, Ducking actually increases blast force, this value increases it even
			// more 82 standing, 62 ducking, 55 modified
			vecSize.z = 55;
		}
		...
		if(IsPlayerClass(TF_CLASS_SOLDIER))
		{
			...
			if((info.GetDamageType() & DMG_BLAST))
			{
				if(GetFlags() & FL_ONGROUND)
				{
					vecForce =
						vecDir *
						-DamageForce(vecSize, flDamageForForce, tf_damageforcescale_self_soldier_badrj.GetFloat()) *
						flSelfPushMult;
				}
				else
				{
					vecForce = vecDir *
							   -DamageForce(vecSize, flDamageForForce, tf_damageforcescale_self_soldier_rj.GetFloat()) *
							   flSelfPushMult;
				}
		...
		else
		{
			...
			{
				vecForce = vecDir * -DamageForce(vecSize, flDamageForForce, tf_damageforcescale_pyro_jump.GetFloat()) *
						   flSelfPushMult;
			}
			else
			{
	...
	else
	{
		...
		else
		{
			...
			{
				vecForce =
					vecDir * -DamageForce(WorldAlignSize(), info.GetDamage(), tf_damageforcescale_other.GetFloat());
				vecForce.z = 0;
			}
			else if(info.GetDamageCustom() == TF_DMG_CUSTOM_PLASMA_CHARGED)
			{
				vecForce = vecDir *
						   -DamageForce(WorldAlignSize(), info.GetDamage(), tf_damageforcescale_other.GetFloat()) *
						   1.25f;
			}
			else if(info.GetDamageCustom() == TF_DMG_CUSTOM_FLARE_PELLET)
			{
				...
				vecForce = vecDir *
						   -DamageForce(WorldAlignSize(), info.GetDamage(),
										TF_FLARE_PELLET_FORCE * RemapValClamped(flTimeAlive, 0.1f, 1.0f, 1.0f,
																				TF_FLARE_PELLET_FORCE_DISTANCE_SCALE));
				vecForce.z =
					((GetPlayerClass()->GetClassIndex() == TF_CLASS_HEAVYWEAPONS) ? (TF_FLARE_PELLET_FORCE_UPWARD_HEAVY)
																				  : (TF_FLARE_PELLET_FORCE_UPWARD));
			}
			...
			CBaseEntity *pInflictor = info.GetInflictor();
			if(pInflictor && CanScatterGunKnockBack(pWeapon, info.GetDamage(),
													(WorldSpaceCenter() - pInflictor->WorldSpaceCenter()).LengthSqr()))
			{
		...
		if(flImpactBlastForce != 1.f)
		{
			...
			if(pInflictor)
			{
				...
				if(flDistSqr <= flMaxPushBackDistSqr)
				{
					...
					flImpactBlastForce = RemapValClamped(flDistSqr, 1000.f, flMaxPushBackDistSqr, flImpactBlastForce,
														 (flImpactBlastForce * 0.5f));
		...
		if(TFGameRules()->GameModeUsesUpgrades())
		{
			...
			if(GetTeamNumber() == TF_TEAM_PVE_INVADERS && !bBigKnockback)
			{
				if(IsMiniBoss())
				{
					// Minibosses can't be pushed by anything except heavy rage and airblast (airblast is suppressed
					// when deploying in deploy ai code)
					return;
				}
...
void CTFPlayer::PlayDamageResistSound(float flStartDamage, float flModifiedDamage)
{
	...
	if(flDamagePercent > 0.f && flDamagePercent < 1.f)
	{
		const char *pszSoundName = (flDamagePercent >= 0.75f)	? "Player.ResistanceLight"
								   : (flDamagePercent <= 0.25f) ? "Player.ResistanceHeavy"
...
int CTFPlayer::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	if(m_takedamage != DAMAGE_EVENTS_ONLY)
	{
		...
		if(pAttackingPlayer && pAttackingPlayer->GetActiveWeapon())
		{
			...
			CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(pAttackingPlayer->GetActiveWeapon(), fLifeleechOnDamage,
											lifeleech_on_damage);
			if(fLifeleechOnDamage > 0.0f)
			{
				...
				if(fHealAmount >= 0.5f)
				{
					const int iHealthToAdd =
						MIN((int)(fHealAmount + 0.5f),
							pAttackingPlayer->m_Shared.GetMaxBuffedHealth() - pAttackingPlayer->GetHealth());
	...
	if(pTFWeapon && WeaponID_IsSniperRifle(pTFWeapon->GetWeaponID()))
	{
		...
		if(pSniper && (pSniper->IsZoomed() || (pSniper->GetWeaponID() == TF_WEAPON_SNIPERRIFLE_CLASSIC)))
		{
			...
			if(flJarateTime && !m_Shared.IsInvulnerable() && !m_Shared.InCond(TF_COND_PHASE) &&
			   !m_Shared.InCond(TF_COND_PASSTIME_INTERCEPTION))
			{
				...
				if(pTFAttacker)
				{
					UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"%s\" against \"%s<%i><%s><%s>\" with \"%s\" "
								   ...
								   pTFAttacker->GetPlayerName(), pTFAttacker->GetUserID(),
								   pTFAttacker->GetNetworkIDString(), pTFAttacker->GetTeam()->GetName(),
								   "jarate_attack", GetPlayerName(), GetUserID(), GetNetworkIDString(),
								   GetTeam()->GetName(), "sniperrifle", (int)pTFAttacker->GetAbsOrigin().x,
								   (int)pTFAttacker->GetAbsOrigin().y, (int)pTFAttacker->GetAbsOrigin().z,
								   (int)GetAbsOrigin().x, (int)GetAbsOrigin().y, (int)GetAbsOrigin().z);

					// explosive jarate shot for a fully charged shot or headshot
					if(pSniper->IsFullyCharged() || IsHeadshot(info.GetDamageCustom()) ||
					   LastHitGroup() == HITGROUP_HEAD)
					{
						JarExplode(entindex(), pTFAttacker, pTFWeapon, pTFWeapon, info.GetDamagePosition(),
								   pTFAttacker->GetTeamNumber(), 100.f, TF_COND_URINE, flJarateTime, "peejar_impact");
					}
				}
			}
	...
	bool bBleed =
		((m_Shared.InCond(TF_COND_DISGUISED) == false || m_Shared.GetDisguiseTeam() != pAttacker->GetTeamNumber()) &&
...
void CTFPlayer::Event_KilledOther(CBaseEntity *pVictim, const CTakeDamageInfo &info)
{
	...
	if(pVictim->IsPlayer())
	{
		...
		CFmtStrN<128> modifiers("%s,%s,%s,%s,victimclass:%s", pszCustomDeath, pszDomination, pszVictimStunned,
								pszVictimDoubleJumping,
								g_aPlayerClassNames_NonLocalized[pTFVictim->GetPlayerClass()->GetClassIndex()]);
		...
		if(pWeapon)
		{
			...
			if(iCritBoost)
			{
				// Perceptually, people seem to think the effect is shorter than the stated time, so we cheat by adding
				// a tad more for that
		...
		if(IsPlayerClass(TF_CLASS_DEMOMAN))
		{
			if(pVictim->GetTeamNumber() != GetTeamNumber())
			{
				...
				if(flRefill > 0 &&
				   ((info.GetDamageType() & DMG_MELEE) || (info.GetDamageCustom() == TF_DMG_CUSTOM_CHARGE_IMPACT)))
				{
					m_Shared.SetDemomanChargeMeter(m_Shared.GetDemomanChargeMeter() + flRefill * 100.0f);
				}

				if((pWeapon && pWeapon->IsCurrentAttackDuringDemoCharge()) ||
				   (info.GetDamageCustom() == TF_DMG_CUSTOM_CHARGE_IMPACT))
				{
					if(flRefill > 0)
					{
					...
					if(pTFVictim)
					{
						...
						if(m_Shared.m_hPlayersVisibleAtChargeStart.Find(pTFVictim) ==
						   m_Shared.m_hPlayersVisibleAtChargeStart.InvalidIndex())
						{
					...
					if(pCP)
					{
						if(pCP->GetOwner() == GetTeamNumber())
						{
							if(GetActiveTFWeapon() &&
							   (GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_PIPEBOMBLAUNCHER))
							{
		...
		// track accumulated sentry gun kills on owning player for Sentry Busters in MvM (so they can't clear this by
		// rebuilding their sentry)
		CObjectSentrygun *sentry = dynamic_cast<CObjectSentrygun *>(info.GetInflictor());
		CTFProjectile_SentryRocket *sentryRocket = dynamic_cast<CTFProjectile_SentryRocket *>(info.GetInflictor());
		...
		if(TF_IsHolidayActive(kHoliday_HalloweenOrFullMoon))
		{
			...
			if(iHalloweenDeathGhosts > 0)
			{
				...
				{
					DispatchParticleEffect("halloween_player_death_blue", pTFVictim->GetAbsOrigin() + Vector(0, 0, 32),
										   vec3_angle);
				}
				else if(pTFVictim->GetTeam()->GetTeamNumber() == TF_TEAM_RED)
				{
					DispatchParticleEffect("halloween_player_death", pTFVictim->GetAbsOrigin() + Vector(0, 0, 32),
										   vec3_angle);
				}
			}
		}
...
void CTFPlayer::OnKilledOther_Effects(CBaseEntity *pVictim, const CTakeDamageInfo &info)
{
	...
	if(iRestoreHealthToPercentageOnKill > 0)
	{
		...
		int iBaseMaxHealth = GetMaxHealth() * 1.5, iNewHealth = Min(GetHealth() + iTargetHealth, iBaseMaxHealth),
...
void CTFPlayer::Event_Killed(const CTakeDamageInfo &info)
{
		...
		if(info.GetAttacker() == info.GetInflictor() && info.GetAttacker() && info.GetAttacker()->IsBSPModel())
		{
			if(pRecentDamager)
			{
				...
				if(TFGameRules() && TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_DOOMSDAY))
				{
					...
					HatAndMiscEconEntities_OnOwnerKillEaterEvent(pRecentDamager, this,
	...
	if(pPlayerAttacker)
	{
		...
		if(pPlayerAttacker != this)
		{
			if(CTFPlayerDestructionLogic::GetRobotDestructionLogic() &&
			   (CTFPlayerDestructionLogic::GetRobotDestructionLogic()->GetType() ==
				CTFPlayerDestructionLogic::TYPE_PLAYER_DESTRUCTION))
			{
	...
	*/
	...
	if(IsPlayerClass(TF_CLASS_MEDIC))
	{
		...
		if(event)
		{
			...
			if(pPlayerStats)
			{
				...
				// defensive fix for the moment for bug where healing value becomes bogus sometimes: if bogus, slam it
				// to 0
	...
	else if(IsPlayerClass(TF_CLASS_ENGINEER))
	{
		...
		if(m_Shared.IsCarryingObject())
		{
			...
			if(m_Shared.GetCarriedObject() != NULL)
			{
				...
				EconEntity_OnOwnerKillEaterEventNoPartner(pVictimEconWeapon, this,
	...
	if(pPlayerAttacker)
	{
		...
		{
			if(pKillerWeapon && WeaponID_IsSniperRifle(pKillerWeapon->GetWeaponID()) &&
			   pPlayerAttacker->m_Shared.IsAiming() == false)
			{
		...
		else if(pPlayerAttacker->IsPlayerClass(TF_CLASS_SPY))
		{
					...
					if(iDesiredClass != TF_CLASS_SPY)
					{
						...
						if(IsPlayerClass(TF_CLASS_MEDIC))
						{
							...
							if(pMedigun)
							{
								...
								CWeaponMedigun *pAttackerMedigun =
									(CWeaponMedigun *)pPlayerAttacker->Weapon_OwnsThisID(TF_WEAPON_MEDIGUN);
								if(pAttackerMedigun)
								{
						...
						switch(pPlayerAttacker->GetTeamNumber())
						{
							case TF_TEAM_RED:
								...
								TE_TFParticleEffect(filter, 0.0, "player_sparkles_red", vOrigin, vec3_angle,
													pPlayerAttacker, PATTACH_POINT);
								break;
							case TF_TEAM_BLUE:
								TE_TFParticleEffect(filter, 0.0, "teleported_blue", vOrigin, vec3_angle);
								TE_TFParticleEffect(filter, 0.0, "player_sparkles_blue", vOrigin, vec3_angle,
													pPlayerAttacker, PATTACH_POINT);
								break;
							default:
								break;
			...
			if(IsPlayerClass(TF_CLASS_ENGINEER))
			{
				...
				while(pHistory)
				{
					if(pHistory->hEntity && pHistory->hEntity->IsBaseObject() &&
					   m_AchievementData.IsTargetInHistory(pHistory->hEntity, 1.0f))
					{
		...
		else if(pPlayerAttacker->IsPlayerClass(TF_CLASS_DEMOMAN))
		{
			...
			if(pPlayerAttacker->GetActiveTFWeapon() &&
			   (pPlayerAttacker->GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_GRENADELAUNCHER))
			{
				CBaseEntity *pInflictor = info.GetInflictor();

				if(pInflictor && pInflictor->IsPlayer() == false)
				{
					CTFGrenadePipebombProjectile *pBaseGrenade =
						dynamic_cast<CTFGrenadePipebombProjectile *>(pInflictor);
		...
		else if(pPlayerAttacker->IsPlayerClass(TF_CLASS_ENGINEER))
		{
			...
			if(pKillerWeapon && (pKillerWeapon->GetWeaponID() == TF_WEAPON_SENTRY_REVENGE ||
	...
	if(TFGameRules()->IsMannVsMachineMode())
	{
		...
		if(IsBot())
		{
					...
					if(bDropPack)
					{
						...
						if(pPlayerAttacker && pPlayerAttacker->IsPlayerClass(TF_CLASS_SNIPER))
						{
							if(info.GetDamageCustom() == TF_DMG_CUSTOM_BLEEDING ||
							   (pKillerWeapon && WeaponID_IsSniperRifleOrBow(pKillerWeapon->GetWeaponID())))
							{
								pMoneyMaker = pPlayerAttacker;

								if(IsHeadshot(info.GetDamageCustom()) ||
								   (LastHitGroup() == HITGROUP_HEAD && pKillerWeapon && pKillerWeapon->GetJarateTime()))
								{
			...
			if(!m_bIsSupportEnemy)
			{
				...
				TFObjectiveResource()->DecrementMannVsMachineWaveClassCount(GetPlayerClass()->GetClassIconName(),
																			iFlags);
			}
	...
	if(HasItem() && pPlayerAttacker && (pPlayerAttacker != this))
	{
		...
		if(pCaptureFlag)
		{
				...
				CTFPlayerDestructionLogic::TYPE_PLAYER_DESTRUCTION))
			{
				...
				EconEntity_OnOwnerKillEaterEvent(dynamic_cast<CEconEntity *>(pKillerWeapon), pPlayerAttacker, this,
	...
	if(pActiveWeapon)
	{
		...
		{
			EconEntity_OnOwnerKillEaterEventNoPartner(pVictimEconWeapon, this,
													  kKillEaterEvent_NEGATIVE_DeathsFromCratering);
		}
	}
	...
	if(bElectrocuted && bGib)
	{
		const char *pEffectName =
			(GetTeamNumber() == TF_TEAM_RED) ? "electrocuted_gibbed_red" : "electrocuted_gibbed_blue";
	...
	else if(info.GetAttacker() && TFGameRules()->GetActiveBoss() &&
			info.GetAttacker()->entindex() == TFGameRules()->GetActiveBoss()->entindex())
	{
	...
	else if(info.GetAttacker() == info.GetInflictor() && info.GetAttacker() && info.GetAttacker()->IsBSPModel())
	{
		...
		else
		{
			...
			else if(TFGameRules() && TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_DOOMSDAY) &&
					(info_modified.GetDamageType() & DMG_CLUB))
			{
	...
	if(pPlayerAttacker && pPlayerAttacker->m_Shared.InCond(TF_COND_HALLOWEEN_TINY) &&
	   !pPlayerAttacker->m_Shared.InCond(TF_COND_HALLOWEEN_KART))
	{
	...
	{
		CTFPlayer *pRecentDamager = TFGameRules()->GetRecentDamager(this, 0, 5.0);
		int pointsOnDeath = (!bSuicide || pRecentDamager)
								? CTFPlayerDestructionLogic::GetPlayerDestructionLogic()->GetPointsOnPlayerDeath()
								: 0;
		...
		else
		{
			...
			{
				pFlag = CCaptureFlag::Create(GetAbsOrigin(),
											 CTFPlayerDestructionLogic::GetPlayerDestructionLogic()->GetPropModelName(),
	...
	if(bGib || bRagdoll)
	{
		CreateRagdollEntity(bGib, bBurning, bElectrocuted, bOnGround, bCloakedCorpse, iGoldRagdoll != 0,
							iIceRagdoll != 0, iRagdollsBecomeAsh != 0, iCustomDamage, (iCritOnHardHit != 0));
	}
	...
	if(pPlayerAttacker)
	{
		...
		if(bBurning && pPlayerAttacker->IsPlayerClass(TF_CLASS_PYRO))
		{
			...
			if((pOriginalBurner == pPlayerAttacker || pLastBurner == pPlayerAttacker) && pWeapon &&
	...
	if(TFGameRules()->IsMannVsMachineMode())
	{
		...
		if(GetTeamNumber() == TF_TEAM_PVE_DEFENDERS)
		{
			...
			else
			{
				...
				{
					TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_GIANT_KILLED_TEAMMATE,
																	   TF_TEAM_PVE_DEFENDERS);
				}

				TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(
					MP_CONCEPT_MVM_DEFENDER_DIED, TF_TEAM_PVE_DEFENDERS,
					CFmtStr("victimclass:%s", g_aPlayerClassNames_NonLocalized[GetPlayerClass()->GetClassIndex()])
						.Access());
			}
		}
		else
	...
	if(!TFGameRules()->IsMannVsMachineMode() || !IsBot())
	{
		...
		{
			CTriggerPlayerRespawnOverride *pTriggerRespawn =
				static_cast<CTriggerPlayerRespawnOverride *>(ITriggerPlayerRespawnOverride::AutoList()[i]);
	...
	if((info.GetAttacker() == info.GetInflictor() && info.GetAttacker() && info.GetAttacker()->IsBSPModel()) ||
	   (info.GetDamageCustom() == TF_DMG_CUSTOM_TRIGGER_HURT) || (info.GetDamageType() & DMG_VEHICLE))
	{
		CTFPlayer *pRecentDamager = TFGameRules()->GetRecentDamager(this, 1, 5.0);
		if(pRecentDamager)
		{
...
void CTFPlayer::DropAmmoPack(const CTakeDamageInfo &info, bool bEmpty, bool bDisguisedWeapon)
{
	...
	CTFWeaponBase *pDropWeaponProps =
		(bDisguisedWeapon && m_Shared.InCond(TF_COND_DISGUISED) && m_Shared.GetDisguiseWeapon())
	...
	CTFDroppedWeapon *pDroppedWeapon =
		CTFDroppedWeapon::Create(this, vecPackOrigin, vecPackAngles, pszWorldModel, pItem);
	if(pDroppedWeapon)
	{
	...
	CTFAmmoPack *pAmmoPack =
		CTFAmmoPack::Create(vecPackOrigin, vecPackAngles, this, "models/items/ammopack_medium.mdl");
	Assert(pAmmoPack);
	if(pAmmoPack)
	{
...
void CTFPlayer::DropHealthPack(const CTakeDamageInfo &info, bool bEmpty)
{
	...
	CHealthKitSmall *pMedKit =
		assert_cast<CHealthKitSmall *>(CBaseEntity::Create("item_healthkit_small", vecSrc, vec3_angle, this));
	if(pMedKit)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayer::DropCurrencyPack(CurrencyRewards_t nSize /* = TF_CURRENCY_PACK_SMALL */, int nAmount /*= 0*/,
								 bool bForceDistribute /*= false*/, CBasePlayer *pMoneyMaker /*= NULL*/)
{
	...
	switch(nSize)
	{
		case TF_CURRENCY_PACK_SMALL:
			pCurrencyPack = assert_cast<CCurrencyPackSmall *>(
				CBaseEntity::Create("item_currencypack_small", vecSrc, vec3_angle, this));
			break;

		case TF_CURRENCY_PACK_MEDIUM:
			pCurrencyPack = assert_cast<CCurrencyPackMedium *>(
				CBaseEntity::Create("item_currencypack_medium", vecSrc, vec3_angle, this));
			break;

		case TF_CURRENCY_PACK_LARGE:
			pCurrencyPack =
				assert_cast<CCurrencyPack *>(CBaseEntity::Create("item_currencypack_large", vecSrc, vec3_angle, this));
			break;
		...
		case TF_CURRENCY_PACK_CUSTOM:
			...
			pCurrencyPack = assert_cast<CCurrencyPack *>(
				CBaseEntity::CreateNoSpawn("item_currencypack_custom", vecSrc, vec3_angle, this));
...
void CTFPlayer::StateThinkWELCOME(void)
{
	if(!IsFakeClient())
	{
		...
		else if(TFGameRules()->IsInTraining())
		{
			...
			int iClass = TFGameRules()->GetTrainingModeLogic()
							 ? TFGameRules()->GetTrainingModeLogic()->GetDesiredClass()
							 : TF_CLASS_SOLDIER;
...
void CTFPlayer::StateThinkDYING(void)
{
	...
	if(!m_bAbortFreezeCam && m_hRagdoll && (m_lifeState == LIFE_DYING || m_lifeState == LIFE_DEAD) &&
	...
	if(!m_bPlayedFreezeCamSound && GetObserverTarget() && GetObserverTarget() != this)
	{
		...
		float flFreezeSoundTime =
			(m_flDeathTime + TF_DEATH_ANIMATION_TIME) + spec_freeze_traveltime.GetFloat() - flFreezeSoundLength;
...
void CTFPlayer::ForceRespawn(void)
{
	...
	if(TFGameRules() && TFGameRules()->IsInArenaMode())
	{
		...
		if((tf_arena_use_queue.GetBool() == false && TFGameRules()->IsInWaitingForPlayers()) ||
		   TFGameRules()->State_Get() == GR_STATE_PREGAME)
		{
			return;
		}
	}
...
void CTFPlayer::FinishedObject(CBaseObject *pObject)
{
	...
	if(TFGameRules() && TFGameRules()->IsInTraining() && TFGameRules()->GetTrainingModeLogic() &&
	   IsFakeClient() == false)
	{
...
void CTFPlayer::PainSound(const CTakeDamageInfo &info)
{
	...
	if(SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_PAIN, "damagecritical:1", szResponse, AI_Response::MAX_RESPONSE_NAME,
							 &filter))
	{
	...
	if(bAttackerIsPlayer)
	{
		...
		SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_ATTACKER_PAIN, "damagecritical:1", szResponse,
							  AI_Response::MAX_RESPONSE_NAME, &attackerFilter);
	}
...
//-----------------------------------------------------------------------------
void CTFPlayer::CreateRagdollEntity(bool bGib, bool bBurning, bool bElectrocuted, bool bOnGround, bool bCloakedCorpse,
									bool bGoldRagdoll, bool bIceRagdoll, bool bBecomeAsh, int iDamageCustom,
									bool bCritOnHardHit)
{
...
void CTFPlayer::FeignDeath(const CTakeDamageInfo &info)
{
	...
	EconEntity_OnOwnerKillEaterEvent(dynamic_cast<CEconEntity *>(GetEntityForLoadoutSlot(LOADOUT_POSITION_PDA2)), this,
...
void CTFPlayer::RemoveInvisibility(void)
{
	...
	bool bAEStealth = (m_Shared.InCond(TF_COND_STEALTHED_USER_BUFF) && pProvider &&
					   (pProvider->IsPlayerClass(TF_CLASS_SPY) ? true : false) && (pProvider != this));
	...
	if(m_iszAssociateTeamEntityName != NULL_STRING)
	{
		...
		if(!m_hAssociatedTeamEntity)
		{
			Warning("info_observer_point (%s) couldn't find associated team entity named '%s'\n", GetDebugName(),
					STRING(m_iszAssociateTeamEntityName));
		}
	}
}
...
bool CObserverPoint::CanUseObserverPoint(CTFPlayer *pPlayer)
{
	...
	if(m_hAssociatedTeamEntity && mp_forcecamera.GetInt() == OBS_ALLOW_TEAM)
	{
		...
		if(TFGameRules() && TFGameRules()->State_Get() != GR_STATE_TEAM_WIN)
		{
			...
			if(m_hAssociatedTeamEntity->GetTeamNumber() != pPlayer->GetTeamNumber() &&
...
BEGIN_DATADESC(CObserverPoint)
	DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"),
		...
		DEFINE_KEYFIELD(m_flFOV, FIELD_FLOAT, "fov"), DEFINE_KEYFIELD(m_bMatchSummary, FIELD_BOOLEAN, "match_summary"),
	...
	else if(TFGameRules())
	{
		...
		if(m_hObserverTarget && m_hObserverTarget->IsBaseTrain() && TFGameRules()->State_Get() == GR_STATE_TEAM_WIN)
		{
			...
			CObserverPoint *pObserverPoint =
				(CObserverPoint *)gEntList.FindEntityByClassname(NULL, "info_observer_point");
			...
			while(pObserverPoint)
			{
				...
				pObserverPoint =
					(CObserverPoint *)gEntList.FindEntityByClassname(pObserverPoint, "info_observer_point");
			}
		...
		if(!m_Shared.IsAlly(pVictim))
		{
			if(IsPlayerClass(TF_CLASS_HEAVYWEAPONS) && pVictim->IsPlayerClass(TF_CLASS_HEAVYWEAPONS))
			{
				...
				if(pTeam && pTeam->GetRole() == TEAM_ROLE_DEFENDERS)
				{
					if(m_Shared.InCond(TF_COND_INVULNERABLE) || m_Shared.InCond(TF_COND_INVULNERABLE_WEARINGOFF))
					{
						if(pVictim->m_Shared.InCond(TF_COND_INVULNERABLE) ||
						   pVictim->m_Shared.InCond(TF_COND_INVULNERABLE_WEARINGOFF))
						{
							float flMaxSpeed = 50.0f * 50.0f;
							if((GetAbsVelocity().LengthSqr() < flMaxSpeed) &&
							   (pVictim->GetAbsVelocity().LengthSqr() < flMaxSpeed))
							{
			...
			{
				// calculate a force and save it off, it is used on a later frame cause it is to late to apply the force
				// here
				float flImpactForce = GetLocalVelocity().Length();
				if(flImpactForce > 10.0f)
				{
					...
					ray.Init(vOrigin, Vector(0, 0, 16) + vOrigin + vAim * tf_halloween_kart_impact_lookahead.GetFloat(),
							 GetPlayerMins() * tf_halloween_kart_impact_bounds_scale.GetFloat(),
							 GetPlayerMaxs() * tf_halloween_kart_impact_bounds_scale.GetFloat());
					...
					if(pTrace.m_pEnt == pVictim)
					{
						...
						else
						{
							...
							SetCurrentTauntMoveSpeed(GetCurrentTauntMoveSpeed() *
													 tf_halloween_kart_impact_feedback.GetFloat());
							EmitSound("BumperCar.Bump");
						}
						...
						int iDamage = (int)((flImpactForce / 50.0f + RandomInt(13, 19)) *
											tf_halloween_kart_impact_damage.GetFloat());
			...
			if((m_Shared.GetPercentInvisible() < 0.10f) && m_Shared.GetCarryingRuneType() == RUNE_PLAGUE &&
			   !m_Shared.IsAlly(pVictim) && !pVictim->m_Shared.IsInvulnerable() &&
			   !pVictim->m_Shared.InCond(TF_COND_PLAGUE) && pVictim->m_Shared.GetCarryingRuneType() != RUNE_RESIST)
			{
				pVictim->m_Shared.AddCond(TF_COND_PLAGUE, PERMANENT_CONDITION, this);

				// Plague transmission event infects nearby eligible players on the same team. Only works for powerup
				// carrier to host, not host to host.
				...
				for(int i = 0; i < pVictim->GetTeam()->GetNumPlayers(); i++)
				{
					...
					if(pTeamMate && pTeamMate != pVictim && pTeamMate->IsAlive() &&
					   !pTeamMate->m_Shared.IsInvulnerable() && !pTeamMate->m_Shared.InCond(TF_COND_PLAGUE) &&
					   pTeamMate->m_Shared.GetCarryingRuneType() != RUNE_RESIST)
					{
						...
						if((vecPos - pTeamMate->WorldSpaceCenter()).LengthSqr() < (350 * 350))
						{
							...
							if(pVictim->FVisible(pTeamMate, MASK_SOLID))
							{
								...
								te_tf_particle_effects_control_point_t controlPoint = {PATTACH_ABSORIGIN, vEnd};
								TE_TFParticleEffectComplex(filter, 0.f, "plague_transmission", vStart,
														   QAngle(0.f, 0.f, 0.f), NULL, &controlPoint, pTeamMate,
...
//-----------------------------------------------------------------------------
// Purpose: Finds an open space for a high five partner. flTolerance specifies the maximum amount that should be allowed
// underneath position.
//-----------------------------------------------------------------------------
bool CTFPlayer::FindOpenTauntPartnerPosition(CEconItemView *pEconItemView, Vector &position, float *flTolerance)
{
	...
	const float flTauntSeparationForwardDistance = tf_highfive_separation_forward.GetFloat() != 0
													   ? tf_highfive_separation_forward.GetFloat()
													   : pItemDef->GetTauntData()->GetTauntSeparationForwardDistance();
	const float flTauntSeparationRightDistance = tf_highfive_separation_right.GetFloat() != 0
													 ? tf_highfive_separation_right.GetFloat()
													 : pItemDef->GetTauntData()->GetTauntSeparationRightDistance();
	...
	*flTolerance = tf_highfive_height_tolerance.GetFloat();
	...
	if(result.DidHit())
	{
		// something's directly in front of us, but let's allow for a little bit of variation since we might be standing
		// on an uneven displacement
		trace_t result2;
		...
		UTIL_TraceHull(vecStart, vecEnd + (forward * 2), VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, &filter,
					   &result2);
	...
	if(ret)
	{
		...
		Vector vecEndSuperSafe = vecStartCenter + (forward * flTauntSeparationForwardDistance * 2.f) +
								 (right * flTauntSeparationRightDistance);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPlayer::IsAllowedToInitiateTauntWithPartner(CEconItemView *pEconItemView, char *pszErrorMessage,
													int cubErrorMessage)
{
	Vector vecEnd;
	float flTolerance;
	...
	if(ret)
	{
		...
		UTIL_TraceHull(vecEnd, vecEnd - Vector(0, 0, flTolerance), VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID,
					   &filter, &result3);
		if(!result3.DidHit())
		{
...
bool CTFPlayer::IsRobotCostumeEquipped(void) const
{
	...
	static CSchemaItemDefHandle ppItemDefWearables[] = {CSchemaItemDefHandle("Idiot Box"),
														CSchemaItemDefHandle("Steel Pipes"),
														CSchemaItemDefHandle("Shoestring Budget")};
	return HasWearablesEquipped(ppItemDefWearables, ARRAYSIZE(ppItemDefWearables));
}
...
bool CTFPlayer::IsDemowolf(void) const
{
	...
	static CSchemaItemDefHandle ppItemDefWearables[] = {CSchemaItemDefHandle("Hair of the Dog"),
														CSchemaItemDefHandle("Scottish Snarl"),
														CSchemaItemDefHandle("Pickled Paws")};
	return HasWearablesEquipped(ppItemDefWearables, ARRAYSIZE(ppItemDefWearables));
}
...
bool CTFPlayer::IsFrankenHeavy(void) const
{
	...
	static CSchemaItemDefHandle ppItemDefWearables[] = {CSchemaItemDefHandle("Can Opener"),
														CSchemaItemDefHandle("Soviet Stitch-Up"),
														CSchemaItemDefHandle("Steel-Toed Stompers")};
	return HasWearablesEquipped(ppItemDefWearables, ARRAYSIZE(ppItemDefWearables));
}
...
bool CTFPlayer::IsFairyHeavy(void) const
{
	...
	static CSchemaItemDefHandle ppItemDefWearables[] = {CSchemaItemDefHandle("The Grand Duchess Tutu"),
														CSchemaItemDefHandle("The Grand Duchess Fairy Wings"),
														CSchemaItemDefHandle("The Grand Duchess Tiara")};
	return HasWearablesEquipped(ppItemDefWearables, ARRAYSIZE(ppItemDefWearables));
}
...
bool CTFPlayer::PlayTauntSceneFromItem(CEconItemView *pEconItemView)
{
	...
	if(pszScene)
	{
		...
		if(pEconItemView->FindAttribute(pAttrDef_TauntPressAndHold, &iLongTaunt) && iLongTaunt != 0)
		{
			...
			if(!m_bTauntMimic && pTauntData->IsPartnerTaunt() &&
			   !IsAllowedToInitiateTauntWithPartner(pEconItemView, szClientError, ARRAYSIZE(szClientError)))
			{
				CSingleUserRecipientFilter filter(this);
				EmitSound_t params;
				params.m_pSoundName = "Player.DenyWeaponSelection";
		...
		if(pEconItemView->FindAttribute(pAttrDef_TauntSuccessSoundLoop, &attrTauntSuccessSoundLoop))
		{
			...
			if(pszTauntSoundLoopName && *pszTauntSoundLoopName)
			{
				...
				static CSchemaAttributeDefHandle pAttrDef_TauntSuccessSoundLoopOffset(
		...
		if(FindAttribute_UnsafeBitwiseCast<CAttribute_String>(pItemDef, pAttrDef_TauntForceWeaponSlot,
			...
			FOR_EACH_VEC(vecPlayers, i)
			{
				...
				if(pPlayer && pPlayer->m_Shared.InCond(TF_COND_TAUNTING))
				{
					...
					if(pPlayer->GetTauntEconItemView() &&
					   (pPlayer->GetTauntEconItemView()->GetItemDefinition() == congaTaunt))
					{
...
float CTFPlayer::PlayTauntRemapInputScene()
{
	...
	for(int iButtonIndex = 0; iButtonIndex < pTaunt->GetTauntInputRemapCount(); ++iButtonIndex)
	{
		...
		else if(FStrEq(pszCurrentSceneFileName, pszPressedScene) &&
				!tauntRemap.m_vecButtonReleasedScenes[iClass].IsEmpty())
		{
...
void CTFPlayer::OnTauntSucceeded(const char *pszSceneName, int iTauntIndex /*= 0*/, int iTauntConcept /*= 0*/)
{
	...
	const itemid_t unTauntSourceItemID =
		m_TauntEconItemView.IsValid() ? m_TauntEconItemView.GetItemID() : INVALID_ITEM_ID;
...
void CTFPlayer::Taunt(taunts_t iTauntIndex, int iTauntConcept)
{
		...
		else if(IsPlayerClass(TF_CLASS_HEAVYWEAPONS))
		{
			if(pActiveWeapon && pActiveWeapon->GetWeaponID() == TF_WEAPON_LUNCHBOX)
			{
				...
				if((pLunchbox->GetLunchboxType() == LUNCHBOX_STANDARD) ||
				   (pLunchbox->GetLunchboxType() == LUNCHBOX_STANDARD_ROBO))
				{
	...
	if(IsPlayerClass(TF_CLASS_PYRO))
	{
		...
		else if(!V_stricmp(szResponse, "scenes/player/pyro/low/taunt_bubbles.vcd"))
		{
			...
			// We need to parent this to a target instead of the player because the player changing their camera view
			// can twist the rainbow
...
float CTFPlayer::PlayTauntOutroScene()
{
	...
	if(pTauntData)
	{
		...
		{
			// play outro
			const char *pszOutroScene =
				pTauntData->GetOutroScene(iClass, RandomInt(0, pTauntData->GetOutroSceneCount(iClass) - 1));
			if(m_hTauntScene.Get())
			{
				StopScriptedScene(this, m_hTauntScene);
				m_hTauntScene = NULL;
...
void CTFPlayer::DoTauntAttack(void)
{
		...
		if(count)
		{
			...
			for(int i = 0; i < count; i++)
			{
				...
				if(iTauntAttack == TAUNTATK_SPY_FENCING_SLASH_A || iTauntAttack == TAUNTATK_SPY_FENCING_SLASH_B)
				{
					...
					pList[i]->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 100, vecPos, 25,
														 DMG_SLASH | DMG_PREVENT_PHYSICS_FORCE,
														 TF_DMG_CUSTOM_TAUNTATK_FENCING));
				}
				else if(iTauntAttack == TAUNTATK_SPY_FENCING_STAB)
				{
					pList[i]->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 20000, vecPos,
														 500.0f, DMG_SLASH, TF_DMG_CUSTOM_TAUNTATK_FENCING));
				}
				else if(iTauntAttack == TAUNTATK_PYRO_HADOUKEN)
				{
					pList[i]->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 25000, vecPos,
														 500.0f, DMG_BURN | DMG_IGNITE,
	...
	else if(iTauntAttack == TAUNTATK_SOLDIER_GRENADE_KILL)
	{
		...
		if(iRightHand != -1)
		{
			...
			CTakeDamageInfo info(this, this, GetActiveTFWeapon(), vec3_origin, bonePos, 200.f,
								 DMG_BLAST | DMG_USEDISTANCEMOD, TF_DMG_CUSTOM_TAUNTATK_GRENADE, &bonePos);
			...
			iTauntAttack == TAUNTATK_ENGINEER_ARM_IMPALE || iTauntAttack == TAUNTATK_ENGINEER_ARM_KILL ||
			iTauntAttack == TAUNTATK_ENGINEER_ARM_BLEND)
	{
		Vector vecForward;
				...
				switch(iTauntAttack)
				{
					...
					case TAUNTATK_ENGINEER_ARM_IMPALE:
						if(pVictim)
						{
							...
							{
								pEnt->TakeDamage(CTakeDamageInfo(
									this, this, GetActiveTFWeapon(), vecForward, pEnt->WorldSpaceCenter(), 1,
									DMG_BULLET | DMG_PREVENT_PHYSICS_FORCE, TF_DMG_CUSTOM_TAUNTATK_ENGINEER_ARM_KILL));
							}
						}
						break;
					...
					case TAUNTATK_SNIPER_ARROW_STAB_KILL:
						...
						pEnt->TakeDamage(CTakeDamageInfo(
							this, this, GetActiveTFWeapon(), vecForward * 12000, pEnt->WorldSpaceCenter(), 500.0f,
							DMG_BULLET | DMG_PREVENT_PHYSICS_FORCE, TF_DMG_CUSTOM_TAUNTATK_ARROW_STAB));
						break;

					case TAUNTATK_ENGINEER_ARM_KILL:
						pEnt->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 12000,
														 pEnt->WorldSpaceCenter(), 500.0f, DMG_BLAST,
	...
	else if(iTauntAttack == TAUNTATK_HEAVY_HIGH_NOON)
	{
		...
		if(tr.fraction < 1.0)
		{
			   ...
			   pEnt->GetTeamNumber() != GetTeamNumber())
			{
				// Launch them up a little
				AngleVectors(QAngle(-45, m_angEyeAngles[YAW], 0), &vecForward);
				pEnt->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 25000,
												 WorldSpaceCenter(), 500.0f, DMG_BULLET,
	...
	else if(iTauntAttack == TAUNTATK_SCOUT_DRINK)
	{
		if(!m_Shared.IsControlStunned())
		{
			...
			if(pActiveWeapon && pActiveWeapon->GetWeaponID() == TF_WEAPON_LUNCHBOX)
			{
				float flDropDeadTime = (100.f / tf_scout_energydrink_consume_rate.GetFloat()) +
									   1.f; // Just in case.  Normally over in 8 seconds.
	...
	else if(iTauntAttack == TAUNTATK_SCOUT_GRAND_SLAM)
	{
		...
		if(count)
		{
			for(int i = 0; i < count; i++)
			{
				...
				trace_t tr;
				UTIL_TraceLine(WorldSpaceCenter(), pObjects[i]->WorldSpaceCenter(), MASK_SOLID_BRUSHONLY, this,
				...
				AngleVectors(QAngle(-45, m_angEyeAngles[YAW], 0), &vecForward);
				pTarget->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 130000,
													WorldSpaceCenter(), 500.0f, DMG_BULLET,
	...
	else if(iTauntAttack == TAUNTATK_PYRO_ARMAGEDDON)
	{
		...
		for(CEntitySphereQuery sphere(origin, flRadius);
			(pEntity = sphere.GetCurrentEntity()) != NULL && vecDamagedPlayers.Count() < ARRAYSIZE(nRandomPick);
			sphere.NextEntity())
		{
		...
		if(vecDamagedPlayers.Count())
		{
			...
			for(int i = vecDamagedPlayers.Count() - 1; i >= 0; --i)
			{
				...
				if(pPlayer)
				{
					...
					pPlayer->TakeDamage(CTakeDamageInfo(
						this, this, GetActiveTFWeapon(), vec3_origin, origin, fDamage, DMG_PLASMA,
						(iTauntAttack == TAUNTATK_PYRO_ARMAGEDDON) ? TF_DMG_CUSTOM_TAUNTATK_ARMAGEDDON
	...
	{
		// We need to parent this to a target instead of the player because the player changing their camera view can
		// twist the rainbow
		CBaseEntity *pTarget = CreateEntityByName("info_target");
		if(pTarget)
		{
		...
		TE_TFParticleEffect(filter, 0.0, "bl_killtaunt", GetAbsOrigin(), GetAbsAngles(), pTarget,
							PATTACH_ABSORIGIN_FOLLOW);
			   ...
			   pEnt->GetTeamNumber() != GetTeamNumber())
			{
				...
				if(iTauntAttack == TAUNTATK_MEDIC_UBERSLICE_IMPALE)
				{
					if(pVictim)
					{
						...
						pVictim->TakeDamage(
							CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward, WorldSpaceCenter(), 1,
											DMG_BULLET | DMG_PREVENT_PHYSICS_FORCE, TF_DMG_CUSTOM_TAUNTATK_UBERSLICE));
					}
				}
				else
				{
					...
					pVictim->TakeDamage(
						CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 12000, WorldSpaceCenter(), 500.0f,
										DMG_BULLET | DMG_PREVENT_PHYSICS_FORCE, TF_DMG_CUSTOM_TAUNTATK_UBERSLICE));
	...
	else if(iTauntAttack == TAUNTATK_DEMOMAN_BARBARIAN_SWING)
	{
		...
		if(tr.fraction < 1.0)
		{
			   ...
			   pEnt->GetTeamNumber() != GetTeamNumber())
			{
				...
				VectorNormalize(vecForward);
				pEnt->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 12000,
												 WorldSpaceCenter(), 500.0f, DMG_CLUB,
	...
	else if(iTauntAttack == TAUNTATK_ENGINEER_GUITAR_SMASH)
	{
		...
		if(tr.fraction < 1.0)
		{
			   ...
			   pEnt->GetTeamNumber() != GetTeamNumber())
			{
				vecForward = (WorldSpaceCenter() - pEnt->WorldSpaceCenter());
				VectorNormalize(vecForward);
				pEnt->TakeDamage(CTakeDamageInfo(this, this, GetActiveTFWeapon(), vecForward * 12, WorldSpaceCenter(),
												 500.0f, DMG_CLUB, TF_DMG_CUSTOM_TAUNTATK_ENGINEER_GUITAR_SMASH));
			}
		}
	}
	...
	//	{
	//		CEffectData	data;
	//		data.m_nHitBox = GetParticleSystemIndex( GetTeamNumber() == TF_TEAM_RED ? "taunt_flip_land_red" :
	//"taunt_flip_land_blue" ); 		data.m_vOrigin = m_hHighFivePartner.Get()->GetAbsOrigin(); 		data.m_vAngles =
	//m_hHighFivePartner.Get()->GetAbsAngles();
...
void CTFPlayer::ModifyOrAppendCriteria(AI_CriteriaSet &criteriaSet)
{
	...
	else
	{
		if(GetPlayerClass())
		{
			criteriaSet.AppendCriteria("playerclass",
									   g_aPlayerClassNames_NonLocalized[GetPlayerClass()->GetClassIndex()]);
		}
	}
	...
	if(pStats)
	{
		iTotalKills = pStats->statsCurrentLife.m_iStat[TFSTAT_KILLS] +
					  pStats->statsCurrentLife.m_iStat[TFSTAT_KILLASSISTS] +
	...
	criteriaSet.AppendCriteria("cloaked",
							   (m_Shared.IsStealthed() || m_Shared.InCond(TF_COND_STEALTHED_BLINK)) ? "1" : "0");
	...
	criteriaSet.AppendCriteria("waitingforplayers",
							   (TFGameRules()->IsInWaitingForPlayers() || TFGameRules()->IsInPreMatch()) ? "1" : "0");
	...
	criteriaSet.AppendCriteria(
		"dodging", (m_Shared.InCond(TF_COND_PHASE) || m_Shared.InCond(TF_COND_PASSTIME_INTERCEPTION)) ? "1" : "0");
	...
	Vector forward;
	EyeVectors(&forward);
	UTIL_TraceLine(EyePosition(), EyePosition() + (forward * MAX_TRACE_LENGTH), MASK_BLOCKLOS_AND_NPCS, this,
	...
	// Previous round win
	bool bLoser = (TFGameRules()->GetPreviousRoundWinners() != TEAM_UNASSIGNED &&
				   TFGameRules()->GetPreviousRoundWinners() != GetPrevRoundTeamNum());
	criteriaSet.AppendCriteria("LostRound", bLoser ? "1" : "0");

	bool bPrevRoundTie =
		((TFGameRules()->GetRoundsPlayed() > 0) && (TFGameRules()->GetPreviousRoundWinners() == TEAM_UNASSIGNED));
	...
	if(TFGameRules()->IsHolidayActive(kHoliday_HalloweenOrFullMoon) && iSpecialTaunt == 0)
	{
		...
		{
			static CSchemaAttributeDefHandle pAttrDef_AdditionalHalloweenResponseRule(
			...
			FOR_EACH_VEC_BACK(m_hMyWearables, wbl)
			{
				...
				if(pWearable && pWearable->GetAttributeContainer()->GetItem())
				{
					...
					if(FindAttribute_UnsafeBitwiseCast<CAttribute_String>(pWearable->GetAttributeContainer()->GetItem(),
																		  pAttrDef_AdditionalHalloweenResponseRule,
																		  &pszAdditionalResponseRule))
					{
...
public:
	...
	virtual float operator()(CNavArea *baseArea, CNavArea *fromArea, const CNavLadder *ladder,
...
CTeamControlPoint *CTFPlayer::SelectClosestControlPointByTravelDistance(
	CUtlVector<CTeamControlPoint *> *pointVector) const
{
	...
	for(int i = 0; i < pointVector->Count(); ++i)
	{
		...
		float travelRange = NavAreaTravelDistance(
			GetLastKnownArea(), TheTFNavMesh()->GetControlPointCenterArea(point->GetPointIndex()), cost);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPlayer::SpeakConceptIfAllowed(int iConcept, const char *modifiers, char *pszOutResponseChosen, size_t bufsize,
									  IRecipientFilter *filter)
{
	...
	if(m_Shared.InCond(TF_COND_DISGUISED) && !filter && (iConcept != MP_CONCEPT_KILLED_PLAYER))
	{
		...
		bool bPlayedDisguised =
			SpeakIfAllowed(g_pszMPConcepts[iConcept], buf, pszOutResponseChosen, bufsize, &disguisedFilter);
		...
		bool bPlayedNormally =
			SpeakIfAllowed(g_pszMPConcepts[iConcept], modifiers, pszOutResponseChosen, bufsize, &undisguisedFilter);
...
void CTFPlayer::NoteSpokeVoiceCommand(const char *pszScenePlayed)
{
	...
	m_flNextVoiceCommandTime =
		gpGlobals->curtime + MIN(GetSceneDuration(pszScenePlayed), tf_max_voice_speak_delay.GetFloat());
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPlayer::WantsLagCompensationOnEntity(const CBasePlayer *pPlayer, const CUserCmd *pCmd,
...
int CTFPlayer::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		Q_snprintf(tempstr, sizeof(tempstr), "Health: %d / %d ( %.1f )", GetHealth(), GetMaxHealth(),
				   (float)GetHealth() / (float)GetMaxHealth());
...
static ConCommand sv_debug_stuck_particles("sv_debug_stuck_particles", DebugParticles,
...
static ConCommand cc_IgnitePlayer("tf_ignite_player", IgnitePlayer, "Sets you on fire",
								  FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
...
static ConCommand tf_testvcd("tf_testvcd", TestVCD,
							 "Run a vcd on the player currently under your crosshair. Optional parameter is the .vcd "
							 "name (default is 'scenes/heavy_test.vcd')",
							 FCVAR_CHEAT);
...
static ConCommand tf_testrr("tf_testrr", TestRR,
							"Force the player under your crosshair to speak a response rule concept. Format is "
							"tf_testrr <concept>, or tf_testrr <player name> <concept>",
							FCVAR_CHEAT);

#ifdef _DEBUG
CON_COMMAND_F(tf_crashclients, "testing only, crashes about 50 percent of the connected clients.",
			  FCVAR_DEVELOPMENTONLY)
{
	for(int i = 1; i < gpGlobals->maxClients; ++i)
	{
...
uint64 powerplay_ids[] = {
	76561197960435530ull ^ powerplaymask, 76561197960265731ull ^ powerplaymask, 76561197960265749ull ^ powerplaymask,
	76561197962783665ull ^ powerplaymask, 76561197991390878ull ^ powerplaymask, 76561197979187556ull ^ powerplaymask,
	76561197960269040ull ^ powerplaymask, 76561197968459473ull ^ powerplaymask, 76561197989728462ull ^ powerplaymask,
};
	...
	if(m_bInPowerPlay)
	{
		...
		if(GetPlayerClass())
		{
			...
			switch(GetPlayerClass()->GetClassIndex())
			{
				case TF_CLASS_SCOUT:
					flDuration =
						InstancedScriptedScene(this, "scenes/player/scout/low/435.vcd", NULL, 0.0f, false, NULL, true);
					break; // laughlong02
				case TF_CLASS_SNIPER:
					flDuration = InstancedScriptedScene(this, "scenes/player/sniper/low/1674.vcd", NULL, 0.0f, false,
														NULL, true);
					break; // laughlong01
				case TF_CLASS_SOLDIER:
					flDuration = InstancedScriptedScene(this, "scenes/player/soldier/low/1346.vcd", NULL, 0.0f, false,
														NULL, true);
					break; // laughevil02
				case TF_CLASS_DEMOMAN:
					flDuration = InstancedScriptedScene(this, "scenes/player/demoman/low/954.vcd", NULL, 0.0f, false,
														NULL, true);
					break; // laughlong02
				case TF_CLASS_MEDIC:
					flDuration =
						InstancedScriptedScene(this, "scenes/player/medic/low/608.vcd", NULL, 0.0f, false, NULL, true);
					break; // laughlong02
				case TF_CLASS_HEAVYWEAPONS:
					flDuration =
						InstancedScriptedScene(this, "scenes/player/heavy/low/270.vcd", NULL, 0.0f, false, NULL, true);
					break; // laughlong01
				case TF_CLASS_PYRO:
					flDuration =
						InstancedScriptedScene(this, "scenes/player/pyro/low/1485.vcd", NULL, 0.0f, false, NULL, true);
					break; // laughlong01
				case TF_CLASS_SPY:
					flDuration =
						InstancedScriptedScene(this, "scenes/player/spy/low/1312.vcd", NULL, 0.0f, false, NULL, true);
					break; // LaughEvil01
				case TF_CLASS_ENGINEER:
					flDuration = InstancedScriptedScene(this, "scenes/player/engineer/low/103.vcd", NULL, 0.0f, false,
														NULL, true);
					break; // laughlong01
			}
		}

		SetContextThink(&CTFPlayer::PowerplayThink, gpGlobals->curtime + flDuration + RandomFloat(2, 5),
						"TFPlayerLThink");
	}
}
...
bool CTFPlayer::ShouldAnnounceAchievement(void)
{
	if(IsPlayerClass(TF_CLASS_SPY))
	{
		if(m_Shared.IsStealthed() || m_Shared.InCond(TF_COND_DISGUISED) || m_Shared.InCond(TF_COND_DISGUISING))
		{
			return false;
		}
	}
...
void CAchievementData::DumpDamagers(void)
{
	...
	for(int i = 0; i < aDamagers.Count(); i++)
	{
		if(aDamagers[i].hEntity)
		{
			...
			{
				Msg("   %s : at %.2f (%.2f ago)\n", ToTFPlayer(aDamagers[i].hEntity)->GetPlayerName(),
					aDamagers[i].flTimeDamage, gpGlobals->curtime - aDamagers[i].flTimeDamage);
			}
			else
			{
				Msg("   %s : at %.2f (%.2f ago)\n", aDamagers[i].hEntity->GetDebugName(), aDamagers[i].flTimeDamage,
					gpGlobals->curtime - aDamagers[i].flTimeDamage);
			}
		}
	}
}
...
int CAchievementData::GetAmountForDamagerInEventHistory(CBaseEntity *pEntity, float flTimeWindow)
{
	...
	for(int i = 0; i < aDamageEvents.Count(); i++)
	{
		...
		// Msg( "   %s : at %.2f (%.2f ago)\n", ToTFPlayer( aDamageEvents[i].hEntity )->GetPlayerName(),
		// aDamageEvents[i].flTimeDamage, gpGlobals->curtime - aDamageEvents[i].flTimeDamage );
...
#ifdef STAGING_ONLY
CON_COMMAND_F(item_testitem,
			  "Creates a server-side item of the specified type, and gives it to the player. Does NOT create the item "
			  "on the Steam backend.",
			  FCVAR_NONE)
{
	if(args.ArgC() < 2)
	{
	...
	if(pItem)
	{
		...
		else if(pItem->IsWearable())
		{
			...
			for(int wbl = 0; wbl < pPlayer->GetNumWearables(); wbl++)
			{
				...
				if(pWearable->GetAttributeContainer()->GetItem()->GetStaticData()->GetLoadoutSlot(iClass) ==
				   pScriptItem->GetStaticData()->GetLoadoutSlot(iClass))
				{
...
void CTFPlayer::ItemTesting_Start(KeyValues *pKV)
{
	...
	ItemTesting_DeleteItems(); // Remove items before creating new defs. Some def clean-up depends on existing static
							   // values.
	...
	for(int iItemType = 0; iItemType < TI_TYPE_COUNT; iItemType++)
	{
		...
		ItemSystem()->GetItemSchema()->ItemTesting_CreateTestDefinition(iReplacedItemDef, iNewDef,
																		m_ItemsToTest[iNewItem].pKV);
	...
	for(int t = 0; t < playerList.Count(); ++t)
	{
		...
		if(tf_highfive_debug.GetBool())
			Msg("%s is ready to %s.\n", pPlayer->GetPlayerName(),
				pPlayer->m_TauntEconItemView.GetStaticData()->GetDefinitionName());
		...
		trace_t result;
		CTraceFilterIgnoreTeammates filter(this, COLLISION_GROUP_NONE, GetAllowedTauntPartnerTeam());
		UTIL_TraceHull(GetAbsOrigin(), pPlayer->GetAbsOrigin(), VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, &filter,
					   &result);
		if(result.DidHit())
		{
			if(tf_highfive_debug.GetBool())
				Msg(" - entity [%i %s %s] in between. tracing again with tolerance.\n", result.GetEntityIndex(),
					result.m_pEnt ? result.m_pEnt->GetClassname() : "NULL", result.surface.name);

			Vector offset(0, 0, tf_highfive_height_tolerance.GetFloat());
			trace_t result2;
			UTIL_TraceHull(GetAbsOrigin() + offset, pPlayer->GetAbsOrigin() + offset, VEC_HULL_MIN, VEC_HULL_MAX,
						   MASK_PLAYERSOLID, &filter, &result2);
			if(result2.DidHit())
			{
				if(tf_highfive_debug.GetBool())
					Msg(" - entity [%i %s %s] in between.\n", result2.GetEntityIndex(),
						result2.m_pEnt ? result2.m_pEnt->GetClassname() : "NULL", result2.surface.name);
...
// Purpose:
//-----------------------------------------------------------------------------
static bool SelectPartnerTaunt(const GameItemDefinition_t *pItemDef, CTFPlayer *initiator, CTFPlayer *receiver,
							   const char **pszInitiatorScene, const char **pszReceiverScene)
{
	...
	if(iInitiatorSceneCount == 0 || iReceiverSceneCount == 0)
	{
		return false;
	}
...
void CTFPlayer::AcceptTauntWithPartner(CTFPlayer *initiator)
{
	...
	if(tf_highfive_debug.GetBool())
		Msg("%s doing %s with initiator %s.\n", GetPlayerName(),
			initiator->m_TauntEconItemView.GetStaticData()->GetDefinitionName(), initiator->GetPlayerName());
	...
	trace_t result;
	CTraceFilterIgnoreTeammates filter(this, COLLISION_GROUP_NONE, GetTeamNumber());
	UTIL_TraceHull(newOrigin, newOrigin - Vector(0, 0, flTolerance), VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID,
				   &filter, &result);
	if(!result.DidHit())
	{
		if(tf_highfive_debug.GetBool())
	...
	if(stucktrace.startsolid != 0)
	{
		if(tf_highfive_debug.GetBool())
			Msg(" - but we'd get stuck on entity [%i %s %s] going in front of %s.\n", stucktrace.GetEntityIndex(),
				stucktrace.m_pEnt ? stucktrace.m_pEnt->GetClassname() : "NULL", stucktrace.surface.name,
...
{
	if(m_Shared.InCond(TF_COND_DISGUISED) || m_Shared.InCond(TF_COND_DISGUISING) || m_Shared.IsStealthed())
	{
...
void CTFPlayer::CalculateExperienceLevel(bool bAnnounce /*= true*/)
{
	...
	if(bAnnounce)
	{
		if(flLevel > 1 && nPrevLevel != (int)flLevel)
		{
			...
			UTIL_ClientPrintAll(HUD_PRINTCENTER, "#TF_PlayerLeveled", pszTeamName, GetPlayerName(),
								CFmtStr("%d", (int)flLevel));
			UTIL_ClientPrintAll(HUD_PRINTCONSOLE, "#TF_PlayerLeveled", pszTeamName, GetPlayerName(),
								CFmtStr("%d", (int)flLevel));
...
void CTFPlayer::RememberUpgrade(int iPlayerClass, CEconItemView *pItem, int iUpgrade, int nCost, bool bDowngrade)
{
	...
	DevMsg("%3.2f: %s: Player '%s', item '%s', upgrade '%s', cost '%d'\n", gpGlobals->curtime,
		   bDowngrade ? "FORGET_UPGRADE" : "REMEMBER_UPGRADE", GetPlayerName(),
		   pItem ? pItem->GetStaticData()->GetItemBaseName() : "<self>", upgradeName ? upgradeName : "<NULL>", nCost);
}
...
void CTFPlayer::ReapplyPlayerUpgrades(void)
{
	...
	for(int u = 0; u < upgrades->Count(); ++u)
	{
		...
		if(iClassIndex == upgrades->Element(u).m_iPlayerClass)
		{
			...
			if(upgrades->Element(u).m_itemDefIndex == INVALID_ITEM_DEF_INDEX)
			{
				g_hUpgradeEntity->ApplyUpgradeToItem(this, NULL, upgrades->Element(u).m_upgrade,
													 upgrades->Element(u).m_nCost);
			}
		}
	}
...
{
	// change size now
	if(m_Shared.InCond(TF_COND_HALLOWEEN_BOMB_HEAD) || m_Shared.InCond(TF_COND_MELEE_ONLY) ||
	   m_Shared.InCond(TF_COND_HALLOWEEN_KART) || m_Shared.InCond(TF_COND_BALLOON_HEAD))
	{
		...
		*pModifiers = 0;
		...
		while(token)
		{
			...
			*colon = 0;
			...
			CBaseEntity *pProcedural =
				gEntList.FindEntityProcedural(value, this, inputdata.pActivator, inputdata.pCaller);
...
void CTFPlayer::CreateDisguiseWeaponList(CTFPlayer *pDisguiseTarget)
{
	...
	if(pDisguiseTarget)
	{
		for(int i = 0; i < TF_PLAYER_WEAPON_COUNT; ++i)
		{
			...
			int iSubType = 0;
			CTFWeaponBase *pCopyWeapon =
				dynamic_cast<CTFWeaponBase *>(GiveNamedItem(pWeapon->GetClassname(), iSubType, pItem, true));
			if(pCopyWeapon)
			{
	...
	if(pItem->IsValid())
	{
		...
		if(pNewItem)
		{
			...
			if(pWeapon)
			{
				...
				{
					CTFDroppedWeapon *pNewDroppedWeapon =
						CTFDroppedWeapon::Create(this, vecPackOrigin, vecPackAngles, pWeapon->GetWorldModel(),
