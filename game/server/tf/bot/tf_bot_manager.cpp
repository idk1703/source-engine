...
ConVar tf_bot_difficulty(
	"tf_bot_difficulty", "1", FCVAR_NONE,
...
ConVar tf_bot_quota_mode("tf_bot_quota_mode", "normal", FCVAR_NONE,
						 ...
						 "the server will adjust bots to keep N players in the game, where N is bot_quota.\nIf "
...
CTFBotManager::CTFBotManager() : NextBotManager(), m_flNextPeriodicThink(0)
{
...
void CTFBotManager::OnRoundRestart(void)
{
	...
	while((sentryHint = (CTFBotHintSentrygun *)(gEntList.FindEntityByClassname(sentryHint, "bot_hint_sentrygun"))) !=
		  NULL)
	{
	...
	while((teleporterHint = (CTFBotHintTeleporterExit *)(gEntList.FindEntityByClassname(
			   teleporterHint, "bot_hint_teleporter_exit"))) != NULL)
	{
...
void CTFBotManager::UpdateCreepWaves()
{
	...
	if(TFGameRules()->InSetup() || TFGameRules()->State_Get() == GR_STATE_STARTGAME ||
...
bool CTFBotManager::RemoveBotFromTeamAndKick(int nTeam)
{
	...
	if(pVictim)
	{
		...
		pVictim->ForceChangeTeam(
			TEAM_UNASSIGNED); // skipping TEAM_SPECTATOR because some servers don't allow spectators
	...
	if(desiredBotCount > nTFBotsOnGameTeams)
	{
		   ...
		   !TFGameRules()->WouldChangeUnbalanceTeams(TF_TEAM_RED, TEAM_UNASSIGNED))
		{
			...
			if(pBot)
			{
				...
				const char *classname = FStrEq(tf_bot_force_class.GetString(), "") ? pBot->GetNextSpawnClassname()
																				   : tf_bot_force_class.GetString();
...
//----------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(tf_bot_debug_stuck_log, "Given a server logfile, visually display bot stuck locations.",
			...
			else if(first[0] == '\"')
			{
				...
				*IDEnd = '\000';
				...
				if(second && !strcmp(second, "stuck"))
				{
					...
					// L 08/08/2012 - 15:15:05: "Scout<53><BOT><Blue>" stuck (position "-180.61 2471.29 216.04")
					// (duration "2.52") L 08/08/2012 - 15:15:05:    path_goal ( "-180.61 2471.29 216.04" )
	...
	for(int i = 0; i < m_stuckBotVector.Count(); ++i)
	{
		...
		for(int j = 0; j < m_stuckBotVector[i]->m_stuckEventVector.Count() - 1; ++j)
		{
			NDebugOverlay::HorzArrow(m_stuckBotVector[i]->m_stuckEventVector[j]->m_stuckSpot,
									 m_stuckBotVector[i]->m_stuckEventVector[j + 1]->m_stuckSpot, 3, 100, 0, 255, 255,
									 true, deltaT);
		}

		NDebugOverlay::Text(m_stuckBotVector[i]->m_stuckEventVector[0]->m_stuckSpot,
							CFmtStr("%s(#%d)", m_stuckBotVector[i]->m_name, m_stuckBotVector[i]->m_id), false, deltaT);
	}
}