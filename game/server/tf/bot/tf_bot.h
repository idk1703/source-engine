...
public:
	...
	virtual void AvoidPlayers(
		CUserCmd *pCmd); // some game types allow players to pass through each other, this method pushes them apart
	...
	virtual bool IsDormantWhenDead(
		void) const; // should this player-bot continue to update itself when dead (respawn logic, etc)

	virtual void OnWeaponFired(CBaseCombatCharacter *whoFired,
							   CBaseCombatWeapon *weapon); // when someone fires their weapon
	...
	bool IsNearPoint(
		CTeamControlPoint *point) const;	// return true if we are within a short travel distance of the current point
	...
	struct SniperSpotInfo
	{
		...
		float m_range;
		float m_advantage; // the difference in how long it takes us to reach our vantage spot vs them to reach the
						   // theater spot
	};
	...
	// search outwards from startSearchArea and collect all reachable objects from the given list that pass the given
	// filter
	void SelectReachableObjects(const CUtlVector<CHandle<CBaseEntity>> &candidateObjectVector,
								CUtlVector<CHandle<CBaseEntity>> *selectedObjectVector, const INextBotFilter &filter,
								CNavArea *startSearchArea, float maxRange = 2000.0f) const;
...
#define LOOK_FOR_ENEMIES true
	void UpdateLookingAroundForIncomingPlayers(
		bool lookForEnemies);				 // update our view to watch where friends or enemies will be coming from
	...
	void SetAttentionFocus(CBaseEntity *focusOn); // restrict bot's attention to only this entity (or radius around this
												  // entity) to the exclusion of everything else
	...
	void DelayedThreatNotice(
		CHandle<CBaseEntity> who,
		float noticeDelay); // notice the given threat after the given number of seconds have elapsed
	void UpdateDelayedThreatNotices(void);

	CTFNavArea *FindVantagePoint(
		float maxTravelDistance = 2000.0f) const; // return a nearby area where we can see a member of the enemy team

	const char *GetNextSpawnClassname(void) const;

	float GetThreatDanger(
		CBaseCombatCharacter *who) const;	 // return perceived danger of threat (0=none, 1=immediate deadly danger)
	...
	void EquipBestWeaponForThreat(
		const CKnownEntity *threat); // equip the best weapon we have to attack the given threat
	bool EquipLongRangeWeapon(void); // equip a weapon that can damage far-away targets

	void PushRequiredWeapon(
		CTFWeaponBase *weapon);	  // force us to equip and use this weapon until popped off the required stack
...
#define MY_CURRENT_GUN NULL							  // can be passed as weapon to following queries
	...
	bool IsHitScanWeapon(CTFWeaponBase *weapon)
		const; // return true if given weapon is a "hitscan" weapon (scattered tracelines with instant damage)
	bool IsContinuousFireWeapon(CTFWeaponBase *weapon)
		const; // return true if given weapon "sprays" bullets/fire/etc continuously (ie: not individual rockets/etc)
	bool IsExplosiveProjectileWeapon(
		CTFWeaponBase *weapon) const; // return true if given weapon launches explosive projectiles with splash damage
	bool IsBarrageAndReloadWeapon(CTFWeaponBase *weapon)
		const; // return true if given weapon has small clip and long reload cost (ie: rocket launcher, etc)
	bool IsQuietWeapon(CTFWeaponBase *weapon)
		const; // return true if given weapon doesn't make much sound when used (ie: spy knife, etc)

	bool IsEnvironmentNoisy(
		void) const; // return true if there are/have been loud noises (ie: non-quiet weapons) nearby very recently
	...
	bool IsLineOfFireClear(const Vector &where)
		const; // return true if a weapon has no obstructions along the line from our eye to the given position
	bool IsLineOfFireClear(CBaseEntity *who)
		const; // return true if a weapon has no obstructions along the line from our eye to the given entity
	bool IsLineOfFireClear(const Vector &from, const Vector &to)
		const; // return true if a weapon has no obstructions along the line between the given points
	bool IsLineOfFireClear(const Vector &from, CBaseEntity *who)
		const; // return true if a weapon has no obstructions along the line between the given point and entity

	bool IsEntityBetweenTargetAndSelf(
		CBaseEntity *other, CBaseEntity *target); // return true if "other" is positioned inbetween us and "target"
	...
	SuspectedSpyInfo_t *IsSuspectedSpy(
		CTFPlayer *player);				// return true if we suspect this player might be an enemy spy
	...
	CTFPlayer *GetClosestHumanLookingAtMe(
		int team = TEAM_ANY) const; // return the nearest human player on the given team who is looking directly at me
	...
	enum AttributeType
	{
		...
		HOLD_FIRE_UNTIL_FULL_RELOAD =
			1 << 11,				   // don't fire our barrage weapon until it is full reloaded (rocket launcher, etc)
	...
	CObjectSentrygun *GetEnemySentry(
		void) const; // if we've been attacked/killed by an enemy sentry, this will return it, otherwise NULL
	...
	float GetSquadFormationError(
		void) const; // return normalized error term where 0 = in formation position and 1 = completely out of position
	...
	float TransientlyConsistentRandomValue(float period = 10.0f, int seedValue = 0)
		const; // compute a pseudo random value (0-1) that stays consistent for the given period of time, but changes
			   // unpredictably each period
...
#define MISSION_DOESNT_RESET_BEHAVIOR_SYSTEM false
	...
	void DisguiseAsMemberOfEnemyTeam(
		void); // set Spy disguise to be a class that someone on the enemy team is actually using
	...
	CTFPlayer *SelectRandomReachableEnemy(
		void); // mostly for MvM - pick a random enemy player that is not in their spawn room
...
private:
	...
	CUtlStack<CHandle<CTFWeaponBase>>
		m_requiredWeaponStack; // if non-empty, bot must equip the weapon on top of the stack
...
public:
							 ...
							 const CFuncElevator *elevator, float length) const
	{
		...
		else
		{
			...
			if(TFGameRules()->IsInTraining() && area->HasAttributeTF(TF_NAV_CONTROL_POINT) &&
			   !m_me->IsAnyPointBeingCaptured() &&
			   !m_me->IsPlayerClass(
				   TF_CLASS_ENGINEER)) // allow engineers to path so they can test travel distance for sentry placement
			{
				return -1.0f;
			}
			...
			if(m_routeType == DEFAULT_ROUTE && !m_me->IsMiniBoss())
			{
				...
				preference =
					1.0f + 50.0f * (1.0f + FastCos((float)(m_me->GetEntity()->entindex() * area->GetID() * timeMod)));
			}
			...
			if(m_me->IsPlayerClass(TF_CLASS_SPY))
			{
				...
				for(int oit = 0; oit < IBaseObjectAutoList::AutoList().Count(); ++oit)
				{
					...
					if((enemyObj->ObjectType() == OBJ_SENTRYGUN) && (enemyObj->GetTeamNumber() == enemyTeam))
					{
