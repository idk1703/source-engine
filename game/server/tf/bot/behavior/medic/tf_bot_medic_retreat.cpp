...
public:
	...
	virtual bool IsAllowed(CBaseEntity *entity) const
	{
		...
		{
			return !ToTFPlayer(entity)->IsPlayerClass(TF_CLASS_MEDIC) &&
				   !ToTFPlayer(entity)->IsPlayerClass(TF_CLASS_SNIPER);
		}
		return false;
	}
...
ActionResult<CTFBot> CTFBotMedicRetreat::Update(CTFBot *me, float interval)
{
	...
	if(m_lookAroundTimer.IsElapsed())
	{
		...
		me->GetBodyInterface()->AimHeadTowards(me->EyePosition() + forward, IBody::IMPORTANT, 0.1f, NULL,
