...
public:
	...
	CTFPlayer *SelectPreferred(CTFPlayer *current, CTFPlayer *contender)
	{
		...
		static int preferredClass[] = {TF_CLASS_HEAVYWEAPONS, TF_CLASS_SOLDIER, TF_CLASS_PYRO, TF_CLASS_DEMOMAN,
		...
		if(!current->IsBot() && current->IsCallingForMedic() &&
		   m_me->IsRangeLessThan(current, tf_bot_medic_max_call_response_range.GetFloat()))
		{
			// check actual travel range
			if(NavAreaTravelDistance(m_me->GetLastKnownArea(), current->GetLastKnownArea(), cost,
									 1.5f * tf_bot_medic_max_call_response_range.GetFloat()) >= 0.0)
			{
				currentCaller = current;
			}
		}

		if(!contender->IsBot() && contender->IsCallingForMedic() &&
		   m_me->IsRangeLessThan(contender, tf_bot_medic_max_call_response_range.GetFloat()))
		{
			// check actual travel range
			if(NavAreaTravelDistance(m_me->GetLastKnownArea(), contender->GetLastKnownArea(), cost,
									 1.5f * tf_bot_medic_max_call_response_range.GetFloat()) >= 0.0)
			{
		...
		if(currentRank == contenderRank)
		{
			...
			return (m_me->GetDistanceBetween(current) - m_me->GetDistanceBetween(contender) > tolerance) ? contender
																										 : current;
		}
	...
	{
		if(!known.GetEntity() || !known.GetEntity()->IsPlayer() || !known.GetEntity()->IsAlive() ||
		   !m_me->IsFriend(known.GetEntity()))
			return true;
		...
		{
			if(player->IsPlayerClass(TF_CLASS_MEDIC) || player->IsPlayerClass(TF_CLASS_SNIPER) ||
			   player->IsPlayerClass(TF_CLASS_ENGINEER) || player->IsPlayerClass(TF_CLASS_SPY))
			{
...
{
#ifdef STAGING_ONLY
	if(TFGameRules()->IsMannVsMachineMode() && me && me->HasAttribute(CTFBot::PROJECTILE_SHIELD) && pMedigun &&
	   (pMedigun->GetMedigunShield() != NULL) && pMedigun->HasPermanentShield() &&
	   ((pMedigun->GetMedigunType() == MEDIGUN_STANDARD) || (pMedigun->GetMedigunType() == MEDIGUN_UBER)))
	{
		return false;
	}
#endif
...
{
	if(who->IsPlayerClass(TF_CLASS_MEDIC) || who->IsPlayerClass(TF_CLASS_SNIPER) ||
	   who->IsPlayerClass(TF_CLASS_ENGINEER) || who->IsPlayerClass(TF_CLASS_SCOUT) || who->IsPlayerClass(TF_CLASS_SPY))
	{
		return false;
	}
...
ActionResult<CTFBot> CTFBotMedicHeal::Update(CTFBot *me, float interval)
{
	...
	if(TFGameRules() && TFGameRules()->IsMannVsMachineMode() && m_patient != NULL &&
	   m_patient->IsPlayerClass(TF_CLASS_MEDIC))
	{
	...
	if(m_patient->m_Shared.InCond(TF_COND_SELECTED_TO_TELEPORT))
	{
		...
		if(closeTeleporter)
		{
			return SuspendFor(new CTFBotUseTeleporter(closeTeleporter, CTFBotUseTeleporter::ALWAYS_USE),
	...
	if(medigun)
	{
		if(medigun->GetMedigunType() == MEDIGUN_RESIST)
		{
			...
			while((me->HasAttribute(CTFBot::PREFER_VACCINATOR_BULLETS) &&
				   medigun->GetResistType() != MEDIGUN_BULLET_RESIST) ||
				  (me->HasAttribute(CTFBot::PREFER_VACCINATOR_BLAST) &&
				   medigun->GetResistType() != MEDIGUN_BLAST_RESIST) ||
		   ...
		   medigun->GetMedigunShield() == NULL)
		{
			...
			else
			{
				...
				if(me->m_Shared.GetRageMeter() >= 100.f && !isUsingProjectileShield && !useUber)
				{
					...
					if(me->GetTimeSinceLastInjury(GetEnemyTeam(me->GetTeamNumber())) < 1.0f ||
					   m_patient->GetTimeSinceLastInjury(GetEnemyTeam(m_patient->GetTeamNumber())) < 1.0f)
					{
						me->PressSpecialFireButton();
						isUsingProjectileShield = true;
					}
	...
	if(!IsReadyToDeployUber(medigun) && !me->m_Shared.InCond(TF_COND_INVULNERABLE) && !isActivelyHealing &&
	   !isUsingProjectileShield && (isThreatened || outOfHealRange || isPatientObscured))
	{
		...
		{
			me->GetBodyInterface()->AimHeadTowards(knownThreat->GetEntity(), IBody::IMPORTANT, 1.0f, NULL,
												   "Aiming at an enemy");
		}
	}
	else
	...
	if(me->m_Shared.InCond(TF_COND_INVULNERABLE) || IsReadyToDeployUber(medigun) || isHealTargetBlocked)
	{
		...
		if(me->IsRangeGreaterThan(m_patient, tf_bot_medic_stop_follow_range.GetFloat()) ||
...
void CTFBotMedicHeal::ComputeFollowPosition(CTFBot *me)
{
	...
	if(!isExposed)
	{
		...
		if(isNearPatient)
		{
			...
			if(!TFGameRules()->InSetup() && m_patient->GetTimeSinceWeaponFired() > 5.0f &&
	...
	for(radius = tf_bot_medic_stop_follow_range.GetFloat() + RandomFloat(0.0f, radiusInc); radius <= maxRadius;
		radius += radiusInc)
	{
		...
		for(angle = 0.0f; angle <= 2.0f * M_PI; angle += inc)
		{
			...
			UTIL_TraceLine(m_patient->WorldSpaceCenter(), pos,
						   MASK_OPAQUE | CONTENTS_IGNORE_NODRAW_OPAQUE | CONTENTS_MONSTER, &traceFilter, &trace);
