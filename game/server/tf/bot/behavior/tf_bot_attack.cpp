...
ActionResult<CTFBot> CTFBotAttack::Update(CTFBot *me, float interval)
{
	...
	bool isUsingCloseRangeWeapon =
		(myWeapon && (myWeapon->IsWeapon(TF_WEAPON_FLAMETHROWER) || myWeapon->IsMeleeWeapon()));
	...
	if(isUsingCloseRangeWeapon && threat->IsVisibleRecently() &&
	   me->IsRangeLessThan(threat->GetLastKnownPosition(), 1.1f * me->GetDesiredAttackRange()))
	{
	   ...
	   !me->IsLineOfFireClear(threat->GetEntity()->EyePosition()))
	{
		if(threat->IsVisibleRecently())
		{
			if(isUsingCloseRangeWeapon &&
			   !TFGameRules()->IsMannVsMachineMode()) // all bots in MvM use the default route
			{
		...
		else
		{
			...
			{
				me->GetBodyInterface()->AimHeadTowards(threat->GetLastKnownPosition() + Vector(0, 0, HumanEyeHeight),
													   IBody::IMPORTANT, 0.2f, NULL,
