...
{
	if(me->IsPlayerClass(TF_CLASS_HEAVYWEAPONS) || me->IsPlayerClass(TF_CLASS_SNIPER) ||
	   me->IsPlayerClass(TF_CLASS_MEDIC) || me->IsPlayerClass(TF_CLASS_ENGINEER) || me->IsPlayerClass(TF_CLASS_PYRO))
	{
	...
	virtual bool operator()(CNavArea *area, CNavArea *priorArea, float travelDistanceSoFar)
	{
		...
		if(area->IsPotentiallyVisible(m_sentryArea))
		{
			...
			for(int i = 0; i < maxTries; ++i)
			{
				...
				{
					if((m_attackSpot - m_me->GetEnemySentry()->GetAbsOrigin())
						   .IsLengthGreaterThan(1.1f * SENTRY_MAX_RANGE))
					{
...
ActionResult<CTFBot> CTFBotDestroyEnemySentry::Update(CTFBot *me, float interval)
{
	...
	if(me->m_Shared.IsInvulnerable())
	{
		if(!m_wasUber)
		{
			...
			float travelDistance = NavAreaTravelDistance(me->GetLastKnownArea(), m_targetSentry->GetLastKnownArea(),
	...
	if(me->IsPlayerClass(TF_CLASS_DEMOMAN))
	{
		...
		else if(FindStickybombAim(me, m_targetSentry, &aimYaw, &aimPitch, &aimCharge))
		{
			...
			return ChangeTo(new CTFBotStickybombSentrygun(me->GetEnemySentry(), aimYaw, aimPitch, aimCharge),
		...
		if((me->IsRangeLessThan(attackSpot, 50.0f) &&
		   ...
		   (me->IsLineOfFireClear(me->GetEnemySentry()) &&
			me->IsRangeLessThan(m_targetSentry, 1000.0f))) // opportunistic shot
		{
			// reached attack spot
	...
	if(isInAttackPosition || me->IsLineOfFireClear(me->GetEnemySentry()))
	{
		...
		me->GetBodyInterface()->AimHeadTowards(me->GetEnemySentry(), IBody::MANDATORY, 1.0f, NULL,
											   "Aiming at enemy sentry");

		// because sentries are stationary, check if XY is on target to allow SelectTargetPoint() to adjust Z for
		// grenades
...
ActionResult<CTFBot> CTFBotUberAttackEnemySentry::Update(CTFBot *me, float interval)
{
	...
	if(me->IsPlayerClass(TF_CLASS_DEMOMAN) && FindGrenadeAim(me, m_targetSentry, &aimYaw, &aimPitch))
	{
		...
		me->GetBodyInterface()->AimHeadTowards(eyeAimTarget, IBody::CRITICAL, 0.3f, NULL,
