	...
	bool IsSelected(const CBaseEntity *constCandidate) const
	{
		...
		if(m_me->InSameTeam(candidate))
		{
			...
			if(candidate->ClassMatches("obj_dispenser*"))
			{
				...
				if(!m_me->IsPlayerClass(TF_CLASS_ENGINEER) || ((CBaseObject *)candidate)->GetUpgradeLevel() >= 3 ||
				   !m_me->GetObjectOfType(OBJ_SENTRYGUN))
				{
...
bool CTFBotGetAmmo::IsPossible(CTFBot *me)
{
	...
	CollectSurroundingAreas(&nearbyAreaVector, me->GetLastKnownArea(), tf_bot_ammo_search_range.GetFloat(),
