...
ConVar tf_bot_engineer_mvm_sentry_hint_bomb_forward_range("tf_bot_engineer_mvm_sentry_hint_bomb_forward_range", "0",
														  FCVAR_CHEAT);
ConVar tf_bot_engineer_mvm_sentry_hint_bomb_backward_range("tf_bot_engineer_mvm_sentry_hint_bomb_backward_range",
														   "3000", FCVAR_CHEAT);
ConVar tf_bot_engineer_mvm_hint_min_distance_from_bomb("tf_bot_engineer_mvm_hint_min_distance_from_bomb", "1300",
													   FCVAR_CHEAT);
...
bool CTFBotMvMEngineerIdle::ShouldAdvanceNestSpot(CTFBot *me)
{
	...
	if(GetBombInfo(&bombInfo))
	{
		...
		{
			CTFNavArea *hintArea =
				(CTFNavArea *)TheNavMesh->GetNearestNavArea(m_nestHint->GetAbsOrigin(), false, 1000.0f);
			if(hintArea)
			{
...
void CTFBotMvMEngineerIdle::TryToDetonateStaleNest()
{
	...
	for(int i = 0; i < ITFBotHintEntityAutoList::AutoList().Count(); ++i)
	{
		...
		if(pHint->IsHintType(CBaseTFBotHintEntity::HINT_ENGINEER_NEST) && pHint->IsEnabled() &&
		   pHint->GetOwnerEntity() == NULL)
		{
...
ActionResult<CTFBot> CTFBotMvMEngineerIdle::Update(CTFBot *me, float interval)
{
	...
	if(!m_bTeleportedToHint && me->HasAttribute(CTFBot::TELEPORT_TO_HINT))
	{
		...
		return SuspendFor(new CTFBotMvMEngineerTeleportSpawn(m_nestHint, bFirstTeleportSpawn),
	...
	if(m_sentryHint)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					return SuspendFor(new CTFBotMvMEngineerBuildSentryGun(m_sentryHint),
	...
	if(m_teleporterHint && m_sentryInjuredTimer.IsElapsed())
	{
		...
		{
			return SuspendFor(new CTFBotMvMEngineerBuildTeleportExit(m_teleporterHint),
							  "Sentry is safe - building a teleport exit");
		}
	}

	// fix teleporter if sentry is not hurt
	if(myTeleporter && m_sentryInjuredTimer.IsElapsed() && myTeleporter->GetHealth() < myTeleporter->GetMaxHealth() &&
	   !myTeleporter->IsBuilding())
	{
		...
		if(rangeToTeleporter < nearTeleporterRange)
		{
			...
			me->GetBodyInterface()->AimHeadTowards(myTeleporter->WorldSpaceCenter(), IBody::CRITICAL, 1.0f, NULL,
	...
	else if(mySentry)
	{
		...
		{
			// we are in position - hit sentry with wrench
			me->GetBodyInterface()->AimHeadTowards(mySentry->WorldSpaceCenter(), IBody::CRITICAL, 1.0f, NULL,
												   "Work on my Sentry");
			me->PressFireButton();
		}
	}
...
//---------------------------------------------------------------------------------------------
bool CTFBotMvMEngineerHintFinder::FindHint(bool bShouldCheckForBlockingObjects, bool bAllowOutOfRangeNest,
	...
	for(int i = 0; i < activeEngineerNest.Count(); ++i)
	{
		...
		if(hintDistanceToTarget > bombInfo.m_flMinBattleFront && hintDistanceToTarget < bombInfo.m_flMaxBattleFront)
		{
			if(bShouldCheckForBlockingObjects)
			{
				...
				int count = UTIL_EntitiesInBox(pList, ARRAYSIZE(pList), vNestPosition + VEC_HULL_MIN,
											   vNestPosition + VEC_HULL_MAX, FL_CLIENT | FL_OBJECT);
				if(count > 0)
				{
					continue;
				}
			...
			{
				if(VectorLength(bombInfo.m_vPosition - vNestPosition) <
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(tf_bot_mvm_show_engineer_hint_region,
			  ...
			  FCVAR_CHEAT)
{
	...
	UTIL_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + forward * 10000.0f, MASK_SOLID, pPlayer,
		...
		if(area)
		{
			...
			for(int i = 0; i < battlefrontAreaVector.Count(); ++i)
			{
				...
				for(int j = 0; j < hintAreaVector.Count(); ++j)
				{
					...
					{
						CBaseTFBotHintEntity *pHint =
							static_cast<CBaseTFBotHintEntity *>(ITFBotHintEntityAutoList::AutoList()[j]);
						Color color;
						...
						{
							bool bTooCloseToBomb = VectorLength(result.endpos - pHint->GetAbsOrigin()) <
												   tf_bot_engineer_mvm_hint_min_distance_from_bomb.GetFloat();
							color = bTooCloseToBomb ? Color(255, 0, 0) : Color(255, 255, 0);
						}
						NDebugOverlay::Sphere(pHint->GetAbsOrigin(), 50, color.r(), color.g(), color.b(), true,
											  flDrawTime);
					}
				}
			}

			NDebugOverlay::Sphere(result.endpos, tf_bot_engineer_mvm_hint_min_distance_from_bomb.GetFloat(), 255, 255,
								  0, false, flDrawTime);
		}
	}
}