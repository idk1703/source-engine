...
ActionResult<CTFBot> CTFBotEngineerBuildDispenser::Update(CTFBot *me, float interval)
{
	...
	if(me->CanBuild(OBJ_DISPENSER) == CB_NEED_RESOURCES)
	{
		...
		/*
		   ...
		   "Work on sentry while I wait for ammo to show up" ); me->PressFireButton(); return Continue();
				}
		*/
	}
	...
	/*
		...
		if ( mySentry->GetTimeSinceLastInjury() < 1.0f )
		{
			...
			me->GetBodyInterface()->AimHeadTowards( mySentry->WorldSpaceCenter(), IBody::CRITICAL, 1.0f, NULL, "Focusing
	   on keeping my besieged sentry alive" ); me->PressFireButton();
	...
	*/
	...
	else if(m_searchTimer.IsElapsed())
	{
		...
		me->GetBodyInterface()->AimHeadTowards(me->EyePosition() - 100.0f * forward, IBody::CRITICAL, 1.0f, &buildReply,
