...
ActionResult<CTFBot> CTFBotEngineerBuildTeleportExit::Update(CTFBot *me, float interval)
{
	...
	if(m_hasPreciseBuildLocation)
	{
		me->GetBodyInterface()->AimHeadTowards(m_buildLocation, IBody::CRITICAL, 1.0f, NULL,
	...
	if(myGun)
	{
		...
		if(builder)
		{
			...
			else if(m_searchTimer.IsElapsed())
			{
				...
				me->GetBodyInterface()->AimHeadTowards(me->EyePosition() - 100.0f * forward, IBody::CRITICAL, 1.0f,
													   NULL, "Trying to place my teleport exit");
