...
ConVar tf_bot_engineer_retaliate_range(
	"tf_bot_engineer_retaliate_range", "750", FCVAR_CHEAT,
	"If attacker who destroyed sentry is closer than this, attack. Otherwise, retreat");
ConVar tf_bot_engineer_exit_near_sentry_range(
	"tf_bot_engineer_exit_near_sentry_range", "2500", FCVAR_CHEAT,
	"Maximum travel distance between a bot's Sentry gun and its Teleporter Exit");
ConVar tf_bot_engineer_max_sentry_travel_distance_to_point(
	"tf_bot_engineer_max_sentry_travel_distance_to_point", "2500", FCVAR_CHEAT,
...
void CTFBotEngineerBuilding::UpgradeAndMaintainBuildings(CTFBot *me)
{
	...
	if(!myDispenser)
	{
		...
		{
			// crouch both for cover behind our buildings, but also to slow us down so we hit our move goal more
			// accurately
	...
	if(fabs(rangeToDispenser - rangeToSentry) > equalTolerance || rangeToSentry > tooFarRange ||
	   rangeToDispenser > tooFarRange)
	{
		if(m_repathTimer.IsElapsed())
		{
			m_repathTimer.Start(RandomFloat(1.0f, 2.0f));
	...
	if(rangeToSentry < tooFarRange || rangeToDispenser < tooFarRange)
	{
		...
		me->StopLookingAroundForEnemies();
		me->GetBodyInterface()->AimHeadTowards(workTarget->WorldSpaceCenter(), IBody::CRITICAL, 1.0f, NULL,
...
bool CTFBotEngineerBuilding::IsMetalSourceNearby(CTFBot *me) const
{
	...
	CollectSurroundingAreas(&nearbyVector, me->GetLastKnownArea(), 2000.0f,
...
bool CTFBotEngineerBuilding::CheckIfSentryIsOutOfPosition(CTFBot *me) const
{
	...
	if(point)
	{
		...
		{
			CTFBotPathCost cost(me, FASTEST_ROUTE);
			if(NavAreaTravelDistance(sentryArea, pointArea, cost,
									 tf_bot_engineer_max_sentry_travel_distance_to_point.GetFloat()) < 0 &&
			   NavAreaTravelDistance(pointArea, sentryArea, cost,
									 tf_bot_engineer_max_sentry_travel_distance_to_point.GetFloat()) < 0)
			{
				return true;
			}
...
ActionResult<CTFBot> CTFBotEngineerBuilding::Update(CTFBot *me, float interval)
{
	...
	if(!mySentry)
	{
		...
		if(!m_hasBuiltSentry && m_sentryTriesLeft > 0)
		{
			...
			{
				return SuspendFor(new CTFBotEngineerBuildSentryGun(m_sentryBuildHint),
	...
	*/
	...
	else if(m_teleportExitRetryTimer.IsElapsed() && myTeleportEntrance && !isUnderAttack)
	{
		...
		if(m_sentryBuildHint != NULL)
		{
			...
			while((hint = (CTFBotHintTeleporterExit *)gEntList.FindEntityByClassname(
					   hint, "bot_hint_teleporter_exit")) != NULL)
			{
			...
			{
				mySentry->UpdateLastKnownArea();
				CBaseEntity *closeHint = SelectClosestEntityByTravelDistance(
					me, hintVector, mySentry->GetLastKnownArea(), tf_bot_engineer_exit_near_sentry_range.GetFloat());

				if(closeHint)
				{
					return SuspendFor(
						new CTFBotEngineerBuildTeleportExit(closeHint->GetAbsOrigin(), closeHint->GetAbsAngles().y),
