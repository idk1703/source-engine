...
ConVar tf_mvm_bot_flag_carrier_interval_to_1st_upgrade("tf_mvm_bot_flag_carrier_interval_to_1st_upgrade", "5",
													   FCVAR_CHEAT);
ConVar tf_mvm_bot_flag_carrier_interval_to_2nd_upgrade("tf_mvm_bot_flag_carrier_interval_to_2nd_upgrade", "15",
													   FCVAR_CHEAT);
ConVar tf_mvm_bot_flag_carrier_interval_to_3rd_upgrade("tf_mvm_bot_flag_carrier_interval_to_3rd_upgrade", "15",
													   FCVAR_CHEAT);
				...
				switch(m_upgradeLevel)
				{
					...
					case 1:
						...
						if(TFObjectiveResource())
						{
							...
							TFObjectiveResource()->SetBaseMvMBombUpgradeTime(gpGlobals->curtime);
							TFObjectiveResource()->SetNextMvMBombUpgradeTime(gpGlobals->curtime +
																			 m_upgradeTimer.GetRemainingTime());
							TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_BOMB_CARRIER_UPGRADE1,
					...
					case 2:
					{
						...
						else
						{
							...
							if(pAttrList)
							{
								pAttrList->SetRuntimeAttributeValue(pAttrDef_HealthRegen,
																	tf_mvm_bot_flag_carrier_health_regen.GetFloat());
							}
						}
						...
						if(TFObjectiveResource())
						{
							...
							TFObjectiveResource()->SetNextMvMBombUpgradeTime(gpGlobals->curtime +
																			 m_upgradeTimer.GetRemainingTime());
							TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_BOMB_CARRIER_UPGRADE2,
																			   TF_TEAM_PVE_DEFENDERS);
					...
					case 3:
						...
						if(TFObjectiveResource())
						{
							...
							TFObjectiveResource()->SetBaseMvMBombUpgradeTime(-1);
							TFObjectiveResource()->SetNextMvMBombUpgradeTime(-1);
							TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_BOMB_CARRIER_UPGRADE3,
																			   TF_TEAM_PVE_DEFENDERS);
...
ActionResult<CTFBot> CTFBotDeliverFlag::Update(CTFBot *me, float interval)
{
	...
	if(m_repathTimer.IsElapsed())
	{
		...
		m_flTotalTravelDistance =
			NavAreaTravelDistance(me->GetLastKnownArea(), TheNavMesh->GetNavArea(zone->WorldSpaceCenter()), cost);
