...
ConVar tf_bot_max_point_defend_range(
	"tf_bot_max_point_defend_range", "1250", FCVAR_CHEAT,
...
ActionResult<CTFBot> CTFBotDefendPoint::OnStart(CTFBot *me, Action<CTFBot> *priorAction)
{
	...
	m_isAllowedToRoam =
		(RandomFloat(0.0f, 100.0f) < roamChance[(int)clamp(me->GetDifficulty(), CTFBot::EASY, CTFBot::EXPERT)]);
...
CTFNavArea *CTFBotDefendPoint::SelectAreaToDefendFrom(CTFBot *me)
{
	...
	/*
		if ( !TFGameRules()->IsInKothMode() &&
			 ...
			 ( ObjectiveResource()->GetPreviousPointForPoint( point->GetPointIndex(), me->GetTeamNumber(), 0 ) < 0 ||
	   // this is the first cap point me->IsPlayerClass( TF_CLASS_PYRO ) ) )						// pyros are
	   skirmishers
		{
