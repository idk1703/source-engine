...
ConVar tf_bot_capture_seek_and_destroy_min_duration(
	"tf_bot_capture_seek_and_destroy_min_duration", "15", FCVAR_CHEAT,
	"If a capturing bot decides to go hunting, this is the min duration he will hunt for before reconsidering");
ConVar tf_bot_capture_seek_and_destroy_max_duration(
	"tf_bot_capture_seek_and_destroy_max_duration", "30", FCVAR_CHEAT,
...
ActionResult<CTFBot> CTFBotCapturePoint::Update(CTFBot *me, float interval)
{
	...
	if(!isPushingToCapture)
	{
		...
		{
			return SuspendFor(
				new CTFBotSeekAndDestroy(RandomFloat(tf_bot_capture_seek_and_destroy_min_duration.GetFloat(),
	...
	if(me->IsCapturingPoint())
	{
		...
		const CUtlVector<CTFNavArea *> *controlPointAreas =
