...
{
	m_vocalizeTimer.Start(RandomFloat(tf_raid_mob_rush_vocalize_min_interval.GetFloat(),
									  tf_raid_mob_rush_vocalize_max_interval.GetFloat()));
	return Continue();
}
...
ActionResult<CTFBot> CTFBotMobRush::Update(CTFBot *me, float interval)
{
	...
	for(int t = 0; t < team->GetNumPlayers(); ++t)
	{
		...
		if(toBuddy.IsLengthLessThan(tf_raid_mob_avoid_range.GetFloat()))
		{
			...
			me->GetLocomotionInterface()->Approach(me->GetAbsOrigin() - 100.0f * toBuddy,
												   1.0f - (range / tf_raid_mob_avoid_range.GetFloat()));
		}
	}
...
//---------------------------------------------------------------------------------------------
EventDesiredResult<CTFBot> CTFBotMobRush::OnOtherKilled(CTFBot *me, CBaseCombatCharacter *victim,
