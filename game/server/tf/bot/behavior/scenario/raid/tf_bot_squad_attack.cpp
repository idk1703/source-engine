...
{
	m_vocalizeTimer.Start(
		RandomFloat(tf_raid_squad_vocalize_min_interval.GetFloat(), tf_raid_squad_vocalize_max_interval.GetFloat()));
	m_victim = NULL;
...
ActionResult<CTFBot> CTFBotSquadAttack::Update(CTFBot *me, float interval)
{
	...
	if(m_victim)
	{
		...
		if(me->IsPlayerClass(TF_CLASS_PYRO) || me->IsRangeGreaterThan(m_victim->GetAbsOrigin(), engageRange) ||
		...
		if(tf_squad_debug.GetBool() && me->IsSelf(leader))
		{
			...
			for(it = squad->GetFirstMember(); it != squad->InvalidIterator(); it = squad->GetNextMember(it))
			{
				...
				NDebugOverlay::Line(me->WorldSpaceCenter(), bot->WorldSpaceCenter(), 0, 255, 0, true,
	...
	{
		m_vocalizeTimer.Start(RandomFloat(tf_raid_squad_vocalize_min_interval.GetFloat(),
										  tf_raid_squad_vocalize_max_interval.GetFloat()));
