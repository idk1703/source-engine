...
ConVar tf_raid_wanderer_aggro_range("tf_raid_wanderer_aggro_range", "500", FCVAR_CHEAT,
									"If wanderers see a threat closer than this, they attack");
ConVar tf_raid_wanderer_notice_friend_death_range(
	"tf_raid_wanderer_notice_friend_death_range", "1000", FCVAR_CHEAT,
	"If a friend dies within this radius of a wanderer, it wakes up and attacks the attacker");
...
{
	m_vocalizeTimer.Start(RandomFloat(tf_raid_wanderer_vocalize_min_interval.GetFloat(),
									  tf_raid_wanderer_vocalize_max_interval.GetFloat()));
...
EventDesiredResult<CTFBot> CTFBotWander::OnContact(CTFBot *me, CBaseEntity *other, CGameTrace *result)
{
	...
	{
		return TrySuspendFor(new CTFBotMobRush((CTFPlayer *)other), RESULT_IMPORTANT,
							 "Attacking threat who touched me!");
	}
...
EventDesiredResult<CTFBot> CTFBotWander::OnInjured(CTFBot *me, const CTakeDamageInfo &info)
{
	...
	{
		return TrySuspendFor(new CTFBotMobRush((CTFPlayer *)info.GetAttacker()), RESULT_IMPORTANT,
							 "Attacking threat who attacked me!");
	}
...
//---------------------------------------------------------------------------------------------
EventDesiredResult<CTFBot> CTFBotWander::OnOtherKilled(CTFBot *me, CBaseCombatCharacter *victim,
		...
		if(info.GetAttacker() && info.GetAttacker()->IsPlayer() && me->IsEnemy(info.GetAttacker()))
		{
			if(me->IsRangeLessThan(victim, tf_raid_wanderer_notice_friend_death_range.GetFloat()))
			{
				   ...
				   me->GetVisionInterface()->IsAbleToSee(info.GetAttacker(), IVision::DISREGARD_FOV))
				{
					...
					{
						reactionTime = tf_raid_wanderer_reaction_factor.GetFloat() *
									   (rangeToAttacker - tf_raid_wanderer_aggro_range.GetFloat()) /
									   tf_raid_wanderer_aggro_range.GetFloat();
					}

					return TrySuspendFor(new CTFBotMobRush((CTFPlayer *)info.GetAttacker(), reactionTime),
										 RESULT_IMPORTANT, "Attacking my friend's attacker!");
				}
			}
		}
	}
