			...
			if(pipeBombVector.Count() > 0)
			{
				...
				for(int p = 0; p < pipeBombVector.Count(); ++p)
				{
					...
					for(int k = 0; k < knownVector.Count(); ++k)
					{
						...
						if((knownVector[k].GetLastKnownPosition() - sticky->GetAbsOrigin())
							   .IsLengthLessThan(closeRange))
						{
...
ActionResult<CTFBot> CTFBotTacticalMonitor::Update(CTFBot *me, float interval)
{
		...
		if(m_acknowledgeRetryTimer.IsElapsed())
		{
			...
			if(watcher)
			{
				...
				if(m_attentionTimer.HasStarted() && m_attentionTimer.IsElapsed())
				{
					...
					{
						// look toward them
						me->GetBodyInterface()->AimHeadTowards(watcher, IBody::IMPORTANT, 3.0f, NULL,
	...
	if(isAvailable && m_maintainTimer.IsElapsed())
	{
		...
		{
			isHurt = me->m_Shared.InCond(TF_COND_BURNING) ||
	...
	if(ShouldOpportunisticallyTeleport(me))
	{
		...
		if(nearbyTeleporter)
		{
			...
			if(teleporterArea && myArea &&
			   myArea->GetIncursionDistance(me->GetTeamNumber()) <
				   350.0f + teleporterArea->GetIncursionDistance(me->GetTeamNumber()))
			{
	...
	{
		return SuspendFor(new CTFBotWaitForOutOfPositionSquadMember,
...
//-----------------------------------------------------------------------------------------
EventDesiredResult<CTFBot> CTFBotTacticalMonitor::OnOtherKilled(CTFBot *me, CBaseCombatCharacter *victim,
	...
	if(newArea && newArea->HasPrerequisite(me) && !me->HasAttribute(CTFBot::AGGRESSIVE))
	{
		...
		for(int i = 0; i < prereqVector.Count(); ++i)
		{
			...
			if(prereq && prereq->IsEnabled() && const_cast<CFuncNavPrerequisite *>(prereq)->PassesTriggerFilters(me))
			{
				...
				{
					return TrySuspendFor(new CTFBotNavEntWait(prereq), RESULT_IMPORTANT,
										 "Prerequisite commands me to wait");
				}
				else if(prereq->IsTask(CFuncNavPrerequisite::TASK_MOVE_TO_ENTITY))
				{
					return TrySuspendFor(new CTFBotNavEntMoveTo(prereq), RESULT_IMPORTANT,
...
EventDesiredResult<CTFBot> CTFBotTacticalMonitor::OnCommandString(CTFBot *me, const char *command)
{
	...
	else if(FStrEq(command, "build sentry at nearest sentry hint"))
	{
		if(me->IsPlayerClass(TF_CLASS_ENGINEER))
		{
			...
			for(sentryHint =
					static_cast<CTFBotHintSentrygun *>(gEntList.FindEntityByClassname(NULL, "bot_hint_sentrygun"));
			...
			if(bestSentryHint)
			{
				...
				return TrySuspendFor(new CTFBotEngineerBuilding(bestSentryHint), RESULT_CRITICAL,
	...
	{
		return TrySuspendFor(new CTFTrainingAttackSentryActionPoint(), RESULT_CRITICAL,
	...
	else if(FStrEq(command, "become raider"))
	{
		...
		me->GetPlayerClass()->SetCustomModel(g_szBotBossModels[me->GetPlayerClass()->GetClassIndex()],
											 USE_CLASS_ANIMATIONS);
		me->UpdateModel();
		...
		botAttribs_t sAttribs[] = {
			{"move speed bonus", 0.5f},			  {"damage bonus", 1.5f},
			{"damage force reduction", 0.3f},	  {"airblast vulnerability multiplier", 0.3f},
		...
		if(pAttribList)
		{
			...
			{
				const CEconItemAttributeDefinition *pDef =
					ItemSystem()->GetItemSchema()->GetAttributeDefinitionByName(sAttribs[i].szName);
				if(pDef)
				{
