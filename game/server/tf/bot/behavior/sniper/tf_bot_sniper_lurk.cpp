...
ConVar tf_bot_sniper_patience_duration(
	"tf_bot_sniper_patience_duration", "10", FCVAR_CHEAT,
...
ConVar tf_bot_sniper_allow_opportunistic(
	"tf_bot_sniper_allow_opportunistic", "1", FCVAR_NONE,
...
ActionResult<CTFBot> CTFBotSniperLurk::Update(CTFBot *me, float interval)
{
	...
	if(threat && threat->GetTimeSinceLastSeen() < tf_bot_sniper_target_linger_duration.GetFloat() &&
...
bool CTFBotSniperLurk::FindHint(CTFBot *me)
{
	...
	if(m_priorHint != NULL && m_failCount < 2)
	{
		...
		for(int i = 0; i < activeHintVector.Count(); ++i)
		{
			...
			if((activeHintVector[i]->WorldSpaceCenter() - m_priorHint->WorldSpaceCenter())
				   .IsLengthGreaterThan(nearRange))
				continue;
	...
	{
		// picking either our first hint, or we haven't seen a victim in a long time - pick a hint that can actually see
		// someone
		...
		for(int i = 0; i < activeHintVector.Count(); ++i)
		{
			...
			{
				if(victimVector[p]->IsLineOfSightClear(activeHintVector[i]->WorldSpaceCenter(),
													   CBaseCombatCharacter::IGNORE_ACTORS))
				{
