...
//---------------------------------------------------------------------------------------------
CTFBotStickybombSentrygun::CTFBotStickybombSentrygun(CObjectSentrygun *sentrygun, float aimYaw, float aimPitch,
													 float aimCharge)
{
	m_sentrygun = sentrygun;
	m_hasGivenAim = true;
...
ActionResult<CTFBot> CTFBotStickybombSentrygun::Update(CTFBot *me, float interval)
{
	...
	else if(m_isChargingShot)
	{
		...
		me->GetBodyInterface()->AimHeadTowards(m_eyeAimTarget, IBody::CRITICAL, 0.3f, NULL,
											   "Aiming a sticky bomb at a sentrygun");
