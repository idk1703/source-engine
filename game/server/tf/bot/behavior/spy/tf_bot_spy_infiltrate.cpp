...
ActionResult<CTFBot> CTFBotSpyInfiltrate::Update(CTFBot *me, float interval)
{
	...
	if(!me->m_Shared.IsStealthed() && !isInMySpawn && myArea->IsInCombat() && !m_hasEnteredCombatZone)
	{
			...
			if(victim)
			{
				...
				if(victimArea)
				{
					...
					if(victimArea->GetIncursionDistance(victimTeam) > myArea->GetIncursionDistance(victimTeam))
					{
						...
						{
							return SuspendFor(new CTFBotRetreatToCover(new CTFBotSpyAttack(victim)),
...
bool CTFBotSpyInfiltrate::FindHidingSpot(CTFBot *me)
{
	...
	for(int x = 0; x < enemySpawnExitVector->Count(); ++x)
	{
		...
		CollectSurroundingAreas(&nearbyThisExitAreaVector, enemySpawnExitArea, nearbyHideRange,
	...
	if(hideAreaVector.Count() == 0)
	{
		if(tf_bot_debug_spy.GetBool())
		{
			DevMsg("%3.2f: Can't find any areas near the enemy spawn exit - just heading to the enemy spawn and "
				   "hoping...\n",
				   gpGlobals->curtime);
		}
