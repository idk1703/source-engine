...
ActionResult<CTFBot> CTFBotSpyAttack::Update(CTFBot *me, float interval)
{
	...
	{
		return ChangeTo(new CTFBotSpySap(sapTarget),
	...
	if(me->IsThreatAimingTowardMe(playerThreat, 0.99f) &&
	   me->GetTimeSinceLastInjury(GetEnemyTeam(me->GetTeamNumber())) < 1.0f)
	{
		m_isCoverBlown |= (playerThreat->GetTimeSinceWeaponFired() < 0.25f);
	}

	if(m_isCoverBlown || me->m_Shared.InCond(TF_COND_BURNING) || me->m_Shared.InCond(TF_COND_URINE) ||
	   me->m_Shared.InCond(TF_COND_STEALTHED_BLINK) || me->m_Shared.InCond(TF_COND_BLEEDING))
	{
		isKnifeFight = false;
	}
		...
		if(threat->IsVisibleInFOVNow())
		{
			...
			if(threatRange < circleStrafeRange)
			{
				...
				if(!isBehindVictim)
				{
					...
					// don't continue to close in if we're already very close so we don't bump them and give ourselves
					// away
