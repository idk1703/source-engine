...
CTFBotEscortSquadLeader::CTFBotEscortSquadLeader(
	Action<CTFBot> *actionToDoAfterSquadDisbands) // : m_path( ChasePath::LEAD_SUBJECT )
{
...
ActionResult<CTFBot> CTFBotEscortSquadLeader::Update(CTFBot *me, float interval)
{
	...
	UTIL_TraceLine(leader->GetAbsOrigin() + Vector(0, 0, HalfHumanHeight),
				   myFormationSpot + Vector(0, 0, HalfHumanHeight), MASK_PLAYERSOLID, &filter, &result);

	if(result.DidHitWorld())
	{
		myFormationSpot = result.endpos - Vector(0, 0, HalfHumanHeight) +
						  0.6f * me->GetBodyInterface()->GetHullWidth() * result.plane.normal;
	}
