...
ActionResult<CTFBot> CTFBotMvMDeployBomb::OnStart(CTFBot *me, Action<CTFBot> *priorAction)
{
	...
	if(me->IsMiniBoss())
	{
		static CSchemaAttributeDefHandle pAttrDef_AirblastVerticalVulnerability(
...
ActionResult<CTFBot> CTFBotMvMDeployBomb::Update(CTFBot *me, float interval)
{
	...
	if(me->GetDeployingBombState() != TF_BOMB_DEPLOYING_COMPLETE)
	{
		...
		me->GetBodyInterface()->AimHeadTowards(pAreaTrigger->WorldSpaceCenter(), IBody::CRITICAL, 0.5f, NULL,
...
{
	// so event doesn't fall thru to buried action which will then redo transition to this state as we stay in contact
	// with the zone
