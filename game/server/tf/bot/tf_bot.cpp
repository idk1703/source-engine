...
ConVar tf_bot_pyro_always_reflect(
	"tf_bot_pyro_always_reflect", "0", FCVAR_CHEAT,
...
ConVar tf_bot_suspect_spy_forget_cooldown("tf_bot_suspect_spy_forget_cooldown", "5", FCVAR_CHEAT,
...
{
	static const char *nameList[] = {"Chucklenuts",
									 "CryBaby",
									 "WITCH",
									 "ThatGuy",
									 "Still Alive",
...
void CreateBotName(int iTeam, int iClassIndex, CTFBot::DifficultyType skill, char *pBuffer, int iBufferSize)
{
	...
	if(TFGameRules()->IsInTraining())
	{
		...
		if(pLocalizedTitle)
		{
			g_pVGuiLocalize->ConvertUnicodeToANSI(pLocalizedTitle, szEnemyOrFriendlyString,
												  sizeof(szEnemyOrFriendlyString));
	...
	CFmtStr name("%s%s%s", pDifficultyString, pFriendlyOrEnemyTitle, pBotName);
...
CON_COMMAND_F(tf_bot_add, "Add a bot.", FCVAR_GAMEDLL)
{
	...
	CTFBot::DifficultyType skill =
		clamp((CTFBot::DifficultyType)tf_bot_difficulty.GetInt(), CTFBot::EASY, CTFBot::EXPERT);
	...
	for(i = 0; i < botCount; ++i)
	{
		...
		if(pBot)
		{
			...
			if(TFGameRules()->IsInTraining())
			{
				CreateBotName(pBot->GetTeamNumber(), pBot->GetPlayerClass()->GetClassIndex(), skill, name,
							  sizeof(name));
...
CON_COMMAND_F(tf_bot_kick, "Remove a TFBot by name, or all bots (\"all\").", FCVAR_GAMEDLL)
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; ++i)
	{
		...
		{
			if(iTeam == TEAM_ANY || FStrEq(pPlayerName, player->GetPlayerName()) ||
			   (player->GetTeamNumber() == iTeam) || (player->GetTeamNumber() == iTeam))
			{
...
static ConCommand tf_bot_warp_team_to_me("tf_bot_warp_team_to_me", CMD_BotWarpTeamToMe, "",
...
const char *CTFBot::GetNextSpawnClassname(void) const
{
	...
	static ClassSelectionInfo offenseRoster[] = {
		...
		{TF_CLASS_DEMOMAN,
		 ...
		 0,
		 0,
		 {2, 3, 3, 3}}, // must limit demomen, or the whole team will go demo to take out tough sentryguns
		{TF_CLASS_PYRO, 3, 0, 0, {NoLimit, NoLimit, NoLimit, NoLimit}},
		{TF_CLASS_HEAVYWEAPONS, 3, 0, 0, {1, 1, 2, 2}},
		{TF_CLASS_MEDIC, 4, 4, 1, {1, 1, 2, 2}},
		{TF_CLASS_SNIPER, 5, 0, 0, {0, 1, 1, 1}},
	...
	static ClassSelectionInfo compRoster[] = {
		...
		{TF_CLASS_DEMOMAN,
		 ...
		 0,
		 0,
		 {0, 0, 2, 2}}, // must limit demomen, or the whole team will go demo to take out tough sentryguns
		{TF_CLASS_PYRO, 0, -1},
...
void CTFBot::PhysicsSimulate(void)
{
	...
	if(!IsAlive() && !m_didReselectClass && tf_bot_keep_class_after_death.GetBool() == false &&
	   TFGameRules()->CanBotChangeClass(this))
	{
		if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
			return;

		const char *classname =
			FStrEq(tf_bot_force_class.GetString(), "") ? GetNextSpawnClassname() : tf_bot_force_class.GetString();
...
void CTFBot::Event_Killed(const CTakeDamageInfo &info)
{
	...
	if(TFGameRules()->IsMannVsMachineMode())
	{
		...
		{
			// in MVM, when an engineer dies, we need to decouple his objects so they stay alive when his bot slot gets
			// recycled
			...
			{
				CBaseTFBotHintEntity *pHint =
					static_cast<CBaseTFBotHintEntity *>(ITFBotHintEntityAutoList::AutoList()[i]);
...
CTeamControlPoint *CTFBot::SelectPointToCapture(CUtlVector<CTeamControlPoint *> *captureVector) const
{
	...
	// if we're near a point that is being captured, go help (in the event multiple points are being simultaneously
	// captured)
...
bool CTFBot::IsNearPoint(CTeamControlPoint *point) const
{
	...
	float travelToPoint =
		fabs(myArea->GetIncursionDistance(GetTeamNumber()) - pointArea->GetIncursionDistance(GetTeamNumber()));
...
void CTFBot::SetupSniperSpotAccumulation(void)
{
	...
	if(TFGameRules()->GetGameType() == TF_GAMETYPE_ESCORT)
	{
		...
		goalEntityArea = (CTFNavArea *)TheTFNavMesh()->GetNearestNavArea(goalEntity->WorldSpaceCenter(),
																		 GETNAVAREA_CHECK_GROUND, 500.0f);
	}
	else
	{
...
void CTFBot::AccumulateSniperSpots(void)
{
	...
	for(int count = 0; count < tf_bot_sniper_spot_search_count.GetInt(); ++count)
	{
		...
		{
			if((info.m_vantageSpot - m_sniperSpotVector[i].m_vantageSpot)
				   .IsLengthLessThan(tf_bot_sniper_spot_epsilon.GetFloat()))
			{
		...
		if(IsLineOfFireClear(info.m_vantageSpot + eyeOffset, info.m_theaterSpot + eyeOffset))
		{
			...
			info.m_advantage = info.m_vantageArea->GetIncursionDistance(GetEnemyTeam(GetTeamNumber())) -
							   info.m_theaterArea->GetIncursionDistance(GetEnemyTeam(GetTeamNumber()));
	...
	if(IsDebugging(NEXTBOT_BEHAVIOR))
	{
		for(int i = 0; i < m_sniperSpotVector.Count(); ++i)
		{
			...
			NDebugOverlay::Line(m_sniperSpotVector[i].m_vantageSpot, m_sniperSpotVector[i].m_theaterSpot, 0, 200, 0,
								true, 0.1f);
		}
	}
}
...
{
public:
	CCollectReachableObjects(const CTFBot *me, float maxRange, const CUtlVector<CHandle<CBaseEntity>> &potentialVector,
							 CUtlVector<CHandle<CBaseEntity>> *collectionVector)
...
// Items in selectedObjectVector will be approximately sorted in nearest-to-farthest order (because of
...
void CTFBot::SelectReachableObjects(const CUtlVector<CHandle<CBaseEntity>> &candidateObjectVector,
									...
									const INextBotFilter &filter, CNavArea *startSearchArea, float maxRange) const
{
...
bool CTFBot::IsAmmoLow(void) const
{
	...
	if(myWeapon)
	{
		...
		float ratio =
			(float)GetAmmoCount(TF_AMMO_PRIMARY) / (float)(const_cast<CTFBot *>(this)->GetMaxAmmo(TF_AMMO_PRIMARY));

		if(ratio < 0.2f)
		{
			return true;
		}
		// if ( !myWeapon->HasPrimaryAmmo() && myWeapon->GetWeaponID() != TF_WEAPON_BUILDER && myWeapon->GetWeaponID()
		// != TF_WEAPON_MEDIGUN )
	}
...
bool CTFBot::IsAmmoFull(void) const
{
	...
	/*
	   ...
	   weaponInfoHandle ) ); if ( weaponInfo && weaponInfo->GetWeaponData( TF_WEAPON_PRIMARY_MODE ).m_iProjectile ==
	   TF_PROJECTILE_NONE )
					{
						// we don't shoot anything, so we don't need ammo
						return true;
			...
			bool isPrimaryFull = GetAmmoCount( TF_AMMO_PRIMARY ) >= const_cast< CTFBot * >( this )->GetMaxAmmo(
	   TF_AMMO_PRIMARY ); bool isSecondaryFull = GetAmmoCount( TF_AMMO_SECONDARY ) >= const_cast< CTFBot * >( this
	   )->GetMaxAmmo( TF_AMMO_SECONDARY );
		...
		if(invasionAreaVector.Count() > 0)
		{
			...
			for(int r = 0; r < retryCount; ++r)
			{
				...
				if(IsRangeGreaterThan(gazeSpot, minGazeRange) && GetVisionInterface()->IsLineOfSightClear(gazeSpot))
				{
					...
					GetBodyInterface()->AimHeadTowards(gazeSpot, IBody::INTERESTING, maxLookInterval, NULL,
...
void CTFBot::UpdateLookingAroundForEnemies(void)
{
	...
	if(known)
	{
		...
		{
			if(IsPlayerClass(TF_CLASS_SPY) && GetDifficulty() >= CTFBot::HARD && m_Shared.InCond(TF_COND_DISGUISED) &&
			...
			// I see you!
			GetBodyInterface()->AimHeadTowards(known->GetEntity(), IBody::CRITICAL, 1.0f, NULL,
		...
		/* apparently sounds update last known position...
				if ( known->WasEverVisible() && known->GetTimeSinceLastSeen() < 3.0f )
				{
					...
					GetBodyInterface()->AimHeadTowards( known->GetLastKnownPosition() + GetClassEyeHeight(),
		   IBody::IMPORTANT, 1.0f, NULL, "Aiming at a last known threat position" ); return;
				}
		*/
		...
		if(IsLineOfSightClear(known->GetEntity(), IGNORE_ACTORS))
		{
			...
			GetBodyInterface()->AimHeadTowards(imperfectAimSpot, IBody::IMPORTANT, 1.0f, NULL,
		...
		if(!IsPlayerClass(TF_CLASS_SNIPER))
		{
			...
			if(myArea)
			{
				...
				if(closeArea)
				{
					...
					for(int r = 0; r < retryCount; ++r)
					{
						...
						if(GetVisionInterface()->IsLineOfSightClear(gazeSpot))
						{
							...
							GetBodyInterface()->AimHeadTowards(
								gazeSpot, IBody::IMPORTANT, maxLookInterval, NULL,
					...
					if(IsDebugging(NEXTBOT_VISION | NEXTBOT_ERRORS))
					{
						ConColorMsg(Color(255, 255, 0, 255),
									"%3.2f: %s can't find clear line to look at potentially visible near known but "
									"hidden entity %s(#%d)\n",
									gpGlobals->curtime, GetDebugIdentifier(), known->GetEntity()->GetClassname(),
				...
				else if(IsDebugging(NEXTBOT_VISION | NEXTBOT_ERRORS))
				{
					ConColorMsg(
						Color(255, 255, 0, 255),
						"%3.2f: %s no potentially visible area to look toward known but hidden entity %s(#%d)\n",
						gpGlobals->curtime, GetDebugIdentifier(), known->GetEntity()->GetClassname(),
						known->GetEntity()->entindex());
				}
			}
 ...
 * @todo: Differentiate between potential threats (that sentry up ahead along our route) and immediate threats (the
 * sentry I'm in range of)
 */
float CTFBot::GetThreatDanger(CBaseCombatCharacter *who) const
{
	...
	else
	{
		...
		if(sentry)
		{
			if(!sentry->IsAlive() || sentry->IsPlacing() || sentry->HasSapper() || sentry->IsPlasmaDisabled() ||
			   sentry->IsUpgrading() || sentry->IsBuilding())
				return 0.0f;
...
void CTFBot::EquipBestWeaponForThreat(const CKnownEntity *threat)
{
	...
	switch(GetPlayerClass()->GetClassIndex())
	{
		...
		case TF_CLASS_SOLDIER:
		{
			// if we've emptied our rocket launcher clip and are fighting a nearby threat, switch to our secondary if it
			// is ready to fire
			if(gun && !gun->Clip1())
			{
...
bool CTFBot::EquipLongRangeWeapon(void)
{
	...
	if(IsPlayerClass(TF_CLASS_SOLDIER) || IsPlayerClass(TF_CLASS_DEMOMAN) || IsPlayerClass(TF_CLASS_HEAVYWEAPONS) ||
...
Action<CTFBot> *CTFBot::OpportunisticallyUseWeaponAbilities(void)
{
	...
	if(IsPlayerClass(TF_CLASS_DEMOMAN) && m_Shared.IsShieldEquipped())
	{
		Vector forward;
		EyeVectors(&forward);
		bool bShouldCharge = GetLocomotionInterface()->IsPotentiallyTraversable(
			GetAbsOrigin(), GetAbsOrigin() + 100.0f * forward, ILocomotion::IMMEDIATELY);
	...
	else if(m_Shared.IsParachuteEquipped())
	{
		...
		if(!m_Shared.InCond(TF_COND_PARACHUTE_DEPLOYED))
		{
			...
			if(flHealthPercent >= flHealthThreshold && !bIsBurning && GetAbsVelocity().z < 0 &&
			   GetLocomotionInterface()->IsPotentiallyTraversable(
				   GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -flMinParachuteGroundDistance),
		...
		else
		{
			...
			if(flHealthPercent < flHealthThreshold || bIsBurning ||
			   !GetLocomotionInterface()->IsPotentiallyTraversable(
				   GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -flCancelParachuteDistance), ILocomotion::IMMEDIATELY))
			{
				PressJumpButton();
			}
		}
...
void CTFBot::OnEventChangeAttributes(const CTFBot::EventChangeAttributes_t *pEvent)
{
	if(pEvent)
	{
		...
		// cache off health value before we clear attribute because ModifyMaxHealth adds new attribute and reset the
		// health
		...
		FOR_EACH_VEC(pEvent->m_itemsAttributes, i)
		{
			...
			for(int iItemSlot = LOADOUT_POSITION_PRIMARY; iItemSlot < CLASS_LOADOUT_POSITION_COUNT; iItemSlot++)
			{
				...
				CEconItemView *pCurItemData =
					CTFPlayerSharedUtils::GetEconItemViewByLoadoutSlot(this, iItemSlot, &pEntity);
				if(pCurItemData && itemDef && (pCurItemData->GetItemDefIndex() == itemDef->GetDefinitionIndex()))
				{
					for(int iAtt = 0; iAtt < itemAttributes.m_attributes.Count(); ++iAtt)
					{
						...
						if(pAttribList)
						{
							pAttribList->SetRuntimeAttributeValue(attrib.GetAttributeDefinition(),
																  attrib.m_value.asFloat);
						}
					}
...
void CTFBot::AddItem(const char *pszItemName)
{
	...
	if(pItem)
	{
		...
		equip_region_mask_t unNewItemRegionMask =
			pScriptItem->GetItemDefinition() ? pScriptItem->GetItemDefinition()->GetEquipRegionConflictMask() : 0;
		...
		if(IsWearableSlot(iSlot))
		{
			...
			for(int wbl = 0; wbl < GetNumWearables(); wbl++)
			{
				...
				{
					unWearableRegionMask = pWearable->GetAttributeContainer()
											   ->GetItem()
											   ->GetItemDefinition()
