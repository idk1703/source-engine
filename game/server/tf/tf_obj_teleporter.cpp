...
SendPropInt(SENDINFO(m_iState), 5), SendPropTime(SENDINFO(m_flRechargeTime)),
	SendPropTime(SENDINFO(m_flCurrentRechargeDuration)), SendPropInt(SENDINFO(m_iTimesUsed), 10, SPROP_UNSIGNED),
	SendPropFloat(SENDINFO(m_flYawToExit), 8, 0, 0.0, 360.0f), SendPropBool(SENDINFO(m_bMatchBuilding)),
END_SEND_TABLE
()
	...
	// other
	DEFINE_THINKFUNC(TeleporterThink), DEFINE_ENTITYFUNC(TeleporterTouch),
END_DATADESC
()
...
ConVar tf_teleporter_fov_start("tf_teleporter_fov_start", "120", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							   "Starting FOV for teleporter zoom.", true, 1, false, 0);
ConVar tf_teleporter_fov_time("tf_teleporter_fov_time", "0.5", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							  "How quickly to restore FOV after teleport.", true, 0.0, false, 0);
...
void CObjectTeleporter::TeleporterSend(CTFPlayer *pPlayer)
{
	...
	if(GetBuilder() && GetBuilder()->GetTeam() == pPlayer->GetTeam())
	{
		...
		EconEntity_OnOwnerKillEaterEvent(
			dynamic_cast<CEconEntity *>(GetBuilder()->GetEntityForLoadoutSlot(LOADOUT_POSITION_PDA)), GetBuilder(),
			pPlayer, kKillEaterEvent_TeleportsProvided);

		if(GetBuilder() != pPlayer && TFGameRules() && TFGameRules()->GameModeUsesUpgrades() &&
...
void CObjectTeleporter::InitializeMapPlacedObject(void)
{
	...
	m_hMatchingTeleporter =
		dynamic_cast<CObjectTeleporter *>(gEntList.FindEntityByName(NULL, m_iszMatchingMapPlacedTeleporter.ToCStr()));

	// Select the teleporter with the most upgrade
	if(m_hMatchingTeleporter.Get())
	{
		bool bFrom = (m_hMatchingTeleporter->GetUpgradeLevel() > GetUpgradeLevel() ||
					  m_hMatchingTeleporter->GetUpgradeMetal() > GetUpgradeMetal());
...
bool CObjectTeleporter::IsPlacementPosValid(void)
{
	...
	trace_t tr;
	UTIL_TraceHull(vecTestPos, vecTestPos, VEC_HULL_MIN, VEC_HULL_MAX, MASK_SOLID | CONTENTS_PLAYERCLIP, this,
...
void CObjectTeleporter::OnGoActive(void)
{
	...
	m_flLastStateChangeTime =
		0.0f; // used as a flag to initialize the playback rate to 0 in the first DeterminePlaybackRate
...
void CObjectTeleporter::ApplySpeedBoost(CTFPlayer *pPlayer)
{
	...
	m_flRechargeTime = gpGlobals->curtime +
					   (BUILD_TELEPORTER_FADEOUT_TIME + BUILD_TELEPORTER_FADEIN_TIME + m_flCurrentRechargeDuration);
...
bool CObjectTeleporter::IsMatchingTeleporterReady(void)
{
	...
	if(m_hMatchingTeleporter && m_hMatchingTeleporter->GetState() != TELEPORTER_STATE_BUILDING &&
	   !m_hMatchingTeleporter->IsUpgrading() && !m_hMatchingTeleporter->IsDisabled())
		return true;
			...
			case TELEPORTER_STATE_RECHARGING:
			{
				...
				if(flTimeSinceChange <= flFirstStage)
				{
					flPlaybackRate = RemapVal(gpGlobals->curtime, m_flLastStateChangeTime,
											  m_flLastStateChangeTime + flFirstStage, 1.0f, flLowSpinSpeed);
				}
				...
				{
					flPlaybackRate = RemapVal(gpGlobals->curtime, m_flLastStateChangeTime + flSecondStage,
											  m_flLastStateChangeTime + flTotalTime, flLowSpinSpeed, 1.0f);
				}
			}
			break;
	...
	if(pTeleportingPlayer)
	{
		...
		if(numEnts)
		{
			...
			for(int i = 0; i < numEnts && bClear; i++)
			{
				...
				if(pEnts[i]->IsSolid() &&
				   pEnts[i]->ShouldCollide(pTeleportingPlayer->GetCollisionGroup(), MASK_SOLID) &&
		...
		if(bClear)
		{
			...
			{
				hPlayersToKill[player]->TakeDamage(
					CTakeDamageInfo(pTeleportingPlayer, pTeleportingPlayer, 1000, DMG_CRUSH, TF_DMG_CUSTOM_TELEFRAG));
			}
			...
			pTeleportingPlayer->SetFOV(pTeleportingPlayer, 0, tf_teleporter_fov_time.GetFloat(),
									   tf_teleporter_fov_start.GetInt());
...
void CObjectTeleporter::TeleporterThink(void)
{
	...
	SetContextThink(&CObjectTeleporter::TeleporterThink, gpGlobals->curtime + BUILD_TELEPORTER_NEXT_THINK,
					TELEPORTER_THINK_CONTEXT);
	...
	switch(m_iState)
	{
		...
		case TELEPORTER_STATE_SENDING:
		{
			...
			if(!m_bWasMapPlaced)
			{
				CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(GetBuilder(), m_flCurrentRechargeDuration,
												mult_teleporter_recharge_rate);
			}

			m_flRechargeTime = gpGlobals->curtime + (BUILD_TELEPORTER_FADEOUT_TIME + BUILD_TELEPORTER_FADEIN_TIME +
...
CObjectTeleporter *CObjectTeleporter::FindMatch(void)
{
	...
	for(i = 0; i < iNumObjects; i++)
	{
		...
		if(pObj && (pObj != this) && (iObjType == pObj->GetType()))
		{
			...
			if(pTele && ((IsEntrance() && pTele->IsExit()) || (IsExit() && pTele->IsEntrance())))
			{
				pMatch = pTele;
...
{
	if(FStrEq(event->GetName(), "player_spawn") || FStrEq(event->GetName(), "player_team"))
	{
