...
SendPropInt(SENDINFO(m_iCollisionCount)), SendPropEHandle(SENDINFO(m_hHomingTarget)),
	SendPropEHandle(SENDINFO(m_hCarrier)), SendPropEHandle(SENDINFO(m_hPrevCarrier)),
END_SEND_TABLE
()
...
void CPasstimeBall::ChangeTeam(int iTeam)
{
	...
	if(TFGameRules() && TFGameRules()->IsPasstimeMode() && g_pPasstimeLogic)
	{
		...
		if(((state == GR_STATE_RND_RUNNING) || (state == GR_STATE_STALEMATE) || (state == GR_STATE_TEAM_WIN)) &&
		   (flElapsedTimeOnThisTeam > 0))
		{
			...
			if(!pPlayer)
				pPlayer = GetCarrier(); // this happens when the round ends or player dies or something
			...
			if(pPlayer)
			{
				...
				{
					CTF_GameStats.m_passtimeStats.summary.nTotalWinningTeamBallCarrySec +=
						Float2Int(flElapsedTimeOnThisTeam);
				}
				else if(pPlayerTeam->GetFlagCaptures() < pPlayerEnemyTeam->GetFlagCaptures())
				{
					CTF_GameStats.m_passtimeStats.summary.nTotalLosingTeamBallCarrySec +=
						Float2Int(flElapsedTimeOnThisTeam);
				}
			}
		}
...
void CPasstimeBall::CreateSphereCollider()
{
	...
	IPhysicsObject *pPhysObj =
		physenv->CreateSphereObject(flBallRadius, iPhysMat, GetAbsOrigin(), GetAbsAngles(), &params, false);
...
void CPasstimeBall::SetStateFree()
{
	...
	if(!m_pHumLoop)
	{
		...
		m_pHumLoop = CSoundEnvelopeController::GetController().SoundCreate(filter, entindex(), "Passtime.BallIdle");
...
void CPasstimeBall::StartLagCompensation(CBasePlayer *player, CUserCmd *cmd)
{
	...
	float flTargetTime = TICKS_TO_TIME(targettick);
	{
		...
		float frac = 0.0f;
		if(prevRecord && (record->flSimulationTime < flTargetTime) &&
		   (record->flSimulationTime < prevRecord->flSimulationTime))
		{
			...
			frac =
				(flTargetTime - record->flSimulationTime) / (prevRecord->flSimulationTime - record->flSimulationTime);
...
bool CPasstimeBall::BIgnorePlayer(CTFPlayer *pPlayer)
{
	...
	{
		const float flDist = CalcDistanceToAABB(pPlayer->WorldAlignMins(), pPlayer->WorldAlignMaxs(),
...
void CPasstimeBall::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	...
	if(g_pPasstimeLogic && (g_pPasstimeLogic->GetBall() == this) &&
	   g_pPasstimeLogic->OnBallCollision(this, index, pEvent) && IsGroundCollision(index, pEvent))
	{
		OnCollision();
	}
...
int CPasstimeBall::OnTakeDamage(const CTakeDamageInfo &info)
{
	...
	if(IPhysicsObject *pPhysObj = VPhysicsGetObject())
	{
		...
		pPhysObj->ApplyForceOffset(info.GetDamageForce().Normalized() * tf_passtime_ball_takedamage_force.GetFloat(),
								   GetAbsOrigin());
	}
