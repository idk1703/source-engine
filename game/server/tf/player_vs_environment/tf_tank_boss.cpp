...
static const char *s_TankModel[TANK_DAMAGE_MODEL_COUNT] = {
	"models/bots/boss_bot/boss_tank.mdl", "models/bots/boss_bot/boss_tank_damage1.mdl",
	"models/bots/boss_bot/boss_tank_damage2.mdl", "models/bots/boss_bot/boss_tank_damage3.mdl"};

static const char *s_TankModelRome[TANK_DAMAGE_MODEL_COUNT] = {
	"models/bots/tw2/boss_bot/boss_tank.mdl", "models/bots/tw2/boss_bot/boss_tank_damage1.mdl",
	"models/bots/tw2/boss_bot/boss_tank_damage2.mdl", "models/bots/tw2/boss_bot/boss_tank_damage3.mdl"};
...
void CTFTankDestruction::Spawn(void)
{
	...
	if(m_bIsAtCapturePoint)
	{
		...
		nDestroySequence = LookupSequence(
			UTIL_VarArgs("destroy_%s%i%s", gpGlobals->mapname.ToCStr(), nDeathAnimPick, m_szDeathPostfix));
	}
...
void CTFTankBoss::InputDestroyIfAtCapturePoint(inputdata_t &inputdata)
{
	...
	if(m_goalNode == NULL)
	{
		TakeDamage(CTakeDamageInfo(GetContainingEntity(INDEXENT(0)), GetContainingEntity(INDEXENT(0)), 9999999.9f,
								   DMG_CRUSH, TF_DMG_CUSTOM_NONE));
	}
}
...
void CTFTankBoss::UpdateOnRemove(void)
{
	...
	{
		TFObjectiveResource()->DecrementMannVsMachineWaveClassCount(MAKE_STRING("tank"),
																	MVM_CLASS_FLAG_NORMAL | MVM_CLASS_FLAG_MINIBOSS);
	}
...
void CTFTankBoss::Event_Killed(const CTakeDamageInfo &info)
{
	...
	if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
	{
		...
		{
			CLogicRelay *pLogicRelay =
				dynamic_cast<CLogicRelay *>(gEntList.FindEntityByName(NULL, "Barricade_Achievement_Check"));
...
void CTFTankBoss::TankBossThink(void)
{
	...
	if(m_goalNode != NULL)
	{
		...
		if(range < 20.0f)
		{
			...
			if(m_goalNode == NULL && m_bomb)
			{
				...
				TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_TANK_DEPLOYING,
	...
	if(m_exhaustAttachment > 0)
	{
		...
		trace_t result;
		UTIL_TraceLine(smokePos, smokePos + Vector(0, 0, 300.0f), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
					   &result);
	...
	if(m_crushTimer.IsElapsed())
	{
		...
		int count = UTIL_EntitiesInBox(
			intersectingEntities, maxCollectedEntities,
			GetAbsOrigin() + WorldAlignMins() * 0.75f, // a little fudge room for players on the top or sides
			GetAbsOrigin() + WorldAlignMaxs() * 0.75f, FL_CLIENT | FL_OBJECT);
...
void CTFTankBoss::Explode(void)
{
	...
	if(m_bIsPlayerKilled)
	{
		...
		if(TFGameRules()->IsMannVsMachineMode())
		{
			...
			if(m_isDroppingBomb)
			{
				// short delay so you only get the achievement if the bomb doors have opened/closed and it's ready to
				// deploy
			...
			FOR_EACH_VEC(m_vecDamagers, i)
			{
				...
				EconEntity_OnOwnerKillEaterEventNoPartner(pTFPlayer->GetActiveTFWeapon(), pTFPlayer,
														  kKillEaterEvent_TanksDestroyed);
			}
		}
	}
}
