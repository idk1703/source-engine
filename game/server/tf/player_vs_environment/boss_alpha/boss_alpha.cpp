...
()
	...
	BEGIN_DATADESC(CBossAlpha) DEFINE_OUTPUT(m_outputOnStunned, "OnStunned"),
...
ConVar tf_boss_alpha_skill_shot_damage_sponge_total("tf_boss_alpha_skill_shot_damage_sponge_total",
													"500" /*, FCVAR_CHEAT */);
ConVar tf_boss_alpha_skill_shot_damage_sponge_decay("tf_boss_alpha_skill_shot_damage_sponge_decay",
													"100" /*, FCVAR_CHEAT */);
...
bool CBossAlpha::CheckSkillShots(const CTakeDamageInfo &info)
{
	...
	*/
	...
	trace_t result;
	// UTIL_TraceLine( hitSpot - 50.0f * hitDir, hitSpot + 50.0f * hitDir, MASK_SOLID | CONTENTS_HITBOX, inflictor,
...
void CBossAlpha::UpdateSkillShots(void)
{
	...
	{
		UTIL_ClientPrintAll(HUD_PRINTTALK, CFmtStr("DAMAGE SPONGE = %3.2f/%3.2f", m_damageSpongeSkillShotAmount,
												   tf_boss_alpha_skill_shot_damage_sponge_total.GetFloat()));
	}
...
void CBossAlpha::UpdateAttackTarget(void)
{
	...
	if(!attackTargetThreat ||
	   maxThreat->m_threat > attackTargetThreat->m_threat + tf_boss_alpha_threat_tolerance.GetFloat())
	{
		// change threats
		SetAttackTarget(maxThreat->m_who);
	}
...
//---------------------------------------------------------------------------------------------
void CBossAlpha::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr,
