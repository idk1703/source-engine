...
ConVar tf_mvm_currency_bonus_ratio_min(
	"tf_mvm_currency_bonus_ratio_min", "0.95f", FCVAR_HIDDEN,
	"The minimum percentage of wave money players must collect in order to qualify for min bonus - 0.1 to 1.0.  Half "
	"the bonus amount will be awarded when reaching min ratio, and half when reaching max.",
	true, 0.1, true, 1.0);
ConVar tf_mvm_currency_bonus_ratio_max(
	"tf_mvm_currency_bonus_ratio_max", "1.f", FCVAR_HIDDEN,
	"The highest percentage of wave money players must collect in order to qualify for max bonus - 0.1 to 1.0.  Half "
	"the bonus amount will be awarded when reaching min ratio, and half when reaching max.",
	true, 0.1, true, 1.0);
...
void OnBotTeleported(CTFBot *bot)
{
	...
	if(!bot->IsPlayerClass(TF_CLASS_SPY))
	{
		...
		// invading bots get uber while they leave their spawn so they don't drop their cash where players can't pick it
		// up
...
SpawnLocationResult CSpawnLocation::FindSpawnLocation(Vector &vSpawnPosition)
{
	...
	if(activeSpawn.Count() > 0)
	{
		...
		SpawnLocationResult result =
			DoTeleporterOverride(activeSpawn[m_nSpawnCount], vSpawnPosition, m_bClosestPointOnNav);
...
CTFNavArea *CSpawnLocation::SelectSpawnArea(void) const
{
	...
	for(int t = 0; t < team->GetNumPlayers(); ++t)
	{
		...
		CollectSurroundingAreas(&nearbyAreaVector, teamMember->GetLastKnownArea(),
								tf_populator_active_buffer_range.GetFloat());
				...
				for(int k = 0; k < spawnVector.Count(); ++k)
				{
					...
					if(bot)
					{
						...
						if(TFObjectiveResource())
						{
							...
							TFObjectiveResource()->IncrementMannVsMachineWaveClassCount(m_spawner->GetClassIcon(k),
																						iFlags);
						}

						if(TFGameRules())
						{
							TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_SENTRY_BUSTER,
					...
					if(bot)
					{
						...
						if(TFObjectiveResource())
						{
							...
							TFObjectiveResource()->IncrementMannVsMachineWaveClassCount(
								bot->GetPlayerClass()->GetClassIconName(), iFlags);
						}
						...
						if(TFGameRules()->IsMannVsMachineMode())
						{
							...
							if(bot->HasMission(CTFBot::MISSION_SNIPER))
							{
								...
								if(nSniperCount == 1)
								{
									TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed(MP_CONCEPT_MVM_SNIPER_CALLOUT,
																					   TF_TEAM_PVE_DEFENDERS);
								}
							}
						}
...
void CMissionPopulator::Update(void)
{
	...
	if(TFGameRules()->InSetup() || GetManager()->GetWaveNumber() < m_beginAtWaveIndex ||
	   GetManager()->GetWaveNumber() >= m_stopAtWaveIndex || TFObjectiveResource()->GetMannVsMachineIsBetweenWaves())
	{
		m_state = NOT_STARTED;
		return;
	}
		...
		case SPAWNING:
			if(m_timer.IsElapsed())
			{
				...
				if(m_myReservedSlotCount <= 0)
				{
					...
					if(currentEnemyCount + m_spawnCount + m_reservedPlayerSlotCount >
					   CPopulationManager::MVM_INVADERS_TEAM_SIZE)
					{
						// no space right now
						return;
					}
				...
				if(m_spawner && m_spawner->IsWhereRequired())
				{
					...
					if(m_spawnLocationResult == SPAWN_LOCATION_NOT_FOUND ||
					   m_spawnLocationResult == SPAWN_LOCATION_TELEPORTER)
					{
				...
				if(m_spawner && m_spawner->Spawn(vSpawnPosition, &m_justSpawnedVector))
				{
					...
					FOR_EACH_VEC(m_justSpawnedVector, i)
					{
						...
						if(bot)
						{
							...
							TFObjectiveResource()->SetMannVsMachineWaveClassActive(
								bot->GetPlayerClass()->GetClassIconName());
					...
					int slotsToReleaseCount =
						(justSpawnedCount <= m_myReservedSlotCount) ? justSpawnedCount : m_myReservedSlotCount;
					m_myReservedSlotCount -= slotsToReleaseCount;
					m_reservedPlayerSlotCount -= slotsToReleaseCount;

					// somehow, duplicate entries can end up in m_activeVector if we just AddVectorToTail() - look into
					// this
		...
		if(!Q_stricmp(kvWave->GetName(), "WaveSpawn"))
		{
			...
			if(wavePopulator->m_spawner)
			{
				if(wavePopulator->m_spawner->IsVarious())
				{
					...
					{
						unsigned int iFlags =
							wavePopulator->IsSupportWave() ? MVM_CLASS_FLAG_SUPPORT : MVM_CLASS_FLAG_NORMAL;
...
void CWave::AddClassType(string_t iszClassIconName, int nCount, unsigned int iFlags)
{
	...
	{
		if((m_nWaveClassCounts[nClass].iszClassIconName == iszClassIconName) &&
		   (m_nWaveClassCounts[nClass].iFlags & iFlags))
		{
			nIndex = nClass;
			break;
		}
	...
	if(bLastWave && !GetManager()->IsInEndlessWaves())
	{
		...
		if(TFGameRules())
		{
			if(GTFGCClientSystem()->GetMatch() &&
			   GTFGCClientSystem()->GetMatch()->m_eMatchGroup == k_nMatchGroup_MvM_MannUp)
			{
...
void CWave::WaveIntermissionUpdate(void)
{
	...
	if(m_GetUpgradesAlertTimer.HasStarted() && m_GetUpgradesAlertTimer.IsElapsed())
	{
		...
		if((m_bCheckBonusCreditsMin || m_bCheckBonusCreditsMax) && gpGlobals->curtime > m_flBonusCreditsTime)
		{
			...
			{
				int nAmount =
					(float)TFGameRules()->CalculateCurrencyAmount_ByType(TF_CURRENCY_WAVE_COLLECTION_BONUS) * 0.5f;
