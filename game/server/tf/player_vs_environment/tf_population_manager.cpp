...
ConVar tf_populator_active_buffer_range(
	"tf_populator_active_buffer_range", "3000", FCVAR_CHEAT,
	"Populate the world this far ahead of lead raider, and this far behind last raider");

ConVar tf_mvm_default_sentry_buster_damage_dealt_threshold("tf_mvm_default_sentry_buster_damage_dealt_threshold",
														   "3000", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar tf_mvm_default_sentry_buster_kill_threshold("tf_mvm_default_sentry_buster_kill_threshold", "15",
...
ConVar tf_mvm_miniboss_scale("tf_mvm_miniboss_scale", "1.75", FCVAR_REPLICATED | TF_MVM_FCVAR_CHEAT,
...
ConVar tf_mvm_victory_reset_time("tf_mvm_victory_reset_time", "60.0", FCVAR_REPLICATED,
								 "Seconds to wait after MvM victory before cycling to the next mission.  (Only used if "
								 "tf_mvm_disconnect_on_victory is false.)");
ConVar tf_mvm_victory_disconnect_time(
...
ConVar tf_mvm_endless_force_on("tf_mvm_endless_force_on", "0",
							   FCVAR_DONTRECORD | FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							   "Force MvM Endless mode on");
ConVar tf_mvm_endless_wait_time("tf_mvm_endless_wait_time", "5.0f",
								FCVAR_DONTRECORD | FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY);
ConVar tf_mvm_endless_bomb_reset("tf_mvm_endless_bomb_reset", "5",
								 FCVAR_DONTRECORD | FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
								 "Number of Waves to Complete before bomb reset");
ConVar tf_mvm_endless_bot_cash("tf_mvm_endless_bot_cash", "120",
							   FCVAR_DONTRECORD | FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							   "In Endless, number of credits bots get per wave");
ConVar tf_mvm_endless_tank_boost("tf_mvm_endless_tank_boost", "0.2",
								 FCVAR_DONTRECORD | FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
								 "In Endless, amount of extra health for the tank per wave");

ConVar tf_populator_health_multiplier("tf_populator_health_multiplier", "1.0",
									  FCVAR_DONTRECORD | FCVAR_REPLICATED | FCVAR_CHEAT);
ConVar tf_populator_damage_multiplier("tf_populator_damage_multiplier", "1.0",
									  FCVAR_DONTRECORD | FCVAR_REPLICATED | FCVAR_CHEAT);
...
static bool HaveMap(const char *pszMapName)
{
	...
	switch(eResult)
	{
		...
		case IVEngineServer::eFindMap_FuzzyMatch:
			// Maps that are contingent on just-in-time preparation should probably not be baked into cycle files...
			// yet?
...
ConVar tf_mvm_skill("tf_mvm_skill", "3", FCVAR_DONTRECORD | FCVAR_REPLICATED | TF_MVM_FCVAR_CHEAT,
					"Sets the challenge level of the invading bot army. 1 = easiest, 3 = normal, 5 = hardest", true, 1,
					true, 5, MVMSkillChangedCallback);
...
void CPopulationManager::FindDefaultPopulationFileShortNames(CUtlVector<CUtlString> &outVecShortNames)
{
	...
	while(pPopFileName && pPopFileName[0] != '\0')
	{
		...
		if(pchPopPostfix)
		{
			...
			V_strncpy(szShortName, ((pchPopPostfix[0] == '_') ? (pchPopPostfix + 1) : "normal"),
					  sizeof(szShortName)); // skip the '_'
	...
	if(wave)
	{
		...
		if(bHasEngineer)
		{
			if(i < MVM_CLASS_TYPES_PER_WAVE_MAX_NEW)
			{
				...
				TFObjectiveResource()->AddMannVsMachineWaveClassFlags(
					i, MVM_CLASS_FLAG_MISSION); // only mission will flash
				i++;
			}
			else
			{
...
void CPopulationManager::StartCurrentWave(void)
{
	...
	if(m_bBonusRound)
	{
		...
		if(m_hBonusBoss)
		{
			...
			{
				AssertMsg(0, "CPopulationManager::StartCurrentWave trying to spawn a bonus boss, but cannot find "
							 "spawn_boss_startpoint info_target in the map");
...
void CPopulationManager::RestoreItemToCheckpointState(CTFPlayer *player, CEconItemView *item)
{
	...
	for(int u = 0; u < snapshot->m_upgradeVector.Count(); ++u)
	{
		if(item->GetItemDefIndex() == snapshot->m_upgradeVector[u].m_itemDefIndex)
		{
			...
			{
				if(g_hUpgradeEntity->ApplyUpgradeToItem(player, item, snapshot->m_upgradeVector[u].m_upgrade,
														snapshot->m_upgradeVector[u].m_nCost))
				{
					if(tf_populator_debug.GetBool())
					{
						const char *upgradeName =
							g_hUpgradeEntity->GetUpgradeAttributeName(snapshot->m_upgradeVector[u].m_upgrade);
						DevMsg("%3.2f: CHECKPOINT_RESTORE_ITEM: Player '%s', item '%s', upgrade '%s'\n",
							   gpGlobals->curtime, player->GetPlayerName(), item->GetStaticData()->GetItemBaseName(),
...
void CPopulationManager::ForgetOtherBottleUpgrades(CTFPlayer *player, CEconItemView *pItem, int upgradeToKeep)
{
	...
	for(int i = 0; i < history->m_upgradeVector.Count(); ++i)
	{
		...
		if(history->m_upgradeVector[i].m_itemDefIndex == pItem->GetItemDefIndex() &&
		   history->m_upgradeVector[i].m_upgrade != upgradeToKeep) // item upgrade
		{
...
void CPopulationManager::GetSentryBusterDamageAndKillThreshold(int &nDamage, int &nKills) const
{
	...
	nDamage = (nSentries >= nSentryThreshold) ? m_sentryBusterDamageDealtThreshold * flScale
											  : m_sentryBusterDamageDealtThreshold;
...
void CPopulationManager::EndlessRollEscalation(void)
{
	...
	{
		char line[255];
		V_sprintf_safe(line, "-%s %.1f\n", m_EndlessActiveBotUpgrades[iUpgrade].szAttrib,
					   m_EndlessActiveBotUpgrades[iUpgrade].flValue);
...
void CPopulationManager::EndlessSetAttributesForBot(CTFBot *pBot)
{
	...
	{
		// DevMsg( "   - %s %d ", m_EndlessActiveBotUpgrades[iUpgrade].szAttrib,
		// m_EndlessActiveBotUpgrades[iUpgrade].flValue ); pBot->m_AttributeManager
		...
		else
		{
			CEconItemAttributeDefinition *pDef =
				ItemSystem()->GetItemSchema()->GetAttributeDefinition(m_EndlessActiveBotUpgrades[i].iAttribIndex);
			if(pDef)
			{
...
CPopulationManager::PlayerUpgradeHistory *CPopulationManager::FindOrAddPlayerUpgradeHistory(CTFPlayer *player)
{
	...
	if(!player->GetSteamID(&steamId))
	{
		Log("MvM : Unable to Find SteamID for player %s, unable to locate their upgrade history!",
...
void CPopulationManager::RemovePlayerAndItemUpgradesFromHistory(CTFPlayer *pPlayer)
{
	...
	FOR_EACH_VEC_BACK(m_checkpointSnapshot, i)
	{
		...
		FOR_EACH_VEC_BACK(pSnapshot->m_upgradeVector, j)
		{
			...
			if(pUpgrade && (pUpgrade->nUIGroup == UIGROUP_UPGRADE_ATTACHED_TO_ITEM ||
							pUpgrade->nUIGroup == UIGROUP_UPGRADE_ATTACHED_TO_PLAYER))
			{
	...
	{
		// This should put us at the right currency, given that we've removed item and player upgrade tracking by this
		// point
...
void CPopulationManager::DebugWaveStats()
{
	...
	if(m_waveVector.Count())
	{
		...
		DevMsg("Waves: %d ( %3.2f credits per wave )\n", nTotalWaves,
			   float((float)nTotalPopCurrency / (float)nTotalWaves));
		DevMsg("---\n");
	}
