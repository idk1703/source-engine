...
{
	// If we're being asked to remove and refund everything, it's a respec (the population manager actually handles
	// refunding later)
		...
		for(int iUpgrade = 0; iUpgrade < g_MannVsMachineUpgrades.m_Upgrades.Count(); iUpgrade++)
		{
			...
			if(pAttribDef)
			{
				...
				for(int iItemSlot = LOADOUT_POSITION_PRIMARY; iItemSlot < nLastLoadoutPos; iItemSlot++)
				{
					...
					if(bRespec ||
					   (TFGameRules() && TFGameRules()->CanUpgradeWithAttrib(
											 pTFPlayer, iItemSlot, pAttribDef->GetDefinitionIndex(), &upgrade)))
					{
						...
						const int iNumMaxUpgradeStep =
							GetUpgradeStepData(pTFPlayer, iItemSlot, iUpgrade, iCurrentUpgradeStep, bOverCap);
...
// Purpose:
//-----------------------------------------------------------------------------
void CUpgrades::PlayerPurchasingUpgrade(CTFPlayer *pTFPlayer, int iItemSlot, int iUpgrade, bool bDowngrade,
										bool bFree /*= false */, bool bRespec /*= false*/)
{
	if(!pTFPlayer || iUpgrade < 0 || iUpgrade >= g_MannVsMachineUpgrades.m_Upgrades.Count())
		return;

	// Verify that this upgrade can be accepted on this player
	CMannVsMachineUpgrades upgrade = g_MannVsMachineUpgrades.m_Upgrades[iUpgrade];
	CEconItemAttributeDefinition *pAttribDef = ItemSystem()->GetStaticDataForAttributeByName(upgrade.szAttrib);
	if(!bRespec && (!TFGameRules() || !TFGameRules()->CanUpgradeWithAttrib(pTFPlayer, iItemSlot,
																		   pAttribDef->GetDefinitionIndex(), &upgrade)))
	{
		return;
	}
	...
	if(!nCost)
	{
		nCost = TFGameRules()->GetCostForUpgrade(&g_MannVsMachineUpgrades.m_Upgrades[iUpgrade], iItemSlot,
												 pTFPlayer->GetPlayerClass()->GetClassIndex(), pTFPlayer);
	}
	if(bDowngrade)
	{
		nCost *= -1;
	...
	{
		if(!(iItemSlot == LOADOUT_POSITION_ACTION ||
			 (iItemSlot >= LOADOUT_POSITION_PRIMARY && iItemSlot <= LOADOUT_POSITION_PDA2)))
			return;
	...
	if(bDowngrade)
	{
		...
		else
		{
			...
			for(int i = 0; i < pTFPlayer->GetRefundableUpgrades()->Count(); ++i)
			{
				...
				if((pInfo.m_iPlayerClass == pTFPlayer->GetPlayerClass()->GetClassIndex()) &&
				   (pInfo.m_itemDefIndex == iItemIndex) && (pInfo.m_upgrade == iUpgrade))
				{
	...
	const attrib_definition_index_t nUpgradedAttrDefIndex =
		ApplyUpgradeToItem(pTFPlayer, pItem, iUpgrade, nCost, bDowngrade, !bFree);
...
// Purpose:
//-----------------------------------------------------------------------------
static attrib_definition_index_t ApplyUpgrade_Bottle(int iUpgrade, CTFPlayer *pTFPlayer, CEconItemView *pEconItemView,
													 int nCost, bool bDowngrade)
{
	Assert(pTFPlayer);
...
// Purpose:
//-----------------------------------------------------------------------------
static attrib_definition_index_t ApplyUpgrade_Default(const CMannVsMachineUpgrades &upgrade, CTFPlayer *pTFPlayer,
													  CEconItemView *pEconItemView, int nCost, bool bDowngrade)
{
	Assert(pTFPlayer);
	Assert(upgrade.nUIGroup == UIGROUP_UPGRADE_ATTACHED_TO_PLAYER ||
		   upgrade.nUIGroup == UIGROUP_UPGRADE_ATTACHED_TO_ITEM);
	// Assert( pEconItemView || upgrade.nUIGroup != UIGROUP_UPGRADE_ATTACHED_TO_ITEM );		// ugh, if loadouts change
	// behind the scenes or if we have bugs elsewhere, we might feed an empty slot in here -- check for this case below
	// if ATTACHED_TO_ITEM
	...
	// ...
	float fDefaultValue = pAttrDef->GetDescriptionFormat() == ATTDESCFORM_VALUE_IS_PERCENTAGE ||
								  pAttrDef->GetDescriptionFormat() == ATTDESCFORM_VALUE_IS_INVERTED_PERCENTAGE
							  ? 1.0f
							  : 0.0f;
	...
	// if the attribute exists, add the increment (but not if it's a set bonus attribute, they're recreated on each
	// respawn)
...
// Purpose:
//-----------------------------------------------------------------------------
attrib_definition_index_t CUpgrades::ApplyUpgradeToItem(CTFPlayer *pTFPlayer, CEconItemView *pView, int iUpgrade,
														int nCost, bool bDowngrade, bool bIsFresh)
{
	Assert(pTFPlayer);
	...
	ReportUpgrade(pTFPlayer, pView ? pView->GetItemDefIndex() : 0, pAttrDef->GetDefinitionIndex(), upgrade.nQuality,
				  nCost, bDowngrade, bIsFresh, bIsBottle);
...
// Purpose:
//-----------------------------------------------------------------------------
void CUpgrades::NotifyItemOnUpgrade(CTFPlayer *pTFPlayer, attrib_definition_index_t nAttrDefIndex,
	...
	switch(nAttrDefIndex)
	{
		...
		case 320: // "robot sapper"
		{
			...
			CTFWeaponBuilder *pBuilder =
				dynamic_cast<CTFWeaponBuilder *>(pTFPlayer->Weapon_OwnsThisID(TF_WEAPON_BUILDER));
			if(pBuilder)
			{
		...
		case 351:
			if(bDowngrade)
			{
				// if we're refunding the engy_disposable_sentries attribute we need to destroy the disposable sentry if
				// we have one
...
//-----------------------------------------------------------------------------
void CUpgrades::ReportUpgrade(CTFPlayer *pTFPlayer, int nItemDef, int nAttributeDef, int nQuality, int nCost,
							  bool bDowngrade, bool bIsFresh, bool bIsBottle /*= false*/)
{
	if(!pTFPlayer)
	{
		return;
...
void CUpgrades::RestoreItemAttributeToBaseValue(CEconItemAttributeDefinition *pAttrib, CEconItemView *pItem)
{
	...
	{
		float fDefaultValue = pAttrib->GetDescriptionFormat() == ATTDESCFORM_VALUE_IS_PERCENTAGE ||
									  pAttrib->GetDescriptionFormat() == ATTDESCFORM_VALUE_IS_INVERTED_PERCENTAGE
								  ? 1.f
								  : 0.f;
...
// Purpose:
//-----------------------------------------------------------------------------
void CUpgrades::ApplyUpgradeAttributeBlock(UpgradeAttribBlock_t *upgradeBlock, int upgradeCount, CTFPlayer *pPlayer,
										   bool bDowngrade)
{
	if(!pPlayer)
		return;
	...
	for(int i = 0; i < upgradeCount; i++)
	{
		...
		CEconItemAttributeDefinition *pDef =
			ItemSystem()->GetItemSchema()->GetAttributeDefinitionByName(upgradeBlock[i].szName);
		if(!pDef)
			continue;
