...
public:
	...
	CUtlVector<CUpgradeInfo> *GetPlayerUpgradeHistory(
		CTFPlayer *player); // Returns the Upgrade vector for a given player
	...
	// inlined
	bool IsRestoringCheckpoint(
		void) const; // return true if we are in the process of restoring players to their checkpointed state
...
private:
	...
	CUtlVector<PlayerUpgradeHistory *>
		m_playerUpgrades; // list of all players and their upgrades who have played on this MVM rotation
	...
	static CUtlVector<CheckpointSnapshotInfo *>
		m_checkpointSnapshot; // snapshot of player state at the saved checkpoint
