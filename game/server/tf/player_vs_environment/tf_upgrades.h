...
public:
	...
	void PlayerPurchasingUpgrade(CTFPlayer *pTFPlayer, int iItemSlot, int iUpgrade, bool bDowngrade, bool bFree = false,
								 bool bRespec = false);

	attrib_definition_index_t ApplyUpgradeToItem(CTFPlayer *pTFPlayer, CEconItemView *pView, int iUpgrade, int nCost,
												 bool bDowngrade = false,
												 bool bIsFresh = false); // needed for checkpoint restore
...
private:
	...
	void ReportUpgrade(CTFPlayer *pTFPlayer, int nItemDef, int nAttributeDef, int nQuality, int nCost, bool bDowngrade,
					   bool bIsFresh, bool bIsBottle = false);
	...
	void ApplyUpgradeAttributeBlock(UpgradeAttribBlock_t *upgradeBlock, int upgradeCount, CTFPlayer *pPlayer,
									bool bDowngrade);
