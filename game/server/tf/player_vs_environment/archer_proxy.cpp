...
CTFPlayer *CTFArcherProxy::SelectTarget(void)
{
	...
	for(int i = 0; i < playerVector.Count(); ++i)
	{
		...
		{
			UTIL_TraceLine(GetAbsOrigin(), playerVector[i]->EyePosition(), MASK_SOLID, this, COLLISION_GROUP_DEBRIS,
						   &result);
...
void CTFArcherProxy::ShootArrowAt(CBaseEntity *target)
{
	...
	CTFProjectile_Arrow *arrow = CTFProjectile_Arrow::Create(GetAbsOrigin() + 20.0f * to, angles, arrowSpeed,
															 arrowGravity, TF_PROJECTILE_ARROW, this, this);
	if(arrow)
	{
...
void CTFArcherProxy::ShootGrenadeAt(CBaseEntity *target)
{
	...
	CTFGrenadePipebombProjectile *grenade = static_cast<CTFGrenadePipebombProjectile *>(
		CBaseEntity::CreateNoSpawn("tf_projectile_pipe", GetAbsOrigin(), angles, NULL));
	if(grenade)
	{
