...
bool IsSpaceToSpawnHere(const Vector &where)
{
	...
	trace_t result;
	float bloat = 5.0f;
	UTIL_TraceHull(where, where, VEC_HULL_MIN - Vector(bloat, bloat, 0), VEC_HULL_MAX + Vector(bloat, bloat, bloat),
				   MASK_SOLID | CONTENTS_PLAYERCLIP, NULL, COLLISION_GROUP_PLAYER_MOVEMENT, &result);
	...
	FOR_EACH_VEC(event.m_itemsAttributes, i)
	{
		...
		if(!Q_stricmp(botItemAttrs.m_itemName, pszItemName))
		{
			...
			FOR_EACH_VEC(vecStaticAttribs, iNewAttribute)
			{
				...
				int iExistingAttribute;
				for(iExistingAttribute = 0; iExistingAttribute < botItemAttrs.m_attributes.Count();
					iExistingAttribute++)
				{
					// matching existing attribute? stomp value
...
bool CTFBotSpawner::Spawn(const Vector &rawHere, EntityHandleVector_t *result)
{
	...
	if(TFGameRules() && TFGameRules()->IsMannVsMachineMode())
	{
		if(m_class == TF_CLASS_ENGINEER && m_defaultAttributes.m_attributeFlags & CTFBot::TELEPORT_TO_HINT &&
		   CTFBotMvMEngineerHintFinder::FindHint(true, false) == false)
		{
			if(tf_populator_debug.GetBool())
			{
	...
	if(newBot)
	{
		...
		const CTFBot::EventChangeAttributes_t *pEventChangeAttributes =
			newBot->GetEventChangeAttributes(g_pPopulationManager->GetDefaultEventChangeAttributesName());
		...
		else
		{
			...
			{
				if((m_scale >= tf_mvm_miniboss_scale.GetFloat() || newBot->IsMiniBoss()) &&
				   g_pFullFileSystem->FileExists(g_szBotBossModels[nClassIndex]))
				{
		...
		if(tf_populator_debug.GetBool())
		{
			if(tf_populator_debug.GetBool())
			{
				DevMsg("%3.2f: Spawned TFBot '%s'\n", gpGlobals->curtime,
					   m_name.IsEmpty() ? newBot->GetPlayerClass()->GetName() : m_name.Get());
			}
		}
	}
	else
