...
private:
	...
	Vector CalculateLaunchVectorPreserve(Vector vecInitialVelocity, CBaseEntity *pVictim, CBaseEntity *pTarget,
										 bool bForcePlayer = false);
	...
	// CNetworkVar( float, m_flAirControlSupressionTime ); // After catapult, stop air control for this long (or default
	// of quarter second if this value is negative) CNetworkVar( bool, m_bDirectionSuppressAirControl );	// Do we
	// want to use air control suppression for directional catapults.
