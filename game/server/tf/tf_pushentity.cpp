...
private:
	bool RotationPushTFPlayer(PhysicsPushedInfo_t &info, const Vector &vecAbsPush,
							  const RotatingPushMove_t &rotPushMove, bool bRotationalPush);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPhysicsPushEntities::RotationPushTFPlayer(PhysicsPushedInfo_t &info, const Vector &vecAbsPush,
												  const RotatingPushMove_t &rotPushMove, bool bRotationalPush)
{
	...
	if(!IsOBBIntersectingOBB(pCollisionPlayer->GetCollisionOrigin(), pCollisionPlayer->GetCollisionAngles(),
							 pCollisionPlayer->OBBMins(), pCollisionPlayer->OBBMaxs(),
							 pCollisionPusher->GetCollisionOrigin(), pCollisionPusher->GetCollisionAngles(),
							 pCollisionPusher->OBBMins(), pCollisionPusher->OBBMaxs(), 0.0f))
		return false;
...
bool CTFPhysicsPushEntities::RotationCheckPush(PhysicsPushedInfo_t &info)
{
	...
	// Check to see if the player is in a good spot and attempt a move again if not - but only if it isn't being ridden
	// on.
	if(IsPlayerAABBIntersetingPusherOBB(pBlocker, pRootEntity))
	{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPhysicsPushEntities::LinearPushTFPlayer(PhysicsPushedInfo_t &info, const Vector &vecAbsPush,
...
bool CTFPhysicsPushEntities::IsPlayerAABBIntersetingPusherOBB(CBaseEntity *pEntity, CBaseEntity *pRootEntity)
{
	...
	// Do we have a collision.
	return IsOBBIntersectingOBB(pCollisionPlayer->GetCollisionOrigin(), pCollisionPlayer->GetCollisionAngles(),
								pCollisionPlayer->OBBMins(), pCollisionPlayer->OBBMaxs(),
								pCollisionPusher->GetCollisionOrigin(), pCollisionPusher->GetCollisionAngles(),
								pCollisionPusher->OBBMins(), pCollisionPusher->OBBMaxs(), 0.0f);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPhysicsPushEntities::MovePlayer(CBaseEntity *pBlocker, PhysicsPushedInfo_t &info, float flMoveScale,
										bool bPusherIsTrain)
{
	...
	for(int iTest = 0; iTest < 4; ++iTest)
	{
		...
		UTIL_TraceEntity(pBlocker, vecStart, vecEnd, MASK_PLAYERSOLID, NULL, COLLISION_GROUP_PLAYER_MOVEMENT,
						 &info.m_Trace);
