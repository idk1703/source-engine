...
void CTFTeamSpawn::Activate(void)
{
	...
	trace_t trace;
	UTIL_TraceHull(GetAbsOrigin(), GetAbsOrigin(), mins, maxs, MASK_PLAYERSOLID, NULL, COLLISION_GROUP_PLAYER_MOVEMENT,
				   &trace);
...
int CTFTeamSpawn::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		if(m_hControlPoint)
		{
			NDebugOverlay::Line(GetAbsOrigin(), m_hControlPoint->GetAbsOrigin(), teamcolor.r, teamcolor.g, teamcolor.b,
								false, 0.1);
		}
	}
	return text_offset;
...
void CTFTeamSpawn::InputRoundSpawn(inputdata_t &input)
{
	...
	if(m_iszRoundBlueSpawn != NULL_STRING)
	{
		...
		m_hRoundBlueSpawn =
			dynamic_cast<CTeamControlPointRound *>(gEntList.FindEntityByName(NULL, m_iszRoundBlueSpawn));
