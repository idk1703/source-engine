...
public:
	...
	// Creation.
	static CTFProjectile_EnergyBall *Create(const Vector &vecOrigin, const QAngle &vecAngles, const float fSpeed,
											const float fGravity, CBaseEntity *pOwner = NULL,
											CBaseEntity *pScorer = NULL);
	virtual void InitEnergyBall(const Vector &vecOrigin, const QAngle &vecAngles, const float fSpeed,
								const float fGravity, CBaseEntity *pOwner = NULL, CBaseEntity *pScorer = NULL);
	virtual void Spawn();
