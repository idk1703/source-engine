...
ConVar tf_mvm_notice_sapped_squadmates_delay("tf_mvm_notice_sapped_squadmates_delay", "1",
											 FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
											 "How long it takes for a squad leader to notice his squadmate was sapped");
...
ConVar obj_sapper_amount("obj_sapper_amount", "25", FCVAR_NONE,
						 "Amount of health inflicted by a Sapper object per second");
...
void CObjectSapper::Spawn()
{
	...
	if(TFGameRules() && TFGameRules()->GameModeUsesMiniBosses() && GetBuilder() && !GetBuilder()->IsBot())
	{
...
const char *CObjectSapper::GetSapperModelName(SapperModel_t nModel, const char *pchModelName /*= NULL */)
{
	...
	if(m_szPlacementModel[0] == '\0' || m_szSapperModel[0] == '\0')
	{
		if(!pchModelName)
		{
			if(GetBuilder())
			{
				CTFWeaponBuilder *pWeapon =
					dynamic_cast<CTFWeaponBuilder *>(GetBuilder()->Weapon_GetWeaponByType(TF_WPN_TYPE_BUILDING));
				if(pWeapon)
				{
		...
		{
			V_snprintf(m_szPlacementModel, sizeof(m_szPlacementModel), "models/workshop_partner/buildables/%s%s",
					   pchModelName, "_placement.mdl");
			V_snprintf(m_szSapperModel, sizeof(m_szSapperModel), "models/workshop_partner/buildables/%s%s",
					   pchModelName, "_placed.mdl");
		}
		else
#endif
		{
			V_snprintf(m_szPlacementModel, sizeof(m_szPlacementModel), "models/buildables/%s%s", pchModelName,
					   "_placement.mdl");
	...
	if(!(info.GetDamageType() & DMG_FROM_OTHER_SAPPER))
	{
		if(GetParentObject())
		{
			...
			if(pParentTeleporter)
			{
				...
				CObjectTeleporter *pMatchingTeleporter = pParentTeleporter->GetMatchingTeleporter()
															 ? pParentTeleporter->GetMatchingTeleporter()
															 : pParentTeleporter->FindMatch();
...
void CObjectSapper::ApplyRoboSapper(CTFPlayer *pTarget, float flDuration, int nRadius /*= 200*/)
{
	...
	if(nRadius)
	{
		...
		for(int i = 1; i < gpGlobals->maxClients; i++)
		{
			...
			trace_t trace;
			UTIL_TraceLine(pPlayer->WorldSpaceCenter(), WorldSpaceCenter(), MASK_SOLID_BRUSHONLY, NULL,
