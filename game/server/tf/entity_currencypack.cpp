...
void CCurrencyPack::Spawn(void)
{
	...
	SetContextThink(&CCurrencyPack::BlinkThink,
					gpGlobals->curtime + GetLifeTime() - TF_CURRENCYPACK_BLINK_PERIOD -
						RandomFloat(0.0, TF_CURRENCYPACK_BLINK_DURATION),
...
void CCurrencyPack::BlinkThink(void)
{
	...
	SetContextThink(&CCurrencyPack::BlinkThink, gpGlobals->curtime + TF_CURRENCYPACK_BLINK_DURATION,
...
bool CCurrencyPack::MyTouch(CBasePlayer *pPlayer)
{
	if(ValidTouch(pPlayer) && !m_bTouched)
	{
		...
		if((!pTFTouchPlayer->IsPlayerClass(TF_CLASS_SPY)) ||
		   (!pTFTouchPlayer->m_Shared.IsStealthed() && !pTFTouchPlayer->m_Shared.InCond(TF_COND_STEALTHED_BLINK) &&
			!pTFTouchPlayer->m_Shared.InCond(TF_COND_DISGUISED)))
		{
