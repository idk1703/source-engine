...
static void SpeakConceptBySteamID(const CSteamID &steamID, int iConcept, const CSteamID &steamIDInitiator,
								  const CSteamID &steamIDTarget)
{
	...
	if(pPlayer != NULL)
	{
		...
		if(pPlayerInitiator && pPlayerTarget)
		{
			Q_snprintf(pModifiers, sizeof(pModifiers), "duelinitiatorclass:%s,dueltargetclass:%s",
					   g_aPlayerClassNames_NonLocalized[pPlayerInitiator->m_Shared.InCond(TF_COND_DISGUISED)
															? pPlayerInitiator->m_Shared.GetDisguiseClass()
															: pPlayerInitiator->GetPlayerClass()->GetClassIndex()],
					   g_aPlayerClassNames_NonLocalized[pPlayerTarget->m_Shared.InCond(TF_COND_DISGUISED)
															? pPlayerTarget->m_Shared.GetDisguiseClass()
															: pPlayerTarget->GetPlayerClass()->GetClassIndex()]);
		}
...
static void UpdateDuelScore(CTFPlayer *pKiller, CTFPlayer *pVictim, eDuelScoreType scoreType)
{
	...
	FOR_EACH_VEC(g_duels, i)
	{
		   ...
		   (duel.m_steamIDInitiator == steamIDVictim && duel.m_steamIDTarget == steamIDKiller))
		{
			...
			{
				bCountScore = (pKiller->GetPlayerClass() != NULL &&
							   duel.m_iPlayerClass == pKiller->GetPlayerClass()->GetClassIndex() &&
							   pVictim->GetPlayerClass() != NULL &&
							   duel.m_iPlayerClass == pVictim->GetPlayerClass()->GetClassIndex());
			}
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		SpeakConceptBySteamID(msg.Body().m_ulInitiatorSteamID, MP_CONCEPT_DUEL_REQUEST, msg.Body().m_ulInitiatorSteamID,
							  msg.Body().m_ulTargetSteamID);
		return true;
	}
};
GC_REG_JOB(GCSDK::CGCClient, CGC_GameServer_Duel_Request, "CGC_GameServer_Duel_Request", k_EMsgGC_Duel_Request,
...
public:
	...
	virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket)
	{
		...
		{
			SpeakConceptBySteamID(msg.Body().m_ulInitiatorSteamID, MP_CONCEPT_DUEL_REJECTED,
								  msg.Body().m_ulInitiatorSteamID, msg.Body().m_ulTargetSteamID);
			SpeakConceptBySteamID(msg.Body().m_ulTargetSteamID, MP_CONCEPT_DUEL_TARGET_REJECT,
								  msg.Body().m_ulInitiatorSteamID, msg.Body().m_ulTargetSteamID);
			return true;
		}

		// duel was accepted
		SpeakConceptBySteamID(msg.Body().m_ulInitiatorSteamID, MP_CONCEPT_DUEL_ACCEPTED,
							  msg.Body().m_ulInitiatorSteamID, msg.Body().m_ulTargetSteamID);
		SpeakConceptBySteamID(msg.Body().m_ulTargetSteamID, MP_CONCEPT_DUEL_TARGET_ACCEPT,
							  msg.Body().m_ulInitiatorSteamID, msg.Body().m_ulTargetSteamID);
		...
		if(duel.m_iPlayerClass >= TF_FIRST_NORMAL_CLASS && duel.m_iPlayerClass < TF_LAST_NORMAL_CLASS)
		{
			...
			if(pPlayer_Initiator && (pPlayer_Initiator->GetPlayerClass() == NULL ||
									 pPlayer_Initiator->GetPlayerClass()->GetClassIndex() != duel.m_iPlayerClass))
			{
				pPlayer_Initiator->SetDesiredPlayerClassIndex(duel.m_iPlayerClass);
				pPlayer_Initiator->ForceRespawn();
			}
			if(pPlayer_Target && (pPlayer_Target->GetPlayerClass() == NULL ||
								  pPlayer_Target->GetPlayerClass()->GetClassIndex() != duel.m_iPlayerClass))
			{
...
GC_REG_JOB(GCSDK::CGCClient, CGC_GameServer_Duel_Response, "CGC_GameServer_Duel_Response", k_EMsgGC_Duel_Response,
		   GCSDK::k_EServerTypeGCClient);
...
void DuelMiniGame_NotifyPlayerChangedTeam(CTFPlayer *pPlayer, int iNewTeam, bool bInitiatedByPlayer)
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(steamIDOpponent == pDuel->m_steamIDInitiator || steamIDOpponent == pDuel->m_steamIDTarget)
		{
			...
			else if(iNewTeam == TEAM_SPECTATOR || iNewTeam == pOpponent->GetTeamNumber())
			{
				SendDuelResults(*pDuel, steamIDOpponent,
								bInitiatedByPlayer ? kDuelEndReason_PlayerSwappedTeams
...
void DuelMiniGame_NotifyPlayerDisconnect(CTFPlayer *pPlayer, bool bKicked)
{
	...
	for(int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		{
			SendDuelResults(*pDuel, steamIDOpponent,
							bKicked ? kDuelEndReason_PlayerKicked : kDuelEndReason_PlayerDisconnected);
			RemoveDuel(pDuel);
			return;
		}
	}
...
void DuelMiniGame_AssignWinners()
{
	...
	FOR_EACH_VEC(g_duels, i)
	{
		...
		{
			SendDuelResults(
				duel, duel.m_usScoreInitiator > duel.m_usScoreTarget ? duel.m_steamIDInitiator : duel.m_steamIDTarget,
...
void DuelMiniGame_LevelShutdown()
{
	...
	{
		duel_minigame_data_t &duel = g_duels[i];
		SendDuelResults(duel,
						duel.m_usScoreInitiator > duel.m_usScoreTarget ? duel.m_steamIDInitiator : duel.m_steamIDTarget,
