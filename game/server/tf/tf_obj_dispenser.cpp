...
//-----------------------------------------------------------------------------
void SendProxy_HealingList(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
						   int objectID)
{
...
SendPropInt(SENDINFO(m_iState), 2), SendPropInt(SENDINFO(m_iAmmoMetal), -1, SPROP_VARINT),
	SendPropInt(SENDINFO(m_iMiniBombCounter), -1, SPROP_VARINT),

	SendPropArray2(SendProxyArrayLength_HealingArray,
				   SendPropInt("healing_array_element", 0, SIZEOF_IGNORE, NUM_NETWORKED_EHANDLE_BITS, SPROP_UNSIGNED,
							   SendProxy_HealingList),
				   MAX_PLAYERS, 0, "healing_array")

END_SEND_TABLE
()

	BEGIN_DATADESC(CObjectDispenser) DEFINE_THINKFUNC(RefillThink),
...
float g_flDispenserHealRates[4] = {0, 10.0, 15.0, 20.0};

float g_flDispenserAmmoRates[4] = {0, 0.2, 0.3, 0.4};
...
void CObjectDispenser::DetonateObject(void)
{
	...
	if(IsMiniBuilding() && !IsCarried() && !IsBuilding() && !IsPlacing())
	{
		...
		TE_TFExplosion(filter, 0.0f, vecOrigin, Vector(0, 0, 0), TF_WEAPON_GRENADE_PIPEBOMB, kInvalidEHandleExplosion,
					   -1, SPECIAL1, INVALID_STRING_INDEX);
	}
#endif
...
void CObjectDispenser::FirstSpawn()
{
	...
	UTIL_SetSize(this, bShouldBeMini ? MINI_DISPENSER_MINS : DISPENSER_MINS,
...
#ifdef STAGING_ONLY
	UTIL_SetSize(this, IsMiniBuilding() ? MINI_DISPENSER_MINS : DISPENSER_MINS,
...
void CObjectDispenser::OnGoActive(void)
{
	...
	if(m_hTouchTrigger.Get() == NULL)
	{
		...
		UTIL_SetSize(m_hTouchTrigger.Get(), Vector(-flRadius, -flRadius, -flRadius),
...
bool CObjectDispenser::DispenseAmmo(CTFPlayer *pPlayer)
{
	...
	CALL_ATTRIB_HOOK_INT_ON_OTHER(pPlayer->GetActiveWeapon(), nNoPrimaryAmmoFromDispensersWhileActive,
	...
	if(nNoPrimaryAmmoFromDispensersWhileActive == 0)
	{
		...
		iTotalPickedUp +=
			pPlayer->GiveAmmo(iAmmoToAdd, TF_AMMO_PRIMARY, !m_bPlayAmmoPickupSound, kAmmoSource_DispenserOrCart);
	}

	// secondary
	iAmmoToAdd = (int)(pPlayer->GetMaxAmmo(TF_AMMO_SECONDARY) * flAmmoRate);
	iTotalPickedUp +=
		pPlayer->GiveAmmo(iAmmoToAdd, TF_AMMO_SECONDARY, !m_bPlayAmmoPickupSound, kAmmoSource_DispenserOrCart);

	// metal
	int iNoMetalFromDispenserWhileActive = 0;
	CALL_ATTRIB_HOOK_INT_ON_OTHER(pPlayer->GetActiveWeapon(), iNoMetalFromDispenserWhileActive,
...
int CObjectDispenser::DispenseMetal(CTFPlayer *pPlayer)
{
	...
	int iMetal =
		pPlayer->GiveAmmo(Min(m_iAmmoMetal.Get(), iMetalToGive), TF_AMMO_METAL, false, kAmmoSource_DispenserOrCart);
	m_iAmmoMetal -= iMetal;
...
void CObjectDispenser::DispenseThink(void)
{
	...
	if(m_flNextAmmoDispense <= gpGlobals->curtime)
	{
		...
		int iNumberOfNearbyEntities = UTIL_EntitiesInSphere(pListOfNearbyEntities, ARRAYSIZE(pListOfNearbyEntities),
															vecOrigin, flRadius, FL_CLIENT);
			...
			if(GetBuilder() && pOther != GetBuilder() && flHealingDone > 0)
			{
				...
				if(GetBuilder()->GetTeam() == pOther->GetTeam())
				{
					...
					EconEntity_OnOwnerKillEaterEvent(
						...
						GetBuilder(), pPlayer, kKillEaterEvent_HealingProvided, (int)flHealingDone);
				}
			}
		}
	}
...
bool CObjectDispenser::CouldHealTarget(CBaseEntity *pTarget)
{
	...
	if(pTarget->IsPlayer() && pTarget->IsAlive())
	{
		...
		if(HasSpawnFlags(SF_DISPENSER_DONT_HEAL_DISGUISED_SPIES))
		{
			...
			if(pTFPlayer->m_Shared.InCond(TF_COND_DISGUISED) && pTFPlayer->m_Shared.GetDisguiseTeam() != iTeam)
			{
				return false;
			}
		}
...
{
	int iMetal =
		pPlayer->GiveAmmo(MIN(m_iAmmoMetal, DISPENSER_DROP_METAL), TF_AMMO_METAL, false, kAmmoSource_DispenserOrCart);
	return iMetal;
}
