...
BEGIN_NETWORK_TABLE(CTFProjectile_Arrow, DT_TFProjectile_Arrow)
	SendPropBool(SENDINFO(m_bArrowAlight)), SendPropBool(SENDINFO(m_bCritical)),
...
// Purpose:
//-----------------------------------------------------------------------------
CTFProjectile_Arrow *CTFProjectile_Arrow::Create(const Vector &vecOrigin, const QAngle &vecAngles, const float fSpeed,
												 const float fGravity, ProjectileType_t projectileType,
												 CBaseEntity *pOwner, CBaseEntity *pScorer)
{
	const char *pszArrowEntityName = GetArrowEntityName(projectileType);
	CTFProjectile_Arrow *pArrow =
		static_cast<CTFProjectile_Arrow *>(CBaseEntity::Create(pszArrowEntityName, vecOrigin, vecAngles, pOwner));
	if(pArrow)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFProjectile_Arrow::InitArrow(const QAngle &vecAngles, const float fSpeed, const float fGravity,
									ProjectileType_t projectileType, CBaseEntity *pOwner, CBaseEntity *pScorer)
{
...
bool CTFProjectile_Arrow::CanHeadshot()
{
	...
	if(m_iProjectileType == TF_PROJECTILE_BUILDING_REPAIR_BOLT || m_iProjectileType == TF_PROJECTILE_HEALING_BOLT ||
...
//-----------------------------------------------------------------------------
void CTFProjectile_Arrow::GetBoneAttachmentInfo(mstudiobbox_t *pBox, CBaseAnimating *pOtherAnim, Vector &bonePosition,
...
bool CTFProjectile_Arrow::StrikeTarget(mstudiobbox_t *pBox, CBaseEntity *pOther)
{
	...
	bool bBreakArrow = IsBreakable() && ((dynamic_cast<CTFTankBoss *>(pOther) != NULL) ||
										 (dynamic_cast<CHalloweenBaseBoss *>(pOther) != NULL));
	...
	if(pAttacker)
	{
		...
		if(!InSameTeam(pOther))
		{
			...
			if(bApplyEffect)
			{
				...
				CTakeDamageInfo info(this, pAttacker, m_hLauncher, vecVelocity, vecOrigin, GetDamage(), nDamageType,
									 nDamageCustom);
...
void CTFProjectile_Arrow::OnArrowImpact(mstudiobbox_t *pBox, CBaseEntity *pOther, CBaseEntity *pAttacker)
{
	...
	if(!pOther->IsAlive() && pOther->IsPlayer())
	{
		...
		if(pTFPlayerOther && pTFPlayerOther->m_hRagdoll)
		{
			...
			if(CheckRagdollPinned(vecOrigin, vecVelocity, boneIndexAttached, physicsBoneIndex,
								  pTFPlayerOther->m_hRagdoll, pBox->group, pTFPlayerOther->entindex()))
			{
...
void CTFProjectile_Arrow::BuildingHealingArrow(CBaseEntity *pOther)
{
	...
	if(!pBuilding || !pBuilding->CanBeRepaired() || pBuilding->HasSapper() || pBuilding->IsPlasmaDisabled() ||
	   pBuilding->IsBuilding() || pBuilding->IsPlacing())
		return;
...
void CTFProjectile_Arrow::OnArrowMissAllPlayers()
{
	...
	{
		EconEntity_OnOwnerKillEaterEventNoPartner(assert_cast<CEconEntity *>(m_hLauncher.Get()), pOwner,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFProjectile_Arrow::CheckRagdollPinned(const Vector &start, const Vector &vel, int boneIndexAttached,
											 int physicsBoneIndex, CBaseEntity *pOther, int iHitGroup, int iVictim)
{
	// Pin to the wall.
	trace_t tr;
...
//-----------------------------------------------------------------------------
void CTFProjectile_HealingBolt::InitArrow(const QAngle &vecAngles, const float fSpeed, const float fGravity,
										  ProjectileType_t projectileType, CBaseEntity *pOwner, CBaseEntity *pScorer)
{
...
void CTFProjectile_HealingBolt::ImpactTeamPlayer(CTFPlayer *pOther)
{
	...
	EconEntity_OnOwnerKillEaterEvent_Batched(dynamic_cast<CEconEntity *>(GetLauncher()), pOwner, pOther,
											 kKillEaterEvent_AllyHealingDone, flHealth);
}

CTFProjectile_GrapplingHook::CTFProjectile_GrapplingHook() : m_pImpactFleshSoundLoop(NULL) {}
...
// Purpose: Setup function.
//-----------------------------------------------------------------------------
void CTFProjectile_GrapplingHook::InitArrow(const QAngle &vecAngles, const float fSpeed, const float fGravity,
											ProjectileType_t projectileType, CBaseEntity *pOwner, CBaseEntity *pScorer)
{
	BaseClass::InitArrow(vecAngles, fSpeed, fGravity, projectileType, pOwner, pScorer);
