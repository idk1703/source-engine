...
const char *CTriggerCatapult::s_szPlayerPassesTriggerFiltersThinkContext =
...
BEGIN_DATADESC(CTriggerCatapult)
	...
	DEFINE_THINKFUNC(LaunchThink), DEFINE_THINKFUNC(PlayerPassesTriggerFiltersThink),
		...
		DEFINE_FIELD(m_hLaunchTarget, FIELD_EHANDLE), DEFINE_ARRAY(m_flRefireDelay, FIELD_TIME, MAX_PLAYERS + 1),
...
void CTriggerCatapult::Spawn(void)
{
	...
	SetContextThink(&CTriggerCatapult::PlayerPassesTriggerFiltersThink, gpGlobals->curtime + 1.0f,
					s_szPlayerPassesTriggerFiltersThinkContext);
}
...
// network that down to the client
