...
protected:
	bool PrintTFEvent(IGameEvent *event) // print Mod specific logs
	{
		...
		if(!Q_strncmp(eventName, "player_death", Q_strlen("player_death")))
		{
			...
			if(pPlayer == pAttacker)
			{
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" committed suicide with \"%s\" (attacker_position \"%d %d %d\")\n",
							   pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
							   pPlayer->GetTeam()->GetName(), weapon, (int)pPlayer->GetAbsOrigin().x,
			...
			else if(pAttacker)
			{
				...
				if(pszCustom)
				{
					UTIL_LogPrintf("\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\" with \"%s\" (customkill \"%s\") "
								   "(attacker_position \"%d %d %d\") (victim_position \"%d %d %d\")\n",
								   pAttacker->GetPlayerName(), attackerid, pAttacker->GetNetworkIDString(),
								   pAttacker->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid,
								   pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName(), weapon, pszCustom,
								   (int)pAttacker->GetAbsOrigin().x, (int)pAttacker->GetAbsOrigin().y,
								   (int)pAttacker->GetAbsOrigin().z, (int)pPlayer->GetAbsOrigin().x,
								   (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
				}
				else
				{
					UTIL_LogPrintf("\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\" with \"%s\" (attacker_position \"%d "
								   "%d %d\") (victim_position \"%d %d %d\")\n",
								   pAttacker->GetPlayerName(), attackerid, pAttacker->GetNetworkIDString(),
								   pAttacker->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid,
								   pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName(), weapon,
								   (int)pAttacker->GetAbsOrigin().x, (int)pAttacker->GetAbsOrigin().y,
								   (int)pAttacker->GetAbsOrigin().z, (int)pPlayer->GetAbsOrigin().x,
								   (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
				}
			...
			else
			{
				...
				if((iDamageBits & DMG_VEHICLE) || (iDamageBits & DMG_NERVEGAS))
				{
					...
					// killed by the world
					UTIL_LogPrintf("\"%s<%i><%s><%s>\" committed suicide with \"world\" (customkill \"%s\") "
								   "(attacker_position \"%d %d %d\")\n",
								   pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
								   pPlayer->GetTeam()->GetName(), pszCustomKill, (int)pPlayer->GetAbsOrigin().x,
								   (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
				}
				else
				{
					// killed by the world
					UTIL_LogPrintf(
						"\"%s<%i><%s><%s>\" committed suicide with \"world\" (attacker_position \"%d %d %d\")\n",
						pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName(),
						(int)pPlayer->GetAbsOrigin().x, (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
				}
			}
			...
			{
				UTIL_LogPrintf(
					"\"%s<%i><%s><%s>\" triggered \"kill assist\" against \"%s<%i><%s><%s>\" (assister_position \"%d "
					"%d %d\") (attacker_position \"%d %d %d\") (victim_position \"%d %d %d\")\n",
					pAssister->GetPlayerName(), assistid, pAssister->GetNetworkIDString(),
					pAssister->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
					pPlayer->GetTeam()->GetName(), (int)pAssister->GetAbsOrigin().x, (int)pAssister->GetAbsOrigin().y,
					(int)pAssister->GetAbsOrigin().z, pAttacker ? (int)pAttacker->GetAbsOrigin().x : 0,
					pAttacker ? (int)pAttacker->GetAbsOrigin().y : 0, pAttacker ? (int)pAttacker->GetAbsOrigin().z : 0,
					(int)pPlayer->GetAbsOrigin().x, (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
			}
			...
			if(event->GetInt("death_flags") & TF_DEATH_DOMINATION && pAttacker)
			{
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"domination\" against \"%s<%i><%s><%s>\"\n",
							   pAttacker->GetPlayerName(), attackerid, pAttacker->GetNetworkIDString(),
							   pAttacker->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid,
							   pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName());
			}
			if(event->GetInt("death_flags") & TF_DEATH_ASSISTER_DOMINATION && pAssister)
			{
				UTIL_LogPrintf(
					"\"%s<%i><%s><%s>\" triggered \"domination\" against \"%s<%i><%s><%s>\" (assist \"1\")\n",
					pAssister->GetPlayerName(), assistid, pAssister->GetNetworkIDString(),
					pAssister->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
			...
			if(event->GetInt("death_flags") & TF_DEATH_ASSISTER_REVENGE && pAssister)
			{
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"revenge\" against \"%s<%i><%s><%s>\" (assist \"1\")\n",
							   pAssister->GetPlayerName(), assistid, pAssister->GetNetworkIDString(),
							   pAssister->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid,
							   pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName());
			}
		...
		else if(FStrEq(eventName, "player_builtobject") || FStrEq(eventName, "player_carryobject") ||
				FStrEq(eventName, "player_dropobject") || FStrEq(eventName, "player_removed") ||
				FStrEq(eventName, "object_detonated"))
		{
			...
			if(pPlayer)
			{
				...
				const int objectid =
					!event->IsEmpty("objecttype") ? event->GetInt("objecttype") : event->GetInt("object");
				...
				if(pInfo)
				{
					UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"%s\" (object \"%s\") (position \"%d %d %d\")\n",
								   pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(),
								   pPlayer->GetTeam()->GetName(), eventName, pInfo->m_pObjectName,
		...
		else if(FStrEq(eventName, "object_destroyed"))
		{
			...
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"killedobject\" (object \"%s\") (weapon \"%s\") (objectowner "
						   ...
						   pAttacker->GetPlayerName(), attackerid, pAttacker->GetNetworkIDString(),
						   pAttacker->GetTeam()->GetName(), pInfo->m_pObjectName, weapon, pObjectOwner->GetPlayerName(),
						   userid, pObjectOwner->GetNetworkIDString(), pObjectOwner->GetTeam()->GetName(),
						   (int)pAttacker->GetAbsOrigin().x, (int)pAttacker->GetAbsOrigin().y,
			...
			{
				UTIL_LogPrintf(
					"\"%s<%i><%s><%s>\" triggered \"killedobject\" (object \"%s\") (objectowner \"%s<%i><%s><%s>\") "
					"(assist \"1\") (assister_position \"%d %d %d\") (attacker_position \"%d %d %d\")\n",
					pAssister->GetPlayerName(), assisterid, pAssister->GetNetworkIDString(),
					pAssister->GetTeam()->GetName(), pInfo->m_pObjectName, pObjectOwner->GetPlayerName(), userid,
					pObjectOwner->GetNetworkIDString(), pObjectOwner->GetTeam()->GetName(),
					(int)pAssister->GetAbsOrigin().x, (int)pAssister->GetAbsOrigin().y,
					(int)pAssister->GetAbsOrigin().z, (int)pAttacker->GetAbsOrigin().x,
					(int)pAttacker->GetAbsOrigin().y, (int)pAttacker->GetAbsOrigin().z);
			}
		}
		else if(FStrEq(eventName, "teamplay_flag_event"))
		{
			...
			if(bPlainLogEntry)
			{
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"flagevent\" (event \"%s\") (position \"%d %d %d\")\n",
							   ...
							   pPlayer->GetTeam()->GetName(), pszEvent, (int)pPlayer->GetAbsOrigin().x,
							   (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
			}
			else
			{
				...
				UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"flagevent\" (event \"%s\") (team_caps \"%d\") "
							   "(caps_per_round \"%d\") (position \"%d %d %d\")\n",
							   pPlayer->GetPlayerName(), pPlayer->GetUserID(), pPlayer->GetNetworkIDString(),
							   pPlayer->GetTeam()->GetName(), pszEvent, pTeam->GetFlagCaptures(),
							   tf_flag_caps_per_round.GetInt(), (int)pPlayer->GetAbsOrigin().x,
							   (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
			}
		...
		else if(FStrEq(eventName, "teamplay_capture_blocked"))
		{
			...
			UTIL_LogPrintf(
				"\"%s<%i><%s><%s>\" triggered \"captureblocked\" (cp \"%d\") (cpname \"%s\") (position \"%d %d %d\")\n",
				pBlocker->GetPlayerName(), pBlocker->GetUserID(), pBlocker->GetNetworkIDString(),
				pBlocker->GetTeam()->GetName(), event->GetInt("cp"), event->GetString("cpname"),
				(int)pBlocker->GetAbsOrigin().x, (int)pBlocker->GetAbsOrigin().y, (int)pBlocker->GetAbsOrigin().z);
		}
		else if(FStrEq(eventName, "teamplay_point_captured"))
		{
			...
			Q_snprintf(buf, sizeof(buf),
					   "Team \"%s\" triggered \"pointcaptured\" (cp \"%d\") (cpname \"%s\") (numcappers \"%d\") ",
					   pTeam->GetName(), event->GetInt("cp"), event->GetString("cpname"), iNumCappers);
			...
			for(int i = 0; i < iNumCappers; i++)
			{
				...
				Q_snprintf(playerBuf, sizeof(playerBuf), "(player%d \"%s<%i><%s><%s>\") (position%d \"%d %d %d\") ",
						   i + 1, pPlayer->GetPlayerName(), pPlayer->GetUserID(), pPlayer->GetNetworkIDString(),
						   pPlayer->GetTeam()->GetName(), i + 1, (int)pPlayer->GetAbsOrigin().x,
						   (int)pPlayer->GetAbsOrigin().y, (int)pPlayer->GetAbsOrigin().z);
			...
			else
			{
				...
				if(pMaster && pMaster->PlayingMiniRounds())
				{
					UTIL_LogPrintf("World triggered \"Mini_Round_Win\" (winner \"%s\") (round \"%s\")\n", pszWinner,
								   pMaster->GetCurrentRound()->GetEntityName().ToCStr());
		...
		else if(FStrEq(eventName, "medic_death"))
		{
			...
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"medic_death\" against \"%s<%i><%s><%s>\" (healing \"%d\") "
						   "(ubercharge \"%s\")\n",
						   pAttacker->GetPlayerName(), attackerid, pAttacker->GetNetworkIDString(),
						   pAttacker->GetTeam()->GetName(), pPlayer->GetPlayerName(), userid,
						   pPlayer->GetNetworkIDString(), pPlayer->GetTeam()->GetName(), iHealing,
						   bCharged ? "1" : "0");
