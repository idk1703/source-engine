...
ConVar tf_raid_wandering_density("tf_raid_wandering_density", "0.00001", FCVAR_CHEAT,
								 "Wandering defenders per unit area");
ConVar tf_raid_spawn_wanderers("tf_raid_spawn_wanderers", "1" /*, FCVAR_CHEAT*/);

ConVar tf_raid_defender_density("tf_raid_defender_density", "0.000001", 0 /*FCVAR_CHEAT*/,
								"Wandering defenders per unit area");
...
ConVar tf_raid_sentry_spacing("tf_raid_sentry_spacing", "750", 0 /*FCVAR_CHEAT*/,
							  "Minimum travel distance between sentry gun spots");
...
ConVar tf_raid_spawn_mob_as_squad_chance_start("tf_raid_spawn_mob_as_squad_chance_start",
											   "100"); // /*, FCVAR_CHEAT*/ );
ConVar tf_raid_spawn_mob_as_squad_chance_halfway("tf_raid_spawn_mob_as_squad_chance_halfway",
												 "100"); // /*, FCVAR_CHEAT*/ );
ConVar tf_raid_spawn_mob_as_squad_chance_final("tf_raid_spawn_mob_as_squad_chance_final",
											   "100");								  // /*, FCVAR_CHEAT*/ );
...
void CRaidLogic::FireGameEvent(IGameEvent *event)
{
	...
	if(!Q_strcmp(eventName, "teamplay_point_captured"))
	{
		...
		StartMobTimer(
			RandomFloat(tf_raid_mob_spawn_min_interval.GetFloat(), tf_raid_mob_spawn_max_interval.GetFloat()));
...
bool CRaidLogic::Unspawn(CTFPlayer *who)
{
	if(who->IsAlive() && who->GetTeamNumber() == TF_TEAM_RED)
	{
		...
		if(who->IsPlayerClass(TF_CLASS_ENGINEER))
		{
			...
			if(area && area->GetIncursionDistance(TF_TEAM_BLUE) >
						   GetMaximumRaiderIncursionDistance() - tf_populator_active_buffer_range.GetFloat())
				return false;
		}
...
public:
	...
	virtual bool operator()(CNavArea *baseArea, CNavArea *priorArea, float travelDistanceSoFar)
	{
		...
		if(area->GetIncursionDistance(TF_TEAM_BLUE) <
		   TFGameRules()->GetRaidLogic()->GetMaximumRaiderIncursionDistance())
		{
...
void CRaidLogic::DrawDebugDisplay(float deltaT)
{
	...
	{
		NDebugOverlay::ScreenText(0.01f, 0.5f, CFmtStr("Mob timer: %3.2f", m_mobSpawnTimer.GetRemainingTime()), 255,
								  255, 0, 255, 0.33f);
		...
		NDebugOverlay::ScreenText(0.01f, 0.60f,
								  CFmtStr("Raider max inc range: %3.2f", GetMaximumRaiderIncursionDistance()), 255, 255,
								  0, 255, 0.33f);
	}
	...
	if(tf_raid_show_escape_route.GetInt() == 2)
	{
		if(m_escapeRouteVector.Count() >= 2)
		{
			...
			{
				NDebugOverlay::HorzArrow(m_escapeRouteVector[i]->GetCenter(), m_escapeRouteVector[i + 1]->GetCenter(),
										 5.0f, 255, 0, 0, 255, true, deltaT);
...
void CRaidLogic::Update(void)
{
	...
	if(IsWaitingForRaidersToLeaveSafeRoom())
	{
		...
		for(int i = 0; i < raidingTeam->GetNumPlayers(); ++i)
		{
			...
			if(!area->HasAttributeTF(TF_NAV_SPAWN_ROOM_BLUE))
			{
				...
				StartMobTimer(RandomFloat(0.5f * tf_raid_mob_spawn_min_interval.GetFloat(),
										  tf_raid_mob_spawn_max_interval.GetFloat()));
	...
	for(i = 0; i < m_escapeRouteVector.Count(); ++i)
	{
		...
		if(area->GetIncursionDistance(TF_TEAM_BLUE) <= GetMaximumRaiderIncursionDistance())
		{
			...
			{
				if(m_farthestAlongEscapeRouteArea->GetIncursionDistance(TF_TEAM_BLUE) <
				   area->GetIncursionDistance(TF_TEAM_BLUE))
				{
	...
	if(validAreas.Count())
	{
		...
		if(hide.m_hiddenArea)
		{
			...
			if(tf_debug_sniper_spots.GetBool())
			{
				...
				NDebugOverlay::HorzArrow(hide.m_hiddenArea->GetCenter() + Vector(0, 0, 10.0f),
										 sniperArea->GetCenter() + Vector(0, 0, 10.0f), 5.0f, 255, 255, 0, 255, true,
										 deltaT);
...
void CRaidLogic::BuildEscapeRoute(void)
{
	...
	if(entity)
	{
		...
		for(pathNode = dynamic_cast<CPathTrack *>(gEntList.FindEntityByClassname(pathNode, "path_track")); pathNode;
		...
		if(firstPathNode)
		{
			...
			if(pathTrackAreaVector.Count() > 1)
			{
				...
				for(int i = 1; i < pathTrackAreaVector.Count(); ++i)
				{
					...
					TheNavMesh->ForAllAreasAlongLine(markAsEscapeRoute, pathTrackAreaVector[i - 1],
													 pathTrackAreaVector[i]);
				}
