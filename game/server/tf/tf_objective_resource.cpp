...
void SendProxy_String_tToString(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut,
...
SendPropInt(SENDINFO(m_nMannVsMachineMaxWaveCount), 9, SPROP_UNSIGNED),
	...
	SendPropInt(SENDINFO(m_nMvMWorldMoney), 16, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_flMannVsMachineNextWaveTime)),
	SendPropBool(SENDINFO(m_bMannVsMachineBetweenWaves)),
	SendPropInt(SENDINFO(m_nFlagCarrierUpgradeLevel), 4, SPROP_UNSIGNED),
	SendPropFloat(SENDINFO(m_flMvMBaseBombUpgradeTime)), SendPropFloat(SENDINFO(m_flMvMNextBombUpgradeTime)),
	SendPropStringT(SENDINFO(m_iszMvMPopfileName)), SendPropInt(SENDINFO(m_iChallengeIndex), 16),
	SendPropInt(SENDINFO(m_nMvMEventPopfileType), 4, SPROP_UNSIGNED),

	SendPropArray3(SENDINFO_ARRAY3(m_nMannVsMachineWaveClassCounts),
				   SendPropInt(SENDINFO_ARRAY(m_nMannVsMachineWaveClassCounts), 16)),
	SendPropArray(SendPropString(SENDINFO_ARRAY(m_iszMannVsMachineWaveClassNames), 0, SendProxy_String_tToString),
				  m_iszMannVsMachineWaveClassNames),
	SendPropArray3(SENDINFO_ARRAY3(m_nMannVsMachineWaveClassFlags),
				   SendPropInt(SENDINFO_ARRAY(m_nMannVsMachineWaveClassFlags), 10, SPROP_UNSIGNED)),

	SendPropArray3(SENDINFO_ARRAY3(m_nMannVsMachineWaveClassCounts2),
				   SendPropInt(SENDINFO_ARRAY(m_nMannVsMachineWaveClassCounts2), 16)),
	SendPropArray(SendPropString(SENDINFO_ARRAY(m_iszMannVsMachineWaveClassNames2), 0, SendProxy_String_tToString),
				  m_iszMannVsMachineWaveClassNames2),
	SendPropArray3(SENDINFO_ARRAY3(m_nMannVsMachineWaveClassFlags2),
				   SendPropInt(SENDINFO_ARRAY(m_nMannVsMachineWaveClassFlags2), 10, SPROP_UNSIGNED)),

	SendPropArray3(SENDINFO_ARRAY3(m_bMannVsMachineWaveClassActive),
				   SendPropBool(SENDINFO_ARRAY(m_bMannVsMachineWaveClassActive))),
	SendPropArray3(SENDINFO_ARRAY3(m_bMannVsMachineWaveClassActive2),
				   SendPropBool(SENDINFO_ARRAY(m_bMannVsMachineWaveClassActive2))),
END_SEND_TABLE
()

	BEGIN_DATADESC(CTFObjectiveResource) DEFINE_FIELD(m_nMannVsMachineMaxWaveCount, FIELD_INTEGER),
	DEFINE_FIELD(m_nMannVsMachineWaveCount, FIELD_INTEGER), DEFINE_FIELD(m_nMannVsMachineWaveEnemyCount, FIELD_INTEGER),
	DEFINE_FIELD(m_nMvMWorldMoney, FIELD_INTEGER), DEFINE_FIELD(m_flMannVsMachineNextWaveTime, FIELD_TIME),
...
{
	int i = 0;
	for(i = 0; i < m_iszMannVsMachineWaveClassNames.Count() && i < m_nMannVsMachineWaveClassFlags.Count() &&
			   i < m_nMannVsMachineWaveClassCounts.Count();
		++i)
	{
		if((m_iszMannVsMachineWaveClassNames[i] == iszClassIconName) && (m_nMannVsMachineWaveClassFlags[i] & iFlags))
		{
	...
	for(i = 0; i < m_iszMannVsMachineWaveClassNames2.Count() && i < m_nMannVsMachineWaveClassFlags2.Count() &&
			   i < m_nMannVsMachineWaveClassCounts2.Count();
		++i)
	{
		if((m_iszMannVsMachineWaveClassNames2[i] == iszClassIconName) && (m_nMannVsMachineWaveClassFlags2[i] & iFlags))
		{
