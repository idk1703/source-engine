...
void CBotNPCDecoy::Spawn(void)
{
	...
	int ownerClass = owner->m_Shared.InCond(TF_COND_DISGUISED) ? owner->m_Shared.GetDisguiseClass()
															   : owner->GetPlayerClass()->GetClassIndex();
	int ownerTeam =
		owner->m_Shared.InCond(TF_COND_DISGUISED) ? owner->m_Shared.GetDisguiseTeam() : owner->GetTeamNumber();
