...
void CBotNPCMinion::BecomeAmmoPack(void)
{
	...
	// CTFAmmoPack *pAmmoPack = CTFAmmoPack::Create( GetAbsOrigin(), GetAbsAngles(), NULL,
	// "models/props_swamp/bug_zapper.mdl" );
	CTFAmmoPack *pAmmoPack =
		CTFAmmoPack::Create(GetAbsOrigin(), GetAbsAngles(), NULL, "models/items/ammopack_medium.mdl");
	if(pAmmoPack)
	{
...
void CBotNPCMinion::UpdateTarget(void)
{
	...
	float closestRangeSq =
		IsAlert()
			? FLT_MAX
			: (tf_bot_npc_minion_notice_threat_range.GetFloat() * tf_bot_npc_minion_notice_threat_range.GetFloat());
...
ActionResult<CBotNPCMinion> CBotNPCMinionRoam::Update(CBotNPCMinion *me, float interval)
{
	...
	if(m_goalArea == NULL || me->GetLastKnownArea() == m_goalArea ||
...
void CNextBotFlyingLocomotion::MaintainAltitude(void)
{
	...
	TraceHull(me->GetAbsOrigin() + Vector(0, 0, ceiling), me->GetAbsOrigin() + Vector(0, 0, -1000.0f),
...
void CNextBotFlyingLocomotion::Update(void)
{
	...
	Vector damping(tf_bot_npc_minion_horiz_damping.GetFloat(), tf_bot_npc_minion_horiz_damping.GetFloat(),
				   tf_bot_npc_minion_vert_damping.GetFloat());
...
void CNextBotFlyingLocomotion::Deflect(CBaseEntity *deflector)
{
	if(deflector)
	{
		...
		{
			m_velocity += (1.0f - (range / tf_bot_npc_minion_deflect_range.GetFloat())) *
						  tf_bot_npc_minion_deflect_force.GetFloat() * fromDeflector;
		}
	}
}
