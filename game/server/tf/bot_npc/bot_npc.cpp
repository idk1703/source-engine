...
ConVar tf_bot_npc_minion_launch_count_increase_interval("tf_bot_npc_minion_launch_count_increase_interval",
														"999999999" /*, FCVAR_CHEAT*/); // 30
			...
			if(GetAttachment("head", headPos, headAngles))
			{
				...
				*/
				...
				/*
								...
								if ( inflictor )
								{
									damagePos = inflictor->GetAbsOrigin() + 3.0f * gpGlobals->frametime *
				   inflictor->GetAbsVelocity();
								}
				*/
...
void CBotNPC::UpdateAttackTarget(void)
{
	...
	if(!attackTargetThreat ||
	   maxThreat->m_threat > attackTargetThreat->m_threat + tf_bot_npc_threat_tolerance.GetFloat())
	{
		// change threats
		SetAttackTarget(maxThreat->m_who);
	}
...
{
	if(target && m_attackTarget != NULL && m_attackTarget->IsAlive() && m_attackTargetTimer.HasStarted() &&
	   !m_attackTargetTimer.IsElapsed())
	{
...
void TossAmmoPack(CBotNPC *me)
{
	...
	CTFAmmoPack *pAmmoPack =
		CTFAmmoPack::Create(me->GetAbsOrigin(), me->GetAbsAngles(), NULL, "models/items/ammopack_medium.mdl");
	if(pAmmoPack)
	{
		/*
				Vector vel;
...
{
	CTFGrenadePipebombProjectile *pProjectile = CTFGrenadePipebombProjectile::Create(
		me->WorldSpaceCenter(), vec3_angle, launchVel, AngularImpulse(600, random->RandomInt(-1200, 1200), 0), me,
		*weaponInfo, TF_PROJECTILE_PIPEBOMB_REMOTE, 1);
	if(pProjectile)
	{
...
void CBotNPCLaunchGrenades::LaunchGrenadeRings(CBotNPC *me)
{
	...
	float deltaVel =
		tf_bot_npc_grenade_ring_max_horiz_vel.GetFloat() - tf_bot_npc_grenade_ring_min_horiz_vel.GetFloat();
...
void CBotNPCLaunchGrenades::LaunchGrenadeSpokes(CBotNPC *me)
{
	...
	float deltaVel =
		tf_bot_npc_grenade_spoke_max_horiz_vel.GetFloat() - tf_bot_npc_grenade_spoke_min_horiz_vel.GetFloat();
...
ActionResult<CBotNPC> CBotNPCShootCrossbow::Update(CBotNPC *me, float interval)
{
	...
	if(m_timer.IsElapsed())
	{
		...
		CTFProjectile_Arrow *arrow = CTFProjectile_Arrow::Create(muzzleOrigin, muzzleAngles, arrowSpeed, arrowGravity,
																 TF_PROJECTILE_ARROW, me, me);
		if(arrow)
		{
...
ActionResult<CBotNPC> CBotNPCChaseVictim::Update(CBotNPC *me, float interval)
{
	...
	if(me->IsLineOfSightClear(m_chaseTarget))
	{
		...
		{
			if((me->GetGrenadeTimer()->IsElapsed() &&
				me->IsRangeLessThan(m_chaseTarget, tf_bot_npc_grenade_launch_range.GetFloat())) ||
...
ConVar tf_bot_npc_laser_damage_building_multiplier("tf_bot_npc_laser_damage_building_multiplier",
												   "4" /*, FCVAR_CHEAT*/);
...
ActionResult<CBotNPC> CBotNPCLaserBlast::Update(CBotNPC *me, float interval)
{
	...
	if(me->IsLineOfSightClear(target))
	{
		...
		float damage = tf_bot_npc_laser_damage_rate.GetFloat() +
					   m_laserTimer.GetElapsedTime() * tf_bot_npc_laser_damage_gain_rate.GetFloat();
...
ActionResult<CBotNPC> CBotNPCAttack::Update(CBotNPC *me, float interval)
{
	...
	if(!me->IsSwingingAxe())
	{
		...
		if(me->GetReceivedDamagePerSecond() > tf_bot_npc_block_dps_react.GetFloat() && target->IsPlayer() &&
...
public:
			...
			if(m_lookTimer.IsElapsed())
			{
				...
				if(myArea)
				{
					const CUtlVector<CTFNavArea *> &invasionAreaVector =
						myArea->GetEnemyInvasionAreaVector(TF_TEAM_RED);
					...
					if(invasionAreaVector.Count() > 0)
					{
						...
						for(int r = 0; r < retryCount; ++r)
						{
							...
							Vector gazeSpot =
								invasionAreaVector[which]->GetRandomPoint() + Vector(0, 0, 0.75f * HumanHeight);

							if(me->IsRangeGreaterThan(gazeSpot, minGazeRange) &&
							   me->GetVisionInterface()->IsLineOfSightClear(gazeSpot))
							{
	...
	virtual EventDesiredResult<CBotNPC> OnInjured(CBotNPC *me, const CTakeDamageInfo &info)
	{
		...
		if(me->HasAbility(CBotNPC::CAN_BE_STUNNED) && attacker)
		{
			...
			{
				return TrySuspendFor(new CBotNPCStunned(tf_bot_npc_stunned_duration.GetFloat()), RESULT_CRITICAL,
									 "CVar force stunned");
			}
					...
					if((info.GetDamagePosition() - headPos).IsLengthLessThan(tf_bot_npc_head_radius.GetFloat()))
					{
						...
						if(isDeflectedRocket)
						{
							...
							else
							{
								...
								{
									return TrySuspendFor(new CBotNPCStunned(tf_bot_npc_stunned_duration.GetFloat()),
														 RESULT_CRITICAL, "My own rockets reflected into my head!");
								}
							}
						...
						if(toAttacker.z > 0.9f)
						{
							...
							return TrySuspendFor(new CBotNPCStunned(tf_bot_npc_stunned_duration.GetFloat()),
												 RESULT_CRITICAL, "Hard head hit from above!");
						}
					}
				}
			}
...
public:
	...
	virtual ActionResult<CBotNPC> Update(CBotNPC *me, float interval)
	{
		...
		/*
				...
				{
					return SuspendFor( new CBotNPCBigJump( me->GetHomePosition(), new CBotNPCLaunchRockets ), "Jumping
		   home" );
				}
		*/
...
bool CBotNPCVision::IsIgnored(CBaseEntity *subject) const
{
	if(subject->IsPlayer())
	{
		...
		if(enemy->m_Shared.InCond(TF_COND_BURNING) || enemy->m_Shared.InCond(TF_COND_URINE) ||
		   enemy->m_Shared.InCond(TF_COND_STEALTHED_BLINK) || enemy->m_Shared.InCond(TF_COND_BLEEDING))
		{
			// always notice players with these conditions
			return false;
		}
		...
		if(enemy->m_Shared.InCond(TF_COND_DISGUISED) &&
		   enemy->m_Shared.GetDisguiseTeam() == GetBot()->GetEntity()->GetTeamNumber())
		{
