...
ConVar obj_damage_factor("obj_damage_factor", "0", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
						 "Factor applied to all damage done to objects");
ConVar obj_child_damage_factor("obj_child_damage_factor", "0.25", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
...
ConVar tf_obj_ground_clearance("tf_obj_ground_clearance", "32", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
							   "Object corners can be this high above the ground");

ConVar tf_obj_damage_tank_achievement_amount("tf_obj_damage_tank_achievement_amount", "2000",
...
ConVar object_deterioration_time("object_deterioration_time", "30", 0,
								 "Time it takes for a fully-healed object to deteriorate.");
...
BEGIN_DATADESC(CBaseObject)
	...
	DEFINE_KEYFIELD_NOT_SAVED(m_SolidToPlayers, FIELD_INTEGER, "SolidToPlayer"),
		...
		DEFINE_INPUTFUNC(FIELD_STRING, "SetBuilder", InputSetBuilder), DEFINE_INPUTFUNC(FIELD_VOID, "Show", InputShow),
		DEFINE_INPUTFUNC(FIELD_VOID, "Hide", InputHide), DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
		DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),

		// Outputs
		DEFINE_OUTPUT(m_OnDestroyed, "OnDestroyed"), DEFINE_OUTPUT(m_OnDamaged, "OnDamaged"),
		DEFINE_OUTPUT(m_OnRepaired, "OnRepaired"), DEFINE_OUTPUT(m_OnBecomingDisabled, "OnDisabled"),
...
SendPropInt(SENDINFO(m_iHealth), -1, SPROP_VARINT), SendPropInt(SENDINFO(m_iMaxHealth), -1, SPROP_VARINT),
	SendPropBool(SENDINFO(m_bHasSapper)), SendPropInt(SENDINFO(m_iObjectType), Q_log2(OBJ_LAST) + 1, SPROP_UNSIGNED),
	SendPropBool(SENDINFO(m_bBuilding)), SendPropBool(SENDINFO(m_bPlacing)), SendPropBool(SENDINFO(m_bCarried)),
	SendPropBool(SENDINFO(m_bCarryDeploy)), SendPropBool(SENDINFO(m_bMiniBuilding)),
	SendPropFloat(SENDINFO(m_flPercentageConstructed), 8, 0, 0.0, 1.0f),
	SendPropInt(SENDINFO(m_fObjectFlags), OF_BIT_COUNT, SPROP_UNSIGNED), SendPropEHandle(SENDINFO(m_hBuiltOnEntity)),
	SendPropBool(SENDINFO(m_bDisabled)), SendPropEHandle(SENDINFO(m_hBuilder)),
	...
	SendPropBool(SENDINFO(m_bServerOverridePlacement)), SendPropInt(SENDINFO(m_iUpgradeLevel), 3),
	SendPropInt(SENDINFO(m_iUpgradeMetal), 10), SendPropInt(SENDINFO(m_iUpgradeMetalRequired), 10),
	SendPropInt(SENDINFO(m_iHighestUpgradeLevel), 3), SendPropInt(SENDINFO(m_iObjectMode), 2, SPROP_UNSIGNED),
	SendPropBool(SENDINFO(m_bDisposableBuilding)), SendPropBool(SENDINFO(m_bWasMapPlaced)),
...
void CBaseObject::SpawnControlPanels()
{
	...
	for(nPanel = 0; true; ++nPanel)
	{
		...
		CVGuiScreen *pScreen =
			CreateVGuiScreen(pScreenClassname, pScreenName, pEntityToSpawnOn, this, nLLAttachmentIndex);
...
void CBaseObject::DeterminePlaybackRate(void)
{
	...
	if(bAdjustCompleteTime)
	{
		...
		flUpgradeTime /=
			((flReverseBuildingConstructionSpeed < 0.0f) ? (flReverseBuildingConstructionSpeed * -1.0) : 1.0f);
...
void CBaseObject::DestroyObject(void)
{
	...
	if(IsCarried() && GetBuilder())
	{
		...
		CTFWeaponBuilder *pBuilder =
			dynamic_cast<CTFWeaponBuilder *>(GetBuilder()->Weapon_OwnsThisID(TF_WEAPON_BUILDER));
		if(pBuilder)
		{
...
//-----------------------------------------------------------------------------
bool CBaseObject::FindNearestBuildPoint(CBaseEntity *pEntity, CBasePlayer *pBuilder, float &flNearestPoint,
										Vector &vecNearestBuildPoint, bool bIgnoreChecks)
{
...
//-----------------------------------------------------------------------------
bool CBaseObject::FindBuildPointOnPlayer(CTFPlayer *pTFPlayer, CBasePlayer *pBuilder, float &flNearestPoint,
										 Vector &vecNearestBuildPoint)
{
	bool bFoundPoint = false;
...
bool CBaseObject::TestPositionForPlayerBlock( Vector vecBuildOrigin, CBasePlayer *pPlayer )
{
	...
	for ( i=0;i<8;i++ )
	{
		...
		// NDebugOverlay::Box( vecTest, VEC_HULL_MIN, VEC_HULL_MAX, bNodeClear[i] ? 0 : 255, bNodeClear[i] ? 255 : 0, 0,
20, 0.1 );
...
bool CBaseObject::FindSnapToBuildPos(CBaseObject *pObjectOverride)
{
	...
	if(!pObjectOverride)
	{
		...
		for(int iTeam = FIRST_GAME_TEAM; iTeam < nTeamCount; ++iTeam)
		{
			...
			if(TFGameRules() && TFGameRules()->GameModeUsesMiniBosses() && GetType() == OBJ_ATTACHMENT_SAPPER &&
			   !pPlayer->IsBot())
			{
				CUtlVector<CTFPlayer *> playerVector;
				CollectPlayers(&playerVector, pPlayer->GetOpposingTFTeam()->GetTeamNumber(),
...
bool CBaseObject::StartBuilding(CBaseEntity *pBuilder)
{
	...
	trace_t trace;
	UTIL_TraceHull(GetAbsOrigin() + Vector(0.0f, 0.0f, 2.0f), GetAbsOrigin() - Vector(0.0f, 0.0f, 2.0f), vec3_origin,
				   vec3_origin, MASK_PLAYERSOLID_BRUSHONLY, this, COLLISION_GROUP_PLAYER_MOVEMENT, &trace);
...
void CBaseObject::BuildingThink(void)
{
	...
	Construct((GetMaxHealth() - OBJECT_CONSTRUCTION_STARTINGHEALTH) / m_flTotalConstructionTime *
		...
		for(idx = g_DamageMap.First(); idx != g_DamageMap.InvalidIndex(); idx = g_DamageMap.Next(idx))
		{
			...
			*strchr(szRowName, ',') = '\0';
			...
			if(g_CompletedRows.Find(szRowName) == g_CompletedRows.InvalidIndex())
			{
				...
				for(int ColumnIdx = g_UniqueColumns.First(); ColumnIdx != g_UniqueColumns.InvalidIndex();
					ColumnIdx = g_UniqueColumns.Next(ColumnIdx))
				{
					char szRowNameCommaColumn[256];
					Q_strncpy(szRowNameCommaColumn, szRowNameComma, sizeof(szRowNameCommaColumn));
					Q_strncat(szRowNameCommaColumn, g_UniqueColumns.GetElementName(ColumnIdx),
							  sizeof(szRowNameCommaColumn), COPY_ALL_CHARACTERS);
...
int CBaseObject::OnTakeDamage(const CTakeDamageInfo &info)
{
	...
	if(info.GetAttacker())
	{
		...
		if(TFGameRules() && TFGameRules()->IsTruceActive())
		{
			...
			if(info.GetAttacker()->IsPlayer() && info.GetAttacker()->IsTruceValidForEnt() &&
			   ((info.GetAttacker()->GetTeamNumber() == TF_TEAM_RED) ||
				(info.GetAttacker()->GetTeamNumber() == TF_TEAM_BLUE)))
				return 0;
		}
	}
	...
	if(TFGameRules()->IsPowerupMode())
	{
		...
		if(pAttacker)
		{
			if(pAttacker->m_Shared.GetCarryingRuneType() == RUNE_STRENGTH ||
			   pAttacker->m_Shared.InCond(TF_COND_RUNE_IMBALANCE))
			{
				flDamage *= 2.f;
			}
	...
	m_AchievementData.AddDamageEventToHistory(info.GetAttacker(),
											  (IsDamageFatal(m_flHealth, flDamage)) ? m_flHealth : flDamage);
...
bool CBaseObject::Construct(float flHealth)
{
	...
	if(IsBuilding())
	{
		...
		float flConstructionTime =
			flHealth / ((GetMaxHealth() - OBJECT_CONSTRUCTION_STARTINGHEALTH) / m_flTotalConstructionTime);
...
void CBaseObject::CreateObjectGibs(void)
{
	...
	const int nTotalMetal = pObjectInfo->m_iMetalToDropInGibs == 0 ? pObjectInfo->m_Cost * flMetalCostPercentage
																   : pObjectInfo->m_iMetalToDropInGibs;
...
CTFAmmoPack *CBaseObject::CreateAmmoPack(const char *pchModel, int nMetal)
{
	...
	if(pAmmoPack)
	{
		...
		Vector vecImpulse(random->RandomFloat(-0.5, 0.5), random->RandomFloat(-0.5, 0.5),
						  random->RandomFloat(0.75, 1.25));
...
void CBaseObject::Killed(const CTakeDamageInfo &info)
{
	...
	// if this object has a sapper on it, and was not killed by the sapper (killed by damage other than crush, since
	// sapper does crushing damage), award an assist to the owner of the sapper since it probably contributed to
	...
	if(pScorer && pInflictor != this)
	{
		...
		if(pSapper)
		{
			...
			if(pSapperOwner)
			{
				EconEntity_OnOwnerKillEaterEvent(
					dynamic_cast<CEconEntity *>(pSapperOwner->GetEntityForLoadoutSlot(LOADOUT_POSITION_BUILDING)),
					pSapperOwner, GetOwner(), kKillEaterEvent_BuildingSapped);
			}
		}
		...
		{
			if(pScorer->GetActiveTFWeapon() &&
			   (pScorer->GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_GRENADELAUNCHER))
			{
	...
	if(IsUsingReverseBuild() && (DMG_CRUSH & info.GetDamageType()) != 0)
	{
		CTFAmmoPack *pAmmoPack =
			CreateAmmoPack("models/weapons/w_models/w_toolbox.mdl", GetObjectInfo(ObjectType())->m_iMetalToDropInGibs);
		if(pAmmoPack)
		{
	...
	EconEntity_OnOwnerKillEaterEvent(dynamic_cast<CEconEntity *>(info.GetWeapon()), pScorer, GetOwner(),
...
void CBaseObject::ChangeTeam(int iTeamNum)
{
	...
	TRACE_OBJECT(UTIL_VarArgs("%0.2f CBaseObject::ChangeTeam old %s new %s\n", gpGlobals->curtime,
							  pExisting ? pExisting->GetName() : "NULL", pTeam ? pTeam->GetName() : "NULL"));
...
void CBaseObject::InputRemoveHealth(inputdata_t &inputdata)
{
	...
	if(GetHealth() <= 0)
	{
		...
		CTakeDamageInfo info(inputdata.pCaller, inputdata.pActivator, vec3_origin, GetAbsOrigin(), iDamage,
							 DMG_GENERIC);
		Killed(info);
	}
}
...
bool CBaseObject::Command_Repair(CTFPlayer *pActivator, float flRepairMod)
{
	...
	TRACE_OBJECT(UTIL_VarArgs("%0.2f CBaseObject::Command_Repair ( %f / %d ) - cost = %d\n", gpGlobals->curtime,
							  GetHealth(), GetMaxHealth(), iRepairCost));
...
void CBaseObject::StartUpgrading(void)
{
	...
	if(TFGameRules() && TFGameRules()->IsInTraining() && TFGameRules()->GetTrainingModeLogic() && GetOwner() &&
	   GetOwner()->IsFakeClient() == false)
	{
...
void CBaseObject::SpawnObjectPoints(void)
{
	...
	for(pkvObjectPoint = pkvAllObjectPoints->GetFirstSubKey(); pkvObjectPoint;
		pkvObjectPoint = pkvObjectPoint->GetNextKey())
	{
		...
		if(iAttachmentNumber <= 0)
		{
			Msg("ERROR: Model %s specifies object point %s, but has no attachment named %s.\n", STRING(GetModelName()),
				pkvObjectPoint->GetString(), pkvObjectPoint->GetString());
			continue;
		}
...
void CBaseObject::SetSolidToPlayers(OBJSOLIDTYPE stp, bool force)
{
	...
	if(changed || force)
	{
		SetCollisionGroup(IsSolidToPlayers() ? TFCOLLISION_GROUP_OBJECT_SOLIDTOPLAYERMOVEMENT
...
int CBaseObject::DrawDebugTextOverlays(void)
{
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		Q_snprintf(tempstr, sizeof(tempstr), "Health: %f / %d ( %.1f )", GetHealth(), GetMaxHealth(),
		...
		Q_snprintf(tempstr, sizeof(tempstr), "Built by: (%d) %s", pBuilder ? pBuilder->entindex() : -1,
...
void CBaseObject::DoQuickBuild(bool bForceMax /* = false */)
{
	...
	int iTargetLevel = (((TFGameRules() && TFGameRules()->IsQuickBuildTime()) || bForceMax) ? OBJ_MAX_UPGRADE_LEVEL
																							: GetUpgradeLevel());
