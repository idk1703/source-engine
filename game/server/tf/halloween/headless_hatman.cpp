...
ConVar tf_halloween_bot_speed_recovery_rate("tf_halloween_bot_speed_recovery_rate", "100", FCVAR_CHEAT,
...
void CHeadlessHatman::Spawn(void)
{
	...
	{
		health +=
			(totalPlayers - tf_halloween_bot_min_player_count.GetInt()) * tf_halloween_bot_health_per_player.GetInt();
	}
			...
			if(!me->WasSpawnedByCheats())
			{
				...
				for(int i = 0; i < attackerVector.Count(); ++i)
				{
					   ...
					   gpGlobals->curtime - attackerVector[i].m_timestamp < deathTime)
					{
						...
						UTIL_SayText2Filter(filter, attackerVector[i].m_attacker, false, "#TF_Halloween_Boss_Killers",
											attackerVector[i].m_attacker->GetPlayerName());

						if(TFGameRules() &&
						   TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_MANN_MANOR))
						{
							// killing the boss with a melee weapon is a separate achievement
							if(attackerVector[i].m_wasLastHitFromMeleeWeapon)
							{
								attackerVector[i].m_attacker->AwardAchievement(
									ACHIEVEMENT_TF_HALLOWEEN_BOSS_KILL_MELEE);
							}
