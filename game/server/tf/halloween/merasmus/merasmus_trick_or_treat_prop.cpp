...
int CTFMerasmusTrickOrTreatProp::OnTakeDamage(const CTakeDamageInfo &info)
{
	...
	if(pTFPlayer && (pTFPlayer->IsPlayerClass(TF_CLASS_SOLDIER) || pTFPlayer->IsPlayerClass(TF_CLASS_DEMOMAN)) &&
	   (newinfo.GetDamageType() & DMG_BLAST))
	{
...
{
	CTFMerasmusTrickOrTreatProp *pTrickOrTreatProp = static_cast<CTFMerasmusTrickOrTreatProp *>(
		CBaseEntity::Create("tf_merasmus_trick_or_treat_prop", vPosition, qAngles, NULL));
...
void CTFMerasmusTrickOrTreatProp::SpawnTrickOrTreatItem()
{
	...
	for(int i = 0; i < nNumAmmo; ++i)
	{
		...
		CTFAmmoPack *pAmmoPack =
			CTFAmmoPack::Create(WorldSpaceCenter(), vec3_angle, this, "models/items/ammopack_medium.mdl");
		Assert(pAmmoPack);
		if(pAmmoPack)
		{
