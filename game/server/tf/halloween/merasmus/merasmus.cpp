...
ConVar tf_merasmus_min_props_to_reveal(
	"tf_merasmus_min_props_to_reveal", "0.7f", FCVAR_CHEAT,
...
ConVar tf_merasmus_health_regen_rate("tf_merasmus_health_regen_rate", "0.001f", FCVAR_CHEAT,
									 "Percentage of Max HP per sec that Merasmus will regenerate while in disguise");
...
static const char *s_pszDisguiseProps[] = {
	...
	"models/props_halloween/pumpkin_02.mdl", "models/props_halloween/pumpkin_03.mdl",
	"models/egypt/palm_tree/palm_tree.mdl", "models/props_spytech/control_room_console01.mdl",
	"models/props_spytech/work_table001.mdl",
	// "models/egypt/tent/tent.mdl",					// 248
	"models/props_coalmines/boulder1.mdl", "models/props_coalmines/boulder2.mdl",
	"models/props_farm/concrete_block001.mdl", // 152
											   // "models/props_farm/tractor_tire001.mdl",			// requires offset
	"models/props_farm/welding_machine01.mdl", "models/props_medieval/medieval_resupply.mdl",
	"models/props_medieval/target/target.mdl", "models/props_swamp/picnic_table.mdl",
	"models/props_manor/baby_grand_01.mdl", // 154
	"models/props_manor/bookcase_132_02.mdl", "models/props_manor/chair_01.mdl", "models/props_manor/couch_01.mdl",
	...
	"models/props_2fort/miningcrate001.mdl", "models/props_gameplay/resupply_locker.mdl",
	...
	"models/props_well/hand_truck01.mdl", "models/props_vehicles/mining_car_metal.mdl",
	"models/props_2fort/tire002.mdl", "models/props_well/computer_cart01.mdl", "models/egypt/palm_tree/palm_tree.mdl"};
...
SendPropBool(SENDINFO(m_bRevealed)), SendPropBool(SENDINFO(m_bIsDoingAOEAttack)), SendPropBool(SENDINFO(m_bStunned)),
END_SEND_TABLE
()
...
void CMerasmus::OnDisguise()
{
	...
	m_nDestroyedPropsToReveal = RandomInt(MAX(1, tf_merasmus_min_props_to_reveal.GetFloat() * m_fakePropVector.Count()),
										  m_fakePropVector.Count());
}
...
CTFWeaponBaseGrenadeProj *CMerasmus::CreateMerasmusGrenade(const Vector &vPosition, const Vector &vVelocity,
														   CBaseCombatCharacter *pOwner, float fScale)
{
	QAngle qAngles = RandomAngle(0, 360);
	CTFWeaponBaseMerasmusGrenade *pGrenade = static_cast<CTFWeaponBaseMerasmusGrenade *>(
		CBaseEntity::Create("tf_weaponbase_merasmus_grenade", vPosition, qAngles, pOwner));
	if(pGrenade)
	{
		pGrenade->SetModel(MERASMUS_BOMB_MODEL);
		DispatchSpawn(pGrenade);
		pGrenade->InitGrenade(vVelocity, AngularImpulse(600, random->RandomInt(-1200, 1200), 0), pOwner, 50 * fScale,
							  300.f * fScale);
		pGrenade->SetDetonateTimerLength(2.f);
		pGrenade->SetModelScale(fScale);
		pGrenade->SetCollisionGroup(
			TFCOLLISION_GROUP_ROCKETS); // we want to use collision_group_rockets so we don't ever collide with players
	}
	...
	for(int n = 0; n < nBombHeadPlayers; ++n)
	{
		...
		for(int i = 0; i < playerVector.Count(); ++i)
		{
			...
			if(pPlayer->GetTimeSinceWasBombHead() > oldestTimeStamp &&
			   !pPlayer->m_Shared.InCond(TF_COND_HALLOWEEN_BOMB_HEAD) && pPlayer->GetLastKnownArea() &&
...
void CMerasmus::StartRespawnTimer() const
{
	if(TFGameRules())
	{
		if(GetLevel() <= 3)
		{
			TFGameRules()->StartHalloweenBossTimer(tf_merasmus_spawn_interval.GetFloat(),
												   tf_merasmus_spawn_interval_variation.GetFloat());
		}
		else
		{
...
void CMerasmus::SW_ReportMerasmusStats(void)
{
	...
	msg.Body().set_scout_count(
		nClassCounts[TF_CLASS_SCOUT]); // Class and player break down at the point of boss despawn
	...
	//
	//	pKVData->SetInt( "LifeTime", (int)m_lifeTimer.GetElapsedTime() );			// Amount of time in seconds, boss was
	//alive for 	pKVData->SetInt( "BombKills", m_bossStats.m_nBombKills );					// Kills from Bombs
	//	pKVData->SetInt( "StaffKills", m_bossStats.m_nStaffKills );					// kills from staff account
	//	pKVData->SetInt( "PvPKills", m_bossStats.m_nPvpKills );						// Number of kills from players while Boss is
	//out (Jerk factor) 	pKVData->SetInt( "PropHuntTime1", m_bossStats.m_flPropHuntTime1 ); 	pKVData->SetInt(
	//"PropHuntTime2", m_bossStats.m_flPropHuntTime2 );
	//
	//	// Class Damage
	//	pKVData->SetInt( "DmgFromScout", m_bossStats.m_arrClassDamage[ TF_CLASS_SCOUT ] );			// Amount of damage done
	//by each class 	pKVData->SetInt( "DmgFromSniper", m_bossStats.m_arrClassDamage[ TF_CLASS_SNIPER ] );
	...
	//	}
	//
	//	pKVData->SetInt( "ScoutCount", nClassCounts[TF_CLASS_SCOUT] );				// Class and player break down at the
	//point of boss despawn 	pKVData->SetInt( "SniperCount", nClassCounts[TF_CLASS_SNIPER] ); 	pKVData->SetInt(
	//"SoldierCount", nClassCounts[TF_CLASS_SOLDIER] ); 	pKVData->SetInt( "DemoCount", nClassCounts[TF_CLASS_DEMOMAN] );
...
void CollectTargets(CBaseCombatCharacter *pCaster, float flSpellRange, int nTargetTeam, int nMaxTarget,
...
void CastSpell(CBaseCombatCharacter *pCaster, const char *pszCastingAttachmentName, float flSpellRange,
			   float flMinDamage, float flMaxDamage, CBaseEntity *pTarget)
{
	...
	TE_TFParticleEffectComplex(filter, 0.0f, "merasmus_zap", vStartPos, qStartAngles, NULL, &controlPoint, pCaster,
							   PATTACH_CUSTOMORIGIN);
}

/*static*/ bool CMerasmus::Zap(CBaseCombatCharacter *pCaster, const char *pszCastingAttachmentName, float flSpellRange,
							   float flMinDamage, float flMaxDamage, int nMaxTarget, int nTargetTeam /*= TEAM_ANY*/)
{
	CUtlVector<CHandle<CBaseEntity>> vecTargets;
		...
		if(!me->IsAlive())
		{
			if(!me->WasSpawnedByCheats())
			{
				...
				for(int i = 0; i < attackerVector.Count(); ++i)
				{
					...
					{
						CReliableBroadcastRecipientFilter filter;
						UTIL_SayText2Filter(filter, attackerVector[i].m_attacker, false,
											"#TF_Halloween_Merasmus_Killers",
											attackerVector[i].m_attacker->GetPlayerName());

						if(TFGameRules() &&
						   TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_LAKESIDE))
						{
				...
				if(GCClientSystem())
				{
					...
					GCSDK::CProtoBufMsg<CMsgUpdateHalloweenMerasmusLootLevel> msg(
