...
private:
	...
	// To save network perf, we don't create all bombs in a single tick. Rather, we fill up a queue of bombs and
	// distribute the creation over time. I originally had another property (start position) as part of
	// MerasmusGrenadeCreateSpec_t, but it didn't make sense, since we want to use his current position when we actually
	// create -- not the position he was at whenever we actually filled the queue with grenades. I'm leaving the struct
	// here, rather than making m_vecGrenadesToCreate a CUtlVector< Vector >, in case we want to add anything else.
	...
	void QueueSingleGrenadeForLaunch(const Vector &vecVelocity); // Don't call directly - call QueueBombRingsForLaunch()
																 // or QueueBombSpokesForLaunch()
