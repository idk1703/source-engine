	...
	if(me->GetVictim() && me->GetVictim()->IsAlive())
	{
		...
		if(me->GetVictim()->IsBaseObject())
		{
			CObjectSentrygun *sentry =
				dynamic_cast<CObjectSentrygun *>(const_cast<CBaseCombatCharacter *>(me->GetVictim()));
			if(sentry)
			{
	...
	if(m_initialDelayTimer.IsElapsed() && m_launchTimer.IsElapsed())
	{
		...
		if(me->IsEnraged() && me->GetVictim())
		{
			...
			if(rangeBetween > veryCloseRange)
			{
				...
				UTIL_TraceLine(me->WorldSpaceCenter(), me->GetVictim()->GetAbsOrigin() + leadOffset,
							   MASK_SOLID_BRUSHONLY, &filter, &result);
				...
				if(result.DidHit())
				{
					...
					{
						// rocket is going to hit an obstruction not near our victim - just aim right for them and hope
						// for splash
		...
		CTFProjectile_Rocket *pRocket =
			CTFProjectile_Rocket::Create(me, me->WorldSpaceCenter(), launchAngles, me, pScorer);
		if(pRocket)
		{
