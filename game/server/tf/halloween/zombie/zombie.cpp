...
void CZombie::Spawn(void)
{
	...
	// force kill oldest skeletons in the level (except skeleton king) to keep the number of skeletons under the max
	// active
...
//---------------------------------------------------------------------------------------------
/*static*/ CZombie *CZombie::SpawnAtPos(const Vector &vSpawnPos, float flLifeTime /*= 0.f*/,
										int nTeam /*= TF_TEAM_HALLOWEEN*/, CBaseEntity *pOwner /*= NULL*/,
										SkeletonType_t nSkeletonType /*= SKELETON_NORMAL*/)
{
...
#ifdef STAGING_ONLY
			iModelIndex =
				tf_halloween_skeleton_test_hat.GetInt() > 0 ? tf_halloween_skeleton_test_hat.GetInt() : iModelIndex;
#endif // STAGING_ONLY
