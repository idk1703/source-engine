...
ConVar tf_halloween_zombie_damage("tf_halloween_zombie_damage", "10", FCVAR_CHEAT,
								  "How much damage a zombie melee hit does.");
...
void CZombieAttack::SelectVictim(CZombie *me)
{
	...
	for(int i = 0; i < playerVector.Count(); ++i)
	{
		...
		{
			if(!pPlayer->m_Shared.InCond(TF_COND_BURNING) && !pPlayer->m_Shared.InCond(TF_COND_URINE) &&
			   !pPlayer->m_Shared.InCond(TF_COND_STEALTHED_BLINK) && !pPlayer->m_Shared.InCond(TF_COND_BLEEDING))
			{
...
EventDesiredResult<CZombie> CZombieAttack::OnContact(CZombie *me, CBaseEntity *other, CGameTrace *result)
{
	if(other->IsPlayer())
	{
		...
		if(pTFPlayer)
		{
			if(pTFPlayer->IsAlive() && me->GetTeamNumber() != TF_TEAM_HALLOWEEN &&
			   me->GetTeamNumber() != pTFPlayer->GetTeamNumber())
			{
...
//---------------------------------------------------------------------------------------------
EventDesiredResult<CZombie> CZombieAttack::OnOtherKilled(CZombie *me, CBaseCombatCharacter *victim,
