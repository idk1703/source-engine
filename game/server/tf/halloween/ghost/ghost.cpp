...
static ConCommand ghost_spawn("ghost_spawn", CC_GhostSpawn, "Spawns a Ghost where the player is looking.",
...
public:
	...
	void DriftAroundAndAvoidObstacles(CGhost *me)
	{
		...
		trace_t resultLeft;
		UTIL_TraceLine(me->WorldSpaceCenter(), me->WorldSpaceCenter() + feelerRange * (m_forward + left),
					   MASK_PLAYERSOLID, &traceFilter, &resultLeft);
		// NDebugOverlay::Line( me->WorldSpaceCenter(), me->WorldSpaceCenter() + feelerRange * ( m_forward + left ), 0,
		// 0, 255, true, interval );

		trace_t resultRight;
		UTIL_TraceLine(me->WorldSpaceCenter(), me->WorldSpaceCenter() + feelerRange * (m_forward + right),
					   MASK_PLAYERSOLID, &traceFilter, &resultRight);
		// NDebugOverlay::Line( me->WorldSpaceCenter(), me->WorldSpaceCenter() + feelerRange * ( m_forward + right ),
		// 255, 0, 0, true, interval );
