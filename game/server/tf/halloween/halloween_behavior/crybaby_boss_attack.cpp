	...
	if(m_axeSwingTimer.HasStarted())
	{
		...
		if(m_axeSwingTimer.IsElapsed())
		{
			...
			if(m_attackTarget != NULL)
			{
				...
				float closeness = (range < closeRange)
									  ? 0.0f
									  : (range - closeRange) / (tf_halloween_bot_attack_range.GetFloat() - closeRange);
				...
				if(DotProduct(forward, toVictim) > hitAngle)
				{
					if(me->IsRangeLessThan(m_attackTarget, 0.9f * tf_halloween_bot_attack_range.GetFloat()))
					{
						...
						{
							// CHOP!
							CTakeDamageInfo info(me, me, 2.0f * m_attackTarget->GetHealth(), DMG_CLUB,
...
//---------------------------------------------------------------------------------------------
EventDesiredResult<CHeadlessHatman> CCryBabyBossAttack::OnContact(CHeadlessHatman *me, CBaseEntity *other,
																  CGameTrace *result)
{
	if(other)
	{
