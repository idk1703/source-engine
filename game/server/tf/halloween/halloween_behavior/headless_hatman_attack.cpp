...
void CHeadlessHatmanAttack::AttackTarget(CHeadlessHatman *me, CBaseCombatCharacter *pTarget, float flAttackRange)
{
	...
	if(pPlayer &&
	   (pPlayer->m_Shared.InCond(TF_COND_HALLOWEEN_KART) || pPlayer->m_Shared.InCond(TF_COND_HALLOWEEN_GHOST_MODE)))
	{
		// Apply Huge Force
	...
	else
	{
		...
		if(DotProduct(forward, toVictim) > hitAngle)
		{
			...
			{
				// CHOP!
				CTakeDamageInfo info(me, me, m_attackTarget->GetMaxHealth() * 0.8f, DMG_CLUB,
...
ActionResult<CHeadlessHatman> CHeadlessHatmanAttack::Update(CHeadlessHatman *me, float interval)
{
	...
	else
	{
		...
		if(chaseVictim && m_warnITTimer.IsElapsed() &&
		   !TFGameRules()->IsHalloweenScenario(CTFGameRules::HALLOWEEN_SCENARIO_DOOMSDAY))
		{
...
//---------------------------------------------------------------------------------------------
EventDesiredResult<CHeadlessHatman> CHeadlessHatmanAttack::OnContact(CHeadlessHatman *me, CBaseEntity *other,
																	 CGameTrace *result)
{
	if(other)
	{
