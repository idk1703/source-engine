...
public:
	...
	struct PlayerMatchData_t
	{
		...
		// Override releasing this player from obligation to this match beyond the normal abandon rules.  Used by MvM
		// mode for marking everyone who completes a wave as allowed to drop without penalty, for instance.
		...
		{
			XPBonusPool_t() : m_flMultiplier(1.f), m_nBonusPoolRemaining(0) {}
	...
	void GiveXPDirectly(CSteamID steamID, CMsgTFXPSource::XPSourceType eType, int nAmount,
						bool bCanAwardBonusXP = true);
	// Give an XP bonus that increases
	void GiveXPBonus(CSteamID steamID, CMsgTFXPSource_XPSourceType eType, float flMultipler, int nBonusPool);
...
class CTFGCServerSystem : public CGCClientSystem,
						  public GCSDK::ISharedObjectListener,
						  public IServerGCLobby,
...
private:
	...
	// match.
	void AcceptGCReservation(CSteamID steamID, const CTFLobbyMember *pMemberData, bool bIsLateJoin, int nEntindex,
							 bool bActive);
