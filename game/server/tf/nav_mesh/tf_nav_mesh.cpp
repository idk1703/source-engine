...
ConVar tf_show_enemy_invasion_areas(
	"tf_show_enemy_invasion_areas", "0", FCVAR_CHEAT,
...
ConVar tf_bot_min_setup_gate_defend_range("tf_bot_min_setup_gate_defend_range", "750", FCVAR_CHEAT,
										  "How close from the setup gate(s) defending bots can take up positions. "
										  "Areas closer than this will be in cover to ambush.");
...
ConVar tf_bot_min_setup_gate_sniper_defend_range(
	"tf_bot_min_setup_gate_sniper_defend_range", "1500", FCVAR_CHEAT,
...
ConVar tf_select_ambush_areas_max_enemy_exposure_area("tf_select_ambush_areas_max_enemy_exposure_area", "500000",
													  FCVAR_CHEAT);
...
public:
	...
	bool operator()(CNavArea *baseArea)
	{
		...
		if(wallCount >= 1)
		{
			...
			const CUtlVector<CTFNavArea *> &invasionVector =
				area->GetEnemyInvasionAreaVector(GetEnemyTeam(m_teamToAmbush));
...
void CMD_SelectAmbushAreas(void)
{
	...
	ScanSelectAmbushAreas selector(&ambushAreaVector, teamToAmbush,
								   searchSourceArea->GetIncursionDistance(teamToAmbush) + 300.0f);
	SearchSurroundingAreas(searchSourceArea, searchSourceArea->GetCenter(), selector,
						   tf_select_ambush_areas_radius.GetFloat());
...
static ConCommand tf_select_ambush_areas("tf_select_ambush_areas", CMD_SelectAmbushAreas,
...
static ConCommand tf_select_incursion_zone("tf_select_incursion_zone", CMD_SelectIncursionZone,
										   "Select areas where invading team approaches the objective. For debugging.",
										   FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand tf_select_control_point_incursion_areas(
	"tf_select_control_point_incursion_areas", CMD_SelectControlPointIncursionAreas,
	"Select areas where invading team leaves cover near the objective. For debugging.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
CON_COMMAND_F(tf_assign_territory, "Divvy up the mesh into red and blue territories. For debugging.", FCVAR_GAMEDLL)
{
	...
	while(!CNavArea::IsOpenListEmpty())
	{
		...
		if(!area->HasAttributeTF(TF_NAV_BLUE_SETUP_GATE | TF_NAV_RED_SETUP_GATE) &&
		   (area->IsBlocked(TF_TEAM_RED) || area->IsBlocked(TF_TEAM_BLUE)))
		{
	...
	while(true)
	{
		...
		for(int i = 0; i < TheNavAreas.Count(); ++i)
		{
			...
			if(!area->HasAttributeTF(TF_NAV_RED_TERRITORY | TF_NAV_BLUE_TERRITORY) && parent &&
			   parent->HasAttributeTF(TF_NAV_RED_TERRITORY | TF_NAV_BLUE_TERRITORY))
			{
...
void CTFNavMesh::ComputeBlockedAreas(void)
{
	...
	{
		// if a closed door is not controlled by a trigger assume it doesn't open at all until the scenario changes and
		// map logic opens it
		...
		while((trigger = (CTriggerMultiple *)gEntList.FindEntityByClassname(trigger, "trigger_multiple")) != NULL)
		{
			...
			// just check overlapping, not encompassing, since some door triggers only are player height tall
			// (cp_gravelpit)
		...
		int blockedTeam = (doorOwnedByTeam == TEAM_UNASSIGNED)
							  ? TEAM_ANY
							  : ((doorOwnedByTeam == TF_TEAM_RED) ? TF_TEAM_BLUE : TF_TEAM_RED);
...
void CTFNavMesh::ComputeBombTargetDistance()
{
	...
	CTFNavArea *zoneArea =
		(CTFNavArea *)TheTFNavMesh()->GetNearestNavArea(zone->WorldSpaceCenter(), false, 500.0f, true);
	if(!zoneArea)
	{
			...
			FOR_EACH_VEC(ActiveSentries, oit)
			{
				...
				if((obj->GetAbsOrigin() - close).IsLengthLessThan(SENTRY_MAX_RANGE))
				{
					...
					if(area->IsPartiallyVisible(obj->GetAbsOrigin() + Vector(0, 0, 30.0f), obj))
					{
						...
						area->SetAttributeTF((obj->GetTeamNumber() == TF_TEAM_RED) ? TF_NAV_RED_SENTRY_DANGER
...
void CTFNavMesh::FireGameEvent(IGameEvent *event)
{
	...
	else if(eventName == "player_builtobject" || eventName == "player_carryobject" || eventName == "object_detonated" ||
...
void CTFNavMesh::ComputeIncursionDistances(void)
{
	...
	if(!isRedComputed)
	{
		Warning("Can't compute incursion distances from the Red spawn room(s). Bots will perform poorly. This is "
				"caused by either a missing func_respawnroom, or missing info_player_teamspawn entities within the "
				"func_respawnroom.\n");
	}

	if(!isBlueComputed)
	{
		Warning("Can't compute incursion distances from the Blue spawn room(s). Bots will perform poorly. This is "
				"caused by either a missing func_respawnroom, or missing info_player_teamspawn entities within the "
				"func_respawnroom.\n");
	}
	...
	if(!TFGameRules()->IsMannVsMachineMode())
	{
		...
		for(int i = 0; i < TheNavAreas.Count(); ++i)
		{
			...
			{
				area->m_distanceFromSpawnRoom[TF_TEAM_RED] =
					maxBlueIncursionDistance - area->GetIncursionDistance(TF_TEAM_BLUE);
			}
		}
	}
...
void CTFNavMesh::ComputeIncursionDistances(CTFNavArea *spawnArea, int team)
{
	...
	while(!CNavArea::IsOpenListEmpty())
	{
		...
		if(!bIgnoreBlockedAreas)
		{
			...
			// ignore setup gates, since they will be open after the setup time
			if(!area->HasAttributeTF(TF_NAV_SPAWN_ROOM_EXIT | TF_NAV_BLUE_SETUP_GATE | TF_NAV_RED_SETUP_GATE) &&
			   area->IsBlocked(team))
			{
				// don't pass through blocked areas
				continue;
...
void CTFNavMesh::DecorateMesh(void)
{
	...
	{
		CFuncRespawnRoom *respawnRoom =
			static_cast<CFuncRespawnRoom *>(IFuncRespawnRoomAutoList::AutoList()[iFuncRespawnRoom]);
		...
		for(int iTFTeamSpawn = 0; iTFTeamSpawn < ITFTeamSpawnAutoList::AutoList().Count(); ++iTFTeamSpawn)
		{
			...
			if(respawnRoom->PointIsWithin(spawnSpot->GetAbsOrigin()))
			{
				...
				collectAndLabel.Init(respawnRoom, spawnSpot->GetTeamNumber(),
									 spawnSpot->GetTeamNumber() == TF_TEAM_RED ? &m_redSpawnRoomAreaVector
																			   : &m_blueSpawnRoomAreaVector);
		...
		for(int dir = 0; dir < NUM_DIRECTIONS; ++dir)
		{
			...
			FOR_EACH_VEC((*adjVector), bit)
			{
				...
				if(adjFlow > baseFlow)
				{
					...
					NDebugOverlay::HorzArrow(area->GetCenter(), adjArea->GetCenter(), 5.0f, r, g, b, 255, true,
...
void CTFNavMesh::UpdateDebugDisplay(void) const
{
	...
	if(tf_show_enemy_invasion_areas.GetBool())
	{
		...
		if(myArea)
		{
			const CUtlVector<CTFNavArea *> &invasionAreaVector =
				myArea->GetEnemyInvasionAreaVector(player->GetTeamNumber());
	...
	if(tf_show_incursion_flow.GetInt() > 0 || tf_show_incursion_flow_gradient.GetInt() > 0)
	{
		...
		CTFNavArea *selectedArea =
			static_cast<CTFNavArea *>(TheNavMesh->GetNearestNavArea(result.endpos, false, 500.0f));
		...
		if(selectedArea)
		{
			if(tf_show_incursion_flow.GetInt() > 0)
			{
				...
				SearchSurroundingAreas(selectedArea, selectedArea->GetCenter(), draw,
									   tf_show_incursion_flow_range.GetFloat());
			}
		...
		if(areaVector)
		{
			for(i = 0; i < areaVector->Count(); ++i)
			{
				...
				if(!area->HasAttributeTF(TF_NAV_SPAWN_ROOM_EXIT))
				{
					...
					{
						NDebugOverlay::Text(area->GetCenter(), "Blue Spawn Room", false,
											NDEBUG_PERSIST_TILL_NEXT_SERVER);
					}
				}
			}
		...
		if(areaVector)
		{
			for(i = 0; i < areaVector->Count(); ++i)
			{
				...
				if(!area->HasAttributeTF(TF_NAV_SPAWN_ROOM_EXIT))
				{
					...
					if(TheNavMesh->GetSelectedArea() == area)
					{
						NDebugOverlay::Text(area->GetCenter(), "Red Spawn Room", false,
											NDEBUG_PERSIST_TILL_NEXT_SERVER);
					}
				}
			}
 ...
 */
void CTFNavMesh::CollectAmbushAreas(CUtlVector<CTFNavArea *> *ambushVector, CTFNavArea *startArea, int teamToAmbush,
									float searchRadius, float incursionTolerance) const
{
	ScanSelectAmbushAreas selector(ambushVector, teamToAmbush,
								   startArea->GetIncursionDistance(teamToAmbush) + incursionTolerance);
...
void CTFNavMesh::CollectAreaWithinBombTravelRange(CUtlVector<CTFNavArea *> *spawnExitAreaVector, float minTravel,
