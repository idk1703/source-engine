...
CSOTrackerManager::SOTrackerMap_t::KeyType_t CTFWarTrackerManager::GetKeyForObjectTracker(const CSharedObject *pItem,
...
CFmtStr CTFWarTrackerManager::GetDebugObjectDescription(const CSharedObject *pItem) const
{
	...
	return CFmtStr("%s: Side: %s Points: %d", pWarDef->GetDefName(), pSide->m_pszLocalizedName,
				   pWarData->Obj().points_scored());
}

CBaseSOTracker *CTFWarTrackerManager::AllocateNewTracker(const CSharedObject *pItem, CSteamID steamIDOwner,
...
int CTFWarTrackerManager::CompareRecords(const ::google::protobuf::Message *pNewProtoMsg,
										 const ::google::protobuf::Message *pExistingProtoMsg) const
{
	...
	const CGCMsgGC_War_IndividualUpdate *pExisting =
		assert_cast<const CGCMsgGC_War_IndividualUpdate *>(pExistingProtoMsg);
