...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD_MP_INTEGRAL),
	SendPropAngle(SENDINFO_VECTORELEM(m_vecAngles, 0), 7, 0), SendPropAngle(SENDINFO_VECTORELEM(m_vecAngles, 1), 7, 0),
	...
	SendPropFloat(SENDINFO(m_flSpread), 8, 0, 0.0f, 1.0f), SendPropBool(SENDINFO(m_bCritical)),
END_SEND_TABLE
()
...
SendPropFloat(SENDINFO_NOCHECK(m_vecOrigin[0]), -1, SPROP_COORD_MP_INTEGRAL),
	...
	SendPropInt(SENDINFO_NOCHECK(m_nDefID), -1), SendPropInt(SENDINFO_NOCHECK(m_nSound), -1),
...
()
	...
	void TE_TFExplosion(IRecipientFilter &filter, float flDelay, const Vector &vecOrigin, const Vector &vecNormal,
						int iWeaponID, int nEntIndex, int nDefID, int nSound, int iCustomParticleIndex)
{
...
void TE_TFExplosion(IRecipientFilter &filter, float flDelay, const Vector &vecOrigin, const Vector &vecNormal,
					int iWeaponID, int nEntIndex, int nDefID, int nSound)
{
...
SendPropFloat(SENDINFO_NOCHECK(m_vecOrigin[0]), -1, SPROP_COORD_MP_INTEGRAL),
	...
	SendPropInt(SENDINFO_NOCHECK(m_iAttachmentPointIndex), -1), SendPropBool(SENDINFO_NOCHECK(m_bResetParticles)),
...
{
	TE_TFParticleEffectComplex(filter, flDelay, pszParticleName, vecOrigin, vecAngles, NULL, NULL, pEntity,
							   eAttachType);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void TE_TFParticleEffectComplex(IRecipientFilter &filter, float flDelay, const char *pszParticleName, Vector vecOrigin,
								QAngle vecAngles, te_tf_particle_effects_colors_t *pOptionalColors /*= NULL*/,
