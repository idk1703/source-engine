...
int CAI_BaseNPC::TranslateSchedule(int scheduleType)
{
	...
	switch(scheduleType)
	{
		...
		{
			// FIXME: crows are set into IDLE_STAND as an failure schedule, not sure if ALERT_STAND or COMBAT_STAND is a
			// better choice Assert( m_NPCState == NPC_STATE_IDLE );
		}
		break;
...
//-----------------------------------------------------------------------------
AI_DEFINE_SCHEDULE(SCHED_SHOOT_ENEMY_COVER,
