...
static ConCommand ai_disable("ai_disable", CC_AI_Disable,
							 "Bi-passes all AI logic routines and puts all NPCs into their idle animations.  Can be "
							 "used to get NPCs out of your way and to test effect of AI logic routines on frame rate",
							 FCVAR_CHEAT);
...
static ConCommand ai_show_hints(
	"ai_show_hints", CC_AI_ShowHints,
	"Displays all hints as small boxes\n\tBlue		- hint is available for use\n\tRed		- hint is currently being "
...
static ConCommand ai_show_hull(
	"ai_show_hull", CC_AI_ShowHull,
	"Displays the allowed hulls between each node for the currently selected hull type.  Hulls are color code as "
...
static ConCommand ai_show_connect(
	"ai_show_connect", CC_AI_ShowConnect,
...
static ConCommand ai_show_connect_jump(
	"ai_show_connect_jump", CC_AI_ShowJumpConnect,
	"Displays the allowed connections between each node for the currently selected hull type.  Hulls are color code as "
	"follows:\n\tGreen		- ground movement \n\tBlue		- jumping movement\n\tCyan		- flying "
	"movement\n\tMagenta	- climbing movement\n\tRed		- connection disabled",
	FCVAR_CHEAT);
...
static ConCommand ai_show_connect_fly(
	"ai_show_connect_fly", CC_AI_ShowFlyConnect,
	"Displays the allowed connections between each node for the currently selected hull type.  Hulls are color code as "
	"follows:\n\tGreen		- ground movement \n\tBlue		- jumping movement\n\tCyan		- flying "
	"movement\n\tMagenta	- climbing movement\n\tRed		- connection disabled",
	FCVAR_CHEAT);
...
static ConCommand ai_step("ai_step", CC_AI_Step,
						  "NPCs will freeze after completing their current task.  To complete the next task, use "
...
static ConCommand ai_resume("ai_resume", CC_AI_Resume,
...
static ConCommand ai_next_hull(
	"ai_next_hull", CC_AI_NextHull,
	"Cycles through the various hull sizes.  Currently selected hull size is written to the screen.  Controls which "
	"connections are shown when ai_show_hull or ai_show_connect commands are used\n\tArguments:	-none-",
	FCVAR_CHEAT);
...
static ConCommand ai_hull("ai_hull", CC_AI_Hull,
						  "Controls which connections are shown when ai_show_hull or ai_show_connect commands are "
...
static ConCommand ai_nodes(
	"ai_nodes", CC_AI_Nodes,
	"Toggles node display.  First call displays the nodes for the given network as green objects.  Second call  "
	"displays the nodes and their IDs.  Nodes are color coded as follows:\n\tGreen		- ground node\n\tCyan		- "
...
static ConCommand ai_show_visibility(
	"ai_show_visibility", CC_AI_ShowVisibility,
	"Toggles visibility display for the node that the player is looking at.  Nodes that are visible from the selected "
	"node will be drawn in red with yellow lines connecting to the selected node.  Nodes that are not visible from the "
	"selected node will be drawn in blue.",
	FCVAR_CHEAT);
...
static ConCommand ai_show_graph_connect(
	"ai_show_graph_connect", CC_AI_GraphConnect,
	"Toggles graph connection display for the node that the player is looking at.  Nodes that are connected to the "
	"selected node by the net graph will be drawn in red with magenta lines connecting to the selected node.  Nodes "
	"that are not connected via the net graph from the selected node will be drawn in blue.",
	FCVAR_CHEAT);
...
static ConCommand npc_bipass(
	"npc_bipass", CC_NPC_Bipass,
	"Displays the local movement attempts by the given NPC(s) (triangulation detours).  Failed bypass routes are "
	"displayed in red, successful bypasses are shown in green.\n\tArguments:   	{entity_name} / {class_name} / no "
...
static ConCommand npc_destroy("npc_destroy", CC_NPC_Destroy,
...
static ConCommand npc_kill("npc_kill", CC_NPC_Kill,
						   "Kills the given NPC(s)\nArguments:   	{npc_name} / {npc_class_name} / no argument picks "
...
static ConCommand npc_enemies(
	"npc_enemies", CC_NPC_Enemies,
	"Shows memory of NPC.  Draws an X on top of each memory.\n\tEluded entities drawn in blue (don't know where it "
	"went)\n\tUnreachable entities drawn in green (can't get to it)\n\tCurrent enemy drawn in red\n\tCurrent target "
	"entity drawn in magenta\n\tAll other entities drawn in pink\n\tArguments:   	{npc_name} / {npc class_name} / no "
	"argument picks what player is looking at",
	FCVAR_CHEAT);
...
static ConCommand npc_focus(
	"npc_focus", CC_NPC_Focus,
...
void CC_NPC_Create(const CCommand &args)
{
	...
	if(baseNPC)
	{
		...
		AI_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH, MASK_NPCSOLID,
		...
		if(tr.fraction != 1.0)
		{
			...
			AI_TraceHull(baseNPC->GetAbsOrigin(), vUpBit, baseNPC->GetHullMins(), baseNPC->GetHullMaxs(), MASK_NPCSOLID,
						 baseNPC, COLLISION_GROUP_NONE, &tr);
			if(tr.startsolid || (tr.fraction < 1.0))
			{
				baseNPC->SUB_Remove();
				DevMsg("Can't create %s.  Bad Position!\n", args[1]);
				NDebugOverlay::Box(baseNPC->GetAbsOrigin(), baseNPC->GetHullMins(), baseNPC->GetHullMaxs(), 255, 0, 0,
								   0, 0);
			}
		}
...
static ConCommand npc_create(
	"npc_create", CC_NPC_Create,
	"Creates an NPC of the given type where the player is looking (if the given NPC can actually stand at that "
	"location).  Note that this only works for npc classes that are already in the world.  You can not create an "
...
static ConCommand npc_create_aimed(
	"npc_create_aimed", CC_NPC_Create_Aimed,
	"Creates an NPC aimed away from the player of the given type where the player is looking (if the given NPC can "
	"actually stand at that location).  Note that this only works for npc classes that are already in the world.  You "
...
static ConCommand npc_destroy_unselected("npc_destroy_unselected", CC_NPC_DestroyUnselected,
...
static ConCommand npc_freeze("npc_freeze", CC_NPC_Freeze,
							 "Selected NPC(s) will freeze in place (or unfreeze). If there are no selected NPCs, uses "
							 "the NPC under the crosshair.\n\tArguments:	-none-",
							 FCVAR_CHEAT);
...
void CC_NPC_Teleport(void)
{
	...
	Vector forward;
	pPlayer->EyeVectors(&forward);
	AI_TraceLine(pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH, MASK_NPCSOLID, pPlayer,
...
static ConCommand npc_teleport("npc_teleport", CC_NPC_Teleport,
							   "Selected NPC will teleport to the location that the player is looking (shown with a "
							   "purple box)\n\tArguments:	-none-",
							   FCVAR_CHEAT);
...
static ConCommand npc_go_random("npc_go_random", CC_NPC_GoRandom,
...
static ConCommand npc_reset("npc_reset", CC_NPC_Reset,
...
static ConCommand npc_nearest("npc_nearest", CC_NPC_Nearest,
...
static ConCommand npc_route(
	"npc_route", CC_NPC_Route,
	"Displays the current route of the given NPC as a line on the screen.  Waypoints along the route are drawn as "
	"small cyan rectangles.  Line is color coded in the following manner:\n\tBlue	- path to a node\n\tCyan	- "
...
static ConCommand npc_select(
	"npc_select", CC_NPC_Select,
...
static ConCommand npc_combat(
	"npc_combat", CC_NPC_Combat,
	"Displays text debugging information about the squad and enemy of the selected NPC  (See Overlay "
...
static ConCommand npc_tasks(
	"npc_tasks", CC_NPC_Tasks,
...
static ConCommand npc_task_text(
	"npc_task_text", CC_NPC_Task_Text,
...
static ConCommand npc_conditions(
	"npc_conditions", CC_NPC_Conditions,
	"Displays all the current AI conditions that an NPC has in the overlay text.\n\tArguments:   	{npc_name} / {npc "
	"class_name} / no argument picks what player is looking at",
	FCVAR_CHEAT);
...
static ConCommand npc_viewcone(
	"npc_viewcone", CC_NPC_Viewcone,
	"Displays the viewcone of the NPC (where they are currently looking and what the extents of there vision "
	"is)\n\tArguments:   	{entity_name} / {class_name} / no argument picks what player is looking at",
	FCVAR_CHEAT);
...
static ConCommand npc_relationships("npc_relationships", CC_NPC_Relationships,
...
static ConCommand npc_steering(
	"npc_steering", CC_NPC_ViewSteeringRegulations,
...
static ConCommand npc_steering_all("npc_steering_all", CC_NPC_ViewSteeringRegulationsAll,
...
CON_COMMAND(ai_test_los, "Test AI LOS from the player's POV")
{
	...
	UTIL_TraceLine(UTIL_GetLocalPlayer()->EyePosition(),
				   UTIL_GetLocalPlayer()->EyePosition() + UTIL_GetLocalPlayer()->EyeDirection3D() * MAX_COORD_RANGE,
				   MASK_BLOCKLOS_AND_NPCS, &traceFilter, &tr);
...
{
	g_VProfCurrentProfile.OutputReport(VPRT_FULL, "AINet",
									   g_VProfCurrentProfile.BudgetGroupNameToBudgetGroupID("AINet"));
}
