...
BEGIN_DATADESC(CRadarTarget)
	DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"), DEFINE_KEYFIELD(m_flRadius, FIELD_FLOAT, "radius"),
		DEFINE_KEYFIELD(m_iType, FIELD_INTEGER, "type"), DEFINE_KEYFIELD(m_iMode, FIELD_INTEGER, "mode"),
...
public:
	...
	static CVehicleCargoTrigger *Create(const Vector &vecOrigin, const Vector &vecMins, const Vector &vecMaxs,
										CBaseEntity *pOwner)
	{
	...
	void CargoTouch(CBaseEntity *pOther)
	{
		...
		if(pOther->VPhysicsGetObject() == NULL ||
		   (pOther->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD) == false)
			return;
...
BEGIN_DATADESC(CPropJeepEpisodic)
	...
	DEFINE_FIELD(m_bEntranceLocked, FIELD_BOOLEAN), DEFINE_FIELD(m_bExitLocked, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hCargoProp, FIELD_EHANDLE), DEFINE_FIELD(m_hCargoTrigger, FIELD_EHANDLE),
		DEFINE_FIELD(m_bAddingCargo, FIELD_BOOLEAN), DEFINE_ARRAY(m_hWheelDust, FIELD_EHANDLE, NUM_WHEEL_EFFECTS),
		DEFINE_ARRAY(m_hWheelWater, FIELD_EHANDLE, NUM_WHEEL_EFFECTS),
		DEFINE_ARRAY(m_hHazardLights, FIELD_EHANDLE, NUM_HAZARD_LIGHTS), DEFINE_FIELD(m_flCargoStartTime, FIELD_TIME),
		DEFINE_FIELD(m_bBlink, FIELD_BOOLEAN), DEFINE_FIELD(m_bRadarEnabled, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bRadarDetectsEnemies, FIELD_BOOLEAN), DEFINE_FIELD(m_hRadarScreen, FIELD_EHANDLE),
		DEFINE_FIELD(m_hLinkControllerFront, FIELD_EHANDLE), DEFINE_FIELD(m_hLinkControllerRear, FIELD_EHANDLE),
		DEFINE_KEYFIELD(m_bBusterHopperVisible, FIELD_BOOLEAN, "CargoVisible"),
		// m_flNextAvoidBroadcastTime
		DEFINE_FIELD(m_flNextWaterSound, FIELD_TIME), DEFINE_FIELD(m_flNextRadarUpdateTime, FIELD_TIME),
...
()
	...
	: m_bEntranceLocked(false), m_bExitLocked(false), m_bAddingCargo(false), m_flNextAvoidBroadcastTime(0.0f)
{
	m_bHasGun = false;
...
void CPropJeepEpisodic::UpdateRadar(bool forceUpdate)
{
	...
	while(pEnt != NULL)
	{
		...
		if(pEnt->m_iClassname == iszRadarTarget)
		{
			...
			{
				if(pTarget->m_flRadius < 0 ||
				   vecJalopyOrigin.DistToSqr(pTarget->GetAbsOrigin()) <= Square(pTarget->m_flRadius))
				{
...
void CPropJeepEpisodic::UpdateCargoEntry(void)
{
	...
	float flPerc = (jalopy_cargo_anim_time.GetFloat())
					   ? ((gpGlobals->curtime - m_flCargoStartTime) / jalopy_cargo_anim_time.GetFloat())
					   : 1.0f;
...
CBaseEntity *CPropJeepEpisodic::OnFailedPhysGunPickup(Vector vPhysgunPos)
{
	...
	if(m_hCargoProp != NULL)
	{
		...
		// We need to make sure the player's position is within a cone near the opening and that they're also facing the
		// right way
...
static void SolveBlockingProps(CPropJeepEpisodic *pVehicleEntity, IPhysicsObject *pVehiclePhysics)
{
	...
	while(pSnapshot->IsValid())
	{
		...
		Assert(pOtherEntity);
		if(pOtherEntity && pOtherEntity->GetMoveType() == MOVETYPE_VPHYSICS && pOther->IsMoveable() &&
		   (otherMass * 4.0f) < vehicleMass)
		{
			Vector normal;
			...
			if(DotProduct(normal, vehicleUp) < -0.9f)
			{
				...
				Vector bottomPoint = physcollision->CollideGetExtent(pVehiclePhysics->GetCollide(), vec3_origin,
																	 vec3_angle, Vector(0, 0, -1));
...
static void SimpleCollisionResponse(Vector velocityIn, const Vector &normal, float coefficientOfRestitution,
									Vector *pVelocityOut)
{
...
static void KillBlockingEnemyNPCs(CBasePlayer *pPlayer, CBaseEntity *pVehicleEntity, IPhysicsObject *pVehiclePhysics)
{
	...
	while(pSnapshot->IsValid())
	{
		...
		if(pNPC && pPlayer->IRelationType(pNPC) != D_LI && ((otherMass * 2.0f) < vehicleMass))
		{
			...
			if(index < 0)
			{
				...
				if(pNPC->VPhysicsGetObject() && pNPC->VPhysicsGetObject()->GetShadowController() &&
				   pNPC->GetMoveType() == MOVETYPE_STEP)
				{
					...
					float maxAngular;
					pNPC->VPhysicsGetObject()->GetShadowController()->GetMaxSpeed(
	...
	if(npcList.Count())
	{
		for(int i = npcList.Count(); --i >= 0;)
		{
			...
			CTakeDamageInfo dmgInfo(pVehicleEntity, pVehicleEntity, damageForce, contactList[i], 200.0f,
									DMG_CRUSH | DMG_VEHICLE);
			npcList[i]->TakeDamage(dmgInfo);
			pVehiclePhysics->ApplyForceOffset(vehicleForce, contactList[i]);
			PhysCollisionSound(pVehicleEntity, npcList[i]->VPhysicsGetObject(), CHAN_BODY,
							   pVehiclePhysics->GetMaterialIndex(), npcList[i]->VPhysicsGetObject()->GetMaterialIndex(),
							   gpGlobals->frametime, 200.0f);
		}
	}
}
...
void CPropJeepEpisodic::InputOutsideTransition(inputdata_t &inputdata)
{
	...
	while((pEntity = (CInfoTargetVehicleTransition *)gEntList.FindEntityByClassname(
			   pEntity, "info_target_vehicle_transition")) != NULL)
	{
		// Must be enabled
		...
		trace_t tr;
		UTIL_TraceHull(vecTeleportPos, vecTeleportPos - Vector(0, 0, 128), vecMins, vecMaxs, MASK_SOLID, this,
...
void CPropJeepEpisodic::InputCreateLinkController(inputdata_t &data)
{
	...
	CAI_RadialLinkController *pLinkController =
...
void CPropJeepEpisodic::InputDestroyLinkController(inputdata_t &data)
{
	...
	{
		CAI_RadialLinkController *pLinkController =
			dynamic_cast<CAI_RadialLinkController *>(m_hLinkControllerFront.Get());
	...
	{
		CAI_RadialLinkController *pLinkController =
			dynamic_cast<CAI_RadialLinkController *>(m_hLinkControllerRear.Get());
		if(pLinkController != NULL)
		{
			pLinkController->ModifyNodeLinks(false);
			UTIL_Remove(pLinkController);
