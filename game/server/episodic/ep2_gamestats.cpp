...
static char const *ep2Maps[] = {"ep2_outland_01", "ep2_outland_02",	 "ep2_outland_03",	"ep2_outland_04",
...
void CEP2GameStats::Event_PlayerKilled(CBasePlayer *pPlayer, const CTakeDamageInfo &info)
{
	...
	StatsLog("CEP2GameStats::Event_PlayerKilled at location [%d %d %d]\n", (int)death.nPosition[0],
			 (int)death.nPosition[1], (int)death.nPosition[2]);
...
void CEP2GameStats::Event_PlayerTraveled(CBasePlayer *pBasePlayer, float distanceInInches, bool bInVehicle,
										 bool bSprinting)
{
...
void CEP2GameStats::Event_WeaponHit(CBasePlayer *pShooter, bool bPrimary, char const *pchWeaponName,
...
void CEP2GameStats::Event_SaveGame(void)
{
	...
	if(pPlayer)
	{
		...
		rec->m_SaveType = Q_stristr(pchSaveFile, "autosave") ? Ep2LevelStats_t::SaveGameInfoRecord2_t::TYPE_AUTOSAVE
															 : Ep2LevelStats_t::SaveGameInfoRecord2_t::TYPE_USERSAVE;

		StatsLog("save pos %i %i %i w/ health %d\n", rec->m_nSavePos[0], rec->m_nSavePos[1], rec->m_nSavePos[2],
				 rec->m_nSaveHealth);
	}
}
...
void CEP2GameStats::Event_LoadGame(void)
{
	...
	if(pSaveGameInfo->m_nCurrentSaveFileTime == 0 || pSaveGameInfo->m_sCurrentSaveFile != name)
	{
		unsigned int uFileTime = filesystem->GetFileTime(name, "GAME");
...
void CEP2GameStats::Event_IncrementCountedStatistic(const Vector &vecAbsOrigin, char const *pchStatisticName,
...
static void CC_ListDeaths(const CCommand &args)
{
	...
	{
		Vector org(map->m_aPlayerDeaths[i].nPosition[0], map->m_aPlayerDeaths[i].nPosition[1],
