...
BEGIN_DATADESC(CAI_PassengerBehaviorCompanion)
	...
	DEFINE_EMBEDDED(m_VehicleMonitor),
		...
		DEFINE_FIELD(m_flOverturnedDuration, FIELD_FLOAT), DEFINE_FIELD(m_flUnseenDuration, FIELD_FLOAT),
		DEFINE_FIELD(m_nExitAttempts, FIELD_INTEGER), DEFINE_FIELD(m_flNextOverturnWarning, FIELD_TIME),
		DEFINE_FIELD(m_flEnterBeginTime, FIELD_TIME), DEFINE_FIELD(m_hCompanion, FIELD_EHANDLE),
		DEFINE_FIELD(m_flNextJostleTime, FIELD_TIME), DEFINE_FIELD(m_nVisibleEnemies, FIELD_INTEGER),
		DEFINE_FIELD(m_flLastLateralLean, FIELD_FLOAT), DEFINE_FIELD(m_flEntraceUpdateTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextEnterAttempt, FIELD_TIME), DEFINE_FIELD(m_flNextFidgetTime, FIELD_TIME),

END_DATADESC();

BEGIN_SIMPLE_DATADESC(FailPosition_t)

	DEFINE_FIELD(vecPosition, FIELD_VECTOR), DEFINE_FIELD(flTime, FIELD_TIME),
...
void CAI_PassengerBehaviorCompanion::GatherVehicleCollisionConditions(const Vector &localVelocity)
{
	...
	if(localVelocity.y > passenger_collision_response_threshold.GetFloat())
	{
		...
		CTraceFilterNoNPCsOrPlayer filter(
			m_hVehicle,
			COLLISION_GROUP_NONE); // We don't care about NPCs or the player (certainly if they're in the vehicle!)

		trace_t tr;
		UTIL_TraceHull(m_hVehicle->GetAbsOrigin(), m_hVehicle->GetAbsOrigin() + (vForward * distance), mins, maxs,
...
bool CAI_PassengerBehaviorCompanion::CanPlayJostle(bool bLargeJostle)
{
	...
	if(bLargeJostle)
	{
		...
		if(IsCurSchedule(SCHED_PASSENGER_COWER) || IsCurSchedule(SCHED_PASSENGER_FIDGET))
			return false;
	}
	else
	{
		// Don't interrupt a larger gesture
		if(GetOuter()->IsPlayingGesture(ACT_PASSENGER_GESTURE_JOSTLE_LARGE) ||
		   GetOuter()->IsPlayingGesture(ACT_PASSENGER_GESTURE_JOSTLE_LARGE_STIMULATED))
			return false;
	}
	...
	if(GetPassengerState() == PASSENGER_STATE_INSIDE)
	{
		...
		if(HasCondition(COND_PASSENGER_ERRATIC_DRIVING))
		{
			if(CanPlayJostle(true))
			{
				...
				int nSequence =
					GetOuter()->AddGesture(GetOuter()->NPC_TranslateActivity(ACT_PASSENGER_GESTURE_JOSTLE_LARGE), true);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehaviorCompanion::CanEnterVehicleImmediately(int *pResultSequence, Vector *pResultPos,
																QAngle *pResultAngles)
{
	...
	const PassengerSeatAnims_t *pEntryAnims =
		m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatAnims(GetOuter(), PASSENGER_SEAT_ENTRY);
	if(pEntryAnims == NULL)
		return -1;
...
// Purpose:
//-----------------------------------------------------------------------------
int CAI_PassengerBehaviorCompanion::SelectFailSchedule(int failedSchedule, int failedTask,
													   AI_TaskFailureCode_t taskFailCode)
{
	switch(failedTask)
	{
		case TASK_GET_PATH_TO_VEHICLE_ENTRY_POINT:
		{
			...
			if(UTIL_DistApprox(m_hVehicle->GetAbsOrigin(), GetOuter()->GetAbsOrigin()) >
			   PASSENGER_NEAR_VEHICLE_THRESHOLD)
...
bool CAI_PassengerBehaviorCompanion::FindPathToVehicleEntryPoint(void)
{
	...
	if(UseRadialRouteToEntryPoint(vecEntryPoint))
	{
		...
		if(GetOuter()->GetNavigator()->SetRadialGoal(vecEntryPoint, vecCenterPoint, flRadius, flArc, 64.0f,
													 bClockwise) == false)
		{
...
bool CAI_PassengerBehaviorCompanion::CanExitAtPosition(const Vector &vecTestPos)
{
	...
	*/
	...
	// These are very approximated (and magical) numbers to allow passengers greater head room and leg room when
	transitioning Vector vecMins = GetOuter()->GetHullMins() + Vector( 0, 0, GetOuter()->StepHeight()*2.0f ); // FIXME:
	...
	if ( tr.fraction < 1.0f || tr.allsolid || tr.startsolid )
	{
		...
		{
			NDebugOverlay::SweptBox( GetOuter()->GetAbsOrigin(), vecGroundPos, vecMins, GetOuter()->GetHullMaxs(),
...
void CAI_PassengerBehaviorCompanion::StartTask(const Task_t *pTask)
{
	...
	if(GetPassengerState() == PASSENGER_STATE_INSIDE)
	{
		if(pTask->iTask == TASK_FACE_TARGET || pTask->iTask == TASK_FACE_ENEMY || pTask->iTask == TASK_FACE_IDEAL ||
		   pTask->iTask == TASK_FACE_HINTNODE || pTask->iTask == TASK_FACE_LASTPOSITION ||
		   pTask->iTask == TASK_FACE_PATH || pTask->iTask == TASK_FACE_PLAYER || pTask->iTask == TASK_FACE_REASONABLE ||
		   pTask->iTask == TASK_FACE_SAVEPOSITION || pTask->iTask == TASK_FACE_SCRIPT)
		{
			return TaskComplete();
		}
	}
	...
	switch(pTask->iTask)
	{
		...
		case TASK_GET_PATH_TO_NEAR_VEHICLE:
		{
			...
			AI_NavGoal_t goal(GOALTYPE_TARGETENT, vecTargetOffset, AIN_DEF_ACTIVITY, 64.0f, AIN_UPDATE_TARGET_POS,
							  m_hVehicle);
			GetOuter()->SetTarget(m_hVehicle);
			if(GetOuter()->GetNavigator()->SetGoal(goal))
			{
...
void CAI_PassengerBehaviorCompanion::BuildScheduleTestBits(void)
{
	...
	if(GetPassengerState() == PASSENGER_STATE_INSIDE)
	{
		GetOuter()->SetCustomInterruptCondition(
			GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_CAN_LEAVE_STUCK_VEHICLE));
		GetOuter()->SetCustomInterruptCondition(
			GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_HARD_IMPACT));

		if(IsCurSchedule(SCHED_PASSENGER_OVERTURNED) == false)
		{
			GetOuter()->SetCustomInterruptCondition(
				GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_OVERTURNED));
		}
		...
		{
			GetOuter()->SetCustomInterruptCondition(
				GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_CAN_FIDGET));
		}
		...
		{
			GetOuter()->SetCustomInterruptCondition(
				GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_PASSENGER_VEHICLE_STOPPED));
		}
	}
...
//-----------------------------------------------------------------------------
float CAI_PassengerBehaviorCompanion::GetArcToEntryPoint(const Vector &vecCenterPoint, const Vector &vecEntryPoint,
														 bool &bClockwise)
{
...
bool CAI_PassengerBehaviorCompanion::PointIsWithinEntryFailureRadius(const Vector &vecPosition)
{
	...
	for(int i = 0; i < m_FailedEntryPositions.Count(); i++)
	{
		...
		if((gpGlobals->curtime - m_FailedEntryPositions[i].flTime) > 3.0f)
		{
			...
			if(passenger_debug_entry.GetBool())
			{
				NDebugOverlay::Box(m_FailedEntryPositions[i].vecPosition, -Vector(12, 12, 12), Vector(12, 12, 12), 255,
								   255, 0, 0, 2.0f);
			}
...
bool CAI_PassengerBehaviorCompanion::CanFidget(void)
{
	...
	if(GetOuter()->IsInAScript() || GetOuter()->GetIdealState() == NPC_STATE_SCRIPT ||
	   IsRunningScriptedScene(GetOuter()))
		return false;
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_PassengerBehaviorCompanion)
	{
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_RUN_TO_ENTER_VEHICLE,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_RUN_TO_ENTER_VEHICLE_FAILED,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_ENTER_VEHICLE_PAUSE,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_RANGE_ATTACK1,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_EXIT_STUCK_VEHICLE,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_RELOAD,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_OVERTURNED,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_IMPACT,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_ENTER_VEHICLE_IMMEDIATELY,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_COWER,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_FIDGET,
