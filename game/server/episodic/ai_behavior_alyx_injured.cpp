...
int CAI_BehaviorAlyxInjured::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	...
	switch(failedTask)
	{
		...
		case TASK_FIND_INJURED_COVER_FROM_ENEMY:
			...
			float flDistToFollowTargetSqr =
				(GetOuter()->GetAbsOrigin() - GetFollowTarget()->GetAbsOrigin()).LengthSqr();
...
bool CAI_BehaviorAlyxInjured::FindThreatDirection2D(const Vector &vecSource, Vector *vecOut)
{
	...
	for(AI_EnemyInfo_t *pMemory = GetOuter()->GetEnemies()->GetFirst(&iter); pMemory != NULL;
		pMemory = GetOuter()->GetEnemies()->GetNext(&iter))
	{
...
bool CAI_BehaviorAlyxInjured::FindCoverFromEnemyBehindTarget(CBaseEntity *pTarget, float flRadius, Vector *vecOut)
{
	...
	if(FindThreatDirection2D(vecTargetPos, &vecThreatDir))
	{
		...
		GetOuter()->GetMoveProbe()->MoveLimit(NAV_GROUND, GetOuter()->GetAbsOrigin(), vecTestPos, MASK_SOLID_BRUSHONLY,
											  NULL, 0, &moveTrace);

		bool bWithinRangeToGoal =
			(moveTrace.vEndPosition - vecTestPos).Length2DSqr() < Square(GetOuter()->GetHullWidth() * 3.0f);
		bool bCanStandAtGoal =
		...
		if(bWithinRangeToGoal == false || bCanStandAtGoal == false)
		{
			if(g_debug_injured_follow.GetBool())
			{
				NDebugOverlay::SweptBox(GetOuter()->GetAbsOrigin(), vecTestPos, GetOuter()->GetHullMins(),
										GetOuter()->GetHullMaxs(), vec3_angle, 255, 0, 0, 0, 2.0f);
			}
		...
		*vecOut = moveTrace.vEndPosition;
		...
		if(g_debug_injured_follow.GetBool())
		{
			NDebugOverlay::SweptBox(GetOuter()->GetAbsOrigin(), (*vecOut), GetOuter()->GetHullMins(),
									GetOuter()->GetHullMaxs(), vec3_angle, 0, 255, 0, 0, 2.0f);
		}
...
void CAI_BehaviorAlyxInjured::GatherConditions(void)
{
	...
	bool bInDanger =
		(HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE) || HasCondition(COND_INJURED_OVERWHELMED));
	...
	if(AI_IsSinglePlayer() && bInDanger)
	{
		...
		if(pPlayer != NULL)
		{
			// FIXME: This distance may need to be the length of the shortest walked path between the follower and the
			// target
			...
			else if(flDistToPlayer > (injured_help_plee_range.GetFloat() * 0.5f) &&
					HasCondition(COND_SEE_PLAYER) == false)
			{
		...
		if(bWarnPlayer)
		{
			// FIXME: This should be routed through the normal speaking code with a system to emit from the player's
			// suit.
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_BehaviorAlyxInjured)
	{
		...
		DEFINE_SCHEDULE(SCHED_INJURED_COWER,
		...
		DEFINE_SCHEDULE(SCHED_INJURED_FEAR_FACE,
		...
		DEFINE_SCHEDULE(SCHED_INJURED_RUN_FROM_ENEMY,
