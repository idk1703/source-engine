...
ConVar hunter_jostle_car_min_speed("hunter_jostle_car_min_speed",
...
ConVar hunter_retreat_striderbusters("hunter_retreat_striderbusters", "1", FCVAR_NONE,
...
#define HUNTER_SHOOT_MAX_YAW_DEG \
	60.0f							  // Once shooting, clamp to +/- these degrees of yaw deflection as our target moves
...
bool IsStriderBuster(CBaseEntity *pEntity)
{
	...
	if(pEntity->m_iClassname == s_iszStriderBusterClassname || pEntity->m_iClassname == s_iszMagnadeClassname)
		return true;
...
bool HateThisStriderBuster(CBaseEntity *pTarget)
{
	...
	{
		if(hunter_hate_held_striderbusters.GetBool() || hunter_hate_thrown_striderbusters.GetBool() ||
...
public:
	...
	static CHunterFlechette *FlechetteCreate(const Vector &vecOrigin, const QAngle &angAngles,
...
BEGIN_DATADESC(CHunterFlechette)
	...
	DEFINE_THINKFUNC(BubbleThink), DEFINE_THINKFUNC(DangerSoundThink), DEFINE_THINKFUNC(ExplodeThink),
		DEFINE_THINKFUNC(DopplerThink), DEFINE_THINKFUNC(SeekThink),

		DEFINE_ENTITYFUNC(FlechetteTouch),

		DEFINE_FIELD(m_vecShootPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_hSeekTarget, FIELD_EHANDLE),
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CHunterFlechette *CHunterFlechette::FlechetteCreate(const Vector &vecOrigin, const QAngle &angAngles,
...
static ConCommand ent_create("hunter_shoot_flechette", CC_Hunter_Shoot_Flechette,
							 "Fires a hunter flechette where the player is looking.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
//-----------------------------------------------------------------------------
void CHunterFlechette::OnParentCollisionInteraction(parentCollisionInteraction_t eType, int index,
...
void CHunterFlechette::FlechetteTouch(CBaseEntity *pOther)
{
	...
	if(pOther->m_takedamage != DAMAGE_NO)
	{
		...
		trace_t tr2;
		UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vForward * 128, MASK_BLOCKLOS, pOther, COLLISION_GROUP_NONE,
					   &tr2);
		...
		if(((pOther->GetMoveType() == MOVETYPE_VPHYSICS) || (pOther->GetMoveType() == MOVETYPE_PUSH)) &&
		   ((pOther->GetHealth() > 0) || (pOther->m_takedamage == DAMAGE_EVENTS_ONLY)))
		{
...
void CHunterFlechette::Explode()
{
	...
	RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), sk_hunter_flechette_explode_dmg.GetFloat(), nDamageType),
				 GetAbsOrigin(), sk_hunter_flechette_explode_radius.GetFloat(), CLASS_NONE, NULL);
				...
				if(entMass < m_minMass)
				{
					if(entMass < m_minMass * 0.666f ||
					   pEntity->CollisionProp()->BoundingRadius() <
						   (assert_cast<const CAI_BaseNPC *>(EntityFromEntityHandle(m_pPassEnt)))->GetHullHeight())
					{
						return false;
					}
				}
...
public:
	...
	CAI_HunterEscortBehavior()
		: BaseClass(AI_FollowParams_t(AIF_HUNTER, true)), m_flTimeEscortReturn(0), m_bEnabled(false)
	{
	}
	...
	bool FarFromFollowTarget()
	{
		return (GetFollowTarget() &&
				(GetAbsOrigin() - GetFollowTarget()->GetAbsOrigin()).LengthSqr() > HUNTER_FOLLOW_DISTANCE_SQR);
	}
...
BEGIN_DATADESC(CAI_HunterEscortBehavior)
	DEFINE_FIELD(m_flTimeEscortReturn, FIELD_TIME), DEFINE_EMBEDDED(m_FollowAttackTimer),
...
impactdamagetable_t s_HunterImpactDamageTable = {
	...
	5, // large mass falling scale (emphasize falling/crushing damage over sideways impacts since the stress will kill
	   // you anyway)
	0.0f, // min vel
};
...
private:
	...
	void GetShootDir(Vector &vecDir, const Vector &vecSrc, CBaseEntity *pTargetEntity, bool bStriderbuster,
...
BEGIN_DATADESC(CNPC_Hunter)
	...
	DEFINE_KEYFIELD(m_iszFollowTarget, FIELD_STRING, "FollowTarget"),
		...
		DEFINE_FIELD(m_flShootAllowInterruptTime, FIELD_TIME), DEFINE_FIELD(m_flNextChargeTime, FIELD_TIME),
		...
		DEFINE_FIELD(m_bTopMuzzle, FIELD_BOOLEAN), DEFINE_FIELD(m_bEnableSquadShootDelay, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bIsBleeding, FIELD_BOOLEAN),

		DEFINE_FIELD(m_bDisableShooting, FIELD_BOOLEAN), DEFINE_FIELD(m_bFlashlightInEyes, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flPupilDilateTime, FIELD_TIME),

		DEFINE_FIELD(m_vecEnemyLastSeen, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vecLastCanPlantHerePos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecStaggerDir, FIELD_VECTOR),

		DEFINE_FIELD(m_bPlanted, FIELD_BOOLEAN), DEFINE_FIELD(m_bLastCanPlantHere, FIELD_BOOLEAN),
		// DEFINE_FIELD( m_bMissLeft, FIELD_BOOLEAN ),
		DEFINE_FIELD(m_bEnableUnplantedShooting, FIELD_BOOLEAN),

		DEFINE_FIELD(m_nKillingDamageType, FIELD_INTEGER), DEFINE_FIELD(m_eDodgeActivity, FIELD_INTEGER),
		DEFINE_EMBEDDED(m_RundownDelay), DEFINE_EMBEDDED(m_IgnoreVehicleTimer),

		DEFINE_FIELD(m_flNextMeleeTime, FIELD_TIME), DEFINE_FIELD(m_flTeslaStopTime, FIELD_TIME),
		...
		DEFINE_FIELD(m_iszSiegeTargetName, FIELD_STRING), DEFINE_FIELD(m_flTimeNextSiegeTargetAttack, FIELD_TIME),
		DEFINE_FIELD(m_hCurrentSiegeTarget, FIELD_EHANDLE), DEFINE_FIELD(m_hHitByVehicle, FIELD_EHANDLE),

		DEFINE_EMBEDDED(m_BeginFollowDelay), DEFINE_EMBEDDED(m_EyeSwitchTimer),

		DEFINE_FIELD(m_nFlechettesQueued, FIELD_INTEGER), DEFINE_FIELD(m_nClampedShots, FIELD_INTEGER),
		DEFINE_FIELD(m_flNextRangeAttack2Time, FIELD_TIME), DEFINE_FIELD(m_flNextFlechetteTime, FIELD_TIME),
		DEFINE_UTLVECTOR(m_hAttachedBusters, FIELD_EHANDLE), DEFINE_UTLVECTOR(m_pSiegeTargets, FIELD_EHANDLE),

		// inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "Dodge", InputDodge), DEFINE_INPUTFUNC(FIELD_VOID, "FlankEnemy", InputFlankEnemy),
		...
		// Function Pointers
		DEFINE_THINKFUNC(TeslaThink), DEFINE_THINKFUNC(BleedThink), DEFINE_THINKFUNC(JostleVehicleThink),
...
bool CNPC_Hunter::OverrideMoveFacing(const AILocalMoveGoal_t &move, float flInterval)
{
	...
	if(GetEnemy() && (bSideStepping || (((GetNavigator()->GetMovementActivity() == ACT_RUN) ||
...
void CNPC_Hunter::Activate()
{
	...
	CMissile::AddCustomDetonator(this, (GetHullMaxs().AsVector2D() - GetHullMins().AsVector2D()).Length() * 0.5,
								 GetHullHeight());
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		if(IsUsingSiegeTargets())
		{
			EntityText(text_offset,
					   CFmtStr("Next Siege Attempt:%f", m_flTimeNextSiegeTargetAttack - gpGlobals->curtime), 0);
			text_offset++;
		}
	}
...
void CNPC_Hunter::GatherChargeConditions()
{
	...
	if(m_EscortBehavior.GetEscortTarget() && GetEnemy()->MyCombatCharacterPointer() &&
	...
	if(m_IgnoreVehicleTimer.Expired() && GetEnemy() && HasCondition(COND_SEE_ENEMY))
	{
		...
		if((pVehicle) && (GlobalEntity_GetCounter(s_iszHuntersToRunOver) <= 0))
		{
			...
			Vector2D vecFuturePos =
				pVehicle->GetAbsOrigin().AsVector2D() + pVehicle->GetSmoothedVelocity().AsVector2D() * predictTime;
			if(pVehicle->GetSmoothedVelocity().LengthSqr() > Square(200))
			{
				...
				{
					NDebugOverlay::Line(pVehicle->GetAbsOrigin(),
										pVehicle->GetAbsOrigin() + pVehicle->GetSmoothedVelocity(), 255, 255, 255, true,
										.1);
				}
				...
				if(DotProduct2D(vDirMovement, vDirToHunter) > hunter_dodge_warning_cone.GetFloat() &&
				   CalcDistanceSqrToLine2D(GetAbsOrigin().AsVector2D(), pVehicle->GetAbsOrigin().AsVector2D(),
										   vecFuturePos, &t) < Square(hunter_dodge_warning_width.GetFloat() * .5) &&
				   t > 0.0 && t < 1.0)
				{
					...
					if(hunter_dodge_debug.GetBool())
					{
						...
						if(timeDrawnArrow != gpGlobals->curtime)
						{
							...
							NDebugOverlay::HorzArrow(UTIL_GetLocalPlayer()->WorldSpaceCenter() - Vector(0, 0, 24),
													 vEndpoint, hunter_dodge_warning_width.GetFloat(), 255, 0, 0, 64,
													 true, .1);
						}
					}
				}
			...
			if(hunter_dodge_debug.GetBool())
			{
				if(timeDrawnArrow != gpGlobals->curtime)
				{
					...
					NDebugOverlay::HorzArrow(UTIL_GetLocalPlayer()->WorldSpaceCenter() - Vector(0, 0, 24), vEndpoint,
											 hunter_dodge_warning_width.GetFloat(), 127, 127, 127, 64, true, .1);
				}
			}
		}
...
void CNPC_Hunter::ManageSiegeTargets()
{
	...
	if(bCreateSiegeTarget)
	{
		...
		if(pSiegeTargetLocation != NULL)
		{
			...
			GetEnemies()->UpdateMemory(GetNavigator()->GetNetwork(), pSiegeTarget, pSiegeTarget->GetAbsOrigin(), 0.0f,
									   true);
			AI_EnemyInfo_t *pMemory = GetEnemies()->Find(pSiegeTarget);

			if(pMemory)
			{
				// Pretend we've known about this target longer than we really have so that our AI doesn't waste time
				// running ALERT schedules.
...
void CNPC_Hunter::GatherIndoorOutdoorConditions()
{
	...
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 40.0f * 12.0f), MASK_SHOT, this, COLLISION_GROUP_NONE,
				   &tr);
	if(tr.fraction < 1.0f)
	{
...
void CNPC_Hunter::BuildScheduleTestBits()
{
	...
	if(!IsCurSchedule(SCHED_HUNTER_DODGE, false) && !IsCurSchedule(SCHED_HUNTER_STAGGER, false) &&
...
{
	return pEntity && pEntity->GetMoveType() == MOVETYPE_VPHYSICS && pEntity->VPhysicsGetObject() &&
...
bool CNPC_Hunter::ShouldCharge(const Vector &startPos, const Vector &endPos, bool useTime, bool bCheckForCancel)
{
	...
	if(g_debug_hunter_charge.GetInt() == 1)
	{
		...
		NDebugOverlay::BoxDirection(startPos, GetHullMins(), GetHullMaxs() + Vector(enemyDist, 0, 0), enemyDir, 0, 255,
									0, 8, 1.0f);
	}
...
int CNPC_Hunter::SelectCombatSchedule()
{
	...
	if(HasCondition(COND_CAN_RANGE_ATTACK2))
	{
		if(bStriderBuster || CountRangedAttackers() < hunter_flechette_max_concurrent_volleys.GetInt())
		{
			DelayRangedAttackers(hunter_flechette_volley_start_min_delay.GetFloat(),
								 hunter_flechette_volley_start_max_delay.GetFloat(), true);
			return SCHED_HUNTER_RANGE_ATTACK2;
		}
	}
	...
	/*
		...
		{
			if ( HasCondition( COND_ENEMY_FACING_ME ) && ( UTIL_DistApprox( GetEnemy()->GetAbsOrigin(), GetAbsOrigin() )
	...
	*/
	...
	if(!bStriderBuster && bIsCorporealEnemy)
	{
		...
		if((NumHuntersInMySquad() > 1) &&
		   OccupyStrategySlotRange(SQUAD_SLOT_HUNTER_FLANK_FIRST, SQUAD_SLOT_HUNTER_FLANK_LAST))
		{
	...
	// Can't see my enemy.
	if(HasCondition(COND_ENEMY_OCCLUDED) || HasCondition(COND_ENEMY_TOO_FAR) || HasCondition(COND_TOO_FAR_TO_ATTACK) ||
...
void CNPC_Hunter::TaskFindDodgeActivity()
{
	...
	while(!bFoundDir && (nTries < 2))
	{
		...
		trace_t tr;
		HunterTraceHull_SkipPhysics(GetAbsOrigin(), GetAbsOrigin() + vecDelta, testHullMins, GetHullMaxs(),
		...
		if(tr.fraction == 1.0f)
		{
			// NDebugOverlay::SweptBox( GetAbsOrigin(), GetAbsOrigin() + vecDelta, testHullMins, GetHullMaxs(), QAngle(
			// 0, 0, 0 ), 0, 255, 0, 128, 5 );
			bFoundDir = true;
			TaskComplete();
		}
...
void CNPC_Hunter::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		{
			// We're committing to shooting. Don't allow interrupts until after we've shot a bit (see
			// TASK_RANGE_ATTACK1).
		...
		case TASK_RANGE_ATTACK2:
		{
			if(GetEnemy())
			{
				...
				if(bIsBuster)
				{
					AddFacingTarget(GetEnemy(), GetEnemy()->GetAbsOrigin() + GetEnemy()->GetSmoothedVelocity() * .5,
									1.0, 0.8);
				}
	...
	switch(pTask->iTask)
	{
			...
			if(gpGlobals->curtime >= m_flNextFlechetteTime)
			{
				...
				if(bDone)
				{
					...
					DelayRangedAttackers(hunter_flechette_volley_end_min_delay.GetFloat(),
										 hunter_flechette_volley_end_max_delay.GetFloat(), true);
					TaskComplete();
				}
			}
		...
		case TASK_HUNTER_CHARGE:
		{
			...
			if(eActivity == ACT_HUNTER_CHARGE_STOP || eActivity == ACT_HUNTER_CHARGE_CRASH)
			{
				if(IsActivityFinished())
				{
					m_flNextChargeTime = gpGlobals->curtime + hunter_charge_min_delay.GetFloat() +
...
bool CNPC_Hunter::EnemyIsRightInFrontOfMe(CBaseEntity **pEntity)
{
	...
	if((GetEnemy()->WorldSpaceCenter() - WorldSpaceCenter()).LengthSqr() < (156 * 156))
	{
		...
		if(DotProduct(vecLOS, vBodyDir) > 0.8)
		{
			...
			trace_t tr;
			UTIL_TraceHull(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), GetHullMins() * 0.5, GetHullMaxs() * 0.5,
...
float CNPC_Hunter::ChargeSteer()
{
	...
	// Probe
	HunterTraceHull_SkipPhysics(GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(), MASK_NPCSOLID, this,
	...
	if(g_debug_hunter_charge.GetInt() == 1)
	{
		if(tr.fraction == 1.0f)
		{
			NDebugOverlay::BoxDirection(GetAbsOrigin(), testHullMins, GetHullMaxs() + Vector(testLength, 0, 0), forward,
										0, 255, 0, 8, 0.1f);
		}
		else
		{
	...
	if(g_debug_hunter_charge.GetInt() == 1)
	{
		...
		NDebugOverlay::Cross3D(GetAbsOrigin() + (steer * 512.0f), Vector(2, 2, 2), -Vector(2, 2, 2), 255, 255, 0, true,
							   0.1f);

		NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (BodyDirection3D() * 256.0f), 255, 0, 255, true, 0.1f);
		NDebugOverlay::Cross3D(GetAbsOrigin() + (BodyDirection3D() * 256.0f), Vector(2, 2, 2), -Vector(2, 2, 2), 255, 0,
							   255, true, 0.1f);
	}
...
bool CNPC_Hunter::HandleChargeImpact(Vector vecImpact, CBaseEntity *pEntity)
{
	...
	if(pEntity->GetMoveType() == MOVETYPE_NONE || pEntity->GetMoveType() == MOVETYPE_PUSH)
	{
		...
		if(pBreakable && pBreakable->IsBreakable() && pBreakable->m_takedamage == DAMAGE_YES &&
		   pBreakable->GetHealth() > 0)
		{
			ChargeDamage(pEntity);
		}
		return true;
			...
			if(entMass < minMass)
			{
				if(entMass < minMass * 0.666f || pEntity->CollisionProp()->BoundingRadius() < GetHullHeight())
				{
					if(pEntity->GetHealth() > 0)
					{
						...
						if(pBreakable && pBreakable->m_takedamage == DAMAGE_YES && pBreakable->GetHealth() > 0 &&
						   pBreakable->GetHealth() <= 50)
						{
							ChargeDamage(pEntity);
						}
					}
...
void CNPC_Hunter::Explode()
{
	...
	PropBreakableCreateAll(GetModelIndex(), NULL, EyePosition(), GetAbsAngles(), velocity, angVelocity, 1.0, 150,
						   COLLISION_GROUP_NPC, this);

	ExplosionCreate(EyePosition(), GetAbsAngles(), this, 500, 256,
					(SF_ENVEXPLOSION_NOPARTICLES | SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS |
					 SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSMOKE),
					false);
...
void CNPC_Hunter::HandleAnimEvent(animevent_t *pEvent)
{
	...
	if(pEvent->event == AE_HUNTER_SPRAY_BLOOD)
	{
		...
		for(int i = 0; i < 3; i++)
		{
			...
			trace_t tr;
			AI_TraceLine(vecOrigin, vecOrigin + (vecTraceDir * 192.0f), MASK_SOLID_BRUSHONLY, this,
...
bool CNPC_Hunter::IsValidEnemy(CBaseEntity *pTarget)
{
	if(IsStriderBuster(pTarget))
	{
		...
		if(pTarget->VPhysicsGetObject())
		{
						...
						if(m_EscortBehavior.GetEscortTarget() &&
						   (m_EscortBehavior.GetEscortTarget()->GetAbsOrigin().AsVector2D() -
							pTarget->GetAbsOrigin().AsVector2D())
								   .LengthSqr() < Square(hunter_hate_held_striderbusters_tolerance.GetFloat()))
						{
							return true;
						}
					}
			...
			if(bAttached && StriderBuster_IsAttachedStriderBuster(pTarget, m_EscortBehavior.GetEscortTarget()) &&
			   hunter_hate_attached_striderbusters.GetBool())
			{
				return true;
			}
...
int CNPC_Hunter::MeleeAttack1ConditionsVsEnemyInVehicle(CBaseCombatCharacter *pEnemy, float flDot)
{
	...
	VectorNormalize(vecDelta);
	trace_t tr;
	AI_TraceHull(WorldSpaceCenter(), WorldSpaceCenter() + (vecDelta * 64), -Vector(8, 8, 8), Vector(8, 8, 8), MASK_SHOT,
...
int CNPC_Hunter::MeleeAttack1Conditions(float flDot, float flDist)
{
	...
	trace_t tr;
	AI_TraceHull(WorldSpaceCenter(), WorldSpaceCenter() + forward * HUNTER_MELEE_REACH, vecMins, vecMaxs, MASK_NPCSOLID,
				 this, COLLISION_GROUP_NONE, &tr);
...
{
	bool bIsBuster = IsStriderBuster(GetEnemy());
	bool bIsPerfectBullseye = (GetEnemy() && dynamic_cast<CNPC_Bullseye *>(GetEnemy()) &&
	...
	else if(!bIsBuster && (!GetEnemy() || !GetEnemy()->ClassMatches("npc_bullseye")) &&
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_Hunter::MeleeAttack(float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch,
									  int BloodOrigin)
{
	...
	{
		trace_t tr;
		AI_TraceHull(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), -Vector(8, 8, 8), Vector(8, 8, 8),
...
{
	return SOUND_WORLD | SOUND_COMBAT | SOUND_PLAYER | SOUND_DANGER | SOUND_PHYSICS_DANGER | SOUND_PLAYER_VEHICLE |
		   SOUND_BULLET_IMPACT | SOUND_MOVE_AWAY;
}
							  ...
							  CDmgAccumulator *pAccumulator)
{
	...
	if((info.GetDamageType() & DMG_BULLET) || (info.GetDamageType() & DMG_BUCKSHOT) ||
	   (info.GetDamageType() & DMG_CLUB) || (info.GetDamageType() & DMG_NEVERGIB))
	{
		float flScale = 1.0;
		...
		DispatchParticleEffect("blood_impact_synth_01_arc_parent", PATTACH_POINT_FOLLOW, this,
...
void CNPC_Hunter::TeslaThink()
{
	...
	{
		SetContextThink(&CNPC_Hunter::TeslaThink, gpGlobals->curtime + random->RandomFloat(0.1f, 0.3f),
						HUNTER_ZAP_THINK);
	}
}
...
void CNPC_Hunter::JostleVehicleThink()
{
	...
	float flForceScale = RemapValClamped(flSpeed, hunter_jostle_car_min_speed.GetFloat(),
...
int CNPC_Hunter::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
		...
		else
		{
			...
			if((info.GetDamageType() & DMG_VEHICLE) ||
			   (pInflictor && pInflictor->GetServerVehicle() &&
				((bHitByUnoccupiedCar = (dynamic_cast<CPropVehicleDriveable *>(pInflictor) &&
										 static_cast<CPropVehicleDriveable *>(pInflictor)->GetDriver() == NULL)) ==
				 false)))
			{
	...
	if(pAttacker && ((pAttacker->Classify() == CLASS_CITIZEN_REBEL) || (pAttacker->Classify() == CLASS_PLAYER_ALLY) ||
...
void CNPC_Hunter::Event_Killed(const CTakeDamageInfo &info)
{
	...
	if(m_EscortBehavior.GetFollowTarget())
	{
		if(AIGetNumFollowers(m_EscortBehavior.GetFollowTarget(), m_iClassname) == 1)
		{
			...
			{
				m_EscortBehavior.GetEscortTarget()->UpdateEnemyMemory(UTIL_GetLocalPlayer(),
																	  UTIL_GetLocalPlayer()->GetAbsOrigin(), this);
			}
		}
	}
...
//-----------------------------------------------------------------------------
void CNPC_Hunter::GetShootDir(Vector &vecDir, const Vector &vecSrc, CBaseEntity *pTargetEntity, bool bStriderBuster,
							  int nShotNum, bool bSingleShot)
{
...
bool CNPC_Hunter::ShouldSeekTarget(CBaseEntity *pTargetEntity, bool bStriderBuster)
{
	...
	if(bStriderBuster)
	{
		...
		if(pTargetEntity->VPhysicsGetObject() &&
		   (pTargetEntity->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD))
		{
			bSeek = true;
		}
		else if(StriderBuster_NumFlechettesAttached(pTargetEntity) == 0)
		{
			...
			else if(hunter_seek_thrown_striderbusters_tolerance.GetFloat() > 0.0)
			{
				...
				if(pEscortTarget && (pEscortTarget->GetAbsOrigin() - pTargetEntity->GetAbsOrigin()).LengthSqr() <
										Square(hunter_seek_thrown_striderbusters_tolerance.GetFloat()))
				{
					bSeek = true;
				}
			}
...
bool CNPC_Hunter::ShootFlechette(CBaseEntity *pTargetEntity, bool bSingleShot)
{
	...
	if(nShotNum == 1 && pTargetEntity->Classify() == CLASS_PLAYER_ALLY_VITAL)
	{
		...
		CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE | SOUND_CONTEXT_EXCLUDE_COMBINE,
...
const char *CNPC_Hunter::SelectRandomExpressionForState(NPC_STATE state)
{
	...
	char *szExpressions[4] = {"scenes/npc/hunter/hunter_scan.vcd", "scenes/npc/hunter/hunter_eyeclose.vcd",
							  "scenes/npc/hunter/hunter_roar.vcd", "scenes/npc/hunter/hunter_pain.vcd"};
...
void CNPC_Hunter::UpdateAim()
{
	...
	if(GetEnemy() && GetState() != NPC_STATE_SCRIPT && (eActivity != ACT_HUNTER_CHARGE_CRASH) &&
...
bool CNPC_Hunter::CanBecomeRagdoll()
{
	return (m_nKillingDamageType & DMG_CRUSH) ||
		   ...
		   IsCurSchedule(SCHED_SCRIPTED_RUN, false) || (GetActivity() == ACT_WALK) || (GetActivity() == ACT_RUN) ||
...
{
	if(info.GetDamage() > 0 && info.GetAttacker()->IsPlayer() && GetFollowTarget() &&
	   (AIGetNumFollowers(GetFollowTarget()) > 1) &&
	   (GetOuter()->GetSquad()->GetSquadSoundWaitTime() <= gpGlobals->curtime)) // && !FarFromFollowTarget()
	{
		...
		GetOuter()->GetSquad()->SetSquadSoundWaitTime(m_flTimeEscortReturn +
													  1.0); // prevent others from breaking escort
	}
}
		...
		case TASK_MOVE_TO_FOLLOW_POSITION:
		{
			if(GetEnemy())
			{
				if(GetOuter()->OccupyStrategySlot(SQUAD_SLOT_RUN_SHOOT))
				{
					...
					{
						GetOuter()->BeginVolley(NUM_FLECHETTE_VOLLEY_ON_FOLLOW, gpGlobals->curtime + 1.0 +
							...
							if(gpGlobals->curtime >= pHunter->m_flNextFlechetteTime)
							{
								...
								if(bHasSlot)
								{
									...
									else
									{
										...
										// pHunter->m_pGunFiringSound, 0, 0.01f );

										bVacate = true;
										pHunter->BeginVolley(NUM_FLECHETTE_VOLLEY_ON_FOLLOW,
															 gpGlobals->curtime + 1.0 + random->RandomFloat(0, .25) +
																 random->RandomFloat(0, .25));
									}
								}
							}
...
void CAI_HunterEscortBehavior::DistributeFreeHunters()
{
	if(g_TimeLastDistributeFreeHunters != -1 &&
	   gpGlobals->curtime - g_TimeLastDistributeFreeHunters < FREE_HUNTER_DISTRIBUTE_INTERVAL)
	{
		return;
	}
...
AI_BEGIN_CUSTOM_NPC(npc_hunter, CNPC_Hunter)
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_RANGE_ATTACK1,
	...
	// Attack (Deploy/shoot/finish)
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_RANGE_ATTACK2,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_RANGE_ATTACK2_VS_STRIDERBUSTER,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_RANGE_ATTACK2_VS_STRIDERBUSTER_LATENT,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_DODGE,
	...
	// Dodge Incoming vehicle
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_FAIL_DODGE,
	...
	//==================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_CHARGE_ENEMY,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_CHASE_ENEMY_MELEE,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_CHASE_ENEMY,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_FLANK_ENEMY,
	...
	//=========================================================
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_COMBAT_FACE,
	...
	// In a fight with nothing to do. Make busy!
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_CHANGE_POSITION_FINISH,
	...
	DEFINE_SCHEDULE(
		...
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_HUNTER_FAIL_IMMEDIATE" // used because sched_fail
																					  // includes a one second pause.
																					  // ick!
		"		TASK_STOP_MOVING						0"
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_PATROL_RUN,
	...
	//=========================================================
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_FOUND_ENEMY,
	...
	//=========================================================
	//=========================================================
	DEFINE_SCHEDULE(SCHED_HUNTER_FOUND_ENEMY_ACK,
	...
	DEFINE_SCHEDULE(
		SCHED_HUNTER_GOTO_HINT,
		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_HUNTER_CLEAR_HINTNODE" // used because sched_fail
																					  // includes a one second pause.
																					  // ick!
	...
	DEFINE_SCHEDULE(SCHED_HUNTER_CLEAR_HINTNODE, "	Tasks"
												 "		TASK_CLEAR_HINTNODE				0"
												 ""
												 ""
												 "	Interrupts")

	DEFINE_SCHEDULE(SCHED_HUNTER_SIEGE_STAND,
					"	Tasks"
					"		TASK_STOP_MOVING				0"
					"		TASK_SET_ACTIVITY				ACTIVITY:ACT_IDLE"
					"		TASK_FACE_PLAYER				0"
	...
	DEFINE_SCHEDULE(SCHED_HUNTER_CHANGE_POSITION_SIEGE,
