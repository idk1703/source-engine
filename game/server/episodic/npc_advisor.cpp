...
ConVar advisor_use_impact_table("advisor_use_impact_table", "1", FCVAR_NONE,
...
ConVar advisor_throw_lead_prefetch_time("advisor_throw_lead_prefetch_time", "0.66", FCVAR_NONE,
										"Save off the player's velocity this many seconds before throwing.");
ConVar advisor_throw_stage_distance("advisor_throw_stage_distance", "180.0", FCVAR_NONE,
									"Advisor will try to hold an object this far in front of him just before throwing "
									"it at you. Small values will clobber the shield and be very bad.");
// ConVar advisor_staging_num("advisor_staging_num","1",FCVAR_NONE,"Advisor will queue up this many objects to throw at
// Gordon.");
ConVar advisor_throw_clearout_vel("advisor_throw_clearout_vel", "200", FCVAR_NONE,
...
BEGIN_SIMPLE_DATADESC(CAdvisorLevitate)
	DEFINE_FIELD(m_flFloat, FIELD_FLOAT), DEFINE_FIELD(m_vecGoalPos1, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vecGoalPos2, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_Advisor, FIELD_EHANDLE),
END_DATADESC()
...
protected:
#if NPC_ADVISOR_HAS_BEHAVIOR
	Vector GetThrowFromPos(
		CBaseEntity *pEnt); ///< Get the position in which we shall hold an object prior to throwing it
#endif
	...
	CBaseEntity *PickThrowable(
		bool bRequireInView); ///< choose an object to throw at the player (so it can get stuffed in the handle array)
	...
	// todo: write accessor functions for m_hvStagedEnts so that it doesn't have members added and removed willy nilly
	// throughout code (will make the networking below more reliable)
	...
	// keep track of up to four objects after we have thrown them, to prevent oscillation or levitation of recently
	// thrown ammo.
...
BEGIN_DATADESC(CNPC_Advisor)
	...
	DEFINE_KEYFIELD(m_iszLevitateGoal1, FIELD_STRING, "levitategoal_bottom"),
		...
		DEFINE_KEYFIELD(m_iszLevitationArea, FIELD_STRING,
		...
		DEFINE_FIELD(m_hLevitateGoal1, FIELD_EHANDLE), DEFINE_FIELD(m_hLevitateGoal2, FIELD_EHANDLE),
		DEFINE_FIELD(m_hLevitationArea, FIELD_EHANDLE),

#if NPC_ADVISOR_HAS_BEHAVIOR
		DEFINE_KEYFIELD(m_iszStagingEntities, FIELD_STRING,
						"staging_ent_names"), ///< entities named this constitute the positions to which we stage
											  ///< objects to be thrown
		DEFINE_KEYFIELD(m_iszPriorityEntityGroupName, FIELD_STRING, "priority_grab_name"),

		DEFINE_UTLVECTOR(m_hvStagedEnts, FIELD_EHANDLE), DEFINE_UTLVECTOR(m_hvStagingPositions, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_hPlayerPinPos, FIELD_EHANDLE), DEFINE_FIELD(m_playerPinFailsafeTime, FIELD_TIME),

		// DEFINE_FIELD( m_hThrowEnt, FIELD_EHANDLE ),
		DEFINE_FIELD(m_flThrowPhysicsTime, FIELD_TIME), DEFINE_FIELD(m_flLastPlayerAttackTime, FIELD_TIME),
		DEFINE_FIELD(m_flStagingEnd, FIELD_TIME), DEFINE_FIELD(m_iStagingNum, FIELD_INTEGER),
		DEFINE_FIELD(m_bWasScripting, FIELD_BOOLEAN),

		DEFINE_FIELD(m_flLastThrowTime, FIELD_TIME), DEFINE_FIELD(m_vSavedLeadVel, FIELD_VECTOR),

		DEFINE_OUTPUT(m_OnPickingThrowable, "OnPickingThrowable"), DEFINE_OUTPUT(m_OnThrowWarn, "OnThrowWarn"),
		DEFINE_OUTPUT(m_OnThrow, "OnThrow"), DEFINE_OUTPUT(m_OnHealthIsNow, "OnHealthIsNow"),
...
void CNPC_Advisor::Activate()
{
	...
	AssertMsg(m_hvStagingPositions.Count() > 0,
			  "You did not specify any staging positions in the advisor's staging_ent_names !");
#endif
}
#pragma warning(pop)
...
void CNPC_Advisor::StartLevitatingObjects()
{
	...
	for(int i = 0; i < nCount; i++)
	{
		...
		// NDebugOverlay::Box( pEnt->GetAbsOrigin(), pEnt->CollisionProp()->OBBMins(), pEnt->CollisionProp()->OBBMaxs(),
		// 0, 255, 0, 1, 0.1 );
...
void CNPC_Advisor::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		{
			// if we have a trigger volume, use the contents of that. If not, use a hardcoded box (for debugging
			// purposes) in both cases we validate the objects using the same helper funclet just above. When we can
			// count on the trigger vol being there, we can elide the else{} clause here.
			...
			else
			{
				...
				//NDebugOverlay::Box( GetAbsOrigin(), Vector( -408, -368, -188 ), Vector( 92, 208, 168 ), 255, 255, 0,
			1, 5 );
				...
				int nCount = UTIL_EntitiesInBox( GetAbsOrigin() - Vector( 554, 368, 188 ), GetAbsOrigin() + Vector( 92,
			208, 168 ), &levitateEnum ); for ( int i = 0; i < nCount; i++ )
				{
					//Msg( "%d found %s\n", m_physicsObjects.Count(), STRING( list[i]->GetModelName() ) );
	...
	switch(pTask->iTask)
	{
		...
		case TASK_ADVISOR_LEVITATE_OBJECTS:
		{
			...
			for ( int i = 0; i < m_physicsObjects.Count(); i++ )
			{
				...
				if ( pPhys && pPhys->IsMoveable() )
				{
					NDebugOverlay::Box( pEnt->GetAbsOrigin(), pEnt->CollisionProp()->OBBMins(),
			pEnt->CollisionProp()->OBBMaxs(), 0, 255, 0, 1, 0.1 );
				}
			}*/
...
CBaseEntity *CNPC_Advisor::PickThrowable(bool bRequireInView)
{
	...
	if(!!m_iszPriorityEntityGroupName) // if the string isn't null
	{
		...
		for(int ii = 0; ii < numObjs; ++ii)
		{
			...
			bool bCanPick =
				AdvisorCanPickObject(pPlayer, pThrowEnt) && !m_hvStagedEnts.HasElement(m_physicsObjects[ii]);
			if(!bCanPick)
				continue;
...
void CNPC_Advisor::PreHurlClearTheWay(CBaseEntity *pThrowable, const Vector &toPos)
{
	...
	ray.Init(pThrowable->GetAbsOrigin(), toPos, Vector(-boundingRadius, -boundingRadius, -boundingRadius),
	...
	AssertMsg(nFoundCt < 128,
			  "Found more than 128 obstructions between advisor and Gordon while throwing. (safe to continue)\n");
	...
	for(int i = 0; i < nFoundCt; ++i)
	{
		...
		if(obstruction != pThrowable && m_physicsObjects.HasElement(obstruction) && // if it's floating
		   ...
		   !DidThrow(obstruction))													// and I didn't just throw it
		{
			...
			Vector thrust;
			CalcClosestPointOnLine(obstruction->GetAbsOrigin(), pThrowable->GetAbsOrigin(), toPos, thrust);
...
void CNPC_Advisor::PullObjectToStaging(CBaseEntity *pEnt, const Vector &stagingPos)
{
	...
	float desiredDisplacementLen =
		ExponentialDecay(STAGING_OBJECT_FALLOFF_TIME, gpGlobals->frametime); // * sqrt(displacementLen);
...
Vector CNPC_Advisor::GetThrowFromPos(CBaseEntity *pEnt)
{
	...
	float howFarInFront =
		advisor_throw_stage_distance.GetFloat() +
		effecRadius * 1.43f; // clamp(lenToPlayer - posDist + effecRadius,effecRadius*2,90.f + effecRadius);
...
bool CNPC_Advisor::QueryHearSound(CSound *pSound)
{
	...
	if(pOwner && pSound->IsSoundType(SOUND_COMBAT) && pSound->SoundChannel() != SOUNDENT_CHANNEL_NPC_FOOTSTEP &&
...
// MOTION CALLBACK
//==============================================================================================
CAdvisorLevitate::simresult_e CAdvisorLevitate::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject,
...
AI_BEGIN_CUSTOM_NPC(npc_advisor, CNPC_Advisor)
	...
	*/
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_ADVISOR_COMBAT,
	...
	//=========================================================
	DEFINE_SCHEDULE(SCHED_ADVISOR_IDLE_STAND,
	...
	DEFINE_SCHEDULE(SCHED_ADVISOR_TOSS_PLAYER,
