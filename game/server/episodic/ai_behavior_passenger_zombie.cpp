...
BEGIN_DATADESC(CAI_PassengerBehaviorZombie)
	...
	DEFINE_FIELD(m_flLastVerticalLean, FIELD_FLOAT), DEFINE_FIELD(m_flLastLateralLean, FIELD_FLOAT),
...
CAI_PassengerBehaviorZombie::CAI_PassengerBehaviorZombie(void)
	: m_flLastVerticalLean(0.0f), m_flLastLateralLean(0.0f), m_flNextLeapTime(0.0f)
{
}
...
bool CAI_PassengerBehaviorZombie::CanJumpToAttachToVehicle(void)
{
	...
	if((flDist < JUMP_ATTACH_DIST_THRESHOLD) &&
	   UTIL_IsFacingWithinTolerance(GetOuter(), pEnemy, JUMP_ATTACH_FACING_THRESHOLD))
		return true;
...
void CAI_PassengerBehaviorZombie::GatherConditions(void)
{
	...
	if(GetPassengerState() == PASSENGER_STATE_OUTSIDE)
	{
		...
		if(pPlayer != NULL && GetOuter()->GetEnemy() == pPlayer && pPlayer->GetVehicleEntity() == m_hVehicle)
		{
...
void CAI_PassengerBehaviorZombie::BuildScheduleTestBits(void)
{
	...
	if(GetPassengerState() == PASSENGER_STATE_OUTSIDE)
	{
		GetOuter()->SetCustomInterruptCondition(
			GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_CAN_RANGE_ATTACK1));
		GetOuter()->SetCustomInterruptCondition(
...
void CAI_PassengerBehaviorZombie::GatherVehicleStateConditions(void)
{
	...
	if(flLateralDelta < -350.0f)
	{
		if(m_hVehicle)
		{
			...
			// NDebugOverlay::HorzArrow( GetAbsOrigin(), GetAbsOrigin() + ( vecDamageForce * 256.0f ), 16.0f, 255, 0, 0,
			// 16, true, 2.0f );

			// Fake it!
			CTakeDamageInfo info(m_hVehicle, m_hVehicle, vecDamageForce, GetOuter()->WorldSpaceCenter(), 200,
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_PassengerBehaviorZombie)
	{
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_ZOMBIE_EXIT_VEHICLE,
		...
		DEFINE_SCHEDULE(SCHED_PASSENGER_ZOMBIE_RUN_TO_VEHICLE,
