...
public:
	...
	Ep2LevelStats_t() : m_bInitialized(false), m_flLevelStartTime(0.0f)
	{
	...
	Ep2LevelStats_t(const Ep2LevelStats_t &other)
	{
		...
		int i;
		for(i = other.m_dictEntityDeaths.First(); i != other.m_dictEntityDeaths.InvalidIndex();
			i = other.m_dictEntityDeaths.Next(i))
		{
	...
	void AppendToBuffer(CUtlBuffer &SaveBuffer)
	{
		...
		// Write out the lumps.
		CBaseGameStats::AppendLump(EP2_MAX_LUMP_COUNT, SaveBuffer, EP2STATS_LUMP_HEADER, 1,
								   sizeof(Ep2LevelStats_t::LevelHeader_t), static_cast<void *>(&m_Header));
		CBaseGameStats::AppendLump(EP2_MAX_LUMP_COUNT, SaveBuffer, EP2STATS_LUMP_TAG, 1, sizeof(Ep2LevelStats_t::Tag_t),
								   static_cast<void *>(&m_Tag));
		CBaseGameStats::AppendLump(EP2_MAX_LUMP_COUNT, SaveBuffer, EP2STATS_LUMP_DEATH, m_aPlayerDeaths.Count(),
								   sizeof(Ep2LevelStats_t::PlayerDeathsLump_t),
								   static_cast<void *>(m_aPlayerDeaths.Base()));
		{
			CUtlBuffer buf;
			...
			for(int i = m_dictEntityDeaths.First(); i != m_dictEntityDeaths.InvalidIndex();
				i = m_dictEntityDeaths.Next(i))
			{
		...
		}
		{
			...
			CBaseGameStats::AppendLump(EP2_MAX_LUMP_COUNT, SaveBuffer, EP2STATS_LUMP_WEAPON, 1, buf.TellPut(),
									   buf.Base());
		}
		{
			CUtlBuffer buf;
			...
			CBaseGameStats::AppendLump(EP2_MAX_LUMP_COUNT, SaveBuffer, EP2STATS_LUMP_SAVEGAMEINFO, 1, buf.TellPut(),
									   buf.Base());
		}
		{
			CUtlBuffer buf;
			...
			CBaseGameStats::AppendLump(EP2_MAX_LUMP_COUNT, SaveBuffer, EP2STATS_LUMP_GENERIC, 1, buf.TellPut(),
									   buf.Base());
		}
	}
		...
		while(CBaseGameStats::GetLumpHeader(EP2_MAX_LUMP_COUNT, LoadBuffer, iLump, iLumpCount, true))
		{
			switch(iLump)
			{
				...
				case EP2STATS_LUMP_DEATH:
				{
					...
					CBaseGameStats::LoadLump(LoadBuffer, iLumpCount, sizeof(Ep2LevelStats_t::PlayerDeathsLump_t),
											 static_cast<void *>(pItem->m_aPlayerDeaths.Base()));
				}
				break;
...
public:
	...
	struct EntityDeathsLump_t
	{
		...
		EntityDeathsLump_t() : m_nBodyCount(0u), m_nKilledPlayer(0u) {}
	...
	struct WeaponLump_t
	{
		...
		WeaponLump_t() : m_nShots(0), m_nHits(0), m_flDamageInflicted(0.0) {}
	...
	{
		SaveGameInfoRecord_t() : m_nFirstDeathIndex(-1), m_nNumDeaths(0), m_nSaveHealth(-1)
		{
...
#pragma pack()
	...
	struct SaveGameInfo_t
	{
		...
		SaveGameInfo_t() : m_nCurrentSaveFileTime(0), m_pCurrentRecord(NULL) {}
	...
	struct GenericStatsLump_t
	{
		...
		GenericStatsLump_t() : m_unCount(0u), m_flCurrentValue(0.0)
		{
...
public:
	...
	virtual void Event_PlayerTraveled(CBasePlayer *pBasePlayer, float distanceInInches, bool bInVehicle,
									  bool bSprinting);
	virtual void Event_WeaponFired(CBasePlayer *pShooter, bool bPrimary, char const *pchWeaponName);
	virtual void Event_WeaponHit(CBasePlayer *pShooter, bool bPrimary, char const *pchWeaponName,
	...
	virtual void Event_IncrementCountedStatistic(const Vector &vecAbsOrigin, char const *pchStatisticName,
