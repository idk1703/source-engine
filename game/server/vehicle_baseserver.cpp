...
BEGIN_DATADESC_NO_BASE(vehicle_crashsound_t)
	DEFINE_FIELD(flMinSpeed, FIELD_FLOAT), DEFINE_FIELD(flMinDeltaSpeed, FIELD_FLOAT),
		DEFINE_FIELD(iszCrashSound, FIELD_STRING), DEFINE_FIELD(gearLimit, FIELD_INTEGER),
END_DATADESC()

BEGIN_DATADESC_NO_BASE(vehiclesounds_t)

	DEFINE_AUTO_ARRAY(iszSound, FIELD_STRING), DEFINE_UTLVECTOR(pGears, FIELD_EMBEDDED),
		DEFINE_UTLVECTOR(crashSounds, FIELD_EMBEDDED), DEFINE_AUTO_ARRAY(iszStateSounds, FIELD_STRING),
		DEFINE_AUTO_ARRAY(minStateTime, FIELD_FLOAT),

END_DATADESC()

BEGIN_SIMPLE_DATADESC(CPassengerInfo)
	DEFINE_FIELD(m_hPassenger, FIELD_EHANDLE), DEFINE_FIELD(m_strRoleName, FIELD_STRING),
...
BEGIN_SIMPLE_DATADESC(CBaseServerVehicle)
	...
	// Controls
	DEFINE_FIELD(m_nNPCButtons, FIELD_INTEGER), DEFINE_FIELD(m_nPrevNPCButtons, FIELD_INTEGER),
		DEFINE_FIELD(m_flTurnDegrees, FIELD_FLOAT), DEFINE_FIELD(m_flVehicleVolume, FIELD_FLOAT),

		// We're going to reparse this data from file in Precache
		DEFINE_EMBEDDED(m_vehicleSounds),

		DEFINE_FIELD(m_iSoundGear, FIELD_INTEGER), DEFINE_FIELD(m_flSpeedPercentage, FIELD_FLOAT),
		DEFINE_SOUNDPATCH(m_pStateSound), DEFINE_SOUNDPATCH(m_pStateSoundFade),
		DEFINE_FIELD(m_soundState, FIELD_INTEGER), DEFINE_FIELD(m_soundStateStartTime, FIELD_TIME),
		...
		DEFINE_FIELD(m_iCurrentExitAnim, FIELD_INTEGER), DEFINE_FIELD(m_vecCurrentExitEndPoint, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_chPreviousTextureType, FIELD_CHARACTER),

		DEFINE_FIELD(m_savedViewOffset, FIELD_VECTOR), DEFINE_FIELD(m_hExitBlocker, FIELD_EHANDLE),
...
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::NPC_GetPassengerSeatPosition(CBaseCombatCharacter *pPassenger, Vector *vecResultPos,
...
// Input  : *pPassenger - Passenger's seat to use
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::NPC_GetPassengerSeatPositionLocal(CBaseCombatCharacter *pPassenger, Vector *vecResultPos,
														   QAngle *vecResultAngles)
{
	// Get our attachment point
	int nSeatAttachment = NPC_GetPassengerSeatAttachment(pPassenger);
...
//-----------------------------------------------------------------------------
const PassengerSeatAnims_t *CBaseServerVehicle::NPC_GetPassengerSeatAnims(CBaseCombatCharacter *pPassenger,
...
bool CBaseServerVehicle::CheckExitPoint(float yaw, int distance, Vector *pEndPoint)
{
	...
	*pEndPoint = vecStart + vecDir * distance;
	...
	trace_t tr;
	UTIL_TraceHull(vecStart, *pEndPoint, VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, m_pVehicle, COLLISION_GROUP_NONE,
				   &tr);
...
bool CBaseServerVehicle::GetPassengerExitPoint(int nRole, Vector *pExitPoint, QAngle *pAngles)
{
	...
	if(pAnimating)
	{
		...
		if(pAnimating->GetAttachment("vehicle_driver_exit", vehicleExitOrigin, vehicleExitAngles))
		{
			...
			trace_t tr;
			UTIL_TraceHull(vehicleExitOrigin + Vector(0, 0, 12), vehicleExitOrigin, VEC_HULL_MIN, VEC_HULL_MAX,
	...
	// Make sure it's clear
	trace_t tr;
	UTIL_TraceHull(m_pVehicle->CollisionProp()->WorldSpaceCenter(), *pExitPoint, VEC_HULL_MIN, VEC_HULL_MAX,
				   MASK_PLAYERSOLID, m_pVehicle, COLLISION_GROUP_NONE, &tr);
	if(!tr.startsolid)
	{
		return true;
...
void CBaseServerVehicle::ParseNPCRoles(KeyValues *pkvPassengerList)
{
	...
	if(g_debug_npc_vehicle_roles.GetBool())
	{
		...
		for(int i = 0; i < m_PassengerRoles.Count(); i++)
		{
			Msg("\tPassenger Role:\t%s (%d seats)\n", STRING(m_PassengerRoles[i].m_strName),
			...
			for(int j = 0; j < m_PassengerRoles[i].m_PassengerSeats.Count(); j++)
			{
				...
				for(int nEntry = 0; nEntry < m_PassengerRoles[i].m_PassengerSeats[j].m_EntryTransitions.Count();
					nEntry++)
				{
				...
				for(int nExits = 0; nExits < m_PassengerRoles[i].m_PassengerSeats[j].m_ExitTransitions.Count();
					nExits++)
				{
					Msg("\t\t\tAnimation:\t%s\t(Priority %d)\n",
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::GetLocalAttachmentAtTime(int nQuerySequence, int nAttachmentIndex, float flCyclePoint,
												  Vector *vecOriginOut, QAngle *vecAnglesOut)
{
	CBaseAnimating *pAnimating = m_pVehicle->GetBaseAnimating();
	if(pAnimating == NULL)
		return false;

	// TODO: It's annoying to stomp and restore this off each time when we're just going to stomp it again later, but
	// the function
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::GetLocalAttachmentAtTime(const char *lpszAnimName, int nAttachmentIndex, float flCyclePoint,
												  Vector *vecOriginOut, QAngle *vecAnglesOut)
{
	CBaseAnimating *pAnimating = m_pVehicle->GetBaseAnimating();
	if(pAnimating == NULL)
...
void CBaseServerVehicle::CacheEntryExitPoints(void)
{
	...
	{
		if(GetLocalAttachmentAtTime(m_ExitAnimations[i].szAnimName, nAttachment, 1.0f,
									&m_ExitAnimations[i].vecExitPointLocal,
									&m_ExitAnimations[i].vecExitAnglesLocal) == false)
		{
...
void CBaseServerVehicle::ParseEntryExitAnims(void)
{
	...
	if(modelKeyValues->LoadFromBuffer(modelinfo->GetModelName(m_pVehicle->GetModel()),
	...
	if(pPlayer != NULL)
	{
		...
		if(GetDrivableVehicle()->CanEnterVehicle(pPlayer))
		{
			...
			if(pPlayer->CanEnterVehicle(this, VEHICLE_ROLE_DRIVER))
			{
				...
				pAnimating->InvalidateBoneCache(); // This is necessary because we need to query attachment points this
												   // frame for blending!
...
bool CBaseServerVehicle::HandlePassengerExit(CBaseCombatCharacter *pPassenger)
{
	...
	if(pPlayer != NULL)
	{
		...
		{
			// Animation-driven exit points are all blocked, or we have none. Fall back to the more simple static exit
			// points.
			if(!GetPassengerExitPoint(nRole, &vecNewPos, &angNewAngles) &&
			   !GetDrivableVehicle()->AllowBlockedExit(pPlayer, nRole))
				return false;

			// At this point, the player has exited the vehicle but did so without playing an animation.  We need to
			// give the vehicle a chance to do any post-animation clean-up it may need to perform.
...
int CBaseServerVehicle::GetExitAnimToUse(Vector &vecEyeExitEndpoint, bool &bAllPointsBlocked)
{
	...
	for(int i = 0; i < iCount; i++)
	{
		...
		// NOTE: HL2 and Ep1 used a method that relied on the animators to place attachment points in the model which
		// marked where
		//		 the player would exit to.  This was rendered unnecessary in Ep2, but the choreo vehicles of these older
		//products 		 did not have proper exit animations and relied on the exact queries that were happening before.  For
		//choreo vehicles, 		 we now just allow them to perform those older queries to keep those products happy.  - jdw
		...
		// NOTE: The hull has no vertical span because we want to test the lateral constraints against the ground, not
		// height (yet)
		trace_t tr;
		UTIL_TraceHull(vecStart, vecEnd, VEC_HULL_MIN, Vector(VEC_HULL_MAX.x, VEC_HULL_MAX.y, VEC_HULL_MIN.z),
					   MASK_PLAYERSOLID, NULL, COLLISION_GROUP_NONE, &tr);

		if(g_debug_vehicleexit.GetBool())
		{
			NDebugOverlay::SweptBox(vecStart, vecEnd, VEC_HULL_MIN,
									Vector(VEC_HULL_MAX.x, VEC_HULL_MAX.y, VEC_HULL_MIN.y), vec3_angle, 255, 255, 255,
									8.0f, 20.0f);
		}
		...
		if(tr.fraction < 1.0f)
		{
			...
			UTIL_TraceHull(tr.endpos, tr.endpos, VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, NULL,
...
void CBaseServerVehicle::ProcessMovement(CBasePlayer *pPlayer, CMoveData *pMoveData)
{
	...
	trace_t tr;
	UTIL_TraceLine(pPlayer->GetAbsOrigin(), pPlayer->GetAbsOrigin() - Vector(0, 0, 256), MASK_PLAYERSOLID,
...
void CBaseServerVehicle::SoundUpdate(vbs_sound_update_t &params)
{
	...
	if(m_vehicleSounds.pGears.Count() > 0)
	{
		if(flCurrentSpeed > m_flSpeedPercentage)
		{
			...
			flCurrentSpeed = Approach(flCurrentSpeed, m_flSpeedPercentage,
									  params.flFrameTime * m_vehicleSounds.pGears[m_iSoundGear].flSpeedApproachFactor);
		}
	}
...
//-----------------------------------------------------------------------------
int CBaseServerVehicle::NPC_GetAvailableSeat(CBaseCombatCharacter *pPassenger, string_t strRoleName,
