...
BEGIN_DATADESC(CBaseAnimating)
	...
	DEFINE_FIELD(m_flGroundSpeed, FIELD_FLOAT), DEFINE_FIELD(m_flLastEventCheck, FIELD_TIME),
		DEFINE_FIELD(m_bSequenceFinished, FIELD_BOOLEAN), DEFINE_FIELD(m_bSequenceLoops, FIELD_BOOLEAN),

		//	DEFINE_FIELD( m_nForceBone, FIELD_INTEGER ),
		//	DEFINE_FIELD( m_vecForce, FIELD_VECTOR ),

		DEFINE_INPUT(m_nSkin, FIELD_INTEGER, "skin"), DEFINE_KEYFIELD(m_nBody, FIELD_INTEGER, "body"),
		DEFINE_INPUT(m_nBody, FIELD_INTEGER, "SetBodyGroup"), DEFINE_KEYFIELD(m_nHitboxSet, FIELD_INTEGER, "hitboxset"),
		...
		DEFINE_CUSTOM_FIELD(m_pIk, &s_IKSaveRestoreOp), DEFINE_FIELD(m_iIKCounter, FIELD_INTEGER),
		DEFINE_FIELD(m_bClientSideAnimation, FIELD_BOOLEAN), DEFINE_FIELD(m_bClientSideFrameReset, FIELD_BOOLEAN),
		DEFINE_FIELD(m_nNewSequenceParity, FIELD_INTEGER), DEFINE_FIELD(m_nResetEventsParity, FIELD_INTEGER),
...
SendPropInt(SENDINFO(m_nForceBone), 8, 0), SendPropVector(SENDINFO(m_vecForce), -1, SPROP_NOSCALE),
	...
	SendPropArray3(SENDINFO_ARRAY3(m_flPoseParameter),
				   SendPropFloat(SENDINFO_ARRAY(m_flPoseParameter), ANIMATION_POSEPARAMETER_BITS, 0, 0.0f, 1.0f)),

	SendPropInt(SENDINFO(m_nSequence), ANIMATION_SEQUENCE_BITS, SPROP_UNSIGNED),
	SendPropFloat(SENDINFO(m_flPlaybackRate), ANIMATION_PLAYBACKRATE_BITS, SPROP_ROUNDUP, -4.0,
				  12.0f), // NOTE: if this isn't a power of 2 than "1.0" can't be encoded correctly

	SendPropArray3(SENDINFO_ARRAY3(m_flEncodedController),
				   SendPropFloat(SENDINFO_ARRAY(m_flEncodedController), 11, SPROP_ROUNDDOWN, 0.0f, 1.0f)),
	...
	// Fading
	SendPropFloat(SENDINFO(m_fadeMinDist), 0, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_fadeMaxDist), 0, SPROP_NOSCALE),
...
void CBaseAnimating::StudioFrameAdvance()
{
	...
	{
		Msg("%5.2f : %s : %s : %5.3f\n", gpGlobals->curtime, GetClassname(), GetSequenceName(GetSequence()),
			GetCycle());
	}
}
	...
	else
	{
		...
		if(!pLightingOrigin)
		{
			DevWarning("%s: Could not find info_lighting_relative '%s'!\n", GetClassname(),
					   STRING(strLightingOriginRelative));
			return;
		}
...
int CBaseAnimating::SelectWeightedSequenceFromModifiers(Activity activity, CUtlSymbol *pActivityModifiers,
														int iModifierCount)
{
	Assert(activity != ACT_INVALID);
	AssertMsg(GetModelPtr(), "GetModelPtr NULL. %s", STRING(GetEntityName()) ? STRING(GetEntityName()) : "");
...
{
	Assert(nSequence == 0 || IsDynamicModelLoading() ||
		   (GetModelPtr() && (nSequence < GetModelPtr()->GetNumSeq()) &&
...
float CBaseAnimating::GetLastVisibleCycle(CStudioHdr *pStudioHdr, int iSequence)
{
	...
	{
		return 1.0f -
			   (pStudioHdr->pSeqdesc(iSequence).fadeouttime) * GetSequenceCycleRate(iSequence) * m_flPlaybackRate;
	}
	else
	{
		return 1.0;
...
void CBaseAnimating::DispatchAnimEvents(CBaseAnimating *eventHandler)
{
	...
	{
		Msg( "%s:%s : checking %.2f %.2f (%d)\n", STRING(GetModelName()), pstudiohdr->pSeqdesc( GetSequence()
		...
		else if(pEvent->event == AE_SV_DUSTTRAIL)
		{
			...
			if(sscanf(pEvent->options, "%s %f %f", szAttachment, &flDuration, &flSize) == 3)
			{
				...
				if(hDustTrail)
				{
					...
					hDustTrail->m_SpawnRadius =
						3;						 // Each particle randomly offset from the center up to this many units
...
void CBaseAnimating::UpdateStepOrigin()
{
	...
	*/
	...
	{
		if((GetFlags() & (FL_FLY | FL_SWIM)) == 0 && GetMoveParent() == NULL && GetGroundEntity() != NULL &&
		...
		for(int i = 0; i < m_pIk->m_target.Count(); i++)
		{
			...
			switch(pTarget->type)
			{
				case IK_GROUND:
				{
					...
					if ( debugoverlay )
					{
						debugoverlay->AddBoxOverlay( p1, Vector(-r,-r,0), Vector(r,r,1), QAngle( 0, 0, 0 ), 255, 0, 0,
					0, 1.0f ); debugoverlay->AddBoxOverlay( trace.endpos, Vector(-r,-r,0), Vector(r,r,1), QAngle( 0, 0,
					0 ), 255, 0, 0, 0, 1.0f ); debugoverlay->AddLineOverlay( p1, trace.endpos, 255, 0, 0, 0, 1.0f );
					}
					*/
...
void CBaseAnimating::BuildMatricesWithBoneMerge(const CStudioHdr *pStudioHdr, const QAngle &angles,
...
inline bool CBaseAnimating::CanSkipAnimation(void)
{
	...
	if(pNPC && !pNPC->HasCondition(COND_IN_PVS) &&
	   (m_fBoneCacheFlags & (BCF_NO_ANIMATION_SKIP | BCF_IS_IN_SPAWN)) == false)
	{
...
void CBaseAnimating::SetupBones(matrix3x4_t *pBoneToWorld, int boneMask)
{
	...
	if(pParent)
	{
		...
		if(pParentCache)
		{
			BuildMatricesWithBoneMerge(pStudioHdr, GetAbsAngles(), adjOrigin, pos, q, pBoneToWorld, pParent,
									   pParentCache);
...
bool CBaseAnimating::GotoSequence(int iCurrentSequence, float flCurrentCycle, float flCurrentRate, int iGoalSequence,
								  int &nNextSequence, float &flNextCycle, int &iNextDir)
{
	return ::GotoSequence(GetModelPtr(), iCurrentSequence, flCurrentCycle, flCurrentRate, iGoalSequence, nNextSequence,
...
// Output :
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetIntervalMovement(float flIntervalUsed, bool &bMoveSeqFinished, Vector &newPosition,
										 QAngle &newAngles)
{
	CStudioHdr *pstudiohdr = GetModelPtr();
	if(!pstudiohdr || !pstudiohdr->SequencesAvailable())
	...
	if(Studio_SeqMovement(pstudiohdr, GetSequence(), GetCycle(), flNextCycle, GetPoseParameterArray(), deltaPos,
						  deltaAngles))
	{
...
// Output :
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetSequenceMovement(int nSequence, float fromCycle, float toCycle, Vector &deltaPosition,
										 QAngle &deltaAngles)
{
	CStudioHdr *pstudiohdr = GetModelPtr();
	if(!pstudiohdr)
	...
	return Studio_SeqMovement(pstudiohdr, nSequence, fromCycle, toCycle, GetPoseParameterArray(), deltaPosition,
							  deltaAngles);
}
		...
		if(hStudioHdr != MDLHANDLE_INVALID)
		{
			...
			if(pStudioHdrContainer && pStudioHdrContainer->GetVirtualModel())
			{
				MDLHandle_t hVirtualModel =
					(MDLHandle_t)(int)(pStudioHdrContainer->GetRenderHdr()->virtualModel) & 0xffff;
...
CBoneCache *CBaseAnimating::GetBoneCache(void)
{
	...
	if(pcache)
	{
		if(pcache->IsValid(gpGlobals->curtime) && (pcache->m_boneMask & boneMask) == boneMask &&
		   pcache->m_timeValid <= gpGlobals->curtime)
		{
			// Msg("%s:%s:%s (%x:%x:%8.4f) cache\n", GetClassname(), GetDebugName(), STRING(GetModelName()), boneMask,
			// pcache->m_boneMask, pcache->m_timeValid ); in memory and still valid, use it!
			return pcache;
		}
	...
	if(GetModelPtr() && sscanf(sInputString, "%d", &iAttachment))
	{
		if(!GetAttachment(iAttachment, pOrigin, pAngles))
		{
			Msg("ERROR: Mapmaker tried to spawn DispatchEffect %s, but %s has no attachment %d\n", sInputString,
				STRING(GetModelName()), iAttachment);
		}
		return;
	}
...
void CBaseAnimating::DrawServerHitboxes(float duration /*= 0.0f*/, bool monocolor /*= false*/)
{
	...
	for(int i = 0; i < set->numhitboxes; i++)
	{
		...
		NDebugOverlay::BoxAngles(position, pbox->bbmin * GetModelScale(), pbox->bbmax * GetModelScale(), angles, r, g,
								 b, 0, duration);
	}
}

void CBaseAnimating::DrawRawSkeleton(matrix3x4_t boneToWorld[], int boneMask, bool noDepthTest, float duration,
									 bool monocolor)
{
	CStudioHdr *pStudioHdr = GetModelPtr();
	if(!pStudioHdr)
...
bool CBaseAnimating::ComputeHitboxSurroundingBox(Vector *pVecWorldMins, Vector *pVecWorldMaxs)
{
	...
	for(int i = 0; i < set->numhitboxes; i++)
	{
		...
		if(pMatrix)
		{
			TransformAABB(*pMatrix, pbox->bbmin * GetModelScale(), pbox->bbmax * GetModelScale(), vecBoxAbsMins,
						  vecBoxAbsMaxs);
							  ...
							  Vector vDissolverOrigin, int iMagnitude)
{
	...
	CEntityDissolve *pDissolve =
		CEntityDissolve::Create(this, pMaterialName, flStartTime, nDissolveType, &bRagdollCreated);
	if(pDissolve)
	{
...
//-----------------------------------------------------------------------------
// Purpose: Async prefetches all anim data used by a particular sequence.  Returns true if all of the required data is
// memory resident Input  : iSequence - Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
