...
SendPropFloat(SENDINFO(m_SpreadSpeed), 0, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_Speed), 0, SPROP_NOSCALE),
	...
	SendPropInt(SENDINFO(m_bEmit), 1, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_flBaseSpread), 0, SPROP_NOSCALE),
	...
	SendPropVector(SENDINFO(m_vWind), 0, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_flTwist), 0, SPROP_NOSCALE),
...
BEGIN_SIMPLE_DATADESC(CSmokeStackLightInfo)
	DEFINE_FIELD(m_vPos, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vColor, FIELD_VECTOR),
...
BEGIN_DATADESC(CSmokeStack)
	...
	DEFINE_KEYFIELD(m_StartSize, FIELD_FLOAT, "StartSize"), DEFINE_KEYFIELD(m_EndSize, FIELD_FLOAT, "EndSize"),
		...
		DEFINE_KEYFIELD(m_flBaseSpread, FIELD_FLOAT, "BaseSpread"), DEFINE_KEYFIELD(m_flTwist, FIELD_FLOAT, "Twist"),
...
{
	m_vWind = Vector(cos(DEG2RAD((float)m_WindAngle)) * m_WindSpeed, sin(DEG2RAD((float)m_WindAngle)) * m_WindSpeed, 0);
}