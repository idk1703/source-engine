...
BEGIN_DATADESC(CGravityPellet)
	...
	DEFINE_PHYSPTR(m_pConstraint), DEFINE_FIELD(m_isInert, FIELD_BOOLEAN),
...
public:
	...
	void SetAutoAlign(const Vector &localDir, const Vector &localPos, const Vector &worldAlignDir,
...
BEGIN_SIMPLE_DATADESC(CGravControllerPoint)
	...
	DEFINE_FIELD(m_localPosition, FIELD_VECTOR), DEFINE_FIELD(m_targetPosition, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_worldPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_localAlignNormal, FIELD_VECTOR),
		DEFINE_FIELD(m_localAlignPosition, FIELD_VECTOR), DEFINE_FIELD(m_targetAlignNormal, FIELD_VECTOR),
		DEFINE_FIELD(m_targetAlignPosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_align, FIELD_BOOLEAN),
		DEFINE_FIELD(m_saveDamping, FIELD_FLOAT), DEFINE_FIELD(m_maxVel, FIELD_FLOAT),
		DEFINE_FIELD(m_maxAcceleration, FIELD_FLOAT), DEFINE_FIELD(m_maxAngularAcceleration, FIELD_VECTOR),
		DEFINE_FIELD(m_attachedEntity, FIELD_EHANDLE), DEFINE_FIELD(m_targetRotation, FIELD_VECTOR),
...
IMotionEvent::simresult_e CGravControllerPoint::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject,
	...
	if(m_align)
	{
		...
		if(len > 0.08)
		{
			...
			if(rotRadius > 0.1)
			{
				...
				//				NDebugOverlay::Line( worldRotCenter, worldRotCenter-m_targetAlignNormal*50, 255, 0, 0, false,
				//0.1 ); 				NDebugOverlay::Line( worldRotCenter, worldRotCenter+tangent*50, 0, 255, 0, false, 0.1 );
...
SendPropVector(SENDINFO_NAME(m_gravCallback.m_targetPosition, m_targetPosition), -1, SPROP_COORD),
	...
	SendPropInt(SENDINFO(m_active), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_glueTouching), 1, SPROP_UNSIGNED),
...
BEGIN_DATADESC(CWeaponGravityGun)
	...
	DEFINE_FIELD(m_active, FIELD_INTEGER), DEFINE_FIELD(m_useDown, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hObject, FIELD_EHANDLE), DEFINE_FIELD(m_distance, FIELD_FLOAT),
		DEFINE_FIELD(m_movementLength, FIELD_FLOAT), DEFINE_FIELD(m_lastYaw, FIELD_FLOAT),
		DEFINE_FIELD(m_soundState, FIELD_INTEGER), DEFINE_FIELD(m_viewModelIndex, FIELD_INTEGER),
		DEFINE_FIELD(m_originalObjectPosition, FIELD_POSITION_VECTOR), DEFINE_EMBEDDED(m_gravCallback),
		// Physptrs can't be saved in embedded classes..
		DEFINE_PHYSPTR(m_gravCallback.m_controller), DEFINE_EMBEDDED_AUTO_ARRAY(m_activePellets),
		DEFINE_FIELD(m_pelletCount, FIELD_INTEGER), DEFINE_FIELD(m_objectPelletCount, FIELD_INTEGER),
		DEFINE_FIELD(m_pelletHeld, FIELD_INTEGER), DEFINE_FIELD(m_pelletAttract, FIELD_INTEGER),
		DEFINE_FIELD(m_glueTime, FIELD_TIME), DEFINE_FIELD(m_glueTouching, FIELD_BOOLEAN),
...
void CWeaponGravityGun::EffectUpdate(void)
{
	...
	if(m_pelletHeld >= 0 && gpGlobals->curtime > m_glueTime)
	{
		...
		m_gravCallback.SetAutoAlign(m_activePellets[m_pelletHeld].localNormal,
									m_activePellets[m_pelletHeld].pellet->GetLocalOrigin(), worldNormal, worldPos);
	}
	else
	{
...
void CWeaponGravityGun::AttachObject(CBaseEntity *pObject, const Vector &start, const Vector &end, float distance)
{
	...
	if(pPhysics && pObject->GetMoveType() == MOVETYPE_VPHYSICS)
	{
		...
		//		Msg( "Object mass: %.2f lbs (%.2f kg) %f %f %f\n", kg2lbs(mass), mass, pObject->GetAbsOrigin().x,
		//pObject->GetAbsOrigin().y, pObject->GetAbsOrigin().z ); 		Msg( "ANG: %f %f %f\n", pObject->GetAbsAngles().x,
		//pObject->GetAbsAngles().y, pObject->GetAbsAngles().z );
