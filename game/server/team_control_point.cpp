...
BEGIN_DATADESC(CTeamControlPoint)
	DEFINE_KEYFIELD(m_iszPrintName, FIELD_STRING, "point_printname"),
		...
		DEFINE_OUTPUT(m_OnOwnerChangedToTeam1,
					  "OnOwnerChangedToTeam1"), // these are fired when a team does the work to change the owner
		DEFINE_OUTPUT(m_OnOwnerChangedToTeam2, "OnOwnerChangedToTeam2"),

		DEFINE_OUTPUT(m_OnRoundStartOwnedByTeam1,
					  "OnRoundStartOwnedByTeam1"), // these are fired when a round is starting
...
{
	if(m_flUnlockTime > 0 && m_flUnlockTime < gpGlobals->curtime &&
