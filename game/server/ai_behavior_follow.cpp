...
public:
	...
	bool AddFollower(CBaseEntity *pTarget, CAI_BaseNPC *pFollower, AI_Formations_t formation,
...
BEGIN_SIMPLE_DATADESC(AI_FollowNavInfo_t)
	DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(position, FIELD_POSITION_VECTOR), DEFINE_FIELD(range, FIELD_FLOAT),
		DEFINE_FIELD(Zrange, FIELD_FLOAT), DEFINE_FIELD(tolerance, FIELD_FLOAT),
		DEFINE_FIELD(followPointTolerance, FIELD_FLOAT), DEFINE_FIELD(targetMoveTolerance, FIELD_FLOAT),
		DEFINE_FIELD(repathOnRouteTolerance, FIELD_FLOAT), DEFINE_FIELD(walkTolerance, FIELD_FLOAT),
		DEFINE_FIELD(coverTolerance, FIELD_FLOAT), DEFINE_FIELD(enemyLOSTolerance, FIELD_FLOAT),
		DEFINE_FIELD(chaseEnemyTolerance, FIELD_FLOAT),
END_DATADESC();

BEGIN_SIMPLE_DATADESC(AI_FollowParams_t)
	DEFINE_FIELD(formation, FIELD_INTEGER), DEFINE_FIELD(bNormalMemoryDiscard, FIELD_BOOLEAN),

END_DATADESC();

BEGIN_DATADESC(CAI_FollowBehavior)
	DEFINE_FIELD(m_hFollowTarget, FIELD_EHANDLE), DEFINE_EMBEDDED(m_FollowNavGoal),
		DEFINE_FIELD(m_flTimeUpdatedFollowPosition, FIELD_TIME), DEFINE_FIELD(m_bFirstFacing, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flTimeFollowTargetVisible, FIELD_TIME), DEFINE_EMBEDDED(m_TargetMonitor),
		DEFINE_FIELD(m_bTargetUnreachable, FIELD_BOOLEAN), DEFINE_FIELD(m_bFollowNavFailed, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bMovingToCover, FIELD_BOOLEAN), DEFINE_FIELD(m_flOriginalEnemyDiscardTime, FIELD_FLOAT),
		DEFINE_FIELD(m_SavedDistTooFar, FIELD_FLOAT), DEFINE_EMBEDDED(m_FollowDelay),
		DEFINE_EMBEDDED(m_RepathOnFollowTimer), DEFINE_CUSTOM_FIELD(m_CurrentFollowActivity, ActivityDataOps()),
		DEFINE_EMBEDDED(m_TimeBlockUseWaitPoint), DEFINE_EMBEDDED(m_TimeCheckForWaitPoint),
		DEFINE_FIELD(m_pInterruptWaitPoint, FIELD_CLASSPTR), DEFINE_EMBEDDED(m_TimeBeforeSpreadFacing),
		...
		DEFINE_FIELD(m_nFailedFollowAttempts, FIELD_INTEGER), DEFINE_FIELD(m_flTimeFailFollowStarted, FIELD_TIME),
...
int CAI_FollowBehavior::DrawDebugTextOverlays(int text_offset)
{
	...
	if(GetOuter()->m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		if(followEnt != NULL)
		{
			Q_snprintf(tempstr, sizeof(tempstr), "Follow: (%d) %s (%s)", followEnt->entindex(),
					   followEnt->GetDebugName(), followEnt->GetClassname());
		}
		else
		{
...
bool CAI_FollowBehavior::SetFollowGoal(CAI_FollowGoal *pGoal, bool fFinishCurSchedule)
{
	if(GetOuter()->ShouldAcceptGoal(this, pGoal))
	{
		...
		Assert(pGoal->m_iFormation == AIF_SIMPLE || pGoal->m_iFormation == AIF_WIDE ||
			   pGoal->m_iFormation == AIF_MEDIUM || pGoal->m_iFormation == AIF_SIDEKICK ||
...
{
	return (IsRunning() &&
			(IsCurSchedule(SCHED_FOLLOW, false) || IsCurSchedule(SCHED_FOLLOWER_GO_TO_WAIT_POINT, false)));
}
...
bool CAI_FollowBehavior::IsFollowTargetInRange(float rangeMultiplier)
{
	...
	{
		if(IsFollowGoalInRange(MAX(m_FollowNavGoal.coverTolerance, m_FollowNavGoal.enemyLOSTolerance) * rangeMultiplier,
	...
	{
		if(IsFollowGoalInRange(MAX(m_FollowNavGoal.tolerance, GetGoalRange()) * rangeMultiplier, GetGoalZRange(),
							   GetGoalFlags()))
		{
...
{
	if(GetEnemy() && (GetEnemy()->WorldSpaceCenter() - m_FollowNavGoal.position).LengthSqr() >
...
CAI_Hint *CAI_FollowBehavior::FindFollowPoint()
{
	...
	hintCriteria.AddExcludePosition(
		GetGoalPosition(),
		(GetFollowTarget()->WorldAlignMins().AsVector2D() - GetFollowTarget()->WorldAlignMaxs().AsVector2D()).Length());
...
{
	return (GetHintNode() && GetHintNode()->HintType() == HINT_FOLLOW_WAIT_POINT &&
			(GetHintNode()->GetAbsOrigin() - GetFollowTarget()->GetAbsOrigin()).LengthSqr() <
				Square(MAX(m_FollowNavGoal.followPointTolerance, GetGoalRange())));
}
...
int CAI_FollowBehavior::SelectScheduleFollowPoints()
{
	bool bShouldUseFollowPoints =
		(ShouldUseFollowPoints() &&
		 IsFollowGoalInRange(m_FollowNavGoal.followPointTolerance + 0.1, GetGoalZRange(), GetGoalFlags()));
	...
	if(bHasFollowPoint)
	{
		...
		if(!bShouldUseFollowPoints || distSqToPoint > Square(2.0 * GetHullWidth()) ||
...
int CAI_FollowBehavior::SelectScheduleMoveToFormation()
{
	   ...
	   !IsFollowGoalInRange(GetGoalRange(), GetGoalZRange(), GetGoalFlags()))
	{
		...
		if(pSquad)
		{
			for(CAI_BaseNPC *pSquadMember = pSquad->GetFirstMember(&iter); pSquadMember;
				pSquadMember = pSquad->GetNextMember(&iter))
			{
		...
		{
			return SCHED_TARGET_FACE; // Code for "SCHED_MOVE_TO_FACE_FOLLOW_TARGET". Used by Talker clients to
									  // interject comment
		}
	}
	return SCHED_NONE;
			...
			else if(m_hFollowTarget && !bFollowingPoint)
			{
				...
				else if(m_TimeNextSpreadFacing.Expired())
				{
					...
					bIsEpisodicVitalAlly =
						(hl2_episodic.GetBool() && GetOuter()->Classify() == CLASS_PLAYER_ALLY_VITAL);
#else
					...
					else
					{
						...
						else
						{
							...
							if(count > 0)
							{
								// Slice up the directions among followers and leader. ( +1 because we count the
								// leader!)
		...
		case TASK_GET_PATH_TO_FOLLOW_POSITION:
		{
			switch(GetOuter()->GetTaskInterrupt())
			{
				...
				case 1:
				{
					...
					{
						AI_NavGoal_t goal(GOALTYPE_COVER, GetOuter()->m_vInterruptSavePosition, ACT_RUN,
		...
		case TASK_MOVE_TO_FOLLOW_POSITION:
		{
			...
			else
			{
					...
					if(GetNavigator()->GetGoalPos() != vFollowPoint)
					{
						...
						if(GetHintNode()->HintActivityName() != NULL_STRING)
						{
							Activity hintActivity =
								(Activity)CAI_BaseNPC::GetActivityID(STRING(GetHintNode()->HintActivityName()));
							if(hintActivity != ACT_INVALID)
							{
								GetNavigator()->SetArrivalActivity(
									GetOuter()->GetHintActivity(GetHintNode()->HintType(), hintActivity));
							}
							else
							{
				...
				if(m_CurrentFollowActivity != ACT_RUN)
				{
					...
					Activity followActivity = (distToTargetSq < Square(m_FollowNavGoal.walkTolerance) &&
...
void CAI_FollowBehavior::BuildScheduleTestBits()
{
	...
	if((GetOuter()->ConditionInterruptsCurSchedule(COND_GIVE_WAY) ||
		...
		(bIsReload = IsCurSchedule(SCHED_RELOAD)) == true || IsCurSchedule(SCHED_STANDOFF) ||
		(bIsTakeCover = IsCurSchedule(SCHED_TAKE_COVER_FROM_ENEMY)) == true || IsCurSchedule(SCHED_COMBAT_FACE) ||
		IsCurSchedule(SCHED_ALERT_FACE) || IsCurSchedule(SCHED_COMBAT_STAND) || IsCurSchedule(SCHED_ALERT_STAND)) ||
	   IsCurSchedule(SCHED_ALERT_FACE_BESTSOUND))
	{
		...
		if(!bIgnoreMovedMark)
		{
			GetOuter()->SetCustomInterruptCondition(
				GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_TARGET_MOVED_FROM_MARK));
		}

		if(!bIsTakeCover && !bIsHideAndReload && !bIsReload)
			GetOuter()->SetCustomInterruptCondition(
				GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_FOLLOW_DELAY_EXPIRED));
	}
	...
	if(hl2_episodic.GetBool())
	{
		...
		if(HL2GameRules()->IsAlyxInDarknessMode())
		{
			...
			{
				GetOuter()->SetCustomInterruptCondition(
					GetClassScheduleIdSpace()->ConditionLocalToGlobal(COND_FOLLOW_PLAYER_IS_NOT_LIT));
			}
		}
#endif // HL2_EPISODIC
	}
...
Activity CAI_FollowBehavior::NPC_TranslateActivity(Activity activity)
{
	if(activity == ACT_IDLE && HasFollowPoint() && GetHintNode()->HintActivityName() != NULL_STRING)
	{
		return GetOuter()->GetHintActivity(
			GetHintNode()->HintType(), (Activity)CAI_BaseNPC::GetActivityID(STRING(GetHintNode()->HintActivityName())));
	}
...
static AI_FollowSlot_t g_SimpleFollowFormationSlots[] = {
	{1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
	{1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
	{1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
	{1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
	{1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
...
static AI_FollowSlot_t g_WideFollowFormationSlots[] = {
	{1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
	{1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
	{1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
	{1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
	{1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
...
static AI_FollowSlot_t g_AntlionFollowFormationSlots[] = {
	{1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
	{1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
	{1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
...
static AI_FollowSlot_t g_MediumFollowFormationSlots[] = {
	{1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
	{1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
	{1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
	{1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
	{1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
...
static AI_FollowSlot_t g_SidekickFollowFormationSlots[] = {
	{1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128},
	{1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128},
	{1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128},
	{1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128},
	{1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128}, {1, {0, 0, 0}, 0, 120, 160, 256, 128},
...
AI_FollowFormation_t *g_AI_Formations[] = {
	&g_SimpleFollowFormation,	 &g_WideFollowFormation,   &g_AntlionFollowFormation,
	&g_CommanderFollowFormation, &g_TightFollowFormation,  &g_MediumFollowFormation,
	&g_SidekickFollowFormation,	 &g_HunterFollowFormation, &g_VortigauntFollowFormation,
};
...
bool CAI_FollowManager::AddFollower(CBaseEntity *pTarget, CAI_BaseNPC *pFollower, AI_Formations_t formation,
									AI_FollowManagerInfoHandle_t *pHandle)
{
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_FollowBehavior)
	...
	DEFINE_SCHEDULE(SCHED_FOLLOWER_GO_TO_WAIT_POINT,
					...
					"	Tasks"
					"		TASK_LOCK_HINTNODE			0		" // this will fail the schedule if no hint node or not
																  // already lockable
