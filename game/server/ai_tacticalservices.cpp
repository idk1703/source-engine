...
#define DebugFindCover(node, from, to, r, g, b)                                                         \
	if(!ai_debug_cover.GetBool() || (g_AIDebugFindCoverNode != -1 && g_AIDebugFindCoverNode != node) || \
...
#define DebugFindCover2(node, from, to, r, g, b)                                                          \
	if(ai_debug_cover.GetInt() < 2 || (g_AIDebugFindCoverNode != -1 && g_AIDebugFindCoverNode != node) || \
	   !GetOuter()->m_bSelected)                                                                          \
		;                                                                                                 \
	else                                                                                                  \
		NDebugOverlay::Line(from, to, r, g, b, false, 1)
...
#define ShouldDebugLos(node)                                                                              \
	(ai_debug_tactical_los.GetBool() && (g_AIDebugFindLosNode == -1 || g_AIDebugFindLosNode == (node)) && \
	 GetOuter()->m_bSelected)
#else
...
bool CAI_TacticalServices::FindLos(const Vector &threatPos, const Vector &threatEyePos, float minThreatDist,
								   float maxThreatDist, float blockTime, FlankType_t eFlankType,
								   const Vector &vecFlankRefPos, float flFlankParam, Vector *pResult)
{
	AI_PROFILE_SCOPE(CAI_TacticalServices_FindLos);

	MARK_TASK_EXPENSIVE();

	int node = FindLosNode(threatPos, threatEyePos, minThreatDist, maxThreatDist, blockTime, eFlankType, vecFlankRefPos,
						   flFlankParam);
...
bool CAI_TacticalServices::FindLos(const Vector &threatPos, const Vector &threatEyePos, float minThreatDist,
								   float maxThreatDist, float blockTime, Vector *pResult)
{
	return FindLos(threatPos, threatEyePos, minThreatDist, maxThreatDist, blockTime, FLANKTYPE_NONE, vec3_origin, 0,
				   pResult);
}
...
bool CAI_TacticalServices::FindCoverPos(const Vector &vThreatPos, const Vector &vThreatEyePos, float flMinDist,
...
bool CAI_TacticalServices::FindCoverPos(const Vector &vNearPos, const Vector &vThreatPos, const Vector &vThreatEyePos,
	...
	if((vecCheckStart - vecCheckEnd).LengthSqr() > Square(flMinDist))
	{
		if(GetOuter()->IsCoverPosition(vecCheckStart, vecCheckEnd + GetOuter()->GetViewOffset()))
		{
			if(GetOuter()->IsValidCover(vecCheckEnd, NULL))
			{
				...
				GetOuter()->GetMoveProbe()->MoveLimit(NAV_GROUND, GetLocalOrigin(), vecCheckEnd, MASK_NPCSOLID, NULL,
													  &moveTrace);
...
bool CAI_TacticalServices::FindLateralCover(const Vector &vecThreat, float flMinDist, float distToCheck,
...
bool CAI_TacticalServices::FindLateralCover(const Vector &vNearPos, const Vector &vecThreat, float flMinDist,
											float distToCheck, int numChecksPerDir, Vector *pResult)
{
...
int CAI_TacticalServices::FindCoverNode(const Vector &vThreatPos, const Vector &vThreatEyePos, float flMinDist,
										float flMaxDist)
{
...
int CAI_TacticalServices::FindCoverNode(const Vector &vNearPos, const Vector &vThreatPos, const Vector &vThreatEyePos,
		...
		if(dist >= flMinDistSqr && dist < flMaxDistSqr)
		{
			...
			if(GetOuter()->IsValidCover(nodeOrigin, pNode->GetHint()))
			{
				...
				if(GetOuter()->IsCoverPosition(vThreatEyePos, vEyePos))
				{
					...
					if(pNode->GetHint() && (pNode->GetHint()->HintType() == HINT_TACTICAL_COVER_MED ||
											pNode->GetHint()->HintType() == HINT_TACTICAL_COVER_LOW))
					{
						if(GetOuter()->GetHintNode())
						{
							GetOuter()->GetHintNode()->Unlock(
								GetOuter()->GetHintDelay(GetOuter()->GetHintNode()->HintType()));
...
int CAI_TacticalServices::FindLosNode(const Vector &vThreatPos, const Vector &vThreatEyePos, float flMinThreatDist,
									  float flMaxThreatDist, float flBlockTime, FlankType_t eFlankType,
									  const Vector &vecFlankRefPos, float flFlankParam)
{
	if(!CAI_NetworkManager::NetworksLoaded())
		return NO_NODE;
	...
	while(list.Count())
	{
		...
		if(nodeIndex != iMyNode)
		{
			   ...
			   GetNetwork()->GetNode(nodeIndex)->GetType() != NODE_CLIMB)
			{
				...
				if(flThreatDist < flMaxThreatDist && flThreatDist > flMinThreatDist)
				{
					CAI_Node *pNode = GetNetwork()->GetNode(nodeIndex);
...
bool CAI_TacticalServices::TestLateralLos(const Vector &vecCheckStart, const Vector &vecCheckEnd)
{
	...
	// it's faster to check the SightEnt's visibility to the potential spot than to check the local move, so we do that
	// first.
...
bool CAI_TacticalServices::FindLateralLos(const Vector &vecThreat, Vector *pResult)
{
	...
	if(!bLookingForEnemy || GetOuter()->HasCondition(COND_SEE_ENEMY) || GetOuter()->HasCondition(COND_HAVE_ENEMY_LOS) ||
	   GetOuter()->GetTimeScheduleStarted() ==
		   gpGlobals->curtime) // Conditions get nuked before tasks run, assume should try
	{
