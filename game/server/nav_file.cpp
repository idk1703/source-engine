...
NavErrorType CNavArea::Load(CUtlBuffer &fileBuffer, unsigned int version, unsigned int subVersion)
{
	...
	else
	{
		/* TODO: Re-enable when latest 360 code gets integrated (MSB 5/5/09)
				...
				new ( &m_potentiallyVisibleAreas ) CAreaBindInfoArray( (AreaBindInfo *)engine->AllocLevelStaticData(
		   nBytes ), visibleAreaCount );
		*/
	}
...
static ConCommand nav_check_file_consistency(
	...
	"Scans the maps directory and reports any missing/out-of-date navigation files.", FCVAR_GAMEDLL | FCVAR_CHEAT);
	...
	FOR_EACH_VEC(TheNavAreas, oit)
	{
		...
		for(int d = 0; d < NUM_DIRECTIONS; d++)
		{
			...
			FOR_EACH_VEC((*connectList), it)
			{
				...
				// if the area we connect to has no connection back to us, allow that area to remember us as an incoming
				// connection
				oneWayLink.backD = OppositeDirection((NavDirType)d);
				const NavConnectVector *backConnectList =
					oneWayLink.destArea->GetAdjacentAreas((NavDirType)oneWayLink.backD);
				bool isOneWay = true;
