...
void NWCEdit::CreateAINode(CBasePlayer *pPlayer)
{
	...
	if(g_pAINetworkManager->GetEditOps()->m_bAirEditMode)
	{
		...
		trace_t tr;
		UTIL_TraceLine(pPlayer->EyePosition(), vNewNodePos, MASK_NPCSOLID_BRUSHONLY, pPlayer, COLLISION_GROUP_NONE,
					   &tr);
		if(tr.fraction == 1.0)
		{
			bPositionValid = true;
	...
	if(bPositionValid)
	{
		...
		vUpBit.z += 1;
		trace_t tr;
		UTIL_TraceHull(testHull->GetAbsOrigin(), vUpBit, NAI_Hull::Mins(hullType), NAI_Hull::Maxs(hullType),
					   MASK_NPCSOLID, testHull, COLLISION_GROUP_NONE, &tr);
		...
		testHull->SetLocalOrigin(Vector(floor(testHull->GetAbsOrigin().x), floor(testHull->GetAbsOrigin().y),
		...
		if(g_pAINetworkManager->GetEditOps()->m_bAirEditMode)
		{
			status = Editor_CreateNode("info_node_air", g_pAINetworkManager->GetEditOps()->m_nNextWCIndex,
		...
		else
		{
			...
			status = Editor_CreateNode("info_node", g_pAINetworkManager->GetEditOps()->m_nNextWCIndex,
									   testHull->GetLocalOrigin().x, testHull->GetLocalOrigin().y,
									   testHull->GetLocalOrigin().z, false);
		}
...
void NWCEdit::UndoDestroyAINode(void)
{
	...
	if(g_pAINetworkManager->GetEditOps()->m_pLastDeletedNode)
	{
		...
		int nOldWCID = g_pAINetworkManager->GetEditOps()
						   ->m_pNodeIndexTable[g_pAINetworkManager->GetEditOps()->m_pLastDeletedNode->GetId()];
...
void NWCEdit::RememberEntityPosition(CBaseEntity *pEntity)
{
	...
	if(!g_EntityPositions)
	{
		...
		// have to save these too because some entities change the classname on spawn (e.g. prop_physics_override,
		// physics_prop)
...
void NWCEdit::UpdateEntityPosition(CBaseEntity *pEntity)
{
	...
	DevMsg(1, "%s\n   origin %f %f %f\n   angles %f %f %f\n", pEntity->GetClassname(), newPos.x, newPos.y, newPos.z,
		   newAng.x, newAng.y, newAng.z);
	...
	else
	{
		...
		result = Editor_SetKeyValue(pClassname, pos.x, pos.y, pos.z, "angles",
									CFmtStr("%f %f %f", newAng.x, newAng.y, newAng.z));
	}
	if(result != Editor_OK)
		goto error;

	result = Editor_SetKeyValue(pClassname, pos.x, pos.y, pos.z, "origin",
...
static ConCommand wc_create("wc_create", CC_WC_Create,
							"When in WC edit mode, creates a node where the player is looking if a node is allowed at "
							"that location for the currently selected hull size (see ai_next_hull)",
							FCVAR_CHEAT);
...
static ConCommand wc_destroy("wc_destroy", CC_WC_Destroy,
							 "When in WC edit mode, destroys the node that the player is nearest to looking at.  (The "
							 "node will be highlighted by a red box).",
							 FCVAR_CHEAT);
...
static ConCommand wc_destroy_undo("wc_destroy_undo", CC_WC_DestroyUndo,
...
static ConCommand wc_air_edit_further("wc_air_edit_further", CC_WC_AirNodeEditFurther,
...
static ConCommand wc_air_edit_nearer("wc_air_edit_nearer", CC_WC_AirNodeEditNearer,
									 "When in WC edit mode and editing air nodes,  moves position of air node "
...
CON_COMMAND(hammer_update_safe_entities,
			"Updates entities in the map that can safely be updated (don't have parents or are affected by "
			"constraints). Also excludes entities mentioned in any hammer_updateignorelist objects in this map.")
{
	int iCount = 0;
	CBaseEntity *pEnt = NULL;
