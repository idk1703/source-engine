...
#pragma GCC diagnostic ignored \
	"-Wdangling-else" // warning: add explicit braces to avoid dangling else [-Wdangling-else]
#endif
...
#define FINDNAMEDENTITY_MAX_ENTITIES \
	32 // max number of entities to be considered for random entity selection in FindNamedEntity
...
ConVar ai_frametime_limit("ai_frametime_limit", "50", FCVAR_NONE,
...
int CAI_BaseNPC::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	if(info.GetAttacker()->GetFlags() & (FL_NPC | FL_CLIENT))
	{
		...
		if(!FInViewCone(info.GetAttacker()) || !FVisible(info.GetAttacker()))
		{
			...
			// ----------------------------------------------------------------
			if(GetEnemy() != NULL && !GetEnemies()->HasMemory(info.GetAttacker()) && !HasCondition(COND_SEE_ENEMY))
			{
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::PointInSpread(CBaseCombatCharacter *pCheckEntity, const Vector &sourcePos, const Vector &targetPos,
								const Vector &testPoint, float flSpread, float maxDistOffCenter)
{
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::PlayerInSpread(const Vector &sourcePos, const Vector &targetPos, float flSpread,
								 float maxDistOffCenter, bool ignoreHatedPlayers)
{
...
void CBaseEntity::TraceAttackToTriggers(const CTakeDamageInfo &info, const Vector &start, const Vector &end,
										const Vector &dir)
{
	Ray_t ray;
	ray.Init(start, end);
...
//-----------------------------------------------------------------------------
void CBaseEntity::HandleShotImpactingGlass(const FireBulletsInfo_t &info, const trace_t &tr, const Vector &vecDir,
...
void CAI_BaseNPC::MakeDamageBloodDecal(int cCount, float flNoise, trace_t *ptr, Vector vecDir)
{
	...
	for(i = 0; i < cCount; i++)
	{
		...
		AI_TraceLine(ptr->endpos, ptr->endpos + vecTraceDir * 172, MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
					 &Bloodtr);
...
#define InterruptFromCondition(iCondition) \
	AI_RemapFromGlobal(                    \
		(AI_IdIsLocal(iCondition) ? GetClassScheduleIdSpace()->ConditionLocalToGlobal(iCondition) : iCondition))
...
void CAI_BaseNPC::OnLooked(int iDistance)
{
	...
	static int conditionsToClear[] = {
		COND_SEE_HATE,	  COND_SEE_DISLIKE, COND_SEE_ENEMY,	  COND_SEE_FEAR,
		COND_SEE_NEMESIS, COND_SEE_PLAYER,	COND_LOST_PLAYER, COND_ENEMY_WENT_NULL,
	};
...
void CAI_BaseNPC::OnListened()
{
	...
	static int conditionsToClear[] = {
		COND_HEAR_DANGER,	 COND_HEAR_COMBAT,		   COND_HEAR_WORLD,			COND_HEAR_PLAYER,	 COND_HEAR_THUMPER,
		COND_HEAR_BUGBAIT,	 COND_HEAR_PHYSICS_DANGER, COND_HEAR_BULLET_IMPACT, COND_HEAR_MOVE_AWAY,
...
CBaseGrenade *CAI_BaseNPC::IncomingGrenade(void)
{
	...
	for(AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter); pEMemory != NULL;
		pEMemory = GetEnemies()->GetNext(&iter))
	{
...
CSound *CAI_BaseNPC::GetBestSound(int validTypes)
{
	...
	if(pResult == NULL)
		DevMsg("Warning: NULL Return from GetBestSound\n"); // condition previously set now no longer true. Have seen
															// this when play too many sounds...
	return pResult;
}
...
CBaseEntity *CAI_BaseNPC::EyeLookTarget(void)
{
	if(m_flNextEyeLookTime < gpGlobals->curtime)
	{
		...
		for(CEntitySphereQuery sphere(GetAbsOrigin(), 1024, 0); (pEntity = sphere.GetCurrentEntity()) != NULL;
			sphere.NextEntity())
		{
			if(pEntity == this)
			{
...
void CAI_BaseNPC::MaintainLookTargets(float flInterval)
{
	...
	if(m_flNextEyeLookTime < gpGlobals->curtime)
	{
		...
		Vector vecDir		= vBodyDir + (x * lookSpread.x * forward) + (y * lookSpread.y * right) + (z * lookSpread.z
		* up); float  lookDist		= random->RandomInt(50,2000); m_vEyeLookTarget	= EyePosition() + lookDist*vecDir;
		*/
...
void CAI_BaseNPC::RunAnimation(void)
{
	...
	if(m_NPCState != NPC_STATE_SCRIPT && m_NPCState != NPC_STATE_DEAD && m_Activity == ACT_IDLE && IsActivityFinished())
	{
		...
		// FIXME: this doesn't reissue a translation, so if the idle activity translation changes over time, it'll never
		// get reset
		if(SequenceLoops())
		{
...
void CAI_BaseNPC::PostRun(void)
{
	...
	if(!IsMoving())
	{
		if(GetIdealActivity() == ACT_WALK || GetIdealActivity() == ACT_RUN || GetIdealActivity() == ACT_WALK_AIM ||
...
void CAI_BaseNPC::UpdateEfficiency(bool bInPVS)
{
	...
	if(m_bForceConditionsGather || gpGlobals->curtime - GetLastAttackTime() < .2 ||
	   gpGlobals->curtime - m_flLastDamageTime < .2 || (GetState() < NPC_STATE_IDLE || GetState() > NPC_STATE_SCRIPT) ||
	   ((bInPVS || bInVisibilityPVS) && ((GetTask() && !TaskIsRunning()) || GetTaskInterrupt() > 0 || m_bInChoreo)))
	{
	...
	if(bInPVS)
	{
		...
		range = (playerDist < 50 * 12) ? DIST_NEAR : (playerDist < 200 * 12) ? DIST_MID : DIST_FAR;
	}
	else
	{
		range = (playerDist < 25 * 12) ? DIST_NEAR : (playerDist < 100 * 12) ? DIST_MID : DIST_FAR;
	}
...
void CAI_BaseNPC::UpdateSleepState(bool bInPVS)
{
	if(GetSleepState() > AISS_AWAKE)
	{
		...
		if(m_flWakeRadius > .1 && !(pLocalPlayer->GetFlags() & FL_NOTARGET) &&
		   (pLocalPlayer->GetAbsOrigin() - GetAbsOrigin()).LengthSqr() <= Square(m_flWakeRadius))
			Wake();
			...
			else
			{
				...
				if((GetSoundInterests() & SOUND_DANGER) && !(HasSpawnFlags(SF_NPC_WAIT_TILL_SEEN)))
				{
					...
					while(iSound != SOUNDLIST_EMPTY)
					{
						...
						if((pCurrentSound->SoundType() & SOUND_DANGER) && GetSenses()->CanHearSound(pCurrentSound) &&
...
void CAI_BaseNPC::RebalanceThinks()
{
	...
	if(bDebugThinkTicks)
	{
		...
		{
			DevMsg("NPC per tick is %d [%d] (tick %d, frame %d)\n", nRebalanceableThinksInTick, nThinksInTick,
				   iPrevTick, gpGlobals->framecount);
	...
	if(ShouldRebalanceThinks() && gpGlobals->tickcount >= gm_iNextThinkRebalanceTick)
	{
		...
		for(i = 0; i < g_AI_Manager.NumAIs(); i++)
		{
			...
			if(pCandidate->CanThinkRebalance() && (pCandidate->GetNextThinkTick() >= iMinTickRebalance &&
		...
		if(rebalanceCandidates.Count())
		{
			...
			int iMaxThinkersPerTick =
				ceil((float)(rebalanceCandidates.Count() + 1) / (float)iTicksPer10Hz); // +1 to account for "this"

			int iCurTickDistributing = MIN(gpGlobals->tickcount, rebalanceCandidates[0].iNextThinkTick);
			int iRemainingThinksToDistribute =
				iMaxThinkersPerTick - 1; // Start with one fewer first time because "this" is
...
void CAI_BaseNPC::NPCThink(void)
{
	...
	if(GetEfficiency() < AIE_DORMANT && GetSleepState() == AISS_AWAKE)
	{
		...
		if(g_pAINetworkManager && g_pAINetworkManager->IsInitialized())
		{
			...
			AI_PROFILE_SCOPE_BEGIN_(GetClassScheduleIdSpace()->GetClassName())
				; // need to use a string stable from map load to map load
	...
	if(m_bUsingStandardThinkTime)
	{
		static const char *ppszEfficiencies[] = {
			"AIE_NORMAL", "AIE_EFFICIENT", "AIE_VERY_EFFICIENT", "AIE_SUPER_EFFICIENT", "AIE_DORMANT",
		};
...
void CAI_BaseNPC::GatherAttackConditions(CBaseEntity *pTarget, float flDist)
{
	...
	// -----------------------------------------------------------------
	if(HasCondition(COND_CAN_RANGE_ATTACK2) || HasCondition(COND_CAN_RANGE_ATTACK1) ||
	   HasCondition(COND_CAN_MELEE_ATTACK2) || HasCondition(COND_CAN_MELEE_ATTACK1))
	{
...
void CAI_BaseNPC::Wake(bool bFireOutput)
{
	if(GetSleepState() != AISS_AWAKE)
	{
		...
		{
			AISquadIter_t iter;
			for(CAI_BaseNPC *pSquadMember = GetSquad()->GetFirstMember(&iter); pSquadMember;
				pSquadMember = GetSquad()->GetNextMember(&iter))
			{
...
{
	static int conditionsToClear[] = {
		COND_SEE_HATE,	   COND_SEE_DISLIKE,  COND_SEE_ENEMY,			COND_SEE_FEAR,		 COND_SEE_NEMESIS,
		COND_SEE_PLAYER,   COND_HEAR_DANGER,  COND_HEAR_COMBAT,			COND_HEAR_WORLD,	 COND_HEAR_PLAYER,
		COND_HEAR_THUMPER, COND_HEAR_BUGBAIT, COND_HEAR_PHYSICS_DANGER, COND_HEAR_MOVE_AWAY, COND_SMELL,
	};
...
{
	bool bScriptedWait =
		(IsCurSchedule(SCHED_WAIT_FOR_SCRIPT) || (m_hCine && m_scriptState == CAI_BaseNPC::SCRIPT_WAIT));
				...
				if(mins != maxs) // some NPCs have no hull, so mins == maxs == vec3_origin
				{
					...
					else
					{
						if(trace.startsolid && trace.m_pEnt->GetMoveType() == MOVETYPE_VPHYSICS &&
						   ...
						   trace.m_pEnt->VPhysicsGetObject()->GetMass() < VPHYSICS_LARGE_OBJECT_MASS)
						{
...
void CAI_BaseNPC::GatherConditions(void)
{
	...
	if(m_NPCState != NPC_STATE_NONE && m_NPCState != NPC_STATE_DEAD)
	{
		...
		if(!IsFlaggedEfficient() &&
		   (bForcedGather || HasCondition(COND_IN_PVS) || ShouldAlwaysThink() || m_NPCState == NPC_STATE_COMBAT))
		{
			CheckOnGround();
			...
			if(GetCurSchedule() && (m_NPCState == NPC_STATE_IDLE || m_NPCState == NPC_STATE_ALERT) && GetEnemy() &&
			   !HasCondition(COND_NEW_ENEMY) && GetCurSchedule()->HasInterrupt(COND_NEW_ENEMY))
			{
...
void CAI_BaseNPC::PrescheduleThink(void)
{
	...
	if((CapabilitiesGet() & bits_CAP_USE_WEAPONS) && (m_iDesiredWeaponState == DESIREDWEAPONSTATE_HOLSTERED ||
...
void CAI_BaseNPC::RunAI(void)
{
	...
	if(ai_debug_squads.GetBool())
	{
		   ...
		   (GetSquad()->IsLeader(this) || GetSquad()->NumMembers() == 1))
		{
			...
			for(CAI_BaseNPC *pSquadMember = pSquad->GetFirstMember(&iter, false); pSquadMember;
				pSquadMember = pSquad->GetNextMember(&iter, false))
			{
...
NPC_STATE CAI_BaseNPC::SelectIdleIdealState()
{
	...
	if(HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_COMBAT) || HasCondition(COND_HEAR_WORLD) ||
	   HasCondition(COND_HEAR_PLAYER) || HasCondition(COND_HEAR_THUMPER) || HasCondition(COND_HEAR_BULLET_IMPACT))
	{
...
NPC_STATE CAI_BaseNPC::SelectAlertIdealState()
{
	...
	if(HasCondition(COND_NEW_ENEMY) || HasCondition(COND_SEE_ENEMY) || GetEnemy() != NULL)
	{
		return NPC_STATE_COMBAT;
	}
...
{
	if(HasCondition(COND_TASK_FAILED) || HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE))
	{
...
{
	return (m_NPCState == NPC_STATE_ALERT || m_NPCState == NPC_STATE_COMBAT) ? ai_reaction_delay_alert.GetFloat()
...
bool CAI_BaseNPC::UpdateEnemyMemory(CBaseEntity *pEnemy, const Vector &position, CBaseEntity *pInformer)
{
	...
	if(GetEnemies())
	{
		...
		bool result =
			GetEnemies()->UpdateMemory(GetNavigator()->GetNetwork(), pEnemy, position, reactionDelay, firstHand);
...
void CAI_BaseNPC::GatherEnemyConditions(CBaseEntity *pEnemy)
{
	...
	if(HasCondition(COND_NEW_ENEMY) || GetSenses()->GetTimeLastUpdate(GetEnemy()) == gpGlobals->curtime)
	{
		...
		if(!bSensesDidSee &&
		   ((EnemyDistance(pEnemy) >= GetSenses()->GetDistLook()) || !FVisible(pEnemy, MASK_BLOCKLOS, &pBlocker)))
		{
			// No LOS to enemy
	...
	if(gpGlobals->curtime - GetEnemyLastTimeSeen() > 8)
	{
		...
		if(((flEnemyLKP - GetAbsOrigin()).Length2D() < 48) && !HasCondition(COND_SEE_ENEMY))
		{
			MarkEnemyAsEluded();
		}
		//-------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
float CAI_BaseNPC::GetGoalRepathTolerance(CBaseEntity *pGoalEnt, GoalType_t type, const Vector &curGoal,
	...
	if(m_AnyUpdateEnemyPosTimer.Expired() && m_UpdateEnemyPosTimer.Expired())
	{
		...
		if(GetNavigator()->GetGoalType() == GOALTYPE_ENEMY)
		{
			...
			else
			{
				...
				float tolerance =
					GetGoalRepathTolerance(GetEnemy(), GOALTYPE_ENEMY, GetNavigator()->GetGoalPos(), vEnemyLKP);
				if((GetNavigator()->GetGoalPos() - vEnemyLKP).Length() > tolerance)
				{
					// FIXME: when fleeing crowds, won't this severely limit the effectiveness of each individual?
					// Shouldn't this be a mutex that's held for some period so that at least one attacker is effective?
...
void CAI_BaseNPC::UpdateTargetPos()
{
	...
	if(GetNavigator()->GetGoalType() == GOALTYPE_TARGETENT)
	{
		...
		{
			if(GetTarget() == NULL ||
			   (GetNavigator()->GetGoalPos() - GetTarget()->GetAbsOrigin()).Length() >
				   GetGoalRepathTolerance(GetTarget(), GOALTYPE_TARGETENT, GetNavigator()->GetGoalPos(),
...
Activity CAI_BaseNPC::NPC_TranslateActivity(Activity eNewActivity)
{
	...
	// ====
	// HACK : LEIPZIG 06 -	The underlying problem is that the AR2 and SMG1 cannot map IDLE_ANGRY to a crouched
	// equivalent automatically
	//						which causes the character to pop up and down in their idle state of firing while crouched. --
	//jdw
	...
	if(CapabilitiesGet() & bits_CAP_DUCK)
	{
		...
		else if((eNewActivity == ACT_COVER) || (eNewActivity == ACT_IDLE && HasMemory(bits_MEMORY_INCOVER)))
		{
	...
	if(bWeaponRequired)
	{
		...
		if(!sUniqueActivities.Find(weaponTranslation))
		{
			...
			DevWarning("%s missing activity \"%s\" needed by weapon\"%s\"\n", GetClassname(),
					   GetActivityName(weaponTranslation), GetActiveWeapon()->GetClassname());
...
//			weaponActivity -
//-----------------------------------------------------------------------------
void CAI_BaseNPC::ResolveActivityToSequence(Activity NewActivity, int &iSequence, Activity &translatedActivity,
		...
		if(iSequence == ACTIVITY_NOT_AVAILABLE)
		{
			...
			if((pLastWarn != this && lastWarnActivity != translatedActivity) || gpGlobals->curtime - timeLastWarn > 5.0)
			{
				DevWarning("%s:%s:%s has no sequence for act:%s\n", GetClassname(), GetDebugName(),
						   STRING(GetModelName()), ActivityList_NameForIndex(translatedActivity));
				pLastWarn = this;
...
void CAI_BaseNPC::SetActivityAndSequence(Activity NewActivity, int iSequence, Activity translatedActivity,
										 Activity weaponActivity)
{
	...
	if(ai_sequence_debug.GetBool() == true && (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
	{
		DevMsg("SetActivityAndSequence : %s: %s:%s -> %s:%s / %s:%s\n", GetClassname(), GetActivityName(GetActivity()),
			   GetSequenceName(GetSequence()), GetActivityName(NewActivity), GetSequenceName(iSequence),
	...
	if((GetActivity() != m_IdealActivity) || (GetSequence() != m_nIdealSequence))
	{
		if(ai_sequence_debug.GetBool() == true && (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
		{
			DevMsg("MaintainActivity %s : %s:%s -> %s:%s\n", GetClassname(), GetActivityName(GetActivity()),
				   GetSequenceName(GetSequence()), GetActivityName(m_IdealActivity), GetSequenceName(m_nIdealSequence));
		}
		...
		else
		{
			...
			ResolveActivityToSequence(m_IdealActivity, m_nIdealSequence, m_IdealTranslatedActivity,
...
//#define _THROWDEBUG
float CAI_BaseNPC::ThrowLimit(const Vector &vecStart, const Vector &vecEnd, float fGravity, float fArcSize,
							  const Vector &mins, const Vector &maxs, CBaseEntity *pTarget, Vector *jumpVel,
...
void CAI_BaseNPC::SetHullSizeNormal(bool force)
{
	...
	{
		// Find out what the height difference will be between the versions and adjust our bbox accordingly to keep us
		// level
...
bool CAI_BaseNPC::IsNavHullValid() const
{
	...
	if((hullMin.x > vecMins.x) || (hullMax.x < vecMaxs.x) || (hullMin.y > vecMins.y) || (hullMax.y < vecMaxs.y) ||
...
void CAI_BaseNPC::StartNPC(void)
{
	...
	if((GetMoveType() != MOVETYPE_FLY) && (GetMoveType() != MOVETYPE_FLYGRAVITY) &&
	   !(CapabilitiesGet() & bits_CAP_MOVE_FLY) && !HasSpawnFlags(SF_NPC_FALL_TO_GROUND) && !IsWaitingToRappel() &&
	   !GetMoveParent())
	{
	...
	static const float nextThinkTimes[20] = {.0,   .150, .075, .225, .030, .180, .120, .270, .045, .210,
...
void CAI_BaseNPC::StartTargetHandling(CBaseEntity *pTargetEnt)
{
	...
	AI_NavGoal_t goal(GOALTYPE_PATHCORNER, pTargetEnt->GetAbsOrigin(), bIsFlying ? ACT_FLY : ACT_WALK,
...
{
	if(GetTaskStatus() != TASKSTATUS_COMPLETE && GetTaskStatus() != TASKSTATUS_RUN_MOVE)
		return 1;
	...
	for(int i = m_UnreachableEnts.Size() - 1; i >= 0; i--)
	{
		...
		else if(pEntity == m_UnreachableEnts[i].hUnreachableEnt)
		{
			...
			if(gpGlobals->curtime > m_UnreachableEnts[i].fExpireTime ||
			   pEntity->GetAbsOrigin().DistToSqr(m_UnreachableEnts[i].vLocationWhenUnreachable) >
		...
		else if(IRelationPriority(pEnemy) > iBestPriority)
		{
			...
			if(pBestEnemy)
			{
				DbgEnemyMsg(this, "    (%s displaced due to priority, %d > %d )\n", pBestEnemy->GetDebugName(),
							IRelationPriority(pEnemy), iBestPriority);
			}
		...
		else if(IRelationPriority(pEnemy) == iBestPriority)
		{
			...
			if(!bAcceptCurrent)
			{
				...
				if(bCloser && fBestVisible == TRS_NONE)
				{
					...
					fCurVisible = ((EnemyDistance(pEnemy) < GetSenses()->GetDistLook()) && FVisible(pEnemy))
									  ? TRS_TRUE
									  : TRS_FALSE;
				...
				if(!bAcceptCurrent)
				{
					...
					{
						fBestVisible =
							((EnemyDistance(pBestEnemy) < GetSenses()->GetDistLook()) && FVisible(pBestEnemy))
								? TRS_TRUE
								: TRS_FALSE;
					}
...
void CAI_BaseNPC::HandleAnimEvent(animevent_t *pEvent)
{
	...
	case SCRIPT_EVENT_ENDANIMATION:		// Set ending animation sequence to
		...
		{
			CBasePropDoor *pDoor =
				(CBasePropDoor *)(CBaseEntity *)GetNavigator()->GetPath()->GetCurWaypoint()->GetEHandleData();
			if(pDoor != NULL)
			{
		...
		default:
			if((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) && (pEvent->type & AE_TYPE_SERVER))
			{
				if(pEvent->event == AE_NPC_HOLSTER)
				{
					...
					// Force the NPC to recalculate it's arrival activity since it'll most likely be wrong now that we
					// don't have a weapon out.
				...
				else if(pEvent->event == AE_NPC_WEAPON_SET_ACTIVITY)
				{
					...
					if((pWeapon) && (pEvent->options))
					{
						...
						if(act != ACT_INVALID)
						{
							// FIXME: where should the duration come from? normally it would come from the current
							// sequence
							Weapon_SetActivity(act, 0);
						}
					}
				...
				else if(pEvent->event == AE_NPC_HURT_INTERACTION_PARTNER)
				{
					...
					if(m_hInteractionPartner)
					{
						...
						DevWarning(1,
								   "Bad %s format. Should be: { AE_NPC_HURT_INTERACTION_PARTNER <frame number> "
								   "\"<ME/THEM> <Amount of damage done>\" }\n",
								   pName);
						return;
					}

					DevWarning("%s received AE_NPC_HURT_INTERACTION_PARTNER anim event, but it's not interacting with "
							   "anything.\n",
							   GetDebugName());
					return;
				}
			...
			if(pEvent->pSource != this || (pEvent->type & AE_TYPE_NEWEVENTSYSTEM && pEvent->type & AE_TYPE_WEAPON) ||
			   (pEvent->event >= EVENT_WEAPON && pEvent->event <= EVENT_WEAPON_LAST))
			{
...
void CAI_BaseNPC::DrawDebugGeometryOverlays(void)
{
	...
	if((m_debugOverlays & OVERLAY_NPC_ROUTE_BIT))
	{
		...
		if(IsMoving())
		{
			...
			NDebugOverlay::Line(WorldSpaceCenter(), WorldSpaceCenter() + vecYaw * GetHullWidth() * .5, 255, 255, 255,
								true, 0.0);
		}
	}
	...
	if((m_debugOverlays & OVERLAY_NPC_NEAREST_BIT))
	{
		...
		if(iNodeID != NO_NODE)
		{
			NDebugOverlay::Box(GetNavigator()->GetNetwork()->AccessNodes()[iNodeID]->GetPosition(GetHullType()),
	...
	{
		AIEnemiesIter_t iter;
		for(AI_EnemyInfo_t *eMemory = GetEnemies()->GetFirst(&iter); eMemory != NULL;
			eMemory = GetEnemies()->GetNext(&iter))
		{
			if(eMemory->hEnemy)
			{
				...
				if(npcEnemy)
				{
					...
					if(npcEnemy->IsPlayer() && (eMemory->vLastKnownLocation - npcEnemy->GetAbsOrigin()).Length() < 10)
					{
					...
					{
						NDebugOverlay::Cross3D(drawPos, NAI_Hull::Mins(npcEnemy->GetHullType()),
	...
	if(m_debugOverlays & OVERLAY_TEXT_BIT)
	{
		...
		if(GetCurSchedule())
		{
			...
			if(m_debugOverlays & OVERLAY_NPC_TASK_BIT)
			{
				...
				{
					Q_snprintf(tempstr, sizeof(tempstr), "%s%s%s%s", ((i == 0) ? "Task:" : "       "),
			...
			else
			{
				...
				if(pTask)
				{
					Q_snprintf(tempstr, sizeof(tempstr), "Task: %s (#%d), ", TaskName(pTask->iTask),
		...
		if(GetAbsVelocity() != vec3_origin || GetLocalAngularVelocity() != vec3_angle)
		{
			...
			Q_snprintf(tmp, sizeof(tmp), "Vel %.1f %.1f %.1f   Ang: %.1f %.1f %.1f\n", GetAbsVelocity().x,
					   GetAbsVelocity().y, GetAbsVelocity().z, GetLocalAngularVelocity().x, GetLocalAngularVelocity().y,
		...
		if(m_LastShootAccuracy != -1 && ai_shot_stats.GetBool())
		{
			...
			if(m_TotalShots)
			{
				EntityText(text_offset,
						   msg.sprintf("Act Accuracy: %.1f", ((float)m_TotalHits / (float)m_TotalShots) * 100.0), 0);
				text_offset++;
			}
...
void CAI_BaseNPC::ReportOverThinkLimit(float time)
{
	DevMsg("%s thinking for %.02fms!!! (%s); r%.2f (c%.2f, pst%.2f, ms%.2f), p-r%.2f, m%.2f\n", GetDebugName(), time,
		   GetCurSchedule()->GetName(), g_AIRunTimer.GetDuration().GetMillisecondsF(),
		   ...
		   g_AIPostRunTimer.GetDuration().GetMillisecondsF(), g_AIMoveTimer.GetDuration().GetMillisecondsF());
...
Vector CAI_BaseNPC::GetShootEnemyDir(const Vector &shootOrigin, bool bNoisy)
{
	...
	if(pEnemy)
	{
		...
		if(!FInViewCone(vecEnemyLKP) || !FVisible(vecEnemyLKP))
		{
			...
			if(pPortal)
			{
				UTIL_Portal_VectorTransform(pPortal->m_hLinkedPortal->MatrixThisToLinked(), vecEnemyOffset,
											vecEnemyOffset);
...
Vector CAI_BaseNPC::GetActualShootPosition(const Vector &shootOrigin)
{
	...
	if(pPortal)
	{
		...
		if(fDistanceToEnemyThroughPortalSqr < fDistanceToEnemySqr || !FInViewCone(vecEnemyLKP) ||
		   !FVisible(vecEnemyLKP))
		{
...
void CAI_BaseNPC::AddFacingTarget(CBaseEntity *pTarget, const Vector &vecPosition, float flImportance, float flDuration,
								  float flRamp)
{
...
int CAI_BaseNPC::PlaySentence(const char *pszSentence, float delay, float volume, soundlevel_t soundlevel,
							  CBaseEntity *pListener)
{
	...
	if(pszSentence && IsAlive())
	{
		...
		if(pszSentence[0] == '!')
		{
			...
			CBaseEntity::EmitSentenceByIndex(filter, entindex(), CHAN_VOICE, sentenceIndex, volume, soundlevel, 0,
											 PITCH_NORM);
		}
		else
		{
...
int CAI_BaseNPC::PlayScriptedSentence(const char *pszSentence, float delay, float volume, soundlevel_t soundlevel,
...
bool CAI_BaseNPC::ShouldChooseNewEnemy()
{
	...
	if(pEnemy)
	{
		...
		if(EnemyHasEludedMe() || (IRelationType(pEnemy) != D_HT && IRelationType(pEnemy) != D_FR) ||
		   !IsValidEnemy(pEnemy))
		{
...
{
	if((m_NPCState == NPC_STATE_IDLE || m_NPCState == NPC_STATE_ALERT) && random->RandomInt(0, 99) == 0 &&
	   !HasSpawnFlags(SF_NPC_GAG))
	{
		return true;
	}
...
BEGIN_DATADESC(CAI_BaseNPC)
	...
	DEFINE_EMBEDDED(m_ScheduleState),
		...
		DEFINE_FIELD(m_bUsingStandardThinkTime, FIELD_BOOLEAN), DEFINE_FIELD(m_flLastRealThinkTime, FIELD_TIME),
		...
		DEFINE_FIELD(m_flTimePingEffect, FIELD_TIME), DEFINE_FIELD(m_bForceConditionsGather, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bConditionsGathered, FIELD_BOOLEAN), DEFINE_FIELD(m_bSkippedChooseEnemy, FIELD_BOOLEAN),
		DEFINE_FIELD(m_NPCState, FIELD_INTEGER), DEFINE_FIELD(m_IdealNPCState, FIELD_INTEGER),
		DEFINE_FIELD(m_flLastStateChangeTime, FIELD_TIME), DEFINE_FIELD(m_Efficiency, FIELD_INTEGER),
		DEFINE_FIELD(m_MoveEfficiency, FIELD_INTEGER), DEFINE_FIELD(m_flNextDecisionTime, FIELD_TIME),
		DEFINE_KEYFIELD(m_SleepState, FIELD_INTEGER, "sleepstate"), DEFINE_FIELD(m_SleepFlags, FIELD_INTEGER),
		DEFINE_KEYFIELD(m_flWakeRadius, FIELD_FLOAT, "wakeradius"),
		DEFINE_KEYFIELD(m_bWakeSquad, FIELD_BOOLEAN, "wakesquad"), DEFINE_FIELD(m_nWakeTick, FIELD_TICK),
		...
		DEFINE_FIELD(m_nIdealSequence, FIELD_INTEGER), DEFINE_EMBEDDEDBYREF(m_pSenses),
		DEFINE_EMBEDDEDBYREF(m_pLockedBestSound), DEFINE_FIELD(m_hEnemy, FIELD_EHANDLE),
		DEFINE_FIELD(m_flTimeEnemyAcquired, FIELD_TIME), DEFINE_FIELD(m_hTargetEnt, FIELD_EHANDLE),
		DEFINE_EMBEDDED(m_GiveUpOnDeadEnemyTimer), DEFINE_EMBEDDED(m_FailChooseEnemyTimer),
		DEFINE_FIELD(m_EnemiesSerialNumber, FIELD_INTEGER), DEFINE_FIELD(m_flAcceptableTimeSeenEnemy, FIELD_TIME),
		DEFINE_EMBEDDED(m_UpdateEnemyPosTimer),
		//		m_flTimeAnyUpdateEnemyPos (static)
		DEFINE_FIELD(m_vecCommandGoal, FIELD_VECTOR), DEFINE_EMBEDDED(m_CommandMoveMonitor),
		DEFINE_FIELD(m_flSoundWaitTime, FIELD_TIME), DEFINE_FIELD(m_nSoundPriority, FIELD_INTEGER),
		DEFINE_FIELD(m_flIgnoreDangerSoundsUntil, FIELD_TIME), DEFINE_FIELD(m_afCapability, FIELD_INTEGER),
		DEFINE_FIELD(m_flMoveWaitFinished, FIELD_TIME), DEFINE_FIELD(m_hOpeningDoor, FIELD_EHANDLE),
		DEFINE_EMBEDDEDBYREF(m_pNavigator), DEFINE_EMBEDDEDBYREF(m_pLocalNavigator),
		DEFINE_EMBEDDEDBYREF(m_pPathfinder), DEFINE_EMBEDDEDBYREF(m_pMoveProbe), DEFINE_EMBEDDEDBYREF(m_pMotor),
		DEFINE_UTLVECTOR(m_UnreachableEnts, FIELD_EMBEDDED), DEFINE_FIELD(m_hInteractionPartner, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_iInteractionPlaying, FIELD_INTEGER), DEFINE_UTLVECTOR(m_ScriptedInteractions, FIELD_EMBEDDED),
		DEFINE_FIELD(m_flInteractionYaw, FIELD_FLOAT), DEFINE_EMBEDDED(m_CheckOnGroundTimer),
		DEFINE_FIELD(m_vDefaultEyeOffset, FIELD_VECTOR), DEFINE_FIELD(m_flNextEyeLookTime, FIELD_TIME),
		DEFINE_FIELD(m_flEyeIntegRate, FIELD_FLOAT), DEFINE_FIELD(m_vEyeLookTarget, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vCurEyeTarget, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_hEyeLookTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_flHeadYaw, FIELD_FLOAT), DEFINE_FIELD(m_flHeadPitch, FIELD_FLOAT),
		DEFINE_FIELD(m_flOriginalYaw, FIELD_FLOAT), DEFINE_FIELD(m_bInAScript, FIELD_BOOLEAN),
		DEFINE_FIELD(m_scriptState, FIELD_INTEGER), DEFINE_FIELD(m_hCine, FIELD_EHANDLE),
		DEFINE_CUSTOM_FIELD(m_ScriptArrivalActivity, ActivityDataOps()),
		DEFINE_FIELD(m_strScriptArrivalSequence, FIELD_STRING), DEFINE_FIELD(m_flSceneTime, FIELD_TIME),
		DEFINE_FIELD(m_iszSceneCustomMoveSeq, FIELD_STRING),
		// 							m_pEnemies					Saved specially in ai_saverestore.cpp
		DEFINE_FIELD(m_afMemory, FIELD_INTEGER), DEFINE_FIELD(m_hEnemyOccluder, FIELD_EHANDLE),
		DEFINE_FIELD(m_flSumDamage, FIELD_FLOAT), DEFINE_FIELD(m_flLastDamageTime, FIELD_TIME),
		DEFINE_FIELD(m_flLastPlayerDamageTime, FIELD_TIME), DEFINE_FIELD(m_flLastSawPlayerTime, FIELD_TIME),
		DEFINE_FIELD(m_flLastAttackTime, FIELD_TIME), DEFINE_FIELD(m_flLastEnemyTime, FIELD_TIME),
		DEFINE_FIELD(m_flNextWeaponSearchTime, FIELD_TIME), DEFINE_FIELD(m_iszPendingWeapon, FIELD_STRING),
		DEFINE_KEYFIELD(m_bIgnoreUnseenEnemies, FIELD_BOOLEAN, "ignoreunseenenemies"), DEFINE_EMBEDDED(m_ShotRegulator),
		DEFINE_FIELD(m_iDesiredWeaponState, FIELD_INTEGER),
		// 							m_pSquad					Saved specially in ai_saverestore.cpp
		DEFINE_KEYFIELD(m_SquadName, FIELD_STRING, "squadname"), DEFINE_FIELD(m_iMySquadSlot, FIELD_INTEGER),
		DEFINE_KEYFIELD(m_strHintGroup, FIELD_STRING, "hintgroup"),
		DEFINE_KEYFIELD(m_bHintGroupNavLimiting, FIELD_BOOLEAN, "hintlimiting"),
		DEFINE_EMBEDDEDBYREF(m_pTacticalServices), DEFINE_FIELD(m_flWaitFinished, FIELD_TIME),
		DEFINE_FIELD(m_flNextFlinchTime, FIELD_TIME), DEFINE_FIELD(m_flNextDodgeTime, FIELD_TIME),
		DEFINE_EMBEDDED(m_MoveAndShootOverlay), DEFINE_FIELD(m_vecLastPosition, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vSavePosition, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vInterruptSavePosition, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_pHintNode, FIELD_EHANDLE),
		DEFINE_FIELD(m_cAmmoLoaded, FIELD_INTEGER), DEFINE_FIELD(m_flDistTooFar, FIELD_FLOAT),
		DEFINE_FIELD(m_hGoalEnt, FIELD_EHANDLE), DEFINE_FIELD(m_flTimeLastMovement, FIELD_TIME),
		DEFINE_KEYFIELD(m_spawnEquipment, FIELD_STRING, "additionalequipment"),
		DEFINE_FIELD(m_fNoDamageDecal, FIELD_BOOLEAN), DEFINE_FIELD(m_hStoredPathTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_vecStoredPathGoal, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_nStoredPathType, FIELD_INTEGER),
		DEFINE_FIELD(m_fStoredPathFlags, FIELD_INTEGER), DEFINE_FIELD(m_bDidDeathCleanup, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bCrouchDesired, FIELD_BOOLEAN), DEFINE_FIELD(m_bForceCrouch, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bIsCrouching, FIELD_BOOLEAN), DEFINE_FIELD(m_bPerformAvoidance, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bIsMoving, FIELD_BOOLEAN), DEFINE_FIELD(m_bFadeCorpse, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iDeathPose, FIELD_INTEGER), DEFINE_FIELD(m_iDeathFrame, FIELD_INTEGER),
		DEFINE_FIELD(m_bCheckContacts, FIELD_BOOLEAN), DEFINE_FIELD(m_bSpeedModActive, FIELD_BOOLEAN),
		DEFINE_FIELD(m_iSpeedModRadius, FIELD_INTEGER), DEFINE_FIELD(m_iSpeedModSpeed, FIELD_INTEGER),
		DEFINE_FIELD(m_hEnemyFilter, FIELD_EHANDLE), DEFINE_KEYFIELD(m_iszEnemyFilterName, FIELD_STRING, "enemyfilter"),
		DEFINE_FIELD(m_bImportanRagdoll, FIELD_BOOLEAN), DEFINE_FIELD(m_bPlayerAvoidState, FIELD_BOOLEAN),

		// Satisfy classcheck
		// DEFINE_FIELD( m_ScheduleHistory, CUtlVector < AIScheduleChoice_t > ),

		//							m_fIsUsingSmallHull			TODO -- This needs more consideration than simple
		//save/load
		// 							m_failText					DEBUG
		...
		// Outputs
		DEFINE_OUTPUT(m_OnDamaged, "OnDamaged"), DEFINE_OUTPUT(m_OnDeath, "OnDeath"),
		DEFINE_OUTPUT(m_OnHalfHealth, "OnHalfHealth"), DEFINE_OUTPUT(m_OnFoundEnemy, "OnFoundEnemy"),
		DEFINE_OUTPUT(m_OnLostEnemyLOS, "OnLostEnemyLOS"), DEFINE_OUTPUT(m_OnLostEnemy, "OnLostEnemy"),
		DEFINE_OUTPUT(m_OnFoundPlayer, "OnFoundPlayer"), DEFINE_OUTPUT(m_OnLostPlayerLOS, "OnLostPlayerLOS"),
		DEFINE_OUTPUT(m_OnLostPlayer, "OnLostPlayer"), DEFINE_OUTPUT(m_OnHearWorld, "OnHearWorld"),
		DEFINE_OUTPUT(m_OnHearPlayer, "OnHearPlayer"), DEFINE_OUTPUT(m_OnHearCombat, "OnHearCombat"),
		...
		DEFINE_OUTPUT(m_OnRappelTouchdown, "OnRappelTouchdown"), DEFINE_OUTPUT(m_OnWake, "OnWake"),
		DEFINE_OUTPUT(m_OnSleep, "OnSleep"), DEFINE_OUTPUT(m_OnForcedInteractionStarted, "OnForcedInteractionStarted"),
		...
		DEFINE_INPUTFUNC(FIELD_STRING, "SetSquad", InputSetSquad), DEFINE_INPUTFUNC(FIELD_VOID, "Wake", InputWake),
		...
		// Function pointers
		DEFINE_USEFUNC(NPCUse), DEFINE_THINKFUNC(CallNPCThink), DEFINE_THINKFUNC(CorpseFallThink),
		DEFINE_THINKFUNC(NPCInitThink),

END_DATADESC()

BEGIN_SIMPLE_DATADESC(AIScheduleState_t)
	DEFINE_FIELD(iCurTask, FIELD_INTEGER), DEFINE_FIELD(fTaskStatus, FIELD_INTEGER),
		DEFINE_FIELD(timeStarted, FIELD_TIME), DEFINE_FIELD(timeCurTaskStarted, FIELD_TIME),
		DEFINE_FIELD(taskFailureCode, FIELD_INTEGER), DEFINE_FIELD(iTaskInterrupt, FIELD_INTEGER),
		DEFINE_FIELD(bTaskRanAutomovement, FIELD_BOOLEAN), DEFINE_FIELD(bTaskUpdatedYaw, FIELD_BOOLEAN),
		DEFINE_FIELD(bScheduleWasInterrupted, FIELD_BOOLEAN),
END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CAI_BaseNPC, DT_AI_BaseNPC)
SendPropInt(SENDINFO(m_lifeState), 3, SPROP_UNSIGNED), SendPropBool(SENDINFO(m_bPerformAvoidance)),
	SendPropBool(SENDINFO(m_bIsMoving)), SendPropBool(SENDINFO(m_bFadeCorpse)),
	SendPropInt(SENDINFO(m_iDeathPose), ANIMATION_SEQUENCE_BITS), SendPropInt(SENDINFO(m_iDeathFrame), 5),
	SendPropBool(SENDINFO(m_bSpeedModActive)), SendPropInt(SENDINFO(m_iSpeedModRadius)),
	SendPropInt(SENDINFO(m_iSpeedModSpeed)), SendPropBool(SENDINFO(m_bImportanRagdoll)),
...
()
	...
	DEFINE_FIELD(fExpireTime, FIELD_TIME), DEFINE_FIELD(vLocationWhenUnreachable, FIELD_POSITION_VECTOR),
...
()
	...
	BEGIN_SIMPLE_DATADESC(ScriptedNPCInteraction_Phases_t) DEFINE_FIELD(iszSequence, FIELD_STRING),
...
()
	...
	BEGIN_SIMPLE_DATADESC(ScriptedNPCInteraction_t) DEFINE_FIELD(iszInteractionName, FIELD_STRING),
	DEFINE_FIELD(iFlags, FIELD_INTEGER), DEFINE_FIELD(iTriggerMethod, FIELD_INTEGER),
	DEFINE_FIELD(iLoopBreakTriggerMethod, FIELD_INTEGER), DEFINE_FIELD(vecRelativeOrigin, FIELD_VECTOR),
	DEFINE_FIELD(angRelativeAngles, FIELD_VECTOR), DEFINE_FIELD(vecRelativeVelocity, FIELD_VECTOR),
	DEFINE_FIELD(flDelay, FIELD_FLOAT), DEFINE_FIELD(flDistSqr, FIELD_FLOAT), DEFINE_FIELD(iszMyWeapon, FIELD_STRING),
	DEFINE_FIELD(iszTheirWeapon, FIELD_STRING), DEFINE_EMBEDDED_ARRAY(sPhases, SNPCINT_NUM_PHASES),
	DEFINE_FIELD(matDesiredLocalToWorld, FIELD_VMATRIX), DEFINE_FIELD(bValidOnCurrentEnemy, FIELD_BOOLEAN),
...
{
	AIExtendedSaveHeader_t() : version(AI_EXTENDED_SAVE_HEADER_VERSION), flags(0), scheduleCrc(0)
	{
		szSchedule[0] = 0;
		szIdealSchedule[0] = 0;
		szFailSchedule[0] = 0;
...
BEGIN_SIMPLE_DATADESC(AIExtendedSaveHeader_t)
	DEFINE_FIELD(version, FIELD_SHORT), DEFINE_FIELD(flags, FIELD_INTEGER),
		DEFINE_AUTO_ARRAY(szSchedule, FIELD_CHARACTER), DEFINE_FIELD(scheduleCrc, FIELD_INTEGER),
		DEFINE_AUTO_ARRAY(szIdealSchedule, FIELD_CHARACTER), DEFINE_AUTO_ARRAY(szFailSchedule, FIELD_CHARACTER),
...
int CAI_BaseNPC::Save(ISave &save)
{
	...
	if(m_pSchedule)
	{
		...
		CRC32_ProcessBuffer(&saveHeader.scheduleCrc, (void *)m_pSchedule->GetTaskList(),
...
int CAI_BaseNPC::Restore(IRestore &restore)
{
	...
	if(saveHeader.version >= AI_EXTENDED_SAVE_HEADER_FIRST_VERSION_WITH_SEQUENCE && saveHeader.szSequence[0] &&
	   GetModelPtr())
	{
	...
	bool bLostScript = (m_NPCState == NPC_STATE_SCRIPT && m_hCine == NULL);
	bool bDiscardScheduleState = (bLostScript || bLostSequence || saveHeader.szSchedule[0] == 0 ||
...
// Output :
//-----------------------------------------------------------------------------
CAI_BaseNPC::CAI_BaseNPC(void) : m_UnreachableEnts(0, 4), m_bDeferredNavigation(false)
{
	m_pMotor = NULL;
	m_pMoveProbe = NULL;
	m_pNavigator = NULL;
	...
	if(m_NPCState == NPC_STATE_SCRIPT && m_hCine)
	{
		if(!bEnrouteAsWell)
		{
			...
			if((m_scriptState == CAI_BaseNPC::SCRIPT_WALK_TO_MARK) ||
			   ...
			   (m_scriptState == CAI_BaseNPC::SCRIPT_CUSTOM_MOVE_TO_MARK) || m_hCine->IsTeleportingDueToMoveTo())
			{
				return;
			}
		}
...
ConVar sv_test_scripted_sequences("sv_test_scripted_sequences", "0", FCVAR_NONE,
								  "Tests for scripted sequences that are embedded in the world. Run through your map "
								  "with this set to check for NPCs falling through the world.");
...
bool CAI_BaseNPC::CineCleanup()
{
	...
	if(IsRunningDynamicInteraction())
	{
		...
		if(m_hInteractionPartner)
		{
			...
			// m_hInteractionPartner->GetClassName(), m_hInteractionPartner->GetDebugName(), gpGlobals->curtime );
		}
	...
	if(m_lifeState == LIFE_DYING)
	{
		...
		{
			StopAnimation();
			IncrementInterpolationFrame(); // Don't interpolate either, assume the corpse is positioned in its final
										   // resting place
		}
...
bool CAI_BaseNPC::FindSpotForNPCInRadius(Vector *pResult, const Vector &vStartPos, CAI_BaseNPC *pNPC, float radius,
										 bool bOutOfPlayerViewcone)
{
	...
	for(fan.y = 0; fan.y < 360; fan.y += 18.0)
	{
		...
		UTIL_TraceHull(tr.endpos, tr.endpos + Vector(0, 0, 10), pNPC->GetHullMins(), pNPC->GetHullMaxs(), MASK_NPCSOLID,
...
bool CAI_BaseNPC::IsNavigationUrgent()
{
	...
	if(IsCurSchedule(SCHED_SCRIPTED_WALK, false) || IsCurSchedule(SCHED_SCRIPTED_RUN, false) ||
...
bool CAI_BaseNPC::OnCalcBaseMove(AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult)
{
...
bool CAI_BaseNPC::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult)
{
	if(pMoveGoal->directTrace.pObstruction)
	{
...
bool CAI_BaseNPC::OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor, float distClear,
...
bool CAI_BaseNPC::OnUpcomingPropDoor(AILocalMoveGoal_t *pMoveGoal, CBasePropDoor *pDoor, float distClear,
									 AIMoveResult_t *pResult)
{
	   ...
	   (pDoor->IsDoorClosed() || pDoor->IsDoorClosing()))
	{
		...
		pOpenDoorRoute = GetPathfinder()->BuildLocalRoute(GetLocalOrigin(), opendata.vecStandPos, NULL,
...
static void AIMsgGuts(CAI_BaseNPC *pAI, unsigned flags, const char *pszMsg)
{
	...
	DevMsg(pszFmt2, pszMsg, pAI->GetClassname(), pAI->entindex(),
		   (pAI->GetEntityName() == NULL_STRING) ? "<unnamed>" : STRING(pAI->GetEntityName()), gpGlobals->tickcount);
}
...
bool CAI_BaseNPC::FindNearestValidGoalPos(const Vector &vTestPoint, Vector *pResult)
{
	...
	if(vCandidate == vec3_invalid)
	{
		...
		{
			GetMoveProbe()->MoveLimit(NAV_GROUND, g_pBigAINet->GetNodePosition(GetHullType(), iNearestNode), vTestPoint,
	...
	if(vCandidate != vec3_invalid)
	{
		AI_Waypoint_t *pPathToPoint =
			GetPathfinder()->BuildRoute(GetAbsOrigin(), vCandidate, AI_GetSinglePlayer(), 5 * 12, NAV_NONE, true);
		if(pPathToPoint)
		{
	...
	if(vCandidate == vec3_invalid)
	{
		GetMoveProbe()->MoveLimit(NAV_GROUND, GetAbsOrigin(), vTestPoint, MASK_SOLID_BRUSHONLY, NULL, 0, &moveTrace);
		vCandidate = moveTrace.vEndPosition;
	}
...
//-----------------------------------------------------------------------------
void AI_TraceLOS(const Vector &vecAbsStart, const Vector &vecAbsEnd, CBaseEntity *pLooker, trace_t *ptr,
		...
		if(pkvInteractions)
		{
			...
			while(pkvNode)
			{
				...
				if(pszDontFixup)
				{
					...
					{
						sInteraction.iFlags |=
SCNPC_FLAG_DONT_TELEPORT_AT_END_ME
;
}
else if(!Q_stricmp(pszDontFixup, "them") || !Q_stricmp(pszDontFixup, "both"))
{
	sInteraction.iFlags |=
SCNPC_FLAG_DONT_TELEPORT_AT_END_THEM
;
}
}
...
void CAI_BaseNPC::AddScriptedNPCInteraction(ScriptedNPCInteraction_t *pInteraction)
{
	...
	m_ScriptedInteractions[nNewIndex].matDesiredLocalToWorld.SetupMatrixOrgAngles(pInteraction->vecRelativeOrigin,
																				  pInteraction->angRelativeAngles);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_BaseNPC::StartScriptedNPCInteraction(CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction,
											  Vector vecOtherOrigin, QAngle angOtherAngles)
{
	...
	if(pOtherNPC)
	{
		...
		// pOtherNPC->GetClassName(), pOtherNPC->GetDebugName(), gpGlobals->curtime );
	...
	if(ai_debug_dyninteractions.GetBool())
	{
		if(pOtherNPC)
		{
			Msg("%s(%s) starting dynamic interaction \"%s\" with %s(%s).\n", GetClassname(), GetDebugName(),
				STRING(pInteraction->iszInteractionName), pOtherNPC->GetClassname(), pOtherNPC->GetDebugName());
			if(pszEntrySequence)
			{
	...
	if(pOtherNPC)
	{
		Q_snprintf(szSSName, sizeof(szSSName), "dss_%s%d%s%d", GetDebugName(), entindex(), pOtherNPC->GetDebugName(),
				   pOtherNPC->entindex());
	}
	else
	{
...
bool CanNPCsTradePlaces(CAI_BaseNPC *pNPC1, CAI_BaseNPC *pNPC2, bool bDebug)
{
	...
	if((pNPC1->GetHullMins().x <= pNPC2->GetHullMins().x) && (pNPC1->GetHullMins().y <= pNPC2->GetHullMins().y) &&
	   (pNPC1->GetHullMins().z <= pNPC2->GetHullMins().z) && (pNPC1->GetHullMaxs().x >= pNPC2->GetHullMaxs().x) &&
	   (pNPC1->GetHullMaxs().y >= pNPC2->GetHullMaxs().y) && (pNPC1->GetHullMaxs().z >= pNPC2->GetHullMaxs().z))
	{
		// 1 bigger than 2 in all axes, skip 2 in 1 test
		bTest2At1 = false;
	}
	else if((pNPC2->GetHullMins().x <= pNPC1->GetHullMins().x) && (pNPC2->GetHullMins().y <= pNPC1->GetHullMins().y) &&
			(pNPC2->GetHullMins().z <= pNPC1->GetHullMins().z) && (pNPC2->GetHullMaxs().x >= pNPC1->GetHullMaxs().x) &&
			(pNPC2->GetHullMaxs().y >= pNPC1->GetHullMaxs().y) && (pNPC2->GetHullMaxs().z >= pNPC1->GetHullMaxs().z))
	{
	...
	if(bTest1At2)
	{
		AI_TraceHull(pNPC2->GetAbsOrigin(), pNPC2->GetAbsOrigin(), pNPC1->GetHullMins(), pNPC1->GetHullMaxs(),
					 MASK_SOLID, &traceFilter, &tr);
		if(tr.startsolid)
		{
			if(bDebug)
			{
				NDebugOverlay::Box(pNPC2->GetAbsOrigin(), pNPC1->GetHullMins(), pNPC1->GetHullMaxs(), 255, 0, 0, true,
								   1.0);
			}
			return false;
		}
	...
	if(bTest2At1)
	{
		AI_TraceHull(pNPC1->GetAbsOrigin(), pNPC1->GetAbsOrigin(), pNPC2->GetHullMins(), pNPC2->GetHullMaxs(),
					 MASK_SOLID, &traceFilter, &tr);
		if(tr.startsolid)
		{
			if(bDebug)
			{
				NDebugOverlay::Box(pNPC1->GetAbsOrigin(), pNPC2->GetHullMins(), pNPC2->GetHullMaxs(), 255, 0, 0, true,
								   1.0);
			}
			return false;
		}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::InteractionCouldStart(CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction,
										Vector &vecOrigin, QAngle &angAngles)
{
			...
			if(m_debugOverlays & OVERLAY_NPC_SELECTED_BIT || pOtherNPC->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)
			{
				if(ai_debug_dyninteractions.GetFloat() == 2)
				{
					Msg("   %s distsqr: %0.2f (%0.2f %0.2f %0.2f), desired: <%0.2f (%0.2f %0.2f %0.2f)\n",
						GetDebugName(), flDistSqr, pOtherNPC->GetAbsOrigin().x, pOtherNPC->GetAbsOrigin().y,
						pOtherNPC->GetAbsOrigin().z, pInteraction->flDistSqr, vecOrigin.x, vecOrigin.y, vecOrigin.z);
				}
			}
		}
		return false;
	...
	if(bDebug)
	{
		...
		Msg("   %s distsqr: %0.2f (%0.2f %0.2f %0.2f), desired: (%0.2f %0.2f %0.2f)\n", GetDebugName(), flDistSqr,
			pOtherNPC->GetAbsOrigin().x, pOtherNPC->GetAbsOrigin().y, pOtherNPC->GetAbsOrigin().z, vecOrigin.x,
	...
	if(pInteraction->iFlags & SCNPC_FLAG_TEST_OTHER_ANGLES)
	{
		...
		if(bDebug)
		{
			Msg("   %s angle matched: (%0.2f %0.2f %0.2f), desired (%0.2f, %0.2f, %0.2f)\n", GetDebugName(),
				anglemod(angEnemyAngles.x), anglemod(angEnemyAngles.y), anglemod(angEnemyAngles.z),
	...
	AI_TraceHull(vecOrigin, vecOrigin, pOtherNPC->GetHullMins(), pOtherNPC->GetHullMaxs(), MASK_SOLID, &traceFilter,
				 &tr);
	if(tr.startsolid)
	{
		if(bDebug)
...
void CAI_BaseNPC::InputForceInteractionWithNPC(inputdata_t &inputdata)
{
	...
	if(!pszParam || !pszParam[0])
	{
		Warning("%s(%s) received ForceInteractionWithNPC input with bad parameters: %s\nFormat should be: "
				"ForceInteractionWithNPC <target NPC> <interaction name>\n",
	...
	for(int i = 0; i < m_ScriptedInteractions.Count(); i++)
	{
		...
		if(m_ScriptedInteractions[i].sPhases[SNPCINT_SEQUENCE].iActivity != ACT_INVALID)
		{
			if(!pNPC->HaveSequenceForActivity((Activity)m_ScriptedInteractions[i].sPhases[SNPCINT_SEQUENCE].iActivity))
			{
				...
				int iMySeq =
					SelectWeightedSequence((Activity)m_ScriptedInteractions[i].sPhases[SNPCINT_SEQUENCE].iActivity);
	...
	if(iInteraction == -1)
	{
		Warning("%s(%s) received ForceInteractionWithNPC input, but couldn't find an interaction named %s that entity "
				"named %s could run.\n",
