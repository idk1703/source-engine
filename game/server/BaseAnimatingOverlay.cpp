...
BEGIN_SIMPLE_DATADESC(CAnimationLayer)
	...
	DEFINE_FIELD(m_fFlags, FIELD_INTEGER), DEFINE_FIELD(m_bSequenceFinished, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bLooping, FIELD_BOOLEAN), DEFINE_FIELD(m_nSequence, FIELD_INTEGER),
		DEFINE_FIELD(m_flCycle, FIELD_FLOAT), DEFINE_FIELD(m_flPrevCycle, FIELD_FLOAT),
		DEFINE_FIELD(m_flPlaybackRate, FIELD_FLOAT), DEFINE_FIELD(m_flWeight, FIELD_FLOAT),
		DEFINE_FIELD(m_flBlendIn, FIELD_FLOAT), DEFINE_FIELD(m_flBlendOut, FIELD_FLOAT),
		DEFINE_FIELD(m_flKillRate, FIELD_FLOAT), DEFINE_FIELD(m_flKillDelay, FIELD_FLOAT),
		DEFINE_CUSTOM_FIELD(m_nActivity, ActivityDataOps()), DEFINE_FIELD(m_nPriority, FIELD_INTEGER),
		DEFINE_FIELD(m_nOrder, FIELD_INTEGER), DEFINE_FIELD(m_flLastEventCheck, FIELD_FLOAT),
		DEFINE_FIELD(m_flLastAccess, FIELD_TIME), DEFINE_FIELD(m_flLayerAnimtime, FIELD_FLOAT),
...
BEGIN_SEND_TABLE_NOBASE(CBaseAnimatingOverlay, DT_OverlayVars)
	SendPropUtlVector(SENDINFO_UTLVECTOR(m_AnimOverlay),
		...
		if(pLayer->IsActive())
		{
			...
			if(pLayer->IsKillMe())
			{
				...
				else
				{
					...
					{
						Msg("removing %d (%d): %s : %5.3f (%.3f)\n", i, pLayer->m_nOrder.Get(),
							GetSequenceName(pLayer->m_nSequence), pLayer->m_flCycle.Get(), pLayer->m_flWeight.Get());
					}
					FastRemoveLayer(i);
			...
			if(m_AnimOverlay[i].IsActive())
			{
				...
				*/
				Msg(" %d (%d): %s : %5.3f (%.3f)\n", i, m_AnimOverlay[i].m_nOrder.Get(),
					GetSequenceName(m_AnimOverlay[i].m_nSequence), m_AnimOverlay[i].m_flCycle.Get(),
	...
	*/
	...
	while((index = GetAnimationEvent(pstudiohdr, m_nSequence, &event, flStart, flEnd, index)) != 0)
	{
		...
		if(flCycleRate > 0.0)
		{
			...
			event.eventtime =
				pOwner->m_flAnimTime + (flCycle - m_flCycle) / flCycleRate + pOwner->GetAnimTimeInterval();
		}
...
void CBaseAnimatingOverlay::GetSkeleton(CStudioHdr *pStudioHdr, Vector pos[], Quaternion q[], int boneMask)
{
	...
	for(i = 0; i < m_AnimOverlay.Count(); i++)
	{
		if(layer[i] >= 0 && layer[i] < m_AnimOverlay.Count())
		{
			...
			boneSetup.AccumulatePose(pos, q, pLayer.m_nSequence, pLayer.m_flCycle, pLayer.m_flWeight,
									 gpGlobals->curtime, m_pIk);
		}
	}
...
void CBaseAnimatingOverlay::OnRestore()
{
	...
	// and Tracker 48843 (Alyx skating after restore) restore the owner entity ptr (otherwise the network layer won't
	// get NetworkStateChanged signals until the layer is re-Init()'ed
	for(i = 0; i < m_AnimOverlay.Count(); i++)
	{
...
float CBaseAnimatingOverlay::GetLayerDuration(int iLayer)
{
	if(IsValidLayer(iLayer))
	{
		...
		{
			return (1.0 - m_AnimOverlay[iLayer].m_flCycle) * SequenceDuration(m_AnimOverlay[iLayer].m_nSequence) /
...
int CBaseAnimatingOverlay::AddLayeredSequence(int sequence, int iPriority)
{
	...
	if(IsValidLayer(i))
	{
		...
		if(ai_sequence_debug.GetBool() == true && m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)
		{
			Msg("%5.3f : adding %d (%d): %s : %5.3f (%.3f)\n", gpGlobals->curtime, i, m_AnimOverlay[i].m_nOrder.Get(),
				GetSequenceName(m_AnimOverlay[i].m_nSequence), m_AnimOverlay[i].m_flCycle.Get(),
				m_AnimOverlay[i].m_flWeight.Get());
		}
	}
...
int CBaseAnimatingOverlay::AllocateLayer(int iPriority)
{
	...
	// make sure there's always an empty unused layer so that history slots will be available on the client when it is
	// used
	if(iNumOpen == 0)
	{
		if(m_AnimOverlay.Count() < MAX_OVERLAYS)
