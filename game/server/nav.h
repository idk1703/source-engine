...
// There are 3 different definitions of StepHeight throughout the code, waiting to produce bugs if the 18.0 is ever
// changed.
...
#if defined(CSTRIKE_DLL)
const float DeathDrop = 200.0f; // (300) distance at which we will die if we fall - should be about 600, and pay
								// attention to fall damage during pathfind
#else
const float DeathDrop = 400.0f;		  // (300) distance at which we will die if we fall - should be about 600, and pay
									  // attention to fall damage during pathfind
#endif
...
enum NavAttributeType
{
	...
	NAV_MESH_NAV_BLOCKER = 0x80000000 // area is blocked by nav blocker ( Alas, needed to hijack a bit in the attributes
									  // to get within a cache line [7/24/2008 tom])
};
...
struct Extent
{
	...
	{
		return (lo.x <= other.lo.x + tolerance && hi.x >= other.hi.x - tolerance && lo.y <= other.lo.y + tolerance &&
				hi.y >= other.hi.y - tolerance && lo.z <= other.lo.z + tolerance && hi.z >= other.hi.z - tolerance);
	}
};
