...
BEGIN_SIMPLE_DATADESC(CAI_Senses)
	...
	DEFINE_FIELD(m_LookDist, FIELD_FLOAT), DEFINE_FIELD(m_LastLookDist, FIELD_FLOAT),
		DEFINE_FIELD(m_TimeLastLook, FIELD_TIME), DEFINE_FIELD(m_iSensingFlags, FIELD_INTEGER),
		//								m_iAudibleList		(no way to save?)
		DEFINE_UTLVECTOR(m_SeenHighPriority, FIELD_EHANDLE), DEFINE_UTLVECTOR(m_SeenNPCs, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_TimeLastLookHighPriority, FIELD_TIME), DEFINE_FIELD(m_TimeLastLookNPCs, FIELD_TIME),
		...
		for(int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if(pPlayer)
			{
				...
				else
				{
					...
					if(pPortal &&
					   UTIL_Portal_DistanceThroughPortalSqr(pPortal, origin, pPlayer->GetAbsOrigin()) < distSq &&
					   LookThroughPortal(pPortal, pPlayer))
					{
						nSeen++;
					}
				}
...
int CAI_Senses::LookForNPCs(int iDistance)
{
	...
	if(gpGlobals->curtime - m_TimeLastLookNPCs > timeNPCs)
	{
		...
		if(efficiency < AIE_SUPER_EFFICIENT)
		{
			...
			{
				if(ppAIs[i] != GetOuter() &&
				   (ppAIs[i]->ShouldNotDistanceCull() || origin.DistToSqr(ppAIs[i]->GetAbsOrigin()) < distSq))
				{
					if(Look(ppAIs[i]))
					{
						nSeen++;
