...
const char *g_ppszGoalTypes[] = {
	"GOALTYPE_NONE",	   "GOALTYPE_TARGETENT", "GOALTYPE_ENEMY",
	"GOALTYPE_PATHCORNER", "GOALTYPE_LOCATION",	 "GOALTYPE_LOCATION_NEAREST_NODE",
...
{
public:
	CAI_NavInHintGroupFilter(string_t iszGroup = NULL_STRING) : m_iszGroup(iszGroup) {}
...
BEGIN_SIMPLE_DATADESC(CAI_Navigator)
	...
	DEFINE_FIELD(m_navType, FIELD_INTEGER),
		...
		DEFINE_FIELD(m_bValidateActivitySpeed, FIELD_BOOLEAN), DEFINE_FIELD(m_bCalledStartMove, FIELD_BOOLEAN),
		DEFINE_FIELD(m_fNavComplete, FIELD_BOOLEAN), DEFINE_FIELD(m_bNotOnNetwork, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bLastNavFailed, FIELD_BOOLEAN), DEFINE_FIELD(m_flNextSimplifyTime, FIELD_TIME),
		DEFINE_FIELD(m_bForcedSimplify, FIELD_BOOLEAN), DEFINE_FIELD(m_flLastSuccessfulSimplifyTime, FIELD_TIME),
		DEFINE_FIELD(m_flTimeLastAvoidanceTriangulate, FIELD_TIME), DEFINE_FIELD(m_timePathRebuildMax, FIELD_FLOAT),
		DEFINE_FIELD(m_timePathRebuildDelay, FIELD_FLOAT), DEFINE_FIELD(m_timePathRebuildFail, FIELD_TIME),
		DEFINE_FIELD(m_timePathRebuildNext, FIELD_TIME), DEFINE_FIELD(m_fRememberStaleNodes, FIELD_BOOLEAN),
		...
		//								m_hPeerWaitingOn	(peer move fields do not need to be saved, tied to current schedule and
		//path, which are not saved) 								m_PeerWaitMoveTimer	(ibid) 								m_PeerWaitClearTimer(ibid) 								m_NextSidestepTimer
		//(ibid)
		DEFINE_FIELD(m_hBigStepGroundEnt, FIELD_EHANDLE), DEFINE_FIELD(m_hLastBlockingEnt, FIELD_EHANDLE),
...
void CAI_Navigator::Restore(IRestore &restore)
{
	...
	if(minPathArray.Count())
	{
		...
		{
			m_pClippedWaypoints->PrependWaypoint(minPathArray[i].GetPos(), minPathArray[i].NavType(),
...
bool CAI_Navigator::SetGoal(const AI_NavGoal_t &goal, unsigned flags)
{
	...
	if(PostFrameNavigationSystem()->IsGameFrameRunning())
	{
		...
		PostFrameNavigationSystem()->EnqueueEntityNavigationQuery(
			GetOuter(), CreateFunctor(this, &CAI_Navigator::SetGoal, RefToVal(goal), flags));
...
bool CAI_Navigator::SetRadialGoal(const Vector &destination, const Vector &center, float radius, float arc,
								  float stepDist, bool bClockwise, bool bAirRoute)
{
	DbgNavMsg(GetOuter(), "Set radial goal\n");
	OnNewGoal();
	GetPath()->SetGoalType(GOALTYPE_LOCATION);

	GetPath()->SetWaypoints(GetPathfinder()->BuildRadialRoute(GetLocalOrigin(), center, destination, radius, arc,
															  stepDist, bClockwise, GetPath()->GetGoalTolerance(),
															  bAirRoute),
							true);
...
bool CAI_Navigator::SetWanderGoal(float minRadius, float maxRadius)
{
	...
	{
		return (SetWanderGoalByRandomVector(this, minRadius, maxRadius, 5) || SetRandomGoal(1));
	}
	else
	{
		return (SetRandomGoal(1) || SetWanderGoalByRandomVector(this, minRadius, maxRadius, 5));
	}
}
...
bool CAI_Navigator::FindVectorGoal(Vector *pResult, const Vector &dir, float targetDist, float minDist,
								   bool fShouldDeflect)
{
...
bool CAI_Navigator::PrependLocalAvoidance(float distObstacle, const AIMoveTrace_t &directTrace)
{
	...
	if(distObstacle < GetHullWidth() * 0.5)
	{
		...
		if(GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), vTestBackaway, MASK_NPCSOLID, GetNavTargetEntity(),
									 100.0, flags, &backawayTrace))
		{
	...
	AI_Waypoint_t *pTriangulation = GetPathfinder()->BuildTriangulationRoute(
		vStart, GetCurWaypointPos(), GetNavTargetEntity(), bits_WP_TO_DETOUR, NO_NODE, 0.0, distObstacle, GetNavType());
	...
	if(!GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), testPos, MASK_NPCSOLID, GetNavTargetEntity(), 100.0,
								  flags))
	{
...
bool CAI_Navigator::UpdateGoalPos(const Vector &goalPos)
{
	...
	if(PostFrameNavigationSystem()->IsGameFrameRunning())
	{
		// Send off the query for queuing
		PostFrameNavigationSystem()->EnqueueEntityNavigationQuery(
			GetOuter(), CreateFunctor(this, &CAI_Navigator::UpdateGoalPos, RefToVal(goalPos)));
...
{
	return GetNetwork()->GetNode((uintp)node)->GetPosition(GetHullType());
}
...
AIMoveResult_t CAI_Navigator::MoveClimb()
{
	...
	AIMoveTrace_t moveTrace;
	if(climbDist > 0.01 && !GetMoveProbe()->MoveLimit(NAV_CLIMB, GetLocalOrigin(),
	...
	AIMoveResult_t result =
		GetMotor()->MoveClimbExecute(climbDest, climbDir, climbDist, GetPath()->CurWaypointYaw(), climbNodesLeft);
	...
	if(result == AIMR_CHANGE_TYPE)
	{
		...
		if(!GetPath()->GetCurWaypoint() || !GetPath()->GetCurWaypoint()->GetNext() ||
...
AIMoveResult_t CAI_Navigator::MoveJump()
{
	...
	if((GetNavType() != NAV_JUMP) && (GetEntFlags() & FL_ONGROUND))
	{
		...
		// --------------------------------------------------
		AIMoveTrace_t moveTrace;
		GetMoveProbe()->MoveLimit(NAV_JUMP, GetLocalOrigin(), GetPath()->CurWaypointPos(), MASK_NPCSOLID,
	...
	{
		// DevMsg( "jump to %f %f %f landed %f %f %f", GetPath()->CurWaypointPos().x, GetPath()->CurWaypointPos().y,
		// GetPath()->CurWaypointPos().z, GetLocalOrigin().x, GetLocalOrigin().y, GetLocalOrigin().z );
...
bool CAI_Navigator::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult)
{
...
#endif
		{
			if(!fTargetIsGoal || (pMoveGoal->directTrace.fStatus != AIMR_BLOCKED_NPC) ||
	...
	if(ai_vehicle_avoidance.GetBool() && pMoveGoal->directTrace.pObstruction != NULL &&
	   pMoveGoal->directTrace.pObstruction->GetServerVehicle() != NULL)
	{
		...
		AI_Waypoint_t *pOBB = GetPathfinder()->BuildOBBAvoidanceRoute(GetOuter()->GetAbsOrigin(), GetGoalPos(),
																	  pMoveGoal->directTrace.pObstruction,
																	  GetNavTargetEntity(), GetNavType());
	...
	if(!m_hBigStepGroundEnt.Get() && pMoveGoal->directTrace.pObstruction && distClear < GetHullWidth() &&
		...
		if(m_NextSidestepTimer.Expired())
		{
			...
			for(int i = 1; i > -2; i -= 2)
			{
				...
				if(moveTrace.fStatus == AIMR_OK)
				{
					...
					GetMoveProbe()->MoveLimit(GetNavType(), vNewWaypoint, pMoveGoal->target, MASK_NPCSOLID_BRUSHONLY,
											  NULL, &moveTrace);
					if(moveTrace.fStatus == AIMR_OK)
					{
...
bool CAI_Navigator::OnFailedSteer(AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult)
{
	...
	if(!TestingSteering() && pMoveGoal->directTrace.fStatus == AIMR_BLOCKED_NPC &&
	...
	if(m_vPosBeginFailedSteer == vec3_invalid ||
	   (m_vPosBeginFailedSteer - GetAbsOrigin()).LengthSqr() > Square(MOVE_TOLERANCE))
	{
		m_vPosBeginFailedSteer = GetAbsOrigin();
		m_timeBeginFailedSteer = gpGlobals->curtime;
	}
	else if(GetNavType() == NAV_GROUND && gpGlobals->curtime - m_timeBeginFailedSteer > TIME_TOLERANCE &&
...
bool CAI_Navigator::DelayNavigationFailure(const AIMoveTrace_t &trace)
{
	...
	Assert(m_fPeerMoveWait == false ||
		   pBlocker == m_hPeerWaitingOn); // expected to be cleared each frame, and never call this function twice
...
bool CAI_Navigator::OnMoveExecuteFailed(const AILocalMoveGoal_t &move, const AIMoveTrace_t &trace,
										AIMotorMoveResult_t fMotorResult, AIMoveResult_t *pResult)
{
	// Allow the NPC to override this behavior
...
AIMoveResult_t CAI_Navigator::MoveNormal()
{
	...
	int nPreMoveSequence = GetOuter()->GetSequence(); // this is an unfortunate necessity to ensure setting back the
													  // activity picks the right one if it had been sticky
	...
	// --------------------------------
	// If we didn't actually move, but it was a success (i.e., blocked but within tolerance), make sure no visual
	// artifact
...
bool CAI_Navigator::IsMovingOutOfWay(const AILocalMoveGoal_t &moveGoal, float distClear)
{
	...
	CAI_BaseNPC *pBlocker =
		moveGoal.directTrace.pObstruction ? moveGoal.directTrace.pObstruction->MyNPCPointer() : NULL;
...
bool CAI_Navigator::Move(float flInterval)
{
	...
	if(!GetOuter()->OverrideMove(flInterval))
	{
		...
		if(fShouldMove)
		{
			...
			if(result == AINR_OK)
			{
				...
				if(IsMoveBlocked(moveResult))
				{
					...
					if(moveResult != AIMR_BLOCKED_NPC || GetNavType() == NAV_CLIMB || GetNavType() == NAV_JUMP ||
		...
		{
			// @TODO (toml 10-30-02): the climb part of this is unacceptable, but needed until navigation can handle
			// commencing 						  a navigation while in the middle of a climb
...
void CAI_Navigator::AdvancePath()
{
	...
	if(bPassingPathcorner)
	{
		Assert(!pCurWaypoint->GetNext() ||
			   (pCurWaypoint->GetNext()->Flags() & (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL)) ==
	...
	if(bPassingPathcorner)
	{
		...
		if(pCurWaypoint)
		{
			Assert((pCurWaypoint->Flags() & (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL)) ==
				   (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL));
...
bool CAI_Navigator::ShouldSimplifyTo(bool passedDetour, const Vector &pos)
{
	...
	AIMoveTrace_t moveTrace;
	GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), pos, MASK_NPCSOLID, GetPath()->GetTarget(), 100, flags,
							  &moveTrace);
											...
											bool skipTest, bool passedDetour, int *pTestCount)
{
	...
	if(distRemaining > 0)
	{
		if(pCurWaypoint->IsReducible())
		{
			...
			if(SimplifyPathForwardScan(params, pRecursionWaypoint, nextPoint, distRemaining, skipTestNext, passedDetour,
									   pTestCount))
				return true;
		}
	}
...
bool CAI_Navigator::SimplifyPathForward(float maxDist)
{
	...
	if(ShouldAttemptSimplifyTo(pNextWaypoint->GetPos()) &&
	   ...
	   ShouldSimplifyTo(((pCurWaypoint->Flags() & bits_WP_TO_DETOUR) != 0),
						pNextWaypoint->GetPos())) // @TODO (toml 04-25-03): need to handle this better. this is here
												  // because forward scan may look out so far that a close obvious
												  // solution is skipped (due to test limiting)
	{
		delete pCurWaypoint;
...
bool CAI_Navigator::SimplifyPathBacktrack()
{
	...
	if(pCurWaypoint->GetNext() && (pNextWaypoint->Flags() & bits_WP_TO_NODE) &&
	   (pNextWaypoint->NavType() == NAV_GROUND) && (pCurWaypoint->NavType() == NAV_GROUND) &&
	   (pCurWaypoint->Flags() & bits_WP_TO_NODE))
	{
		...
		if(firstProj > 0.5 * firstDist)
		{
			...
			AI_Waypoint_t *route1 =
				GetPathfinder()->BuildLocalRoute(GetLocalOrigin(), targetPos, GetPath()->GetTarget(), bits_WP_TO_DETOUR,
												 NO_NODE, buildFlags, goalTolerance);
			if(!route1)
				return false;

			// Make sure the new point gets me to the target location
			AI_Waypoint_t *route2 =
				GetPathfinder()->BuildLocalRoute(targetPos, pNextWaypoint->GetPos(), GetPath()->GetTarget(),
...
bool CAI_Navigator::SimplifyPath(bool bFirstForPath, float scanDist)
{
	...
	if(navType == NAV_GROUND || navType == NAV_FLY)
	{
		...
		{
			float simplifyDelay =
				(bInPVS) ? ROUTE_SIMPLIFY_TIME_DELAY[AIStrongOpt()] : NO_PVS_ROUTE_SIMPLIFY_TIME_DELAY[AIStrongOpt()];
		...
		{
			if(!AIStrongOpt() ||
			   gpGlobals->curtime - m_flLastSuccessfulSimplifyTime > QUICK_SIMPLIFY_TIME_DELAY[AIStrongOpt()])
			{
	...
	if(IsGoalActive())
	{
		...
		else
		{
			bool bIsStrictWaypoint =
				((GetPath()->CurWaypointFlags() & (bits_WP_TO_PATHCORNER | bits_WP_DONT_SIMPLIFY)) != 0);
			float tolerance = (bIsStrictWaypoint) ? params.strictPointTolerance : params.waypointTolerance;
			if(waypointDist <= tolerance)
			{
				trace_t tr;
				AI_TraceLine(GetAbsOrigin(), GetPath()->GetCurWaypoint()->GetNext()->GetPos(), MASK_NPCSOLID,
...
void CAI_Navigator::SimplifyFlyPath(unsigned collisionMask, const CBaseEntity *pTarget, float strictPointTolerance,
									float blockTolerance, AI_NpcBlockHandling_t blockHandling)
{
...
bool CAI_Navigator::SimplifyFlyPath(const AI_ProgressFlyPathParams_t &params)
{
	...
	bool bIsStrictWaypoint = (!params.bTrySimplify ||
							  ((GetPath()->CurWaypointFlags() & (bits_WP_TO_PATHCORNER | bits_WP_DONT_SIMPLIFY)) != 0));
	...
	if(!bIsStrictWaypoint || length < params.strictPointTolerance)
	{
		...
		AIMoveTrace_t moveTrace;
		GetMoveProbe()->MoveLimit(NAV_FLY, GetLocalOrigin(), GetPath()->NextWaypointPos(), params.collisionMask,
								  params.pTarget, &moveTrace);
...
{
	CTraceFilterNav traceFilter(const_cast<CAI_BaseNPC *>(GetOuter()), bIgnoreTransients, GetOuter(),
...
{
	return (GetPath()->GetCurWaypoint()) ? (GetPathDistToCurWaypoint() + GetPath()->GetCurWaypoint()->flPathDistGoal)
										 : 0;
}
...
bool CAI_Navigator::MarkCurWaypointFailedLink(void)
{
	...
	if(endID != NO_NODE)
	{
		...
		if(m_hLastBlockingEnt != NULL && !m_hLastBlockingEnt->IsPlayer() && !m_hLastBlockingEnt->IsNPC() &&
		   m_hLastBlockingEnt->GetMoveType() == MOVETYPE_VPHYSICS && m_hLastBlockingEnt->VPhysicsGetObject() &&
			...
			m_hLastBlockingEnt->VPhysicsGetObject()->GetMass() > 200))
		{
			...
			if(bFoundLarge && !bFoundSmall)
			{
				...
				UTIL_TraceModel(vStartPos, vEndPos, GetHullMins(), GetHullMaxs(), m_hLastBlockingEnt,
...
bool CAI_Navigator::DoFindPathToPos(void)
{
	...
	AI_Waypoint_t *pFirstWaypoint = pPathfinder->BuildRoute(origin, actualGoalPos, pTarget, tolerance, GetNavType(),
	...
	if(pFirstWaypoint->IsReducible() && pFirstWaypoint->GetNext() &&
	   pFirstWaypoint->GetNext()->NavType() == GetNavType() && ShouldOptimizeInitialPathSegment(pFirstWaypoint))
	{
		// If we're seemingly beyond the waypoint, and our hull is over the line, move on
		const float EPS = 0.1;
		Vector vClosest;
		CalcClosestPointOnLineSegment(origin, pFirstWaypoint->GetPos(), pFirstWaypoint->GetNext()->GetPos(), vClosest);
		if((pFirstWaypoint->GetPos() - vClosest).Length() > EPS && (origin - vClosest).Length() < GetHullWidth() * 0.5)
		{
			pPath->Advance();
		}
	}
	...
	if(pPathCorner != NULL)
	{
		...
		if((GetGoalFlags() & AIN_NO_PATHCORNER_PATHFINDING) || m_bNoPathcornerPathfinds)
		{
			...
			AI_Waypoint_t *pRoute =
				new AI_Waypoint_t(pPathCorner->GetLocalOrigin(), 0, GetNavType(), bits_WP_TO_PATHCORNER, NO_NODE);
		...
		else
		{
			...
			if((returnCode = DoFindPathToPos()) != false)
			{
				...
				if(pPathCorner)
				{
					...
					AI_Waypoint_t *curWaypoint = new AI_Waypoint_t(pPathCorner->GetLocalOrigin(), 0, GetNavType(),
...
bool CAI_Navigator::GetStoppingPath(CAI_WaypointList *pClippedWaypoints)
{
	...
	if(pCurWaypoint)
	{
		...
		{
			float distToCurrent =
				ComputePathDistance(pCurWaypoint->NavType(), GetLocalOrigin(), pCurWaypoint->GetPos());
			if(pCurWaypoint->NavType() == NAV_CLIMB)
			{
				if(pCurWaypoint->GetNext() && pCurWaypoint->GetNext()->NavType() == NAV_CLIMB)
					distToCurrent +=
						ComputePathDistance(NAV_CLIMB, pCurWaypoint->GetPos(), pCurWaypoint->GetNext()->GetPos());
		...
		if(bMustCompleteCurrent || distRemaining > 0.1)
		{
			...
			{
				if((pNextPoint->NavType() == NAV_CLIMB || pNextPoint->NavType() == NAV_JUMP) && !bMustCompleteCurrent)
				{
					break;
				}
				...
				else
				{
					...
					AIMoveTrace_t trace;
					if(GetMoveProbe()->MoveLimit(pNextPoint->NavType(), vPosPrev, remainder, MASK_NPCSOLID, NULL, 100,
												 AIMLF_DEFAULT | AIMLF_2D, &trace))
					{
						pNewWaypoint =
							new AI_Waypoint_t(trace.vEndPosition, yaw, pNextPoint->NavType(), bits_WP_TO_GOAL, 0);
	...
	if(m_pClippedWaypoints && !m_pClippedWaypoints->IsEmpty())
	{
		if(m_PreviousMoveActivity <= ACT_RESET && GetMovementActivity() <= ACT_RESET)
		{
			...
			DevWarning(2, "%s has a stopping path and no valid. Movement activity: %s (prev %s)\n",
					   GetOuter()->GetDebugName(), ActivityList_NameForIndex(GetMovementActivity()),
			...
			m_pClippedWaypoints->GetFirst()->NavType() == NAV_JUMP))
		{
			...
			if(pCurTask && pCurTask->iTask == TASK_STOP_MOVING)
			{
				// Clipped paths are used for 2 reasons: Prepending movement that must be finished in the case of
				// climbing/jumping, and bringing an NPC to a stop. In the second case, we should never be starting
				// climb or jump movement.
				m_pClippedWaypoints->RemoveAll();
				return false;
			}
		}
