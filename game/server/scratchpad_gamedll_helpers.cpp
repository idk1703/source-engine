...
void ScratchPad_DrawWorldToScratchPad(IScratchPad3D *pPad, unsigned int flags)
{
	...
	if(flags & (SPDRAWWORLD_DRAW_PLAYERS | SPDRAWWORLD_DRAW_ENTITIES))
	{
		...
		while(pCur)
		{
			...
			if((bPlayer && !(flags & SPDRAWWORLD_DRAW_PLAYERS)) || (!bPlayer && !(flags & SPDRAWWORLD_DRAW_ENTITIES)))
			{
				pCur = gEntList.NextEnt(pCur);
				continue;
			}

			ScratchPad_DrawEntityToScratchPad(pPad, flags, pCur, bPlayer ? Vector(1.0, 0.5, 0) : Vector(0.3, 0.3, 1.0));
