...
BEGIN_SIMPLE_DATADESC(AI_LeadArgs_t)
	...
	DEFINE_FIELD(flags, FIELD_INTEGER),
		...
		DEFINE_FIELD(iRetrievePlayer, FIELD_INTEGER), DEFINE_FIELD(iRetrieveWaitForSpeak, FIELD_INTEGER),
		DEFINE_FIELD(iComingBackWaitForSpeak, FIELD_INTEGER), DEFINE_FIELD(bStopScenesWhenPlayerLost, FIELD_BOOLEAN),
...
BEGIN_DATADESC(CAI_LeadBehavior)
	DEFINE_EMBEDDED(m_args),
		...
		DEFINE_FIELD(m_hSinkImplementor, FIELD_EHANDLE), DEFINE_FIELD(m_goal, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_goalyaw, FIELD_FLOAT), DEFINE_FIELD(m_waitpoint, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_waitdistance, FIELD_FLOAT), DEFINE_FIELD(m_leaddistance, FIELD_FLOAT),
		DEFINE_FIELD(m_retrievedistance, FIELD_FLOAT), DEFINE_FIELD(m_successdistance, FIELD_FLOAT),
		DEFINE_FIELD(m_weaponname, FIELD_STRING), DEFINE_FIELD(m_run, FIELD_BOOLEAN),
		DEFINE_FIELD(m_gagleader, FIELD_BOOLEAN), DEFINE_FIELD(m_hasspokenstart, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hasspokenarrival, FIELD_BOOLEAN), DEFINE_FIELD(m_hasPausedScenes, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flSpeakNextNagTime, FIELD_TIME), DEFINE_FIELD(m_flWeaponSafetyTimeOut, FIELD_TIME),
		DEFINE_FIELD(m_flNextLeadIdle, FIELD_TIME), DEFINE_FIELD(m_bInitialAheadTest, FIELD_BOOLEAN),
		DEFINE_EMBEDDED(m_MoveMonitor), DEFINE_EMBEDDED(m_LostTimer), DEFINE_EMBEDDED(m_LostLOSTimer),
END_DATADESC();
...
bool CAI_LeadBehavior::SetGoal(const AI_LeadArgs_t &args)
{
	...
	m_retrievedistance =
		args.flRetrieveDistance ? args.flRetrieveDistance : (m_leaddistance + LEAD_MIN_RETRIEVEDIST_OFFSET);
...
bool CAI_LeadBehavior::GetClosestPointOnRoute(const Vector &targetPos, Vector *pVecClosestPoint)
{
	...
	if(!waypoint)
	{
		...
		//	- When the lead behaviour is first enabled. We have no schedule. We want to know if the player is ahead of
		//us.
		...
		builtwaypoints = GetOuter()->GetPathfinder()->BuildRoute(GetOuter()->GetAbsOrigin(), m_goal, NULL,
																 GetOuter()->GetDefaultNavGoalTolerance(),
																 GetOuter()->GetNavType(), true);
		if(!builtwaypoints)
			return false;
...
void CAI_LeadBehavior::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_LEAD_WALK_PATH:
		{
			...
			if(m_run &&
			   (IsCurSchedule(SCHED_LEAD_WAITFORPLAYER, false) || IsCurSchedule(SCHED_LEAD_PLAYER, false) ||
				IsCurSchedule(SCHED_LEAD_SPEAK_THEN_LEAD_PLAYER, false) || IsCurSchedule(SCHED_LEAD_RETRIEVE, false)))
			{
		...
		case TASK_LEAD_WALK_PATH:
		{
			...
			if(TaskIsRunning() && IsCurSchedule(SCHED_LEAD_PLAYER, false))
			{
				...
				if(m_flNextLeadIdle < gpGlobals->curtime && !IsSpeaking())
				{
					...
					if(!m_args.iRetrievePlayer && HasCondition(COND_LEAD_FOLLOWER_LOST) &&
...
bool CAI_LeadBehavior::Speak(AIConcept_t concept)
{
	...
	bool bNag = (FStrEq(concept, TLK_LEAD_COMINGBACK) || FStrEq(concept, TLK_LEAD_CATCHUP) ||
				 FStrEq(concept, TLK_LEAD_RETRIEVE));
...
bool CAI_LeadBehavior::Connect(CAI_LeadBehaviorHandler *pSink)
{
	...
	if(m_hSinkImplementor == NULL)
		DevMsg(2, "Note: CAI_LeadBehaviorHandler connected to a sink that isn't an entity. Manual fixup on load will "
				  "be necessary\n");
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_LeadBehavior)
	...
	DEFINE_SCHEDULE(SCHED_LEAD_RETRIEVE,
	...
	DEFINE_SCHEDULE(SCHED_LEAD_PAUSE,
	...
	DEFINE_SCHEDULE(SCHED_LEAD_PAUSE_COMBAT,
	...
	DEFINE_SCHEDULE(SCHED_LEAD_WAITFORPLAYER,
	...
	DEFINE_SCHEDULE(SCHED_LEAD_WAITFORPLAYERIDLE,
	...
	DEFINE_SCHEDULE(SCHED_LEAD_PLAYERNEEDSWEAPON,
...
BEGIN_DATADESC(CAI_LeadGoal)
	...
	DEFINE_FIELD(m_fArrived, FIELD_BOOLEAN),
		...
		DEFINE_KEYFIELD(m_bRun, FIELD_BOOLEAN, "Run"), DEFINE_KEYFIELD(m_iRetrievePlayer, FIELD_INTEGER, "Retrieve"),
		...
		DEFINE_OUTPUT(m_OnSuccess, "OnSuccess"), DEFINE_OUTPUT(m_OnArrival, "OnArrival"),
		DEFINE_OUTPUT(m_OnArrivalDone, "OnArrivalDone"), DEFINE_OUTPUT(m_OnFailure, "OnFailure"),
...
void CAI_LeadGoal::InputActivate(inputdata_t &inputdata)
{
	...
	{
		Warning("ai_goal_lead '%s': lead distance (%.2f) * 4 is < retrieve distance (%.2f). This will make the NPC act "
				"stupid. Either reduce the retrieve distance, or increase the lead distance.\n",
	...
	{
#ifdef HL2_EPISODIC
		Warning("ai_goal_lead '%s': retrieve distance (%.2f) < lead distance (%.2f) + %d. Retrieve distance should be "
				"at least %d greater than the lead distance, or NPC will ping-pong while retrieving.\n",
				GetDebugName(), m_flRetrieveDistance, m_flLeadDistance, LEAD_MIN_RETRIEVEDIST_OFFSET,
				LEAD_MIN_RETRIEVEDIST_OFFSET);
#endif
...
{
	if(m_iszStartConceptModifier != NULL_STRING && *STRING(m_iszStartConceptModifier) &&
	   strcmp(pszConcept, TLK_LEAD_START) == 0)
		return STRING(m_iszStartConceptModifier);

	if(m_iszAttractPlayerConceptModifier != NULL_STRING && *STRING(m_iszAttractPlayerConceptModifier) &&
	   strcmp(pszConcept, TLK_LEAD_ATTRACTPLAYER) == 0)
		return STRING(m_iszAttractPlayerConceptModifier);

	if(m_iszWaitOverConceptModifier != NULL_STRING && *STRING(m_iszWaitOverConceptModifier) &&
	   strcmp(pszConcept, TLK_LEAD_WAITOVER) == 0)
		return STRING(m_iszWaitOverConceptModifier);

	if(m_iszArrivalConceptModifier != NULL_STRING && *STRING(m_iszArrivalConceptModifier) &&
	   strcmp(pszConcept, TLK_LEAD_ARRIVAL) == 0)
		return STRING(m_iszArrivalConceptModifier);

	if(m_iszSuccessConceptModifier != NULL_STRING && *STRING(m_iszSuccessConceptModifier) &&
	   strcmp(pszConcept, TLK_LEAD_SUCCESS) == 0)
		return STRING(m_iszSuccessConceptModifier);

	if(m_iszFailureConceptModifier != NULL_STRING && *STRING(m_iszFailureConceptModifier) &&
	   strcmp(pszConcept, TLK_LEAD_FAILURE) == 0)
		return STRING(m_iszFailureConceptModifier);

	if(m_iszRetrieveConceptModifier != NULL_STRING && *STRING(m_iszRetrieveConceptModifier) &&
	   strcmp(pszConcept, TLK_LEAD_RETRIEVE) == 0)
		return STRING(m_iszRetrieveConceptModifier);

	if(m_iszComingBackConceptModifier != NULL_STRING && *STRING(m_iszComingBackConceptModifier) &&
...
{
	if(m_iszMissingWeaponConceptModifier != NULL_STRING && *STRING(m_iszMissingWeaponConceptModifier) &&
