...
{
	m_hRemotePortal =
		dynamic_cast<CTriggerPortal *>(gEntList.FindEntityByName(NULL, strRemotePortalName, NULL, NULL, NULL));
...
void CTriggerPortal::Touch(CBaseEntity *pOther)
{
	...
	if(PassesTriggerFilters(pOther))
	{
		...
		if(bDebug)
		{
			NDebugOverlay::Box(pOther->GetAbsOrigin(), pOther->WorldAlignMins(), pOther->WorldAlignMaxs(), 255, 0, 0, 8,
							   20);
		...
		if(bDebug)
		{
			NDebugOverlay::Box(pOther->GetAbsOrigin(), pOther->WorldAlignMins(), pOther->WorldAlignMaxs(), 0, 255, 0, 8,
							   20);
