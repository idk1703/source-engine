...
BEGIN_DATADESC(CBasePlatTrain)
	...
	DEFINE_KEYFIELD(m_NoiseMoving, FIELD_SOUNDNAME, "noise1"),
		...
		DEFINE_FIELD(m_flTWidth, FIELD_FLOAT), DEFINE_FIELD(m_flTLength, FIELD_FLOAT),
		DEFINE_KEYFIELD(m_flLip, FIELD_FLOAT, "lip"), DEFINE_KEYFIELD(m_flWait, FIELD_FLOAT, "wait"),
...
BEGIN_DATADESC(CFuncPlat)
	...
	DEFINE_FIELD(m_sNoise, FIELD_STRING),
		...
		// Function Pointers
		DEFINE_FUNCTION(PlatUse), DEFINE_FUNCTION(CallGoDown), DEFINE_FUNCTION(CallHitTop),
		DEFINE_FUNCTION(CallHitBottom),

		// Inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle), DEFINE_INPUTFUNC(FIELD_VOID, "GoUp", InputGoUp),
...
BEGIN_DATADESC(CFuncTrain)
	...
	DEFINE_FIELD(m_hCurrentTarget, FIELD_EHANDLE), DEFINE_FIELD(m_activated, FIELD_BOOLEAN),
		DEFINE_FIELD(m_hEnemy, FIELD_EHANDLE), DEFINE_FIELD(m_iszLastTarget, FIELD_STRING),
		...
		// Inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle), DEFINE_INPUTFUNC(FIELD_VOID, "Start", InputStart),
...
BEGIN_DATADESC(CFuncTrackTrain)
	...
	DEFINE_KEYFIELD(m_length, FIELD_FLOAT, "wheels"), DEFINE_KEYFIELD(m_height, FIELD_FLOAT, "height"),
		DEFINE_KEYFIELD(m_maxSpeed, FIELD_FLOAT, "startspeed"), DEFINE_KEYFIELD(m_flBank, FIELD_FLOAT, "bank"),
		DEFINE_KEYFIELD(m_flBlockDamage, FIELD_FLOAT, "dmg"),
		...
		DEFINE_FIELD(m_controlMins, FIELD_VECTOR), DEFINE_FIELD(m_controlMaxs, FIELD_VECTOR),
		DEFINE_FIELD(m_flVolume, FIELD_FLOAT), DEFINE_FIELD(m_oldSpeed, FIELD_FLOAT),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle), DEFINE_INPUTFUNC(FIELD_VOID, "Resume", InputResume),
		DEFINE_INPUTFUNC(FIELD_VOID, "Reverse", InputReverse), DEFINE_INPUTFUNC(FIELD_FLOAT, "SetSpeed", InputSetSpeed),
...
int CFuncTrackTrain::DrawDebugTextOverlays(void)
{
	...
	{
		char tempstr[512];
		Q_snprintf(tempstr, sizeof(tempstr), "angles: %g %g %g", (double)GetLocalAngles()[PITCH],
				   (double)GetLocalAngles()[YAW], (double)GetLocalAngles()[ROLL]);
...
void CFuncTrackTrain::SoundUpdate(void)
{
	...
	if(!m_bSoundPlaying)
	{
		...
		m_flNextMoveSoundTime =
			gpGlobals->curtime + RemapVal(flSpeedRatio, 0, 1, m_flMoveSoundMaxTime, m_flMoveSoundMinTime);
...
// Input  : pnext -
//-----------------------------------------------------------------------------
void CFuncTrackTrain::UpdateTrainOrientation(CPathTrack *pPrev, CPathTrack *pNext, const Vector &nextPos,
											 float flInterval)
{
...
//-----------------------------------------------------------------------------
void CFuncTrackTrain::UpdateOrientationAtPathTracks(CPathTrack *pPrev, CPathTrack *pNext, const Vector &nextPos,
													float flInterval)
{
	if(!m_ppath)
		return;
...
//			FIXME: move into path_track, have the angles come back from LookAhead
//-----------------------------------------------------------------------------
void CFuncTrackTrain::UpdateOrientationBlend(TrainOrientationType_t eOrientationType, CPathTrack *pPrev,
...
void CFuncTrackTrain::Next(void)
{
	...
	if(m_debugOverlays & OVERLAY_BBOX_BIT)
	{
		...
		if(pNextNext != NULL)
		{
			...
			NDebugOverlay::Line(pNextNext->GetAbsOrigin(), pNextNext->GetAbsOrigin() + Vector(0, 0, 32), 0, 255, 0,
								true, 0.1);
...
void CFuncTrackTrain::NearestPath(void)
{
	...
	for(CEntitySphereQuery sphere(GetAbsOrigin(), 1024); (pTrack = sphere.GetCurrentEntity()) != NULL;
		sphere.NextEntity())
	{
...
BEGIN_DATADESC(CFuncTrackChange)
	...
	DEFINE_GLOBAL_FIELD(m_trackTop, FIELD_CLASSPTR), DEFINE_GLOBAL_FIELD(m_trackBottom, FIELD_CLASSPTR),
		DEFINE_GLOBAL_FIELD(m_train, FIELD_CLASSPTR), DEFINE_GLOBAL_KEYFIELD(m_trackTopName, FIELD_STRING, "toptrack"),
		DEFINE_GLOBAL_KEYFIELD(m_trackBottomName, FIELD_STRING, "bottomtrack"),
		DEFINE_GLOBAL_KEYFIELD(m_trainName, FIELD_STRING, "train"), DEFINE_FIELD(m_code, FIELD_INTEGER),
		DEFINE_FIELD(m_targetState, FIELD_INTEGER), DEFINE_FIELD(m_use, FIELD_INTEGER),
