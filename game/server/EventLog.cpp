...
bool CEventLog::PrintPlayerEvent(IGameEvent *event)
{
	...
	if(!Q_strncmp(eventName, "player_connect",
				  Q_strlen("player_connect"))) // player connect is before the CBasePlayer pointer is setup
	{
	...
	if(!Q_strncmp(eventName, "player_team", Q_strlen("player_team")))
	{
		...
		if(!bDisconnecting)
		{
			...
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" joined team \"%s\"\n", pPlayer->GetPlayerName(), pPlayer->GetUserID(),
						   pPlayer->GetNetworkIDString(), oldteam->GetName(), team->GetName());
		}
	...
	else if(!Q_strncmp(eventName, "player_death", Q_strlen("player_death")))
	{
		...
		if(pPlayer == pAttacker && pPlayer)
		{
			...
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" committed suicide with \"%s\"\n", pPlayer->GetPlayerName(), userid,
						   pPlayer->GetNetworkIDString(), team ? team->GetName() : "", weapon);
#else
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" committed suicide with \"%s\"\n", pPlayer->GetPlayerName(), userid,
						   pPlayer->GetNetworkIDString(), team ? team->GetName() : "", pAttacker->GetClassname());
#endif
		}
		else if(pAttacker)
		{
			...
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\" with \"%s\"\n", pAttacker->GetPlayerName(),
						   attackerid, pAttacker->GetNetworkIDString(), attackerTeam ? attackerTeam->GetName() : "",
						   pPlayer->GetPlayerName(), userid, pPlayer->GetNetworkIDString(), team ? team->GetName() : "",
						   weapon);
#else
			UTIL_LogPrintf("\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\"\n", pAttacker->GetPlayerName(), attackerid,
						   pAttacker->GetNetworkIDString(), attackerTeam ? attackerTeam->GetName() : "",
	...
	else if(!Q_strncmp(eventName, "player_changename", Q_strlen("player_changename")))
	{
		...
		CTeam *team = pPlayer->GetTeam();
		UTIL_LogPrintf("\"%s<%i><%s><%s>\" changed name to \"%s\"\n", oldName, userid, pPlayer->GetNetworkIDString(),
