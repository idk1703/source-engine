...
private:
	...
	void UpdateOrientationAtPathTracks(CPathTrack *pnext, CPathTrack *pNextNext, const Vector &nextPos,
									   float flInterval);
	void UpdateOrientationBlend(TrainOrientationType_t eOrientationType, CPathTrack *pPrev, CPathTrack *pNext,
	...
	Vector m_lastBlockPos; // These are used to build a heuristic decision about being temporarily blocked by physics
						   // objects
	int m_lastBlockTick;   // ^^^^^^^
	float m_flVolume;
	float m_flBank;
	...
	int m_nMoveSoundMaxPitch; // The sound pitch to approach as we approach our max speed (actually, it's hardcoded to
							  // 1000 in/sec)
