...
BEGIN_DATADESC(CNPC_VehicleDriver)
	DEFINE_KEYFIELD(m_iszVehicleName, FIELD_STRING, "vehicle"),
		...
		DEFINE_FIELD(m_vecDesiredVelocity, FIELD_VECTOR), DEFINE_FIELD(m_vecDesiredPosition, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vecPrevPoint, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecPrevPrevPoint, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vecPostPoint, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecPostPostPoint, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_flDistanceAlongSpline, FIELD_FLOAT),
		DEFINE_KEYFIELD(m_flDriversMaxSpeed, FIELD_FLOAT, "drivermaxspeed"),
		DEFINE_KEYFIELD(m_flDriversMinSpeed, FIELD_FLOAT, "driverminspeed"), DEFINE_FIELD(m_flMaxSpeed, FIELD_FLOAT),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Stop", InputStop), DEFINE_INPUTFUNC(FIELD_VOID, "StartFiring", InputStartFiring),
...
void CNPC_VehicleDriver::Activate(void)
{
	...
	if(!m_pVehicleInterface->NPC_CanDrive())
	{
		Warning("npc_vehicledriver %s doesn't know how to drive vehicle %s.\n", STRING(GetEntityName()),
				STRING(m_hVehicleEntity->GetEntityName()));
		UTIL_Remove(this);
		return;
	}
...
void CNPC_VehicleDriver::StartTask(const Task_t *pTask)
{
	switch(pTask->iTask)
	{
		...
		case TASK_VEHICLEDRIVER_GET_PATH:
		{
			...
			{
				NDebugOverlay::Box(GetGoalEnt()->GetAbsOrigin(), -Vector(50, 50, 50), Vector(50, 50, 50), 255, 255, 255,
								   true, 5);
			}
	...
	if(pCurWaypoint->Flags() & bits_WP_TO_PATHCORNER)
	{
		...
		if(pEntity)
		{
			if(pEntity->m_flSpeed > 0)
			{
				...
				{
					Warning("path_track %s tried to tell the npc_vehicledriver to set speed to %.3f. npc_vehicledriver "
...
bool CNPC_VehicleDriver::OverridePathMove(float flInterval)
{
	...
	Vector vSplinePoint = pCurrentSplineBeingTraversed->GetPointAt(
		m_flDistanceAlongSpline > 1 ? m_flDistanceAlongSpline - 1 : m_flDistanceAlongSpline);
	...
	{
		VectorLerp(vSplineTangent, vecToDesired * flTangentLength, (flDistToDesired / (flTangentLength * 0.5)),
	...
	if(g_debug_vehicledriver.GetInt() & DRIVER_DEBUG_PATH)
	{
		...
		// pCurrentSplineBeingTraversed->splinePoints[1], 30, 255,255,255,0, false, 0.1f ); NDebugOverlay::HorzArrow(
		// pCurrentSplineBeingTraversed->splinePoints[1], pCurrentSplineBeingTraversed->splinePoints[2], 20,
		// 255,255,255,0, false, 0.1f ); NDebugOverlay::HorzArrow( pCurrentSplineBeingTraversed->splinePoints[2],
		// pCurrentSplineBeingTraversed->splinePoints[3], 10, 255,255,255,0, false, 0.1f );
