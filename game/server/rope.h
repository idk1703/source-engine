...
public:
	...
	static CRopeKeyframe *Create(CBaseEntity *pStartEnt, CBaseEntity *pEndEnt, int iStartAttachment = 0,
								 int iEndAttachment = 0, int ropeWidth = 2,
								 const char *pMaterialName = "cable/cable.vmt", // Note: whoever creates the rope must
																				// use PrecacheModel for whatever
																				// material it specifies here.
								 int numSegments = 5);
...
public:
	...
	void PropagateForce(CBaseEntity *pActivator, CBaseEntity *pCaller, CBaseEntity *pFirstLink, float x, float y,
						float z);
