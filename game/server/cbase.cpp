...
BEGIN_SIMPLE_DATADESC(CEventAction)
	DEFINE_FIELD(m_iTarget, FIELD_STRING), DEFINE_FIELD(m_iTargetInput, FIELD_STRING),
		DEFINE_FIELD(m_iParameter, FIELD_STRING), DEFINE_FIELD(m_flDelay, FIELD_FLOAT),
		DEFINE_FIELD(m_nTimesToFire, FIELD_INTEGER), DEFINE_FIELD(m_iIDStamp, FIELD_INTEGER),
...
CUtlMemoryPool g_EntityListPool(MAX(sizeof(CEventAction), sizeof(CMultiInputVar::inputitem_t)), 512,
								CUtlMemoryPool::GROW_FAST, "g_EntityListPool");
...
void CBaseEntityOutput::FireOutput(variant_t Value, CBaseEntity *pActivator, CBaseEntity *pCaller, float fDelay)
{
	...
	while(ev != NULL)
	{
		if(ev->m_iParameter == NULL_STRING)
		{
			...
			//
			g_EventQueue.AddEvent(STRING(ev->m_iTarget), STRING(ev->m_iTargetInput), Value, ev->m_flDelay + fDelay,
								  pActivator, pCaller, ev->m_iIDStamp);
		}
		else
		{
			//
			...
			g_EventQueue.AddEvent(STRING(ev->m_iTarget), STRING(ev->m_iTargetInput), ValueOverride, ev->m_flDelay,
								  pActivator, pCaller, ev->m_iIDStamp);
		}
		...
		if(ev->m_flDelay)
		{
			...
			Q_snprintf(szBuffer, sizeof(szBuffer), "(%0.2f) output: (%s,%s) -> (%s,%s,%.1f)(%s)\n",
					   ...
					   STRING(ev->m_iTargetInput), ev->m_flDelay, STRING(ev->m_iParameter));
		...
		else
		{
			...
			Q_snprintf(szBuffer, sizeof(szBuffer), "(%0.2f) output: (%s,%s) -> (%s,%s)(%s)\n",
					   ...
					   pCaller ? STRING(pCaller->m_iClassname) : "NULL",
					   pCaller ? STRING(pCaller->GetEntityName()) : "NULL", STRING(ev->m_iTarget),
					   STRING(ev->m_iTargetInput), STRING(ev->m_iParameter));
...
public:
	...
	{
		// No reason why any future version shouldn't try to retain backward compatability. The default here is to not
		// do so.
		short version;
...
void CEventQueue::Dump(void)
{
	...
	while(pe != NULL)
	{
		...
		Msg("   (%.2f) Target: '%s', Input: '%s', Parameter '%s'. Activator: '%s', Caller '%s'.  \n", pe->m_flFireTime,
			STRING(pe->m_iTarget), STRING(pe->m_iTargetInput), pe->m_VariantValue.String(),
...
//-----------------------------------------------------------------------------
void CEventQueue::AddEvent(const char *target, const char *targetInput, variant_t Value, float fireDelay,
						   CBaseEntity *pActivator, CBaseEntity *pCaller, int outputID)
{
	// build the new event
...
//-----------------------------------------------------------------------------
void CEventQueue::AddEvent(CBaseEntity *target, const char *targetInput, variant_t Value, float fireDelay,
						   CBaseEntity *pActivator, CBaseEntity *pCaller, int outputID)
{
	// build the new event
	EventQueuePrioritizedEvent_t *newEvent = new EventQueuePrioritizedEvent_t;
...
void CEventQueue::AddEvent(CBaseEntity *target, const char *action, float fireDelay, CBaseEntity *pActivator,
						   CBaseEntity *pCaller, int outputID)
{
	variant_t Value;
...
void CEventQueue::ServiceEvents(void)
{
...
#endif
	{
		...
		if(pe->m_iTarget != NULL_STRING)
		{
			...
			{
				target =
					gEntList.FindEntityByName(target, pe->m_iTarget, pSearchingEntity, pe->m_pActivator, pe->m_pCaller);
				if(!target)
					break;

				// pump the action into the target
				target->AcceptInput(STRING(pe->m_iTargetInput), pe->m_pActivator, pe->m_pCaller, pe->m_VariantValue,
									pe->m_iOutputID);
				targetFound = true;
			}
		}
		...
		{
			pe->m_pEntTarget->AcceptInput(STRING(pe->m_iTargetInput), pe->m_pActivator, pe->m_pCaller,
										  pe->m_VariantValue, pe->m_iOutputID);
			targetFound = true;
		}
...
static ConCommand dumpeventqueue("dumpeventqueue", CC_DumpEventQueue,
...
BEGIN_SIMPLE_DATADESC(EventQueuePrioritizedEvent_t)
	DEFINE_FIELD(m_flFireTime, FIELD_TIME), DEFINE_FIELD(m_iTarget, FIELD_STRING),
		DEFINE_FIELD(m_iTargetInput, FIELD_STRING), DEFINE_FIELD(m_pActivator, FIELD_EHANDLE),
		DEFINE_FIELD(m_pCaller, FIELD_EHANDLE), DEFINE_FIELD(m_pEntTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_iOutputID, FIELD_INTEGER), DEFINE_CUSTOM_FIELD(m_VariantValue, variantFuncs),
...
int CEventQueue::Restore(IRestore &restore)
{
	...
	{
		if(!restore.ReadFields("PEvent", &tmpEvent, NULL, tmpEvent.m_DataMap.dataDesc,
							   tmpEvent.m_DataMap.dataNumFields))
			return 0;

		// add the restored event into the list
		if(tmpEvent.m_pEntTarget)
		{
			AddEvent(tmpEvent.m_pEntTarget, STRING(tmpEvent.m_iTargetInput), tmpEvent.m_VariantValue,
#ifdef TF_DLL
					 tmpEvent.m_flFireTime - engine->GetServerTime(),
#else
					  tmpEvent.m_flFireTime - gpGlobals->curtime,
#endif
					 tmpEvent.m_pActivator, tmpEvent.m_pCaller, tmpEvent.m_iOutputID);
		}
		else
		{
			AddEvent(STRING(tmpEvent.m_iTarget), STRING(tmpEvent.m_iTargetInput), tmpEvent.m_VariantValue,
#ifdef TF_DLL
					 tmpEvent.m_flFireTime - engine->GetServerTime(),
#else
					  tmpEvent.m_flFireTime - gpGlobals->curtime,
...
const char *variant_t::ToString(void) const
{
	...
	switch(fieldType)
	{
		...
		case FIELD_COLOR32:
		{
			Q_snprintf(szBuf, sizeof(szBuf), "%d %d %d %d", (int)rgbaVal.r, (int)rgbaVal.g, (int)rgbaVal.b,
					   (int)rgbaVal.a);
			return (szBuf);
		}
