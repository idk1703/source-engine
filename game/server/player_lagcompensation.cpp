...
static ConVar sv_lagcompensation_teleport_dist(
	"sv_lagcompensation_teleport_dist", "64", FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT,
...
ConVar sv_maxunlag("sv_maxunlag", "1.0", FCVAR_DEVELOPMENTONLY, "Maximum lag compensation in seconds", true, 0.0f, true,
				   1.0f);
ConVar sv_lagflushbonecache("sv_lagflushbonecache", "1", FCVAR_DEVELOPMENTONLY,
							"Flushes entity bone cache on lag compensation");
ConVar sv_showlagcompensation("sv_showlagcompensation", "0", FCVAR_CHEAT,
...
static void RestorePlayerTo(CBasePlayer *pPlayer, const Vector &vWantedPos)
{
	...
	if(tr.startsolid || tr.allsolid)
	{
		...
		UTIL_TraceEntity(pPlayer, pPlayer->GetLocalOrigin(), vWantedPos, MASK_PLAYERSOLID, pPlayer,
...
private:
	...
	bool m_isCurrentlyDoingCompensation; // Sentinel to prevent calling StartLagCompensation a second time before a
										 // Finish.
};
...
void CLagCompensationManager::BacktrackPlayer(CBasePlayer *pPlayer, float flTargetTime)
{
	   ...
	   (record->m_flSimulationTime < prevRecord->m_flSimulationTime))
	{
		...
		frac =
			(flTargetTime - record->m_flSimulationTime) / (prevRecord->m_flSimulationTime - record->m_flSimulationTime);
	...
	{
		// If the master state changes, all layers will be invalid too, so don't interp (ya know, interp barely ever
		// happens anyway)
		...
		if(currentLayer)
		{
			...
			if((frac > 0.0f) && interpolationAllowed)
			{
				   ...
				   (recordsLayerRecord.m_sequence == prevRecordsLayerRecord.m_sequence))
				{
					...
					{
						// the older record is higher in frame than the newer, it must have wrapped around from 1 back
						// to 0 add one to the newer so it is lerping from .9 to 1.1 instead of .9 to .1, for example.
						float newCycle = Lerp(frac, recordsLayerRecord.m_cycle, prevRecordsLayerRecord.m_cycle + 1);
						currentLayer->m_flCycle =
							newCycle < 1 ? newCycle : newCycle - 1; // and make sure .9 to 1.2 does not end up 1.05
					}
					else
					{
						currentLayer->m_flCycle =
							Lerp(frac, recordsLayerRecord.m_cycle, prevRecordsLayerRecord.m_cycle);
					}
...
{
	VPROF_BUDGET_FLAGS("FinishLagCompensation", VPROF_BUDGETGROUP_OTHER_NETWORKING,
