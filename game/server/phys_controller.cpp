...
BEGIN_SIMPLE_DATADESC(CConstantForceController)
	DEFINE_FIELD(m_controlType, FIELD_INTEGER), DEFINE_FIELD(m_linear, FIELD_VECTOR),
		DEFINE_FIELD(m_angular, FIELD_VECTOR), DEFINE_FIELD(m_linearSave, FIELD_VECTOR),
...
IMotionEvent::simresult_e CConstantForceController::Simulate(IPhysicsMotionController *pController,
...
BEGIN_DATADESC(CPhysForce)
	...
	DEFINE_PHYSPTR(m_pController), DEFINE_KEYFIELD(m_nameAttach, FIELD_STRING, "attach1"),
		DEFINE_KEYFIELD(m_force, FIELD_FLOAT, "force"), DEFINE_KEYFIELD(m_forceTime, FIELD_FLOAT, "forcetime"),
...
void CalculateVelocityOffsetLocal(IPhysicsObject *pPhys, const Vector &forceLocal, const Vector &positionLocal,
								  Vector &outVelLocal, AngularImpulse &outAngular)
{
...
BEGIN_SIMPLE_DATADESC(CMotorController)
	...
	DEFINE_FIELD(m_speed, FIELD_FLOAT), DEFINE_FIELD(m_maxTorque, FIELD_FLOAT),
		DEFINE_KEYFIELD(m_axis, FIELD_VECTOR, "axis"), DEFINE_KEYFIELD(m_inertiaFactor, FIELD_FLOAT, "inertiafactor"),
		DEFINE_FIELD(m_lastSpeed, FIELD_FLOAT), DEFINE_FIELD(m_lastAcceleration, FIELD_FLOAT),
		DEFINE_FIELD(m_lastForce, FIELD_FLOAT), DEFINE_FIELD(m_restistanceDamping, FIELD_FLOAT),

END_DATADESC()

IMotionEvent::simresult_e CMotorController::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject,
...
BEGIN_DATADESC(CPhysMotor)
	...
	DEFINE_KEYFIELD(m_nameAttach, FIELD_STRING, "attach1"), DEFINE_FIELD(m_attachedObject, FIELD_EHANDLE),
		DEFINE_KEYFIELD(m_spinUp, FIELD_FLOAT, "spinup"),
		DEFINE_KEYFIELD(m_additionalAcceleration, FIELD_FLOAT, "addangaccel"),
		DEFINE_FIELD(m_angularAcceleration, FIELD_FLOAT), DEFINE_FIELD(m_lastTime, FIELD_TIME),
		DEFINE_PHYSPTR(m_pHinge), DEFINE_PHYSPTR(m_pController),
...
void CPhysMotor::Activate(void)
{
	...
	if(m_attachedObject == NULL)
	{
		...
		if(pAttach && pAttach->GetMoveType() == MOVETYPE_VPHYSICS)
		{
			...
			m_motor.m_maxTorque =
				inertia * m_motor.m_inertiaFactor * (m_angularAcceleration + m_additionalAcceleration);
...
void CPhysMotor::Think(void)
{
	...
	m_motor.m_speed =
		UTIL_Approach(m_flSpeed, m_motor.m_speed, m_angularAcceleration * (gpGlobals->curtime - m_lastTime));
	m_lastTime = gpGlobals->curtime;
...
private:
	friend CBaseEntity *CreateKeepUpright(const Vector &vecOrigin, const QAngle &vecAngles, CBaseEntity *pOwner,
...
BEGIN_DATADESC(CKeepUpright)
	...
	DEFINE_FIELD(m_worldGoalAxis, FIELD_VECTOR), DEFINE_FIELD(m_localTestAxis, FIELD_VECTOR),
		DEFINE_PHYSPTR(m_pController), DEFINE_KEYFIELD(m_nameAttach, FIELD_STRING, "attach1"),
		DEFINE_FIELD(m_attachedObject, FIELD_EHANDLE), DEFINE_KEYFIELD(m_angularLimit, FIELD_FLOAT, "angularlimit"),
		DEFINE_FIELD(m_bActive, FIELD_BOOLEAN), DEFINE_FIELD(m_bDampAllRotation, FIELD_BOOLEAN),
...
//-----------------------------------------------------------------------------
CBaseEntity *CreateKeepUpright(const Vector &vecOrigin, const QAngle &vecAngles, CBaseEntity *pOwner,
							   float flAngularLimit, bool bActive)
{
...
IMotionEvent::simresult_e CKeepUpright::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject,
												 float deltaTime, Vector &linear, AngularImpulse &angular)
{
	if(!m_bActive)
		return SIM_NOTHING;
	...
	{
		angular = ComputeRotSpeedToAlignAxes(m_localTestAxis, currentLocalTargetAxis, angVel, 0, invDeltaTime,
											 m_angularLimit);
		angular -= angVel;
	...
	angular =
		ComputeRotSpeedToAlignAxes(m_localTestAxis, currentLocalTargetAxis, angVel, 1.0, invDeltaTime, m_angularLimit);
	angular *= invDeltaTime;
...
AngularImpulse ComputeRotSpeedToAlignAxes(const Vector &testAxis, const Vector &alignAxis,
										  const AngularImpulse &currentSpeed, float damping, float scale,
										  float maxSpeed)
{
