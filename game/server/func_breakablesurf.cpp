...
{
	if(pOther && pOther->GetCollisionGroup() != COLLISION_GROUP_BREAKABLE_GLASS)
	{
		Die();
	}
}
...
void CWindowPane::Die(void)
{
	...
	te->ShatterSurface(filter, 0.0, &GetAbsOrigin(), &GetAbsAngles(), &GetAbsVelocity(), &GetAbsOrigin(),
					   WINDOW_PANEL_SIZE, WINDOW_PANEL_SIZE, WINDOW_SMALL_SHARD_SIZE, SHATTERSURFACE_GLASS, 255, 255,
					   255, 255, 255, 255);
...
BEGIN_DATADESC(CBreakableSurface)
	...
	DEFINE_KEYFIELD(m_nSurfaceType, FIELD_INTEGER, "surfacetype"),
		...
		DEFINE_KEYFIELD(m_vURVertex, FIELD_VECTOR, "upperright"), DEFINE_KEYFIELD(m_nQuadError, FIELD_INTEGER, "error"),

		DEFINE_FIELD(m_nNumWide, FIELD_INTEGER), DEFINE_FIELD(m_nNumHigh, FIELD_INTEGER),
		DEFINE_FIELD(m_flPanelWidth, FIELD_FLOAT), DEFINE_FIELD(m_flPanelHeight, FIELD_FLOAT),
		DEFINE_FIELD(m_vNormal, FIELD_VECTOR), DEFINE_FIELD(m_vCorner, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_bIsBroken, FIELD_BOOLEAN), DEFINE_FIELD(m_nNumBrokenPanes, FIELD_INTEGER),
	...
	// DEFINE_FIELD( m_ForceUpdateClientData, CBitVec < MAX_PLAYERS > ),  // No need to save/restore this, it's just a
	// temporary flag field
END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CBreakableSurface, DT_BreakableSurface)
SendPropInt(SENDINFO(m_nNumWide), 8, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nNumHigh), 8, SPROP_UNSIGNED),
	SendPropFloat(SENDINFO(m_flPanelWidth), 0, SPROP_NOSCALE),
	SendPropFloat(SENDINFO(m_flPanelHeight), 0, SPROP_NOSCALE), SendPropVector(SENDINFO(m_vNormal), -1, SPROP_COORD),
	SendPropVector(SENDINFO(m_vCorner), -1, SPROP_COORD), SendPropInt(SENDINFO(m_bIsBroken), 1, SPROP_UNSIGNED),
	...
	else if(info.GetDamageType() & (DMG_SONIC | DMG_BLAST))
	{
		...
		else
		{
			...
			else if(m_nNumBrokenPanes < 0.1 * (m_nNumWide * m_nNumHigh))
			{
				...
				CreateShards(m_vCorner, vAngles, vBlastDir, ptr->endpos, m_nNumWide * m_flPanelWidth,
							 m_nNumHigh * m_flPanelHeight, WINDOW_LARGE_SHARD_SIZE);
			}
			// ---------------------------------------------------------------
				...
				for(int width = 0; width < m_nNumWide; width++)
				{
					...
					for(height = 0; height < m_nNumHigh; height++)
					{
						...
						else if(nHCount > 0)
						{
							...
							CreateShards(vBreakPos, vAngles, vBlastDir, ptr->endpos, m_flPanelWidth,
										 nHCount * m_flPanelHeight, WINDOW_LARGE_SHARD_SIZE);
...
void CBreakableSurface::Die(CBaseEntity *pBreaker, const Vector &vAttackDir)
{
	...
	if(m_nNumWide > MAX_NUM_PANELS)
		m_nNumWide = MAX_NUM_PANELS;
	if(m_nNumHigh > MAX_NUM_PANELS)
...
void CBreakableSurface::InputShatter(inputdata_t &inputdata)
{
	...
	for(int width = nMinX; width < nMaxX; width++)
	{
		for(int height = nMinY; height < nMaxY; height++)
		{
			...
			{
				Vector vBreakPos =
					m_vCorner + (width * vWidthDir * m_flPanelWidth) + (height * vHeightDir * m_flPanelHeight);
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableSurface::Event_Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, float flDamage,
									 int bitsDamageType)
{
	return;
}
...
{
	if(nWidth < 0 || nWidth >= m_nNumWide)
		return true;
	if(nHeight < 0 || nHeight >= m_nNumHigh)
		return true;
...
void CBreakableSurface::DropPane(int nWidth, int nHeight)
{
	...
	if(!IsBroken(nWidth, nHeight))
	{
		...
		Vector vWidthDir, vHeightDir;
		AngleVectors(vAngles, NULL, &vWidthDir, &vHeightDir);
		Vector vBreakPos = m_vCorner + (nWidth * vWidthDir * m_flPanelWidth) + (nHeight * vHeightDir * m_flPanelHeight);

		CreateShards(vBreakPos, vAngles, vec3_origin, vec3_origin, WINDOW_PANEL_SIZE, WINDOW_PANEL_SIZE,
...
void CBreakableSurface::CreateShards(const Vector &vBreakPos, const QAngle &vAngles, const Vector &vForce,
									 const Vector &vForcePos, float flWidth, float flHeight, int nShardSize)
{
	...
	te->ShatterSurface(filter, 0.0, &vAdjustedBreakPos, &vAngles, &vAdjustedForce, &vForcePos, flWidth, flHeight,
					   WINDOW_SMALL_SHARD_SIZE, m_nSurfaceType, front_r, front_g, front_b, back_r, back_g,
					   back_b); // 4);
}
...
void CBreakableSurface::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	if(!m_bIsBroken)
	{
		...
		float damage = CalculateDefaultPhysicsDamage(index, pEvent, 1.0, false, damageType, iszDamageTable,
		...
		else if(damage > 0)
		{
			if(m_spawnflags & SF_BREAKABLESURF_CRACK_DECALS)
			{
				...
				trace_t tr;
				UTIL_TraceLine(damagePos - normal, damagePos + normal, MASK_SOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE,
							   &tr);
