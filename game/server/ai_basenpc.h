...
#define bits_MEMORY_TASK_EXPENSIVE \
	(1 << 3) // NPC has completed a task which is considered costly, so don't do another task this frame
...
#define SF_NPC_WAIT_FOR_SCRIPT \
	(1 << 7) // spawnflag that makes npcs wait to check for attacking until the script is done or they've been attacked
...
Vector VecCheckToss(CBaseEntity *pEdict, Vector vecSpot1, Vector vecSpot2, float flHeightMaxRatio, float flGravityAdj,
					bool bRandomize, Vector *vecMins = NULL, Vector *vecMaxs = NULL);
Vector VecCheckToss(CBaseEntity *pEntity, ITraceFilter *pFilter, Vector vecSpot1, Vector vecSpot2,
					float flHeightMaxRatio, float flGravityAdj, bool bRandomize, Vector *vecMins = NULL,
					Vector *vecMaxs = NULL);
Vector VecCheckThrow(CBaseEntity *pEdict, const Vector &vecSpot1, Vector vecSpot2, float flSpeed,
					 float flGravityAdj = 1.0f, Vector *vecMins = NULL, Vector *vecMaxs = NULL);
...
float ChangeDistance(float flInterval, float flGoalDistance, float flGoalVelocity, float flCurVelocity,
					 float flIdealVelocity, float flAccelRate, float &flNewDistance, float &flNewVelocity);
...
public:
	...
	// This function implements a decision tree for the NPC.  It is responsible for choosing the next behavior
	// (schedule) based on the current conditions and state.
...
public:
	...
	{
		return AI_IdIsLocal(globalScheduleID) ? globalScheduleID
											  : GetClassScheduleIdSpace()->ScheduleGlobalToLocal(globalScheduleID);
	}
	virtual int GetGlobalScheduleId(int localScheduleID)
	{
		return AI_IdIsGlobal(localScheduleID) ? localScheduleID
											  : GetClassScheduleIdSpace()->ScheduleLocalToGlobal(localScheduleID);
	}
...
public:
	...
	void SetActivityAndSequence(Activity NewActivity, int iSequence, Activity translatedActivity,
								Activity weaponActivity);

private:
	void AdvanceToIdealActivity(void);
	void ResolveActivityToSequence(Activity NewActivity, int &iSequence, Activity &translatedActivity,
								   Activity &weaponActivity);
...
protected:
	virtual float GetGoalRepathTolerance(CBaseEntity *pGoalEnt, GoalType_t type, const Vector &curGoal,
...
protected:
	...
	void StartScriptedNPCInteraction(CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction,
	...
	bool InteractionCouldStart(CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction, Vector &vecOrigin,
							   QAngle &angAngles);
...
public:
	...
	virtual int PlaySentence(const char *pszSentence, float delay, float volume, soundlevel_t soundlevel,
							 CBaseEntity *pListener = NULL);
	virtual int PlayScriptedSentence(const char *pszSentence, float delay, float volume, soundlevel_t soundlevel,
...
public:
	...
	static bool FindSpotForNPCInRadius(Vector *pResult, const Vector &vStartPos, CAI_BaseNPC *pNPC, float radius,
	...
	virtual void AddFacingTarget(CBaseEntity *pTarget, const Vector &vecPosition, float flImportance, float flDuration,
								 float flRamp = 0.0);
	...
	bool IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos, float maxUp, float maxDown,
					 float maxDist) const;
	bool ShouldMoveWait();
	...
	virtual bool OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor, float distClear,
								   AIMoveResult_t *pResult);

	virtual bool OnUpcomingPropDoor(AILocalMoveGoal_t *pMoveGoal, CBasePropDoor *pDoor, float distClear,
									AIMoveResult_t *pResult);
...
public:
	...
	enum SCRIPTSTATE
	{
		...
		SCRIPT_CUSTOM_MOVE_TO_MARK, // Moving to the scripted sequence position while playing a custom movement
									// animation.
	};
...
public:
	...
	float ThrowLimit(const Vector &vecStart, const Vector &vecEnd, float fGravity, float fArcSize, const Vector &mins,
					 const Vector &maxs, CBaseEntity *pTarget, Vector *jumpVel, CBaseEntity **pBlocker);
	...
	virtual bool PlayerInSpread(const Vector &sourcePos, const Vector &targetPos, float flSpread,
								float maxDistOffCenter, bool ignoreHatedPlayers = true);
	CBaseEntity *PlayerInRange(const Vector &vecLocation, float flDist);
	bool PointInSpread(CBaseCombatCharacter *pCheckEntity, const Vector &sourcePos, const Vector &targetPos,
	...
	void FireBullets(int cShots, const Vector &vecSrc, const Vector &vecDirShooting, const Vector &vecSpread,
					 float flDistance, int iAmmoType, int iTracerFreq = 4, int firingEntID = -1, int attachmentID = -1,
					 int iDamage = 0, CBaseEntity *pAttacker = NULL, bool bFirstShotAccurate = false);
...
//-----------------------------------------------------------------------------
inline void CAI_BaseNPC::FireBullets(int cShots, const Vector &vecSrc, const Vector &vecDirShooting,
...
#define AI_END_CUSTOM_SCHEDULE_PROVIDER()                                                          \
	...
	int i;                                                                                         \
                                                                                                   \
	CNpc::AccessClassScheduleIdSpaceDirect().Init(pszClassName, BaseClass::GetSchedulingSymbols(), \
	...
	{                                                                                              \
		...
		{                                                                                          \
			CNpc::gm_SchedLoadStatus.fValid = g_AI_SchedulesManager.LoadSchedulesFromBuffer(       \
				pszClassName, schedulesToLoad[i], &AccessClassScheduleIdSpaceDirect());            \
		}                                                                                          \
		else                                                                                       \
			break;                                                                                 \
	}                                                                                              \
...
{
	AI_NamespaceAddInfo_t(const char *pszName, int localId) : pszName(pszName), localId(localId) {}
...
#define IMPLEMENT_CUSTOM_SCHEDULE_PROVIDER(derivedClass)                                                     \
	...
	{                                                                                                        \
		return AI_DoLoadSchedules(derivedClass::BaseClass::LoadSchedules, derivedClass::InitCustomSchedules, \
...
#define ADD_CUSTOM_SCHEDULE_NAMED(derivedClass, schedName, schedEN)                                       \
	if(!derivedClass::AccessClassScheduleIdSpaceDirect().AddSchedule(schedName, schedEN,                  \
...
#define ADD_CUSTOM_TASK_NAMED(derivedClass, taskName, taskEN)                                         \
	if(!derivedClass::AccessClassScheduleIdSpaceDirect().AddTask(taskName, taskEN,                    \
																 derivedClass::gm_pszErrorClassName)) \
		return;
...
#define ADD_CUSTOM_CONDITION_NAMED(derivedClass, condName, condEN)                                         \
	if(!derivedClass::AccessClassScheduleIdSpaceDirect().AddCondition(condName, condEN,                    \
																	  derivedClass::gm_pszErrorClassName)) \
		return;
...
#define INIT_CUSTOM_AI(derivedClass)                                                                        \
	derivedClass::AccessClassScheduleIdSpaceDirect().Init(#derivedClass, BaseClass::GetSchedulingSymbols(), \
														  &BaseClass::AccessClassScheduleIdSpaceDirect());  \
...
#define ADD_CUSTOM_SQUADSLOT_NAMED(derivedClass, squadSlotName, squadSlotEN)                 \
	if(!derivedClass::gm_SquadSlotIdSpace.AddSymbol(squadSlotName, squadSlotEN, "squadslot", \
													derivedClass::gm_pszErrorClassName))     \
		return;
