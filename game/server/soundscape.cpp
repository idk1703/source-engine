...
ConVar soundscape_debug("soundscape_debug", "0", FCVAR_CHEAT,
						"When on, draws lines to all env_soundscape entities. Green lines show the active soundscape, "
						"red lines show soundscapes that aren't in range, and white lines show soundscapes that are in "
						"range, but not the active soundscape.");
...
void CEnvSoundscape::UpdateForPlayer(ss_update_t &update)
{
	...
	if(update.pCurrentSoundscape == this)
	{
		...
		if(m_flRadius > range || m_flRadius == -1)
		{
			...
			update.traceCount++;
			UTIL_TraceLine(target, update.playerPosition, MASK_SOLID_BRUSHONLY | MASK_WATER, update.pPlayer,
	...
	{
		// draw myself
		NDebugOverlay::Box(GetAbsOrigin(), Vector(-10, -10, -10), Vector(10, 10, 10), 255, 0, 255, 64,
		...
		if(update.pPlayer)
		{
			...
			if(audio.ent.Get() != this)
			{
				...
				{
					NDebugOverlay::Line(GetAbsOrigin(), update.pPlayer->WorldSpaceCenter(), 255, 255, 255, true,
										NDEBUG_PERSIST_TILL_NEXT_SERVER);
				}
				else
				{
					NDebugOverlay::Line(GetAbsOrigin(), update.pPlayer->WorldSpaceCenter(), 255, 0, 0, true,
										NDEBUG_PERSIST_TILL_NEXT_SERVER);
				}
			}
			else
			{
				if(InRangeOfPlayer(update.pPlayer))
				{
					NDebugOverlay::Line(GetAbsOrigin(), update.pPlayer->WorldSpaceCenter(), 0, 255, 0, true,
										NDEBUG_PERSIST_TILL_NEXT_SERVER);
				}
				else
				{
					NDebugOverlay::Line(GetAbsOrigin(), update.pPlayer->WorldSpaceCenter(), 255, 170, 0, true,
										NDEBUG_PERSIST_TILL_NEXT_SERVER);
				}
				...
				for(int ii = 0; ii < NUM_AUDIO_LOCAL_SOUNDS; ++ii)
				{
					if(soundbits & (1 << ii))
					{
						...
						NDebugOverlay::Line(GetAbsOrigin(), soundLoc, 0, 32, 255, false,
											NDEBUG_PERSIST_TILL_NEXT_SERVER);
						NDebugOverlay::Cross3D(soundLoc, 16.0f + periodic, 0, 0, 255, false,
											   NDEBUG_PERSIST_TILL_NEXT_SERVER);
					}
				}
			}
		...
		NDebugOverlay::EntityTextAtPosition(GetAbsOrigin(), 0, STRING(m_soundscapeName),
											NDEBUG_PERSIST_TILL_NEXT_SERVER);
	}
}
...
{
	Precache();
	// Because the soundscape has no model, need to make sure it doesn't get culled from the PVS for this reason and
	// therefore
...
void CEnvSoundscapeTriggerable::DelegateEndTouch(CBaseEntity *pEnt)
{
	...
	{
		CEnvSoundscapeTriggerable *pSS =
			dynamic_cast<CEnvSoundscapeTriggerable *>(pPlayer->m_hTriggerSoundscapeList[0].Get());
		if(pSS)
		{
...
BEGIN_DATADESC(CTriggerSoundscape)
	DEFINE_THINKFUNC(PlayerUpdateThink), DEFINE_KEYFIELD(m_SoundscapeName, FIELD_STRING, "soundscape"),
		DEFINE_FIELD(m_hSoundscape, FIELD_EHANDLE), DEFINE_UTLVECTOR(m_spectators, FIELD_EHANDLE),
END_DATADESC()
