...
#endif
{
	...
	if((aiMove0 && !pObj1->IsMoveable()) || (aiMove1 && !pObj0->IsMoveable()) || (aiMove0 && aiMove1))
		return 0;
...
bool FindMaxContact(IPhysicsObject *pObject, float minForce, IPhysicsObject **pOtherObject, Vector *contactPos,
					Vector *pForce)
{
...
static ConVar phys_penetration_error_time("phys_penetration_error_time", "10", 0,
...
int CCollisionEvent::ShouldSolvePenetration(IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0,
											void *pGameData1, float dt)
{
	CallbackContext check(this);
	...
	if(pEntity0 == pEntity1)
	{
		if(pObj0->GetGameFlags() & FVPHYSICS_PART_OF_RAGDOLL)
		{
			DevMsg(2, "Solving ragdoll self penetration! %s (%s) (%d v %d)\n", pObj0->GetName(),
				   pEntity0->GetDebugName(), pObj0->GetGameIndex(), pObj1->GetGameIndex());
	...
	if(eventTime > 3)
	{
		...
		if(!pEntity0->IsPlayer() && !pEntity1->IsPlayer() && !pObj0->GetShadowController() &&
		   !pObj1->GetShadowController())
		{
	...
	if(index >= 0)
	{
		...
		switch(override.alignType)
		{
			...
			case masscenteroverride_t::ALIGN_AXIS:
			{
				...
				massCenterWS += override.axis * (DotProduct(defaultMassCenterWS, override.axis) -
...
CON_COMMAND_F(surfaceprop, "Reports the surface properties at the cursor", FCVAR_CHEAT)
{
	...
	if(tr.DidHit())
	{
		...
		Msg("Hit surface \"%s\" (entity %s, model \"%s\" %s), texture \"%s\"\n",
			physprops->GetPropName(tr.surface.surfaceProps), tr.m_pEnt->GetClassname(), pModelName, modelStuff.Access(),
			tr.surface.name);
...
static void OutputVPhysicsDebugInfo(CBaseEntity *pEntity)
{
	if(pEntity)
	{
		Msg("Entity %s (%s) %s Collision Group %d\n", pEntity->GetClassname(), pEntity->GetDebugName(),
			pEntity->IsNavIgnored() ? "NAV IGNORE" : "", pEntity->GetCollisionGroup());
...
{
	extern bool GetSpringAttachments(CBaseEntity * pEntity, CBaseEntity * pAttach[2],
									 IPhysicsObject * pAttachVPhysics[2]);
	extern bool GetConstraintAttachments(CBaseEntity * pEntity, CBaseEntity * pAttach[2],
										 IPhysicsObject * pAttachVPhysics[2]);
	...
	for(int i = 0; i < constraints.Count(); i++)
	{
		...
		Msg("**********************\n%s connects %s(%s:%d) to %s(%s:%d)\n", constraints[i]->GetClassname(), pName0,
			pModel0, index0, pName1, pModel1, index1);
	...
	if(activeCount)
	{
		...
		for(i = 0; i < ents.Count(); i++)
		{
			...
			Msg("%s (%s): %.3fms (%.3f%%) @ %s\n", ents[i]->GetClassname(), ents[i]->GetDebugName(),
				fraction * totalTime * 1000.0f, fraction * 100.0f, VecToString(ents[i]->GetAbsOrigin()));
		}
...
void PhysFrame(float deltaTime)
{
	...
	if(bProfile)
	{
		...
		{
			Msg("Physics: %3d objects, %4.1fms / AVG: %4.1fms\n", activeCount, simRealTime * 1000,
				g_PhysAverageSimTime * 1000);
		}
...
void CCollisionEvent::Friction(IPhysicsObject *pObject, float energy, int surfaceProps, int surfacePropsHit,
...
void CCollisionEvent::FlushQueuedOperations()
{
	...
	while(loopCount < 20)
	{
		int count = m_triggerEvents.Count() + m_touchEvents.Count() + m_damageEvents.Count() + m_removeObjects.Count() +
...
void CCollisionEvent::FrameUpdate(void)
{
	...
	UpdateFluidEvents();
	UpdateDamageEvents(); // if there was no PSI in physics, we'll still need to do some of these because collisions are
						  // solved in between PSIs
	g_PostSimulationQueue.CallQueued();
	UpdateRemoveObjects();
...
void CCollisionEvent::DispatchStartTouch(CBaseEntity *pEntity0, CBaseEntity *pEntity1, const Vector &point,
										 const Vector &normal)
{
	trace_t trace;
...
bool PhysGetDamageInflictorVelocityStartOfFrame(IPhysicsObject *pInflictor, Vector &velocity,
												AngularImpulse &angVelocity)
{
	return g_Collisions.GetInflictorVelocity(pInflictor, velocity, angVelocity);
}

void CCollisionEvent::AddTouchEvent(CBaseEntity *pEntity0, CBaseEntity *pEntity1, int touchType, const Vector &point,
									const Vector &normal)
{
	if(!pEntity0 || !pEntity1)
		return;
...
void CCollisionEvent::AddDamageEvent(CBaseEntity *pEntity, const CTakeDamageInfo &info,
									 IPhysicsObject *pInflictorPhysics, bool bRestoreVelocity, const Vector &savedVel,
...
// Impulse events
//-----------------------------------------------------------------------------
static void PostSimulation_ImpulseEvent(IPhysicsObject *pObject, const Vector &centerForce,
...
int CCollisionEvent::AddDamageInflictor(IPhysicsObject *pInflictorPhysics, float otherMass, const Vector &savedVel,
...
void PhysicsImpactSound(CBaseEntity *pEntity, IPhysicsObject *pPhysObject, int channel, int surfaceProps,
						int surfacePropsHit, float volume, float impactSpeed)
{
	physicssound::AddImpactSound(g_PhysicsHook.m_impactSounds, pEntity, pEntity->entindex(), channel, pPhysObject,
								 surfaceProps, surfacePropsHit, volume, impactSpeed);
}

void PhysCollisionSound(CBaseEntity *pEntity, IPhysicsObject *pPhysObject, int channel, int surfaceProps,
						int surfacePropsHit, float deltaTime, float speed)
{
...
void PhysCollisionScreenShake(gamevcollisionevent_t *pEvent, int index)
{
	   ...
	   !(pEvent->pObjects[index]->GetGameFlags() & FVPHYSICS_PENETRATING))
	{
		...
		if(pEvent->collisionSpeed > 30 && pEvent->deltaCollisionTime > 0.25f)
		{
			...
			UTIL_ScreenShake(vecPos, amplitude, collision_shake_freq.GetFloat(), collision_shake_time.GetFloat(),
							 amplitude * 60, SHAKE_START);
		}
	}
}
...
#if HL2_EPISODIC
			// this is probably redundant because BaseEntity::VHandleCollision should have already dispatched us
			// elsewhere
...
void PhysFrictionSound(CBaseEntity *pEntity, IPhysicsObject *pObject, const char *pSoundName,
					   HSOUNDSCRIPTHANDLE &handle, float flVolume)
{
	if(!pEntity)
		return;
	...
	if(flVolume > (1.0f / 128.0f))
	{
		...
		else
		{
			...
			CSoundEnvelopeController::GetController().SoundChangeVolume(pFriction->patch, params.volume * flVolume,
																		0.1f);
...
//-----------------------------------------------------------------------------
void PhysCallbackImpulse(IPhysicsObject *pPhysicsObject, const Vector &vecCenterForce,
						 const AngularImpulse &vecCenterTorque)
{
	Assert(physenv->IsInSimulation());
	g_PostSimulationQueue.QueueCall(PostSimulation_ImpulseEvent, pPhysicsObject, RefToVal(vecCenterForce),
...
void PhysCallbackDamage(CBaseEntity *pEntity, const CTakeDamageInfo &info, gamevcollisionevent_t &event, int hurtIndex)
{
	...
	g_Collisions.AddDamageEvent(pEntity, info, event.pObjects[otherIndex], true, event.preVelocity[otherIndex],
...
static ConCommand air_density("air_density", CC_AirDensity, "Changes the density of air for drag computations.",
							  FCVAR_CHEAT);
...
void DebugDrawContactPoints(IPhysicsObject *pPhysics)
{
	...
	while(pSnapshot->IsValid())
	{
		...
		CFmtStr str("%s (%s): %s [%0.2f]", pEntity0->GetClassname(), STRING(pEntity0->GetModelName()),
					pEntity0->GetDebugName(), pSnapshot->GetFrictionCoefficient());
