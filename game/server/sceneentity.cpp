...
static ConVar scene_forcecombined("scene_forcecombined", "0", 0,
...
BEGIN_DATADESC(CSceneManager)
	...
	// DEFINE_FIELD( m_QueuedSceneSounds, CUtlVector < CRestoreSceneSound > ),  // Don't save/restore this, it's created
	// and used by OnRestore only
...
public:
	...
	void BuildSortedSpeakEventSoundsPrefetchList(CChoreoScene *scene, CUtlSymbolTable &table,
	...
	virtual void DispatchStartSpeak(CChoreoScene *scene, CBaseFlex *actor, CChoreoEvent *event,
...
public:
	...
	virtual CBaseEntity *FindNamedEntityClosest(const char *name, CBaseEntity *pActor = NULL,
												bool bBaseFlexOnly = false, bool bUseClear = false,
...
SendPropInt(SENDINFO(m_nSceneStringIndex), MAX_CHOREO_SCENES_STRING_BITS, SPROP_UNSIGNED),
	SendPropBool(SENDINFO(m_bIsPlayingBack)), SendPropBool(SENDINFO(m_bPaused)), SendPropBool(SENDINFO(m_bMultiplayer)),
...
()
	...
	DEFINE_FIELD(m_hWaitingForThisResumeScene, FIELD_EHANDLE), DEFINE_FIELD(m_bWaitingForResumeScene, FIELD_BOOLEAN),

	DEFINE_KEYFIELD(m_iszTarget1, FIELD_STRING, "target1"), DEFINE_KEYFIELD(m_iszTarget2, FIELD_STRING, "target2"),
	DEFINE_KEYFIELD(m_iszTarget3, FIELD_STRING, "target3"), DEFINE_KEYFIELD(m_iszTarget4, FIELD_STRING, "target4"),
	DEFINE_KEYFIELD(m_iszTarget5, FIELD_STRING, "target5"), DEFINE_KEYFIELD(m_iszTarget6, FIELD_STRING, "target6"),
	DEFINE_KEYFIELD(m_iszTarget7, FIELD_STRING, "target7"), DEFINE_KEYFIELD(m_iszTarget8, FIELD_STRING, "target8"),

	DEFINE_KEYFIELD(m_BusyActor, FIELD_INTEGER, "busyactor"),

	DEFINE_FIELD(m_hTarget1, FIELD_EHANDLE), DEFINE_FIELD(m_hTarget2, FIELD_EHANDLE),
	DEFINE_FIELD(m_hTarget3, FIELD_EHANDLE), DEFINE_FIELD(m_hTarget4, FIELD_EHANDLE),
	DEFINE_FIELD(m_hTarget5, FIELD_EHANDLE), DEFINE_FIELD(m_hTarget6, FIELD_EHANDLE),
	DEFINE_FIELD(m_hTarget7, FIELD_EHANDLE), DEFINE_FIELD(m_hTarget8, FIELD_EHANDLE),

	DEFINE_FIELD(m_bIsPlayingBack, FIELD_BOOLEAN), DEFINE_FIELD(m_bPaused, FIELD_BOOLEAN),
	...
	DEFINE_FIELD(m_bCancelAtNextInterrupt, FIELD_BOOLEAN), DEFINE_FIELD(m_fPitch, FIELD_FLOAT),
	DEFINE_FIELD(m_bAutomated, FIELD_BOOLEAN), DEFINE_FIELD(m_nAutomatedAction, FIELD_INTEGER),
	DEFINE_FIELD(m_flAutomationDelay, FIELD_FLOAT),
	DEFINE_FIELD(m_flAutomationTime, FIELD_FLOAT), // relative, not absolute time

	DEFINE_FIELD(m_bPausedViaInput, FIELD_BOOLEAN), DEFINE_FIELD(m_bWaitingForActor, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bWaitingForInterrupt, FIELD_BOOLEAN), DEFINE_FIELD(m_bInterruptedActorsScenes, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bBreakOnNonIdle, FIELD_BOOLEAN),

	DEFINE_UTLVECTOR(m_hActorList, FIELD_EHANDLE), DEFINE_UTLVECTOR(m_hRemoveActorList, FIELD_EHANDLE),
	...
	DEFINE_FIELD(m_nInterruptCount, FIELD_INTEGER), DEFINE_FIELD(m_bInterrupted, FIELD_BOOLEAN),
	DEFINE_FIELD(m_hInterruptScene, FIELD_EHANDLE), DEFINE_FIELD(m_bCompletedEarly, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bInterruptSceneFinished, FIELD_BOOLEAN),

	DEFINE_FIELD(m_bGenerated, FIELD_BOOLEAN), DEFINE_FIELD(m_iszSoundName, FIELD_STRING),
	DEFINE_FIELD(m_hActor, FIELD_EHANDLE), DEFINE_FIELD(m_hActivator, FIELD_EHANDLE),
	...
	// Outputs
	DEFINE_OUTPUT(m_OnStart, "OnStart"), DEFINE_OUTPUT(m_OnCompletion, "OnCompletion"),
	DEFINE_OUTPUT(m_OnCanceled, "OnCanceled"), DEFINE_OUTPUT(m_OnTrigger1, "OnTrigger1"),
	DEFINE_OUTPUT(m_OnTrigger2, "OnTrigger2"), DEFINE_OUTPUT(m_OnTrigger3, "OnTrigger3"),
	DEFINE_OUTPUT(m_OnTrigger4, "OnTrigger4"), DEFINE_OUTPUT(m_OnTrigger5, "OnTrigger5"),
	DEFINE_OUTPUT(m_OnTrigger6, "OnTrigger6"), DEFINE_OUTPUT(m_OnTrigger7, "OnTrigger7"),
	DEFINE_OUTPUT(m_OnTrigger8, "OnTrigger8"), DEFINE_OUTPUT(m_OnTrigger9, "OnTrigger9"),
	DEFINE_OUTPUT(m_OnTrigger10, "OnTrigger10"), DEFINE_OUTPUT(m_OnTrigger11, "OnTrigger11"),
	DEFINE_OUTPUT(m_OnTrigger12, "OnTrigger12"), DEFINE_OUTPUT(m_OnTrigger13, "OnTrigger13"),
	DEFINE_OUTPUT(m_OnTrigger14, "OnTrigger14"), DEFINE_OUTPUT(m_OnTrigger15, "OnTrigger15"),
...
void CSceneEntity::OnRestore()
{
	...
	if(!m_pScene)
	{
		...
		{
			m_nSceneStringIndex =
				g_pStringTableClientSideChoreoScenes->AddString(CBaseEntity::IsServer(), STRING(m_iszSceneFile));
		}
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartSpeak(CChoreoScene *scene, CBaseFlex *actor, CChoreoEvent *event,
									  soundlevel_t iSoundlevel)
{
	// Emit sound
	if(actor)
	{
		...
		for(int i = 0; i < c; ++i)
		{
			...
			Q_strncpy(speechListSounds[speechListIndex].sceneName, (scene) ? scene->GetFilename() : "",
		...
		if(event->GetCloseCaptionType() == CChoreoEvent::CC_MASTER)
		{
			...
			if(validtoken)
			{
				...
				if(!event->IsSuppressingCaptionAttenuation() && (filter.GetRecipientCount() > 0))
				{
					int c = filter.GetRecipientCount();
					for(int i = c - 1; i >= 0; --i)
					{
						...
						if(AttenuateCaption(lowercase, playerOrigin, es.m_UtlVecSoundOrigin))
						{
							...
							if(!player->GetViewEntity() ||
							   AttenuateCaption(lowercase, player->GetViewEntity()->GetAbsOrigin(),
			...
			if(pActor)
			{
				...
				if(hl2_episodic.GetBool())
				{
					...
					if(pActor->IsInAVehicle())
					{
						...
						if(pAlyx != NULL && (pAlyx->GetPassengerState() == PASSENGER_STATE_ENTERING ||
...
//-----------------------------------------------------------------------------
void CSceneEntity::BuildSortedSpeakEventSoundsPrefetchList(CChoreoScene *scene, CUtlSymbolTable &table,
														   CUtlRBTree<SpeakEventSound_t> &soundnames, float timeOffset)
{
	Assert(scene);
	...
	for(int i = 0; i < scene->GetNumEvents(); i++)
	{
		...
		switch(event->GetType())
		{
			...
			case CChoreoEvent::SPEAK:
			{
				...
				{
					// UNDONE:  Probably need some other solution in multiplayer... (not sure how to "prefetch" on
				certain players
...
void CSceneEntity::QueueResumePlayback(void)
{
	...
	if(m_iszResumeSceneFile != NULL_STRING)
	{
		...
		{
			bStartedScene = InstancedScriptedScene(NULL, STRING(m_iszResumeSceneFile), &m_hWaitingForThisResumeScene, 0,
												   false) != 0;
		}
			...
			if(pActor)
			{
				...
				if(pBaseActor)
				{
					...
					if(result)
					{
						...
						bStartedScene =
							InstancedScriptedScene(NULL, szResponse, &m_hWaitingForThisResumeScene, 0, false) != 0;
					}
				}
			}
		}
	...
	switch(event->GetType())
	{
		...
		case CChoreoEvent::MOVETO:
		{
			...
			if(pActor && !IsMultiplayer())
			{
				...
				{
					pActor2 =
						FindNamedEntityClosest(event->GetParameters(), pActor, false, true, event->GetParameters3());
				}
				else
				{
	...
	if(m_bMultiplayer)
	{
		if(m_pScene &&
		   (m_pScene->HasEventsOfType(CChoreoEvent::FLEXANIMATION) ||
			m_pScene->HasEventsOfType(CChoreoEvent::EXPRESSION) || m_pScene->HasEventsOfType(CChoreoEvent::GESTURE) ||
...
CBaseFlex *CSceneEntity::FindNamedActor(int index)
{
	...
	if(!m_hActorList.IsValidIndex(index))
	{
		DevWarning("Scene %s has %d actors, but scene entity only has %d actors\n", m_pScene->GetFilename(),
				   m_pScene->GetNumActors(), m_hActorList.Size());
		return NULL;
	}
...
public:
	...
	bool ShouldFindEntity(CBaseEntity *pEntity)
	{
		...
		AI_TraceHull(vecOrigin, vecOrigin, m_hActor->WorldAlignMins(), m_hActor->WorldAlignMaxs(), MASK_SOLID, m_hActor,
...
// Output : CBaseFlex
//-----------------------------------------------------------------------------
CBaseEntity *CSceneEntity::FindNamedEntityClosest(const char *name, CBaseEntity *pActor, bool bBaseFlexOnly,
												  bool bUseClear, const char *pszSecondary)
{
	CBaseEntity *entity = NULL;
...
//-----------------------------------------------------------------------------
// Purpose: An interrupt scene has finished or been canceled, we can resume once we pick up this state in
// CheckInterruptCompletion Input  : *interruptor -
//-----------------------------------------------------------------------------
...
public:
	...
	{
		if(PassThrough(actor))
			BaseClass::DispatchEndMoveTo(scene, actor, event);
	};
	...
	{
		if(PassThrough(actor))
			BaseClass::DispatchEndFace(scene, actor, event);
	};
...
BEGIN_DATADESC(CInstancedSceneEntity)
	...
	DEFINE_FIELD(m_hOwner, FIELD_EHANDLE), DEFINE_FIELD(m_bHadOwner, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flPostSpeakDelay, FIELD_FLOAT), DEFINE_FIELD(m_flPreDelay, FIELD_FLOAT),
		DEFINE_AUTO_ARRAY(m_szInstanceFilename, FIELD_CHARACTER), DEFINE_FIELD(m_bIsBackground, FIELD_BOOLEAN),
...
// Output :
//-----------------------------------------------------------------------------
float InstancedScriptedScene(CBaseFlex *pActor, const char *pszScene, EHANDLE *phSceneEnt, float flPostDelay,
							 bool bIsBackground, AI_Response *response, bool bMultiplayer,
							 IRecipientFilter *filter /* = NULL */)
{
	VPROF("InstancedScriptedScene");

	CInstancedSceneEntity *pScene =
		(CInstancedSceneEntity *)CBaseEntity::CreateNoSpawn("instanced_scripted_scene", vec3_origin, vec3_angle);

	// This code expands any $gender tags into male or female tags based on the gender of the actor (based on his/her
	// .mdl)
	if(pActor)
	{
...
float InstancedAutoGeneratedSoundScene(CBaseFlex *pActor, char const *soundname, EHANDLE *phSceneEnt /*= NULL*/)
{
	...
	Q_strncpy(pScene->m_szInstanceFilename, UTIL_VarArgs("AutoGenerated(%s)", soundname),
...
void PrecacheInstancedScene(char const *pszScene)
{
	...
	if(!scenefilecache->GetSceneCachedData(pszScene, &sceneData))
	{
		...
		if(developer.GetInt() && (IsX360() && (g_pFullFileSystem->GetDVDMode() != DVDMODE_STRICT) &&
		...
		else
		{
			...
			if(pInstancedScene && pInstancedScene->m_hOwner)
			{
				if(pInstancedScene->m_hOwner == pActor)
				{
					...
					LocalScene_Printf("%s : removed for '%s'\n", STRING(pInstancedScene->m_iszSceneFile),
									  pActor ? pActor->GetDebugName() : "NULL");
										  ...
										  const char *pszThisSceneOnly)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		if(bInstancedOnly && (dynamic_cast<CInstancedSceneEntity *>(pScene) == NULL))
		{
			continue;
		}
...
//			soundtime -
//-----------------------------------------------------------------------------
void CSceneManager::QueueRestoredSound(CBaseFlex *actor, char const *soundname, soundlevel_t soundlevel,
									   float time_in_past)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void RemoveActorFromScriptedScenes(CBaseFlex *pActor, bool instancedscenesonly, bool nonidlescenesonly,
								   const char *pszThisSceneOnly)
{
...
BEGIN_DATADESC(CSceneListManager)
	DEFINE_UTLVECTOR(m_hListManagers, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_hScenes[0], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[1], FIELD_EHANDLE),
		DEFINE_FIELD(m_hScenes[2], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[3], FIELD_EHANDLE),
		DEFINE_FIELD(m_hScenes[4], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[5], FIELD_EHANDLE),
		DEFINE_FIELD(m_hScenes[6], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[7], FIELD_EHANDLE),
		DEFINE_FIELD(m_hScenes[8], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[9], FIELD_EHANDLE),
		DEFINE_FIELD(m_hScenes[10], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[11], FIELD_EHANDLE),
		DEFINE_FIELD(m_hScenes[12], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[13], FIELD_EHANDLE),
		DEFINE_FIELD(m_hScenes[14], FIELD_EHANDLE), DEFINE_FIELD(m_hScenes[15], FIELD_EHANDLE),
			...
			if(m_iszScenes[i] != NULL_STRING)
			{
				...
				if(m_hScenes[i])
				{
					...
					else
					{
						...
						{
							Warning("%s(%s) found an entity that wasn't a logic_choreographed_scene or "
									"logic_scene_list_manager in slot %d, named %s\n",
				...
				{
					Warning("%s(%s) could not find scene %d, named %s\n", GetDebugName(), GetClassname(), i,
							STRING(m_iszScenes[i]));
				}
			}
		}
	}
...
static ConCommand ListRecentNPCSpeechCmd("listRecentNPCSpeech", ListRecentNPCSpeech,
