...
ConVar sv_maxusrcmdprocessticks_warning("sv_maxusrcmdprocessticks_warning", "-1", FCVAR_NONE,
										"Print a warning when user commands get dropped due to insufficient usrcmd "
										"ticks allocated, number of seconds to throttle, negative disabled");
static ConVar sv_maxusrcmdprocessticks_holdaim(
	"sv_maxusrcmdprocessticks_holdaim", "1", FCVAR_CHEAT,
	"Hold client aim for multiple server sim ticks when client-issued usrcmd contains multiple actions (0: off; 1: "
	"hold this server tick; 2+: hold multiple ticks)");
...
void CPlayerMove::StartCommand(CBasePlayer *player, CUserCmd *cmd)
{
	...
	for(i = 0; i < cmd->entitygroundcontact.Count(); i++)
	{
		...
		if(pEntity)
		{
			...
			if(pAnimating)
			{
				pAnimating->SetIKGroundContactInfo(cmd->entitygroundcontact[i].minheight,
