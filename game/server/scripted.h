...
#define SF_SCRIPT_ALLOW_DEATH \
	4096 // the actor using this scripted sequence may die without interrupting the scene (used for scripted deaths)
...
private:
	...
	string_t m_iszPreIdle;	// String index for idle animation to play before playing the action anim (only played while
	...
	bool m_bDelayed; // This moderately hacky hack ensures that we don't calls to DelayStart(true) or DelayStart(false)
					 // twice in succession. This is necessary because we didn't want to remove the call to
					 // DelayStart(true) from StartScript, even though DelayStart(true) is called from TASK_PRE_SCRIPT.
	...
	bool m_bWaitForBeginSequence; // Set to true when we are told to MoveToPosition. Holds the actor in the pre-action
								  // idle until BeginSequence is called.
