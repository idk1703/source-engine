...
BEGIN_DATADESC(CAI_ScriptConditions)
	...
	DEFINE_THINKFUNC(EvaluationThink),
		...
		DEFINE_OUTPUT(m_OnConditionsTimeout, "OnConditionsTimeout"), DEFINE_OUTPUT(m_NoValidActors, "NoValidActors"),
		...
		DEFINE_FIELD(m_hTarget, FIELD_EHANDLE), DEFINE_KEYFIELD(m_Actor, FIELD_STRING, "Actor"),

		DEFINE_KEYFIELD(m_flRequiredTime, FIELD_FLOAT, "RequiredTime"),

#ifndef HL2_EPISODIC
		DEFINE_FIELD(m_hActor, FIELD_EHANDLE), DEFINE_EMBEDDED(m_Timer), DEFINE_EMBEDDED(m_Timeout),
#endif
		...
		DEFINE_UTLVECTOR(m_ElementList, FIELD_EMBEDDED), DEFINE_FIELD(m_bLeaveAsleep, FIELD_BOOLEAN),
...
#ifdef HL2_EPISODIC
		// This is the code we want to have written for HL2, but HL2 shipped without the QuerySeeEntity() call. This
		// #ifdef really wants to be something like #ifndef HL2_RETAIL, since this change does want to be in any
		// products that are built henceforth. (sjb)
...
{
	return (!args.pTarget || m_PlayerTargetProxTester.Check(args.pPlayer, args.pTarget));
}
...
{
	return (!args.pTarget || !args.pActor || m_ActorTargetProxTester.Check(args.pActor, args.pTarget));
}
...
{
	return (!args.pActor || IsInFOV(args.pPlayer, args.pActor, m_flPlayerActorFOV, m_bPlayerActorFOVTrueCone));
}
...
void CAI_ScriptConditions::EvaluationThink()
{
	...
	for(int i = 0; i < m_ElementList.Count();)
	{
		...
		AssertMsg(!m_fDisabled,
				  ("Violated invariant between CAI_ScriptConditions disabled state and think func setting"));
