...
public:
	...
	unsigned int operator()(const NavVisPair_t &item) const
	{
		...
		int64 key[2] = {(int64)(item.pAreas[0] + item.pAreas[1]->GetID()),
						(int64)(item.pAreas[1] + item.pAreas[0]->GetID())};
		return Hash16(key);
#else
...
public:
	...
	/**
	 * Return true if nav mesh can be trusted for all climbing/jumping decisions because game environment is fairly
	 * simple. Authoritative meshes mean path followers can skip CPU intensive realtime scanning of unpredictable
	 * geometry.
	 */
	...
	const CUtlVector<Place> *GetPlacesFromNavFile(
		bool *hasUnnamedPlaces); // Reads the used place names from the nav file (can be used to selectively precache
	...
	} // store custom mesh data for derived classes that needs to be loaded before areas are read in
	virtual void LoadCustomDataPreArea(CUtlBuffer &fileBuffer, unsigned int subVersion) {
	} // load custom mesh data for derived classes that needs to be loaded before areas are read in
	...
	virtual void OnRoundRestartPreEntity(
		void); // invoked when a game round restarts, but before entities are deleted and recreated
	...
	virtual void OnEditCreateNotify(
		CNavArea *newArea); // invoked when given area has just been added to the mesh in edit mode
	virtual void OnEditDestroyNotify(
		CNavArea *deadArea); // invoked when given area has just been deleted from the mesh in edit mode
	virtual void OnEditDestroyNotify(
		CNavLadder *deadLadder); // invoked when given ladder has just been deleted from the mesh in edit mode
	...
	CNavArea *GetNavArea(const Vector &pos, float beneathLimt = 120.0f)
		const; // given a position, return the nav area that IsOverlapping and is *immediately* beneath it
	...
	CNavArea *GetNearestNavArea(const Vector &pos, bool anyZ = false, float maxDist = 10000.0f, bool checkLOS = false,
								bool checkGround = true, int team = TEAM_ANY) const;
	CNavArea *GetNearestNavArea(CBaseEntity *pEntity, int nGetNavAreaFlags = GETNAVAREA_CHECK_GROUND,
	...
	Place PartialNameToPlace(const char *name) const; // given the first part of a place name, return a place ID or zero
													  // if no place is defined, or the partial match is ambiguous
	...
	int PlaceNameAutocomplete(
		...
		char commands[COMMAND_COMPLETION_MAXITEMS]
					 [COMMAND_COMPLETION_ITEM_LENGTH]); // Given a partial place name, fill in possible place names for
														// ConCommand autocomplete

	bool GetGroundHeight(const Vector &pos, float *height, Vector *normal = NULL)
		const; // get the Z coordinate of the topmost ground level below the given point
	bool GetSimpleGroundHeight(const Vector &pos, float *height, Vector *normal = NULL)
		const; // get the Z coordinate of the ground level directly below the given point
...
#define INCREMENTAL_GENERATION true
	...
	void BeginAnalysis(
		bool quitWhenFinished = false); // re-analyze an existing Mesh.  Determine Hiding Spots, Encounter Spots, etc.
	...
	void AddWalkableSeed(
		const Vector &pos,
		const Vector &normal);			 // add given walkable position to list of seed positions for map sampling
	...
	virtual void CommandNavFloodSelect(
		const CCommand &args);				// select current area and all connected areas, recursively
	...
	void CommandNavToggleSelecting(
		bool playSound = true);			   // start/stop continuously selecting areas into the selected set
	...
	void CommandNavToggleDeselecting(
		bool playSound = true);					// start/stop continuously de-selecting areas from the selected set
	...
	void CommandNavDisconnectOutgoingOneWays(
		void);							  // disconnect all outgoing one-way connects from each area in the selected set
	...
	void CommandNavSubdivide(
		const CCommand &args); // subdivide each nav area in X and Y to create 4 new areas - limit min size
	...
	void CreateLadder(const Vector &top, const Vector &bottom, float width, const Vector2D &ladderDir,
	...
	Vector SnapToGrid(const Vector &in, bool snapX = true, bool snapY = true, bool forceGrid = false)
		const; // snap given vector's X & Y coordinates to generation grid boundary
	...
	void CollectAreasOverlappingExtent(const Extent &extent, CUtlVector<NavAreaType *> *outVector)
	{
		...
		for(int x = startX; x <= endX; ++x)
		{
			for(int y = startY; y <= endY; ++y)
			{
				...
				{
					ExecuteNTimes(
						10, Warning("** Walked off of the CNavMesh::m_grid in CollectAreasOverlappingExtent()\n"));
					return;
				}
	...
	bool FindNavAreaOrLadderAlongRay(const Vector &start, const Vector &end, CNavArea **area, CNavLadder **ladder,
									 CNavArea *ignore = NULL);

	void PostProcessCliffAreas();
	void SimplifySelectedAreas(
		void); // Simplifies the selected set by reducing to 1x1 areas and re-merging them up with loosened tolerances
...
protected:
	...
	bool FindActiveNavArea(void); // Finds the area or ladder the local player is currently pointing at.  Returns true
								  // if a surface was hit by the traceline.
...
private:
	...
	bool FindLadderCorners(
		Vector *c1, Vector *c2,
		Vector *c3); // computes the other corners of a ladder given m_ladderAnchor, m_editCursorPos, and m_ladderNormal
	...
	// Auto-generation
	//
	bool UpdateGeneration(float maxTime = 0.25f); // process the auto-generation for 'maxTime' seconds. return false if
	...
	CNavNode *AddNode(const Vector &destPos, const Vector &destNormal, NavDirType dir, CNavNode *source,
					  bool isOnDisplacement, float obstacleHeight, float flObstacleStartDist,
					  float flObstacleEndDist); // add a nav node and connect it, update current node
	...
	bool TestArea(
		CNavNode *node, int width,
		int height); // check if an area of size (width, height) can fit, starting from node as upper left corner
	int BuildArea(CNavNode *node, int width,
				  int height); // create a CNavArea of size (width, height) starting fom node at upper left corner
	...
	void HandleObstacleTopAreas(
		void); // Handles fixing/generating areas on top of slim obstacles such as fences and railings
	...
	CUtlVector<int> m_storedSelectedSet; // "Stored" selected set, so we can do some editing and then restore the old
										 // selected set.  Done by ID, so we don't have to worry about split/delete/etc.

	void BeginVisibilityComputations(void);
	void EndVisibilityComputations(void);

	void TestAllAreasForBlockedStatus(
		void); // Used to update blocked areas after a round restart. Need to delay so the map logic has all fired.
