...
//--------------------------------------------------------------------------------------------------------
ConVar NavGUIRebuild("nav_gui_rebuild", "0", FCVAR_CHEAT,
					 "Rebuilds the nav ui windows from scratch every time they're opened");
...
CNavUIBasePanel::CNavUIBasePanel() : vgui::Frame(NULL, "NavUI")
{
	...
	if(menuButton)
	{
		...
		menu->AddMenuItem("Flood Select (fog)", "Flood Select (Fog)",
						  new KeyValues("Command", "command", "FloodSelect fog"), this);
		menuButton->SetMenu(menu);
		menuButton->SetOpenDirection(Menu::DOWN);
	}
}
