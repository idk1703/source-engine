...
public:
	...
	void GetSpringObjectConnections(string_t nameStart, string_t nameEnd, IPhysicsObject **pStart,
									IPhysicsObject **pEnd);
	void NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
...
BEGIN_DATADESC(CPhysicsSpring)
	...
	DEFINE_PHYSPTR(m_pSpring),
		...
		DEFINE_KEYFIELD(m_tempConstant, FIELD_FLOAT, "constant"), DEFINE_KEYFIELD(m_tempLength, FIELD_FLOAT, "length"),
		...
		DEFINE_FIELD(m_start, FIELD_POSITION_VECTOR), DEFINE_KEYFIELD(m_end, FIELD_POSITION_VECTOR, "springaxis"),
...
void CPhysicsSpring::GetSpringObjectConnections(string_t nameStart, string_t nameEnd, IPhysicsObject **pStart,
												IPhysicsObject **pEnd)
{
...
void CPhysicsSpring::Activate(void)
{
	...
	if(!m_pSpring)
	{
		...
		{
			DevMsg("ERROR: Can't init spring %s from \"%s\" to \"%s\"\n", GetDebugName(), STRING(m_nameAttachStart),
				   STRING(m_nameAttachEnd));
			UTIL_Remove(this);
			return;
		}
...
void CPhysicsSpring::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
									   const notify_system_event_params_t &params)
{
	// don't recurse
	...
	PhysTeleportConstrainedEntity(pNotify, m_pSpring->GetStartObject(), m_pSpring->GetEndObject(),
...
BEGIN_DATADESC(CPhysBox)
	...
	DEFINE_FIELD(m_hCarryingPlayer, FIELD_EHANDLE),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnDamaged, "OnDamaged"), DEFINE_OUTPUT(m_OnAwakened, "OnAwakened"),
		DEFINE_OUTPUT(m_OnMotionEnabled, "OnMotionEnabled"), DEFINE_OUTPUT(m_OnPhysGunPickup, "OnPhysGunPickup"),
		DEFINE_OUTPUT(m_OnPhysGunPunt, "OnPhysGunPunt"), DEFINE_OUTPUT(m_OnPhysGunOnlyPickup, "OnPhysGunOnlyPickup"),
...
BEGIN_DATADESC(CPhysExplosion)
	...
	DEFINE_KEYFIELD(m_damage, FIELD_FLOAT, "magnitude"), DEFINE_KEYFIELD(m_radius, FIELD_FLOAT, "radius"),
...
void CPhysExplosion::Explode(CBaseEntity *pActivator, CBaseEntity *pCaller)
{
	...
	while((pEntity = FindEntity(pEntity, pActivator, pCaller)) != NULL)
	{
		...
		if(pEntity->m_takedamage != DAMAGE_NO &&
		   (pEntity->GetMoveType() == MOVETYPE_VPHYSICS || (pEntity->VPhysicsGetObject() /*&& !pEntity->IsPlayer()*/)))
		{
			...
			if(m_radius == 0 || flDist <= m_radius)
			{
				if(HasSpawnFlags(SF_PHYSEXPLOSION_TEST_LOS))
				{
					...
					trace_t tr;
					UTIL_TraceLine(vecStartPos, pEntity->BodyTarget(vecStartPos, false), MASK_SOLID_BRUSHONLY, this,
...
BEGIN_DATADESC(CPhysImpact)
	...
	DEFINE_KEYFIELD(m_damage, FIELD_FLOAT, "magnitude"), DEFINE_KEYFIELD(m_distance, FIELD_FLOAT, "distance"),
...
void CPhysConvert::InputConvertTarget(inputdata_t &inputdata)
{
	...
	int count = 0;
	while((pEntity = gEntList.FindEntityByName(pEntity, m_target, NULL, inputdata.pActivator, inputdata.pCaller)) !=
		  NULL)
	{
...
BEGIN_DATADESC(CPhysMagnet)
	...
	DEFINE_OUTPUT(m_OnMagnetAttach, "OnAttach"), DEFINE_OUTPUT(m_OnMagnetDetach, "OnDetach"),
		...
		DEFINE_FIELD(m_bActive, FIELD_BOOLEAN), DEFINE_FIELD(m_bHasHitSomething, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flTotalMass, FIELD_FLOAT), DEFINE_FIELD(m_flRadius, FIELD_FLOAT),
		DEFINE_FIELD(m_flNextSuckTime, FIELD_FLOAT),

		// Inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle), DEFINE_INPUTFUNC(FIELD_VOID, "TurnOn", InputTurnOn),
...
//-----------------------------------------------------------------------------
void SendProxy_MagnetAttachedObjectList(const void *pStruct, const void *pData, DVariant *pOut, int iElement,
										int objectID)
{
...
void CPhysMagnet::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	...
	if(pPhysics && pOther->GetMoveType() == MOVETYPE_VPHYSICS && pPhysics->IsMoveable())
	{
		...
		if(HasSpawnFlags(SF_MAGNET_ALLOWROTATION))
		{
			...
			// newEntityOnMagnet.pConstraint = physenv->CreateBallsocketConstraint( pMagnetPhysObject, pPhysics,
			// m_pConstraintGroup, ballsocket );
			newEntityOnMagnet.pConstraint =
				physenv->CreateBallsocketConstraint(pMagnetPhysObject, pPhysics, NULL, ballsocket);
		}
		else
		{
			...
			// newEntityOnMagnet.pConstraint = physenv->CreateFixedConstraint( pMagnetPhysObject, pPhysics,
			// m_pConstraintGroup, fixed );
...
BEGIN_DATADESC(CPointPush)
	...
	DEFINE_THINKFUNC(PushThink),
		...
		DEFINE_KEYFIELD(m_bEnabled, FIELD_BOOLEAN, "enabled"), DEFINE_KEYFIELD(m_flMagnitude, FIELD_FLOAT, "magnitude"),
...
void CPointPush::PushEntity(CBaseEntity *pTarget)
{
	...
	float flFalloff =
		(HasSpawnFlags(SF_PUSH_NO_FALLOFF)) ? 1.0f : RemapValClamped(dist, m_flRadius, m_flRadius * 0.25f, 0.0f, 1.0f);
	...
	switch(pTarget->GetMoveType())
	{
		...
		case MOVETYPE_VPHYSICS:
		{
			...
			if(pPhys)
			{
				...
				pPhys->ApplyForceCenter(m_flMagnitude * flFalloff * 100.0f * vecPushDir * pPhys->GetMass() *
										gpGlobals->frametime);
				return;
			}
		}
...
void CPointPush::PushThink(void)
{
	...
	for(int i = 0; i < numEnts; i++)
	{
		...
		// Must be moveable
		if(pEnts[i]->GetMoveType() != MOVETYPE_VPHYSICS && pEnts[i]->GetMoveType() != MOVETYPE_WALK &&
		...
		if(HasSpawnFlags(SF_PUSH_TEST_LOS))
		{
			...
			trace_t tr;
			UTIL_TraceLine(vecStartPos, pEnts[i]->BodyTarget(vecStartPos, false), MASK_SOLID_BRUSHONLY, this,
						   COLLISION_GROUP_NONE, &tr);
