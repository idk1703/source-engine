...
void CAI_Node::AddLink(CAI_Link *newLink)
{
	...
	AssertMsg(newLink->m_iDestID == m_iID || newLink->m_iSrcID == m_iID,
			  "Added link to node that doesn't reference the node");
#endif
...
float GetFloorZ(const Vector &origin, float fMaxDrop)
{
	...
	//
	trace_t tr;
	AI_TraceLine(origin, origin - Vector(0, 0, fMaxDrop), MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &tr);

	// This trace is ONLY used if we hit an entity flagged with FL_WORLDBRUSH
	trace_t trEnt;
	AI_TraceLine(origin, origin - Vector(0, 0, fMaxDrop), MASK_NPCSOLID, NULL, COLLISION_GROUP_NONE, &trEnt);
...
// Output :
//-----------------------------------------------------------------------------
CAI_Node::CAI_Node(int id, const Vector &origin, float yaw) : m_Links(0, 4)
{
	m_vOrigin = origin;
	m_iID = id;
