...
int CAI_Network::ListNodesInBox(CNodeList &list, int maxListCount, const Vector &mins, const Vector &maxs,
								INodeListFilter *pFilter)
{
	...
	for(int node = 0; node < m_iNumNodes; node++)
	{
		...
		// in box?
		if(origin.x < mins.x || origin.x > maxs.x || origin.y < mins.y || origin.y > maxs.y || origin.z < mins.z ||
		   origin.z > maxs.z)
			continue;
...
int CAI_Network::NearestNodeToPoint(CAI_BaseNPC *pNPC, const Vector &vecOrigin, bool bCheckVisibility,
									INearestNodeFilter *pFilter)
{
	...
	if(cachedNode != NO_NODE)
	{
		if(bCheckVisibility)
		{
			...
			Vector vTestLoc = (pNPC) ? m_pAInode[cachedNode]->GetPosition(pNPC->GetHullType()) + pNPC->GetViewOffset()
	...
	for(; list.Count(); list.RemoveAtHead())
	{
		...
		if(bCheckVisibility)
		{
			...
			Vector vTestLoc = (pNPC) ? m_pAInode[smallest]->GetPosition(pNPC->GetHullType()) + pNPC->GetNodeViewOffset()
...
CAI_Node *CAI_Network::AddNode(const Vector &origin, float yaw)
{
	...
	m_pNodeTree->CreateHandle((IHandleEntity *)m_iNumNodes, PARTITION_NODE, origin,
							  origin); // ignore result for now as we'll never move or remove
#endif
