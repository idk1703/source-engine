...
#define DrawLine(from, to, duration, red, green, blue) \
	NDebugOverlay::Line(from, to, red, green, blue, true, NDEBUG_PERSIST_TILL_NEXT_SERVER)
...
ConVar nav_edit("nav_edit", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
				"Set to one to interactively edit the Navigation Mesh. Set to zero to leave edit mode.");
ConVar nav_quicksave("nav_quicksave", "1", FCVAR_GAMEDLL | FCVAR_CHEAT,
					 "Set to one to skip the time consuming phases of the analysis.  Useful for data collection and "
					 "testing."); // TERROR: defaulting to 1, since we don't need the other data
ConVar nav_show_approach_points("nav_show_approach_points", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
...
ConVar nav_show_player_counts("nav_show_player_counts", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
							  "Show current player counts in each area.");
ConVar nav_show_func_nav_avoid("nav_show_func_nav_avoid", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
							   "Show areas of designer-placed bot avoidance due to func_nav_avoid entities");
ConVar nav_show_func_nav_prefer("nav_show_func_nav_prefer", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
								"Show areas of designer-placed bot preference due to func_nav_prefer entities");
ConVar nav_show_func_nav_prerequisite(
	"nav_show_func_nav_prerequisite", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
...
CNavArea *CNavMesh::GetNavArea(CBaseEntity *pEntity, int nFlags, float flBeneathLimit) const
{
	...
	if(use && (nFlags && GETNAVAREA_CHECK_LOS) && (useZ < testPos.z - flStepHeight))
	{
		...
		trace_t result;
		UTIL_TraceLine(testPos, Vector(testPos.x, testPos.y, useZ), MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE,
					   &result);
 ...
 */
CNavArea *CNavMesh::GetNearestNavArea(const Vector &pos, bool anyZ, float maxDist, bool checkLOS, bool checkGround,
									  int team) const
{
		...
		for(int x = originX - shift; x <= originX + shift; ++x)
		{
			...
			for(int y = originY - shift; y <= originY + shift; ++y)
			{
				...
				FOR_EACH_VEC((*areaVector), it)
				{
					...
					// check LOS to area
					// REMOVED: If we do this for !anyZ, it's likely we wont have LOS and will enumerate every area in
					// the mesh It is still good to do this in some isolated cases, however
					if(checkLOS)
					{
						trace_t result;
						...
						UTIL_TraceLine(pos, pos + Vector(0, 0, StepHeight), MASK_NPCSOLID_BRUSHONLY, NULL,
						...
						// Don't bother tracing from the nav area up to safePos.z if it's within StepHeight of the area,
						// since areas can be embedded in the ground a bit
						float heightDelta = fabs(areaPos.z - safePos.z);
						if(heightDelta > StepHeight)
						{
							// trace to the height of the original point
							UTIL_TraceLine(areaPos + Vector(0, 0, StepHeight), Vector(areaPos.x, areaPos.y, safePos.z),
						...
						UTIL_TraceLine(safePos, Vector(areaPos.x, areaPos.y, safePos.z + StepHeight),
									   MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &result);
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Given the first part of a place name, return a place ID or zero if no place is defined, or the partial match is
 * ambiguous
 */
 ...
 */
int CNavMesh::PlaceNameAutocomplete(char const *partial,
...
public:
	...
	virtual bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask)
	{
		...
		if(FClassnameIs(pEntity, "prop_door") || FClassnameIs(pEntity, "prop_door_rotating") ||
...
void CNavMesh::DrawPlayerCounts(void) const
{
	...
	FOR_EACH_VEC(TheNavAreas, it)
	{
		...
		if(area->GetPlayerCount() > 0)
		{
			NDebugOverlay::Text(
				area->GetCenter(),
				msg.sprintf("%d (%d/%d)", area->GetPlayerCount(), area->GetPlayerCount(1), area->GetPlayerCount(2)),
...
static ConCommand nav_remove_jump_areas("nav_remove_jump_areas", CommandNavRemoveJumpAreas,
...
static ConCommand nav_delete_marked("nav_delete_marked", CommandNavDeleteMarked,
									"Deletes the currently marked Area (if any).", FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_flood_select,
			  "Selects the current Area and all Areas connected to it, recursively. To clear a selection, use this "
			  "command again.",
			  FCVAR_GAMEDLL | FCVAR_CHEAT)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
...
static ConCommand nav_toggle_selected_set("nav_toggle_selected_set", CommandNavToggleSelectedSet,
...
static ConCommand nav_store_selected_set("nav_store_selected_set", CommandNavStoreSelectedSet,
...
static ConCommand nav_recall_selected_set("nav_recall_selected_set", CommandNavRecallSelectedSet,
										  "Re-selects the stored selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_add_to_selected_set("nav_add_to_selected_set", CommandNavAddToSelectedSet,
										  "Add current area to the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_remove_from_selected_set("nav_remove_from_selected_set", CommandNavRemoveFromSelectedSet,
...
static ConCommand nav_toggle_in_selected_set("nav_toggle_in_selected_set", CommandNavToggleInSelectedSet,
											 "Remove current area from the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_clear_selected_set("nav_clear_selected_set", CommandNavClearSelectedSet,
										 "Clear the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);

//----------------------------------------------------------------------------------
CON_COMMAND_F(nav_dump_selected_set_positions,
...
//----------------------------------------------------------------------------------
CON_COMMAND_F(nav_show_dumped_positions, "Show the (x,y,z) coordinate positions of the given dump file.",
			  FCVAR_GAMEDLL | FCVAR_CHEAT)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
		return;
...
//----------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_larger_than, "Select nav areas where both dimensions are larger than the given size.",
			  FCVAR_GAMEDLL | FCVAR_CHEAT)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
		return;
...
static ConCommand nav_begin_selecting("nav_begin_selecting", CommandNavBeginSelecting,
									  "Start continuously adding to the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_end_selecting("nav_end_selecting", CommandNavEndSelecting,
									"Stop continuously adding to the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_begin_drag_selecting("nav_begin_drag_selecting", CommandNavBeginDragSelecting,
										   "Start dragging a selection area.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_end_drag_selecting("nav_end_drag_selecting", CommandNavEndDragSelecting,
										 "Stop dragging a selection area.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_begin_drag_deselecting("nav_begin_drag_deselecting", CommandNavBeginDragDeselecting,
											 "Start dragging a selection area.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_end_drag_deselecting("nav_end_drag_deselecting", CommandNavEndDragDeselecting,
										   "Stop dragging a selection area.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_raise_drag_volume_max("nav_raise_drag_volume_max", CommandNavRaiseDragVolumeMax,
											"Raise the top of the drag select volume.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_lower_drag_volume_max("nav_lower_drag_volume_max", CommandNavLowerDragVolumeMax,
											"Lower the top of the drag select volume.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_raise_drag_volume_min("nav_raise_drag_volume_min", CommandNavRaiseDragVolumeMin,
											"Raise the bottom of the drag select volume.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_lower_drag_volume_min("nav_lower_drag_volume_min", CommandNavLowerDragVolumeMin,
											"Lower the bottom of the drag select volume.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_toggle_selecting("nav_toggle_selecting", CommandNavToggleSelecting,
...
static ConCommand nav_begin_deselecting("nav_begin_deselecting", CommandNavBeginDeselecting,
...
static ConCommand nav_end_deselecting("nav_end_deselecting", CommandNavEndDeselecting,
									  "Stop continuously removing from the selected set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_toggle_deselecting("nav_toggle_deselecting", CommandNavToggleDeselecting,
										 "Start or stop continuously removing from the selected set.",
										 FCVAR_GAMEDLL | FCVAR_CHEAT);

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_half_space, "Selects any areas that intersect the given half-space.",
			  FCVAR_GAMEDLL | FCVAR_CHEAT)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
		return;
...
static ConCommand nav_begin_shift_xy("nav_begin_shift_xy", CommandNavBeginShiftXY, "Begin shifting the Selected Set.",
									 FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_end_shift_xy("nav_end_shift_xy", CommandNavEndShiftXY, "Finish shifting the Selected Set.",
								   FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_select_invalid_areas("nav_select_invalid_areas", CommandNavSelectInvalidAreas,
										   "Adds all invalid areas to the Selected Set.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_select_orphans, "Adds all orphan areas to the selected set (highlight a valid area first).",
			  FCVAR_CHEAT)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
		return;
...
static ConCommand nav_split(
	"nav_split", CommandNavSplit,
...
static ConCommand nav_make_sniper_spots("nav_make_sniper_spots", CommandNavMakeSniperSpots,
...
static ConCommand nav_merge("nav_merge", CommandNavMerge,
							"To merge two Areas into one, mark the first Area, highlight the second by pointing your "
							"cursor at it, and invoke the merge command.",
							FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_mark("nav_mark", CommandNavMark,
...
static ConCommand nav_begin_area("nav_begin_area", CommandNavBeginArea,
								 "Defines a corner of a new Area or Ladder. To complete the Area or Ladder, drag the "
								 "opposite corner to the desired location and issue a 'nav_end_area' command.",
								 FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_end_area("nav_end_area", CommandNavEndArea,
...
static ConCommand nav_connect("nav_connect", CommandNavConnect,
							  "To connect two Areas, mark the first Area, highlight the second Area, then invoke the "
							  "connect command. Note that this creates a ONE-WAY connection from the first to the "
							  "second Area. To make a two-way connection, also connect the second area to the first.",
							  FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_disconnect("nav_disconnect", CommandNavDisconnect,
								 "To disconnect two Areas, mark an Area, highlight a second Area, then invoke the "
								 "disconnect command. This will remove all connections between the two Areas.",
								 FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_disconnect_outgoing_oneways(
	"nav_disconnect_outgoing_oneways", CommandNavDisconnectOutgoingOneWays,
	"For each area in the selected set, disconnect all outgoing one-way connections.", FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_splice("nav_splice", CommandNavSplice,
							 "To splice, mark an area, highlight a second area, then invoke the splice command to "
							 "create a new, connected area between them.",
							 FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_crouch("nav_crouch", CommandNavCrouch,
...
static ConCommand nav_precise("nav_precise", CommandNavPrecise,
...
static ConCommand nav_jump("nav_jump", CommandNavJump,
...
static ConCommand nav_no_jump("nav_no_jump", CommandNavNoJump,
...
static ConCommand nav_stop("nav_stop", CommandNavStop,
...
static ConCommand nav_walk("nav_walk", CommandNavWalk,
...
static ConCommand nav_run("nav_run", CommandNavRun,
...
static ConCommand nav_avoid("nav_avoid", CommandNavAvoid,
...
static ConCommand nav_transient("nav_transient", CommandNavTransient,
...
static ConCommand nav_dont_hide("nav_dont_hide", CommandNavDontHide,
...
static ConCommand nav_stand("nav_stand", CommandNavStand,
...
static ConCommand nav_no_hostages("nav_no_hostages", CommandNavNoHostages,
...
static ConCommand nav_strip("nav_strip", CommandNavStrip,
...
//--------------------------------------------------------------------------------------------------------------
static int PlaceNameAutocompleteCallback(char const *partial,
										 char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH])
{
...
static ConCommand nav_use_place("nav_use_place", CommandNavUsePlace,
								"If used without arguments, all available Places will be listed. If a Place argument "
								"is given, the current Place is set.",
								FCVAR_GAMEDLL | FCVAR_CHEAT, PlaceNameAutocompleteCallback);
...
static ConCommand nav_place_replace("nav_place_replace", CommandNavPlaceReplace,
...
static ConCommand nav_toggle_place_mode(
	"nav_toggle_place_mode", CommandNavTogglePlaceMode,
...
static ConCommand nav_set_place_mode(
	"nav_set_place_mode", CommandNavSetPlaceMode,
...
static ConCommand nav_place_floodfill(
	"nav_place_floodfill", CommandNavPlaceFloodFill,
	"Sets the Place of the Area under the cursor to the curent Place, and 'flood-fills' the Place to all adjacent "
	"Areas. Flood-filling stops when it hits an Area with the same Place, or a different Place than that of the "
	"initial Area.",
	FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_place_set("nav_place_set", CommandNavPlaceSet,
...
static ConCommand nav_place_pick("nav_place_pick", CommandNavPlacePick,
...
static ConCommand nav_toggle_place_painting(
	"nav_toggle_place_painting", CommandNavTogglePlacePainting,
...
static ConCommand nav_mark_unnamed(
	"nav_mark_unnamed", CommandNavMarkUnnamed,
...
static ConCommand nav_corner_select(
	"nav_corner_select", CommandNavCornerSelect,
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(nav_corner_place_on_ground, "Places the selected corner of the currently marked Area on the ground.",
			  FCVAR_GAMEDLL | FCVAR_CHEAT)
{
	if(!UTIL_IsCommandIssuedByServerAdmin())
		return;
...
static ConCommand nav_generate("nav_generate", CommandNavGenerate,
...
static ConCommand nav_generate_incremental("nav_generate_incremental", CommandNavGenerateIncremental,
										   "Generate a Navigation Mesh for the current map and save it to disk.",
										   FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_analyze("nav_analyze", CommandNavAnalyze,
...
static ConCommand nav_analyze_scripted("nav_analyze_scripted", CommandNavAnalyzeScripted,
...
static ConCommand nav_mark_walkable("nav_mark_walkable", CommandNavMarkWalkable,
									"Mark the current location as a walkable position. These positions are used as "
									"seed locations when sampling the map to generate a Navigation Mesh.",
									FCVAR_GAMEDLL | FCVAR_CHEAT);
...
static ConCommand nav_clear_walkable_marks("nav_clear_walkable_marks", CommandNavClearWalkableMarks,
...
static ConCommand nav_compress_id("nav_compress_id", CommandNavCompressID,
...
void CommandNavShowLadderBounds(void)
{
	...
	while((ladder = dynamic_cast<CFuncSimpleLadder *>(gEntList.FindEntityByClassname(ladder, "func_simpleladder"))) !=
		  NULL)
	{
		Vector mins, maxs;
...
static ConCommand nav_show_ladder_bounds("nav_show_ladder_bounds", CommandNavShowLadderBounds,
...
static ConCommand nav_build_ladder("nav_build_ladder", CommandNavBuildLadder,
...
static ConCommand ClearAllNavAttributes("wipe_nav_attributes", NavEditClearAllAttributes,
										"Clear all nav attributes of selected area.", FCVAR_CHEAT);
...
//--------------------------------------------------------------------------------------------------------
NavAttributeLookup TheNavAttributeTable[] = {{"CROUCH", NAV_MESH_CROUCH},
											 {"JUMP", NAV_MESH_JUMP},
 ...
 */
static int NavAttributeAutocomplete(const char *input,
									char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH])
{
	...
	*partialArg = '\000';
	...
	for(unsigned int i = 0; TheNavAttributeTable[i].name && count < COMMAND_COMPLETION_MAXITEMS; ++i)
	{
		if(!Q_strnicmp(TheNavAttributeTable[i].name, partialArg, partialArgLength))
		{
			...
			Q_snprintf(commands[count++], COMMAND_COMPLETION_ITEM_LENGTH, "%s %s", command,
					   TheNavAttributeTable[i].name);
		}
	}
...
static ConCommand NavClearAttribute("nav_clear_attribute", NavEditClearAttribute,
									"Remove given nav attribute from all areas in the selected set.", FCVAR_CHEAT,
									NavAttributeAutocomplete);
...
static ConCommand NavMarkAttribute("nav_mark_attribute", NavEditMarkAttribute,
								   "Set nav attribute for all areas in the selected set.", FCVAR_CHEAT,
								   NavAttributeAutocomplete);
...
static ConCommand nav_pick_area( "nav_pick_area", CommandNavPickArea, "Marks an area (and corner) based on the surface
under the cursor.", FCVAR_GAMEDLL | FCVAR_CHEAT );
...
static ConCommand nav_resize_horizontal( "nav_resize_horizontal", CommandNavResizeHorizontal, "TODO", FCVAR_GAMEDLL |
FCVAR_CHEAT );
...
static ConCommand nav_resize_vertical( "nav_resize_vertical", CommandNavResizeVertical, "TODO", FCVAR_GAMEDLL |
FCVAR_CHEAT );
