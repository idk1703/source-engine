...
const Vector CPhysicsCloneArea::vLocalMins(3.0f, -PORTAL_HALF_WIDTH *PHYSICSCLONEAREASCALE,
										   -PORTAL_HALF_HEIGHT *PHYSICSCLONEAREASCALE);
const Vector CPhysicsCloneArea::vLocalMaxs(
	PORTAL_HALF_HEIGHT
		*PHYSICSCLONEAREASCALE, // x is the forward which is fairly thin for portals, replacing with halfheight
	...
	for(int i = 0; i < count; i++)
	{
		...
		if(pEntity && (pEntity != this))
		{
			...
			if(pPhysicsObject)
			{
				...
				// double check intersection at the OBB vs OBB level, we don't want to affect large piles of physics
				// objects if we don't have to, it gets slow
				if(IsOBBIntersectingOBB(ptOrigin, qAngles, vLocalMins, vLocalMaxs, ptEntityCenter,
										pEntCollision->GetCollisionAngles(), pEntCollision->OBBMins(),
