...
BEGIN_DATADESC(CFunc_LiquidPortal)
	DEFINE_FIELD(m_hLinkedPortal, FIELD_EHANDLE), DEFINE_FIELD(m_bFillInProgress, FIELD_BOOLEAN),
		DEFINE_FIELD(m_fFillStartTime, FIELD_TIME), DEFINE_FIELD(m_fFillEndTime, FIELD_TIME),
		DEFINE_FIELD(m_matrixThisToLinked, FIELD_VMATRIX), DEFINE_UTLVECTOR(m_hTeleportList, FIELD_EHANDLE),
...
SendPropEHandle(SENDINFO(m_hLinkedPortal)), SendPropFloat(SENDINFO(m_fFillStartTime)),
...
void CFunc_LiquidPortal::InputRemoveActivatorFromTeleportList(inputdata_t &inputdata)
{
	...
	for(int i = m_hTeleportList.Count(); --i >= 0;)
	{
		if(m_hTeleportList[i].Get() == inputdata.pActivator)
		{
			...
			if(inputdata.pActivator->IsPlayer() &&
			   (((CPortal_Player *)inputdata.pActivator)->m_hSurroundingLiquidPortal.Get() == this))
