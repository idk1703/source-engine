...
#define MAXIMUM_BUMP_DISTANCE                                     \
	((PORTAL_HALF_WIDTH * 2.0f) * (PORTAL_HALF_WIDTH * 2.0f) +    \
	 (PORTAL_HALF_HEIGHT * 2.0f) * (PORTAL_HALF_HEIGHT * 2.0f)) / \
		2.0f
...
void TracePortals(const CProp_Portal *pIgnorePortal, const Vector &vForward, const Vector &vStart, const Vector &vEnd,
				  trace_t &tr)
{
	UTIL_ClearTrace(tr);
...
bool TracePortalCorner(const CProp_Portal *pIgnorePortal, const Vector &vOrigin, const Vector &vCorner,
					   const Vector &vForward, int iPlacedBy, ITraceFilter *pTraceFilterPortalShot, trace_t &tr,
					   bool &bSoftBump)
{
	...
	if(trSurfaceEdge.startsolid)
	{
		...
		{
			UTIL_TraceLine(vOrigin + vOriginToCorner * (fTotalFraction + 0.05f) - vForward,
						   vCorner + vOriginToCorner * (fTotalFraction + 0.05f) - vForward, MASK_SHOT_PORTAL,
						   pTraceFilterPortalShot, &trSurfaceEdge);
		...
		{
			UTIL_TraceLine(vOrigin + vOriginToCorner * (fTotalFraction + 0.05f) - vForward, vOrigin - vForward,
						   MASK_SHOT_PORTAL, pTraceFilterPortalShot, &trSurfaceEdge);
	...
	UTIL_TraceLine(vOrigin + vForward, vCorner + vForward, MASK_SOLID_BRUSHONLY | CONTENTS_MONSTER,
				   pTraceFilterPortalShot, &trEnclosingWall);
	...
	else if(!trBumpingEntity.startsolid && trBumpingEntity.fraction <= trEnclosingWall.fraction &&
			trBumpingEntity.fraction <= trPortal.fraction)
	{
		tr = trBumpingEntity;
...
Vector FindBumpVectorInCorner(const Vector &ptCorner1, const Vector &ptCorner2, const Vector &ptIntersectionPoint1,
							  const Vector &ptIntersectionPoint2, const Vector &vIntersectionDirection1,
							  const Vector &vIntersectionDirection2, const Vector &vIntersectionBumpDirection1,
							  const Vector &vIntersectionBumpDirection2)
{
	...
	if(fPortalEdgeDotLeg < -0.9999f || fPortalEdgeDotLeg > 0.9999f || fPortalEdgeDotLeg2 < -0.9999f ||
	   fPortalEdgeDotLeg2 > 0.9999f)
	{
		// Do a one corner bump with corner 1
		float fBumpDistance1 =
			CalcDistanceToLine(ptCorner1, ptIntersectionPoint1, ptIntersectionPoint1 + vIntersectionDirection1);

		fBumpDistance1 += PORTAL_BUMP_FORGIVENESS;

		// Do a one corner bump with corner 2
		float fBumpDistance2 =
			CalcDistanceToLine(ptCorner2, ptIntersectionPoint2, ptIntersectionPoint2 + vIntersectionDirection2);
...
bool FitPortalOnSurface(const CProp_Portal *pIgnorePortal, Vector &vOrigin, const Vector &vForward,
						const Vector &vRight, const Vector &vTopEdge, const Vector &vBottomEdge,
						const Vector &vRightEdge, const Vector &vLeftEdge, int iPlacedBy,
						ITraceFilter *pTraceFilterPortalShot, int iRecursions /*= 0*/,
						const CPortalCornerFitData *pPortalCornerFitData /*= 0*/,
						const int *p_piIntersectionIndex /*= 0*/, const int *piIntersectionCount /*= 0*/)
{
	...
	for(int iIntersection = 0; iIntersection < 4; ++iIntersection)
	{
		...
		if(iIntersectionCount < 4)
		{
			...
			if(!sFitData[iIntersection].bCornerIntersection)
			{
				...
				sFitData[iIntersection].bCornerIntersection = TracePortalCorner(
					pIgnorePortal, vOrigin, pptCorner[iIntersection], vForward, iPlacedBy, pTraceFilterPortalShot,
					sFitData[iIntersection].trCornerTrace, sFitData[iIntersection].bSoftBump);
				...
				if(sFitData[iIntersection].bCornerIntersection)
				{
					sFitData[iIntersection].ptIntersectionPoint =
						vOrigin + (pptCorner[iIntersection] - vOrigin) * sFitData[iIntersection].trCornerTrace.fraction;
					VectorNormalize(sFitData[iIntersection].trCornerTrace.plane.normal);
					sFitData[iIntersection].vIntersectionDirection =
						sFitData[iIntersection].trCornerTrace.plane.normal.Cross(vForward);
					VectorNormalize(sFitData[iIntersection].vIntersectionDirection);
					sFitData[iIntersection].vBumpDirection =
						vForward.Cross(sFitData[iIntersection].vIntersectionDirection);
					...
					if(sv_portal_placement_debug.GetBool())
					{
						for(int iIntersection = 0; iIntersection < 4; ++iIntersection)
						{
							NDebugOverlay::Line(sFitData[iIntersection].ptIntersectionPoint -
													sFitData[iIntersection].vIntersectionDirection * 32.0f,
												sFitData[iIntersection].ptIntersectionPoint +
													sFitData[iIntersection].vIntersectionDirection * 32.0f,
												0, 0, 255, true, 0.5f);
						}
	...
	switch(iIntersectionCount)
	{
		...
		case 1:
		{
			float fBumpDistance = CalcDistanceToLine(pptCorner[piIntersectionIndex[0]],
													 ...
													 sFitData[piIntersectionIndex[0]].ptIntersectionPoint +
														 sFitData[piIntersectionIndex[0]].vIntersectionDirection);

			fBumpDistance += PORTAL_BUMP_FORGIVENESS;

			vOrigin += sFitData[piIntersectionIndex[0]].vBumpDirection * fBumpDistance;

			return FitPortalOnSurface(pIgnorePortal, vOrigin, vForward, vRight, vTopEdge, vBottomEdge, vRightEdge,
									  vLeftEdge, iPlacedBy, pTraceFilterPortalShot, iRecursions + 1, sFitData,
									  piIntersectionIndex, &iIntersectionCount);
		}
		break;

		case 2:
		{
			if(sFitData[piIntersectionIndex[0]].ptIntersectionPoint ==
			   sFitData[piIntersectionIndex[1]].ptIntersectionPoint)
			{
				return false;
			}

			float fDot =
				sFitData[piIntersectionIndex[0]].vBumpDirection.Dot(sFitData[piIntersectionIndex[1]].vBumpDirection);
			...
			if(fDot < -0.9f)
			{
				...
				trace_t trPerpWall1;
				bool bSoftBump1;
				bool bDir1 = TracePortalCorner(pIgnorePortal, vOrigin,
											   vOrigin + sFitData[piIntersectionIndex[0]].vIntersectionDirection *
															 PORTAL_HALF_WIDTH * 2.0f,
											   vForward, iPlacedBy, pTraceFilterPortalShot, trPerpWall1, bSoftBump1);

				trace_t trPerpWall2;
				bool bSoftBump2;
				bool bDir2 = TracePortalCorner(pIgnorePortal, vOrigin,
											   vOrigin + sFitData[piIntersectionIndex[0]].vIntersectionDirection *
															 -PORTAL_HALF_WIDTH * 2.0f,
											   vForward, iPlacedBy, pTraceFilterPortalShot, trPerpWall2, bSoftBump2);
			...
			if(fDot > 0.9f)
			{
				...
				int iClosestIntersection = ((vOrigin.DistTo(sFitData[piIntersectionIndex[0]].ptIntersectionPoint) <
											 vOrigin.DistTo(sFitData[piIntersectionIndex[1]].ptIntersectionPoint))
												? (0)
												: (1));
				...
				{
					pfBumpDistance[iIntersection] = CalcDistanceToLine(
						pptCorner[piIntersectionIndex[iIntersection]],
						sFitData[piIntersectionIndex[iClosestIntersection]].ptIntersectionPoint,
						sFitData[piIntersectionIndex[iClosestIntersection]].ptIntersectionPoint +
							sFitData[piIntersectionIndex[iClosestIntersection]].vIntersectionDirection);
				...
				// Bump the portal
				vOrigin +=
					sFitData[piIntersectionIndex[iClosestIntersection]].vBumpDirection * pfBumpDistance[iLargestBump];
				...
				if(pfBumpDistance[0] == pfBumpDistance[1])
				{
					...
					return FitPortalOnSurface(pIgnorePortal, vOrigin, vForward, vRight, vTopEdge, vBottomEdge,
											  vRightEdge, vLeftEdge, iPlacedBy, pTraceFilterPortalShot, iRecursions + 1,
											  sFitData, piIntersectionIndex, &iIntersectionCount);
				}
				else
				{
					// Prepare data for recursion
					...
					{
						sFitData[piIntersectionIndex[iLargestBump]] =
							sFitData[piIntersectionIndex[iClosestIntersection]];
					}
			...
			vOrigin += FindBumpVectorInCorner(pptCorner[piIntersectionIndex[0]], pptCorner[piIntersectionIndex[1]],
											  ...
											  sFitData[piIntersectionIndex[1]].ptIntersectionPoint,
											  sFitData[piIntersectionIndex[0]].vIntersectionDirection,
											  sFitData[piIntersectionIndex[1]].vIntersectionDirection,
											  sFitData[piIntersectionIndex[0]].vBumpDirection,
											  sFitData[piIntersectionIndex[1]].vBumpDirection);
		...
		case 3:
		{
			...
			fDot[0] =
				sFitData[piIntersectionIndex[0]].vBumpDirection.Dot(sFitData[piIntersectionIndex[1]].vBumpDirection);
			fDot[1] =
				sFitData[piIntersectionIndex[1]].vBumpDirection.Dot(sFitData[piIntersectionIndex[2]].vBumpDirection);
			fDot[2] =
				sFitData[piIntersectionIndex[2]].vBumpDirection.Dot(sFitData[piIntersectionIndex[0]].vBumpDirection);
			...
			for(int iDot = 0; iDot < 3; ++iDot)
			{
				...
				if(fDot[iDot] < -0.99f)
				{
					...
					trace_t trPerpWall1;
					bool bSoftBump1;
					bool bDir1 = TracePortalCorner(
						pIgnorePortal, vOrigin,
						vOrigin + sFitData[piIntersectionIndex[iDot]].vIntersectionDirection * PORTAL_HALF_WIDTH * 2.0f,
						vForward, iPlacedBy, pTraceFilterPortalShot, trPerpWall1, bSoftBump1);

					trace_t trPerpWall2;
					bool bSoftBump2;
					bool bDir2 =
						TracePortalCorner(pIgnorePortal, vOrigin,
										  vOrigin + sFitData[piIntersectionIndex[iDot]].vIntersectionDirection *
														-PORTAL_HALF_WIDTH * 2.0f,
										  vForward, iPlacedBy, pTraceFilterPortalShot, trPerpWall2, bSoftBump2);
			...
			if(iSimilarWalls == 0)
			{
				...
				if(M_PI_F - 0.01f < fAngleTotal && fAngleTotal < M_PI_F + 0.01f)
				{
					...
					if(sFitData[piIntersectionIndex[0]].bSoftBump || sFitData[piIntersectionIndex[1]].bSoftBump ||
					   sFitData[piIntersectionIndex[2]].bSoftBump)
					{
						// Prepare data for recursion
						iIntersectionCount = 0;
						sFitData[piIntersectionIndex[0]].bCornerIntersection = false;
						...
						return FitPortalOnSurface(pIgnorePortal, vOrigin, vForward, vRight, vTopEdge, vBottomEdge,
												  vRightEdge, vLeftEdge, iPlacedBy, pTraceFilterPortalShot,
												  iRecursions + 1, sFitData, piIntersectionIndex, &iIntersectionCount);
					}
					else
					{
						return false;
			...
			if(iSimilarWalls == 3)
			{
				...
				for(int iIntersection = 0; iIntersection < 3; ++iIntersection)
				{
					...
					{
						sFitData[piIntersectionIndex[iIntersection]] =
							sFitData[piIntersectionIndex[iClosestIntersection]];
			...
			for(int iIntersection = 0; iIntersection < 3; ++iIntersection)
			{
				float fDist = pptCorner[piIntersectionIndex[iIntersection]].DistTo(
					pptCorner[piIntersectionIndex[(iIntersection + 1) % 3]]);
			...
			float fCornerDot = sFitData[piIntersectionIndex[iIndex1]].vIntersectionDirection.Dot(
				sFitData[piIntersectionIndex[iIndex2]].vIntersectionDirection);
			...
			if(fCornerDot < 0.0001f && fCornerDot > -0.0001f)
			{
				...
				{
					float fBump1 =
						CalcDistanceToLine(pptCorner[piIntersectionIndex[iIndex1]],
										   sFitData[piIntersectionIndex[iIndex1]].ptIntersectionPoint,
										   sFitData[piIntersectionIndex[iIndex1]].ptIntersectionPoint +
											   sFitData[piIntersectionIndex[iIndex1]].vIntersectionDirection);

					fBump1 += PORTAL_BUMP_FORGIVENESS;

					float fBump2 =
						CalcDistanceToLine(pptCorner[piIntersectionIndex[iIndex2]],
			...
			vOrigin +=
				FindBumpVectorInCorner(pptCorner[piIntersectionIndex[iIndex1]], pptCorner[piIntersectionIndex[iIndex2]],
									   sFitData[piIntersectionIndex[iIndex1]].ptIntersectionPoint,
									   sFitData[piIntersectionIndex[iIndex2]].ptIntersectionPoint,
									   sFitData[piIntersectionIndex[iIndex1]].vIntersectionDirection,
									   sFitData[piIntersectionIndex[iIndex2]].vIntersectionDirection,
									   sFitData[piIntersectionIndex[iIndex1]].vBumpDirection,
									   sFitData[piIntersectionIndex[iIndex2]].vBumpDirection);
		...
		{
			if(sFitData[piIntersectionIndex[0]].bSoftBump || sFitData[piIntersectionIndex[1]].bSoftBump ||
			   sFitData[piIntersectionIndex[2]].bSoftBump || sFitData[piIntersectionIndex[3]].bSoftBump)
			{
				// Prepare data for recursion
				iIntersectionCount = 0;
				sFitData[piIntersectionIndex[0]].bCornerIntersection = false;
...
void FitPortalAroundOtherPortals(const CProp_Portal *pIgnorePortal, Vector &vOrigin, const Vector &vForward,
...
bool IsPortalIntersectingNoPortalVolume(const Vector &vOrigin, const QAngle &qAngles, const Vector &vForward)
{
	...
	for(CFuncNoPortalVolume *pNoPortalEnt = GetNoPortalVolumeList(); pNoPortalEnt != NULL;
		pNoPortalEnt = pNoPortalEnt->m_pNext)
	{
...
bool IsPortalOverlappingOtherPortals(const CProp_Portal *pIgnorePortal, const Vector &vOrigin, const QAngle &qAngles,
									 bool bFizzle /*= false*/)
{
	...
	if(iPortalCount != 0)
	{
		...
		for(int i = 0; i != iPortalCount; ++i)
		{
			...
			if(pTempPortal != pIgnorePortal && pTempPortal->m_bActivated)
			{
				...
				if(IsOBBIntersectingOBB(vOrigin, qAngles, vPortalOBBMin, vPortalOBBMax, vOtherOrigin, qOtherAngles,
										vPortalOBBMin, vPortalOBBMax, 0.0f))
				{
					if(sv_portal_placement_debug.GetBool())
					{
						UTIL_Portal_NDebugOverlay(vOrigin, qAngles, 0, 0, 255, 128, false, 0.5f);
...
bool IsPortalOnValidSurface(const Vector &vOrigin, const Vector &vForward, const Vector &vRight, const Vector &vUp,
							ITraceFilter *traceFilterPortalShot)
{
	trace_t tr;
...
float VerifyPortalPlacement(const CProp_Portal *pIgnorePortal, Vector &vOrigin, QAngle &qAngles, int iPlacedBy,
							bool bTest /*= false*/)
{
	...
	if(!sv_portal_placement_never_bump.GetBool())
	{
		...
		if(!FitPortalOnSurface(pIgnorePortal, vOrigin, vForward, vRight, vTopEdge, vBottomEdge, vRightEdge, vLeftEdge,
							   iPlacedBy, &traceFilterPortalShot))
		{
		...
		if(vUp.z > 0.7f)
		{
			...
			trace_t FloorTrace;
			UTIL_TraceLine(vOrigin + vSmallForward, vOrigin + vSmallForward - (vUp * (PORTAL_HALF_HEIGHT + 1.5f)),
						   MASK_SOLID_BRUSHONLY, &traceFilterPortalShot, &FloorTrace);
			if(FloorTrace.fraction < 1.0f)
			{
				...
				UTIL_TraceLine(vOrigin + vSmallForward, vOrigin + vSmallForward - (vUp * (PORTAL_HALF_HEIGHT - 0.1f)),
							   MASK_SOLID_BRUSHONLY, &traceFilterPortalShot, &FloorTrace_Verify);
				if(FloorTrace_Verify.fraction == 1.0f)
				{
					// if we're in here, we're definitely in a floor matching configuration, bump down to match the
					// floor better
					vOrigin = FloorTrace.endpos + (vUp * PORTAL_HALF_HEIGHT) -
							  vSmallForward; // - vUp * PORTAL_WALL_MIN_THICKNESS;
				}
			}
		}
	}
	...
	return fAnalogSuccessMultiplier * (PORTAL_ANALOG_SUCCESS_NO_BUMP - PORTAL_ANALOG_SUCCESS_BUMPED) +
		   PORTAL_ANALOG_SUCCESS_BUMPED;
}