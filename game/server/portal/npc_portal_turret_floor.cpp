...
const char *g_PortalTalkNames[PORTAL_TURRET_STATE_TOTAL - TURRET_STATE_TOTAL] = {
	"NPC_FloorTurret.TalkDisabled", "NPC_FloorTurret.TalkCollide", "NPC_FloorTurret.TalkPickup",
	"NPC_FloorTurret.TalkShotAt", "NPC_FloorTurret.TalkDissolved"};
...
BEGIN_DATADESC(CNPC_Portal_FloorTurret)
	...
	DEFINE_ARRAY(m_hRopes, FIELD_EHANDLE, PORTAL_FLOOR_TURRET_NUM_ROPES),
		...
		DEFINE_FIELD(m_bOutOfAmmo, FIELD_BOOLEAN), DEFINE_FIELD(m_bLaserOn, FIELD_BOOLEAN),
		DEFINE_FIELD(m_sLaserHaloSprite, FIELD_INTEGER), DEFINE_FIELD(m_flDistToEnemy, FIELD_FLOAT),

		//	DEFINE_ARRAY( m_iBarrelAttachments, FIELD_INTEGER, 4 ),
		DEFINE_FIELD(m_bShootWithBottomBarrels, FIELD_BOOLEAN), DEFINE_FIELD(m_fSearchSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_fMovingTargetThreashold, FIELD_FLOAT), DEFINE_FIELD(m_iLastState, FIELD_INTEGER),
		DEFINE_FIELD(m_fNextTalk, FIELD_FLOAT), DEFINE_FIELD(m_bDelayTippedTalk, FIELD_BOOLEAN),
		...
		DEFINE_THINKFUNC(HeldThink), DEFINE_THINKFUNC(InactiveThink), DEFINE_THINKFUNC(SuppressThink),
...
SendPropBool(SENDINFO(m_bOutOfAmmo)), SendPropBool(SENDINFO(m_bLaserOn)), SendPropInt(SENDINFO(m_sLaserHaloSprite)),
...
void CNPC_Portal_FloorTurret::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
												const notify_system_event_params_t &params)
{
...
bool CNPC_Portal_FloorTurret::PreThink(turretState_e state)
{
	...
	if(m_iLastState != iNewState &&
	   ((iNewState == TURRET_TIPPED && !m_bDelayTippedTalk) || iNewState == TURRET_RETIRING ||
		iNewState == PORTAL_TURRET_DISSOLVED || iNewState == PORTAL_TURRET_PICKUP))
	{
...
void CNPC_Portal_FloorTurret::ActiveThink(void)
{
	...
	if(pEnemy->IsAlive())
	{
		...
		if(pPortal && FVisibleThroughPortal(pPortal, pEnemy))
		{
			...
			UTIL_Portal_PointTransform(pPortal->m_hLinkedPortal->MatrixThisToLinked(), vecMidEnemy,
									   vecMidEnemyTransformed);
			...
			// If it's not visible through normal means or the enemy is closer through the portal, use the translated
			// info
		...
		if(bCanShoot) // 10 degree slop XY
		{
			...
			else
			{
				if(dot3d >= minCos3d)
				{
					...
					SetActivity(
						(Activity)((m_bShootWithBottomBarrels) ? (ACT_FLOOR_TURRET_FIRE2) : (ACT_FLOOR_TURRET_FIRE)));
	...
	if(bEnemyVisible)
	{
		...
		{
			// Translate our target across the portal
			UTIL_Portal_PointTransform(pPortal->m_hLinkedPortal->MatrixThisToLinked(), vEnemyWorldSpaceCenter,
									   vEnemyWorldSpaceCenter);
		}
...
void CNPC_Portal_FloorTurret::HackFindEnemy(void)
{
	...
	if(GetEnemy() == NULL)
	{
		...
		for(pObject = GetSenses()->GetFirstSeenEntity(&iter, SEEN_MISC); pObject;
			pObject = GetSenses()->GetNextSeenEntity(&iter))
		{
			Vector vVelocity;
...
void CNPC_Portal_FloorTurret::StartTouch(CBaseEntity *pOther)
{
	...
	if(m_pMotionController->Enabled())
	{
		...
		if(!pOther->IsPlayer() && pOther->GetMoveType() == MOVETYPE_VPHYSICS &&
		   !(pTurretPhys && ((pTurretPhys->GetGameFlags() & FVPHYSICS_PLAYER_HELD) != 0)))
		{
			...
			Vector vAngleImpulse((vTurretRight.Dot(vVelocityImpulse) < 0.0f) ? (-1.6f) : (1.6f),
