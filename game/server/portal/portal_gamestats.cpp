...
public:
	...
	~CPortalGameStatsSingleton(void)
	{
		AssertMsg((CBaseGameStats::StatTrackingAllowed() == false) ||
					  ((s_DummyStats.m_pDeaths->Count() == 0) && (s_DummyStats.m_pPlacements->Count() == 0) &&
					   (s_DummyStats.m_pUseEvents->Count() == 0) && (s_DummyStats.m_pStuckSpots->Count() == 0) &&
					   (s_DummyStats.m_pJumps->Count() == 0) && (s_DummyStats.m_pTimeSpentInVisLeafs->Count() == 0)),
...
CPortalGameStats::CPortalGameStats(void) : m_pCurrentMapStats(&s_DummyStats) {}
...
void Portal_Gamestats_LevelStats_t::AppendSubChunksToBuffer(CUtlBuffer &SaveBuffer)
{
	...
	if(m_pTimeSpentInVisLeafs->Count() != 0)
	{
		...
		for(int i = 0; i != iLeafTimeStatsCount; ++i)
		{
			...
			SaveBuffer.PutFloat(
				LeafTimeStat.fTimeSpentInVisLeaf); // assumes visleafs will be the same when this data is loaded again,
												   // or that there will be a way to invalidate the data
		}
...
void Portal_Gamestats_LevelStats_t::LoadSubChunksFromBuffer(CUtlBuffer &LoadBuffer, unsigned int iChunkEndPosition)
{
	...
	{
		Assert((iChunkEndPosition - LoadBuffer.TellGet()) >
			   (sizeof(unsigned short) + sizeof(unsigned int))); // at least an empty chunk left

		unsigned short iChunkID = LoadBuffer.GetUnsignedShort();
		unsigned int iChunkSize =
			LoadBuffer.GetUnsignedInt() - sizeof(unsigned int); // chunk size includes the chunk size data itself
#ifdef _DEBUG
...
{
#ifdef PORTAL_GAMESTATS_VERBOSE // we only have verbose chunks for now, so only write custom data if we're verbosely
								// tracking
	...
	// you can simulate a header by enclosing the entirety of the custom data in a chunk that starts with a header, but
	// you'll kill loading in old versions
	...
	if(m_CustomMapStats.Count() != 0) // we have some map stats
	{
		...
		for(int i = m_CustomMapStats.First(); i != m_CustomMapStats.InvalidIndex(); i = m_CustomMapStats.Next(i))
		{
			...
			// we can trivially find the chunk size after the chunk is written, but chunk size needs to be at the
			// beginning, reserve the space for chunk size now
			...
			// write out the map stats chunk size
			{
				...
				Assert(iOldTellPut ==
					   SaveBuffer.TellPut()); // writing the chunk size should have overwritten, not inserted
			}
		}
	}
...
void CPortalGameStats::LoadCustomDataFromBuffer(CUtlBuffer &LoadBuffer)
{
	...
	{
		Assert((iEndPosition - LoadBuffer.TellGet()) >
			   (sizeof(unsigned short) + sizeof(unsigned int))); // at least an empty chunk left
		...
		switch(iChunkID)
		{
#ifdef PORTAL_GAMESTATS_VERBOSE // levelstats only have verbose data for the time being, so only bother to load
								// verboseness if we're still tracking it
...
void CPortalGameStats::Event_PortalPlacement(const Vector &ptPlayerFiredFrom, const Vector &ptAttemptedPosition,
											 char iSuccessCode)
{
#ifdef PORTAL_GAMESTATS_VERBOSE
	if(CBaseGameStats::StatTrackingAllowed() == false)
...
void CPortalGameStats::Event_PlayerUsed(const Vector &ptTraceStart, const Vector &vTraceDelta, CBaseEntity *pUsedEntity)
{
	...
	if(fLastUseTime >
	   gpGlobals->curtime) // I'm not positive, but I think curtime resets between levels, cheap to do this
		fLastUseTime = 0.0f;
...
void CPortalGameStats::Event_PlayerStuck(CPortal_Player *pPlayer)
{
	...
	if((gpGlobals->curtime - fLastStuckTime) <
	   10.0f) // only log one stuck spot per 10 second interval (in case it oscillates)
		return;
