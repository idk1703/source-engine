...
static char *g_pszPortalNonCleansable[] = {
	"func_door",		  "func_door_rotating", "prop_door_rotating", "func_tracktrain",
	"env_ghostanimating", "physicsshadowclone", "prop_energy_ball",	  NULL,
};
...
BEGIN_DATADESC(CTriggerPortalCleanser)
	...
	// Outputs
	DEFINE_OUTPUT(m_OnDissolve, "OnDissolve"), DEFINE_OUTPUT(m_OnFizzle, "OnFizzle"),
...
void CTriggerPortalCleanser::Touch(CBaseEntity *pOther)
{
	...
	if(pOther->IsPlayer())
	{
		...
		if(pPlayer)
		{
			CWeaponPortalgun *pPortalgun =
				dynamic_cast<CWeaponPortalgun *>(pPlayer->Weapon_OwnsThisType("weapon_portalgun"));
	...
	if(pBaseAnimating && !pBaseAnimating->IsDissolving())
	{
		...
		// always being 'box'. We use special logic when the cleanser dissolves a box so this is a special output for
		// it.
		...
		if(pOldPhys && (pOldPhys->GetGameFlags() & FVPHYSICS_PLAYER_HELD))
		{
			...
			if(pPlayer)
			{
				...
				vOldVel = pPlayer->GetAbsVelocity() +
						  Vector(pPlayer->EyeDirection2D().x * 4.0f, pPlayer->EyeDirection2D().y * 4.0f, -32.0f);
			}
		}
