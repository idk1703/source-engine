...
BEGIN_DATADESC(CProp_Portal)
	// saving
	DEFINE_FIELD(m_hLinkedPortal, FIELD_EHANDLE), DEFINE_KEYFIELD(m_iLinkageGroupID, FIELD_CHARACTER, "LinkageGroupID"),
		DEFINE_FIELD(m_matrixThisToLinked, FIELD_VMATRIX), DEFINE_KEYFIELD(m_bActivated, FIELD_BOOLEAN, "Activated"),
		DEFINE_KEYFIELD(m_bIsPortal2, FIELD_BOOLEAN, "PortalTwo"), DEFINE_FIELD(m_vPrevForward, FIELD_VECTOR),
		DEFINE_FIELD(m_hMicrophone, FIELD_EHANDLE), DEFINE_FIELD(m_hSpeaker, FIELD_EHANDLE),

		DEFINE_SOUNDPATCH(m_pAmbientSound),

		DEFINE_FIELD(m_vAudioOrigin, FIELD_VECTOR), DEFINE_FIELD(m_vDelayedPosition, FIELD_VECTOR),
		DEFINE_FIELD(m_qDelayedAngles, FIELD_VECTOR), DEFINE_FIELD(m_iDelayedFailure, FIELD_INTEGER),
		...
		// Function Pointers
		DEFINE_THINKFUNC(DelayedPlacementThink), DEFINE_THINKFUNC(TestRestingSurfaceThink),
...
SendPropEHandle(SENDINFO(m_hLinkedPortal)), SendPropBool(SENDINFO(m_bActivated)), SendPropBool(SENDINFO(m_bIsPortal2)),
END_SEND_TABLE
()
...
void CProp_Portal::OnRestore()
{
	...
	if(m_bActivated)
	{
		DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_particles") : ("portal_1_particles")), PATTACH_POINT_FOLLOW,
							   this, "particles_2", true);
		DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_edge") : ("portal_1_edge")), PATTACH_POINT_FOLLOW, this,
							   "particlespin");
	}
}

void DumpActiveCollision(const CPortalSimulator *pPortalSimulator, const char *szFileName);
void PortalSimulatorDumps_DumpCollideToGlView(CPhysCollide *pCollide, const Vector &origin, const QAngle &angles,
...
void CProp_Portal::DelayedPlacementThink(void)
{
	...
	if(m_iDelayedFailure != PORTAL_FIZZLE_BAD_SURFACE && m_iDelayedFailure != PORTAL_FIZZLE_NEAR_BLUE &&
...
//-----------------------------------------------------------------------------
// Purpose: When placed on a surface that could potentially go away (anything but world geo), we test for that condition
// and fizzle
//-----------------------------------------------------------------------------
void CProp_Portal::TestRestingSurfaceThink(void)
{
	...
	for(int iCorner = 0; iCorner < 4; ++iCorner)
	{
		...
		// This corner isn't on a valid brush (skipping phys converts or physboxes because they frequently go through
		// portals and can't be placed upon).
		if(tr.fraction == 1.0f && !tr.startsolid &&
		   (!tr.m_pEnt || (tr.m_pEnt && !FClassnameIs(tr.m_pEnt, "func_physbox") &&
						   !FClassnameIs(tr.m_pEnt, "simple_physics_brush"))))
		{
	...
	{
		SetContextThink(&CProp_Portal::TestRestingSurfaceThink, gpGlobals->curtime + 0.1f,
						s_pTestRestingSurfaceContext);
	}
	else
	{
...
void CProp_Portal::DoFizzleEffect(int iEffect, bool bDelayedPos /*= true*/)
{
	...
	if(pPortalGun && (iEffect != PORTAL_FIZZLE_CLOSE) && (iEffect != PORTAL_FIZZLE_SUCCESS) &&
	...
	switch(iEffect)
	{
		case PORTAL_FIZZLE_CANT_FIT:
			...
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_nofit") : ("portal_1_nofit")), fxData.m_vOrigin,
								   fxData.m_vAngles, this);
			break;
		...
		case PORTAL_FIZZLE_OVERLAPPED_LINKED:
		{
			...
			VectorAngles(vUp, vForward, fxData.m_vAngles);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_overlap") : ("portal_1_overlap")), fxData.m_vOrigin,
								   fxData.m_vAngles, this);
			ep.m_pSoundName = "Portal.fizzle_invalid_surface";
			break;
		}
		...
		case PORTAL_FIZZLE_BAD_VOLUME:
			...
			VectorAngles(vUp, vForward, fxData.m_vAngles);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_badvolume") : ("portal_1_badvolume")), fxData.m_vOrigin,
								   fxData.m_vAngles);
			ep.m_pSoundName = "Portal.fizzle_invalid_surface";
			break;
		...
		case PORTAL_FIZZLE_BAD_SURFACE:
			...
			VectorAngles(vUp, vForward, fxData.m_vAngles);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_badsurface") : ("portal_1_badsurface")),
								   fxData.m_vOrigin, fxData.m_vAngles);
			ep.m_pSoundName = "Portal.fizzle_invalid_surface";
			break;
		...
		case PORTAL_FIZZLE_KILLED:
			...
			VectorAngles(vUp, vForward, fxData.m_vAngles);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_close") : ("portal_1_close")), fxData.m_vOrigin,
								   fxData.m_vAngles);
		...
		case PORTAL_FIZZLE_CLEANSER:
			...
			VectorAngles(vUp, vForward, fxData.m_vAngles);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_cleanser") : ("portal_1_cleanser")), fxData.m_vOrigin,
								   fxData.m_vAngles, this);
			ep.m_pSoundName = "Portal.fizzle_invalid_surface";
			break;
		...
		case PORTAL_FIZZLE_NEAR_BLUE:
		{
			...
			AngleVectors(fxData.m_vAngles, &vForward, &vUp, NULL);
			VectorAngles(vUp, vForward, fxData.m_vAngles);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_near") : ("portal_1_near")), fxData.m_vOrigin,
								   fxData.m_vAngles);
			ep.m_pSoundName = "Portal.fizzle_invalid_surface";
			break;
		}
		...
		case PORTAL_FIZZLE_SUCCESS:
			VectorAngles(vUp, vForward, fxData.m_vAngles);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_success") : ("portal_1_success")), fxData.m_vOrigin,
								   fxData.m_vAngles);
			// Don't make a sound!
			return;
		...
		if(pSpeaker)
		{
			...
			if(m_hLinkedPortal.Get())
			{
				...
				if(pRemotePortal->m_hMicrophone)
				{
					...
					CEnvMicrophone *pRemotePortalMic =
						dynamic_cast<CEnvMicrophone *>(pRemotePortal->m_hMicrophone.Get());
					if(pRemotePortalMic)
					{
			...
			for(touchlink_t *link = root->nextLink; link != root; link = link->nextLink)
			{
				...
				if(CProp_Portal_Shared::IsEntityTeleportable(pOther))
				{
					...
					if(m_plane_Origin.normal.Dot(ptOtherCenter) > m_plane_Origin.dist)
					{
						...
						{
							Assert(((m_PortalSimulator.GetLinkedPortalSimulator() == NULL) &&
									(m_hLinkedPortal.Get() == NULL)) ||
								   (m_PortalSimulator.GetLinkedPortalSimulator() ==
									&m_hLinkedPortal->m_PortalSimulator)); // make sure this entity is linked to the
																		   // same portal as our simulator
...
bool CProp_Portal::ShouldTeleportTouchingEntity(CBaseEntity *pOther)
{
	if(!m_PortalSimulator.OwnsEntity(pOther)) // can't teleport an entity we don't own
	{
		...
		if(!IsMarkedForDeletion())
		{
			ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
							  UTIL_VarArgs("Portal %i not teleporting %s because it's not simulated by this portal.\n",
	...
	if(m_hLinkedPortal.Get() == NULL)
	{
		...
		{
			ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
							  UTIL_VarArgs("Portal %i not teleporting %s because it has no linked partner portal.\n",
										   ((m_bIsPortal2) ? (2) : (1)), pOther->GetDebugName()));
		}
#endif
		if(sv_portal_debug_touch.GetBool())
	...
	// grab current velocity
	{
		...
		else
		{
			...
			if(vOtherVelocity == vec3_origin)
			{
				// Recorded velocity is sometimes zero under pushed or teleported movement, or after position
				// correction. In these circumstances, we want implicit velocity ((last pos - this pos) / timestep )
				if(pOtherPhysObject)
				{
	...
	if(m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot(ptOtherCenter) <
	   m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Dist)
	{
		...
		else
		{
			...
			{
				ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
								  UTIL_VarArgs("Portal %i not teleporting %s because it was not in the portal hole.\n",
											   ((m_bIsPortal2) ? (2) : (1)), pOther->GetDebugName()));
			}
#endif
			if(sv_portal_debug_touch.GetBool())
...
void CProp_Portal::TeleportTouchingEntity(CBaseEntity *pOther)
{
	...
	if(bPlayer)
	{
		...
		bNonPhysical = true;
		// if( (fabs( RemotePortalDataAccess.Placement.vForward.z ) + fabs( LocalPortalDataAccess.Placement.vForward.z
		// )) > 0.7071f ) //some combination of floor/ceiling
		if(fabs(LocalPortalDataAccess.Placement.vForward.z) > 0.0f)
		{
			...
			if((fabs(fAbsLocalZ - 1.0f) < 0.01f) && (fabs(fAbsRemoteZ - 1.0f) < 0.01f))
	...
	// apply transforms to relevant variables (applied to the entity later)
	{
		...
		// Reorient object angles, originally we did a transformation on the angles, but that doesn't quite work right
		// for gimbal lock cases
	...
	if(bPlayer)
	{
		...
		// Sometimes reorienting by pitch is more desirable than by roll depending on the portals' orientations and the
		// relative player facing direction
		...
		else if((LocalPortalDataAccess.Placement.vForward.z > 0.99f ||
				 LocalPortalDataAccess.Placement.vForward.z < -0.99f) && // entering floor or ceiling portal
				(RemotePortalDataAccess.Placement.vForward.z > 0.99f ||
				 RemotePortalDataAccess.Placement.vForward.z < -0.99f) && // exiting floor or ceiling portal
				(fPlayerForwardZ < -0.5f || fPlayerForwardZ > 0.5f)		  // facing mustly up or down
		)
		{
			pOtherAsPlayer->m_bPitchReorientation = true;
		}
		else if((RemotePortalDataAccess.Placement.vForward.z > 0.75f &&
				 RemotePortalDataAccess.Placement.vForward.z <= 0.99f) && // exiting wedge portal
	...
	// do the actual teleportation
	{
		...
		else
		{
			...
			{
				// doing velocity in two stages as a bug workaround, setting the velocity to anything other than 0 will
				// screw up how objects rest on this entity in the future
	...
	else if(bPlayer)
	{
		...
		// we haven't found a good way of fixing the problem of "how do you reorient an AABB". So we just move the
		// player so that they fit m_hLinkedPortal->ForceEntityToFitInPortalWall( pOtherAsPlayer );
	}
	...
	// force the entity to be touching the other portal right this millisecond
	{
		...
		// UTIL_TraceEntity( pOther, ptNewOrigin, ptNewOrigin, MASK_SOLID, pOther, COLLISION_GROUP_NONE, &Trace );
		// //fires off some asserts, and we just need a dummy anyways
...
#ifdef _DEBUG
	{
		...
		fNewDist = RemotePortalDataAccess.Placement.PortalPlane.m_Normal.Dot(ptTestCenter) -
				   RemotePortalDataAccess.Placement.PortalPlane.m_Dist;
		fOldDist = LocalPortalDataAccess.Placement.PortalPlane.m_Normal.Dot(ptOtherCenter) -
				   LocalPortalDataAccess.Placement.PortalPlane.m_Dist;
...
void CProp_Portal::Touch(CBaseEntity *pOther)
{
	...
	Assert(
		((m_PortalSimulator.GetLinkedPortalSimulator() == NULL) && (m_hLinkedPortal.Get() == NULL)) ||
		(m_PortalSimulator.GetLinkedPortalSimulator() ==
		 &m_hLinkedPortal->m_PortalSimulator)); // make sure this entity is linked to the same portal as our simulator
	...
	if(vVelocityCheck != vec3_origin || vAngularImpulseCheck != vec3_origin)
	{
		if(modelinfo->GetModelType(pOther->GetModel()) == mod_brush)
		{
			   ...
			   !FClassnameIs(pOther, "simple_physics_brush")) // except CPhysBox
			{
				...
				if(UTIL_IsBoxIntersectingPortal((vMin + vMax) / 2.0f, (vMax - vMin) / 2.0f - Vector(2.0f, 2.0f, 2.0f),
												this, 0.0f))
				{
				...
				else
				{
					...
					UTIL_TraceLine(vOrigin, vOrigin - vForward * PORTAL_HALF_DEPTH, MASK_SOLID_BRUSHONLY, NULL,
	...
	// see if we should even be interacting with this object, this is a bugfix where some objects get added to physics
	// environments through walls
	if(!m_PortalSimulator.OwnsEntity(pOther))
	{
		// hmm, not in our environment, plane tests, sharing tests
		if(SharedEnvironmentCheck(pOther))
		{
			bool bObjectCenterInFrontOfPortal =
				(m_plane_Origin.normal.Dot(pOther->WorldSpaceCenter()) > m_plane_Origin.dist);
			bool bIsStuckPlayer = (pOther->IsPlayer())
									  ? (!UTIL_IsSpaceEmpty(pOther, pOther->WorldAlignMins(), pOther->WorldAlignMaxs()))
									  : (false);

			if(bIsStuckPlayer)
			{
				Assert(!"Player stuck");
				DevMsg("Player in solid behind behind portal %i's plane, Adding to it's environment to run find "
					   "closest passable space.\n",
					   ((m_bIsPortal2) ? (2) : (1)));
			}
			...
			if(bObjectCenterInFrontOfPortal || bIsStuckPlayer)
			{
				...
				{
					ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
									  UTIL_VarArgs("Portal %i took control of shared object: %s\n",
												   ((m_bIsPortal2) ? (2) : (1)), pOther->GetClassname()));
				}
#endif
...
void CProp_Portal::StartTouch(CBaseEntity *pOther)
{
	...
	{
		ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
						  UTIL_VarArgs("Portal %i StartTouch: %s : %f\n", ((m_bIsPortal2) ? (2) : (1)),
									   pOther->GetClassname(), gpGlobals->curtime));
	}
#endif
		...
		if(m_plane_Origin.normal.Dot(ptOtherCenter) > m_plane_Origin.dist)
		{
			...
			if(SharedEnvironmentCheck(pOther))
			{
				Assert(
					...
					(m_PortalSimulator.GetLinkedPortalSimulator() ==
					 &m_hLinkedPortal
						  ->m_PortalSimulator)); // make sure this entity is linked to the same portal as our simulator
...
void CProp_Portal::EndTouch(CBaseEntity *pOther)
{
	...
	if(ShouldTeleportTouchingEntity(pOther)) // an object passed through the plane and all the way out of the touch box
		TeleportTouchingEntity(pOther);
			...
			(m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot(pOther->WorldSpaceCenter()) <
			 m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Dist) && // but behind the portal plane
			(((CPortal_Player *)pOther)->m_Local.m_bInDuckJump))			 // while ducking
	{
		// player has pulled their feet up (moving their center instantaneously) while falling downward out of the
		// portal, send them back (probably only for a frame)
	...
	{
		ADD_DEBUG_HISTORY(HISTORY_PLAYER_DAMAGE,
						  UTIL_VarArgs("Portal %i EndTouch: %s : %f\n", ((m_bIsPortal2) ? (2) : (1)),
									   pOther->GetClassname(), gpGlobals->curtime));
	}
#endif
}
...
bool CProp_Portal::SharedEnvironmentCheck(CBaseEntity *pEntity)
{
	...
	if((ptCenter - m_PortalSimulator.m_DataAccess.Placement.ptCenter).LengthSqr() <
	   (ptCenter - pOwningSimulator->m_DataAccess.Placement.ptCenter).LengthSqr())
		return true;
	...
	/*if( !m_hLinkedPortal->m_PortalSimulator.EntityIsInPortalHole( pEntity ) )
	{
		...
		if( vOtherVelocity.Dot( m_PortalSimulator.m_DataAccess.Placement.vForward ) < vOtherVelocity.Dot(
	m_hLinkedPortal->m_PortalSimulator.m_DataAccess.Placement.vForward ) ) return true; //entity is going towards this
	portal more than the other
	}*/
	return false;

	// we're in the shared configuration, and the other portal already owns the object, see if we'd be a better
	// caretaker (distance check
...
void CProp_Portal::WakeNearbyEntities(void)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		if(pEntity && (pEntity != this))
		{
			...
			// double check intersection at the OBB vs OBB level, we don't want to affect large piles of physics objects
			// if we don't have to. It gets slow
			if(IsOBBIntersectingOBB(ptOrigin, qAngles, CProp_Portal_Shared::vLocalMins, CProp_Portal_Shared::vLocalMaxs,
									ptEntityCenter, pEntCollision->GetCollisionAngles(), pEntCollision->OBBMins(),
									pEntCollision->OBBMaxs()))
			{
...
void CProp_Portal::ForceEntityToFitInPortalWall(CBaseEntity *pEntity)
{
	...
	Vector ptProjectedEntityCenter =
		ptPortalCenter + (vPortalForward * vPortalCenterToEntityCenter.Dot(vPortalForward));
	...
	if(m_PortalSimulator.IsReadyToSimulate())
	{
		...
		if(m_PortalSimulator.m_DataAccess.Simulation.Static.Wall.Local.Brushes.pCollideable)
		{
			physcollision->TraceBox(ray,
									m_PortalSimulator.m_DataAccess.Simulation.Static.Wall.Local.Brushes.pCollideable,
									vec3_origin, vec3_angle, &ShortestTrace);
		}
		...
		); if( TempTrace.fraction < ShortestTrace.fraction ) ShortestTrace = TempTrace;
		}

		if( pEnvironment->LocalCollide.pWallShellCollide )
		{
			trace_t TempTrace;
			physcollision->TraceBox( ray, pEnvironment->LocalCollide.pWallShellCollide, vec3_origin, vec3_angle,
		&TempTrace ); if( TempTrace.fraction < ShortestTrace.fraction ) ShortestTrace = TempTrace;
		}

		if( pEnvironment->LocalCollide.pRemoteWorldWallCollide )
		{
			trace_t TempTrace;
			physcollision->TraceBox( ray, pEnvironment->LocalCollide.pRemoteWorldWallCollide, vec3_origin, vec3_angle,
		&TempTrace ); if( TempTrace.fraction < ShortestTrace.fraction ) ShortestTrace = TempTrace;
		}
		...
		if(ShortestTrace.fraction < 2.0f)
		{
			...
			if(!IsMarkedForDeletion())
			{
				ADD_DEBUG_HISTORY(
					...
					UTIL_VarArgs("Teleporting %s inside 'ForceEntityToFitInPortalWall'\n", pEntity->GetDebugName()));
			}
#endif
			if(sv_portal_debug_touch.GetBool())
			{
...
void CProp_Portal::UpdatePortalTeleportMatrix(void)
{
	...
	if(m_hLinkedPortal != NULL)
	{
		CProp_Portal_Shared::UpdatePortalTransformationMatrix(
			EntityToWorldTransform(), m_hLinkedPortal->EntityToWorldTransform(), &m_matrixThisToLinked);
	...
	if(m_bActivated)
	{
		...
		if(pLink != NULL)
		{
			...
			if(m_hMicrophone == 0)
			{
				...
				pMicrophone->AddSpawnFlags(SF_MICROPHONE_SOUND_COMBAT | SF_MICROPHONE_SOUND_WORLD |
										   SF_MICROPHONE_SOUND_PLAYER | SF_MICROPHONE_SOUND_BULLET_IMPACT |
										   SF_MICROPHONE_SOUND_EXPLOSION);
			...
			if(m_hLinkedPortal->m_hMicrophone == 0)
			{
				...
				pLinkedMicrophone->AddSpawnFlags(SF_MICROPHONE_SOUND_COMBAT | SF_MICROPHONE_SOUND_WORLD |
												 SF_MICROPHONE_SOUND_PLAYER | SF_MICROPHONE_SOUND_BULLET_IMPACT |
												 SF_MICROPHONE_SOUND_EXPLOSION);
...
void CProp_Portal::PlacePortal(const Vector &vOrigin, const QAngle &qAngles, float fPlacementSuccess,
							   bool bDelay /*= false*/)
{
	Vector vOldOrigin = GetLocalOrigin();
	QAngle qOldAngles = GetLocalAngles();
...
void CProp_Portal::InputSetActivatedState(inputdata_t &inputdata)
{
	...
	if(m_bActivated)
	{
		...
		if(fPlacementSuccess)
		{
			...
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_particles") : ("portal_1_particles")),
								   PATTACH_POINT_FOLLOW, this, "particles_2", true);
			DispatchParticleEffect(((m_bIsPortal2) ? ("portal_2_edge") : ("portal_1_edge")), PATTACH_POINT_FOLLOW, this,
								   "particlespin");
...
CProp_Portal *CProp_Portal::FindPortal(unsigned char iLinkageGroupID, bool bPortal2,
									   bool bCreateIfNothingFound /*= false*/)
{
