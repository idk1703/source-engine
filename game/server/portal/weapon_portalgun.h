...
public:
	unsigned char m_iPortalLinkageGroupID; // which portal linkage group this gun is tied to, usually set by mapper, or
										   // inherited from owning player's index
...
public:
	...
	void DoEffectBlast(bool bPortal2, int iPlacedBy, const Vector &ptStart, const Vector &ptFinalPos,
	...
	void SetLastFiredPortal(int iLastFiredPortal)
	{
		m_iLastFiredPortal = iLastFiredPortal;
	}
	...
	float TraceFirePortal(bool bPortal2, const Vector &vTraceStart, const Vector &vDirection, trace_t &tr,
						  Vector &vFinalPosition, QAngle &qFinalAngles, int iPlacedBy, bool bTest = false);
	...
	PortalWeaponID GetWeaponID(void) const
	{
