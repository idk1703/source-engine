...
BEGIN_DATADESC(CDinosaurSignal)
	DEFINE_AUTO_ARRAY(m_szSoundName, FIELD_CHARACTER), DEFINE_FIELD(m_flOuterRadius, FIELD_FLOAT),
		DEFINE_FIELD(m_flInnerRadius, FIELD_FLOAT), DEFINE_FIELD(m_nSignalID, FIELD_INTEGER),
END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CDinosaurSignal, DT_DinosaurSignal)
SendPropString(SENDINFO(m_szSoundName)), SendPropFloat(SENDINFO(m_flOuterRadius)),
	SendPropFloat(SENDINFO(m_flInnerRadius)), SendPropInt(SENDINFO(m_nSignalID)),
END_SEND_TABLE
()
...
BEGIN_DATADESC(CPortal_Dinosaur)
	DEFINE_FIELD(m_hDinosaur_Signal, FIELD_EHANDLE), DEFINE_FIELD(m_bAlreadyDiscovered, FIELD_BOOLEAN),
END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CPortal_Dinosaur, DT_PropDinosaur)
SendPropEHandle(SENDINFO(m_hDinosaur_Signal)), SendPropBool(SENDINFO(m_bAlreadyDiscovered)),
END_SEND_TABLE
()
...
void CPortal_Dinosaur::Activate(void)
{
	...
	CBaseAchievement *pTransmissionRecvd =
		dynamic_cast<CBaseAchievement *>(g_AchievementMgrPortal.GetAchievementByName("PORTAL_TRANSMISSION_RECEIVED"));
...
static const radiolocs s_radiolocs[] = {
	{"testchmb_a_00", "UpdateItem.Dinosaur01", 0, {0, 0, 0}, {0, 0, 0}, {-506, -924, 161}, 200, 64},
	{
		"testchmb_a_00",
	...
	{"testchmb_a_03", "UpdateItem.Dinosaur07", 5, {-53.2337, 78.181, 236}, {0, 225, 0}, {304, 0, -96}, 256, 128},
	// new entry
	{"testchmb_a_03",
	 ...
	 165,
	 128},
	{"testchmb_a_04", "UpdateItem.Dinosaur09", 7, {118, -56.6, -38.8}, {0, 180, 0}, {-640, 256, 8}, 512, 128},
	// new entry
	{"testchmb_a_05", "UpdateItem.Dinosaur10", 8, {64, 144, 160}, {0, 270, 0}, {64, 740, 7}, 350, 128},
	{"testchmb_a_06", "UpdateItem.Dinosaur11", 9, {529, 315, 320}, {0, 270, 0}, {608, 128, -184}, 384, 160},
	{"testchmb_a_07", "UpdateItem.Dinosaur12", 10, {192, -1546, 1425}, {0, 113, 0}, {272, -496, 1328}, 432, 88},
	// new entry
	{"testchmb_a_07", "UpdateItem.Dinosaur13", 11, {-144, -768, 256}, {0, 90, 0}, {-192, -384, 176}, 256, 128},
	{
		"testchmb_a_08",
	...
	{"testchmb_a_09", "UpdateItem.Dinosaur15", 13, {634, 1308, 256}, {0, 180, 0}, {386.699, 1792.43, 7}, 548, 64},
	{
		"testchmb_a_10",
...
void CSpawnDinosaurHack::LevelInitPostEntity()
{
	...
	{
#if defined(RADIO_DEBUG_SERVER)
		Msg("Not spawning any Dinosaurs: Player has not beat the game, or failed to get heartbreaker achievement from "
			"mgr\n");
#endif
	...
	for(int i = 0; i < ARRAYSIZE(s_radiolocs); ++i)
	{
		...
		if(V_strcmp(STRING(gpGlobals->mapname), loc.mapname) == 0)
		{
			...
			Msg("Dinosaur pos: %f %f %f, ang: %f %f %f\n", loc.radiopos[0], loc.radiopos[1], loc.radiopos[2],
				loc.radioang[0], loc.radioang[1], loc.radioang[2]);
			Msg("Signal pos: %f %f %f, inner rad: %f, outter rad: %f\n", loc.soundpos[0], loc.soundpos[1],
				loc.soundpos[2], loc.soundinnerrad, loc.soundouterrad);
#endif
...
CPortal_Dinosaur *CSpawnDinosaurHack::SpawnDinosaur(radiolocs &loc)
{
	...
	if(loc.radiopos[0] == 0 && loc.radiopos[1] == 0 && loc.radiopos[2] == 0 && loc.radioang[0] == 0 &&
	   loc.radioang[1] == 0 && loc.radioang[2] == 0)
	{
