...
BEGIN_DATADESC(CPropEnergyBall)
	...
	DEFINE_FIELD(m_hTouchedPortal, FIELD_EHANDLE), DEFINE_FIELD(m_bTouchingPortal1, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bTouchingPortal2, FIELD_BOOLEAN), DEFINE_FIELD(m_vLastKnownDirection, FIELD_VECTOR),
		DEFINE_FIELD(m_fMinLifeAfterPortal, FIELD_FLOAT), DEFINE_FIELD(m_bIsInfiniteLife, FIELD_BOOLEAN),
...
void CPropEnergyBall::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	...
	{
		trace_t tr;
		UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + 60 * preVelocity, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);
...
void CPropEnergyBall::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
										const notify_system_event_params_t &params)
{
	...
	if(eventType == NOTIFY_EVENT_TELEPORT)
	{
		...
		// If an energy ball passes a portal (teleports), add a make sure its life is >=
		// sk_energy_ball_min_life_after_portal
...
ConCommand fire_energy_ball("fire_energy_ball", fire_energy_ball_f, "Fires a test energy ball out of your face",
							FCVAR_CHEAT);