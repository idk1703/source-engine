...
public:
	...
	void InputStartFilling(inputdata_t &inputdata); // start filling with portal liquid, will teleport entities as they
	...
	void InputAddActivatorToTeleportList(
		inputdata_t &inputdata); // add an activator entity to the list of entities to teleport when filling
	void InputRemoveActivatorFromTeleportList(
		inputdata_t &inputdata); // remove an activator entity from the list of entities to teleport when filling
	...
	CNetworkVar(float, m_fFillStartTime); // time started filling with portal liquid, will teleport entities as they
										  // become completely enveloped
	...
	CUtlVector<EHANDLE> m_hLeftToTeleportThisFill; // list of entities that have not yet teleported during this fill,
												   // they get teleported out when fully immersed in the liquid
