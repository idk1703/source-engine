...
()
	...
	DEFINE_FIELD(m_hPlayer, FIELD_EHANDLE), DEFINE_FIELD(m_vecRagdollVelocity, FIELD_VECTOR),
...
SendPropExclude("DT_BaseAnimating", "m_flPlaybackRate"), SendPropExclude("DT_BaseAnimating", "m_nSequence"),
	SendPropExclude("DT_BaseAnimating", "m_nNewSequenceParity"),
	SendPropExclude("DT_BaseAnimating", "m_nResetEventsParity"), SendPropExclude("DT_BaseEntity", "m_angRotation"),
	SendPropExclude("DT_BaseAnimatingOverlay", "overlay_vars"), SendPropExclude("DT_BaseFlex", "m_viewtarget"),
	...
	SendPropEHandle(SENDINFO(m_hRagdoll)), SendPropInt(SENDINFO(m_iSpawnInterpCounter), 4),
	SendPropInt(SENDINFO(m_iPlayerSoundType), 3), SendPropBool(SENDINFO(m_bHeldObjectOnOppositeSideOfPortal)),
	SendPropEHandle(SENDINFO(m_pHeldObjectPortal)), SendPropBool(SENDINFO(m_bPitchReorientation)),
	SendPropEHandle(SENDINFO(m_hPortalEnvironment)), SendPropEHandle(SENDINFO(m_hSurroundingLiquidPortal)),
	SendPropBool(SENDINFO(m_bSuppressingCrosshair)), SendPropExclude("DT_BaseAnimating", "m_flPoseParameter"),
...
()
	...
	DEFINE_FIELD(m_bHeldObjectOnOppositeSideOfPortal, FIELD_BOOLEAN), DEFINE_FIELD(m_pHeldObjectPortal, FIELD_EHANDLE),
	DEFINE_FIELD(m_bIntersectingPortalPlane, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bStuckOnPortalCollisionObject, FIELD_BOOLEAN), DEFINE_FIELD(m_fTimeLastHurt, FIELD_TIME),
	DEFINE_FIELD(m_StatsThisLevel.iNumPortalsPlaced, FIELD_INTEGER),
	DEFINE_FIELD(m_StatsThisLevel.iNumStepsTaken, FIELD_INTEGER),
	DEFINE_FIELD(m_StatsThisLevel.fNumSecondsTaken, FIELD_FLOAT), DEFINE_FIELD(m_fTimeLastNumSecondsUpdate, FIELD_TIME),
	DEFINE_FIELD(m_iNumCamerasDetatched, FIELD_INTEGER), DEFINE_FIELD(m_bPitchReorientation, FIELD_BOOLEAN),
	DEFINE_FIELD(m_bIsRegenerating, FIELD_BOOLEAN), DEFINE_FIELD(m_fNeuroToxinDamageTime, FIELD_TIME),
	DEFINE_FIELD(m_hPortalEnvironment, FIELD_EHANDLE), DEFINE_FIELD(m_flExpressionLoopTime, FIELD_TIME),
	DEFINE_FIELD(m_iszExpressionScene, FIELD_STRING), DEFINE_FIELD(m_hExpressionSceneEnt, FIELD_EHANDLE),
	DEFINE_FIELD(m_vecTotalBulletForce, FIELD_VECTOR), DEFINE_FIELD(m_bSilentDropAndPickup, FIELD_BOOLEAN),
	DEFINE_FIELD(m_hRagdoll, FIELD_EHANDLE), DEFINE_FIELD(m_angEyeAngles, FIELD_VECTOR),
	DEFINE_FIELD(m_iPlayerSoundType, FIELD_INTEGER), DEFINE_FIELD(m_qPrePortalledViewAngles, FIELD_VECTOR),
	...
	DEFINE_FIELD(m_hSurroundingLiquidPortal, FIELD_EHANDLE), DEFINE_FIELD(m_bSuppressingCrosshair, FIELD_BOOLEAN),
...
void CPortal_Player::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
...
bool CPortal_Player::WantsLagCompensationOnEntity(const CBasePlayer *pPlayer, const CUserCmd *pCmd,
	...
	if(bOwnsWeaponAlready == true)
	{
		...
		if(pPickupPortalgun)
		{
			CWeaponPortalgun *pPortalGun =
				static_cast<CWeaponPortalgun *>(Weapon_OwnsThisType(pWeapon->GetClassname()));
...
const Vector &CPortal_Player::WorldSpaceCenter() const
{
	...
	m_vWorldSpaceCenterHolder.z +=
		((IsDucked()) ? (VEC_DUCK_HULL_MAX_SCALED(this).z) : (VEC_HULL_MAX_SCALED(this).z)) * 0.5f;
...
void CPortal_Player::VPhysicsShadowUpdate(IPhysicsObject *pPhysics)
{
	...
	// below is mostly a cut/paste of existing CBasePlayer::VPhysicsShadowUpdate code with some minor tweaks to avoid
	// getting stuck in stuff when in a portal environment
			...
			if(trace.allsolid || trace.startsolid)
			{
				...
				else
				{
					if(!FindClosestPassableSpace(this, newPosition - GetAbsOrigin(), MASK_PLAYERSOLID))
					{
						...
						// Deal 1 damage per frame... this will kill a player very fast, but allow for the above
						// correction to fix some cases
...
void CPortal_Player::PlayerUse(void)
{
		...
		else
		{
			...
			{ // Start controlling the train!
				...
				if(pTrain && !(m_nButtons & IN_JUMP) && (GetFlags() & FL_ONGROUND) &&
				   (pTrain->ObjectCaps() & FCAP_DIRECTIONAL_USE) && pTrain->OnControls(this))
				{
					m_afPhysicsFlags |= PFLAG_DIROVERRIDE;
	...
	if(pUseEntity)
	{
		...
		//	CPortalSimulator *pPortalSimulator = CPortalSimulator::GetSimulatorThatOwnsEntity(
...
void CPortal_Player::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper)
{
	...
	{
		// the idea here is to handle the notion that the player has portalled, but they sent us an angle update before
		// receiving that message. If we don't handle this here, we end up sending back their old angles which makes
		// them hiccup their angles for a frame
...
void CPortal_Player::CreateRagdollEntity(const CTakeDamageInfo &info)
{
	...
	/*
		...
		if ( pRagdoll )
		{
			...
			pRagdoll->InitRagdoll( info.GetDamageForce(), m_nForceBone, info.GetDamagePosition(), pBoneToWorld,
	   pBoneToWorldNext, 0.1f, COLLISION_GROUP_INTERACTIVE_DEBRIS, true ); pRagdoll->SetMoveType( MOVETYPE_VPHYSICS );
	...
	if(pSpot)
	{
			...
			sphere.NextEntity())
		{
			...
			if(ent->IsPlayer() && !(ent->edict() == player))
				ent->TakeDamage(CTakeDamageInfo(GetContainingEntity(INDEXENT(0)), GetContainingEntity(INDEXENT(0)), 300,
												DMG_GENERIC));
		}
		goto ReturnSpot;
	}
			...
			"Displays current level stats for portals placed, steps taken, and seconds taken.")
{
	...
	if(pPlayer)
	{
		...
		CFmtStr msg;
		NDebugOverlay::ScreenText(0.5f, 0.5f,
								  msg.sprintf("Portals Placed: %d\nSteps Taken: %d\nTime: %d:%d",
											  pPlayer->NumPortalsPlaced(), pPlayer->NumStepsTaken(), iMinutes,
											  iSeconds),
