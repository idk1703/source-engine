...
private:
	...
	VMatrix m_matrixShadowTransform; // all cloned coordinates and angles will be run through this matrix before being
									 // applied
	VMatrix m_matrixShadowTransform_Inverse;

	CUtlVector<PhysicsObjectCloneLink_t>
		m_CloneLinks;				   // keeps track of which of our physics objects are linked to the source's objects
...
public:
	...
	// syncs to the source entity in every way possible, assumed sync does some rudimentary tests to see if the object
	// is in sync, and if so, skips the update
	void FullSync(bool bAllowAssumedSync = false);

	// syncs just the physics objects, bPullChanges should be true when this clone should match it's source, false when
	// it should force differences onto the source entity
	...
	inline bool IsUntransformedClone(void) const
	{
		return m_bShadowTransformIsIdentity;
	};
	void SetCloneTransformationMatrix(const matrix3x4_t &matTransform);

	inline bool IsInAssumedSyncState(void) const
	{
		return m_bInAssumedSyncState;
	}
	inline IPhysicsEnvironment *GetOwnerEnvironment(void) const
	{
		return m_pOwnerPhysEnvironment;
	}
	...
	static CPhysicsShadowClone *CreateShadowClone(IPhysicsEnvironment *pInPhysicsEnvironment, EHANDLE hEntToClone,
...
class CTraceFilterTranslateClones : public CTraceFilter // give it another filter, and it'll translate shadow clones
														// into their source entity for tests
{
