...
BEGIN_DATADESC(CFuncPortalDetector)
	...
	DEFINE_FIELD(m_bActive, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_iLinkageGroupID, FIELD_INTEGER, "LinkageGroupID"),
...
void CFuncPortalDetector::OnActivate(void)
{
	...
	if(iPortalCount != 0)
	{
		...
		for(int i = 0; i != iPortalCount; ++i)
		{
			...
			if(pTempPortal->GetLinkageGroup() == m_iLinkageGroupID &&
			   UTIL_IsBoxIntersectingPortal(vBoxCenter, vBoxExtents, pTempPortal))
			{
