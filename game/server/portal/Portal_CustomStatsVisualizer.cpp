...
#ifdef PORTAL_GAMESTATS_VERBOSE // this shouldn't be in release versions, and most loading/saving code is only enabled
								// with this defined
...
public:
	...
	CPortalGameStatsVisualizer(void) : m_fRefreshTimer(0.0f) {};
};
...
void CPortalGameStatsVisualizer::PrepareLevelData(void)
{
	...
	for(int i = m_pCurrentMapStats->m_pPlacements->Count(); --i >= 0;)
	{
		...
		UTIL_TraceLine(PlacementStat.ptPlayerFiredFrom, PlacementStat.ptPlayerFiredFrom + (vDirection * 100000.0f),
					   MASK_SHOT_PORTAL, NULL, &tr);

		Vector vUp(0.0f, 0.0f, 1.0f);
		if((tr.plane.normal.x > -0.001f && tr.plane.normal.x < 0.001f) &&
		   (tr.plane.normal.y > -0.001f && tr.plane.normal.y < 0.001f))
		{
...
void CPortalGameStatsVisualizer::FrameUpdatePostEntityThink()
{
	...
	static const Vector vPlayerBoxMins(-16.0f, -16.0f, 0.0f), vPlayerBoxMaxs(16.0f, 16.0f, 72.0f),
		vPlayerTextOffset(0.0f, 0.0f, 36.0f);
	...
	if(portalstats_visdeaths.GetBool())
	{
		for(int i = m_pCurrentMapStats->m_pDeaths->Count(); --i >= 0;)
		{
			...
			NDebugOverlay::Box(DeathStat.ptPositionOfDeath, vPlayerBoxMins, vPlayerBoxMaxs, 255, 0, 0, 100,
							   PORTALSTATSVISUALIZER_DISPLAYTIME);
			NDebugOverlay::Text(DeathStat.ptPositionOfDeath + vPlayerTextOffset, DeathStat.szAttackerClassName, true,
								PORTALSTATSVISUALIZER_DISPLAYTIME);
		}
	}
	...
	if(portalstats_visjumps.GetBool())
	{
		for(int i = m_pCurrentMapStats->m_pJumps->Count(); --i >= 0;)
		{
			...
			NDebugOverlay::Box(JumpStat.ptPlayerPositionAtJumpStart, vPlayerBoxMins, vPlayerBoxMaxs, 0, 255, 0, 100,
							   PORTALSTATSVISUALIZER_DISPLAYTIME);
			NDebugOverlay::Line(JumpStat.ptPlayerPositionAtJumpStart + vPlayerTextOffset,
								JumpStat.ptPlayerPositionAtJumpStart + vPlayerTextOffset +
									(JumpStat.vPlayerVelocityAtJumpStart),
								0, 255, 0, false, PORTALSTATSVISUALIZER_REFRESHTIME);
		}
	}
	...
	if(portalstats_visusages.GetBool())
	{
		for(int i = m_pCurrentMapStats->m_pUseEvents->Count(); --i >= 0;)
		{
			...
			NDebugOverlay::Box(UseEvent.ptTraceStart, vSmallBoxMins, vSmallBoxMaxs, 0, 0, 255, 100,
							   PORTALSTATSVISUALIZER_DISPLAYTIME);
			NDebugOverlay::Line(UseEvent.ptTraceStart, UseEvent.ptTraceStart + (UseEvent.vTraceDelta * 100.0f), 0, 0,
								255, false, PORTALSTATSVISUALIZER_DISPLAYTIME);
			NDebugOverlay::Text(UseEvent.ptTraceStart, UseEvent.szUseEntityClassName, true,
								PORTALSTATSVISUALIZER_DISPLAYTIME);
		}
	}
	...
	if(portalstats_visstucks.GetBool())
	{
		for(int i = m_pCurrentMapStats->m_pStuckSpots->Count(); --i >= 0;)
		{
			...
			NDebugOverlay::Box(StuckEvent.ptPlayerPosition, vPlayerBoxMins, vPlayerBoxMaxs, 255, 0, 255, 100,
							   PORTALSTATSVISUALIZER_DISPLAYTIME);
		}
	}
	...
	if(portalstats_visplacement.GetBool())
	{
		...
		for(int i = m_pCurrentMapStats->m_pPlacements->Count(); --i >= 0;)
		{
			Portal_Gamestats_LevelStats_t::PortalPlacement_t &PlacementStat =
				m_pCurrentMapStats->m_pPlacements->Element(i);
			...
			NDebugOverlay::BoxAngles(PlacementStat.ptPlacementPosition, CProp_Portal_Shared::vLocalMins,
									 CProp_Portal_Shared::vLocalMaxs, m_PortalPlacementAngles[i], 0, brightnessval,
									 brightnessval, 100, PORTALSTATSVISUALIZER_DISPLAYTIME);
			NDebugOverlay::Box(PlacementStat.ptPlayerFiredFrom, vSmallBoxMins, vSmallBoxMaxs, 0, brightnessval,
							   brightnessval, 100, PORTALSTATSVISUALIZER_DISPLAYTIME);
			NDebugOverlay::Line(PlacementStat.ptPlayerFiredFrom, PlacementStat.ptPlacementPosition, 0, brightnessval,
								brightnessval, false, PORTALSTATSVISUALIZER_DISPLAYTIME);
		}
	}
}
...
static int PortalStats_LoadFile_f_CompletionFunc(
	char const *partial, char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH])
{
...
static void PortalStats_LoadNextFile_f(void)
{
	...
	else
	{
		...
		if(i == s_PortalStatsDataFileList.Count())
		{
			...
			NDebugOverlay::ScreenText(0.3f, 0.5f, "PortalStats_LoadNextFile looping to first file in directory.", 255,
									  255, 255, 255, 2.0f);
			i = 0;
		}
...
static void PortalStats_LoadPrevFile_f(void)
{
	...
	else
	{
		...
		if(i < 0)
		{
			...
			NDebugOverlay::ScreenText(0.3f, 0.5f, "PortalStats_LoadPrevFile looping to last file in directory.", 255,
									  255, 255, 255, 2.0f);
		}
...
// static ConCommand Portal_LoadCustomStatsFile("portal_loadcustomstatsfile", Portal_LoadCustomStatsFile_f, "Load a
// custom stats file for visualizing.", FCVAR_DONTRECORD, Portal_LoadCustomStatsFile_f_CompletionFunc );
static ConCommand PortalStats_LoadFile("portalstats_loadfile", PortalStats_LoadFile_f,
									   "Load a custom stats file for visualizing.", FCVAR_DONTRECORD,
									   PortalStats_LoadFile_f_CompletionFunc);
static ConCommand PortalStats_LoadNextFile("portalstats_loadnextfile", PortalStats_LoadNextFile_f,
										   "Load the next custom stats file for visualizing.", FCVAR_DONTRECORD);
static ConCommand PortalStats_LoadPrevFile("portalstats_loadprevfile", PortalStats_LoadPrevFile_f,
										   "Load the next custom stats file for visualizing.", FCVAR_DONTRECORD);
