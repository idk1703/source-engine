...
const char *pRocketTurretFollowerBoneNames[] = {
	"Root",		  "Base",		   "Arm_1",			"Arm_2",  "Arm_3",	   "Arm_4",		"Rot_LR", "Rot_UD",
	"Gun_casing", "Gun_Barrel_01", "gun_barrel_02", "loader", "missle_01", "missle_02", "panel",
};
...
BEGIN_DATADESC(CNPC_RocketTurret)
	...
	DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN), DEFINE_FIELD(m_vecGoalAngles, FIELD_VECTOR),
		DEFINE_FIELD(m_vecCurrentAngles, FIELD_VECTOR), DEFINE_FIELD(m_bHasSightOfEnemy, FIELD_BOOLEAN),
		DEFINE_FIELD(m_vecAnglesToEnemy, FIELD_VECTOR), DEFINE_FIELD(m_vecDirToEnemy, FIELD_VECTOR),
		DEFINE_FIELD(m_flDistToEnemy, FIELD_FLOAT), DEFINE_FIELD(m_flTimeSpentDying, FIELD_FLOAT),
		DEFINE_FIELD(m_flTimeLocking, FIELD_FLOAT), DEFINE_FIELD(m_flTimeLastFired, FIELD_FLOAT),
		DEFINE_FIELD(m_iLaserState, FIELD_INTEGER), DEFINE_FIELD(m_nSiteHalo, FIELD_INTEGER),
		DEFINE_FIELD(m_flTimeSpentPaused, FIELD_FLOAT), DEFINE_FIELD(m_flPauseLength, FIELD_FLOAT),
		DEFINE_FIELD(m_flTotalDivergenceX, FIELD_FLOAT), DEFINE_FIELD(m_flTotalDivergenceY, FIELD_FLOAT),
		DEFINE_FIELD(m_iPosePitch, FIELD_INTEGER), DEFINE_FIELD(m_iPoseYaw, FIELD_INTEGER),
		DEFINE_FIELD(m_hCurRocket, FIELD_EHANDLE), DEFINE_FIELD(m_iMuzzleAttachment, FIELD_INTEGER),
		DEFINE_FIELD(m_iLightAttachment, FIELD_INTEGER), DEFINE_FIELD(m_muzzleToWorldTick, FIELD_INTEGER),
		DEFINE_FIELD(m_muzzleToWorld, FIELD_MATRIX3X4_WORLDSPACE),
		//	DEFINE_FIELD( m_filterBeams, CTraceFilterSkipTwoEntities ),

		DEFINE_EMBEDDED(m_BoneFollowerManager),

		DEFINE_THINKFUNC(SearchThink), DEFINE_THINKFUNC(FollowThink), DEFINE_THINKFUNC(LockingThink),
		DEFINE_THINKFUNC(FiringThink), DEFINE_THINKFUNC(DyingThink), DEFINE_THINKFUNC(DeathThink),
		DEFINE_THINKFUNC(OpeningThink), DEFINE_THINKFUNC(ClosingThink),
		...
		DEFINE_OUTPUT(m_OnFoundTarget, "OnFoundTarget"), DEFINE_OUTPUT(m_OnLostTarget, "OnLostTarget"),

END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CNPC_RocketTurret, DT_NPC_RocketTurret)

SendPropInt(SENDINFO(m_iLaserState), 2), SendPropInt(SENDINFO(m_nSiteHalo)),
...
BEGIN_DATADESC(CRocket_Turret_Projectile)
	...
	DEFINE_FIELD(m_hLauncher, FIELD_EHANDLE),
		...
		DEFINE_FUNCTION(MissileTouch), DEFINE_SOUNDPATCH(m_pAmbientSound),
...
// Constructor
//-----------------------------------------------------------------------------
CNPC_RocketTurret::CNPC_RocketTurret(void) : m_filterBeams(NULL, NULL, COLLISION_GROUP_DEBRIS)
{
	m_bEnabled = false;
...
{
	m_BoneFollowerManager.InitBoneFollowers(this, ARRAYSIZE(pRocketTurretFollowerBoneNames),
											pRocketTurretFollowerBoneNames);
...
void CNPC_RocketTurret::UpdateAimPoint(void)
{
	...
	Vector vecMidEnemy =
		GetEnemy()->GetAbsOrigin() + (GetEnemy()->WorldAlignMins() + GetEnemy()->WorldAlignMaxs()) * 0.5f;
...
void CNPC_RocketTurret::SearchThink()
{
	...
	if(!m_bHasSightOfEnemy)
	{
		if(m_flTimeSpentPaused >= m_flPauseLength)
		{
			...
			if(fabs(m_flTotalDivergenceX) <= MAX_DIVERGENCE_X || SignDiffers(m_flTotalDivergenceX, flOffsetX))
			{
				m_flTotalDivergenceX += flOffsetX;
				m_vecGoalAngles.x += flOffsetX;
			}

			if(fabs(m_flTotalDivergenceY) <= MAX_DIVERGENCE_Y || SignDiffers(m_flTotalDivergenceY, flOffsetY))
			{
...
void CNPC_RocketTurret::FireRocket(void)
{
	...
	CRocket_Turret_Projectile *pRocket = (CRocket_Turret_Projectile *)CBaseEntity::Create(
		"rocket_turret_projectile", EyePosition(), m_vecCurrentAngles, this);
...
void CNPC_RocketTurret::SyncPoseToAimAngles(void)
{
	...
	// Update yaw -- NOTE: This yaw movement is screwy for this model, we must invert the yaw delta and also skew an
	// extra 90 deg to get the 'forward face' of the turret to match up with the look direction. If the model and it's
	// pose parameters change, this will be wrong.
...
//-----------------------------------------------------------------------------
// Purpose: Tests if this prop's front point will have direct line of sight to it's target entity once the pose
// parameters are set to face it Input  : vAimPoint - The point to aim at Output : Returns true if target is in direct
// line of sight, false otherwise.
//-----------------------------------------------------------------------------
...
//			bConsiderNonPortalAimPoint - Output in pOutVec the non portal (direct) aimpoint if it requires the least
//rotation
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
...
bool CNPC_RocketTurret::FindAimPointThroughPortal(const CProp_Portal *pPortal, Vector *pVecOut)
{
	if(pPortal && pPortal->m_bActivated)
	{
		...
		if(fDot < 0.0f && pLinked && pLinked->m_bActivated && pTarget)
		{
			...
			Vector vTargetAimPoint =
				pTarget->GetAbsOrigin() + (pTarget->WorldAlignMins() + pTarget->WorldAlignMaxs()) * 0.5f;
...
void CRocket_Turret_Projectile::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
												  const notify_system_event_params_t &params)
{
...
void CRocket_Turret_Projectile::DoExplosion(void)
{
	...
	while((pTurretIter = gEntList.FindEntityByClassnameWithin(pTurretIter, "npc_portal_turret_floor", GetAbsOrigin(),
															  128)) != NULL)
	{
...
static void fire_rocket_projectile_f(void)
{
	...
	CRocket_Turret_Projectile *pRocket =
		(CRocket_Turret_Projectile *)CBaseEntity::Create("rocket_turret_projectile", ptEyes, vLookAng, pPlayer);
...
ConCommand fire_rocket_projectile("fire_rocket_projectile", fire_rocket_projectile_f,
								  "Fires a rocket turret projectile from the player's eyes for testing.", FCVAR_CHEAT);