...
BEGIN_DATADESC(CNPC_SecurityCamera)
	...
	DEFINE_ARRAY(m_hRopes, FIELD_EHANDLE, SECURITY_CAMERA_NUM_ROPES), DEFINE_FIELD(m_hEyeGlow, FIELD_EHANDLE),
		...
		DEFINE_FIELD(m_bAutoStart, FIELD_BOOLEAN), DEFINE_FIELD(m_bActive, FIELD_BOOLEAN),
		DEFINE_FIELD(m_bBlinkState, FIELD_BOOLEAN), DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
		DEFINE_FIELD(m_flLastSight, FIELD_TIME), DEFINE_FIELD(m_flPingTime, FIELD_TIME),
		DEFINE_FIELD(m_vecGoalAngles, FIELD_VECTOR), DEFINE_FIELD(m_vecCurrentAngles, FIELD_VECTOR),
		DEFINE_FIELD(m_vNoisePos, FIELD_VECTOR), DEFINE_FIELD(m_iTicksTillNextNoise, FIELD_INTEGER),

		DEFINE_SOUNDPATCH(m_pMovementSound),

		DEFINE_THINKFUNC(Retire), DEFINE_THINKFUNC(Deploy), DEFINE_THINKFUNC(ActiveThink),
		DEFINE_THINKFUNC(SearchThink), DEFINE_THINKFUNC(DeathThink),

		// Inputs
		DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle), DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
		DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable), DEFINE_INPUTFUNC(FIELD_VOID, "Ragdoll", InputRagdoll),
...
void CNPC_SecurityCamera::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType,
											const notify_system_event_params_t &params)
{
...
void CNPC_SecurityCamera::InputRagdoll(inputdata_t &inputdata)
{
	...
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin() + 10.0f * vForward, GetAbsOrigin() - 60.0f * vForward, MASK_SHOT, this,
...
void PlayDismountSounds()
{
	...
	if(!pPlayer || !pGlaDOS)
	{
		DevMsg(2, "Could not play CNPC_SecurityCamera dismount scene, make sure actor named 'Aperture_AI' is present "
				  "in map.\n");
		return;
	}
	...
	// Should we play the dismount sound after she's done? or is that too disjointed from the camera dismounting act to
	// make sense...
