...
int CPortal_CollisionEvent::ShouldCollide(IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0,
										  void *pGameData1)
{
	...
	if(bShadowClonesInvolved)
	{
		...
		if((pObj0->GetGameFlags() | pObj1->GetGameFlags()) & FVPHYSICS_PLAYER_HELD)
		{
			...
			if(pGameData0 && ((CBaseEntity *)pGameData0)->IsPlayer() &&
			   (GetPlayerHeldEntity((CBasePlayer *)pGameData0) == (CBaseEntity *)pGameData1))
				return 0;

			if(pGameData1 && ((CBaseEntity *)pGameData1)->IsPlayer() &&
			   (GetPlayerHeldEntity((CBasePlayer *)pGameData1) == (CBaseEntity *)pGameData0))
				return 0;
		}
	}
	...
	if(pGameData0 != pGameData1)
	{
		// this code only decides what CAN'T collide due to portal environment differences, things that should collide
		// will pass through here to deeper ShouldCollide() code
		...
		AssertOnce((bStatic[0] && bStatic[1]) ==
				   false); // hopefully the system doesn't even call in for this, they're both static and can't collide
		...
		for(int i = 0; i != 2; ++i)
		{
			if((pSimulators[i] != NULL) && CPhysicsShadowClone::IsShadowClone(pEntities[i]))
			{
				...
				Assert((pSimulators[i]->m_DataAccess.Simulation.Dynamic.EntFlags[pClone->entindex()] &
						PSEF_IS_IN_PORTAL_HOLE) ==
					   (pSourceSimulator->m_DataAccess.Simulation.Dynamic.EntFlags[pSource->entindex()] &
						PSEF_IS_IN_PORTAL_HOLE));
			}
		}
#endif
						...
						if(bStatic[i])
						{
							if(CPSCollisionEntity::IsPortalSimulatorCollisionEntity(pEntities[i]))
							{
								...
								if(pSimulators[i]->CreatedPhysicsObject(pPhysObjects[i], &objectSource) &&
								   ((objectSource == PSPOST_REMOTE_BRUSHES) ||
									(objectSource == PSPOST_REMOTE_STATICPROPS)))
								{
									if((pSimulators[1 - i]
											->m_DataAccess.Simulation.Dynamic.EntFlags[pEntities[1 - i]->entindex()] &
										PSEF_IS_IN_PORTAL_HOLE) == 0)
										return 0; // require that the entity be in the portal hole before colliding with
												  // transformed geometry
									// FIXME: The above requirement might fail horribly for transformed collision
									// blocking the portal from the other side and fast moving objects
								}
							}
							break;
						}
		...
		else // different simulators
		{
			...
			if(bStatic[0] || bStatic[1])
			{
				for(int i = 0; i != 2; ++i)
				{
					if(bStatic[i])
					{
						...
						{
							CPortalSimulator *pSimulator_Static =
								CPortalSimulator::GetSimulatorThatCreatedPhysicsObject(
									pPhysObjects[i]); // might have been a static prop which would yield a new simulator
			...
			else
			{
				...
				for(int i = 0; i != 2; ++i)
				{
					if(pSimulators[i])
					{
						// entities in the physics environment only collide with statics created by the environment
						// (handled above), entities in the same environment (also above), or entities that should be
						// cloned from main to the same environment
						if((pSimulators[i]->m_DataAccess.Simulation.Dynamic.EntFlags[pEntities[1 - i]->entindex()] &
							PSEF_CLONES_ENTITY_FROM_MAIN) == 0) // not cloned from main
							return 0;
					}
				}
			}
...
int CPortal_CollisionEvent::ShouldSolvePenetration(IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0,
												   void *pGameData1, float dt)
{
	...
	if((pObj0->GetGameFlags() | pObj1->GetGameFlags()) & FVPHYSICS_PLAYER_HELD)
	{
		...
		// held objects are clipping into other objects when travelling across a portal. We're close to ship, so this
		// seems to be the most localized way to make a fix. Note that we're not actually going to change whether it
		// should solve, we're just going to tack on some hacks
		...
		if(!pHoldingPlayer && CPhysicsShadowClone::IsShadowClone(pHeld))
			pHoldingPlayer =
				(CPortal_Player *)GetPlayerHoldingEntity(((CPhysicsShadowClone *)pHeld)->GetClonedEntity());
	...
	if((pObj0->GetGameFlags() | pObj1->GetGameFlags()) & FVPHYSICS_IS_SHADOWCLONE)
	{
		...
		if((pObj0->GetGameFlags() & pObj1->GetGameFlags()) &
		   FVPHYSICS_IS_SHADOWCLONE) // don't solve between two shadowclones, they're just going to resync in a frame
									 // anyways
			return 0;
		...
		for(int i = 0; i != 2; ++i)
		{
			if(pObjects[i]->GetGameFlags() & FVPHYSICS_IS_SHADOWCLONE)
			{
				...
				if(((CPhysicsShadowClone *)(pObjects[i]->GetGameData()))->GetClonedEntity() ==
				   (pObjects[j]->GetGameData()))
	...
	for(int i = 0; i != 2; ++i)
	{
		if((pUnshadowedObjects[i] && pUnshadowedObjects[i]->GetGameFlags() & FVPHYSICS_PLAYER_HELD))
		{
			...
			else if(pEvent->pObjects[j]->GetGameFlags() & FVPHYSICS_IS_SHADOWCLONE)
			{
				...
				GetSavedParamsForCarriedPhysObject(pGrabController, pUnshadowedObjects[i], &fSavedMass,
												   &fSavedRotationalDamping);
...
{
	// this actually happens once per physics environment simulation, and we don't want that, so do nothing and we'll
	// get a different version manually called
}
...
void CPortal_CollisionEvent::AddDamageEvent(CBaseEntity *pEntity, const CTakeDamageInfo &info,
											IPhysicsObject *pInflictorPhysics, bool bRestoreVelocity,
											const Vector &savedVel, const AngularImpulse &savedAngVel)
{
	const CTakeDamageInfo *pPassDownInfo = &info;
	CTakeDamageInfo ReplacementDamageInfo; // only used some of the time

	if((info.GetDamageType() & DMG_CRUSH) && (pInflictorPhysics->GetGameFlags() & FVPHYSICS_IS_SHADOWCLONE) &&
	   (!info.BaseDamageIsValid()) && (info.GetDamageForce().LengthSqr() > (20000.0f * 20000.0f)))
	{
		// VERY likely this was caused by the penetration solver. Since a shadow clone is involved we're going to ignore
		// it becuase it causes more problems than it solves in this case
