...
BEGIN_DATADESC(CEnv_Lightrail_Endpoint)
	DEFINE_KEYFIELD(m_flSmallScale, FIELD_FLOAT, "small_fx_scale"),
		DEFINE_KEYFIELD(m_flLargeScale, FIELD_FLOAT, "large_fx_scale"), DEFINE_FIELD(m_nState, FIELD_INTEGER),
...
SendPropFloat(SENDINFO(m_flSmallScale), 0, SPROP_NOSCALE), SendPropFloat(SENDINFO(m_flLargeScale), 0, SPROP_NOSCALE),
	SendPropInt(SENDINFO(m_nState), 8, SPROP_UNSIGNED), SendPropFloat(SENDINFO(m_flDuration), 0, SPROP_NOSCALE),
	SendPropFloat(SENDINFO(m_flStartTime), 0, SPROP_NOSCALE), SendPropInt(SENDINFO(m_spawnflags), 0, SPROP_UNSIGNED),
END_SEND_TABLE
()
