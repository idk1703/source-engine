...
ConVar sv_use_shadow_clones("sv_use_shadow_clones", "1",
							FCVAR_REPLICATED | FCVAR_CHEAT); // should we create shadow clones?
...
void CPhysicsShadowClone::FullSync(bool bAllowAssumedSync)
{
	...
	{
		AssertMsg(VPhysicsGetObject() != NULL,
				  "Been linkless for more than this update, something should have killed this clone.");
	...
	if(bAllowAssumedSync)
	{
		...
		if(bIsSynced)
		{
			...
			if(!m_bInAssumedSyncState)
			{
				...
				{
					if((m_CloneLinks[i].pSource->GetShadowController() == NULL) &&
					   m_CloneLinks[i].pClone->IsMotionEnabled())
					{
	...
	if(bBigChanges)
	{
		...
		if((sourceMoveType == MOVETYPE_CUSTOM) || (sourceMoveType == MOVETYPE_STEP) ||
		   (sourceMoveType == MOVETYPE_WALK) ||
		   (pPhysObject &&
			((pPhysObject->GetGameFlags() & FVPHYSICS_PLAYER_HELD) || (pPhysObject->GetShadowController() != NULL))))
		{
			//#ifdef _DEBUG
			...
			SetMoveType(MOVETYPE_VPHYSICS,
						pClonedEntity->GetMoveCollide()); // either an unclonable movetype, or a shadow/held object
		}
...
static void FullSyncPhysicsObject(IPhysicsObject *pSource, IPhysicsObject *pDest, const VMatrix *pTransform,
								  bool bTeleport)
{
	...
	float
		fSavedMass = 0.0f,
		fSavedRotationalDamping; // setting mass to 0.0f purely to kill a warning that I can't seem to kill with pragmas
	...
	// stuff that we really care about
	{
		...
		// avoid oversetting variables (I think that even setting them to the same value they already are disrupts the
		// delicate physics balance)
		...
		else
		{
			...
			if(pDestController == NULL)
			{
				...
				pDest->SetShadow(fMaxSpeed, fMaxAngularSpeed, pSourceController->AllowsTranslation(),
								 pSourceController->AllowsRotation());
...
void CPhysicsShadowClone::FullSyncClonedPhysicsObjects(bool bTeleport)
{
	...
	// copy the existing list of clone links to a temp array, we're going to be starting from scratch and copying links
	// as we need them
	...
	{
		pExistingLinks =
			(PhysicsObjectCloneLink_t *)stackalloc(sizeof(PhysicsObjectCloneLink_t) * m_CloneLinks.Count());
	...
	for(i = 0; i != iObjectCount; ++i)
	{
		...
		if(j != iExistingLinkCount)
		{
			...
			memset(&pExistingLinks[j], 0,
				   sizeof(PhysicsObjectCloneLink_t)); // zero out this slot so we don't destroy it in cleanup
		}
		else
		{
			...
			// apparently some collision code gets called on creation before we've set extra game flags, so we're going
			// to cheat a bit and temporarily set our extra flags on the source
			...
			physenv->SerializeObjectToBuffer(pSource, pBuffer,
											 size); // this should work across physics environments because the
													// serializer doesn't write anything about itself to the template
			pSource->SetGameFlags(iOldGameFlags);
			cloneLink.pClone = m_pOwnerPhysEnvironment->UnserializeObjectFromBuffer(
				this, pBuffer, size, false); // unserializer has to be in the target environment
			assert(
				cloneLink
					.pClone); // there should be absolutely no case where we can't clone a valid existing physics object
...
void CPhysicsShadowClone::SetCloneTransformationMatrix(const matrix3x4_t &sourceMatrix)
{
	...
	{
		m_matrixShadowTransform.InverseTR(
			m_matrixShadowTransform_Inverse); // probably not the right matrix, but we're out of options
	}
...
CPhysicsShadowClone *CPhysicsShadowClone::CreateShadowClone(IPhysicsEnvironment *pInPhysicsEnvironment,
															EHANDLE hEntToClone, const char *szDebugMarker,
															const matrix3x4_t *pTransformationMatrix /*= NULL*/)
{
	AssertMsg(szDebugMarker != NULL,
			  "All shadow clones must have a debug marker for where it came from in debug builds.");
	...
	if(pTransformationMatrix)
	{
		...
		if(!pClone->m_bShadowTransformIsIdentity)
		{
			...
			{
				pClone->m_matrixShadowTransform.InverseTR(
					pClone->m_matrixShadowTransform_Inverse); // probably not the right matrix, but we're out of options
			}
		}
	}
...
{
	// the baseclass just screenshakes, makes sounds, and outputs dust, we rely on the original entity to do this when
	// applicable
}
