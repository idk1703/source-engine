...
public:
	...
	virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave)
	{
		...
		for(CAI_Enemies::CMemMap::IndexType_t i = pMemMap->FirstInorder(); i != pMemMap->InvalidIndex();
			i = pMemMap->NextInorder(i))
		{
...
BEGIN_SIMPLE_DATADESC(CAI_Enemies)
	...
	DEFINE_CUSTOM_FIELD(m_Map, &g_AI_MemoryListSaveRestoreOps), DEFINE_FIELD(m_flFreeKnowledgeDuration, FIELD_FLOAT),
		DEFINE_FIELD(m_flEnemyDiscardTime, FIELD_FLOAT), DEFINE_FIELD(m_vecDefaultLKP, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_vecDefaultLSP, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_serial, FIELD_INTEGER),

END_DATADESC()

BEGIN_SIMPLE_DATADESC(AI_EnemyInfo_t)
	DEFINE_FIELD(vLastKnownLocation, FIELD_POSITION_VECTOR), DEFINE_FIELD(vLastSeenLocation, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(hEnemy, FIELD_EHANDLE), DEFINE_FIELD(timeLastSeen, FIELD_TIME),
		DEFINE_FIELD(timeFirstSeen, FIELD_TIME), DEFINE_FIELD(timeLastReacquired, FIELD_TIME),
		DEFINE_FIELD(timeValidEnemy, FIELD_TIME), DEFINE_FIELD(timeLastReceivedDamageFrom, FIELD_TIME),
		DEFINE_FIELD(timeAtFirstHand, FIELD_TIME), DEFINE_FIELD(bDangerMemory, FIELD_BOOLEAN),
		DEFINE_FIELD(bEludedMe, FIELD_BOOLEAN), DEFINE_FIELD(bUnforgettable, FIELD_BOOLEAN),
...
bool CAI_Enemies::ShouldDiscardMemory(AI_EnemyInfo_t *pMemory)
{
	...
	if(!pMemory->bUnforgettable && gpGlobals->curtime > pMemory->timeLastSeen + m_flEnemyDiscardTime)
	{
		return true;
	}
...
bool CAI_Enemies::UpdateMemory(CAI_Network *pAINet, CBaseEntity *pEnemy, const Vector &vPosition, float reactionDelay,
							   bool firstHand)
{
	...
	if(pMemory)
	{
		...
		if(deltaDist > DIST_TRIGGER_REACQUIRE_SQ ||
		   (deltaDist > MIN_DIST_TIME_TRIGGER_REACQUIRE_SQ &&
			(gpGlobals->curtime - pMemory->timeLastSeen) > TIME_TRIGGER_REACQUIRE))
		{
	...
	if(firstHand)
	{
		pAddMemory->timeLastReacquired = pAddMemory->timeFirstSeen = pAddMemory->timeLastSeen =
			pAddMemory->timeAtFirstHand = gpGlobals->curtime;
	}
	else
	{
