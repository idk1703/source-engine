...
BEGIN_DATADESC(CBaseDoor)
	...
	DEFINE_KEYFIELD(m_vecMoveDir, FIELD_VECTOR, "movedir"),
		...
		DEFINE_FIELD(m_bLockedSentence, FIELD_CHARACTER), DEFINE_FIELD(m_bUnlockedSentence, FIELD_CHARACTER),
		...
		DEFINE_KEYFIELD(m_ls.sUnlockedSound, FIELD_SOUNDNAME, "unlocked_sound"), DEFINE_FIELD(m_bLocked, FIELD_BOOLEAN),
		DEFINE_KEYFIELD(m_flWaveHeight, FIELD_FLOAT, "WaveHeight"),
		DEFINE_KEYFIELD(m_flBlockDamage, FIELD_FLOAT, "dmg"),
		DEFINE_KEYFIELD(m_eSpawnPosition, FIELD_INTEGER, "spawnpos"),

		DEFINE_KEYFIELD(m_bForceClosed, FIELD_BOOLEAN, "forceclosed"), DEFINE_FIELD(m_bDoorGroup, FIELD_BOOLEAN),

#ifdef HL1_DLL
		DEFINE_KEYFIELD(m_iBlockFilterName, FIELD_STRING, "filtername"), DEFINE_FIELD(m_hBlockFilter, FIELD_EHANDLE),
#endif

		DEFINE_KEYFIELD(m_bLoopMoveSound, FIELD_BOOLEAN, "loopmovesound"),
		DEFINE_KEYFIELD(m_bIgnoreDebris, FIELD_BOOLEAN, "ignoredebris"),

		DEFINE_INPUTFUNC(FIELD_VOID, "Open", InputOpen), DEFINE_INPUTFUNC(FIELD_VOID, "Close", InputClose),
		DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle), DEFINE_INPUTFUNC(FIELD_VOID, "Lock", InputLock),
		DEFINE_INPUTFUNC(FIELD_VOID, "Unlock", InputUnlock), DEFINE_INPUTFUNC(FIELD_FLOAT, "SetSpeed", InputSetSpeed),
		...
		DEFINE_OUTPUT(m_OnOpen, "OnOpen"), DEFINE_OUTPUT(m_OnLockedUse, "OnLockedUse"),

		// Function Pointers
		DEFINE_FUNCTION(DoorTouch), DEFINE_FUNCTION(DoorGoUp), DEFINE_FUNCTION(DoorGoDown), DEFINE_FUNCTION(DoorHitTop),
		DEFINE_FUNCTION(DoorHitBottom), DEFINE_THINKFUNC(MovingSoundThink), DEFINE_THINKFUNC(CloseAreaPortalsThink),
	...
	void PlayLockSounds(CBaseEntity *pEdict, locksound_t *pls, int flocked, int fbutton)
{
	...
	if(flocked)
	{
		...
		int fplaysentence =
			(pls->sLockedSentence != NULL_STRING && !pls->bEOFLocked && gpGlobals->curtime > pls->flwaitSentence);
		...
		if(fplaysentence)
		{
			...
			pls->iLockedSentence = SENTENCEG_PlaySequentialSz(pEdict->edict(), STRING(pls->sLockedSentence), 0.85f,
															  SNDLVL_NORM, 0, 100, pls->iLockedSentence, FALSE);
	...
	else
	{
		...
		int fplaysentence =
			(pls->sUnlockedSentence != NULL_STRING && !pls->bEOFUnlocked && gpGlobals->curtime > pls->flwaitSentence);
		float fvol;
		...
		if(fplaysentence)
		{
			...
			pls->iUnlockedSentence = SENTENCEG_PlaySequentialSz(pEdict->edict(), STRING(pls->sUnlockedSentence), 0.85,
																SNDLVL_NORM, 0, 100, pls->iUnlockedSentence, FALSE);
...
void CBaseDoor::Spawn()
{
	...
	if(!FClassnameIs(this, "func_water"))
	{
		...
		if(HasSpawnFlags(SF_DOOR_NONSOLID_TO_PLAYER))
		{
			...
			// NOTE: We could decouple this as a separate behavior, but managing player collisions is already complex
			// enough. NOTE: This is necessary to prevent the player from blocking the wrecked train car in
			// ep2_outland_01
	...
	if((m_eSpawnPosition == FUNC_DOOR_SPAWN_OPEN) && HasSpawnFlags(SF_DOOR_START_OPEN_OBSOLETE))
	{
		Warning("Door %s using obsolete 'Start Open' spawnflag with 'Spawn Position' set to 'Open'. Reverting to old "
				"behavior.\n",
				GetDebugName());
	}
...
void CBaseDoor::UpdateAreaPortals(bool isOpen)
{
	...
	if(IsRotatingDoor() &&
	   HasSpawnFlags(SF_DOOR_START_OPEN_OBSOLETE)) // logic inverted when using rot doors that start open
		isOpen = !isOpen;
...
void CBaseDoor::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if(HasSpawnFlags(SF_DOOR_NEW_USE_RULES))
	{
		...
		if((m_toggle_state == TS_AT_BOTTOM || m_toggle_state == TS_GOING_DOWN) ||
		   (HasSpawnFlags(SF_DOOR_NO_AUTO_RETURN) && (m_toggle_state == TS_AT_TOP || m_toggle_state == TS_GOING_UP)))
			bAllowUse = true;
	}
	else
	{
...
void CBaseDoor::Blocked(CBaseEntity *pOther)
{
	...
	if(GetEntityName() != NULL_STRING)
	{
		...
		for(int i = 0; i < doorCount; i++)
		{
			...
			if(pDoor->m_flWait >= 0)
			{
				if(m_bDoorGroup && pDoor->m_vecMoveDir == m_vecMoveDir && pDoor->GetAbsVelocity() == GetAbsVelocity() &&
				   pDoor->GetLocalAngularVelocity() == GetLocalAngularVelocity())
				{
					pDoor->m_nSimulationTick =
						m_nSimulationTick; // don't run simulation this frame if you haven't run yet
