...
CAI_GlobalNamespace::CAI_GlobalNamespace() : m_pSymbols(new CStringRegistry), m_NextGlobalBase(GLOBAL_IDS_BASE) {}
...
bool CAI_LocalIdSpace::AddSymbol(const char *pszSymbol, int localId, const char *pszDebugSymbolType,
								 const char *pszDebugOwner)
{
	...
	AssertMsg3(LocalToGlobal(localId) == -1 || !m_pGlobalNamespace->IdToSymbol(LocalToGlobal(localId)),
			   "Duplicate symbol ID passed to CAI_LocalIdSpace::AddSymbol(): %s (%d), had %s", pszSymbol, localId,
			   m_pGlobalNamespace->IdToSymbol(LocalToGlobal(localId)));
	AssertMsg(m_pGlobalNamespace->SymbolToId(pszSymbol) == -1,
			  "Duplicate symbol passed to CAI_LocalIdSpace::AddSymbol()");
...
bool CAI_LocalIdSpace::SetLocalBase(int newBase)
{
	...
	if(m_localBase == MAX_STRING_INDEX)
	{
		...
		m_localBase = newBase;
		if(!m_pParentIDSpace || !m_pParentIDSpace->IsLocalBaseSet() ||
		   m_localBase >
			   m_pParentIDSpace
				   ->m_localBase) // @Note (toml 08-20-02): this doesn't actually catch namespace collisions, only
								  // woefully bad situations where a derived class' offset is less than the base
			return true;
	}
	return false;
}
...
int CAI_LocalIdSpace::GlobalToLocal(int globalID) const
{
	...
	do
	{
		if(pCurrentMap->IsLocalBaseSet() && globalID >= pCurrentMap->GetGlobalBase() &&
		   globalID <= pCurrentMap->GetGlobalTop())
...
int CAI_LocalIdSpace::LocalToGlobal(int localID) const
{
	...
	do
	{
		if(pCurrentMap->IsLocalBaseSet() && localID >= pCurrentMap->GetLocalBase() &&
		   localID <= pCurrentMap->GetLocalTop())
