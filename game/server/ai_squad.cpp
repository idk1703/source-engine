...
BEGIN_SIMPLE_DATADESC(CAI_Squad)
	...
	DEFINE_FIELD(m_flSquadSoundWaitTime, FIELD_TIME), DEFINE_FIELD(m_nSquadSoundPriority, FIELD_INTEGER),
		DEFINE_FIELD(m_hSquadInflictor, FIELD_EHANDLE), DEFINE_AUTO_ARRAY(m_SquadData, FIELD_INTEGER),
	//							m_pLastFoundEnemyInfo  (think transient)

#ifdef PER_ENEMY_SQUADSLOTS
		DEFINE_UTLVECTOR(m_EnemyInfos, FIELD_EMBEDDED), DEFINE_FIELD(m_flEnemyInfoCleanupTime, FIELD_TIME),
#else
...
CAI_BaseNPC *CAI_Squad::NearestSquadMember(CAI_BaseNPC *pMember)
{
	...
	for(int i = 0; i < m_SquadMembers.Count(); i++)
	{
		if(m_SquadMembers[i] != NULL)
		{
			...
			if(m_SquadMembers[i] != pMember && fDist < fBestDist)
			{
				fBestDist = fDist;
...
void CAI_Squad::SquadNewEnemy(CBaseEntity *pEnemy)
{
	...
	for(int i = 0; i < m_SquadMembers.Count(); i++)
	{
		...
		if(pMember)
		{
			// reset members who aren't activly engaged in fighting (only do this if the NPC's using the squad memory,
			// or it'll fail)
			if(!pMember->GetEnemy() || (pMember->GetEnemy() != pEnemy && !pMember->HasCondition(COND_SEE_ENEMY) &&
...
AISquadEnemyInfo_t *CAI_Squad::FindEnemyInfo(CBaseEntity *pEnemy)
{
	...
	if(gpGlobals->curtime > m_flEnemyInfoCleanupTime)
	{
		if(m_EnemyInfos.Count())
		{
			...
			{
				if(m_EnemyInfos[i].hEnemy == NULL ||
				   activeEnemies.Find(m_EnemyInfos[i].hEnemy) == activeEnemies.InvalidIndex())
				{
