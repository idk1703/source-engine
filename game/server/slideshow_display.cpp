...
SendPropBool(SENDINFO(m_bEnabled)), SendPropString(SENDINFO(m_szDisplayText)),
	...
	SendPropArray3(SENDINFO_ARRAY3(m_chCurrentSlideLists),
				   SendPropInt(SENDINFO_ARRAY(m_chCurrentSlideLists), 8, SPROP_UNSIGNED)),
	SendPropFloat(SENDINFO(m_fMinSlideTime), 11, 0, 0.0f, 20.0f),
	SendPropFloat(SENDINFO(m_fMaxSlideTime), 11, 0, 0.0f, 20.0f),
	SendPropInt(SENDINFO(m_iCycleType), 2, SPROP_UNSIGNED), SendPropBool(SENDINFO(m_bNoListRepeats)),
END_SEND_TABLE
()
...
void CSlideshowDisplay::BuildSlideShowImagesList(void)
{
	...
	while(szMatFileName[0])
	{
		...
		Q_snprintf(szFullFileName, sizeof(szFullFileName), "materials/vgui/%s/%s", m_szSlideshowDirectory.Get(),
				   szMatFileName);
