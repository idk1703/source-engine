...
bool CAI_PlaneSolver::MoveLimit(Navigation_t navType, const Vector &target, bool ignoreTransients, bool fCheckStep,
								int contents, AIMoveTrace_t *pMoveTrace)
{
	...
	return pProbe->MoveLimit(navType, GetLocalOrigin(), target, contents, m_pNpc->GetNavTargetEntity(),
							 (fCheckStep) ? 100 : 0, flags, pMoveTrace);
}
...
void CAI_PlaneSolver::GenerateSuggestionFromTrace(const AILocalMoveGoal_t &goal, const AIMoveTrace_t &moveTrace,
												  float probeDist, float arcCenter, float arcSpan, int probeOffset)
{
	...
	suggestion.Set(type, (favorRight) ? favoredWeight : weight, arcCenter - thirdSpan, thirdSpan,
				   moveTrace.pObstruction);

	m_Solver.AddRegulation(suggestion);

	suggestion.Set(type, (favorLeft) ? favoredWeight : weight, arcCenter + thirdSpan, thirdSpan,
				   moveTrace.pObstruction);
...
void CAI_PlaneSolver::CalcYawsFromOffset(float yawScanCenter, float spanPerProbe, int probeOffset, float *pYawTest,
										 float *pYawCenter)
{
	if(probeOffset != 0)
	{
																 ...
																 float probeDist, float spanPerProbe, int probeOffset)
{
	...
	{
		fTraceClear =
			MoveLimit(goal.navType, GetLocalOrigin() + probeDir * probeDist, !ProbeForNpcs(), false, &moveTrace);
	}
...
AI_SuggestorResult_t CAI_PlaneSolver::GenerateObstacleSuggestions(const AILocalMoveGoal_t &goal,
																  const AIMoveTrace_t &directTrace, float distClear,
																  float probeDist, float degreesToProbe, int nProbes)
{
	Assert(nProbes % 2 == 1);
	...
	if(PLANE_SOLVER_THINK_FREQUENCY[AIStrongOpt()] == 0.0 || m_RefreshSamplesTimer.Expired())
	{
		...
		for(i = 1; i <= nSideProbes; i++)
		{
			if(!foundClear)
			{
				AI_SuggestorResult_t curSeekResult =
					GenerateObstacleSuggestion(goal, yawGoalDir, probeDist, spanPerProbe, i);
...
bool CAI_PlaneSolver::RunMoveSolver(const AILocalMoveGoal_t &goal, const AIMoveTrace_t &directTrace,
									float degreesPositiveArc, bool fDeterOscillation, Vector *pResult)
{
...
bool CAI_PlaneSolver::GenerateCircleObstacleSuggestions(const AILocalMoveGoal_t &moveGoal, float probeDist)
{
	...
	for(int i = 0; i < m_Obstacles.Count(); i++)
	{
		...
		if(m_Obstacles[i].hEntity && m_Obstacles[i].hEntity->CollisionProp())
		{
			...
			{
				// HEY! I'm trying to avoid the very thing I'm trying to get to. This will make we wobble like a drunk
				// as I approach. Don't do it.
				continue;
			}
		...
		if(distToObstacleSq > 0.001 && distToObstacleSq < sq(radius + probeDist))
		{
			...
			if(radiusSq < distToObstacleSq)
			{
				...
				NDebugOverlay::Line(pointTangent - Vector(0, 0, 64), pointTangent + Vector(0, 0, 64), 0, 255, 0, false,
									0.1);
...
bool CAI_PlaneSolver::Solve(const AILocalMoveGoal_t &goal, float distClear, Vector *pSolution)
{
	...
	if(GenerateObstacleSuggestions(goal, goal.directTrace, distClear, probeDist, degreesPositiveArc, NUM_PROBES) !=
	   SR_FAIL)
	{
