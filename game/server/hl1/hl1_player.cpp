...
//#define HL1_DMG_SHOWNHUD	(DMG_POISON | DMG_ACID | DMG_FREEZE | DMG_SLOWFREEZE | DMG_DROWN | DMG_BURN | DMG_SLOWBURN |
...
BEGIN_DATADESC(CHL1_Player)
	DEFINE_FIELD(m_nControlClass, FIELD_INTEGER), DEFINE_AUTO_ARRAY(m_vecMissPositions, FIELD_POSITION_VECTOR),
		...
		DEFINE_FIELD(m_bHasLongJump, FIELD_BOOLEAN), DEFINE_FIELD(m_nFlashBattery, FIELD_INTEGER),
		DEFINE_FIELD(m_flFlashLightTime, FIELD_TIME),

		DEFINE_FIELD(m_flStartCharge, FIELD_FLOAT), DEFINE_FIELD(m_flAmmoStartCharge, FIELD_FLOAT),
		DEFINE_FIELD(m_flPlayAftershock, FIELD_FLOAT), DEFINE_FIELD(m_flNextAmmoBurn, FIELD_FLOAT),

		DEFINE_PHYSPTR(m_pPullConstraint), DEFINE_FIELD(m_hPullObject, FIELD_EHANDLE),
		DEFINE_FIELD(m_bIsPullingObject, FIELD_BOOLEAN),

END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CHL1_Player, DT_HL1Player)
SendPropInt(SENDINFO(m_bHasLongJump), 1, SPROP_UNSIGNED), SendPropInt(SENDINFO(m_nFlashBattery), 8, SPROP_UNSIGNED),
	SendPropBool(SENDINFO(m_bIsPullingObject)),

	SendPropFloat(SENDINFO(m_flStartCharge)), SendPropFloat(SENDINFO(m_flAmmoStartCharge)),
	SendPropFloat(SENDINFO(m_flPlayAftershock)), SendPropFloat(SENDINFO(m_flNextAmmoBurn))
...
void CHL1_Player::StartPullingObject(CBaseEntity *pObject)
{
	...
	m_pPullConstraint =
		physenv->CreateBallsocketConstraint(VPhysicsGetObject(), pObject->VPhysicsGetObject(), NULL, ballsocket);
...
int CHL1_Player::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
	...
	if(ArmorValue() &&
	   !(info.GetDamageType() &
		 (DMG_FALL | DMG_DROWN | DMG_POISON)) && // armor doesn't protect against fall or drown damage!
	   !(GetFlags() & FL_GODMODE))
	{
...
void CHL1_Player::FindMissTargets(void)
{
	...
	for(int i = 0; i < numEnts; i++)
	{
		...
		if(FClassnameIs(pEnts[i], "prop_dynamic") || FClassnameIs(pEnts[i], "dynamic_prop") ||
		   FClassnameIs(pEnts[i], "prop_physics") || FClassnameIs(pEnts[i], "physics_prop"))
		{
			// NDebugOverlay::Cross3D( pEnts[i]->WorldSpaceCenter(), -Vector(4,4,4), Vector(4,4,4), 0, 255, 0,
			// true, 1.0f );
...
public:
	...
	~CGrabController(void);
	void AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys,
					  bool bIsMegaPhysCannon = false);
	void DetachEntity();
	void OnRestore();
