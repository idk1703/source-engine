...
//#define bits_COND_NODE_SEQUENCE			( COND_SPECIAL1 )		// pev->netname contains the name of a sequence to
//play
...
BEGIN_DATADESC(CBMortar)
	DEFINE_FIELD(m_maxFrame, FIELD_INTEGER), DEFINE_FIELD(m_flDmgTime, FIELD_FLOAT), DEFINE_FUNCTION(Animate),
		DEFINE_FIELD(m_iFrame, FIELD_INTEGER), DEFINE_FIELD(pSprite, FIELD_CLASSPTR),
END_DATADESC()
...
BEGIN_DATADESC(CInfoBM)
	DEFINE_FIELD(m_flRadius, FIELD_FLOAT), DEFINE_FIELD(m_flDelay, FIELD_FLOAT),
...
BEGIN_DATADESC(CNPC_BigMomma)
	DEFINE_FIELD(m_nodeTime, FIELD_TIME), DEFINE_FIELD(m_crabTime, FIELD_TIME), DEFINE_FIELD(m_mortarTime, FIELD_TIME),
		DEFINE_FIELD(m_painSoundTime, FIELD_TIME), DEFINE_KEYFIELD(m_iszNetName, FIELD_STRING, "netname"),
		DEFINE_FIELD(m_flWait, FIELD_TIME), DEFINE_FIELD(m_iszTarget, FIELD_STRING),

		DEFINE_FIELD(m_crabCount, FIELD_INTEGER), DEFINE_FIELD(m_flDmgTime, FIELD_TIME),
		DEFINE_FIELD(m_bDoneWithPath, FIELD_BOOLEAN), DEFINE_FIELD(m_vTossDir, FIELD_VECTOR),
	...
	switch(pEvent->event)
	{
		...
		case BIG_AE_MELEE_ATTACK1:
		{
			...
			if(pHurt)
			{
				...
				{
					case BIG_AE_MELEE_ATTACKBR:
						//						pHurt->pev->velocity = pHurt->pev->velocity + (vecFwd * 150) + Vector(0,0,250) -
						//(vecRight * 200);
						pHurt->SetAbsVelocity(pHurt->GetAbsVelocity() + (vecFwd * 150) + Vector(0, 0, 250) -
											  (vecRight * 200));
						break;
...
Vector VecCheckSplatToss(CBaseEntity *pEnt, const Vector &vecSpot1, Vector vecSpot2, float maxHeight)
{
	...
	UTIL_TraceLine(vecMidPoint, vecMidPoint + Vector(0, 0, maxHeight), MASK_SOLID_BRUSHONLY, pEnt, COLLISION_GROUP_NONE,
				   &tr);
	vecApex = tr.endpos;
...
int CNPC_BigMomma::RangeAttack1Conditions(float flDot, float flDist)
{
	...
	if(flDist <= BIG_MORTARDIST && m_mortarTime < gpGlobals->curtime)
	{
		...
		if(pEnemy)
		{
			...
			m_vTossDir =
				VecCheckSplatToss(this, startPos, pEnemy->BodyTarget(GetAbsOrigin()), random->RandomFloat(150, 500));
...
void CBMortar::Touch(CBaseEntity *pOther)
{
	...
	if(pOther->IsBSPModel())
	{
		...
		UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + GetAbsVelocity() * 10, MASK_SOLID, this, COLLISION_GROUP_NONE,
					   &tr);
		UTIL_DecalTrace(&tr, "Splash");
	}
	else
	...
	RadiusDamage(CTakeDamageInfo(this, pOwner, sk_bigmomma_dmg_blast.GetFloat(), DMG_ACID), GetAbsOrigin(),
				 sk_bigmomma_radius_blast.GetFloat(), CLASS_NONE, NULL);
