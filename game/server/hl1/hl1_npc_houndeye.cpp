...
BEGIN_DATADESC(CNPC_Houndeye)
	DEFINE_FIELD(m_iSpriteTexture, FIELD_INTEGER), DEFINE_FIELD(m_fAsleep, FIELD_BOOLEAN),
		DEFINE_FIELD(m_fDontBlink, FIELD_BOOLEAN), DEFINE_FIELD(m_vecPackCenter, FIELD_POSITION_VECTOR),
END_DATADESC()
...
int CNPC_Houndeye::RangeAttack1Conditions(float flDot, float flDist)
{
	...
	trace_t tr;
	UTIL_TraceHull(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 0.1), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID,
		...
		if(pEntity->m_takedamage != DAMAGE_NO)
		{
			...
			{ // houndeyes don't hurt other houndeyes with their attack
				...
				if(m_pSquad && m_pSquad->NumMembers() > 1)
				{
					...
					flAdjustedDamage =
						sk_houndeye_dmg_blast.GetFloat() +
						sk_houndeye_dmg_blast.GetFloat() * (HOUNDEYE_SQUAD_BONUS * (m_pSquad->NumMembers() - 1));
				}
				else
				{
					// solo
