...
public:
	CControllerNavigator(CNPC_Controller *pOuter) : BaseClass(pOuter) {}

	bool ActivityIsLocomotive(Activity activity)
	{
		return true;
	}
};
...
BEGIN_DATADESC(CNPC_Controller)
	...
	DEFINE_ARRAY(m_pBall, FIELD_CLASSPTR, 2), DEFINE_ARRAY(m_iBall, FIELD_INTEGER, 2),
		DEFINE_ARRAY(m_iBallTime, FIELD_TIME, 2), DEFINE_ARRAY(m_iBallCurrent, FIELD_INTEGER, 2),
		DEFINE_FIELD(m_vecEstVelocity, FIELD_VECTOR), DEFINE_FIELD(m_velocity, FIELD_VECTOR),
...
void CNPC_Controller::Spawn()
{
	...
	CapabilitiesAdd(bits_CAP_MOVE_FLY | bits_CAP_INNATE_RANGE_ATTACK1 | bits_CAP_INNATE_RANGE_ATTACK2 |
					bits_CAP_MOVE_SHOOT);
	...
	if(m_flShootEnd > gpGlobals->curtime)
	{
		...
		while(m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->curtime)
		{
			...
			if(GetEnemy() != NULL)
			{
				...
				vecDir = Intersect(vecSrc, GetEnemy()->BodyTarget(GetAbsOrigin()), m_vecEstVelocity,
								   sk_controller_speedball.GetFloat());

				float delta = 0.03490; // +-2 degree
				vecDir = vecDir + Vector(random->RandomFloat(-delta, delta), random->RandomFloat(-delta, delta),
										 random->RandomFloat(-delta, delta)) *
									  sk_controller_speedball.GetFloat();
...
void CNPC_Controller::RunAI(void)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		te->DynamicLight(filter, 0.0, &vecStart, 255, 192, 64, 0 /*exponent*/, m_iBallCurrent[i] / 8 /*radius*/, 0.5,
						 0);
	}
}
...
bool CNPC_Controller::OverridePathMove(float flInterval)
{
	...
	GetMoveProbe()->MoveLimit(NAV_FLY, GetLocalOrigin(), checkPos, MASK_NPCSOLID | CONTENTS_WATER, pMoveTarget,
							  &moveTrace);
...
bool CNPC_Controller::OverrideMove(float flInterval)
{
	...
	if(pMoveTarget /*|| HaveInspectTarget()*/)
	{
		...
		if(pMoveTarget)
		{
			UTIL_TraceEntity(this, GetAbsOrigin(), vMoveTargetPos, MASK_NPCSOLID_BRUSHONLY, pMoveTarget,
...
BEGIN_DATADESC(CNPC_ControllerHeadBall)
	...
	DEFINE_THINKFUNC(HuntThink), DEFINE_THINKFUNC(KillThink), DEFINE_ENTITYFUNC(BounceTouch),
		...
		DEFINE_FIELD(m_flSpawnTime, FIELD_TIME), DEFINE_FIELD(m_vecIdeal, FIELD_VECTOR),
...
void CNPC_ControllerHeadBall::HuntThink(void)
{
	...
	if(gpGlobals->curtime - m_flSpawnTime > 5 ||
	   m_pSprite->GetBrightness() < 64 /*|| GetEnemy() == NULL || m_hOwner == NULL*/ || !IsInWorld())
	{
		SetTouch(NULL);
	...
	if((GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()).Length() < 64)
	{
		...
		if(pEntity != NULL && pEntity->m_takedamage == DAMAGE_YES)
		{
			...
			te->BeamEntPoint(filter, 0.0, entindex(), NULL, 0, &(tr.m_pEnt->GetAbsOrigin()), g_sModelIndexLaser,
							 haloindex /* no halo */, 0, 10, 3, 20, 20, fadelength, amplitude, 255, 255, 255, 255, 10);
		}

		UTIL_EmitAmbientSound(GetSoundSourceIndex(), GetAbsOrigin(), "Controller.ElectroSound", 0.5, SNDLVL_NORM, 0,
							  100);
...
BEGIN_DATADESC(CNPC_ControllerZapBall)
	...
	DEFINE_THINKFUNC(AnimateThink), DEFINE_ENTITYFUNC(ExplodeTouch),
		...
		DEFINE_FIELD(m_hOwner, FIELD_EHANDLE), DEFINE_FIELD(m_flSpawnTime, FIELD_TIME),
		DEFINE_FIELD(m_pSprite, FIELD_CLASSPTR),
...
void CNPC_ControllerZapBall::ExplodeTouch(CBaseEntity *pOther)
{
	if(m_takedamage = DAMAGE_YES)
	{
		...
		//	void UTIL_EmitAmbientSound( CBaseEntity *entity, const Vector &vecOrigin, const char *samp, float vol,
		//soundlevel_t soundlevel, int fFlags, int pitch, float soundtime /*= 0.0f*/ )

		UTIL_EmitAmbientSound(GetSoundSourceIndex(), tr.endpos, "Controller.ElectroSound", 0.3, SNDLVL_NORM, 0,
							  random->RandomInt(90, 99));
	}
