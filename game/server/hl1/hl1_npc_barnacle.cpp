...
BEGIN_DATADESC(CNPC_Barnacle)
	...
	DEFINE_FIELD(m_flAltitude, FIELD_FLOAT), DEFINE_FIELD(m_flKillVictimTime, FIELD_TIME),
		DEFINE_FIELD(m_cGibs, FIELD_INTEGER), // barnacle loads up on gibs each time it kills something.
		DEFINE_FIELD(m_fLiftingPrey, FIELD_BOOLEAN), DEFINE_FIELD(m_flTongueAdj, FIELD_FLOAT),
...
void CNPC_Barnacle::BarnacleThink(void)
{
	...
	else if(GetEnemy() != NULL)
	{
		...
		if(m_fLiftingPrey)
		{
			...
			trace_t trace;
			UTIL_TraceEntity(pEnemy, pEnemy->GetAbsOrigin(), vecNewEnemyOrigin, MASK_SOLID_BRUSHONLY, pEnemy,
			...
			if(trace.fraction != 1.0)
			{
				...
				{
					pEnemy->MyCombatCharacterPointer()->DispatchInteraction(g_interactionBarnacleVictimReleased, NULL,
																			this);
				}
		...
		else
		{
			...
			if(m_flKillVictimTime != -1 && gpGlobals->curtime > m_flKillVictimTime)
			{
				...
				if(pVictim)
				{
					...
					pVictim->TakeDamage(
						CTakeDamageInfo(this, this, pVictim->m_iHealth, DMG_SLASH | DMG_ALWAYSGIB | DMG_CRUSH));
					m_cGibs = 3;
				}
	...
	else
	{
		...
		// NDebugOverlay::Box( GetAbsOrigin() - Vector( 0, 0, flLength ), Vector( -2, -2, -2 ), Vector( 2, 2, 2 ),
		// 255,0,0, 0, 0.1 );
...
CBaseEntity *CNPC_Barnacle::TongueTouchEnt(float *pflLength)
{
	...
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 2048), MASK_SOLID_BRUSHONLY, this,
