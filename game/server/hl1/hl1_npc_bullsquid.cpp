...
BEGIN_DATADESC(CSquidSpit)
	DEFINE_FIELD(m_nSquidSpitSprite, FIELD_INTEGER), DEFINE_FIELD(m_hSprite, FIELD_EHANDLE),
END_DATADESC()
...
void CSquidSpit::Shoot(CBaseEntity *pOwner, Vector vecStart, Vector vecVelocity)
{
	...
	if(pSprite)
	{
		...
		pSprite->SetTransparency(pSpit->m_nRenderMode, pSpit->m_clrRender->r, pSpit->m_clrRender->g,
								 pSpit->m_clrRender->b, pSpit->m_clrRender->a, pSpit->m_nRenderFX);
	}
...
BEGIN_DATADESC(CNPC_Bullsquid)
	DEFINE_FIELD(m_fCanThreatDisplay, FIELD_BOOLEAN), DEFINE_FIELD(m_flLastHurtTime, FIELD_TIME),
...
void CNPC_Bullsquid::HandleAnimEvent(animevent_t *pEvent)
{
	switch(pEvent->event)
	{
		...
		{
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack(70, Vector(-16, -16, -16), Vector(16, 16, 16),
...
int CNPC_Bullsquid::MeleeAttack2Conditions(float flDot, float flDist)
{
	if(flDist <= 85 && flDot >= 0.7 &&
	   !HasCondition(COND_CAN_MELEE_ATTACK1)) // The player & bullsquid can be as much as their bboxes
...
{
	return SOUND_WORLD | SOUND_COMBAT | SOUND_CARCASS | SOUND_MEAT | SOUND_GARBAGE | SOUND_PLAYER;
}
...
AI_BEGIN_CUSTOM_NPC(monster_bullchicken, CNPC_Bullsquid)
	...
	// > SCHED_SQUID_EAT
	//=========================================================
	DEFINE_SCHEDULE(SCHED_SQUID_EAT,
	...
	// > SCHED_SQUID_SNIFF_AND_EAT
	//=========================================================
	DEFINE_SCHEDULE(SCHED_SQUID_SNIFF_AND_EAT,
	...
	// > SCHED_SQUID_WALLOW
	//=========================================================
	DEFINE_SCHEDULE(SCHED_SQUID_WALLOW,
	...
	// > SCHED_SQUID_CHASE_ENEMY
	//=========================================================
	DEFINE_SCHEDULE(SCHED_SQUID_CHASE_ENEMY,
