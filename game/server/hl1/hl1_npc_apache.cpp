...
BEGIN_DATADESC(CNPC_Apache)
	DEFINE_FIELD(m_iRockets, FIELD_INTEGER), DEFINE_FIELD(m_flForce, FIELD_FLOAT),
		DEFINE_FIELD(m_flNextRocket, FIELD_TIME), DEFINE_FIELD(m_vecTarget, FIELD_VECTOR),
		DEFINE_FIELD(m_posTarget, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecDesired, FIELD_VECTOR),
		DEFINE_FIELD(m_posDesired, FIELD_POSITION_VECTOR), DEFINE_FIELD(m_vecGoal, FIELD_VECTOR),
		DEFINE_FIELD(m_angGun, FIELD_VECTOR), DEFINE_FIELD(m_flLastSeen, FIELD_TIME),
		...
		DEFINE_FIELD(m_pBeam, FIELD_CLASSPTR), DEFINE_FIELD(m_flGoalSpeed, FIELD_FLOAT),
...
void CNPC_Apache::Flight(void)
{
	...
	// ALERT( at_console, "%.0f %.0f : %.0f %.0f : %.0f %.0f : %.0f\n", GetAbsOrigin().x, pev->velocity.x, flDist,
	// flSpeed, GetAbsAngles().x, pev->avelocity.x, m_flForce ); ALERT( at_console, "%.0f %.0f : %.0f %0.f : %.0f\n",
...
void CNPC_Apache::AimRocketGun(void)
{
	...
	// ALERT( at_console, "%d %d %d %4.2f\n", GetAbsAngles().x < 0, DotProduct( pev->velocity, gpGlobals->v_forward ) >
	// -100, m_flNextRocket < gpGlobals->curtime, DotProduct( m_vecTarget, vecEst ) );
		...
		if(m_flLastSeen + 60 > gpGlobals->curtime)
		{
			if(GetEnemy() != NULL)
			{
				...
				// if (DotProduct( vTargetDir, vecEst) > 0.5)
				{
					...
					UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecEst * 4096, MASK_ALL, this, COLLISION_GROUP_NONE,
								   &tr);
...
void CNPC_Apache::LaunchRocket(Vector &viewDir, int damage, int radius, Vector vecLaunchPoint)
{
	...
	CGrenadeHomer *pGrenade = CGrenadeHomer::CreateGrenadeHomer(
		MAKE_STRING("models/weapons/w_missile.mdl"), MAKE_STRING("Apache.RPG"), vecLaunchPoint, vec3_angle, edict());
	pGrenade->Spawn();
	pGrenade->SetHoming(MISSILE_HOMING_STRENGTH, MISSILE_HOMING_DELAY, MISSILE_HOMING_RAMP_UP, MISSILE_HOMING_DURATION,
...
void CNPC_Apache::DyingThink(void)
{
	...
	if(gpGlobals->curtime > m_flNextCrashExplosion)
	{
		...
		te->BreakModel(pvsFilter, 0.0, GetAbsOrigin(), vec3_angle, vecSize, vec3_origin, m_nDebrisModel, 100, 0, 2.5,
					   BREAK_METAL);
...
void CNPC_Apache::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr,
