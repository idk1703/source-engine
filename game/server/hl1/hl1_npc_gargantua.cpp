...
BEGIN_DATADESC(CNPC_Gargantua)
	DEFINE_FIELD(m_pEyeGlow, FIELD_CLASSPTR), DEFINE_FIELD(m_eyeBrightness, FIELD_INTEGER),
		DEFINE_FIELD(m_seeTime, FIELD_TIME), DEFINE_FIELD(m_flameTime, FIELD_TIME),
		DEFINE_FIELD(m_streakTime, FIELD_TIME), DEFINE_ARRAY(m_pFlame, FIELD_CLASSPTR, 4),
		DEFINE_FIELD(m_flameX, FIELD_FLOAT), DEFINE_FIELD(m_flameY, FIELD_FLOAT), DEFINE_FIELD(m_flDmgTime, FIELD_TIME),
...
BEGIN_DATADESC(CStomp)
	DEFINE_FIELD(m_vecMoveDir, FIELD_VECTOR), DEFINE_FIELD(m_flScale, FIELD_FLOAT),
		DEFINE_FIELD(m_flSpeed, FIELD_FLOAT), DEFINE_FIELD(m_uiFramerate, FIELD_INTEGER),
		DEFINE_FIELD(m_flDmgTime, FIELD_TIME), DEFINE_FIELD(m_pOwner, FIELD_CLASSPTR),
END_DATADESC()
...
void CStomp::Think(void)
{
	...
	UTIL_TraceHull(vecStart, vecEnd, Vector(-32, -32, -32), Vector(32, 32, 32), MASK_SOLID, m_pOwner,
	...
	while(gpGlobals->curtime - m_flDmgTime > STOMP_INTERVAL)
	{
		...
		for(int i = 0; i < 2; i++)
		{
			...
			if(pSprite)
			{
				UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 500), MASK_SOLID, this,
							   COLLISION_GROUP_NONE, &tr);
...
void CNPC_Gargantua::HandleAnimEvent(animevent_t *pEvent)
{
	...
	switch(pEvent->event)
	{
		case GARG_AE_SLASH_LEFT:
		{
			// HACKHACK!!!
			CBaseEntity *pHurt =
				GargantuaCheckTraceHullAttack(GARG_ATTACKDIST + 10.0, sk_gargantua_dmg_slash.GetFloat(), DMG_SLASH);
	...
	switch(pTask->iTask)
	{
		case TASK_DIE:
			...
			if(gpGlobals->curtime > m_flWaitFinished)
			{
				...
				te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, vecSize, vec3_origin, gGargGibModel, 200, 50,
							   3.0, BREAK_FLESH);
...
void CNPC_Gargantua::FlameUpdate(void)
{
	...
	for(i = 0; i < 2; i++)
	{
		if(m_pFlame[i])
		{
			...
			FlameDamage(vecStart, trace.endpos, this, this, sk_gargantua_dmg_fire.GetFloat(), CLASS_ALIEN_MONSTER,
						DMG_BURN);
...
void CNPC_Gargantua::FlameDamage(Vector vecStart, Vector vecEnd, CBaseEntity *pevInflictor, CBaseEntity *pevAttacker,
...
AI_BEGIN_CUSTOM_NPC(monster_gargantua, CNPC_Gargantua)
	...
	// > SCHED_GARG_FLAME
	//=========================================================
	DEFINE_SCHEDULE(SCHED_GARG_FLAME,
	...
	// > SCHED_GARG_SWIPE
	//=========================================================
	DEFINE_SCHEDULE(SCHED_GARG_SWIPE,
	...
	DEFINE_SCHEDULE(SCHED_GARG_CHASE_ENEMY,
	...
	DEFINE_SCHEDULE(SCHED_GARG_CHASE_ENEMY_FAILED,
