...
public:
	...
	virtual int PlayScriptedSentence(const char *pszSentence, float duration, float volume, soundlevel_t soundlevel,
...
void CNPC_GMan::Spawn()
{
	...
	CapabilitiesAdd(bits_CAP_MOVE_GROUND | bits_CAP_OPEN_DOORS | bits_CAP_USE_WEAPONS | bits_CAP_ANIMATEDFACE |
					bits_CAP_TURN_HEAD);
...
void CNPC_GMan::TraceAttack(CBaseEntity *pAttacker, float flDamage, const Vector &vecDir, trace_t *ptr,
							int bitsDamageType)
{
...
int CNPC_GMan::PlayScriptedSentence(const char *pszSentence, float delay, float volume, soundlevel_t soundlevel,
									bool bConcurrent, CBaseEntity *pListener)
{
