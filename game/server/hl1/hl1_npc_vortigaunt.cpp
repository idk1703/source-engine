...
BEGIN_DATADESC(CNPC_Vortigaunt)
	DEFINE_FIELD(m_iBravery, FIELD_INTEGER), DEFINE_ARRAY(m_pBeam, FIELD_CLASSPTR, VORTIGAUNT_MAX_BEAMS),
		DEFINE_FIELD(m_iBeams, FIELD_INTEGER), DEFINE_FIELD(m_flNextAttack, FIELD_TIME),
		DEFINE_FIELD(m_iVoicePitch, FIELD_INTEGER), DEFINE_FIELD(m_hDead, FIELD_EHANDLE),
END_DATADESC()
...
void CNPC_Vortigaunt::HandleAnimEvent(animevent_t *pEvent)
{
	...
	switch(pEvent->event)
	{
		...
		{
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack(40, Vector(-10, -10, -10), Vector(10, 10, 10),
													  sk_islave_dmg_claw.GetFloat(), DMG_SLASH);
			CPASAttenuationFilter filter(this);
			if(pHurt)
			{
		...
		{
			CBaseEntity *pHurt = CheckTraceHullAttack(40, Vector(-10, -10, -10), Vector(10, 10, 10),
													  sk_islave_dmg_clawrake.GetFloat(), DMG_SLASH);
		...
		case ISLAVE_AE_ZAP_SHOOT:
		{
			...
			if(m_hDead != NULL)
			{
				...
				trace_t trace;
				UTIL_TraceHull(vecDest, vecDest, GetHullMins(), GetHullMaxs(), MASK_SOLID, m_hDead,
...
int CNPC_Vortigaunt::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
	...
	if((inputInfo.GetDamageType() & DMG_SLASH) && inputInfo.GetAttacker() &&
	   IRelationType(inputInfo.GetAttacker()) == D_NU)
		return 0;
...
void CNPC_Vortigaunt::ZapBeam(int side)
{
	...
	vecAim =
		vecAim + side * right * random->RandomFloat(0, deflection) + up * random->RandomFloat(-deflection, deflection);
