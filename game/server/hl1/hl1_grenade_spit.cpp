...
BEGIN_DATADESC(CGrenadeSpit)
	...
	// Function pointers
	DEFINE_THINKFUNC(SpitThink), DEFINE_ENTITYFUNC(GrenadeSpitTouch),
...
void CGrenadeSpit::GrenadeSpitTouch(CBaseEntity *pOther)
{
	...
	if(!pOther->m_takedamage)
	{
		...
		te->SpriteSpray(filter, 0.0, &tr.endpos, &tr.plane.normal, m_nSquidSpitSprite, random->RandomInt(90, 160), 50,
						random->RandomInt(5, 15));
	}
	else
	{
		RadiusDamage(CTakeDamageInfo(this, GetThrower(), m_flDamage, DMG_BLAST), GetAbsOrigin(), m_DmgRadius,
					 CLASS_NONE, NULL);
	}
...
{
	if(m_fSpitDeathTime != 0 && m_fSpitDeathTime < gpGlobals->curtime)
	{
		UTIL_Remove(this);
	}
	SetNextThink(gpGlobals->curtime + 0.1f);
