...
bool CAI_SchedulesManager::LoadSchedulesFromBuffer(const char *prefix, const char *pStartFile,
		...
		while((token[0] != '\0') && (stricmp("Interrupts", token)))
		{
			...
			else if(!stricmp("Task", token))
			{
				...
				tempTask[taskNum].flTaskData =
					(pIdSpace) ? pIdSpace->TaskGlobalToLocal(taskID) : AI_RemapFromGlobal(taskID);
			...
			else if(!stricmp("Schedule", token))
			{
				...
				tempTask[taskNum].flTaskData =
					(pIdSpace) ? pIdSpace->ScheduleGlobalToLocal(schedID) : AI_RemapFromGlobal(schedID);
			...
			{
				// a parse error.  Interrupts is the next section, TASK_ is probably the next task, missing task
				// argument?
				Warning("ERROR: LoadSchd (%s): (%s) Bad syntax at task #%d (wasn't expecting %s)\n", prefix,
						new_schedule->GetName(), taskNum, token);
				Assert(0);
				return false;
			}
			else
			...
			if(!stricmp(token, ":"))
			{
				DevMsg("ERROR: LoadSchd (%s): Schedule (%s),\n        Task (%d), has a malformed AI Task Argument = "
					   "(%s)\n",
...
{
	for(CAI_Schedule *schedule = CAI_SchedulesManager::allSchedules; schedule != NULL;
