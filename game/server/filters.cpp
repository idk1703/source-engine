...
bool CFilterEnemy::PassesFilterImpl(CBaseEntity *pCaller, CBaseEntity *pEntity)
{
	...
	//	This allows us to use test criteria to initially pick an enemy, then disregard the test until a new enemy comes
	//along
	if(HasSpawnFlags(SF_FILTER_ENEMY_NO_LOSE_AQUIRED) && (pEntity == pCaller->GetEnemy()))
		return true;

	// This is a little weird, but it's saying that if we're not the entity we're excluding the filter to, then just
	// pass it throughZ
...
bool CFilterEnemy::PassesNameFilter(CBaseEntity *pEnemy)
{
	...
	bool bNameOrClassnameMatches =
		(m_iszEnemyName == pEnemy->GetEntityName() || m_iszEnemyName == pEnemy->m_iClassname);
...
bool CFilterEnemy::PassesMobbedFilter(CBaseEntity *pCaller, CBaseEntity *pEnemy)
{
	...
	for(CAI_BaseNPC *pSquadMember = pNPC->GetSquad()->GetFirstMember(&iter); pSquadMember != NULL;
