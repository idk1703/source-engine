...
//-----------------------------------------------------------------------------
CEntityBlocker *CEntityBlocker::Create(const Vector &origin, const Vector &mins, const Vector &maxs,
									   CBaseEntity *pOwner, bool bBlockPhysics)
{
...
void CC_Test_Entity_Blocker(void)
{
	...
	UTIL_TraceHull(vecOrigin + Vector(0, 0, 256), vecOrigin - Vector(0, 0, 256), VEC_HULL_MIN_SCALED(pPlayer),
				   VEC_HULL_MAX_SCALED(pPlayer), MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr);
	if(!tr.allsolid && !tr.startsolid)
	{
		CEntityBlocker::Create(tr.endpos, VEC_HULL_MIN_SCALED(pPlayer), VEC_HULL_MAX_SCALED(pPlayer), NULL, true);
		NDebugOverlay::Box(tr.endpos, VEC_HULL_MIN_SCALED(pPlayer), VEC_HULL_MAX_SCALED(pPlayer), 0, 255, 0, 64,
						   1000.0);
	}
}
static ConCommand test_entity_blocker("test_entity_blocker", CC_Test_Entity_Blocker,
