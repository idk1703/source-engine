...
BEGIN_DATADESC(CTeamControlPointRound)
	DEFINE_KEYFIELD(m_bDisabled, FIELD_BOOLEAN, "StartDisabled"),
		...
		DEFINE_OUTPUT(m_OnStart, "OnStart"), DEFINE_OUTPUT(m_OnEnd, "OnEnd"),
...
//			win the game.
//-----------------------------------------------------------------------------
int CTeamControlPointRound::TeamOwnsAllPoints(CTeamControlPoint *pOverridePoint /* = NULL */,
											  int iOverrideNewTeam /* = TEAM_UNASSIGNED */) const
{
	int i;
...
bool CTeamControlPointRound::IsPlayable(void)
{
	...
	if((iWinners >= FIRST_GAME_TEAM) && (iWinners != m_iInvalidCapWinner))
	{
	...
	if(pMaster)
	{
		if(!IsPlayable())
		{
			...
			for(int iTeam = FIRST_GAME_TEAM; iTeam < GetNumberOfTeams(); iTeam++)
			{
				...
				{
					if((!pMaster->IsBaseControlPoint(
						   m_ControlPoints[iControlPoint]
							   ->GetPointIndex())) && // this is NOT the base point for one of the teams (we don't want
													  // to assign the base to the wrong team)
					   (!WouldNewCPOwnerWinGame(m_ControlPoints[iControlPoint],
												iTeam))) // making this change would make this round playable
					{
						...
						for(int iObj = 0; iObj < ITriggerAreaCaptureAutoList::AutoList().Count(); ++iObj)
						{
							CTriggerAreaCapture *pArea =
								static_cast<CTriggerAreaCapture *>(ITriggerAreaCaptureAutoList::AutoList()[iObj]);
