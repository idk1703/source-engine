...
static ConCommand dumpentityfactories("dumpentityfactories", DumpEntityFactories_f, "Lists all entity factory names.",
									  FCVAR_GAMEDLL);
...
bool CFlaggedEntitiesEnum::AddToList(CBaseEntity *pEntity)
{
	...
	{
		AssertMsgOnce(0, "reached enumerated list limit.  Increase limit, decrease radius, or make it so entity flags "
						 "will work for you");
		return false;
	}
...
int UTIL_DropToFloor(CBaseEntity *pEntity, unsigned int mask, CBaseEntity *pIgnore)
{
	...
	UTIL_TraceEntity(pEntity, pEntity->GetAbsOrigin(), pEntity->GetAbsOrigin(), mask, pIgnore,
					 pEntity->GetCollisionGroup(), &trace);
	if(trace.fraction == 0.0)
		return -1;
#endif // HL2MP

	UTIL_TraceEntity(pEntity, pEntity->GetAbsOrigin(), pEntity->GetAbsOrigin() - Vector(0, 0, 256), mask, pIgnore,
					 pEntity->GetCollisionGroup(), &trace);
...
void UTIL_Remove(IServerNetworkable *oldObj)
{
	...
	if(PhysIsInCallback())
	{
		...
		// Since this is hidden from the calling code it's preferred to call PhysCallbackRemove() directly from the
		// caller in case the deferred delete will have unwanted results (like continuing to receive callbacks).  That
		// will make it obvious why the unwanted results are happening so the caller can handle them appropriately.
		// (some callbacks can be masked or the calling entity can be flagged to filter them in most cases)
		Assert(0);
	...
	if(pBaseEnt)
	{
#ifdef PORTAL // make sure entities are in the primary physics environment for the portal mod, this code should be safe
			  // even if the entity is in neither extra environment
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Returns true if the command was issued by the listenserver host, or by the dedicated server, via rcon or the server
 * console. This is valid during ConCommand execution.
 */
...
//-----------------------------------------------------------------------------
inline void TransmitShakeEvent(CBasePlayer *pPlayer, float localAmplitude, float frequency, float duration,
...
void UTIL_ScreenShake(const Vector &center, float amplitude, float frequency, float duration, float radius,
					  ShakeCommand_t eCommand, bool bAirShake)
{
	int i;
...
//-----------------------------------------------------------------------------
void UTIL_ScreenShakeObject(CBaseEntity *pEnt, const Vector &center, float amplitude, float frequency, float duration,
							float radius, ShakeCommand_t eCommand, bool bAirShake)
{
	int i;
	float localAmplitude;
	...
	for(i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if(pHighestParent == pPlayer->GetRootMoveParent())
		{
			localAmplitude = amplitude;
				...
				(pPlayer->GetGroundEntity()->GetRootMoveParent() == pHighestParent))
		{
...
void UTIL_ClientPrintFilter(IRecipientFilter &filter, int msg_dest, const char *msg_name, const char *param1,
...
void ClientPrint(CBasePlayer *player, int msg_dest, const char *msg_name, const char *param1, const char *param2,
...
void UTIL_SayText2Filter(IRecipientFilter &filter, CBasePlayer *pEntity, bool bChat, const char *msg_name,
...
void UTIL_SetTrace(trace_t &trace, const Ray_t &ray, edict_t *ent, float fraction, int hitgroup, unsigned int contents,
...
//			Speed -
//-----------------------------------------------------------------------------
void UTIL_Beam(Vector &Start, Vector &End, int nModelIndex, int nHaloIndex, unsigned char FrameStart,
			   unsigned char FrameRate, float Life, unsigned char Width, unsigned char EndWidth,
			   unsigned char FadeLength, unsigned char Noise, unsigned char Red, unsigned char Green,
			   unsigned char Blue, unsigned char Brightness, unsigned char Speed)
{
	CBroadcastRecipientFilter filter;

	te->BeamPoints(filter, 0.0, &Start, &End, nModelIndex, nHaloIndex, FrameStart, FrameRate, Life, Width, EndWidth,
...
int DispatchSpawn(CBaseEntity *pEntity)
{
	if(pEntity)
	{
		...
		int iClassname =
			((CEntityFactoryDictionary *)EntityFactoryDictionary())->m_Factories.Find(pEntity->GetClassname());
		if(iClassname != ((CEntityFactoryDictionary *)EntityFactoryDictionary())->m_Factories.InvalidIndex())
			pszClassname =
				((CEntityFactoryDictionary *)EntityFactoryDictionary())->m_Factories.GetElementName(iClassname);
		if(pszClassname)
		{
...
{
	if((!name || strlen((char *)STRING(name)) < 1) || !Q_stricmp((char *)STRING(name), "0"))
	{
...
void UTIL_SetClientVisibilityPVS(edict_t *pClient, const unsigned char *pvs, int pvssize)
{
	if(pClient == UTIL_GetCurrentCheckClient())
	{
		...
		{
			((unsigned char *)&pTo[limit])[i] =
				((unsigned char *)&pFrom[limit])[i] & !((unsigned char *)&pMask[limit])[i];
...
// Output : edict_t*
//-----------------------------------------------------------------------------
ConVar sv_strict_notarget("sv_strict_notarget", "0", 0,
...
//-----------------------------------------------------------------------------
Vector UTIL_PointOnLineNearestPoint(const Vector &vStartPos, const Vector &vEndPos, const Vector &vPoint,
									bool clampEnds)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool UTIL_IsFacingWithinTolerance(CBaseEntity *pViewer, const Vector &vecPosition, float flDotTolerance,
								  float *pflDot /*= NULL*/)
{
	if(pflDot)
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool UTIL_IsFacingWithinTolerance(CBaseEntity *pViewer, CBaseEntity *pTarget, float flDotTolerance,
								  float *pflDot /*= NULL*/)
{
...
//-----------------------------------------------------------------------------
bool UTIL_LoadAndSpawnEntitiesFromScript(CUtlVector<CBaseEntity *> &entities, const char *pScriptFile,
...
void CC_KDTreeTest(const CCommand &args)
{
	...
	switch(nTestType)
	{
		...
		case 1:
		{
			...
			for(int iTest = 0; iTest < NUM_KDTREE_TESTS; ++iTest)
			{
				UTIL_TraceHull(vecStart, vecTargets[iTest], VEC_HULL_MIN_SCALED(pPlayer), VEC_HULL_MAX_SCALED(pPlayer),
...
static ConCommand kdtree_test("kdtree_test", CC_KDTreeTest, "Tests spatial partition for entities queries.",
							  FCVAR_CHEAT);
...
void CC_VoxelTreePlayerView(void)
{
	...
	CBasePlayer *pPlayer = static_cast<CBasePlayer *>(UTIL_GetLocalPlayer());
	Vector vecStart = pPlayer->GetAbsOrigin();
	::partition->RenderObjectsInPlayerLeafs(vecStart - VEC_HULL_MIN_SCALED(pPlayer),
											vecStart + VEC_HULL_MAX_SCALED(pPlayer), 3.0f);
}

static ConCommand voxeltree_playerview("voxeltree_playerview", CC_VoxelTreePlayerView,
...
static ConCommand voxeltree_box("voxeltree_box", CC_VoxelTreeBox,
								"View entities in the voxel-tree inside box <Vector(min), Vector(max)>.", FCVAR_CHEAT);
