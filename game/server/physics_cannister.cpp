...
BEGIN_SIMPLE_DATADESC(CThrustController)
	...
	DEFINE_FIELD(m_thrustVector, FIELD_VECTOR), DEFINE_FIELD(m_torqueVector, FIELD_VECTOR),
...
BEGIN_DATADESC(CPhysicsCannister)
	...
	DEFINE_OUTPUT(m_onActivate, "OnActivate"), DEFINE_OUTPUT(m_OnAwakened, "OnAwakened"),
		DEFINE_FIELD(m_thrustOrigin, FIELD_VECTOR), // this is a position, but in local space
		DEFINE_EMBEDDED(m_thruster), DEFINE_PHYSPTR(m_pController), DEFINE_FIELD(m_pJet, FIELD_CLASSPTR),
		DEFINE_FIELD(m_active, FIELD_BOOLEAN), DEFINE_KEYFIELD(m_thrustTime, FIELD_FLOAT, "fuel"),
		DEFINE_KEYFIELD(m_damage, FIELD_FLOAT, "expdamage"), DEFINE_KEYFIELD(m_damageRadius, FIELD_FLOAT, "expradius"),
		DEFINE_FIELD(m_activateTime, FIELD_TIME), DEFINE_KEYFIELD(m_gasSound, FIELD_SOUNDNAME, "gassound"),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Explode", InputExplode), DEFINE_INPUTFUNC(FIELD_VOID, "Wake", InputWake),
...
void CPhysicsCannister::Explode(CBaseEntity *pAttacker)
{
	...
	PropBreakableCreateAll(GetModelIndex(), pPhysics, GetAbsOrigin(), GetAbsAngles(), velocity, angVelocity, 1.0, 20,
