...
// Input  : *name -
//-----------------------------------------------------------------------------
CTEBloodSprite::CTEBloodSprite(const char *name) : CBaseTempEntity(name)
{
	m_vecOrigin.Init();
	m_nSprayModel = 0;
	m_nDropModel = 0;
...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD), SendPropVector(SENDINFO(m_vecDirection), -1, SPROP_COORD),
	...
	SendPropModelIndex(SENDINFO(m_nSprayModel)), SendPropModelIndex(SENDINFO(m_nDropModel)),
