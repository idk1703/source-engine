...
BEGIN_DATADESC(CFish)
	DEFINE_FIELD(m_pool, FIELD_EHANDLE), DEFINE_FIELD(m_id, FIELD_INTEGER), DEFINE_FIELD(m_angle, FIELD_FLOAT),
		DEFINE_FIELD(m_angleChange, FIELD_FLOAT), DEFINE_FIELD(m_forward, FIELD_VECTOR),
		DEFINE_FIELD(m_perp, FIELD_VECTOR), DEFINE_FIELD(m_poolOrigin, FIELD_POSITION_VECTOR),
		DEFINE_FIELD(m_waterLevel, FIELD_FLOAT), DEFINE_FIELD(m_speed, FIELD_FLOAT),
		DEFINE_FIELD(m_desiredSpeed, FIELD_FLOAT), DEFINE_FIELD(m_calmSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_panicSpeed, FIELD_FLOAT), DEFINE_FIELD(m_avoidRange, FIELD_FLOAT),
 ...
 */
void SendProxy_FishOriginX(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
						   int objectID)
{
...
void SendProxy_FishOriginY(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
						   int objectID)
{
	CFish *fish = (CFish *)pStruct;
	Assert(fish);
...
void SendProxy_FishAngle(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement,
						 int objectID)
{
...
SendPropVector(SENDINFO(m_poolOrigin), -1, SPROP_COORD, 0.0f, HIGH_DEFAULT), // only sent once
	...
	SendPropModelIndex(SENDINFO(m_nModelIndex)), SendPropInt(SENDINFO(m_lifeState)),
...
float CFish::Avoid(void)
{
	...
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + m_avoidRange * (m_forward + sideOffset * m_perp), MASK_PLAYERSOLID,
				   this, COLLISION_GROUP_NONE, &result);
...
BEGIN_DATADESC(CFishPool)
	...
	DEFINE_FIELD(m_fishCount, FIELD_INTEGER), DEFINE_FIELD(m_maxRange, FIELD_FLOAT),
		DEFINE_FIELD(m_swimDepth, FIELD_FLOAT), DEFINE_FIELD(m_waterLevel, FIELD_FLOAT),
		DEFINE_FIELD(m_isDormant, FIELD_BOOLEAN), DEFINE_UTLVECTOR(m_fishes, FIELD_EHANDLE),
	...
	if(m_visTimer.IsElapsed())
	{
		...
		for(i = 0; i < m_fishes.Count(); ++i)
		{
			...
			for(j = i + 1; j < m_fishes.Count(); ++j)
			{
				...
				UTIL_TraceLine(m_fishes[i]->GetAbsOrigin(), m_fishes[j]->GetAbsOrigin(), MASK_PLAYERSOLID, m_fishes[i],
