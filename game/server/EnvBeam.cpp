...
BEGIN_DATADESC(CEnvBeam)
	...
	DEFINE_FIELD(m_active, FIELD_INTEGER), DEFINE_FIELD(m_spriteTexture, FIELD_INTEGER),

		DEFINE_KEYFIELD(m_iszStartEntity, FIELD_STRING, "LightningStart"),
		DEFINE_KEYFIELD(m_iszEndEntity, FIELD_STRING, "LightningEnd"), DEFINE_KEYFIELD(m_life, FIELD_FLOAT, "life"),
		...
		DEFINE_KEYFIELD(m_frameStart, FIELD_INTEGER, "framestart"), DEFINE_KEYFIELD(m_radius, FIELD_FLOAT, "Radius"),
...
void CEnvBeam::Precache(void)
{
	...
	{
		// HACK/YWB:  This was almost always the laserbeam.spr, so alloc'ing the name a second time with the proper
		// extension isn't going to
		//  kill us on memrory.
...
void CEnvBeam::Strike(void)
{
	...
	if(pointStart || pointEnd)
	{
		...
		te->BeamEntPoint(filter, 0.0, pointStart ? 0 : pStart->entindex(), pointStart ? &pStart->GetAbsOrigin() : NULL,
						 pointEnd ? 0 : pEnd->entindex(), pointEnd ? &pEnd->GetAbsOrigin() : NULL, m_spriteTexture,
						 0, // No halo
