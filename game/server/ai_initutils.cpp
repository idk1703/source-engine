...
BEGIN_SIMPLE_DATADESC(HintNodeData)
	...
	DEFINE_FIELD(strEntityName, FIELD_STRING),
		...
		DEFINE_KEYFIELD(nHintType, FIELD_SHORT, "hinttype"), DEFINE_KEYFIELD(strGroup, FIELD_STRING, "Group"),
		DEFINE_KEYFIELD(iDisabled, FIELD_INTEGER, "StartHintDisabled"), DEFINE_FIELD(nNodeID, FIELD_INTEGER),
	...
	if(FClassnameIs(this, "info_hint"))
	{
		...
		else
		{
			Warning("info_hint (HammerID: %d, position (%.2f, %.2f, %.2f)) with no hint type.\n", m_NodeData.nWCNodeID,
					m_NodeData.vecPosition.x, m_NodeData.vecPosition.y, m_NodeData.vecPosition.z);
		}
