...
ConVar g_debug_transitions("g_debug_transitions", "0", FCVAR_NONE,
						   "Set to 1 and restart the map to be warned if the map has no trigger_transition volumes. "
						   "Set to 2 to see a dump of all entities & associated results during a transition.");
...
static ConCommand showtriggers_toggle("showtriggers_toggle", Cmd_ShowtriggersToggle_f, "Toggle show triggers",
									  FCVAR_CHEAT);
...
BEGIN_DATADESC(CBaseTrigger)
	...
	// Keyfields
	DEFINE_KEYFIELD(m_iFilterName, FIELD_STRING, "filtername"), DEFINE_FIELD(m_hFilter, FIELD_EHANDLE),
		...
		DEFINE_INPUTFUNC(FIELD_VOID, "Toggle", InputToggle), DEFINE_INPUTFUNC(FIELD_VOID, "TouchTest", InputTouchTest),

		DEFINE_INPUTFUNC(FIELD_VOID, "StartTouch", InputStartTouch),
		DEFINE_INPUTFUNC(FIELD_VOID, "EndTouch", InputEndTouch),

		// Outputs
		DEFINE_OUTPUT(m_OnStartTouch, "OnStartTouch"), DEFINE_OUTPUT(m_OnStartTouchAll, "OnStartTouchAll"),
		DEFINE_OUTPUT(m_OnEndTouch, "OnEndTouch"), DEFINE_OUTPUT(m_OnEndTouchAll, "OnEndTouchAll"),
		DEFINE_OUTPUT(m_OnTouching, "OnTouching"), DEFINE_OUTPUT(m_OnNotTouching, "OnNotTouching"),
	...
	if(IsTouching(pOther))
	{
		...
		for(int i = iSize - 1; i >= 0; i--)
		{
			...
			else if(hOther->IsPlayer() && !hOther->IsAlive())
			{
				...
				if(!HushAsserts())
				{
					AssertMsg(false, "Dead player [%s] is still touching this trigger at [%f %f %f]",
							  hOther->GetEntityName().ToCStr(), XYZ(hOther->GetAbsOrigin()));
				}
				Warning("Dead player [%s] is still touching this trigger at [%f %f %f]",
						hOther->GetEntityName().ToCStr(), XYZ(hOther->GetAbsOrigin()));
#endif
				m_hTouchingEntities.Remove(i);
			}
...
BEGIN_DATADESC(CTriggerHurt)
	...
	// Function Pointers
	DEFINE_FUNCTION(CTriggerHurtShim::RadiationThinkShim), DEFINE_FUNCTION(CTriggerHurtShim::HurtThinkShim),

		// Fields
		DEFINE_FIELD(m_flOriginalDamage, FIELD_FLOAT), DEFINE_KEYFIELD(m_flDamage, FIELD_FLOAT, "damage"),
		...
		DEFINE_FIELD(m_flLastDmgTime, FIELD_TIME), DEFINE_FIELD(m_flDmgResetTime, FIELD_TIME),
		...
		// Outputs
		DEFINE_OUTPUT(m_OnHurt, "OnHurt"), DEFINE_OUTPUT(m_OnHurtPlayer, "OnHurtPlayer"),
...
BEGIN_DATADESC(CTriggerLook)
	...
	DEFINE_FIELD(m_hLookTarget, FIELD_EHANDLE), DEFINE_FIELD(m_flLookTimeTotal, FIELD_FLOAT),
		DEFINE_FIELD(m_flLookTimeLast, FIELD_TIME), DEFINE_KEYFIELD(m_flTimeoutDuration, FIELD_FLOAT, "timeout"),
		DEFINE_FIELD(m_bTimeoutFired, FIELD_BOOLEAN), DEFINE_FIELD(m_hActivator, FIELD_EHANDLE),
		...
		// Inputs
		DEFINE_INPUT(m_flFieldOfView, FIELD_FLOAT, "FieldOfView"), DEFINE_INPUT(m_flLookTime, FIELD_FLOAT, "LookTime"),
...
private:
	...
	static int AddTransitionToList(levellist_t *pLevelList, int listCount, const char *pMapName,
	...
	static int BuildEntityTransitionList(CBaseEntity *pLandmarkEntity, const char *pLandmarkName,
	...
	static int AddEntityToTransitionList(CBaseEntity *pEntity, int flags, int nCount, CBaseEntity **ppEntList,
										 int *pEntityFlags);
...
BEGIN_DATADESC(CChangeLevel)
	...
	DEFINE_AUTO_ARRAY(m_szMapName, FIELD_CHARACTER), DEFINE_AUTO_ARRAY(m_szLandmarkName, FIELD_CHARACTER),
...
int CChangeLevel::AddTransitionToList(levellist_t *pLevelList, int listCount, const char *pMapName,
									  const char *pLandmarkName, edict_t *pentLandmark)
{
	int i;
	...
	if(pTrigger->Intersects(pEntity)) // It touches one, it's in the volume
	{
		switch(pEntity->GetSolid())
		{
			case SOLID_BBOX:
			{
				...
				Ray_t ray;
				trace_t tr;
				ray.Init(pEntity->GetAbsOrigin(), pEntity->GetAbsOrigin(), pEntity->WorldAlignMins(),
						 pEntity->WorldAlignMaxs());
				enginetrace->ClipRayToCollideable(ray, MASK_ALL, pCollide, &tr);
			...
			case SOLID_VPHYSICS:
			{
				...
				for(int i = collideList.Count() - 1; i >= 0; --i)
				{
					...
					trace_t tr;
					physcollision->TraceCollide(element.origin, element.origin, element.pCollide, element.angles,
												pTriggerCollide, pTrigger->GetAbsOrigin(), pTrigger->GetAbsAngles(),
												&tr);
					if(tr.startsolid)
						return true;
				}
...
int CChangeLevel::InTransitionVolume(CBaseEntity *pEntity, const char *pVolumeName)
{
	...
	while(pVolume)
	{
		if(pVolume && FClassnameIs(pVolume, "trigger_transition"))
		{
			...
			inVolume = TRANSITION_VOLUME_SCREENED_OUT; // Found a trigger_transition, but I don't intersect it -- if I
													   // don't find another, don't go!
		}
	...
	while(pentChangelevel)
	{
		...
		if(pTrigger)
		{
			...
			if(pentLandmark)
			{
				...
				if(AddTransitionToList(pLevelList, nCount, pTrigger->m_szMapName, pTrigger->m_szLandmarkName,
									   pentLandmark->edict()))
				{
					++nCount;
...
// Adds a single entity to the transition list, if appropriate. Returns the new count
//------------------------------------------------------------------------------
inline int CChangeLevel::AddEntityToTransitionList(CBaseEntity *pEntity, int flags, int nCount, CBaseEntity **ppEntList,
												   int *pEntityFlags)
{
...
int CChangeLevel::ChangeList(levellist_t *pLevelList, int maxList)
{
	...
	for(i = 0; i < count; i++)
	{
		...
		int iEntity =
			BuildEntityTransitionList(pLandmarkEntity, pLevelList[i].landmarkName, pEntList, entityFlags, MAX_ENTITY);
...
void CAI_ChangeTarget::InputActivate(inputdata_t &inputdata)
{
	...
	while((pTarget = gEntList.FindEntityByName(pTarget, m_target, NULL, inputdata.pActivator, inputdata.pCaller)) !=
		  NULL)
	{
...
CAI_BaseNPC *CAI_ChangeHintGroup::FindQualifiedNPC(CAI_BaseNPC *pPrev, CBaseEntity *pActivator, CBaseEntity *pCaller)
{
	...
	while(!pResult)
	{
		...
		switch(m_iSearchType)
		{
			case 0:
			{
				pEntity = gEntList.FindEntityByNameWithin(pEntity, pszSearchName, GetLocalOrigin(), m_flRadius, NULL,
														  pActivator, pCaller);
				break;
			}
			...
			{
				pEntity =
					gEntList.FindEntityInSphere(pEntity, GetLocalOrigin(), (m_flRadius != 0.0) ? m_flRadius : FLT_MAX);
				break;
			}
		}
...
BEGIN_DATADESC(CTriggerCamera)
	...
	DEFINE_FIELD(m_hPlayer, FIELD_EHANDLE), DEFINE_FIELD(m_hTarget, FIELD_EHANDLE),
		DEFINE_FIELD(m_pPath, FIELD_CLASSPTR), DEFINE_FIELD(m_sPath, FIELD_STRING), DEFINE_FIELD(m_flWait, FIELD_FLOAT),
		DEFINE_FIELD(m_flReturnTime, FIELD_TIME), DEFINE_FIELD(m_flStopTime, FIELD_TIME),
		DEFINE_FIELD(m_moveDistance, FIELD_FLOAT), DEFINE_FIELD(m_targetSpeed, FIELD_FLOAT),
		DEFINE_FIELD(m_initialSpeed, FIELD_FLOAT), DEFINE_FIELD(m_acceleration, FIELD_FLOAT),
		DEFINE_FIELD(m_deceleration, FIELD_FLOAT), DEFINE_FIELD(m_state, FIELD_INTEGER),
		DEFINE_FIELD(m_vecMoveDir, FIELD_VECTOR),
		DEFINE_KEYFIELD(m_iszTargetAttachment, FIELD_STRING, "targetattachment"),
		DEFINE_FIELD(m_iAttachmentIndex, FIELD_INTEGER), DEFINE_FIELD(m_bSnapToGoal, FIELD_BOOLEAN),
#if HL2_EPISODIC
		DEFINE_KEYFIELD(m_bInterpolatePosition, FIELD_BOOLEAN, "interpolatepositiontoplayer"),
		DEFINE_FIELD(m_vStartPos, FIELD_VECTOR), DEFINE_FIELD(m_vEndPos, FIELD_VECTOR),
		DEFINE_FIELD(m_flInterpStartTime, FIELD_TIME),
#endif
		DEFINE_FIELD(m_nPlayerButtons, FIELD_INTEGER), DEFINE_FIELD(m_nOldTakeDamage, FIELD_INTEGER),
...
void CTriggerCamera::Enable(void)
{
	...
	// Make the player invulnerable while under control of the camera.  This will prevent situations where the player
	// dies while under camera control but cannot restart their game due to disabled player inputs.
		...
		if(m_iszTargetAttachment != NULL_STRING)
		{
			if(!m_hTarget->GetBaseAnimating())
			{
				Warning("%s tried to target an attachment (%s) on target %s, which has no model.\n", GetClassname(),
						STRING(m_iszTargetAttachment), STRING(m_hTarget->GetEntityName()));
			}
			else
			{
...
BEGIN_DATADESC(CTriggerProximity)
	...
	DEFINE_FUNCTION(MeasureThink),
		...
		DEFINE_FIELD(m_hMeasureTarget, FIELD_EHANDLE), DEFINE_KEYFIELD(m_fRadius, FIELD_FLOAT, "radius"),
...
public:
	simresult_e Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime,
						 Vector &linear, AngularImpulse &angular)
	{
		...
		int nNoiseMod = 5 + (uintp)pObject % 15; //
...
BEGIN_SIMPLE_DATADESC(CPhysicsWind)
	...
	DEFINE_FIELD(m_nWindYaw, FIELD_INTEGER), DEFINE_FIELD(m_flWindSpeed, FIELD_FLOAT),
...
BEGIN_DATADESC(CTriggerWind)
	...
	DEFINE_FIELD(m_nSpeedCurrent, FIELD_INTEGER), DEFINE_FIELD(m_nSpeedTarget, FIELD_INTEGER),
		DEFINE_FIELD(m_nDirBase, FIELD_INTEGER), DEFINE_FIELD(m_nDirCurrent, FIELD_INTEGER),
		DEFINE_FIELD(m_nDirTarget, FIELD_INTEGER), DEFINE_FIELD(m_bSwitch, FIELD_BOOLEAN),

		DEFINE_FIELD(m_nSpeedBase, FIELD_INTEGER), DEFINE_KEYFIELD(m_nSpeedNoise, FIELD_INTEGER, "SpeedNoise"),
...
BEGIN_DATADESC(CTriggerImpact)
	...
	DEFINE_KEYFIELD(m_flMagnitude, FIELD_FLOAT, "Magnitude"), DEFINE_KEYFIELD(m_flNoise, FIELD_FLOAT, "Noise"),
...
void CTriggerPlayerMovement::Spawn(void)
{
	if(HasSpawnFlags(SF_TRIGGER_ONLY_PLAYER_ALLY_NPCS))
	{
		// @Note (toml 01-07-04): fix up spawn flag collision coding error. Remove at some point once all maps fixed up
		// please!
		DevMsg("*** trigger_playermovement using obsolete spawnflag. Remove and reset with new value for \"Disable "
...
BEGIN_DATADESC(CTriggerVPhysicsMotion)
	DEFINE_PHYSPTR(m_pController),
		...
		DEFINE_FIELD(m_linearLimitTime, FIELD_FLOAT), DEFINE_FIELD(m_linearLimitStart, FIELD_TIME),
		DEFINE_FIELD(m_linearLimitStartTime, FIELD_TIME), DEFINE_INPUT(m_linearScale, FIELD_FLOAT, "SetVelocityScale"),
...
//------------------------------------------------------------------------------
IMotionEvent::simresult_e CTriggerVPhysicsMotion::Simulate(IPhysicsMotionController *pController,
...
BEGIN_DATADESC(CTriggerApplyImpulse)
	DEFINE_KEYFIELD(m_vecImpulseDir, FIELD_VECTOR, "impulse_dir"), DEFINE_KEYFIELD(m_flForce, FIELD_FLOAT, "force"),
