...
BEGIN_SIMPLE_DATADESC(CAI_SentenceBase)
	DEFINE_FIELD(m_voicePitch, FIELD_INTEGER), DEFINE_FIELD(m_nQueuedSentenceIndex, FIELD_INTEGER),
		DEFINE_FIELD(m_flQueueTimeout, FIELD_TIME), DEFINE_FIELD(m_nQueueSoundPriority, FIELD_INTEGER),
END_DATADESC();
...
void CAI_SentenceBase::SentenceMsg(const char *pStatus, const char *pSentence)
{
	...
	switch(nMode)
	{
		...
		case 2:
			DevMsg(GetOuter(), "SENTENCE [%d %.2f] %s: %s\n", GetOuter()->entindex(), gpGlobals->curtime, pStatus,
				   pSentence);
			break;
	}
}
...
void CAI_SentenceBase::UpdateSentenceQueue()
{
	...
	{
		SENTENCEG_PlaySentenceIndex(GetOuter()->edict(), m_nQueuedSentenceIndex, GetVolume(), GetSoundLevel(), 0,
									GetVoicePitch());
...
{
	int nSentenceIndex =
		SENTENCEG_PlayRndSz(GetOuter()->edict(), pSentence, GetVolume(), GetSoundLevel(), 0, GetVoicePitch());
	if(nSentenceIndex < 0)
	{
...
// Speech w/ queue
//-----------------------------------------------------------------------------
int CAI_SentenceBase::SpeakQueued(const char *pSentence, SentencePriority_t nSoundPriority,
