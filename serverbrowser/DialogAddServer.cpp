...
CDialogAddServer::CDialogAddServer(vgui::Panel *parent, IGameList *gameList) : Frame(parent, "DialogAddServer")
{
	...
	m_pDiscoveredGames->AddColumnHeader(0, "Password", "#ServerBrowser_Password", 16,
										ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
	m_pDiscoveredGames->AddColumnHeader(1, "Bots", "#ServerBrowser_Bots", 16,
										ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE |
											ListPanel::COLUMN_HIDDEN);
	m_pDiscoveredGames->AddColumnHeader(2, "Secure", "#ServerBrowser_Secure", 16,
										ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
	...
	m_pDiscoveredGames->AddColumnHeader(3, "Replay", "#ServerBrowser_Replay", nReplayWidth,
										ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
	m_pDiscoveredGames->AddColumnHeader(4, "Name", "#ServerBrowser_Servers", 20,
...
void CDialogAddServer::OnCommand(const char *command)
{
	...
	else if(!Q_stricmp(command, "addselected"))
	{
		if(m_pDiscoveredGames->GetSelectedItemsCount())
		{
			...
			m_pDiscoveredGames->RemoveItem(
				m_pDiscoveredGames->GetSelectedItem(0)); // as we add to favs remove from the list
...
void CDialogAddServer::TestServers()
{
	...
	else
	{
		...
		g_pVGuiLocalize->ConstructString(wstr, sizeof(wstr), g_pVGuiLocalize->Find("#ServerBrowser_ServersResponding"),
										 1, waddress);
	}
	...
	{
		m_Queries.AddToTail(steamapicontext->SteamMatchmakingServers()->PingServer(
			vecAdress[iAddress].GetIPHostByteOrder(), vecAdress[iAddress].GetPort(), this));
	}
}
