...
void CServerBrowserDialog::AddServerToFavorites(gameserveritem_t &server)
{
	if(steamapicontext->SteamMatchmaking())
	{
		steamapicontext->SteamMatchmaking()->AddFavoriteGame(
			server.m_nAppID, server.m_NetAdr.GetIP(), server.m_NetAdr.GetConnectionPort(),
			server.m_NetAdr.GetQueryPort(), k_unFavoriteFlagFavorite, time(NULL));
...
CDialogGameInfo *CServerBrowserDialog::OpenGameInfoDialog(IGameList *gameList, unsigned int serverIndex)
{
	...
	CDialogGameInfo *gameDialog =
		new CDialogGameInfo(NULL, pServer->m_NetAdr.GetIP(), pServer->m_NetAdr.GetQueryPort(),
							pServer->m_NetAdr.GetConnectionPort(), gameList->GetConnectCode());
...
//-----------------------------------------------------------------------------
// Purpose: return the app id to limit game queries to, set by Source/HL1 engines (NOT by filter settings, that is per
// page)
//-----------------------------------------------------------------------------
...
void CServerBrowserDialog::OnConnectToGame(KeyValues *pMessageValues)
{
	...
	{
		steamapicontext->SteamMatchmaking()->AddFavoriteGame(0, unIP, connectionPort, queryPort,
															 k_unFavoriteFlagHistory, time(NULL));
...
void CServerBrowserDialog::OnKeyCodePressed(vgui::KeyCode code)
{
	...
	else if(nButtonCode == KEY_XBUTTON_A || nButtonCode == STEAMCONTROLLER_A)
	{
		...
		// return;
			...
			nButtonCode == STEAMCONTROLLER_DPAD_UP || nButtonCode == KEY_UP || nButtonCode == KEY_XBUTTON_DOWN ||
			nButtonCode == KEY_XSTICK1_DOWN || nButtonCode == KEY_XSTICK2_DOWN ||
