...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerBrowser, IServerBrowser, SERVERBROWSER_INTERFACE_VERSION,
								  g_ServerBrowserSingleton);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerBrowser, IVGuiModule, "VGuiModuleServerBrowser001",
								  g_ServerBrowserSingleton); // the interface loaded by PlatformMenu.vdf
...
ConVar sb_firstopentime("sb_firstopentime", "0", FCVAR_DEVELOPMENTONLY,
						"Indicates the time the server browser was first opened.");
ConVar sb_numtimesopened("sb_numtimesopened", "0", FCVAR_DEVELOPMENTONLY,
...
bool CServerBrowser::Activate()
{
	...
	if(firstTimeOpening)
	{
		m_hInternetDlg->LoadUserData(); // reload the user data the first time the dialog is made visible, helps with
										// the lag between module load and steamui getting Deactivate() call
...
bool CServerBrowser::OpenGameInfoDialog(uint64 ulSteamIDFriend, const char *pszConnectCode)
{
	...
	if(m_hInternetDlg.Get())
	{
		...
		if(steamapicontext->SteamFriends()->GetFriendGamePlayed(ulSteamIDFriend, &friendGameInfo))
		{
			...
			CDialogGameInfo *pDialogGameInfo = m_hInternetDlg->OpenGameInfoDialog(
				friendGameInfo.m_unGameIP, friendGameInfo.m_usGamePort, usConnPort, pszConnectCode);
...
//-----------------------------------------------------------------------------
const char *CServerBrowser::GetMapFriendlyNameAndGameType(const char *pszMapName, char *szFriendlyMapName,
														  int cchFriendlyName)
{
