...
#define FL_DONTTOUCH \
	(1 << 22) // Doesn't generate touch functions, generates Untouch() for anything it was touching when this flag was
			  // set
#define FL_BASEVELOCITY \
	(1 << 23) // Base velocity has been applied this frame (used to convert base velocity into momentum)
#define FL_WORLDBRUSH \
	(1 << 24) // Not moveable/removeable brush entity (really part of the world, but represented as an entity for
...
#define FL_ANIMDUCKING \
	(1 << 2) // Player flag -- Player is in the process of crouching or uncrouching but could be in transition
...
#define FL_ONTRAIN \
	(1 << 4) // Player is _controlling_ a train, so movement commands should be ignored on client during prediction.
...
#define FL_DONTTOUCH \
	(1 << 23) // Doesn't generate touch functions, generates Untouch() for anything it was touching when this flag was
			  // set
#define FL_BASEVELOCITY \
	(1 << 24) // Base velocity has been applied this frame (used to convert base velocity into momentum)
#define FL_WORLDBRUSH \
...
enum MoveCollide_t
{
	...
	MOVECOLLIDE_FLY_BOUNCE, // bounces, reflects, based on elasticity of surface and object - applies friction (adjust
							// velocity)
...
enum SolidFlags_t
{
	...
	FSOLID_FORCE_WORLD_ALIGNED =
		0x0040, // Forces the collision rep to be world-aligned even if it's SOLID_BSP or SOLID_VPHYSICS
