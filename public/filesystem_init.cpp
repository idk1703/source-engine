...
// The .xtx file only exists outside the zips (same as .txt and is made during the image build) and is read to setup the
// search paths. So all other code should be able to safely expect gameinfo.txt after the zip is mounted as the .txt
// file exists inside the zips. The .xtx concept is private and should only have to occurr here. As a safety measure, if
// the .xtx file is not found a retry is made with the original .txt name
...
FSReturnCode_t SetupFileSystemError(bool bRunVConfig, FSReturnCode_t retVal, const char *pMsg, ...)
{
	...
	// Run vconfig?
	// Don't do it if they specifically asked for it not to, or if they manually specified a vconfig with -game or
	// -vproject.
...
FSReturnCode_t LoadGameInfoFile(const char *pDirectoryName, KeyValues *&pMainFile, KeyValues *&pFileSystemInfo,
...
static void FileSystem_AddLoadedSearchPath(CFSSearchPathsInit &initInfo, const char *pPathID,
										   const char *fullLocationPath, bool bLowViolence)
{
...
#define BASESOURCEPATHS_TOKEN "|all_source_engine_paths|"
	...
	for(KeyValues *pCur = pSearchPaths->GetFirstValue(); pCur; pCur = pCur->GetNextValue())
	{
		...
		{
			// This is a special identifier that tells it to add the specified path for all source engine versions equal
			// to or prior to this version. So in Orange Box, if they specified:
			//		|all_source_engine_paths|hl2
			// it would add the ep2\hl2 folder and the base (ep1-era) hl2 folder.
			//
			// We need a special identifier in the gameinfo.txt here because the base hl2 folder exists in different
			// places. In the case of a game or a Steam-launched dedicated server, all the necessary prior engine
			// content is mapped in with the Steam depots, so we can just use the path as-is.
			...
			if(pszFoundShortName)
			{
				do
				{
					...
					if(pszFoundShortName[0] != '.' &&
					   (initInfo.m_pFileSystem->FindIsDirectory(findHandle) || V_stristr(pszFoundShortName, ".vpk")))
					{
						...
						if(!V_stricmp(pszFoundShortName, "materials") || !V_stricmp(pszFoundShortName, "maps") ||
						   !V_stricmp(pszFoundShortName, "resource") || !V_stricmp(pszFoundShortName, "scripts") ||
						   !V_stricmp(pszFoundShortName, "sound") || !V_stricmp(pszFoundShortName, "models"))
						{
		...
		FOR_EACH_VEC(vecFullLocationPaths, idxLocation)
		{
			FOR_EACH_VEC(vecPathIDs, idxPathID)
			{
				FileSystem_AddLoadedSearchPath(initInfo, vecPathIDs[idxPathID], vecFullLocationPaths[idxLocation],
											   bLowViolence);
			}
		}
	}
...
static FSReturnCode_t TryLocateGameInfoFile(char *pOutDir, int outDirLen, bool bBubbleDir)
{
	...
	for(char *pchFix = spchCopyNameBuffer.Get(), *pchEnd = pchFix + outDirLen; pchFix < pchEnd; ++pchFix)
	{
		if('\\' == *pchFix)
		{
			*pchFix = '/';
...
FSReturnCode_t LocateGameInfoFile(const CFSSteamSetupInfo &fsInfo, char *pOutDir, int outDirLen)
{
	...
	if(fsInfo.m_bOnlyUseDirectoryName)
	{
		if(!fsInfo.m_pDirectoryName)
			return SetupFileSystemError(false, FS_MISSING_GAMEINFO_FILE,
	...
	// Ask the application if it can provide us with a game info directory
	{
		...
		if(pfnSuggestGameInfoDirFn && (*pfnSuggestGameInfoDirFn)(&fsInfo, pOutDir, outDirLen, &bBubbleDir) &&
		   FS_OK == TryLocateGameInfoFile(pOutDir, outDirLen, bBubbleDir))
			return FS_OK;
	}

	// Try to use the environment variable / registry
	if((pProject = getenv(GAMEDIR_TOKEN)) != NULL && (Q_MakeAbsolutePath(pOutDir, outDirLen, pProject), 1) &&
...
bool DoesPathExistAlready(const char *pPathEnvVar, const char *pTestPath)
{
	...
	while(1)
	{
		...
		// Ok, we found pTestPath in the path, but it's only valid if it's followed by an optional slash and a
		// semicolon.
...
FSReturnCode_t FileSystem_GetFileSystemDLLName(char *pFileSystemDLL, int nMaxLen, bool &bSteam)
{
	...
	// Inside of here, we don't have a filesystem yet, so we have to assume that the filesystem_stdio or
	// filesystem_steam is in this same directory with us.
	char executablePath[MAX_PATH];
	if(!FileSystem_GetExecutableDir(executablePath, sizeof(executablePath)))
		return SetupFileSystemError(false, FS_INVALID_PARAMETERS, "FileSystem_GetExecutableDir failed.");
...
FSReturnCode_t FileSystem_LoadFileSystemModule(CFSLoadModuleInfo &fsInfo)
{
	...
	if(!Sys_LoadInterface(fsInfo.m_pFileSystemDLLName, FILESYSTEM_INTERFACE_VERSION, &fsInfo.m_pModule,
	...
	if(!fsInfo.m_pFileSystem->Connect(fsInfo.m_ConnectFactory))
		return SetupFileSystemError(false, FS_UNABLE_TO_INIT, "%s IFileSystem::Connect failed.",
									fsInfo.m_pFileSystemDLLName);

	if(fsInfo.m_pFileSystem->Init() != INIT_OK)
		return SetupFileSystemError(false, FS_UNABLE_TO_INIT, "%s IFileSystem::Init failed.",
									fsInfo.m_pFileSystemDLLName);
