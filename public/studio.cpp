...
mstudioanim_t *mstudioanimdesc_t::pAnim(int *piFrame, float &flStall) const
{
	...
	else if(panim != NULL && zeroframestalltime != 0.0f)
	{
		...
		{
			Msg("[%8.3f] stall blend %.2f on %s:%s:%d:%d\n", Plat_FloatTime(), flStall, pStudiohdr()->pszName(),
				pszName(), section, block);
		}
	}
...
const studiohdr_t *CStudioHdr::GroupStudioHdr(int i)
{
	...
	if(!m_pStudioHdrCache.IsValidIndex(i))
	{
		...
		return m_pStudioHdr; // return something known to probably exist, certainly things will be messed up, but
							 // hopefully not crash before the warning is noticed
	}
			...
			switch(pops->op)
			{
				...
				case STUDIO_DME_LOWER_EYELID:
				{
					...
					const mstudioflexcontroller_t *const pCloseLid =
						pFlexcontroller(static_cast<LocalFlexController_t>((int)stack[k - 1]));
					...
					if(nBlinkIndex >= 0)
					{
						const mstudioflexcontroller_t *const pBlink =
							pFlexcontroller(static_cast<LocalFlexController_t>((int)stack[k - 2]));
					...
					if(nEyeUpDownIndex >= 0)
					{
						const mstudioflexcontroller_t *const pEyeUpDown =
							pFlexcontroller(static_cast<LocalFlexController_t>((int)stack[k - 3]));
						flEyeUpDown = RemapValClamped(src[pEyeUpDown->localToGlobal], pEyeUpDown->min, pEyeUpDown->max,
													  -1.0f, 1.0f);
					}
...
void CStudioHdr::CActivityToSequenceMapping::Initialize(CStudioHdr *__restrict pstudiohdr)
{
	...
	for(int i = 0; i < NumSeq; ++i)
	{
		...
		{
			// AssertMsg2( false, "Sequence %d on studiohdr %s didn't have its activity initialized!", i,
			// pstudiohdr->pszName() );
	...
	int sequenceCount = 0;
	int topActivity =
		0; // this will store the highest seen activity number (used later to make an ad hoc map on the stack)
...
#define ALIGN_VALUE(val, alignment) ((val + alignment - 1) & ~(alignment - 1)) //  need macro for constant expression
	...
	for(int i = 0; i < NumSeq; ++i)
	{
		...
		if(seqdesc.activity >= 0)
		{
			const HashValueType &element =
				m_ActToSeqHash[m_ActToSeqHash.Find(HashValueType(seqdesc.activity, 0, 0, 0))];
			...
			if(seqdesc.numactivitymodifiers > 0)
			{
				...
				(tupleList + element.startingIdx + tupleOffset)->pActivityModifiers =
					new CUtlSymbol[seqdesc.numactivitymodifiers];
...
const CStudioHdr::CActivityToSequenceMapping::SequenceTuple *CStudioHdr::CActivityToSequenceMapping::GetSequences(
	int forActivity, int *__restrict outSequenceCount, int *__restrict outTotalWeight)
{
