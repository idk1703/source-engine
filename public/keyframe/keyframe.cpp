...
{
	Catmull_Rom_Spline(g_KeyFramePtr[-1].vPos, g_KeyFramePtr[0].vPos, g_KeyFramePtr[1].vPos, g_KeyFramePtr[2].vPos,
					   time, vOut);
}
...
void CPositionInterpolator_Rope::InterpolatePosition(float time, Vector &vOut)
{
	...
	VectorLerp(m_RopePhysics.GetNode(iNode)->m_vPredicted, m_RopePhysics.GetNode(iNode + 1)->m_vPredicted,
			   flNode - iNode, vOut);
}
...
PositionInterpolatorCreateFn g_PositionInterpolatorCreateFns[] = {GetLinearInterpolator, GetCatmullRomInterpolator,
																  GetRopeInterpolator};
