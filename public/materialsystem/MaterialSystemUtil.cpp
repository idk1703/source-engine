...
void CMaterialReference::Init(char const *pMaterialName, const char *pTextureGroupName, bool bComplain)
{
	...
	if(IsErrorMaterial(pMaterial))
	{
		if(IsOSX())
		{
			printf("\n ##### CMaterialReference::Init got error material for %s in tex group %s", pMaterialName,
				   pTextureGroupName);
		}
	}
...
void CTextureReference::InitProceduralTexture(const char *pTextureName, const char *pTextureGroupName, int w, int h,
...
void CTextureReference::InitRenderTarget(int w, int h, RenderTargetSizeMode_t sizeMode, ImageFormat fmt,
										 ...
										 char *pStrOptionalName /* = NULL */)
{
	...
	m_pTexture = materials->CreateNamedRenderTargetTextureEx(pStrOptionalName, w, h, sizeMode, fmt, depth, textureFlags,
															 renderTargetFlags);
...
//-----------------------------------------------------------------------------
#if defined(_X360)
void CTextureReference::InitRenderTargetTexture(int w, int h, RenderTargetSizeMode_t sizeMode, ImageFormat fmt,
