...
{
	return reinterpret_cast<float *>(reinterpret_cast<unsigned char *>(pBufferPointer) + nVertexCount * vertexSize);
}

inline const float *OffsetFloatPointer(const float *pBufferPointer, int nVertexCount, int vertexSize)
{
	return reinterpret_cast<const float *>(reinterpret_cast<unsigned char const *>(pBufferPointer) +
...
public:
	...
	void Fast4VerticesSSE(ModelVertexDX7_t const *vtx_a, ModelVertexDX7_t const *vtx_b, ModelVertexDX7_t const *vtx_c,
...
inline void CVertexBuilder::Bind(IMatRenderContext *pContext, int nStreamID, VertexFormat_t usage)
{
	...
	{
		pContext->BindVertexBuffer(nStreamID, m_pVertexBuffer, m_nBufferOffset, m_nFirstVertex, m_nTotalVertexCount,
...
inline void CVertexBuilder::ValidateCompressionType()
{
	...
	if(vbCompressionType != VERTEX_COMPRESSION_NONE)
	{
		...
		{
			Warning("ERROR: CVertexBuilder::SetCompressionType() must be called to specify the same vertex compression "
					"type (%s) as the vertex buffer being modified."
...
inline void CVertexBuilder::Fast4VerticesSSE(ModelVertexDX7_t const *vtx_a, ModelVertexDX7_t const *vtx_b,
											 ModelVertexDX7_t const *vtx_c, ModelVertexDX7_t const *vtx_d)
{
	Assert(m_CompressionType == VERTEX_COMPRESSION_NONE); // FIXME: support compressed verts if needed
...
inline void CVertexBuilder::VertexDX8ToX360(const ModelVertexDX8_t &vertex)
{
	...
	if(m_VertexSize_BoneWeight)
	{
		...
		Assert(GetVertexElementSize(VERTEX_ELEMENT_BONEWEIGHTS2, VERTEX_COMPRESSION_NONE) ==
		...
		{
			Assert(GetVertexElementSize(VERTEX_ELEMENT_BONEINDEX, VERTEX_COMPRESSION_NONE) ==
	...
	{
		Assert(GetVertexElementSize(VERTEX_ELEMENT_TEXCOORD2D_0, VERTEX_COMPRESSION_NONE) ==
...
inline void CVertexBuilder::Normal3f(float nx, float ny, float nz)
{
	Assert(m_CompressionType ==
		   VERTEX_COMPRESSION_NONE); // Use the templatized version if you want to support compression
	Assert(m_pNormal);
...
// note that on the OSX target (OpenGL) whenever there is vertex data being written as bytes - they need to be written
// in R,G,B,A memory order
...
inline void CVertexBuilder::TexCoordSubRect2f(int stage, float s, float t, float offsetS, float offsetT, float scaleS,
											  float scaleT)
{
	Assert(m_pTexCoord[stage] && m_pCurrTexCoord[stage]);
	Assert(IsFinite(s) && IsFinite(t));
...
#else //( COMPRESSED_NORMALS_TYPE == COMPRESSED_NORMALS_COMBINEDTANGENTS_UBYTE4 )
	  // FIXME: add a combined CompressedNormalAndTangent() accessor, to avoid reading back from write-combined memory
	  // here The normal should have already been written into the lower 16 bits - here, we OR in the tangent into the
	  // upper 16 bits
...
// Constructor
//-----------------------------------------------------------------------------
inline CIndexBuilder::CIndexBuilder() : m_pIndexBuffer(0), m_nIndexCount(0), m_nCurrentIndex(0), m_nMaxIndexCount(0)
{
...
public:
	...
	void Begin(IVertexBuffer *pVertexBuffer, IIndexBuffer *pIndexBuffer, MaterialPrimitiveType_t type, int nVertexCount,
	...
	// Passing nVertexCount == -1 says to lock all the vertices for modification.
	// Pass 0 for nIndexCount to not lock the index buffer.
	void BeginModify(IMesh *pMesh, int nFirstVertex = 0, int nVertexCount = -1, int nFirstIndex = 0,
					 int nIndexCount = 0);
	void EndModify(bool bSpewData = false);

	// A helper method since this seems to be done a whole bunch.
	void DrawQuad(IMesh *pMesh, const float *v1, const float *v2, const float *v3, const float *v4,
				  unsigned char const *pColor, bool wireframe = false);
...
private:
	...
	void ComputeNumVertsAndIndices(int *pMaxVertices, int *pMaxIndices, MaterialPrimitiveType_t type,
								   int nPrimitiveCount);
...
inline void CMeshBuilder::Begin(IVertexBuffer *pVertexBuffer, IIndexBuffer *pIndexBuffer, MaterialPrimitiveType_t type,
								int nVertexCount, int nIndexCount, int *nFirstVertex)
{
	Assert(0);
...
//-----------------------------------------------------------------------------
inline void CMeshBuilder::ComputeNumVertsAndIndices(int *pMaxVertices, int *pMaxIndices, MaterialPrimitiveType_t type,
													int nPrimitiveCount)
{
	switch(type)
	{
...
// Locks the vertex buffer to modify existing data
//-----------------------------------------------------------------------------
inline void CMeshBuilder::BeginModify(IMesh *pMesh, int nFirstVertex, int nVertexCount, int nFirstIndex,
									  int nIndexCount)
{
	Assert(pMesh && (!m_pMesh));
...
// A helper method since this seems to be done a whole bunch.
//-----------------------------------------------------------------------------
inline void CMeshBuilder::DrawQuad(IMesh *pMesh, const float *v1, const float *v2, const float *v3, const float *v4,
								   unsigned char const *pColor, bool wireframe)
{
	if(!wireframe)
	{
...
FORCEINLINE void CMeshBuilder::Fast4VerticesSSE(const ModelVertexDX7_t *vtx_a, const ModelVertexDX7_t *vtx_b,
...
FORCEINLINE void CMeshBuilder::TexCoordSubRect2f(int nStage, float s, float t, float offsetS, float offsetT,
