...
public:
	...
	// creates a texture compositor that will attempt to composite a new textuer from the steps of the specified
	// KeyValues.
	...
	// Loads the texture with the specified name, calls pRecipient->OnAsyncFindComplete with the result from the main
	// thread. once the texture load is complete. If the texture cannot be found, the returned texture will return true
	// for IsError().
...
public:
	...
	// rendering clip planes, beware that only the most recently pushed plane will actually be used in a sizeable chunk
	// of hardware configurations and that changes to the clip planes mid-frame while UsingFastClipping() is true will
...
#if defined(_X360)
	// Seems best to expose GPR allocation to scene rendering code. 128 total to split between vertex/pixel shaders
	// (pixel will be set to 128 - vertex). Minimum value of 16. More GPR's = more threads.
	...
	// Create a texture from the specified src render target, then call pRecipient->OnAsyncCreateComplete from the main
	// thread. The texture will be created using the destination format, and will optionally have mipmaps generated. In
	// case of error, the provided callback function will be called with the error texture.
