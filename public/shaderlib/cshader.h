...
#define SOFTWARE_VERTEX_SHADER(name)                                                          \
	static void SoftwareVertexShader_##name(CMeshBuilder &meshBuilder, IMaterialVar **params, \
...
#define USE_SOFTWARE_VERTEX_SHADER(name) m_SoftwareVertexShader = SoftwareVertexShader_##name

#define SHADER_INIT_PARAMS() virtual void OnInitShaderParams(IMaterialVar **params, const char *pMaterialName)

#define SHADER_FALLBACK virtual char const *GetFallbackShader(IMaterialVar **params) const
...
#define __BEGIN_SHADER_INTERNAL(_baseclass, name, help, flags)                                                    \
	...
	{                                                                                                             \
		...
		{                                                                                                         \
		public:                                                                                                   \
			CShaderParam(ShaderMaterialVars_t var, ShaderParamType_t type, const char *pDefaultParam,             \
						 const char *pHelp, int nFlags)                                                           \
			{                                                                                                     \
...
// 		static CShaderParam COLOR( (ShaderMaterialVars_t)COLOR, SHADER_PARAM_TYPE_COLOR, "{255 255 255}", "unused",
// SHADER_PARAM_NOT_EDITABLE ) and GCC thinks that the reference to COLOR in the arg list is actually a reference to the
// object we're in the middle of making. and you get --> error: invalid cast from type ‘Cloak_DX90::CShaderParam’ to
// type ‘ShaderMaterialVars_t’ Resolved: add the "::" so compiler knows that reference is to the enum, not to the name
