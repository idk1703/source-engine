...
namespace vgui
{
	...
	public:
		...
		enum ColumnFlags_e
		{
			...
			COLUMN_IMAGE = 0x02,  // set if the column contains an image instead of text (images are looked up by index
								  // from the ImageList) (see SetImageList below)
		...
		virtual bool AddColumnToSection(int sectionID, const char *columnName, const char *columnText, int columnFlags,
										int width, HFont fallbackFont = INVALID_FONT);
		virtual bool AddColumnToSection(int sectionID, const char *columnName, const wchar_t *columnText,
