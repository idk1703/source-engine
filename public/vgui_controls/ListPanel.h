...
namespace vgui
{
	...
	typedef int __cdecl SortFunc(ListPanel *pPanel, const ListPanelItem &item1, const ListPanelItem &item2);
	...
	public:
		...
		virtual void AddColumnHeader(int index, const char *columnName, const char *columnText, int startingWidth,
									 int minWidth, int maxWidth, int columnFlags = 0);
		virtual void AddColumnHeader(int index, const char *columnName, const char *columnText, int width,
									 int columnFlags = 0);
		...
		virtual int AddItem(
			const KeyValues *data, unsigned int userData, bool bScrollToItem,
			bool bSortOnAdd); // Takes a copy of the data for use in the table. Returns the index the item is at.
		void SetItemDragData(
			int itemID, const KeyValues *data); // Makes a copy of the keyvalues to store in the table. Used when
												// dragging from the table. Only used if the caller enables drag support
		...
		virtual void ApplyItemChanges(
			int itemID); // applies any changes to the data, performed by modifying the return of GetItem() above
		virtual void RemoveItem(
			int itemID);			   // removes an item from the table (changing the indices of all following items)
		...
		virtual void GetCellText(int itemID, int column, OUT_Z_BYTECAP(bufferSizeInBytes) wchar_t *buffer,
								 int bufferSizeInBytes); // returns the data held by a specific cell
		virtual IImage *GetCellImage(int itemID,
									 int column); //, ImagePanel *&buffer); // returns the image held by a specific cell
		...
		virtual bool GetCellAtPos(
			int x, int y, int &row,
			int &column); // returns true if any found, row and column are filled out. x, y are in screen space
	...
	private:
		...
		// persistent list of all columns ever created, indexes into m_ColumnsData - used for matching up DATAITEM
		// m_SortedTreeIndexes
