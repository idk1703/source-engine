...
#define PANEL_ROUND_CORNER_ALL                                                                    \
	...
	public:
		...
		void MakeReadyForUse(); // fully construct this panel so its ready for use right now (i.e fonts loaded, colors
								// set, default label text set, ...)
		...
		virtual VPANEL IsWithinTraverse(
			int x, int y, bool traversePopups); // recursive; returns a pointer to the panel at those coordinates
		...
		virtual void SetAutoDelete(
			bool state); // if set to true, panel automatically frees itself when parent is deleted
		...
		virtual bool RequestInfo(
			KeyValues *outputData); // returns true if output is successfully written.  You should always chain back to
									// the base class if info request is not handled
		...
		virtual void InstallMouseHandler(
			Panel *pHandler); // mouse events will be send to handler panel instead of this panel
		...
		virtual void SetPaintBackgroundType(int type); // 0 for normal(opaque), 1 for single texture from Texture1, and
													   // 2 for rounded box w/ four corner textures
		...
		static wchar_t const *KeyCodeModifiersToDisplayString(KeyCode code,
															  int modifiers); // L"Ctrl+Alt+Shift+Backspace"
		...
		static void SaveKeyBindingsToFile(KeyBindingContextHandle_t handle, char const *filename,
										  char const *pathID = 0);
		...
		// If a panel implements this and returns true, then the IsKeyRebound check will fail and OnKeyCodeTyped
		...
		virtual void DrawBoxFade(int x, int y, int wide, int tall, Color color, float normalizedAlpha,
								 unsigned int alpha0, unsigned int alpha1, bool bHorizontal, bool hollow = false);
		virtual void DrawHollowBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha);
		//=============================================================================
		// HPE_BEGIN:
		//=============================================================================

		// [menglish] Draws a hollow box similar to the already existing draw hollow box function, but takes the indents
		// as params
		...
		// Use this to prevent chaining up from a parent which can mess with mouse functionality if you don't want to
		// chain up from a child panel to the best
		//  draggable parent.
		...
		// If hover context time is non-zero, then after the drop cursor is hovering over the panel for that amount of
		// time the Show hover context menu function will be invoked
		virtual void SetDropEnabled(bool enabled, float m_flHoverContextTime = 0.0f);
		virtual bool IsDropEnabled() const;

		// Called if m_flHoverContextTime was non-zero, allows droppee to preview the drop data and show an appropriate
		// menu Return false if not using context menu
		...
		// if set, Panel gets PerformLayout called after the camera and the renderer's m_matrixWorldToScreen has been
		// setup, so panels can be correctly attached to entities in the world
	...
	private:
		...
		bool m_bWorldPositionCurrentFrame; // if set, Panel gets PerformLayout called after the camera and the
