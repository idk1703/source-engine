...
CBlockingUDPSocket::CBlockingUDPSocket() : m_cserIP(), m_Socket(0), m_pImpl(new CImpl)
{
	CreateSocket();
}
...
unsigned int CBlockingUDPSocket::ReceiveSocketMessage(struct sockaddr_in *packet_from, unsigned char *buf,
													  size_t bufsize)
{
	...
	int packet_length = VCRHook_recvfrom(m_Socket, (char *)buf, (int)bufsize, 0, &fromaddress, &fromlen);
...
bool CBlockingUDPSocket::SendSocketMessage(const struct sockaddr_in &rRecipient, const unsigned char *buf,
										   size_t bufsize)
{
	// Send data
	int bytesSent = sendto(m_Socket, (const char *)buf, (int)bufsize, 0,
						   reinterpret_cast<const sockaddr *>(&rRecipient), sizeof(rRecipient));
