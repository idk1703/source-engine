...
void ConnectHaptics(CreateInterfaceFn appFactory)
{
	...
	if(pFalconModule)
	{
		...
		if(factory)
		{
			...
			if(haptics &&
			   haptics->Initialize(engine, view, g_InputInternal, gpGlobals, appFactory, g_pVGuiInput->GetIMEWindow(),
								   filesystem, enginevgui, ActivityList_IndexForName, ActivityList_NameForIndex))
			{
				success = true;
...
#ifdef TERROR
	else if((bitDamageType &
			 (DMG_PARALYZE | DMG_NERVEGAS | DMG_POISON | DMG_RADIATION | DMG_DROWNRECOVER | DMG_ACID | DMG_SLOWBURN)) &&
			(bitDamageType & ~(DMG_PARALYZE | DMG_NERVEGAS | DMG_POISON | DMG_RADIATION | DMG_DROWNRECOVER | DMG_ACID |
							   DMG_SLOWBURN)) == 0)
	{
