...
#define SPROP_CHANGES_OFTEN \
	(1 << 10) // this is an often changed field, moved to head of sendtable so it gets a small index
...
#define SPROP_COORD_MP_LOWPRECISION \
	(1 << 14) // Like SPROP_COORD, but special handling for multiplayer games where the fractional component only gets a
			  // 3 bits instead of 5
#define SPROP_COORD_MP_INTEGRAL (1 << 15) // SPROP_COORD_MP, but coordinates are rounded to integral boundaries

#define SPROP_VARINT \
	SPROP_NORMAL // reuse existing flag so we don't break demo. note you want to include SPROP_UNSIGNED if needed, its
				 // more efficient

#define SPROP_NUMFLAGBITS_NETWORKED 16

// This is server side only, it's used to mark properties whose SendProxy_* functions encode against
// gpGlobals->tickcount (the only ones that currently do this are
