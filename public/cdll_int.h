...
public:
	...
	virtual IMaterial *TraceLineMaterialAndLighting(const Vector &start, const Vector &end, Vector &diffuseLightColor,
													Vector &baseColor) = 0;
	...
	// Note: Calls to this are checked against FCVAR_CLIENTCMD_CAN_EXECUTE (if that bit is not set, then this function
	// can't change it).
	...
	// Given a CAudioSource (opaque pointer), retrieve the underlying CSentence object ( stores the words, phonemes, and
	// close
	//  captioning data )
	...
	// Given the name of the key "mouse1", "e", "tab", etc., return the string it is bound to "+jump", "impulse 50",
	// etc.
	...
	// Allow the sound system to paint additional data (during lengthy rendering operations) to prevent stuttering
	// sound.
	...
	// The save restore system allocates memory from a shared memory pool, use this allocator to allocate/free
	// saverestore
	//  memory.
	...
	// By default, Valve games restrict the server to only being able to execute commands marked with
	// FCVAR_SERVER_CAN_EXECUTE. By default, mods are allowed to execute any server commands, and they can restrict the
...
public:
	...
	//  This call is used to try to prevent the mouse from appearing out of the side of a windowed version of the engine
	//  if rendering or other processing is taking too long
	...
	// This function is called once per tick to create the player CUserCmd (used for prediction/physics simulation of
	// the player) Because the mouse can be sampled at greater than the tick interval, there is a separate
	...
	// If the game is running faster than the tick_interval framerate, then we do extra mouse sampling to avoid jittery
	// input
