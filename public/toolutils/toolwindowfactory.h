...
public:
	virtual vgui::ToolWindow *InstanceToolWindow(
		...
		vgui::Panel *firstPage, char const *title,
...
//-----------------------------------------------------------------------------
template<class T>
vgui::ToolWindow *CToolWindowFactory<T>::InstanceToolWindow(vgui::Panel *parent, bool contextLabel,
															vgui::Panel *firstPage, char const *title, bool contextMenu)
{
	Assert(parent);
	if(!parent)
		return NULL;
