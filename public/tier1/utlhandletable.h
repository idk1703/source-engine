...
void CUtlHandleTable<T, HandleBits>::RemoveHandle(UtlHandle_t handle)
{
	...
	//  serial # wraps around we'll possibly revalidate old handles and they'll start to point at the wrong objects.
...
//-----------------------------------------------------------------------------
template<class T, int HandleBits>
const typename CUtlHandleTable<T, HandleBits>::EntryType_t *CUtlHandleTable<T, HandleBits>::GetEntry(
	UtlHandle_t handle, bool checkValidity) const
{
	if(handle == UTLHANDLE_INVALID)
		return NULL;
