...
public:
	...
	// KeyValues::AutoDelete instance:   call_my_function( KeyValues::AutoDelete( new KeyValues( "test" ) ) )
	//
	class AutoDelete
	{
	public:
	...
	bool SaveToFile(IBaseFileSystem *filesystem, const char *resourceName, const char *pathID = NULL,
					bool sortKeys = false, bool bAllowEmptyString = false, bool bCacheResult = false);
	...
	KeyValues *FindLastSubKey(); // returns the LAST subkey in the list.  This requires a linked list iteration to find
								 // the key.  Returns NULL if we don't have any children
...
private:
	...
	char m_bEvaluateConditionals; // true, if while parsing this KeyValue, conditionals blocks are evaluated (default
								  // true)
	char unused[1];
