...
Data *CTHash<Data, I>::PvRecordFind(I unKey) const
{
	...
	for(RecHdr_t *pRecHdr = m_pBucket[iBucket].m_pRecHdrFirst; NULL != pRecHdr && pRecHdr->m_iBucket == iBucket;
...
Data *CTHash<Data, I>::PvRecordRun()
{
	...
	for(;;)
	{
		...
		if(0 != m_pRecHdrRunNext->m_iCycleLast)
		{
			...
			{
				DMsg(SPEW_CONSOLE, 1,
					 "Double cycle: hdr = 0x%x, last frame = %d, curFrame = %d, first = %d, last = %d, bucket = %d\n",
					 m_pRecHdrRunNext, m_pRecHdrRunNext->m_iFrameLast, m_iFrame, m_iBucketRunFirst, m_iBucketRunMax,
