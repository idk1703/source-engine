...
public:
	...
	FORCEINLINE void *RowPtr(int nAttributeIdx, int nRowNumber, int nSliceNumber = 0) const
	{
		...
		return m_pAttributePtrs[nAttributeIdx] + +nRowNumber * m_nRowStrideInBytes[nAttributeIdx] +
	...
	template<class T>
	FORCEINLINE T *ElementPointer(int nAttributeIdx, int nX = 0, int nY = 0, int nZ = 0) const
	{
		Assert(nAttributeIdx < MAX_SOA_FIELDS);
		...
		return reinterpret_cast<T *>(m_pAttributePtrs[nAttributeIdx] + nX * sizeof(float) +
	...
	void FillAttrWithInterpolatedValues(int nAttr, float flValue00, float flValue10, float flValue01,
										float flValue11) const;
	void FillAttrWithInterpolatedValues(int nAttr, Vector flValue00, Vector flValue10, Vector const &flValue01,
										Vector const &flValue11) const;
};
...
class CFltX4AttributeWriteIterator : public CStridedPtr<fltx4>
{
	FORCEINLINE CFltX4AttributeWriteIterator(CSOAContainer const *pContainer, int nAttribute, int nRowNumber = 0)
		: CStridedPtr<fltx4>(pContainer->RowPtr(nAttribute, nRowNumber), pContainer->ItemByteStride(nAttribute))
	{
	}
};
