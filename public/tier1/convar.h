...
public:
	...
	// These just call into the IConCommandBaseAccessor to check flags and set the var (which ends up calling
	// InternalSetValue).
...
private:
	...
	void Create(const char *pName, const char *pDefaultValue, int flags = 0, const char *pHelpString = 0,
				bool bMin = false, float fMin = 0.0, bool bMax = false, float fMax = 0.0, bool bCompMin = false,
				float fCompMin = 0.0, bool bCompMax = false, float fCompMax = 0.0, FnChangeCallback_t callback = 0);
...
FORCEINLINE_CVAR bool ConVar::IsCompetitiveRestricted() const
{
	...
	const bool bInternalUseOnly = (nFlags & (FCVAR_HIDDEN | FCVAR_DEVELOPMENTONLY | FCVAR_INTERNAL_USE | FCVAR_GAMEDLL |
											 FCVAR_REPLICATED | FCVAR_CHEAT)) != 0;
...
public:
	CConCommandMemberAccessor(T *pOwner, const char *pName, FnMemberCommandCallback_t callback,
...
#define CON_COMMAND_MEMBER_F(_thisclass, name, _funcname, description, flags)               \
	...
	public:                                                                                 \
		CCommandMemberInitializer_##_funcname()                                             \
			: m_ConCommandAccessor(NULL, name, &_thisclass::_funcname, description, flags)  \
		{                                                                                   \
