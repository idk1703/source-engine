...
void CUtlQueue<T, M>::Insert(T const &element)
{
	...
	else
	{
		...
		if(nextTail == m_head) // if non-empty, and growing by 1 appears to make the queue of length 1, then we were
							   // already full before the Insert
		{
			...
			{
				// if the queue wraps around the end of m_memory, move the part at the end of memory to the new end of
				// memory
