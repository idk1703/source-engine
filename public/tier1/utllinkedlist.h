...
#define FOR_EACH_LL(listName, iteratorName)                                                                            \
	for(int iteratorName = (listName).Head(); (listName).IsUtlLinkedList && iteratorName != (listName).InvalidIndex(); \
...
public:
	...
	public:
		...
		_CUtlLinkedList_constiterator_t() : m_list(0), m_index(List_t::InvalidIndex()) {}
		// Normal constructor.
		_CUtlLinkedList_constiterator_t(const List_t &list, IndexType_t index) : m_list(&list), m_index(index) {}
...
#ifdef _DEBUG // it's safe to skip this here, since the only way to get indices after m_LastAlloc is to use
			  // MaxElementIndex
...
void CUtlLinkedList<T, S, ML, I, M>::Purge()
{
	...
	// Routing "m_LastAlloc = m_Memory.InvalidIterator();" through a local const to sidestep an internal compiler error
	// on 360 builds
...
I CUtlLinkedList<T, S, ML, I, M>::AllocInternal(bool multilist)
{
	...
	if(m_FirstFree == InvalidIndex())
	{
		...
		typename M::Iterator_t it =
			m_Memory.IsValidIterator(m_LastAlloc) ? m_Memory.Next(m_LastAlloc) : m_Memory.First();
