...
BEGIN_BYTESWAP_DATADESC(xZipHeader_t)
	DEFINE_FIELD(Magic, FIELD_INTEGER), DEFINE_FIELD(Version, FIELD_INTEGER),
		DEFINE_FIELD(PreloadDirectoryEntries, FIELD_INTEGER), DEFINE_FIELD(DirectoryEntries, FIELD_INTEGER),
		DEFINE_FIELD(PreloadBytes, FIELD_INTEGER), DEFINE_FIELD(HeaderLength, FIELD_INTEGER),
		DEFINE_FIELD(FilenameEntries, FIELD_INTEGER), DEFINE_FIELD(FilenameStringsOffset, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(xZipFilenameEntry_t)
	DEFINE_FIELD(FilenameCRC, FIELD_INTEGER), DEFINE_FIELD(FilenameOffset, FIELD_INTEGER),
...
bool CXZip::Load(FILE *handle, int nOffset, int nSize,
				 bool bPreload) // Load a pack file into this instance.  Returns true on success.
{
	Unload();
	...
	// Read the directory:
	{
		...
		m_pRead(m_pDirectory, m_Header.HeaderLength, -1, sizeof(xZipDirectoryEntry_t) * m_Header.DirectoryEntries,
				m_hUser);
...
void CXZip::PreloadData()
{
	...
	m_nRegular2PreloadEntryMapping =
		(unsigned short *)(((unsigned char *)m_pPreloadDirectory) +
						   (sizeof(xZipDirectoryEntry_t) * m_Header.PreloadDirectoryEntries));

	// Swap the regular to preload mapping
	if(m_bByteSwapped)
	{
		m_Swap.SwapBufferToTargetEndian<short>((short *)m_nRegular2PreloadEntryMapping,
...
char *CXZip::GetEntryFileName(unsigned CRC, char *pDefault)
{
	...
	else
	{
		...
		xZipFilenameEntry_t *found =
			(xZipFilenameEntry_t *)bsearch(&entry, m_pFilenames, m_Header.FilenameEntries, sizeof(xZipFilenameEntry_t),
...
int CXZip::ReadIndex(int nEntryIndex, int nFileOffset, int nDestBytes, int nLength, void *pBuffer)
{
	...
	// HACK HACK HACK - convert the pack file index to a local file index (ie, assuming the full file index is being
	// passed in)
	...
	if(pPreloadEntry)
	{
		...
		if(nFileOffset + nLength <= (int)pPreloadEntry->Length)
		{
			...
			memcpy(pBuffer, (char *)m_pPreloadedData + pPreloadEntry->StoredOffset + nFileOffset - m_nPreloadStart,
				   nLength);
			return nLength;
		}
	}
...
bool CXZip::GetSimpleFileOffsetLength(const char *FileName, int &nBaseIndex, int &nFileOffset, int &nLength)
{
	...
	xZipDirectoryEntry_t *found =
		(xZipDirectoryEntry_t *)bsearch(&entry, m_pDirectory, m_Header.DirectoryEntries, sizeof(xZipDirectoryEntry_t),
...
{
	xZipDirectoryEntry_t *l = (xZipDirectoryEntry_t *)left, *r = (xZipDirectoryEntry_t *)right;
...
{
	xZipFilenameEntry_t *l = (xZipFilenameEntry_t *)left, *r = (xZipFilenameEntry_t *)right;
...
void AddFilename(const char *filename)
{
	...
	pFilenameEntries =
		(xZipFilenameEntry_t *)realloc(pFilenameEntries, sizeof(xZipFilenameEntry_t) * Header.FilenameEntries);
...
bool xZipAddFile(const char *filename, CUtlBuffer &fileBuff, bool bPrecacheEntireFile, bool bProcessPrecacheHeader,
				 bool bProcessPrecacheHeaderOnly)
{
	...
	if(customPreloadSize > 0)
	{
		...
		pPreloadDirectoryEntries = (xZipDirectoryEntry_t *)realloc(
			pPreloadDirectoryEntries, sizeof(xZipDirectoryEntry_t) * Header.PreloadDirectoryEntries);
	...
	pDirectoryEntries =
		(xZipDirectoryEntry_t *)realloc(pDirectoryEntries, sizeof(xZipDirectoryEntry_t) * Header.DirectoryEntries);
	memcpy(pDirectoryEntries + Header.DirectoryEntries - 1, &entry, sizeof(entry));

	WriteFileBytes(hTempFileData, fileBuff, entry.Length);

	// Align the data region to a 512 byte boundry:  (has to be on last entry as well to ensure enough space to perform
	// the final read, and initial alignment is taken careof by assembexzip)
...
bool xZipAddFile(const char *zipname, bool bPrecacheEntireFile, bool bProcessPrecacheHeader,
				 bool bProcessPrecacheHeaderOnly)
{
...
bool xZipEnd()
{
	...
	if(Header.PreloadDirectoryEntries)
	{
		...
		Header.PreloadBytes =
			ftell(hTempFilePreload) + nPreloadDirectorySize + nRegular2PreloadSize; // Raw# of bytes to preload
	...
	// Write out the header: (will need to be rewritten at the end as well) - note: not even bothering to byteswap at
	// this point
	...
	qsort(pDirectoryEntries, Header.DirectoryEntries, sizeof(xZipDirectoryEntry_t),
	...
	if(Header.PreloadBytes > 0)
	{
		...
		qsort(pPreloadDirectoryEntries, Header.PreloadDirectoryEntries, sizeof(xZipDirectoryEntry_t),
			  &xZipDirectoryEntry_t::xZipDirectoryEntrySortCompare);
		...
		g_xzpSwap.SwapBufferToTargetEndian<short>((short *)Regular2Preload, (short *)Regular2Preload,
		...
		fseek(hTempFilePreload, 0, SEEK_SET);
		CopyFileBytes(hOutputFile, hTempFilePreload,
					  Header.PreloadBytes - nPreloadDirectorySize - nRegular2PreloadSize);
	}
	...
	if(nFilenameDataLength && Header.FilenameEntries)
	{
		...
		for(unsigned i = 0; i < Header.FilenameEntries; i++)
		{
			pFilenameEntries[i].FilenameOffset +=
				(Header.FilenameStringsOffset + (Header.DirectoryEntries * sizeof(xZipFilenameEntry_t)));
		}
	...
	printf("\n\nSummary: Input:%u, XZip:%u, Directory Entries:%u (%u preloaded), Preloaded Bytes:%u\n\n",
		   InputFileBytes, OutputFileBytes, Header.DirectoryEntries, Header.PreloadDirectoryEntries,
		   Header.PreloadBytes);
