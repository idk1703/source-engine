...
protected:
	...
	int AABBTree_BuildTreeTrisInSphere_r(const Vector &center, float radius, int iNode, unsigned short *pIndexOut,
										 unsigned short indexMax);

	void AABBTree_TreeTrisRayTest(const Ray_t &ray, const Vector &vecInvDelta, int iNode, CBaseTrace *pTrace,
								  bool bSide, CDispCollTri **pImpactTri);
	void AABBTree_TreeTrisRayBarycentricTest(const Ray_t &ray, const Vector &vecInvDelta, int iNode,
	...
	{
		AABBTree_TreeTrisSweepTest_Args_t(const Ray_t &ray, const Vector &vecInvDelta, const Vector &rayDir,
										  CBaseTrace *pTrace)
...
protected:
	void SweepAABBTriIntersect(const Ray_t &ray, const Vector &rayDir, int iTri, CDispCollTri *pTri,
							   CBaseTrace *pTrace);

	void Cache_Create(CDispCollTri *pTri, int iTri); // Testing!
	bool Cache_EdgeCrossAxisX(const Vector &vecEdge, const Vector &vecOnEdge, const Vector &vecOffEdge,
							  CDispCollTri *pTri, unsigned short &iPlane);
	bool Cache_EdgeCrossAxisY(const Vector &vecEdge, const Vector &vecOnEdge, const Vector &vecOffEdge,
							  CDispCollTri *pTri, unsigned short &iPlane);
	bool Cache_EdgeCrossAxisZ(const Vector &vecEdge, const Vector &vecOnEdge, const Vector &vecOffEdge,
							  CDispCollTri *pTri, unsigned short &iPlane);
	...
	// Utility
	inline void CalcClosestBoxPoint(const Vector &vecPlaneNormal, const Vector &vecBoxStart,
