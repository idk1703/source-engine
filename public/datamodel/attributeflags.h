...
enum
{
	...
	FATTRIB_TOPOLOGICAL =
		(1 << 10), // Indicates this attribute effects the scene's topology (ie it's an attribute name or element)
	...
	FATTRIB_USERDEFINED =
		(1 << 14), // This flag is used to sort attributes in the element properties view. User defined flags come last.
	FATTRIB_NODUPLICATES =
		(1 << 15), // For element array types, disallows duplicate values from being inserted into the array.
	FATTRIB_HAS_ARRAY_CALLBACK = (1 << 16), // Indicates that this will notify its owner and/or other elements array
											// elements changes. Note that when elements shift (say, inserting at head,
											// or fast remove), callbacks are not executed for these elements.
	FATTRIB_HAS_PRE_CALLBACK =
		(1 << 17), // Indicates that this will notify its owner and/or other elements right before it changes
