...
struct ElementPathItem_t
{
	ElementPathItem_t(DmElementHandle_t hElem = DMELEMENT_HANDLE_INVALID,
					  DmAttributeHandle_t hAttr = DMATTRIBUTE_HANDLE_INVALID, int idx = -1)
...
struct DmElementReference_t
{
	...
	bool IsWeaklyReferenced() // should we keep this element's DmElementHandle_t mapped to it's id (even if the element
							  // is deleted)
	{
	...
	unsigned short m_nStrongHandleCount; // CDmeCountedElementRef - for preventing elements from being truly deleted,
										 // mainly used by undo and file root
...
public:
	...
	bool FindElement(const CDmElement *pElement, CUtlVector<ElementPathItem_t> &elementPath,
					 TraversalDepth_t depth) const;
	bool FindReferer(DmElementHandle_t hElement, CUtlVector<ElementPathItem_t> &elementPath,
					 TraversalDepth_t depth) const;
...
protected:
	...
	CDmElement(DmElementHandle_t handle, const char *objectType, const DmObjectId_t &id, const char *objectName,
			   DmFileId_t fileid);
	virtual ~CDmElement();
...
private:
	...
	// internal recursive copy method - builds refmap of old element's handle -> copy's handle, and uses it to fixup
	// references
	void CopyAttributesTo(CDmElement *pCopy, CRefMap &refmap, TraversalDepth_t depth) const;
	void CopyElementAttribute(const CDmAttribute *pAttr, CDmAttribute *pCopyAttr, CRefMap &refmap,
							  TraversalDepth_t depth) const;
	void CopyElementArrayAttribute(const CDmAttribute *pAttr, CDmAttribute *pCopyAttr, CRefMap &refmap,
								   TraversalDepth_t depth) const;
	...
	void SetFileId_R(CUtlHashFast<DmElementHandle_t> &visited, DmFileId_t fileid, TraversalDepth_t depth,
...
private:
	...
	friend void CopyElements(const CUtlVector<CDmElement *> &from, CUtlVector<CDmElement *> &to,
...
protected:                                                                                    \
	className(DmElementHandle_t handle, const char *pElementTypeName, const DmObjectId_t &id, \
...
#define IMPLEMENT_ELEMENT(className) CUtlSymbol className::m_classType = UTL_INVAL_SYMBOL;
