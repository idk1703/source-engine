...
public:
	...
	virtual CDmElement *Create(DmElementHandle_t handle, const char *pElementType, const char *pElementName,
							   DmFileId_t fileid, const DmObjectId_t &id) = 0;
...
// current file encodings supported: binary, xml, xml_flat, keyvalues2, keyvalues2_flat, keyvalues (vmf/vmt/actbusy),
// text? (qc/obj)
class IDmSerializer
{
public:
	...
	// if we decide to implement non-identity conversions between formats on write, then the source and dest format will
	...
	virtual bool Unserialize(CUtlBuffer &buf, const char *pEncodingName, int nEncodingVersion,
							 ...
							 DmConflictResolution_t idConflictResolution, CDmElement **ppRoot) = 0;
};
...
//-----------------------------------------------------------------------------
// Purpose: Optional helper passed in with clipboard data which is called when it's time to clean up the clipboard data
...
enum DmNotifyFlags_t
{
	...
	NOTIFY_CHANGE_TOPOLOGICAL =
		(1 << (NOTIFY_SOURCE_BITS +
			   4)), // Element created, destroyed, element attribute or element array attribute value changed
	...
	NOTIFY_CHANGE_ATTRIBUTE_ARRAY_SIZE =
		(1 << (NOTIFY_SOURCE_BITS + 6)),				   // Non-element array attribute added or removed
	NOTIFY_CHANGE_OTHER = (1 << (NOTIFY_SOURCE_BITS + 7)), // Non attribute related change (a change in UI, for example)

	NOTIFY_CHANGE_MASK = (NOTIFY_CHANGE_TOPOLOGICAL | NOTIFY_CHANGE_ATTRIBUTE_VALUE |
...
public:
	...
	virtual DmElementHandle_t CreateElement(UtlSymId_t typeSymbol, const char *pElementName,
											DmFileId_t fileid = DMFILEID_INVALID,
											const DmObjectId_t *pObjectID = NULL) = 0;
	virtual DmElementHandle_t CreateElement(const char *pTypeName, const char *pElementName,
											DmFileId_t fileid = DMFILEID_INVALID,
											const DmObjectId_t *pObjectID = NULL) = 0;
	...
	virtual bool Serialize(CUtlBuffer &outBuf, const char *pEncodingName, const char *pFormatName,
	...
	virtual bool Unserialize(CUtlBuffer &inBuf, const char *pEncodingName, const char *pSourceFormatName,
							 const char *pFormatHint, const char *pFileName,
							 DmConflictResolution_t idConflictResolution, DmElementHandle_t &hRoot) = 0;

	// converts from elements from old file formats to elements for the current file format
	virtual bool UpdateUnserializedElements(const char *pSourceFormatName, int nSourceFormatVersion, DmFileId_t fileid,
											DmConflictResolution_t idConflictResolution, CDmElement **ppRoot) = 0;
	...
	virtual bool SaveToFile(const char *pFileName, const char *pPathID, const char *pEncodingName,
							const char *pFormatName, CDmElement *pRoot) = 0;
	...
	virtual DmFileId_t RestoreFromFile(const char *pFileName, const char *pPathID, const char *pFormatHint,
									   CDmElement **ppRoot, DmConflictResolution_t idConflictResolution = CR_DELETE_NEW,
	...
	// If chaining ID is != 0 and the next StartUndo uses the same ID, then the operations will be chained together into
	...
	virtual void AbortUndoableOperation() = 0; // called instead of FinishUndo, essentially performs and Undo() and
											   // WipeRedo() if any undo items have been added to the stack
...
public:
	...
	explicit CUndoScopeGuard(int nNotifySource, int nNotifyFlags, const char *udesc, const char *rdesc = NULL,
							 int nChainingID = 0)
	{
		m_bReleased = false;
		m_bNotify = true;
	...
	explicit CUndoScopeGuard(int nNotifySource, int nNotifyFlags, IDmNotify *pNotify, const char *udesc,
...
public:
	...
	CChangeUndoScopeGuard(bool bNewState, const char *pDesc, int nNotifySource, int nNotifyFlags,
...
#define DEFINE_SOURCE_UNDO_SCOPE_GUARD(_classnameprefix, _source)                                                  \
	...
	{                                                                                                              \
		typedef CUndoScopeGuard BaseClass;                                                                         \
                                                                                                                   \
	public:                                                                                                        \
		C##_classnameprefix##UndoScopeGuard(int nNotifyFlags, const char *pUndoDesc, const char *pRedoDesc = NULL, \
											int nChainingID = 0)                                                   \
			: BaseClass(_source, nNotifyFlags, pUndoDesc, pRedoDesc, nChainingID)                                  \
		{                                                                                                          \
		}                                                                                                          \
		C##_classnameprefix##UndoScopeGuard(int nNotifyFlags, IDmNotify *pNotify, const char *pUndoDesc,           \
											const char *pRedoDesc = NULL, int nChainingID = 0)                     \
