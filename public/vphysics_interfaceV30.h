...
#define VPHYSICS_DEBUG_OVERLAY_INTERFACE_VERSION_1 "VPhysicsDebugOverlay001"
	...
	public:
		...
		virtual void AddLineOverlay(const Vector &origin, const Vector &dest, int r, int g, int b, bool noDepthTest,
									float duration) = 0;
		virtual void AddTextOverlay(const Vector &origin, float duration, PRINTF_FORMAT_STRING const char *format,
									...) = 0;
...
#define VPHYSICS_COLLISION_INTERFACE_VERSION_7 "VPhysicsCollision007"
	...
	public:
		...
		virtual void CollideGetAABB(Vector & mins, Vector & maxs, const CPhysCollide *pCollide,
	...
	public:
		...
		// GLOBAL/LOCAL refer to the coordinate system of the values, whereas acceleration/force determine whether or
		// not mass is divided out (forces must be divided by mass to compute acceleration)
		enum simresult_e
		{
			SIM_NOTHING = 0,
	...
	{
	public:
		virtual int ShouldCollide(IPhysicsObject * pObj0, IPhysicsObject * pObj1, void *pGameData0,
								  void *pGameData1) = 0;
	...
	public:
		...
		// Calculates the force/torque on the center of mass for an offset force impulse (pass output to
		// ApplyForceCenter / ApplyTorqueCenter)
		virtual void CalculateForceOffset(const Vector &forceVector, const Vector &worldPosition, Vector *centerForce,
										  AngularImpulse *centerTorque) = 0;
		...
		virtual bool IsFluid() = 0; // fluids are special triggers with fluid controllers attached, they return true to
									// IsTrigger() as well!
