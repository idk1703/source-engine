...
#varName, offsetof(currentSendDTClass, varName), sizeof(((currentSendDTClass *)0)->varName[0]), \
...
#define SendPropUtlVectorDataTable(varName, nMaxElements, dataTableName) \
	SendPropUtlVector(SENDINFO_UTLVECTOR(varName), nMaxElements,         \
...
SendProp SendPropUtlVector(
	...
	SendTableProxyFn varProxy =
		SendProxy_DataTableToDataTable // This can be overridden to control who the array is sent to.
);
