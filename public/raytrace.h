...
{
public:
	virtual bool VisitTriangle_ShouldContinue(const TriIntersectData_t &triangle, const FourRays &rays, fltx4 *hitMask,
											  fltx4 *b0, fltx4 *b1, fltx4 *b2, int32 hitID) = 0;
};
...
public:
	...
	void AddTriangle(int32 id, const Vector &v1, const Vector &v2, const Vector &v3, const Vector &color, uint16 flags,
					 int32 materialIndex);
	...
	void Trace4Rays(const FourRays &rays, fltx4 TMin, fltx4 TMax, int DirectionSignMask, RayTracingResult *rslt_out,
					int32 skip_id = -1, ITransparentTriangleCallback *pCallback = NULL);

	// higher level intersection routine that handles computing the mask and handling rays which do not match in
	// direciton sign
	void Trace4Rays(const FourRays &rays, fltx4 TMin, fltx4 TMax, RayTracingResult *rslt_out, int32 skip_id = -1,
	...
	float CalculateCostsOfSplit(int split_plane, int32 const *tri_list, int ntris, Vector MinBound, Vector MaxBound,
								float &split_value, int &nleft, int &nright, int &nboth);
