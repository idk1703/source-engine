...
struct mstudioanim_t
{
	...
	{
		return (Vector48 *)(pData() + ((flags & STUDIO_ANIM_RAWROT) != 0) * sizeof(*pQuat48()) +
...
struct mstudioanimdesc_t
{
	...
	mstudioanim_t *pAnimBlock(int block,
							  int index) const; // returns pointer to a specific anim block (local or external)
...
struct mstudioseqdesc_t
{
	...
	inline mstudioactivitymodifier_t *pActivityModifier(int i) const
	{
		...
		return activitymodifierindex != 0 ? (mstudioactivitymodifier_t *)(((byte *)this) + activitymodifierindex) + i
										  : NULL;
	};
...
enum FlexControllerRemapType_t
{
	...
	FLEXCONTROLLER_REMAP_NWAY, // StepSize = 1 / (control count-1) Control n -> ramps from 0-1-0 from (n-1)*StepSize to
							   // n*StepSize to (n+1)*StepSize. A second control is needed to specify amount to use
...
public:
	...
	{
		return Vector(delta[0] * flVertAnimFixedPointScale, delta[1] * flVertAnimFixedPointScale,
					  delta[2] * flVertAnimFixedPointScale);
	}
	inline Vector GetNDeltaFixed(float flVertAnimFixedPointScale)
	{
		return Vector(ndelta[0] * flVertAnimFixedPointScale, ndelta[1] * flVertAnimFixedPointScale,
	...
	{
		vFillIn->Set(delta[0] * flVertAnimFixedPointScale, delta[1] * flVertAnimFixedPointScale,
	...
	{
		vFillIn->Set(ndelta[0] * flVertAnimFixedPointScale, ndelta[1] * flVertAnimFixedPointScale,
...
struct mstudiovertanim_wrinkle_t : public mstudiovertanim_t
{
	...
	{
		return Vector4D(delta[0] * flVertAnimFixedPointScale, delta[1] * flVertAnimFixedPointScale,
						delta[2] * flVertAnimFixedPointScale, wrinkledelta * flVertAnimFixedPointScale);
	}
...
struct studiohdr_t
{
	...
	// Src bone transforms are transformations that will convert .dmx or .smd-based animations into .mdl-based
	// animations
...
public:
	...
	private:
		...
		int SelectWeightedSequenceFromModifiers(CStudioHdr *pstudiohdr, int activity, CUtlSymbol *pActivityModifiers,
												int iModifierCount);
...
#endif
		return m_ActivityToSequence.SelectWeightedSequenceFromModifiers(this, activity, pActivityModifiers,
																		iModifierCount);
	}
...
inline int mstudioflexcontrollerui_t::nWayValueIndex(const CStudioHdr &cStudioHdr) const
{
	return remaptype == FLEXCONTROLLER_REMAP_NWAY
			   ? pNWayValueController() - cStudioHdr.pFlexcontroller((LocalFlexController_t)0)
			   : -1;
}
...
#define STUDIO_AL_XFADE \
	0x0080 // pre-bias the ramp curve to compense for a non-1 weight, assuming a second layer is also going to
		   // accumulate
//							0x0100
	...
	if(!pNewVvdHdr->numFixups)
	{
		...
		if(bNeedsTangentS)
		{
			...
			memcpy((byte *)pNewVvdHdr + pNewVvdHdr->tangentDataStart,
				   (byte *)pTempVvdHdr + pTempVvdHdr->tangentDataStart, numVertexes * sizeof(Vector4D));
		}
