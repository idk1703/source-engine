...
const char *Interpolator_NameForCurveType(int type, bool printname)
{
	...
	{
		Q_snprintf(outname, sizeof(outname), "%s <-> %s", Interpolator_NameForInterpolator(leftside, printname),
				   Interpolator_NameForInterpolator(rightside, printname));
	}
								   ...
								   const Vector &vNext, float f, Vector &vOut)
{
	...
	switch(interpolationType)
	{
		...
		case INTERPOLATE_KOCHANEK_BARTELS_LATE:
		{
			...
			Kochanek_Bartels_Spline_NormalizeX(t, b, c, vPre, vStart, vEnd, vNext, f, vOut);
		}
		break;
