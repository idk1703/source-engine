...
void SlerpBones(const CStudioHdr *pStudioHdr, Quaternion q1[MAXSTUDIOBONES], Vector pos1[MAXSTUDIOBONES],
				...
				int sequence, const Quaternion q2[MAXSTUDIOBONES], const Vector pos2[MAXSTUDIOBONES], float s,
				int boneMask);
...
public:
	CIKContext();
	void Init(const CStudioHdr *pStudioHdr, const QAngle &angles, const Vector &pos, float flTime, int iFramecounter,
			  int boneMask);
	...
	void SolveLock(const mstudioiklock_t *plock, int i, Vector pos[], Quaternion q[], matrix3x4_t boneToWorld[],
...
private:
	...
	void BuildBoneChain(const Vector pos[], const Quaternion q[], int iBone, matrix3x4_t *pBoneToWorld,
						CBoneBitList &boneComputed);
...
void Studio_CalcDefaultPoseParameters(const CStudioHdr *pStudioHdr, float flPoseParameter[MAXSTUDIOPOSEPARAM],
									  int nCount);
...
bool Studio_SeqMovement(const CStudioHdr *pStudioHdr, int iSequence, float flCycleFrom, float flCycleTo,
						const float poseParameter[], Vector &deltaMovement, QAngle &deltaAngle);
...
float Studio_FindSeqVelocity(const CStudioHdr *pStudioHdr, int iSequence, const float poseParameter[],
							 float flVelocity);
...
public:
	...
	void Init(const bonecacheparams_t &params, unsigned int size, short *pStudioToCached, short *pCachedToStudio,
			  int cachedBoneCount);
...
bool TraceToStudio(class IPhysicsSurfaceProps *pProps, const Ray_t &ray, CStudioHdr *pStudioHdr,
				   mstudiohitboxset_t *set, matrix3x4_t **hitboxbones, int fContentsMask, const Vector &vecOrigin,
				   float flScale, trace_t &trace);
...
void Studio_RunBoneFlexDrivers(float *pFlexController, const CStudioHdr *pStudioHdr, const Vector *pPositions,
