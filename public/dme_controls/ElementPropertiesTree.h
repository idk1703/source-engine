...
public:
	...
	virtual int AddItem(KeyValues *data, bool allowLabelEditing, int parentItemIndex,
...
public:
	...
	CElementPropertiesTreeInternal(vgui::Panel *parent, IDmNotify *pNotify, CDmElement *pObject, bool autoApply = true,
...
protected:
	...
	struct TreeItem_t
	{
		TreeItem_t() : m_pElement(0), m_pAttributeName(), m_pArrayElement(0) {}
	...
	bool BuildExpansionListToFindElement_R(CUtlRBTree<CDmElement *, int> &visited, int depth, SearchResult_t &sr,
										   CDmElement *owner, CDmElement *element, char const *attributeName,
										   int arrayIndex, CUtlVector<int> &expandIndices);
	void FindMatchingElements_R(CUtlRBTree<CDmElement *, int> &visited, char const *searchstr, CDmElement *root,
	...
	void CreateTreeEntry(int parentNodeIndex, CDmElement *obj, CDmAttribute *pAttribute, int nArrayIndex,
						 AttributeWidgets_t &entry);
	...
	vgui::Panel *CreateAttributeDataWidget(CDmElement *pElement, const char *pWidgetName, CDmElement *obj,
										   CDmAttribute *pAttribute, int nArrayIndex = -1);
	...
	// Gets tree view text
	void GetTreeViewText(CDmElement *obj, CDmAttribute *pAttribute, int nArrayIndex, char *pBuffer, int nMaxLen,
						 bool &editableText);
...
protected:
	...
	void DropItemsIntoArray(CDmrElementArray<> &array, CUtlVector<KeyValues *> &msglist, CUtlVector<CDmElement *> &list,
	...
	bool ShowSetElementAttributeDialog(CDmElement *pOwner, const char *pAttributeName, int nArrayItem,
...
public:
	CElementPropertiesTree(vgui::Panel *parent, IDmNotify *pNotify, CDmElement *pObject,
						   CDmeEditorTypeDictionary *pDict = NULL);

	virtual void Init();
	virtual void Refresh(
		CElementPropertiesTreeInternal::RefreshType_t rebuild = CElementPropertiesTreeInternal::REFRESH_REBUILD,
