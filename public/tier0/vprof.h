...
// these macros exist to create VProf_<line number> variables. This is important because it avoids /analyze warnings
// about variable aliasing when VPROF's are nested within each other, and allows for multiple VPROF's to exist within
// the same scope. Three macros must be used to force the __LINE__ to be resolved prior to the token concatenation, but
// just ignore the _INTERNAL macros and use the VPROF_VAR_NAME
...
public:
	CVProfNode(const tchar *pszName, int detailLevel, CVProfNode *pParent, const tchar *pBudgetGroupName,
			   int budgetFlags);
	~CVProfNode();
...
public:
	...
	bool PMCEnableL2Upon(const tchar *pszNodeName,
						 bool bRecursive = false); ///< enable l2 and lhs recording for one given node
	bool PMCDisableL2Upon(const tchar *pszNodeName,
						  bool bRecursive = false); ///< enable l2 and lhs recording for one given node
	...
	enum CPUTraceState
	{
		...
		kAllNodesInFrame_WaitingForMark, // we're going to record all the times a node is hit in a frame, but are
		...
		kAllNodesInFrame_WaitingForMarkMultiFrame, // we're going to record all the times a node is hit in a frame, but
												   // are waiting for the frame to start
	...
	CVProfNode *CPUTraceEnableForNode(
		const tchar *pszNodeName); // enable cpu trace on this node only, disabling it wherever else it may be on.
	CVProfNode *CPUTraceGetEnabledNode(
		CVProfNode *pStartNode = NULL);					   // return the node enabled for CPU tracing, or NULL.
	const char *GetCPUTraceFilename();					   // get the filename the trace should write into.
	const char *SetCPUTraceFilename(const char *filename); // set the filename the trace should write into. (don't
														   // specify the extension; I'll do that.)
...
inline void CVProfile::PushGroup(int nGroupID)
{
	...
	m_GroupIDStackDepth++;
	if(m_GroupIDStack[m_GroupIDStackDepth - 2] != nGroupID && VTuneGroupEnabled() && nGroupID == VTuneGroupID())
	{
		vtune(true);
	}
}
...
inline void CVProfile::PopGroup(void)
{
	...
	// the first element to be VPROF_BUDGETGROUP_OTHER_UNACCOUNTED.
	Assert(m_GroupIDStackDepth > 0);
	if(m_GroupIDStack[m_GroupIDStackDepth] != m_GroupIDStack[m_GroupIDStackDepth + 1] && VTuneGroupEnabled() &&
...
{
public:
	CVProfScope(const tchar *pszName, int detailLevel, const tchar *pBudgetGroupName, bool bAssertAccounted,
				int budgetFlags);
	~CVProfScope();
...
inline CVProfNode::CVProfNode(const tchar *pszName, int detailLevel, CVProfNode *pParent, const tchar *pBudgetGroupName,
							  int budgetFlags)
	: m_pszName(pszName),
	  m_nCurFrameCalls(0),
	  m_nPrevFrameCalls(0),
...
#endif
{
	...
	{
		m_BudgetGroupID =
			0; // "m_Root" can't call BudgetGroupNameToBudgetGroupID because g_VProfCurrentProfile not yet initialized
	}
...
inline void CVProfile::EnterScope(const tchar *pszName, int detailLevel, const tchar *pBudgetGroupName,
								  bool bAssertAccounted, int budgetFlags)
{
	if((m_enabled != 0 || !m_fAtRoot) &&
	   InTargetThread()) // if became disabled, need to unwind back to root before stopping
	{
...
inline void CVProfile::SetCPUTraceEnabled(CPUTraceState enabled, bool bTraceCompleteEvent /*=true*/,
										  int nNumFrames /*= -1*/)
{
...
inline CVProfScope::CVProfScope(const tchar *pszName, int detailLevel, const tchar *pBudgetGroupName,
								bool bAssertAccounted, int budgetFlags)
