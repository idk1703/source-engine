...
#define IsPS3()		false
#endif
// Adding IsPlatformOpenGL() to help fix a bunch of code that was using IsPosix() to infer if the DX->GL translation
// layer was being used.
...
#pragma warning(disable : 4748) // warning C4748: /GS can not protect parameters and local variables from local buffer
...
// called to exit the process due to a fatal error. This allows for the application to handle providing a hook as well
// which can be called before exiting
PLATFORM_INTERFACE void Plat_ExitProcessWithError(int nCode, bool bGenerateMinidump = false);

// sets the callback that will be triggered by Plat_ExitProcessWithError. NULL is valid. The return value true indicates
// that the exit has been handled and no further processing should be performed. False will cause a minidump to be
// generated, and the process to be terminated
