...
#define TM_ZONE_DEFAULT_PARAM(context, string_param) \
	tmZone(context, TMZF_NONE, "%s( %s )", __FUNCTION__, tmDynamicString(context, (string_param)))
...
public:
	...
	~CTelemetrySpikeDetector()
	{
		...
		if(time >= m_threshold)
		{
			tmMessage(TELEMETRY_LEVEL0, TMMF_ICON_NOTE | TMMF_SEVERITY_WARNING, "(source/spike)%s %.2fms %t", m_message,
					  time, tmSendCallStack(TELEMETRY_LEVEL0, 0));
		}
	}
