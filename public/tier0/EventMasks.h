...
typedef union EVENT_MASK(MOB_load_replay)
{
	struct
	{
		...
		uint16 PARTIAL_DATA : 1; // Replayed because of partially overlapped data access between the load and store
								 // operations },
		uint16 UNALGN_ADDR : 1; // Replayed because the lower 4 bits of the linear address do not match between the load
