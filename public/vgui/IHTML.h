...
namespace vgui
{
	...
	enum EWebPageLoadError
	{
		...
		eLoadErrorUnknown, // not a load type we classify right now, check out cef_handler_errorcode_t for the full list
						   // we could translate
	};
	...
	public:
		...
		virtual bool OnStartRequestInternal(const char *url, const char *target, const char *pchPostData,
											bool bIsRedirect) = 0;
