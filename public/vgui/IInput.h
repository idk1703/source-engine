...
#define MESSAGE_CURRENT_KEYFOCUS -2
	...
	public:
		...
		virtual void GetIMELanguageName(OUT_Z_BYTECAP(unicodeBufferSizeInBytes) wchar_t *buf,
										int unicodeBufferSizeInBytes) = 0;
		// Returns the short code for the language (EN, CH, KO, JP, RU, TH, etc. ).
		virtual void GetIMELanguageShortCode(OUT_Z_BYTECAP(unicodeBufferSizeInBytes) wchar_t *buf,
											 int unicodeBufferSizeInBytes) = 0;
		...
		//  to know if handler actually swallowed the specified keycode.  To get around this, I set a global before
		//  calling the kb focus OnKeyCodeTyped function and if we ever get to a Panel::OnKeyCodeTypes we know that
		//  nobody handled the message and in that case we can post a message to any "unhandled keycode" listeners
		...
		//  if restrictMessagesToSubTree is false, then mouse and kb messages are routed as normal except that they are
		//  not routed down into the subtree
		...
		// excluded from receiving messages Sends a "ModalSubTree", state message
