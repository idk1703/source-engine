...
public:
	...
	virtual void AccumulatePose(const CStudioHdr *pStudioHdr, CIKContext *pIKContext, Vector pos[], Quaternion q[],
								int sequence, float cycle, const float poseParameter[], int boneMask, float flWeight,
								float flTime)
	{
	}
};
	...
	if(pOld)
	{
		...
		if(x)
		{
			for(; k < o.m_arrTxt.Count(); ++k)
			{
				...
				if((txt.m_uiFlags & F_SEEN_THIS_FRAME) && !stricmp(x->m_chActivity, txt.m_chActivity) &&
				   !stricmp(x->m_chLabel, txt.m_chLabel) && (x->m_iActivity == txt.m_iActivity))
				{
...
void CPoseDebuggerImpl::StartBlending(IClientNetworkable *pEntity, const CStudioHdr *pStudioHdr)
{
	...
	if(!pRMdl || !pRMdl->numincludemodels)
		return;
	...
	if(m_mapModel.Find(pStudioHdr) != m_mapModel.InvalidIndex())
	{
		...
		for(int k = m_mapModelOld.FirstInorder(); k != m_mapModelOld.InvalidIndex(); k = m_mapModelOld.NextInorder(k))
		{
...
void CPoseDebuggerImpl::AccumulatePose(const CStudioHdr *pStudioHdr, CIKContext *pIKContext, Vector pos[],
									   ...
									   int boneMask, float flWeight, float flTime)
{
	...
	if(!txt.m_chActivity[0])
	{
		...
		for(int iLast = mpi.m_arrTxt.Count(); iLast-- > 0;)
		{
			...
			if(lastSeenTxt.m_uiFlags & ModelPoseDebugInfo::F_SEEN_THIS_FRAME && lastSeenTxt.m_chActivity[0])
			{
	...
	sprintf(txt.m_chTextLines[numLines], "%-*s  %-*s  %*.2f  %*.1f/%-*d  %*.0f%% ", widthActivity,
			seqdesc.pszActivityName(), widthLayer, seqdesc.pszLabel(), 7, pOldTxt ? pOldTxt->m_flTimeAlive : 0.f, 5,
			cycle * (((CStudioHdr *)pStudioHdr)->pAnimdesc(seqdesc.anim(0, 0)).numframes - 1), 3,
			((CStudioHdr *)pStudioHdr)->pAnimdesc(seqdesc.anim(0, 0)).numframes, widthPercent, flWeight * 100.0f);
	++numLines;
...
static ConCommand posedebuggerstart("+posedebug", IN_PoseDebuggerStart,
									"Turn on pose debugger or add ents to pose debugger UI", FCVAR_CHEAT);
static ConCommand posedebuggerend("-posedebug", IN_PoseDebuggerEnd,
