...
static CNodeVert const g_NodeChildLookup[4][2] = {{CNodeVert(0, 0), CNodeVert(1, 1)},
												  {CNodeVert(1, 0), CNodeVert(2, 1)},
												  {CNodeVert(0, 1), CNodeVert(1, 2)},
												  {CNodeVert(1, 1), CNodeVert(2, 2)}};

static CNodeVert const g_NodeTriWinding[9] = {CNodeVert(0, 1), CNodeVert(0, 0), CNodeVert(1, 0),
...
static inline void RotateVertIndex(NeighborOrientation neighor, int sideLengthMinus1, CVertIndex const &in,
								   CVertIndex &out)
{
...
static inline void RotateVertIncrement(NeighborOrientation neighor, CVertIndex const &in, CVertIndex &out)
{
	if(neighor == ORIENTATION_CCW_0)
	{
		out = in;
...
CDispUtilsHelper *SetupEdgeIncrements(CDispUtilsHelper *pDisp, int iEdge, int iSub, CVertIndex &myIndex,
									  CVertIndex &myInc, CVertIndex &nbIndex, CVertIndex &nbInc, int &myEnd,
									  int &iFreeDim)
{
...
int GetSubNeighborIndex(CDispUtilsHelper *pDisp, int iEdge, CVertIndex const &nodeIndex)
{
	const CPowerInfo *pPowerInfo = pDisp->GetPowerInfo();
	...
	{
		if(iSub == 1 && pSide->m_SubNeighbors[0].IsValid() && pSide->m_SubNeighbors[0].m_Span == CORNER_TO_CORNER)
		{
			iSub = 0;
		}
		else
...
CDispUtilsHelper *TransformIntoSubNeighbor(CDispUtilsHelper *pDisp, int iEdge, int iSub, CVertIndex const &nodeIndex,
										   CVertIndex &out)
{
	...
	int iFreeDim = !g_EdgeDims[iEdge];
	int fixedPercent =
		((nodeIndex[iFreeDim] - viSrcStart[iFreeDim]) * (1 << 16)) / (viSrcEnd[iFreeDim] - viSrcStart[iFreeDim]);
...
CDispUtilsHelper *TransformIntoNeighbor(CDispUtilsHelper *pDisp, int iEdge, CVertIndex const &nodeIndex,
										CVertIndex &out)
{
	if(iEdge == -1)
...
bool DoesPointHaveAnyNeighbors(CDispUtilsHelper *pDisp, const CVertIndex &index)
{
...
static inline void SetupCoordXY(CNodeVert &out, CNodeVert const &bottomLeft, CNodeVert const &topRight,
...
static unsigned short *DispCommon_GenerateTriIndices_R(CNodeVert const &bottomLeft, CNodeVert const &topRight,
...
void DispCommon_GenerateTriIndices(int power, unsigned short *indices)
{
	...
	DispCommon_GenerateTriIndices_R(CNodeVert(0, 0), CNodeVert(sideLength, sideLength), indices, power, sideLength + 1);
}
	...
	for(int iEdge = 0; iEdge < 4; iEdge++)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					AddNeighbor(pMain, iEdge, g_bEdgeNeighborFlip[iEdge], CORNER_TO_MIDPOINT, pOther, iNBEdge,
								CORNER_TO_CORNER);
				}

				if(FindEdge(pOther, pt[1], mid, iNBEdge))
				{
					AddNeighbor(pMain, iEdge, !g_bEdgeNeighborFlip[iEdge], MIDPOINT_TO_CORNER, pOther, iNBEdge,
								CORNER_TO_CORNER);
				}
			}
		}
	...
	while(1)
	{
		...
		for(iDisp = 0; iDisp < nDisps; ++iDisp)
		{
			...
			for(int iEdge = 0; iEdge < 4; iEdge++)
			{
				...
				while(it.Next())
				{
					if(!VerifyNeighborVertConnection(pHelper, it.GetVertIndex(), it.GetCurrentNeighbor(),
...
void DisableUnallowedVerts_R(CDispUtilsHelper *pDisp, CVertIndex const &nodeIndex, int iLevel, int &nUnallowed)
{
	...
	if(iLevel + 1 < pDisp->GetPower())
	{
		...
		{
			DisableUnallowedVerts_R(pDisp, pDisp->GetPowerInfo()->m_pChildVerts[iNodeIndex].m_Verts[iChild], iLevel + 1,
									nUnallowed);
		}
	}
}
