...
public:
	...
	bool BCanSatisfyFromReadCache(uint8 *pOutData, CPackedStoreFileHandle &handle, FileHandleTracker_t &fHandle,
								  int nDesiredPos, int nNumBytes, int &nRead);
	bool BCanSatisfyFromReadCacheInternal(uint8 *pOutData, CPackedStoreFileHandle &handle, FileHandleTracker_t &fHandle,
										  int nDesiredPos, int nNumBytes, int &nRead);
...
public:
	...
	ePackedStoreAddResultCode AddFile(char const *pFile, uint16 nMetaDataSize, const void *pFileData,
									  uint32 nFullFileSize, bool bMultiChunk, uint32 const *pCrcToUse = NULL);
	...
	int GetFileAndDirLists(const char *pWildCard, CUtlStringList &outDirnames, CUtlStringList &outFilenames,
						   bool bSortedOutput);
	...
	bool HashEntirePackFile(CPackedStoreFileHandle &handle, int64 &nFileSize, int nFileFraction, int nFractionSize,
							FileHash_t &fileHash);
	...
	void SetKeysForSigning(int nPrivateKeySize, const void *pPrivateKeyData, int nPublicKeySize,
...
private:
	...
	struct CFileHeaderFixedData *FindFileEntry(char const *pDirname, char const *pBaseName, char const *pExtension,
