...
{
	DataCacheLimits_t(unsigned _nMaxBytes = (unsigned)-1, unsigned _nMaxItems = (unsigned)-1, unsigned _nMinBytes = 0,
					  unsigned _nMinItems = 0)
		: nMaxBytes(_nMaxBytes), nMaxItems(_nMaxItems), nMinBytes(_nMinBytes), nMinItems(_nMinItems)
	{
	}
...
public:
	...
	// Purpose: Add an item to the cache.  Purges old items if over budget, returns false if item was already in cache.
	//--------------------------------------------------------
	virtual bool Add(DataCacheClientID_t clientId, const void *pItemData, unsigned size,
					 DataCacheHandle_t *pHandle) = 0;

	//--------------------------------------------------------
	// Purpose: Finds an item in the cache, returns NULL if item is not in cache. Not a cheap operation if section not
	// configured for fast find.
	//--------------------------------------------------------
	...
	//--------------------------------------------------------
	virtual DataCacheRemoveResult_t Remove(DataCacheHandle_t handle, const void **ppItemData,
	...
	// Purpose: Add an item to the cache.  Purges old items if over budget, returns false if item was already in cache.
	//--------------------------------------------------------
	virtual bool AddEx(DataCacheClientID_t clientId, const void *pItemData, unsigned size, unsigned flags,
					   DataCacheHandle_t *pHandle) = 0;
};
...
public:
	...
	//--------------------------------------------------------
	virtual IDataCacheSection *AddSection(IDataCacheClient * pClient, const char *pszSectionName,
...
public:
	...
	void Init(IDataCache *pSharedCache, const char *pszSectionName,
			  const DataCacheLimits_t &limits = DataCacheLimits_t(), bool bSupportFastFind = false)
	{
		if(!m_pCache)
		{
