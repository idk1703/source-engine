...
#define ATTRIBUTES_WHICH_ARE_VEC3S_MASK                                                                          \
	(PARTICLE_ATTRIBUTE_TRACE_P0_MASK | PARTICLE_ATTRIBUTE_TRACE_P1_MASK | PARTICLE_ATTRIBUTE_TRACE_HIT_NORMAL | \
	 PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_PREV_XYZ_MASK | PARTICLE_ATTRIBUTE_TINT_RGB_MASK |         \
...
enum ParticleFunctionType_t
{
	...
	FUNCTION_EMITTER,
	FUNCTION_CHILDREN, // NOTE: This one is a fake function type, only here to help eliminate a ton of duplicated code
					   // in the editor
...
public:
	...
	virtual void TraceLine(const Vector &vecAbsStart, const Vector &vecAbsEnd, unsigned int mask,
						   const class IHandleEntity *ignore, int collisionGroup, CBaseTrace *ptr) = 0;
	...
	virtual bool IsPointInControllingObjectHitBox(CParticleCollection *pParticles, int nControlPointNumber,
												  Vector vecPos, bool bBBoxOnly = false)
	{
		return true;
	}
...
public:
	...
	// methods to add builtin operators. If you don't call these at startup, you won't be able to sim or draw. These are
	// done separately from Init, so that the server can omit the code needed for rendering/simulation, if desired.
	...
	bool ReadParticleConfigFile(CUtlBuffer &buf, bool bPrecache, bool bDecommitTempMemory = true,
	...
	CParticleCollection *CreateParticleCollection(const char *pParticleSystemName, float flDelay = 0.0f,
												  int nRandomSeed = 0);
...
public:
	...
	virtual void AddForces(FourVectors *AccumulatedForces, CParticleCollection *pParticles, int nBlocks,
	...
	virtual bool EnforceConstraint(int nStartBlock, int nNumBlocks, CParticleCollection *pParticles, void *pContext,
	...
	virtual void InitNewParticlesScalar(CParticleCollection *pParticles, int nFirstParticle, int n_particles,
										int attribute_write_mask, void *pContext) const
	{
	}

	// init new particles in blocks of 4. initters that have sse smarts should over ride this. the scalar particle
	// initter will still be cllaed for head/tail.
	...
	void InitNewParticles(CParticleCollection *pParticles, int nFirstParticle, int n_particles,
						  int attribute_write_mask, void *pContext) const;
...
struct CParticleSIMDTransformation
{
	...
	{
		fltx4 fl4OutX =
			SubSIMD(AddSIMD(MulSIMD(InPnt.x, m_v4Fwd.x), MulSIMD(InPnt.z, m_v4Up.x)), MulSIMD(InPnt.y, m_v4Right.x));
		fltx4 fl4OutY =
			SubSIMD(AddSIMD(MulSIMD(InPnt.x, m_v4Fwd.y), MulSIMD(InPnt.z, m_v4Up.y)), MulSIMD(InPnt.y, m_v4Right.y));
		InPnt.z =
			SubSIMD(AddSIMD(MulSIMD(InPnt.x, m_v4Fwd.z), MulSIMD(InPnt.z, m_v4Up.z)), MulSIMD(InPnt.y, m_v4Right.z));
		InPnt.x = fl4OutX;
		InPnt.y = fl4OutY;
	}
...
public:
	...
	void GetControlPointOrientationAtCurrentTime(int nControlPoint, Vector *pForward, Vector *pRight,
												 Vector *pUp) const;
	...
	void GetControlPointOrientationAtTime(int nControlPoint, float flTime, Vector *pForward, Vector *pRight,
										  Vector *pUp);
	...
	void CalculatePathValues(CPathParameters const &PathIn, float flTimeStamp, Vector *pStartPnt, Vector *pMidPnt,
							 Vector *pEndPnt);
...
public:
	...
	struct CWorldCollideContextData
		*m_pCollisionCacheData[NUM_COLLISION_CACHE_MODES]; // children can share collision caches w/ parent
...
inline void CParticleCollection::SetControlPointOrientation(int nWhichPoint, const Vector &forward, const Vector &right,
															const Vector &up)
{
	Assert((nWhichPoint >= 0) && (nWhichPoint < MAX_PARTICLE_CONTROL_POINTS));

	// check perpendicular
	if(fabs(DotProduct(forward, up)) <= 0.1f && fabs(DotProduct(forward, right)) <= 0.1f &&
...
inline void CParticleCollection::RandomVector(int nRandomSampleId, const Vector &vecMin, const Vector &vecMax,
											  Vector *pVector)
{
	// do not call
	int nBaseId = m_nRandomSeed + nRandomSampleId;
...
inline const float *CParticleCollection::GetFloatAttributePtr(int nAttribute, int nParticleNumber) const
{
	...
	return m_pParticleAttributes[nAttribute] + m_nParticleFloatStrides[nAttribute] * block_ofs + (nParticleNumber & 3);
}
...
inline const float *CParticleCollection::GetInitialFloatAttributePtr(int nAttribute, int nParticleNumber) const
{
	Assert(nParticleNumber < m_nAllocatedParticles);
	int block_ofs = nParticleNumber / 4;
	return m_pParticleInitialAttributes[nAttribute] + m_nParticleInitialFloatStrides[nAttribute] * block_ofs +
		   (nParticleNumber & 3);
}
...
private:
	...
	void ParseOperators(const char *pszName, ParticleFunctionType_t nFunctionType, CDmxElement *pElement,
...
FORCEINLINE void CParticleCollection::GetControlPointOrientationAtCurrentTime(int nControlPoint, Vector *pForward,
																			  Vector *pRight, Vector *pUp) const
{
	Assert(nControlPoint <= GetHighestControlPoint());
	Assert(m_pDef->ReadsControlPoint(nControlPoint));
