...
namespace AudioEncodeOptions
{
	enum EAudioEncodeOptions_t // Options to control audio encoding
	{
		...
		LIMIT_AUDIO_TRACK_TO_VIDEO_DURATION =
			0x00000004,						 // Don't let the Audio Track exceed the video track in duration
...
public:
	...
	virtual VideoResult_t SoundDeviceCommand(VideoSoundDeviceOperation_t operation, void *pDevice = nullptr,
											 void *pData = nullptr) = 0; // Assign Sound Device for this Video Material

	// Video playback state functions
	virtual bool IsVideoReadyToPlay() = 0; // Queries if the video material was initialized successfully and is ready
										   // for playback, but not playing or finished
	virtual bool IsVideoPlaying() = 0;	// Is the video currently playing (and needs update calls, etc), or paused while
										// playing?
	...
	virtual void GetVideoTexCoordRange(float *pMaxU,
									   float *pMaxV) = 0; // Returns the max texture coordinate of the video portion of
														  // the material surface ( 0.0, 0.0 to U, V )
	virtual void GetVideoImageSize(int *pWidth,
								   int *pHeight) = 0; // Returns the frame size of the Video Image Frame in pixels ( the
													  // stored in a subrect of the material itself)
};
...
public:
	...
	virtual bool SetMovieSourceImageParameters(VideoEncodeSourceFormat_t srcImageFormat, int imgWidth,
											   int imgHeight) = 0;
	virtual bool SetMovieSourceAudioParameters(
		...
		AudioEncodeOptions_t audioOptions = AudioEncodeOptions::NO_AUDIO_OPTIONS, int audioSampleGroupSize = 0) = 0;
...
public:
	...
	virtual VideoSystem_t FindNextSystemWithFeature(VideoSystemFeature_t features,
	...
	virtual VideoSystem_t LocateVideoSystemForPlayingFile(
		const char *pFileName, VideoSystemFeature_t playMode = VideoSystemFeature::PLAY_VIDEO_FILE_IN_MATERIAL) = 0;
	virtual VideoResult_t LocatePlayableVideoFile(
		const char *pSearchFileName, const char *pPathID, VideoSystem_t *pPlaybackSystem, char *pPlaybackFileName,
		int fileNameMaxLen, VideoSystemFeature_t playMode = VideoSystemFeature::FULL_PLAYBACK) = 0;

	// Create/destroy a video material
	virtual IVideoMaterial *CreateVideoMaterial(
		const char *pMaterialName, const char *pVideoFileName, const char *pPathID = nullptr,
	...
	virtual VideoResult_t PlayVideoFileFullScreen(
		...
		int desktopWidth, int desktopHeight, bool windowed, float forcedMinTime,
