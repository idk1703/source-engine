...
#varName, offsetof(currentRecvDTClass, varName), sizeof(((currentRecvDTClass *)0)->varName[0]), \
...
#varName, offsetof(currentRecvDTClass, varName), sizeof(((currentRecvDTClass *)0)->varName[0]), resizeFn, \
		GetEnsureCapacityTemplate(((currentRecvDTClass *)0)->varName)

#define RecvPropUtlVectorDataTable(varName, nMaxElements, dataTableName) \
	RecvPropUtlVector(RECVINFO_UTLVECTOR(varName), nMaxElements,         \
...
//
RecvProp RecvPropUtlVector(const char *pVarName, // Use RECVINFO_UTLVECTOR to generate these first 5 parameters.
