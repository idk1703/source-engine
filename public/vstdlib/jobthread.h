...
{
	ThreadPoolStartParams_t(bool bIOThreads = false, unsigned nThreads = -1, int *pAffinities = NULL,
							ThreeState_t fDistribute = TRS_NONE, unsigned nStackSize = -1,
...
public:
	...
	//-----------------------------------------------------
	virtual int YieldWait(CThreadEvent * *pEvents, int nEvents, bool bWaitAll = true,
	...
	//-----------------------------------------------------
	virtual void AddFunctor(CFunctor * pFunctor, CJob **ppJob = NULL, const char *pszDescription = NULL,
							unsigned flags = 0)
	{
...
#define DEFINE_MEMBER_ADD_CALL(N)                                                                           \
	...
	CJob *AddCall(OBJECT_TYPE *pObject, FUNCTION_RETTYPE (FUNCTION_CLASS::*pfnProxied)(                     \
...
#define DEFINE_CONST_MEMBER_ADD_CALL(N)                                                                           \
	template<typename OBJECT_TYPE, typename FUNCTION_CLASS,                                                       \
			 typename FUNCTION_RETTYPE FUNC_TEMPLATE_FUNC_PARAMS_##N FUNC_TEMPLATE_ARG_PARAMS_##N>                \
	CJob *AddCall(OBJECT_TYPE *pObject, FUNCTION_RETTYPE (FUNCTION_CLASS::*pfnProxied)(                           \
											FUNC_BASE_TEMPLATE_FUNC_PARAMS_##N) const FUNC_ARG_FORMAL_PARAMS_##N) \
	{                                                                                                             \
		CJob *pJob;                                                                                               \
		if(!NumIdleThreads())                                                                                     \
		{                                                                                                         \
...
#define DEFINE_REF_COUNTING_MEMBER_ADD_CALL(N)                                                                   \
	template<typename OBJECT_TYPE, typename FUNCTION_CLASS,                                                      \
			 typename FUNCTION_RETTYPE FUNC_TEMPLATE_FUNC_PARAMS_##N FUNC_TEMPLATE_ARG_PARAMS_##N>               \
	CJob *AddRefCall(OBJECT_TYPE *pObject, FUNCTION_RETTYPE (FUNCTION_CLASS::*pfnProxied)(                       \
											   FUNC_BASE_TEMPLATE_FUNC_PARAMS_##N) FUNC_ARG_FORMAL_PARAMS_##N)   \
	{                                                                                                            \
		CJob *pJob;                                                                                              \
		if(!NumIdleThreads())                                                                                    \
...
#define DEFINE_MEMBER_QUEUE_CALL(N)                                                                               \
	template<typename OBJECT_TYPE, typename FUNCTION_CLASS,                                                       \
			 typename FUNCTION_RETTYPE FUNC_TEMPLATE_FUNC_PARAMS_##N FUNC_TEMPLATE_ARG_PARAMS_##N>                \
	CJob *QueueCall(OBJECT_TYPE *pObject, FUNCTION_RETTYPE (FUNCTION_CLASS::*pfnProxied)(                         \
											  FUNC_BASE_TEMPLATE_FUNC_PARAMS_##N) FUNC_ARG_FORMAL_PARAMS_##N)     \
	{                                                                                                             \
		CJob *pJob;                                                                                               \
...
#define DEFINE_REF_COUNTING_MEMBER_QUEUE_CALL(N)                                                                  \
	template<typename OBJECT_TYPE, typename FUNCTION_CLASS,                                                       \
			 typename FUNCTION_RETTYPE FUNC_TEMPLATE_FUNC_PARAMS_##N FUNC_TEMPLATE_ARG_PARAMS_##N>                \
	CJob *QueueRefCall(OBJECT_TYPE *pObject, FUNCTION_RETTYPE (FUNCTION_CLASS::*pfnProxied)(                      \
												 FUNC_BASE_TEMPLATE_FUNC_PARAMS_##N) FUNC_ARG_FORMAL_PARAMS_##N)  \
	{                                                                                                             \
		CJob *pJob;                                                                                               \
		AddFunctorInternal(CreateRefCountingFunctor(pObject, pfnProxied FUNC_FUNCTOR_CALL_ARGS_##N), &pJob, NULL, \
						   JF_QUEUE);                                                                             \
		return pJob;                                                                                              \
	}
...
private:
	virtual void AddFunctorInternal(CFunctor *, CJob ** = NULL, const char *pszDescription = NULL,
									unsigned flags = 0) = 0;
...
public:
	...
	{
		return (m_status != JOB_STATUS_PENDING && m_status != JOB_STATUS_INPROGRESS &&
				m_status != JOB_STATUS_UNSERVICED);
	}
...
{
public:
	CFunctorJob(CFunctor *pFunctor, const char *pszDescription = NULL) : m_pFunctor(pFunctor)
	{
		if(pszDescription)
		{
			Q_strncpy(m_szDescription, pszDescription, sizeof(m_szDescription));
...
#define DEFINE_NON_MEMBER_ITER_RANGE_PARALLEL(N)                                                                       \
	template<typename FUNCTION_CLASS,                                                                                  \
			 typename FUNCTION_RETTYPE FUNC_TEMPLATE_FUNC_PARAMS_##N FUNC_TEMPLATE_ARG_PARAMS_##N, typename ITERTYPE1, \
			 typename ITERTYPE2>                                                                                       \
	void IterRangeParallel(                                                                                            \
		FUNCTION_RETTYPE (FUNCTION_CLASS::*pfnProxied)(ITERTYPE1, ITERTYPE2 FUNC_ADDL_TEMPLATE_FUNC_PARAMS_##N),       \
...
public:
	...
	// CFuncJobItemProcessor(OBJECT_TYPE_PTR pObject, void (FUNCTION_CLASS::*pfnProcess)( ITEM_TYPE & ), void
	// (*pfnBegin)() = NULL, void (*pfnEnd)() = NULL );
	void Begin()
	{
		if(m_pfnBegin)
			(*m_pfnBegin)();
...
{
public:
	void Init(OBJECT_TYPE *pObject, void (FUNCTION_CLASS::*pfnProcess)(T &), void (FUNCTION_CLASS::*pfnBegin)() = NULL,
...
inline void ParallelProcess(const char *pszDescription, ITEM_TYPE *pItems, unsigned nItems,
							void (*pfnProcess)(ITEM_TYPE &), void (*pfnBegin)() = NULL, void (*pfnEnd)() = NULL,
...
inline void ParallelProcess(const char *pszDescription, ITEM_TYPE *pItems, unsigned nItems, OBJECT_TYPE *pObject,
							void (FUNCTION_CLASS::*pfnProcess)(ITEM_TYPE &), void (FUNCTION_CLASS::*pfnBegin)() = NULL,
...
template<typename ITEM_TYPE>
inline void ParallelProcess(const char *pszDescription, IThreadPool *pPool, ITEM_TYPE *pItems, unsigned nItems,
							void (*pfnProcess)(ITEM_TYPE &), void (*pfnBegin)() = NULL, void (*pfnEnd)() = NULL,
							int nMaxParallel = INT_MAX)
{
	CParallelProcessor<ITEM_TYPE, CFuncJobItemProcessor<ITEM_TYPE>> processor(pszDescription);
