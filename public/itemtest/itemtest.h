...
public:
	...
	void SetItemLog(CItemLog *pItemLog)
	{
		m_pItemLog = pItemLog;
	}
...
public:
	...
	static void SetDevMode(bool bDev)
	{
		m_bDev = bDev;
	}

	static bool IgnoreEnvironmentVariables()
	{
		return m_bIgnoreEnvVars;
	}
	static void SetIgnoreEnvironmentVariables(bool bIgnore)
	{
		m_bIgnoreEnvVars = bIgnore;
	}

	static void ForceSteamID(const char *pszSteamID)
	{
		m_szForcedSteamID = pszSteamID;
	}
	static const char *GetForcedSteamID(void)
	{
		return m_szForcedSteamID;
	}
	...
	static CItemTestManifest *Manifest(void)
	{
		return m_pItemTestManifest;
	}
...
public:
	...
	bool IsValid(void)
	{
		return m_pManifestKV != NULL;
	}

	int GetNumClasses(void)
	{
		return m_vecClasses.Count();
	}
	const char *GetClass(int nClass)
	{
		return (nClass >= 0 && nClass < m_vecClasses.Count()) ? m_vecClasses[nClass].Get() : NULL;
	}
	const char *GetClassVMTTemplate(int nClass)
	{
		return (nClass >= 0 && nClass < m_vecClassTemplates.Count()) ? m_vecClassTemplates[nClass] : NULL;
	}
	bool HasClassVMTTemplates()
	{
		return m_vecClassTemplates.Count() > 0;
	}

	int GetNumMaterialTypes(void)
	{
		return m_vecMaterialTypes.Count();
	}
	const char *GetMaterialType(int nMaterial)
	{
		return (nMaterial >= 0 && nMaterial < m_vecMaterialTypes.Count())
				   ? m_vecMaterialTypes[nMaterial].pszMaterialType
				   : NULL;
	}
	int GetMaterialType(const char *pszMaterialType);
	int GetDefaultMaterialType(void)
	{
		return m_nDefaultMaterialType;
	}

	int GetNumMaterialSkins(void)
	{
		return max(m_vecMaterialSkins.Count(), 1);
	}
	const char *GetMaterialSkin(int nMaterial)
	{
		return (nMaterial >= 0 && nMaterial < m_vecMaterialSkins.Count())
				   ? m_vecMaterialSkins[nMaterial].pszMaterialSkin
				   : NULL;
	}
	int GetMaterialSkin(const char *pszMaterialSkin);
	const char *GetMaterialSkinFilenameAppend(int nMaterial)
	{
		return (nMaterial >= 0 && nMaterial < m_vecMaterialSkins.Count())
				   ? m_vecMaterialSkins[nMaterial].pszFilenameAppend
				   : "";
	}
	int GetDefaultMaterialSkin(void)
	{
		return m_nDefaultMaterialSkin;
	}

	int GetNumTextureTypes(void)
	{
		return m_vecTextureTypes.Count();
	}
	const char *GetTextureType(int nTexture)
	{
		return (nTexture >= 0 && nTexture < m_vecTextureTypes.Count()) ? m_vecTextureTypes[nTexture].pszTextureType
																	   : NULL;
	}
	bool IsTextureTypeRequired(int nTexture)
	{
		return (nTexture >= 0 && nTexture < m_vecTextureTypes.Count()) ? !m_vecTextureTypes[nTexture].bOptional : false;
	}
	int GetTextureType(const char *pszTextureType);
	KeyValues *GetTextureAddToVTEXConfig(const char *pszTextureType);

	int GetNumIconTypes(void)
	{
		return m_vecIconTypes.Count();
	}
	const char *GetIconType(int nIcon)
	{
		return (nIcon >= 0 && nIcon < m_vecIconTypes.Count()) ? m_vecIconTypes[nIcon].pszIconType : NULL;
	}
	int GetIconType(const char *pszIconType);
	const char *GetIconFilenameAppend(int nIcon)
	{
		return (nIcon >= 0 && nIcon < m_vecIconTypes.Count()) ? m_vecIconTypes[nIcon].pszFilenameAppend : "";
	}
	bool GetIconDimensions(int nIcon, int &nWidth, int &nHeight);
	KeyValues *GetIconAddToVTEXConfig(int nIcon)
	{
		return (nIcon >= 0 && nIcon < m_vecIconTypes.Count()) ? m_vecIconTypes[nIcon].pkvAddToVTEXConfig : NULL;
	}
	KeyValues *GetIconVMTTemplate(int nIcon)
	{
		return (nIcon >= 0 && nIcon < m_vecIconTypes.Count()) ? m_vecIconTypes[nIcon].pkvVMTTemplate : NULL;
	}

	ExtensionList *GetMDLExtensions(void)
	{
		return &m_vecMDLExtensions;
	}
	ExtensionList *GetAnimationMDLExtentions(void)
	{
		return &m_vecAnimationMDLExtensions;
	}
	const char *GetVMTVarForTextureType(const char *pszTexture);
	const char *GetItemDirectory(void)
	{
		return m_strItemDirectoryOverride.IsEmpty() ? m_pItemDirectory : m_strItemDirectoryOverride.String();
	}
	const char *GetAnimationDirectory(void)
	{
		return m_strAnimationDirectoryOverride.IsEmpty() ? m_pAnimationDirectory
														 : m_strAnimationDirectoryOverride.String();
	}
	const char *GetIconDirectory(void)
	{
		return m_strIconDirectoryOverride.IsEmpty() ? m_pIconDirectory : m_strIconDirectoryOverride.String();
	}
	const char *GetZipSourceDirectory(void)
	{
		return m_pZipSourceDirectory;
	}
	const char *GetZipOutputDirectory(void)
	{
		return m_pZipOutputDirectory;
	}
	const char *GetQCTemplate(void) const
	{
		return m_pQCTemplate;
	}
	const char *GetQCITemplate(void) const
	{
		return m_pQCITemplate;
	}
	int GetQCLODDistance(int nLOD)
	{
		return (nLOD > 0 && nLOD <= m_vecQCLODDistances.Count()) ? m_vecQCLODDistances[nLOD - 1] : nLOD;
	}
	bool UseTerseMessages(void)
	{
		return m_bTerseMessages;
	}
	bool GetItemPathUsesSteamId(void)
	{
		return m_bItemPathUsesSteamId;
	}

	void SetItemDirectoryOverride(const char *pszItemDirOverride)
	{
		m_strItemDirectoryOverride = pszItemDirOverride;
	}
	void SetAnimationDirectoryOverride(const char *pszAnimationDirOverride)
	{
		m_strAnimationDirectoryOverride = pszAnimationDirOverride;
	}
	void SetIconDirectoryOverride(const char *pszIconDirOverride)
	{
		m_strIconDirectoryOverride = pszIconDirOverride;
	}
...
public:
	...
	enum PathFlags
	{
		...
		PATH_FLAG_EXTENSION =
			0x10, // If set and PATH_FLAG_FILE, the returned path will include the extension on the filename
		...
		PATH_FLAG_MODELS = 0x02, // if not set and PATH_FLAG_PATH & !PATH_FLAG_ABSOLUTE, the returned path will include
								 // the 'models' prefix, !MODELS implies !PREFIX
		PATH_FLAG_PREFIX = 0x01, // If not set and PATH_FLAG_PATH & !PATH_FLAG_ABSOLUTE, the returned path will include
								 // the prefix (materials | materialsrc)
	...
	// Appends all output paths to the passed array with the specified flags.  See flag usage below.  If bRecurse is
	// true, calls it for each output CTargetBase as well
	bool GetOutputPaths(CUtlVector<CUtlString> &sOutputPaths, uint nPathFlags = PATH_FLAG_ALL,
	...
	// Returns the output filenames for this target and optionally for all inputs recursively, either relative or
	// absolute
	bool GetOutputPaths(CUtlVector<CUtlString> &sOutputPaths, bool bRelative, bool bRecurse, bool bExtension,
						bool bPrefix = true) const;

	// Returns the input filenames for this target and optionally for all inputs recursively, either relative or
	// absolute
	virtual bool GetInputPaths(CUtlVector<CUtlString> &sInputPaths, bool bRelative = true, bool bRecurse = true,
	...
	void SetCustomOutputName(const char *pszCustomOutputName)
	{
		m_strCustomOutputName = pszCustomOutputName;
	}
	const CUtlString &GetCustomOutputName() const
	{
		return m_strCustomOutputName;
	}

	virtual void SetNameSuffix(const char *pszSuffix)
	{
		m_sNameSuffix = pszSuffix;
	}
	virtual const char *GetNameSuffix() const
	{
		return m_sNameSuffix.Get();
	}

	virtual void SetCustomRelativeDir(const char *pszCustomRelativeDir)
	{
		m_sCustomRelativeDir = pszCustomRelativeDir;
	}
	virtual const char *GetCustomRelativeDir() const
	{
		return m_sCustomRelativeDir.IsEmpty() ? NULL : m_sCustomRelativeDir.Get();
	}
	...
	void SetIgnoreP4(bool bIgnoreP4)
	{
		m_bIgnoreP4 = bIgnoreP4;
	}

	KeyValues *GetCustomKeyValues()
	{
		return m_kvCustomKeys;
	}

	void SetCustomModPath(const char *pszCustomModPath)
	{
		m_sCustomModPath = pszCustomModPath;
	}
	const char *GetCustomModPath() const
	{
		return m_sCustomModPath.IsEmpty() ? NULL : m_sCustomModPath.String();
	}
...
public:
	...
	void SetNoNiceFiltering(bool bNoNice)
	{
		m_bNoNiceFiltering = bNoNice;
	}
...
protected:
	template<class T>
	static T NearestPowerOfTwo(T nVal);
...
public:
	...
	virtual void SetNameSuffix(const char *pszSuffix)
	{
		m_pTargetTGA->SetNameSuffix(pszSuffix);
	}
	virtual const char *GetNameSuffix() const
	{
		return m_pTargetTGA->GetNameSuffix();
	}

	bool SetInputFile(const char *pszFilename)
	{
		return m_pTargetTGA->SetInputFile(pszFilename);
	}

	const CUtlString &GetInputFile() const
	{
		return m_pTargetTGA->GetInputFile();
	}

	bool HasAlpha() const
	{
		return m_pTargetTGA->HasAlpha();
	}
	...
	CTargetTGA *GetTargetTGA(void) const
	{
		return m_pTargetTGA.IsValid() ? m_pTargetTGA.GetObject() : NULL;
	}
...
public:
	...
	{
		static const char *pszColorAlphaTypeStr[] = {"NoColorAlpha", "Transparency", "Paintable", "ColorSpecPhong"};
	...
	{
		static const char *pszNormalAlphaTypeStr[] = {"NoNormalAlpha", "NormalSpecPhong"};
	...
	virtual void GetMaterialId(CUtlString &sMaterialId) const
	{
		sMaterialId = m_sMaterialId;
	}
	const CUtlString &GetMaterialId() const
	{
		return m_sMaterialId;
	}

	bool SetTargetVTF(const char *pszTextureType, const char *pszFilename,
					  int nSkinIndex = CItemUpload::Manifest()->GetDefaultMaterialSkin());

	void SetColorAlphaType(ColorAlpha_t nColorAlphaType)
	{
		m_nColorAlphaType = nColorAlphaType;
	}
	ColorAlpha_t GetColorAlphaType() const
	{
		return m_nColorAlphaType;
	}

	void SetNormalAlphaType(NormalAlpha_t nNormalAlphaType)
	{
		m_nNormalAlphaType = nNormalAlphaType;
	}
	NormalAlpha_t GetNormalAlphaType() const
	{
		return m_nNormalAlphaType;
	}
	...
	void SetTargetResolution(int nWidth, int nHeight)
	{
	...
	CTargetVTF *GetTargetVTF(int iVTF, int nSkinIndex = 0)
	{
		if(nSkinIndex >= 0 && nSkinIndex < m_vecTargetVTFs.Count())
		{
			if(iVTF >= 0 && iVTF < m_vecTargetVTFs[nSkinIndex].Count())
				return m_vecTargetVTFs[nSkinIndex][iVTF].IsValid() ? m_vecTargetVTFs[nSkinIndex][iVTF].GetObject()
																   : NULL;
		}
		return NULL;
	}
...
public:
	...
	virtual const char *GetItemDirectory() const
	{
		return CItemUpload::Manifest()->GetIconDirectory();
	}

	virtual KeyValues *GetTextureAddToVTEXConfigForTGA(CTargetTGA *pTGA) const
	{
		return CItemUpload::Manifest()->GetIconAddToVTEXConfig(m_nIconType);
	}
...
public:
	...
	virtual const char *GetItemDirectory() const
	{
		return m_strQCITemplate.IsEmpty() ? CItemUpload::Manifest()->GetItemDirectory()
										  : CItemUpload::Manifest()->GetAnimationDirectory();
	}
	...
	void SetQCITemplate(const char *pszQCITemplate)
	{
		m_strQCITemplate = pszQCITemplate;
	}
	...
	virtual void UpdateManifest(KeyValues *pKV)
	{
		...
		CUtlString sOutName;
		if(GetOutputPath(sOutName, 0,
						 PATH_FLAG_PATH | PATH_FLAG_FILE | PATH_FLAG_PREFIX | PATH_FLAG_MODELS | PATH_FLAG_EXTENSION))
		{
	...
	void SetSoundScriptFilePath(const char *pszSoundScriptFile)
	{
		m_strSoundScriptFile = pszSoundScriptFile;
	}
	const char *GetSoundScriptFilePath() const
	{
		return m_strSoundScriptFile.String();
	}
	bool WriteVCD(CUtlBuffer &vcdBuf);
	void SetAnimationLoopStartTime(float flStartTime)
	{
		m_flAnimationLoopStartTime = flStartTime;
	}
...
public:
	...
	void SetInputFile(const char *pszVCD)
	{
		m_strVCDPath = pszVCD;
	}
...
public:
	...
	const char *GetQCITemplate()
	{
		return m_strQCITemplate.IsEmpty() ? NULL : m_strQCITemplate.Get();
	}
...
public:
	...
	const CUtlVector<CUtlString> &GetBuiltFiles() const
	{
		return m_sAbsPaths;
	}
	const CUtlVector<CUtlString> &GetRelativePathBuiltFiles() const
	{
		return m_sRelPaths;
	}
	const CUtlVector<CUtlString> &GetModFiles() const
	{
		return m_sModOutputs;
	}
	void AddModOutput(const char *pszCustomModOutput)
	{
		m_sModOutputs.AddToTail(pszCustomModOutput);
	}
	...
	void SetSkinToBipHead(bool bSkinToBipHead)
	{
		m_bSkinToBipHead = bSkinToBipHead;
	}
	bool SkinToBipHead() const
	{
		return m_bSkinToBipHead;
	}
	...
	CSmartPtr<CTargetIcon> GetTargetIcon(int nIcon) const
	{
		return m_vecTargetIcons[nIcon];
	}

	int AddModel();
	int GetNumModels() const
	{
		return m_vecModels.Count();
	}
	...
	CSmartPtr<T> NewTarget(const C *pTargetParent)
	{
	...
	void SetArchivePath(const char *pszArchivePath)
	{
		m_sArchivePath = pszArchivePath;
	}
	const char *GetArchivePath() const
	{
		return m_sArchivePath;
	}

	const char *CheckRedundantOutputFilePath(const char *pszInputFilePath, const char *pszVTEXConfig,
											 const char *pszOutputFilePath);

	void ExcludeFileExtension(const char *pszExtension)
	{
		m_sExcludeFileExtensions.AddToTail(pszExtension);
	}

	void SetBuildScenesImage(bool bShouldBuildScenesImage)
	{
		m_bShouldBuildScenesImage = bShouldBuildScenesImage;
	}
...
public:
	...
	CAssetGame(const char *pszClass, const char *pszName, bool *pbOk /* = NULL */) : CAsset(pszName, pbOk)
	{
...
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo.vmt 	true	true	true	true	true
//false	d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo.vmt
//	true	true	true	true	false	true
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo.vmt 	true	true	true	true	false
//false	d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo.vmt
//
...
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo 	true	false	true	true	true
//false	d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo
//	true	false	true	true	false	true
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo 	true	false	true	true	false
//false	d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo
//
...
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo 	false	true	true	true	true	false
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo 	false	true	true	true	false	true
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo 	false	true	true	true	false	false
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo
//
...
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo 	false	false	true	true	true
//false	d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo
//	false	false	true	true	false	true
//d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo 	false	false	true	true	false
//false	d:/dev/main/game/tf/materials/models/player/items/pyro/0x00017714/foo/foo
//
