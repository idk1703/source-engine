...
#define CHAR_DOPPLER \
	'>' // as one of 1st 2 chars in name, indicates doppler encoded stereo wav: left wav (incomming) and right wav
		// (outgoing).
#define CHAR_DIRECTIONAL \
	'<' // as one of 1st 2 chars in name, indicates stereo wav has direction cone: mix left wav (front facing) with
		// right wav (rear facing) based on soundfacing direction
...
inline bool IsSoundChar(char c)
{
	...
	b = b || (c == CHAR_DOPPLER || c == CHAR_DIRECTIONAL || c == CHAR_DISTVARIANT || c == CHAR_SPATIALSTEREO ||
			  c == CHAR_FAST_PITCH);
