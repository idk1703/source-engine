...
#define _FIELD(name, fieldtype, count, flags, mapname, tolerance)                                                 \
	{                                                                                                             \
		fieldtype, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, count, flags, mapname, NULL, \
			NULL, NULL, sizeof(((classNameTypedef *)0)->name), NULL, 0, tolerance                                 \
	}
...
#define DEFINE_CUSTOM_FIELD(name, datafuncs)                                                                     \
	{                                                                                                            \
		FIELD_CUSTOM, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1, FTYPEDESC_SAVE, NULL, \
			datafuncs, NULL                                                                                      \
	}
#define DEFINE_CUSTOM_KEYFIELD(name, datafuncs, mapname)                                   \
	{                                                                                      \
		FIELD_CUSTOM, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1, \
...
#define DEFINE_EMBEDDED(name)                                                                                       \
	{                                                                                                               \
		FIELD_EMBEDDED, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1, FTYPEDESC_SAVE, NULL,  \
			NULL, NULL, &(((classNameTypedef *)0)->name.m_DataMap), sizeof(((classNameTypedef *)0)->name), NULL, 0, \
			0.0f                                                                                                    \
	}
...
#define DEFINE_EMBEDDED_OVERRIDE(name, overridetype)                                                               \
	{                                                                                                              \
		FIELD_EMBEDDED, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1, FTYPEDESC_SAVE, NULL, \
			NULL, NULL, &((overridetype *)0)->m_DataMap, sizeof(((classNameTypedef *)0)->name), NULL, 0, 0.0f      \
	}

#define DEFINE_EMBEDDEDBYREF(name)                                                                         \
	{                                                                                                      \
		FIELD_EMBEDDED, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1,               \
			FTYPEDESC_SAVE | FTYPEDESC_PTR, NULL, NULL, NULL, &(((classNameTypedef *)0)->name->m_DataMap), \
			sizeof(*(((classNameTypedef *)0)->name)), NULL, 0, 0.0f                                        \
	}

#define DEFINE_EMBEDDED_ARRAY(name, count)                                                                             \
	{                                                                                                                  \
		FIELD_EMBEDDED, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, count, FTYPEDESC_SAVE, NULL, \
			NULL, NULL, &(((classNameTypedef *)0)->name->m_DataMap), sizeof(((classNameTypedef *)0)->name[0]), NULL,   \
			0, 0.0f                                                                                                    \
	}

#define DEFINE_EMBEDDED_AUTO_ARRAY(name)                                                                         \
	{                                                                                                            \
		FIELD_EMBEDDED, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0},                        \
			SIZE_OF_ARRAY(((classNameTypedef *)0)->name), FTYPEDESC_SAVE, NULL, NULL, NULL,                      \
...
#define DEFINE_PRED_TYPEDESCRIPTION(name, fieldtype)                                                               \
	{                                                                                                              \
		FIELD_EMBEDDED, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1, FTYPEDESC_SAVE, NULL, \
			NULL, NULL, &fieldtype::m_PredMap                                                                      \
	}

#define DEFINE_PRED_TYPEDESCRIPTION_PTR(name, fieldtype)                                     \
	{                                                                                        \
		FIELD_EMBEDDED, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1, \
			FTYPEDESC_SAVE | FTYPEDESC_PTR, NULL, NULL, NULL, &fieldtype::m_PredMap          \
	}
...
#define DEFINE_INPUT(name, fieldtype, inputname)                                           \
	{                                                                                      \
		fieldtype, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1,    \
			FTYPEDESC_INPUT | FTYPEDESC_SAVE | FTYPEDESC_KEY, inputname, NULL, NULL, NULL, \
			sizeof(((classNameTypedef *)0)->name)                                          \
	}
...
#define DEFINE_OUTPUT(name, outputname)                                                    \
	{                                                                                      \
		FIELD_CUSTOM, #name, {(int)_hacky_datamap_offsetof(classNameTypedef, name), 0}, 1, \
			FTYPEDESC_OUTPUT | FTYPEDESC_SAVE | FTYPEDESC_KEY, outputname, eventFuncs      \
	}

// replaces EXPORT table for portability and non-DLL based systems (xbox)
#define DEFINE_FUNCTION_RAW(function, func_type)                                                              \
	{                                                                                                         \
		FIELD_VOID, nameHolder.GenerateName(#function), {NULL, NULL}, 1, FTYPEDESC_FUNCTIONTABLE, NULL, NULL, \
...
#define BEGIN_DATADESC_GUTS(className)                                                                 \
	...
	{                                                                                                  \
		datamap_t *g_DataMapHolder =                                                                   \
			DataMapInit((className *)NULL); /* This can/will be used for some clean up duties later */ \
	}                                                                                                  \
                                                                                                       \
	template<>                                                                                         \
	datamap_t *DataMapInit<className>(className *)                                                     \
