...
// ------------------------------------------------------------------------ //
typedef void (*SendVarProxyFn)(const SendProp *pProp, const void *pStructBase, const void *pData, DVariant *pOut,
...
typedef void *(*SendTableProxyFn)(const SendProp *pProp, const void *pStructBase, const void *pData,
...
public:
	...
	void SetRecipient(
		int iClient); // Note: these are CLIENT indices, not entity indices (so the first player's index is 0).
...
public:
	...
	const char
		*m_pExcludeDTName; // If this is an exclude prop, then this is the name of the datatable to exclude a prop from.
	...
	{                                                                      \
		...
		static SendProp g_SendProps[] = {                                  \
			SendPropInt("should_never_see_this", 0,                        \
						sizeof(int)), // It adds a dummy property at the start so you can define "empty" SendTables.
...
#varName, _hacky_dtsend_offsetof(currentSendDTClass::MakeANetworkVar_##varName, varName), \
...
#varName, _hacky_dtsend_offsetof(currentSendDTClass::MakeANetworkVar_##varName, varName), \
		sizeof(((currentSendDTClass *)0)->varName[0]),                                        \
		sizeof(((currentSendDTClass *)0)->varName) / sizeof(((currentSendDTClass *)0)->varName[0])
#define SENDINFO_ARRAYELEM(varName, i)                                                                      \
#varName "[" #i "]", _hacky_dtsend_offsetof(currentSendDTClass::MakeANetworkVar_##varName, varName[i]), \
		sizeof(((currentSendDTClass *)0)->varName[0])
#define SENDINFO_NETWORKARRAYELEM(varName, i)                                                                       \
#varName "[" #i "]", _hacky_dtsend_offsetof(currentSendDTClass::MakeANetworkVar_##varName, varName.m_Value[i]), \
...
#varName "[" #i "]",                                                                                 \
		-(int)_hacky_dtsend_offsetof(currentSendDTClass::MakeANetworkVar_##varName, varName.m_Value[i]), \
		sizeof(((currentSendDTClass *)0)->varName.m_Value[0])

#define SENDINFO_STRUCTELEM(varName) \
#varName, _hacky_dtsend_offsetof(currentSendDTClass, varName), sizeof(((currentSendDTClass *)0)->varName.m_Value)
#define SENDINFO_STRUCTARRAYELEM(varName, i)                                             \
#varName "[" #i "]", _hacky_dtsend_offsetof(currentSendDTClass, varName.m_Value[i]), \
		sizeof(((currentSendDTClass *)0)->varName.m_Value[0])

// Use this when you're not using a CNetworkVar to represent the data you're sending.
#define SENDINFO_NOCHECK(varName) \
#varName, _hacky_dtsend_offsetof(currentSendDTClass, varName), sizeof(((currentSendDTClass *)0)->varName)
...
#remoteVarName, _hacky_dtsend_offsetof(currentSendDTClass, varName), sizeof(((currentSendDTClass *)0)->varName)
...
// ------------------------------------------------------------------------ //
SendProp SendPropFloat(const char *pVarName, // Variable name.
...
SendProp SendPropVector(const char *pVarName, int offset, int sizeofVar = SIZEOF_IGNORE,
...
SendProp SendPropVectorXY(const char *pVarName, int offset, int sizeofVar = SIZEOF_IGNORE,
						  int nBits = 32, // Number of bits (for each floating-point component) to use when encoding.
						  int flags = SPROP_NOSCALE,
						  float fLowValue = 0.0f,		   // For floating point, low and high values.
						  float fHighValue = HIGH_DEFAULT, // High value. If HIGH_DEFAULT, it's (1<<nBits).
...
SendProp SendPropAngle(const char *pVarName, int offset, int sizeofVar = SIZEOF_IGNORE, int nBits = 32, int flags = 0,
					   SendVarProxyFn varProxy = SendProxy_AngleToFloat);

SendProp SendPropQAngles(const char *pVarName, int offset, int sizeofVar = SIZEOF_IGNORE, int nBits = 32, int flags = 0,
						 SendVarProxyFn varProxy = SendProxy_QAngles);

SendProp SendPropInt(const char *pVarName, int offset,
...
SendProp SendPropString(const char *pVarName, int offset, int bufferLen, int flags = 0,
...
#define SendPropArray(varTemplate, arrayName) SendPropVariableLengthArray(0, varTemplate, arrayName)
...
#define SendPropVirtualArray(arrayLengthSendProxy, maxArrayLength, varTemplate, propertyName) \
	varTemplate, InternalSendPropArray(maxArrayLength, 0, #propertyName, arrayLengthSendProxy)

#define SendPropVariableLengthArray(arrayLengthSendProxy, varTemplate, arrayName)        \
	varTemplate, InternalSendPropArray(sizeof(((currentSendDTClass *)0)->arrayName) /    \
										   PROPSIZEOF(currentSendDTClass, arrayName[0]), \
									   PROPSIZEOF(currentSendDTClass, arrayName[0]), #arrayName, arrayLengthSendProxy)
...
#define SendPropArray2(arrayLengthSendProxy, varTemplate, elementCount, elementStride, arrayName) \
	varTemplate, InternalSendPropArray(elementCount, elementStride, #arrayName, arrayLengthSendProxy)
