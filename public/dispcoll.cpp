...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::SweptAABBTriIntersect(Vector &rayStart, Vector &rayEnd, Vector &extents, CDispCollTri const *pTri,
										  Vector &plNormal, float *plDist, float *fraction)
{
	...
	*fraction = 1.0f;
	...
	//
	if((pTri->m_Normal[0] > ONE_MINUS_COLLISION_EPSILON) || (pTri->m_Normal[1] > ONE_MINUS_COLLISION_EPSILON) ||
	...
	// test face plane
	//
	dist = (pTri->m_Normal.x * (boxPt.x - pTri->m_Points[0].x)) + (pTri->m_Normal.y * (boxPt.y - pTri->m_Points[0].y)) +
...
bool CDispCollTree::AABBTriIntersect(CDispCollTreeTempData *pTemp, CDispCollData *pData)
{
	...
	for(int i = 0; i < pTemp->m_TriListCount; i++)
	{
		...
		{
			bResult = SweptAABBTriIntersect(pData->m_StartPos, pData->m_EndPos, pData->m_Extents, pTemp->m_ppTriList[i],
											normal, &dist, &fraction);
			if(bResult)
			{
				if(fraction < pData->m_Fraction)
				{
...
bool CDispCollTree::IntersectAABBTriTest(Vector &rayStart, Vector &extents, CDispCollTri const *pTri)
{
	...
	if(((pTri->m_Normal.x * (boxPt.x - pTri->m_Points[0].x)) + (pTri->m_Normal.y * (boxPt.y - pTri->m_Points[0].y)) +
...
bool CDispCollTree::SweptAABBTriTest(Vector &rayStart, Vector &rayEnd, Vector &extents, CDispCollTri const *pTri)
{
	...
	//
	// do the opposite because the ray has been negated
	if(((pTri->m_Normal.x * (boxPt.x - points[0].x)) + (pTri->m_Normal.y * (boxPt.y - points[0].y)) +
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::CullTriList(CDispCollTreeTempData *pTemp, Vector &rayStart, Vector &rayEnd, Vector &extents,
								bool bIntersect)
{
	//
...
// Purpose:
//-----------------------------------------------------------------------------
bool CDispCollTree::SweptAABBAABBTest(CDispCollTreeTempData *pTemp, const Vector &rayStart, const Vector &rayEnd,
...
// Purpose:
//-----------------------------------------------------------------------------
void CDispCollTree::BuildTriList_r(CDispCollTreeTempData *pTemp, int nodeIndex, Vector &rayStart, Vector &rayEnd,
								   Vector &extents, bool bIntersect)
{
	//
