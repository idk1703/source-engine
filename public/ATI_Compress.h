...
typedef enum
{
	...
	ATI_TC_FORMAT_ARGB_2101010, ///< An ARGB format with 10-bit fixed channels for color & a 2-bit fixed channel for
								///< alpha.
	...
	ATI_TC_FORMAT_DXT5_RxBG, ///< A swizzled DXT5 format with the green component swizzled into the alpha channel. Eight
							 ///< bits per pixel.
	ATI_TC_FORMAT_DXT5_RBxG, ///< A swizzled DXT5 format with the green component swizzled into the alpha channel & the
							 ///< blue component swizzled into the green channel. Eight bits per pixel.
	ATI_TC_FORMAT_DXT5_xRBG, ///< A swizzled DXT5 format with the green component swizzled into the alpha channel & the
							 ///< red component swizzled into the green channel. Eight bits per pixel.
	ATI_TC_FORMAT_DXT5_RGxB, ///< A swizzled DXT5 format with the blue component swizzled into the alpha channel. Eight
							 ///< bits per pixel.
	ATI_TC_FORMAT_DXT5_xGxR, ///< A two-component swizzled DXT5 format with the red component swizzled into the alpha
							 ///< channel & the green component in the green channel. Eight bits per pixel.
	ATI_TC_FORMAT_ATI1N,	///< A single component compression format using the same technique as DXT5 alpha. Four bits
							///< per pixel.
	ATI_TC_FORMAT_ATI2N,	///< A two component compression format using the same technique as DXT5 alpha. Designed for
							///< compression object space normal maps. Eight bits per pixel.
	ATI_TC_FORMAT_ATI2N_XY, ///< A two component compression format using the same technique as DXT5 alpha. The same as
							///< ATI2N but with the channels swizzled. Eight bits per pixel.
	ATI_TC_FORMAT_ATI2N_DXT5, ///< An ATI2N like format using DXT5. Intended for use on GPUs that do not natively
							  ///< support ATI2N. Eight bits per pixel.
	ATI_TC_FORMAT_BC1, ///< A four component opaque (or 1-bit alpha) compressed texture format for Microsoft DirectX10.
					   ///< Identical to DXT1.  Four bits per pixel.
	ATI_TC_FORMAT_BC2, ///< A four component compressed texture format with explicit alpha for Microsoft DirectX10.
					   ///< Identical to DXT3. Eight bits per pixel.
	ATI_TC_FORMAT_BC3, ///< A four component compressed texture format with interpolated alpha for Microsoft DirectX10.
					   ///< Identical to DXT5. Eight bits per pixel.
	ATI_TC_FORMAT_BC4, ///< A single component compressed texture format for Microsoft DirectX10. Identical to ATI1N.
					   ///< Four bits per pixel.
	ATI_TC_FORMAT_BC5, ///< A two component compressed texture format for Microsoft DirectX10. Identical to ATI2N. Eight
					   ///< bits per pixel.
	ATI_TC_FORMAT_ATC_RGB,				 ///< ATI_TC - a compressed RGB format for cellphones & hand-held devices.
	ATI_TC_FORMAT_ATC_RGBA_Explicit,	 ///< ATI_TC - a compressed ARGB format with explicit alpha for cellphones &
										 ///< hand-held devices.
	ATI_TC_FORMAT_ATC_RGBA_Interpolated, ///< ATI_TC - a compressed ARGB format with interpolated alpha for cellphones &
										 ///< hand-held devices.
...
typedef struct
{
	...
	BOOL bUseChannelWeighting;	 ///< Use channel weightings. With swizzled formats the weighting applies to the data
								 ///< within the specified channel not the channel itself.
	...
	ATI_TC_BYTE nAlphaThreshold; ///< The alpha threshold to use when compressing to DXT1 & BC1 with bDXT1UseAlpha.
								 ///< Texels with an alpha value less than the threshold are treated as transparent.
	BOOL bDisableMultiThreading; ///< Disable multi-threading of the compression. This will slow the compression but can
								 ///< be useful if you're managing threads in your application.
...
extern "C"
{
	...
	ATI_TC_ERROR ATI_TC_API ATI_TC_ConvertTexture(const ATI_TC_Texture *pSourceTexture, ATI_TC_Texture *pDestTexture,
												  ...
												  ATI_TC_Feedback_Proc pFeedbackProc, DWORD_PTR pUser1,
												  DWORD_PTR pUser2);
