...
public:
	...
	bool FindMultiTrackGroupForClip(CDmeClip *pClip, int *pTrackGroupIndex, int *pTrackIndex = NULL,
									int *pClipIndex = NULL) const;

	// Finding clips in tracks by time
	virtual void FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time, DmeClipSkipFlag_t flags,
								 CUtlVector<CDmeClip *> &clips) const;
	virtual void FindClipsWithinTime(DmeClipType_t clipType, DmeTime_t startTime, DmeTime_t endTime,
...
public:
	...
	virtual void ApplyEffect(DmeTime_t time, Rect_t &currentRect, Rect_t &totalRect,
...
public:
	...
	void ScaleAssociatedClips(CDmeClip *pClip, CUtlVector<ClipAssociation_t> &association, float ratio,
							  DmeTime_t oldOffset);
...
CDmeChannel *FindChannelTargetingElement(CDmeFilmClip *pClip, CDmElement *pElement, const char *pAttributeName,
										 CDmeChannelsClip **ppChannelsClip, CDmeTrack **ppTrack = NULL,
										 CDmeTrackGroup **ppTrackGroup = NULL);
