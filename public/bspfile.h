...
struct dleaf_t
{
	...
	// NOTE: removed this for version 1 and moved into separate lump "LUMP_LEAF_AMBIENT_LIGHTING" or
	// "LUMP_LEAF_AMBIENT_LIGHTING_HDR" Precaculated light info for entities.
	//	CompressedLightCube m_AmbientLighting;
};
...
inline void dwateroverlay_t::SetRenderOrder(unsigned short order)
{
	...
	m_nFaceCountAndRenderOrder |=
		(order << (16 - WATEROVERLAY_RENDER_ORDER_NUM_BITS)); // leave 2 bits for render order.
}
