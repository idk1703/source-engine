...
{
public:
	CUtlCachedFileData(char const *repositoryFileName, int version, PFNCOMPUTECACHEMETACHECKSUM checksumfunc = NULL,
					   UtlCachedFileDataType_t fileCheckType = UTL_CACHED_FILE_USE_TIMESTAMP,
					   bool nevercheckdisk = false, bool readonly = false, bool savemanifest = false)
	...
	// If you create a cache and don't call init/shutdown, you can call this to do a quick check to see if the
	// checksum/version
...
private:
	...
	{
		ElementType_t() : handle(0), fileinfo(0), diskfileinfo(UTL_CACHED_FILE_DATA_UNDEFINED_DISKINFO), dataIndex(-1)
		{
		}
		...
		if(version == m_nVersion)
		{
			...
			if(cache_meta_checksum == m_uCurrentMetaChecksum)
			{
				...
				for(int i = 0; i < count; ++i)
				{
					...
					if((element.fileinfo == -1) && (m_fileCheckType == UTL_CACHED_FILE_USE_FILESIZE))
					{
						element.fileinfo = 0;
					}
...
void CUtlCachedFileData<T>::Save()
{
	...
	{
		ExecuteNTimes(
			25, Warning("Unable to persist cache '%s', check file permissions\n", m_sRepositoryFileName.String()));
	}
	else
	{
		SetDirty(false);
...
void CUtlCachedFileData<T>::SaveManifest()
{
	...
	if(g_pFullFileSystem->FileExists(manifest_name, "MOD") && !g_pFullFileSystem->IsFileWritable(manifest_name, "MOD"))
	{
