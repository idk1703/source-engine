...
#pragma warning(disable : 4284) // warning C4284: return type for 'CNetworkVarT<int>::operator ->' is 'int *' (ie; not a
								// UDT or reference to a UDT.  Will produce errors if applied using infix notation)

#define MyOffsetOf(type, var) ((uintp)&((type *)0)->var)
...
inline int InternalCheckDeclareClass(const char *pClassName, const char *pClassNameMatch, void *pTestPtr,
									 void *pBasePtr)
{
...
#define DECLARE_CLASS_GAMEROOT(className, baseClassName)                              \
	...
	static int CheckDeclareClass(const char *pShouldBe)                               \
	{                                                                                 \
		return InternalCheckDeclareClass(pShouldBe, #className, (ThisClass *)0xFFFFF, \
...
#define IMPLEMENT_NETWORKVAR_CHAIN(varName) (varName)->__m_pChainEntity.m_pEnt = this;
...
// NOTE: Assignment operator is disabled because it doesn't call copy constructors of scalar types within the aggregate,
// so they are not marked changed
...
#define CNetworkArrayInternal(type, name, count, stateChangedFn)                              \
	...
	protected:                                                                                \
		...
		{                                                                                     \
			CHECK_USENETWORKVARS((ThisClass *)(((char *)this) - MyOffsetOf(ThisClass, name))) \
				->stateChangedFn(&m_Value[net_change_index]);                                 \
		}                                                                                     \
		type m_Value[count];                                                                  \
	};                                                                                        \
	NetworkVar_##name name;
