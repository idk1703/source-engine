			...
			for(int try1 = 0; try1 < n_desired; try1++)
			{
				...
				if(li.IsDirectionWithinLightCone(trial_dir))
				{
					...
					if((rslt.HitIds[0] != -1))
					{
						...
						l1.m_Direction =
							Vector(rslt.surface_normal.X(0), rslt.surface_normal.Y(0), rslt.surface_normal.Z(0));
...
void RayTracingEnvironment::AddToRayStream(RayStream &s, Vector const &start, Vector const &end,
