...
public:
	...
	{
		return m_pFileSystemPassThru->GetTier();
	}
	...
	virtual void FindFileAbsoluteList(CUtlVector<CUtlString> &outAbsolutePathNames, const char *pWildCard,
									  const char *pPathID)
	{
		m_pFileSystemPassThru->FindFileAbsoluteList(outAbsolutePathNames, pWildCard, pPathID);
	}
	...
	virtual FSAsyncStatus_t AsyncReadMultipleCreditAlloc(const FileAsyncRequest_t *pRequests, int nRequests,
														 const char *pszFile, int line, FSAsyncControl_t *pControls)
	{
	...
	{
		return m_pFileSystemPassThru->AsyncDirectoryScan(pSearchSpec, recurseFolders, pContext, pfnAdd, pfnDone,
														 pControl);
	}
	...
	{
		return m_pFileSystemPassThru->PrefetchFile(pFileName, nPriority, bPersist, nOffset, nSize);
	}
	virtual void FlushCache()
	{
		m_pFileSystemPassThru->FlushCache();
	}
	...
	virtual FSAsyncStatus_t AsyncAppendFile(const char *pDestFileName, const char *pSrcFileName,
											FSAsyncControl_t *pControl)
	{
	...
	{
		return m_pFileSystemPassThru->RelativePathToFullPath(pFileName, pPathID, pLocalPath, localPathBufferSize,
	...
	{
		return m_pFileSystemPassThru->GetPackFileInfoFromRelativePath(pFileName, pPathID, pPackPath,
																	  nPackPathBufferSize, nPosition, nLength);
	}
#endif
	...
	virtual int ReadFileEx(const char *pFileName, const char *pPath, void **ppBuf, bool bNullTerminate,
						   bool bOptimalAlloc, int nMaxBytes = 0, int nStartingByte = 0, FSAllocFunc_t pfnAlloc = NULL)
	{
		return m_pFileSystemPassThru->ReadFileEx(pFileName, pPath, ppBuf, bNullTerminate, bOptimalAlloc, nMaxBytes,
	...
	virtual void RegisterFileWhitelist(IFileList *pForceMatchList, IFileList *pAllowFromDiskList,
	...
	virtual bool IsDVDHosted()
	{
		return m_pFileSystemPassThru->IsDVDHosted();
	}
	...
	virtual void AddVPKFile(char const *pPkName, SearchPathAdd_t addType = PATH_ADD_TO_TAIL)
	{
	...
	{
		return m_pFileSystemPassThru->GetStringFromKVPool(poolKey, key, pOutBuff, buflen);
	}
	...
	{
		return m_pFileSystemPassThru->GetAnyDLCInfo(iDLC, pLicenseMask, pTitleBuff, nOutTitleSize);
	}
	...
	{
		return m_pFileSystemPassThru->GetAnyCorruptDLCInfo(iCorruptDLC, pTitleBuff, nOutTitleSize);
	}
	...
	{
		return m_pFileSystemPassThru->GetIoStats();
	}
...
// This is so people who change the filesystem interface are forced to add the passthru wrapper into
// CFileSystemPassThru, so they don't break VMPI.
