...
public:
	...
	virtual const char *RelativePathToFullPath(const char *pFileName, const char *pPathID, char *pLocalPath,
											   int localPathBufferSize, PathTypeFilter_t pathFilter = FILTER_NONE,
											   PathTypeQuery_t *pPathType = NULL) = 0;
#if IsGameConsole()
	// Given a relative path, gets the PACK file that contained this file and its offset and size. Can be used to
	// prefetch a file to a HDD for caching reason.
	virtual bool GetPackFileInfoFromRelativePath(const char *pFileName, const char *pPathID, char *pPackPath,
	...
	// Searches for a file in all paths and results absolute path names for the file, works in pack files (zip and vpk)
	// too Lets you search for something like sound/sound.cache and get a list of every sound cache
	virtual void FindFileAbsoluteList(CUtlVector<CUtlString> & outAbsolutePathNames, const char *pWildCard,
	...
	// Suspends all prefetches (like when the game is doing a file intensive operation not controlled by the HDD cache,
	// like Bink movies).
	virtual void SuspendPrefetches(const char *pWhy) = 0;
	// Resumes prefetches. This function has to to be called as many time as SuspendPrefetches() to effectively resumes
	// prefetches.
	virtual void ResumePrefetches(const char *pWhy) = 0;

	// Gets called when we are starting / ending a save (it allows the file system to reduce its HDD usage and use
	// BluRay instead).
	...
	virtual FSAsyncStatus_t AsyncDirectoryScan(const char *pSearchSpec, bool recurseFolders, void *pContext,
	...
	// are now "dirty". For example, if you were on a non-pure server and you loaded a certain model, and then you
	// connected to a pure server that said that model had to come from Steam, then pFilesToReload would specify that
	// model and the engine should reload it so it can come from Steam.
	//
	// Be sure to call Release() on pFilesToReload.
	virtual void RegisterFileWhitelist(IFileList * pWantCRCList, IFileList * pAllowFromDiskList,
...
#define SetLastProfileFileRead(s) \
	V_strncpy(g_szXboxProfileLastFileOpened, sizeof(g_szXboxProfileLastFileOpened), pFileName)
