...
#if(defined(PLATFORM_WINDOWS) || defined(PLATFORM_X360)) && !defined(STEAM) && !defined(_CERT) && \
	defined(TCHAR_IS_CHAR) // designed for windows/x360, not built/tested with wide characters, not intended for release
						   // builds (but probably wouldn't damage anything)
#define ENABLE_RUNTIME_STACK_TRANSLATION // uncomment to enable runtime stack translation tools. All of which use
										 // on-demand loading of necessary dll's and pdb's
#endif

#if defined(ENABLE_RUNTIME_STACK_TRANSLATION)
//#define ENABLE_THREAD_PARENT_STACK_TRACING 1 //uncomment to actually enable tracking stack traces from threads and
//jobs to their parent thread. Must also define THREAD_PARENT_STACK_TRACE_SUPPORTED in threadtools.h
...
enum TranslateStackInfo_StyleFlags_t
{
	...
	TSISTYLEFLAG_DEFAULT = (TSISTYLEFLAG_MODULENAME | TSISTYLEFLAG_SYMBOLNAME | TSISTYLEFLAG_FULLPATH |
...
// On 360 this generates a string that can be decoded by VXConsole in print functions. Optimal path for translation
// because it's one way. Other paths require multiple transactions.
PLATFORM_INTERFACE int TranslateStackInfo(const void *const *pCallStack, int iCallStackCount, tchar *szOutput,
...
PLATFORM_INTERFACE void PreloadStackInformation(
	...
	int iAddressCount); // caches data and reduces communication with VXConsole to speed up 360 decoding when using any
						// of the Get***FromAddress() functions. Nop on PC.
PLATFORM_INTERFACE bool GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut, int iMaxFileNameLength,
												  uint32 &iLineNumberOut, uint32 *pDisplacementOut = NULL);
PLATFORM_INTERFACE bool GetSymbolNameFromAddress(const void *pAddress, tchar *pSymbolNameOut, int iMaxSymbolNameLength,
...
class PLATFORM_CLASS
	CCallStackStorage // a helper class to grab a stack trace as close to the leaf code surface as possible, then pass
					  // it on to deeper functions intact with less unpredictable inlining pollution
{
public:
	...
	void *pStack[128]; // probably too big, possibly too small for some applications. Don't want to spend the time
					   // figuring out how to generalize this without templatizing pollution or mallocs
	uint32 iValidEntries;
};

// Hold onto one of these to denote the top of a functional stack trace. Also allows us to string together threads to
// their parents
...
// just references the parent stack. Assuming that you'll keep that memory around as long as you're keeping this Stack
// Top marker.
...
public:
	...
	void ReleaseParentStackReferences(
		void); // in case you need to delete the parent stack trace before this class goes out of scope
};
...
PLATFORM_INTERFACE int EncodeBinaryToString(const void *pToEncode, int iDataLength, char *pEncodeOut,
...
PLATFORM_INTERFACE int DecodeBinaryFromString(const char *pString, void *pDestBuffer, int iDestBufferSize,
