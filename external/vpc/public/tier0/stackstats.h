...
{
	void (*pfn_GetDumpInfo)(void *, const char *&, size_t &, size_t &, void *&, size_t &,
							CCallStackStatsGatherer_Standardized_t *&, size_t &);
...
// templatized classes are fun, can't really use a base pointer effectively, so we'll have a translator struct and store
// pointers to instances of the translator function
class CCallStackStatsGatherer_Standardized_t
{
public:
	CCallStackStatsGatherer_Standardized_t(void) {};
	CCallStackStatsGatherer_Standardized_t(void *pThis, const CCallStackStatsGatherer_FunctionTable_t &FnTable)
	...
	void PushSubTree(const CCallStackStatsGatherer_Standardized_t &SubTree,
...
PLATFORM_INTERFACE bool _CCallStackStatsGatherer_Internal_DumpStatsToFile(
	const char *szFileName, const CCallStackStatsGatherer_Standardized_t &StatsGatherer, bool bAllowMemoryAllocations);

template<class STATSTRUCT>
class CCallStackStatsGatherer_StructAccessor_Base
{
public:
	CCallStackStatsGatherer_StructAccessor_Base(const CCallStackStatsGatherer_Standardized_t &Gatherer, int iEntryIndex)
		: m_Gatherer(Gatherer), m_iEntryIndex(iEntryIndex) {};

protected:
	uint32 m_iEntryIndex; // index of the stat entry we want in the vector. Stored as index as the vector base address
						  // can change.
	CCallStackStatsGatherer_Standardized_t
		m_Gatherer; // so we can lock the vector memory in place while manipulating the values
};
...
// STACKACQUISITIONFUNCTION - The function to use to gather the current call stack. GetCallStack() is safe,
// GetCallStack_Fast() is faster, but has special requirements STATMUTEXHANDLER - If you want automatic mutex management
// for your individual entries, supply a handler here. 						You'll need to not only call GetEntry(), but lock/unlock the
//entry while accessing it. TEMPLATIZEDMEMORYALLOCATOR - We'll need to allocate memory, supply an allocator if you want
// to manage that
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION = GetCallStack,
		 typename STATMUTEXHANDLER = CCallStackStatsGatherer_StatMutexPool<4>,
		 template<typename T> class TEMPLATIZEDMEMORYALLOCATOR = CDefaultStatsGathererAllocator>
...
public:
	...
	CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> GetEntry(
		void *const CallStack[CAPTUREDCALLSTACKLENGTH]); // get the entry using some callstack grabbed a while ago.
														 // Assumes ALL invalid entries have been nullified
	CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> GetEntry(
		void *const CallStack[CAPTUREDCALLSTACKLENGTH],
		uint32 iValidEntries); // same as above, but does the work of nullifying invalid entries
	CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> GetEntry(
		const CCallStackStorage &PushStack = CCallStackStorage(STACKACQUISITIONFUNCTION));
	CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> GetEntry(uint32 iEntryIndex);

	// get the entry index for the caller's current call stack. Pre-nullified entries count as valid entries (and save
	// re-nullification work)
	uint32 GetEntryIndex(
		void *const CallStack[CAPTUREDCALLSTACKLENGTH],
		uint32 iValidEntries); // index is unchanging, safe to keep and use later (designed for exactly that purpose)
	...
	static void GetDumpInfo(void *pThis, const char *&szStructName, size_t &iCapturedStackLength,
							size_t &iEntrySizeWithStack, void *&pEntries, size_t &iEntryCount,
							CCallStackStatsGatherer_Standardized_t *&pSubTrees, size_t &iSubTreeCount);
	static void PushSubTree(void *pParent, const CCallStackStatsGatherer_Standardized_t &SubTree,
	...
	const static FN_GetCallStack StackFunction; // publish the requested acquisition function so you can easily key all
												// your stack gathering to the class instance
...
#pragma pack(pop)
	...
	typedef CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
	...
	struct IndexMapKey_t
	{
		...
		bool operator<(const IndexMapKey_t &key) const
		{
			...
			// only here if there's a hash match. Do a full check. Extremely likely to be the exact same call stack. But
			// not 100% guaranteed.
			for(int i = 0; i < CAPTUREDCALLSTACKLENGTH; ++i)
			{
	...
	void KeepSubTree(CCallStackStatsGatherer_Standardized_t &SubTree)
	{
		...
		for(StoredSubTreeVector_t::iterator treeIter = m_StoredSubTrees.begin(); treeIter != m_StoredSubTrees.end();
			++treeIter)
		{
	...
	uint32 PatchInSubTrees(void *const CallStackIn[CAPTUREDCALLSTACKLENGTH],
						   void *CallStackOut[CAPTUREDCALLSTACKLENGTH], uint32 iValidEntries)
	{
		...
		void *pMatchAddress = treeIter->Stack[1]; // while the first entry is where the actual push was made. The second
												  // entry is the first that is matchable in most cases
	...
	typedef std::map<IndexMapKey_t, StatIndex_t, std::less<IndexMapKey_t>,
	...
	typedef std::vector<CCallStackStatsGatherer_Standardized_t,
	...
	static size_t NULL_DescribeCallStackStatStruct(uint8 *pDescribeWriteBuffer, size_t iDescribeMaxLength)
	{
		return 0;
	}
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
const FN_GetCallStack CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
											  STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::StackFunction =
	STACKACQUISITIONFUNCTION;

template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
const size_t CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
									 TEMPLATIZEDMEMORYALLOCATOR>::CapturedCallStackLength = CAPTUREDCALLSTACKLENGTH;

#if defined(ENABLE_STACK_STATS_GATHERING)

class CallStackStatStructDescFuncs;
PLATFORM_INTERFACE size_t _CCallStackStatsGatherer_Write_FieldDescriptions(
	CallStackStatStructDescFuncs *pFieldDescriptions, uint8 *pWriteBuffer, size_t iWriteBufferSize);
// PLATFORM_INTERFACE size_t _CCallStackStatsGatherer_Write_FieldMergeScript( CallStackStatStructDescFuncs
// *pFieldDescriptions, CallStackStatStructDescFuncs::MergeScript_Language scriptMergeLanguage, uint8 *pWriteBuffer,
...
#define BEGIN_STATSTRUCTFIELDDESCRIPTION(className)                               \
	CallStackStatStructDescFuncs *className::GetStatStructFieldDescriptions(void) \
	{                                                                             \
...
#define _DEFINE_STATSTRUCTFIELD_VARNAME(varName, fieldName, fieldStruct, fieldParmsInParentheses) \
...
#define DEFINE_STATSTRUCTFIELD(fieldName, fieldStruct, fieldParmsInParentheses) \
	_DEFINE_STATSTRUCTFIELD_VARNAME(fieldName, fieldName, fieldStruct, fieldParmsInParentheses)
#define DEFINE_STATSTRUCTFIELD_ARRAYENTRY(arrayName, arrayIndex, fieldStruct, fieldParmsInParentheses)  \
	_DEFINE_STATSTRUCTFIELD_VARNAME(arrayName##_##arrayIndex, arrayName##[##arrayIndex##], fieldStruct, \
...
#define WRITE_STATSTRUCT_FIELDDESCRIPTION()                          \
	iWroteBytes += _CCallStackStatsGatherer_Write_FieldDescriptions( \
		GetStatStructFieldDescriptions(), pDescribeWriteBuffer + iWroteBytes, iDescribeMaxLength - iWroteBytes);
...
//CallStackStatStructDescFuncs::scriptMergeLanguage, pDescribeWriteBuffer + iWroteBytes, iDescribeMaxLength -
//iWroteBytes );
...
CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> CCallStackStatsGatherer<
	STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::
	GetEntry(void *const CallStack[CAPTUREDCALLSTACKLENGTH]) // get the entry using some callstack grabbed a while ago.
															 // Assumes ALL invalid entries have been nullified
{
#if defined(ENABLE_STACK_STATS_GATHERING)
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> CCallStackStatsGatherer<
	STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
uint32 CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							   TEMPLATIZEDMEMORYALLOCATOR>::GetEntryIndex(const CCallStackStorage &PushStack)
{
#if defined(ENABLE_STACK_STATS_GATHERING)
	COMPILE_TIME_ASSERT(CAPTUREDCALLSTACKLENGTH <= ARRAYSIZE(PushStack.pStack));
...
CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> CCallStackStatsGatherer<
	STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
	TEMPLATIZEDMEMORYALLOCATOR>::GetEntry(uint32 iEntryIndex)
{
#if defined(ENABLE_STACK_STATS_GATHERING)
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
uint32 CCallStackStatsGatherer<
	STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
	TEMPLATIZEDMEMORYALLOCATOR>::GetEntryIndex(void *const CallStack[CAPTUREDCALLSTACKLENGTH],
																	 ...
																	 // (designed for exactly that purpose)
{
	...
	// if we have a sub-tree. We'll be splicing it into the original call stack as if it were a return address. Then
	// patching that when we interpret the results later A stack with a sub-tree along the line is treated as distinctly
	// different than one without a sub-tree
	...
	if(indexMapIter.first->second.m_Index == -1)
	{
		...
		memcpy(m_StatEntries[indexMapIter.first->second.m_Index].m_CallStack, PatchedStack,
			   sizeof(void *) * CAPTUREDCALLSTACKLENGTH);
...
void CCallStackStatsGatherer<
	STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
	TEMPLATIZEDMEMORYALLOCATOR>::GetCallStackForIndex(uint32 iEntryIndex, void *CallStackOut[CAPTUREDCALLSTACKLENGTH])
{
#if defined(ENABLE_STACK_STATS_GATHERING)
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
size_t CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							   TEMPLATIZEDMEMORYALLOCATOR>::NumEntries(void) const
{
#if defined(ENABLE_STACK_STATS_GATHERING)
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
bool CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
...
const CCallStackStatsGatherer_FunctionTable_t &CCallStackStatsGatherer<
	STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
	TEMPLATIZEDMEMORYALLOCATOR>::GetFunctionTable(void)
{
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							 TEMPLATIZEDMEMORYALLOCATOR>::GetDumpInfo(void *pThis, const char *&szStructName,
																	  ...
																	  size_t &iSubTreeCount)
{
	...
	iEntrySizeWithStack =
		sizeof(CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							 TEMPLATIZEDMEMORYALLOCATOR>::PushSubTree(void *pParent,
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							 TEMPLATIZEDMEMORYALLOCATOR>::PushSubTree(CCallStackStatsGatherer_Standardized_t &Parent,
																	  const CCallStackStorage &PushStack)
{
#if defined(ENABLE_STACK_STATS_GATHERING)
	CCallStackStatsGatherer_Standardized_t StandardThis = Standardized();
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							 TEMPLATIZEDMEMORYALLOCATOR>::PopSubTree(void *pParent)
{
#if defined(ENABLE_STACK_STATS_GATHERING)
	ThisCast *pParentCast = (ThisCast *)pParent;
	pParentCast->m_SubTreeMutex.Lock();
	...
	for(PushedSubTreeVector_t::reverse_iterator treeIter = pParentCast->m_PushedSubTrees.rbegin();
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							 TEMPLATIZEDMEMORYALLOCATOR>::SyncMutexes(void *pParent,
																	  bool bLock) // true for lock, false for unlock
{
	...
	if(bLock)
	{
		...
		for(StoredSubTreeVector_t::iterator treeIter = pParentCast->m_StoredSubTrees.begin();
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							 TEMPLATIZEDMEMORYALLOCATOR>::ApplyTreeAccessLock(void *pParent, bool bLock)
{
#if defined(ENABLE_STACK_STATS_GATHERING)
	ThisCast *pParentCast = (ThisCast *)pParent;
	if(bLock)
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
							 TEMPLATIZEDMEMORYALLOCATOR>::LockEntry(void *pParent, uint32 iEntryIndex, bool bLock)
{
#if defined(ENABLE_STACK_STATS_GATHERING)
	ThisCast *pParentCast = (ThisCast *)pParent;
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
						TEMPLATIZEDMEMORYALLOCATOR>::operator CCallStackStatsGatherer_Standardized_t(void)
{
...
template<class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH, FN_GetCallStack STACKACQUISITIONFUNCTION,
		 typename STATMUTEXHANDLER, template<typename T> class TEMPLATIZEDMEMORYALLOCATOR>
CCallStackStatsGatherer_Standardized_t CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
															   STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
															   TEMPLATIZEDMEMORYALLOCATOR>::Standardized(void)
{
	return CCallStackStatsGatherer_Standardized_t(this, GetFunctionTable());
}
...
public:
	...
	{
		DFV_BasicStatStructFieldTypes_t, // Format: 1 byte BasicStatStructFieldTypes_t type, 4 byte field offset, null
										 // terminated string field name
	};

	const char *m_szFieldName;
	size_t m_iFieldOffset;
	CallStackStatStructDescFuncs *m_pNext; // needed for how the description macros are laid out. Couldn't figure out a
										   // way to store the static struct instances as a static array
};
...
{
public:
	BasicStatStructFieldDesc(BasicStatStructFieldTypes_t type, BasicStatStructFieldCombineMethods_t combineMethod)
...
public:
		...
		: CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT>(copyFrom)
	{
		...
		this->m_pStruct = (STATSTRUCT *)this->m_Gatherer.pFunctionTable->pfn_GetEntry(this->m_Gatherer.pGatherer,
																					  this->m_iEntryIndex);
	}
...
// struct is locked in place only between Lock() and paired Unlock() calls. Get a base, then create one of these from
// it. It's safe to hold onto this for an extended period of time. The entry index is unchanging in the gatherer tree.
template<class STATSTRUCT>
...
{
public:
	CCallStackStats_PushSubTree_AutoPop(const CCallStackStatsGatherer_Standardized_t &Parent,
