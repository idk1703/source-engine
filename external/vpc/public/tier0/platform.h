...
// There are known bugs in the PS3 optimizer.  The following macros allow us to lower optimization for a subset of a
// file If you run into build problems with optimization on, try turning off optimization for the selected file.  If
// that fixes the problem, use process of elimination and the below macros to find the bare minimum that needs to be
...
inline __attribute__((always_inline)) void __vec_st_unaligned3(__m128 in, float *out)
{
	...
	__m128 temp1 = vec_ld(16, out);
	vector unsigned char align = vec_lvsr(0, out);
	vector unsigned char mask = vec_perm(
...
#ifdef GNUC
#undef offsetof
// Note: can't use builtin offsetof because many use cases (esp. in templates) wouldn't compile due to restrictions on
// the builtin offsetof
...
// #pragma diag_suppress=1700, 83, 162, 182, 192, 194, 229, 238, 262, 341, 382, 401, 402, 403, 481, 817, 828, 833, 1363,
// 1771, 1774, 1779, 1780, 1783, 1785, 1786, 1788
#endif
...
static bool sPS3_SuppressAssertsInThisFile =
	false; // you can throw this in the debugger to temporarily disable asserts inside any particular .cpp module.
#define DebuggerBreak()                 \
...
// Convert a time_t (specified in nTime - seconds since Jan 1, 1970 UTC) to a local calendar time in a threadsafe and
// non-crash-prone way.
