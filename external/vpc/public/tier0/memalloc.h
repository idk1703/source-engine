...
public:
	...
	inline void *IndirectAlloc(size_t nSize)
	{
		return Alloc(nSize);
	}
	...
	// Request 'generic' memory stats (returns a list of N named values; caller should assume this list will change over
	// time)
...
{
	static const char *pszName = typeid(*p).name(); // @TODO: support having debug heap ignore certain allocations, and
													// ignore memory allocated here [5/7/2009 tom]
	return pszName;
}
...
#define MEMALLOC_DEFINE_EXTERNAL_TRACKING(tag)                                            \
	...
	const char *g_psz##tag##Alloc =                                                       \
		strcpy((char *)MemAlloc_Alloc(strlen(#tag "Alloc") + 1, "intentional leak", 0), #tag "Alloc");
