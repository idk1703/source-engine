...
#pragma pack(pop)
	...
	const tstring version_text() const
	{
		...
		_stprintf(b, _T("%d.%d.%d %s XVersion(%d.%d)"), version.Family, version.Model, version.Stepping, type_text(),
				  version.XFamily, version.XModel);
...
protected:
	...
	{
		static const tchar *text[] = {_T("Intel OEM Processor"), _T("Intel OverDrive(R) Processor"),
	...
	const tstring brand_text() const
	{
		static const tchar *text[] = {
			_T("n/a"),			_T("Celeron"),		_T("Pentium III"),		  _T("Pentium III Xeon"),
			_T("reserved (4)"), _T("reserved (5)"), _T("Pentium III Mobile"), _T("reserved (7)"),
			_T("Pentium 4")};
