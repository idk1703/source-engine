...
#define UTLBLOCKMEMORY_TRACK_ALLOC()                                                            \
	MemAlloc_RegisterAllocation("||Sum of all UtlBlockMemory||", 0, NumAllocated() * sizeof(T), \
								NumAllocated() * sizeof(T), 0)
#define UTLBLOCKMEMORY_TRACK_FREE()                                                                   \
	if(!m_pMemory)                                                                                    \
		;                                                                                             \
	else                                                                                              \
		MemAlloc_RegisterDeallocation("||Sum of all UtlBlockMemory||", 0, NumAllocated() * sizeof(T), \
									  NumAllocated() * sizeof(T), 0)
#else
...
{
	UTLBLOCKMEMORY_TRACK_FREE(); // this must stay before the recalculation of m_nBlocks, since it implicitly uses the
								 // old value

	int nBlocksOld = m_nBlocks;
	m_nBlocks = nBlocks;

	UTLBLOCKMEMORY_TRACK_ALLOC(); // this must stay after the recalculation of m_nBlocks, since it implicitly uses the
								  // new value
