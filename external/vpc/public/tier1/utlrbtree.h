...
#define FOR_EACH_UTLRBTREE(treeName, iteratorName)                                           \
	for(int iteratorName = treeName.FirstInorder(); iteratorName != treeName.InvalidIndex(); \
...
public:
	...
	CUtlFixedRBTree(const LessFunc_t &lessfunc) : CUtlRBTree<T, I, L, CUtlFixedMemory<UtlRBTreeNode_t<T, I>>>(lessfunc)
	{
	}
...
private:
	// this doesn't make sense for fixed rbtrees, since there's no useful max pointer, and the index space isn't
	// contiguous anyways
	I MaxElement() const;
};
...
I CUtlRBTree<T, I, L, M>::NewNode()
{
	...
	if(m_FirstFree == InvalidIndex())
	{
		...
		typename M::Iterator_t it =
			m_Elements.IsValidIterator(m_LastAlloc) ? m_Elements.Next(m_LastAlloc) : m_Elements.First();
