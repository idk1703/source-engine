...
public:
	...
	CUtlMemoryPool(int blockSize, int numElements, int growMode = GROW_FAST, const char *pszAllocOwner = NULL,
				   int nAlignment = 0);
	~CUtlMemoryPool();

	void *Alloc(); // Allocate the element size you specified in the constructor.
	void *Alloc(size_t amount);
	void *AllocZero(); // Allocate the element size you specified in the constructor, zero the memory before
					   // construction
	...
	int Size() const
	{
		return m_NumBlobs * m_BlocksPerBlob * m_BlockSize;
	}
...
{
public:
	CMemoryPoolMT(int blockSize, int numElements, int growMode = GROW_FAST, const char *pszAllocOwner = NULL,
				  int nAlignment = 0)
...
//-----------------------------------------------------------------------------
template<int ITEM_SIZE, int ALIGNMENT, int CHUNK_SIZE, class CAllocator, bool GROWMODE = false,
...
private:
	...
	CUtlVector<void *>
		m_Chunks; // Chunks are tracked outside blocks (unlike CUtlMemoryPool) to simplify alignment issues
...
#define DEFINE_FIXEDSIZE_ALLOCATOR_EXTERNAL(_class, _allocator) CUtlMemoryPool *_class::s_pAllocator = _allocator
...
template<int ITEM_SIZE, int ALIGNMENT, int CHUNK_SIZE, class CAllocator, bool GROWMODE, int COMPACT_THRESHOLD>
inline int __cdecl CAlignedMemPool<ITEM_SIZE, ALIGNMENT, CHUNK_SIZE, CAllocator, GROWMODE,
								   COMPACT_THRESHOLD>::CompareChunk(void *const *ppLeft, void *const *ppRight)
{
