...
template<class A>
static const char *GetFmtStr(int nRadix = 10, bool bPrint = true)
{
	Assert(0);
	return "";
}
...
template<>
FMSTRRETTYPE GetFmtStr<short>(int nRadix, bool bPrint)
{
	Assert(nRadix == 10);
	return "%hd";
}
template<>
FMSTRRETTYPE GetFmtStr<ushort>(int nRadix, bool bPrint)
{
	Assert(nRadix == 10);
	return "%hu";
}
template<>
FMSTRRETTYPE GetFmtStr<int>(int nRadix, bool bPrint)
{
	Assert(nRadix == 10);
	return "%d";
}
template<>
FMSTRRETTYPE GetFmtStr<uint>(int nRadix, bool bPrint)
{
	Assert(nRadix == 10 || nRadix == 16);
	return nRadix == 16 ? "%x" : "%u";
}
template<>
FMSTRRETTYPE GetFmtStr<int64>(int nRadix, bool bPrint)
{
	Assert(nRadix == 10);
	return "%lld";
}
template<>
FMSTRRETTYPE GetFmtStr<float>(int nRadix, bool bPrint)
{
	Assert(nRadix == 10);
	return "%f";
}
template<>
FMSTRRETTYPE GetFmtStr<double>(int nRadix, bool bPrint)
{
	Assert(nRadix == 10);
	return bPrint ? "%.15lf" : "%lf";
} // force Printf to print DBL_DIG=15 digits of precision for doubles - defaults to FLT_DIG=6
...
class CUtlBuffer
{
	// Brian has on his todo list to revisit this as there are issues in some cases with CUtlVector using operator =
	// instead of copy construtor in InsertMultiple, etc. The unsafe case is something like this:
...
public:
	...
	// This one isn't actually defined so that we catch contructors that are trying to pass a bool in as the third
	// param.
...
{
	void *p;
	// LEGACY WARNING: in text mode, PutPtr writes 32 bit pointers in hex, while GetPtr reads 32 or 64 bit pointers in
	// decimal
#ifndef X64BITS
