...
void CopyMemory3D(void *pDestAdr, void const *pSrcAdr, int nNumCols, int nNumRows, int nNumSlices, // dimensions of copy
...
// there are some users of these via tier1 templates in used in tier0. but tier0 can't depend on vstdlib which means in
// tier0 we always need the inlined ones
...
// To avoid cross-prx calls, making the V_* fucntions that don't do anything but debug checks and call through to the
// non V_* function go ahead and call the non-V_* functions directly.
...
#ifndef COMPILER_PS3
inline char *V_strupr(char *start)
{
	return strupr(start);
}
inline char *V_strlower(char *start)
{
	return strlwr(start);
}
...
{
// The standard white-space characters are the following: space, tab, carriage-return, newline, vertical tab, and
// form-feed. In the C locale, V_isspace() returns true only for the standard white-space characters.
...
#else
	// this is 11% faster on Core i7 than the previous, VC2005 compiler generates a seemingly unbalanced search tree
	// that's faster
	switch(c)
	{
		case ' ':
...
void V_SplitString2(const char *pString, const char **pSeparators, int nSeparators,
					CUtlVector<char *, CUtlMemory<char *, int>> &outStrings);
...
{
	// Functions used by Asian language line wrapping to determine if a character can end a line, begin a line, or be
	// broken up when repeated (eg: "...")
	bool CanEndLine(wchar_t wcCandidate);
	bool CanBeginLine(wchar_t wcCandidate);
	bool CanBreakRepeated(wchar_t wcCandidate);

	// Used to determine if we can break a line between the first two characters passed; calls the above functions on
	// each character
