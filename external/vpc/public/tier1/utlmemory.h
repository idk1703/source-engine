...
#define UTLMEMORY_TRACK_ALLOC()                                                                \
	MemAlloc_RegisterAllocation("||Sum of all UtlMemory||", 0, m_nAllocationCount * sizeof(T), \
								m_nAllocationCount * sizeof(T), 0)
#define UTLMEMORY_TRACK_FREE()                                                                       \
	if(!m_pMemory)                                                                                   \
		;                                                                                            \
	else                                                                                             \
		MemAlloc_RegisterDeallocation("||Sum of all UtlMemory||", 0, m_nAllocationCount * sizeof(T), \
									  m_nAllocationCount * sizeof(T), 0)
#else
...
void CUtlMemoryAligned<T, nAlignment>::Grow(int num)
{
	...
	CUtlMemory<T>::m_nAllocationCount = UtlMemory_CalcNewAllocationCount(
		CUtlMemory<T>::m_nAllocationCount, CUtlMemory<T>::m_nGrowSize, nAllocationRequested, sizeof(T));
	...
	if(CUtlMemory<T>::m_pMemory)
	{
		...
		CUtlMemory<T>::m_pMemory = (T *)MemAlloc_ReallocAligned(
			CUtlMemory<T>::m_pMemory, CUtlMemory<T>::m_nAllocationCount * sizeof(T), nAlignment);
	...
	else
	{
		...
		CUtlMemory<T>::m_pMemory =
			(T *)MemAlloc_AllocAligned(CUtlMemory<T>::m_nAllocationCount * sizeof(T), nAlignment);
		Assert(CUtlMemory<T>::m_pMemory);
	}
}
