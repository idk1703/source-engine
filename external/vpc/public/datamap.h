...
#define _FIELD(name, fieldtype, count, flags, mapname, tolerance)                                    \
	{                                                                                                \
		fieldtype, #name, offsetof(classNameTypedef, name), count, flags, mapname, NULL, NULL, NULL, \
			sizeof(((classNameTypedef *)0)->name), NULL, 0, tolerance                                \
	}
...
#define DEFINE_CUSTOM_FIELD(name, datafuncs)                                                            \
	{                                                                                                   \
		FIELD_CUSTOM, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE, NULL, datafuncs, NULL \
	}
#define DEFINE_CUSTOM_KEYFIELD(name, datafuncs, mapname)                                                              \
	{                                                                                                                 \
		FIELD_CUSTOM, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE | FTYPEDESC_KEY, mapname, datafuncs, \
			NULL                                                                                                      \
	}
...
#define DEFINE_BITFIELD(name, fieldtype, bitcount) \
	DEFINE_ARRAY(name, fieldtype,                  \
				 ((bitcount + FIELD_BITS(fieldtype) - 1) & ~(FIELD_BITS(fieldtype) - 1)) / FIELD_BITS(fieldtype))
#define DEFINE_INDEX(name, fieldtype) _FIELD(name, fieldtype, 1, FTYPEDESC_INDEX, NULL, 0)

#define DEFINE_EMBEDDED(name)                                                                                \
	{                                                                                                        \
		FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE, NULL, NULL, NULL,        \
			&(((classNameTypedef *)0)->name.m_DataMap), sizeof(((classNameTypedef *)0)->name), NULL, 0, 0.0f \
	}

#define DEFINE_EMBEDDED_OVERRIDE(name, overridetype)                                                  \
	{                                                                                                 \
		FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE, NULL, NULL, NULL, \
			&((overridetype *)0)->m_DataMap, sizeof(((classNameTypedef *)0)->name), NULL, 0, 0.0f     \
	}

#define DEFINE_EMBEDDEDBYREF(name)                                                                                    \
	{                                                                                                                 \
		FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE | FTYPEDESC_PTR, NULL, NULL, NULL, \
			&(((classNameTypedef *)0)->name->m_DataMap), sizeof(*(((classNameTypedef *)0)->name)), NULL, 0, 0.0f      \
	}

#define DEFINE_EMBEDDED_ARRAY(name, count)                                                                       \
	{                                                                                                            \
		FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), count, FTYPEDESC_SAVE, NULL, NULL, NULL,        \
			&(((classNameTypedef *)0)->name->m_DataMap), sizeof(((classNameTypedef *)0)->name[0]), NULL, 0, 0.0f \
	}

#define DEFINE_EMBEDDED_AUTO_ARRAY(name)                                                                       \
	{                                                                                                          \
		FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), SIZE_OF_ARRAY(((classNameTypedef *)0)->name), \
			FTYPEDESC_SAVE, NULL, NULL, NULL, &(((classNameTypedef *)0)->name->m_DataMap),                     \
...
#define DEFINE_PRED_TYPEDESCRIPTION(name, fieldtype)                                                                  \
	{                                                                                                                 \
		FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE | FTYPEDESC_KEY, NULL, NULL, NULL, \
			&fieldtype::m_PredMap                                                                                     \
	}

#define DEFINE_PRED_TYPEDESCRIPTION_PTR(name, fieldtype)                                                            \
	{                                                                                                               \
		FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE | FTYPEDESC_PTR | FTYPEDESC_KEY, \
			NULL, NULL, NULL, &fieldtype::m_PredMap                                                                 \
	}
...
#define DEFINE_INPUT(name, fieldtype, inputname)                                                                 \
	{                                                                                                            \
		fieldtype, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_INPUT | FTYPEDESC_SAVE | FTYPEDESC_KEY, \
			inputname, NULL, NULL, NULL, sizeof(((classNameTypedef *)0)->name)                                   \
	}
...
#define DEFINE_OUTPUT(name, outputname)                                                                              \
	{                                                                                                                \
		FIELD_CUSTOM, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_OUTPUT | FTYPEDESC_SAVE | FTYPEDESC_KEY, \
			outputname, eventFuncs                                                                                   \
	}

// replaces EXPORT table for portability and non-DLL based systems (xbox)
#define DEFINE_FUNCTION_RAW(function, func_type)                                                      \
	{                                                                                                 \
		FIELD_VOID, nameHolder.GenerateName(#function), NULL, 1, FTYPEDESC_FUNCTIONTABLE, NULL, NULL, \
...
#define FTYPEDESC_OVERRIDE \
	0x0080 // The field is an override for one in a base class (only used by prediction system for now)
...
#define FTYPEDESC_PRIVATE \
	0x0200 // The field is local to the client or server only (not referenced by prediction code and not replicated by
		   // networking)
#define FTYPEDESC_NOERRORCHECK \
	0x0400 // The field is part of the prediction typedescription, but doesn't get compared when checking for errors
...
#define TD_MSECTOLERANCE \
	0.001f // This is a FIELD_FLOAT and should only be checked to be within 0.001 of the networked info
...
#define BEGIN_DATADESC_GUTS(className)                                                                            \
	...
	{                                                                                                             \
		datamap_t *g_DataMapHolder =                                                                              \
			DataMapInit<className>((className *)NULL); /* This can/will be used for some clean up duties later */ \
	}                                                                                                             \
                                                                                                                  \
	template<>                                                                                                    \
	datamap_t *DataMapInit<className>(className *)                                                                \
...
#define BEGIN_DATADESC_GUTS_NAMESPACE(className, nameSpace)                                                       \
	...
	{                                                                                                             \
		datamap_t *g_DataMapHolder =                                                                              \
			nameSpace::DataMapInit((className *)NULL); /* This can/will be used for some clean up duties later */ \
	}                                                                                                             \
                                                                                                                  \
	template<>                                                                                                    \
	datamap_t *nameSpace::DataMapInit<className>(className *)                                                     \
