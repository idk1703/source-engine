...
static volatile bool bSpewAllocations =
	false; // TODO: Register CMemoryStacks with g_pMemAlloc, so it can spew a summary
...
bool CMemoryStack::Init(const char *pszAllocOwner, unsigned maxSize, unsigned commitSize, unsigned initialCommit,
						unsigned alignment)
{
	Assert(!m_pBase);
...
#ifdef _GAMECONSOLE
bool CMemoryStack::InitPhysical(const char *pszAllocOwner, uint size, uint nBaseAddrAlignment, uint alignment,
								uint32 nFlags)
{
	m_bPhysical = true;
...
void CMemoryStack::RegisterAllocation()
{
	...
	if(GetSize())
	{
		...
		// NOTE: we deliberately don't use MemAlloc_RegisterExternalAllocation. CMemoryStack needs to bypass
		// 'GetActualDbgInfo' due to the way it allocates memory: there's just one representative memory address
		// (m_pBase), it grows at unpredictable times (in CommitTo, not every Alloc call) and it is freed en-masse
		// (instead of freeing each individual allocation).
...
// Init, shutdown
//-----------------------------------------------------------------------------
bool CPhysicalMemoryStack::Init(size_t nChunkSizeInBytes, size_t nAlignment, int nInitialChunkCount,
								uint32 nAdditionalFlags)
{
	...
	MemAlloc_RegisterExternalAllocation(CPhysicalMemoryStack, m_InitialChunk.m_pBase,
