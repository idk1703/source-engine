...
#error "define vsnwprintf type."
	...
	if((len < 0) || (maxLenInNumWideCharacters > 0 && len >= maxLenInNumWideCharacters))
	{
...
bool V_RemoveDotSlashes(char *pFilename, char separator)
{
	...
	*pOut = 0;
	...
	while(*pIn)
	{
		...
		if(pIn[0] == '.' && PATHSEPARATOR(pIn[1]) && (pIn == pFilename || pIn[-1] != '.'))
		{
			pIn += 2;
		}
		else
...
bool V_MakeRelativePath(const char *pFullPath, const char *pDirectory, char *pRelativePath, int nBufLen)
{
	...
	while(*pPath &&
		  (tolower(*pPath) == tolower(*pDir) || (PATHSEPARATOR(*pPath) && (PATHSEPARATOR(*pDir) || (*pDir == 0)))))
	{
...
static const wchar_t wszCantBeginLine[] = {
	0x0020, 0x0021, 0x0025, 0x0029, 0x002c, 0x002e, 0x003a, 0x003b, 0x003e, 0x003f, 0x005d, 0x007d, 0x00a2, 0x00a8,
	0x00b0, 0x00b7, 0x00bb, 0x02c7, 0x02c9, 0x2010, 0x2013, 0x2014, 0x2015, 0x2016, 0x2019, 0x201d, 0x201e, 0x201f,
	0x2020, 0x2021, 0x2022, 0x2025, 0x2026, 0x2027, 0x203a, 0x203c, 0x2047, 0x2048, 0x2049, 0x2103, 0x2236, 0x2574,
	0x3001, 0x3002, 0x3003, 0x3005, 0x3006, 0x3009, 0x300b, 0x300d, 0x300f, 0x3011, 0x3015, 0x3017, 0x3019, 0x301b,
	0x301c, 0x301e, 0x301f, 0x303b, 0x3041, 0x3043, 0x3045, 0x3047, 0x3049, 0x3063, 0x3083, 0x3085, 0x3087, 0x308e,
	0x3095, 0x3096, 0x30a0, 0x30a1, 0x30a3, 0x30a5, 0x30a7, 0x30a9, 0x30c3, 0x30e3, 0x30e5, 0x30e7, 0x30ee, 0x30f5,
	0x30f6, 0x30fb, 0x30fd, 0x30fe, 0x30fc, 0x31f0, 0x31f1, 0x31f2, 0x31f3, 0x31f4, 0x31f5, 0x31f6, 0x31f7, 0x31f8,
	0x31f9, 0x31fa, 0x31fb, 0x31fc, 0x31fd, 0x31fe, 0x31ff, 0xfe30, 0xfe31, 0xfe32, 0xfe33, 0xfe36, 0xfe38, 0xfe3a,
	0xfe3c, 0xfe3e, 0xfe40, 0xfe42, 0xfe44, 0xfe4f, 0xfe50, 0xfe51, 0xfe52, 0xfe53, 0xfe54, 0xfe55, 0xfe56, 0xfe57,
	0xfe58, 0xfe5a, 0xfe5c, 0xfe5e, 0xff01, 0xff02, 0xff05, 0xff07, 0xff09, 0xff0c, 0xff0e, 0xff1a, 0xff1b, 0xff1f,
	0xff3d, 0xff40, 0xff5c, 0xff5d, 0xff5e, 0xff60, 0xff64};
...
static const wchar_t wszCantEndLine[] = {
	0x0024, 0x0028, 0x002a, 0x003c, 0x005b, 0x005c, 0x007b, 0x00a3, 0x00a5, 0x00ab, 0x00ac, 0x00b7, 0x02c6,
	0x2018, 0x201c, 0x201f, 0x2035, 0x2039, 0x3005, 0x3007, 0x3008, 0x300a, 0x300c, 0x300e, 0x3010, 0x3014,
	0x3016, 0x3018, 0x301a, 0x301d, 0xfe34, 0xfe35, 0xfe37, 0xfe39, 0xfe3b, 0xfe3d, 0xfe3f, 0xfe41, 0xfe43,
	0xfe59, 0xfe5b, 0xfe5d, 0xff04, 0xff08, 0xff0e, 0xff3b, 0xff5b, 0xff5f, 0xffe1, 0xffe5, 0xffe6};
...
bool AsianWordWrap::CanBreakAfter(const wchar_t *wsz)
{
	...
	if((iswascii(first_char) && iswascii(second_char)) // If not both CJK, return early
	   || (iswalnum(first_char) &&
		   iswalnum(second_char))) // both characters are alphanumeric - Don't split a number or a word!
	{
		return false;
	}
