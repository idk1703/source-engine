...
public:
	...
	struct CCommandHashIterator_t
	{
		...
		CCommandHashIterator_t(int _bucket, const CCommandHashHandle_t &_handle) : bucket(_bucket), handle(_handle) {};
	...
	inline ConCommandBase *const &operator[](const CCommandHashIterator_t &iter) const
	{
		return (*this)[iter.handle];
	}
...
protected:
	...
	struct HashEntry_t
	{
		...
		HashEntry_t(unsigned int _hash, ConCommandBase *_cmd) : m_uiKey(_hash), m_Data(_cmd) {};
...
//-----------------------------------------------------------------------------
CConCommandHash::CCommandHashIterator_t CConCommandHash::Next(const CConCommandHash::CCommandHashIterator_t &iter) const
{
