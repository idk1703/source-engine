...
int V_UnicodeToUTF8(const wchar_t *pUnicode, char *pUTF8, int cubDestSizeInBytes)
{
	...
	if(pUnicode && pUTF8)
	{
		...
		int cchResult = -1;
		size_t nLenUnicde =
			(wcslen(pUnicode) + 1) * sizeof(wchar_t); // 4 bytes per wchar vs. 1 byte for utf8 for simple english
...
// The first 128 characters of the Unicode character set (which correspond directly to the ASCII) use a single octet
// with the same binary value as in ASCII. url:http://en.wikipedia.org/wiki/UTF-8
