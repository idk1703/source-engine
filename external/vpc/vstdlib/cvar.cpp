...
#ifdef _DEBUG
	CON_COMMAND_MEMBER_F(CCvar, "ccvar_hash_report", HashReport, "report info on bucket distribution of internal hash.",
						 0)
#endif
};
...
void CCvar::RegisterConCommand(ConCommandBase *variable)
{
	...
	if(pOther)
	{
		...
		else
		{
			...
			if(s_pCVarQuery->AreConVarsLinkable(pChildVar, pParentVar))
			{
				...
				static int const nFlags[] = {FCVAR_CHEAT, FCVAR_REPLICATED, FCVAR_DONTRECORD, FCVAR_ARCHIVE,
											 FCVAR_ARCHIVE_GAMECONSOLE};
				static char const *const szFlags[] = {"FCVAR_CHEAT", "FCVAR_REPLICATED", "FCVAR_DONTRECORD",
				...
				for(int k = 0; k < ARRAYSIZE(nFlags); ++k)
				{
					if((pChildVar->m_nFlags & nFlags[k]) != (pParentVar->m_nFlags & nFlags[k]))
					{
						Warning("Convar %s has conflicting %s flags (child: %s%s, parent: %s%s, parent wins)\n",
								variable->GetName(), szFlags[k], (pChildVar->m_nFlags & nFlags[k]) ? "has " : "no ",
								szFlags[k], (pParentVar->m_nFlags & nFlags[k]) ? "has " : "no ", szFlags[k]);
					}
				}
			}
		}
	...
	AssertMsg1(FindCommandBase(variable->GetName()) == NULL, "Console command %s added twice!", variable->GetName());
...
{
	for(CConCommandHash::CCommandHashIterator_t i = m_CommandHash.First(); m_CommandHash.IsValidIterator(i);
...
void CCvar::Find(const CCommand &args)
{
		...
		i = m_CommandHash.Next(i))
	{
		...
		if(!V_stristr(var->GetName(), search) && !V_stristr(var->GetHelpText(), search))
			continue;
...
CConCommandHash::CCommandHashHandle_t CConCommandHash::Find(const char *name, HashKey_t hashkey) const
{
	...
	for(datapool_t::IndexLocalType_t iElement = m_aBuckets[iBucket]; iElement != m_aDataPool.InvalidIndex();
...
CConCommandHash::CCommandHashHandle_t CConCommandHash::Find(const ConCommandBase *cmd) const
{
		...
		iElement = m_aDataPool.Next(iElement))
	{
		   ...
		   element.m_Data == cmd)		 // and the pointers...
		{
			...
			AssertMsg1(iElement == Find(cmd->GetName()), "ConCommand %s had two entries in the hash!", cmd->GetName());
	...
	AssertMsg1(InvalidHandle() == dbghand, "ConCommand %s couldn't be found by pointer, but was found by name!",
			   cmd->GetName());
#endif
	return InvalidHandle();
#endif
