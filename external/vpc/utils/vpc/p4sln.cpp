...
static const char *FixPerforceFilename(const char *filename)
{
	if(filename && V_strlen(filename) > 2 && !V_strnicmp(filename + 1, ":\\\\", 3))
	{
		...
		V_snprintf(newFilename, sizeof(newFilename), "%c:%s", filename[0], &filename[3]);
...
static void GetChangelistFilenames(CUtlVector<int> &changelists, CUtlVector<CUtlString> &changelistFilenames)
{
	...
	{
		g_pVPC->VPCWarning(
			"P4SLN: Perforce interface not available. Unable to generate solution from the given Perforce changelist.");
		return;
	}
...
static void AddAdditionalDependencies(CUtlVector<CDependency_Project *> &projects,
									  CUtlVector<CDependency_Project *> &allProjects)
{
	for(int nProject = 0; nProject < projects.Count(); nProject++)
	{
		...
		for(int nDependency = 0; nDependency < pCurProject->m_AdditionalProjectDependencies.Count(); nDependency++)
		{
			...
			if(nFound == -1)
			{
				g_pVPC->VPCError("P4SLN: Project %s lists '%s' in its $AdditionalProjectDependencies, but there is no "
...
static void GetProjectsDependingOnFiles(CProjectDependencyGraph &dependencyGraph, CUtlVector<CUtlString> &filenames,
										CUtlVector<CDependency_Project *> &projects)
{
	...
	for(int iFile = 0; iFile < filenames.Count(); iFile++)
	{
		...
		for(int iProject = 0; iProject < dependencyGraph.m_Projects.Count(); iProject++)
		{
			...
			if(pProject->DependsOn(pFile, k_EDependsOnFlagCheckNormalDependencies | k_EDependsOnFlagRecurse |
	...
	// Make sure that each of the dependent projects are members of the restricted groups, otherwise prevent their
	// inclusion.
										   ...
										   IBaseSolutionGenerator *pGenerator, const char *pSolutionFilename)
{
	...
	// Add g_targetProjects, which will include any other projects that they added on the command line with +tier0
	// *engine syntax.
