...
static const char *s_TypeKeyNames[] = {"Library", "ClInclude", "ClCompile", "ResourceCompile", "CustomBuild", "None"};
...
bool CProjectGenerator_Win32_2010::WritePropertyGroupTool(CProjectTool *pProjectTool,
														  CProjectConfiguration *pConfiguration)
{
	...
	for(int i = 0; i < pProjectTool->m_PropertyStates.m_PropertiesInOutputOrder.Count(); i++)
	{
		...
		if(!WriteProperty(&pProjectTool->m_PropertyStates.m_Properties[sortedIndex], true,
...
bool CProjectGenerator_Win32_2010::WriteConfiguration(CProjectConfiguration *pConfig)
{
	if(!pConfig->m_bIsFileConfig)
	{
		...
		m_XMLWriter.PushNode("PropertyGroup",
							 CFmtStr("Condition=\"'$(Configuration)|$(Platform)'=='%s|%s'\" Label=\"Configuration\"",
...
bool CProjectGenerator_Win32_2010::WriteTools(CProjectConfiguration *pConfig)
{
	...
	m_XMLWriter.PushNode("ItemDefinitionGroup", CFmtStr("Condition=\"'$(Configuration)|$(Platform)'=='%s|%s'\"",
														pConfig->m_Name.Get(), pTargetPlatformName));
...
bool CProjectGenerator_Win32_2010::WritePrimaryXML(const char *pOutputFilename)
{
	...
	for(int i = 0; i < configurationNames.Count(); i++)
	{
		m_XMLWriter.PushNode("ProjectConfiguration",
							 CFmtStr("Include=\"%s|%s\"", configurationNames[i].Get(), pTargetPlatformName));
	...
	if(g_pVPC->BUseP4SCC())
	{
		...
		if(!V_MakeRelativePath(g_pVPC->GetStartDirectory(), szCurrentDirectory, szRelativeFilename,
							   sizeof(szRelativeFilename)))
	...
	for(int i = 0; i < configurationNames.Count(); i++)
	{
		m_XMLWriter.PushNode("ImportGroup",
							 CFmtStr("Condition=\"'$(Configuration)|$(Platform)'=='%s|%s'\" Label=\"PropertySheets\"",
									 configurationNames[i].Get(), pTargetPlatformName));
		m_XMLWriter.Write("<Import Project=\"$(UserRootDir)\\Microsoft.Cpp.$(Platform).user.props\" "
	...
	for(int i = 0; i < configurationNames.Count(); i++)
	{
		...
		if(m_pVCProjGenerator->GetRootConfiguration(configurationNames[i].Get(), &pConfiguration))
		{
			for(int j = 0; j < pConfiguration->m_PropertyStates.m_PropertiesInOutputOrder.Count(); j++)
			{
				...
				if(!WriteProperty(&pConfiguration->m_PropertyStates.m_Properties[sortedIndex], true,
								  pConfiguration->m_Name.Get()))
					return false;
			}
...
bool CProjectGenerator_Win32_2010::WriteFileToSecondaryXML(CProjectFile *pFile, const char *pParentPath,
...
bool CProjectGenerator_Win32_2010::WriteFolderContentsToSecondaryXML(CProjectFolder *pFolder, const char *pParentPath,
...
bool CProjectGenerator_Win32_2010::WriteSecondaryXML(const char *pOutputFilename)
{
	...
	m_XMLFilterWriter.PushNode("Project",
...
bool CProjectGenerator_Win32_2010::WriteTool(const char *pToolName, const CProjectTool *pProjectTool,
...
bool CProjectGenerator_Win32_2010::WriteProperty(const PropertyState_t *pPropertyState, bool bEmitConfiguration,
												 ...
												 const char *pOutputValue)
{
	...
	if(bEmitConfiguration)
	{
		...
		conditionString =
			CFmtStr(" Condition=\"'$(Configuration)|$(Platform)'=='%s|%s'\"", pConfigName, pTargetPlatformName);
