...
void CGeneratorDefinition::IterateAttributesKey(ToolProperty_t *pProperty, KeyValues *pAttributesKV)
{
	...
	else if(!V_stricmp(pAttributeName, "ordinals"))
	{
		...
		for(KeyValues *pKV = pAttributesKV->GetFirstSubKey(); pKV; pKV = pKV->GetNextKey())
		{
			...
			if(!pOrdinalValue[0])
			{
				g_pVPC->VPCError("Unknown ordinal value for name '%s' in '%s'", pOrdinalName,
								 pProperty->m_ParseString.Get());
			}
...
void CGeneratorDefinition::AssignIdentifiers()
{
	...
	for(int i = 0; i < m_Tools.Count(); i++)
	{
		...
		for(int j = 0; j < pTool->m_Properties.Count(); j++)
		{
			...
			if(!bFound)
			{
				g_pVPC->VPCError("Could not find PROPERTYNAME( %s, %s ) for %s", prefixString.Get(), pPropertyName,
								 m_ScriptName.Get());
			}
		}
	}
	...
	if(g_pVPC->IsVerbose())
	{
		for(int i = 0; i < usedPropertyNames.Count(); i++)
		{
			...
			{
				g_pVPC->VPCWarning("Unused PROPERTYNAME( %s, %s ) in %s", m_pPropertyNames[i].m_pPrefixName,
								   m_pPropertyNames[i].m_pPropertyName, m_ScriptName.Get());
			}
		}
	}
}
...
ToolProperty_t *CGeneratorDefinition::GetProperty(configKeyword_e keyword, const char *pPropertyName)
{
	for(int i = 0; i < m_Tools.Count(); i++)
	{
		...
		for(int j = 0; j < pTool->m_Properties.Count(); j++)
		{
			...
			if(!pToolProperty->m_LegacyString.IsEmpty() &&
			   !V_stricmp(pToolProperty->m_LegacyString.Get(), pPropertyName))
			{
				// found
				return pToolProperty;
			}
