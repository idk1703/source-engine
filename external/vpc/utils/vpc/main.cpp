...
bool CVPC::Init(int argc, char **argv)
{
	...
	m_bQuiet = (HasCommandLineParameter("/q") || HasCommandLineParameter("/quiet") ||
	...
	// needs to occur early and before any other expensive setup, a crc check just exits with an error code used by
	// caller
	InProcessCRCCheck();
...
bool CVPC::IsProjectCurrent(const char *pOutputFilename, bool bSpewStatus)
{
	...
	bool bCRCValid = VPC_CheckProjectDependencyCRCs(pOutputFilename, m_SupplementalCRCString.Get(), errorString,
													sizeof(errorString));
...
void CVPC::SpewUsage(void)
{
	...
	bool bNoHeader = m_bSpewPlatforms || m_bSpewGames || m_bSpewProjects || m_bSpewGroups || m_bSpewProperties ||
	...
	if(!bNoHeader)
	{
		...
		else
		{
			...
			Log_Msg(LOG_VPC, "    vpc +gamedlls /allgames        <-- Creates ALL the Win32 .vcprojs for client and "
							 "server for all GAMES.\n");
			Log_Msg(LOG_VPC, "    vpc +tools -tier0 /win32 /x360 <-- Creates ALL the Win32 and Xbox360 .vcprojs for "
							 "the tool projects but not the tier0 project.\n");
			...
			Log_Msg(LOG_VPC, "\n--- OPTIONS ---\n");
			Log_Msg(LOG_VPC, "[/q]:          Quiet mode (quiet mode is automatically on if the VPC_QUIET environment "
							 "variable is set)\n");
...
#ifdef VPC_SCC_INTEGRATION
			Log_Msg(LOG_VPC, "[/nosrcctl]:   Disable P4SCC source control integration - can also set environment "
							 "variable VPC_SRCCTL to 0\n");
#else
			Log_Msg(LOG_VPC, "[/srcctl]:     Enable P4SCC source control integration - can also set environment "
							 "variable VPC_SRCCTL to 1\n");
#endif
			...
			//			Log_Msg( LOG_VPC, "               By default if a single game is specified on command line, then
			//that specified\n" ); 			Log_Msg( LOG_VPC, "               game name will be used as a value for $VPCGAME and
			//$VPCGAMECAPS macros.\n" );
			Log_Msg(LOG_VPC,
...
void CVPC::ParseBuildOptions(int argc, char *argv[])
{
	...
	for(int i = 1; i < argc; i++)
	{
		...
		if(!V_stricmp(pArg, "/mksln"))
		{
			...
			// If the next parameter is a standard + or - or / or * parameter, then we take that to be the name of the
			// solution file. So vpc /mksln +engine would generate engine.sln.
			if(argv[i + 1][0] == '+' || argv[i + 1][0] == '-' || argv[i + 1][0] == '/' || argv[i + 1][0] == '*' ||
			   argv[i + 1][0] == '@')
			{
		...
		else if(!V_stricmp(pArg, "/p4sln"))
		{
			...
			++i;
			if(i >= argc || argv[i][0] == '+' || argv[i][0] == '-' || argv[i][0] == '/' || argv[i][0] == '*' ||
			   argv[i][0] == '@')
			{
			...
			{
				VPCError("%s <solution filename> <changelist number> [additional changelist numbers] [ "
						 "[restrict_to_group] ].",
						 pArg);
			}
...
void CVPC::CreateOutputFilename(project_t *pProject, const char *pchPlatform, const char *pchPhase,
								const char *pGameName, const char *pchExtension)
{
							  ...
							  const char *pGameName)
{
	...
	CreateOutputFilename(pProject, pPlatformName, pPhaseName, pGameName,
...
void CVPC::IterateTargetProjects(CUtlVector<projectIndex_t> &projectList, IProjectIterator *pIterator)
{
	...
	for(int nProject = 0; nProject < projectList.Count(); nProject++)
	{
		...
		// each project can have 1 or more scripts that are predicated by game/platform conditionals (i.e. client or
		// server)
		for(int nScript = 0; nScript < pProject->scripts.Count(); nScript++)
		{
			...
			bool bHasGameCondition =
				g_pVPC->ConditionHasDefinedType(pProjectScript->m_condition.String(), CONDITIONAL_GAME);
...
void CVPC::SetMacrosAndConditionals()
{
	...
	for(int i = 0; i < m_Conditionals.Count(); i++)
	{
		if(&m_Conditionals[i] != pPlatformConditional && m_Conditionals[i].type == CONDITIONAL_PLATFORM &&
...
#error "Unsupported platform."
	...
	// DO NOT INTEGRATE OR TAKE THIS - THIS IS TEMP PORTING GLUE.
	{
		...
		if(pRetailConditional && pRetailConditional->m_bDefined &&
		   (!V_stricmp(cVPCPlatform.String(), "X360") || !V_stricmp(cVPCPlatform.String(), "PS3")))
		{
			// CERT is a restricted console RETAIL concept, with publisher dictated rules, there is no CERT process for
	...
	if(m_bEnableVpcGameMacro)
	{
		...
		{
			if(m_Conditionals[iOtherGameDefine].type == CONDITIONAL_GAME && m_Conditionals[iOtherGameDefine].m_bDefined)
			{
		...
		SetMacro("VPCGAMECAPS",
				 (nGameDefineIndex >= 0) ? m_Conditionals[nGameDefineIndex].upperCaseName.Get() : "VALVE", true);
...
{
#ifdef WIN32
	// If they want to generate a solution based on a Perforce changelist, adjust m_targetProjects and set it up like
	// /mksln had been passed in.
	if(m_iP4Changelists.Count() == 0)
		return false;
	...
	{
		V_ComposeFileName(g_pVPC->GetStartDirectory(), m_P4SolutionFilename.Get(), szFullSolutionPath,
...
void CVPC::HandleMKSLN(IBaseSolutionGenerator *pSolutionGenerator)
{
	...
	{
		V_ComposeFileName(g_pVPC->GetStartDirectory(), m_MKSolutionFilename.Get(), szFullSolutionPath,
						  sizeof(szFullSolutionPath));
	}
...
const char *CVPC::BuildTempGroupScript(const char *pVPCScriptName)
{
	...
	V_ComposeFileName(szCurrentDirectory, CFmtStr("%08x%08xvgc.tmp", tmpHash, tmpTime), tempGroupScriptFilename,
					  sizeof(tempGroupScriptFilename));
...
#endif
{
	...
	if(bWindowsTwoPhase)
	{
		...
		for(int i = 0; i < argc; i++)
		{
			...
			{
				// If the next parameter is a standard + or - or / or * parameter, then we take that to be the name of
				// the solution file. So vpc /mksln +engine would generate engine.sln.
				if(argv[i + 1][0] == '+' || argv[i + 1][0] == '-' || argv[i + 1][0] == '/' || argv[i + 1][0] == '*' ||
				   argv[i + 1][0] == '@')
				{
