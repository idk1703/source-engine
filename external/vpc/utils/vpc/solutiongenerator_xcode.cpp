...
static CRelevantPropertyNames g_RelevantPropertyNames = {g_pRelevantProperties, V_ARRAYSIZE(g_pRelevantProperties)};
...
private:
	...
	void EmitBuildSettings(const char *pszProjectName, const char *pszProjectDir,
						   CUtlDict<CFileConfig *, int> *pDictFiles, KeyValues *pConfigKV, KeyValues *pReleaseKV,
						   bool bIsDebug);
	void WriteFilesFolder(uint64_t oid, const char *pFolderName, const char *pExtensions,
...
// Make an oid for a unique string tuple, per type, per ordinal
uint64_t makeoid3(const char *pszIdentifierA, const char *pszIdentifierB, const char *pszIdentifierC, EOIDType type,
				  int16_t ordinal = 0)
{
...
static CUtlString OutputFileWithDirectoryFromConfig(KeyValues *pConfigKV)
{
	...
	{
		g_pVPC->VPCWarning("$OutputDirectory '%s' or $OutputFile directive '%s' contains what looks like a shell "
						   "variable reference -- "
						   "this will be treated literally in most circumstances by the Xcode build system and is "
						   "likely not intended.",
	...
	{
		g_pVPC->VPCWarning(
			"Final output file '%s' (composited from $OutputDirectory '%s' and $OutputFile '%s') escapes the relative "
			"BUILT_PRODUCTS_DIR used by Xcode -- this may break things, and should be achieved by using "
			"$GameOutputFile "
...
static bool NeedsBuildFileEntry(const char *pszFileName)
{
	...
	if(!pchExtension)
		return false;
	else if(!V_stricmp(pchExtension, "cpp") || !V_stricmp(pchExtension, "cxx") || !V_stricmp(pchExtension, "cc") ||
			!V_stricmp(pchExtension, "c") || !V_stricmp(pchExtension, "m") || !V_stricmp(pchExtension, "mm") ||
...
static bool IsSourceFile(const char *pszFileName)
{
	...
	if(!pchExtension)
		return false;
	else if(!V_stricmp(pchExtension, "cpp") || !V_stricmp(pchExtension, "cc") || !V_stricmp(pchExtension, "cxx") ||
			!V_stricmp(pchExtension, "c") || !V_stricmp(pchExtension, "m") || !V_stricmp(pchExtension, "mm") ||
			!V_stricmp(pchExtension, "cc"))
		return true;
	return false;
...
static bool FileBuildsWithCustomBuildRule(const CBaseProjectDataCollector *pProjectDataCollector,
										  CFileConfig *pFileConfig)
{
	if(!pFileConfig || !pProjectDataCollector)
		return false;
	CSpecificConfig *pFileSpecificData =
		pFileConfig->GetOrCreateConfig(pProjectDataCollector->m_BaseConfigData.m_Configurations[1]->GetConfigName(),
...
static bool IsCreatedByCustomBuildStep(const CBaseProjectDataCollector *pProjectDataCollector,
									   CFileConfig *pDynamicFileConfig)
{
	for(int i = pProjectDataCollector->m_Files.First(); i != pProjectDataCollector->m_Files.InvalidIndex();
		i = pProjectDataCollector->m_Files.Next(i))
	{
		...
		V_snprintf(sInputFile.Get(), MAX_PATH, "%s/%s", pProjectDataCollector->m_ProjectName.String(),
				   UsePOSIXSlashes(pDynamicFileConfig->m_Filename.String()));
		V_RemoveDotSlashes(sInputFile.Get());

		CBaseProjectDataCollector::DoStandardVisualStudioReplacements(pFileSpecificData->GetOption(g_pOption_Outputs),
																	  sInputFile, sOutputFiles.Get(), MAX_PATH);
		V_StrSubstInPlace(sOutputFiles.Get(), MAX_PATH, "$(OBJ_DIR)", "${OBJECT_FILE_DIR_normal}", false);

		if(V_stristr(sOutputFiles, V_UnqualifiedFileName(pDynamicFileConfig->GetName())) &&
		   !pFileSpecificData->GetOption(g_pOption_AdditionalDependencies))
		{
			g_pVPC->VPCWarning("Not adding '%s' to the build sources list in project '%s', it's a dyanmic file and "
							   "seems to be created by building '%s'\n",
							   pDynamicFileConfig->GetName(), pProjectDataCollector->m_ProjectName.String(),
							   pFileConfig->GetName());
			return true;
		}
	}
...
void ResolveAdditionalProjectDependencies(CDependency_Project *pCurProject, CUtlVector<CDependency_Project *> &projects,
										  CUtlVector<CDependency_Project *> &additionalProjectDependencies)
{
	for(int i = 0; i < pCurProject->m_AdditionalProjectDependencies.Count(); i++)
	{
		...
		if(j == projects.Count())
			g_pVPC->VPCError(
				"Project %s lists '%s' in its $AdditionalProjectDependencies, but there is no project by that name.",
...
void CSolutionGenerator_Xcode::WriteFilesFolder(uint64_t oid, const char *pFolderName, const char *pExtensions,
												CBaseProjectDataCollector *pProject)
{
	...
	for(int i = files.First(); i != files.InvalidIndex(); i = files.Next(i))
	{
		...
		if(bValidExt)
		{
			Write("%024llX /* %s in %s */,\n", makeoid2(pProject->GetProjectName(), pFileName, EOIDTypeFileReference),
...
void CSolutionGenerator_Xcode::XcodeFileTypeFromFileName(const char *pszFileName, char *pchOutBuf, int cchOutBuf)
{
	...
	if(!pchExtension)
		snprintf(pchOutBuf, cchOutBuf, "compiled.mach-o.executable");
			...
			!V_stricmp(pchExtension, "h") || !V_stricmp(pchExtension, "hxx") || !V_stricmp(pchExtension, "cc"))
...
void CSolutionGenerator_Xcode::EmitBuildSettings(const char *pszProjectName, const char *pszProjectDir,
												 ...
												 KeyValues *pFirstConfigKV, bool bIsDebug)
{
	...
	for(int iConfig = 1; iConfig < V_ARRAYSIZE(k_rgchXCConfigFiles); iConfig++)
	{
		...
		if(sConfigOutputFile != sBuildOutputFile || sConfigGameOutputFile != sGameOutputFile)
		{
			g_pVPC->VPCWarning("Config '%s' for project '%s' has effective output files:\n"
							   ...
							   k_rgchConfigNames[iConfig], pszProjectName, sConfigOutputFile.String(),
							   sConfigGameOutputFile.String(), sBuildOutputFile.String(), sGameOutputFile.String());
		}
	}
	...
	if(sGameOutputFile.Length())
	{
		...
		if(V_strlen(pConfigKV->GetString(g_pOption_ForceInclude, "")))
		{
			...
			CSplitString outStrings(pConfigKV->GetString(g_pOption_ForceInclude), (const char **)g_IncludeSeparators,
	...
	CSplitString outStrings(pConfigKV->GetString(g_pOption_AdditionalIncludeDirectories),
	...
	if(outStrings.Count())
	{
		...
		for(int i = 0; i < outStrings.Count(); i++)
		{
			...
			CBaseProjectDataCollector::DoStandardVisualStudioReplacements(
				outStrings[i], CFmtStr("%s/dummy.txt", pszProjectDir).Access(), sExpandedOutString,
				sizeof(sExpandedOutString));
			V_StrSubstInPlace(sExpandedOutString, sizeof(sExpandedOutString), "$(OBJ_DIR)",
	...
	CSplitString localFrameworks(pConfigKV->GetString(g_pOption_LocalFrameworks), (const char **)g_IncludeSeparators,
								 V_ARRAYSIZE(g_IncludeSeparators));
	if(localFrameworks.Count())
	{
		...
		++m_nIndent;
		{
			...
			for(int i = 0; i < localFrameworks.Count(); i++)
			{
				...
				rgchFrameworkPath[V_strlen(rgchFrameworkPath) - V_strlen(V_UnqualifiedFileName(localFrameworks[i]))] =
					'\0';
	...
	CSplitString preprocessorDefines(pConfigKV->GetString(g_pOption_PreprocessorDefinitions),
									 (const char **)g_IncludeSeparators, V_ARRAYSIZE(g_IncludeSeparators));
	...
	CSplitString additionalLibraryDirectories(pConfigKV->GetString(g_pOption_AdditionalLibraryDirectories),
											  (const char **)g_IncludeSeparators, V_ARRAYSIZE(g_IncludeSeparators));
...
void CSolutionGenerator_Xcode::GenerateSolutionFile(const char *pSolutionFilename,
		...
		Write("objects = {\n");
		{
			...
			**/
			...
			++m_nIndent;
			{
				FOR_EACH_VEC(g_vecPGenerators, iGenerator)
				{
					// poke into the project we're looking @ in the dependency projects vector to figure out it's
					// location on disk
						...
						i = g_vecPGenerators[iGenerator]->m_Files.Next(i))
					{
						...
						V_snprintf(rgchFilePath, sizeof(rgchFilePath), "%s/%s", rgchProjectDir,
						...
						// dynamic files - generated as part of the build - may be automatically added to the build set
						// by xcode, if we add them twice, bad things (duplicate symbols) happen.
						...
						if((!bIsDynamicFile ||
							!IsCreatedByCustomBuildStep(g_vecPGenerators[iGenerator], pFileConfig)) &&
						   AppearsInSourcesBuildPhase(g_vecPGenerators[iGenerator],
													  g_vecPGenerators[iGenerator]->m_Files[i]))
						{
							...
							{
								sCompilerFlags +=
									pFileConfig->m_Configurations[i]->m_pKV->GetString(g_pOption_ExtraCompilerFlags);
							}
							// File reference OIDs are unique per project per file
							Write(
								"%024llX /* %s in Sources */ = {isa = PBXBuildFile; fileRef = %024llX /* %s */; ",
								makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), pFileName, EOIDTypeBuildFile),
								V_UnqualifiedFileName(pFileName),
								makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), pFileName,
										 EOIDTypeFileReference),
								pFileName);
						...
						if(IsDynamicLibrary(pFileName))
						{
							Write("\n");
							Write(
								"%024llX /* %s in Frameworks */ = {isa = PBXBuildFile; fileRef = %024llX /* %s */; };",
								makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), pFileName, EOIDTypeBuildFile),
								V_UnqualifiedFileName(pFileName),
								makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), pFileName,
										 EOIDTypeFileReference),
					...
					CSplitString libs(pKV->GetString(g_pOption_SystemLibraries), (const char **)g_IncludeSeparators,
									  V_ARRAYSIZE(g_IncludeSeparators));
					for(int i = 0; i < libs.Count(); i++)
					{
						Write("\n");
						Write("%024llX /* lib%s.dylib in Frameworks */ = {isa = PBXBuildFile; fileRef = %024llX /* "
							  "lib%s.dylib */; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_SystemLibraries), EOIDTypeBuildFile, i),
							  libs[i],
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_SystemLibraries), EOIDTypeFileReference, i),
							  libs[i]);
					}
					...
					{
						Write("\n");
						Write("%024llX /* %s.framework in Frameworks */ = {isa = PBXBuildFile; fileRef = %024llX /* "
							  "%s.framework */; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_SystemFrameworks), EOIDTypeBuildFile, i),
							  sysFrameworks[i],
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_SystemFrameworks), EOIDTypeFileReference, i),
							  sysFrameworks[i]);
					}
					...
					for(int i = 0; i < localFrameworks.Count(); i++)
					{
						...
						V_StripExtension(V_UnqualifiedFileName(localFrameworks[i]), rgchFrameworkName,
										 sizeof(rgchFrameworkName));

						Write("\n");
						Write("%024llX /* %s.framework in Frameworks */ = {isa = PBXBuildFile; fileRef = %024llX /* "
							  "%s.framework */; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_LocalFrameworks), EOIDTypeBuildFile, i),
							  rgchFrameworkName,
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_LocalFrameworks), EOIDTypeFileReference, i),
							  rgchFrameworkName);
					}
					...
					if(sOutputFile.Length() && (IsStaticLibrary(sOutputFile) || IsDynamicLibrary(sOutputFile)))
					{
						for(int iTestProject = 0; iTestProject < projects.Count(); iTestProject++)
						{
							...
							int dependsOnFlags = k_EDependsOnFlagTraversePastLibs |
												 k_EDependsOnFlagCheckNormalDependencies | k_EDependsOnFlagRecurse;
							...
							{
								Write("\n");
								Write("%024llX /* (lib)%s */ = {isa = PBXBuildFile; fileRef = %024llX /* (lib)%s - "
									  "depended on by %s */; };",
									  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile,
											   EOIDTypeBuildFile, iTestProject),
									  sOutputFile.String(),
									  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile,
											   EOIDTypeFileReference),
					...
					if(sGameOutputFile.Length())
					{
						...
						Write("%024llX /* %s */ = {isa = PBXBuildFile; fileRef = %024llX /* %s */; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sGameOutputFile,
									   EOIDTypeBuildFile, -1),
					...
					if(sOutputFile.Length())
					{
						...
						Write("%024llX /* %s in Products */ = {isa = PBXBuildFile; fileRef = %024llX /* %s */; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile, EOIDTypeBuildFile,
									   -1),
							  sOutputFile.String(),
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile,
									   EOIDTypeFileReference),
			...
			++m_nIndent;
			{
				FOR_EACH_VEC(g_vecPGenerators, iGenerator)
				{
						...
						i = g_vecPGenerators[iGenerator]->m_Files.Next(i))
					{
						...
						CUtlString sCustomBuildCommandLine =
						...
						if(sOutputFiles.Length() && !sCustomBuildCommandLine.IsEmpty())
						{
							...
							Write("%024llX /* PBXbuildRule */ = {\n",
								  makeoid(projects[iGenerator]->m_ProjectName, EOIDTypeCustomBuildRule,
							...
							++m_nIndent;
							{
								...
								// DoStandardVisualStudioReplacements needs to know where the file is, so make sure it's
								// got a path on it
								...
								{
									V_snprintf(sInputFile.Get(), MAX_PATH, "%s/%s", rgchProjectDir,
											   UsePOSIXSlashes(pFileConfig->m_Filename.String()));
									V_RemoveDotSlashes(sInputFile.Get());
								}
								...
								++m_nIndent;
								{
									...
									for(int i = 0; i < outFiles.Count(); i++)
									{
										...
										CBaseProjectDataCollector::DoStandardVisualStudioReplacements(
											outFiles[i], sInputFile, sOutputFile.Get(), MAX_PATH);
										...
										{
											V_snprintf(sOutputPath.Get(), MAX_PATH, "%s/%s", rgchProjectDir,
													   sOutputFile.String());
								...
								Write(");\n");
								CBaseProjectDataCollector::DoStandardVisualStudioReplacements(
									sCustomBuildCommandLine, sInputFile, sCommand.Get(), cCommand);
			...
			**/
			...
			++m_nIndent;
			{
				...
				{
					char rgchFilePath[MAX_PATH];
					V_snprintf(rgchFilePath, sizeof(rgchFilePath), "%s.xcodeproj/../devtools/%s", pSolutionFilename,
							   k_rgchXCConfigFiles[iConfig]);
					V_RemoveDotSlashes(rgchFilePath);

					Write("\n");
					Write("%024llX /* %s */ = {isa = PBXFileReference; fileEncoding = 4; lastKnownFileType = "
						  "text.xcconfig; name = \"%s\"; path = \"%s\"; sourceTree = \"<absolute>\"; };",
						  makeoid2(oidStrSolutionRoot, k_rgchXCConfigFiles[iConfig], EOIDTypeFileReference),
						  k_rgchXCConfigFiles[iConfig], k_rgchXCConfigFiles[iConfig], rgchFilePath);
				}

				FOR_EACH_VEC(g_vecPGenerators, iGenerator)
				{
					// find the project we're looking @ in the dependency projects vector to figure out it's location on
					// disk
					char rgchProjectDir[MAX_PATH];
					rgchProjectDir[0] = '\0';
					V_strncpy(rgchProjectDir, projects[iGenerator]->m_ProjectFilename.String(), sizeof(rgchProjectDir));
						...
						i = g_vecPGenerators[iGenerator]->m_Files.Next(i))
					{
						...
						const char *pFileName =
							V_UnqualifiedFileName(g_vecPGenerators[iGenerator]->m_Files[i]->m_Filename.String());

						char rgchFileType[MAX_PATH];

						// Can't support compiling as different types in different configurations, but that would be
						// insane anyway, right!? Grab the Release settings.
						CSpecificConfig *pFileSpecificData =
						...
						Write("\n");
						Write("%024llX /* %s */ = {isa = PBXFileReference; fileEncoding = 4; explicitFileType = "
							  "\"%s\"; name = \"%s\"; path = \"%s\"; sourceTree = \"<absolute>\"; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   g_vecPGenerators[iGenerator]->m_Files[i]->m_Filename, EOIDTypeFileReference),
					...
					{
						Write("\n");
						Write("%024llX /* lib%s.dylib */ = {isa = PBXFileReference; lastKnownFileType = "
							  "\"compiled.mach-o.dylib\"; name = \"lib%s.dylib\"; path = \"usr/lib/lib%s.dylib\"; "
							  "sourceTree = SDKROOT; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_SystemLibraries), EOIDTypeFileReference, i),
					...
					{
						Write("\n");
						Write("%024llX /* %s.framework */ = {isa = PBXFileReference; lastKnownFileType = "
							  "wrapper.framework; name = \"%s.framework\"; path = "
							  "\"System/Library/Frameworks/%s.framework\"; sourceTree = SDKROOT; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_SystemFrameworks), EOIDTypeFileReference, i),
					...
					for(int i = 0; i < localFrameworks.Count(); i++)
					{
						...
						char rgchFrameworkPath[MAX_PATH];
						V_snprintf(rgchFrameworkPath, sizeof(rgchFrameworkPath), "%s/%s", rgchProjectDir,
								   localFrameworks[i]);
						V_RemoveDotSlashes(rgchFrameworkPath);

						Write("\n");
						Write("%024llX /* %s.framework */ = {isa = PBXFileReference; lastKnownFileType = "
							  "wrapper.framework; name = \"%s.framework\"; path = \"%s\"; sourceTree = \"<absolute>\"; "
							  "};",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
									   pKV->GetString(g_pOption_LocalFrameworks), EOIDTypeFileReference, i),
					...
					if(sOutputFile.Length())
					{
						...
						Write("\n");
						Write("%024llX /* %s */ = {isa = PBXFileReference; explicitFileType = \"%s\"; includeInIndex = "
							  "0; path = \"%s\"; sourceTree = BUILT_PRODUCTS_DIR; };",
							  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile,
									   EOIDTypeFileReference),
			...
			**/
			...
			++m_nIndent;
			{
				FOR_EACH_VEC(g_vecPGenerators, iGenerator)
				{
					...
					static const char *folderExtensions[] = {
						"*.c;*.C;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.prg;*.pas;*.dpr;*.asm;*.s;*.bas;*.java;*.cs;*.sc;*.e;*."
						"cob;*.html;*.tcl;*.py;*.pl;*.m;*.mm",
						"*.h;*.H;*.hh;*.hpp;*.hxx;*.inc;*.sh;*.cpy;*.if", "*.plist;*.strings;*.xib;*.rc;*.proto;*.nut",
						"*.vpc"};

					FOR_EACH_VEC(folderNames, iFolder)
					{
						WriteFilesFolder(
							makeoid(g_vecPGenerators[iGenerator]->m_ProjectName, EOIDTypeGroup, iFolder + 1),
							folderNames[iFolder], folderExtensions[iFolder], g_vecPGenerators[iGenerator]);
					}

					Write("%024llX /* %s */ = {\n", makeoid(g_vecPGenerators[iGenerator]->m_ProjectName, EOIDTypeGroup),
						...
						++m_nIndent;
						{
								...
								i = g_vecPGenerators[iGenerator]->m_Files.Next(i))
							{
								...
								if(IsStaticLibrary(sFileName))
								{
									...
									{
										// don't include static libs generated by other projects - we'll pull them out
										// of the built products tree
										KeyValues *pKV =
											g_vecPGenerators[iGenerator2]->m_BaseConfigData.m_Configurations[0]->m_pKV;
							...
							if(sOutputFile.Length())
								Write("%024llX /* %s */,\n",
									  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile.String(),
											   EOIDTypeFileReference),
									  sOutputFile.String());
						}
						...
						FOR_EACH_VEC(vecSortedProjectNames, iProjectName)
						{
							...
							FOR_EACH_VEC(g_vecPGenerators, iGenerator)
							{
										  ...
										  vecSortedProjectNames[iProjectName]))
								{
									...
									// g_vecPGenerators[iGenerator]->GetProjectName().String(),
									// vecSortedProjectNames[iProjectName].String(),
									// vecSortedProjectNames[iProjectName].String(), iGenerator, iProjectName );
									continue;
								}
			...
			/**
			**
			** the sources build phases - each target that compiles source references on of these, it in turn references
			...
			**/
							...
							i = g_vecPGenerators[iProject]->m_Files.Next(i))
						{
							...
							if(AppearsInSourcesBuildPhase(g_vecPGenerators[iProject], pFileConfig))
							{
								Write("%024llX /* %s in Sources */,\n",
									  makeoid2(g_vecPGenerators[iProject]->GetProjectName(), pFileName,
											   EOIDTypeBuildFile),
			...
			/**
			**
			** the frameworks build phases - each target that links libraries (static, dyamic, framework) has one of
			*these, it references the linked thing
			**
			**/
					...
					++m_nIndent;
					{
							...
							i = g_vecPGenerators[iProject]->m_Files.Next(i))
						{
							   ...
							   IsDynamicLibrary(UsePOSIXSlashes(pFileName)))
							{
								...
								FOR_EACH_VEC(g_vecPGenerators, iGenerator)
								{
									...
									// different projects.
									KeyValues *pKV =
										g_vecPGenerators[iGenerator]->m_BaseConfigData.m_Configurations[0]->m_pKV;
									char szAbsoluteGameOutputFile[MAX_PATH] = {0};
									V_MakeAbsolutePath(szAbsoluteGameOutputFile, sizeof(szAbsoluteGameOutputFile),
													   GameOutputFileFromConfig(pKV).String(),
													   projects[iGenerator]->m_szStoredCurrentDirectory);
						...
						for(int iTestProject = projects.Count() - 1; iTestProject >= 0; --iTestProject)
						{
								   ...
								   additionalProjectDependencies.InvalidIndex())
							{
								...
								KeyValues *pKV =
									g_vecPGenerators[iTestProject]->m_BaseConfigData.m_Configurations[0]->m_pKV;
								CUtlString sOutputFile = OutputFileWithDirectoryFromConfig(pKV);
								if(sOutputFile.Length() &&
								   (IsStaticLibrary(sOutputFile) || IsDynamicLibrary(sOutputFile)))
								{
									// The project in question will have generated a BuildFile dependency for us under
									// its name with ordinal set to our index
									Write("%024llX /* (lib)%s (dependency) */,\n",
										  makeoid2(g_vecPGenerators[iTestProject]->GetProjectName(), sOutputFile,
												   EOIDTypeBuildFile, iProject),
										  sOutputFile.String());
								}
							}
						}
			...
			/**
			**
			** the shell script (pre/post build step) build phases - each target that generates a "gameoutputfile" has
			*one of these,
				...
				FOR_EACH_VEC(g_vecPGenerators, iGenerator)
				{
					...
					if(sPreBuildCommandLine.Length())
					{
						...
						Write("%024llX /* ShellScript */ = {\n",
							  makeoid(projects[iGenerator]->m_ProjectName, EOIDTypePreBuildPhase,
						...
						++m_nIndent;
						{
							...
							Write("inputPaths = (\n);\n");
							Write("name = \"%s\";\n",
								  CFmtStr("PreBuild Event for %s", projects[iGenerator]->m_ProjectName.String())
									  .Access());
							Write("outputPaths = (\n);\n");
							Write("runOnlyForDeploymentPostprocessing = 0;\n");
							...
							CBaseProjectDataCollector::DoStandardVisualStudioReplacements(
								sPreBuildCommandLine, CFmtStr("%s/dummy.txt", rgchProjectDir).Access(), sCommand.Get(),
								cCommand);
							V_StrSubstInPlace(sCommand.Get(), cCommand, "$(OBJ_DIR)", "\"${OBJECT_FILE_DIR_normal}\"",
											  false);
							V_StrSubstInPlace(sCommand.Get(), cCommand, ";", ";\\n", false);
							V_StrSubstInPlace(sCommand.Get(), cCommand, "\"", "\\\"", false);

							// xcode wants to run your custom shell scripts anytime the pbxproj has changed (which makes
							// some sense - the script might be different) - we can't and don't want to early out in
							// this case.
						...
						i = g_vecPGenerators[iGenerator]->m_Files.Next(i))
					{
						...
						// if the project produces a binary, it's a native target and we'll handle this custom build
						// step as a build rule unless the custom build has additional dependencies
						...
						if(sOutputFiles.Length() && !sCustomBuildCommandLine.IsEmpty())
						{
							...
							Write("\n");
							Write("%024llX /* ShellScript */ = {\n",
								  makeoid(projects[iGenerator]->m_ProjectName, EOIDTypeShellScriptBuildPhase,
							...
							++m_nIndent;
							{
								...
								++m_nIndent;
								{
									// DoStandardVisualStudioReplacements needs to know where the file is, so make sure
									// it's got a path on it
									if(V_IsAbsolutePath(UsePOSIXSlashes(pFileConfig->m_Filename.String())))
										V_snprintf(sInputFile.Get(), MAX_PATH, "%s",
												   UsePOSIXSlashes(pFileConfig->m_Filename.String()));
									else
									...
									FOR_EACH_VEC(additionalDeps, i)
									{
										...
										if(*pchOneFile != '\0')
										{
											...
											// DoStandardVisualStudioReplacements needs to know where the file is, so
											// make sure it's got a path on it
											if(V_IsAbsolutePath(UsePOSIXSlashes(pchOneFile)))
												V_snprintf(szDependency, MAX_PATH, "%s", UsePOSIXSlashes(pchOneFile));
											else
											{
												V_snprintf(szDependency, MAX_PATH, "%s/%s", rgchProjectDir,
								...
								{
									int cDescription =
										V_strlen(pFileSpecificData->GetOption(g_pOption_Description)) * 2;
									sDescription.SetLength(cDescription);
									CBaseProjectDataCollector::DoStandardVisualStudioReplacements(
										pFileSpecificData->GetOption(g_pOption_Description), sInputFile,
										sDescription.Get(), cDescription);
								}
								else
									sDescription =
										CFmtStr("Custom Build Step for %s", pFileConfig->m_Filename.String()).Access();
...
#define TELL_XCODE_ABOUT_OUTPUT_FILES 1
								...
								// doesn't change them, which caused us to rebuild a bunch of stuff we didn't need to
								// rebuild but testing with Xcode 6 suggests they fixed that bug, and lying less to the
								// build system is generally good.
								++m_nIndent;
								{
									CSplitString outFiles(sOutputFiles, ";");
									for(int i = 0; i < outFiles.Count(); i++)
								...
								// this is something of a dirty ugly hack.  it seems that xcode wants to run your custom
								// shell scripts anytime the pbxproj has changed (which makes some sense - the script
								// might be different) since we generate one big project, any vpc change means we'll run
								// all the custom build steps again, which will generate code, and link code, and
								// generally take time so if this project was up-to-date (i.e. no vpc changes), add an
								// early out that checks if all the output files are newer than the input files and
								// early out if that's the case
					...
					if(sDebugGameOutputFile.Length() || sReleaseGameOutputFile.Length())
					{
						...
						V_snprintf(rgchDebugFilePath, sizeof(rgchDebugFilePath), "%s/%s", rgchProjectDir,
								   sDebugGameOutputFile.String());
						V_RemoveDotSlashes(rgchDebugFilePath);

						char rgchReleaseFilePath[MAX_PATH];
						V_snprintf(rgchReleaseFilePath, sizeof(rgchReleaseFilePath), "%s/%s", rgchProjectDir,
						...
						++m_nIndent;
						{
							...
							++m_nIndent;
							{
								V_StrSubstInPlace(rgchDebugFilePath, sizeof(rgchDebugFilePath), "/lib/osx32/debug/",
												  "/lib/osx32/${CONFIGURATION}/", false);
								CBaseProjectDataCollector::DoStandardVisualStudioReplacements(
									rgchDebugFilePath, rgchDebugFilePath, rgchDebugFilePath, sizeof(rgchDebugFilePath));
								V_StrSubstInPlace(rgchReleaseFilePath, sizeof(rgchReleaseFilePath),
												  "/lib/osx32/release/", "/lib/osx32/${CONFIGURATION}/", false);
								CBaseProjectDataCollector::DoStandardVisualStudioReplacements(
									rgchReleaseFilePath, rgchReleaseFilePath, rgchReleaseFilePath,
							...
							bool bHasReleasePostBuildCmd = V_strlen(SkipLeadingWhitespace(pReleaseKV->GetString(
															   g_pOption_PostBuildEventCommandLine, ""))) > 0;
							bool bHasDebugPostBuildCmd = V_strlen(SkipLeadingWhitespace(pDebugKV->GetString(
															 g_pOption_PostBuildEventCommandLine, ""))) > 0;
							strScriptExtra.Format(
								...
								"  P4_EDIT_CHANGELIST_CMD=\\\"p4 changes -c $(p4 client -o | grep ^Client | cut -f 2) "
								"-s pending | fgrep 'POSIX Auto Checkout' | cut -d' ' -f 2 | tail -n 1\\\"\\n"
								"  P4_EDIT_CHANGELIST=$(eval \\\"$P4_EDIT_CHANGELIST_CMD\\\")\\n"
								"  if [ -z \\\"$P4_EDIT_CHANGELIST\\\" ]; then\\n"
								"    P4_EDIT_CHANGELIST=$(echo -e \\\"Change: new\\\\nDescription: POSIX Auto "
								"Checkout\\\" | p4 change -i | cut -f 2 -d ' ')\\n"
								"  fi\\n"
								"fi\\n"
								"if [ -f \\\"$OUTPUTFILE\\\" -o -d \\\"$OUTPUTFILE.dSYM\\\" ]; then\\n"
								"  if [ -z \\\"$VALVE_NO_AUTO_P4\\\" ]; then\\n"
								"    p4 edit -c $P4_EDIT_CHANGELIST \\\"$OUTPUTFILE...\\\" | grep -v \\\"also "
								"opened\\\"\\n"
								"  else\\n"
								...
								"  cp -pR \\\"${TARGET_BUILD_DIR}/${FULL_PRODUCT_NAME}.dSYM\\\" "
								"\\\"${OUTPUTFILE}.dSYM\\\"\\n"
								"fi\\n"
								...
								"\";\n",
								rgchReleaseFilePath,
								bHasReleasePostBuildCmd ? UsePOSIXSlashes(pReleaseKV->GetString(
															  g_pOption_PostBuildEventCommandLine, "true"))
														: "true",
								bHasDebugPostBuildCmd
									? UsePOSIXSlashes(pDebugKV->GetString(g_pOption_PostBuildEventCommandLine, "true"))
									: "true");
			...
			**/
			...
			FOR_EACH_VEC(projects, iProject)
			{
				...
				++m_nIndent;
				{
					...
					Write(
						"buildConfigurationList = %024llX /* Build configuration list for PBXNativeTarget \"%s\" */;\n",
						makeoid(projects[iProject]->m_ProjectName, EOIDTypeConfigurationList),
						projects[iProject]->m_ProjectName.String());
					Write("buildPhases = (\n");
					++m_nIndent;
					{
					...
					++m_nIndent;
					{
						// these dependencies point to the dependency objects, which reference other projects through
						// the container item proxy objects
						CDependency_Project *pCurProject = projects[iProject];
						...
						for(int iTestProject = 0; iTestProject < projects.Count(); iTestProject++)
						{
							...
							{
								Write("%024llX /* %s */,\n",
									  makeoid(projects[iProject]->m_ProjectName, EOIDTypeTargetDependency,
					...
					if(sGameOutputFile.Length())
					{
						Write("productReference = %024llX /* %s */;\n",
							  makeoid2(projects[iProject]->m_ProjectName, sGameOutputFile, EOIDTypeFileReference),
							  sGameOutputFile.String());
					}

					char rgchProductType[MAX_PATH];
					XcodeProductTypeFromFileName(V_UnqualifiedFileName(sGameOutputFile), rgchProductType,
			...
			++m_nIndent;
			{
				...
				++m_nIndent;
				{
					...
					++m_nIndent;
					{
						FOR_EACH_VEC(projects, iProject)
						{
							// note the sneaky -1 ordinal here, is we can later generate a dependency block for the
							// target thats not tied to any other targets dependency.
				...
				FOR_EACH_VEC(projects, iProject)
				{
					...
					// NOTE: the use of EOIDTypeNativeTarget here is intentional - a project will never appear as both,
					// and this makes things link up without having to special case in dependencies and aggregate
					// targets NOTE: the driving loop is the number of shell script phases we have - aggregate targets
					// with > 1 shell script phase get broken into N aggregate targets, each with 1 shell script phase
					// so xcode can execute them in parallel
					int cSubAggregateTargets =
						(pGenerator->m_nShellScriptPhases / k_nShellScriptPhasesPerAggregateTarget) +
						(pGenerator->m_nShellScriptPhases % k_nShellScriptPhasesPerAggregateTarget ? 1 : 0);
					for(int i = 0; i < cSubAggregateTargets; i++)
					{
						Write("%024llX /* %s_%d */ = {\n",
							  makeoid(projects[iProject]->m_ProjectName, EOIDTypeNativeTarget, i),
							  projects[iProject]->m_ProjectName.String(), i);
						++m_nIndent;
						{
							Write("isa = PBXAggregateTarget;\n");

							Write("buildConfigurationList = %024llX /* Build configuration list for PBXAggregateTarget "
								  "\"%s\" */;\n",
								  makeoid(projects[iProject]->m_ProjectName, EOIDTypeConfigurationList),
								  projects[iProject]->m_ProjectName.String());
							Write("buildPhases = (\n");
							++m_nIndent;
							{
								for(int j = 0; j < k_nShellScriptPhasesPerAggregateTarget; j++)
									Write("%024llX /* ShellScript %d/%d*/,\n",
										  makeoid(projects[iProject]->m_ProjectName, EOIDTypeShellScriptBuildPhase,
							...
							++m_nIndent;
							{
								// these dependencies point to the dependency objects, which reference other projects
								// through the container item proxy objects
								CDependency_Project *pCurProject = projects[iProject];
								...
								for(int iTestProject = 0; iTestProject < projects.Count(); iTestProject++)
								{
									if(iProject == iTestProject)
									{
										// the "parent" aggregate depends on all the subaggregates, so the vpc
										// dependency structure doesn't need to change.
										if(i == 0)
											for(int j = 1; j < cSubAggregateTargets; j++)
												// the 0-(j+1) is to avoid colliding with the All aggregate dependency
												// at -1
			...
			**/
					...
					FOR_EACH_VEC(vecSortedProjectNames, iProjectName)
					{
						FOR_EACH_VEC(projects, iProject)
						{
							...
							Write("%024llX /* %s */,\n",
								  makeoid(projects[iProject]->m_ProjectName, EOIDTypeNativeTarget),
								  projects[iProject]->m_ProjectName.String());
							// if this is an aggregate target with more than one shell script, emit the "child"
							// aggregates
							if(!ProjectProducesBinary(((CProjectGenerator_Xcode *)g_vecPGenerators[iProject])))
							{
								int cSubAggregateTargets =
									(((CProjectGenerator_Xcode *)g_vecPGenerators[iProject])->m_nShellScriptPhases /
			...
			/**
			**
			** container item proxies (no clue, I just work here...) - they sit between projects when expressing
			*dependencies
			**
			**/
			Write("\n/* Begin PBXContainerItemProxy section */");
			{
				FOR_EACH_VEC(projects, iProject)
				{
					...
					// for each project, figure out what projects it depends on, and spit out a containeritemproxy for
					// that dependency of particular note is that there are many item proxies for a given project, so we
					// make their oids with the ordinal of the project they depend on - this must be consistent within
						...
						if(iProject == iTestProject)
						{
							...
							for(int i = 1; i < cSubAggregateTargets; i++)
							{
								...
								Write("%024llX /* PBXContainerItemProxy (subproject) */ = {\n",
									  makeoid(projects[iProject]->m_ProjectName, EOIDTypeContainerItemProxy,
											  0 - (i + 1)));
								++m_nIndent;
								{
									Write("isa = PBXContainerItemProxy;\n");
									// it looks like if you cross ref between xcodeprojs, this is the oid for the other
									// xcode proj
									Write("containerPortal = %024llX; /* Project object */\n",
										  makeoid(oidStrSolutionRoot, EOIDTypeProject));
									Write("proxyType = 1;\n");
			...
			/**
			**
			** target dependencies - referenced by each project, in turn references the proxy container objects to
			...
			**/
			...
			FOR_EACH_VEC(projects, iProject)
			{
				...
				for(int iTestProject = 0; iTestProject < projects.Count(); iTestProject++)
				{
					...
					{
						for(int i = 1;
							i < ((CProjectGenerator_Xcode *)g_vecPGenerators[iProject])->m_nShellScriptPhases; i++)
						{
							Write("\n");
							Write("%024llX /* PBXTargetDependency */ = {\n",
					...
					CDependency_Project *pTestProject = projects[iTestProject];
					int dependsOnFlags = k_EDependsOnFlagTraversePastLibs | k_EDependsOnFlagCheckNormalDependencies |
										 k_EDependsOnFlagRecurse;
					if(pCurProject->DependsOn(pTestProject, dependsOnFlags) ||
			...
			/**
			**
			** build configurations - each target (and the project) has a set of build configurations (one release, one
			*debug), each with their own set of build settings
			** the "baseConfigurationReference" points back to the appropriate .xcconfig file that gets referenced by
			*the project and has all the non-target specific settings
			**
			**/
			...
			++m_nIndent;
			{
				...
				for(int iConfig = 0; iConfig < V_ARRAYSIZE(k_rgchConfigNames); iConfig++)
				{
					...
					Write("\n");
					Write("%024llX /* %s */ = {\n",
						  makeoid2(oidStrSolutionRoot, k_rgchConfigNames[iConfig], EOIDTypeBuildConfiguration),
					...
					Write("\n");
					Write("%024llX /* %s */ = {\n",
						  makeoid2(oidStrSolutionRoot, k_rgchConfigNames[iConfig], EOIDTypeBuildConfiguration, 1),
						  k_rgchConfigNames[iConfig]);
					++m_nIndent;
					{
						Write("isa = XCBuildConfiguration;\n");
				...
				FOR_EACH_VEC(projects, iProject)
				{
					...
					for(int iConfig = 0; iConfig < V_ARRAYSIZE(k_rgchConfigNames); iConfig++)
					{
						...
						Write("\n");
						Write("%024llX /* %s */ = {\n",
							  makeoid3(oidStrSolutionRoot, projects[iProject]->m_ProjectName,
						...
						++m_nIndent;
						{
							...
							{
								KeyValues *pConfigKV =
									g_vecPGenerators[iProject]->m_BaseConfigData.m_Configurations[iConfig]->m_pKV;
								char rgchProjectDir[MAX_PATH];
								V_strncpy(rgchProjectDir, projects[iProject]->m_ProjectFilename.String(),
										  sizeof(rgchProjectDir));
								V_StripFilename(rgchProjectDir);

								EmitBuildSettings(projects[iProject]->m_ProjectName, rgchProjectDir,
												  &(g_vecPGenerators[iProject]->m_Files), pConfigKV, pReleaseKV,
												  bIsDebug);
							}
							--m_nIndent;
							Write("};\n");
			...
			/**
			**
			** configuration lists - aggregates the build configurations above into sets, which are referenced by the
			*individual targets.
			**
			**/
	...
	// we don't need to be quite as careful as project script, as we're only looking to catch cases where the rest of
	// VPC thinks we're up-to-date
