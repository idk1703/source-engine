...
CSpecificConfig::CSpecificConfig(CSpecificConfig *pParentConfig) : m_pParentConfig(pParentConfig)
{
	m_pKV = new KeyValues("");
	...
	for(int i = 0; i < m_RelevantPropertyNames.m_nNames; i++)
	{
		for(int j = 0; j < V_ARRAYSIZE(s_rgsAmbiguousPropertyNames); j++)
		{
			if(V_stricmp(m_RelevantPropertyNames.m_pNames[i], s_rgsAmbiguousPropertyNames[j]) == 0)
				g_pVPC->VPCWarning("Property name %s may occur in multiple contexts and should be fully qualified",
...
void CBaseProjectDataCollector::GetAllConfigurationNames(CUtlVector<CUtlString> &configurationNames)
{
	...
	for(int i = m_BaseConfigData.m_Configurations.First(); i != m_BaseConfigData.m_Configurations.InvalidIndex();
...
void CBaseProjectDataCollector::StartConfigurationBlock(const char *pConfigName, bool bFileSpecific)
{
	...
	if(index == -1)
	{
		CSpecificConfig *pParent =
			(pFileConfig == &m_BaseConfigData ? NULL : m_BaseConfigData.GetOrCreateConfig(sLowerCaseConfigName, NULL));
...
{
	CFmtStr sQualifiedProperty("%s%s%s",
							   m_CurPropertySection.Count() ? g_pVPC->KeywordToName(m_CurPropertySection.Top()) : "",
	...
	if(pNextToken && pNextToken[0] != 0)
	{
		...
		const char *pBaseString =
			pConfig->m_pKV->GetString(bSetQualifiedProperty ? sQualifiedProperty.Access() : pProperty);
...
void CBaseProjectDataCollector::DoStandardVisualStudioReplacements(const char *pStartString,
																   const char *pFullInputFilename, char *pOut,
																   int outLen)
{
