...
bool CPropertyStates::SetStringProperty(ToolProperty_t *pToolProperty, CProjectTool *pRootTool)
{
	...
	{
		g_pVPC->VPCWarning("%s matches default setting, [%s line:%d]", pToolProperty->m_ParseString.Get(),
...
bool CPropertyStates::SetProperty(ToolProperty_t *pToolProperty, CProjectTool *pRootTool)
{
	...
	switch(pToolProperty->m_nType)
	{
		...
		case PT_DEPRECATED:
			g_pVPC->VPCError("SetProperty: Property %s has been deprecated and is no longer supported!",
							 pToolProperty->m_ParseString.Get());
			break;

		default:
			g_pVPC->VPCError("SetProperty: Unknown type for %s - requires implementation",
							 pToolProperty->m_ParseString.Get());
	}
...
CProjectConfiguration::CProjectConfiguration(CVCProjGenerator *pGenerator, const char *pConfigName,
...
bool CProjectConfiguration::SetProperty(ToolProperty_t *pToolProperty)
{
	...
	// have to mimic what the COM layer used to do which is to configure itself based on the type of application its
	// building VPC enforces a strict order, configuration blocks must come before any tool block to allow this to be
	// rational
	if(bHandled && !V_stricmp(pToolProperty->m_ParseString, "$ConfigurationType"))
	{
		...
		if(pPropertyState && ((V_stristr(pPropertyState->m_OrdinalString.Get(), "static library") ||
...
CRelevantPropertyNames g_RelevantSchemaPropertyNames = {g_pRelevantSchemaProperties,
														V_ARRAYSIZE(g_pRelevantSchemaProperties)};

CVCProjGenerator::CVCProjGenerator() : BaseClass(&g_RelevantSchemaPropertyNames)
{
...
void CVCProjGenerator::SetupGeneratorDefinition(IVCProjWriter *pVCProjWriter, const char *pDefinitionName,
...
#error( "NEEDS TO BE FIXED" )
	...
	g_pVPC->VPCStatus(true, "Saving... Project: '%s' File: '%s'", GetProjectName().String(),
	...
	if(!bValid)
	{
		g_pVPC->VPCError("Cannot save the specified project '%s' to '%s'", GetProjectName().Get(),
						 m_OutputFilename.Get());
	}
...
void CVCProjGenerator::SetGUID(const char *pOutputFilename)
{
	...
	V_snprintf(szGUID, sizeof(szGUID), "{%8.8s-%4.4s-%4.4s-%4.4s-%12.12s}", szMD5, &szMD5[8], &szMD5[12], &szMD5[16],
			   &szMD5[20]);
	m_GUIDString = szGUID;
}
...
bool CVCProjGenerator::StartFile(const char *pFilename, bool bWarnIfAlreadyExists)
{
	...
	// remove any incoming surrounding quotes, this only causes string handling problems (i.e. extension comparison,
	// etc) all files get serialized to xml output with mandatory surrounding quotes
	if(cleanFilename[0] == '\"')
	{
...
void CVCProjGenerator::StartConfigurationBlock(const char *pConfigName, bool bFileSpecific)
{
	...
	if(bFileSpecific)
	{
		...
		if(!bValid)
		{
			...
			if(!bValid)
			{
				g_pVPC->VPCError("File %s, Could not get file configuration '%s'", m_pProjectFile->m_Name.Get(),
								 pConfigName);
			}
		}
...
void CVCProjGenerator::HandleProperty(const char *pPropertyName, const char *pCustomScriptData)
{
	...
	switch(m_nActivePropertySection)
	{
		...
		default:
			g_pVPC->VPCError("HandleProperty: No support for Tool:%s Property:%s - requires implementation",
...
void CVCProjGenerator::AddFileToFolder(const char *pFilename, CProjectFolder *pFolder, bool bWarnIfAlreadyExists,
									   CProjectFile **ppFile)
{
...
void CVCProjGenerator::ApplyInternalPreprocessorDefinitions()
{
	...
	for(int iIndex = m_FileDictionary.FirstInorder(); iIndex != m_FileDictionary.InvalidIndex();
		...
		for(int i = 0; i < pProjectFile->m_Configs.Count(); i++)
		{
			...
			if(pCompilerTool)
			{
				PropertyState_t *pPropertyState =
					pCompilerTool->m_PropertyStates.GetProperty("$PreprocessorDefinitions");
				if(pPropertyState)
				{
					pPropertyState->m_StringValue += extraDefineString;
				}
