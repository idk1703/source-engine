...
public:
	void GetVCPROJSolutionGUID(char (&szSolutionGUID)[256])
	{
		...
		for(int vsVer = firstVer; vsVer <= lastVer; ++vsVer)
		{
			...
			RegStartPoint searchPoints[] = {
				...
				{HKEY_CURRENT_USER,
				 "Software\\Microsoft\\WDExpress\\%d.0_Config\\Projects"}, // WinDev Express -- VS Express starting with
																		   // VS 2013
			};
			for(int j = 0; j < ARRAYSIZE(searchPoints); ++j)
			{
				...
				for(int i = 0; i < 200; i++)
				{
					...
					if(ret == ERROR_SUCCESS)
					{
						...
						ret =
							RegQueryValueEx(hSubKey, "DefaultProjectExtension", NULL, &dwType, (BYTE *)ext, &dwExtLen);
						RegCloseKey(hSubKey);

						// VS 2012 and beyond has the DefaultProjectExtension as vcxproj instead of vcproj
						if(ret == ERROR_SUCCESS && dwType == REG_SZ &&
						   (V_stricmp(ext, "vcproj") == 0 || V_stricmp(ext, "vcxproj") == 0))
						{
	...
	virtual void GenerateSolutionFile(const char *pSolutionFilename, CUtlVector<CDependency_Project *> &projects)
	{
		...
		if(!g_pVPC->Is2010())
		{
			...
			{
				fprintf(fp, "Project(\"{2150E333-8FDC-42A3-9474-1A3956D46DE8}\") = \"Solution Items\", \"Solution "
							"Items\", \"{AAAAAAAA-8B4A-11D0-8D11-90A07D6D6F7D}\"\n");
		...
		for(int i = 0; i < projects.Count(); i++)
		{
			...
			if(!V_MakeRelativePath(pFullProjectFilename, g_pVPC->GetSourcePath(), szRelativeFilename,
								   sizeof(szRelativeFilename)))
				g_pVPC->VPCError("Can't make a relative path (to the base source directory) for %s.",
								 pFullProjectFilename);

			fprintf(fp, "Project(\"%s\") = \"%s\", \"%s\", \"{%s}\"\n", szSolutionGUID,
					pProjInfo->m_ProjectName.String(), szRelativeFilename, pProjInfo->m_ProjectGUID.String());
			...
			for(int iTestProject = 0; iTestProject < projects.Count(); iTestProject++)
			{
				...
				if(pCurProject->DependsOn(pTestProject, k_EDependsOnFlagCheckNormalDependencies |
	...
	void WriteSolutionItems(FILE *fp)
	{
		...
		if(V_IsAbsolutePath(g_pVPC->GetSolutionItemsFilename()))
			...
			V_ComposeFileName(g_pVPC->GetStartDirectory(), g_pVPC->GetSolutionItemsFilename(), szFullSolutionItemsPath,
