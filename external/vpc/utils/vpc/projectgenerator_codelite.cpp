...
void CProjectGenerator_CodeLite::GenerateCodeLiteProject(CBaseProjectDataCollector *pCollector,
														 const char *pOutFilename, const char *pMakefileFilename)
{
	char szProjectFile[MAX_PATH];
	sprintf(szProjectFile, "%s.project", pOutFilename);

	g_pVPC->VPCStatus(true, "Saving CodeLite project for: '%s' File: '%s'", pCollector->GetProjectName().String(),
					  szProjectFile);
	...
	Write("<CodeLite_Project Name=\"%s\" InternalType=\"\">\n", pCollector->GetProjectName().String());
	{
		...
		Write("<Settings Type=\"Dynamic Library\">\n");
		{
			...
			Write("<Configuration Name=\"Debug\" CompilerType=\"gnu g++\" DebuggerType=\"GNU gdb debugger\" "
				  "Type=\"Dynamic Library\" BuildCmpWithGlobalSettings=\"append\" "
				  "BuildLnkWithGlobalSettings=\"append\" BuildResWithGlobalSettings=\"append\">\n");
			{
				++m_nIndent;
		...
		{
			++m_nIndent;
			WriteFilesFolder(k_pchSource, "*.c;*.C;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.prg;*.pas;*.dpr;*.asm;*.s;*.bas;*."
										  "java;*.cs;*.sc;*.e;*.cob;*.html;*.rc;*.tcl;*.py;*.pl;*.m;*.mm");
...
void CProjectGenerator_CodeLite::WriteFilesFolder(const char *pFolderName, const char *pExtensions)
{
	...
	{
		++m_nIndent;
		for(int i = m_pCollector->m_Files.First(); i != m_pCollector->m_Files.InvalidIndex();
