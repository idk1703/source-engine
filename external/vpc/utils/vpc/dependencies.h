...
#define BUILDPROJDEPS_FULL_DEPENDENCY_SET \
	0x01 // This tells it to build a graph of all projects in the source tree _including_ all games.
#define BUILDPROJDEPS_CHECK_ALL_PROJECTS \
	0x02 // If this is set, then it reads all .vpc files.
		 // If this is not set, then it only includes the files from the command line with the "vpc +tier0 *bitmap
		 // +client /tf" syntax
...
public:
	...
	// Returns true if the absolute filename of this thing (CDependency::m_Filename) matches the absolute path
	// specified.
...
public:
	CUtlString m_Filename; // Full paths (slashes are platform dependent).
						   // This is the VPC filename for a project (use CDependency_Project::m_ProjectFilename for the
						   // VCPROJ/VPJ filename).
...
// There can be separate .vcproj files (and thus separate CDependency_Project) for each game and platform of a
// projectIndex_t. If m_Type == k_eDependencyType_Project, then you can cast to this.
...
public:
	...
	CUtlString m_ProjectFilename; // Absolute path to the VCPROJ file (g_pVPC->GetOutputFilename() - see
	...
	// This is used by /p4sln. It uses this to call into VPC_ParseProjectScript. These are the values of
	// g_pVPC->GetOutputFilename(), szScriptName, and the defines at the time of building this project.
...
public:
	...
	void GetProjectDependencyTree(projectIndex_t iProject, CUtlVector<projectIndex_t> &dependentProjects,
								  bool bDownwards);

	// This solves the central mismatch between the way VPC references projects and the way the CDependency stuff does.
	//
	// - VPC uses projectIndex_t, but a single projectIndex_t can turn into multiple games (server_tf, server_episodic,
	// etc) in VPC_IterateTargetProjects.
	// - The dependency code has a separate CDependency_Project for each game.
	//
	// This takes a bunch of project indices (usually m_targetProjects, which comes from the command line's "+this -that
	// *theother" syntax), which are game-agnostic, and based on what games were specified on the command line, it
	// builds the list of CDependency_Project*s.
	void TranslateProjectIndicesToDependencyProjects(CUtlVector<projectIndex_t> &projectList,
...
public:
	...
	CUtlDict<CDependency *, int> m_AllFiles; // All files go in here. They should never be duplicated. These are indexed
											 // by the full filename (except .lib files, which have that stripped off).
