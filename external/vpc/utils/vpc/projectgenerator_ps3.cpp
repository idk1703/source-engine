...
bool CProjectGenerator_PS3::WriteFolder(CProjectFolder *pFolder)
{
	...
	for(int iIndex = pFolder->m_Folders.Head(); iIndex != pFolder->m_Folders.InvalidIndex();
	...
	for(int iIndex = pFolder->m_Files.Head(); iIndex != pFolder->m_Files.InvalidIndex();
	...
	for(int i = 0; i < pPreBuildEventTool->m_PropertyStates.m_Properties.Count(); i++)
	{
		switch(pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_PREBUILDEVENT_CommandLine:
				m_XMLWriter.Write(
					CFmtStrMax("CommandLine=\"%s\"",
							   m_XMLWriter.FixupXMLString(
								   pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;

			case PS3_PREBUILDEVENT_Description:
				m_XMLWriter.Write(CFmtStrMax("Description=\"%s\"",
											 pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
		...
		switch(pCustomBuildTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_CUSTOMBUILDSTEP_CommandLine:
				m_XMLWriter.Write(CFmtStrMax(
					"CommandLine=\"%s\"", m_XMLWriter.FixupXMLString(
											  pCustomBuildTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;
			...
			case PS3_CUSTOMBUILDSTEP_Outputs:
				m_XMLWriter.Write(CFmtStrMax("Outputs=\"%s\"",
											 pCustomBuildTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_CUSTOMBUILDSTEP_AdditionalDependencies:
				m_XMLWriter.Write(CFmtStrMax("AdditionalDependencies=\"%s\"",
											 pCustomBuildTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
		}
	}
	...
	for(int i = 0; i < pCompilerTool->m_PropertyStates.m_Properties.Count(); i++)
	{
		...
		switch(pCompilerTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_SNCCOMPILER_AdditionalIncludeDirectories:
				m_XMLWriter.Write(CFmtStrMax("AdditionalIncludeDirectories=\"%s\"",
											 pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_SNCCOMPILER_PreprocessorDefinitions:
				m_XMLWriter.Write(CFmtStrMax("PreprocessorDefinitions=\"%s\"",
											 pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_SNCCOMPILER_ForceIncludes:
				m_XMLWriter.Write(CFmtStrMax("ForcedIncludeFiles=\"%s\"",
											 pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
			...
			case PS3_SNCCOMPILER_DisableSpecificWarnings:
				m_XMLWriter.Write(CFmtStrMax("DisableSpecificWarnings=\"%s\"",
											 pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_SNCCOMPILER_ObjectFileName:
				m_XMLWriter.Write(CFmtStrMax("ObjectFile=\"%s\"",
											 pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
				...
				if(nOrdinalValue == 1)
				{
					additionalOptions +=
						CFmtStrMax("--create_pch=&quot;%s&quot; ",
								   pCompilerTool->m_PropertyStates.GetProperty(PS3_SNCCOMPILER_PrecompiledHeaderFile)
									   ->m_StringValue);
				}
				else if(nOrdinalValue == 2)
				{
		...
		switch(pCompilerTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_GCCCOMPILER_AdditionalIncludeDirectories:
				m_XMLWriter.Write(CFmtStrMax(
					...
					m_XMLWriter.FixupXMLString(pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;
		...
		switch(pPreLinkEventTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_PRELINKEVENT_CommandLine:
				m_XMLWriter.Write(
					CFmtStrMax("CommandLine=\"%s\"",
							   m_XMLWriter.FixupXMLString(
								   pPreLinkEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;
	...
	for(int i = 0; i < pLinkerTool->m_PropertyStates.m_Properties.Count(); i++)
	{
		...
		switch(pLinkerTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_SNCLINKER_OutputFile:
				m_XMLWriter.Write(
					CFmtStrMax("OutputFile=\"%s\"", pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
			...
			case PS3_SNCLINKER_AdditionalLibraryDirectories:
				m_XMLWriter.Write(CFmtStrMax("AdditionalLibraryDirectories=\"%s\"",
											 pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
			...
			case PS3_SNCLINKER_ForceSymbolReferences:
				m_XMLWriter.Write(CFmtStrMax("ForceSymbolReferences=\"%s\"",
											 pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
			...
			case PS3_SNCLINKER_ImportLibrary:
				m_XMLWriter.Write(CFmtStrMax("ImportLibrary=\"%s\"",
											 pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
			...
			case PS3_SNCLINKER_GenerateMapFile:
				if(nOrdinalValue == 1)
				{
					additionalOptions +=
						CFmtStrMax("-Map=&quot;%s&quot; ",
								   pLinkerTool->m_PropertyStates.GetProperty(PS3_SNCLINKER_MapFileName)->m_StringValue);
				}
				else if(nOrdinalValue == 2)
				{
					additionalOptions +=
			...
			case PS3_SNCLINKER_MapFileName:
				m_XMLWriter.Write(CFmtStrMax("MapFileName=\"%s\"",
											 pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_SNCLINKER_LinkLibraryDependencies:
				m_XMLWriter.Write(CFmtStrMax(
					"LinkLibraryDependencies=\"%s\"",
					BoolStringToTrueFalseString(pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;
		...
		switch(pLinkerTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			...
			case PS3_GCCLINKER_GenerateMapFile:
				if(nOrdinalValue == 1)
				{
					additionalOptions +=
						CFmtStrMax("-Map=&quot;%s&quot; ",
								   pLinkerTool->m_PropertyStates.GetProperty(PS3_GCCLINKER_MapFileName)->m_StringValue);
				}
				break;
	...
	for(int i = 0; i < pLibrarianTool->m_PropertyStates.m_Properties.Count(); i++)
	{
		switch(pLibrarianTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_LIBRARIAN_OutputFile:
				m_XMLWriter.Write(CFmtStrMax("OutputFile=\"%s\"",
											 pLibrarianTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
			...
			case PS3_LIBRARIAN_LinkLibraryDependencies:
				m_XMLWriter.Write(CFmtStrMax(
					"LinkLibraryDependencies=\"%s\"",
					BoolStringToTrueFalseString(pLibrarianTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;
		}
	}
		...
		switch(pPostBuildEventTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_POSTBUILDEVENT_CommandLine:
				m_XMLWriter.Write(
					CFmtStrMax("CommandLine=\"%s\"",
							   m_XMLWriter.FixupXMLString(
								   pPostBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;

			case PS3_POSTBUILDEVENT_Description:
				m_XMLWriter.Write(CFmtStrMax(
					"Description=\"%s\"", pPostBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
	...
	for(int i = 0; i < pConfig->m_PropertyStates.m_Properties.Count(); i++)
	{
		switch(pConfig->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId)
		{
			case PS3_GENERAL_ConfigurationType:
				m_XMLWriter.Write(CFmtStrMax("ConfigurationType=\"%s\"",
			...
			case PS3_GENERAL_ExcludedFromBuild:
				m_XMLWriter.Write(CFmtStrMax(
					"ExcludedFromBuild=\"%s\"",
					BoolStringToTrueFalseString(pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
				break;

			case PS3_GENERAL_OutputDirectory:
				m_XMLWriter.Write(CFmtStrMax("OutputDirectory=\"%s\"",
											 pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_GENERAL_IntermediateDirectory:
				m_XMLWriter.Write(CFmtStrMax("IntermediateDirectory=\"%s\"",
											 pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_GENERAL_ExtensionsToDeleteOnClean:
				m_XMLWriter.Write(CFmtStrMax("DeleteExtensionsOnClean=\"%s\"",
											 pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;

			case PS3_GENERAL_BuildLogFile:
				m_XMLWriter.Write(
					CFmtStrMax("BuildLogFile=\"%s\"", pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
				break;
...
bool CProjectGenerator_PS3::WriteToXML()
{
	...
	for(int iIndex = pRootFolder->m_Folders.Head(); iIndex != pRootFolder->m_Folders.InvalidIndex();
	...
	for(int iIndex = pRootFolder->m_Files.Head(); iIndex != pRootFolder->m_Files.InvalidIndex();
