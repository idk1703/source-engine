...
static inline bool CheckExtension(const char *pFilename, const char *pExt)
{
	...
	return (nFilenameLen > nExtensionLen && pFilename[nFilenameLen - nExtensionLen - 1] == '.' &&
...
public:
	void Setup(CUtlDict<CFileConfig *, int> &files, CFileConfig *pBaseConfig)
	{
		for(int i = files.First(); i != files.InvalidIndex(); i = files.Next(i))
		{
			...
			for(int iSpecific = pFile->m_Configurations.First(); iSpecific != pFile->m_Configurations.InvalidIndex();
				iSpecific = pFile->m_Configurations.Next(iSpecific))
			{
				...
				if(pPrecompiledHeaderOption && V_stristr(pPrecompiledHeaderOption, "Create"))
				{
					...
					{
						g_pVPC->VPCError("File %s creates a precompiled header in config %s but no UsePCHThroughFile "
										 "option specified.",
					...
					{
						g_pVPC->VPCError("File %s has UsePCHThroughFile of %s but another file already does.",
...
public:
	...
	virtual void EndProject()
	{
		...
		{
			g_pVPC->VPCStatus(true, "Saving makefile project for: '%s' File: '%s'", strProjectName.String(),
	...
	void WriteNonConfigSpecificStuff(FILE *fp)
	{
		...
		// forestw: if PLATFORM macro exists we should use its value, this accommodates overrides of PLATFORM in .vpc
		// files
								  ...
								  CSpecificConfig *pConfig1)
	{
		...
		// If we've got a pConfig1, then that means pConfig0 == pConfig1, except for $PreprocessorDefinitions. So don't
		// special case anything other than that one section.
		if(!pConfig1)
		{
		...
		// If that does become a problem, we can put some token around the pathnames we need to be fixed up and leave
		// the rest alone.
		...
		// system libraries
		{
			   ...
			   !V_stricmp(g_pVPC->GetTargetPlatformName(), "OSX64"))
			{
				...
				CSplitString systemFrameworks(pKV->GetString(g_pOption_SystemFrameworks),
		...
		// ForceIncludes
		{
			CSplitString outStrings(pKV->GetString(g_pOption_ForceInclude), (const char **)g_IncludeSeparators,
									V_ARRAYSIZE(g_IncludeSeparators));
		...
		else
		{
			...
			CSplitString outStrings0(pKV->GetString(g_pOption_PreprocessorDefinitions),
									 (const char **)g_IncludeSeparators, V_ARRAYSIZE(g_IncludeSeparators));
			fprintf(fp, "DEFINES += ");
			...
			CSplitString outStrings1(pConfig1->m_pKV->GetString(g_pOption_PreprocessorDefinitions),
									 (const char **)g_IncludeSeparators, V_ARRAYSIZE(g_IncludeSeparators));
			fprintf(fp, "DEFINES += ");
		...
		// INCLUDEDIRS
		{
			CSplitString outStrings(pKV->GetString(g_pOption_AdditionalIncludeDirectories),
									(const char **)g_IncludeSeparators, V_ARRAYSIZE(g_IncludeSeparators));
		...
		DoStandardVisualStudioReplacements(szFixedOutputFile, szAbsPath, sFormattedOutputFile,
		...
		{
			V_strncpy(rgchPostBuildCommand, pKV->GetString(g_pOption_PostBuildEventCommandLine, NULL),
		...
		for(int i = 0; i < OriginalSort.Count(); i++)
		{
			...
			V_strncpy(Filename.szFilename, UsePOSIXSlashes(pFileConfig->m_Filename.String()),
			...
			if(IsLibraryFile(pFilename))
			{
				...
				if((!sImportLibraryFile[0] ||
					V_stricmp(sImportLibraryFile,
							  pFilename)) && // only link this as a library if it isn't our own output!
				   (!sOutputFile[0] || V_stricmp(sOutputFile, pFilename)))
				{
					...
					if(!(!V_stricmp(g_pVPC->GetTargetPlatformName(), "OSX32") ||
						 !V_stricmp(g_pVPC->GetTargetPlatformName(), "OSX64")) &&
					   IsLibraryFile(pFilename) && (pchFileName - 1) && pchFileName[0] == 'l' &&
					   pchFileName[1] == 'i' && pchFileName[2] == 'b' &&
		...
		for(int i = 0; i < ImportLib.Count(); i++)
		{
			fprintf(fp, "    -L%s -l%s \\\n", ImportLib[i].szFilename,
					&ImportLib[i].szFilename[ImportLib[i].iBasename]); // +3 to dodge the lib ext
		}
		...
		for(int i = m_Files.First(); i != m_Files.InvalidIndex(); i = m_Files.Next(i))
		{
			...
			if(pOutputFile && pCustomBuildCommandLine && V_strlen(pCustomBuildCommandLine) > 0)
			{
				...
				DoStandardVisualStudioReplacements(pOutputFile, szAbsPath, sFormattedOutputFile,
												   sizeof(sFormattedOutputFile));

				CSplitString outFiles(sFormattedOutputFile, sDependenciesSeparators,
									  sizeof(sDependenciesSeparators) / sizeof(sDependenciesSeparators[0]));
		...
		// Do this after we output otherDependencies definition since $(OTHER_DEPENDENCIES) is a dependency of all:
		// target
		...
		for(int i = m_Files.First(); i != m_Files.InvalidIndex(); i = m_Files.Next(i))
		{
			...
			if(pOutputFile && pCustomBuildCommandLine && V_strlen(pCustomBuildCommandLine) > 0)
			{
				...
				DoStandardVisualStudioReplacements(pOutputFile, UsePOSIXSlashes(pFilename), sFormattedOutputFile,
												   sizeof(sFormattedOutputFile));
				...
				if(pAdditionalDeps)
					DoStandardVisualStudioReplacements(pAdditionalDeps, szAbsPath, sFormattedDependencies,
													   sizeof(sFormattedDependencies));
				else
					sFormattedDependencies[0] = 0;

				CSplitString additionalDeps(sFormattedDependencies, sDependenciesSeparators,
											sizeof(sDependenciesSeparators) / sizeof(sDependenciesSeparators[0]));
				...
				// ABSPATH NOTE
				//
				// Make will not do what we expect with ../../this/dir/foo.cpp, and these are often the result of nested
				// macros in VPC files, so ensure we are giving make absolute paths for our targets via $(abspath ...)
				...
				/// XXX(JohnS): Was this double-added as an accident, or is there some arcane make reason to have it be
				/// the
				///             first and last dep?
				fprintf(fp, " $(abspath %s)\n", UsePOSIXSlashes(pFilename));
				const char *pDescription = pFileSpecificData->GetOption(g_pOption_Description);
				DoStandardVisualStudioReplacements(pDescription, UsePOSIXSlashes(pFilename), sFormattedOutputFile,
												   sizeof(sFormattedOutputFile));
				...
				if(outFiles.Count() > 1)
				{
					FOR_EACH_VEC(outFiles, i)
					{
						...
						fprintf(fp, "$(abspath %s) : %s %s\n\t @touch %s\n\n", outFiles[i], rgchIntermediateFile,
								g_pVPC->GetOutputFilename(), outFiles[i]);
					}
				}
			}
			else if(CheckExtensions(pFilename, (const char **)sSourceFileExtensions))
			{
				...
				if(!g_pVPC->IsPosixPCHDisabled() && pPrecompiledHeaderOption && pUsePCHThroughFile)
				{
					...
					else if(V_stristr(pPrecompiledHeaderOption, "Create"))
					{
						...
						fprintf(fp, "\n%s.gch : %s $(PWD)/%s %s $(OTHER_DEPENDENCIES)\n", sIncludeFilename,
								pUsePCHThroughFile, g_pVPC->GetOutputFilename(), sMakeFileDependency);
						...
						fprintf(fp, "\n%s : %s %s.gch $(PWD)/%s %s\n", sIncludeFilename, pUsePCHThroughFile,
								sIncludeFilename, g_pVPC->GetOutputFilename(), sMakeFileDependency);
						fprintf(fp, "\tcp -f $< %s\n", sIncludeFilename);
					}
					else if(V_stristr(pPrecompiledHeaderOption, "Use"))
					{
						CFileConfig *pCreator =
							pAccel->FindFileThatCreatesPrecompiledHeader(pConfig->GetConfigName(), pUsePCHThroughFile);
						if(pCreator && !pCreator->IsExcludedFrom(pConfig->GetConfigName()))
						{
							...
							fprintf(fp, "%s : $(abspath %s) %s.gch %s $(PWD)/%s %s\n", sObjFilename,
									UsePOSIXSlashes(pFilename), sIncludeFilename, sIncludeFilename,
				...
				if(!bUsedPrecompiledHeader)
				{
					...
					fprintf(fp, "\n%s : $(abspath %s) $(PWD)/%s %s $(OTHER_DEPENDENCIES)\n", sObjFilename,
							UsePOSIXSlashes(pFilename), g_pVPC->GetOutputFilename(), sMakeFileDependency);
					fprintf(fp, "\t$(PRE_COMPILE_FILE)\n");
					if(pCompileAsOption && strstr(pCompileAsOption, "(/TC)")) // Compile as C code (/TC)
					{
	...
	void WriteMakefile(const char *pFilename)
	{
		...
		else
		{
			...
			for(int i = m_BaseConfigData.m_Configurations.First();
				i != m_BaseConfigData.m_Configurations.InvalidIndex(); i = m_BaseConfigData.m_Configurations.Next(i))
			{
