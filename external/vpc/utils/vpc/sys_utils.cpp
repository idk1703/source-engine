...
CUtlString CXMLWriter::FixupXMLString(const char *pInput)
{
	...
	else
	{
		...
		for(int i = 0; i < ARRAYSIZE(xmlFixups); i++)
		{
			...
			if(!V_StrSubst(bigBuffer[flip], xmlFixups[i].m_pFrom, xmlFixups[i].m_pTo, bigBuffer[flip ^ 1],
						   sizeof(bigBuffer[0]), false))
			{
...
{
	if(!V_strnicmp(pString, "no", 2) || !V_strnicmp(pString, "off", 3) || !V_strnicmp(pString, "false", 5) ||
	   !V_strnicmp(pString, "not set", 7) || !V_strnicmp(pString, "disabled", 8) || !V_strnicmp(pString, "0", 1))
	{
		// false
		return false;
	}
	else if(!V_strnicmp(pString, "yes", 3) || !V_strnicmp(pString, "on", 2) || !V_strnicmp(pString, "true", 4) ||
			!V_strnicmp(pString, "set", 3) || !V_strnicmp(pString, "enabled", 7) || !V_strnicmp(pString, "1", 1))
	{
		// true
		return true;
	}
...
bool Sys_CopyToMirror(const char *pFilename)
{
	...
	{
		g_pVPC->VPCWarning("Cannot mirror '%s', missing expected prefix '%s' in '%s'", pFilename,
