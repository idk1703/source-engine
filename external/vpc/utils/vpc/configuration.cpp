...
void VPC_Config_Keyword(configKeyword_e keyword, const char *pkeywordToken)
{
	...
	else
	{
		...
		while(1)
		{
			...
			// Copy off the token name so HandleProperty() doesn't have to (or else the parser will overwrite it on the
			// next token).
...
void VPC_Keyword_Configuration()
{
	...
	if(!configs.Count())
	{
		...
		{
			g_pVPC->VPCError(
				"Trying to parse a configuration block and no configs have been defined yet.\n[%s line:%d]",
...
void VPC_Keyword_FolderConfiguration(folderConfig_t *pFolderConfig)
{
	...
	//	it would be cool to parse just once, but leaf code in the project generator expects the parser to be in the
	//right position and parses directly.
	while(1)
	{
		g_pVPC->GetScript().SkipToValidToken();
