...
void VPC_Keyword_AddFile(const char *pFileFlag = NULL, const folderConfig_t *pFolderConfig = NULL)
{
	...
	if(g_pVPC->IsShowCaseIssues() && !bDynamicFile)
	{
		for(int i = 0; i < files.Count(); i++)
		{
			...
			{
				g_pVPC->VPCWarning(
					"Case Consistency Issue! File '%s' specified in '%s' is inconsistent with OS version '%s'.",
	...
	for(int i = 0; i < files.Count(); i++)
	{
		...
		if(!g_pVPC->m_bInMkSlnPass && V_stristr(pFilename, "$os"))
		{
			...
			g_pVPC->VPCStatus(false, "$OS: Resolved %s -> %s, rejected %s", pFilename, filename.String(),
							  rgchRejectList);
		...
		if(!g_pVPC->m_sUnityCurrent.IsEmpty() && !V_stricmp(pExtension, "cpp") && !bHasSection && !bHasConditional)
		{
			bool bEmitUnityFiles =
				(!g_pVPC->m_bInMkSlnPass &&
				 (g_pVPC->IsForceGenerate() || !g_pVPC->IsProjectCurrent(g_pVPC->GetOutputFilename(), false)));
			if(bEmitUnityFiles)
			{
				// append to the unity file
				g_pVPC->VPCStatus(false, "Unity: adding '%s' to unity file '%s'", pFilename,
...
static void VPC_HandleLibraryExpansion(char const *pDefaultPath, char const *pFileNamePrefix, char const *pSuffix,
									   bool bRemove)
{
	if(!pFileNamePrefix)
	{
	...
	for(int i = 0; i < impFiles.Count(); i++)
	{
		...
		if(pLastSlash)
		{
			*pLastSlash = 0;
			...
			if(V_strlen(szResolvedFilePrefix) &&
			   !V_strnicmp(pFilenamePart, szResolvedFilePrefix, V_strlen(szResolvedFilePrefix)))
				bAddPrefix = false;
		...
		else
		{
			bool bAddPrefix = true;
			if(V_strlen(szResolvedFilePrefix) &&
			   !V_strnicmp(impFiles[i], szResolvedFilePrefix, V_strlen(szResolvedFilePrefix)))
				bAddPrefix = false;
...
static void VPC_Keyword_ImportLibrary(uint32 Flags)
{
	...
	if((Flags & k_eVPCKeywordFlag_External) && g_pVPC->FindOrCreateMacro("_EXTERNAL_IMPLIB_EXT", false, NULL))
	{
...
static void VPC_Keyword_LinkerLibrary(uint32 Flags)
{
	...
	if((Flags & k_eVPCKeywordFlag_External) && g_pVPC->FindOrCreateMacro("_EXTERNAL_STATICLIB_EXT", false, NULL))
	{
...
void VPC_Keyword_Folder(bool bUnity, const folderConfig_t *pInheritedFolderConfig /* = NULL */)
{
	...
	bool bEmitUnityFiles = (!g_pVPC->m_bInMkSlnPass && (g_pVPC->IsForceGenerate() ||
	...
	if(bUnity)
	{
		...
		{
			V_snprintf(unityName, sizeof(unityName), "%s_%s_unity_%d.cpp", folderName, g_pVPC->GetProjectName(),
					   ++cAttempt);
			V_StrSubstInPlace(unityName, sizeof(unityName), " ", "_", false);
			g_pVPC->m_sUnityCurrent = unityName;
		}
	...
	while(1)
	{
		...
		else if(!V_stricmp(pToken, "$file") || !V_stricmp(pToken, "$schemafile") ||
		...
		//		}
		else if(!V_stricmp(pToken, "$implib") || !V_stricmp(pToken, "$implibexternal") ||
				!V_stricmp(pToken, "-$implib") || !V_stricmp(pToken, "-$implibexternal"))
		{
		...
		else if(!V_stricmp(pToken, "$lib") || !V_stricmp(pToken, "$libexternal") || !V_stricmp(pToken, "-$lib") ||
...
void VPC_Keyword_Shaders(int depth)
{
	...
	if(!fxcList.Count() && !vshList.Count() && !pshList.Count() && !vfxList.Count() && !otherList.Count())
	{
...
void Internal_LoadAddressMacroAuto(bool bPad)
{
	...
	while(1)
	{
		...
		else
		{
			...
			if(dllLength == 0)
			{
				g_pVPC->VPCSyntaxError("$LoadAddressMacroAuto no longer supports 0 size dlls. Use "
									   "$LoadAddressMacroAlias to have two orthogonal projects load in the same space");
			}
...
void VPC_PrepareToReadScript(const char *pInputScriptName, int depth, bool bQuiet, char *&pScriptBuffer,
	...
	if(bDoCRCCheck && g_pVPC->EvaluateConditionalExpression("$WINDOWS"))
	{
		...
		for(int i = 0; i < configurationNames.Count(); i++)
		{
			...
			g_pVPC->GetProjectGenerator()->HandleProperty(
				"$Description", CFmtStr("\"Running VPC CRC Check - %s\"", g_pVPC->GetScript().GetName()));
			g_pVPC->GetProjectGenerator()->HandleProperty(
				"$CommandLine", CFmtStr("\"rem IncrediBuild_AllowOverlap\n%s\necho crc_complete > %s\"",
...
void VPC_HandleIncludeStatement(int depth, bool bQuiet,
								void (*CallbackFn)(const char *pScriptName, int depth, bool bQuiet))
{
...
static const char *fileTypes[] = {"c", "cpp", "cxx", "cc", "h", "hh", "hxx", "hpp", ""};
...
void VPC_Keyword_CustomBuildStep(void)
{
	...
	while(1)
	{
		...
		if(pToken[0] == '[')
		{
			...
			{
				g_pVPC->VPCSyntaxError(
					"Conditional specified on a $CustomBuildStep without any extensions preceding it.");
			}
...
bool CVPC::ParseProjectScript(const char *pScriptName, int depth, bool bQuiet, bool bWriteCRCCheckFile)
{
	...
	if(!depth)
	{
		...
		crcCheckString = "if exist $QUOTE$SRCDIR\\devtools\\bin\\" VPCCRCCHECK_EXE_FILENAME
						 "$QUOTE $QUOTE$SRCDIR\\devtools\\bin\\" VPCCRCCHECK_EXE_FILENAME "$QUOTE -crc2 $QUOTE";
	...
	if(!depth)
	{
		...
		{
			// Finally write out the file with all the CRCs in it. This is referenced by the $CRCCHECK macro in the
			// prebuild steps.
...
void VPC_FakeKeyword_SchemaFolder(CBaseProjectDataCollector *pDataCollector)
{
	...
	g_pVPC->ResolveMacrosInString("$SRCDIR/devtools/bin/schemacompiler.dll", schemaCompilerPath,
								  sizeof(schemaCompilerPath));
	...
	for(int i = 0; i < g_pVPC->m_SchemaFiles.Count(); i++)
	{
		...
		if(IsCFileExtension(szExt))
		{
			...
			V_snprintf(szGenRelPath, sizeof(szGenRelPath), "generated_code_%s_%s/%s_schema_cpp_gen.cpp",
					   g_pVPC->GetProjectName(), pPlatformName, szBase);
		}
		else
		{
			fileInfo.bIsCppFile = false;
			V_snprintf(szGenRelPath, sizeof(szGenRelPath), "generated_code_%s_%s/%s_schema_gen.cpp",
					   g_pVPC->GetProjectName(), pPlatformName, szBase);
		}
	...
	V_snprintf(szSchemaOutAnchorPath, sizeof(szSchemaOutAnchorPath), "generated_code_%s_%s/%s_schema_anchor.cpp",
	...
	g_pVPC->ResolveMacrosInString("$SRCDIR/devtools/bin/schemacompiler.exe", schemaCompilerExePath,
...
KeyValues *ConfigPreprocessorSettingsAsKV(CSpecificConfig *pConfig)
{
	...
	CSplitString outStrings(pInConfigKV->GetString(g_pOption_AdditionalIncludeDirectories),
							(const char **)g_IncludeSeparators, V_ARRAYSIZE(g_IncludeSeparators));
	for(int i = 0; i < outStrings.Count(); i++)
	{
