...
void CVPC::ResolveMacrosInStringInternal(char const *pString, char *pOutBuff, int outBuffSize,
										 bool bStringIsConditional)
{
	...
	do
	{
		...
		for(i = 0; i < m_Macros.Count(); i++)
		{
				...
				for(int j = 0; j < m_Conditionals.Count(); j++)
				{
					if(V_stristr(pFound + 1, m_Conditionals[j].name.String()) == pFound + 1)
					{
						...
						if(V_stricmp(macroName, "$LINUX") && V_stricmp(macroName, "$POSIX"))
							g_pVPC->VPCWarning(
								"Not replacing macro %s with its value (%s) in conditional %s\n", macroName,
								m_Macros[i].value.Length() ? m_Macros[i].value.String() : "null", pFound);
						break;
					}
				}
			}
			...
			if(bDoReplace &&
			   Sys_ReplaceString(buffer1, macroName, m_Macros[i].value.String(), buffer2, sizeof(buffer2)))
			{
				bDone = false;
			}
