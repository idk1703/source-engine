...
static CRelevantPropertyNames g_DependencyRelevantPropertyNames = {g_pDependencyRelevantProperties,
...
static inline bool IsSourceFile(const char *pFilename)
{
	...
	if(pExt && (IsCFileExtension(pExt) || IsHFileExtension(pExt) || !V_stricmp(pExt, "rc") || !V_stricmp(pExt, "inc")))
	{
		return true;
	}
	else
...
CDependency::CDependency(CProjectDependencyGraph *pDependencyGraph) : m_pDependencyGraph(pDependencyGraph)
{
...
bool CDependency::FindDependency_Internal(CUtlVector<CUtlBuffer> &callTreeOutputStack, CDependency *pTest, int flags,
										  int depth)
{
	if(pTest == this)
		return true;
...
public:
	...
	void AddIncludesForFile(CProjectDependencyGraph *pGraph, CDependency *pFile)
	{
		...
		for(int iIncludeFile = 0; iIncludeFile < includes.Count(); iIncludeFile++)
		{
			for(int iIncludeDir = 0; iIncludeDir < includeDirs.Count(); iIncludeDir++)
			{
				...
				V_ComposeFileName(includeDirs[iIncludeDir].String(), includes[iIncludeFile].String(), szFullName,
								  sizeof(szFullName));
	...
	void GetIncludeFiles(const char *pFilename, CUtlVector<CUtlString> &includes)
	{
		...
		if(ret == -1)
		{
			if(g_pVPC->IsVerbose())
			{
				g_pVPC->VPCWarning("GetIncludeFiles( %s ) - can't open file (included by project %s).", pFilename,
								   m_ScriptName.String());
			}
			return;
		}
	...
	void SetupIncludeDirectories(CSpecificConfig *pConfig, const char *szScriptName)
	{
		...
		CSplitString relativeIncludeDirs(pIncludes, (const char **)g_IncludeSeparators,
													   ...
													   CUtlVector<CDependency_Project *> *pPhase1Projects)
{
	...
	if(m_bFullDependencySet)
	{
		...
		{
			Log_Msg(LOG_VPC,
					"\n\nNo vpc.cache file found.\nThis will take a minute to generate dependency info from all the "
...
void CProjectDependencyGraph::ResolveAdditionalProjectDependencies(CUtlVector<CDependency_Project *> *pPhase1Projects)
{
	for(int iMainProject = 0; iMainProject < m_Projects.Count(); iMainProject++)
	{
		...
		for(int i = 0; i < pMainProject->m_AdditionalProjectDependencies.Count(); i++)
		{
			...
			{
				// VPCError( "Project %s lists '%s' in its $AdditionalProjectDependencies, but there is no project by
				// that name.", pMainProject->GetName(), pLookingFor );
				continue;
			}
...
void CProjectDependencyGraph::GetProjectDependencyTree(projectIndex_t iProject,
													   CUtlVector<projectIndex_t> &dependentProjects, bool bDownwards)
{
		...
		for(int iOther = 0; iOther < m_Projects.Count(); iOther++)
		{
			...
			if(bDownwards)
				bThereIsADependency = pProject->DependsOn(
					pOther, k_EDependsOnFlagCheckNormalDependencies | k_EDependsOnFlagCheckAdditionalDependencies |
								k_EDependsOnFlagRecurse | k_EDependsOnFlagTraversePastLibs);
			else
				bThereIsADependency = pOther->DependsOn(
					pProject, k_EDependsOnFlagCheckNormalDependencies | k_EDependsOnFlagCheckAdditionalDependencies |
								  k_EDependsOnFlagRecurse | k_EDependsOnFlagTraversePastLibs);
...
void CProjectDependencyGraph::CheckCacheEntries()
{
	for(int i = m_AllFiles.First(); i != m_AllFiles.InvalidIndex(); i = m_AllFiles.Next(i))
	{
		...
		if(!Sys_FileInfo(pDep->m_Filename.String(), fileSize, modTime) || pDep->m_nCacheFileSize != fileSize ||
...
public:
	virtual bool VisitProject(projectIndex_t iProject, const char *szProjectName)
	{
		...
		if(!bAdded)
		{
			g_pVPC->VPCError("CGameFilterProjectIterator::VisitProject( %s ) - no project found by that name.",
							 szProjectName);
			return false;
		}
...
void CProjectDependencyGraph::TranslateProjectIndicesToDependencyProjects(CUtlVector<projectIndex_t> &projectList,
