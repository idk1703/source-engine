...
static void PerformFileSubstitions(char *line, int linelen)
{
	...
	if(bFindFilePending)
	{
		...
		if(V_stristr(token, "$os"))
		{
			if(!IsValidPathChar(*token))
				fprintf(stderr,
						"Warning: can't expand %s for crc calculation.  Changes to this file set won't trigger "
						"automatic rebuild\n",
						token);
	...
	if(bFindFilePatternPending)
	{
		...
		else
		{
			if(!IsValidPathChar(*token))
				fprintf(stderr,
						"Warning: %s couldn't be expanded during crc calculation.  Changes to this file set won't "
						"trigger automatic project rebuild\n",
						token);
		}
	}
}
...
bool CheckSupplementalString(const char *pSupplementalString, const char *pReferenceSupplementalString)
{
	...
	// The supplemental string contains various options that tell how the project was built. It's generated in
	// VPC_GenerateCRCOptionString.
	//
	// If there's no reference supplemental string (which is the case if we're running vpccrccheck.exe), then we ignore
	// it and continue.
...
bool VPC_CheckProjectDependencyCRCs(const char *pProjectFilename, const char *pReferenceSupplementalString,
									char *pErrorString, int nErrorStringLength)
{
			...
			if(CheckSupplementalString(pSupplementalString, pReferenceSupplementalString))
			{
				...
				while(1)
				{
					...
					*pSpace = 0;
					...
					{
						SafeSnprintf(pErrorString, nErrorStringLength, "Unable to load %s for CRC comparison.",
									 pVPCFilename);
						break;
					}
	...
	{
		SafeSnprintf(pErrorString, nErrorStringLength, "CRC file %s has an invalid version string ('%s')", szFilename,
...
int VPC_CommandLineCRCChecks(int argc, char **argv)
{
	if(argc < 2)
	{
		fprintf(stderr, "Invalid arguments to " VPCCRCCHECK_EXE_FILENAME ". Format: " VPCCRCCHECK_EXE_FILENAME
						" [project filename]\n");
		return 1;
	}
	...
	// If the first argument starts with -crc but is not -crc2, then this is an old CRC check command line with all the
	// CRCs and filenames directly on the command line. The new format puts all that in a separate file.
