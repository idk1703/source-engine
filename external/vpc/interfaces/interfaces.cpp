...
void ConnectInterfaces(CreateInterfaceFn *pFactoryList, int nFactoryCount)
{
	...
	else if(s_nRegistrationCount == 0)
	{
		for(int i = 0; i < nFactoryCount; ++i)
		{
			...
			{
				RegisterInterface(pFactoryList[i], g_pInterfaceGlobals[j].m_pInterfaceName,
	...
	{
		// This is no longer questionable: ConnectInterfaces() is expected to be called multiple times for a file that
		// exports multiple interfaces. Warning("APPSYSTEM: ConnectInterfaces() was called twice for the same DLL.\nThis
		// is expected behavior in building reslists, but questionable otherwise.\n");
		for(int i = 0; i < nFactoryCount; ++i)
		{
			for(int j = 0; j < ARRAYSIZE(g_pInterfaceGlobals); ++j)
			{
				ReconnectInterface(pFactoryList[i], g_pInterfaceGlobals[j].m_pInterfaceName,
								   (void **)g_pInterfaceGlobals[j].m_ppGlobal);
			}
		}
	}
