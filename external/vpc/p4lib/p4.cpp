	...
	void OutputRecordInternal(P4File_t &file, const char *szCmd, const char *szInfo)
	{
		...
		else if(!V_strcmp(szCmd, "otherOpen"))
		{
			file.m_bOpenedByOther = atoi(szInfo) != 0;
				...
				(!V_strcmp(szCmd, "user") || !V_strcmp(szCmd, "client") || !V_strcmp(szCmd, "time") ||
...
public:
	...
	void AddVarToFullClientSpec(const char *variable, const char *data)
	{
		V_snprintf(&m_szFullClientspec[m_iFullClientspecWritePosition],
				   CLIENTSPEC_BUFFER_SIZE - m_iFullClientspecWritePosition, "%s: %s\n\n", variable, data);
	...
	void RemovePathFromClient(const char *path)
	{
		...
		for(int i = 0; i < m_PathMap.Count(); i++)
		{
			...
			if(!V_strnicmp(m_PathMap[i].m_szDepotPath, path, V_strlen(m_PathMap[i].m_szDepotPath) - 4))
			{
				...
				V_strncat(negativeRecord.m_szClientPath, "/", sizeof(negativeRecord.m_szClientPath),
						  COPY_ALL_CHARACTERS);
				V_strncat(negativeRecord.m_szClientPath, pszNegPaths, sizeof(negativeRecord.m_szClientPath),
						  COPY_ALL_CHARACTERS);
...
public:
	void CreateChangelist(const char *pDescription)
	{
		...
		m_pDescription = (pDescription && pDescription[0])
							 ? pDescription
							 : "I'm a loser who didn't type a description. Mock me at your earliest convenience.";
		char *argv[] = {"-i", NULL};
		s_p4.GetClientApi().SetArgv(1, argv);
...
public:
	...
	virtual void HandleError(Error *err)
	{
		...
		if(V_strstr(buf.Text(), "can't edit exclusive file already opened"))
		{
			m_errorSeverity =
				max(m_errorSeverity,
					E_WARN); // s_p4 sends this is an info message, even though the file doesn't get edited!
		}
...
bool CP4::DepotFileToLocalFile(const char *pDepotFile, char *pLocalPath, int nBufLen)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		{
			V_snprintf(pLocalPath, nBufLen, "%s\\%s%s", szLocalRootWithoutSlashes, pTruncatedClientPath + nClientLen,
...
void CP4::SetOpenFileChangeList(const char *pChangeListName)
{
	if(!m_sChangeListName.Length() || !pChangeListName || !*pChangeListName ||
	   strcmp(m_sChangeListName.String(), pChangeListName) || pChangeListName == m_sChangeListName.String())
	{
