...
namespace
{
	...
	public:
		...
		/*virtual*/ DWORD NumberOfProcessCores() const
		{
			...
			{
				if((RelationProcessorCore == m_pSlpi[i].Relationship) && (m_pSlpi[i].ProcessorMask & dwProcessAffinity))
				{
					++nCores;
				}
			}
	...
	private:
		...
		static GlpiFnPtr VerifyGlpiFn_()
		{
			...
			if(pVvi)
			{
				...
				if(pVvi(&osvi, VER_MAJORVERSION | VER_MINORVERSION | VER_SERVICEPACKMAJOR | VER_SERVICEPACKMINOR,
						dwlMask))
				{
	...
	public:
		...
		CpuidImpl() : m_nItems(0)
		{
			...
			else
			{
				...
				for(DWORD_PTR dwThreadAffinity = 1; dwThreadAffinity && dwThreadAffinity <= dwSystemAffinity;
					dwThreadAffinity <<= 1)
				{
		...
		static BOOL IsSupported()
		{
			BOOL bSupported = Cpuid::IsVendor(GenuineIntel) || Cpuid::IsVendor(AuthenticAMD);
