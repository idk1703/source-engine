...
HRESULT PME::Init(void)
{
	...
	if(OS.dwPlatformId == VER_PLATFORM_WIN32_NT)
	{
		hFile = CreateFile( // WINDOWS NT
			"\\\\.\\GDPERF", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	}
	else
	{
...
//
HRESULT PME::SelectP5P6PerformanceEvent(uint32 dw_event, uint32 dw_counter, bool b_user, bool b_kernel)
{
	HRESULT hr = S_OK;
	...
	switch(version.Family)
	{
...

		{
			...
			hr = WriteMSR((dw_counter == 0) ? 0x186 : 0x187,
						  ...
						  uint64((u1_event | (b_user ? (1 << 16) : 0) | (b_kernel ? (1 << 17) : 0) | (1 << 22) |
								  (1 << 18) | (u1_mask << 8))));
		}
		break;
