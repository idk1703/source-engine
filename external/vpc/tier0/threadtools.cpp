...
static bool InternalLookupHandleToThreadIDMap(HANDLE hThread, CThreadHandleToIDMap *&pMap,
											  CThreadHandleToIDMap **&ppPrev)
{
...
ThreadHandle_t *CreateTestThreads(ThreadFunc_t fnThread, int numThreads, int nProcessorsToDistribute)
{
	...
	for(int i = 0; i < numThreads; ++i)
	{
		...
		/*
				...
				if ( int nError = sys_ppu_thread_create( &hThread, ThreadProcConvertUnion, info.val64, 1001,
		   nDefaultStackSize, SYS_PPU_THREAD_CREATE_JOINABLE, "SimpleThread" ) != CELL_OK )
				{
					printf( "PROBLEM!\n" );
...
ThreadHandle_t CreateSimpleThread(ThreadFunc_t pfnThread, void *pParam, unsigned stackSize)
{
	...
	DWORD threadID;
	HANDLE hThread =
		(HANDLE)CreateThread(NULL, stackSize, ThreadProcConvert, new ThreadProcInfo_t(pfnThread, pParam), 0, &threadID);
	...
	const unsigned int nDefaultStackSize = 64 * 1024; // this stack size is used in case stackSize == 0
	if(sys_ppu_thread_create(&th, ThreadProcConvertUnion, info.val64, 1001, stackSize ? stackSize : nDefaultStackSize,
							 SYS_PPU_THREAD_CREATE_JOINABLE, "SimpleThread") != CELL_OK)
	{
...
{
	// <sergiy> Note: if you have a null-access crash here, it may mean that CTHREADEVENT_MAX_WAITING_THREADS is not
	// high enough
...
#pragma optimize("", off)
CThreadFastMutex *g_pIgnoredMutexes[256]; // Ignore noisy non-problem mutex. Probably could be an array. Right now
										  // needed only for sound thread
float g_MutexTimingTolerance = 5;
bool g_bMutexTimingOutput;

void TrapMutexTimings(uint32 probableBlocker, uint32 thisThread, volatile CThreadFastMutex *pMutex,
					  CFastTimer &spikeTimer, CAverageCycleCounter &sleepTimer)
{
	spikeTimer.End();
	if(spikeTimer.GetDuration().GetMillisecondsF() > g_MutexTimingTolerance)
	{
		...
		if(!bIgnore && spikeTimer.GetDuration().GetMillisecondsF() < 100)
		{
			...
			volatile float peak = sleepTimer.GetPeakMilliseconds();
			volatile int xx = 6;
...
// 1ms/frame for other platforms, we include the .inl in the .cpp file where it existed before
