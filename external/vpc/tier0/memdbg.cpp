...
#if defined(SORT_STACK_TRACE_DESCRIPTION_DUMPS) // Assuming StackDescribe is called iteratively on a sorted set of
												// stacks (as in DumpStackStats()). We can save work by skipping
												// unchanged parts at the beginning of the string.
	...
	for(k = 0; k < nMaxAddresses; ++k)
	{
...
#endif
		{
			...
			if(GetFileAndLineFromAddress(ppAddresses[k], szTemp, MAX_PATH, iLine, &iLineDisplacement))
			{
				...
				sprintf(pchBuffer, iLineDisplacement ? "%s:%d+0x%I32X" : "%s:%d", pchFileName, iLine,
						iLineDisplacement);
			}
			else if(GetSymbolNameFromAddress(ppAddresses[k], szTemp, MAX_PATH, &iSymbolDisplacement))
			{
				sprintf(pchBuffer, (iSymbolDisplacement > 0 && !(iSymbolDisplacement >> 63)) ? "%s+0x%llX" : "%s",
...
#endif
{
	...
	byte reserved[16 - (sizeof(unsigned int) *
						2)]; // MS allocator always returns mem aligned on 16 bytes, which some of our code depends on
#else
	byte reserved[16 - sizeof(unsigned int)]; // MS allocator always returns mem aligned on 16 bytes, which some of our
											  // code depends on
#endif
	Sentinal_t sentinal;
};
...
{
	static DbgMemHeader_t **g_pRecentFrees = (DbgMemHeader_t **)
#ifdef _PS3
...
void LMDReportInvalidBlock(DbgMemHeader_t *pHeader, const char *pszMessage)
{
	...
	if(pHeader)
	{
		sprintf(szMsg, "HEAP IS CORRUPT: %s (block 0x%x, %d bytes)\n", pszMessage,
				(size_t)(((byte *)pHeader) + sizeof(DbgMemHeader_t)), pHeader->nLogicalSize);
	}
	else
	{
...
void LMDValidateBlock(DbgMemHeader_t *pHeader, bool bFreeList)
{
	...
	if(memcmp(((Sentinal_t *)((((byte *)pHeader) + sizeof(DbgMemHeader_t) + pHeader->nLogicalSize))), &g_TailSentinel,
			  sizeof(Sentinal_t)) != 0)
	{
...
inline void *InternalRealloc(void *pMem, size_t nNewSize, const char *pFileName, int nLine)
{
...
#ifdef OSX
	pNewAllocedMem = (DbgMemHeader_t *)malloc_zone_realloc(malloc_default_zone(), pMem,
	...
	pNewAllocedMem = (DbgMemHeader_t *)(g_pMemOverrideRawCrtFns->pfn_realloc)(
		*((void **)pInternalMem->m_Reserved2), nNewSize + sizeof(DbgMemHeader_t) + sizeof(Sentinal_t));
	...
	*((Sentinal_t *)(((byte *)pInternalMem) + sizeof(DbgMemHeader_t) + nNewSize)) = g_TailSentinel;
	...
	pInternalMem->m_nLineNumber = nLine;
#else
	pInternalMem = (DbgMemHeader_t *)_realloc_dbg(pInternalMem, nNewSize + sizeof(DbgMemHeader_t), _NORMAL_BLOCK,
												  pFileName, nLine);
#endif
...
inline void *InternalReallocAligned(void *pMem, size_t nNewSize, size_t align, const char *pFileName, int nLine)
{
	...
	DbgMemHeader_t *pInternalMem = GetCrtDbgMemHeader(pMem);
	pNewAllocedMem = (DbgMemHeader_t *)(g_pMemOverrideRawCrtFns->pfn_reallocalign)(
...
private:
	...
	typedef std::map<MemInfoKey_FileLine_t, MemInfo_t, std::less<MemInfoKey_FileLine_t>,
...
private:
	...
	// Updates stats
	virtual void RegisterAllocation(const char *pFileName, int nLine, size_t nLogicalSize, size_t nActualSize,
									unsigned nTime);
	virtual void RegisterDeallocation(const char *pFileName, int nLine, size_t nLogicalSize, size_t nActualSize,
									  unsigned nTime);
#if defined(USE_STACK_TRACES)
...
#if defined(USE_STACK_TRACES)
	typedef CCallStackStatsGatherer<MemInfo_t, STACK_TRACE_LENGTH, GetCallStack_Fast,
...
#define IfDbgInfoIsReady() \
	if(TLSGlobals *IfDbgInfoIsReady_pTlsGlobals = g_pfnElfGetTlsGlobals ? g_pfnElfGetTlsGlobals() : NULL)
#else
...
const char *CDbgMemAlloc::s_pCountHeader[CDbgMemAlloc::NUM_BYTE_COUNT_BUCKETS] = {
	"<=16 byte allocations", "17-32 byte allocations", "33-128 byte allocations", "129-2048 byte allocations",
...
const char *CDbgMemAlloc::FindOrCreateFilename(const char *pFileName)
{
...
#if defined(USE_STACK_TRACES_DETAILED)
	{
		...
		int numStackAddrRetrieved =
			GetCallStack_Fast(arrStackAddresses, 10,
							  2); // Skip this function, and either CDbgMemAlloc::Alloc() or CDbgMemAlloc::Realloc()
...
// Updates stats
//-----------------------------------------------------------------------------
void CDbgMemAlloc::RegisterAllocation(const char *pFileName, int nLine, size_t nLogicalSize, size_t nActualSize,
									  unsigned nTime)
{
	HEAP_LOCK();
...
void CDbgMemAlloc::RegisterDeallocation(const char *pFileName, int nLine, size_t nLogicalSize, size_t nActualSize,
										unsigned nTime)
{
	HEAP_LOCK();
...
void CDbgMemAlloc::RegisterDeallocation(unsigned int nStatIndex, size_t nLogicalSize, size_t nActualSize,
										unsigned nTime)
{
	HEAP_LOCK();
	RegisterDeallocation(m_GlobalInfo, nLogicalSize, nActualSize, nTime);
...
#if defined(USE_STACK_TRACES)
	RegisterAllocation(GetAllocationStatIndex_Internal(pMem), InternalLogicalSize(pMem), InternalMSize(pMem),
					   m_Timer.GetDuration().GetMicroseconds());
#else
	RegisterAllocation(GetAllocatonFileName(pMem), GetAllocatonLineNumber(pMem), InternalLogicalSize(pMem),
					   InternalMSize(pMem), m_Timer.GetDuration().GetMicroseconds());
#endif
...
#else
		RegisterDeallocation(GetAllocatonFileName(pMem), GetAllocatonLineNumber(pMem), InternalLogicalSize(pMem),
							 InternalMSize(pMem), 0);
#endif
	}
...
int CDbgMemAlloc::CrtDbgReport(int nRptType, const char *szFile, int nLine, const char *szModule, const char *pMsg)
{
#ifdef POSIX
	return 0;
#else
...
{
	m_OutputFunc("%s, line %i\t%.1f\t%.1f\t%.1f\t%.1f\t%.1f\t%d\t%d\t%d\t%d\t%d\t%d\t%d", pAllocationName, line,
				 info.m_nCurrentSize / 1024.0f, info.m_nPeakSize / 1024.0f, info.m_nTotalSize / 1024.0f,
				 info.m_nOverheadSize / 1024.0f, info.m_nPeakOverheadSize / 1024.0f, (int)(info.m_nTime / 1000),
				 info.m_nCurrentCount, info.m_nPeakCount, info.m_nTotalCount, info.m_nSumTargetRange,
				 info.m_nCurTargetRange, info.m_nMaxTargetRange);
...
void CDbgMemAlloc::DumpStatsFileBase(char const *pchFileBase)
{
	...
	if(m_OutputFunc == DefaultHeapReportFunc)
	{
		...
		//_snprintf( szFileName, sizeof( szFileName ), "%s%s_%2.2d%2.2d_%2.2d%2.2d%2.2d_%d.txt", pPath,
		//s_szStatsMapName, systemTime.wMonth, systemTime.wDay, systemTime.wHour, systemTime.wMinute,
...

	{
		...
		m_OutputFunc("Allocation type\tCurrent Size(k)\tPeak Size(k)\tTotal Allocations(k)\tOverhead Size(k)\tPeak "
					 "Overhead Size(k)\tTime(ms)\tCurrent Count\tPeak Count\tTotal Count\tTNum\tTCur\tTMax");
...
#ifdef _MEMTEST
		{
			...
			if(7 == sscanf(s_szStatsComment, "%d %d %d %d %d %d %d", &nGPUMemSize, &nGPUMemFree, &nTextureSize,
						   &nRTSize, &nVBSize, &nIBSize, &nUnknown))
			{
...
void CDbgMemAlloc::DumpCallStackFlow(char const *pchFileBase)
{
	...
	SYSTEMTIME systemTime;
	GetLocalTime(&systemTime);
	_snprintf(szFileName, sizeof(szFileName), "%s%s_%2.2d%2.2d_%2.2d%2.2d%2.2d_%d.csf", pPath, s_szStatsMapName,
