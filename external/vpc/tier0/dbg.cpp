...
#pragma optimize( \
	"g", off) // variable argument functions seem to screw up stack walking unless this optimization is disabled
...
static bool s_bCallStacksWithAllWarnings =
	false; // if true, attach a call stack to every SPEW_WARNING message. Warning()/DevWarning()/...
...
static bool s_bCallStacksWithAllErrors =
	false; // if true, attach a call stack to every SPEW_ERROR message. Mostly just Error()
...
void AppendCallStackToLogMessage(tchar *formattedMessage, int iMessageLength, int iAppendCallStackLength)
{
	...
	if(iAppendCallStackLength > 0)
	{
		int iExistingMessageLength = (int)strlen(
			formattedMessage); // no V_strlen in tier 0, plus we're only compiling this for windows and 360. Seems safe
		...
		if(TranslateStackInfo(CallStackBuffer, iCount, formattedMessage + iAppendedLength,
		...
		{
			iAppendedLength +=
				(int)strlen(formattedMessage + iAppendedLength); // no V_strlen in tier 0, plus we're only compiling
...
#define Log_LegacyHelper_Stack(Channel, Severity, MessageFormat, AppendCallStackLength)                     \
	Log_LegacyHelperColor_Stack(Channel, Severity, pLoggingSystem->GetChannelColor(Channel), MessageFormat, \
								AppendCallStackLength)
#define Log_LegacyHelper(Channel, Severity, MessageFormat) \
	Log_LegacyHelperColor(Channel, Severity, pLoggingSystem->GetChannelColor(Channel), MessageFormat)
...
HardwareBreakpointHandle_t SetHardwareBreakpoint(EHardwareBreakpointType eType, EHardwareBreakpointSize eSize,
