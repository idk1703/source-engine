...
// Only errors are spewed by default.
BEGIN_DEFINE_LOGGING_CHANNEL(LOG_DEVELOPER_VERBOSE, "DeveloperVerbose", LCF_CONSOLE_ONLY, LS_ERROR,
...
CLoggingSystem::CLoggingSystem()
	: m_nChannelCount(0), m_nChannelTagCount(0), m_nTagNamePoolIndex(0), m_nGlobalStateIndex(0)
{
...
LoggingChannelID_t CLoggingSystem::RegisterLoggingChannel(const char *pChannelName, RegisterTagsFunc registerTagsFunc,
														  int flags, LoggingSeverity_t severity, Color spewColor)
{
	...
	else
	{
		...
		for(int i = 0; i < m_nChannelCount; ++i)
		{
			if(V_tier0_stricmp(m_RegisteredChannels[i].m_Name, pChannelName) == 0)
			{
				...
				// This allows multiple people to register a logging channel, and the union of all tags will be
				// registered.
				...
				// If a logging channel is registered multiple times, only one of the registrations should specify
				// flags/severity/color.
				if(m_RegisteredChannels[i].m_Flags == 0 && m_RegisteredChannels[i].m_MinimumSeverity == LS_MESSAGE &&
				...
				else
				{
					...
					AssertMsg(severity == LS_MESSAGE || severity == m_RegisteredChannels[i].m_MinimumSeverity,
							  "Non-default or mismatched severity specified in logging channel re-registration!");
...
LoggingResponse_t CLoggingSystem::LogDirect(LoggingChannelID_t channelID, LoggingSeverity_t severity, Color color,
...
LoggingChannelID_t LoggingSystem_RegisterLoggingChannel(const char *pName, RegisterTagsFunc registerTagsFunc, int flags,
...
LoggingResponse_t LoggingSystem_Log(LoggingChannelID_t channelID, LoggingSeverity_t severity,
...
LoggingResponse_t LoggingSystem_Log(LoggingChannelID_t channelID, LoggingSeverity_t severity, Color spewColor,
									const char *pMessageFormat, ...)
{
	if(!GetGlobalLoggingSystem()->IsChannelEnabled(channelID, severity))
		return LR_CONTINUE;
...
LoggingResponse_t LoggingSystem_LogDirect(LoggingChannelID_t channelID, LoggingSeverity_t severity, Color spewColor,
										  const char *pMessage)
{
	if(!GetGlobalLoggingSystem()->IsChannelEnabled(channelID, severity))
		return LR_CONTINUE;
