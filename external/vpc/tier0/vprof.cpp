	...
	if(GetRoot() != pNode)
	{
		   ...
		   (budgetGroupID == -1 || pNode->GetBudgetGroupID() == budgetGroupID))
		{
			...
			iter = g_TimeSumsMap.find(
				pNode->GetName()); // intenionally using address of string rather than string compare (toml 01-27-03)
			if(iter == g_TimeSumsMap.end())
			{
				TimeSums_t timeSums = {pNode->GetName(), pNode->GetTotalCalls(), pNode->GetTotalTime(),
									   timeLessChildren, pNode->GetPeakTime()};
...
const char *CVProfile::GetCPUTraceFilename()
{
	...
	switch(m_iCPUTraceEnabled)
	{
		...
		case kAllNodesInFrame_RecordingMultiFrame:
			_snprintf(retBuf, sizeof(retBuf), "e:\\%.128s_%.4d_%.4d.pix2", m_CPUTraceFilename, m_nFrameCount,
	...
	if(!fIsRoot)
	{
		...
		if(bAverageAndCountOnly)
		{
			Msg(_T("  %10.3f %9.2f      %8d"),
				...
				(pNode->GetTotalCalls() > 0) ? dNodeTime / (double)NumFramesSampled() : 0, pNode->GetTotalCalls());
		}
		else
		{
			Msg(_T("  %10.3f %9.2f      %10.3f %6.2f      %10.3f %6.2f   %8d %6.2f"), pNode->GetTotalTime(), dNodeTime,
				(pNode->GetTotalCalls() > 0) ? pNode->GetTotalTime() / (double)NumFramesSampled() : 0,
				...
				(pNode->GetTotalCalls() > 0) ? dNodeTime / (double)pNode->GetTotalCalls() : 0, pNode->GetTotalCalls(),
				pNode->GetPeakTime());
		}
...
static void CalcBudgetGroupL2CacheMisses_Recursive(CVProfNode *pNode, unsigned int *groupTimes, int numGroups,
												   float flScale)
{
	int groupID;
	CVProfNode *nodePtr;
...
void CVProfile::VXProfileStart()
{
	...
	if(m_UpdateMode & (VPROF_UPDATE_TEXTURE_GLOBAL | VPROF_UPDATE_TEXTURE_PERFRAME))
	{
		...
		numGroups = 0;
		counterGroup = (m_UpdateMode & VPROF_UPDATE_TEXTURE_GLOBAL) ? COUNTER_GROUP_TEXTURE_GLOBAL
		...
		if(pNode && pNode->GetChild())
		{
			switch(m_ReportMode)
			{
				...
				case VXCONSOLE_REPORT_TIME:
					CalcBudgetGroupTimes_Recursive(pNode->GetChild(), groupData, numGroups,
												   m_pReportScale[VXCONSOLE_REPORT_TIME]);
					break;

				case VXCONSOLE_REPORT_L2CACHE_MISSES:
					CalcBudgetGroupL2CacheMisses_Recursive(pNode->GetChild(), groupData, numGroups,
														   m_pReportScale[VXCONSOLE_REPORT_L2CACHE_MISSES]);
					break;

				case VXCONSOLE_REPORT_LOAD_HIT_STORE:
					CalcBudgetGroupLHS_Recursive(pNode->GetChild(), groupData, numGroups,
...
static void DumpSorted(const tchar *pszHeading, double totalTime,
					   bool (*pfnSort)(const TimeSums_t &, const TimeSums_t &), int maxLen = 999999)
{
	unsigned i;
	...
	Msg(_T("%s\n"), pszHeading);
	Msg(_T("  Scope                                                      Calls Calls/Frame  Time+Child    Pct        ")
		_T("Time    Pct   Avg/Frame    Avg/Call Avg-NoChild        Peak\n"));
	Msg(_T("  ---------------------------------------------------- ----------- ----------- ----------- ------ ")
		_T("----------- ------ ----------- ----------- ----------- -----------\n"));
	for(i = 0; i < sortedSums.size() && i < (unsigned)maxLen; i++)
	{
		...
		double avgLessChildren =
			(sortedSums[i].calls) ? sortedSums[i].timeLessChildren / (double)sortedSums[i].calls : 0.0;

		Msg(_T("  %52.52s%12d%12.3f%12.3f%7.2f%12.3f%7.2f%12.3f%12.3f%12.3f%12.3f\n"), sortedSums[i].pszProfileScope,
			sortedSums[i].calls, (float)sortedSums[i].calls / (float)g_TotalFrames, sortedSums[i].time,
			min((sortedSums[i].time / totalTime) * 100.0, 100.0), sortedSums[i].timeLessChildren,
			min((sortedSums[i].timeLessChildren / totalTime) * 100.0, 100.0), sortedSums[i].time / (float)g_TotalFrames,
			avg, avgLessChildren, sortedSums[i].peak);
	}
}
...
static void DumpPMC(CVProfNode *pNode, bool &bPrintHeader, uint64 L2thresh = 1, uint64 LHSthresh = 1)
{
	...
	if(l2 > L2thresh && lhs > LHSthresh)
	{
		...
		Msg(_T("%52.52s %9.2f %9.2f %9.2f %9.2f\n"), pNode->GetName(), l2 / calls, l2 / frames, lhs / calls,
			lhs / frames);
	}
...
void CVProfile::OutputReport(int type, const tchar *pszStartNode, int budgetGroupID)
{
...
#ifdef _MSC_VER
#if(_MSC_VER < 1300)
	Msg(_T("  (note: this report exceeds the output capacity of MSVC debug window. Use console window or console ")
		_T("log.) \n"));
#endif
#endif
	...
	else
	{
		...
		{
			DumpSorted(_T("-- Profile scopes sorted by time (including children) --"), GetTotalTimeSampled(),
					   TimeCompare, maxLen);
			Msg(_T("\n"));
		}
		if(type & VPRT_LIST_BY_TIME_LESS_CHILDREN)
		{
			DumpSorted(_T("-- Profile scopes sorted by time (without children) --"), GetTotalTimeSampled(),
					   TimeLessChildrenCompare, maxLen);
			Msg(_T("\n"));
		}
		if(type & VPRT_LIST_BY_AVG_TIME)
		{
			DumpSorted(_T("-- Profile scopes sorted by average time (including children) --"), GetTotalTimeSampled(),
					   AverageTimeCompare, maxLen);
			Msg(_T("\n"));
		}
		if(type & VPRT_LIST_BY_AVG_TIME_LESS_CHILDREN)
		{
			DumpSorted(_T("-- Profile scopes sorted by average time (without children) --"), GetTotalTimeSampled(),
					   AverageTimeLessChildrenCompare, maxLen);
			Msg(_T("\n"));
		}
		...
		{
			DumpSorted(_T("-- Profile scopes sorted by peak over average (including children) --"),
...
CVProfile::CVProfile()
	: m_Root(_T("Root"), 0, NULL, VPROF_BUDGETGROUP_OTHER_UNACCOUNTED, 0),
	  ...
	  m_nFrames(0),
	  m_enabled(
		  0), // don't change this. if m_enabled is anything but zero coming out of this constructor, vprof will break.
...
#define GET_INTERNAL_MAP_ALLOC_PTR(pMap) (*((void **)(((byte *)(pMap)) + k_cSTLMapAllocOffset)))
//-----------------------------------------------------------------------------
// Purpose: Ensure that all of our internal structures are consistent, and
//			account for all memory that we've allocated.
// Input:	validator -		Our global validator object
