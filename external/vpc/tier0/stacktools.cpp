...
int TranslateStackInfo(const void *const *pCallStack, int iCallStackCount, tchar *szOutput, int iOutBufferSize,
					   const tchar *szEntrySeparator, TranslateStackInfo_StyleFlags_t style)
{
	if(iOutBufferSize > 0)
		*szOutput = '\0';
...
bool GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut, int iMaxFileNameLength,
							   uint32 &iLineNumberOut, uint32 *pDisplacementOut)
{
...
bool GetSymbolNameFromAddress(const void *pAddress, tchar *pSymbolNameOut, int iMaxSymbolNameLength,
...
inline bool ValidStackAddress(void *pAddress, const void *pNoLessThan, const void *pNoGreaterThan)
{
	...
	if(IsBadReadPtr(pAddress, (sizeof(void *) *
							   2))) // safety net, but also throws an exception (handled internally) to stop bad access
		return false;
#endif
...
int GetCallStack_Fast(void **pReturnAddressesOut, int iArrayCount, int iSkipCount)
{
...
#if(defined(TIER0_FPO_DISABLED) || defined(_DEBUG)) && (defined(WIN32) && !defined(_X360))
	void *pStackCrawlEBP;
	__asm
	{
	...
	*/
	...
	{
		void *pNoGreaterThan =
			((unsigned char *)pNoLessThan) + (1024 * 1024); // standard stack is 1MB. TODO: Get actual stack end address
															// if available since this check isn't foolproof
...
//#	define USE_CAPTURESTACKBACKTRACE //faster than StackWalk64, but only works on XP or newer and only with Frame
//Pointer Omission optimization disabled(/Oy-) for every function it traces through
#endif
...
typedef BOOL(WINAPI *PFN_SymEnumerateModules64)(IN HANDLE hProcess, IN PSYM_ENUMMODULES_CALLBACK64 EnumModulesCallback,
												IN PVOID UserContext);
typedef BOOL(WINAPI *PFN_EnumerateLoadedModules64)(IN HANDLE hProcess,
												   IN PENUMLOADED_MODULES_CALLBACK64 EnumLoadedModulesCallback,
												   IN PVOID UserContext);
typedef DWORD64(WINAPI *PFN_SymLoadModule64)(IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN PSTR ModuleName,
											 IN DWORD64 BaseOfDll, IN DWORD SizeOfDll);
typedef BOOL(WINAPI *PFN_SymUnloadModule64)(IN HANDLE hProcess, IN DWORD64 BaseOfDll);
typedef BOOL(WINAPI *PFN_SymFromAddr)(IN HANDLE hProcess, IN DWORD64 Address, OUT PDWORD64 Displacement,
									  IN OUT PSYMBOL_INFO Symbol);
typedef BOOL(WINAPI *PFN_SymGetLineFromAddr64)(IN HANDLE hProcess, IN DWORD64 qwAddr, OUT PDWORD pdwDisplacement,
											   OUT PIMAGEHLP_LINE64 Line64);
typedef BOOL(WINAPI *PFN_SymGetModuleInfo64)(IN HANDLE hProcess, IN DWORD64 dwAddr, OUT PIMAGEHLP_MODULE64 ModuleInfo);
typedef BOOL(WINAPI *PFN_StackWalk64)(DWORD MachineType, HANDLE hProcess, HANDLE hThread, LPSTACKFRAME64 StackFrame,
									  PVOID ContextRecord, PREAD_PROCESS_MEMORY_ROUTINE64 ReadMemoryRoutine,
									  PFUNCTION_TABLE_ACCESS_ROUTINE64 FunctionTableAccessRoutine,
									  PGET_MODULE_BASE_ROUTINE64 GetModuleBaseRoutine,
									  PTRANSLATE_ADDRESS_ROUTINE64 TranslateAddress);
typedef USHORT(WINAPI *PFN_CaptureStackBackTrace)(IN ULONG FramesToSkip, IN ULONG FramesToCapture, OUT PVOID *BackTrace,
...
BOOL WINAPI SymEnumerateModules64_DummyFn(IN HANDLE hProcess, IN PSYM_ENUMMODULES_CALLBACK64 EnumModulesCallback,
										  IN PVOID UserContext)
{
	return FALSE;
}
...
DWORD64 WINAPI SymLoadModule64_DummyFn(IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN PSTR ModuleName,
									   IN DWORD64 BaseOfDll, IN DWORD SizeOfDll)
{
	return 0;
}
...
BOOL WINAPI SymFromAddr_DummyFn(IN HANDLE hProcess, IN DWORD64 Address, OUT PDWORD64 Displacement,
								IN OUT PSYMBOL_INFO Symbol)
{
	return FALSE;
}

BOOL WINAPI SymGetLineFromAddr64_DummyFn(IN HANDLE hProcess, IN DWORD64 qwAddr, OUT PDWORD pdwDisplacement,
										 OUT PIMAGEHLP_LINE64 Line64)
{
	return FALSE;
}
...
BOOL WINAPI StackWalk64_DummyFn(DWORD MachineType, HANDLE hProcess, HANDLE hThread, LPSTACKFRAME64 StackFrame,
								PVOID ContextRecord, PREAD_PROCESS_MEMORY_ROUTINE64 ReadMemoryRoutine,
								PFUNCTION_TABLE_ACCESS_ROUTINE64 FunctionTableAccessRoutine,
								PGET_MODULE_BASE_ROUTINE64 GetModuleBaseRoutine,
								PTRANSLATE_ADDRESS_ROUTINE64 TranslateAddress)
...
USHORT WINAPI CaptureStackBackTrace_DummyFn(IN ULONG FramesToSkip, IN ULONG FramesToCapture, OUT PVOID *BackTrace,
											OUT OPTIONAL PULONG BackTraceHash)
{
	return 0;
}
...
public:
	...
	{
		const CHelperFunctionsLoader *pThis = ((CHelperFunctionsLoader *)UserContext);
		// SymLoadModule64( IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN PSTR ModuleName, IN DWORD64
		// BaseOfDll, IN DWORD SizeOfDll );
	...
	void TryLoadingNewSymbols(void)
	{
		...
		if(m_bIsInitialized)
		{
			// m_pSymEnumerateModules64( m_hProcess, UnloadSymbolsCallback, this ); //unloaded modules we've already
			// loaded
	...
	void SetStackTranslationSymbolSearchPath(const char *szSemicolonSeparatedList)
	{
		...
		// re-init search paths. Or if we haven't yet loaded dbghelp.dll, this will go to the dummy function and do
		// nothing
	...
	bool TranslatePointer(const void *const pAddress, tchar *pTranslationOut, int iTranslationBufferLength,
						  TranslateStackInfo_StyleFlags_t style)
	{
		...
		//  u:\Dev\L4D\src\tier0\stacktools.cpp(162) + 4 bytes" fallback translation -	"tier0.dll!0x01234567"
		...
		*pWrite = '\0';
		...
		{
			if(pWrite !=
			   pTranslationOut) // if we've written anything yet, separate the printed data from the file name and line
			{
			...
			if(this->GetFileAndLineFromAddress(pAddress, szFileName, MAX_PATH, iLine, &iDisplacement))
			{
				...
				if(style & (TSISTYLEFLAG_LINE | TSISTYLEFLAG_LINEANDOFFSET))
				{
					int nBytesWritten = _snprintf(
						...
						((style & TSISTYLEFLAG_LINEANDOFFSET) && (iDisplacement != 0)) ? "(%d) + %d bytes" : "(%d)",
						iLine, iDisplacement);
					if(nBytesWritten < 0)
					{
						*pWrite = '\0'; // if we can't write all of the line/lineandoffset, don't write any at all
	...
	void EnsureReady(void)
	{
		...
		if(!m_hDbgHelpDll)
		{
			// it's possible it's just way too early to initialize (as shown with attempts at using these tools in the
			// memory allocator)
			if(m_szPDBSearchPath == NULL) // not a rock solid check, but pretty good compromise between endless failing
										  // initialization and general failure due to trying too early
				m_bIsInitialized = false;
		...
		m_pEnumerateLoadedModules64 =
			(PFN_EnumerateLoadedModules64)::GetProcAddress(m_hDbgHelpDll, "EnumerateLoadedModules64");
		...
		m_pSymSetOptions(m_pSymGetOptions() | SYMOPT_DEFERRED_LOADS |	   // load on demand
...
int CrawlStack_StackWalk64(CONTEXT *pExceptionContext, void **pReturnAddressesOut, int iArrayCount, int iSkipCount)
{
	...
	{
		if(!s_HelperFunctions.m_pStackWalk64(STACKWALK64_MACHINETYPE, s_HelperFunctions.m_hProcess, hThread, &sfFrame,
											 &currentContext, NULL, NULL, NULL, NULL) ||
...
void GetCallStackReturnAddresses_Exception(void **CallStackReturnAddresses, int *pRetCount, int iSkipCount,
										   _EXCEPTION_POINTERS *pExceptionInfo)
{
	int iCount = CrawlStack_StackWalk64(pExceptionInfo->ContextRecord, CallStackReturnAddresses, *pRetCount,
...
int GetCallStack(void **pReturnAddressesOut, int iArrayCount, int iSkipCount)
{
	...
	if(s_HelperFunctions.m_pCaptureStackBackTrace != CaptureStackBackTrace_DummyFn)
	{
		...
		int iRetVal = s_HelperFunctions.m_pCaptureStackBackTrace(iSkipCount, MIN(iArrayCount, 63 - iSkipCount),
																 pReturnAddressesOut, NULL);
	...
	if(s_HelperFunctions.m_pStackWalk64 != StackWalk64_DummyFn)
	{
		...
		__except(GetCallStackReturnAddresses_Exception(pReturnAddressesOut, &iInOutArrayCount, iSkipCount,
...
{
	s_HelperFunctions.EnsureReady();
	return s_HelperFunctions.GetFileAndLineFromAddress(pAddress, pFileNameOut, iMaxFileNameLength, iLineNumberOut,
													   pDisplacementOut);
}
...
public:
	...
	{
		std::pair<AddressInfoMapIter_t, bool> retval =
			m_AddressInfoMap.insert(AddressInfoMapEntry_t(info.pAddress, StackAddressInfo_t()));
	...
	inline int RetrieveStackInfo(const void *const *pAddresses, FullStackInfo_t *pReturnedStructs, int iAddressCount)
	{
		...
		//				Header	Finished Count(out)		Input Count					Input Array			Returned data write
		//address
		int iMessageSize =
			2 + sizeof(int *) + sizeof(uint32) + (sizeof(void *) * iAddressCount) + sizeof(FullStackInfo_t *);
	...
	inline StackAddressInfo_t *CreateEntry(const void *pAddress)
	{
		...
		ReturnedData.szFileName[0] =
			'\0'; // strncpy( ReturnedData.szFileName, "FileUninitialized", sizeof( ReturnedData.szFileName ) );
		ReturnedData.szModuleName[0] =
			'\0'; // strncpy( ReturnedData.szModuleName, "ModuleUninitialized", sizeof( ReturnedData.szModuleName ) );
		ReturnedData.szSymbol[0] =
			'\0'; // strncpy( ReturnedData.szSymbol, "SymbolUninitialized", sizeof( ReturnedData.szSymbol ) );
	...
	inline void LoadStackInformation(void *const *pAddresses, int iAddressCount)
	{
		...
		int iNeedLoading = 0;
		void **pNeedLoading =
			(void **)stackalloc(sizeof(const void *) * iAddressCount); // addresses we need to ask VXConsole about
		...
		if(iNeedLoading != 0)
		{
			...
			for(int i = 0; i < iNeedLoading; ++i)
			{
				...
				pReturnedStructs[i].szFileName[0] =
					'\0'; // strncpy( pReturnedStructs[i].szFileName, "FileUninitialized", sizeof(
						  // pReturnedStructs[i].szFileName ) );
				pReturnedStructs[i].szModuleName[0] =
					'\0'; // strncpy( pReturnedStructs[i].szModuleName, "ModuleUninitialized", sizeof(
						  // pReturnedStructs[i].szModuleName ) );
				pReturnedStructs[i].szSymbol[0] = '\0'; // strncpy( pReturnedStructs[i].szSymbol, "SymbolUninitialized",
														// sizeof( pReturnedStructs[i].szSymbol ) );
	...
	inline bool GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut, int iMaxFileNameLength,
										  uint32 &iLineNumberOut, uint32 *pDisplacementOut)
	{
	...
	inline bool GetSymbolNameFromAddress(const void *pAddress, tchar *pSymbolNameOut, int iMaxSymbolNameLength,
										 uint64 *pDisplacementOut)
	{
		StackAddressInfo_t *pInfo = FindOrCreateEntry(pAddress);
...
int GetCallStack(void **pReturnAddressesOut, int iArrayCount, int iSkipCount)
{
	...
	if(DmCaptureStackBackTrace(iArrayCount + iSkipCount, pAllResults) == XBDM_NOERR)
	{
		...
		{
			if(*pAllResults ==
			   NULL) // DmCaptureStackBackTrace() NULL terminates the list instead of telling us how many were returned
				return AppendParentStackTrace(pReturnAddressesOut, iArrayCount, 0);
					   ...
					   const tchar *szEntrySeparator, TranslateStackInfo_StyleFlags_t style)
{
	...
	if(iOutBufferSize > (iEncodedSize + (int)cControlLength + 2)) //+2 for ']' and null term
	{
		COMPILE_TIME_ASSERT(TSISTYLEFLAG_LAST <
							(1 << 8)); // need to update the encoder/decoder to use more than a byte for style flags
		...
		int iLength = cControlLength + EncodeBinaryToString(pData, iDataSize, &szOutput[cControlLength],
...
{
	AUTO_LOCK_FM(s_360StackTranslator.m_hMutex);
	return s_360StackTranslator.GetFileAndLineFromAddress(pAddress, pFileNameOut, iMaxFileNameLength, iLineNumberOut,
														  pDisplacementOut);
}
...
{
	AUTO_LOCK_FM(s_360StackTranslator.m_hMutex);
	return s_360StackTranslator.GetSymbolNameFromAddress(pAddress, pSymbolNameOut, iMaxSymbolNameLength,
														 pDisplacementOut);
}
...
CStackTop_ReferenceParentStack::CStackTop_ReferenceParentStack(void *const *pParentStackTrace,
...
int EncodeBinaryToString(const void *pToEncode, int iDataLength, char *pEncodeOut, int iEncodeBufferSize)
{
	...
	while(pEncodeRead < pEncodeStop)
	{
		...
		{
			*pEncodeWrite =
				*pEncodeRead | 0x80; // encoded data always has the MSB bit set (cheap avoidance of null terminators)
			*pEncodeWriteLastControlByte |=
				(((*pEncodeRead) & 0x80) ^ 0x80) >>
				iControl; // We use the control byte to XOR the MSB back to original values on decode
			++pEncodeRead;
		}
...
// Decodes a string produced by EncodeBinaryToString(). Safe to decode in place if you don't mind trashing your string,
// binary byte count always less than string byte count. Returns:
