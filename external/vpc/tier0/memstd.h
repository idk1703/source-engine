...
public:
	void Push(void *p)
	{
		CTSListBase::Push((TSLNodeBase_t *)p);
	}
	byte *Pop()
	{
		return (byte *)CTSListBase::Pop();
	}
};
...
public:
	...
	int CrtDbgReport(int nRptType, const char *szFile, int nLine, const char *szModule, const char *pMsg);
	virtual int heapchk();
...
#error
	...
	public:
		...
		byte *AllocatePoolMemory()
		{
			...
			if(!pSection)
				Error("CFixedAllocator::AllocatePoolMemory() failed in "
...
#error
	...
	typedef CSmallBlockHeap<CFixedAllocator<MBYTES_SECONDARY_SBH, false>>
		CFixedVirtualSmallBlockHeap; // @TODO: move back into above heap if number stays at 16 [7/15/2009 tom]
#endif
