...
static bool HTSupported(void)
{
	...
	enum
	{
		...
		FAMILY_ID_PENTIUM_PRO =
			0x0600, //            P Pro, P II, P III, P M, Celeron M, Core Duo, Core Solo, Core2 Duo, Core2 Extreme, P
					//            D, Xeon model F, also 45-nm : Intel Atom, Core i7, Xeon MP ; see Intel Processor
					//            Identification and the CPUID instruction pg 20,21

		FAMILY_ID_EXTENDED = 0x0F00 //               P IV, Xeon, Celeron D, P D,
	};

	uint32 unused, reg_eax = 0, reg_ebx = 0, reg_edx = 0, vendor_id[3] = {0, 0, 0};

	// verify cpuid instruction is supported
	if(!cpuid(0, unused, vendor_id[0], vendor_id[2], vendor_id[1]) || !cpuid(1, reg_eax, reg_ebx, unused, reg_edx))
		return false;
...
int LogicalProcessorsPerCore()
{
	...
	if(nFn1_Edx & HTT)
	{
		...
		{
			// NOTE:  This is only tested on Intel CPUs; I don't know if it's true on AMD, as I have no HT AMD to test
			// on
...
const CPUInformation &GetCPUInformation()
{
	...
	// Sergiy: fixing: si.dwNumberOfProcessors is the number of logical processors according to experiments on i7, P4
	// and a DirectX sample (Aug'09)
	...
	{
		Assert(!"Sergiy: apparently I didn't fix some CPU detection code completely. Let me know and I'll do my best "
				"to fix it soon.");
		pi.m_nPhysicalProcessors = 1;
