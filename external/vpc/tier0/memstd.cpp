...
void *g_AllocRegions[] = {
	...
	// @TODO: per DLL regions didn't work out very well. flux of usage left too much overhead. need to try
	// lifetime-based management [6/9/2009 tom]
...
inline void heapstats_internal(FILE *pFile)
{
	...
	for(int i = 0; i < ARRAYSIZE(g_AllocRegions); i++)
	{
		...
		_snprintf(buf, sizeof(buf),
				  ...
				  i, i ? "medium-block" : "large-block", i, footPrint, i, maxFootPrint, i, info.arena, i, info.ordblks,
				  i, info.hblkhd, i, info.usmblks, i, info.uordblks, i, info.fordblks, i, info.keepcost);
		if(pFile)
			fprintf(pFile, "%s", buf);
		else
			Msg("%s", buf);
...
#define PrintOne(name)                                                                                                \
	Msg("%-48s: %6.4f avg (%8.1f total, %7.3f peak, %5d iters)\n", #name, g_##name##Counter.GetAverageMilliseconds(), \
		g_##name##Counter.GetTotalMilliseconds(), g_##name##Counter.GetPeakMilliseconds(),                            \
...
#if MEM_SBH_ENABLED
CSmallBlockPool<CStdMemAlloc::CFixedAllocator<MBYTES_PRIMARY_SBH, true>>::SharedData_t
	CSmallBlockPool<CStdMemAlloc::CFixedAllocator<MBYTES_PRIMARY_SBH, true>>::gm_SharedData CONSTRUCT_EARLY;
#ifdef MEMALLOC_USE_SECONDARY_SBH
CSmallBlockPool<CStdMemAlloc::CFixedAllocator<MBYTES_SECONDARY_SBH, false>>::SharedData_t
	CSmallBlockPool<CStdMemAlloc::CFixedAllocator<MBYTES_SECONDARY_SBH, false>>::gm_SharedData CONSTRUCT_EARLY;
#endif
#ifndef MEMALLOC_NO_FALLBACK
CSmallBlockPool<CStdMemAlloc::CVirtualAllocator>::SharedData_t
	CSmallBlockPool<CStdMemAlloc::CVirtualAllocator>::gm_SharedData CONSTRUCT_EARLY;
#endif
...
CStdMemAlloc::CStdMemAlloc()
	: m_pfnFailHandler(DefaultFailHandler), m_sMemoryAllocFailed((size_t)0), m_bInCompact(false)
{
#ifdef _PS3
...
// Define VALIDATE_SBH_FREE_LIST to a given block size to validate that pool's freelist (it'll crash on the next
// alloc/free after the list is corrupted) NOTE: this may affect perf more than USE_LIGHT_MEM_DEBUG
					...
					if(!m_pNextAlloc)
					{
						...
						else
						{
							while(1)
							{
								...
								if(ThreadInterlockedAssignPointerIf((void **)&pSharedData->m_pNextBlock,
	...
	if(pFile)
	{
		...
		for(int i = 0; i < NUM_POOLS; i++)
		{
			...
			fprintf(pFile,
					...
					m_Pools[i].GetBlockSize(), m_Pools[i].CountAllocatedBlocks(), m_Pools[i].CountFreeBlocks(),
					m_Pools[i].CountCommittedBlocks(), m_Pools[i].GetCommittedSize());
		}
		fprintf(pFile, "Totals (%s): Committed:%5u kb Allocated:%5u kb\n", pszTag, bytesCommitted / 1024,
				bytesAllocated / 1024);
	}
	else
	{
		for(int i = 0; i < NUM_POOLS; i++)
		{
			Msg("Pool %2i: (size: %4u) blocks: allocated:%5i free:%5i committed:%5i (committed size:%4u kb)\n", i,
				m_Pools[i].GetBlockSize(), m_Pools[i].CountAllocatedBlocks(), m_Pools[i].CountFreeBlocks(),
				m_Pools[i].CountCommittedBlocks(), m_Pools[i].GetCommittedSize() / 1024);
		}
...
void LMDReportInvalidBlock(AllocHeader_t *pHeader, const char *pszMessage)
{
	...
	if(pHeader)
	{
		sprintf(szMsg, "HEAP IS CORRUPT: %s (block 0x%x, size %d, alignment %d)\n", pszMessage,
				(size_t)LMDFromHeader(pHeader), pHeader->nBytes, pHeader->align);
	}
	else
	{
...
size_t LMDComputeHeaderSize(size_t align = 0)
{
	...
	if(align > LMD_MAX_ALIGN)
		s_StdMemAlloc.SetCRTAllocFailed(align); // TODO: could convert alignment to exponent to get around this, or use
												// a flag for alignments over 1KB or 1MB...
...
FORCEINLINE void *LMDRealloc(void *pMem, size_t nSize, size_t align = 0, const char *pszModule = NULL, int line = 0)
{
	return NULL;
}
...
{
	size_t nAdjustedSize = LMDAdjustSize(nSize, align);
	return LMDNoteAlloc(CStdMemAlloc::InternalAllocAligned(DEF_REGION, nAdjustedSize, align), nSize, align, pFileName,
						nLine);
}
#endif // MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS
...
int CStdMemAlloc::CrtDbgReport(int nRptType, const char *szFile, int nLine, const char *szModule, const char *pMsg)
{
	return 0;
}
...
int CStdMemAlloc::GetGenericMemoryStats(GenericMemoryStat_t **ppMemoryStats)
{
...
#if !defined(MEMALLOC_REGIONS) && defined(MEMALLOC_SEGMENT_MIXED)
	{
		...
		size_t nMBHCurUsed = infoMBH.uordblks; // nMBH_WRONG_MaxUsed = infoMBH.usmblks; // TODO: figure out why dlmalloc
											   // mis-reports MBH max usage (it just returns the footprint)
		...
		size_t nLBHCurUsed = mspace_footprint(g_AllocRegions[0]), nLBHMaxUsed = mspace_max_footprint(g_AllocRegions[0]),
		...
		// LBH arena used+free (these are non-virtual allocations - there should be none, since we only allocate 256KB+
		// items in the LBH)
		// TODO: I currently see the arena grow to 320KB due to a larger allocation being realloced down... if this gets
		// worse, add an 'ALWAYS use VMM' flag to the mspace.
...
#ifdef _PS3
	{
		...
		extern void VirtualMemoryManager_GetStats(size_t & nReserved, size_t & nReservedMax, size_t & nCommitted,
	...
	if(IsX360())
	{
		...
		ExecuteOnce({
			...
			_snprintf(buffer, sizeof(buffer),
					  "***** Memory pool overflow, attempted allocation size: %u (not a critical error)\n", nBytes);
