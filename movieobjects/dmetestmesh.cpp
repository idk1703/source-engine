...
void CDmeTestMesh::OnConstruction()
{
	...
	m_pControlCage = NULL;
	SetValue("transform",
			 g_pDataModel->IsUnserializing() ? NULL : CreateElement<CDmeTransform>("transform", GetFileId()));
...
void CDmeTestMesh::CreateMesh()
{
	...
	static Vector s_pPositions[8] = {
		Vector(-10, -10, -10), Vector(10, -10, -10), Vector(-10, 10, -10), Vector(10, 10, -10),
		Vector(-10, -10, 10),  Vector(10, -10, 10),	 Vector(-10, 10, 10),  Vector(10, 10, 10),
	};

	static Vector2D s_pTexCoords[8] = {
		Vector2D(0, 0),		Vector2D(0.5, 0), Vector2D(0, 0.5), Vector2D(0.5, 0.5),
		Vector2D(0.5, 0.5), Vector2D(1, 0.5), Vector2D(0.5, 1), Vector2D(1, 1),
	};

	static unsigned char s_pColor[8][3] = {
		{255, 255, 255}, {0, 255, 255}, {255, 0, 255}, {255, 255, 0}, {255, 0, 0}, {0, 255, 0}, {0, 0, 255}, {0, 0, 0},
	};

	static int s_pIndices[12][3] = {
		{0, 1, 5}, {0, 5, 4}, {4, 5, 7}, {4, 7, 6}, {0, 4, 6}, {0, 6, 2},
		{0, 2, 3}, {0, 3, 1}, {1, 3, 7}, {1, 7, 5}, {2, 6, 7}, {2, 7, 3},
	};
...
{
	int m_pFace[2];
	int m_pRelativeEdgeIndex[2]; // Goes from 0-N always, specifies the Nth edge of the polygon it's part of for each of
								 // the two faces
	int m_pVertex[2];
};
...
static void SubdivideMesh(const SubdivMesh_t &src, SubdivMesh_t &dest)
{
	...
	// Add modified versions of the vertices in the src mesh based on the new computed points and add them to the dest
	// mesh
...
void CDmeTestMesh::CreateControlCage()
{
	...
	// Draw a simple cube
	static Vector s_pPositions[8] = {
		Vector(-30, -30, -30), Vector(30, -30, -30), Vector(-30, 30, -30), Vector(30, 30, -30),
		Vector(-30, -30, 30),  Vector(30, -30, 30),	 Vector(-30, 30, 30),  Vector(30, 30, 30),
	};
	...
	// in the list s_pIndices[s_pEdgeFaces[i][j]] below? Note the #s 0, 1, 2, and 3 should appear 6 times each in this
	// array representing the fact that each face has a 0th,1st,2nd, and 3rd edge.
...
void CDmeTestMesh::DrawSubdividedBox()
{
	...
	while(--nSubdivLevel > 0)
	{
		...
		if((subdivMesh[1 - nCurrMesh].m_nTotalLineCount * 2 >= 32768) ||
		   (subdivMesh[1 - nCurrMesh].m_Vertices.Count() >= 32768))
			break;
		...
		for(int vi = 0; vi < vn; ++vi)
		{
			...
			switch(vertex.skinning[0].index)
			{
				...
				case 2:
					...
					break;
				case 3:
					meshBuilder.Color3f(1, 1, 0);
					break;
				case 4:
					meshBuilder.Color3f(0, 1, 1);
					break;
				case 5:
					meshBuilder.Color3f(1, 0, 1);
					break;
				case 6:
					meshBuilder.Color3f(0, 0, 0);
					break;
				case 7:
					meshBuilder.Color3f(1, 1, 1);
					break;
				default:
					meshBuilder.Color3f(0.5f, 0.5f, 0.5f);
					break;
			}
...
void ReadTrianglesFromSMD(std::vector<submesh_t *> &meshes, int numbones, std::istream &is)
{
	...
	while(is.getline(line, MAXLINE))
	{
		...
		if(texname[0] == '\0' || stricmp(texname, "null.bmp") == 0 || stricmp(texname, "null.tga") == 0)
		{
			is.getline(line, MAXLINE);
			is.getline(line, MAXLINE);
			is.getline(line, MAXLINE);
