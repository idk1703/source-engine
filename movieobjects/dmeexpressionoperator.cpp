...
{
public:
	ParseState_t(const CUtlStack<float> &stack, const char *expr) : m_stacksize(stack.Count()), m_startingExpr(expr) {}
...
bool CExpressionCalculator::ParseConditional(const char *&expr)
{
	...
	if(ParseString(expr, "?") && ParseExpr(expr) && ParseString(expr, ":") && ParseExpr(expr))
	{
...
bool CExpressionCalculator::Parse1ArgFunc(const char *&expr)
{
	...
	const char *pFuncs[] = {"abs", "sqr", "sqrt", "sin",  "asin",  "cos",	  "acos",  "tan",
							"exp", "log", "dtor", "rtod", "floor", "ceiling", "round", "sign"};
	int nFunc = 16;

	if(ParseStringList(expr, pFuncs, nFunc) && ParseString(expr, "(") && ParseExpr(expr) && ParseString(expr, ")"))
	{
		float f1 = m_stack.Top();
		m_stack.Pop();
		switch(nFunc)
...
bool CExpressionCalculator::Parse3ArgFunc(const char *&expr)
{
	...
	if(ParseStringList(expr, pFuncs, nFunc) && ParseString(expr, "(") && ParseExpr(expr) && ParseString(expr, ",") &&
	   ParseExpr(expr) && ParseString(expr, ",") && ParseExpr(expr) && ParseString(expr, ")"))
	{
		float f3 = m_stack.Top();
		m_stack.Pop();
		float f2 = m_stack.Top();
...
bool CExpressionCalculator::Parse5ArgFunc(const char *&expr)
{
	...
	if(ParseStringList(expr, pFuncs, nFunc) && ParseString(expr, "(") && ParseExpr(expr) && ParseString(expr, ",") &&
	   ParseExpr(expr) && ParseString(expr, ",") && ParseExpr(expr) && ParseString(expr, ",") && ParseExpr(expr) &&
