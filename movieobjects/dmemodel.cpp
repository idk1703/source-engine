	...
	if(nJointCount > nCurrentCount)
	{
		for(int i = nCurrentCount; i < nJointCount; ++i)
		{
			CDmeTransform *pTransform =
				CreateElement<CDmeTransform>(m_JointTransforms[i]->GetName(), pTransformList->GetFileId());
...
//-----------------------------------------------------------------------------
void CDmeModel::LoadJointTransform(CDmeDag *pJoint, CDmeTransformList *pBindPose, const matrix3x4_t &parentToWorld,
								   const matrix3x4_t &parentToBindPose, bool bSetHardwareState)
{
	...
	if(pBindPose)
	{
		...
		{
			Warning("Model is in an invalid state! There are different numbers of bones in the bind pose and joint "
					"transform list!\n");
			return;
		}
