...
public:
	...
	inline void AddPosition(const Vector &p)
	{
		m_positions.AddToTail(p);
	}

	inline void AddPositionIndex(int i)
	{
		m_pIndices.AddToTail(i);
	}

	inline void AddNormal(const Vector &n)
	{
		m_normals.AddToTail(n);
	}

	inline void AddNormalIndex(int i)
	{
...
protected:
	template<class T_t>
	void AddUniqueValue(const T_t &v, CUtlVector<T_t> &vs, CUtlVector<int> &map, float flThresh = FLT_EPSILON)
	{
...
//-----------------------------------------------------------------------------
CDmElement *CDmObjSerializer::ReadOBJ(const char *pFilename, CDmeMesh **ppCreatedMesh, bool bLoadAllDeltas /* = true */,
									  bool bAbsolute /* = true */)
{
	...
	if(pRoot && pMesh)
	{
		...
		*(pSuffix + 2) = '.';
		...
		for(const char *pFindFile(g_pFullFileSystem->FindFirst(findPath, &hFind)); pFindFile && *pFindFile;
...
//-----------------------------------------------------------------------------
CDmElement *CDmObjSerializer::ReadOBJ(CUtlBuffer &buf, DmFileId_t dmFileId, const char *pName,
									  const char *pFilename /* = NULL */, CDmeMesh *pBaseMesh /* = NULL */,
									  CDmeMesh **ppCreatedMesh /* = NULL */, bool bAbsolute /* = true */)
{
	...
	while(buf.IsValid())
	{
		...
		if(!pBaseMesh)
		{
			...
			if(*pBuf == 'f' && (*(pBuf + 1) == ' ' || *(pBuf + 1) == '\t'))
			{
				if(pDmeDag == NULL)
				{
					pDmeDag = CreateElement<CDmeDag>(pName ? pName : (groupName.IsEmpty() ? "obj" : groupName.Get()),
													 pRoot->GetFileId());
					Assert(pDmeDag);
					pDmeMesh = CreateElement<CDmeMesh>(pName ? pName : (groupName.IsEmpty() ? "obj" : groupName.Get()),
													   pRoot->GetFileId());
...
//
//-----------------------------------------------------------------------------
int CDmObjSerializer::OutputVectors(CUtlBuffer &b, const char *pPrefix, const CUtlVector<Vector> &vData,
...
//
//-----------------------------------------------------------------------------
int CDmObjSerializer::OutputVectors(CUtlBuffer &b, const char *pPrefix, const CUtlVector<Vector2D> &vData)
{
	Vector v;
...
//
//-----------------------------------------------------------------------------
void CDmObjSerializer::MeshToObj(CUtlBuffer &b, const matrix3x4_t &parentWorldMatrix, CDmeMesh *pMesh,
								 const char *pDeltaName, bool absolute)
{
			...
			if(absolute)
			{
				for(int i(0); i < nCompList; ++i)
				{
					...
					for(int j(0); j < nDepDeltas; ++j)
					{
						pTmpDeltaState = pMesh->GetDeltaState(depDeltas[j]);
						b << "# Dependent Delta: " << pTmpDeltaState->GetName() << "\n";
						pMesh->AddDelta(pTmpDeltaState, nData.Base(), nData.Count(), CDmeVertexData::FIELD_NORMAL,
										1.0f);
					}
				}
			}
		...
		if(pCount == uvCount && pCount == nCount)
		{
			...
			while(pnFaceSetIndex < pEnd)
			{
				...
				b << pFaceSep << (pIndices[fIndex] + m_nPositionOffset) << '/' << (uvIndices[fIndex] + m_nTextureOffset)
				  << '/' << (nvIndices[fIndex] + m_nNormalOffset);
				pFaceSep = pFaceNext;
			}
		}
...
//
//-----------------------------------------------------------------------------
void CDmObjSerializer::DagToObj(CUtlBuffer &b, const matrix3x4_t &parentWorldMatrix, CDmeDag *pDag,
								const char *pDeltaName, bool absolute)
{
...
// Convert from OBJ -> DME
//-----------------------------------------------------------------------------
bool CDmObjSerializer::WriteOBJ(const char *pFilename, CDmElement *pRoot, bool bWriteOBJs, const char *pDeltaName,
								bool absolute)
{
...
void CDmObjSerializer::ParseMtlLib(CUtlBuffer &buf)
{
	...
	while(buf.IsValid())
	{
		...
		if(StringHasPrefix(tmpBuf0, "map_Kd "))
		{
			...
			{
				// Try a cheesy hack - look for /materialsrc/ and set the material name off the entire path minus
				// extension
...
CDmeVertexDeltaData *CDmObjSerializer::GetDelta(const char *pDeltaName, bool bAbsolute)
{
	...
	deltaInfo.m_pDeltaData = CastElement<CDmeVertexDeltaData>(
		ReadOBJ(utlBuf, deltaInfo.m_pMesh->GetFileId(), pDeltaName, deltaPath, deltaInfo.m_pMesh, NULL, bAbsolute));
