...
bool CDmeClip::FindMultiTrackGroupForClip(CDmeClip *pClip, int *pTrackGroupIndex, int *pTrackIndex,
...
//-----------------------------------------------------------------------------
void CDmeClip::FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time, DmeClipSkipFlag_t flags,
...
void CDmeClip::FindClipsWithinTime(DmeClipType_t clipType, DmeTime_t startTime, DmeTime_t endTime,
								   DmeClipSkipFlag_t flags, CUtlVector<CDmeClip *> &clips) const
{
	if(clipType == DMECLIP_FILM)
		return;
...
static int BuildReferringClipList(CDmeClip *pClip, CDmeClip **ppParents, int nMaxCount)
{
	...
	for(it = g_pDataModel->FirstAttributeReferencingElement(pClip->GetHandle());
		it != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID; it = g_pDataModel->NextAttributeReferencingElement(it))
	{
		...
		for(it2 = g_pDataModel->FirstAttributeReferencingElement(pTrack->GetHandle());
			it2 != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID; it2 = g_pDataModel->NextAttributeReferencingElement(it2))
		{
			...
			for(it3 = g_pDataModel->FirstAttributeReferencingElement(pTrackGroup->GetHandle());
				it3 != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID; it3 = g_pDataModel->NextAttributeReferencingElement(it3))
			{
...
CDmeChannel *CDmeChannelsClip::CreatePassThruConnection(char const *passThruName, CDmElement *pFrom,
														char const *pFromAttribute, CDmElement *pTo,
														char const *pToAttribute, int index /*= 0*/)
{
...
// Finding clips in tracks by time
//-----------------------------------------------------------------------------
void CDmeFilmClip::FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time, DmeClipSkipFlag_t flags,
								   CUtlVector<CDmeClip *> &clips) const
{
...
void CDmeFilmClip::FindClipsWithinTime(DmeClipType_t clipType, DmeTime_t startTime, DmeTime_t endTime,
									   DmeClipSkipFlag_t flags, CUtlVector<CDmeClip *> &clips) const
{
	if((clipType == DMECLIP_FILM) || (clipType == DMECLIP_UNKNOWN))
	{
...
void CDmeFilmClip::OnElementUnserialized()
{
	...
	// I'm hoping we don't care about any files that old - if we ever hit this, we should move this code into an
	// unserialization converter
...
{
	if(m_MaterialOverlayEffect->GetMaterial()->IsTranslucent() || (m_MaterialOverlayEffect->GetAlpha() < 1.0f))
	{
		return false;
	}
...
// Rolls associated clips so they remain in the same relative time
//-----------------------------------------------------------------------------
void CDmeFilmClip::ScaleAssociatedClips(CDmeClip *pClip, CUtlVector<ClipAssociation_t> &association, float ratio,
										DmeTime_t oldOffset)
{
	int c = association.Count();
	for(int i = 0; i < c; ++i)
	{
		...
		DmeTime_t newStartTime =
			(oldStartTime - clipStartTime + oldOffset) / ratio + clipStartTime - pClip->GetTimeOffset();
		association[i].m_hClip->SetStartTime(newStartTime);
	}
}
...
//-----------------------------------------------------------------------------
CDmeChannel *FindChannelTargetingElement(CDmeChannelsClip *pChannelsClip, CDmElement *pElement,
...
CDmeChannel *FindChannelTargetingElement(CDmeFilmClip *pClip, CDmElement *pElement, const char *pAttributeName,
										 CDmeChannelsClip **ppChannelsClip, CDmeTrack **ppTrack,
