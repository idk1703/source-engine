...
CDmeGlobalFlexControllerOperator *CDmeGameModel::AddGlobalFlexController(const char *controllerName, int globalIndex)
{
	...
	if(i >= c)
	{
		CDmeGlobalFlexControllerOperator *newOperator =
			CreateElement<CDmeGlobalFlexControllerOperator>(controllerName, GetFileId());
		Assert(newOperator);
		if(!newOperator)
			return NULL;
...
void CDmeGameSprite::SetState(bool bVisible, float nFrame, int nRenderMode, int nRenderFX, float flRenderScale,
							  float flProxyRadius, const Vector &pos, const Quaternion &rot, const Color &color)
{
	m_Visible = bVisible;
	m_frame = nFrame;
