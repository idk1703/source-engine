...
//-----------------------------------------------------------------------------
static const matrix3x4_t *ComputeSkinMatrix(int nBoneCount, const float *pJointWeight, const int *pJointIndices,
											const matrix3x4_t *pPoseToWorld, matrix3x4_t &result)
{
...
public:
	...
	void ComputeVertex(int vi, const matrix3x4_t *pPoseToWorld, CDmeMesh::RenderVertexDelta_t *pDelta,
	...
	inline bool HasPositionData() const
	{
		return m_bHasPositionData;
	}
	inline bool HasNormalData() const
	{
		return m_bHasNormalData;
	}
	inline bool HasTangentData() const
	{
		return m_bHasTangentData;
	}
...
//-----------------------------------------------------------------------------
void CRenderInfo::ComputeVertex(int vi, const matrix3x4_t *pPoseToWorld, Vector *pDeltaPosition, int nDeltaStride,
								Vector *pPosition)
{
	matrix3x4_t result;
...
// Computes where a vertex is
//-----------------------------------------------------------------------------
void CRenderInfo::ComputePosition(int posIndex, const matrix3x4_t *pPoseToWorld, Vector *pDeltaPosition,
								  Vector *pPosition)
{
	matrix3x4_t result;
...
// Computes where a vertex is
//-----------------------------------------------------------------------------
void CRenderInfo::ComputeVertex(int vi, const matrix3x4_t *pPoseToWorld, CDmeMesh::RenderVertexDelta_t *pDelta,
								Vector *pPosition, Vector *pNormal, Vector4D *pTangent)
{
	matrix3x4_t result;
	Vector vecMorphPosition, vecMorphNormal;
...
//-----------------------------------------------------------------------------
template<class T>
bool CDmeMesh::AddVertexDelta(CDmeVertexData *pBaseState, void *pVertexData, int nStride,
							  CDmeVertexDataBase::StandardFields_t fieldId, int nIndex, bool bDoLag)
{
	...
	const FieldIndex_t nBaseFieldIndex =
		pBaseState->FindFieldIndex(fieldId == CDmeVertexData::FIELD_WRINKLE ? CDmeVertexData::FIELD_TEXCOORD : fieldId);
...
template<class T>
bool CDmeMesh::AddStereoVertexDelta(CDmeVertexData *pBaseState, void *pVertexData, int nStride,
									CDmeVertexDataBase::StandardFields_t fieldId, int nIndex, bool bDoLag)
{
	CDmeVertexDeltaData *pDeltaState = GetDeltaState(nIndex);
	if(!pBaseState || !pDeltaState)
...
bool CDmeMesh::BuildDeltaMesh(int nVertices, RenderVertexDelta_t *pRenderDelta)
{
	...
	if(nBalanceFieldIndex < 0)
	{
		for(int i = 0; i < nCount; ++i)
		{
			...
			AddVertexDelta<Vector>(pBindState, &pRenderDelta->m_vecDeltaPosition, sizeof(RenderVertexDelta_t),
								   CDmeVertexDeltaData::FIELD_POSITION, i, bDoLag);
			AddVertexDelta<Vector>(pBindState, &pRenderDelta->m_vecDeltaNormal, sizeof(RenderVertexDelta_t),
			...
			bool bWrinkle =
				AddVertexDelta<float>(pBindState, &pRenderDelta->m_flDeltaWrinkle, sizeof(RenderVertexDelta_t),
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		bool bWrinkle;
		CDmeVertexDeltaData *pDeltaState = GetDeltaState(i);
		AddStereoVertexDelta<Vector>(pBindState, &pRenderDelta->m_vecDeltaPosition, sizeof(RenderVertexDelta_t),
									 CDmeVertexDeltaData::FIELD_POSITION, i, bDoLag);
		AddStereoVertexDelta<Vector>(pBindState, &pRenderDelta->m_vecDeltaNormal, sizeof(RenderVertexDelta_t),
									 CDmeVertexDeltaData::FIELD_NORMAL, i, bDoLag);
		bWrinkle = AddStereoVertexDelta<float>(pBindState, &pRenderDelta->m_flDeltaWrinkle, sizeof(RenderVertexDelta_t),
											   CDmeVertexDeltaData::FIELD_WRINKLE, i, bDoLag);
		bHasWrinkleDelta = bHasWrinkleDelta || bWrinkle;
...
//-----------------------------------------------------------------------------
void CDmeMesh::WriteTriangluatedIndices(const CDmeVertexData *pBaseState, CDmeFaceSet *pFaceSet,
...
// Draws the mesh when it uses too many bones
//-----------------------------------------------------------------------------
void CDmeMesh::DrawDynamicMesh(CDmeFaceSet *pFaceSet, matrix3x4_t *pPoseToWorld, bool bHasActiveDeltaStates,
...
//-----------------------------------------------------------------------------
void CDmeMesh::DrawWireframeFaceSet(CDmeFaceSet *pFaceSet, matrix3x4_t *pPoseToWorld, bool bHasActiveDeltaStates,
									CDmeDrawSettings *pDrawSettings)
{
	...
	Vector *pDeltaVertices =
		bHasActiveDeltaStates ? pDeltaVertices = reinterpret_cast<Vector *>(alloca(nVertices * sizeof(Vector))) : NULL;
	...
	if(bHasActiveDeltaStates)
	{
		...
		if(nBalanceFieldIndex < 0)
		{
			for(int i = 0; i < nCount; ++i)
			{
				...
				AddVertexDelta<Vector>(pBind, pDeltaVertices, sizeof(Vector), CDmeVertexDeltaData::FIELD_POSITION, i,
									   bDoLag);
			}
		}
		else
		{
			for(int i = 0; i < nCount; ++i)
			{
				...
				if(flLeftWeight <= 0.0f && flRightWeight <= 0.0f &&
				   (!bDoLag || (flLeftWeightLagged <= 0.0f && flRightWeightLagged <= 0.0f)))
					continue;

				AddStereoVertexDelta<Vector>(pBind, pDeltaVertices, sizeof(Vector), CDmeVertexDeltaData::FIELD_POSITION,
											 i, bDoLag);
			}
		}
	}
...
void CDmeMesh::Draw(const matrix3x4_t &shapeToWorld, CDmeDrawSettings *pDrawSettings /* = NULL */)
{
	...
	const CDmeDrawSettings::DrawType_t drawType =
		pDrawSettings ? pDrawSettings->GetDrawType() : CDmeDrawSettings::DRAW_SMOOTH;
	...
	matrix3x4_t *pPoseToWorld =
		CDmeModel::SetupModelRenderState(shapeToWorld, pBind->HasSkinningData(), bSoftwareSkinning);
...
VertexFormat_t CDmeMesh::ComputeHwMeshVertexFormat(void)
{
	...
	VertexFormat_t vertexFormat = VERTEX_POSITION | VERTEX_COLOR | VERTEX_NORMAL | VERTEX_TEXCOORD_SIZE(0, 2) |
								  VERTEX_BONEWEIGHT(2) | VERTEX_BONE_INDEX | (bIsDX7 ? 0 : VERTEX_USERDATA_SIZE(4));

	// FIXME: set VERTEX_FORMAT_COMPRESSED if there are no artifacts and if it saves enough memory (use
	// 'mem_dumpvballocs') vertexFormat |= VERTEX_FORMAT_COMPRESSED;
...
//-----------------------------------------------------------------------------
void CDmeMesh::ComputeTriangulatedIndices(const CDmeVertexData *pBaseState, CDmeFaceSet *pFaceSet, int nFirstIndex,
...
//-----------------------------------------------------------------------------
void CDmeMesh::BuildTriangleMap(const CDmeVertexData *pBaseState, CDmeFaceSet *pFaceSet,
								CUtlVector<Triangle_t> &triangles, CUtlVector<CUtlVector<int>> *pVertToTriMap)
{
	// prepare indices
	int nFirstIndex = 0;
	int nIndexCount = pFaceSet->NumIndices();
...
// Build a map from vertex index to a list of triangles that share the vert.
//-----------------------------------------------------------------------------
void CDmeMesh::ComputeAverageTangent(CDmeVertexData *pVertexData, bool bSmoothTangents,
									 CUtlVector<CUtlVector<int>> &vertToTriMap, CUtlVector<Triangle_t> &triangles)
{
...
//-----------------------------------------------------------------------------
void CDmeMesh::BuildVertToTriMap(const CDmeVertexData *pVertexData, CUtlVector<Triangle_t> &triangles,
...
//-----------------------------------------------------------------------------
void CDmeMesh::AddDelta(CDmeVertexData *pBaseState, Vector *pDeltaPosition, int nDeltaStateIndex,
...
//-----------------------------------------------------------------------------
void CDmeMesh::ComputeNormalsFromPositions(CDmeVertexData *pBase, const Vector *pPosition,
										   const CUtlVector<Triangle_t> &triangles, int nNormalCount, Vector *pNormals)
{
...
//-----------------------------------------------------------------------------
void CDmeMesh::ComputeCorrectedNormalsFromActualNormals(const CUtlVector<int> &deltaStateList, int nNormalCount,
														Vector *pNormals)
{
	CDmeVertexData *pBind = GetBindBaseState();
	if(!pBind)
...
//
//-----------------------------------------------------------------------------
void CDmeMesh::BuildAtomicControlLists(int nCount, DeltaComputation_t *pInfo,
...
// NOTE: This assumes a naming scheme where delta state names have _ that separate control names
//-----------------------------------------------------------------------------
void CDmeMesh::SetDeltaNormalDataFromActualNormals(int nDeltaIndex, const CUtlVector<int> &deltaStateList,
...
//-----------------------------------------------------------------------------
void CDmeMesh::Combinations(int n, int k, CUtlVector<CUtlVector<int>> &combos, int *pTmpArray, int start, int currentK)
{
	if(!pTmpArray)
	{
...
//-----------------------------------------------------------------------------
bool CDmeMesh::GetControlDeltaIndices(CDmeVertexDeltaData *pDeltaState, CUtlVector<int> &controlDeltaIndices) const
{
	Assert(pDeltaState);
...
//-----------------------------------------------------------------------------
bool CDmeMesh::GetControlDeltaIndices(const char *pDeltaStateName, CUtlVector<int> &controlDeltaIndices) const
{
...
// Returns true if all of the control states exist, false otherwise
//-----------------------------------------------------------------------------
bool CDmeMesh::BuildCompleteDeltaStateControlList(CUtlVector<CUtlVector<int>> &deltaStateControlList) const
{
	...
	for(int i = 0; i < nDeltas; ++i)
	{
		...
		deltaStateControlList[deltaStateControlList.AddToTail()].CopyArray(tmpControlDeltaIndices.Base(),
																		   tmpControlDeltaIndices.Count());
	}
...
//-----------------------------------------------------------------------------
int CDmeMesh::FindDeltaIndexFromControlIndices(const CUtlVector<int> &controlIndices,
...
// Returns true if all of the control states exist, false otherwise
//-----------------------------------------------------------------------------
bool CDmeMesh::BuildMissingDependentDeltaList(CDmeVertexDeltaData *pDeltaState, CUtlVector<int> &controlIndices,
...
//-----------------------------------------------------------------------------
template<class T_t>
int CDmeMesh::GenerateCompleteDataForDelta(const CDmeVertexDeltaData *pDelta, T_t *pFullData, int nFullData,
...
//-----------------------------------------------------------------------------
template<class T_t>
void CDmeMesh::AddDelta(const CDmeVertexDeltaData *pDelta, T_t *pFullData, int nFullData, FieldIndex_t fieldIndex,
...
template void CDmeMesh::AddDelta<float>(const CDmeVertexDeltaData *, float *, int, FieldIndex_t, float,
										const CDmeSingleIndexedComponent *);
template void CDmeMesh::AddDelta<Vector2D>(const CDmeVertexDeltaData *, Vector2D *, int, FieldIndex_t, float,
										   const CDmeSingleIndexedComponent *);
template void CDmeMesh::AddDelta<Vector>(const CDmeVertexDeltaData *, Vector *, int, FieldIndex_t, float,
										 const CDmeSingleIndexedComponent *);
...
//-----------------------------------------------------------------------------
template<class T_t>
void CDmeMesh::AddDelta(const CDmeVertexDeltaData *pDelta, T_t *pFullData, int nFullData,
						CDmeVertexData::StandardFields_t standardField, float weight,
...
template void CDmeMesh::AddDelta<Vector2D>(const CDmeVertexDeltaData *, Vector2D *, int,
										   CDmeVertexData::StandardFields_t, float, const CDmeSingleIndexedComponent *);
template void CDmeMesh::AddDelta<Vector>(const CDmeVertexDeltaData *, Vector *, int, CDmeVertexData::StandardFields_t,
										 float, const CDmeSingleIndexedComponent *);
...
//-----------------------------------------------------------------------------
template<class T_t>
void CDmeMesh::AddCorrectedDelta(CDmrArray<T_t> &baseDataArray, const CUtlVector<int> &baseIndices,
								 const DeltaComputation_t &deltaComputation, const char *pFieldName, float weight,
								 const CDmeSingleIndexedComponent *pMask)
{
...
//-----------------------------------------------------------------------------
template<class T_t>
void CDmeMesh::AddCorrectedDelta(CUtlVector<T_t> &baseData, const CUtlVector<int> &baseIndices,
								 const DeltaComputation_t &deltaComputation, const char *pFieldName, float weight,
								 const CDmeSingleIndexedComponent *pMask)
{
	const int nData(baseData.Count());
...
//-----------------------------------------------------------------------------
template<class T_t>
void CDmeMesh::AddRawDelta(CDmeVertexDeltaData *pDelta, CDmrArray<T_t> &baseDataArray, FieldIndex_t nDeltaFieldIndex,
						   float weight, const CDmeSingleIndexedComponent *pMask)
{
...
//-----------------------------------------------------------------------------
template<class T_t>
void CDmeMesh::AddRawDelta(CDmeVertexDeltaData *pDelta, CUtlVector<T_t> &baseData, FieldIndex_t nDeltaFieldIndex,
						   float weight, const CDmeSingleIndexedComponent *pMask)
{
	if(!pDelta || nDeltaFieldIndex < 0)
		return;
...
//
//-----------------------------------------------------------------------------
void CDmeMesh::SelectVerticesFromDelta(CDmeVertexDeltaData *pDelta, CDmeSingleIndexedComponent *pSelection)
{
	if(!pSelection)
		return;
...
//
//-----------------------------------------------------------------------------
void CDmeMesh::SelectHalfVertices(SelectHalfType_t selectHalfType, CDmeSingleIndexedComponent *pSelection,
...
//
//-----------------------------------------------------------------------------
bool CDmeMesh::CreateDeltaFieldFromBaseField(CDmeVertexData::StandardFields_t nStandardFieldIndex,
...
//-----------------------------------------------------------------------------
CDmeVertexDeltaData *CDmeMesh::ModifyOrCreateDeltaStateFromBaseState(const char *pDeltaName,
																	 CDmeVertexData *pPassedBase /* = NULL */,
																	 bool absolute /* = false */)
{
	...
	CDmeVertexData::StandardFields_t deltaFields[] = {CDmeVertexData::FIELD_POSITION, CDmeVertexData::FIELD_NORMAL,
	...
	for(int i = 0; i < sizeof(deltaFields) / sizeof(deltaFields[0]); ++i)
	{
		...
		{
			case AT_FLOAT_ARRAY:
				CreateDeltaFieldFromBaseField(standardFieldIndex, CDmrArrayConst<float>(pBaseData),
											  CDmrArrayConst<float>(pBindData), pDelta);
				break;
			case AT_COLOR_ARRAY:
				CreateDeltaFieldFromBaseField(standardFieldIndex, CDmrArrayConst<Vector>(pBaseData),
											  CDmrArrayConst<Vector>(pBindData), pDelta);
				break;
			case AT_VECTOR2_ARRAY:
				CreateDeltaFieldFromBaseField(standardFieldIndex, CDmrArrayConst<Vector2D>(pBaseData),
...
CDmeSingleIndexedComponent *CDmeMesh::FeatherSelection(float falloffDistance, Falloff_t falloffType,
													   Distance_t distanceType, CDmeSingleIndexedComponent *pSelection,
...
//-----------------------------------------------------------------------------
template<int T>
CDmeSingleIndexedComponent *CDmeMesh::FeatherSelection(float fDistance, Distance_t distanceType,
													   ...
													   CDmMeshComp *pPassedMeshComp)
{
	...
	CDmeSingleIndexedComponent *pNewSelection =
		CreateElement<CDmeSingleIndexedComponent>("feather", pSelection->GetFileId());
...
//-----------------------------------------------------------------------------
bool CDmeMesh::AddMaskedDelta(CDmeVertexDeltaData *pDelta, CDmeVertexData *pDst /* = NULL */, float weight /* = 1.0f */,
...
// Only the delta specified is added.  No dependent states are added.
//-----------------------------------------------------------------------------
bool CDmeMesh::AddCorrectedMaskedDelta(CDmeVertexDeltaData *pDelta, CDmeVertexData *pDst /* = NULL */,
									   float weight /* = 1.0f */, const CDmeSingleIndexedComponent *pMask /* = NULL */)
{
	CDmeVertexData *pBase = pDst ? pDst : GetCurrentBaseState();
	...
	for(int i = 0; i < nDeltas; ++i)
	{
		...
		for(int j(0); j < nBaseField; ++j)
		{
			...
			for(int k(0); k < nDeltaField; ++k)
			{
				...
				{
					case AT_FLOAT_ARRAY:
						AddCorrectedDelta(CDmrArray<float>(pBaseData), baseIndices, compList[i], baseFieldName, weight,
										  pMask);
						break;
					case AT_COLOR_ARRAY:
						AddCorrectedDelta(CDmrArray<Vector>(pBaseData), baseIndices, compList[i], baseFieldName, weight,
										  pMask);
						break;
					case AT_VECTOR2_ARRAY:
						AddCorrectedDelta(CDmrArray<Vector2D>(pBaseData), baseIndices, compList[i], baseFieldName,
...
//-----------------------------------------------------------------------------
template<class T_t>
bool CDmeMesh::InterpMaskedData(CDmrArray<T_t> &aData, const CUtlVector<T_t> &bData, float weight,
...
//-----------------------------------------------------------------------------
bool CDmeMesh::InterpMaskedData(CDmeVertexData *paData, const CDmeVertexData *pbData, float weight,
								const CDmeSingleIndexedComponent *pMask) const
{
		...
		for(int j = 0; j < nbField; ++j)
		{
			...
			switch(paAttr->GetType())
			{
				case AT_FLOAT_ARRAY:
					...
					break;
				case AT_COLOR_ARRAY:
					InterpMaskedData(CDmrArray<Vector4D>(paAttr), CDmrArrayConst<Vector4D>(pbAttr).Get(), weight,
									 pMask);
					break;
				case AT_VECTOR2_ARRAY:
					InterpMaskedData(CDmrArray<Vector2D>(paAttr), CDmrArrayConst<Vector2D>(pbAttr).Get(), weight,
									 pMask);
					break;
				case AT_VECTOR3_ARRAY:
...
//-----------------------------------------------------------------------------
bool CDmeMesh::InterpMaskedDelta(CDmeVertexDeltaData *pDelta, CDmeVertexData *pDst /* = NULL */,
								 float weight /*= 1.0f */, const CDmeSingleIndexedComponent *pMask /*= NULL */)
{
...
// -1 on failure
//-----------------------------------------------------------------------------
int CDmeMesh::ClosestSelectedVertex(int vIndex, CDmeSingleIndexedComponent *pSelection,
...
//-----------------------------------------------------------------------------
void CDmeMesh::GetBoundingSphere(Vector &c, float &r, CDmeVertexData *pPassedBase /* = NULL */,
...
//
//-----------------------------------------------------------------------------
void CDmeMesh::GetBoundingBox(Vector &min, Vector &max, CDmeVertexData *pPassedBase /* = NULL */,
							  CDmeSingleIndexedComponent *pPassedSelection /* = NULL */) const
{
	min.Zero();
	max.Zero();
...
//-----------------------------------------------------------------------------
template<class T_t>
bool CDmeMesh::SetBaseDataToDeltas(CDmeVertexData *pBase, CDmeVertexData::StandardFields_t nStandardField,
								   CDmrArrayConst<T_t> &srcData, CDmrArray<T_t> &dstData, bool bDoStereo, bool bDoLag)
{
...
bool CDmeMesh::SetBaseStateToDeltas(CDmeVertexData *pPassedBase /*= NULL */)
{
	...
	for(int i = 0; i < sizeof(deltaFields) / sizeof(deltaFields[0]); ++i)
	{
		...
		{
			case AT_FLOAT_ARRAY:
				SetBaseDataToDeltas(pBind, nStandardField, CDmrArrayConst<float>(pSrcAttr), CDmrArray<float>(pDstAttr),
									bDoStereo, false);
				break;
			case AT_VECTOR3_ARRAY:
				SetBaseDataToDeltas(pBind, nStandardField, CDmrArrayConst<Vector>(pSrcAttr),
...
// return false.
//-----------------------------------------------------------------------------
static bool CollapseRedundantBaseNormals(CDmeVertexData *pDmeVertexData, CUtlVector<int> &normalMap,
										 float flNormalBlend)
{
	if(!pDmeVertexData)
		return false;
	...
	for(int i = 0; i < nPositionDataCount; ++i)
	{
		...
		const CUtlVector<int> &vertexIndices =
			pDmeVertexData->FindVertexIndicesFromDataIndex(CDmeVertexData::FIELD_POSITION, i);
