...
//
//-----------------------------------------------------------------------------
void CDmeSingleIndexedComponent::AddComponents(const CUtlVector<int> &components, const CUtlVector<float> &weights)
{
...
void CDmeSingleIndexedComponent::Add(const CDmeSingleIndexedComponent &rhs)
{
	...
	{
		CUtlVector<int> newComponents;
		newComponents.EnsureCapacity(
			nLhs + nRhs * 0.5); // Just an estimate assuming 50% of the components in rhs aren't in lhs
		CUtlVector<float> newWeights;
