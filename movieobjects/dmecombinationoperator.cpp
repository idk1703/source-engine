...
// Finds the index of the input control with the specified name (and creates one if necessary)
//-----------------------------------------------------------------------------
ControlIndex_t CDmeCombinationOperator::FindOrCreateControl(const char *pControlName, bool bStereo,
															bool bAutoAddRawControl)
{
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetWrinkleScale(ControlIndex_t nControl, const char *pRawControlName,
											  float flWrinkleScale)
{
	CDmeCombinationInputControl *pInputControl = m_InputControls[nControl];
	Assert(pInputControl);
...
void CDmeCombinationOperator::MoveControlBefore(const char *pDragControlName, const char *pDropControlName)
{
	...
	// Have to copy because InsertAfter may reallocate memory before doing the copy and therefore might be referencing
	// garabage
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetControlValue(ControlIndex_t nControlIndex, float flValue,
...
void CDmeCombinationOperator::SetControlValue(ControlIndex_t nControlIndex, float flLevel, float flBalance,
											  CombinationControlType_t type)
{
...
void CDmeCombinationOperator::SetControlValue(ControlIndex_t nControlIndex, const Vector2D &vec,
											  CombinationControlType_t type)
{
	Assert(IsStereoControl(nControlIndex));
	m_IsDefaultValue[nControlIndex] = false;
...
// Sets the value of a control
//-----------------------------------------------------------------------------
const Vector2D &CDmeCombinationOperator::GetStereoControlValue(ControlIndex_t nControlIndex,
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::SetMultiControlLevel(ControlIndex_t nControlIndex, float flMultiLevel,
												   CombinationControlType_t type)
{
	m_IsDefaultValue[nControlIndex] = false;
...
//-----------------------------------------------------------------------------
CDmeCombinationOperator::RawControlIndex_t CDmeCombinationOperator::FindRawControlIndex(
	const char *pControlName, bool bIgnoreDefaultControls) const
{
...
// Adds a dominator. Dominators are specified using raw control names
//-----------------------------------------------------------------------------
CDmeCombinationDominationRule *CDmeCombinationOperator::AddDominationRule(int nDominatorCount,
																		  const char **ppDominatorControlNames,
																		  int nSuppressedCount,
																		  const char **ppSuppressedControlNames)
{
...
// Adds a dominator. Dominators are specified using raw control names
//-----------------------------------------------------------------------------
CDmeCombinationDominationRule *CDmeCombinationOperator::AddDominationRule(const CUtlVector<const char *> dominators,
																		  const CUtlVector<const char *> suppressed)
{
	return AddDominationRule(dominators.Count(), (const char **)dominators.Base(), suppressed.Count(),
							 (const char **)suppressed.Base());
}
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::AttachChannelToControlValue(ControlIndex_t nControlIndex, CombinationControlType_t type,
...
void CDmeCombinationOperator::ComputeCombinationInfo(int nIndex)
{
	...
	info.m_hDestAttribute[COMBO_CONTROL_LAGGED] =
		weightsLagged.IsValid() ? weightsLagged.GetAttribute()->GetHandle() : DMATTRIBUTE_HANDLE_INVALID;
...
//-----------------------------------------------------------------------------
void CDmeCombinationOperator::ComputeInternalControlValue(RawControlIndex_t nRawControlIndex,
														  CombinationControlType_t type, Vector2D &value)
{
...
void CDmeCombinationOperator::ComputeLaggedInputValues()
{
	...
	{
		Vector vecLerp;
		VectorLerp(m_ControlValues[COMBO_CONTROL_NORMAL][i], m_ControlValues[COMBO_CONTROL_LAGGED][i], flFactor,
				   vecLerp);
...
void CDmeCombinationOperator::Operate()
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		for(CombinationControlType_t type = COMBO_CONTROL_FIRST; type < COMBO_CONTROL_TYPE_COUNT;
			type = (CombinationControlType_t)(type + 1))
		{
			if((info.m_hDestAttribute[type] == DMATTRIBUTE_HANDLE_INVALID) ||
			   !g_pDataModel->IsAttributeHandleValid(info.m_hDestAttribute[type]))
				continue;
			...
			for(int j = 0; j < nOutputCount; ++j)
			{
				...
				{
					const CUtlVector<int> &dominantIndices =
						m_DominatorInfo[op.m_DominatorIndices[k]].m_DominantIndices;
...
void CDmeCombinationOperator::GetOutputAttributes(CUtlVector<CDmAttribute *> &attrs)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		{
			if((info.m_hDestAttribute[j] == DMATTRIBUTE_HANDLE_INVALID) ||
			   !g_pDataModel->IsAttributeHandleValid(info.m_hDestAttribute[j]))
				continue;
...
const CUtlVector<int> &CDmeCombinationOperator::GetOperationDominator(int nTargetIndex, int nOpIndex,
...
void CDmeCombinationOperator::CopyControls(CDmeCombinationOperator *pSrc)
{
	...
	m_ControlValues[COMBO_CONTROL_NORMAL].CopyArray(pSrc->m_ControlValues[COMBO_CONTROL_NORMAL].Base(),
													pSrc->m_ControlValues[COMBO_CONTROL_NORMAL].Count());

	m_ControlValues[COMBO_CONTROL_LAGGED].CopyArray(pSrc->m_ControlValues[COMBO_CONTROL_LAGGED].Base(),
