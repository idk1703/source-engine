...
void CDmeTrack::OnAttributeChanged(CDmAttribute *pAttribute)
{
	...
	// At some point, we'll probably have to start sending more data through OnAttributeChanged, (like an event string
	// or chain path) or perhaps add a new callback OnElementChanged() with this data
...
void CDmeTrack::FindClipsIntersectingTime(DmeTime_t startTime, DmeTime_t endTime, DmeClipSkipFlag_t flags,
...
void CDmeTrack::FindClipsWithinTime(DmeTime_t startTime, DmeTime_t endTime, DmeClipSkipFlag_t flags,
									CUtlVector<CDmeClip *> &clips) const
{
	if((flags & DMESKIP_INVISIBLE) && IsCollapsed())
		return;
