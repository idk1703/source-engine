...
LayerSelectionData_t::DataLayer_t::DataLayer_t(float frac, CDmeLogLayer *layer) : m_flStartFraction(frac)
{
	m_hData = layer;
}

LayerSelectionData_t::LayerSelectionData_t() : m_DataType(AT_UNKNOWN), m_nDuration(0), m_tStartOffset(DMETIME_ZERO)
{
...
{
	return (type == AT_FLOAT) || (type == AT_COLOR) || (type == AT_VECTOR2) || (type == AT_VECTOR3) ||
		   (type == AT_QANGLE) || (type == AT_QUATERNION);
}

static Vector s_pInterolationPoints[4] = {Vector(0.0f, 0.0f, 0.0f), Vector(0.0f, 0.0f, 0.0f), Vector(1.0f, 1.0f, 0.0f),
...
static inline float ComputeInterpolationFactor(float flFactor, int nInterpolatorType)
{
	...
	Interpolator_CurveInterpolate(nInterpolatorType,
								  ...
								  s_pInterolationPoints[1], s_pInterolationPoints[2],
...
T Average(const T *pValues, int nValues)
{
	if(IsInterpolableType(CDmAttributeInfo<T>::AttributeType()))
	{
		...
		{
			first = false;
			Warning("CDmeLog: interpolable type %s doesn't have an averaging function!",
...
T Interpolate(float t, const T &ti, const T &tj)
{
	if(IsInterpolableType(CDmAttributeInfo<T>::AttributeType()))
	{
		...
		{
			first = false;
			Warning("CDmeLog: interpolable type %s doesn't have an interpolation function!",
					CDmAttributeInfo<T>::AttributeTypeName());
		}
	}
...
Color Interpolate(float t, const Color &ti, const Color &tj)
{
	...
	return Color(t * rj + (1.0f - t) * ri, t * gj + (1.0f - t) * gi, t * bj + (1.0f - t) * bi,
				 t * aj + (1.0f - t) * ai);
}
...
Vector Curve_Interpolate(float t, DmeTime_t times[4], const Vector values[4], int curveTypes[4], float fmin, float fmax)
{
	...
	else
	{
		...
		else // curves differ, sigh
		{
			...
			Interpolator_CurveInterpolate_NonNormalized(earlypart, values[0], values[1], values[2], values[3], t,
														vOut1);
			Interpolator_CurveInterpolate_NonNormalized(laterpart, values[0], values[1], values[2], values[3], t,
														vOut2);
...
// Quaternion version
template<>
Quaternion Curve_Interpolate(float t, DmeTime_t times[4], const Quaternion values[4], int curveTypes[4], float fmin,
							 float fmax)
{
	Quaternion vOut;
	int dummy;
...
{
	return (float)sqrt(
		(float)(value.r() * value.r() + value.g() * value.g() + value.b() * value.b() + value.a() * value.a()));
}
template<>
...
public:
	CUndoLayerAdded(const char *desc, CDmeLog *pLog) : BaseClass(desc), m_bNeedsCleanup(false), m_hLog(pLog)
	{
	...
	virtual const char *GetDesc()
	{
		...
		if(iLayer >= 0)
		{
			...
			Q_snprintf(sz, sizeof(sz), "addlayer: log %p lc[%d], layer %p", m_hLog.Get(), m_hLog->GetNumLayers(),
					   layer);
		}
		else
		{
			Q_snprintf(sz, sizeof(sz), "addlayer: log %p lc[%d], layer NULL", m_hLog.Get(), m_hLog->GetNumLayers());
		}
		return sz;
	}
...
public:
	CUndoFlattenLayers(const char *desc, CDmeTypedLog<T> *pLog, float threshold, int flags)
		: BaseClass(desc), m_bNeedsCleanup(true), m_hLog(pLog), m_nFlags(flags), m_flThreshold(threshold)
	{
		Assert(pLog && pLog->GetFileId() != DMFILEID_INVALID);
		LatchCurrentLayers();
	}
	...
	{
		static char sz[512];
		Q_snprintf(sz, sizeof(sz), "flatten log %p lc[%d]", m_hLog.Get(), m_hLayers.Count());
		return sz;
	}
...
void CDmeTypedLogLayer<T>::SetKey(DmeTime_t time, const T &value, int curveType /*=CURVE_DEFAULT*/)
{
	...
	// If adding the new means that the penultimate key's value was unneeded, then we will remove the penultimate key
	// value
...
//-----------------------------------------------------------------------------
template<class T>
int CDmeTypedLogLayer<T>::FindOrAddKey(DmeTime_t nTime, DmeTime_t nTolerance, const T &value,
...
const T &CDmeTypedLogLayer<T>::GetValue(DmeTime_t time) const
{
	...
	if(ti < 0)
	{
		...
		CDmAttributeInfo<T>::SetDefaultValue(
			s_value); // TODO - create GetDefaultValue that returns a default T, to avoid rebuilding every time
		return s_value;
	}
...
template<class T>
void CDmeTypedLogLayer<T>::SetKey(DmeTime_t time, const CDmAttribute *pAttr, uint index,
								  int curveType /*= CURVE_DEFAULT*/)
{
...
// Implementation of Douglas-Peucker curve simplification routine (hacked to only care about error against original
// curve (sort of 1D)
...
template<>
void CDmeTypedLogLayer<int>::CurveSimplify_R(float thresholdSqr, int startPoint, int endPoint,
											 CDmeTypedLogLayer<int> *output) {};
template<>
void CDmeTypedLogLayer<Color>::CurveSimplify_R(float thresholdSqr, int startPoint, int endPoint,
											   CDmeTypedLogLayer<Color> *output) {};
template<>
void CDmeTypedLogLayer<Quaternion>::CurveSimplify_R(float thresholdSqr, int startPoint, int endPoint,
													CDmeTypedLogLayer<Quaternion> *output) {};
template<>
void CDmeTypedLogLayer<VMatrix>::CurveSimplify_R(float thresholdSqr, int startPoint, int endPoint,
												 CDmeTypedLogLayer<VMatrix> *output) {};

// We can't just walk the keys linearly since it'll accumulate too much error and give us a bad curve after
// simplification.  We do a recursive subdivide which has a worst case of O(n^2) but
	...
	CDmeTypedLogLayer<T> *save = 0;
	{
		...
		else
		{
			if(GetDataType() == AT_FLOAT)
			{
				...
				// Recursively finds the point with the largest error from the "simplified curve" and subdivides the
				// problem on both sides until the largest delta from the simplified
			...
			else
			{
				...
				// copy over keys that differ from their prior or next keys - this keeps the first and last key of a run
				// of same-valued keys
	...
	if(!IsUsingCurveTypes())
	{
		if(ti < 0)
		{
			CDmAttributeInfo<float>::SetDefaultValue(
				out); // TODO - create GetDefaultValue that returns a default T, to avoid rebuilding every time
			return;
		}
...
template<class T>
void CDmeTypedLogLayer<T>::ExplodeLayer(const CDmeLogLayer *src, DmeTime_t startTime, DmeTime_t endTime,
										bool bRebaseTimestamps, DmeTime_t tResampleInterval)
{
	const CDmeTypedLogLayer<T> *pSrc = static_cast<const CDmeTypedLogLayer<T> *>(src);
	Assert(pSrc);
...
template<class T>
void CDmeTypedLogLayer<T>::CopyPartialLayer(const CDmeLogLayer *src, DmeTime_t startTime, DmeTime_t endTime,
...
{
	ActiveLayer_t() : bValid(false), priority(0), firstTime(0), lastTime(0), layer(NULL) {}
...
static void SpewEvents(CUtlRBTree<LayerEvent_t<T>> &events)
{
	for(unsigned short idx = events.FirstInorder(); idx != events.InvalidIndex(); idx = events.NextInorder(idx))
	{
		...
		Msg("Event %u layer %i at time %i type %s\n", (unsigned)idx, pEvent->m_nLayer, pEvent->m_Time.GetTenthsOfMS(),
...
{
	Msg("Layer %d:  adding key at time %f [%d -> %d], value ", pActiveLayer->priority, t.GetSeconds(),
		pActiveLayer->firstTime.GetTenthsOfMS(), pActiveLayer->lastTime.GetTenthsOfMS());
	SpewKey(val);
	Msg("\n");
}
							   ...
							   CUtlRBTree<LayerEvent_t<T>> &events)
{
	...
	for(int i = 0; i < pLog->GetNumLayers(); ++i)
	{
		...
		if((layer.firstTime == DMETIME_MINTIME || layer.lastTime == DMETIME_MAXTIME) &&
		...
		DmeTime_t tNeighbor =
			(layer.firstTime != DMETIME_MINTIME) ? (layer.firstTime - DMETIME_MINDELTA) : DMETIME_MINTIME;
...
template<class T>
static void AddDiscontinitySample(CDmeTypedLogLayer<T> *pTargetLayer, CDmeTypedLog<T> *pLog, DmeTime_t tKeyTime,
...
template<class T>
static DmeTime_t ProcessStartLayerStartEvent(bool bSpew, bool bFixupDiscontinuities, CDmeTypedLog<T> *pLog,
											 ...
											 CDmeTypedLogLayer<T> *flattenedlayer)
{
	...
	AddDiscontinitySample(flattenedlayer, pLog, pEvent->m_Time,
						  GetActiveLayerValue(layerlist, pEvent->m_Time, pEvent->m_nLayer), bSpew ? "start" : NULL);
	return pEvent->m_Time;
}

template<class T>
static DmeTime_t ProcessStartLayerEndEvent(bool bSpew, bool bFixupDiscontinuities, CDmeTypedLog<T> *pLog,
										   LayerEvent_t<T> *pEvent, CUtlVector<ActiveLayer_t<T>> &layerlist,
										   CUtlRBTree<ActiveLayer_t<T> *> &active, CDmeTypedLogLayer<T> *pBaseLayer)
{
	...
	{
		AddDiscontinitySample(pBaseLayer, pLog, pEvent->m_Time,
							  GetActiveLayerValue(layerlist, pEvent->m_Time, pEvent->m_nLayer), bSpew ? "end" : NULL);
		if(active.Count() >= 2)
		{
...
void CDmeTypedLog<T>::FlattenLayers(float threshold, int flags)
{
	...
	// NOTE:  UNDO IS DISABLED FOR THE REST OF THIS OPERATION (the above function does what we need to preserve the
	// layers)
	CDisableUndoScopeGuard guard;
	...
	// Stack of active events, sorted by event "priority", which means last item is the one writing data into the new
	// base layer
	...
	// Now walk from the earliest time in any layer until the latest time, going key by key and checking if the active
	// layer should change as we go
	...
	while(1)
	{
		...
		switch(pEvent->m_Type)
		{
			...
			case LayerEvent_t<T>::LE_START:
				iCurrentKeyTime = ProcessStartLayerStartEvent(bSpew, bFixupDiscontinuities, this, pEvent, layerlist,
															  active, flattenedlayer);
				break;

			case LayerEvent_t<T>::LE_END:
				iCurrentKeyTime = ProcessStartLayerEndEvent(bSpew, bFixupDiscontinuities, this, pEvent, layerlist,
															active, flattenedlayer);
				break;
		}
		...
		// Splat all keys betweeen the current head position and the next event time (layerFinishTime) into the
		// flattened layer
		...
		for(int j = 0; j < keyCount; ++j)
		{
			...
			// Because it's a key, the interpolated value should == the actual value (not true for certain 4 point curve
			// types, but we shouldn't support them
			...
			flattenedlayer->SetKey(iCurrentKeyTime, val,
								   loglayer->IsUsingCurveTypes() ? loglayer->GetKeyCurveType(j) : CURVE_DEFAULT);
		}
		idx = nNextIndex;
	}
...
template<class T>
void CDmeTypedLog<T>::StampKeyAtHead(DmeTime_t tHeadPosition, DmeTime_t tPreviousHeadPosition,
									 const DmeLog_TimeSelection_t &params, const CDmAttribute *pAttr,
									 uint index /*= 0*/)
{
	DmAttributeType_t type = pAttr->GetType();
	if(IsValueType(type))
...
); template<> void CDmeTypedLog< QAngle >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t& params, const QAngle&
value );
*/
...
{
public:
	void Init(CDmeTypedLog<T> *pLog, DmeTime_t tFalloff, DmeTime_t tHold, bool bLeftFalloff, int nInterpolatorType,
			  const T &delta);
	...
	void StampKey(CDmeTypedLogLayer<T> *pWriteLayer, DmeTime_t t, const CDmeTypedLogLayer<T> *pReadLayer,
				  float flIntensity, LogClampHelper_t<T> &helper, bool bSpew, const T *pInterpTarget);
	void UpdateClampHelper(DmeTime_t t, const CDmeTypedLogLayer<T> *pReadLayer, float flIntensity,
						   LogClampHelper_t<T> &helper, const T *pInterpTarget);

private:
	void ComputeDelta(CDmeTypedLog<T> *pLog, const T &delta, const T &holdValue);
	void InsertClampTransitionPoints(CDmeTypedLogLayer<T> *pWriteLayer, DmeTime_t t, LogClampHelper_t<T> &clampHelper,
...
template<class T>
void CLogFalloffBlend<T>::Init(CDmeTypedLog<T> *pLog, DmeTime_t tFalloffTime, DmeTime_t tHoldTime, bool bLeftFalloff,
...
//-----------------------------------------------------------------------------
template<class T>
void CLogFalloffBlend<T>::InsertClampTransitionPoints(CDmeTypedLogLayer<T> *pWriteLayer, DmeTime_t t,
...
template<>
void CLogFalloffBlend<float>::InsertClampTransitionPoints(CDmeTypedLogLayer<float> *pWriteLayer, DmeTime_t t,
														  LogClampHelper_t<float> &clampHelper, const float &val,
														  bool bSpew)
{
...
void CLogFalloffBlend<T>::StampKey(CDmeTypedLogLayer<T> *pWriteLayer, DmeTime_t t,
								   ...
								   LogClampHelper_t<T> &clampHelper, bool bSpew, const T *pInterpTarget)
{
...
//-----------------------------------------------------------------------------
template<class T>
void CLogFalloffBlend<T>::UpdateClampHelper(DmeTime_t t, const CDmeTypedLogLayer<T> *pReadLayer, float flIntensity,
...
//-----------------------------------------------------------------------------
template<class T>
void CDmeTypedLog<T>::_StampKeyAtHeadResample(DmeTime_t tHeadPosition, const DmeLog_TimeSelection_t &params,
											  const T &value, bool bSkipToHead, bool bClearPreviousKeys)
{
	...
	bool bUsePresetValue = bUsePresetRules && params.m_pPresetValue &&
	...
	blend[0].Init(this, params.m_nTimes[TS_FALLOFF(0)], params.m_nTimes[TS_HOLD(0)], true,
				  params.m_nFalloffInterpolatorTypes[0], headDelta);
	blend[1].Init(this, headDelta);
	blend[2].Init(this, params.m_nTimes[TS_FALLOFF(1)], params.m_nTimes[TS_HOLD(1)], false,
	...
	DmeTime_t tNextSampleTime =
		nLayerSampleIndex >= pReadLayer->GetKeyCount() ? tLayerEndTime : pReadLayer->GetKeyTime(nLayerSampleIndex);
	...
	while(nNextTransition < TS_TIME_COUNT)
	{
		...
		{
			blend[nNextTransition - 1].StampKey(pWriteLayer, tCurrent, pReadLayer, params.m_flIntensity, clampHelper,
												bSpew, pInterpTarget);
		}
		...
		if(tCurrent == tTransitionTimes[nNextTransition])
		{
			...
			{
				blend[nNextTransition].UpdateClampHelper(tCurrent, pReadLayer, params.m_flIntensity, clampHelper,
														 pInterpTarget);
			}
...
//-----------------------------------------------------------------------------
template<class T>
void CDmeTypedLog<T>::_StampKeyFilteredByTimeSelection(CDmeTypedLogLayer<T> *pWriteLayer, DmeTime_t t,
													   const DmeLog_TimeSelection_t &params, const T &value,
													   bool bForce)
{
...
//-----------------------------------------------------------------------------
template<class T>
void CDmeTypedLog<T>::_StampKeyAtHeadFilteredByTimeSelection(DmeTime_t tHeadPosition, DmeTime_t tPreviousHeadPosition,
															 const DmeLog_TimeSelection_t &params, const T &value)
{
	// Should be in "layer recording" mode!!!
	Assert(GetNumLayers() >= 2);
...
//-----------------------------------------------------------------------------
template<class T>
int CDmeTypedLog<T>::FindOrAddKey(DmeTime_t nTime, DmeTime_t nTolerance, const T &value,
								  int curveType /*=CURVE_DEFAULT*/)
{
	int bestLayer = GetTopmostLayer();
	if(bestLayer == -1)
...
void CDmeTypedLog<T>::GetValue(DmeTime_t time, CDmAttribute *pAttr, uint index) const
{
	...
	if(bestLayer < 0)
	{
		T value;
		CDmAttributeInfo<T>::SetDefaultValue(
			value); // TODO - create GetDefaultValue that returns a default T, to avoid rebuilding every time
...
template<class T>
void CDmeTypedLog<T>::BlendTimesUsingTimeSelection(const CDmeLogLayer *firstLayer, const CDmeLogLayer *secondLayer,
...
template<class T>
void CDmeTypedLog<T>::BlendLayersUsingTimeSelection(const CDmeLogLayer *firstLayer, const CDmeLogLayer *secondLayer,
													CDmeLogLayer *outputLayer, const DmeLog_TimeSelection_t &params,
...
void CDmeTypedLog<T>::RevealUsingTimeSelection(const DmeLog_TimeSelection_t &params, CDmeLogLayer *savedLayer)
{
	...
	for(int t = params.m_nTimes[TS_LEFT_FALLOFF].GetTenthsOfMS();
		t < params.m_nTimes[TS_RIGHT_FALLOFF].GetTenthsOfMS() + resample.GetTenthsOfMS(); t += resample.GetTenthsOfMS())
	{
...
template<>
void RandomValue(const Vector &average, const Vector &oldValue, Vector &newValue)
{
	newValue = oldValue;
...
template<>
void RandomValue(const Quaternion &average, const Quaternion &oldValue, Quaternion &newValue)
{
	newValue = oldValue;
...
template<>
void RandomValue(const Vector4D &average, const Vector4D &oldValue, Vector4D &newValue)
{
	newValue = oldValue;
...
template<>
void RandomValue(const Vector2D &average, const Vector2D &oldValue, Vector2D &newValue)
{
	newValue = oldValue;
...
// Builds a layer with samples matching the times in reference layer, from the data in pDataLayer, putting the resulting
// keys into pOutputLayer
template<class T>
void CDmeTypedLog<T>::BuildCorrespondingLayer(const CDmeLogLayer *pReferenceLayer, const CDmeLogLayer *pDataLayer,
...
template<class T>
void CDmeTypedLog<T>::StaggerUsingTimeSelection(const DmeLog_TimeSelection_t &params, DmeTime_t tStaggerAmount,
												const CDmeLogLayer *pBaseLayer, CDmeLogLayer *pWriteLayer)
{
	...
	for(int i = 0; i < kc; ++i)
	{
		...
		{
			curtime =
				params.m_nTimes[TS_RIGHT_FALLOFF] - (params.m_nTimes[TS_RIGHT_FALLOFF] - curtime) * flScaleFactor[1];
		}
		else
		{
...
template<class T>
void CDmeTypedLog<T>::FilterUsingTimeSelection(IUniformRandomStream *random, const DmeLog_TimeSelection_t &params,
...
template<class T>
void CDmeTypedLog<T>::FilterUsingTimeSelection(IUniformRandomStream *random, float flScale,
		...
		case FILTER_SMOOTH:
		{
			...
			if(bResample)
			{
				for(t = params.m_nTimes[TS_LEFT_FALLOFF].GetTenthsOfMS();
					t < params.m_nTimes[TS_RIGHT_FALLOFF].GetTenthsOfMS() + resample.GetTenthsOfMS();
			...
			else
			{
				...
				for(int i = 0; i < kc; ++i)
				{
					...
					T oldValue = baseLayer->GetKeyValue(i);
					writeLayer->InsertKey(curtime, oldValue,
			...
			if(bApplyFalloff)
			{
				if(bResample)
				{
						...
						t += resample.GetTenthsOfMS())
					{
						...
						// Overwrite key
						writeLayer->InsertKey(curtime, newValue,
											  IsUsingCurveTypes() ? GetDefaultCurveType() : CURVE_DEFAULT);
					}
				}
				else
		...
		case FILTER_SOFTEN:
		{
			...
			for(int i = 0; i < kc; ++i)
			{
				...
				if(frac != 1.0f)
				{
					...
					if(curtime <= params.m_nTimes[TS_LEFT_HOLD])
					{
						// Get the value at the crossing point (either green edge for sharpen, or left edge for
						// soften...)
...
template<class T>
void CDmeTypedLog<T>::PasteAndRescaleSamples(const CDmeLogLayer *pBase, const CDmeLogLayer *pDataLayer,
											 ...
											 const DmeLog_TimeSelection_t &destParams, bool bBlendAreaInFalloffRegion)
{
	...
	T pStartValue[PASTE_STATE_COUNT + 1] = {
		bBlendAreaInFalloffRegion ? pBaseLayer->GetValue(tStartTime[PASTE_STATE_RAMP_IN][1])
		...
		bBlendAreaInFalloffRegion ? pBaseLayer->GetValue(tStartTime[PASTE_STATE_AFTER][1])
		...
		if(nPrevState < nState)
		{
			...
			if(nState != PASTE_STATE_AFTER)
			{
				...
				tMaxKeyTime = bCollapseSamples ? tStartTime[nState][1]
											   : (tStartTime[nState + 1][1] - DmeTime_t(pKeyCount[nState] + 1));
			}
			else
			{
...
// template<> void CDmeTypedLog< bool >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const bool& value ) {
// Assert( 0 ); } template<> void CDmeTypedLog< bool >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t& params,
// const bool& value ) { Assert( 0 ); } template<> void CDmeTypedLog< bool >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0 ); } template<> void
// CDmeTypedLog< bool >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 ); }
//
// template<> void CDmeTypedLog< Color >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const Color& value ) {
// Assert( 0 ); } template<> void CDmeTypedLog< Color >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t& params,
// const Color& value ) { Assert( 0 ); } template<> void CDmeTypedLog< Color >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0 ); } template<> void
// CDmeTypedLog< Color >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 ); }
//
// template<> void CDmeTypedLog< Vector4D >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const Vector4D& value
// ) { Assert( 0 ); } template<> void CDmeTypedLog< Vector4D >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t&
// params, const Vector4D& value ) { Assert( 0 ); } template<> void CDmeTypedLog< Vector4D >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0 ); } template<> void
// CDmeTypedLog< Vector4D >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 ); }
//
// template<> void CDmeTypedLog< Vector2D >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const Vector2D& value
// ) { Assert( 0 ); } template<> void CDmeTypedLog< Vector2D >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t&
// params, const Vector2D& value ) { Assert( 0 ); } template<> void CDmeTypedLog< Vector2D >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0 ); } template<> void
// CDmeTypedLog< Vector2D >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 ); }

// template<> void CDmeTypedLog< Vector >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const Vector& value ) {
// Assert( 0 ); } template<> void CDmeTypedLog< Vector >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t& params,
// const Vector& value ) { Assert( 0 ); } template<> void CDmeTypedLog< Vector >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0 ); } template<> void
// CDmeTypedLog< Vector >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 ); }

// template<> void CDmeTypedLog< VMatrix >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const VMatrix& value )
// { Assert( 0 ); } template<> void CDmeTypedLog< VMatrix >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t&
// params, const VMatrix& value ) { Assert( 0 ); } template<> void CDmeTypedLog< VMatrix >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0 ); } template<> void
// CDmeTypedLog< VMatrix >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 ); }
//
// template<> void CDmeTypedLog< Quaternion >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const Quaternion&
// value ) { Assert( 0 ); } template<> void CDmeTypedLog< Quaternion >::_StampKeyAtHeadResample( const
// DmeLog_TimeSelection_t& params, const Quaternion& value ) { Assert( 0 ); } template<> void CDmeTypedLog< Quaternion
// >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0
// ); } template<> void CDmeTypedLog< Quaternion >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 );
// }
//
// template<> void CDmeTypedLog< QAngle >::StampKeyAtHead( const DmeLog_TimeSelection_t& params, const QAngle& value ) {
// Assert( 0 ); } template<> void CDmeTypedLog< QAngle >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t& params,
// const QAngle& value ) { Assert( 0 ); } template<> void CDmeTypedLog< QAngle >::StampKeyAtHead( const
// DmeLog_TimeSelection_t& params, const CDmAttribute *pAttr, uint index /*= 0*/ ) { Assert( 0 ); } template<> void
// CDmeTypedLog< QAngle >::FinishTimeSelection( DmeLog_TimeSelection_t& params ) { Assert( 0 ); }

//-----------------------------------------------------------------------------
// Helpers for particular types of log layers
//-----------------------------------------------------------------------------
void GenerateRotationLog(CDmeQuaternionLogLayer *pLayer, const Vector &vecAxis, DmeTime_t pTime[4],
						 float pRevolutionsPerSec[4])
{
	for(int i = 1; i < 4; ++i)
	{
	...
	float flMaxRotationTime =
		(flMaxRPS != 0.0f) ? (0.125f / flMaxRPS) : (pTime[3].GetSeconds() - pTime[0].GetSeconds());
	...
	for(DmeTime_t t = pTime[0]; t <= pTime[3]; t += dt)
	{
		...
		float dRot = pRevolutionsPerSec[i] - pRevolutionsPerSec[i - 1];
		float flRotation =
			360.0f * (dRot * flInterval * flInterval * 0.5f * flOOSegmentDur + pRevolutionsPerSec[i - 1] * flInterval) +
			pTotalRotation[i - 1];
