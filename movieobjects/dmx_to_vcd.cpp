...
bool ConvertEventToDmx(CChoreoEvent *event, CDmeChannelsClip *clip)
{
	...
	if(array.IsValid())
	{
		...
		if(ramp)
		{
			...
			pCurveInfo->SetDefaultCurveType(
				MAKE_CURVE_TYPE(INTERPOLATE_CATMULL_ROM_NORMALIZE, INTERPOLATE_CATMULL_ROM_NORMALIZE));
			...
			// Left edge
			pCurveInfo->SetEdgeInfo(0, event->GetRamp()->IsEdgeActive(true), event->GetRamp()->GetEdgeZeroValue(true),
									event->GetRamp()->GetEdgeCurveType(true));
			// Right edge
			pCurveInfo->SetEdgeInfo(1, event->GetRamp()->IsEdgeActive(false), event->GetRamp()->GetEdgeZeroValue(false),
	...
	for(tagtype = 0; tagtype < CChoreoEvent::NUM_ABS_TAG_TYPES; tagtype++)
	{
		if(event->GetNumAbsoluteTags((CChoreoEvent::AbsTagType)tagtype) > 0)
		{
			...
			Q_snprintf(sz, sizeof(sz), "absolutetags %s",
	...
	switch(event->GetType())
	{
		...
		case CChoreoEvent::SPEAK:
		{
			...
			if(speakProperties)
			{
				...
				speakProperties->SetValue("closedcaption_type",
										  CChoreoEvent::NameForCCType(event->GetCloseCaptionType()));
				speakProperties->SetValue("closedcaption_token", event->GetCloseCaptionToken());
				bool usingCombined =
					(event->GetCloseCaptionType() != CChoreoEvent::CC_DISABLED && event->IsUsingCombinedFile());
				speakProperties->SetValue<bool>("closedcaption_usingcombinedfile", usingCombined);
				speakProperties->SetValue<bool>("closedcaption_combinedusesgender",
												event->IsCombinedUsingGenderToken());
		...
		case CChoreoEvent::FLEXANIMATION:
		{
			...
			if(array.IsValid())
			{
				...
				for(int i = 0; i < numTracks; ++i)
				{
					...
					CDmeChannel *channel =
						CreateElement<CDmeChannel>(track->GetFlexControllerName(), clip->GetFileId());
					array.AddToTail(channel);
					...
					if(log)
					{
						CDmeFloatCurveInfo *pCurveInfo =
							CreateElement<CDmeFloatCurveInfo>("floatcurveinfo", clip->GetFileId());

						pCurveInfo->SetDefaultCurveType(
							MAKE_CURVE_TYPE(INTERPOLATE_CATMULL_ROM_NORMALIZEX, INTERPOLATE_CATMULL_ROM_NORMALIZEX));
						pCurveInfo->SetRightEdgeTime(DmeTime_t(event->GetDuration()));

						pCurveInfo->SetUseEdgeInfo(true);
						pCurveInfo->SetDefaultEdgeZeroValue(0.0f);
						// Left edge
						pCurveInfo->SetEdgeInfo(0, track->IsEdgeActive(true), track->GetEdgeZeroValue(true),
												track->GetEdgeCurveType(true));
						// Right edge
						pCurveInfo->SetEdgeInfo(1, track->IsEdgeActive(false), track->GetEdgeZeroValue(false),
					...
					if(track->IsComboType())
					{
						...
						Q_snprintf(balanceChannelName, sizeof(balanceChannelName), "%s_balance",
						...
						if(balance)
						{
							...
							pCurveInfo->SetDefaultCurveType(MAKE_CURVE_TYPE(INTERPOLATE_CATMULL_ROM_NORMALIZEX,
	...
	if(rampCount > 0)
	{
		...
		if(array.IsValid())
		{
			...
			if(ramp)
			{
				...
				pCurveInfo->SetDefaultEdgeZeroValue(0.0f);
				// Left edge
				pCurveInfo->SetEdgeInfo(0, scene->GetSceneRamp()->IsEdgeActive(true),
										scene->GetSceneRamp()->GetEdgeZeroValue(true),
										scene->GetSceneRamp()->GetEdgeCurveType(true));
				// Right edge
				pCurveInfo->SetEdgeInfo(1, scene->GetSceneRamp()->IsEdgeActive(false),
...
bool ConvertDmxToEvent(CChoreoScene *scene, CDmeTrackGroup *pActor, CDmeTrack *pChannel, CDmeChannelsClip *clip,
					   bool globalEvent)
{
	bool bret = true;
		...
		for(int i = 0; i < c; ++i)
		{
			...
			for(int j = 0; j < rampCount; ++j)
			{
				CExpressionSample *sample =
					event->AddRamp(ramp->GetKeyTime(j).GetSeconds(), ramp->GetKeyValue(j), false);
	...
	if(clip->HasAttribute("relative_tag"))
	{
		...
		if(relativeTag)
		{
			event->SetUsingRelativeTag(true, relativeTag->GetValueString("tagname"),
	...
	switch(event->GetType())
	{
		...
		case CChoreoEvent::SPEAK:
		{
			...
			if(speakProperties)
			{
				event->SetCloseCaptionType(
					CChoreoEvent::CCTypeForName(speakProperties->GetValueString("closedcaption_type")));
						...
						if(balanceChannel)
						{
							...
							else
							{
								...
								Q_snprintf(msg, sizeof(msg),
										   "Error:  Missing balance channel for combo flex track (%s) in (%s)\n",
...
bool ConvertDmxToScene(CDmeFilmClip *dmx, CChoreoScene *scene)
{
	...
	if(pTrackGroup)
	{
		...
		if(track)
		{
			CDmeChannelsClip *pClip =
				CastElement<CDmeChannelsClip>(track->FindNamedClip(VCD_SCENE_RAMP_TRACK_GROUP_NAME));
			if(pClip)
			{
				...
				if(array.IsValid() && (array.Count() > 0))
				{
					...
					if(channel)
					{
						...
						if(ramp)
						{
							...
							{
								CExpressionSample *sample =
									scene->AddSceneRamp(ramp->GetKeyTime(j).GetSeconds(), ramp->GetKeyValue(j), false);
								sample->SetCurveType(ramp->GetKeyCurveType(j));
							}
						}
					}
