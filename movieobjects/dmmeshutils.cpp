...
//-----------------------------------------------------------------------------
void RemoveUnusedData(CDmeMesh *pMesh, CDmeVertexData *pVertexData, bool bBind, const char *pFieldName, int *pIndices,
					  int nIndicesCount, CDmrGenericArray &data)
{
...
//
//-----------------------------------------------------------------------------
void RemoveUnusedVerticesFromBaseState(CDmeMesh *pMesh, CDmeVertexData *pVertexData,
									   const CUtlVector<int> &newToOldIndexMap)
{
	...
	for(int i = 0; i < nFieldCount; ++i)
	{
		...
		RemoveUnusedData(pMesh, pVertexData, bBind, pFieldName, pNewVertexIndices, nNewToOldIndexMapCount,
...
//-----------------------------------------------------------------------------
const int *CDmMeshUtils::BuildDataMirrorMap(CDmeVertexData *pBase, int axis,
...
//-----------------------------------------------------------------------------
template<class T_t>
void MirrorVertexData(CDmeVertexData *pBase, FieldIndex_t fieldIndex, int axis, int nOrigVertexCount, int nMirrorCount,
					  const CDmrArrayConst<T_t> &origData, const CUtlVector<int> &origIndices,
					  const CUtlVector<int> &mirrorMap, const CUtlVector<int> &dataMirrorMap)
{
	if(nMirrorCount <= 0)
		return;
...
bool CDmMeshUtils::MirrorVertices(CDmeVertexData *pBase, int axis, int nOldVertexCount, int nMirrorCount,
								  const CUtlVector<int> &mirrorMap, const CUtlVector<int> &posMirrorMap,
								  const CUtlVector<int> &normalMirrorMap, const CUtlVector<int> &uvMirrorMap)
{
			...
			case AT_VECTOR2_ARRAY:
				if(i == uvFieldIndex)
				{
					MirrorVertexData(pBase, i, axis % 2, nOldVertexCount, nMirrorCount,
									 CDmrArrayConst<Vector2D>(pBaseData), baseIndices, mirrorMap, uvMirrorMap);
					continue;
				}
				break;
			case AT_VECTOR3_ARRAY:
				if(i == posFieldIndex)
				{
					MirrorVertexData(pBase, i, axis, nOldVertexCount, nMirrorCount, CDmrArrayConst<Vector>(pBaseData),
									 baseIndices, mirrorMap, posMirrorMap);
					continue;
				}
				else if(i == normalFieldIndex)
				{
					MirrorVertexData(pBase, i, axis, nOldVertexCount, nMirrorCount, CDmrArrayConst<Vector>(pBaseData),
									 baseIndices, mirrorMap, normalMirrorMap);
					continue;
				}
				break;
			default:
...
//-----------------------------------------------------------------------------
void CDmMeshUtils::MirrorVertices(CDmeVertexData *pBase, FieldIndex_t fieldIndex, int nOldVertexCount, int nMirrorCount,
								  const CUtlVector<int> &baseIndices, const CUtlVector<int> &mirrorMap)
{
	if(nMirrorCount <= 0)
		return;
...
//-----------------------------------------------------------------------------
template<class T_t>
void MirrorDeltaData(CDmeVertexDeltaData *pDelta, FieldIndex_t fieldIndex, int axis,
					 const CDmrArrayConst<T_t> &origData, const CUtlVector<int> &origIndices,
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::MirrorDelta(CDmeVertexDeltaData *pDelta, int axis, const CUtlVector<int> &posMirrorMap,
							   const CUtlVector<int> &normalMirrorMap, const CUtlVector<int> &uvMirrorMap)
{
	...
	for(int i = 0; i < nFields; ++i)
	{
		...
		switch(pDeltaData->GetType())
		{
			case AT_VECTOR2_ARRAY:
				if(i == uvFieldIndex)
				{
					MirrorDeltaData(pDelta, i, axis % 2, CDmrArrayConst<Vector2D>(pDeltaData), deltaIndices,
									uvMirrorMap);
					continue;
				}
				break;
...
// -1 if not found
//-----------------------------------------------------------------------------
int CDmMeshUtils::FindMergeSocket(const CUtlVector<CUtlVector<CDmMeshComp::CEdge *>> &srcBorderEdgesList,
								  CDmeMesh *pDstMesh)
{
...
bool CDmMeshUtils::Merge(CDmeMesh *pSrcMesh, CDmElement *pRoot)
{
	...
	Msg("Error: Merge() - No Merge Socket Found - i.e. A Set Of Border Edges On The Source Model That Are Found On The "
		"Merge Model");
...
//-----------------------------------------------------------------------------
template<class T_t>
void AppendData(const CDmrArrayConst<T_t> &srcData, CDmrArray<T_t> &dstData, const matrix3x4_t *pMat = NULL)
{
...
//-----------------------------------------------------------------------------
template<>
void AppendData(const CDmrArrayConst<Vector> &srcData, CDmrArray<Vector> &dstData, const matrix3x4_t *pMat)
{
	const int nSrcCount = srcData.Count();
	const int nDstCount = dstData.Count();
...
//-----------------------------------------------------------------------------
int MergeBaseState(CDmeVertexData *pSrcBase, CDmeVertexData *pDstBase, const matrix3x4_t &pMat, const matrix3x4_t &nMat,
				   int nSkinningJointIndex, int &nPositionOffset, int &nNormalOffset, int &nWrinkleOffset)
{
	int nRetVal = -1;
...
//
//-----------------------------------------------------------------------------
void MergeDeltaState(CDmeMesh *pDmeMesh, CDmeVertexDeltaData *pSrcDelta, CDmeVertexDeltaData *pDstDelta,
...
bool CDmMeshUtils::Merge(CDmeMesh *pSrcMesh, CDmeMesh *pDstMesh, int nSkinningJointIndex)
{
	...
	for(int i = 0; i < pSrcMesh->BaseStateCount(); ++i)
	{
		...
		for(int j = 0; j < pDstMesh->BaseStateCount(); ++j)
		{
			...
			bMerged = true;
			const int nTmpVertexOffset = MergeBaseState(pSrcBase, pDstBase, pMat, nMat, nSkinningJointIndex,
		...
		if(!bMerged)
		{
			Msg("Error: Merge( %s, %s ) - Can't Find Base State %s On %s\n", pSrcMesh->GetName(), pDstMesh->GetName(),
				pSrcBase->GetName(), pDstMesh->GetName());
		}
	}
					  ...
					  const CUtlVector<VertexWeightMap_s> &vertexWeightMap)
{
	...
	const CUtlVector<float> &srcJointWeights =
		CDmrArrayConst<float>(pSrcData->GetVertexData(nSrcJointWeightsField)).Get();
	...
	for(int i = 0; i < nDstCount; ++i)
	{
		...
		if(nVertexWeights > 0)
		{
			...
			memcpy(pDstJointWeights + i * nJointCount, pSrcJointWeights + nMatchIndex * nJointCount,
				   nJointCount * sizeof(float));
			memcpy(pDstJointIndices + i * nJointCount, pSrcJointIndices + nMatchIndex * nJointCount,
	...
	if(pSrcComboOp)
	{
		...
		else
		{
			...
			for(;;)
			{
				...
				{
					// Can't find anyone referring to this via "children" so, hopefully it's the DmeModel referred to by
					// "model"
...
//-----------------------------------------------------------------------------
template<class T_t>
void CopyFieldData(const CDmrArrayConst<T_t> &srcData, const CUtlVector<int> &srcIndices,
...
//
//-----------------------------------------------------------------------------
bool CopyField(CDmeVertexData::StandardFields_t field, CDmeVertexData *pSrcData, CDmeVertexData *pDstData,
			   const CUtlVector<VertexWeightMap_s> &vertexWeightMap)
{
	...
	switch(pSrcVertexData->GetType())
	{
		case AT_FLOAT_ARRAY:
			...
			break;
		case AT_VECTOR2_ARRAY:
			CopyFieldData(CDmrArrayConst<Vector2D>(pSrcVertexData), srcIndices, pDstData, dstFieldIndex,
						  vertexWeightMap);
			break;
		case AT_VECTOR3_ARRAY:
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::Merge(CDmMeshComp &srcComp, const CUtlVector<CDmMeshComp::CEdge *> &edgeList, CDmeMesh *pDstMesh)
{
	...
	for(int i = 0; i < nSrcCount; ++i)
	{
		...
		for(int j = 0; j < nDstCount; ++j)
		{
			...
			if(vSrc.DistToSqr(vDst) < FLT_EPSILON * 10.0f)
			{
				...
				vertexWeight.m_vertexWeights[0].m_pVertexIndices =
					&pDstData->FindVertexIndicesFromDataIndex(CDmeVertexData::FIELD_POSITION, j);
				break;
			}
		...
		if(vertexWeight.m_nVertexWeights == 0)
		{
			Warning("Warning: Merge() - No Match For Src Vertex: %f %f %f, Using Closest: %f %f %f\n", vSrc.x, vSrc.y,
					vSrc.z, dstPosData[nClosestIndex].x, dstPosData[nClosestIndex].y, dstPosData[nClosestIndex].z);
			...
			vertexWeight.m_vertexWeights[0].m_vertexWeight = 1.0f;
			vertexWeight.m_vertexWeights[0].m_pVertexIndices =
				&pDstData->FindVertexIndicesFromDataIndex(CDmeVertexData::FIELD_POSITION, nClosestIndex);
...
//
//-----------------------------------------------------------------------------
bool CreateExpressionFile(const char *pExpressionFile, const CUtlVector<CUtlString> *pPurgeAllButThese,
						  CDmeCombinationOperator *pComboOp, CDmePresetGroup *pPresetGroup)
{
	if(!pPresetGroup)
		return false;
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::CreateDeltasFromPresets(CDmeMesh *pMesh, CDmeVertexData *pPassedDst,
										   ...
										   const CUtlVector<CUtlString> *pPurgeAllButThese /*= NULL */)
{
	...
	for(int i = 0; i < presetExpressionMap.GetNumStrings(); ++i)
	{
		...
		CreateDeltasFromPresetGroup(pPresetGroup, pComboOp, pPurgeAllButThese, pMesh, pDst, conflictingNames,
									presetMap);
	}
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::CreateWrinkleDeltaFromBaseState(CDmeVertexDeltaData *pDelta, float flScale /* = 1.0f */,
												   ...
												   CDmeVertexData *pPassedCurrent /* = NULL */)
{
	...
	const CDmeMesh *pMesh = pPassedMesh ? pPassedMesh
							: pBind		? FindReferringElement<CDmeMesh>(pBind, "baseStates")
										: NULL;
	...
	if(!pDelta || !pBind || !pCurr || pBind == pCurr || !pMesh || pMesh != pBindMesh || pMesh != pCurrMesh ||
	   pMesh != pDeltaMesh)
	{
		return false;
	}
	...
	if(pOldWrinkleData)
	{
		for(int i = 0; i < nPosCount; ++i)
		{
			...
			for(int j = 0; j < baseVerts.Count(); ++j)
			{
				...
				if(fabs(pOldWrinkleData[nTexCoordIndex]) > 0.0001 || fabs(v.x) >= (1 / 4096.0f) ||
				   fabs(v.y) >= (1 / 4096.0f) || fabs(v.z) >= (1 / 4096.0f))
				{
					flDelta = v.Length();
	...
	for(int i = 0; i < nWrinkleCount; ++i)
	{
		...
		const CUtlVector<int> &baseVerts =
			pBind->FindVertexIndicesFromDataIndex(CDmeVertexData::FIELD_POSITION, pWrinkleIndices[i]);
...
//-----------------------------------------------------------------------------
ControlIndex_t FindComboOpControlIndexForAnimSetControl(CDmeCombinationOperator *pComboOp, const char *pControlName,
														bool *pIsMulti /*= NULL*/)
{
...
//
//-----------------------------------------------------------------------------
void CDmMeshUtils::CreateDeltasFromPresetGroup(CDmePresetGroup *pPresetGroup, CDmeCombinationOperator *pComboOp,
											   ...
											   CUtlStringMap<CDmePreset *> &presetMap)
{
	...
	for(int i = 0; i < nPresetsCount; ++i)
	{
		...
		for(int j = 0; j < nControlValues; ++j)
		{
			...
			{
				pComboOp->SetControlValue(nControlIndex, pControlPreset->GetValue<float>("value", 0.0),
			...
			{
				pComboOp->SetMultiControlLevel(nControlIndex, pControlPreset->GetValue<float>("multilevel", 0.5));
			}
		}
		...
		for(int presetSuffix = 1;
			pComboOp->FindControlIndex(presetName) >= 0 || pMesh->FindDeltaState(presetName) != NULL ||
...
//
//-----------------------------------------------------------------------------
void CDmMeshUtils::PurgeUnreferencedDeltas(CDmeMesh *pMesh, CUtlStringMap<CDmePreset *> &presetMap,
