...
CDmElement *CDmeDCCMakefile::CreateOutputElement()
{
	if(m_bFlushFile)
	{
		...
		if(GetFileId() != DMFILEID_INVALID)
		{
			...
			if(g_pDataModel->RestoreFromFile(g_pDataModel->GetFileName(fileId), NULL, NULL, &pRoot, CR_DELETE_OLD) !=
			   DMFILEID_INVALID)
			{
...
static DmeMakefileType_t s_pMayaAnimationSourceTypes[] = {
	{"DmeSourceMayaAnimationFile", "Maya Animation File", true, "makefiledir:../maya", "*.ma;*.mb",
	 "Maya File (*.ma,*.mb)"},
	{NULL, NULL, false, NULL, NULL, NULL},
};
