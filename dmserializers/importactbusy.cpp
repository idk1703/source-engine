...
void CImportActBusy::SerializeHeader(CUtlBuffer &buf)
{
	...
	buf.Printf("// \t\"busy_sequence\"\t\t\"Sequence Name\". If specified, this is used over the activity name. "
			   "Specify it in the hint node.\n");
	...
	buf.Printf("// \t\"max_time\"\t\t\t\"Maximum time to spend in this busy anim\"	0 = only stop when interrupted by "
			   "external event\n");
	buf.Printf("// \t\"interrupts\"\t\tOne of:\n");
	buf.Printf("// \t\t\t\t\t\t\"BA_INT_NONE\"\t\tbreak out only when time runs out. No external influence will break "
			   "me out.\n");
	...
	buf.Printf("// \t\t\t\t\t\t\"BA_INT_COMBAT\"\t\tbreak out of this anim if combat occurs in my line of sight "
			   "(bullet hits, grenades, etc), -OR- the max time is reached\n");
	buf.Printf("// }\n");
	buf.Printf("//\n");
}
...
CDmElement *CImportActBusy::UnserializeFromKeyValues(KeyValues *pKeyValues)
{
	...
	for(KeyValues *pActBusyKey = pKeyValues->GetFirstTrueSubKey(); pActBusyKey != NULL;
		pActBusyKey = pActBusyKey->GetNextTrueSubKey())
	{
