...
bool CImportVMF::SerializeEntityEditorKey(CUtlBuffer &buf, DmElementHandle_t hEditor)
{
	...
	for(CDmAttribute *pAttribute = pEditorElement->FirstAttribute(); pAttribute != NULL;
		pAttribute = pAttribute->NextAttribute())
	{
		...
		if(Q_stricmp(pKeyName, "color") && Q_stricmp(pKeyName, "id") && Q_stricmp(pKeyName, "comments") &&
		   Q_stricmp(pKeyName, "visgroupshown") && Q_stricmp(pKeyName, "visgroupautoshown"))
		{
...
bool CImportVMF::Serialize(CUtlBuffer &buf, CDmElement *pRoot)
{
	...
	const char *pOtherFilter1[] = {"versioninfo", "visgroups", "viewsettings", "world", NULL};
