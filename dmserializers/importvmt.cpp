...
bool CImportVMT::UnserializeFallbacks(CDmElement *pElement, KeyValues *pFallbackKeyValues)
{
	...
	for(KeyValues *pShaderParam = pFallbackKeyValues->GetFirstValue(); pShaderParam != NULL;
		pShaderParam = pShaderParam->GetNextValue())
	{
...
void CImportVMT::ExpandPatchFile(KeyValues *pKeyValues)
{
	...
	while(count < 10 && stricmp(pKeyValues->GetName(), "patch") == 0)
	{
		...
		if(pIncludeFileName)
		{
			...
			bool success =
				includeKeyValues->LoadFromFile(g_pFullFileSystem, pIncludeFileName, IsX360() ? "GAME" : NULL);
			if(success)
			{
...
CDmElement *CImportVMT::UnserializeFromKeyValues(KeyValues *pKeyValues)
{
	...
	// Normal keys are shader parameters
	for(KeyValues *pShaderParam = pKeyValues->GetFirstValue(); pShaderParam != NULL;
		pShaderParam = pShaderParam->GetNextValue())
	{
