...
void CMDL::Draw(const matrix3x4_t &rootToWorld, const matrix3x4_t *pBoneToWorld)
{
	...
	g_pStudioRender->DrawModel(NULL, info, const_cast<matrix3x4_t *>(pBoneToWorld), pFlexWeights, NULL, vecModelOrigin,
...
void CMDL::SetUpBones(const matrix3x4_t &rootToWorld, int nMaxBoneCount, matrix3x4_t *pBoneToWorld,
					  const float *pPoseParameters, MDLSquenceLayer_t *pSequenceLayers, int nNumSequenceLayers)
{
								   ...
								   const matrix3x4_t &matModelToWorld)
{
	...
	for(int iMergeBone = 0; iMergeBone < pMergeHdr->numbones(); ++iMergeBone)
	{
		...
		if(!bMerged)
		{
			...
			{
				ConcatTransforms(pMergeBoneToWorld[pMergeBones[iMergeBone].parent], matBone,
								 pMergeBoneToWorld[iMergeBone]);
			}
		}
	}
}