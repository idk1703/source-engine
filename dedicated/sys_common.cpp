...
static const char *get_consolelog_filename()
{
	...
	if(!s_bInited)
	{
		...
		if(!CommandLine()->FindParm("-consoledebug") && CommandLine()->CheckParm("-consolelog", &filename) && filename)
		{
...
const char *UTIL_GetExecutableDir()
{
	...
	if(exedir[exeLen - 4] != CORRECT_PATH_SEPARATOR || exedir[exeLen - 3] != 'b' || exedir[exeLen - 2] != 'i' ||
	   exedir[exeLen - 1] != 'n')
	{
...
const char *UTIL_GetBaseDir(void)
{
	...
	if(pExeDir)
	{
		...
		if(basedir[dirlen - 3] == 'b' && basedir[dirlen - 2] == 'i' && basedir[dirlen - 1] == 'n')
		{
			basedir[dirlen - 4] = 0;
		}
	}
