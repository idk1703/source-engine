...
//-----------------------------------------------------------------------------
void CCreateMultiplayerGameServerPage::SetConfig(const char *serverName, const char *rconPassword, int maxPlayers,
...
void CCreateMultiplayerGameServerPage::OnCommand(const char *cmd)
{
	...
	else if(!stricmp(cmd, "start"))
	{
		...
		else
		{
			...
			//
			//					// Mods just specify their app ID (CS, HL2, HL2MP, etc), and it mounts all the necessary
			//caches. 					int iAppId = pFileSystem->GetInt( "SteamAppId" ); 					if ( iAppId )
			//					{
			//						CUtlVector<unsigned int> depList;
			//						MountDependencies( iAppId, depList );
			//
			//						char gameinfoFilename[MAX_PATH];
			//						Q_snprintf( gameinfoFilename, sizeof( gameinfoFilename ), "%s\\gameinfo.txt",
			//m_iServer.gameDir ); 						g_pFullFileSystem->GetLocalCopy( gameinfoFilename );
			//					}
			//				}
			//			}
	...
	if(pSub)
	{
		...
		if(iSteamAppId != -1)
		{
			if(IsEp1EraAppID(iSteamAppId))
			{
				...
				if(!system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Steam\\SteamPath", steamDir,
												sizeof(steamDir)))
...
void CCreateMultiplayerGameServerPage::LoadMODList()
{
	...
	if(system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Steam\\SourceModInstallPath", sourceModsDir,
								   sizeof(sourceModsDir)))
...
void CCreateMultiplayerGameServerPage::AddMod(const char *pGameDirName, const char *pGameInfoFilename,
											  KeyValues *pGameInfo)
{
