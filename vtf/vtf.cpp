...
BEGIN_BYTESWAP_DATADESC(VTFFileBaseHeader_t)
	DEFINE_ARRAY(fileTypeString, FIELD_CHARACTER, 4), DEFINE_ARRAY(version, FIELD_INTEGER, 2),
		DEFINE_FIELD(headerSize, FIELD_INTEGER),
END_DATADESC()

BEGIN_BYTESWAP_DATADESC_(VTFFileHeaderV7_1_t, VTFFileBaseHeader_t)
	DEFINE_FIELD(width, FIELD_SHORT), DEFINE_FIELD(height, FIELD_SHORT), DEFINE_FIELD(flags, FIELD_INTEGER),
		DEFINE_FIELD(numFrames, FIELD_SHORT), DEFINE_FIELD(startFrame, FIELD_SHORT),
		DEFINE_FIELD(reflectivity, FIELD_VECTOR), DEFINE_FIELD(bumpScale, FIELD_FLOAT),
		DEFINE_FIELD(imageFormat, FIELD_INTEGER), DEFINE_FIELD(numMipLevels, FIELD_CHARACTER),
		DEFINE_FIELD(lowResImageFormat, FIELD_INTEGER), DEFINE_FIELD(lowResImageWidth, FIELD_CHARACTER),
...
BEGIN_BYTESWAP_DATADESC_(VTFFileHeaderX360_t, VTFFileBaseHeader_t)
	DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(width, FIELD_SHORT), DEFINE_FIELD(height, FIELD_SHORT),
		DEFINE_FIELD(depth, FIELD_SHORT), DEFINE_FIELD(numFrames, FIELD_SHORT),
		DEFINE_FIELD(preloadDataSize, FIELD_SHORT), DEFINE_FIELD(mipSkipCount, FIELD_CHARACTER),
		DEFINE_FIELD(numResources, FIELD_CHARACTER), DEFINE_FIELD(reflectivity, FIELD_VECTOR),
		DEFINE_FIELD(bumpScale, FIELD_FLOAT), DEFINE_FIELD(imageFormat, FIELD_INTEGER),
		DEFINE_ARRAY(lowResImageSample, FIELD_CHARACTER, 4), DEFINE_FIELD(compressedSize, FIELD_INTEGER),
END_DATADESC()
...
char *S3TC_GetBlock(void *pCompressed, ImageFormat format,
					int nBlocksWide, // How many blocks wide is the image (pixels wide / 4).
					int xBlock, int yBlock)
{
	return NULL;
...
void S3TC_SetPaletteIndex(unsigned char *pFaceData, ImageFormat format, int imageWidth, int x, int y,
...
int *g_skybox_FaceVerts[6] = {g_skybox_rightFaceVerts, g_skybox_leftFaceVerts, g_skybox_backFaceVerts,
							  g_skybox_frontFaceVerts, g_skybox_upFaceVerts,   g_skybox_downFaceVerts};
		...
		case VTF_MAJOR_VERSION:
			switch(nMinorVersion)
			{
				...
				case VTF_MINOR_VERSION:
					...
					// printf("\n VTFFileHeaderSize (%i %i) is %i + %i -> %i",nMajorVersion,nMinorVersion, size1, size2,
					// result );
					return result;
			}
			break;
...
// Initialization
//-----------------------------------------------------------------------------
bool CVTFTexture::Init(int nWidth, int nHeight, int nDepth, ImageFormat fmt, int iFlags, int iFrameCount,
					   int nForceMipCount)
{
	if(nDepth == 0)
	{
...
void CVTFTexture::InitLowResImage(int nWidth, int nHeight, ImageFormat fmt)
{
	...
	int iLowResImageSize =
		ImageLoader::GetMemRequired(m_nLowResImageWidth, m_nLowResImageHeight, 1, m_LowResImageFormat, false);
...
void CVTFTexture::ReleaseResources()
{
	...
	for(ResourceMemorySection *pRms = m_arrResourcesData.Base(), *pRmsEnd = pRms + m_arrResourcesData.Count();
		pRms < pRmsEnd; ++pRms)
	{
		delete[] pRms->m_pData;
	}
...
bool CVTFTexture::LoadImageData(CUtlBuffer &buf, const VTFFileHeader_t &header, int nSkipMipLevels)
{
	...
	// Even if we are preloading partial data, always do the full allocation here. We'll use LOD clamping to ensure we
	// only reference data that is available.
	...
	TextureStreamSettings_t const *pStreamSettings =
		(TextureStreamSettings_t const *)GetResourceData(VTF_RSRC_TEXTURE_STREAM_SETTINGS, NULL);
	if(pStreamSettings)
	{
	...
	{
		// Don't need to subtract nSkipMipLevels: m_nMipCount has subtracted that above--assuming this assert doesn't
		// fire.
	...
	for(int iMip = m_nMipCount; --iMip >= 0;)
	{
		...
		for(int iFrame = 0; iFrame < m_nFrameCount; ++iFrame)
		{
			for(int iFace = 0; iFace < nFacesToRead; ++iFace)
			{
				// printf("\n tex %p mip %i frame %i face %i  size %i  buf offset %i", this, iMip, iFrame, iFace,
				// iMipSize, buf.TellGet() );
...
{
	for(ResourceEntryInfo const *pInfo = m_arrResourcesInfo.Base(), *pInfoEnd = pInfo + m_arrResourcesInfo.Count();
...
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s (header only: %d, nForceFlags: %d, skipMips: %d)", __FUNCTION__,
		   bHeaderOnly ? 1 : 0, nForceFlags, nSkipMipLevels);
	...
	if((Q_log2(header.width) + Q_log2(header.height) + Q_log2(header.depth) + Q_log2(header.numFrames) >
		cMaxImageSizeLog2) ||
	   (header.numResources > MAX_RSRC_DICTIONARY_ENTRIES))
	{
	...
	{
		m_arrResourcesData_ForReuse.EnsureCapacity(m_arrResourcesData_ForReuse.Count() + m_arrResourcesData.Count() -
												   header.numResources);
...
{
	return const_cast<ResourceEntryInfo *>(((CVTFTexture const *)this)->FindResourceEntryInfo(eType));
}
...
{
	Assert(pSrcRect->x >= 0 && pSrcRect->y >= 0 && (pSrcRect->x + pSrcRect->width <= m_nWidth) &&
			...
			for(int iFace = 0; iFace < m_nFaceCount; ++iFace)
			{
				...
				for(int z = 0; z < nMipDepth; ++z, pSrcData += nSrcFaceStride, pDstData += nDstFaceStride)
				{
					if(bNormalToDUDV)
					{
						...
						{
							ImageLoader::ConvertNormalMapRGBA8888ToDUDVMapUV88(pSrcData, nMipWidth, nMipHeight,
																			   pDstData);
						}
						else if(fmt == IMAGE_FORMAT_UVWQ8888)
						{
							ImageLoader::ConvertNormalMapRGBA8888ToDUDVMapUVWQ8888(pSrcData, nMipWidth, nMipHeight,
																				   pDstData);
						}
						else if(fmt == IMAGE_FORMAT_UVLX8888)
						{
							ImageLoader::ConvertNormalMapRGBA8888ToDUDVMapUVLX8888(pSrcData, nMipWidth, nMipHeight,
																				   pDstData);
						}
						else
						{
					...
					{
						ImageLoader::ConvertImageFormat(pSrcData, m_Format, pDstData, fmt, nMipWidth, nMipHeight);
					}
				}
			}
		}
...
void CVTFTexture::GenerateSpheremap(LookDir_t lookDir)
{
	...
	for(int iFrame = 0; iFrame < m_nFrameCount; ++iFrame)
	{
		...
		ImageLoader::GenerateMipmapLevels(pSphereMapBits, pFinalSphereMapBits, m_nWidth, m_nHeight, m_nDepth, m_Format,
										  2.2, 2.2, m_nMipCount);
	}
...
void CVTFTexture::GenerateHemisphereMap(unsigned char *pSphereMapBitsRGBA, int targetWidth, int targetHeight,
...
void CVTFTexture::GenerateMipmaps()
{
	...
	if(TextureLODControlSettings_t const *pLodSettings =
		   (TextureLODControlSettings_t const *)GetResourceData(VTF_RSRC_TEXTURE_LOD_SETTINGS, NULL))
	{
	...
	for(int iMipLevel = 1; iMipLevel < m_nMipCount; ++iMipLevel)
	{
		...
		for(int ch = 0; ch < 4; ++ch)
		{
			...
			if(!(((iMipLevel == m_nMipCount - 1) ||
				  (iMipLevel > iLastNonDecayMip)) && // last 1x1 mip  or  past clamped and skipped
		...
		for(int iFrame = 0; iFrame < m_nFrameCount; ++iFrame)
		{
			for(int iFace = 0; iFace < m_nFaceCount; ++iFace)
			{
				...
				{
					ImageLoader::NormalizeNormalMapRGBA8888(pDstLevel,
...
bool CVTFTexture::ConstructLowResImage()
{
	...
	return ImageLoader::ConvertImageFormat(tmpImage, IMAGE_FORMAT_RGBA8888, m_pLowResImageData, m_LowResImageFormat,
...
void CVTFTexture::SetupTextureEdgeIncrements(int iMipLevel, int iFace1Edge, int iFace2Edge, bool bFlipFace2Edge,
...
void CVTFTexture::BlendCubeMapFaceEdges(int iFrame, int iMipLevel, const CEdgeMatch *pMatch)
{
	int nMipWidth, nMipHeight, nMipDepth;
	ComputeMipLevelDimensions(iMipLevel, &nMipWidth, &nMipHeight, &nMipDepth);
	Assert(nMipDepth == 1);
	...
	if(m_Format == IMAGE_FORMAT_DXT1 || m_Format == IMAGE_FORMAT_DXT5)
	{
		if(iFace1Cur != incs.iFace1End)
		{
			while(iFace1Cur != incs.iFace1End)
			{
				...
				S3PaletteIndex paletteIndex =
					S3TC_GetPaletteIndex(pFace1Data, m_Format, nMipWidth, iFace1Cur.x, iFace1Cur.y);
	...
	else if(m_Format == IMAGE_FORMAT_RGBA8888)
	{
		if(iFace1Cur != incs.iFace1End)
		{
			while(iFace1Cur != incs.iFace1End)
			{
				...
				unsigned char *texels[2] = {pFace1Data + ((iFace1Cur.y * nMipWidth) + iFace1Cur.x) * 4,
...
void CVTFTexture::BlendCubeMapFaceCorners(int iFrame, int iMipLevel, const CCornerMatch *pMatch)
{
	int nMipWidth, nMipHeight, nMipDepth;
	ComputeMipLevelDimensions(iMipLevel, &nMipWidth, &nMipHeight, &nMipDepth);
	Assert(nMipDepth == 1);
	...
	if(m_Format == IMAGE_FORMAT_DXT1 || m_Format == IMAGE_FORMAT_DXT5)
	{
		...
		S3PaletteIndex paletteIndex =
			S3TC_GetPaletteIndex(pImageData[0], m_Format, nMipWidth, texelPos[0].x, texelPos[0].y);
...
void CVTFTexture::BuildCubeMapMatchLists(CEdgeMatch edgeMatches[NUM_EDGE_MATCHES],
...
void CVTFTexture::BlendCubeMapEdgePalettes(int iFrame, int iMipLevel, const CEdgeMatch *pMatch)
{
	...
	S3RGBA *pFace1Original =
		&m_OriginalData[GetImageOffset(iFrame, pMatch->m_iFaces[0], iMipLevel, IMAGE_FORMAT_RGBA8888) / 4];
	S3RGBA *pFace2Original =
		&m_OriginalData[GetImageOffset(iFrame, pMatch->m_iFaces[1], iMipLevel, IMAGE_FORMAT_RGBA8888) / 4];
	...
	while(iFace1Cur != incs.iFace1End) // We intentionally want to not process the last block here..
	{
		...
		char *blocks[2] = {S3TC_GetBlock(pFace1Data, m_Format, nMipWidth >> 2, iFace1Cur.x, iFace1Cur.y),
						   S3TC_GetBlock(pFace2Data, m_Format, nMipWidth >> 2, iFace2Cur.x, iFace2Cur.y)};

		S3RGBA *originals[2] = {&pFace1Original[(iFace1Cur.y * 4 * nMipWidth) + iFace1Cur.x * 4],
...
void CVTFTexture::BlendCubeMapCornerPalettes(int iFrame, int iMipLevel, const CCornerMatch *pMatch)
{
	int nMipWidth, nMipHeight, nMipDepth;
	ComputeMipLevelDimensions(iMipLevel, &nMipWidth, &nMipHeight, &nMipDepth);
	Assert(nMipDepth == 1);
	...
	for(int iEdge = 0; iEdge < 3; iEdge++)
	{
		...
		S3RGBA *pFaceOriginal =
			&m_OriginalData[GetImageOffset(iFrame, iFaceIndex, iMipLevel, IMAGE_FORMAT_RGBA8888) / 4];
...
void CVTFTexture::MatchCubeMapS3TCPalettes(CEdgeMatch edgeMatches[NUM_EDGE_MATCHES],
										   CCornerMatch cornerMatches[NUM_CORNER_MATCHES])
{
	for(int iMipLevel = 0; iMipLevel < m_nMipCount; ++iMipLevel)
	{
		for(int iFrame = 0; iFrame < m_nFrameCount; ++iFrame)
		{
			...
			{
				BlendCubeMapCornerPalettes(iFrame, iMipLevel, &cornerMatches[iCornerMatch]);
			}
		}
	}
}
...
void CVTFTexture::MatchCubeMapBorders(int iStage, ImageFormat finalFormat, bool bSkybox)
{
	...
	for(int iMipLevel = 0; iMipLevel < m_nMipCount; ++iMipLevel)
	{
		for(int iFrame = 0; iFrame < m_nFrameCount; ++iFrame)
		{
			...
			for(int iCornerMatch = 0; iCornerMatch < NUM_CORNER_MATCHES; iCornerMatch++)
			{
				BlendCubeMapFaceCorners(iFrame, iMipLevel, &cornerMatches[iCornerMatch]);
			}
		}
	}
}
