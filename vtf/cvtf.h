...
public:
	...
	virtual bool Init(int nWidth, int nHeight, int nDepth, ImageFormat fmt, int iFlags, int iFrameCount,
					  int nForceMipCount);
	...
	virtual void GenerateHemisphereMap(unsigned char *pSphereMapBitsRGBA, int targetWidth, int targetHeight,
	...
	virtual bool UnserializeFromBuffer(CUtlBuffer &buf, bool bBufferIsVolatile, bool bHeaderOnly, bool bPreloadOnly,
									   int nMipSkipCount);
...
private:
	...
	void SetupTextureEdgeIncrements(int iMipLevel, int iFace1Edge, int iFace2Edge, bool bFlipFace2Edge,
									CEdgeIncrements *incs);

	void BlendCubeMapFaceEdges(int iFrame, int iMipLevel, const CEdgeMatch *pMatch);
...
private:
	...
	CUtlVector<ResourceMemorySection>
		m_arrResourcesData_ForReuse; // Maintained to keep allocated memory blocks when unserializing from files
