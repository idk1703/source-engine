...
//-----------------------------------------------------------------------------
ImageFormat PreferredFormat(IVTFTexture *pVTFTexture, ImageFormat fmt, int width, int height, int mipCount,
							int faceCount)
{
	switch(fmt)
	{
...
//-----------------------------------------------------------------------------
bool ComputeTargetDimensions(const char *pDebugName, IVTFTexture *pVTFTexture, int picmip, int &width, int &height,
							 int &mipCount, int &mipSkipCount, bool &bNoMip)
{
	...
	int nClampX = 1 << 30;
	int nClampY = 1 << 30;
	TextureLODControlSettings_t const *pLODInfo = reinterpret_cast<TextureLODControlSettings_t const *>(
...
//-----------------------------------------------------------------------------
bool ConvertImageFormatEx(unsigned char *pSourceImage, int sourceImageSize, ImageFormat sourceFormat,
						  unsigned char *pTargetImage, int targetImageSize, ImageFormat targetFormat, int width,
						  int height, bool bSrgbGammaConvert)
{
	...
	bool bRetVal =
		ImageLoader::ConvertImageFormat(pSourceImage, sourceFormat, pTargetImage, targetFormat, width, height);
	if(!bRetVal)
	{
		return false;
	}
	...
	if(bSrgbGammaConvert == true)
	{
		...
		else if((targetFormat == IMAGE_FORMAT_DXT1_ONEBITALPHA) || (targetFormat == IMAGE_FORMAT_DXT1) ||
				(targetFormat == IMAGE_FORMAT_DXT3) || (targetFormat == IMAGE_FORMAT_DXT5))
		{
			...
			for(int i = 0; i < (targetImageSize / nStrideBytes); i++) // For each color or color/alpha block
			{
				...
				n565Color0 |= ((unsigned short)(unsigned char)(pTargetImage[(i * nStrideBytes) + nOffsetBytes + 0]))
							  << 8;
				n565Color0 |= ((unsigned short)(unsigned char)(pTargetImage[(i * nStrideBytes) + nOffsetBytes + 1]));

				unsigned short n565Color1 = 0;
				n565Color1 |= ((unsigned short)(unsigned char)(pTargetImage[(i * nStrideBytes) + nOffsetBytes + 2]))
							  << 8;
...
//-----------------------------------------------------------------------------
bool SerializeImageData(IVTFTexture *pSourceVTF, int frame, int face, int mip, ImageFormat targetFormat,
						CUtlBuffer &targetBuf)
{
	...
	// format conversion expects pc oriented data
	bool bRetVal =
		ConvertImageFormatEx(pSourceImage, sourceImageSize, pSourceVTF->Format(), pTargetImage, targetImageSize,
							 targetFormat, width, height, (pSourceVTF->Flags() & TEXTUREFLAGS_SRGB) ? true : false);
	if(!bRetVal)
	{
		return false;
	}
...
//-----------------------------------------------------------------------------
bool ConvertVTFTo360Format(const char *pDebugName, CUtlBuffer &sourceBuf, CUtlBuffer &targetBuf,
						   CompressFunc_t pCompressFunc)
{
	...
	if(!ComputeTargetDimensions(pDebugName, pSourceVTF, 0, targetWidth, targetHeight, targetMipCount, mipSkipCount,
								bNoMip))
		goto cleanUp;
	...
	targetFormat =
		PreferredFormat(pSourceVTF, pSourceVTF->Format(), targetWidth, targetHeight, targetMipCount, targetFaceCount);
	...
	targetBuf.SeekPut(CUtlBuffer::SEEK_CURRENT,
					  sizeof(VTFFileHeaderX360_t) + targetResources.Count() * sizeof(ResourceEntryInfo));
	...
	if(targetLowResWidth && targetLowResHeight)
	{
		...
		int sourceLowResImageSize = ImageLoader::GetMemRequired(pSourceVTF->LowResWidth(), pSourceVTF->LowResHeight(),
																1, pSourceVTF->LowResFormat(), false);
		int targetLowResImageSize =
			ImageLoader::GetMemRequired(targetLowResWidth, targetLowResHeight, 1, IMAGE_FORMAT_RGB888, false);
		...
		bRetVal = ConvertImageFormatEx(pSourceVTF->LowResImageData(), sourceLowResImageSize, pSourceVTF->LowResFormat(),
									   pTargetLowResImage, targetLowResImageSize, IMAGE_FORMAT_RGB888,
									   targetLowResWidth, targetLowResHeight, false);
		if(!bRetVal)
		{
			goto cleanUp;
		}
...
bool GetVTFPreload360Data(const char *pDebugName, CUtlBuffer &fileBufferIn, CUtlBuffer &preloadBufferOut)
{
	...
	if(V_strnicmp(header.fileTypeString, "VTFX", 4) || header.version[0] != VTF_X360_MAJOR_VERSION ||
