...
#define TGA_MAX_ROW_LENGTH_IN_PIXELS IMAGE_MAX_DIM
	...
	//-----------------------------------------------------------------------------
	bool GetInfo(char const *pFileName, int *width, int *height, ImageFormat *imageFormat, float *sourceGamma)
	{
		// temporary memory
		CTempImage image;
	...
	void OutputRowBGR565(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
		...
		{
			unsigned char *pSrc = (unsigned char *)buf.PeekGet();
			unsigned short rgba = (pSrc[2] & 0x1F) | ((pSrc[1] & 0x3F) << 5) | ((pSrc[0] & 0x1F) << 11);
	...
	void OutputRowBGRX5551(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
		...
		{
			unsigned char *pSrc = (unsigned char *)buf.PeekGet();
			unsigned short rgba = (pSrc[2] & 0x1F) | ((pSrc[1] & 0x1F) << 5) | ((pSrc[0] & 0x1F) << 10) | 0x8000;
...
static void InitDestGammaConversionTable( float dstGamma )
{
	...
	void ReadRow8BitUncompressedWithColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
		int i;
	...
	void ReadRow8BitUncompressedWithoutColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
		for(int i = 0; i < header.width; ++i, pDst += 4)
		{
	...
	void ReadRow24BitUncompressedWithoutColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
		for(int i = 0; i < header.width; ++i, pDst += 4)
		{
	...
	void ReadRow32BitUncompressedWithoutColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
		for(int i = 0; i < header.width; ++i, pDst += 4)
		{
			pDst[2] = buf.GetUnsignedChar();
	...
	//-----------------------------------------------------------------------------
	void ReadRow8BitCompressedWithColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
	...
	//-----------------------------------------------------------------------------
	void ReadRow8BitCompressedWithoutColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
		unsigned char rowI_8[TGA_MAX_ROW_LENGTH_IN_PIXELS];
	...
	void ReadRow24BitCompressedWithoutColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
	...
	void ReadRow32BitCompressedWithoutColormap(CUtlBuffer &buf, TGAHeader_t const &header, unsigned char *pDst)
	{
	...
	static bool ReadSourceImage(CUtlBuffer &buf, TGAHeader_t &header, CTempImage &image)
	{
		...
		int memRequired = ImageLoader::GetMemRequired(header.width, header.height, 1, IMAGE_FORMAT_RGBA8888, false);
		...
		if(header.attributes & 0x20)
		{
			...
			{
				pDstBits = image.Base() + row * header.width * ImageLoader::SizeInBytes(IMAGE_FORMAT_RGBA8888);
						 ...
						 ImageFormat imageFormat, unsigned char* pDst )
{
	...
	//-----------------------------------------------------------------------------
	bool Load(unsigned char *pOutputImage, CUtlBuffer &buf, int width, int height, ImageFormat imageFormat,
			  float targetGamma, bool mipmap)
	{
		...
		ImageLoader::GenerateMipmapLevels(tmpImage.Base(), pOutputImage, header.width, header.height, 1, imageFormat,
	...
	//-----------------------------------------------------------------------------
	bool Load(unsigned char *pOutputImage, const char *pFileName, int width, int height, ImageFormat imageFormat,
			  float targetGamma, bool mipmap)
	{
	...
	bool LoadRGBA8888(CUtlBuffer &buf, CUtlMemory<unsigned char> &outputData, int &outWidth, int &outHeight)
	{
		...
		// Stores the RGBA8888 temp version of the image which we'll use to
		// to do mipmapping...
		int memSize = ImageLoader::GetMemRequired(header.width, header.height, 1, IMAGE_FORMAT_RGBA8888, false);
