...
namespace ImageLoader
{
	...
	void GammaCorrectRGBA8888(unsigned char *src, unsigned char *dst, int width, int height, int depth, float srcGamma,
							  float dstGamma)
	{
	...
	//-----------------------------------------------------------------------------
	static void GenerateNiceFilter(float wratio, float hratio, float dratio, int kernelDiameter, float *pKernel,
								   float *pInvKernel)
	{
		// Compute a kernel...
		int h, i, j;
	...
	typedef void (*ApplyKernelFunc_t)(const KernelInfo_t &kernel, const ResampleInfo_t &info, int wratio, int hratio,
									  int dratio, float *gammaToLinear, float *pAlphaResult);
	...
	public:
		...
		static void ComputeAveragedColor(const KernelInfo_t &kernel, const ResampleInfo_t &info, int startX, int startY,
										 int startZ, float *gammaToLinear, float *total)
		{
		...
		static void AddAlphaToAlphaResult(const KernelInfo_t &kernel, const ResampleInfo_t &info, int startX,
										  int startY, int startZ, float flAlpha, float *pAlphaResult)
		{
			for(int j = 0, srcZ = startZ; j < kernel.m_nDepth; ++j, ++srcZ)
			{
				int sz = ActualZ(srcZ, info);
		...
		static void AdjustAlphaChannel(const KernelInfo_t &kernel, const ResampleInfo_t &info, int wratio, int hratio,
									   int dratio, float *pAlphaResult)
		{
			...
			float flAlphaThreshhold =
				(info.m_flAlphaHiFreqThreshhold >= 0) ? 255.0f * info.m_flAlphaHiFreqThreshhold : 255.0f * 0.4f;
		...
		static void ApplyKernel(const KernelInfo_t &kernel, const ResampleInfo_t &info, int wratio, int hratio,
								int dratio, float *gammaToLinear, float *pAlphaResult)
		{
			...
			float flAlphaThreshhold =
				(info.m_flAlphaThreshhold >= 0) ? 255.0f * info.m_flAlphaThreshhold : 255.0f * 0.4f;
					...
					for(int j = 0; j < info.m_nDestWidth; ++j, dstPixel += 4)
					{
							...
							for(int ch = 0; ch < 4; ++ch)
								info.m_pDest[dstPixel + ch] =
									Clamp(info.m_flColorGoal[ch] +
										  (info.m_flColorScale[ch] * (total[ch] - info.m_flColorGoal[ch])));
						}
						else if(type == KERNEL_ALPHATEST)
						{
							// If there's more than 40% coverage, then keep the pixel (renormalize the color based on
							// coverage)
								...
								info.m_pDest[dstPixel + ch] =
									Clamp(255.0f * pow((info.m_flColorGoal[ch] +
														(info.m_flColorScale[ch] *
														 ((total[ch] > 0 ? total[ch] : 0) - info.m_flColorGoal[ch]))) /
														   255.0f,
													   invDstGamma));
						...
						else
						{
							...
							info.m_pDest[dstPixel + 3] = Clamp(
								info.m_flColorGoal[3] + (info.m_flColorScale[3] * (total[3] - info.m_flColorGoal[3])));
						}
					}
				}
	...
	bool ResampleRGBA8888(const ResampleInfo_t &info)
	{
		   ...
		   info.m_nSrcDepth == info.m_nDestDepth)
		{
			...
			GammaCorrectRGBA8888(info.m_pSrc, info.m_pDest, info.m_nSrcWidth, info.m_nSrcHeight, info.m_nSrcDepth,
								 info.m_flSrcGamma, info.m_flDestGamma);
			return true;
		}
		...
		if(info.m_nFlags & RESAMPLE_NICE_FILTER)
		{
			...
			if(power >= 0)
			{
				if(!kernelCache[power])
				{
					...
					GenerateNiceFilter(wratio, hratio, dratio, kernel.m_nDiameter, kernelCache[power],
									   pInvKernelCache[power]);
				}
		...
		for(y = 0; y < info.m_nDestHeight; y++)
		{
			for(x = 0; x < info.m_nDestWidth; x++)
			{
				...
				for(nSampleY = 0; nSampleY < nSampleHeight; nSampleY++)
				{
					...
					{
						accum[0] += (int)
							pSrc[((x * nSampleWidth + nSampleX) + (y * nSampleHeight + nSampleY) * info.m_nSrcWidth) *
									 4 +
								 0];
						accum[1] += (int)
							pSrc[((x * nSampleWidth + nSampleX) + (y * nSampleHeight + nSampleY) * info.m_nSrcWidth) *
	...
	void GenerateMipmapLevels(unsigned char *pSrc, unsigned char *pDst, int width, int height, int depth,
							  ImageFormat imageFormat, float srcGamma, float dstGamma, int numLevels)
	{
		int dstWidth = width;
		...
		do
		{
			...
			for(int j = 0; j < dstHeight; ++j)
			{
				for(int i = 0; i < dstWidth; ++i)
				{
					...
					pDstPixel[0] = ((unsigned int)pSrcPixel[0] + (unsigned int)pSrcPixel[0 + cSrcStride] +
									(unsigned int)pSrcPixel[0 + cSrcPitch] +
									(unsigned int)pSrcPixel[0 + cSrcPitch + cSrcStride]) >>
								   2;
					pDstPixel[1] = ((unsigned int)pSrcPixel[1] + (unsigned int)pSrcPixel[1 + cSrcStride] +
									(unsigned int)pSrcPixel[1 + cSrcPitch] +
									(unsigned int)pSrcPixel[1 + cSrcPitch + cSrcStride]) >>
								   2;
					pDstPixel[2] = ((unsigned int)pSrcPixel[2] + (unsigned int)pSrcPixel[2 + cSrcStride] +
									(unsigned int)pSrcPixel[2 + cSrcPitch] +
									(unsigned int)pSrcPixel[2 + cSrcPitch + cSrcStride]) >>
								   2;
					pDstPixel[3] = ((unsigned int)pSrcPixel[3] + (unsigned int)pSrcPixel[3 + cSrcStride] +
									(unsigned int)pSrcPixel[3 + cSrcPitch] +
									(unsigned int)pSrcPixel[3 + cSrcPitch + cSrcStride]) >>
								   2;
