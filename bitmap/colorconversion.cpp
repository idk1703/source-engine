...
#pragma pack()
	...
	static inline void GetColorBlockColorsBGRA8888(DXTColBlock *pBlock, BGRA8888_t *col_0, BGRA8888_t *col_1,
												   BGRA8888_t *col_2, BGRA8888_t *col_3, WORD &wrd)
	{
	...
	static inline void DecodeColorBlock(CDestPixel *pOutputImage, DXTColBlock *pColorBlock, int width,
										BGRA8888_t *col_0, BGRA8888_t *col_1, BGRA8888_t *col_2, BGRA8888_t *col_3)
	{
	...
	template<class CDestPixel>
	static inline void DecodeAlpha3BitLinear(CDestPixel *pImPos, DXTAlphaBlock3BitLinear *pAlphaBlock, int width,
											 int nChannelSelect = 3)
	{
	...
	bool ConvertToATIxN(const uint8 *src, ImageFormat srcImageFormat, uint8 *dst, ImageFormat dstImageFormat, int width,
						int height, int srcStride, int dstStride)
	{
		...
		destTexture.format = dstImageFormat == IMAGE_FORMAT_ATI2N
								 ? ATI_TC_FORMAT_ATI2N
	...
	bool ConvertToDXTLegacy(const uint8 *src, ImageFormat srcImageFormat, uint8 *dst, ImageFormat dstImageFormat,
	...
	bool ConvertToDXTRuntime(const uint8 *src, ImageFormat srcImageFormat, uint8 *dst, ImageFormat dstImageFormat,
							 int width, int height, int srcStride, int dstStride)
	{
		if(srcStride != 0 || dstStride != 0)
			return false;
		...
		{
			case IMAGE_FORMAT_RGBA8888:
				CompressSTB<RGBA8888_t>(dst, dstImageFormat, src, width, height);
				return true;
			case IMAGE_FORMAT_RGB888:
				CompressSTB<RGB888_t>(dst, dstImageFormat, src, width, height);
				return true;
			case IMAGE_FORMAT_BGRA8888:
				CompressSTB<BGRA8888_t>(dst, dstImageFormat, src, width, height);
				return true;
			case IMAGE_FORMAT_BGRX8888:
				CompressSTB<BGRX8888_t>(dst, dstImageFormat, src, width, height);
				return true;
			default:
	...
	bool ConvertToDXT(const uint8 *src, ImageFormat srcImageFormat, uint8 *dst, ImageFormat dstImageFormat, int width,
					  int height, int srcStride, int dstStride)
	{
		// The STB compressor (the new compressor) is faster and higher quality in most cases, and has less error
		// overall than the S3TC compressor. So use it by default, unless we're working with a format that STB doesn't
		// support.
		bool bUseNewCompressor = dstImageFormat != IMAGE_FORMAT_DXT1_ONEBITALPHA && dstImageFormat != IMAGE_FORMAT_DXT3;
	...
	void ConvertImageFormat_RGBA16161616_To_RGB323232F(unsigned short *pSrcImage, float *pDstImage, int width,
													   int height)
	{
	...
	// HDRFIXME: This assumes that the 16-bit integer values are 4.12 fixed-point.
	void ConvertImageFormat_RGB323232F_To_RGBA16161616(float *pSrcImage, unsigned short *pDstImage, int width,
													   int height)
	{
	...
	void ConvertImageFormat_RGBA16161616F_To_RGBA323232F(float16 *pSrcImage, float *pDstImage, int width, int height,
														 size_t src_stride)
	{
	...
	// HDRFIXME: This assumes that the 16-bit integer values are 4.12 fixed-point.
	void ConvertImageFormat_RGBA16161616_To_RGBA16161616F(unsigned short *pSrcImage, float *pDstImage, int width,
														  int height)
	{
		int srcSize = width * height * 4;
		unsigned short *pSrcEnd = pSrcImage + srcSize;
	...
	void ConvertImageFormat_RGBA16161616F_To_RGBA16161616(float16 *pSrcImage, unsigned short *pDstImage, int width,
														  int height)
	{
		int srcSize = width * height * 4;
		float16 *pSrcEnd = pSrcImage + srcSize;
	...
	bool ConvertImageFormat(const uint8 *src, ImageFormat srcImageFormat, uint8 *dst, ImageFormat dstImageFormat,
							int width, int height, int srcStride, int dstStride)
	{
		...
		if(((dstImageFormat == IMAGE_FORMAT_DXT1 || dstImageFormat == IMAGE_FORMAT_DXT1_RUNTIME ||
			 ...
			 dstImageFormat == IMAGE_FORMAT_DXT5_RUNTIME || dstImageFormat == IMAGE_FORMAT_ATI1N ||
		   ...
		   (dstImageFormat == IMAGE_FORMAT_DXT1_RUNTIME && srcImageFormat == IMAGE_FORMAT_DXT1))
		{
			...
			return true;
		...
		else if((srcImageFormat == IMAGE_FORMAT_RGBA8888 || srcImageFormat == IMAGE_FORMAT_RGB888 || // RGBA source
				...
				(dstImageFormat == IMAGE_FORMAT_DXT1 ||												 //
				 ...
				 dstImageFormat == IMAGE_FORMAT_DXT5 || dstImageFormat == IMAGE_FORMAT_DXT1_RUNTIME ||
				 dstImageFormat == IMAGE_FORMAT_DXT5_RUNTIME))
		{
		...
		else if((dstImageFormat == IMAGE_FORMAT_RGBA8888 || dstImageFormat == IMAGE_FORMAT_BGRX8888 ||
				 dstImageFormat == IMAGE_FORMAT_BGRA8888 || dstImageFormat == IMAGE_FORMAT_BGRA4444 ||
				 dstImageFormat == IMAGE_FORMAT_BGRA5551 || dstImageFormat == IMAGE_FORMAT_BGRX5551 ||
				 dstImageFormat == IMAGE_FORMAT_BGR565 || dstImageFormat == IMAGE_FORMAT_BGR888 ||
				 dstImageFormat == IMAGE_FORMAT_RGB888) &&
				(srcImageFormat == IMAGE_FORMAT_DXT1 || srcImageFormat == IMAGE_FORMAT_DXT3 ||
				 srcImageFormat == IMAGE_FORMAT_DXT5 || srcImageFormat == IMAGE_FORMAT_ATI1N ||
				 srcImageFormat == IMAGE_FORMAT_ATI2N))
		{
			...
			if(srcImageFormat == IMAGE_FORMAT_DXT1)
			{
				...
				if(dstImageFormat == IMAGE_FORMAT_BGRA8888 || dstImageFormat == IMAGE_FORMAT_BGRX8888)
				{
				...
				if(dstImageFormat == IMAGE_FORMAT_BGRA5551 || dstImageFormat == IMAGE_FORMAT_BGRX5551)
				{
		...
		else if(dstImageFormat == IMAGE_FORMAT_DXT1 || dstImageFormat == IMAGE_FORMAT_DXT3 ||
				dstImageFormat == IMAGE_FORMAT_DXT5 || dstImageFormat == IMAGE_FORMAT_ATI1N ||
				dstImageFormat == IMAGE_FORMAT_ATI2N || srcImageFormat == IMAGE_FORMAT_DXT1 ||
				srcImageFormat == IMAGE_FORMAT_DXT3 || srcImageFormat == IMAGE_FORMAT_DXT5 ||
				srcImageFormat == IMAGE_FORMAT_ATI1N || srcImageFormat == IMAGE_FORMAT_ATI2N)
		{
			// DxtN to DxtN
			Assert(IsPC());
			return false;
