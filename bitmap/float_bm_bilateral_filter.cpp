			...
			for(int c = 0; c < 4; c++)
			{
				...
				for(int iy = -ctx->radius_in_pixels; iy <= ctx->radius_in_pixels; iy++)
					for(int ix = -ctx->radius_in_pixels; ix <= ctx->radius_in_pixels; ix++)
					{
						...
						s = (cdiff > ctx->edge_threshold_value) ? 0 : FLerp(1, 0, 0, ctx->edge_threshold_value, cdiff);
						sum_weights += s * g;
...
void FloatBitMap_t::TileableBilateralFilter(int radius_in_pixels, float edge_threshold_value)
{
