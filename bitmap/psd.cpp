...
//	byte			palette[ numBytesPalette ]; = { (all red palette entries), (all green palette entries), (all blue palette
//entries) }, where numEntries = numBytesPalette/3; 	unsigned int	numBytesImgResources; 	byte			imgresources[
//numBytesImgResources ]; = { sequence of PSDImgResEntry_t } 	unsigned int	numBytesLayers; 	byte			layers[
//numBytesLayers ]; 	unsigned short	uCompressionInfo; 	< ~ image data ~ >
//
// END PSD FILE
//
//////////////////////////////////////////////////////////////////////////
...
bool PSDGetInfo(const char *pFileName, const char *pPathID, int *pWidth, int *pHeight, ImageFormat *pImageFormat,
				float *pSourceGamma)
{
	CUtlBuffer buf;
	if(!g_pFullFileSystem->ReadFile(pFileName, pPathID, buf, sizeof(PSDHeader_t)))
...
static void PSDReadUncompressedChannels(CUtlBuffer &buf, int nChannelsCount, PSDMode_t mode, PSDPalette_t &palette,
										Bitmap_t &bitmap)
{
...
//-----------------------------------------------------------------------------
static void PSDReadCompressedChannels(CUtlBuffer &buf, int nChannelsCount, PSDMode_t mode, PSDPalette_t &palette,
									  Bitmap_t &bitmap)
{
	unsigned char *pChannelRow = (unsigned char *)_alloca(bitmap.Width());
	for(int i = 0; i < nChannelsCount; ++i)
