...
static unsigned SSBumpCalculationThreadFN(void *ctx1)
{
	...
	RayTracingSingleResult *rslts = new RayTracingSingleResult[ctx->ret_bm->Width * ctx->nrays_to_trace_per_pixel];
		...
		for(int x = 0; x < ctx->src_bm->Width; x++)
		{
			...
			for(int c = 0; c < nNumChannels; c++)
			{
				...
				else
				{
					if(ctx->nrays_to_trace_per_pixel)
					{
						RayTracingSingleResult *this_rslt = rslts + ctx->nrays_to_trace_per_pixel * (x);
						for(int r = 0; r < ctx->nrays_to_trace_per_pixel; r++)
						{
							float dot;
...
FloatBitMap_t *FloatBitMap_t::ComputeSelfShadowedBumpmapFromHeightInAlphaChannel(float bump_scale,
																				 int nrays_to_trace_per_pixel,
