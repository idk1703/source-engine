...
float FloatBitMap_t::InterpolatedPixel(float x, float y, int comp) const
{
	...
	return BiLinInterp(Xfrac, Yfrac, Pixel(Left, Top, comp), Pixel(Right, Top, comp), Pixel(Left, Bot, comp),
...
void FloatBitMap_t::ReSize(int NewWidth, int NewHeight)
{
	...
	for(int y = 0; y < NewHeight; y++)
	{
		...
		for(int x = 0; x < NewWidth; x++)
		{
			...
			for(int c = 0; c < 4; c++)
			{
				newrgba[4 * (y * NewWidth + x) + c] = BiLinInterp(
					Xfrac, Yfrac, Pixel(Left, Top, c), Pixel(Right, Top, c), Pixel(Left, Bot, c), Pixel(Right, Bot, c));
			}
			SourceX += XRatio;
		}
		SourceY += YRatio;
...
FloatBitMap_t::FloatBitMap_t(char const *tgafilename)
{
	...
	uint8 *pImage1 = new uint8[ImageLoader::GetMemRequired(width1, height1, 1, IMAGE_FORMAT_ABGR8888, false)];
...
void FloatBitMap_t::SmartPaste(FloatBitMap_t const &b, int xofs, int yofs, uint32 Flags)
{
	...
	for(int x = 0; x < Width; x++)
		...
		{
			float modify = 0;
			if((x > xofs + 1) && (x <= xofs + b.Width - 2) && (y > yofs + 1) && (y <= yofs + b.Height - 2))
				modify = 1;
			Alpha(x, y) = modify;
		}
