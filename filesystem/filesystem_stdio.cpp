...
#endif
private:
	CStdioFile(FILE *pFile, bool bWriteable) : m_pFile(pFile), m_bWriteable(bWriteable) {}
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CFileSystem_Stdio, IBaseFileSystem, BASEFILESYSTEM_INTERFACE_VERSION,
								  g_FileSystem_Stdio);
...
bool UseOptimalBufferAllocation()
{
	static bool bUseOptimalBufferAllocation =
		(IsX360() || (!IsLinux() && Q_stristr(Plat_GetCommandLine(), "-unbuffered_io") != NULL));
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CFileSystem_Stdio::GetOptimalIOConstraints(FileHandle_t hFile, unsigned *pOffsetAlign, unsigned *pSizeAlign,
												unsigned *pBufferAlign)
{
	unsigned sectorSize;
...
static int WindowsXPStatShim(const char *pathT, struct _stat *buf)
{
	...
	else
	{
		...
		if(V_strcasecmp(pExt, "exe") == 0 || V_strcasecmp(pExt, "bat") == 0 || V_strcasecmp(pExt, "com") == 0 ||
	...
	uint64_t actualAccessTime = (uint64_t)fileAttributes.ftLastAccessTime.dwHighDateTime << 32 |
								(uint64_t)fileAttributes.ftLastAccessTime.dwLowDateTime;
	uint64_t actualModTime = (uint64_t)fileAttributes.ftLastWriteTime.dwHighDateTime << 32 |
							 (uint64_t)fileAttributes.ftLastWriteTime.dwLowDateTime;
	uint64_t actualCreationTime = (uint64_t)fileAttributes.ftCreationTime.dwHighDateTime << 32 |
...
// Purpose: steam call, unnecessary in stdio
//-----------------------------------------------------------------------------
bool CFileSystem_Stdio::GetWaitForResourcesProgress(WaitForResourcesHandle_t handle, float *progress /* out */,
													bool *complete /* out */)
{
#if defined(DEBUG_WAIT_FOR_RESOURCES_API)
	...
	if(pFile)
	{
		...
		if(bWriteable)
		{
...

			{
				...
				// thread can open a file at a time) so add a lock here to mimic that behavior
...
size_t CStdioFile::FS_fwrite(const void *src, size_t size)
{
	...
	return fwrite(
		src, 1, size,
		m_pFile); // return number of bytes written (because we have size = 1, count = bytes, so it return bytes)
}
...
{
	if((!pszFilename[0] || !pszFilename[1]) || (pszFilename[0] == '\\' && pszFilename[1] == '\\') ||
...
size_t CWin32ReadOnlyFile::FS_fread(void *dest, size_t destSize, size_t size)
{
	...
	if(m_hFileUnbuffered != INVALID_HANDLE_VALUE)
	{
		...
		bool bCanReadUnbufferedDirect =
			(bDestBaseIsAligned && (destSize % m_SectorSize == 0) && (m_ReadPos % m_SectorSize == 0));
