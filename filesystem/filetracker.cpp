	...
	while(m_bThreadShouldRun)
	{
		...
		while(m_PendingJobs.PopItem(&stuff))
		{
...

			{
				...
				memcpy(trackedfile.m_filehashFinal.m_md5contents.bits, stuff.m_md5Value.bits,
					   sizeof(trackedfile.m_filehashFinal.m_md5contents.bits));
...
//-----------------------------------------------------------------------------
int CFileTracker2::SubmitThreadedMD5Request(uint8 *pubBuffer, int cubBuffer, int PackFileID, int nPackFileNumber,
...
CFileTracker2::CFileTracker2(CBaseFileSystem *pFileSystem)
	: m_treeAllOpenedFiles(TrackedFile_t::Less), m_treeTrackedVPKFiles(TrackedVPKFile_t::Less)
{
#if defined(DEDICATED)
	Assert(0);
#endif
...
EFileCRCStatus CFileTracker2::CheckCachedFileHash(const char *pPathID, const char *pRelativeFilename, int nFileFraction,
												  FileHash_t *pFileHash)
{
	...
	if(idx != m_treeAllOpenedFiles.InvalidIndex())
	{
		...
		if(trackedfile.m_bFileInVPK)
		{
			...
			// if the user is sending us a hash for this file, it means he has extracted it from the VPK and tricked the
			// client into loading it instead of the version in the VPK.
...
void TrackedFile_t::RebuildFileName(CStringPool &stringPool, const char *pFilename, const char *pPathID,
									int nFileFraction)
{
...
void CFileTracker2::NotePackFileAccess(const char *pFilename, const char *pPathID, int iSearchPathStoreId,
...
IFileList *CFileTracker2::GetFilesToUnloadForWhitelistChange(IPureServerWhitelist *pNewWhiteList)
{
	...
	for(int i = m_treeAllOpenedFiles.FirstInorder(); i >= 0; i = m_treeAllOpenedFiles.NextInorder(i))
	{
		...
		if(f.m_iLoadedSearchPathStoreId != 0 && pNewWhiteList &&
		   pNewWhiteList->GetFileClass(f.m_filename) == ePureServerFileClass_AnyTrusted)
		{
			// Check if we loaded it from a path that no longer exists or is no longer trusted
			const CBaseFileSystem::CSearchPath *pSearchPath =
				m_pFileSystem->FindSearchPathByStoreId(f.m_iLoadedSearchPathStoreId);
			if(pSearchPath == NULL)
			{
#ifdef PURE_SERVER_DEBUG_SPEW
...
void CFileTracker2::AddFileHashForVPKFile(int nPackFileNumber, int nFileFraction, int cbFileLen, MD5Value_t &md5,
										  CPackedStoreFileHandle &VPKHandle)
{
#if !defined(DEDICATED)
	AUTO_LOCK(m_Mutex);
...
void CFileTracker2::NoteFileLoadedFromDisk(const char *pFilename, const char *pPathID, int iSearchPathStoreId, FILE *fp,
										   int64 nLength)
{
#if !defined(_GAMECONSOLE) && !defined(DEDICATED)
	AUTO_LOCK(m_Mutex);
...
int CFileTracker2::ListOpenedFiles(bool bAllOpened, const char *pchFilenameFind)
{
	...
	while(i != InvalidIndex)
	{
		...
		if(file.m_PackFileID)
			cPackFiles++;
		if(!pchFilenameFind || Q_strstr(file.m_filename, pchFilenameFind) || Q_strstr(file.m_path, pchFilenameFind))
		{
			Msg("%d %s %s ( %d, %d ) %d %d%s%s\n", count, file.m_path, file.m_filename, file.m_PackFileID,
				file.m_nPackFileNumber, file.m_filehashFinal.m_cbFileLen,
				file.m_nFileFraction /*, file.m_crcIdentifier*/, file.m_bFileInVPK ? " (invpk)" : "",
	...
	Msg("TrackedVPKFiles:%d AllOpenedFiles:%d files VPKfiles:%d StringPoolCount:%d\n", m_treeTrackedVPKFiles.Count(),
		m_treeAllOpenedFiles.Count(), cPackFiles, m_stringPool.Count());
