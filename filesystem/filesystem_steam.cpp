...
void CheckError(SteamHandle_t fp, TSteamError &steamError)
{
	if(steamError.eSteamError == eSteamErrorContentServerConnect)
	{
		...
		// show the error
		MessageBox(NULL, "Could not acquire necessary game files because the connection to Steam servers was lost.",
...
{
public:
	explicit CSteamFile(SteamHandle_t file, bool bWriteable, const char *pchName)
		: m_File(file), m_bWriteable(bWriteable), m_FileName(pchName)
	{
	}
	~CSteamFile() {}
	SteamHandle_t Handle()
	{
		return m_File;
	}
	bool BWriteable()
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CFileSystem_Steam, IBaseFileSystem, BASEFILESYSTEM_INTERFACE_VERSION,
								  g_FileSystem_Steam);
...
void CFileSystem_Steam::LoadAndStartSteam()
{
	...
	if(!steam)
	{
		...
		::MessageBox(NULL, "CFileSystem_Steam::Init() failed: failed to find steam interface",
	...
	TSteamError steamError;
	if(!steam->Startup(STEAM_USING_FILESYSTEM | STEAM_USING_LOGGING | STEAM_USING_USERID | STEAM_USING_ACCOUNT,
					   &steamError))
	{
...
FilesystemMountRetval_t CFileSystem_Steam::MountSteamContent(int nExtraAppId)
{
	...
	// MWD: This is here because of Hammer's funky startup sequence that requires MountSteamContent() be called in
	// CHammerApp::PreInit(). Once that root problem is addressed this will be removed;
	if(NULL == steam)
	{
		LoadAndStartSteam();
	}
	...
	// this enables both the game and the platform to use this same code, even though they mount caches at different
	// times
	int loggedIn = 0;
	TSteamError steamError;
...
//-----------------------------------------------------------------------------
FILE *CFileSystem_Steam::FS_fopen(const char *filenameT, const char *options, unsigned flags, int64 *size,
								  CFileLoadInfo *pInfo)
{
	...
	if(bWriteable)
	{
		...
		if(pFile)
		{
			...
			// Win32 has an undocumented feature that is serialized ALL writes to a file across threads (i.e only 1
			// thread can open a file at a time) so use flock here to mimic that behavior
...

			{
				...
				if(ret < 0)
				{
					...
					{
						if(iLockID != m_LockedFDMap.InvalidIndex() && m_LockedFDMap[iLockID] != -1 &&
...
// Purpose: steam call, unnecessary in stdio
//-----------------------------------------------------------------------------
bool CFileSystem_Steam::GetWaitForResourcesProgress(WaitForResourcesHandle_t handle, float *progress /* out */,
													bool *complete /* out */)
{
	// clear the input
	*progress = 0.0f;
...
#define WRITE_CHUNK (256 * 1024)
		...
		return fwrite(
			src, 1, size,
			fp); // return number of bytes written (because we have size = 1, count = bytes, so it return bytes)
	}
	else
	{
#endif
...
CSysModule *CFileSystem_Steam::LoadModule(const char *pFileName, const char *pPathID, bool bValidatedDllOnly)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		if(m_SearchPaths[i].GetPathID() == lookup)
		{
			...
			Q_snprintf(newPathName, sizeof(newPathName), "%s%s", m_SearchPaths[i].GetPathString(),
...
extern "C" SteamHandle_t SteamFindFirst(const char *cszPattern, ESteamFindFilter eFilter, TSteamElemInfo *pFindInfo,
										TSteamError *pError);
