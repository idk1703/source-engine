...
// How many bytes compressed filehandles should attempt to read (and cache) at a time from the underlying compressed
// data.
...
{
public:
	CZipPackFileHandle(CZipPackFile *pOwner, int64 nBase, unsigned int nLength, unsigned int nIndex = -1,
...
private:
	...
	// Contains the last PACKFILE_COMPRESSED_FILEHANDLE_SEEK_BUFFER decompressed bytes. The Put and Get locations mimic
	// our filehandle -- TellPut() == TelGet() when we are not back seeking.
...
public:
	...
	// Returns the filename for a given file in the pack. Returns true if a filename is found, otherwise buffer is
	// filled with "unknown"
	...
	virtual void GetFileAndDirLists(const char *pFindWildCard, CUtlStringList &outDirnames,
									CUtlStringList &outFilenames, bool bSortedOutput) = 0;
	...
	// Path management:
	void SetPath(const CUtlSymbol &path)
	{
		m_Path = path;
	}
...
protected:
	// This is the core IO routine for reading anything from a pack file, everything should go through here at some
	// point
...
public:
	...
	virtual void GetFileAndDirLists(const char *pFindWildCard, CUtlStringList &outDirnames,
									CUtlStringList &outFilenames, bool bSortedOutput) OVERRIDE;

	virtual int64 GetPackFileBaseOffset() OVERRIDE
	{
		return m_nBaseOffset;
	}
...
#pragma pack()
	...
	bool GetFileInfo(const char *pFileName, int &nBaseIndex, int64 &nFileOffset, int &nOriginalSize,
