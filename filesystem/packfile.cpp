...
//-----------------------------------------------------------------------------
bool CZipPackFile::GetFileInfo(const char *pFileName, int &nBaseIndex, int64 &nFileOffset, int &nOriginalSize,
							   int &nCompressedSize, unsigned short &nCompressionMethod)
{
...
//-----------------------------------------------------------------------------
void CZipPackFile::GetFileAndDirLists(const char *pRawWildCard, CUtlStringList &outDirnames,
									  CUtlStringList &outFilenames, bool bSortedOutput)
{
	...
	// Workaround edge case in crappy path code. ExtractFilePath extracts a/b/ from a/b/c/ but FileBase would return the
	// empty string.
	...
	FOR_EACH_VEC(m_PackFiles, filesIdx)
	{
		...
		CUtlDict<int, int>
			ConsideredDirectories; // Will have duplicate directory matches when multiple files reside in them
...
void CZipPackFile::SetupPreloadData()
{
	...
	m_pPreloadRemapTable = (unsigned short *)((byte *)m_pPreloadDirectory + m_pPreloadHeader->PreloadDirectoryEntries *
...
bool CZipPackFile::Prepare(int64 fileLen, int64 nFileOfs)
{
	...
	if(!V_stricmp(filename, PRELOAD_SECTION_NAME))
	{
		...
		m_nPreloadSectionOffset = zipFileHeader.relativeOffsetOfLocalHeader + sizeof(ZIP_LocalFileHeader) +
	...
	for(int i = firstFileIdx; i < numFilesInZip; ++i)
	{
		...
		if(zipFileHeader.compressionMethod != ZIP_COMPRESSION_NONE &&
		   zipFileHeader.compressionMethod != ZIP_COMPRESSION_LZMA)
		{
		...
		lookup.m_nPosition = zipFileHeader.relativeOffsetOfLocalHeader + sizeof(ZIP_LocalFileHeader) +
							 zipFileHeader.fileNameLength + zipFileHeader.extraFieldLength;
...
//
//-----------------------------------------------------------------------------
CZipPackFile::CZipPackFile(CBaseFileSystem *fs, void *pSection) : m_PackFiles()
{
	m_fs = fs;
...
//-----------------------------------------------------------------------------
bool CZipPackFile::CPackFileLessFunc::Less(CZipPackFile::CPackFileEntry const &src1,
...
//-----------------------------------------------------------------------------
CZipPackFileHandle::CZipPackFileHandle(CZipPackFile *pOwner, int64 nBase, unsigned int nLength, unsigned int nIndex,
...
CLZMAZipPackFileHandle::CLZMAZipPackFileHandle(CZipPackFile *pOwner, int64 nBase, unsigned int nOriginalSize,
											   unsigned int nCompressedSize, unsigned int nIndex,
											   unsigned int nFilePointer)
