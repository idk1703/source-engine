...
CBaseFileSystem::CBaseFileSystem() : m_FileTracker2(this)
{
...
InitReturnVal_t CBaseFileSystem::Init()
{
	...
	// This is a special tag to allow iterating just the BSP file, it doesn't show up in the list per se, but gets
	// converted to "GAME" in the filter function
...
//-----------------------------------------------------------------------------
inline void CBaseFileSystem::ComputeFullWritePath(char *pDest, int maxlen, const char *pRelativePath,
...
void CBaseFileSystem::LogAccessToFile(char const *accesstype, char const *fullpath, char const *options)
{
	...
	if(m_fwLevel >= FILESYSTEM_WARNING_REPORTALLACCESSES && !V_stristr(fullpath, "console.log"))
	{
		Warning(FILESYSTEM_WARNING_REPORTALLACCESSES, "---FS%s:  %s %s (%.3f)\n", ThreadInMainThread() ? "" : "[a]",
				accesstype, fullpath, Plat_FloatTime());
	}
...
{
	AUTOBLOCKREPORTER_FN(Trace_FOpen, this, true, filenameT, FILESYSTEM_BLOCKING_SYNCHRONOUS,
		...
		if(result != -1 /*m_OpenedFiles.InvalidIdx()*/)
		{
			...
			if(m_fwLevel >= FILESYSTEM_WARNING_REPORTALLACCESSES && !V_stristr(found.GetName(), "console.log"))
			{
				Warning(FILESYSTEM_WARNING_REPORTALLACCESSES, "---FS%s:  close %s %p %i (%.3f)\n",
						ThreadInMainThread() ? "" : "[a]", found.GetName(), fp, m_OpenedFiles.Count(),
						Plat_FloatTime());
			}
...
void CBaseFileSystem::AddVPKFile(char const *pPath, const char *pPathID, SearchPathAdd_t addType)
{
	...
	CSearchPath *sp =
		&m_SearchPaths[(addType == PATH_ADD_TO_TAIL) ? m_SearchPaths.AddToTail() : m_SearchPaths.AddToHead()];
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseFileSystem::AddPackFileFromPath(const char *pPath, const char *pakfile, bool bCheckForAppendedPack,
										  const char *pathID)
{
...
void CBaseFileSystem::AddMapPackFile(const char *pPath, const char *pPathID, SearchPathAdd_t addType)
{
	...
	int iStoreId = CRC32_ProcessSingleBuffer(fullpath, V_strlen(fullpath)) |
				   0x80000000; // set store ID based on .bsp filename, so if we remount the same map file, it will have
							   // the same storeid
		...
		for(int i = 0; i < c; i++)
		{
			...
			if(pPackFile && pPackFile->m_bIsMapPath)
			{
				...
				if(pPackFile->m_nOpenFiles == 0 && pPackFile->m_hPackFileHandleFS == NULL &&
				   !pPackFile->m_hPackFileHandleVPK)
#else
				if(pPackFile->m_nOpenFiles == 0 && pPackFile->m_hPackFileHandleFS == NULL)
#endif
				{
...
void CBaseFileSystem::PrintSearchPaths(void)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		Msg("\"%s\" \"%s\" %s%s\n", pSearchPath->GetPathString(), (const char *)pSearchPath->GetPathIDString(), pszType,
			pszPack);
	}
...
// Input  : *pPath -
//-----------------------------------------------------------------------------
void CBaseFileSystem::AddSearchPathInternal(const char *pPath, const char *pathID, SearchPathAdd_t addType,
											bool bAddPackFiles)
{
	AsyncFinishAll();
...
//-----------------------------------------------------------------------------
int CBaseFileSystem::GetSearchPath(const char *pathID, bool bGetPackFiles, OUT_Z_CAP(maxLenInChars) char *pDest,
								   int maxLenInChars)
{
...
bool CBaseFileSystem::ReadToBuffer(FileHandle_t fp, CUtlBuffer &buf, int nMaxBytes, FSAllocFunc_t pfnAlloc)
{
	...
	if(nBytesToRead != 0)
	{
		...
		if(bBinary && !IsLinux() && !buf.IsExternallyAllocated() && !pfnAlloc && (buf.TellPut() == 0) &&
		   (buf.TellGet() == 0) && (iStartPos % 4 == 0) &&
...
//-----------------------------------------------------------------------------
bool CBaseFileSystem::ReadFile(const char *pFileName, const char *pPath, CUtlBuffer &buf, int nMaxBytes,
...
//
//-----------------------------------------------------------------------------
int CBaseFileSystem::ReadFileEx(const char *pFileName, const char *pPath, void **ppBuf, bool bNullTerminate,
								bool bOptimalAlloc, int nMaxBytes, int nStartingByte, FSAllocFunc_t pfnAlloc)
{
...
bool CBaseFileSystem::UnzipFile(const char *pFileName, const char *pPath, const char *pDestination)
{
	...
	while(1)
	{
		...
		if(szFileName[iFileNameLength - 1] != '/')
		{
			...
			{
				char szFinalName[MAX_PATH];
				Q_snprintf(szFinalName, sizeof(szFinalName), "%s%c%s", pDestination, CORRECT_PATH_SEPARATOR,
						   szFileName);

				// Make sure the directory actually exists in case the ZIP doesn't list it (our zip utils create zips
				// like this)
...
{
public:
	CFileOpenInfo(CBaseFileSystem *pFileSystem, const char *pFileName, const CBaseFileSystem::CSearchPath *path,
	...
	void HandleFileCRCTracking(const char *pRelativeFileName)
	{
		...
		if(m_pFileHandle && m_pSearchPath)
		{
			...
			if(m_pVPKFile)
			{
				m_pFileSystem->m_FileTracker2.NotePackFileAccess(pRelativeFileName, m_pSearchPath->GetPathIDString(),
																 m_pSearchPath->m_storeId, m_pFileHandle->m_VPKHandle);
				return;
			}
#endif
			// we always record hashes of everything we load. we may filter later.
			m_pFileSystem->m_FileTracker2.NoteFileLoadedFromDisk(pRelativeFileName, m_pSearchPath->GetPathIDString(),
																 m_pSearchPath->m_storeId, fp, nLength);
		}
	}
...
FileHandle_t CBaseFileSystem::FindFileInSearchPath(CFileOpenInfo &openInfo)
{
	...
	if(pPackFile)
	{
		...
		if(openInfo.m_pFileHandle)
		{
			...
			V_snprintf(tempStr, sizeof(tempStr), "%s%c%s", pPackFile->m_ZipName.String(), CORRECT_PATH_SEPARATOR,
...
// Purpose:
//-----------------------------------------------------------------------------
FileHandle_t CBaseFileSystem::OpenForRead(const char *pFileNameT, const char *pOptions, unsigned flags,
										  const char *pathID, char **ppszResolvedFilename)
{
...
// In VPK?
	...
	for(openInfo.m_pSearchPath = iter.GetFirst(); openInfo.m_pSearchPath != NULL;
		openInfo.m_pSearchPath = iter.GetNext())
	{
		...
		if(filehandle)
		{
			...
			if(!openInfo.m_pSearchPath->m_bIsTrustedForPureServer &&
			   openInfo.m_ePureFileClass == ePureServerFileClass_AnyTrusted)
			{
#ifdef PURE_SERVER_DEBUG_SPEW
				Msg("Ignoring %s from %s for pure server operation\n", openInfo.m_pFileName,
					openInfo.m_pSearchPath->GetDebugString());
#endif

				m_FileTracker2.NoteFileIgnoredForPureServer(openInfo.m_pFileName, pathID,
															openInfo.m_pSearchPath->m_storeId);
				Close(filehandle);
				openInfo.m_pFileHandle = NULL;
									 ...
									 char **ppszResolvedFilename)
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s(%s, %s, %u %s )", __FUNCTION__,
		   tmDynamicString(TELEMETRY_LEVEL0, pFileName), tmDynamicString(TELEMETRY_LEVEL0, pOptions), flags,
		   tmDynamicString(TELEMETRY_LEVEL0, pathID));
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseFileSystem::LoadKeyValues(KeyValues &head, KeyValuesPreloadType_t type, char const *filename,
...
//-----------------------------------------------------------------------------
// Purpose: If the "PreloadedData" hasn't been purged, then this'll try and instance the KeyValues using the fast path
// of
...
// Output : KeyValues
//-----------------------------------------------------------------------------
KeyValues *CBaseFileSystem::LoadKeyValues(KeyValuesPreloadType_t type, char const *filename,
										  char const *pPathID /*= 0*/)
{
	KeyValues *kv = NULL;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseFileSystem::LookupKeyValuesRootKeyName(char const *filename, char const *pPathID, char *rootName,
												 size_t bufsize)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseFileSystem::ExtractRootKeyName(KeyValuesPreloadType_t type, char *outbuf, size_t bufsize,
...
void CBaseFileSystem::SetupPreloadData()
{
	...
	if(!CommandLine()->FindParm("-fs_nopreloaddata"))
	{
		...
		for(i = 0; i < NUM_PRELOAD_TYPES; ++i)
		{
			...
			char fn[MAX_PATH];
			if(preloader.m_CacheFile != 0 && String(preloader.m_CacheFile, fn, sizeof(fn)))
			{
...
EFileCRCStatus CBaseFileSystem::CheckCachedFileHash(const char *pPathID, const char *pRelativeFilename,
													int nFileFraction, FileHash_t *pFileHash)
{
	return m_FileTracker2.CheckCachedFileHash(pPathID, pRelativeFilename, nFileFraction, pFileHash);
}

void CBaseFileSystem::EnableWhitelistFileTracking(bool bEnable, bool bCacheAllVPKHashes,
	...
	for(int i = 0; i < m_SearchPaths.Count(); i++)
	{
		...
		if(vecChunkHash.Count() == 0)
		{
			...
			if(vecChunkHashFractionCopy.Count() != 0)
			{
				...
				else if(Q_memcmp(vecChunkHash.Base(), vecChunkHashFractionCopy.Base(),
								 vecChunkHash.Count() * sizeof(vecChunkHash[0])) != 0)
				{
					...
					{
						if(Q_memcmp(vecChunkHashFractionCopy[iHash].m_md5contents.bits,
									vecChunkHash[iHash].m_md5contents.bits,
									sizeof(vecChunkHashFractionCopy[iHash].m_md5contents.bits)) != 0)
						{
							Msg("VPK hash for file %d failure at offset %x.\n",
								vecChunkHashFractionCopy[iHash].m_nPackFileNumber,
								vecChunkHashFractionCopy[iHash].m_nFileFraction);
							cFailures++;
						}
					}
				}
		...
		if(bCacheAllVPKHashes)
		{
			...
			{
				m_FileTracker2.AddFileHashForVPKFile(vecChunkHash[i].m_nPackFileNumber, vecChunkHash[i].m_nFileFraction,
													 vecChunkHash[i].m_cbChunkLen, vecChunkHash[i].m_md5contents,
													 fhandle);
			}
		}
		else
...
bool CBaseFileSystem::CheckVPKFileHash(int PackFileID, int nPackFileNumber, int nFileFraction, MD5Value_t &md5Value)
{
	...
	for(int i = 0; i < m_SearchPaths.Count(); i++)
	{
		...
		if(pVPK->FindFileHashFraction(nPackFileNumber, nFileFraction, fileHashFraction))
		{
			...
			Q_binarytohex((const byte *)fileHashFraction.m_md5contents.bits,
						  sizeof(fileHashFraction.m_md5contents.bits), hex2, sizeof(hex2));

			if(Q_memcmp(fileHashFraction.m_md5contents.bits, md5Value.bits, sizeof(md5Value.bits)) != 0)
			{
				Msg("File %s offset %x hash %s does not match ( should be %s ) \n", szFileName, nFileFraction, hex,
					hex2);
				return false;
			}
			else
...
const char *CBaseFileSystem::FindFirstHelper(const char *pWildCardT, const char *pPathID, FileFindHandle_t *pHandle,
											 int *pFoundStoreID)
{
	...
	{
		int c = m_SearchPaths.Count();
		for(pFindData->currentSearchPathID = 0; pFindData->currentSearchPathID < c; pFindData->currentSearchPathID++)
		{
			...
			if(pSearchPath->GetPackFile())
			{
				...
				pSearchPath->GetPackFile()->GetFileAndDirLists(pWildCard, pFindData->m_dirMatchesFromVPKOrPak,
															   pFindData->m_fileMatchesFromVPKOrPak, true);
				bIsVPKOrPak = true;
			}
			...
			if(pSearchPath->GetPackedStore())
			{
				Assert(pFindData->m_dirMatchesFromVPKOrPak.Count() == 0);
				Assert(pFindData->m_fileMatchesFromVPKOrPak.Count() == 0);
				pSearchPath->GetPackedStore()->GetFileAndDirLists(pWildCard, pFindData->m_dirMatchesFromVPKOrPak,
																  pFindData->m_fileMatchesFromVPKOrPak, true);
				bIsVPKOrPak = true;
			}
#endif
			...
			char pTmpFileName[MAX_FILEPATH];
			Q_snprintf(pTmpFileName, sizeof(pTmpFileName), "%s%s", pSearchPath->GetPathString(),
...
bool CBaseFileSystem::FindNextFileHelper(FindData_t *pFindData, int *pFoundStoreID)
{
	...
	for(; pFindData->currentSearchPathID < c; ++pFindData->currentSearchPathID)
	{
		...
		if(pSearchPath->GetPackFile())
		{
			...
			Assert(pFindData->m_dirMatchesFromVPKOrPak.Count() == 0);
			Assert(pFindData->m_fileMatchesFromVPKOrPak.Count() == 0);
			pSearchPath->GetPackFile()->GetFileAndDirLists(pFindData->wildCardString.Base(),
														   pFindData->m_dirMatchesFromVPKOrPak,
														   pFindData->m_fileMatchesFromVPKOrPak, true);
			if(FindNextFileInVPKOrPakHelper(pFindData))
				return true;
			continue;
		...
		if(pSearchPath->GetPackedStore())
		{
			Assert(pFindData->m_dirMatchesFromVPKOrPak.Count() == 0);
			Assert(pFindData->m_fileMatchesFromVPKOrPak.Count() == 0);
			pSearchPath->GetPackedStore()->GetFileAndDirLists(pFindData->wildCardString.Base(),
															  pFindData->m_dirMatchesFromVPKOrPak,
															  pFindData->m_fileMatchesFromVPKOrPak, true);
			if(FindNextFileInVPKOrPakHelper(pFindData))
				return true;
...
bool CBaseFileSystem::FindNextFileInVPKOrPakHelper(FindData_t *pFindData)
{
	...
	{
		V_strncpy(pFindData->findData.cFileName, V_UnqualifiedFileName(pFindData->m_fileMatchesFromVPKOrPak[0]),
				  sizeof(pFindData->findData.cFileName));
	...
	{
		V_strncpy(pFindData->findData.cFileName, V_UnqualifiedFileName(pFindData->m_dirMatchesFromVPKOrPak[0]),
				  sizeof(pFindData->findData.cFileName));
...
//-----------------------------------------------------------------------------
const char *CBaseFileSystem::RelativePathToFullPath(const char *pFileName, const char *pPathID,
													OUT_Z_CAP(maxLenInChars) char *pDest, int maxLenInChars,
													PathTypeFilter_t pathFilter, PathTypeQuery_t *pPathType)
{
	CHECK_DOUBLE_SLASHES(pFileName);
...
const char *CBaseFileSystem::GetLocalPath(const char *pFileName, OUT_Z_CAP(maxLenInChars) char *pDest,
										  int maxLenInChars)
{
	CHECK_DOUBLE_SLASHES(pFileName);
...
//-----------------------------------------------------------------------------
bool CBaseFileSystem::FullPathToRelativePathEx(const char *pFullPath, const char *pPathId,
											   OUT_Z_CAP(maxLenInChars) char *pDest, int maxLenInChars)
{
...
// Obsolete version
//-----------------------------------------------------------------------------
bool CBaseFileSystem::FullPathToRelativePath(const char *pFullPath, OUT_Z_CAP(maxLenInChars) char *pDest,
											 int maxLenInChars)
{
...
//-----------------------------------------------------------------------------
bool CBaseFileSystem::GetCaseCorrectFullPath_Ptr(const char *pFullPath, OUT_Z_CAP(maxLenInChars) char *pDest,
												 int maxLenInChars)
{
	CHECK_DOUBLE_SLASHES(pFullPath);
...
CSysModule *CBaseFileSystem::LoadModule(const char *pFileName, const char *pPathID, bool bValidatedDllOnly)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		Q_snprintf(tempPathID, sizeof(tempPathID), "%s%s", m_SearchPaths[i].GetPathString(),
...
bool CBaseFileSystem::GetFileTypeForFullPath(char const *pFullPath, wchar_t *buf, size_t bufSizeInBytes)
{
	...
	SHFILEINFOW info = {0};
	DWORD_PTR dwResult = SHGetFileInfoW(wcharpath, 0, &info, sizeof(info), SHGFI_TYPENAME);
	if(dwResult)
	{
...
bool CBaseFileSystem::GetOptimalIOConstraints(FileHandle_t hFile, unsigned *pOffsetAlign, unsigned *pSizeAlign,
...
void CBaseFileSystem::AddFilesToFileCache(FileCacheHandle_t cacheId, const char **ppFileNames, int nFileNames,
										  const char *pPathID)
{
...
void CBaseFileSystem::CFileCacheObject::IOCallback(const FileAsyncRequest_t &request, int nBytesRead,
												   FSAsyncStatus_t err)
{
