...
void GLMRendererInfo::Init(GLMRendererInfoFields *info)
{
	...
	// We don't currently used bindable uniforms, but I've been experimenting with them so I might as well check this in
	// just in case they turn out to be useful.
	m_info.m_hasBindableUniforms = gGL->m_bHave_GL_EXT_bindable_uniform;
	m_info.m_hasBindableUniforms =
		false; // !!! FIXME hardwiring this path to false until we see how to accelerate it properly
	...
	if(m_info.m_hasBindableUniforms)
	{
		...
		if((m_info.m_maxVertexBindableUniforms < 1) || (m_info.m_maxFragmentBindableUniforms < 1) ||
		   (m_info.m_maxBindableUniformSize < (sizeof(float) * 4 * 256)))
		{
	...
	m_info.m_hasPerfPackage1 = true; // this flag is Mac-specific. We do slower things if you don't have Mac OS X 10.x.y
									 // or later. Linux always does the fast path!
...
#if defined(OSX)
	m_info.m_cantBlitReliably =
		m_info.m_intel; // FIXME X3100&10.6.3 has problems blitting.. adjust this if bug fixed in 10.6.4
#else
...
void GLMRendererInfo::Dump(int which)
{
	...
	GLMPRINTF(("\n     #%d: GLMRendererInfo @ %p, renderer-id=(%08x)  display-mask=%08x  vram=%dMB", which, this,
			   m_info.m_rendererID, m_info.m_displayMask, m_info.m_vidMemory >> 20));
	GLMPRINTF(("\n       VendorID=%04x  DeviceID=%04x  Model=%s", m_info.m_pciVendorID, m_info.m_pciDeviceID,
...
void GLMDisplayDB::PopulateRenderers(void)
{
	...
	else if(gGL->m_bHave_GL_ATI_meminfo)
	{
		...
		else
		{
			...
			DebugPrintf("GL_ATI_meminfo: GL_TEXTURE_FREE_MEMORY_ATI: Total Free: %i, Largest Avail: %i, Total Aux: %i, "
						"Largest Aux Avail: %i\n",
	...
	// Clamp the min amount of video memory to 256MB in case a query returned something bogus, or we interpreted it
	// badly.
	...
	if((gGL->m_nDriverProvider == cGLDriverProviderIntel) ||
	   (gGL->m_nDriverProvider == cGLDriverProviderIntelOpenSource))
	{
...
void GLMDisplayDB::PopulateFakeAdapters(
...
bool GLMDisplayDB::GetFakeAdapterInfo(int fakeAdapterIndex, int *rendererOut, int *displayOut,
...
void GLMDisplayInfo::PopulateModes(void)
{
	...
	static const struct
	{
		...
		{640, 480}, // 4x3
		{800, 600},	 {1024, 768},  {1152, 864},	 {1280, 960},  {1600, 1200}, {1920, 1440}, {2048, 1536},
	...
	while(i < nummodes)
	{
		...
		if((info0.m_modePixelWidth == info1.m_modePixelWidth) && (info0.m_modePixelHeight == info1.m_modePixelHeight) &&
...
void GLMDisplayInfo::Dump(int which)
{
	...
	GLMPRINTF(("\n         #%d: GLMDisplayInfo @ %08x, pixwidth=%d  pixheight=%d", which, (uintp)this,
			   m_info.m_displayPixelWidth, m_info.m_displayPixelHeight));
