...
#error Unimplemented
	...
	if(bRequired && !okay)
	{
		...
		Error("Could not find required OpenGL entry point '%s'! Either your video card is unsupported, or your OpenGL "
...
public:
	...
	virtual void GetNativeDisplayInfo(int nDisplay, uint &nWidth, uint &nHeight,
									  uint &nRefreshHz); // Retrieve the size of the monitor (desktop)
	virtual void RenderedSize(uint &width, uint &height,
							  bool set); // either set or retrieve rendered size value (from dxabstract)
	virtual void DisplayedSize(uint &width, uint &height); // query backbuffer size (window size whether FS or windowed)

#if defined(DX_TO_GL_ABSTRACTION)
	virtual void GetDesiredPixelFormatAttribsAndRendererInfo(uint **ptrOut, uint *countOut,
	...
	{
		return m_flPrevGLSwapWindowTime;
	}
...
#if defined(OSX)
	// bool					m_leopard;					// true if <10.6.3 and we have to do extra work for fullscreen
	// handling
...
bool CSDLMgr::CreateHiddenGameWindow(const char *pTitle, int width, int height)
{
	...
	static CDynamicFunctionOpenGL<true, const GLubyte *(APIENTRY *)(GLenum name), const GLubyte *> glGetString(
		"glGetString");
	static CDynamicFunctionOpenGL<true, GLvoid(APIENTRY *)(GLenum pname, GLint * params), GLvoid> glGetIntegerv(
		"glGetIntegerv");
...
void CSDLMgr::OnFrameRendered()
{
	...
	if(m_bSetMouseVisibleCalled)
	{
		...
		// force non-fullscreen windows to the foreground if grabbed, so you can't get your mouse locked to something in
		// the background.
...
void CSDLMgr::ShowPixels(CShowPixelsParams *params)
{
	...
	if((swapInterval != m_lastKnownSwapInterval) || (swapLimit != m_lastKnownSwapLimit))
	{
		...
		printf("\n ##### swap interval = %d     swap limit = %d #####\n", m_lastKnownSwapInterval,
			   m_lastKnownSwapLimit);
		fflush(stdout);
	}
	...
	if(!params->m_noBlit)
	{
		if(params->m_useBlit) // FBO blit path - which is what we *should* be using.  But if the params say no, then
							  // don't do it because the ext is not there.
		{
			...
			glFramebufferTexture2DEXT(GL_READ_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D,
									  params->m_srcTexName, 0);
			CheckGLError(__LINE__);
			...
			GLenum filter =
				(((srcxmax - srcxmin) == (dstxmax - dstxmin)) && ((srcymax - srcymin) == (dstymax - dstymin)))
					? GL_NEAREST
					: GL_LINEAR;
...
void CSDLMgr::SetWindowFullScreen(bool bFullScreen, int nWidth, int nHeight)
{
	...
	if((m_bFullScreen != bFullScreen) || (bFullScreen && (sdl_displayindex_fullscreen.GetInt() != displayIndex)))
	{
		if(bFullScreen)
		{
			int x = 0;
...
void CSDLMgr::SizeWindow(int width, int tall)
{
	...
	if((m_WindowWidth == width) && (m_WindowHeight == tall) && (m_SizeWindowFullScreenState == m_bFullScreen) &&
...
#ifdef OSX
	if(event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_TAB && SDL_GetModState() & KMOD_GUI &&
	   CommandLine()->FindParm("-exclusivefs"))
	{
	...
	if(CommandLine()->FindParm("-nonqwerty"))
	{
		...
		if(keyname != NULL && strlen(keyname) == 1)
		{
			...
			else
			{
				...
				{
					case '=':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_EQUALS;
						break;
					case '-':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_MINUS;
						break;
					case '[':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_LEFTBRACKET;
						break;
					case ']':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_RIGHTBRACKET;
						break;
					case ';':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_SEMICOLON;
						break;
					case '\'':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_APOSTROPHE;
						break;
					case ',':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_COMMA;
						break;
					case '.':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_PERIOD;
						break;
					case '/':
						theEvent.m_VirtualKeyCode = SDL_SCANCODE_SLASH;
						break;
				}
			}
		}
...
#if GLMDEBUG
	bool bIsShifted = (((theEvent.m_ModifierKeyMask & (1 << eCapsLockKey)) != 0) ||
					   ((theEvent.m_ModifierKeyMask & (1 << eShiftKey)) != 0));
	...
	while(SDL_PollEvent(&event) && nEventsProcessed < 100)
	{
		...
		switch(event.type)
		{
			case SDL_MOUSEMOTION:
			{
				...
				// mouse motion events.
				if(m_bExpectSyntheticMouseMotion && event.motion.x == m_nMouseTargetX &&
				...
				if(!m_bRawInput && !m_bCursorVisible &&
				   (event.motion.x < m_nMouseTargetX - m_nWarpDelta ||
					event.motion.x > m_nMouseTargetX + m_nWarpDelta ||
					event.motion.y < m_nMouseTargetY - m_nWarpDelta || event.motion.y > m_nMouseTargetY + m_nWarpDelta))
				{
			...
			case SDL_MOUSEBUTTONDOWN:
			{
				...
				switch(event.button.button)
				{
					...
					default:
						...
						//    4 = Wheel Forward		; These events are handled by SDL_MOUSEWHEEL and don't come through
						//    here. 5 = Wheel Back		; 6 = Wheel Tilt Left	; 7 = Wheel Tilt Right	; 8 = Browser
						//    Back 9 = Browser Forward 10 = Task button (probably similar to Alt-Tab)
...
#if defined(DX_TO_GL_ABSTRACTION)
void CSDLMgr::GetDesiredPixelFormatAttribsAndRendererInfo(uint **ptrOut, uint *countOut,
														  GLMRendererInfoFields *rendInfoOut)
{
	SDLAPP_FUNC;
	...
	if(ptrOut)
		*ptrOut = (uint *)m_pixelFormatAttribs;
	if(countOut)
		*countOut = m_pixelFormatAttribCount;
	if(rendInfoOut)
	{
...
void GLMDisplayMode::Dump(int which)
{
	...
	GLMPRINTF(("\n             # %-2d  width=%-4d  height=%-4d  refreshHz=%-2d", which, m_info.m_modePixelWidth,
			   m_info.m_modePixelHeight, m_info.m_modeRefreshHz));
}
