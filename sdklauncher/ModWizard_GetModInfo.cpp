...
public:
	...
	CModalPreserveDirectorySelectDialog(vgui::Panel *pParent, const char *title) : BaseClass(pParent, title)
	{
...
bool CModWizardSubPanel_GetModInfo::OnNextButton()
{
	...
	else
	{
		...
		// Tony; after the other check for invalid characters, make the actual outputModGamedirName lowercase with
		// spaces stripped out.
		...
		Q_strncpy(outputModGamedirName, GetSDKLauncherBaseDirectory(),
				  sizeof(outputModGamedirName)); // steamapps\username\sourcesdk
		...
		Q_strncat(outputModGamedirName, "SourceMods", sizeof(outputModGamedirName),
				  COPY_ALL_CHARACTERS); // steamapps\sourcemods
		Q_AppendSlash(outputModGamedirName, sizeof(outputModGamedirName));
		Q_strncat(outputModGamedirName, strippedModName, sizeof(outputModGamedirName),
				  COPY_ALL_CHARACTERS); // steamapps\sourcemods\modname
		Q_AppendSlash(outputModGamedirName, sizeof(outputModGamedirName));

		if(!CreateFullDirectory(outputModGamedirName) || !CreateSubdirectory(outputModGamedirName, "resource") ||
		   !CreateSubdirectory(outputModGamedirName, "resource\\ui") ||
		   !CreateSubdirectory(outputModGamedirName, "maps") || !CreateSubdirectory(outputModGamedirName, "cfg") ||
	...
	CModWizardSubPanel_CopyFiles *pCopyPanel =
		dynamic_cast<CModWizardSubPanel_CopyFiles *>(GetWizardPanel()->FindChildByName("CModWizardSubPanel_CopyFiles"));
	if(!pCopyPanel)
	{
...
void CModWizardSubPanel_GetModInfo::OnCommand(const char *command)
{
	...
	{
		CModalPreserveDirectorySelectDialog *pDlg =
			vgui::SETUP_PANEL(new CModalPreserveDirectorySelectDialog(this, "#SelectInstallDirectory"));
