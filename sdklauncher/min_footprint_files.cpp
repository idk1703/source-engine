...
extern FSReturnCode_t LoadGameInfoFile(const char *pDirectoryName, KeyValues *&pMainFile, KeyValues *&pFileSystemInfo,
...
public:
	...
	CMinFootprintFilesPanel(Panel *parent, const char *panelName) : BaseClass(parent, panelName)
	{
	...
	void ShowDeprecatedAppIDNotice()
	{
		...
		else
		{
			...
			Q_strncpy(szProblemMods,
					  "The SteamAppId values for the following mods should be changed from 220 to 215:\r\n\r\n",
					  5 * MAX_PATH);
			...
			if(bProblemModExists)
			{
				...
				Q_strncat(szProblemMods,
						  ...
						  "author any of the above mods you should change the SteamAppId\nvalue in the gameinfo.txt "
						  "file for each mod listed above.\r\n",
...
void AddMinFootprintFile(CUtlVector<CMinFootprintFilename> &filenames, const char *pSrcFilename,
						 const char *pDestFilename, int flags)
{
...
void GetMinFootprintFiles_R(CUtlVector<CMinFootprintFilename> &filenames, const char *pSrcDirName,
							const char *pDestDirName)
{
	// pDirName\*.*
	...
	while(pFilename)
	{
		if(Q_stricmp(pFilename, ".") != 0 && Q_stricmp(pFilename, "..") != 0)
		{
			...
			Q_snprintf(fullSrcFilename, sizeof(fullSrcFilename), "%s%c%s", pSrcDirName, CORRECT_PATH_SEPARATOR,
					   pFilename);
			Q_snprintf(fullDestFilename, sizeof(fullDestFilename), "%s%c%s", pDestDirName, CORRECT_PATH_SEPARATOR,
					   pFilename);
...
void DumpMinFootprintFiles(bool bForceRefresh)
{
	...
	CMinFootprintFilesPanel *pDisplay =
		new CMinFootprintFilesPanel((vgui::Frame *)g_pMainFrame, "MinFootprintFilesPanel");
	...
	for(KeyValues *pCur = pFileList->GetFirstSubKey(); pCur; pCur = pCur->GetNextKey())
	{
		if((Q_stricmp(pCur->GetName(), "mapping") == 0) && bVersionChanged)
		{
			...
			Q_snprintf(destDirTemp, sizeof(destDirTemp), "%s%c%s", GetSDKLauncherBaseDirectory(),
					   CORRECT_PATH_SEPARATOR, pDestMapping);
		...
		else if(Q_stricmp(pCur->GetName(), "single_file") == 0)
		{
			...
			Q_snprintf(destFileTemp, sizeof(destFileTemp), "%s%c%s", GetSDKLauncherBaseDirectory(),
					   CORRECT_PATH_SEPARATOR, pDestMapping);
		...
		else if((Q_stricmp(pCur->GetName(), "create_directory") == 0) && bVersionChanged)
		{
			...
			char destFile[MAX_PATH], destFileTemp[MAX_PATH];
			Q_snprintf(destFileTemp, sizeof(destFileTemp), "%s%c%s", GetSDKLauncherBaseDirectory(),
					   CORRECT_PATH_SEPARATOR, pCur->GetString());
			Q_MakeAbsolutePath(destFile, sizeof(destFile), destFileTemp);
