...
ConVar joy_active("joy_active", "-1", FCVAR_NONE,
				  "Which of the connected joysticks / gamepads to use (-1 means first found)", &joy_active_changed_f);

static void joy_gamecontroller_config_changed_f(IConVar *var, const char *pOldValue, float flOldValue);
ConVar joy_gamecontroller_config("joy_gamecontroller_config", "", FCVAR_ARCHIVE,
								 "Game controller mapping (passed to SDL with SDL_HINT_GAMECONTROLLERCONFIG), can also "
								 "be configured in Steam Big Picture mode.",
								 &joy_gamecontroller_config_changed_f);
...
void CInputSystem::JoystickHotplugAdded(int joystickIndex)
{
	...
	{
		Warning("Joystick is not recognized by the game controller system. You can configure the controller in Steam "
				"Big Picture mode.\n");
		return;
	}
	...
	if(activeJoystick < 0)
	{
		...
		if(info.m_nDeviceId != -1)
		{
			Msg("Detected supported joystick #%i '%s'. Currently active joystick is #%i.\n", joystickId,
				SDL_JoystickNameForIndex(joystickIndex), info.m_nDeviceId);
			return;
		}
	}
	...
	{
		Msg("Detected supported joystick #%i '%s'. Currently active joystick is #%i.\n", joystickId,
			SDL_JoystickNameForIndex(joystickIndex), activeJoystick);
		return;
	}
...
{
	// We only pump the SDL event loop if we're not an SDL app, since otherwise PollInputState_Platform calls into
	// CSDLMgr to pump it. Our state updates happen in events posted by SDL_GameController in JoystickSDLWatcher, so the
	// loop is empty.
#if !defined(USE_SDL)
	SDL_Event event;
