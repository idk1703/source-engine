...
#endif
ConVar joy_xcontroller_found("joy_xcontroller_found", "1", FCVAR_HIDDEN,
...
typedef UINT(WINAPI *GetRawInputData_t)(HRAWINPUT hRawInput, UINT uiCommand, LPVOID pData, PUINT pcbSize,
										UINT cbSizeHeader);
...
InitReturnVal_t CInputSystem::Init()
{
	...
	// Initialize the input system copy of the steam API context, for use by controller stuff (don't do this if we're a
	// dedicated server).
	...
	if(m_pRawInputDLL)
	{
		pfnRegisterRawInputDevices =
			(RegisterRawInputDevices_t)GetProcAddress((HMODULE)m_pRawInputDLL, "RegisterRawInputDevices");
...
//-----------------------------------------------------------------------------
void CInputSystem::PostButtonPressedEvent(InputEventType_t nType, int nTick, ButtonCode_t scanCode,
...
//-----------------------------------------------------------------------------
void CInputSystem::PostButtonReleasedEvent(InputEventType_t nType, int nTick, ButtonCode_t scanCode,
										   ButtonCode_t virtualCode)
{
	InputState_t &state = m_InputState[m_bIsPolling];
	if(state.m_ButtonState.IsBitSet(scanCode))
...
void CInputSystem::PollInputState_Platform()
{
	...
	// These are Carbon virtual key codes. AFAIK they don't have a header that defines these, but they are supposed to
	// map to the same letters across international keyboards, so our mapping here should work.
...
void CInputSystem::SetCursorPosition(int x, int y)
{
	...
	if(bXChanged)
	{
		PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, MOUSE_X, state.m_pAnalogValue[MOUSE_X],
				  state.m_pAnalogDelta[MOUSE_X]);
	}
	if(bYChanged)
	{
		PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, MOUSE_Y, state.m_pAnalogValue[MOUSE_Y],
				  state.m_pAnalogDelta[MOUSE_Y]);
	}
	if(bXChanged || bYChanged)
	{
		PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, MOUSE_XY, state.m_pAnalogValue[MOUSE_X],
				  state.m_pAnalogValue[MOUSE_Y]);
	}
}
		...
		case WM_SYSKEYDOWN:
		{
			...
			if(!(lParam & (1 << 30)))
			{
				...
				if(scanCode == KEY_CAPSLOCK || scanCode == KEY_SCROLLLOCK || scanCode == KEY_NUMLOCK)
				{
					...
					PostButtonPressedEvent(bToggleState ? IE_ButtonPressed : IE_ButtonReleased, m_nLastSampleTick,
