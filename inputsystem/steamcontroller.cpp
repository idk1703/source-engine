...
ConVar sc_joystick_map(
	"sc_joystick_map", "1", FCVAR_ARCHIVE,
...
#define STEAMPAD_ANALOG_SCALE_LEFT(x) \
	((float)STEAMPAD_MAX_ANALOGSAMPLE_LEFT / (float)(STEAMPAD_MAX_ANALOGSAMPLE_LEFT - (x)))
#define STEAMPAD_ANALOG_SCALE_RIGHT(x) \
	((float)STEAMPAD_MAX_ANALOGSAMPLE_RIGHT / (float)(STEAMPAD_MAX_ANALOGSAMPLE_RIGHT - (x)))
#define STEAMPAD_ANALOG_SCALE_DOWN(x) \
	((float)STEAMPAD_MAX_ANALOGSAMPLE_DOWN / (float)(STEAMPAD_MAX_ANALOGSAMPLE_DOWN - (x)))
...
static SDigitalMenuAction g_DigitalMenuActions[] = {
	...
	{"cl_trigger_first_notification",
	 ...
	 0,
	 {false},
	 {false}}, // Command is in the in-game action set (use for hitting accept/ok on dialogs)
	{"cl_decline_first_notification",
	 STEAMCONTROLLER_F2,
	 0,
	 {false},
	 {false}}, // Command is in the in-game action set (use for hitting cancel/decline on dialogs)
	{
		...
		{false},
		{false},
	},															   // Command is in the in-game HUD action set
...
void CInputSystem::PollSteamControllers(void)
{
	...
	if(m_unNumConnected > 0)
	{
		...
		if(m_bSteamControllerActionsInitialized)
		{
			// If we successfully initialized all the actions, and we have a connected controller, then we're considered
			// "active".
			...
			for(int i = 0; i != ARRAYSIZE(g_DigitalMenuActions); ++i)
			{
				...
				for(uint64 j = 0; j < m_unNumConnected; ++j)
				{
					...
					ControllerDigitalActionData_t data =
						steamcontroller->GetDigitalActionData(nControllerHandles[j], action.handle);
...
void CInputSystem::ActivateSteamControllerActionSetForSlot(uint64 nSlot, GameActionSet_t eActionSet)
{
	...
	if(steamcontroller)
	{
		if(nSlot == STEAM_CONTROLLER_HANDLE_ALL_CONTROLLERS)
		{
			...
			steamcontroller->ActivateActionSet(STEAM_CONTROLLER_HANDLE_ALL_CONTROLLERS,
											   g_GameActionSets[eActionSet].handle);
		}
		else
		{
			uint64 nControllerHandles[STEAM_CONTROLLER_MAX_COUNT];
	...
	{
		// If we changed action set, then flag everything for a debounce (meaning we demand to see an unpressed state
		// before we'll register a pressed one)
		for(int i = 0; i < STEAM_CONTROLLER_MAX_COUNT; i++)
		{
...
//-----------------------------------------------------------------------------
/* void CInputSystem::PulseHapticOnSteamController( uint32 nControllerIndex, ESteamControllerPad ePad, unsigned short
durationMicroSec )
{
	auto steamcontroller = SteamControllerInterface();
	if ( steamcontroller )
...
// Gets the action origin (i.e. which physical input) maps to the given virtual button for the given action set
EControllerActionOrigin CInputSystem::GetSteamControllerActionOrigin(const char *action,
																	 ControllerActionSetHandle_t action_set_handle)
{
	auto pSC = SteamControllerInterface();
