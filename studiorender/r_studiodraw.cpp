...
typedef void (*SoftwareProcessMeshFunc_t)(const mstudio_meshvertexdata_t *, matrix3x4_t *pPoseToWorld,
										  ...
										  unsigned short *pGroupToMesh, unsigned int nAlphaMask, IMaterial *pMaterial);
...
static TableVector hullcolor[8] = {{1.0, 1.0, 1.0}, {1.0, 0.5, 0.5}, {0.5, 1.0, 0.5}, {1.0, 1.0, 0.5},
...
int CStudioRender::R_StudioRenderModel(IMatRenderContext *pRenderContext, int skin, int body, int hitboxset,
									   void /*IClientEntity*/ *pEntity, IMaterial **ppMaterials, int *pMaterialFlags,
									   int flags, int boneMask, int lod, ColorMeshInfo_t *pColorMeshes)
{
...
================
*/
int CStudioRender::R_StudioRenderFinal(IMatRenderContext *pRenderContext, int skin, int nBodyPartCount,
									   ...
									   ColorMeshInfo_t *pColorMeshes)
{
	...
	for(int i = 0; i < nBodyPartCount; i++)
	{
		...
		numTrianglesRendered += R_StudioDrawPoints(pRenderContext, skin, pClientEntity, ppMaterials, pMaterialFlags,
												   boneMask, lod, pColorMeshes);
	}
	return numTrianglesRendered;
}
...
void CStudioRender::DrawShadows(const DrawModelInfo_t &info, int flags, int boneMask)
{
	...
	for(i = 0; i < m_ShadowState.Count(); ++i)
	{
		if(!m_ShadowState[i].m_pMaterial)
		{
			...
			pRenderContext->SetFlashlightStateEx(*m_ShadowState[i].m_pFlashlightState,
			...
			R_StudioRenderModel(pRenderContext, info.m_Skin, info.m_Body, info.m_HitboxSet, info.m_pClientEntity,
								...
								info.m_pHardwareData->m_pLODs[info.m_Lod].pMaterialFlags, flags, boneMask, info.m_Lod,
								info.m_pColorMeshes);
	...
	for(int i = 0; i < m_ShadowState.Count(); ++i)
	{
		if(m_ShadowState[i].m_pMaterial)
		{
			...
			R_StudioRenderModel(pRenderContext, 0, info.m_Body, 0, m_ShadowState[i].m_pProxyData, NULL, NULL, flags,
...
void CStudioRender::DrawStaticPropShadows(const DrawModelInfo_t &info, const StudioRenderContext_t &rc,
										  const matrix3x4_t &rootToWorld, int flags)
{
...
static matrix3x4_t *ComputeSkinMatrixSSE(mstudioboneweight_t &boneweights, matrix3x4_t *pPoseToWorld,
										 matrix3x4_t &result)
{
...
void CStudioRender::R_MouthLighting(fltx4 fIllum, const FourVectors &normal, const FourVectors &forward,
									FourVectors &light)
{
...
inline void CStudioRender::R_ComputeLightAtPoints3(const FourVectors &pos, const FourVectors &normal,
												   FourVectors &color)
{
	if(m_pRC->m_Config.fullbright)
	{
	...
	for(int i = 0; i < m_pRC->m_NumLocalLights; i++)
	{
		...
		Assert((wl->m_Type == MATERIAL_LIGHT_POINT) || (wl->m_Type == MATERIAL_LIGHT_SPOT) ||
		...
		switch(wl->m_Type)
		{
			...
			{
				fltx4 dot2 =
					SubSIMD(Four_Zeros, delta * wl->m_Direction); // dot position with spot light dir for cone falloff

				fltx4 cone_falloff_scale =
					MulSIMD(OneOver_ThetaDot_Minus_PhiDot[i], SubSIMD(dot2, ReplicateX4(wl->m_PhiDot)));
...
{
public:
	static void R_PerformLighting(const Vector &forward, float fIllum, const Vector &pos, const Vector &norm,
								...
								matrix3x4_t *pSkinMat, VectorAligned &pos, Vector &norm, Vector4DAligned &tangentS)
	{
		...
		pos.x = pSrcPos->x * (*pSkinMat)[0][0] + pSrcPos->y * (*pSkinMat)[0][1] + pSrcPos->z * (*pSkinMat)[0][2] +
				(*pSkinMat)[0][3];
		norm.x = pSrcNorm->x * (*pSkinMat)[0][0] + pSrcNorm->y * (*pSkinMat)[0][1] + pSrcNorm->z * (*pSkinMat)[0][2];

		pos.y = pSrcPos->x * (*pSkinMat)[1][0] + pSrcPos->y * (*pSkinMat)[1][1] + pSrcPos->z * (*pSkinMat)[1][2] +
				(*pSkinMat)[1][3];
		norm.y = pSrcNorm->x * (*pSkinMat)[1][0] + pSrcNorm->y * (*pSkinMat)[1][1] + pSrcNorm->z * (*pSkinMat)[1][2];

		pos.z = pSrcPos->x * (*pSkinMat)[2][0] + pSrcPos->y * (*pSkinMat)[2][1] + pSrcPos->z * (*pSkinMat)[2][2] +
				(*pSkinMat)[2][3];
		norm.z = pSrcNorm->x * (*pSkinMat)[2][0] + pSrcNorm->y * (*pSkinMat)[2][1] + pSrcNorm->z * (*pSkinMat)[2][2];

		if(nHasTangentSpace)
		{
			tangentS.x = pSrcTangentS->x * (*pSkinMat)[0][0] + pSrcTangentS->y * (*pSkinMat)[0][1] +
						 pSrcTangentS->z * (*pSkinMat)[0][2];
			tangentS.y = pSrcTangentS->x * (*pSkinMat)[1][0] + pSrcTangentS->y * (*pSkinMat)[1][1] +
						 pSrcTangentS->z * (*pSkinMat)[1][2];
			tangentS.z = pSrcTangentS->x * (*pSkinMat)[2][0] + pSrcTangentS->y * (*pSkinMat)[2][1] +
											...
											unsigned short *pGroupToMesh, unsigned int nAlphaMask, IMaterial *pMaterial)
	{
		...
		for(int j = 0; j < numVertices; ++j)
		{
			...
			R_TransformVert(pSrcPos, pSrcNorm, pSrcTangentS, pSkinMat, *(VectorAligned *)&dstVertex.m_vecPosition,
							dstVertex.m_vecNormal, *(Vector4DAligned *)&dstVertex.m_vecUserData);
...
#endif
			// Compute lighting
			R_PerformLighting(forward, fIllum, dstVertex.m_vecPosition, dstVertex.m_vecNormal, nAlphaMask,
							  &dstVertex.m_nColor);
	...
	// this special version of the vertex processor does 4 vertices at once, so that they can be lit using SSE
	// instructions. This provides a >2x speedup in the lit case
	static void R_PerformVectorizedLightingSSE(const FourVectors &forward, fltx4 fIllum, ModelVertexDX8_t *dst,
...
// 			}
			...
			Position.LoadAndSwizzleAligned(dst[0].m_vecPosition, dst[1].m_vecPosition, dst[2].m_vecPosition,
										   dst[3].m_vecPosition);
			...
			for(int i = 0; i < 4; i++)
			{
				...
				if(NotCloseEnough(color.x, Color.X(i)) || NotCloseEnough(color.y, Color.Y(i)) ||
	...
	static void R_StudioSoftwareProcessMeshSSE_DX7(const mstudio_meshvertexdata_t *vertData, matrix3x4_t *pPoseToWorld,
												   ...
												   unsigned int nAlphaMask, IMaterial *pMaterial)
	{
		Assert(numVertices > 0);
		mstudiovertex_t *pVertices = vertData->Vertex(0);
...
#define N_VERTS_TO_DO_AT_ONCE 4 // for SSE processing
		...
		while(1)
		{
			for(int subc = 0; subc < 4; subc++)
			{
				...
				// Transform the vert into world space
				R_TransformVert(pSrcPos, pSrcNorm, 0, pSkinMat, *(VectorAligned *)&dst->m_vecPosition, dst->m_vecNormal,
			...
			{
				meshBuilder.Fast4VerticesSSE((ModelVertexDX7_t *)&(dst[0]), (ModelVertexDX7_t *)&(dst[1]),
...
#if !defined(_X360)
	ProcessMesh000H7_t::R_StudioSoftwareProcessMesh,		ProcessMesh000S7_t::R_StudioSoftwareProcessMesh,
	ProcessMesh000M7_t::R_StudioSoftwareProcessMesh,

	ProcessMesh001H7_t::R_StudioSoftwareProcessMesh,
#ifdef SPECIAL_SSE_MESH_PROCESSOR
	ProcessMesh001S7_t::R_StudioSoftwareProcessMeshSSE_DX7, ProcessMesh001M7_t::R_StudioSoftwareProcessMeshSSE_DX7,
#else
	ProcessMesh001S7_t::R_StudioSoftwareProcessMesh, ProcessMesh001M7_t::R_StudioSoftwareProcessMesh,
#endif

	ProcessMesh010H7_t::R_StudioSoftwareProcessMesh,		ProcessMesh010S7_t::R_StudioSoftwareProcessMesh,
	ProcessMesh010M7_t::R_StudioSoftwareProcessMesh,

	ProcessMesh011H7_t::R_StudioSoftwareProcessMesh,
#ifdef SPECIAL_SSE_MESH_PROCESSOR
	ProcessMesh011S7_t::R_StudioSoftwareProcessMeshSSE_DX7, ProcessMesh011M7_t::R_StudioSoftwareProcessMeshSSE_DX7,
#else
	ProcessMesh011S7_t::R_StudioSoftwareProcessMesh, ProcessMesh011M7_t::R_StudioSoftwareProcessMesh,
#endif

	ProcessMesh100H7_t::R_StudioSoftwareProcessMesh,		ProcessMesh100S7_t::R_StudioSoftwareProcessMesh,
	ProcessMesh100M7_t::R_StudioSoftwareProcessMesh,

	ProcessMesh101H7_t::R_StudioSoftwareProcessMesh,		ProcessMesh101S7_t::R_StudioSoftwareProcessMesh,
	ProcessMesh101M7_t::R_StudioSoftwareProcessMesh,

	ProcessMesh110H7_t::R_StudioSoftwareProcessMesh,		ProcessMesh110S7_t::R_StudioSoftwareProcessMesh,
	ProcessMesh110M7_t::R_StudioSoftwareProcessMesh,

	ProcessMesh111H7_t::R_StudioSoftwareProcessMesh,		ProcessMesh111S7_t::R_StudioSoftwareProcessMesh,
	ProcessMesh111M7_t::R_StudioSoftwareProcessMesh,
#endif

#if !defined(_X360)
	ProcessMesh000H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh000S8_t::R_StudioSoftwareProcessMesh,
	ProcessMesh000M8_t::R_StudioSoftwareProcessMesh,
#endif
	ProcessMesh001H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh001S8_t::R_StudioSoftwareProcessMesh,
	ProcessMesh001M8_t::R_StudioSoftwareProcessMesh,
#if !defined(_X360)
	ProcessMesh010H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh010S8_t::R_StudioSoftwareProcessMesh,
	ProcessMesh010M8_t::R_StudioSoftwareProcessMesh,
#endif
	ProcessMesh011H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh011S8_t::R_StudioSoftwareProcessMesh,
	ProcessMesh011M8_t::R_StudioSoftwareProcessMesh,
#if !defined(_X360)
	ProcessMesh100H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh100S8_t::R_StudioSoftwareProcessMesh,
	ProcessMesh100M8_t::R_StudioSoftwareProcessMesh,
#endif
	ProcessMesh101H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh101S8_t::R_StudioSoftwareProcessMesh,
	ProcessMesh101M8_t::R_StudioSoftwareProcessMesh,
#if !defined(_X360)
	ProcessMesh110H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh110S8_t::R_StudioSoftwareProcessMesh,
	ProcessMesh110M8_t::R_StudioSoftwareProcessMesh,
#endif
	ProcessMesh111H8_t::R_StudioSoftwareProcessMesh,		ProcessMesh111S8_t::R_StudioSoftwareProcessMesh,
...
void CStudioRender::R_StudioSoftwareProcessMesh(mstudiomesh_t *pmesh, CMeshBuilder &meshBuilder, int numVertices,
												unsigned short *pGroupToMesh, StudioModelLighting_t lighting,
												bool doFlex, float r_blend, bool bNeedsTangentSpace, bool bDX8Vertex,
												IMaterial *pMaterial)
{
	...
	if(pVertData)
	{
		...
		g_SoftwareProcessMeshFunc[idx](pVertData, m_PoseToWorld, m_VertexCache, meshBuilder, numVertices, pGroupToMesh,
									   nAlphaMask, pMaterial);
	}
}

static void R_SlowTransformVert(const Vector *pSrcPos, const Vector *pSrcNorm, matrix3x4_t *pSkinMat,
								VectorAligned &pos, VectorAligned &norm)
{
	pos.x = pSrcPos->x * (*pSkinMat)[0][0] + pSrcPos->y * (*pSkinMat)[0][1] + pSrcPos->z * (*pSkinMat)[0][2] +
			(*pSkinMat)[0][3];
	norm.x = pSrcNorm->x * (*pSkinMat)[0][0] + pSrcNorm->y * (*pSkinMat)[0][1] + pSrcNorm->z * (*pSkinMat)[0][2];
								...
								matrix3x4_t *pSkinMat, VectorAligned &pos, VectorAligned &norm, VectorAligned &tangentS)
{
	...
	norm.x = pSrcNorm->x * (*pSkinMat)[0][0] + pSrcNorm->y * (*pSkinMat)[0][1] + pSrcNorm->z * (*pSkinMat)[0][2];
	tangentS.x =
		pSrcTangentS->x * (*pSkinMat)[0][0] + pSrcTangentS->y * (*pSkinMat)[0][1] + pSrcTangentS->z * (*pSkinMat)[0][2];

	pos.y = pSrcPos->x * (*pSkinMat)[1][0] + pSrcPos->y * (*pSkinMat)[1][1] + pSrcPos->z * (*pSkinMat)[1][2] +
			(*pSkinMat)[1][3];
	norm.y = pSrcNorm->x * (*pSkinMat)[1][0] + pSrcNorm->y * (*pSkinMat)[1][1] + pSrcNorm->z * (*pSkinMat)[1][2];
	tangentS.y =
		pSrcTangentS->x * (*pSkinMat)[1][0] + pSrcTangentS->y * (*pSkinMat)[1][1] + pSrcTangentS->z * (*pSkinMat)[1][2];

	pos.z = pSrcPos->x * (*pSkinMat)[2][0] + pSrcPos->y * (*pSkinMat)[2][1] + pSrcPos->z * (*pSkinMat)[2][2] +
			(*pSkinMat)[2][3];
	norm.z = pSrcNorm->x * (*pSkinMat)[2][0] + pSrcNorm->y * (*pSkinMat)[2][1] + pSrcNorm->z * (*pSkinMat)[2][2];
	tangentS.z =
		pSrcTangentS->x * (*pSkinMat)[2][0] + pSrcTangentS->y * (*pSkinMat)[2][1] + pSrcTangentS->z * (*pSkinMat)[2][2];
}

void CStudioRender::R_StudioSoftwareProcessMesh_Normals(mstudiomesh_t *pmesh, CMeshBuilder &meshBuilder,
														int numVertices, unsigned short *pGroupToMesh,
														StudioModelLighting_t lighting, bool doFlex, float r_blend,
...
template void CCachedRenderData::ComputeFlexedVertex_StreamOffset<mstudiovertanim_t>(studiohdr_t *pStudioHdr,
																					 mstudioflex_t *pflex,
...
void CStudioRender::R_StudioProcessFlexedMesh_StreamOffset(mstudiomesh_t *pmesh, int lod)
{
	...
	for(int i = 0; i < pmesh->numflexes; i++)
	{
		...
		if(pflex[i].vertanimtype == STUDIO_VERT_ANIM_NORMAL)
		{
			...
			m_VertexCache.ComputeFlexedVertex_StreamOffset_Optimized(m_pStudioHdr, &pflex[i], pvanim, vertCount, w1, w2,
																	 w3, w4);
		}
		else
		{
			mstudiovertanim_wrinkle_t *pvanim = pflex[i].pVertanimWrinkle(0);
			m_VertexCache.ComputeFlexedVertexWrinkle_StreamOffset_Optimized(m_pStudioHdr, &pflex[i], pvanim, vertCount,
																			w1, w2, w3, w4);
		}
...
//-----------------------------------------------------------------------------
void CStudioRender::R_StudioProcessFlexedMesh(mstudiomesh_t *pmesh, CMeshBuilder &meshBuilder, int numVertices,
...
void CStudioRender::R_StudioRestoreMesh(mstudiomesh_t *pmesh, studiomeshgroup_t *pMeshData)
{
	Vector4D *pStudioTangentS;
...
//-----------------------------------------------------------------------------
int CStudioRender::R_StudioDrawGroupHWSkin(IMatRenderContext *pRenderContext, studiomeshgroup_t *pGroup, IMesh *pMesh,
										   ColorMeshInfo_t *pColorMeshInfo)
{
	...
	for(int j = 0; j < pGroup->m_NumStrips; ++j)
	{
		...
		pMesh->SetPrimitiveType(pStrip->flags & OptimizedModel::STRIP_IS_TRISTRIP ? MATERIAL_TRIANGLE_STRIP
																				  : MATERIAL_TRIANGLES);
...
inline VertexFormat_t CStudioRender::ComputeSWSkinVertexFormat(IMaterial *pMaterial) const
{
	...
	VertexFormat_t fmt = VERTEX_POSITION | VERTEX_NORMAL | VERTEX_COLOR | VERTEX_BONE_INDEX | VERTEX_BONEWEIGHT(2) |
										  ...
										  IMaterial *pMaterial, int lod, ColorMeshInfo_t *pColorMeshes)
{
	...
	bool bDoSoftwareLighting =
		...
		((m_pRC->m_Config.bSoftwareSkin != 0) || m_pRC->m_Config.bDrawNormals || m_pRC->m_Config.bDrawTangentFrame ||
		 (pMaterial ? pMaterial->NeedsSoftwareSkinning() : false) || (m_pRC->m_Config.bSoftwareLighting != 0) ||
	...
	if(bDoSoftwareLighting || m_pRC->m_Config.m_bStatsMode == true)
	{
		...
		R_StudioSoftwareProcessMesh(pmesh, meshBuilder, pGroup->m_NumVertices, pGroup->m_pGroupIndexToMeshIndex,
	...
	if(pColorMeshes && (pGroup->m_ColorMeshID != -1))
	{
		...
		numTrianglesRendered =
			R_StudioDrawGroupHWSkin(pRenderContext, pGroup, pGroup->m_pMesh, &(pColorMeshes[pGroup->m_ColorMeshID]));
	}
	else
	{
										   ...
										   IMaterial *pMaterial, int lod)
{
	...
	bool doSoftwareLighting =
		(m_pRC->m_Config.bSoftwareLighting != 0) || ((lighting != LIGHTING_HARDWARE) && (lighting != LIGHTING_MOUTH));

	bool swSkin = doSoftwareLighting || m_pRC->m_Config.bDrawNormals || m_pRC->m_Config.bDrawTangentFrame ||
				  ((pGroup->m_Flags & MESHGROUP_IS_HWSKINNED) == 0) || m_pRC->m_Config.bSoftwareSkin ||
	...
	{
		R_StudioSoftwareProcessMesh(pmesh, meshBuilder, pGroup->m_NumVertices, pGroup->m_pGroupIndexToMeshIndex,
									lighting, doFlex, r_blend, bNeedsTangentSpace, bDX8Vertex, pMaterial);
	}
	else if(doFlex)
	{
		R_StudioProcessFlexedMesh(pmesh, meshBuilder, pGroup->m_NumVertices, pGroup->m_pGroupIndexToMeshIndex);
	}
	...
	if(m_pRC->m_Config.bDrawNormals || m_pRC->m_Config.bDrawTangentFrame)
	{
		...
		R_StudioSoftwareProcessMesh_Normals(pmesh, meshBuilder, pGroup->m_NumVertices, pGroup->m_pGroupIndexToMeshIndex,
											lighting, doFlex, r_blend, m_pRC->m_Config.bDrawNormals,
...
void CStudioRender::SetEyeMaterialVars(IMaterial *pMaterial, mstudioeyeball_t *peyeball, Vector const &eyeOrigin,
...
int CStudioRender::R_StudioDrawEyeball(IMatRenderContext *pRenderContext, mstudiomesh_t *pmesh,
									   studiomeshdata_t *pMeshData, StudioModelLighting_t lighting,
									   IMaterial *pMaterial, int lod)
{
	...
	ComputeGlintTextureProjection(&m_pEyeballState[pmesh->materialparam], m_pRC->m_ViewRight, m_pRC->m_ViewUp,
								  glintMat);
	...
	if(!m_pRC->m_Config.bWireframe)
	{
		...
		if(pGlintVar)
		{
			R_StudioEyeballGlint(&m_pEyeballState[pmesh->materialparam], pGlintVar, m_pRC->m_ViewRight, m_pRC->m_ViewUp,
								 m_pRC->m_ViewOrigin);
		}
	...
	if(bShouldHardwareSkin)
	{
		...
		{
			studiomeshgroup_t *pGroup = &pMeshData->m_pMeshGroup[j];
			numTrianglesRendered += R_StudioDrawStaticMesh(pRenderContext, pmesh, pGroup, lighting, m_pRC->m_AlphaMod,
														   pMaterial, lod, NULL);
		}
	...
	for(j = 0; j < pMeshData->m_NumGroup; ++j)
	{
		...
		for(int i = 0; i < pGroup->m_NumVertices; ++i)
		{
			...
			{
				R_StudioEyeballNormal(peyeball, org, pWorldVert->m_Position.AsVector3D(),
...
// Draws a mesh
//-----------------------------------------------------------------------------
int CStudioRender::R_StudioDrawMesh(IMatRenderContext *pRenderContext, mstudiomesh_t *pmesh,
									studiomeshdata_t *pMeshData, StudioModelLighting_t lighting, IMaterial *pMaterial,
									ColorMeshInfo_t *pColorMeshes, int lod)
{
	...
	for(int j = 0; j < pMeshData->m_NumGroup; ++j)
	{
		...
		bool bShouldHardwareSkin =
			bIsHardwareSkinnedData && (!bIsFlexed || bFlexStatic) && (lighting != LIGHTING_SOFTWARE);

		if(bShouldHardwareSkin && !m_pRC->m_Config.bDrawNormals && !m_pRC->m_Config.bDrawTangentFrame &&
		   !m_pRC->m_Config.bWireframe)
		{
			if(!m_pRC->m_Config.bNoHardware)
			{
				numTrianglesRendered += R_StudioDrawStaticMesh(pRenderContext, pmesh, pGroup, lighting,
															   m_pRC->m_AlphaMod, pMaterial, lod, pColorMeshes);
			}
		}
		else
		{
			...
			{
				numTrianglesRendered +=
					R_StudioDrawDynamicMesh(pRenderContext, pmesh, pGroup, lighting, m_pRC->m_AlphaMod, pMaterial, lod);
			}
		}
	}
	return numTrianglesRendered;
...
// Sorts the meshes
//-----------------------------------------------------------------------------
int CStudioRender::SortMeshes(int *pIndices, IMaterial **ppMaterials, short *pskinref, Vector const &vforward,
...
int CStudioRender::R_StudioDrawPoints(IMatRenderContext *pRenderContext, int skin,
									  void /*IClientEntity*/ *pClientEntity, IMaterial **ppMaterials,
									  int *pMaterialFlags, int boneMask, int lod, ColorMeshInfo_t *pColorMeshes)
{
	...
	for(i = 0; i < m_pSubModel->nummeshes; ++i)
	{
		...
		IMaterial *pMaterial = R_StudioSetupSkinAndLighting(pRenderContext, pskinref[pmesh->material], ppMaterials,
															materialFlags, pClientEntity, pColorMeshes, lighting);
		if(!pMaterial)
			continue;
		...
		switch(pmesh->materialtype)
		{
			...
			default:
				numTrianglesRendered +=
					R_StudioDrawMesh(pRenderContext, pmesh, pMeshData, lighting, pMaterial, pColorMeshes, lod);
				break;
		}
	}
