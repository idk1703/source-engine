...
void CStudioRender::MaterialPlanerProjection(const matrix3x4_t &mat, int count, const Vector *psrcverts,
...
void CStudioRender::R_StudioFlexVerts(mstudiomesh_t *pmesh, int lod)
{
	...
	if(!vertData)
	{
		...
		if(warnCount++ < 20)
			Warning("ERROR: R_StudioFlexVerts, model verts have been compressed, cannot render! (use "
					"\"-no_compressed_vvds\")");
		return;
	}
	...
	if((m_pStudioHdr->flags & STUDIOHDR_FLAGS_FLEXES_CONVERTED) == 0)
	{
		...
		if(flexConversionTimesWarned++ < 6)
			Warning("ERROR: flex verts have not been converted (queued loader refcount bug?) - expect to see "
					"'exploded' faces");
	}
...
// static void R_StudioEyeballNormals( const mstudioeyeball_t *peyeball, int count, const Vector *psrcverts, Vector
// *pdestnorms )
...
void CStudioRender::AddGlint(CPixelWriter &pixelWriter, float x, float y, const Vector &color)
{
	...
	for(int v = y0; v <= y1; ++v)
	{
		...
		for(int u = x0; u <= x1; ++u)
		{
			...
			pixelWriter.WritePixel(LinearToTexture(outColor.x), LinearToTexture(outColor.y),
...
public:
	virtual void RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pRect)
	{
		...
		int nGlintCount = m_pStudioRender->BuildGlintRenderData(pRenderData, ARRAYSIZE(pRenderData), m_pState,
																*m_pVRight, *m_pVUp, *m_pROrigin);
		...
		for(int i = 0; i < nGlintCount; ++i)
		{
			...
			m_pStudioRender->AddGlint(pixelWriter, pRenderData[i].m_vecPosition[0], pRenderData[i].m_vecPosition[1],
...
void CStudioRender::PrecacheGlint()
{
	if(!m_pGlintTexture)
	{
		...
		{
			// Get the texture that we are going to be updating procedurally.
			s_pProcGlint = g_pMaterialSystem->CreateProceduralTexture("proc_eyeglint", TEXTURE_GROUP_MODEL, 32, 32,
...
int CStudioRender::BuildGlintRenderData(GlintRenderData_t *pData, int nMaxGlints, const eyeballstate_t *pState,
...
//-----------------------------------------------------------------------------
ITexture *CStudioRender::RenderGlintTexture(const eyeballstate_t *pState, const Vector &vright, const Vector &vup,
											const Vector &r_origin)
{
	GlintRenderData_t pRenderData[16];
	int nGlintCount = BuildGlintRenderData(pRenderData, ARRAYSIZE(pRenderData), pState, vright, vup, r_origin);
...
void CStudioRender::R_StudioEyeballGlint(const eyeballstate_t *pstate, IMaterialVar *pGlintVar, const Vector &vright,
...
void CStudioRender::ComputeGlintTextureProjection(eyeballstate_t const *pState, const Vector &vright, const Vector &vup,
												  matrix3x4_t &mat)
{
