...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CStudioRenderContext, IStudioRender, STUDIO_RENDER_INTERFACE_VERSION,
...
//-----------------------------------------------------------------------------
static const char *GetTextureName(studiohdr_t *phdr, OptimizedModel::FileHeader_t *pVtxHeader, int lodID,
								  int inMaterialID)
{
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::LoadMaterials(studiohdr_t *phdr, OptimizedModel::FileHeader_t *pVtxHeader,
...
int CStudioRenderContext::CountFlexedVertices(mstudiomesh_t *pMesh, OptimizedModel::StripGroupHeader_t *pStripGroup)
{
	...
	unsigned short *pMeshIndexToGroupIndex =
		(unsigned short *)_alloca(pMesh->pModel()->numvertices * sizeof(unsigned short));
...
bool CStudioRenderContext::R_AddVertexToMesh(const char *pModelName, bool bNeedsTangentSpace, CMeshBuilder &meshBuilder,
											 ...
											 const mstudio_meshvertexdata_t *vertData, bool hwSkin)
{
	...
	*/
	...
	if(vertData->HasTangentData())
	{
		...
		Warning( "MODELBUG %s: nearly colinear tangentS (%f %f %f) and normal (%f %f %f) at position %f %f %f Probably
		have 2 or more texcoords that are the same on a triangle.\n", pModelName, tangentS.x, tangentS.y, tangentS.y,
		vert.m_vecNormal.x, vert.m_vecNormal.y, vert.m_vecNormal.z, vert.m_vecPosition.x, vert.m_vecPosition.y,
		vert.m_vecPosition.z );
		}
		}
		#endif
		}
...
// Builds the group
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioBuildMeshGroup(const char *pModelName, bool bNeedsTangentSpace,
												  ...
												  studiohdr_t *pStudioHdr, VertexFormat_t vertexFormat)
{
	...
	meshBuilder.Begin(pMeshGroup->m_pMesh, MATERIAL_HETEROGENOUS, hwSkin ? pStripGroup->numVerts : 0,
			...
			switch(compressionType)
			{
				case VERTEX_COMPRESSION_ON:
					success = R_AddVertexToMesh<VERTEX_COMPRESSION_ON>(
						pModelName, bNeedsTangentSpace, meshBuilder, pStripGroup->pVertex(i), pMesh, vertData, hwSkin);
					break;
...
// Builds the group
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioBuildMorph(studiohdr_t *pStudioHdr, studiomeshgroup_t *pMeshGroup,
												   ...
												   OptimizedModel::StripGroupHeader_t *pStripGroup)
{
	...
	for(i = 0; i < pStripGroup->numStrips; ++i)
	{
		...
		stripDataSize += pStripGroup->pStrip(i)->numBoneStateChanges * sizeof(OptimizedModel::BoneStateChangeHeader_t);
	}
	...
	for(i = 0; i < pStripGroup->numStrips; ++i)
	{
		memcpy(&pMeshGroup->m_pStripData[i], pStripGroup->pStrip(i), sizeof(OptimizedModel::StripHeader_t));
		...
		int boneWeightSize =
			pMeshGroup->m_pStripData[i].numBoneStateChanges * sizeof(OptimizedModel::BoneStateChangeHeader_t);
...
//-----------------------------------------------------------------------------
VertexFormat_t CStudioRenderContext::CalculateVertexFormat(const studiohdr_t *pStudioHdr,
	...
	else
	{
		...
		for(int i = 0; i < pStudioHdr->numskinfamilies; i++)
		{
			...
			VertexFormat_t vertexFormat =
				bIsDX7 ? MATERIAL_VERTEX_FORMAT_MODEL_DX7 : (MATERIAL_VERTEX_FORMAT_MODEL & ~VERTEX_COLOR);
			...
			// FIXME: re-enable this test, fix it to work and see how much memory we save (Q: why is this different to
...
bool CStudioRenderContext::MeshNeedsTangentSpace(studiohdr_t *pStudioHdr, studioloddata_t *pStudioLodData,
												 mstudiomesh_t *pMesh)
{
	// iterate each skin table
...
void CStudioRenderContext::R_StudioCreateSingleMesh(studiohdr_t *pStudioHdr, studioloddata_t *pStudioLodData,
													mstudiomesh_t *pMesh, OptimizedModel::MeshHeader_t *pVtxMesh,
													int numBones, studiomeshdata_t *pMeshData, int *pColorMeshID)
{
	...
	for(int i = 0; i < pVtxMesh->numStripGroups; ++i)
	{
		...
		VertexFormat_t vertexFormat =
			CalculateVertexFormat(pStudioHdr, pStudioLodData, pMesh, pStripGroup, bIsHwSkinned);

		// Build the vertex + index buffers
		R_StudioBuildMeshGroup(pStudioHdr->pszName(), bNeedsTangentSpace, pMeshGroup, pStripGroup, pMesh, pStudioHdr,
							   vertexFormat);
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioCreateStaticMeshes(studiohdr_t *pStudioHdr, OptimizedModel::FileHeader_t *pVtxHdr,
	...
	for(i = 0; i < pStudioHdr->numbodyparts; i++)
	{
		...
		for(j = 0; j < pBodyPart->nummodels; ++j)
		{
			...
			for(k = 0; k < pModel->nummeshes; ++k)
			{
				...
				R_StudioCreateSingleMesh(pStudioHdr, &pStudioHWData->m_pLODs[nLodID], pMesh, pVtxMesh,
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::BuildDecalBoneMap(studiohdr_t *pStudioHdr, int *pUsedBones, int *pBoneRemap,
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::ComputeHWMorphDecalBoneRemap(studiohdr_t *pStudioHdr, OptimizedModel::FileHeader_t *pVtxHdr,
...
int CStudioRenderContext::GetMaterialList(studiohdr_t *pStudioHdr, int count, IMaterial **ppMaterials)
{
	...
	for(i = 0; i < pStudioHdr->numtextures; i++)
	{
		...
		for(j = 0; j < pStudioHdr->numcdtextures && IsErrorMaterial(pMaterial); j++)
		{
			...
			{
				pMaterial = g_pMaterialSystem->FindMaterialEx("models/obsolete/obsolete", TEXTURE_GROUP_MODEL,
															  MATERIAL_FINDCONTEXT_ISONAMODEL, false);
			}
			else
			{
				pMaterial = g_pMaterialSystem->FindMaterialEx(szPath, TEXTURE_GROUP_MODEL,
															  MATERIAL_FINDCONTEXT_ISONAMODEL, false);
			}
		}
...
int CStudioRenderContext::GetMaterialListFromBodyAndSkin(MDLHandle_t studio, int nSkin, int nBody,
														 int nCountOutputMaterials, IMaterial **ppOutputMaterials)
{
	int found = 0;
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::GetPerfStats(DrawModelResults_t *pResults, const DrawModelInfo_t &info,
...
// Sets the view state
//-----------------------------------------------------------------------------
void CStudioRenderContext::SetViewState(const Vector &viewOrigin, const Vector &viewRight, const Vector &viewUp,
...
// Computes LOD
//-----------------------------------------------------------------------------
int CStudioRenderContext::ComputeRenderLOD(IMatRenderContext *pRenderContext, const DrawModelInfo_t &info,
										   const Vector &origin, float *pMetric)
{
	int lod = info.m_Lod;
									 ...
									 const Vector &origin, int flags)
{
	...
	if(!info.m_pStudioHdr || !info.m_pHardwareData || !info.m_pHardwareData->m_NumLODs ||
	   !info.m_pHardwareData->m_pLODs)
	{
		return;
	}
...
void CStudioRenderContext::DrawModelArray(const DrawModelInfo_t &drawInfo, int arrayCount,
										  model_array_instance_t *pInstanceData, int instanceStride, int flags)
{
...
void CStudioRenderContext::DrawStaticPropShadows(const DrawModelInfo_t &info, const matrix3x4_t &modelToWorld,
												 int flags)
{
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::AddShadow(IMaterial *pMaterial, void *pProxyData, FlashlightState_t *pFlashlightState,
									 VMatrix *pWorldToTexture, ITexture *pFlashlightDepthTexture)
{
	...
	if(!pCallQueue || studio_queue_mode.GetInt() == 0)
	{
		g_pStudioRenderImp->AddShadow(pMaterial, pProxyData, pFlashlightState, pWorldToTexture,
	...
	else
	{
		...
		pCallQueue->QueueCall(g_pStudioRenderImp, &CStudioRender::AddShadow, pMaterial, (void *)NULL,
							  rdFlashlight.Base(), rdMatrix.Base(), pFlashlightDepthTexture);
	}
}
...
void CStudioRenderContext::AddDecal(StudioDecalHandle_t handle, studiohdr_t *pStudioHdr, matrix3x4_t *pBoneToWorld,
									...
									int body, bool noPokethru, int maxLODToDecal)
{
	...
	QUEUE_STUDIORENDER_CALL_RC(AddDecal, CStudioRender, g_pStudioRenderImp, pRenderContext, handle, m_RC, pBoneToWorld,
							   pStudioHdr, ray, decalUp, pDecalMaterial, radius, body, noPokethru, maxLODToDecal);
}
...
IMaterial *GetModelSpecificDecalMaterial(IMaterial *pDecalMaterial)
{
	...
	if(found)
	{
		IMaterial *pModelMaterial =
			g_pMaterialSystem->FindMaterial(pModelMaterialVar->GetStringValue(), TEXTURE_GROUP_DECAL, false);
