...
void CStudioRender::R_GatherStats(studiomeshgroup_t *pGroup, CMeshBuilder &MeshBuilder, IMesh *pMesh,
								  IMaterial *pMaterial)
{
	int nCount = 0;
	...
	for(int j = 0; j < pGroup->m_NumStrips; ++j)
	{
		...
		for(int i = 0; i < pStrip->numIndices; i += 3)
		{
			...
			float flTriArea =
				TriangleArea(*(const Vector *)(pPos1), *(const Vector *)(pPos2), *(const Vector *)(pPos3));
	...
	//	Msg( "%d / %d / %g / %g ||| %d / %g\n", MeshBuilder.VertexCount(), nCount, flSurfaceArea, flTriSurfaceArea,
	//nFrontFacing, flTriSurfaceArea / (float)nFrontFacing );
	...
	for(int j = 0; j < pGroup->m_NumStrips; ++j)
	{
		...
		for(int i = 0; i < pStrip->numIndices; i += 3)
		{
			...
			if(flArea > 0.0f)
			{
				...
				{
					float flArea =
						TextureAreas[(pStrip->indexOffset + i) / 3] / TriAreas[(pStrip->indexOffset + i) / 3];
...
//-----------------------------------------------------------------------------
void CStudioRender::ModelStats(const DrawModelInfo_t &info, const StudioRenderContext_t &rc, matrix3x4_t *pBoneToWorld,
