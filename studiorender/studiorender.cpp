...
void CStudioRender::InitDebugMaterials(void)
{
	...
	m_pMaterialMRMWireframeZBuffer = g_pMaterialSystem->FindMaterial(
		"//platform/materials/debug/debugmrmwireframezbuffer", TEXTURE_GROUP_OTHER, true);
	...
	m_pMaterialTranslucentModelHulls = g_pMaterialSystem->FindMaterial(
		"//platform/materials/debug/debugtranslucentmodelhulls", TEXTURE_GROUP_OTHER, true);
	...
	m_pMaterialAdditiveVertexColorVertexAlpha = g_pMaterialSystem->FindMaterial(
		"//platform/materials/debug/additivevertexcolorvertexalpha", TEXTURE_GROUP_OTHER, true);
...
//-----------------------------------------------------------------------------
void CStudioRender::AddShadow(IMaterial *pMaterial, void *pProxyData, FlashlightState_t *pFlashlightState,
...
//-----------------------------------------------------------------------------
void CStudioRender::DrawModel(const DrawModelInfo_t &info, const StudioRenderContext_t &rc, matrix3x4_t *pBoneToWorld,
...
void CStudioRender::DrawModelStaticProp(const DrawModelInfo_t &info, const StudioRenderContext_t &rc,
										const matrix3x4_t &rootToWorld, int flags)
{
	...
	R_StudioRenderModel(pRenderContext, info.m_Skin, info.m_Body, info.m_HitboxSet, info.m_pClientEntity,
						...
						info.m_pHardwareData->m_pLODs[lod].pMaterialFlags, flags, BONE_USED_BY_ANYTHING, lod,
						info.m_pColorMeshes);
...
void CStudioRender::DrawModelArray(const DrawModelInfo_t &drawInfo, const StudioRenderContext_t &rc, int arrayCount,
								   model_array_instance_t *pInstanceData, int instanceStride, int flags)
{
	...
	for(int body = 0; body < pStudioHdr->numbodyparts; ++body)
	{
		...
		for(int meshIndex = 0; meshIndex < pSubmodel->nummeshes; ++meshIndex)
		{
			...
			IMaterial *pMaterial =
				R_StudioSetupSkinAndLighting(pRenderContext, pskinref[pmesh->material], ppMaterials, materialFlags,
											 drawInfo.m_pClientEntity, NULL, lighting);
			if(!pMaterial)
				continue;
						...
						for(int j = 0; j < pGroup->m_NumStrips; ++j)
						{
							...
							for(int k = 0; k < pStrip->numBoneStateChanges; ++k)
							{
								...
								pRenderContext->LoadBoneMatrix(pStateChange->hardwareID,
															   pBones[pStateChange->newBoneID]);
							}
							MaterialPrimitiveType_t localStripType = pStrip->flags & OptimizedModel::STRIP_IS_TRISTRIP
