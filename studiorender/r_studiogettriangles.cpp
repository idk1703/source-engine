...
void CStudioRenderContext::GetTriangles(const DrawModelInfo_t &info, matrix3x4_t *pBoneToWorld,
		...
		for(k = 0; k < pModel->nummeshes; ++k)
		{
			...
			for(stripGroupID = 0; stripGroupID < pMeshData->m_NumGroup; stripGroupID++)
			{
				...
				for(stripID = 0; stripID < pMeshGroup->m_NumStrips; stripID++)
				{
					...
					//pStripData->pBoneStateChange( boneID ); 						hardwareBoneToGlobalBone[pBoneStateChange->hardwareID] =
