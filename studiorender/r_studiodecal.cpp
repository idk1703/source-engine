...
//-----------------------------------------------------------------------------
bool CStudioRender::ProjectDecalOntoMesh(DecalBuildInfo_t &build, DecalBuildVertexInfo_t *pVertexInfo,
										 mstudiomesh_t *pMesh)
{
...
//-----------------------------------------------------------------------------
static int IntersectPlane(DecalClipState_t &state, int start, int end, int normalInd, float val)
{
	...
	// Interpolate normal
	Vector vNormal;
	// FIXME: this is a bug (it's using position data to compute interpolated normals!)... not seeing any obvious
	// artifacts, though
...
//-----------------------------------------------------------------------------
void CStudioRender::ConvertMeshVertexToDecalVertex(DecalBuildInfo_t &build, int meshIndex, DecalVertex_t &decalVertex,
												   int nGroupIndex)
{
	// Copy over the data;
...
inline unsigned short CStudioRender::AddVertexToDecal(DecalBuildInfo_t &build, DecalVertex_t &vert)
{
	...
	for(i = build.m_FirstVertex; i != decalVertexList.InvalidIndex(); i = decalVertexList.Next(i), ++vertexCount)
	{
		...
		for(int k = 0; k < pGroup->m_NumStrips; ++k)
		{
			...
			if(pStrip->flags & OptimizedModel::STRIP_IS_TRISTRIP)
			{
				for(int i = 0; i < pStrip->numIndices - 2; ++i)
				{
					...
					AddTriangleToDecal(build, i1, i2, i3, pGroup->m_pIndices[ti1], pGroup->m_pIndices[ti2],
									   pGroup->m_pIndices[ti3]);
				}
			}
			else
			{
				...
				for(int i = 0; i < pStrip->numIndices; i += 3)
				{
					...
					AddTriangleToDecal(build, i1, i2, i3, pGroup->m_pIndices[idx], pGroup->m_pIndices[idx + 1],
...
//-----------------------------------------------------------------------------
int CStudioRender::ComputeVertexAllocation(int iMaxLOD, int body, studiohwdata_t *pHardwareData,
...
void CStudioRender::AddDecal(StudioDecalHandle_t hDecal, const StudioRenderContext_t &rc, matrix3x4_t *pBoneToWorld,
							 studiohdr_t *pStudioHdr, const Ray_t &ray, const Vector &decalUp,
							 IMaterial *pDecalMaterial, float radius, int body, bool noPokethru, int maxLODToDecal)
{
	...
	if(IsX360())
	{
		while(g_nTotalDecalVerts * sizeof(DecalVertex_t) > 256 * 1024 && m_DecalLRU.Head() != m_DecalLRU.InvalidIndex())
		{
			...
			DecalModelList_t &modelList = m_DecalList[(uintp)hRetire];
	...
	if(pHistoryList->Count() >= m_pRC->m_Config.maxDecalsPerModel)
	{
		...
		RetireDecal(m_DecalList[(uintp)hRetire], nRetireID, nRootLOD, nFinalLOD);
	}
...
void CStudioRender::DrawSingleBoneFlexedDecals(IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder,
...
//-----------------------------------------------------------------------------
bool CStudioRender::DrawMultiBoneDecals(CMeshBuilder &meshBuilder, DecalMaterial_t &decalMaterial,
										studiohdr_t *pStudioHdr)
{
	...
	for(DecalVertexList_t::IndexLocalType_t i = verts.Head(); i != verts.InvalidIndex(); i = verts.Next(i))
	{
		...
		else
		{
			...
			VectorMA(pCachedVert->m_Position.AsVector3D(), 0.1, pCachedVert->m_Normal.AsVector3D(),
											  ...
											  studioloddata_t *pStudioLOD)
{
	...
	for(DecalVertexList_t::IndexLocalType_t i = verts.Head(); i != verts.InvalidIndex(); i = verts.Next(i))
	{
		...
		if(!pMorph)
		{
			...
			if(meshBuilder.NumBoneWeights() >
			   0) // bone weight of 0 will not write anything, so these calls would be wasted
			{
				meshBuilder.BoneWeight(0, 1.0f);
				meshBuilder.BoneWeight(1, 0.0f);
				meshBuilder.BoneWeight(2, 0.0f);
...
//-----------------------------------------------------------------------------
void CStudioRender::DrawDecalMaterial(IMatRenderContext *pRenderContext, DecalMaterial_t &decalMaterial,
...
//-----------------------------------------------------------------------------
static Vector s_pWhite[6] = {Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0),
...
void CStudioRender::DrawDecal(const DrawModelInfo_t &drawInfo, int lod, int body)
{
	...
	const DecalModelList_t &list = m_DecalList[(uintp)handle];
...
void CStudioRender::DrawStaticPropDecals(const DrawModelInfo_t &drawInfo, const StudioRenderContext_t &rc,
