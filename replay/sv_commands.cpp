...
void PrintSessionInfo(const char *pFilename)
{
	...
	else
	{
		...
		pCompressor->Decompress((char *)pUncompressedPayload, &uUncompressedPayloadSize, (const char *)pPayload,
	...
	else
	{
		...
		Msg("\n\tblocks:\n\n");
		Msg("\t   index        status                        MD5               compressor    size (uncompressed)  size "
			"(compressed)\n");
		...
		for(int i = 0; i < header.m_nNumBlocks; ++i)
		{
			...
			Msg("%20s", CBaseRecordingSessionBlock::GetRemoteStatusStringSafe(
							(CBaseRecordingSessionBlock::RemoteStatus_t)DummyBlock.m_uRemoteStatus));
			Msg("%35s", szDigestStr);
