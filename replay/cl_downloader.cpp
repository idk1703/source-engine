...
void CHttpDownloader::Think()
{
	...
	if(!m_pThreadState->shouldStop)
	{
		...
		const bool bEvilFileSize = m_pThreadState->nBytesTotal && m_pThreadState->nBytesTotal >= DOWNLOAD_MAX_SIZE;
#if _DEBUG
		...
		switch(m_pThreadState->status)
		{
			...
			case HTTP_ERROR:
				...
				m_uBytesDownloaded = (uint32)m_pThreadState->nBytesCurrent;
				m_uSize = m_pThreadState->nBytesTotal; // NOTE: Need to do this here in the case that a file is small
													   // enough that we never hit HTTP_FETCH
				m_bDone = true;
...
/*static*/ const char *CHttpDownloader::GetHttpErrorToken(HTTPError_t nError)
{
	switch(nError)
	{
		case HTTP_ERROR_ZERO_LENGTH_FILE:
			return "#HTTPError_ZeroLengthFile";
		case HTTP_ERROR_CONNECTION_CLOSED:
			return "#HTTPError_ConnectionClosed";
		case HTTP_ERROR_INVALID_URL:
			return "#HTTPError_InvalidURL";
		case HTTP_ERROR_INVALID_PROTOCOL:
			return "#HTTPError_InvalidProtocol";
		case HTTP_ERROR_CANT_BIND_SOCKET:
			return "#HTTPError_CantBindSocket";
		case HTTP_ERROR_CANT_CONNECT:
			return "#HTTPError_CantConnect";
		case HTTP_ERROR_NO_HEADERS:
			return "#HTTPError_NoHeaders";
