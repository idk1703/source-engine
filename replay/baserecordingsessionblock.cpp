...
bool CBaseRecordingSessionBlock::Read(KeyValues *pIn)
{
	...
	m_nRemoteStatus = (RemoteStatus_t)pIn->GetInt("remote_status", (int)STATUS_INVALID);
	Assert(m_nRemoteStatus != STATUS_INVALID);
	m_nHttpError = (Error_t)pIn->GetInt("error", (int)ERROR_NONE);
	m_iReconstruction = pIn->GetInt("recon_index", -1);
	Assert(m_iReconstruction >= 0);
	m_hSession = (ReplayHandle_t)pIn->GetInt("session", REPLAY_HANDLE_INVALID);
	Assert(m_hSession != REPLAY_HANDLE_INVALID);
	m_uFileSize = pIn->GetInt("size", 0);
...
{
	CFmtStr fmtHash("%03i %03i %03i %03i %03i %03i %03i %03i %03i %03i %03i %03i %03i %03i %03i %03i", m_aHash[0],
					m_aHash[1], m_aHash[2], m_aHash[3], m_aHash[4], m_aHash[5], m_aHash[6], m_aHash[7], m_aHash[8],
					m_aHash[9], m_aHash[10], m_aHash[11], m_aHash[12], m_aHash[13], m_aHash[14], m_aHash[15]);
