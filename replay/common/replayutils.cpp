									  ...
									  const char *pFilePath, int nStartIndex)
{
	...
	while(1)
	{
		...
		CFmtStr fmtTestFilename("%s%c%s", pFilePath, CORRECT_PATH_SEPARATOR, pDst);
...
void Replay_ConstructReplayFilenameString(CUtlString &strOut, const char *pReplaySubDir, const char *pFilename,
										  const char *pGameDir)
{
	// Construct full filename
	strOut.Format("%s%creplays%c%s%c%s", pGameDir, CORRECT_PATH_SEPARATOR, CORRECT_PATH_SEPARATOR, pReplaySubDir,
...
{
	return Replay_va("%s%creplays%c", Replay_GetGameDir(), CORRECT_PATH_SEPARATOR, CORRECT_PATH_SEPARATOR);
}
