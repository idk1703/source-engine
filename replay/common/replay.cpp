...
bool CReplay::Read(KeyValues *pIn)
{
	...
	m_iMaxSessionBlockRequired = pIn->GetInt("max_block", 0);
	Assert(m_iMaxSessionBlockRequired >= 0);
	m_flStartTime = pIn->GetFloat("start_time", -1.0f);
	Assert(m_flStartTime >= 0.0f);
...
{
	return !m_strReconstructedFilename.IsEmpty() && g_pFullFileSystem->FileExists(m_strReconstructedFilename.Get());
}
