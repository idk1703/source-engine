...
CReplayTime::CReplayTime() : m_fDate(0), m_fTime(0) {}
														...
														bool bForceFullFormat /*=false*/)
{
	...
	bool bToday =
		(pNowTime->tm_mday == nDay) && (pNowTime->tm_mon + 1 == nMonth) && (1900 + pNowTime->tm_year == nYear);

	// Yesterday?
	time_t yesterday = today - time_t(86400);
	tm *pYesterdayTime = localtime(&yesterday);
	bool bYesterday = (pYesterdayTime->tm_mday == nDay) && (pYesterdayTime->tm_mon + 1 == nMonth) &&
					  (1900 + pYesterdayTime->tm_year == nYear);
	...
	if(bTime)
	{
		...
		{
			pLocalize->ConstructString(s_wBuf, sizeof(s_wBuf), pLocalize->Find("#Replay_DateAndTime"), 4, pMonth, pDay,
									   pYear, pTime);
		}
		else
		{
			pLocalize->ConstructString(s_wBuf, sizeof(s_wBuf), pLocalize->Find("#Replay_SingleWordDateAndTime"), 2,
									   bToday ? pToday : pYesterday, pTime);
		}
	}
	else
	{
		...
		{
			pLocalize->ConstructString(s_wBuf, sizeof(s_wBuf), pLocalize->Find("#Replay_Date"), 3, pMonth, pDay, pYear);
		}
		else
		{
