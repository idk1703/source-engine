...
CServerRecordingSessionBlock::CServerRecordingSessionBlock(IReplayContext *pContext)
	: CBaseRecordingSessionBlock(pContext), m_nWriteStatus(WRITESTATUS_INVALID), m_pPublisher(NULL)
{
}
...
bool CServerRecordingSessionBlock::Read(KeyValues *pIn)
{
	...
	m_nWriteStatus = (WriteStatus_t)pIn->GetInt("write_status", (int)WRITESTATUS_INVALID);
	Assert(m_nWriteStatus != WRITESTATUS_INVALID);
	V_strcpy_safe(m_szFullFilename, pIn->GetString("filename"));
	Assert(V_strlen(m_szFullFilename) > 0);
...
void CServerRecordingSessionBlock::Write(KeyValues *pOut)
{
	...
	pOut->SetInt("write_status", (int)m_nWriteStatus);
	Assert(m_nWriteStatus != WRITESTATUS_INVALID);
