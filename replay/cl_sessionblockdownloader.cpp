...
CSessionBlockDownloader::CSessionBlockDownloader() : m_nMaxBlock(-1) {}
	...
	if(!AtMaxConcurrentDownloads())
	{
		...
		FOR_EACH_OBJ(pBlockManager, i)
		{
			...
			CClientRecordingSession *pSession =
				CL_CastSession(CL_GetRecordingSessionManager()->Find(pBlock->m_hSession));
			if(!pSession)
			{
			...
			const char *pURL =
				pForceURL[0] ? pForceURL : Replay_va("%s%s", pSession->m_strBaseDownloadURL.Get(), pFilename);
#else
			...
			IF_REPLAY_DBG(Warning("%s block %i from %s to path %s...\n",
...
{
	CClientRecordingSessionBlock *pBlock = (CClientRecordingSessionBlock *)pDownloader->GetUserData();
	AssertValidReadPtr(pBlock);
...
void CSessionBlockDownloader::OnDownloadComplete(CHttpDownloader *pDownloader, const unsigned char *pData)
{
	...
	switch(nStatus)
	{
...

		{
			...
			const bool bSizesDiffer =
				replay_simulate_size_discrepancy.GetBool() || pBlock->m_uFileSize != pDownloader->GetBytesDownloaded();
			const bool bHashFail =
				replay_simulate_bad_hash.GetBool() || !pBlock->ValidateData(pData, nSize, aLocalHash);
#else
			...
			if(bTryAgain)
			{
				...
				// Report error to OGS.
				CL_GetErrorSystem()->OGS_ReportSessionBlockDownloadError(pDownloader, pBlock,
																		 pDownloader->GetBytesDownloaded(), m_nMaxBlock,
																		 &bSizesDiffer, &bHashFail, aLocalHash);
			}
		}
		...
		case HTTP_ERROR:
			...
			// Report the error to user & OGS
			{
				...
				CL_GetErrorSystem()->AddFormattedErrorFromTokenName("#Replay_DL_Err_HTTP_Prefix",
																	new KeyValues("args", "err", pToken));
			}
	...
	CClientRecordingSession *pSession =
		CL_CastSession(CL_GetRecordingSessionManager()->FindSession(pBlock->m_hSession));
	Assert(pSession);
