...
//
class CClientRecordingSessionManager : public CBaseRecordingSessionManager, public IGameEventListener2
{
...
public:
	...
	void ClearServerRecordingState()
	{
	...
	struct ServerRecordingState_t
	{
		...
		{
			m_strSessionName = "";
			m_nDumpInterval = m_nCurrentBlock = m_nStartTick = 0;
		}
		...
		int m_nCurrentBlock; // Current session block being written to on the server (approximation - may be ahead but
							 // never behind)
		int m_nStartTick; // The tick on the server when the session began - used to calculate an adjusted spawn tick on
						  // the client

		bool IsValid()
		{
			return !m_strSessionName.IsEmpty() && m_nDumpInterval >= MIN_SERVER_DUMP_INTERVAL &&
