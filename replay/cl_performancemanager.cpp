...
void CReplayPerformanceManager::DeletePerformance(CReplayPerformance *pPerformance)
{
	...
	pOwnerReplay->m_vecPerformances.FindAndRemove(
		pPerformance); // This can fail if the replay doesn't own the performance yet - which is no problem.
...
const char *CReplayPerformanceManager::GeneratePerformanceFilename(CReplay *pReplay)
{
	...
	Replay_GetFirstAvailableFilename(s_szBaseFilename, sizeof(s_szBaseFilename), szIdealBaseFilename,
									 "." GENERIC_FILE_EXTENSION, CL_GetPerformanceManager()->GetRelativePath(),
									 pReplay->GetPerformanceCount());
