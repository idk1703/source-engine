...
CON_COMMAND(save_replay, "Save a replay of the current life if possible.")
{
	...
	CClientRecordingSession *pSession =
		CL_CastSession(CL_GetRecordingSessionManager()->Find(pReplayForCurrentLife->m_hSession));
	if(!pSession)
	{
...
CON_COMMAND_F(replay_deleteclientreplays,
...
CON_COMMAND_F(replay_printclientreplays, "Prints out all client replay info", FCVAR_DONTRECORD)
{
	FOR_EACH_REPLAY(i)
	{
		...
		// TODO: All of this should go into a virtual function in CReplay, rather than some here and some in
		// DumpGameSpecificData()
		...
		{
			case CReplay::REPLAYSTATUS_INVALID:
				pStatus = "invalid";
				break;
			case CReplay::REPLAYSTATUS_DOWNLOADPHASE:
				pStatus = "download phase";
				break;
			case CReplay::REPLAYSTATUS_READYTOCONVERT:
				pStatus = "ready to convert";
				break;
			case CReplay::REPLAYSTATUS_RENDERING:
				pStatus = "rendering";
				break;
			case CReplay::REPLAYSTATUS_RENDERED:
				pStatus = "rendered";
				break;
			case CReplay::REPLAYSTATUS_ERROR:
				pStatus = "error";
				break;
			default:
				pStatus = "";
