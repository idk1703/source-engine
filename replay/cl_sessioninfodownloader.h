...
class CSessionInfoDownloader : public CBaseThinker, public IDownloadHandler
{
public:
	...
	enum ServerSessionInfoError_t
	{
		...
		ERROR_BAD_NUM_BLOCKS,	// The "nb" field either didn't exist or was invalid - there should always been at least
								// one block by the time we're downloading
		ERROR_REPLAY_NOT_FOUND, // The server index was downloaded but the replay was not found
		ERROR_INVALID_REPLAY_STATUS, // The server index was downloaded and the replay was found, but it had an invalid
									 // status
		ERROR_INVALID_ORDER,		 // The server index was downloaded and the replay was found, but it had an invalid
									 // reconstruction order (-1)
		ERROR_NO_SESSION_NAME,		 // No session name for entry
		ERROR_UNKNOWN_SESSION, // The session info file points to a session (via its name) that the client doesn't know
							   // about
