...
CScreenshotManager::CScreenshotManager() : m_flLastScreenshotTime(0.0f), m_hScreenshotReplay(REPLAY_HANDLE_INVALID) {}
...
bool CScreenshotManager::Init()
{
	...
	CFmtStr fmtThumbnailsPath("%s%cmaterials%cvgui%creplay%cthumbnails", g_pEngine->GetGameDir(),
							  CORRECT_PATH_SEPARATOR, CORRECT_PATH_SEPARATOR, CORRECT_PATH_SEPARATOR,
...
void CScreenshotManager::Think()
{
	...
	IReplayScreenshotSystem *pScreenshotSystem = g_pClient->GetReplayScreenshotSystem();
	Assert(pScreenshotSystem);
	...
	if(!CL_GetMovieManager()->IsRendering() &&
	   (m_nPrevScreenDims[0] != nScreenWidth || m_nPrevScreenDims[1] != nScreenHeight))
	{
...
void CScreenshotManager::DoCaptureScreenshot()
{
	...
	V_snprintf(szScreenshotPath, sizeof(szScreenshotPath), "materials\\vgui\\replay\\thumbnails\\%s.vtf",
			   szBaseFilename);
	...
	V_snprintf(szVTFFullPath, sizeof(szVTFFullPath), "%s\\materials\\vgui\\replay\\thumbnails\\%s.vtf",
			   g_pEngine->GetGameDir(), szBaseFilename);
...
void CScreenshotManager::DeleteScreenshotsForReplay(CReplay *pReplay)
{
	...
	for(int i = 0; i < pReplay->GetScreenshotCount(); ++i)
	{
		...
		V_snprintf(szFilename, sizeof(szFilename), "materials\\vgui\\replay\\thumbnails\\%s.vtf",
				   pScreenshot->m_szBaseFilename);
		V_FixSlashes(szFilename);
		g_pFullFileSystem->RemoveFile(szFilename);

		// Delete the VGUI thumbnail VMT
		V_snprintf(szFilename, sizeof(szFilename), "materials\\vgui\\replay\\thumbnails\\%s.vmt",
				   pScreenshot->m_szBaseFilename);
		V_FixSlashes(szFilename);
		g_pFullFileSystem->RemoveFile(szFilename);
	}
