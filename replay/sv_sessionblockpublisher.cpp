...
CSessionBlockPublisher::CSessionBlockPublisher(CServerRecordingSession *pSession,
	...
	: m_pSession(pSession), m_pSessionInfoPublisher(pSessionInfoPublisher)
{
...
void CSessionBlockPublisher::WriteAndPublishSessionBlock()
{
	...
	g_pEngine->GetSessionRecordBuffer(
		&pSessionBuffer, &nSessionBufferSize); // This will get called the last client disconnects from the server - but
											   // in waiting for players state we won't have a demo buffer
	if(!pSessionBuffer || nSessionBufferSize == 0)
		return;

	// Create a new block
	CServerRecordingSessionBlock *pNewBlock =
		SV_CastBlock(SV_GetRecordingSessionBlockManager()->CreateAndGenerateHandle());
	if(!pNewBlock)
	{
	...
	const char *pFullFilename =
		Replay_va("%s%s_part_%u.%s", SV_GetTmpDir(), SV_GetRecordingSessionManager()->GetCurrentSessionName(),
				  iCurrentSessionBlock, BLOCK_FILE_EXTENSION);
	...
	IF_REPLAY_DBG(Warning("%f: (%i) Publishing new block, %s\n", g_pEngine->GetHostTime(), iCurrentSessionBlock,
						  pNewBlock->GetFilename()));
}

void CSessionBlockPublisher::GatherBlockData(uint8 *pSessionBuffer, int nSessionBufferSize,
											 CServerRecordingSessionBlock *pBlock, unsigned char **ppSafeBlockData,
											 int *pBlockSize)
{
...
{
	AssertMsg(m_pSession->IsLocked(),
			  "The session isn't locked, which means blocks can be being deleted and will probably cause a crash.");
	...
	if(m_flLastBlockWriteTime != 0.0f && flHostTime - m_flLastBlockWriteTime >= m_nDumpInterval &&
