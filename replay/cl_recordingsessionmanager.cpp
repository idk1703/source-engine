...
CBaseRecordingSession *CClientRecordingSessionManager::OnSessionStart(int nCurrentRecordingStartTick,
...
void CClientRecordingSessionManager::OnSessionEnd()
{
	if(m_pRecordingSession)
	{
		...
		AssertMsg(!m_pRecordingSession->m_bRecording,
				  "This flag should have been cleared already!  See CBaseRecordingSession::OnStopRecording()");
...
void CClientRecordingSessionManager::FireGameEvent(IGameEvent *pEvent)
{
	...
	if(!V_stricmp("replay_sessioninfo", pEventName))
	{
		...
		if(nDumpInterval < MIN_SERVER_DUMP_INTERVAL || nDumpInterval > MAX_SERVER_DUMP_INTERVAL)
		{
		...
		m_ServerRecordingState.m_nCurrentBlock =
			nCurrentBlock + 1; // This will account for any different between when the server actually dumps a block and
							   // the client predicts a dump
...
void CClientRecordingSessionManager::DownloadThink()
{
	...
	{
		m_flNextPossibleDownloadTime =
			flHostTime +
			MAX(MIN_SERVER_DUMP_INTERVAL, CL_GetRecordingSessionManager()->m_ServerRecordingState.m_nDumpInterval);
	}
}
...
void CClientRecordingSessionManager::OnReplaysLoaded()
{
	...
	FOR_EACH_REPLAY(i)
	{
		...
		CClientRecordingSession *pOwnerSession =
			CL_CastSession(CL_GetRecordingSessionManager()->FindSession(pCurReplay->m_hSession));
		Assert(pOwnerSession);
		if(!pOwnerSession)
		{
