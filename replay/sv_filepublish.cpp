...
ConVar replay_publish_simulate_delay_local_http(
	"replay_publish_simulate_delay_local_http", "0", FCVAR_DONTRECORD,
...
ConVar replay_publish_simulate_rename_fail(
	"replay_publish_simulate_rename_fail", "0", FCVAR_DONTRECORD,
	"Simulate a rename failure during local HTTP publishing, which will force a manual copy & delete.", true, 0.0f,
	true, 1.0f);

//----------------------------------------------------------------------------------------

CBasePublishJob::CBasePublishJob(JobPriority_t nPriority /*=JP_NORMAL*/, ISpewer *pSpewer /*=g_pDefaultSpewer*/)
...
JobStatus_t CLocalPublishJob::DoExecute()
{
	...
	{
		PrintEventResult(false);
		CFmtStr fmtError("Fileserver path '%s' invalid (see replay_local_fileserver_path)", pFileserverPath);
...
CCompressionJob::CCompressionJob(const uint8 *pSrcData, uint32 nSrcSize, CompressorType_t nType, bool *pOutResult,
								 uint32 *pResultSize)
	: m_pSrcData(pSrcData), m_nSrcSize(nSrcSize), m_pCompressionResult(pOutResult), m_pResultSize(pResultSize)
{
...
JobStatus_t CCompressionJob::DoExecute()
{
	...
	const int nMaxCompressedSize =
		ceil(m_nSrcSize * 1.1f) + 600; // see "destLen" - http://www.bzip.org/1.0.3/html/util-fns.html
...
CMd5Job::CMd5Job(const void *pSrcData, int nSrcSize, bool *pOutHashed, uint8 *pOutHash, unsigned int pSeed[4] /*=NULL*/)
	: m_pSrcData(pSrcData), m_nSrcSize(nSrcSize), m_pHashed(pOutHashed), m_pHash(pOutHash), m_pSeed(pSeed)
{
	*m_pHashed = false;
...
public:
	...
	virtual void Think()
	{
		...
		{
			g_pBlockSpewer->PrintMsg(
				Replay_va("ERROR: Publish timed out after %i seconds.", replay_fileserver_offload_aborttime.GetInt()));
			Abort();
			return;
		}
	...
	CJob *GetJobForPhase(Phase_t nPhase)
	{
		...
		switch(nPhase)
		{
			case PHASE_COMPRESSION:
				pResult =
					new CCompressionJob(m_pInData, m_nInSize, m_nCompressorType, &m_bCompressedOk, &m_nCompressedSize);
				break;
			...
			case PHASE_ADJUSTHEADER:
			{
				...
				if(m_pHeaderData && m_nHeaderSize)
				{
					...
					g_pFullFileSystem->AsyncWrite(m_szOutFilename, m_pHeaderData, m_nHeaderSize, bFreeMemory, false,
												  &hFileJob);
		...
		Assert(GetCurrentPhaseInputType() == IO_DONTCARE || m_nInType == IO_DONTCARE ||
