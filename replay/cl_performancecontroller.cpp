...
{
public:
	CSaveJob(KeyValues *pInData, const char *pFullFilename) : m_pInData(pInData)
	{
...
{
	AssertMsg(!m_pScratchPerformance || m_pScratchPerformance != m_pSavedPerformance,
...
void CPerformanceController::StartRecording(CReplay *pReplay, bool bSnip)
{
	...
	AssertMsg(m_nState == STATE_PLAYING || !m_pRoot, "Unless we're playing, root should be NULL here");
...
bool CPerformanceController::SaveAsync()
{
	...
	// Copy title
	V_wcsncpy(m_pSavedPerformance->m_wszTitle, m_pScratchPerformance->m_wszTitle,
			  sizeof(m_pSavedPerformance->m_wszTitle));
...
bool CPerformanceController::SaveAsAsync(const wchar_t *pTitle)
{
	...
	if(!pExistingPerformance)
	{
		...
		if(!DumpStreamToFileAsync(pCopy->GetFullPerformanceFilename()) || !FlushReplay())
		{
			return false;
		}
...
#define CREATE_EVENT(time_, type_) new KeyValues(Replay_va("%f", time_), "type", type_)

#define RECORD_EVENT_(event_, time_, type_) \
	event_ = CREATE_EVENT(time_, type_);    \
	AddEvent(event_)

#define RECORD_EVENT(event_, time_, type_) KeyValues *event_ = RECORD_EVENT_(event_, time_, type_)
...
{
	IF_REPLAY_DBG2(CUtlBuffer buf; pEvent->RecursiveSaveToFile(buf, 1);
...
void CPerformanceController::FinishBeginPerformancePlayback()
{
	...
	IF_REPLAY_DBG(m_pDbgRoot = m_pRoot->MakeCopy(););
...
{
	AssertMsg(!m_pSavedPerformance, "This probably hit because either SaveNow() or Discard() were not called.  One of "
									"those should always be called on disconnect after watching or editing a replay.");
...
void CPerformanceController::PlaybackThink()
{
	...
	while(1)
	{
		...
		IF_REPLAY_DBG2(CUtlBuffer buf; m_pCurEvent->RecursiveSaveToFile(buf, 1);
					   Warning("%s\n", (const char *)buf.Base()););
		...
		switch(m_pCurEvent->GetInt("type", EVENTTYPE_INVALID))
		{
			...
			case EVENTTYPE_CAMERA_SETVIEW:
				...
				if(m_bViewOverrideMode)
				{
					...
					bShouldCut =
						bShouldCut ||
						m_pCurEvent->GetBool("cut"); // We cut if any set-view event cuts, otherwise we will interpolate
				}
				break;
				...
				if(pSearch->GetInt("type", EVENTTYPE_INVALID) == EVENTTYPE_CAMERA_SETVIEW)
				{
					...
					if(flDiff > 0.0f)
					{
						...
						if(pSearch->GetBool("cut"))
						{
							const int iSrc =
								clamp((int)(.5f + t), 0,
									  1); // Round t to 0 or 1, so we set the camera to the current frame if t < 0.5f,
										  // and we set the camera to the 'cut'/next frame if t >= 0.5.
						...
						{
							ReadSetViewEvent(pSearch, aOrigin[1], aAngles[1], aFov[1], &flAccel, &flSpeed,
											 &flRotFilter);
						}
					}
					break;
