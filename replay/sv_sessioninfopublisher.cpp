...
void CSessionInfoPublisher::RefreshSessionInfoBlockData(CUtlBuffer &buf)
{
	...
	const int itStart = m_itLastCompletedBlockWrittenToBuffer == vecBlocks.InvalidIndex()
							? 0
							: m_itLastCompletedBlockWrittenToBuffer + 1;
...
void CSessionInfoPublisher::Think()
{
	...
	Assert(m_pSession->m_strName.Length() <
		   MAX_SESSIONNAME_LENGTH); // The only way this name is going to get very long is if
	...
	params.m_pOutFilename = fmtTmpSessionInfoFile.Access(), params.m_pSrcData = (uint8 *)m_bufSessionInfo.Base();
