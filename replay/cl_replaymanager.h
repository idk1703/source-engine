...
//
class CReplayManager : public CGenericPersistentManager<CReplay>, public IReplayManager
{
...
public:
	...
	virtual int GetReplayCount() const
	{
		return Count();
	}
	...
	CReplay
		*m_pReplayThisLife; // Valid only between replay completion (ie player death) and player respawn, otherwise NULL
...
private:
	...
	{
		return "replays." GENERIC_FILE_EXTENSION;
	}
	...
	CReplay *m_pReplayLastLife; // The previous life (ie between the player's previous spawn and the current spawn), if
								// any (otherwise NULL)
