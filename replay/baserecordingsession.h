...
class CBaseRecordingSession : public CBaseReplaySerializeable, public IRecordingSession
{
...
public:
	...
	const BlockContainer_t &GetBlocks() const
	{
		return m_vecBlocks;
	}
	...
	int m_nServerStartRecordTick;	 // The tick at which the server began recording the given session (based on
								  // g_ServerGlobalVariables.tickcount) - which is the tick client spawn/death ticks are
								  // relative to
	float m_flStartTime;

	// Non-persistent:
	IReplayContext *m_pContext;
	bool m_bAutoDelete; // Set to true if a session is removed while it's recording - if flagged, session will
						// auto-delete once recording ends
