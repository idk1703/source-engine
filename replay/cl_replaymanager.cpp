...
bool CReplayManager::Init(CreateInterfaceFn fnCreateFactory)
{
	...
	m_pReplayFactory = GetReplayFactory(fnCreateFactory);
	Assert(m_pReplayFactory);
...
void CReplayManager::OnSessionStart()
{
	...
	if(m_pPendingReplay)
	{
		...
		{
			ReplayHandle_t hSessionInProgress =
				CL_GetRecordingSessionManager()->GetRecordingSessionInProgress()->GetHandle();
...
void CReplayManager::CommitPendingReplayAndBeginDownload()
{
	...
	CClientRecordingSession *pSession =
		CL_CastSession(CL_GetRecordingSessionManager()->FindSession(m_pReplayThisLife->m_hSession));
...
void CReplayManager::CompletePendingReplay()
{
	...
	// Calc replay length
	m_pPendingReplay->m_flLength =
		g_pEngine->TicksToTime(m_pPendingReplay->m_nDeathTick - m_pPendingReplay->m_nSpawnTick +
	...
	// Before we copy the pointer to "this life," end recording so the replay can do any cleanup (eg listening for game
	// events)
...
int CReplayManager::GetUnrenderedReplayCount()
{
	...
	FOR_EACH_VEC(m_vecObjs, i)
	{
		...
		if(!pCurReplay->m_bRendered && pCurReplay->m_nStatus == CReplay::REPLAYSTATUS_READYTOCONVERT)
		{
			++nCount;
		}
	}
	...
	if(replay_debug.GetBool())
	{
		...
		for(int i = 0; i < 3; ++i)
		{
			...
			g_pEngineClient->Con_NPrintf(
				i, "%s:  handle=%i  [%i, %i]  C? %s R? %s  MaxBlock: %i", pReplayNames[i], pCurReplay->GetHandle(),
				pCurReplay->m_nSpawnTick, pCurReplay->m_nDeathTick, pCurReplay->m_bComplete ? "YES" : "NO",
				pCurReplay->m_bRequestedByUser ? "YES" : "NO", pCurReplay->m_iMaxSessionBlockRequired);

			// Screenshot handle
			int nCurLine = 5;
			g_pEngineClient->Con_NPrintf(nCurLine, "Screenshot replay: handle=%i",
										 CL_GetScreenshotManager()->GetScreenshotReplay());
			nCurLine += 2;
...
void CReplayManager::GetReplaysAsQueryableItems(CUtlLinkedList<IQueryableReplayItem *, int> &lstReplays)
{
	...
	if(m_pPendingReplay && !m_pPendingReplay->m_bComplete && m_pPendingReplay->m_bRequestedByUser)
	{
...
float CReplayManager::GetDownloadProgress(const CReplay *pReplay)
{
	...
	FOR_EACH_VEC(vecBlocks, i)
	{
		...
		const float flSubProgress =
			pCurBlock->m_uFileSize == 0
				? 0.0f
				: clamp((float)pCurBlock->m_uBytesDownloaded / pCurBlock->m_uFileSize, 0.0f, 1.0f);
