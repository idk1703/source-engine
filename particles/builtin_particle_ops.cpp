...
void C_OP_BasicMovement::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	fltx4 adj_dt = ReplicateX4((pParticles->m_flDt / pParticles->m_flPreviousDt) *
							   ExponentialDecay((1.0f - max(0.f, m_fDrag)), (1.0f / 30.0f), pParticles->m_flDt));
		...
		for(int i = 0; i < nAccumulators; i++)
		{
			...
			if(pParticles->CheckIfOperatorShouldRun(pOp, &flStrengthOp))
			{
				...
				pParticles->m_pDef->m_ForceGenerators[i]->AddForces(
					PerParticleForceAccumulator, pParticles, nblocks, flStrengthOp,
					pParticles->m_pOperatorContextData + pParticles->m_pDef->m_nForceGeneratorsCtxOffsets[i]);
END_OP
;
}
}
...
do
{
	...
	prev_xyz->x = AddSIMD(xyz->x, AddSIMD(accFactorX, MulSIMD(adj_dt, SubSIMD(xyz->x, prev_xyz->x))));
	prev_xyz->y = AddSIMD(xyz->y, AddSIMD(accFactorY, MulSIMD(adj_dt, SubSIMD(xyz->y, prev_xyz->y))));
	prev_xyz->z = AddSIMD(xyz->z, AddSIMD(accFactorZ, MulSIMD(adj_dt, SubSIMD(xyz->z, prev_xyz->z))));
	CHECKSYSTEM(pParticles);
	++prev_xyz;
	++xyz;
	pAccIn += nForceStride;
...
if(nConstraints && pParticles->m_nPaddedActiveParticles)
{
	...
	for(int i = 0; i < nConstraints; i++)
	{
		...
		pParticles->m_pDef->m_Constraints[i]->SetupConstraintPerFrameData(
			pParticles, pParticles->m_pOperatorContextData + pParticles->m_pDef->m_nConstraintsCtxOffsets[i]);
	}
	...
	for(int p = 0; p < m_nMaxConstraintPasses; p++)
	{
		...
		for(int i = 0; i < nConstraints; i++)
		{
			...
			if(!bConstraintSatisfied[i])
			{
				...
				{
					START_OP;
					bool bDidSomething = pOp->EnforceConstraint(0, pParticles->m_nPaddedActiveParticles, pParticles,
...
for(int i = 0; i < nConstraints; i++)
{
	...
	{
		START_OP;
		pOp->EnforceConstraint(0, pParticles->m_nPaddedActiveParticles, pParticles,
							   pParticles->m_pOperatorContextData + pParticles->m_pDef->m_nConstraintsCtxOffsets[i],
							   pParticles->m_nActiveParticles);
END_OP
;
CHECKSYSTEM(pParticles);
...
void C_OP_FadeAndKill::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	for(int i = 0; i < nLimit; i += 4)
	{
		...
		fltx4 fl4FadingInMask = AndNotSIMD(
			fl4KillMask, AndSIMD(CmpLeSIMD(fl4StartFadeInTime, fl4Age), CmpGtSIMD(fl4EndFadeInTime, fl4Age)));
		fltx4 fl4FadingOutMask = AndNotSIMD(
			fl4KillMask, AndSIMD(CmpLeSIMD(fl4StartFadeOutTime, fl4Age), CmpGtSIMD(fl4EndFadeOutTime, fl4Age)));
		if(IsAnyNegative(fl4FadingInMask))
		{
			fltx4 fl4Goal = MulSIMD(*pInitialAlpha, fl4StartAlpha);
			fltx4 fl4NewAlpha =
				SimpleSplineRemapValWithDeltasClamped(fl4Age, fl4StartFadeInTime, fl4FadeInDuration,
													  fl4OOFadeInDuration, fl4Goal, SubSIMD(*pInitialAlpha, fl4Goal));
		...
		if(IsAnyNegative(fl4FadingOutMask))
		{
			...
			fltx4 fl4NewAlpha = SimpleSplineRemapValWithDeltasClamped(fl4Age, fl4StartFadeOutTime, fl4FadeOutDuration,
																	  fl4OOFadeOutDuration, *pInitialAlpha,
																	  SubSIMD(fl4Goal, *pInitialAlpha));
...
void C_OP_FadeIn::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	{
		fltx4 FadeInTime =
			Pow_FixedPoint_Exponent_SIMD(pParticles->RandomFloat(*pParticleID, nRandomOffset), nSSEFixedExponent);
		...
		{
			flLifeTime =
				MaxSIMD(Four_Zeros, MinSIMD(Four_Ones, MulSIMD(flLifeTime, ReciprocalEstSIMD(*pLifeDuration))));
		}
		...
		if(IsAnyNegative(ApplyMask))
		{
			...
			fltx4 NewAlpha = SimpleSplineRemapValWithDeltasClamped(
				flLifeTime, Four_Zeros, FadeInTime, ReciprocalEstSIMD(FadeInTime), Four_Zeros, *pInitialAlpha);
...
void C_OP_FadeOut::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	do
	{
		...
		if(IsAnyNegative(ApplyMask))
		{
			...
			if(m_bEaseInAndOut)
			{
				NewAlpha = SimpleSplineRemapValWithDeltasClamped(fl4LifeTime, fl4FadeOutTime, fl4Lifespan,
...
void C_OP_OscillateScalar::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	do
	{
		...
		if(IsAnyNegative(fl4GoodMask))
		{
			...
			if(m_nField == 7)
			{
				*pOscField = MaskedAssign(fl4GoodMask, MaxSIMD(MinSIMD(fl4OscVal, Four_Ones), Four_Zeros), *pOscField);
			}
			else
			{
...
void C_OP_OscillateVector::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	do
	{
		...
		if(IsAnyNegative(fl4GoodMask))
		{
			...
			{
				pOscField->x =
					MaskedAssign(fl4GoodMask, MaxSIMD(MinSIMD(fvOscVal.x, Four_Ones), Four_Zeros), fvOutput.x);
				pOscField->y =
					MaskedAssign(fl4GoodMask, MaxSIMD(MinSIMD(fvOscVal.y, Four_Ones), Four_Zeros), fvOutput.y);
				pOscField->z =
					MaskedAssign(fl4GoodMask, MaxSIMD(MinSIMD(fvOscVal.z, Four_Ones), Four_Zeros), fvOutput.z);
			}
			else
			{
...
void C_OP_Cull::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	for(int i = 0; i < pParticles->m_nActiveParticles; ++i)
	{
		...
		float flCullTime =
			pParticles->RandomFloatExp(nParticleId + nRandomOffset + 12, m_flCullStart, m_flCullEnd, m_flCullExp);
...
void CGeneralSpin::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	for(int i = 0; i < nActive; i += 4)
	{
		// HACK: Rather than redo this, I'm simply remapping the stop time into the percentage of lifetime, rather than
		// seconds
		...
		fltx4 RScale = MaxSIMD(Four_Zeros, SubSIMD(Four_Ones, MulSIMD(Age, OOSpinFadeRate)));
		...
		NewRot = OrSIMD(AndSIMD(Toobig, SubSIMD(NewRot, Pi_2)), AndNotSIMD(Toobig, NewRot));

		NewRot = OrSIMD(AndSIMD(Toosmall, AddSIMD(NewRot, Pi_2)), AndNotSIMD(Toosmall, NewRot));
...
protected:
	...
	uint32 GetReadAttributes(void) const
	{
		return (1 << GetAttributeToSpin()) | (1 << GetSpinSpeedAttribute()) | PARTICLE_ATTRIBUTE_CREATION_TIME_MASK;
	}
...
void C_OP_InterpolateRadius::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	if(m_bEaseInAndOut)
	{
		do
		{
			fltx4 fl4LifeDuration = *pLifeDuration;
			fltx4 fl4GoodMask = CmpGtSIMD(fl4LifeDuration, Four_Zeros);
			fltx4 fl4LifeTime = MulSIMD(SubSIMD(fl4CurTime, *pCreationTime),
										ReciprocalEstSIMD(fl4LifeDuration)); // maybe need accurate div here?
			fl4GoodMask = AndSIMD(fl4GoodMask, CmpGeSIMD(fl4LifeTime, fl4StartTime));
			fl4GoodMask = AndSIMD(fl4GoodMask, CmpLtSIMD(fl4LifeTime, fl4EndTime));
			if(IsAnyNegative(fl4GoodMask))
...
#endif
					fl4FadeWindow =
						AddSIMD(fl4StartScale, MulSIMD(BiasSIMD(fl4FadeWindow, m_fl4BiasParam), fl4ScaleWidth));
					*pRadius = MaskedAssign(fl4GoodMask, MulSIMD(*pInitialRadius, fl4FadeWindow), *pRadius);
				}
				++pCreationTime;
				++pLifeDuration;
		...
		do
		{
			...
			if(IsAnyNegative(goodMask))
			{
				...
				pColor->x = MaskedAssign(
					goodMask, AddSIMD(pInitialColor->x, MulSIMD(T, SubSIMD(targetR, pInitialColor->x))), pColor->x);
				pColor->y = MaskedAssign(
					goodMask, AddSIMD(pInitialColor->y, MulSIMD(T, SubSIMD(targetG, pInitialColor->y))), pColor->y);
				pColor->z = MaskedAssign(
					goodMask, AddSIMD(pInitialColor->z, MulSIMD(T, SubSIMD(targetB, pInitialColor->z))), pColor->z);
			}
			++pColor;
			++pCreationTime;
			++pLifeDuration;
...
class C_OP_PositionLock : public CParticleOperatorInstance
{
	...
	virtual void InitializeContextData(CParticleCollection *pParticles, void *pContext) const
	{
		...
		pParticles->GetControlPointTransformAtTime(m_nControlPointNumber, pParticles->m_flCurTime,
...
void C_OP_PositionLock::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	for(int i = 0; i < pParticles->m_nActiveParticles; ++i)
	{
		...
		pCreationTime = pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_CREATION_TIME, i);
		pLifeDuration = pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_LIFE_DURATION, i);
		float flLifeTime = *pLifeDuration != 0.0f
							   ? clamp((pParticles->m_flCurTime - *pCreationTime) / (*pLifeDuration), 0.0f, 1.0f)
							   : 0.0f;
		...
		int nParticleId = *pParticles->GetIntAttributePtr(PARTICLE_ATTRIBUTE_PARTICLE_ID, i);
		float flStartTime = pParticles->RandomFloatExp(nParticleId + nRandomOffset + 9, m_flStartTime_min,
													   m_flStartTime_max, m_flStartTime_exp);
		float flEndTime = pParticles->RandomFloatExp(nParticleId + nRandomOffset + 10, m_flEndTime_min, m_flEndTime_max,
													 m_flEndTime_exp);
...
void C_OP_PositionLock::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	else
	{
		...
		do
		{
			...
			fl4LifeTime =
				MaxSIMD(Four_Zeros, MinSIMD(Four_Ones, MulSIMD(fl4LifeTime, ReciprocalEstSIMD(*pLifeDuration))));
...
class C_OP_ControlpointLight : public CParticleOperatorInstance
{
	...
	float m_LightFiftyDist1, m_LightZeroDist1, m_LightFiftyDist2, m_LightZeroDist2, m_LightFiftyDist3, m_LightZeroDist3,
		m_LightFiftyDist4, m_LightZeroDist4;
	Color m_LightColor1, m_LightColor2, m_LightColor3, m_LightColor4;
	bool m_bLightType1, m_bLightType2, m_bLightType3, m_bLightType4, m_bLightDynamic1, m_bLightDynamic2,
		m_bLightDynamic3, m_bLightDynamic4, m_bUseNormal, m_bUseHLambert, m_bLightActive1, m_bLightActive2,
	...
	virtual uint64 GetReadControlPointMask() const
	{
		return (1ULL << m_nControlPoint1) | (1ULL << m_nControlPoint2) | (1ULL << m_nControlPoint3) |
			   (1ULL << m_nControlPoint4);
	}
...
DEFINE_PARTICLE_OPERATOR(C_OP_SetChildControlPoints, "Set child control points from particle positions",
						 OPERATOR_GENERIC);
...
DEFINE_PARTICLE_OPERATOR(C_OP_DistanceBetweenCPs, "Remap Distance Between Two Control Points to Scalar",
						 OPERATOR_GENERIC);
...
class C_OP_PerParticleEmitter : public CParticleOperatorInstance
{
	...
	virtual bool MayCreateMoreParticles(CParticleCollection *pParticles, void *pContext) const
	{
		...
		if(m_flEmissionDuration != 0.0f &&
		   (pParticles->m_flCurTime - pParticles->m_flDt) > (m_flStartTime + m_flEmissionDuration))
			return false;
	...
	for(CParticleCollection *pChild = pParticles->m_Children.m_pHead; pChild; pChild = pChild->m_pNext)
	{
		if(pChild->GetGroupID() == m_nChildGroupID)
		{
			for(int i = 0; i < pParticles->m_nActiveParticles; ++i)
			{
				C_OP_PerParticleEmitterContext_t *pCtx = reinterpret_cast<C_OP_PerParticleEmitterContext_t *>(pContext);
				const float *pXYZ = pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_XYZ, i);
				const float *pXYZ_Prev = pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_PREV_XYZ, i);
				Vector vecParticlePosition, vecParticlePosition_prev, vParticleDelta;
				...
				// Calculate emission rate by delta time from last frame to determine number of particles to emit this
				// frame as a fractional float
				...
				// Floor float accumulated value and subtract whole int emitted so far from the result to determine
				// total whole particles to emit this frame
				...
				if(nParticlesEmitted != pChild->m_nActiveParticles)
				{
					...
					uint32 nInittedMask = (PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_PREV_XYZ_MASK |
										   PARTICLE_ATTRIBUTE_CREATION_TIME_MASK);
					// init newly emitted particles
					pChild->InitializeNewParticles(nParticlesEmitted, pChild->m_nActiveParticles - nParticlesEmitted,
												   nInittedMask);
					// CHECKSYSTEM( this );
				}
...
void C_OP_LockToBone::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	if(pParticles->m_ControlPointHitBoxes[m_nControlPointNumber].CurAndPrevValid())
	{
		...
		for(int i = 0; i < pParticles->m_nActiveParticles; ++i)
		{
			...
			{
				if((nBoxIndex < pParticles->m_ControlPointHitBoxes[m_nControlPointNumber].m_nNumHitBoxes) &&
				   ...
				   (nBoxIndex >= 0))
				{
					...
					ModelHitBoxInfo_t const &hb =
						pParticles->m_ControlPointHitBoxes[m_nControlPointNumber].m_pHitBoxes[nBoxIndex];
					...
					ModelHitBoxInfo_t phb =
						pParticles->m_ControlPointHitBoxes[m_nControlPointNumber].m_pPrevBoxes[nBoxIndex];
...
void C_OP_ModelCull::Operate(CParticleCollection *pParticles, float flStrength, void *pContext) const
{
	...
	if(pParticles->m_ControlPointHitBoxes[m_nControlPointNumber].CurAndPrevValid())
	{
		for(int i = 0; i < pParticles->m_nActiveParticles; ++i)
		{
			...
			bool bInside = g_pParticleSystemMgr->Query()->IsPointInControllingObjectHitBox(
				pParticles, m_nControlPointNumber, vecParticlePosition, m_bBoundBox);
...
DEFINE_PARTICLE_OPERATOR(C_OP_MovementRotateParticleAroundAxis, "Movement Rotate Particle Around Axis",
						 OPERATOR_GENERIC);
...
void C_OP_MovementRotateParticleAroundAxis::Operate(CParticleCollection *pParticles, float flStrength,
													void *pContext) const
{
