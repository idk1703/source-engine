...
class C_OP_ConstrainDistance : public CParticleOperatorInstance
{
	...
	bool EnforceConstraint(int nStartBlock, int nEndBlock, CParticleCollection *pParticles, void *pContext,
...
bool C_OP_ConstrainDistance::EnforceConstraint(int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
											   void *pContext, int nNumValidParticlesInLastChunk) const
{
	size_t nStride;
	FourVectors *pXYZ = pParticles->Get4VAttributePtrForWrite(PARTICLE_ATTRIBUTE_XYZ, &nStride);
...
bool C_OP_ConstrainDistanceToPath::EnforceConstraint(int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
													 void *pContext, int nNumValidParticlesInLastChunk) const
{
	...
	do
	{
		fltx4 TScale = MinSIMD(Four_Ones, MulSIMD(TimeScale, SubSIMD(CurTime, *pCreationTime)));
...
DEFINE_PARTICLE_OPERATOR(C_OP_ConstrainDistanceToPath, "Constrain distance to path between two control points",
						 OPERATOR_GENERIC);
...
BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_ConstrainDistanceToPath)
	...
	DMXELEMENT_UNPACK_FIELD("bulge control 0=random 1=orientation of start pnt 2=orientation of end point", "0", int,
							m_PathParameters.m_nBulgeControl)
...
bool C_OP_PlanarConstraint::EnforceConstraint(int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
											  void *pContext, int nNumValidParticlesInLastChunk) const
{
	C4VAttributeWriteIterator pXYZ(PARTICLE_ATTRIBUTE_XYZ, pParticles);
	pXYZ += nStartBlock;
	...
	PlaneNormal.DuplicateVector(pParticles->TransformAxis(m_PlaneNormal, !m_bGlobalNormal, m_nControlPointNumber));
...
void CWorldCollideContextData::SetBaseTrace(int nIndex, Vector const &rayStart, Vector const &traceDir,
											int nCollisionGroup, bool bKeepMisses)
{
	CBaseTrace tr;
...
void CWorldCollideContextData::CalculatePlanes(CParticleCollection *pParticles, int nCollisionMode, int nCollisionGroup,
											   Vector const *pCPOffset, float flDistanceTolerance)
{
...
void C_OP_WorldCollideConstraint::SetupConstraintPerFrameData(CParticleCollection *pParticles, void *pContext) const
{
...
bool C_OP_WorldCollideConstraint::EnforceConstraint(int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
													void *pContext, int nNumValidParticlesInLastChunk) const
{
	C4VAttributeWriteIterator pXYZ(PARTICLE_ATTRIBUTE_XYZ, pParticles);
	pXYZ += nStartBlock;
...
class C_OP_WorldTraceConstraint : public CParticleOperatorInstance
{
	...
	bool EnforceConstraint(int nStartBlock, int nEndBlock, CParticleCollection *pParticles, void *pContext,
						   int nNumValidParticlesInLastChunk) const;
	template<bool bKillOnContact, bool bCached>
	bool EnforceConstraintInternal(int nStartBlock, int nEndBlock, CParticleCollection *pParticles, void *pContext,
								   int nNumValidParticlesInLastChunk) const;
};
...
static void WorldIntersectTNew(FourVectors const *pStartPnt, FourVectors const *pEndPnt, int nCollisionGroup, int nMask,
							   ISectData_t *pISectData, int nCollisionMode, CWorldCollideContextData *pCtx,
							   fltx4 const &fl4ParticleValidMask, float flTolerance = 0.0)
{
	...
	if(pCtx)
	{
		...
		if(nCollisionMode == COLLISION_MODE_USE_NEAREST_TRACE)
		{
			...
			for(int i = 0; i < pCtx->m_nActivePlanes; i++)
			{
				if(pCtx->m_bPlaneActive[i])
				{
					fltx4 fl4TrialDistance =
						MaxSIMD(pStartPnt->DistSqrToLineSegment(pCtx->m_TraceStartPnt[i], pCtx->m_TraceEndPnt[i]),
			...
			fltx4 fl4OutOfRange = AndSIMD(fl4ParticleValidMask, CmpGtSIMD(fl4ClosestDist, ReplicateX4(flTolerance)));
			if(IsAnyNegative(fl4OutOfRange))
			{
				...
				for(int i = 0; i < 4; i++)
				{
					if(nMask & (1 << i))
					{
						...
						CBaseTrace tr;
						g_pParticleSystemMgr->Query()->TraceLine(start, end, nMask, NULL, nCollisionGroup, &tr);
			...
			if(IsAnyNegative(fl4CrossMask))
			{
				...
				if(IsAnyNegative(fl4CrossMask))
				{
					...
					pISectData->m_ISectNormal.x =
						MaskedAssign(fl4CrossMask, v4PlaneNormal.x, pISectData->m_ISectNormal.x);
					pISectData->m_ISectNormal.y =
						MaskedAssign(fl4CrossMask, v4PlaneNormal.y, pISectData->m_ISectNormal.y);
					pISectData->m_ISectNormal.z =
						MaskedAssign(fl4CrossMask, v4PlaneNormal.z, pISectData->m_ISectNormal.z);
				}
			}
		}
...
static void WorldIntersectT(FourVectors const *pStartPnt, FourVectors const *pEndPnt, int nCollisionGroup, int nMask,
							ISectData_t *pISectData, CWorldCollideContextData *pCtx)
{
	pISectData->m_ISectT = Four_Zeros;
	pISectData->m_LeftOverT = Four_Zeros;
	pISectData->m_ISectNormal.x = Four_Zeros;
			...
			if(pCtx->m_bPlaneActive[i])
			{
				...
				if(IsAnyNegative(fl4CrossMask))
				{
					...
					if(IsAnyNegative(fl4CrossMask))
					{
						pISectData->m_ISectT = MaskedAssign(fl4CrossMask, fl4T, pISectData->m_ISectT);
						pISectData->m_ISectNormal.x =
							MaskedAssign(fl4CrossMask, pCtx->m_PlaneNormal[i].x, pISectData->m_ISectNormal.x);
						pISectData->m_ISectNormal.y =
							MaskedAssign(fl4CrossMask, pCtx->m_PlaneNormal[i].y, pISectData->m_ISectNormal.y);
						pISectData->m_ISectNormal.z =
							MaskedAssign(fl4CrossMask, pCtx->m_PlaneNormal[i].z, pISectData->m_ISectNormal.z);
					}
				}
			}
		}
...
bool C_OP_WorldTraceConstraint::EnforceConstraint(int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
												  void *pContext, int nNumValidParticlesInLastChunk) const
{
	if(m_nCollisionMode == COLLISION_MODE_USE_NEAREST_TRACE)
	{
		if(m_bKillonContact)
			return EnforceConstraintInternal<true, true>(nStartBlock, nNumBlocks, pParticles, pContext,
														 nNumValidParticlesInLastChunk);
		else
			return EnforceConstraintInternal<false, true>(nStartBlock, nNumBlocks, pParticles, pContext,
														  nNumValidParticlesInLastChunk);
	}
	else
	{
		if(m_bKillonContact)
			return EnforceConstraintInternal<true, false>(nStartBlock, nNumBlocks, pParticles, pContext,
														  nNumValidParticlesInLastChunk);
		else
			return EnforceConstraintInternal<false, false>(nStartBlock, nNumBlocks, pParticles, pContext,
														   nNumValidParticlesInLastChunk);
	}
}

template<bool bKillonContact, bool bCached>
bool C_OP_WorldTraceConstraint::EnforceConstraintInternal(int nStartBlock, int nNumBlocks,
														  ...
														  int nNumValidParticlesInLastChunk) const
{
	   ...
	   (m_nCollisionMode == COLLISION_MODE_INITIAL_TRACE_DOWN))
	{
		...
		{
			pCtx->CalculatePlanes(pParticles, m_nCollisionMode, m_nCollisionGroupNumber, &m_vecCpOffset,
	...
	do
	{
		...
		if(bCached)
		{
			...
			WorldIntersectTNew(pPrevXYZ, &endPnt, m_nCollisionGroupNumber, nMask, &iData, m_nCollisionMode, pCtx,
							   fl4TailMask, flTol);
		}
		else
