...
uint32 C_OP_InstantaneousEmitter::Emit(CParticleCollection *pParticles, float flCurStrength, void *pContext) const
{
...
class C_OP_ContinuousEmitter : public CParticleOperatorInstance
{
	...
	virtual void SkipToTime(float flTime, CParticleCollection *pParticles, void *pContext) const
	{
		...
		// if ( !IsInfinitelyEmitting() )
		//	pCtx->m_flTotalActualParticlesSoFar = min( pCtx->m_ActualParticlesToEmit,
	...
	virtual bool MayCreateMoreParticles(CParticleCollection *pParticles, void *pContext) const
	{
		...
		float flStartTime = m_flStartTime + pCtx->m_flTimeOffset;
		if(m_flEmissionDuration != 0.0f &&
		   (pParticles->m_flCurTime - pParticles->m_flDt) > (flStartTime + m_flEmissionDuration))
			return false;
...
uint32 C_OP_ContinuousEmitter::Emit(CParticleCollection *pParticles, float flCurStrength, void *pContext) const
{
	...
	// Calculate emission rate by delta time from last frame to determine number of particles to emit this frame as a
	// fractional float
	...
	// Floor float accumulated value and subtract whole int emitted so far from the result to determine total whole
...
uint32 C_OP_NoiseEmitter::Emit(CParticleCollection *pParticles, float flCurStrength, void *pContext) const
{
	// Have we emitted all the particles we're going to emit?
	// NOTE: Using C_OP_ContinuousEmitter:: avoids a virtual function call
	NoiseEmitterContext_t *pCtx = reinterpret_cast<NoiseEmitterContext_t *>(pContext);
	...
	Coord = Vector((pParticles->m_flCurTime + Offset), (pParticles->m_flCurTime + Offset),
