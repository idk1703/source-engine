...
void CParticleOperatorInstance::InitScalarAttributeRandomRangeBlock(int attr_num, float fMin, float fMax,
																	CParticleCollection *pParticles, int start_block,
																	int n_blocks) const
{
	size_t attr_stride;
...
void CParticleOperatorInstance::InitScalarAttributeRandomRangeExpBlock(int attr_num, float fMin, float fMax, float fExp,
																	   CParticleCollection *pParticles, int start_block,
																	   int n_blocks) const
{
	size_t attr_stride;
...
void CParticleOperatorInstance::AddScalarAttributeRandomRangeBlock(int nAttributeId, float fMin, float fMax, float fExp,
																   CParticleCollection *pParticles, int nStartBlock,
																   int nBlockCount, bool bRandomlyInvert) const
{
	size_t nAttrStride;
	...
	else
	{
		...
		if(fExp != 1.0f)
		{
			...
			{
				fltx4 fl4RandVal =
					AddSIMD(val0, MulSIMD(Pow_FixedPoint_Exponent_SIMD(RandSIMD(nRandContext), nExp), val_d));
...
void C_INIT_CreateOnModel::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												  int nAttributeWriteMask, void *pContext) const
{
	...
	while(nParticleCount)
	{
		...
		g_pParticleSystemMgr->Query()->GetRandomPointsOnControllingObjectHitBox(
			pParticles, m_nControlPointNumber, nToDo, m_flHitBoxScale, m_nForceInModel, vecPnts, m_vecDirectionBias,
			vecUVW, nHitBoxIndex);
		...
		for(int i = 0; i < nToDo; i++)
		{
			...
			float *pHitboxRelXYZ =
				pParticles->GetFloatAttributePtrForWrite(PARTICLE_ATTRIBUTE_HITBOX_RELATIVE_XYZ, start_p);
...
class C_INIT_CreateWithinSphere : public CParticleOperatorInstance
{
	...
	virtual void InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
									   int nAttributeWriteMask, void *pContext) const;
	...
	void InitParams(CParticleSystemDefinition *pDef, CDmxElement *pElement)
	{
		...
		m_bDistanceBias =
			(m_vecDistanceBias.x != 1.0f) || (m_vecDistanceBias.y != 1.0f) || (m_vecDistanceBias.z != 1.0f);
		m_bDistanceBiasAbs =
...
void C_INIT_CreateWithinSphere::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													   int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
		...
		if(pxyz && (nAttributeWriteMask & PARTICLE_ATTRIBUTE_PREV_XYZ_MASK))
		{
			...
			poffset -= pParticles->RandomFloat(m_LocalCoordinateSystemSpeedMin.x, m_LocalCoordinateSystemSpeedMax.x) *
					   pParticles->m_ControlPoints[nCurrentControlPoint].m_ForwardVector;
			poffset -= pParticles->RandomFloat(m_LocalCoordinateSystemSpeedMin.y, m_LocalCoordinateSystemSpeedMax.y) *
					   pParticles->m_ControlPoints[nCurrentControlPoint].m_RightVector;
			poffset -= pParticles->RandomFloat(m_LocalCoordinateSystemSpeedMin.z, m_LocalCoordinateSystemSpeedMax.z) *
...
void C_INIT_CreateWithinSphere::InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
													  int nAttributeWriteMask, void *pContext) const
{
	   ...
	   r_sse_s.GetInt())
	{
		...
		bool bDoRandSpeed = (m_fSpeedMax > 0.) || (m_LocalCoordinateSystemSpeedMax.x != 0) ||
							(m_LocalCoordinateSystemSpeedMax.y != 0) || (m_LocalCoordinateSystemSpeedMax.z != 0) ||
							(m_LocalCoordinateSystemSpeedMin.x != 0) || (m_LocalCoordinateSystemSpeedMin.y != 0) ||
		...
		fltx4 fl4LocalSpeedXSpread = ReplicateX4(m_LocalCoordinateSystemSpeedMax.x - m_LocalCoordinateSystemSpeedMin.x);
		fltx4 fl4LocalSpeedMinY = ReplicateX4(m_LocalCoordinateSystemSpeedMin.y);
		fltx4 fl4LocalSpeedYSpread = ReplicateX4(m_LocalCoordinateSystemSpeedMax.y - m_LocalCoordinateSystemSpeedMin.y);
		fltx4 fl4LocalSpeedMinZ = ReplicateX4(m_LocalCoordinateSystemSpeedMin.z);
		fltx4 fl4LocalSpeedZSpread = ReplicateX4(m_LocalCoordinateSystemSpeedMax.z - m_LocalCoordinateSystemSpeedMin.z);
			...
			*(pXYZ) = v4RandPos;
			...
			if(nAttributeWriteMask & PARTICLE_ATTRIBUTE_PREV_XYZ_MASK)
			{
				if(bDoRandSpeed)
				{
					...
					v4LocalOffset *= AddSIMD(fl4LocalSpeedMinX, MulSIMD(fl4LocalSpeedXSpread, RandSIMD(nContext)));
					v4randDir += v4LocalOffset;

					v4LocalOffset = v4CPRight;
					v4LocalOffset *= AddSIMD(fl4LocalSpeedMinY, MulSIMD(fl4LocalSpeedYSpread, RandSIMD(nContext)));
					v4randDir += v4LocalOffset;

					v4LocalOffset = v4CPUp;
					v4LocalOffset *= AddSIMD(fl4LocalSpeedMinZ, MulSIMD(fl4LocalSpeedZSpread, RandSIMD(nContext)));
					v4randDir += v4LocalOffset;
		...
		CParticleOperatorInstance::InitNewParticlesBlock(pParticles, start_block, n_blocks, nAttributeWriteMask,
														 pContext);
}
...
void C_INIT_CreateWithinBox::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													int nAttributeWriteMask, void *pContext) const
{
...
void C_INIT_PositionOffset::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												   int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
...
class C_INIT_VelocityRandom : public CParticleOperatorInstance
{
	...
	void InitParams(CParticleSystemDefinition *pDef, CDmxElement *pElement)
	{
		m_nControlPointNumber = max(0, min(MAX_PARTICLE_CONTROL_POINTS - 1, m_nControlPointNumber));
		m_bHasLocalSpeed =
			(m_LocalCoordinateSystemSpeedMin != vec3_origin) || (m_LocalCoordinateSystemSpeedMax != vec3_origin);
...
void C_INIT_VelocityRandom::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												   int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
...
class C_INIT_InitialVelocityNoise : public CParticleOperatorInstance
{
	...
	void InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks, int nAttributeWriteMask,
							   void *pContext) const;
...
void C_INIT_InitialVelocityNoise::InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
														int nAttributeWriteMask, void *pContext) const
{
...
void C_INIT_InitialVelocityNoise::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
...
class C_INIT_RandomLifeTime : public CParticleOperatorInstance
{
							   ...
							   void *pContext) const
	{
		...
		{
			InitScalarAttributeRandomRangeExpBlock(PARTICLE_ATTRIBUTE_LIFE_DURATION, m_fLifetimeMin, m_fLifetimeMax,
												   m_fLifetimeRandExponent, pParticles, start_block, n_blocks);
		}
		else
		{
			InitScalarAttributeRandomRangeBlock(PARTICLE_ATTRIBUTE_LIFE_DURATION, m_fLifetimeMin, m_fLifetimeMax,
...
void C_INIT_RandomLifeTime::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												   int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
...
class C_INIT_RandomRadius : public CParticleOperatorInstance
{
									   ...
									   int nAttributeWriteMask, void *pContext) const
	{
		...
		{
			InitScalarAttributeRandomRangeExpBlock(PARTICLE_ATTRIBUTE_RADIUS, m_flRadiusMin, m_flRadiusMax,
												   m_flRadiusRandExponent, pParticles, start_block, n_blocks);
		}
		else
		{
			InitScalarAttributeRandomRangeBlock(PARTICLE_ATTRIBUTE_RADIUS, m_flRadiusMin, m_flRadiusMax, pParticles,
...
void C_INIT_RandomRadius::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												 int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
...
class C_INIT_RandomAlpha : public CParticleOperatorInstance
{
									   ...
									   int nAttributeWriteMask, void *pContext) const
	{
		...
		{
			InitScalarAttributeRandomRangeExpBlock(PARTICLE_ATTRIBUTE_ALPHA, m_flAlphaMin, m_flAlphaMax,
												   m_flAlphaRandExponent, pParticles, start_block, n_blocks);
		}
		else
		{
			InitScalarAttributeRandomRangeBlock(PARTICLE_ATTRIBUTE_ALPHA, m_flAlphaMin, m_flAlphaMax, pParticles,
												start_block, n_blocks);
		}
	}
...
protected:
									   ...
									   int nAttributeWriteMask, void *pContext) const
	{
		...
		{
			InitScalarAttributeRandomRangeExpBlock(GetAttributeToInit(), m_flRadiansMin, m_flRadiansMax,
												   m_flRotationRandExponent, pParticles, start_block, n_blocks);
		}
		else
		{
			InitScalarAttributeRandomRangeBlock(GetAttributeToInit(), m_flRadiansMin, m_flRadiansMax, pParticles,
												start_block, n_blocks);
		}
	}
...
protected:
	...
	virtual void InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
									   int nAttributeWriteMask, void *pContext) const
	{
		AddScalarAttributeRandomRangeBlock(GetAttributeToInit(), m_flRadiansMin, m_flRadiansMax,
										   m_flRotationRandExponent, pParticles, start_block, n_blocks,
								...
								int nAttributeWriteMask, void *pContext) const
	{
		if(!m_bRandomlyFlipDirection)
		{
			for(; nParticleCount--; start_p++)
			{
				...
				*pAttr +=
					m_flRadians + pParticles->RandomFloatExp(m_flRadiansMin, m_flRadiansMax, m_flRotationRandExponent);
			}
		}
		else
		{
...
class C_INIT_RandomColor : public CParticleOperatorInstance
{
								...
								int nAttributeWriteMask, void *pContext) const
	{
		...
		if(m_flTintPerc)
		{
			...
			else
			{
				...
				if(((pCtx->m_vPrevPosition - vecOrigin).Length() >= m_flUpdateThreshold) ||
				   (pParticles->m_LocalLightingCP == -1))
				{
									   ...
									   int nAttributeWriteMask, void *pContext) const
	{
		...
		fvColorWidth.DuplicateVector(Vector(m_flNormColorMax[0] - m_flNormColorMin[0],
											m_flNormColorMax[1] - m_flNormColorMin[1],
											m_flNormColorMax[2] - m_flNormColorMin[2]));
...
class C_INIT_RandomTrailLength : public CParticleOperatorInstance
{
									   ...
									   int nAttributeWriteMask, void *pContext) const
	{
		...
		{
			InitScalarAttributeRandomRangeExpBlock(PARTICLE_ATTRIBUTE_TRAIL_LENGTH, m_flMinLength, m_flMaxLength,
												   m_flLengthRandExponent, pParticles, start_block, n_blocks);
		}
		else
		{
			InitScalarAttributeRandomRangeBlock(PARTICLE_ATTRIBUTE_TRAIL_LENGTH, m_flMinLength, m_flMaxLength,
												pParticles, start_block, n_blocks);
		}
	}
...
class C_INIT_RandomSequence : public CParticleOperatorInstance
{
	...
	virtual void InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
									   int nAttributeWriteMask, void *pContext) const
	{
		InitScalarAttributeRandomRangeBlock(PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER, m_nSequenceMin, m_nSequenceMax,
											pParticles, start_block, n_blocks);
	}
...
void C_INIT_PositionWarp::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												 int nAttributeWriteMask, void *pContext) const
{
...
void C_INIT_CreationNoise::InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
												 int nAttributeWriteMask, void *pContext) const
{
	float flAbsScale;
	fltx4 fl4AbsVal;
...
void C_INIT_CreationNoise::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												  int nAttributeWriteMask, void *pContext) const
{
	float flAbsScale;
	int nAbsVal;
	...
	CoordWorldTime = Vector((Plat_MSTime() * m_flWorldTimeScale), (Plat_MSTime() * m_flWorldTimeScale),
...
BEGIN_PARTICLE_OPERATOR_UNPACK(C_INIT_CreateAlongPath)
	...
	DMXELEMENT_UNPACK_FIELD("bulge control 0=random 1=orientation of start pnt 2=orientation of end point", "0", int,
...
void C_INIT_CreateAlongPath::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
...
void C_INIT_MoveBetweenPoints::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													  int nAttributeWriteMask, void *pContext) const
{
...
void C_INIT_RemapScalar::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												int nAttributeWriteMask, void *pContext) const
{
...
void C_INIT_InheritVelocity::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
...
void C_INIT_AgeNoise::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
											 int nAttributeWriteMask, void *pContext) const
{
	float flAbsScale;
	int nAbsVal;
...
void C_INIT_SequenceLifeTime::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													 int nAttributeWriteMask, void *pContext) const
{
...
void C_INIT_CreateInHierarchy::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													  int nAttributeWriteMask, void *pContext) const
{
	int nEndCP;
	float flGrowth;
	...
	for(; nParticleCount--; start_p++)
	{
		...
		{
			float flCurrentEndCP =
				RemapValClamped(*ct, 0.0f, m_flGrowthTime, min(m_nDesiredStartPoint + 1, nRealEndPoint), nRealEndPoint);
		...
		//	g_pParticleSystemMgr->Query()->TraceLine( RayStart, ( RayStart + ( TraceDir * flRadius ) ), MASK_SOLID,
		//NULL, COLLISION_GROUP_NONE, &tr ); 	if ( tr.fraction == 1.0 )
		//	{
...
void C_INIT_RemapScalarToVector::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
														int nParticleCount, int nAttributeWriteMask,
														void *pContext) const
{
	const float *pCreationTime;
...
void C_INIT_CreateSequentialPath::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
														 int nParticleCount, int nAttributeWriteMask,
														 void *pContext) const
{
...
void C_INIT_InitialRepulsionVelocity::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
															 int nParticleCount, int nAttributeWriteMask,
															 void *pContext) const
{
	...
	if(m_bPerParticle)
	{
		for(; nParticleCount--; start_p++)
		{
			...
			for(int i = 0; i < 6; i++)
			{
				...
				CBaseTrace tr;
				g_pParticleSystemMgr->Query()->TraceLine(vecCurrentPos, endpos, CONTENTS_SOLID, NULL,
...
void C_INIT_RandomYawFlip::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
												  int nAttributeWriteMask, void *pContext) const
{
	for(; nParticleCount--; start_p++)
	{
...
class C_INIT_RandomSecondSequence : public CParticleOperatorInstance
{
	...
	virtual void InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
									   int nAttributeWriteMask, void *pContext) const
	{
		InitScalarAttributeRandomRangeBlock(PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER1, m_nSequenceMin, m_nSequenceMax,
											pParticles, start_block, n_blocks);
	}
...
void C_INIT_RemapCPtoScalar::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													int nAttributeWriteMask, void *pContext) const
{
	const float *pCreationTime;
	// clamp the result to 0 and 1 if it's alpha
...
void C_INIT_RemapCPtoVector::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													int nAttributeWriteMask, void *pContext) const
{
	Vector vecControlPoint;
	...
	for(; nParticleCount--; start_p++)
	{
		...
		Vector vOutput;
		vOutput.x =
			RemapValClamped(vecControlPoint.x, m_vInputMin.x, m_vInputMax.x, vOutputMinLocal.x, vOutputMaxLocal.x);
		vOutput.y =
			RemapValClamped(vecControlPoint.y, m_vInputMin.y, m_vInputMax.y, vOutputMinLocal.y, vOutputMaxLocal.y);
		vOutput.z =
			RemapValClamped(vecControlPoint.z, m_vInputMin.z, m_vInputMax.z, vOutputMinLocal.z, vOutputMaxLocal.z);
...
void C_INIT_CreateFromParentParticles::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
															  int nParticleCount, int nAttributeWriteMask,
															  void *pContext) const
{
	...
	for(; nParticleCount--; start_p++)
	{
		...
		float *pxyz = pParticles->GetFloatAttributePtrForWrite(PARTICLE_ATTRIBUTE_PREV_XYZ, start_p);
		const float *ct = pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_CREATION_TIME, start_p);
		const float *pParent_xyz =
			pParticles->m_pParent->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_XYZ, pCtx->m_nCurrentParentParticle);
		const float *pParent_pxyz =
			pParticles->m_pParent->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_PREV_XYZ, pCtx->m_nCurrentParentParticle);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_DistanceToCPInit, "Remap Initial Distance to Control Point to Scalar",
						 OPERATOR_GENERIC);
...
void C_INIT_DistanceToCPInit::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p, int nParticleCount,
													 int nAttributeWriteMask, void *pContext) const
{
	// clamp the result to 0 and 1 if it's alpha
	float flMin = m_flOutputMin;
	...
	for(; nParticleCount--; start_p++)
	{
		...
		if(m_bLOS)
		{
			...
			CBaseTrace tr;
			g_pParticleSystemMgr->Query()->TraceLine(vecControlPoint1, vecEndPoint, MASK_OPAQUE_AND_NPCS, NULL,
													 m_nCollisionGroupNumber, &tr);
			if(tr.fraction != 1.0f)
			{
...
void C_INIT_LifespanFromVelocity::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
														 int nParticleCount, int nAttributeWriteMask,
														 void *pContext) const
{
	...
	while(nParticleCount--)
	{
		...
		for(int i = 0; i < pCtx->m_nActivePlanes; i++)
		{
			...
			{
				fltx4 fl4TrialDistance =
					MaxSIMD(fvStartPnt.DistSqrToLineSegment(pCtx->m_TraceStartPnt[i], pCtx->m_TraceEndPnt[i]),
		...
		if(IsAllGreaterThan(fl4ClosestDist, ReplicateX4(flTol)))
		{
			...
			g_pParticleSystemMgr->Query()->TraceLine(vecStartPnt, vecEndPnt, CONTENTS_SOLID, NULL,
													 m_nCollisionGroupNumber, &tr);
...
void C_INIT_LifespanFromVelocity::InitNewParticlesBlock(CParticleCollection *pParticles, int start_block, int n_blocks,
														int nAttributeWriteMask, void *pContext) const
{
	CWorldCollideContextData **ppCtx;
	if(pParticles->m_pParent)
		...
		if(IsAnyNegative(fl4OutOfRange))
		{
			...
			for(int i = 0; i < 4; i++)
			{
				if(nMask & (1 << i))
				{
					...
					CBaseTrace tr;
					tr.plane.normal = vec3_invalid;
					g_pParticleSystemMgr->Query()->TraceLine(start, end, CONTENTS_SOLID, NULL, m_nCollisionGroupNumber,
															 &tr);

					// Set the lifespan to 0 if we start solid, our trace distance is 0, or we hit within the offset
					// area
					if((tr.fraction < (1 - flOffsetPct)) || tr.startsolid || flTotalTraceDist == 0.0f)
					{
...
void C_INIT_CreateFromPlaneCache::InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
														 int nParticleCount, int nAttributeWriteMask,
														 void *pContext) const
{
	if(!pParticles->m_pParent)
