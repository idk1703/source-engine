...
// These are some limits that control g_pParticleSystemMgr->ParticleThrottleScaling() and
// g_pParticleSystemMgr->ParticleThrottleRandomEnable()
...
BEGIN_DMXELEMENT_UNPACK(CParticleSystemDefinition)
	...
	DMXELEMENT_UNPACK_FIELD("control point to disable rendering if it is the camera", "-1", int,
...
//-----------------------------------------------------------------------------
void CParticleSystemDefinition::ParseOperators(const char *pszOpKey, ParticleFunctionType_t nFunctionType,
...
bool CParticleSystemDefinition::UsesFullFrameBufferTexture()
{
	// NOTE: This has to be this way to ensure we don't load every freaking material @ startup
	Assert(IsPrecached());
	return m_Material->NeedsFullFrameBufferTexture(
		false); // The false checks if it will ever need the frame buffer, not just this frame
}
...
//-----------------------------------------------------------------------------
void CParticleSystemDefinition::WriteOperators(CDmxElement *pElement, const char *pOpKeyName,
...
void CParticleSystemDefinition::SetupContextData(void)
{
	...
	CUtlVector<CParticleOperatorInstance *> *olists[] = {&m_Operators, &m_Renderers,	   &m_Initializers,
														 &m_Emitters,  &m_ForceGenerators, &m_Constraints};
	CUtlVector<size_t> *offsetLists[] = {
		&m_nOperatorsCtxOffsets, &m_nRenderersCtxOffsets,		&m_nInitializersCtxOffsets,
		&m_nEmittersCtxOffsets,	 &m_nForceGeneratorsCtxOffsets, &m_nConstraintsCtxOffsets,
	};
...
// Finds an operator by id
//-----------------------------------------------------------------------------
CParticleOperatorInstance *CParticleSystemDefinition::FindOperatorById(ParticleFunctionType_t type,
...
//
//-----------------------------------------------------------------------------
void CParticleOperatorInstance::InitNewParticles(CParticleCollection *pParticles, int nFirstParticle,
...
void CParticleCollection::Init(CParticleSystemDefinition *pDef, float flDelay, int nRandomSeed)
{
	...
	SetAttributeToConstant(PARTICLE_ATTRIBUTE_TINT_RGB, pDef->m_ConstantColor.r() / 255.0f,
	...
	{
		m_pOperatorContextData = reinterpret_cast<uint8 *>(MemAlloc_AllocAligned(m_pDef->m_nContextDataSize, 16));
	}
	...
	CUtlVector<CParticleOperatorInstance *> *olists[] = {
		...
		&(m_pDef->m_Emitters),	&(m_pDef->m_ForceGenerators), &(m_pDef->m_Constraints),
	};
	CUtlVector<size_t> *offsetlists[] = {
		&(m_pDef->m_nOperatorsCtxOffsets),		 &(m_pDef->m_nRenderersCtxOffsets),
		&(m_pDef->m_nInitializersCtxOffsets),	 &(m_pDef->m_nEmittersCtxOffsets),
		&(m_pDef->m_nForceGeneratorsCtxOffsets), &(m_pDef->m_nConstraintsCtxOffsets),
	...
	for(int i = 0; i < nChildCount; ++i)
	{
		...
		if(pDef->m_Children[i].m_bUseNameBasedLookup)
		{
			pChild = g_pParticleSystemMgr->CreateParticleCollection(
				pDef->m_Children[i].m_Name, -m_flCurTime + pDef->m_Children[i].m_flDelay, nRandomSeed);
		}
		else
		{
			pChild = g_pParticleSystemMgr->CreateParticleCollection(
				pDef->m_Children[i].m_Id, -m_flCurTime + pDef->m_Children[i].m_flDelay, nRandomSeed);
		}
		if(pChild)
		{
...
bool CParticleCollection::CheckIfOperatorShouldRun(CParticleOperatorInstance const *pOp, float *pflCurStrength)
{
	...
	float flStrength = FadeInOut(pOp->m_flOpStartFadeInTime, pOp->m_flOpEndFadeInTime, pOp->m_flOpStartFadeOutTime,
								 pOp->m_flOpEndFadeOutTime, flTime);
	if(pflCurStrength)
		...
		if(!bTranslucentOnly || m_pDef->GetMaterial()->IsTranslucent())
		{
			...
			for(int i = 0; i < nCount; i++)
			{
				if(CheckIfOperatorShouldRun(m_pDef->m_Renderers[i]))
				{
					...
					m_pDef->m_Renderers[i]->Render(pRenderContext, this,
...
static void CHECKSYSTEM(CParticleCollection *pParticles)
{
	...
	for(int i = 0; i < pParticles->m_nActiveParticles; ++i)
	{
		...
		/*
				...
				RJ: Disabling this assert.  While the proper way is to fix the math which leads to the bad number, the
		   fix would result in more particles being drawn and in a post shipping world, users were not happy. See
		   Changelists #1368648, #1368635, and #1368434 for proper math calculation fixes.

				In a post shipping world, as these particles would not render with infinites, code was added
		   C_OP_RenderSprites::RenderSpriteCard() to check for the infinite and not add the vert to meshbuilder.
		*/
...
void CParticleCollection::SimulateFirstFrame()
{
	...
	for(int i = 0; i < nCount; i++)
	{
		...
		if(pOp->ShouldRunBeforeEmitters() && CheckIfOperatorShouldRun(pOp, &flStrength))
		{
...
void CParticleCollection::Simulate(float dt, bool updateBboxOnly)
{
	...
	if(!updateBboxOnly)
	{
		...
		// Limit timestep if needed (prevents short lived particles from being created and destroyed before being
		// rendered.
...
for(int i = 0; i < nEmitterCount; i++)
{
	...
	{
		uint32 nInittedMask = m_pDef->m_Emitters[i]->Emit(this, flEmitStrength,
...
void CParticleCollection::GetControlPointAtTime(int nControlPoint, float flTime, Vector *pControlPoint) const
{
	...
	VectorLerp(m_ControlPoints[nControlPoint].m_PrevPosition, m_ControlPoints[nControlPoint].m_Position, t,
			   *pControlPoint);
...
void CParticleCollection::GetControlPointTransformAtCurrentTime(int nControlPoint, matrix3x4_t *pMat)
{
	...
	pMat->Init(m_ControlPoints[nControlPoint].m_ForwardVector, left, m_ControlPoints[nControlPoint].m_UpVector,
			   vecControlPoint);
}
...
void CParticleCollection::GetControlPointTransformAtCurrentTime(int nControlPoint, VMatrix *pMat)
{
	...
	pMat->m[3][0] = pMat->m[3][1] = pMat->m[3][2] = 0.0f;
	pMat->m[3][3] = 1.0f;
}

void CParticleCollection::GetControlPointOrientationAtTime(int nControlPoint, float flTime, Vector *pForward,
...
void CParticleCollection::GetControlPointTransformAtTime(int nControlPoint, float flTime,
...
bool CParticleCollection::IsFinished(void)
{
	...
	for(int i = 0; i < nEmitterCount; i++)
	{
		if(m_pDef->m_Emitters[i]->MayCreateMoreParticles(this,
														 m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i]))
			return false;
	}
...
void CParticleCollection::StopEmission(bool bInfiniteOnly, bool bRemoveAllParticles, bool bWakeOnStop)
{
	...
	{
		m_pDef->m_Emitters[i]->StopEmission(this, m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i],
											bInfiniteOnly);
	}
...
void CParticleCollection::StartEmission(bool bInfiniteOnly)
{
	...
	for(int i = 0; i < m_pDef->m_Emitters.Count(); i++)
	{
		m_pDef->m_Emitters[i]->StartEmission(this, m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i],
											 bInfiniteOnly);
	}
...
void CParticleCollection::ApplyKillList(void)
{
	...
	if(!m_bRequiresOrderInvariance)
	{
		while(nLeftInKillList)
		{
			...
			while(nLeftInKillList && (pCurKillListSlot[nLeftInKillList - 1] == nParticlesActiveNow - 1))
			{
				nLeftInKillList--;
				nParticlesActiveNow--;
			}
...
void CParticleCollection::CalculatePathValues(CPathParameters const &PathIn, float flTimeStamp, Vector *pStartPnt,
											  Vector *pMidPnt, Vector *pEndPnt)
{
	Vector StartPnt;
...
public:
	...
	virtual void TraceLine(const Vector &vecAbsStart, const Vector &vecAbsEnd, unsigned int mask,
						   const class IHandleEntity *ignore, int collisionGroup, CBaseTrace *ptr)
	{
		ptr->fraction = 1.0; // no hit
	}

	virtual void GetRandomPointsOnControllingObjectHitBox(CParticleCollection *pParticles, int nControlPointNumber,
														  int nNumPtsOut, float flBBoxScale,
														  int nNumTrysToGetAPointInsideTheModel, Vector *pPntsOut,
														  Vector vecDirectionBias, Vector *pHitBoxRelativeCoordOut,
														  int *pHitBoxIndexOut)
	{
...
//-----------------------------------------------------------------------------
void CParticleSystemMgr::AddParticleOperator(ParticleFunctionType_t nOpType, IParticleOperatorDefinition *pOpFactory)
{
	m_ParticleOperators[nOpType].AddToTail(pOpFactory);
}

CUtlVector<IParticleOperatorDefinition *> &CParticleSystemMgr::GetAvailableParticleOperatorList(
...
//-----------------------------------------------------------------------------
bool CParticleSystemMgr::ReadParticleDefinitions(CUtlBuffer &buf, const char *pFileName, bool bPrecache,
...
void CParticleSystemMgr::SetLastSimulationTime(float flTime)
{
	...
	// We need to take an average over a decent number of frames because this throttling has a direct feedback effect.
	// Worried about oscillation problems!
...
// Read the particle config file from a utlbuffer
//-----------------------------------------------------------------------------
bool CParticleSystemMgr::ReadParticleConfigFile(CUtlBuffer &buf, bool bPrecache, bool bDecommitTempMemory,
...
//-----------------------------------------------------------------------------
bool CParticleSystemMgr::WriteParticleConfigFile(const char *pParticleSystemName, CUtlBuffer &buf,
...
bool CParticleSystemMgr::WriteParticleConfigFile(CDmxElement *pParticleSystem, CUtlBuffer &buf,
												 bool bPreventNameBasedLookup)
{
...
//-----------------------------------------------------------------------------
CParticleCollection *CParticleSystemMgr::CreateParticleCollection(const char *pParticleSystemName, float flDelay,
																  int nRandomSeed)
{
	if(!pParticleSystemName)
		return NULL;
...
CParticleCollection *CParticleSystemMgr::CreateParticleCollection(const DmObjectId_t &id, float flDelay,
																  int nRandomSeed)
{
...
void CParticleSystemMgr::AddToRenderCache(CParticleCollection *pParticles)
{
	...
	pParticles->m_flNextSleepTime = Max(pParticles->m_flNextSleepTime, (g_pParticleSystemMgr->GetLastSimulationTime() +
...
void CParticleSystemMgr::BuildBatchList(int iRenderCache, IMatRenderContext *pRenderContext,
		...
		for(int j = 0; j < nRenderCount; ++j)
		{
			...
			while(nFirstParticle < pParticles->m_nActiveParticles)
			{
				...
				step.m_nParticleCount = step.m_pRenderer->GetParticlesToRender(
					pParticles, step.m_pContext, nFirstParticle, nRemainingVertices, nRemainingIndices,
					&step.m_nVertCount, &iPart);
...
void CParticleSystemMgr::DumpProfileInformation(void)
{
	...
	for(int i = 0; i < m_pParticleSystemDictionary->NameCount(); i++)
	{
		...
		if(p->m_nMaximumActiveParticles)
			g_pFullFileSystem->FPrintf(fh, "%s,%f,%f,%d,%d\n", p->m_Name.Get(), p->m_flTotalSimTime,
									   p->m_flMaxMeasuredSimTime, p->m_nMaximumActiveParticles, p->m_nMaxParticles);
	}
	...
	for(int iRenderCache = 0; iRenderCache < nRenderCacheCount; ++iRenderCache)
	{
		...
		for(int i = 0; i < nBatchCount; ++i)
		{
			...
			for(int j = 0; j < nBatchStepCount; ++j)
			{
				...
				step.m_pRenderer->RenderUnsorted(step.m_pParticles, step.m_pContext, pRenderContext, meshBuilder,
...
void IParticleSystemQuery::GetRandomPointsOnControllingObjectHitBox(
	CParticleCollection *pParticles, int nControlPointNumber, int nNumPtsOut, float flBBoxScale,
	int nNumTrysToGetAPointInsideTheModel, Vector *pPntsOut, Vector vecDirectionalBias, Vector *pHitBoxRelativeCoordOut,
	int *pHitBoxIndexOut)
{
	for(int i = 0; i < nNumPtsOut; i++)
	{
...
void CParticleCollection::UpdateHitBoxInfo(int nControlPointNumber)
{
	...
	// issue hitbox query
	hb.m_nNumHitBoxes = g_pParticleSystemMgr->Query()->GetControllingObjectHitBoxInfo(this, nControlPointNumber,
