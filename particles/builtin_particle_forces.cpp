...
class C_OP_RandomForce : public CParticleOperatorInstance
{
	...
	virtual void AddForces(FourVectors *pAccumulatedForces, CParticleCollection *pParticles, int nBlocks,
...
void C_OP_RandomForce::AddForces(FourVectors *pAccumulatedForces, CParticleCollection *pParticles, int nBlocks,
								 float flStrength, void *pContext) const
{
	...
	for(int i = 0; i < nBlocks; i++)
	{
		pAccumulatedForces->x =
			AddSIMD(pAccumulatedForces->x, AddSIMD(box_min.x, MulSIMD(box_max.x, RandSIMD(nContext))));
		pAccumulatedForces->y =
			AddSIMD(pAccumulatedForces->y, AddSIMD(box_min.y, MulSIMD(box_max.y, RandSIMD(nContext))));
		pAccumulatedForces->z =
			AddSIMD(pAccumulatedForces->z, AddSIMD(box_min.z, MulSIMD(box_max.z, RandSIMD(nContext))));
		pAccumulatedForces++;
	}
...
void C_OP_TwistAroundAxis::AddForces(FourVectors *pAccumulatedForces, CParticleCollection *pParticles, int nBlocks,
									 float flStrength, void *pContext) const
{
...
void C_OP_AttractToControlPoint::AddForces(FourVectors *pAccumulatedForces, CParticleCollection *pParticles,
...
// TODO: ts is not used
void C_OP_LennardJonesForce::addParticleForce(PhysParticle *a, PhysParticle *b, float distSq, float flStrength,
											  float ts) const
{
	...
	if(a->group == b->group) // In the same group
	{
		...
		//  larger of the two neighbor counts, so the attraction between two particles on the surface will be strong,
		//  but the attraction between a particle inside and a particle on the surface will be weak. I can also try
		...
		// This is a trick to prevent single particles from floating off... the less neighbors a particle has.. the more
		// it sticks This also tends to simulate surface tension
		float surface_tension_modifier =
			((24.0f * m_fSurfaceTension) / (a->neighbor_count + b->neighbor_count + 0.1f)) + 1.0f;
	...
	if(f < -m_fMaxRepulsion)
		f = -m_fMaxRepulsion;
	if(f > m_fMaxAttraction)
		f = m_fMaxAttraction;

	Point3D scaledr =
		(b->center - a->center) * (f / (d + FLT_EPSILON)); // Dividing by d scales distance down to a unit vector
	a->force.add(scaledr);
	b->force.subtract(scaledr);
}

void C_OP_LennardJonesForce::AddForces(FourVectors *pAccumulatedForces, CParticleCollection *pParticles, int nBlocks,
									   float flStrength, void *pContext) const
{
	...
	*/
	...
	for(int i = 0; i < nParticles; i++)
	{
		...
		while(node->particle != NULL)
		{
			...
			// Compare addresses of the two particles. This makes sure we apply a force only once between a pair of
			// particles.
