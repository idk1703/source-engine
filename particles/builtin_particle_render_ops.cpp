...
void SetupParticleVisibility(CParticleCollection *pParticles, CParticleVisibilityData *pVisibilityData,
							 const CParticleVisibilityInputs *pVisibilityInputs, int *nQueryHandle)
{
	...
	*/
	...
	pVisibilityData->m_flAlphaVisibility =
		RemapValClamped(flVisibility, pVisibilityInputs->m_flInputMin, pVisibilityInputs->m_flInputMax,
						pVisibilityInputs->m_flAlphaScaleMin, pVisibilityInputs->m_flAlphaScaleMax);
	pVisibilityData->m_flRadiusVisibility =
		RemapValClamped(flVisibility, pVisibilityInputs->m_flInputMin, pVisibilityInputs->m_flInputMax,
						pVisibilityInputs->m_flRadiusScaleMin, pVisibilityInputs->m_flRadiusScaleMax);
...
void C_OP_RenderPoints::Render(IMatRenderContext *pRenderContext, CParticleCollection *pParticles, void *pContext) const
{
	...
	while(nParticles)
	{
		...
		for(int i = 0; i < nParticlesInBatch; i++)
		{
			...
			meshBuilder.Position3f(SubFloat(xyz[nIndex], nOffset), SubFloat(xyz[nIndex + 1], nOffset),
...
struct SpriteRenderInfo_t
{
	...
	void Init(CParticleCollection *pParticles, int nVertexOffset, float flAgeScale, float flAgeScale2, CSheet *pSheet)
	{
		...
		m_pCreationTimeStamp =
			pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_CREATION_TIME, &m_nCreationTimeStride);
...
class C_OP_RenderSprites : public C_OP_RenderPoints
{
	...
	uint32 GetReadAttributes(void) const
	{
		return PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_ROTATION_MASK | PARTICLE_ATTRIBUTE_RADIUS_MASK |
			   ...
			   PARTICLE_ATTRIBUTE_CREATION_TIME_MASK | PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER1_MASK |
	...
	virtual int GetParticlesToRender(CParticleCollection *pParticles, void *pContext, int nFirstParticle,
									 int nRemainingVertices, int nRemainingIndices, int *pVertsUsed,
									 int *pIndicesUsed) const;
	virtual void Render(IMatRenderContext *pRenderContext, CParticleCollection *pParticles, void *pContext) const;
	virtual void RenderUnsorted(CParticleCollection *pParticles, void *pContext, IMatRenderContext *pRenderContext,
								CMeshBuilder &meshBuilder, int nVertexOffset, int nFirstParticle,
								int nParticleCount) const;
	void RenderSpriteCard(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx, SpriteRenderInfo_t &info,
						  int hParticle, ParticleRenderData_t const *pSortList, Vector *pCamera) const;
	void RenderTwoSequenceSpriteCard(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
									 SpriteRenderInfo_t &info, int hParticle, ParticleRenderData_t const *pSortList,
									 Vector *pCamera) const;

	void RenderNonSpriteCardCameraFacing(CParticleCollection *pParticles, void *pContext,
										 IMatRenderContext *pRenderContext, IMaterial *pMaterial) const;

	void RenderNonSpriteCardZRotating(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
									  SpriteRenderInfo_t &info, int hParticle, const Vector &vecCameraPos,
									  ParticleRenderData_t const *pSortList) const;
	void RenderNonSpriteCardZRotating(CParticleCollection *pParticles, void *pContext,
									  IMatRenderContext *pRenderContext, IMaterial *pMaterial) const;
	void RenderUnsortedNonSpriteCardZRotating(CParticleCollection *pParticles, void *pContext,
											  IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder,
											  int nVertexOffset, int nFirstParticle, int nParticleCount) const;

	void RenderNonSpriteCardOriented(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
									 SpriteRenderInfo_t &info, int hParticle, const Vector &vecCameraPos,
									 ParticleRenderData_t const *pSortList, bool bUseYaw) const;
	void RenderNonSpriteCardOriented(CParticleCollection *pParticles, void *pContext, IMatRenderContext *pRenderContext,
									 IMaterial *pMaterial, bool bUseYaw) const;
	void RenderUnsortedNonSpriteCardOriented(CParticleCollection *pParticles, void *pContext,
											 IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder,
											 int nVertexOffset, int nFirstParticle, int nParticleCount) const;
...
const SheetSequenceSample_t *GetSampleForSequence(CSheet *pSheet, float flCreationTime, float flCurTime,
												  float flAgeScale, int nSequence)
{
	if(pSheet == NULL)
		return NULL;
...
int C_OP_RenderSprites::GetParticlesToRender(CParticleCollection *pParticles, void *pContext, int nFirstParticle,
											 int nRemainingVertices, int nRemainingIndices, int *pVertsUsed,
											 int *pIndicesUsed) const
{
	int nMaxParticles =
		((nRemainingVertices / 4) > (nRemainingIndices / 6)) ? nRemainingIndices / 6 : nRemainingVertices / 4;
...
void C_OP_RenderSprites::RenderNonSpriteCardCameraFacing(CParticleCollection *pParticles, void *pContext,
														 IMatRenderContext *pRenderContext, IMaterial *pMaterial) const
{
	...
	while(nParticles)
	{
		...
		for(int i = 0; i < nParticlesInBatch; i++)
		{
			...
			Vector vecWorldPos(SubFloat(xyz[nXYZIndex], nOffset), SubFloat(xyz[nXYZIndex + 1], nOffset),
			...
			if(pSheet)
			{
				...
				pSample = GetSampleForSequence(pSheet, SubFloat(pCreationTimeStamp[nGroup * ct_stride], nOffset),
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderSprites::RenderNonSpriteCardZRotating(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
													  ...
													  ParticleRenderData_t const *pSortList) const
{
	...
	Vector vecWorldPos(SubFloat(info.m_pXYZ[nXYZIndex], nOffset), SubFloat(info.m_pXYZ[nXYZIndex + 1], nOffset),
	...
	if(info.m_pSheet)
	{
		pSample = GetSampleForSequence(
			info.m_pSheet, SubFloat(info.m_pCreationTimeStamp[nGroup * info.m_nCreationTimeStride], nOffset),
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderSprites::RenderNonSpriteCardZRotating(CParticleCollection *pParticles, void *pContext,
													  IMatRenderContext *pRenderContext, IMaterial *pMaterial) const
{
	C_OP_RenderSpritesContext_t *pCtx = reinterpret_cast<C_OP_RenderSpritesContext_t *>(pContext);
...
void C_OP_RenderSprites::RenderUnsortedNonSpriteCardZRotating(CParticleCollection *pParticles, void *pContext,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderSprites::RenderNonSpriteCardOriented(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
													 SpriteRenderInfo_t &info, int hParticle,
													 const Vector &vecCameraPos, ParticleRenderData_t const *pSortList,
													 bool bUseYaw) const
{
	Assert(hParticle != -1);
	int nGroup = hParticle / 4;
	...
	{
		info.m_pParticles->GetControlPointOrientationAtCurrentTime(m_nOrientationControlPoint, &vecRight, &vecUp,
																   &vecNormal);
	}
...
void C_OP_RenderSprites::RenderNonSpriteCardOriented(CParticleCollection *pParticles, void *pContext,
													 IMatRenderContext *pRenderContext, IMaterial *pMaterial,
													 bool bUseYaw) const
{
	C_OP_RenderSpritesContext_t *pCtx = reinterpret_cast<C_OP_RenderSpritesContext_t *>(pContext);
...
void C_OP_RenderSprites::RenderUnsortedNonSpriteCardOriented(CParticleCollection *pParticles, void *pContext,
															 IMatRenderContext *pRenderContext,
															 CMeshBuilder &meshBuilder, int nVertexOffset,
															 int nFirstParticle, int nParticleCount) const
{
...
void C_OP_RenderSprites::RenderSpriteCard(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
										  SpriteRenderInfo_t &info, int hParticle,
										  ParticleRenderData_t const *pSortList, Vector *pCamera) const
{
	Assert(hParticle != -1);
	int nGroup = hParticle / 4;
	int nOffset = hParticle & 0x3;
	...
	// FIXME: change the vertex decl (remove texcoord3/cornerid) if instancing - need to adjust elements beyond
	// texcoord3 down, though
	if(!bUseInstancing)
		meshBuilder.TexCoord2f(3, 0, 0);
	meshBuilder.TexCoord4f(4, pSecondTexture0->m_fLeft_U0, pSecondTexture0->m_fTop_V0, pSecondTexture0->m_fRight_U0,
						   pSecondTexture0->m_fBottom_V0);
	meshBuilder.AdvanceVertex();
	...
	if(!bUseInstancing)
	{
		meshBuilder.Position3f(vecWorldPos.x, vecWorldPos.y, vecWorldPos.z);
		meshBuilder.Color4ub(rc, gc, bc, ac);
		meshBuilder.TexCoord4f(0, pSample0->m_fLeft_U0, pSample0->m_fTop_V0, pSample0->m_fRight_U0,
							   pSample0->m_fBottom_V0);
		meshBuilder.TexCoord4f(1, pSample0->m_fLeft_U1, pSample0->m_fTop_V1, pSample0->m_fRight_U1,
							   pSample0->m_fBottom_V1);
		meshBuilder.TexCoord4f(2, pSample->m_fBlendFactor, rot, rad, yaw);
...
void C_OP_RenderSprites::RenderTwoSequenceSpriteCard(CMeshBuilder &meshBuilder, C_OP_RenderSpritesContext_t *pCtx,
													 SpriteRenderInfo_t &info, int hParticle,
													 ParticleRenderData_t const *pSortList, Vector *pCamera) const
{
	Assert(hParticle != -1);
	...
	meshBuilder.TexCoord4f(4, pSecondTexture0->m_fLeft_U0, pSecondTexture0->m_fTop_V0, pSecondTexture0->m_fRight_U0,
						   pSecondTexture0->m_fBottom_V0);
	meshBuilder.TexCoord4f(5, pSample1Frame->m_fLeft_U0, pSample1Frame->m_fTop_V0, pSample1Frame->m_fRight_U0,
						   pSample1Frame->m_fBottom_V0);
	meshBuilder.TexCoord4f(6, pSample1Frame->m_fLeft_U1, pSample1Frame->m_fTop_V1, pSample1Frame->m_fRight_U1,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderSprites::Render(IMatRenderContext *pRenderContext, CParticleCollection *pParticles,
								void *pContext) const
{
	IMaterial *pMaterial = pParticles->m_pDef->GetMaterial();
	C_OP_RenderSpritesContext_t *pCtx = reinterpret_cast<C_OP_RenderSpritesContext_t *>(pContext);
	...
	while(nParticles)
	{
		...
		{
			g_pParticleSystemMgr->TallyParticlesRendered(vertexCount * (primType == MATERIAL_TRIANGLES ? 3 : 4),
														 indexCount * (primType == MATERIAL_TRIANGLES ? 3 : 4));
...
void C_OP_RenderSprites::RenderUnsorted(CParticleCollection *pParticles, void *pContext,
										IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder, int nVertexOffset,
										int nFirstParticle, int nParticleCount) const
{
	if(!pParticles->m_pDef->GetMaterial()->IsSpriteCard())
	{
		switch(m_nOrientationType)
		{
			case 0:
				...
				Warning("C_OP_RenderSprites::RenderUnsorted: Attempting to use an unimplemented sprite renderer for "
						"system \"%s\"!\n",
						pParticles->m_pDef->GetName());
				//			RenderUnsortedNonSpriteCardCameraFacing( pParticles, pContext, pRenderContext, meshBuilder,
				//nVertexOffset, nFirstParticle, nParticleCount );
				break;

			case 1:
				RenderUnsortedNonSpriteCardZRotating(pParticles, pContext, pRenderContext, meshBuilder, nVertexOffset,
													 nFirstParticle, nParticleCount);
				break;

			case 2:
				RenderUnsortedNonSpriteCardOriented(pParticles, pContext, pRenderContext, meshBuilder, nVertexOffset,
													nFirstParticle, nParticleCount);
				break;
		}
		return;
...
class C_OP_RenderSpritesTrail : public CParticleRenderOperatorInstance
{
	...
	uint32 GetReadAttributes(void) const
	{
		return PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_PREV_XYZ_MASK | PARTICLE_ATTRIBUTE_RADIUS_MASK |
			   PARTICLE_ATTRIBUTE_TINT_RGB_MASK | PARTICLE_ATTRIBUTE_ALPHA_MASK |
			   PARTICLE_ATTRIBUTE_CREATION_TIME_MASK | PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER_MASK |
	...
	void RenderSpriteTrail(CMeshBuilder &meshBuilder, SpriteTrailRenderInfo_t &info, int hParticle,
...
int C_OP_RenderSpritesTrail::GetParticlesToRender(CParticleCollection *pParticles, void *pContext, int nFirstParticle,
												  int nRemainingVertices, int nRemainingIndices, int *pVertsUsed,
												  int *pIndicesUsed) const
{
	int nMaxParticles =
...
void C_OP_RenderSpritesTrail::RenderSpriteTrail(CMeshBuilder &meshBuilder, SpriteTrailRenderInfo_t &info, int hParticle,
												const Vector &vecCameraPos, float flOODt,
												ParticleRenderData_t const *pSortList) const
{
	Assert(hParticle != -1);
	int nGroup = hParticle / 4;
...
void C_OP_RenderSpritesTrail::Render(IMatRenderContext *pRenderContext, CParticleCollection *pParticles,
									 void *pContext) const
{
	C_OP_RenderSpriteTrailContext_t *pCtx = reinterpret_cast<C_OP_RenderSpriteTrailContext_t *>(pContext);
	IMaterial *pMaterial = pParticles->m_pDef->GetMaterial();
...
void C_OP_RenderSpritesTrail::RenderUnsorted(CParticleCollection *pParticles, void *pContext,
											 IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder,
											 int nVertexOffset, int nFirstParticle, int nParticleCount) const
{
	C_OP_RenderSpriteTrailContext_t *pCtx = reinterpret_cast<C_OP_RenderSpriteTrailContext_t *>(pContext);
...
struct RopeRenderInfo_t
{
	...
	void GenerateSeg(int hParticle, BeamSeg_t &seg)
	{
		...
		seg.m_vPos.Init(SubFloat(m_pXYZ[nXYZIndex], nOffset), SubFloat(m_pXYZ[nXYZIndex + 1], nOffset),
						SubFloat(m_pXYZ[nXYZIndex + 2], nOffset));
		seg.m_vColor.Init(SubFloat(m_pRGB[nColorIndex], nOffset), SubFloat(m_pRGB[nColorIndex + 1], nOffset),
...
class C_OP_RenderRope : public CParticleOperatorInstance
{
	...
	uint32 GetReadAttributes(void) const
	{
		return PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_RADIUS_MASK | PARTICLE_ATTRIBUTE_TINT_RGB_MASK |
	...
	virtual void InitializeContextData(CParticleCollection *pParticles, void *pContext) const
	{
		...
		const_cast<C_OP_RenderRope *>(this)->m_flTextureScale =
			1.0f / (pParticles->m_pDef->GetMaterial()->GetMappingHeight() * m_flTexelSizeInUnits);
	}
	...
	// We connect neighboring particle instances to each other, so if the order isn't maintained we will have a particle
	// that jumps back to the wrong place and look terrible.
...
//-----------------------------------------------------------------------------
int C_OP_RenderRope::GetParticlesToRender(CParticleCollection *pParticles, void *pContext, int nFirstParticle,
										  int nRemainingVertices, int nRemainingIndices, int *pVertsUsed,
										  int *pIndicesUsed) const
{
...
void C_OP_RenderRope::RenderSpriteCard(CParticleCollection *pParticles, void *pContext, IMaterial *pMaterial) const
{
	...
	int nNumSegmentsPerBatch = min((nMaxVertices - 2) / nNumVerticesPerSegment, (nMaxIndices) / nNumIndicesPerSegment);
	...
	g_pParticleSystemMgr->TallyParticlesRendered(2 + nNumSegmentsIWillRenderPerBatch * nNumVerticesPerSegment * 3,
												 nNumIndicesPerSegment * nNumSegmentsIWillRenderPerBatch * 3);
	...
	do
	{
		if(!nSegmentsAvailableInBuffer)
		{
			...
			g_pParticleSystemMgr->TallyParticlesRendered(2 + nNumSegmentsIWillRenderPerBatch * nNumVerticesPerSegment *
																 3,
														 nNumIndicesPerSegment * nNumSegmentsIWillRenderPerBatch * 3);
		...
		if(nSegmentsToRender > 1)
		{
			...
			vecP1 = vecP2;
			vecP2 = vecP3;
			pRadius = pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_RADIUS, nPnt);
			pAlpha = pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_ALPHA, nPnt - 2);
...
void C_OP_RenderRope::Render(IMatRenderContext *pRenderContext, CParticleCollection *pParticles, void *pContext) const
{
	...
	while(nParticles)
	{
		...
		int nParticlesInBatch = GetParticlesToRender(pParticles, pContext, nFirstParticle, nMaxVertices, nMaxIndices,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderRope::RenderUnsorted(CParticleCollection *pParticles, void *pContext, IMatRenderContext *pRenderContext,
									 CMeshBuilder &meshBuilder, int nVertexOffset, int nFirstParticle,
									 int nParticleCount) const
{
	IMaterial *pMaterial = pParticles->m_pDef->GetMaterial();
	...
	beamSegment.Start(pRenderContext, (nParticleCount - 1) * m_nSubdivCount + 1, pMaterial, &meshBuilder,
					  nVertexOffset);
	...
	else
	{
		...
		int nXYZIndex = nGroup * info.m_nXYZStride;
		vecCatmullRom[0].Init(SubFloat(info.m_pXYZ[nXYZIndex], nOffset), SubFloat(info.m_pXYZ[nXYZIndex + 1], nOffset),
							  SubFloat(info.m_pXYZ[nXYZIndex + 2], nOffset));
	}
	...
	for(int i = 1; i < nParticleCount; ++i, ++hParticle)
	{
		...
		if(m_nSubdivCount > 1)
		{
			...
			if(hParticle != info.m_pParticles->m_nActiveParticles - 1)
			{
				...
				int nXYZIndex = nGroup * info.m_nXYZStride;
				vecCatmullRom[(i + 2) & 0x3].Init(SubFloat(info.m_pXYZ[nXYZIndex], nOffset),
			...
			for(int iSubdiv = 1; iSubdiv < m_nSubdivCount; ++iSubdiv)
			{
				...
				Catmull_Rom_Spline(vecCatmullRom[(i + 3) & 0x3], vecCatmullRom[i & 0x3], vecCatmullRom[(i + 1) & 0x3],
								   vecCatmullRom[(i + 2) & 0x3], pSubdivList[iSubdiv], subDivSeg.m_vPos);
...
void C_OP_RenderBlobs::Render(IMatRenderContext *pRenderContext, CParticleCollection *pParticles, void *pContext) const
{
	...
	// FIXME: Make this configurable. Not all shaders perform lighting. Although it's pretty likely for isosurface
	// shaders.
...
class C_OP_RenderScreenVelocityRotate : public CParticleRenderOperatorInstance
{
	...
	virtual void InitializeContextData(CParticleCollection *pParticles, void *pContext) const
	{
		C_OP_RenderScreenVelocityRotateContext_t *pCtx =
			reinterpret_cast<C_OP_RenderScreenVelocityRotateContext_t *>(pContext);
		if(VisibilityInputs.m_nCPin >= 0)
			pCtx->m_VisibilityData.m_bUseVisibility = true;
		else
			pCtx->m_VisibilityData.m_bUseVisibility = false;
...
void C_OP_RenderScreenVelocityRotate::Render(IMatRenderContext *pRenderContext, CParticleCollection *pParticles,
											 void *pContext) const
{
	C_OP_RenderScreenVelocityRotateContext_t *pCtx =
		reinterpret_cast<C_OP_RenderScreenVelocityRotateContext_t *>(pContext);
