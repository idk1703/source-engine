...
template<EParticleSortKeyType eSortKeyMode>
void s_GenerateData(Vector CameraPos, CParticleVisibilityData *pVisibilityData, CParticleCollection *pParticles)
{
	...
	do
	{
		...
		if(eSortKeyMode == SORT_KEY_DISTANCE)
		{
			...
			fl4SortKey = AddSIMD(MulSIMD(Xdiff, Xdiff), AddSIMD(MulSIMD(Ydiff, Ydiff), MulSIMD(Zdiff, Zdiff)));
		}
		else
		{
...
void CParticleCollection::GenerateSortedIndexList(Vector vecCamera, CParticleVisibilityData *pVisibilityData,
												  bool bSorted)
{
...
const ParticleRenderData_t *CParticleCollection::GetRenderList(IMatRenderContext *pRenderContext, bool bSorted,
															   int *pNparticles,
