...
static ConVar cl_timeout(
	"cl_timeout", "30", FCVAR_ARCHIVE,
	"After this many seconds without receiving a packet from the server, the client will disconnect itself");
ConVar cl_logofile("cl_logofile", "materials/decals/spraylogo.vtf", FCVAR_ARCHIVE,
...
static ConVar cl_downloadfilter(
	"cl_downloadfilter", "all", FCVAR_ARCHIVE,
...
#endif
static ConVar cl_always_flush_models(
	"cl_always_flush_models", CONVAR_DEFAULT_ALWAYS_FLUSH_MODELS, FCVAR_INTERNAL_USE,
	"If set, always flush models between map loads.  Useful on systems under memory pressure.");

extern ConVar sv_downloadurl;

#if defined(_DEBUG) || defined(STAGING_ONLY)
static ConVar debug_clientstate_fake_hltv("debug_clientstate_fake_hltv", "0", 0,
										  "If set, spoof as HLTV for testing purposes.");
#if defined(REPLAY_ENABLED)
static ConVar debug_clientstate_fake_replay("debug_clientstate_fake_replay", "0", 0,
											"If set, spoof as REPLAY for testing purposes.");
...
{
	return m_bPaused || (g_LostVideoMemory && Host_IsSinglePlayerGame()) || !host_initialized ||
		   demoplayer->IsPlaybackPaused() || EngineVGui()->ShouldPause();
}
...
void CClientState::SetModel(int tableIndex)
{
	...
	if(tableIndex == 1)
	{
		// The world model must match the LevelFileName -- it is the path we just checked the CRC for, and paths may
		// differ from what the server is using based on what the gameDLL override does
...
void CClientState::SetGeneric(int tableIndex)
{
	...
	// Bogus index
	if(tableIndex < 0 || tableIndex >= m_pGenericPrecacheTable->GetNumStrings())
	{
		return;
	}
...
void CClientState::AddCustomFile(int slot, const char *resourceFile)
{
	...
	if(g_pFileSystem->RelativePathToFullPath(filehex.m_Filename, "game", szAbsFilename, sizeof(szAbsFilename),
											 FILTER_CULLPACK))
	{
...
void CClientState::DumpPrecacheStats(const char *name)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		ConMsg("%03i:  %s (%s):   ", i, pchName, GetFlagString(p->flags));
...
void CClientState::ReadPreserveEnt(CEntityReadInfo &u)
{
	...
	{
		Host_Error("CL_ReadPreserveEnt: Entity out of bounds. Old: %i, New: %i", u.m_nOldEntity, u.m_nNewEntity);
	}
	...
	if(m_bPrepareClientDLL)
	{
		...
		if(g_iServerGameDLLVersion >= 10)
		{
			eResult = serverGameDLL->AsyncPrepareLevelResources(szMutableLevelName, sizeof(szMutableLevelName),
																m_szLevelFileName, sizeof(m_szLevelFileName),
																&flPrepareProgress);
		}
		...
		switch(eResult)
		{
			case IServerGameDLL::ePrepareLevelResources_InProgress:
				...
				EngineVGui()->UpdateCustomProgressBar(flPrepareProgress,
													  g_pVGuiLocalize->Find("#Valve_UpdatingSteamResources"));
				break;
			default:
...
bool CheckSimpleMaterial(IMaterial *pMaterial)
{
	...
	if(Q_strncasecmp(name, "VertexLitGeneric", 16) && Q_strncasecmp(name, "UnlitGeneric", 12))
		return false;
...
//-----------------------------------------------------------------------------
// Purpose: find a filename in the string table, ignoring case and slash mismatches.  Returns the index, or
// INVALID_STRING_INDEX if not found.
//-----------------------------------------------------------------------------
...
void CClientState::FinishSignonState_New()
{
	...
	{
		Host_Error("Unable to verify map %s",
				   (m_szLevelFileName && m_szLevelFileName[0]) ? m_szLevelFileName : "unknown");
		return;
	}
	...
	{
		// Reset the last used count on all models before beginning the new load -- The nServerCount value on models
		// should always resolve as different from values from previous servers.
	...
	// Check for a new whitelist. It's good to do it early in the connection process here because if we wait until
	// later, the client may have loaded some files w/o the proper whitelist restrictions and we'd have to reload them.
	m_bCheckCRCsWithServer = false; // Don't check CRCs yet.. wait until we got a whitelist and cleaned out our files
									// based on it to send CRCs.
...
void CClientState::ConsistencyCheck(bool bChanged)
{
	...
	for(int i = 0; i < m_pDownloadableFileTable->GetNumStrings(); ++i)
	{
		...
		else if(userData && userData[0] == CONSISTENCY_BOUNDS && length == sizeof(ModelBoundsUserData))
		{
			...
			{
				if(pModel->mins.x < boundsData->mins.x || pModel->mins.y < boundsData->mins.y ||
				   pModel->mins.z < boundsData->mins.z)
				{
					ConColorMsg(red, "Model %s exceeds mins (%.1f %.1f %.1f vs. %.1f %.1f %.1f)\n", filename,
								pModel->mins.x, pModel->mins.y, pModel->mins.z, boundsData->mins.x, boundsData->mins.y,
								boundsData->mins.z);
					errorMsg = "Server is enforcing model bounds";
					errorFilename = filename;
				}
				if(pModel->maxs.x > boundsData->maxs.x || pModel->maxs.y > boundsData->maxs.y ||
				   pModel->maxs.z > boundsData->maxs.z)
				{
					ConColorMsg(red, "Model %s exceeds maxs (%.1f %.1f %.1f vs. %.1f %.1f %.1f)\n", filename,
								pModel->maxs.x, pModel->maxs.y, pModel->maxs.z, boundsData->maxs.x, boundsData->maxs.y,
								boundsData->maxs.z);
					errorMsg = "Server is enforcing model bounds";
					errorFilename = filename;
				}
...
void CClientState::RunFrame()
{
	   ...
	   ((ppi.numPagesFree * 12) < (ppi.numPagesUsed + ppi.numPagesFree)))
	{
		   ...
		   (Plat_FloatTime() - s_flLastWarningTime > 3.0f)) // print a warning no faster than once every 3 sec
		{
			...
			Warning("  Currently using %lu pages (%lu Kb) of total %lu pages (%lu Kb total)\n", ppi.numPagesUsed,
					ppi.numPagesUsed * Plat_GetMemPageSize(), (ppi.numPagesFree + ppi.numPagesUsed),
					(ppi.numPagesFree + ppi.numPagesUsed) * Plat_GetMemPageSize());
	...
	else if(s_bLowPagedPoolMemoryWarning)
	{
		...
		Msg("Info: OS Paged Pool Memory restored - currently %lu pages free (%lu Kb) of total %lu pages (%lu Kb "
			"total).\n",
			ppi.numPagesFree, ppi.numPagesFree * Plat_GetMemPageSize(), (ppi.numPagesFree + ppi.numPagesUsed),
			(ppi.numPagesFree + ppi.numPagesUsed) * Plat_GetMemPageSize());
	}
}