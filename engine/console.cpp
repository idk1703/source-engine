...
ConVar con_logfile("con_logfile", "", 0, "Console output gets written to this file", false, 0.0f, false, 0.0f,
				   ConsoleLogFileCallback);
...
static const char *GetTimestampString(void)
{
	...
	Q_snprintf(string, sizeof(string), "%02i/%02i/%04i - %02i:%02i:%02i", today.tm_mon + 1, today.tm_mday,
...
static ConVar con_notifytime("con_notifytime", "8", FCVAR_MATERIAL_SYSTEM_THREAD,
							 "How long to display recent console text to the upper part of the game window");
static ConVar con_times("contimes", "8", FCVAR_MATERIAL_SYSTEM_THREAD,
						"Number of console lines to overlay for debugging.");
static ConVar con_drawnotify("con_drawnotify", "1", 0,
...
static ConVar con_filter_enable("con_filter_enable", "0", FCVAR_MATERIAL_SYSTEM_THREAD,
								"Filters console output based on the setting of con_filter_text. 1 filters completely, "
								"2 displays filtered text brighter than other text.");
static ConVar con_filter_text("con_filter_text", "", FCVAR_MATERIAL_SYSTEM_THREAD,
							  "Text with which to filter console spew. Set con_filter_enable 1 or 2 to activate.");
static ConVar con_filter_text_out(
	"con_filter_text_out", "", FCVAR_MATERIAL_SYSTEM_THREAD,
...
void Con_ShowConsole_f(void)
{
	...
	if(con_enable.GetBool() || developer.GetInt() || CommandLine()->CheckParm("-console") ||
...
#ifdef WIN32
	_snwprintf(da_notify[info->index].szNotify, sizeof(da_notify[info->index].szNotify) / sizeof(wchar_t) - 1, L"%S",
			   msg);
#else
	_snwprintf(da_notify[info->index].szNotify, sizeof(da_notify[info->index].szNotify) / sizeof(wchar_t) - 1, L"%s",
			   msg);
...
void CConPanel::AddToNotify(const Color &clr, char const *msg)
{
	...
	while(m_NotifyText.Count() > 0 && (m_NotifyText.Count() >= con_times.GetInt()))
	{
...
{
	int len = DrawColoredText(font, x, y, 255, 255, 255, 255, data);
...
int CConPanel::ProcessNotifyLines(int &left, int &top, int &right, int &bottom, bool bDraw)
{
	...
	for(int i = 0; i < MAX_DBG_NOTIFY; i++)
	{
		if(realtime < da_notify[i].expire || da_notify[i].expire == -1)
		{
			...
			if(bDraw)
			{
				DrawColoredText(font, x, y, da_notify[i].color[0] * 255, da_notify[i].color[1] * 255,
								da_notify[i].color[2] * 255, 255, da_notify[i].szNotify);
			}
...
void CConPanel::PaintBackground()
{
	...
	if(cl.IsActive())
	{
		...
		{
			Q_snprintf(ver, sizeof(ver), "Server '%s' Map '%s'", cl.m_NetChannel->GetRemoteAddress().ToString(),
