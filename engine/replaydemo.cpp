...
void CReplayDemoRecorder::GetUniqueDemoFilename(char *pOut, int nLength)
{
	...
	Q_snprintf(pOut, nLength, "%04i%02i%02i-%02i%02i%02i-%s.dem", 1900 + today.tm_year, today.tm_mon + 1, today.tm_mday,
...
const char *CReplayDemoRecorder::GetDemoFilename()
{
	...
	const char *pFilename = replay->m_DemoRecorder.GetRecordingFilename();
...
void CReplayDemoRecorder::StopRecording()
{
	...
	// replay_stoprecording gets set to 0 from within the replay session recorder, but only if we aren't starting to
	// record a new round
}
