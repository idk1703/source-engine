...
private:
	...
	CON_COMMAND_MEMBER_F(CEngineVGui, "debugsystemui", ToggleDebugSystemUI, "Show/hide the debug system UI.",
						 FCVAR_CHEAT);
...
void CEngineVGui::Init()
{
	...
	m_hStaticGameUIModule =
		g_pFileSystem->LoadModule(szDllName, "EXECUTABLE_PATH", true); // LoadModule() does a GetLocalCopy() call
	...
	if(staticGameConsole && !CommandLine()->CheckParm("-forcestartupmenu") &&
	   ...
	   (CommandLine()->FindParm("-toconsole") || CommandLine()->FindParm("-console") ||
		CommandLine()->FindParm("-rpt") || CommandLine()->FindParm("-allowdebug")))
	{
...
void CEngineVGui::Shutdown()
{
	...
	// HACK HACK: There was a bug in the old versions of the viewport which would crash in the case where the client
	// .dll hadn't been fully unloaded, so
...
bool CEngineVGui::HideGameUI()
{
	...
	else
	{
		...
		if(levelName && levelName[0] && (engineClient->GetMaxClients() <= 1) && engineClient->IsPaused())
		{
...
void CEngineVGui::OnLevelLoadingFinished()
{
	...
	for(int i = 0; i < m_LoadingProgress.Count() - 1; i++)
	{
		...
		Msg("\t%d\t%.3f\t\ttime: %d%%\t\tinterp: %d%%\t\trepeat: %d\n", m_LoadingProgress[i].eProgress, flTimeTaken,
			nPerc, nInterpPerc, nRepeatCount);
...
bool CEngineVGui::Key_Event(const InputEvent_t &event)
{
	...
	if(bDown && (code == KEY_ESCAPE || code == KEY_XBUTTON_START || code == STEAMCONTROLLER_START) &&
	   !g_ClientDLL->HandleUiToggle())
	{
		if(IsPC())
		{
			if(IsGameUIVisible())
...
void CEngineVGui::UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam, byte cVoiceState, int nPlayersNeeded,
								   bool bHost)
{
...
static void VGui_RecursePanel(CUtlVector<vgui::VPANEL> &panelList, int x, int y, vgui::VPANEL check,
							  bool include_hidden)
{
