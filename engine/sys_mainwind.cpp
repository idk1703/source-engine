...
#define AssertExit(_exp) \
	if(!(_exp))          \
		return;
#define AssertExitF(_exp) \
	if(!(_exp))           \
		return false;
...
private:
	...
	void PlayVideoAndWait(const char *filename,
						  bool bNeedHealthWarning = false); // plays a video file and waits till it's done to return.
...
#ifdef WIN32
void VCR_HandlePlaybackMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if(uMsg == WM_KEYDOWN)
	{
...
void XBX_HandleInvite(DWORD nUserId)
{
	...
	else
	{
		...
		// NOTE: We may need to actually save off the inviter's XID and search for them later on if we took too long or
		// the
	...
	switch(uMsg)
	{
		...
		case WM_SIZE:
			...
			else
			{
				...
				::MoveWindow(hWnd, rcWindow.left, rcWindow.top, rcWindow.right - rcWindow.left,
...
bool CGame::CreateGameWindow(void)
{
	...
	// PIX_ENABLE/PIX_INSTRUMENTATION is a big slowdown (that should never be checked in, but sometimes is by accident),
	// so add this to the Window title too.
...
#if !defined(_X360)
	HWND hwnd = unicode->CreateWindowExW(exFlags, CLASSNAME, uc, style, 0, 0, w, h, NULL, NULL, m_hInstance, NULL);
	// NOTE: On some cards, CreateWindowExW slams the FPU control word
	SetupFPUControlWord();
#else
	HWND hwnd = CreateWindowEx(exFlags, CLASSNAME, windowName, style, 0, 0, w, h, NULL, NULL, m_hInstance, NULL);
#endif
...
void CGame::PlayStartupVideos(void)
{
	...
	// FIXME: There's really no way to know when this is completed, so we have to guess a time that will mostly be
	// correct
	...
	bool bRecap = CommandLine()->CheckParm(
		"-recapvid"); // FIXME: This is a temp addition until the movie playback is centralized -- jdw
	...
	// There is no access to steam at this point so we are checking for the presence of an empty file that will only
	// exist in the chinese depot
	...
	if(!bNeedHealthWarning && !bEndGame && !bRecap &&
	...
	// have to use the malloc memory allocation option in COM_LoadFile since the memory system isn't set up at this
	// point.
...
// hide cursor while playing videos
	...
	while(true)
	{
		...
		localPath[0] = 0; // just to make sure we don't play the same avi file twice in the case that one movie is there
						  // but another isn't.
	}
...
void CGame::PlayVideoAndWait(const char *filename, bool bNeedHealthWarning)
{
	...
	VideoResult_t status = g_pVideo->PlayVideoFileFullScreen(
		filename, "GAME", GetMainWindowPlatformSpecificHandle(), m_width, m_height, m_iDesktopWidth, m_iDesktopHeight,
		videomode->IsWindowedMode(), forcedMinTime,
		VideoPlaybackFlags::DEFAULT_FULLSCREEN_OPTIONS | VideoPlaybackFlags::FILL_WINDOW);
