...
static void HLTV_Callback_InstanceBaseline(void *object, INetworkStringTable *stringTable, int stringNumber,
										   char const *newString, void const *newData)
{
...
void CHLTVClientState::CopyNewEntity(CEntityReadInfo &u, int iClass, int iSerialNum)
{
	...
	int nChangedProps =
		RecvTable_MergeDeltas(pClientClass->m_pRecvTable, &fromBuf, u.m_pBuf, &writeBuf, -1, changedProps);
	...
	{
		SetEntityBaseline((u.m_nBaseline == 0) ? 1 : 0, pClientClass, u.m_nNewEntity, packedData,
...
bool CHLTVClientState::ProcessClassInfo(SVC_ClassInfo *msg)
{
	...
	if(!RecvTable_CreateDecoders(serverGameDLL->GetStandardSendProxies(),
								 bAllowMismatches)) // create receive table decoders
	{
...
void CHLTVClientState::ReadDeltaEnt(CEntityReadInfo &u)
{
	...
	int nChangedProps = RecvTable_MergeDeltas(pToPackedEntity->m_pClientClass->m_pRecvTable, &fromBuf, u.m_pBuf,
											  &writeBuf, -1, changedProps, false);
	...
	if(m_bSaveMemory)
	{
		...
		const char *compressedData =
			m_pHLTV->CompressPackedEntity(pToPackedEntity->m_pServerClass, (char *)writeBuf.GetData(), bits);
