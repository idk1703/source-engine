...
ConVar cl_clock_correction_adjustment_max_amount(
	...
	"It will only correct this amount if the difference between the client and server clock is equal to or larger than "
...
{
	flCurDiffInMS = clamp(flCurDiffInMS, cl_clock_correction_adjustment_min_offset.GetFloat(),
						  cl_clock_correction_adjustment_max_offset.GetFloat());

	float flReturnValue = RemapVal(flCurDiffInMS, cl_clock_correction_adjustment_min_offset.GetFloat(),
...
void CClockDriftMgr::SetServerTick(int nTick)
{
	...
	if(cl_clock_correction_force_server_tick.GetInt() == 999)
	{
		...
		if(!IsClockCorrectionEnabled() || clientTick == 0 || abs(nTick - clientTick) > nMaxDriftTicks)
		{
...
void CClockDriftMgr::ShowDebugInfo(float flAdjustment)
{
	...
	if(IsClockCorrectionEnabled())
	{
		...
		Msg("Clock drift: adjustment (per sec): %.2fms, avg: %.3f, lo: %d, hi: %d, ex: %d\n", flAdjustment * 1000.0f,
			GetCurrentClockDifference(), low, high, exactDiff);
	}
	else
	{
...
bool CClockDriftMgr::IsClockCorrectionEnabled()
{
	...
	// Only in multi-threaded client/server OR in multi player, but don't use it if we're the listen server w/ no fake
	// lag
	return cl_clock_correction.GetInt() && (IsEngineThreaded() || bWantsClockDriftMgr);
#endif
}