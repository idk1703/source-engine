...
public:
	...
	CUtlVector<CProxyPathEntry>
		m_ProxyPathEntries;				 // For each proxy index, this is all the DT proxies that generate it.
...
void SetupArrayProps_R(TableType *pTable)
{
	...
	for(int i = 0; i < pTable->GetNumProps(); i++)
	{
		...
		{
			ErrorIfNot(i >= 1, ("SetupArrayProps_R: array prop '%s' is at index zero.", pProp->GetName()));
