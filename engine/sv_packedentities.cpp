...
void SV_EnsureInstanceBaseline(ServerClass *pServerClass, int iEdict, const void *pData, int nBytes)
{
	...
	ErrorIfNot(pEnt->GetNetworkable(), ("SV_EnsureInstanceBaseline: edict %d missing ent", iEdict));
...
static inline void SV_PackEntity(int edictIdx, edict_t *edict, ServerClass *pServerClass, CFrameSnapshot *pSnapshot)
{
	...
	CUtlMemory<CSendProxyRecipients> recip((CSendProxyRecipients *)tempData,
	...
	PackedEntity *pPrevFrame =
		framesnapshotmanager->GetPreviouslySentPacket(edictIdx, pSnapshot->m_pEntities[edictIdx].m_nSerialNumber);
	if(pPrevFrame)
	{
		// Calculate a delta.
		...
		int nChanges = SendTable_CalcDelta(pSendTable, pPrevFrame->GetData(), pPrevFrame->GetNumBits(), packedData,
			...
			if(!edict->HasStateChanged())
			{
				for(int iDeltaProp = 0; iDeltaProp < nChanges; iDeltaProp++)
				{
					...
					Msg("Entity %d (class '%s') reported ENTITY_CHANGE_NONE but '%s' changed.\n", edictIdx,
						edict->GetClassName(), pProp->GetName());
				}
			}
		}
		...
		ErrorIfNot(pChangeFrame->GetNumProps() == nFlatProps,
				   ("SV_PackEntity: SnagChangeFrameList mismatched number of props[%d vs %d]", nFlatProps,
					pChangeFrame->GetNumProps()));
...
void PackEntities_NetworkBackDoor(int clientCount, CGameClient **clients, CFrameSnapshot *snapshot)
{
	...
	for(int iValidEdict = 0; iValidEdict < snapshot->m_nValidEntities; iValidEdict++)
	{
		...
		g_pLocalNetworkBackdoor->EntState(index, edict->m_NetworkSerialNumber, pSVClass->m_ClassID, pSVClass->m_pTable,
										  edict->GetUnknown(), edict->HasStateChanged(), bShouldTransmit);
		edict->ClearStateChanged();
	}
...
void SV_ComputeClientPacks(int clientCount, CGameClient **clients, CFrameSnapshot *snapshot)
{
