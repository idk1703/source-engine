...
#ifndef SWDS
	if(g_pMaterialSystemHardwareConfig->GetHardwareHDRType() != HDR_TYPE_NONE && saverestore->IsValidSave() &&
	   modelloader->LastLoadedMapHasHDRLighting() && sv.GetMaxClients() == 1 && !sv.IsLevelMainMenuBackground())
	{
...
//-----------------------------------------------------------------------------
static const char *s_pRegistryConVars[] = {"mat_forceaniso", "mat_picmip",
										   //	"mat_dxlevel",
...
static void OverrideMaterialSystemConfigFromCommandLine(MaterialSystem_Config_t &config)
{
	...
	if(CommandLine()->FindParm("-sw") || CommandLine()->FindParm("-startwindowed") ||
	   CommandLine()->FindParm("-windowed") || CommandLine()->FindParm("-window"))
	{
...
void OverrideMaterialSystemConfig(MaterialSystem_Config_t &config)
{
	...
	if(mat_supportflashlight.GetInt() == -1)
	{
		...
		if(!V_stricmp(gameName, "portal") || !V_stricmp(gameName, "tf") || !V_stricmp(gameName, "tf_beta"))
		{
...
void InitMaterialSystemConfig(bool bInEditMode)
{
	...
	if(driverInfo.m_VendorID == currentVendorID && driverInfo.m_DeviceID == currentDeviceID &&
	   !CommandLine()->FindParm("-autoconfig") && !CommandLine()->FindParm("-dxlevel"))
	{
...
void GetMaterialSystemConfigForBenchmarkUpload(KeyValues *dataToUpload)
{
	...
	dataToUpload->SetInt("NoWaitForVSync",
						 (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_NO_WAIT_FOR_VSYNC) ? 1 : 0);
	dataToUpload->SetInt("DisableSpecular",
						 (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_DISABLE_SPECULAR) ? 1 : 0);
	dataToUpload->SetInt("DisableBumpmapping",
						 (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_DISABLE_BUMPMAP) ? 1 : 0);
	dataToUpload->SetInt("EnableParallaxMapping",
						 (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_ENABLE_PARALLAX_MAPPING) ? 1 : 0);
	dataToUpload->SetInt("ZPrefill", (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_USE_Z_PREFILL) ? 1 : 0);
	dataToUpload->SetInt("ReduceFillRate",
						 (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_REDUCE_FILLRATE) ? 1 : 0);
...
CON_COMMAND(mat_enable_vrmode, "Switches the material system to VR mode (after restart)")
{
	...
	if(bVRMode)
	{
		...
		if(nVRModeAdapter == -1)
		{
			Warning("Unable to get VRModeAdapter from OpenVR. VR mode will not be enabled. Try restarting and then "
					"enabling VR again.\n");
		}
		mat_vrmode_adapter.SetValue(nVRModeAdapter);
#else
		mat_vrmode_adapter.SetValue(
			0); // This convar isn't actually used on other platforms so just use 0 to indicate that it's set
#endif
	}
	else
	{
...
static ITexture *CreatePowerOfTwoFBTexture(void)
{
	...
	return materials->CreateNamedRenderTargetTextureEx2("_rt_PowerOfTwoFB", 1024, 1024, RT_SIZE_DEFAULT,
...
static ITexture *CreateWaterReflectionTexture(void)
{
	return materials->CreateNamedRenderTargetTextureEx2(
		"_rt_WaterReflection", 1024, 1024, RT_SIZE_PICMIP, materials->GetBackBufferFormat(), MATERIAL_RT_DEPTH_SHARED,
...
{
	return materials->CreateNamedRenderTargetTextureEx2("_rt_Camera", 256, 256, RT_SIZE_DEFAULT,
														materials->GetBackBufferFormat(), MATERIAL_RT_DEPTH_SHARED, 0,
														CREATERENDERTARGETFLAGS_HDR);
}

static ITexture *CreateBuildCubemaps16BitTexture(void)
{
	return materials->CreateNamedRenderTargetTextureEx2("_rt_BuildCubemaps16bit", 0, 0, RT_SIZE_FULL_FRAME_BUFFER,
...
static ITexture *CreateQuarterSizedFBTexture(int n, unsigned int iRenderTargetFlags)
{
	...
	return materials->CreateNamedRenderTargetTextureEx2(nbuf, 0, 0, RT_SIZE_HDR, fmt, MATERIAL_RT_DEPTH_SHARED,
...
static ITexture *CreateTeenyFBTexture(int n)
{
	...
	return materials->CreateNamedRenderTargetTextureEx2(nbuf, 32, 32, RT_SIZE_DEFAULT, fmt, MATERIAL_RT_DEPTH_SHARED);
}
...
static ITexture *CreateFullFrameFBTexture(int textureIndex, int iExtraFlags = 0)
{
	...
	return materials->CreateNamedRenderTargetTextureEx2(textureName, 1, 1, RT_SIZE_FULL_FRAME_BUFFER,
...
static ITexture *CreateResolvedFullFrameDepthTexture(void)
{
	...
	{
		return materials->CreateNamedRenderTargetTextureEx2(
			"_rt_ResolvedFullFrameDepth", 1, 1, RT_SIZE_FULL_FRAME_BUFFER, IMAGE_FORMAT_RGBA8888,
			MATERIAL_RT_DEPTH_SEPARATE, TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_POINTSAMPLE,
			CREATERENDERTARGETFLAGS_NOEDRAM);
	}
...
void InitWellKnownRenderTargets(void)
{
	...
	if(g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80)
	{
		...
		if(IsX360())
			materials->AddTextureAlias("_rt_SmallFB1",
									   "_rt_SmallFB0"); // an alias is good enough on the 360 since we don't have a
														// texture lock problem during post processing
		else
	...
	pRenderContext->SetNonInteractiveTempFullscreenBuffer(g_FullFrameFBTexture0,
...
static void InitDebugMaterials(void)
{
	...
	g_pMaterialWireframeVertexColorIgnoreZ =
		GL_LoadMaterial("debug/debugwireframevertexcolorignorez", TEXTURE_GROUP_OTHER);
	...
	if(g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90)
	{
		...
		g_pMaterialDepthWrite[0][0] =
			g_pMaterialSystem->FindProceduralMaterial("__DepthWrite00", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		...
		g_pMaterialDepthWrite[0][1] =
			g_pMaterialSystem->FindProceduralMaterial("__DepthWrite01", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		...
		pVMTKeyValues->SetInt("$nocull", 0);
		g_pMaterialDepthWrite[1][0] =
			g_pMaterialSystem->FindProceduralMaterial("__DepthWrite10", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		...
		pVMTKeyValues->SetInt("$alphatest", 1);
		pVMTKeyValues->SetInt("$nocull", 1);
		g_pMaterialDepthWrite[1][1] =
			g_pMaterialSystem->FindProceduralMaterial("__DepthWrite11", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		...
		g_pMaterialSSAODepthWrite[0][0] =
			g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite00", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		...
		pVMTKeyValues->SetInt("$nocull", 1);
		pVMTKeyValues->SetInt("$color_depth", 1);
		g_pMaterialSSAODepthWrite[0][1] =
			g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite01", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		...
		pVMTKeyValues->SetInt("$nocull", 0);
		pVMTKeyValues->SetInt("$color_depth", 1);
		g_pMaterialSSAODepthWrite[1][0] =
			g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite10", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		...
		pVMTKeyValues->SetInt("$nocull", 1);
		pVMTKeyValues->SetInt("$color_depth", 1);
		g_pMaterialSSAODepthWrite[1][1] =
			g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite11", TEXTURE_GROUP_OTHER, pVMTKeyValues);
		g_pMaterialSSAODepthWrite[1][1]->IncrementReferenceCount();
	}
	else
	{
		g_pMaterialDepthWrite[0][0] = g_pMaterialDepthWrite[0][1] = g_pMaterialDepthWrite[1][0] =
			g_pMaterialDepthWrite[1][1] = NULL;
		g_pMaterialSSAODepthWrite[0][0] = g_pMaterialSSAODepthWrite[0][1] = g_pMaterialSSAODepthWrite[1][0] =
			g_pMaterialSSAODepthWrite[1][1] = NULL;
	}
#endif
}
...
void TangentSpaceComputeBasis(Vector &tangentS, Vector &tangentT, const Vector &normal, const Vector &tVect,
							  bool negateTangent)
{
	...
	for(int i = 0; i < MSurf_VertCount(surfID); i++)
	{
		...
		if(!SurfaceHasDispInfo(surfID) && (MSurf_TexInfo(surfID)->texinfoFlags & TEXINFO_USING_BASETEXTURE2))
		{
			...
			if(!bWarned)
			{
				const char *pMaterialName = MSurf_TexInfo(surfID)->material->GetName();
				bWarned = true;
				Warning("Warning: WorldTwoTextureBlend found on a non-displacement surface (material: %s). This wastes "
						"perf for no benefit.\n",
						pMaterialName);
			}
...
void WorldStaticMeshCreate(void)
{
	...
	for(i = 0; i < g_Meshes.Count(); i++)
	{
		...
		g_Meshes[i].pVertexBuffer =
			pRenderContext->CreateStaticVertexBuffer(GetUncompressedFormat(g_Meshes[i].pMaterial),
													 g_Meshes[i].vertCount, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_WORLD);
	...
	for(i = 0; i < g_Meshes.Count(); i++)
	{
		...
		g_Meshes[i].pMesh = pRenderContext->CreateStaticMesh(vertexFormat, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_WORLD,
...
void SurfComputeTextureCoordinate(SurfaceCtx_t const &ctx, SurfaceHandle_t surfID, Vector const &vec, Vector2D &uv)
{
...
void SurfComputeLightmapCoordinate(SurfaceCtx_t const &ctx, SurfaceHandle_t surfID, Vector const &vec, Vector2D &uv)
{
...
{
	materials->GetLightmapPageSize(SortInfoToLightmapPage(MSurf_MaterialSortID(surfID)), &ctx.m_LightmapPageSize[0],
								   &ctx.m_LightmapPageSize[1]);
