...
static ConVar sv_creationtickcheck("sv_creationtickcheck", "1", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
...
{
	AssertMsg1(m_FrameSnapshots.Count() == 0 || IsInErrorExit(),
			   "Expected m_FrameSnapshots to be empty. It had %i items.", m_FrameSnapshots.Count());

	// TODO: This assert has been failing. HenryG says it's a valid assert and that we're probably leaking memory.
	AssertMsg1(m_PackedEntitiesPool.Count() == 0 || IsInErrorExit(),
			   "Expected m_PackedEntitiesPool to be empty. It had %i items.", m_PackedEntitiesPool.Count());
}
...
bool CFrameSnapshotManager::ShouldForceRepack(CFrameSnapshot *pSnapshot, int entity, PackedEntityHandle_t handle)
{
	if(sv_creationtickcheck.GetBool())
	{
		...
		if(pe && pe->ShouldCheckCreationTick())
		{
			...
			int nPackedEntityNetworkBase =
				g_ServerGlobalVariables.GetNetworkBase(pe->GetSnapshotCreationTick(), entity);
...
bool CFrameSnapshotManager::UsePreviouslySentPacket(CFrameSnapshot *pSnapshot, int entity, int entSerialNumber)
{
