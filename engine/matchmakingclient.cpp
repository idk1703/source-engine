...
void CMatchmaking::HandleSystemLinkReply(netpacket_t *pPacket)
{
	...
	DevMsg("Result #%d: %d open public, %d open private\n", m_pSystemLinkResults.Count() - 1,
		   pResult->dwOpenPublicSlots, pResult->dwOpenPrivateSlots);
}
...
bool CMatchmaking::SearchForSession()
{
	...
	ret = g_pXboxSystem->SessionSearch(SESSION_MATCH_QUERY_PLAYER_MATCH, XBX_GetPrimaryUserId(), MAX_SEARCHRESULTS, 1,
									   0, 0, NULL, NULL,
	...
	ret = g_pXboxSystem->SessionSearch(SESSION_MATCH_QUERY_PLAYER_MATCH, // Procedure index
	...
	if(!m_Session.IsSystemLink())
	{
		...
		else
		{
			if(ret == ERROR_SUCCESS && m_pSearchResults && m_pSearchResults->dwSearchResults)
			{
				...
				XNetQosLookup(m_pSearchResults->dwSearchResults, m_QoSxnaddr, m_QoSxnkid, m_QoSxnkey,
	...
	for(unsigned int i = 0; i < m_pSearchResults->dwSearchResults; ++i)
	{
		...
		else if((m_pQoSResult->axnqosinfo[i].bFlags & XNET_XNQOSINFO_TARGET_DISABLED))
		{
			...
			continue;
				...
				!m_pQoSResult->axnqosinfo[i].cbData || !m_pQoSResult->axnqosinfo[i].pbData)
		{
		...
		unsigned short ping =
			m_pQoSResult->axnqosinfo[i]
				.wRttMinInMsecs; // Use min ping to suit better for traffic bursts and lossy connections
		DevMsg("Result #%d: ping min %d ms, med %d ms\n", i, m_pQoSResult->axnqosinfo[i].wRttMinInMsecs,
	...
	switch(m_InviteState)
	{
		...
		case INVITE_VALIDATING:
			...
			if((m_InviteWaitingInfo.m_SignInState != eXUserSigninState_SignedInToLive) ||
			   (ERROR_SUCCESS !=
				XUserGetSigninInfo(m_InviteWaitingInfo.m_UserIdx, 0, &m_InviteWaitingInfo.m_SignInInfo)) ||
			...
			if((ERROR_SUCCESS != XUserCheckPrivilege(m_InviteWaitingInfo.m_UserIdx, XPRIVILEGE_MULTIPLAYER_SESSIONS,
		...
		case INVITE_AWAITING_STORAGE:
			// Wait for user to select storage, but keep an eye on user change or logout or etc
			{
				...
				if((InviteCurrentInfo.m_SignInState != m_InviteWaitingInfo.m_SignInState) ||
				   (ERROR_SUCCESS !=
					XUserGetSigninInfo(InviteCurrentInfo.m_UserIdx, 0, &InviteCurrentInfo.m_SignInInfo)) ||
				...
				if((ERROR_SUCCESS != XUserCheckPrivilege(InviteCurrentInfo.m_UserIdx, XPRIVILEGE_MULTIPLAYER_SESSIONS,
...
bool CMatchmaking::ProcessJoinResponse(MM_JoinResponse *pMsg)
{
	switch(pMsg->m_ResponseType)
	{
		...
		case MM_JoinResponse::JOINRESPONSE_APPROVED_JOINGAME:
			...
			ApplySessionProperties(pMsg->m_ContextCount, pMsg->m_PropertyCount, pMsg->m_SessionContexts.Base(),
...
//-----------------------------------------------------------------------------
void CMatchmaking::ApplySessionProperties(int numContexts, int numProperties, XUSER_CONTEXT *pContexts,
