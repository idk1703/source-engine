...
struct overlayvert_t
{
	...
	Vector2D texCoord[NUM_OVERLAY_TEXCOORDS]; // texcoord 0 = the mapped tex coord from worldcraft
											  // texcoord 1 is used for alpha and maps the whole texture into the whole
											  // overlay
	float lightCoord[2];
	float flAlpha;
...
void Overlay_OverlayUVToOverlayPlane(const Vector &vecBasisOrigin, const Vector &vecBasisU, const Vector &vecBasisV,
									 const Vector &vecUVPoint, Vector &vecPlanePoint);
...
void Overlay_OverlayPlaneToWorld(const Vector &vecBasisNormal, SurfaceHandle_t surfID, const Vector &vecPlanePoint,
...
void Overlay_TriTLToBR(CDispInfo *pDisp, Vector &vecWorld, float flU, float flV, int nSnapU, int nSnapV, int nWidth,
					   int nHeight);
void Overlay_TriBLToTR(CDispInfo *pDisp, Vector &vecWorld, float flU, float flV, int nSnapU, int nSnapV, int nWidth,
					   int nHeight);
...
private:
	...
	bool Surf_PreClipFragment(moverlay_t *pOverlay, moverlayfragment_t &overlayFrag, SurfaceHandle_t surfID,
							  moverlayfragment_t &surfaceFrag);
	void Surf_PostClipFragment(moverlay_t *pOverlay, moverlayfragment_t &overlayFrag, SurfaceHandle_t surfID);
	void Surf_ClipFragment(moverlay_t *pOverlay, moverlayfragment_t &overlayFrag, SurfaceHandle_t surfID,
						   moverlayfragment_t &surfaceFrag);
	...
	void Disp_PostClipFragment(CDispInfo *pDisp, CMeshReader *pReader, moverlay_t *pOverlay,
	...
	void BuildClipPlanes(SurfaceHandle_t surfID, moverlayfragment_t &surfaceFrag, const Vector &vecBasisNormal,
						 CUtlVector<cplane_t> &m_ClipPlanes);
	void DoClipFragment(moverlayfragment_t *pFragment, cplane_t *pClipPlane, moverlayfragment_t **ppFront,
...
int COverlayMgr::GetMaterialSortID(IMaterial *pMaterial, int nLightmapPage)
{
	...
	unsigned short i;
	for(i = m_RenderQueue.Head(); i != m_RenderQueue.InvalidIndex(); i = m_RenderQueue.Next(i))
	{
...
void COverlayMgr::RenderOverlays(int nSortGroup)
{
	...
	while(iCurrentRenderOrder <= iHighestRenderOrder)
	{
		...
		for(int i = m_nFirstRenderQueue[nSortGroup]; i != RENDER_QUEUE_INVALID; i = nNextRenderQueue)
		{
			...
			int nMaxVertices = pRenderContext->GetMaxVerticesToRender(!bWireframeFragments ? renderQueueHead.m_pMaterial
					...
					if(!pMesh) // have we output any vertices yet? if first verts, init material and meshbuilder
					{
						if(!bWireframeFragments && !bBoundMaterial)
						{
							...
							bLightmappedMaterial =
								renderQueueHead.m_pMaterial->GetPropertyFlag(MATERIAL_PROPERTY_NEEDS_LIGHTMAP) ||
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool COverlayMgr::Surf_PreClipFragment(moverlay_t *pOverlay, moverlayfragment_t &overlayFrag, SurfaceHandle_t surfID,
									   moverlayfragment_t &surfaceFrag)
{
	MEM_ALLOC_CREDIT();
	...
	{
		Overlay_OverlayUVToOverlayPlane(pOverlay->m_vecOrigin, pOverlay->m_vecBasis[0], pOverlay->m_vecBasis[1],
	...
	{
		// Position.
		Overlay_WorldToOverlayPlane(pOverlay->m_vecOrigin, pOverlay->m_vecBasis[2], surfaceFrag.m_aPrimVerts[iVert].pos,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void COverlayMgr::Surf_PostClipFragment(moverlay_t *pOverlay, moverlayfragment_t &overlayFrag, SurfaceHandle_t surfID)
{
#ifndef SWDS
	...
	{
		Vector2D vecUV;
		PointInQuadToBarycentric(origOverlay.m_aPrimVerts[0].pos, origOverlay.m_aPrimVerts[3].pos,
								 origOverlay.m_aPrimVerts[2].pos, origOverlay.m_aPrimVerts[1].pos,
								 overlayFrag.m_aPrimVerts[iVert].pos, vecUV);

		Overlay_OverlayPlaneToWorld(pOverlay->m_vecBasis[2], surfID, overlayFrag.m_aPrimVerts[iVert].pos,
		...
		{
			TexCoordInQuadFromBarycentric(origOverlay.m_aPrimVerts[0].texCoord[iTexCoord],
...
//-----------------------------------------------------------------------------
void COverlayMgr::Surf_ClipFragment(moverlay_t *pOverlay, moverlayfragment_t &overlayFrag, SurfaceHandle_t surfID,
									moverlayfragment_t &surfaceFrag)
{
	MEM_ALLOC_CREDIT();
	...
	if(pClippedFrag)
	{
		...
		for(int iVert = 0; iVert < pClippedFrag->m_aPrimVerts.Count(); ++iVert)
		{
			...
			memcpy(overlayFrag.m_aPrimVerts[iVert].texCoord, pClippedFrag->m_aPrimVerts[iVert].texCoord,
				   sizeof(overlayFrag.m_aPrimVerts[iVert].texCoord));
		}
	}
...
void COverlayMgr::Surf_CreateFragments(moverlay_t *pOverlay, SurfaceHandle_t surfID)
{
	...
	for(int iTri = 0; iTri < nSurfTriangleCount; ++iTri)
	{
		...
		if(TriangleArea(surfaceFrag.m_aPrimVerts[0].pos, surfaceFrag.m_aPrimVerts[1].pos,
	...
	for(int iOverlay = 0; iOverlay < nOverlayCount; ++iOverlay, ++pOverlayIn)
	{
		...
		if(pOverlayFadesIn)
		{
			...
			pOverlayOut->m_flInvFadeRangeSq =
				1.0f / (pOverlayFadesIn->flFadeDistMaxSq - pOverlayFadesIn->flFadeDistMinSq);
		...
		if(pOverlayOut->m_vecBasis[0].x == 0.0f && pOverlayOut->m_vecBasis[0].y == 0.0f &&
		   pOverlayOut->m_vecBasis[0].z == 0.0f)
		{
			Warning("Bad overlay basis at (%f %f %f)!\n", pOverlayOut->m_vecOrigin.x, pOverlayOut->m_vecOrigin.y,
					pOverlayOut->m_vecOrigin.z);
		}
									   ...
									   SurfaceHandle_t surfID)
{
	...
	for(int iVert = 0; iVert < nVertCount; ++iVert)
	{
		PointInQuadToBarycentric(
			surfaceFrag.m_aPrimVerts[iPointStart].pos, surfaceFrag.m_aPrimVerts[(iPointStart + 3) % 4].pos,
			surfaceFrag.m_aPrimVerts[(iPointStart + 2) % 4].pos, surfaceFrag.m_aPrimVerts[(iPointStart + 1) % 4].pos,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void COverlayMgr::DoClipFragment(moverlayfragment_t *pFragment, cplane_t *pClipPlane, moverlayfragment_t **ppFront,
								 moverlayfragment_t **ppBack)
{
	...
	for(iVert = 0; iVert < nVertCount; ++iVert)
	{
		...
		vert.pos =
			pFragment->m_aPrimVerts[iVert].pos +
			fraction * (pFragment->m_aPrimVerts[(iVert + 1) % nVertCount].pos - pFragment->m_aPrimVerts[iVert].pos);
		for(int iTexCoord = 0; iTexCoord < NUM_OVERLAY_TEXCOORDS; iTexCoord++)
		{
			vert.texCoord[iTexCoord][0] =
				pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][0] +
				fraction * (pFragment->m_aPrimVerts[(iVert + 1) % nVertCount].texCoord[iTexCoord][0] -
							pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][0]);
			vert.texCoord[iTexCoord][1] =
				pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][1] +
				fraction * (pFragment->m_aPrimVerts[(iVert + 1) % nVertCount].texCoord[iTexCoord][1] -
							pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][1]);
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void COverlayMgr::BuildClipPlanes(SurfaceHandle_t surfID, moverlayfragment_t &surfaceFrag, const Vector &vecBasisNormal,
								  CUtlVector<cplane_t> &m_ClipPlanes)
{
	int nVertCount = surfaceFrag.m_aPrimVerts.Count();
	for(int iVert = 0; iVert < nVertCount; ++iVert)
...
// Purpose:
//-----------------------------------------------------------------------------
void Overlay_TriTLToBR(CDispInfo *pDisp, CMeshReader *pReader, Vector &vecWorld, float flU, float flV, int nWidth,
					   const Vector &vecIntersectPoint)
{
	...
	if((flFracU + flFracV) >= (1.0f + TRIEDGE_EPSILON))
	{
		...
		if(nSnapU == nNextU)
		{
			...
			{
				float flFrac =
					(vecIntersectPoint - vecFlatVerts[0]).Length() / (vecFlatVerts[2] - vecFlatVerts[0]).Length();
		...
		{
			float flCfs[3];
			if(CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1], vecFlatVerts[2], vecIntersectPoint, flCfs[0],
									 flCfs[1], flCfs[2]))
			{
			...
			else
			{
				...
				{
					CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1], vecFlatVerts[2], vecIntersectPoint,
										  flCfs[0], flCfs[1], flCfs[2]);
					vecWorld = (vecVerts[0] * flCfs[0]) + (vecVerts[1] * flCfs[1]) + (vecVerts[2] * flCfs[2]);
				}
			}
		}
	...
	else
	{
		...
		if(nSnapU == nNextU)
		{
			...
			{
				float flFrac =
					(vecIntersectPoint - vecFlatVerts[0]).Length() / (vecFlatVerts[1] - vecFlatVerts[0]).Length();
				vecWorld = vecVerts[0] + (flFrac * (vecVerts[1] - vecVerts[0]));
			}
		}
		else if(nSnapV == nNextV)
...
// Purpose:
//-----------------------------------------------------------------------------
void Overlay_TriBLToTR(CDispInfo *pDisp, CMeshReader *pReader, Vector &vecWorld, float flU, float flV, int nWidth,
					   const Vector &vecIntersectPoint)
{
	int nHeight = nWidth;
						   ...
						   moverlayfragment_t &surfaceFrag)
{
	...
	PointInQuadFromBarycentric(surfaceFrag.m_aPrimVerts[(0 + pDisp->m_iPointStart) % 4].pos,
							   ...
							   surfaceFrag.m_aPrimVerts[(1 + pDisp->m_iPointStart) % 4].pos, vecUV, vecIntersectPoint);
