...
#define SETUP_VISIT() \
	class CDummyClass \
	{                 \
	} // Workaround for parser bug in VC7.1
...
bool CompareRecvPropToSendProp(const RecvProp *pRecvProp, const SendProp *pSendProp)
{
	while(1)
	{
		ErrorIfNot(pRecvProp && pSendProp, ("CompareRecvPropToSendProp: missing a property."));
...
static bool MatchRecvPropsToSendProps_R(CUtlRBTree<MatchingProp_t, unsigned short> &lookup, char const *sendTableName,
										SendTable *pSendTable, RecvTable *pRecvTable, bool bAllowMismatches,
										bool *pAnyMismatches)
{
	for(int i = 0; i < pSendTable->m_nProps; i++)
	{
		...
		{
			if(!MatchRecvPropsToSendProps_R(lookup, sendTableName, pSendProp->GetDataTable(),
											FindRecvTable(pSendProp->GetDataTable()->m_pNetTableName), bAllowMismatches,
											pAnyMismatches))
				return false;
		}
	}
...
static void CopySendPropsToRecvProps(CUtlRBTree<MatchingProp_t, unsigned short> &lookup,
...
bool RecvTable_CreateDecoders(const CStandardSendProxies *pSendProxies, bool bAllowMismatches, bool *pAnyMismatches)
{
	...
	FOR_EACH_LL(g_RecvDecoders, i)
	{
		...
		if(!MatchRecvPropsToSendProps_R(PropLookup, pDecoder->GetSendTable()->m_pNetTableName, pDecoder->GetSendTable(),
										FindRecvTable(pDecoder->GetSendTable()->m_pNetTableName), bAllowMismatches,
										pAnyMismatches))
			return false;
...
bool RecvTable_Decode(RecvTable *pTable, void *pStruct, bf_read *pIn, int objectID, bool updateDTI)
{
	CRecvDecoder *pDecoder = pTable->m_pDecoder;
	ErrorIfNot(pDecoder, ("RecvTable_Decode: table '%s' missing a decoder.", pTable->GetName()));
	...
	while((iProp = deltaBitsReader.ReadNextPropIndex()) < MAX_DATATABLE_PROPS)
	{
		...
		decodeInfo.m_pRecvProp =
			theStack.IsCurProxyValid() ? pProp : NULL; // Just skip the data if the proxies are screwed.
...
{
	CRecvDecoder *pDecoder = pTable->m_pDecoder;
	ErrorIfNot(pDecoder, ("RecvTable_DecodeZeros: table '%s' missing a decoder.", pTable->GetName()));
...
int RecvTable_MergeDeltas(RecvTable *pTable,
						  ...
						  int objectID, int *pChangedProps, bool updateDTI)
{
	ErrorIfNot(pTable && pNewState && pOut, ("RecvTable_MergeDeltas: invalid parameters passed."));
	...
	ErrorIfNot(!(pOldState && pOldState->IsOverflowed()) && !pNewState->IsOverflowed() && !pOut->IsOverflowed(),
