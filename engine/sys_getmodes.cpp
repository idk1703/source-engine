...
ConVar cl_savescreenshotstosteam("cl_savescreenshotstosteam", "0", FCVAR_HIDDEN,
...
public:
	...
	virtual void TakeSnapshotTGARect(const char *pFilename, int x, int y, int w, int h, int resampleWidth,
									 int resampleHeight, bool bPFM, CubeMapFaceIndex_t faceIndex);
...
private:
	...
	void BlitGraphicToHDCWithAlpha(HDC hdc, byte *rgba, int imageWidth, int imageHeight, int x0, int y0, int x1,
								   int y1);
	...
	void TakeSnapshotPFMRect(const char *pFilename, int x, int y, int w, int h, int resampleWidth, int resampleHeight,
	...
	if(UseVR() || ShouldForceVRActive())
	{
		...
		if(g_pSourceVR->GetDisplayBounds(&vrBounds))
		{
			...
			for(int i = 0; i < SDL_GetNumVideoDisplays(); i++)
			{
				...
				if(sdlRect.x == vrBounds.nX && sdlRect.y == vrBounds.nY && sdlRect.w == vrBounds.nWidth &&
				   sdlRect.h == vrBounds.nHeight)
				{
...
void CVideoMode_Common::ComputeStartupGraphicName(char *pBuf, int nBufLen)
{
	...
	{
		Q_strncpy(pBuf,
				  (aspectRatio >= 1.6f) ? "materials/console/background01_widescreen.vtf"
	...
	if(cl_savescreenshotstosteam.GetBool())
	{
		if(Steam3Client().SteamScreenshots())
		{
			ScreenshotHandle hScreenshot =
				Steam3Client().SteamScreenshots()->WriteScreenshot(pImage, cubImage, width, height);
...
void CVideoMode_Common::AddScreenshotToSteam(const char *pchFilename, int width, int height)
{
	...
	if(cl_savescreenshotstosteam.GetBool())
	{
		...
		{
			ScreenshotHandle hScreenshot =
				Steam3Client().SteamScreenshots()->AddScreenshotToLibrary(pchFilename, NULL, width, height);
			ApplySteamScreenshotTags(hScreenshot);
		}
	}
	cl_screenshotusertag.SetValue(0);
...
void CVideoMode_Common::ApplySteamScreenshotTags(ScreenshotHandle hScreenshot)
{
	if(hScreenshot != INVALID_SCREENSHOT_HANDLE)
	{
		if(cl_screenshotusertag.GetBool())
		{
			...
			{
				CSteamID steamID(cl_screenshotusertag.GetInt(), Steam3Client().SteamUtils()->GetConnectedUniverse(),
...
void CVideoMode_Common::SetupStartupGraphic()
{
	...
	{
		// use the widescreen version
		Q_snprintf(material, sizeof(material), "materials/console/%s_widescreen.vtf", szBackgroundName);
	}
	else
	{
		Q_snprintf(material, sizeof(material), "materials/console/%s.vtf", szBackgroundName);
	}
	...
	if(!m_pBackgroundTexture)
	{
		...
		m_pBackgroundTexture = LoadVTF(buf, (aspectRatio >= 1.6f) ? "materials/console/background01_widescreen.vtf"
																  : "materials/console/background01.vtf");
		if(!m_pBackgroundTexture)
		{
	...
	// loading.vtf
	buf.Clear(); // added this Clear() because we saw cases where LoadVTF was not emptying the buf fully in the above
				 // section
...
void CVideoMode_Common::DrawStartupGraphic()
{
	...
	if(debugstartup)
	{
		for(int repeat = 0; repeat < 100000; repeat++)
		{
			...
			if(1) // draw normal BK
			{
				...
				DrawScreenSpaceRectangle(pMaterial, 0, 0 + slide, w, h - 50, 0, 0, tw - 1, th - 1, tw, th, NULL, 1, 1,
										 depth);
				DrawScreenSpaceRectangle(pLoadingMaterial, w - lw, h - lh + slide / 2, lw, lh, 0, 0, lw - 1, lh - 1, lw,
										 lh, NULL, 1, 1, depth - 0.1);
			}
			...
			if(0)
			{
				...
				for(int x = 0; x < grid_size; x++)
				{
					...
					for(int y = 0; y < grid_size; y++)
					{
						...
						// if (! ((x^y) & 1) )
						{
							DrawScreenSpaceRectangle(pMaterial, 10.0f + cornerx, 10.0f + cornery, 15, 15, 0, 0, tw - 1,
													 th - 1, tw, th, NULL, 1, 1, depthacc);
						}
	...
	else
	{
		...
		for(int i = 0; i < 2; ++i)
		{
			...
			DrawScreenSpaceRectangle(pLoadingMaterial, w - lw, h - lh, lw, lh, 0, 0, lw - 1, lh - 1, lw, lh, NULL, 1, 1,
									 depth);
			g_pMaterialSystem->SwapBuffers();
		}
	}
...
//-----------------------------------------------------------------------------
void CVideoMode_Common::BlitGraphicToHDCWithAlpha(HDC hdc, byte *rgba, int imageWidth, int imageHeight, int x0, int y0,
												  int x1, int y1)
{
#ifdef WIN32
	if(IsX360())
...
void CVideoMode_Common::DrawNullBackground(void *hHDC, int w, int h)
{
	...
	if(CommandLine()->FindParm("-noshaderapi"))
	{
		HFONT fnt = CreateFontA(-18, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_TT_PRECIS,
...
// Purpose: Blits an image to the loading window hdc
//-----------------------------------------------------------------------------
void CVideoMode_Common::BlitGraphicToHDC(HDC hdc, byte *rgba, int imageWidth, int imageHeight, int x0, int y0, int x1,
										 int y1)
{
	if(IsX360())
		return;
...
#define FIXED_BLEND(a, b, out, frac)                                                 \
	...
	for(int v = 0; v < tall; v++)
	{
		...
		for(int u = 0; u < dibwide; u++, dest += 3)
		{
			...
			byte *xsrc[4] = {srcTopLine + (iBilinearU + 0) * 4, srcTopLine + (iBilinearU + 1) * 4,
...
void CVideoMode_Common::AdjustWindow(int nWidth, int nHeight, int nBPP, bool bWindowed)
{
	...
	SetWindowPos((HWND)game->GetMainWindow(), hWndAfter, 0, 0, WindowRect.right - WindowRect.left,
...
#if defined(USE_SDL)
	g_pLauncherMgr->SetWindowFullScreen(!bWindowed, WindowRect.right - WindowRect.left,
...
void CVideoMode_Common::BlitHiLoScreenBuffersTo16Bit(void)
{
	if(IsX360())
	{
		// FIXME: this breaks in 480p due to (at least) the multisampled depth buffer (need to cache, clear and restore
		// the depth target)
		Assert(0);
		return;
	}
...
//-----------------------------------------------------------------------------
void CVideoMode_Common::TakeSnapshotPFMRect(const char *pFilename, int x, int y, int w, int h, int resampleWidth,
											int resampleHeight, CubeMapFaceIndex_t faceIndex)
{
	if(IsX360())
	{
		// FIXME: this breaks in 480p due to (at least) the multisampled depth buffer (need to cache, clear and restore
	...
	pRenderContext->DrawScreenSpaceRectangle(materials->FindMaterial("dev/copyfullframefb", ""), offsetX, offsetY,
	...
	ImageLoader::ConvertImageFormat((unsigned char *)pImage, IMAGE_FORMAT_RGBA16161616F, (unsigned char *)pImage1,
									IMAGE_FORMAT_RGB323232F, w, h);

	Assert(w == h); // garymcthack - this only works for square images

	float *pFloatImage =
		(float *)malloc(resampleWidth * resampleHeight * ImageLoader::SizeInBytes(IMAGE_FORMAT_RGB323232F));
...
// Takes a snapshot
//-----------------------------------------------------------------------------
void CVideoMode_Common::TakeSnapshotTGARect(const char *pFilename, int x, int y, int w, int h, int resampleWidth,
											int resampleHeight, bool bPFM, CubeMapFaceIndex_t faceIndex)
{
	if(IsX360())
	{
	...
	CUtlBuffer outBuf;
	if(TGAWriter::WriteToBuffer(pImage1, outBuf, resampleWidth, resampleHeight, IMAGE_FORMAT_RGBA8888,
...
// Output : static void
//-----------------------------------------------------------------------------
static void VID_ProcessMovieFrame(const MovieInfo_t &info, bool jpeg, const char *filename, int width, int height,
								  byte *data)
{
	CUtlBuffer outBuf;
	bool bSuccess = false;
...
void CVideoMode_Common::WriteMovieFrame(const MovieInfo_t &info)
{
	...
	if(info.DoTga())
	{
		VID_ProcessMovieFrame(info, false, va("%s%04d.tga", pMovieName, nMovieFrame), GetModeStereoWidth(),
							  GetModeStereoHeight(), (unsigned char *)hp);
	}

	if(info.DoJpg())
	{
		VID_ProcessMovieFrame(info, true, va("%s%04d.jpg", pMovieName, nMovieFrame), GetModeStereoWidth(),
							  GetModeStereoHeight(), (unsigned char *)hp);
	}
...
void CVideoMode_Common::TakeSnapshotJPEG(const char *pFilename, int quality)
{
	...
	Msg("Wrote '%s':  %s (%dx%d) compresssed (quality %i) to %s\n", pFilename, orig, GetModeStereoWidth(),
		GetModeStereoHeight(), quality, final);
		...
		for(int j = 0; j < m_nNumModes; j++)
		{
			if(info.m_Width == m_rgModeList[j].width && info.m_Height == m_rgModeList[j].height)
			{
...
#endif
				{
					...
					if(info.m_RefreshRate <= nDesktopRefresh && (m_rgModeList[j].refreshRate > nDesktopRefresh ||
																 m_rgModeList[j].refreshRate < info.m_RefreshRate))
					{
						m_rgModeList[j].refreshRate = info.m_RefreshRate;
					}
...
bool CVideoMode_MaterialSystem::SetMode(int nWidth, int nHeight, bool bWindowed)
{
	...
	config.SetFlag(MATSYS_VIDCFG_FLAGS_SCALE_TO_OUTPUT_RESOLUTION,
				   (DWORD)nWidth != videoMode.dwDisplayWidth || (DWORD)nHeight != videoMode.dwDisplayHeight);
...
void CVideoMode_MaterialSystem::ChangeDisplaySettingsToFullscreen(int nWidth, int nHeight, int nBPP)
{
	...
	if(EnumDisplaySettings(materials->GetDisplayDeviceName(), ENUM_CURRENT_SETTINGS, &dmCurrent) &&
	   dmCurrent.dmBitsPerPel == dm.dmBitsPerPel && dmCurrent.dmPelsWidth == dm.dmPelsWidth &&
...
void CVideoMode_MaterialSystem::ReadScreenPixels(int x, int y, int w, int h, void *pBuffer, ImageFormat format)
{
	if(!g_LostVideoMemory)
	{
		...
		pRenderContext->ReadPixelsAndStretch(&rect, &rect, (unsigned char *)pBuffer, format,
