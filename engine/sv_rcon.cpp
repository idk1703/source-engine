...
ConVar rcon_password("rcon_password", "", FCVAR_SERVER_CANNOT_QUERY | FCVAR_DONTRECORD, "remote console password.",
					 RconPasswordChanged_f);

ConVar sv_rcon_banpenalty("sv_rcon_banpenalty", "0", 0, "Number of minutes to ban users who fail rcon authentication",
						  true, 0, false, 0);
ConVar sv_rcon_maxfailures("sv_rcon_maxfailures", "10", 0,
						   "Max number of times a user can fail rcon authentication before being banned", true, 1, true,
						   20);
ConVar sv_rcon_minfailures(
	"sv_rcon_minfailures", "5", 0,
	"Number of times a user can fail rcon authentication in sv_rcon_minfailuretime before being banned", true, 1, true,
	20);
ConVar sv_rcon_minfailuretime("sv_rcon_minfailuretime", "30", 0,
							  "Number of seconds to track failed rcon authentications", true, 1, false, 0);
ConVar sv_rcon_whitelist_address("sv_rcon_whitelist_address", "", 0,
								 "When set, rcon failed authentications will never ban this address, e.g. '127.0.0.1'");

ConVar sv_rcon_maxpacketsize("sv_rcon_maxpacketsize", "1024", 0,
							 "The maximum number of bytes to allow in a command packet", true, 0, false, 0);
ConVar sv_rcon_maxpacketbans("sv_rcon_maxpacketbans", "1", 0,
...
void CRConServer::RunFrame()
{
	...
	for(int i = nCount - 1; i >= 0; --i)
	{
		...
		while(pData->m_OutstandingSends.Count() > 0)
		{
			...
			if(bSent) // all this packet was sent, remove it
			{
				pData->m_OutstandingSends.Remove(
					pData->m_OutstandingSends
						.Head()); // delete this entry no matter what, SendRCONResponse() will re-queue if needed
			}
		...
		if(readLen > sizeof(int)) // we have a command to process
		{
			...
			// Check and see if socket was closed as a result of processing - this can happen if the user has entered
			// too many passwords
...
void CRConServer::FinishRedirect(const char *msg, const netadr_t &adr)
{
	...
	for(int i = nCount - 1; i >= 0; --i)
	{
		...
		//		OutputDebugString( va("RCON: String is %i long\n", Q_strlen(msg)) ); // can't use DevMsg(), we are
		//potentially inside the RedirectFlush() function 		printf("RCON: String is %i long, packet size %i\n",
		//Q_strlen(msg), size );
...
bool CRConServer::SendRCONResponse(int nIndex, const void *data, int len, bool fromQueue)
{
	...
	while(sendLen < len)
	{
		...
		if(ret == -1)
		{
			...
			{
				pSocketData->m_OutstandingSends[pSocketData->m_OutstandingSends.Head()].SeekGet(
					CUtlBuffer::SEEK_CURRENT, sendLen);
			}
			return false;
		}
		else if(ret > 0)
	...
	//	OutputDebugString( va("RCON: Sending packet %i in len\n", len) ); // can't use DevMsg(), we are potentially
	//inside the RedirectFlush() function
	return true;
}
...
bool CRConServer::HandleFailedRconAuth(const netadr_t &adr)
{
	...
	//	ConMsg( "%d of %d bad password times tracked\n", failedRcon->badPasswordTimes.Count(),
	//failedRcon->badPasswordCount ); 	ConMsg( "min=%d, max=%d, time=%.2f\n", sv_rcon_minfailures.GetInt(),
