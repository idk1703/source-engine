		...
		if(data->buffer)
		{
			   ...
			   (compressedSize < data->bytes))
			{
				...
				DevMsg("Compressing fragments (%d -> %d bytes): %.2fms\n", data->bytes, compressedSize,
					   compressTimer.GetDuration().GetMillisecondsF());
...
==============
*/
void CNetChan::Setup(int sock, netadr_t *adr, const char *name, INetChannelHandler *handler, int nProtocolVersion)
{
	Assert(name);
	Assert(handler);
...
void CNetChan::FlowNewPacket(int flow, int seqnr, int acknr, int nChoked, int nDropped, int nSize)
{
	...
	if(seqnr > pflow->currentindex)
	{
		...
		for(int i = pflow->currentindex + 1, numPacketFramesOverflow = 0;
			(i <= seqnr) && (numPacketFramesOverflow < NET_FRAMES_BACKUP); ++i, ++numPacketFramesOverflow)
		{
...
bool CNetChan::SendSubChannelData(bf_write &buf)
{
	...
	for(i = 0; i < MAX_STREAMS; i++)
	{
		...
		bool bSingleBlock =
			(subChan->numFragments[i] == data->numFragments) && (data->file == FILESYSTEM_INVALID_HANDLE);
...
bool CNetChan::ReadSubChannelData(bf_read &buf, int stream)
{
	...
	else
	{
		if(data->buffer == NULL)
		{
			// This can occur if the packet containing the "header" (offset == 0) is dropped.  Since we need the header
			// to arrive we'll just wait
			//  for a retry
...
int CNetChan::SendDatagram(bf_write *datagram)
{
	...
	if(!IsX360() && m_StreamVoice.GetNumBitsWritten() > 0 && m_StreamVoice.GetNumBitsWritten() < send.GetNumBitsLeft())
	{
	...
	if(net_showudp.GetInt() && net_showudp.GetInt() != 2)
	{
		int mask = 63;
		char comp[64] = {0};
		if(net_compresspackets.GetBool() && bytesSent && (bytesSent < send.GetNumBytesWritten()))
		{
...
{
	return (m_StreamReliable.GetNumBitsWritten() > 0) || (m_WaitingList[FRAG_NORMAL_STREAM].Count() > 0) ||
...
{
	AssertMsg(m_nProtocolVersion >= 0 && m_nProtocolVersion <= PROTOCOL_VERSION,
...
bool CNetChan::IsValidFileForTransfer(const char *pszFilename)
{
	...
	if(V_stristr(szTemp, "lua/") || V_stristr(szTemp, "gamemodes/") || V_stristr(szTemp, "addons/") ||
	   V_stristr(szTemp, "~/") ||
	...
	if(V_stristr(szTemp, ".cfg") || V_stristr(szTemp, ".lst") || V_stristr(szTemp, ".exe") ||
	   V_stristr(szTemp, ".vbs") || V_stristr(szTemp, ".com") || V_stristr(szTemp, ".bat") ||
	   V_stristr(szTemp, ".cmd") || V_stristr(szTemp, ".dll") || V_stristr(szTemp, ".so") ||
	   V_stristr(szTemp, ".dylib") || V_stristr(szTemp, ".ini") || V_stristr(szTemp, ".log") ||
	   V_stristr(szTemp, ".lua") || V_stristr(szTemp, ".vdf") || V_stristr(szTemp, ".smx") ||
	   V_stristr(szTemp, ".gcf") || V_stristr(szTemp, ".lmp") || V_stristr(szTemp, ".sys"))
	{
		return false;
	}
	...
	if((extension_len != 3) && (extension_len != 4) && V_stricmp(extension, ".bsp.bz2") &&
	   V_stricmp(extension, ".xbox.vtx") && V_stricmp(extension, ".dx80.vtx") && V_stricmp(extension, ".dx90.vtx") &&
