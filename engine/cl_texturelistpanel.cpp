...
static ConVar mat_texture_list("mat_texture_list", "0", FCVAR_CHEAT,
...
ConVar mat_texture_list_all("mat_texture_list_all", "0", FCVAR_NEVER_AS_STRING | FCVAR_CHEAT,
							"If this is nonzero, then the texture list panel will show all currently-loaded textures.");

ConVar mat_texture_list_view(
	"mat_texture_list_view", "1", FCVAR_NEVER_AS_STRING | FCVAR_CHEAT,
	"If this is nonzero, then the texture list panel will render thumbnails of currently-loaded textures.");

ConVar mat_show_texture_memory_usage("mat_show_texture_memory_usage", "0", FCVAR_NEVER_AS_STRING | FCVAR_CHEAT,
...
CAutoMatSysDebugMode::CAutoMatSysDebugMode() : arrCleanupVars(DefLessFunc(IMaterialVar *))
{
...
static IMaterial *UseDebugMaterial(char const *szMaterial, ITexture *pMatTexture, CAutoMatSysDebugMode *pRestoreVars)
{
	...
	if(foundVar && FrameVar)
	{
		...
		if(pMatTexture->IsRenderTarget() || pMatTexture->IsProcedural())
			numAnimFrames = 0; // Cannot use the stencil parts of the render targets and shouldn't use the other frames
...
static void RenderTexturedRect(vgui::Panel *pPanel, IMaterial *pMaterial, int x, int y, int x1, int y1, int xoff = 0,
							   int yoff = 0)
{
...
CON_COMMAND_F(mat_texture_list_txlod,
			  "Adjust LOD of the last viewed texture +1 to inc resolution, -1 to dec resolution", FCVAR_DONTRECORD)
{
	if(args.ArgC() == 2)
	{
...
namespace MatViewOverride
{
	...
	{
		// Nothing selected
		if(!s_viewParamsLast.lstMaterials.Count() && !s_viewParamsReq.lstMaterials.Count())
			return;
		...
		else
		{
			...
			if(fCurTime >= s_viewParamsLast.flTime && fCurTime < s_viewParamsLast.flTime + 0.4)
				return;
...
void CRenderTextureEditor::SetDispInfo(KeyValues *kv, int iHint)
{
	...
	if(kv)
	{
		...
		for(MaterialHandle_t hm = materials->FirstMaterial(); hm != materials->InvalidMaterial();
			hm = materials->NextMaterial(hm))
		{
...
void CRenderTextureEditor::PerformLayout()
{
	...
	if(m_pInfo)
	{
		...
		Q_snprintf(chResolveNameArg, sizeof(chResolveNameArg) - 1, "materials/%s.vtf",
				   m_pInfo->GetString(KEYNAME_NAME));
		char const *szResolvedName =
			g_pFileSystem->RelativePathToFullPath(chResolveNameArg, "game", chResolveName, sizeof(chResolveName) - 1);
		if(szResolvedName)
		{
...
bool GetTextureContentPath(const char *pszVTF, char *szTextureContentPath, int nBufSize)
{
	...
	{
		szTextureContentPath = const_cast<char *>(
			g_pFileSystem->RelativePathToFullPath(vhRelVTF, "game", szTextureContentPath, nBufSize - 1));
...
void CRenderTextureEditor::OnCommand(const char *command)
{
	...
	if(!stricmp(command, "ToggleNoMip"))
	{
		...
		// 3. else error
		char *pExtPut = szFileName + strlen(szFileName) -
						strlen(".vtf"); // compensating the TEXTURE_FNAME_EXTENSION(.vtf) extension
...
void CRenderTextureEditor::Paint()
{
	...
	//
	{
		...
		g_pMatSystemSurface->DrawOutlinedRect(x + 1, y + 1, x + tileWidth - 2, y + tileHeight - 2);
	}
	...
	g_pMatSystemSurface->DrawColoredTextRect(GetFont(), x, y, TILE_SIZE, TILE_TEXT / 2, 255, 255, 255, 255,
											 "%s%s\n"
											 "%s Kb    %dx%d    %s",
	...
	{
		m_bufInfoText.Printf("%s%s\r\n"
							 "%s Kb    %dx%d    %s",
							 szPrintFilePrefix, szPrintFileName, chSizeBuf, iTxWidth, iTxHeight, szTxFormat);
	}
	...
	if(!kv->GetInt("SpecialTx"))
	{
		...
		{
			int wmap = pMatTexture->GetMappingWidth(), hmap = pMatTexture->GetMappingHeight(),
				dmap = pMatTexture->GetMappingDepth();
			int wact = pMatTexture->GetActualWidth(), hact = pMatTexture->GetActualHeight(),
			...
			{
				char chbuf[50];
				int mem = ImageLoader::GetMemRequired(max(min(wact, 4), wact / 2), max(min(hact, 4), hact / 2),
			...
			{
				char chbuf[50];
				int mem = ImageLoader::GetMemRequired(min(wmap, wact * 2), min(hmap, hact * 2), min(dmap, dact * 2),
													  fmt, true);
				mem = (mem + 511) / 1024;
				FmtCommaNumber(chbuf, mem);
		...
		if(chLine1[0])
		{
			...
			g_pMatSystemSurface->DrawFilledRect(x - TILE_BORDER / 2, y + TILE_TEXT / 2,
												x + TILE_BORDER / 2 + (TILE_SIZE * 0.5),
												y + TILE_TEXT / 2 + TILE_TEXT / 4);
			g_pMatSystemSurface->DrawColoredTextRect(GetFont(), x, y + TILE_TEXT / 2, TILE_SIZE, TILE_TEXT / 4, 255,
													 255, 255, 255, "%s", chLine1);
		}
		if(chLine2[0])
		{
			// g_pMatSystemSurface->DrawSetColor( 200, 0, 0, 255 );
			// g_pMatSystemSurface->DrawFilledRect( x - TILE_BORDER/2, y + TILE_TEXT/2 + TILE_TEXT/4, x + TILE_BORDER/2
			// + TILE_SIZE, y + TILE_TEXT );
			g_pMatSystemSurface->DrawColoredTextRect(GetFont(), x, y + TILE_TEXT / 2 + TILE_TEXT / 4, TILE_SIZE,
	...
	{
		g_pMatSystemSurface->DrawSetColor(255, 255, 255, 255);
		g_pMatSystemSurface->DrawOutlinedRect(x + orgTxX + (extTxWidth - iDrawWidth) / 2 - IMG_FRAME_OFF,
											  y + orgTxY + (extTxHeight - iDrawHeight) / 2 - IMG_FRAME_OFF,
											  x + orgTxX + (extTxWidth + iDrawWidth) / 2 + IMG_FRAME_OFF,
											  y + orgTxY + (extTxHeight + iDrawHeight) / 2 + IMG_FRAME_OFF);
		RenderTexturedRect(this, pMaterial, x + orgTxX + (extTxWidth - iDrawWidth) / 2,
						   y + orgTxY + (extTxHeight - iDrawHeight) / 2, x + orgTxX + (extTxWidth + iDrawWidth) / 2,
		...
		if(bHasAlpha)
		{
			orgTxX += orgTxXA;
			orgTxY += orgTxYA;
			if(IMaterial *pMaterialDebug =
				   UseDebugMaterial("debug/debugtexturealpha", pMatTexture, &auto_matsysdebugmode))
			{
				g_pMatSystemSurface->DrawOutlinedRect(x + orgTxX + (extTxWidth - iDrawWidth) / 2 - IMG_FRAME_OFF,
													  y + orgTxY + (extTxHeight - iDrawHeight) / 2 - IMG_FRAME_OFF,
													  x + orgTxX + (extTxWidth + iDrawWidth) / 2 + IMG_FRAME_OFF,
													  y + orgTxY + (extTxHeight + iDrawHeight) / 2 + IMG_FRAME_OFF);
				RenderTexturedRect(this, pMaterialDebug, x + orgTxX + (extTxWidth - iDrawWidth) / 2,
								   y + orgTxY + (extTxHeight - iDrawHeight) / 2,
...
public:
	...
	CRenderTextureEditor *GetRenderTxEditor() const
	{
		return m_pRenderTxEditor;
	}
...
CRenderTexturesListViewPanel::CRenderTexturesListViewPanel(vgui::Panel *parent, char const *szName)
	: vgui::TileViewPanelEx(parent, szName), m_pListPanel(NULL), m_bPaintAlpha(false)
{
...
void CRenderTexturesListViewPanel::RenderTile(int iTile, int x, int y)
{
	...
	sprintf(chInfoText, "%s Kb  %dx%d  %.*s%s  %s", chSizeBuf, iTxWidth, iTxHeight, iTxFormatLen, szTxFormat,
			szTxFormatSuffix,
			(pMatTexture->GetFlags() & (TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_ONEBITALPHA)) ? "***"
																											 : "");
	...
	if(bWarnTile)
	{
		...
		if(iTxWidth > g_warn_texdimensions || iTxHeight > g_warn_texdimensions)
			g_pMatSystemSurface->DrawFilledRect(x + iTextMargins[0] - 2, y + iTextHeight + 1, x + iTextMargins[1] - 1,
												y + TILE_TEXT);
		if(strcmp(szTxFormat, "DXT1") && strcmp(szTxFormat, "DXT5"))
			g_pMatSystemSurface->DrawFilledRect(x + iTextMargins[1] + 2, y + iTextHeight + 1, x + iTextMargins[2] - 1,
												y + TILE_TEXT);
		if(pMatTexture->GetFlags() & (TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_ONEBITALPHA))
			g_pMatSystemSurface->DrawFilledRect(x + iTextMargins[2] + 3, y + iTextHeight + 1, x + iTextMargins[3] + 2,
												y + TILE_TEXT);
	}

	g_pMatSystemSurface->DrawColoredTextRect(GetFont(), x, y, TILE_SIZE, TILE_TEXT, 255, 255, 255, 255,
											 "%s%s\n"
											 "%s",
...
static int __cdecl KilobytesSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1,
	...
	: BaseClass(parent, "CTextureListPanel"), m_numDisplayedSizeKB(0)
{
	...
	// Init the buttons.
	m_pCVarListLabel = vgui::SETUP_PANEL(new vgui::Label(
		this, "m_pCVarListLabel",
	...
	m_pShowTextureMemoryUsageOption =
		vgui::SETUP_PANEL(new CConVarCheckButton(this, "m_pShowTextureMemoryUsageOption", "Show Memory Usage on HUD"));
	...
	m_pReloadAllMaterialsButton =
		vgui::SETUP_PANEL(new vgui::Button(this, "ReloadAllMaterials", "Reload All Materials"));
	...
	int col = -1;
	m_pListPanel->AddColumnHeader(++col, KEYNAME_NAME, "Texture Name", 200, 100, 700,
...
void CTextureListPanel::PerformLayout()
{
	...
	vgui::Panel *buttons[] = {
		m_pSpecialTexs, m_pShowTextureMemoryUsageOption, m_pAllTextures,		  m_pViewTextures, m_pFilteringChk,
		m_pHideMipped,	m_pResolveTexturePath,			 m_pCopyToClipboardButton};
...
void KeepSpecialKeys(KeyValues *textureList, bool bServiceKeys)
{
	...
	for(KeyValues *pCur = textureList->GetFirstSubKey(); pCur; pCur = pNext)
	{
		...
		if(StringHasPrefix(szName, "_") || StringHasPrefix(szName, "[") || !stricmp(szName, "backbuffer") ||
		   StringHasPrefix(szName, "colorcorrection") || !stricmp(szName, "depthbuffer") ||
		   !stricmp(szName, "frontbuffer") || !stricmp(szName, "normalize") || !*szName)
		{
			bIsServiceKey = true;
		}
...
void CTextureListPanel::UpdateTotalUsageLabel()
{
	...
	FmtCommaNumber(
		kb1,
		(g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(IDebugTextureInfo::MEMORY_BOUND_LAST_FRAME) + 511) /
			1024);
	FmtCommaNumber(
		kb2,
		(g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(IDebugTextureInfo::MEMORY_TOTAL_LOADED) + 511) / 1024);
	...
	else
	{
		...
		FmtCommaNumber(kbMip1, (g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(
									IDebugTextureInfo::MEMORY_ESTIMATE_PICMIP_1) +
								511) /
								   1024);
		FmtCommaNumber(kbMip2, (g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(
									IDebugTextureInfo::MEMORY_ESTIMATE_PICMIP_2) +
								511) /
								   1024);
		Q_snprintf(data, sizeof(data),
				   "%s:  frame %s Kb  /  total %s Kb ( picmip1 = %s Kb, picmip2 = %s Kb )  /  shown %s Kb", szTitle,
				   kb1, kb2, kbMip1, kbMip2, kb3);
	}
...
void CTextureListPanel::Paint()
{
		...
		if(kv && iHint)
		{
			...
			if(plv && !strcmp(plv->GetString(KEYNAME_NAME), kv->GetString(KEYNAME_NAME)))
			{
				...
				for(; pValData && pValRendered;
					pValData = pValData->GetNextValue(), pValRendered = pValRendered->GetNextValue())
				{
	...
	for(KeyValues *pCur = textureList.Get()->GetFirstSubKey(); pCur; pCur = pCur->GetNextKey())
	{
		...
		if(m_pResolveTexturePath->IsSelected())
		{
			char chResolveName[256] = {0}, chResolveNameArg[256] = {0};
			Q_snprintf(chResolveNameArg, sizeof(chResolveNameArg) - 1, "materials/%s.vtf",
					   pCur->GetString(KEYNAME_NAME));
			char const *szResolvedName = g_pFileSystem->RelativePathToFullPath(chResolveNameArg, "game", chResolveName,
...
void VGui_UpdateTextureListPanel()
{
	if(mat_show_texture_memory_usage.GetInt())
	{
		...
		char kb1[20], kb2[20];
		FmtCommaNumber(
			kb1, (g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(IDebugTextureInfo::MEMORY_BOUND_LAST_FRAME) +
				  511) /
					 1024);
		FmtCommaNumber(
			kb2,
			(g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(IDebugTextureInfo::MEMORY_TOTAL_LOADED) + 511) /
				1024);
