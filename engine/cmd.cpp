...
#ifdef _DEBUG
ConVar cl_debug_respect_cheat_vars("cl_debug_respect_cheat_vars", "0", 0,
...
int g_iFilterCommandsByServerCanExecute =
	0; // If this is nonzero, then they want us to only run commands marked with FCVAR_SERVER_CAN_EXECUTE.
int g_iFilterCommandsByClientCmdCanExecute =
	0; // If this is nonzero, then they want us to only run commands marked with FCVAR_CLIENTCMD_CAN_EXECUTE.
...
CON_COMMAND_F(PerfMark, "inserts a telemetry marker into the stream. If args are provided, they will be included.",
			  FCVAR_NONE)
{
...
bool Cbuf_AddTextWithMarkers(ECmdExecutionMarker markerLeft, const char *text, ECmdExecutionMarker markerRight)
{
	...
	bool bSuccess = Cbuf_AddExecutionMarker(markerLeft, szMarkerLeft) && s_CommandBuffer.AddText(text) &&
...
static char const *Cmd_TranslateFileAssociation(char const *param)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		if(!Q_strcmp(extension, info.extension + 1) && !CommandLine()->FindParm(va("+%s", info.command_to_issue)))
		{
...
bool IsValidFileExtension(const char *pszFilename)
{
	...
	if(Q_strstr(pszFilename, ".exe") || Q_strstr(pszFilename, ".vbs") || Q_strstr(pszFilename, ".com") ||
	   Q_strstr(pszFilename, ".bat") || Q_strstr(pszFilename, ".dll") || Q_strstr(pszFilename, ".ini") ||
	   Q_strstr(pszFilename, ".gcf") || Q_strstr(pszFilename, ".sys") || Q_strstr(pszFilename, ".blob"))
	{
		return false;
	}
...
void Cmd_Exec_f(const CCommand &args)
{
	...
	if(IsPC())
	{
		...
		else
		{
			...
			if(!V_stristr(szFile, "autoexec.cfg") && !V_stristr(szFile, "joystick.cfg") &&
			   !V_stristr(szFile, "game.cfg"))
			{
...
// Users can alias these commands to remove their functionality because the game systems use convars to implement these
// features The blacklist prevents that exploit
...
static bool ShouldPreventServerCommand(const ConCommandBase *pCommand)
{
	if(!Host_IsSinglePlayerGame())
	{
		...
		{
			// If we don't understand the command, and it came from a server command, then forward it back to the
			// server. Lots of server plugins use this.  They use engine->ClientCommand() to have a client execute a
			// command that they have hooked on the server. We disabled it once and they freaked. It IS redundant since
			// they could just code the call to their command on the server, but they complained loudly enough that
			// we're adding it back in since there's no exploit that we know of by allowing it.
			if(!pCommand)
				return false;
...
{
	if(g_iFilterCommandsByClientCmdCanExecute > 0 && !pCommand->IsFlagSet(FCVAR_CLIENTCMD_CAN_EXECUTE) &&
...
const ConCommandBase *Cmd_ExecuteCommand(const CCommand &command, cmd_source_t src, int nClientSlot)
{
	...
	if(pCommand && pCommand->IsCommand())
	{
		if(!ShouldPreventClientCommand(pCommand) && pCommand->IsCommand())
		{
			...
			if(pCommand->IsFlagSet(FCVAR_CHEAT))
			{
				...
				{
					// But.. if the server is allowed to run this command and the server DID run this command, then let
					// it through. (used by soundscape_flush)
	...
	if(pCommand && src == src_command && CommandLine()->CheckParm("-default") &&
	   !pCommand->IsFlagSet(FCVAR_EXEC_DESPITE_DEFAULT))
	{
