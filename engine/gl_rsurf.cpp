...
void Shader_BrushEnd(IMatRenderContext *pRenderContext, VMatrix const *brushToWorld, model_t *model, bool bShadowDepth,
...
static ConVar r_fastzreject("r_fastzreject", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
							"Activate/deactivates a fast z-setting algorithm to take advantage of hardware with fast z "
							"reject. Use -1 to default to hardware settings");
...
void Shader_TranslucentDisplacementSurface(CWorldRenderList *pRenderList, SurfaceHandle_t surfID)
{
	...
	if(MSurf_Flags(surfID) & (SURFDRAW_HASLIGHTSYTLES | SURFDRAW_HASDLIGHT))
	{
		...
		if(!DlightSurfaceSetQueuingFlag(surfID))
		{
			pRenderList->m_DispAlphaSortList.AddSurfaceToTail(surfID, sortGroup,
															  pRenderList->m_VisibleLeaves.Count() - 1);
		}
	}
	else
	{
...
//-----------------------------------------------------------------------------
void Shader_SetChainTextureState(IMatRenderContext *pRenderContext, SurfaceHandle_t surfID, IClientEntity *pBaseEntity,
								 bool bShadowDepth)
{
	...
	if(bShadowDepth)
	{
		...
		if(nAlphaTest == 1)
		{
			...
			IMaterialVar *pOriginalAlphaRefVar =
				pDrawMaterial->FindVarFast("$AlphaTestReference", &originalAlphaRefCache);
	...
	while(listIndex < count)
	{
		...
		for(int m = 0; m < meshTotal; m++)
		{
			...
			IMaterial *pBindMaterial =
				materialSortInfoArray[MSurf_MaterialSortID(batchList[mesh.firstbatch].surfID)].material;
#ifdef NEWMESH
				...
				if(bShadowDepth)
				{
					...
					if(nAlphaTest == 1)
					{
						...
						IMaterialVar *pAlphaRefVar =
							pDepthWriteMaterial->FindVarFast("$AlphaTestReference", &alphaRefCache);
				...
				else
				{
					...
					else
					{
						pRenderContext->BindLightmapPage(
							materialSortInfoArray[MSurf_MaterialSortID(batch.surfID)].lightmapPageID);
					}
				}
#ifdef NEWMESH
...
static void Shader_DrawChainNormals(const CUtlVector<msurface2_t *> &surfaceList)
{
	...
	for(int i = 0; i < surfaceList.Count(); i++)
	{
		...
		for(vertID = 0; vertID < MSurf_VertCount(surfID); ++vertID)
		{
			...
			Vector &norm =
				pBrushData->vertnormals[pBrushData->vertnormalindices[MSurf_FirstVertNormal(surfID) + vertID]];
...
static void Shader_DrawChainBumpBasis(const CUtlVector<msurface2_t *> &surfaceList)
{
	...
	for(int i = 0; i < surfaceList.Count(); i++)
	{
		...
		for(vertID = 0; vertID < MSurf_VertCount(surfID); ++vertID)
		{
			...
			for(int j = 0; j < 3; j++)
			{
				worldSpaceBumpBasis[j][0] = g_localBumpBasis[j][0] * tangentS[0] +
											g_localBumpBasis[j][1] * tangentS[1] + g_localBumpBasis[j][2] * tangentS[2];
				worldSpaceBumpBasis[j][1] = g_localBumpBasis[j][0] * tangentT[0] +
											g_localBumpBasis[j][1] * tangentT[1] + g_localBumpBasis[j][2] * tangentT[2];
				worldSpaceBumpBasis[j][2] = g_localBumpBasis[j][0] * norm[0] + g_localBumpBasis[j][1] * norm[1] +
...
static void DrawDebugInformation(const CUtlVector<msurface2_t *> &surfaceList)
{
	...
	if(g_ShaderDebug.surfaceid)
	{
		...
		Shader_DrawSurfaceDebuggingInfo(surfaceList,
										(g_ShaderDebug.surfaceid != 2) ? DrawSurfaceID : DrawSurfaceIDAsInt);
	}
...
//-----------------------------------------------------------------------------
void Shader_DrawDispChain(int nSortGroup, const CMSurfaceSortList &list, unsigned int flags,
						  ERenderDepthMode DepthMode)
{
	...
	if(count)
	{
		...
		MSL_FOREACH_GROUP_BEGIN(list, nSortGroup,
								group) {MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(list, group, surfID) {pList[i] = surfID;
		++i;
	}
	MSL_FOREACH_SURFACE_IN_GROUP_END()
}
...
// Performs the z-fill
//-----------------------------------------------------------------------------
static void Shader_WorldZFillSurfChain(const CMSurfaceSortList &sortList, const surfacesortgroup_t &group,
									   CMeshBuilder &meshBuilder, int &nStartVertIn, unsigned int includeFlags)
{
...
//-----------------------------------------------------------------------------
static void Shader_WorldShadowDepthFill(CWorldRenderList *pRenderList, unsigned int flags)
{
	tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "%s", __FUNCTION__);
...
// Performs the z-fill
//-----------------------------------------------------------------------------
static void Shader_WorldZFill(CWorldRenderList *pRenderList, unsigned int flags)
{
	tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "%s", __FUNCTION__);
...
//-----------------------------------------------------------------------------
static void Shader_WorldEnd(CWorldRenderList *pRenderList, unsigned int flags, float waterZAdjust)
{
...
//-----------------------------------------------------------------------------
bool Shader_LeafContainsTranslucentSurfaces(IWorldRenderList *pRenderListIn, int sortIndex, unsigned int flags)
{
...
void Shader_DrawTranslucentSurfaces(IWorldRenderList *pRenderListIn, int sortIndex, unsigned int flags,
									bool bShadowDepth)
{
	...
	for(i = 0; i < MAX_MAT_SORT_GROUPS; ++i)
	{
		...
		for(int listIndex = surfaceList.Count(); --listIndex >= 0;)
		{
			...
			bool bFlashlightMask =
				!((flags & DRAWWORLDLISTS_DRAW_REFRACTION) || (flags & DRAWWORLDLISTS_DRAW_REFLECTION));
		...
		DispInfo_RenderList(i, surfaceList.Base(), surfaceList.Count(), g_EngineRenderer->ViewGetCurrent().m_bOrtho,
							flags, DEPTH_MODE_NORMAL);
	}
}
...
static void FASTCALL R_DrawLeaf(CWorldRenderList *pRenderList, mleaf_t *pleaf)
{
	...
	for(; i < pleaf->nummarksurfaces; i++)
	{
		...
		{
			if((DotProduct(MSurf_Plane(surfID).normal, modelorg) - MSurf_Plane(surfID).dist) < BACKFACE_EPSILON)
				continue;
		}
...
static inline bool R_CullNodeTopView(mnode_t *pNode)
{
	...
	return (FloatMakePositive(delta.x) > size.x) || (FloatMakePositive(delta.y) > size.y);
}
...
//-----------------------------------------------------------------------------
void R_BuildWorldLists(IWorldRenderList *pRenderListIn, WorldListInfo_t *pInfo, int iForceViewLeaf,
					   const VisOverrideData_t *pVisData, bool bShadowDepth /* = false */,
	...
	if(!r_drawtopview)
	{
		...
		if(bShadowDepth)
		{
			R_RecursiveWorldNodeNoCull(pRenderList, host_state.worldbrush->nodes,
									   r_frustumcullworld.GetBool() ? FRUSTUM_CLIP_ALL : FRUSTUM_SUPPRESS_CLIPPING);
		}
		else
		{
...
//-----------------------------------------------------------------------------
void CVisibleFogVolumeQuery::FindVisibleFogVolume(const Vector &vecViewPoint, int *pVisibleFogVolume,
...
void R_DrawWorldLists(IWorldRenderList *pRenderListIn, unsigned int flags, float waterZAdjust)
{
	CWorldRenderList *pRenderList = assert_cast<CWorldRenderList *>(pRenderListIn);
...
bool Shader_DrawBrushSurfaceOverride(IMatRenderContext *pRenderContext, SurfaceHandle_t surfID,
...
public:
	...
	// Surfaces are stored in a list like this temporarily for sorting purposes only.  The compact structs do not store
	// these.
...
#ifdef NEWMESH
				IIndexBuffer *pBuildIndexBuffer =
					pRenderContext->GetDynamicIndexBuffer(MATERIAL_INDEX_FORMAT_16BIT, false);
				indexBufferBuilder.Begin(pBuildIndexBuffer, batch.indexCount);
#else
				IMesh *pBuildMesh =
					pRenderContext->GetDynamicMesh(false, g_WorldStaticMeshes[batch.sortID], NULL, NULL);
	...
	void DrawOpaqueBrushModel(IClientEntity *baseentity, model_t *model, const Vector &origin,
							  ERenderDepthMode DepthMode);
	void DrawTranslucentBrushModel(IClientEntity *baseentity, model_t *model, const Vector &origin, bool bShadowDepth,
								   bool bDrawOpaque, bool bDrawTranslucent);
...
//-----------------------------------------------------------------------------
void CBrushBatchRender::DrawOpaqueBrushModel(IClientEntity *baseentity, model_t *model, const Vector &origin,
											 ERenderDepthMode DepthMode)
{
	...
	for(i = 0; i < pRender->planeCount; i++)
	{
		...
		backface[i] = (DepthMode == DEPTH_MODE_NORMAL && (dot < BACKFACE_EPSILON))
						  ? true
		...
		for(int j = 0; j < mesh.batchCount; j++)
		{
			...
			if(DepthMode != DEPTH_MODE_NORMAL)
			{
				...
				if(nAlphaTest == 1)
				{
					static unsigned int originalTextureVarCache = 0;
					IMaterialVar *pOriginalTextureVar =
						batch.pMaterial->FindVarFast("$basetexture", &originalTextureVarCache);
					static unsigned int originalTextureFrameVarCache = 0;
					IMaterialVar *pOriginalTextureFrameVar =
						batch.pMaterial->FindVarFast("$frame", &originalTextureFrameVarCache);
					static unsigned int originalAlphaRefCache = 0;
					IMaterialVar *pOriginalAlphaRefVar =
						batch.pMaterial->FindVarFast("$AlphaTestReference", &originalAlphaRefCache);
			...
			pRenderContext->BindVertexBuffer(0, g_WorldStaticMeshes[batch.sortID], 0,
...
//-----------------------------------------------------------------------------
void CBrushBatchRender::DrawTranslucentBrushModel(IClientEntity *baseentity, model_t *model, const Vector &origin,
...
int R_MarkDlightsOnBrushModel(model_t *model, IClientRenderable *pRenderable)
{
	...
	if(g_bActiveDlights)
	{
		...
		{
			if((cl_dlights[k].die < cl.GetTime()) || (!cl_dlights[k].IsRadiusGreaterThanZero()))
				continue;
			...
			if(IsBoxIntersectingSphereExtents(node->m_vecCenter, node->m_vecHalfDiagonal, cl_dlights[k].origin,
...
void Shader_BrushEnd(IMatRenderContext *pRenderContext, VMatrix const *pBrushToWorld, model_t *model, bool bShadowDepth,
					 IClientEntity *baseentity /* = NULL */)
{
	if(bShadowDepth)
		return;
...
//-----------------------------------------------------------------------------
void R_DrawBrushModel(IClientEntity *baseentity, model_t *model, const Vector &origin, const QAngle &angles,
					  ERenderDepthMode DepthMode, bool bDrawOpaque, bool bDrawTranslucent)
{
	...
	else
	{
		if(model->flags & MODELFLAG_TRANSLUCENT)
		{
			if(DepthMode == DEPTH_MODE_NORMAL)
			{
				g_BrushBatchRenderer.DrawTranslucentBrushModel(baseentity, model, origin, false, bDrawOpaque,
															   bDrawTranslucent);
			}
		}
		else if(bDrawOpaque)
	...
	Shader_BrushEnd(pRenderContext, brushTransform.GetNonIdentityMatrix(), model, DepthMode != DEPTH_MODE_NORMAL,
					baseentity);
...
static bool EnumerateLeafInBox_R(mnode_t *node, EnumLeafBoxInfo_t &info)
{
	...
	// rough cull...
	if(!IsBoxIntersectingBoxExtents(node->m_vecCenter, node->m_vecHalfDiagonal, info.m_vecBoxCenter,
...
static bool EnumerateLeafInBox_R(mnode_t *RESTRICT node, const EnumLeafBoxInfo_t *RESTRICT pInfo)
{
	...
	Assert(IsBoxIntersectingBoxExtents(AlignThatVector(node->m_vecCenter), AlignThatVector(node->m_vecHalfDiagonal),
									   LoadAlignedSIMD(pInfo->m_vecBoxCenter),
									   LoadAlignedSIMD(pInfo->m_vecBoxHalfDiagonal)) ==
		   IsBoxIntersectingBoxExtents((node->m_vecCenter), node->m_vecHalfDiagonal, pInfo->m_vecBoxCenter,
									   pInfo->m_vecBoxHalfDiagonal));
	...
	if(plane->type <= PLANE_Z)
	{
		...
		{
			// Here the box is split by the node
			return EnumerateLeafInBox_R(node->children[0], pInfo) && EnumerateLeafInBox_R(node->children[1], pInfo);
		}
	}
	...
	*/
	...
	cornermin =
		XMVectorSelect(vecBoxMax, vecBoxMin, control); // cornermin[i] = control[i] ? vecBoxMin[i] : vecBoxMax[i]
...
bool EnumerateLeafInSphere_R(mnode_t *node, EnumLeafSphereInfo_t &info, int nTestFlags)
{
	while(true)
	{
		...
		if(node->contents >= 0)
		{
			...
			if(nTestFlags)
			{
				if(!IsBoxIntersectingSphereExtents(node->m_vecCenter, node->m_vecHalfDiagonal, info.m_vecCenter,
												   info.m_flRadius))
					return true;
			}
...
static bool EnumerateLeavesAlongRay_R(mnode_t *node, Ray_t const &ray, float start, float end,
...
static bool EnumerateLeavesAlongExtrudedRay_R(mnode_t *node, Ray_t const &ray, float start, float end,
											  ISpatialLeafEnumerator *pEnum, int context)
{
	// no polygons in solid nodes (don't report these leaves either)
	if(node->contents == CONTENTS_SOLID)
	...
	else
	{
		startDotN = DotProduct(ray.m_Start, plane->normal);
		deltaDotN = DotProduct(ray.m_Delta, plane->normal);
		offset = fabs(ray.m_Extents[0] * plane->normal[0]) + fabs(ray.m_Extents[1] * plane->normal[1]) +
	...
	//	if (t1 >= offset && t2 >= offset)
	{
		return EnumerateLeavesAlongExtrudedRay_R(node->children[0], ray, start, end, pEnum, context);
	}
	if(t1 < -offset && t2 < -offset)
	{
		return EnumerateLeavesAlongExtrudedRay_R(node->children[1], ray, start, end, pEnum, context);
	}
	...
	if(fabs(t1 - t2) < DIST_EPSILON)
	{
		...
		bool ret = EnumerateLeavesAlongExtrudedRay_R(node->children[0], ray, start, end, pEnum, context);
		if(!ret)
			return false;
		return EnumerateLeavesAlongExtrudedRay_R(node->children[1], ray, start, end, pEnum, context);
	}
...
bool CEngineBSPTree::EnumerateLeavesAtPoint(const Vector &pt, ISpatialLeafEnumerator *pEnum, int context)
{
	int leaf = CM_PointLeafnum(pt);
	return pEnum->EnumerateLeaf(leaf, context);
}

static ConVar opt_EnumerateLeavesFastAlgorithm("opt_EnumerateLeavesFastAlgorithm", "1", FCVAR_NONE,
											   "Use the new SIMD version of CEngineBSPTree::EnumerateLeavesInBox.");

bool CEngineBSPTree::EnumerateLeavesInBox(const Vector &mins, const Vector &maxs, ISpatialLeafEnumerator *pEnum,
										  int context)
{
...
bool CEngineBSPTree::EnumerateLeavesInSphere(const Vector &center, float radius, ISpatialLeafEnumerator *pEnum,
											 int context)
{
