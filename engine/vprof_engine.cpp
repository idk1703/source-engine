...
static ConVar vprof_dump_spikes_node("vprof_dump_spikes_node", "", 0,
									 "Node to start report from when doing a dump spikes");
static ConVar vprof_dump_spikes_budget_group("vprof_dump_spikes_budget_group", "", 0,
...
static ConVar vprof_report_oninterval("vprof_report_oninterval", "0", 0,
									  "Interval (in seconds) at which vprof will batch up a full report to the console "
									  "-- more detailed than vprof_dump_oninterval.");
...
void PreUpdateProfile(float filteredtime)
{
	...
	if(g_VProfCurrentProfile.IsEnabled())
	{
		...
		if((s_flIntervalStartTime + flIntervalTime) < flCurrentTime)
		{
			...
			else if(vprof_dump_oninterval.GetFloat() != 0.0f)
			{
				...
				g_VProfCurrentProfile.OutputReport(VPRT_SUMMARY | VPRT_LIST_BY_TIME | VPRT_LIST_BY_AVG_TIME |
	...
	if(g_VProfCurrentProfile.IsEnabled() && vprof_dump_spikes.GetFloat())
	{
		...
		{
			if(g_VProfSignalSpike || (Sys_FloatTime() - LastSpikeTime > MAX_SPIKE_REPORT &&
									  g_ServerGlobalVariables.framecount > LastSpikeFrame + MAX_SPIKE_REPORT_FRAMES))
			{
				ConsoleLogger consoleLog;
				...
				g_VProfCurrentProfile.OutputReport(
					VPRT_SUMMARY | VPRT_LIST_BY_TIME | VPRT_LIST_BY_TIME_LESS_CHILDREN | VPRT_LIST_TOP_ITEMS_ONLY,
...
CON_COMMAND(budget_toggle_group, "Turn a budget group on/off")
{
	...
	g_VProfCurrentProfile.HideBudgetGroup(
		budgetGroup, !(g_VProfCurrentProfile.GetBudgetGroupFlags(budgetGroup) & BUDGETFLAG_HIDDEN));
}
...
DEFERRED_CON_COMMAND(vprof_generate_report, "Generate a report to the console.")
{
	...
	g_VProfCurrentProfile.OutputReport(VPRT_LIST_BY_TIME | VPRT_LIST_BY_TIME_LESS_CHILDREN,
...
DEFERRED_CON_COMMAND(vprof_generate_report_AI_only, "Generate a report to the console.")
{
	...
	ConsoleLogger consoleLog;
	g_VProfCurrentProfile.OutputReport((VPRT_FULL & ~VPRT_HIERARCHY), "NPCs",
									   g_VProfCurrentProfile.BudgetGroupNameToBudgetGroupID(VPROF_BUDGETGROUP_NPCS));
	g_VProfCurrentProfile.Resume();
}
...
DEFERRED_CON_COMMAND(vprof_360_disable_counters,
					 "Disable 360 L2 and LHS counters for a node. Specify 'all' to mean all nodes.")
{
	g_VProfCurrentProfile.Pause();
...
DEFERRED_CON_COMMAND(vprof_360_disable_counters_recursive,
					 "Disable 360 L2 and LHS counters for a node and all children.")
{
	g_VProfCurrentProfile.Pause();
...
DEFERRED_CON_COMMAND(vprof_360_report_counters, "Report L2/LHS info for specified node")
{
	...
	if(pNode)
	{
		Msg("NODE %s\n\tL2 misses: %d\n\tLHS misses: %d\n", g_szDefferedArg1, pNode->GetL2CacheMisses(),
			pNode->GetLoadHitStores());
	}
	else
	{
...
DEFERRED_CON_COMMAND(vprof_360_cpu_trace_enable, "Enable CPU tracing: it will begin when specified node starts, and "
												 "end when it stops. Do this before calling vprof_360_cpu_trace_go.")
{
	CVProfNode *RESTRICT upon = g_VProfCurrentProfile.CPUTraceEnableForNode(g_szDefferedArg1);
	if(upon != NULL)
	{
		Msg("%s will be traced from start to end. Make sure vprof is enabled, and enter \nvprof_360_cpu_trace_go "
			"<filename> to engage!\n",
			upon->GetName());
	}
	else
	{
		Msg("Could not find node %s. Maybe you need to run vprof for a bit so I can know about that node? Or, you "
			"might need to wrap it in \"double-quotes\". \n",
			g_szDefferedArg1);
	}
}
...
DEFERRED_CON_COMMAND(vprof_360_cpu_trace_go,
					 "syntax: vprof_360_cpu_trace_go <filename>. Will record one CPU trace of the node specified in "
					 "vprof_360_cpu_trace_enable, dumping it to e:/filename.pix2.")
{
	if(!g_fVprofOnByUI)
	{
		Msg("VProf enabled.\n");
	...
	if(g_VProfCurrentProfile.CPUTraceGetEnabledNode() == NULL ||
	   g_VProfCurrentProfile.CPUTraceGetEnabledNode() == g_VProfCurrentProfile.GetRoot())
	{
	...
	if(g_VProfCurrentProfile.CPUTraceGetEnabledNode() != NULL)
	{
		if(!g_szDefferedArg1[0])
		{
			...
			V_snprintf(g_szDefferedArg1, ARRAYSIZE(g_szDefferedArg1), "vprof_%d_%d_%d_%d_%d_%d", systemTime.wMonth,
...
DEFERRED_CON_COMMAND(
	vprof_360_cpu_trace_go_repeat,
	"syntax: vprof_360_cpu_trace_go_repeat <filename>. For each time the node specified in vprof_360_cpu_trace_enable "
	"is hit during the next frame, dump a CPU trace to e:/filenameXXXX.pix2.")
{
	if(g_VProfCurrentProfile.CPUTraceGetEnabledNode() != NULL)
	{
		const char *filename = g_VProfCurrentProfile.SetCPUTraceFilename(g_szDefferedArg1);
