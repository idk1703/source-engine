...
{
public:
	CPropCullStack(CSendTablePrecalc *pPrecalc, int iClient, const CSendProxyRecipients *pOldStateProxies,
				   const int nOldStateProxies, const CSendProxyRecipients *pNewStateProxies, const int nNewStateProxies)
		:
...
public:
	CEncodeInfo(CSendTablePrecalc *pPrecalc, unsigned char *pStructBase, int objectID, bf_write *pOut)
		: CServerDatatableStack(pPrecalc, pStructBase, objectID), m_DeltaBitsWriter(pOut)
	{
	}
...
static inline void ShowEncodeDeltaWatchInfo(const SendTable *pTable, const SendProp *pProp, bf_read &buffer,
											const int objectID, const int index)
{
	...
	ConDMsg("+ %s %s, %s, index %i, bits %i, value %s\n", pTable->GetName(), pProp->GetName(), type, index, bits,
			value);
}
...
static FORCEINLINE void SendTable_EncodeProp(CEncodeInfo *pInfo, unsigned int iProp)
{
	...
	pProp->GetProxyFn()(pProp, pStructBase, pStructBase + pProp->GetOffset(), &var,
						0, // iElement
						pInfo->GetObjectID());
	...
	g_PropTypeFns[pProp->m_Type].Encode(pStructBase, &var, pProp, pInfo->m_DeltaBitsWriter.GetBitBuf(),
										pInfo->GetObjectID());
}
...
static bool SendTable_IsPropZero(CEncodeInfo *pInfo, unsigned int iProp)
{
	...
	pProp->GetProxyFn()(pProp, pBase, pBase + pProp->GetOffset(), &var,
						0, // iElement
						pInfo->GetObjectID());
...
int SendTable_CullPropsFromProxies(const SendTable *pTable,
								   ...
								   const CSendProxyRecipients *pOldStateProxies, const int nOldStateProxies,

								   const CSendProxyRecipients *pNewStateProxies, const int nNewStateProxies,

								   int *pOutProps, int nMaxOutProps)
{
	Assert(!(nNewStateProxies && !pNewStateProxies));
	CPropCullStack stack(pTable->m_pPrecalc, iClient, pOldStateProxies, nOldStateProxies, pNewStateProxies,
						 nNewStateProxies);
...
int SendTable_WriteAllDeltaProps(const SendTable *pTable, const void *pFromData, const int nFromDataBits,
								 const void *pToData, const int nToDataBits, const int nObjectID, bf_write *pBufOut)
{
	// Calculate the delta props.
	int deltaProps[MAX_DATATABLE_PROPS];

	int nDeltaProps = SendTable_CalcDelta(pTable, pFromData, nFromDataBits, pToData, nToDataBits, deltaProps,
...
bool SendTable_Encode(const SendTable *pTable, const void *pStruct, bf_write *pOut, int objectID,
					  CUtlMemory<CSendProxyRecipients> *pRecipients, bool bNonZeroOnly)
{
	...
	if(pRecipients)
	{
		ErrorIfNot(pRecipients->NumAllocated() >= pPrecalc->GetNumDataTableProxies(),
...
void SendTable_WritePropList(const SendTable *pTable, const void *pState, const int nBits, bf_write *pOut,
							 const int objectID, const int *pCheckProps, const int nCheckProps)
{
	if(nCheckProps == 0)
	{
...
static void SendTable_Validate(CSendTablePrecalc *pPrecalc)
{
	...
	for(int i = 0; i < pTable->m_nProps; i++)
	{
		...
		if(pProp->GetArrayProp())
		{
			...
			{
				Error("Invalid property: %s/%s (array of datatables) [on prop %d of %d (%s)].", pTable->m_pNetTableName,
					  pProp->GetName(), i, pTable->m_nProps, pProp->GetArrayProp()->GetName());
			}
		}
		else
		{
			ErrorIfNot(pProp->GetNumElements() == 1, ("Prop %s/%s has an invalid element count for a non-array.",
		...
		{
			DataTable_Warning("SendTable prop %s::%s is a 1-bit signed property. Use SPROP_UNSIGNED or the client will "
