...
#define GAMESTATSUPLOADER_CONNECT_RETRY_TIME 1.0
	...
	virtual bool UploadGameStats(char const *mapname, unsigned int blobversion, unsigned int blobsize,
...
#error
			   ...
			   (k_EUniverseBeta == Steam3Client().SteamUtils()->GetConnectedUniverse()))
			{
				...
				if(bOk)
				{
					...
					// NOTE:  This doesn't copy the NULL terminator from username because we want the "random" bits
					// after the name
		...
		{
			// For Linux dedicated servers, where we won't get a unique ID: set the ID to "unknown" so we have
			// something.  (If there's no ID, stats don't get sent.)  This will later get altered to be a hash of
			// IP&port, but this gets called early before IP is determined so we can't make the hash now.
	...
	bool UploadGameStatsInternal(char const *mapname, unsigned int blobversion, unsigned int blobsize,
								 const void *pvBlobData)
	{
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CUploadGameStats, IUploadGameStats, INTERFACEVERSION_UPLOADGAMESTATS,
								  g_UploadGameStats);
...
{
public:
	explicit CWin32UploadGameStats(const netadr_t &harvester, const TGameStatsParameters &rGameStatsParameters,
								   u32 contextid);
...
private:
	...
	struct FSMState_t
	{
		FSMState_t(uint f, pfnProtocolStateHandler s) : first(f), second(s) {}
...
CWin32UploadGameStats::CWin32UploadGameStats(const netadr_t &harvester,
...
bool CWin32UploadGameStats::SendUploadCommand(EGameStatsUploadStatus &status, CUtlBuffer &buf)
{
	...
	NetworkMessageLengthPrefix_t messageSize(
		sizeof(Command_t) + sizeof(ContextID_t) + sizeof(HarvestFileCommand::FileSize_t) +
		sizeof(HarvestFileCommand::SendMethod_t) + sizeof(HarvestFileCommand::FileSize_t));
...
bool CWin32UploadGameStats::SendWholeFile(EGameStatsUploadStatus &status, CUtlBuffer & /*buf*/)
{
	...
	// Send to server
	bool bret = true;
	if(send(m_SocketTCP, (const char *)m_rCrashParameters.m_pStatsBlobData, (int)m_rCrashParameters.m_uStatsBlobSize,
			0) == SOCKET_ERROR)
	{
		bret = false;
		UpdateProgress(m_rCrashParameters, "Send failed.");
...
EGameStatsUploadStatus Win32UploadGameStatsBlocking(const TGameStatsParameters &rGameStatsParameters)
{
	...
	if(bcs.WaitForMessage(2.0f))
	{
		...
		if(bytesReceived > 0)
		{
			...
			UpdateProgress(rGameStatsParameters, "Server requested game stats upload to %s.",
...
{
public:
	CAsyncUploaderThread() : m_hThread(NULL), m_bRunning(false), m_eventQueue(false), m_eventInitShutdown(false) {}
...
void CAsyncUploaderThread::ThreadProc()
{
	...
	while(m_bRunning)
	{
		...
		if(m_bRunning && pUpload)
		{
			...
			bool bSuccess = g_UploadGameStats.UploadGameStatsInternal(pUpload->szMapName, pUpload->uiBlobVersion,
																	  pUpload->uiBlobSize, pUpload->pvBlob);
			bSuccess;
