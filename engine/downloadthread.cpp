 ...
 */
void __stdcall DownloadStatusCallback(HINTERNET hOpenResource, DWORD dwContext, DWORD dwStatus, LPVOID pStatusInfo,
...
const char *ErrorString[] = {
	"HTTP_ERROR_NONE",		   "HTTP_ERROR_ZERO_LENGTH_FILE", "HTTP_ERROR_CONNECTION_CLOSED",
	"HTTP_ERROR_INVALID_URL",  "HTTP_ERROR_INVALID_PROTOCOL", "HTTP_ERROR_CANT_BIND_SOCKET",
	"HTTP_ERROR_CANT_CONNECT", "HTTP_ERROR_NO_HEADERS",		  "HTTP_ERROR_FILE_NONEXISTENT",
	"HTTP_ERROR_MAX",
};
...
DWORD __stdcall DownloadThread(void *voidPtr)
{
	...
	flags |= INTERNET_FLAG_NO_CACHE_WRITE; // Don't write to IE's cache, since we're doing our own caching of partial
										   // downloads
	flags |=
		INTERNET_FLAG_KEEP_CONNECTION; // Use keep-alive semantics.  Since each file downloaded is a separate connection
...
void SetProxiesForURL(CURL *hMasterCURL, const char *pszURL)
{
	...
	for(iProxy = 0; iProxy < CFArrayGetCount(proxies); iProxy++)
	{
		...
		else if(CFEqual(proxyType, kCFProxyTypeHTTP))
		{
			...
			if(val != NULL &&
			   CFStringGetCString((CFStringRef)val, rgchProxyHost + cbOffset, sizeof(rgchProxyHost) - cbOffset,
								  kCFStringEncodingASCII) &&
			   rgchProxyHost[cbOffset] != '\0')
			{
		...
		else if(CFEqual(proxyType, kCFProxyTypeAutoConfigurationURL))
		{
			...
			CFStringGetCString((CFStringRef)CFStringCreateWithFormat(NULL, NULL, CFSTR("%@"), cfUrl), rgchPacURL,
							   sizeof(rgchPacURL), kCFStringEncodingASCII);
			...
			if(res == CURLE_OK)
			{
				...
				CFStringRef cfPacStr = CFStringCreateWithCString(
					kCFAllocatorDefault, (const char *)g_pacRequestCtx.data, kCFStringEncodingASCII);
				...
				if(proxiesForUrl)
				{
					// we're re-assigning the value that the loop is iterating over, the postincrement will fire after
					// we do this, hence the -1 (rather than 0) assignment to iProxy
...
#warning "CHTTPDownloadThread doesn't know how to set proxy config"
	...
	if(pszProxyExceptionList)
	{
		curl_easy_setopt(hMasterCURL, (CURLoption)(CURLOPTTYPE_OBJECTPOINT + 177) /*CURLOPT_NOPROXY*/,
						 pszProxyExceptionList);
		free(pszProxyExceptionList);
	}
}
...
void DownloadThread(void *voidPtr)
{
	...
	// turn off certificate verification similar to how we set INTERNET_FLAG_IGNORE_CERT_CN_INVALID and
	// INTERNET_FLAG_IGNORE_CERT_DATE_INVALID on Windows
