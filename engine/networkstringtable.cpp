...
ConVar sv_compressstringtablebaselines_threshhold(
	"sv_compressstringtablebaselines_threshold", "2048", 0,
...
private:
	CUtlStableHashtable<CUtlConstString, CNetworkStringTableItem, CaselessStringHashFunctor,
...
//			maxentries -
//-----------------------------------------------------------------------------
CNetworkStringTable::CNetworkStringTable(TABLEID id, const char *tableName, int maxentries, int userdatafixedsize,
...
void CNetworkStringTable::SetAllowClientSideAddString(bool state)
{
	...
	if(m_bAllowClientSideAddString)
	{
		...
		m_pItemsClientSide->Insert("___clientsideitemsplaceholder1___"); // -1 can't be used since it looks like the
																		 // "invalid" index from other string lookups
	}
}
...
// Output : int
//-----------------------------------------------------------------------------
int CNetworkStringTable::AddString(bool bIsServer, const char *string, int length /*= -1*/,
...
void CNetworkStringTableContainer::SetAllowClientSideAddString(INetworkStringTable *table,
...
// Output : TABLEID
//-----------------------------------------------------------------------------
INetworkStringTable *CNetworkStringTableContainer::CreateStringTableEx(const char *tableName, int maxentries,
...
void CNetworkStringTableContainer::WriteBaselines(bf_write &buf)
{
	...
	for(int i = 0; i < m_Tables.Count(); i++)
	{
		...
		if(msg.m_DataOut.GetNumBytesWritten() >= sv_compressstringtablebaselines_threshhold.GetInt())
		{
			...
			// TERROR: bzip-compress the stringtable before adding it to the packet.  Yes, the whole packet will be
			// bzip'd, but the uncompressed data also has to be under the NET_MAX_PAYLOAD limit.
			...
			if(COM_BufferToBufferCompress_Snappy(compressedData, &compressedSize, (char *)msg.m_DataOut.GetData(),
												 numBytes))
			{
				...
				{
					compressTimer.End();
					DevMsg("Stringtable %s compression: %d -> %d bytes: %.2fms\n", table->GetTableName(), numBytes,
						   compressedSize, compressTimer.GetDuration().GetMillisecondsF());
				}
			}
