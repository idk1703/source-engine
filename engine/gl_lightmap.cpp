...
ALIGN128 Vector4D
	blocklights[NUM_BUMP_VECTS + 1][MAX_LIGHTMAP_DIM_INCLUDING_BORDER * MAX_LIGHTMAP_DIM_INCLUDING_BORDER];
...
//-----------------------------------------------------------------------------
static bool AddSingleDynamicLight(dlight_t &dl, SurfaceHandle_t surfID, const Vector &lightOrigin, float perpDistSq,
								  float lightRadiusSq)
{
...
//-----------------------------------------------------------------------------
static void AddSingleDynamicLightToBumpLighting(dlight_t &dl, SurfaceHandle_t surfID, const Vector &lightOrigin,
												...
												const Vector &luxelBasePosition)
{
...
#if 1
	bool useLightDirection = (dl.m_OuterAngle != 0.0f) && (fabs(dl.m_Direction.LengthSqr() - 1.0f) < 1e-3);
	...
	for(int t = 0; t < tmax; ++t)
	{
		...
		// Move along u
		VectorMA(texelWorldPosition, fixupFactor, pTexInfo->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D(),
				 texelWorldPosition);
	}
}
...
static void R_ComputeSurfaceBasis(SurfaceHandle_t surfID, Vector *pBumpNormals, Vector &luxelBasePosition)
{
	...
	VectorMultiply(MSurf_TexInfo(surfID)->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D(),
				   (MSurf_LightmapMins(surfID)[0] - MSurf_TexInfo(surfID)->lightmapVecsLuxelsPerWorldUnits[0][3]) *
					   fixupFactor,
				   luxelBasePosition);

	// Do the same thing for the v direction.
	VectorMA(luxelBasePosition,
			 (MSurf_LightmapMins(surfID)[1] - MSurf_TexInfo(surfID)->lightmapVecsLuxelsPerWorldUnits[1][3]) *
				 fixupFactor,
			 MSurf_TexInfo(surfID)->lightmapVecsLuxelsPerWorldUnits[1].AsVector3D(), luxelBasePosition);
...
//-----------------------------------------------------------------------------
unsigned int R_ComputeDynamicLightMask(dlight_t *pLights, SurfaceHandle_t surfID, msurfacelighting_t *pLighting,
...
//-----------------------------------------------------------------------------
void R_AddDynamicLights(dlight_t *pLights, SurfaceHandle_t surfID, const matrix3x4_t &entityToWorld, bool needsBumpmap,
						unsigned int lightMask)
{
	...
	for(int lnum = 0, testBit = 1, mask = lightMask; lnum < MAX_DLIGHTS && mask != 0; lnum++, mask >>= 1, testBit <<= 1)
	{
		...
		if(mask & 1)
		{
			...
			AddSingleDynamicLightToBumpLighting(pLights[lnum], surfID, lightOrigin, perpDistSq, lightRadiusSq,
...
static void AccumulateLightstyles_EightAtAtime(
	ColorRGBExp32 *RESTRICT pLightmap, // the input lightmap (not necessarily aligned)
	int lightmapSize, fltx4 vScalar,
	Vector4D *RESTRICT bLights // pointer to the blocklights row we'll be writing into -- should be cache aligned, but
...
)
{
	...
	AssertMsg((lightmapSize & 7) == 0,
			  "Input to Accumulate...EightAtATime not divisible by eight. Data corruption is the likely result.");
	...
	for(int i = 0; i < lightmapSize; i += 8)
	{
		...
		fltx4 bytePackedLightMap0 =
			XMLoadVector4(pLightmap + i); // because each colorrgbexp is actually a 32-bit struct,
		...
		// unpack rgbe 0 and 1:
		// like an unsigned unpack: { 0x00, colorLightMap[0].r, 0x00, colorLightMap[0].g, 0x00, colorLightMap[0].b,
		// 0x00, colorLightMap[0].e,
		//							  0x00, colorLightMap[1].r, 0x00, colorLightMap[1].g, 0x00, colorLightMap[1].b, 0x00,
		//colorLightMap[1].e}
		...
		fltx4 signedUnpackHi0 = __vupkhsb(bytePackedLightMap0); // signed unpack of words 0 and 1, like the unsigned
																// unpack but replaces 0x00 w/ sign extension
		fltx4 signedUnpackLo0 = __vupklsb(bytePackedLightMap0); // GROUP 0
		fltx4 signedUnpackHi1 = __vupkhsb(bytePackedLightMap1); // signed unpack of words 0 and 1, like the unsigned
																// unpack but replaces 0x00 w/ sign extension
		...
		// now complete the unpack from halfwords to words (we can just use signed because there are 0x00's above the
		// rgb channels)
		...
		{
			Assert(
				colorLightMap0[group].v[0] == TexLightToLinear(pLightmap[i + group].r, pLightmap[i + group].exponent) &&
...
static void AccumulateLightstyles(ColorRGBExp32 *pLightmap, int lightmapSize, fltx4 vScalar)
{
	...
	for(int i = lightmapSizeEightAligned; i < lightmapSize; ++i)
	{
		...
		fltx4 exponentiator =
			XMVectorExpEst(XMVectorSplatW(LoadSignedByte4NoAssert(reinterpret_cast<XMBYTE4 *>(pLightmap + i))));
...
static void AccumulateLightstylesFlat(ColorRGBExp32 *pLightmap, int lightmapSize, fltx4 vScalar)
{
	...
	// fish out the exponent component from a signed load
	fltx4 exponentiator =
		XMVectorExpEst(XMVectorSplatW(LoadSignedByte4NoAssert(reinterpret_cast<XMBYTE4 *>(pLightmap))));
...
static void AccumulateBumpedLightstyles(ColorRGBExp32 *RESTRICT pLightmap, int lightmapSize, fltx4 vScalar)
{
	...
	*/
	...
	AssertMsg(((reinterpret_cast<unsigned int>(pLightmap) & 0x03) == 0),
...
// Compute the portion of the lightmap generated from lightstyles
//-----------------------------------------------------------------------------
static void ComputeLightmapFromLightstyle(msurfacelighting_t *pLighting, bool computeLightmap, bool computeBumpmap,
										  int lightmapSize, bool hasBumpmapLightmapData)
{
...
static void UpdateLightmapTextures(SurfaceHandle_t surfID, bool needsBumpmap)
{
	...
	if(materialSortInfoArray)
	{
		...
		if(needsBumpmap)
		{
			materials->UpdateLightmap(materialSortInfoArray[MSurf_MaterialSortID(surfID)].lightmapPageID, lightmapSize,
									  offsetIntoLightmapPage, &blocklights[0][0][0], &blocklights[1][0][0],
									  &blocklights[2][0][0], &blocklights[3][0][0]);
		}
		else
		{
			materials->UpdateLightmap(materialSortInfoArray[MSurf_MaterialSortID(surfID)].lightmapPageID, lightmapSize,
									  offsetIntoLightmapPage, &blocklights[0][0][0], NULL, NULL, NULL);
		}
	}
}

unsigned int R_UpdateDlightState(dlight_t *pLights, SurfaceHandle_t surfID, const matrix3x4_t &entityToWorld,
						 ...
						 unsigned int dlightMask, bool needsBumpmap, bool needsLightmap)
{
	...
	{
		ComputeLightmapFromLightstyle(pLighting, (hasLightmap && needsLightmap), (hasBumpmap && needsBumpmap), size,
									  hasBumpmap);
	}
...
void R_BuildLightMap(dlight_t *pLights, ICallQueue *pCallQueue, SurfaceHandle_t surfID,
					 const matrix3x4_t &entityToWorld, bool bOnlyUseLightStyles)
{
	...
	unsigned int dlightMask =
		R_UpdateDlightState(pLights, surfID, entityToWorld, bOnlyUseLightStyles, bDlightsInLightmap);
	...
	{
		pCallQueue->QueueCall(R_BuildLightMapGuts, pLights, surfID, RefToVal(entityToWorld), dlightMask, needsBumpmap,
							  needsLightmap);
	}
}
...
void R_RedownloadAllLightmaps()
{
	...
	if(!initializedBlockLights)
	{
		memset(&blocklights[0][0][0], 0,
			   MAX_LIGHTMAP_DIM_INCLUDING_BORDER * MAX_LIGHTMAP_DIM_INCLUDING_BORDER * (NUM_BUMP_VECTS + 1) *
				   sizeof(Vector));
...
#endif
void FASTCALL R_RenderDynamicLightmaps(dlight_t *pLights, ICallQueue *pCallQueue, SurfaceHandle_t surfID,
