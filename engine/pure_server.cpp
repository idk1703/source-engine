...
bool CPureServerWhitelist::CommandDictDifferent(const CUtlDict<CCommand *, int> &a, const CUtlDict<CCommand *, int> &b)
{
	FOR_EACH_DICT(a, idxA)
	{
		...
		if(b[idxB]->m_LoadOrder == kLoadOrder_HardcodedOverride)
			continue;
		if(a[idxA]->m_eFileClass != b[idxB]->m_eFileClass || a[idxA]->m_LoadOrder != b[idxB]->m_LoadOrder)
			return true;
	}
...
bool CPureServerWhitelist::LoadCommandsFromKeyValues(KeyValues *kv)
{
	for(KeyValues *pCurItem = kv->GetFirstValue(); pCurItem; pCurItem = pCurItem->GetNextValue())
	{
		...
		{
			if(V_stricmp(mods[i], "from_steam") == 0 || V_stricmp(mods[i], "trusted_source") == 0)
				bFromTrustedSource = true;
			else if(V_stricmp(mods[i], "allow_from_disk") == 0)
				bAllowFromDisk = true;
			else if(V_stricmp(mods[i], "check_crc") == 0 || V_stricmp(mods[i], "check_hash") == 0)
				bCheckCRC = true;
		...
		if((bFromTrustedSource && (bAllowFromDisk || bCheckCRC || bAny)) || (bAny && bCheckCRC))
		{
...
void CPureServerWhitelist::AddFileCommand(const char *pszFilePath, EPureServerFileClass eFileClass,
										  unsigned short nLoadOrder)
{
...
void CPureServerWhitelist::UpdateCommandStats(CUtlDict<CPureServerWhitelist::CCommand *, int> &commands, int *pHighest,
...
void CPureServerWhitelist::PrintCommand(const char *pFileSpec, const char *pExt, int maxPathnameLen,
...
int CPureServerWhitelist::FindCommandByLoadOrder(CUtlDict<CPureServerWhitelist::CCommand *, int> &commands,
												 int iLoadOrder)
{
	for(int i = commands.First(); i != commands.InvalidIndex(); i = commands.Next(i))
	{
...
void CPureServerWhitelist::PrintWhitelistContents()
{
	...
	for(int iLoadOrder = 0; iLoadOrder <= highestLoadOrder; iLoadOrder++)
	{
		...
		else
		{
			...
			if(iCommand != -1)
			{
				PrintCommand(m_RecursiveDirCommands.GetElementName(iCommand), "...", longestPathName,
							 m_RecursiveDirCommands[iCommand]);
			}
			else
			{
				// Check *.* commands.
				...
				if(iCommand != -1)
				{
					PrintCommand(m_NonRecursiveDirCommands.GetElementName(iCommand), "*.*", longestPathName,
...
CPureServerWhitelist::CCommand *CPureServerWhitelist::GetBestEntry(const char *pFilename)
{
	...
	if(!V_IsAbsolutePath(relativeFilename) ||
	   m_pFileSystem->FullPathToRelativePath(pFilename, relativeFilename, sizeof(relativeFilename)))
	{
	...
	// Either we couldn't find an entry, or they specified an absolute path that we could not convert to a relative
	// path.
	return NULL;
}

CPureServerWhitelist::CCommand *CPureServerWhitelist::CheckEntry(CUtlDict<CPureServerWhitelist::CCommand *, int> &dict,
...
EPureServerFileClass CPureServerWhitelist::GetFileClass(const char *pszFilename)
{
	...
	// Default action is to be permissive.  (The default whitelist protects certain directories and files at a root
	// level.)
	return ePureServerFileClass_Any;
}

void CPureFileTracker::AddUserReportedFileHash(int idxFile, FileHash_t *pFileHash, USERID_t userID,
											   bool bAddMasterRecord)
{
	...
	if(idxUserReported == m_treeUserReportedFileHash.InvalidIndex())
	{
		...
		if(bAddMasterRecord)
		{
			...
			while(idx != m_treeUserReportedFileHash.InvalidIndex() &&
				  m_treeUserReportedFileHash[idx].m_idxFile == m_treeUserReportedFileHash[idxUserReported].m_idxFile)
			{
			...
			if(ctMatches >= sv_pure_consensus.GetInt())
			{
				...
				while(idxRemove != m_treeUserReportedFileHash.InvalidIndex())
				{
					...
					if(m_treeUserReportedFileHash[idxRemove].m_FileHash ==
					   m_treeUserReportedFileHash[idxUserReported].m_FileHash)
...
void FileRenderHelper(USERID_t userID, const char *pchMessage, const char *pchPath, const char *pchFileName,
					  FileHash_t *pFileHash, int nFileFraction, FileHash_t *pFileHashLocal)
{
	char rgch[256];
	char hex[34];
	Q_memset(hex, 0, sizeof(hex));
	Q_binarytohex((const byte *)&pFileHash->m_md5contents.bits, sizeof(pFileHash->m_md5contents.bits), hex,
				  sizeof(hex));

	char hex2[34];
	Q_memset(hex2, 0, sizeof(hex2));
	if(pFileHashLocal)
		Q_binarytohex((const byte *)&pFileHashLocal->m_md5contents.bits, sizeof(pFileHashLocal->m_md5contents.bits),
					  hex2, sizeof(hex2));
	...
	else
	{
		Q_snprintf(rgch, 256, "Pure server: file: %s\\%s ( %d %d %x ) %s : %s : %s\n", pchPath, pchFileName,
				   pFileHash->m_eFileHashType, pFileHash->m_cbFileLen,
				   pFileHash->m_eFileHashType ? pFileHash->m_crcIOSequence : 0, pchMessage, hex, hex2);
	}
	if(userID.idtype != 0)
...
bool CPureFileTracker::DoesFileMatch(const char *pPathID, const char *pRelativeFilename, int nFileFraction,
									 FileHash_t *pFileHash, USERID_t userID)
{
	...
	//			V_snprintf( warningStr, sizeof( warningStr ), "Pure server: file [%s]\\%s ignored by whitelist.", pPathID,
	//pRelativeFilename ); 			Msg( "[%s] %s\n", GetUserIDString(userID), warningStr );
	//		}
	//
	//		return true;
	//	}
	...
	//	EFileCRCStatus eStatus = g_pFileSystem->CheckCachedFileHash( pPathID, rgchFilenameFixed, nFileFraction,
	//&filehashLocal ); 	if ( eStatus == k_eFileCRCStatus_FileInVPK)
	//	{
	...
	//			FileRenderHelper( userID, "could not open file to hash ( benign for now )", pPathID, rgchFilenameFixed,
	//pFileHash, nFileFraction, NULL );
	//		}
	//	}
	...
	//) 				AddUserReportedFileHash( idxFile, pFileHash, userID, false ); 				FileRenderHelper( userID, "file does not match",
	//pPathID, rgchFilenameFixed, pFileHash, nFileFraction, &filehashLocal ); 				return false;
	//			}
	//		}
	//	}
	//
	...
	//			FileRenderHelper( userID, "file does not match server master file", pPathID, rgchFilenameFixed, pFileHash,
	//nFileFraction, pFileHashLocal ); 			return false;
	//		}
	//	}
	//
	...
	//	{
	//		FileRenderHelper( userID, "server does not have hash for this file. Waiting for consensus", pPathID,
	//rgchFilenameFixed, pFileHash, nFileFraction, NULL );
	//	}
	//
...
int CPureFileTracker::ListUserFiles(bool bListAll, const char *pchFilenameFind)
{
	...
	while(idxFindFile != m_treeAllReportedFiles.InvalidIndex())
	{
		...
		{
			FileHash_t filehashLocal;
			EFileCRCStatus eStatus =
				g_pFileSystem->CheckCachedFileHash(ufile.m_path.String(), ufile.m_filename.String(), 0, &filehashLocal);

			if(eStatus == k_eFileCRCStatus_GotCRC)
			{
				USERID_t useridFake;
				useridFake.idtype = IDTYPE_STEAM;
				FileRenderHelper(useridFake, "Found: ", ufile.m_path.String(), ufile.m_filename.String(),
								 &filehashLocal, 0, NULL);
				FindFileIndex_t ffi;
	...
	while(idx != m_treeUserReportedFileHash.InvalidIndex())
	{
		...
		while(idxNext != m_treeUserReportedFileHash.InvalidIndex() &&
			  m_treeUserReportedFileHash[idx].m_idxFile == m_treeUserReportedFileHash[idxNext].m_idxFile)
		{
		...
		bool bOutput = false;
		if(Q_stristr(ufile.m_filename.String(), "bin\\pak01") != NULL ||
		   Q_stristr(ufile.m_filename.String(), ".vpk") != NULL)
			bOutput = true;
		else
		{
			FileHash_t filehashLocal;
		...
		if(ctMatches != ctFiles || idxMaster != m_treeMasterFileHashes.InvalidIndex() || bListAll ||
		   (pchFilenameFind && m_vecReportedFiles.Find(ffi) != -1) || bOutput)
		{
			char rgch[256];
			...
			FileRenderHelper(file.m_userID, rgch, ufile.m_path.String(), ufile.m_filename.String(), &file.m_FileHash, 0,
							 pFileHashMaster);
		}
	}
	Msg("Total user files %d %d %d \n", m_treeUserReportedFileHash.Count(), cTotalFiles, cTotalMatches);
	Msg("Total files %d, total with authoritative hashes %d \n", m_treeAllReportedFiles.Count(),
		m_treeMasterFileHashes.Count());
	Msg("Matching files %d %d %d \n", m_cMatchedFile, m_cMatchedMasterFile, m_cMatchedMasterFileHash);

	return 0;
}

int CPureFileTracker::ListAllTrackedFiles(bool bListAll, const char *pchFilenameFind, int nFileFractionMin,
										  int nFileFractionMax)
{
	...
	do
	{
		...
		if(count && (bListAll || (pchFilenameFind && Q_stristr(rgUnverifiedFiles[0].m_Filename, pchFilenameFind) &&
								  rgUnverifiedFiles[0].m_nFileFraction >= nFileFractionMin &&
								  rgUnverifiedFiles[0].m_nFileFraction <= nFileFractionMax)))
		{
			USERID_t useridFake;
			useridFake.idtype = IDTYPE_STEAM;
			FileRenderHelper(useridFake, "", rgUnverifiedFiles[0].m_PathID, rgUnverifiedFiles[0].m_Filename,
							 &rgUnverifiedFiles[0].m_FileHash, rgUnverifiedFiles[0].m_nFileFraction, NULL);
			if(rgUnverifiedFiles[0].m_FileHash.m_PackFileID)
			{
				g_pFileSystem->CheckVPKFileHash(
					rgUnverifiedFiles[0].m_FileHash.m_PackFileID, rgUnverifiedFiles[0].m_FileHash.m_nPackFileNumber,
					rgUnverifiedFiles[0].m_nFileFraction, rgUnverifiedFiles[0].m_FileHash.m_md5contents);
			}
			cTotalMatch++;
		}
		if(count)
...
void CC_PureServerListTrackedFiles(const CCommand &args)
{
	...
	g_PureFileTracker.ListAllTrackedFiles(args.ArgC() <= 1, args.ArgC() >= 2 ? args[1] : NULL, nFileFractionMin,
										  nFileFractionMax);
}
