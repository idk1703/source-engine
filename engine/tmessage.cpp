...
#define MAX_MESSAGES \
	600 // I don't know if this table will balloon like every other feature in Half-Life
...
const char *gNetworkMessageNames[MAX_NETMESSAGE] = {NETWORK_MESSAGE1, NETWORK_MESSAGE2, NETWORK_MESSAGE3,
		...
		switch(mode)
		{
			...
			case MSGFILE_TEXT:
				if(IsEndOfText(trim))
				{
					...
					{
						Q_strncpy(nameHeap + lastNamePos, textMessages[messageCount].pClearMessage,
					...
					if(bSpew)
					{
						...
						Msg("  effect %d, color1(%d,%d,%d,%d), color2(%d,%d,%d,%d)\n", m->effect, m->r1, m->g1, m->b1,
							m->a1, m->r2, m->g2, m->b2, m->a2);
						Msg("  pos %f,%f, fadein %f fadeout %f hold %f fxtime %f\n", m->x, m->y, m->fadein, m->fadeout,
							m->holdtime, m->fxtime);
						Msg("  '%s'\n", m->pMessage ? m->pMessage : "(null)");

						Msg("  box %s, size %f, color(%d,%d,%d,%d)\n", m->bRoundedRectBackdropBox ? "yes" : "no",
							m->flBoxSize, m->boxcolor[0], m->boxcolor[1], m->boxcolor[2], m->boxcolor[3]);
