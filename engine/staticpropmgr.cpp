...
{
	if(r_drawstaticprops.GetInt() != 1 || r_DrawSpecificStaticProp.GetInt() >= 0 || r_colorstaticprops.GetBool() ||
	   r_staticpropinfo.GetInt() || mat_fullbright.GetInt() || r_drawmodellightorigin.GetBool() ||
...
public:
	...
	void SetFadeIndex(unsigned short nIndex)
	{
		m_FadeIndex = nIndex;
	}
	unsigned short FadeIndex() const
	{
		return m_FadeIndex;
	}
	float ForcedFadeScale() const
	{
		return m_flForcedFadeScale;
	}
...
public:
	...
	virtual void AddDecalToStaticProp(Vector const &rayStart, Vector const &rayEnd, int staticPropIndex, int decalIndex,
									  bool doTrace, trace_t &tr);
	virtual void AddColorDecalToStaticProp(Vector const &rayStart, Vector const &rayEnd, int staticPropIndex,
										   int decalIndex, bool doTrace, trace_t &tr, bool bUseColor, Color cColor);
	...
	virtual void GetStaticPropMaterialColorAndLighting(trace_t *pTrace, int staticPropIndex, Vector &lighting,
													   Vector &matColor);
	virtual void CreateVPhysicsRepresentations(IPhysicsEnvironment *physenv, IVPhysicsKeyHandler *pDefaults,
											   void *pGameData);
	...
	virtual void GetAllStaticPropsInOBB(const Vector &ptOrigin, const Vector &vExtent1, const Vector &vExtent2,
	...
	// Internal methods
	const Vector &ViewOrigin() const
	{
		return m_vecLastViewOrigin;
	}
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CStaticPropMgr, IStaticPropMgrClient, INTERFACEVERSION_STATICPROPMGR_CLIENT,
								  s_StaticPropMgr);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CStaticPropMgr, IStaticPropMgrServer, INTERFACEVERSION_STATICPROPMGR_SERVER,
								  s_StaticPropMgr);
...
bool CStaticProp::Init(int index, StaticPropLump_t &lump, model_t *pModel)
{
	...
	m_Flags =
		(lump.m_Flags & (STATIC_PROP_SCREEN_SPACE_FADE | STATIC_PROP_FLAG_FADES | STATIC_PROP_NO_PER_VERTEX_LIGHTING));
...
void CStaticProp::SetupWeights(const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights,
							   float *pFlexDelayedWeights)
{
}
...
void CStaticProp::PrecacheLighting()
{
	...
	if(m_ModelInstance == MODEL_INSTANCE_INVALID)
	{
		LightCacheHandle_t lightCacheHandle =
			CreateStaticLightingCache(m_LightingOrigin, m_WorldRenderBBoxMin, m_WorldRenderBBoxMax);
...
int CStaticProp::DrawModelSlow(int flags)
{
	...
	//	CDebugOverlay::AddBoxOverlay( vec3_origin, m_WorldRenderBBoxMin, m_WorldRenderBBoxMax, vec3_angle, 255, 0, 0,
	//32, 0.01 ); 	CDebugOverlay::AddBoxOverlay( GetRenderOrigin(), m_RenderBBoxMin, m_RenderBBoxMax, GetRenderAngles(),
	//0, 255, 0, 32, 0.01 );
...
void CStaticProp::CreateVPhysics(IPhysicsEnvironment *pPhysEnv, IVPhysicsKeyHandler *pDefaults, void *pGameData)
{
	...
	pPhysEnv->CreatePolyObjectStatic(pPhysCollide, surfaceData, m_Origin, m_Angles, &solid.params);
...
void CStaticPropMgr::UnserializeModelDict(CUtlBuffer &buf)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		dict.m_pModel = (model_t *)modelloader->GetModelForName(lump.m_Name, IModelLoader::FMODELLOADER_STATICPROP);
...
void CStaticPropMgr::LevelInitClient()
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		if(prop.LeafCount() > 0)
		{
			...
			clientleafsystem->AddRenderableToLeaves(handle, prop.LeafCount(),
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::CreateVPhysicsRepresentations(IPhysicsEnvironment *pPhysEnv, IVPhysicsKeyHandler *pDefaults,
												   void *pGameData)
{
...
void CStaticPropMgr::GetAllStaticPropsInAABB(const Vector &vMins, const Vector &vMaxs,
											 CUtlVector<ICollideable *> *pOutput)
{
	...
	for(counter = 0; counter != iPropVectorSize; ++counter)
	{
		...
		if(vPropMaxs.x < vMins.x)
			continue;
		if(vPropMaxs.y < vMins.y)
			continue;
		if(vPropMaxs.z < vMins.z)
			continue;

		if(vPropMins.x > vMaxs.x)
			continue;
		if(vPropMins.y > vMaxs.y)
			continue;
		if(vPropMins.z > vMaxs.z)
			continue;
...
void CStaticPropMgr::GetAllStaticPropsInOBB(const Vector &ptOrigin, const Vector &vExtent1, const Vector &vExtent2,
											const Vector &vExtent3, CUtlVector<ICollideable *> *pOutput)
{
	if(pOutput == NULL)
		return;
	...
	for(counter = 0; counter != 8; ++counter)
	{
		...
		if(counter & (1 << 0))
			ptOBBExtents[counter] += vExtent1;
		if(counter & (1 << 1))
			ptOBBExtents[counter] += vExtent2;
		if(counter & (1 << 2))
			ptOBBExtents[counter] += vExtent3;
	...
	for(counter = 0; counter != iPropVectorSize; ++counter)
	{
		...
		if(vPropMaxs.x < vAABBMins.x)
			continue;
		if(vPropMaxs.y < vAABBMins.y)
			continue;
		if(vPropMaxs.z < vAABBMins.z)
			continue;

		if(vPropMins.x > vAABBMaxs.x)
			continue;
		if(vPropMins.y > vAABBMaxs.y)
			continue;
		if(vPropMins.z > vAABBMaxs.z)
			continue;
		...
		for(counter2 = 0; counter2 != 6; ++counter2) // loop over OBB planes
		{
			for(counter3 = 0; counter3 != 8; ++counter3) // loop over prop extents
			{
				...
				{
					// an extent of the prop is within the OBB halfspace, this halfspace does not eliminate our prop,
					// move to the next halfspace
					break;
				}
			}
			if(counter3 == 8)
				break; // if all 8 extents lie outside the halfspace, then the prop is not in the OBB
		}
		if(counter2 == 6)
		{
			// if all 6 planes failed to eliminate the extents, the OBB and prop intersect

			// FIXME: Halfspace elimination will never remove props that do intersect, but leaves some false positives
			// in some cases.
...
bool CStaticPropMgr::IsStaticProp(IHandleEntity *pHandleEntity) const
{
	return (!pHandleEntity) ||
		   ((pHandleEntity->GetRefEHandle().GetSerialNumber() == (STATICPROP_EHANDLE_MASK >> NUM_ENT_ENTRY_BITS)) != 0);
}
		...
		if(g_bLoadedMapHasBakedPropLighting && g_pMaterialSystemHardwareConfig->SupportsStreamOffset())
		{
			...
			while(--i >= 0)
			{
				...
				studiohwdata_t *pStudioHWData =
					g_pMDLCache->GetHardwareData(((model_t *)m_StaticProps[i].GetModel())->studio);
...
void CStaticPropMgr::DrawStaticProps_Slow(IClientRenderable **pProps, int count, bool bShadowDepth,
...
void CStaticPropMgr::DrawStaticProps(IClientRenderable **pProps, int count, bool bShadowDepth,
									 bool drawVCollideWireframe)
{
	...
	else
	{
		...
		if(pipeline_static_props.GetBool() && g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80 &&
...
// Computes fade from screen-space fading
//-----------------------------------------------------------------------------
unsigned char CStaticPropMgr::ComputeScreenFade(CStaticProp &prop, float flMinSize, float flMaxSize,
...
{
#ifndef SWDS
	static RenderGroup_t opaqueRenderGroup =
		(g_bClientLeafSystemV1) ? RENDER_GROUP_OPAQUE_ENTITY : RENDER_GROUP_OPAQUE_STATIC;
...
void CStaticPropMgr::ComputePropOpacity(CStaticProp &prop)
{
	...
	{
		modelinfoclient->RecomputeTranslucency(prop.GetModel(), prop.GetSkin(), prop.GetBody(),
											   prop.GetClientRenderable(), (float)(prop.GetFxBlend()) / 255.0f);
	}
#endif
	...
	if(!IsXbox())
	{
		...
		unsigned char alpha =
			modelinfoclient->ComputeLevelScreenFade(prop.GetRenderOrigin(), prop.Radius(), prop.ForcedFadeScale());
		unsigned char nViewAlpha =
			modelinfoclient->ComputeViewScreenFade(prop.GetRenderOrigin(), prop.Radius(), prop.ForcedFadeScale());
		if(nViewAlpha < alpha)
		{
			alpha = nViewAlpha;
		}
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::AddDecalToStaticProp(Vector const &rayStart, Vector const &rayEnd, int staticPropIndex,
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::AddColorDecalToStaticProp(Vector const &rayStart, Vector const &rayEnd, int staticPropIndex,
											   int decalIndex, bool doTrace, trace_t &tr, bool bUseColor, Color cColor)
{
#ifndef SWDS
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::GetStaticPropMaterialColorAndLighting(trace_t *pTrace, int staticPropIndex, Vector &lighting,
														   Vector &matColor)
{
#ifndef SWDS
	// Invalid static prop? Blow it off!
	...
	modelinfoclient->GetModelMaterialColorAndLighting((model_t *)prop.GetModel(), prop.GetRenderOrigin(),
