...
unsigned int CDispInfo::ComputeDynamicLightMask(dlight_t *pLights)
{
	...
	for(int lnum = 0, testBit = 1, mask = r_dlightactive; lnum < MAX_DLIGHTS && mask != 0;
...
DispDecalHandle_t CDispInfo::NotifyAddDecal(decal_t *pDecal, float flSize)
{
	...
	if(h != s_DispDecals.InvalidIndex())
	{
		...
		R_SetupDecalClip(pOutVerts, pDispDecal->m_pDecal, MSurf_Plane(m_ParentSurfID).normal,
...
// ----------------------------------------------------------------------------- //
void CDispInfo::GenerateDecalFragments_R(CVertIndex const &nodeIndex, int iNodeBitIndex, unsigned short decalHandle,
...
void CDispInfo::GenerateDecalFragments(CVertIndex const &nodeIndex, int iNodeBitIndex, unsigned short decalHandle,
...
bool CDispInfo::ComputeShadowFragments(DispShadowHandle_t h, int &vertexCount, int &indexCount)
{
	...
	if((pShadowDecal->m_Flags & CDispDecalBase::NODE_BITFIELD_COMPUTED) == 0)
	{
		...
		SetupDecalNodeIntersect(m_pPowerInfo->m_RootNode,
		...
		if(DepthMode != DEPTH_MODE_NORMAL)
		{
			...
			if(nAlphaTest == 1)
			{
				...
				IMaterialVar *pOriginalTextureVar =
					pGroup->m_pMaterial->FindVarFast("$basetexture", &originalTextureVarCache);
				static unsigned int originalTextureFrameVarCache = 0;
				IMaterialVar *pOriginalTextureFrameVar =
					pGroup->m_pMaterial->FindVarFast("$frame", &originalTextureFrameVarCache);
				static unsigned int originalAlphaRefCache = 0;
				IMaterialVar *pOriginalAlphaRefVar =
					pGroup->m_pMaterial->FindVarFast("$AlphaTestReference", &originalAlphaRefCache);
	...
	for(int iDisp = 0; iDisp < nVisibleDisps; ++iDisp)
	{
		...
		while(hDecal != DISP_DECAL_HANDLE_INVALID)
		{
			...
			DecalMaterialBucket_t &materialBucket =
				g_aDispDecalSortTrees[iSortTree].m_aDecalSortBuckets[iGroup][iTreeType].Element(iSortMaterial);
			...
			while(iElement != g_aDispDecalSortPool.InvalidIndex())
			{
				...
				while(hFrag != DISP_DECAL_FRAGMENT_HANDLE_INVALID)
				{
					...
					if(bMeshInit)
					{
						...
						{
							meshList.m_pMesh =
								pRenderContext->GetDynamicMesh(false, NULL, NULL, g_materialDecalWireframe);
						}

						meshBuilder.Begin(meshList.m_pMesh, MATERIAL_TRIANGLES, nDecalSortMaxVerts,
										  nDecalSortMaxIndices);
					...
					for(int iVert = 0; iVert < fragment.m_nVerts; ++iVert)
					{
						...
						meshBuilder.Color4ub(fragment.m_pDecal->color.r, fragment.m_pDecal->color.g,
											 fragment.m_pDecal->color.b, fragment.m_pDecal->color.a);
				...
				for(v = 0; v < fragment.m_nVerts - 2; ++v)
				{
					...
					if(pMaterial->InMaterialPage())
					{
						meshBuilder.TexCoordSubRect2f(0, fragment.m_pVerts[v].m_ctCoords.x,
													  fragment.m_pVerts[v].m_ctCoords.y, matOffset[0], matOffset[1],
						 ...
						 ERenderDepthMode DepthMode)
{
	...
	DispInfo_BuildPrimLists(nSortGroup, pList, listCount, (DepthMode != DEPTH_MODE_NORMAL), visibleDisps,
							nVisibleDisps);
