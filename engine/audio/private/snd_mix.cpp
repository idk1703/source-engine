...
bool MIX_InitAllPaintbuffers(void)
{
	...
	g_temppaintbuffer =
		(portable_samplepair_t *)_aligned_malloc(TEMP_COPY_BUFFER_SIZE * sizeof(portable_samplepair_t), 16);
...
void MIX_MixChannelsToPaintbuffer(CChannelList &list, int endtime, int flags, int rate, int outputRate)
{
	...
	Assert(!((endtime - g_paintedtime) & 0x3) ||
...
void S_MixBufferUpsample2x(int count, portable_samplepair_t *pbuffer, portable_samplepair_t *pfiltermem, int cfltmem,
						   int filtertype)
{
	...
	if(sdebug_in_count > SDEBUG_TOTAL_COUNT)
	{
		if((int)sdebug_avg_in > 20.0 && (int)sdebug_avg_out > 20.0)
			DevMsg("dsp avg gain:%1.2f in:%1.2f out:%1.2f 1/gain:%1.2f\n", sdebug_avg_out / sdebug_avg_in,
				   sdebug_avg_in, sdebug_avg_out, sdebug_avg_in / sdebug_avg_out);
...
ConVar snd_cull_duplicates("snd_cull_duplicates", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
						   "If nonzero, aggressively cull duplicate sounds during mixing. The number specifies the "
						   "number of duplicates allowed to be played.");
...
// A mix pass is performed, resulting in mixed sounds in SOUND_BUFFER_ROOM, SOUND_BUFFER_FACING,
// SOUND_BUFFER_FACINGAWAY, SOUND_BUFFER_DRY, SOUND_BUFFER_SPEAKER, SOUND_BUFFER_SPECIALs
...
void MIX_PaintChannels(int endtime, bool bIsUnderwater)
{
	...
	while(g_paintedtime < endtime)
	{
		...
		// SOUND_BUFFER_ROOM, SOUND_BUFFER_FACING, SOUND_BUFFER_FACINGAWAY, SOUND_BUFFER_DRY, SOUND_BUFFER_SPEAKER,
		// SOUND_BUFFER_SPECIALs
		...
		if(g_bdirectionalfx)
		{
			g_AudioDevice->ApplyDSPEffects(idsp_facingaway, MIX_GetPFrontFromIPaint(SOUND_BUFFER_FACINGAWAY),
										   MIX_GetPRearFromIPaint(SOUND_BUFFER_FACINGAWAY),
										   MIX_GetPCenterFromIPaint(SOUND_BUFFER_FACINGAWAY), count);
		}

		if(!g_bDspOff && list.m_hasSpeakerChannels)
		{
			// apply 1ch filtering to SOUND_BUFFER_SPEAKER
			g_AudioDevice->ApplyDSPEffects(idsp_speaker, MIX_GetPFrontFromIPaint(SOUND_BUFFER_SPEAKER),
										   MIX_GetPRearFromIPaint(SOUND_BUFFER_SPEAKER),
										   MIX_GetPCenterFromIPaint(SOUND_BUFFER_SPEAKER), count);
			...
			MIX_MixPaintbuffers(SOUND_BUFFER_SPEAKER, SOUND_BUFFER_FACING, SOUND_BUFFER_FACING, count,
								1.0); // +70% dry speaker

			MIX_ScalePaintBuffer(SOUND_BUFFER_SPEAKER, count, 0.43);

			MIX_MixPaintbuffers(SOUND_BUFFER_SPEAKER, SOUND_BUFFER_ROOM, SOUND_BUFFER_ROOM, count,
								1.0); // +30% wet speaker
		}
		...
		if(!g_bDspOff)
		{
			...
			for(int iDSP = 0; iDSP < list.m_nSpecialDSPs.Count(); ++iDSP)
			{
				...
				{
					paintbuffer_t *pSpecialBuffer = MIX_GetPPaintFromIPaint(i);
					if(pSpecialBuffer->nSpecialDSP == list.m_nSpecialDSPs[iDSP] &&
					   pSpecialBuffer->idsp_specialdsp != -1)
					{
						g_AudioDevice->ApplyDSPEffects(pSpecialBuffer->idsp_specialdsp, MIX_GetPFrontFromIPaint(i),
													   MIX_GetPRearFromIPaint(i), MIX_GetPCenterFromIPaint(i), count);

						// mix SOUND_BUFFER_SPECIALs with SOUND_BUFFER_ROOM and SOUND_BUFFER_FACING
						MIX_ScalePaintBuffer(i, count, 0.7);

						MIX_MixPaintbuffers(i, SOUND_BUFFER_FACING, SOUND_BUFFER_FACING, count,
											1.0); // +70% dry speaker
		...
		g_AudioDevice->ApplyDSPEffects(Get_idsp_room(), MIX_GetPFrontFromIPaint(SOUND_BUFFER_ROOM),
									   MIX_GetPRearFromIPaint(SOUND_BUFFER_ROOM),
									   MIX_GetPCenterFromIPaint(SOUND_BUFFER_ROOM), count);
		...
		if(b_spatial_delays && !g_bDspOff && !DSP_RoomDSPIsOff())
		{
			...
			g_AudioDevice->ApplyDSPEffects(idsp_spatial, MIX_GetPFrontFromIPaint(SOUND_BUFFER_ROOM),
										   MIX_GetPRearFromIPaint(SOUND_BUFFER_ROOM),
										   MIX_GetPCenterFromIPaint(SOUND_BUFFER_ROOM), count);
		}
		...
		if(g_bdirectionalfx) // KDB: perf
		{
			...
			MIX_MixPaintbuffers(SOUND_BUFFER_FACING, SOUND_BUFFER_FACINGAWAY, SOUND_BUFFER_PAINT, count,
								DSP_NOROOM_MIX);
		...
		else
		{
			...
			// (SOUND_BUFFER_FACINGAWAY contains no data, IFACINGBBUFFER has full dry mix based on distance from
			// listener) if dsp disabled, mix 100% facingbuffer, otherwise, mix 75% facingbuffer + roombuffer
		...
		if(bIsUnderwater)
		{
			...
			g_AudioDevice->ApplyDSPEffects(idsp_water, MIX_GetPFrontFromIPaint(SOUND_BUFFER_PAINT),
										   MIX_GetPRearFromIPaint(SOUND_BUFFER_PAINT),
										   MIX_GetPCenterFromIPaint(SOUND_BUFFER_PAINT), count);
		}
		...
		g_AudioDevice->ApplyDSPEffects(idsp_player, MIX_GetPFrontFromIPaint(SOUND_BUFFER_PAINT),
									   MIX_GetPRearFromIPaint(SOUND_BUFFER_PAINT),
									   MIX_GetPCenterFromIPaint(SOUND_BUFFER_PAINT), count);
		...
		/*
				...
				if ( b_spatial_delays )
					g_AudioDevice->ApplyDSPEffects( idsp_spatial, MIX_GetPFrontFromIPaint(SOUND_BUFFER_PAINT),
		   MIX_GetPRearFromIPaint(SOUND_BUFFER_PAINT), MIX_GetPCenterFromIPaint(SOUND_BUFFER_PAINT), count );
		*/
...
bool MIX_ScaleChannelVolume(paintbuffer_t *ppaint, channel_t *pChannel, int volume[CCHANVOLUMES], int mixchans)
{
	...
	// if ( !g_bdirectionalfx || wavtype == CHAR_DOPPLER || wavtype == CHAR_OMNI || (wavtype == CHAR_DIRECTIONAL &&
	// mixchans == 2) )
	if(!g_bdirectionalfx || wavtype != CHAR_DIRECTIONAL)
	{
		...
		// if wavtype is DIRECTIONAL and stereo encoded, mix 0% to facingaway buffer - DIRECTIONAL STEREO wavs have a
		// custom mixer
...
void SW_Mix8StereoDopplerLeft(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
...
void SW_Mix8StereoDopplerRight(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
							   fixedint rateScaleFix, int outCount)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void SW_Mix16StereoDopplerLeft(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
							   fixedint rateScaleFix, int outCount)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void SW_Mix16StereoDopplerRight(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
								fixedint rateScaleFix, int outCount)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void SW_Mix8StereoDirectional(float soundfacing, portable_samplepair_t *pOutput, int *volume, byte *pData,
...
void SW_Mix8StereoDirectional_Interp(float soundfacing, portable_samplepair_t *pOutput, int *volume, byte *pData,
									 int inputOffset, fixedint rateScaleFix, int outCount)
{
...
void SW_Mix16StereoDirectional(float soundfacing, portable_samplepair_t *pOutput, int *volume, short *pData,
							   int inputOffset, fixedint rateScaleFix, int outCount)
{
	fixedint sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void SW_Mix16StereoDirectional_Interp(float soundfacing, portable_samplepair_t *pOutput, int *volume, short *pData,
									  int inputOffset, fixedint rateScaleFix, int outCount)
{
	fixedint sampleIndex = 0;
	fixedint rateScaleFix14 = FIX_28TO14(rateScaleFix); // convert 28 bit fixed point to 14 bit fixed point
...
void SW_Mix8StereoDistVar(float distmix, portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
						  fixedint rateScaleFix, int outCount)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
// interpolating pitch shifter - sample(s) from preceding buffer are preloaded in
// pData buffer, ensuring we can always provide 'outCount' samples.
void SW_Mix8StereoDistVar_Interp(float distmix, portable_samplepair_t *pOutput, int *volume, byte *pData,
								 int inputOffset, fixedint rateScaleFix, int outCount)
{
	int x;
...
void SW_Mix16StereoDistVar(float distmix, portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
						   fixedint rateScaleFix, int outCount)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void SW_Mix16StereoDistVar_Interp(float distmix, portable_samplepair_t *pOutput, int *volume, short *pData,
								  int inputOffset, fixedint rateScaleFix, int outCount)
{
	int x;
...
void SW_Mix8Mono(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset, fixedint rateScaleFix,
				 int outCount)
{
...
// interpolating pitch shifter - sample(s) from preceding buffer are preloaded in
// pData buffer, ensuring we can always provide 'outCount' samples.
void SW_Mix8Mono_Interp(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
						fixedint rateScaleFix, int outCount)
{
	fixedint sampleIndex = 0;
	fixedint rateScaleFix14 = FIX_28TO14(rateScaleFix); // convert 28 bit fixed point to 14 bit fixed point
...
void SW_Mix8Stereo(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset, fixedint rateScaleFix,
				   int outCount)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
// interpolating pitch shifter - sample(s) from preceding buffer are preloaded in
// pData buffer, ensuring we can always provide 'outCount' samples.
void SW_Mix8Stereo_Interp(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
						  fixedint rateScaleFix, int outCount)
{
	fixedint sampleIndex = 0;
	fixedint rateScaleFix14 = FIX_28TO14(rateScaleFix); // convert 28 bit fixed point to 14 bit fixed point
...
void SW_Mix16Mono_Shift(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
						fixedint rateScaleFix, int outCount)
{
	int vol0 = volume[0];
	int vol1 = volume[1];
...
void SW_Mix16Mono(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset, fixedint rateScaleFix,
				  int outCount)
{
	if(rateScaleFix == FIX(1))
	{
...
void SW_Mix16Mono_Interp(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
						 fixedint rateScaleFix, int outCount)
{
	fixedint sampleIndex = 0;
	fixedint rateScaleFix14 = FIX_28TO14(rateScaleFix); // convert 28 bit fixed point to 14 bit fixed point
...
void SW_Mix16Stereo(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset, fixedint rateScaleFix,
					int outCount)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void SW_Mix16Stereo_Interp(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
						   fixedint rateScaleFix, int outCount)
{
	fixedint sampleIndex = 0;
	fixedint rateScaleFix14 = FIX_28TO14(rateScaleFix); // convert 28 bit fixed point to 14 bit fixed point
...
//===============================================================================
void Mix8MonoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
					 fixedint rateScaleFix, int outCount)
{
...
void Mix16MonoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
					  fixedint rateScaleFix, int outCount)
{
	if(FUseHighQualityPitch(pChannel))
...
void Mix8StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
					   fixedint rateScaleFix, int outCount)
{
	switch(pChannel->wavtype)
	{
		...
		case CHAR_DIRECTIONAL:
			if(FUseHighQualityPitch(pChannel))
				SW_Mix8StereoDirectional_Interp(pChannel->dspface, pOutput, volume, pData, inputOffset, rateScaleFix,
												outCount);
			else
				SW_Mix8StereoDirectional(pChannel->dspface, pOutput, volume, pData, inputOffset, rateScaleFix,
										 outCount);
			break;

		case CHAR_DISTVARIANT:
			if(FUseHighQualityPitch(pChannel))
				SW_Mix8StereoDistVar_Interp(pChannel->distmix, pOutput, volume, pData, inputOffset, rateScaleFix,
											outCount);
			else
...
void Mix16StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
						fixedint rateScaleFix, int outCount)
{
	switch(pChannel->wavtype)
	{
		...
		case CHAR_DIRECTIONAL:
			if(FUseHighQualityPitch(pChannel))
				SW_Mix16StereoDirectional_Interp(pChannel->dspface, pOutput, volume, pData, inputOffset, rateScaleFix,
												 outCount);
			else
				SW_Mix16StereoDirectional(pChannel->dspface, pOutput, volume, pData, inputOffset, rateScaleFix,
										  outCount);
			break;

		case CHAR_DISTVARIANT:
			if(FUseHighQualityPitch(pChannel))
				SW_Mix16StereoDistVar_Interp(pChannel->distmix, pOutput, volume, pData, inputOffset, rateScaleFix,
											 outCount);
			else
...
bool SND_IsMouth(channel_t *pChannel)
{
	...
	if(pChannel->sfx && pChannel->sfx->pSource && pChannel->sfx->pSource->GetSentence())
	{
		return true;
	}
