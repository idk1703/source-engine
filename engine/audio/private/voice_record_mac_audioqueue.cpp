...
VoiceRecord_AudioQueue::VoiceRecord_AudioQueue()
	: m_nSampleRate(0),
	  ...
	  m_SampleBuffer(NULL),
	  m_SampleBufferReadPos(0),
	  m_SampleBufferWritePos(0),
...
OSStatus ComplexBufferFillPlayback(AudioConverterRef inAudioConverter, UInt32 *ioNumberDataPackets,
								   AudioBufferList *ioData, AudioStreamPacketDescription **outDataPacketDesc,
								   void *inUserData)
{
								  ...
								  AudioBufferList *ioData)
{
	...
	if(vr->m_nMicInputSamplesAvaialble == 0)
	{
		...
		if(err == noErr)
			vr->m_nMicInputSamplesAvaialble =
				vr->m_MicInputBuffer.mBuffers[0].mDataByteSize / vr->m_InputBytesPerPacket;
	}
	...
	if(vr->m_nMicInputSamplesAvaialble > 0)
	{
		...
		OSStatus err = AudioConverterFillComplexBuffer(vr->GetConverter(), ComplexBufferFillPlayback, vr,
													   &nConverterSamples, &vr->m_ConverterBuffer, NULL);
		if(err == noErr || err == -1)
			vr->RenderBuffer((short *)vr->m_ConverterBuffer.mBuffers[0].mData,
							 vr->m_ConverterBuffer.mBuffers[0].mDataByteSize / sizeof(short));
	}
...
void VoiceRecord_AudioQueue::RenderBuffer(const short *pszBuf, int nSamples)
{
	...
	{
		m_SampleBufferReadPos = (readPos + m_SampleBufferSize / 2) %
								m_SampleBufferSize; // if we crossed the read pointer then bump it forward
...
bool VoiceRecord_AudioQueue::InitalizeInterfaces()
{
	...
	UInt32 flag = 1;
	status = AudioUnitSetProperty(m_AudioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, 1, &flag,
								  sizeof(flag));
	if(status != noErr)
		return false;
	...
	flag = 0;
	status = AudioUnitSetProperty(m_AudioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0, &flag,
								  sizeof(flag));
	if(status != noErr)
		return false;
	...
	status = AudioUnitGetProperty(m_AudioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input,
								  1, // input bus
								  &audioDeviceFormat, &size);
	...
	// Apply format
	status = AudioUnitSetProperty(m_AudioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1,
								  &audioDeviceFormat, sizeof(audioDeviceFormat));
	if(status != noErr)
		return false;
	...
	status =
		AudioConverterSetProperty(m_Converter, kAudioConverterSampleRateConverterQuality, sizeof(UInt32), &quality);
	if(status != noErr)
		return false;
	...
	status = AudioUnitSetProperty(m_AudioUnit, kAudioOutputUnitProperty_SetInputCallback, kAudioUnitScope_Global, 0,
	...
	m_MicInputBuffer.mBuffers[0].mDataByteSize =
		m_nBufferFrameSize * audioDeviceFormat.mBitsPerChannel / 8 * audioDeviceFormat.mChannelsPerFrame;
	...
	m_ConverterBuffer.mBuffers[0].mDataByteSize =
		m_nBufferFrameSize * audioOutputFormat.mBitsPerChannel / 8 * audioOutputFormat.mChannelsPerFrame;
...
int VoiceRecord_AudioQueue::GetRecordedData(short *pOut, int nSamples)
{
	...
	// printf( "Returning %d samples, %d %d (%d)\n", cbSamples/2, (int)m_SampleBufferReadPos,
	// (int)m_SampleBufferWritePos, m_SampleBufferSize );
	return cbSamples / 2;
}
