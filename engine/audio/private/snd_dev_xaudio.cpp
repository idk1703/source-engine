...
public:
	...
	CXboxVoice *GetVoiceData(void)
	{
		return &m_VoiceData;
	}
	IXAudio2 *GetXAudio2(void)
	{
		return m_pXAudio2;
	}
...
private:
	...
	int TransferSurroundInterleaved(const portable_samplepair_t *pFront, const portable_samplepair_t *pRear,
									const portable_samplepair_t *pCenter, int paintedTime, int endTime,
									char *pOutputBuffer);
...
bool CAudioXAudio::Init(void)
{
	...
	snd_surround.SetValue((xAudioConfig & XAUDIOSPEAKERCONFIG_DIGITAL_DOLBYDIGITAL) ? SURROUND_DIGITAL5DOT1
																					: SURROUND_STEREO);
	...
	m_pSourceVoice = NULL;
	hr = m_pXAudio2->CreateSourceVoice(&m_pSourceVoice, &waveFormatEx, 0, XAUDIO2_DEFAULT_FREQ_RATIO,
									   &s_XAudio2VoiceCallback, NULL, NULL);
	if(FAILED(hr))
		return false;
	...
	if(snd_xaudio_spew_packets.GetBool())
	{
		...
		else
		{
			Msg("XAudio: Packet Callback, Submit: %2d, Free: %2d\n", m_BufferHead - m_BufferTail,
				MAX_XAUDIO2_BUFFERS - (m_BufferHead - m_BufferTail));
		}
	}
...
//-----------------------------------------------------------------------------
int CAudioXAudio::TransferStereo(const portable_samplepair_t *pFrontBuffer, int paintedTime, int endTime,
								 char *pOutputBuffer)
{
	int linearCount;
	int i;
...
//-----------------------------------------------------------------------------
int CAudioXAudio::TransferSurroundInterleaved(const portable_samplepair_t *pFrontBuffer,
...
void CAudioXAudio::TransferSamples(int endTime)
{
	...
	{
		pBuffer->AudioBytes = TransferSurroundInterleaved(PAINTBUFFER, REARPAINTBUFFER, CENTERPAINTBUFFER,
														  g_paintedtime, endTime, (char *)pBuffer->pAudioData);
	}
...
bool CXboxVoice::VoiceUpdateData(void)
{
	...
	return bShouldSend || (wVoiceBytes && (GetTickCount() - m_dwLastVoiceSend) > MAX_VOICE_BUFFER_TIME);
}
