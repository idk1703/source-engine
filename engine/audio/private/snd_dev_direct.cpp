...
static GUID IID_IDirectSound3DBufferDef = {
...
public:
	...
	int DeviceChannels(void)
	{
		return m_deviceChannels;
	}
	int DeviceSampleBits(void)
	{
		return m_deviceSampleBits;
	}
	int DeviceSampleBytes(void)
	{
		return m_deviceSampleBits / 8;
	}
	int DeviceDmaSpeed(void)
	{
		return m_deviceDmaSpeed;
	}
	int DeviceSampleCount(void)
	{
		return m_deviceSampleCount;
	}
...
private:
	...
	bool LockDSBuffer(LPDIRECTSOUNDBUFFER pBuffer, DWORD **pdwWriteBuffer, DWORD *pdwSizeBuffer,
					  const char *pBufferName, int lockFlags = 0);
	...
	void S_TransferSurround16(portable_samplepair_t *pfront, portable_samplepair_t *prear,
							  portable_samplepair_t *pcenter, int lpaintedtime, int endtime, int cchan);
	void S_TransferSurround16Interleaved(const portable_samplepair_t *pfront, const portable_samplepair_t *prear,
										 const portable_samplepair_t *pcenter, int lpaintedtime, int endtime);
	void S_TransferSurround16Interleaved_FullLock(const portable_samplepair_t *pfront,
...
void CAudioDirectSound::ClearBuffer(void)
{
	...
	if(IsUsingBufferPerSpeaker())
	{
		...
		while((SURROUNDhresult = pDSBufFL->Lock(0, m_bufferSizeBytes, (void **)&pDataFL, &dwSizeFL, NULL, NULL, 0)) !=
			  DS_OK)
		{
		...
		SURROUNDreps = 0;
		while((SURROUNDhresult = pDSBufFR->Lock(0, m_bufferSizeBytes, (void **)&pDataFR, &dwSizeFR, NULL, NULL, 0)) !=
			  DS_OK)
		{
			if(SURROUNDhresult != DSERR_BUFFERLOST)
			{
		...
		SURROUNDreps = 0;
		while((SURROUNDhresult = pDSBufRL->Lock(0, m_bufferSizeBytes, (void **)&pDataRL, &dwSizeRL, NULL, NULL, 0)) !=
			  DS_OK)
		{
			if(SURROUNDhresult != DSERR_BUFFERLOST)
			{
		...
		SURROUNDreps = 0;
		while((SURROUNDhresult = pDSBufRR->Lock(0, m_bufferSizeBytes, (void **)&pDataRR, &dwSizeRR, NULL, NULL, 0)) !=
			  DS_OK)
		{
			if(SURROUNDhresult != DSERR_BUFFERLOST)
			{
		...
		if(m_bSurroundCenter)
		{
			...
			while((SURROUNDhresult =
					   pDSBufFC->Lock(0, m_bufferSizeBytes, (void **)&pDataFC, &dwSizeFC, NULL, NULL, 0)) != DS_OK)
			{
				if(SURROUNDhresult != DSERR_BUFFERLOST)
				{
...
bool CAudioDirectSound::SNDDMA_InitInterleaved(LPDIRECTSOUND lpDS, WAVEFORMATEX *lpFormat, int channelCount)
{
	...
	else if(channelCount == 6)
	{
		...
		wfx.dwChannelMask =
			KSAUDIO_SPEAKER_5POINT1; // SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT | SPEAKER_FRONT_CENTER |
	...
	// Make sure mixer is active (this was moved after the zeroing to avoid popping on startup -- at least when using
	// the dx9.0b debug .dlls)
	pDSBuf->Play(0, 0, DSBPLAY_LOOPING);
...
sndinitstat CAudioDirectSound::SNDDMA_InitDirect(void)
{
	...
	if(!m_hInstDS)
	{
		...
		pDirectSoundCreate = (long(__stdcall *)(struct _GUID *, struct IDirectSound **,
												struct IUnknown *))GetProcAddress(m_hInstDS, "DirectSoundCreate");
	...
	if(!m_bSurround)
	{
		...
		// Make sure mixer is active (this was moved after the zeroing to avoid popping on startup -- at least when
		// using the dx9.0b debug .dlls)
		pDSBuf->Play(0, 0, DSBPLAY_LOOPING);
...
void CAudioDirectSound::UpdateListener(const Vector &position, const Vector &forward, const Vector &right,
									   const Vector &up)
{
}
...
bool CAudioDirectSound::LockDSBuffer(LPDIRECTSOUNDBUFFER pBuffer, DWORD **pdwWriteBuffer, DWORD *pdwSizeBuffer,
									 const char *pBufferName, int lockFlags)
{
	...
	HRESULT hr;
	int reps = 0;
	while((hr = pBuffer->Lock(0, m_bufferSizeBytes, (void **)pdwWriteBuffer, pdwSizeBuffer, NULL, NULL, lockFlags)) !=
		  DS_OK)
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// Given front, rear and center stereo paintbuffers, split samples into 4 or 5 mono directsound buffers (FL, FC, FR, RL,
// RR)
void CAudioDirectSound::S_TransferSurround16(portable_samplepair_t *pfront, portable_samplepair_t *prear,
											 portable_samplepair_t *pcenter, int lpaintedtime, int endtime, int cchan)
{
	int lpos;
	...
	if(!LockDSBuffer(pDSBufFL, &pdwWriteFL, &dwSizeFL, "FL") || !LockDSBuffer(pDSBufFR, &pdwWriteFR, &dwSizeFR, "FR") ||
	   !LockDSBuffer(pDSBufRL, &pdwWriteRL, &dwSizeRL, "RL") || !LockDSBuffer(pDSBufRR, &pdwWriteRR, &dwSizeRR, "RR"))
	{
		S_Shutdown();
		S_Startup();
		return;
	...
	int sampleMonoCount =
		DeviceSampleCount(); // number of mono samples per output buffer (was;(DeviceSampleCount()>>1))
...
void CAudioDirectSound::S_TransferSurround16Interleaved_FullLock(const portable_samplepair_t *pfront,
																 const portable_samplepair_t *prear,
																 const portable_samplepair_t *pcenter, int lpaintedtime,
																 int endtime)
{
	int lpos;
	DWORD *pdwWrite = NULL;
	...
	int linearCount; // space in output buffer for linearCount mono samples
	int sampleMonoCount = m_bufferSizeBytes /
						  (DeviceSampleBytes() *
						   DeviceChannels()); // number of mono samples per output buffer (was;(DeviceSampleCount()>>1))
	int sampleMask = sampleMonoCount - 1;
...
void CAudioDirectSound::S_TransferSurround16Interleaved(const portable_samplepair_t *pfront,
														const portable_samplepair_t *prear,
														const portable_samplepair_t *pcenter, int lpaintedtime,
														int endtime)
{
