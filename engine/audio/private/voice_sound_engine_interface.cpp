...
public:
	...
	// The compressed audio mixers understand this, but in general do not assume that SampleSize() * SampleCount() =
	// filesize or even that SampleSize() is 100% accurate due to compression.
	...
	virtual bool IsStereoWav()
	{
		return false;
	}
	virtual int GetCacheStatus()
	{
		return AUDIO_IS_LOADED;
	}
	virtual void CacheLoad() {}
	virtual void CacheUnload() {}
	virtual CSentence *GetSentence()
	{
		return NULL;
	}
...
static ConVar voice_overdrivefadetime("voice_overdrivefadetime",
...
CAudioSourceVoice::CAudioSourceVoice(CSfxTable *pSfx, int iChannel) : CAudioSourceWave(pSfx)
{
	m_iChannel = iChannel;
	m_refCount = 0;
...
int CAudioSourceVoice::GetOutputData(void **pData, int samplePosition, int sampleCount,
									 char copyBuf[AUDIOSOURCE_COPYBUF_SIZE])
{
	int nSamplesGotten =
		Voice_GetOutputData(m_iChannel, copyBuf, AUDIOSOURCE_COPYBUF_SIZE, samplePosition, sampleCount);
