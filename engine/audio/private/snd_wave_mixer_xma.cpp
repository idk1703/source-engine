...
// 15 msec seems to be fast enough for XMA to decode enough to keep the smaller buffer sizes satisfied, and have slop
// for +/- 5 msec swings.
...
public:
	static void *Alloc(int bytes)
	{
		...
		return XMemAlloc(bytes, MAKE_XALLOC_ATTRIBUTES(
									0, false, TRUE, FALSE, eXALLOCAllocatorId_XAUDIO, XALLOC_PHYSICAL_ALIGNMENT_4K,
									XALLOC_MEMPROTECT_WRITECOMBINE_LARGE_PAGES, FALSE, XALLOC_MEMTYPE_PHYSICAL));
	}

	static void Free(void *p)
	{
		XMemFree(p,
				 MAKE_XALLOC_ATTRIBUTES(0, false, TRUE, FALSE, eXALLOCAllocatorId_XAUDIO, XALLOC_PHYSICAL_ALIGNMENT_4K,
										XALLOC_MEMPROTECT_WRITECOMBINE_LARGE_PAGES, FALSE, XALLOC_MEMTYPE_PHYSICAL));
	}
};
...
public:
	...
	{
		return CalcSampleSize(16, m_NumChannels);
	}
...
CON_COMMAND(snd_xma_info, "Spew XMA Info")
{
	...
	for(int hMixer = g_XMAMixerList.Head(); hMixer != g_XMAMixerList.InvalidIndex();
		hMixer = g_XMAMixerList.Next(hMixer))
	{
		CAudioMixerWaveXMA *pXMAMixer = g_XMAMixerList[hMixer];
		Msg("  rate:%5d ch:%1d '%s'\n", pXMAMixer->GetSource()->SampleRate(),
			pXMAMixer->GetSource()->IsStereoWav() ? 2 : 1, pXMAMixer->GetSource()->GetFileName());
	}
}
...
CAudioMixerWaveXMA::CAudioMixerWaveXMA(IWaveData *data, int initialStreamPosition) : CAudioMixerWave(data)
{
	...
	{
		Msg("XMA: 0x%8.8x (%2d), Mixer Alloc, '%s'\n", (unsigned int)this, g_XMAMixerList.Count(),
...
void CAudioMixerWaveXMA::Mix(IAudioDevice *pDevice, channel_t *pChannel, void *pData, int outputOffset, int inputOffset,
							 fixedint fracRate, int outCount, int timecompress)
{
	if(m_NumChannels == 1)
	{
...
int CAudioMixerWaveXMA::UpdatePositionForLooping(int *pNumRequestedSamples)
{
	...
	// want to split the straddle into two regions, due to loops possibly requiring a trailer and leader of discarded
	// samples
...
int CAudioMixerWaveXMA::GetXMABlocksAndSubmitToDecoder(bool bDecoderIsLocked)
{
	...
	if(snd_xma_spew_decode.GetBool())
	{
		Msg("XMA: 0x%8.8x, XMABuffer: 0x%8.8x, BufferSize: %d, NextDataOffset: %d, %s\n", (unsigned int)this,
			pXMABuffer, bufferSize, m_DataOffset, m_pData->Source().GetFileName());
	}
...
int CAudioMixerWaveXMA::GetPCMSamples(int numSamplesToCopy, char *pData)
{
	...
	if(snd_xma_spew_drain.GetBool())
	{
		...
		Msg("XMA: 0x%8.8x, SamplePosition: %d, Ready: %d, Requested: %d, %s: %d, Elapsed: %d ms '%s'\n",
			(unsigned int)this, m_SamplePosition, numReadySamples, numSamplesToCopy, pOperation, numCopiedSamples,
			Plat_MSTime() - m_LastDrainTime, m_pData->Source().GetFileName());
	}
	...
	if(!m_pXMAPlayback)
	{
		...
		if(g_XMAMemoryPool.BytesAllocated() != numBuffers * g_XMAMemoryPool.ChunkSize())
		{
			...
			xmaPlaybackInit.outputBufferSizeInSamples =
				(m_NumChannels == 2) ? XMA_STEREO_OUTPUT_BUFFER_SAMPLES : XMA_MONO_OUTPUT_BUFFER_SAMPLES;
	...
	if(numReadySamples > minSamplesNeeded)
	{
		...
		if(snd_xma_spew_startup.GetBool())
		{
			Msg("XMA: 0x%8.8x, Startup Latency: %d ms, Samples Ready: %d, '%s'\n", (unsigned int)this,
				Plat_MSTime() - m_StartTime, numReadySamples, m_pData->Source().GetFileName());
		}
		return true;
	}
...
bool CAudioMixerWaveXMA::ShouldContinueMixing()
{
	if(!IsRetail() && m_Error && snd_xma_spew_warnings.GetBool())
	{
		...
		Warning("XMA: 0x%8.8x, Mixer Aborted: %s, SamplePosition: %d/%d, DataOffset: %d/%d, '%s'\n", (unsigned int)this,
				pErrorString, m_SamplePosition, m_SampleCount, m_DataOffset, m_TotalBytes,
				m_pData->Source().GetFileName());
	}
...
int CAudioMixerWaveXMA::GetOutputData(void **pData, int numSamplesToCopy, char copyBuf[AUDIOSOURCE_COPYBUF_SIZE])
{
	...
	else
	{
		...
		{
			// XMA latency error occurs when decoder not finished (not at EOF) and caller wanted samples but can't get
			// any
			if(snd_xma_spew_warnings.GetInt())
			{
				Warning("XMA: 0x%8.8x, No Decoded Data Ready: %d samples needed, '%s'\n", (unsigned int)this,
						numSamplesToCopy, m_pData->Source().GetFileName());
			}
			m_Error = ERROR_XMA_NO_PCM_DATA;
		}
	}
