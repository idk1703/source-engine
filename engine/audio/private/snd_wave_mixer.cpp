...
char *CAudioMixerWave::LoadMixBuffer(channel_t *pChannel, int sample_load_request, int *pSamplesLoaded,
									 char copyBuf[AUDIOSOURCE_COPYBUF_SIZE])
{
	...
	if(IsX360() || IsDebug())
	{
		...
		if(((sample_load_request + 1) * samplesize) > nTempCopyBufferSize)
		{
			...
			// NOTE: to prevent this, pitch should always be capped in MixDataToDevice (but isn't nor are the sample
			// counters).
	...
	if(samples_loaded < sample_load_request)
	{
		...
		for(int k = 0; (k < 10000 && samples_loaded_retry && samples_loaded < sample_load_request); k++)
		{
			...
			if(samples_loaded_retry)
			{
				...
				{
					Warning(
						"CAudioMixerWave::LoadMixBuffer:  samples_loaded_retry * samplesize = %i but pData == NULL\n",
...
int CAudioMixerWave::MixDataToDevice(IAudioDevice *pDevice, channel_t *pChannel, int sampleCount, int outputRate,
									 int outputOffset)
{
...
//-----------------------------------------------------------------------------
int CAudioMixerWave::MixDataToDevice_(IAudioDevice *pDevice, channel_t *pChannel, int sampleCount, int outputRate,
									  int outputOffset, bool bSkipAllMixing)
{
	...
	^         ^         ^        ^        ^         ^         ^         ^         ^
	|         |         |        |        |         |         |         |         |
	0		 1.2	   2.4      3.6      4.8       6.0       7.2	    8.4		  9.6		m_fsample_index
	(rate*sample)
	...
	*/
	while(sampleCount > 0)
	{
		...
		VPROF_(bInterpolated_pitch ? "CAudioMixerWave::MixData innerloop interpolated"
		...
		if(!bSkipAllMixing)
		{
			...
			Assert(floor(sampleFraction + RoundToFixedPoint(rate, (outputSampleCount - 1), bInterpolated_pitch)) <=
				   samples_loaded);
