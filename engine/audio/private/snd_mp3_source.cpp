...
//-----------------------------------------------------------------------------
CAudioSourceMP3Cache::CAudioSourceMP3Cache(CSfxTable *pSfx) : CAudioSourceMP3(pSfx)
{
	m_hCache = 0;
}

CAudioSourceMP3Cache::CAudioSourceMP3Cache(CSfxTable *pSfx, CAudioSourceCachedInfo *info) : CAudioSourceMP3(pSfx, info)
{
	m_hCache = 0;
...
char *CAudioSourceMP3Cache::GetDataPointer(void)
{
	...
	wavedatacache->GetDataPointer(m_hCache, m_pSfx->GetFileName(), m_dataSize, m_dataStart, (void **)&pMP3Data, 0,
								  &dummy);

	return pMP3Data;
}

int CAudioSourceMP3Cache::GetOutputData(void **pData, int samplePosition, int sampleCount,
...
CAudioMixer *CAudioSourceStreamMP3::CreateMixer(int intialStreamPosition)
{
	...
	IWaveData *pWaveData = CreateWaveDataStream(*this, static_cast<IWaveStreamSource *>(this), m_pSfx->GetFileName(), 0,
												m_dataSize, m_pSfx, 0);
	if(pWaveData)
	{
...
int CAudioSourceStreamMP3::GetOutputData(void **pData, int samplePosition, int sampleCount,
										 char copyBuf[AUDIOSOURCE_COPYBUF_SIZE])
{
	return 0;
}
...
CAudioSource *Audio_CreateStreamedMP3(CSfxTable *pSfx)
{
	...
	CAudioSourceCachedInfo *info =
		audiosourcecache->GetInfo(CAudioSource::AUDIO_SOURCE_MP3, pSfx->IsPrecachedSound(), pSfx);
	if(info)
	{
