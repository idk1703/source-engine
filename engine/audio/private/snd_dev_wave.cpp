...
void CAudioDeviceWave::OpenWaveOut(void)
{
	...
	while(errorCode != MMSYSERR_NOERROR)
	{
		...
		int nRetVal = MessageBox(NULL,
								 ...
								 "Sound not available", MB_RETRYCANCEL | MB_SETFOREGROUND | MB_ICONEXCLAMATION);
...
void CAudioDeviceWave::UpdateListener(const Vector &position, const Vector &forward, const Vector &right,
									  const Vector &up)
{
}
...
void CAudioDeviceWave::Mix8Mono(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
...
void CAudioDeviceWave::Mix8Stereo(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
								  fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	paintbuffer_t *ppaint = MIX_GetCurrentPaintbufferPtr();
...
void CAudioDeviceWave::Mix16Mono(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
								 fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	paintbuffer_t *ppaint = MIX_GetCurrentPaintbufferPtr();
...
void CAudioDeviceWave::Mix16Stereo(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
								   fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	paintbuffer_t *ppaint = MIX_GetCurrentPaintbufferPtr();
...
void CAudioDeviceWave::SpatializeChannel(int volume[CCHANVOLUMES / 2], int master_vol, const Vector &sourceDir,
...
void CAudioDeviceWave::ApplyDSPEffects(int idsp, portable_samplepair_t *pbuffront, portable_samplepair_t *pbufrear,
									   portable_samplepair_t *pbufcenter, int samplecount)
{
