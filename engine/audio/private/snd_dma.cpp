...
#define SNDLVL_TO_DIST_MULT(sndlvl) \
	(sndlvl ? ((pow(10.0f, snd_refdb.GetFloat() / 20) / pow(10.0f, (float)sndlvl / 20)) / snd_refdist.GetFloat()) : 0)
#define DIST_MULT_TO_SNDLVL(dist_mult) \
	(soundlevel_t)(int)(               \
		dist_mult ? (20 * log10(pow(10.0f, snd_refdb.GetFloat() / 20) / (dist_mult * snd_refdist.GetFloat()))) : 0)
...
const float g_DashboardMusicFadeRate =
	0.5f; // Fades one half full-scale volume per second (two seconds for complete fadeout)
...
ConVar snd_musicvolume("snd_musicvolume", "1.0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX, "Music volume", true, 0.0f, true,
					   1.0f);
...
static ConCommand stopsound(
	"stopsound", S_StopAllSoundsC, 0,
	FCVAR_CHEAT); // Marked cheat because it gives an advantage to players minimising ambient noise.
	...
	if(!pSfx->pSource)
	{
		if(IsX360())
		{
			if(SND_IsInGame() && !g_pQueuedLoader->IsMapLoading())
			{
				...
				// loading a sound during gameplay is a bad experience, causes a very expensive sync i/o to fetch the
				// header and in the case of a memory wave, the actual audio data
				bool bFound = false;
				if(!pSfx->m_bIsLateLoad)
				{
					if(pSfx->getname() != PSkipSoundChars(pSfx->getname()))
					{
						...
						if(i != s_Sounds.InvalidIndex())
						{
							...
							{
								// Can safely let the "load" continue because the headers are expected to be in the
								// preload that are now persisted and the wave data cache will find an existing audio
								// buffer match, so no sync i/o should occur from either.
								bFound = true;
							}
						}
					}
		...
		if(IsX360())
		{
			const char *s_CriticalSounds[] = {
				"common", "items", "physics", "player", "ui", "weapons",
			};
			...
			for(int i = 0; i < ARRAYSIZE(s_CriticalSounds); i++)
			{
				...
				if(!Q_strnicmp(pFileName, s_CriticalSounds[i], len) &&
...
void S_InternalReloadSound(CSfxTable *sfx)
{
	...
	audiosourcecache->GetInfo(nSource, sfx->IsPrecachedSound(),
...
void S_ReloadFilesInList(IFileList *pFilesToReload)
{
	...
	wavedatacache->Flush();
	audiosourcecache
		->ForceRecheckDiskInfo(); // Force all cached audio data to recheck size/date info next time it's accessed.
...
channel_t *SND_StealDynamicChannel(SoundSource soundsource, int entchannel, const Vector &origin, CSfxTable *sfx,
								   float flDelay, bool bDoNotOverwriteExisting)
{
	...
	for(int ch_idx = 0; ch_idx < MAX_DYNAMIC_CHANNELS; ch_idx++)
	{
		...
		if(ch->activeIndex)
		{
			...
			if(entchannel != CHAN_AUTO)
			{
				...
				{
					// we found an exact match for this entity and this channel, but the sound we want to play is
					// considered low priority so instead of stomping this entry pretend we couldn't find a free slot to
					// play and let the existing sound keep going
...
{
	channel_t *pChannel =
		SND_StealDynamicChannel(soundsource, entchannel, origin, sfx, flDelay, bDoNotOverwriteExisting);
	if(!pChannel)
		return NULL;
...
void S_SpatializeChannel(int pVolume[CCHANVOLUMES / 2], int master_vol, const Vector *psourceDir, float gain,
						 float mono)
{
...
ConVar dsp_dist_max("dsp_dist_max", "1440.0",
					FCVAR_DEMO | FCVAR_CHEAT); // range at which sounds are mixed at dsp_mix_max
...
ConVar dsp_db_mixdrop("dsp_db_mixdrop", "0.5",
					  FCVAR_DEMO); // sounds with sndlvl below dsp_db_min get dsp_db_mixdrop % less mix
...
void SND_GetDopplerPoints(channel_t *pChannel, QAngle &source_angles, Vector &vnearpoint, Vector &source_doppler_left,
...
void SND_SetSpatialDelays()
{
	...
	Vector v_dir, v_dir2;
	int chan_max =
		(g_AudioDevice->IsSurround() ? 4 : 2) + (g_AudioDevice->IsSurroundCenter() ? 1 : 0); // 2, 4, 5 channels
	...
	if((listener_origin == vec3_origin) && (listener_forward == vec3_origin) && (listener_right == vec3_origin) &&
	...
	switch(g_ssp.chan)
	{
		...
		case 0: // front left: trace max 100' 'cone' to player's left
			...
			else
			{
				...
				v_dir2 =
					g_ssp.cycle ? (g_ssp.cycle == 1 ? listener_forward2d * 0.5 : -listener_forward2d * 0.5) : v_dir;
...
void DAS_SetDiffusion(das_room_t *proom)
{
	...
	volroom = (proom->room_maxs.x - proom->room_mins.x) * (proom->room_maxs.y - proom->room_mins.y) *
	(proom->room_maxs.z - proom->room_mins.z); volroom = fabs(volroom);
...
void DAS_DisplayRoomDEBUG(das_room_t *proom, bool fnew, float preset)
{
	...
	if(fnew)
		DevMsg("NEW DSP NODE: size:(%.0f,%.0f) height:(%.0f) dif %.4f : refl %.4f : cobj: %.0f : sky %.0f \n", dx, dy,
			   dz, proom->diffusion, refl, count, fsky);
...
extern int DSP_ConstructPreset(bool bskyabove, int width, int length, int height, float fdiffusion, float freflectivity,
...
int DAS_GetRoomDSP(das_room_t *proom, int inode)
{
	...
	return DSP_ConstructPreset(bskyabove, width, length, height, fdiffusion, freflectivity, surf_refl, inode,
							   DAS_CNODES);
}
...
void RemapPlayerOrMusicVols(channel_t *ch, int volumes[CCHANVOLUMES / 2], bool fplayersound, bool fmusicsound,
							float mono)
{
	...
	if(fplayersound) // && (ch->bstereowav && ch->wavtype != CHAR_DIRECTIONAL && ch->wavtype != CHAR_DISTVARIANT) )
	{
		...
		// float vol_dist5[]   = {0.29, 0.29, 0.09, 0.09, 0.63};	// FL, FR, RL, RR, FC - 5 channel (mono source) volume
		// distribution float vol_dist5st[] = {0.29, 0.29, 0.09, 0.09, 0.63};	// FL, FR, RL, RR, FC - 5 channel
		// (stereo source) volume distribution

		float vol_dist5[] = {0.30, 0.30, 0.09, 0.09,
							 0.59}; // FL, FR, RL, RR, FC - 5 channel (mono source) volume distribution
		float vol_dist5st[] = {0.30, 0.30, 0.09, 0.09,
							   0.59}; // FL, FR, RL, RR, FC - 5 channel (stereo source) volume distribution

		float vol_dist4[] = {0.50, 0.50, 0.15, 0.15,
							 0.00}; // FL, FR, RL, RR, 0  - 4 channel (mono source) volume distribution
		float vol_dist4st[] = {0.50, 0.50, 0.15, 0.15,
							   0.00}; // FL, FR, RL, RR, 0  - 4 channel (stereo source)volume distribution
...
void SND_Spatialize(channel_t *ch)
{
...
#if !defined(_X360)
	if(ch->sfx && ch->sfx->pSource && ch->sfx->pSource->GetType() == CAudioSource::AUDIO_SOURCE_VOICE)
	{
		Voice_Spatialize(ch);
	}
#endif
	...
	si.info.Set(ch->soundsource, ch->entchannel, ch->sfx ? ch->sfx->getname() : "", ch->origin, ch->direction,
				ch->master_vol, DIST_MULT_TO_SNDLVL(ch->dist_mult), looping, ch->pitch, listener_origin,
				ch->speakerentity);
...
int S_AlterChannel(int soundsource, int entchannel, CSfxTable *sfx, int vol, int pitch, int flags)
{
	...
	if(name && TestSoundChar(name, CHAR_SENTENCE))
	{
		...
		for(int i = 0; i < list.Count(); i++)
		{
			...
			if(channels[ch_idx].soundsource == soundsource && channels[ch_idx].entchannel == entchannel &&
	...
	for(int i = 0; i < list.Count(); i++)
	{
		...
		if(channels[ch_idx].soundsource == soundsource &&
		   ((flags & SND_IGNORE_NAME) || (channels[ch_idx].entchannel == entchannel && channels[ch_idx].sfx == sfx)))
		{
			if(flags & SND_CHANGE_PITCH)
				channels[ch_idx].basePitch = pitch;
...
int S_StartDynamicSound(StartSoundParams_t &params)
{
	...
	target_chan = SND_PickDynamicChannel(params.soundsource, params.entchannel, params.origin, params.pSfx,
										 params.delay, (params.flags & SND_DO_NOT_OVERWRITE_EXISTING_ON_CHANNEL) != 0);
	if(!target_chan)
		return 0;
	...
	if(nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4)
	{
		...
		DevMsg("DynamicSound %s : src %d : channel %d : %d dB : vol %.2f : time %.3f\n", sndname, params.soundsource,
			   params.entchannel, params.soundlevel, params.fvol, g_pSoundServices->GetHostTime());
		if(nSndShowStart == 2 || nSndShowStart == 5)
			DevMsg("\t dspmix %1.2f : distmix %1.2f : dspface %1.2f : lvol %1.2f : cvol %1.2f : rvol %1.2f : rlvol "
				   "%1.2f : rrvol %1.2f\n",
				   pTargetChan->dspmix, pTargetChan->distmix, pTargetChan->dspface, pTargetChan->fvolume[IFRONT_LEFT],
				   pTargetChan->fvolume[IFRONT_CENTER], pTargetChan->fvolume[IFRONT_RIGHT],
				   pTargetChan->fvolume[IREAR_LEFT], pTargetChan->fvolume[IREAR_RIGHT]);
		if(nSndShowStart == 3)
...
int S_StartStaticSound(StartSoundParams_t &params)
{
	...
	*/
	...
	SpatializationInfo_t si;
	si.info.Set(params.soundsource, params.entchannel, params.pSfx ? sndname : "", params.origin, params.direction, vol,
				params.soundlevel, looping, params.pitch, listener_origin, params.speakerentity);
	...
	if(S_IsMusic(ch))
	{
		// See if we have "music" of same name playing from "world" which means we save/restored this sound already.  If
		// so,
	...
	if(nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4)
	{
		DevMsg("StaticSound %s : src %d : channel %d : %d dB : vol %.2f : radius %.0f : time %.3f\n", sndname,
			   params.soundsource, params.entchannel, params.soundlevel, params.fvol, flSoundRadius,
			   g_pSoundServices->GetHostTime());
		if(nSndShowStart == 2 || nSndShowStart == 5)
			DevMsg("\t dspmix %1.2f : distmix %1.2f : dspface %1.2f : lvol %1.2f : cvol %1.2f : rvol %1.2f : rlvol "
				   "%1.2f : rrvol %1.2f\n",
				   ch->dspmix, ch->distmix, ch->dspface, ch->fvolume[IFRONT_LEFT], ch->fvolume[IFRONT_CENTER],
				   ch->fvolume[IFRONT_RIGHT], ch->fvolume[IREAR_LEFT], ch->fvolume[IREAR_RIGHT]);
		if(nSndShowStart == 3)
...
{
	return (channels[iChannel].sfx && channels[iChannel].sfx->pSource && channels[iChannel].sfx->pSource->IsLooped());
}
...
int S_GetCurrentStaticSounds(SoundInfo_t *pResult, int nSizeResult, int entchannel)
{
	...
	for(int i = MAX_DYNAMIC_CHANNELS; i < total_channels && nSpaceRemaining; i++)
	{
		...
		{
			pResult->Set(channels[i].soundsource, channels[i].entchannel, channels[i].sfx->getname(),
						 channels[i].origin, channels[i].direction, ((float)channels[i].master_vol / 255.0),
						 DIST_MULT_TO_SNDLVL(channels[i].dist_mult), IsChannelLooped(i), channels[i].basePitch,
						 listener_origin, channels[i].speakerentity);
			pResult++;
			nSpaceRemaining--;
		}
	}
...
void S_StopSound(int soundsource, int entchannel)
{
	...
	{
		channel_t *pChannel = list.GetChannel(i);
		if(pChannel->soundsource == soundsource && pChannel->entchannel == entchannel)
		{
			S_FreeChannel(pChannel);
		}
	}
...
ConVar snd_spatialize_roundrobin("snd_spatialize_roundrobin", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
								 "Lowend optimization: if nonzero, spatialize only a fraction of sound channels each "
								 "frame. 1/2^x of channels will be spatialized per frame.");
/*
============
S_Update
...
void S_Update(const AudioState_t *pAudioState)
{
	...
	g_DashboardMusicMixValue =
		Approach(g_DashboardMusicMixTarget, g_DashboardMusicMixValue, g_DashboardMusicFadeRate * frametime);
	...
	if(snd_show.GetInt())
	{
		...
		for(int i = 0; i < activeChannels.Count(); i++)
		{
			...
			{
				Con_NXPrintf(
					&np, "%02i l(%03d) r(%03d) vol(%03d) ent(%03d) pos(%6d %6d %6d) timeleft(%f) looped(%d) %50s",
					total + 1, (int)channel->fvolume[IFRONT_LEFT], (int)channel->fvolume[IFRONT_RIGHT],
					channel->master_vol, channel->soundsource, (int)channel->origin[0], (int)channel->origin[1],
					(int)channel->origin[2], timeleft, bLooping, channel->sfx->getname());
			}
			else
			{
				Con_NXPrintf(&np,
							 "%02i l(%03d) c(%03d) r(%03d) rl(%03d) rr(%03d) vol(%03d) ent(%03d) pos(%6d %6d %6d) "
							 "timeleft(%f) looped(%d) %50s",
							 total + 1, (int)channel->fvolume[IFRONT_LEFT], (int)channel->fvolume[IFRONT_CENTER],
							 (int)channel->fvolume[IFRONT_RIGHT], (int)channel->fvolume[IREAR_LEFT],
							 (int)channel->fvolume[IREAR_RIGHT], channel->master_vol, channel->soundsource,
							 (int)channel->origin[0], (int)channel->origin[1], (int)channel->origin[2], timeleft,
							 bLooping, channel->sfx->getname());
			}
...
CON_COMMAND(snd_dumpclientsounds, "Dump sounds to VXConsole")
{
	...
	for(int i = 0; i < list.Count(); i++)
	{
		...
		Msg("%02i %s l(%03d) c(%03d) r(%03d) rl(%03d) rr(%03d) vol(%03d) pos(%6d %6d %6d) timeleft(%f) looped(%d) %50s "
			"chan:%d ent(%03d):%s\n",
			total + 1, ch->flags.fromserver ? "SERVER" : "CLIENT", (int)ch->fvolume[IFRONT_LEFT],
			(int)ch->fvolume[IFRONT_CENTER], (int)ch->fvolume[IFRONT_RIGHT], (int)ch->fvolume[IREAR_LEFT],
			(int)ch->fvolume[IREAR_RIGHT], ch->master_vol, (int)ch->origin[0], (int)ch->origin[1], (int)ch->origin[2],
			timeleft, bLooping, ch->sfx->getname(), ch->entchannel, ch->soundsource,
...
void GetSoundTime(void)
{
	...
	if(cl_movieinfo.IsRecording() || IsReplayRendering())
	{
		...
		{
			IReplayMovieRenderer *pMovieRenderer =
				(g_pClientReplayContext != NULL) ? g_pClientReplayContext->GetMovieRenderer() : NULL;
			...
			if(pMovieRenderer && pMovieRenderer->IsAudioSyncFrame())
			{
				...
				if(s_lastsoundtime != t)
				{
					...
					float fSamples =
						frameTime * (float)g_AudioDevice->DeviceDmaSpeed() + g_ReplaySoundTimeFracAccumulator;
...
CON_COMMAND(vx_soundlist, "Dump sounds to VXConsole")
{
	...
	int i = 0;
	for(int iSrcSound = s_Sounds.FirstInorder(); iSrcSound != s_Sounds.InvalidIndex();
...
void MXR_DebugGraphMixVolumes(debug_showvols_t *groupvols, int cgroups)
{
	...
	for(int i = 0; i < cgroups; i++)
	{
		...
		Q_snprintf(text, 128, "%s: %1.2f (%1.2f)", groupvols[i].psz, groupvols[i].vol * g_DuckScale,
	...
	for(i = 0; i < g_cgrouprules; i++)
	{
		if(g_grouprules[i].is_ducked)
		{
			...
			if(g_DuckScale >= 1.0)
			{
				...
				{
					if(g_grouprules[j].priority > priority && g_grouprules[j].causes_ducking &&
