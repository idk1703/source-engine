...
struct channel_t
{
	...
	float
		fvolume_inc[CCHANVOLUMES]; // volume increment, per frame, moves volume[i] to vol_target[i] (per spatialization)
	...
	union
	{
		...
		struct
		{
			...
			bool delayed_start : 1; // If true, sound had a delay and so same sound on same channel won't channel steal
									// from it
			...
			bool m_bCompatibilityAttenuation : 1; // True when we want to use goldsrc compatibility mode for the
												  // attenuation In that case, dist_mul is set to a relatively
												  // meaningful value in StartDynamic/StartStaticSound, but we interpret
												  // it totally differently in SND_GetGain.
...
public:
	...
	bool m_quashed[MAX_CHANNELS]; // if true, the channel should be advanced, but not mixed, because it's been
								  // heuristically suppressed
