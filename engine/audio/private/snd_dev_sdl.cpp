...
extern void S_SpatializeChannel(/*int nSlot,*/ int volume[6], int master_vol, const Vector *psourceDir, float gain,
								float mono);
...
public:
	...
	void SpatializeChannel(int nSlot, int volume[CCHANVOLUMES / 2], int master_vol, const Vector &sourceDir, float gain,
						   float mono);
...
//CloseWaveOut(); return; } while (false)
...
void CAudioDeviceSDLAudio::AudioCallback(Uint8 *stream, int len)
{
	...
	while(len > 0)
	{
		...
		m_readPos =
			len ? 0
				: (m_readPos + writeLen); // if still bytes to write to stream, we're rolling around the ring buffer.
	}
	...
	if(g_pVideo != NULL)
	{
		g_pVideo->SoundDeviceCommand(VideoSoundDeviceOperation::SDLMIXER_CALLBACK, (void *)stream_orig,
									 (void *)&totalWriteable);
	}
#endif
...
void CAudioDeviceSDLAudio::Mix8Mono(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
...
void CAudioDeviceSDLAudio::Mix8Stereo(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
									  fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	paintbuffer_t *ppaint = MIX_GetCurrentPaintbufferPtr();
...
void CAudioDeviceSDLAudio::Mix16Mono(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
									 fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	paintbuffer_t *ppaint = MIX_GetCurrentPaintbufferPtr();
...
void CAudioDeviceSDLAudio::Mix16Stereo(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
									   fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	paintbuffer_t *ppaint = MIX_GetCurrentPaintbufferPtr();
...
void CAudioDeviceSDLAudio::SpatializeChannel(int nSlot, int volume[CCHANVOLUMES / 2], int master_vol,
...
void CAudioDeviceSDLAudio::ApplyDSPEffects(int idsp, portable_samplepair_t *pbuffront, portable_samplepair_t *pbufrear,
										   portable_samplepair_t *pbufcenter, int samplecount)
{
