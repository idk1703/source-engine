...
bool CMixerControls::Init()
{
	...
	for(int iDevice = static_cast<int>(mixerGetNumDevs()) - 1; iDevice >= 0 && !bFoundConnectionWithMicVolume;
		--iDevice)
	{
		// Open the mixer.
		...
		for(UINT u = 0; u < mxcaps.cDestinations; u++)
		{
			...
			// Now iterate over each connection (things like wave out, microphone, speaker, CD audio), looking for
			// Microphone.
				...
				for(UINT i = 0; i < micLine.cControls; i++)
				{
					...
					if(micLine.dwComponentType == MIXERLINE_COMPONENTTYPE_SRC_MICROPHONE)
					{
						if(pControl->dwControlType == MIXERCONTROL_CONTROLTYPE_ONOFF &&
						   (strstr(pControl->szShortName, "Gain") || strstr(pControl->szShortName, "Boos") ||
...
bool CMixerControls::GetValue_Float(Control iControl, float &flValue)
{
	...
	{
		bool bValue = false;
		bool ret = GetControlOption_Bool(m_ControlInfos[iControl].m_dwControlID,
	...
	else if(iControl == MicVolume)
	{
		...
		if(GetControlOption_Unsigned(m_ControlInfos[iControl].m_dwControlID, m_ControlInfos[iControl].m_cMultipleItems,
									 dwValue))
		{
...
bool CMixerControls::SetValue_Float(Control iControl, float flValue)
{
	...
	{
		bool bValue = !!flValue;
		return SetControlOption_Bool(m_ControlInfos[iControl].m_dwControlID, m_ControlInfos[iControl].m_cMultipleItems,
									 bValue);
	}
	else if(iControl == MicVolume)
	{
		DWORD dwValue = (DWORD)(flValue * 65535.0f);
		return SetControlOption_Unsigned(m_ControlInfos[iControl].m_dwControlID,
...
bool CMixerControls::SelectMicrophoneForWaveInput()
{
							  ...
							  MIXER_OBJECTF_HMIXER | MIXER_GETCONTROLDETAILSF_VALUE) == MMSYSERR_NOERROR)
	{
		...
		{
			ZeroMemory(pmxcdSelectValue, m_dwMicSelectMultipleItems * sizeof(MIXERCONTROLDETAILS_BOOLEAN));
		}
...
void CMixerControls::FindMicSelectControl(DWORD dwLineID, DWORD nControls)
{
	...
	for(UINT iRecControl = 0; iRecControl < nControls; iRecControl++)
	{
		   ...
		   recControls[iRecControl].dwControlType == MIXERCONTROL_CONTROLTYPE_MUX)
		{
			...
			MIXERCONTROLDETAILS_LISTTEXT *pmxcdSelectText = (MIXERCONTROLDETAILS_LISTTEXT *)_alloca(
				sizeof(MIXERCONTROLDETAILS_LISTTEXT) * m_dwMicSelectMultipleItems);
									  ...
									  MIXER_OBJECTF_HMIXER | MIXER_GETCONTROLDETAILSF_LISTTEXT) == MMSYSERR_NOERROR)
			{
				...
				for(DWORD dwi = 0; dwi < m_dwMicSelectMultipleItems; dwi++)
				{
					...
					if(mixerGetLineInfo((HMIXEROBJ)m_hMixer, &mxl, MIXER_OBJECTF_HMIXER | MIXER_GETLINEINFOF_LINEID) ==
						   MMSYSERR_NOERROR &&
