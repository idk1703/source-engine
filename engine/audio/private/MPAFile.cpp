...
const char *m_szErrors[] = {"Can't open the file.",
...
void CMPAFile::Open(const char *szFilename)
{
	...
	m_hFile =
		g_pFullFileSystem->Open(szFilename, "rb", "GAME"); //::CreateFile( szFilename, GENERIC_READ, FILE_SHARE_READ,
...
uint32 CMPAFile::ExtractBytes(uint32 &dwOffset, uint32 dwNumBytes, bool bMoveOffset)
{
	...
	{
		dwResult |=
			((byte)m_pBuffer[n]) << (8 * dwNumByteShifts); // the bit shift will do the correct byte order for you
		dwNumByteShifts--;
	}
