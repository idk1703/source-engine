...
void CAudioSourceWave::GetCacheData(CAudioSourceCachedInfo *info)
{
	...
	if(GetStartupData(tempbuf, sizeof(tempbuf), datalen) && info->s_bIsPrecacheSound && datalen > 0)
	{
...
CAudioSourceMemWave::CAudioSourceMemWave(CSfxTable *pSfx) : CAudioSourceWave(pSfx)
{
	m_hCache = 0;
	m_hStream = INVALID_STREAM_HANDLE;
...
CAudioSourceMemWave::CAudioSourceMemWave(CSfxTable *pSfx, CAudioSourceCachedInfo *info) : CAudioSourceWave(pSfx, info)
{
	m_hCache = 0;
	m_hStream = INVALID_STREAM_HANDLE;
}
...
{
	CAudioMixer *pMixer =
		CreateWaveMixer(CreateWaveDataMemory(*this), m_format, m_channels, m_bits, initialStreamPosition);
...
//-----------------------------------------------------------------------------
int CAudioSourceMemWave::GetOutputData(void **pData, int samplePosition, int sampleCount,
									   char copyBuf[AUDIOSOURCE_COPYBUF_SIZE])
{
...
void CAudioSourceMemWave::CacheLoad(void)
{
	...
	else
	{
		if(m_hStream == INVALID_STREAM_HANDLE)
		{
			...
			m_hStream =
				wavedatacache->OpenStreamedLoad(pFilename, m_dataSize, m_dataStart, 0, -1, m_dataSize, 1, streamFlags);
...
char *CAudioSourceMemWave::GetDataPointer(void)
{
	...
	if(IsPC() || !IsX360())
	{
		...
		wavedatacache->GetDataPointer(m_hCache, m_pSfx->GetFileName(), m_dataSize, m_dataStart, (void **)&pWaveData, 0,
									  &bSamplesConverted);
...
CAudioMixer *CAudioSourceStreamWave::CreateMixer(int initialStreamPosition)
{
	...
	if(IsX360() && (m_format == WAVE_FORMAT_XMA || m_format == WAVE_FORMAT_PCM))
	{
		...
		if(m_format == WAVE_FORMAT_XMA)
		{
			if((initialStreamPosition % XBOX_DVD_SECTORSIZE) || (initialStreamPosition % XMA_BLOCK_SIZE) ||
	...
	IWaveData *pWaveData = CreateWaveDataStream(*this, static_cast<IWaveStreamSource *>(this), pFileName, m_dataStart,
...
//-----------------------------------------------------------------------------
int CAudioSourceStreamWave::GetOutputData(void **pData, int samplePosition, int sampleCount,
										  char copyBuf[AUDIOSOURCE_COPYBUF_SIZE])
{
	return 0;
}
...
#if defined(_DEBUG)
	// For some reason you can't usually do pSfx->getname() in the dev studio debugger, so for convenience we'll grab
	// the name here in debug builds at least...
...
void CAudioSourceCachedInfo::Rebuild(char const *filename)
{
	...
	// Note though these instantiate a specific AudioSource subclass, it doesn't matter, we just need one for .wav and
	// one for .mp3
...
public:
	...
	{
		SearchPathCache(const char *pszRepositoryFilename, const char *pszSearchPath,
						UtlCachedFileDataType_t eOutOfDateMethod)
			: CUtlCachedFileData(pszRepositoryFilename, AUDIOSOURCE_CACHE_VERSION, AsyncLookaheadMetaChecksum,
								 eOutOfDateMethod)
		{
...
private:
	SearchPathCache *LookUpCacheEntry(const char *szCleanedFilename, int audiosourcetype, bool soundisprecached,
									  CSfxTable *sfx);
...
bool CAudioSourceCache::Init(unsigned int memSize)
{
	...
	FOR_EACH_VEC(vecSearchPaths, idxSearchPath)
	{
		...
		FOR_EACH_VEC(m_vecCaches, idxCache)
		{
			...
			{
				Assert(V_strcmp(szSearchPath, m_vecCaches[idxCache]->m_szSearchPath) ==
					   0); // case *should* match exactly
				bFound = true;
				break;
			}
		}
...
//-----------------------------------------------------------------------------
CAudioSourceCache::SearchPathCache *CAudioSourceCache::LookUpCacheEntry(const char *fn, int audiosourcetype,
