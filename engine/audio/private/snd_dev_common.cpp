...
static float GetSpeakerVol(float yaw_source, float pitch_source, float mono, float yaw_speaker, int ispeaker,
						   int cspeaker, bool fmix2channels)
{
...
void CAudioDeviceBase::SpatializeChannel(int volume[CCHANVOLUMES / 2], int master_vol, const Vector &sourceDir,
...
void CAudioDeviceBase::ApplyDSPEffects(int idsp, portable_samplepair_t *pbuffront, portable_samplepair_t *pbufrear,
...
void CAudioDeviceBase::MixUpsample(int sampleCount, int filtertype)
{
	...
	if(pPaint->fsurround)
	{
		...
		S_MixBufferUpsample2x(sampleCount, pPaint->pbufrear, &(pPaint->fltmemrear[ifilter][0]), CPAINTFILTERMEM,
							  filtertype);

		if(pPaint->fsurround_center)
		{
			Assert(pPaint->pbufcenter);
			S_MixBufferUpsample2x(sampleCount, pPaint->pbufcenter, &(pPaint->fltmemcenter[ifilter][0]), CPAINTFILTERMEM,
								  filtertype);
		}
	}
...
void CAudioDeviceBase::Mix8Mono(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
								fixedint rateScaleFix, int outCount, int timecompress)
{
	...
	{
		Mix8MonoWavtype(pChannel, pPaint->pbuf + outputOffset, volume, (byte *)pData, inputOffset, rateScaleFix,
						outCount);
	}
	...
	if(pPaint->fsurround)
	{
		...
		{
			Assert(pPaint->pbufrear);
			Mix8MonoWavtype(pChannel, pPaint->pbufrear + outputOffset, &volume[IREAR_LEFT], (byte *)pData, inputOffset,
		...
		{
			Assert(pPaint->pbufcenter);
			Mix8MonoWavtype(pChannel, pPaint->pbufcenter + outputOffset, &volume[IFRONT_CENTER], (byte *)pData,
							inputOffset, rateScaleFix, outCount);
		}
	}
}

void CAudioDeviceBase::Mix8Stereo(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
								  fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	...
	if(FVolumeFrontNonZero(volume))
	{
		Mix8StereoWavtype(pChannel, pPaint->pbuf + outputOffset, volume, (byte *)pData, inputOffset, rateScaleFix,
						  outCount);
	}
	...
	if(pPaint->fsurround)
	{
		...
		{
			Assert(pPaint->pbufrear);
			Mix8StereoWavtype(pChannel, pPaint->pbufrear + outputOffset, &volume[IREAR_LEFT], (byte *)pData,
							  inputOffset, rateScaleFix, outCount);
		}
		...
		{
			Assert(pPaint->pbufcenter);
			Mix8StereoWavtype(pChannel, pPaint->pbufcenter + outputOffset, &volume[IFRONT_CENTER], (byte *)pData,
							  inputOffset, rateScaleFix, outCount);
		}
	}
}

void CAudioDeviceBase::Mix16Mono(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
								 fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	...
	if(pPaint->fsurround)
	{
		...
		{
			Assert(pPaint->pbufrear);
			Mix16MonoWavtype(pChannel, pPaint->pbufrear + outputOffset, &volume[IREAR_LEFT], pData, inputOffset,
							 rateScaleFix, outCount);
		}
		...
		{
			Assert(pPaint->pbufcenter);
			Mix16MonoWavtype(pChannel, pPaint->pbufcenter + outputOffset, &volume[IFRONT_CENTER], pData, inputOffset,
							 rateScaleFix, outCount);
		}
	}
}

void CAudioDeviceBase::Mix16Stereo(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
								   fixedint rateScaleFix, int outCount, int timecompress)
{
	int volume[CCHANVOLUMES];
	...
	if(pPaint->fsurround)
	{
		...
		{
			Assert(pPaint->pbufrear);
			Mix16StereoWavtype(pChannel, pPaint->pbufrear + outputOffset, &volume[IREAR_LEFT], pData, inputOffset,
							   rateScaleFix, outCount);
		}
		...
		{
			Assert(pPaint->pbufcenter);
			Mix16StereoWavtype(pChannel, pPaint->pbufcenter + outputOffset, &volume[IFRONT_CENTER], pData, inputOffset,
							   rateScaleFix, outCount);
		}
	}
}
...
public:
	...
	int PaintBegin(float, int, int)
	{
		return 0;
	}
	void PaintEnd(void) {}

	void SpatializeChannel(int volume[CCHANVOLUMES / 2], int master_vol, const Vector &sourceDir, float gain,
						   float mono)
	{
	}
	void ApplyDSPEffects(int idsp, portable_samplepair_t *pbuffront, portable_samplepair_t *pbufrear,
						 portable_samplepair_t *pbufcenter, int samplecount)
	{
	}
	...
	void Mix8Mono(channel_t *pChannel, char *pData, int outputOffset, int inputOffset, fixedint rateScaleFix,
				  int outCount, int timecompress)
	{
	}
	void Mix8Stereo(channel_t *pChannel, char *pData, int outputOffset, int inputOffset, fixedint rateScaleFix,
					int outCount, int timecompress)
	{
	}
	void Mix16Mono(channel_t *pChannel, short *pData, int outputOffset, int inputOffset, fixedint rateScaleFix,
				   int outCount, int timecompress)
	{
	}
	void Mix16Stereo(channel_t *pChannel, short *pData, int outputOffset, int inputOffset, fixedint rateScaleFix,
					 int outCount, int timecompress)
	{
	}
	...
	bool IsSurround(void)
	{
		return false;
	}
	bool IsSurroundCenter(void)
	{
		return false;
	}
