...
static ConVar tv_nochat("tv_nochat", "0", FCVAR_ARCHIVE | FCVAR_USERINFO,
						"Don't receive chat messages from other SourceTV spectators");
static ConVar cl_LocalNetworkBackdoor("cl_localnetworkbackdoor", "1", 0,
...
static ConVar cl_playback_screenshots("cl_playback_screenshots", "0", 0,
									  "Allows the client to playback screenshot and jpeg commands in demos.");

#if defined(STAGING_ONLY) || defined(_DEBUG)
static ConVar cl_block_usercommand("cl_block_usercommand", "0", FCVAR_CHEAT,
								   "Force client to not send usercommand (for debugging).");
...
const CPrecacheUserData *CL_GetPrecacheUserData(INetworkStringTable *table, int index)
{
	...
	if(data)
	{
		ErrorIfNot(testLength == sizeof(*data),
...
void CL_CheckClientState(void)
{
	...
	bool useBackdoor = cl_LocalNetworkBackdoor.GetInt() && (cl.m_NetChannel ? cl.m_NetChannel->IsLoopback() : false) &&
					   sv.IsActive() && !demorecorder->IsRecording() && !demoplayer->IsPlayingBack() &&
...
bool CL_CheckCRCs(const char *pszMap)
{
	...
	// Don't verify CRC if we are running a local server (i.e., we are playing single player, or we are the server in
	// multiplay
...
void CL_ReadPackets(bool bFinalTick)
{
	...
	//  ywb 9/5/03
	if((cl.m_NetChannel ? cl.m_NetChannel->IsTimedOut() : false) && bFinalTick && !demoplayer->IsPlayingBack() &&
	   cl.IsConnected())
	{
...
void CL_DispatchSound(const SoundInfo_t &sound)
{
	...
	if(snd_show.GetInt() >= 2)
	{
		DevMsg("%i (seq %i) %s : src %d : ch %d : %d dB : vol %.2f : time %.3f (%.4f delay) @%.1f %.1f %.1f\n",
			   ...
			   sound.fVolume, cl.GetTime(), sound.fDelay, sound.vOrigin.x, sound.vOrigin.y, sound.vOrigin.z);
	}
...
void CL_Retry()
{
	...
	// We need to temporarily disable this execution marker so the connect command succeeds if it was executed by the
	// server. We would still need this even if we called CL_Connect directly because the connect process may execute
	// commands which we want to succeed.
	...
	if(cl.m_bRestrictServerCommands && bCanAddExecutionMarkers)
		Cbuf_AddTextWithMarkers(eCmdExecutionMarker_Disable_FCVAR_SERVER_CAN_EXECUTE, pszCommand,
...
void CL_NextDemo(void)
{
	...
	Q_snprintf(str, sizeof(str), "%s %s", CommandLine()->FindParm("-timedemoloop") ? "timedemo" : "playdemo",
...
CON_COMMAND_F(devshots_screenshot,
			  "Used by the -makedevshots system to take a screenshot. For taking your own screenshots, use the "
			  "'screenshot' command instead.",
			  FCVAR_DONTRECORD)
{
	CL_TakeScreenshot(NULL);
...
ConCommand screenshot_internal_command("__screenshot_internal", screenshot_internal,
		...
		else
		{
			...
			else
			{
				while(1)
				{
					if(cl_snapshot_subdirname[0])
					{
						Q_snprintf(filename, sizeof(filename), "screenshots/%s/%s/%s%04d%s", base,
								   cl_snapshot_subdirname, base, cl_snapshotnum++, extension);
					}
					else
					{
						Q_snprintf(filename, sizeof(filename), "screenshots/%s%04d%s", base, cl_snapshotnum++,
								   extension);
					}
			...
			if(g_pVideoRecorder != NULL)
			{
				...
				if(IsIntegralValue(flFrameRate))
				{
					theFps.SetFPS(RoundFloatToInt(flFrameRate), false);
										...
										1000.0f)) // 1001 is the ntsc divisor (30*1000/1001 = 29.97, etc)
				{
...
#ifdef USE_WEBM_FOR_REPLAY
				g_pVideoRecorder->SetMovieVideoParameters(VideoEncodeCodec::WEBM_CODEC, nJpegQuality, nWidth, nHeight,
														  theFps);
#else
				g_pVideoRecorder->SetMovieVideoParameters(VideoEncodeCodec::DEFAULT_CODEC, nJpegQuality, nWidth,
														  nHeight, theFps);
#endif

				if(cl_movieinfo.DoVideo())
				{
					g_pVideoRecorder->SetMovieSourceImageParameters(VideoEncodeSourceFormat::BGR_24BIT, nWidth,
																	nHeight);
				}

				if(cl_movieinfo.DoVideoSound())
				{
					g_pVideoRecorder->SetMovieSourceAudioParameters(
						AudioEncodeSourceFormat::AUDIO_16BIT_PCMStereo, SOUND_DMA_SPEED,
...
#else
		Warning("Failed to launch startmovie!  If you are trying to use h264, please make sure you have QuickTime "
				"installed.\n");
#endif
		CL_EndMovie();
	}
...
CON_COMMAND_F(startmovie, "Start recording movie frames.", FCVAR_DONTRECORD)
{
	...
	if(args.ArgC() > 2)
	{
		...
		for(int i = 2; i < args.ArgC(); ++i)
		{
...
#else
				Warning("AVI is no longer supported!  Make sure QuickTime is installed and use \"h264\" - if you "
						"install QuickTime, you will need to reboot before using startmovie.\n");
#endif
				return;
			}
#ifdef USE_WEBM_FOR_REPLAY
			...
			{
				Warning("WebM is not supported on this platform!  Make sure QuickTime is installed and use \"h264\" - "
						"if you install QuickTime, you will need to reboot before using startmovie.\n");
				return;
			}
#endif
	...
	CL_StartMovie(args[1], flags, videomode->GetModeStereoWidth(), videomode->GetModeStereoHeight(), flFrameRate,
				  nJpegQuality, videoSystem);
...
void CL_Move(float accumulated_extra_samples, bool bFinalTick)
{
	...
	if(cl.IsActive())
	{
		...
		// Have client .dll create and store usercmd structure
		g_ClientDLL->CreateMove(nextcommandnr, host_state.interval_per_tick - accumulated_extra_samples,
								!cl.IsPaused());
	...
	// COM_Log( "cl.log", "Sending command number %i(%i) to server\n", cl.m_NetChan->m_nOutSequenceNr,
	// cl.m_NetChan->m_nOutSequenceNr & CL_UPDATE_MASK );
...
bool CL_ShouldLoadBackgroundLevel(const CCommand &args)
{
	...
	if(developer.GetInt() || CommandLine()->CheckParm("-console") || CommandLine()->CheckParm("-dev"))
		return false;

	// don't load the map if we're going straight into a level
	if(CommandLine()->CheckParm("+map") || CommandLine()->CheckParm("+connect") ||
	   CommandLine()->CheckParm("+playdemo") || CommandLine()->CheckParm("+timedemo") ||
	   CommandLine()->CheckParm("+timedemoquit") || CommandLine()->CheckParm("+load") ||
...
void CL_SetSteamCrashComment()
{
	...
	Q_snprintf(szDXDriverVersion, sizeof(szDXDriverVersion), "%d.%d.%d.%d", (int32)(info.m_nDriverVersionHigh >> 16),
			   (int32)(info.m_nDriverVersionHigh & 0xffff), (int32)(info.m_nDriverVersionLow >> 16),
			   (int32)(info.m_nDriverVersionLow & 0xffff));

	Q_snprintf(driverinfo, sizeof(driverinfo),
			   "Driver Name:  %s\nDriver Version: %s\nVendorId / DeviceId:  0x%x / 0x%x\nSubSystem / Rev:  0x%x / "
			   "0x%x\nDXLevel:  %s [%d]\nVid:  %i x %i",
			   info.m_pDriverName, szDXDriverVersion, info.m_VendorID, info.m_DeviceID, info.m_SubSysID,
			   info.m_Revision, dxlevel ? dxlevel : "Unk", nDxLevel, videomode->GetModeWidth(),
	...
	Q_snprintf(videoinfo, sizeof(videoinfo),
			   "picmip: %i forceansio: %i trilinear: %i antialias: %i vsync: %i rootlod: %i reducefillrate: %i\n"
			   ...
			   mat_aaquality.GetInt(), mat_vsync.GetInt(), r_rootlod.GetInt(), mat_reducefillrate.GetInt(),
			   r_shadowrendertotexture.GetInt(), r_flashlightdepthtexture.GetInt(),
			   r_waterforcereflectentities.GetInt(), mat_motion_blur_enabled.GetInt());
#else
	Q_snprintf(videoinfo, sizeof(videoinfo),
			   "picmip: %i forceansio: %i trilinear: %i antialias: %i vsync: %i rootlod: %i reducefillrate: %i\n"
			   "shadowrendertotexture: %i r_flashlightdepthtexture %i waterforceexpensive: %i "
			   "waterforcereflectentities: %i mat_motion_blur_enabled: %i mat_queue_mode %i",
			   mat_picmip.GetInt(), mat_forceaniso.GetInt(), mat_trilinear.GetInt(), mat_antialias.GetInt(),
			   mat_vsync.GetInt(), r_rootlod.GetInt(), mat_reducefillrate.GetInt(), r_shadowrendertotexture.GetInt(),
			   r_flashlightdepthtexture.GetInt(), r_waterforceexpensive.GetInt(), r_waterforcereflectentities.GetInt(),
	...
	Q_snprintf(misc, sizeof(misc), "skill:%i rate %i update %i cmd %i latency %i msec", skill.GetInt(),
			   cl_rate->GetInt(), (int)cl_updaterate->GetFloat(), (int)cl_cmdrate->GetFloat(), latency);
	...
	Q_snprintf(g_minidumpinfo, sizeof(g_minidumpinfo),
			   ...
			   "video: %s\n"
			   "OS: %s\n",
			   map, com_gamedir, build_number(), misc, pNetChannel, CommandLine()->GetCmdLine(), driverinfo, videoinfo,
			   osversion);

	char full[4096];
	Q_snprintf(full, sizeof(full), "%sPP PAGES: used: %d, free %d\n", g_minidumpinfo, (int)g_pagedpoolinfo.numPagesUsed,
			   (int)g_pagedpoolinfo.numPagesFree);
...
static ConCommand startupmenu("startupmenu", &CL_CheckToDisplayStartupMenus,
							  "Opens initial menu screen and loads the background bsp, but only if no other level is "
...
ConVar cl_cloud_settings("cl_cloud_settings", "1", FCVAR_HIDDEN,
						 "Cloud enabled from (from HKCU\\Software\\Valve\\Steam\\Apps\\appid\\Cloud)",
						 CL_ChangeCloudSettingsCvar);
void CL_ChangeCloudSettingsCvar(IConVar *var, const char *pOldValue, float flOldValue)
{
	...
	if(pCommand)
	{
		...
		if(pCommand->IsFlagSet(FCVAR_NOT_CONNECTED))
		{
			...
			if(cl.IsConnected())
			{
				...
				if(pCommand->IsFlagSet(FCVAR_USERINFO) && g_ClientDLL &&
				   g_ClientDLL->IsConnectedUserInfoChangeAllowed(NULL))
				{
		...
		if(IsPC())
		{
			...
			if(((eUniverse != k_EUniverseBeta) && (eUniverse != k_EUniverseDev)) &&
			   pCommand->IsFlagSet(FCVAR_DEVELOPMENTONLY))
				return;
#endif
		}
...
void CL_HookClientStringTables()
{
	...
	for(int i = 0; i < numTables; i++)
	{
		...
		CNetworkStringTable *pTable = (CNetworkStringTable *)cl.m_StringTableContainer->GetTable(i);
...
void CL_InstallAndInvokeClientStringTableCallbacks()
{
	...
	for(int i = 0; i < numTables; i++)
	{
		...
		// We already had it installed (e.g., from client .dll) so all of the callbacks have been called and don't need
		// a second dose
