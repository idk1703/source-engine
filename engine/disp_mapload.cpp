...
public:
	CConstructorChecker()
	{
		m_nConstructedObjects = 0;
	}
	~CConstructorChecker()
	{
		Assert(m_nConstructedObjects == 0);
	}
...
{
	return static_cast<CDispInfo *>(DispInfo_IndexArray(pWorld->brush.pShared->hDispInfos, i));
}

static void BuildDispSurfInit(model_t *pWorld, CCoreDispInfo *pBuildDisp, SurfaceHandle_t worldSurfID)
{
...
VertexFormat_t ComputeDisplacementStaticMeshVertexFormat(const IMaterial *pMaterial, const CDispGroup *pCombo,
...
void AddEmptyMesh(model_t *pWorld, CDispGroup *pCombo, const ddispinfo_t *pMapDisps, int *pDispInfos, int nDisps,
...
void FillStaticBuffer(CGroupMesh *pMesh, CDispInfo *pDisp, const CCoreDispInfo *pCoreDisp, const CDispVert *pVerts,
					  int nLightmaps)
{
#ifndef SWDS
...
void DispInfo_CreateEmptyStaticBuffers(model_t *pWorld, const ddispinfo_t *pMapDisps)
{
	...
	for(int i = 0; i < g_DispGroups.Size(); i++)
	{
		...
		for(int iDisp = 0; iDisp < pCombo->m_DispInfos.Size(); iDisp++)
		{
			...
			{
				AddEmptyMesh(pWorld, pCombo, pMapDisps, &pCombo->m_DispInfos[iStart], iDisp - iStart, nTotalVerts,
							 nTotalIndices);
			...
			{
				AddEmptyMesh(pWorld, pCombo, pMapDisps, &pCombo->m_DispInfos[iStart], iDisp - iStart + 1,
							 nTotalVerts + nVerts, nTotalIndices + nIndices);
				break;
			}
			else
			{
...
//-----------------------------------------------------------------------------
bool DispInfo_CreateFromMapDisp(model_t *pWorld, int iDisp, const ddispinfo_t *pMapDisp, CCoreDispInfo *pCoreDisp,
								const CDispVert *pVerts, const CDispTri *pTris,
								const MaterialSystem_SortInfo_t *pSortInfos, bool bRestoring)
{
	...
	ErrorIfNot(pCoreDisp->GetAllowedVerts().GetNumDWords() == sizeof(pMapDisp->m_AllowedVerts) / 4,
...
bool DispInfo_LoadDisplacements(model_t *pWorld, bool bRestoring)
{
	...
	if(bRestoring)
	{
		...
		ErrorIfNot(pWorld->brush.pShared->numDispInfos == nDisplacements,
		...
		ErrorIfNot(g_DispLMAlpha.Count() == nLuxels,
				   ("DispInfo_LoadDisplacements: lightmap alpha counts (%d and %d) don't match.", g_DispLMAlpha.Count(),
					nLuxels));
	}
	else
	{
	...
	for(iDisp = 0; iDisp < nDisplacements; ++iDisp)
	{
		...
		if(!DispInfo_CreateFromMapDisp(pWorld, iDisp, pMapDisp, aCoreDisps[iDisp], tempVerts, tempTris, pSortInfos,
									   bRestoring))
			return false;
	}
...
bool CDispInfo::CopyCoreDispData(model_t *pWorld, const MaterialSystem_SortInfo_t *pSortInfos,
								 const CCoreDispInfo *pCoreDisp, bool bRestoring)
{
...
void BuildTagData(CCoreDispInfo *pCoreDisp, CDispInfo *pDisp)
{
	...
	for(iTri = 0; iTri < pCoreDisp->GetTriCount(); ++iTri)
	{
		if(pCoreDisp->IsTriTag(iTri, DISPTRI_TAG_WALKABLE))
		{
			pCoreDisp->GetTriIndices(iTri, pDisp->m_pWalkIndices[nWalkCount], pDisp->m_pWalkIndices[nWalkCount + 1],
		...
		if(pCoreDisp->IsTriTag(iTri, DISPTRI_TAG_BUILDABLE))
		{
			pCoreDisp->GetTriIndices(iTri, pDisp->m_pBuildIndices[nBuildCount], pDisp->m_pBuildIndices[nBuildCount + 1],
...
void BlendSubNeighbors(CCoreDispInfo **ppListBase, int nListSize)
{
	...
	for(int iDisp = 0; iDisp < nListSize; ++iDisp)
	{
		...
		for(int iEdge = 0; iEdge < 4; ++iEdge)
		{
			...
			{
				CVertIndex viCorners[2] = {pNeighbor1->GetCornerPointIndex(iCorners[0]),
			...
			for(int iSubEdge = 0; iSubEdge < 2; ++iSubEdge)
			{
				...
				while(it.Next())
				{
					...
					if(!it.IsLastVert())
					{
						...
						Vector vAvgTanS =
							pDisp->GetTangentS(it.GetVertIndex()) + pNeighbor->GetTangentS(it.GetNBVertIndex());
						VectorNormalize(vecAverage);
						VectorNormalize(vAvgTanS);
					...
					for(int iTween = iPrevPos + 1; iTween < iCurPos; iTween++)
					{
						...
						Vector vAvgTanS;
						VectorLerp(pDisp->GetTangentS(viPrevPos), pDisp->GetTangentS(it.GetVertIndex()), flPercent,
								   vAvgTanS);
						VectorNormalize(vAvgTanS);
