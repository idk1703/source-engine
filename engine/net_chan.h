...
public: // INetChannel interface
	...
	void RequestFile_OLD(const char *filename,
						 unsigned int transferID);				  // request remote file to upload, returns request ID
...
public:
	...
	int m_nProtocolVersion; // PROTOCOL_VERSION if we're not playing a demo - otherwise, whatever was in the demo
							// header's networkprotocol if the CNetChan instance was created by a demo player.
};
