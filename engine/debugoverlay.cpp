...
ConVar enable_debug_overlays("enable_debug_overlays", "1", FCVAR_GAMEDLL | FCVAR_CHEAT,
...
namespace CDebugOverlay
{
	...
	// Output :
	//-----------------------------------------------------------------------------
	void AddEntityTextOverlay(int ent_index, int line_offset, float duration, int r, int g, int b, int a,
							  const char *text)
	{
		if(cl.IsPaused())
			return;
	...
	{
		static int s_defaultColorTable[3][3] = {{255, 0, 0}, {0, 255, 0}, {0, 0, 255}};
		...
		for(int k = 0; k < 3; k++)
		{
			...
			AddLineOverlay(startPt, endPt, vColorTable[k][0], vColorTable[k][1], vColorTable[k][2], 255, true,
	...
	// Output  :
	//------------------------------------------------------------------------------
	void AddScreenTextOverlay(float flXPos, float flYPos, int line_offset, float duration, int r, int g, int b, int a,
							  const char *text)
	{
		if(cl.IsPaused())
			return;
	...
	void AddTextOverlay(const Vector &textPos, int line_offset, float duration, float r, float g, float b, float alpha,
						const char *text)
	{
		if(cl.IsPaused())
			return;
	...
	// Output :
	//-----------------------------------------------------------------------------
	void AddBoxOverlay(const Vector &origin, const Vector &mins, const Vector &maxs, QAngle const &angles, int r, int g,
					   int b, int a, float flDuration)
	{
		if(cl.IsPaused())
			return;
	...
	void AddBoxOverlay2(const Vector &origin, const Vector &mins, const Vector &maxs, QAngle const &orientation,
						const Color &faceColor, const Color &edgeColor, float duration)
	{
		if(cl.IsPaused())
			return;
	...
	// Output :
	//-----------------------------------------------------------------------------
	void AddSphereOverlay(const Vector &vOrigin, float flRadius, int nTheta, int nPhi, int r, int g, int b, int a,
						  float flDuration)
	{
		if(cl.IsPaused())
			return;
	...
	void AddSweptBoxOverlay(const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs,
							QAngle const &angles, int r, int g, int b, int a, float flDuration)
	{
		if(cl.IsPaused())
			return;
	...
	// Output :
	//-----------------------------------------------------------------------------
	void AddLineOverlay(const Vector &origin, const Vector &dest, int r, int g, int b, int a, bool noDepthTest,
						float flDuration)
	{
		if(cl.IsPaused())
			return;
	...
	//-----------------------------------------------------------------------------
	void AddTriangleOverlay(const Vector &p1, const Vector &p2, const Vector &p3, int r, int g, int b, int a,
	...
	void DrawOverlay(OverlayBase_t *pOverlay)
	{
		...
		switch(pOverlay->m_Type)
		{
			case OVERLAY_LINE:
			{
				...
				RenderLine(pLine->origin, pLine->dest, Color(pLine->r, pLine->g, pLine->b, pLine->a),
						   !pLine->noDepthTest);
			}
			break;
			...
			case OVERLAY_BOX:
			{
				...
				if(pCurrBox->a > 0)
				{
					RenderBox(pCurrBox->origin, pCurrBox->angles, pCurrBox->mins, pCurrBox->maxs,
							  Color(pCurrBox->r, pCurrBox->g, pCurrBox->b, pCurrBox->a), false);
				}
				RenderWireframeBox(pCurrBox->origin, pCurrBox->angles, pCurrBox->mins, pCurrBox->maxs,
			...
			case OVERLAY_BOX2:
			{
				...
				{
					RenderBox(pCurrBox->origin, pCurrBox->angles, pCurrBox->mins, pCurrBox->maxs, pCurrBox->faceColor,
							  false);
				}
				if(pCurrBox->edgeColor.a() > 0)
				{
					RenderWireframeBox(pCurrBox->origin, pCurrBox->angles, pCurrBox->mins, pCurrBox->maxs,
									   pCurrBox->edgeColor, false);
				}
			}
			break;
			...
			case OVERLAY_SPHERE:
			{
				...
				RenderSphere(pSphere->vOrigin, pSphere->flRadius, pSphere->nTheta, pSphere->nPhi,
							 Color(pSphere->r, pSphere->g, pSphere->b, pSphere->a), g_pMaterialAmbientCube);
			}
			break;
			...
			case OVERLAY_SWEPT_BOX:
			{
				...
				RenderWireframeSweptBox(pBox->start, pBox->end, pBox->angles, pBox->mins, pBox->maxs,
	...
	void PurgeTextOverlays(void)
	{
		...
		while(pCurrOverlay)
		{
			if(pCurrOverlay->m_flEndTime == 0.0f && pCurrOverlay->m_nCreationTick != -1)
			{
...
public:
	...
	void AddBoxOverlay(const Vector &origin, const Vector &mins, const Vector &max, QAngle const &angles, int r, int g,
					   int b, int a, float duration)
	{
		CDebugOverlay::AddBoxOverlay(origin, mins, max, angles, r, g, b, a, duration);
	}

	void AddSweptBoxOverlay(const Vector &start, const Vector &end, const Vector &mins, const Vector &max,
	...
	void AddTextOverlayRGB(const Vector &origin, int line_offset, float duration, float r, float g, float b,
	...
	void AddTextOverlayRGB(const Vector &origin, int line_offset, float flDuration, int r, int g, int b, int alpha,
						   const char *format, ...)
	{
		va_start(m_argptr, format);
		Q_vsnprintf(m_text, sizeof(m_text), format, m_argptr);
		...
		CDebugOverlay::AddTextOverlay(origin, line_offset, flDuration, r * 1.0f / 255.0f, g * 1.0f / 255.0f,
									  b * 1.0f / 255.0f, alpha * 1.0f / 255.0f, m_text);
	}

	void AddScreenTextOverlay(float flXPos, float flYPos, float flDuration, int r, int g, int b, int a,
							  const char *text)
	{
	...
	void AddLineOverlayAlpha(const Vector &origin, const Vector &dest, int r, int g, int b, int a, bool noDepthTest,
							 float duration)
	{
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CIVDebugOverlay, IVPhysicsDebugOverlay, VPHYSICS_DEBUG_OVERLAY_INTERFACE_VERSION,
								  g_DebugOverlay);