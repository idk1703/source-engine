...
int MapList_ListMaps(const char *pszSubString, bool listobsolete, bool verbose, int maxcount, int maxitemlength,
					 char maplist[][64]);
...
public:
	...
	virtual eFindMapResult FindMap(/* in/out */ char *pMapName, int nMapNameMax)
	{
		...
		AssertMsg(eCanGameDLLProvide == IServerGameDLL::eCanProvideLevel_CannotProvide, "Unhandled enum member");
	...
	EmitAmbientSound
	  ...
	  =================
	*/
	virtual void EmitAmbientSound(int entindex, const Vector &pos, const char *samp, float vol, soundlevel_t soundlevel,
	...
	virtual void FadeClientVolume(const edict_t *clientent, float fadePercent, float fadeOutSeconds, float holdTime,
								  float fadeInSeconds)
	{
	...
	*/
	virtual void ClientCommand(edict_t *pEdict, const char *szFmt, ...)
	{
		...
		if((entnum < 1) || (entnum > sv.GetClientCount()))
		{
			ConMsg("\n!!!\n\nStuffCmd:  Some entity tried to stuff '%s' to console buffer of entity %i when maxclients "
				   "was set to %i, ignoring\n\n",
	...
	virtual void ClientCommandKeyValues(edict_t *pEdict, KeyValues *pCommand)
	{
		...
		if((entnum < 1) || (entnum > sv.GetClientCount()))
		{
			ConMsg("\n!!!\n\nClientCommandKeyValues:  Some entity tried to stuff '%s' to console buffer of entity %i "
				   "when maxclients was set to %i, ignoring\n\n",
	...
	*/
	...
	void Message_DetermineMulticastRecipients(bool usepas, const Vector &origin,
	...
	*/
	...
	int Message_CheckMessageLength()
	{
		if(s_MsgData.currentMsg == &s_MsgData.userMsg)
		{
			...
			if(msgsize == -1)
			{
				...
				{
					Warning("DLL_MessageEnd:  Refusing to send user message %s of %i bytes to client, user message "
		...
		if(s_MsgData.currentMsg == &s_MsgData.entityMsg)
		{
			...
			if(bytesWritten > MAX_ENTITY_MSG_DATA) // TODO use a define or so
			{
				Warning("Entity Message to %i, %i bytes written (max is %d)\n", s_MsgData.entityMsg.m_nEntityIndex,
						bytesWritten, MAX_ENTITY_MSG_DATA);
				return -1;
			}
	...
	virtual edict_t *CreateFakeClientEx(const char *netname, bool bReportFakeClient /*= true*/)
	{
		...
		sv.SetReportNewFakeClients(
			true); // Leave this set as true so other callers of sv.CreateFakeClient behave correctly.
...
private:
	...
	virtual void PlaybackTempEntity(IRecipientFilter &filter, float delay, const void *pSender, const SendTable *pST,
									int classID);
	...
	virtual ISpatialPartition *CreateSpatialPartition(const Vector &worldmin, const Vector &worldmax)
	{
...
// INTERFACEVERSION_VENGINESERVER_VERSION_21 is compatible with 22 latest since we only added virtuals to the end, so
// expose that as well.
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVEngineServer, IVEngineServer021, INTERFACEVERSION_VENGINESERVER_VERSION_21,
								  g_VEngineServer22);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVEngineServer, IVEngineServer022, INTERFACEVERSION_VENGINESERVER_VERSION_22,
								  g_VEngineServer22);
...
//			classID -
//-----------------------------------------------------------------------------
void CVEngineServer::PlaybackTempEntity(IRecipientFilter &filter, float delay, const void *pSender,
...
//-----------------------------------------------------------------------------
void CVEngineServer::SolidMoved(edict_t *pSolidEnt, ICollideable *pSolidCollide, const Vector *pPrevAbsOrigin,
								bool accurateBboxTriggerChecks)
{
