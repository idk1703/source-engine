...
static void voiceconvar_file_changed_f(IConVar *pConVar, const char *pOldValue, float flOldValue)
{
	...
	if(var.GetInt() == 0)
	{
		// Force voice recording to stop if they turn off voice_inputfromfile or if sv_allow_voice_from_file is set to
		// 0. Prevents an exploit where clients turn it on, start voice sending a long file, and then turn it off
		// immediately.
...
ConVar voice_recordtofile(
	"voice_recordtofile", "0", 0,
...
ConVar sv_allow_voice_from_file("sv_allow_voice_from_file", "1", FCVAR_REPLICATED,
...
#define LIMIT_PER_CLIENT_COMMAND_EXECUTION_ONCE_PER_INTERVAL(seconds)                                      \
	{                                                                                                      \
		static float g_flLastTime__Limit[ABSOLUTE_PLAYER_LIMIT] = {                                        \
			0.0f}; /* we don't have access to any of the three MAX_PLAYERS #define's here unfortunately */ \
		int playerindex = cmd_clientslot;                                                                  \
		if(playerindex >= 0 && playerindex < (ARRAYSIZE(g_flLastTime__Limit)) &&                           \
		   realtime - g_flLastTime__Limit[playerindex] > (seconds))                                        \
		{                                                                                                  \
...
CON_COMMAND(status, "Display map and connection status.")
{
	...
	print("version : %s/%d %d %s%s%s\n", GetSteamInfIDVersionInfo().szVersionString, PROTOCOL_VERSION, build_number(),
		  bGSSecure ? "secure" : "insecure", pchSecureReasonString, pchUniverse);
...
bool CL_HL2Demo_MapCheck(const char *name)
{
	...
	{
		if(!Q_stricmp(name, "d1_trainstation_01") || !Q_stricmp(name, "d1_trainstation_02") ||
		   !Q_stricmp(name, "d1_town_01") || !Q_stricmp(name, "d1_town_01a") || !Q_stricmp(name, "d1_town_02") ||
		   !Q_stricmp(name, "d1_town_03") || !Q_stricmp(name, "background01") || !Q_stricmp(name, "background03"))
		{
			return true;
		}
		return false;
...
bool CL_PortalDemo_MapCheck(const char *name)
{
	...
	{
		if(!Q_stricmp(name, "testchmb_a_00") || !Q_stricmp(name, "testchmb_a_01") ||
		   !Q_stricmp(name, "testchmb_a_02") || !Q_stricmp(name, "testchmb_a_03") ||
		   !Q_stricmp(name, "testchmb_a_04") || !Q_stricmp(name, "testchmb_a_05") ||
...
void Host_Map_Helper(const CCommand &args, bool bEditmode, bool bBackground, bool bCommentary)
{
	...
	if(args.ArgC() == 10)
	{
		if(Q_stricmp(args[2], "setpos") == 0 && Q_stricmp(args[6], "setang") == 0)
		{
			Vector newpos;
...
CON_COMMAND(fs_find, "Run virtual filesystem find")
{
	...
	size_t matches = 0;
	for(pFile = g_pFullFileSystem->FindFirstEx(pWildcard, pPathID, &findhandle); pFile;
