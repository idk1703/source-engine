...
void CDemoFile::WriteCmdHeader(unsigned char cmd, int tick)
{
	...
	static const char *cmdname[] = {"dem_unknown", "dem_signon",	 "dem_packet", "dem_synctick",	  "dem_consolecmd",
...
demoheader_t *CDemoFile::ReadDemoHeader()
{
	...
	if((m_DemoHeader.demoprotocol > DEMO_PROTOCOL) || (m_DemoHeader.demoprotocol < 2))
	{
...
bool CDemoFile::Open(const char *name, bool bReadOnly, bool bMemoryBuffer, int nBufferSize /*=0*/,
					 bool bAllowHeaderWrite /*=true*/)
{
	if(m_pBuffer && m_pBuffer->IsValid())
	{
