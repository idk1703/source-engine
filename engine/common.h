...
void COM_Log(const char *pszFile, PRINTF_FORMAT_STRING const char *fmt, ...)
	FMTFUNCTION(2, 3); // Log a debug message to specified file ( if pszFile == NULL uses c:\\hllog.txt )
...
void *COM_CompressBuffer_LZSS(const void *source, unsigned int sourceLen, unsigned int *compressedLen,
							  unsigned int maxCompressedLen = 0);
...
/// early in this case
inline void *COM_CompressBuffer(const void *source, unsigned int sourceLen, unsigned int *compressedLen,
								unsigned int maxCompressedLen = 0)
{
