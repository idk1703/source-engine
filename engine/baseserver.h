...
public:
	...
	bool RestartOnLevelChange()
	{
		return m_bRestartOnLevelChange;
	}
...
public:									   // IServer implementation
	...
	{
		return NET_GetUDPPort(m_Socket);
	}
	...
	virtual int GetSpawnCount(void) const
	{
	...
	virtual int GetClassBits(void) const
	{
	...
	virtual float GetCPUUsage(void)
	{
	...
	virtual bool IsDedicated(void) const
	{
		return m_bIsDedicated;
	}
	virtual bool IsPaused(void) const
	{
		return m_State == ss_paused;
	}
	virtual bool IsMultiplayer(void) const
	{
		return m_nMaxclients > 1;
	}
	virtual bool IsPausable(void) const
	{
		return false;
	}
	virtual bool IsHLTV(void) const
	{
		return false;
	}
	virtual bool IsReplay(void) const
	{
		return false;
	}
	...
	virtual void WriteTempEntities(CBaseClient *client, CFrameSnapshot *to, CFrameSnapshot *from, bf_write &pBuf,
								   int nMaxEnts);
...
public: // IConnectionlessPacketHandler implementation
	...
	void SetReportNewFakeClients(bool bReportNewFakeClients)
	{
		m_bReportNewFakeClients = bReportNewFakeClients;
	}
...
protected:
	...
	{
		AssertMsg(0, "CBaseServer::CreateNewClient() being called - must be implemented in derived class!");
		return NULL;
	}; // must be derived

	virtual bool FinishCertificateCheck(netadr_t &adr, int nAuthProtocol, const char *szRawCertificate,
										int clientChallenge)
	{
		return true;
	};
	...
	virtual bool CheckChallengeType(CBaseClient *client, int nNewUserID, netadr_t &adr, int nAuthProtocol,
	...
	void SetRestartOnLevelChange(bool state)
	{
		m_bRestartOnLevelChange = state;
	}
...
public:
	...
	MD5Value_t worldmapMD5; // For detecting that client has a hacked local copy of map, the client will be dropped if
							// this occurs.
...
protected:
	...
	bool m_bReportNewFakeClients; // Whether or not newly created fake clients should be included in server browser
								  // totals
