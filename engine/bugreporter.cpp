...
static ConVar bugreporter_uploadasync("bugreporter_uploadasync", "0", FCVAR_ARCHIVE,
...
void DisplaySystemVersion(char *osversion, int maxlen)
{
	...
	switch(osvi.dwPlatformId)
	{
		case VER_PLATFORM_WIN32_NT:
			...
			char build[256];
			Q_snprintf(build, sizeof(build), "%s (Build %d) version %d.%d", osvi.szCSDVersion,
					   osvi.dwBuildNumber & 0xFFFF, osvi.dwMajorVersion, osvi.dwMinorVersion);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CBugReportUploadProgressDialog::CBugReportUploadProgressDialog(Panel *parent, const char *name, const char *title,
															   const char *message)
	: Frame(parent, name)
{
	SetSize(300, 160);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CBugReportFinishedDialog::CBugReportFinishedDialog(Panel *parent, const char *name, const char *title,
												   const char *message)
	: Frame(parent, name)
{
	SetSize(300, 160);
...
protected:
	...
	bool UploadBugSubmission(char const *levelname, int bugId,
							 ...
							 char const *savefile, char const *screenshot, char const *bsp, char const *vmf,
...
CBugUIPanel::CBugUIPanel(bool bIsPublic, vgui::Panel *parent)
	: BaseClass(parent, "BugUIPanel"), m_bIsPublic(bIsPublic), m_bAddVMF(false)
{
	m_sDllName = m_bIsPublic ? BUG_REPORTER_PUBLIC_DLLNAME : GetInternalBugReporterDLL();
...
void CBugUIPanel::OnTick()
{
	...
	if(m_flPauseTime > 0.0f)
	{
		if(m_flPauseTime <= system()->GetFrameTime())
		{
			...
			if(!m_hFinishedDialog.Get())
			{
				m_hFinishedDialog = new CBugReportFinishedDialog(
					NULL, "FinishDialog", "#Steam_FinishedBug_WorkingTitle", "#Steam_FinishedBug_Text");
		...
		else
		{
			...
			{
				float percent =
					clamp(1.0f - (float)(m_flPauseTime - system()->GetFrameTime()) / (float)PUBLIC_BUGREPORT_WAIT_TIME,
						  0.0f, 1.0f);
...
void CBugUIPanel::OnSaveGame()
{
	...
	if(m_pBugReporter && m_pBugReporter->IsPublicUI())
	{
		// External users send us a "minisave" file which doesn't contain data from other previously
		// encoudntered/connected levels
...
void CBugUIPanel::OnSubmit()
{
	...
	Q_snprintf(driverinfo, sizeof(driverinfo),
			   ...
			   "DXLevel:  %s\nVid:  %i x %i\n"
			   "Framerate:  %.3f\n",
			   osversion, info.m_pDriverName, info.m_VendorID, info.m_DeviceID, info.m_SubSysID, info.m_Revision,
			   dxlevel ? dxlevel : "Unk", videomode->GetModeWidth(), videomode->GetModeHeight(), g_fFramesPerSecond);
	...
	Q_snprintf(misc, sizeof(misc),
			   ...
			   skill.GetInt(), cl_rate->GetInt(), (int)cl_updaterate->GetFloat(), (int)cl_cmdrate->GetFloat(), latency,
			   host_thread_mode.GetInt(), sv_alternateticks.GetInt(), ai_strong_optimizations.GetInt());
	...
	if(m_pBugReporter->IsPublicUI())
	{
		m_pProgressDialog = new CBugReportUploadProgressDialog(
			NULL, "ProgressDialog", "#Steam_SubmittingBug_WorkingTitle", "#Steam_SubmittingBug_WorkingText");
...
void CBugUIPanel::OnFinishBugReport()
{
	...
	if(success)
	{
		...
		if(!m_pBugReporter->IsPublicUI())
		{
			if(!UploadBugSubmission(m_szLevel, bugId, m_szSaveGameName, m_szScreenShotName, m_szBSPName, m_szVMFName,
									m_IncludedFiles))
			{
...
bool CBugUIPanel::UploadBugSubmission(char const *levelname, int bugId, char const *savefile, char const *screenshot,
									  char const *bsp, char const *vmf, CUtlVector<includedfile> &files)
{
	bool bret = true;
	...
	if(savefile && savefile[0])
	{
		...
		g_UploadQueue.QueueCall(this, &CBugUIPanel::UploadFile, CUtlEnvelope<const char *>(localfile),
...
void CBugUIPanel::OnCommand(char const *command)
{
	...
	else if(!Q_strcasecmp(command, "savegame"))
	{
		...
		if(bugreporter_includebsp.GetBool() && m_pBugReporter->IsPublicUI() == false)
		{
			OnSaveBSP();
		}
	}
...
{
	if(code == KEY_ESCAPE || GetBaseButtonCode(code) == KEY_XBUTTON_B)
	{
		Close();
		WipeData();
	}
