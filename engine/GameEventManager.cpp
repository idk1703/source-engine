...
static const char *s_GameEnventTypeMap[] = {"local",  // 0 : don't network this field
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CGameEventManager, IGameEventManager2, INTERFACEVERSION_GAMEEVENTSMANAGER2,
								  s_GameEventManager);
...
bool CGameEventManager::FireEventIntern(IGameEvent *event, bool bServerOnly, bool bClientOnly)
{
	...
	tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "%s (name: %s listeners: %d)", __FUNCTION__,
				   tmDynamicString(TELEMETRY_LEVEL0, event->GetName()), descriptor->listeners.Count());
	...
	for(int i = 0; i < descriptor->listeners.Count(); i++)
	{
		...
		if((listener->m_nListenerType == SERVERSIDE || listener->m_nListenerType == SERVERSIDE_OLD) && bClientOnly)
			continue;

		// don't trigger clientside events, if not explicit a clientside event
		if((listener->m_nListenerType == CLIENTSIDE || listener->m_nListenerType == CLIENTSIDE_OLD) && !bClientOnly)
			continue;
		...
		if(listener->m_nListenerType == CLIENTSIDE_OLD || listener->m_nListenerType == SERVERSIDE_OLD)
		{
			tmZone(TELEMETRY_LEVEL1, TMZF_NONE, "FireGameEvent (i: %d, listenertype: %d (old))", i,
				   listener->m_nListenerType);
		...
		{
			tmZone(TELEMETRY_LEVEL1, TMZF_NONE, "FireGameEvent (i: %d, listenertype: %d (new))", i,
				   listener->m_nListenerType);
...
bool CGameEventManager::RegisterEvent(KeyValues *event)
{
	...
	if(!descriptor)
	{
		...
		AssertMsg2(V_strlen(event->GetName()) <= MAX_EVENT_NAME_LENGTH,
				   "Event named '%s' exceeds maximum name length %d", event->GetName(), MAX_EVENT_NAME_LENGTH);
