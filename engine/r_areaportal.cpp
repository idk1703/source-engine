...
static inline bool GetPortalScreenExtents(dareaportal_t *pPortal, portalclip_t *RESTRICT clip, CPortalRect &portalRect,
	...
	for(int iAreaPortal = 0; iAreaPortal < pArea->numareaportals; iAreaPortal++)
	{
		...
		if(portalVis)
		{
			...
			if(GetRectIntersection(&portalRect, pClipRect, &intersection))
			{
				...
				R_FlowThroughArea(pAreaPortal->otherarea, vecVisOrigin, &intersection, pVisData,
...
static void R_SetupGlobalFrustum()
{
	...
	else
	{
		...
		// Rather than try to deal with crazy projection matrices (shear, trapezoid, etc), take whatever FOV we're
		// given, assume it's conservative, and then construct a matching projection matrix for it. Then use that
		// consistent hallucination throughout, rather than refer back to the engine's actual proj. matrix for anything.
		VMatrix matrixView;
		VMatrix matrixProjection;
		VMatrix matrixWorldToScreen;
		g_viewSetup.m_bViewToProjectionOverride = false;

		ComputeViewMatrices(&matrixView, &matrixProjection, &matrixWorldToScreen, g_viewSetup);
					 ...
					 float *pWaterReflectionHeight /* = NULL */)
{
	...
	if(host_state.worldbrush->leafs[leaf].contents & CONTENTS_SOLID || cl.ishltv || cl.isreplay ||
	   !cl.m_bAreaBitsValid || r_portalsopenall.GetBool())
#else
	if(host_state.worldbrush->leafs[leaf].contents & CONTENTS_SOLID || cl.ishltv || !cl.m_bAreaBitsValid ||
