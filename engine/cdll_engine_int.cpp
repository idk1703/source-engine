...
void GetIntersectingSurfaces_R(GetIntersectingSurfaces_Struct *pStruct, mnode_t *pNode)
{
	if(pStruct->m_nSetInfos >= pStruct->m_nMaxInfos)
		return;
...
//-----------------------------------------------------------------------------
void DebugDrawPhysCollide(const CPhysCollide *pCollide, IMaterial *pMaterial, matrix3x4_t &transform,
						  const color32 &color, bool drawAxes)
{
	if(!pMaterial)
	{
...
public:
	...
	int GetIntersectingSurfaces(const model_t *model, const Vector &vCenter, const float radius,
	...
	IMaterial *TraceLineMaterialAndLighting(const Vector &start, const Vector &end, Vector &diffuseLightColor,
											Vector &baseColor);
	...
	virtual void DebugDrawPhysCollide(const CPhysCollide *pCollide, IMaterial *pMaterial, matrix3x4_t &transform,
									  const color32 &color);
	...
	{
		return GetSteamInfIDVersionInfo().szVersionString;
	}
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineClient, IVEngineClient013, VENGINE_CLIENT_INTERFACE_VERSION_13,
								  s_VEngineClient);
...
int CEngineClient::GetIntersectingSurfaces(const model_t *model, const Vector &vCenter, const float radius,
										   const bool bOnlyVisible, SurfInfo *pInfos, const int nMaxInfos)
{
	if(!model)
		return 0;
...
void CEngineClient::ClientCmd(const char *szCmdString)
{
	...
	{
		AssertMsg(false,
				  "CEngineClient::ClientCmd called but there is no room for the execution markers. Ignoring command.");
		return;
	}

	// Only let them run commands marked with FCVAR_CLIENTCMD_CAN_EXECUTE.
	if(cl.m_bRestrictClientCommands)
		Cbuf_AddTextWithMarkers(eCmdExecutionMarker_Enable_FCVAR_CLIENTCMD_CAN_EXECUTE, szCmdString,
...
{
	return modelloader->GetModelForName(pName, bProp ? IModelLoader::FMODELLOADER_DETAILPROP
...
{
	// FIXME: this is only valid if we're currently rendering.  If not, it should use the player, or it really should
	// pass one in.
...
// Computes light due to dynamic lighting at a point
// If the normal isn't specified, then it'll return the maximum lighting
void CEngineClient::ComputeLighting(const Vector &pt, const Vector *pNormal, bool bClamp, Vector &color,
									Vector *pBoxColors)
{
...
void CEngineClient::DebugDrawPhysCollide(const CPhysCollide *pCollide, IMaterial *pMaterial, matrix3x4_t &transform,
										 const color32 &color)
{
...
{
	return IsPlayingDemo() && demoplayer && demoplayer->GetDemoFile() &&
...
static void ClientDLL_InitRecvTableMgr()
{
	...
	{
		ErrorIfNot(nRecvTables < ARRAYSIZE(pRecvTables), ("ClientDLL_InitRecvTableMgr: overflowed MAX_DATATABLES"));
...
bool ClientDLL_Load()
{
	...
	// NOTE: Its OK if this returns NULL, as some mods won't provide the interface and will just use the default
...
{
	// This can go away when we ship a client DLL with the FCVAR_CLIENTCMD_CAN_EXECUTE flag set on these
	// cvars/concommands.
...
void ClientDLL_Init(void)
{
	...
	if(g_ClientDLL)
	{
		...
		if(g_ClientFactory)
		{
			...
			{
				clientleafsystem =
					(IClientLeafSystemEngine *)g_ClientFactory(CLIENTLEAFSYSTEM_INTERFACE_VERSION_1, NULL);
				if(!clientleafsystem)
				{
		...
		{
			if((Q_stricmp(COM_GetModDirectory(), "tf") == 0) || (Q_stricmp(COM_GetModDirectory(), "ep2") == 0) ||
			   (Q_stricmp(COM_GetModDirectory(), "portal") == 0) || (Q_stricmp(COM_GetModDirectory(), "tf_beta") == 0))
			{
