...
#define S2A_EXTRA_DATA_HAS_SPECTATOR_DATA \
	0x40									 // Next 2 bytes include the spectator port, then the spectator server name.
...
static ConVar tv_password("tv_password", "", FCVAR_NOTIFY | FCVAR_PROTECTED | FCVAR_DONTRECORD,
...
static ConVar tv_dispatchmode("tv_dispatchmode", "1", 0,
...
void CHLTVServer::InitClientRecvTables()
{
	...
	for(pCur = serverGameDLL->GetAllServerClasses(); pCur; pCur = pCur->m_pNext)
	{
		ErrorIfNot(m_nRecvTables < ARRAYSIZE(m_pRecvTables), ("ClientDLL_InitRecvTableMgr: overflowed MAX_DATATABLES"));
...
void CHLTVFrame::CopyHLTVData(CHLTVFrame &frame)
{
	...
	if(bits > 0)
	{
		...
		m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(frame.m_Messages[HLTV_BUFFER_UNRELIABLE].GetData(),
													 frame.m_Messages[HLTV_BUFFER_UNRELIABLE].GetNumBitsWritten());
		m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(frame.m_Messages[HLTV_BUFFER_TEMPENTS].GetData(),
													 frame.m_Messages[HLTV_BUFFER_TEMPENTS].GetNumBitsWritten());
		m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(frame.m_Messages[HLTV_BUFFER_SOUNDS].GetData(),
													 frame.m_Messages[HLTV_BUFFER_SOUNDS].GetNumBitsWritten());

		if(tv_relayvoice.GetBool())
			m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(frame.m_Messages[HLTV_BUFFER_VOICE].GetData(),
...
void CHLTVServer::InstallStringTables(void)
{
	...
	for(int i = 0; i < numTables; i++)
	{
		...
		CNetworkStringTable *hltvTable = (CNetworkStringTable *)m_StringTables->CreateStringTableEx(
			serverTable->GetTableName(), serverTable->GetMaxStrings(), serverTable->GetUserDataSize(),
			serverTable->GetUserDataSizeBits(), serverTable->HasFileNameStrings());
...
CHLTVEntityData *FindHLTVDataInSnapshot(CFrameSnapshot *pSnapshot, int iEntIndex)
{
	...
	if(iEntIndex < pSnapshot->m_pValidEntities[a] || iEntIndex > pSnapshot->m_pValidEntities[z])
		return NULL;
...
{
	bf_read msg = packet->message; // We're copying the message, so we don't need to seek back when passing packet to
								   // the base class.
	...
	while(true)
	{
		...
		switch(cmd)
		{
			...
			case dem_stringtables:
			{
				...
				{
					Warning("ReadCompleteDemoFile failed to read string tables. Trying to read string tables that's "
							"bigger than max string table size\n");
				}
...
{
	IClient *client = (CHLTVClient *)CBaseServer::ConnectClient(adr, protocol, challenge, clientChallenge, authProtocol,
																name, password, hashedCDkey, cdKeyLen);
...
CON_COMMAND(tv_status, "Show SourceTV server status.")
{
	...
	ConMsg("Game Time %s, Mod \"%s\", Map \"%s\", Players %i\n", COM_FormatSeconds(hltv->GetTime()), gd,
		   hltv->GetMapName(), hltv->GetNumPlayers());
...
CON_COMMAND(tv_clients, "Shows list of connected SourceTV clients.")
{
	...
	for(int i = 0; i < hltv->GetClientCount(); i++)
	{
		...
		ConMsg("ID: %i, \"%s\" %s, Time %s, %s, In %.1f, Out %.1f.\n", client->GetUserID(), client->GetClientName(),
			   client->IsHLTV() ? "(Relay)" : "", COM_FormatSeconds(netchan->GetTimeConnected()), netchan->GetAddress(),
			   netchan->GetAvgData(FLOW_INCOMING) / 1024, netchan->GetAvgData(FLOW_OUTGOING) / 1024);
