...
static ConVar dt_UsePartialChangeEnts("dt_UsePartialChangeEnts", "1", 0,
...
template<class T>
inline void LocalTransfer_FastType(T *pBlah, CServerDatatableStack &serverStack, CClientDatatableStack &clientStack,
								   CFastLocalTransferPropInfo *pPropList, int nProps)
{
...
public:
	...
	{
		if(fn == m_pSendProxies->m_DataTableToDataTable || fn == m_pSendProxies->m_SendLocalDataTable)
		{
			return true;
		}
...
void LocalTransfer_InitFastCopy(
	const SendTable *pSendTable, const CStandardSendProxies *pSendProxies, RecvTable *pRecvTable,
	...
	int &nFastCopyProps)
{
	...
	for(int iProp = 0; iProp < iNumProp; iProp++)
	{
		...
		if(pRecvProp)
		{
			...
			else if(pSendProp->GetType() == DPT_Float && pSendProp->GetProxyFn() == pSendProxies->m_FloatToFloat &&
			...
			else if(pSendProp->GetType() == DPT_Vector && pSendProp->GetProxyFn() == pSendProxies->m_VectorToVector &&
...
inline int MapPropOffsetsToIndices(const CBaseEdict *pEdict, CSendTablePrecalc *pPrecalc,
								   const unsigned short *pOffsets, unsigned short nOffsets, unsigned short *pOut)
{
	int iOut = 0;
	...
	for(unsigned short i = 0; i < nOffsets; i++)
	{
		...
		if(index == pPrecalc->m_PropOffsetToIndexMap.InvalidIndex())
		{
			...
			if(dt_ShowPartialChangeEnts.GetInt())
			{
				...
				char str[512];
				Q_snprintf(str, sizeof(str), "LocalTransfer offset miss - class: %s, DT: %s, offset: %d",
						   pEdict->GetClassName(), pPrecalc->m_pSendTable->m_pNetTableName, pOffsets[i]);
...
void PrintPartialChangeEntsList()
{
	...
	Con_NPrintf(iCurRow++, "");
	Con_NPrintf(iCurRow++, "Ent changes: %3d, prop changes: %3d", g_nTotalEntChanges, g_nTotalPropChanges);
	Con_NPrintf(iCurRow++, "");
...
void LocalTransfer_TransferEntity(const CBaseEdict *pEdict, const SendTable *pSendTable, const void *pSrcEnt,
								  RecvTable *pRecvTable, void *pDestEnt, bool bNewlyCreated, bool bJustEnteredPVS,
								  int objectID)
{
	++g_nTotalEntChanges;
	...
	// This code tries to only copy fields expressly marked as "changed" (by having the field offsets added to the
	// changeoffsets vectors)
	if(pEdict->GetChangeInfoSerialNumber() == g_pSharedChangeInfo->m_iSerialNumber && !bNewlyCreated &&
	   !bJustEnteredPVS && dt_UsePartialChangeEnts.GetInt())
	{
		CSendTablePrecalc *pPrecalc = pSendTable->m_pPrecalc;

		int nChangeOffsets =
			MapPropOffsetsToIndices(pEdict, pPrecalc, pCI->m_ChangeOffsets, pCI->m_nChangeOffsets, propIndices);
		if(nChangeOffsets == 0)
			return;
	...
	// Whereas the below code copies _all_ fields, regardless of whether they were changed or not.  We run this only
	// newly created entities, or entities
	//  which were previously dormant/outside the pvs but are now back in the PVS since we could have missed field
	//  updates since the changeoffsets get cleared every frame.
	else
	{
		// Setup the structure to traverse the source tree.
		CSendTablePrecalc *pPrecalc = pSendTable->m_pPrecalc;
		...
		LocalTransfer_FastType((int *)0, serverStack, clientStack, pPrecalc->m_FastLocalTransfer.m_FastInt32.Base(),
							   pPrecalc->m_FastLocalTransfer.m_FastInt32.Count());
		LocalTransfer_FastType((short *)0, serverStack, clientStack, pPrecalc->m_FastLocalTransfer.m_FastInt16.Base(),
							   pPrecalc->m_FastLocalTransfer.m_FastInt16.Count());
		LocalTransfer_FastType((char *)0, serverStack, clientStack, pPrecalc->m_FastLocalTransfer.m_FastInt8.Base(),
							   pPrecalc->m_FastLocalTransfer.m_FastInt8.Count());
		LocalTransfer_FastType((Vector *)0, serverStack, clientStack, pPrecalc->m_FastLocalTransfer.m_FastVector.Base(),
