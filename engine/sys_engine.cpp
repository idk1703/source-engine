...
int Sys_InitGame(CreateInterfaceFn appSystemFactory, char const *pBaseDir, void *pwnd, int bIsDedicated);
...
ConVar fps_max("fps_max", DEFAULT_FPS_MAX_S, FCVAR_NOT_CONNECTED,
			   "Frame rate limiter, cannot be set while connected to a server.", fps_max_callback);
...
static ConVar mat_powersavingsmode("mat_powersavingsmode", "0", FCVAR_ARCHIVE, "Power Savings Mode",
								   mat_powersavingsmode_callback);
...
ConVar cpu_frequency_monitoring("cpu_frequency_monitoring", "0", 0,
								"Set CPU frequency monitoring interval in seconds. Zero means disabled.", true, 0.0f,
								true, 10.0f, cpu_frequency_monitoring_callback);
#endif
...
bool CEngine::Load(bool bDedicated, const char *rootdir)
{
	...
	if(Sys_InitGame(g_AppSystemFactory, rootdir, game->GetMainWindowAddress(), bDedicated))
	{
		success = true;
...
bool CEngine::FilterTime(float dt)
{
	...
	if(fps > 0.0f)
	{
		...
		//		fps = max( MIN_FPS, fps ); // red herring - since we're only checking if dt < 1/fps, clamping against
		//MIN_FPS has no effect
...
#endif
			!g_bDedicatedServerBenchmarkMode && dt < minframetime)
		{
