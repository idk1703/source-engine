...
public:
	...
	int GetDispIndex()
	{
		return this - m_pDispArray->m_pDispInfos;
	}
	...
	bool TestRay(Ray_t const &ray, float start, float end, float &dist, Vector2D *lightmapUV, Vector2D *texureUV);
...
public:
	...
	bool CopyCoreDispData(model_t *pWorld, const MaterialSystem_SortInfo_t *pSortInfos, const CCoreDispInfo *pInfo,
						  bool bRestoring);
...
public:
	...
	void CullDecals(int iNodeBit, CDispDecal **decals, int nDecals, CDispDecal **childDecals, int &nChildDecals);
	...
	void ComputeLightmapAndTextureCoordinate(RayDispOutput_t const &output, Vector2D *luv, Vector2D *tuv);

	// This little beastie generate decal fragments
	void GenerateDecalFragments(CVertIndex const &nodeIndex, int iNodeBitIndex, unsigned short decalHandle,
...
private:
	...
	void GenerateDecalFragments_R(CVertIndex const &nodeIndex, int iNodeBitIndex, unsigned short decalHandle,
								  CDispDecalBase *pDispDecal, int iLevel);

	// Used to create a bitfield to help cull decal tests
	void SetupDecalNodeIntersect(CVertIndex const &nodeIndex, int iNodeBitIndex, CDispDecalBase *pDispDecal,
								 ShadowInfo_t const *pInfo);

	// Used by SetupDecalNodeIntersect
	bool SetupDecalNodeIntersect_R(CVertIndex const &nodeIndex, int iNodeBitIndex, CDispDecalBase *pDispDecal,
