...
public:
	...
	// Currently the engine doesn't like to do networking when it's paused, but if a tool changes entity state, it can
	// be useful to force
	...
	virtual int StartSound(int iUserData, bool staticsound, int iEntIndex, int iChannel, const char *pSample,
						   ...
						   int iFlags = 0, int iPitch = PITCH_NORM, bool bUpdatePositions = true, float delay = 0.0f,
						   int speakerentity = -1);
	...
	virtual SpatialPartitionHandle_t CreatePartitionHandle(IHandleEntity *pEntity, SpatialPartitionListMask_t listMask,
...
int CEngineTool::StartSound(int iUserData, bool staticsound, int iEntIndex, int iChannel, const char *pSample,
							float flVolume, soundlevel_t iSoundlevel, const Vector &origin, const Vector &direction,
							int iFlags /*= 0*/, int iPitch /*= PITCH_NORM*/, bool bUpdatePositions /*= true*/,
...
bool CEngineTool::GetPlayerView(CViewSetup &viewSetup, int x, int y, int w, int h)
{
	if(g_ClientDLL)
	{
		...
		viewSetup.m_flAspectRatio =
			(viewSetup.height != 0) ? (float)viewSetup.width / (float)viewSetup.height : 4.0f / 3.0f;
		...
		viewSetup.fovViewmodel =
			ScaleFOVByWidthRatio(viewSetup.fovViewmodel, viewSetup.m_flAspectRatio / (4.0f / 3.0f));
		return true;
	}
	return false;
}
...
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s(%s, %s)", __FUNCTION__, tmDynamicString(TELEMETRY_LEVEL0, pName),
		   bPreload ? "true" : "false");
...
SpatialPartitionHandle_t CEngineTool::CreatePartitionHandle(IHandleEntity *pEntity, SpatialPartitionListMask_t listMask,
															const Vector &mins, const Vector &maxs)
{
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineTool, IEngineToolFramework, VENGINETOOLFRAMEWORK_INTERFACE_VERSION,
								  g_EngineTool);