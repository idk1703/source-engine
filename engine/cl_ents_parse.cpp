...
static ConVar cl_flushentitypacket("cl_flushentitypacket", "0", FCVAR_CHEAT,
...
{
	ErrorIfNot(u.m_nPostDataUpdateCalls < MAX_EDICTS, ("CL_AddPostDataUpdateCall: overflowed u.m_PostDataUpdateCalls"));
...
void CL_CopyNewEntity(CEntityReadInfo &u, int iClass, int iSerialNum)
{
	...
	if(u.m_bUpdateBaselines)
	{
		...
		cl.SetEntityBaseline((u.m_nBaseline == 0) ? 1 : 0, pClass, u.m_nNewEntity, packedData,
...
bool CL_ProcessPacketEntities(SVC_PacketEntities *entmsg)
{
	...
	if(entmsg->m_bIsDelta)
	{
		...
		// If we have cl_debug_player_perf set and we see a huge delta between what we've ack'd to the server and where
		// it's at
		//  ask it for an instantaneous perf snapshot
		if(cl_debug_player_perf.GetBool() && (flDeltaSeconds > 0.5f) && // delta is pretty out of date
...
void CL_PreprocessEntities(void)
{
	...
	// We always want to re-run prediction when using the multiplayer networking, or if we're the listen server and we
	// get a packet
	//  before any frames have run
	if(bIsUsingMultiplayerNetworking || bLastOutgoingCommandEqualsLastAcknowledgedCommand)
	{
