...
float Engine_WorldLightAngle(const dworldlight_t *wl, const Vector &lnormal, const Vector &snormal,
							 const Vector &delta);
...
ConVar r_oldlightselection("r_oldlightselection", "0", FCVAR_CHEAT,
						   "Set this to revert to HL2's method of selecting lights");

static void ComputeAmbientFromSphericalSamples(const Vector &start, Vector *lightBoxColor);
...
public:
	...
	bool HasLightStyle()
	{
		return (m_LightingFlags &
				(HACKLIGHTCACHEFLAGS_HASSWITCHABLELIGHTSTYLE | HACKLIGHTCACHEFLAGS_HASNONSWITCHABLELIGHTSTYLE))
				   ? true
				   : false;
	}
...
public:
	...
	int m_DLightMarkFrame; // last frame in which a dlight was marked on this prop (helps detect lights that are marked
						   // but have moved away from this prop)
	...
	bool HasDlights()
	{
		return m_DLightActive ? true : false;
	}
	PropLightcache_t()
	{
		m_Flags = 0;
...
ConVar r_radiosity("r_radiosity", "4", FCVAR_CHEAT,
				   ...
				   "samples for static props, 6 samples for everything else");
...
void R_StudioCheckReinitLightingCache()
{
	...
	if(cached_r_worldlights != r_worldlights.GetInt() || cached_r_radiosity != r_radiosity.GetInt() ||
	   cached_r_avglight != r_avglight.GetInt() || cached_mat_fullbright != g_pMaterialSystemConfig->nFullbright ||
	   cached_r_lightcache_numambientsamples != r_lightcache_numambientsamples.GetInt())
	{
...
//-----------------------------------------------------------------------------
static void ComputeAmbientFromSurface(SurfaceHandle_t surfID, dworldlight_t *pSkylight, Vector &radcolor)
{
...
//-----------------------------------------------------------------------------
static void ComputeAmbientFromAxisAlignedSamples(const Vector &start, Vector *lightBoxColor)
{
	Vector upend;
...
//-----------------------------------------------------------------------------
static void R_StudioGetAmbientLightForPoint(int leafID, const Vector &start, Vector *pLightBoxColor, bool bIsStaticProp,
...
static float LightIntensityAndDirectionAtPointOld(dworldlight_t *pLight, const Vector &mid, int fFlags,
												  IHandleEntity *pIgnoreEnt, Vector *pDirection)
{
...
//-----------------------------------------------------------------------------
static float LightIntensityAndDirectionAtPointNew(dworldlight_t *pLight, lightzbuffer_t *pZBuf, const Vector &mid,
												  int fFlags, IHandleEntity *pIgnoreEnt, Vector *pDirection)
{
	CTraceFilterWorldOnly worldTraceFilter;
	CTraceFilterWorldAndProps propTraceFilter(pIgnoreEnt);
	ITraceFilter *pTraceFilter = &worldTraceFilter;
...
static float LightIntensityAndDirectionAtPoint(dworldlight_t *pLight, lightzbuffer_t *pZBuf, const Vector &mid,
											   int fFlags, IHandleEntity *pIgnoreEnt, Vector *pDirection)
{
	if(pZBuf)
...
// a particular point. PVS is used to speed up the task.
//-----------------------------------------------------------------------------
static float LightIntensityAndDirectionInBox(dworldlight_t *pLight, lightzbuffer_t *pZBuf, const Vector &mid,
	...
	if(!r_oldlightselection.GetBool())
	{
		switch(pLight->type)
		{
			case emit_spotlight: // directional & positional
			{
				...
				if(!IsSphereIntersectingCone(mid, sphereRadius, pLight->origin, pLight->normal, sinAngle,
											 pLight->stopdot2))
					return 0;
			}
...
//-----------------------------------------------------------------------------
bool ComputeVertexLightingFromSphericalSamples(const Vector &vecVertex, const Vector &vecNormal,
											   IHandleEntity *pIgnoreEnt, Vector *pLinearColor)
{
	if(IsX360())
		return false;
	...
	for(i = 0; i < host_state.worldbrush->numworldlights; ++i)
	{
		...
		float flRatio =
			LightIntensityAndDirectionAtPoint(wl, NULL, vecVertex, LIGHT_OCCLUDE_VS_PROPS, pIgnoreEnt, &vecDirection);
...
//-----------------------------------------------------------------------------
static void AddWorldLightToLightCube(dworldlight_t *pWorldLight, Vector *pBoxColor, const Vector &direction,
									 float ratio)
{
	if(ratio == 0.0f)
		return;
...
static inline const byte *FastRejectLightSource(bool bIgnoreVis, const byte *pVis, const Vector &bucketOrigin,
												int lightType, int lightCluster, bool &bReject)
{
	bReject = false;
	if(!bIgnoreVis)
	{
...
//-----------------------------------------------------------------------------
static const byte *AddWorldLightToLightingState(dworldlight_t *pWorldLight, lightzbuffer_t *pZBuf,
												...
												bool bIgnoreVis = false, bool bIgnoreVisTest = false)
{
	...
	else
	{
		...
		ratio = LightIntensityAndDirectionInBox(pWorldLight, pZBuf, bucketOrigin, mins, maxs,
...
//-----------------------------------------------------------------------------
static void WorldLightFromDynamicLight(dlight_t const &dynamicLight, dworldlight_t &worldLight)
{
...
//-----------------------------------------------------------------------------
static const byte *ComputeLightStyles(lightcache_t *pCache, LightingState_t &lightingState, const Vector &origin,
									  int leaf, const byte *pVis)
{
...
static const byte *AddDLights(LightingStateInfo_t &info, LightingState_t &lightingState, const Vector &origin, int leaf,
							  const byte *pVis)
{
	if(!g_bActiveDlights)
		return pVis;
	...
	for(int i = 0; i < MAX_DLIGHTS; ++i, ++dl)
	{
		...
		// Now add that world light into our list of worldlights
		pVis = AddWorldLightToLightingState(&s_pDynamicLight[i], NULL, lightingState, info, origin, pVis, true,
											bIgnoreVis, bIgnoreVisTest);
	}
	return pVis;
}

static const byte *AddELights(LightingStateInfo_t &info, LightingState_t &lightingState, const Vector &origin, int leaf,
							  const byte *pVis)
{
	if(!g_bActiveElights)
		return pVis;
	...
	for(int i = 0; i < MAX_ELIGHTS; ++i, ++dl)
	{
		...
		// Now add that world light into our list of worldlights
		pVis = AddWorldLightToLightingState(&s_pDynamicLight[i + MAX_DLIGHTS], NULL, lightingState, info, origin, pVis,
											true, bIgnoreVis, bIgnoreVisTest);
	}
	return pVis;
}
...
//-----------------------------------------------------------------------------
static const byte *ComputeDynamicLighting(lightcache_t *pCache, LightingState_t &lightingState, const Vector &origin,
										  int leaf, const byte *pVis = 0)
{
...
// Adds a world light to the list of lights
//-----------------------------------------------------------------------------
static void AddWorldLightToLightingStateForStaticProps(dworldlight_t *pWorldLight, LightingState_t &lightingState,
													   LightingStateInfo_t &info, PropLightcache_t *pCache,
													   bool dynamic = false)
{
	// FIXME: This is sort of a hack; only one skylight is allowed in the
	// lighting...
	...
	else
	{
		...
		ratio = LightIntensityAndDirectionInBox(pWorldLight, NULL, pCache->m_LightingOrigin, pCache->mins, pCache->maxs,
									 ...
									 PropLightcache_t *pCache)
{
	...
	for(int i = 0; i < MAX_DLIGHTS; ++i, ++dl)
	{
		...
		// Now add that world light into our list of worldlights
		AddWorldLightToLightingStateForStaticProps(&s_pDynamicLight[i], lightingState, info, pCache, true);
	}
}
...
static void AddStaticLighting(CBaseLightCache *pCache, const Vector &origin, const byte *pVis, bool bStaticProp,
							  bool bAddedLeafAmbientCube)
{
	...
	for(i = 0; i < host_state.worldbrush->numworldlights; ++i)
	{
		...
		// Don't add lights without lightstyles... we cache static lighting + lightstyles separately from static
		// lighting
		if(wl->style == 0)
		{
			// Now add that world light into our list of worldlights
			AddWorldLightToLightingState(wl, pZBuf, pCache->m_StaticLightingState, *pCache, origin, pVis, false, false);
		}
		else
		{
			...
			if(!bStaticProp)
			{
				...
				if(!(pCache->m_pLightstyles[byte] & (1 << bit)))
				{
					...
					float ratio = LightIntensityAndDirectionInBox(
						wl, NULL, origin, mins, maxs, LIGHT_NO_OCCLUSION_CHECK | LIGHT_IGNORE_LIGHTSTYLE_VALUE,
						&dummyDirection);
...
static void DebugRenderLightcache(Vector &sampleOrigin, LightingState_t &lightingState, bool bDebugModel)
{
	...
	if(r_drawlightcache.GetInt() == 5)
	{
		if(bDebugModel)
		{
			CDebugOverlay::AddSphereOverlay(sampleOrigin, 2.5f, 32, 32, 255, 255, 255, 255,
											0.0f); // 8 inch solid white sphere
	...
	else
	{
		...
		for(int j = 0; j < lightingState.numlights; ++j)
		{
			...
			CDebugOverlay::AddLineOverlay(sampleOrigin, vLightPosition, r, g, b, 255, true, 0.0f);
			CDebugOverlay::AddBoxOverlay(lightingState.locallight[j]->origin, lightMins, lightMaxs, vec3_angle, 255,
										 255, 100, 0, 0.0f);
		}
	}
#endif
...
// Compute the cache...
//-----------------------------------------------------------------------------
static const byte *ComputeStaticLightingForCacheEntry(CBaseLightCache *pcache, const Vector &origin, int leaf,
													  bool bStaticProp = false)
{
	...
	R_StudioGetAmbientLightForPoint(leaf, origin, pcache->m_StaticLightingState.r_boxcolor, bStaticProp,
...
static void BuildStaticLightingCacheLightStyleInfo(PropLightcache_t *pcache, const Vector &mins, const Vector &maxs)
{
	...
	pcache->m_LightingFlags &=
		~(HACKLIGHTCACHEFLAGS_HASSWITCHABLELIGHTSTYLE | HACKLIGHTCACHEFLAGS_HASSWITCHABLELIGHTSTYLE);
	// clear lightstyles
...
LightingState_t *LightcacheGetStatic(LightCacheHandle_t cache, ITexture **pEnvCubemapTexture, unsigned int flags)
{
	...
	bool bRecalcLightStyles = (pcache->HasLightStyle() && pcache->m_LastFrameUpdated_LightStyles != r_framecount) &&
...
inline const byte *AddLightingState(LightingState_t &dst, const LightingState_t &src, LightingStateInfo_t &info,
									const Vector &bucketOrigin, const byte *pVis, bool bDynamic, bool bIgnoreVis)
{
	int i;
	for(i = 0; i < src.numlights; i++)
	{
		pVis =
			AddWorldLightToLightingState(src.locallight[i], NULL, dst, info, bucketOrigin, pVis, bDynamic, bIgnoreVis);
	}
	for(i = 0; i < 6; i++)
	{
...
const byte *PrecalcLightingState(lightcache_t *pCache, const byte *pVis)
{
	...
	{
		pVis = AddWorldLightToLightingState(pCache->m_StaticLightingState.locallight[i], NULL, lightingState,
											pCache->m_StaticPrecalc_LightingStateInfo, pCache->m_LightingOrigin, pVis,
											true, // bDynamic
											false // bIgnoreVis
		);
	}
...
ITexture *LightcacheGetDynamic(const Vector &origin, LightingState_t &lightingState, LightcacheGetDynamic_Stats &stats,
							   unsigned int flags, bool bDebugModel)
{
	...
	*/
	...
	// NOTE: On a cache miss, this has to be after ComputeStaticLightingForCacheEntry since these flags are computed
	// there.
	...
	{
		pVis =
			ComputeLightStyles(pCache, pCache->m_LightStyleLightingState, pCache->m_LightingOrigin, originLeaf, pVis);
	...
	{
		pVis =
			ComputeDynamicLighting(pCache, pCache->m_DynamicLightingState, pCache->m_LightingOrigin, originLeaf, pVis);
...
void ComputeDynamicLighting(const Vector &pt, const Vector *pNormal, Vector &color)
{
	...
	// Next, add each world light with a lightstyle into the lighting state,
	// ejecting less relevant local lights + folding them into the ambient cube
	static Vector ambientTerm[6] = {Vector(0, 0, 0), Vector(0, 0, 0), Vector(0, 0, 0),
	...
	if(lightCount)
	{
		g_pStudioRender->ComputeLighting(ambientTerm, lightCount, pLightDesc, pt, *pNormal, color);
	}
	else
	{
		VectorFill(color, 0);
...
void ComputeLighting(const Vector &pt, const Vector *pNormal, bool bClamp, Vector &color, Vector *pBoxColors)
{
	...
	if(pNormal)
	{
		...
		g_pStudioRender->ComputeLighting(lightingState.r_boxcolor, lightingState.numlights, pLightDesc, pt, *pNormal,
										 color);
	}
	else
	{
		...
		for(i = 0; i < lightingState.numlights; ++i)
		{
			...
			float ratio = LightIntensityAndDirectionAtPoint(lightingState.locallight[i], NULL, pt,
															LIGHT_NO_OCCLUSION_CHECK, NULL, &direction);
			float angularRatio = Engine_WorldLightAngle(lightingState.locallight[i],
														lightingState.locallight[i]->normal, direction, direction);
			AddWorldLightToLightCube(lightingState.locallight[i], lightingState.r_boxcolor, direction,
									 ratio * angularRatio);
		}
...
public:
	...
	virtual IterationRetval_t EnumElement(IHandleEntity *pHandleEntity)
	{
		...
		PropLightcache_t *pCache = (PropLightcache_t *)StaticPropMgr()->GetLightCacheHandleForStaticProp(pHandleEntity);
		...
		if(r_drawlightcache.GetInt() == 4)
		{
			Vector mins(-5, -5, -5);
			Vector maxs(5, 5, 5);
			CDebugOverlay::AddLineOverlay(cl_dlights[m_nLightID].origin, pCache->m_LightingOrigin, 0, 0, 255, 255, true,
										  0.001f);
...
void MarkDLightsOnStaticProps(void)
{
	...
	for(int i = 0; i < MAX_DLIGHTS; i++, l++)
	{
		...
		SpatialPartition()->EnumerateElementsInSphere(PARTITION_ENGINE_STATIC_PROPS, l->origin, l->GetRadius(), true,
