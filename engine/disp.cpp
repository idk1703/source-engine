...
inline void CDispInfo::DecalProjectVert(Vector const &vPos, CDispDecalBase *pDecalBase, ShadowInfo_t const *pInfo,
										Vector &out)
{
	if(!pInfo)
	{
...
void CDispInfo::TestAddDecalTri(int iIndexStart, unsigned short decalHandle, CDispDecal *pDispDecal)
{
	...
	unsigned short tempIndices[3] = {(unsigned short)(m_MeshReader.Index(iIndexStart + 0) - m_iVertOffset),
...
#ifndef SWDS
	// Setup verts.
	Vector vPositions[3] = {GetOverlayPos(&m_MeshReader, tempIndices[0]), GetOverlayPos(&m_MeshReader, tempIndices[1]),
...
void CDispInfo::CullDecals(int iNodeBit, CDispDecal **decals, int nDecals, CDispDecal **childDecals, int &nChildDecals)
{
...
{
	if(ShouldDrawInWireFrameMode() || mat_luxels.GetInt() || r_DispWalkable.GetInt() || r_DispBuildable.GetInt()
#if !defined(SWDS)
...
bool CDispInfo::Render(CGroupMesh *pGroup, bool bAllowDebugModes)
{
	...
	if(bAllowDebugModes)
	{
		...
		if(mat_surfaceid.GetInt())
		{
			...
			int nInt = (mat_surfaceid.GetInt() != 2) ? (intp)m_ParentSurfID
													 : (msurface2_t *)m_ParentSurfID - host_state.worldbrush->surfaces2;
			char buf[32];
...
typedef void ProcessLightmapSampleFunc_t(const ProcessLightmapSampleData_t &data, const Vector &vPos,
										 const Vector &vNormal, const Vector &vTangentS, const Vector &vTangentT, int t,
										 int s, int tmax, int smax);
...
#ifndef DEDICATED
static void ProcessLightmapSample(const ProcessLightmapSampleData_t &data, const Vector &vPos, const Vector &vNormal,
								  const Vector &vTangentS, const Vector &vTangentT, int t, int s, int tmax, int smax)
{
	...
	if(distSqr < data.m_LightDistSqr)
	{
		...
		int index = t * smax + s;
		VectorMA(blocklights[0][index].AsVector3D(), scale, data.m_Intensity, blocklights[0][index].AsVector3D());
	}
}

static void ProcessLightmapSampleBumped(const ProcessLightmapSampleData_t &data, const Vector &vPos,
										const Vector &vNormal, const Vector &vTangentS, const Vector &vTangentT, int t,
										int s, int tmax, int smax)
{
	float distSqr = data.m_vLightOrigin.DistToSqr(vPos);
	if(distSqr < data.m_LightDistSqr)
	{
		...
		VectorMA(blocklights[0][index].AsVector3D(), scale * directionalAtten, data.m_Intensity,
				 blocklights[0][index].AsVector3D());
		directionalAtten = fpmax(0.0f, DotProduct(vLightVecTangent, g_localBumpBasis[0]));
		VectorMA(blocklights[1][index].AsVector3D(), scale * directionalAtten, data.m_Intensity,
				 blocklights[1][index].AsVector3D());
		directionalAtten = fpmax(0.0f, DotProduct(vLightVecTangent, g_localBumpBasis[1]));
		VectorMA(blocklights[2][index].AsVector3D(), scale * directionalAtten, data.m_Intensity,
				 blocklights[2][index].AsVector3D());
		directionalAtten = fpmax(0.0f, DotProduct(vLightVecTangent, g_localBumpBasis[2]));
		VectorMA(blocklights[3][index].AsVector3D(), scale * directionalAtten, data.m_Intensity,
...
//-----------------------------------------------------------------------------
static void ProcessLightmapSampleAlpha(const ProcessLightmapSampleData_t &data, const Vector &vPos,
									   const Vector &vNormal, const Vector &vTangentS, const Vector &vTangentT, int t,
									   int s, int tmax, int smax)
{
	float distSqr = data.m_vLightOrigin.DistToSqr(vPos);
...
public:
	...
	bool IsCached(int v)
	{
		return m_CacheIndex[v] == m_CurrentCacheIndex;
	}
	void MarkCached(int v)
	{
		m_CacheIndex[v] = m_CurrentCacheIndex;
	}
...
//-----------------------------------------------------------------------------
bool CDispInfo::SetupDecalNodeIntersect_R(CVertIndex const &nodeIndex, int iNodeBitIndex, CDispDecalBase *pDispDecal,
										  ShadowInfo_t const *pInfo, int iLevel, CDecalNodeSetupCache *pCache)
{
...
void CDispInfo::SetupDecalNodeIntersect(CVertIndex const &nodeIndex, int iNodeBitIndex, CDispDecalBase *pDispDecal,
...
void CDispInfo::ComputeLightmapAndTextureCoordinate(RayDispOutput_t const &output, Vector2D *luv, Vector2D *tuv)
{
#ifndef SWDS
	// lightmap coordinate
	if(luv)
	{
		ComputePointFromBarycentric(m_MeshReader.TexCoordVector2D(output.ndxVerts[0], DISP_LMCOORDS_STAGE),
									m_MeshReader.TexCoordVector2D(output.ndxVerts[1], DISP_LMCOORDS_STAGE),
									m_MeshReader.TexCoordVector2D(output.ndxVerts[2], DISP_LMCOORDS_STAGE), output.u,
									output.v, *luv);
		...
		materials->GetLightmapPageSize(SortInfoToLightmapPage(MSurf_MaterialSortID(m_ParentSurfID)), &lightmapPageWidth,
									   &lightmapPageHeight);
	...
	if(tuv)
	{
		...
		Vector2D basefaceUV;
		ComputePointFromBarycentric(baseUV[0], baseUV[1], baseUV[2], output.u, output.v, basefaceUV);

		// Convert the base face uv to a texture uv based on the base face texture coords
		TexCoordInQuadFromBarycentric(m_BaseSurfaceTexCoords[0], m_BaseSurfaceTexCoords[3], m_BaseSurfaceTexCoords[2],
									  m_BaseSurfaceTexCoords[1], basefaceUV, *tuv);
	}
#endif
}
...
bool CDispInfo::TestRay(Ray_t const &ray, float start, float end, float &dist, Vector2D *luv, Vector2D *tuv)
{
