...
public:
	...
	virtual void OnFinishedSending(INetChannel *pDest, const void *pUserData, int userDataLen, FileTransferID_t id)
	{
	...
	virtual void OnFileReceived(INetChannel *pChan, const void *pUserData, int userDataLength, const char *pFileData,
								int fileLength)
	{
	...
	void UpdatePendingFiles()
	{
		...
		for(int i = 0; i < sv.clients.Count(); i++)
		{
			...
			for(int iFile = 0; iFile < pClient->m_pLogoInfo->m_PendingFiles.Count(); iFile++)
			{
				...
				StartSending(pClient->GetNetChannel(), &lastCRC, sizeof(lastCRC), fileData.Base(), fileData.Count(),
							 sv_logo_rate.GetInt());
...
{
	for(int i = g_ServerFileTransferMgr.FirstIncoming(); i != g_ServerFileTransferMgr.InvalidIncoming();
