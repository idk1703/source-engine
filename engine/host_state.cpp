...
void HostState_GameShutdown()
{
	...
	if(g_HostState.m_currentState != HS_SHUTDOWN && g_HostState.m_currentState != HS_RESTART &&
...
{
	return (g_HostState.m_currentState == HS_SHUTDOWN || g_HostState.m_currentState == HS_RESTART ||
			g_HostState.m_currentState == HS_GAME_SHUTDOWN);
}
...
void CHostState::FrameUpdate(float time)
{
	...
	while(true)
	{
		...
		// shutting down
		if(oldState == HS_SHUTDOWN || oldState == HS_RESTART)
			break;
