...
static ConVar r_lightinterp("r_lightinterp", "5", FCVAR_CHEAT,
							"Controls the speed of light interpolation, 0 turns off interpolation");
static ConVar r_eyeglintlodpixels(
	"r_eyeglintlodpixels", "20.0", FCVAR_CHEAT,
	"The number of pixels wide an eyeball has to be before rendering an eyeglint.  Is a floating point value.");
ConVar r_rootlod("r_rootlod", "0", FCVAR_MATERIAL_SYSTEM_THREAD | FCVAR_ARCHIVE, "Root LOD", true, 0, true,
...
ConVar r_debugrandomstaticlighting(
	"r_debugrandomstaticlighting", "0", FCVAR_CHEAT,
...
static ConVar r_proplightingpooling("r_proplightingpooling", "-1.0", FCVAR_CHEAT,
									"0 - off, 1 - static prop color meshes are allocated from a single shared vertex "
									"buffer (on hardware that supports stream offset)");
...
bool WorldLightToMaterialLight(dworldlight_t *pWorldLight, LightDesc_t &light)
{
	...
	// No attenuation case..
	if((light.m_Attenuation0 == 0.0f) && (light.m_Attenuation1 == 0.0f) && (light.m_Attenuation2 == 0.0f))
	{
...
static void R_SetNonAmbientLightingState(int numLights, dworldlight_t *locallight[MAXLOCALLIGHTS], int *pNumLightDescs,
										 LightDesc_t *pLightDescs, bool bUpdateStudioRenderLights)
{
...
//-----------------------------------------------------------------------------
void R_ComputeLightingOrigin(IClientRenderable *pRenderable, studiohdr_t *pStudioHdr, const matrix3x4_t &matrix,
							 Vector &center)
{
...
// TODO: cone clipping calc's wont work for boxlight since the player asks for a single point.  Not sure what the volume
// is.
...
public:
	...
	virtual IMesh *GetSharedMesh()
	{
		return m_pMesh;
	}

	// Get a pointer to the start of the vertex buffer data
	virtual void *GetVertexBufferBase()
	{
		return m_pVertexBufferBase;
	}
	virtual int GetNumVertsAllocated()
	{
		return m_totalVerts;
	}
	...
	void DestroyResource()
	{
		...
		for(int i = 0; i < m_nMeshes; i++)
		{
			if(m_pMeshInfos[i].m_pPooledVBAllocator)
			{
				...
				m_pMeshInfos[i].m_pPooledVBAllocator->Deallocate(m_pMeshInfos[i].m_nVertOffsetInBytes,
																 m_pMeshInfos[i].m_nNumVerts);
			}
			else
			{
		...
		for(int i = 0; i < params.m_nMeshes; i++)
		{
			...
			if(params.m_pPooledVBAllocator != NULL)
			{
				...
				else
				{
					...
					data->m_ppTargets[i] = ((unsigned char *)params.m_pPooledVBAllocator->GetVertexBufferBase()) +
										   data->m_pMeshInfos[i].m_nVertOffsetInBytes;
				}
			}
			...
			if(data->m_pMeshInfos[i].m_pMesh == NULL)
			{
				...
				data->m_pMeshInfos[i].m_pMesh =
					pRenderContext->CreateStaticMesh(vertexFormat, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_COLOR);
...
class CModelRender : public IVModelRender, public CManagedDataCacheClient<CColorMeshData, colormeshparams_t>
{
public:
	...
	virtual int DrawModel(int flags, IClientRenderable *cliententity, ModelInstanceHandle_t instance, int entity_index,
						  const model_t *model, const Vector &origin, QAngle const &angles, int skin, int body,
						  int hitboxset, const matrix3x4_t *pModelToWorld, const matrix3x4_t *pLightingOffset);
	...
	virtual void AddDecal(ModelInstanceHandle_t handle, Ray_t const &ray, const Vector &decalUp, int decalIndex,
						  int body, bool noPokethru = false, int maxLODToDecal = ADDDECAL_TO_ALL_LODS);
	virtual void AddColoredDecal(ModelInstanceHandle_t handle, Ray_t const &ray, const Vector &decalUp, int decalIndex,
								 int body, Color cColor, bool noPokethru = false,
	...
	virtual matrix3x4_t *DrawModelShadowSetup(IClientRenderable *pRenderable, int body, int skin,
											  DrawModelInfo_t *pInfo, matrix3x4_t *pBoneToWorld);
	...
	virtual bool DrawModelSetup(ModelRenderInfo_t &pInfo, DrawModelState_t *pState, matrix3x4_t *pBoneToWorld,
...
private:
	...
	void DrawModelExecute(const DrawModelState_t &state, const ModelRenderInfo_t &pInfo,
						  matrix3x4_t *pCustomBoneToWorld = NULL);

	void InitColormeshParams(ModelInstance_t &instance, studiohwdata_t *pStudioHWData,
	...
	bool LoadStaticPropColorData(IHandleEntity *pProp, DataCacheHandle_t colorMeshHandle,
	...
	void StudioSetupLighting(const DrawModelState_t &state, const Vector &absEntCenter, LightCacheHandle_t *pLightcache,
							 bool bVertexLit, bool bNeedsEnvCubemap, bool &bStaticLighting, DrawModelInfo_t &drawInfo,
							 const ModelRenderInfo_t &pInfo, int drawFlags);

	// Time average the ambient term
	void TimeAverageAmbientLight(LightingState_t &actualLightingState, ModelInstance_t &inst, float flAttenFactor,
								 LightingState_t *pLightingState, const Vector *pLightingOrigin);

	// Old-style computation of vertex lighting
	void ComputeModelVertexLightingOld(mstudiomodel_t *pModel, matrix3x4_t &matrix,
	...
	void ComputeModelVertexLighting(IHandleEntity *pProp, mstudiomodel_t *pModel,
									OptimizedModel::ModelLODHeader_t *pVtxLOD, matrix3x4_t &matrix, Vector4D *pTempMem,
									color24 *pLighting);

	// Internal Decal
	void AddDecalInternal(ModelInstanceHandle_t handle, Ray_t const &ray, const Vector &decalUp, int decalIndex,
						  int body, bool bUseColor, Color cColor, bool noPokeThru, int maxLODToDecal);
...
// Time average the ambient term
//-----------------------------------------------------------------------------
void CModelRender::TimeAverageAmbientLight(LightingState_t &actualLightingState, ModelInstance_t &inst,
	...
	for(int i = 0; i < 6; ++i)
	{
		...
		if(pLightingOrigin)
		{
			...
			CDebugOverlay::AddLineOverlay(*pLightingOrigin + Vector(5, 5, 5), *pLightingOrigin + vecDir * 50,
										  255 * pLightingState->r_boxcolor[i].x, 255 * pLightingState->r_boxcolor[i].y,
		...
		inst.m_CurrentLightingState.r_boxcolor[i].x =
			clamp(inst.m_CurrentLightingState.r_boxcolor[i].x, 0.f, AMBIENT_MAX);
		inst.m_CurrentLightingState.r_boxcolor[i].y =
			clamp(inst.m_CurrentLightingState.r_boxcolor[i].y, 0.f, AMBIENT_MAX);
		inst.m_CurrentLightingState.r_boxcolor[i].z =
			clamp(inst.m_CurrentLightingState.r_boxcolor[i].z, 0.f, AMBIENT_MAX);
	}
	memcpy(&actualLightingState.r_boxcolor, &inst.m_CurrentLightingState.r_boxcolor,
														...
														int nEntIndex, const Vector *pLightingOrigin)
{
	...
	for(i = 0; i < nCurrLight; ++i)
	{
		...
		vecColor *= flRatio;
		CDebugOverlay::AddLineOverlay(*pLightingOrigin, pSourceLight[i]->origin, vecColor.x, vecColor.y, vecColor.z,
									  255, false, 5.0f);
#endif
	}
...
static ConVar r_ambientboost("r_ambientboost", "1", FCVAR_ARCHIVE,
...
static ConVar r_drawlightcache("r_drawlightcache", "0", FCVAR_CHEAT,
...
void CModelRender::StudioSetupLighting(const DrawModelState_t &state, const Vector &absEntCenter,
									   ...
									   bool &bStaticLighting, DrawModelInfo_t &drawInfo, const ModelRenderInfo_t &pInfo,
									   int drawFlags)
{
	...
	drawInfo.m_bStaticLighting =
		bStaticLighting && g_pMaterialSystemHardwareConfig->SupportsStaticPlusDynamicLighting();
		...
		if(r_decalstaticprops.GetBool() && pModelInst && drawInfo.m_bStaticLighting && bHasDecals)
		{
			for(int iCube = 0; iCube < 6; ++iCube)
			{
				drawInfo.m_vecAmbientCube[iCube] =
					pModelInst->m_AmbientLightingState.r_boxcolor[iCube] + lightingState.r_boxcolor[iCube];
			}
	...
	else if(bVertexLit)
	{
		...
		else
		{
			...
			if((pState->numlights > 0) && (pInfo.pModel->flags & MODELFLAG_STUDIOHDR_AMBIENT_BOOST) &&
			   r_ambientboost.GetBool())
			{
				...
				for(int i = 0; i < pState->numlights; i++)
				{
					...
					float denom = pState->locallight[i]->constant_attn + pState->locallight[i]->linear_attn * d +
				...
				if(avgCubeLuminance < r_ambientmin.GetFloat() &&
				   (avgCubeLuminance < (fDirectLight * r_ambientfraction.GetFloat())))
				{
					Vector vFinalAmbientCube[6];
					float fBoostFactor = min((fDirectLight * r_ambientfraction.GetFloat()) / maxCubeLuminance,
											 5.f); // boost no more than a certain factor
					for(int i = 0; i < 6; i++)
					{
		...
		R_SetNonAmbientLightingState(pState->numlights, pState->locallight, &drawInfo.m_nLocalLightCount,
...
int GetLightingConditions(const Vector &vecLightingOrigin, Vector *pColors, int nMaxLocalLights,
...
void DrawModelDebugOverlay(const DrawModelInfo_t &info, const DrawModelResults_t &results, const Vector &origin,
...
static ConVar r_drawmodelstatsoverlaymin(
	"r_drawmodelstatsoverlaymin", "0.1", FCVAR_ARCHIVE,
	"time in milliseconds that a model must take to render before showing an overlay in r_drawmodelstatsoverlay 2");
static ConVar r_drawmodelstatsoverlaymax(
	"r_drawmodelstatsoverlaymax", "1.5", FCVAR_ARCHIVE,
...
void DrawSavedModelDebugOverlays(void)
{
	...
	for(i = 0; i < s_SavedModelInfo.Count(); i++)
	{
		...
		if(t > min)
		{
			...
			DrawModelDebugOverlay(s_SavedModelInfo[i].m_ModelInfo, s_SavedModelInfo[i].m_ModelResults,
...
//-----------------------------------------------------------------------------
void CModelRender::DrawModelExecute(const DrawModelState_t &state, const ModelRenderInfo_t &pInfo,
									matrix3x4_t *pBoneToWorld)
{
#ifndef SWDS
	...
	bool bUsesBumpmapping = (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80) &&
	...
	bool bStaticLighting = (state.m_drawFlags & STUDIORENDER_DRAW_STATIC_LIGHTING) &&
						   (state.m_pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP) && (!bUsesBumpmapping) &&
	...
	if((bVertexLit || bNeedsEnvCubemap) && !bShadowDepth && !bSSAODepth)
	{
		...
		{
			if((m_ModelInstances[pInfo.instance].m_nFlags & MODEL_INSTANCE_HAS_STATIC_LIGHTING) &&
		...
		StudioSetupLighting(state, entOrigin, pLightCache, bVertexLit, bNeedsEnvCubemap, bStaticLighting, info, pInfo,
							state.m_drawFlags);
	}
	...
	g_pStudioRender->DrawModel(&results, info, pBoneToWorld, pFlexWeights, pFlexDelayedWeights, pInfo.origin,
							   drawFlags);
...
//-----------------------------------------------------------------------------
int CModelRender::DrawModel(int flags, IClientRenderable *pRenderable, ModelInstanceHandle_t instance, int entity_index,
							const model_t *pModel, const Vector &origin, const QAngle &angles, int skin, int body,
							int hitboxset, const matrix3x4_t *pModelToWorld, const matrix3x4_t *pLightingOffset)
{
...
// Input  : &pInfo -
//-----------------------------------------------------------------------------
bool CModelRender::DrawModelSetup(ModelRenderInfo_t &pInfo, DrawModelState_t *pState, matrix3x4_t *pCustomBoneToWorld,
								  matrix3x4_t **ppBoneToWorldOut)
{
...
int CModelRender::DrawModelExStaticProp(ModelRenderInfo_t &pInfo)
{
	...
	bool bStaticLighting =
		((drawFlags & STUDIORENDER_DRAW_STATIC_LIGHTING) && (state.m_pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP) &&
...
int CModelRender::DrawStaticPropArrayFast(StaticPropRenderInfo_t *pProps, int count, bool bShadowDepth)
{
	...
	for(int i = 0; i < modelList.Count(); i++)
	{
		...
		modelList[i].bStaticLighting =
			((modelList[i].pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP) != 0) && !bBumpMapped;
	...
	if(!bShadowDepth)
	{
		for(int i = 0; i < objectList.Count(); i++)
		{
			...
			bool bHasDecals =
				(m_ModelInstances[obj.instance].m_DecalHandle != STUDIORENDER_DECAL_INVALID) ? true : false;
			...
			if(bStaticLighting)
			{
				...
				if(pColorMeshData && (pColorMeshData->m_bColorMeshValid || pColorMeshData->m_bColorTextureValid))
				{
					...
					{
						if(CacheLock(
							   hColorMeshData)) // CacheCreate above will call functions that won't take place until
												// later. If color mesh isn't used right away, it could get dumped
						{
			...
			if((bVertexLit || bNeedsEnvCubemap))
			{
				...
				{
					if((m_ModelInstances[obj.instance].m_nFlags & MODEL_INSTANCE_HAS_STATIC_LIGHTING) &&
					...
					if(bHasDecals)
					{
						for(int iCube = 0; iCube < 6; ++iCube)
						{
							pDecalLightState->r_boxcolor[iCube] =
								m_ModelInstances[obj.instance].m_AmbientLightingState.r_boxcolor[iCube] +
				...
				else // !pLightcache
				{
					...
					LightcacheGetDynamic_Stats stats;
					pEnvCubemapTexture =
						LightcacheGetDynamic(vLightingOrigin, lightingState, stats, lightCacheFlags, false);
					Assert(lightingState.numlights >= 0 && lightingState.numlights <= MAXLOCALLIGHTS);
					pState = &lightingState;
	...
	if(lightObjects.Count())
	{
		for(int i = 0; i < lightObjects.Count(); i++)
		{
			...
			R_SetNonAmbientLightingState(pState->numlights, pState->locallight, &nLocalLightCount, localLightDescs,
										 true);
			info.m_pStudioHdr = model.pStudioHdr;
			info.m_pHardwareData = model.pStudioHWData;
			info.m_Skin = obj.skin;
...
// Shadow rendering
//-----------------------------------------------------------------------------
matrix3x4_t *CModelRender::DrawModelShadowSetup(IClientRenderable *pRenderable, int body, int skin,
												DrawModelInfo_t *pInfo, matrix3x4_t *pCustomBoneToWorld)
{
#ifndef SWDS
	...
	const bool bOk = pRenderable->SetupBones(pBoneToWorld, info.m_pStudioHdr->numbones,
...
void CModelRender::DrawModelShadow(IClientRenderable *pRenderable, const DrawModelInfo_t &info,
								   matrix3x4_t *pBoneToWorld)
{
#ifndef SWDS
...
void CModelRender::InitColormeshParams(ModelInstance_t &instance, studiohwdata_t *pStudioHWData,
									   colormeshparams_t *pColorMeshParams)
{
	...
	if((instance.m_nFlags & MODEL_INSTANCE_HAS_DISKCOMPILED_COLOR) &&
	   g_pMaterialSystemHardwareConfig->SupportsStreamOffset() && (r_proplightingpooling.GetInt() == 1))
	{
...
void CModelRender::ProtectColorDataIfQueued(DataCacheHandle_t hColorMesh)
{
	if(hColorMesh != DC_INVALID_HANDLE)
	{
		...
		if(pCallQueue)
		{
			if(CacheLock(hColorMesh)) // CacheCreate above will call functions that won't take place until later. If
									  // color mesh isn't used right away, it could get dumped
			{
...
//-----------------------------------------------------------------------------
void CModelRender::ComputeModelVertexLightingOld(mstudiomodel_t *pModel, matrix3x4_t &matrix,
												 const LightingState_t &lightingState, color24 *pLighting,
												 bool bUseConstDirLighting, float flConstDirLightAmount)
{
	...
	R_SetNonAmbientLightingState(pLightingState->numlights, pLightingState->locallight, &nNumLightDesc, lightDesc,
								 false);
	...
	for(int i = 0; i < pModel->numvertices; ++i)
	{
		...
		{
			g_pStudioRender->ComputeLightingConstDirectional(pLightingState->r_boxcolor, nNumLightDesc, lightDesc,
															 worldPos, worldNormal, destColor, flConstDirLightAmount);
		}
		else
		{
			g_pStudioRender->ComputeLighting(pLightingState->r_boxcolor, nNumLightDesc, lightDesc, worldPos,
...
//-----------------------------------------------------------------------------
void CModelRender::ComputeModelVertexLighting(IHandleEntity *pProp, mstudiomodel_t *pModel,
											  ...
											  Vector4D *pTempMem, color24 *pLighting)
{
	...
	for(i = 0; i < pModel->numvertices; ++i)
	{
		...
		VectorRotate(normal, matrix, worldNormal);
		bool bNonSolid =
			ComputeVertexLightingFromSphericalSamples(worldPos, worldNormal, pProp, &(pTempMem[i].AsVector3D()));
...
void CModelRender::ValidateStaticPropColorData(ModelInstanceHandle_t handle)
{
	...
	{
		Q_snprintf(fileName, sizeof(fileName), "sp_%d%s.vhv", StaticPropMgr()->GetStaticPropIndex(pProp),
				   GetPlatformExt());
	}
	else
	{
		Q_snprintf(fileName, sizeof(fileName), "sp_hdr_%d%s.vhv", StaticPropMgr()->GetStaticPropIndex(pProp),
				   GetPlatformExt());
	}
	...
	if(pVhvHdr->m_nVersion != VHV_VERSION || pVhvHdr->m_nChecksum != (unsigned int)pStudioHdr->checksum ||
...
//-----------------------------------------------------------------------------
void CModelRender::StaticPropColorMeshCallback(void *pContext, const void *pData, int numReadBytes,
											   FSAsyncStatus_t asyncStatus)
{
	...
	for(meshID = startMesh; meshID < pVhvHdr->m_nMeshes; meshID++)
	{
		...
		CMeshBuilder meshBuilder;
		meshBuilder.Begin(pStaticPropContext->m_pColorMeshData->m_pMeshInfos[nID].m_pMesh, MATERIAL_HETEROGENOUS,
						  numVertexes, 0);
...
cleanUp:
	if(IsX360())
	{
		...
		m_CachedStaticPropColorData.Insert(pStaticPropContext->m_szFilenameVertex,
										   pStaticPropContext->m_ColorMeshHandle);
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
void CModelRender::StaticPropColorTexelCallback(void *pContext, const void *pData, int numReadBytes,
												FSAsyncStatus_t asyncStatus)
{
	// get our preserved data
	Assert(pContext);
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
static void StaticPropColorMeshCallback(const FileAsyncRequest_t &request, int numReadBytes,
										FSAsyncStatus_t asyncStatus)
{
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
static void StaticPropColorTexelCallback(const FileAsyncRequest_t &request, int numReadBytes,
										 FSAsyncStatus_t asyncStatus)
{
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
static void QueuedLoaderCallback_PropLighting(void *pContext, void *pContext2, const void *pData, int nSize,
...
//-----------------------------------------------------------------------------
bool CModelRender::LoadStaticPropColorData(IHandleEntity *pProp, DataCacheHandle_t colorMeshHandle,
										   studiohwdata_t *pStudioHWData)
{
				...
				for(int meshID = 0; meshID < pModel->nummeshes; ++meshID)
				{
					...
					for(int stripGroupID = 0; stripGroupID < pStudioMeshData[pMesh->meshid].m_NumGroup; ++stripGroupID)
					{
						...
						for(int i = 0; i < pMeshGroup->m_NumVertices; ++i)
						{
							...
							meshBuilder.Specular3ub(tmpLightingMem[nVertIndex].r, tmpLightingMem[nVertIndex].g,
...
bool CModelRender::RecomputeStaticLighting(ModelInstanceHandle_t handle)
{
	...
	if(pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP)
	{
		...
		{
			instance.m_AmbientLightingState =
				*(LightcacheGetStatic(instance.m_LightCacheHandle, NULL, LIGHTCACHEFLAGS_STATIC));
		}
...
void CModelRender::DestroyInstance(ModelInstanceHandle_t handle)
{
	...
	bool bPersistLighting = IsX360() && (m_ModelInstances[handle].m_nFlags & MODEL_INSTANCE_HAS_DISKCOMPILED_COLOR) &&
...
// Creates a decal on a model instance by doing a planar projection
//-----------------------------------------------------------------------------
void CModelRender::AddDecal(ModelInstanceHandle_t handle, Ray_t const &ray, const Vector &decalUp, int decalIndex,
							int body, bool noPokeThru, int maxLODToDecal)
{
	Color cColorTemp;
...
//-----------------------------------------------------------------------------
void CModelRender::AddColoredDecal(ModelInstanceHandle_t handle, Ray_t const &ray, const Vector &decalUp,
								   int decalIndex, int body, Color cColor, bool noPokeThru, int maxLODToDecal)
{
...
//-----------------------------------------------------------------------------
void CModelRender::AddDecalInternal(ModelInstanceHandle_t handle, Ray_t const &ray, const Vector &decalUp,
									int decalIndex, int body, bool bUseColor, Color cColor, bool noPokeThru,
									int maxLODToDecal)
{
	if(handle == MODEL_INSTANCE_INVALID)
		return;
	...
	g_pStudioRender->AddDecal(inst.m_DecalHandle, g_pMDLCache->GetStudioHdr(inst.m_pModel->studio), pBoneToWorld, ray,
							  decalUp, pDecalMaterial, radius, body, noPokeThru, maxLODToDecal);
}
...
{
	for(ModelInstanceHandle_t i = m_ModelInstances.Head(); i != m_ModelInstances.InvalidIndex();
...
int CPooledVBAllocator_ColorMesh::Allocate(int numVerts)
{
	...
	{
		Warning("ERROR: CPooledVBAllocator_ColorMesh::Allocate being called after some (but not all) calls to "
				"Deallocate have been called - invalid! (expect visual artifacts)");
...
void CPooledVBAllocator_ColorMesh::Deallocate(int offset, int numVerts)
{
	...
	{
		Warning("ERROR: CPooledVBAllocator_ColorMesh::Deallocate called with too many verts, trying to free more than "
				"were allocated (bug in calling code)");
	...
	if(m_numAllocations == 0)
	{
		...
		{
			Warning("ERROR: CPooledVBAllocator_ColorMesh::Deallocate, after all allocations have been freed too few "
					"verts total have been deallocated (bug in calling code)");
...
static void CreateLightmapsFromData(CColorMeshData *_colorMeshData)
{
	...
	for(int mesh = 0; mesh < _colorMeshData->m_nMeshes; ++mesh)
	{
		...
		meshInfo->m_pLightmap =
			g_pMaterialSystem->CreateTextureFromBits(cti->m_nWidth, cti->m_nHeight, cti->m_nMipmapCount,
													 cti->m_ImageFormat, cti->m_nByteCount, cti->m_pTexelData);

		// If this triggers, we need to figure out if it's reasonable to fail. If it is, then we should figure out how
		// to signal back that we shouldn't try to create this again (probably by clearing
		// _colorMeshData->m_bColoTextureValid)
		Assert(meshInfo->m_pLightmap);
