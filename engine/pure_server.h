...
private:
	...
	public:
		...
		unsigned short
			m_LoadOrder; // What order this thing was specified in the whitelist file?  Used to resolve rule conflicts.
	};

	void UpdateCommandStats(CUtlDict<CPureServerWhitelist::CCommand *, int> &commands, int *pHighest,
	...
	CPureServerWhitelist::CCommand *CheckEntry(CUtlDict<CPureServerWhitelist::CCommand *, int> &dict,
	...
	enum
	{
		kLoadOrder_HardcodedOverride = 0xffff
	};
...
public:
	...
	bool DoesFileMatch(const char *pPathID, const char *pRelativeFilename, int nFileFraction, FileHash_t *pFileHash,
					   USERID_t userID);
