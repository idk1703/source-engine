...
public:
	...
	STEAM_GAMESERVER_CALLBACK(CSteam3Server, OnValidateAuthTicketResponse, ValidateAuthTicketResponse_t,
							  m_CallbackValidateAuthTicketResponse);
	STEAM_GAMESERVER_CALLBACK(CSteam3Server, OnComputeNewPlayerCompatibilityResponse,
							  ComputeNewPlayerCompatibilityResult_t, m_CallbackPlayerCompatibilityResponse);
	...
	bool NotifyClientConnect(CBaseClient *client, uint32 unUserID, netadr_t &adr, const void *pvCookie,
							 uint32 ucbCookie);
	...
	bool BSecure()
	{
		return SteamGameServer() && SteamGameServer()->BSecure();
	}
	bool BIsActive()
	{
	...
	bool BLanOnly() const
	{
		return m_eServerMode == eServerModeNoAuthentication;
	}
	bool BWantsSecure()
	{
	...
	const char *GetAccountToken() const
	{
		return m_sAccountToken.String();
	}
