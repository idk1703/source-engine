...
void EXPORT F(IEngineAPI **api)
{
	CreateInterfaceFn factory =
		Sys_GetFactoryThis(); // This silly construction is necessary to prevent the LTCG compiler from crashing.
...
{
#if defined(_WIN32)
	static const char *s_pFileNames[] = {"metamod.2.tf2.dll", "sourcemod.2.tf2.dll", "sdkhooks.ext.2.ep2v.dll",
	...
	{
		char buf[1024];
		static const char *s_pFileNames[] = {"metamod.2.tf2.so", "sourcemod.2.tf2.so", "sdkhooks.ext.2.ep2v.so",
		...
		DO_TRY
		{
			...
			if(Plat_GetPagedPoolInfo(&ppi_info) != SYSCALL_UNSUPPORTED)
			{
				CommentPrintf("\nPaged Pool\nprev PP PAGES: used: %lu, free %lu\nfinal PP PAGES: used: %lu, free %lu\n",
							  g_pagedpoolinfo.numPagesUsed, g_pagedpoolinfo.numPagesFree, ppi_info.numPagesUsed,
							  ppi_info.numPagesFree);
			}

			CommentPrintf("memallocfail? = %u\nActive: %s\nSpawnCount %d MapLoad Count %d\nError count %d, end demo "
						  "%d, abort count %d\n",
						  MemAlloc_MemoryAllocFailed(), (game && game->IsActiveApp()) ? "active" : "inactive",
						  gHostSpawnCount, g_nMapLoadCount, g_HostErrorCount, g_HostEndDemo, g_HostServerAbortCount);
	...
	void BuildCommentMemStatus()
	{
		...
		if(GlobalMemoryStatusEx(&memStat))
		{
			CommentPrintf("\nMemory\nmemusage( %d %% )\ntotalPhysical Mb(%.2f)\nfreePhysical Mb(%.2f)\ntotalPaging "
						  ...
						  memStat.dwMemoryLoad, (double)memStat.ullTotalPhys / MbDiv,
						  (double)memStat.ullAvailPhys / MbDiv, (double)memStat.ullTotalPageFile / MbDiv,
						  (double)memStat.ullAvailPageFile / MbDiv, (double)memStat.ullTotalVirtual / MbDiv,
						  (double)memStat.ullAvailVirtual / MbDiv, (double)memStat.ullAvailExtendedVirtual / MbDiv);
		}
			...
			if(fn)
			{
				...
				if(fn(GetCurrentProcess(), &counters, sizeof(PROCESS_MEMORY_COUNTERS)))
				{
					CommentPrintf("\nProcess Memory\nWorkingSetSize Mb(%.2f)\nQuotaPagedPoolUsage "
								  ...
								  (double)counters.WorkingSetSize / MbDiv, (double)counters.QuotaPagedPoolUsage / MbDiv,
...
static eSteamInfoInit Sys_TryInitSteamInfo(void *pvAPI, SteamInfVersionInfo_t &VerInfo, const char *pchMod,
										   const char *pchBaseDir, bool bDedicated)
{
	...
	// In partial state the ServerAppID might be unknown, but if we found the full steam.inf and it's not set, it shares
	// AppID.
	...
	if(BreakpadAppId != k_uAppIdInvalid && initState > previousInitState && bUseBreakpad)
	{
		...
		SteamAPI_UseBreakpadCrashHandler(pchVersion, __DATE__, __TIME__, bFullMemoryDump, pvMiniDumpContext,
...
bool CEngineAPI::Connect(CreateInterfaceFn factory)
{
	...
	if(!g_pStudioRender || !g_pDataCache || !g_pPhysics || !g_pMDLCache || !g_pMatSystemSurface ||
	   !g_pInputSystem /* || !g_pVideo */)
	{
...
void CEngineAPI::SetStartupInfo(StartupInfo_t &info)
{
	...
	eSteamInfoInit steamInfo =
		Sys_TryInitSteamInfo(this, g_SteamInfIDVersionInfo, info.m_pInitialMod, info.m_pBaseDirectory, bDedicated);
	...
	if(steamInfo != eSteamInfo_Initialized)
	{
		...
		if(steamInfo != eSteamInfo_Initialized)
		{
			Warning("Failed to find steam.inf or equivalent steam info. May not have proper information to connect to "
					"Steam.\n");
		}
	}
...
void CEngineAPI::ActivateSimulation(bool bActive)
{
	...
	if((eng->GetState() != IEngine::DLL_ACTIVE) && (eng->GetState() != IEngine::DLL_PAUSED))
	{
		return;
	}
...
CON_COMMAND(gprofilerstart, "Starts the gperftools profiler recording to the specified file.")
{
	...
	if(args.ArgC() < 2)
	{
		...
		V_sprintf_safe(buffer, "%s/valveprofile_%4d_%02d_%02d_%02d.%02d.%02d.prof", s_pszHomeDir, tm.tm_year + 1900,
...
static bool IsValveMod(const char *pModName)
{
	...
	return (Q_stricmp(GetCurrentMod(), "cstrike") == 0 || Q_stricmp(GetCurrentMod(), "dod") == 0 ||
			Q_stricmp(GetCurrentMod(), "hl1mp") == 0 || Q_stricmp(GetCurrentMod(), "tf") == 0 ||
			Q_stricmp(GetCurrentMod(), "tf_beta") == 0 || Q_stricmp(GetCurrentMod(), "hl2mp") == 0);
}
...
bool CEngineAPI::ModInit(const char *pModName, const char *pGameDir)
{
	...
	if(!videomode || !videomode->CreateGameWindow(g_pMaterialSystemConfig->m_VideoMode.m_Width,
												  g_pMaterialSystemConfig->m_VideoMode.m_Height, bWindowed))
	{
		return false;
	}
...
bool CModAppSystemGroup::AddLegacySystems()
{
	...
	AppSystemInfo_t appSystems[] = {
		{"soundemittersystem", SOUNDEMITTERSYSTEM_INTERFACE_VERSION}, {"", ""} // Required to terminate the list
	};
...
bool CModAppSystemGroup::Create()
{
	...
	IServerDLLSharedAppSystems *serverSharedSystems =
		(IServerDLLSharedAppSystems *)g_ServerFactory(SERVER_DLL_SHARED_APPSYSTEMS, NULL);
...
//-----------------------------------------------------------------------------
bool CModAppSystemGroup::ModuleAlreadyInList(CUtlVector<AppSystemInfo_t> &list, const char *moduleName,
...
DWORD __stdcall LongTickWatcherThread(void *voidPtr)
{
	...
	while(eng->GetQuitting() == IEngine::QUIT_NOTQUITTING && !g_bQuitLongTickWatcherThread)
	{
		if(sv.m_State == ss_active && sv.m_bSimulatingTicks)
		{
			...
			{
				if(curTime > flNextPossibleDumpTime && curTime > flWarnTickTime &&
				   nNumDumpsThisTick < MAX_DUMPS_PER_LONG_TICK)
				{
					nNumDumpsThisTick++;
					g_bTotalDumps++;
					Warning("Long tick after tick %i. Writing minidump #%i (%i total).\n", nLastTick, nNumDumpsThisTick,
							g_bTotalDumps);
					...
					// If you're debugging a minidump and you ended up here, you probably want to switch to the main
					// thread.
...
bool CDedicatedServerAPI::ModInit(ModInfo_t &info)
{
	...
	if(CommandLine()->CheckParm("+sv_pure"))
		...
		pure_mode = CommandLine()->ParmValue("-sv_pure", 1);
	if(pure_mode)
		g_pFullFileSystem->EnableWhitelistFileTracking(
			true, true, CommandLine()->FindParm("-sv_pure_verify_hashes") ? true : false);
	else
