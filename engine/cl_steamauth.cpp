...
//-----------------------------------------------------------------------------
void CSteam3Client::GetAuthSessionTicket(void *pTicket, int cbMaxTicket, uint32 *pcbTicket, uint32 unIP, uint16 usPort,
										 uint64 unGSSteamID, bool bSecure)
{
	...
	*pcbTicket = 0;
	...
	if(m_hAuthTicket == k_HAuthTicketInvalid || m_unIP != unIP || m_usPort != usPort || m_bGSSecure != bSecure ||
	   m_steamIDGS != steamIDGS || m_nTicketSize <= 0)
	{
		...
		*(uint64 *)m_arbTicketData = SteamUser()->GetSteamID().ConvertToUint64();
		...
		m_nTicketSize = 0;
		m_hAuthTicket = SteamUser()->GetAuthSessionTicket(m_arbTicketData + sizeof(uint64),
														  sizeof(m_arbTicketData) - sizeof(uint64), &m_nTicketSize);
...
// Tell the steam backend about the server we are playing on - so that it can broadcast this to our friends and they can
// join. This may be something that you should NOT do if you are on a listen server - or you know that the game is not
// joinable for some reason.
#ifdef _DEBUG
	Msg("Sending AdvertiseGame %s %s (%s)\n", steamIDGS.Render(), netadr_t(unIP, usPort).ToString(),
		m_bGSSecure ? "secure" : "insecure");
#endif
