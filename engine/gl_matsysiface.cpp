...
void TangentSpaceComputeBasis(Vector &tangent, Vector &binormal, const Vector &normal, const Vector &tVect,
							  bool negateTangent);
...
CON_COMMAND_F(mat_crosshair_edit,
			  "open the material under the crosshair in the editor defined by mat_crosshair_edit_editor", FCVAR_CHEAT)
{
	IMaterial *pMaterial = GetMaterialAtCrossHair();
	if(!pMaterial)
	{
...
// A console command to open the material under the crosshair in the associated editor.
//-----------------------------------------------------------------------------
CON_COMMAND_F(mat_crosshair_explorer, "open the material under the crosshair in explorer and highlight the vmt file",
			  FCVAR_CHEAT)
{
	IMaterial *pMaterial = GetMaterialAtCrossHair();
	if(!pMaterial)
...
static void RegisterLightmappedSurface(SurfaceHandle_t surfID)
{
	...
	MSurf_MaterialSortID(surfID) = materials->AllocateLightmap(allocationWidth, allocationHeight,
															   offsetIntoLightmapPage, MSurf_TexInfo(surfID)->material);
...
void MaterialSystem_RegisterLightmapSurfaces(void)
{
	...
	for(int surfaceIndex = 0; surfaceIndex < host_state.worldbrush->numsurfaces; surfaceIndex++)
	{
		...
		if((MSurf_TexInfo(surfID)->flags & SURF_NOLIGHT) || (MSurf_Flags(surfID) & SURFDRAW_NOLIGHT))
		{
		...
		for(j = 0; j < MSurf_NumPrims(surfID); j++)
		{
			...
			for(k = 0; k < pPrim->vertCount; k++)
			{
				...
				for(l = 0; l < pPrim->vertCount; l++)
				{
					...
					vert.texCoord[0] =
						DotProduct(vec, MSurf_TexInfo(surfID)->textureVecsTexelsPerWorldUnits[0].AsVector3D()) +
						MSurf_TexInfo(surfID)->textureVecsTexelsPerWorldUnits[0][3];
					vert.texCoord[0] /= MSurf_TexInfo(surfID)->material->GetMappingWidth();

					vert.texCoord[1] =
						DotProduct(vec, MSurf_TexInfo(surfID)->textureVecsTexelsPerWorldUnits[1].AsVector3D()) +
					...
					{
						vert.lightCoord[0] =
							DotProduct(vec, MSurf_TexInfo(surfID)->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D()) +
						...
						vert.lightCoord[1] += 0.5f;
						vert.lightCoord[1] /=
							(float)MSurf_LightmapExtents(surfID)[1]; // pSurf->texinfo->texture->height;
...
int __cdecl SortMapCompareFunc(const void *pElem0, const void *pElem1)
{
	...
	return pSortMapRenderContext->CompareMaterialCombos(pMap0->info.material, pMap1->info.material,
...
void MaterialSystem_CreateSortinfo(void)
{
	...
	for(i = 0; i < nSortIDs; i++)
	{
		...
		// Msg("Material %s, lightmap %d ", materialSortInfoArray[i].material->GetName(),
...
bool SurfHasBumpedLightmaps(SurfaceHandle_t surfID)
{
	...
	if((MSurf_TexInfo(surfID)->flags & SURF_BUMPLIGHT) && (!(MSurf_TexInfo(surfID)->flags & SURF_NOLIGHT)) &&
...
bool SurfHasLightmap(SurfaceHandle_t surfID)
{
	...
	if((!(MSurf_TexInfo(surfID)->flags & SURF_NOLIGHT)) && (host_state.worldbrush->lightdata) &&
...
void BuildMSurfaceVerts(const worldbrushdata_t *pBrushData, SurfaceHandle_t surfID, Vector *verts, Vector2D *texCoords,
...
void BuildMSurfacePrimVerts(worldbrushdata_t *pBrushData, mprimitive_t *prim, CMeshBuilder &builder,
							SurfaceHandle_t surfID)
{
	Vector tVect;
	bool negate = false;
	// FIXME: For some reason, normals are screwed up on water surfaces.  Revisit this once we have normals started in
	// primverts.
...
void CMSurfaceSortList::GetSurfaceListForGroup(CUtlVector<msurface2_t *> &list, const surfacesortgroup_t &group) const {
	MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(*this, group, surfID) {list.AddToTail(surfID);
}
...
void SaveSurfAtCrossHair()
{
	...
	s_CrossHairSurfID = R_LightVec(MainViewOrigin(), endPoint, false, lightmapColor, &textureS, &textureT,
								   &lightmapCoords[0], &lightmapCoords[1]);
}
