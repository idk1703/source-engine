...
public:
	bool LockWithRetVal(bool bLock)
	{
...
public:
	...
	virtual void RecomputeTranslucency(const model_t *model, int nSkin, int nBody,
									   void /*IClientRenderable*/ *pClientRenderable, float fInstanceAlphaModulate);
...
protected:
	static int CLIENTSIDE_TO_MODEL(int i)
	{
		return i >= 0 ? (-2 - (i * 2 + 1)) : -1;
	}
	static int NETDYNAMIC_TO_MODEL(int i)
	{
		return i >= 0 ? (-2 - (i * 2)) : -1;
	}
	static int MODEL_TO_CLIENTSIDE(int i)
	{
		return (i <= -2 && (i & 1)) ? (-2 - i) >> 1 : -1;
	}
	static int MODEL_TO_NETDYNAMIC(int i)
	{
		return (i <= -2 && !(i & 1)) ? (-2 - i) >> 1 : -1;
	}
...
public:
	struct ModelFileHandleHash
	{
		...
		{
			return Mix32HashFunctor()((uint32)(uintp)(p->fnHandle));
		}
		uint operator()(FileNameHandle_t fn) const
		{
			return Mix32HashFunctor()((uint32)(uintp)fn);
		}
	};
	struct ModelFileHandleEq
	{
		bool operator()(model_t *a, model_t *b) const
		{
			return a == b;
		}
		bool operator()(model_t *a, FileNameHandle_t b) const
		{
			return a->fnHandle == b;
		}
	};
...
protected:
	...
	CUtlStableHashtable<model_t *, empty_t, ModelFileHandleHash, ModelFileHandleEq, int16, FileNameHandle_t>
		m_ClientDynamicModels;
};
...
int CModelInfo::GetModelIndex(const char *name) const
{
	...
	if(pTable)
	{
		...
		{
			Assert(!m_NetworkedDynamicModels.IsValidIndex(netdyn) ||
				   V_strcmp(m_NetworkedDynamicModels[netdyn]->strName, name) == 0);
		...
		if(V_strnicmp(name, "models/", 7) == 0 && demoplayer && demoplayer->IsPlayingBack() &&
		   demoplayer->GetProtocolVersion() < PROTOCOL_VERSION_20)
		{
...
bool CModelInfo::RegisterModelLoadCallback(int modelIndex, IModelLoadCallback *pCallback, bool bCallImmediatelyIfLoaded)
{
	...
	{
		return modelloader->RegisterModelLoadCallback(const_cast<model_t *>(pModel), IsClientOnlyModelIndex(modelIndex),
													  pCallback, bCallImmediatelyIfLoaded);
	}
...
void CModelInfo::UnregisterModelLoadCallback(int modelIndex, IModelLoadCallback *pCallback)
{
	...
	else if(IsDynamicModelIndex(modelIndex))
	{
		...
		if(pModel)
		{
			modelloader->UnregisterModelLoadCallback(const_cast<model_t *>(pModel), IsClientOnlyModelIndex(modelIndex),
													 pCallback);
		}
	}
}
...
#if defined(DEMO_BACKWARDCOMPATABILITY) && !defined(SWDS)
		// dynamic model tables in old system did not have a full path with "models/" prefix
		if(pTable && demoplayer && demoplayer->IsPlayingBack() &&
		   demoplayer->GetProtocolVersion() < PROTOCOL_VERSION_20 && V_strnicmp(model->strName, "models/", 7) == 0 &&
		   pTable->FindStringIndex(model->strName + 7) != INVALID_STRING_INDEX)
		{
			return mod_studio;
		}
#endif
...
bool CModelInfo::IsUsingFBTexture(const model_t *model, int nSkin, int nBody,
								  void /*IClientRenderable*/ *pClientRenderable) const
{
	...
	if(bMightUseFbTextureThisFrame)
	{
		...
		switch(model->type)
		{
			case mod_brush:
			{
				for(int i = 0; i < model->brush.nummodelsurfaces; ++i)
				{
					SurfaceHandle_t surfID =
						SurfaceHandleFromIndex(model->brush.firstmodelsurface + i, model->brush.pShared);
			...
			case mod_studio:
			{
				...
				int materialCount = g_pStudioRender->GetMaterialListFromBodyAndSkin(model->studio, nSkin, nBody,
...
void CModelInfo::RecomputeTranslucency(const model_t *model, int nSkin, int nBody,
									   void /*IClientRenderable*/ *pClientRenderable, float fInstanceAlphaModulate)
{
	if(model != NULL)
	{
...
public:
	...
	virtual void GetModelMaterialColorAndLighting(const model_t *model, const Vector &origin, const QAngle &angles,
...
void CModelInfoServer::GetModelMaterialColorAndLighting(const model_t *model, const Vector &origin,
														const QAngle &angles, trace_t *pTrace, Vector &lighting,
														Vector &matColor)
{
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CModelInfoServer, IVModelInfo003, VMODELINFO_SERVER_INTERFACE_VERSION_3,
								  g_ModelInfoServer);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CModelInfoServer, IVModelInfo, VMODELINFO_SERVER_INTERFACE_VERSION,
								  g_ModelInfoServer);
...
private:
	...
	unsigned char ComputeScreenFade(const Vector &vecAbsOrigin, float flRadius, float flFadeScale,
...
void CModelInfoClient::OnDynamicModelsStringTableChange(int nStringIndex, const char *pString, const void *pData)
{
	...
	bool bServerLoaded =
		pData ? (*(char *)pData != 0) : (g_ClientGlobalVariables.network_protocol <= PROTOCOL_VERSION_20);
...
// Computes fade alpha based on distance fade + screen fade
//-----------------------------------------------------------------------------
inline unsigned char CModelInfoClient::ComputeScreenFade(const Vector &vecAbsOrigin, float flRadius, float flFadeScale,
														 const ScreenFadeInfo_t &fade) const
{
...
unsigned char CModelInfoClient::ComputeLevelScreenFade(const Vector &vecAbsOrigin, float flRadius,
...
unsigned char CModelInfoClient::ComputeViewScreenFade(const Vector &vecAbsOrigin, float flRadius,
													  float flFadeScale) const
{
...
//-----------------------------------------------------------------------------
IMaterial *BrushModel_GetLightingAndMaterial(const Vector &start, const Vector &end, Vector &diffuseLightColor,
											 Vector &baseColor)
{
	...
	else
	{
		...
		if(material)
		{
			...
			//		ConMsg( "%s: diff: %f %f %f base: %f %f %f\n", material->GetName(), diffuseLightColor[0],
			//diffuseLightColor[1], diffuseLightColor[2], baseColor[0], baseColor[1], baseColor[2] );
		}
		else
		{
			baseColor.Init();
														...
														Vector &matColor)
{
	switch(model->type)
	{
		...
		case mod_studio:
		{
			...
			g_pStudioRender->ComputeLighting(lightingState.r_boxcolor, count, desc, pTrace->endpos,
											 pTrace->plane.normal, lighting);
			return;
		}
	}
}
