...
{
	if(!m_Session.IsHost() || m_CurrentState < MMSTATE_ACCEPTING_CONNECTIONS || m_CurrentState == MMSTATE_PREGAME ||
	   m_CurrentState == MMSTATE_LOADING || GameIsLocked())
	{
		return false;
	}
	return true;
		...
		if(pInfo->m_cPlayers)
		{
			...
			for(int i = 0; i < pInfo->m_cPlayers; ++i)
			{
				if(pInfo->m_iTeam[i] != tempClient.m_iTeam[i] || pInfo->m_cVoiceState[i] != tempClient.m_cVoiceState[i])
				{
					...
					EngineVGui()->UpdatePlayerInfo(pInfo->m_xuids[i], pInfo->m_szGamertags[i], pInfo->m_iTeam[i],
...
void CMatchmaking::GenerateMutelist(MM_Mutelist *pMsg)
{
	...
	for(int iLocal = 0; iLocal < m_Local.m_cPlayers; ++iLocal)
	{
		...
		for(int iRemote = 0; iRemote < numRemoteTalkers; ++iRemote)
		{
			...
			if(bIsMuted)
			{
				Audio_GetXVoice()->SetPlaybackPriority(remoteTalkers[iRemote], m_Local.m_iControllers[iLocal],
...
bool CMatchmaking::ProcessMutelist(MM_Mutelist *pMsg)
{
	...
	for(int i = 0; i < m_Local.m_cPlayers; ++i)
	{
		...
		for(int j = 0; j < pMsg->m_cPlayers; ++j)
		{
			...
			if(bIsMuted)
			{
				Audio_GetXVoice()->SetPlaybackPriority(pMsg->m_xuid[j], m_Local.m_iControllers[i],
													   XHV_PLAYBACK_PRIORITY_NEVER);
			}
			else
			{
...
bool CMatchmaking::ProcessCheckpoint(MM_Checkpoint *pMsg)
{
	switch(pMsg->m_Checkpoint)
	{
		...
		case MM_Checkpoint::CHECKPOINT_CONNECT:
			...
			if(m_CurrentState == MMSTATE_CONNECTED_TO_SERVER || m_CurrentState == MMSTATE_INGAME)
			{
				break;
			}
			...
			{
				char cmd[MAX_PATH];
				Q_snprintf(cmd, sizeof(cmd), "connect %d.%d.%d.%d", m_Host.m_adr.ip[0], m_Host.m_adr.ip[1],
						   m_Host.m_adr.ip[2], m_Host.m_adr.ip[3]);
				Cbuf_AddText(cmd);
...
void CMatchmaking::SendPlayerInfoToLobby(CClientInfo *pClient, int iHostIdx)
{
	for(int i = 0; i < pClient->m_cPlayers; ++i)
	{
		EngineVGui()->UpdatePlayerInfo(pClient->m_xuids[i], pClient->m_szGamertags[i], pClient->m_iTeam[i],
...
{
	Msg("[MM] Filled Slots:\n[MM] Public: %d of %d\n[MM] Private: %d of %d\n",
		m_Session.GetSessionSlots(SLOTS_FILLEDPUBLIC), m_Session.GetSessionSlots(SLOTS_TOTALPUBLIC),
		m_Session.GetSessionSlots(SLOTS_FILLEDPRIVATE), m_Session.GetSessionSlots(SLOTS_TOTALPRIVATE));
...
void CMatchmaking::SetPreventFullServerStartup(bool bState, char const *fmt, ...)
{
	...
	DevMsg(1, "Setting state from prevent %s to prevent %s:  %s", m_bPreventFullServerStartup ? "yes" : "no",
		   bState ? "yes" : "no", desc);
