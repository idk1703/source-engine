...
ConVar sv_namechange_cooldown_seconds(
	"sv_namechange_cooldown_seconds", "30.0", FCVAR_NONE,
	"When a client name change is received, wait N seconds before allowing another name change");
ConVar sv_netspike_on_reliable_snapshot_overflow(
	"sv_netspike_on_reliable_snapshot_overflow", "0", FCVAR_NONE,
	"If nonzero, the server will dump a netspike trace if a client is dropped due to reliable snapshot overflow");
ConVar sv_netspike_sendtime_ms(
	"sv_netspike_sendtime_ms", "0", FCVAR_NONE,
	"If nonzero, the server will dump a netspike trace if it takes more than N ms to prepare a snapshot to a single "
	"client.  This feature does take some CPU cycles, so it should be left off when not in use.");
ConVar sv_netspike_output(
	"sv_netspike_output", "1", FCVAR_NONE,
...
bool CBaseClient::SetSignonState(int state, int spawncount)
{
	...
	{
		case SIGNONSTATE_CONNECTED: // client is connected, leave client in this state and let SendPendingSignonData do
									// the rest
...
bool CBaseClient::SendServerInfo(void)
{
	...
	if(developer.GetBool() || m_Server->IsMultiplayer())
	{
		...
		Q_snprintf(devtext, sizeof(devtext), "\n%s\nMap: %s\nPlayers: %i / %i\nBuild: %d\nServer Number: %i\n\n",
				   serverGameDLL->GetGameDescription(), m_Server->GetMapName(), curplayers, m_Server->GetMaxClients(),
				   build_number(), m_Server->GetSpawnCount());
...
bool CBaseClient::ProcessSetConVar(NET_SetConVar *msg)
{
	for(int i = 0; i < msg->m_ConVars.Count(); i++)
	{
		...
		if(bFunky)
		{
			Msg("Ignoring convar change request for variable '%s' from client %s; invalid characters in the variable "
				"name\n",
				name, GetClientName());
			continue;
		}
...
void CBaseClient::EndTrace(bf_write &msg)
{
	...
	if(m_iTracing < 2								   // not forced
	   ...
	   && (sv_netspike_sendtime_ms.GetFloat() <= 0.0f ||
		   flElapsedMs < sv_netspike_sendtime_ms.GetFloat())) // didn't exceed time threshold
	{
	...
	logData.Printf("%f/%d Player [%s][%d][adr:%s] was sent a datagram %d bits (%8.3f bytes), took %.2fms\n", realtime,
				   host_tickcount, GetClientName(), GetPlayerSlot(), GetNetChannel()->GetAddress(), bits,
				   (float)bits / 8.0f, flElapsedMs);
...
void CBaseClient::UpdateSendState(void)
{
	...
	{
		if(m_NetChannel && m_NetChannel->HasPendingReliableData() && m_NetChannel->GetTimeSinceLastReceived() < 1.0f)
		{
...
bool CBaseClient::UpdateAcknowledgedFramecount(int tick)
{
	...
	if(m_nForceWaitForTick > 0)
	{
		...
		else if(tick == -1)
		{
			...
			{
				// that's strange: we sent the client a full update, and it was fully received ( no reliable data in
				// waiting buffers ) but the client is requesting another full update.
				//
				// This can happen if they request full updates in succession really quickly (using cl_fullupdate or
				// "record X;stop" quickly). There was a bug here where if we just return out, the client will have
				// nuked its entities and we'd send it a supposedly uncompressed update but m_nDeltaTick was not -1, so
				// it was delta'd and it'd miss lots of stuff. Led to clients getting full spectator mode radar while
