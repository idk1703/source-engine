...
void CL_NameCvarChanged(IConVar *pConVar, const char *pOldString, float flOldValue)
{
	...
	if(!bPreventRent)
	{
		...
		if(0 != Q_stricmp(var.GetString(), var.GetDefault()) && 0 != Q_stricmp(var.GetString(), "player"))
		{
...
ConCommand askconnect_accept("askconnect_accept", askconnect_accept_f, "Accept a redirect request by the server.",
							 FCVAR_DONTRECORD);
#endif
...
ConVar cl_resend("cl_resend", "6", FCVAR_NONE, "Delay in seconds before the client will resend the 'connect' attempt",
				 true, CL_MIN_RESEND_TIME, true, CL_MAX_RESEND_TIME);
ConVar cl_name("name", "unnamed", FCVAR_ARCHIVE | FCVAR_USERINFO | FCVAR_PRINTABLEONLY | FCVAR_SERVER_CAN_EXECUTE,
			   "Current user name", CL_NameCvarChanged);
ConVar password("password", "", FCVAR_ARCHIVE | FCVAR_SERVER_CANNOT_QUERY | FCVAR_DONTRECORD,
				"Current server access password");
ConVar cl_interpolate("cl_interpolate", "1.0", FCVAR_USERINFO | FCVAR_DEVELOPMENTONLY | FCVAR_NOT_CONNECTED,
					  "Interpolate entities on the client.");
ConVar cl_clanid("cl_clanid", "0", FCVAR_ARCHIVE | FCVAR_USERINFO | FCVAR_HIDDEN, "Current clan ID for name decoration",
				 CL_ClanIdChanged);
ConVar cl_show_connectionless_packet_warnings(
	"cl_show_connectionless_packet_warnings", "0", FCVAR_NONE,
...
bool CBaseClientState::SetSignonState(int state, int count)
{
	...
	if(state == SIGNONSTATE_FULL)
	{
		...
		if(IsX360() && g_pMatchmaking->PreventFullServerStartup())
		{
			return true;
		}
	}
...
void CBaseClientState::SendConnectPacket(int challengeNr, int authProtocol, uint64 unGSSteamID, bool bGSSecure)
{
	...
	//	msg.WriteByte( ( g_pServerPluginHandler->GetNumLoadedPlugins() > 0 ) ? 1 : 0 ); // have any client-side server
	//plug-ins been loaded?
	...
	switch(authProtocol)
	{
		...
		case PROTOCOL_STEAM:
			if(!PrepareSteamConnectResponse(unGSSteamID, bGSSecure, adr, msg))
			{
				return;
			}
			break;
...
//-----------------------------------------------------------------------------
bool CBaseClientState::PrepareSteamConnectResponse(uint64 unGSSteamID, bool bGSSecure, const netadr_t &adr,
												   bf_write &msg)
{
	...
	Steam3Client().GetAuthSessionTicket(steam3Cookie, sizeof(steam3Cookie), &steam3CookieLen,
										checkAdr.GetIPHostByteOrder(), checkAdr.GetPort(), unGSSteamID, bGSSecure);
...
static ConVar cl_connectmethod("cl_connectmethod", "", FCVAR_USERINFO | FCVAR_HIDDEN,
...
/* static */ bool CBaseClientState::ConnectMethodAllowsRedirects()
{
	...
	if(V_strcmp(pConnectMethod, "serverbrowser_internet") == 0 || V_strncmp(pConnectMethod, "quickpick", 9) == 0 ||
	   V_strncmp(pConnectMethod, "quickplay", 9) == 0 || V_strcmp(pConnectMethod, "matchmaking") == 0 ||
...
bool CBaseClientState::ProcessConnectionlessPacket(netpacket_t *packet)
{
	...
	if((packet->from.GetType() != NA_LOOPBACK) &&
	   (packet->from.GetIPNetworkByteOrder() != adrServerConnectingTo.GetIPNetworkByteOrder()))
	{
...
bool CBaseClientState::ProcessStringCmd(NET_StringCmd *msg)
{
	...
	{
		AssertMsg(false, "CBaseClientState::ProcessStringCmd called but there is no room for the execution markers. "
						 "Ignoring command.");
		return true;
	}

	// Run the command, but make sure the command parser knows to only execute commands marked with
	// FCVAR_SERVER_CAN_EXECUTE.
	Cbuf_AddTextWithMarkers(eCmdExecutionMarker_Enable_FCVAR_SERVER_CAN_EXECUTE, msg->m_szCommand,
...
bool CBaseClientState::ProcessServerInfo(SVC_ServerInfo *msg)
{
...
#ifndef SWDS
	if(!sv.IsActive() && !(m_NetChannel->IsLoopback() || m_NetChannel->IsNull()))
	{
	...
	if(msg->m_fTickInterval < MINIMUM_TICK_INTERVAL || msg->m_fTickInterval > MAXIMUM_TICK_INTERVAL)
	{
		ConMsg("Interval_per_tick %f out of range [%f to %f]\n", msg->m_fTickInterval, MINIMUM_TICK_INTERVAL,
	...
	// Set default filename, but this is finalized by ClientState later, so it should not be depended on yet. See
	// PrepareLevelResources call
...
bool CBaseClientState::ProcessCreateStringTable(SVC_CreateStringTable *msg)
{
	...
	CNetworkStringTable *table = (CNetworkStringTable *)m_StringTableContainer->CreateStringTableEx(
		msg->m_szTableName, msg->m_nMaxEntries, msg->m_nUserDataSize, msg->m_nUserDataSizeBits, msg->m_bIsFilenames);
	...
	if(msg->m_bDataCompressed)
	{
		...
		if(msg->m_DataIn.TotalBytesAvailable() > 0 &&
		   msgCompressedSize <= (unsigned int)msg->m_DataIn.TotalBytesAvailable() && msgCompressedSize < UINT_MAX / 2 &&
		   msgUncompressedSize < UINT_MAX / 2)
		{
			...
			// uncompress data
			bSuccess = COM_BufferToBufferDecompress(uncompressedBuffer, &uncompressedSize, compressedBuffer,
													msgCompressedSize);
...
bool CBaseClientState::ProcessUpdateStringTable(SVC_UpdateStringTable *msg)
{
	...
	// m_StringTableContainer is NULL on level transitions, Seems to be caused by a UpdateStringTable packet comming in
	// before the ServerInfo packet
	//   I'm not sure this is safe, but at least we won't crash. The realy odd thing is this can happen on the server as
	//   well.//tmauer
	if(m_StringTableContainer != NULL)
	{
		CNetworkStringTable *table = (CNetworkStringTable *)m_StringTableContainer->GetTable(msg->m_nTableID);
...
bool CBaseClientState::LinkClasses()
{
	...
	for(int i = 0; i < m_nServerClasses; i++)
	{
		...
		if(pServerClass->m_pClientClass)
		{
			...
			{
				Host_EndGame(true,
							 "CL_ParseClassInfo_EndClasses: server and client classes for '%s' use different "
							 "datatables (server: %s, client: %s)",
...
void CBaseClientState::SetEntityBaseline(int iBaseline, ClientClass *pClientClass, int index, char *packedData,
										 int length)
{
...
{
	ErrorIfNot(iClass >= 0 && iClass < m_nServerClasses, ("GetDynamicBaseline: invalid class index '%d'", iClass));
...
{
	return (V_strnistr(cl_connectmethod.GetString(), "quickplay", 9) ||
			V_strnistr(cl_connectmethod.GetString(), "matchmaking", 11));
}