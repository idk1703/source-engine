...
void CPluginGameUIDialog::OnCommand(const char *cmd)
{
	if(!Q_stricmp(cmd, "close"))
	{
		if(Q_strlen(m_szEntryCommand) > 0)
		{
			...
			{
				AssertMsg(false, "CPluginGameUIDialog::OnCommand called but there is no room for the execution "
								 "markers. Ignoring command.");
				return;
			}
			...
			Cbuf_AddTextWithMarkers(eCmdExecutionMarker_Enable_FCVAR_CLIENTCMD_CAN_EXECUTE, userCMD,
...
void CPluginUIManager::OnTick()
{
	...
	if(m_iMessageDisplayUntil != 0 && !EngineVGui()->IsGameUIVisible() &&
	   m_iMessageDisplayUntil < Sys_FloatTime()) // check the GameUI large message
	{
