...
static ConVar net_udp_rcvbuf("net_udp_rcvbuf", NETSTRING(UDP_SO_RCVBUF_SIZE), FCVAR_ALLOWED_IN_COMPETITIVE,
							 "Default UDP receive buffer size", true, 8192, true, 128 * 1024);

static ConVar net_showsplits("net_showsplits", "0", 0, "Show info about packet splits");

static ConVar net_splitrate("net_splitrate", "1", 0,
...
static ConVar matchmakingport("matchmakingport", NETSTRING(PORT_MATCHMAKING), FCVAR_ALLOWED_IN_COMPETITIVE,
							  "Host Matchmaking port");
static ConVar systemlinkport("systemlinkport", NETSTRING(PORT_SYSTEMLINK), FCVAR_ALLOWED_IN_COMPETITIVE,
							 "System Link port");
...
static ConVar fakeloss("net_fakeloss", "0", FCVAR_CHEAT,
...
static ConVar net_savelargesplits(
	"net_savelargesplits", "-1", 0,
	"If not -1, then if a split has this many or more split parts, save the entire packet to disc for analysis.");
#endif
...
// For metering out splitpackets, don't do them too fast as remote UDP socket will drop some payloads causing them to
// always fail to be reconstituted This problem is largely solved by increasing the buffer sizes for UDP sockets on
// Windows
#define SPLITPACKET_MAX_DATA_BYTES_PER_SECOND V_STRINGIFY(DEFAULT_RATE)

static ConVar sv_maxroutable("sv_maxroutable", "1260", 0,
							 "Server upper bound on net_maxroutable that a client can use.", true,
							 MIN_USER_MAXROUTABLE_SIZE, true, MAX_USER_MAXROUTABLE_SIZE);

ConVar net_maxroutable("net_maxroutable", "1260", FCVAR_ARCHIVE | FCVAR_USERINFO,
					   "Requested max packet size before packets are 'split'.", true, MIN_USER_MAXROUTABLE_SIZE, true,
...
int NET_OpenSocket(const char *net_interface, int &port, int protocol)
{
	...
	if(protocol == IPPROTO_TCP)
	{
		if(!IsX360()) // SO_KEEPALIVE unsupported on the 360
		{
			...
			VCR_NONPLAYBACKFN(setsockopt(newsocket, SOL_SOCKET, SO_KEEPALIVE, (char *)&opt, sizeof(opt)), ret,
							  "setsockopt");
			if(ret == -1)
			{
				NET_GetLastError();
		...
		optlinger.l_onoff = 0;
		VCR_NONPLAYBACKFN(setsockopt(newsocket, SOL_SOCKET, SO_LINGER, (char *)&optlinger, sizeof(optlinger)), ret,
						  "setsockopt");
		if(ret == -1)
		{
			NET_GetLastError();
		...
		VCR_NONPLAYBACKFN(setsockopt(newsocket, IPPROTO_TCP, TCP_NODELAY, (char *)&opt, sizeof(opt)), ret,
						  "setsockopt");
		if(ret == -1)
		{
			NET_GetLastError();
	...
	if(!X360SecureNetwork() || protocol != IPPROTO_VDP)
	{
		...
		VCR_NONPLAYBACKFN(setsockopt(newsocket, SOL_SOCKET, SO_BROADCAST, (char *)&opt, sizeof(opt)), ret,
						  "setsockopt");
		if(ret == -1)
		{
			NET_GetLastError();
	...
	if(CommandLine()->FindParm("-reuse"))
	{
		...
		VCR_NONPLAYBACKFN(setsockopt(newsocket, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof(opt)), ret,
						  "setsockopt");
		if(ret == -1)
		{
			NET_GetLastError();
...
INetChannel *NET_CreateNetChannel(int socket, netadr_t *adr, const char *name, INetChannelHandler *handler,
								  bool bForceNewChannel /*=false*/, int nProtocolVersion /*=PROTOCOL_VERSION*/)
{
	CNetChan *chan = NULL;
...
bool NET_GetLong(const int sock, netpacket_t *packet)
{
	...
	if(nSplitSizeMinusHeader < MIN_SPLIT_SIZE || nSplitSizeMinusHeader > MAX_SPLIT_SIZE)
	{
		Msg("NET_GetLong:  Split packet from %s with invalid split size (number %i/ count %i) where size %i is out of "
			"valid range [%llu - %llu]\n",
			packet->from.ToString(), packetNumber, packetCount, nSplitSizeMinusHeader, (uint64)MIN_SPLIT_SIZE,
	...
	{
		Msg("NET_GetLong:  Split packet from %s with too many split parts (number %i/ count %i) where %llu is max "
			"count allowed\n",
			packet->from.ToString(), packetNumber, packetCount, (uint64)MAX_SPLITPACKET_SPLITS);
		return false;
	}
	...
	{
		Msg("NET_GetLong:  Split packet from %s with inconsistent split size (number %i/ count %i) where size %i not "
			"equal to initial size of %i\n",
			packet->from.ToString(), packetNumber, packetCount, nSplitSizeMinusHeader,
	...
	if(entry->splitflags[packetNumber] != sequenceNumber)
	{
		...
		if(net_showsplits.GetInt() && net_showsplits.GetInt() != 3)
		{
			Msg("<-- [%s] Split packet %4i/%4i seq %5i size %4i mtu %4llu from %s\n", DescribeSocket(sock),
				packetNumber + 1, packetCount, sequenceNumber, size,
				(uint64)(nSplitSizeMinusHeader + sizeof(SPLITPACKET)), packet->from.ToString());
		}
	}
	else
	{
...
bool NET_ReceiveDatagram(const int sock, netpacket_t *packet)
{
	...
	int ret = 0;
	{
		...
		ret = VCRHook_recvfrom(net_socket, (char *)packet->data, NET_MAX_MESSAGE, 0, (struct sockaddr *)&from,
							   (int *)&fromlen);
	}
	if(ret >= NET_MIN_MESSAGE)
	{
		...
		if(X360SecureNetwork())
		{
			...
			if(nVoiceBytes > 0)
			{
				...
				if(nDecompressedVoice >= 0)
				{
					...
					if(!COM_BufferToBufferDecompress((char *)bufVoice.Base(), &unActualDecompressedSize, pVoice,
													 nCompressedSize))
						return false;
		...
		if(ret < NET_MAX_MESSAGE)
		{
			...
			if(LittleLong(*(int *)packet->data) == NET_HEADER_FLAG_COMPRESSEDPACKET)
			{
				...
				MEM_ALLOC_CREDIT();
				CUtlMemoryFixedGrowable<byte, NET_COMPRESSION_STACKBUF_SIZE> memDecompressed(
					NET_COMPRESSION_STACKBUF_SIZE);
				memDecompressed.EnsureCapacity(actualSize);

				unsigned uDecompressedSize = (unsigned)actualSize;
				COM_BufferToBufferDecompress((char *)memDecompressed.Base(), &uDecompressedSize, pCompressedData,
											 nCompressedDataSize);
				if(uDecompressedSize == 0 || ((unsigned int)actualSize) != uDecompressedSize)
				{
					if(net_showudp.GetBool())
					{
						Msg("UDP:  discarding %d bytes from %s due to decompression error [%d decomp, actual %d] at "
							"tm=%f rt=%f\n",
							ret, packet->from.ToString(), uDecompressedSize, actualSize, (float)net_time,
...
int NET_QueuePacketForSend(CNetChan *chan, bool verbose, SOCKET s, const char FAR *buf, int len,
						   const struct sockaddr FAR *to, int tolen, uint32 msecDelay)
{
...
void NET_SendQueuedPacket(SendQueueItem_t *sq)
{
	...
	NET_SendTo(false, sq->m_Socket, (const char FAR *)sq->m_Buffer.Base(), sq->m_Buffer.TellPut(),
			   (const struct sockaddr FAR *)sq->m_To.Base(), sq->m_To.TellPut(), -1);
...
static ConVar net_splitpacket_maxrate("net_splitpacket_maxrate", SPLITPACKET_MAX_DATA_BYTES_PER_SECOND, 0,
									  "Max bytes per second when queueing splitpacket chunks", true, MIN_RATE, true,
									  MAX_RATE);

int NET_SendLong(INetChannel *chan, int sock, SOCKET s, const char FAR *buf, int len, const struct sockaddr FAR *to,
				 int tolen, int nMaxRoutableSize)
{
	...
	while(nBytesLeft > 0)
	{
		...
		// Setting net_queued_packet_thread to NET_QUEUED_PACKET_THREAD_DEBUG_VALUE goes into a mode where all packets
		// are queued.. can be used to stress-test it. Linux threads aren't prioritized well enough for this to work
		// well (i.e. the queued packet thread doesn't get enough attention to flush itself well). The behavior the
		// queue fixes is that if you send too many DP packets without giving up your timeslice, it'll just discard the
		// 7th and later packets until you Sleep() (issue might be on client recipient side, need to snif packets to
		// double check)

		if(netchan && (nFragmentsSent >= net_splitrate.GetInt() ||
					   net_queued_packet_thread.GetInt() == NET_QUEUED_PACKET_THREAD_DEBUG_VALUE))
		{
			...
			float flMaxSplitpacketDataRateBytesPerSecond =
				min((float)netchan->GetDataRate(), (float)net_splitpacket_maxrate.GetInt());

			// Calculate the delay (measured from now) for when this packet should be sent.
			uint32 delay = (int)(1000.0f * ((float)(nPacketNumber * (nMaxRoutableSize + UDP_HEADER_SIZE)) /
											flMaxSplitpacketDataRateBytesPerSecond) +
								 0.5f);
		...
		else
		{
			...
			// w/o a netchan, if there are too many splits, its possible the packet can't be delivered.  However, this
			// would only apply to out of band stuff like
		...
		if(net_showsplits.GetInt() && net_showsplits.GetInt() != 2)
		{
			...
			Msg("--> [%s] Split packet %4i/%4i seq %5i size %4i mtu %4i to %s [ total %4i ]\n", DescribeSocket(sock),
				nPacketNumber, nPacketCount, nSequenceNumber, size, nMaxRoutableSize, adr.ToString(), sendlen);
		}
	}
				   ...
				   bf_write *pVoicePayload /* = NULL */, bool bUseCompression /*=false*/)
{
	...
	if(pVoicePayload)
	{
		...
		unsigned int nCompressedLength =
			COM_GetIdealDestinationCompressionBufferSize_Snappy(pVoicePayload->GetNumBytesWritten());
		...
		*(unsigned short *)pVoice = LittleShort(usVoiceBits);
		...
		{
			if(COM_BufferToBufferCompress_Snappy(pVoice, &nCompressedLength, pVoicePayload->GetData(),
	...
	if(bUseCompression)
	{
		...
		*(int *)memCompressed.Base() = LittleLong(NET_HEADER_FLAG_COMPRESSEDPACKET);
		...
		if(COM_BufferToBufferCompress_Snappy(memCompressed.Base() + sizeof(unsigned int), &nCompressedLength, data,
											 length) &&
	...
	if(chan)
	{
		nMaxRoutable = clamp(chan->GetMaxRoutablePayloadSize(), MIN_USER_MAXROUTABLE_SIZE,
							 min(sv_maxroutable.GetInt(), MAX_USER_MAXROUTABLE_SIZE));
	}

	if(length <= nMaxRoutable && !(net_queued_packet_thread.GetInt() == NET_QUEUED_PACKET_THREAD_DEBUG_VALUE && chan))
	{
...
void NET_FlushAllSockets(void)
{
	...
	for(int i = 0; i < net_sockets.Count(); i++)
	{
		if(net_sockets[i].hUDP)
		{
			...
			while(bytes > 0)
			{
				bytes = VCRHook_recvfrom(net_sockets[i].hUDP, data, sizeof(data), 0, (struct sockaddr *)&from,
										 (int *)&fromlen);
			}
		}
	}
...
static bool OpenSocketInternal(int nModule, int nSetPort, int nDefaultPort, const char *pName, int nProtocol,
							   bool bTryAny, int flags = (OSOCKET_FLAG_USE_IPNAME | OSOCKET_FLAG_FAIL))
{
...
void NET_OpenSockets(void)
{
	...
	if(sv_lan.GetBool())
	{
		...
		if(net_interface[0] && (Q_strcmp(net_interface, "localhost") != 0))
		{
			...
			static int s_ports[] = {26900, 26901, 26902, 26903, 26904, 26905, 27015, 27016, 27017, 27018, 27019, 27020};
...
void NET_GetLocalAddress(void)
{
	...
	else
	{
		...
		int ipaddr = (net_local_adr.ip[0] << 24) + (net_local_adr.ip[1] << 16) + (net_local_adr.ip[2] << 8) +
					 net_local_adr.ip[3];
...
void NET_LogServerStatus(void)
{
	...
	char msg[size];
	Q_snprintf(msg, size, "%.0f,%d,%.0f,%.0f,%.0f,%.1f,%.1f,%.1f,%.1f,%.1f\n", fCurrentTime, numChannels,
			   fStats[NET_LATENCY], fStats[NET_LOSS], fStats[NET_PACKETS_IN], fStats[NET_PACKETS_OUT],
			   fStats[NET_FLOW_IN] / 1024.0f, fStats[NET_FLOW_OUT] / 1024.0f, fStats[NET_CHOKE_IN],
...
void NET_LogServerCallback(IConVar *pConVar, const char *pOldString, float flOldValue)
{
	...
	if(var.GetBool())
	{
		...
		Q_snprintf(
			msg, 128,
			"Time,Channels,Latency,Loss,Packets In,Packets Out,Flow In(kB/s),Flow Out(kB/s),Choke In,Choke Out\n");
		g_pFileSystem->Write(msg, Q_strlen(msg), g_fh);
	}
	else
	{
...
void NET_Config(void)
{
	...
	Msg("Network: IP %s, mode %s, dedicated %s, ports %i SV / %i CL\n", net_local_adr.ToString(true),
		net_multiplayer ? "MP" : "SP", net_dedicated ? "Yes" : "No", net_sockets[NS_SERVER].nPort,
...
CON_COMMAND(net_status, "Shows current network status")
{
	...
	ConMsg("- Ports: Client %u, Server %u, HLTV %u, Matchmaking %u, Systemlink %u%s\n", net_sockets[NS_CLIENT].nPort,
		   net_sockets[NS_SERVER].nPort, net_sockets[NS_HLTV].nPort, net_sockets[NS_MATCHMAKING].nPort,
		   net_sockets[NS_SYSTEMLINK].nPort, lan_str.Get());
