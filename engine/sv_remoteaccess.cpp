...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerRemoteAccess, IGameServerData, GAMESERVERDATA_INTERFACE_VERSION,
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::WriteDataRequest(CRConServer *pNetworkListener, ra_listener_id listener, const void *buffer,
										   int bufferSize)
{
	...
	while(invalidRequest == false &&
		  (int)cmd.TellGet() < (int)(cmd.Size() - 2 * sizeof(int))) // while there is commands to read
	{
		// parse out the buffer
		int requestID = cmd.GetInt();
		pNetworkListener->SetRequestID(
			listener, requestID); // tell the rcon server the ID so it can reflect it when the console redirect flushes
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::CheckPassword(CRConServer *pNetworkListener, ra_listener_id listener, int requestID,
										const char *password)
{
...
void CServerRemoteAccess::GetUserBanList(CUtlBuffer &value)
{
	...
	for(i = 0; i < g_IPFilters.Count(); i++)
	{
		...
		*(unsigned *)b = g_IPFilters[i].compare;
		value.Printf("%i %i.%i.%i.%i : %.3f min\n", i + 1 + g_UserFilters.Count(), b[0], b[1], b[2], b[3],
...
void CServerRemoteAccess::GetStatsString(char *buf, int bufSize)
{
	...
	_snprintf(buf, bufSize - 1, "%-6.2f %-10.2f %-11.2f %-7i %-12i %-8.2f %-8i %-8i", sv.GetCPUUsage() * 100,
			  avgIn / 1024.0f, avgOut / 1024.0f, (int)(Sys_FloatTime()) / 60, sv.GetSpawnCount() - 1,
			  1.0 / host_frametime, // frame rate
			  sv.GetNumClients() - sv.GetNumProxies(), sv.GetNumConnections());
	buf[bufSize - 1] = 0;
};
...
void CServerRemoteAccess::GetPlayerList(CUtlBuffer &value)
{
	...
	for(int i = 0; i < sv.GetClientCount(); i++)
	{
		...
		{
			value.Printf("\"%s\" %s 0 0 0 %d 0\n", client->GetClientName(), client->GetNetworkIDString(), pl->frags);
		}
		else
		{
			value.Printf("\"%s\" %s %s %d %d %d %d\n", client->GetClientName(), client->GetNetworkIDString(),
						 ...
						 (int)(client->GetNetChannel()->GetAvgLoss(FLOW_INCOMING)), pl->frags,
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::SendResponseToClient(ra_listener_id listenerID, ServerDataResponseType_t type, void *pData,
											   int nDataLen)
{
	// allocate a spot in the list for the response
	int i = m_ResponsePackets.AddToTail();
