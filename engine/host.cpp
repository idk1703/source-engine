...
BEGIN_BYTESWAP_DATADESC(player_info_s)
	DEFINE_ARRAY(name, FIELD_CHARACTER, MAX_PLAYER_NAME_LENGTH), DEFINE_FIELD(userID, FIELD_INTEGER),
		DEFINE_ARRAY(guid, FIELD_CHARACTER, SIGNED_GUID_LEN + 1), DEFINE_FIELD(friendsID, FIELD_INTEGER),
		DEFINE_ARRAY(friendsName, FIELD_CHARACTER, MAX_PLAYER_NAME_LENGTH), DEFINE_FIELD(fakeplayer, FIELD_BOOLEAN),
		DEFINE_FIELD(ishltv, FIELD_BOOLEAN),
#if defined(REPLAY_ENABLED)
		DEFINE_FIELD(isreplay, FIELD_BOOLEAN),
#endif
		DEFINE_ARRAY(customFiles, FIELD_INTEGER, MAX_CUSTOM_FILES), DEFINE_FIELD(filesDownloaded, FIELD_INTEGER),
...
ConVar closecaption("closecaption", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX | FCVAR_USERINFO,
...
static ConVar host_competitive_ever_enabled("host_competitive_ever_enabled", "0", FCVAR_HIDDEN,
											"Has competitive ever been enabled this run?", true, 0, true, 1, true, 1,
											false, 1, NULL);
...
static ConVar singlestep("singlestep", "0", FCVAR_CHEAT,
...
ConVar host_showcachemiss("host_showcachemiss", "0", 0,
						  "Print a debug message when the client or server cache is missed.");
static ConVar mem_dumpstats(
	"mem_dumpstats", "0", 0,
	"Dump current and max heap usage info to console at end of frame ( set to 2 for continuous output )\n");
static ConVar host_ShowIPCCallCount(
	"host_ShowIPCCallCount", "0", 0,
...
ConVar telemetry_framecount("telemetry_framecount", "0", 0, "Set Telemetry count of frames to capture",
							OnChangeTelemetryFrameCount);
...
ConVar telemetry_demostart("telemetry_demostart", "0", 0, "When playing demo, start telemetry on tick #",
						   OnChangeTelemetryDemoStart);
ConVar telemetry_demoend("telemetry_demoend", "0", 0, "When playing demo, stop telemetry on tick #",
						 OnChangeTelemetryDemoEnd);
#endif
...
ConVar host_thread_mode("host_thread_mode", (IsX360()) ? "1" : "0", 0,
...
ConVar threadpool_affinity("threadpool_affinity", "1", 0, "Enable setting affinity", 0, 0, 0, 0,
...
ConVar host_timer_spin_ms("host_timer_spin_ms", "0", FCVAR_NONE,
						  "Use CPU busy-loop for improved timer precision (dedicated only)",
						  HostTimerSpinMsChangedCallback);
...
bool GetFileFromRemoteStorage(ISteamRemoteStorage *pRemoteStorage, const char *pszRemoteFileName,
...
void CheckForFlushMemory(const char *pCurrentMapName, const char *pDestMapName)
{
	...
	static const char *mapBlackList[] = {
		// --hl2--
		"d1_canals_01", "d1_canals_05", "d1_eli_01", "d1_town_01", "d2_coast_01", "d2_prison_01", "d3_c17_01",
		"d3_c17_05", "d3_c17_09", "d3_citadel_01", "d3_breen_01",
		// --ep1--
		"ep1_c17_02", "ep1_c17_02b", "ep1_c17_05", "ep1_c17_06",
		// --ep2--
		"ep2_outland_06a", "ep2_outland_09", "ep2_outland_11", "ep2_outland_12", "ep2_outland_12a",
		// --tf--
		"tc_hydro"};
	...
	DevMsg("---CURRENT(%s), NEXT(%s)\n", (pCurrentMapName[0] ? pCurrentMapName : "----"),
		   (pDestMapName[0] ? pDestMapName : "----"));
	if(bIsMapChanging)
	{
	...
	if((stat.dwAvailPhys < host_flush_threshold.GetInt() * 1024 * 1024) || (bIsDestMapBlacklisted && bIsMapChanging))
	{
...
void SetupNewBindings()
{
	...
	FOR_EACH_TRUE_SUBKEY(pNewBindingsData, pSubKey)
	{
		...
		for(int nCurButton = (int)KEY_0; nCurButton <= (int)KEY_LAST; ++nCurButton, ++nNumAttempts)
		{
			...
			for(int iKeyIndex = 0; iKeyIndex < sizeof(nInvalidKeyBindings) / sizeof(nInvalidKeyBindings[0]);
				iKeyIndex++)
			{
			...
			{
				// Yes - use it.
				V_snprintf(szBindCmd, sizeof(szBindCmd), "bind \"%s\" \"%s\"",
						   g_pInputSystem->ButtonCodeToString(bcCurButton), pBinding);
				Cbuf_AddText(szBindCmd);
				bFound = true;
				break;
			}
...
bool XBX_SetProfileDefaultSettings(void)
{
	...
	const DWORD dwSettingIds[NUM_PROFILE_SETTINGS] = {XPROFILE_GAMER_DIFFICULTY, XPROFILE_GAMER_ACTION_MOVEMENT_CONTROL,
	...
	dwError = XUserReadProfileSettings(0,		   // Family ID (current title)
									   nPrimaryID, // User ID
									   NUM_PROFILE_SETTINGS, dwSettingIds, &dwResultSize, pResults,
	...
	Assert(pSetting->data.type == XUSER_DATA_TYPE_INT32);
	Q_snprintf(szScratch, sizeof(szScratch), "joy_movement_stick %d",
			   (pSetting->data.nData == XPROFILE_ACTION_MOVEMENT_CONTROL_L_THUMBSTICK) ? 0 : 1);
	Cbuf_AddText(szScratch);
	Q_snprintf(szScratch, sizeof(szScratch), "joy_movement_stick_default %d",
			   (pSetting->data.nData == XPROFILE_ACTION_MOVEMENT_CONTROL_L_THUMBSTICK) ? 0 : 1);
	Cbuf_AddText(szScratch);
...
void Host_ReadConfiguration_360(void)
{
	...
	bool bStorageDeviceValid =
		(XBX_GetStorageDeviceId() != XBX_INVALID_STORAGE_ID && XBX_GetStorageDeviceId() != XBX_STORAGE_DECLINED);
	bool bSaveConfig = false;

	// Call through normal API function once the content container is opened
	if(CommandLine()->CheckParm("-forcexboxreconfig") || bStorageDeviceValid == false ||
	   g_pFileSystem->FileExists(strFileName) == false)
	{
		// If we've already done this in this session, never do it again (we don't want to stomp their settings under
		// any circumstances)
		if(g_bConfigCfgExecuted == false)
		{
...
void Host_ReadConfiguration()
{
	...
	if(pRemoteStorage)
	{
		// if cloud settings is default but remote storage does not exist yet, set it to sync all because this is the
		// first computer the game is run on--default to copying everything to the cloud
...
static ConCommand recompute_speed("recompute_speed", Host_RecomputeSpeed_f,
...
void CFrameTimer::MarkFrame()
{
	...
	if(host_speeds.GetInt())
	{
		...
		Q_snprintf(
			...
			(int)fps, fs_input, fs_server, fs_client, fs_render, fs_sound, fs_cldll, fs_exec, ent_count, ticks);
...
void Host_SetClientInSimulation(bool bInSimulation)
{
	...
	// TODO:  Since this rounds down on the frame we paused, we could see a slight backsliding.  We could remember the
	// last "remainder" before pause and re-use it and
...
static ConVar host_Sleep("host_sleep", "0", FCVAR_CHEAT,
...
void _Host_RunFrame(float time)
{
	...
	int numticks;
	{
		...
		if(host_remainder >= host_state.interval_per_tick)
		{
			...
			if(Host_IsSinglePlayerGame() && sv_alternateticks.GetBool())
			{
...
// threaded path only supported in listen server
...
#endif
		{
			...
			for(int tick = 0; tick < numticks; tick++)
			{
				...
				// Only send updates on final tick so we don't re-encode network data multiple times per frame
				// unnecessarily
				bool bFinalTick = (tick == (numticks - 1));
				...
				// Additional networking ops for SPLITPACKET stuff (99.9% of the time this will be an empty list of
				// work)
			...
			if(!sv.IsDedicated())
			{
				...
				// This causes cl.gettime() to return the true clock being used for rendering (tickcount * rate +
				// remainder)
			...
			if(!cl.IsPaused() || !sv.IsPaused())
			{
				Msg("=============SIM: CLIENT %5d + %d, SERVER %5d + %d\t REM: %.2f\n", cl.GetClientTickCount(),
					numticks, sv.m_nTickCount, numticks, host_remainder * 1000.0f);
			}
#endif
		}
#ifndef SWDS
		else
		{
			...
			{
				Msg("=============SIM: CLIENT %5d + %d, SERVER %5d + %d\t REM: %.2f\n", cl.GetClientTickCount(),
					clientticks, sv.m_nTickCount, serverticks, host_remainder * 1000.0f);
			}
#endif
		}
#endif // SWDS
...
void Host_InitProcessor(void)
{
	...
	if(pi.m_bSSE)
	{
		if(MathLib_SSEEnabled())
			Q_strncat(szFeatureString, "SSE ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
		else
			Q_strncat(szFeatureString, "(SSE) ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
	}
	...
	if(pi.m_bSSE2)
	{
		if(MathLib_SSE2Enabled())
			Q_strncat(szFeatureString, "SSE2 ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
		else
			Q_strncat(szFeatureString, "(SSE2) ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
	}
	...
	if(pi.m_bMMX)
	{
		if(MathLib_MMXEnabled())
			Q_strncat(szFeatureString, "MMX ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
		else
			Q_strncat(szFeatureString, "(MMX) ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
	}
	...
	if(pi.m_b3DNow)
	{
		if(MathLib_3DNowEnabled())
			Q_strncat(szFeatureString, "3DNow ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
		else
			Q_strncat(szFeatureString, "(3DNow) ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
	}

	if(pi.m_bRDTSC)
		Q_strncat(szFeatureString, "RDTSC ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
	if(pi.m_bCMOV)
		Q_strncat(szFeatureString, "CMOV ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
	if(pi.m_bFCMOV)
		Q_strncat(szFeatureString, "FCMOV ", sizeof(szFeatureString), COPY_ALL_CHARACTERS);
	...
	else
	{
		if(Steam3Client().SteamUtils())
		{
			...
			else
			{
				...
				Steam3Client().SteamUtils()->GetAPICallResult(hAPICall, &result, sizeof(result), result.k_iCallback,
															  &bAPICallFailed);
				if(bAPICallFailed)
				{
				...
				{
					if(result.m_eCheckFileSignature == k_ECheckFileSignatureValidSignature ||
					   result.m_eCheckFileSignature == k_ECheckFileSignatureNoSignaturesFoundForThisApp)
						return true;
					if(bAllowUnknown &&
					   result.m_eCheckFileSignature == k_ECheckFileSignatureNoSignaturesFoundForThisFile)
						return true;
...
bool DLL_LOCAL Host_AllowLoadModule(const char *pFilename, const char *pPathID, bool bAllowUnknown,
			...
			if(Steam3Client().SteamUtils())
			{
				...
				if(pPathID)
				{
					...
					const char *pFullPath =
						g_pFileSystem->RelativePathToFullPath(szDllname, pPathID, szFullPath, sizeof(szFullPath));
					if(!pFullPath)
					{
...
void Host_Init(bool bDedicated)
{
	...
	if(CommandLine()->FindParm("-dev") ||
	   (CommandLine()->FindParm("-allowdebug") && !CommandLine()->FindParm("-nodev")))
	{
	...
	{
		// stop the various windows error message boxes from showing up (used by the auto-builder so it doesn't block on
		// error)
		Sys_NoCrashDialog();
	}
	...
	// if running outside of steam and NOT a dedicated server then phone home (or if "-phonehome" is passed on the
	// command line)
...
void AddTransitionResources(CSaveRestoreData *pSaveData, const char *pLevelName, const char *pLandmarkName)
{
	...
	{
		if(!Q_stricmp(pLevelName, pSaveData->levelInfo.levelList[i].mapName) &&
		   !Q_stricmp(pLandmarkName, pSaveData->levelInfo.levelList[i].landmarkName))
		{
			transitionMask = 1 << i;
			break;
		}
...
bool Host_Changelevel(bool loadfromsavedgame, const char *mapname, const char *start)
{
	...
	if(bTransitionBySave)
	{
		...
		saverestore->SetMostRecentElapsedMinutes((int)(totalSeconds / 60.0f) +
				  ...
				  bool bOldSave)
{
	...
	g_pServerPluginHandler->LevelInit(szMapName, CM_EntityString(), pszOldMap, pszLandmark, loadGame && !bOldSave,
									  bBackgroundLevel);
