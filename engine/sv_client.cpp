...
static ConVar sv_minrate("sv_minrate", "3500", FCVAR_REPLICATED,
						 "Min bandwidth rate allowed on server, 0 == unlimited");

ConVar sv_maxupdaterate("sv_maxupdaterate", "66", FCVAR_REPLICATED,
						"Maximum updates per second that the server will allow");
ConVar sv_minupdaterate("sv_minupdaterate", "10", FCVAR_REPLICATED,
...
// static ConVar sv_failuretime( "sv_failuretime", "0.5", 0, "After this long without a packet from client, don't send
// any more until client starts sending again" );
...
bool CGameClient::ProcessRespondCvarValue(CLC_RespondCvarValue *msg)
{
	if(msg->m_iCookie > 0)
	{
		if(g_pServerPluginHandler)
			g_pServerPluginHandler->OnQueryCvarValueFinished(msg->m_iCookie, edict, msg->m_eStatusCode,
															 msg->m_szCvarName, msg->m_szCvarValue);
	}
	else
	{
		...
		if(serverGameDLL && g_iServerGameDLLVersion >= 6)
		{
			...
			serverGameDLL->OnQueryCvarValueFinished(msg->m_iCookie, edict, msg->m_eStatusCode, msg->m_szCvarName,
													msg->m_szCvarValue);
#endif
		}
	}
...
bool CGameClient::ProcessFileCRCCheck(CLC_FileCRCCheck *msg)
{
	...
	{
		V_snprintf(warningStr, sizeof(warningStr), "Pure server: file [%s]\\%s does not match the server's file.", path,
				   fileName);
	}
...
void CGameClient::SetupPackInfo(CFrameSnapshot *pSnapshot)
{
	...
	serverGameClients->ClientSetupVisibility((edict_t *)m_pViewEntity, m_PackInfo.m_pClientEnt, m_PackInfo.m_PVS,
...
void CGameClient::SendSnapshot(CClientFrame *pFrame)
{
	if(m_bIsHLTV)
	{
		...
		hltv->WriteTempEntities(this, pFrame->GetSnapshot(), m_pLastSnapshot.GetObject(),
	...
	if(m_bIsReplay)
	{
		...
		int maxEnts = 255;
		replay->WriteTempEntities(this, pFrame->GetSnapshot(), m_pLastSnapshot.GetObject(),
								  *replay->GetBuffer(REPLAY_BUFFER_TEMPENTS), maxEnts);
