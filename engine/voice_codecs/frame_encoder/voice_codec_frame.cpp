...
public:
	...
	virtual int Compress(const char *pUncompressedBytes, int nSamples, char *pCompressed, int maxCompressedBytes,
						 bool bFinal)
	{
		if(!m_pFrameEncoder)
			return 0;
		...
		if(nNewSamples)
		{
			memcpy(&m_EncodeBuffer[m_nEncodeBufferSamples], &pUncompressed[nSamples - nNewSamples],
	...
	virtual int Decompress(const char *pCompressed, int compressedBytes, char *pUncompressed, int maxUncompressedBytes)
	{
		...
		{
			m_pFrameEncoder->DecodeFrame(pCompressed ? &pCompressed[curCompressedByte] : NULL,
