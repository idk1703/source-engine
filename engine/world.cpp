...
public:
	...
	void TriggerMoved(edict_t *pTriggerEntity)
	{
		...
		SpatialPartition()->EnumerateElementsInBox(PARTITION_ENGINE_SOLID_EDICTS, vecAbsMins, vecAbsMaxs, false, this);
	}
...
void SV_SolidMoved(edict_t *pSolidEnt, ICollideable *pSolidCollide, const Vector *pPrevAbsOrigin,
				   bool accurateBboxTriggerChecks)
{
	if(!pPrevAbsOrigin)
	{
		...
		SpatialPartition()->EnumerateElementsInBox(PARTITION_ENGINE_TRIGGER_EDICTS, vecWorldMins, vecWorldMaxs, false,
												   &touchEnumerator);
	...
	else
	{
		...
		SpatialPartition()->EnumerateElementsAlongRay(PARTITION_ENGINE_TRIGGER_EDICTS, touchEnumerator.m_Ray, false,
													  &touchEnumerator);
