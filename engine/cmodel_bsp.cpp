...
inline void ExtractBoxBrush(cboxbrush_t *pBox, const cbrush_t &brush, dbrushside_t *pSides, cplane_t *pPlanes,
							CUtlVector<unsigned short> &map_texinfo)
{
...
void CollisionBSPData_LoadBrushSides(CCollisionBSPData *pBSPData, CUtlVector<unsigned short> &map_texinfo)
{
	...
	pBSPData->map_boxbrushes.Attach(boxBrushCount,
...
void CollisionBSPData_LoadPhysics(CCollisionBSPData *pBSPData)
{
	...
	do
	{
		...
		if(physModel.dataSize > 0)
		{
			...
			physcollision->VCollideLoad(&pModel->vcollisionData, physModel.solidCount, (const char *)ptr,
...
void CollisionBSPData_LoadDispInfo(CCollisionBSPData *pBSPData)
{
	...
	for(i = 0; i < coreDispCount; ++i)
	{
		...
		{
			IMaterial *pMaterial =
				materials->FindMaterial(pBSPData->map_surfaces[pTex->texdata].name, TEXTURE_GROUP_WORLD, true);
