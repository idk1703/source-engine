...
ConVar save_history_count("save_history_count", "1", 0,
...
struct GAME_HEADER
{
	...
	char mapName[32];
	char comment[80];
	int mapCount; // the number of map state files in the save file.  This is usually number of maps * 3 (.hl1, .hl2,
				  // .hl3 files)
...
struct SAVE_HEADER
{
	...
	int lightStyleCount;
	int mapVersion;
	float time__USE_VCR_MODE; // This is renamed to include the __USE_VCR_MODE prefix due to a #define on win32 from the
							  // VCR mode changes The actual save games have the string "time__USE_VCR_MODE" in them
	char mapName[32];
	char skyName[32];
};
...
public:
	...
	int SaveGameSlot(const char *pSaveName, const char *pSaveComment, bool onlyThisLevel, bool bSetMostRecent,
					 const char *pszDestMap = NULL, const char *pszLandmark = NULL);
	bool SaveGameState(bool bTransition, CSaveRestoreData ** = NULL, bool bOpenContainer = true,
	...
	virtual bool IsXSave()
	{
		return (m_bIsXSave && !save_noxsave.GetBool());
	}
...
private:
	...
	int SaveReadNameAndComment(FileHandle_t f, OUT_Z_CAP(nameSize) char *name, int nameSize,
							   OUT_Z_CAP(commentSize) char *comment, int commentSize) OVERRIDE;
...
BEGIN_SIMPLE_DATADESC(GAME_HEADER)
	...
	DEFINE_FIELD(mapCount, FIELD_INTEGER), DEFINE_ARRAY(mapName, FIELD_CHARACTER, 32),
		DEFINE_ARRAY(comment, FIELD_CHARACTER, 80), DEFINE_ARRAY(originMapName, FIELD_CHARACTER, 32),
...
BEGIN_SIMPLE_DATADESC(SAVE_HEADER)
	...
	DEFINE_FIELD(skillLevel, FIELD_INTEGER), DEFINE_FIELD(connectionCount, FIELD_INTEGER),
		DEFINE_FIELD(lightStyleCount, FIELD_INTEGER), DEFINE_FIELD(mapVersion, FIELD_INTEGER),
		DEFINE_FIELD(time__USE_VCR_MODE, FIELD_TIME), DEFINE_ARRAY(mapName, FIELD_CHARACTER, 32),
		DEFINE_ARRAY(skyName, FIELD_CHARACTER, 32),
END_DATADESC()

BEGIN_SIMPLE_DATADESC(levellist_t)
	DEFINE_ARRAY(mapName, FIELD_CHARACTER, 32), DEFINE_ARRAY(landmarkName, FIELD_CHARACTER, 32),
		DEFINE_FIELD(pentLandmark, FIELD_EDICT), DEFINE_FIELD(vecLandmarkOrigin, FIELD_VECTOR),
END_DATADESC()
...
void CSaveRestore::AgeSaveFile(const char *pName, const char *ext, int count, bool bIsXSave)
{
	...
	if(!IsXSave())
	{
		if(count == 1)
		{
			Q_snprintf(oldName, sizeof(oldName), "//%s/%s%s.%s", MOD_DIR, GetSaveDir(), pName,
					   ext); // quick.sav. DON'T FixSlashes on this, it needs to be //MOD
		}
		else
		{
			Q_snprintf(oldName, sizeof(oldName), "//%s/%s%s%02d.%s", MOD_DIR, GetSaveDir(), pName, count - 1,
					   ext); // quick04.sav, etc. DON'T FixSlashes on this, it needs to be //MOD
		}

		Q_snprintf(newName, sizeof(newName), "//%s/%s%s%02d.%s", MOD_DIR, GetSaveDir(), pName, count,
...
static ConVar save_asyncdelay("save_asyncdelay", "0", 0,
...
//-----------------------------------------------------------------------------
int CSaveRestore::SaveGameSlot(const char *pSaveName, const char *pSaveComment, bool onlyThisLevel, bool bSetMostRecent,
							   const char *pszDestMap, const char *pszLandmark)
{
	...
	if(bIsQuick || bIsAutosave || bIsAutosaveDangerous)
	{
		...
		{
			g_AsyncSaveCallQueue.QueueCall(this, &CSaveRestore::AgeSaveList, CUtlEnvelope<const char *>(pSaveName),
	...
	gameHeader.mapCount =
		0; // No longer used. The map packer will place the map count at the head of the compound files (toml 7/18/2007)
	serverGameDLL->SaveWriteFields(pSaveData, "GameHeader", &gameHeader, NULL, GAME_HEADER::m_DataMap.dataDesc,
	...
	g_AsyncSaveCallQueue.QueueCall(g_pSaveRestoreFileSystem, &ISaveRestoreFileSystem::AsyncWrite,
								   CUtlEnvelope<const char *>(name), saveHeader.Base(), saveHeader.TellPut(), true,
								   false, (FSAsyncControl_t *)NULL);
	g_AsyncSaveCallQueue.QueueCall(this, &CSaveRestore::DirectoryCopy, CUtlEnvelope<const char *>(hlPath),
								   CUtlEnvelope<const char *>(name), m_bIsXSave);

	// Finish all writes and close the save game container
	// @TODO: this async finish all writes has to go away, very expensive and will make game hitchy. switch to a wait on
	// the last async op
	...
	if(!IsX360() && save_screenshot.GetBool()) // X360TBD: Faster savegame screenshots
	{
		...
		{
			Q_snprintf(m_szSaveGameScreenshotFile, sizeof(m_szSaveGameScreenshotFile), "%s%s%s.tga", GetSaveDir(),
					   pSaveName, GetPlatformExt());
		}
	}
...
int CSaveRestore::SaveReadHeader(FileHandle_t pFile, GAME_HEADER *pHeader, int readGlobalState, bool *pbOldSave)
{
	...
	serverGameDLL->SaveReadFields(pSaveData, "GameHeader", pHeader, NULL, GAME_HEADER::m_DataMap.dataDesc,
								  GAME_HEADER::m_DataMap.dataNumFields);
	...
	if(readGlobalState &&
	   pHeader->mapCount == 0) // Alfred: Only load save games from the OB era engine where mapCount is forced to zero
	{
	...
	{
		if(g_pSaveRestoreFileSystem->Read(&pHeader->mapCount, sizeof(pHeader->mapCount), pFile) !=
...
bool CSaveRestore::LoadGame(const char *pName)
{
	...
	bool retval = Host_NewGame(gameHeader.mapName, true, false, (bIsTransitionSave) ? gameHeader.originMapName : NULL,
							   (bIsTransitionSave) ? gameHeader.landmark : NULL, bOldSave);
...
void CSaveRestore::SaveGameStateGlobals(CSaveRestoreData *pSaveData)
{
	...
	pSaveData->levelInfo.time =
		0; // prohibits rebase of header.time (why not just save time as a field_float and ditch this hack?)
	serverGameDLL->SaveWriteFields(pSaveData, "Save Header", &header, NULL, SAVE_HEADER::m_DataMap.dataDesc,
	...
	for(i = 0; i < pSaveData->levelInfo.connectionCount; i++)
		serverGameDLL->SaveWriteFields(pSaveData, "ADJACENCY", pSaveData->levelInfo.levelList + i, NULL,
	...
	for(i = 0; i < MAX_LIGHTSTYLES; i++)
	{
		...
		if(ligthStyle && ligthStyle[0])
		{
			...
			serverGameDLL->SaveWriteFields(pSaveData, "LIGHTSTYLE", &light, NULL, SAVELIGHTSTYLE::m_DataMap.dataDesc,
...
bool CSaveRestore::SaveGameState(bool bTransition, CSaveRestoreData **ppReturnSaveData, bool bOpenContainer,
								 bool bIsAutosaveOrDangerous)
{
	...
	// Output to disk
	char name[256];
	int nBytesStateFile = sizeof(CURRENT_SAVEFILE_HEADER_TAG) + sizeof(sectionsInfo) + sectionsInfo.nBytesSymbols +
						  sectionsInfo.nBytesDataHeaders + sectionsInfo.nBytesData;
	...
	if(!IsXSave())
	{
		Q_snprintf(name, 256, "//%s/%s%s.HL1", MOD_DIR, GetSaveDir(),
				   GetSaveGameMapName(sv.GetMapName())); // DON'T FixSlashes on this, it needs to be //MOD
...
BEGIN_SIMPLE_DATADESC(musicsave_t)
	...
	DEFINE_ARRAY(songname, FIELD_CHARACTER, 128), DEFINE_FIELD(sampleposition, FIELD_INTEGER),
...
BEGIN_SIMPLE_DATADESC(decallist_t)
	...
	DEFINE_FIELD(position, FIELD_POSITION_VECTOR), DEFINE_ARRAY(name, FIELD_CHARACTER, 128),
		DEFINE_FIELD(entityIndex, FIELD_SHORT),
		//	DEFINE_FIELD( depth, FIELD_CHARACTER ),
		DEFINE_FIELD(flags, FIELD_CHARACTER), DEFINE_FIELD(impactPlaneNormal, FIELD_VECTOR),
...
void CSaveRestore::ReapplyDecal(bool adjacent, RestoreLookupTable *table, decallist_t *entry)
{
			...
			if(dot >= 0.99)
			{
				...
				if(hit != NULL)
				{
					...
					if(entityToHit >= 0)
					{
						...
						if(!found)
						{
							// This is a serious HACK because we're grabbing the index that the server hasn't networked
							// down to us and forcing
							//  the decal name directly.  However, the message should eventually arrive and set the
							//  decal back to the same name on the server's index...we can live with that I suppose.
							decalIndex = sv.PrecacheDecal(entry->name, RES_FATALIFMISSING);
							Draw_DecalSetName(decalIndex, entry->name);
						}

						g_pEfx->DecalShoot(decalIndex, entityToHit, clientEntity->GetModel(),
										   clientEntity->GetAbsOrigin(), clientEntity->GetAbsAngles(), entry->position,
										   0, flags);
					}
				}
			}
	...
	{
		int entityToHit =
			entry->entityIndex != 0 ? LookupRestoreSpotSaveIndex(table, entry->entityIndex) : entry->entityIndex;
		if(entityToHit >= 0)
		{
			// NOTE: I re-initialized the origin and angles as the decals pos/angle are saved in local space (ie.
			// relative to
			...
			if(pModel)
			{
				...
				if(!found)
				{
					// This is a serious HACK because we're grabbing the index that the server hasn't networked down to
					// us and forcing
					//  the decal name directly.  However, the message should eventually arrive and set the decal back
					//  to the same name on the server's index...we can live with that I suppose.
					decalIndex = sv.PrecacheDecal(entry->name, RES_FATALIFMISSING);
					Draw_DecalSetName(decalIndex, entry->name);
				}
...
void CSaveRestore::RestoreClientState(char const *fileName, bool adjacent)
{
	...
	if(r_decals.GetInt())
	{
		...
		{
			decallist_t entry;
			g_ClientDLL->SaveReadFields(pSaveData, "DECALLIST", &entry, NULL, decallist_t::m_DataMap.dataDesc,
	...
	for(int i = 0; i < sections.musiccount; i++)
	{
		...
		g_ClientDLL->SaveReadFields(pSaveData, "MUSICLIST", &song, NULL, musicsave_t::m_DataMap.dataDesc,
...
void CSaveRestore::RestoreAdjacenClientState(char const *map)
{
	...
	{
		Q_snprintf(name, sizeof(name), "//%s/%s%s.HL2", MOD_DIR, GetSaveDir(),
				   GetSaveGameMapName(map)); // DON'T FixSlashes on this, it needs to be //MOD
	}
	else
	{
		Q_snprintf(name, sizeof(name), "%s:/%s.HL2", GetCurrentMod(),
...
bool CSaveRestore::SaveClientState(const char *name)
{
	...
	for(i = 0; i < sections.decalcount; i++)
	{
		...
		g_ClientDLL->SaveWriteFields(pSaveData, "DECALLIST", entry, NULL, decallist_t::m_DataMap.dataDesc,
									 decallist_t::m_DataMap.dataNumFields);
	}
	...
	for(i = 0; i < sections.musiccount; ++i)
	{
		...
		g_ClientDLL->SaveWriteFields(pSaveData, "MUSICLIST", song, NULL, musicsave_t::m_DataMap.dataDesc,
									 musicsave_t::m_DataMap.dataNumFields);
	}
	...
	unsigned nBytes = sizeof(CURRENT_SAVEFILE_HEADER_TAG) + sizeof(magicnumber) + sizeof(sectionheaderversion) +
					  sizeof(baseclientsections_t) + sections.symbolsize + sections.headersize + sections.entitysize +
	...
	SaveMsg("Queue AsyncWrite (%s)\n", name);
	g_AsyncSaveCallQueue.QueueCall(g_pSaveRestoreFileSystem, &ISaveRestoreFileSystem::AsyncWrite,
								   CUtlEnvelope<const char *>(name), pBuffer, nBytes, true, false,
								   (FSAsyncControl_t *)NULL);
...
//-----------------------------------------------------------------------------
int CSaveRestore::SaveReadNameAndComment(FileHandle_t f, OUT_Z_CAP(nameSize) char *name, int nameSize,
...
CSaveRestoreData *CSaveRestore::LoadSaveData(const char *level)
{
	...
	if(g_pSaveRestoreFileSystem->Read((char *)(pSaveData + 1), sectionsInfo.SumBytes(), pFile) !=
...
void CSaveRestore::ParseSaveTables(CSaveRestoreData *pSaveData, SAVE_HEADER *pHeader, int updateGlobals)
{
	...
	pSaveData->Rebase();
	// Process SAVE_HEADER
	serverGameDLL->SaveReadFields(pSaveData, "Save Header", pHeader, NULL, SAVE_HEADER::m_DataMap.dataDesc,
								  SAVE_HEADER::m_DataMap.dataNumFields);
	...
	// Read adjacency list
	for(i = 0; i < pSaveData->levelInfo.connectionCount; i++)
		serverGameDLL->SaveReadFields(pSaveData, "ADJACENCY", pSaveData->levelInfo.levelList + i, NULL,
									  levellist_t::m_DataMap.dataDesc, levellist_t::m_DataMap.dataNumFields);
	...
	{
		serverGameDLL->SaveReadFields(pSaveData, "LIGHTSTYLE", &light, NULL, SAVELIGHTSTYLE::m_DataMap.dataDesc,
									  SAVELIGHTSTYLE::m_DataMap.dataNumFields);
		if(updateGlobals)
		{
...
void CSaveRestore::EntityPatchWrite(CSaveRestoreData *pSaveData, const char *level, bool bAsync)
{
	...
	else
	{
		SaveMsg("Queue AsyncWrite (%s)\n", name);
		g_AsyncSaveCallQueue.QueueCall(g_pSaveRestoreFileSystem, &ISaveRestoreFileSystem::AsyncWrite,
									   CUtlEnvelope<const char *>(name), pBuffer, nBytesEntityPatch, true, false,
									   (FSAsyncControl_t *)NULL);
	}
}

//-----------------------------------------------------------------------------
// Purpose: Read the list of entities that are no longer in the save file for this level (they've been moved to another
// level)
...
void CSaveRestore::EntityPatchRead(CSaveRestoreData *pSaveData, const char *level)
{
	...
	{
		Q_snprintf(name, sizeof(name), "//%s/%s%s.HL3", MOD_DIR, GetSaveDir(),
				   GetSaveGameMapName(level)); // DON'T FixSlashes on this, it needs to be //MOD
	}
	else
	{
		Q_snprintf(name, sizeof(name), "%s:/%s.HL3", GetCurrentMod(),
...
void CSaveRestore::AutoSaveDangerousIsSafe()
{
	...
	if(!IsX360())
	{
		Q_snprintf(szOldName, sizeof(szOldName), "//%s/%sautosavedangerous%s.tga", MOD_DIR, GetSaveDir(),
				   GetPlatformExt());
	...
	if(!IsXSave())
	{
		Q_snprintf(szOldName, sizeof(szOldName), "//%s/%sautosavedangerous%s.sav", MOD_DIR, GetSaveDir(),
				   GetPlatformExt());
...
static void SaveGame(const CCommand &args)
{
	...
	char comment[80];
	GetServerSaveCommentEx(comment, sizeof(comment), saverestore->GetMostRecentElapsedMinutes() + iAdditionalMinutes,
...
static ConVar save_huddelayframes("save_huddelayframes", "1", 0,
...
{
	int minplayers = 1;
	// serverGameClients should have been initialized by the CModAppSystemGroup Create method (so it's before the
	// Host_Init stuff which calls this)
	Assert(serverGameClients);
