...
static ConVar sv_deltaprint("sv_deltaprint", "0", 0,
...
void PrintChangeTracks()
{
	...
	FOR_EACH_LL(g_Tracks, j)
	{
		...
		ConMsg("%6.2fms       %5.2f    %6.2fms    %4d        %4d          %s\n", pCur->m_Count.GetMillisecondsF(),
			   ...
			   pCur->m_EncodeCount.GetMillisecondsF(), pCur->m_nChanged, pCur->m_nUnchanged, pCur->m_pName);
	}
...
//
static inline void SV_WriteDeltaHeader(CEntityWriteInfo &u, int entnum, int flags)
{
...
static int SV_CalcDeltaAndWriteProps(CEntityWriteInfo &u,
									 ...
									 PackedEntity *pTo)
{
	...
	int nDeltaProps = SendTable_CalcDelta(pToTable,
	...
	{
		nCulledProps =
			SendTable_CullPropsFromProxies(pToTable, deltaProps, nDeltaProps, u.m_nClientEntity - 1, NULL, -1,

										   pTo->GetRecipients(), pTo->GetNumRecipients(),

										   culledProps, ARRAYSIZE(culledProps));
	}

	// Write the properties.
	SendTable_WritePropList(pToTable,
							...
							pTo->m_nEntityIndex, culledProps, nCulledProps);
...
static inline void SV_WritePropsFromPackedEntity(CEntityWriteInfo &u, const int *pCheckProps, const int nCheckProps)
{
	...
	if(u.m_bCullProps)
	{
		...
		nSendProps = SendTable_CullPropsFromProxies(pSendTable, pCheckProps, nCheckProps, u.m_nClientEntity - 1,
	...
	SendTable_WritePropList(pSendTable, pToData, nToBits, u.m_pBuf, pTo->m_nEntityIndex,
		...
		if(pFromEnt->m_pClass->GetName() != pToEnt->m_pClass->GetName())
		{
			if(++nWhines < 4)
			{
				Msg("ERROR in SV_NeedsExplicitCreate: ent %d from/to classname (%s/%s) mismatch.\n", u.m_nNewEntity,
					pFromEnt->m_pClass->GetName(), pToEnt->m_pClass->GetName());
			}
		}
	}
#endif
...
static inline void SV_DetermineUpdateType(CEntityWriteInfo &u)
{
	...
	{
		unsigned char *pBuffer =
			hltv->m_DeltaCache.FindDeltaBits(u.m_nNewEntity, u.m_pFromSnapshot->m_nTickCount, nBits);
#endif
	...
	int nCheckProps =
		u.m_pNewPack->GetPropsChangedAfterTick(u.m_pFromSnapshot->m_nTickCount, checkProps, ARRAYSIZE(checkProps));
	...
	if(nCheckProps == -1)
	{
		...
		nCheckProps = SendTable_CalcDelta(u.m_pOldPack->m_pServerClass->m_pTable, pOldData, nOldBits, pNewData,
...
static inline void SV_WriteEnterPVS(CEntityWriteInfo &u)
{
	...
	PackedEntity *pBaseline =
		u.m_bAsDelta ? framesnapshotmanager->GetPackedEntity(u.m_pBaseline, u.m_nNewEntity) : NULL;
	...
	else
	{
		...
		ErrorIfNot(pFromData, ("SV_WriteEnterPVS: missing pFromData for '%s'.", pClass->m_pNetworkName));
	...
	u.m_nFullProps += SendTable_WriteAllDeltaProps(pClass->m_pTable, pFromData, nFromBits, pToData, nToBits,
...
static inline void SV_WriteLeavePVS(CEntityWriteInfo &u)
{
	...
	TRACE_PACKET(("  SV Leave PVS (%d) %s %s\n", u.m_nOldEntity, deleteentity ? "deleted" : "left pvs",
...
static inline int SV_WriteDeletions(CEntityWriteInfo &u)
{
	...
	for(int i = 0; i < nLast; i++)
	{
		...
		// If the entity is marked to transmit in the u.m_pTo, then it can never be destroyed by the
		// m_iExplicitDeleteSlots Another possible fix would be to clear any slots in the explicit deletes list that
		// were actually occupied when a snapshot was taken
		...
		{
			bNeedsExplicitDelete =
				(pToSnapShot->m_iExplicitDeleteSlots.Find(i) != pToSnapShot->m_iExplicitDeleteSlots.InvalidIndex());
			// We used to do more stuff here as a sanity check, but I don't think it was necessary since the only thing
			// that would unset the bould would be a "recreate" in the same slot which is already implied by the
			// u.m_pTo->transmit_entity.Get(i) check
		}
...
void CBaseServer::WriteDeltaEntities(CBaseClient *client, CClientFrame *to, CClientFrame *from, bf_write &pBuf)
{
	...
	if(!g_pLocalNetworkBackdoor)
	{
		...
		while((u.m_nOldEntity != ENTITY_SENTINEL) || (u.m_nNewEntity != ENTITY_SENTINEL))
		{
			u.m_pNewPack = (u.m_nNewEntity != ENTITY_SENTINEL)
							   ? framesnapshotmanager->GetPackedEntity(u.m_pToSnapshot, u.m_nNewEntity)
							   : NULL;
			u.m_pOldPack = (u.m_nOldEntity != ENTITY_SENTINEL)
							   ? framesnapshotmanager->GetPackedEntity(u.m_pFromSnapshot, u.m_nOldEntity)
							   : NULL;
	...
	bool bUpdateBaseline = ((client->m_nBaselineUpdateTick == -1) && (u.m_nFullProps > 0 || !u.m_bAsDelta));
