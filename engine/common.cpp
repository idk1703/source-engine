...
void COM_InitFilesystem(const char *pFullModPath)
{
	...
	if(IsPC())
	{
		...
		if(Steam3Client().SteamApps())
		{
			...
			Q_strncpy(language,
					  CommandLine()->ParmValue("-audiolanguage", Steam3Client().SteamApps()->GetCurrentGameLanguage()),
					  sizeof(language) - 1);
		}
		else
		{
...
bool COM_IsValidPath(const char *pszFilename)
{
	...
	if(Q_strlen(pszFilename) <= 0 || Q_strstr(pszFilename, "\\\\") || // to protect network paths
...
bool COM_IsValidLogFilename(const char *pszFilename)
{
	...
	if(extension)
	{
		if(Q_stricmp(extension, ".log") &&
		   Q_stricmp(extension, ".txt")) // must use .log or .txt if an extension is specified
			return false;
...
//-----------------------------------------------------------------------------
void *COM_CompressBuffer_Snappy(const void *source, unsigned int sourceLen, unsigned int *compressedLen,
...
//-----------------------------------------------------------------------------
void *COM_CompressBuffer_LZSS(const void *source, unsigned int sourceLen, unsigned int *compressedLen,
							  unsigned int maxCompressedLen)
{
	Assert(source);
	Assert(compressedLen);
...
int COM_GetUncompressedSize(const void *compressed, unsigned int compressedLen)
{
	...
	{
		size_t snappySize;
		if(snappy::GetUncompressedLength((const char *)compressed + sizeof(pHeader->id),
										 compressedLen - sizeof(pHeader->id), &snappySize))
...
bool COM_BufferToBufferDecompress(void *dest, unsigned int *destLen, const void *source, unsigned int sourceLen)
{
	...
	if(nDecompressedSize >= 0)
	{
		...
		if(pHeader->id == LZSS_ID)
		{
			...
			{
				Warning("NET_BufferToBufferDecompress: header said %d bytes would be decompressed, but we LZSS "
						"decompressed %d\n",
