...
public:
	...
	virtual int GetConnectionRetryNumber() const
	{
		return CL_CONNECTION_RETRIES;
	}
	virtual const char *GetClientName()
	{
		return cl_name.GetString();
	}
...
public:
	...
	CUtlString m_sRetrySourceTag; // string that describes why we decided to connect to this server (empty for command
								  // line, "serverbrowser", "quickplay", etc)
	...
	uint64 m_ulGameServerSteamID; // Steam ID of the game server we are trying to connect to, or are connected to.  Zero
								  // if unknown
	...
	bool m_bRestrictServerCommands; // If true, then the server is only allowed to execute commands marked with
									// FCVAR_SERVER_CAN_EXECUTE on the client.
	bool m_bRestrictClientCommands; // If true, then IVEngineClient::ClientCmd is only allowed to execute commands
									// marked with FCVAR_CLIENTCMD_CAN_EXECUTE on the client.
};
