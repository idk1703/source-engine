...
// Not to be confused with the game's server - this is a completely different thing that allows multiple game clients to
// connect to a single client, so con commands can be sent simultaneously to multiple running game clients, screen
// captures can be sent in real-time to a single client for comparison purposes, and all convar's can be diff'd between
// all connected clients and the server. The server portion of this code needs socketlib, which hasn't been ported to
// Linux yet, and shouldn't be shipped because it could be a security risk (it's only intended for primarily graphics
// debugging and detailed comparisons of GL vs. D3D9).
...
public:
	frame_buf_window(const char *pTitle = "frame_buf_window", int width = 640, int height = 480, int scaleX = 1,
					 int scaleY = 1);

	virtual ~frame_buf_window();

	// Return true if you handle the window message.
	typedef bool (*user_window_proc_ptr_t)(LRESULT &hres, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam,
										   void *pData);
	void set_window_proc_callback(user_window_proc_ptr_t windowProcPtr, void *pData)
	{
		m_pWindow_proc = windowProcPtr;
		m_pWindow_proc_data = pData;
	}
	...
	const simple_bgr_bitmap &frameBuffer(void) const
	{
		return m_frame_buffer;
	}

	simple_bgr_bitmap &frameBuffer(void)
	{
		return m_frame_buffer;
	}
...
void frame_buf_window::create_window(const char *pTitle)
{
	...
	m_window =
		CreateWindowEx(0, pTitle, pTitle, WS_OVERLAPPEDWINDOW, m_orig_x, m_orig_y, rect.right, rect.bottom, 0, 0, 0, 0);
	...
	switch(message)
	{
		case WM_PAINT:
		{
			if(m_frame_buffer.is_valid())
			{
				...
				StretchDIBits(m_windowHDC, 0, 0, window_size.right, window_size.bottom, 0, 0, m_width, m_height,
							  m_frame_buffer.get_ptr(), m_pBitmap_hdr, DIB_RGB_COLORS, SRCCOPY);
...
struct DumpedConVar_t
{
	...
	bool operator<(const DumpedConVar_t &rhs) const
	{
		return V_strcmp(m_Name.Get(), rhs.m_Name.Get()) < 0;
	}
};
...
public:
	...
	bool TickConnectionSend()
	{
		...
		{
			int nBytesWritten = 0;
			SocketErrorCode_t nWriteErr = m_pSocket->WriteToEndpoint(m_nEndpointIndex, &m_SendBuf[m_nSendBufOfs],
																	 m_SendBuf.Count() - m_nSendBufOfs, &nBytesWritten);
	...
	bool TickConnection()
	{
		...
		if(m_bReceivedNewCameraPos)
		{
			...
			char buf[256];
			// V_snprintf( buf, sizeof( buf ), "setpos_exact %f %f %f;setang_exact %f %f %f\n", m_NewCameraPos.m_Pos.x,
			// m_NewCameraPos.m_Pos.y, m_NewCameraPos.m_Pos.z, m_NewCameraPos.m_Angle.x, m_NewCameraPos.m_Angle.y,
			// m_NewCameraPos.m_Angle.z );
			V_snprintf(buf, sizeof(buf), "setpos_exact 0x%X 0x%X 0x%X;setang_exact 0x%X 0x%X 0x%X\n",
					   *(DWORD *)&m_NewCameraPos.m_Pos.x, *(DWORD *)&m_NewCameraPos.m_Pos.y,
					   *(DWORD *)&m_NewCameraPos.m_Pos.z, *(DWORD *)&m_NewCameraPos.m_Angle.x,
					   *(DWORD *)&m_NewCameraPos.m_Angle.y, *(DWORD *)&m_NewCameraPos.m_Angle.z);
	...
	bool HasNewScreenshotFlag() const
	{
		return m_bHasNewScreenshot;
	}
	void ClearNewScreenshotFlag()
	{
		m_bHasNewScreenshot = false;
	}
	uint GetScreenshotID() const
	{
		return m_nScreenshotID;
	}
	simple_bitmap &GetScreenshot()
	{
		return m_screenshot;
	}

	bool HasNewConVarDumpFlag() const
	{
		return m_bHasNewConVarDump;
	}
	void ClearHasNewConVarDumpFlag()
	{
		m_bHasNewConVarDump = false;
	}
	DumpedConVarVector_t &GetConVarDumpVector()
	{
		return m_DumpedConVars;
	}
	const DumpedConVarVector_t &GetConVarDumpVector() const
	{
		return m_DumpedConVars;
	}
	...
	bool ProcessMessages()
	{
		while(m_RecvBuf.Count() >= cPacketHeaderSize)
		{
			...
			switch(header.m_nType)
			{
				...
				{
					const ScreenshotPacket_t &requestPacket =
						*reinterpret_cast<const ScreenshotPacket_t *>(&m_RecvBuf[0]);
					...
					size_t nPNGSize = 0;
					void *pPNGData =
						tdefl_write_image_to_png_file_in_memory(s_pBuf, nWidth, nHeight, 3, &nPNGSize, true);
				...
				{
					const ScreenshotPacket_t &replyPacket =
						*reinterpret_cast<const ScreenshotPacket_t *>(&m_RecvBuf[0]);
					...
					{
						char szFilename[512];
						V_snprintf(szFilename, sizeof(szFilename), "%s_%u.png", replyPacket.m_szFilename,
								   m_nEndpointIndex);
					...
					else
					{
						...
						unsigned char *pImageData = stbi_load_from_memory(reinterpret_cast<stbi_uc const *>(pPNGData),
																		  nPNGDataSize, &nWidth, &nHeight, &nComp, 3);
				...
				{
					const SetCameraPosPacket_t &setCameraPosPacket =
						reinterpret_cast<const SetCameraPosPacket_t &>(m_RecvBuf[0]);
...
public:
	CConCommandServer() : m_bInitialized(false), m_pFrameBufWindow(NULL), m_nViewEndpointIndex(0) {}
	...
	void TickFrame(float flTime)
	{
		...
		if(ccs_broadcast_camera.GetBool())
		{
			...
			if(ccs_camera_sync.GetBool())
			{
				...
				char buf[256];
				V_snprintf(buf, sizeof(buf), "setpos_exact 0x%X 0x%X 0x%X;setang_exact 0x%X 0x%X 0x%X\n",
						   *(DWORD *)&setCameraPacket.m_Pos.x, *(DWORD *)&setCameraPacket.m_Pos.y,
						   *(DWORD *)&setCameraPacket.m_Pos.z, *(DWORD *)&setCameraPacket.m_Angle.x,
						   *(DWORD *)&setCameraPacket.m_Angle.y, *(DWORD *)&setCameraPacket.m_Angle.z);
				Cbuf_AddText(buf);
				Cbuf_Execute();
			}
		}
				...
				if(client.HasNewScreenshotFlag())
				{
					...
					if(ccs_remote_screenshots_delta.GetBool())
					{
						if(videomode && !bHasUpdatedScreenshot)
						{
							...
							videomode->ReadScreenPixels(0, 0, nScreenWidth, nScreenHeight, m_screenshot.get_ptr(),
														IMAGE_FORMAT_RGB888);
						}
			...
			else
			{
				...
				else
				{
					...
					Warning("%s: Convar diff: server=\"%s\", client=\"%s\"\n", pServerConVarName, pServerConVarValue,
							clientConVars[nClientConVarIndex].m_Value);
				}
			}
		}
...
CON_COMMAND_F(ccs_cmd_local,
			  "Send a console message to any connected con command server clients, and also issue the command locally.",
			  FCVAR_CHEAT)
{
	if(g_ConCommandServer.IsInitialized())
	{
