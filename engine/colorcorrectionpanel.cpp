...
static const char *s_pColorCorrectionToolNames[CC_TOOL_COUNT] = {
	"No Tool Active", "Curves Tool", "Levels Tool", "Selected HSV Tool", "Lookup Tool", "Color Balance Tool",
};
...
static void ComputeSplinePoint(float flInColor, Vector *pControlPoints[4], Vector &vecOut)
{
	...
	while(true)
	{
		Catmull_Rom_Spline(*pControlPoints[0], *pControlPoints[1], *pControlPoints[2], *pControlPoints[3], flMid,
						   vecOut);
...
public:
	...
	{
		V_strcpy_safe(m_pName, pName);
	}

	virtual IColorOperation *Clone();

	virtual ColorCorrectionTool_t ToolID()
	{
		return CC_TOOL_CURVES;
	}
...
const char *CColorCurvesUIPanel::s_pColorMaskLabel[CColorCurvesUIPanel::COLOR_MASK_TYPE_COUNT] = {"RGB", "Red", "Green",
																								  "Blue"};

//-----------------------------------------------------------------------------
// constructor
//-----------------------------------------------------------------------------
CColorCurvesUIPanel::CColorCurvesUIPanel(vgui::Panel *pParent, CCurvesColorOperation *pOp)
...
public:
	...
	CColorOperationList *GetColorOpList()
	{
		return m_pOpList;
	}
	void SetColorOpList(CColorOperationList *pList)
	{
		m_pOpList = pList;
	}
...
// constructor, destructor
//-----------------------------------------------------------------------------
CColorHistogramPanel::CColorHistogramPanel(vgui::Panel *pParent, const char *pName, CLevelsColorOperation *pOp)
...
void CColorSlider::OnCursorMoved(int x, int y)
{
	...
	if(m_nSelectedKnob >= 0)
	{
		...
		if(m_nSelectedKnob < 2 && m_nKnobCount == 3)
		{
			...
			float fOldRelGrey =
				(m_flKnobPosition[2] - m_flKnobPosition[0]) / (m_flKnobPosition[1] - m_flKnobPosition[0]);
...
const char *CColorLevelsUIPanel::s_pColorMaskLabel[CColorLevelsUIPanel::COLOR_MASK_TYPE_COUNT] = {"RGB", "Red", "Green",
																								  "Blue"};

//-----------------------------------------------------------------------------
// constructor
//-----------------------------------------------------------------------------
CColorLevelsUIPanel::CColorLevelsUIPanel(vgui::Panel *pParent, CLevelsColorOperation *pOp)
	...
	for(int i = 0; i < m_SelectedRGBs.Count(); i++)
	{
		...
		switch(m_SelectionMethod)
		{
			...
			case SELECT_NEARBY_HUE:
				RGBtoHSV(rgb, hsv);
				flCurSelAmount =
					FuzzyLessThan(fabsf(hsv.x - m_SelectedHSVs[i].x) / 360.0f, m_Tolerance, m_Fuzziness * m_Tolerance);
				break;
			...
			case SELECT_NEARBY_SATURATION:
				RGBtoHSV(rgb, hsv);
				flCurSelAmount =
					FuzzyLessThan(fabsf(hsv.y - m_SelectedHSVs[i].y), m_Tolerance, m_Fuzziness * m_Tolerance);
				break;
			...
			case SELECT_NEARBY_VALUE:
				RGBtoHSV(rgb, hsv);
				flCurSelAmount =
					FuzzyLessThan(fabsf(hsv.z - m_SelectedHSVs[i].z), m_Tolerance, m_Fuzziness * m_Tolerance);
				break;
		}
...
CFullScreenSelectionPanel::CFullScreenSelectionPanel(const char *pName, CSelectedHSVOperation *pOp,
													 vgui::Panel *pParent)
	: BaseClass(pParent, pName)
{
	m_bMouseDown = false;
...
//-----------------------------------------------------------------------------
CSelectedHSVUIPanel::CSelectedHSVUIPanel(vgui::Panel *parent, CSelectedHSVOperation *pOp)
...
void CSelectedHSVUIPanel::OnSelectionMethodSelected()
{
	...
	CSelectedHSVOperation::SelectionMethod_t method =
		(CSelectedHSVOperation::SelectionMethod_t)m_pSelectionMethod->GetActiveItem();
	m_pHSVOperation->SetSelectionMethod(method);

	if(method == CSelectedHSVOperation::SELECT_NEARBY_RGB || method == CSelectedHSVOperation::SELECT_NEARBY_HUE ||
...
void CSelectedHSVUIPanel::ReadUncorrectedImage(Rect_t *pSrcRect, unsigned char *pPreviewImage)
{
	...
	for(int i = 0; i < g_nPreviewImageHeight; i++)
	{
		Q_memcpy(m_pUncorrectedImage->GetImageBuffer() + i * IMAGE_BUFFER_MAX_DIM,
				 pPreviewImage + i * g_nPreviewImageWidth * 4, g_nPreviewImageWidth * 4);
	}
...
// constructor
//-----------------------------------------------------------------------------
CColorLookupUIPanel::CColorLookupUIPanel(vgui::Panel *pParent, CColorLookupOperation *pOp)
...
void CColorBalanceOperation::CreateLookupTables()
{
	...
	cyan_red_transfer[CC_BALANCE_MODE_SHADOWS] =
		(m_CyanRedBalance[CC_BALANCE_MODE_SHADOWS] > 0.0f) ? m_ShadowsAddTransfer : m_ShadowsSubTransfer;
	cyan_red_transfer[CC_BALANCE_MODE_MIDTONES] =
		(m_CyanRedBalance[CC_BALANCE_MODE_MIDTONES] > 0.0f) ? m_MidtonesAddTransfer : m_MidtonesSubTransfer;
	cyan_red_transfer[CC_BALANCE_MODE_HIGHLIGHTS] =
		(m_CyanRedBalance[CC_BALANCE_MODE_HIGHLIGHTS] > 0.0f) ? m_HighlightsAddTransfer : m_HighlightsSubTransfer;

	magenta_green_transfer[CC_BALANCE_MODE_SHADOWS] =
		(m_MagentaGreenBalance[CC_BALANCE_MODE_SHADOWS] > 0.0f) ? m_ShadowsAddTransfer : m_ShadowsSubTransfer;
	...
	magenta_green_transfer[CC_BALANCE_MODE_HIGHLIGHTS] =
		(m_MagentaGreenBalance[CC_BALANCE_MODE_HIGHLIGHTS] > 0.0f) ? m_HighlightsAddTransfer : m_HighlightsSubTransfer;

	yellow_blue_transfer[CC_BALANCE_MODE_SHADOWS] =
		(m_YellowBlueBalance[CC_BALANCE_MODE_SHADOWS] > 0.0f) ? m_ShadowsAddTransfer : m_ShadowsSubTransfer;
	yellow_blue_transfer[CC_BALANCE_MODE_MIDTONES] =
		(m_YellowBlueBalance[CC_BALANCE_MODE_MIDTONES] > 0.0f) ? m_MidtonesAddTransfer : m_MidtonesSubTransfer;
	yellow_blue_transfer[CC_BALANCE_MODE_HIGHLIGHTS] =
		(m_YellowBlueBalance[CC_BALANCE_MODE_HIGHLIGHTS] > 0.0f) ? m_HighlightsAddTransfer : m_HighlightsSubTransfer;
...
// constructor
//-----------------------------------------------------------------------------
CColorBalanceUIPanel::CColorBalanceUIPanel(vgui::Panel *pParent, CColorBalanceOperation *pOp)
...
CLookupViewPanel::CLookupViewPanel(vgui::Panel *parent, ColorCorrectionHandle_t CCHandle)
...
CLookupViewWindow::CLookupViewWindow(vgui::Panel *parent, ColorCorrectionHandle_t CCHandle)
...
CNewOperationDialog::CNewOperationDialog(vgui::Panel *parent, CColorOperationList *pOpList)
...
CColorOperationListPanel::CColorOperationListPanel(vgui::Panel *parent, ColorCorrectionHandle_t CCHandle)
...
void CColorOperationListPanel::LaunchOperationPanel(IColorOperation *pOp)
{
	if(pOp)
	{
		...
		{
			case CC_TOOL_BALANCE:
				pOpPanel = new CColorBalanceUIPanel(this, (CColorBalanceOperation *)pOp);
				break;
			case CC_TOOL_CURVES:
...
void CColorCorrectionTools::GrabPreColorCorrectedFrame(int x, int y, int width, int height)
{
	...
	pRenderContext->ReadPixelsAndStretch(&srcRect, &dstRect, (unsigned char *)m_pPreviewImage, IMAGE_FORMAT_BGRX8888,
...
static ConCommand colorcorrectionui("colorcorrectionui", ShowHideColorCorrectionUI,
									"Show/hide the color correction tools UI.", FCVAR_CHEAT);
...
static ConCommand print_colorcorrection("print_colorcorrection", PrintColorCorrection,
