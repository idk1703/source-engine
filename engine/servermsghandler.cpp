...
void CClientState::ConnectionClosing(const char *reason)
{
	...
	{
		ConMsg("Disconnect: %s.\n", reason);
		if(!Q_stricmp(reason, INVALID_STEAM_TICKET) || !Q_stricmp(reason, INVALID_STEAM_LOGON_TICKET_CANCELED))
		{
...
bool CClientState::ProcessTick(NET_Tick *msg)
{
	...
	g_ClientGlobalVariables.frametime =
		(tick - oldtickcount) *
		host_state.interval_per_tick; // We used to call GetFrameTime() here, but 'insimulation' is always
...
bool CClientState::ProcessVoiceData(SVC_VoiceData *msg)
{
	...
	bool bInReplay =
		engineClient->IsPlayingDemo() && g_pEngineClientReplay && g_pEngineClientReplay->IsPlayingReplayDemo();
...
bool CClientState::ProcessBSPDecal(SVC_BSPDecal *msg)
{
	...
	if(model == NULL)
	{
		...
		Warning("Warning! Static BSP decal (%s), on NULL model index %i for entity index %i.\n", matname,
				msg->m_nModelIndex, msg->m_nEntityIndex);
		return true;
	}

	if(r_decals.GetInt())
	{
		g_pEfx->DecalShoot(msg->m_nDecalTextureIndex, msg->m_nEntityIndex, model, vec3_origin, vec3_angle, msg->m_Pos,
						   NULL, msg->m_bLowPriority ? 0 : FDECAL_PERMANENT);
	}
