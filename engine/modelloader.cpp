...
#endif
static ConVar mod_offline_hdr_switch("mod_offline_hdr_switch", CONVAR_DEFAULT_MOD_OFFLINE_HDR_SWITCH,
									 FCVAR_INTERNAL_USE,
...
bool g_bBakedPropLightingNoSeparateHDR =
	false; // Some maps only have HDR lighting on props, contained in the file for non-hdr light data
...
public:
	...
	virtual void UpdateDynamicModels()
	{
		InternalUpdateDynamicModels(false);
	}

	// Called by server and client engine code to flush unreferenced dynamic models
	virtual void FlushDynamicModels()
	{
	...
	virtual bool RegisterModelLoadCallback(model_t *pModel, bool bClientOnly, IModelLoadCallback *pCallback,
										   bool bCallImmediatelyIfLoaded);
...
private:
	...
	public:
		...
		CDynamicModelInfo()
			: m_iRefCount(0), m_iClientRefCount(0), m_nLoadFlags(INVALIDFLAG), m_uLastTouchedMS_Div256(0)
		{
		}
		int16 m_iRefCount;
...
void CMapLoadHelper::Init(model_t *pMapModel, const char *loadname)
{
	...
	s_MapFileHandle =
		g_pFileSystem->OpenEx(s_szMapName, "rb", IsX360() ? FSOPEN_NEVERINPACK : 0, IsX360() ? "GAME" : NULL);
	...
	// Store map version, but only do it once so that the communication between the engine and Hammer isn't broken. The
	// map version is incremented whenever a Hammer to Engine session is established so resetting the global map version
	// each time causes a problem.
			...
			for(int i = 0; i < HEADER_LUMPS; i++)
			{
				if((LumpOffset(i) > lightingOffset) && (i != LUMP_PAKFILE))
				{
					Warning("CMapLoadHelper: Cannot free lighting lump (should be last before the PAK lump). "
							"Regenerate the .360.bsp file with the latest version of makegamedata.");
					return;
				}
			}
...
CMapLoadHelper::CMapLoadHelper(int lumpToLoad)
{
	...
	if(lump->uncompressedSize != 0)
	{
		...
		AssertMsg(CLZMA::IsCompressed(m_pData), "Lump claims to be compressed but is not recognized as LZMA");

		m_nLumpSize = CLZMA::GetActualSize(m_pData);
		AssertMsg(lump->uncompressedSize == m_nLumpSize, "Lump header disagrees with lzma header for compressed lump");
...
void Map_CheckFeatureFlags()
{
	...
	if(CMapLoadHelper::LumpSize(LUMP_MAP_FLAGS) > 0)
	{
		...
		g_bBakedPropLightingNoSeparateHDR = (flags_lump.m_LevelFlags & LVLFLAGS_BAKED_STATIC_PROP_LIGHTING_HDR) == 0;
	}
}
...
bool Map_CheckForHDR(model_t *pModel, const char *pLoadName)
{
	...
	else
	{
		...
		bHasHDR = CMapLoadHelper::LumpSize(LUMP_LIGHTING_HDR) > 0 && CMapLoadHelper::LumpSize(LUMP_WORLDLIGHTS_HDR) > 0;
	...
	bool bEnableHDR = (IsX360() && bHasHDR) || (bHasHDR && (mat_hdr_level.GetInt() >= 2) &&
...
void Mod_LoadWorldlights(CMapLoadHelper &lh, bool bIsHDR)
{
	...
	lh.GetMap()->worldlights =
		(dworldlight_t *)Hunk_AllocName(lh.LumpSize(), va("%s [%s]", lh.GetLoadName(), "worldlights"));
	...
	if(r_lightcache_zbuffercache.GetInt())
	{
		...
		lh.GetMap()->shadowzbuffers =
			(lightzbuffer_t *)Hunk_AllocName(zbufSize, va("%s [%s]", lh.GetLoadName(), "shadowzbuffers"));
	...
	for(int i = 0; i < lh.GetMap()->numworldlights; i++)
	{
		...
		{
			if((lh.GetMap()->worldlights[i].constant_attn == 0.0) && (lh.GetMap()->worldlights[i].linear_attn == 0.0) &&
...
static void CalcSurfaceExtents(CMapLoadHelper &lh, SurfaceHandle_t surfID)
{
	...
	for(i = 0; i < 2; i++)
	{
		...
		if(!(tex->flags & SURF_NOLIGHT) &&
		   MSurf_LightmapExtents(surfID, pBrushData)[i] > MSurf_MaxLightmapSizeWithBorder(surfID))
		{
...
void Mod_LoadVertNormalIndices(void)
{
	...
	unsigned short *out =
		(unsigned short *)Hunk_AllocName(lh.LumpSize(), va("%s [%s]", lh.GetLoadName(), "vertnormalindices"));
...
void Mod_LoadLump(model_t *loadmodel, int iLump, char *loadname, int elementSize, void **ppData, int *nElements)
{
...
void Mod_LoadFaces(void)
{
	...
	if(g_pMaterialSystemHardwareConfig->GetHDRType() != HDR_TYPE_NONE && CMapLoadHelper::LumpSize(LUMP_FACES_HDR) > 0)
	{
	...
	msurfacelighting_t *pLighting =
		Hunk_AllocNameAlignedClear<msurfacelighting_t>(count, 32, va("%s [%s]", lh.GetLoadName(), "surfacelighting"));

	lh.GetMap()->surfaces1 = out1;
	lh.GetMap()->surfaces2 = out2;
	lh.GetMap()->surfacelighting = pLighting;
	lh.GetMap()->surfacenormals =
		Hunk_AllocNameAlignedClear<msurfacenormal_t>(count, 2, va("%s [%s]", lh.GetLoadName(), "surfacenormal"));
...
void Mod_LoadLeafs_Version_0(CMapLoadHelper &lh)
{
	...
	// one sample per leaf
	lh.GetMap()->m_pLeafAmbient =
		(mleafambientindex_t *)Hunk_AllocName(count * sizeof(*lh.GetMap()->m_pLeafAmbient), "LeafAmbient");
	lh.GetMap()->m_pAmbientSamples =
		(mleafambientlighting_t *)Hunk_AllocName(count * sizeof(*lh.GetMap()->m_pAmbientSamples), "LeafAmbientSamples");
...
//			*loadname -
//-----------------------------------------------------------------------------
void Mod_LoadLeafs_Version_1(CMapLoadHelper &lh, CMapLoadHelper &ambientLightingLump,
							 CMapLoadHelper &ambientLightingTable)
{
	...
	else
	{
		...
		lh.GetMap()->m_pLeafAmbient =
			(mleafambientindex_t *)Hunk_AllocName(ambientLightingTable.LumpSize(), "LeafAmbient");
		lh.GetMap()->m_pAmbientSamples =
			(mleafambientlighting_t *)Hunk_AllocName(ambientLightingLump.LumpSize(), "LeafAmbientSamples");
...
void Mod_LoadLeafMinDistToWater(void)
{
	...
	else
	{
		...
		count = lh.LumpSize() / sizeof(*in);
		out = (unsigned short *)Hunk_AllocName(count * sizeof(*out),
...
void Mod_LoadMarksurfaces(void)
{
	...
	SurfaceHandle_t *surfList = (SurfaceHandle_t *)Hunk_AllocName(realCount * sizeof(SurfaceHandle_t),
...
bool Mod_LoadGameLump(int lumpId, void *pOutBuffer, int size)
{
	...
	if(!CLZMA::IsCompressed(pData) ||
	   CLZMA::GetActualSize((unsigned char *)pData) != g_GameLumpDict[i].uncompressedSize)
	{
...
void Mod_LoadGameLumpDict(void)
{
	...
	if(lhSize >= sizeof(dgamelumpheader_t))
	{
		...
		if(pGameLumpHeader->lumpCount < 0 || pGameLumpHeader->lumpCount > nMaxGameLumps ||
		...
		else
		{
			...
			{
				if(pGameLump[i].fileofs >= 0 && (unsigned int)pGameLump[i].fileofs >= (unsigned int)lh.LumpOffset() &&
				   (unsigned int)pGameLump[i].fileofs < (unsigned int)lh.LumpOffset() + lhSize &&
				   pGameLump[i].filelen > 0)
				{
					unsigned int compressedSize = 0;
					if(i + 1 < pGameLumpHeader->lumpCount && pGameLump[i + 1].fileofs > pGameLump[i].fileofs &&
...
void CMDLCacheNotify::ComputeModelFlags(model_t *pModel, MDLHandle_t handle)
{
	...
	pModel->flags &= ~(MODELFLAG_TRANSLUCENT_TWOPASS | MODELFLAG_VERTEXLIT | MODELFLAG_TRANSLUCENT |
					   MODELFLAG_MATERIALPROXY | MODELFLAG_FRAMEBUFFER_TEXTURE | MODELFLAG_STUDIOHDR_USES_FB_TEXTURE |
					   MODELFLAG_STUDIOHDR_USES_BUMPMAPPING | MODELFLAG_STUDIOHDR_USES_ENV_CUBEMAP);
	...
	for(int i = 0; i < materialCount; ++i)
	{
		...
		if(pMaterial->NeedsPowerOfTwoFrameBufferTexture(
			   false)) // The false checks if it will ever need the frame buffer, not just this frame
		{
...
{
	static void QueuedLoaderMapCallback(void *pContext, void *pContext2, const void *pData, int nSize,
	...
	switch(mod->type)
	{
		...
		case mod_brush:
		{
			...
			if(IsX360())
			{
				...
				bQueuedLoader = g_pQueuedLoader->BeginMapLoading(
					mod->strName, g_pMaterialSystemHardwareConfig->GetHDREnabled(), bOptimizeMapReload);
			}
...
static void BuildSpriteLoadName(const char *pName, char *pOut, int outLen, bool &bIsVideo)
{
	...
	if((bIsVideo || bIsVMT) && (strchr(pName, '/') || strchr(pName, '\\')))
	{
		...
		if(bIsVMT)
		{
			...
			if(Q_stristr(pName, "materials/") == pName || Q_stristr(pName, "materials\\") == pName)
			{
				// skip past materials/
				pNameStart = &pName[10];
			}
	...
	FOR_EACH_MAP_FAST(m_Models, i)
	{
		...
		else
		{
			if(g_pMDLCache->IsDataLoaded(pModel->studio, MDLCACHE_STUDIOHWDATA))
			{
				...
				if(pStudioHdr)
				{
					// Ok, we didn't have to do a full reload, but if any of our materials changed, flush out the
					// studiohwdata because the vertex format may have changed.
					IMaterial *pMaterials[128];
					int nMaterials =
						g_pStudioRender->GetMaterialList(pStudioHdr, ARRAYSIZE(pMaterials), &pMaterials[0]);
...
void CModelLoader::UnloadAllModels(bool bCheckReference)
{
	...
	FOR_EACH_MAP_FAST(m_Models, i)
	{
		...
		{
			// models preloaded by the queued loader are not initially claimed and MUST remain until the end of the load
			// process unclaimed models get unloaded during the post load purge
			continue;
		}
...
static void Mod_ComputeBrushModelFlags(model_t *mod)
{
	...
	for(i = 0; i < scount; ++i)
	{
		...
		if((pTex->flags & SURF_NODECALS) || pMaterial->GetMaterialVarFlag(MATERIAL_VAR_SUPPRESS_DECALS) ||
							   ...
							   float fInstanceAlphaModulate)
{
	...
	switch(mod->type)
	{
		...
		case mod_studio:
		{
			...
			IMaterial *pMaterials[128];
			int materialCount = g_pStudioRender->GetMaterialListFromBodyAndSkin(mod->studio, nSkin, nBody,
																				ARRAYSIZE(pMaterials), pMaterials);
			for(int i = 0; i < materialCount; i++)
			{
		...
		case mod_brush:
		{
			for(i = 0; i < pModel->brush.nummodelsurfaces; ++i)
			{
				SurfaceHandle_t surfID =
					SurfaceHandleFromIndex(pModel->brush.firstmodelsurface + i, pModel->brush.pShared);
				if(MSurf_Flags(surfID) & SURFDRAW_NODRAW)
					continue;
...
static void MarkBrushModelWaterSurfaces(model_t *world, Vector const &mins, Vector const &maxs, model_t *brush)
{
...
void CModelLoader::Map_LoadModel(model_t *mod)
{
	...
	Mod_LoadLump(mod, LUMP_CLIPPORTALVERTS, va("%s [%s]", m_szLoadName, "clipportalverts"),
				 sizeof(m_worldBrushData.m_pClipPortalVerts[0]), (void **)&m_worldBrushData.m_pClipPortalVerts,
				 &m_worldBrushData.m_nClipPortalVerts);

	COM_TimestampedLog("  LUMP_AREAPORTALS");
	Mod_LoadLump(mod, LUMP_AREAPORTALS, va("%s [%s]", m_szLoadName, "areaportals"),
				 sizeof(m_worldBrushData.m_pAreaPortals[0]), (void **)&m_worldBrushData.m_pAreaPortals,
				 &m_worldBrushData.m_nAreaPortals);

	COM_TimestampedLog("  LUMP_AREAS");
	Mod_LoadLump(mod, LUMP_AREAS, va("%s [%s]", m_szLoadName, "areas"), sizeof(m_worldBrushData.m_pAreas[0]),
...
static void GetSpriteInfo(const char *pName, bool bIsVideo, int &nWidth, int &nHeight, int &nFrameCount)
{
	...
	{
		pVideoMaterial =
			g_pVideo->CreateVideoMaterial(pName, pName, "GAME", VideoPlaybackFlags::DEFAULT_MATERIAL_OPTIONS,
...
void CModelLoader::DumpVCollideStats()
{
	...
	for(i = 0; (m_Models).IsUtlMap && i < (m_Models).MaxElement(); ++i)
		if(!(m_Models).IsValidIndex(i))
			continue;
		else
		{
			model_t *pModel = m_Models[i].modelpointer;
...
bool CModelLoader::Map_IsValid(char const *pMapFile, bool bQuiet /* = false */)
{
	...
	// - Don't allow quotes or ;"' as defense-in-depth against script abuses (and, no real reason for mapnames to use
	// these)
...
modtype_t CModelLoader::GetTypeFromName(const char *pModelName)
{
	...
	if(pExt)
	{
		...
		else if(g_pVideo != NULL &&
				g_pVideo->LocateVideoSystemForPlayingFile(pModelName) != VideoSystem::NONE) // video sprite
		{
			return mod_sprite;
		}
	}
...
model_t *CModelLoader::GetDynamicModel(const char *name, bool bClientOnly)
{
	...
	CDynamicModelInfo &dyn =
		m_DynamicModels[m_DynamicModels.Insert(pModel)]; // Insert returns existing if key is already set
...
void CModelLoader::UpdateDynamicModelLoadQueue()
{
	...
	if(mod_dynamicloadthrottle.GetFloat() > 0 &&
	   Plat_FloatTime() < s_LastDynamicLoadTime + mod_dynamicloadthrottle.GetFloat())
		return;
...
bool CModelLoader::RegisterModelLoadCallback(model_t *pModel, bool bClientOnly, IModelLoadCallback *pCallback,
											 bool bCallImmediatelyIfLoaded)
{
	UtlHashHandle_t hDyn = m_DynamicModels.Find(pModel);
	Assert(hDyn != m_DynamicModels.InvalidHandle());
...
void CModelLoader::AddRefDynamicModel(model_t *pModel, bool bClientSideRef)
{
	...
	DynamicModelDebugMsg("model %p [%s] addref %d (%d)\n", pModel, pModel->strName.String(), dyn.m_iRefCount,
						 dyn.m_iClientRefCount);
...
void CModelLoader::UnregisterModelLoadCallback(model_t *pModel, bool bClientOnly, IModelLoadCallback *pCallback)
{
	...
	if(int *pCallbackRegistrationCount = m_RegisteredDynamicCallbacks.GetPtr(callbackID))
	{
		...
		{
			for(UtlHashHandle_t i = m_DynamicModels.FirstHandle(); i != m_DynamicModels.InvalidHandle();
...
void CModelLoader::InternalUpdateDynamicModels(bool bIgnoreTime)
{
	...
	while(i != m_DynamicModels.InvalidHandle())
	{
		...
		if(dyn.m_iRefCount <= 0 && !(dyn.m_nLoadFlags & CDynamicModelInfo::LOADING) &&
		   ((now - (dyn.m_uLastTouchedMS_Div256 << 8)) >= delay ||
			!(dyn.m_nLoadFlags & CDynamicModelInfo::CLIENTREADY)))
		{
	...
	if(i != m_DynamicModels.InvalidHandle())
	{
		...
		if(!bServerLoaded)
		{
			if(dyn.m_nLoadFlags & CDynamicModelInfo::ALLREADY)
				DynamicModelDebugMsg("dynamic model [%s] loaded on client but not server! is this bad? unknown...",
									 pModel->strName.String());
			dyn.m_nLoadFlags &= ~CDynamicModelInfo::ALLREADY;
...
void Mod_LeafAmbientColorAtPos(Vector *pOut, const Vector &pos, int leafIndex)
{
	...
	{
		// this leaf references another leaf, move there (this leaf is a solid leaf so it borrows samples from a
		// neighbor)
...
CON_COMMAND_F(model_list, "Dump model list to file", FCVAR_CHEAT | FCVAR_DONTRECORD)
{
	...
	if(g_pFileSystem)
	{
		...
		if(fileHandle)
		{
			...
			g_pFileSystem->FPrintf(fileHandle, "name,dataSize,numVerts,nTriCount,dataSizeLod0,numVertsLod0,"
											   "nTriCountLod0,numBones,numParts,numLODs,numMeshes\n");
			...
			for(int i = 0; i < modelloader->GetCount(); i++)
			{
				...
				if(model)
				{
					...
					g_pFileSystem->FPrintf(fileHandle, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", name, dataSize, numVerts,
										   nTriCount, dataSizeLod0, numVertsLod0, nTriCountLod0, numBones, numParts,
										   numLODs, numMeshes);
				}
			}
...
void CModelLoader::DebugPrintDynamicModels()
{
	...
	Msg("\ndynamic models:\n");
	for(UtlHashHandle_t h = m_DynamicModels.FirstHandle(); h != m_DynamicModels.InvalidHandle();
		h = m_DynamicModels.NextHandle(h))
	{
		CDynamicModelInfo &dyn = m_DynamicModels[h];
		int idx = modelinfo->GetModelIndex(m_DynamicModels.Key(h)->strName);
#ifndef SWDS
		if(idx == -1)
			idx = modelinfoclient->GetModelIndex(m_DynamicModels.Key(h)->strName);
#endif
