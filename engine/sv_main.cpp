...
// ConVar sv_hibernate_when_empty( "sv_hibernate_when_empty", "1", 0, "Puts the server into extremely low CPU usage mode
// when no clients connected", OnHibernateWhenEmptyChanged ); ConVar sv_hibernate_ms( "sv_hibernate_ms", "20", 0, "# of
// milliseconds to sleep per frame while hibernating" ); ConVar sv_hibernate_ms_vgui( "sv_hibernate_ms_vgui", "20", 0,
// "# of milliseconds to sleep per frame while hibernating but running the vgui dedicated server frontend" ); static
// ConVar sv_hibernate_postgame_delay( "sv_hibernate_postgame_delay", "5", 0, "# of seconds to wait after final client
// leaves before hibernating.");
ConVar sv_shutdown_timeout_minutes(
	"sv_shutdown_timeout_minutes", "360", FCVAR_REPLICATED,
...
ConVar sv_pure_kick_clients("sv_pure_kick_clients", "1", 0,
							"If set to 1, the server will kick clients with mismatching files. Otherwise, it will "
...
ConVar sv_pure_retiretime("sv_pure_retiretime", "900", 0,
...
static ConVar sv_voiceenable("sv_voiceenable", "1",
...
static ConVar sv_consistency("sv_consistency", "1", FCVAR_REPLICATED,
							 "Legacy variable with no effect!  This was deleted and then added as a temporary kludge "
							 "to prevent players from being banned by servers running old versions of SMAC");
...
ConVar sv_mincmdrate("sv_mincmdrate", "10", FCVAR_REPLICATED,
					 "This sets the minimum value for cl_cmdrate. 0 == unlimited.");
ConVar sv_maxcmdrate("sv_maxcmdrate", "66", FCVAR_REPLICATED,
...
void CGameServer::CreateEngineStringTables(void)
{
	...
	m_pDownloadableFileTable = m_StringTables->CreateStringTableEx(szDownloadableFileTablename, MAX_DOWNLOADABLE_FILES,
																   0, 0, bUseFilenameTables);

	m_pModelPrecacheTable =
		m_StringTables->CreateStringTableEx(szModelPrecacheTablename, MAX_MODELS, sizeof(CPrecacheUserData),
											PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

	m_pGenericPrecacheTable =
		m_StringTables->CreateStringTableEx(szGenericPrecacheTablename, MAX_GENERIC, sizeof(CPrecacheUserData),
											PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

	m_pSoundPrecacheTable =
		m_StringTables->CreateStringTableEx(szSoundPrecacheTablename, MAX_SOUNDS, sizeof(CPrecacheUserData),
											PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

	m_pDecalPrecacheTable =
		m_StringTables->CreateStringTableEx(szDecalPrecacheTablename, MAX_BASE_DECALS, sizeof(CPrecacheUserData),
											PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

	m_pInstanceBaselineTable = m_StringTables->CreateStringTable(INSTANCE_BASELINE_TABLENAME, MAX_DATATABLES);

	m_pLightStyleTable = m_StringTables->CreateStringTable(LIGHT_STYLES_TABLENAME, MAX_LIGHTSTYLES);

	m_pUserInfoTable = m_StringTables->CreateStringTable(USER_INFO_TABLENAME,
	...
	// Send the query info..
	m_pServerStartupTable = m_StringTables->CreateStringTable(SERVER_STARTUP_DATA_TABLENAME, 4);
	SetQueryPortFromSteamServer();
	CopyPureServerWhitelistToStringTable();

	Assert(m_pModelPrecacheTable && m_pGenericPrecacheTable && m_pSoundPrecacheTable && m_pDecalPrecacheTable &&
		   m_pInstanceBaselineTable && m_pLightStyleTable && m_pUserInfoTable && m_pServerStartupTable &&
		   m_pDownloadableFileTable && m_pDynamicModelsTable);
...
void SV_InstallClientStringTableMirrors(void)
{
	...
	for(int i = 0; i < numTables; i++)
	{
		...
		CNetworkStringTable *serverTable = (CNetworkStringTable *)networkStringTableContainerServer->GetTable(i);
...
void SV_InitGameDLL(void)
{
	...
	if(host_state.interval_per_tick < MINIMUM_TICK_INTERVAL || host_state.interval_per_tick > MAXIMUM_TICK_INTERVAL)
	{
		Sys_Error("GetTickInterval returned bogus tick interval (%f)[%f to %f is valid range]",
				  host_state.interval_per_tick, MINIMUM_TICK_INTERVAL, MAXIMUM_TICK_INTERVAL);
	}
...
void SV_StartSound(IRecipientFilter &filter, edict_t *pSoundEmittingEntity, int iChannel, const char *pSample,
				   ...
				   const Vector *pOrigin, float soundtime, int speakerentity, CUtlVector<Vector> *pUtlVecOrigins)
{
...
void CGameServer::RemoveClientFromGame(CBaseClient *client)
{
	...
	if(!pClient->edict || !pClient->IsSpawned() || !IsActive() ||
	   (pClient->GetNetChannel() && pClient->GetNetChannel()->IsLoopback()))
		return;
	...
	switch(nAuthProtocol)
	{
		...
		case PROTOCOL_HASHEDCDKEY:
			...
			if(!Host_IsSinglePlayerGame() ||
			   sv.IsDedicated()) // PROTOCOL_HASHEDCDKEY isn't allowed for multiplayer servers
			{
...
void CGameServer::BroadcastSound(SoundInfo_t &sound, IRecipientFilter &filter)
{
	...
	for(int i = 0; i < num; i++)
	{
		...
		{
			Msg("CGameServer::BroadcastSound:  Recipient Filter for sound (reliable: %s, init: %s) with bogus client "
				"index (%i) in list of %i clients\n",
				filter.IsReliable() ? "yes" : "no", filter.IsInitMessage() ? "yes" : "no", index, num);
		...
		if(m_bHibernating)
		{
			...
			for(int iClient = 0; iClient < m_Clients.Count(); iClient++)
			{
				...
				if(pClient->IsFakeClient() && pClient->IsConnected() && !pClient->IsSplitScreenUser() &&
				   !pClient->IsReplay() && !pClient->IsHLTV())
				{
			...
			else
			{
				if(sv_memlimit.GetInt())
				{
					if(ApproximateProcessMemoryUsage() > 1024 * 1024 * sv_memlimit.GetInt())
					{
						...
						{
							Warning("Server is using %dMB with an sv_memory_limit of %dMB, but will not shutdown "
									"because sv_minuptimelimit is %.3f hr while current uptime is %.3f\n",
									ApproximateProcessMemoryUsage() / (1024 * 1024), sv_memlimit.GetInt(),
									sv_minuptimelimit.GetFloat(),
									(Plat_FloatTime() - s_flPlatFloatTimeBeginUptime) / 3600.0);
						}
						else
						{
				...
				{
					Warning("Server will shutdown because sv_maxuptimelimit is %.3f hr while current uptime is %.3f, "
							"using %dMB with an sv_memory_limit of %dMB\n",
			...
			//#ifdef _LINUX
			//			// if we are a child process running forked, we want to exit now. We want to "really" exit. no
			//destructors, no nothing 			if ( IsChildProcess() )							// are we a subprocess?
			//			{
			//				syscall( SYS_exit_group, 0 );	// we are not going to perform a normal c++ exit. We _dont_ want to
			//run destructors, etc.
			//			}
			//#endif
			if(bExit)
...
void CGameServer::UpdateHibernationState()
{
	...
	bool hibernateFromGCServer = (g_iServerGameDLLVersion < 8) || !serverGameDLL->GetServerGCLobby() ||
								 serverGameDLL->GetServerGCLobby()->ShouldHibernate();

	// If a restart was requested and we're supposed to reboot after XX amount of time, reboot the server.
	if(!bHaveAnyClients && (sv_maxuptimelimit.GetFloat() > 0.0f) && Steam3Server().SteamGameServer() &&
	   Steam3Server().SteamGameServer()->WasRestartRequested())
	{
...
void CGameServer::SendClientMessages(bool bSendSnapshots)
{
	...
	for(int i = 0; i < GetClientCount(); i++)
	{
		...
		else
		{
			...
			{
				NET_OutOfBandPrintf(m_Socket, client->m_NetChannel->GetRemoteAddress(), "%c00000000000000",
									S2C_CONNECTION);
			}
	...
	if(receivingClientCount)
	{
		...
		if(receivingClientCount > 1 && sv_parallel_sendsnapshot.GetBool())
		{
			...
			ParallelProcess("SV_ParallelSendSnapshot", pReceivingClients, receivingClientCount,
...
void SV_CreateBaseline(void)
{
	...
	if(!g_pLocalNetworkBackdoor)
	{
		...
		for(int entnum = 0; entnum < sv.num_edicts; entnum++)
		{
			...
			if(!SendTable_Encode(pSendTable, edict->GetUnknown(), &writeBuf, entnum, NULL, false))
			{
...
static void SV_AllocateEdicts()
{
	...
	sv.edictchangeinfo =
		(IChangeInfoAccessor *)Hunk_AllocName(sv.max_edicts * sizeof(IChangeInfoAccessor), "edictchangeinfo");
}
...
bool CGameServer::SpawnServer(const char *szMapName, const char *szMapFile, const char *startspot)
{
	...
	for(i = 1; i < host_state.worldbrush->numsubmodels; i++)
	{
		...
		PrecacheModel(localmodel, RES_FATALIFMISSING | RES_PRELOAD,
					  modelloader->GetModelForName(localmodel, IModelLoader::FMODELLOADER_SERVER));
	}
