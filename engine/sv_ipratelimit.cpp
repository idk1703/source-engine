...
CIPRateLimit::CIPRateLimit(ConVar *maxSec, ConVar *maxWindow, ConVar *maxSecGlobal)
	: m_IPTree(0, START_TREE_SIZE, LessIP), m_maxSec(maxSec), m_maxWindow(maxWindow), m_maxSecGlobal(maxSecGlobal)
{
	m_iGlobalCount = 0;
	m_lLastTime = -1;
	m_flFlushTime = 0;
...
bool CIPRateLimit::CheckIPInternal(netadr_t adr)
{
	...
	float query_rate =
		static_cast<float>(m_iGlobalCount) / m_maxWindow->GetFloat(); // add one so the bottom is never zero
	...
	if(m_IPTree.Count() > MAX_TREE_SIZE &&
	   curTime > (m_flFlushTime + FLUSH_TIMEOUT / 4)) // if we have stored too many items
	{
		...
		while((m_IPTree.Count() > (2 * MAX_TREE_SIZE) / 3) &&
			  i < m_IPTree.MaxElement()) // trim 1/3 the entries from the tree and only traverse the max nodes
		{
			if(!m_IPTree.IsValidIndex(tmp))
				break;

			if((curTime - m_IPTree[tmp].lastTime) > FLUSH_TIMEOUT && m_IPTree[tmp].ip != clientIP)
			{
				ip_t removeIPT = tmp;
	...
	if(m_IPTree.IsValidIndex(entry))
	{
		...
		{
			float flQueryRate = static_cast<float>(m_IPTree[entry].count) /
								m_maxWindow->GetFloat(); // add one so the bottom is never zero
