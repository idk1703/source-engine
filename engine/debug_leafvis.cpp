...
static ConVar r_drawclipbrushes("r_drawclipbrushes", "0", FCVAR_CHEAT,
...
static void CubeFace(CMeshBuilder &meshBuilder, const Vector org, int v0, int v1, int v2, int v3, float size,
					 const color32 &color)
{
	...
	if(g_LeafVis && r_visambient.GetBool())
	{
		...
		for(int i = 0; i < pAmbient->ambientSampleCount; i++)
		{
			...
			const mleafambientlighting_t &sample =
				host_state.worldbrush->m_pAmbientSamples[pAmbient->firstAmbientSample + i];
