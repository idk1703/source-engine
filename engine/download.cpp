...
public:
	...
	void PersistToCache(
		const RequestContext_t *rc); ///< Writes out a partial download (lost connection, user abort, etc) to cache
...
void DownloadCache::PersistToDisk(const RequestContext_t *rc)
{
	...
	if(rc && rc->data && rc->nBytesTotal)
	{
		...
		if(!g_pFileSystem->FileExists(absPath))
		{
			...
			if(rc->bIsBZ2)
			{
				success = DecompressBZipToDisk(absPath, rc->absLocalPath, reinterpret_cast<char *>(rc->data),
											   rc->nBytesTotal);
			}
			else
			{
...
private:
	void QueueInternal(const char *pBaseURL, const char *pURLPath, const char *pGamePath, bool bAsHttp,
					   bool bCompressed);

protected:
	virtual void UpdateProgressBar();

	virtual RequestContext_t *NewRequestContext(); ///< Call this to allocate a RequestContext_t - calls setup functions
												   ///< for derived classes
	virtual bool ShouldAttemptCompressedFileDownload()
	{
		return true;
	}
	...
	void PruneCompletedRequests(); ///< Check download requests that have been completed to see if their threads have
								   ///< exited
...
//--------------------------------------------------------------------------------------------------------------
void CDownloadManager::QueueInternal(const char *pBaseURL, const char *pURLPath, const char *pGamePath, bool bAsHttp,
									 bool bCompressed)
{
				...
				if(percent != m_lastPercent)
				{
					if(download_debug.GetBool())
					{
						ConDColorMsg(DownloadColor, "Downloading %s%s: %3.3d%% - %d of %d bytes\n",
									 m_activeRequest->baseURL, m_activeRequest->gamePath, percent,
									 m_activeRequest->nBytesCurrent, m_activeRequest->nBytesTotal);
					}
