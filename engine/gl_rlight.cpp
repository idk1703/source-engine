...
// Computes s and t coords of texture at intersection pt
//-----------------------------------------------------------------------------
static void ComputeLightmapCoordsAtIntersection(msurfacelighting_t *pLighting, float ds, float dt, float *lightmapS,
												float *lightmapT)
{
...
//-----------------------------------------------------------------------------
static bool FASTCALL FindIntersectionAtSurface(SurfaceHandle_t surfID, float f, Vector &c, LightVecState_t &state)
{
...
// returns a surfID
static SurfaceHandle_t FindIntersectionSurfaceAtNode(mnode_t *node, float t, Vector &c, LightVecState_t &state)
{
	SurfaceHandle_t surfID = SurfaceHandleFromIndex(node->firstsurface);
...
static SurfaceHandle_t R_LightVecDisplacementChain(LightVecState_t &state, bool bUseLightStyles, Vector &c)
{
	...
	for(int i = 0; i < state.m_LightTestDisps.Count(); i++)
	{
		...
		if(pDispInfo->TestRay(state.m_Ray, 0.0f, state.m_HitFrac, dist, &luv, &tuv))
		{
			...
			{
				ComputeLightmapCoordsAtIntersection(SurfaceLighting(surfID), (int)luv.x, (int)luv.y, state.m_pLightmapS,
													state.m_pLightmapT);
			}
...
// returns surfID
static SurfaceHandle_t FASTCALL FindIntersectionSurfaceAtLeaf(mleaf_t *pLeaf, float start, float end, Vector &c,
...
// returns surfID
SurfaceHandle_t RecursiveLightPoint(mnode_t *node, float start, float end, Vector &c, LightVecState_t &state)
{
	// didn't hit anything
	if(node->contents >= 0)
	{
...
// returns surfID
//-----------------------------------------------------------------------------
SurfaceHandle_t R_LightVec(const Vector &start, const Vector &end, bool bUseLightStyles, Vector &c, float *textureS,
						   float *textureT, float *lightmapS, float *lightmapT)
{
	...
	retSurfID = RecursiveLightPoint(&model->brush.pShared->nodes[model->brush.firstnode], 0.0f, 1.0f, c, state);
