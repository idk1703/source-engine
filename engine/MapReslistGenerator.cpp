...
void MapReslistGenerator_Usage()
{
	...
	Msg("  [ -makereslists <optionalscriptfile> ] -- script file to control more complex makereslists operations "
		"(multiple passes, etc.)\n");
...
bool BuildGeneralMapList(CUtlVector<maplist_map_t> *aMaps, bool bUseMapListFile, const char *pMapFile, char *pSystemMsg,
						 int *iCurrentMap)
{
	if(!bUseMapListFile)
	{
...
void CMapReslistGenerator::BuildEngineLogFromReslist()
{
	...
	if(!g_pFileSystem->ReadFile(CFmtStr("%s\\%s", m_sResListDir.String(), ENGINE_RESLIST_FILE), "DEFAULT_WRITE_PATH",
								buffer))
	{
		// does not exist
		return;
...
void CMapReslistGenerator::LogToEngineReslist(char const *pLine)
{
	...
	FileHandle_t fh =
		g_pFileSystem->Open(CFmtStr("%s\\%s", m_sResListDir.String(), ENGINE_RESLIST_FILE), "at", "DEFAULT_WRITE_PATH");
...
void CMapReslistGenerator::EnableDeletionsTracking()
{
	...
	Msg("Tracking deletions (%u files in deletion list in '%s', %u previous warnings loaded from '%s'\n", deletions,
		DELETIONS_BATCH_FILE, warnings, DELETIONS_WARNINGS_FILE);
}
...
void CMapReslistGenerator::SpewTrackedDeletionsLog()
{
	...
	for(int i = m_DeletionListWarnings.FirstInorder(); i != m_DeletionListWarnings.InvalidIndex();
