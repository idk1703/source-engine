...
void ServerDTI_Flush()
{
	...
	{
		// Write the header.
		g_pFileSystem->FPrintf(fp, "DTName"
		...
		FOR_EACH_LL(g_DTISendTables, j)
		{
			...
			g_pFileSystem->FPrintf(
				...
				pTable->m_nCalcDeltaCalls, pTable->m_nCalcDeltaCycles.GetMillisecondsF(),

				pTable->m_nEncodeCalls, pTable->m_nEncodeCycles.GetMillisecondsF(),

				pTable->m_nShouldTransmitCalls, pTable->m_nShouldTransmitCycles.GetMillisecondsF(),
