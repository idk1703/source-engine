...
void CM_SnapPointToReferenceLeaf_r(CCollisionBSPData *pBSPData, const Vector &p, int num, float tolerance,
								   Vector *pSnapPoint)
{
	float d, snapDist;
	cnode_t *node;
...
static int FASTCALL CM_BrushBoxContents(CCollisionBSPData *pBSPData, const Vector &vMins, const Vector &vMaxs,
										cbrush_t *pBrush)
{
	if(pBrush->IsBox())
	{
...
// slightly different version of the above.  This folds in more of the trace_t output because CM_ComputeTraceEndpts()
// isn't called after this so this routine needs to properly compute start/end points and fractions in all cases
bool IntersectRayWithBox(const Ray_t &ray, const VectorAligned &inInvDelta, const VectorAligned &inBoxMins,
...
void FASTCALL CM_TraceToLeaf(TraceInfo_t *RESTRICT pTraceInfo, int ndxLeaf, float startFrac, float endFrac)
{
	...
	if(pLeaf->dispCount)
	{
		...
		if(IsX360())
		{
			...
			static const fltx4 vecEpsilon = {DISPCOLL_DIST_EPSILON, DISPCOLL_DIST_EPSILON, DISPCOLL_DIST_EPSILON,
			...
			for(int i = 0; i < pLeaf->dispCount; i++)
			{
				...
				if(IS_POINT)
				{
					if(!IsBoxIntersectingRay(LoadAlignedSIMD(pDispBounds->mins.Base()),
											 LoadAlignedSIMD(pDispBounds->maxs.Base()), traceStart, traceDelta,
											 traceInvDelta, vecEpsilon))
						continue;
				}
				else
				{
					...
					if(!IsBoxIntersectingRay(mins, maxs, traceStart, traceDelta, traceInvDelta, vecEpsilon))
						continue;
				}
		...
		else
		{
			...
			for(int i = 0; i < pLeaf->dispCount; i++)
			{
				...
				if(IS_POINT &&
				   !IsBoxIntersectingRay(pDispBounds->mins, pDispBounds->maxs, pTraceInfo->m_start, pTraceInfo->m_delta,
				...
				if(!IS_POINT &&
				   !IsBoxIntersectingRay(pDispBounds->mins - pTraceInfo->m_extents,
										 pDispBounds->maxs + pTraceInfo->m_extents, pTraceInfo->m_start,
										 pTraceInfo->m_delta, pTraceInfo->m_invDelta, DISPCOLL_DIST_EPSILON))
				{
					continue;
				}
...
static void FASTCALL CM_TestInLeaf(TraceInfo_t *pTraceInfo, int ndxLeaf)
{
	...
	if(pLeaf->dispCount)
	{
		...
		CM_TestInDispTree(pTraceInfo, pLeaf, pTraceInfo->m_start, pTraceInfo->m_mins, pTraceInfo->m_maxs,
...
//-----------------------------------------------------------------------------
void CM_RayLeafnums_r(const Ray_t &ray, CCollisionBSPData *pBSPData, int iNode, float p1f, float p2f,
					  const Vector &vecPoint1, const Vector &vecPoint2, int *pLeafList, int nMaxLeafCount,
					  int &nLeafCount)
{
	cnode_t *pNode = NULL;
	...
	else
	{
		while(iNode >= 0)
		{
			...
			else
			{
				flDist1 = DotProduct(pPlane->normal, vecPoint1) - pPlane->dist;
				flDist2 = DotProduct(pPlane->normal, vecPoint2) - pPlane->dist;
				flOffset = fabs(ray.m_Extents[0] * pPlane->normal[0]) + fabs(ray.m_Extents[1] * pPlane->normal[1]) +
	...
	CM_RayLeafnums_r(ray, pBSPData, pNode->children[nSide], p1f, flMid, vecPoint1, vecMid, pLeafList, nMaxLeafCount,
					 nLeafCount);
	...
	CM_RayLeafnums_r(ray, pBSPData, pNode->children[nSide ^ 1], flMid, p2f, vecMid, vecPoint2, pLeafList, nMaxLeafCount,
					 nLeafCount);
}
...
void CM_RayLeafnums(const Ray_t &ray, int *pLeafList, int nMaxLeafCount, int &nLeafCount)
{
	...
	CM_RayLeafnums_r(ray, pBSPData, 0 /*headnode*/, 0.0f, 1.0f, ray.m_Start, vecEnd, pLeafList, nMaxLeafCount,
					 nLeafCount);
}
...
*/
template<bool IS_POINT>
static void FASTCALL CM_RecursiveHullCheckImpl(TraceInfo_t *pTraceInfo, int num, const float p1f, const float p2f,
...
//-----------------------------------------------------------------------------
void CM_BoxTraceAgainstLeafList(const Ray_t &ray, int *pLeafList, int nLeafCount, int nBrushMask, bool bComputeEndpoint,
								trace_t &trace)
{
	...
	if(!ray.m_IsSwept)
	{
		Vector vecBoxMin((ray.m_Start.x - ray.m_Extents.x - 1), (ray.m_Start.y - ray.m_Extents.y - 1),
						 (ray.m_Start.z - ray.m_Extents.z - 1));
		Vector vecBoxMax((ray.m_Start.x + ray.m_Extents.x + 1), (ray.m_Start.y + ray.m_Extents.y + 1),
						 (ray.m_Start.z + ray.m_Extents.z + 1));
...
void CM_TransformedBoxTrace(const Ray_t &ray, int headnode, int brushmask, const Vector &origin, QAngle const &angles,
							trace_t &tr)
{
...
void CM_WorldAlignBounds(ICollideable *pCollideable, Vector *pMins, Vector *pMaxs)
{
	...
	{
		ITransformAABB(pCollideable->CollisionToWorldTransform(), pCollideable->OBBMins(), pCollideable->OBBMaxs(),
					   *pMins, *pMaxs);
...
bool FASTCALL IsBoxIntersectingRayNoLowest(fltx4 boxMin, fltx4 boxMax, const fltx4 &origin, const fltx4 &delta,
...
)
{
	...
	*/
	...
	unsigned int r;
	AssertMsg((XMVectorGreaterOrEqualR(&r, SetWToZeroSIMD(boxMax), SetWToZeroSIMD(boxMin)), XMComparisonAllTrue(r)),
