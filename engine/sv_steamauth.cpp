...
ConVar sv_steamblockingcheck("sv_steamblockingcheck", "0", 0,
							 "Check each new player for Steam blocking compatibility, 1 = message only, 2 >= drop if "
							 "any member of owning clan blocks,"
							 "3 >= drop if any player has blocked, 4 >= drop if player has blocked anyone on server",
							 sv_setsteamblockingcheck_f);
...
void CSteam3Server::Activate(EServerType serverType)
{
	...
	// Figure out the game port. If we're doing a SrcTV relay, then ignore the NS_SERVER port and don't tell Steam that
	...
	// Note that SteamGameServer_InitSafe() calls SteamAPI_SetBreakpadAppID() for you, which is what we don't want if we
	// wish to report crashes under a different AppId. Reset it back to our crashing one now.
	if(sv.IsDedicated())
	{
	...
	{
		m_bWantsPersistentAccountLogon = false;
		MsgAndLog("No account token specified; logging into anonymous game server account.  (Use sv_setsteamaccount to "
				  "login to a persistent account.)\n");
	...
	if(!m_bLogOnResult)
	{
		...
		else
		{
			...
			if(!BLanOnly())
			{
				...
				/*if ( m_bWantsPersistentAccountLogon && ( pLogonFailure->m_eResult == k_EResultInvalidParam ||
				pLogonFailure->m_eResult == k_EResultAccountNotFound ) )
				{
...
void CSteam3Server::OnValidateAuthTicketResponse(ValidateAuthTicketResponse_t *pValidateAuthTicketResponse)
{
	...
	{
		sv.RejectConnection(client->GetNetChannel()->GetRemoteAddress(), client->GetClientChallenge(),
...
void CSteam3Server::OnValidateAuthTicketResponseHelper(CBaseClient *cl, EAuthSessionResponse eAuthSessionResponse)
{
	...
	// If the client is timing out, the Steam failure is probably related (e.g. game crashed). Let's just print that the
	// client timed out.
	...
	switch(eAuthSessionResponse)
	{
		...
		case k_EAuthSessionResponseVACCheckTimedOut:
			cl->Disconnect("An issue with your computer is blocking the VAC system. You cannot play on secure "
...
//-----------------------------------------------------------------------------
bool CSteam3Server::NotifyClientConnect(CBaseClient *client, uint32 unUserID, netadr_t &adr, const void *pvCookie,
										uint32 ucbCookie)
{
	if(!BIsActive())
		return true;
	...
	if(steamID.GetEUniverse() != SteamGameServer()->GetSteamID().GetEUniverse())
	{
		WarningAndLog("Client %d %s connected to universe %d, but game server %s is running in universe %d\n", unUserID,
					  steamID.Render(), steamID.GetEUniverse(), SteamGameServer()->GetSteamID().Render(),
					  SteamGameServer()->GetSteamID().GetEUniverse());
		return false;
	}
	if(!steamID.IsValid() || !steamID.BIndividualAccount())
	{
		WarningAndLog("Client %d connected from %s with invalid Steam ID %s\n", unUserID, adr.ToString(),
					  steamID.Render());
		return false;
	}
...
void CSteam3Server::SendUpdatedServerDetails()
{
	...
	{
		// This is also the case when we're a relay, in which case we never set a game port, so we'll only have a
		// spectator port
	...
	//nMaxClients, nFakeClients ); 	for ( int i = 0 ; i < sv.GetClientCount() ; ++i )
	//	{
	//		IClient	*c = sv.GetClient( i );
	//		Msg("  %d: %s, connected=%d, replay=%d, fake=%d\n", i, c->GetClientName(), c->IsConnected() ? 1 : 0,
	//c->IsReplay() ? 1 : 0, c->IsFakeClient() ? 1 : 0 );
	//	}
}
...
void sv_setsteamaccount_f(const CCommand &args)
{
	...
	{
		Warning("Warning: Game server already logged into steam.  You need to use the sv_setsteamaccount command "
				"earlier.\n");
		return;
	}
...
static ConCommand sv_setsteamaccount(
	"sv_setsteamaccount", sv_setsteamaccount_f,
...
ConVar sv_steamgroup("sv_steamgroup", "", FCVAR_NOTIFY,
					 "The ID of the steam group that this server belongs to. You can find your group's ID on the admin "
					 "profile page in the steam community.",
					 sv_setsteamgroup_f);
