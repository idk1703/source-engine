...
ConVar cl_entityreport_sorted(
	"cl_entityreport_sorted", "0", FCVAR_CHEAT,
...
// Input  : *parent -
//-----------------------------------------------------------------------------
CEntityReportPanel::CEntityReportPanel(vgui::Panel *parent) : CBasePanel(parent, "CEntityReportPanel")
{
...
bool CEntityReportPanel::DrawEntry(int row, int col, int rowheight, int colwidth, int entityIdx)
{
	...
	if(pNet && ((pClientClass = pNet->GetClientClass())) != NULL)
	{
		...
		if(inpvs)
		{
			...
			vgui::surface()->DrawFilledRect(rcleft + wide * fracs[2], rctop + rowheight / 2,
											rcleft + wide * fracs[2] + 1, rcbottom - 1);

			// Draw peak as a vertical red tick
			vgui::surface()->DrawSetColor(192, 0, 0, 255);
			vgui::surface()->DrawFilledRect(rcleft + wide * fracs[1], rctop + 1, rcleft + wide * fracs[1] + 1,
											rctop + rowheight / 2);
		}
