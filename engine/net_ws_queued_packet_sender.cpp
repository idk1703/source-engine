...
ConVar net_queued_packet_thread(
	"net_queued_packet_thread", "1", 0,
...
CQueuedPacketSender::CQueuedPacketSender() : m_QueuedPackets(0, 0, CQueuedPacket::LessFunc)
{
...
void CQueuedPacketSender::QueuePacket(INetChannel *pChan, SOCKET s, const char FAR *buf, int len,
		...
		// OK, now send a packet.
		{
			...
			while(m_QueuedPackets.Count() > 0)
			{
				   ...
				   && pInternetAddr->sin_port != 0)
				{
					...
					NET_SendToImpl(pPacket->m_Socket, pPacket->buf.Base(), pPacket->buf.Count(),
								   (sockaddr *)pPacket->to.Base(), pPacket->to.Count(), -1);
				}
