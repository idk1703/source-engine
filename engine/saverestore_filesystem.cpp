...
#ifdef _X360
ConVar save_in_memory("save_in_memory", "1", 1, "Set to 1 to save to memory instead of disk (Xbox 360)",
					  SaveInMemoryCallback);
#else
ConVar save_in_memory("save_in_memory", "0", 0, "Set to 1 to save to memory instead of disk (Xbox 360)",
					  SaveInMemoryCallback);
#endif // _X360
...
public:
	...
	FSAsyncStatus_t AsyncWrite(const char *pFileName, const void *pSrc, int nSrcBytes, bool bFreeMemory, bool bAppend,
							   FSAsyncControl_t *pControl = NULL);
	FSAsyncStatus_t AsyncFinish(FSAsyncControl_t hControl, bool wait = false);
	FSAsyncStatus_t AsyncAppend(const char *pFileName, const void *pSrc, int nSrcBytes, bool bFreeMemory,
								FSAsyncControl_t *pControl = NULL);
	FSAsyncStatus_t AsyncAppendFile(const char *pDestFileName, const char *pSrcFileName,
									FSAsyncControl_t *pControl = NULL);
...
private:
	...
	SaveFile_t &GetFile(const int idx)
	{
		return m_pSaveDirectory->m_Files[idx];
	}
	SaveFile_t &GetFile(const FileHandle_t hFile)
	{
		return GetFile((uintp)hFile);
	}
...
void CSaveRestoreFileSystem::Compress(SaveFile_t *pFile)
{
	...
	for(int i = 0; i < 10; i++)
	{
		...
		unsigned char *pCompressedBuffer =
			compressor_test.Compress((unsigned char *)pFile->pBuffer->Base(), pFile->nSize, &sz);
	...
	for(int i = 0; i < 10; i++)
	{
		uint32 sz;
		unsigned char *pCompressedBuffer =
			newcompressor_test.Compress((unsigned char *)pFile->pBuffer->Base(), pFile->nSize, &sz);
		delete[] pCompressedBuffer;
	}
	...
	if(1)
	{
		uint32 sz;
		uint32 sz1;
		unsigned char *pNewCompressedBuffer =
			newcompressor_test.Compress((unsigned char *)pFile->pBuffer->Base(), pFile->nSize, &sz);
		unsigned char *pOldCompressedBuffer =
			compressor_test.Compress((unsigned char *)pFile->pBuffer->Base(), pFile->nSize, &sz1);
	...
	unsigned char *pCompressedBuffer =
		compressor.Compress((unsigned char *)pFile->pBuffer->Base(), pFile->nSize, &pFile->nCompressedSize);
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t CSaveRestoreFileSystem::AsyncWrite(const char *pFileName, const void *pSrc, int nSrcBytes,
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t CSaveRestoreFileSystem::AsyncAppend(const char *pFileName, const void *pSrc, int nSrcBytes,
													bool bFreeMemory, FSAsyncControl_t *pControl)
{
	FSAsyncStatus_t retval = FSASYNC_ERR_FAILURE;
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t CSaveRestoreFileSystem::AsyncAppendFile(const char *pDestFileName, const char *pSrcFileName,
...
//-----------------------------------------------------------------------------
// Purpose: Package up all intermediate files to a save game as per usual, but keep them in memory instead of commiting
// them to disk
//-----------------------------------------------------------------------------
void CSaveRestoreFileSystem::DirectorCopyToMemory(const char *pPath, const char *pDestFileName)
{
	...
	// We need to do this because the file goes out to disk as a mixture of an uncompressed header and tags, and
	// compressed intermediate files, so this emulates that in memory
	...
	for(int i = GetDirectory().FirstInorder(); GetDirectory().IsValidIndex(i); i = GetDirectory().NextInorder(i))
	{
		...
		if(Q_stristr(pName, ".hl"))
		{
			...
			if(fileSize)
			{
				...
				// This behaves like AsyncAppendFile (due to 5th param) but gets src file from memory instead of off
				// disk.
...
void CSaveRestoreFileSystem::AuditFiles(void)
{
	...
	while(GetDirectory().IsValidIndex(i))
	{
		...
		Msg("SIM: File: %s [c: %.02f KB / u: %.02f KB]\n", GetString(file.name), (float)file.nCompressedSize / 1024.0f,
			(float)file.nSize / 1024.0f);
	}

	Msg("SIM: ------------------------------------------------------------");
	Msg("SIM: Total files: %d [c: %.02f KB / c: %.02f KB] : Total Size: %.02f KB\n", nTotalFiles,
		(float)nTotalCompressed / 1024.0f, (float)nTotalUncompressed / 1024.0f,
	...
	void DirectoryCopy(const char *pPath, const char *pDestFileName, bool bIsXSave)
	{
		...
		for(int i = 0; i < list.Count(); i++)
		{
			...
			if(fileSize)
			{
				...
				g_pFileSystem->AsyncAppend(pDestFileName, memcpy(new char[MAX_PATH], list[i].szFileName, MAX_PATH),
										   MAX_PATH, true); // Filename can only be as long as a map name + extension
				g_pFileSystem->AsyncAppend(pDestFileName, new int(fileSize), sizeof(int), true);
