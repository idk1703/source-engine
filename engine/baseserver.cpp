...
static ConVar sv_max_queries_sec("sv_max_queries_sec", "3.0", 0,
								 "Maximum queries per second to respond to from a single IP address.");
static ConVar sv_max_queries_window("sv_max_queries_window", "30", 0,
									"Window over which to average queries per second averages.");
static ConVar sv_max_queries_sec_global("sv_max_queries_sec_global", "3000", 0,
										"Maximum queries per second to respond to from anywhere.");

static ConVar sv_max_connects_sec("sv_max_connects_sec", "2.0", 0,
								  "Maximum connections per second to respond to from a single IP address.");
static ConVar sv_max_connects_window("sv_max_connects_window", "4", 0,
...
static ConVar sv_max_connects_sec_global("sv_max_connects_sec_global", "0", 0,
...
static ConVar sv_instancebaselines("sv_instancebaselines", "1", FCVAR_DEVELOPMENTONLY,
...
static ConVar sv_password("sv_password", "", FCVAR_NOTIFY | FCVAR_PROTECTED | FCVAR_DONTRECORD,
						  "Server password for entry into multiplayer games");
ConVar sv_tags("sv_tags", "", FCVAR_NOTIFY,
			   "Server tags. Used to provide extra information to clients when they're browsing for servers. Separate "
			   "tags with a comma.",
			   SvTagsChangeCallback);
ConVar sv_visiblemaxplayers("sv_visiblemaxplayers", "-1", 0,
...
ConVar sv_allow_wait_command("sv_allow_wait_command", "1", FCVAR_REPLICATED,
							 "Allow or disallow the wait command on clients connected to this server.");
ConVar sv_allow_color_correction("sv_allow_color_correction", "1", FCVAR_REPLICATED,
...
bool CBaseServer::CheckIPConnectionReuse(netadr_t &adr)
{
	...
	for(int slot = 0; slot < m_Clients.Count(); slot++)
	{
		...
		if(client->IsConnected() && !client->IsActive() && !client->IsFakeClient() &&
									...
									const char *name, const char *password, const char *hashedCDkey, int cdKeyLen)
{
	...
	if(!CheckChallengeType(client, nNextUserID, adr, authProtocol, hashedCDkey, cdKeyLen,
						   clientChallenge)) // we use the client pointer to track steam requests
	{
		return NULL;
	}
	...
	switch(c)
	{
		...
		case C2S_CONNECT:
		{
			...
			if(authProtocol == PROTOCOL_STEAM)
			{
				...
				ConnectClient(packet->from, protocol, challengeNr, clientChallenge, authProtocol, name, password, cdkey,
		...
		default:
		{
			...
			if(Steam3Server().SteamGameServer() && Steam3Server().IsMasterServerUpdaterSharingGameSocket())
			{
				Steam3Server().SteamGameServer()->HandleIncomingPacket(
					packet->message.GetBasePointer(), packet->message.TotalBytesAvailable(),
					packet->from.GetIPHostByteOrder(), packet->from.GetPort());
...
const char *CBaseServer::CompressPackedEntity(ServerClass *pServerClass, const char *data, int &bits)
{
	...
	SendTable_WriteAllDeltaProps(pServerClass->m_pTable, pBaselineData, nBaselineBits, data, bits, -1, &writeBuf);
...
const char *CBaseServer::UncompressPackedEntity(PackedEntity *pPackedEntity, int &bits)
{
	...
	RecvTable_MergeDeltas(pPackedEntity->m_pClientClass->m_pRecvTable, &oldBuf, &newBuf, &outBuf);
...
================
*/
bool CBaseServer::CheckChallengeType(CBaseClient *client, int nNewUserID, netadr_t &adr, int nAuthProtocol,
									 const char *pchLogonCookie, int cbCookie, int clientChallenge)
{
	if(AllowDebugDedicatedServerOutsideSteam())
		return true;
	...
	{
		if(!Steam3Server().NotifyLocalClientConnect(
			   client)) // the userID isn't alloc'd yet so we need to fill it in manually
		{
...
bool CBaseServer::GetClassBaseline(ServerClass *pClass, void const **pData, int *pDatalen)
{
	if(sv_instancebaselines.GetInt())
	{
		...
		*pData = GetInstanceBaselineTable()->GetStringUserData(pClass->m_InstanceBaselineIndex, pDatalen);
...
void CBaseServer::RunFrame(void)
{
	...
	if(m_flLastRandomNumberGenerationTime < 0 ||
	   (m_flLastRandomNumberGenerationTime + CHALLENGE_NONCE_LIFETIME) < g_ServerGlobalVariables.realtime)
	{
		m_LastRandomNonce = m_CurrentRandomNonce;

		// RandomInt maps a uniform distribution on the interval [0,INT_MAX], so make two calls to get the random
		// number. RandomInt will always return the minimum value if the difference in min and max is greater than or
		// equal to INT_MAX.
...
CBaseClient *CBaseServer::CreateFakeClient(const char *name)
{
	...
	{
		netadr_t adrNull(
			0, 0); // 0.0.0.0:0 signifies a bot. It'll plumb all the way down to winsock calls but it won't make them.
...
void CBaseServer::BroadcastMessage(INetMessage &msg, IRecipientFilter &filter)
{
	...
	else
	{
		...
		for(int i = 0; i < num; i++)
		{
			...
			{
				Msg("SV_BroadcastMessage:  Recipient Filter for message type %i (reliable: %s, init: %s) with bogus "
					"client index (%i) in list of %i clients\n",
					msg.GetType(), filter.IsReliable() ? "yes" : "no", filter.IsInitMessage() ? "yes" : "no", index,
					num);
...
void CBaseServer::WriteTempEntities(CBaseClient *client, CFrameSnapshot *pCurrentSnapshot,
									CFrameSnapshot *pLastSnapshot, bf_write &buf, int ev_max)
{
	...
	// Build list of events sorted by send table classID (makes the delta work better in cases with a lot of the same
	// message type )
	...
	for(int i = sorted.FirstInorder(); i != sorted.InvalidIndex(); i = sorted.NextInorder(i))
	{
		...
		if(pLastEvent && pLastEvent->classID == event->classID)
		{
			...
			SendTable_WriteAllDeltaProps(event->pSendTable, pLastEvent->pData, pLastEvent->bits, event->pData,
										 event->bits, -1, &buffer);
