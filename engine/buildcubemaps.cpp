...
//-----------------------------------------------------------------------------
static void TakeCubemapSnapshot(const Vector &origin, const char *pFileNameBase, int screenBufSize, int tgaSize,
								bool bPFM)
{
	if(IsX360())
		return;
	...
	static CubeMapFaceIndex_t face_idx[6] = {CUBEMAP_FACE_RIGHT, CUBEMAP_FACE_LEFT, CUBEMAP_FACE_BACK,
	...
	if(bPFM)
	{
		...
		for(int side = 0; side < 6; side++)
		{
			...
			else
			{
				...
				videomode->TakeSnapshotTGARect(name, 0, 0, screenBufSize, screenBufSize, tgaSize, tgaSize, bPFM,
											   face_idx[side]);
			}
		}
		if(g_pMaterialSystemHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER)
...
//-----------------------------------------------------------------------------
static void BuildSingleCubemap(const char *pVTFName, const Vector &vecOrigin, int nSize, bool bHDR,
							   const char *pGameDir, IVTex *ivt)
{
	if(IsX360())
		return;
...
static bool LoadSrcVTFFiles(IVTFTexture *pSrcVTFTextures[6], const char *pSkyboxBaseName)
{
	...
	for(i = 0; i < 6; i++)
	{
		// !!! FIXME: This needs to open the vmt (or some other method) to find the correct LDR or HDR set of skybox
		// textures! Look in vbsp\cubemap.cpp!
		...
		{
			Warning(
				"*** Error: Skybox vtf files for %s weren't compiled with the same size texture and/or same flags!\n",
				pSkyboxBaseName);
			return false;
		}
	}
...
void Cubemap_CreateDefaultCubemap(const char *pMapName, IBSPPack *iBSPPack)
{
	...
	pDstCubemap->Init(DEFAULT_CUBEMAP_SIZE, DEFAULT_CUBEMAP_SIZE, 1, pSrcVTFTextures[0]->Format(),
					  pSrcVTFTextures[0]->Flags() | TEXTUREFLAGS_ENVMAP, pSrcVTFTextures[0]->FrameCount());
		...
		for(int iFace = 0; iFace < 6; ++iFace)
		{
			...
			for(int iMip = 0; iMip < pDstCubemap->MipCount(); ++iMip)
			{
				...
				if((pSrcVTFTextures[iFace]->Width() == 4) &&
				   (pSrcVTFTextures[iFace]->Height() == 4)) // If texture is 4x4 square
				{
				...
				else if(pSrcVTFTextures[iFace]->Width() == pSrcVTFTextures[iFace]->Height()) // If texture is square
				{
					...
					{
						Warning(
							"%s - ERROR! Cannot copy square face for default cubemap! iSrcMipSize(%d) != iSize(%d)\n",
					...
					else
					{
						...
						pSrcVTFTextures[iFace]->Height() * 2) // If texture is rectangle 2x wide
				{
					...
					else
					{
						...
						else
						{
							...
							{
								memcpy(pDstBits + iSize / 2 + i * nSrcRowSize, pSrcBits + iSrcMipSize - nSrcRowSize,
									   nSrcRowSize);
							}
						}
					}
				...
				else
				{
					...
					Warning("%s - Couldn't create default cubemap because texture res is %dx%d\n", pSkyboxBaseName,
							pSrcVTFTextures[iFace]->Width(), pSrcVTFTextures[iFace]->Height());
					memset(pDstBits, 0, iSize);
					return;
				}
			}
	...
	bool bHasAlpha = ((flagUnion & (TEXTUREFLAGS_ONEBITALPHA | TEXTUREFLAGS_EIGHTBITALPHA)) != 0);
...
// Added these to seperate from R_BuildCubemapSamples to a) clean it up abit and b) fix the issue where if it fails,
// mouse is disabled.
...
void R_BuildCubemapSamples(int numIterations)
{
	...
	for(bounce = 0; bounce < numIterations; bounce++)
	{
		...
		for(i = 0; i < pWorldModel->brush.pShared->m_nCubemapSamples; i++)
		{
			...
			{
				Warning("Cube map buffer size %d x %d is bigger than screen!\nRun at a higher resolution! or reduce "
						"your cubemap resolution (needs 4X)\n",
		...
		for(i = 0; i < pWorldModel->brush.pShared->m_nCubemapSamples; i++)
		{
			...
			Q_snprintf(pVTFName, sizeof(pVTFName), "%s/c%d_%d_%d", pMaterialSrcDir, (int)pCubemapSample->origin[0],
					   (int)pCubemapSample->origin[1], (int)pCubemapSample->origin[2]);
