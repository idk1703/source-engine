...
static ConVar sv_logsdir("sv_logsdir", "logs", FCVAR_ARCHIVE,
...
static ConVar sv_logbans(
	...
	"Log server bans in the server logs."); // should sv_banid() calls be logged in the server logs?
static ConVar sv_logsecret(
	"sv_logsecret", "0", 0,
	"If set then include this secret when doing UDP logging (will use 0x53 as packet type, not usual 0x52)");

static ConVar sv_logfilename_format("sv_logfilename_format", "", FCVAR_ARCHIVE,
									"Log filename format. See strftime for formatting codes.");
static ConVar sv_logfilecompress("sv_logfilecompress", "0", FCVAR_ARCHIVE,
...
void CLog::Print(const char *text)
{
	...
	V_sprintf_safe(string, "L %02i/%02i/%04i - %02i:%02i:%02i: %s", today.tm_mon + 1, today.tm_mday,
				   1900 + today.tm_year, today.tm_hour, today.tm_min, today.tm_sec, text);
	...
	if(m_LogAddresses.Count() > 0)
	{
		...
		for(int i = 0; i < m_LogAddresses.Count(); i++)
		{
			if(sv_logsecret.GetInt() != 0)
				NET_OutOfBandPrintf(NS_SERVER, m_LogAddresses.Element(i), "%c%s%s", S2A_LOGSTRING2,
									sv_logsecret.GetString(), string);
			else
	...
	else if(Q_strcmp(name, "server_addban") == 0)
	{
		if(sv_logbans.GetInt() > 0)
		{
			...
			else
			{
				...
				if(userid > 0)
				{
					Printf("Banid: \"%s<%i><%s><>\" %s \"%s\" by \"%s\"\n", pszName, userid, pszNetworkid, pszResult,
						   pszDuration, pszCmdGiver);
				}
				else
				{
					Printf("Banid: \"<><%s><>\" %s \"%s\" by \"%s\"\n", pszNetworkid, pszResult, pszDuration,
						   pszCmdGiver);
				}
			}
		}
