...
void SendProxy_DTTestServerSubString(const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut,
...
BEGIN_SEND_TABLE_NOBASE(CTestStruct, DT_TestStruct)
	SendPropInt(SENDINFO_NOCHECK(a)), SendPropInt(SENDINFO_NOCHECK(b)), SendPropFloat(SENDINFO_NOCHECK(f))
END_SEND_TABLE()
...
BEGIN_SEND_TABLE_NOBASE(DTTestServerSub, DT_DTTestSub)
	...
	SendPropArray(SendPropFloat(SENDINFO_NOCHECK(m_FloatArray[0]), 0, SPROP_NOSCALE), m_FloatArray),
		...
		SendPropUtlVector(SENDINFO_UTLVECTOR(m_UtlVectorChar), MAX_CHARARRAY_ELEMENTS,
...
void SendProxy_DTTestServerFloat(const SendProp *pProp, void *pStruct, void *pData, DVariant *pOut, int iElement,
								 int objectID)
{
	SendProxy_FloatToFloat(pProp, pStruct, pData, pOut, iElement, objectID);
}

void SendProxy_DTTestServerVector(const SendProp *pProp, void *pStruct, void *pData, DVariant *pOut, int iElement,
								  int objectID)
{
	SendProxy_VectorToVector(pProp, pStruct, pData, pOut, iElement, objectID);
}

void SendProxy_DTTestServerString(const SendProp *pProp, void *pStruct, void *pData, DVariant *pOut, int iElement,
								  int objectID)
{
	SendProxy_StringToString(pProp, pStruct, pData, pOut, iElement, objectID);
}

void SendProxy_DTTestServerInt(const SendProp *pProp, void *pStruct, void *pData, DVariant *pOut, int iElement,
							   int objectID)
{
...
void *SendProxy_DTTestServerSub(const SendProp *pProp, const void *pStruct, const void *pData,
...
BEGIN_SEND_TABLE_NOBASE(DTTestServer, DT_DTTest)
	SendPropVariableLengthArray(ArrayLengthSendProxy_VLALength, SendPropInt(SENDINFO_NOCHECK(m_VLA[0])), m_VLA),
...
#if defined(SUPPORT_ARRAYS_OF_DATATABLES)
		SendPropArray(SendPropDataTable(SENDINFO_DT(m_SubArray[0]), &REFERENCE_SEND_TABLE(DT_DTTestSub),
										SendProxy_DTTestServerSub),
					  m_SubArray),
#endif
...
BEGIN_RECV_TABLE_NOBASE(DTTestClientSub, DT_DTTestSub)
	...
	RecvPropArray(RecvPropFloat(RECVINFO(m_FloatArray[0])), m_FloatArray),
		...
		RecvPropUtlVector(RECVINFO_UTLVECTOR(m_UtlVectorChar), MAX_CHARARRAY_ELEMENTS,
						  RecvPropInt(NULL, 0, sizeof(char))),

		RecvPropArray(RecvPropString(RECVINFO(m_Strings[0]), 0, RecvProxy_DTTestClientSubString), m_Strings),
END_RECV_TABLE()
...
BEGIN_RECV_TABLE_NOBASE(DTTestSub2, DT_DTTestSub2)
	RecvPropDataTable(RECVINFO_DT(m_Sub), 0, &REFERENCE_RECV_TABLE(DT_DTTestSub2Sub)), RecvPropInt(RECVINFO(m_Int)),
END_RECV_TABLE()
...
#if defined(SUPPORT_ARRAYS_OF_DATATABLES)
	RecvPropArray(RecvPropDataTable(RECVINFO_DT(m_SubArray[0]), 0, &REFERENCE_RECV_TABLE(DT_DTTestSub),
		...
		RecvPropVector(RECVINFO(m_Vector), 0), RecvPropString(RECVINFO_STRING(m_String), 0),
		...
		RecvPropVariableLengthArray(RecvProxyArrayLength_VLA, RecvPropInt(RECVINFO(m_VLA[0])), m_VLA)
END_RECV_TABLE()
...
{
	return pClient->m_Vector.x == pServer->m_Vector.x && pClient->m_Vector.y == pServer->m_Vector.y &&
...
bool CompareIntArray(DTTestClient *pClient, DTTestServer *pServer)
{
	...
	int leastElements = (sizeof(pClient->m_IntArray) < sizeof(pServer->m_IntArray))
...
bool CompareFloatArray(DTTestClient *pClient, DTTestServer *pServer)
{
	...
	/*
		int leastElements = (sizeof(pClient->m_Sub.m_FloatArray) < sizeof(pServer->m_Sub.m_FloatArray)) ?
	   (sizeof(pClient->m_Sub.m_FloatArray)/sizeof(pClient->m_Sub.m_FloatArray[0])) :
	   (sizeof(pServer->m_Sub.m_FloatArray)/sizeof(pServer->m_Sub.m_FloatArray[0])); for(int i=0; i < leastElements;
	   i++)
		{
...
bool CompareSubArray(DTTestClient *pClient, DTTestServer *pServer)
{
	...
	for(int i = 0; i < 2; i++)
	{
		for(int z = 0; z < sizeof(pServer->m_SubArray[0].m_FloatArray) / sizeof(pServer->m_SubArray[0].m_FloatArray[0]);
			z++)
		{
		...
		for(int iString = 0;
			iString < sizeof(pServer->m_SubArray[0].m_Strings) / sizeof(pServer->m_SubArray[0].m_Strings[0]); iString++)
		{
			for(z = 0; z < sizeof(pServer->m_SubArray[0].m_Strings[0]) / sizeof(pServer->m_SubArray[0].m_Strings[0][0]);
				z++)
			{
...
void RandomlyChangeSubArray(DTTestServer *pServer)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		{
			int stringLen =
				sizeof(pServer->m_SubArray[0].m_Strings[0]) / sizeof(pServer->m_SubArray[0].m_Strings[0][0]);
...
VarTestInfo g_VarTestInfos[] = {{CompareVLA, RandomlyChangeVLA},
...
int g_GuardOffsets[] = {
	offsetof(DTTestClient, m_Guard1), offsetof(DTTestClient, m_Guard2), offsetof(DTTestClient, m_Guard3),
	offsetof(DTTestClient, m_Guard4), offsetof(DTTestClient, m_Guard5), offsetof(DTTestClient, m_Guard6),
	offsetof(DTTestClient, m_Guard7), offsetof(DTTestClient, m_Guard8), offsetof(DTTestClient, m_Guard9)};
...
for(int iIteration = 0; iIteration < nIterations; iIteration++)
{
	...
	else
	{
		...
		bf_read fullEncodedRead("RunDataTableTest->fullEncodedRead", fullEncoded, sizeof(fullEncoded),
								bfFullEncoded.GetNumBitsWritten());
		bf_read prevEncodedRead("RunDataTableTest->prevEncodedRead", prevEncoded, sizeof(prevEncoded));

		int nDeltaProps = SendTable_CalcDelta(pSendTable, prevEncoded, sizeof(prevEncoded) * 8, fullEncoded,
											  bfFullEncoded.GetNumBitsWritten(), deltaProps, ARRAYSIZE(deltaProps), -1);

		Assert(nDeltaProps != -1); // BAD: buffer overflow

		// Reencode with just the delta. This is what is actually sent to the client.
		SendTable_WritePropList(pSendTable, fullEncoded, bfFullEncoded.GetNumBitsWritten(), &bfDeltaEncoded, -1111,
								deltaProps, nDeltaProps);
	}
