...
ConVar debugrayenable("debugrayenable", "0", NULL,
					  "Use this to enable ray testing.  To reset: bind \"F1\" \"clearalloverlays; debugrayreset  0; "
					  "host_framerate 66.66666667\"");
ConVar debugrayreset("debugrayreset", "0");
ConVar debugraylimit("debugraylimit", "500", NULL,
...
public:
	...
	virtual void HandleEntityToCollideable(IHandleEntity * pHandleEntity, ICollideable * *ppCollide,
	...
	// retrieve brush planes and contents, returns true if data is being returned in the output pointers, false if the
	// brush doesn't exist
...
private:
	...
	bool ClipRayToVPhysics(const Ray_t &ray, unsigned int fMask, ICollideable *pCollideable, studiohdr_t *pStudioHdr,
						   trace_t *pTrace);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineTraceServer, IEngineTrace, INTERFACEVERSION_ENGINETRACE_SERVER,
								  s_EngineTraceServer);

#ifndef SWDS
static CEngineTraceClient s_EngineTraceClient;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineTraceClient, IEngineTrace, INTERFACEVERSION_ENGINETRACE_CLIENT,
								  s_EngineTraceClient);
#endif
...
public:
	...
	static inline bool TestEntity(CEngineTrace *pEngineTrace, ICollideable *pCollide, const Vector &vPos,
								  int *pContents, ICollideable **pWorldCollideable)
	{
		...
		if(pModel && pModel->type == mod_brush)
		{
			...
			int contents = CM_TransformedPointContents(vPos, nHeadNode, pCollide->GetCollisionOrigin(),
...
int CEngineTrace::GetPointContents(const Vector &vecAbsPosition, IHandleEntity **ppEntity)
{
	...
	if(nContents != CONTENTS_SOLID)
	{
		...
		SpatialPartition()->EnumerateElementsAtPoint(SpatialPartitionMask(), vecAbsPosition, false, &contentsEnum);
...
static void FASTCALL GetBrushesInAABB_ParseLeaf(const Vector *pExtents, CCollisionBSPData *pBSPData, cleaf_t *pLeaf,
		...
		else
		{
			...
			for(j = 0; j != pBrush->numsides; ++j)
			{
				...
				if((pExtents[pPlane->signbits].Dot(pPlane->normal) - pPlane->dist) > 0.0f)
					break; // the bounding box extent that was most likely to be encapsulated by the plane is outside
						   // the halfspace, brush not in bbox
			}
...
void CEngineTrace::GetBrushesInAABB(const Vector &vMins, const Vector &vMaxs, CUtlVector<int> *pOutput,
									int iContentsMask)
{
	if(pOutput == NULL)
		return;
	...
	for(int i = 0; i != iNumLeafs; ++i)
		GetBrushesInAABB_ParseLeaf(ptBBoxExtents, pBSPData, &pBSPData->map_leafs[pLeafList[i]], pOutput, iContentsMask,
								   counters.Base());
}
...
CPhysCollide *CEngineTrace::GetCollidableFromDisplacementsInAABB(const Vector &vMins, const Vector &vMaxs)
{
	...
	// For each leaf in which the box lies, Get all displacements in that leaf and use their triangles to create the
	// mesh
	for(int i = 0; i < iLeafCount; ++i)
	{
		...
		for(int k = 0; k < curLeaf.dispCount; k++)
		{
			...
			if(iTriCount > 65535)
			{
				AssertMsg(0, "Displacement surfaces have too many triangles to duplicate in "
							 "GetCollidableFromDisplacementsInBox.");
				EndTrace(pTraceInfo);
				return NULL;
			}
			...
			for(int j = 0; j < meshTriList.indexCount; j += 3)
			{
				...
				// We don't need exact clipping to the box... Include any triangle that has at least one vert on the
				// inside
				if(IsPointInBox(v0, vMins, vMaxs) || IsPointInBox(v1, vMins, vMaxs) || IsPointInBox(v2, vMins, vMaxs))
				{
					// This is for collision only, so we don't need to worry about blending-- Use the first surface
					// prop.
...
bool CEngineTrace::GetBrushInfo(int iBrush, CUtlVector<Vector4D> *pPlanesOut, int *pContentsOut)
{
	...
	if(pPlanesOut)
	{
		...
		else
		{
			...
			// Note:  Don't do this in the [] since the final one on the last brushside will be past the end of the
			// array end by one index
...
// Perform vphysics trace
//-----------------------------------------------------------------------------
bool CEngineTrace::ClipRayToVPhysics(const Ray_t &ray, unsigned int fMask, ICollideable *pEntity,
									 studiohdr_t *pStudioHdr, trace_t *pTrace)
{
	...
	if(pStudioHdr)
	{
		...
		{
			physcollision->TraceBox(ray, fMask, &studioConvex,
	...
	else
	{
		...
		if(!ray.m_IsRay || pModel->type != mod_brush)
		{
			...
			if(pCollide && pCollide->solidCount)
			{
				...
				physcollision->TraceBox(ray, fMask, pConvexInfo,
										pCollide->solids[0], // UNDONE: Support other solid indices?!?!?!? (forced zero)
										pEntity->GetCollisionOrigin(), pEntity->GetCollisionAngles(), pTrace);
				bTraced = true;
			}
...
bool CEngineTrace::ClipRayToBSP(const Ray_t &ray, unsigned int fMask, ICollideable *pEntity, trace_t *pTrace)
{
	...
	CM_TransformedBoxTrace(ray, nHeadNode, fMask, pEntity->GetCollisionOrigin(), pEntity->GetCollisionAngles(),
						   *pTrace);
	return true;
}
...
{
	extern bool IntersectRayWithBox(const Ray_t &ray, const VectorAligned &inInvDelta, const VectorAligned &inBoxMins,
									const VectorAligned &inBoxMaxs, trace_t *RESTRICT pTrace);
...
bool CEngineTrace::ClipRayToOBB(const Ray_t &ray, unsigned int fMask, ICollideable *pEntity, trace_t *pTrace)
{
	...
	IntersectRayWithOBB(ray, pEntity->GetCollisionOrigin(), pEntity->GetCollisionAngles(), pEntity->OBBMins(),
...
//-----------------------------------------------------------------------------
void CEngineTraceServer::HandleEntityToCollideable(IHandleEntity *pHandleEntity, ICollideable **ppCollide,
...
#ifndef SWDS
void CEngineTraceClient::HandleEntityToCollideable(IHandleEntity *pHandleEntity, ICollideable **ppCollide,
												   const char **ppDebugName)
{
	*ppCollide = StaticPropMgr()->GetStaticProp(pHandleEntity);
	if(*ppCollide)
...
//-----------------------------------------------------------------------------
void CEngineTrace::SetupLeafAndEntityListBox(const Vector &vecBoxMin, const Vector &vecBoxMax,
											 CTraceListData &traceData)
{
	...
	traceData.m_nLeafCount =
		CM_BoxLeafnums(vecBoxMin, vecBoxMax, traceData.m_aLeafList.Base(), traceData.LeafCountMax(), &iTopNode);
...
//-----------------------------------------------------------------------------
void CEngineTrace::TraceRayAgainstLeafAndEntityList(const Ray_t &ray, CTraceListData &traceData, unsigned int fMask,
													ITraceFilter *pTraceFilter, trace_t *pTrace)
{
...
#endif
	{
		...
		for(int i = 0; i < s_BenchmarkRays.Count(); i++)
		{
			...
			if(0)
			{
				...
				enumerator.Reset();
				SpatialPartition()->EnumerateElementsAlongRay(PARTITION_ENGINE_SOLID_EDICTS, entityRay, false,
															  &enumerator);
				trace_t tr;
				ICollideable *pCollideable;
		...
		Msg("RAY TEST: %d hits, %d misses, %.2fms   (%d rays, %d sweeps) (%d ray/prop, %d box/prop)\n", hit, miss, ms,
			point, swept, rayVsProp, boxVsProp);
	}
#if VPROF_LEVEL > 0
	g_VProfCurrentProfile.MarkFrame();
...
void CEngineTrace::EnumerateEntities(const Vector &vecAbsMins, const Vector &vecAbsMaxs, IEntityEnumerator *pEnumerator)
{
	...
	// basically becomes obsolete. The spatial partition can be queried directly.
	CEnumerationFilter enumerator(this, pEnumerator);
	SpatialPartition()->EnumerateElementsInBox(SpatialPartitionMask(), vecAbsMins, vecAbsMaxs, false, &enumerator);
}
...
CON_COMMAND(test_sweepaabb, "method to test out sweeps")
{
	...
	g_pEngineTraceServer->EnumerateEntities(MainViewOrigin() - Vector(200, 200, 200),
	...
	QAngle test(0, 45, 0);
#ifndef SWDS
	CDebugOverlay::AddBoxOverlay(pCollide->GetCollisionOrigin(), pCollide->OBBMins(), pCollide->OBBMaxs(),
...
#ifndef SWDS
	CDebugOverlay::AddSweptBoxOverlay(tr.startpos, tr.endpos, Vector(-10, -20, -10), Vector(30, 30, 20), vec3_angle, r,
									  g, b, a, 5.0);
#endif
}
