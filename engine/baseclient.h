...
{
public:
	CNetworkStatTrace() : m_nMinWarningBytes(0), m_nStartBit(0), m_nCurBit(0), m_StartSendTime(0.0) {}
...
public:
	...
	IServer *GetServer()
	{
		return (IServer *)m_Server;
	};
	const char *GetUserSetting(const char *cvar) const;
	const char *GetNetworkIDString() const;
	uint GetFriendsID() const
	{
		return m_nFriendsID;
	}
	const char *GetFriendsName() const
	{
		return m_FriendsName;
	}
	void UpdateName(const char *pszDefault);
	int GetClientChallenge() const
	{
		return m_clientChallenge;
	}

	void SetReportThisFakeClient(bool bReport)
	{
		m_bReportFakeClient = bReport;
	}
	bool ShouldReportThisFakeClient(void) const
	{
		return m_bReportFakeClient;
	}
	...
	virtual bool IsSplitScreenUser(void) const
	{
		return false;
	} // !KLUDGE! We don't have splitscreen support, but this makes merges easier
...
public:
	...
	void SetPlayerNameLocked(bool bValue)
	{
		m_bPlayerNameLocked = bValue;
	}
	bool IsPlayerNameLocked(void)
	{
		return m_bPlayerNameLocked;
	}
