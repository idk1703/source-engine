...
public:
	...
	virtual void Mix8Mono(channel_t *pChannel, char *pData, int outputOffset, int inputOffset, fixedint rateScaleFix,
						  int outCount, int timecompress, bool forward = true);
	virtual void Mix8Stereo(channel_t *pChannel, char *pData, int outputOffset, int inputOffset, fixedint rateScaleFix,
							int outCount, int timecompress, bool forward = true);
	virtual void Mix16Mono(channel_t *pChannel, short *pData, int outputOffset, int inputOffset, fixedint rateScaleFix,
						   int outCount, int timecompress, bool forward = true);
	virtual void Mix16Stereo(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
							 fixedint rateScaleFix, int outCount, int timecompress, bool forward = true);
...
void CAudioDeviceWave::Mix8Mono(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
								fixedint rateScaleFix, int outCount, int timecompress, bool forward)
{
	int sampleIndex = 0;
	...
	if(time > m_mixTime)
	{
		...
		while(tempCount > 0)
		{
			...
			for(int i = 0; i < MAX_CHANNELS; i++)
			{
				...
				{
					if(!pSource->MixDataToDevice(this, pSource->GetChannel(), currentsample, sampleCount,
				...
				else
				{
					...
					{
						pSource->IncrementSamples(pSource->GetChannel(), currentsample, sampleCount, DeviceSampleRate(),
												  forward);
					}
				}
			}
