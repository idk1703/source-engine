...
public:
	...
	virtual bool ConvertImageFormat(unsigned char *src, enum ImageFormat srcImageFormat, unsigned char *dst,
	...
	virtual void DrawClearBufferQuad(unsigned char r, unsigned char g, unsigned char b, unsigned char a,
									 bool bClearColor, bool bClearAlpha, bool bClearDepth) OVERRIDE
	{
	}

#if defined(_X360)
	virtual void ReadBackBuffer(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *pData, ImageFormat dstFormat,
								int nDstStride)
	{
	}
#endif
	...
	virtual bool OnSetFlexMesh(IMesh *pStaticMesh, IMesh *pMesh, int nVertexOffsetInBytes)
	{
		return false;
	}
	virtual bool OnSetColorMesh(IMesh *pStaticMesh, IMesh *pMesh, int nVertexOffsetInBytes)
	{
		return false;
	}
	...
	virtual bool OnFlushBufferedPrimitives()
	{
		return false;
	}
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderUtilTemp, IShaderUtil, SHADER_UTIL_INTERFACE_VERSION, g_pTemp)
...
private:
	...
	// Creates shaders
	void CreateShaders(const char *pVShader, int nVBufLen, const char *pGShader, int nGBufLen, const char *pPShader,
					   int nPBufLen);
...
bool CShaderAPITestApp::CreateAppWindow(const char *pTitle, bool bWindowed, int w, int h)
{
	...
	// Create the window
	m_HWnd = CreateWindow(wc.lpszClassName, pTitle, style, 0, 0, windowRect.right - windowRect.left,
						  windowRect.bottom - windowRect.top, NULL, NULL, (HINSTANCE)GetAppInstance(), NULL);
...
void CShaderAPITestApp::DisplayAdapterInfo()
{
	...
	for(int i = 0; i < nAdapterCount; ++i)
	{
		...
		Msg("Adapter %d\n", i);
		Msg("\tName: %s\n\tVendor: 0x%X\n\tDevice: 0x%X\n\tSubSystem: 0x%X\n\tRevision: 0x%X\n\tRecommended DX Level: "
			"%d\n\tMax DX Level: %d\n",
			info.m_pDriverName, info.m_VendorID, info.m_DeviceID, info.m_SubSysID, info.m_Revision,
			info.m_nDXSupportLevel, info.m_nMaxDXSupportLevel);
		...
		for(int j = 0; j < nModeCount; ++j)
		{
			...
			Msg("\t\tH: %5d W: %5d Format: %3d Refresh %3d/%3d\n", mode.m_nWidth, mode.m_nHeight, mode.m_Format,
				mode.m_nRefreshRateNumerator, mode.m_nRefreshRateDenominator);
		}
	}
}
...
void CShaderAPITestApp::CreateSimpleBuffers(ShaderBufferType_t nVBType, ShaderBufferType_t nIBType, bool bBuffered)
{
	...
	{
		m_pVertexBuffer = m_pShaderDevice->CreateVertexBuffer(nVBType, VERTEX_FORMAT_UNKNOWN, 1024, "test");
	}
	else
	{
		m_pVertexBuffer = m_pShaderDevice->CreateVertexBuffer(nVBType, fmt, 4, "test");
	}
...
//-----------------------------------------------------------------------------
void CShaderAPITestApp::CreateShaders(const char *pVShader, int nVBufLen, const char *pGShader, int nGBufLen,
									  const char *pPShader, int nPBufLen)
{
	const char *pVertexShaderVersion =
		g_pMaterialSystemHardwareConfig->GetDXSupportLevel() == 100 ? "vs_4_0" : "vs_2_0";
...
void CShaderAPITestApp::TestColoredQuad(ShaderBufferType_t nVBType, ShaderBufferType_t nIBType, bool bBuffered)
{
	...
	// Draw a quad!
	CreateShaders(s_pSimpleVertexShader, sizeof(s_pSimpleVertexShader), NULL, 0, s_pSimplePixelShader,
				  sizeof(s_pSimplePixelShader));
...
{
	m_pVertexBuffer =
		m_pShaderDevice->CreateVertexBuffer(SHADER_BUFFER_TYPE_DYNAMIC, VERTEX_FORMAT_UNKNOWN, 0x100, "test");
	m_pIndexBuffer =
		m_pShaderDevice->CreateIndexBuffer(SHADER_BUFFER_TYPE_DYNAMIC, MATERIAL_INDEX_FORMAT_UNKNOWN, 30, "test");
	...
	for(int i = 0; i < nLoopCount; ++i)
	{
		...
		m_pShaderAPI->BindVertexBuffer(0, m_pVertexBuffer, vb.Offset(), vb.GetFirstVertex(), vb.TotalVertexCount(),
									   fmt);
		m_pShaderAPI->BindIndexBuffer(m_pIndexBuffer, ib.Offset());
...
int CShaderAPITestApp::Main()
{
	...
	// Test drawing a full-screen quad with interpolated vertex colors for every combo of static/dynamic VB, static
	// dynamic IB, buffered/non-buffered.
	char buf[1024];
	for(int nVBType = 0; nVBType < SHADER_BUFFER_TYPE_COUNT; ++nVBType)
	{
		...
		for(int nIBType = 0; nIBType < SHADER_BUFFER_TYPE_COUNT; ++nIBType)
		{
			...
			//			for( nBuffered = 0; nBuffered < 2; nBuffered++ )
			{
				...
				sprintf(buf, "TestColoredQuad results VB: %d IB: %d Buffered: %d HIT A KEY!", nVBType, nIBType,
						nBuffered != 0);
