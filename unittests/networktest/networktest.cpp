...
bool CNetworkTestApp::Create()
{
	...
	AppSystemInfo_t appSystems[] = {
		{"networksystem.dll", NETWORKSYSTEM_INTERFACE_VERSION}, {"", ""} // Required to terminate the list
	};
...
public:
	...
	CTestNetworkMessage(int nValue) : m_Data(nValue)
	{
		SetReliable(false);
	}
		...
		while(true)
		{
			...
			for(; pEvent; pEvent = g_pNetworkSystem->NextNetworkEvent())
			{
				switch(pEvent->m_nType)
				{
					...
					{
						NetworkMessageReceivedEvent_t *pReceivedEvent =
							static_cast<NetworkMessageReceivedEvent_t *>(pEvent);
						if((pReceivedEvent->m_pNetworkMessage->GetGroup() == TEST_GROUP) &&
						   (pReceivedEvent->m_pNetworkMessage->GetType() == TEST_MESSAGE_1))
						{
