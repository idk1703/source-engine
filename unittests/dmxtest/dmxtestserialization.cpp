...
bool AssertEqualAttributes(bool quiet, CDmAttribute *pAttribute1, CDmAttribute *pAttribute2)
{
	...
	if(pAttribute1->GetType() != AT_ELEMENT_ARRAY && pAttribute1->GetType() != AT_ELEMENT)
	{
	...
	switch(pAttribute1->GetType())
	{
		...
		case AT_VOID:
			return AssertEqualsTest(quiet, pAttribute1->GetValue<CUtlBinaryBlock>(),
									pAttribute2->GetValue<CUtlBinaryBlock>());

		case AT_OBJECTID:
			return true; // skip this for now - two elements can't have the same id, and CreateTestScene currently
						 // creates random test_id's each time...
						 /*
								 {
									 if ( !g_pDataModel->IsEqual( pAttribute1->GetValue<DmObjectId_t>( ),
							pAttribute2->GetValue<DmObjectId_t>( ) ) )
									 {
										 if ( !quiet )
										 {
											 Assert( g_pDataModel->IsEqual( pAttribute1->GetValue<DmObjectId_t>( ),
							pAttribute2->GetValue<DmObjectId_t>( ) ) );
										 }
										 return false;
									 }
		...
		case AT_ELEMENT:
			return AssertEqualElementHierarchies(quiet, pAttribute1->GetValue<DmElementHandle_t>(),
