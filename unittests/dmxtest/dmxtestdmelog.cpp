...
static data_t *defaultvaluetest[NUM_DEF_TESTS] = {values1, values2, values3};
...
static tvpair_t *expectedvalues[NUM_DEF_TESTS] = {expectedvalues1, expectedvalues2, expectedvalues3};
...
void CompareLogToChoreo(CFlexAnimationTrack *track, CDmeFloatLog *log)
{
	...
	for(DmeTime_t t(0); t < DmeTime_t(20.0f); t += DmeTime_t(0.1f))
	{
		...
		CompareFloats(dmevalue, choreovalue, 0.001f, "Time(%f sec) , dme [%f] choreo[%f], diff[%f]\n", t.GetSeconds(),
					  dmevalue, choreovalue, fabs(dmevalue - choreovalue));
	}
}
