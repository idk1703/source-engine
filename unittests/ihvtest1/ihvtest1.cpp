...
static const char *g_LightCombinationNames[] = {"DISABLE                ",
												//	"SPOT                   ",
												"POINT                  ", "DIRECTIONAL            ",
												"SPOT_SPOT              ",
												//	"SPOT_POINT             ",
												//	"SPOT_DIRECTIONAL       ",
												//	"POINT_POINT            ",
												"POINT_DIRECTIONAL      ", "DIRECTIONAL_DIRECTIONAL"};
...
private:
	...
	matrix3x4_t *SetUpBones(studiohdr_t *pStudioHdr, const matrix3x4_t &shapeToWorld, int iRun, int model,
							int boneMask);
...
bool CIHVTestApp::CreateAppWindow(const char *pAppName, int width, int height)
{
	...
	m_hWnd = CreateWindow(pAppName, pAppName, WS_OVERLAPPEDWINDOW, 0, 0, width, height, GetDesktopWindow(), NULL,
						  wc.hInstance, NULL);
...
static BenchRunInfo g_BenchRuns[NUM_BENCH_RUNS] = {
	{{"models/props_foliage/tree_dead01.mdl"}, 100, MODEL_ROWS, MODEL_COLUMNS, 1000.0f, {0}, -1},
};
...
static void WriteBenchResults(void)
{
	...
	for(i = 0; i < NUM_BENCH_RUNS; i++)
	{
		...
		for(j = 0; j < LIGHTING_COMBINATION_COUNT; j++)
		{
			...
			fprintf(fp, "%0.0lf\n", (double)g_BenchResults[i][j].totalTris / (double)g_BenchResults[i][j].totalTime);
...
//-----------------------------------------------------------------------------
matrix3x4_t *CIHVTestApp::SetUpBones(studiohdr_t *pStudioHdr, const matrix3x4_t &shapeToWorld, int iRun, int model,
									 int boneMask)
{
	...
	for(r = 0; r < g_NumRows; r++)
	{
		for(c = 0; c < g_NumCols; c++)
		{
			...
			matrix3x4_t *pBoneToWorld =
				SetUpBones(pModel->pStudioHdr, cameraMatrix, currentRun, modelAlternator, boneMask);
