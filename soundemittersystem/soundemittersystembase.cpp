...
// Purpose:
//-----------------------------------------------------------------------------
CSoundEmitterSystemBase::CSoundEmitterSystemBase() : m_nInitCount(0), m_uManifestPlusScriptChecksum(0) {}
	...
	*/
	...
	if(filesystem->LoadKeyValues(*manifest, IFileSystem::TYPE_SOUNDEMITTER, MANIFEST_FILE, "GAME"))
	{
		...
		for(KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey())
		{
			...
			Warning("CSoundEmitterSystemBase::BaseInit:  Manifest '%s' with bogus file type '%s', expecting "
...
void CSoundEmitterSystemBase::InternalModShutdown()
{
	...
	for(UtlHashHandle_t nIndex = m_Sounds.FirstHandle(); nIndex != m_Sounds.InvalidHandle();
		nIndex = m_Sounds.NextHandle(nIndex))
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSoundEmitterSystemBase::GetParametersForSound(const char *soundname, CSoundParameters &params, gender_t gender,
...
static void SplitName(char const *input, int splitchar, int splitlen, char *before, int beforelen, char *after,
					  int afterlen)
{
	char const *in = input;
	char *out = before;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSoundEmitterSystemBase::InitSoundInternalParameters(const char *soundname, KeyValues *kv,
												...
												bool bRefresh /*=false*/)
{
	...
	if(bIsOverride)
	{
		DevMsg("SoundEmitter:  adding map sound overrides from %s [%i total, %i replacements, %i duplicated "
			   "replacements]\n",
...
bool CSoundEmitterSystemBase::AddSound(const char *soundname, const char *scriptfile,
...
bool CSoundEmitterSystemBase::GetParametersForSoundEx(const char *soundname, HSOUNDSCRIPTHANDLE &handle,
	...
	if(internal->HadMissingWaveFiles() && params.soundname[0] != CHAR_SENTENCE)
	{
		...
		if(!filesystem->FileExists(testfile))
		{
			...
			if(UTL_INVAL_SYMBOL == soundWarnings.Find(key))
			{
				...
				DevMsg("CSoundEmitterSystemBase::GetParametersForSound:  sound '%s' references wave '%s' which doesn't "
					   "exist on disk!\n",
...
// Called from both client and server (single player) or just one (server only in dedicated server and client only if
// connected to a remote server) Called by LevelInitPreEntity to override sound scripts for the mod with level specific
// overrides based on custom mapnames, etc.
