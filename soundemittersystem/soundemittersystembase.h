...
public:
	...
	virtual bool GetParametersForSound(const char *soundname, CSoundParameters &params, gender_t gender,
	...
	virtual bool GetParametersForSoundEx(const char *soundname, HSOUNDSCRIPTHANDLE &handle, CSoundParameters &params,
	...
	// Called from both client and server (single player) or just one (server only in dedicated server and client only
...
private:
	...
	CUtlHashtable<CUtlConstString, gender_t, CaselessStringHashFunctor, UTLConstStringCaselessStringEqualFunctor<char>>
		m_ActorGenders;
	CUtlStableHashtable<CSoundEntry *, empty_t, CSoundEntryHashFunctor, CSoundEntryEqualFunctor, uint16, const char *>
		m_Sounds;
