...
//
bool CMdlLib::StripModelBuffers(CUtlBuffer &mdlBuffer, CUtlBuffer &vvdBuffer, CUtlBuffer &vtxBuffer,
								IMdlStripInfo **ppStripInfo)
{
	...
	if((mdlHdr->checksum != vvdHdr->checksum) || (mdlHdr->checksum != vtxHdr->checkSum))
	{
	...
	ITERATE_CHILDREN2(OptimizedModel::BodyPartHeader_t, mstudiobodyparts_t, vtxBodyPart, mdlBodyPart, vtxHdr, mdlHdr,
					  pBodyPart, pBodypart, numBodyParts)
	ITERATE_CHILDREN2(OptimizedModel::ModelHeader_t, mstudiomodel_t, vtxModel, mdlModel, vtxBodyPart, mdlBodyPart,
					  pModel, pModel, numModels)

	OptimizedModel::ModelLODHeader_t *vtxLod = CHILD_AT(vtxModel, pLOD, 0);
	ITERATE_CHILDREN2(OptimizedModel::MeshHeader_t, mstudiomesh_t, vtxMesh, mdlMesh, vtxLod, mdlModel, pMesh, pMesh,
					  numMeshes)
	...
	CUtlSortVector<CMdlStripInfo::MdlRangeItem, CLessSimple<CMdlStripInfo::MdlRangeItem>> &arrMdlOffsets =
	...
	DECLARE_PTR(Vector4D, vvdTangentSrc,
				vvdHdr->tangentDataStart ? BYTE_OFF_PTR(vvdHdr, vvdHdr->tangentDataStart) : NULL);
	...
	if(vvdHdr->numFixups)
	{
		...
		vvdVertexSrc ? memcpy(vvdVertexSrc, BYTE_OFF_PTR(memTempVVD.Get(), vvdHdr->vertexDataStart),
							  mdlNumVerticesOld * sizeof(*vvdVertexSrc))
					 : 0;
		vvdTangentSrc ? memcpy(vvdTangentSrc, BYTE_OFF_PTR(memTempVVD.Get(), vvdHdr->tangentDataStart),
	...
	ITERATE_CHILDREN(OptimizedModel::MaterialReplacementListHeader_t, vtxMatList, vtxHdr, pMaterialReplacementList,
					 numLODs)
	if(!vtxMatList_idx)
		continue;
	vtxRemove.RemoveElements(CHILD_AT(vtxMatList, pMaterialReplacement, 0), vtxMatList->numReplacements);
	ITERATE_CHILDREN(OptimizedModel::MaterialReplacementHeader_t, vtxMat, vtxMatList, pMaterialReplacement,
					 numReplacements)
	...
	for(OptimizedModel::Vertex_t *vtxVertexElement = vtxVertexBuffer; vtxVertexElement < vtxVertexBufferEnd;
		++vtxVertexElement)
	{
		...
		{ // Index is in use and must be remapped
			...
			int iMesh =
				arrMdlOffsets.FindLessOrEqual(CMdlStripInfo::MdlRangeItem(0, 0, vtxVertexElement - vtxVertexBuffer));
			Assert(iMesh >= 0 && iMesh < arrMdlOffsets.Count());

			CMdlStripInfo::MdlRangeItem &mri = arrMdlOffsets[iMesh];
			Assert((vtxVertexElement - vtxVertexBuffer >= mri.m_offNew) &&
			...
			vtxVertexElement->origMeshVertID =
				srcIndices.Find(vtxVertexElement->origMeshVertID + mri.m_offOld) - mri.m_offNew;
	...
	ITERATE_CHILDREN(OptimizedModel::StripHeader_t, vtxStrip, vtxStripGroup, pStrip, numStrips)
	vtxStrip->indexOffset = vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->indexOffset + vtxStrip->indexOffset) -
							vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->indexOffset);
	vtxStrip->vertOffset = vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->vertOffset + vtxStrip->vertOffset) -
