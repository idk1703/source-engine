...
int CInsertionTracker::GetNumBytesInserted() const
{
	...
	for(Map::IndexType_t idx = m_map.FirstInorder(); idx != m_map.InvalidIndex(); idx = m_map.NextInorder(idx))
	{
...
void CInsertionTracker::MemMove(void *ptrBase, int &length) const
{
	...
	for(Map::IndexType_t idx = m_map.LastInorder(); idx != m_map.InvalidIndex(); idx = m_map.PrevInorder(idx))
	{
...
void CRemoveTracker::MemMove(void *ptrBase, int &length) const
{
	...
	for(Map::IndexType_t idx = m_map.FirstInorder(); idx != m_map.InvalidIndex(); idx = m_map.NextInorder(idx))
	{
		...
		memmove(ptrDest, BYTE_OFF_PTR(ptrDest, numBytes),
				BYTE_DIFF_PTR(BYTE_OFF_PTR(ptr, numBytes), BYTE_OFF_PTR(ptrBase, length)));
		iRemoved += numBytes;
	}
