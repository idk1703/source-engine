...
CMdlStripInfo::CMdlStripInfo() : m_eMode(MODE_UNINITIALIZED), m_lChecksumOld(0), m_lChecksumNew(0)
{
	NULL;
}
	...
	switch(m_eMode)
	{
		...
		case MODE_STRIP_LOD_1N:
			...
			for(MdlRangeItem const *pmri = m_vtxMdlOffsets.Base(), *pmriEnd = pmri + m_vtxMdlOffsets.Count();
				pmri < pmriEnd; ++pmri)
				bufStorage.PutInt(pmri->m_offOld), bufStorage.PutInt(pmri->m_offNew), bufStorage.PutInt(pmri->m_numOld),
...
bool CMdlStripInfo::UnSerialize(CUtlBuffer &bufData)
{
	...
	switch(chHeader[3])
	{
		...
		case MODE_STRIP_LOD_1N:
			...
			for(uint32 *pdwBase = m_vtxVerts.Base(), *pdwEnd = pdwBase + m_vtxVerts.GetNumDWords(); pdwBase < pdwEnd;
				++pdwBase)
			...
			for(MdlRangeItem *pmri = m_vtxMdlOffsets.Base(), *pmriEnd = pmri + m_vtxMdlOffsets.Count(); pmri < pmriEnd;
				++pmri)
				pmri->m_offOld = bufData.GetInt(), pmri->m_offNew = bufData.GetInt(), pmri->m_numOld = bufData.GetInt(),
...
bool CMdlStripInfo::StripHardwareVertsBuffer(CUtlBuffer &vhvBuffer)
{
	...
	vhvRemove.RemoveBytes(BYTE_OFF_PTR(vhvHdr, vhvVertOffset + vhvHdr->m_nVertexes * vhvHdr->m_nVertexSize),
						  vhvLength - (vhvVertOffset + vhvHdr->m_nVertexes * vhvHdr->m_nVertexSize));
