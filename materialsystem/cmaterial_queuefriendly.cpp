...
#define USE_QUEUED_MATERIAL_CALLS // uncomment to queue up material changing calls. Comment out to always use instant
								  // calls.
...
PreviewImageRetVal_t CMaterial_QueueFriendly::GetPreviewImageProperties(int *width, int *height,
...
PreviewImageRetVal_t CMaterial_QueueFriendly::GetPreviewImage(unsigned char *data, int width, int height,
...
bool CMaterial_QueueFriendly::IsTranslucent()
{
	...
	return m_pRealTimeVersion->IsTranslucentInternal(
		GetMaterialVarFlag(MATERIAL_VAR_IGNORE_ALPHA_MODULATION) ? 1.0f : m_fAlphaModulationOnQueueCompletion);
}
