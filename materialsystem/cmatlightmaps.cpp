...
static ConVar mat_lightmap_pfms(
	"mat_lightmap_pfms", "0", FCVAR_MATERIAL_SYSTEM_THREAD,
	"Outputs .pfm files containing lightmap data for each lightmap page when a level exits."); // Write PFM files for
																							   // each lightmap page in
																							   // the game directory
																							   // when exiting a level

#define USE_32BIT_LIGHTMAPS_ON_360 // uncomment to use 32bit lightmaps, be sure to keep this in sync with the same
								   // #define in stdshaders/lightmappedgeneric_ps2_3_x.h
...
void CMatLightmaps::EnumerateMaterials(void)
{
	...
	int id = 0;
	for(MaterialHandle_t i = GetMaterialDict()->FirstMaterial(); i != GetMaterialDict()->InvalidMaterial();
...
//-----------------------------------------------------------------------------
int CMatLightmaps::AllocateLightmap(int width, int height, int offsetIntoLightmapPage[2], IMaterial *iMaterial)
{
...
void CMatLightmaps::EndLightmapAllocation()
{
	...
	for(i = 0; i < GetNumLightmapPages(); i++)
	{
		...
		m_pLightmapPages[i].m_Width =
			(unsigned short)(lastStaticLightmap ? lastLightmapPageWidth : GetMaxLightmapPageWidth());
		m_pLightmapPages[i].m_Height =
			(unsigned short)(lastStaticLightmap ? lastLightmapPageHeight : GetMaxLightmapPageHeight());
...
void CMatLightmaps::AllocateLightmapTexture(int lightmap)
{
	...
	switch(m_eLightmapsState)
	{
		case STATE_DEFAULT:
			// Allow allocations in default state
			{
				m_LightmapPageTextureHandles[lightmap] = g_pShaderAPI->CreateTexture(
					GetLightmapWidth(lightmap), GetLightmapHeight(lightmap), 1, imageFormat, 1, 1, flags, debugName,
		...
		case STATE_RELEASED:
			...
			DevMsg(
				"AllocateLightmapTexture(%d) in released lightmap state (STATE_RELEASED), delayed till \"Restore\".\n",
				lightmap);
			return;
...
fltx4 ConvertLightmapColorToRGBScale(FLTX4 lightmapColor)
{
	...
	result = MaskedAssign(fl4OutofRange, SetWSIMD(result, scale),
...
void CMatLightmaps::BumpedLightmapBitsToPixelWriter_LDR(float *pFloatImage, float *pFloatImageBump1,
														...
														FloatBitMap_t *pfmOut)
{
	...
	for(int t = 0; t < pLightmapSize[1]; t++)
	{
		...
		for(int s = 0; s < nLightmapSize0; s++, m_LightmapPixelWriter.SkipBytes(nRewindToNextPixel),
				srcTexelOffset += (sizeof(Vector4D) / sizeof(float)))
		{
			unsigned char color[4][3];

			ColorSpace::LinearToBumpedLightmap(&pFloatImage[srcTexelOffset], &pFloatImageBump1[srcTexelOffset],
...
void CMatLightmaps::BumpedLightmapBitsToPixelWriter_HDRF(float *pFloatImage, float *pFloatImageBump1,
														 float *pFloatImageBump2, float *pFloatImageBump3,
														 int pLightmapSize[2], int pOffsetIntoLightmapPage[2],
														 FloatBitMap_t *pfmOut)
{
	...
	for(int t = 0; t < pLightmapSize[1]; t++)
	{
				...
				srcTexelOffset += (sizeof(Vector4D) / sizeof(float)))
		{
			...
			m_LightmapPixelWriter.WritePixelNoAdvanceF(
				pFloatImageBump1[srcTexelOffset], pFloatImageBump1[srcTexelOffset + 1],
				pFloatImageBump1[srcTexelOffset + 2], pFloatImage[srcTexelOffset + 3]);

			m_LightmapPixelWriter.SkipBytes(nLightmap0WriterSizeBytes);
			m_LightmapPixelWriter.WritePixelNoAdvanceF(
				pFloatImageBump2[srcTexelOffset], pFloatImageBump2[srcTexelOffset + 1],
				pFloatImageBump2[srcTexelOffset + 2], pFloatImage[srcTexelOffset + 3]);

			m_LightmapPixelWriter.SkipBytes(nLightmap0WriterSizeBytes);
			m_LightmapPixelWriter.WritePixelNoAdvanceF(
				pFloatImageBump3[srcTexelOffset], pFloatImageBump3[srcTexelOffset + 1],
...
namespace
{
	...
	FORCEINLINE fltx4 PackPixel_BGRA8888(FLTX4 rgba)
	{
		...
		static const XMVECTOR PackScale = {(1.0f / (FLOAT)(1 << 22)), (1.0f / (FLOAT)(1 << 22)),
										   (1.0f / (FLOAT)(1 << 22)),
										   (1.0f / (FLOAT)(1 << 22))}; // 255.0f / (FLOAT)(1 << 22)
	...
	struct ALIGN128 CPixelWriterStoreGather
	{
		...
		ALIGN128 uint32 m_data[kRows][kWordsPerRow]; // four rows of bgra data, aligned to 4 cache lines. dwords so
													 // memcpy works better.
		int m_wordsGathered;
		int m_bytesBetweenWriterRows; // the number of bytes spacing the maps inside the writer from each other
									  // if we weren't gathering, we'd SkipBytes this many between the base map, bump1,
									  // etc.

		// write four rows, as SIMD registers, into the buffers
		inline void write(CPixelWriter *RESTRICT pLightmapPixelWriter, FLTX4 row0, FLTX4 row1, FLTX4 row2,
						  FLTX4 row3) RESTRICT
		{
			// if full, commit
			Assert(m_wordsGathered <= kWordsPerRow);
			AssertMsg((m_wordsGathered & 3) == 0,
					  "Don't call CPixelWriterStoreGather::write after ::writeJustX"); // single-word writes have
																					   // misaligned me
		...
		inline void writeJustW(CPixelWriter *RESTRICT pLightmapPixelWriter, FLTX4 row0, FLTX4 row1, FLTX4 row2,
							   FLTX4 row3) RESTRICT
		{
			// if full, commit
			Assert(m_wordsGathered <= kWordsPerRow);
...
static void BumpedLightmapBitsToPixelWriter_HDRI_BGRA_X360(float *RESTRICT pFloatImage,
														   float *RESTRICT pFloatImageBump1,
														   float *RESTRICT pFloatImageBump2,
														   float *RESTRICT pFloatImageBump3, int pLightmapSize[2],
														   int pOffsetIntoLightmapPage[2], FloatBitMap_t *pfmOut,
														   CPixelWriter *RESTRICT m_LightmapPixelWriter)
{
	AssertMsg(m_LightmapPixelWriter->GetPixelSize() == 4,
			  "BGRA format is no longer four bytes long? This is unsupported on 360, and probably immoral as well.");
...
#define FOUR (sizeof(Vector4D) / sizeof(float)) //  make explicit when we're incrementing by length of a 4dvec
		...
		for(s = 0; s < groupsOfFourLimit; s += 4, srcTexelOffset += 4 * (FOUR))
		{
			...
			ColorSpace::LinearToBumpedLightmap(
				vFloatImage[0], vFloatImageBump1[0], vFloatImageBump2[0], vFloatImageBump3[0],
				// transform "in place":
				vFloatImage[0], vFloatImageBump1[0], vFloatImageBump2[0], vFloatImageBump3[0]);
			ColorSpace::LinearToBumpedLightmap(
				vFloatImage[1], vFloatImageBump1[1], vFloatImageBump2[1], vFloatImageBump3[1],
				// transform "in place":
				vFloatImage[1], vFloatImageBump1[1], vFloatImageBump2[1], vFloatImageBump3[1]);
			ColorSpace::LinearToBumpedLightmap(
				vFloatImage[2], vFloatImageBump1[2], vFloatImageBump2[2], vFloatImageBump3[2],
				// transform "in place":
				vFloatImage[2], vFloatImageBump1[2], vFloatImageBump2[2], vFloatImageBump3[2]);
			ColorSpace::LinearToBumpedLightmap(
			...
			srcTexelOffset += (FOUR))
		{
			...
			// perform an arcane averaging operation upon the bump map values
			ColorSpace::LinearToBumpedLightmap(vFloatImage, vFloatImageBump1, vFloatImageBump2, vFloatImageBump3,
			...
			*/
			...
			Assert(*reinterpret_cast<unsigned int *>(m_LightmapPixelWriter->GetCurrentPixel()) == PackPixel_BGRA8888(
			vColor[0] ).u[3] ); void * RESTRICT pBits = m_LightmapPixelWriter->SkipBytes( nLightmap0WriterSizeBytes );
			m_LightmapPixelWriter->WritePixelNoAdvance_BGRA8888( vColor[1], pBits );
			Assert(*reinterpret_cast<unsigned int *>(m_LightmapPixelWriter->GetCurrentPixel()) == PackPixel_BGRA8888(
			vColor[1] ).u[3] ); pBits = m_LightmapPixelWriter->SkipBytes( nLightmap0WriterSizeBytes );
			m_LightmapPixelWriter->WritePixelNoAdvance_BGRA8888( vColor[2], pBits );
			Assert(*reinterpret_cast<unsigned int *>(m_LightmapPixelWriter->GetCurrentPixel()) == PackPixel_BGRA8888(
			vColor[2] ).u[3] ); pBits = m_LightmapPixelWriter->SkipBytes( nLightmap0WriterSizeBytes );
...
void CMatLightmaps::BumpedLightmapBitsToPixelWriter_HDRI(float *RESTRICT pFloatImage, float *RESTRICT pFloatImageBump1,
														 float *RESTRICT pFloatImageBump2,
														 float *RESTRICT pFloatImageBump3, int pLightmapSize[2],
														 int pOffsetIntoLightmapPage[2], FloatBitMap_t *pfmOut) RESTRICT
{
	...
	{
		AssertMsg(!IsX360(),
				  "Tried to use a floating-point pixel format for lightmaps on 360, which is not supported.");
		if(!IsX360())
		{
			for(int t = 0; t < pLightmapSize[1]; t++)
			{
						...
						srcTexelOffset += (sizeof(Vector4D) / sizeof(float)))
				{
					...
					*/
					...
					m_LightmapPixelWriter.WritePixelNoAdvanceF(toFloat * color[0][0], toFloat * color[0][1],
															   toFloat * color[0][2], toFloat * color[0][3]);

					m_LightmapPixelWriter.SkipBytes(nLightmap0WriterSizeBytes);
					m_LightmapPixelWriter.WritePixelNoAdvanceF(toFloat * color[1][0], toFloat * color[1][1],
															   toFloat * color[1][2], toFloat * color[1][3]);

					m_LightmapPixelWriter.SkipBytes(nLightmap0WriterSizeBytes);
					m_LightmapPixelWriter.WritePixelNoAdvanceF(toFloat * color[2][0], toFloat * color[2][1],
															   toFloat * color[2][2], toFloat * color[2][3]);

					m_LightmapPixelWriter.SkipBytes(nLightmap0WriterSizeBytes);
					m_LightmapPixelWriter.WritePixelNoAdvanceF(toFloat * color[3][0], toFloat * color[3][1],
...
#define FOUR (sizeof(Vector4D) / sizeof(float)) // in case this ever changes
					...
					s++, m_LightmapPixelWriter.SkipBytes(nRewindToNextPixel), srcTexelOffset += (FOUR))
				{
					...
					// perform an arcane averaging operation upon the bump map values
					ColorSpace::LinearToBumpedLightmap(vFloatImage, vFloatImageBump1, vFloatImageBump2,
													   vFloatImageBump3, vColor[0], vColor[1], vColor[2], vColor[3]);
					...
					if(pfmOut)
					{
						...
						pixelData.Blue = color[0][2];
						pixelData.Alpha = alpha;
						pfmOut->WritePixelRGBAF(pOffsetIntoLightmapPage[0] + s, pOffsetIntoLightmapPage[1] + t,
						pixelData);
						*/
					}
				}
...
void CMatLightmaps::LightmapBitsToPixelWriter_LDR(float *pFloatImage, int pLightmapSize[2],
												  int pOffsetIntoLightmapPage[2], FloatBitMap_t *pfmOut)
{
...
void CMatLightmaps::LightmapBitsToPixelWriter_HDRF(float *pFloatImage, int pLightmapSize[2],
												   int pOffsetIntoLightmapPage[2], FloatBitMap_t *pfmOut)
{
	if(IsX360())
	{
...
void CMatLightmaps::LightmapBitsToPixelWriter_HDRI(float *RESTRICT pFloatImage, int pLightmapSize[2],
...
#pragma error("This function only supports 32 bit lightmaps.")
		...
		switch(m_LightmapPixelWriter.GetFormat())
		{
			...
			default:
				AssertMsg1(false, "Unsupported pixel format %d while writing lightmaps!",
...
int CMatLightmaps::AllocateDynamicLightmap(int lightmapSize[2], int *pOutOffsetIntoPage, int frameID)
{
	...
	for(int i = 0; i < COUNT_DYNAMIC_LIGHTMAP_PAGES; i++)
	{
		...
		if(m_dynamic.lightmapLockFrame[dynamicIndex] != frameID)
		{
			...
			m_dynamic.imagePackers[dynamicIndex].Reset(0, m_pLightmapPages[lightmapPageIndex].m_Width,
													   m_pLightmapPages[lightmapPageIndex].m_Height);
		}

		if(m_dynamic.imagePackers[dynamicIndex].AddBlock(lightmapSize[0], lightmapSize[1], &pOutOffsetIntoPage[0],
...
//-----------------------------------------------------------------------------
void CMatLightmaps::UpdateLightmap(int lightmapPageID, int lightmapSize[2], int offsetIntoLightmapPage[2],
								   ...
								   float *pFloatImageBump3)
{
	...
	if(mat_lightmap_pfms.GetBool())
	{
		...
		if(NULL == m_pLightmapDataPtrArray[lightmapPageID])
		{
			m_pLightmapDataPtrArray[lightmapPageID] =
				new FloatBitMap_t(m_pLightmapPages[lightmapPageID].m_Width, m_pLightmapPages[lightmapPageID].m_Height);
...

	{
		...
		if(hasBump)
		{
			...
			{
				case HDR_TYPE_NONE:
					BumpedLightmapBitsToPixelWriter_LDR(pFloatImage, pFloatImageBump1, pFloatImageBump2,
														pFloatImageBump3, lightmapSize,
														bLockSubRect ? subRectOffset : offsetIntoLightmapPage, pfmOut);
					break;
				case HDR_TYPE_INTEGER:
					BumpedLightmapBitsToPixelWriter_HDRI(pFloatImage, pFloatImageBump1, pFloatImageBump2,
														 pFloatImageBump3, lightmapSize,
														 bLockSubRect ? subRectOffset : offsetIntoLightmapPage, pfmOut);
					break;
				case HDR_TYPE_FLOAT:
					BumpedLightmapBitsToPixelWriter_HDRF(pFloatImage, pFloatImageBump1, pFloatImageBump2,
														 pFloatImageBump3, lightmapSize,
														 bLockSubRect ? subRectOffset : offsetIntoLightmapPage, pfmOut);
					break;
			}
		...
		else
		{
			...
			{
				case HDR_TYPE_NONE:
					LightmapBitsToPixelWriter_LDR(pFloatImage, lightmapSize,
												  bLockSubRect ? subRectOffset : offsetIntoLightmapPage, pfmOut);
					break;

				case HDR_TYPE_INTEGER:
					LightmapBitsToPixelWriter_HDRI(pFloatImage, lightmapSize,
												   bLockSubRect ? subRectOffset : offsetIntoLightmapPage, pfmOut);
					break;

				case HDR_TYPE_FLOAT:
					LightmapBitsToPixelWriter_HDRF(pFloatImage, lightmapSize,
												   bLockSubRect ? subRectOffset : offsetIntoLightmapPage, pfmOut);
					break;
...
void CMatLightmaps::ComputeSortInfo(MaterialSystem_SortInfo_t *pInfo, int &sortId, bool alpha)
{
		...
		i = GetMaterialDict()->NextMaterial(i))
	{
		...
		for(lightmapPageID = pMaterial->GetMinLightmapPageID(); lightmapPageID <= pMaterial->GetMaxLightmapPageID();
			++lightmapPageID)
		{
...
void CMatLightmaps::ComputeWhiteLightmappedSortInfo(MaterialSystem_SortInfo_t *pInfo, int &sortId, bool alpha)
{
		...
		i = GetMaterialDict()->NextMaterial(i))
	{
		...
		if(pMaterial->GetNeedsWhiteLightmap() && (pMaterial->GetReferenceCount() > 0))
		{
			// const IMaterialVar *pTransVar = pMaterial->GetMaterialProperty( MATERIAL_PROPERTY_OPACITY );
			//		if( ( !alpha && ( pTransVar->GetIntValue() == MATERIAL_TRANSLUCENT ) ) ||
			//			( alpha && !( pTransVar->GetIntValue() == MATERIAL_TRANSLUCENT ) ) )
