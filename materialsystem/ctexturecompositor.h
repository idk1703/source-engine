...
{
public:
	CTextureCompositor(int _width, int _height, int nTeam, const char *pCompositeName, uint64 nRandomSeed,
	...
	const CUtlString &GetName() const
	{
		return m_CompositeName;
	}
	uint32 GetCreateFlags() const
	{
		return m_nTexCompositeCreateFlags;
	}
	void GetTextureName(char *pOutBuffer, int nBufferLen) const;
	void GetSeed(uint32 *pOutHi, uint32 *pOutLo) const;

	void SetTemplate(const char *pTemplate)
	{
		m_TemplateName = pTemplate;
	}
	bool UsesTemplate() const
	{
		return m_TemplateName.IsEmpty() == false;
	}
	const CUtlString &GetTemplateName() const
	{
...
public:
	...
	void SetImplementsName(const CUtlString &implementsName)
	{
		m_ImplementsName = implementsName;
	}
	bool ImplementsTemplate() const
	{
		return m_ImplementsName.IsEmpty() == false;
	}
	const CUtlString &GetImplementsName() const
	{
		return m_ImplementsName;
	}

	void SetCheckedForCycles(bool checked)
	{
		m_bCheckedForCycles = checked;
	}
	bool HasCheckedForCycles() const
	{
		return m_bCheckedForCycles;
	}

private:
	CTextureCompositorTemplate(const char *pName, KeyValues *pKV)
		: m_pKV(pKV), m_Name(pName), m_bCheckedForCycles(false)
	{
	}
...
CTextureCompositor *CreateTextureCompositor(int _w, int _h, const char *pCompositeName, int nTeamNum,
											uint64 _randomSeed, KeyValues *_stageDesc, uint32 texCompositeCreateFlags);
