...
public:
	...
	virtual ShaderAPITextureHandle_t GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrameVar,
	...
	virtual bool InitRenderState(IShader *pShader, int numParams, IMaterialVar **params,
								 ShaderRenderState_t *pRenderState, char const *pMaterialName);
	...
	virtual void DrawElements(IShader *pShader, IMaterialVar **params, ShaderRenderState_t *pShaderState,
							  VertexCompressionType_t vertexCompression, uint32 nVarChangeID);
...
private:
	...
	void PrepForShaderDraw(IShader *pShader, IMaterialVar **ppParams, ShaderRenderState_t *pRenderState,
						   int modulation);
	...
	void CopyMaterialVarToDebugShader(IMaterialInternal *pDebugMaterial, IShader *pShader, IMaterialVar **ppParams,
									  const char *pSrcVarName, const char *pDstVarName = NULL);
	...
	bool DrawEnvmapMask(IShader *pShader, IMaterialVar **ppParams, ShaderRenderState_t *pRenderState,
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderSystem, IShaderSystem, SHADERSYSTEM_INTERFACE_VERSION, s_ShaderSystem);

//-----------------------------------------------------------------------------
// Debugging shader names
//-----------------------------------------------------------------------------
const char *CShaderSystem::s_pDebugShaderName[MATERIAL_DEBUG_COUNT] = {
	"FillRate", "DebugNormalMap", "DebugDrawEnvmapMask", "DebugDepth", "DebugDepth", "Wireframe_DX9"};
...
void CShaderSystem::Init()
{
	...
	if(CommandLine()->FindParm("-noshaderapi") || CommandLine()->FindParm("-makereslists"))
	{
...
void CShaderSystem::LoadModShaderDLLs(int dxSupportLevel)
{
	...
	// Don't do this for Valve mods. They don't need them, and attempting to load them is an opportunity for cheaters to
	// get their code into the process
...
int CShaderSystem::GetShaders(int nFirstShader, int nMaxCount, IShader **ppShaderList) const
{
	...
	for(int i = m_ShaderDLLs.Count(); --i >= 0;)
	{
		...
		for(unsigned short j = info.m_ShaderDict.First(); j != info.m_ShaderDict.InvalidIndex();
...
void CShaderSystem::CreateDebugMaterials()
{
	...
	for(i = 0; i < MATERIAL_DEBUG_COUNT; ++i)
	{
		...
		m_pDebugMaterials[i] =
			static_cast<IMaterialInternal *>(MaterialSystem()->CreateMaterial(shaderName, pVMTKeyValues[i]));
...
//-----------------------------------------------------------------------------
void CShaderSystem::PrepForShaderDraw(IShader *pShader, IMaterialVar **ppParams, ShaderRenderState_t *pRenderState,
									  int nModulation)
{
	Assert(!m_pRenderState);
...
void CShaderSystem::InitStateSnapshots(IShader *pShader, IMaterialVar **params, ShaderRenderState_t *pRenderState)
{
	...
	for(int i = 0; i < nSnapshotCount; ++i)
	{
		...
		pRenderState->m_pSnapshots[i].m_nPassCount = 0;
		pShader->DrawElements(params, i, g_pShaderShadow, 0, VERTEX_COMPRESSION_NONE,
...
bool CShaderSystem::ComputeVertexFormatFromSnapshot(IMaterialVar **params, ShaderRenderState_t *pRenderState)
{
	...
	for(int mod = 1; mod < nModulationSnapshotCount; ++mod)
	{
		...
		StateSnapshot_t *pSnapshotsTest = (StateSnapshot_t *)_alloca(numSnapshotsTest * sizeof(StateSnapshot_t));
...
//-----------------------------------------------------------------------------
bool CShaderSystem::InitRenderState(IShader *pShader, int numParams, IMaterialVar **params,
									ShaderRenderState_t *pRenderState, char const *pMaterialName)
{
	Assert(!m_pRenderState);
...
void CShaderSystem::DrawElements(IShader *pShader, IMaterialVar **params, ShaderRenderState_t *pRenderState,
								 VertexCompressionType_t vertexCompression, uint32 nMaterialVarChangeTimeStamp)
{
...
{
	// YWB Hack if running with -noshaderapi/-makereslists this forces materials to "precache" which means they will
	// resolve their .vtf files for
...
// Copies material vars
//-----------------------------------------------------------------------------
void CShaderSystem::CopyMaterialVarToDebugShader(IMaterialInternal *pDebugMaterial, IShader *pShader,
												 IMaterialVar **ppParams, const char *pSrcVarName,
...
// Draws envmapmask
//-----------------------------------------------------------------------------
bool CShaderSystem::DrawEnvmapMask(IShader *pShader, IMaterialVar **ppParams, ShaderRenderState_t *pRenderState,
								   VertexCompressionType_t vertexCompression)
{
	...
	else
	{
		...
		CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams, "$envmapmasktransform",
...
ShaderAPITextureHandle_t CShaderSystem::GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrame,
																	  int nTextureChannel)
{
								...
								int nAdditionalCreationFlags /* = 0 */)
{
	...
	{
		pTexture = static_cast<ITextureInternal *>(
			MaterialSystem()->FindTexture(pName, pTextureGroupName, false, nAdditionalCreationFlags));
	}
