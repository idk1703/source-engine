...
ConVar mat_forcemanagedtextureintohardware("mat_forcemanagedtextureintohardware", "1",
										   FCVAR_HIDDEN | FCVAR_ALLOWED_IN_COMPETITIVE);

ConVar mat_supportflashlight(
	"mat_supportflashlight", "-1", FCVAR_HIDDEN,
...
#endif
ConVar mat_texture_reload_frame_swap_workaround("mat_texture_reload_frame_swap_workaround",
												CV_FRAME_SWAP_WORKAROUND_DEFAULT, FCVAR_INTERNAL_USE,
												"Workaround certain GL drivers holding unnecessary amounts of data "
												"when loading many materials by forcing synthetic frame swaps");
...
#else
static ConVar mat_dxlevel("mat_dxlevel", "0", 0,
						  "Current DirectX Level. Competitive play requires at least mat_dxlevel 90", false, 0, false,
						  0, true, 90, false, 0, NULL);
#endif
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CMaterialSystem, IMaterialSystem, MATERIAL_SYSTEM_INTERFACE_VERSION,
								  g_MaterialSystem);

// Expose this to the external shader DLLs
MaterialSystem_Config_t g_config;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(MaterialSystem_Config_t, MaterialSystem_Config_t, MATERIALSYSTEM_CONFIG_VERSION,
								  g_config);
			...
			hMaterial != g_MaterialSystem.InvalidMaterial(); hMaterial = hNext)
		{
			...
			if(!pMatInternal->IsPreloaded())
			{
				...
				if(pMatInternal->GetReferenceCount() <= 0)
				{
					if(bSpew)
					{
						Msg("CResourcePreloadMaterial: Purging: %s (%d)\n", pMatInternal->GetName(),
							pMatInternal->GetReferenceCount());
					}
...
void CMaterialSystem::CreateDebugMaterials()
{
	if(!m_pDrawFlatMaterial)
	{
		...
		g_pErrorMaterial =
			static_cast<IMaterialInternal *>(CreateMaterial("___error.vmt", pVMTKeyValues))->GetRealTimeVersion();

		pVMTKeyValues = new KeyValues("UnlitGeneric");
		pVMTKeyValues->SetInt("$flat", 1);
		pVMTKeyValues->SetInt("$vertexcolor", 1);
		m_pDrawFlatMaterial =
			static_cast<IMaterialInternal *>(CreateMaterial("___flat.vmt", pVMTKeyValues))->GetRealTimeVersion();

		pVMTKeyValues = new KeyValues("BufferClearObeyStencil");
		pVMTKeyValues->SetInt("$nocull", 1);
		m_pBufferClearObeyStencil[BUFFER_CLEAR_NONE] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil0.vmt", pVMTKeyValues))
				->GetRealTimeVersion();
		...
		m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil1.vmt", pVMTKeyValues))
				->GetRealTimeVersion();
		...
		pVMTKeyValues->SetInt("$vertexcolor", 1);
		m_pBufferClearObeyStencil[BUFFER_CLEAR_ALPHA] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil2.vmt", pVMTKeyValues))
				->GetRealTimeVersion();
		...
		pVMTKeyValues->SetInt("$clearalpha", 1);
		pVMTKeyValues->SetInt("$vertexcolor", 1);
		m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR_AND_ALPHA] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil3.vmt", pVMTKeyValues))
				->GetRealTimeVersion();
		...
		m_pBufferClearObeyStencil[BUFFER_CLEAR_DEPTH] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil4.vmt", pVMTKeyValues))
				->GetRealTimeVersion();
		...
		m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR_AND_DEPTH] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil5.vmt", pVMTKeyValues))
				->GetRealTimeVersion();
		...
		pVMTKeyValues->SetInt("$clearalpha", 1);
		pVMTKeyValues->SetInt("$vertexcolor", 1);
		m_pBufferClearObeyStencil[BUFFER_CLEAR_ALPHA_AND_DEPTH] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil6.vmt", pVMTKeyValues))
				->GetRealTimeVersion();
		...
		pVMTKeyValues->SetInt("$clearalpha", 1);
		pVMTKeyValues->SetInt("$vertexcolor", 1);
		m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR_AND_ALPHA_AND_DEPTH] =
			static_cast<IMaterialInternal *>(CreateMaterial("___buffer_clear_obey_stencil7.vmt", pVMTKeyValues))
				->GetRealTimeVersion();

		if(IsX360())
		{
			pVMTKeyValues = new KeyValues("RenderTargetBlit_X360");
			m_pRenderTargetBlitMaterial =
				static_cast<IMaterialInternal *>(CreateMaterial("___renderTargetBlit.vmt", pVMTKeyValues))
					->GetRealTimeVersion();
		}
...
void CMaterialSystem::CreateCompositorMaterials()
{
	...
	for(int i = ECO_FirstPrecacheMaterial; i < ECO_LastPrecacheMaterial; i++)
	{
		...
		IMaterialInternal *pMatqf =
			assert_cast<IMaterialInternal *>(FindMaterial(pszMaterial, TEXTURE_GROUP_RUNTIME_COMPOSITE));
		Assert(pMatqf);
...
//-----------------------------------------------------------------------------
CreateInterfaceFn CMaterialSystem::Init(char const *pShaderAPIDLL, IMaterialProxyFactory *pMaterialProxyFactory,
...
void CMaterialSystem::ModShutdown()
{
	...
	// HACK - this is here to unhook ourselves from the client interface, since we're not actually notified when it
	// happens
	m_pMaterialProxyFactory = NULL;
}
...
bool CMaterialSystem::AllowThreading(bool bAllow, int nServiceThread)
{
	...
	return false;
#else
	if(CommandLine()->ParmValue("-threads", 2) <
	   2) // if -threads specified on command line to restrict all the pools then obey and not turn on QMS
		bAllow = false;
...
static void ConvertModeStruct(MaterialVideoMode_t *pMode, const ShaderDisplayMode_t &info)
{
	...
	pMode->m_RefreshRate =
		info.m_nRefreshRateDenominator ? (info.m_nRefreshRateNumerator / info.m_nRefreshRateDenominator) : 0;
}
	...
	if(!bPreviouslyUsingGraphics)
	{
		if(IsPC())
		{
			...
			if(MaterialSystem()->CanUseEditorMaterials())
			{
				...
				// lightmaps.
				// HACK!
				// NOTE! : Overbright is 1.0 so that Hammer will work properly with the white bumped and unbumped
				// lightmaps.
...
void CMaterialSystem::RestoreShaderObjects(CreateInterfaceFn shaderFactory, int nChangeFlags)
{
	...
	for(MaterialHandle_t i = m_MaterialDict.FirstMaterial(); i != m_MaterialDict.InvalidMaterial();
...
bool CMaterialSystem::ConvertImageFormat(unsigned char *src, enum ImageFormat srcImageFormat, unsigned char *dst,
										 enum ImageFormat dstImageFormat, int width, int height, int srcStride,
										 int dstStride)
{
	return ImageLoader::ConvertImageFormat(src, srcImageFormat, dst, dstImageFormat, width, height, srcStride,
										   dstStride);
}
...
static void LoadFlags(KeyValues *pGroup, const char *pName, bool bDefaultValue, unsigned int nFlag,
					  unsigned int *pFlags)
{
...
//-----------------------------------------------------------------------------
void CMaterialSystem::GenerateConfigFromConfigKeyValues(MaterialSystem_Config_t *pConfig,
...
//-----------------------------------------------------------------------------
static ConVar mat_vsync("mat_vsync", "0", FCVAR_ALLOWED_IN_COMPETITIVE, "Force sync to vertical retrace", true, 0.0,
						true, 1.0);
...
#elif defined(OSX)
static ConVar mat_forceaniso("mat_forceaniso", "1", FCVAR_ARCHIVE, "Filtering level", true, 0, true,
							 8); // 0 = Bilinear, 1 = Trilinear, 2+ = Aniso
#else
...
// Debugging textures
static ConVar mat_showmiplevels("mat_showmiplevels", "0", FCVAR_CHEAT,
								"color-code miplevels 2: normalmaps, 1: everything else", mat_showmiplevels_Callback_f);

static ConVar mat_specular("mat_specular", "1", FCVAR_ALLOWED_IN_COMPETITIVE,
...
// OSX users are currently running OOM. We limit them to texture quality high here, which avoids the problem while we
...
static ConVar mat_slopescaledepthbias_decal("mat_slopescaledepthbias_decal", "-0.5",
...
static ConVar mat_monitorgamma("mat_monitorgamma", "2.2", FCVAR_ARCHIVE,
							   "monitor gamma (typically 2.2 for CRT and 1.7 for LCD)", true, 1.6f, true, 2.6f);
...
// TV's generally have a 2.5 gamma, so we need to convert our 2.2 frame buffer into a 2.5 frame buffer for display on a
// TV
...
#ifdef DX_TO_GL_ABSTRACTION
static ConVar mat_bufferprimitives("mat_bufferprimitives",
								   "0"); // I'm not seeing any benefit speed wise for buffered primitives on GLM/POSIX
										 // (checked via TF2 timedemo) - default to zero
#else
...
static ConVar mat_fastspecular(
	"mat_fastspecular", "1", 0,
...
static const char *pConvarsAllowedInDXSupport[] = {"cl_detaildist",
												   "cl_detailfade",
												   "cl_ejectbrass",
												   "dsp_off",
												   "dsp_slow_cpu",
...
bool CMaterialSystem::OverrideConfig(const MaterialSystem_Config_t &_config, bool forceUpdate)
{
	...
	if(config.dxSupportLevel != g_config.dxSupportLevel)
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: Setting dxSupportLevelChanged, bResetAnisotropy, and bReloadMaterials because "
					"new dxlevel = %d and old dxlevel = %d\n",
	...
	if(config.HDREnabled() != g_config.HDREnabled())
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: Setting forceUpdate, bReloadMaterials, and bForceAltTab because new hdr level = "
					"%d and old hdr level = %d\n",
	...
	if(config.ShadowDepthTexture() != g_config.ShadowDepthTexture())
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: Setting forceUpdate, bReloadMaterials and recomputeSnapshots (ShadowDepthTexture "
					"changed: %d -> %d)\n",
	...
	if(forceUpdate)
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: forceUpdate is true, therefore setting recomputeSnapshots, bRedownloadLightmaps, "
					"bRedownloadTextures, bResetAnisotropy, and bSetStandardVertexShaderConstants\n");
		}
	...
	if(config.UseSpecular() != g_config.UseSpecular())
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: new usespecular=%d, old usespecular=%d, setting recomputeSnapshots, "
					"bReloadMaterials, and bResetAnisotropy\n",
	...
	if(config.bNoTransparency != g_config.bNoTransparency)
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: new bNoTransparency=%d, old bNoTransparency=%d, setting recomputeSnapshots and "
					"bResetAnisotropy\n",
	...
	if(config.bFilterLightmaps != g_config.bFilterLightmaps)
	{
		...
		{
			Warning(
				"mat_debugalttab: new bFilterLightmaps=%d, old bFilterLightmaps=%d, setting EnableLightmapFiltering\n",
	...
	if(config_internal.r_waterforceexpensive != g_config_internal.r_waterforceexpensive)
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: new r_waterforceexpensive=%d, old r_waterforceexpensive=%d, setting "
					"bReloadMaterials\n",
	...
	if(config.bAllowCheats != g_config.bAllowCheats || config.skipMipLevels != g_config.skipMipLevels ||
	...
	if(config.m_nForceAnisotropicLevel != g_config.m_nForceAnisotropicLevel)
	{
		if(mat_debugalttab.GetBool())
		{
			Warning("mat_debugalttab: new m_nForceAnisotropicLevel: %d, old m_nForceAnisotropicLevel: %d, setting "
					"bResetAnisotropy and bResetTextureFilter\n",
					(int)config.ForceTrilinear(), (int)g_config.ForceTrilinear());
		}
		bResetAnisotropy = true;
		bResetTextureFilter = true;
	...
	if(config.m_VideoMode.m_Width != g_config.m_VideoMode.m_Width ||
	   ...
	   config.m_nAASamples != g_config.m_nAASamples || config.m_nAAQuality != g_config.m_nAAQuality ||
	   config.Windowed() != g_config.Windowed() || config.Stencil() != g_config.Stencil())
	{
		if(mat_debugalttab.GetBool())
		{
	...
	if(!IsX360())
	{
		if(bResetAnisotropy || recomputeSnapshots || bRedownloadLightmaps || bRedownloadTextures || bResetAnisotropy ||
		   bVideoModeChange || bSetStandardVertexShaderConstants || bResetTextureFilter)
		{
			Unlock(hLock);
			ForceSingleThreaded();
			hLock = Lock();
	...
	if(bMonitorGammaChanged)
	{
		...
		g_pShaderDevice->SetHardwareGammaRamp(config.m_fMonitorGamma, config.m_fGammaTVRangeMin,
...
ShaderAPITextureHandle_t CMaterialSystem::GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrame,
																		int nTextureChannel)
{
...
//-----------------------------------------------------------------------------
ITexture *CMaterialSystem::CreateProceduralTexture(const char *pTextureName, const char *pTextureGroupName, int w,
												   int h, ImageFormat fmt, int nFlags)
{
	ITextureInternal *pTex =
		TextureManager()->CreateProceduralTexture(pTextureName, pTextureGroupName, w, h, 1, fmt, nFlags);
	return pTex;
}
...
//-----------------------------------------------------------------------------
IMaterial *CMaterialSystem::FindProceduralMaterial(const char *pMaterialName, const char *pTextureGroupName,
...
bool CMaterialSystem::IsMaterialLoaded(char const *pMaterialName)
{
	...
	return m_MaterialDict.FindMaterial(pTemp, false) !=
		   NULL; // 'false' causes the search to find only file-created materials
}

//-----------------------------------------------------------------------------
// Search by name
//-----------------------------------------------------------------------------
IMaterial *CMaterialSystem::FindMaterial(char const *pMaterialName, const char *pTextureGroupName, bool bComplain,
...
// Search by name
//-----------------------------------------------------------------------------
IMaterial *CMaterialSystem::FindMaterialEx(char const *pMaterialName, const char *pTextureGroupName, int nContext,
										   bool bComplain, const char *pComplainPrefix)
{
	// We need lower-case symbols for this to work
	int nLen = Q_strlen(pMaterialName) + 1;
	char *pFixedNameTemp = (char *)stackalloc(nLen);
	...
	IMaterialInternal *pExistingMaterial =
		m_MaterialDict.FindMaterial(pTemp, false); // 'false' causes the search to find only file-created materials
	...
	else
	{
		...
		{
			pMat =
				m_MaterialDict.AddMaterialSubRect(matNameWithExtension, pTextureGroupName, pKeyValues, pPatchKeyValues);
		}
		else
		{
...
ITexture *CMaterialSystem::FindTexture(char const *pTextureName, const char *pTextureGroupName,
									   bool bComplain /* = false */, int nAdditionalCreationFlags /* = 0 */)
{
	...
	ITextureInternal *pTexture =
		TextureManager()->FindOrLoadTexture(pTextureName, pTextureGroupName, nAdditionalCreationFlags);
	Assert(pTexture);
	if(pTexture->IsError())
	{
		if(IsPC())
...
void CMaterialSystem::UpdateExcludedTextures(void)
{
	...
	// Have to re-setup the representative textures since they may have been removed out from under us by the queued
	// loader.
...
void CMaterialSystem::AllocateStandardTextures()
{
	...
	m_FullbrightLightmapTextureHandle = g_pShaderAPI->CreateTexture(
		1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags, "[FULLBRIGHT_LIGHTMAP_TEXID]", TEXTURE_GROUP_LIGHTMAP);
...
#if !defined(_X360)
	m_BlackTextureHandle = g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlagsSRGB,
...
#if !defined(_X360)
	m_WhiteTextureHandle = g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlagsSRGB,
...
#if !defined(_X360)
	m_GreyTextureHandle = g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlagsSRGB, "[GREY_TEXID]",
													  TEXTURE_GROUP_OTHER);
	...
	// allocate a grey single texel texture with an alpha of zero (for mat_fullbright 2)
#if !defined(_X360)
	m_GreyAlphaZeroTextureHandle = g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_RGBA8888, 1, 1, tcFlagsSRGB,
															   "[GREYALPHAZERO_TEXID]", TEXTURE_GROUP_OTHER);
	...
	m_GreyAlphaZeroTextureHandle =
		((ITextureInternal *)FindTexture("greyalphazero", TEXTURE_GROUP_OTHER, true))->GetTextureHandle(0);
#endif
	...
	m_FlatNormalTextureHandle = g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags,
	...
	m_FullbrightBumpedLightmapTextureHandle = g_pShaderAPI->CreateTexture(
		1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags, "[FULLBRIGHT_BUMPED_LIGHTMAP_TEXID]", TEXTURE_GROUP_LIGHTMAP);
...

	{
		...
		// generate the linear->gamma conversion table texture.
		{
			...
			m_LinearToGammaTableTextureHandle =
				g_pShaderAPI->CreateTexture(LINEAR_TO_GAMMA_TABLE_WIDTH, 1, 1, gammalookupfmt, 1, 1, iGammaLookupFlags,
											"[LINEAR_TO_GAMMA_LOOKUP_SRGBON_TEXID]", TEXTURE_GROUP_PIXEL_SHADERS);
			...
			float pixelData[LINEAR_TO_GAMMA_TABLE_WIDTH]; // sometimes used as float, sometimes as uint8, sizeof(float)
														  // > sizeof(uint8)
			for(int i = 0; i != LINEAR_TO_GAMMA_TABLE_WIDTH; ++i)
			{
				...
				fLookupResult = g_pShaderAPI->LinearToGamma_HardwareSpecific(
					fLookupResult); // that's right, linear->gamma->gamma2x so that that gamma->linear srgb read still
									// ends up in gamma
			...
			g_pShaderAPI->TexImage2D(0, 0, gammalookupfmt, 0, LINEAR_TO_GAMMA_TABLE_WIDTH, 1, gammalookupfmt, false,
									 (void *)pixelData);
		}
		...
		// generate the identity conversion table texture.
		{
			...
			m_LinearToGammaIdentityTableTextureHandle =
				g_pShaderAPI->CreateTexture(LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH, 1, 1, gammalookupfmt, 1, 1, tcFlags,
											"[LINEAR_TO_GAMMA_LOOKUP_SRGBOFF_TEXID]", TEXTURE_GROUP_PIXEL_SHADERS);
			...
			float pixelData[LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH]; // sometimes used as float, sometimes as uint8,
																   // sizeof(float) > sizeof(uint8)
			...
			g_pShaderAPI->TexImage2D(0, 0, gammalookupfmt, 0, LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH, 1, gammalookupfmt,
	...
	{
		m_MaxDepthTextureHandle = g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_RGBA8888, 1, 1, tcFlags,
															  "[MAXDEPTH_TEXID]", TEXTURE_GROUP_OTHER);
		g_pShaderAPI->ModifyTexture(m_MaxDepthTextureHandle);
		g_pShaderAPI->TexMinFilter(SHADER_TEXFILTERMODE_LINEAR);
		g_pShaderAPI->TexMagFilter(SHADER_TEXFILTERMODE_LINEAR);

		// 360 gets depth out of the red channel (which doubles as depth in D24S8) and may be 0/1 depending on
		// REVERSE_DEPTH_ON_X360 PC gets depth out of the alpha channel
	...
	g_pShaderAPI->SetLinearToGammaConversionTextures(m_LinearToGammaTableTextureHandle,
													 m_LinearToGammaIdentityTableTextureHandle);
}
...
void CMaterialSystem::ReleaseStandardTextures()
{
	if(m_StandardTexturesAllocated)
	{
		...
		g_pShaderAPI->SetLinearToGammaConversionTextures(INVALID_SHADERAPI_TEXTURE_HANDLE,
...
void CMaterialSystem::BeginFrame(float frameTime)
{
	...
	g_nDebugVarsSignature = ((mat_specular.GetInt() != 0) + (mat_normalmaps.GetInt() << 1) +
...
static const char *GetMatString(enum MaterialThreadMode_t ThreadMode)
{
	switch(ThreadMode)
	{
		...
		case MATERIAL_QUEUED_SINGLE_THREADED:
			return "queued_single";
		case MATERIAL_QUEUED_THREADED:
			return "queued_threaded";
		default:
			return "???";
	}
}
...
ConVar mat_queue_mode("mat_queue_mode", "-1", FCVAR_ARCHIVE,
					  "The queue/thread mode the material system should use: -1=default, 0=synchronous single thread"
...
ConVar mat_queue_report(
	"mat_queue_report", "0", FCVAR_ARCHIVE,
...
void CMaterialSystem::EndFrame(void)
{
	...
	MaterialThreadMode_t nextThreadMode =
		(iConVarThreadMode >= 0) ? (MaterialThreadMode_t)iConVarThreadMode : m_IdealThreadMode;
	...
	switch(m_ThreadMode)
	{
		...
		case MATERIAL_QUEUED_THREADED:
		{
			...
			m_pActiveAsyncJob =
				new CFunctorJob(CreateFunctor(this, &CMaterialSystem::ThreadExecuteQueuedContext, pPrevContext),
								"ThreadExecuteQueuedContext");
			if(IsX360())
			{
	...
	if(m_ThreadMode != nextThreadMode)
	{
		...
		g_pShaderAPI->EnableShaderShaderMutex(
			m_ThreadMode !=
			MATERIAL_SINGLE_THREADED); // use mutex even for queued to allow "disalow access" to function properly
						...
						if(var)
						{
							switch(var->GetType())
							{
								case MATERIAL_VAR_TYPE_TEXTURE:
								{
									...
									{
										DevMsg("    \"%s\" \"%s\"\n", var->GetName(), texture->GetName());
										DevMsg("        %dx%d refCount: %d numframes: %d\n", texture->GetActualWidth(),
											   texture->GetActualHeight(), texture->GetReferenceCount(),
...
void CMaterialSystem::ListUsedMaterials(void)
{
	...
	numMaterials = 0;
	for(MaterialHandle_t hMaterial = FirstMaterial(); hMaterial != InvalidMaterial();
...
{
	return CreateNamedRenderTargetTextureEx(NULL, w, h, sizeMode, format, depth,
...
ITexture *CMaterialSystem::CreateNamedRenderTargetTexture(
	...
	RenderTargetSizeMode_t sizeMode, // Controls how size is generated (and regenerated on video mode change).
	ImageFormat format, MaterialRenderTargetDepth_t depth, bool bClampTexCoords, bool bAutoMipMap)
{
...
ITexture *CMaterialSystem::CreateNamedRenderTargetTextureEx(
	const char *pRTName, int w, int h,
	RenderTargetSizeMode_t sizeMode, // Controls how size is generated (and regenerated on video mode change).
	ImageFormat format, MaterialRenderTargetDepth_t depth, unsigned int textureFlags, unsigned int renderTargetFlags)
{
	RenderTargetType_t rtType;

	bool gl_canMixTargetSizes = (HardwareConfig() && HardwareConfig()->SupportsGLMixedSizeTargets());

	// On GL, the depth buffer for a render target must be the same size (until we pick up mixed-sized attachments
	// in 10.6.3)
	...
	ITextureInternal *pTex = TextureManager()->CreateRenderTargetTexture(pRTName, w, h, sizeMode, format, rtType,
	...
	ImageFormat format, MaterialRenderTargetDepth_t depth, unsigned int textureFlags, unsigned int renderTargetFlags)
{
	...
	if(!m_bAllocatingRenderTargets)
	{
		Warning("Tried to create render target outside of "
				"CMaterialSystem::BeginRenderTargetAllocation/EndRenderTargetAllocation block\n");
		return NULL;
	}

	ITexture *pTexture =
		CreateNamedRenderTargetTextureEx(pRTName, w, h, sizeMode, format, depth, textureFlags, renderTargetFlags);

	pTexture->DecrementReferenceCount(); // Follow the same convention as CTextureManager::LoadTexture (return refcount
										 // of 0).
	return pTexture;
}
...
public:
	CTextureBitsRegenerator(int w, int h, int mips, ImageFormat fmt, int srcBufferSize, byte *srcBits)
		: m_nWidth(w), m_nHeight(h), m_nMipmaps(mips), m_ImageFormat(fmt)
	{
		Assert(srcBits);
	...
	virtual void RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pRect)
	{
		...
		else
		{
			...
			for(; mip < m_nMipmaps; ++mip)
			{
				...
				ImageLoader::ConvertImageFormat(m_ImageData.Base() + srcOffset, m_ImageFormat, pDest + dstOffset,
												destFmt, srcResX, srcResY);
...
ITexture *CMaterialSystem::CreateTextureFromBits(int w, int h, int mips, ImageFormat fmt, int srcBufferSize,
												 byte *srcBits)
{
	int flags = TEXTUREFLAGS_SINGLECOPY | (mips > 1 ? TEXTUREFLAGS_ALL_MIPS : TEXTUREFLAGS_NOMIP);

	return CreateNamedTextureFromBitsEx("frombits", TEXTURE_GROUP_OTHER, w, h, mips, fmt, srcBufferSize, srcBits,
										flags);
}
...
ITextureCompositor *CMaterialSystem::NewTextureCompositor(int w, int h, const char *pCompositeName, int nTeamNum,
														  uint64 randomSeed, KeyValues *stageDesc,
														  uint32 texCompositeCreateFlags)
{
...
void CMaterialSystem::AsyncFindTexture(const char *pFilename, const char *pTextureGroupName,
									   IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs, bool bComplain,
									   int nAdditionalCreationFlags)
{
	...
	// Bump the ref count on the recipient before handing it off. This ensures the receiver won't go away before we have
	// completed our work.
	pRecipient->AddRef();

	TextureManager()->AsyncFindOrLoadTexture(pFilename, pTextureGroupName, pRecipient, pExtraArgs, bComplain,
											 nAdditionalCreationFlags);
}
...
ITexture *CMaterialSystem::CreateNamedTextureFromBitsEx(const char *pName, const char *pTextureGroupName, int w, int h,
														...
														int nFlags)
{
	...
	ITextureInternal *tex =
		TextureManager()->CreateProceduralTexture(pName, pTextureGroupName, w, h, 1, fmt, nFlags, regen);
	return tex;
}

bool CMaterialSystem::AddTextureCompositorTemplate(const char *pName, KeyValues *pTmplDesc,
												   int /* nTexCompositeTemplateFlags */)
{
...
//
//-----------------------------------------------------------------------------------------------------
void CMaterialSystem::UpdateLightmap(int lightmapPageID, int lightmapSize[2], int offsetIntoLightmapPage[2],
									 ...
									 float *pFloatImageBump3)
{
	...
	if(!pCallQueue)
	{
		m_Lightmaps.UpdateLightmap(lightmapPageID, lightmapSize, offsetIntoLightmapPage, pFloatImage, pFloatImageBump1,
								   pFloatImageBump2, pFloatImageBump3);
	}
	else
	{
...
bool CMaterialSystem::GetTrueTypeFontMetrics(HXUIFONT hFont, XUIFontMetrics *pFontMetrics,
...
bool CMaterialSystem::GetTrueTypeGlyphs(HXUIFONT hFont, int numChars, wchar_t *pWch, int *pOffsetX, int *pOffsetY,
										int *pWidth, int *pHeight, unsigned char *pRGBA, int *pRGBAOffset)
{
	MaterialLock_t hLock = Lock();
	bool result =
		g_pShaderAPI->GetTrueTypeGlyphs(hFont, numChars, pWch, pOffsetX, pOffsetY, pWidth, pHeight, pRGBA, pRGBAOffset);
	Unlock(hLock);
	return result;
}
#endif
...
//-----------------------------------------------------------------------------------------------------
#if defined(_X360)
void CMaterialSystem::ReadBackBuffer(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *pDstData, ImageFormat dstFormat,
									 int dstStride)
{
	...
	Assert(!pTempRT->IsError() && pDstRect->width <= pTempRT->GetActualWidth() &&
		   pDstRect->height <= pTempRT->GetActualHeight());
	...
	XGUntileTextureLevel(pTempRT->GetActualWidth(), pTempRT->GetActualHeight(), 0,
						 XGGetGpuFormat(ImageLoader::ImageFormatToD3DFormat(pTempRT->GetImageFormat())), 0,
						 (char *)writer.GetPixelMemory(), srcStride, NULL, writer.GetPixelMemory(), NULL);

	// swap back to x86 order as expected by image conversion
	ImageLoader::ByteSwapImageData((unsigned char *)writer.GetPixelMemory(), srcStride * pTempRT->GetActualHeight(),
								   pTempRT->GetImageFormat());

	// convert to callers format
	Assert(dstFormat == IMAGE_FORMAT_RGB888);
	ImageLoader::ConvertImageFormat((unsigned char *)writer.GetPixelMemory(), pTempRT->GetImageFormat(), pDstData,
									dstFormat, pDstRect->width, pDstRect->height, srcStride, dstStride);
	...
	if(do_report)
	{
		...
		if(do_report == -1 || flResult > mat_queue_report.GetFloat())
		{
			...
			ConColorMsg(red, "CMaterialSystem::ThreadRelease: %0.2fms = Release:%0.2fms + Acquire:%0.2fms\n", flResult,
						(flEndThreadRelease - flStartTime) * 1000.0, (flEndTime - flEndThreadRelease) * 1000.0);
		}
	}
}
...
IMaterial *CReplacementProxy::GetMaterial()
{
	...
	bool bVisionOverride =
		(localplayer_visionflags.IsValid() && (localplayer_visionflags.GetInt() & (0x01))); // Pyro-vision Goggles
...
static const char *pszReplacementForceCopy[] = {"$nocull",
			...
			else
			{
				for(KeyValues *pSubKey = pPatternsKV->GetFirstSubKey(); pSubKey; pSubKey = pSubKey->GetNextKey())
				{
					...
					{ // We found a replacement!
						...
						{
							for(KeyValues *pCopyKV = pFallbackKeyValues->GetFirstSubKey(); pCopyKV;
						...
						else
						{
							...
							while(pszReplacementForceCopy[nReplaceIndex])
							{
								const char *pszCopyValue =
									pFallbackKeyValues->GetString(pszReplacementForceCopy[nReplaceIndex], NULL);
								if(pszCopyValue)
								{
									pReplacementMaterial->SetString(pszReplacementForceCopy[nReplaceIndex],
																	pszCopyValue);
								}
								nReplaceIndex++;
							}
						...
						for(KeyValues *pSearchKV = pReplacementMaterial->GetFirstSubKey(); pSearchKV;
