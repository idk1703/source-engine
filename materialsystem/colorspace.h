...
{
	void SetGamma(float screenGamma, float texGamma, float overbright, bool allowCheats, bool linearFrameBuffer);
	...
	// assume 0..4 range
	void LinearToBumpedLightmap(const float *linearColor, const float *linearBumpColor1, const float *linearBumpColor2,
								const float *linearBumpColor3, unsigned char *ret, unsigned char *retBump1,
...
#define CONDITION(a, b, c)                           \
	...
	FORCEINLINE void LinearToBumpedLightmap(FLTX4 linearColor, FLTX4 linearBumpColor1, FLTX4 linearBumpColor2,
											...
											fltx4 &retBump2, fltx4 &retBump3) // these will be returned on registers
	{
		...
		// areas.
		fltx4 bumpAverage =
			AddSIMD(AddSIMD(linearBumpColor1, linearBumpColor2), linearBumpColor3); // actually average * 3
		...
		correctionScale = MulSIMD(
			AndNotSIMD(zeroTerms, linearColor), // crush values that were zero in bumpAverage. (saves on dep latency)
											...
											unsigned short *retBump3)
	{
		...
		LinearToBumpedLightmap(linearColor, linearBumpColor1, linearBumpColor2, linearBumpColor3, &linearUnbumped.x,
							   &linearBump1.x, &linearBump2.x, &linearBump3.x);
