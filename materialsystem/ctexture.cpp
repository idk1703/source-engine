...
static ConVar mat_managedtextures(
	"mat_managedtextures", "1", FCVAR_ARCHIVE,
	"If set, allows Direct3D to manage texture uploading at the cost of extra system memory",
	&ConVarChanged_mat_managedtextures);
static void ConVarChanged_mat_managedtextures(IConVar *var, const char *pOldValue, float flOldValue)
{
...
static ConVar mat_spew_on_texture_size("mat_spew_on_texture_size", "0", 0,
...
struct TexDimensions_t
{
	...
	TexDimensions_t(uint16 nWidth = 0, uint nHeight = 0, uint nMipCount = 0, uint16 nDepth = 1)
		: m_nWidth(nWidth), m_nHeight(nHeight), m_nMipCount(nMipCount), m_nDepth(nDepth)
	{
	}
};
...
struct TexInfo_t
{
	...
	TexInfo_t(const char *name = "", unsigned short w = 0, unsigned short h = 0, unsigned short d = 0,
			  unsigned short mips = 0, unsigned short frames = 0, unsigned short copies = 0,
...
enum InternalTextureFlags
{
	...
	TEXTUREFLAGSINTERNAL_TEMPRENDERTARGET =
		0x00000080, // 360: should only allocate texture bits upon first resolve, destroy at level end
};

static int GetThreadId();
static bool SLoadTextureBitsFromFile(IVTFTexture **ppOutVtfTexture, FileHandle_t hFile, unsigned int nFlags,
									 TextureLODControlSettings_t *pInOutCachedFileLodSettings,
									 int nDesiredDimensionLimit, unsigned short *pOutStreamedMips, const char *pName,
									 const char *pCacheFileName, TexDimensions_t *pOptOutMappingDims = NULL,
									 TexDimensions_t *pOptOutActualDims = NULL,
									 TexDimensions_t *pOptOutAllocatedDims = NULL,
									 unsigned int *pOptOutStripFlags = NULL);
static int ComputeActualMipCount(const TexDimensions_t &actualDims, unsigned int nFlags);
static int ComputeMipSkipCount(const char *pName, const TexDimensions_t &mappingDims, bool bIgnorePicmip,
							   IVTFTexture *pOptVTFTexture, unsigned int nFlags, int nDesiredDimensionLimit,
							   unsigned short *pOutStreamedMips,
							   TextureLODControlSettings_t *pInOutCachedFileLodSettings,
							   TexDimensions_t *pOptOutActualDims, TexDimensions_t *pOptOutAllocatedDims,
...
public:
	...
	void InitProceduralTexture(const char *pTextureName, const char *pTextureGroupName, int w, int h, int d,
							   ImageFormat fmt, int nFlags, ITextureRegenerator *generator = NULL);
	...
	virtual ResidencyType_t GetCurrentResidence() const
	{
		return m_residenceCurrent;
	}
	virtual ResidencyType_t GetTargetResidence() const
	{
		return m_residenceTarget;
	}
...
protected:
	...
	void WriteDataToShaderAPITexture(int nFrameCount, int nFaceCount, int nFirstFace, int nMipCount,
...
public:
	void InitRenderTarget(const char *pRTName, int w, int h, RenderTargetSizeMode_t sizeMode, ImageFormat fmt,
						  RenderTargetType_t type, unsigned int textureFlags, unsigned int renderTargetFlags);
...
protected:
	...
	struct ScratchVTF
	{
		...
		IVTFTexture *Get() const
		{
			return m_pScratchVTF;
		}
		void TakeOwnership()
		{
			m_pScratchVTF = NULL;
...
{
public:
	CTextureStreamingJob(CTexture *pTex) : m_referenceCount(0), m_pOwner(pTex)
	{
		Assert(m_pOwner != NULL);
		m_pOwner->AddRef();
	}
	virtual ~CTextureStreamingJob()
	{
	...
	virtual void OnAsyncMapComplete(ITexture *pTex, void *pExtraArgs, void *pMemory, int nPitch)
	{
		Assert(!"unimpl");
	}
	...
	void ForgetOwner(ITextureInternal *pTex)
	{
		Assert(pTex == m_pOwner);
		SafeRelease(&m_pOwner);
	}
...
public:
	...
	virtual bool AsyncReadTextureFromFile(IVTFTexture *pVTFTexture, unsigned int nAdditionalCreationFlags)
	{
...
public:
	...
	void InitFromHandle(const char *pTextureName, const char *pTextureGroupName, ShaderAPITextureHandle_t hTexture);
};
...
void CReferenceToHandleTexture::InitFromHandle(const char *pTextureName, const char *pTextureGroupName,
...
ITextureInternal *ITextureInternal::CreateReferenceTextureFromHandle(const char *pTextureName,
...
ITextureInternal *ITextureInternal::CreateProceduralTexture(const char *pTextureName, const char *pTextureGroupName,
															int w, int h, int d, ImageFormat fmt, int nFlags,
...
// GR - named RT
ITextureInternal *ITextureInternal::CreateRenderTarget(const char *pRTName, int w, int h,
...
//-----------------------------------------------------------------------------
void ITextureInternal::ChangeRenderTarget(ITextureInternal *pTex, int w, int h, RenderTargetSizeMode_t sizeMode,
										  ImageFormat fmt, RenderTargetType_t type, unsigned int textureFlags,
										  unsigned int renderTargetFlags)
{
	pTex->ReleaseMemory();
	dynamic_cast<CTexture *>(pTex)->InitRenderTarget(pTex->GetName(), w, h, sizeMode, fmt, type, textureFlags,
													 renderTargetFlags);
}
...
void CTexture::Shutdown()
{
	...
	for(DataChunk const *pDataChunk = m_arrDataChunks.Base(), *pDataChunkEnd = pDataChunk + m_arrDataChunks.Count();
...
{
	const bool cbThreadInMatQueue = (MaterialSystem()->GetRenderThreadId() == ThreadGetCurrentId());
	cbThreadInMatQueue;
...
void CTexture::ReleaseScratchVTFTexture(IVTFTexture *tex)
{
	...
	Assert(cbThreadInMatQueue || ThreadInMainThread());
	Assert(m_pStreamingVTF == NULL ||
		   ThreadInMainThread()); // Can only manipulate m_pStreamingVTF to release safely in main thread.
		...
		default:
		{
			if(!HushAsserts())
			{
				...
				Assert(m_nOriginalRenderTargetType ==
					   RENDER_TARGET_NO_DEPTH); // Only can use NO_CHANGE if we don't have a depth buffer.
			}
		}
		break;
	}
...
void CTexture::CopyToStagingTexture(ITexture *pDstTex)
{
	...
	if((m_pTextureHandles == NULL || m_nFrameCount == 0) ||
	   (pDstTexActual->m_pTextureHandles == NULL || pDstTexActual->m_nFrameCount == 0))
	{
...
void CTexture::Map(void **pOutBits, int *pOutPitch)
{
	...
	g_pShaderAPI->LockRect(pOutBits, pOutPitch, m_pTextureHandles[0], 0, 0, 0, GetActualWidth(), GetActualHeight(),
						   false, true);
}
...
void CTexture::UpdateLodBias()
{
	...
	if(!bPopIn)
		m_lodBiasCurrent =
			m_lodBiasInitial - (Plat_FloatTime() - m_lodBiasStartTime) / mat_lodin_time.GetFloat() * m_lodBiasInitial;
	else
...
void CTexture::MakePartiallyResident()
{
	...
	else
	{
		...
		MaterialSystem()->AsyncFindTexture(GetName(), GetTextureGroupName(), m_pStreamingJob, (void *)RESIDENT_PARTIAL,
										   false, TEXTUREFLAGS_STREAMABLE_COARSE);
	}
}
...
bool CTexture::MakeFullyResident()
{
	...
	SafeAssign(&m_pStreamingJob, new CTextureStreamingJob(this));
	MaterialSystem()->AsyncFindTexture(GetName(), GetTextureGroupName(), m_pStreamingJob, (void *)RESIDENT_FULL, false,
...
//-----------------------------------------------------------------------------
void CTexture::InitRenderTarget(const char *pRTName, int w, int h, RenderTargetSizeMode_t sizeMode, ImageFormat fmt,
								RenderTargetType_t type, unsigned int textureFlags, unsigned int renderTargetFlags)
{
	if(pRTName)
	{
	...
	else if(type == RENDER_TARGET_WITH_DEPTH || type == RENDER_TARGET_ONLY_DEPTH ||
...
//-----------------------------------------------------------------------------
void CTexture::InitProceduralTexture(const char *pTextureName, const char *pTextureGroupName, int w, int h, int d,
									 ImageFormat fmt, int nFlags, ITextureRegenerator *generator)
{
...
bool CTexture::AllocateShaderAPITextures()
{
	...
	int nShaderApiCreateTextureDepth =
		((m_nFlags & TEXTUREFLAGS_DEPTHRENDERTARGET) && (m_nOriginalRenderTargetType == RENDER_TARGET_ONLY_DEPTH))
			? 1
			: m_dimsAllocated.m_nDepth;

	// Create all animated texture frames in a single call
	g_pShaderAPI->CreateTextures(m_pTextureHandles, nCount, m_dimsAllocated.m_nWidth, m_dimsAllocated.m_nHeight,
								 nShaderApiCreateTextureDepth, m_ImageFormat, m_dimsAllocated.m_nMipCount, nCopies,
	...
	if(m_nFlags & TEXTUREFLAGS_DEPTHRENDERTARGET)
	{
		...
		Assert(m_nFrameCount >= 2);
		m_pTextureHandles[1] =
			g_pShaderAPI->CreateDepthTexture(m_ImageFormat, m_dimsAllocated.m_nWidth, m_dimsAllocated.m_nHeight,
											 debugName, (m_nOriginalRenderTargetType == RENDER_TARGET_ONLY_DEPTH));
		accountingCount += 1;
	}

	STAGING_ONLY_EXEC(g_currentTextures.InsertOrReplace(
		this, TexInfo_t(GetName(), m_dimsAllocated.m_nWidth, m_dimsAllocated.m_nHeight, m_dimsAllocated.m_nDepth,
						m_dimsAllocated.m_nMipCount, accountingCount, nCopies, m_ImageFormat)));
...
ImageFormat CTexture::ComputeActualFormat(ImageFormat srcFormat)
{
	...
	{
		// for the runtime compressed formats the srcFormat won't equal the dstFormat, and we need to return srcFormat
		// here
...
int CTexture::ComputeActualSize(bool bIgnorePicmip, IVTFTexture *pVTFTexture, bool bTextureMigration)
{
	...
	return ComputeMipSkipCount(GetName(), m_dimsMapping, bIgnorePicmip, pVTFTexture, m_nFlags, m_nDesiredDimensionLimit,
							   &m_nStreamingMips, &m_cachedFileLodSettings, &m_dimsActual, &m_dimsAllocated,
							   &stripFlags);
...
void CTexture::SetFilterState()
{
	...
	{
		bIsAnisotropic =
			((m_nFlags & TEXTUREFLAGS_ANISOTROPIC) != 0) && (HardwareConfig()->MaximumAnisotropicLevel() > 1);
		bIsTrilinear = true;
	}
	else
	{
...
bool CTexture::SaveToFile(const char *fileName)
{
	...
	if(pTexture->GetImageFormat() == IMAGE_FORMAT_RGBA8888 || pTexture->GetImageFormat() == IMAGE_FORMAT_ABGR8888 ||
	   pTexture->GetImageFormat() == IMAGE_FORMAT_ARGB8888 || pTexture->GetImageFormat() == IMAGE_FORMAT_BGRA8888 ||
	   pTexture->GetImageFormat() == IMAGE_FORMAT_BGRX8888)
	{
		...
		if(width > pTexture->GetActualWidth() || height > pTexture->GetActualHeight())
		{
			...
			pTexture = materials->CreateNamedRenderTargetTextureEx("_rt_savetofile", width, height, RT_SIZE_LITERAL,
																   IMAGE_FORMAT_BGRA8888, MATERIAL_RT_DEPTH_NONE,
			...
			if(!pTexture || pTexture->IsError())
			{
				...
				Msg("SaveToFile: texture '_rt_FullFrameFB1' failed. Ptr:%p Format:%d\n", pTexture,
					(pTexture ? pTexture->GetImageFormat() : 0));
				return false;
			}
		...
		if((width > 0) && (height > 0))
		{
			...
			if(pixelValue)
			{
				...
				pRenderContext->ReadPixels(0, 0, width, height, (unsigned char *)pixelValue,
										   pTexture->GetImageFormat());

				// Slap the alpha channel at the bottom of the tga file so we don't have to deal with crappy tools that
				// can't
				//  handle rgb + alpha well. This means we can just do a "mat_texture_save_fonts" concommand, and then
				//  use
				...
				if(TGAWriter::WriteTGAFile(fileName, width, height * 2, pTexture->GetImageFormat(), (uint8 *)pixelValue,
										   width * sizeof(BGRA8888_t)))
				{
					bRet = true;
				}
...
bool CTexture::AsyncReadTextureFromFile(IVTFTexture *pVTFTexture, unsigned int nAdditionalCreationFlags)
{
	...
	if(!SLoadTextureBitsFromFile(&pVTFTexture, fileHandle, m_nFlags | nPreserveFlags, &settings,
								 m_nDesiredDimensionLimit, &dontCareStreamedMips, GetName(), pCacheFileName,
								 &m_dimsMapping))
	{
...
void CTexture::Bind(Sampler_t sampler1, int nFrame, Sampler_t sampler2 /* = -1 */)
{
	if(g_pShaderDevice->IsUsingGraphics())
	{
		...
		{
			ExecuteNTimes(
				20, Warning("Trying to bind texture %s, but texture handles are not valid. Binding a white texture!\n",
							GetName()));
...
void CTexture::LoadLowResTexture(IVTFTexture *pTexture)
{
	...
	bool retVal =
#endif
		ImageLoader::ConvertImageFormat(pTexture->LowResImageData(), pTexture->LowResFormat(), m_pLowResImage,
				...
				for(int iFace = 0; iFace < pTexture->FaceCount(); ++iFace)
				{
					for(int iMip = 0; iMip < pTexture->MipCount(); ++iMip)
					{
						   ...
						   pTexture->Format() == IMAGE_FORMAT_ATI1N || pTexture->Format() == IMAGE_FORMAT_ATI2N)
						{
							...
							int alpha = 255;
							FillCompressedTextureWithSingleColor(red, green, blue, alpha, pImageData, nWidth, nHeight,
																 nDepth, pTexture->Format());
						}
						else
						{
...
void CTexture::CopyLowResImageToTexture(IVTFTexture *pTexture)
{
	...
	CPixelWriter pixelWriter;
	pixelWriter.SetPixelMemory(pTexture->Format(), pTexture->ImageData(0, 0, 0), pTexture->RowSizeInBytes(0));
...
bool CTexture::SetupDebuggingTextures(IVTFTexture *pVTFTexture)
{
	...
	else if(g_config.bShowLowResImage && pVTFTexture->FrameCount() == 1 && pVTFTexture->FaceCount() == 1 &&
			((pVTFTexture->Flags() & TEXTUREFLAGS_NORMAL) == 0) && m_LowResImageWidth != 0 && m_LowResImageHeight != 0)
	{
...
bool CTexture::ConvertToActualFormat(IVTFTexture *pVTFTexture)
{
	...
	else if(HardwareConfig()->GetHDRType() == HDR_TYPE_INTEGER && fmt == dstFormat &&
...
IVTFTexture *CTexture::LoadTextureBitsFromFile(char *pCacheFileName, char **ppResolvedFilename)
{
	...
	if(!pVTFTexture)
	{
		...
		TextureLODControlSettings_t settings = m_cachedFileLodSettings;
		if(!SLoadTextureBitsFromFile(&pVTFTexture, fileHandle, m_nFlags | nPreserveFlags, &settings,
									 m_nDesiredDimensionLimit, &m_nStreamingMips, GetName(), pCacheFileName,
									 &m_dimsMapping, &m_dimsActual, &m_dimsAllocated, &stripFlags))
		{
			g_pFullFileSystem->Close(fileHandle);
			return HandleFileLoadFailedTexture(pVTFTexture);
		}
	...
	if((m_nFlags & TEXTUREFLAGS_STREAMABLE_FINE) == 0)
	{
		...
		Init(m_dimsMapping.m_nWidth, m_dimsMapping.m_nHeight, m_dimsMapping.m_nDepth, pVTFTexture->Format(),
			 pVTFTexture->Flags() | nPreserveFlags, pVTFTexture->FrameCount());
	...
	if(unsigned int uiRsrcCount = pVTFTexture->GetResourceTypes(NULL, 0))
	{
		...
		for(uint32 *arrRsrcTypesEnd = arrRsrcTypes + uiRsrcCount; arrRsrcTypes < arrRsrcTypesEnd; ++arrRsrcTypes)
		{
			switch(*arrRsrcTypes)
			{
	...
	if(ConvertToActualFormat(pVTFTexture))
		pVTFTexture; // STAGING_ONLY_EXEC ( Warning( "\"%s\" not in final format, this is causing stutters or load time
					 // bloat!\n", pCacheFileName ) );
...
IVTFTexture *CTexture::HandleFileLoadFailedTexture(IVTFTexture *pVTFTexture)
{
	...
	Init(pVTFTexture->Width(), pVTFTexture->Height(), pVTFTexture->Depth(), pVTFTexture->Format(), pVTFTexture->Flags(),
	...
	for(int iFrame = 0; iFrame < m_nFrameCount; ++iFrame)
	{
		...
		for(int iFace = 0; iFace < nFaceCount; ++iFace)
		{
			for(int iMip = 0; iMip < m_dimsActual.m_nMipCount; ++iMip)
			{
				...
				unsigned char *pBits =
					pVTFTexture->ImageData(iFrame, iFace + nFirstFace, iMip, mipRect.x, mipRect.y, 0);
				g_pShaderAPI->TexSubImage2D(iMip, iFace, mipRect.x, mipRect.y, 0, mipRect.width, mipRect.height,
...
void CTexture::WriteDataToShaderAPITexture(int nFrameCount, int nFaceCount, int nFirstFace, int nMipCount,
										   IVTFTexture *pVTFTexture, ImageFormat fmt)
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
...
{
	return ((m_ImageFormat == IMAGE_FORMAT_NV_DST16) || (m_ImageFormat == IMAGE_FORMAT_NV_DST24) ||
			(m_ImageFormat == IMAGE_FORMAT_NV_INTZ) || (m_ImageFormat == IMAGE_FORMAT_NV_RAWZ) ||
			(m_ImageFormat == IMAGE_FORMAT_ATI_DST16) || (m_ImageFormat == IMAGE_FORMAT_ATI_DST24));
}
...
void CTexture::ReconstructTexture(bool bCopyFromCurrent)
{
...

	{
		...
		else
		{
			...
			// NOTE: Reloading the texture bits can cause the texture size, frames, format, pretty much *anything* can
			// change.
			pVTFTexture = LoadTextureBitsFromFile(pCacheFileName, &pResolvedFilename);
		}
	}

	if(!HasBeenAllocated() || m_dimsAllocated.m_nWidth != oldWidth || m_dimsAllocated.m_nHeight != oldHeight ||
	   m_dimsAllocated.m_nDepth != oldDepth || m_dimsAllocated.m_nMipCount != oldMipCount ||
	   m_nFrameCount != oldFrameCount)
	{
		...
		const bool cbShouldMigrateTextures =
			((m_nFlags & TEXTUREFLAGS_STREAMABLE_FINE) != 0) && m_nFrameCount == oldFrameCount;
		...
		else
		{
			...
			// If we're doing a wholesale copy, we need to restore these values that will be cleared by
			// FreeShaderAPITextures. Record them here, restore them below.
	...
	if(IsRenderTarget())
	{
		...
		if(!IsDepthTextureFormat(m_ImageFormat))
		{
			...
			g_pShaderAPI->ClearBuffers(true, false, false, m_dimsActual.m_nWidth,
									   m_dimsActual.m_nHeight); // Clear the target
	...
	WriteDataToShaderAPITexture(m_nFrameCount, nFaceCount, nFirstFace, m_dimsActual.m_nMipCount, pVTFTexture,
								m_ImageFormat);
...
bool CTexture::GetFileHandle(FileHandle_t *pOutFileHandle, char *pCacheFileName, char **ppResolvedFilename) const
{
	...
	{
		fileHandle = g_pFullFileSystem->OpenEx(pCacheFileName, "rb", 0, MaterialSystem()->GetForcedTextureLoadPathID(),
											   ppResolvedFilename);
...
ShaderAPITextureHandle_t CTexture::GetTextureHandle(int nFrame, int nTextureChannel)
{
	...
	{
		// NOTE: This can happen during alt-tab.  If you alt-tab while loading a level then the first local cubemap bind
...
void CTexture::GetLowResColorSample(float s, float t, float *color) const
{
	...
	{
		//		Warning( "Programming error: GetLowResColorSample \"%s\": %dx%d\n", m_pName, ( int )m_LowResImageWidth, (
		//int )m_LowResImageHeight );
		return;
	}
	...
	sColor[1][0] =
		(1.0f - fracS) * (m_pLowResImage[(wholeS + wholeTPlusOne * m_LowResImageWidth) * 3 + 0] * (1.0f / 255.0f));
	sColor[1][1] =
		(1.0f - fracS) * (m_pLowResImage[(wholeS + wholeTPlusOne * m_LowResImageWidth) * 3 + 1] * (1.0f / 255.0f));
	sColor[1][2] =
		(1.0f - fracS) * (m_pLowResImage[(wholeS + wholeTPlusOne * m_LowResImageWidth) * 3 + 2] * (1.0f / 255.0f));
	sColor[1][0] +=
		fracS * (m_pLowResImage[(wholeSPlusOne + wholeTPlusOne * m_LowResImageWidth) * 3 + 0] * (1.0f / 255.0f));
	sColor[1][1] +=
		fracS * (m_pLowResImage[(wholeSPlusOne + wholeTPlusOne * m_LowResImageWidth) * 3 + 1] * (1.0f / 255.0f));
	sColor[1][2] +=
		fracS * (m_pLowResImage[(wholeSPlusOne + wholeTPlusOne * m_LowResImageWidth) * 3 + 2] * (1.0f / 255.0f));
...
// Swap everything about a texture except the name. Created to support Portal mod's need for swapping out water render
// targets in recursive stencil views
...
bool SLoadTextureBitsFromFile(IVTFTexture **ppOutVtfTexture, FileHandle_t hFile, unsigned int nFlags,
							  TextureLODControlSettings_t *pInOutCachedFileLodSettings, int nDesiredDimensionLimit,
							  unsigned short *pOutStreamedMips, const char *pName, const char *pCacheFileName,
							  TexDimensions_t *pOptOutDimsMapping, TexDimensions_t *pOptOutDimsActual,
							  TexDimensions_t *pOptOutDimsAllocated, unsigned int *pOptOutStripFlags)
{
...

	{
		...
		int nBytesRead = g_pFullFileSystem->ReadEx(buf.Base(), nBytesOptimalRead,
												   Min(nHeaderSize, (int)g_pFullFileSystem->Size(hFile)),
												   hFile); // only read as much as the file has
	...
	TexDimensions_t dimsMappingCurrent((*ppOutVtfTexture)->Width(), (*ppOutVtfTexture)->Height(),
									   (*ppOutVtfTexture)->MipCount(), (*ppOutVtfTexture)->Depth());
	...
	int nMipSkipCount = ComputeMipSkipCount(pName, dimsMappingCurrent, false, *ppOutVtfTexture, nFullFlags,
											nDesiredDimensionLimit, pOutStreamedMips, pInOutCachedFileLodSettings,
											pOptOutDimsActual, pOptOutDimsAllocated, pOptOutStripFlags);
...
#ifdef DX_TO_GL_ABSTRACTION
	if((false && !g_bForceTextureAllMips && !(nFlags & TEXTUREFLAGS_ALL_MIPS)) ||
	   (true && (nFlags & TEXTUREFLAGS_BORDER)))
#else
	if((true && !g_bForceTextureAllMips && !(nFlags & TEXTUREFLAGS_ALL_MIPS)) ||
	   (false && (nFlags & TEXTUREFLAGS_BORDER)))
#endif
	{
		int nNumMipLevels = 1;
...
// ------------------------------------------------------------------------------------------------
int ComputeMipSkipCount(const char *pName, const TexDimensions_t &mappingDims, bool bIgnorePicmip,
						IVTFTexture *pOptVTFTexture, unsigned int nFlags, int nDesiredDimensionLimit,
						unsigned short *pOutStreamedMips, TextureLODControlSettings_t *pInOutCachedFileLodSettings,
						TexDimensions_t *pOptOutActualDims, TexDimensions_t *pOptOutAllocatedDims,
						unsigned int *pOptOutStripFlags)
{
	// NOTE! NOTE! NOTE! If you are making changes to this function, be aware that it has threading
	...
	{
		while(nClampX > nDesiredDimensionLimit || nClampY > nDesiredDimensionLimit)
		{
			nClampX >>= 1;
			nClampY >>= 1;
		}
	...
	if(!bIgnorePicmip)
	{
		...
		if(!(nFlags & TEXTUREFLAGS_NOLOD) && (g_config.skipMipLevels > 0))
		{
		...
		if(g_config.skipMipLevels < 0)
		{
			...
			{
				if(nClampX < actualDims.m_nWidth && nClampY < actualDims.m_nHeight)
				{
					nClampX <<= 1;
					nClampY <<= 1;
				}
	...
	while((actualDims.m_nWidth > nClampX) || (actualDims.m_nHeight > nClampY) || (actualDims.m_nDepth > nClampZ))
	{
	...
	if((nFlags & TEXTUREFLAGS_STREAMABLE) == TEXTUREFLAGS_STREAMABLE_COARSE)
	{
		if(actualDims.m_nMipCount > 1)
		{
			...
			for(int i = 0; i < STREAMING_START_MIPMAP; ++i)
			{
				...
				// but some textures really can't function if they're less than 4 pixels (compressed textures, for
				// example).
...
static bool SetBufferValue(char *chTxtFileBuffer, char const *szLookupKey, char const *szNewValue)
{
	...
	for(char *pch = chTxtFileBuffer; (NULL != (pch = strstr(pch, szLookupKey))); ++pch)
	{
...
static ConVar mat_texture_list_content_path("mat_texture_list_content_path", "", FCVAR_ARCHIVE,
											"The content path to the materialsrc directory. If left unset, it'll "
											"assume your content directory is next to the currently running game dir.");
	...
	FCVAR_DONTRECORD)
{
	...
	else if(!stricmp(szCmd, "save"))
	{
		...
		for(int k = 0; k < s_OverrideMap.GetNumStrings(); ++k)
		{
			...
			{
				szTextureContentPath = const_cast<char *>(g_pFullFileSystem->RelativePathToFullPath(
			...
			char *pExtPut = szTextureContentPath + strlen(szTextureContentPath) -
							strlen(TEXTURE_FNAME_EXTENSION); // compensating the TEXTURE_FNAME_EXTENSION(.vtf) extension
			...
			if(g_pFullFileSystem->FileExists(szTextureContentPath))
			{
				...
				for(int kCh = 0; kCh < 1024; ++kCh)
					bufTxtFileBuffer.PutChar(0);
