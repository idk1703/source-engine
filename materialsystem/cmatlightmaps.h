...
public:
	...
	int AllocateLightmap(int width, int height, int offsetIntoLightmapPage[2], IMaterial *pMaterial);
	...
	void UpdateLightmap(int lightmapPageID, int lightmapSize[2], int offsetIntoLightmapPage[2], float *pFloatImage,
						float *pFloatImageBump1, float *pFloatImageBump2, float *pFloatImageBump3);
	...
	int GetNumLightmapPages() const
	{
		return m_NumLightmapPages;
	}
...
private:
	...
	void LightmapBitsToPixelWriter_HDRF(float *pFloatImage, int pLightmapSize[2], int pOffsetIntoLightmapPage[2],
										FloatBitMap_t *pfmOut);
	void LightmapBitsToPixelWriter_HDRI(float *pFloatImage, int pLightmapSize[2], int pOffsetIntoLightmapPage[2],
										FloatBitMap_t *pfmOut);
	...
	enum LightmapsState
	{
		...
		STATE_RELEASED //	Lightmaps in released state - usually due to lost/released D3D device, D3D operations
					   //disallowed, Restore() call to follow
	};
