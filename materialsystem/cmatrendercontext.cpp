...
// FIXME: right now, always keeping shader API in sync, because debug overlays don't seem to work 100% with the delayed
// matrix loading
...
void CMatRenderContextBase::InitializeFrom(CMatRenderContextBase *pInitialState)
{
	...
	memcpy(m_pCurrentFrameBufferCopyTexture, pInitialState->m_pCurrentFrameBufferCopyTexture,
...
void CMatRenderContextBase::PerspectiveOffCenterX(double flFovX, double flAspect, double flZNear, double flZFar,
...
void CMatRenderContextBase::RecomputeViewState()
{
	...
	m_vecViewOrigin.x = -(viewMatrix[0][3] * viewMatrix[0][0] + viewMatrix[1][3] * viewMatrix[1][0] +
						  viewMatrix[2][3] * viewMatrix[2][0]);
	m_vecViewOrigin.y = -(viewMatrix[0][3] * viewMatrix[0][1] + viewMatrix[1][3] * viewMatrix[1][1] +
						  viewMatrix[2][3] * viewMatrix[2][1]);
	m_vecViewOrigin.z = -(viewMatrix[0][3] * viewMatrix[0][2] + viewMatrix[1][3] * viewMatrix[1][2] +
...
// The push and pop methods also implicitly set the render target to the new top of stack
//-----------------------------------------------------------------------------
void CMatRenderContextBase::PushRenderTargetAndViewport(ITexture *pTexture, int nViewX, int nViewY, int nViewW,
														int nViewH)
{
...
// The push and pop methods also implicitly set the render target to the new top of stack
//-----------------------------------------------------------------------------
void CMatRenderContextBase::PushRenderTargetAndViewport(ITexture *pTexture, ITexture *pDepthTexture, int nViewX,
														int nViewY, int nViewW, int nViewH)
{
		...
		else
		{
			if(goalScale < m_CurToneMapScale)
			{
				...
				rate = min((acc_exposure_adjust * rate),
						   FLerp(rate, (acc_exposure_adjust * rate), 0.0f, 1.5f, (m_CurToneMapScale - goalScale)));
			}
			...
			if(mat_tonemap_algorithm.GetInt() == 1)
			{
				...
				flRateTimesTime =
					min(flRateTimesTime,
						(1.0f / 16.0f) * 0.25f); // 16 is number of HDR sample bins defined in viewpostprocess.cpp
												 // Warning( " --> %.4f\n", flRateTimesTime );
			}
...
void CMatRenderContextBase::OnAsyncCreateTextureFromRenderTarget(ITexture *pSrcRt, const char **ppDstName,
...
void CMatRenderContextBase::OnAsyncMap(ITextureInternal *pTexToMap, IAsyncTextureOperationReceiver *pRecipient,
									   void *pExtraArgs)
{
...
void CMatRenderContextBase::OnAsyncCopyRenderTargetToStagingTexture(ITexture *pDst, ITexture *pSrc,
																	IAsyncTextureOperationReceiver *pRecipient)
{
	Assert(pDst != NULL);
	Assert(pSrc != NULL);
...
void CMatRenderContext::PerspectiveOffCenterX(double flFovX, double flAspect, double flZNear, double flZFar,
											  double bottom, double top, double left, double right)
{
...
void CMatRenderContext::PushCustomClipPlane(const float *pPlane)
{
	...
	m_CustomClipPlanes.AddToTail(
		psePlane); // we're doing this as a UtlVector so height clip planes never change their cached index
	ApplyCustomClipPlanes();
}
...
IMesh *CMatRenderContext::GetDynamicMesh(bool buffered, IMesh *pVertexOverride, IMesh *pIndexOverride,
										 IMaterial *pAutoBind)
{
	...
	return g_pShaderAPI->GetDynamicMesh(GetCurrentMaterialInternal(), nCurrentBoneCount, buffered, pVertexOverride,
										pIndexOverride);
}

IMesh *CMatRenderContext::GetDynamicMeshEx(VertexFormat_t vertexFormat, bool bBuffered, IMesh *pVertexOverride,
										   IMesh *pIndexOverride, IMaterial *pAutoBind)
{
	VPROF_ASSERT_ACCOUNTED("CMatRenderContext::GetDynamicMesh");
	if(pAutoBind)
	{
	...
	return g_pShaderAPI->GetDynamicMeshEx(GetCurrentMaterialInternal(), vertexFormat, nCurrentBoneCount, bBuffered,
...
void CMatRenderContext::SetNonInteractivePacifierTexture(ITexture *pTexture, float flNormalizedX, float flNormalizedY,
...
void CMatRenderContext::EnableNonInteractiveMode(MaterialNonInteractiveMode_t mode)
{
	...
	else
	{
		...
		ITextureInternal *pTexInternal =
			static_cast<ITextureInternal *>((ITexture *)m_pNonInteractiveTempFullscreenBuffer[mode]);
		info.m_hTempFullscreenTexture =
			pTexInternal ? pTexInternal->GetTextureHandle(0) : INVALID_SHADERAPI_TEXTURE_HANDLE;
...
void CMatRenderContext::DrawScreenSpaceRectangle(IMaterial *pMaterial, int destx, int desty, int width, int height,
												 ...
												 int nYDice) // Amount to tessellate the quad
{
	...
	::DrawScreenSpaceRectangle(pMaterial, destx, desty, width, height, src_texture_x0, src_texture_y0, src_texture_x1,
							   src_texture_y1, src_texture_width, src_texture_height, pClientRenderable, nXDice,
							   nYDice);
	return;
}
...
int CMatRenderContext::CompareMaterialCombos(IMaterial *pMaterial1, IMaterial *pMaterial2, int lightMapID1,
											 int lightMapID2)
{
	pMaterial1 = ((IMaterialInternal *)pMaterial1)
					 ->GetRealTimeVersion(); // always work with the real time version of materials internally.
	pMaterial2 = ((IMaterialInternal *)pMaterial2)
					 ->GetRealTimeVersion(); // always work with the real time version of materials internally.
	...
	for(int i = 0; i < pState1->m_pSnapshots->m_nPassCount; i++)
	{
		...
		if(dSnapshot)
		{
			dSnapshot = g_pShaderAPI->CompareSnapshots(pState1->m_pSnapshots->m_Snapshot[i],
	...
	if(nBaseTexParamType1 == MATERIAL_VAR_TYPE_TEXTURE || nBaseTexParamType2 == MATERIAL_VAR_TYPE_TEXTURE)
	{
		...
		int dBaseTexture = Q_stricmp(pParams1[BASETEXTURE]->GetTextureValue()->GetName(),
...
void CMatRenderContext::Bind(IMaterial *iMaterial, void *proxyData)
{
	...
	if(material->GetReferenceCount() <= 0)
	{
		   ...
		   (!matShowWaterTextureConVar.IsValid() || !matShowWaterTextureConVar.GetBool()))
		{
			...
			// The usual solution for this for global materials that really don't need refcounting is to do
			// material->AddRef();
			Assert(0);
			iMaterial = g_pErrorMaterial;
		}
...
void CMatRenderContext::CopyRenderTargetToTextureEx(ITexture *pTexture, int nRenderTargetID, Rect_t *pSrcRect,
													Rect_t *pDstRect)
{
	if(!pTexture)
	{
	...
	else
	{
		...
		if((pDstRect->x == 0) && (pDstRect->y == 0) && (pDstRect->width == pTexture->GetActualWidth()) &&
		   (pDstRect->height == pTexture->GetActualHeight()) && (pDstRect->width >= pSrcRect->width) &&
		   (pDstRect->height >= pSrcRect->height))
		{
			// Resolve takes up the whole texture, and the texture is large enough to hold the resolve.
			// This is turned into a 1:1 resolve within shaderapi by making D3D think the texture is smaller from now
			// on. (Until it resolves from a bigger source)
			pTextureInternal->CopyFrameBufferToMe(nRenderTargetID, pSrcRect, pDstRect);
			return;
		}
		...
		ITextureInternal *pFullFrameFB =
			(ITextureInternal *)GetMaterialSystem()->FindTexture("_rt_FullFrameFB", TEXTURE_GROUP_RENDER_TARGET);
		...
		DrawScreenSpaceRectangle(GetMaterialSystem()->GetRenderTargetBlitMaterial(), pDstRect->x, pDstRect->y,
								 pDstRect->width, pDstRect->height, pSrcRect->x, pSrcRect->y,
								 pSrcRect->x + pSrcRect->width - 1, pSrcRect->y + pSrcRect->height - 1,
...
void CMatRenderContext::CopyTextureToRenderTargetEx(int nRenderTargetID, ITexture *pTexture, Rect_t *pSrcRect,
													Rect_t *pDstRect)
{
	if(!pTexture)
	{
...
void CMatRenderContext::DrawClearBufferQuad(unsigned char r, unsigned char g, unsigned char b, unsigned char a,
											bool bClearColor, bool bClearAlpha, bool bClearDepth)
{
	IMaterialInternal *pClearMaterial =
		GetBufferClearObeyStencil(bClearColor + (bClearAlpha << 1) + (bClearDepth << 2));
	Bind(pClearMaterial);
...
void CMatRenderContext::BindLightmap(Sampler_t sampler)
{
	...
	{
		default:
			Assert((m_lightmapPageID == 0 && GetLightmaps()->GetNumLightmapPages() == 0) ||
				   (m_lightmapPageID >= 0 && m_lightmapPageID < GetLightmaps()->GetNumLightmapPages()));
	...
	switch(id)
	{
		...
		case TEXTURE_COLOR_CORRECTION_VOLUME_3:
		{
			ITextureInternal *pTexture =
				TextureManager()->ColorCorrectionTexture(id - TEXTURE_COLOR_CORRECTION_VOLUME_0);
			if(pTexture)
			{
...
void CMatRenderContext::SetFlashlightStateEx(const FlashlightState_t &state, const VMatrix &worldToTexture,
...
void CMatRenderContext::SetScissorRect(const int nLeft, const int nTop, const int nRight, const int nBottom,
...
void CMatRenderContext::AsyncCreateTextureFromRenderTarget(ITexture *pSrcRt, const char *pDstName, ImageFormat dstFmt,
														   ...
														   IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs)
{
	...
	TextureManager()->AsyncCreateTextureFromRenderTarget(pSrcRt, pDstName, dstFmt, bGenMips, nAdditionalCreationFlags,
														 pRecipient, pExtraArgs);
}

void CMatRenderContext::AsyncMap(ITextureInternal *pTexToMap, IAsyncTextureOperationReceiver *pRecipient,
								 void *pExtraArgs)
{
	if(g_pMaterialSystem->GetThreadMode() == MATERIAL_SINGLE_THREADED)
	{
...
void CMatRenderContext::AsyncCopyRenderTargetToStagingTexture(ITexture *pDst, ITexture *pSrc,
															  IAsyncTextureOperationReceiver *pRecipient,
															  void *pExtraArgs)
{
	if(g_pMaterialSystem->GetThreadMode() == MATERIAL_SINGLE_THREADED)
