...
// Does a texture have alpha?
//-----------------------------------------------------------------------------
static bool DoesTextureUseNormal(const char *pTextureName, const char *pMaterialName, bool &bUsesAlpha,
								 bool &bIsCompressed, int &nSizeInBytes)
{
	...
	else
	{
		if(pVTFTexture->Flags() & TEXTUREFLAGS_NORMAL)
		{
			...
			bUsesNormal = true;
			bIsCompressed =
				ImageLoader::IsCompressed(pVTFTexture->Format()) || (pVTFTexture->Format() == IMAGE_FORMAT_A8);
...
static bool MaterialVectorKeyValue(KeyValues *pKeyValues, const char *pKeyName, int nDefaultDim, float *pDefault,
								   int *pDim, float *pVector)
{
	int nDim;
	float retVal[4];
	...
	/*
		...
		if ( pBaseTextureName != NULL )
		{
			if ( DoesTextureUseAlpha( pBaseTextureName, pFileName ) )
			{
				...
				bool bHasVertexAlpha = DoesMaterialHaveKey( pKeyValues, "$vertexalpha" );	// Modulation always happens
	   here whether we want it to or not bool bHasAlphaTest = DoesMaterialHaveKey( pKeyValues, "$alphatest" ); bool
	   bHasTranslucent = DoesMaterialHaveKey( pKeyValues, "$translucent" ); bool bHasSelfIllum = DoesMaterialHaveKey(
	   pKeyValues, "$selfillum" ); bool bHasBaseAlphaEnvMapMask = DoesMaterialHaveKey( pKeyValues,
	   "$basealphaenvmapmask" ); if ( (flAlpha == 1.0f) && !bHasVertexAlpha && !bHasAlphaTest && !bHasTranslucent &&
	   !bHasSelfIllum && !bHasBaseAlphaEnvMapMask )
				{
...
//	Msg( "Normal only %d/%d/%d Normal w alpha %d/%d\n", s_nNormalBytes, s_nNormalPalettizedBytes,
//s_nNormalCompressedBytes, s_nNormalWithAlphaBytes, s_nNormalWithAlphaCompressedBytes );
#undef BUF_SIZE
}
