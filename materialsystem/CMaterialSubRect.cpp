...
public:
	...
	CMaterialSubRect(char const *pMaterialName, const char *pTextureGroupName, KeyValues *pVMTKeyValues,
					 KeyValues *pPatchKeyValues, bool bAssumeCreateFromFile);
	...
	{
		return m_pMaterialPage->UsesEnvCubemap();
	}
	...
	{
		return m_pMaterialPage->NeedsTangentSpace();
	}
	bool NeedsPowerOfTwoFrameBufferTexture(bool bCheckSpecificToThisFrame = true)
	{
		return m_pMaterialPage->NeedsPowerOfTwoFrameBufferTexture(bCheckSpecificToThisFrame);
	}
	bool NeedsFullFrameBufferTexture(bool bCheckSpecificToThisFrame = true)
	{
		return m_pMaterialPage->NeedsFullFrameBufferTexture(bCheckSpecificToThisFrame);
	}
	...
	{
		m_pMaterialPage->AlphaModulate(alpha);
	}
	...
	{
		return m_pMaterialPage->GetAlphaModulation();
	}
	...
	bool IsTranslucent()
	{
		return m_pMaterialPage->IsTranslucent();
	}
	...
	bool IsAlphaTested()
	{
		return m_pMaterialPage->IsAlphaTested();
	}
	bool IsVertexLit()
	{
		return m_pMaterialPage->IsVertexLit();
	}
	...
	bool IsTwoSided()
	{
		return m_pMaterialPage->IsTwoSided();
	}

	int GetNumPasses(void)
	{
		return m_pMaterialPage->GetNumPasses();
	}
	...
	{
		m_pMaterialPage->ReloadTextures();
	}
	...
	{
		return m_pMaterialPage->GetShader();
	}
	void CallBindProxy(void *proxyData)
	{
		m_pMaterialPage->CallBindProxy(proxyData);
	}
	IMaterial *CheckProxyReplacement(void *proxyData)
	{
		return m_pMaterialPage->CheckProxyReplacement(proxyData);
	}
	bool HasProxy(void) const
	{
		return m_pMaterialPage->HasProxy();
	}
	...
	{
		return m_pMaterialPage->IsSuppressed();
	}

	// Do we use fog?
	bool UseFog(void) const
	{
		return m_pMaterialPage->UseFog();
	}

	// Should we draw?
	void ToggleSuppression()
	{
		m_pMaterialPage->ToggleSuppression();
	}
	void ToggleDebugTrace()
	{
		m_pMaterialPage->ToggleDebugTrace();
	}

	// Refresh material based on current var values
	void Refresh()
	{
		m_pMaterialPage->Refresh();
	}
	...
	{
		m_pMaterialPage->RecomputeStateSnapshots();
	}

	// Gets at the shader parameters
	int ShaderParamCount() const
	{
		return m_pMaterialPage->ShaderParamCount();
	}
	...
	{
		return m_pMaterialPage->IsUsingVertexID();
	}
...
{
	return new CMaterialSubRect(pMaterialName, pTextureGroupName, pVMTKeyValues, pPatchKeyValues,
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMaterialSubRect::CMaterialSubRect(const char *pMaterialName, const char *pTextureGroupName, KeyValues *pVMTKeyValues,
								   KeyValues *pPatchKeyValues, bool bAssumeCreateFromFile)
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMaterialSubRect::PrecacheVars(KeyValues *pVMTKeyValues, KeyValues *pPatchKeyValues,
...
void CMaterialSubRect::ParseMaterialVars(KeyValues &keyValues)
{
	...
	if(!Q_stricmp(pShaderName, "Subrect"))
	{
		...
		while(pVar)
		{
			...
			{
				m_pMaterialPage = static_cast<IMaterialInternal *>(
					MaterialSystem()->FindMaterial(pVar->GetString(), TEXTURE_GROUP_DECAL));
				m_pMaterialPage =
					m_pMaterialPage->GetRealTimeVersion(); // always work with the realtime material internally
			}
