		...
		for(int iFrame = 0; iFrame < pVTFTexture->FrameCount(); ++iFrame)
		{
			for(int iFace = 0; iFace < pVTFTexture->FaceCount(); ++iFace)
			{
				...
				for(int z = 0; z < nDepth; ++z)
				{
					...
					pixelWriter.SetPixelMemory(pVTFTexture->Format(), pVTFTexture->ImageData(iFrame, iFace, 0, 0, 0, z),
...
public:
	virtual void RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pSubRect)
	{
		...
		{
			pixelWriter.SetPixelMemory(pVTFTexture->Format(), pVTFTexture->ImageData(0, iFace, 0),
									   pVTFTexture->RowSizeInBytes(0));
...
public:
	...
	virtual void RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pSubRect)
	{
		...
		int nHeight = pVTFTexture->Height();
		int nDepth = pVTFTexture->Depth();
		Assert(nWidth == COLOR_CORRECTION_TEXTURE_SIZE && nHeight == COLOR_CORRECTION_TEXTURE_SIZE &&
			   nDepth == COLOR_CORRECTION_TEXTURE_SIZE);

		for(int z = 0; z < nDepth; ++z)
		{
			CPixelWriter pixelWriter;
			pixelWriter.SetPixelMemory(pVTFTexture->Format(), pVTFTexture->ImageData(0, 0, 0, 0, 0, z),
									   pVTFTexture->RowSizeInBytes(0));
...
public:
	...
	virtual ITextureInternal *CreateProceduralTexture(const char *pTextureName, const char *pTextureGroupName, int w,
													  int h, int d, ImageFormat fmt, int nFlags,
	...
	virtual void AsyncFindOrLoadTexture(const char *pTextureName, const char *pTextureGroupName,
...
protected:
	...
	ITextureInternal *LoadTexture(const char *textureName, const char *pTextureGroupName,
								  int nAdditionalCreationFlags = 0, bool bDownload = true);
...
struct AsyncLoadJob_t
{
	...
	AsyncLoadJob_t()
		: m_pRecipient(NULL), m_pExtraArgs(NULL), m_bComplain(false), m_nAdditionalCreationFlags(0), m_pResultData(NULL)
	{
	}

	AsyncLoadJob_t(const char *pTextureName, const char *pTextureGroupName, IAsyncTextureOperationReceiver *pRecipient,
				   void *pExtraArgs, bool bComplain, int nAdditionalCreationFlags)
...
{
public:
	CAsyncCopyRequest() : m_nReferenceCount(0), m_bSignalled(false) {}
...
public:
	CAsyncMapResult(ITextureInternal *pTex)
		: m_pTexToMap(pTex), m_nReferenceCount(0), m_pMemory(NULL), m_nPitch(0), m_bSignalled(false)
	{
	}
...
struct AsyncReadJob_t
{
	...
	AsyncReadJob_t(ITexture *pSrcRt, const char *pDstName, ImageFormat dstFmt, bool bGenMips,
				   int nAdditionalCreationFlags, IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs)
		: m_pSrcRt(pSrcRt),
		  m_pSysmemTex(NULL),
		  m_pAsyncRead(NULL),
		  m_pAsyncMap(NULL),
...
public:
	...
	void ThreadMain_Update()
	{
		...
		while(!m_queuedMaps.IsEmpty())
		{
			...
			Assert(pMapped != NULL);
			{
				...
				if(pMapped->m_pAsyncMap->IsSignalled())
				{
					...
					{
						Assert(
							!"Failed to perform a map that shouldn't fail, need to deal with this if it ever happens.");
						DevWarning(
							"Failed to perform a map that shouldn't fail, need to deal with this if it ever happens.");
					}
		...
		if(m_requestedCopies.PopItem(&pRequestCopy))
		{
			...
			extern CMaterialSystem g_MaterialSystem;
			g_MaterialSystem.GetRenderContextInternal()->AsyncCopyRenderTargetToStagingTexture(
...
private:
	...
	void ThreadReader_ProcessRead(AsyncReadJob_t *pJob)
	{
...

		{
			...
			ImageLoader::ConvertImageFormat((unsigned char *)pJob->m_pAsyncMap->m_pMemory,
											GetImageFormatRawReadback(srcFmt), srcBufferFinestMip.Base(), srcFmt, w, h);
		}
	...
	void ConvertTexelData(CUtlMemory<unsigned char> *outBuffer, ImageFormat dstFmt,
						  /* const */ CUtlMemory<unsigned char> &inBuffer, int w, int h, ImageFormat srcFmt,
						  bool bGenMips)
	{
		tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
...
void CTextureManager::Init(int nFlags)
{
	...
	m_pErrorTexture = CreateProceduralTexture("error", TEXTURE_GROUP_OTHER, ERROR_TEXTURE_SIZE, ERROR_TEXTURE_SIZE, 1,
	...
	m_pWhiteTexture = CreateProceduralTexture("white", TEXTURE_GROUP_OTHER, WHITE_TEXTURE_SIZE, WHITE_TEXTURE_SIZE, 1,
											  IMAGE_FORMAT_BGRX8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
	color.r = color.g = color.b = color.a = 255;
	CreateSolidTexture(m_pWhiteTexture, color);

	// Create a black texture
	m_pBlackTexture = CreateProceduralTexture("black", TEXTURE_GROUP_OTHER, BLACK_TEXTURE_SIZE, BLACK_TEXTURE_SIZE, 1,
											  IMAGE_FORMAT_BGRX8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
	color.r = color.g = color.b = 0;
	CreateSolidTexture(m_pBlackTexture, color);

	// Create a grey texture
	m_pGreyTexture = CreateProceduralTexture("grey", TEXTURE_GROUP_OTHER, GREY_TEXTURE_SIZE, GREY_TEXTURE_SIZE, 1,
											 IMAGE_FORMAT_BGRA8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
	...
	// Create a grey texture
	m_pGreyAlphaZeroTexture =
		CreateProceduralTexture("greyalphazero", TEXTURE_GROUP_OTHER, GREY_TEXTURE_SIZE, GREY_TEXTURE_SIZE, 1,
								IMAGE_FORMAT_BGRA8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
	color.r = color.g = color.b = 128;
	color.a = 0;
	...
	if(HardwareConfig()->GetMaxDXSupportLevel() >= 80)
	{
		...
		m_pNormalizationCubemap =
			CreateProceduralTexture("normalize", TEXTURE_GROUP_CUBE_MAP, NORMALIZATION_CUBEMAP_SIZE,
	...
	if(HardwareConfig()->GetMaxDXSupportLevel() >= 90)
	{
		...
		int nTextureFlags = TEXTUREFLAGS_ENVMAP | TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_SINGLECOPY |
							TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_CLAMPU;
		...
		m_pSignedNormalizationCubemap =
			CreateProceduralTexture("normalizesigned", TEXTURE_GROUP_CUBE_MAP, NORMALIZATION_CUBEMAP_SIZE,
									NORMALIZATION_CUBEMAP_SIZE, 1, fmt, nTextureFlags);
	...
	if(IsPC() && (bAllocateFullscreenTexture || bAllocateMorphAccumTexture))
	{
		...
		if(bAllocateFullscreenTexture)
		{
			m_pFullScreenTexture = CreateRenderTargetTexture(
				"_rt_FullScreen", 1, 1, RT_SIZE_FULL_FRAME_BUFFER_ROUNDED_UP, MaterialSystem()->GetBackBufferFormat(),
				RENDER_TARGET, TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT, 0);
...
{
	m_pFullScreenDepthTexture = FindTexture(
		"_rt_FullFrameDepth"); // created/destroyed in engine/matsys_interface.cpp to properly track hdr changes
}
...
void CTextureManager::CleanupPossiblyUnreferencedTextures()
{
	...
	CUtlVector<ITextureInternal *> texturesToDelete(/* growSize */ 0,
													/* initialSize */ m_PossiblyUnreferencedTextures.Count());
	...
	while(m_PossiblyUnreferencedTextures.PopItem(&pMaybeUnreferenced))
	{
		...
		if(pMaybeUnreferenced->GetReferenceCount() == 0 &&
		   texturesToDelete.Find(pMaybeUnreferenced) == texturesToDelete.InvalidIndex())
		{
...
void CTextureManager::ForceAllTexturesIntoHardware(void)
{
	...
	pMaterial =
		((IMaterialInternal *)pMaterial)->GetRealTimeVersion(); // always work with the realtime material internally
...
//-----------------------------------------------------------------------------
ITextureInternal *CTextureManager::CreateProceduralTexture(const char *pTextureName, const char *pTextureGroupName,
														   int w, int h, int d, ImageFormat fmt, int nFlags,
														   ITextureRegenerator *generator)
{
	ITextureInternal *pNewTexture =
		ITextureInternal::CreateProceduralTexture(pTextureName, pTextureGroupName, w, h, d, fmt, nFlags, generator);
	if(!pNewTexture)
		return NULL;
		...
		if(iLightMapNum >= 0 && iLightMapNum < plm->GetNumLightmapPages())
		{
			...
			if(hTex != INVALID_SHADERAPI_TEXTURE_HANDLE)
			{
				...
				ITextureInternal *pTxInt =
					ITextureInternal::CreateReferenceTextureFromHandle(pTextureName, TEXTURE_GROUP_LIGHTMAP, hTex);
															 ...
															 unsigned int textureFlags, unsigned int renderTargetFlags)
{
	...
	pTexture =
		ITextureInternal::CreateRenderTarget(pRTName, w, h, sizeMode, fmt, type, textureFlags, renderTargetFlags);
	if(!pTexture)
		return NULL;
...
void CTextureManager::RemoveTexture(ITextureInternal *pTexture)
{
	...
	for(int i = m_TextureList.First(); i != m_TextureList.InvalidIndex(); i = m_TextureList.Next(i))
	{
		...
		{
			// This code is always sure that the texture we're tryign to clean up is no longer in the the possibly
			// unreferenced list, So let Destroy work without checking.
			ITextureInternal::Destroy(m_TextureList[i], true);
			m_TextureList.RemoveAt(i);
			break;
		}
...
// Load a texture asynchronously and then call the provided callback.
void CTextureManager::AsyncFindOrLoadTexture(const char *pTextureName, const char *pTextureGroupName,
											 IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs,
											 bool bComplain, int nAdditionalCreationFlags)
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
	...
	AsyncLoadJob_t asyncLoad(pTextureName, pTextureGroupName, pRecipient, pExtraArgs, bComplain,
							 nAdditionalCreationFlags);

	// If this is the first person asking to load this, then remember so we don't load the same thing over and over
	// again.
	int pendingIndex = m_PendingAsyncLoads.Find(pTextureName);
	if(pendingIndex == m_PendingAsyncLoads.InvalidIndex())
	{
		// Create the texture here, we'll load the data in the async thread. Load is a misnomer, because it doesn't
		// actually load the data--Download does.
		if(bStreamingRequest)
...
void CTextureManager::AsyncCreateTextureFromRenderTarget(ITexture *pSrcRt, const char *pDstName, ImageFormat dstFmt,
														 bool bGenMips, int nAdditionalCreationFlags,
														 IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs)
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
	...
	AsyncReadJob_t *pAsyncRead =
		new AsyncReadJob_t(pSrcRt, pDstName, dstFmt, bGenMips, nAdditionalCreationFlags, pRecipient, pExtraArgs);
...
void CTextureManager::CompleteAsyncRead(AsyncReadJob_t *pJob)
{
	...
	int nFlags = pJob->m_nAdditionalCreationFlags | TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_IGNORE_PICMIP |
				 (mips > 1 ? TEXTUREFLAGS_ALL_MIPS : TEXTUREFLAGS_NOMIP);

	// Create the texture
	ITexture *pFinalTex = materials->CreateNamedTextureFromBitsEx(
		pJob->m_pDstName, TEXTURE_GROUP_RUNTIME_COMPOSITE, w, h, mips, pJob->m_dstFmt, pJob->m_finalTexelData.Count(),
		pJob->m_finalTexelData.Base(), nFlags);
	Assert(pFinalTex);
...
void CTextureManager::AsyncReadTexture(AsyncReadJob_t *pJob)
{
	...
	pJob->m_pSysmemTex = AcquireReadbackTexture(pJob->m_pSrcRt->GetActualWidth(), pJob->m_pSrcRt->GetActualHeight(),
												pJob->m_pSrcRt->GetImageFormat());
...
ITextureInternal *CTextureManager::AcquireReadbackTexture(int w, int h, ImageFormat fmt)
{
...

	{
		...
		FOR_EACH_VEC(m_ReadbackTextures, i)
		{
			...
			if(pTex->GetActualWidth() == w && pTex->GetActualHeight() == h && pTex->GetImageFormat() == fmt)
			{
	...
	ITextureInternal *stagingTex = CreateProceduralTexture(
		"readbacktex", TEXTURE_GROUP_OTHER, w, h, 1, fmt,
		TEXTUREFLAGS_STAGING_MEMORY | TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_IMMEDIATE_CLEANUP);
...
void CTextureManager::WarmTextureCache()
{
	...
	FOR_EACH_DICT(filesToLoad, i)
	{
		...
		{
			STAGING_ONLY_EXEC(Warning("%s is listed in texture_preload_list.txt or is otherwise marked for streaming. "
									  "It cannot be streamed and should be removed from the streaming system.\n",
									  pFilename));
...
void CTextureManager::FindFilesToLoad(CUtlDict<int> *pOutFilesToLoad, const char *pFilename)
{
	...
	while(pFilename != NULL)
	{
		...
		else
		{
			...
			// Add the file to the list, which we will later traverse in order to ensure we're hitting these in the
			// expected order for the VPK.
...
static ImageFormat GetImageFormatRawReadback(ImageFormat fmt)
{
	switch(fmt)
	{
		...
		default:
			Assert(
				!"Unsupported format in GetImageFormatRawReadback, this will likely result in color-swapped textures");
	};
