...
public:
	...
	IMesh *GetActualMesh()
	{
		return m_pActualMesh;
	}
	int GetActualVertexOffsetInBytes()
	{
		return m_nActualVertexOffsetInBytes;
	}

	void DeferredGetDynamicMesh(VertexFormat_t vertexFormat, unsigned flags, IMesh *pVertexOverride,
								IMesh *pIndexOverride, IMaterialInternal *pMaterial)
	{
		if(!(flags & MQM_FLEX))
		{
			if(vertexFormat == 0)
			{
				m_pActualMesh = m_pHardwareContext->GetDynamicMesh(((flags & MQM_BUFFERED) != 0), pVertexOverride,
																   pIndexOverride, pMaterial);
			}
			else
			{
				m_pActualMesh = m_pHardwareContext->GetDynamicMeshEx(vertexFormat, ((flags & MQM_BUFFERED) != 0),
																	 pVertexOverride, pIndexOverride, pMaterial);
			}
		}
		else
		{
	...
	bool OnGetDynamicMesh(VertexFormat_t vertexFormat, unsigned flags, IMesh *pVertexOverride, IMesh *pIndexOverride,
						  IMaterialInternal *pMaterial, int nHWSkinBoneCount)
	{
		...
		if(!(flags & MQM_FLEX))
		{
			...
			else
			{
				...
				m_VertexFormat =
					(vertexFormat == 0) ? (pMaterial->GetVertexFormat() & ~VERTEX_FORMAT_COMPRESSED) : vertexFormat;
		...
		m_pCallQueue->QueueCall(this, &CMatQueuedMesh::DeferredGetDynamicMesh, vertexFormat, flags, pVertexOverride,
								pIndexOverride, pMaterial);
		return true;
	}
	...
	void CopyToMeshBuilder(int iStartVert, int nVerts, int iStartIndex, int nIndices, int indexOffset,
...
#undef FindMin
		...
		if(pIndexData && pIndexData != &gm_ScratchIndexBuffer[0] && desc.m_nIndexSize)
		{
			...
			{
				// AssertMsg(desc.m_pIndices & 0x03 == 0,"desc.m_pIndices is misaligned in
				// CMatQueuedMesh::ExecuteDefferedBuild\n");
	...
	{
		m_pCallQueue->QueueCall(this, &CMatQueuedMesh::ExecuteDefferredBuild, m_pVertexData, m_nVerts,
								m_nVerts * m_VertexSize, m_pIndexData, m_nIndices);
		if(bDetachBuffers)
		{
			DetachBuffers();
			m_Type = MATERIAL_TRIANGLES;
...
void CMatQueuedRenderContext::Bind(IMaterial *iMaterial, void *proxyData)
{
	...
	else
	{
		iMaterial = ((IMaterialInternal *)iMaterial)
						->GetRealTimeVersion(); // always work with the real time versions of materials internally
	}
...
void CMatQueuedRenderContext::CallQueued(bool bTermAfterCall)
{
	...
	{
		Msg("%d calls queued for %llu bytes in parameters and overhead, %d bytes verts, %d bytes indices, %d bytes "
			"other\n",
			m_queue.Count(), (uint64)(m_queue.GetMemoryUsed()), m_Vertices.GetUsed(), m_Indices.GetUsed(),
			RenderDataSizeUsed());
	}
...
{
	FogColor3ub(clamp((int)(rgb[0] * 255.0f), 0, 255), clamp((int)(rgb[1] * 255.0f), 0, 255),
...
//
//-----------------------------------------------------------------------------
IMesh *CMatQueuedRenderContext::GetDynamicMesh(bool buffered, IMesh *pVertexOverride, IMesh *pIndexOverride,
											   IMaterial *pAutoBind)
{
	if(pAutoBind)
		Bind(pAutoBind, NULL);
	...
	m_pQueuedMesh->OnGetDynamicMesh(0, (buffered) ? MQM_BUFFERED : 0, pVertexOverride, pIndexOverride,
									GetCurrentMaterialInternal(), nCurrentBoneCount);
	return m_pQueuedMesh;
}

IMesh *CMatQueuedRenderContext::GetDynamicMeshEx(VertexFormat_t vertexFormat, bool bBuffered, IMesh *pVertexOverride,
												 IMesh *pIndexOverride, IMaterial *pAutoBind)
{
	...
	m_pQueuedMesh->OnGetDynamicMesh(vertexFormat, (bBuffered) ? MQM_BUFFERED : 0, pVertexOverride, pIndexOverride,
									GetCurrentMaterialInternal(), nCurrentBoneCount);
	return m_pQueuedMesh;
}
...
void CMatQueuedRenderContext::DrawScreenSpaceRectangle(
	IMaterial *pMaterial, int destx, int desty, int width, int height, float src_texture_x0,
	...
	void *pClientRenderable, int nXDice, int nYDice) // Amount to tessellate the quad
{
	...
	m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::DrawScreenSpaceRectangle, pMaterial, destx, desty, width,
					  height, src_texture_x0, src_texture_y0, src_texture_x1, src_texture_y1, src_texture_width,
...
{
	m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::CopyRenderTargetToTextureEx, pTexture, i,
					  CUtlEnvelope<Rect_t>(pSrc), CUtlEnvelope<Rect_t>(pDst));
}
...
{
	m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::CopyTextureToRenderTargetEx, i, pTexture,
					  CUtlEnvelope<Rect_t>(pSrc), CUtlEnvelope<Rect_t>(pDst));
}
...
inline void CMatQueuedRenderContext::QueueMatrixSync()
{
	void (IMatRenderContext::*pfnLoadMatrix)(const VMatrix &) =
		&IMatRenderContext::LoadMatrix; // need assignment to disambiguate overloaded function
...
void CMatQueuedRenderContext::PerspectiveOffCenterX(double flFovX, double flAspect, double flZNear, double flZFar,
...
void CMatQueuedRenderContext::BeginBatch(IMesh *pIndices)
{
	...
	m_queue.QueueCall(this, &CMatQueuedRenderContext::DeferredBeginBatch, m_pQueuedMesh->GetIndexData(),
					  m_pQueuedMesh->IndexCount());
...
byte *CMatQueuedRenderContext::AllocVertices(int nVerts, int nVertexSize)
{
	...
	const byte *pNextAlloc =
		(const byte *)(m_Vertices.GetBase()) + m_Vertices.GetUsed() + AlignValue(nVerts * nVertexSize, 16);
	...
	if(!pResult)
	{
		...
		Msg("AllocVertices( %d, %d ) on %p failed. m_Vertices is based at %p with a size of 0x%x.\n", nVerts,
			nVertexSize, this, m_Vertices.GetBase(), m_Vertices.GetSize());
		Msg("%d vertices used.\n", m_Vertices.GetUsed());
		if(pNextAlloc > pCommitLimit)
		{
			...
			Msg("VirtualAlloc( %p, %d ) returned %p on repeat. VirtualAlloc %s with code %x.\n", pCommitLimit,
				commitSize, pRet, (pRet != NULL) ? "succeeded" : "failed", (uint32)status);
		}
		else
		{
...
uint16 *CMatQueuedRenderContext::AllocIndices(int nIndices)
{
...
#if defined(_WIN32) && !defined(_X360)
	const byte *pNextAlloc =
		(const byte *)(m_Indices.GetBase()) + m_Indices.GetUsed() + AlignValue(nIndices * sizeof(uint16), 16);
	...
	if(!pResult)
	{
		...
		Msg("AllocIndices( %d ) on %p failed. m_Indices is based at %p with a size of 0x%x.\n", nIndices, this,
			m_Indices.GetBase(), m_Indices.GetSize());
		Msg("%d indices used.\n", m_Indices.GetUsed());
		if(pNextAlloc > pCommitLimit)
		{
			Msg("VirtualAlloc would have been called. %p > %p.\n", pNextAlloc, pCommitLimit);
...
byte *CMatQueuedRenderContext::ReallocVertices(byte *pVerts, int nVertsOld, int nVertsNew, int nVertexSize)
{
	...
	if(nVertsNew < nVertsOld)
	{
		...
		m_Vertices.FreeToAllocPoint(AlignValue(m_Vertices.GetCurrentAllocPoint() - nBytes, 16),
...
uint16 *CMatQueuedRenderContext::ReallocIndices(uint16 *pIndices, int nIndicesOld, int nIndicesNew)
{
	...
	if(nIndicesNew < nIndicesOld)
	{
		...
		m_Indices.FreeToAllocPoint(AlignValue(m_Indices.GetCurrentAllocPoint() - nBytes, 16),
								   false); // memstacks 128 bit aligned
	}
	return pIndices;
}
...
// NOTE: These are synchronous calls!  The rendering thread is stopped, the current queue is drained and the pixels are
// read NOTE: We should also have a queued read pixels in the API for doing mid frame reads (as opposed to screenshots)
void CMatQueuedRenderContext::ReadPixels(int x, int y, int width, int height, unsigned char *data,
...
void CMatQueuedRenderContext::ReadPixelsAndStretch(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *pBuffer,
