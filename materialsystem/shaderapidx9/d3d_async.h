...
// Slamming this off - it's causing very hot D3D9 function calls to not be inlined and contain a bunch of unused code.
// (Does this code even work/add real value any more?)
#define SHADERAPI_USE_SMP 0

// Set this to 1 to allow buffering of the whole frame to memory and then playback (singlethreaded).
// This is for debugging only and is used to test the performance of just calling D3D and rendering without other CPU
// overhead.
...
#define SHADERAPI_BUFFER_MAXRENDERTARGETS 4
	...
	FORCEINLINE void Push(PushBufferCommand cmd, void *arg1, uint32 arg2, uint32 arg3, uint32 arg4, void *arg5)
	{
	...
	void AsynchronousLock(IDirect3DVertexBuffer9 *vb, size_t offset, size_t size, void **ptr, DWORD flags,
						  LockedBufferContext *lb);

	void AsynchronousLock(IDirect3DIndexBuffer9 *ib, size_t offset, size_t size, void **ptr, DWORD flags,
						  LockedBufferContext *lb);
...
public:
	...
	{
		// hack!  We are going to assume that calling this immediately when in buffered mode isn't going to cause
		// problems.
#if !SHADERAPI_BUFFER_D3DCALLS
		Synchronize();
#endif
	...
	{
		Synchronize();
		return m_pD3DDevice->CreateRenderTarget(Width, Height, Format, MultiSample, MultisampleQuality, Lockable,
	...
	{
		Synchronize();
		return m_pD3DDevice->CreateDepthStencilSurface(Width, Height, Format, MultiSample, MultisampleQuality, Discard,
													   ppSurface, pSharedHandle);
	}
	...
	FORCEINLINE void SetRenderTarget(int idx, IDirect3DSurface9 *new_rt)
	{
		...
		{
			// NOTE: If the debug runtime breaks here on the shadow depth render target that is normal.  dx9 doesn't
			// directly support shadow depth texturing so we are forced to initialize this texture without the render
			// target flagr
	...
	FORCEINLINE void SetVertexShaderConstantF(UINT StartRegister, CONST float *pConstantData, UINT Vector4fCount)
	{
	...
	FORCEINLINE void SetVertexShaderConstantB(UINT StartRegister, CONST int *pConstantData, UINT BoolCount)
	{
		RECORD_COMMAND(DX8_SET_VERTEX_SHADER_CONSTANT, 3);
		RECORD_INT(StartRegister);
	...
	FORCEINLINE void SetVertexShaderConstantI(UINT StartRegister, CONST int *pConstantData, UINT Vector4IntCount)
	{
		RECORD_COMMAND(DX8_SET_VERTEX_SHADER_CONSTANT, 3);
		RECORD_INT(StartRegister);
	...
	FORCEINLINE void SetPixelShaderConstantF(UINT StartRegister, CONST float *pConstantData, UINT Vector4fCount)
	{
	...
	FORCEINLINE void SetPixelShaderConstantB(UINT StartRegister, CONST int *pConstantData, UINT BoolCount)
	{
		RECORD_COMMAND(DX8_SET_PIXEL_SHADER_CONSTANT, 3);
		RECORD_INT(StartRegister);
		RECORD_INT(BoolCount);
	...
	FORCEINLINE void SetPixelShaderConstantI(UINT StartRegister, CONST int *pConstantData, UINT Vector4IntCount)
	{
		RECORD_COMMAND(DX8_SET_PIXEL_SHADER_CONSTANT, 3);
		RECORD_INT(StartRegister);
		RECORD_INT(Vector4IntCount);
	...
	FORCEINLINE HRESULT Lock(IDirect3DVertexBuffer9 *vb, size_t offset, size_t size, void **ptr, DWORD flags)
	{
		...
		switch(hr)
		{
			case D3DERR_INVALIDCALL:
				Warning(
					"D3DERR_INVALIDCALL - Vertex Buffer Lock Failed in %s on line %d(offset %d, size %d, flags 0x%x)\n",
					V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
				break;
			case D3DERR_DRIVERINTERNALERROR:
				Warning("D3DERR_DRIVERINTERNALERROR - Vertex Buffer Lock Failed in %s on line %d (offset %d, size %d, "
						"flags 0x%x)\n",
						V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
				break;
			case D3DERR_OUTOFVIDEOMEMORY:
				Warning("D3DERR_OUTOFVIDEOMEMORY - Vertex Buffer Lock Failed in %s on line %d (offset %d, size %d, "
						"flags 0x%x)\n",
						V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
				break;
		}
							 ...
							 LockedBufferContext *lb)
	{
		...
		else
		{
			...
			{
				case D3DERR_INVALIDCALL:
					Warning("D3DERR_INVALIDCALL - Vertex Buffer Lock Failed in %s on line %d(offset %d, size %d, flags "
							"0x%x)\n",
							V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
					break;
				case D3DERR_DRIVERINTERNALERROR:
					Warning("D3DERR_DRIVERINTERNALERROR - Vertex Buffer Lock Failed in %s on line %d (offset %d, size "
							"%d, flags 0x%x)\n",
							V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
					break;
				case D3DERR_OUTOFVIDEOMEMORY:
	...
	FORCEINLINE HRESULT Lock(IDirect3DIndexBuffer9 *ib, size_t offset, size_t size, void **ptr, DWORD flags)
	{
		...
		switch(hr)
		{
			case D3DERR_INVALIDCALL:
				...
				break;
			case D3DERR_DRIVERINTERNALERROR:
				Warning("D3DERR_DRIVERINTERNALERROR - Index Buffer Lock Failed in %s on line %d (offset %d, size %d, "
						"flags 0x%x)\n",
						V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
				break;
			case D3DERR_OUTOFVIDEOMEMORY:
				Warning("D3DERR_OUTOFVIDEOMEMORY - Index Buffer Lock Failed in %s on line %d (offset %d, size %d, "
						"flags 0x%x)\n",
						V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
				break;
		}
							 ...
							 LockedBufferContext *lb)
	{
		...
		else
		{
			...
			{
				case D3DERR_INVALIDCALL:
					Warning("D3DERR_INVALIDCALL - Index Buffer Lock Failed in %s on line %d(offset %d, size %d, flags "
							"0x%x)\n",
							V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
					break;
				case D3DERR_DRIVERINTERNALERROR:
					Warning("D3DERR_DRIVERINTERNALERROR - Index Buffer Lock Failed in %s on line %d (offset %d, size "
							"%d, flags 0x%x)\n",
							V_UnqualifiedFileName(__FILE__), __LINE__, offset, size, flags);
					break;
				case D3DERR_OUTOFVIDEOMEMORY:
	...
	FORCEINLINE HRESULT UpdateSurface(IDirect3DSurface9 *pSourceSurface, CONST RECT *pSourceRect,
	...
	FORCEINLINE void DrawPrimitive(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount)
	{
	...
	HRESULT CreateVertexShader(CONST DWORD *pFunction, IDirect3DVertexShader9 **ppShader, const char *pShaderName,
	...
	HRESULT CreatePixelShader(CONST DWORD *pFunction, IDirect3DPixelShader9 **ppShader, const char *pShaderName,
							  char *debugLabel = NULL)
	{
		Synchronize();
#ifdef DX_TO_GL_ABSTRACTION
	...
	FORCEINLINE void SetStreamSource(UINT StreamNumber, IDirect3DVertexBuffer9 *pStreamData, UINT OffsetInBytes,
									 UINT Stride)
	{
		if(ASyncMode())
	...
	FORCEINLINE void DrawIndexedPrimitive(D3DPRIMITIVETYPE Type, INT BaseVertexIndex, UINT MinIndex, UINT NumVertices,
										  UINT StartIndex, UINT PrimitiveCount)
	{
		...
		if(ASyncMode())
		{
			Push(PBCMD_DRAWINDEXEDPRIM, Type, BaseVertexIndex, MinIndex, NumVertices, StartIndex, PrimitiveCount);
			//			SubmitIfNotBusy();
		}
		else
		{
	...
	void DrawIndexedPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType, UINT MinVertexIndex, UINT NumVertices,
								UINT PrimitiveCount, CONST void *pIndexData, D3DFORMAT IndexDataFormat,
								CONST void *pVertexStreamZeroData, UINT VertexStreamZeroStride)
	{
		Synchronize();
		DO_D3D(DrawIndexedPrimitiveUP(PrimitiveType, MinVertexIndex, NumVertices, PrimitiveCount, pIndexData,
									  IndexDataFormat, pVertexStreamZeroData, VertexStreamZeroStride));
	}

	HRESULT Present(CONST RECT *pSourceRect, CONST RECT *pDestRect, VD3DHWND hDestWindowOverride,
					CONST RGNDATA *pDirtyRegion)
	{
		...
		if(ASyncMode())
		{
			...
			AllocatePushBufferSpace(1 + 1 + N_DWORDS(RECT) + 1 + N_DWORDS(RECT) + 1 + 1 + N_DWORDS(RGNDATA));
