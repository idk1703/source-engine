...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderShadowDX8, IShaderShadow, SHADERSHADOW_INTERFACE_VERSION, g_ShaderShadow)
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CShaderShadowDX8::CShaderShadowDX8() : m_DrawFlags(0), m_pHardwareConfig(0), m_HasConstantColor(false)
{
...
void CShaderShadowDX8::DepthFunc(ShaderDepthFunc_t depthFunc)
{
	...
	switch(depthFunc)
	{
		...
		case SHADER_DEPTHFUNC_NEAREROREQUAL:
			zFunc = (ShaderUtil()->GetConfig().bReverseDepth ^ ReverseDepthOnX360()) ? D3DCMP_GREATEREQUAL
																					 : D3DCMP_LESSEQUAL;
			break;
		...
		case SHADER_DEPTHFUNC_FARTHEROREQUAL:
			zFunc = (ShaderUtil()->GetConfig().bReverseDepth ^ ReverseDepthOnX360()) ? D3DCMP_LESSEQUAL
																					 : D3DCMP_GREATEREQUAL;
			break;
...
case SHADER_BLEND_ONE_MINUS_SRC_COLOR:
	...
	{
		if(m_pHardwareConfig->SupportsOverbright() && (stage < m_pHardwareConfig->GetTextureStageCount()))
		{
	...
	void CShaderShadowDX8::CustomTextureOperation(TextureStage_t stage, ShaderTexChannel_t channel, ShaderTexOp_t op,
	...
	//-----------------------------------------------------------------------------
	void CShaderShadowDX8::VertexShaderVertexFormat(unsigned int nFlags, int nTexCoordCount, int *pTexCoordDimensions,
													int nUserDataSize)
	{
		...
		m_ShadowShaderState.m_VertexUsage =
			MeshMgr()->ComputeVertexFormat(nFlags, nTexCoordCount, pTexCoordDimensions, 0, nUserDataSize);
		...
		if(CVertexBufferBase::VertexFormatSize(m_ShadowShaderState.m_VertexUsage) <= 16)
		{
			...
			if(!bComplained)
			{
				Warning("ERROR: shader asking for a too-narrow vertex format - you will see errors if running with "
			...
			nTexCoordCount = 1;
			m_ShadowShaderState.m_VertexUsage =
				MeshMgr()->ComputeVertexFormat(nFlags, nTexCoordCount, NULL, 0, nUserDataSize);
		}
	}
	...
	void CShaderShadowDX8::SetVertexShader(const char *pFileName, int nStaticVshIndex)
	{
		...
		m_ShadowShaderState.m_VertexShader =
			ShaderManager()->CreateVertexShader(pFileName, nStaticVshIndex, debugLabel);
	...
	void CShaderShadowDX8::ConfigureAlphaPipe(unsigned int flags)
	{
		...
		for(int i = 0; i < m_pHardwareConfig->GetTextureStageCount(); ++i)
		{
			...
			if(m_TextureStage[i].m_TextureAlphaEnable)
			{
				...
				{
					m_ShadowState.m_TextureStage[i].m_AlphaArg1 = D3DTA_TEXTURE;
					m_ShadowState.m_TextureStage[i].m_AlphaArg2 = isUsingConstantAlpha ? D3DTA_TFACTOR : D3DTA_DIFFUSE;
	...
	void CShaderShadowDX8::ConfigureFVFVertexShader(unsigned int flags)
	{
		...
		if(isUsingConstantColor)
		{
			...
			if(isUsingConstantColor && (lastUsedTextureStage >= m_pHardwareConfig->GetTextureStageCount()))
			{
	...
	VertexFormat_t CShaderShadowDX8::FlagsToVertexFormat(int flags) const
	{
		...
		return MeshMgr()->ComputeVertexFormat(formatFlags, VERTEX_MAX_TEXTURE_COORDINATES, texCoordSize, numBones,
											  userDataSize);
	}
	...
	void CShaderShadowDX8::ComputeAggregateShadowState()
	{
		...
		{
			m_ShadowState.m_SrcBlend =
	D3DBLEND_ONE
	;
	m_ShadowState.m_DestBlend =
	D3DBLEND_ZERO
	;
	...
	{
		m_ShadowState.m_SrcBlendAlpha =
	D3DBLEND_ONE
	;
	m_ShadowState.m_DestBlendAlpha =
	D3DBLEND_ZERO
	;
