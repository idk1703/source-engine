...
// Less functions
//-----------------------------------------------------------------------------
bool CTransitionTable::ShadowStateDictLessFunc::Less(const CTransitionTable::ShadowStateDictEntry_t &src1,
													 const CTransitionTable::ShadowStateDictEntry_t &src2, void *pCtx)
{
	return src1.m_nChecksum < src2.m_nChecksum;
}

bool CTransitionTable::SnapshotDictLessFunc::Less(const CTransitionTable::SnapshotDictEntry_t &src1,
												  const CTransitionTable::SnapshotDictEntry_t &src2, void *pCtx)
{
	return src1.m_nChecksum < src2.m_nChecksum;
}

bool CTransitionTable::UniqueSnapshotLessFunc::Less(const CTransitionTable::TransitionList_t &src1,
...
//-----------------------------------------------------------------------------
StateSnapshot_t CTransitionTable::CreateStateSnapshot(ShadowStateId_t shadowStateId,
...
#define SetRenderStateConstMacro(state, val)                    \
	{                                                           \
...
void ApplyFetch4Enable(const ShadowState_t &shaderState, int stage)
{
	if(ShaderAPI()->SupportsFetch4())
	{
		SetSamplerState(stage, ATISAMP_FETCH4,
						shaderState.m_SamplerState[stage].m_Fetch4Enable ? ATI_FETCH4_ENABLE : ATI_FETCH4_DISABLE);
	}
...
void CTransitionTable::ApplySRGBWriteEnable(const ShadowState_t &shaderState)
{
	...
	else
	{
		...
		{
			if(HardwareConfig()->SupportsPixelShaders_2_b()) // in 2b supported devices, we never actually enable SRGB
															 // writes, but instead handle the conversion in the pixel
															 // shader. But we want all other code to be unaware.
			{
				SetRenderStateConstMacro(D3DRS_SRGBWRITEENABLE, 0);
			}
			else
		...
		{
			ShaderAPI()->ApplyFogMode(shaderState.m_FogMode, shaderState.m_SRGBWriteEnable,
...
{
	ShaderAPI()->ApplyFogMode(shadowState.m_FogMode, shadowState.m_SRGBWriteEnable,
...
int CTransitionTable::CreateNormalTransitions(const ShadowState_t &fromState, const ShadowState_t &toState, bool bForce)
{
	...
	bool blendSeparateAlphaEnableDifferent =
		(toState.m_SeparateAlphaBlendEnable != fromState.m_SeparateAlphaBlendEnable);
	bool srcBlendAlphaDifferent =
		toState.m_SeparateAlphaBlendEnable && (toState.m_SrcBlendAlpha != fromState.m_SrcBlendAlpha);
	bool destBlendAlphaDifferent =
		toState.m_SeparateAlphaBlendEnable && (toState.m_DestBlendAlpha != fromState.m_DestBlendAlpha);
	bool blendOpAlphaDifferent =
		toState.m_SeparateAlphaBlendEnable && (toState.m_BlendOpAlpha != fromState.m_BlendOpAlpha);
	if(bForce || blendOpAlphaDifferent || blendSeparateAlphaEnableDifferent || srcBlendAlphaDifferent ||
...
void CTransitionTable::CreateTransitionTableEntry(int to, int from)
{
	...
	TransitionList_t &transition = (from >= 0) ? m_TransitionTable[to][from] : m_DefaultTransition;
	Assert(numOps <= 255);
...
#define PERFORM_RENDER_STATE_TRANSITION(_state, _func)				  ::Apply##_func(_state, 0);
#define PERFORM_TEXTURE_STAGE_STATE_TRANSITION(_state, _stage, _func) ::Apply##_func(_state, _stage);
#define PERFORM_SAMPLER_STATE_TRANSITION(_state, _stage, _func)		  ::Apply##_func(_state, _stage);
...
//-----------------------------------------------------------------------------
unsigned int CTransitionTable::FindIdenticalTransitionList(unsigned int firstElem, unsigned short numOps,
														   unsigned int nFirstTest) const
{
...
void CTransitionTable::ApplyTransition(TransitionList_t &list, int snapshot)
{
	...
	for(i = nSamplerCount; i < MAX_SAMPLERS; ++i)
	{
		m_BoardState.m_SamplerState[i].m_TextureEnable = CurrentShadowState()->m_SamplerState[i].m_TextureEnable;
	}

	int nTextureStageCount = HardwareConfig()->GetTextureStageCount();
	for(i = nTextureStageCount; i < MAX_TEXTURE_STAGES; ++i)
	{
		memcpy(&m_BoardState.m_TextureStage[i], &CurrentShadowState()->m_TextureStage[i],
...
void CTransitionTable::UseDefaultState()
{
	...
	m_CurrentState.m_SrcBlend =
D3DBLEND_ONE
;
m_CurrentState.m_DestBlend =
D3DBLEND_ZERO
;
...
m_CurrentState.m_SrcBlendAlpha =
D3DBLEND_ONE
;
m_CurrentState.m_DestBlendAlpha =
D3DBLEND_ZERO
;
...
#if defined(_X360)
			// SetRenderStateConstMacro( D3DRS_HIZWRITEENABLE, m_CurrentState.m_OverrideZWriteEnable ? D3DHIZ_AUTOMATIC
			// : D3DHIZ_DISABLE );
#endif
		}
		else
...
#if defined(_X360)
				// SetRenderStateConstMacro( D3DRS_HIZWRITEENABLE, CurrentShadowState()->m_ZWriteEnable ?
	...
	if(bOverrideEnable != m_CurrentState.m_bOverrideColorWriteEnable)
	{
		...
		else
		{
			if(CurrentShadowState())
			{
				// probably being paranoid, but only copy the alpha flag from the shadow state
				dwSetValue &= ~(D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN | D3DCOLORWRITEENABLE_BLUE);
				dwSetValue |= CurrentShadowState()->m_ColorWriteEnable &
...
void CTransitionTable::PerformShadowStateOverrides()
{
	...
	if(m_CurrentState.m_bOverrideColorWriteEnable)
	{
		DWORD dwSetValue = m_CurrentState.m_ColorWriteEnable &
						   ~(D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN | D3DCOLORWRITEENABLE_BLUE);
		dwSetValue |= m_CurrentState.m_bOverriddenColorWriteValue
						  ? (D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN | D3DCOLORWRITEENABLE_BLUE)
						  : 0;
		if(dwSetValue != m_CurrentState.m_ColorWriteEnable)
		{
