...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderDeviceMgrDx10, IShaderDeviceMgr, SHADER_DEVICE_MGR_INTERFACE_VERSION,
...
bool CShaderDeviceDx10::InitDevice(void *hWnd, int nAdapter, const ShaderDeviceInfo_t &mode)
{
	...
	HRESULT hr = D3D10CreateDeviceAndSwapChain(pAdapter, D3D10_DRIVER_TYPE_HARDWARE, NULL, nDeviceFlags,
...
// Camma ramp
//-----------------------------------------------------------------------------
void CShaderDeviceDx10::SetHardwareGammaRamp(float fGamma, float fGammaTVRangeMin, float fGammaTVRangeMax,
...
IShaderBuffer *CShaderDeviceDx10::CompileShader(const char *pProgram, size_t nBufLen, const char *pShaderVersion)
{
	...
	HRESULT hr = D3DX10CompileFromMemory(pProgram, nBufLen, "", NULL, NULL, "main", pShaderVersion, nCompileFlags, 0,
										 NULL, &pCompiledShader, &pErrorMessages, NULL);
...
VertexShaderHandle_t CShaderDeviceDx10::CreateVertexShader(IShaderBuffer *pShaderBuffer)
{
	...
	HRESULT hr = m_pDevice->CreateVertexShader(pShaderBuffer->GetBits(), pShaderBuffer->GetSize(), &pShader);
...
GeometryShaderHandle_t CShaderDeviceDx10::CreateGeometryShader(IShaderBuffer *pShaderBuffer)
{
	...
	HRESULT hr = m_pDevice->CreateGeometryShader(pShaderBuffer->GetBits(), pShaderBuffer->GetSize(), &pShader);
...
PixelShaderHandle_t CShaderDeviceDx10::CreatePixelShader(IShaderBuffer *pShaderBuffer)
{
	...
	HRESULT hr = m_pDevice->CreatePixelShader(pShaderBuffer->GetBits(), pShaderBuffer->GetSize(), &pShader);
...
//-----------------------------------------------------------------------------
IVertexBuffer *CShaderDeviceDx10::CreateVertexBuffer(ShaderBufferType_t type, VertexFormat_t fmt, int nVertexCount,
...
IIndexBuffer *CShaderDeviceDx10::CreateIndexBuffer(ShaderBufferType_t type, MaterialIndexFormat_t fmt, int nIndexCount,
												   const char *pBudgetGroup)
{
	LOCK_SHADERAPI();
