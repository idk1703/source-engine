...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderDeviceMgrDx8, IShaderDeviceMgr, SHADER_DEVICE_MGR_INTERFACE_VERSION,
...
bool CShaderDeviceMgrDx8::Connect(CreateInterfaceFn factory)
{
	...
	bool bD3D9ExForceDisable =
		(CommandLine()->FindParm("-nod3d9ex") != 0) || (CommandLine()->ParmValue("-dxlevel", 95) < 90);
...
#if defined(PIX_INSTRUMENTATION) && defined(DX_TO_GL_ABSTRACTION) && defined(_WIN32)
	// This is a little odd, but AMD PerfStudio hooks D3D9.DLL and intercepts all of the D3DPERF API's (even for OpenGL
	// apps). So dynamically load d3d9.dll and get the address of these exported functions.
	if(!m_hD3D9)
	{
...
void CShaderDeviceMgrDx8::CheckVendorDependentShadowMappingSupport(HardwareCaps_t *pCaps, int nAdapter)
{
	...
	if(m_pD3D->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
								 D3DFMT_R5G6B5) == S_OK)
	{
	...
	if(IsPC() || !IsX360())
	{
		...
		if((pCaps->m_VendorID == VENDORID_NVIDIA) && (pCaps->m_SupportsShaderModel_3_0)) // ps_3_0 parts from nVidia
		{
			...
			if(m_pD3D->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET,
			...
			//   NVFMT_RAWZ is supported by NV4x (not supported here yet...requires a dp3 to reconstruct in shader code,
			//   which doesn't seem to work) NVFMT_INTZ is supported on newer chips as of G8x (just read like ATI
			//   non-fetch4 mode)
			//
			/*
						if ( m_pD3D->CheckDeviceFormat( nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET,
			   D3DRTYPE_TEXTURE, NVFMT_INTZ ) == S_OK )
						{
			...
			*/
			if(m_pD3D->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_DEPTHSTENCIL,
		...
		else if((pCaps->m_VendorID == VENDORID_ATI) && pCaps->m_SupportsPixelShaders_2_b) // ps_2_b parts from ATI
		{
			...
			if(m_pD3D->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_DEPTHSTENCIL,
										 D3DRTYPE_TEXTURE, ATIFMT_D16) == S_OK) // Prefer 16-bit
			{
...
bool CShaderDeviceMgrDx8::ComputeCapsFromD3D(HardwareCaps_t *pCaps, int nAdapter)
{
	...
	pCaps->m_SupportsPixelShaders_2_b =
		...
		(caps.PS20Caps.NumInstructionSlots >= 512); // More caps to this, but this will do
...
#ifdef OSX
	// Static control flow is disabled by default on OSX (the Mac version of togl has known bugs preventing this path
	// from working properly that we've fixed in togl linux/win)
	...
	// OpenGL does not support this--at least not on OSX which is the primary GL target, so just don't use that path on
	// GL at all.
#if !defined(DX_TO_GL_ABSTRACTION)
	pCaps->m_bCanStretchRectFromTextures =
		((caps.DevCaps2 & D3DDEVCAPS2_CAN_STRETCHRECT_FROM_TEXTURES) != 0) && (pCaps->m_VendorID != VENDORID_INTEL);
#else
	...
	if(pCaps->m_bSoftwareVertexProcessing)
	{
		...
		pCaps->m_NumBooleanVertexShaderConstants =
			pCaps->m_SupportsPixelShaders_2_0 ? 16 : 0; // 2.0 parts have 16 bool vs registers
		pCaps->m_NumBooleanPixelShaderConstants =
			pCaps->m_SupportsPixelShaders_2_0 ? 16 : 0; // 2.0 parts have 16 bool ps registers
	...
	if(IsOpenGL())
	{
		...
		pCaps->m_MaxNumLights =
			MIN(pCaps->m_MaxNumLights, (pCaps->m_bSupportsStaticControlFlow && pCaps->m_SupportsPixelShaders_2_b)
										   ? MAX_NUM_LIGHTS
										   : (MAX_NUM_LIGHTS - 2));
	}
	...
	// for polyoffset.
	pCaps->m_ZBiasAndSlopeScaledDepthBiasSupported = ((caps.RasterCaps & D3DPRASTERCAPS_DEPTHBIAS) != 0) &&
	...
	if(CommandLine()->CheckParm(
		   "-nouserclip") /* || (IsOpenGL() && (!CommandLine()->FindParm("-glslmode"))) || r_emulategl.GetBool() */)
	{
	...
	if(IsPC() || !IsX360())
	{
		pCaps->m_SupportsSRGB =
			(D3D()->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_QUERY_SRGBREAD, D3DRTYPE_TEXTURE,
									  D3DFMT_DXT1) == S_OK);

		if(pCaps->m_SupportsSRGB)
		{
			pCaps->m_SupportsSRGB = (D3D()->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8,
	...
	pCaps->m_bSupportsVertexTextures =
		(D3D()->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_QUERY_VERTEXTEXTURE,
	...
	bool bSupportsInteger16Textures =
		(D3D()->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_QUERY_FILTER, D3DRTYPE_TEXTURE,
	...
	bool bSupportsFloat16RenderTargets =
		(D3D()->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8,
								  D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING | D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
								  D3DFMT_A16B16G16R16F) == S_OK);
	...
	bool bSupportsIntegerHDR = pCaps->m_SupportsPixelShaders_2_0 && pCaps->m_SupportsVertexShaders_2_0 &&
							   //		(caps.Caps3 & D3DCAPS3_ALPHA_FULLSCREEN_FLIP_OR_DISCARD) &&
							   //		(caps.PrimitiveMiscCaps & D3DPMISCCAPS_SEPARATEALPHABLEND) &&
							   (bSupportsInteger16Textures || bSupportsFloat16Textures) && pCaps->m_SupportsSRGB;

	// Do we have everything necessary to run with float HDR?
	bool bSupportsFloatHDR = pCaps->m_SupportsShaderModel_3_0 &&
							 //		(caps.Caps3 & D3DCAPS3_ALPHA_FULLSCREEN_FLIP_OR_DISCARD) &&
							 //		(caps.PrimitiveMiscCaps & D3DPMISCCAPS_SEPARATEALPHABLEND) &&
							 bSupportsFloat16Textures && bSupportsFloat16RenderTargets && pCaps->m_SupportsSRGB &&
							 !IsX360();
	...
	pCaps->m_bSupportsStreamOffset =
		((caps.DevCaps2 & D3DDEVCAPS2_STREAMOFFSET) && // Tie these caps together since we want to filter out
	...
	pCaps->m_SupportsCompressedVertices = (pCaps->m_nDXSupportLevel >= 90) && (pCaps->m_CanDoSRGBReadFromRTs)
											  ? VERTEX_COMPRESSION_ON
											  : VERTEX_COMPRESSION_NONE;
	if(CommandLine()->CheckParm(
		   "-no_compressed_verts")) // m_CanDoSRGBReadFromRTs limits us to Snow Leopard or later on OSX
	{
	...
	// If we're not on a 3.0 part, these values are more appropriate (X800 & X850 parts from ATI do shadow mapping but
	// not 3.0 )
	if(!IsOpenGL())
	{
		if(!pCaps->m_SupportsShaderModel_3_0)
...
bool CShaderDeviceMgrDx8::SetAdapter(int nAdapter, int nAdapterFlags)
{
	...
	g_pShaderDeviceDx8->m_DeviceType =
		(nAdapterFlags & MATERIAL_INIT_REFERENCE_RASTERIZER) ? D3DDEVTYPE_REF : D3DDEVTYPE_HAL;
...
#else
	return 1; // Only one mode, which is the current mode set in the 360 dashboard.  Going to fill it in with exactly
			  // what the 360 is set to.
#endif
}
...
bool CShaderDeviceMgrDx8::ValidateMode(int nAdapter, const ShaderDeviceInfo_t &info) const
{
	...
	ImageFormat backBufferFormat = FindNearestSupportedBackBufferFormat(nAdapter, DX8_DEVTYPE, displayMode.m_Format,
...
void CShaderDeviceDx8::SetPresentParameters(void *hWnd, int nAdapter, const ShaderDeviceInfo_t &info)
{
	...
	ImageFormat backBufferFormat = FindNearestSupportedBackBufferFormat(nAdapter, DX8_DEVTYPE, m_AdapterFormat,
																		info.m_DisplayMode.m_Format, info.m_bWindowed);
	...
	m_PresentParameters.AutoDepthStencilFormat =
		FindNearestSupportedDepthFormat(nAdapter, m_AdapterFormat, backBufferFormat, nDepthFormat);
	...
	if(IsX360() || !info.m_bWindowed)
	{
		...
		m_PresentParameters.FullScreen_RefreshRateInHz =
			info.m_DisplayMode.m_nRefreshRateDenominator
				? info.m_DisplayMode.m_nRefreshRateNumerator / info.m_DisplayMode.m_nRefreshRateDenominator
		...
		if(info.m_bScaleToOutputResolution)
		{
			...
			DevMsg("VIDEO SCALING: scaling from %dx%d to %dx%d\n", (int)m_PresentParameters.BackBufferWidth,
				   (int)m_PresentParameters.BackBufferHeight, (int)videoMode.dwDisplayWidth,
				   (int)videoMode.dwDisplayHeight);
		}
		else
		{
	...
	if(info.m_nAASamples > 0 && (m_PresentParameters.SwapEffect == D3DSWAPEFFECT_DISCARD))
	{
		...
		if((info.m_nAASamples == 16) && (adapterCaps.m_VendorID == VENDORID_NVIDIA))
		{
			...
			hr = D3D()->CheckDeviceMultiSampleType(nAdapter, DX8_DEVTYPE, m_PresentParameters.BackBufferFormat,
...
void CShaderDeviceDx8::SpewDriverInfo() const
{
	...
	Warning("Vertex Shader Version : %d.%d Pixel Shader Version : %d.%d\n", (caps.VertexShaderVersion >> 8) & 0xFF,
			caps.VertexShaderVersion & 0xFF, (caps.PixelShaderVersion >> 8) & 0xFF, caps.PixelShaderVersion & 0xFF);
	Warning("\nDevice Caps :\n");
...
const char *GetD3DErrorText(HRESULT hr)
{
	...
	switch(hr)
	{
		...
		case D3DERR_UNSUPPORTEDCOLOROPERATION:
			pszMoreInfo = "D3DERR_UNSUPPORTEDCOLOROPERATION: The device does not support a specified texture-blending "
						  "operation for color values.";
			break;
		case D3DERR_UNSUPPORTEDCOLORARG:
			pszMoreInfo = "D3DERR_UNSUPPORTEDCOLORARG: The device does not support a specified texture-blending "
						  "argument for color values.";
			break;
		case D3DERR_UNSUPPORTEDALPHAOPERATION:
			pszMoreInfo = "D3DERR_UNSUPPORTEDALPHAOPERATION: The device does not support a specified texture-blending "
						  "operation for the alpha channel.";
			break;
		case D3DERR_UNSUPPORTEDALPHAARG:
			pszMoreInfo = "D3DERR_UNSUPPORTEDALPHAARG: The device does not support a specified texture-blending "
						  "argument for the alpha channel.";
			break;
		case D3DERR_TOOMANYOPERATIONS:
			pszMoreInfo = "D3DERR_TOOMANYOPERATIONS: The application is requesting more texture-filtering operations "
		...
		case D3DERR_DEVICELOST:
			pszMoreInfo = "D3DERR_DEVICELOST: The device has been lost but cannot be reset at this time. Therefore, "
		...
		case D3DERR_OUTOFVIDEOMEMORY:
			pszMoreInfo =
				"D3DERR_OUTOFVIDEOMEMORY: Direct3D does not have enough display memory to perform the operation. The "
				"device is using more resources in a single scene than can fit simultaneously into video memory.";
			break;
		...
		case D3DERR_WASSTILLDRAWING:
			pszMoreInfo = "D3DERR_WASSTILLDRAWING: The previous blit operation that is transferring information to or "
...
IDirect3DDevice9 *CShaderDeviceDx8::InvokeCreateDevice(void *hWnd, int nAdapter, DWORD deviceCreationFlags)
{
	...
	HRESULT hr =
		D3D()->CreateDevice(nAdapter, devType, (VD3DHWND)hWnd, deviceCreationFlags, &m_PresentParameters, &pD3DDevice);
	...
	if(m_PresentParameters.Windowed)
	{
		...
		hr = D3D()->CreateDevice(nAdapter, devType, (VD3DHWND)hWnd, deviceCreationFlags, &m_PresentParameters,
								 &pD3DDevice);
	}
	if(!FAILED(hr) && pD3DDevice)
		return pD3DDevice;
	...
	switch(hr)
	{
		...
		case D3DERR_INVALIDCALL:
			...
			pszMoreInfo = "D3DERR_INVALIDCALL: The device or the device driver may not support Direct3D or may not "
						  "support the resolution or color depth specified.";
			break;
#endif // _WIN32
		default:
...
bool CShaderDeviceDx8::CreateD3DDevice(void *pHWnd, int nAdapter, const ShaderDeviceInfo_t &info)
{
	...
	// Create the depth buffer, created manually to enable hierarchical z
	{
		...
		DepthStencilParams.Base =
			XGSurfaceSize(m_PresentParameters.BackBufferWidth, m_PresentParameters.BackBufferHeight,
						  m_PresentParameters.BackBufferFormat, m_PresentParameters.MultiSampleType);
		DepthStencilParams.ColorExpBias = 0;
		DepthStencilParams.HierarchicalZBase =
			GPU_HIERARCHICAL_Z_TILES - XGHierarchicalZSize(m_PresentParameters.BackBufferWidth,
		...
		hr = Dx9Device()->CreateDepthStencilSurface(
			...
			m_PresentParameters.AutoDepthStencilFormat, m_PresentParameters.MultiSampleType,
			m_PresentParameters.MultiSampleQuality, TRUE, &pDepthStencilSurface, &DepthStencilParams);
		Assert(SUCCEEDED(hr));
		if(FAILED(hr))
			return false;
...
bool CShaderDeviceDx8::AllocNonInteractiveRefreshObjects()
{
	...
	const char *strPixelShaderProgram2 = " struct PS_IN"
										 " {"
										 " float2 TexCoord : TEXCOORD;"
										 " };"
										 " sampler detail : register( s0 );"
	...
	D3DVERTEXELEMENT9 VertexElements[4] = {{0, 0, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0},
	...
	hr = D3DXCompileShader(strPixelShaderProgram3, (UINT)strlen(strPixelShaderProgram3), NULL, NULL, "main", "ps_2_0",
	...
	hr = D3DXCompileShader(strPixelShaderProgram2, (UINT)strlen(strPixelShaderProgram2), NULL, NULL, "main", "ps_2_0",
						   0, &pShaderCode, &pErrorMsg, NULL);
	if(FAILED(hr))
		return false;
...
void CShaderDeviceDx8::EnableNonInteractiveMode(MaterialNonInteractiveMode_t mode, ShaderNonInteractiveInfo_t *pInfo)
{
	...
	if(mode != MATERIAL_NON_INTERACTIVE_MODE_NONE)
	{
		...
		SetHardwareGammaRamp(mat_monitorgamma.GetFloat(), mat_monitorgamma_tv_range_min.GetFloat(),
							 mat_monitorgamma_tv_range_max.GetFloat(), mat_monitorgamma_tv_exp.GetFloat(),
	...
	//m_NonInteractiveRefresh.m_flStartTime, 		m_NonInteractiveRefresh.m_flPeakDt, m_NonInteractiveRefresh.m_flTotalDt /
	//m_NonInteractiveRefresh.m_nSamples, m_NonInteractiveRefresh.m_nSamples, m_NonInteractiveRefresh.m_nCountAbove66 );
...
void CShaderDeviceDx8::RefreshFrontBufferNonInteractive()
{
	...
	IDirect3DBaseTexture *pTexture =
		g_pShaderAPI->GetD3DTexture(m_NonInteractiveRefresh.m_Info.m_hTempFullscreenTexture);
	...
	Dx9Device()->SetPixelShader(bInStartupMode ? m_NonInteractiveRefresh.m_pPixelShaderStartup
	...
	if(bInStartupMode)
	{
		...
		pTexture = g_pShaderAPI->GetD3DTexture(
			m_NonInteractiveRefresh.m_Info.m_pPacifierTextures[m_NonInteractiveRefresh.m_nPacifierFrame]);
...
void CShaderDeviceDx8::Present()
{
	...
	{
		g_pShaderAPI->CopyRenderTargetToTextureEx(m_NonInteractiveRefresh.m_Info.m_hTempFullscreenTexture, 0, NULL,
												  NULL);
	}
...
//    the 0-255 scale, but it looks like the 360 deals with this for us by lowering the bright saturated color
//    components.
// NOTE: I'm leaving the max at 255 to retain whiter than whites.  On most TV's, we seems a little dark in the bright
// colors
//    compared to TV and movies when played in the same conditions.  This keeps out brights on par with what customers
//    are used to seeing.
// TV's generally have a 2.5 gamma, so we need to convert our 2.2 frame buffer into a 2.5 frame buffer for display on a
// TV

void CShaderDeviceDx8::SetHardwareGammaRamp(float fGamma, float fGammaTVRangeMin, float fGammaTVRangeMax,
											float fGammaTVExponent, bool bTVEnabled)
{
	...
	for(int i = 0; i < 256; i++)
	{
		...
		// Since the 360's sRGB read/write is a piecewise linear approximation, we need to correct for the difference in
		// gamma space here
		...
		if(IsX360()) // Should we also do this for the PS3?
		{
			...
			flSrgbGammaValue =
				SrgbLinearToGamma(flLinearValue); // ( flLinearValue <= 0.0031308f ) ? ( flLinearValue * 12.92f ) :
												  // ( 1.055f * powf( flLinearValue, ( 1.0f / 2.4f ) ) ) - 0.055f;
		...
		if(bTVEnabled)
		{
			...
			flCorrection =
				(flCorrection * (fGammaTVRangeMax - fGammaTVRangeMin) / 255.0f) + (fGammaTVRangeMin / 255.0f);
			flCorrection = clamp(flCorrection, 0.0f, 1.0f);
		}
...
//-----------------------------------------------------------------------------
IMesh *CShaderDeviceDx8::CreateStaticMesh(VertexFormat_t vertexFormat, const char *pTextureBudgetGroup,
										  IMaterial *pMaterial)
{
	LOCK_SHADERAPI();
...
//-----------------------------------------------------------------------------
IVertexBuffer *CShaderDeviceDx8::CreateVertexBuffer(ShaderBufferType_t type, VertexFormat_t fmt, int nVertexCount,
...
IIndexBuffer *CShaderDeviceDx8::CreateIndexBuffer(ShaderBufferType_t bufferType, MaterialIndexFormat_t fmt,
												  int nIndexCount, const char *pBudgetGroup)
{
	LOCK_SHADERAPI();
...
void CShaderDeviceDx8::SpewVideoInfo360(const CCommand &args)
{
	...
	Warning("display resolution: %dx%d %s\n", videoMode.dwDisplayWidth, videoMode.dwDisplayHeight,
			videoMode.fIsInterlaced ? "interlaced" : "progressive");
