...
-get the format for GetAdapterModeCount and EnumAdapterModes from somewhere (shaderapidx8.cpp, GetModeCount,
GetModeInfo) -record frame sync objects (allocframesyncobjects, free framesync objects, ForceHardwareSync) -Need to fix
ENVMAPMASKSCALE, BUMPOFFSET in lightmappedgeneric*.cpp and vertexlitgeneric*.cpp fix this:
...
ConVar mat_frame_sync_force_texture("mat_frame_sync_force_texture", "0", FCVAR_CHEAT,
...
//-----------------------------------------------------------------------------
typedef void (*StateCommitFunc_t)(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState,
								  DynamicState_t &currentState, bool bForce);
static void CommitSetViewports(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState,
							   DynamicState_t &currentState, bool bForce);
...
public:
	...
	virtual void ClearBuffers(bool bClearColor, bool bClearDepth, bool bClearStencil, int renderTargetWidth,
...
public:
	...
	void PerspectiveOffCenterX(double fovx, double aspect, double zNear, double zFar, double bottom, double top,
	...
	IMesh *GetDynamicMesh(IMaterial *pMaterial, int nHWSkinBoneCount, bool buffered, IMesh *pVertexOverride,
						  IMesh *pIndexOverride);
	IMesh *GetDynamicMeshEx(IMaterial *pMaterial, VertexFormat_t vertexFormat, int nHWSkinBoneCount, bool bBuffered,
	...
	ShaderAPITextureHandle_t CreateTexture(int width, int height, int depth, ImageFormat dstImageFormat,
										   int numMipLevels, int numCopies, int creationFlags, const char *pDebugName,
										   const char *pTextureGroupName);

	// Create a multi-frame texture (equivalent to calling "CreateTexture" multiple times, but more efficient)
	void CreateTextures(ShaderAPITextureHandle_t *pHandles, int count, int width, int height, int depth,
						ImageFormat dstImageFormat, int numMipLevels, int numCopies, int flags, const char *pDebugName,
						const char *pTextureGroupName);

	ShaderAPITextureHandle_t CreateDepthTexture(ImageFormat renderTargetFormat, int width, int height,
												const char *pDebugName, bool bTexture);

	void TexImage2D(int level, int cubeFaceID, ImageFormat dstFormat, int zOffset, int width, int height,
					ImageFormat srcFormat, bool bSrcIsTiled, void *imageData);

	void TexSubImage2D(int level, int cubeFaceID, int xOffset, int yOffset, int zOffset, int width, int height,
	...
	virtual void ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *data, ImageFormat dstFormat,
							int nDstStride);
	...
	void CopyRenderTargetToTextureEx(ShaderAPITextureHandle_t textureHandle, int nRenderTargetID,
									 Rect_t *pSrcRect = NULL, Rect_t *pDstRect = NULL);
	void CopyTextureToRenderTargetEx(int nRenderTargetID, ShaderAPITextureHandle_t textureHandle,
									 Rect_t *pSrcRect = NULL, Rect_t *pDstRect = NULL);
	void CopyRenderTargetToScratchTexture(ShaderAPITextureHandle_t srcHandle, ShaderAPITextureHandle_t dstHandle,
										  Rect_t *pSrcRect = NULL, Rect_t *pDstRect = NULL);

	virtual void LockRect(void **pOutBits, int *pOutPitch, ShaderAPITextureHandle_t texHandle, int mipmap, int x, int y,
						  int w, int h, bool bWrite, bool bRead);
	...
	void SetRenderState(D3DRENDERSTATETYPE state, DWORD val, bool bFlushBufferedPrimitivesIfChanged = false);
	...
	virtual bool IsRenderingMesh() const
	{
		return m_pRenderMesh != 0;
	}
	...
	bool GetTrueTypeGlyphs(HXUIFONT hFont, int numChars, wchar_t *pWch, int *pOffsetX, int *pOffsetY, int *pWidth,
						   int *pHeight, unsigned char *pRGBA, int *pRGBAOffset);
	ShaderAPITextureHandle_t CreateRenderTargetSurface(int width, int height, ImageFormat format,
													   const char *pDebugName, const char *pTextureGroupName);
	void PersistDisplay();
	bool PostQueuedTexture(const void *pData, int nSize, ShaderAPITextureHandle_t *pHandles, int nHandles, int nWidth,
						   int nHeight, int nDepth, int nMips, int *pRefCount);
	void *GetD3DDevice();
...
private:
	...
	// Minimized or device lost or device not reset causes transition from OK to LOST_DEVICE, during transition we must
	// release resources Minimized or device lost or device not reset causes transition from OtherAppInit to LOST_DEVICE
	...
	// Successful TryDeviceReset and !Other app init causes transition from NEEDS_RESET to OK, during transition we must
	// restore resources Successful TryDeviceReset and Other app init causes transition from NEEDS_RESET to OtherAppInit
	...
	void CopyBitsFromHostSurface(IDirect3DSurface *pSurfaceBits, const Rect_t &dstRect, unsigned char *pData,
	...
	//  Some shaders set constants based on rendering states, and some rendering states aren't updated until after a
	//  shader's already called Draw().
	//   So, for some functions that are state based, we save the constant we set and if the state changes between when
	//   it's set in the shader setup code
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderAPIDx8, IShaderAPI, SHADERAPI_INTERFACE_VERSION, g_ShaderAPIDX8)

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderAPIDx8, IShaderDevice, SHADER_DEVICE_INTERFACE_VERSION, g_ShaderAPIDX8)

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderAPIDx8, IDebugTextureInfo, DEBUG_TEXTURE_INFO_VERSION, g_ShaderAPIDX8)
...
static int AdjustUpdateRange(float const *pVec, void const *pOut, int numVecs, int *pSkip)
{
	...
	*pSkip = skip;
	...
	while(numVecs > 1 && !((pSrcLast[0] ^ pDstLast[0]) | (pSrcLast[1] ^ pDstLast[1]) | (pSrcLast[2] ^ pDstLast[2]) |
						   (pSrcLast[3] ^ pDstLast[3])))
	{
		pSrcLast -= 4;
		pDstLast -= 4;
		numVecs--;
...
void CShaderAPIDx8::AcquireInternalRenderTargets()
{
	...
	if(!m_pBackBufferSurfaceSRGB)
	{
		...
		m_pBackBufferSurfaceSRGB = g_TextureHeap.AllocRenderTargetSurface(
			backWidth, backHeight, (D3DFORMAT)MAKESRGBFMT(backBufferFormat), true, 0);
	}
#endif
...
void CShaderAPIDx8::ReleaseInternalRenderTargets()
{
	...
	if(m_pBackBufferSurface)
	{
		...
		int nRetVal = m_pBackBufferSurface->Release(
			0, "-B  CShaderAPIDx8::ReleaseInternalRenderTargets public release color buffer");
#else
		int nRetVal = m_pBackBufferSurface->Release();
#endif
		// Assert( nRetVal == 0 );
	...
	if(m_pZBufferSurface)
	{
		...
		// dxabstract's AddRef/Release have optional args to help track usage
		int nRetVal =
			m_pZBufferSurface->Release(0, "-B  CShaderAPIDx8::ReleaseInternalRenderTargets public release zbuffer");
#else
...
bool CShaderAPIDx8::RestorePersistedDisplay(bool bUseFrontBuffer)
{
	...
	const char *strVertexShaderProgram = " float4x4 matWVP : register(c0);"
										 " struct VS_IN"
										 " {"
	...
	const char *strPixelShaderProgram = " struct PS_IN"
										" {"
										" float2 TexCoord : TEXCOORD;"
										" };"
										" sampler detail;"
	...
	D3DVERTEXELEMENT9 VertexElements[3] = {{0, 0, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0},
	...
	HRESULT hr = D3DXCompileShader(strVertexShaderProgram, (UINT)strlen(strVertexShaderProgram), NULL, NULL, "main",
								   "vs_2_0", 0, &pShaderCode, &pErrorMsg, NULL);
	if(FAILED(hr))
	{
		return false;
	}
	...
	pErrorMsg = NULL;
	pShaderCode = NULL;
	hr = D3DXCompileShader(strPixelShaderProgram, (UINT)strlen(strPixelShaderProgram), NULL, NULL, "main", "ps_2_0", 0,
	...
	g_pShaderDeviceDx8->SetHardwareGammaRamp(mat_monitorgamma.GetFloat(), mat_monitorgamma_tv_range_min.GetFloat(),
...
#ifdef DX_TO_GL_ABSTRACTION
// Purposely always writing the new state (even if it's not changed), in case SetRenderStateConstInline() compiles away
// to nothing (it sometimes does)
...
// Commits viewports
//-----------------------------------------------------------------------------
static void CommitSetScissorRect(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState,
								 DynamicState_t &currentState, bool bForce)
{
	// Set the enable/disable renderstate

	bool bEnableChanged =
		desiredState.m_RenderState[D3DRS_SCISSORTESTENABLE] != currentState.m_RenderState[D3DRS_SCISSORTESTENABLE];
	if(bEnableChanged)
	{
...
inline void CShaderAPIDx8::SetScissorRect(const int nLeft, const int nTop, const int nRight, const int nBottom,
										  const bool bEnableScissor)
{
...
void CShaderAPIDx8::SetStandardVertexShaderConstants(float fOverbright)
{
	...
	*/
	...
	else
	{
		...
		standardVertexShaderConstant.Init(VERTEX_SHADER_LIGHTS, VERTEX_SHADER_LIGHTS + 5,
	...
	if(g_pHardwareConfig->Caps().m_SupportsPixelShaders)
	{
		// pixel shaders
		{
			...
			m_DynamicState.m_pIntegerPixelShaderConstant =
				new IntVector4D[g_pHardwareConfig->Caps().m_NumIntegerPixelShaderConstants];
			...
			if(nNumIntegerPixelShaderConstants)
			{
				...
				SetIntegerPixelShaderConstant(0, m_DesiredState.m_pIntegerPixelShaderConstant[0].Base(),
		...
		// vertex shaders
		{
			...
			m_DynamicState.m_pIntegerVertexShaderConstant =
				new IntVector4D[g_pHardwareConfig->Caps().m_NumIntegerVertexShaderConstants];
...
//-----------------------------------------------------------------------------
static void CommitVertexTextures(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState,
								 DynamicState_t &currentState, bool bForce)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		Texture_t *pTexture =
			(textureHandle != INVALID_SHADERAPI_TEXTURE_HANDLE) ? &g_ShaderAPIDX8.GetTexture(textureHandle) : NULL;
		...
		if(bForce || (currentVTState.m_BoundTexture != textureHandle))
		{
			...
			IDirect3DBaseTexture *pD3DTexture =
				(textureHandle >= 0) ? g_ShaderAPIDX8.GetD3DTexture(textureHandle) : NULL;
...
void CShaderAPIDx8::ResetDXRenderState(void)
{
	...
	// -disable_d3d9_hacks is for debugging. For example, the "CENT" driver hack thing causes the flashlight pass to
	// appear much brighter on NVidia drivers.
...
void CShaderAPIDx8::ResetRenderState(bool bFullReset)
{
	...
	else
	{
		...
		Q_memset(m_DynamicState.m_pVectorPixelShaderConstant, 0,
				 g_pHardwareConfig->Caps().m_NumPixelShaderConstants * sizeof(Vector4D));
		...
		Q_memset(m_DynamicState.m_pVectorVertexShaderConstant, 0,
				 g_pHardwareConfig->Caps().m_NumVertexShaderConstants * sizeof(Vector4D));
	...
	// Set the default compressed depth range written to dest alpha. Only need to compress it for 8bit alpha to get a
	// useful gradient.
	...
	m_DynamicState.m_Viewport.X = m_DynamicState.m_Viewport.Y = m_DynamicState.m_Viewport.Width =
		m_DynamicState.m_Viewport.Height = 0xFFFFFFFF;
	...
	viewport.Init(windowRect.left, windowRect.top, windowRect.right - windowRect.left,
...
// Sets the vertex
//-----------------------------------------------------------------------------
inline void CShaderAPIDx8::SetVertexDecl(VertexFormat_t vertexFormat, bool bHasColorMesh, bool bUsingFlex,
										 bool bUsingMorph)
{
...
// Gets the dynamic mesh
//-----------------------------------------------------------------------------
IMesh *CShaderAPIDx8::GetDynamicMesh(IMaterial *pMaterial, int nHWSkinBoneCount, bool buffered, IMesh *pVertexOverride,
									 IMesh *pIndexOverride)
{
									   ...
									   bool bBuffered, IMesh *pVertexOverride, IMesh *pIndexOverride)
{
	...
	LOCK_SHADERAPI();
	return MeshMgr()->GetDynamicMesh(pMaterial, vertexFormat, nHWSkinBoneCount, bBuffered, pVertexOverride,
									 pIndexOverride);
}
...
{
	pMaterial =
		((IMaterialInternal *)pMaterial)->GetRealTimeVersion(); // always work with the realtime version internally
...
void CShaderAPIDx8::DrawMesh(CMeshBase *pMesh)
{
	...
	SetVertexDecl(vertexFormat, m_pRenderMesh->HasColorMesh(), m_pRenderMesh->HasFlexMesh(),
				  m_pMaterial->IsUsingVertexID());
	CommitStateChanges();
...
void CShaderAPIDx8::DrawWithVertexAndIndexBuffers(void)
{
	...
	SetVertexDecl(vertexFormat, false /*m_pRenderMesh->HasColorMesh()*/, false /*m_pRenderMesh->HasFlexMesh()*/,
...
void CShaderAPIDx8::ForceHardwareSync(void)
{
...
#if !defined(_X360)
	// How do you query dx9 for how many frames behind the hardware is or, alternatively, how do you tell the hardware
	// to never be more than N frames behind? 1) The old QueryPendingFrameCount design was removed.  It was a simple
	// transaction with the driver through the GetDriverState, trivial for the drivers to lie.  We came up with a much
	// better scheme for tracking pending frames where the driver can not lie without a possible performance loss: use
	// the asynchronous query system with D3DQUERYTYPE_EVENT and data size 0.  When GetData returns S_OK for the query,
	// you know that frame has finished.
	...
	else if(m_pFrameSyncQueryObject[0])
	{
		...
		// Disable VCR mode here or else it'll screw up (and we don't really care if this part plays back in exactly the
		// same amount of time).
		VCRSetEnabled(false);
		...
		int waitIndex = ((m_currentSyncQuery + NUM_FRAME_SYNC_QUERIES) - (NUM_FRAME_SYNC_FRAMES_LATENCY + 1)) %
						NUM_FRAME_SYNC_QUERIES;
...
void CShaderAPIDx8::ExportTextureList()
{
	...
	if(IsPC() || !IsX360())
	{
		...
		m_nTextureMemoryUsedPicMip1 = 0;
		m_nTextureMemoryUsedPicMip2 = 0;
		for(ShaderAPITextureHandle_t hTexture = m_Textures.Head(); hTexture != m_Textures.InvalidIndex();
			hTexture = m_Textures.Next(hTexture))
		{
			...
			// Compute picmip memory usage
			{
				...
				if(tex.m_NumLevels > 1)
				{
					...
					{
						int topmipsize = ImageLoader::GetMemRequired(tex.GetWidth(), tex.GetHeight(), tex.GetDepth(),
						...
						{
							int othermipsizeRatio = ((tex.GetWidth() > 8) ? 2 : 1) * ((tex.GetHeight() > 8) ? 2 : 1) *
													((tex.GetDepth() > 8) ? 2 : 1);
		...
		//	if( imageFormat >= 0 )
		{
			VPROF_INCREMENT_GROUP_COUNTER(
				"TexGroup_frame_" TEXTURE_GROUP_RENDER_TARGET, COUNTER_GROUP_TEXTURE_PER_FRAME,
				//			ImageLoader::SizeInBytes( imageFormat ) * desc.Width * desc.Height );
		...
		hTexture = m_Textures.Next(hTexture))
	{
		...
		pXTextureList[numTextures].pFormatName =
			D3DFormatName(ImageLoader::ImageFormatToD3DFormat(tex.GetImageFormat()));
	...
	D3DSURFACE_DESC desc;
	m_pBackBufferSurface->GetDesc(&desc);
	int size = ImageLoader::GetMemRequired(desc.Width, desc.Height, 0, ImageLoader::D3DFormatToImageFormat(desc.Format),
										   false);
...
#if defined(_WIN32)
	// AMD PerfStudio integration: Call into D3D9.DLL's D3DPERF_BeginEvent() (this gets intercepted by PerfStudio even
	// in GL mode).
...
#if defined(_WIN32)
	// AMD PerfStudio integration: Call into D3D9.DLL's D3DPERF_EndEvent() (this gets intercepted by PerfStudio even in
	// GL mode).
...
void CShaderAPIDx8::IncrementPIXError()
{
	...
	{
		Warning("Source engine built with PIX instrumentation, but PIX doesn't seem to have been used to instantiate "
				"the game, which is necessary on PC.\n");
	}
#endif
}
...
void CShaderAPIDx8::ApplyZBias(const ShadowState_t &shaderState)
{
	...
	// GR - hack for R200
	bool bPS14Only =
		g_pHardwareConfig->Caps().m_SupportsPixelShaders_1_4 && !g_pHardwareConfig->Caps().m_SupportsPixelShaders_2_0;
	...
	// bias = (s * D3DRS_SLOPESCALEDEPTHBIAS) + D3DRS_DEPTHBIAS, where s is the maximum depth slope of the triangle
	// being rendered
...
void CShaderAPIDx8::ApplyTextureEnable(const ShadowState_t &state, int nSampler)
{
	...
	if(state.m_SamplerState[nSampler].m_TextureEnable)
	{
		...
		// Should not be necessary/possible (SetTextureState() calls D3D9/DXAbstract, so the calling thread must already
		// own the device.
		// LOCK_SHADERAPI();
...
VertexFormat_t CShaderAPIDx8::ComputeVertexUsage(int num, StateSnapshot_t *pIds) const
{
	...
	return MeshMgr()->ComputeVertexFormat(flags, VERTEX_MAX_TEXTURE_COORDINATES, texCoordSize, numBones, userDataSize);
}
											...
											DynamicState_t &currentState, bool bForce, int nFirstConstant, int nCount)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		bool bDifferentValue = bForce || (desiredState.m_pVectorVertexShaderConstant[nVar] !=
		...
		if(!bDifferentValue)
		{
			if(nCommitCount != 0)
			{
				...
				pDevice->SetVertexShaderConstantF(
					nFirstCommit, desiredState.m_pVectorVertexShaderConstant[nFirstCommit].Base(), nCommitCount);
	...
	{
		// flush range
		pDevice->SetVertexShaderConstantF(nFirstCommit, desiredState.m_pVectorVertexShaderConstant[nFirstCommit].Base(),
										  nCommitCount);
...
{
	unsigned int color = D3DCOLOR_ARGB(255, (int)(r * 255), (int)(g * 255), (int)(b * 255));
...
{
	unsigned int color = D3DCOLOR_ARGB((int)(a * 255), (int)(r * 255), (int)(g * 255), (int)(b * 255));
	if(color != m_DynamicState.m_ConstantColor)
	{
		m_DynamicState.m_ConstantColor = color;
		SetSupportedRenderState(D3DRS_TEXTUREFACTOR, color);
...
{
	Assert(c);
	unsigned int color = D3DCOLOR_ARGB(255, (int)(c[0] * 255), (int)(c[1] * 255), (int)(c[2] * 255));
	if(color != m_DynamicState.m_ConstantColor)
	{
		m_DynamicState.m_ConstantColor = color;
		SetSupportedRenderState(D3DRS_TEXTUREFACTOR, color);
...
{
	Assert(c);
	unsigned int color = D3DCOLOR_ARGB((int)(c[3] * 255), (int)(c[0] * 255), (int)(c[1] * 255), (int)(c[2] * 255));
	if(color != m_DynamicState.m_ConstantColor)
	{
		m_DynamicState.m_ConstantColor = color;
		SetSupportedRenderState(D3DRS_TEXTUREFACTOR, color);
...
#define ALLOW_FOR_FASTCLIPDUMPS 0
	...
	static ConVar shader_dumpfastclipprojectioncoords("shader_dumpfastclipprojectioncoords", "0", 0,
													  "dump fast clip projected matrix");
	if(shader_dumpfastclipprojectioncoords.GetBool())
		DevMsg("Fast clip plane projected coordinates: %f %f %f %f", clipPlane.x, clipPlane.y, clipPlane.z,
			   clipPlane.w);
#endif

	if((clipPlane.z * clipPlane.w) <= -0.4f) // a plane with (z*w) > -0.4 at this point is behind the camera and will
											 // cause graphical glitches. Toss it. (0.4 found through experimentation)
	{
#if(ALLOW_FOR_FASTCLIPDUMPS == 1)
		if(shader_dumpfastclipprojectioncoords.GetBool())
...
void CShaderAPIDx8::ApplySRGBReadState(int iTextureStage, bool bSRGBReadEnabled)
{
	...
	DWORD linearFormatBackup =
		pBindTexture->Format.dword[0]; // if we convert to srgb format, we need the original format for reverting. We
									   // only need the first DWORD of GPUTEXTURE_FETCH_CONSTANT.
	if(bSRGBReadEnabled)
	{
		pBindTexture->Format.SignX = pBindTexture->Format.SignY = pBindTexture->Format.SignZ =
			3; // convert to srgb format for the bind. This effectively emulates the old srgb read sampler state
	}
...
void CShaderAPIDx8::UpdatePixelFogColorConstant(void)
{
	...
	switch(GetPixelFogMode())
	{
		...
		case MATERIAL_FOG_LINEAR:
		{
			...
			{
				// since the fog color will assuredly get converted from linear to gamma, we should probably convert it
			...
			else
			{
				for(int i = 0; i != 3; ++i)
					fogColor[i] = GammaToLinear_HardwareSpecific(
						m_DynamicState
							.m_PixelFogColor[i]); // this is how water fog color has always been setup in the past
			}
...
void CShaderAPIDx8::SceneFogMode(MaterialFogMode_t fogMode)
{
	...
	if(m_SceneFogMode != fogMode)
	{
		...
		if(m_TransitionTable.CurrentShadowState())
		{
			...
			ApplyFogMode(m_TransitionTable.CurrentShadowState()->m_FogMode,
...
int CShaderAPIDx8::GetPixelFogCombo(void)
{
	if((m_SceneFogMode != MATERIAL_FOG_NONE) && ShouldUsePixelFogForMode(m_SceneFogMode))
		return m_SceneFogMode -
			   1; // PIXELFOGTYPE dynamic combos are shifted down one. MATERIAL_FOG_NONE is simulated by range fog with
				  // rigged parameters. Gets rid of a dynamic combo across the ps2x set
	else
		return MATERIAL_FOG_NONE;
}
...
bool CShaderAPIDx8::ShouldUsePixelFogForMode(MaterialFogMode_t fogMode)
{
	...
	switch(m_SceneFogMode)
	{
		case MATERIAL_FOG_LINEAR:
			return (
				g_pHardwareConfig
					->SupportsPixelShaders_2_b() && // lightmappedgeneric_ps20.fxc can't handle the instruction count
...
// Gamma correction of fog color, or not...
//-----------------------------------------------------------------------------
D3DCOLOR CShaderAPIDx8::ComputeGammaCorrectedFogColor(unsigned char r, unsigned char g, unsigned char b,
													  bool bSRGBWritesEnabled)
{
	...
	bool bLinearSpace = g_pHardwareConfig->Caps().m_bFogColorAlwaysLinearSpace ||
						(bSRGBWritesEnabled && (g_pHardwareConfig->Caps().m_bFogColorSpecifiedInLinearSpace ||
...
void CShaderAPIDx8::SetBooleanVertexShaderConstant(int var, int const *pVec, int numBools, bool bForce)
{
	...
	if(IsPC() && !bForce &&
	   memcmp(pVec, &m_DesiredState.m_pBooleanVertexShaderConstant[var], numBools * sizeof(BOOL)) == 0)
	{
		return;
	}
...
void CShaderAPIDx8::SetIntegerVertexShaderConstant(int var, int const *pVec, int numIntVecs, bool bForce)
{
	...
	if(IsPC() && !bForce &&
	   memcmp(pVec, &m_DesiredState.m_pIntegerVertexShaderConstant[var], numIntVecs * sizeof(IntVector4D)) == 0)
	{
		return;
	}
...
FORCEINLINE void CShaderAPIDx8::SetPixelShaderConstantInternal(int nStartConst, float const *pValues, int nNumConsts,
															   bool bForce)
{
	...
	if(IsPC())
	{
		...
		{
			int skip = 0;
			nNumConsts = AdjustUpdateRange(pValues, &m_DesiredState.m_pVectorPixelShaderConstant[nStartConst],
										   nNumConsts, &skip);
			if(!nNumConsts)
				return;
			nStartConst += skip;
...
template<class T>
FORCEINLINE T GetData(uint8 const *pData)
{
...
void CShaderAPIDx8::SetBooleanPixelShaderConstant(int var, int const *pVec, int numBools, bool bForce)
{
	...
	if(IsPC() && !bForce &&
	   memcmp(pVec, &m_DesiredState.m_pBooleanPixelShaderConstant[var], numBools * sizeof(BOOL)) == 0)
	{
		return;
	}
...
void CShaderAPIDx8::SetIntegerPixelShaderConstant(int var, int const *pVec, int numIntVecs, bool bForce)
{
	...
	if(IsPC() && !bForce &&
	   memcmp(pVec, &m_DesiredState.m_pIntegerPixelShaderConstant[var], numIntVecs * sizeof(IntVector4D)) == 0)
	{
		return;
	}
...
void CShaderAPIDx8::SetTextureState(Sampler_t sampler, ShaderAPITextureHandle_t hTexture, bool force)
{
	...
	if((samplerState.m_MinFilter != minFilter) || (samplerState.m_MagFilter != magFilter) ||
	   (samplerState.m_MipFilter != mipFilter) || (samplerState.m_UTexWrap != uTexWrap) ||
	   (samplerState.m_VTexWrap != vTexWrap) || (samplerState.m_WTexWrap != wTexWrap) ||
...
void CShaderAPIDx8::BindVertexTexture(VertexTextureSampler_t nStage, ShaderAPITextureHandle_t textureHandle)
{
	...
	LOCK_SHADERAPI();
	ADD_VERTEX_TEXTURE_FUNC(COMMIT_PER_PASS, COMMIT_VERTEX_SHADER, CommitVertexTextures, nStage, m_BoundTexture,
							textureHandle);
}
	...
	if(IsPC() || !IsX360())
	{
		if(isCubeMap)
		{
			...
			for(int i = 0; i < numLevels; ++i)
			{
				...
				textureData.m_SizeBytes +=
					6 * ImageLoader::GetMemRequired(desc.Width, desc.Height, 1, textureData.GetImageFormat(), false);
		...
		else if(isVolumeTexture)
		{
			...
			for(int i = 0; i < numLevels; ++i)
			{
				...
				HRESULT hr = pTex->GetLevelDesc(i, &desc);
				Assert(!FAILED(hr));
				textureData.m_SizeBytes += ImageLoader::GetMemRequired(desc.Width, desc.Height, desc.Depth,
		...
		else
		{
			...
			for(int i = 0; i < numLevels; ++i)
			{
				...
				Assert(!FAILED(hr));
				textureData.m_SizeBytes +=
					ImageLoader::GetMemRequired(desc.Width, desc.Height, 1, textureData.GetImageFormat(), false);
				textureData.m_SizeTexels += desc.Width * desc.Height;
			}
		}
	}
...
ShaderAPITextureHandle_t CShaderAPIDx8::CreateDepthTexture(ImageFormat renderTargetFormat, int width, int height,
														   const char *pDebugName, bool bTexture)
{
	LOCK_SHADERAPI();
	...
	if(!bTexture)
	{
		...
		hr = Dx9Device()->CreateDepthStencilSurface(width, height, format, multisampleType, 0, TRUE,
...
ShaderAPITextureHandle_t CShaderAPIDx8::CreateTexture(int width, int height, int depth, ImageFormat dstImageFormat,
													  int numMipLevels, int numCopies, int creationFlags,
													  const char *pDebugName, const char *pTextureGroupName)
{
	ShaderAPITextureHandle_t handle = 0;
	CreateTextures(&handle, 1, width, height, depth, dstImageFormat, numMipLevels, numCopies, creationFlags, pDebugName,
				   pTextureGroupName);
	return handle;
}

void CShaderAPIDx8::CreateTextures(ShaderAPITextureHandle_t *pHandles, int count, int width, int height, int depth,
								   ...
								   const char *pDebugName, const char *pTextureGroupName)
{
	...
	usSetFlags |=
		(IsPosix() || (creationFlags & (TEXTURE_CREATE_DYNAMIC | TEXTURE_CREATE_MANAGED))) ? Texture_t::IS_LOCKABLE : 0;
	...
	for(int idxFrame = 0; idxFrame < count; ++idxFrame)
	{
		...
		if(numCopies <= 1)
		{
			...
			pTexture->m_NumCopies = 1;
			pD3DTex =
				CreateD3DTexture(width, height, depth, dstImageFormat, numMipLevels, creationFlags, (char *)pDebugName);
...
void CShaderAPIDx8::WriteTextureToFile(ShaderAPITextureHandle_t hTexture, const char *szFileName)
{
...
#if !defined(_X360) // TODO: x360 version
	{
		...
		hr = Dx9Device()->CreateOffscreenPlainSurface(surfaceDesc.Width, surfaceDesc.Height, surfaceDesc.Format,
	...
	TGAWriter::WriteTGAFile(szFileName, surfaceDesc.Width, surfaceDesc.Height, pTexInt->GetImageFormat(),
							(const uint8 *)lockedRect.pBits, lockedRect.Pitch);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetRenderTargetEx(int nRenderTargetID, ShaderAPITextureHandle_t colorTextureHandle,
...
// Changes the render target
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetRenderTarget(ShaderAPITextureHandle_t colorTextureHandle,
									ShaderAPITextureHandle_t depthTextureHandle)
{
	LOCK_SHADERAPI();
...
bool CShaderAPIDx8::TexLock(int level, int cubeFaceID, int xOffset, int yOffset, int width, int height,
							CPixelWriter &writer)
{
	tmZone(TELEMETRY_LEVEL2, TMZF_NONE, "%s", __FUNCTION__);
	...
	bool bOK = LockTexture(hTexture, tex.m_CurrentCopy, pTexture, level, (D3DCUBEMAP_FACES)cubeFaceID, xOffset, yOffset,
...
void CShaderAPIDx8::TexUnlock()
{
	...
	if(m_ModifyTextureLockedLevel >= 0)
	{
		...
		UnlockTexture(GetModifyTextureHandle(), tex.m_CurrentCopy, GetModifyTexture(), m_ModifyTextureLockedLevel,
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::TexImage2D(int level, int cubeFaceID, ImageFormat dstFormat, int z, int width, int height,
							   ImageFormat srcFormat, bool bSrcIsTiled, void *pSrcData)
{
	LOCK_SHADERAPI();
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::TexSubImage2D(int level, int cubeFaceID, int xOffset, int yOffset, int zOffset, int width,
								  int height, ImageFormat srcFormat, int srcStride, bool bSrcIsTiled, void *pSrcData)
{
	LOCK_SHADERAPI();
	...
	switch(texFilterMode)
	{
		...
		case SHADER_TEXFILTERMODE_ANISOTROPIC:
			GetTexture(hModifyTexture).m_MagFilter =
				g_pHardwareConfig->Caps().m_bSupportsMagAnisotropicFiltering ? D3DTEXF_ANISOTROPIC : D3DTEXF_LINEAR;
			break;
		default:
...
{
	LOCK_SHADERAPI();
	unsigned int ambient = D3DCOLOR_ARGB(255, (int)(r * 255), (int)(g * 255), (int)(b * 255));
...
void CShaderAPIDx8::CopyRenderTargetToTextureEx(ShaderAPITextureHandle_t textureHandle, int nRenderTargetID,
												Rect_t *pSrcRect, Rect_t *pDstRect)
{
	...
	if(tryblit)
	{
		...
		hr = Dx9Device()->StretchRect(pRenderTargetSurface, RectToRECT(pSrcRect, srcRect), pDstSurf,
	...
	if(pSrcRect)
	{
		...
		// resolve has no stretching ability, and we can only compensate when doing a resolve to a whole texture larger
		// than the source
	...
	if((srcWidth != destWidth) || (srcHeight != destHeight))
	{
		...
		// All outside code still thinks this texture is it's original size. And it still owns enough memory to go back
	...
	else
	{
		...
		pD3DTexture->Format.Size.TwoD.Height =
			pTexture->GetHeight() - 1; // no idea why they store it as size-1, but they do
	}

	// if we convert to srgb format, we need the original format for reverting. We only need the first DWORD of
...
void CShaderAPIDx8::CopyRenderTargetToScratchTexture(ShaderAPITextureHandle_t srcRt, ShaderAPITextureHandle_t dstTex,
													 Rect_t *pSrcRect, Rect_t *pDstRect)
{
	LOCK_SHADERAPI();
...
// the lock fails.
//-------------------------------------------------------------------------
void CShaderAPIDx8::LockRect(void **pOutBits, int *pOutPitch, ShaderAPITextureHandle_t texHandle, int mipmap, int x,
							 int y, int w, int h, bool bWrite, bool bRead)
{
	LOCK_SHADERAPI();
...
static int FindCommonMipmapRange(int *pOutSrcFine, int *pOutDstFine, Texture_t *pSrcTex, Texture_t *pDstTex)
{
	...
	TextureExtents_t *pLarger = NULL, *pSmaller = NULL;
...
void CShaderAPIDx8::CopyTextureToTexture(ShaderAPITextureHandle_t srcTex, ShaderAPITextureHandle_t dstTex)
{
	...
	if(mipmapCount <= 0)
	{
		...
		// Warning( "Attempted to copy textures that had non-overlapping mipmap pyramids. This has failed and no copy
		// has taken place.\n" );
		return;
	}
...
void CShaderAPIDx8::CopyTextureToRenderTargetEx(int nRenderTargetID, ShaderAPITextureHandle_t textureHandle,
												Rect_t *pSrcRect, Rect_t *pDstRect)
{
	LOCK_SHADERAPI();
	...
	if(tryblit)
	{
		...
		RECT srcRect, dstRect;
		hr = Dx9Device()->StretchRect(pDstSurf, RectToRECT(pSrcRect, srcRect), pRenderTargetSurface,
									  RectToRECT(pDstRect, dstRect), D3DTEXF_LINEAR);
		Assert(!FAILED(hr));
	}
...
void CShaderAPIDx8::SpewBoardState()
{
	...
	*/
	...
	sprintf(buf, "Depth States: ZFunc %d, ZWrite %d, ZEnable %d, ZBias %d\n", boardState.m_ZFunc,
			boardState.m_ZWriteEnable, boardState.m_ZEnable, boardState.m_ZBias);
	Plat_DebugString(buf);
	sprintf(buf, "Cull Enable %d Cull Mode %d Color Write %d Fill %d Const Color Mod %d sRGBWriteEnable %d\n",
			boardState.m_CullEnable, m_DynamicState.m_CullMode, boardState.m_ColorWriteEnable, boardState.m_FillMode,
	...
	sprintf(buf, "Blend States: Blend Enable %d Test Enable %d Func %d SrcBlend %d (%s) DstBlend %d (%s)\n",
			boardState.m_AlphaBlendEnable, boardState.m_AlphaTestEnable, boardState.m_AlphaFunc, boardState.m_SrcBlend,
			BlendModeToString(boardState.m_SrcBlend), boardState.m_DestBlend,
			BlendModeToString(boardState.m_DestBlend));
	Plat_DebugString(buf);
	int len = sprintf(buf, "Alpha Ref %d, Lighting: %d, Ambient Color %x, LightsEnabled ", boardState.m_AlphaRef,
					  boardState.m_Lighting, m_DynamicState.m_Ambient);
	...
	sprintf(buf, "Pass Vertex Usage: %llx Pixel Shader %p Vertex Shader %p\n", boardShaderState.m_VertexUsage,
			ShaderManager()->GetCurrentPixelShader(), ShaderManager()->GetCurrentVertexShader());
	Plat_DebugString(buf);

	// REGRESSED!!!!
	/*
	D3DXMATRIX* m = &GetTransform(MATERIAL_MODEL);
	sprintf(buf,"WorldMat [%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f
	...
	sprintf(buf,"WorldMat2 [%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f
	%4.3f]\n", m->m[0][0],	m->m[0][1],	m->m[0][2],	m->m[0][3], m->m[1][0],	m->m[1][1],	m->m[1][2],	m->m[1][3],
		m->m[2][0],	m->m[2][1],	m->m[2][2],	m->m[2][3],
		m->m[3][0], m->m[3][1], m->m[3][2], m->m[3][3] );
	Plat_DebugString(buf);
	...
	*/
	...
	sprintf(buf, "Viewport (%d %d) [%d %d] %4.3f %4.3f\n", m_DynamicState.m_Viewport.X, m_DynamicState.m_Viewport.Y,
			m_DynamicState.m_Viewport.Width, m_DynamicState.m_Viewport.Height, m_DynamicState.m_Viewport.MinZ,
			m_DynamicState.m_Viewport.MaxZ);
	Plat_DebugString(buf);
	...
	for(i = 0; i < MAX_TEXTURE_STAGES; ++i)
	{
		sprintf(buf, "Stage %d :\n", i);
		Plat_DebugString(buf);
		sprintf(buf, "   Color Op: %d (%s) Color Arg1: %d (%s)", boardState.m_TextureStage[i].m_ColorOp,
				TextureOpToString(boardState.m_TextureStage[i].m_ColorOp), boardState.m_TextureStage[i].m_ColorArg1,
		...
		Plat_DebugString(buf);
		sprintf(buf, "   Alpha Op: %d (%s) Alpha Arg1: %d (%s)", boardState.m_TextureStage[i].m_AlphaOp,
				TextureOpToString(boardState.m_TextureStage[i].m_AlphaOp), boardState.m_TextureStage[i].m_AlphaArg1,
	...
	{
		sprintf(buf, "	Texture Enabled: %d Bound Texture: %d UWrap: %d VWrap: %d\n", SamplerState(i).m_TextureEnable,
				GetBoundTextureBindId((Sampler_t)i), SamplerState(i).m_UTexWrap, SamplerState(i).m_VTexWrap);
		Plat_DebugString(buf);
		sprintf(buf, "	Mag Filter: %d Min Filter: %d Mip Filter: %d\n", SamplerState(i).m_MagFilter,
				SamplerState(i).m_MinFilter, SamplerState(i).m_MipFilter);
...
void CShaderAPIDx8::CacheWorldSpaceCameraPosition()
{
	...
	m_WorldSpaceCameraPositon[0] = -(view(3, 0) * view(0, 0) + view(3, 1) * view(0, 1) + view(3, 2) * view(0, 2));
	m_WorldSpaceCameraPositon[1] = -(view(3, 0) * view(1, 0) + view(3, 1) * view(1, 1) + view(3, 2) * view(1, 2));
	m_WorldSpaceCameraPositon[2] = -(view(3, 0) * view(2, 0) + view(3, 1) * view(2, 1) + view(3, 2) * view(2, 2));
...
void CShaderAPIDx8::ComputePolyOffsetMatrix(const D3DXMATRIX &matProjection, D3DXMATRIX &matProjectionOffset)
{
	...
	float offsetVal = -1.0f * (m_DesiredState.m_Viewport.MaxZ - m_DesiredState.m_Viewport.MinZ) / 16384.0f;
...
void CShaderAPIDx8::SetBumpEnvMatrix(TextureStage_t textureStage, float m00, float m01, float m10, float m11)
{
	...
	if(textureStageState.m_BumpEnvMat00 != m00 || textureStageState.m_BumpEnvMat01 != m01 ||
	   textureStageState.m_BumpEnvMat10 != m10 || textureStageState.m_BumpEnvMat11 != m11)
	{
...
// for shaders to set vertex shader constants. returns a packed state which can be used to set the dynamic combo
int CShaderAPIDx8::GetPackedDeformationInformation(int nMaskOfUnderstoodDeformations, float *pConstantValuesOut,
...
{
	// Check for identity...
	if((fabs(m[0] - 1.0f) < 1e-3) && (fabs(m[5] - 1.0f) < 1e-3) && (fabs(m[10] - 1.0f) < 1e-3) &&
	   (fabs(m[15] - 1.0f) < 1e-3) && (fabs(m[1]) < 1e-3) && (fabs(m[2]) < 1e-3) && (fabs(m[3]) < 1e-3) &&
	   (fabs(m[4]) < 1e-3) && (fabs(m[6]) < 1e-3) && (fabs(m[7]) < 1e-3) && (fabs(m[8]) < 1e-3) &&
	   (fabs(m[9]) < 1e-3) && (fabs(m[11]) < 1e-3) && (fabs(m[12]) < 1e-3) && (fabs(m[13]) < 1e-3) &&
	   (fabs(m[14]) < 1e-3))
	{
		LoadIdentity();
		return;
							  ...
							  DynamicState_t &currentState, bool bForce)
{
	...
	CommitVertexShaderConstantRange(pDevice, desiredState, currentState, bForce, VERTEX_SHADER_FLEX_WEIGHTS,
...
void CShaderAPIDx8::PerspectiveOffCenterX(double fovx, double aspect, double zNear, double zFar, double bottom,
										  double top, double left, double right)
{
	if(MatrixIsChanging())
	{
		...
		Assert(m_CurrStack == MATERIAL_PROJECTION);
		D3DXMATRIX rh;
		D3DXMatrixPerspectiveOffCenterRH(&rh, flFrontPlaneLeft, flFrontPlaneRight, flFrontPlaneBottom, flFrontPlaneTop,
										 zNear, zFar);
		m_pMatrixStack[m_CurrStack]->MultMatrixLocal(&rh);
		UpdateMatrixTransform();
	}
...
{
	bool bUsingZBiasProjectionMatrix = !g_pHardwareConfig->Caps().m_ZBiasAndSlopeScaledDepthBiasSupported &&
...
static void printmat4x4(char *label, float *m00)
{
	...
	for(int n = 0; n < 4; n++)
	{
		...
		GLMPRINTF(("-M-    [ %7.4f %7.4f %7.4f %7.4f ] T=> [ %7.4f %7.4f %7.4f %7.4f ]", row[0], row[1], row[2], row[3],
...
void CShaderAPIDx8::SetVertexShaderViewProj()
{
	...
	if(g_pHardwareConfig->Caps().m_SupportsPixelShaders)
	{
		...
		transpose =
			GetTransform(MATERIAL_VIEW) * GetTransform(MATERIAL_PROJECTION); // Get the non-FastClip projection matrix
		D3DXMatrixTranspose(&transpose, &transpose);
...
void CShaderAPIDx8::SetVertexShaderModelViewProjAndModelView(void)
{
	...
	if(g_pHardwareConfig->Caps().m_SupportsPixelShaders)
	{
		...
		D3DXMatrixMultiply(&transpose, &modelView,
						   &GetTransform(MATERIAL_PROJECTION)); // Get the non-FastClip projection matrix
		D3DXMatrixTranspose(&transpose, &transpose);
...
void CShaderAPIDx8::CommitVertexShaderTransforms()
{
	...
	// GLMPRINTF(( "-M-     viewChanged=%s  projChanged=%s  modelChanged = %s in CommitVertexShaderTransforms",
	// viewChanged?"Y":"N",projChanged?"Y":"N",modelChanged?"Y":"N" ));
	if(viewChanged)
	{
...
void CShaderAPIDx8::CommitVertexShaderLighting()
{
	   ...
	   g_pHardwareConfig->NumBooleanVertexShaderConstants() > 0)
	{
		...
		SetBooleanVertexShaderConstant(VERTEX_SHADER_LIGHT_ENABLE_BOOL_CONST, nLightEnable,
...
void CShaderAPIDx8::CommitUserClipPlanes(bool bUsingFixedFunction)
{
	...
	{
		// fixed function clip planes are in world space, vertex shader clip planes are in clip space, so we need to
		// update every clip plane whenever there's a flip
		m_DynamicState.m_UserClipPlaneChanged = (1 << g_pHardwareConfig->MaxUserClipPlanes()) - 1;
...
#ifndef _DEBUG
	if(m_DynamicState.m_UserClipPlaneChanged & m_DynamicState.m_UserClipPlaneEnabled &
...
#endif
	{
		...
		if(bUsingFixedFunction)
		{
			if(m_DynamicState.m_bUserClipTransformOverride)
			{
				...
				// Since GetUserClipTransform() returns the view matrix if a user supplied transform doesn't exist, the
				// general solution to this should be to transform the user transform by the inverse view matrix Since
				// we don't know if the user clip is invertable, we'll premultiply by inverse view and cross our fingers
				...
				worldToProjectionInvTrans =
					viewInverse * GetUserClipTransform(); // taking a cue from the multiplication below, multiplication
														  // goes left into right
...
// Buffer clear
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ClearBuffers(bool bClearColor, bool bClearDepth, bool bClearStencil, int renderTargetWidth,
								 int renderTargetHeight)
{
	LOCK_SHADERAPI();
	if(ShaderUtil()->GetConfig().m_bSuppressRendering)
...
IDirect3DSurface *CShaderAPIDx8::GetFrontBufferImage(ImageFormat &format)
{
	...
	hr =
		Dx9Device()->CreateOffscreenPlainSurface(w, h, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &pFullScreenSurfaceBits, NULL);
	if(FAILED(hr))
		return 0;
	...
	hr = Dx9Device()->CreateOffscreenPlainSurface(windowWidth, windowHeight, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH,
												  &pSurfaceBits, NULL);
	Assert(hr == D3D_OK);
...
// Lets the shader know about the full-screen texture so it can
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetLinearToGammaConversionTextures(ShaderAPITextureHandle_t hSRGBWriteEnabledTexture,
													   ShaderAPITextureHandle_t hIdentityTexture)
{
	LOCK_SHADERAPI();
...
IDirect3DSurface *CShaderAPIDx8::GetBackBufferImageHDR(Rect_t *pSrcRect, Rect_t *pDstRect, ImageFormat &format)
{
	...
	filter = ((pDstRect->width != pSrcRect->width) || (pDstRect->height != pSrcRect->height)) ? D3DTEXF_LINEAR
																							  : D3DTEXF_NONE;
	   ...
	   (pDstRect->y + pDstRect->height <= SMALL_BACK_BUFFER_SURFACE_HEIGHT))
	{
		...
		{
			hr = Dx9Device()->CreateRenderTarget(SMALL_BACK_BUFFER_SURFACE_WIDTH, SMALL_BACK_BUFFER_SURFACE_HEIGHT,
	...
	else
	{
		...
		hr = Dx9Device()->CreateRenderTarget(desc.Width, desc.Height, desc.Format, D3DMULTISAMPLE_NONE, 0, TRUE,
											 &pTmpSurface, NULL);
		if(FAILED(hr))
			goto CleanUp;
	...
	hr = Dx9Device()->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, D3DPOOL_SYSTEMMEM,
												  &pSurfaceBits, NULL);
	if(FAILED(hr))
		goto CleanUp;
...
IDirect3DSurface *CShaderAPIDx8::GetBackBufferImage(Rect_t *pSrcRect, Rect_t *pDstRect, ImageFormat &format)
{
	...
	else
	{
		...
		D3DTEXTUREFILTERTYPE filter = ((pDstRect->width != pSrcRect->width) || (pDstRect->height != pSrcRect->height))
										  ? D3DTEXF_LINEAR
										  : D3DTEXF_NONE;
	...
	// Create a buffer the same size and format
	hr = Dx9Device()->CreateOffscreenPlainSurface(tmpDesc.Width, tmpDesc.Height, desc.Format, D3DPOOL_SYSTEMMEM,
												  &pSurfaceBits, NULL);
	if(FAILED(hr))
	{
...
// Copy bits from a host-memory surface
//-----------------------------------------------------------------------------
void CShaderAPIDx8::CopyBitsFromHostSurface(IDirect3DSurface *pSurfaceBits, const Rect_t &dstRect, unsigned char *pData,
											ImageFormat srcFormat, ImageFormat dstFormat, int nDstStride)
{
	// Copy out the bits...
	RECT rect;
	...
	if(!FAILED(hr))
	{
		...
		ShaderUtil()->ConvertImageFormat((unsigned char *)pImage, srcFormat, pData, dstFormat, dstRect.width,
										 dstRect.height, lockedRect.Pitch, nDstStride);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *pData, ImageFormat dstFormat,
							   int nDstStride)
{
	LOCK_SHADERAPI();
	Assert(pDstRect);
...
void CShaderAPIDx8::Bind(IMaterial *pMaterial)
{
	...
	{
		bMaterialChanged = (m_pMaterial != pMatInt) || (m_pMaterial && m_pMaterial->InMaterialPage()) ||
...
void CShaderAPIDx8::WriteHitRecord()
{
	...
	if(m_SelectionNames.Count() && (m_SelectionMinZ != FLT_MAX))
	{
		...
		*m_pCurrSelectionRecord++ = m_SelectionNames.Count();
		...
		// [ ALSO: As of Nov 2011, VS2010 exhibits a debug build code-gen bug if we cast the result to int32 instead of
		// uint32 ]
...
int CShaderAPIDx8::GetCurrentLightCombo(void) const
{
	...
	if(m_DynamicState.m_AmbientLightCube[0][0] == 0.0f && m_DynamicState.m_AmbientLightCube[0][1] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[0][2] == 0.0f && m_DynamicState.m_AmbientLightCube[1][0] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[1][1] == 0.0f && m_DynamicState.m_AmbientLightCube[1][2] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[2][0] == 0.0f && m_DynamicState.m_AmbientLightCube[2][1] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[2][2] == 0.0f && m_DynamicState.m_AmbientLightCube[3][0] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[3][1] == 0.0f && m_DynamicState.m_AmbientLightCube[3][2] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[4][0] == 0.0f && m_DynamicState.m_AmbientLightCube[4][1] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[4][2] == 0.0f && m_DynamicState.m_AmbientLightCube[5][0] == 0.0f &&
	   m_DynamicState.m_AmbientLightCube[5][1] == 0.0f && m_DynamicState.m_AmbientLightCube[5][2] == 0.0f)
	{
...
void CShaderAPIDx8::SetPixelShaderFogParams(int reg, ShaderFogMode_t fogMode)
{
	m_DelayedShaderConstants.iPixelShaderFogParams =
		reg; // save it off in case the ShaderFogMode_t disables fog. We only find out later.
	float fogParams[4];
	...
	{
		// emulating MATERIAL_FOG_NONE by setting the parameters so that CalcRangeFog() always returns 0. Gets rid of a
...
void CShaderAPIDx8::SetFlashlightStateEx(const FlashlightState_t &state, const VMatrix &worldToTexture,
										 ITexture *pFlashlightDepthTexture)
{
	LOCK_SHADERAPI();
...
const FlashlightState_t &CShaderAPIDx8::GetFlashlightStateEx(VMatrix &worldToTexture,
															 ITexture **ppFlashlightDepthTexture) const
{
	worldToTexture = m_FlashlightWorldToTexture;
...
bool CShaderAPIDx8::SupportsMSAAMode(int nMSAAMode)
{
	...
	return (D3D_OK ==
			D3D()->CheckDeviceMultiSampleType(m_DisplayAdapter, m_DeviceType, m_PresentParameters.BackBufferFormat,
											  m_PresentParameters.Windowed, ComputeMultisampleType(nMSAAMode), NULL));
}
...
bool CShaderAPIDx8::SupportsCSAAMode(int nNumSamples, int nQualityLevel)
{
	...
	HRESULT hr = D3D()->CheckDeviceMultiSampleType(m_DisplayAdapter, m_DeviceType, m_PresentParameters.BackBufferFormat,
...
void CShaderAPIDx8::SetToneMappingScaleLinear(const Vector &scale)
{
	if(g_pHardwareConfig->SupportsPixelShaders_2_0())
	{
		...
		if(HardwareConfig()->GetHDRType() == HDR_TYPE_INTEGER && m_TransitionTable.CurrentShadowState())
		{
			...
			ApplyFogMode(m_TransitionTable.CurrentShadowState()->m_FogMode, mode_uses_srgb,
						 m_TransitionTable.CurrentShadowState()->m_bDisableFogGammaCorrection);
		}
	}
}
...
void CShaderAPIDx8::ClearStencilBufferRectangle(int xmin, int ymin, int xmax, int ymax, int value)
{
	LOCK_SHADERAPI();
	D3DRECT clear;
	clear.x1 = xmin;
...
//-----------------------------------------------------------------------------
#if defined(_X360)
bool CShaderAPIDx8::GetTrueTypeFontMetrics(HXUIFONT hFont, XUIFontMetrics *pFontMetrics,
...
//-----------------------------------------------------------------------------
#if defined(_X360)
bool CShaderAPIDx8::GetTrueTypeGlyphs(HXUIFONT hFont, int numChars, wchar_t *pWch, int *pOffsetX, int *pOffsetY,
									  int *pWidth, int *pHeight, unsigned char *pRGBA, int *pRGBAOffset)
{
	if(!hFont)
		return false;
	...
	for(int i = 0; i < numChars; i++)
	{
		// FIXME: the drawRect params don't make much sense (should use "(xCursor+pWidth[i]), pHeight[i]", but then some
		// characters disappear!)
		XUIRect drawRect = XUIRect(xCursor + pOffsetX[i], pOffsetY[i], rtWidth, rtHeight);
		wchar_t text[2] = {pWch[i], 0};
		XuiDrawText(m_hDC, text, XUI_FONT_STYLE_NORMAL | XUI_FONT_STYLE_SINGLE_LINE | XUI_FONT_STYLE_NO_WORDWRAP, 0,
					&drawRect);
		xCursor += pWidth[i];
	}
...
//-----------------------------------------------------------------------------
#if defined(_X360)
ShaderAPITextureHandle_t CShaderAPIDx8::CreateRenderTargetSurface(int width, int height, ImageFormat format,
																  const char *pDebugName, const char *pTextureGroupName)
{
	LOCK_SHADERAPI();
	...
	pTexture->GetRenderTargetSurface(true) =
		g_TextureHeap.AllocRenderTargetSurface(width, height, (D3DFORMAT)MAKESRGBFMT(actualFormat));
...
void CShaderAPIDx8::WriteShaderConstantsToGPU()
{
	...
	if(m_MaxVectorVertexShaderConstant)
	{
		if(m_bGPUOwned)
		{
			...
			memcpy(pCommandBufferData, m_DesiredState.m_pVectorVertexShaderConstant[0].Base(),
				   numVectors * (sizeof(float) * 4));
			Dx9Device()->GpuEndVertexShaderConstantF4();
		}
		else
		{
			Dx9Device()->SetVertexShaderConstantF(0, m_DesiredState.m_pVectorVertexShaderConstant[0].Base(),
												  m_MaxVectorVertexShaderConstant);
		}

		memcpy(m_DynamicState.m_pVectorVertexShaderConstant[0].Base(),
			   m_DesiredState.m_pVectorVertexShaderConstant[0].Base(),
			   m_MaxVectorVertexShaderConstant * 4 * sizeof(float));
		m_MaxVectorVertexShaderConstant = 0;
	}
	...
	if(m_MaxVectorPixelShaderConstant)
	{
		if(m_bGPUOwned)
		{
			...
			memcpy(pCommandBufferData, m_DesiredState.m_pVectorPixelShaderConstant[0].Base(),
				   numVectors * (sizeof(float) * 4));
			Dx9Device()->GpuEndPixelShaderConstantF4();
		}
		else
		{
			Dx9Device()->SetPixelShaderConstantF(0, m_DesiredState.m_pVectorPixelShaderConstant[0].Base(),
												 m_MaxVectorPixelShaderConstant);
		}

		memcpy(m_DynamicState.m_pVectorPixelShaderConstant[0].Base(),
			   m_DesiredState.m_pVectorPixelShaderConstant[0].Base(),
			   m_MaxVectorPixelShaderConstant * 4 * sizeof(float));
		m_MaxVectorPixelShaderConstant = 0;
	}
	...
	{
		Dx9Device()->SetVertexShaderConstantB(0, m_DesiredState.m_pBooleanVertexShaderConstant,
											  m_MaxBooleanVertexShaderConstant);
		memcpy(m_DynamicState.m_pBooleanVertexShaderConstant, m_DesiredState.m_pBooleanVertexShaderConstant,
			   m_MaxBooleanVertexShaderConstant * sizeof(BOOL));
		m_MaxBooleanVertexShaderConstant = 0;
	}
	if(m_MaxIntegerVertexShaderConstant)
	{
		...
		memcpy(m_DynamicState.m_pIntegerVertexShaderConstant[0].Base(),
			   m_DesiredState.m_pIntegerVertexShaderConstant[0].Base(),
			   m_MaxIntegerVertexShaderConstant * sizeof(IntVector4D));
		m_MaxIntegerVertexShaderConstant = 0;
	}

	if(m_MaxBooleanPixelShaderConstant)
	{
		Dx9Device()->SetPixelShaderConstantB(0, m_DesiredState.m_pBooleanPixelShaderConstant,
											 m_MaxBooleanPixelShaderConstant);
		memcpy(m_DynamicState.m_pBooleanPixelShaderConstant, m_DesiredState.m_pBooleanPixelShaderConstant,
			   m_MaxBooleanPixelShaderConstant * sizeof(BOOL));
		m_MaxBooleanPixelShaderConstant = 0;
	}
...
#if defined(_X360)
bool CShaderAPIDx8::PostQueuedTexture(const void *pData, int nDataSize, ShaderAPITextureHandle_t *pHandles,
									  int numHandles, int nWidth, int nHeight, int nDepth, int numMips, int *pRefCount)
{
...
ConVar r_enable_gpr_allocations("r_enable_gpr_allocations", "1", 0,
								"Enable usage of IDirect3DDevice9::SetShaderGPRAllocation()",
								r_enable_gpr_allocations_callback);

static void CommitShaderGPRs(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState,
							 DynamicState_t &currentState, bool bForce)
{
	if(desiredState.m_iVertexShaderGPRAllocation != currentState.m_iVertexShaderGPRAllocation)
	{
		pDevice->SetShaderGPRAllocation(0, desiredState.m_iVertexShaderGPRAllocation,
										128 - desiredState.m_iVertexShaderGPRAllocation);
...
void CShaderAPIDx8::BindVertexBuffer(int streamID, IVertexBuffer *pVertexBuffer, int nOffsetInBytes, int nFirstVertex,
									 int nVertexCount, VertexFormat_t fmt, int nRepetitions)
{
	LOCK_SHADERAPI();
...
bool CShaderAPIDx8::SetRenderTargetInternalXbox(ShaderAPITextureHandle_t hRenderTargetTexture, bool bForce)
{
	...
	if(!bForce && (hRenderTargetTexture == m_hCachedRenderTarget &&
				   m_DynamicState.m_bSRGBWritesEnabled == m_bUsingSRGBRenderTarget))
	{
...
ConVar r_blocking_spew_threshold(
	"r_blocking_spew_threshold", "-1", 0,
	"Enable spew of Direct3D Blocks. Specify the minimum blocking time in milliseconds before spewing a warning.",
	r_blocking_spew_threshold_callback);
#endif