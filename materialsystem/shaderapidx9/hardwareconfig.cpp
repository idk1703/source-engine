...
CON_COMMAND_F(ccs_create_convars_from_hwconfig,
			  "Create convars from the current hardware config, useful for diffing purposes", FCVAR_CHEAT)
{
...
{
	//	printf("\n CHardwareConfig::GetHDREnabled returning m_bHDREnabled value of %s on %8x",
	//m_bHDREnabled?"true":"false", this );
	return m_bHDREnabled;
}

void CHardwareConfig::SetHDREnabled(bool bEnable)
{
	//	printf("\n CHardwareConfig::SetHDREnabled setting m_bHDREnabled to value of %s on %8x", bEnable?"true":"false",
	//this );
		...
		case 90:
			...
			if(IsOpenGL())
			{
				...
				if(1) //(CommandLine()->FindParm("-glslmode"))
				{
					...
					// it's difficult to make this fluid without teaching the engine about a cap that could change
					// during run.
			...
			else
			{
				...
				pCaps->m_MaxNumLights =
					pCaps->m_SupportsPixelShaders_2_b ? 4 : 2; // 2b gets four lights, 2.0 gets two...
			}
...
void CHardwareConfig::SetupHardwareCaps(int nDXLevel, const HardwareCaps_t &actualCaps)
{
	...
	if(IsOpenGL())
	{
		m_Caps.m_MaxNumLights =
			MIN(m_Caps.m_MaxNumLights, (m_Caps.m_bSupportsStaticControlFlow && m_Caps.m_SupportsPixelShaders_2_b)
										   ? MAX_NUM_LIGHTS
										   : (MAX_NUM_LIGHTS - 2));
...
static ConVar r_shader_srgb(
	"r_shader_srgb", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
...
static ConVar mat_disablehwmorph("mat_disablehwmorph", "0", FCVAR_ALLOWED_IN_COMPETITIVE,
...
bool CHardwareConfig::HasFastVertexTextures() const
{
	...
	//	return m_Caps.m_bDX10Card && ( GetDXSupportLevel() >= 95 ) && ( bEnableFastVertexTextures != 0 ) && (
	//!bDisableHWMorph );
}
