...
static void PrintInputDesc(int nCount, const D3D10_INPUT_ELEMENT_DESC *pDecl)
{
	...
	{
		Msg("%s (%d): Stream: %d, Offset: %d, Instanced? %c\n", pDecl[i].SemanticName, pDecl[i].SemanticIndex,
			(int)pDecl[i].InputSlot, (int)pDecl[i].AlignedByteOffset,
...
//-----------------------------------------------------------------------------
static bool CheckShaderSignatureExpectations(ID3D10ShaderReflection *pReflection, const char *pSemantic,
...
// Computes the required input desc based on the vertex format
//-----------------------------------------------------------------------------
static unsigned int ComputeInputDesc(VertexFormat_t fmt, D3D10_INPUT_ELEMENT_DESC *pDecl,
									 ID3D10ShaderReflection *pReflection)
{
	...
	for(int i = 0; s_pFieldInfo[i].m_pSemanticString; ++i)
	{
		...
		else if(CheckShaderSignatureExpectations(pReflection, s_pFieldInfo[i].m_pSemanticString,
...
//-----------------------------------------------------------------------------
ID3D10InputLayout *CreateInputLayout(VertexFormat_t fmt, ID3D10ShaderReflection *pReflection, const void *pByteCode,
									 size_t nByteCodeLen)
{
