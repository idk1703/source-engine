...
//-----------------------------------------------------------------------------
void CFunctionCommit::CallCommitFuncs(ID3D10Device *pDevice, const ShaderStateDx10_t &desiredState,
...
#define IMPLEMENT_RENDERSTATE_FUNC(_func_name, _state, _d3dFunc)                         \
	static void _func_name(ID3D10Device *pDevice, const ShaderStateDx10_t &desiredState, \
						   ShaderStateDx10_t &currentState, bool bForce)                 \
	{                                                                                    \
...
static void CommitSetInputLayout(ID3D10Device *pDevice, const ShaderStateDx10_t &desiredState,
								 ShaderStateDx10_t &currentState, bool bForce)
{
	...
	if(bForce || memcmp(&newState, &currentState.m_InputLayout, sizeof(ShaderInputLayoutStateDx10_t)))
	{
		...
		ID3D10InputLayout *pInputLayout =
			g_pShaderDeviceDx10->GetInputLayout(newState.m_hVertexShader, newState.m_pVertexDecl[0]);
...
static void CommitSetViewports(ID3D10Device *pDevice, const ShaderStateDx10_t &desiredState,
							   ShaderStateDx10_t &currentState, bool bForce)
{
...
static void CommitSetIndexBuffer(ID3D10Device *pDevice, const ShaderStateDx10_t &desiredState,
								 ShaderStateDx10_t &currentState, bool bForce)
{
...
static void CommitSetVertexBuffer(ID3D10Device *pDevice, const ShaderStateDx10_t &desiredState,
								  ShaderStateDx10_t &currentState, bool bForce)
{
	...
	for(int i = 0; i < MAX_DX10_STREAMS; ++i)
	{
		...
		bool bMatch =
			!bForce && !memcmp(&newState, &currentState.m_pVertexBuffer[i], sizeof(ShaderVertexBufferStateDx10_t));
		if(!bMatch)
		{
		...
		{
			bInMatch = true;
			pDevice->IASetVertexBuffers(nFirstBuffer, nBufferCount, &ppVertexBuffers[nFirstBuffer],
	...
	if(!bInMatch)
	{
		pDevice->IASetVertexBuffers(nFirstBuffer, nBufferCount, &ppVertexBuffers[nFirstBuffer], &pStrides[nFirstBuffer],
									&pOffsets[nFirstBuffer]);
	}
}
...
static void CommitSetRasterState(ID3D10Device *pDevice, const ShaderStateDx10_t &desiredState,
								 ShaderStateDx10_t &currentState, bool bForce)
{
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderAPIDx10, IShaderAPI, SHADERAPI_INTERFACE_VERSION, s_ShaderAPIDx10)

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShaderAPIDx10, IDebugTextureInfo, DEBUG_TEXTURE_INFO_VERSION, s_ShaderAPIDx10)
...
void CShaderAPIDx10::ClearBuffers(bool bClearColor, bool bClearDepth, bool bClearStencil, int renderTargetWidth,
...
void CShaderAPIDx10::BindVertexBuffer(int nStreamID, IVertexBuffer *pVertexBuffer, int nOffsetInBytes, int nFirstVertex,
									  int nVertexCount, VertexFormat_t fmt, int nRepetitions)
{
	...
	if(m_bResettingRenderState ||
	   memcmp(&m_DesiredState.m_pVertexBuffer[nStreamID], &state, sizeof(ShaderVertexBufferStateDx10_t)))
	{
...
void CShaderAPIDx10::BindIndexBuffer(IIndexBuffer *pIndexBuffer, int nOffsetInBytes)
{
	...
	if(pIndexBufferDx10)
	{
		...
		state.m_Format = (pIndexBufferDx10->GetIndexFormat() == MATERIAL_INDEX_FORMAT_16BIT) ? DXGI_FORMAT_R16_UINT
...
IMesh *CShaderAPIDx10::GetDynamicMesh(IMaterial *pMaterial, int nHWSkinBoneCount, bool buffered, IMesh *pVertexOverride,
...
IMesh *CShaderAPIDx10::GetDynamicMeshEx(IMaterial *pMaterial, VertexFormat_t fmt, int nHWSkinBoneCount, bool buffered,
...
void CShaderAPIDx10::PerspectiveOffCenterX(double fovx, double aspect, double zNear, double zFar, double bottom,
...
// disabled(identity)
void CShaderAPIDx10::SetLinearToGammaConversionTextures(ShaderAPITextureHandle_t hSRGBWriteEnabledTexture,
...
void CShaderAPIDx10::TexSubImage2D(int level, int cubeFace, int xOffset, int yOffset, int zOffset, int width,
...
bool CShaderAPIDx10::TexLock(int level, int cubeFaceID, int xOffset, int yOffset, int width, int height,
							 CPixelWriter &writer)
{
	return false;
}
...
ShaderAPITextureHandle_t CShaderAPIDx10::CreateTexture(int width, int height, int depth, ImageFormat dstImageFormat,
													   ...
													   const char *pDebugName, const char *pTextureGroupName)
{
	...
	CreateTextures(&handle, 1, width, height, depth, dstImageFormat, numMipLevels, numCopies, flags, pDebugName,
				   pTextureGroupName);
	return handle;
}

void CShaderAPIDx10::CreateTextures(ShaderAPITextureHandle_t *pHandles, int count, int width, int height, int depth,
...
ShaderAPITextureHandle_t CShaderAPIDx10::CreateDepthTexture(ImageFormat renderFormat, int width, int height,
...
void CShaderAPIDx10::ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *data, ImageFormat dstFormat,
								int nDstStride)
{
}
