...
//-----------------------------------------------------------------------------
static HardwareShader_t CreateD3DVertexShader(DWORD *pByteCode, int numBytes, const char *pShaderName,
...
#ifdef DX_TO_GL_ABSTRACTION
	HRESULT hr =
		Dx9Device()->CreateVertexShader(pByteCode, (IDirect3DVertexShader9 **)&hShader, pShaderName, debugLabel);
#else
	if(IsEmulatingGL())
	{
											 ...
											 const char *pShaderName, char *debugLabel = NULL)
{
	...
	if(IsPC() && nCentroidMask && (HardwareConfig()->NeedsATICentroidHack() || mat_force_ps_patch.GetInt()))
	{
...
template<class T>
int BinarySearchCombos(uint32 nStaticComboID, int nCombos, T const *pRecords)
{
...
struct ShaderFileCache_t
{
	...
	{
		int nSearchAliases =
			BinarySearchCombos(nStaticComboID, m_StaticComboDupRecords.Count(), m_StaticComboDupRecords.Base());
		if(nSearchAliases != -1)
...
private:
	...
	void SaveShaderCache(
		char *cacheName); // query GLM pair cache for all active shader pairs and write them to disk in named file
	bool LoadShaderCache(
		char *cacheName); // read named file, establish compiled shader sets for each vertex+static and pixel+static,
						  // then link pairs as listed in table return true on success, false if file not found
...
void CShaderManager::Init()
{
	...
	if(!IsX360())
	{
		...
		if(m_pShaderCompiler30)
		{
			m_ShaderCompileFileFunc30 =
				(ShaderCompileFromFileFunc_t)GetProcAddress((HMODULE)m_pShaderCompiler30, "D3DXCompileShaderFromFileA");
		}
...
IShaderBuffer *CShaderManager::CompileShader(const char *pProgram, size_t nBufLen, const char *pShaderVersion)
{
	...
	HRESULT hr = D3DXCompileShader(pProgram, nBufLen, NULL, NULL, "main", pShaderVersion, nCompileFlags,
...
void CShaderManager::CreateStaticShaders()
{
	...
	if(IsPC())
	{
		...
		const DWORD psIllegalMaterial[] = {
			...
			0xffff0200, 0x05000051, 0xa00f0000, 0x3f800000, 0x00000000, 0x3f800000, 0x3f800000, 0x02000001,
			0x800f0000, 0xa0e40000, 0x02000001, 0x800f0800, 0x80e40000, 0x0000ffff
#else
...
const CShaderManager::ShaderCombos_t *CShaderManager::FindOrCreateShaderCombos(const char *pShaderName)
{
	...
	while(char *line = bffr.InplaceGetLinePtr())
	{
		...
		if(Q_stristr(pShaderName, "_ps20") && !Q_stristr(pShaderName, "_ps20b") && Q_stristr(line, "[ps") &&
		   !Q_stristr(line, "[ps20]"))
			continue;
		if(Q_stristr(pShaderName, "_ps20b") && Q_stristr(line, "[ps") && !Q_stristr(line, "[ps20b]"))
			continue;
		if(Q_stristr(pShaderName, "_ps30") && Q_stristr(line, "[ps") && !Q_stristr(line, "[ps30]"))
			continue;
		if(Q_stristr(pShaderName, "_vs20") && Q_stristr(line, "[vs") && !Q_stristr(line, "[vs20]"))
			continue;
		if(Q_stristr(pShaderName, "_vs30") && Q_stristr(line, "[vs") && !Q_stristr(line, "[vs30]"))
			continue;
...
#if defined(_X360)
	virtual HRESULT WINAPI Open(D3DXINCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID *ppData,
...
#if defined(_X360)
HRESULT CDxInclude::Open(D3DXINCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID *ppData,
						 UINT *pBytes, LPSTR pFullPath, DWORD cbFullPath)
#else
HRESULT CDxInclude::Open(D3DXINCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID *ppData,
						 UINT *pBytes)
...
#if defined(_X360)
static ConVar mat_flushshaders_generate_updbs("mat_flushshaders_generate_updbs", "0", 0,
											  "Generates UPDBs whenever you flush shaders.");
#endif

HardwareShader_t CShaderManager::CompileShader(const char *pShaderName, int nStaticIndex, int nDynamicIndex,
											   bool bVertexShader)
{
...
#define RECV_BUF_SIZE 40000
	...
	if(m_ShaderCompileFileFunc30 && b30Shader)
	{
		...
		hr = m_ShaderCompileFileFunc30(filename, macros.Base(), &dxInclude, "main", pShaderModel, 0 /* DWORD Flags */,
									   &pShader, &pErrorMessages, NULL /* LPD3DXCONSTANTTABLE *ppConstantTable */);
	}
	else
	{
#if(!defined(_X360))
		{
			...
			hr = D3DXCompileShaderFromFile(filename, macros.Base(), NULL /* LPD3DXINCLUDE */, "main", pShaderModel,
...
#else
		{
			...
			if(mat_flushshaders_generate_updbs.GetBool())
			{
				...
				Q_snprintf(pUPDBOutputFile, MAX_PATH, "%s\\UPDB_X360\\%s_S%d_D%d.updb", GetShaderSourcePath(),
						   pShaderName, nStaticIndex, nDynamicIndex);

				// replace "net:\smb" with another "\" turning the xbox network address format into the pc network
				// address format
				V_strcpy_safe(pUPDBPIXLookup, &pUPDBOutputFile[7]);
				pUPDBPIXLookup[0] = '\\';
			}

			hr = D3DXCompileShaderFromFileEx(filename, macros.Base(), NULL /* LPD3DXINCLUDE */, "main", pShaderModel,
											 0 /* DWORD Flags */, &pShader, &pErrorMessages,
	...
	if(hr != D3D_OK)
	{
		...
		if(retriesLeft-- > 0)
		{
			DevMsg(0, "Failed dynamic shader compiled - fix the shader while the debugger is at the breakpoint, then "
					  "continue\n");
...

	{
		...
		memcpy((char *)tempBuffer.Base() + exampleCommandLineLength, pd3dxBuffer->GetBufferPointer(),
		...
		{
			return CreateD3DPixelShader((DWORD *)pRemotelyCompiledShader, 0, nRemotelyCompiledShaderLength,
										pShaderName); // hack hack hack!  need to get centroid info from the source
		}
		...
		{
			return CreateD3DPixelShader((DWORD *)pShader->GetBufferPointer(), 0, pShader->GetBufferSize(),
										pShaderName); // hack hack hack!  need to get centroid info from the source
		}
#endif

#if defined(DYNAMIC_SHADER_COMPILE)
		// We keep up with whether we hit a compile error above.  If we did, then we likely need to recompile everything
		// again since we could have changed global code.
...
//---------------------------------------------------------------------------------------------------------
void CShaderManager::WriteTranslatedFile(ShaderLookup_t *pLookup, int dynamicCombo, char *pFileContents,
										 char *pFileExtension)
{
...
bool CShaderManager::CreateDynamicCombos_Ver4(void *pContext, uint8 *pComboBuffer)
{
	...
	for(int i = 0; i < pHeader->m_nDynamicCombos; i++)
	{
		...
		uint8 *pByteCode =
			(uint8 *)pComboBuffer + pLookup->m_nDataOffset + pLookup->m_pComboDictionary[i].m_Offset - nStartingOffset;
		...
		if(pReferenceShader)
		{
			...
			int nOriginalSize;
			ApplyDiffs(pReferenceShader, pByteCode, nReferenceComboSizeForDiffs, nByteCodeSize, nOriginalSize,
					   pDiffOutputBuffer, nReferenceComboSizeForDiffs);
		...
		else
		{
			...
			{
				hardwareShader =
					CreateD3DVertexShader(reinterpret_cast<DWORD *>(pByteCode), nByteCodeSize, pShaderName);
			}
			else
			{
				hardwareShader = CreateD3DPixelShader(reinterpret_cast<DWORD *>(pByteCode), pHeader->m_nCentroidMask,
	...
	while(bOK)
	{
		...
		switch(nBlockSize & 0xc0000000)
		{
			case 0: // bzip2
			{
				...
				int nRslt = BZ2_bzBuffToBuffDecompress(reinterpret_cast<char *>(pUnpackBuffer), &nOutsize,
			...
			case 0x40000000: // lzma compressed
			{
				...
				size_t nOutsize = CLZMA::Uncompress(reinterpret_cast<uint8 *>(pCompressedShaders), pUnpackBuffer);
				pCompressedShaders += nBlockSize;
				nBlockSize = nOutsize; // how much data there is
			}
			break;
		...
		while(pReadPtr < pUnpackBuffer + nBlockSize)
		{
			...
			if(IsPC() && m_bCreateShadersOnDemand)
			{
				...
				pLookup->m_ShaderStaticCombos.m_pCreationData[iIndex].iCentroidMask =
					pFileCache->m_bVertexShader ? 0 : pFileCache->m_Header.m_nCentroidMask;
			}
			else
			{
				const char *pShaderName = m_ShaderSymbolTable.String(pLookup->m_Name);
...
//					}
					...
					// pass binary code to d3d interface, on GL it will invoke the translator back to asm
					hardwareShader =
						CreateD3DPixelShader(reinterpret_cast<DWORD *>(pReadPtr), pFileCache->m_Header.m_nCentroidMask,
											 nShaderSize, pShaderName, debugLabelPtr);
				}
				if(hardwareShader == INVALID_HARDWARE_SHADER)
				{
...
//-----------------------------------------------------------------------------
void CShaderManager::QueuedLoaderCallback(void *pContext, void *pContext2, const void *pData, int nSize,
										  LoaderError_t loaderError)
{
	ShaderLookup_t *pLookup = (ShaderLookup_t *)pContext;
...
bool CShaderManager::LoadAndCreateShaders(ShaderLookup_t &lookup, bool bVertexShader, char *debugLabel)
{
	...
	if(pFileCache->IsOldVersion())
	{
		...
		g_pFullFileSystem->Seek(hFile, nDictionaryOffset + lookup.m_nStaticIndex * sizeof(ShaderDictionaryEntry_t),
								FILESYSTEM_SEEK_HEAD);
		g_pFullFileSystem->Read(lookup.m_pComboDictionary, pHeader->m_nDynamicCombos * sizeof(ShaderDictionaryEntry_t),
								hFile);
	...
	{
		// printf("\n CShaderManager::LoadAndCreateShaders - reading %d bytes from file offset %d", nAlignedBytesToRead,
		// nAlignedOffset);
		//  single optimal read of all dynamic combos into monolithic buffer
		uint8 *pOptimalBuffer =
			(uint8 *)g_pFullFileSystem->AllocOptimalReadBuffer(hFile, nAlignedBytesToRead, nAlignedOffset);
...
bool CShaderManager::LoadShaderCache(char *cacheName)
{
	...
	FOR_EACH_SUBKEY(pProgramCache, pProgramKey)
	{
		...
		ShaderLookup_t vshLookup;
		vshLookup.m_Name =
			m_ShaderSymbolTable.AddString(pVertexShaderName); // TODO: use String() here and catch this odd case
		vshLookup.m_nStaticIndex = nVertexShaderStaticIndex;
		VertexShader_t vertexShader = m_VertexShaderDict.Find(vshLookup);
		...
		if((vertexShader != m_VertexShaderDict.InvalidIndex()) && (pixelShader != m_PixelShaderDict.InvalidIndex()))
		{
			// double check that the hardware shader arrays are actually instantiated.. bail on the attempt if not
			// (odd...)
			if(m_VertexShaderDict[vertexShader].m_ShaderStaticCombos.m_pHardwareShaders &&
			   m_PixelShaderDict[pixelShader].m_ShaderStaticCombos.m_pHardwareShaders)
			{
				// and sanity check the indices..
				if((nVertexShaderDynamicIndex >= 0) && (nPixelShaderDynamicIndex >= 0))
				{
					HardwareShader_t hardwareVertexShader =
						m_VertexShaderDict[vertexShader]
							.m_ShaderStaticCombos.m_pHardwareShaders[nVertexShaderDynamicIndex];
					HardwareShader_t hardwarePixelShader =
						m_PixelShaderDict[pixelShader]
							.m_ShaderStaticCombos.m_pHardwareShaders[nPixelShaderDynamicIndex];

					if((hardwareVertexShader != INVALID_HARDWARE_SHADER) &&
					   (hardwarePixelShader != INVALID_HARDWARE_SHADER))
					{
						if(S_OK != Dx9Device()->LinkShaderPair((IDirect3DVertexShader9 *)hardwareVertexShader,
															   (IDirect3DPixelShader9 *)hardwarePixelShader))
						{
							Warning("Could not link OpenGL shaders: %s (%d, %d) : %s (%d, %d)\n", pVertexShaderName,
									nVertexShaderStaticIndex, nVertexShaderDynamicIndex, pPixelShaderName,
									nPixelShaderStaticIndex, nPixelShaderDynamicIndex);
						}
						else
						{
							nTotalLinkedShaders++;
		...
		{
			Warning("Invalid shader linkage: %s (%d, %d) : %s (%d, %d)\n", pVertexShaderName, nVertexShaderStaticIndex,
					nVertexShaderDynamicIndex, pPixelShaderName, nPixelShaderStaticIndex, nPixelShaderDynamicIndex);
		}
	}

	Msg("Loaded program cache file \"%s\", total keyvalues: %i, total successfully linked: %i\n", cacheName,
		nTotalKeyValues, nTotalLinkedShaders);
...
VertexShader_t CShaderManager::CreateVertexShader(const char *pFileName, int nStaticVshIndex, char *debugLabel)
{
	...
	{
		// printf("\nCShaderManager::CreateVertexShader( filename = %s, staticVshIndex = %d - not in cache", pFileName,
		// nStaticVshIndex );
...
void CShaderManager::SetVertexShader(VertexShader_t shader)
{
	...
	if(dxshader == INVALID_HARDWARE_SHADER)
	{
		...
		dxshader =
			CompileShader(m_ShaderSymbolTable.String(vshLookup.m_Name), vshLookup.m_nStaticIndex, vshIndex, true);
		Assert(dxshader != INVALID_HARDWARE_SHADER);

		if(IsX360())
		{
			// 360 does not respond well at all to bad shaders or Error() calls. So we're staying here until we get
			// something that compiles
			while(dxshader == INVALID_HARDWARE_SHADER)
			{
				Warning("A dynamically compiled vertex shader has failed to build. Pausing for 5 seconds and "
						"attempting rebuild.\n");
#ifdef _WIN32
				Sleep(5000);
#elif POSIX
				usleep(5000);
#endif
				dxshader = CompileShader(m_ShaderSymbolTable.String(vshLookup.m_Name), vshLookup.m_nStaticIndex,
										 vshIndex, true);
			}
		}
	}
	...
	if(IsPC() && (dxshader == INVALID_HARDWARE_SHADER) && m_bCreateShadersOnDemand)
	{
		...
		ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
			&m_VertexShaderDict[shader].m_ShaderStaticCombos.m_pCreationData[vshIndex];
#else
		ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
			&vshLookup.m_ShaderStaticCombos.m_pCreationData[vshIndex];
#endif
	...
	{
		Error(
			"!!!!!Using invalid shader combo!!!!!  Consult a programmer and tell them to build debug "
			"materialsystem.dll and stdshader*.dll.  Run with \"mat_bufferprimitives 0\" and look for CMaterial in the "
			"call stack and see what m_pDebugName is.  You are likely using a shader combo that has been skipped.\n");
	}
#endif
...
void CShaderManager::SetPixelShader(PixelShader_t shader)
{
	...
	if(dxshader == INVALID_HARDWARE_SHADER)
	{
		// compile it since we haven't already!
		dxshader =
			CompileShader(m_ShaderSymbolTable.String(pshLookup.m_Name), pshLookup.m_nStaticIndex, pshIndex, false);
		//		Assert( dxshader != INVALID_HARDWARE_SHADER );
		...
		if(IsX360())
		{
			...
			while(dxshader == INVALID_HARDWARE_SHADER)
			{
				Warning("A dynamically compiled pixel shader has failed to build. Pausing for 5 seconds and attempting "
						"rebuild.\n");
#ifdef _WIN32
				Sleep(5000);
#elif POSIX
				usleep(5000);
#endif
				dxshader = CompileShader(m_ShaderSymbolTable.String(pshLookup.m_Name), pshLookup.m_nStaticIndex,
										 pshIndex, false);
			}
		}
	}
...
#ifdef DYNAMIC_SHADER_COMPILE
		ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
			&m_PixelShaderDict[shader].m_ShaderStaticCombos.m_pCreationData[pshIndex];
#else
		ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
			&pshLookup.m_ShaderStaticCombos.m_pCreationData[pshIndex];
#endif

		const char *pShaderName = m_ShaderSymbolTable.String(pshLookup.m_Name);
		dxshader = CreateD3DPixelShader((DWORD *)pCreationData->ByteCode.Base(), pCreationData->iCentroidMask,
										pCreationData->ByteCode.Count(), pShaderName);
...
{
	// Remarking this out because it's conflicting with dxabstract's shutdown resource leak detection code (we leak
	// thousands of shaders at shutdown with this in place). I see no reason why we would want to do this in D3D9 but
	// not GL?
	//#ifdef DX_TO_GL_ABSTRACTION
	//		return;
	//#endif
...
void CShaderManager::SpewVertexAndPixelShaders(void)
{
		...
		vshIndex = m_VertexShaderDict.Next(vshIndex))
	{
		...
		Msg("vsh 0x%8.8x: static combo:%9d dynamic combos:%6d refcount:%4d \"%s\"\n", vshIndex,
			(int)lookup.m_nStaticIndex, (int)lookup.m_ShaderStaticCombos.m_nCount, lookup.m_nRefCount, pName);
...
{
	for(VertexShader_t shader = m_VertexShaderDict.Head(); shader != m_VertexShaderDict.InvalidIndex();
	...
	for(PixelShader_t shader = m_PixelShaderDict.Head(); shader != m_PixelShaderDict.InvalidIndex();
...
void CShaderManager::DoStartupShaderPreloading()
{
	if(mat_autoload_glshaders.GetInt())
	{
		...
		Msg("Precache: Took %d ms, Vertex %d, Pixel %d\n", (int)((flEndTime - flStartTime) * 1000.0),
			s_NumVertexShadersCreated, s_NumPixelShadersCreated);
	}
}
#endif