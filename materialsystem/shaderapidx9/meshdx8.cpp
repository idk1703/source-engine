...
public:
	// constructor
	CIndexBufferDx8(ShaderBufferType_t bufferType, MaterialIndexFormat_t fmt, int nIndexCount,
...
public:
	...
	void ModifyBeginEx(bool bReadOnly, int nFirstVertex, int nVertexCount, int nFirstIndex, int nIndexCount,
					   MeshDesc_t &desc);
	...
	virtual CVertexBuffer *GetVertexBuffer()
	{
		return 0;
	}
...
public:
	...
	const CBaseMeshDX8 *GetMesh() const
	{
		return m_pMesh;
	}
...
public:
	...
	IVertexBuffer *CreateVertexBuffer(ShaderBufferType_t type, VertexFormat_t fmt, int nVertexCount,
									  const char *pBudgetGroup);
	IIndexBuffer *CreateIndexBuffer(ShaderBufferType_t bufferType, MaterialIndexFormat_t fmt, int nIndexCount,
									const char *pBudgetGroup);
	...
	IDirect3DVertexBuffer9 *GetZeroVertexBuffer() const
	{
		return m_pZeroVertexBuffer;
	}
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CIndexBufferDx8::CIndexBufferDx8(ShaderBufferType_t bufferType, MaterialIndexFormat_t fmt, int nIndexCount,
								 const char *pBudgetGroupName)
...
bool CIndexBufferDx8::Allocate()
{
	...
	if((hr == D3DERR_OUTOFVIDEOMEMORY) || (hr == E_OUTOFMEMORY))
	{
		...
		hr = Dx9Device()->CreateIndexBuffer(m_nBufferSize, usage, format, D3DPOOL_DEFAULT, &m_pIndexBuffer, NULL);
	}
#endif // !X360
...
#ifdef CHECK_INDICES
	memcpy((unsigned char *)m_pShadowIndices + m_nLockIndexOffset, m_pLockIndexBuffer,
...
// constructor
//-----------------------------------------------------------------------------
CVertexBufferDx8::CVertexBufferDx8(ShaderBufferType_t type, VertexFormat_t fmt, int nVertexCount,
								   const char *pBudgetGroupName)
	: BaseClass(pBudgetGroupName)
{
	//	Debugger();
...
bool CVertexBufferDx8::Allocate()
{
	...
	if((hr == D3DERR_OUTOFVIDEOMEMORY) || (hr == E_OUTOFMEMORY))
	{
		...
		// FIXME: need to record this
		Dx9Device()->EvictManagedResources();
		hr = Dx9Device()->CreateVertexBuffer(m_nBufferSize, usage, 0, pool, &m_pVertexBuffer, NULL);
	}
#endif // !X360
...
bool CVertexBufferDx8::Lock(int nMaxVertexCount, bool bAppend, VertexDesc_t &desc)
{
	...
	if(FAILED(hr))
	{
		...
		{
			Error(
				"Out of OS Paged Pool Memory! For more information, please "
...
// Locks mesh for modifying
//-----------------------------------------------------------------------------
void CBaseMeshDX8::ModifyBeginEx(bool bReadOnly, int nFirstVertex, int nVertexCount, int nFirstIndex, int nIndexCount,
								 MeshDesc_t &desc)
{
	LOCK_SHADERAPI();
...
bool CMeshDX8::Lock(int nVertexCount, bool bAppend, VertexDesc_t &desc)
{
	...
	if(!m_pVertexBuffer)
	{
		...
		m_pVertexBuffer = new CVertexBuffer(Dx9Device(), m_VertexFormat, 0, size, nVertexCount, m_pTextureGroupName,
...
int CMeshDX8::Lock(bool bReadOnly, int nFirstIndex, int nIndexCount, IndexDesc_t &desc)
{
	...
	if(!m_pIndexBuffer)
	{
		SafeAssign(&m_pIndexBuffer,
				   new CIndexBuffer(Dx9Device(), nIndexCount, ShaderAPI()->UsingSoftwareVertexProcessing()));
	}
...
// Locks mesh for modifying
//-----------------------------------------------------------------------------
void CMeshDX8::ModifyBeginEx(bool bReadOnly, int nFirstVertex, int nVertexCount, int nFirstIndex, int nIndexCount,
							 MeshDesc_t &desc)
{
...
bool CMeshDX8::IsValidVertexFormat(VertexFormat_t vertexFormat)
{
	...
	bool bCheckCompression =
		(m_VertexFormat & VERTEX_FORMAT_COMPRESSED) &&
	...
	if(bCheckCompression || IsPC() || IsDebug())
	{
		...
		{
			vertexFormat =
				pMaterial->GetVertexUsage() & ~(VERTEX_FORMAT_VERTEX_SHADER | VERTEX_FORMAT_USE_EXACT_FORMAT);
...
void CMeshDX8::SetColorStreamState()
{
	if((m_pColorMesh != g_pLastColorMesh) || (m_nColorMeshVertOffsetInBytes != g_nLastColorMeshVertOffsetInBytes))
	{
		if(m_pColorMesh)
		{
			...
			D3DSetStreamSource(1, m_pColorMesh->GetVertexBuffer()->GetInterface(), m_nColorMeshVertOffsetInBytes,
...
void CMeshDX8::SetVertexStreamState(int nVertOffsetInBytes)
{
	...
	if(HardwareConfig()->SupportsVertexAndPixelShaders())
	{
		if(HasFlexMesh())
		{
			...
			D3DSetStreamSource(2, m_pFlexVertexBuffer->GetInterface(), m_nFlexVertOffsetInBytes,
							   m_pFlexVertexBuffer->VertexSize());

			if(g_pHardwareConfig->Caps().m_SupportsVertexShaders_2_0)
			{
				float c[4] = {1.0f, g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_b() ? 1.0f : 0.0f, 0.0f,
							  0.0f};
		...
		else
		{
			...
			if(m_pVertexBuffer->VertexSize() < minimumStreamZeroStride)
			{
				...
				if(!bWarned)
				{
					Warning("Shader specifying too-thin vertex format, should be at least %d bytes! (Suppressing "
							"furthur warnings)\n",
			...
			// Set a 4kb all-zero static VB into the flex/wrinkle stream with a stride of 1 bytes, so the vertex shader
			// always reads valid floating point values (otherwise it can get NaN's/Inf's, and under OpenGL this is bad
			// on NVidia) togl requires non-zero strides, but on D3D9 we can set a stride of 0 for a little more
			// efficiency.
...
#ifdef _X360
	if((g_pLastVertex != m_pVertexBuffer) || (m_pVertexBuffer->IsDynamic()) ||
...
{
#ifdef _X360
	if((g_pLastIndexBuffer != NULL) || (g_pLastIndex != m_pIndexBuffer) || (m_pIndexBuffer->IsDynamic()) ||
...
void CMeshDX8::CheckIndices(CPrimList *pPrim, int numPrimitives)
{
	...
	if(m_Mode == D3DPT_TRIANGLELIST || m_Mode == D3DPT_TRIANGLESTRIP)
	{
		...
		for(i = 0; i < 2; i++)
		{
			...
			Assert(s_FirstVertex >= 0 && (int)(s_FirstVertex + m_FirstIndex) < pMesh->VertexCount());
			int nIndexCount = 0;
			...
			for(j = 0; j < nIndexCount; j++)
			{
				...
				if((index < (int)s_FirstVertex) || (index >= (int)(s_FirstVertex + s_NumVertices)))
					Warning("%s invalid index: %d [%u..%u]\n", __FUNCTION__, index, s_FirstVertex,
							s_FirstVertex + s_NumVertices - 1);
		...
		else
		{
...
#endif // CHECK_INDICES
			{
				...
				Dx9Device()->DrawIndexedPrimitive(
					m_Mode, // Member of the D3DPRIMITIVETYPE enumerated type, describing the type of primitive to
...
void CDynamicMeshDX8::Draw(int nFirstIndex, int nIndexCount)
{
	...
	if(m_IndexOverride || m_VertexOverride ||
	   ((m_TotalVertices > 0) &&
		(m_TotalIndices > 0 || m_Type == MATERIAL_POINTS || m_Type == MATERIAL_INSTANCED_QUADS)))
	{
		Assert(!m_IsDrawing);
...
void CTempMeshDX8::ModifyBeginEx(bool bReadOnly, int nFirstVertex, int nVertexCount, int nFirstIndex, int nIndexCount,
								 MeshDesc_t &desc)
{
	Assert(!m_Locked);
...
static void IntersectPlane(D3DXVECTOR3 *pStart, D3DXVECTOR3 *pEnd, int normalInd, float val, D3DXVECTOR3 *pOutVert)
{
	D3DXVECTOR3 dir;
...
static int ClipTriangleAgainstPlane(D3DXVECTOR3 **ppVert, int nVertexCount, D3DXVECTOR3 **ppOutVert, int normalInd,
...
{
	DestroyDynamicIndexBuffer();
	SafeAssign(&m_pDynamicIndexBuffer,
			   new CIndexBuffer(Dx9Device(), INDEX_BUFFER_SIZE, ShaderAPI()->UsingSoftwareVertexProcessing(), true));
}
	...
	if(g_pHardwareConfig->HasFastVertexTextures())
	{
		m_pVertexIDBuffer =
			new CVertexBuffer(Dx9Device(), 0, 0, sizeof(float), VERTEX_BUFFER_SIZE,
							  TEXTURE_GROUP_STATIC_VERTEX_BUFFER_OTHER, ShaderAPI()->UsingSoftwareVertexProcessing());
...
void CMeshMgr::CreateZeroVertexBuffer()
{
	...
	{
		// In GL glVertexAttribPointer() doesn't support strides of 0, so we need to allocate a dummy vertex buffer
		// large enough to handle 16-bit indices with a stride of 4 byte per vertex, plus a bit more for safety (in case
		// basevertexindex is > 0). We could also try just disabling any vertex attribs that fetch from stream 2 and
		// need 0's, but AMD reports this could hit a slow path in the driver. Argh.
		uint nBufSize = IsOpenGL() ? (65536 * 2 * 4) : 4096;
		HRESULT hr = Dx9Device()->CreateVertexBuffer(nBufSize, D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT,
													 &m_pZeroVertexBuffer, NULL);
		if(!FAILED(hr))
		{
			void *pData = NULL;
...
//-----------------------------------------------------------------------------
void CMeshMgr::CopyStaticMeshIndexBufferToTempMeshIndexBuffer(CTempMeshDX8 *pDstIndexMesh, CMeshDX8 *pSrcIndexMesh)
{
...
IMesh *CMeshMgr::GetFlexMesh()
{
	if(g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_b())
	{
		...
		m_DynamicFlexMesh.SetVertexFormat(VERTEX_POSITION | VERTEX_NORMAL | VERTEX_WRINKLE |
...
//-----------------------------------------------------------------------------
IMesh *CMeshMgr::GetDynamicMesh(IMaterial *pMaterial, VertexFormat_t vertexFormat, int nHWSkinBoneCount, bool buffered,
								IMesh *pVertexOverride, IMesh *pIndexOverride)
{
	tmZoneFiltered(TELEMETRY_LEVEL0, 50, TMZF_NONE, "%s", __FUNCTION__);
	...
	if(needTempMesh)
	{
		...
		if(pIndexOverride)
		{
			CopyStaticMeshIndexBufferToTempMeshIndexBuffer(&m_DynamicTempMesh, (CMeshDX8 *)pIndexOverride);
		}
...
//-----------------------------------------------------------------------------
void CMeshMgr::ComputeVertexDescription(unsigned char *pBuffer, VertexFormat_t vertexFormat, MeshDesc_t &desc) const
{
...
//-----------------------------------------------------------------------------
VertexFormat_t CMeshMgr::ComputeVertexFormat(unsigned int flags, int nTexCoordArraySize, int *pTexCoordDimensions,
											 int numBoneWeights, int userDataSize) const
{
...
CVertexBuffer *CMeshMgr::FindOrCreateVertexBuffer(int nDynamicBufferId, VertexFormat_t vertexFormat)
{
	...
	while(m_DynamicVertexBuffers.Count() <= nDynamicBufferId)
	{
		...
		m_DynamicVertexBuffers[nIndex].m_pBuffer = new CVertexBuffer(
			Dx9Device(), 0, 0, nBufferMemory / VERTEX_BUFFER_SIZE, VERTEX_BUFFER_SIZE,
			TEXTURE_GROUP_STATIC_VERTEX_BUFFER_OTHER, ShaderAPI()->UsingSoftwareVertexProcessing(), true);
...
//-----------------------------------------------------------------------------
IVertexBuffer *CMeshMgr::CreateVertexBuffer(ShaderBufferType_t type, VertexFormat_t fmt, int nVertexCount,
											const char *pBudgetGroup)
{
	// FIXME: Use a fixed-size allocator
...
IIndexBuffer *CMeshMgr::CreateIndexBuffer(ShaderBufferType_t bufferType, MaterialIndexFormat_t fmt, int nIndexCount,
										  const char *pBudgetGroup)
{
	switch(bufferType)
	{
...
void CMeshMgr::SetVertexStreamState(int nVertOffsetInBytes, int nVertexStride)
{
	...
	if(HardwareConfig()->SupportsVertexAndPixelShaders())
	{
		// Set a 4kb all-zero static VB into the flex/wrinkle stream with a stride of 0 bytes, so the vertex shader
		// always reads valid floating point values (otherwise it can get NaN's/Inf's, and under OpenGL this is bad on
		// NVidia) togl requires non-zero strides, but on D3D9 we can set a stride of 0 for a little more efficiency.
		D3DSetStreamSource(2, g_MeshMgr.GetZeroVertexBuffer(), 0, IsOpenGL() ? 4 : 0);
...
bool CMeshMgr::SetRenderState(int nVertexOffsetInBytes, int nFirstVertexIdx, VertexFormat_t vertexFormat,
							  int nVertexStride)
{
	// Can't set the state if we're deactivated
	if(g_pShaderDeviceDx8->IsDeactivated())
...
void CMeshMgr::BindVertexBuffer(int nStreamID, IVertexBuffer *pVertexBuffer, int nOffsetInBytes, int nFirstVertex,
...
void CMeshMgr::RenderPassWithVertexAndIndexBuffers(void)
{
	...
	//	for ( int iPrim=0; iPrim < s_nPrims; iPrim++ )
	{
		...
		else
		{
			...
			//			Warning( "CMeshMgr::RenderPassWithVertexAndIndexBuffers: DrawIndexedPrimitive: m_nFirstIndex = %d
			//numPrimitives = %d\n", ( int )( ( CDynamiCIndexBufferDx8 * )m_pCurrentIndexBuffer )->m_FirstIndex, ( int
			//)( m_nNumIndices / 3 ) );
			{
				VPROF("Dx9Device()->DrawIndexedPrimitive");
				//				VPROF_INCREMENT_COUNTER( "DrawIndexedPrimitive", 1 );
				...
				Dx9Device()->DrawIndexedPrimitive(
					ComputeMode(
						m_PrimitiveType), // Member of the D3DPRIMITIVETYPE enumerated type, describing the type of
										  // primitive to render. D3DPT_POINTLIST is not supported with this method.

					/*m_FirstIndex*/ 0, // Offset from the start of the vertex buffer to the first vertex index. An
										// index of 0 in the index buffer refers to this location in the vertex buffer.

					/*s_FirstVertex*/ m_pFirstVertex[0], // Minimum vertex index for vertices used during this call.
														 // This is a zero based index relative to BaseVertexIndex. This
														 // is zero for now since we don't do more than one batch yet
														 // with the new mesh interface.
					...
					// do more than one batch with the new mesh interface.

					m_nFirstIndex /*pPrim->m_FirstIndex*/, // Index of the first index to use when accesssing the vertex
														   // buffer. Beginning at StartIndex to index vertices from the
														   // vertex buffer.

					m_nNumIndices / 3 /*numPrimitives*/ // Number of primitives to render. The number of vertices used
