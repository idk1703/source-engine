...
static HRESULT GetSurfaceFromTexture(IDirect3DBaseTexture *pBaseTexture, UINT level, D3DCUBEMAP_FACES cubeFaceID,
...
//-----------------------------------------------------------------------------
IDirect3DBaseTexture *CreateD3DTexture(int width, int height, int nDepth, ImageFormat dstFormat, int numLevels,
									   int nCreationFlags, char *debugLabel) // OK to skip the last param
{
	if(nDepth <= 0)
	{
		nDepth = 1;
	...
	{
		d3dFormat = ImageLoader::ImageFormatToD3DFormat(
			FindNearestSupportedFormat(dstFormat, bVertexTexture, bIsRenderTarget, bAllowNonFilterable));
	}
...
#ifdef DX_TO_GL_ABSTRACTION
	{
		...
		{
			usage |=
				D3DUSAGE_TEXTURE_SRGB; // does not exist in real DX9... just for GL to know that this is an SRGB tex
		}
	}
#endif
	...
	{
#if !defined(_X360)
		hr = Dx9Device()->CreateCubeTexture(width, numLevels, usage, d3dFormat, pool, &pD3DCubeTexture, NULL
	...
	{
#if !defined(_X360)
		hr = Dx9Device()->CreateVolumeTexture(width, height, nDepth, numLevels, usage, d3dFormat, pool,
	...
	else
	{
		...
		if((d3dFormat == NVFMT_RAWZ) || (d3dFormat == NVFMT_INTZ) || (d3dFormat == D3DFMT_D16) ||
		   (d3dFormat == D3DFMT_D24S8) || (d3dFormat == ATIFMT_D16) || (d3dFormat == ATIFMT_D24S8))
		{
			// Not putting D3DUSAGE_RENDERTARGET here causes D3D debug spew later, but putting the flag causes this
			// create to fail...
		...
		hr = Dx9Device()->CreateTexture(width, height, numLevels, usage, d3dFormat, pool, &pD3DTexture, NULL
#if defined(DX_TO_GL_ABSTRACTION)
										,
										debugLabel // tex create funcs take extra arg for debug name on GL
#endif
...
static void BlitSurfaceBits(TextureLoadInfo_t &info, int xOffset, int yOffset, int srcStride)
{
	...
	if(!info.m_bTextureIsLockable)
	{
		...
		if(g_ShaderDeviceUsingD3D9Ex &&
		   (info.m_SrcFormat == dstFormat ||
			(info.m_SrcFormat == IMAGE_FORMAT_DXT1_ONEBITALPHA && dstFormat == IMAGE_FORMAT_DXT1)) &&
		   (!ImageLoader::IsCompressed(dstFormat) || (info.m_nWidth >= 4 || info.m_nHeight >= 4)))
		{
			if(srcStride == 0 || srcStride == info.m_nWidth * ImageLoader::SizeInBytes(info.m_SrcFormat))
			{
				...
				if(Dx9Device()->CreateTexture(info.m_nWidth, info.m_nHeight, 1, 0, dstFormatD3D, D3DPOOL_SYSTEMMEM,
											  &pTempTex, (HANDLE *)&info.m_pSrcData) == S_OK)
				{
							...
							(IsPowerOfTwo(info.m_nWidth) && IsPowerOfTwo(info.m_nHeight))))
		{
			...
			IDirect3DTexture9 *pTempTex = NULL;
			IDirect3DSurface *pTempSurf = NULL;
			if(Dx9Device()->CreateTexture(tempW, tempH, mip + 1, 0, dstFormatD3D, D3DPOOL_SYSTEMMEM, &pTempTex, NULL) ==
			   S_OK)
			{
		...
		{
			IDirect3DSurface *pTempSurf = NULL;
			if(Dx9Device()->CreateOffscreenPlainSurface(info.m_nWidth, info.m_nHeight, dstFormatD3D, D3DPOOL_SYSTEMMEM,
														&pTempSurf, NULL) == S_OK)
			{
				pSrcSurface = pTempSurf;
				bCopyBitsToSrcSurface = true;
		...
		if(bCopyBitsToSrcSurface && pSrcSurface)
		{
			if(pSrcSurface->LockRect(&lockedRect, NULL, D3DLOCK_NOSYSLOCK) == S_OK)
			{
				...
				ShaderUtil()->ConvertImageFormat(info.m_pSrcData, info.m_SrcFormat, pImage, dstFormat, info.m_nWidth,
												 info.m_nHeight, srcStride, lockedRect.Pitch);
...
static void BlitSurfaceBits(TextureLoadInfo_t &info, int xOffset, int yOffset, int srcStride)
{
	...
	if(dstFormat != info.m_SrcFormat)
	{
		...
		ImageLoader::PreConvertSwapImageData((unsigned char *)info.m_pSrcData, srcSize, info.m_SrcFormat, info.m_nWidth,
											 srcStride);

		// slow conversion operation
		if(!ShaderUtil()->ConvertImageFormat(info.m_pSrcData, info.m_SrcFormat,
											 (unsigned char *)formatConvertMemory.Base(), dstFormat, info.m_nWidth,
											 info.m_nHeight, srcStride, 0))
		{
			// conversion failed
			Assert(0);
			return;
	...
	if(!bDstIsTiled)
	{
		...
		hr = XGCopySurface(pTargetImage, mipDesc.RowPitch, mipDesc.Width, mipDesc.Height, mipDesc.Format, &p, pSrcData,
						   srcStride, mipDesc.Format, &r, 0, 0);
		if(FAILED(hr))
		{
	...
	else
	{
		...
		XGTileTextureLevel(baseDesc.Width, baseDesc.Height, info.m_nLevel, XGGetGpuFormat(baseDesc.Format), tileFlags,
						   pTargetImage, &p, pSrcData, srcStride, &r);
	}
...
static void BlitVolumeBits(TextureLoadInfo_t &info, int xOffset, int yOffset, int srcStride)
{
	...
	ShaderUtil()->ConvertImageFormat(info.m_pSrcData, info.m_SrcFormat, pImage, dstFormat, info.m_nWidth,
									 info.m_nHeight, srcStride, lockedBox.RowPitch);
...
static void BlitVolumeBits(TextureLoadInfo_t &info, int xOffset, int yOffset, int srcStride)
{
	...
	XGGetTextureDesc(info.m_pTexture, info.m_nLevel, &mipDesc);
	bool bFullSurfBlit =
		(mipDesc.Width == (unsigned int)info.m_nWidth && mipDesc.Height == (unsigned int)info.m_nHeight);
	...
	if(!info.m_bSrcIsTiled && bDstIsTiled)
	{
		...
		XGTileVolumeTextureLevel(baseDesc.Width, baseDesc.Height, baseDesc.Depth, info.m_nLevel,
								 XGGetGpuFormat(baseDesc.Format), tileFlags, pTargetImage, NULL, pSrcData,
								 mipDesc.RowPitch, mipDesc.SlicePitch, NULL);
	}
...
// FIXME: How do I blit from D3DPOOL_SYSTEMMEM to D3DPOOL_MANAGED?  I used to use CopyRects for this.  UpdateSurface
// doesn't work because it can't blit to anything besides D3DPOOL_DEFAULT. We use this only in the case where we need to
// create a < 4x4 miplevel for a compressed texture.  We end up creating a 4x4 system memory texture, and blitting it
// into the proper miplevel. 6) LockRects should be used for copying between SYSTEMMEM and MANAGED.  For such a small
// copy, you'd avoid a significant amount of overhead from the old CopyRects code.  Ideally, you should just lock the
...
void LoadVolumeTextureFromVTF(TextureLoadInfo_t &info, IVTFTexture *pVTF, int iVTFFrame)
{
	...
	if(pVTF->Depth() != (int)desc.Depth || pVTF->Width() != (int)desc.Width || pVTF->Height() != (int)desc.Height ||
	   pVTF->MipCount() < iMipCount)
	{
	...
	if(!info.m_bTextureIsLockable)
	{
		...
		if(Dx9Device()->CreateVolumeTexture(desc.Width, desc.Height, desc.Depth, iMipCount, 0, desc.Format,
											D3DPOOL_SYSTEMMEM, &pTemp, NULL) != S_OK)
		{
...
void LoadCubeTextureFromVTF(TextureLoadInfo_t &info, IVTFTexture *pVTF, int iVTFFrame)
{
	...
	if(pVTF->Depth() != 1 || pVTF->Width() != (int)desc.Width || pVTF->Height() != (int)desc.Height ||
	   pVTF->FaceCount() < 6 || pVTF->MipCount() < iMipCount)
	{
	...
	if(!info.m_bTextureIsLockable)
	{
		...
		if(Dx9Device()->CreateCubeTexture(desc.Width, iMipCount, 0, desc.Format, D3DPOOL_SYSTEMMEM, &pTemp, NULL) !=
		   S_OK)
		{
...
void LoadTextureFromVTF(TextureLoadInfo_t &info, IVTFTexture *pVTF, int iVTFFrame)
{
	...
	if(pVTF->Depth() != 1 || pVTF->Width() != (int)desc.Width || pVTF->Height() != (int)desc.Height ||
	   pVTF->MipCount() < iMipCount || pVTF->FaceCount() <= (int)info.m_CubeFaceID)
	{
	...
	if(!info.m_bTextureIsLockable && iMipCount > 1)
	{
		...
		if(Dx9Device()->CreateTexture(desc.Width, desc.Height, iMipCount, 0, desc.Format, D3DPOOL_SYSTEMMEM, &pTemp,
									  NULL) != S_OK)
		{
