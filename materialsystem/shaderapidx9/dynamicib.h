...
//#define X360_BLOCK_ON_IB_FLUSH //uncomment to block until all data is consumed when a flush is requested. Otherwise we
//only block when absolutely necessary
...
public:
	...
	{
		// If this buffer still exists, then Late Creation didn't happen. Best case: we'll render the wrong image. Worst
		// case: Crash.
	...
	bool IsPooled(void)
	{
		return m_GPUBufferHandle.IsValid();
	}
	...
	{
		return m_bExternalMemory;
	}

	// Block until there's a free portion of the buffer of this size, m_Position will be updated to point at where this
	// section starts
...
private:
	...
	int m_iAllocationCount; // The total number of indices the buffer we allocated can hold. Usually greater than the
							// number of indices asked for
	IDirect3DIndexBuffer9 m_D3DIndexBuffer; // Only need one shared D3D header for our usage patterns.
	CUtlLinkedList<DynamicBufferAllocation_t>
		m_AllocationRing; // tracks what chunks of our memory are potentially still in use by D3D

	GPUBufferHandle_t m_GPUBufferHandle; // Handle to a memory allocation within a shared physical memory pool (see
										 // CGPUBufferAllocator)
#endif
...
inline CIndexBuffer::CIndexBuffer(IDirect3DDevice9 *pD3D, int count, bool bSoftwareVertexProcessing, bool dynamic)
	: m_pIB(0),
	  m_Position(0),
	  m_bFlush(true),
	  m_bLocked(false),
	  ...
	  m_nReferenceCount(0)
{
	...
	MEM_ALLOC_CREDIT_(m_bDynamic ? ("D3D: " TEXTURE_GROUP_DYNAMIC_INDEX_BUFFER)
								 : ("D3D: " TEXTURE_GROUP_STATIC_INDEX_BUFFER));
...
if(m_bDynamic)
{
	...
	m_pAllocatedMemory = (unsigned char *)XPhysicalAlloc(m_iAllocationCount * IndexSize(), MAXULONG_PTR, 0,
...
{
	// Successfully allocated in a shared ShaderAPI memory pool (SetBufferAllocationHandle will have been called to set
	// the pointer and stream offset)
...
else
{
	...
	// NOTE: write-combining (PAGE_WRITECOMBINE) is deliberately not used, since it slows down CPU access to the data
	// (decals+defragmentation)
	m_iAllocationCount = count;
...
void CIndexBuffer::Create(IDirect3DDevice9 *pD3D)
{
...
#if !defined(_X360)
	HRESULT hr = pD3D->CreateIndexBuffer(m_IndexCount * IndexSize(), desc.Usage, desc.Format, desc.Pool, &m_pIB, NULL);
	if(hr != D3D_OK)
	{
	...
	if((hr == D3DERR_OUTOFVIDEOMEMORY) || (hr == E_OUTOFMEMORY))
	{
		...
		hr = pD3D->CreateIndexBuffer(m_IndexCount * IndexSize(), desc.Usage, desc.Format, desc.Pool, &m_pIB, NULL);
	}
...
inline CIndexBuffer::~CIndexBuffer()
{
	...
	if(m_pAllocatedMemory && !m_bExternalMemory)
	{
		...
		{
			MemAlloc_RegisterExternalDeallocation(XMem_CIndexBuffer, m_pAllocatedMemory,
...
inline void CIndexBuffer::BlockUntilUnused(int nAllocationSize)
{
	...
	Assert((m_AllocationRing[m_AllocationRing.Head()].m_iStartOffset == 0) ||
		   ((m_iNextBlockingPosition == m_AllocationRing[m_AllocationRing.Head()].m_iStartOffset) &&
			(m_Position <= m_iNextBlockingPosition)));
	...
	if(iMinBlockPosition > m_iAllocationCount)
	{
		...
		// treat all allocations between the current position and the tail end of the ring as freed since they will be
		// before we unblock
	...
	// now we go through the allocations until we find the last fence we care about. Treat everything up until that
	// fence as freed.
	DWORD FinalFence = 0;
	...
	if(Dx9Device()->IsFencePending(FinalFence))
	{
		...
		{
			// We're about to overrun our IB ringbuffer in a single Z prepass. To avoid rendering corruption, close out
			// the Z prepass and continue. This will reduce early-Z rejection efficiency and could cause a momentary
			// framerate drop, but it's better than rendering corruption.
...
inline unsigned short *CIndexBuffer::Lock(bool bReadOnly, int numIndices, int &startIndex, int startPosition)
{
...
#if !defined(_X360)
	// If the caller isn't in the thread that owns the render lock, need to return a system memory pointer--cannot talk
	// to GL from the non-current thread.
	if(!m_pSysmemBuffer && !g_pShaderUtil->IsRenderThreadSafe())
	{
	...
	{
		hr = m_pIB->Lock(position * IndexSize(), numIndices * IndexSize(), reinterpret_cast<void **>(&pLockedData),
						 dwFlags);
	}
#else
	if(m_bDynamic)
	...
	switch(hr)
	{
		...
		case D3DERR_DRIVERINTERNALERROR:
			Msg("D3DERR_DRIVERINTERNALERROR - Index Buffer Lock Failed in %s on line %d (offset %d, size %d, flags "
				"0x%x)\n",
				V_UnqualifiedFileName(__FILE__), __LINE__, position * IndexSize(), numIndices * IndexSize(), dwFlags);
			break;
		case D3DERR_OUTOFVIDEOMEMORY:
			Msg("D3DERR_OUTOFVIDEOMEMORY - Index Buffer Lock Failed in %s on line %d (offset %d, size %d, flags "
				"0x%x)\n",
				V_UnqualifiedFileName(__FILE__), __LINE__, position * IndexSize(), numIndices * IndexSize(), dwFlags);
			break;
	}
...
inline void CIndexBuffer::Unlock(int numIndices)
{
...
#if DX_TO_GL_ABSTRACTION
		// Knowing how much data was actually written is critical for performance under OpenGL.
		// Important notes: numIndices indicates how much we could move the current position. For dynamic buffer, it
		// should indicate the # of actually written indices, for static buffers it's typically 0. If it's a dynamic
		// buffer (where we actually care about perf), assume the caller isn't lying about numIndices, otherwise just
		// assume they wrote the entire thing. If you modify this code, be sure to test on both AMD and NVidia drivers!
	...
	if(m_bDynamic)
	{
		...
		LockData.m_Fence =
			Dx9Device()->GetCurrentFence(); // This isn't the correct fence, but it's all we have access to for now and
											// it'll provide marginal safety if something goes really wrong.
		LockData.m_iStartOffset = m_Position;
		LockData.m_iEndOffset = LockData.m_iStartOffset + numIndices;
		LockData.m_iZPassIdx =
			(Dx9Device()->GetDeviceState() & D3DDEVICESTATE_ZPASS_BRACKET) ? ShaderAPI()->Get360ZPassCounter() : 0;
		Assert((LockData.m_iStartOffset == 0) ||
			   (LockData.m_iStartOffset == m_AllocationRing[m_AllocationRing.Tail()].m_iEndOffset));
...
inline void CIndexBuffer::HandleLateCreation()
{
	...
	HRESULT hr = m_pIB->Lock(m_nSysmemBufferStartBytes, dataToWriteBytes, &pWritePtr, dwFlags);
