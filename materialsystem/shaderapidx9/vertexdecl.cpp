...
void PrintVertexDeclaration(const D3DVERTEXELEMENT9 *pDecl)
{
	...
	for(i = 0;; i++)
	{
		...
		Msg("%d: Stream: %d, Offset: %d, Type: %s, Method: %s, Usage: %s, UsageIndex: %d\n", i, (int)pDecl[i].Stream,
			(int)pDecl[i].Offset, DeclTypeToString(pDecl[i].Type), DeclMethodToString(pDecl[i].Method),
...
void ComputeVertexSpec(VertexFormat_t fmt, D3DVERTEXELEMENT9 *pDecl, bool bStaticLit, bool bUsingFlex, bool bUsingMorph)
{
	...
	VertexElement_t texCoordDimensions[4] = {VERTEX_ELEMENT_TEXCOORD1D_0, VERTEX_ELEMENT_TEXCOORD2D_0,
													...
													bool bUsingMorph)
{
	...
	if(hr != D3D_OK)
	{
		Warning(" ERROR: failed to create vertex decl for vertex format 0x%08llX! You'll probably see messed-up mesh "
				"rendering - to diagnose, build shaderapidx9.dll in debug.\n",
				fmt);
	}
