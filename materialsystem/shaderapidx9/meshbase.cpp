...
void CVertexBufferBase::PrintVertexFormat(VertexFormat_t vertexFormat)
{
	...
	{
		// FIXME: genericise this stuff using VertexElement_t data tables (so funcs like 'just work' if we make
		// compression changes)
	...
	{
		Msg("VERTEX_BONEWEIGHT(%d)%s|", NumBoneWeights(vertexFormat), (compression ? "[COMPRESSED]" : ""));
	}
...
//-----------------------------------------------------------------------------
void CVertexBufferBase::ComputeVertexDescription(unsigned char *pBuffer, VertexFormat_t vertexFormat,
												 VertexDesc_t &desc)
{
...
void CVertexBufferBase::Spew(int nVertexCount, const VertexDesc_t &desc)
{
	...
	for(int i = 0; i < nVertexCount; ++i)
	{
		...
		if(desc.m_VertexSize_Position)
		{
			...
			nLen +=
				Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "P %8.2f %8.2f %8.2f ", pos[0], pos[1], pos[2]);
		}
		...
		if(desc.m_VertexSize_BoneMatrixIndex)
		{
			...
			nLen += Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "BI %d %d %d %d ", (int)pIndex[0],
							   (int)pIndex[1], (int)pIndex[2], (int)pIndex[3]);
		...
		if(desc.m_VertexSize_Normal)
		{
			...
			nLen += Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "N %1.2f %1.2f %1.2f ", normal[0], normal[1],
							   normal[2]);
		}
		...
		if(desc.m_VertexSize_Color)
		{
			...
			nLen += Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "C b %3d g %3d r %3d a %3d ", pColor[0],
		...
		for(int j = 0; j < VERTEX_MAX_TEXTURE_COORDINATES; ++j)
		{
			if(desc.m_VertexSize_TexCoord[j])
			{
				...
				nLen +=
					Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "T%d %.2f %.2f ", j, texcoord[0], texcoord[1]);
			}
		}
		...
		if(desc.m_VertexSize_TangentS)
		{
			...
			nLen += Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "S %1.2f %1.2f %1.2f ", tangentS[0],
		...
		if(desc.m_VertexSize_TangentT)
		{
			...
			nLen += Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "T %1.2f %1.2f %1.2f ", tangentT[0],
							   tangentT[1], tangentT[2]);
		}
