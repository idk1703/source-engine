...
//#define X360_BLOCK_ON_VB_FLUSH //uncomment to block until all data is consumed when a flush is requested. Otherwise we
...
{
public:
	CVertexBuffer(IDirect3DDevice9 *pD3D, VertexFormat_t fmt, DWORD theFVF, int vertexSize, int theVertexCount,
				  const char *pTextureBudgetName, bool bSoftwareVertexProcessing, bool dynamic = false);

#ifdef _X360
	CVertexBuffer();
	void Init(IDirect3DDevice9 *pD3D, VertexFormat_t fmt, DWORD theFVF, uint8 *pVertexData, int vertexSize,
			  int theVertexCount);
#endif
	...
	bool IsPooled(void)
	{
		creturn m_GPUBufferHandle.IsValid();
	}
	...
	int NumVerticesUntilFlush() const
	{
		...
		if(m_AllocationRing.Count())
		{
			...
			if(m_AllocationRing.IsValidIndex(iNode))
			{
				...
				{
					// Wrapped. Making the arbitrary decision that the return value for this function *should* handle
					// the singe giant allocation case which requires contiguous memory
...
//-----------------------------------------------------------------------------
inline CVertexBuffer::CVertexBuffer(IDirect3DDevice9 *pD3D, VertexFormat_t fmt, DWORD theFVF, int vertexSize,
									int vertexCount, const char *pTextureBudgetName, bool bSoftwareVertexProcessing,
									bool dynamic)
	: m_pVB(0),
	  m_Position(0),
	  m_VertexSize(vertexSize),
...
void CVertexBuffer::Create(IDirect3DDevice9 *pD3D)
{
	...
	if(m_bDynamic)
	{
		...
		m_pAllocatedMemory = (unsigned char *)XPhysicalAlloc(m_iAllocationSize, MAXULONG_PTR, 0,
															 PAGE_READWRITE | MEM_LARGE_PAGES | PAGE_WRITECOMBINE);
	}
	else if(MeshMgr()->AllocatePooledVB(this, m_nBufferSize, pTextureBudgetName))
	{
		// Successfully allocated in a shared ShaderAPI memory pool (SetBufferAllocationHandle will have been called to
		// set the pointer and stream offset)
	...
	else
	{
		...
		// NOTE: write-combining (PAGE_WRITECOMBINE) is deliberately not used, since it slows down CPU access to the
		// data (decals+defragmentation)
		m_iAllocationSize = m_nBufferSize;
...
inline void CVertexBuffer::Init(IDirect3DDevice9 *pD3D, VertexFormat_t fmt, DWORD theFVF, uint8 *pVertexData,
...
inline CVertexBuffer::~CVertexBuffer()
{
	...
	if(m_pAllocatedMemory && !m_bExternalMemory)
	{
		...
		{
			MemAlloc_RegisterExternalDeallocation(XMem_CVertexBuffer, m_pAllocatedMemory,
...
inline void CVertexBuffer::BlockUntilUnused(int nBufferSize)
{
	...
	if(Dx9Device()->IsFencePending(FinalFence))
	{
		...
		{
			// We're about to overrun our VB ringbuffer in a single Z prepass. To avoid rendering corruption, close out
...
inline unsigned char *CVertexBuffer::Lock(int numVerts, int &baseVertexIndex)
{
	...
	if(IsX360() && m_bDynamic)
	{
		// Block until we have enough room in the buffer, this affects the result of NextLockOffset() in wrap
		// conditions.
	...
	{
		m_pVB->Lock(nLockOffset, nBufferSize, reinterpret_cast<void **>(&pLockedData), dwFlags);
	}
#else
...
#if !defined(_X360)
	m_pVB->Lock(firstVertex * m_VertexSize, numVerts * m_VertexSize, reinterpret_cast<void **>(&pLockedData), dwFlags);
#else
...
inline void CVertexBuffer::Unlock(int numVerts)
{
	...
	if(m_bDynamic)
	{
		if(numVerts > 0)
		{
			...
			LockData.m_Fence =
				Dx9Device()->GetCurrentFence(); // This isn't the correct fence, but it's all we have access to for now
												// and it'll provide marginal safety if something goes really wrong.
			...
			// Always re-use the same vertex buffer header based on the assumption that D3D copies it off in the draw
			// calls.
