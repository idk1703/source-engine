...
KeyValues *CShaderDeviceMgrBase::FindCPUSpecificConfig(KeyValues *pKeyValues, int nCPUMhz, bool bAMD)
{
	...
	for(KeyValues *pGroup = pKeyValues->GetFirstSubKey(); pGroup; pGroup = pGroup->GetNextKey())
	{
		...
		if((bAMD && Q_stristr(pName, "AMD")) || (!bAMD && Q_stristr(pName, "Intel")))
		{
...
void CShaderDeviceMgrBase::ReadDXSupportLevels(HardwareCaps_t &caps)
{
	...
	if(pDeviceKeyValues)
	{
		...
		if(nDXSupportLevel != 0)
		{
			...
			// Don't slam up the dxlevel level to 92 on DX10 cards in OpenGL Linux/Win mode (otherwise Intel will get
			// dxlevel 92 when we want 90)
...
void CShaderDeviceMgrBase::LoadHardwareCaps(KeyValues *pGroup, HardwareCaps_t &caps)
{
	...
	// don't just blanket kill clip planes on POSIX, only shoot them down if we're running ARB, or asked for
	// nouserclipplanes.
	// FIXME need to take into account the caps bit that GLM can now provide, so NV can use normal clipping and ATI can
	// fall back to fastclip.
	...
	caps.m_bDisableShaderOptimizations =
		ReadBool(pGroup, "DisableShaderOptimizations", caps.m_bDisableShaderOptimizations);
}
...
void CShaderDeviceMgrBase::ReadHardwareCaps(HardwareCaps_t &caps, int nDxLevel)
{
	...
	{
		// The card specific case is a catch all for device ids, so run it before running the dxlevel and card specific
		// stuff.
...
//-----------------------------------------------------------------------------
bool CShaderDeviceMgrBase::GetRecommendedConfigurationInfo(int nAdapter, int nDXLevel, int nVendorID, int nDeviceID,
