...
public:
	...
	// Frees a given entry (just marks it as freed, the memory will not be reused by Allocate() until
	// CGPUBufferAllocator::Defrag() is called )
...
private:
	...
	// Helper for Compact
	void MoveBufferMemory(int nDstPool, int *pnDstEntry, int *pnDstOffset, CGPUBufferPool &srcPool,
