...
// Set this to 1 to get vprof nodes for playing back the command stream.  This is good for counting calls in a frame,
// etc.
...
void D3DDeviceWrapper::AsynchronousLock(IDirect3DIndexBuffer9 *ib, size_t offset, size_t size, void **ptr, DWORD flags,
...
void D3DDeviceWrapper::AsynchronousLock(IDirect3DVertexBuffer9 *vb, size_t offset, size_t size, void **ptr, DWORD flags,
										LockedBufferContext *lb)
{
	...
	for(;;)
	{
		...
		switch(dptr[0])
		{
			...
			case PBCMD_DRAWINDEXEDPRIM:
			{
				...
				Dx9Device()->DrawIndexedPrimitive((D3DPRIMITIVETYPE)dptr[1], dptr[2], dptr[3], dptr[4], dptr[5],
												  dptr[6]);
				dptr += 7;
				break;
			}
			...
			case PBCMD_SET_STREAM_SOURCE:
			{
				...
				Dx9Device()->SetStreamSource(dptr[1], (IDirect3DVertexBuffer9 *)FetchPtr(dptr + 2), dptr[3], dptr[4]);
			...
			case PBCMD_STRETCHRECT:
			{
				...
				Dx9Device()->StretchRect(pSourceSurface, pSourceRect, pDestSurface, pDestRect, Filter);
			}
			break;
