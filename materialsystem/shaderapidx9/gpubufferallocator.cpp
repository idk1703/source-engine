...
CGPUBufferAllocator::CGPUBufferAllocator(void) : m_nBufferPools(0), m_bEnabled(true)
{
...
bool CGPUBufferAllocator::AllocatePool(int nPoolSize)
{
	...
	if(m_BufferPools[m_nBufferPools]->m_pMemory == NULL)
	{
		...
		ExecuteNTimes(
			15, Warning("CGPUBufferAllocator::AllocatePool - physical allocation failed! Physical fragmentation is in "
						"bad shape... falling back to non-pooled VB/IB allocations. Brace for a crash :o/\n"));
...
//-----------------------------------------------------------------------------
bool CGPUBufferAllocator::AllocateBuffer(GPUBufferHandle_t *pHandle, int nBufferSize, void *pObject,
...
//-----------------------------------------------------------------------------
void CGPUBufferAllocator::MoveBufferMemory(int nDstPool, int *pnDstEntry, int *pnDstOffset, CGPUBufferPool &srcPool,
										   GPUBufferPoolEntry_t &srcEntry)
{
	// Move the data
...
{
	// NOTE: this must only be called during map transitions, no rendering must be in flight and everything must be
	// single-threaded!
	AUTO_LOCK(m_mutex);
	...
	// Now free empty pools (keep the first (very large) one around, since fragmentation makes freeing+reallocing it a
	// big risk)
	int nBytesFreed = 0;
	...
	if(m_nBufferPools > 1)
	{
		// The above compaction algorithm could waste space due to large allocs causing nDstPool to increment before
		// that pool is actually full. With our current usage pattern (total in-use memory is less than
		// INITIAL_POOL_SIZE, whenever Compact is called), that doesn't matter. If that changes (i.e. the below warning
		// fires), then the fix would be:
		//  - for each pool, sort its entries by size (largest first) and try to allocate them on the end of prior
		//  (already-compacted) pools
		//  - pack whatever remains in the pool down, and proceed to the next pool
		ExecuteOnce(Warning("CGPUBufferAllocator::Compact may be wasting memory due to changed usage patterns (see "
							"code for suggested fix)."));
	}
...
void CGPUBufferAllocator::SpewStats(bool bBrief)
{
	...
	for(int i = 0; i < m_nBufferPools; i++)
	{
		...
		for(int j = 0; j < pool->m_PoolEntries.Count(); j++)
		{
			...
			{
				// Figure out how much memory we WOULD have allocated for this buffer, if we'd allocated it
				// individually:
				nOldMemWasted += ALIGN_VALUE(poolEntry.nSize, 4096) - poolEntry.nSize;
				if(poolEntry.bIsVertexBuffer)
					nVBsInPools++;
				if(!poolEntry.bIsVertexBuffer)
					nIBsInPools++;
			}
	...
	// NOTE: 'unused' memory doesn't count memory used by freed buffers, which should be zero during gameplay. The
	// purpose is
	//       to measure wastage at the END of a pool, to help determine ideal values for ADDITIONAL_POOL_SIZE and
	//       MAX_BUFFER_SIZE.
	...
	if(bBrief)
	{
		ConMsg("[GPUBUFLOG] Pools:%2d | Size:%5.1fMB | Unused:%5.1fMB | Freed:%5.1fMB | Unpooled:%5.1fMB\n",
			   m_nBufferPools, nMemAllocated / MB, nMemUnused / MB, nFreedBufferMem / MB,
			   (g_SizeIndividualVBPhysAllocs + g_SizeIndividualIBPhysAllocs) / MB);
	}
	else
	{
...
CGPUBufferPool::CGPUBufferPool(int nSize)
	: m_PoolEntries(POOL_ENTRIES_GROW_SIZE, POOL_ENTRIES_INIT_SIZE), m_nSize(0), m_nBytesUsed(0)
{
	// NOTE: write-combining (PAGE_WRITECOMBINE) is deliberately not used, since it slows down 'Compact' hugely (and
	// doesn't noticeably benefit load times)
