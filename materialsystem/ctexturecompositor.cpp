...
ConVar r_texcomp_dump("r_texcomp_dump", "0", FCVAR_NONE,
					  "Whether we should dump the textures to disk or not. 1: Save all; 2: Save Final; 3: Save Final "
					  "with name suitable for scripting; 4: Save Final and skip saving workshop icons.");
#endif
...
public:
	...
	inline ECompositeResolveStatus GetResolveStatus() const
	{
		return m_ResolveStatus;
	}
	inline const CTCStageResult_t &GetResult() const
	{
		Assert(GetResolveStatus() == ECRS_Complete);
		return m_Result;
	}
	...
	inline void SetFirstChild(CTCStage *_stage)
	{
		m_pFirstChild = _stage;
	}
	inline void SetNextSibling(CTCStage *_stage)
	{
		m_pNextSibling = _stage;
	}

	inline CTCStage *GetFirstChild()
	{
		return m_pFirstChild;
	}
	inline CTCStage *GetNextSibling()
	{
		return m_pNextSibling;
	}

	inline const CTCStage *GetFirstChild() const
	{
		return m_pFirstChild;
	}
	inline const CTCStage *GetNextSibling() const
	{
		return m_pNextSibling;
	}
	...
	void Render(ITexture *_destRT, IMaterial *_mat, const CUtlVector<CTCStageResult_t> &_inputs,
				CTextureCompositor *_comp, bool bClear);
...
public:
	CTCTextureStage(const TextureStageParameters &_tsp, uint32 nTexCompositeCreateFlags)
		: m_Parameters(_tsp), m_pTex(NULL), m_pTexRed(NULL), m_pTexBlue(NULL)
	{
	}
	...
	virtual bool DoesTargetRenderTarget() const
	{
		return false;
	}
...
protected:
	...
	virtual void RequestTextures()
	{
		if(!m_Parameters.m_pTexFilename.IsEmpty())
			materials->AsyncFindTexture(m_Parameters.m_pTexFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE, this,
										(void *)Neutral, false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
		if(!m_Parameters.m_pTexRedFilename.IsEmpty())
			materials->AsyncFindTexture(m_Parameters.m_pTexRedFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE, this,
										(void *)Red, false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
		if(!m_Parameters.m_pTexBlueFilename.IsEmpty())
			materials->AsyncFindTexture(m_Parameters.m_pTexBlueFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE, this,
										(void *)Blue, false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
	}
	...
	virtual void ResolveThis(CTextureCompositor *_comp)
	{
		...
		{
			_comp->Error(false, "Invalid texture_lookup node, must specify at least texture (or texture_red and "
								"texture_blue) or all of them.\n");
			return;
		}
...
static_assert(ARRAYSIZE(cCombineMaterialName) == ECO_COUNT + 1,
			  "cCombineMaterialName and ECombineOperation are out of sync.");
...
public:
	CTCCombineStage(const CombineStageParameters &_csp, uint32 nTexCompositeCreateFlags)
		: m_Parameters(_csp), m_pMaterial(NULL)
	{
		Assert(m_Parameters.m_CombineOp >= 0 && m_Parameters.m_CombineOp < ECO_COUNT);

		SafeAssign(&m_pMaterial, materials->FindMaterial(cCombineMaterialName[m_Parameters.m_CombineOp],
...
public:
	CTCSelectStage(const SelectStageParameters &_ssp, uint32 nTexCompositeCreateFlags)
		: m_Parameters(_ssp), m_pMaterial(NULL), m_pTex(NULL)
	{
		SafeAssign(&m_pMaterial,
				   materials->FindMaterial(cCombineMaterialName[ECO_Select], TEXTURE_GROUP_RUNTIME_COMPOSITE));
	}
...
protected:
	virtual void RequestTextures()
	{
		materials->AsyncFindTexture(m_Parameters.m_pTexFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE, this, NULL,
									false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
	}
...
struct Sticker_t
{
	...
	CUtlString m_specFilename; // Name of the specular file for the sticker, or if blank we will assume it is
							   // baseFilename + _spec + baseExtension
...
void ParseTFromKV<Sticker_t>(KeyValues *_kv, void *_pDest)
{
	...
	{
		CUtlString specPath = tmpDest.m_baseFilename.StripExtension() + "_s" + tmpDest.m_baseFilename.GetExtension();
...
{
	char buffer[80];
	V_sprintf_safe(buffer, "[ weight %.2f; base \"%s\"; spec \"%s\" ]", _val.m_fWeight, _val.m_baseFilename.Get(),
				   _val.m_specFilename.Get());
	return CUtlString(buffer);
}
...
public:
	CTCApplyStickerStage(const ApplyStickerStageParameters &_assp, uint32 nTexCompositeCreateFlags)
		: m_Parameters(_assp), m_pMaterial(NULL), m_pTex(NULL), m_pTexSpecular(NULL), m_nChoice(0)
	{
		SafeAssign(&m_pMaterial,
				   materials->FindMaterial(cCombineMaterialName[ECO_Blend], TEXTURE_GROUP_RUNTIME_COMPOSITE));
	}
...
protected:
	...
	virtual void RequestTextures()
	{
		if(!m_Parameters.m_possibleStickers[m_nChoice].m_baseFilename.IsEmpty())
			materials->AsyncFindTexture(m_Parameters.m_possibleStickers[m_nChoice].m_baseFilename.Get(),
										TEXTURE_GROUP_RUNTIME_COMPOSITE, this, (void *)Albedo, false,
										TEXTUREFLAGS_IMMEDIATE_CLEANUP);

		if(!m_Parameters.m_possibleStickers[m_nChoice].m_specFilename.IsEmpty())
			materials->AsyncFindTexture(m_Parameters.m_possibleStickers[m_nChoice].m_specFilename.Get(),
										TEXTURE_GROUP_RUNTIME_COMPOSITE, this, (void *)Specular, false,
										TEXTUREFLAGS_IMMEDIATE_CLEANUP);
	}
	...
	virtual bool ComputeRandomValuesThis(CUniformRandomStream *pRNG) OVERRIDE
	{
		...
		ComputeTextureMatrixFromRectangle(&m_mTextureAdjust, m_Parameters.m_vDestBL.m_val, m_Parameters.m_vDestTL.m_val,
										  m_Parameters.m_vDestTR.m_val);
		return true;
	}
...
private:
	...
	virtual void ResolveThis(CTextureCompositor *_comp)
	{
		...
		if(m_pTex)
		{
			...
			{
				char buffer[128];
				V_snprintf(buffer, ARRAYSIZE(buffer), "composite_%s_result_%02d.tga", _comp->GetName().Get(),
						   s_nDumpCount++);
		...
		pRenderContext->AsyncCreateTextureFromRenderTarget(GetFirstChild()->GetResult().m_pRenderTarget, buffer, fmt,
														   bGenMipmaps, nCreateFlags, this, NULL);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
CTextureCompositor::CTextureCompositor(int _width, int _height, int nTeam, const char *pCompositeName,
									   uint64 nRandomSeed, uint32 nTexCompositeCreateFlags)
	: m_nReferenceCount(0),
	  m_nWidth(_width),
	  m_nHeight(_height),
	  m_nTeam(nTeam),
...
void CTextureCompositor::Update()
{
	...
	if(m_pRootStage->GetResolveStatus() == ECRS_Complete)
	{
		...
		tmEndTimeSpan(TELEMETRY_LEVEL0, m_nCompositePaintKitId, 0, "Composite: %s",
					  tmDynamicString(TELEMETRY_LEVEL0, buffer));
#endif
	}
}
...
void CTextureCompositor::ScheduleResolve()
{
	...
	char buffer[256];
	GetTextureName(buffer, ARRAYSIZE(buffer));
	tmBeginTimeSpan(TELEMETRY_LEVEL0, m_nCompositePaintKitId, 0, "Composite: %s",
					tmDynamicString(TELEMETRY_LEVEL0, buffer));
#endif
...
ITexture *CTextureCompositor::AllocateCompositorRenderTarget()
{
	...
	ITexture *retVal = materials->CreateNamedRenderTargetTextureEx("", m_nWidth, m_nHeight, RT_SIZE_LITERAL_PICMIP,
...
void CTextureCompositor::GetTextureName(char *pOutBuffer, int nBufferLen) const
{
	...
	if(m_pRootStage->HasTeamSpecifics())
		V_snprintf(pOutBuffer, nBufferLen, "proc/texcomp/%s_flags%08x_seedhi%08x_seedlo%08x_team%d_w%d_h%d",
				   GetName().Get(), GetCreateFlags(), seedhi, seedlo, m_nTeam, m_nWidth, m_nHeight);
	else
		V_snprintf(pOutBuffer, nBufferLen, "proc/texcomp/%s_flags%08x_seedhi%08x_seedlo%08x_w%d_h%d", GetName().Get(),
				   GetCreateFlags(), seedhi, seedlo, m_nWidth, m_nHeight);
}
...
// ------------------------------------------------------------------------------------------------
CTCStage::CTCStage()
	: m_nReferenceCount(1) // This is 1 because the common case is to assign these as children, and we don't want to
						   // play with refs there.
	  ,
	  m_pFirstChild(NULL),
	  m_pNextSibling(NULL),
	  m_ResolveStatus(ECRS_Idle)
...
// ------------------------------------------------------------------------------------------------
void CTCStage::Render(ITexture *_destRT, IMaterial *_mat, const CUtlVector<CTCStageResult_t> &_inputs,
					  CTextureCompositor *_comp, bool bClear)
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
	...
	FOR_EACH_VEC(_inputs, i)
	{
		...
		ITexture *inTex = stageParams.m_pTexture ? stageParams.m_pTexture : stageParams.m_pRenderTarget;
	...
	if(r_texcomp_dump.GetInt() == 1)
	{
		FOR_EACH_VEC(_inputs, i)
		{
			...
			{
				V_snprintf(buffer, ARRAYSIZE(buffer), "composite_%s_input_%02d_in%01d_%08x.tga", _comp->GetName().Get(),
						   s_nDumpCount, i, (int)this);
				_inputs[i].m_pTexture->SaveToFile(buffer);
			}
		}

		V_snprintf(buffer, ARRAYSIZE(buffer), "composite_%s_result_%02d_%08x.tga", _comp->GetName().Get(),
				   s_nDumpCount++, (int)this);
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
typedef bool (*TBuildNodeFromKVFunc)(CTCStage **ppOutStage, const char *_key, KeyValues *_kv,
									 uint32 nTexCompositeCreateFlags);
...
// ------------------------------------------------------------------------------------------------
template<typename S>
void ParseIntoStruct(S *_outStruct, CUtlVector<KeyValues *> *_leftovers, KeyValues *_kv,
					 uint32 nTexCompositeCreateFlags, const ParseTableEntry *_entries)
{
	Assert(_leftovers);
...
// ------------------------------------------------------------------------------------------------
bool ParseNodes(CUtlVector<CTCStage *> *_outStages, const CUtlVector<KeyValues *> &_kvs,
				uint32 nTexCompositeCreateFlags)
{
	tmZone(TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__);
...
bool ApplyStickerStageFromKV(CTCStage **ppOutStage, const char *_key, KeyValues *_kv, uint32 nTexCompositeCreateFlags)
{
	...
	int setCount = 0;
	if(assp.m_vDestBL.m_bSet)
		++setCount;
	if(assp.m_vDestTL.m_bSet)
		++setCount;
	if(assp.m_vDestTR.m_bSet)
		++setCount;
	if(setCount != 3)
		return false;
...
// ------------------------------------------------------------------------------------------------
KeyValues *ResolveTemplate(const char *pRootName, KeyValues *pValues, uint32 nTexCompositeCreateFlags,
						   bool *pInOutAllocdNew)
{
...
CUtlString GetErrorTrail(CUtlVector<const char *> &errorStack)
{
	...
	char *pDstOrig = retStr.GetForModify();
	pDstOrig;
...
int SubstituteVarsRecursive(char *pOutBuffer, int *pOutSubsts, CUtlVector<const char *> &errorStack, const char *pStr,
							uint32 nTexCompositeCreateFlags, const VariableDefs_t &varDefs)
{
	ParseMode mode = Copy;
		...
		switch(mode)
		{
			...
			case DetermineStringForReplace:
				if(srcC[0] == ']')
				{
					...
					int charsWritten =
						SubstituteVarsRecursive(pOutBuffer ? &pDst[dstPos] : NULL, pOutSubsts, errorStack, pSubstText,
												nTexCompositeCreateFlags, varDefs);
					if(charsWritten < 0)
						return -1;
...
bool SubstituteVars(CUtlString *pOutStr, int *pOutSubsts, CUtlVector<const char *> &errorStack, const char *pStr,
					uint32 nTexCompositeCreateFlags, const VariableDefs_t &varDefs)
{
	...
	// Even though this involves a traversal, we're saving a malloc by walking this thing once looking for the start
	// token.
	...
	int finalLen =
		SubstituteVarsRecursive(outStr.GetForModify(), pOutSubsts, errorStack, pStr, nTexCompositeCreateFlags, varDefs);
...
// ------------------------------------------------------------------------------------------------
bool ResolveAllVariablesRecursive(CUtlVector<const char *> &errorStack, const VariableDefs_t &varDefs,
								  KeyValues *pKeyValues, uint32 nTexCompositeCreateFlags, CUtlString &tmpStr)
{
	// hope for the best
	bool success = true;
...
// ------------------------------------------------------------------------------------------------
KeyValues *ResolveAllVariables(const char *pRootName, const VariableDefs_t &varDefs, KeyValues *pKeyValues,
							   uint32 nTexCompositeCreateFlags, bool *pInOutAllocdNew)
{
	...
	if(!(*pInOutAllocdNew) && varDefs.Count() > 0 &&
	   ((nTexCompositeCreateFlags & TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) == 0))
	{
...
KeyValues *ParseTopLevelIntoKV(const char *pRootName, KeyValues *pValues, uint32 nTexCompositeCreateFlags,
							   bool *pOutAllocdNew)
{
	...
	Assert(bRequiresCleanup || varDefs.Count() == 0 ||
		   ((nTexCompositeCreateFlags & TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) != 0));
...
// ------------------------------------------------------------------------------------------------
CTextureCompositor *CreateTextureCompositor(int _w, int _h, const char *pCompositeName, int nTeamNum,
											uint64 nRandomSeed, KeyValues *_stageDesc, uint32 nTexCompositeCreateFlags)
{
	...
	const char *pTemplateName = NULL;
	// If we're just doing a template verification, and we still have keys or values that look like template stuff, bail
	// out now.
	if(HasTemplateOrVariables(&pTemplateName, _stageDesc) &&
	   ((nTexCompositeCreateFlags & TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY) != 0))
	{
		CTextureCompositor *pComp =
			new CTextureCompositor(_w, _h, nTeamNum, pCompositeName, nRandomSeed, nTexCompositeCreateFlags);
		if(pTemplateName)
	...
	CTextureCompositor *texCompositor =
		new CTextureCompositor(_w, _h, nTeamNum, pCompositeName, nRandomSeed, nTexCompositeCreateFlags);
	if(pTemplateName)
...
CTextureCompositorTemplate *CTextureCompositorTemplate::Create(const char *pName, KeyValues *pTmplDesc)
{
	...
	CTextureCompositor *texCompositor = CreateTextureCompositor(1, 1, pName, 2, 0, pTmplDesc,
																TEX_COMPOSITE_CREATE_FLAGS_VERIFY_SCHEMA_ONLY |
																	TEX_COMPOSITE_CREATE_FLAGS_VERIFY_TEMPLATE_ONLY);
...
void ComputeTextureMatrixFromRectangle(VMatrix *pOutMat, const Vector2D &bl, const Vector2D &tl, const Vector2D &tr)
{
	...
	float rotationD = RAD2DEG(acos(leftEdge.y / magLeftEdge)) * (leftEdge.x < 0 ? 1 : -1);
