...
public:
	...
	{
		return GetCurrentMaterialInternal();
	}
	virtual void *GetCurrentProxy()
	{
		return m_pCurrentProxyData;
	}
	...
	void PushRenderTargetAndViewport(ITexture *pTexture, ITexture *pDepthTexture, int nViewX, int nViewY, int nViewW,
									 int nViewH);
...
protected:
	void OnAsyncCreateTextureFromRenderTarget(ITexture *pSrcRt, const char **pDstName,
	...
	void OnAsyncCopyRenderTargetToStagingTexture(ITexture *pDst, ITexture *pSrc,
												 IAsyncTextureOperationReceiver *pRecipient);
...
protected:
	...
	{
		float fValues[4];
		bool bHack_IsHeightClipPlane; // used to hack in compatibility between the user clip planes and the existing
									  // height clip plane code
		// I'm doing the hack this way to retain modder's flexibility to mess with clip plane ordering so that they can
		// make special hacks on their side if necessary
...
public:
	...
	IMesh *GetDynamicMesh(bool buffered, IMesh *pVertexOverride = 0, IMesh *pIndexOverride = 0,
	...
	// custom clip planes, beware that only the most recently pushed plane will actually be used in some hardware
	// configurations
	void PushCustomClipPlane(const float *fPlane);
	void PopCustomClipPlane(void);
	void ApplyCustomClipPlanes(
		void); // updates the clip planes based on how many are supported by the hardware using the top of the stack
			   // first, at the end of the stack, the height clip plane will be evaluated
	...
	void DrawClearBufferQuad(unsigned char r, unsigned char g, unsigned char b, unsigned char a, bool bClearColor,
							 bool bClearAlpha, bool bClearDepth);
	...
	void CopyRenderTargetToTextureEx(ITexture *pTexture, int nRenderTargetID, Rect_t *pSrcRect,
									 Rect_t *pDstRect = NULL);
	void CopyTextureToRenderTargetEx(int nRenderTargetID, ITexture *pTexture, Rect_t *pSrcRect,
									 Rect_t *pDstRect = NULL);
...
//-----------------------------------------------------------------------------
inline IMesh *CMatRenderContext::CreateStaticMesh(VertexFormat_t vertexFormat, const char *pTextureBudgetGroup,
												  IMaterial *pMaterial)
{
