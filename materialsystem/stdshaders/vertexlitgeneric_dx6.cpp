...
BEGIN_SHADER(VertexLitGeneric_DX6, "Help for VertexLitGeneric_DX6")
	...
	void MultiplyByVertexLighting(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow)
	{
		SHADOW_STATE
		{
			...
			pShaderShadow->CustomTextureOperation(SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_COLOR,
			...
			pShaderShadow->CustomTextureOperation(SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_ALPHA,
												  SHADER_TEXOP_SELECTARG1, SHADER_TEXARG_NONE, SHADER_TEXARG_NONE);
	...
	void DrawBaseTimesVertexLighting(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI,
	...
	void DrawEnvmapTimesVertexLighting(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI,
									   IShaderShadow * pShaderShadow)
	{
		SHADOW_STATE
		{
		...
		DYNAMIC_STATE
		{
			SetDynamicEnvMappingState(ENVMAP, ENVMAPMASK, BASETEXTURE, ENVMAPFRAME, ENVMAPMASKFRAME, FRAME,
	...
	void DrawMode1(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow)
	{
		...
		if(!hasEnvMapTint && !texDefined && !IS_FLAG_SET(MATERIAL_VAR_VERTEXCOLOR) && !IsColorModulating())
		{
		...
		{
			FixedFunctionMaskedEnvmapPass(ENVMAP, ENVMAPMASK, BASETEXTURE, ENVMAPFRAME, ENVMAPMASKFRAME, FRAME,
		...
		if(IS_FLAG_SET(MATERIAL_VAR_MULTIPASS))
		{
			...
			{
				FixedFunctionAdditiveMaskedEnvmapPass(ENVMAP, ENVMAPMASK, BASETEXTURE, ENVMAPFRAME, ENVMAPMASKFRAME,
													  FRAME, BASETEXTURETRANSFORM, ENVMAPMASKSCALE, ENVMAPTINT);
			}
		}
		...
		{
			FixedFunctionSelfIlluminationPass(SHADER_SAMPLER0, BASETEXTURE, FRAME, BASETEXTURETRANSFORM, SELFILLUMTINT);
		}
	}
	...
	void DrawMode0(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow)
	{
		...
		if(params[BASETEXTURE]->IsTexture())
		{
			...
			if(IS_FLAG_SET(MATERIAL_VAR_SELFILLUM))
			{
				FixedFunctionSelfIlluminationPass(SHADER_SAMPLER0, BASETEXTURE, FRAME, BASETEXTURETRANSFORM,
												  SELFILLUMTINT);
			}
		}
		else
		...
		{
			FixedFunctionAdditiveMaskedEnvmapPass(ENVMAP, ENVMAPMASK, BASETEXTURE, ENVMAPFRAME, ENVMAPMASKFRAME, FRAME,
												  BASETEXTURETRANSFORM, ENVMAPMASKSCALE, ENVMAPTINT);
		}
	}
