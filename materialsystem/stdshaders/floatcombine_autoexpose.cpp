...
BEGIN_VS_SHADER(floatcombine_autoexpose, "Help for floatcombine_autoexpose")
	...
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			float c0[4] = {params[SHARPNESS]->GetFloatValue(), params[WOODCUT]->GetFloatValue(),
						   params[BLOOMAMOUNT]->GetFloatValue(), params[ALPHASHARPENFACTOR]->GetFloatValue()};
			float c1[4] = {params[BLOOMEXPONENT]->GetFloatValue(), params[VIGNETTE_MIN_BRIGHT]->GetFloatValue(),
						   params[VIGNETTE_POWER]->GetFloatValue(), params[EDGE_SOFTNESS]->GetFloatValue()};
			float c2[4] = {params[AUTOEXPOSE_MIN]->GetFloatValue(), params[AUTOEXPOSE_MAX]->GetFloatValue(), 0, 0};
