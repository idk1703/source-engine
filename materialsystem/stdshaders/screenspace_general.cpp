...
BEGIN_VS_SHADER_FLAGS(screenspace_general_dx9, "Help for screenspace_general", SHADER_NOT_EDITABLE)
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			if(params[BASETEXTURE]->IsDefined())
			{
				...
				if((fmt == IMAGE_FORMAT_RGBA16161616F) || (fmt == IMAGE_FORMAT_RGBA16161616))
					...
					pShaderShadow->EnableSRGBRead(SHADER_SAMPLER0, !params[LINEARREAD_BASETEXTURE]->IsDefined() ||
																	   !params[LINEARREAD_BASETEXTURE]->GetIntValue());
			}
			if(params[TEXTURE1]->IsDefined())
			{
				...
				if((fmt == IMAGE_FORMAT_RGBA16161616F) || (fmt == IMAGE_FORMAT_RGBA16161616))
					...
					pShaderShadow->EnableSRGBRead(SHADER_SAMPLER1, !params[LINEARREAD_TEXTURE1]->IsDefined() ||
																	   !params[LINEARREAD_TEXTURE1]->GetIntValue());
			}
			if(params[TEXTURE2]->IsDefined())
			{
				...
				if((fmt == IMAGE_FORMAT_RGBA16161616F) || (fmt == IMAGE_FORMAT_RGBA16161616))
					...
					pShaderShadow->EnableSRGBRead(SHADER_SAMPLER2, !params[LINEARREAD_TEXTURE2]->IsDefined() ||
																	   !params[LINEARREAD_TEXTURE2]->GetIntValue());
			}
			if(params[TEXTURE3]->IsDefined())
			{
				...
				if((fmt == IMAGE_FORMAT_RGBA16161616F) || (fmt == IMAGE_FORMAT_RGBA16161616))
					...
					pShaderShadow->EnableSRGBRead(SHADER_SAMPLER3, !params[LINEARREAD_TEXTURE3]->IsDefined() ||
																	   !params[LINEARREAD_TEXTURE3]->GetIntValue());
			}
			...
			SET_STATIC_VERTEX_SHADER_COMBO(
				X360APPCHOOSER, IS_PARAM_DEFINED(X360APPCHOOSER) ? params[X360APPCHOOSER]->GetIntValue() : 0);
			...
			if(g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				...
				if((iLength > 5) && (Q_stricmp(&szPixelShader[iLength - 5], "_ps20") ==
									 0)) // detect if it's trying to load a ps20 shader
				{
		...
		DYNAMIC_STATE
		{
			...
			float c0[] = {params[C0_X]->GetFloatValue(), params[C0_Y]->GetFloatValue(), params[C0_Z]->GetFloatValue(),
						  params[C0_W]->GetFloatValue(), params[C1_X]->GetFloatValue(), params[C1_Y]->GetFloatValue(),
						  params[C1_Z]->GetFloatValue(), params[C1_W]->GetFloatValue(), params[C2_X]->GetFloatValue(),
						  params[C2_Y]->GetFloatValue(), params[C2_Z]->GetFloatValue(), params[C2_W]->GetFloatValue(),
						  params[C3_X]->GetFloatValue(), params[C3_Y]->GetFloatValue(), params[C3_Z]->GetFloatValue(),
