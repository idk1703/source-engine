...
BEGIN_VS_SHADER_FLAGS(Spritecard_DX8, "Help for Spritecard_DX8", SHADER_NOT_EDITABLE)
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(SEQUENCE_BLEND_MODE, SHADER_PARAM_TYPE_INTEGER, "0",
					 "defines the blend mode between the images un dual sequence particles. 0 = avg, 1=alpha from "
					 "first, rgb from 2nd, 2= first over second")
		SHADER_PARAM(MAXLUMFRAMEBLEND1, SHADER_PARAM_TYPE_INTEGER, "0",
					 "instead of blending between animation frames for the first sequence, select pixels based upon "
					 "max luminance")
		SHADER_PARAM(
			MAXLUMFRAMEBLEND2, SHADER_PARAM_TYPE_INTEGER, "0",
			"instead of blending between animation frames for the 2nd sequence, select pixels based upon max luminance")
		SHADER_PARAM(
			RAMPTEXTURE, SHADER_PARAM_TYPE_TEXTURE, "",
		...
		SHADER_PARAM(ZOOMANIMATESEQ2, SHADER_PARAM_TYPE_FLOAT, "1.0",
					 "amount to gradually zoom between frames on the second sequence. 2.0 will double the size of a "
					 "frame over its lifetime.")
		...
		SHADER_PARAM(USEINSTANCING, SHADER_PARAM_TYPE_BOOL, "1",
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			pShaderShadow->VertexShaderVertexFormat(flags, numTexCoords,
			...
			else
			{
				...
				if(g_pHardwareConfig->SupportsPixelShaders_2_b())
				{
					...
					SET_STATIC_PIXEL_SHADER_COMBO(SEQUENCE_BLEND_MODE,
					...
					SET_STATIC_PIXEL_SHADER_COMBO(MAXLUMFRAMEBLEND2,
		...
		DYNAMIC_STATE
		{
			...
			float VC0[8] = {params[MINSIZE]->GetFloatValue(),
							...
							flStartFade,
							(float)(1.0 / (flMaxDistance - flStartFade)),
							0,
							0};
			...
			float C0[4] = {params[ADDBASETEXTURE2]->GetFloatValue(), params[OVERBRIGHTFACTOR]->GetFloatValue(),
