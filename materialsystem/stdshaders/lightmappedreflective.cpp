...
BEGIN_VS_SHADER(LightmappedReflective_DX90, "Help for Lightmapped Reflective")
	...
	SHADER_INIT
	{
		if(params[REFRACTTEXTURE]->IsDefined())
		{
			LoadTexture(REFRACTTEXTURE,
						g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER || IsOSX() ? TEXTUREFLAGS_SRGB : 0);
		}
										 ...
										 IShaderDynamicAPI * pShaderAPI, bool bReflection, bool bRefraction)
	{
		...
		SHADOW_STATE
		{
			...
			if(bRefraction)
			{
				...
				pShaderShadow->EnableSRGBRead(SHADER_SAMPLER0,
											  g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER || IsOSX());
			...
			if(g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				...
				SET_STATIC_PIXEL_SHADER_COMBO(ENVMAPMASK,
		...
		DYNAMIC_STATE
		{
			...
			float c3[4] = {flFresnelFactor, params[FRESNELPOWER]->GetFloatValue(),
						   params[MINREFLECTIVITY]->GetFloatValue(), 0.0f};
