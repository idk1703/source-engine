								   ...
								   VertexCompressionType_t vertexCompression)
{
	...
	DYNAMIC_STATE
	{
		...
		if(1)
		{
			...
			vPsConst0[0] = IS_PARAM_DEFINED(info.m_nBlendStrength) ? params[info.m_nBlendStrength]->GetFloatValue()
			...
			vPsConst0[1] = IS_PARAM_DEFINED(info.m_nTime) && params[info.m_nTime]->GetFloatValue() > 0.0f
							   ? params[info.m_nTime]->GetFloatValue()
							   : pShaderAPI->CurrentTime();
			vPsConst0[1] -= (float)((int)(vPsConst0[1] / 1000.0f)) * 1000.0f;

			// Dest alpha value for warping mask - NOTE: If we want to use this, we have to modify the blending mode
			// above!
			// if ( ( params[info.m_nWarpParam]->GetFloatValue() > 0.0f ) && (
			// params[info.m_nWarpParam]->GetFloatValue() < 1.0f ) ) 	tmpVec[2] = 1.0f; else 	tmpVec[2] = 0.0f;
		}
		pShaderAPI->SetPixelShaderConstant(0, vPsConst0, 1);

		// Scroll vector
		pShaderAPI->SetPixelShaderConstant(1,
										   IS_PARAM_DEFINED(info.m_nEmissiveScrollVector)
											   ? params[info.m_nEmissiveScrollVector]->GetVecValue()
											   : kDefaultEmissiveScrollVector,
										   1);
