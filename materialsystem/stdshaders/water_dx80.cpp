...
BEGIN_VS_SHADER(Water_DX80, "Help for Water_DX80")
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(CHEAPWATERSTARTDISTANCE, SHADER_PARAM_TYPE_FLOAT, "",
					 "This is the distance from the eye in inches that the shader should start transitioning to a "
					 "cheaper water shader.")
		SHADER_PARAM(CHEAPWATERENDDISTANCE, SHADER_PARAM_TYPE_FLOAT, "",
					 "This is the distance from the eye in inches that the shader should finish transitioning to a "
					 "cheaper water shader.")
	...
	inline void SetCheapWaterFactors(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, int nConstantReg)
	{
		...
		float flCheapWaterConstants[4] = {flCheapWaterStartDistance,
										  1.0f / (flCheapWaterEndDistance - flCheapWaterStartDistance), 0.0f, 0.0f};
		pShaderAPI->SetVertexShaderConstant(nConstantReg, flCheapWaterConstants);
	}

	inline void DrawReflection(IMaterialVar * *params, IShaderShadow * pShaderShadow, IShaderDynamicAPI * pShaderAPI,
							   bool bBlendReflection)
	{
		SHADOW_STATE
		{
	...
	inline void DrawCheapWater(IMaterialVar * *params, IShaderShadow * pShaderShadow, IShaderDynamicAPI * pShaderAPI,
							   bool bBlend, bool bBlendFresnel, bool bNoPerVertexFresnel)
	{
		SHADOW_STATE
		{
			SetInitialShadowState();
			...
			pShaderShadow->VertexShaderVertexFormat(
				VERTEX_POSITION | VERTEX_NORMAL | VERTEX_TANGENT_S | VERTEX_TANGENT_T, 1, 0, 0);
	...
	SHADER_DRAW
	{
		...
		{
			bool bNoPerVertexFresnel =
				(params[NOFRESNEL]->GetIntValue() || bReflectionUseFresnel || bForceCheap || !bRefraction);
			DrawCheapWater(params, pShaderShadow, pShaderAPI, bBlendReflection, bReflectionUseFresnel,
						   bNoPerVertexFresnel);
		}
	}
END_SHADER