...
BEGIN_VS_SHADER(pyro_vision, "Help for pyro vision")
	...
	SHADER_DRAW
	{
		...
		bool bHasBlendModulateTexture = bHasBaseTexture2 && (params[BLENDMODULATETEXTURE]->IsDefined()) &&
										(params[BLENDMODULATETEXTURE]->IsTexture());
		...
		if(pShaderAPI && pContextData->m_bMaterialVarsChanged)
		{
			...
			if(bHasBlendModulateTexture)
			{
				pContextData->m_SemiStaticCmdsOut.SetVertexShaderTextureTransform(
					VERTEX_SHADER_SHADER_SPECIFIC_CONST_10, BLENDMASKTRANSFORM);
			}
