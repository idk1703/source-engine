...
BEGIN_SHADER(ShatteredGlass_DX7, "Help for ShatteredGlass_DX7")
	...
	SHADER_DRAW
	{
		...
		FixedFunctionMultiplyByLightmapPass(BASETEXTURE, FRAME, BASETEXTURETRANSFORM,
											params[LIGHTMAPTINT]->GetFloatValue());

		// Pass 3 : + env
		bool envDefined = params[ENVMAP]->IsTexture();
		if(envDefined)
		{
			FixedFunctionAdditiveMaskedEnvmapPass(ENVMAP, BASETEXTURE, BASETEXTURE, ENVMAPFRAME, FRAME, FRAME,
