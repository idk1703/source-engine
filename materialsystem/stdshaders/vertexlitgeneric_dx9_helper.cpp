...
static inline bool WantsSkinShader(IMaterialVar **params, const VertexLitGeneric_DX9_Vars_t &info)
{
	...
	if((info.m_nDiffuseWarpTexture != -1) &&
	   params[info.m_nDiffuseWarpTexture]->IsTexture()) // If there's Phong and diffuse warp do skin
		return true;
									...
									bool bVertexLitGeneric, VertexLitGeneric_DX9_Vars_t &info)
{
	...
	if(info.m_nAlbedo != -1 && g_pConfig->UseBumpmapping() && info.m_nBumpmap != -1 &&
	   params[info.m_nBumpmap]->IsDefined() && params[info.m_nAlbedo]->IsDefined() &&
	...
	)
	{
		SET_FLAGS2(MATERIAL_VAR2_NEEDS_TANGENT_SPACES);
			...
			params[info.m_nDiffuseWarpTexture]->IsDefined()) // diffuse warp goes down bump path...
	{
		SET_FLAGS2(MATERIAL_VAR2_NEEDS_TANGENT_SPACES);
	}
	...
	if(IS_FLAG_SET(MATERIAL_VAR_BASEALPHAENVMAPMASK) && info.m_nBumpmap != -1 && params[info.m_nBumpmap]->IsDefined() &&
	   !hasNormalMapAlphaEnvmapMask)
	{
		Warning("material %s has a normal map and $basealphaenvmapmask.  Must use $normalmapalphaenvmapmask to get "
				"specular.\n\n",
				pMaterialName);
		params[info.m_nEnvmap]->SetUndefined();
	}

	if(info.m_nEnvmapMask != -1 && params[info.m_nEnvmapMask]->IsDefined() && info.m_nBumpmap != -1 &&
	...
	if(!g_pConfig->UseSpecular() && info.m_nEnvmap != -1 && params[info.m_nEnvmap]->IsDefined() &&
	   params[info.m_nBaseTexture]->IsDefined())
	{
		params[info.m_nEnvmap]->SetUndefined();
	}
							  ...
							  VertexLitGeneric_DX9_Vars_t &info)
{
	...
	if(bNeedsSkinBecauseOfDetail ||
	   (info.m_nPhong != -1 && params[info.m_nPhong]->GetIntValue() && g_pHardwareConfig->SupportsPixelShaders_2_b()))
	{
	...
	if(info.m_nBaseTexture != -1 && params[info.m_nBaseTexture]->IsDefined())
	{
		pShader->LoadTexture(info.m_nBaseTexture,
							 (info.m_nGammaColorRead != -1) && (params[info.m_nGammaColorRead]->GetIntValue() == 1)
								 ? 0
								 : TEXTUREFLAGS_SRGB);
	...
	bool bHasSelfIllumMask = IS_FLAG_SET(MATERIAL_VAR_SELFILLUM) && (info.m_nSelfIllumMask != -1) &&
	...
	{
		bool bHasSelfIllumFresnel = IS_FLAG_SET(MATERIAL_VAR_SELFILLUM) && (info.m_nSelfIllumFresnel != -1) &&
	...
	if(info.m_nDetail != -1 && params[info.m_nDetail]->IsDefined())
	{
		int nDetailBlendMode =
			(info.m_nDetailTextureCombineMode == -1) ? 0 : params[info.m_nDetailTextureCombineMode]->GetIntValue();
	...
	if(info.m_nEnvmap != -1 && params[info.m_nEnvmap]->IsDefined())
	{
		if(!IS_FLAG_SET(MATERIAL_VAR_ENVMAPSPHERE))
		{
			pShader->LoadCubeMap(info.m_nEnvmap,
								 g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE ? TEXTUREFLAGS_SRGB : 0);
		}
		else
		{
...

{
	...
	bool bHasDiffuseWarp = (!bHasFlashlight || IsX360()) && hasDiffuseLighting && (info.m_nDiffuseWarpTexture != -1) &&
	...
	if(bHasBaseTexture)
	{
		...
		nBlendType = pShader->EvaluateBlendRequirements(bBlendTintByBaseAlpha ? -1 : info.m_nBaseTexture, true,
	...
	if(pShader->IsSnapshotting() || (!pContextData) || (pContextData->m_bMaterialVarsChanged))
	{
		/*^*/ // 	printf("\t\t[1] snapshotting=%d  pContextData=%08x  pContextData->m_bMaterialVarsChanged=%d
			  // \n",(int)pShader->IsSnapshotting(), (int)pContextData, pContextData ?
			  // (int)pContextData->m_bMaterialVarsChanged : -1 );
		...
		bool bHasEnvmapMask =
			(!bHasFlashlight || IsX360()) && info.m_nEnvmapMask != -1 && params[info.m_nEnvmapMask]->IsTexture();
		bool bHasSelfIllumFresnel = (!IsTextureSet(info.m_nDetail, params)) && (bHasSelfIllum) &&
									(info.m_nSelfIllumFresnel != -1) &&
									(params[info.m_nSelfIllumFresnel]->GetIntValue() != 0);

		bool bHasSelfIllumMask = bHasSelfIllum && IsTextureSet(info.m_nSelfIllumMask, params);
		bool hasSelfIllumInEnvMapMask = (info.m_nSelfIllumEnvMapMask_Alpha != -1) &&
		...
		if(pShader->IsSnapshotting())
		{
			...
			bool bHasEnvmap =
				(!bHasFlashlight || IsX360()) && (info.m_nEnvmap != -1) && params[info.m_nEnvmap]->IsTexture();
			...
			if(IsPC())
			{
				// On PC, LIGHTING_PREVIEW requires normals (they won't use much memory - unlitgeneric isn't used on
				// many models)
				bHasNormal = true;
			}
			...
			if(bHasBaseTexture)
			{
				...
				// If we're on OSX GL on a crappy OS which can't do sRGB from render targets, check to see if we're
				// reading from one...
			...
			if((info.m_nDepthBlend != -1) && (params[info.m_nDepthBlend]->GetIntValue()))
			{
				if(bHasBump)
					Warning(
						"DEPTHBLEND not supported by bump mapped variations of vertexlitgeneric to avoid shader bloat. "
						"Either remove the bump map or convince a graphics programmer that it's worth it.\n");
			...
			// Lightmaps are generated in gamma space, but not sRGB, so leave that disabled. Conversion is done in the
			// shader.
			...
			if(bHasBump || bHasDiffuseWarp)
			{
...
#endif
				{
					...
					if(g_pHardwareConfig->SupportsPixelShaders_2_b() ||
					   g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders()) // Always send GL this way
					{
						...
						SET_STATIC_PIXEL_SHADER_COMBO(NORMALMAPALPHAENVMAPMASK,
													  hasNormalMapAlphaEnvmapMask && bHasEnvmap);
		...
		{
			/*^*/ // 		printf("\t\t[3] pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged )
				  // )  TRUE \n");
			...
			if(bHasBaseTexture)
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture(pShader, SHADER_SAMPLER0, info.m_nBaseTexture,
			...
			if(bHasDetailTexture)
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture(pShader, SHADER_SAMPLER2, info.m_nDetail,
															  info.m_nDetailFrame);
			}
			if(bHasSelfIllum)
			{
				...
				{
					pContextData->m_SemiStaticCmdsOut.BindTexture(pShader, SHADER_SAMPLER11, info.m_nSelfIllumMask,
																  -1); // Bind it
				}
				else // else
				{
					pContextData->m_SemiStaticCmdsOut.BindStandardTexture(SHADER_SAMPLER11,
																		  TEXTURE_BLACK); // Bind dummy
				}
			}

			if((info.m_nDepthBlend != -1) && (params[info.m_nDepthBlend]->GetIntValue()))
			{
				pContextData->m_SemiStaticCmdsOut.BindStandardTexture(SHADER_SAMPLER10,
			...
			if(bSeamlessDetail || bSeamlessBase)
			{
				...
				pContextData->m_SemiStaticCmdsOut.SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_2,
																		  flSeamlessData);
			}

			if(info.m_nBaseTextureTransform != -1)
			{
				pContextData->m_SemiStaticCmdsOut.SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
			...
			if(bHasDetailTexture)
			{
				if(IS_PARAM_DEFINED(info.m_nDetailTextureTransform))
					pContextData->m_SemiStaticCmdsOut.SetVertexShaderTextureScaledTransform(
						VERTEX_SHADER_SHADER_SPECIFIC_CONST_4, info.m_nDetailTextureTransform, info.m_nDetailScale);
				else
					pContextData->m_SemiStaticCmdsOut.SetVertexShaderTextureScaledTransform(
			...
			if(bHasEnvmap)
			{
				pContextData->m_SemiStaticCmdsOut.SetEnvMapTintPixelShaderDynamicStateGammaToLinear(
					0, info.m_nEnvmapTint, fTintReplaceFactor);
			}
			else
			{
				pContextData->m_SemiStaticCmdsOut.SetEnvMapTintPixelShaderDynamicStateGammaToLinear(0, -1,
																									fTintReplaceFactor);
			}

			if(bHasEnvmapMask)
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture(pShader, SHADER_SAMPLER4, info.m_nEnvmapMask,
			...
			if(bHasSelfIllumFresnel && (!bHasFlashlight || IsX360()))
			{
				...
				float flMin = IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
								  ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[0]
								  : 0.0f;
				float flMax = IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
								  ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[1]
								  : 1.0f;
				float flExp = IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
			...
			if(bHasDiffuseWarp && (!bHasFlashlight || IsX360()) && !bHasSelfIllumFresnel)
			{
				...
				{
					pContextData->m_SemiStaticCmdsOut.BindTexture(pShader, SHADER_SAMPLER9, info.m_nDiffuseWarpTexture,
																  -1);
				}
			}
			...
			bool bLightingOnly =
				bVertexLitGeneric && mat_fullbright.GetInt() == 2 && !IS_FLAG_SET(MATERIAL_VAR_NO_DEBUG_OVERRIDE);
			if(bLightingOnly)
			{
				if(bHasBaseTexture)
				{
			...
			if(bHasBump || bHasDiffuseWarp)
			{
				pContextData->m_SemiStaticCmdsOut.BindStandardTexture(SHADER_SAMPLER5,
	...
	if(pShaderAPI)
	{
		...
		{
			pShader->SetModulationPixelShaderDynamicState_LinearColorSpace_LinearScale(
		...
		if(!bHasBump && !bHasDiffuseWarp)
		{
			DynamicCmdsOut.SetDepthFeatheringPixelShaderConstant(13,
																 GetFloatParam(info.m_nDepthBlendScale, params, 50.0f));
		}
...
{
	if(WantsSkinShader(params, info) && g_pHardwareConfig->SupportsPixelShaders_2_b() && g_pConfig->UseBumpmapping() &&
	   g_pConfig->UsePhong())
	{
	...
	DrawVertexLitGeneric_DX9_Internal(pShader, params, pShaderAPI, pShaderShadow, bVertexLitGeneric, bHasFlashlight,
