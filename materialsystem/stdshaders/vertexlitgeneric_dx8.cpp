...
BEGIN_VS_SHADER(VertexLitGeneric_DX8, "Help for VertexLitGeneric")
	BEGIN_SHADER_PARAMS
	...
	void DrawUnbumpedUsingVertexShader(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI,
									   IShaderShadow * pShaderShadow, bool bSkipEnvmap)
	{
		SHADOW_STATE
		{
		...
		DYNAMIC_STATE
		{
			...
			{
				// write 255 to alpha if we aren't told to write depth to dest alpha (We don't ever actually write depth
				// to dest alpha in dx8)
	...
	SHADER_DRAW
	{
		...
		if(false /*disabled! no effect*/ && params[CLOAKPASSENABLED]->GetIntValue() &&
		...
		if(bDrawStandardPass)
		{
			...
			else if(bBump)
			{
				...
				if(params[ENVMAP]->IsTexture())
				{
					DrawModelBumpedSpecularLighting(BUMPMAP, BUMPFRAME, ENVMAP, ENVMAPFRAME, ENVMAPTINT, ALPHA,
													ENVMAPCONTRAST, ENVMAPSATURATION, BUMPTRANSFORM, bBlendSpecular);
				}
			}
			else
			{
