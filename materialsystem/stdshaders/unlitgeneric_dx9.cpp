...
BEGIN_VS_SHADER(UnlitGeneric, "Help for UnlitGeneric")
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(PHONGWARPTEXTURE, SHADER_PARAM_TYPE_TEXTURE, "shadertest/BaseTexture",
		...
		SHADER_PARAM(DISTANCEALPHA, SHADER_PARAM_TYPE_BOOL, "0",
					 "Use distance-coded alpha generated from hi-res texture by vtex.")
		SHADER_PARAM(DISTANCEALPHAFROMDETAIL, SHADER_PARAM_TYPE_BOOL, "0",
		...
		SHADER_PARAM(SCALEEDGESOFTNESSBASEDONSCREENRES, SHADER_PARAM_TYPE_BOOL, "0",
					 "Scale the size of the soft edges based upon resolution. 1024x768 = nominal.")
		SHADER_PARAM(EDGESOFTNESSSTART, SHADER_PARAM_TYPE_FLOAT, "0.6",
		...
		SHADER_PARAM(GLOWALPHA, SHADER_PARAM_TYPE_FLOAT, "1",
		...
		SHADER_PARAM(SCALEOUTLINESOFTNESSBASEDONSCREENRES, SHADER_PARAM_TYPE_BOOL, "0",
	...
	SHADER_DRAW
	{
		...
		if((pShaderShadow == NULL) && (pShaderAPI != NULL) && !bNewFlashlightPath &&
		   pShaderAPI->InFlashlightMode()) // Not snapshotting && flashlight pass
		{
			Draw(false);
		}
		else
		{
			DrawVertexLitGeneric_DX9(this, params, pShaderAPI, pShaderShadow, false, vars, vertexCompression,
									 pContextDataPtr);
		}
	}
END_SHADER