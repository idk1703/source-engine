...
BEGIN_VS_SHADER_FLAGS(ParticleSphere_DX9, "Help for BumpmappedEnvMap", SHADER_NOT_EDITABLE)
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(USINGPIXELSHADER, SHADER_PARAM_TYPE_BOOL, "0",
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_COLOR, 1, tCoordDimensions, 0);
		...
		DYNAMIC_STATE
		{
			...
			pShaderAPI->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
												params[LIGHT_POSITION]->GetVecValue());
			...
			SET_DYNAMIC_VERTEX_SHADER_COMBO(FOGTYPE,
											s_pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z);
