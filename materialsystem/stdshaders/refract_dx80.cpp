...
BEGIN_VS_SHADER(Refract_DX80, "Help for Refract_DX80")
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(FADEOUTONSILHOUETTE, SHADER_PARAM_TYPE_BOOL, "1",
		...
		SHADER_PARAM(FORCEREFRACT, SHADER_PARAM_TYPE_BOOL, "0",
					 "Forces refraction on boards that have poor performance")
	...
	SHADER_DRAW
	{
		...
		if(bHasEnvmap)
		{
			...
			if(bIsModel)
			{
				DrawModelBumpedSpecularLighting(NORMALMAP, BUMPFRAME, ENVMAP, ENVMAPFRAME, ENVMAPTINT, ALPHA,
												ENVMAPCONTRAST, ENVMAPSATURATION, BUMPTRANSFORM, bBlendSpecular,
												bNoWriteZ);
			}
			else
			{
				DrawWorldBumpedSpecularLighting(NORMALMAP, ENVMAP, BUMPFRAME, ENVMAPFRAME, ENVMAPTINT, ALPHA,
