...
// SKIP: defined $PIXELFOGTYPE && defined $WRITEWATERFOGTODESTALPHA && ( $PIXELFOGTYPE != 1 ) &&
// $WRITEWATERFOGTODESTALPHA
// --------------------------------------------------------------------------------
// We don't need fog in the pixel shader if we aren't in float fog mode2
// NOSKIP: defined $HDRTYPE && defined $HDRENABLED && defined $PIXELFOGTYPE && $HDRTYPE != HDR_TYPE_FLOAT && $FOGTYPE !=
// 0
// --------------------------------------------------------------------------------
...
#define PIXEL_FOG_TYPE_NONE \
	-1 // MATERIAL_FOG_NONE is handled by PIXEL_FOG_TYPE_RANGE, this is for explicitly disabling fog in the shader
...
{
	return dot( color, HALF3( HALF_CONSTANT(0.30f) / MAX_HDR_OVERBRIGHT, HALF_CONSTANT(0.59f) / MAX_HDR_OVERBRIGHT,
HALF_CONSTANT(0.11f) / MAX_HDR_OVERBRIGHT ) );
}
*/
...
float CalcWaterFogAlpha(const float flWaterZ, const float flEyePosZ, const float flWorldPosZ, const float flProjPosZ,
						const float flFogOORange)
{
	//	float flDepthFromWater = flWaterZ - flWorldPosZ + 2.0f; // hackity hack . .this is for the DF_FUDGE_UP in
	//view_scene.cpp
...
float CalcRangeFog(const float flProjPosZ, const float flFogStartOverRange, const float flFogMaxDensity,
				   const float flFogOORange)
{
...
float CalcPixelFogFactor(int iPIXELFOGTYPE, const float4 fogParams, const float flEyePosZ, const float flWorldPosZ,
...
{
	if(iPIXELFOGTYPE ==
	   PIXEL_FOG_TYPE_RANGE) // either range fog or no fog depending on fog parameters and whether this is ps20 or ps2b
	{
#if !(defined(SHADER_MODEL_PS_1_1) || defined(SHADER_MODEL_PS_1_4) || \
	  defined(SHADER_MODEL_PS_2_0)) // Minimum requirement of ps2b
		pixelFogFactor = saturate(pixelFogFactor);
		return lerp(vShaderColor.rgb, vFogColor.rgb,
					pixelFogFactor *
						pixelFogFactor); // squaring the factor will get the middle range mixing closer to hardware fog
#else
		return vShaderColor;
#endif
	}
...
LPREVIEW_PS_OUT FinalOutput(const LPREVIEW_PS_OUT vShaderColor, float pixelFogFactor, const int iPIXELFOGTYPE,
...
float2 CalcParallaxedTexCoord(float2 inTexCoord, float2 vParallax, float3 vNormal, float3 vViewW,
...
float DepthFeathering(sampler DepthSampler, const float2 vScreenPos, float fProjZ, float fProjW,
					  float4 vDepthBlendConstants)
{
#if(!(defined(SHADER_MODEL_PS_1_1) || defined(SHADER_MODEL_PS_1_4) || \
	  defined(SHADER_MODEL_PS_2_0))) // minimum requirement of ps2b
	{
...
#define flSpriteDepth flDepths.y
...
#else
		{
			...
			flFeatheredAlpha = max(
				...
				flFeatheredAlpha); // as the sprite approaches the edge of our compressed depth space, the math stops
								   // working. So as the sprite approaches the far depth, smoothly remove feathering.
			flFeatheredAlpha = saturate(flFeatheredAlpha);
		}
#endif
