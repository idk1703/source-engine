			...
			bool NeedsPowerOfTwoFrameBufferTexture( IMaterialVar **params, bool bCheckSpecificToThisFrame ) const
			{
				if ( params[CLOAKPASSENABLED]->GetIntValue() ) // If material supports cloaking
				{
					if ( bCheckSpecificToThisFrame == false ) // For setting model flag at load time
						return true;
					else if ( ( params[CLOAKFACTOR]->GetFloatValue() > 0.0f ) && ( params[CLOAKFACTOR]->GetFloatValue()
< 1.0f ) ) // Per-frame check return true;
			...
			bool IsTranslucent( IMaterialVar **params ) const
			{
				if ( params[CLOAKPASSENABLED]->GetIntValue() ) // If material supports cloaking
				{
					if ( ( params[CLOAKFACTOR]->GetFloatValue() > 0.0f ) && ( params[CLOAKFACTOR]->GetFloatValue()
< 1.0f ) ) // Per-frame check return true;
		...
		Modify SHADER_DRAW to look something like this:
			...
			if ( params[CLOAKPASSENABLED]->GetIntValue() )
			{
				...
				if ( ( pShaderShadow != NULL ) || ( ( params[CLOAKFACTOR]->GetFloatValue() > 0.0f ) && (
...
void InitParamsCloakBlendedPass(CBaseVSShader *pShader, IMaterialVar **params, const char *pMaterialName,
								CloakBlendedPassVars_t &info)
{
	// Set material flags
	...
	{
		params[info.m_nCloakColorTint]->SetVecValue(kDefaultCloakColorTint[0], kDefaultCloakColorTint[1],
													kDefaultCloakColorTint[2], kDefaultCloakColorTint[3]);
	}
}
...
void DrawCloakBlendedPass(CBaseVSShader *pShader, IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
						  IShaderShadow *pShaderShadow, CloakBlendedPassVars_t &info,
						  VertexCompressionType_t vertexCompression)
{
	...
	DYNAMIC_STATE
	{
		...
		vVsConst3[1] = clamp(IS_PARAM_DEFINED(info.m_nCloakFactor) ? params[info.m_nCloakFactor]->GetFloatValue()
																   : kDefaultCloakFactor,
							 0.0f, 1.0f);
		...
		vVsConst4[0] = params[info.m_nRefractAmount]->GetFloatValue() *
					   (1.0f - clamp(params[info.m_nCloakFactor]->GetFloatValue(), 0.0f, 1.0f));
		...
		float fColorTintStrength =
			clamp((clamp(IS_PARAM_DEFINED(info.m_nCloakFactor) ? params[info.m_nCloakFactor]->GetFloatValue()
															   : kDefaultCloakFactor,
						 0.0f, 1.0f) -
				   0.75f) *
					  4.0f,
				  0.0f, 1.0f);
		vPsConst0[0] = IS_PARAM_DEFINED(info.m_nCloakColorTint) ? params[info.m_nCloakColorTint]->GetVecValue()[0]
																: kDefaultCloakColorTint[0];
		vPsConst0[1] = IS_PARAM_DEFINED(info.m_nCloakColorTint) ? params[info.m_nCloakColorTint]->GetVecValue()[1]
																: kDefaultCloakColorTint[1];
		vPsConst0[2] = IS_PARAM_DEFINED(info.m_nCloakColorTint) ? params[info.m_nCloakColorTint]->GetVecValue()[2]
																: kDefaultCloakColorTint[2];
...
{
	float flCloakFactor =
		IS_PARAM_DEFINED(info.m_nCloakFactor) ? params[info.m_nCloakFactor]->GetFloatValue() : kDefaultCloakFactor;
	// float flRefractAmount = IS_PARAM_DEFINED( info.m_nRefractAmount ) ?
	// params[info.m_nRefractAmount]->GetFloatValue() : kDefaultRefractAmount;
