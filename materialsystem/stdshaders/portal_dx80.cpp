...
BEGIN_VS_SHADER(Portal_DX80, "Help for Portal shader")

	BEGIN_SHADER_PARAMS
		SHADER_PARAM_OVERRIDE(COLOR, SHADER_PARAM_TYPE_COLOR, "{255 255 255}", "unused", SHADER_PARAM_NOT_EDITABLE)
		SHADER_PARAM_OVERRIDE(ALPHA, SHADER_PARAM_TYPE_FLOAT, "1.0", "unused", SHADER_PARAM_NOT_EDITABLE)
		SHADER_PARAM(STATICAMOUNT, SHADER_PARAM_TYPE_FLOAT, "0.0",
		...
		SHADER_PARAM(USEALTERNATEVIEWMATRIX, SHADER_PARAM_TYPE_INTEGER, "1",
					 "Use the alternate view matrix instead of the current view matrix")
		SHADER_PARAM(ALTERNATEVIEWMATRIX, SHADER_PARAM_TYPE_MATRIX, "0",
	...
	SHADER_DRAW
	{
		...
		bool bAlphaMaskTexture = ((iMaxTextureStages > 1) && (params[ALPHAMASKTEXTURE]->IsTexture()))
									 ? (1)
									 : (0); // must support at least 2 texture stages to use any kind of mask
		...
		SHADOW_STATE
		{
			...
			{
				// pShaderShadow->EnablePolyOffset( SHADER_POLYOFFSET_DECAL ); //a portal is effectively a decal on top
				// of a wall
			...
			if(((iMaxTextureStages < 3) && bStaticBlendTexture) ||
			   bAlphaMaskTexture) // in multipass modes, we need alpha to mix portal static, other
			{
		...
		else
		{
			...
			if(bStaticBlendTexture) // if we don't have a static texture, the cutout pass will mix in gray for any
									// static amount
				MultiPass_StaticPass(pShaderShadow, pShaderAPI, params, bAlphaMaskTexture);
		}
	}

	void SinglePass(IShaderShadow * pShaderShadow, IShaderDynamicAPI * pShaderAPI, IMaterialVar * *params,
					float fStaticAmount, bool bStaticBlendTexture, bool bAlphaMaskTexture)
	{
		SHADOW_STATE
		{
	...
	void MultiPass_CutoutPass(IShaderShadow * pShaderShadow, IShaderDynamicAPI * pShaderAPI, IMaterialVar * *params,
							  bool bAlphaMaskTexture)
	{
	...
	void MultiPass_StaticPass(IShaderShadow * pShaderShadow, IShaderDynamicAPI * pShaderAPI, IMaterialVar * *params,
							  bool bAlphaMaskTexture)
	{
		SHADOW_STATE
		{
