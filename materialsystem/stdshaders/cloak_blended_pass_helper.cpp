...
{
	bool bBumpMapping =
		(!g_pConfig->UseBumpmapping()) || (info.m_nBumpmap == -1) || !params[info.m_nBumpmap]->IsTexture() ? 0 : 1;
	...
	DYNAMIC_STATE
	{
		...
		// Refract color tint
		pShaderAPI->SetPixelShaderConstant(7,
										   IS_PARAM_DEFINED(info.m_nCloakColorTint)
											   ? params[info.m_nCloakColorTint]->GetVecValue()
											   : kDefaultCloakColorTint,
										   1);
