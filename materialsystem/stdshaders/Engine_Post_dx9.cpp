...
BEGIN_VS_SHADER_FLAGS(Engine_Post_dx9,
					  "Engine post-processing effects (software anti-aliasing, bloom, color-correction",
					  SHADER_NOT_EDITABLE)
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(AAINTERNAL1, SHADER_PARAM_TYPE_VEC4, "[0 0 0 0]",
					 "Internal anti-aliasing values set via material proxy")
		SHADER_PARAM(AAINTERNAL2, SHADER_PARAM_TYPE_VEC4, "[0 0 0 0]",
					 "Internal anti-aliasing values set via material proxy")
		SHADER_PARAM(AAINTERNAL3, SHADER_PARAM_TYPE_VEC4, "[0 0 0 0]",
					 "Internal anti-aliasing values set via material proxy")
	...
	SHADER_FALLBACK
	{
		// This shader should not be *used* unless we're >= DX9  (bloomadd.vmt/screenspace_general_dx8 should be used
		// for DX8)
		return 0;
	}
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			bool bLinearOutput =
				IsOSX() && !g_pHardwareConfig->FakeSRGBWrite() && g_pHardwareConfig->CanDoSRGBReadFromRTs();
			...
			if(g_pHardwareConfig->SupportsPixelShaders_2_b() ||
			   g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders()) // GL always goes the ps2b way for this shader,
																		 // even on "ps20" parts
			{
		...
		DYNAMIC_STATE
		{
			...
			{
				pShaderAPI->BindStandardTexture((Sampler_t)(SHADER_SAMPLER2 + i),
												(StandardTextureId_t)(TEXTURE_COLOR_CORRECTION_VOLUME_0 + i));
			}
			...
			//   x - strength (this can be used to toggle the AA off, or to weaken it where pathological cases are
			//   showing) y - reduction of 1-pixel-line blurring (blurring of 1-pixel lines causes issues, so it's
			//   tunable) z - edge threshold multiplier (default 1.0, < 1.0 => more edges softened, > 1.0 => fewer edges
			//   softened) w - tap offset multiplier (default 1.0, < 1.0 => sharper image, > 1.0 => blurrier image)
			float tweakables[4] = {params[AAINTERNAL1]->GetVecValue()[0], params[AAINTERNAL1]->GetVecValue()[1],
								   params[AAINTERNAL3]->GetVecValue()[0], params[AAINTERNAL3]->GetVecValue()[1]};
			...
			float uvTrans[4] = {params[AAINTERNAL2]->GetVecValue()[0], params[AAINTERNAL2]->GetVecValue()[1],
								params[AAINTERNAL2]->GetVecValue()[3], params[AAINTERNAL2]->GetVecValue()[2]};
