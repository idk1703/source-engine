					 ...
					 IShaderShadow *pShaderShadow, Refract_DX9_Vars_t &info, VertexCompressionType_t vertexCompression)
{
	...
	SHADOW_STATE
	{
		...
		bool bShaderSRGBConvert =
			IsOSX() && (g_pHardwareConfig->FakeSRGBWrite() || !g_pHardwareConfig->CanDoSRGBReadFromRTs());
		if(g_pHardwareConfig->SupportsPixelShaders_2_b() ||
		   g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders()) // always send OpenGL down the ps2b path
		{
	...
	DYNAMIC_STATE
	{
		...
		if(g_pHardwareConfig->SupportsPixelShaders_2_b() ||
		   g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders()) // always send Posix down the ps2b path
		{
		...
		pShader->SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_3,
												 info.m_nBumpTransform2); // 3 & 4
		...
		float c5[4] = {params[info.m_nRefractAmount]->GetFloatValue(), params[info.m_nRefractAmount]->GetFloatValue(),
					   0.0f, 0.0f};
