...
BEGIN_VS_SHADER(WorldVertexTransition_DX8, "Help for WorldVertexTransition_DX8")
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(BASETEXTURETRANSFORM2, SHADER_PARAM_TYPE_MATRIX, "center .5 .5 scale 1 1 rotate 0 translate 0 0",
		...
		SHADER_PARAM(BLENDMASKTRANSFORM, SHADER_PARAM_TYPE_MATRIX, "center .5 .5 scale 1 1 rotate 0 translate 0 0",
	...
	SHADER_INIT_PARAMS()
	{
		...
		{
			// seamless scale is going to be used, so kill some other features. . might implement with these features
			// later.
	...
	void WriteVertexAlphaToDestAlpha(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI,
									 IShaderShadow * pShaderShadow)
	{
		if(pShaderShadow)
		{
			...
			pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_COLOR, 2, 0, 0);
		}
		else
		{
	...
	void DrawFlashlightPass(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow,
							int passID)
	{
