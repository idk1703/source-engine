...
BEGIN_VS_SHADER(LightmappedGeneric_DX8, "Help for LightmappedGeneric_DX8")
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(FORCEBUMP, SHADER_PARAM_TYPE_BOOL, "0",
					 "0 == Do bumpmapping if the card says it can handle it. 1 == Always do bumpmapping.")
	...
	SHADER_INIT_PARAMS()
	{
		...
		if(ShouldUseBumpmapping(params) && params[ALBEDO]->IsDefined() && params[BASETEXTURE]->IsDefined() &&
		...
		if(params[ENVMAPOPTIONAL]->IsDefined() &&
		   (params[ENVMAPOPTIONAL]->GetIntValue() > g_pHardwareConfig->GetDXSupportLevel()))
		{
			params[ENVMAP]->SetUndefined();
		}
		...
		if(params[SEAMLESS_SCALE]->IsDefined() && params[SEAMLESS_SCALE]->GetFloatValue() != 0.0f)
		{
			...
			{
				Warning("Can't use $bumpmap with $seamless_scale for lightmappedgeneric_dx8.  Implicitly disabling "
						"$bumpmap: %s\n",
						pMaterialName);
			...
			{
				Warning("Can't use $envmap with $seamless_scale for lightmappedgeneric_dx8. Implicitly disabling "
						"$envmap: %s\n",
						pMaterialName);
				params[ENVMAP]->SetUndefined();
			}
		...
		if(params[ENVMAP]->IsDefined() && params[BUMPMAP]->IsDefined() &&
		   IS_FLAG_SET(MATERIAL_VAR_NORMALMAPALPHAENVMAPMASK) && !ShouldUseBumpmapping(params))
		{
	...
	void DrawUnbumpedUsingVertexShader(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI,
									   IShaderShadow * pShaderShadow, bool bBumpedEnvMap)
	{
		...
		if(bBumpedEnvMap)
		{
			DrawWorldBumpedSpecularLighting(BUMPMAP, ENVMAP, BUMPFRAME, ENVMAPFRAME, ENVMAPTINT, ALPHA, ENVMAPCONTRAST,
											ENVMAPSATURATION, BUMPTRANSFORM, FRESNELREFLECTION,
											hasEnvmap || hasBaseTexture || hasVertexColor);
		}
	}

	void DrawDetailNoEnvmap(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow,
							bool doSelfIllum)
	{
		SHADOW_STATE
		{
		...
		DYNAMIC_STATE
		{
			...
			SetVertexShaderTextureScaledTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_4, BASETEXTURETRANSFORM,
												  DETAILSCALE);
	...
	inline const char *GetAdditiveEnvmapPixelShaderName(bool usingMask, bool usingBaseTexture,
	...
	void DrawAdditiveEnvmap(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow)
	{
		...
		SHADOW_STATE
		{
			...
			const char *pshName =
				GetAdditiveEnvmapPixelShaderName(usingMask, usingBaseTexture, usingBaseAlphaEnvmapMask);
			pShaderShadow->SetPixelShader(pshName);
			FogToBlack();
		}
		DYNAMIC_STATE
		{
			...
			if(usingMask || IS_FLAG_SET(MATERIAL_VAR_BASEALPHAENVMAPMASK))
			{
				...
				SetVertexShaderTextureScaledTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_2, BASETEXTURETRANSFORM,
													  ENVMAPMASKSCALE);
			}
						 ...
						 bool bBumpedEnvMap)
	{
		...
		{
			DrawWorldBumpedSpecularLighting(BUMPMAP, ENVMAP, BUMPFRAME, ENVMAPFRAME, ENVMAPTINT, ALPHA, ENVMAPCONTRAST,
											ENVMAPSATURATION, BUMPTRANSFORM, FRESNELREFLECTION, true);
		}
	}

	void DrawDetailUsingVertexShader(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI,
									 IShaderShadow * pShaderShadow, bool bBumpedEnvMap)
	{
	...
	void DrawUnbumpedSeamlessUsingVertexShader(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI,
	...
	SHADER_DRAW
	{
		...
		{
			DrawWorldBumpedUsingVertexShader(BASETEXTURE, BASETEXTURETRANSFORM, BUMPMAP, BUMPFRAME, BUMPTRANSFORM,
											 ENVMAPMASK, ENVMAPMASKFRAME, ENVMAP, ENVMAPFRAME, ENVMAPTINT, COLOR, ALPHA,
											 ENVMAPCONTRAST, ENVMAPSATURATION, FRAME, FRESNELREFLECTION, false, -1, -1,
											 -1, bSSBump);
		}
		else
		{
			bool bBumpedEnvMap =
				ShouldUseBumpmapping(params) && params[BUMPMAP]->IsTexture() && params[ENVMAP]->IsTexture();
