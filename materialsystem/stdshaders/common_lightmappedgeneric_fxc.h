...
void GetBaseTextureAndNormal(sampler base, sampler base2, sampler bump, bool bBase2, bool bBump, float3 coords,
...
float3 LightMapSample(sampler LightmapSampler, float2 vTexCoord)
{
...
#else
	{
...
#else
		{
			...
			Weights = float4((1 - Weights.x) * (1 - Weights.y), Weights.x * (1 - Weights.y),
