...
BEGIN_VS_SHADER_FLAGS(BufferClearObeyStencil_DX9, "", SHADER_NOT_EDITABLE)
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(CLEARALPHA, SHADER_PARAM_TYPE_INTEGER, "-1",
					 "activates clearing of alpha. -1 == copy CLEARCOLOR setting")
	...
	SHADER_DRAW
	{
		...
		bool bEnableAlphaWrites =
			(params[CLEARALPHA]->GetIntValue() >= 0) ? (params[CLEARALPHA]->GetIntValue() != 0) : bEnableColorWrites;
		...
		SHADOW_STATE
		{
			...
			SET_STATIC_VERTEX_SHADER_COMBO(USESCOLOR,
										   bUsesColor || g_pHardwareConfig->PlatformRequiresNonNullPixelShaders());
