...
BEGIN_VS_SHADER(UnlitTwoTexture_DX9, "Help for UnlitTwoTexture_DX9")
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(TEXTURE2TRANSFORM, SHADER_PARAM_TYPE_MATRIX, "center .5 .5 scale 1 1 rotate 0 translate 0 0",
	...
	SHADER_DRAW
	{
		...
		if(bDrawStandardPass && (pShaderShadow == NULL) && (pShaderAPI != NULL) && !bNewFlashlightPath &&
		...
		if(bDrawStandardPass)
		{
			...
			SHADOW_STATE
			{
				...
				isTranslucent =
					isTranslucent || TextureIsTranslucent(BASETEXTURE, true) || TextureIsTranslucent(TEXTURE2, true);
