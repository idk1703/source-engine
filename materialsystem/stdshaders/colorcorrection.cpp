...
BEGIN_VS_SHADER_FLAGS(ColorCorrection, "Help for ColorCorrection", SHADER_NOT_EDITABLE)
	...
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			...
			{
				pShaderAPI->BindStandardTexture((Sampler_t)(SHADER_SAMPLER1 + i),
												(StandardTextureId_t)(TEXTURE_COLOR_CORRECTION_VOLUME_0 + i));
			}

			float default_weight = params[WEIGHT_DEFAULT]->GetFloatValue();
			float weights[4] = {params[WEIGHT0]->GetFloatValue(), params[WEIGHT1]->GetFloatValue(),
								params[WEIGHT2]->GetFloatValue(), params[WEIGHT3]->GetFloatValue()};
