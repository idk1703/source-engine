...
BEGIN_VS_SHADER(Sky_DX9, "Help for Sky_DX9 shader")
	...
	SHADER_INIT
	{
		if(params[BASETEXTURE]->IsDefined())
		{
			...
			LoadTexture(BASETEXTURE, (fmt == IMAGE_FORMAT_RGBA16161616F) || (fmt == IMAGE_FORMAT_RGBA16161616)
										 ? 0
										 : TEXTUREFLAGS_SRGB);
		}
	}
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			...
			if((fmt == IMAGE_FORMAT_RGBA16161616) ||
			   ((fmt == IMAGE_FORMAT_RGBA16161616F) && (g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER)))
			{
				c0[0] *= 16.0;
				c0[1] *= 16.0;
				c0[2] *= 16.0;
