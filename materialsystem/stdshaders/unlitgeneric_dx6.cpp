...
BEGIN_SHADER(UnlitGeneric_DX6, "Help for UnlitGeneric_DX6")
	...
	void DrawAdditiveNonTextured(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow,
								 bool doDetail)
	{
		SHADOW_STATE
		{
	...
	void DrawAdditiveTextured(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow,
							  bool doDetail)
	{
		SHADOW_STATE
		{
	...
	void DrawNonTextured(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow,
						 bool doDetail)
	{
		SHADOW_STATE
		{
	...
	void DrawTextured(IMaterialVar * *params, IShaderDynamicAPI * pShaderAPI, IShaderShadow * pShaderShadow,
					  bool doDetail)
	{
		SHADOW_STATE
		{
	...
	SHADER_DRAW
	{
		...
		if(doFirstPass)
		{
			...
			if(IS_FLAG_SET(MATERIAL_VAR_ADDITIVE))
			{
				...
				else
				{
					...
					{
						FixedFunctionMultiplyByDetailPass(BASETEXTURE, FRAME, BASETEXTURETRANSFORM, DETAIL,
														  DETAILSCALE);
					}
				}
			}
		...
		// Second pass...
		if(params[ENVMAP]->IsTexture() && (!doFirstPass || IS_FLAG_SET(MATERIAL_VAR_MULTIPASS)))
		{
