...
static void DrawCompositorStage_ps20(CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
									 IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
									 const CompositorInfo_t &info);
static void DrawCompositorStage_ps20b(CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
									  IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
									  const CompositorInfo_t &info);

BEGIN_VS_SHADER(Compositor, "Help for Compositor")
	BEGIN_SHADER_PARAMS
		SHADER_PARAM(COMBINE_MODE, SHADER_PARAM_TYPE_INTEGER, "",
					 "Which mode should the combiner operate in? 0: Mul, 1: Add, 2: Lerp, 3: Select")
...
static void DrawCompositorStage_common(CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
									   IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
									   const CompositorInfo_t &info)
{
	...
	DYNAMIC_STATE
	{
		...
		if(textureCountThisPass > 0)
			pShader->BindTexture(SHADER_SAMPLER0, info.m_nSrcTexture[0]);
		if(textureCountThisPass > 0)
			pShader->SetVertexShaderMatrix2x4(2, info.m_nTexTransform[0]);
		...
		if(nCombineMode == ECO_Select)
		{
			...
			float selectors[] = {
				cFac * GetIntParam(info.m_nSelector[0], params),  cFac * GetIntParam(info.m_nSelector[1], params),
				cFac * GetIntParam(info.m_nSelector[2], params),  cFac * GetIntParam(info.m_nSelector[3], params),
				cFac * GetIntParam(info.m_nSelector[4], params),  cFac * GetIntParam(info.m_nSelector[5], params),
				cFac * GetIntParam(info.m_nSelector[6], params),  cFac * GetIntParam(info.m_nSelector[7], params),
				cFac * GetIntParam(info.m_nSelector[8], params),  cFac * GetIntParam(info.m_nSelector[9], params),
				cFac * GetIntParam(info.m_nSelector[10], params), cFac * GetIntParam(info.m_nSelector[11], params),
				cFac * GetIntParam(info.m_nSelector[12], params), cFac * GetIntParam(info.m_nSelector[13], params),
				cFac * GetIntParam(info.m_nSelector[14], params), cFac * GetIntParam(info.m_nSelector[15], params)};
...
static void DrawCompositorStage_ps20_muladdblend(CBaseVSShader *pShader, IMaterialVar **params,
												 ...
												 const CompositorInfo_t &info)
{
	...
	DYNAMIC_STATE
	{
		...
		if(textureCountThisPass > 1)
			pShader->BindTexture(SHADER_SAMPLER1, info.m_nSrcTexture[1]);
		if(textureCountThisPass > 1)
			pShader->SetVertexShaderMatrix2x4(4, info.m_nTexTransform[1]);
		if(textureCountThisPass > 1)
			pShader->SetPixelShaderConstantGammaToLinear(3, info.m_nTexAdjustLevels[1]);
		...
		if(textureCountThisPass > 0)
			pShader->BindTexture(SHADER_SAMPLER0, info.m_nSrcTexture[2]);
		if(textureCountThisPass > 0)
			pShader->SetVertexShaderMatrix2x4(2, info.m_nTexTransform[2]);
		if(textureCountThisPass > 0)
			pShader->SetPixelShaderConstantGammaToLinear(2, info.m_nTexAdjustLevels[2]);

		if(textureCountThisPass > 1)
			pShader->BindTexture(SHADER_SAMPLER1, info.m_nSrcTexture[3]);
		if(textureCountThisPass > 1)
			pShader->SetVertexShaderMatrix2x4(4, info.m_nTexTransform[3]);
		if(textureCountThisPass > 1)
			pShader->SetPixelShaderConstantGammaToLinear(3, info.m_nTexAdjustLevels[3]);
...
static void DrawCompositorStage_ps20_lerp(CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
										  IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
										  const CompositorInfo_t &info)
{
	SHADOW_STATE
...
// Helper function when using ps20 and performing lerp operations.
static void DrawCompositorStage_ps20_select(CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow,
											IShaderDynamicAPI *pShaderAPI, VertexCompressionType_t vertexCompression,
											const CompositorInfo_t &info)
{
	int nCombineMode = params[info.m_nCombineMode]->GetIntValue();
									  ...
									  const CompositorInfo_t &info)
{
	...
	DYNAMIC_STATE
	{
		...
		if(textureCountThisPass > 0)
			pShader->SetPixelShaderConstant(2, info.m_nTexAdjustLevels[0]);
		...
		if(textureCountThisPass > 1)
			pShader->SetVertexShaderMatrix2x4(4, info.m_nTexTransform[1]);
		if(textureCountThisPass > 1)
			pShader->SetPixelShaderConstant(3, info.m_nTexAdjustLevels[1]);

		if(textureCountThisPass > 2)
			pShader->BindTexture(SHADER_SAMPLER2, info.m_nSrcTexture[2]);
		if(textureCountThisPass > 2)
			pShader->SetVertexShaderMatrix2x4(6, info.m_nTexTransform[2]);
		if(textureCountThisPass > 2)
			pShader->SetPixelShaderConstant(4, info.m_nTexAdjustLevels[2]);

		if(textureCountThisPass > 3)
			pShader->BindTexture(SHADER_SAMPLER3, info.m_nSrcTexture[3]);
		if(textureCountThisPass > 3)
			pShader->SetVertexShaderMatrix2x4(8, info.m_nTexTransform[3]);
		if(textureCountThisPass > 3)
			pShader->SetPixelShaderConstant(5, info.m_nTexAdjustLevels[3]);
