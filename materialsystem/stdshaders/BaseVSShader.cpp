...
void CBaseVSShader::LoadViewportTransformScaledIntoVertexShaderConstant(int vertexReg)
{
	...
	Vector4D viewportTransform(1.0f * viewport.m_nWidth / bbWidth, 1.0f * viewport.m_nHeight / bbHeight,
							   1.0f * viewport.m_nTopLeftX / bbWidth, 1.0f * viewport.m_nTopLeftY / bbHeight);
...
#ifndef GAME_SHADER_DLL
const char *CBaseVSShader::UnlitGeneric_ComputePixelShaderName(bool bMask, bool bEnvmap, bool bBaseTexture,
															   bool bBaseAlphaEnvmapMask, bool bDetail,
															   bool bDetailMultiplyMode, bool bMaskBaseByDetailAlpha)
{
...
//-----------------------------------------------------------------------------
void CBaseVSShader::VertexShaderUnlitGenericPass(int baseTextureVar, int frameVar, int baseTextureTransformVar,
												 ...
												 int nSeparateDetailUVsVar)
{
	...
	if(IsSnapshotting())
	{
		...
		const char *pshName = UnlitGeneric_ComputePixelShaderName(bMask, bEnvmap, bBaseTexture, bBaseAlphaEnvmapMask,
																  bDetail, bDetailMultiply, bMaskBaseByDetailAlpha);
	...
	else
	{
		...
		if(bEnvmap)
		{
			...
			if(bMask || bBaseAlphaEnvmapMask)
			{
				...
				SetVertexShaderTextureScaledTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_2, baseTextureTransformVar,
													  envmapMaskScaleVar);
			}
...
void CBaseVSShader::DrawWorldBaseTexture(int baseTextureVar, int baseTextureTransformVar, int frameVar, int colorVar,
										 int alphaVar)
{
	if(IsSnapshotting())
	{
...
void CBaseVSShader::DrawWorldBumpedDiffuseLighting(int bumpmapVar, int bumpFrameVar, int bumpTransformVar,
												   bool bMultiply, bool bSSBump)
{
	if(IsSnapshotting())
	{
		s_pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
...
void CBaseVSShader::DrawWorldBumpedDiffuseLighting_Base_ps14(int bumpmapVar, int bumpFrameVar, int bumpTransformVar,
...
void CBaseVSShader::DrawWorldBumpedDiffuseLighting_Blend_ps14(int bumpmapVar, int bumpFrameVar, int bumpTransformVar,
															  int baseTextureVar, int baseTextureTransformVar,
...
void CBaseVSShader::DrawWorldBumpedSpecularLighting(int bumpmapVar, int envmapVar, int bumpFrameVar, int envmapFrameVar,
													int envmapTintVar, int alphaVar, int envmapContrastVar,
													int envmapSaturationVar, int bumpTransformVar,
													int fresnelReflectionVar, bool bBlend, bool bNoWriteZ)
{
	// + BUMPED CUBEMAP
	if(IsSnapshotting())
	{
		...
		s_pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_NORMAL | VERTEX_TANGENT_S | VERTEX_TANGENT_T,
												  1, 0, 0);
...
void CBaseVSShader::DrawModelBumpedSpecularLighting(int bumpMapVar, int bumpMapFrameVar, int envMapVar,
													int envMapVarFrame, int envMapTintVar, int alphaVar,
													int envMapContrastVar, int envMapSaturationVar,
													int bumpTransformVar, bool bBlendSpecular, bool bNoWriteZ)
{
	IMaterialVar **params = s_ppParams;
	...
	if(IsSnapshotting())
	{
		...
		s_pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_NORMAL, 1, 0, 4 /* userDataSize */);
...
void CBaseVSShader::DrawBaseTextureBlend(int baseTextureVar, int baseTextureTransformVar, int baseTextureFrameVar,
										 ...
										 int colorVar, int alphaVar)
{
	if(IsSnapshotting())
	{
		...
		s_pShaderShadow->EnableTexture(SHADER_SAMPLER1, true);
		s_pShaderShadow->EnableTexture(SHADER_SAMPLER2, true);
		s_pShaderShadow->DrawFlags(SHADER_DRAW_POSITION | SHADER_DRAW_TEXCOORD0 | SHADER_DRAW_LIGHTMAP_TEXCOORD1);
		// FIXME: Remove the normal (needed for tangent space gen)
...
void CBaseVSShader::DrawWorldBumpedUsingVertexShader(
	int baseTextureVar, int baseTextureTransformVar, int bumpmapVar, int bumpFrameVar, int bumpTransformVar,
	int envmapMaskVar, int envmapMaskFrame, int envmapVar, int envmapFrameVar, int envmapTintVar, int colorVar,
	int alphaVar, int envmapContrastVar, int envmapSaturationVar, int frameVar, int fresnelReflectionVar,
	bool doBaseTexture2, int baseTexture2Var, int baseTextureTransform2Var, int baseTextureFrame2Var, bool bSSBump)
{
	IMaterialVar **params = s_ppParams;
	// Draw base texture
	...
	{
		DrawBaseTextureBlend(baseTextureVar, baseTextureTransformVar, frameVar, baseTexture2Var,
							 baseTextureTransform2Var, baseTextureFrame2Var, colorVar, alphaVar);
	...
	{
		DrawWorldBumpedSpecularLighting(bumpmapVar, envmapVar, bumpFrameVar, envmapFrameVar, envmapTintVar, alphaVar,
										envmapContrastVar, envmapSaturationVar, bumpTransformVar, fresnelReflectionVar,
										bBlendSpecular);
	}
}
#endif // GAME_SHADER_DLL
...
void CBaseVSShader::SetFlashlightVertexShaderConstants(bool bBump, int bumpTransformVar, bool bDetail,
													   int detailScaleVar, bool bSetTextureTransforms)
{
	Assert(!IsSnapshotting());
	...
	if(bDetail)
	{
		SetVertexShaderTextureScaledTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_8, BASETEXTURETRANSFORM,
											  detailScaleVar);
	}
	...
	if(bSetTextureTransforms)
	{
		...
		{
			SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_8,
...
#if SUPPORT_DX8
void CBaseVSShader::DrawFlashlight_dx80(IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
										...
										bool bTeeth, int nTeethForwardVar, int nTeethIllumFactorVar)
{
	// FLASHLIGHTFIXME: hack . . need to fix the vertex shader so that it can deal with and without bumps for
	// vertexlitgeneric
										...
										IShaderShadow *pShaderShadow, DrawFlashlight_dx90_Vars_t &vars)
{
	...
	bool bBump2 = vars.m_bWorldVertexTransition && vars.m_bBump && vars.m_nBumpmap2Var != -1 &&
				  params[vars.m_nBumpmap2Var]->IsTexture();
	bool bSeamless = vars.m_fSeamlessScale != 0.0;
	bool bDetail = vars.m_bLightmappedGeneric && (vars.m_nDetailVar != -1) && params[vars.m_nDetailVar]->IsDefined() &&
...
else
{
	...
	if(g_pHardwareConfig->SupportsPixelShaders_2_b())
	{
		...
		SET_DYNAMIC_PIXEL_SHADER_COMBO(FLASHLIGHTSHADOWS,
									   flashlightState.m_bEnableShadows && (pFlashlightDepthTexture != NULL));
	...
	SetFlashlightVertexShaderConstants(vars.m_bBump, vars.m_nBumpTransform, bDetail, vars.m_nDetailScale,
...
void CBaseVSShader::InitParamsUnlitGeneric_DX8(int baseTextureVar, int detailScaleVar, int envmapOptionalVar,
											   int envmapVar, int envmapTintVar, int envmapMaskScaleVar,
											   int nDetailBlendMode)
{
	...
	if(envmapVar >= 0 && baseTextureVar >= 0 && !g_pConfig->UseSpecular() && params[envmapVar]->IsDefined() &&
...
void CBaseVSShader::InitUnlitGeneric_DX8(int baseTextureVar, int detailVar, int envmapVar, int envmapMaskVar)
{
	IMaterialVar **params = s_ppParams;
