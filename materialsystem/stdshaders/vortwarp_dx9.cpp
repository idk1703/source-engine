...
void DrawVortWarp_DX9(CBaseVSShader *pShader, IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
					  IShaderShadow *pShaderShadow, bool bVertexLitGeneric, bool hasFlashlight,
					  VortWarp_DX9_Vars_t &info, VertexCompressionType_t vertexCompression)
{
	...
	if(pShader->IsSnapshotting())
	{
		...
		bool hasEnvmapMask = (hasSelfIllumInEnvMapMask || !hasFlashlight) && params[info.m_nEnvmapMask]->IsTexture();
	...
	else
	{
...
#endif
		{
			...
			if(g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				...
				SET_DYNAMIC_PIXEL_SHADER_COMBO(WRITEWATERFOGTODESTALPHA, fogType == MATERIAL_FOG_LINEAR_BELOW_FOG_Z &&
																			 ...
																			 blendType != BT_BLEND && !bIsAlphaTested);
		...
		if(hasEnvmapMask)
		{
			pShader->SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_4,
