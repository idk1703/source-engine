...
public:
	...
	CreateInterfaceFn Init(const char *pShaderDLL, IMaterialProxyFactory *pMaterialProxyFactory,
...
public:
	...
	bool ConvertImageFormat(unsigned char *src, enum ImageFormat srcImageFormat, unsigned char *dst,
	...
	IMaterial *FindMaterial(const char *materialName, const char *pTextureGroupName, bool complain = true,
	...
	ITexture *CreateNamedRenderTargetTexture(
		...
		RenderTargetSizeMode_t sizeMode, // Controls how size is generated (and regenerated on video mode change).
		ImageFormat format, MaterialRenderTargetDepth_t depth = MATERIAL_RT_DEPTH_SHARED, bool bClampTexCoords = true,
	...
	virtual ITextureCompositor *NewTextureCompositor(int w, int h, const char *pCompositeName, int nTeamNum,
													 ...
													 uint32 texCompositeCreateFlags) OVERRIDE;
	...
	virtual void AsyncFindTexture(const char *pFilename, const char *pTextureGroupName,
								  IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs, bool bComplain = true,
								  int nAdditionalCreationFlags = 0) OVERRIDE;
	...
	virtual bool AddTextureCompositorTemplate(const char *pName, KeyValues *pTmplDesc,
											  int nTexCompositeTemplateFlags = 0) OVERRIDE;
	...
	ShaderAPITextureHandle_t GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrame,
														   int nTextureChannel); // JasonM ????
	...
	DELEGATE_TO_OBJECT_2V(BindStandardVertexTexture, VertexTextureSampler_t, StandardTextureId_t,
	...
	DELEGATE_TO_OBJECT_7V(DrawClearBufferQuad, unsigned char, unsigned char, unsigned char, unsigned char, bool, bool,
						  bool, &m_HardwareRenderContext);
	...
	void ReadBackBuffer(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *pData, ImageFormat dstFormat,
						int nDstStride);
	void PersistDisplay();
	void *GetD3DDevice();
...
private:
	...
	CON_COMMAND_MEMBER_F(CMaterialSystem, "mat_showmaterialsverbose", DebugPrintUsedMaterialsVerbose,
	...
	CON_COMMAND_MEMBER_F(CMaterialSystem, "mat_reloadallmaterials", ReloadAllMaterials, "Reloads all materials",
						 FCVAR_CHEAT);
	CON_COMMAND_MEMBER_F(CMaterialSystem, "mat_reloadmaterial", ReloadMaterials, "Reloads a single material",
						 FCVAR_CHEAT);
	...
	ShaderAPITextureHandle_t
		m_LinearToGammaTableTextureHandle; // linear to gamma srgb conversion lookup for the current hardware
	ShaderAPITextureHandle_t
		m_LinearToGammaIdentityTableTextureHandle;	  // An identity lookup for when srgb writes are off
