...
//-----------------------------------------------------------------------------
bool CVertexMorphDict::VertexMorphDictLess::Less(const CVertexMorphDict::MorphVertexList_t &src1,
...
public:
	...
	bool RenderMorphWeights(IMatRenderContext *pRenderContext, int nRenderId, int nWeightCount,
...
private:
	...
	void RenderMorphQuads(IMatRenderContext *pRenderContext, int nRenderId, int nTotalQuadCount, int nWeightCount,
...
public:
	...
	virtual void AccumulateMorph(IMorphMgrRenderContext *pIRenderContext, IMorph *pMorph, int nMorphCount,
								 const MorphWeight_t *pWeights);
...
public:
	...
	bool IsUsingConstantRegisters() const
	{
		return m_bUsingConstantRegisters;
	}
...
void CMorph::Bind(IMorphMgrRenderContext *pIRenderContext)
{
	...
	g_pShaderAPI->SetIntRenderingParameter(INT_RENDERPARM_MORPH_ACCUMULATOR_4TUPLE_COUNT,
...
int CMorph::ComputeMorphTextureSizeInBytes() const
{
	...
	if(m_pMorphTexture[MORPH_TEXTURE_POS_NORMAL_DELTA])
	{
		...
		nSize +=
			m_nTextureWidth * m_nTextureHeight * nTotal4Tuples * ImageLoader::SizeInBytes(IMAGE_FORMAT_RGBA16161616);
	}
...
void CMorph::PackMorphData()
{
	...
	if(m_Format & (MORPH_POSITION | MORPH_WRINKLE | MORPH_NORMAL))
	{
		...
		ITexture *pTexture = g_pMaterialSystem->CreateProceduralTexture(
			pTemp, TEXTURE_GROUP_MORPH_TARGETS, m_nTextureWidth * nTotal4Tuples, m_nTextureHeight,
			IMAGE_FORMAT_RGBA16161616,
			TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NODEBUGOVERRIDE | TEXTUREFLAGS_SINGLECOPY |
	...
	if(m_Format & (MORPH_SIDE | MORPH_SPEED))
	{
		...
		ITexture *pTexture = g_pMaterialSystem->CreateProceduralTexture(
			pTemp, TEXTURE_GROUP_MORPH_TARGETS, m_nTextureWidth, m_nTextureHeight, IMAGE_FORMAT_RGBA8888,
			TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NODEBUGOVERRIDE | TEXTUREFLAGS_SINGLECOPY |
				TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_POINTSAMPLE);
...
void CMorph::CreateStaticMesh()
{
	...
	m_pMorphBuffer =
		pRenderContext->CreateStaticMesh(vertexFormat, TEXTURE_GROUP_MORPH_TARGETS, m_MorphAccumulationMaterial);
	...
	for(int i = 0; i < nMorphCount; ++i)
	{
		...
		for(int j = 0; j < nQuadCount; ++j)
		{
			...
			meshBuilder.TexCoord4f(0, sx * flOOTexWidth, sy * flOOTexHeight, (dx - 0.5f) * flOODestWidth,
			...
			meshBuilder.TexCoord4f(0, sx * flOOTexWidth, (sy + quad.m_nCount) * flOOTexHeight,
								   ...
								   (dy + quad.m_nCount - 0.5f) * flOODestHeight); // Stores the source to read from
			meshBuilder.TexCoord1f(1, i);
			meshBuilder.AdvanceVertex();
...
// Renders to the morph accumulator texture
//-----------------------------------------------------------------------------
void CMorph::RenderMorphQuads(IMatRenderContext *pRenderContext, int nRenderId, int nTotalQuadCount, int nWeightCount,
							  int *pWeightLookup, const MorphWeight_t *pWeights)
{
...
//-----------------------------------------------------------------------------
bool CMorph::RenderMorphWeights(IMatRenderContext *pRenderContext, int nRenderId, int nWeightCount,
								const MorphWeight_t *pWeights)
{
...
void CMorphMgr::AllocateScratchTextures()
{
	...
	m_pMorphAccumTexture = TextureManager()->CreateRenderTargetTexture(
		"_rt_MorphAccumulator", m_nAccumulatorWidth * nMultFactor, m_nAccumulatorHeight * nMultFactor,
		RT_SIZE_OFFSCREEN, fmt, RENDER_TARGET_NO_DEPTH,
		TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NODEBUGOVERRIDE | TEXTUREFLAGS_CLAMPS |
			TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_POINTSAMPLE | TEXTUREFLAGS_VERTEXTEXTURE,
		0);
	...
	m_bUsingConstantRegisters = false; //( g_pMaterialSystemHardwareConfig->NumVertexShaderConstants() >=
	...
	if(!m_bUsingConstantRegisters)
	{
		m_pMorphWeightTexture = TextureManager()->CreateRenderTargetTexture(
			"_rt_MorphWeight", m_nWeightWidth * nMultFactor, m_nWeightHeight * nMultFactor, RT_SIZE_OFFSCREEN, fmt,
...
// Computes texel offsets for the upper corner of the morph accumulator for a particular block
//-----------------------------------------------------------------------------
void CMorphMgr::ComputeAccumulatorSubrect(int *pXOffset, int *pYOffset, int *pWidth, int *pHeight,
...
// Used to compute stats of memory used
//-----------------------------------------------------------------------------
CON_COMMAND_F(mat_reporthwmorphmemory, "Reports the amount of size in bytes taken up by hardware morph textures.",
			  FCVAR_CHEAT)
{
...
void CMorphMgr::DrawMorphTempTexture(IMatRenderContext *pRenderContext, IMaterial *pMaterial, ITexture *pTexture)
{
	...
	int nWidth = pTexture->GetActualWidth();
	int nHeight = pTexture->GetActualHeight();
	::DrawScreenSpaceRectangle(pMaterial, s_nX, s_nY, nWidth, nHeight, 0, 0, nWidth - 1, nHeight - 1, nWidth, nHeight);
...
//-----------------------------------------------------------------------------
bool CMorphMgr::GetMorphAccumulatorTexCoord(IMorphMgrRenderContext *pRenderContext, Vector2D *pTexCoord, IMorph *pMorph,
											int nVertex)
{
...
void CMorphMgr::BeginMorphAccumulation(IMorphMgrRenderContext *pIRenderContext)
{
	...
	IMatRenderContextInternal *pRenderContextInternal =
		static_cast<IMatRenderContextInternal *>((IMatRenderContext *)pRenderContext);
...
//-----------------------------------------------------------------------------
void CMorphMgr::AccumulateMorph(IMorphMgrRenderContext *pIRenderContext, IMorph *pMorph, int nMorphCount,
								const MorphWeight_t *pWeights)
{
	CMorphMgrRenderContext *pMorphRenderContext = static_cast<CMorphMgrRenderContext *>(pIRenderContext);
	Assert(pMorphRenderContext->m_bInMorphAccumulation);
