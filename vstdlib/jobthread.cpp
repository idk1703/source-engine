...
public:
	...
	// Thread functions
	//-----------------------------------------------------
	bool Start(const ThreadPoolStartParams_t &startParams = ThreadPoolStartParams_t())
	{
...
public:
	CJobThread(CThreadPool *pOwner, int iThread)
		: m_SharedQueue(pOwner->m_SharedQueue), m_pOwner(pOwner), m_iThread(iThread)
	{
	}
...
{
	tmZone(TELEMETRY_LEVEL0, TMZF_IDLE, "%s(%d) SPINNING %t", __FUNCTION__, timeout,
		...
		for(i = 0; i < m_Threads.Count(); i++)
		{
			...
			while(queue.Count((JobPriority_t)iCurPriority))
			{
				...
				if(pfnFilter && !(*pfnFilter)(pJob))
				{
					...
					{
						m_nJobs--;
						pJob->Release(); // an already serviced job in queue, may as well ditch it (as in, main thread
										 // probably force executed)
					}
					continue;
				}
				ServiceJobAndRelease(pJob);
		...
		if(ci.m_nLogicalProcessors > 1)
		{
			if(!pAffinityTable)
			{
				...
				if(hInst)
				{
					...
					SetThreadIdealProcessorFn Thread_SetIdealProcessor =
						(SetThreadIdealProcessorFn)GetProcAddress(hInst, "SetThreadIdealProcessor");
		...
		for(int bInterleavePushPop = 0; bInterleavePushPop < 2; bInterleavePushPop++)
		{
			for(g_iSleep = -10; g_iSleep <= 10; g_iSleep += 10)
			{
				...
				for(int i = 1; i <= nMaxThreads; i += nIncrement)
				{
					...
					Msg("ThreadPoolTest:         %d threads -- %d (%d) jobs processed in %fms, %fms to suspend (%f/%f) "
						"[%d, %d, %d, %d, %d, %d, %d, %d]\n",
						i, g_nTotalAtFinish, (int)CCountJob::m_nCount, timer.GetDuration().GetMillisecondsF(),
						suspendTimer.GetDuration().GetMillisecondsF() - timer.GetDuration().GetMillisecondsF(),
						timer.GetDuration().GetMillisecondsF() / (float)CCountJob::m_nCount,
						(suspendTimer.GetDuration().GetMillisecondsF()) / (float)g_nTotalAtFinish, counts[0], counts[1],
