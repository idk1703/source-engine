...
const uint32 ALIGN16 g_SIMD_AllOnesMask[4] ALIGN16_POST = {0xffffffff, 0xffffffff, 0xffffffff,
														   0xffffffff};						  // ~0,~0,~0,~0
...
void FourVectors::RotateManyBy(FourVectors *RESTRICT pVectors, unsigned int numVectors,
							   const matrix3x4_t &rotationMatrix)
{
	...
	if(numVectors & 0x01)
	{
		...
		pVectNext = pVectors++;
		outX1 = AddSIMD(AddSIMD(MulSIMD(pVectNext->x, matSplat00), MulSIMD(pVectNext->y, matSplat01)),
						MulSIMD(pVectNext->z, matSplat02));
		outY1 = AddSIMD(AddSIMD(MulSIMD(pVectNext->x, matSplat10), MulSIMD(pVectNext->y, matSplat11)),
						MulSIMD(pVectNext->z, matSplat12));
		outZ1 = AddSIMD(AddSIMD(MulSIMD(pVectNext->x, matSplat20), MulSIMD(pVectNext->y, matSplat21)),
						MulSIMD(pVectNext->z, matSplat22));
	}
	else
	{
		// even number of total vectors to process;
		// prime the zero group and jump into the middle of the loop
		outX0 = AddSIMD(AddSIMD(MulSIMD(pVectors->x, matSplat00), MulSIMD(pVectors->y, matSplat01)),
						MulSIMD(pVectors->z, matSplat02));
		outY0 = AddSIMD(AddSIMD(MulSIMD(pVectors->x, matSplat10), MulSIMD(pVectors->y, matSplat11)),
						MulSIMD(pVectors->z, matSplat12));
		outZ0 = AddSIMD(AddSIMD(MulSIMD(pVectors->x, matSplat20), MulSIMD(pVectors->y, matSplat21)),
						MulSIMD(pVectors->z, matSplat22));
		goto EVEN_CASE;
	}
	...
	while(pVectors < stop)
	{
		outX0 = MaddSIMD(pVectors->z, matSplat02,
						 AddSIMD(MulSIMD(pVectors->x, matSplat00), MulSIMD(pVectors->y, matSplat01)));
		outY0 = MaddSIMD(pVectors->z, matSplat12,
						 AddSIMD(MulSIMD(pVectors->x, matSplat10), MulSIMD(pVectors->y, matSplat11)));
		outZ0 = MaddSIMD(pVectors->z, matSplat22,
						 AddSIMD(MulSIMD(pVectors->x, matSplat20), MulSIMD(pVectors->y, matSplat21)));
	...
	EVEN_CASE:
		...
		outX1 = MaddSIMD(pVectNext->z, matSplat02,
						 AddSIMD(MulSIMD(pVectNext->x, matSplat00), MulSIMD(pVectNext->y, matSplat01)));
		outY1 = MaddSIMD(pVectNext->z, matSplat12,
						 AddSIMD(MulSIMD(pVectNext->x, matSplat10), MulSIMD(pVectNext->y, matSplat11)));
		outZ1 = MaddSIMD(pVectNext->z, matSplat22,
						 AddSIMD(MulSIMD(pVectNext->x, matSplat20), MulSIMD(pVectNext->y, matSplat21)));
...
void FourVectors_TransformManyGroupsOfEightBy(FourVectors *RESTRICT pVectors, unsigned int numVectors,
											  const matrix3x4_t &rotationMatrix, FourVectors *RESTRICT pOut)
{
	Assert(numVectors > 0);
	if(numVectors == 0)
		return;

	AssertMsg((pOut < pVectors && pOut + numVectors <= pVectors) || (pOut > pVectors && pVectors + numVectors <= pOut),
...
#define COMPUTE(res, invec, xterm, yterm, zterm, transterm)                                                           \
	res =                                                                                                             \
		AddSIMD(AddSIMD(MulSIMD((invec)->z, zterm), AddSIMD(MulSIMD((invec)->x, xterm), MulSIMD((invec)->y, yterm))), \
				transterm)
...
// #define FROZ(out,in,offset) COMPUTE((out+offset)->x, (in + offset), matSplat00, matSplat01, matSplat02, matSplat03);
// COMPUTE((out + offset )->y, (in + offset), matSplat10, matSplat11, matSplat12, matSplat13); COMPUTE((out +
// offset)->z, (in + offset), matSplat20, matSplat21, matSplat22, matSplat23)
...
#define COMPUTE_STAGE2_ROW(res, tempvar, invec, xsplat, ysplat, zsplat, transplat) \
	res = MaddSIMD((invec)->x, xsplat, res)
#define COMPUTE_STAGE3_ROW(res, tempvar, invec, xsplat, ysplat, zsplat, transplat) \
	res = AddSIMD(res, tempvar) // frees up the tempvar
...
void FourVectors_TransformManyGroupsOfEightBy_128byteAligned(FourVectors *RESTRICT pVectors, unsigned int numVectors,
															 const matrix3x4_t &rotationMatrix,
															 FourVectors *RESTRICT pOut)
{
	...
	// Assert alignment
	AssertMsg(((reinterpret_cast<uint32>(pVectors) & 127) == 0) && ((reinterpret_cast<uint32>(pOut) & 127) == 0),
...
#define WRITE(term, reg, toptr) toptr->term = reg
	...
	// #define FROZ(out,in,offset) COMPUTE((out+offset)->x, (in + offset), matSplat00, matSplat01, matSplat02,
	// matSplat03); COMPUTE((out + offset )->y, (in + offset), matSplat10, matSplat11, matSplat12, matSplat13);
	// COMPUTE((out + offset)->z, (in + offset), matSplat20, matSplat21, matSplat22, matSplat23)
#define COMPUTE_GROUP(resgroup, dataptr)                                             \
	COMPUTE(resgroup##X, (dataptr), matSplat00, matSplat01, matSplat02, matSplat03); \
	COMPUTE(resgroup##Y, (dataptr), matSplat10, matSplat11, matSplat12, matSplat13); \
	COMPUTE(resgroup##Z, (dataptr), matSplat20, matSplat21, matSplat22, matSplat23)
...
void FourVectors::TransformManyBy(FourVectors *RESTRICT pVectors, unsigned int numVectors,
								  const matrix3x4_t &rotationMatrix, FourVectors *RESTRICT pOut)
{
	Assert(numVectors > 0);
	...
	if(numVectors >= 8)
	{
		...
		{
			FourVectors_TransformManyGroupsOfEightBy_128byteAligned(pVectors, numVectorsRoundedToNearestEight,
																	rotationMatrix, pOut);
		}
		else
		{
	...
	if(numVectors > 0)
	{
		...
		do
		{
			...
			pOut->x = MaddSIMD(pVectors->z, matSplat02,
							   MaddSIMD(pVectors->y, matSplat01, MaddSIMD(pVectors->x, matSplat00, matSplat03)));
			pOut->y = MaddSIMD(pVectors->z, matSplat12,
							   MaddSIMD(pVectors->y, matSplat11, MaddSIMD(pVectors->x, matSplat00, matSplat13)));
			pOut->z = MaddSIMD(pVectors->z, matSplat22,
							   MaddSIMD(pVectors->y, matSplat21, MaddSIMD(pVectors->x, matSplat00, matSplat23)));
...
#ifdef _X360
// Loop-scheduled code to process FourVectors in groups of eight quite efficiently.
static void FourVectors_TransformManyGroupsOfEightBy_InPlace(FourVectors *RESTRICT pVectors, unsigned int numVectors,
															 const matrix3x4_t &rotationMatrix)
{
	Assert(numVectors > 0);
	if(numVectors == 0)
								  ...
								  const matrix3x4_t &rotationMatrix)
{
	...
	if(numVectors > 0)
	{
		...
		do
		{
			...
			// Trust in the compiler to schedule these operations correctly:
			resultX = MaddSIMD(pVectors->z, matSplat02,
							   MaddSIMD(pVectors->y, matSplat01, MaddSIMD(pVectors->x, matSplat00, matSplat03)));
			resultY = MaddSIMD(pVectors->z, matSplat12,
							   MaddSIMD(pVectors->y, matSplat11, MaddSIMD(pVectors->x, matSplat00, matSplat13)));
			resultZ = MaddSIMD(pVectors->z, matSplat22,
							   MaddSIMD(pVectors->y, matSplat21, MaddSIMD(pVectors->x, matSplat00, matSplat23)));
...
#ifdef _X360
void TransformManyPointsBy(VectorAligned *RESTRICT pVectors, unsigned int numVectors, FLTX4 mRow0, FLTX4 mRow1,
						   FLTX4 mRow2)
{
	/**************************************************
...

	{
		...
		UINT_PTR InputStreamCGroupBase =
			XMMin(InputVector + (XM_CACHE_LINE_SIZE - 1), InputStreamEnd) & ~(XM_CACHE_LINE_SIZE - 1);
		UINT_PTR InputStreamCGroupEnd =
			InputStreamCGroupBase + ((InputStreamEnd - InputStreamCGroupBase) & ~(4 * XM_CACHE_LINE_SIZE - 1));
