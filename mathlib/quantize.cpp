...
struct QuantizedValue *FindQNode(struct QuantizedValue const *q, int32 code)
{
	...
	else
	{
		...
		if(!found)
			found = FindQNode(q->Children[1], code);
		return found;
	}
}
...
#define PUSHNODE(a)                                              \
	{                                                            \
		...
		if((a)->MinError < besterror)                            \
			HeapInsert(&TheQueue, &(a)->MinError);               \
	}
...
struct QuantizedValue *FindMatch(uint8 const *sample, int ndims, uint8 *weights, struct QuantizedValue *q)
{
	...
	for(;;)
	{
		struct QuantizedValue *test = (struct QuantizedValue *)RemoveHeapItem(&TheQueue);
		if(!test)
			break; // heap empty
