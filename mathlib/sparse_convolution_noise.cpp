					   ...
					   float (*pNoiseShapeFunction)(float))
{
	...
	for(int idx = 0; idx < N_IMPULSES_PER_CELL; idx++)
	{
		...
		float dsq = SQ(impulse_xcoords[coord_idx] - xfrac) + SQ(impulse_ycoords[coord_idx] - yfrac) +
...
float SparseConvolutionNoise(Vector const &pnt, float (*pNoiseShapeFunction)(float))
{
	...
	for(int ox = -1; ox <= 1; ox++)
		for(int oy = -1; oy <= 1; oy++)
			...
			{
				sum_out +=
					CellNoise(ix + ox, iy + oy, iz + oz, xfrac - ox, yfrac - oy, zfrac - oz, pNoiseShapeFunction);
			}
#ifdef MEASURE_RANGE
