...
void LightDesc_t::ComputeLightAtPointsForDirectional(const FourVectors &pos, const FourVectors &normal,
...
void LightDesc_t::ComputeLightAtPoints(const FourVectors &pos, const FourVectors &normal, FourVectors &color,
	...
	switch(m_Type)
	{
		...
		case MATERIAL_LIGHT_SPOT:
		{
			...
			fltx4 cone_falloff_scale =
				MulSIMD(ReplicateX4(OneOver_ThetaDot_Minus_PhiDot), SubSIMD(dot2, ReplicateX4(m_PhiDot)));
...
void LightDesc_t::SetupOldStyleAttenuation(float fQuadraticAttn, float fLinearAttn, float fConstantAttn)
{
	...
	if((fConstantAttn < EQUAL_EPSILON) && (fLinearAttn < EQUAL_EPSILON) && (fQuadraticAttn < EQUAL_EPSILON))
		fConstantAttn = 1;
