...
// Matrix is right-handed x=forward, y=left, z=up.  We a left-handed convention for vectors in the game code (forward,
// right, up)
...
void VectorAngles(const Vector &forward, const Vector &pseudoup, QAngle &angles)
{
	...
	else // forward is mostly Z, gimbal lock-
	{
		// (yaw)	y = ATAN( -left.x, left.y );			-- forward is mostly z, so use right for yaw
		angles[1] =
			RAD2DEG(atan2f(-left[0], left[1])); // This was originally copied from the "void MatrixAngles( const
												// matrix3x4_t& matrix, float *angles )" code, and it's 180 degrees off,
												// negated the values and it all works now (Dave Kircher)
...
//			p4n -
//-----------------------------------------------------------------------------
void Spline_Normalize(const Vector &p1, const Vector &p2, const Vector &p3, const Vector &p4, Vector &p1n, Vector &p4n)
{
	float dt = p3.x - p2.x;
...
void Catmull_Rom_Spline_Integral(const Vector &p1, const Vector &p2, const Vector &p3, const Vector &p4, float t,
								 Vector &output)
{
	output = p2 * t - 0.25f * (p1 - p3) * t * t + (1.0f / 6.0f) * (2.0f * p1 - 5.0f * p2 + 4.0f * p3 - p4) * t * t * t -
...
//-----------------------------------------------------------------------------
void TransformAABB(const matrix3x4_t &transform, const Vector &vecMinsIn, const Vector &vecMaxsIn, Vector &vecMinsOut,
				   Vector &vecMaxsOut)
{
	Vector localCenter;
...
//-----------------------------------------------------------------------------
void ITransformAABB(const matrix3x4_t &transform, const Vector &vecMinsIn, const Vector &vecMaxsIn, Vector &vecMinsOut,
					Vector &vecMaxsOut)
{
	Vector worldCenter;
...
//-----------------------------------------------------------------------------
void RotateAABB(const matrix3x4_t &transform, const Vector &vecMinsIn, const Vector &vecMaxsIn, Vector &vecMinsOut,
				Vector &vecMaxsOut)
{
	Vector localCenter;
	VectorAdd(vecMinsIn, vecMaxsIn, localCenter);
...
// Uses the inverse transform of in1
//-----------------------------------------------------------------------------
void IRotateAABB(const matrix3x4_t &transform, const Vector &vecMinsIn, const Vector &vecMaxsIn, Vector &vecMinsOut,
				 Vector &vecMaxsOut)
{
	Vector oldCenter;
...
void CalcClosestPointOnLine2D(const Vector2D &P, const Vector2D &vLineA, const Vector2D &vLineB, Vector2D &vClosest,
							  float *outT)
{
	Assert(s_bMathlibInitialized);
	Vector2D vDir;
...
void CalcClosestPointOnLineSegment2D(const Vector2D &P, const Vector2D &vLineA, const Vector2D &vLineB,
									 Vector2D &vClosest, float *outT)
{
	Vector2D vDir;
	float t = CalcClosestPointToLineT2D(P, vLineA, vLineB, vDir);
	t = clamp(t, 0.f, 1.f);
...
void MathLib_Init(float gamma, float texGamma, float brightness, int overbright, bool bAllow3DNow, bool bAllowSSE,
				  bool bAllowSSE2, bool bAllowMMX)
{
							...
							double fOnPlaneEpsilon)
{
	...
	{
		// dot = DotProduct( inVerts[i], normal) - dist;
		dot = ((inVerts[i * 3 + 0] * normal[0]) + (inVerts[i * 3 + 1] * normal[1]) + (inVerts[i * 3 + 2] * normal[2])) -
			  dist;
		dists[i] = dot;
		if(dot > fOnPlaneEpsilon)
		{
						  ...
						  float *pValueB, float *pInterpolationValue)
{
	...
	if(idx == 0)
	{
		if(bWrap)
		{
			...
			nKnot2 = 0;
			flSizeOfGap = (pKnotPositions[nKnot2] + (nInterpolationRange - pKnotPositions[nKnot1]));
			flOffsetFromStartOfGap = flPositionToInterpolateAt + (nInterpolationRange - pKnotPositions[nKnot1]);
		}
		else
		{
