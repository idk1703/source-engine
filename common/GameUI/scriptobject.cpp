...
objtypedesc_t objtypes[] = {
	{O_BOOL, "BOOL"},		  {O_NUMBER, "NUMBER"}, {O_LIST, "LIST"},		  {O_STRING, "STRING"},
	{O_OBSOLETE, "OBSOLETE"}, {O_SLIDER, "SLIDER"}, {O_CATEGORY, "CATEGORY"},
};
...
void CDescription::TransferCurrentValues(const char *pszConfigFile)
{
	...
	while(pObj)
	{
		...
		if ( pObj->bSetInfo )
		{
			value = engine->LocalPlayerInfo_ValueForKey( pObj->cvarname ); // use LocalPlayerInfo because PlayerInfo
		strips keys prefixed with "_"
		}
		else
		{
...
//-----------------------------------------------------------------------------
CInfoDescription::CInfoDescription(void) : CDescription()
{
...
void CInfoDescription::WriteScriptHeader(FileHandle_t fp)
{
	...
	g_pFullFileSystem->FPrintf(
		fp, "// Half-Life User Info Configuration Layout Script (stores last settings chosen, too)\r\n");
