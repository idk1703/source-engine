...
#define HFX_PARAM_ENCODE_CHAR_LOWER(lowercase_alpha_or_underscore)                                       \
	(lowercase_alpha_or_underscore                                                                       \
		 ? ((lowercase_alpha_or_underscore < ('~' + 1)) ? ((lowercase_alpha_or_underscore) - ('^')) : 2) \
		 : 0)

#define HFX_PARAM_ENCODE_CHAR_UPPER(upper_alpha)                                                                       \
	(upper_alpha != 0                                                                                                  \
		 ? ((upper_alpha > '>') ? ((upper_alpha) - ('>'))                                                              \
								: (((upper_alpha > '/') && (upper_alpha < ':')) ? ((upper_alpha + 32) - ('>')) : (2))) \
		 : 0)

#define HFX_PARAM_ENCODE_CHAR(alpha_or_underscore)                                                        \
	(alpha_or_underscore ? (alpha_or_underscore < '_') ? HFX_PARAM_ENCODE_CHAR_UPPER(alpha_or_underscore) \
													   : HFX_PARAM_ENCODE_CHAR_LOWER(alpha_or_underscore) \
						 : 0)

#define HFX_PARAM_PACK_CHAR(char, index) (char ? ((char) << (index * 5)) : 0)

#define HFX_PARAM_ENCODE_STRING_CHAR(char) \
	((char < '_') ? (HFX_PARAM_ENCODE_CHAR_UPPER(char)) : (HFX_PARAM_ENCODE_CHAR_LOWER(char)))

#define HFX_PARAM_ENCODE_AND_PACK_CHAR(char, index) HFX_PARAM_PACK_CHAR(HFX_PARAM_ENCODE_CHAR(char), index)
									  ...
									  ? ((HFX_PARAM_ENCODE_STRING_CHAR((ConstCharPtr7)[2])) << 10) |              \
											(ConstCharPtr7[3] != 0                                                \
												 ? ((HFX_PARAM_ENCODE_STRING_CHAR((ConstCharPtr7)[3])) << 15) |   \
													   (ConstCharPtr7[4] != 0                                     \
															? ((HFX_PARAM_ENCODE_STRING_CHAR((ConstCharPtr7)[4])) \
															   << 20) |                                           \
																  (ConstCharPtr7[5] != 0                          \
																	   ? ((HFX_PARAM_ENCODE_STRING_CHAR(          \
																			  (ConstCharPtr7)[5]))                \
																		  << 25)                                  \
																	   : (unsigned)0)                             \
															: (unsigned)0)                                        \
...
#define HFX_PARAM_ENCODE_6(c1, c2, c3, c4, c5, c6)                                                \
	((HFXParamID)(HFX_PARAM_ENCODE_AND_PACK_CHAR(c1, 0) | HFX_PARAM_ENCODE_AND_PACK_CHAR(c2, 1) | \
				  HFX_PARAM_ENCODE_AND_PACK_CHAR(c3, 2) | HFX_PARAM_ENCODE_AND_PACK_CHAR(c4, 3) | \
				  HFX_PARAM_ENCODE_AND_PACK_CHAR(c5, 4) | HFX_PARAM_ENCODE_AND_PACK_CHAR(c6, 5)))

#define HFX_PARAM_ENCODE_5(c1, c2, c3, c4, c5) HFX_PARAM_ENCODE_6(c1, c2, c3, c4, c5, 0)

#define HFX_PARAM_ENCODE_4(c1, c2, c3, c4) HFX_PARAM_ENCODE_6(c1, c2, c3, c4, 0, 0)

#define HFX_PARAM_ENCODE_3(c1, c2, c3) HFX_PARAM_ENCODE_6(c1, c2, c3, 0, 0, 0)

#define HFX_PARAM_ENCODE_2(c1, c2) HFX_PARAM_ENCODE_6(c1, c2, 0, 0, 0, 0)

#define HFX_PARAM_ENCODE_1(c1) HFX_PARAM_ENCODE_6(c1, 0, 0, 0, 0, 0)
...
#define HFX_CHAR_ID(paramid, index) (HFX_MASK_ID(paramid, index) ? HFX_MASK_ID(paramid, index) + '^' : 0)

// usage : char hfxParamText[] = HFX_PARAMETER_DECODE(hfxParamID);
#define HFX_PARAM_DECODE(hfxparam)                                                                              \
	{                                                                                                           \
		HFX_CHAR_ID(hfxparam, 0), HFX_CHAR_ID(hfxparam, 1), HFX_CHAR_ID(hfxparam, 2), HFX_CHAR_ID(hfxparam, 3), \
			HFX_CHAR_ID(hfxparam, 4), HFX_CHAR_ID(hfxparam, 5), 0                                               \
	}
...
#define HFX_PARAMETER_DOUBLE_ARRAY(size, name, membername) \
	...
	{                                                      \
		double name[size];                                 \
		double m_dbl##size##name[size];                    \
		double membername[size];                           \
	};                                                     \
...
#define HFX_PARAMETER_BOOL_ARRAY(size, name, membername) \
	...
	{                                                    \
		bool name[size];                                 \
		bool m_b##size##name[size];                      \
		bool membername[size];                           \
	};                                                   \
...
#define HFX_PARAMETER_INT_ARRAY(size, name, membername) \
	...
	// ACTUAL OVERRIDES!
	virtual bool SetVar(const HFXParamID id, void *value, HFXParamType t, HFXARRAY settype = HFXARRAY_NOTARRAY,
						unsigned int indexORcount = 0, bool allowMake = false) = 0;

	virtual bool GetVar(const HFXParamID id, void *value, HFXParamType t, HFXARRAY gettype = HFXARRAY_NOTARRAY,
						unsigned int indexORcount = 0) const = 0;

#ifndef HFX_NO_OVERLOADED_PARAMETER

	HFX_INLINE bool SetVarByName(const char *name, HFXVarValue value, HFXParamType t, bool allowMake)
	{
		return SetVar(HFX_PARAM_ENCODE_STRING(name), value, t, HFXARRAY_NOTARRAY, 0, allowMake);
	}

	HFX_INLINE bool SetVar(const HFXParamID name, HFXVarValue value, HFXParamType t, bool allowMake)
	{
		return SetVar(name, value, t, HFXARRAY_NOTARRAY, 0, allowMake);
	}

	HFX_INLINE bool SetVarByName(const char *name, HFXVarValue value, HFXParamType t, HFXARRAY settype,
								 unsigned int indexORcount, bool allowMake)
	{
	...
	HFX_INLINE bool SetVarArrayByName(const char *name, unsigned int arraySize, const double *value,
									  bool allowMake = false)
	{
		return SetVarByName(name, (HFXVarValue)&value, HFXPARAM_DOUBLE_ARRAY, HFXARRAY_COPY, arraySize, allowMake);
	}
	HFX_INLINE bool SetVarArray(const HFXParamID name, unsigned int arraySize, const double *value,
								bool allowMake = false)
	{
		return SetVar(name, (HFXVarValue)&value, HFXPARAM_DOUBLE_ARRAY, HFXARRAY_COPY, arraySize, allowMake);
	}
	HFX_INLINE bool SetVarArrayByName(const char *name, unsigned int arraySize, const bool *value,
									  bool allowMake = false)
	{
		return SetVarByName(name, (HFXVarValue)&value, HFXPARAM_BOOL_ARRAY, HFXARRAY_COPY, arraySize, allowMake);
	}
	HFX_INLINE bool SetVarArray(const HFXParamID name, unsigned int arraySize, const bool *value,
								bool allowMake = false)
	{
		return SetVar(name, (HFXVarValue)&value, HFXPARAM_BOOL_ARRAY, HFXARRAY_COPY, arraySize, allowMake);
	}
	HFX_INLINE bool SetVarArrayByName(const char *name, unsigned int arraySize, const float *value,
									  bool allowMake = false)
	{
		return SetVarByName(name, (HFXVarValue)&value, HFXPARAM_FLOAT_ARRAY, HFXARRAY_COPY, arraySize, allowMake);
	}
	HFX_INLINE bool SetVarArray(const HFXParamID name, unsigned int arraySize, const float *value,
								bool allowMake = false)
	{
		return SetVar(name, (HFXVarValue)&value, HFXPARAM_FLOAT_ARRAY, HFXARRAY_COPY, arraySize, allowMake);
	}
	HFX_INLINE bool SetVarArrayByName(const char *name, unsigned int arraySize, const int *value,
									  bool allowMake = false)
	{
	...
	HFX_INLINE bool GetVarByName(const char *name, void *value, HFXParamType t, HFXARRAY gettype = HFXARRAY_NOTARRAY,
								 unsigned int indexORcount = 0) const
	{
	...
	HFX_INLINE bool SetVarDoubleArrayByName(const char *name, unsigned int size, const double *value,
											bool allowMake = false)
	{
		return SetVarArrayByName(name, size, value, allowMake);
	}
	HFX_INLINE bool SetVarDoubleArray(const HFXParamID name, unsigned int size, const double *value,
									  bool allowMake = false)
	{
		return SetVarArray(name, size, value, allowMake);
	}
	HFX_INLINE bool SetVarFloatArrayByName(const char *name, unsigned int size, const float *value,
										   bool allowMake = false)
	{
		return SetVarArrayByName(name, size, value, allowMake);
	}
	HFX_INLINE bool SetVarFloatArray(const HFXParamID name, unsigned int size, const float *value,
									 bool allowMake = false)
	{
		return SetVarArray(name, size, value, allowMake);
	}
	...
	HFX_INLINE bool SetVarBoolArrayByName(const char *name, unsigned int size, const bool *value,
										  bool allowMake = false)
	{
		return SetVarArrayByName(name, size, value, allowMake);
	}
	...
	HFX_INLINE bool SetVarDoubleArrayByFloatArray(const HFXParamID name, unsigned int size, const float *value,
												  bool allowMake = false)
	{
	...
	HFX_INLINE bool SetVarDoubleArrayByFloatArrayByName(const char *name, unsigned int size, const float *value,
														bool allowMake = false)
	{
	...
	*/
	...
	// creation
	virtual bool CreateStandardEffect(IHFXEffect *&pHandle, class IStack *createOnStack = 0,
									  const char *customName = 0) = 0;
	...
	HFX_INLINE HFXParamType GetVarTypeByName(const char *param, unsigned int *arraysize = 0) const
	{
		return GetVarType(HFX_PARAM_ENCODE_STRING(param), arraysize);
	}
...
template<typename T>
void HFXDefaultAllocateParameterGroup(IHFXParamGroup *&ptr)
{
	ptr = new T;
};

template<typename T>
void HFXDefaultDeallocateParameterGroup(IHFXParamGroup *&ptr)
{
	if(!ptr)
		return;
	delete((T *)ptr);
