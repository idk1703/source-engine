...
public:
	...
	HFX_INLINE HFXStateFlags(bool bRunning, bool bPaused, bool bMuted, bool bHaulted, bool bExiting)
		: Storage((bRunning ? HFXSTATE_RUNNING : 0) | (bPaused ? HFXSTATE_PAUSE : 0) | (bMuted ? HFXSTATE_MUTE : 0) |
				  (bHaulted ? HFXSTATE_HAULT : 0) | (bExiting ? HFXSTATE_EXITING : 0)) {};
	...
	HFX_INLINE operator HFXStateStorage &()
	{
		return Storage;
	}
	HFX_INLINE operator const HFXStateStorage &() const
	{
		return Storage;
	}
	HFX_INLINE operator HFXStateStorage()
	...
	HFX_INLINE HFXStateStorage &operator*()
	{
		return Storage;
	};
	HFX_INLINE const HFXStateStorage &operator*() const
	{
		return Storage;
	};

	// bool operator, to see if there are any flags set.
	HFX_INLINE bool operator()(int bit)
	{
		return (Storage & bit) != 0;
	};

	// -> operator to get boolean access flags quickly.
	HFX_INLINE HFXStateFlagGroup *operator->()
	{
		return &Flags;
	};
	HFX_INLINE const HFXStateFlagGroup *operator->() const
	{
		return &Flags;
	};
	...
	HFX_INLINE HFXStateStorage &operator=(HFXStateTransfer state)
	{
		Storage = state;
		return Storage;
	}
	...
	HFX_INLINE HFXStateStorage operator~() const
	{
		return ~Storage;
	}
	HFX_INLINE HFXStateStorage operator|(HFXStateTransfer state)
	...
	HFX_INLINE bool operator==(HFXStateTransfer state)
	{
		return (Storage == state);
	}
	HFX_INLINE bool operator!=(HFXStateTransfer state)
	...
	HFX_INLINE bool operator!()
	{
		return (Storage == 0);
	}
	HFX_INLINE operator bool()
	{
		return (Storage != 0);
	}
	HFX_INLINE HFXStateFlags *UtilPointer()
	{
		return this;
	};
	HFX_INLINE const HFXStateFlags *UtilPointer() const
	{
		return this;
	};
	HFX_INLINE HFXStateFlags &Util()
	{
		return *this;
	};
	HFX_INLINE const HFXStateFlags &Util() const
	{
		return *this;
	};
};
...
enum HFXNeeds
{
	...
	HFXNEED_DECL_TOTAL_INFORM = (HFXNEED_RUNNING_INFORM | HFXNEED_PAUSE_INFORM | HFXNEED_MUTE_INFORM |
								 HFXNEED_EXITING_INFORM | HFXNEED_STATE_INFORM),
	// INFORMATION FLAGS
	HFXNEED_INFO_DEVICE = (HFXNEED_DEVICE1 | HFXNEED_DEVICE2 | HFXNEED_DEVICE3 | HFXNEED_DEVICE4 | HFXNEED_DEVICE5 |
						   HFXNEED_DEVICE6 | HFXNEED_DEVICE7 | HFXNEED_DEVICE8),
};

#define HFXNEED_UTIL_COUNT_DEVICES(flags)                                                             \
	((((flags)&HFXNEED_INFO_DEVICE) != 0)                                                             \
		 ? (((((flags)&HFXNEED_DEVICE1) != 0) ? 1 : 0) + ((((flags)&HFXNEED_DEVICE2) != 0) ? 1 : 0) + \
			((((flags)&HFXNEED_DEVICE3) != 0) ? 1 : 0) + ((((flags)&HFXNEED_DEVICE4) != 0) ? 1 : 0) + \
			((((flags)&HFXNEED_DEVICE5) != 0) ? 1 : 0) + ((((flags)&HFXNEED_DEVICE6) != 0) ? 1 : 0) + \
			((((flags)&HFXNEED_DEVICE7) != 0) ? 1 : 0) + ((((flags)&HFXNEED_DEVICE8) != 0) ? 1 : 0))  \
		 : 0)
...
template<typename T>
void HFXDefaultAllocateEffect(IHFXEffect *&ptr)
{
	ptr = new T;
};

template<typename T>
void HFXDefaultDeallocateEffect(IHFXEffect *&ptr)
{
	if(!ptr)
		return;
	delete((T *)ptr);
...
// T == effect class
template<typename T>
inline HFXEffectID HFXDefaultRegisterEffect(IHFXSystem &hfxSystem, const char *tag, HFXNEED needs,
											IHFXParamGroup *defaults,
											HFXCreate_t specialCreate = HFXDefaultAllocateEffect<T>,
											HFXDestroy_t specialDestroy = HFXDefaultDeallocateEffect<T>)
{
	hfxSystem.LogMessage((const int)1, "EFFECT REGISTERING! %s Size = %i \n", tag, sizeof(T));
	return hfxSystem.RegisterEffectClass(tag, specialCreate, specialDestroy, needs, defaults);
};
...
inline HFXEffectID HFXDefaultRegisterEffect(IHFXSystem &hfxSystem, const char *tag, HFXNEED needs,
											HFXCreate_t specialCreate = 0, HFXDestroy_t specialDestroy = 0)
{
	P *pGroup = new P;
