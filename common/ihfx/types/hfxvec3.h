...
#ifdef HFX_INTERNAL
	HFX_INLINE hfxVec3internal *GetVec3Pointer()
	{
		return (this);
	};
	HFX_INLINE const hfxVec3internal *GetVec3Pointer() const
	{
		return (this);
	};
	HFX_INLINE operator hfxVec3 &()
	{
		return *(reinterpret_cast<hfxVec3 *>(this));
	}
	HFX_INLINE operator const hfxVec3 &() const
	{
		return *(reinterpret_cast<const hfxVec3 *>(this));
	}
#endif
...
struct BoundingBox3 : public _box3Base
{
	...
	inline hfxVec3 &BoxMin()
	{
		return (hfxVec3 &)box.bound_min;
	};
	inline const hfxVec3 &BoxMin() const
	{
		return (const hfxVec3 &)box.bound_min;
	};
	inline hfxVec3 &BoxMax()
	{
		return (hfxVec3 &)box.bound_max;
	};
	inline const hfxVec3 &BoxMax() const
	{
		return (const hfxVec3 &)box.bound_max;
	};
	inline hfxVec3 Center() const
	{
		return (BoxMin() + BoxMax()) / (double)2.0;
	};
	inline hfxVec3 Dimensions() const
	{
		return BoxMax() - BoxMin();
	};
	inline hfxVec3 SpaceBetweenCentered(const BoundingBox3 &other) const
	{
		return other.Dimensions() - Dimensions();
	};
	inline BoundingBox3 SpaceBetween(const BoundingBox3 &other) const
	{
		return other - *this;
	}
	inline BoundingBox3 &SetDimensions(const hfxVec3 &dimm)
	{
		hfxVec3 cent = Center();
		hfxVec3 dim = dimm / 2.0;
		BoxMin() = cent - dim;
		BoxMax() = cent + dim;
		return *this;
	}
	inline BoundingBox3 &SetCenter(const hfxVec3 &cent)
	{
		hfxVec3 dim = Dimensions();
		BoxMin() = cent - dim;
		BoxMax() = cent + dim;
		return *this;
	}
	inline operator hfxVec3 *()
	{
		return (hfxVec3 *)min_max;
	};
	inline operator const hfxVec3 *() const
	{
		return (const hfxVec3 *)min_max;
	};
	...
	inline BoundingBox3 operator-(const hfxVec3 &vect) const
	{
		return BoundingBox3(BoxMin() - vect, BoxMax() - vect);
	}
	inline BoundingBox3 operator+(const hfxVec3 &vect) const
	{
		return BoundingBox3(BoxMin() + vect, BoxMax() + vect);
	...
	inline BoundingBox3 operator*(const double &df) const
	{
		return BoundingBox3(BoxMin() * df, BoxMax() * df);
	}
	inline BoundingBox3 operator/(const double &df) const
	{
		return BoundingBox3(BoxMin() / df, BoxMax() / df);
	}
	inline BoundingBox3 operator-(const BoundingBox3 &bbox) const
	{
		return BoundingBox3(BoxMin() - bbox.BoxMin(), BoxMax() - bbox.BoxMax());
	}
	inline BoundingBox3 operator+(const BoundingBox3 &bbox) const
	{
		return BoundingBox3(BoxMin() + bbox.BoxMin(), BoxMax() + bbox.BoxMax());
	...
	inline BoundingBox3 &operator=(const BoundingBox3 &bbox)
	{
		BoxMin() = bbox.BoxMin();
		BoxMax() = bbox.BoxMax();
		return *this;
	}
	inline BoundingBox3 &operator-=(const hfxVec3 &vect)
	{
		BoxMin() -= vect;
		BoxMax() -= vect;
		return *this;
	...
	inline BoundingBox3 &operator*=(const double &df)
	{
		BoxMin() *= df;
		BoxMax() *= df;
		return *this;
