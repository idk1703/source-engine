...
#define SET_TOS(prec, tos) (((0x7 & (prec)) << 5) | (0x1c & (tos)))
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	OTInetGetDHCPConfigInfo(InetDHCPOption *buf, UInt32 bufSize, SInt32 index, SInt32 opt);
	...
	*/
	...
	OTOpenInternetServicesInContext(OTConfigurationRef cfig, OTOpenFlags oflag, OSStatus *err,
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenInternetServicesInContext(OTConfigurationRef cfig, OTOpenFlags oflag, OTNotifyUPP upp, void *contextPtr,
										 OTClientContextPtr clientContext);
	 ...
	 */
	EXTERN_API(InetSvcRef)
	OTOpenInternetServices(OTConfigurationRef cfig, OTOpenFlags oflag, OSStatus *err);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenInternetServices(OTConfigurationRef cfig, OTOpenFlags oflag, OTNotifyUPP proc, void *contextPtr);
...
#define OTAsyncOpenInternetServices(cfig, oflags, proc, contextPtr) \
	 ...
	 */
	EXTERN_API(OSStatus)
	OTInetMailExchange(InetSvcRef ref, char *name, UInt16 *num, InetMailExchange *mx);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTInetQuery(InetSvcRef ref, char *name, UInt16 qClass, UInt16 qType, char *buf, OTByteCount buflen, void **argv,
				OTByteCount argvlen, OTFlags flags);
...
public:
	...
	OSStatus Query(char *name, UInt16 qClass, UInt16 qType, char *buf, OTByteCount buflen, void **argv,
				   OTByteCount argvlen, OTFlags flags)
	{
...
#define kLTalkBName "ltlkB"
	...
	*/
	...
	enum
	{
		...
		ATP_OPT_RELTIMER = 0x2112, /* AppleTalk - ATP Release Timer Type, Value is (unsigned long) timer, (See Inside
									  AppleTalk, second edition */
		ATP_OPT_TRANID = 0x2113,   /* Value is (unsigned long) Boolean, Used to request Transaction ID, Returned with
									  Transaction ID on requests */
	...
	*/
	...
	enum
	{
		...
		T_ATALKROUTERDOWNEVENT =
			kAppleTalkEvent + 51, /* No routers have been seen for a while.  If the cookie is NULL, all routers are
									 gone.  Otherwise, there is still an ARA router hanging around being used, and only
									 the local cable has been  timed out.*/
		T_ATALKROUTERUPEVENT =
			kAppleTalkEvent + 52, /* We didn't have a router, but now one has come up. Cookie is NULL for a normal
									 router coming up, non-NULL for an ARA router coming on-line*/
		T_ATALKZONENAMECHANGEDEVENT =
			kAppleTalkEvent + 53, /* A Zone name change was issued from the router, so our AppleTalk Zone has changed.*/
		T_ATALKCONNECTIVITYCHANGEDEVENT =
			kAppleTalkEvent +
			54, /* An ARA connection was established (cookie != NULL), or was disconnected (cookie == NULL)*/
		T_ATALKINTERNETAVAILABLEEVENT = kAppleTalkEvent + 55, /* A router has appeared, and our address is in the
																 startup range.  Cookie is hi/lo of new cable range.*/
		T_ATALKCABLERANGECHANGEDEVENT = kAppleTalkEvent + 56  /* A router has appeared, and it's incompatible with our
	...
	enum
	{
		...
		kLocalATalkRoutersDown =
			-1L, /* This indicates that all local routers went offline, but an ARA router is still active*/
		kARARouterDisconnected = -2L /* This indicates that ARA was disconnected, do it's router went offline, and we
										have no local routers to fall back onto.*/
	};
...
#define IsAppleTalkEvent(x)         ((x) & 0xffff0000) == kAppleTalkEvent)
	...
	enum
	{
		ATALK_IOC_FULLSELFSEND =
			((MIOC_ATALK << 8) | 47), /* Turn on/off full self-send (it's automatic for non-backward-compatible links)*/
...
#define kDefaultAppleTalkServicesPath ((OTConfigurationRef)-3L)
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenAppleTalkServicesInContext(OTConfigurationRef cfig, OTOpenFlags flags, OTNotifyUPP proc,
										  void *contextPtr, OTClientContextPtr clientContext);
	 ...
	 */
	EXTERN_API(ATSvcRef)
	OTOpenAppleTalkServicesInContext(OTConfigurationRef cfig, OTOpenFlags flags, OSStatus *err,
									 OTClientContextPtr clientContext);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenAppleTalkServices(OTConfigurationRef cfig, OTOpenFlags flags, OTNotifyUPP proc, void *contextPtr);
	 ...
	 */
	EXTERN_API(ATSvcRef)
	OTOpenAppleTalkServices(OTConfigurationRef cfig, OTOpenFlags flags, OSStatus *err);
...
{	   /* resume C definitions */
	...
	*/
	 ...
	 */
	EXTERN_API(void)
	OTInitDDPAddress(DDPAddress *addr, UInt16 net, UInt8 node, UInt8 socket, UInt8 ddpType);
	 ...
	 */
	EXTERN_API(OTByteCount)
	OTInitDDPNBPAddress(DDPNBPAddress *addr, const char *name, UInt16 net, UInt8 node, UInt8 socket, UInt8 ddpType);
	...
	*/
	 ...
	 */
	EXTERN_API(Boolean)
	OTSetNBPEntityFromAddress(NBPEntity *entity, const UInt8 *addrBuf, OTByteCount len);
...
#define kFibreChannelName "fibre"
	...
	enum
	{
		...
		k8022SNAPLength = 5,
		kEnetAddressLength =
			k48BitAddrLength + k8022DLSAPLength, /* length of an address field used by the ENET enpoint*/
...
#define kSerialPortBName "serialB"
	...
	enum
	{
		...
		kOTSerialSetDTROn = 1,
		I_SetSerialBreak =
			((MIOC_SRL << 8) | 1), /* Send a break on the line - kOTSerialSetBreakOff = off, kOTSerialSetBreakOn = on,*/
		...
		I_SetSerialXOffState =
			((MIOC_SRL << 8) |
			 2), /* Force XOFF state - 0 = Unconditionally clear XOFF state, 1 = unconditionally set it*/
		kOTSerialForceXOffTrue = 1,
		kOTSerialForceXOffFalse = 0,
		I_SetSerialXOn =
			((MIOC_SRL << 8) | 3), /* Send an XON character, 0 = send only if in XOFF state, 1 = send always*/
		kOTSerialSendXOnAlways = 1,
		kOTSerialSendXOnIfXOffTrue = 0,
		I_SetSerialXOff =
			((MIOC_SRL << 8) | 4), /* Send an XOFF character, 0 = send only if in XON state, 1 = send always*/
	...
	*/
	...
	enum
	{
		...
		SERIAL_OPT_RCVTIMEOUT =
			0x0106, /* The "RcvTimeout" option defines how long the receiver should wait before delivering*/
		...
		SERIAL_OPT_EXTCLOCK =
			0x0108, /* The "ExtClock" requests an external clock.  A 0-value turns off external clocking.*/
		...
		SERIAL_OPT_BURSTMODE =
			0x0109, /* The "BurstMode" option informs the serial driver that it should continue looping,*/
