...
#pragma pack(2)
	...
	enum
	{
		...
		kFSAliasInfoVolumeFlags = 0x00000040	   /* volumeIsBootVolume, volumeIsAutomounted, volumeIsEjectable and
													  volumeHasPersistentFileIDs are valid*/
	};
	...
	NewAlias(const FSSpec *fromFile, /* can be NULL */
			 const FSSpec *target, AliasHandle *alias) TWOWORDINLINE(0x7002, 0xA823);
	 ...
	 */
	EXTERN_API(OSErr)
	NewAliasMinimal(const FSSpec *target, AliasHandle *alias) TWOWORDINLINE(0x7008, 0xA823);
	 ...
	 */
	EXTERN_API(OSErr)
	NewAliasMinimalFromFullPath(short fullPathLength, const void *fullPath, ConstStr32Param zoneName,
								ConstStr31Param serverName, AliasHandle *alias) TWOWORDINLINE(0x7009, 0xA823);
	...
	ResolveAlias(const FSSpec *fromFile, /* can be NULL */
				 AliasHandle alias, FSSpec *target, Boolean *wasChanged) TWOWORDINLINE(0x7003, 0xA823);
	 ...
	 */
	EXTERN_API(OSErr)
	GetAliasInfo(AliasHandle alias, AliasInfoType index, Str63 theString) TWOWORDINLINE(0x7007, 0xA823);
	 ...
	 */
	EXTERN_API(OSErr)
	IsAliasFile(const FSSpec *fileFSSpec, Boolean *aliasFileFlag, Boolean *folderFlag) TWOWORDINLINE(0x702A, 0xA823);
	...
	ResolveAliasWithMountFlags(const FSSpec *fromFile, /* can be NULL */
							   AliasHandle alias, FSSpec *target, Boolean *wasChanged, unsigned long mountFlags)
	 ...
	 */
	EXTERN_API(OSErr)
	ResolveAliasFile(FSSpec *theSpec, Boolean resolveAliasChains, Boolean *targetIsFolder, Boolean *wasAliased)
	 ...
	 */
	EXTERN_API(OSErr)
	ResolveAliasFileWithMountFlags(FSSpec *theSpec, Boolean resolveAliasChains, Boolean *targetIsFolder,
								   Boolean *wasAliased, unsigned long mountFlags) TWOWORDINLINE(0x7029, 0xA823);
	...
	FollowFinderAlias(const FSSpec *fromFile, /* can be NULL */
					  AliasHandle alias, Boolean logon, FSSpec *target, Boolean *wasChanged)
	...
	*/
	...
	UpdateAlias(const FSSpec *fromFile, /* can be NULL */
				const FSSpec *target, AliasHandle alias, Boolean *wasChanged) TWOWORDINLINE(0x7006, 0xA823);
	...
	{
		return (AliasFilterUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppAliasFilterProcInfo,
...
#define DisposeAliasFilterUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	...
	InvokeAliasFilterUPP(CInfoPBPtr cpbPtr, Boolean *quitFlag, Ptr myDataPtr, AliasFilterUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define CallAliasFilterProc(userRoutine, cpbPtr, quitFlag, myDataPtr) \
	...
	MatchAlias(const FSSpec *fromFile, /* can be NULL */
			   unsigned long rulesMask, AliasHandle alias, short *aliasCount, FSSpecArrayPtr aliasList,
			   Boolean *needsUpdate, AliasFilterUPP aliasFilter, void *yourDataPtr) TWOWORDINLINE(0x7005, 0xA823);
	 ...
	 */
	EXTERN_API(OSErr)
	ResolveAliasFileWithMountFlagsNoUI(FSSpec *theSpec, Boolean resolveAliasChains, Boolean *targetIsFolder,
	 ...
	 */
	EXTERN_API_C(OSErr)
	FSNewAliasUnicode(const FSRef *fromFile, /* can be NULL */
					  const FSRef *targetParentRef, UniCharCount targetNameLength, const UniChar *targetName,
					  AliasHandle *inAlias, Boolean *isDirectory); /* can be NULL */
	 ...
	 */
	EXTERN_API_C(OSErr)
	FSMatchAlias(const FSRef *fromFile, /* can be NULL */
				 unsigned long rulesMask, AliasHandle inAlias, short *aliasCount, FSRef *aliasList,
				 Boolean *needsUpdate, AliasFilterUPP aliasFilter, void *yourDataPtr);
	 ...
	 */
	EXTERN_API_C(OSErr)
	FSMatchAliasNoUI(const FSRef *fromFile, /* can be NULL */
					 unsigned long rulesMask, AliasHandle inAlias, short *aliasCount, FSRef *aliasList,
					 Boolean *needsUpdate, AliasFilterUPP aliasFilter, void *yourDataPtr);
	...
	FSCopyAliasInfo(AliasHandle inAlias, HFSUniStr255 *targetName, /* can be NULL */
