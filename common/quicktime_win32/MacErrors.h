...
#pragma import on
	...
	enum
	{
		...
		wrongApplicationPlatform =
			-875, /* The application could not launch because the required platform is not available    */
		appVersionTooOld =
			-876, /* The application's creator and version are incompatible with the current version of Mac OS. */
	...
	enum
	{
		...
		errAEUnknownSendMode =
			-1710,				   /* mode wasn't NoReply, WaitReply, or QueueReply or Interaction level is unknown */
		...
		errAEHandlerNotFound = -1717,  /* no handler in the dispatch tables fits the parameters to AEGetEventHandler or
		...
		errAENoSuchObject = -1728,	   /* e.g.,: specifier asked for the 3rd, but there are only 2. Basically, this
		...
		errAEDuplicateHandler =
			-1736, /* attempt to install handler in table for identical class and id (1.1 or greater) */
	...
	enum
	{
		...
		tsmDefaultIsNotInputMethodErr =
			-2524, /* Current Input source is KCHR or uchr, not Input Method  (GetDefaultInputMethod) */
		...
		tsmCantChangeForcedClassStateErr =
			-2530 /* Enabled state of a TextService class has been forced and cannot be changed */
	};
	...
	*/
	enum
	{
		...
		errOSAGeneralError =
			-2700, /* Signaled by user scripts or applications when no actual error code is to be returned.*/
		...
		errOSACantLaunch = -2703,	   /* Signaled when application can't be launched or when it is remote and program
		...
		errOSADataBlockTooLarge =
			-2708, /* Signaled when an intrinsic limitation is exceeded for the size of a value or data structure.*/
	...
	*/
	enum
	{
		...
		OSAMessageNotUnderstood =
			errAEEventNotHandled, /* Signaled when a message was sent to an object that didn't handle it*/
		...
		OSAIllegalIndex =
			errAEIllegalIndex, /* Signaled when index was out of range. Specialization of errOSACantAccess*/
		...
		OSASyntaxError =
			-2740, /* Signaled when a syntax error occurs. (e.g. "Syntax error" or "<this> can't go after <that>")*/
		OSASyntaxTypeError =
			-2741, /* Signaled when another form of syntax was expected. (e.g. "expected a <type> but found <this>")*/
		...
		OSAParameterMismatch = errAEWrongNumberArgs, /* Signaled when function is called with the wrong number of
														parameters, or a parameter pattern cannot be matched*/
		OSADuplicateParameter = -2750, /* Signaled when a formal parameter, local variable, or instance variable is
										  specified more than once*/
		OSADuplicateProperty = -2751,  /* Signaled when a formal parameter, local variable, or instance variable is
										  specified more than once.*/
		OSADuplicateHandler = -2752,   /* Signaled when more than one handler is defined with the same name in a scope
										  where the language doesn't allow it*/
		OSAUndefinedVariable = -2753,  /* Signaled when a variable is accessed that has no value*/
		OSAInconsistentDeclarations = -2754, /* Signaled when a variable is declared inconsistently in the same scope,
												such as both local and global*/
		OSAControlFlowError = -2755 /* Signaled when illegal control flow occurs in an application (no catcher for
									   throw, non-lexical loop exit, etc.)*/
	};
	...
	**************************************************************************/
	...
	enum
	{
		...
		errASNoResultReturned = -2763, /*    The range -2780 thru -2799 is reserved for dialect specific error codes.
	...
	enum
	{
		...
		cmRangeOverFlow =
			-4214, /* Color conversion warning that some output color values over/underflowed and were clipped */
	...
	enum
	{
		...
		afpBadIDErr = -5039,
		afpPwdSameErr =
			-5040, /* Someone tried to change their password to the same password on a mantadory password change */
		afpPwdTooShortErr =
			-5041, /* The password being set is too short: there is a minimum length that must be met or exceeded */
		afpPwdExpiredErr = -5042,	/* The password being used is too old: this requires the user to change the password
									   before log-in can continue */
		afpInsideSharedErr = -5043, /* The folder being shared is inside a shared folder OR the folder contains a shared
									   folder and is being moved into a shared folder */
		/* OR the folder contains a shared folder and is being moved into the descendent of a shared folder.*/
		afpInsideTrashErr = -5044, /* The folder being shared is inside the trash folder OR the shared folder is being
		...
		afpAlreadyLoggedInErr = -5047, /* User has been authenticated but is already logged in from another machine (and
										  that's not allowed on this server) */
	...
	enum
	{
		...
		errUnsupportedWindowAttributesForClass =
			-5601, /* tried to create a window with WindowAttributes not supported by the WindowClass*/
		errWindowDoesNotHaveProxy =
			-5602, /* tried to do something requiring a proxy to a window which doesn't have a proxy*/
		...
		errCorruptWindowDescription =
			-5606, /* tried to load a corrupt window description (size or version fields incorrect)*/
		errUserWantsToDragWindow =
			-5607, /* if returned from TrackWindowProxyDrag, you should call DragWindow on the window*/
		errWindowsAlreadyInitialized =
			-5608, /* tried to call InitFloatingWindows twice, or called InitWindows and then floating windows*/
	...
	*/
	...
	enum
	{
		...
		kTECBufferBelowMinimumSizeErr =
			-8750, /* output buffer too small to allow processing of first input text element*/
		kTECArrayFullErr =
			-8751, /* supplied name buffer or TextRun, TextEncoding, or UnicodeMapping array is too small*/
		...
		kTECItemUnavailableErr =
			-8771, /* item (e.g. name) not available for specified region (& encoding if relevant)*/
				   /* text conversion status codes*/
		kTECUsedFallbacksStatus = -8783,
		kTECNeedFlushStatus = -8784,
		kTECOutputBufferFullStatus =
			-8785, /* output buffer has no room for conversion of next input text element (partial conversion)*/
	...
	enum
	{
		...
		errAEPropertiesClash =
			-10025 /* illegal combination of properties settings for Set Data, make new, or duplicate */
	};
	...
	*/
	...
	enum
	{
		...
		dsGibblyMovedToDisabledFolder =
			117, /* For debug builds only, signals that active gibbly was disabled during boot. */
		dsUnBootableSystem =
			118, /* Active system file will not boot on this system because it was designed only to boot from a CD. */
		dsMustUseFCBAccessors = 119,   /* FCBSPtr and FSFCBLen are invalid - must use FSM FCB accessor functions */
		dsMacOSROMVersionTooOld = 120, /* The version of the "Mac OS ROM" file is too old to be used with the installed
