...
#pragma pack(2)
	...
	enum
	{
		...
		kNSLClientHandlesRecents =
			0x00000004 /* Stops NSLStandardGetURL from adding the selection to the recent items folder */
	};
	...
	{
		return (NSLURLFilterUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppNSLURLFilterProcInfo,
													 GetCurrentArchitecture());
	}
#else
#define NewNSLURLFilterUPP(userRoutine) \
	(NSLURLFilterUPP) NewRoutineDescriptor((ProcPtr)(userRoutine), uppNSLURLFilterProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define NewNSLEventUPP(userRoutine) \
	(NSLEventUPP) NewRoutineDescriptor((ProcPtr)(userRoutine), uppNSLEventProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define InvokeNSLURLFilterUPP(url, displayString, userUPP) \
	(Boolean) CALL_TWO_PARAMETER_UPP((userUPP), uppNSLURLFilterProcInfo, (url), (displayString))
#endif
#endif
	 ...
	 */
	EXTERN_API_C(void)
	InvokeNSLEventUPP(EventRecord *newEvent, void *userContext, NSLEventUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define CallNSLEventProc(userRoutine, newEvent, userContext)  InvokeNSLEventUPP(newEvent, userContext, userRoutine)
	...
	/* <--- function returns OSStatus of the operation.  noErr will be returned if valid, kNSLUserCanceled will be
	...
	*/
	 ...
	 */
	EXTERN_API(OSErr)
	NSLSaveURLAliasToFolder(OSType folderSelectorType, const char *url, const char *userFriendlyName);
