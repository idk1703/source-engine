...
#if defined(__MWERKS__) || defined(THINK_C) || defined(SYMANTEC_CPLUS) || defined(SYMANTEC_C) || defined(__xlc) || \
	defined(__xlC) || defined(__xlC__)
...
#define DisposeOTNotifyUPP(userUPP)
	 ...
	 */
	EXTERN_API_C(void)
	InvokeOTNotifyUPP(void *contextPtr, OTEventCode code, OTResult result, void *cookie, OTNotifyUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define OPT_NEXTHDR(theBuffer, theBufLen, prevOption)                                        \
	...
	*/
	...
	OTCreatePortRef(OTBusType busType, OTDeviceType devType, OTSlotNumber slot, UInt16 other);
...
#define OTNextLookupBuffer(buf)                                                                                    \
	((TLookupBuffer *)((char *)buf +                                                                               \
					   ((OTOffsetOf(TLookupBuffer, fAddressBuffer) + buf->fAddressLength + buf->fNameLength + 3) & \
						~3)))
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	InitOpenTransportInContext(OTInitializationFlags flags, OTClientContextPtr *outClientContext); /* can be NULL */
...
#define InvokeOTProcessUPP(arg, userUPP) (*userUPP)(arg)
	...
	*/
	 ...
	 */
	EXTERN_API(long)
	OTCreateDeferredTaskInContext(OTProcessUPP upp, void *arg, OTClientContextPtr clientContext); /* can be NULL */
...
/* ***** Interface to Providers ******/
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenProvider(OTConfigurationRef cfig, OTOpenFlags flags, OTNotifyUPP proc, void *contextPtr);
	 ...
	 */
	EXTERN_API(ProviderRef)
	OTTransferProviderOwnership(ProviderRef ref, OTClient prevOwner, OSStatus *errPtr);
	 ...
	 */
	EXTERN_API(OTResult)
	OTGetMessage(ProviderRef ref, strbuf *ctlbuf, strbuf *databuf, OTFlags *flagsPtr);
	 ...
	 */
	EXTERN_API(OTResult)
	OTGetPriorityMessage(ProviderRef ref, strbuf *ctlbuf, strbuf *databuf, OTBand *bandPtr, OTFlags *flagsPtr);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTPutMessage(ProviderRef ref, const strbuf *ctlbuf, const strbuf *databuf, OTFlags flags);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTPutPriorityMessage(ProviderRef ref, const strbuf *ctlbuf, const strbuf *databuf, OTBand band, OTFlags flags);
...
/* ***** Interface to Endpoints ******/
	...
	*/
	...
	OTOpenEndpointInContext(OTConfigurationRef config, OTOpenFlags oflag, TEndpointInfo *info, /* can be NULL */
							OSStatus *err, OTClientContextPtr clientContext);				   /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenEndpointInContext(OTConfigurationRef config, OTOpenFlags oflag, TEndpointInfo *info,	   /* can be NULL */
								 OTNotifyUPP upp, void *contextPtr, OTClientContextPtr clientContext); /* can be NULL */
	 ...
	 */
	EXTERN_API(EndpointRef)
	OTOpenEndpoint(OTConfigurationRef cfig, OTOpenFlags oflag, TEndpointInfo *info, /* can be NULL */
				   OSStatus *err);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenEndpoint(OTConfigurationRef cfig, OTOpenFlags oflag, TEndpointInfo *info, /* can be NULL */
...
#define OTAsyncOpenEndpoint(config, oflag, info, proc, contextPtr) \
	 ...
	 */
	EXTERN_API(OSStatus)
	OTResolveAddress(EndpointRef ref, TBind *reqAddr, TBind *retAddr, OTTimeout timeOut);
	...
	*/
	 ...
	 */
	EXTERN_API(void *)
	OTAllocInContext(EndpointRef ref, OTStructType structType, UInt32 fields, OSStatus *err,
					 OTClientContextPtr clientContext); /* can be NULL */
	 ...
	 */
	EXTERN_API(void *)
	OTAlloc(EndpointRef ref, OTStructType structType, OTFieldsType fields, OSStatus *err);
...
#define OTAlloc(ref, structType, fields, err) OTAllocInContext(ref, structType, fields, err, NULL)
	 ...
	 */
	EXTERN_API(OSStatus)
	OTCreateOptionString(const char *endPtName, TOption **opt, void *bufEnd, char *str, OTByteCount stringSize);
...
*/
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenMapperInContext(OTConfigurationRef config, OTOpenFlags oflag, OTNotifyUPP upp, void *contextPtr,
							   OTClientContextPtr clientContext); /* can be NULL */
	...
	OTOpenMapperInContext(OTConfigurationRef config, OTOpenFlags oflag, OSStatus *err,
						  OTClientContextPtr clientContext); /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncOpenMapper(OTConfigurationRef cfig, OTOpenFlags oflag, OTNotifyUPP proc, void *contextPtr);
...
#define OTOpenMapper(config, oflag, err) OTOpenMapperInContext(config, oflag, err, NULL)
	...
	*/
	 ...
	 */
	EXTERN_API_C(void *)
	OTAllocMemInContext(OTByteCount size, OTClientContextPtr clientContext); /* can be NULL */
...
#define OTAllocMem(s) OTAllocMemInContext(s, NULL)
	...
	*/
	...
	OTSubtractTimeStamps(OTTimeStamp *result, OTTimeStamp *startTime, OTTimeStamp *endEnd);
...
#define DisposeOTListSearchUPP(userUPP)
	 ...
	 */
	EXTERN_API_C(Boolean)
	InvokeOTListSearchUPP(const void *ref, OTLink *linkToCheck, OTListSearchUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#endif
	EXTERN_API_C(Boolean)
	OTAtomicSetBit(UInt8 *bytePtr, OTByteCount bitNumber) FOURWORDINLINE(0x01D0, 0x56C0, 0x7201, 0xC081);
...
#endif
	EXTERN_API_C(Boolean)
	OTAtomicClearBit(UInt8 *bytePtr, OTByteCount bitNumber) FOURWORDINLINE(0x0190, 0x56C0, 0x7201, 0xC081);
...
#endif
	EXTERN_API_C(Boolean)
	OTAtomicTestBit(UInt8 *bytePtr, OTByteCount bitNumber) FOURWORDINLINE(0x0110, 0x56C0, 0x7201, 0xC081);
...
#endif
	EXTERN_API_C(SInt32)
	OTAtomicAdd32(SInt32 toAdd, SInt32 *dest) SEVENWORDINLINE(0x2240, 0x2210, 0x2001, 0xD089, 0x0ED0, 0x0001, 0x66F4);
...
public:
	...
	// Allocating structures
	//
	void *AllocInContext(OTStructType structType, UInt32 fields, OSStatus *err = NULL,
