...
#pragma pack(2)
	...
	*/
	 ...
	 */
	EXTERN_API(long)
	GetScrap(Handle destination, ScrapFlavorType flavorType, SInt32 *offset) ONEWORDINLINE(0xA9FD);
	 ...
	 */
	EXTERN_API(OSStatus)
	PutScrap(SInt32 sourceBufferByteCount, ScrapFlavorType flavorType, const void *sourceBuffer) ONEWORDINLINE(0xA9FE);
...
#define kScrapFindScrap		 CFSTR("com.apple.scrap.find")
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	GetScrapFlavorFlags(ScrapRef scrap, ScrapFlavorType flavorType, ScrapFlavorFlags *flavorFlags);
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	GetScrapFlavorData(ScrapRef scrap, ScrapFlavorType flavorType, Size *byteCount, void *destination);
	...
	*/
	...
	/*
		...
		pointer value  actual data size    The data of size dataSize pointed to by dataPtr is added to the scrap.
			...
			ignored             0           A flavor with no data expected is placed on the scrap.  This is not a
	   promise.
	*/
	/*
	 *  PutScrapFlavor()
	 ...
	 */
	EXTERN_API(OSStatus)
	PutScrapFlavor(ScrapRef scrap, ScrapFlavorType flavorType, ScrapFlavorFlags flavorFlags, Size flavorSize,
	...
	*/
	...
	typedef CALLBACK_API(OSStatus, ScrapPromiseKeeperProcPtr)(ScrapRef scrap, ScrapFlavorType flavorType,
															  void *userData);
	...
	{
		return (ScrapPromiseKeeperUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppScrapPromiseKeeperProcInfo,
														   GetCurrentArchitecture());
	}
#else
#define NewScrapPromiseKeeperUPP(userRoutine) \
	(ScrapPromiseKeeperUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppScrapPromiseKeeperProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeScrapPromiseKeeperUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	...
	InvokeScrapPromiseKeeperUPP(ScrapRef scrap, ScrapFlavorType flavorType, void *userData,
								ScrapPromiseKeeperUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(OSStatus) InvokeScrapPromiseKeeperUPP(ScrapRef scrap, ScrapFlavorType flavorType,
...
#define InvokeScrapPromiseKeeperUPP(scrap, flavorType, userData, userUPP) \
	(OSStatus) CALL_THREE_PARAMETER_UPP((userUPP), uppScrapPromiseKeeperProcInfo, (scrap), (flavorType), (userData))
#endif
#endif
...
#define CallScrapPromiseKeeperProc(userRoutine, scrap, flavorType, userData) \
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	SetScrapPromiseKeeper(ScrapRef scrap, ScrapPromiseKeeperUPP upp, const void *userData);
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	GetScrapFlavorInfoList(ScrapRef scrap, UInt32 *infoCount, ScrapFlavorInfo info[]);
