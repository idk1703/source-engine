...
#define FirstCFragResourceExtensionPtr(aMemberPtr)           \
	((CFragResourceExtensionHeaderPtr)((BytePtr)aMemberPtr + \
									   AlignToFour(kBaseCFragResourceMemberSize + aMemberPtr->name[0] + 1)))
...
#define CFragHasFileLocation(where) (((where) == kDataForkCFragLocator) || ((where) == kResourceCFragLocator))
	 ...
	 */
	EXTERN_API(OSErr)
	GetSharedLibrary(ConstStr63Param libName, CFragArchitecture archType, CFragLoadOptions options,
	 ...
	 */
	EXTERN_API(OSErr)
	GetDiskFragment(const FSSpec *fileSpec, UInt32 offset, UInt32 length, ConstStr63Param fragName, /* can be NULL */
					CFragLoadOptions options, CFragConnectionID *connID,							/* can be NULL */
	 ...
	 */
	EXTERN_API(OSErr)
	GetMemFragment(void *memAddr, UInt32 length, ConstStr63Param fragName, /* can be NULL */
				   CFragLoadOptions options, CFragConnectionID *connID,	   /* can be NULL */
				   Ptr *mainAddr,										   /* can be NULL */
	 ...
	 */
	EXTERN_API(OSErr)
	FindSymbol(CFragConnectionID connID, ConstStr255Param symName, Ptr *symAddr, /* can be NULL */
	 ...
	 */
	EXTERN_API(OSErr)
	CountSymbols(CFragConnectionID connID, long *symCount) THREEWORDINLINE(0x3F3C, 0x0006, 0xAA5A);
	 ...
	 */
	EXTERN_API(OSErr)
	GetIndSymbol(CFragConnectionID connID, long symIndex, Str255 symName, /* can be NULL */
