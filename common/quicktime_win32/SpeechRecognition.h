...
#pragma pack(2)
	...
	enum
	{
		...
		kSRCantSetDuringRecognition =
			-5113, /* the property can't be set while recognition is in progress -- do before or between utterances. */
		...
		kSROtherRecAlreadyModal = -5121,  /* another recognizer is modal at the moment, so can't set this recognizer's
											 kSRBlockModally property right now */
		kSRHasNoSubItems = -5122, /* SRCountItems or related routine was called on an object without subelements -- e.g.
									 a word -- rather than phrase, path, or LM. */
		...
		kSRCantAdd =
			-5131, /* Can't add given type of object to the base SRLanguageObject (e.g.in SRAddLanguageObject)   */
		kSRSndInSourceDisconnected = -5132, /* Sound input source is disconnected */
		kSRCantReadLanguageObject =
			-5133, /* An error while trying to create new Language object from file or pointer -- possibly bad format */
	...
	{
		return (SRCallBackUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSRCallBackProcInfo,
...
#define CallSRCallBackProc(userRoutine, param) InvokeSRCallBackUPP(param, userRoutine)
	...
	enum
	{
		kSRFeedbackAndListeningModes =
			FOUR_CHAR_CODE('fbwn'), /* short: one of kSRNoFeedbackHasListenModes, kSRHasFeedbackHasListenModes,
									   kSRNoFeedbackNoListenModes */
		kSRRejectedWord = FOUR_CHAR_CODE('rejq'),		/* the SRWord used to represent a rejection */
		kSRCleanupOnClientExit = FOUR_CHAR_CODE('clup') /* Boolean: Default is true. The rec system and everything it
	...
	{
		kSRNoFeedbackNoListenModes =
			0, /* next allocated recognizer has no feedback window and doesn't use listening modes   */
		...
		kSRNoFeedbackHasListenModes =
			2 /* next allocated recognizer has no feedback window but does use listening modes  */
	};
	...
	enum
	{
		kSRNotifyRecognitionBeginning =
			1L << 0, /* recognition can begin. client must now call SRContinueRecognition or SRCancelRecognition */
	...
	enum
	{
		...
		kSRForegroundOnly = FOUR_CHAR_CODE(
			'fgon'), /* Boolean. Default is true. If true, client recognizer only active when in foreground.   */
		kSRBlockBackground = FOUR_CHAR_CODE('blbg'), /* Boolean. Default is false. If true, when client recognizer in
														foreground, rest of LMs are inactive.    */
		kSRBlockModally =
			FOUR_CHAR_CODE('blmd'), /* Boolean. Default is false. When true, this client's LM is only active LM; all
									   other LMs are inactive. Be nice, don't be modal for long periods! */
		kSRWantsResultTextDrawn = FOUR_CHAR_CODE(
			'txfb'), /* Boolean. Default is true. If true, search results are posted to Feedback window */
		kSRWantsAutoFBGestures = FOUR_CHAR_CODE('dfbr'), /* Boolean. Default is true. If true, client needn't call
															SRProcessBegin/End to get default feedback behavior */
		kSRSoundInVolume =
			FOUR_CHAR_CODE('volu'), /* short in [0..100] log scaled sound input power. Can't set this property */
		kSRReadAudioFSSpec = FOUR_CHAR_CODE('aurd'),	 /* *FSSpec. Specify FSSpec where raw audio is to be read (AIFF
															format) using kSRCanned22kHzSpeechSource. Reads until EOF */
		kSRCancelOnSoundOut = FOUR_CHAR_CODE('caso'),	 /* Boolean: Default is true.  If any sound is played out during
	...
	enum
	{
		...
		kSRListenKeyCombo =
			FOUR_CHAR_CODE('lkey'), /* short: Push-To-Talk key combination; high byte is high byte of event->modifiers,
									   the low byte is the keycode from event->message */
		...
		kSRKeyExpected = FOUR_CHAR_CODE(
			'kexp') /* Boolean: Must the PTT key be depressed or the key word spoken before recognition can occur? */
	};
	...
	enum
	{
		...
		kSRMustCancelSearch =
			1L << 3, /* something has occurred (sound played, non-speech detected) requiring the search to abort */
	...
	{
		kSRSpelling =
			FOUR_CHAR_CODE('spel'), /* spelling of a SRWord or SRPhrase or SRPath, or name of a SRLanguageModel */
		...
		kSRRejectable = FOUR_CHAR_CODE(
			'rjbl'), /* Boolean -- true if SRLanguageObject is rejectable (Recognition System's kSRRejectedWord */
	 ...
	 */
	EXTERN_API(OSErr)
	SROpenRecognitionSystem(SRRecognitionSystem *system, OSType systemID) THREEWORDINLINE(0x303C, 0x0400, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRSetProperty(SRSpeechObject srObject, OSType selector, const void *property, Size propertyLen)
	 ...
	 */
	EXTERN_API(OSErr)
	SRGetProperty(SRSpeechObject srObject, OSType selector, void *property, Size *propertyLen)
	 ...
	 */
	EXTERN_API(OSErr)
	SRGetReference(SRSpeechObject srObject, SRSpeechObject *newObjectRef) THREEWORDINLINE(0x303C, 0x0425, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRNewRecognizer(SRRecognitionSystem system, SRRecognizer *recognizer, OSType sourceID)
	 ...
	 */
	EXTERN_API(OSErr)
	SRSetLanguageModel(SRRecognizer recognizer, SRLanguageModel languageModel) THREEWORDINLINE(0x303C, 0x040E, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRGetLanguageModel(SRRecognizer recognizer, SRLanguageModel *languageModel) THREEWORDINLINE(0x303C, 0x040F, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRNewLanguageModel(SRRecognitionSystem system, SRLanguageModel *model, const void *name, Size nameLength)
	 ...
	 */
	EXTERN_API(OSErr)
	SRNewPath(SRRecognitionSystem system, SRPath *path) THREEWORDINLINE(0x303C, 0x0413, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRNewPhrase(SRRecognitionSystem system, SRPhrase *phrase, const void *text, Size textLength)
	 ...
	 */
	EXTERN_API(OSErr)
	SRNewWord(SRRecognitionSystem system, SRWord *word, const void *text, Size textLength)
	 ...
	 */
	EXTERN_API(OSErr)
	SRPutLanguageObjectIntoHandle(SRLanguageObject languageObject, Handle lobjHandle)
	 ...
	 */
	EXTERN_API(OSErr)
	SRNewLanguageObjectFromHandle(SRRecognitionSystem system, SRLanguageObject *languageObject, Handle lObjHandle)
	 ...
	 */
	EXTERN_API(OSErr)
	SRNewLanguageObjectFromDataFile(SRRecognitionSystem system, SRLanguageObject *languageObject, short fRefNum)
	 ...
	 */
	EXTERN_API(OSErr)
	SRChangeLanguageObject(SRLanguageObject languageObject, const void *text, Size textLength)
	 ...
	 */
	EXTERN_API(OSErr)
	SRAddLanguageObject(SRLanguageObject base, SRLanguageObject addon) THREEWORDINLINE(0x303C, 0x041A, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRRemoveLanguageObject(SRLanguageObject base, SRLanguageObject toRemove) THREEWORDINLINE(0x303C, 0x041C, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRCountItems(SRSpeechObject container, long *count) THREEWORDINLINE(0x303C, 0x0405, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRSetIndexedItem(SRSpeechObject container, SRSpeechObject item, long index) THREEWORDINLINE(0x303C, 0x0607, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRRemoveIndexedItem(SRSpeechObject container, long index) THREEWORDINLINE(0x303C, 0x0408, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRDrawText(SRRecognizer recognizer, const void *dispText, Size dispLength) THREEWORDINLINE(0x303C, 0x0621, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRDrawRecognizedText(SRRecognizer recognizer, const void *dispText, Size dispLength)
	 ...
	 */
	EXTERN_API(OSErr)
	SRProcessBegin(SRRecognizer recognizer, Boolean failed) THREEWORDINLINE(0x303C, 0x031D, 0xAA56);
	 ...
	 */
	EXTERN_API(OSErr)
	SRProcessEnd(SRRecognizer recognizer, Boolean failed) THREEWORDINLINE(0x303C, 0x031E, 0xAA56);
