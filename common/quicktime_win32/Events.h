...
#define CallFKEYProc(userRoutine) InvokeFKEYUPP(userRoutine)
	 ...
	 */
	EXTERN_API(UInt32)
	KeyTranslate(const void *transData, UInt16 keycode, UInt32 *state) ONEWORDINLINE(0xA9C3);
...
typedef BigEndianLong KeyMap[4];
	...
	{
		return (GetNextEventFilterUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppGetNextEventFilterProcInfo,
														   GetCurrentArchitecture());
	}
#else
#define NewGetNextEventFilterUPP(userRoutine) \
	(GetNextEventFilterUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppGetNextEventFilterProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeGetNextEventFilterUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(void)
	InvokeGetNextEventFilterUPP(EventRecord *theEvent, Boolean *result, GetNextEventFilterUPP userUPP);
...
#endif
	EXTERN_API(OSErr)
	PPostEvent(EventKind eventCode, UInt32 eventMsg, EvQElPtr *qEl) TWOWORDINLINE(0xA12F, 0x2288);
...
#endif
	EXTERN_API(Boolean)
	OSEventAvail(EventMask mask, EventRecord *theEvent) TWOWORDINLINE(0xA030, 0x5240);
...
#pragma parameter __D0 GetOSEvent(__D0, __A0)
	 ...
	 */
	EXTERN_API(void)
	FlushEvents(EventMask whichMask, EventMask stopMask) TWOWORDINLINE(0x201F, 0xA032);
	 ...
	 */
	EXTERN_API(void)
	SystemClick(const EventRecord *theEvent, WindowRef theWindow) ONEWORDINLINE(0xA9B3);
...
#define KeyTrans(transData, keycode, state) KeyTranslate(transData, keycode, state)
	...
	*/
	 ...
	 */
	EXTERN_API(Boolean)
	IsCmdChar(const EventRecord *event, short test) FOURWORDINLINE(0x2F3C, 0x8206, 0xFFD0, 0xA8B5);
