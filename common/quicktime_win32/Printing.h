...
#define InvokePrIdleUPP(userUPP) CALL_ZERO_PARAMETER_UPP((userUPP), uppPrIdleProcInfo)
	 ...
	 */
	EXTERN_API_C(void)
	InvokePItemUPP(DialogPtr theDialog, short item, PItemUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define CallPDlgInitProc(userRoutine, hPrint) InvokePDlgInitUPP(hPrint, userRoutine)
	 ...
	 */
	EXTERN_API(void)
	PrJobMerge(THPrint hPrintSrc, THPrint hPrintDst) FOURWORDINLINE(0x2F3C, 0x5804, 0x089C, 0xA8FD);
	 ...
	 */
	EXTERN_API(Boolean)
	PrDlgMain(THPrint hPrint, PDlgInitUPP pDlgInit) FOURWORDINLINE(0x2F3C, 0x4A04, 0x0894, 0xA8FD);
	 ...
	 */
	EXTERN_API(void)
	PrOpenPage(TPPrPort pPrPort, TPRect pPageFrame) FOURWORDINLINE(0x2F3C, 0x1000, 0x0808, 0xA8FD);
	 ...
	 */
	EXTERN_API(void)
	PrPicFile(THPrint hPrint, TPPrPort pPrPort, Ptr pIOBuf, Ptr pDevBuf, TPPrStatus prStatus)
	 ...
	 */
	EXTERN_API(void)
	PrCtlCall(short iWhichCtl, long lParam1, long lParam2, long lParam3) FOURWORDINLINE(0x2F3C, 0xA000, 0x0E00, 0xA8FD);
