...
#pragma pack(2)
	 ...
	 *
	 *  Enter:   appVRefNumHint      volume where application resides (can be wrong, and if is, will be used as a
	 * starting point) appSignature        signature (creator) of application nativeTypes         pointer to a buffer to
	 * be filled with up to 64 FileTypes
	 *
	 ...
	 */
	EXTERN_API(OSErr)
	GetFileTypesThatAppCanNativelyOpen(short appVRefNumHint, OSType appSignature, FileType *nativeTypes)
	 ...
	 */
	EXTERN_API(OSErr)
	ExtendFileTypeList(const FileType *originalTypeList, short numberOriginalTypes, FileType *extendedTypeList,
	 ...
	 *           appVRefNumHint      vRefNum of application to open doc ( can be wrong, and if is, will be used as a
	 * starting point) appSignature        signature (creator) of application to open doc nativeTypes         zero
	 * terminated list of FileTypes app can open natively, or NULL to use default list onlyNative          whether to
	 * consider if document can be translated before opening howToOpen           pointer to buffer in which to put how
	 * the document can be opened howToTranslate      pointer to buffer in which to put a FileTranslationSpec record
	 *
	 ...
	 */
	EXTERN_API(OSErr)
	CanDocBeOpened(const FSSpec *targetDocument, short appVRefNumHint, OSType appSignature, const FileType *nativeTypes,
				   Boolean onlyNative, DocOpenMethod *howToOpen, FileTranslationSpec *howToTranslate)
	 ...
	 */
	EXTERN_API(short)
	GetFileTranslationPaths(const FSSpec *srcDocument, FileType dstDocType, unsigned short maxResultCount,
	 ...
	 */
	EXTERN_API(OSErr)
	GetPathFromTranslationDialog(const FSSpec *theDocument, const FSSpec *theApplication, TypesBlockPtr typeList,
								 DocOpenMethod *howToOpen, FileTranslationSpec *howToTranslate)
	 ...
	 */
	EXTERN_API(OSErr)
	GetDocumentKindString(short docVRefNum, OSType docType, OSType docCreator, Str63 kindString)
	 ...
	 */
	EXTERN_API(OSErr)
	GetTranslationExtensionName(const FileTranslationSpec *translationMethod, Str31 extensionName)
	 ...
	 */
	typedef CALLBACK_API(OSErr, GetScrapDataProcPtr)(ScrapType requestedFormat, Handle dataH,
	...
	{
		return (GetScrapDataUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppGetScrapDataProcInfo,
...
#define DisposeGetScrapDataUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(OSErr)
	InvokeGetScrapDataUPP(ScrapType requestedFormat, Handle dataH, void *srcDataGetterRefCon, GetScrapDataUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(OSErr) InvokeGetScrapDataUPP(ScrapType requestedFormat, Handle dataH, void *srcDataGetterRefCon,
													 GetScrapDataUPP userUPP)
	{
		return (OSErr)CALL_THREE_PARAMETER_UPP(userUPP, uppGetScrapDataProcInfo, requestedFormat, dataH,
											   srcDataGetterRefCon);
	}
#else
#define InvokeGetScrapDataUPP(requestedFormat, dataH, srcDataGetterRefCon, userUPP)                  \
	(OSErr) CALL_THREE_PARAMETER_UPP((userUPP), uppGetScrapDataProcInfo, (requestedFormat), (dataH), \
									 (srcDataGetterRefCon))
#endif
#endif
...
#define CallGetScrapDataProc(userRoutine, requestedFormat, dataH, srcDataGetterRefCon) \
	InvokeGetScrapDataUPP(requestedFormat, dataH, srcDataGetterRefCon, userRoutine)
	 ...
	 */
	EXTERN_API(OSErr)
	TranslateScrap(GetScrapDataUPP sourceDataGetter, void *sourceDataGetterRefCon, ScrapType destinationFormat,
				   Handle destinationData, short progressDialogID) TWOWORDINLINE(0x700E, 0xABFC);
