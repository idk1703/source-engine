...
#pragma pack(2)
	...
	enum
	{
		...
		kAppleSysexCmdDeveloper =
			0x7F00 /* F0 11 7F 00 ww xx yy zz ... F7 is available for non-Apple developers, where wxyz is unique app
					  signature with 8th bit cleared, unique to developer, and 00 and 7f are reserved */
	};
	...
	typedef CALLBACK_API(ComponentResult, MusicMIDISendProcPtr)(ComponentInstance self, long refCon,
																MusicMIDIPacket *mmp);
	...
	*/
	...
	QTMIDIGetMIDIPorts(QTMIDIComponent ci, QTMIDIPortListHandle *inputPorts, QTMIDIPortListHandle *outputPorts)
	...
	enum
	{
		...
		kSynthesizerDynamicChannel = 1 << 7, /* can move any part to any channel or disable each part. (else we assume
												it lives on all channels in masks) */
		kSynthesizerHogsSystemChannel =
			1 << 8, /* can be channelwise dynamic, but always responds on its system channel */
		kSynthesizerHasSystemChannel = 1 << 9, /* has some "system channel" notion to distinguish it from multiple
												  instances of the same device (GM devices dont) */
		kSynthesizerSlowSetPart =
			1 << 10, /* SetPart() and SetPartInstrumentNumber() calls do not have rapid response, may glitch notes */
		...
		kSynthesizerSoundLocalization = 1 << 16 /* synth supports extremely baroque, nonstandard, and proprietary "apple
												   game sprockets" localization parameter set */
	};
	...
	enum
	{
		...
		kControllerPitchBend =
			32, /* positive & negative semitones, with 8 bits fraction, same units as transpose controllers*/
		...
		kControllerTuneVolume = 43,	   /* another volume control, used for "song global" volume - since we share one
										  synthesizer across multiple tuneplayers*/
		...
		kControllerEditPart =
			113, /* last 16 controllers 113-128 and above are global controllers which respond on part zero */
	...
	enum
	{
		...
		kFirstDrumkit = 0x00004000, /* (first value is "no drum". instrument numbers from 16384->16384+128 are drumkits,
									   and for GM they are _defined_ drumkits! */
	...
	*/
	enum
	{
		...
		kSetAtomicInstDontPreprocess = 1 << 7	 /* perform no further preprocessing because either 1)you know the
													instrument is digitally clean, or 2) you got it from a GetPartAtomic*/
	};
	...
	enum
	{
		...
		kGenericMusicResModifiableInstrumentHW =
			9, /* Shrt same as the hw shorts trailing the instrument names, a shortlist */
		kGenericMusicResGMTranslation =
			10, /* Long 128 long entries, 1 for each gm inst, of local instrument numbers 1-n (not hw numbers) */
	...
	{
		kGenericMusicDoMIDI =
			1 << 0, /* implement normal MIDI messages for note, controllers, and program changes 0-127 */
		...
		kGenericMusicCallNumber = 1 << 7, /* call MusicDerivedSetPartInstrumentNumber for MusicSetPartInstrumentNumber
											 calls, & don't send any C0 or bank stuff */
		kGenericMusicCallROMInstrument = 1 << 8, /* call MusicSetInstrument for MusicSetPartInstrumentNumber for "ROM"
													instruments, passing params from the ROMi resource */
		kGenericMusicAllDefaults = 1 << 9 /* indicates that when a new instrument is recalled, all knobs are reset to
											 DEFAULT settings. True for GS modules */
	};
...
#define MusicSetInstrumentNumber(ci, part, instrumentNumber) MusicSetPartInstrumentNumber(ci, part, instrumentNumber)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetPartInstrumentNumber(MusicComponent mc, long part) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0005, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetPartAtomicInstrument(MusicComponent mc, long part, AtomicInstrument *ai, long flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicSetPartAtomicInstrument(MusicComponent mc, long part, AtomicInstrumentPtr aiP, long flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetPartKnob(MusicComponent mc, long part, long knobID) FIVEWORDINLINE(0x2F3C, 0x0008, 0x0010, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetKnob(MusicComponent mc, long knobID) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0012, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicSetKnob(MusicComponent mc, long knobID, long knobValue) FIVEWORDINLINE(0x2F3C, 0x0008, 0x0013, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicFindTone(MusicComponent mc, ToneDescription *td, long *libraryIndexOut, unsigned long *fit)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicResetPart(MusicComponent mc, long part) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0018, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicSetPartController(MusicComponent mc, long part, MusicController controllerNumber, long controllerValue)
...
#define MusicSetController(ci, part, controllerNumber, controllerValue) \
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetPartController(MusicComponent mc, long part, MusicController controllerNumber)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetInstrumentNames(MusicComponent mc, long modifiableInstruments, Handle *instrumentNames,
							Handle *instrumentCategoryLasts, Handle *instrumentCategoryNames)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetDrumNames(MusicComponent mc, long modifiableInstruments, Handle *instrumentNumbers, Handle *instrumentNames)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicSetMasterTune(MusicComponent mc, long masterTune) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0020, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetKnobSettingStrings(MusicComponent mc, long knobIndex, long isGlobal, Handle *settingsNames,
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetMIDIPorts(MusicComponent mc, long *inputPortCount, long *outputPortCount)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicStartOffline(MusicComponent mc, unsigned long *numChannels, UnsignedFixed *sampleRate,
					  unsigned short *sampleSize, MusicOfflineDataUPP dataProc, long dataProcRefCon)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicSetOfflineTimeTo(MusicComponent mc, long newTimeStamp) FIVEWORDINLINE(0x2F3C, 0x0004, 0x002A, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetInstrumentKnobDescription(MusicComponent mc, long knobIndex, KnobDescription *mkd)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetDrumKnobDescription(MusicComponent mc, long knobIndex, KnobDescription *mkd)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetInfoText(MusicComponent mc, long selector, Handle *textH, Handle *styleH)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGetInstrumentInfo(MusicComponent mc, long getInstrumentInfoFlags, InstrumentInfoListHandle *infoListH)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicGenericGetKnobList(MusicComponent mc, long knobType, GenericKnobDescriptionListHandle *gkdlH)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicDerivedSetKnob(MusicComponent mc, long knobType, long knobNumber, long knobValue, long partNumber, GCPart *p,
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicDerivedSetPartInstrumentNumber(MusicComponent mc, long partNumber, GCPart *p)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicDerivedSetMIDI(MusicComponent mc, MusicMIDISendUPP midiProc, long refcon, long midiChannel)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicDerivedStorePartInstrument(MusicComponent mc, long partNumber, GCPart *p, long instrumentNumber)
	 ...
	 */
	EXTERN_API(ComponentResult)
	MusicDerivedCloseResFile(MusicComponent mc, short resRefNum) FIVEWORDINLINE(0x2F3C, 0x0002, 0x0208, 0x7000, 0xA82A);
	...
	*/
	...
	struct NoteRequestInfo
	{
		...
		NoteRequestMIDIChannel midiChannelAssignment; /* (kNoteRequestSpecifyMIDIChannel | 1->16) as MIDI Channel
														 assignement or zero - see notes above  */
	 ...
	 */
	EXTERN_API(ComponentResult)
	NARegisterMusicDevice(NoteAllocator na, OSType synthType, Str31 name, SynthesizerConnections *connections)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAUnregisterMusicDevice(NoteAllocator na, long index) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0001, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAGetRegisteredMusicDevice(NoteAllocator na, long index, OSType *synthType, Str31 name,
							   SynthesizerConnections *connections, MusicComponent *mc)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NANewNoteChannel(NoteAllocator na, NoteRequest *noteRequest, NoteChannel *outChannel)
		FIVEWORDINLINE(0x2F3C, 0x0008, 0x0004, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAGetNoteChannelInfo(NoteAllocator na, NoteChannel noteChannel, long *index, long *part)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NASetController(NoteAllocator na, NoteChannel noteChannel, long controllerNumber, long controllerValue)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NASetKnob(NoteAllocator na, NoteChannel noteChannel, long knobNumber, long knobValue)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAFindNoteChannelTone(NoteAllocator na, NoteChannel noteChannel, ToneDescription *td, long *instrumentNumber)
...
#define NASetInstrument(ci, noteChannel, instrumentNumber) NASetInstrumentNumber(ci, noteChannel, instrumentNumber)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAPickInstrument(NoteAllocator na, ModalFilterUPP filterProc, StringPtr prompt, ToneDescription *sd,
					 unsigned long flags, long refCon, long reserved1, long reserved2)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAPickArrangement(NoteAllocator na, ModalFilterUPP filterProc, StringPtr prompt, long zero1, long zero2, Track t,
	 ...
	 */
	EXTERN_API(ComponentResult)
	NACopyrightDialog(NoteAllocator na, PicHandle p, StringPtr author, StringPtr copyright, StringPtr other,
					  StringPtr title, ModalFilterUPP filterProc, long refCon)
	...
	*/
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAGetMIDIPorts(NoteAllocator na, QTMIDIPortListHandle *inputPorts, QTMIDIPortListHandle *outputPorts)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAPickEditInstrument(NoteAllocator na, ModalFilterUPP filterProc, StringPtr prompt, long refCon, NoteChannel nc,
						 AtomicInstrument ai, long flags) FIVEWORDINLINE(0x2F3C, 0x0018, 0x0024, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	NANewNoteChannelFromAtomicInstrument(NoteAllocator na, AtomicInstrumentPtr instrument, long flags,
	 ...
	 */
	EXTERN_API(ComponentResult)
	NASetAtomicInstrument(NoteAllocator na, NoteChannel noteChannel, AtomicInstrumentPtr instrument, long flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAGetKnob(NoteAllocator na, NoteChannel noteChannel, long knobNumber, long *knobValue)
	 ...
	 */
	EXTERN_API(ComponentResult)
	NASetNoteChannelSoundLocalization(NoteAllocator na, NoteChannel noteChannel, Handle data)
		FIVEWORDINLINE(0x2F3C, 0x0008, 0x002C, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	NAGetController(NoteAllocator na, NoteChannel noteChannel, long controllerNumber, long *controllerValue)
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetHeader(TunePlayer tp, unsigned long *header) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0004, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneGetTimeBase(TunePlayer tp, TimeBase *tb) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0005, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetTimeScale(TunePlayer tp, TimeScale scale) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0006, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneGetTimeScale(TunePlayer tp, TimeScale *scale) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0007, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneQueue(TunePlayer tp, unsigned long *tune, Fixed tuneRate, unsigned long tuneStartPosition,
			  unsigned long tuneStopPosition, unsigned long queueFlags, TuneCallBackUPP callBackProc, long refCon)
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneGetStatus(TunePlayer tp, TuneStatus *status) FIVEWORDINLINE(0x2F3C, 0x0004, 0x000C, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneStop(TunePlayer tp, long stopFlags) FIVEWORDINLINE(0x2F3C, 0x0004, 0x000D, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetVolume(TunePlayer tp, Fixed volume) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0010, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetNoteChannels(TunePlayer tp, unsigned long count, NoteChannel *noteChannelList,
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetPartTranspose(TunePlayer tp, unsigned long part, long transpose, long velocityShift)
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetSofter(TunePlayer tp, long softer) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0018, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetBalance(TunePlayer tp, long balance) FIVEWORDINLINE(0x2F3C, 0x0004, 0x001A, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetSoundLocalization(TunePlayer tp, Handle data) FIVEWORDINLINE(0x2F3C, 0x0004, 0x001B, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneSetPartMix(TunePlayer tp, unsigned long partNumber, long volume, long balance, long mixFlags)
		FIVEWORDINLINE(0x2F3C, 0x0010, 0x001D, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	TuneGetPartMix(TunePlayer tp, unsigned long partNumber, long *volumeOut, long *balanceOut, long *mixFlagsOut)
...
#define qtma_XControlController(m, l) \
	(qtma_EXT(l, kXControlEventControllerFieldPos, kXControlEventControllerFieldWidth))
...
#define qtma_KnobValue(m, l)                                                           \
	((qtma_EXT(m, kKnobEventValueHighFieldPos, kKnobEventValueHighFieldWidth) << 16) | \
...
#define qtma_StuffRestEvent(x, duration)                                                            \
	(x = (kRestEventType << kEventTypeFieldPos) | ((long)(duration) << kRestEventDurationFieldPos), \
	 x = EndianU32_NtoB(x))
#define qtma_StuffNoteEvent(x, part, pitch, volume, duration)                            \
	(x = (kNoteEventType << kEventTypeFieldPos) | ((long)(part) << kEventPartFieldPos) | \
...
#define qtma_StuffControlEvent(x, part, control, value)                                           \
	(x = (kControlEventType << kEventTypeFieldPos) | ((long)(part) << kEventPartFieldPos) |       \
...
#define qtma_StuffMarkerEvent(x, markerType, markerValue)                                                 \
	(x = (kMarkerEventType << kEventTypeFieldPos) | ((long)(markerType) << kMarkerEventSubtypeFieldPos) | \
		 ((long)(markerValue) << kMarkerEventValueFieldPos),                                              \
	 x = EndianU32_NtoB(x))
#define qtma_StuffXNoteEvent(w1, w2, part, pitch, volume, duration)                                         \
	(w1 = (kXNoteEventType << kXEventTypeFieldPos) | ((long)(part) << kXEventPartFieldPos) |                \
		  ((long)(pitch) << kXNoteEventPitchFieldPos),                                                      \
	 w1 = EndianU32_NtoB(w1),                                                                               \
	 w2 = (kXEventLengthBits << kEventLengthFieldPos) | ((long)(duration) << kXNoteEventDurationFieldPos) | \
		  ((long)(volume) << kXNoteEventVolumeFieldPos),                                                    \
	 w2 = EndianU32_NtoB(w2))
#define qtma_StuffXControlEvent(w1, w2, part, control, value)                                                  \
	(w1 = (kXControlEventType << kXEventTypeFieldPos) | ((long)(part) << kXEventPartFieldPos) |                \
		  ((long)((value)&qtma_MASK(kXControlEventValueFieldWidth)) << kXControlEventValueFieldPos),           \
	 w1 = EndianU32_NtoB(w1),                                                                                  \
	 w2 = (kXEventLengthBits << kEventLengthFieldPos) | ((long)(control) << kXControlEventControllerFieldPos), \
	 w2 = EndianU32_NtoB(w2))
#define qtma_StuffKnobEvent(w1, w2, part, knob, value)                                             \
	(w1 = (kKnobEventType << kXEventTypeFieldPos) | ((long)(part) << kXEventPartFieldPos) |        \
		  ((long)(value >> 16) << kKnobEventValueLowFieldPos),                                     \
	 w1 = EndianU32_NtoB(w1),                                                                      \
	 w2 = (kXEventLengthBits << kEventLengthFieldPos) | ((long)(knob) << kKnobEventKnobFieldPos) | \
		  ((long)(value & 0xFFFF) << kKnobEventValueLowFieldPos),                                  \
	 w2 = EndianU32_NtoB(w2))
#define qtma_StuffGeneralEvent(w1, w2, part, subType, length)                                                     \
	(w1 = (kGeneralEventType << kXEventTypeFieldPos) | ((long)(part) << kXEventPartFieldPos) |                    \
		  ((long)(length) << kGeneralEventLengthFieldPos),                                                        \
	 w1 = EndianU32_NtoB(w1),                                                                                     \
	 w2 = (kGeneralEventLengthBits << kEventLengthFieldPos) | ((long)(subType) << kGeneralEventSubtypeFieldPos) | \
		  ((long)(length) << kGeneralEventLengthFieldPos),                                                        \
	 w2 = EndianU32_NtoB(w2))
#define qtma_NeedXGeneralEvent(length) (((unsigned long)(length)) > (unsigned long)0xffff)
	...
	enum
	{
		...
		kGeneralEventTuneDifference = 5, /* Contains a standard sequence, with end marker, for the tune difference of a
											sequence piece (halts QuickTime 2.0 Music) */
		...
		kGeneralEventMIDIChannel =
			8, /* used in tune header, one longword identifies the midi channel it originally came from */
		kGeneralEventPartChange =
			9, /* used in tune sequence, one longword identifies the tune part which can now take over this part's note
		...
		kGeneralEventUsedNotes =
			11,					  /* four longwords specifying which midi notes are actually used, 0..127 msb to lsb */
	...
	enum
	{
		...
		kMarkerEventTempo = 2 /* value same as beat marker, but indicates that a tempo event should be computed (based
								 on where the next beat or tempo marker is) and emitted upon export*/
	};
	...
	{
		return (MusicMIDISendUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppMusicMIDISendProcInfo,
...
#define NewMusicMIDISendUPP(userRoutine) \
	...
	{
		return (MusicOfflineDataUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppMusicOfflineDataProcInfo,
														 GetCurrentArchitecture());
	}
#else
#define NewMusicOfflineDataUPP(userRoutine) \
	(MusicOfflineDataUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppMusicOfflineDataProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define NewTuneCallBackUPP(userRoutine) \
	...
	{
		return (TunePlayCallBackUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppTunePlayCallBackProcInfo,
														 GetCurrentArchitecture());
	}
#else
#define NewTunePlayCallBackUPP(userRoutine) \
	(TunePlayCallBackUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppTunePlayCallBackProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeTunePlayCallBackUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	...
	InvokeMusicMIDISendUPP(ComponentInstance self, long refCon, MusicMIDIPacket *mmp, MusicMIDISendUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define InvokeMusicMIDISendUPP(self, refCon, mmp, userUPP) \
	(ComponentResult) CALL_THREE_PARAMETER_UPP((userUPP), uppMusicMIDISendProcInfo, (self), (refCon), (mmp))
#endif
#endif
	 ...
	 */
	EXTERN_API_C(ComponentResult)
	InvokeMusicOfflineDataUPP(Ptr SoundData, long numBytes, long myRefCon, MusicOfflineDataUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(ComponentResult)
		InvokeMusicOfflineDataUPP(Ptr SoundData, long numBytes, long myRefCon, MusicOfflineDataUPP userUPP)
	{
		return (ComponentResult)CALL_THREE_PARAMETER_UPP(userUPP, uppMusicOfflineDataProcInfo, SoundData, numBytes,
														 myRefCon);
	}
#else
#define InvokeMusicOfflineDataUPP(SoundData, numBytes, myRefCon, userUPP) \
	(ComponentResult)                                                     \
		CALL_THREE_PARAMETER_UPP((userUPP), uppMusicOfflineDataProcInfo, (SoundData), (numBytes), (myRefCon))
#endif
#endif
	 ...
	 */
	EXTERN_API_C(void)
	InvokeTuneCallBackUPP(const TuneStatus *status, long refCon, TuneCallBackUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define InvokeTuneCallBackUPP(status, refCon, userUPP) \
	 ...
	 */
	EXTERN_API_C(void)
	InvokeTunePlayCallBackUPP(unsigned long *event, long seed, long refCon, TunePlayCallBackUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
