...
#pragma pack(2)
	...
	*/
	...
	enum
	{
		...
		hmMatchInTitleBit = 4,
		hmUseSubIDMask =
			(1 << hmUseSubIDBit), /* treat resID's in resources as subID's of driver base ID (for Desk Accessories) */
		hmAbsoluteCoordsMask = (1 << hmAbsoluteCoordsBit), /* ignore window port origin and treat rectangles as absolute
															  coords (local to window) */
		hmSaveBitsNoWindowMask = (1 << hmSaveBitsNoWindowBit), /* don't create a window, just blast bits on screen. No
																  update event is generated */
		hmSaveBitsWindowMask = (1 << hmSaveBitsWindowBit),	   /* create a window, but restore bits behind window when
																  window goes away & generate update event */
		hmMatchInTitleMask =
			(1 << hmMatchInTitleBit) /* for hwin resources, match string anywhere in window title string */
	};
	...
	enum
	{
		...
		kHMNamedResourceItem =
			1024,				/* Use pstring in menu item to get 'STR#', 'PICT', or 'STR ' resource ('hmnu' only) */
	...
	typedef CALLBACK_API(OSErr, TipFunctionProcPtr)(Point tip, RgnHandle structure, Rect *r,
	...
	{
		return (TipFunctionUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppTipFunctionProcInfo,
...
#define DisposeTipFunctionUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(OSErr)
	InvokeTipFunctionUPP(Point tip, RgnHandle structure, Rect *r, BalloonVariant *balloonVariant,
						 TipFunctionUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(OSErr) InvokeTipFunctionUPP(Point tip, RgnHandle structure, Rect *r,
...
#define InvokeTipFunctionUPP(tip, structure, r, balloonVariant, userUPP) \
	(OSErr) CALL_FOUR_PARAMETER_UPP((userUPP), uppTipFunctionProcInfo, (tip), (structure), (r), (balloonVariant))
#endif
#endif
...
#define HMGetHelpMenuRef HMGetHelpMenuHandle
	 ...
	 */
	EXTERN_API(OSErr)
	HMShowBalloon(const HMMessageRecord *inHelpMessage, Point inTip, Rect *inHotRect, /* can be NULL */
				  TipFunctionUPP inTipProc, SInt16 inWindowProcID, BalloonVariant inBalloonVariant, SInt16 inMethod)
	 ...
	 */
	EXTERN_API(OSErr)
	HMShowMenuBalloon(SInt16 itemNum, SInt16 itemMenuID, SInt32 itemFlags, SInt32 itemReserved, Point tip,
					  Rect *alternateRect, /* can be NULL */
					  TipFunctionUPP tipProc, SInt16 theProc, BalloonVariant balloonVariant)
	 ...
	 */
	EXTERN_API(OSErr)
	HMGetIndHelpMsg(ResType inWhichResType, SInt16 inWhichResID, SInt16 inMessageIndex, SInt16 inMessageState,
					UInt32 *outOptions, Point *outTip, Rect *outHotRect, SInt16 *outWindowProcID,
					BalloonVariant *outBalloonVariant, HMMessageRecord *outHelpMessage, SInt16 *outMessageCount)
	 ...
	 */
	EXTERN_API(OSErr)
	HMSetMenuResID(SInt16 menuID, SInt16 resID) THREEWORDINLINE(0x303C, 0x020D, 0xA830);
	 ...
	 */
	EXTERN_API(OSErr)
	HMBalloonRect(const HMMessageRecord *inMessage, Rect *outRect) THREEWORDINLINE(0x303C, 0x040E, 0xA830);
	 ...
	 */
	EXTERN_API(OSErr)
	HMBalloonPict(const HMMessageRecord *inMessage, PicHandle *outPict) THREEWORDINLINE(0x303C, 0x040F, 0xA830);
	 ...
	 */
	EXTERN_API(OSErr)
	HMScanTemplateItems(SInt16 whichID, SInt16 whichResFile, ResType whichType) THREEWORDINLINE(0x303C, 0x0410, 0xA830);
	 ...
	 */
	EXTERN_API(OSErr)
	HMExtractHelpMsg(ResType inType, SInt16 inResID, SInt16 inMessageIndex, SInt16 inMessageState,
	 ...
	 */
	EXTERN_API(OSErr)
	HMGetMenuResID(SInt16 menuID, SInt16 *resID) THREEWORDINLINE(0x303C, 0x0314, 0xA830);
