...
enum
{
	...
	typeQDRegion = FOUR_CHAR_CODE('Qrgn'),	  /*  0x51447074  (data is actual region data, including rectangle and size,
												 _not_ region handle or ptr)*/
...
enum
{
	...
	kUnicodeNotFromInputMethod =
		FOUR_CHAR_CODE('unim'), /* Unicode text when event not handled by Input Method or no Input Method */
	...
	keyAETextServiceEncoding = FOUR_CHAR_CODE(
		'tsen'), /* Text Service encoding, mac or Unicode in UpdateActiveInputArea or GetSelectedText events. */
	keyAETextServiceMacEncoding =
		FOUR_CHAR_CODE('tmen'), /* Target mac encoding for TSM conversion of text from Unicode text service. */
	...
	typeGlyphInfoArray = FOUR_CHAR_CODE(
		'glia'), /* Glyph/FMFont info array for sub ranges of Unicode text.  See GlyphInfoArray in TextServices.h  */
