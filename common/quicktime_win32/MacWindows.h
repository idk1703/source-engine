...
#pragma pack(2)
	...
	enum
	{
		...
		kWindowOpaqueRgn =
			35, /* Mac OS X: Area of window considered to be opaque. Only valid for windows with alpha channels.*/
		kWindowGlobalPortRgn =
			40 /* Carbon forward - bounds of the window's port in global coordinates; not affected by CollapseWindow*/
	};
	...
	*/
	...
	struct SetupWindowProxyDragImageRec
	{
		...
		RgnHandle imageRgn;	   /* image clip region, contains the portion of the image which gets blitted to screen -
								  preallocated output - if imageGWorld is NULL, this is ignored*/
		RgnHandle outlineRgn;  /* the outline region used on shallow monitors - preallocated output - must always be
								  non-empty*/
	};
	...
	*/
	...
	enum
	{
		...
		kWindowWantsDisposeAtProcessDeath =
			(1 << 12), /* window definition wants a Dispose message for windows still extant during ExitToShell*/
		kWindowSupportsGetGrowImageRegion =
			(1 << 13), /* window definition will calculate the grow image region manually.*/
...
/*--------------------------------------------------------------------------------------*/
	...
	struct BasicWindowDescription
	{
		...
		WindowPositionMethod
			windowPositionMethod; /* method last used by RepositionWindow to position the window (if any)*/
...
#define kLastWindowOfClass	((WindowRef)0)
	...
	typedef CALLBACK_API(OSStatus, WindowPaintProcPtr)(GDHandle device, GrafPtr qdContext, WindowRef window,
	...
	{
		return (WindowDefUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppWindowDefProcInfo,
...
#define NewDeskHookUPP(userRoutine) \
	...
	{
		return (WindowPaintUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppWindowPaintProcInfo,
													GetCurrentArchitecture());
	}
#else
#define NewWindowPaintUPP(userRoutine) \
	(WindowPaintUPP) NewRoutineDescriptor((ProcPtr)(userRoutine), uppWindowPaintProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeWindowPaintUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(long)
	InvokeWindowDefUPP(short varCode, WindowRef window, short message, long param, WindowDefUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(long)
		InvokeWindowDefUPP(short varCode, WindowRef window, short message, long param, WindowDefUPP userUPP)
	{
...
#define InvokeWindowDefUPP(varCode, window, message, param, userUPP) \
	(long)CALL_FOUR_PARAMETER_UPP((userUPP), uppWindowDefProcInfo, (varCode), (window), (message), (param))
#endif
#endif
...
#endif
	EXTERN_API_C(void)
	InvokeDeskHookUPP(Boolean mouseClick, EventRecord *theEvent, DeskHookUPP userUPP) ONEWORDINLINE(0x4E91);
...
#define InvokeDeskHookUPP(mouseClick, theEvent, userUPP) \
	...
	InvokeWindowPaintUPP(GDHandle device, GrafPtr qdContext, WindowRef window, RgnHandle inClientPaintRgn,
						 RgnHandle outSystemPaintRgn, void *refCon, WindowPaintUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(OSStatus)
		InvokeWindowPaintUPP(GDHandle device, GrafPtr qdContext, WindowRef window, RgnHandle inClientPaintRgn,
							 RgnHandle outSystemPaintRgn, void *refCon, WindowPaintUPP userUPP)
	{
		return (OSStatus)CALL_SIX_PARAMETER_UPP(userUPP, uppWindowPaintProcInfo, device, qdContext, window,
...
#define InvokeWindowPaintUPP(device, qdContext, window, inClientPaintRgn, outSystemPaintRgn, refCon, userUPP) \
	(OSStatus) CALL_SIX_PARAMETER_UPP((userUPP), uppWindowPaintProcInfo, (device), (qdContext), (window),     \
									  (inClientPaintRgn), (outSystemPaintRgn), (refCon))
#endif
#endif
...
#define CallWindowPaintProc(userRoutine, device, qdContext, window, inClientPaintRgn, outSystemPaintRgn, refCon) \
	InvokeWindowPaintUPP(device, qdContext, window, inClientPaintRgn, outSystemPaintRgn, refCon, userRoutine)
#endif /* CALL_NOT_IN_CARBON */
	...
	GetNewCWindow(short windowID, void *wStorage, WindowRef behind) ONEWORDINLINE(0xAA46);
	 ...
	 */
	EXTERN_API(WindowRef)
	NewWindow(void *wStorage, const Rect *boundsRect, ConstStr255Param title, Boolean visible, short theProc,
			  WindowRef behind, Boolean goAwayFlag, long refCon) ONEWORDINLINE(0xA913);
	 ...
	 */
	EXTERN_API(WindowRef)
	GetNewWindow(short windowID, void *wStorage, WindowRef behind) ONEWORDINLINE(0xA9BD);
	 ...
	 */
	EXTERN_API(WindowRef)
	NewCWindow(void *wStorage, const Rect *boundsRect, ConstStr255Param title, Boolean visible, short procID,
			   WindowRef behind, Boolean goAwayFlag, long refCon) ONEWORDINLINE(0xAA45);
...
#define MacCloseWindow CloseWindow
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	CreateNewWindow(WindowClass windowClass, WindowAttributes attributes, const Rect *contentBounds,
					WindowRef *outWindow);
	 ...
	 */
	EXTERN_API(OSStatus)
	CreateCustomWindow(const WindowDefSpec *def, WindowClass windowClass, WindowAttributes attributes,
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowModality(WindowRef inWindow, WindowModality inModalKind, WindowRef inUnavailableWindow);
...
#define kLastWindowGroup  ((WindowGroupRef)NULL)
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetWindowGroupAttributes(WindowGroupRef inGroup, WindowGroupAttributes *outAttributes);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	ChangeWindowGroupAttributes(WindowGroupRef inGroup, WindowGroupAttributes setTheseAttributes,
	 ...
	 */
	EXTERN_API_C(ItemCount)
	CountWindowGroupContents(WindowGroupRef inGroup, WindowGroupContentOptions inOptions);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetWindowGroupContents(WindowGroupRef inGroup, WindowGroupContentOptions inOptions, ItemCount inAllowedItems,
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetIndexedWindow(WindowGroupRef inGroup, UInt32 inIndex, WindowGroupContentOptions inOptions, WindowRef *outWindow);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetWindowIndex(WindowRef inWindow, WindowGroupRef inStartGroup, WindowGroupContentOptions inOptions,
				   UInt32 *outIndex);
...
*/
	 ...
	 */
	EXTERN_API(OSStatus)
	InstallWindowContentPaintProc(WindowRef window, WindowPaintUPP paintProc, WindowPaintProcOptions options,
	...
	enum
	{
		...
		kScrollWindowEraseToPortBackground =
			(1L << 1) /* erase the exposed area using the background color/pattern of the window's grafport*/
	};
	 ...
	 */
	EXTERN_API(OSStatus)
	ScrollWindowRect(WindowRef inWindow, const Rect *inScrollRect, SInt16 inHPixels, SInt16 inVPixels,
					 ScrollWindowOptions inOptions, RgnHandle outExposedRgn); /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	ScrollWindowRegion(WindowRef inWindow, RgnHandle inScrollRgn, SInt16 inHPixels, SInt16 inVPixels,
					   ScrollWindowOptions inOptions, RgnHandle outExposedRgn); /* can be NULL */
...
#define MacFindWindow FindWindow
	...
	*/
	 ...
	 */
	EXTERN_API(WindowRef)
	GetNextWindowOfClass(WindowRef inWindow, WindowClass inWindowClass, Boolean mustBeVisible);
	 ...
	 */
	EXTERN_API(OSStatus)
	FindWindowOfClass(const Point *where, WindowClass inWindowClass, WindowRef *outWindow,
...
/*--------------------------------------------------------------------------------------*/
	 ...
	 */
	EXTERN_API(OSStatus)
	GetWindowFeatures(WindowRef window, UInt32 *outFeatures) THREEWORDINLINE(0x303C, 0x0013, 0xAA74);
	 ...
	 */
	EXTERN_API(OSStatus)
	GetWindowRegion(WindowRef window, WindowRegionCode inRegionCode, RgnHandle ioWinRgn)
		THREEWORDINLINE(0x303C, 0x0014, 0xAA74);
...
*/
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowProxyAlias(WindowRef inWindow, AliasHandle inAlias);
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowProxyCreatorAndType(WindowRef window, OSType fileCreator, OSType fileType, SInt16 vRefNum);
	 ...
	 */
	EXTERN_API(OSStatus)
	BeginWindowProxyDrag(WindowRef window, DragReference *outNewDrag, RgnHandle outDragOutlineRgn);
	 ...
	 */
	EXTERN_API(OSStatus)
	TrackWindowProxyFromExistingDrag(WindowRef window, Point startPt, DragReference drag, RgnHandle inDragOutlineRgn);
	 ...
	 */
	EXTERN_API(OSStatus)
	HiliteWindowFrameForDrag(WindowRef window, Boolean hilited) TWOWORDINLINE(0x7019, 0xA829);
	 ...
	 */
	EXTERN_API(OSStatus)
	TransitionWindow(WindowRef inWindow, WindowTransitionEffect inEffect, WindowTransitionAction inAction,
	 ...
	 */
	EXTERN_API(OSStatus)
	TransitionWindowAndParent(WindowRef inWindow, WindowRef inParentWindow, WindowTransitionEffect inEffect,
							  WindowTransitionAction inAction, const Rect *inRect); /* can be NULL */
...
#define MacMoveWindow MoveWindow
#endif
	EXTERN_API(void)
	MacMoveWindow(WindowRef window, short hGlobal, short vGlobal, Boolean front) ONEWORDINLINE(0xA91B);
	 ...
	 */
	EXTERN_API(void)
	SizeWindow(WindowRef window, short w, short h, Boolean fUpdate) ONEWORDINLINE(0xA91D);
	 ...
	 */
	EXTERN_API(long)
	GrowWindow(WindowRef window, Point startPt, const Rect *bBox) /* can be NULL */ ONEWORDINLINE(0xA92B);
	 ...
	 */
	EXTERN_API(void)
	DragWindow(WindowRef window, Point startPt, const Rect *boundsRect) /* can be NULL */ ONEWORDINLINE(0xA925);
	 ...
	 */
	EXTERN_API(void)
	ZoomWindow(WindowRef window, WindowPartCode partCode, Boolean front) ONEWORDINLINE(0xA83A);
	 ...
	 */
	EXTERN_API(OSStatus)
	CollapseWindow(WindowRef window, Boolean collapse) THREEWORDINLINE(0x303C, 0x0011, 0xAA74);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	CreateQDContextForCollapsedWindowDockTile(WindowRef inWindow, CGrafPtr *outContext);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	ReleaseQDContextForCollapsedWindowDockTile(WindowRef inWindow, CGrafPtr inContext);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetWindowDockTileMenu(WindowRef inWindow, MenuRef inMenu); /* can be NULL */
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	GetWindowBounds(WindowRef window, WindowRegionCode regionCode, Rect *globalBounds);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetWindowResizeLimits(WindowRef inWindow, const HISize *inMinLimits, /* can be NULL */
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetWindowResizeLimits(WindowRef inWindow, HISize *outMinLimits, /* can be NULL */
	 ...
	 */
	EXTERN_API(Boolean)
	ResizeWindow(WindowRef inWindow, Point inStartPoint, const Rect *inSizeConstraints, /* can be NULL */
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowBounds(WindowRef window, WindowRegionCode regionCode, const Rect *globalBounds);
	 ...
	 */
	EXTERN_API(OSStatus)
	RepositionWindow(WindowRef window, WindowRef parentWindow, WindowPositionMethod method);
	...
	*/
	 ...
	 */
	EXTERN_API(Boolean)
	IsWindowInStandardState(WindowRef inWindow, const Point *inIdealSize, /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	ZoomWindowIdeal(WindowRef inWindow, WindowPartCode inPartCode, Point *ioIdealSize);
	 ...
	 */
	EXTERN_API(OSStatus)
	GetWindowIdealUserState(WindowRef inWindow, Rect *outUserState);
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowIdealUserState(WindowRef inWindow, const Rect *inUserState);
	 ...
	 */
	EXTERN_API(OSStatus)
	ConstrainWindowToScreen(WindowRef inWindowRef, WindowRegionCode inRegionCode, WindowConstrainOptions inOptions,
	 ...
	 */
	EXTERN_API(OSStatus)
	GetAvailableWindowPositioningBounds(GDHandle inDevice, Rect *outAvailableRect);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetAvailableWindowPositioningRegion(GDHandle inDevice, RgnHandle ioRgn);
...
#define MacIsWindowVisible(window) (((WindowPeek)window)->visible != 0)
	...
	*/
	 ...
	 */
	EXTERN_API_C(Boolean)
	IsWindowLatentVisible(WindowRef inWindow, WindowLatentVisibility *outLatentVisible); /* can be NULL */
	...
	*/
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetDrawerPreferredEdge(WindowRef inDrawerWindow, OptionBits inEdge);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetDrawerParent(WindowRef inDrawerWindow, WindowRef inParent);
...
#define kWindowOffsetUnchanged ((float)(-1.0))
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetDrawerOffsets(WindowRef inDrawerWindow, float inLeadingOffset, float inTrailingOffset);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetDrawerOffsets(WindowRef inDrawerWindow, float *outLeadingOffset, /* can be NULL */
	 ...
	 */
	EXTERN_API_C(OSStatus)
	OpenDrawer(WindowRef inDrawerWindow, OptionBits inEdge, Boolean inAsync);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	CloseDrawer(WindowRef inDrawerWindow, Boolean inAsync);
...
#define _SetWindowToolbar SetWindowToolbar
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetWindowToolbar(WindowRef inWindow, HIToolbarRef inToolbar);
...
#define _GetWindowToolbar GetWindowToolbar
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetWindowToolbar(WindowRef inWindow, HIToolbarRef *outToolbar);
...
#define _ShowHideWindowToolbar ShowHideWindowToolbar
	 ...
	 */
	EXTERN_API_C(OSStatus)
	ShowHideWindowToolbar(WindowRef inWindow, Boolean inShow, Boolean inAnimate);
...
#define _IsWindowToolbarVisible IsWindowToolbarVisible
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetWindowAlpha(WindowRef inWindow, float inAlpha);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetWindowAlpha(WindowRef inWindow, float *outAlpha);
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	GetWindowProperty(WindowRef window, PropertyCreator propertyCreator, PropertyTag propertyTag, UInt32 bufferSize,
	 ...
	 */
	EXTERN_API(OSStatus)
	GetWindowPropertySize(WindowRef window, PropertyCreator creator, PropertyTag tag, UInt32 *size);
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowProperty(WindowRef window, PropertyCreator propertyCreator, PropertyTag propertyTag, UInt32 propertySize,
	 ...
	 */
	EXTERN_API(OSStatus)
	RemoveWindowProperty(WindowRef window, PropertyCreator propertyCreator, PropertyTag propertyTag);
	 ...
	 */
	EXTERN_API(OSStatus)
	GetWindowPropertyAttributes(WindowRef window, OSType propertyCreator, OSType propertyTag, UInt32 *attributes);
	 ...
	 */
	EXTERN_API(OSStatus)
	ChangeWindowPropertyAttributes(WindowRef window, OSType propertyCreator, OSType propertyTag, UInt32 attributesToSet,
	 ...
	 */
	EXTERN_API(Boolean)
	TrackBox(WindowRef window, Point thePt, WindowPartCode partCode) ONEWORDINLINE(0xA83B);
	 ...
	 */
	EXTERN_API(long)
	DragGrayRgn(RgnHandle theRgn, Point startPt, const Rect *limitRect, const Rect *slopRect, short axis,
	 ...
	 */
	EXTERN_API(long)
	DragTheRgn(RgnHandle theRgn, Point startPt, const Rect *limitRect, const Rect *slopRect, short axis,
...
/*--------------------------------------------------------------------------------------*/
	 ...
	 */
	EXTERN_API_C(WindowRef)
	newwindow(void *wStorage, const Rect *boundsRect, const char *title, Boolean visible, short theProc,
	 ...
	 */
	EXTERN_API_C(WindowRef)
	newcwindow(void *wStorage, const Rect *boundsRect, const char *title, Boolean visible, short procID,
			   WindowRef behind, Boolean goAwayFlag, long refCon);
	 ...
	 */
	EXTERN_API_C(long)
	draggrayrgn(RgnHandle theRgn, Point *startPt, const Rect *boundsRect, const Rect *slopRect, short axis,
...
#define IsWindowUpdatePending(window) \
	((((WindowPeek)window)->updateRgn != NULL) && !EmptyRgn(((WindowPeek)window)->updateRgn))
#endif
#endif
...
#define GetWindowStandardState(window, rect)                                                                           \
	((*(rect) = ((*(((WindowPeek)window)->dataHandle) == NULL) ? *(rect)                                               \
															   : ((Rect *)(*(((WindowPeek)window)->dataHandle)))[1])), \
	 (rect))
#endif
#endif
