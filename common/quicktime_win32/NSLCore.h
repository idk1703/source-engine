...
#define kNSLErrorNoErr       \
	...
	*/
	...
	struct NSLClientAsyncInfo
	{
		...
		UInt32 alertThreshold; /* call client's notifier or return, every this many items found ( 0 == don't use this
								  param)*/
	...
	{
		return (NSLMgrNotifyUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppNSLMgrNotifyProcInfo,
													 GetCurrentArchitecture());
	}
#else
#define NewNSLMgrNotifyUPP(userRoutine) \
	(NSLMgrNotifyUPP) NewRoutineDescriptor((ProcPtr)(userRoutine), uppNSLMgrNotifyProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define NewNSLClientNotifyUPP(userRoutine) \
	(NSLClientNotifyUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppNSLClientNotifyProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeNSLClientNotifyUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(void)
	InvokeNSLMgrNotifyUPP(NSLPluginAsyncInfo *thePluginAsyncInfo, NSLMgrNotifyUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define InvokeNSLMgrNotifyUPP(thePluginAsyncInfo, userUPP) \
	 ...
	 */
	EXTERN_API_C(void)
	InvokeNSLClientNotifyUPP(NSLClientAsyncInfo *theClientAsyncInfo, NSLClientNotifyUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define CallNSLClientNotifyProc(userRoutine, theClientAsyncInfo) \
	...
	/*
		this struct is a format for dealing with our internal data representation.  Typed data will be contiguous chunk
	   of memory, with the first 4 bytes being a data "descriptor".
	*/
	struct NSLTypedData
	{
...
#define NSLLibraryPresent() ((NSLLibraryVersion != (void *)kUnresolvedCFragSymbolAddress))
	...
	*/
	...
	/* ---> urlToRegister is a null terminated url that has only legal characters defined for URLs.  Use HexEncodeText
	 * to encode*/
	 ...
	 */
	EXTERN_API(NSLError)
	NSLStandardRegisterURL(NSLPath urlToRegister, NSLNeighborhood neighborhoodToRegisterIn); /* can be NULL */
	 ...
	 */
	EXTERN_API(NSLError)
	NSLStandardDeregisterURL(NSLPath urlToDeregister, NSLNeighborhood neighborhoodToDeregisterIn); /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	NSLHexEncodeText(const char *rawText, UInt16 rawTextLen, char *newTextBuffer, UInt16 *newTextBufferLen,
					 Boolean *textChanged);
	 ...
	 */
	EXTERN_API(OSStatus)
	NSLHexDecodeText(const char *encodedText, UInt16 encodedTextLen, char *decodedTextBuffer,
					 UInt16 *decodedTextBufferLen, Boolean *textChanged);
	...
	*/
	 ...
	 */
	EXTERN_API(NSLError)
	NSLAddServiceToServicesList(NSLServicesList serviceList, NSLServiceType serviceType);
	...
	*/
	...
	NSLMakeNewNeighborhood(const char *name, const char *protocolList); /* can be NULL */
	 ...
	 */
	EXTERN_API(void)
	NSLGetNameFromNeighborhood(NSLNeighborhood neighborhood, char **name, long *length);
	...
	*/
	 ...
	 */
	EXTERN_API(OSStatus)
	NSLMakeServicesRequestPB(NSLServicesList serviceList, NSLTypedDataPtr *newDataPtr);
	...
	*/
	 ...
	 */
	EXTERN_API(Boolean)
	NSLGetNextNeighborhood(NSLClientAsyncInfoPtr infoPtr, NSLNeighborhood *neighborhood, long *neighborhoodLength);

	/*
		NSLErrorToString:    convert a numeric error code to its string equivalent.  Caller must
													have allocated sufficient space to store both strings.  (Max 255
	   chars each)

													The errorString parameter will return a textual explanation of what
	   is wrong, while the solutionString returns a possible solution to get around the problem
	*/
	...
	*/
	...
	/*
			...
			2) infoPtr will point to a newly created ClientAsycnInfoPtr which will be disposed by the manager when the
	   search is completed NOTE: Only one search can be running at a time per clientRef.
	*/
	 ...
	 */
	EXTERN_API(NSLError)
	NSLPrepareRequest(NSLClientNotifyUPP notifier, void *contextPtr, NSLClientRef theClient, NSLRequestRef *ref,
					  char *bufPtr, unsigned long bufLen, NSLClientAsyncInfoPtr *infoPtr);
	...
	*/
	 ...
	 */
	EXTERN_API(NSLError)
	NSLStartNeighborhoodLookup(NSLRequestRef ref, NSLNeighborhood neighborhood, NSLClientAsyncInfo *asyncInfo);
	...
	*/
	 ...
	 */
	EXTERN_API(NSLError)
	NSLStartServicesLookup(NSLRequestRef ref, NSLNeighborhood neighborhood, NSLTypedDataPtr requestData,
	...
	*/
	...
	/* NSLParseServicesRequestPB provides the inverse of NSLMakeRequestPB, filling out the offsets found within
	 * newDataPtr*/
	/* <--- returns an OSStatus if any errors occur parsing the data */
	/* <--- newDataPtr is the construct passed to the plugin */
	/* ---> serviceListPtr is the address of a pointer which will be set to point at the portion of the newDataPtr that
	 * holds the serviceList to be searched */
	 ...
	 */
	EXTERN_API(OSStatus)
	NSLParseServicesRequestPB(NSLTypedDataPtr newDataPtr, char **serviceListPtr, UInt16 *serviceListLen);
	...
	/* ---> urlPtr is the address of a pointer which will be set to point at the portion of the newDataPtr that holds
	 ...
	 */
	EXTERN_API(OSStatus)
	NSLParseServiceRegistrationPB(NSLTypedDataPtr newDataPtr, NSLNeighborhood *neighborhoodPtr, UInt16 *neighborhoodLen,
								  char **urlPtr, UInt16 *urlLen);
	...
	/* <--> errorString is a pointer to valid memory of at least 256 bytes which will be filled out by the error portion
	 * of the error string */
	/* <--> solutionString is a pointer to valid memory of at least 256 bytes which will be filled out by the solution
	 * portion of the error string */
	/*
	 ...
	 */
	EXTERN_API(OSStatus)
	NSLGetErrorStringsFromResource(OSStatus theErr, const FSSpec *fileSpecPtr, SInt16 errorResID, char *errorString,
								   char *solutionString);
	 ...
	 */
	EXTERN_API(Boolean)
	NSLServiceIsInServiceList(NSLServicesList serviceList, NSLServiceType svcToFind);

	/* <--- returns an OSStatus if any errors occur parsing the data */
	/* ---> svcString is the address of a pointer which will be set to point at the portion of theURL that holds the
	 * serviceType of theURL */
	...
	*/
	 ...
	 */
	EXTERN_API(OSErr)
	NSLNewThread(ThreadStyle threadStyle, ThreadEntryProcPtr threadEntry, void *threadParam, Size stackSize,
				 ThreadOptions options, void **threadResult, ThreadID *threadMade);
	 ...
	 */
	EXTERN_API(OSErr)
	NSLDisposeThread(ThreadID threadToDump, void *threadResult, Boolean recycleThread);
