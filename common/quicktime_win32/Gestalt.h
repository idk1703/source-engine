...
#endif
	EXTERN_API(OSErr)
	ReplaceGestalt(OSType selector, SelectorFunctionUPP gestaltFunction, SelectorFunctionUPP *oldGestaltFunction)
...
#pragma parameter __D0 NewGestalt(__D0, __A0)
#endif
	EXTERN_API(OSErr)
	NewGestalt(OSType selector, SelectorFunctionUPP gestaltFunction) ONEWORDINLINE(0xA3AD);
	 ...
	 */
	EXTERN_API(OSErr)
	NewGestaltValue(OSType selector, long newValue) THREEWORDINLINE(0x303C, 0x0401, 0xABF1);
	 ...
	 */
	EXTERN_API(OSErr)
	ReplaceGestaltValue(OSType selector, long replacementValue) THREEWORDINLINE(0x303C, 0x0402, 0xABF1);
	 ...
	 */
	EXTERN_API(OSErr)
	SetGestaltValue(OSType selector, long newValue) THREEWORDINLINE(0x303C, 0x0404, 0xABF1);
	...
	{
		return (SelectorFunctionUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSelectorFunctionProcInfo,
														 GetCurrentArchitecture());
	}
#else
#define NewSelectorFunctionUPP(userRoutine) \
	(SelectorFunctionUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppSelectorFunctionProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeSelectorFunctionUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(OSErr)
	InvokeSelectorFunctionUPP(OSType selector, long *response, SelectorFunctionUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define InvokeSelectorFunctionUPP(selector, response, userUPP) \
	(OSErr) CALL_TWO_PARAMETER_UPP((userUPP), uppSelectorFunctionProcInfo, (selector), (response))
#endif
#endif
...
#define CallSelectorFunctionProc(userRoutine, selector, response) \
	...
	enum
	{
		...
		gestaltAliasMgrResolveAliasFileWithMountOptions =
			3, /* True if the Alias Mgr implements gestaltAliasMgrResolveAliasFileWithMountOptions() and IsAliasFile()
				*/
		...
		gestaltAliasMgrPrefersPath =
			7 /* True if the Alias Mgr prioritizes the path over file id during resolution by default */
	};
	...
	*/
	...
	enum
	{
		gestaltControlMgrVersion =
			FOUR_CHAR_CODE('cmvr') /* NOTE: The first version we return is 3.0.1, on Mac OS X plus update 2*/
	};
	...
	*/
	...
	enum
	{
		...
		gestaltDisplayMgrGeneratesProfiles =
			6, /* True if Display Manager will automatically generate profiles for displays */
		gestaltDisplayMgrSleepNotifies =
			7 /* True if Display Mgr generates "displayWillSleep", "displayDidWake" notifications */
	};
	...
	enum
	{
		gestaltDigitalSignatureVersion =
			FOUR_CHAR_CODE('dsig') /* returns Digital Signature Toolbox version in low-order word*/
	};
	...
	*/
	...
	enum
	{
		...
		gestaltFloppyUsesDiskInPlace =
			2 /* Floppy drive must have special DISK-IN-PLACE output; standard DISK-CHANGED not used */
	};
	...
	enum
	{
		...
		gestaltFinderFloppyRootComments =
			8, /* in MacOS 8 and later, will be set if Finder ever supports comments on Floppy icons */
		gestaltFinderLargeAndNotSavedFlavorsOK = 9, /* in MacOS 8 and later, this bit is set if drags with >1024-byte
													   flavors and flavorNotSaved flavors work reliably */
		gestaltFinderUsesExtensibleFolderManager =
			10, /* Finder uses Extensible Folder Manager (for example, for Magic Routing) */
		gestaltFinderUnderstandsRedirectedDesktopFolder =
			11 /* Finder deals with startup disk's desktop folder residing on another disk */
	};
	...
	enum
	{
		...
		gestaltFolderMgrSupportsDomains =
			4, /* True if Folder Mgr supports domains for the first parameter to FindFolder */
		gestaltFolderMgrSupportsFSCalls =
			5 /* True if FOlder manager supports __FindFolderFSRef & __FindFolderExtendedFSRef */
	};
	...
	enum
	{
		...
		gestaltFSUsesPOSIXPathsForConversion =
			14, /* The path interchange routines operate on POSIX paths instead of HFS paths */
	...
	enum
	{
		gestaltIconUtilitiesAttr =
			FOUR_CHAR_CODE('icon'), /* Icon Utilities attributes  (Note: available in System 7.0, despite gestalt) */
	...
	*/
	enum
	{
		...
		gestaltPwrBkEKDomKbd =
			195, /* (0xC3) PowerBook Domestic Keyboard with Embedded Keypad, function keys & inverted T    */
		gestaltPwrBkEKISOKbd =
			196, /* (0xC4) PowerBook International Keyboard with Embedded Keypad, function keys & inverted T   */
		gestaltPwrBkEKJISKbd =
			197, /* (0xC5) PowerBook Japanese Keyboard with Embedded Keypad, function keys & inverted T      */
		...
		gestaltPwrBk99JISKbd =
			201, /* (0xC9) '99 PowerBook JIS Keyboard with Embedded Keypad, function keys & inverted T               */
			...
			Line Names
					...
					The following table contains lineNames for some discontinued lines:
							...
							Macintosh Quadra            "MacQuadra" (preferred)
																					"Quadra" (also used, but not
	   preferred) Macintosh Centris           "MacCentris"
	...
	*/
	...
	{
		gestaltMenuMgrAttr =
			FOUR_CHAR_CODE('menu'), /* If this Gestalt exists, the Mac OS 8.5 Menu Manager is installed */
		gestaltMenuMgrPresent =
			(1L << 0), /* NOTE: this is a bit mask, whereas all other Gestalt constants of this nature */
		...
		gestaltMenuMgrMultipleItemsWithCommandIDBit = 2, /* CountMenuItemsWithCommandID/GetIndMenuItemWithCommandID
															support multiple items with the same command ID*/
		...
		gestaltMenuMgrSendsMenuBoundsToDefProcBit =
			4, /* kMenuSizeMsg and kMenuPopUpMsg have menu bounding rect information*/
		gestaltMenuMgrMoreThanFiveMenusDeepBit =
			5, /* the Menu Manager supports hierarchical menus more than five deep*/
	...
	enum
	{
		gestaltMultipleUsersState =
			FOUR_CHAR_CODE('mfdr') /* Gestalt selector returns MultiUserGestaltHandle (in Folders.h)*/
	};
	...
	*/
	...
	enum
	{
		...
		gestaltMixedModeCFM68KHasTrap =
			2, /* true if CFM-68K Mixed Mode implements _MixedModeDispatch (versions 1.0.1 and prior did not) */
	...
	enum
	{
		gestaltUserVisibleMachineName =
			FOUR_CHAR_CODE('mnam') /* Coerce response into a StringPtr to get a user visible machine name */
	};

	enum
	{
		gestaltMPCallableAPIsAttr =
			FOUR_CHAR_CODE('mpsc'), /* Bitmap of toolbox/OS managers that can be called from MPLibrary MPTasks */
	...
	enum
	{
		gestaltNameRegistryVersion =
			FOUR_CHAR_CODE('nreg') /* NameRegistryLib version number, for System 7.5.2+ usage */
	};
	...
	*/
	...
	enum
	{
		gestaltQuickTimeConferencingInfo =
			FOUR_CHAR_CODE('qtci') /* returns pointer to QuickTime Conferencing information */
	};
	...
	enum
	{
		...
		gestaltSanityCheckResourceFiles =
			2, /* Resource manager does sanity checking on resource files before opening them */
		gestaltSupportsFSpResourceFileAlreadyOpenBit = 3, /* The resource manager supports GetResFileRefNum and
															 FSpGetResFileRefNum and FSpResourceFileAlreadyOpen */
		gestaltRMSupportsFSCalls = 4, /* The resource manager supports OpenResFileFSRef, CreateResFileFSRef and
										 ResourceFileAlreadyOpenFSRef */
		gestaltRMTypeIndexOrderingReverse =
			8 /* GetIndType() calls return resource types in opposite order to original 68k resource manager */
	};
	...
	enum
	{
		...
		gestaltSplitOSMachineNameSetToNetworkNameTemp =
			3, /* The machine name was set to the value given us from the BootP server */
		gestaltSplitOSMachineNameStartupDiskIsNonPersistent = 5 /* The startup disk ( vRefNum == -1 ) is non-persistent,
																   meaning changes won't persist across a restart. */
	};
	...
	enum
	{
		gestaltSpeechRecognitionVersion = FOUR_CHAR_CODE(
			'srtb') /* speech recognition version (0x0150 is the first version that fully supports the API) */
	};
	...
	enum
	{
		...
		gestaltAVLTreeSupportsHandleBasedTreeBit =
			1, /* if set, then the AVL Tree routines can store tree data in a single handle */
		gestaltAVLTreeSupportsTreeLockingBit =
			2 /* if set, the AVLLockTree() and AVLUnlockTree() routines are available. */
	};
	...
	enum
	{
		...
		gestaltATSUFallbacksObjFeatures =
			0x00000008, /* feature introduced - ATSUFontFallbacks objects introduced in ATSUI version 2.3 */
		gestaltATSUIgnoreLeadingFeature = 0x00000008, /* feature introduced - kATSLineIgnoreFontLeading LineLayoutOption
														 introduced in ATSUI version 2.3 */
	...
	enum
	{
		...
		gestaltVMHasPagingControl =
			4 /* true if MakeMemoryResident, MakeMemoryNonResident, FlushMemory, and ReleaseMemoryData are available */
	};

	enum
	{
		gestaltVMInfoType =
			FOUR_CHAR_CODE('vmin'),		  /* Indicates how the Finder should display information about VM in */
	...
	enum
	{
		gestaltVMBackingStoreFileRefNum = FOUR_CHAR_CODE(
			'vmbs') /* file refNum of virtual memory's main backing store file (returned in low word of result) */
	};
	...
	enum
	{
		gestaltWindowMgrAttr =
			FOUR_CHAR_CODE('wind'),			 /* If this Gestalt exists, the Mac OS 8.5 Window Manager is installed*/
		...
		gestaltExtendedWindowAttributesBit =
			1,								/* Has ChangeWindowAttributes; GetWindowAttributes works for all windows*/
		...
		gestaltFrontWindowMayBeHiddenBit =
			8, /* FrontWindow and related APIs will return the front window even when the app is hidden*/
			   /* masks for the above bits*/
