...
#define MP_API_Version	  "2.3"
	...
	*/
	...
	enum
	{
		...
		kMPCreateTaskValidOptionsMask =
			kMPCreateTaskSuspendedMask | kMPCreateTaskTakesAllExceptionsMask | kMPCreateTaskNotDebuggableMask
	};
	...
	MPCreateTask(TaskProc entryPoint, void *parameter, ByteCount stackSize, MPQueueID notifyQueue,
				 void *terminationParameter1, void *terminationParameter2, MPTaskOptions options, MPTaskID *task);
	...
	*/
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MPWaitOnQueue(MPQueueID queue, void **param1, void **param2, void **param3, Duration timeout);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MPCreateSemaphore(MPSemaphoreCount maximumValue, MPSemaphoreCount initialValue, MPSemaphoreID *semaphore);
...
#define MPCreateBinarySemaphore(semaphore) MPCreateSemaphore(1, 1, (semaphore))
	 ...
	 */
	EXTERN_API(OSStatus)
	MPWaitForEvent(MPEventID event, MPEventFlags *flags, Duration timeout);
	...
	*/
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MPModifyNotification(MPNotificationID notificationID, MPOpaqueID anID, void *notifyParam1, void *notifyParam2,
						 void *notifyParam3);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MPModifyNotificationParameters(MPNotificationID notificationID, MPOpaqueIDClass kind, void *notifyParam1,
...
AbsoluteToDuration(AbsoluteTime time);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MPSetTimerNotify(MPTimerID timerID, MPOpaqueID anID, void *notifyParam1, void *notifyParam2, void *notifyParam3);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MPCancelTimer(MPTimerID timerID, AbsoluteTime *timeRemaining);
		...
		MPTaskStateKind values and MachineExceptions types is:
			...
			For reference, on PowerPC the MachineExceptions types contain:
					...
					VectorInformation   -> The AltiVec vector registers plus VSCR and VRSave, 32 values
																	of 128 bits each, one value of 128 bits, and one 32
	   bit value. MachineInformation  -> The CTR, LR, PC, each of 64 bits.  The CR, XER, MSR, MQ, exception kind, and
	   DSISR, each of 32 bits.  The 64 bit DAR. ExceptionInfo       -> Only memory exceptions are specified, 4 fields of
	   32 bits each. Note that this type only covers memory exceptions on 32-bit CPUs! The following types are declared
	   here: kMPTaskStateTaskInfo                -> MPTaskInfo
	*/
	...
	*/
	 ...
	 */
	EXTERN_API_C(void *)
	MPRemoteCall(MPRemoteProcedure remoteProc, void *parameter, MPRemoteContext context);
...
#define MPLibraryIsLoaded() \
	...
	*/
	 ...
	 */
	EXTERN_API_C(void)
	_MPLibraryVersion(const char **versionCString, UInt32 *major, UInt32 *minor, UInt32 *release, UInt32 *revision);
	...
	*/
	 ...
	 */
	EXTERN_API_C(Boolean)
	_MPLibraryIsCompatible(const char *versionCString, UInt32 major, UInt32 minor, UInt32 release, UInt32 revision);
