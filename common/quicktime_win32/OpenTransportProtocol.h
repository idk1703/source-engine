...
/* Opening and closing raw streams*/
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncStreamOpen(const char *name, OTOpenFlags oFlags, OTNotifyUPP proc, void *contextPtr);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTAsyncCreateStream(OTConfigurationRef cfig, OTOpenFlags oFlags, OTNotifyUPP proc, void *contextPtr);
	 ...
	 */
	EXTERN_API(OTResult)
	OTAsyncStreamPoll(PollRef *fds, UInt32 nfds, OTTimeout timeout, OTNotifyUPP proc, void *contextPtr);
	 ...
	 */
	EXTERN_API(OTResult)
	OTStreamGetMessage(StreamRef strm, strbuf *ctlbuf, strbuf *databuf, OTFlags *flags);
	 ...
	 */
	EXTERN_API(OTResult)
	OTStreamGetPriorityMessage(StreamRef strm, strbuf *ctlbuf, strbuf *databuf, OTBand *band, OTFlags *flags);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTStreamPutMessage(StreamRef strm, const strbuf *ctlbuf, const strbuf *databuf, OTFlags flags);
	 ...
	 */
	EXTERN_API(OSStatus)
	OTStreamPutPriorityMessage(StreamRef strm, const strbuf *ctlbuf, const strbuf *databuf, OTBand band, OTFlags flags);
	...
	*/
	 ...
	 */
	EXTERN_API(void)
	OTPutBackPartialBuffer(StreamRef strm, OTBufferInfo *readInfo, OTBuffer *buffer);
...
*/
	...
	OTYieldPortRequest(ProviderRef ref, OTPortRef portRef, OTClientList *buffer, OTByteCount size);
...
/* ***** OTConfiguration ******/
	...
	*/
	 ...
	 */
	EXTERN_API_C(OTConfigurationRef)
	OTCfigPushNewSingleChild(OTConfigurationRef cfig, const char *path, OSStatus *errPtr);
	 ...
	 */
	EXTERN_API_C(OTConfigurationRef)
	OTCfigPushChild(OTConfigurationRef cfig, OTItemCount index, const char *path, OSStatus *errPtr);
...
#define kOTSetupConfiguratorID "OTSetupConfigurator"
	...
	*/
	...
	typedef CALLBACK_API_C(OSStatus, OTCFCreateStreamProcPtr)(TOTConfiguratorRef cfigor, OTConfigurationRef cfig,
															  OTOpenFlags oFlags, OTNotifyUPP proc, void *contextPtr);
	typedef CALLBACK_API_C(void, OTCFHandleSystemEventProcPtr)(TOTConfiguratorRef cfigor, OTEventCode code,
...
*/
	...
	OTNewConfigurator(void *userData, OTCFConfigureProcPtr configure, OTCFCreateStreamProcPtr createStream,
	...
	*/
	 ...
	 */
	EXTERN_API_C(OSStatus)
	OTNotifyUser(FSSpec *theFile, SInt32 rsrcID, OTItemCount index, char *parm1, char *parm2);
	...
	*/
	 ...
	 */
	EXTERN_API_C(OSStatus)
	OTCreateControlStream(OTConfigurationRef cfig, TOTConfiguratorRef cfigor, OTNotifyUPP proc, void *contextPtr);
	...
	*/
	 ...
	 */
	EXTERN_API_C(void)
	OTCloseProvidersByUseCount(SInt32 *useCount, OTResult reason, OTBooleanParam doneDeal);
	 ...
	 */
	EXTERN_API_C(void)
	OTCloseProvidersByPortRef(OTPortRef ref, OTResult reason, OTBooleanParam doneDeal);
	 ...
	 */
	EXTERN_API_C(void)
	OTCloseProviderByStream(StreamRef ref, OTResult reason, OTBooleanParam doneDeal);
	 ...
	 */
	EXTERN_API_C(void)
	OTCloseMatchingProviders(UInt32 mask, OTPortRef port, OTResult reason, OTBooleanParam doneDeal);
...
*/
	...
	OTCreateStateMachine(void *buf, OTByteCount bufSize, OTByteCount structSize, OTNotifyUPP proc, void *contextPtr);
	...
	*/
	 ...
	 */
	EXTERN_API_C(Boolean)
	OTSMCreateStream(OTStateMachine *sm, OTConfigurationRef cfig, OTOpenFlags flags);
	 ...
	 */
	EXTERN_API_C(Boolean)
	OTSMPutMessage(OTStateMachine *sm, StreamRef strm, strbuf *ctlbuf, strbuf *databuf, OTFlags flags);
	 ...
	 */
	EXTERN_API_C(Boolean)
	OTSMGetMessage(OTStateMachine *sm, StreamRef strm, strbuf *ctlbuf, strbuf *databuf, OTFlags *flagsPtr);
	 ...
	 */
	EXTERN_API_C(void)
	OTSMInstallCompletionProc(OTStateMachine *sm, OTSMCompleteProcPtr completeProc, void *contextPtr);
	 ...
	 */
	EXTERN_API_C(Boolean)
	OTSMCreateControlStream(OTStateMachine *sm, OTConfigurationRef cfig, TOTConfiguratorRef cfigor);
...
#define kOTGetUserPortNameID "OTGetUserPortName"
	typedef CALLBACK_API_C(void, OTGetPortNameProcPtr)(OTPortRecord *port, OTBooleanParam includeSlot,
													   OTBooleanParam includePort, Str255 userVisibleName);
...
*/
	...
	*/
	 ...
	 */
	EXTERN_API_C(OTLink *)
	OTFindInHashList(OTHashList *hashList, OTHashSearchProcPtr searchProc, const void *refPtr, UInt32 hashValue);
	 ...
	 */
	EXTERN_API_C(OTLink *)
	OTRemoveFromHashList(OTHashList *hashList, OTHashSearchProcPtr searchProc, const void *refPtr, UInt32 hashValue);
...
/* Find CFM libraries of the specified kind and type*/
	 ...
	 */
	EXTERN_API_C(OSStatus)
	OTFindCFMLibraries(OSType libKind, const char *libType, OTList *theList, OTAllocMemProcPtr allocator);
	 ...
	 */
	EXTERN_API_C(void *)
	OTGetCFMPointer(const char *libName, const char *entryName, UInt32 *connID, UInt32 loadFlags);
