...
#pragma pack(2)
	 ...
	 */
	EXTERN_API(OSErr)
	SCSIComplete(short *stat, short *message, unsigned long wait) THREEWORDINLINE(0x3F3C, 0x0004, 0xA815);
	...
	{
		return (AENCallbackUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppAENCallbackProcInfo,
...
#define NewSIMInitUPP(userRoutine) \
	...
	{
		return (SIMActionUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSIMActionProcInfo,
												  GetCurrentArchitecture());
	}
#else
...
#define NewSCSIUPP(userRoutine) \
	...
	{
		return (SCSIMakeCallbackUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSCSIMakeCallbackProcInfo,
														 GetCurrentArchitecture());
	}
#else
#define NewSCSIMakeCallbackUPP(userRoutine) \
	(SCSIMakeCallbackUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppSCSIMakeCallbackProcInfo, GetCurrentArchitecture())
#endif
#endif
	...
	{
		return (SCSIInterruptPollUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSCSIInterruptPollProcInfo,
														  GetCurrentArchitecture());
	}
#else
#define NewSCSIInterruptPollUPP(userRoutine) \
	(SCSIInterruptPollUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppSCSIInterruptPollProcInfo, GetCurrentArchitecture())
#endif
#endif
	...
	{
		return (SCSIInterruptUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSCSIInterruptProcInfo,
													  GetCurrentArchitecture());
	}
#else
...
#define InvokeSCSIInterruptPollUPP(SIMGlobals, userUPP) \
	(long)CALL_ONE_PARAMETER_UPP((userUPP), uppSCSIInterruptPollProcInfo, (SIMGlobals))
#endif
#endif
...
#define InvokeSCSIInterruptUPP(SIMGlobals, userUPP) \
	(long)CALL_ONE_PARAMETER_UPP((userUPP), uppSCSIInterruptProcInfo, (SIMGlobals))
#endif
#endif
...
#define CallSCSIInterruptProc(userRoutine, SIMGlobals)	   InvokeSCSIInterruptUPP(SIMGlobals, userRoutine)
	...
	{
		return (SCSICallbackUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSCSICallbackProcInfo,
													 GetCurrentArchitecture());
	}
#else
...
#define SCSIPBHdr                   \
	...
	enum
	{
		scsiOddDisconnectUnsafeRead1 =
			0x0001, /* Disconnects on odd byte boundries are unsafe with DMA and/or blind reads */
		scsiOddDisconnectUnsafeWrite1 =
			0x0002, /* Disconnects on odd byte boundries are unsafe with DMA and/or blind writes */
		scsiBusErrorsUnsafe =
			0x0004, /* Non-handshaked delays or disconnects during blind transfers may cause a crash */
		scsiRequiresHandshake =
			0x0008, /* Non-handshaked delays or disconnects during blind transfers may cause data corruption */
		...
		scsiOddCountForPhysicalUnsafe =
			0x0020, /* If using physical addrs all counts must be even, and disconnects must be on even boundries */
