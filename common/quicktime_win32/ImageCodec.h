	...
	*/
	...
	enum
	{
		...
		codecImageBufferIsOnScreen =
			1L << 21, /* old def of codec using overlay surface, = ( codecIsDirectToScreenOnly | codecUsesOverlaySurface
	...
	enum
	{
		...
		codecImageBufferIsOverlaySurface =
			1L << 1, /* codec image buffer is overlay surface, the bits in the buffer are on the screen */
		...
		codecImageBufferMemoryFlagsValid =
			1L << 6, /* set by ImageCodecNewImageBufferMemory/NewImageGWorld to indicate that it set the AGP/PCI flags
						(supported in QuickTime 6.0 and later) */
		codecDrawsHigherQualityScaled =
			1L << 7, /* codec will draw higher-quality image if it performs scaling (eg, wipe effect with border) */
		codecSupportsOutOfOrderDisplayTimes =
			1L << 8, /* codec supports frames queued in one order for display in a different order, eg, IPB content */
		codecSupportsScheduledBackwardsPlaybackWithDifferenceFrames =
			1L << 9 /* codec can use additional buffers to minimise redecoding during backwards playback */
	};
	...
	struct CodecDecompressParams
	{
		...
		Boolean pad3;
		UInt8 destinationBufferMemoryPreference; /* a codec's PreDecompress/Preflight call can set this to express a
													preference about what kind of memory its destination buffer should
													go into.  no guarantees.*/
		UInt8 codecBufferMemoryPreference;		 /* may indicate preferred kind of memory that
													NewImageGWorld/NewImageBufferMemory should create its buffer in, if
													applicable.*/
		Boolean onlyUseCodecIfItIsInUserPreferredCodecList; /* set to prevent this codec from being used unless it is in
															   the userPreferredCodec list*/
		...
		UInt8 deinterlaceAnswer;  /* codec should set this in PreDecompress/Preflight if it will satisfy the
									 deinterlaceRequest */
	...
	typedef CALLBACK_API(void, ImageCodecDrawBandCompleteProcPtr)(void *refcon, ComponentResult drawBandResult,
	...
	{
		return (ImageCodecTimeTriggerUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppImageCodecTimeTriggerProcInfo,
															  GetCurrentArchitecture());
	}
#else
#define NewImageCodecTimeTriggerUPP(userRoutine) \
	(ImageCodecTimeTriggerUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppImageCodecTimeTriggerProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#ifdef __cplusplus
	inline DEFINE_API_C(ImageCodecDrawBandCompleteUPP)
		NewImageCodecDrawBandCompleteUPP(ImageCodecDrawBandCompleteProcPtr userRoutine)
	{
		return (ImageCodecDrawBandCompleteUPP)NewRoutineDescriptor(
			(ProcPtr)(userRoutine), uppImageCodecDrawBandCompleteProcInfo, GetCurrentArchitecture());
	}
#else
#define NewImageCodecDrawBandCompleteUPP(userRoutine) \
	(ImageCodecDrawBandCompleteUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppImageCodecDrawBandCompleteProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define InvokeImageCodecTimeTriggerUPP(refcon, userUPP) \
	 ...
	 */
	EXTERN_API_C(void)
	InvokeImageCodecDrawBandCompleteUPP(void *refcon, ComponentResult drawBandResult, UInt32 drawBandCompleteFlags,
	...
	inline DEFINE_API_C(void)
		InvokeImageCodecDrawBandCompleteUPP(void *refcon, ComponentResult drawBandResult, UInt32 drawBandCompleteFlags,
											ImageCodecDrawBandCompleteUPP userUPP)
	{
		CALL_THREE_PARAMETER_UPP(userUPP, uppImageCodecDrawBandCompleteProcInfo, refcon, drawBandResult,
...
#define CallImageCodecDrawBandCompleteProc(userRoutine, refcon, drawBandResult, drawBandCompleteFlags) \
	InvokeImageCodecDrawBandCompleteUPP(refcon, drawBandResult, drawBandCompleteFlags, userRoutine)
	...
	struct ImageSubCodecDecompressCapabilities
	{
		...
		/* The following fields only exist for QuickTime 7.0 and greater */
		Boolean subCodecIsMultiBufferAware; /* set if subcodec always draws using
											   ImageSubCodecDecompressRecord.baseAddr/rowBytes passed to
											   ImageCodecDrawBand, and always writes every pixel in the buffer*/
		...
		UInt8 pad5[2];
		Boolean
			subCodecSupportsDrawInDecodeOrder;	 /* indicates that it's okay for the subcodec to get a single DrawBand
													call for each frame in decode order even when frames need reordering.
													(This will only happen when other circumstances allow it.)*/
		Boolean subCodecSupportsDecodeSmoothing; /* Frame-reordering subcodecs should set this to indicate that they can
													safely decode a non-droppable frame before drawing the previous
													non-droppable frame.  This enables smoother playback in GWorlds.*/
		UInt8 pad6[4];
	};
	...
	struct ImageSubCodecDecompressRecord
	{
		...
		UInt8 frameType;
		Boolean inhibitMP; /* set this in BeginBand to tell the base decompressor not to call DrawBand from an MP task
							  for this frame.  (Only has any effect for MP-capable subcodecs.  New in QuickTime 5.0.)*/
		UInt8 pad[2];
		long priv[2];

		/* The following fields only exist for QuickTime 5.0 and greater */
		ImageCodecDrawBandCompleteUPP
			drawBandCompleteUPP; /* only used if subcodec set subCodecCallsDrawBandComplete; if drawBandCompleteUPP is
									non-nil, codec must call it when a frame is finished, but may return from DrawBand
									before the frame is finished. */
		void *drawBandCompleteRefCon; /* Note: do not call drawBandCompleteUPP directly from a hardware interrupt;
										 instead, use DTInstall to run a function at deferred task time, and call
										 drawBandCompleteUPP from that. */
	...
	*/
	...
	enum
	{
		codecSuggestedBufferSentinel = FOUR_CHAR_CODE(
			'sent') /* codec public resource containing suggested data pattern to put past end of data buffer */
	};
	...
	*/
	...
	enum
	{
		...
		kAtomMayBeIndexed =
			0x00000004 /* more than one value of atom can exist with accending IDs (ie, lists of colors)*/
	};
...
#define kNoScaleDouble	 (0) /* don't perform any scaling of value */
	...
	enum
	{
		...
		kPixelAspectRatioImageDescriptionExtension =
			FOUR_CHAR_CODE('pasp'), /* big-endian PixelAspectRatioImageDescriptionExtension */
	...
	enum
	{
		...
		kICCProfileColorInfoImageDescriptionExtensionType =
			FOUR_CHAR_CODE('prof') /* For ICC Profile color descriptions (not defined here)*/
	};
	...
	enum
	{
		kQTTransferFunction_ITU_R709_2 = 1, /* Recommendation ITU-R BT.709-2, SMPTE 274M-1995, SMPTE 296M-1997, SMPTE
											   293M-1996 and SMPTE 170M-1994 */
	...
	enum
	{
		kQTMatrix_ITU_R_709_2 =
			1, /* Recommendation ITU-R BT.709-2 (1125/60/2:1 only), SMPTE 274M-1995 and SMPTE 296M-1997 */
		kQTMatrix_Unknown = 2,	   /* Unknown */
		kQTMatrix_ITU_R_601_4 = 6, /* Recommendation ITU-R BT.601-4, Recommendation ITU-R BT.470-4 System B and G, SMPTE
									  170M-1994 and SMPTE 293M-1996 */
	...
	{
		return (ImageCodecMPDrawBandUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppImageCodecMPDrawBandProcInfo,
															 GetCurrentArchitecture());
	}
#else
#define NewImageCodecMPDrawBandUPP(userRoutine) \
	(ImageCodecMPDrawBandUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppImageCodecMPDrawBandProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeImageCodecMPDrawBandUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	...
	InvokeImageCodecMPDrawBandUPP(void *refcon, ImageSubCodecDecompressRecord *drp, ImageCodecMPDrawBandUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define InvokeImageCodecMPDrawBandUPP(refcon, drp, userUPP) \
	(ComponentResult) CALL_TWO_PARAMETER_UPP((userUPP), uppImageCodecMPDrawBandProcInfo, (refcon), (drp))
#endif
#endif
...
#define CallImageCodecMPDrawBandProc(userRoutine, refcon, drp) InvokeImageCodecMPDrawBandUPP(refcon, drp, userRoutine)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetCompressionTime(ComponentInstance ci, PixMapHandle src, const Rect *srcRect, short depth,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetMaxCompressionSize(ComponentInstance ci, PixMapHandle src, const Rect *srcRect, short depth,
									CodecQ quality, long *size) FIVEWORDINLINE(0x2F3C, 0x0012, 0x0002, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecBusy(ComponentInstance ci, ImageSequence seq) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0007, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetCompressedImageSize(ComponentInstance ci, ImageDescriptionHandle desc, Ptr data, long bufferSize,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetSimilarity(ComponentInstance ci, PixMapHandle src, const Rect *srcRect, ImageDescriptionHandle desc,
							Ptr data, Fixed *similarity) FIVEWORDINLINE(0x2F3C, 0x0014, 0x0009, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecTrimImage(ComponentInstance ci, ImageDescriptionHandle Desc, Ptr inData, long inBufferSize,
						ICMDataProcRecordPtr dataProc, Ptr outData, long outBufferSize, ICMFlushProcRecordPtr flushProc,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecRequestSettings(ComponentInstance ci, Handle settings, Rect *rp, ModalFilterUPP filterProc)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetSettings(ComponentInstance ci, Handle settings) FIVEWORDINLINE(0x2F3C, 0x0004, 0x000C, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecSetSettings(ComponentInstance ci, Handle settings) FIVEWORDINLINE(0x2F3C, 0x0004, 0x000D, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecSetTimeCode(ComponentInstance ci, void *timeCodeFormat, void *timeCodeTime)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecIsImageDescriptionEquivalent(ComponentInstance ci, ImageDescriptionHandle newDesc, Boolean *equivalent)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecNewMemory(ComponentInstance ci, Ptr *data, Size dataSize, long dataUse,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDisposeMemory(ComponentInstance ci, Ptr data) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0012, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecHitTestData(ComponentInstance ci, ImageDescriptionHandle desc, void *data, Size dataSize, Point where,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecNewImageBufferMemory(ComponentInstance ci, CodecDecompressParams *params, long flags,
								   ICMMemoryDisposedUPP memoryGoneProc, void *refCon)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecExtractAndCombineFields(ComponentInstance ci, long fieldFlags, void *data1, long dataSize1,
									  ImageDescriptionHandle desc1, void *data2, long dataSize2,
									  ImageDescriptionHandle desc2, void *outputData, long *outDataSize,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetMaxCompressionSizeWithSources(ComponentInstance ci, PixMapHandle src, const Rect *srcRect, short depth,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecSetTimeBase(ComponentInstance ci, void *base) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0017, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecSourceChanged(ComponentInstance ci, UInt32 majorSourceChangeSeed, UInt32 minorSourceChangeSeed,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecFlushFrame(ComponentInstance ci, UInt32 flags) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0019, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetParameterListHandle(ComponentInstance ci, Handle *parameterDescriptionHandle)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetParameterList(ComponentInstance ci, QTAtomContainer *parameterDescription)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecCreateStandardParameterDialog(ComponentInstance ci, QTAtomContainer parameterDescription,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecIsStandardParameterDialogEvent(ComponentInstance ci, EventRecord *pEvent, QTParameterDialog createdDialog)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDismissStandardParameterDialog(ComponentInstance ci, QTParameterDialog createdDialog)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecStandardParameterDialogDoAction(ComponentInstance ci, QTParameterDialog createdDialog, long action,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecNewImageGWorld(ComponentInstance ci, CodecDecompressParams *params, GWorldPtr *newGW, long flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecHitTestDataWithFlags(ComponentInstance ci, ImageDescriptionHandle desc, void *data, Size dataSize,
	...
	ImageCodecValidateParameters(ComponentInstance ci, QTAtomContainer parameters,
								 QTParameterValidationOptions validationFlags, StringPtr errorString)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetBaseMPWorkFunction(ComponentInstance ci, ComponentMPWorkFunctionUPP *workFunction, void **refCon,
									ImageCodecMPDrawBandUPP drawProc, void *drawProcRefCon)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecLockBits(ComponentInstance ci, CGrafPtr port) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0026, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecUnlockBits(ComponentInstance ci, CGrafPtr port) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0027, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecRequestGammaLevel(ComponentInstance ci, Fixed srcGammaLevel, Fixed dstGammaLevel, long *codecCanMatch)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetSourceDataGammaLevel(ComponentInstance ci, Fixed *sourceDataGammaLevel)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecMergeFloatingImageOntoWindow(ComponentInstance ci, UInt32 flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecRemoveFloatingImage(ComponentInstance ci, UInt32 flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetDITLForSize(ComponentInstance ci, Handle *ditl, Point *requestedSize)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDITLInstall(ComponentInstance ci, DialogRef d, short itemOffset)
		FIVEWORDINLINE(0x2F3C, 0x0006, 0x002F, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDITLEvent(ComponentInstance ci, DialogRef d, short itemOffset, const EventRecord *theEvent,
						short *itemHit, Boolean *handled) FIVEWORDINLINE(0x2F3C, 0x0012, 0x0030, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDITLItem(ComponentInstance ci, DialogRef d, short itemOffset, short itemNum)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDITLRemove(ComponentInstance ci, DialogRef d, short itemOffset)
		FIVEWORDINLINE(0x2F3C, 0x0006, 0x0032, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDITLValidateInput(ComponentInstance ci, Boolean *ok)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecGetPreferredChunkSizeAndAlignment(ComponentInstance ci, ImageDescriptionHandle desc, Fixed frameRate,
												UInt32 dataRate, long *bytesPerChunk, long *alignment)
		FIVEWORDINLINE(0x2F3C, 0x0014, 0x0036, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecPrepareToCompressFrames(ComponentInstance ci, ICMCompressorSessionRef session,
									  ICMCompressionSessionOptionsRef compressionSessionOptions,
									  ImageDescriptionHandle imageDescription, void *reserved,
									  CFDictionaryRef *compressorPixelBufferAttributesOut)
		FIVEWORDINLINE(0x2F3C, 0x0014, 0x0037, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEncodeFrame(ComponentInstance ci, ICMCompressorSourceFrameRef sourceFrame, UInt32 flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecCompleteFrame(ComponentInstance ci, ICMCompressorSourceFrameRef sourceFrame, UInt32 flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecBeginPass(ComponentInstance ci, ICMCompressionPassModeFlags passModeFlags, UInt32 flags,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecProcessBetweenPasses(ComponentInstance ci, ICMMultiPassStorageRef multiPassStorage,
								   Boolean *interpassProcessingDoneOut,
								   ICMCompressionPassModeFlags *requestedNextPassModeFlagsOut)
		FIVEWORDINLINE(0x2F3C, 0x000C, 0x003C, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecInitialize(ComponentInstance ci, ImageSubCodecDecompressCapabilities *cap)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecBeginBand(ComponentInstance ci, CodecDecompressParams *params, ImageSubCodecDecompressRecord *drp,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEndBand(ComponentInstance ci, ImageSubCodecDecompressRecord *drp, OSErr result, long flags)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDroppingFrame(ComponentInstance ci, const ImageSubCodecDecompressRecord *drp)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecScheduleFrame(ComponentInstance ci, const ImageSubCodecDecompressRecord *drp,
							ImageCodecTimeTriggerUPP triggerProc, void *triggerProcRefCon)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecDecodeBand(ComponentInstance ci, ImageSubCodecDecompressRecord *drp, unsigned long flags)
		FIVEWORDINLINE(0x2F3C, 0x0008, 0x020F, 0x7000, 0xA82A);
	...
	enum
	{
		...
		kFieldInfoImageDescriptionExtension =
			FOUR_CHAR_CODE('fiel') /* image description extension describing the field count and field orderings*/
	};
	 ...
	 */
	EXTERN_API(ComponentResult)
	QTPhotoSetSampling(ComponentInstance codec, short yH, short yV, short cbH, short cbV, short crH, short crV)
	 ...
	 */
	EXTERN_API(ComponentResult)
	QTPhotoSetRestartInterval(ComponentInstance codec, unsigned short restartInterval)
	 ...
	 */
	EXTERN_API(ComponentResult)
	QTPhotoDefineHuffmanTable(ComponentInstance codec, short componentNumber, Boolean isDC, unsigned char *lengthCounts,
	 ...
	 */
	EXTERN_API(ComponentResult)
	QTPhotoDefineQuantizationTable(ComponentInstance codec, short componentNumber, unsigned char *table)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEffectBegin(ComponentInstance effect, CodecDecompressParams *p, EffectsFrameParamsPtr ePtr)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEffectConvertEffectSourceToFormat(ComponentInstance effect, EffectSourcePtr sourceToConvert,
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEffectGetSpeed(ComponentInstance effect, QTAtomContainer parameters, Fixed *pFPS)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEffectPrepareSMPTEFrame(ComponentInstance effect, PixMapPtr destPixMap, SMPTEFrameReference *returnValue)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEffectDisposeSMPTEFrame(ComponentInstance effect, SMPTEFrameReference frameRef)
	 ...
	 */
	EXTERN_API(ComponentResult)
	ImageCodecEffectRenderSMPTEFrame(ComponentInstance effect, PixMapPtr destPixMap, SMPTEFrameReference frameRef,
									 Fixed effectPercentageEven, Fixed effectPercentageOdd, Rect *pSourceRect,
									 MatrixRecord *matrixP, SMPTEWipeType effectNumber, long xRepeat, long yRepeat,
									 SMPTEFlags flags, Fixed penWidth, long strokeValue)
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveGetLength(ComponentInstance effect, gxPaths *target, long index, wide *wideLength)
		FIVEWORDINLINE(0x2F3C, 0x000C, 0x0100, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveLengthToPoint(ComponentInstance effect, gxPaths *target, long index, Fixed length, FixedPoint *location,
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveNewPath(ComponentInstance effect, Handle *pPath) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0102, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveCountPointsInPath(ComponentInstance effect, gxPaths *aPath, unsigned long contourIndex, unsigned long *pCount)
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveGetPathPoint(ComponentInstance effect, gxPaths *aPath, unsigned long contourIndex, unsigned long pointIndex,
					  gxPoint *thePoint, Boolean *ptIsOnPath) FIVEWORDINLINE(0x2F3C, 0x0014, 0x0104, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveInsertPointIntoPath(ComponentInstance effect, gxPoint *aPoint, Handle thePath, unsigned long contourIndex,
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveSetPathPoint(ComponentInstance effect, gxPaths *aPath, unsigned long contourIndex, unsigned long pointIndex,
					  gxPoint *thePoint, Boolean ptIsOnPath) FIVEWORDINLINE(0x2F3C, 0x0012, 0x0106, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveGetNearestPathPoint(ComponentInstance effect, gxPaths *aPath, FixedPoint *thePoint,
							 unsigned long *contourIndex, unsigned long *pointIndex, Fixed *theDelta)
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurvePathPointToLength(ComponentInstance ci, gxPaths *aPath, Fixed startDist, Fixed endDist, FixedPoint *thePoint,
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveAddAtomToVectorStream(ComponentInstance effect, OSType atomType, Size atomSize, void *pAtomData,
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveAddPathAtomToVectorStream(ComponentInstance effect, Handle pathData, Handle vectorStream)
	 ...
	 */
	EXTERN_API(ComponentResult)
	CurveGetAtomDataFromVectorStream(ComponentInstance effect, Handle vectorStream, long atomType, long *dataSize,
