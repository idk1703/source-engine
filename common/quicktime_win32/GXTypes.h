...
typedef struct gxPaths                  gxPaths;
	...
	enum
	{
		...
		gxAnyPart = gxBoundsPart | gxGeometryPart | gxPenPart | gxCornerPointPart | gxControlPointPart | gxEdgePart |
					gxJoinPart | gxStartCapPart | gxEndCapPart | gxDashPart | gxPatternPart
	};
...
#define DisposegxSpoolUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(long)
	InvokegxSpoolUPP(gxSpoolCommand command, gxSpoolBlock *block, gxSpoolUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define InvokegxSpoolUPP(command, block, userUPP) \
	(long)CALL_TWO_PARAMETER_UPP((userUPP), uppgxSpoolProcInfo, (command), (block))
#endif
#endif
...
#define CallgxSpoolProc(userRoutine, command, block) InvokegxSpoolUPP(command, block, userRoutine)
	...
	enum
	{
		...
		gxFontGlyphsFlatten =
			0x02, /* if set, generate a gxTag containing the list of glyphs referenced inside the gxFont */
...
#define gxGraphicsSystemClient (gxGraphicsClient)(-1)
	...
	enum
	{
		...
		gxAllJustificationFlags = (gxOverridePriority | gxOverrideLimits | gxOverrideUnlimited |
	...
	enum
	{
		gxAllRunControlFlags =
			(gxNoLigatureSplits | gxNoCaretAngle | gxImposeWidth | gxNoCrossKerning | gxNoOpticalAlignment |
			 gxForceHanging | gxNoSpecialJustification | gxDirectionOverrideMask)
	};
	...
	enum
	{
		...
		gxAllLayoutOptionsFlags = gxLineIsDisplayOnly | gxKeepSpacesInMargin | gxLimitReorderingToTwoLevels |
								  gxLineLeftEdgeNotAtMargin | gxLineRightEdgeNotAtMargin,
		gxMaxRunLevel = 15,
		gxFlushLeft = 0,
	...
	xx yyyyyy   xx == 0x00, 0x40, 0x80, 0xC0: defines graphics operation (see gxGraphicsOperationOpcode)
							...
							if (yyyyyy == 0), byte size follows. If byte following == 0, word size follows; if == 0,
	long follows word and long, if present, are specified in high-endian order (first byte is largest magnitude)
