...
#define kIOModuleIntfIDStr "F4F7510C-C2DD-11D3-93A5-00050209D9C1"
	...
	typedef CALLBACK_API_C(OSStatus, PMIOModuleGetConnectionInfoProc)(CFStringRef *connectionType,
																	  CFStringRef *pbmPath);
	typedef CALLBACK_API_C(OSStatus, PMIOModuleInitializeProc)(CFDataRef printerAddress,
															   IOMContext *ioModuleContextPtr);
	typedef CALLBACK_API_C(OSStatus, PMIOModuleOpenProc)(IOMContext ioModuleContext, PMTicketRef jobTicket,
														 UInt32 *bufferSize);
	typedef CALLBACK_API_C(OSStatus, PMIOModuleReadProc)(IOMContext ioModuleContext, Ptr buffer, UInt32 *size,
														 Boolean *eoj);
	typedef CALLBACK_API_C(OSStatus, PMIOModuleWriteProc)(IOMContext ioModuleContext, Ptr buffer, UInt32 *size,
														  Boolean eoj);
	typedef CALLBACK_API_C(OSStatus, PMIOModuleStatusProc)(IOMContext ioModuleContext, CFStringRef *status);
	typedef CALLBACK_API_C(OSStatus, PMIOModuleGetAttributeProc)(IOMContext ioModuleContext, CFStringRef attribute,
																 CFTypeRef *result);
	typedef CALLBACK_API_C(OSStatus, PMIOModuleSetAttributeProc)(IOMContext ioModuleContext, CFStringRef attribute,
																 CFTypeRef data);
	...
	struct IOMProcs
	{
		...
		/* to /Library/Printers/.  The connection types: AppleTalk, USB, LPR, and Directory Services are reserved by
		 * Apple.*/
		...
		/* 'attribute' is supported and available, it's returned in 'result'.  If supported but not available, error
		 * code */
		...
		/* is to be canceled before closing connection (true; i.e. user cancels) or not (false; i.e. normal job
		 * completion).*/
