...
*/
	...
	typedef CALLBACK_API(void, TSMTEPostUpdateProcPtr)(TEHandle textH, long fixLen, long inputAreaStart,
	...
	{
		return (TSMTEPreUpdateUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppTSMTEPreUpdateProcInfo,
													   GetCurrentArchitecture());
	}
#else
#define NewTSMTEPreUpdateUPP(userRoutine) \
	(TSMTEPreUpdateUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppTSMTEPreUpdateProcInfo, GetCurrentArchitecture())
#endif
#endif
	...
	}; /* pascal no_return_value Func(4_bytes, 4_bytes, 4_bytes, 4_bytes, 4_bytes, 4_bytes, 4_bytes) */
#ifdef __cplusplus
	inline DEFINE_API_C(TSMTEPostUpdateUPP) NewTSMTEPostUpdateUPP(TSMTEPostUpdateProcPtr userRoutine)
	{
		return (TSMTEPostUpdateUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppTSMTEPostUpdateProcInfo,
														GetCurrentArchitecture());
	}
#else
#define NewTSMTEPostUpdateUPP(userRoutine) \
	(TSMTEPostUpdateUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppTSMTEPostUpdateProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define InvokeTSMTEPreUpdateUPP(textH, refCon, userUPP) \
	 ...
	 */
	EXTERN_API_C(void)
	InvokeTSMTEPostUpdateUPP(TEHandle textH, long fixLen, long inputAreaStart, long inputAreaEnd, long pinStart,
							 long pinEnd, long refCon, TSMTEPostUpdateUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(void)
		InvokeTSMTEPostUpdateUPP(TEHandle textH, long fixLen, long inputAreaStart, long inputAreaEnd, long pinStart,
								 long pinEnd, long refCon, TSMTEPostUpdateUPP userUPP)
	{
		CALL_SEVEN_PARAMETER_UPP(userUPP, uppTSMTEPostUpdateProcInfo, textH, fixLen, inputAreaStart, inputAreaEnd,
...
#define InvokeTSMTEPostUpdateUPP(textH, fixLen, inputAreaStart, inputAreaEnd, pinStart, pinEnd, refCon, userUPP) \
	CALL_SEVEN_PARAMETER_UPP((userUPP), uppTSMTEPostUpdateProcInfo, (textH), (fixLen), (inputAreaStart),         \
...
#define CallTSMTEPostUpdateProc(userRoutine, textH, fixLen, inputAreaStart, inputAreaEnd, pinStart, pinEnd, refCon) \
	InvokeTSMTEPostUpdateUPP(textH, fixLen, inputAreaStart, inputAreaEnd, pinStart, pinEnd, refCon, userRoutine)
#endif /* CALL_NOT_IN_CARBON */
