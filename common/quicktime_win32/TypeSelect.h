...
#pragma pack(2)
	...
	typedef CALLBACK_API(Boolean, IndexToStringProcPtr)(short item, ScriptCode *itemsScript, StringPtr *itemsStringPtr,
														void *yourDataPtr);
	 ...
	 */
	EXTERN_API(Boolean)
	TypeSelectNewKey(const EventRecord *theEvent, TypeSelectRecord *tsr) FOURWORDINLINE(0x7000, 0x3F3C, 0x002A, 0xA9ED);
	 ...
	 */
	EXTERN_API(short)
	TypeSelectFindItem(const TypeSelectRecord *tsr, short listSize, TSCode selectMode, IndexToStringUPP getStringProc,
	 ...
	 */
	EXTERN_API(short)
	TypeSelectCompare(const TypeSelectRecord *tsr, ScriptCode testStringScript, StringPtr testStringPtr)
	...
	{
		return (IndexToStringUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppIndexToStringProcInfo,
													  GetCurrentArchitecture());
	}
#else
#define NewIndexToStringUPP(userRoutine) \
	(IndexToStringUPP) NewRoutineDescriptor((ProcPtr)(userRoutine), uppIndexToStringProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define DisposeIndexToStringUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	...
	InvokeIndexToStringUPP(short item, ScriptCode *itemsScript, StringPtr *itemsStringPtr, void *yourDataPtr,
						   IndexToStringUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(Boolean) InvokeIndexToStringUPP(short item, ScriptCode *itemsScript, StringPtr *itemsStringPtr,
														void *yourDataPtr, IndexToStringUPP userUPP)
	{
		return (Boolean)CALL_FOUR_PARAMETER_UPP(userUPP, uppIndexToStringProcInfo, item, itemsScript, itemsStringPtr,
												yourDataPtr);
	}
#else
#define InvokeIndexToStringUPP(item, itemsScript, itemsStringPtr, yourDataPtr, userUPP)                             \
	(Boolean) CALL_FOUR_PARAMETER_UPP((userUPP), uppIndexToStringProcInfo, (item), (itemsScript), (itemsStringPtr), \
									  (yourDataPtr))
#endif
#endif
...
#define CallIndexToStringProc(userRoutine, item, itemsScript, itemsStringPtr, yourDataPtr) \
	InvokeIndexToStringUPP(item, itemsScript, itemsStringPtr, yourDataPtr, userRoutine)
