...
typedef QHdr *QHdrPtr;
	...
	{
		return (DeferredTaskUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppDeferredTaskProcInfo,
...
#endif
	EXTERN_API_C(void)
	InvokeDeferredTaskUPP(long dtParam, DeferredTaskUPP userUPP) ONEWORDINLINE(0x4E90);
...
#endif
	EXTERN_API(void)
	Delay(unsigned long numTicks, unsigned long *finalTicks) TWOWORDINLINE(0xA03B, 0x2280);
...
#endif
	EXTERN_API(OSErr)
	FlushCodeCacheRange(void *address, unsigned long count) TWOWORDINLINE(0x7009, 0xA098);
