...
#pragma pack(2)
	...
	TrackMouseLocationWithOptions(GrafPtr inPort, /* can be NULL */
								  OptionBits inOptions, EventTimeout inTimeout, Point *outPt,
	...
	TrackMouseRegion(GrafPtr inPort, /* can be NULL */
					 RgnHandle inRegion, Boolean *ioWasInRgn, MouseTrackingResult *outResult);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetMouseCoalescingEnabled(Boolean inNewState, Boolean *outOldState); /* can be NULL */
	...
	/*
		...
		kEventMouseDown
			...
			-->     kEventParamTabletPointRec       typeTabletPointRec          (if kEventParamTabletEventType is
	   kEventTabletPoint)
			-->     kEventParamTabletProximityRec   typeTabletProximityRec  (if kEventParamTabletEventType is
	...
	*/
	...
	/*
		...
		kEventMenuBeginTracking
			...
			-->     kEventParamMenuContext              typeUInt32 (on Mac OS X 10.1 and later, and CarbonLib 1.5 and
	   later)
	   ...
	   later)
		...
		kEventMenuPopulate (on Mac OS X 10.1 and later, and CarbonLib 1.5 and later)
			-->     kEventParamDirectObject             typeMenuRef
			-->     kEventParamMenuContext              typeUInt32
			-->     kEventParamMenuCommand              typeMenuCommand (Jaguar and CarbonLib 1.6, for
	...
	*/
	...
	AXUIElementCreateWithHIObjectAndIdentifier(HIObjectRef inHIObject, UInt64 inIdentifier);
	 ...
	 */
	EXTERN_API_C(void)
	AXUIElementGetIdentifier(AXUIElementRef inUIElement, UInt64 *outIdentifier);
	 ...
	 */
	EXTERN_API_C(void)
	AXNotificationHIObjectNotify(CFStringRef inNotification, HIObjectRef inHIObject, UInt64 inIdentifier);
	...
	enum
	{
		...
		kEventParamTextInputSendText =
			FOUR_CHAR_CODE('tstx'), /*    typeUnicodeText (if TSMDocument is Unicode), otherwise typeChar*/
		kEventParamTextInputReplyText =
			FOUR_CHAR_CODE('trtx'), /*    typeUnicodeText (if TSMDocument is Unicode), otherwise typeChar*/
	...
	enum
	{
		...
		kEventParamTabletPointerRec =
			FOUR_CHAR_CODE('tbrc'), /* typeTabletPointerRec      -- deprecated, for compatibility only*/
		typeTabletPointerRec =
			FOUR_CHAR_CODE('tbrc') /* kEventParamTabletPointerRec    -- deprecated, for compatibility only*/
	};
...
#define SendEventToUserFocus(e) SendEventToEventTarget((e), GetUserFocusEventTarget())
	 ...
	 */
	EXTERN_API(OSStatus)
	RegisterToolboxObjectClass(CFStringRef inClassID, ToolboxObjectClassRef inBaseClass, /* can be NULL */
							   UInt32 inNumEvents, const EventTypeSpec *inEventList, EventHandlerUPP inEventHandler,
							   void *inEventHandlerData, ToolboxObjectClassRef *outClassRef);
...
#define kUserFocusAuto ((WindowRef)(-1))
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowDefaultButton(WindowRef inWindow, ControlRef inControl); /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	SetWindowCancelButton(WindowRef inWindow, ControlRef inControl); /* can be NULL */
	 ...
	 */
	EXTERN_API_C(OSStatus)
	RegisterEventHotKey(UInt32 inHotKeyCode, UInt32 inHotKeyModifiers, EventHotKeyID inHotKeyID,
	 ...
	 */
	EXTERN_API_C(OSStatus)
	CreateMouseTrackingRegion(WindowRef inWindow, RgnHandle inRegion, RgnHandle inClip, /* can be NULL */
							  MouseTrackingOptions inOptions, MouseTrackingRegionID inID, void *inRefCon,
	 ...
	 */
	EXTERN_API_C(OSStatus)
	ChangeMouseTrackingRegion(MouseTrackingRef inMouseRef, RgnHandle inRegion, RgnHandle inClip); /* can be NULL */
	 ...
	 */
	EXTERN_API_C(OSStatus)
	ClipMouseTrackingRegion(MouseTrackingRef inMouseRef, RgnHandle inRegion);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetMouseTrackingRegionID(MouseTrackingRef inMouseRef, MouseTrackingRegionID *outID);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	GetMouseTrackingRegionRefCon(MouseTrackingRef inMouseRef, void **outRefCon);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MoveMouseTrackingRegion(MouseTrackingRef inMouseRef, SInt16 deltaH, SInt16 deltaV,
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetMouseTrackingRegionEnabled(MouseTrackingRef inMouseRef, Boolean inEnabled);
	...
	*/
	 ...
	 */
	EXTERN_API_C(OSStatus)
	ClipWindowMouseTrackingRegions(WindowRef inWindow, OSType inSignature, RgnHandle inClip); /* can be NULL */
	 ...
	 */
	EXTERN_API_C(OSStatus)
	MoveWindowMouseTrackingRegions(WindowRef inWindow, OSType inSignature, SInt16 deltaH, SInt16 deltaV,
								   RgnHandle inClip); /* can be NULL */
	 ...
	 */
	EXTERN_API_C(OSStatus)
	SetWindowMouseTrackingRegionsEnabled(WindowRef inWindow, OSType inSignature, Boolean inEnabled);
	 ...
	 */
	EXTERN_API_C(OSStatus)
	ReleaseWindowMouseTrackingRegions(WindowRef inWindow, OSType inSignature);
