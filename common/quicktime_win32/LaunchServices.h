...
#pragma pack(2)
	...
	enum
	{
		kLSMinCatInfoBitmap = (kFSCatInfoNodeFlags | kFSCatInfoParentDirID | kFSCatInfoFinderInfo |
							   kFSCatInfoFinderXInfo) /* minimum info needed to avoid a FSGetCatalogInfo call when
	...
	enum
	{
		kLSInvalidExtensionIndex =
			(unsigned long)0xFFFFFFFF /* Index returned from LSGetExtensionInfo when name has no extension*/
	};
	...
	enum
	{
		...
		kLSRequestBasicFlagsOnly =
			0x00000004, /* all but type of application and extension flags - safe to use from threads*/
	 ...
	 */
	EXTERN_API(OSStatus)
	LSCopyKindStringForTypeInfo(OSType inType, OSType inCreator, CFStringRef inExtension, /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	LSGetApplicationForItem(const FSRef *inItemRef, LSRolesMask inRoleMask, FSRef *outAppRef, /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	LSGetApplicationForInfo(OSType inType, OSType inCreator, CFStringRef inExtension, /* can be NULL */
							LSRolesMask inRoleMask, FSRef *outAppRef,				  /* can be NULL */
							CFURLRef *outAppURL);									  /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	LSGetApplicationForURL(CFURLRef inURL, LSRolesMask inRoleMask, FSRef *outAppRef, /* can be NULL */
						   CFURLRef *outAppURL);									 /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	LSFindApplicationForInfo(OSType inCreator, CFStringRef inBundleID, /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	LSCanRefAcceptItem(const FSRef *inItemFSRef, const FSRef *inTargetRef, LSRolesMask inRoleMask,
					   LSAcceptanceFlags inFlags, Boolean *outAcceptsItem);
	 ...
	 */
	EXTERN_API(OSStatus)
	LSOpenFromRefSpec(const LSLaunchFSRefSpec *inLaunchSpec, FSRef *outLaunchedRef); /* can be NULL */
	 ...
	 */
	EXTERN_API(OSStatus)
	LSOpenFromURLSpec(const LSLaunchURLSpec *inLaunchSpec, CFURLRef *outLaunchedURL); /* can be NULL */
