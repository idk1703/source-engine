...
#define twelfthRootTwo 1.05946309435
	...
	enum
	{
		...
		kSoundCodecInfoResourceType =
			FOUR_CHAR_CODE('snfo') /*Resource type holding codec information (optional public component resource)*/
	};
	...
	enum
	{
		...
		siCompressionOutputSampleRate = FOUR_CHAR_CODE(
			'cort'), /* GetInfo only: only implemented by compressors that have differing in and out rates */
		siCompressionInputRateList = FOUR_CHAR_CODE(
			'crtl'), /* GetInfo only: only implemented by compressors that only take certain input rates */
		...
		siEQSpectrumResolution = FOUR_CHAR_CODE(
			'eqrs'), /* set the resolution of the FFT, 0 = low res (<=16 bands), 1 = high res (16-64 bands)*/
		...
		siSupportedExtendedFlags =
			FOUR_CHAR_CODE('exfl'), /*which flags are supported in Extended sound data structures*/
		siRateConverterRollOffSlope =
			FOUR_CHAR_CODE('rcdb'), /*the roll-off slope for the rate converter's filter, in whole dB as a long this
									   value is a long whose range is from 20 (worst quality/fastest performance) to 90
									   (best quality/slowest performance)*/
		...
		siSourceIsExhausted = FOUR_CHAR_CODE(
			'srcx'), /*the ultimate source of data has run out (keep asking, but when you get nothing, that's it)*/
		siMediaContextID = FOUR_CHAR_CODE('uuid'), /*media context id -- UUID */
		siCompressionMaxPacketSize =
			FOUR_CHAR_CODE('cmxp'), /*maximum compressed packet size for current configuration -- unsigned long */
		siAudioCodecPropertyValue =
			FOUR_CHAR_CODE('spva'), /*audio codec property value -- SoundAudioCodecPropertyRequestParams* */
		siAudioCodecPropertyInfo =
			FOUR_CHAR_CODE('spin') /*audio codec property info -- SoundAudioCodecPropertyRequestParams* */
	};
	...
	{
		kInvalidSource =
			(long)0xFFFFFFFF, /*this source may be returned from GetInfo if no other source is the monitored source*/
	...
	enum
	{
		...
		kHALCustomComponentSubType =
			FOUR_CHAR_CODE('halx'), /*Registered by the HAL output component ('hal!') for each HAL output device*/
...
enum
{
	...
	enum
	{
		...
		kSoundCodecInfoCanChangeOutputRate =
			(1L << 3), /* compressor may output a different sample rate than it receives*/
		kSoundCodecInfoRequiresExternalFraming =
			(1L << 4), /* format requires external framing information during decode/encode*/
	...
	enum
	{
		...
		kAVDisplayPlainTalkRemove =
			2, /* monitor either sending no input through CPU input port or unable to tell if input is coming in*/
		kAVDisplayPlainTalkInsert =
			3 /* monitor sending PlainTalk level microphone source input through sound input port*/
	};
	...
	enum
	{
		kSoundConverterDidntFillBuffer =
			(1 << 0), /*set if the converter couldn't completely satisfy a SoundConverterFillBuffer request*/
		kSoundConverterHasLeftOverData = (1 << 1) /*set if the converter had left over data after completely satisfying
													 a SoundConverterFillBuffer call*/
	};

	/* flags for extendedFlags fields of ExtendedSoundComponentData, ExtendedSoundParamBlock, and
	 * ExtendedScheduledSoundHeader*/
	enum
	{
		kExtendedSoundSampleCountNotValid =
			1L << 0, /* set if sampleCount of SoundComponentData isn't meaningful; use buffer size instead*/
		kExtendedSoundBufferSizeValid = 1L << 1, /* set if bufferSize field is valid*/
		kExtendedSoundFrameSizesValid = 1L << 2, /* set if frameSizesArray is valid (will be nil if all sizes are common
													and kExtendedSoundCommonFrameSizeValid is set*/
		kExtendedSoundCommonFrameSizeValid =
			1L << 3, /* set if all audio frames have the same size and the commonFrameSize field is valid*/
	...
	/* flags passed in bufferFlags/bufferFlagsMask extended fields if kExtendedSoundBufferFlagsValid extended flag is
	 * set*/
	enum
	{
	...
	*/
	...
	{
		return (SndCallBackUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSndCallBackProcInfo,
...
#define DisposeSndCallBackUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(void)
	InvokeSndCallBackUPP(SndChannelPtr chan, SndCommand *cmd, SndCallBackUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define CallSndCallBackProc(userRoutine, chan, cmd) InvokeSndCallBackUPP(chan, cmd, userRoutine)
	...
	{
		return (SndDoubleBackUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSndDoubleBackProcInfo,
													  GetCurrentArchitecture());
	}
#else
...
#define DisposeSndDoubleBackUPP(userUPP) DisposeRoutineDescriptor(userUPP)
	 ...
	 */
	EXTERN_API_C(void)
	InvokeSndDoubleBackUPP(SndChannelPtr channel, SndDoubleBufferPtr doubleBufferPtr, SndDoubleBackUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
...
#define CallSndDoubleBackProc(userRoutine, channel, doubleBufferPtr) \
	...
	{
		return (SoundParamUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSoundParamProcInfo,
												   GetCurrentArchitecture());
	}
#else
...
#ifdef __cplusplus
	inline DEFINE_API_C(SoundConverterFillBufferDataUPP)
		NewSoundConverterFillBufferDataUPP(SoundConverterFillBufferDataProcPtr userRoutine)
	{
		return (SoundConverterFillBufferDataUPP)NewRoutineDescriptor(
			(ProcPtr)(userRoutine), uppSoundConverterFillBufferDataProcInfo, GetCurrentArchitecture());
	}
#else
#define NewSoundConverterFillBufferDataUPP(userRoutine)     \
	...
	{
		return (SIInterruptUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSIInterruptProcInfo,
													GetCurrentArchitecture());
	}
#else
#define NewSIInterruptUPP(userRoutine) \
	...
	{
		return (SICompletionUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppSICompletionProcInfo,
													 GetCurrentArchitecture());
	}
#else
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
	inline DEFINE_API_C(Boolean) InvokeSoundConverterFillBufferDataUPP(SoundComponentDataPtr *data, void *refCon,
																	   SoundConverterFillBufferDataUPP userUPP)
	{
...
#define InvokeSoundConverterFillBufferDataUPP(data, refCon, userUPP) \
	(Boolean) CALL_TWO_PARAMETER_UPP((userUPP), uppSoundConverterFillBufferDataProcInfo, (data), (refCon))
#endif
#endif
	 ...
	 */
	EXTERN_API_C(void)
	InvokeSIInterruptUPP(SPBPtr inParamPtr, Ptr dataBuffer, short peakAmplitude, long sampleSize,
						 SIInterruptUPP userUPP);
#if !OPAQUE_UPP_TYPES && (!TARGET_OS_MAC || !TARGET_CPU_68K || TARGET_RT_MAC_CFM)
#ifdef __cplusplus
	inline DEFINE_API_C(void) InvokeSIInterruptUPP(SPBPtr inParamPtr, Ptr dataBuffer, short peakAmplitude,
...
#define InvokeSIInterruptUPP(inParamPtr, dataBuffer, peakAmplitude, sampleSize, userUPP)                    \
	CALL_FOUR_PARAMETER_UPP((userUPP), uppSIInterruptProcInfo, (inParamPtr), (dataBuffer), (peakAmplitude), \
							(sampleSize))
#endif
#endif
...
#define CallSIInterruptProc(userRoutine, inParamPtr, dataBuffer, peakAmplitude, sampleSize) \
	InvokeSIInterruptUPP(inParamPtr, dataBuffer, peakAmplitude, sampleSize, userRoutine)
#define CallSICompletionProc(userRoutine, inParamPtr) InvokeSICompletionUPP(inParamPtr, userRoutine)
	...
	{
		return (FilePlayCompletionUPP)NewRoutineDescriptor((ProcPtr)(userRoutine), uppFilePlayCompletionProcInfo,
														   GetCurrentArchitecture());
	}
#else
#define NewFilePlayCompletionUPP(userRoutine) \
	(FilePlayCompletionUPP)                   \
		NewRoutineDescriptor((ProcPtr)(userRoutine), uppFilePlayCompletionProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#define CallFilePlayCompletionProc(userRoutine, chan) InvokeFilePlayCompletionUPP(chan, userRoutine)
	...
	*/
	 ...
	 */
	EXTERN_API(OSErr)
	SndDoCommand(SndChannelPtr chan, const SndCommand *cmd, Boolean noWait) ONEWORDINLINE(0xA803);
	 ...
	 */
	EXTERN_API(OSErr)
	SndNewChannel(SndChannelPtr *chan, short synth, long init, SndCallBackUPP userRoutine) ONEWORDINLINE(0xA807);
	 ...
	 */
	EXTERN_API(OSErr)
	SndPlay(SndChannelPtr chan, SndListHandle sndHandle, Boolean async) ONEWORDINLINE(0xA805);
	 ...
	 */
	EXTERN_API(OSErr)
	SndAddModifier(SndChannelPtr chan, Ptr modifier, short id, long init) ONEWORDINLINE(0xA802);
...
/* Sound Manager 2.0 and later, uses _SoundDispatch */
	 ...
	 */
	EXTERN_API(OSErr)
	SndStartFilePlay(SndChannelPtr chan, short fRefNum, short resNum, long bufferSize, void *theBuffer,
					 AudioSelectionPtr theSelection, FilePlayCompletionUPP theCompletion, Boolean async)
	 ...
	 */
	EXTERN_API(OSErr)
	SndStopFilePlay(SndChannelPtr chan, Boolean quietNow) FOURWORDINLINE(0x203C, 0x0308, 0x0008, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SndManagerStatus(short theLength, SMStatusPtr theStatus) FOURWORDINLINE(0x203C, 0x0314, 0x0008, 0xA800);
	 ...
	 */
	EXTERN_API(void)
	Comp3to1(const void *inBuffer, void *outBuffer, unsigned long cnt, StateBlockPtr inState, StateBlockPtr outState,
			 unsigned long numChannels, unsigned long whichChannel) FOURWORDINLINE(0x203C, 0x0004, 0x0010, 0xA800);
	 ...
	 */
	EXTERN_API(void)
	Exp1to3(const void *inBuffer, void *outBuffer, unsigned long cnt, StateBlockPtr inState, StateBlockPtr outState,
			unsigned long numChannels, unsigned long whichChannel) FOURWORDINLINE(0x203C, 0x0008, 0x0010, 0xA800);
	 ...
	 */
	EXTERN_API(void)
	Comp6to1(const void *inBuffer, void *outBuffer, unsigned long cnt, StateBlockPtr inState, StateBlockPtr outState,
			 unsigned long numChannels, unsigned long whichChannel) FOURWORDINLINE(0x203C, 0x000C, 0x0010, 0xA800);
	 ...
	 */
	EXTERN_API(void)
	Exp1to6(const void *inBuffer, void *outBuffer, unsigned long cnt, StateBlockPtr inState, StateBlockPtr outState,
			unsigned long numChannels, unsigned long whichChannel) FOURWORDINLINE(0x203C, 0x0010, 0x0010, 0xA800);
...
/* Sound Manager 3.0 and later calls, uses _SoundDispatch */
	 ...
	 */
	EXTERN_API(OSErr)
	GetSoundHeaderOffset(SndListHandle sndHandle, long *offset) FOURWORDINLINE(0x203C, 0x0404, 0x0018, 0xA800);
	...
	UnsignedFixedMulDiv(UnsignedFixed value, UnsignedFixed multiplier, UnsignedFixed divisor)
	 ...
	 */
	EXTERN_API(OSErr)
	GetCompressionInfo(short compressionID, OSType format, short numChannels, short sampleSize, CompressionInfoPtr cp)
	 ...
	 */
	EXTERN_API(OSErr)
	SetSoundPreference(OSType theType, Str255 name, Handle settings) FOURWORDINLINE(0x203C, 0x0634, 0x0018, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	GetSoundPreference(OSType theType, Str255 name, Handle settings) FOURWORDINLINE(0x203C, 0x0638, 0x0018, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	OpenMixerSoundComponent(SoundComponentDataPtr outputDescription, long outputFlags,
	 ...
	 */
	EXTERN_API(OSErr)
	SndGetInfo(SndChannelPtr chan, OSType selector, void *infoPtr) FOURWORDINLINE(0x203C, 0x063C, 0x0018, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SndSetInfo(SndChannelPtr chan, OSType selector, const void *infoPtr) FOURWORDINLINE(0x203C, 0x0640, 0x0018, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	GetCompressionName(OSType compressionType, Str255 compressionName) FOURWORDINLINE(0x203C, 0x044C, 0x0018, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SoundConverterOpen(const SoundComponentData *inputFormat, const SoundComponentData *outputFormat,
	 ...
	 */
	EXTERN_API(OSErr)
	SoundConverterGetBufferSizes(SoundConverter sc, unsigned long inputBytesTarget, unsigned long *inputFrames,
	 ...
	 */
	EXTERN_API(OSErr)
	SoundConverterConvertBuffer(SoundConverter sc, const void *inputPtr, unsigned long inputFrames, void *outputPtr,
	 ...
	 */
	EXTERN_API(OSErr)
	SoundConverterEndConversion(SoundConverter sc, void *outputPtr, unsigned long *outputFrames,
	 ...
	 */
	EXTERN_API(OSErr)
	SoundConverterFillBuffer(SoundConverter sc, SoundConverterFillBufferDataUPP fillBufferDataUPP,
							 void *fillBufferDataRefCon, void *outputBuffer, unsigned long outputBufferByteSize,
							 unsigned long *bytesWritten, unsigned long *framesWritten, unsigned long *outputFlags)
	 ...
	 */
	EXTERN_API(OSErr)
	SoundManagerGetInfo(OSType selector, void *infoPtr) FOURWORDINLINE(0x203C, 0x047C, 0x0018, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SoundManagerSetInfo(OSType selector, const void *infoPtr) FOURWORDINLINE(0x203C, 0x0480, 0x0018, 0xA800);
	...
	*/
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentSetSource(ComponentInstance ti, SoundSource sourceID, ComponentInstance source)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentGetSource(ComponentInstance ti, SoundSource sourceID, ComponentInstance *source)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentGetSourceData(ComponentInstance ti, SoundComponentDataPtr *sourceData)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentSetOutput(ComponentInstance ti, SoundComponentDataPtr requested, SoundComponentDataPtr *actual)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentStartSource(ComponentInstance ti, short count, SoundSource *sources)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentStopSource(ComponentInstance ti, short count, SoundSource *sources)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentPauseSource(ComponentInstance ti, short count, SoundSource *sources)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SoundComponentPlaySourceBuffer(ComponentInstance ti, SoundSource sourceID, SoundParamBlockPtr pb, long actions)
...
*/
	 ...
	 */
	EXTERN_API(ComponentResult)
	AudioGetInfo(ComponentInstance ac, AudioInfoPtr info) FIVEWORDINLINE(0x2F3C, 0x0004, 0x0005, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	AudioMuteOnEvent(ComponentInstance ac, short muteOnEvent) FIVEWORDINLINE(0x2F3C, 0x0002, 0x0081, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(OSErr)
	SndRecord(ModalFilterUPP filterProc, Point corner, OSType quality, SndListHandle *sndHandle)
	 ...
	 */
	EXTERN_API(OSErr)
	SndRecordToFile(ModalFilterUPP filterProc, Point corner, OSType quality, short fRefNum)
	 ...
	 */
	EXTERN_API(OSErr)
	SPBSignInDevice(short deviceRefNum, ConstStr255Param deviceName) FOURWORDINLINE(0x203C, 0x030C, 0x0014, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SPBRecord(SPBPtr inParamPtr, Boolean asynchFlag) FOURWORDINLINE(0x203C, 0x0320, 0x0014, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SPBGetRecordingStatus(long inRefNum, short *recordingStatus, short *meterLevel, unsigned long *totalSamplesToRecord,
						  unsigned long *numberOfSamplesRecorded, unsigned long *totalMsecsToRecord,
	 ...
	 */
	EXTERN_API(OSErr)
	SPBGetDeviceInfo(long inRefNum, OSType infoType, void *infoData) FOURWORDINLINE(0x203C, 0x0638, 0x0014, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SPBSetDeviceInfo(long inRefNum, OSType infoType, void *infoData) FOURWORDINLINE(0x203C, 0x063C, 0x0014, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SPBMillisecondsToBytes(long inRefNum, long *milliseconds) FOURWORDINLINE(0x203C, 0x0440, 0x0014, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SPBBytesToMilliseconds(long inRefNum, long *byteCount) FOURWORDINLINE(0x203C, 0x0444, 0x0014, 0xA800);
	 ...
	 */
	EXTERN_API(OSErr)
	SetupSndHeader(SndListHandle sndHandle, short numChannels, UnsignedFixed sampleRate, short sampleSize,
				   OSType compressionType, short baseNote, unsigned long numBytes, short *headerLen)
	 ...
	 */
	EXTERN_API(OSErr)
	SetupAIFFHeader(short fRefNum, short numChannels, UnsignedFixed sampleRate, short sampleSize,
	 ...
	 */
	EXTERN_API(OSErr)
	ParseAIFFHeader(short fRefNum, SoundComponentData *sndInfo, unsigned long *numFrames, unsigned long *dataOffset)
	 ...
	 */
	EXTERN_API(OSErr)
	ParseSndHeader(SndListHandle sndHandle, SoundComponentData *sndInfo, unsigned long *numFrames,
	 ...
	 */
	EXTERN_API(ComponentResult)
	SndInputGetStatus(ComponentInstance self, short *recordingStatus, unsigned long *totalSamplesToRecord,
