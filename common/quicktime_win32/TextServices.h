...
#define mGetLanguageCode(cdRec) ((LangCode)cdRec.componentFlags & bLanguageMask)
	 ...
	 */
	EXTERN_API(OSErr)
	NewTSMDocument(short numOfInterface, InterfaceTypeList supportedInterfaceTypes, TSMDocumentID *idocID, long refcon)
	 ...
	 */
	EXTERN_API(OSErr)
	GetServiceList(short numOfInterface, OSType *supportedInterfaceTypes, TextServiceListHandle *serviceInfo,
	 ...
	 */
	EXTERN_API(OSErr)
	OpenTextService(TSMDocumentID idocID, Component aComponent, ComponentInstance *aComponentInstance)
	 ...
	 */
	EXTERN_API(OSErr)
	CloseTextService(TSMDocumentID idocID, ComponentInstance aComponentInstance) TWOWORDINLINE(0x700A, 0xAA54);
	 ...
	 */
	EXTERN_API(OSErr)
	SendAEFromTSMComponent(const AppleEvent *theAppleEvent, AppleEvent *reply, AESendMode sendMode,
	 ...
	 */
	EXTERN_API(OSErr)
	SetDefaultInputMethod(Component ts, ScriptLanguageRecord *slRecordPtr) TWOWORDINLINE(0x700C, 0xAA54);
	 ...
	 */
	EXTERN_API(OSErr)
	GetDefaultInputMethod(Component *ts, ScriptLanguageRecord *slRecordPtr) TWOWORDINLINE(0x700D, 0xAA54);
	 ...
	 */
	EXTERN_API(OSErr)
	UseInputWindow(TSMDocumentID idocID, Boolean useWindow) TWOWORDINLINE(0x7010, 0xAA54);
	 ...
	 */
	EXTERN_API(OSStatus)
	TSMSetInlineInputRegion(TSMDocumentID inTSMDocument, WindowRef inWindow, RgnHandle inRegion);
...
/* Following calls from Classic event loops not needed for Carbon clients. */
	 ...
	 */
	EXTERN_API(OSErr)
	NewServiceWindow(void *wStorage, const Rect *boundsRect, ConstStr255Param title, Boolean visible, short theProc,
					 WindowRef behind, Boolean goAwayFlag, ComponentInstance ts, WindowRef *window)
	 ...
	 */
	EXTERN_API(short)
	FindServiceWindow(Point thePoint, WindowRef *theWindow) TWOWORDINLINE(0x7017, 0xAA54);
	 ...
	 */
	EXTERN_API(OSErr)
	NewCServiceWindow(void *wStorage, const Rect *boundsRect, ConstStr255Param title, Boolean visible, short theProc,
					  WindowRef behind, Boolean goAwayFlag, ComponentInstance ts, WindowRef *window)
		TWOWORDINLINE(0x701A, 0xAA54);
...
/* Component Manager Interfaces to Input Methods */
	 ...
	 */
	EXTERN_API(ComponentResult)
	TextServiceEventRef(ComponentInstance ts, EventRef event) FIVEWORDINLINE(0x2F3C, 0x0006, 0x0006, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	UCTextServiceEvent(ComponentInstance ts, short numOfEvents, EventRecord *event, UniChar unicodeString[],
	 ...
	 */
	EXTERN_API(ComponentResult)
	SetTextServiceCursor(ComponentInstance ts, Point mousePos) FIVEWORDINLINE(0x2F3C, 0x0004, 0x000A, 0x7000, 0xA82A);
	 ...
	 */
	EXTERN_API(ComponentResult)
	GetTextServiceProperty(ComponentInstance ts, OSType propertySelector, SInt32 *result)
	 ...
	 */
	EXTERN_API(ComponentResult)
	SetTextServiceProperty(ComponentInstance ts, OSType propertySelector, SInt32 value)
	 ...
	 */
	EXTERN_API(OSStatus)
	GetDefaultInputMethodOfClass(Component *aComp, ScriptLanguageRecord *slRecPtr, TextServiceClass tsClass);
	 ...
	 */
	EXTERN_API(OSStatus)
	SetDefaultInputMethodOfClass(Component aComp, ScriptLanguageRecord *slRecPtr, TextServiceClass tsClass);
	 ...
	 */
	EXTERN_API(OSStatus)
	TSMSetDocumentProperty(TSMDocumentID docID, OSType propertyTag, UInt32 propertySize, void *propertyData);
	 ...
	 */
	EXTERN_API(OSStatus)
	TSMGetDocumentProperty(TSMDocumentID docID, OSType propertyTag, UInt32 bufferSize, UInt32 *actualSize,
	 ...
	 */
	EXTERN_API(OSStatus)
	TSMRemoveDocumentProperty(TSMDocumentID docID, OSType propertyTag);
