...
// Convert Jpeg data to raw RGBA, pubData is the jpeg data, cubData is it's size, buf is for output, and width/height
// are output as well. pcubUsed is an optional output param for how many bytes of data were used in the image
bool ConvertJpegToRGBA(const byte *pubJpegData, int cubJpegData, CUtlBuffer &bufOutput, int &width, int &height,
					   int *pcubUsed = NULL);

// Convert Jpeg data to raw RGB, pubData is the jpeg data, cubData is it's size, buf is for output, and width/height are
// output as well. pcubUsed is an optional output param for how many bytes of data were used in the image
...
// Convert PNG data to raw RGBA, pubData is the PNG data, cubData is it's size, buf is for output, and width/height are
// output as well.
