...
class SVC_SetPause : public CNetMessage
{
	...
	SVC_SetPause() {}
	SVC_SetPause(bool state, float end = -1.f)
	{
		m_bPaused = state;
	}
...
class SVC_UpdateStringTable : public CNetMessage
{
	...
	int GetGroup() const
	{
		return INetChannelInfo::STRINGTABLE;
	}
...
class SVC_VoiceInit : public CNetMessage
{
	...
	SVC_VoiceInit(const char *codec, int nSampleRate) : m_nSampleRate(nSampleRate)
	{
...
class SVC_CrosshairAngle : public CNetMessage
{
	...
	SVC_CrosshairAngle() {}
	SVC_CrosshairAngle(QAngle angle)
	{
		m_Angle = angle;
	}
...
class SVC_EntityMessage : public CNetMessage
{
	...
	int GetGroup() const
	{
		return INetChannelInfo::ENTMESSAGES;
	}
...
public:
	...
	QueryCvarCookie_t m_iCookie;
	const char
		*m_szCvarName; // The sender sets this, and it automatically points it at m_szCvarNameBuffer when receiving.
