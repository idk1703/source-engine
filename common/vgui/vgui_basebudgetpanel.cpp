...
CBaseBudgetPanel::CBaseBudgetPanel(vgui::Panel *pParent, const char *pElementName) : vgui::Panel(pParent, pElementName)
{
...
void CBaseBudgetPanel::PerformLayout()
{
	...
	m_pBudgetBarGraphPanel->SetPos(maxLabelWidth, totalHeightMinusTimeLabels * bottomOfHistoryPercentage);
	m_pBudgetBarGraphPanel->SetSize(totalWidth - maxLabelWidth,
									totalHeightMinusTimeLabels * (1 - bottomOfHistoryPercentage));

	for(i = 0; i < m_GraphLabels.Count(); i++)
	{
		m_GraphLabels[i]->SetPos(0, (bottomOfHistoryPercentage * totalHeightMinusTimeLabels) +
		...
		m_GraphLabels[i]->SetSize(maxLabelWidth, 1 + (totalHeightMinusTimeLabels * (1 - bottomOfHistoryPercentage)) /
														 m_ConfigData.m_BudgetGroupInfo.Count());
...
{
	m_pBudgetHistoryPanel->SetData(&m_BudgetGroupTimes[0].m_Time[0], GetNumCachedBudgetGroups(), BUDGET_HISTORY_COUNT,
