...
ConVar budget_bargraph_background_alpha("budget_bargraph_background_alpha", "128", FCVAR_ARCHIVE,
										"how translucent the budget panel is");

ConVar budget_peaks_window("budget_peaks_window", "30", FCVAR_ARCHIVE,
						   "number of frames to look at when figuring out peak frametimes");
ConVar budget_averages_window("budget_averages_window", "30", FCVAR_ARCHIVE,
...
void CBudgetBarGraphPanel::DrawTimeLines(void)
{
	...
	for(i = 0; i < nTotalLines; i++)
	{
		...
		float flTemp =
			(config.m_flBarGraphRange != 0.0f) ? (flValueInterval / config.m_flBarGraphRange) : flValueInterval;
