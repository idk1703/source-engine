...
public:
	enum loadStatus_t
	{
		...
		LOADSTATUS_CREATED,	  // GameCfg.INI was not found, the system created the default configuration based on found
							  // GameInfo.txt resources
	...
	bool AddDefaultConfig(const defaultConfigInfo_t &info, KeyValues *out, const char *rootDirectory,
	...
	void SetSDKEpoch(eSDKEpochs epoch)
	{
		m_eSDKEpoch = epoch;
	};
...
private:
	...
	eSDKEpochs m_eSDKEpoch; // Holds the "working version" of the SDK for times when we need to create an older set of
							// game configurations. This is required now that the SDK is deploying the tools for both
							// the latest and previous versions of the engine.
};
