...
//-----------------------------------------------------------------------------
bool BResizeImageInternal(CUtlBuffer &bufImage, int nWidth, int nHeight, int &nNewWidth, int &nNewHeight,
						  bool bIsRGBA = true)
{
	...
	if(nPaddingLeft > 0 || nPaddingRight > 0 || nPaddingTop > 0 || nPaddingBottom > 0)
	{
	...
	for(int yNew = 0; yNew < nNewContentHeight; ++yNew)
	{
		...
		for(int xNew = 0; xNew < nNewContentWidth; ++xNew)
		{
			...
			byte red =
				(byte)(pSrcBits[aOffset] * (1 - xDiff) * (1 - yDiff) + pSrcBits[bOffset] * (xDiff) * (1 - yDiff) +
					   pSrcBits[cOffset] * (yDiff) * (1 - xDiff) + pSrcBits[dOffset] * (xDiff) * (yDiff));

			byte green =
				(byte)(pSrcBits[aOffset + 1] * (1 - xDiff) * (1 - yDiff) +
					   pSrcBits[bOffset + 1] * (xDiff) * (1 - yDiff) + pSrcBits[cOffset + 1] * (yDiff) * (1 - xDiff) +
					   pSrcBits[dOffset + 1] * (xDiff) * (yDiff));

			byte blue =
				(byte)(pSrcBits[aOffset + 2] * (1 - xDiff) * (1 - yDiff) +
					   pSrcBits[bOffset + 2] * (xDiff) * (1 - yDiff) + pSrcBits[cOffset + 2] * (yDiff) * (1 - xDiff) +
			...
			{
				alpha =
					(byte)(pSrcBits[aOffset + 3] * (1 - xDiff) * (1 - yDiff) +
						   pSrcBits[bOffset + 3] * (xDiff) * (1 - yDiff) +
						   pSrcBits[cOffset + 3] * (yDiff) * (1 - xDiff) + pSrcBits[dOffset + 3] * (xDiff) * (yDiff));
			}
