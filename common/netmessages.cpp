...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: backup %i, new %i, bytes %i", GetName(), m_nNewCommands, m_nBackupCommands,
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: SendTableCRC %i", GetName(), m_nSendTableCRC);
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: status: %d, value: %s, cookie: %d", GetName(), m_eStatusCode, m_szCvarValue,
			   m_iCookie);
	return s_text;
}
...
bool CLC_FileMD5Check::ReadFromBuffer(bf_read &buffer)
{
	...
	else if((iCode - 1) < ARRAYSIZE(g_MostCommonPrefixes))
	{
		char szTemp[MAX_PATH];
		buffer.ReadString(szTemp, sizeof(szTemp));
		V_snprintf(m_szFilename, sizeof(m_szFilename), "%s%c%s", g_MostCommonPrefixes[iCode - 1],
				   CORRECT_PATH_SEPARATOR, szTemp);
	}
	else
	{
...
{
	V_snprintf(s_text, sizeof(s_text), "%s: filename: %s, start byte: %i, post death record time: %f", GetName(),
			   m_szFilename, m_nStartSendByte, m_flPostDeathRecordTime);
	return s_text;
}
#endif
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: %s", GetName(), m_pKeyValues ? m_pKeyValues->GetName() : "<<null>>");
	return s_text;
}
...
bool SVC_ServerInfo::WriteToBuffer(bf_write &buffer)
{
	...
	buffer.WriteLong(
		0xffffffff); // Used to be client.dll CRC.  This was far before signed binaries, VAC, and cross-platform play
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: game \"%s\", map \"%s\", max %i", GetName(), m_szGameDir, m_szMapName,
			   m_nMaxClients);
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: tex %i, ent %i, mod %i lowpriority %i", GetName(), m_nDecalTextureIndex,
			   m_nEntityIndex, m_nModelIndex, m_bLowPriority ? 1 : 0);
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: %i cvars, \"%s\"=\"%s\"", GetName(), m_ConVars.Count(), m_ConVars[0].name,
			   m_ConVars[0].value);
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: table %i, changed %i, bytes %i", GetName(), m_nTableID, m_nChangedEntries,
			   Bits2Bytes(m_nLength));
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: table %s, entries %i, bytes %i userdatasize %i userdatabits %i", GetName(),
			   m_szTableName, m_nNumEntries, Bits2Bytes(m_nLength), m_nUserDataSize, m_nUserDataSizeBits);
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: number %i,%s bytes %i", GetName(), m_nNumSounds,
			   m_bReliableSound ? " reliable," : "", Bits2Bytes(m_nLength));
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: type %i index %i", GetName(), (int)m_fType, (int)m_nSoundIndex);
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: num %i, %s", GetName(), m_nNumServerClasses,
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: needs Decoder %s,bytes %i", GetName(), m_bNeedsDecoder ? "yes" : "no",
			   Bits2Bytes(m_nLength));
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: entity %i, class %i, bytes %i", GetName(), m_nEntityIndex, m_nClassID,
			   Bits2Bytes(m_nLength));
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: delta %i, max %i, changed %i,%s bytes %i", GetName(), m_nDeltaFrom,
			   m_nMaxEntries, m_nUpdatedEntries, m_bUpdateBaseline ? " BL update," : "", Bits2Bytes(m_nLength));
	return s_text;
}
...
{
	Q_snprintf(s_text, sizeof(s_text), "%s: %i \"%s\" (len:%i)", GetName(), m_Type,
