...
public:
	...
	// Non-persistent data
	mutable IReplayDownloadEventHandler
		*m_pDownloadEventHandler; // Implemented by replay browser - the reason we've got one per replay rather than
	...
	const char *m_pFileURL; // In the form <protocol>://<server address>:<port number>/path/file - points to the string
							// in the download group
	...
	int m_iMaxSessionBlockRequired; // The maximum session block required to reconstruct a viewable .dem file from spawn
									// tick until length
