...
public:
	...
	virtual void PlayReplay(ReplayHandle_t hReplay, int iPerformance,
							bool bPlaySound) = 0; // Play the given replay, from spawn tick to death tick
	...
	virtual void OnPlayerClassChanged() = 0; // Called when the player's class changes - we use this instead of an event
											 // for immediacy
	virtual void GetPlaybackTimes(
		float &flOutTime, float &flOutLength, const CReplay *pReplay,
		const CReplayPerformance
			*pPerformance) = 0; // Calculate the current time and length of a replay or performance - takes in tick and
								// out tick into account for performances - flCurTime should be gpGlobals->curtime.
								// pPerformance can be NULL.
