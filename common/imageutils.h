...
ConversionErrorType ImgUtl_ConvertJPEGToTGA(const char *jpgPath, const char *tgaPath,
											bool bRequirePowerOfTwoSize = true);
...
unsigned char *ImgUtl_ReadPNGAsRGBAFromBuffer(CUtlBuffer &buffer, int &width, int &height,
...
ConversionErrorType ImgUtl_WriteRGBAAsPNGToBuffer(const unsigned char *pRGBAData, int nWidth, int nHeight,
												  CUtlBuffer &bufOutData, int nStride = 0);
...
// as well.
//
ConversionErrorType ImgUtl_ConvertToVTFAndDumpVMT(const char *pInPath, const char *pMaterialsSubDir, int nMaxWidth = -1,
												  int nMaxHeight = -1);
...
ConversionErrorType ImgUtl_ResizeBitmap(Bitmap_t &destBitmap, int nWidth, int nHeight,
										const Bitmap_t *pImgSource = NULL);
