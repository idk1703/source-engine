...
{
	BufferSize = 256
}; ///< BufferSize is used extensively within the download system to size char buffers.
...
struct RequestContext_t
{
	...
	bool bIsBZ2;  ///< true if the file is a .bz2 file that should be uncompressed at the end of the download.  Set and
				  ///< used by main thread.
	...
	char serverURL[BufferSize]; ///< Server URL (IP:port, loopback, etc).  Set by main thread, and used for HTTP Referer
								///< header.
