...
#define FIXUP_OFFSETS(ptr, base, index)                                                                        \
	...
	//-----------------------------------------------------------------------------
	void ProcessFieldByName(void *pBaseAddress, void *pData, datamap_t *pDataMap, const char *pName,
							datadescProcessFunc_t pfnProcessFunc)
	{
		...
		for(int i = 0; i < fieldCount; ++i)
		{
			...
			{
				ProcessField(pBaseAddress, (BYTE *)pData + pField->fieldOffset[TD_OFFSET_NORMAL], pField,
							 pfnProcessFunc);
				break;
			}
		}
...
#pragma bitfield_order(pop)
	...
	int ByteswapVTX(void *pDestBase, const void *pSrcBase, const int fileSize)
	{
				...
				ITERATE_BLOCK(pModelLODHeader, pModelHeader->numLODs)
				{
					...
					ITERATE_BLOCK(pMeshHeader, pModelLODHeader->numMeshes)
					{
						...
						ITERATE_BLOCK(pStripGroupHeader, pMeshHeader->numStripGroups)
						{
							...
							WriteObjects<OptimizedModel::Vertex_t>(pDataDest, pDataSrc,
																   SrcNative(&pStripGroupHeader->numVerts));
							...
							ITERATE_BLOCK(pStripHeader, pStripGroupHeader->numStrips)
							{
								...
								WriteObjects<OptimizedModel::BoneStateChangeHeader_t>(
									pDataDest, pDataSrc, SrcNative(&pStripHeader->numBoneStateChanges));
							}
						}
					}
				}
		...
		ITERATE_BLOCK(pMatRepListHeader, pVtxHeader->numLODs)
		{
			...
			WriteObjects<OptimizedModel::MaterialReplacementHeader_t>(&pDataDest, &pDataSrc,
																	  SrcNative(&pMatRepListHeader->numReplacements));
		}
	...
	int ByteswapIKRules(studiohdr_t *&pHdrSrc, int numikrules, int numFrames, byte *&pDataSrc, byte *&pDataDest,
						int &fixedFileSize, const int fileSize)
	{
				...
				for(int idx = 0; idx < 6; ++idx)
				{
					if(pCompressed->offset[idx])
					{
						...
						while(numerror < totalerror)
						{
							// Write the first animation value (struct of 2 bytes)
							mstudioanimvalue_t *pDestAnimvalue =
								(mstudioanimvalue_t *)(g_bNativeSrc ? pAnimvalueSrc : pAnimvalueDest);
	...
	int ByteswapANIFile(studiohdr_t *pHdr, void *pDestBase, const void *pSrcBase, const int fileSize)
	{
		...
		for(int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc)
		{
			...
			if(pAnimDesc->sectionframes == 0)
			{
				...
				// printf("block %d : start %d + %d\n", pAnimDesc->animblock, pAnimBlock->datastart,
				// pAnimDesc->animindex ); Base address of the animblock
			...
			if(pAnimDesc->animblockikruleindex)
			{
				...
				ByteswapIKRules(pHdrSrc, pAnimDesc->numikrules, pAnimDesc->numframes, pDataSrc, pDataDest,
								fixedFileSize, fileSize);
			}
			...
			if(pAnimDesc->localhierarchyindex)
			{
				...
				{
					WriteObjects<mstudiolocalhierarchy_t>(pLocalHierarchyDest, pLocalHierarchySrc,
			...
			if(pStudioBone->procindex)
			{
				...
				switch(index)
				{
					...
					case STUDIO_PROC_QUATINTERP:
					{
						...
						WriteObjects<mstudioquatinterpinfo_t>(pDataDest, pDataSrc,
															  SrcNative(&pQuatInterpBone->numtriggers));
						break;
					}
		...
		ITERATE_BLOCK(pAnimDesc, pHdr->numlocalanim)
		{
			...
			if(pAnimDesc->animblock == 0)
			{
				...
				if(pAnimDesc->ikruleindex)
				{
					...
					ByteswapIKRules(pHdrSrc, pAnimDesc->numikrules, numframes, pDataSrc, pDataDest, fixedFileSize,
									fileSize);
				}
		...
		ITERATE_BLOCK(pSequence, pHdr->numlocalseq)
		{
			...
			if(pSequence->posekeyindex)
			{
				...
				WriteBuffer<float>(pDataDest, pDataSrc,
								   SrcNative(&pSequence->groupsize[0]) + SrcNative(&pSequence->groupsize[1]));
			}
			...
			{
				int numBoneWeights =
					(SrcNative(&pSequence->iklockindex) - SrcNative(&pSequence->weightlistindex)) / sizeof(float);
	  ...
	  // Don't need pupil textures
		...
		if(pHdr->studiohdr2index)
		{
			...
			ITERATE_BLOCK(pStudioHdr2, studiohdr2ct)
			{
				...
				{
					// Note, srcbonetransformindex is an offset from the start of the file, not the start of the
					// studiohdr2 as is the convention.  That's why the macros can't be used here.
					...
					WriteObjects<mstudiosrcbonetransform_t>(&pDataDest, &pDataSrc,
				...
				if(pStudioHdr2->m_nBoneFlexDriverIndex)
				{
					...
					ITERATE_BLOCK(pBoneFlexDriver, pStudioHdr2->m_nBoneFlexDriverCount)
					{
						...
						WriteObjects<mstudioboneflexdrivercontrol_t>(&pDataDest, &pDataSrc,
																	 SrcNative(&pBoneFlexDriver->m_nControlCount));
					}
				}
			}
		}
	...
	int ByteswapStudioFile(const char *pFilename, void *pOutBase, const void *pFileBase, int fileSize,
						   studiohdr_t *pHdr, CompressFunc_t pCompressFunc)
	{
		assert(pFilename);
	...
	void ProcessANIFields(void *pDataBase, datadescProcessFunc_t pfnProcessFunc)
	{
		...
		for(int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc)
		{
			...
			ProcessFieldByName(pBlockBase, pAnimDesc, &mstudioanimdesc_t::m_DataMap, "animblockikruleindex",
							   pfnProcessFunc);
		}
