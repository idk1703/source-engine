...
public:
	...
	void Setup(bool serverSide, const netadr_t *remote_address, CUDPSocket *sendSocket, char const *name,
...
private:
	...
	byte m_ReliableDataBuffer[8 * 1024]; // In SP, we don't need much reliable buffer, so save the memory (this is
										 // mostly for xbox).
