...
CUDPSocket::CUDPSocket() : m_socketIP(), m_Socket(INVALID_SOCKET), m_Port(0), m_pImpl(new CImpl)
{
...
bool CUDPSocket::SendTo(const netadr_t &recipient, const byte *data, size_t datalength)
{
	...
	// Send data
	int bytesSent = sendto(m_Socket, (const char *)data, (int)datalength, 0, reinterpret_cast<const sockaddr *>(&dest),
						   sizeof(dest));
