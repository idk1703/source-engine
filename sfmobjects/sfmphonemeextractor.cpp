...
static const char *s_pDefaultAttributeValueNames[LOG_PREVIEW_FLEX_CHANNEL_COUNT] = {"defaultValue", "defaultBalance",
																					"defaultMultilevel"};
...
private:
	...
	bool GetWaveFormat(const char *filename, CUtlBuffer *pFormat, int *pDataSize, CSentence &sentence,
					   bool &bGotSentence);
	void LogPhonemes(int nItemIndex, ExtractDesc_t &info);
	void ClearInterstitialSpaces(CDmeChannelsClip *pChannelsClip, CUtlDict<LogPreview_t *, int> &controlLookup,
								 ExtractDesc_t &info);

	void StampControlValueLogs(CDmePreset *preset, DmeTime_t tHeadPosition, float flIntensity,
	...
	void BuildPhonemeToPresetMapping(const CUtlVector<CBasePhonemeTag *> &stream, CDmeAnimationSet *pSet,
...
bool CSFMPhonemeExtractor::GetWaveFormat(const char *filename, CUtlBuffer *pBuf, int *pDataSize, CSentence &sentence,
										 bool &bGotSentence)
{
...
void CSFMPhonemeExtractor::Extract(const PE_APITYPE &apiType, ExtractDesc_t &info, bool bWritePhonemesToWavFiles)
{
	...
	for(nWorkItem = 0; nWorkItem < info.m_WorkList.Count(); ++nWorkItem)
	{
		...
		{
			extractor.extractor->Extract(pFullPath, (int)(workItem.m_flDuration * flSampleRate * flTrueSampleSize), Msg,
										 in, out);
...
void CSFMPhonemeExtractor::BuildPhonemeToPresetMapping(const CUtlVector<CBasePhonemeTag *> &stream,
													   CDmeAnimationSet *pSet, CDmePresetGroup *pPresetGroup,
													   CUtlDict<CDmePreset *, unsigned short> &phonemeToPresetDict)
{
	int i;
...
CDmeChannelsClip *CSFMPhonemeExtractor::FindFacialChannelsClip(const CUtlVector<LogPreview_t> &list)
{
	...
	for(i = list.Count() - 1; i >= 0; --i)
	{
		...
		CDmeChannelsClip *check =
			FindAncestorReferencingElement<CDmeChannelsClip>((CDmElement *)lp.m_hChannels[0].Get());
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::WriteDefaultValuesIntoLogLayers(DmeTime_t tHeadPosition,
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::WriteCurrentValuesIntoLogLayers(DmeTime_t tHeadPosition,
														   const CUtlDict<LogPreview_t *, int> &controlLookup)
{
	// Write a zero into all relevant log layers
	for(int j = controlLookup.First(); j != controlLookup.InvalidIndex(); j = controlLookup.Next(j))
...
//-----------------------------------------------------------------------------
void CSFMPhonemeExtractor::StampControlValueLogs(CDmePreset *preset, DmeTime_t tHeadPosition, float flIntensity,
												 CUtlDict<LogPreview_t *, int> &controlLookup)
{
	...
	for(int j = 0; j < controlValues.Count(); ++j)
	{
		...
		for(int chIndex = 0; chIndex < LOG_PREVIEW_FLEX_CHANNEL_COUNT; ++chIndex)
		{
			...
			// Whereas this control contains the "default" value for the slider (since the presetControl won't have that
			// value)
...
void CSFMPhonemeExtractor::ClearInterstitialSpaces(CDmeChannelsClip *pChannelsClip,
												   CUtlDict<LogPreview_t *, int> &controlLookup, ExtractDesc_t &info)
{
	Assert(info.m_pShot);
...
void AddAnimSetBookmarkAtSoundMediaTime(const char *pName, DmeTime_t tStart, DmeTime_t tEnd,
										const CUtlVector<CDmeHandle<CDmeClip>> &srcStack, ExtractDesc_t &info)
{
	...
	if(info.m_nFilterType == EXTRACT_FILTER_HOLD || info.m_nFilterType == EXTRACT_FILTER_LINEAR)
	{
		...
		for(i = 0; i < item.m_ApplyTags.Count(); ++i)
		{
			...
			if(info.m_nFilterType == EXTRACT_FILTER_HOLD)
			{
				...
				// NOTE - this ignores phoneme length, but since all phonemes directly abut one another, this doesn't
				// matter
