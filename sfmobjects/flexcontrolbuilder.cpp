...
static const char *s_pChannelControls[CONTROL_CHANNEL_ATTRIBUTE_COUNT] = {"channel", "valuechannel", "balancechannel",
																		  "multilevelchannel"};
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		if(!Q_strnicmp("right_", pFlexName, 6) && (i < nCount - 1))
		{
			...
			LeftRightToValueBalance(&controlInfo.m_pDefaultValue[CONTROL_VALUE],
									&controlInfo.m_pDefaultValue[CONTROL_BALANCE], leftInfo.m_flDefaultValue,
									info.m_flDefaultValue);
		...
		controlInfo.m_bIsMulti =
			(i + 1 < nCount) && !Q_strnicmp("multi_", m_FlexControllerInfo[i + 1].m_pFlexControlName, 6);
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::CleanupExistingFlexController(CDmeGameModel *pGameModel,
...
void CFlexControlBuilder::RemoveUnusedControlsAndChannels(CDmeAnimationSet *pAnimationSet,
														  CDmeChannelsClip *pChannelsClip)
{
	...
	for(int i = nControls - 1; i >= 0; --i)
	{
		...
		if(pControl)
		{
			...
			bRemoved = bRemoved ||
					   RemoveChannelIfUnused(pControl->GetValueElement<CDmeChannel>("valuechannel"), pChannelsClip);
			bRemoved = bRemoved ||
					   RemoveChannelIfUnused(pControl->GetValueElement<CDmeChannel>("balancechannel"), pChannelsClip);
			bRemoved = bRemoved || RemoveChannelIfUnused(pControl->GetValueElement<CDmeChannel>("multilevelchannel"),
														 pChannelsClip);
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::GetExistingMonoLog(ExistingLogInfo_t *pExistingLog, CDmeFilmClip *pClip,
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::ComputeChannelTimeTransform(DmeTime_t *pOffset, double *pScale,
													  CDmeChannelsClip *pChannelsClip)
{
...
bool CFlexControlBuilder::ComputeChannelTimeTransform(DmeTime_t *pOffset, double *pScale, CDmeFilmClip *pClip,
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::GetExistingVBLog(ExistingLogInfo_t *pLogs, CDmeFilmClip *pClip,
										   CDmeBalanceToStereoCalculatorOperator *pStereoOp)
{
	...
	for(; i != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID; i = g_pDataModel->NextAttributeReferencingElement(i))
	{
		...
		if(!ComputeChannelTimeTransform(&pLogs[nLogIndex].m_GlobalOffset, &pLogs[nLogIndex].m_flGlobalScale, pClip,
										pChannel))
			continue;
...
static void AddKeyToLogs(CDmeTypedLog<float> *valueLog, CDmeTypedLog<float> *balanceLog, const DmeTime_t &keyTime,
...
static void ConvertLRToVBLog(CDmeFloatLog *pValueLog, CDmeFloatLog *pBalanceLog, CDmeFloatLog *pLeftLog,
							 CDmeFloatLog *pRightLog, DmeTime_t rightOffset, double flRightScale)
{
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::ConvertExistingLRLogs(ExistingLogInfo_t *pLogs, CDmeFilmClip *pClip,
												CDmeChannel *pLeftChannel, CDmeChannel *pRightChannel)
{
	...
	pLogs[CONTROL_BALANCE].m_GlobalOffset =
		leftOffset; // NOTE: This is correct! All logs are transformed into left channel time
	pLogs[CONTROL_BALANCE].m_flGlobalScale = flLeftScale;

	ConvertLRToVBLog(pLogs[CONTROL_VALUE].m_pLog, pLogs[CONTROL_BALANCE].m_pLog, pLeftLog, pRightLog, rightOffset,
					 flRightScale);
...
void CFlexControlBuilder::GetExistingStereoLog(ExistingLogInfo_t *pLogs, CDmeFilmClip *pClip,
											   CDmeGlobalFlexControllerOperator *pRightOp,
											   CDmeGlobalFlexControllerOperator *pLeftOp)
{
	...
	// Sometimes a stereo op will be read from by this channel
	CDmeBalanceToStereoCalculatorOperator *pStereoOp =
		CastElement<CDmeBalanceToStereoCalculatorOperator>(pChannel->GetFromElement());
	if(pStereoOp)
	{
...
void CFlexControlBuilder::FixupExistingFlexControlLogList(CDmeFilmClip *pCurrentClip, CDmeGameModel *pGameModel)
{
	...
	for(int gi = 0; gi < nTrackGroups; ++gi)
	{
		...
		for(int ci = 0; ci < nChannels; ++ci)
		{
			...
			CDmeGlobalFlexControllerOperator *pOp =
				CastElement<CDmeGlobalFlexControllerOperator>(pChannel->GetToElement());
			if(!pOp)
				continue;
...
void CFlexControlBuilder::BuildExistingFlexControlLogList(CDmeFilmClip *pCurrentClip, CDmeGameModel *pGameModel)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		if(info.m_bIsStereo)
		{
			...
			CDmeGlobalFlexControllerOperator *pRightOp =
				pGameModel->FindGlobalFlexController(pRightInfo->m_nGlobalIndex);
		...
		if(info.m_bIsMulti)
		{
			...
			CDmeGlobalFlexControllerOperator *pMultiOp =
				pGameModel->FindGlobalFlexController(pMultiInfo->m_nGlobalIndex);
			if(pMultiOp)
			{
...
void CFlexControlBuilder::BuildFlexControllerOps(CDmeGameModel *pGameModel, CDmeChannelsClip *pChannelsClip,
												 ControlInfo_t &info, ControlField_t field)
{
	...
	CDmeGlobalFlexControllerOperator *pFlexControllerOp =
		pGameModel->AddGlobalFlexController(fcInfo.m_pFlexControlName, fcInfo.m_nGlobalIndex);
	...
	info.m_ppControlChannel[field] = pChannelsClip->CreatePassThruConnection(
		pName, info.m_pControl, flexInfo.m_pControlAttributeName, pFlexControllerOp, "flexWeight");
...
void CFlexControlBuilder::BuildStereoFlexControllerOps(CDmeAnimationSet *pAnimationSet, CDmeGameModel *pGameModel,
													   CDmeChannelsClip *pChannelsClip, ControlInfo_t &info)
{
	...
	for(int i = 0; i < 2; ++i)
	{
		...
		pChannelsClip->CreatePassThruConnection(pChannelName, pStereoCalcOp, pResultName, pFlexControllerOp,
												"flexWeight");
		...
		info.m_ppControlChannel[i] = pChannelsClip->CreatePassThruConnection(
			pChannelName, info.m_pControl, s_pStereoInputPrefix[i], pStereoCalcOp, s_pStereoInputPrefix[i]);
...
//-----------------------------------------------------------------------------
void CFlexControlBuilder::AttachControlsToGameModel(CDmeAnimationSet *pAnimationSet, CDmeGameModel *pGameModel,
													CDmeChannelsClip *pChannelsClip)
{
	// Build the infrastructure of the ops that connect that control to the dmegamemodel
...
void CFlexControlBuilder::CreateAnimationSetControls(CDmeFilmClip *pMovie, CDmeAnimationSet *pAnimationSet,
													 CDmeGameModel *pGameModel, CDmeFilmClip *pSourceClip,
													 CDmeChannelsClip *pDestClip, bool bUseExistingLogs)
{
	m_pMovie = pMovie;
