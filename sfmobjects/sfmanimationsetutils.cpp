...
static CDmeChannelsClip *CreateChannelsClip(CDmeAnimationSet *pAnimationSet, CDmeFilmClip *pOwnerClip)
{
	...
	CDmeChannelsClip *pChannelsClip =
		CreateElement<CDmeChannelsClip>(pAnimationSet->GetName(), pAnimationSet->GetFileId());
...
//-----------------------------------------------------------------------------
template<class T>
CDmeChannel *CreateConstantValuedLog(CDmeChannelsClip *channelsClip, const char *basename, const char *pName,
									 CDmElement *pToElement, const char *pToAttr, const T &value)
{
	char name[256];
...
//-----------------------------------------------------------------------------
static void CreateTransformChannels(CDmeTransform *pTransform, const char *pBaseName, int bi,
									CDmeChannelsClip *pChannelsClip)
{
	char name[256];
...
static void CreateAnimationLogs(CDmeChannelsClip *channelsClip, CDmeGameModel *pModel, studiohdr_t *pStudioHdr,
								const char *basename, int sequence, float flStartTime, float flDuration,
								float flTimeStep = 0.015f)
{
	Assert(pModel);
	Assert(pStudioHdr);
...
static void SetupBoneTransform(CDmeFilmClip *shot, CDmeChannelsClip *srcChannelsClip, CDmeChannelsClip *channelsClip,
							   CDmElement *control, CDmeGameModel *gameModel, const char *basename, studiohdr_t *hdr,
							   int bonenum, const char *boneName, bool bAttachToGameRecording)
{
...
//-----------------------------------------------------------------------------
static void SetupRootTransform(CDmeFilmClip *shot, CDmeChannelsClip *srcChannelsClip, CDmeChannelsClip *channelsClip,
							   CDmElement *control, CDmeGameModel *gameModel, const char *basename,
							   bool bAttachToGameRecording)
{
...
static void CreateProceduralPreset(CDmePresetGroup *pPresetGroup, const char *pPresetName,
								   const CDmaElementArray<CDmElement> &controls, bool bIdentity,
								   float flForceValue = 0.5f)
{
...
void LoadDefaultGroupMappings(CUtlDict<CUtlString, int> &defaultGroupMapping,
...
CDmElement *FindOrAddDefaultGroupForControls(const char *pGroupName, CDmaElementArray<CDmElement> &groups,
											 DmFileId_t fileid)
{
									 ...
									 const char *pAnimationSetName, int nSequenceToUse, bool bAttachToGameRecording)
{
	...
	// First the root transform
	{
		...
		SetupRootTransform(pShot, srcChannelsClip, pChannelsClip, ctrl, pGameModel, pAnimationSetName,
						   bAttachToGameRecording);
	}
	...
	// Now add the bone transforms as well
	{
		...
		for(int b = 0; b < numbones; ++b)
		{
			...
			ctrl->SetValue<bool>("transform", true);
			controls.AddToTail(ctrl);
			SetupBoneTransform(pShot, srcChannelsClip, pChannelsClip, ctrl, pGameModel, pAnimationSetName, hdr, b, name,
							   bAttachToGameRecording);
		}
	}
