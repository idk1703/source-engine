...
private:
	...
	bool SaveElement(CUtlBuffer &buf, CDmxSerializationDictionary &dict, CUtlRBTree<const char *> &stringTable,
	...
	void UnserializeElementArrayAttribute(CUtlBuffer &buf, CDmxAttribute *pAttribute,
										  CUtlVector<CDmxElement *> &elementList);
	bool UnserializeAttributes(CUtlBuffer &buf, CDmxElement *pElement, CUtlVector<CDmxElement *> &elementList,
...
//-----------------------------------------------------------------------------
void CDmxSerializer::SerializeElementAttribute(CUtlBuffer &buf, CDmxSerializationDictionary &list,
...
//-----------------------------------------------------------------------------
void CDmxSerializer::SerializeElementArrayAttribute(CUtlBuffer &buf, CDmxSerializationDictionary &list,
													CDmxAttribute *pAttribute)
{
...
//-----------------------------------------------------------------------------
bool CDmxSerializer::SaveElement(CUtlBuffer &buf, CDmxSerializationDictionary &list,
								 CUtlRBTree<const char *> &stringTable, CDmxElement *pElement)
{
...
//-----------------------------------------------------------------------------
void CDmxSerializer::UnserializeElementAttribute(CUtlBuffer &buf, CDmxAttribute *pAttribute,
												 CUtlVector<CDmxElement *> &elementList)
{
...
//-----------------------------------------------------------------------------
void CDmxSerializer::UnserializeElementArrayAttribute(CUtlBuffer &buf, CDmxAttribute *pAttribute,
													  CUtlVector<CDmxElement *> &elementList)
{
...
//-----------------------------------------------------------------------------
bool CDmxSerializer::UnserializeAttributes(CUtlBuffer &buf, CDmxElement *pElement,
...
bool SerializeDMX(CUtlBuffer &buf, CDmxElement *pRoot, const char *pFileName)
{
	...
	int nEncodingVersion =
		buf.IsText()
			? 1
			: 2; // HACK - we should have some way of automatically updating this when the encoding version changes!
	const char *pFormatName = GENERIC_DMX_FORMAT;
	int nFormatVersion =
		1; // HACK - we should have some way of automatically updating this when the encoding version changes!
	buf.Printf("%s encoding %s %d format %s %d %s\n", DMX_VERSION_STARTING_TOKEN, pEncodingName, nEncodingVersion,
			   pFormatName, nFormatVersion, DMX_VERSION_ENDING_TOKEN);
...
//-----------------------------------------------------------------------------
bool ReadDMXHeader(CUtlBuffer &buf, char *pEncodingName, int nEncodingNameLen, int &nEncodingVersion, char *pFormatName,
				   int nFormatNameLen, int &nFormatVersion)
{
	...
	{
#ifdef _WIN32
		int nAssigned = sscanf_s(header, "encoding %s %d format %s %d\n", pEncodingName, nEncodingNameLen,
								 &nEncodingVersion, pFormatName, nFormatNameLen, &nFormatVersion);
#else
		// sscanf considered harmful. We don't have POSIX 2008 support on OS X and "C11 Annex K" is optional...
		// (optional specs considered useful)
		char pTmpEncodingName[sizeof(header)] = {0};
		char pTmpFormatName[sizeof(header)] = {0};
		int nAssigned = sscanf(header, "encoding %s %d format %s %d\n", pTmpEncodingName, &nEncodingVersion,
							   pTmpFormatName, &nFormatVersion);
	...
	if(!bOk)
	{
		...
		bOk = buf.ParseToken(DMX_LEGACY_VERSION_STARTING_TOKEN, DMX_LEGACY_VERSION_ENDING_TOKEN, pFormatName,
							 sizeof(pFormatName));
		if(bOk)
		{
			nEncodingVersion = 0;
...
bool UnserializeDMX(CUtlBuffer &buf, CDmxElement **ppRoot, const char *pFileName)
{
	...
	*ppRoot = NULL;
	...
	if(!ReadDMXHeader(buf, pEncodingName, sizeof(pEncodingName), nEncodingVersion, pFormatName, sizeof(pFormatName),
					  nFormatVersion))
		return false;
