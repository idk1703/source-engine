...
public:
	...
	//-----------------------------------------------------------------------------
	static PreviewImageRetVal_t GetPreviewImageProperties(IMaterial *pMaterial, int *width, int *height,
														  ImageFormat *imageFormat, bool *isTranslucent)
	{
		...
		if(i == s_PreviewImagePropertiesCache.InvalidIndex())
		{
			...
			entry.m_RetVal = pMaterial->GetPreviewImageProperties(&entry.m_Width, &entry.m_Height, &entry.m_ImageFormat,
...
CUtlMap<IMaterial *, CPreviewImagePropertiesCache::CEntry> CPreviewImagePropertiesCache::s_PreviewImagePropertiesCache(
...
//-----------------------------------------------------------------------------
bool CMaterial::LoadMaterialsInDirectory(char const *pDirectoryName, int nDirectoryNameLen, IMaterialEnumerator *pEnum,
...
bool CMaterial::IsIgnoredMaterial(const char *pName)
{
	...
	if(!Q_strnicmp(pName, ".svn", 4) || strstr(pName, ".svn") || !Q_strnicmp(pName, "models", 6) ||
	   strstr(pName, "models"))
		return true;
...
void CMaterial::DrawBitmap(CDC *pDC, RECT &srcRect, RECT &dstRect)
{
	...
	if(StretchDIBits(pDC->m_hDC, dstRect.left, dstRect.top, dest_width, dest_height, srcRect.left, -srcRect.top,
					 srcWidth, srcHeight, m_pData, (BITMAPINFO *)&bmi, DIB_RGB_COLORS, SRCCOPY) == GDI_ERROR)
	{
...
//			dwFlags -
//-----------------------------------------------------------------------------
void CMaterial::Draw(CDC *pDC, RECT &rect, int iFontHeight, int iIconHeight,
...
bool CMaterial::LoadMaterialHeader(IMaterial *pMat)
{
	...
	ImageFormat eImageFormat;
	int width, height;
	retVal = CPreviewImagePropertiesCache::GetPreviewImageProperties(pMat, &width, &height, &eImageFormat,
																	 &translucentBaseTexture);
	if(retVal == MATERIAL_PREVIEW_IMAGE_BAD)
		return false;
...
static char const *s_rt_names[] = {"_rt_albedo", "_rt_normal",	 "_rt_position",
								   "_rt_flags",	 "_rt_accbuf_0", "_rt_accbuf_1"};
ImageFormat s_rt_formats[] = {IMAGE_FORMAT_RGBA32323232F, IMAGE_FORMAT_RGBA32323232F, IMAGE_FORMAT_RGBA32323232F,
							  IMAGE_FORMAT_RGBA32323232F, IMAGE_FORMAT_RGBA16161616F, IMAGE_FORMAT_RGBA16161616F};
...
void AllocateLightingPreviewtextures(void)
{
	...
	if(!bHaveAllocated)
	{
		...
		for(int idx = 0; idx < NELEMS(sg_ExtraFP16Targets); idx++)
			sg_ExtraFP16Targets[idx].Init(materials->CreateNamedRenderTargetTextureEx2(
				s_rt_names[idx], 512, 512, RT_SIZE_DEFAULT, s_rt_formats[idx], MATERIAL_RT_DEPTH_SHARED,
