...
static CClassNameFlagsMatcher s_ClassFlagsTable[] = {
	{"light_environment", ENTITY_FLAG_IS_LIGHT}, {"light", ENTITY_FLAG_IS_LIGHT},
	{"light_spot", ENTITY_FLAG_IS_LIGHT},		 {"prop_static", ENTITY_FLAG_SHOW_IN_LPREVIEW2},
...
// Input  : pClone -
//-----------------------------------------------------------------------------
void CMapEntity::OnPreClone(CMapClass *pClone, CMapWorld *pWorld, const CMapObjectList &OriginalList,
...
//			NewList -
//-----------------------------------------------------------------------------
void CMapEntity::OnClone(CMapClass *pClone, CMapWorld *pWorld, const CMapObjectList &OriginalList,
						 CMapObjectList &NewList)
{
...
//-----------------------------------------------------------------------------
void CMapEntity::OnPrePaste(CMapClass *pCopy, CMapWorld *pSourceWorld, CMapWorld *pDestWorld,
...
//			NewList -
//-----------------------------------------------------------------------------
void CMapEntity::OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld, CMapWorld *pDestWorld,
						 const CMapObjectList &OriginalList, CMapObjectList &NewList)
{
	if(IsNodeClass())
	{
		...
		if(pObject == m_pMoveParent)
		{
			...
			if((pWorld != NULL) && (pszParentName != NULL))
			{
				CMapEntity *pMoveParent =
					(CMapEntity *)UpdateDependency(m_pMoveParent, pWorld->FindEntityByName(pszParentName));
				SetMoveParent(pMoveParent);
			}
			else
			{
...
void CMapEntity::Render2D(CRender2D *pRender)
{
	...
	if((GetSelectionState() != SELECT_NONE) && (!GetClassName() || (strnicmp(GetClassName(), "light_", 6) != 0)) &&
...
void CMapEntity::GetRenderLogicalBox(Vector2D &mins, Vector2D &maxs)
{
	...
	maxs.x = m_vecLogicalPosition.x + LOGICAL_BOX_WIDTH + LOGICAL_BOX_CONNECTOR_INPUT_WIDTH +
...
void CMapEntity::RenderLogical(CRender2D *pRender)
{
	...
	color32 rgbColor = GetRenderColor(pRender);
	color32 rgbHighlight = {(byte)(7 * rgbColor.r / 8), (byte)(7 * rgbColor.g / 8), (byte)(7 * rgbColor.b / 8),
							(byte)255};
	color32 rgbLowlight = {(byte)(5 * rgbColor.r / 8), (byte)(5 * rgbColor.g / 8), (byte)(5 * rgbColor.b / 8),
						   (byte)255};
	color32 rgbEdgeColor = {(byte)(3 * rgbColor.r / 8), (byte)(3 * rgbColor.g / 8), (byte)(3 * rgbColor.b / 8),
							(byte)255};
	color32 rgbInterior = {(byte)(2 * rgbColor.r / 8), (byte)(2 * rgbColor.g / 8), (byte)(2 * rgbColor.b / 8),
						   (byte)255};
	...
	pRender->DrawRectangle(Vector(vecInnerMins.x, vecBoxMins.y, 0.0f), Vector(vecBoxMaxs.x, vecInnerMaxs.y, 0.0f), true,
						   0);
	...
	pRender->DrawRectangle(Vector(vecBoxMins.x, vecBoxMins.y, 0.0f), Vector(vecBoxMaxs.x, vecBoxMaxs.y, 0.0f), false,
						   0);
	...
	pRender->DrawRectangle(Vector(vecInnerMins.x, vecInnerMins.y, 0.0f), Vector(vecInnerMaxs.x, vecInnerMaxs.y, 0.0f),
						   true, 0);
	...
	if(!bFoundSpriteHelper)
	{
		...
		pRender->SetDrawColor(rgbEdgeColor.r, rgbEdgeColor.g, rgbEdgeColor.b);
		pRender->DrawRectangle(Vector(vecInnerMins.x, vecInnerMins.y, 0.0f),
							   Vector(vecInnerMaxs.x, vecInnerMaxs.y, 0.0f), false, 0);
	}
	...
	pRender->DrawCircle(Vector(vecMins.x + LOGICAL_BOX_CONNECTOR_RADIUS, flConnectorY, 0.0f),
	...
	if(pszTargetName != NULL)
	{
		pRender->DrawText(pszTargetName, Vector2D((vecMins.x + vecMaxs.x) / 2, vecMaxs.y), 0, -1,
	...
	if(pszClassName != NULL)
	{
		pRender->DrawText(pszClassName, Vector2D((vecMins.x + vecMaxs.x) / 2, vecMins.y), 0, 1,
