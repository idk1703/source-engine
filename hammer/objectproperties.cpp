...
// Purpose: Constructor.
//-----------------------------------------------------------------------------
CObjectProperties::CObjectProperties(void) : CPropertySheet()
{
	m_bDummy = false;
	m_pDummy = NULL;
	m_pInputButton = NULL;
...
void CObjectProperties::CreateButtons(void)
{
	...
	m_pInstanceButton->Create(_T( "Edit Instance" ), WS_CHILD | WS_VISIBLE | BS_TEXT,
							  CRect(6, rect.bottom - 28, 140, rect.bottom - 4), this, IDD_EDIT_INSTANCE);
}
...
BOOL CObjectProperties::SetupPages(void)
{
	...
	struct
	{
		...
		bool m_bWantVisible;
		CObjectPage *m_pPage;
	} pages[] = {{false, bEntity, m_pEntity}, {false, bEntity, m_pOutput}, {false, bEntity, m_pInput},
				 {false, bModel, m_pModel},	  {false, bFlags, m_pFlags},   {false, bGroups, m_pGroups}};
...
void CObjectProperties::UpdateAnchors(CWnd *pPage)
{
	...
	CAnchorDef anchorDefs[] = {CAnchorDef(IDOK, k_eSimpleAnchorBottomRight),
