...
GDinputvariable *Selection3D::ChooseEyedropperVar(CMapView *pView, CUtlVector<GDinputvariable *> &VarList)
{
	...
	CPoint point;
	GetCursorPos(&point);
	int nID = menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, point.x, point.y, NULL,
								  NULL);
	if(nID == 0)
	{
		return NULL;
...
bool Selection3D::OnContextMenu2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	if(!IsEmpty() && !IsBoxSelecting())
	{
		if(HitTest(pView, vPoint, false))
		{
			...
			menuSelection.TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_LEFTALIGN, ptScreen.x, ptScreen.y,
										 pView);
			return true;
		}
	}
...
void Selection3D::EyedropperPick3D(CMapView3D *pView, const Vector2D &vPoint)
{
	...
	{
		MessageBox(NULL, "No entities are selected, so the eyedropper has nothing to assign to.",
...
void Selection3D::EyedropperPick(CMapView *pView, CMapClass *pObject)
{
	...
	if(nCount <= 0)
	{
		...
		//
		MessageBox(NULL,
				   "No selected entities have keyvalues that accept an entity name, so the eyedropper has nothing to "
				   "assign to.",
