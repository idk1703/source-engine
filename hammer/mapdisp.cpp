...
bool CMapDisp::EntityInBoundingBox(Vector const &vOrigin)
{
	...
	if((vOrigin.x < vMin.x) || (vOrigin.x > vMax.x) || (vOrigin.y < vMin.y) || (vOrigin.y > vMax.y) ||
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapDisp::SamplePoints(int index, int width, int height, bool *pValidPoints, float *pValue, float *pAlpha,
...
void CMapDisp::DownSample(int oldPower)
{
	...
	for(int oh = 0, nh = 0; oh < oldHeight; oh += 2, nh++)
	{
		for(int ow = 0, nw = 0; ow < oldWidth; ow += 2, nw++)
		{
			...
			SamplePoints(oldIndex, oldWidth, oldHeight, validPoints, &newValue, &newAlpha, newDispVector, newSubdivPos,
						 newSubdivNormal);
...
#define SPLIT_EPSILON 0.001f
	...
	for(int iVert = 0; iVert < nVertCount; ++iVert)
	{
		...
		Vector2D vecDispUV;
		PointInQuadToBarycentric(vecSurfPoints[0], vecSurfPoints[3], vecSurfPoints[2], vecSurfPoints[1], vecVert,
								 vecDispUV);
...
void CMapDisp::CreatePlanesFromBoundingBox(Plane_t *planes, const Vector &bbMin, const Vector &bbMax)
{
	...
	//
	planes[0].normal[0] = -1;
	planes[0].dist = -bbMin[0];
	planes[1].normal[0] = 1;
	planes[1].dist = bbMax[0];

	planes[2].normal[1] = -1;
	planes[2].dist = -bbMin[1];
	planes[3].normal[1] = 1;
	planes[3].dist = bbMax[1];

	planes[4].normal[2] = -1;
	planes[4].dist = -bbMin[2];
	planes[5].normal[2] = 1;
	planes[5].dist = bbMax[2];
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapDisp::CollideWithBoundingBoxes(const Vector &rayStart, const Vector &rayEnd, BBox_t *pBBox, int bboxCount,
		...
		for(int j = 0; j < (postSpacing - 1); j++)
		{
			for(int k = 0; k < 4; k++)
			{
				switch(k)
				{
					case 0:
					{
						index = (postSpacing * i) + j;
						break;
					}
					case 1:
					{
						index = (postSpacing * (i + 1)) + j;
						break;
					}
					case 2:
					{
						index = (postSpacing * i) + (j + 1);
						break;
					}
					case 3:
					{
						index = (postSpacing * (i + 1)) + (j + 1);
						break;
					}
				}
				...
				if(v[0] < pBBox[bboxIndex].min[0])
				{
					pBBox[bboxIndex].min[0] = v[0];
				}
				if(v[1] < pBBox[bboxIndex].min[1])
				{
					pBBox[bboxIndex].min[1] = v[1];
				}
				if(v[2] < pBBox[bboxIndex].min[2])
				{
					pBBox[bboxIndex].min[2] = v[2];
				}

				if(v[0] > pBBox[bboxIndex].max[0])
				{
					pBBox[bboxIndex].max[0] = v[0];
				}
				if(v[1] > pBBox[bboxIndex].max[1])
				{
					pBBox[bboxIndex].max[1] = v[1];
				}
				if(v[2] > pBBox[bboxIndex].max[2])
				{
					pBBox[bboxIndex].max[2] = v[2];
				}
			}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapDisp::TraceLineSnapTo(Vector &HitPos, Vector &HitNormal, Vector const &RayStart, Vector const &RayEnd)
{
...
static void RenderDisplacementNormals(CCoreDispInfo &coreDispInfo, int numVerts)
{
	...
	for(int i = 0; i < numVerts; i++)
	{
		...
		meshBuilder.Color3f(0.0f, 1.0f, 0.0f);
		meshBuilder.Position3f(points[0][0] + (normal[0] * 10.0f), points[0][1] + (normal[1] * 10.0f),
...
static void RenderDisplacementTangentsS(CCoreDispInfo &coreDispInfo, int numVerts)
{
	...
	for(int i = 0; i < numVerts; i++)
	{
		...
		meshBuilder.Color3f(1.0f, 0.0f, 0.0f);
		meshBuilder.Position3f(points[0][0] + (tangentS[0] * 10.0f), points[0][1] + (tangentS[1] * 10.0f),
...
static void RenderDisplacementTangentsT(CCoreDispInfo &coreDispInfo, int numVerts)
{
	...
	for(int i = 0; i < numVerts; i++)
	{
		...
		meshBuilder.Color3f(0.0f, 0.0f, 1.0f);
		meshBuilder.Position3f(points[0][0] + (tangentT[0] * 10.0f), points[0][1] + (tangentT[1] * 10.0f),
...
static void RenderFaceVertexNormals(CCoreDispInfo &coreDispInfo)
{
	...
	for(int i = 0; i < 4; i++)
	{
		...
		meshBuilder.Color3f(1.0f, 0.0f, 0.0f);
		meshBuilder.Position3f(points[0][0] + (normal[0] * 25.0f), points[0][1] + (normal[1] * 25.0f),
...
static void RenderDisplacementVectorField(CCoreDispInfo &coreDispInfo, int numVerts)
{
	...
	for(int i = 0; i < numVerts; i++)
	{
		...
		meshBuilder.Color3f(1.0f, 1.0f, 0.0f);
		meshBuilder.Position3f(points[0][0] + (normal[0] * 50.0f), points[0][1] + (normal[1] * 50.0f),
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapDisp::CalcColor(CRender3D *pRender, bool bIsSelected, SelectionState_t faceSelectionState, Color &pColor)
{
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDisp::LoadDispOffsetNormalsKeyCallback(const char *szKey, const char *szValue, CMapDisp *pDisp)
{
	Vector normalVector;
		...
		if(eResult == ChunkFile_Ok)
		{
			...
			for(int nRow = 0; nRow < nRows; nRow++)
			{
				...
				for(int nCol = 0; nCol < nCols; nCol++)
				{
					...
					sprintf(szTemp, "%g %g %g", (double)vectorFieldVector[0], (double)vectorFieldVector[1],
							(double)vectorFieldVector[2]);
					strcat(szBuf, szTemp);
				}
		...
		if(eResult == ChunkFile_Ok)
		{
			...
			for(int nRow = 0; nRow < nRows; nRow++)
			{
				...
				for(int nCol = 0; nCol < nCols; nCol++)
				{
					...
					sprintf(szTemp, "%g %g %g", (double)subdivNormal[0], (double)subdivNormal[1],
							(double)subdivNormal[2]);
					strcat(szBuf, szTemp);
				}
...
bool CMapDisp::SerializedLoadMAP(std::fstream &file, CMapFace *pFace, UINT version)
{
	...
	if(version < 350)
	{
		sscanf(buf, "%d [ %f %f %f ] [ %f %f %f ] %f %d %f", &power, &m_MapAxes[0][0], &m_MapAxes[0][1],
			   &m_MapAxes[0][2], &m_MapAxes[1][0], &m_MapAxes[1][1], &m_MapAxes[1][2], &maxData, &minTess,
			   &smoothingAngle);
	}
	else
	{
		sscanf(buf, "%d [ %f %f %f ] [ %f %f %f ] %d %f", &power, &m_MapAxes[0][0], &m_MapAxes[0][1], &m_MapAxes[0][2],
			   &m_MapAxes[1][0], &m_MapAxes[1][1], &m_MapAxes[1][2], &minTess, &smoothingAngle);
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapDisp::PointSurfIntersection(Vector const &ptCenter, float radius, float &distMin, Vector &ptMin)
{
...
//-----------------------------------------------------------------------------
void EditDisp_ForUndo(EditDispHandle_t editHandle, char *pszPositionName, bool bNeighborsUndo)
{
