...
void CTextureConverter::ConvertFaceTexture(CMapFace *pFace)
{
	...
	char *replacements[][2] = {{"sky", "tools/toolsskybox"},		 {"skip", "tools/toolsskip"},
...
void CTextureConverter::RescaleFaceTexture(CMapFace *pFace, IEditorTexture *pNewTexture)
{
	...
	{
		MsgConvertFace(pFace, "WARNING: Invalid old texture dimensions (%dx%d).  Texture converted but not re-scaled.",
	...
	{
		MsgConvertFace(pFace, "WARNING: Invalid new material dimensions (%dx%d).  Texture converted but not re-scaled.",
...
void CTextureConverter::MsgConvertFace(CMapFace *pFace, const char *format, ...)
{
	...
	Msg(mwStatus, "[face] %s at (%d,%d,%d):  %s", pFace->GetTexture()->GetName(), (int)vecFaceCenter[0],
		(int)vecFaceCenter[1], (int)vecFaceCenter[2], message);
}
...
void CTextureConverter::MsgConvertDecal(CMapEntity *pEnt, const char *format, ...)
{
	...
	Msg(mwStatus, "[decal] %s at (%d,%d,%d):  %s", pEnt->GetKeyValue("texture"), (int)vecOrigin.x, (int)vecOrigin.y,
		(int)vecOrigin.z, message);
}
