...
{
	static class CGlobalHammerClipboard : public CHammerClipboard
	{
		virtual void Destroy()
		{
			Assert(0);
...
void CMapDoc::VisGroups_Validate()
{
	...
	while(pChild)
	{
		...
		if(((g_nFileFormatVersion < 100) && (!IsWorldObject(pParent))) || !VisGroups_ObjectCanBelongToVisGroup(pChild))
		{
			int nCount = pChild->GetVisGroupCount();
			if(nCount != 0)
			{
				Msg(mwWarning, "'%s', child of '%s', was in visgroups illegally. Removed from visgroups.",
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
// dvs: Seems better to move some of this functionality into DeleteObject and replace calls to this with
// FindEntity/DeleteObject calls.
//      Probably need to replicate all functions in the doc as follows: DeleteObject, DeleteObjectList to optimize
//      updates. Either that
...
//
//-----------------------------------------------------------------------------
bool CMapDoc::FindEntitiesByKeyValue(CMapEntityList &Found, const char *pszKey, const char *pszValue,
									 bool bVisiblesOnly)
{
...
void CMapDoc::OnViewGotoCoords()
{
	...
	while(dlg.DoModal() == IDOK)
	{
		...
		if(sscanf(dlg.m_string, "%f %f %f", &posVec.x, &posVec.y, &posVec.z) == 3)
		{
			...
			{
				if(AfxMessageBox("Seriously?\n\n(I mean, that was just an example, you were supposed to type in your "
								 "own numbers)",
		...
		if(sscanf(dlg.m_string, "%s %f %f %f%s %s %f %f %f", setposString, &posVec.x, &posVec.y, &posVec.z,
				  semicolonString, setangString, &angVec.x, &angVec.y, &angVec.z) == 9)
		{
...
void CMapDoc::OnMapShowSelectedBrushNumber()
{
	...
	m_pWorld->EnumChildrenRecurseGroupsOnly((ENUMMAPCHILDRENPROC)&CMapDoc::GetBrushNumberCallback, (DWORD)&info,
...
void CMapDoc::Postload(const char *pszFileName)
{
	...
	if(CMapSolid::GetBadSolidCount() > 0)
	{
		...
		V_sprintf_safe(szError,
					   "For your information, %d solid(s) were not loaded due to errors in the file. Would you like to "
					   "Re-Save your map with the invalid solids removed?",
...
//			pszEntityNamePrefix
//-----------------------------------------------------------------------------
void CMapDoc::ManifestPaste(CMapWorld *pDestWorld, Vector vecOffset, QAngle vecRotate, CMapClass *pParent,
							bool bMakeEntityNamesUnique, const char *pszEntityNamePrefix)
{
	Paste(GetHammerClipboard()->Objects, GetHammerClipboard()->pSourceWorld, pDestWorld, vecOffset, vecRotate, pParent,
		  bMakeEntityNamesUnique, pszEntityNamePrefix);
...
void CMapDoc::CollapseInstances(bool bOnlySelected)
{
	...
	if(AfxMessageBox("Collapsing does not perform all of the operations that the BSP process does for instancing.  "
					 "There may be some issues or differences between results.  Are you sure you want to do this?",
	...
	FOR_EACH_OBJ(*pChildren, pos)
	{
		...
		if(pEntity && stricmp(pEntity->GetClassName(), "func_instance") == 0 &&
		   (!bOnlySelected || pEntity->GetSelectionState() != SELECT_NONE))
		{
			CMapInstance *pMapInstance = pEntity->GetChildOfType((CMapInstance *)NULL);
			if(pMapInstance)
			{
				if(pMapInstance->GetInstancedMap())
				{
					...
					SetActiveMapDoc(this); // just in case the last instance copy forces the map to close, we need to
										   // make ourselves active again
					...
					PasteInstance(GetHammerClipboard()->Objects, pMapInstance->GetInstancedMap()->GetMapWorld(),
								  GetMapWorld(), origin, angles, NULL, true, temp);
					Update();
				}

				DeleteObject(pEntity);
				m_UpdateList.RemoveAll();
				SetActiveMapDoc(this); // just in case the last instance copy forces the map to close, we need to make
									   // ourselves active again
...
void CMapDoc::PopulateInstanceParms_r(CMapEntity *pEntity, const CMapObjectList *pChildren,
									  CUtlVector<CString> &ParmList)
{
	FOR_EACH_OBJ(*pChildren, pos)
	{
		...
		{
			for(int i = pInstanceEntity->GetFirstKeyValue(); i != pInstanceEntity->GetInvalidKeyValue();
				i = pInstanceEntity->GetNextKeyValue(i))
			{
...
//-----------------------------------------------------------------------------
void CMapDoc::VisGroups_CreateNamedVisGroup(CMapObjectList &Objects, const char *szName, bool bHide,
...
//-----------------------------------------------------------------------------
void CMapDoc::VisGroups_AddObjectsToVisGroup(CMapObjectList &Objects, CVisGroup *pVisGroup, bool bHide,
											 bool bRemoveFromOtherVisGroups)
{
	//
...
BOOL CMapDoc::OnSaveDocument(LPCTSTR lpszPathName)
{
	...
	if(access(lpszPathName, 0) != -1)
	{
		if(!CopyFile(lpszPathName, szFile, FALSE))
		{
			...
			wsprintf(szError,
					 "Hammer was unable to backup the existing file \"%s\" (Error: 0x%lX). Please verify that the "
					 "there is space on the hard drive and that the path still exists.",
	...
	if(!file.is_open())
	{
		char szError[_MAX_PATH];
		wsprintf(szError,
				 "Hammer was unable to open the file \"%s\" for writing. Please verify that the file is writable and "
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::BuildCascadingSelectionList(CMapClass *pObj, CUtlRBTree<CMapClass *, unsigned short> &list,
										  bool bRecursive)
{
...
void CMapDoc::OnEditApplytexture(void)
{
	...
	for(int i = 0; i < pSelList->Count(); i++)
	{
		...
		pobj->EnumChildren((ENUMMAPCHILDRENPROC)ApplyTextureToSolid, (DWORD)GetDefaultTextureName(),
						   MAPCLASS_TYPE(CMapSolid));
	}
...
void CMapDoc::Paste(IHammerClipboard *pClipboard, CMapWorld *pDestWorld, Vector vecOffset, QAngle vecRotate,
					CMapClass *pParent, bool bMakeEntityNamesUnique, const char *pszEntityNamePrefix)
{
	...
	Paste(pSrc->Objects, pSrc->pSourceWorld, pDestWorld, vecOffset, vecRotate, pParent, bMakeEntityNamesUnique,
		  pszEntityNamePrefix);
}
...
//			pParent -
//-----------------------------------------------------------------------------
void CMapDoc::Paste(CMapObjectList &Objects, CMapWorld *pSourceWorld, CMapWorld *pDestWorld, Vector vecOffset,
					QAngle vecRotate, CMapClass *pParent, bool bMakeEntityNamesUnique, const char *pszEntityNamePrefix)
{
	//
...
//			pParent -
//-----------------------------------------------------------------------------
void CMapDoc::PasteInstance(CMapObjectList &Objects, CMapWorld *pSourceWorld, CMapWorld *pDestWorld, Vector vecOffset,
...
void CMapDoc::OnEditPaste(void)
{
	...
	Paste(GetHammerClipboard()->Objects, GetHammerClipboard()->pSourceWorld, pWorld, vecPasteOffset, QAngle(0, 0, 0),
		  NULL, false, NULL);
...
void CMapDoc::OnToolsGroup(void)
{
	...
	for(int i = 0; i < pSelList->Count(); i++)
	{
		...
		{
			if(GetMainWnd()->MessageBox("Some selected objects are part of an entity or belong to a group. Grouping "
										"them now will remove them from the entity or group! Continue?",
	...
	do
	{
		...
		if(access(str, 0) != -1)
		{
			...
			if(access(str, 2) == -1)
			{
				...
				wsprintf(szConfirm, "The file %s is read-only. You must change the file's attributes to overwrite it.",
						 (const char *)str);
...
public:
	CExportDlg(CString &strFile, LPCTSTR pszExt, LPCTSTR pszDesc)
		: CFileDialog(FALSE, pszExt, strFile,
					  OFN_NOCHANGEDIR | OFN_LONGNAMES | OFN_HIDEREADONLY | OFN_EXPLORER | OFN_ENABLETEMPLATE, pszDesc)
	{
...
void CMapDoc::OnFileExport(void)
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		{
			if(GetMainWnd()->MessageBox("You are about to export over your current work file. Some data loss will "
										"occur if you have any objects hidden. Continue?",
...
void CMapDoc::OnFileRunmap(void)
{
	...
	if((dlg.m_iLight) && (m_pGame->szLIGHT[0] != '\0'))
	{
		...
		sprintf(cmd.szParms, "%s -game $gamedir %s$path\\$file", dlg.m_bHDRLight ? "-both" : "",
	...
	if(dlg.m_bNoQuake == FALSE)
	{
		...
		// When running under steam, use applaunch always because if a user forgets to run the game they're trying to
		// launch, it will fail with an obscure error they won't understand.
		{
...
void CMapDoc::UpdateForApplicator(BOOL bApplicator)
{
	if(bApplicator)
	{
		...
		for(int i = 0; i < pSelList->Count(); i++)
		{
			...
			pObject->EnumChildren((ENUMMAPCHILDRENPROC)AddLeavesToListCallback, (DWORD)&Solids,
								  MAPCLASS_TYPE(CMapSolid));
		}
...
void CMapDoc::OnToolsHollow(void)
{
	...
	CStrDlg dlg(CStrDlg::Spin, szBuf, "How thick do you want the walls? Use a negative number to hollow outward.",
				"Hammer");
...
void CMapDoc::OnEditPastespecial(void)
{
	...
	for(int i = 0; i < dlg.m_iCopies; i++)
	{
		...
		//
		Paste(GetHammerClipboard()->Objects, GetHammerClipboard()->pSourceWorld, pWorld, vecPasteOffset, vecPasteAngles,
			  pParent, bMakeNamesUnique, pszPrefix);
...
{
	if(GetMainWnd()->MessageBox("This will place one of every possible point entity in the current map! Performing "
								"this operation in an empty map is recommended. Continue?",
...
{
	CString str;
	CFileDialog dlg(FALSE, NULL, str, OFN_LONGNAMES | OFN_NOCHANGEDIR | OFN_HIDEREADONLY,
...
void CMapDoc::OnToolsCreateprefab(void)
{
	if(m_pSelection->IsEmpty())
	{
		AfxMessageBox("This feature creates a prefab with the selected objects. You must select some objects before "
					  "you can use it.",
							  ...
							  bool bRescaleTextureCoordinates)
{
	...
	else
	{
		str.Format("%d %s marked.", info.nReplaced,
				   (m_pToolManager->GetActiveToolID() == TOOL_FACEEDIT_MATERIAL) ? "faces" : "solids");
	}
...
void CMapDoc::BatchReplaceTextures(FileHandle_t fp)
{
	...
	while(g_pFullFileSystem->ReadLine(buf, sizeof(buf), fp))
	{
		...
		m_pWorld->EnumChildren((ENUMMAPCHILDRENPROC)BatchReplaceTextureCallback, (DWORD)&Info,
							   MAPCLASS_TYPE(CMapSolid));
	next_line:;
	}
}
...
void CMapDoc::OnToolsToggletexlockScale(void)
{
	...
	SetStatusText(SBI_PROMPT,
				  Options.IsScaleLockingTextures() ? "Scale texture locking on" : "Scale texture locking off");
}
...
void CMapDoc::OnMapLoadportalfile(void)
{
	...
	{
		CFileDialog dlg(TRUE, ".prt", m_pPortalFile->fileName, OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR,
...
void CMapDoc::OnMapLoadpointfile(void)
{
	...
	if(GetFileAttributes(m_strLastPointFile) == 0xFFFFFFFF || AfxMessageBox(str, MB_ICONQUESTION | MB_YESNO) == IDNO)
	{
		CFileDialog dlg(TRUE, ".pts", m_strLastPointFile, OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR,
...
//-----------------------------------------------------------------------------
// Purpose: Toggles the state of Tools | Instances | Show Normal. When enabled, instances are shown just like the rest
// of the world.
//-----------------------------------------------------------------------------
	...
	if(pGD != NULL)
	{
		...
		if(pEntity != NULL)
		{
			...
			for(gindex = 0; gindex < pGD->m_FGDAutoVisGroups.Count(); gindex++)
			{
				...
				{
					// Object/Entity
					for(eindex = 0; eindex < pGD->m_FGDAutoVisGroups[gindex].m_Classes[cindex].szEntities.Count();
						eindex++)
					{
						//
						if(!V_stricmp(pEntity->GetClassName(),
									  pGD->m_FGDAutoVisGroups[gindex].m_Classes[cindex].szEntities[eindex]))
						{
							AddChildGroupToAutoVisGroup(pEntity,
														pGD->m_FGDAutoVisGroups[gindex].m_Classes[cindex].szClass,
														pGD->m_FGDAutoVisGroups[gindex].szParent);
						}
					}
				}
			}
