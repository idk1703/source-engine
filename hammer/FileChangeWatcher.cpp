...
bool CFileChangeWatcher::AddDirectory(const char *pSearchPathBase, const char *pDirName, bool bRecursive)
{
	...
	HANDLE hDir = CreateFile(fullDirName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
...
int CFileChangeWatcher::Update()
{
	...
	while(i < m_DirWatches.Count())
	{
		...
		for(int iQueuedChange = queuedChanges.First(); iQueuedChange != queuedChanges.InvalidIndex();
...
{
	return ReadDirectoryChangesW(pDirWatch->m_hDir, pDirWatch->m_Buffer, sizeof(pDirWatch->m_Buffer), true,
