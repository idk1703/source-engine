...
bool GetRequiredMaterial(const char *pName, IMaterial *&pMaterial)
{
	...
	else
	{
		...
		Q_snprintf(str, sizeof(str),
				   "Missing material '%s'. Go to Tools | Options | Game Configurations and verify that your game "
...
// Purpose:
//-----------------------------------------------------------------------------
CRender3D::CRender3D(void) : CRender()
{
...
void CRender3D::BeginRenderHitTarget(CMapAtom *pObject, unsigned int uHandle)
{
	...
	if((m_Pick.m_nFlags & FLAG_OBJECTS_AT_RESOLVE_INSTANCES) == 0 && m_bInstanceRendering &&
	   !GetInstanceClass()->IsEditable())
	{
...
void CRender3D::ComputeFrustumRenderGeometry(CCamera *pCamera)
{
	...
	for(i = 0; i < 4; ++i)
	{
		CrossProduct(m_FrustumPlanes[edgeIdx[i][0]].AsVector3D(), m_FrustumPlanes[edgeIdx[i][1]].AsVector3D(),
					 edges[i]);
...
void CRender3D::StartRenderFrame(void)
{
	...
	if(m_Pick.bPicking)
	{
		...
		pRenderContext->PerspectiveX(pCamera->GetFOV(), aspect, pCamera->GetNearClip(), pCamera->GetFarClip());
	}
	else
	{
		//
...
static void AddEntityLightToLightList(CMapEntity *e, CUtlVector<CLightingPreviewLightDescription> &listout)
{
...
void CRender3D::EndRenderFrame(void)
{
	...
	if(!m_Pick.bPicking)
	{
		   ...
		   (m_eCurrentRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED))
		{
			...
			if((pCamera->GetYaw() != m_fLastLPreviewAngles[0]) || (pCamera->GetPitch() != m_fLastLPreviewAngles[1]) ||
			   (pCamera->GetRoll() != m_fLastLPreviewAngles[2]) || (m_nLastLPreviewHeight != height) ||
			   (m_nLastLPreviewWidth != width) || (new_vp != m_LastLPreviewCameraPos) ||
			   ...
			   (!g_pLPreviewOutputBitmap))
			{
				...
				for(int i = 0; i < lightList.Count(); i++)
				{
					...
					if((pLight->m_Type == MATERIAL_LIGHT_SPOT) || (pLight->m_Type == MATERIAL_LIGHT_POINT))
					{
						Vector lpnt;
				...
				for(int i = 0; i < nlights; i++)
				{
					...
					pRenderContext->DrawScreenSpaceRectangle(src_mat, 0, 0, nTargetWidth, nTargetHeight, 0, 0,
															 nTargetWidth - 1, nTargetHeight - 1,
															 dest_rt->GetActualWidth(), dest_rt->GetActualHeight());
					V_swap(dest_rt_current, dest_rt_other);
					V_swap(sample_last, sample_other);
					V_swap(add_0_to_1, add_1_to_0);
				}
				pRenderContext->SetRenderTarget(NULL);
				pRenderContext->DrawScreenSpaceRectangle(sample_last, xl, yl, dest_width, dest_height, 0, 0,
														 nTargetWidth, nTargetHeight, dest_rt->GetActualWidth(),
		...
		if(g_bShowStatistics)
		{
			...
			int nLen =
				sprintf(szText, "FPS=%3.2f Pos=[%.f %.f %.f]", m_fFrameRate, ViewPoint[0], ViewPoint[1], ViewPoint[2]);
...
void CRender3D::RenderTranslucentObjects(void)
{
	...
	while(m_TranslucentRenderObjects.Count() > 0)
	{
		...
		{
			if(pInstanceClass != current.m_InstanceState.m_pInstanceClass || !m_bInstanceRendering ||
			   current.m_InstanceState.m_InstanceMatrix != m_CurrentInstanceState.m_InstanceMatrix)
			{
				if(bAddedTransform)
				{
					EndLocalTransfrom();
...
void CRender3D::Render(void)
{
	...
	if((m_eCurrentRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED) && g_pLPreviewOutputBitmap && (!view_changed))
	{
		// blit it
		BITMAPINFOHEADER mybmh;
		mybmh.biHeight = -g_pLPreviewOutputBitmap->Height();
...
//-----------------------------------------------------------------------------
void CRender3D::RenderArrow(Vector const &vStartPt, Vector const &vEndPt, unsigned char chRed, unsigned char chGreen,
...
//			chBlue -
//-----------------------------------------------------------------------------
void CRender3D::RenderBox(const Vector &Mins, const Vector &Maxs, unsigned char chRed, unsigned char chGreen,
						  unsigned char chBlue, SelectionState_t eBoxSelectionState)
{
	Vector FacePoints[8];
	...
	for(int nPass = 1; nPass <= nPasses; nPass++)
	{
		if(nPass == 1)
		{
			...
			if((eRenderModeThisPass == RENDER_MODE_TEXTURED) || (eRenderModeThisPass == RENDER_MODE_TEXTURED_SHADED) ||
		...
		for(int nFace = 0; nFace < 6; nFace++)
		{
			...
			CMatRenderContextPtr pRenderContext(MaterialSystemInterface());
			IMesh *pMesh = pRenderContext->GetDynamicMesh();
			meshBuilder3D.DrawQuad(pMesh, FacePoints[nP1].Base(), FacePoints[nP2].Base(), FacePoints[nP3].Base(),
...
//-----------------------------------------------------------------------------
void CRender3D::RenderCone(Vector const &vBasePt, Vector const &vTipPt, float fRadius, int nSlices, unsigned char chRed,
...
//			chBlue -
//-----------------------------------------------------------------------------
void CRender3D::RenderSphere(Vector const &vCenter, float flRadius, int nTheta, int nPhi, unsigned char chRed,
							 unsigned char chGreen, unsigned char chBlue)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CRender3D::RenderWireframeSphere(Vector const &vCenter, float flRadius, int nTheta, int nPhi, unsigned char chRed,
									  unsigned char chGreen, unsigned char chBlue)
{
	PushRenderMode(RENDER_MODE_WIREFRAME);
	...
	for(int i = 0; i < nPhi; ++i)
	{
		for(int j = 0; j < nTheta; ++j)
		{
			...
			meshBuilder3D.Position3f(vCenter.x + (flRadius * sin(phi) * cos(theta)),
									 vCenter.y + (flRadius * sin(phi) * sin(theta)), vCenter.z + (flRadius * cos(phi)));
...
//-----------------------------------------------------------------------------
void CRender3D::RenderWireframeBox(const Vector &Mins, const Vector &Maxs, unsigned char chRed, unsigned char chGreen,
								   unsigned char chBlue)
{
	//
...
// Output : none
//-----------------------------------------------------------------------------
void CRender3D::RenderInstanceMapClass(CMapInstance *pInstanceClass, CMapClass *pMapClass, Vector &InstanceOrigin,
