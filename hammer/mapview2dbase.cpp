...
{
public:
	CMapView2DBasePanel(CMapView2DBase *pMapView, const char *panelName) : vgui::EditablePanel(NULL, panelName)
	{
		m_pMapView = pMapView;
	}
...
void CMapView2DBase::AdjustColorIntensity(Color &color, int nIntensity)
{
	...
	//
	color.SetColor(min((color.r() * nIntensity) / 100, 255), min((color.g() * nIntensity) / 100, 255),
				   min((color.b() * nIntensity) / 100, 255), color.a());
}
...
{
	if((fabs(vecRef.x - vecCheck.x) <= nDist) && (fabs(vecRef.y - vecCheck.y) <= nDist))
	{
		return true;
	}
...
BOOL CMapView2DBase::PreCreateWindow(CREATESTRUCT &cs)
{
	...
	{
		className = AfxRegisterWndClass(CS_BYTEALIGNCLIENT | CS_DBLCLKS, AfxGetApp()->LoadStandardCursor(IDC_ARROW),
										HBRUSH(NULL));
	}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
int CMapView2DBase::ObjectsAt(CMapWorld *pWorld, const Vector2D &vPoint, HitInfo_t *pHitData, int nMaxObjects,
							  unsigned int nFlags)
{
	int nIndex = 0;
...
BOOL CMapView2DBase::OnMouseWheel(UINT nFlags, short zDelta, CPoint point)
{
	...
	if(pTool)
	{
		if(IsLogical())
		{
			if(pTool->OnMouseWheelLogical(static_cast<CMapViewLogical *>(this), nFlags, zDelta,
