		...
		if(arg == NUM_KEY_VALUE_ARGS)
		{
			...
			if(pEntity != NULL)
			{
				...
				else if(pEntity->IsSolidClass() && !Q_stricmp(szArgs[4], "angles"))
				{
					...
					// NOTE: This only works once since solid classes destructively modify transform info (GetAngles
					// always returns identity) NOTE: Use rotateIncremental instead!
					QAngle oldAngles;
...
bool CShell::RunCommand(const char *pszCommand)
{
	for(int nCommand = 0; nCommand < sizeof(m_DispatchTable) / sizeof(m_DispatchTable[0]); nCommand++)
	{
		...
		{
			return ((this->*m_DispatchTable[nCommand].pfnHandler)(m_DispatchTable[nCommand].pszCommand,
																  &pszCommand[nCommandLen]));
		}
	}
