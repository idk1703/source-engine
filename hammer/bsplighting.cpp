...
{
	return (flags & SURF_BUMPLIGHT) && (!(flags & SURF_NOLIGHT));
}
...
bool CBSPLighting::Load(char const *pFilename)
{
	...
	for(int iFace = 0; iFace < file.numfaces; iFace++)
	{
		...
		// Setup its lightmap.
		memcpy(pOut->m_LightmapVecs, file.texinfo[pIn->texinfo].lightmapVecsLuxelsPerWorldUnits,
			   sizeof(pOut->m_LightmapVecs));
		memcpy(pOut->m_LightmapTextureMinsInLuxels, pIn->m_LightmapTextureMinsInLuxels,
			   sizeof(pOut->m_LightmapTextureMinsInLuxels));
	...
	FOR_EACH_LL(m_FaceMaterials, iMat)
	{
		...
		FOR_EACH_LL(pMat->m_Faces, iFace)
		{
			...
			pOut->m_LightmapSortID = pMatSys->AllocateLightmap(
				bumpedSize, pStoredFace->m_LightmapSize[1], pStoredFace->m_OffsetIntoLightmapPage, pMat->m_pMaterial);
		}
	}
	...
	for(int iFace = 0; iFace < faces.Size(); iFace++)
	{
		...
		pMatSys->GetLightmapPageSize(pFace->m_pStoredFace->m_LightmapPageID, &lightmapPageSize[0],
									 &lightmapPageSize[1]);
		...
		for(int iVert = 0; iVert < pFace->m_nVerts; iVert++)
		{
			...
			for(int iCoord = 0; iCoord < 2; iCoord++)
			{
				...
				float flVal = lmVec[0] * vPos[0] + lmVec[1] * vPos[1] + lmVec[2] * vPos[2] + lmVec[3] -
							  pFace->m_LightmapTextureMinsInLuxels[iCoord];
...
void DrawLightmapPage(IMaterialSystem *materialSystemInterface, int lightmapPageID)
{
	IMaterial *g_materialDebugLightmap =
		materialSystemInterface->FindMaterial("debug/debuglightmap", TEXTURE_GROUP_OTHER);
...
void CBSPLighting::AssignFaceMaterialCounts(CBSPInfo &file, CUtlVector<CFace> &faces)
{
...
VertexFormat_t CBSPLighting::ComputeLMGroupVertexFormat(IMaterial *pMaterial)
{
	...
	// FIXME: check for and strip unused vertex elements (bone weights+indices, TANGENT_S/T?) - requires reliable
	// material vertex formats first

	return vertexFormat;
}

void CBSPLighting::BuildLMGroups(CBSPInfo &file, CUtlVector<CFace> &faces, CUtlVector<CVert> &verts,
								 CUtlVector<CDispInfoFaces> &dispInfos)
{
		...
		FOR_EACH_LL(pMat->m_MaterialBufs, iBuf)
		{
			...
			FOR_EACH_LL(pBuf->m_Faces, iFace)
			{
				...
				if(pFace->m_iDispInfo == -1)
				{
					...
					{
						mb.Index(iCurBaseVert);
						mb.AdvanceIndex();
						mb.Index(iCurBaseVert + iTri + 1);
						mb.AdvanceIndex();
						mb.Index(iCurBaseVert + iTri + 2);
						mb.AdvanceIndex();
					}
...
void CBSPLighting::ReloadLightmaps()
{
	...
	for(int iFace = 0; iFace < m_StoredFaces.Count(); iFace++)
	{
		...
		if(bNeedsBumpmap)
		{
			pMatSys->UpdateLightmap(pFace->m_LightmapPageID, pFace->m_LightmapSize, pFace->m_OffsetIntoLightmapPage,
									(float *)blocklights[0], (float *)blocklights[1], (float *)blocklights[2],
									(float *)blocklights[3]);
		}
		else
		{
