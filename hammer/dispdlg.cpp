...
// Purpose:  constructor
//-----------------------------------------------------------------------------
CDispCreateDlg::CDispCreateDlg(CWnd *pParent) : CDialog(CDispCreateDlg::IDD, pParent)
{
	m_Power = 0;
}
...
// Purpose:  constructor
//-----------------------------------------------------------------------------
CDispNoiseDlg::CDispNoiseDlg(CWnd *pParent) : CDialog(CDispNoiseDlg::IDD, pParent)
{
	m_Min = m_Max = 0.0f;
}
...
BOOL CDispPaintDistDlg::OnInitDialog(void)
{
	...
	{
		SetWindowPos(&wndTop, m_DialogPosRect.left, m_DialogPosRect.top, m_DialogPosRect.Width(),
					 m_DialogPosRect.Height(), SWP_NOZORDER);
	}
...
void CDispPaintDistDlg::FilterComboBoxBrushGeo(unsigned int nEffect, bool bInit)
{
	...
	if(pTool)
	{
		...
		switch(nEffect)
		{
			case DISPPAINT_EFFECT_RAISELOWER:
			{
				pFilterMgr = pTool->GetFilterRaiseLowerMgr();
				break;
			}
			case DISPPAINT_EFFECT_RAISETO:
			{
				pFilterMgr = pTool->GetFilterRaiseToMgr();
				break;
			}
			case DISPPAINT_EFFECT_SMOOTH:
			{
				pFilterMgr = pTool->GetFilterSmoothMgr();
				break;
			}
			default:
				return;
...
void CDispPaintDistDlg::UpdateAxis(int nAxis)
{
	...
	switch(nAxis)
	{
		case DISPPAINT_AXIS_X:
		{
			pTool->SetPaintAxis(nAxis, Vector(1.0f, 0.0f, 0.0f));
			return;
		}
		case DISPPAINT_AXIS_Y:
		{
			pTool->SetPaintAxis(nAxis, Vector(0.0f, 1.0f, 0.0f));
			return;
		}
		case DISPPAINT_AXIS_Z:
		{
			pTool->SetPaintAxis(nAxis, Vector(0.0f, 0.0f, 1.0f));
			return;
		}
		case DISPPAINT_AXIS_SUBDIV:
		{
			pTool->SetPaintAxis(nAxis, Vector(0.0f, 0.0f, 0.0f));
			return;
		}
		case DISPPAINT_AXIS_FACE:
		{
			pTool->SetPaintAxis(nAxis, Vector(0.0f, 0.0f, 1.0f));
			return;
		}
...
void CDispPaintDistDlg::UpdateSliderDistance(float flDistance, bool bForceInit)
{
	if((flDistance != m_flPrevDistance) || bForceInit)
	{
		...
		// clamp
		if(nDistance < DISPPAINT_DISTANCE_MIN)
		{
			nDistance = DISPPAINT_DISTANCE_MIN;
		}
		if(nDistance > DISPPAINT_DISTANCE_MAX)
		{
			nDistance = DISPPAINT_DISTANCE_MAX;
		}
...
void CDispPaintDistDlg::UpdateSliderRadius(float flRadius, bool bForceInit)
{
	if((flRadius != m_flPrevRadius) || bForceInit)
	{
		...
		// clamp
		if(nRadius < DISPPAINT_SPATIALRADIUS_MIN)
		{
			nRadius = DISPPAINT_SPATIALRADIUS_MIN;
		}
		if(nRadius > DISPPAINT_SPATIALRADIUS_MAX)
		{
			nRadius = DISPPAINT_SPATIALRADIUS_MAX;
		}
...
// Purpose:  constructor
//-----------------------------------------------------------------------------
CPaintSculptDlg::CPaintSculptDlg(CWnd *pParent) : CDialog(CPaintSculptDlg::IDD, pParent)
{
	m_bAutoSew = true;
...
void CDispPaintDataDlg::FilterComboBoxBrushData(unsigned int uiEffect, bool bInit)
{
	...
	if(pDispTool)
	{
		...
		switch(uiEffect)
		{
			case DISPPAINT_EFFECT_RAISELOWER:
			{
				pFilterMgr = pDispTool->GetFilterRaiseLowerMgr();
				break;
			}
			case DISPPAINT_EFFECT_RAISETO:
			{
				pFilterMgr = pDispTool->GetFilterRaiseToMgr();
				break;
			}
			case DISPPAINT_EFFECT_SMOOTH:
			{
				pFilterMgr = pDispTool->GetFilterSmoothMgr();
				break;
			}
			default:
				return;
