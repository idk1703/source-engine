...
public:
	...
	void CalculateForLightTask(int nLineMask, int nLineMatch, CLightingPreviewLightDescription &l, int calc_mask,
...
bool CIncrementalLightInfo::IsHighPriority(CLightingPreviewThread *pLPV) const
{
	...
	if(m_eIncrState == INCR_STATE_NEW)
	{
		...
		if(m_pLight->m_Position.WithinAABox(pLPV->m_MinViewCoords, pLPV->m_MaxViewCoords))
			return true;
	}
	return false;
}
...
{
	m_Albedos.CreateFromRGBA_FloatImageData(msg_in.m_pDefferedRenderingBMs[0]->Width,
											msg_in.m_pDefferedRenderingBMs[0]->Height,
											msg_in.m_pDefferedRenderingBMs[0]->RGBAData);
	m_Normals.CreateFromRGBA_FloatImageData(msg_in.m_pDefferedRenderingBMs[1]->Width,
											msg_in.m_pDefferedRenderingBMs[1]->Height,
											msg_in.m_pDefferedRenderingBMs[1]->RGBAData);
	m_Positions.CreateFromRGBA_FloatImageData(msg_in.m_pDefferedRenderingBMs[2]->Width,
	...
	for(int i = 0; i < m_pLightList->Count(); i++)
	{
		...
		if((l_info->m_fTotalContribution > 0.0) && (l_info->m_eIncrState >= INCR_STATE_PARTIAL_RESULTS))
		{
			...
			for(int y = 0; y < m_ResultImage.m_nHeight; y++)
			{
				...
				int src_y = (y & ~(N_INCREMENTAL_STEPS - 1)) + m_ClosestLineOffset[l_info->m_PartialResultsStage][yo];
...
void CLightingPreviewThread::CalculateForLightTask(int nLineMask, int nLineMatch, CLightingPreviewLightDescription &l,
			...
			if((work_line_number & nLineMatch) == nLineMatch)
			{
				for(int x = 0; x < rslt.m_nPaddedWidth; x++)
				{
					...
					if(!IsAllZeros(v_or))
					{
						...
						{
							if((r_rslt.HitIds[c] != -1) && (r_rslt.HitDistance.m128_f32[c] < len.m128_f32[c]))
							{
				...
				//				if ( msg.m_nBitmapGenerationCounter == g_nBitmapGenerationCounter )
				{
					...
					if(g_pLPreviewOutputBitmap && (g_pLPreviewOutputBitmap->Width() > 10))
					{
						   ...
						   (existing_rect.bottom != g_pLPreviewOutputBitmap->Height() - 1))
						{
							...
							w->SetWindowPos(NULL, 0, 0, myRect.Width(), myRect.Height(), SWP_NOMOVE | SWP_NOZORDER);
						}
