...
public:
	...
	{
		return (GetSelectionState() == SELECT_NORMAL);
	}
	...
	CMapFace *GetFace(int iFace)
	{
		return m_Faces.Element(iFace);
	}

	void SetMaterial(const char *szMaterialName);
	void SetMaterial(IEditorTexture *pTexture)
	{
		m_Material.m_pTexture = pTexture;
	}
	IEditorTexture *GetMaterial()
	{
		return m_Material.m_pTexture;
	}
	...
	void SetOverlayType(unsigned short uiType)
	{
		m_uiFlags |= uiType;
	}
	void ResetOverlayType(unsigned short uiType)
	{
		m_uiFlags &= ~uiType;
	}
	unsigned short GetOverlayType(void)
	{
		return m_uiFlags;
	}
...
private:
	...
	struct Handles_t
	{
		...
		Vector2D m_vecBasisCoords[OVERLAY_HANDLES_COUNT]; // U,V coordinates of the 4 corners in the editable plane (use
														  // basis)
	...
	void ClipFace_Clip(ClipFace_t *pClipFace, cplane_t *pClipPlane, float flEpsilon, ClipFace_t **ppFront,
					   ClipFace_t **ppBack);
	void ClipFace_ClipBarycentric(ClipFace_t *pClipFace, cplane_t *pClipPlane, float flEpsilon, int iClip,
	...
	void ClipFace_ResolveBarycentricClip(CMapDisp *pDisp, ClipFace_t *pClipFace, int iClipFacePoint,
										 const Vector2D &vecPointUV, float *pCoefs, int *pTris, Vector2D *pVertsUV);
	...
	void ClipFace_BuildBlend(ClipFace_t *pClipFace, CMapDisp *pDisp, cplane_t *pClipPlane, int iClip,
	...
	void Disp_DoClip(CMapDisp *pDisp, ClipFaces_t &aDispFragments, cplane_t &clipPlane, float clipDistStart,
