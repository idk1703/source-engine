...
void CRender::PushInstanceData(CMapInstance *pInstanceClass, Vector &InstanceOrigin, QAngle &InstanceAngles)
{
	...
	//	RotateInstanceVector( ( Vector )InstanceState.m_InstanceAngles, m_CurrentInstanceState.m_InstanceAngles );		no
	//one uses this right now   make sure to store it in the same fashion as vecTransformedOrigin
...
void CRender::DrawText(const char *text, int x, int y, int nFlags)
{
	...
	bool bJustifyText =
		nFlags & (TEXT_JUSTIFY_LEFT | TEXT_JUSTIFY_TOP | TEXT_JUSTIFY_HORZ_CENTER | TEXT_JUSTIFY_VERT_CENTER);
...
void CRender::DrawHandle(const Vector &vCenter, const Vector2D *vOffset)
{
	...
	{
		case HANDLE_SQUARE:
			DrawFilledRect(ul, lr, (byte *)&m_HandleColor, true);
			break;
		case HANDLE_CIRCLE:
			DrawCircle(vCenter2D, size, 16, (byte *)&m_HandleColor);
			break;
		case HANDLE_DIAMOND:
			break;
		case HANDLE_CROSS:
			DrawCross(ul, lr, (byte *)&m_HandleColor);
			break;
	}
...
bool CRender::SetView(CMapView *pView)
{
	...
	if(!GetRequiredMaterial("editor/wireframe", m_pWireframe[0]) || !GetRequiredMaterial("editor/flat", m_pFlat[0]) ||
...
void CRender::DrawModel(DrawModelInfo_t *pInfo, matrix3x4_t *pBoneToWorld, const Vector &vOrigin, float fAlpha,
						bool bWireFrame)
{
...
void CRender::SetRenderMode(EditorRenderMode_t eRenderMode, bool bForce)
{
	...
	if((eRenderMode == RENDER_MODE_LIGHT_PREVIEW2) || (eRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED))
