...
CToolDisplace *GetDisplacementTool()
{
	return dynamic_cast<CToolDisplace *>(ToolManager()->GetToolForID(TOOL_FACEEDIT_DISP));
}
	...
	if(bAllDisps)
	{
		...
		if(pSheet)
		{
			...
			for(int faceIndex = 0; faceIndex < faceCount; faceIndex++)
			{
				...
				if(!pDisp->CheckFlags(CCoreDispInfo::SURF_NOPHYSICS_COLL))
				{
					bAllNoPhysics = false;
				}
				if(!pDisp->CheckFlags(CCoreDispInfo::SURF_NOHULL_COLL))
				{
					bAllNoHull = false;
				}
				if(!pDisp->CheckFlags(CCoreDispInfo::SURF_NORAY_COLL))
				{
					bAllNoRay = false;
				}
			}
...
void CFaceEditDispPage::SetTool(unsigned int tool)
{
	...
	if((m_uiTool == FACEEDITTOOL_TAG_WALK) || (m_uiTool == FACEEDITTOOL_TAG_BUILD) ||
