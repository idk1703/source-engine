...
int GetEdgePointIndex(CMapDisp *pDisp, int edgeIndex, int edgePtIndex, bool bCCW)
{
	...
	if(bCCW)
	{
		switch(edgeIndex)
		{
			case 0:
			{
				return (edgePtIndex * height);
			}
			case 1:
			{
				return (((height - 1) * width) + edgePtIndex);
			}
			case 2:
			{
				return ((height * width - 1) - (edgePtIndex * height));
			}
			case 3:
			{
				return ((width - 1) - edgePtIndex);
			}
			default:
			{
				return -1;
	...
	else
	{
		switch(edgeIndex)
		{
			case 0:
			{
				return (((height - 1) * width) - (edgePtIndex * height));
			}
			case 1:
			{
				return ((height * width - 1) - edgePtIndex);
			}
			case 2:
			{
				return ((width - 1) + (edgePtIndex * height));
			}
			case 3:
			{
				return (edgePtIndex);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void BlendVectorFieldData(CMapDisp *pDisp1, int ndxSrc1, int ndxDst1, CMapDisp *pDisp2, int ndxSrc2, int ndxDst2,
						  float blendFactor)
{
	//
...
void SewEdge_Build(void)
{
	...
	{
		AfxMessageBox(
			"Not all selected faces could be sewn because too many selected faces share a single edge.\n\nLook for "
			"places where 3 or more selected faces (displacement or non-displacement) all share an edge.");
	}
}
