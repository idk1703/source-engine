...
public:
	...
	CMapDoc *GetOwningDocument(void)
	{
		return m_pOwningDocument;
	}
	...
	inline CCullTreeNode *CullTree_GetCullTree(void)
	{
		return (m_pCullTree);
	}
	...
	void FaceID_StringToFaceIDLists(CMapFaceIDList *pFullFaceList, CMapFaceIDList *pPartialFaceList,
									const char *pszValue);
	void FaceID_StringToFaceLists(CMapFaceList *pFullFaceList, CMapFaceList *pPartialFaceList, const char *pszValue);
	static bool FaceID_FaceIDListsToString(char *pszList, int nSize, CMapFaceIDList *pFullFaceIDList,
										   CMapFaceIDList *pPartialFaceIDList);
	static bool FaceID_FaceListsToString(char *pszValue, int nSize, CMapFaceList *pFullFaceList,
	...
	const CMapEntityList *EntityList_GetList(void)
	{
		return (&m_EntityList);
	}
	...
	bool GenerateNewTargetname(const char *startName, char *newName, int newNameBufferSize, bool bMakeUnique,
							   const char *szPrefix, CMapClass *pRoot = NULL);

	// displacement management
	inline IWorldEditDispMgr *GetWorldEditDispManager(void)
	{
		return m_pWorldDispMgr;
	}
...
protected:
	...
	ChunkFileResult_t SaveObjectListVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo, const CMapObjectList *pList,
										int saveFlags);
	...
	CMapEntityList m_EntityListByName[NUM_HASHED_ENTITY_BUCKETS]; // A list of all the entities in the world, hashed by
																  // name checksum.
