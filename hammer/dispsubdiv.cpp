...
public: // typedefs, enums, structs
	...
	{
		EDITDISP_QUADSIZE = 4
	}; // should be in mapdisp (general define)
...
private: // functions
	...
	SubdivEdgeHandle_t BuildSubdivEdge(int ndxEdge, SubdivQuadHandle_t quadHandle, SubdivQuadHandle_t parentHandle,
									   int ndxChild);
...
void CEditDispSubdivMesh::Point_CalcNewPoint(SubdivPointHandle_t ptHandle)
{
	...
	{
		case SUBDIV_POINTORDINARY:
		{
			Point_PointOrdinary(pPoint);
			break;
		}
		case SUBDIV_POINTCORNER:
		{
			Point_PointCorner(pPoint);
			break;
		}
		case SUBDIV_POINTCREASE:
		{
			Point_PointCrease(pPoint);
			break;
		}
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEditDispSubdivMesh::CompareSubdivPoints(Vector const &pt1, Vector const &pt2, float flTolerance)
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CEditDispSubdivMesh::CompareSubdivEdges(SubdivPointHandle_t ptEdge0Handle0, SubdivPointHandle_t ptEdge0Handle1,
											 SubdivPointHandle_t ptEdge1Handle0, SubdivPointHandle_t ptEdge1Handle1)
{
																			 ...
																			 int ndxChild)
{
	...
	for(edgeHandle = m_Edges.Head(); edgeHandle != m_Edges.InvalidIndex(); edgeHandle = m_Edges.Next(edgeHandle))
	{
		SubdivEdge_t *pEdge = GetEdge(edgeHandle);
		if(pEdge)
		{
								  ...
								  pEdge->m_PointHandles[0], pEdge->m_PointHandles[1]))
			{
				...
				if((pEdge->m_QuadHandles[0] != quadHandle) && (pEdge->m_QuadHandles[1] == m_Quads.InvalidIndex()))
				{
