...
bool CToolDisplace::OnLMouseDown3D(CMapView3D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	if((m_uiTool == DISPTOOL_PAINT) && (IsSpatialPainting()) && (GetAsyncKeyState(VK_MENU) & 0x8000))
	{
...
bool CToolDisplace::OnRMouseDown3D(CMapView3D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	//
	if((m_uiTool == DISPTOOL_PAINT) && (m_iPaintAxis == DISPPAINT_AXIS_FACE) && (GetAsyncKeyState(VK_MENU) & 0x8000))
	{
...
bool CToolDisplace::OnMouseMove3D(CMapView3D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	// nudging
	if((m_uiTool == DISPTOOL_PAINT) && (GetAsyncKeyState(VK_SHIFT) & 0x8000) && m_bLMBDown && m_bNudge)
	{
		Nudge_Do();
	}
	// Resizing the spatial sphere.
	else if((m_uiTool == DISPTOOL_PAINT) && (GetAsyncKeyState(VK_MENU) & 0x8000) && m_bLMBDown && m_bSpatialRadius)
	{
	...
	else
	{
		...
		//
		if((m_bLMBDown || m_bRMBDown) && !(GetAsyncKeyState(VK_CONTROL) & 0x8000) && (m_uiTool == DISPTOOL_PAINT))
		{
			if(m_bSpatial)
			{
				ApplySpatialPaintTool(nFlags, vPoint, pDisp);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::BuildParallelepiped(const Vector &boxMin, const Vector &boxMax, PLANE planes[6])
{
	int planeIndex = 0;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CToolDisplace::DistFromPointToRay(const Vector &rayStart, const Vector &rayEnd, const Vector &point)
{
	//
	// calculate the ray
	//
