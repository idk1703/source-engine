...
void DetailObjects::AddDetailModelToFace(const char *pModelName, const Vector &pt, const QAngle &angles,
										 int nOrientation)
{
...
void DetailObjects::AddDetailSpriteToFace(const Vector &vecOrigin, const QAngle &vecAngles, DetailModel_t const &model,
										  float flScale)
{
...
void DetailObjects::PlaceDetail(DetailModel_t const &model, const Vector &pt, const Vector &normal)
{
	...
	{
		float probability = (cosAngle - model.m_MaxCosAngle) / (model.m_MinCosAngle - model.m_MaxCosAngle);
...
void DetailObjects::EmitDetailObjectsOnFace(CMapFace *pMapFace, DetailObject_t &detail)
{
	...
	for(int i = 1; i < nPoints - 1; i++)
	{
		...
		for(int j = 0; j < numSamples; ++j)
		{
			...
			// Make sure the u,v coordinate stay within the triangle boundaries (ie they NOT in the far half of the
			// parallelogram)
			if(v > 1.0f - u)
			{
			...
			// Compute alpha - assumed to be 1.0 across entire face for non-displacement map faces, since there is no
			// alpha channel
			float alpha = 1.0f;
...
// Places Detail Objects on a face
//-----------------------------------------------------------------------------
void DetailObjects::EmitDetailObjectsOnDisplacementFace(CMapFace *pMapFace, DetailObject_t &detail)
{
