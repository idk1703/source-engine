...
static char *InvisToolTextures[] = {
	"occluder", "areaportal", "invisible", "skip", "trigger", "hint", "fog", "origin", "toolsnodraw",
};
...
void CMapFace::InitializeTextureAxes(TextureAlignment_t eAlignment, DWORD dwFlags)
{
	...
	if((!(dwFlags & INIT_TEXTURE_FORCE)) &&
	   ((texture.UAxis[0] != 0) || (texture.UAxis[1] != 0) || (texture.UAxis[2] != 0) || (texture.VAxis[0] != 0) ||
...
void CMapFace::SetTexture(IEditorTexture *pTexture, bool bRescaleTextureCoordinates)
{
	...
	if(m_pTexture != NULL)
	{
		...
		bTexValid = !(m_pTexture->GetWidth() == 0 || m_pTexture->GetHeight() == 0 || m_pTexture->GetImageWidth() == 0 ||
...
void CMapFace::CalcTextureCoords(void)
{
	...
	for(i = 0; i < nPoints; i++)
	{
		...
		if(m_DispHandle == EDITDISPHANDLE_INVALID)
		{
			...
			m_pLightmapCoords[i][0] = DotProduct(texture.UAxis.AsVector3D(), Points[i]) / texture.nLightmapScale +
									  texture.UAxis[3] * shiftScaleU + 0.5;
			m_pLightmapCoords[i][1] = DotProduct(texture.VAxis.AsVector3D(), Points[i]) / texture.nLightmapScale +
...
void CMapFace::ComputeColor(CRender3D *pRender, bool bRenderAsSelected, SelectionState_t faceSelectionState,
...
{
	return ((mode == RENDER_MODE_TEXTURED) || (mode == RENDER_MODE_LIGHTMAP_GRID) ||
			(mode == RENDER_MODE_TEXTURED_SHADED) || (mode == RENDER_MODE_LIGHT_PREVIEW2) ||
...
//-----------------------------------------------------------------------------
void CMapFace::AddFaceVertices(CMeshBuilder &meshBuilder, CRender3D *pRender, bool bRenderSelected,
...
//-----------------------------------------------------------------------------
void CMapFace::RenderFacesBatch(CMeshBuilder &meshBuilder, IMesh *pMesh, CRender3D *pRender, MapFaceRender_t **ppFaces,
								int nFaceCount, int nVertexCount, int nIndexCount, bool bWireframe)
{
	if(bWireframe)
	{
...
void CMapFace::RenderFaces(CRender3D *pRender, int nCount, MapFaceRender_t **ppFaces)
{
	...
	for(int nFace = 0; nFace < nCount; nFace++)
	{
		...
		if(((nVertexCount + newVertices) > nMaxVerts) || ((nIndexCount + newIndices) > nMaxIndices))
		{
			...
			RenderFacesBatch(meshBuilder, pMesh, pRender, &ppFaces[nBatchStart], nFace - nBatchStart, nVertexCount,
							 nIndexCount, bWireframe);
	...
	RenderFacesBatch(meshBuilder, pMesh, pRender, &ppFaces[nBatchStart], nCount - nBatchStart, nVertexCount,
					 nIndexCount, bWireframe);
...
//-----------------------------------------------------------------------------
void CMapFace::RenderFace3D(CRender3D *pRender, EditorRenderMode_t renderMode, bool renderSelected,
							SelectionState_t faceSelectionState)
{
...
//-----------------------------------------------------------------------------
void CMapFace::AddFaceToQueue(CMapFace *pMapFace, IEditorTexture *pTexture, EditorRenderMode_t renderMode,
							  bool selected, SelectionState_t faceSelectionState)
{
...
void CMapFace::RenderOpaqueFaces(CRender3D *pRender)
{
	...
	MapFaceRender_t **ppMapFaces =
		(MapFaceRender_t **)_alloca(g_CurrentOpaqueFaces->Count() * sizeof(MapFaceRender_t *));
	int nFaceCount = 0;

	int nLastRenderMode = RENDER_MODE_NONE;
	IEditorTexture *pLastTexture = NULL;

	for(int i = g_CurrentOpaqueFaces->FirstInorder(); i != g_CurrentOpaqueFaces->InvalidIndex();
		i = g_CurrentOpaqueFaces->NextInorder(i))
	{
		...
		if(mapFace.m_pMapFace->HasDisp())
		{
			...
			mapFace.m_pMapFace->RenderFace3D(pRender, mapFace.m_RenderMode, mapFace.m_RenderSelected,
...
void CMapFace::Render3D(CRender3D *pRender)
{
	...
	if(!Options.general.bShowNoDrawBrushes && eSolidSelectionState == SELECT_NONE &&
...
ChunkFileResult_t CMapFace::LoadKeyCallback(const char *szKey, const char *szValue, LoadFace_t *pLoadFace)
{
	...
	{
		int nRead = sscanf(szValue, "(%f %f %f) (%f %f %f) (%f %f %f)", &pFace->plane.planepts[0][0],
						   &pFace->plane.planepts[0][1], &pFace->plane.planepts[0][2], &pFace->plane.planepts[1][0],
						   &pFace->plane.planepts[1][1], &pFace->plane.planepts[1][2], &pFace->plane.planepts[2][0],
						   &pFace->plane.planepts[2][1], &pFace->plane.planepts[2][2]);
	...
	{
		int nRead = sscanf(szValue, "[%f %f %f %f] %f", &pFace->texture.UAxis[0], &pFace->texture.UAxis[1],
						   &pFace->texture.UAxis[2], &pFace->texture.UAxis[3], &pFace->texture.scale[0]);
	...
	{
		int nRead = sscanf(szValue, "[%f %f %f %f] %f", &pFace->texture.VAxis[0], &pFace->texture.VAxis[1],
						   &pFace->texture.VAxis[2], &pFace->texture.VAxis[3], &pFace->texture.scale[1]);
...
ChunkFileResult_t CMapFace::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo)
{
	...
	if(eResult == ChunkFile_Ok)
	{
		sprintf(szBuf, "(%g %g %g) (%g %g %g) (%g %g %g)", (double)plane.planepts[0][0], (double)plane.planepts[0][1],
				(double)plane.planepts[0][2], (double)plane.planepts[1][0], (double)plane.planepts[1][1],
				(double)plane.planepts[1][2], (double)plane.planepts[2][0], (double)plane.planepts[2][1],
	...
	if(eResult == ChunkFile_Ok)
	{
		sprintf(szBuf, "[%g %g %g %g] %g", (double)texture.UAxis[0], (double)texture.UAxis[1], (double)texture.UAxis[2],
	...
	if(eResult == ChunkFile_Ok)
	{
		sprintf(szBuf, "[%g %g %g %g] %g", (double)texture.VAxis[0], (double)texture.VAxis[1], (double)texture.VAxis[2],
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CMapFace::TraceLineInside(Vector &HitPos, Vector &HitNormal, Vector const &Start, Vector const &End, bool bNoDisp)
{
