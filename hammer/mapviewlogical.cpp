...
const color32 &CMapViewLogical::GetWireColor(const char *pszName, const bool bSelected, const bool bError,
											 const bool bAnySelected)
{
...
void CMapViewLogical::DrawConnectingWire(float x, float y, CMapEntity *pSource, CEntityConnection *pConnection,
										 CMapEntity *pTarget)
{
	...
	if(nInputs)
	{
		...
		if(bFound)
		{
			...
			vecEndPosition.y +=
				((nInputs - 1) * LOGICAL_CONN_VERT_SPACING / 2) / 2 - (nInput * LOGICAL_CONN_VERT_SPACING / 2);
...
void CMapViewLogical::RenderConnections(const bool bDrawSelected, const bool bAnySelected)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		for(int j = 0; j < nConnectionCount; ++j)
		{
			...
			{
				color32 c = GetWireColor(pConn->GetOutputName(), bEntitySelected || bInputSelected,
										 bBadConnection || bBadInput, bAnySelected);
				...
				if(bDrawDelay || bDrawOutput || bDrawInput || bDrawTarget)
				{
					...
					if(nChars)
						pRender->DrawText(pBuf, Vector2D(vecStartPosition.x + LOGICAL_CONN_SPREAD_DIST, y), 2, 1,
										  CRender2D::TEXT_JUSTIFY_TOP | CRender2D::TEXT_JUSTIFY_RIGHT);
					...
					if(nChars)
						pRender->DrawText(pBuf, Vector2D(vecStartPosition.x + LOGICAL_CONN_SPREAD_DIST, y), 2, -1,
										  CRender2D::TEXT_JUSTIFY_BOTTOM | CRender2D::TEXT_JUSTIFY_RIGHT);
				}
				...
				else
				{
					...
					pRender->DrawCircle(Vector(x + LOGICAL_CONN_MULTI_CIRCLE_RADIUS, y, 0.0f),
										LOGICAL_CONN_MULTI_CIRCLE_RADIUS);
					...
					for(int k = 0; k < nInputCount; ++k)
					{
						// bBadInput = false; // This should be based on whether downstream entity has the specificied
						// named input
						bInputSelected = (pEntityList->Element(k)->GetSelectionState() != SELECT_NONE);
						color32 col = GetWireColor(pConn->GetOutputName(), bEntitySelected || bInputSelected, bBadInput,
												   bAnySelected);
