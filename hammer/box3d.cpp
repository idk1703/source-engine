...
unsigned long Box3D::UpdateCursor(CMapView *pView, const Vector &vHandleHit, TransformMode_t eTransformMode)
{
	...
	Vector2D ptOrigin;
	pView->WorldToClient(ptOrigin, Vector(0, 0, 0));
	Vector2D ptHit;
	pView->WorldToClient(ptHit, vHandleHit);
	Vector2D pt;
	pt.x = ptHit.x - ptOrigin.x;
	pt.y = ptHit.y - ptOrigin.y;
...
//-----------------------------------------------------------------------------
void Box3D::StartTranslation(CMapView *pView, const Vector2D &vPoint, const Vector &vHandleOrigin,
							 const Vector *pRefPoint, const Vector *pCustomHandleBox)
{
	...
	if(!IsTranslating() || m_TranslateMode == modeScale || m_TranslateMode == modeMove)
	{
		if(!IsEmpty())
		{
			...
			switch(m_eWorldUnits)
			{
				case Units_None:
				{
					sprintf(pszBuf, " %dw %dl %dh @(%.0f %.0f %.0f)", (int)fabs(size.x), (int)fabs(size.y),
							(int)fabs(size.z), center.x, center.y, center.z);
					break;
				}
				...
				case Units_Feet_Inches:
				{
					...
					sprintf(pszBuf, " %d' %d\"w %d' %d\"l %d' %d\"h", nFeetWide, nInchesWide, nFeetLong, nInchesLong,
							nFeetHigh, nInchesHigh);
					break;
				}
			}
		}
...
int Box3D::GetVisibleHandles(Vector *handles, CMapView *pView, int nMode)
{
	...
	Vector vViewPoint;
	pView->GetCamera()->GetViewPoint(vViewPoint);
	...
	for(int x = -1; x < 2; x++)
	{
		...
		for(int y = -1; y < 2; y++)
		{
			...
			for(int z = -1; z < 2; z++)
			{
				...
				{
					Vector vHandle;
					HandleToWorld(vHandle, Vector(x, y, z));
					...
					float fIntersection =
						IntersectionLineAABBox(bmins, bmaxs, vViewPoint, vViewPoint + vDelta * 99999, nFace);
...
bool Box3D::UpdateTranslation(const Vector &vUpdate, UINT uConstraints)
{
	...
	{
		Vector vCenter;
		ProjectOnTranslationPlane(m_vTranslationFixPoint, vCenter);
		Vector vStart;
		HandleToWorld(vStart, m_TranslateHandle);
		Vector v1 = vStart - vCenter;
		VectorNormalize(v1);
		Vector v2 = (vStart + vUpdate) - vCenter;
		VectorNormalize(v2);
...
void Box3D::RenderTool2D(CRender2D *pRender)
{
	...
	if(IsTranslating())
	{
		...
		pRender->SetDrawColor(GetRValue(Options.colors.clrToolDrag), GetGValue(Options.colors.clrToolDrag),
	...
	if(IsTranslating())
	{
		if(m_TranslateMode == modeMove || m_TranslateMode == modeRotate)
		{
			...
			pRender->SetHandleColor(GetRValue(Options.colors.clrToolDrag), GetGValue(Options.colors.clrToolDrag),
									GetBValue(Options.colors.clrToolDrag));
