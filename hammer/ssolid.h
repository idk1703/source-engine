...
public:
	...
	inline SSHANDLE GetEdgeHandle(int nEdge)
	{
		Assert(nEdge < GetEdgeCount());
		return (Edges[nEdge]);
	}
...
public:
	...
	inline CSSFace *GetFace(int nFace)
	{
		Assert(nFace < m_nFaces);
		return (&m_Faces[nFace]);
	}
	...
	inline BOOL ShowEdges(void)
	{
		return (m_bShowEdges);
	}
	inline BOOL ShowVertices(void)
	{
...
private:
	...
	CSSEdge **FindAffectedEdges(SSHANDLE *pHandles, int iNumHandles, int &iNumEdges);
	...
	SSHANDLE *CreateNewVertexList(CSSFace *pFace, CSSEdge *pEdge1, CSSEdge *pEdge2, int &nv1index, int &nv2index,
