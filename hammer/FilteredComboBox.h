...
public:
	...
	void SetSuggestions(CUtlVector<CString> &suggestions,
						int flags = SETSUGGESTIONS_SELECTFIRST | SETSUGGESTIONS_CALLBACK);
	...
	// the box, it will find the first possible substring match OR it will revert to the last valid suggestion it was
	// on).
	//
	...
	// When the user presses enter, it does NOT automatically select the first suggestion. They have to use the arrow
	// keys for that.
