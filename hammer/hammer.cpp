...
public:
	...
	CString m_strGame; // The name of the game to use for this session, ie "hl2" or "cstrike". This should match the mod
					   // dir, not the config name.
};
...
bool CHammer::Connect(CreateInterfaceFn factory)
{
	...
	if(!g_pMDLCache || !g_pFileSystem || !g_pFullFileSystem || !materials || !g_pMaterialSystemHardwareConfig ||
	   !g_pStudioRender)
		return false;
	...
	// Default location for GameConfig.txt is the same directory as Hammer.exe but this may be overridden on the command
	// line
...
void CHammer::Help(const char *pszTopic)
{
	...
	GetErrorString(), hResult); AfxMessageBox(szError);
	}
	*/
}
...
InitReturnVal_t CHammer::HammerInternalInit()
{
	...
	*/
	...
	pMapDocTemplate = new CHammerDocTemplate(IDR_MAPDOC, RUNTIME_CLASS(CMapDoc),
											 RUNTIME_CLASS(CChildFrame), // custom MDI child frame
											 RUNTIME_CLASS(CMapView2D));
	AddDocTemplate(pMapDocTemplate);

	pManifestDocTemplate = new CHammerDocTemplate(IDR_MANIFESTDOC, RUNTIME_CLASS(CManifest),
												  RUNTIME_CLASS(CChildFrame), // custom MDI child frame
												  RUNTIME_CLASS(CMapView2D));
	...
	// Now that we've initialized the file system, we can parse this config's gameinfo.txt for the additional settings
	// there.
	...
	if(Options.general.bClosedCorrectly == FALSE)
	{
		...
		if(strLastGoodSave.GetLength() != 0)
		{
			...
			V_snprintf(msg, sizeof(msg),
					   "Hammer did not shut down correctly the last time it was used.\nWould you like to load the last "
					   "saved file?\n(%s)",
...
#elif DEMO_VERSION == 3
	20, 10, 9, 23, 16,	84, 12, 1,	0, 38, 65,
	25, 6,	1, 11, 119, 50, 11, 21, 9, 68, 0 // Encodes "Computer Gaming World Demo"
#elif DEMO_VERSION == 4
	25, 0, 28, 19, 72, 103, 12, 29, 69, 19, 65, 0, 6, 0, 2, 0 // Encodes "Next-Generation Demo"
#elif DEMO_VERSION == 5
	20, 10, 9,	23, 16, 84, 12, 1, 0,  38, 65, 25, 10, 79,
	41, 57, 17, 1,	21, 17, 65, 0, 29, 77, 4,  78, 0,  0 // Encodes "Computer Game Entertainment"
#elif DEMO_VERSION == 6
	20, 10, 9, 23, 16, 84, 12, 1, 0,  0,  78, 16,
	79, 33, 9, 35, 69, 52, 11, 4, 89, 12, 1,  0 // Encodes "Computer and Net Player"
#elif DEMO_VERSION == 7
...
void CHammer::OnFileOpen(void)
{
	...
	CFileDialog dlg(TRUE, NULL, NULL, OFN_LONGNAMES | OFN_HIDEREADONLY | OFN_NOCHANGEDIR,
					"Valve Map Files (*.vmf;*.vmm)|*.vmf;*.vmm|Valve Map Files Autosave "
...
CDocument *CHammer::OpenDocumentFile(LPCTSTR lpszFileName)
{
	...
	if(pDoc && ((CMapDoc *)pDoc)->IsAutosave())
	{
		...
		sprintf(szRenameMessage,
				...
				"This will not save the file with the new name; it will only rename it.",
				lpszFileName, (const char *)newMapPath);
...
BOOL CHammer::PreTranslateMessage(MSG *pMsg)
{
	...
	if(IsKeyStrokeMessage(pMsg))
	{
		...
		// The classname of dialog window in the VGUI model browser and particle browser is AfxWnd100sd in Debug and
		// AfxWnd100s in Release
...
//-------------------------------------------------------------------------------
int CHammer::GetNextAutosaveNumber(CUtlMap<FILETIME, WIN32_FIND_DATA, int> *pFileMap, DWORD *pdwTotalDirSize,
								   const CString *pstrMapTitle) const
{
	...
	{
		// go through and for each file check to see if it is an autosave for this map; also keep track of total file
		// size for directory.
		while(GetLastError() != ERROR_NO_MORE_FILES && hFile != INVALID_HANDLE_VALUE)
		{
			...
			{
				// keep track of real number of autosaves with map name; deals with instance where older maps get
				// deleted and create sequence holes in autosave map names.
...
bool CHammer::VerifyAutosaveDirectory(char *szAutosaveDirectory) const
{
	...
	CString strAutosaveDirectory(szRootDir);
	{
		...
		if((strAutosaveDirectory[1] != ':') || (strAutosaveDirectory[2] != '\\'))
		{
			AfxMessageBox("The current autosave directory does not have an absolute path.\nThe autosave feature will "
						  "be disabled until a new directory is entered.",
						  MB_OK);
			return false;
		}
	}

	hDir = CreateFile(strAutosaveDirectory, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
	...
	if(hDir == INVALID_HANDLE_VALUE)
	{
		...
		if(!bDirResult)
		{
			AfxMessageBox("The current autosave directory does not exist and could not be created.  \nThe autosave "
						  "feature will be disabled until a new directory is entered.",
						  MB_OK);
			return false;
		}
	}
	else
	{
		...
		hTestFile = CreateFile(strAutosaveDirectory + "test.txt", GENERIC_READ,
		...
		if(hTestFile == INVALID_HANDLE_VALUE)
		{
			...
			{
				AfxMessageBox(
					"The autosave directory is marked as read only.  Please remove the read only attribute or select a "
			...
			{
				AfxMessageBox("There is a problem with the autosave directory.  Please select a new directory in "
...
void CHammer::LoadLastGoodSave(void)
{
	...
	if(!strLastGoodSave.IsEmpty())
	{
		...
		{
			// This handles the case where someone recovers from a crash and tries to load an autosave file that doesn't
			// have the new autosave chunk in it It assumes the file should go into the gameConfig map directory
			char szRenameMessage[MAX_PATH + MAX_PATH + 256];
