...
public:
	CTextAttributes() : m_bsBold("\\b"), m_bsUnderline("\\ul"), m_bsItalic("\\i"), m_bsStrike("\\strike")
	{
	...
	{
		CString s;
		s.Format("\\plain%s%s%s%s\\f%d\\fs%d\\cb%d\\cf%d ", ((CString)m_bsBold).GetBuffer(),
				 ((CString)m_bsUnderline).GetBuffer(), ((CString)m_bsItalic).GetBuffer(),
				 ((CString)m_bsStrike).GetBuffer(), m_nFontNumber, m_nFontSize, m_nColorBground, m_nColorFground);
		return s;
	}
