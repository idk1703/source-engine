...
public:
	...
	public:
		...
		// The RECT inside the DRAWITEMSTRUCT contains the whole row, and pRect contains the rect for the value column
		// only. Return true if you don't want CColoredListControl to draw its value.
...
{
public:
	void AttachEntityDlg(COP_Entity *pDlg)
	{
		m_pDlg = pDlg;
	}
...
protected:
	...
	void CreateSmartControls_BasicEditControl(GDinputvariable *pVar, CRect &ctrlrect, HFONT hControlFont,
...
private:
	...
	int GetKeyValueRowByShortName(
		const char *pShortName); // Find the row in the listctrl that the var is at. Returns -1 if not found.
