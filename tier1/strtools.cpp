...
// warning C6059: Incorrect length parameter in call to 'strncat'. Pass the number of remaining characters, not the
// buffer size of 'argument 1' warning C6386: Buffer overrun: accessing 'argument 1', the writable size is
// 'destBufferSize' bytes, but '1000' bytes might be written These warnings were investigated through code inspection
// and writing of tests and they are believed to all be spurious.
#ifdef _PREFAST_
#pragma warning(push)
...
char *V_strncat(char *pDest, const char *pSrc, size_t destBufferSize, int max_chars_to_copy)
{
	...
	ANALYZE_SUPPRESS(6059); // warning C6059: : Incorrect length parameter in call to 'strncat'. Pass the number of
							// remaining characters, not the buffer size of 'argument 1'
...
//-----------------------------------------------------------------------------
// Purpose: This function fixes cases of filenames like materials\\blah.vmt or somepath\otherpath\\ and removes the
// extra double slash.
//-----------------------------------------------------------------------------
...
bool V_StrSubst(const char *pIn, const char *pMatch, const char *pReplaceWith, char *pOut, int outLen,
				bool bCaseSensitive)
{
...
void Q_URLEncodeInternal(char *pchDest, int nDestLen, const char *pchSource, int nSourceLen, bool bUsePlusForSpace)
{
	if(nDestLen < 3 * nSourceLen)
	{
		pchDest[0] = '\0';
		AssertMsg(false,
				  "Target buffer for Q_URLEncode needs to be 3 times larger than source to guarantee enough space\n");
		return;
	}
	...
	for(int i = 0; i < nSourceLen; ++i)
	{
		...
		if(!(pchSource[i] >= 'a' && pchSource[i] <= 'z') && !(pchSource[i] >= 'A' && pchSource[i] <= 'Z') &&
		   !(pchSource[i] >= '0' && pchSource[i] <= '9') && pchSource[i] != '-' && pchSource[i] != '_' &&
		   pchSource[i] != '.')
		{
...
//-----------------------------------------------------------------------------
// Purpose: Decodes a string (or binary data) from URL encoding format, see rfc1738 section 2.2.
//          This version will not recognize + as a space (which HTML form encoding uses despite not being part of the
//          RFC)
//
//          Dest buffer should be at least as large as source buffer to guarantee room for decode.
//			Dest buffer being the same as the source buffer (decode in-place) is explicitly allowed.
...
extern "C" void qsort_s(void *base, size_t num, size_t width, int (*compare)(void *, const void *, const void *),
						void *context);
#endif
...
const static HtmlEntity_t g_BasicHTMLEntities[] = {
	{'"', "&quot;", 6}, {'\'', "&#039;", 6}, {'<', "&lt;", 4},
	{'>', "&gt;", 4},	{'&', "&amp;", 5},	 {0, NULL, 0} // sentinel for end of array
};
...
const Tier1FullHTMLEntity_t g_Tier1_FullHTMLEntities[] = {
	...
	{L'\u2014', "&mdash;", 7},	{L'\u20AC', "&euro;", 6},
	{L'\u00A1', "&iexcl;", 7},	{L'\u00A2', "&cent;", 6},
	{L'\u00A3', "&pound;", 7},	{L'\u00A4', "&curren;", 8},
	{L'\u00A5', "&yen;", 5},	{L'\u00A6', "&brvbar;", 8},
	{L'\u00A7', "&sect;", 6},	{L'\u00A8', "&uml;", 5},
	{L'\u00AA', "&ordf;", 6},	{L'\u00AB', "&laquo;", 7},
	{L'\u00AC', "&not;", 8},	{L'\u00AD', "&shy;", 5},
	{L'\u00AF', "&macr;", 6},	{L'\u00B0', "&deg;", 5},
	{L'\u00B1', "&plusmn;", 8}, {L'\u00B2', "&sup2;", 6},
	{L'\u00B3', "&sup3;", 6},	{L'\u00B4', "&acute;", 7},
	{L'\u00B5', "&micro;", 7},	{L'\u00B6', "&para;", 6},
	{L'\u00B7', "&middot;", 8}, {L'\u00B8', "&cedil;", 7},
	{L'\u00B9', "&sup1;", 6},	{L'\u00BA', "&ordm;", 6},
	{L'\u00BB', "&raquo;", 7},	{L'\u00BC', "&frac14;", 8},
	{L'\u00BD', "&frac12;", 8}, {L'\u00BE', "&frac34;", 8},
	{L'\u00BF', "&iquest;", 8}, {L'\u00D7', "&times;", 7},
	{L'\u00F7', "&divide;", 8}, {L'\u00C0', "&Agrave;", 8},
	{L'\u00C1', "&Aacute;", 8}, {L'\u00C2', "&Acirc;", 7},
	{L'\u00C3', "&Atilde;", 8}, {L'\u00C4', "&Auml;", 6},
	{L'\u00C5', "&Aring;", 7},	{L'\u00C6', "&AElig;", 7},
	{L'\u00C7', "&Ccedil;", 8}, {L'\u00C8', "&Egrave;", 8},
	{L'\u00C9', "&Eacute;", 8}, {L'\u00CA', "&Ecirc;", 7},
	{L'\u00CB', "&Euml;", 6},	{L'\u00CC', "&Igrave;", 8},
	{L'\u00CD', "&Iacute;", 8}, {L'\u00CE', "&Icirc;", 7},
	{L'\u00CF', "&Iuml;", 6},	{L'\u00D0', "&ETH;", 5},
	{L'\u00D1', "&Ntilde;", 8}, {L'\u00D2', "&Ograve;", 8},
	{L'\u00D3', "&Oacute;", 8}, {L'\u00D4', "&Ocirc;", 7},
	{L'\u00D5', "&Otilde;", 8}, {L'\u00D6', "&Ouml;", 6},
	{L'\u00D8', "&Oslash;", 8}, {L'\u00D9', "&Ugrave;", 8},
	{L'\u00DA', "&Uacute;", 8}, {L'\u00DB', "&Ucirc;", 7},
	{L'\u00DC', "&Uuml;", 6},	{L'\u00DD', "&Yacute;", 8},
	{L'\u00DE', "&THORN;", 7},	{L'\u00DF', "&szlig;", 7},
	{L'\u00E0', "&agrave;", 8}, {L'\u00E1', "&aacute;", 8},
	{L'\u00E2', "&acirc;", 7},	{L'\u00E3', "&atilde;", 8},
	{L'\u00E4', "&auml;", 6},	{L'\u00E5', "&aring;", 7},
	{L'\u00E6', "&aelig;", 7},	{L'\u00E7', "&ccedil;", 8},
	{L'\u00E8', "&egrave;", 8}, {L'\u00E9', "&eacute;", 8},
	{L'\u00EA', "&ecirc;", 7},	{L'\u00EB', "&euml;", 6},
	{L'\u00EC', "&igrave;", 8}, {L'\u00ED', "&iacute;", 8},
	{L'\u00EE', "&icirc;", 7},	{L'\u00EF', "&iuml;", 6},
	{L'\u00F0', "&eth;", 5},	{L'\u00F1', "&ntilde;", 8},
	{L'\u00F2', "&ograve;", 8}, {L'\u00F3', "&oacute;", 8},
	{L'\u00F4', "&ocirc;", 7},	{L'\u00F5', "&otilde;", 8},
	{L'\u00F6', "&ouml;", 6},	{L'\u00F8', "&oslash;", 8},
	{L'\u00F9', "&ugrave;", 8}, {L'\u00FA', "&uacute;", 8},
	{L'\u00FB', "&ucirc;", 7},	{L'\u00FC', "&uuml;", 6},
	{L'\u00FD', "&yacute;", 8}, {L'\u00FE', "&thorn;", 7},
	{L'\u00FF', "&yuml;", 6},	{0, NULL, 0} // sentinel for end of array
};
#pragma warning(pop)

bool V_BasicHtmlEntityEncode(char *pDest, const int nDestSize, char const *pIn, const int nInSize,
...
static const char *g_pszSimpleBBCodeReplacements[] = {
	"[b]",	   "<b>",	 "[/b]", "</b>",   "[i]",	"<i>",	 "[/i]",		"</i>",	  "[u]",
	"<u>",	   "[/u]",	 "</u>", "[s]",	   "<s>",	"[/s]",	 "</s>",		"[code]", "<pre>",
	"[/code]", "</pre>", "[h1]", "<h1>",   "[/h1]", "</h1>", "[list]",		"<ul>",	  "[/list]",
	"</ul>",   "[*]",	 "<li>", "[/url]", "</a>",	"[img]", "<img src=\"", "[/img]", "\"></img>",
};
	...
	if((ch & 0xC0) == 0xC0)
	{
		...
		if(iAdvance)
		{
			...
			if(rgch32Buf < 0x10000 && (V_IsMeanSpaceW((wchar_t)rgch32Buf) || V_IsDeprecatedW((wchar_t)rgch32Buf) ||
									   V_IsMeanUnderscoreW((wchar_t)rgch32Buf)))
			{
				return false;
			}
					...
					else
					{
						...
						for(uint iTag = 0; iTag < cPreserveTags; iTag++)
						{
							...
							if(Q_strnicmp(szTag, szTagStart, cchTag) == 0 && !V_isalnum(szTagStart[cchTag]))
							{
								...
								if(bEndTag)
								{
									// ending a paragraph tag is optional. If we were expecting to find one, and didn't,
									// skip
									if(Q_stricmp(szTag, "p") != 0)
									{
										while(vecTagStack.Count() > 0 &&
											  Q_stricmp(vecTagStack[vecTagStack.Count() - 1], "p") == 0)
										{
									...
									{
										// don't preserve this unbalanced tag.  All open tags will be closed at the end
										// of the blurb
										bPreserve = false;
									}
								}
							...
							uint cMaxResultSize)
{
	...
	V_StripAndPreserveHTMLCore(pbuffer, pchHTML, rgszPreserveTags, cPreserveTags, rgszNoCloseTags,
							   V_ARRAYSIZE(rgszNoCloseTags), cMaxResultSize);
}