...
void KeyValues::RecursiveSaveToFile(CUtlBuffer &buf, int indentLevel, bool sortKeys /*= false*/,
							 ...
							 bool refreshCache)
{
	...
	*/
	...
	// If pathID is null, we cannot cache the result because that has a weird iterate-through-a-bunch-of-locations
	// behavior.
	const bool bUseCacheForRead = bUseCache && !refreshCache && pathID != NULL;
	const bool bUseCacheForWrite = bUseCache && pathID != NULL;

	COM_TimestampedLog("KeyValues::LoadFromFile(%s%s%s): Begin", pathID ? pathID : "",
	...
	{
		COM_TimestampedLog("KeyValues::LoadFromFile(%s%s%s): End / CacheHit", pathID ? pathID : "",
						   pathID && resourceName ? "/" : "", resourceName ? resourceName : "");
		return true;
	}
	...
	if(!f)
	{
		COM_TimestampedLog("KeyValues::LoadFromFile(%s%s%s): End / FileNotFound", pathID ? pathID : "",
						   pathID && resourceName ? "/" : "", resourceName ? resourceName : "");
		return false;
	}
	...
	COM_TimestampedLog("KeyValues::LoadFromFile(%s%s%s): End / Success", pathID ? pathID : "",
					   pathID && resourceName ? "/" : "", resourceName ? resourceName : "");
...
void KeyValues::SaveKeyToFile(KeyValues *dat, IBaseFileSystem *filesystem, FileHandle_t f, CUtlBuffer *pBuf,
							  int indentLevel, bool sortKeys, bool bAllowEmptyString)
{
	if(dat->m_pSub)
	{
							   ...
							   const char *pPathID)
{
	...
	COM_TimestampedLog("KeyValues::LoadFromBuffer(%s%s%s): Begin", pPathID ? pPathID : "",
	...
	COM_TimestampedLog("KeyValues::LoadFromBuffer(%s%s%s): End", pPathID ? pPathID : "",
					   pPathID && resourceName ? "/" : "", resourceName ? resourceName : "");
