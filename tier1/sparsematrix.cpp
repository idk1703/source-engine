...
{
	// now, we need to offset the starting position of all subsequent rows by -1 to compensate for the removal of this
	// element
...
void CSparseMatrix::SetElement(int nRow, int nCol, float flValue)
{
	...
	if(nCount)
	{
		...
		for(i = 0; i < nCount; i++)
		{
			...
			if(nIdx == nCol) // we found it!
			{
				...
				else
				{
					...
					// now, we need to offset the starting position of all subsequent rows by -1 to compensate for the
		...
		if(!bValueIsZero)
		{
			...
			// now, we need to offset the starting position of all subsequent rows by -1 to compensate for the addition
			// of this element
			AdjustAllRowIndicesAfter(nRow, +1);
		}
	}
