...
void CPathString::PopulateWCharPath()
{
	...
	if(m_pchUTF8Path[0] == '\\' && m_pchUTF8Path[1] == '\\')
	{
		...
		// Be sure we NULL terminate within our allocated region incase Q_UTF8ToUnicode failed, though we're already in
		// bad shape then.
...
public:
	static void WINAPI DirWatchCallback(DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, OVERLAPPED *pOverlapped)
	{
		...
		{
			FILE_NOTIFY_INFORMATION *pFileNotifyInformation =
				(FILE_NOTIFY_INFORMATION *)pDirWatcherOverlapped->m_pDirWatcher->m_pFileInfo;
			do
			{
				...
				pFileNotifyInformation = (FILE_NOTIFY_INFORMATION *)(((byte *)pFileNotifyInformation) +
																	 pFileNotifyInformation->NextEntryOffset);
			} while(1);
		}
	...
	while((dirent = readdir(dir)) != NULL)
	{
		...
		if(S_ISDIR(st.st_mode) && bRecurse)
		{
			CheckDirectoryForChanges(fullpath, pDirWatch, bRecurse);
				...
				(st.st_mtimespec.tv_sec == pDirWatch->m_modTime.tv_sec &&
				 st.st_mtimespec.tv_nsec > pDirWatch->m_modTime.tv_nsec))
		{
			ts = st.st_mtimespec;
			bTimeSet = true;
							  ...
							  const FSEventStreamEventId eventIDs[])
{
	...
	for(int i = 0; i < numEvents; i++)
	{
		...
		if(eventMasks[i] & kFSEventStreamEventFlagMustScanSubDirs ||
		   eventMasks[i] & kFSEventStreamEventFlagUserDropped || eventMasks[i] & kFSEventStreamEventFlagKernelDropped)
		{
			bRecurse = true;
		}
...
void CDirWatcher::SetDirToWatch(const char *pchDir)
{
...
#ifdef WIN32
	// open the directory
	m_hFile = ::CreateFileW(strPath.GetWCharPathPrePended(), FILE_LIST_DIRECTORY,
	...
	m_WatcherStream =
		(void *)FSEventStreamCreate(NULL, &fsevents_callback, &callbackInfo, pathsToWatch,
									kFSEventStreamEventIdSinceNow, latency, kFSEventStreamCreateFlagNoDefer);
...
void CDirWatcher::PostDirWatch()
{
	...
	DWORD dwBytes;
	::ReadDirectoryChangesW(m_hFile, m_pFileInfo, k_cubDirWatchBufferSize, TRUE,
							FILE_NOTIFY_CHANGE_LAST_WRITE | FILE_NOTIFY_CHANGE_FILE_NAME, &dwBytes,
							(OVERLAPPED *)m_pOverlapped, &CDirWatcherFriend::DirWatchCallback);
}
#endif
