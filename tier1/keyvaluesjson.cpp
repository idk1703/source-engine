...
KeyValues *KeyValuesJSONParser::ParseFile()
{
	...
	V_sprintf_safe(m_szErrMsg,
				   "%s not expected here.  A valid JSON document should be a single object, which begins with '{' and "
				   "ends with '}'",
				   GetTokenDebugText());
	return NULL;
}
...
bool KeyValuesJSONParser::ParseObject(KeyValues *pObject)
{
	...
	while(m_eToken != '}')
	{
		...
		{
			V_sprintf_safe(m_szErrMsg, "%s not expected here; expected string for key name or '}'",
						   GetTokenDebugText());
			return false;
		}
...
void KeyValuesJSONParser::ParseNumberToken()
{
	...
	{
		case '0':
			// Leading 0 cannot be followed by any more digits, as per JSON spec (and to make sure nobody tries to parse
			// octal).
			++m_cur;
			bHasWholePart = true;
			break;
	...
	while(m_cur < m_end)
	{
		...
		switch(c)
		{
			...
			case 'u':
			{
				...
				if(m_cur + 4 > m_end || !V_isxdigit(m_cur[0]) || !V_isxdigit(m_cur[1]) || !V_isxdigit(m_cur[2]) ||
				   !V_isxdigit(m_cur[3]))
				{
