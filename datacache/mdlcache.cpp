...
static ConVar mod_forcedata("mod_forcedata", (AsyncMdlCache()) ? "0" : "1", 0,
...
static ConVar mod_load_fakestall("mod_load_fakestall", "0", 0,
...
public:
	...
	void CacheData(DataCacheHandle_t *c, void *pData, int size, const char *name, MDLCacheDataType_t type,
	...
	static void QueuedLoaderCallback_MDL(void *pContext, void *pContext2, const void *pData, int nSize,
...
private:
	...
	FSAsyncStatus_t LoadData(const char *pszFilename, const char *pszPathID, bool bAsync, FSAsyncControl_t *pControl)
	{
		return LoadData(pszFilename, pszPathID, NULL, 0, 0, bAsync, pControl);
	}
	FSAsyncStatus_t LoadData(const char *pszFilename, const char *pszPathID, void *pDest, int nBytes, int nOffset,
	...
	bool BuildHardwareData(MDLHandle_t handle, studiodata_t *pStudioData, studiohdr_t *pStudioHdr,
	...
	int UpdateOrCreate(studiohdr_t *pHdr, const char *pFilename, char *pX360Filename, int maxLen, const char *pPathID,
					   bool bForce = false);
	...
	vertexFileHeader_t *CreateThinVertexes(vertexFileHeader_t *originalData, const studiohdr_t *pStudioHdr,
										   int *cacheLength);

	// Processes raw data (from an I/O source) into the cache. Sets the cache state as expected for bad data.
	bool ProcessDataIntoCache(MDLHandle_t handle, MDLCacheDataType_t type, int iAnimBlock, void *pData, int nDataSize,
							  bool bDataValid);
...
bool CMDLCache::LoadHardwareData(MDLHandle_t handle)
{
	...
	if(LogMdlCache() && GetAsyncInfoIndex(handle, MDLCACHE_STUDIOHWDATA) == NO_ASYNC &&
		...
		for(int j = 0; j < pBody->nummodels; j++)
		{
			...
			for(int k = 0; k < pModel->nummeshes; k++)
			{
				...
				for(int l = 0; l < pMesh->numflexes; l++)
				{
					...
					{
						mstudiovertanim_t *pVAnim = bIsWrinkleAnim ? pFlex->pVertanimWrinkle(m) : pFlex->pVertanim(m);
...
//
//-----------------------------------------------------------------------------
bool CMDLCache::BuildHardwareData(MDLHandle_t handle, studiodata_t *pStudioData, studiohdr_t *pStudioHdr,
								  OptimizedModel::FileHeader_t *pVtxHdr)
{
	if(pVtxHdr)
	{
	...
	if(IsX360())
	{
		...
		if(CLZMA::IsCompressed(pInputData))
		{
			...
			unsigned int nOutputSize = CLZMA::Uncompress(pInputData, sizeof(OptimizedModel::FileHeader_t) +
																		 (unsigned char *)pOriginalData.Get());
	...
	GetCacheSection(MDLCACHE_STUDIOHWDATA)
		->Add(MakeCacheID(handle, MDLCACHE_STUDIOHWDATA), &pStudioData->m_HardwareData,
			  ComputeHardwareDataSize(&pStudioData->m_HardwareData), &pStudioData->m_HardwareDataCache);
#endif
	return true;
}
...
static bool MdlcacheCreateCallback(const char *pSourceName, const char *pTargetName, const char *pPathID, void *pHdr)
{
	...
	if(bOk)
	{
		...
		int bytes = StudioByteSwap::ByteswapStudioFile(pTargetName, targetBuf.Base(), sourceBuf.Base(),
													   sourceBuf.TellPut(), (studiohdr_t *)pHdr);
		if(bytes)
		{
...
//-----------------------------------------------------------------------------
int CMDLCache::UpdateOrCreate(studiohdr_t *pHdr, const char *pSourceName, char *pTargetName, int targetLen,
...
studiohdr_t *CMDLCache::UnserializeMDL(MDLHandle_t handle, void *pData, int nDataSize, bool bDataValid)
{
	...
	CacheData(&m_MDLDict[handle]->m_MDLCache, pHdr, pStudioHdrIn->length, GetModelName(handle), MDLCACHE_STUDIOHDR,
	...
	if(IsX360())
	{
		if(CLZMA::IsCompressed((unsigned char *)buf.PeekGet()))
		{
			...
			unsigned int nOutputSize =
				CLZMA::Uncompress((unsigned char *)buf.PeekGet(), (unsigned char *)pOriginalData);
			if(nOutputSize != nOriginalSize)
			{
				// decoder failure
				free(pOriginalData);
	...
	if(!pHdr)
	{
		...
		if(!ReadMDLFile(handle, pModelName, buf))
		{
			...
			if(!bOk)
			{
				if(IsOSX())
				{
					// rbarris wants this to go somewhere like the console.log prior to crashing, which is what the
					// Error call will do next
...
bool CMDLCache::VerifyHeaders(studiohdr_t *pStudioHdr)
{
	...
	// check
	if((pVertexHdr->id != MODEL_VERTEX_FILE_ID) || (pVertexHdr->version != MODEL_VERTEX_FILE_VERSION) ||
	...
	if((pVtxHdr->version != OPTIMIZED_MODEL_FILE_VERSION) || (pVtxHdr->checkSum != pStudioHdr->checksum))
	{
		return false;
	}
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t CMDLCache::LoadData(const char *pszFilename, const char *pszPathID, void *pDest, int nBytes,
									int nOffset, bool bAsync, FSAsyncControl_t *pControl)
{
	if(!*pControl)
	{
		if(IsX360() && g_pQueuedLoader->IsMapLoading())
...
//-----------------------------------------------------------------------------
vertexFileHeader_t *CMDLCache::CreateThinVertexes(vertexFileHeader_t *originalData, const studiohdr_t *pStudioHdr,
												  int *cacheLength)
{
...
//-----------------------------------------------------------------------------
bool CMDLCache::ProcessDataIntoCache(MDLHandle_t handle, MDLCacheDataType_t type, int iAnimBlock, void *pData,
									 int nDataSize, bool bDataValid)
{
	...
	switch(type)
	{
		...
		case MDLCACHE_STUDIOHWDATA:
		{
			...
			// FIXME: thin VVD data on PC too (have to address alt-tab, various DX8/DX7/debug software paths in
			// studiorender, tools, etc)
			...
			if(IsX360() && !(pStudioDataCurrent->m_nFlags & STUDIODATA_FLAGS_NO_STUDIOMESH) && bCompressedVVDs)
			{
				...
				if(pStudioHdrCurrent->numflexdesc == 0)
				{
					...
					if(originalVertexData)
					{
						...
						vertexFileHeader_t *thinVertexData =
							CreateThinVertexes(originalVertexData, pStudioHdrCurrent, &thinVertexDataSize);
						...
						if(thinVertexData && (thinVertexDataSize > 0))
						{
							...
							// Add the new one
							CacheData(&pStudioDataCurrent->m_VertexCache, thinVertexData, thinVertexDataSize,
									  pStudioHdrCurrent->pszName(), MDLCACHE_VERTEXES,
			...
			if(bDataValid)
			{
				...
				if(IsX360())
				{
					if(CLZMA::IsCompressed((unsigned char *)pData))
					{
						...
						void *pOriginalData =
							g_pFullFileSystem->AllocOptimalReadBuffer(FILESYSTEM_INVALID_HANDLE, nOriginalSize, 0);
						unsigned int nOutputSize =
							CLZMA::Uncompress((unsigned char *)pData, (unsigned char *)pOriginalData);
						if(nOutputSize != nOriginalSize)
						{
							// decoder failure
				...
				CacheData(&pStudioDataCurrent->m_pAnimBlock[iAnimBlock], pData, nDataSize, pCacheName,
						  MDLCACHE_ANIMBLOCK, MakeCacheID(handle, MDLCACHE_ANIMBLOCK));
			}
			else
			{
			...
			if(bDataValid)
			{
				...
				if(IsX360())
				{
					if(CLZMA::IsCompressed((unsigned char *)pData))
					{
						...
						pOriginalData.Alloc(nOriginalSize);
						unsigned int nOutputSize =
							CLZMA::Uncompress((unsigned char *)pData, (unsigned char *)pOriginalData.Get());
						if(nOutputSize != nOriginalSize)
						{
							// decoder failure
							return NULL;
				...
				if((header.size == sizeof(header)) && header.solidCount > 0)
				{
					...
					g_pPhysicsCollision->VCollideLoad(pCollide, header.solidCount, (const char *)buf.PeekGet(),
													  nBufSize);
					if(m_pCacheNotify)
					{
...
int CMDLCache::ProcessPendingAsync(int iAsync)
{
	...
	switch(pInfo->type)
	{
		...
		case MDLCACHE_ANIMBLOCK:
		{
			...
			if(!ProcessDataIntoCache(pInfo->hModel, MDLCACHE_ANIMBLOCK, pInfo->iAnimBlock, pData, nBytesRead,
									 status == FSASYNC_OK))
			{
				g_pFullFileSystem->FreeOptimalReadBuffer(pData);
			}
...
vertexFileHeader_t *CMDLCache::BuildAndCacheVertexData(studiohdr_t *pStudioHdr, vertexFileHeader_t *pRawVvdHdr)
{
	...
	if(IsX360())
	{
		...
		if(CLZMA::IsCompressed(pInput))
		{
			...
			unsigned int nOutputSize =
				CLZMA::Uncompress(pInput, sizeof(vertexFileHeader_t) + (unsigned char *)pOriginalData.Get());
			if(nOutputSize != nOriginalSize)
			{
				// decoder failure
				return NULL;
	...
	CacheData(&m_MDLDict[handle]->m_VertexCache, pVvdHdr, cacheLength, pStudioHdr->pszName(), MDLCACHE_VERTEXES,
			  MakeCacheID(handle, MDLCACHE_VERTEXES));
...
// Caches an item
//-----------------------------------------------------------------------------
void CMDLCache::CacheData(DataCacheHandle_t *c, void *pData, int size, const char *name, MDLCacheDataType_t type,
...
//-----------------------------------------------------------------------------
void CMDLCache::QueuedLoaderCallback_MDL(void *pContext, void *pContext2, const void *pData, int nSize,
										 LoaderError_t loaderError)
{
	...
	ModelParts_t::BufferType_t bufferType = static_cast<ModelParts_t::BufferType_t>((uintp)pContext2);
				...
				if(studiohdr_t *pHdr = g_MDLCache.GetStudioHdr(pModelParts->hMDL))
				{
					if(!(pHdr->flags & STUDIOHDR_FLAGS_OBSOLETE))
					{
						...
						for(int t = 0; t < pHdr->numtextures; ++t)
						{
							...
							pTexture +=
								(pTexture[0] == CORRECT_PATH_SEPARATOR || pTexture[0] == INCORRECT_PATH_SEPARATOR);
							for(int cd = 0; cd < pHdr->numcdtextures; ++cd)
							{
								const char *pCdTexture = pHdr->pCdtexture(cd);
								pCdTexture += (pCdTexture[0] == CORRECT_PATH_SEPARATOR ||
		...
		if(pHdr && !(pHdr->flags & STUDIOHDR_FLAGS_OBSOLETE))
		{
			...
			for(int t = 0; t < pHdr->numtextures; ++t)
			{
				...
				for(int cd = 0; cd < pHdr->numcdtextures; ++cd)
				{
					const char *pCdTexture = pHdr->pCdtexture(cd);
					pCdTexture +=
						(pCdTexture[0] == CORRECT_PATH_SEPARATOR || pCdTexture[0] == INCORRECT_PATH_SEPARATOR);
					V_ComposeFileName(pCdTexture, pTexture, buf + prefixLen, MAX_PATH - prefixLen);
