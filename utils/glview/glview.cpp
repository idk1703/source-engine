...
void Benchmark_PHY(const CPhysCollide *pCollide)
{
	...
	duration = endTime - startTime;
	{
		CFmtStr str("%d collisions in %.2f ms [%.2f X] %d hits\n", NUM_COLLISION_TESTS, duration * 1000,
					IMPROVEMENT_FACTOR(duration * 1000.0f, baselineTotal), hitCount);
		OutputDebugStr(str.Access());
	}
	{
		float rayTime = (midTime - startTime) * 1000.0f;
		float boxTime = (endTime - midTime) * 1000.0f;
		CFmtStr str("%.2f ms rays [%.2f X] %.2f ms boxes [%.2f X]\n", rayTime, IMPROVEMENT_FACTOR(rayTime, baselineRay),
					boxTime, IMPROVEMENT_FACTOR(boxTime, baselineBox));
		OutputDebugStr(str.Access());
	}
...
BOOL ReadDisplacementFile(const char *filename)
{
	...
	while(1)
	{
		...
		fileCount = fscanf(pFile, "%f %f %f %f %f %f", &dispPoints[dispPointCount][0], &dispPoints[dispPointCount][1],
						   &dispPoints[dispPointCount][2], &dispNormals[dispPointCount][0],
						   &dispNormals[dispPointCount][1], &dispNormals[dispPointCount][2]);
		dispPointCount++;
...
void DrawDisplacementData(void)
{
	...
	for(i = 0; i < dispPointCount; i++)
	{
		...
		glVertex3f(dispPoints[i][0] + (dispNormals[i][0] * 50.0f), dispPoints[i][1] + (dispNormals[i][1] * 50.0f),
	...
	for(i = 0; i < width - 1; i++)
	{
		for(j = 0; j < width - 1; j++)
		{
			...
			glVertex3f(dispPoints[(i + 1) * width + j][0], dispPoints[(i + 1) * width + j][1],
					   dispPoints[(i + 1) * width + j][2]);
			glVertex3f(dispPoints[(i + 1) * width + (j + 1)][0], dispPoints[(i + 1) * width + (j + 1)][1],
					   dispPoints[(i + 1) * width + (j + 1)][2]);
			glVertex3f(dispPoints[i * width + (j + 1)][0], dispPoints[i * width + (j + 1)][1],
...
void WCam_Create(HINSTANCE hInstance)
{
	...
	camerawindow = CreateWindow("camera", "Camera View",
