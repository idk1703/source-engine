...
bool CVCProjConvert::LoadProject(const char *project)
{
	...
	if(FAILED(hr))
	{
		...
		Msg("http://msdn.microsoft.com/downloads/default.asp?url=/downloads/sample.asp?url=/msdn-files/027/001/766/"
			...
			if(pNode)
			{
				...
				if(toolName == "VCCLCompilerTool")
				{
					...
					while(delim)
					{
						*delim = 0;
						delim++;
						if(Q_stricmp(curpos, "WIN32") && Q_stricmp(curpos, "_WIN32") && Q_stricmp(curpos, "_WINDOWS") &&
