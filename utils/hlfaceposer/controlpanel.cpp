...
{
public:
	CControlPanelTabWindow(mxWindow *parent, int x, int y, int w, int h) : mxWindow(parent, x, y, w, h)
	{
...
void ControlPanel::setSpeed(float value)
{
...
void ControlPanel::initBoneControllerChoices()
{
	CStudioHdr *hdr = models->GetActiveStudioModel()->GetStudioHdr();
	if(hdr)
	{
...
void ControlPanel::initPoseParameters()
{
	CStudioHdr *hdr = models->GetActiveStudioModel()->GetStudioHdr();
	if(hdr)
	{
...
void ControlPanel::setModelInfo()
{
	...
	sprintf(str,
			...
			"Sequences: %d\n",
			hdr->numbones(), hdr->numbonecontrollers(), hbcount, hdr->numhitboxsets(), hdr->GetNumSeq());
	...
	sprintf(str,
			...
			"Bodyparts: %d\n"
			"Attachments: %d\n",
			hdr->numtextures(), hdr->numskinfamilies(), hdr->numbodyparts(), hdr->GetNumAttachments());
