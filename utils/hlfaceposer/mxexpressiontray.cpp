...
void mxExpressionTray::CreateButtons(void)
{
	...
	AddButton("undo", "Undo", "gfx/hlfaceposer/undo.bmp", &mxExpressionTray::ET_Undo, true, x, y, m_nButtonSquare,
			  m_nButtonSquare);

	x += (m_nButtonSquare + 4);

	AddButton("redo", "Redo", "gfx/hlfaceposer/redo.bmp", &mxExpressionTray::ET_Redo, true, x, y, m_nButtonSquare,
			  m_nButtonSquare);
}
...
mxExpressionTray::mxETButton *mxExpressionTray::GetItemUnderCursor(int x, int y)
{
	...
	while(p)
	{
		if(p->m_bActive && x >= cx && x <= cx + cw && y >= cy && y <= cy + ch)
		{
			// In-side cell
			int cellx = x - cx;
			int celly = y - cy;

			if(cellx >= p->m_rc.left && cellx <= p->m_rc.right && celly >= p->m_rc.top && celly <= p->m_rc.bottom)
			{
				return p;
			}
		}
...
void mxExpressionTray::DrawExpressionFocusRect(CChoreoWidgetDrawHelper &helper, int x, int y, int w, int h,
											   COLORREF clr)
{
	helper.DrawOutlinedRect(clr, PS_SOLID, 4, x, y, x + w, y + h);
}

void mxExpressionTray::DrawExpressionDescription(CChoreoWidgetDrawHelper &helper, int x, int y, int w, int h,
												 const char *expressionname, const char *description)
{
	int textheight = 15;
	...
	//	DrawText( hdc, expressionname, strlen( expressionname ), &textRect, DT_NOPREFIX | DT_SINGLELINE | DT_LEFT |
	...
	//	DrawText( hdc, description, strlen( description ), &textRect, DT_NOPREFIX | DT_SINGLELINE | DT_LEFT | DT_VCENTER
	//| DT_WORD_ELLIPSIS );
}
...
//			rch -
//-----------------------------------------------------------------------------
void mxExpressionTray::DrawDirtyFlag(CChoreoWidgetDrawHelper &helper, CExpression *current, int rcx, int rcy, int rcw,
									 int rch)
{
	// Not dirty
...
void mxExpressionTray::DrawThumbNail(CExpClass *active, CExpression *current, CChoreoWidgetDrawHelper &helper, int rcx,
									 int rcy, int rcw, int rch, int c, int selected, bool updateselection)
{
	if(!current)
		return;
	...
	if(c == selected)
	{
		...
		if(current->CanUndo() || current->CanRedo())
		{
			...
			helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(200, 200, 200), rc, "%i/%i", current->UndoCurrent(),
								   current->UndoLevels());
		}
	}
	else
...
static bool IsWindowOrChild(mxWindow *parent, HWND test)
{
	...
	if(test == parentHwnd || IsChild(parentHwnd, test))
	{
		return true;
	}
	return false;
	...
	switch(event->event)
	{
		...
		case mxEvent::MouseDrag:
		{
			if(m_bDragging)
			{
				...
				// update pos
				m_rcFocus = m_rcOrig;
				OffsetRect(&m_rcFocus, ((short)event->x - m_nXStart), ((short)event->y - m_nYStart));
		...
		case mxEvent::MouseUp:
		{
			...
			{
				// Over a new cell
				if(cell >= 0 && cell < active->GetNumExpressions() && cell != m_nCurCell && m_nCurCell != -1)
				{
					// Swap cells
					CExpression *exp = active->GetExpression(m_nCurCell);
					if(exp)
