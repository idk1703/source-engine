...
FlexPanel::FlexPanel(mxWindow *parent) : IFacePoserToolWindow("FlexPanel", "Flex Sliders"), mxWindow(parent, 0, 0, 0, 0)
{
...
void FlexPanel::PositionSliders(int sboffset)
{
	...
	for(int i = 0; i < GLOBAL_STUDIO_FLEX_CONTROL_COUNT; i++)
	{
		...
		if(i >= rowsneeded || (y + LINE_HEIGHT - 5 > (this->h2() - reservedheight)))
		{
	...
	switch(event->event)
	{
		...
		case mxEvent::Action:
		{
			...
			switch(event->action)
			{
				...
				case IDC_FLEXSCROLL:
				{
					if(event->event == mxEvent::Action && event->modifiers == SB_THUMBTRACK)
					{
			...
			if(event->action >= IDC_FLEXSCALE && event->action < IDC_FLEXSCALE + GLOBAL_STUDIO_FLEX_CONTROL_COUNT)
			{
				...
				if(active)
				{
					...
					if(pushundo && index != -1)
					{
						...
						if(exp)
						{
							...
							if(settings[flex] != value || settings[flex2] != value2 || weights[flex] != influ)
							{
				...
				LocalFlexController_t localflex =
					FindFlexControllerIndexByName(models->GetActiveStudioModel(), GetGlobalFlexControllerName(flex));
				if(localflex >= 0)
				{
					// Update the face
					...
					if(flex2 != flex)
					{
						LocalFlexController_t localflex2 = FindFlexControllerIndexByName(
							models->GetActiveStudioModel(), GetGlobalFlexControllerName(flex2));
						if(localflex2 >= 0)
						{
...
float FlexPanel::GetSlider(int iFlexController)
{
...
void FlexPanel::SetSlider(int iFlexController, float value)
{
	if(!IsValidSlider(iFlexController))
	{
...
float FlexPanel::GetInfluence(int iFlexController)
{
	if(!IsValidSlider(iFlexController))
	{
...
void FlexPanel::SetEdited(int iFlexController, bool isEdited)
{
	if(!IsValidSlider(iFlexController))
	{
...
bool FlexPanel::IsEdited(int iFlexController)
{
	if(!IsValidSlider(iFlexController))
	{
		Msg("IsEdited(%d) invalid controller index\n", iFlexController);
...
void FlexPanel::SetInfluence(int iFlexController, float value)
{
	if(!IsValidSlider(iFlexController))
	{
	...
	// Con_Printf( "SetInfluence( %d, %.0f ) : %d %d\n", iFlexController, value, nFlexSliderIndex[ iFlexController ],
	// nFlexSliderBarnum[ iFlexController ] );
...
int FlexPanel::LookupFlex(int iSlider, int barnum)
{
	for(int i = 0; i < GLOBAL_STUDIO_FLEX_CONTROL_COUNT; i++)
	{
...
int FlexPanel::LookupPairedFlex(int iFlexController)
{
	if(!IsValidSlider(iFlexController))
	{
...
void FlexPanel::setExpression(int index)
{
