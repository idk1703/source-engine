...
int mxStatusWindow::handleEvent(mxEvent *event)
{
	...
	switch(event->event)
	{
		...
		case mxEvent::Size:
		{
			m_pScrollbar->setBounds(w2() - STATUS_SCROLLBAR_SIZE, GetCaptionHeight(), STATUS_SCROLLBAR_SIZE,
									h2() - GetCaptionHeight());
			PositionSliders(0);
			m_pScrollbar->setValue(m_pScrollbar->getMaxValue());
			iret = 1;
...
void mxStatusWindow::DrawActiveTool()
{
	...
	sprintf(sz, "%s (%i) at %.3f (%.2f fps) (soundcount %i)", activeTool ? activeTool->GetToolName() : "None",
			g_MDLViewer->GetCurrentFrame(), (float)realtime, fps, models->CountActiveSources());
