			...
			switch(LOWORD(wParam))
			{
				case IDOK:
				{
					...
					if(soundemitter->IsValidIndex(idx))
					{
						...
						{
							mxMessageBox(NULL, va("Sound '%s' already exists", g_Params.m_szSoundName), g_appTitle,
										 MX_MB_OK);
						}
						else
						{
					...
					{
						mxMessageBox(NULL, va("Script '%s' does not exist", g_Params.m_szScriptName), g_appTitle,
									 MX_MB_OK);
						return TRUE;
					}
					...
					if(!filesystem->IsFileWritable(g_Params.m_szScriptName))
					{
						mxMessageBox(
							NULL,
							va("Script '%s' is read-only, you need to check it out of VSS", g_Params.m_szScriptName),
							g_appTitle, MX_MB_OK);
						return TRUE;
					}
...
int AddSound(CAddSoundParams *params, HWND parent)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_ADDSOUNDENTRY), parent,
