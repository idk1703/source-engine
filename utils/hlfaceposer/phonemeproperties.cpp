...
static void CreateAndLayoutControls(HWND hwndDlg, CPhonemeParams *params)
{
	...
	if(params->m_bPositionDialog)
	{
		...
		MoveWindow(hwndDlg, left, top, dialogW, dialogH, TRUE);
	}
	else
	{
		MoveWindow(hwndDlg, (GetSystemMetrics(SM_CXFULLSCREEN) - dialogW) / 2,
				   (GetSystemMetrics(SM_CYFULLSCREEN) - dialogH) / 2, dialogW, dialogH, TRUE);
	}
	...
	for(int i = 0; i < g_nPhonemeCount; i++)
	{
		...
		HWND button = CreateWindowEx(0, "BUTTON", va("%s", exp->name), WS_CHILD | WS_VISIBLE | BS_LEFT,
									 startx + c * (PHONEME_WIDTH + PHONEME_GAP),
									 starty + r * (PHONEME_HEIGHT + PHONEME_VGAP), PHONEME_WIDTH, PHONEME_HEIGHT,
									 hwndDlg, (HMENU)(IDC_PHONEME + i), (HINSTANCE)GetModuleHandle(0), NULL);
		Assert(button);
...
static BOOL CALLBACK PhonemePropertiesDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
			...
			// ...
			{
				...
				if(control)
				{
					SendMessage(control, WM_SETTEXT, 0,
								(LPARAM)(g_Params.m_bMultiplePhoneme
		...
		case WM_COMMAND:
		{
			...
			if((cmd >= IDC_PHONEME) && (cmd < (IDC_PHONEME + g_nPhonemeCount)))
			{
...
int PhonemeProperties(CPhonemeParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_PHONEMEPROPERTIES),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)PhonemePropertiesDialogProc);
