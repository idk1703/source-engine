...
void CEventPropertiesSequenceDialog::InitControlData(CEventParams *params)
{
	...
	SendMessage(GetControl(IDC_CHECK_PLAYOVERSCRIPT), BM_SETCHECK,
				(WPARAM)g_Params.m_bPlayOverScript ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
	...
	switch(uMsg)
	{
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				{
					g_Params.m_bPlayOverScript =
						SendMessage(GetControl(IDC_CHECK_PLAYOVERSCRIPT), BM_GETCHECK, 0, 0) == BST_CHECKED ? true
																											: false;
				}
				break;
...
int EventProperties_Sequence(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_SEQUENCE),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesSequenceDialogProc);
