...
void CEventPropertiesMoveToDialog::InitControlData(CEventParams *params)
{
	...
	SendMessage(GetControl(IDC_CHECK_ENDTIME), BM_SETCHECK,
				(WPARAM)g_Params.m_bHasEndTime ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);

	SendMessage(GetControl(IDC_CHECK_RESUMECONDITION), BM_SETCHECK,
				(WPARAM)g_Params.m_bResumeCondition ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
	...
	SendMessage(GetControl(IDC_CHECK_FORCESHORTMOVEMENT), BM_SETCHECK,
				(WPARAM)g_Params.m_bForceShortMovement ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDOK:
				{
					...
					if(control)
					{
						SendMessage(control, WM_GETTEXT, (WPARAM)sizeof(g_Params.m_szParameters),
									(LPARAM)g_Params.m_szParameters);
					}
				...
				{
					g_Params.m_bHasEndTime =
						SendMessage(GetControl(IDC_CHECK_ENDTIME), BM_GETCHECK, 0, 0) == BST_CHECKED ? true : false;
				...
				{
					g_Params.m_bResumeCondition =
						SendMessage(GetControl(IDC_CHECK_RESUMECONDITION), BM_GETCHECK, 0, 0) == BST_CHECKED ? true
																											 : false;
				}
				break;
				...
				case IDC_EVENTCHOICES2:
				{
					...
					if(control)
					{
						...
						{
							SendMessage(control, WM_GETTEXT, (WPARAM)sizeof(g_Params.m_szParameters2),
										(LPARAM)g_Params.m_szParameters2);
						}
						else
						{
							char buf1[256];
				...
				case IDC_EVENTCHOICES3:
				{
					...
					if(control)
					{
						if(g_Params.m_nType != CChoreoEvent::MOVETO)
						{
							SendMessage(control, WM_GETTEXT, (WPARAM)sizeof(g_Params.m_szParameters3),
										(LPARAM)g_Params.m_szParameters3);
						}
						else
						{
							char buf1[256];
				...
				{
					g_Params.m_bForceShortMovement =
						SendMessage(GetControl(IDC_CHECK_FORCESHORTMOVEMENT), BM_GETCHECK, 0, 0) == BST_CHECKED ? true
																												: false;
				}
				break;
				default:
...
int EventProperties_MoveTo(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_MOVETO),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesMoveToDialogProc);
