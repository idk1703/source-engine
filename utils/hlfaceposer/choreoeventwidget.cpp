...
// Input  : *parent -
//-----------------------------------------------------------------------------
CChoreoEventWidget::CChoreoEventWidget(CChoreoWidget *parent) : CChoreoWidget(parent)
{
	m_pEvent = NULL;
	m_pParent = parent;
	m_pWaveFile = NULL;
...
//			&rcWAV -
//-----------------------------------------------------------------------------
void CChoreoEventWidget::DrawRelativeTags(CChoreoWidgetDrawHelper &drawHelper, RECT &rcWAV, float length,
										  CChoreoEvent *event)
{
...
//			&rcWAV -
//-----------------------------------------------------------------------------
void CChoreoEventWidget::DrawAbsoluteTags(CChoreoWidgetDrawHelper &drawHelper, RECT &rcWAV, float length,
										  CChoreoEvent *event)
{
...
void CChoreoEventWidget::redrawStatus(CChoreoWidgetDrawHelper &drawHelper, RECT &rcClient)
{
	...
	if(CChoreoEventWidget::GetImage(event->GetType()))
	{
		...
		if(image)
		{
			...
			DrawBitmapToDC(dc, rcFixed.left, rcFixed.top, leftAdd, leftAdd, *image);
		}
	}
	...
	if(event->IsResumeCondition())
	{
		...
		RECT rcFixed = rc;
		drawHelper.OffsetSubRect(rcFixed);
		DrawBitmapToDC(dc, rcFixed.left, rcFixed.top, rcFixed.right - rcFixed.left, rcFixed.bottom - rcFixed.top,
	...
	drawHelper.CalcTextWidth("Arial", fontsize, FW_NORMAL, "%s event \"%s\"", event->NameForType(event->GetType()),
							 event->GetName());
	...
	drawHelper.DrawColoredText("Arial", fontsize, FW_NORMAL, COLOR_INFO_TEXT, rcEventLabel, "parameters \"%s\"",
							   GetLabelText());
}
...
void CChoreoEventWidget::DrawSpeakEvent(CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventLine)
{
	...
	if(IsSelected())
	{
		sound->RenderWavToDC(dc, rcEventLine,
							 GrayOutColor(IsSelected() ? COLOR_CHOREO_EVENT_SELECTED : COLOR_CHOREO_EVENT), 0.0,
							 m_pWaveFile->GetRunningLength(), m_pWaveFile);
	}

	// FrameRect( dc, &rcEventLine, brEvent );
	drawHelper.DrawColoredLine(GrayOutColor(COLOR_CHOREO_EVENT), PS_SOLID, 3, rcEventLine.left, rcEventLine.top,
							   rcEventLine.left, rcEventLine.bottom);
	drawHelper.DrawColoredLine(GrayOutColor(COLOR_CHOREO_EVENT), PS_SOLID, 3, rcEventLine.right, rcEventLine.top,
...
void CChoreoEventWidget::DrawGestureEvent(CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventLine)
{
	...
	*/
	...
	{
		drawHelper.DrawOutlinedRect(GrayOutColor(RGB(150, 180, 250)), PS_SOLID, 1, rcEventLine2);
	}
	else
	{
		drawHelper.DrawColoredLine(GrayOutColor(RGB(127, 127, 127)), PS_SOLID, 1, rcEventLine2.left,
								   rcEventLine2.bottom, rcEventLine2.left, rcEventLine2.top);
	...
	if(!nullevent)
	{
		drawHelper.DrawColoredRamp(clrEvent, PS_SOLID, 1, rampstart, rcEventLine2.bottom, rcEventLine2.left,
								   rcEventLine2.top, 0.0f, 1.0f);
		drawHelper.DrawColoredRamp(clrEvent, PS_SOLID, 1, rcEventLine2.right, rcEventLine2.top, rampend,
								   rcEventLine2.bottom, 0.0f, 1.0f);
	}
...
void CChoreoEventWidget::DrawGenericEvent(CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventLine)
{
	...
	if(ramponly && IsSelected())
	{
		drawHelper.DrawOutlinedRect(GrayOutColor(RGB(150, 180, 250)), PS_SOLID, 1, rcEventLine);
	}
	else
	{
...
void CChoreoEventWidget::redraw(CChoreoWidgetDrawHelper &drawHelper)
{
	...
	else
	{
		...
		drawHelper.DrawColoredLine(GrayOutColor(COLOR_CHOREO_EVENT), PS_SOLID, 3, rcEventLine.left - 1, rcEventLine.top,
								   rcEventLine.left - 1, rcEventLine.bottom);
	}
	...
	if(event->IsUsingRelativeTag())
	{
		...
		drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, GrayOutColor(RGB(0, 100, 200)), rcTagName,
	...
	if(CChoreoEventWidget::GetImage(event->GetType()))
	{
		...
		if(image)
		{
			DrawBitmapToDC(dc, rcEventLabel.left, rcEventLabel.top, leftAdd, leftAdd, *image);
		}
	}
	...
	drawHelper.DrawColoredText("Arial", fontsize, FW_NORMAL, GrayOutColor(RGB(0, 0, 120)), rcEventLabel,
							   event->GetName());
}
...
//-----------------------------------------------------------------------------
// Purpose: If the user changes the association of .mdls to actors, then the gender could change and we could need to
// access a different .wav file Input  :  -
//-----------------------------------------------------------------------------
