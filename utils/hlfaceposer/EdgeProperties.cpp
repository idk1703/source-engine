...
{
	SendMessage(GetDlgItem(hwndDlg, left ? IDC_LEFT_ACTIVE : IDC_RIGHT_ACTIVE), BM_SETCHECK, (WPARAM)BST_UNCHECKED,
				(LPARAM)0);
	...
	SendMessage(GetDlgItem(hwndDlg, IDC_HOLD_OUT), BM_SETCHECK, (WPARAM)BST_UNCHECKED, (LPARAM)0);
}
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			SendMessage(GetDlgItem(hwndDlg, IDC_LEFT_ACTIVE), BM_SETCHECK,
						(WPARAM)g_Params.m_bActive[0] ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);

			SendMessage(GetDlgItem(hwndDlg, IDC_RIGHT_ACTIVE), BM_SETCHECK,
						(WPARAM)g_Params.m_bActive[1] ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				case IDC_LEFT_CURVETYPE:
				{
					...
					{
						SendMessage(GetDlgItem(hwndDlg, IDC_LEFT_ACTIVE), BM_SETCHECK, (WPARAM)BST_CHECKED, (LPARAM)0);
					}
				}
				break;
				case IDC_RIGHT_CURVETYPE:
				{
					if(HIWORD(wParam) == CBN_SELCHANGE)
					{
						SendMessage(GetDlgItem(hwndDlg, IDC_RIGHT_ACTIVE), BM_SETCHECK, (WPARAM)BST_CHECKED, (LPARAM)0);
					}
				}
				break;
				...
				case IDOK:
				{
					...
					g_Params.m_bActive[0] =
						SendMessage(GetDlgItem(hwndDlg, IDC_LEFT_ACTIVE), BM_GETCHECK, 0, 0) == BST_CHECKED ? true
																											: false;
					g_Params.m_bActive[1] =
						SendMessage(GetDlgItem(hwndDlg, IDC_RIGHT_ACTIVE), BM_GETCHECK, 0, 0) == BST_CHECKED ? true
																											 : false;
...
int EdgeProperties(CEdgePropertiesParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EDGEPROPERTIES),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EdgePropertiesDialogProc);
