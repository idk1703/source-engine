...
{
public:
	CWaveList(mxWindow *parent, int id = 0) : mxListView(parent, 0, 0, 0, 0, id)
	{
		// Add column headers
...
{
public:
	CWaveFileTree(mxWindow *parent, int id = 0) : mxTreeView(parent, 0, 0, 0, 0, id), m_Paths(0, 0, FileTreeLessFunc) {}
...
//-----------------------------------------------------------------------------
bool CWaveBrowser::LoadWaveFilesInDirectory(CUtlDict<CWaveFile *, int> &soundlist, char const *pDirectoryName,
...
void CWaveBrowser::PopulateTree(char const *subdirectory)
{
	...
	for(i = m_Sorted.FirstInorder(); i != m_Sorted.InvalidIndex(); i = m_Sorted.NextInorder(i))
	{
		...
		if((check_load_sentence_data || m_bTextSearch) && !wav->HasLoadedSentenceInfo() && !wav->IsAsyncLoading())
		{
		...
		if(wav->HasLoadedSentenceInfo())
		{
			...
			m_pListView->setLabel(slot, COL_PHONEMES,
								  wav->GetPhonemeCount() || wav->GetWordCount()
									  ? va("%i [ %i ]", wav->GetWordCount(), wav->GetPhonemeCount())
									  : "");
...
void CWaveBrowser::SetCurrent(char const *filename)
{
	...
	if(p && (!Q_strnicmp(p, "sound/", 6) || !Q_strnicmp(p, "sound\\", 6)))
	{
		p += 6;
	}
