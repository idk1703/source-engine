	...
	switch(event->event)
	{
		...
		case mxEvent::MouseDown:
		{
			...
			if(m_nDragging == DRAGTYPE_NONE)
			{
				...
				CExpressionSample *sample = GetSampleUnderMouse(
					event->x, event->y, ctrlDown ? FP_TL_ADDSAMPLE_TOLERANCE : FP_TL_SELECTION_TOLERANCE);
		...
		case mxEvent::MouseMove:
		{
			if(m_nDragging != DRAGTYPE_NONE)
			{
				...
				if(m_nDragging == DRAGTYPE_MOVEPOINTS_TIME || m_nDragging == DRAGTYPE_MOVEPOINTS_VALUE)
				{
					DrawSelf();
				}
			}
		...
		case mxEvent::MouseUp:
		{
			...
			if(m_nDragging != DRAGTYPE_NONE)
			{
				...
				else if(m_nDragging != DRAGTYPE_MOVEPOINTS_VALUE && m_nDragging != DRAGTYPE_MOVEPOINTS_TIME)
				{
					SelectPoints();
				}
				else
...
void TimelineItem::Draw(CChoreoWidgetDrawHelper &drawHelper)
{
	...
	if(!IsCollapsed() && track)
	{
		if(m_nEditType == 1)
		{
			...
			drawHelper.DrawColoredLine(RGB(180, 200, 220), PS_SOLID, 1, rcClient.left,
									   (rcClient.top * zero + rcClient.bottom * (1 - zero)), rcClient.right,
									   (rcClient.top * zero + rcClient.bottom * (1 - zero)));
		}
	...
	if(track)
	{
		...
		if(scount > 0)
		{
			...
			drawHelper.DrawColoredText("Arial", 9, 500, RGB(120, 120, 0), title, sz);
...
void TimelineItem::DrawGrowHandle(CChoreoWidgetDrawHelper &helper, RECT &handleRect)
{
	...
	helper.DrawColoredLine(RGB(63, 63, 63), PS_SOLID, 1, handleRect.left + lineinset, midy,
