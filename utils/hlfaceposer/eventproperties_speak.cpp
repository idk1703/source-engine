		...
		if(soundindex >= 0)
		{
			...
			if(params)
			{
				...
				if(scriptfile)
				{
					...
					if(filesystem->FileExists(scriptfile) && filesystem->IsFileWritable(scriptfile))
					{
						enabled = true;
					}
				}
...
void CEventPropertiesSpeakDialog::OnCheckChangedVolumeLevel(CEventParams *params)
{
	...
	char const *scriptfile = soundemitter->GetSourceFileForSound(soundindex);
	if(!scriptfile || !filesystem->FileExists(scriptfile) || !filesystem->IsFileWritable(scriptfile))
	{
		return;
	}
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				case IDC_CAPTION_ATTENUATION:
				{
					...
					g_Params.m_bCloseCaptionNoAttenuate =
						SendMessage(control, BM_GETCHECK, 0, 0) == BST_CHECKED ? true : false;
				}
				break;
				case IDC_PLAY_SOUND:
				{
					// Get sound name from soundemitter
					sound->PlaySound(NULL, 1.0f, va("sound/%s", FacePoser_TranslateSoundName(g_Params.m_szParameters)),
									 NULL);
				}
				break;
				case IDC_OPENSOURCE:
				...
				{
					m_bShowAll =
						SendMessage(GetControl(IDC_SHOW_ALL_SOUNDS), BM_GETCHECK, 0, 0) == BST_CHECKED ? true : false;
...
int EventProperties_Speak(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_SPEAK),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesSpeakDialog);
