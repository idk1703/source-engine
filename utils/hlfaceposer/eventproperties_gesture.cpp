...
void CEventPropertiesGestureDialog::InitControlData(CEventParams *params)
{
	...
	SendMessage(GetControl(IDC_CHECK_SYNCTOFOLLOWINGGESTURE), BM_SETCHECK,
				(WPARAM)g_Params.m_bSyncToFollowingGesture ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
...
void CEventPropertiesGestureDialog::ShowControlsForEventType(CEventParams *params)
{
	...
	if(g_Params.m_nType == CChoreoEvent::GESTURE && !Q_stricmp(g_Params.m_szName, "NULL"))
	{
	...
	switch(uMsg)
	{
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				{
					g_Params.m_bSyncToFollowingGesture =
						SendMessage(GetControl(IDC_CHECK_SYNCTOFOLLOWINGGESTURE), BM_GETCHECK, 0, 0) == BST_CHECKED
							? true
							: false;
				}
				break;
...
int EventProperties_Gesture(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_GESTURE),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesGestureDialogProc);
