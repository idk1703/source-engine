...
void SetCloseCaptionLanguageId(int id, bool force /* = false */)
{
	...
	if(changed || force)
	{
		...
		if(Q_stricmp(suffix, "unknown_language"))
		{
			...
			if(Q_stricmp(suffix, "english") && filesystem->FileExists("resource/closecaption_english.txt"))
			{
...
bool MakeFileWriteablePrompt(const char *filename, char const *promptTitle)
{
	...
	{
		int retval = mxMessageBox(NULL, va("File '%s' is Read-Only, make writable?", filename), promptTitle,
								  MX_MB_WARNING | MX_MB_YESNO);
...
void FacePoser_LoadWindowPositions(char const *name, bool &visible, int &x, int &y, int &w, int &h, bool &locked,
								   bool &zoomed)
{
	char subkey[512];
	int v;
...
bool FacePoser_ShowFileNameDialog(bool openFile, char *relative, size_t bufsize, char const *subdir,
								  char const *wildcard)
{
	Assert(relative);
	relative[0] = 0;
	...
	if(openFile)
	{
		fullpath = mxGetOpenFileName(
			0, inWorkingDirectoryAlready ? "." : FacePoser_MakeWindowsSlashes(va("%s%s/", GetGameDirectory(), subdir)),
			wildcard);
	}
	else
	{
...
char const *FacePoser_DescribeSnappedTime(float t)
{
	...
	{
		Q_snprintf(desc, sizeof(desc), "frame %i (time %i + %i/%i s.)", frame, ipart, fracpart, fps);
	}
...
char const *FacePoser_TranslateSoundName(char const *soundname, StudioModel *model /*= NULL*/)
{
	...
	if(model)
	{
		Q_strncpy(temp, PSkipSoundChars(soundemitter->GetWavFileForSound(soundname, model->GetFileName())),
				  sizeof(temp));
	}
	else
	{
...
char const *FacePoser_TranslateSoundName(CChoreoEvent *event)
{
	...
	Q_strncpy(temp, PSkipSoundChars(soundemitter->GetWavFileForSound(soundname, model ? model->GetFileName() : NULL)),
			  sizeof(temp));
	return temp;
}
