	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			switch(g_Params.m_nType)
			{
				...
				{
					SendMessage(GetDlgItem(hwndDlg, IDC_LOOPCOUNT), WM_SETTEXT, 0,
								(LPARAM)va("%i", g_Params.m_nLoopCount));
					SendMessage(GetDlgItem(hwndDlg, IDC_LOOPTIME), WM_SETTEXT, 0,
								(LPARAM)va("%f", g_Params.m_flLoopTime));
			...
			SendMessage(GetDlgItem(hwndDlg, IDC_CHECK_AUTOCHECK), BM_SETCHECK,
						(WPARAM)g_Params.m_bAutomate ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDOK:
				{
					...
					SendMessage(GetDlgItem(hwndDlg, IDC_CB_AUTOACTION), WM_GETTEXT, (WPARAM)sizeof(g_Params.m_szType),
								(LPARAM)g_Params.m_szType);

					GetDlgItemText(hwndDlg, IDC_DURATION, szTime, sizeof(szTime));
					g_Params.m_flWaitTime = atof(szTime);

					g_Params.m_bAutomate =
						SendMessage(GetDlgItem(hwndDlg, IDC_CHECK_AUTOCHECK), BM_GETCHECK, 0, 0) == BST_CHECKED ? true
																												: false;
...
int GlobalEventProperties(CGlobalEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_GLOBALEVENTPROPERTIES),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)GlobalEventPropertiesDialogProc);
