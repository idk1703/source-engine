...
void CEventPropertiesLookAtDialog::SetupLookAtControls(CEventParams *params)
{
	...
	if(params->pitch != 0 || params->yaw != 0)
	{
...
BOOL CEventPropertiesLookAtDialog::HandleMessage(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		...
		{
			HWND control = (HWND)lParam;
			if(control == GetControl(IDC_SLIDER_YAW) || control == GetControl(IDC_SLIDER_PITCH))
			{
				...
				if(g_Params.pitch != 0 || g_Params.yaw != 0)
				{
...
int EventProperties_LookAt(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_LOOKAT),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesLookAtDialogProc);
