...
AnimationBrowser::AnimationBrowser(mxWindow *parent, int id /*=0*/)
	: IFacePoserToolWindow("AnimationBrowser", "Animations"), mxWindow(parent, 0, 0, 0, 0, "AnimationBrowser", id)
{
	setId(id);
...
void AnimationBrowser::DrawSequenceDescription(CChoreoWidgetDrawHelper &helper, int x, int y, int w, int h,
											   int sequence, mstudioseqdesc_t &seqdesc)
{
	int textheight = 15;
...
void AnimationBrowser::redraw()
{
	...
	helper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(63, 63, 63), rcText, "%i sequences", curcount);
}
