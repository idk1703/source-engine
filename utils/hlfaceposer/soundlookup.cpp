		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDOK:
				{
					...
					Q_strncpy(g_Params.m_szSoundName, soundemitter->GetSoundName(soundindex),
							  sizeof(g_Params.m_szSoundName));
					EndDialog(hwndDlg, 1);
				}
				break;
				case IDCANCEL:
				...
				case IDC_ADDENTRY:
				{
					...
					if(AddSound(&params, hwndDlg))
					{
						...
						if(params.m_szSoundName[0] && params.m_szScriptName[0])
						{
...
int SoundLookup(CSoundLookupParams *params, HWND parent)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_WAVELOOKUP), parent,
