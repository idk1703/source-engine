...
void CAudioWaveInput::OpenDevice(void)
{
	...
	MMRESULT errorCode =
		waveInOpen(&m_deviceHandle, m_deviceId, &format, (DWORD)WaveData, (DWORD)this, CALLBACK_FUNCTION);
	if(errorCode == MMSYSERR_NOERROR)
	{
		// valid device opened
...
#define WAVE_FORMAT_STEREO \
	(WAVE_FORMAT_1S08 | WAVE_FORMAT_1S16 | WAVE_FORMAT_2S08 | WAVE_FORMAT_2S16 | WAVE_FORMAT_4S08 | WAVE_FORMAT_4S16)
...
void CAudioDeviceSWMix::Mix8Mono(channel_t *pChannel, char *pData, int outputOffset, int inputOffset, int rateScaleFix,
								 int outCount, int timecompress, bool forward)
{
	int sampleIndex = 0;
...
void CAudioDeviceSWMix::Mix8Stereo(channel_t *pChannel, char *pData, int outputOffset, int inputOffset,
								   int rateScaleFix, int outCount, int timecompress, bool forward)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void CAudioDeviceSWMix::Mix16Mono(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
								  int rateScaleFix, int outCount, int timecompress, bool forward)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
...
void CAudioDeviceSWMix::Mix16Stereo(channel_t *pChannel, short *pData, int outputOffset, int inputOffset,
									int rateScaleFix, int outCount, int timecompress, bool forward)
{
	int sampleIndex = 0;
	fixedint sampleFrac = inputOffset;
	...
	if(time > m_mixTime)
	{
		...
		while(tempCount > 0)
		{
			...
			for(int i = 0; i < MAX_CHANNELS; i++)
			{
				...
				{
					if(!pSource->MixDataToDevice(&m_audioDevice, pSource->GetChannel(), currentsample, sampleCount,
												 SampleRate(), forward))
					{
				...
				else
				{
					...
					{
						pSource->IncrementSamples(pSource->GetChannel(), currentsample, sampleCount, SampleRate(),
												  forward);
					}
				}
			}
...
public:
	...
	void PlayPartialSound(StudioModel *model, float volume, const char *wavfile, CAudioMixer **ppMixer, int startSample,
						  int endSample);
	...
	void RenderWavToDC(HDC dc, RECT &outrect, COLORREF clr, float starttime, float endtime, CAudioSource *pWave,
...
void CFacePoserSound::PlayPartialSound(StudioModel *model, float volume, const char *wavfile, CAudioMixer **ppMixer,
					...
					for(int k = 0; k < word->m_Phonemes.Size(); k++)
					{
						...
						if(t > phoneme->GetStartTime() && t < phoneme->GetEndTime())
						{
							...
							if(next)
							{
								...
								{
									// no gap, so increase the blend length to the end of the next phoneme, as long as
									// it's not longer than the current phoneme
									dt = max(dt, min(next->GetEndTime() - t,
													 phoneme->GetEndTime() - phoneme->GetStartTime()));
								}
								else
								{
									// dead space, so increase the blend length to the start of the next phoneme, as
									// long as it's not longer than the current phoneme
									dt = max(dt, min(next->GetStartTime() - t,
													 phoneme->GetEndTime() - phoneme->GetStartTime()));
								}
							}
							else
...
void CFacePoserSound::RenderWavToDC(HDC dc, RECT &outrect, COLORREF clr, float starttime, float endtime,
									CAudioSource *pWave, bool selected /*= false*/, int selectionstart /*= 0*/,
									int selectionend /*= 0*/)
{
	channel_t channel;
	...
	while(currenttime < endtime)
	{
		...
		if(!pMixer->MixDataToDevice(&pWaveOutput->m_audioDevice, &channel, currentsample, samplecount,
									pWave->SampleRate(), true))
			break;
		...
		if(currenttime >= starttime)
		{
			...
			{
				bool boundary = false;
				bool inselection = (currenttime >= selectionstarttime && currenttime <= selectionendtime);
