...
// Input  : *parent -
//-----------------------------------------------------------------------------
CChoreoChannelWidget::CChoreoChannelWidget(CChoreoActorWidget *parent) : CChoreoWidget(parent)
{
	m_pChannel = NULL;
	m_pParent = parent;
	m_bHasAudio = false;
...
void CChoreoChannelWidget::redraw(CChoreoWidgetDrawHelper &drawHelper)
{
	...
	drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() + 2, FW_HEAVY,
	...
	if(!channel->GetActive())
	{
		...
		drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() - 2, 500, COLOR_CHOREO_ACTORNAME_INACTIVE,
								   rcInactive, n);
	}
...
void CChoreoChannelWidget::RenderCloseCaptionInfo(CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventArea)
{
...

	{
		...
		drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() - 2, 500, COLOR_CHOREO_TEXT, rcText, "token/data:");
	}
	...
	for(int j = GetNumEvents() - 1; j >= 0; j--)
	{
		...
		if(e->GetCloseCaptionType() == CChoreoEvent::CC_MASTER)
		{
			...
			{
				stateValid = closecaptionmanager->LookupUnicodeText(GetCloseCaptionLanguageId(), cctoken, wstr,
		...
		if(showState)
		{
			...
			drawHelper.DrawColoredTextCharset("Marlett", m_pView->GetFontSize() - 2, 500, SYMBOL_CHARSET, symColor,
											  rcState, stateValid ? "a" : "r");
		}

		if(e->IsSuppressingCaptionAttenuation())
		{
			drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() - 2, 500, RGB(80, 80, 100), rcText,
									   "%s [no attenuate]", label);
		}
		else
		{
			drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() - 2, 500, RGB(80, 80, 100), rcText, label);
		}
	}
}
...
{
	{
		...
		rcText.bottom = rcText.top + 12;
		rcText.left += 12;
		drawHelper.DrawColoredText("Arial", m_pView->GetFontSize() - 2, 500, COLOR_CHOREO_TEXT, rcText, "%s",
								   CSentence::NameForLanguage(GetCloseCaptionLanguageId()));

		// Previous
		GetCloseCaptionLanguageRect(rcText, true);
		drawHelper.DrawColoredTextCharset("Marlett", m_pView->GetFontSize(), 500, SYMBOL_CHARSET, COLOR_CHOREO_TEXT,
										  rcText, "3");
	...
	for(int j = GetNumEvents() - 1; j >= 0; j--)
	{
		...
		if(e->GetCloseCaptionType() == CChoreoEvent::CC_SLAVE || e->GetCloseCaptionType() == CChoreoEvent::CC_DISABLED)
			continue;
		...
		valid = closecaptionmanager->LookupStrippedUnicodeText(GetCloseCaptionLanguageId(), cctoken, wstr,
															   sizeof(wstr) / sizeof(wchar_t));
...
void CChoreoChannelWidget::GetCloseCaptionLanguageRect(RECT &rc, bool previous)
{
	...
	if(!previous)
	{
		int textlen = CChoreoWidgetDrawHelper::CalcTextWidth("Arial", m_pView->GetFontSize() - 2, 500,
															 CSentence::NameForLanguage(GetCloseCaptionLanguageId()));
...
void CChoreoChannelWidget::RenderCloseCaptionSelectors(CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventArea)
{
	...
	if(vecSelectors.Count() > 0)
	{
		for(int i = 0; i < c; ++i)
		{
			...
			drawHelper.DrawCircle(clr, endpt.x, endpt.y, 3, true);
...
void CChoreoChannelWidget::RenderCloseCaptionExpandCollapseRect(CChoreoWidgetDrawHelper &drawHelper, RECT &rcEventArea)
{
	...
	drawHelper.DrawColoredTextCharset("Marlett", m_pView->GetFontSize(), 900, SYMBOL_CHARSET, symColor, rcCCArea,
...
void CChoreoChannelWidget::redrawStatus(CChoreoWidgetDrawHelper &drawHelper, RECT &rcClient, int areaUnderMouse)
{
	...
	*/
	...
	if(showState)
	{
		...
		drawHelper.DrawColoredTextCharset("Marlett", fontsize + 2, 500, SYMBOL_CHARSET, symColor, rcState,
										  stateValid ? "a" : "r");
	}
	...
	{
		if(e->GetNumSlaves() >= 1 || e->GetCloseCaptionType() == CChoreoEvent::CC_SLAVE)
		{
			...
			char sz[256];
			Q_snprintf(sz, sizeof(sz), "combined file active [ %s ] gender[ %s ] up-to-date[ ",
					   e->IsUsingCombinedFile() ? "yes" : "no", e->IsCombinedUsingGenderToken() ? "yes" : "no");
			...
			drawHelper.DrawColoredText("Arial", fontsize, 500, combinedValid ? RGB(20, 150, 20) : RGB(255, 0, 0),
									   rcPartial, sz);
		...
		if(soundindex >= 0)
		{
			...
			if(scriptfile)
			{
				drawHelper.DrawColoredText("Arial", fontsize, 500, labelColor, rcText, "sound script:  %s", scriptfile);
			}
		}
		else
		{
			drawHelper.DrawColoredText("Arial", fontsize, 500, RGB(255, 0, 0), rcText,
									   "sound not in game_sounds script files!");
		}
	}
}