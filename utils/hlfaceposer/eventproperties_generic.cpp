		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDOK:
				{
					...
					{
						Q_snprintf(g_Params.m_szName, sizeof(g_Params.m_szName), "%s to %s", g_Params.m_szParameters,
...
int EventProperties_Generic(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_GENERIC),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesGenericDialogProc);
