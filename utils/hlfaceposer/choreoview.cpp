...
bool CChoreoView::Close(void)
{
	...
	{
		int retval =
			mxMessageBox(NULL, va("Save changes to scene '%s'?", GetChoreoFile()), g_appTitle, MX_MB_YESNOCANCEL);
		if(retval == 2)
		{
			return false;
		}
...
void CChoreoView::DrawTimeLine(CChoreoWidgetDrawHelper &drawHelper, RECT &rc, float left, float right)
{
	...
	drawHelper.DrawColoredLine(COLOR_CHOREO_TIMELINE, PS_SOLID, 1, rc.left, GetStartRow() - 1, rc.right,
							   GetStartRow() - 1);
	...
	while(f < right)
	{
		...
		if(frac >= 0.0f && frac <= 1.0f)
		{
			...
			if(f != left)
			{
				drawHelper.DrawColoredLine(RGB(220, 220, 240), PS_DOT, 1, rcLabel.left, GetStartRow(), rcLabel.left,
										   h2());
			}
		...
		for(int j = 0; j < a->GetNumChannels(); j++)
		{
			...
			for(int k = 0; k < c->GetNumEvents(); k++)
			{
				...
				CEventRelativeTag *tag =
					m_pScene->FindTagByName(event->GetRelativeWavName(), event->GetRelativeTagName());
...
void CChoreoView::redraw()
{
	...
	if(!clipped)
	{
		drawHelper.DrawColoredLine(COLOR_CHOREO_ENDTIME, PS_DOT, 1, finishx, rc.top + GetStartRow(), finishx,
								   rc.bottom);
	}
	...
	{
		int length = drawHelper.CalcTextWidth("Arial", 9, FW_NORMAL, "undo %i/%i", m_nUndoLevel, m_UndoStack.Size());
		RECT rcText = rc;
		...
		drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(100, 180, 100), rcText, "undo %i/%i", m_nUndoLevel,
								   m_UndoStack.Size());
	}
...
void CChoreoView::PlaceABPoint(int mx)
{
	...
	if(m_rgABPoints[0].active && m_rgABPoints[1].active && m_rgABPoints[0].time > m_rgABPoints[1].time)
	{
...
void CChoreoView::ShowContextMenu(int mx, int my)
{
	...
	bool onmaster = (m_pClickedChannel && m_pClickedChannel->GetCaptionClickedEvent() &&
					 m_pClickedChannel->GetCaptionClickedEvent()->GetCloseCaptionType() == CChoreoEvent::CC_MASTER)
						? true
						: false;
	bool ondisabled = (m_pClickedChannel && m_pClickedChannel->GetCaptionClickedEvent() &&
					   m_pClickedChannel->GetCaptionClickedEvent()->GetCloseCaptionType() == CChoreoEvent::CC_DISABLED)
						  ? true
						  : false;

	// The close captioning menu
	if(m_bShowCloseCaptionData &&
	   (AreSelectedEventsCombinable() || AreSelectedEventsInSpeakGroup() || onmaster || ondisabled))
	{
		pop->addSeparator();
...
void CChoreoView::AssociateModel(void)
{
	...
	// Chose something new...
	if(params.m_nSelected >= 0 && params.m_nSelected < params.m_Choices.Count())
	{
	...
	for(int i = 0; i < m_SceneActors.Size(); i++)
	{
		...
		for(int j = 0; j < actor->GetNumChannels(); j++)
		{
			...
			for(int k = 0; k < channel->GetNumEvents(); k++)
			{
				...
				if(event == m_pClickedEvent && (m_nClickedTag != -1 || m_pClickedAbsoluteTag))
				{
					int leftEdge = 0;
					int tagWidth = 1;
					...
					else
					{
						...
						{
							leftEdge =
								(int)(m_pClickedAbsoluteTag->GetPercentage() * (float)(bounds.right - bounds.left) +
									  0.5f);
						}
					}
...
void CChoreoView::MouseStartDrag(mxEvent *event, int mx, int my)
{
	...
	GetObjectsUnderMouse(mx, my, &m_pClickedActor, &m_pClickedChannel, &m_pClickedEvent, &m_pClickedGlobalEvent,
						 &m_nClickedTag, &m_pClickedAbsoluteTag, &m_nClickedChannelCloseCaptionButton);
	...
	if(m_pClickedEvent)
	{
		...
		bool bIsEdgeRescale =
			(dtPreview == DRAGTYPE_EVENT_ENDTIME_RESCALE || dtPreview == DRAGTYPE_EVENT_STARTTIME_RESCALE);
	...
	else if(m_pClickedChannel && m_nClickedChannelCloseCaptionButton != CChoreoChannelWidget::CLOSECAPTION_NONE &&
...
void CChoreoView::MouseContinueDrag(mxEvent *event, int mx, int my)
{
	...
	if(m_nDragType == DRAGTYPE_RESCALELEFT || m_nDragType == DRAGTYPE_RESCALERIGHT)
	{
					...
					switch(m_nDragType)
					{
						...
						case DRAGTYPE_EVENT_STARTTIME_RESCALE:
						{
							...
							switch(e->GetType())
							{
								...
								case CChoreoEvent::GESTURE:
								{
									e->RescaleGestureTimes(e->GetStartTime() + dt, e->GetEndTime(),
							...
							if(m_nClickedTag != -1)
							{
								...
								if(tag)
								{
									...
									if(bounds.right - bounds.left > 0)
									{
										int left =
											bounds.left +
											(int)(tag->GetPercentage() * (float)(bounds.right - bounds.left) + 0.5f);
		...
		if(flSelectionDuration > 0.0f)
		{
			...
			for(i = 0; i < rescaleHelper.Count(); ++i)
			{
				...
				switch(e->GetType())
				{
					...
					case CChoreoEvent::GESTURE:
					{
						e->RescaleGestureTimes(flNewStartTime, flNewStartTime + flNewDuration,
											   m_nDragType == DRAGTYPE_EVENT_STARTTIME ||
												   m_nDragType == DRAGTYPE_EVENT_ENDTIME);
					}
					break;
...
void SetupFlexControllerTracks(CStudioHdr *hdr, CChoreoEvent *event)
{
	...
	/*
	// FIXME:  Brian hooked this stuff up for some took work, but at this point the .mdl files don't look like they've
	been updated to include the remapping data yet... int c = hdr->numflexcontrollerremaps(); for ( i = 0; i < c; ++i )
	{
		...
		for ( int j = 0 ; j < remap->numremaps; ++j )
		{
			...
			Msg( "  %d:  maps to %d (%s) with %s\n", j, index, hdr->pFlexcontroller( index )->pszName(),
	remap->pRemapControl( j ) );
		}
	}
	*/
	...
	for(LocalFlexController_t i = LocalFlexController_t(0); i < hdr->numflexcontrollers(); ++i)
	{
		...
		if(combo)
		{
			track->SetFlexControllerIndex(LocalFlexController_t(i + 1),
...
void CChoreoView::ProcessLookat(CChoreoScene *scene, CChoreoEvent *event)
{
	...
	else if(!stricmp(event->GetParameters(), "player") || !stricmp(event->GetParameters(), "!player"))
	{
...
void CChoreoView::ProcessGesture(CChoreoScene *scene, CChoreoEvent *event)
{
	...
	float referencetime = event->GetOriginalPercentageFromPlaybackPercentage(eventlocaltime / event->GetDuration()) *
						  event->GetDuration();
	...
	if(tokenprocessor->TokenAvailable())
	{
		...
		if(!stricmp(tokenprocessor->CurrentToken(), "automate"))
		{
			if(tokenprocessor->TokenAvailable())
			{
				...
				if(tokenprocessor->TokenAvailable() && m_nAutomatedAction != SCENE_ACTION_UNKNOWN)
				{
					tokenprocessor->GetToken(false);
...
void CChoreoView::LayoutScene(void)
{
	...
	for(i = 0; i < m_SceneGlobalEvents.Size(); i++)
	{
		...
		rcEvent.left =
			GetLabelWidth() + rcEvent.left + (int)(frac * (m_rcTimeLine.right - m_rcTimeLine.left - GetLabelWidth()));
		rcEvent.left -= 4;
...
void CChoreoView::RepositionVSlider(void)
{
	...
	m_pVertScrollBar->setBounds(w2() - m_nScrollbarHeight, GetStartRow(), m_nScrollbarHeight,
...
void CChoreoView::Save(void)
{
	...
	{
		mxMessageBox(this, va("Unable to write \"%s\"", GetChoreoFile()), "SaveToFile", MX_MB_OK | MX_MB_ERROR);
	}
...
void CChoreoView::AddEvent(int type, int subtype /*= 0*/, char const *defaultparameters /*= NULL*/)
{
	...
	while(1)
	{
		...
		{
			mxMessageBox(this, va("Event must have a valid name"), "Edit Event", MX_MB_OK | MX_MB_ERROR);
			continue;
		}
...
void CChoreoView::EditEvent(CChoreoEvent *event)
{
	...
	while(1)
	{
		...
		if(Q_strlen(params.m_szName) <= 0)
		{
			mxMessageBox(this, va("Event %s must have a valid name", event->GetName()), "Edit Event",
						 MX_MB_OK | MX_MB_ERROR);
			continue;
		}
		...
		case CChoreoEvent::SPEAK:
		{
			if(ShouldProcessSpeak())
			{
				...
				if(event->GetCloseCaptionType() == CChoreoEvent::CC_MASTER)
				{
					...
					{
						float duration =
							max(event->GetDuration(), event->GetLastSlaveEndTime() - event->GetStartTime());
				...
				if(!mixer || !sound->IsSoundPlaying(mixer))
				{
					...
					if(!Q_stristr(soundname, ".wav") && soundemitter->GetParametersForSound(soundname, params, gender))
					{
						volume = params.volume;
					}

					sound->PlaySound(model, volume, va("sound/%s", FacePoser_TranslateSoundName(soundname, model)),
									 &mixer);
...
//			**event -
//-----------------------------------------------------------------------------
void CChoreoView::GetObjectsUnderMouse(int mx, int my, CChoreoActorWidget **actor, CChoreoChannelWidget **channel,
...
bool CChoreoView::CreateAnimationEvent(int mx, int my, char const *animationname)
{
	...
	GetObjectsUnderMouse(pt.x, pt.y, &m_pClickedActor, &m_pClickedChannel, &m_pClickedEvent, &m_pClickedGlobalEvent,
						 &m_nClickedTag, &m_pClickedAbsoluteTag, &m_nClickedChannelCloseCaptionButton);
	...
	// At this point we need to ask the user what type of even to create (gesture or sequence) and just show the
	// approprite dialog
	CChoiceParams params;
...
void CChoreoView::RedrawStatusArea(CChoreoWidgetDrawHelper &drawHelper, RECT &rcStatus)
{
	...
	drawHelper.DrawColoredLine(COLOR_INFO_BORDER, PS_SOLID, 1, rcStatus.left, rcStatus.top, rcStatus.right,
							   rcStatus.top);
...
void CChoreoView::ImportVCDFile(char const *filename)
{
	...
	// Now walk through the "new" events and select everything that wasn't already there (all of the stuff that was
...
void CChoreoView::ProcessSpeak(CChoreoScene *scene, CChoreoEvent *event)
{
	...
	if(!mixer || !sound->IsSoundPlaying(mixer))
	{
		...
		sound->PlaySound(model, volume, va("sound/%s", FacePoser_TranslateSoundName(soundname, model)), &mixer);
		event->SetMixer(mixer);
	}
...
// Output : float
//-----------------------------------------------------------------------------
float CChoreoView::FindNextEventTime(CChoreoEvent::EVENTTYPE type, CChoreoChannel *channel, CChoreoEvent *e,
									 bool forward)
{
	bool foundone = false;
	float bestTime = -1.0f;
...
void CChoreoView::SelectEvents(SelectionParams_t &params)
{
	...
	for(int i = 0; i < m_SceneActors.Size(); i++)
	{
		...
		for(int j = 0; j < a->GetNumChannels(); j++)
		{
			...
			if(params.type == SelectionParams_t::SP_CHANNEL && c->GetChannel() != channel)
				continue;

			if(params.type == SelectionParams_t::SP_ACTIVE && !c->GetChannel()->GetActive())
				continue;
...
void CChoreoView::CheckInsertTime(CChoreoEvent *e, float dt, float starttime, float endtime)
{
	...
	else if(!e->IsFixedLength() && e->HasEndTime()) // the event starts before start, but ends after start time, need to
													// insert space into the event, act like user dragged end time
	{
		float newduration = e->GetDuration() + dt;
		RescaleRamp(e, newduration);
		switch(e->GetType())
...
bool CChoreoView::GenerateCombinedFile(char const *outfilename, char const *cctoken, gender_t gender,
									   CUtlRBTree<CChoreoEvent *> &sorted)
{
	...
	if(i != sorted.InvalidIndex())
	{
		...
		do
		{
			...
			CombinerEntry ce;
			Q_snprintf(ce.wavefile, sizeof(ce.wavefile), "sound/%s",
...
bool CChoreoView::ValidateCombinedFileCheckSum(char const *outfilename, char const *cctoken, gender_t gender,
											   CUtlRBTree<CChoreoEvent *> &sorted)
{
	CUtlVector<CombinerEntry> work;
...
bool CChoreoView::AreSelectedEventsInSpeakGroup()
{
	...
	if(GetSelectedEvents(selected) <= 0)
	{
		...
		{
			CChoreoEvent *e = m_pClickedChannel->GetCaptionClickedEvent();
			if(e && e->GetCloseCaptionType() == CChoreoEvent::CC_MASTER && e->GetNumSlaves() >= 1)
			{
				return true;
			}
		}
...
void CChoreoView::OnChangeCloseCaptionToken(CChoreoEvent *e)
{
	...
	// User hit okay and value actually changed?
	if(CloseCaptionLookup(&params) && Q_stricmp(e->GetCloseCaptionToken(), params.m_szCCToken))
	{
