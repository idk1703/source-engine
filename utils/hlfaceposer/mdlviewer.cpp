...
public:
	CFacePoserVguiFrame(Panel *parent, const char *panelName) : BaseClass(parent, panelName)
	{
...
public:
	TestWindow(mxWindow *parent, int x, int y, int w, int h)
		: BaseClass(parent, x, y, w, h), IFacePoserToolWindow("FacePoser Frame", "FacePoser Frame")
	{
...
bool MDLViewer::CanClose()
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		if(!filesystem->FileExists(scriptname) || !filesystem->IsFileWritable(scriptname))
		{
			continue;
		}

		int retval =
			mxMessageBox(NULL, va("Save changes to sound script '%s'?", scriptname), g_appTitle, MX_MB_YESNOCANCEL);
		if(retval == 2)
		{
			return false;
		}
...
public:
	...
	CMDLViewerGridSettings(mxWindow *parent, int x, int y, int w, int h) : mxWindow(parent, x, y, w, h)
	{
					...
					case IDC_GRIDSETTINGS_FPS:
					{
						if(g_pChoreoView)
						{
							...
							if(scene)
							{
								...
								Q_snprintf(params.m_szInputText, sizeof(params.m_szInputText), "%i", currentFPS);
		...
		case mxEvent::Action:
		{
			...
			switch(event->action)
			{
				...
				case IDC_HELP_GOTOHOMEPAGE:
					ShellExecute(0, "open", "http://developer.valvesoftware.com/wiki/Category:Choreography", 0, 0,
								 SW_SHOW);
					break;
#endif
...
void MDLViewer::OnSaveSoundScriptChanges()
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		{
			mxMessageBox(NULL, va("Can't save changes to sound script '%s', file is READ-ONLY?", scriptname),
						 g_appTitle, MX_MB_OK);
			continue;
		}
...
bool CHLFacePoserApp::PreInit()
{
	...
	if(!soundemitter || !g_pLocalize || !filesystem || !physprop || !physcollision || !g_pMaterialSystem ||
...
static bool CHLFacePoserApp_SuggestGameInfoDirFn(CFSSteamSetupInfo const *pFsSteamSetupInfo, char *pchPathBuffer,
