		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDOK:
					...
					GetDlgItemTextW(hwndDlg, IDC_INPUTSTRING, g_Params.m_szInputText,
									ARRAYSIZE(g_Params.m_szInputText));
					EndDialog(hwndDlg, 1);
					break;
				case IDCANCEL:
					EndDialog(hwndDlg, 0);
...
int EditPhrase(CEditPhraseParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EDITPHRASE),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EditPhraseDialogProc);
