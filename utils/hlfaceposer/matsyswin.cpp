	...
	: IFacePoserToolWindow("3D View", "3D View"), mxMatSysWindow(parent, x, y, w, h, label, style)
{
	...
	g_materialBackground =
		g_pMaterialSystem->FindMaterial("particle/particleapp_background", TEXTURE_GROUP_OTHER, true);
...
void MatSysWindow::TakeScreenShot(const char *filename)
{
	redraw();
	int w = w2();
	int h = h2();
