		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDOK:
				{
					...
					{
						Q_snprintf(g_Params.m_szName, sizeof(g_Params.m_szName), "%s/%s", g_Params.m_szParameters,
								   g_Params.m_szParameters2);
					}
...
int EventProperties_Expression(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_EXPRESSION),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesExpressionDialogProc);
