			...
			switch(LOWORD(wParam))
			{
				case IDOK:
				{
					...
					*/
					...
					SendMessage(GetDlgItem(hwndDlg, IDC_CCTOKEN), WM_GETTEXT, (WPARAM)sizeof(g_Params.m_szCCToken),
								(LPARAM)g_Params.m_szCCToken);
					...
					if(item >= 0)
					{
						...
						if(ListView_GetItem(control, &lvi))
						{
							...
							if(name)
							{
								...
								SendMessage(GetDlgItem(hwndDlg, IDC_CCTOKEN), WM_SETTEXT,
											(WPARAM)sizeof(g_Params.m_szCCToken), (LPARAM)g_Params.m_szCCToken);
							}
						}
					}
					return FALSE;
...
int CloseCaptionLookup(CCloseCaptionLookupParams *params)
{
	...
	g_UnicodeFont = CreateFont(-10, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_TT_PRECIS,
							   CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, "Tahoma");

	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_CCLOOKUP), (HWND)g_MDLViewer->getHandle(),
