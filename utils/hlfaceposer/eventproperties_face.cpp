...
void CEventPropertiesFaceDialog::InitControlData(CEventParams *params)
{
	...
	SendMessage(GetControl(IDC_CHECK_LOCKBODYFACING), BM_SETCHECK,
				(WPARAM)params->m_bLockBodyFacing ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
}
	...
	switch(uMsg)
	{
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				...
				{
					g_Params.m_bLockBodyFacing =
						SendMessage(GetControl(IDC_CHECK_LOCKBODYFACING), BM_GETCHECK, 0, 0) == BST_CHECKED ? true
																											: false;
				}
				break;
...
int EventProperties_Face(CEventParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_EVENTPROPERTIES_FACE),
						   (HWND)g_MDLViewer->getHandle(), (DLGPROC)EventPropertiesFaceDialogProc);
