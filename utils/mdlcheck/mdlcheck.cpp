...
bool ValidateModelFile(char const *modelname, int offset)
{
	...
	if(checkani)
	{
		// HACK: since the sequence data is written after the animation data, this is rough way to determine how much
		// anim data there really is
...
void CheckForUnbuiltModels()
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		{
			vprint_queued(0, "Can't find %s,\n\tbuilt by %s\n", g_Analysis.models.GetElementName(i),
						  g_Analysis.models[i].qcfile);
		}
		...
		else if(g_Analysis.models[i].toobig)
		{
			vprint_queued(0, "%s needs $animblocksize command (%d of animdata),\n\tbuilt by %s\n",
						  g_Analysis.models.GetElementName(i), g_Analysis.models[i].toobig,
						  g_Analysis.models[i].qcfile);
		}
		else
		{
