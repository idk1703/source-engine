...
private:
	SmartIVtfTexture &operator=(IVTFTexture *pVtf)
	{
		m_p = pVtf;
		return *this;
	}
	operator IVTFTexture *() const
	{
		return m_p;
	}
...
void VTexConfigInfo_t::ParseOptionKey(const char *pKeyName, const char *pKeyValue)
{
	...
	else if(!stricmp(pKeyName, "specvar"))
	{
		...
		{
			m_vtfProcOptions.flags0 |= (VtfProcessingOptions::OPT_DECAY_R | VtfProcessingOptions::OPT_DECAY_EXP_R)
									   << iDecayChannel;
		...
		*/
		else
		{
			...
			for(char const *szParse = pKeyValue; szParse; szParse = strchr(szParse, ':'), szParse ? ++szParse : 0)
			{
				...
				else if(StringHasPrefix(szParse, "r=") || StringHasPrefix(szParse, "g=") ||
						StringHasPrefix(szParse, "b=") || StringHasPrefix(szParse, "a="))
				{
					int ch = 0;
					switch(*szParse)
					{
...
static ImageFormat ComputeDesiredImageFormat(IVTFTexture *pTexture, VTexConfigInfo_t &info)
{
	...
	if((nFlags & TEXTUREFLAGS_PROCEDURAL) || (info.m_vtfProcOptions.flags0 & VtfProcessingOptions::OPT_NOCOMPRESS) ||
...
//-----------------------------------------------------------------------------
void MakeSrcFileName(char *pSrcName, unsigned int flags, const char *pFullNameWithoutExtension, int frameID, int faceID,
					 int z, bool isCubeMap, int startFrame, int endFrame, bool bNormalToDUDV)
{
	...
	if(bAnimated)
	{
		...
		{
			Assert(z == -1);
			sprintf(pSrcName, "%s%s%03d%s", pFullNameWithoutExtension, g_CubemapFacingNames[faceID],
		...
		else
		{
			...
			{
				sprintf(pSrcName, "%s%03d_z%03d%s", pFullNameWithoutExtension, z, frameID + startFrame,
						GetSourceExtension());
			}
		}
	}
...
//-----------------------------------------------------------------------------
static void InitializeSrcTexture_PSD(IVTFTexture *pTexture, const char *pInputFileName, CUtlBuffer &psdBuffer,
									 int nDepth, int nFrameCount, const VTexConfigInfo_t &info)
{
	int nWidth, nHeight;
	ImageFormat imageFormat;
	float flSrcGamma;
...
//-----------------------------------------------------------------------------
static void InitializeSrcTexture_TGA(IVTFTexture *pTexture, const char *pInputFileName, CUtlBuffer &tgaBuffer,
									 int nDepth, int nFrameCount, const VTexConfigInfo_t &info)
{
	int nWidth, nHeight;
	ImageFormat imageFormat;
...
//-----------------------------------------------------------------------------
static void InitializeSrcTexture_PFM(IVTFTexture *pTexture, const char *pInputFileName, CUtlBuffer &fileBuffer,
									 int nDepth, int nFrameCount, const VTexConfigInfo_t &info)
{
...
static void InitializeSrcTexture(IVTFTexture *pTexture, const char *pInputFileName, CUtlBuffer &tgaBuffer, int nDepth,
								 int nFrameCount, const VTexConfigInfo_t &info)
{
	switch(g_eMode)
	{
		case eModePSD:
...
//-----------------------------------------------------------------------------
static bool LoadFaceFromPSD(IVTFTexture *pTexture, CUtlBuffer &psdBuffer, int z, int nFrame, int nFace, float flGamma,
							const VTexConfigInfo_t &info)
{
	...
	if((info.m_bAlphaToDistance) || (nWidth != pTexture->Width()) || (nHeight != pTexture->Height()))
	{
		// Load into temp
		Bitmap_t bmPsdData;
			...
			else
			{
				for(int x = 0; x < pTexture->Width(); x++)
				{
					for(int y = 0; y < pTexture->Height(); y++)
					{
						...
						if((nOutAlpha != 0) &&
						   ((x == 0) || (y == 0) || (x == pTexture->Width() - 1) || (y == pTexture->Height() - 1)))
						{
							bWarnEdges = true;
		...
		// now, store in dest
		ImageLoader::ConvertImageFormat(tmpDest.Base(), IMAGE_FORMAT_RGBA8888, pDestBits, pTexture->Format(),
...
//-----------------------------------------------------------------------------
static bool LoadFaceFromTGA(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z, int nFrame, int nFace, float flGamma,
							const VTexConfigInfo_t &info)
{
	// NOTE: This only works because all mip levels are stored sequentially
	// in memory, starting with the highest mip level. It also only works
	...
	if((info.m_bAlphaToDistance) || (nWidth != pTexture->Width()) || (nHeight != pTexture->Height()))
	{
		...
		if(!TGALoader::Load(tmpImage.Base(), tgaBuffer, nWidth, nHeight, IMAGE_FORMAT_RGBA8888, flGamma, false))
		{
			return false;
		}
	...
	{
		return TGALoader::Load(pDestBits, tgaBuffer, pTexture->Width(), pTexture->Height(), pTexture->Format(), flGamma,
							   false);
	}
}
...
static bool LoadFaceFromPFM(IVTFTexture *pTexture, CUtlBuffer &fileBuffer, int z, int nFrame, int nFace, float flGamma,
							const VTexConfigInfo_t &info)
{
	fileBuffer.SeekGet(CUtlBuffer::SEEK_HEAD, 0);
	if(fileBuffer.GetChar() != 'P')
...
static bool LoadFaceFromX(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z, int nFrame, int nFace, float flGamma,
						  const VTexConfigInfo_t &info)
{
	switch(g_eMode)
	{
...
static bool LoadFace(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z, int nFrame, int nFace, float flGamma,
					 const VTexConfigInfo_t &info)
{
...
//-----------------------------------------------------------------------------
static bool LoadSourceImages(IVTFTexture *pTexture, const char *pFullNameWithoutExtension, bool *pbGenerateSphereMaps,
	...
	for(int iFrame = 0; iFrame < nFrameCount; ++iFrame)
	{
		for(int iFace = 0; iFace < nFaceCount; ++iFace)
		{
			for(int z = 0; z < info.m_nVolumeTextureDepth; ++z)
			{
				...
				MakeSrcFileName(pSrcName, info.m_nFlags, pFullNameWithoutExtension, iFrame, iFace,
								bIsVolumeTexture ? z : -1, bIsCubeMap, info.m_nStartFrame, info.m_nEndFrame,
				...
				if(!LoadFile(pSrcName, tgaBuffer, bFailOnError, (g_eMode != eModePSD) ? &info.m_uiInputHash : NULL))
				{
...
{
	if((info.m_nMaxDimensionX > 0 && info.m_nMaxDimensionX < pTexture->Width()) ||
...
//-----------------------------------------------------------------------------
bool ProcessFiles(const char *pFullNameWithoutExtension, const char *pOutputDir, const char *pBaseName, bool isCubeMap,
				  VTexConfigInfo_t &info)
{
	...
	bool bGenerateSpheremaps = false;
	bool bLoadedSourceImages =
		LoadSourceImages(pVTFTexture.Get(), pFullNameWithoutExtension, &bGenerateSpheremaps, info);
	...
	bool bAllowFixCubemapOrientation =
		!info.m_bSkyBox; // Don't let it rotate our pseudo-cubemap faces around if it's a skybox.
	...
	if(Q_stristr(pFileBaseName, "_dudv"))
	{
		if(!info.m_bNormalToDuDv && !info.m_bDuDv)
		{
			if(!g_Quiet)
			{
				...
				fprintf(stderr, "If you are trying to convert from a normal map to a dudv map, put \"normaltodudv\" "
								"\"1\" in description.\n");
			}
			info.m_bDuDv = true;
		}
...
void Usage(void)
{
	VTexError("Usage: vtex [-outdir dir] [-quiet] [-nopause] [-mkdir] [-shader ShaderName] [-vmtparam Param Value] "
			  "tex1.txt tex2.txt . . .\n"
...
bool Process_File(char *pInputBaseName, int maxlen)
{
	...
	if(CommandLine()->FindParm("-deducepath"))
	{
		...
		if(pGame && pMaterials && (pGame < pMaterials))
		{
			...
			memmove(outputDir + numConvertBytes + numExtraBytes, outputDir + numConvertBytes,
	...
	if((info.m_nStartFrame == -1 && info.m_nEndFrame != -1) || (info.m_nStartFrame != -1 && info.m_nEndFrame == -1))
	{
...
{
public:
	static bool SuggestFn(CFSSteamSetupInfo const *pFsSteamSetupInfo, char *pchPathBuffer, int nBufferLength,
						  bool *pbBubbleDirectories);
	bool MySuggestFn(CFSSteamSetupInfo const *pFsSteamSetupInfo, char *pchPathBuffer, int nBufferLength,
					 bool *pbBubbleDirectories);
...
bool CSuggestGameDirHelper::SuggestFn(CFSSteamSetupInfo const *pFsSteamSetupInfo, char *pchPathBuffer,
...
bool CSuggestGameDirHelper::MySuggestFn(CFSSteamSetupInfo const *pFsSteamSetupInfo, char *pchPathBuffer,
										int nBufferLength, bool *pbBubbleDirectories)
{
