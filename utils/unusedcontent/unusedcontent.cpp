...
#define MIN_STRING_POOL_SIZE 2048
	...
	{
		char const *str1 =
			(i1 == INVALID_STRING_INDEX) ? g_LessCtx.m_pUserString : g_LessCtx.m_pTable->StringFromIndex(i1);
		char const *str2 =
			(i2 == INVALID_STRING_INDEX) ? g_LessCtx.m_pUserString : g_LessCtx.m_pTable->StringFromIndex(i2);
...
char *directories_to_check[] = {
	"", "bin", "maps", "materials", "models", "scenes", "scripts", "sound", "hl2",
};

char *directories_to_ignore[] = // don't include these dirs in the others list
	{"reslists", "reslists_temp", "logs", "media", "downloads", "save", "screenshots", "testscripts", "logos"};
...
void BuildFileList_R(int depth, CUtlVector<FileEntry> &files, CUtlVector<FileEntry> *otherfiles, char const *dir,
					 char const *wild, int skipchars)
{
	WIN32_FIND_DATA wfd;
...
void BuildFileList(int depth, CUtlVector<FileEntry> &files, CUtlVector<FileEntry> *otherfiles, char const *rootdir,
				   int skipchars)
{
	files.RemoveAll();
	Assert(otherfiles);
...
void BuildFileListWildcard(int depth, CUtlVector<FileEntry> &files, char const *rootdir, char const *wildcard,
						   int skipchars)
{
	files.RemoveAll();
...
void BuildWhiteList()
{
	...
	if(verbose || printwhitelist)
	{
		...
		for(int i = g_WhiteList.FirstInorder(); i != g_WhiteList.InvalidIndex(); i = g_WhiteList.NextInorder(i))
		{
...
void ParseFilesFromResList(UnusedContent::CUtlSymbol &resfilesymbol, CUtlRBTree<ReferencedFile, int> &files,
						   char const *resfile)
{
	int addedStrings = 0;
...
bool BuildReferencedFileList(CUtlVector<UnusedContent::CUtlSymbol> &resfiles, CUtlRBTree<ReferencedFile, int> &files,
							 const char *resfile)
{
	...
	if(resfiles.Count() <= 0)
	{
		vprint(0, "%s didn't have any actual .lst files in the reslists folder, have you run the engine with %s\n",
			   resfile, "-makereslists -usereslistfile maplist.txt");
		return false;
	}
...
void Correlate(CUtlRBTree<ReferencedFile, int> &referencedfiles, CUtlVector<FileEntry> &contentfiles,
			   const char *modname)
{
	int i;
	...
	vprint(0, "Found %i referenced (%i whitelist) files in tree, %s\n", refcounted, whitelisted,
		   Q_pretifymem(totalReferencedDiskSize, 2));
	...
	if(showmapfileusage)
	{
		...
		while(i != invalidindex)
		{
			...
			logprint("referenced.csv", "\"%s\",\"%s\",%d", g_Analysis.symbols.String(entry.sym), ext,
					 entry.maplist.Count());
	...
	vprint(0, "% 15s               % 15s               % 15s               % 15s %12s\n", "Referenced", "WhiteListed",
		   "Unreferenced", "Total", "Directory");
	...
	while(i != invalidindex)
	{
		...
		vprint(0, "%15s (%8.3f%%)   %15s (%8.3f%%)   %15s (%8.3f%%)   %15s => dir: %s\n", refstring,
			   100.0f - percent_unref, whiteliststring, percent_white, unrefstring, percent_unref, totalstring,
			   directories.GetElementName(i));
	...
	vprint(0, "-----------------------------------------------------------------------------------------\n");
	vprint(0, "%15s (%8.3f%%)   %15s (%8.3f%%)   %15s (%8.3f%%)   %15s\n", refstring, 100.0f - percent_unref,
		   whiteliststring, percent_white, unrefstring, percent_unref, totalstring);
}
