		...
		case VMPI_SUBPACKETID_CRASH:
		{
			...
			switch(chCrashInfoType)
			{
				...
				case 'f':
				{
					...
					sprintf(chSaveFileName, "%s\\vmpi_%s_on_%s_%d%.2d%2d%.2d%.2d%.2d_%d.mdmp", szFolder, pModuleName,
							VMPI_GetMachineName(iSource), pTime->tm_year + 1900, /* Year less 2000 */
...
int VMPI_SendFileChunk(const void *pvChunkPrefix, int lenPrefix, tchar const *ptchFileName)
{
	...
	if(VMPI_Send3Chunks(pvChunkPrefix, lenPrefix, &iMappedFileSize, sizeof(iMappedFileSize), pvMappedData,
						iMappedFileSize, VMPI_MASTER_ID))
...
void VMPI_HandleCrash(const char *pMessage, void *pvExceptionInfo, bool bAssert)
{
	...
	if(InterlockedIncrement(&crashHandlerCount) == 1)
	{
		...
		VMPI_Send2Chunks(crashMsg, sizeof(crashMsg), pMessage, strlen(pMessage) + 1, VMPI_MASTER_ID);
		...
		if(pvExceptionInfo)
		{
			...
			bool bSucceededWritingMinidump = WriteMiniDumpUsingExceptionInfo(
				...
				// | MiniDumpWithPrivateReadWriteMemory  ), ( MINIDUMP_TYPE )( MiniDumpNormal ),
				tchMinidumpFileName);
