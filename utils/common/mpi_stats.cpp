...
public:
	virtual int RunCommand()
	{
		...
		char query[2048];
		Q_snprintf(query, sizeof(query), "update job_master_start set NumWorkers=%d where JobID=%lu", nCurConnections,
				   g_JobPrimaryID);
		...
		for(int i = 1; i < nCurConnections; i++)
		{
			...
			if(jobWorkerID != 0xFFFFFFFF)
			{
				Q_snprintf(query, sizeof(query),
						   ...
						   VMPI_IsProcConnected(i), (int)VMPI_GetNumWorkUnitsCompleted(i), VMPI_GetJobWorkerID(i));
				g_pSQL->Execute(query);
			}
		}
		return 1;
...
void UpdateJobWorkerRunningTime()
{
	...
	query.Format("update job_worker_start set RunningTimeMS=%lu, CurrentStage=\"%s\", "
				 ...
				 runningTimeMS, curStage, (int)g_ThreadWUs[0], (int)g_ThreadWUs[1], (int)g_ThreadWUs[2],
				 (int)g_ThreadWUs[3], g_JobWorkerID);
	query.Execute(g_pSQL);
}
...
void PerfThread_AddGraphEntry(DWORD startTicks, DWORD &lastSent, DWORD &lastReceived)
{
	...
	g_pDB->AddCommandToQueue(
		new CSQLDBCommand_GraphEntry(GetTickCount() - startTicks, curSent - lastSent, curReceived - lastReceived),
		NULL);
...
bool LoadMySQLWrapper(const char *pHostName, const char *pDBName, const char *pUserName)
{
	UnloadMySQLWrapper();
							...
							unsigned long *pDBJobID)
{
	...
	g_JobPrimaryID = 0;
	CMySQLQuery query;
	query.Format("insert into job_master_start ( BSPFilename, StartTime, MachineName, RunningTimeMS ) values ( \"%s\", "
				 "null, \"%s\", %lu )",
...
bool VMPI_Stats_Init_Worker(const char *pHostName, const char *pDBName, const char *pUserName, unsigned long DBJobID)
{
	...
	CMySQLQuery query;
	query.Format("insert into job_worker_start ( JobID, CurrentStage, IsMaster, MachineName ) values ( %lu, \"none\", "
				 "%d, \"%s\" )",
...
void StatsDB_InitStatsDatabase(int argc, char **argv, const char *pDBInfoFilename)
{
