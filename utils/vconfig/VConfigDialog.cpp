...
CVConfigDialog::CVConfigDialog(Panel *parent, const char *name) : BaseClass(parent, name), m_bChanged(false)
{
	...
	if(g_ConfigManager.WasConvertedOnLoad())
	{
		...
		// GameCfg.INI.OLD has been created.\n\nPlease visit http://www.valve-erc.com/srcsdk/faq.html#GameConfigUpdate
...
void CVConfigDialog::PopulateConfigList(bool bSelectActiveConfig /*=true*/)
{
	...
	if(activeItem < 0)
	{
		...
		VGUIMessageBox(this, "Invalid Game Directory",
					   "The currently selected game directory: %s is invalid.\nChoose a new directory, or select "
					   "'Cancel' to exit.\n",
					   szKeyValue);
