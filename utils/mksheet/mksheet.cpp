...
static void ApplyMacros(char *in_buf)
{
	...
	if((Words.Count() == 4) && (!stricmp(Words[0], "ga_frame")))
	{
		...
		sprintf(in_buf, "frame %s{r=0},%s{g=a},%s{b=0},%s{a=a} %s", Words[1], Words[1], Words[1], Words[2], Words[3]);
	}
		...
		if(in_str[0])
		{
			...
			else if((Words.Count() == 2) && StringHasPrefix(Words[0], "sequence"))
			{
				...
				if(!stricmp(szSeqType, "") || !stricmp(szSeqType, "-rgba"))
					...
					printf("*** line %d: invalid sequence type '%s', allowed 'sequence-rgba' or 'sequence-rgb' or "
						   "'sequence-a'!\n",
					...
					case PCKM_FLAT:
						switch(pCurSequence->m_eMode)
						{
							case Sequence::SQM_RGBA:
								break;
							default:
								printf("*** line %d: invalid sequence type '%s', packing 'flat' allows only "
									   "'sequence-rgba'!\n",
									   numActualLinesRead, Words[0]),
									exit(-1);
						}
					...
					case PCKM_RGB_A:
						switch(pCurSequence->m_eMode)
						{
							...
							case Sequence::SQM_ALPHA:
								break;
							default:
								printf("*** line %d: invalid sequence type '%s', packing 'rgb+a' allows only "
									   "'sequence-rgb' or 'sequence-a'!\n",
									   numActualLinesRead, Words[0]),
									exit(-1);
						}
						break;
					...
					for(int i = 0; i < Words.Count() - 2; i++)
					{
						...
						if(s_ePackingMode == PCKM_RGB_A)
						{
							for(uint16 idx = 0; idx < pBM->m_mapSequences.Count(); ++idx)
							{
								...
								if(pSeq->m_eMode != Sequence::SQM_RGBA && pSeq->m_eMode != pCurSequence->m_eMode)
								{
									printf("*** line %d: 'rgb+a' packing cannot pack frame '%s' belonging to sequences "
										   "%d and %d!\n",
			...
			{
				printf("*** line %d: Bad command \"%s\"!\n", numActualLinesRead, in_str), exit(-1);
			}
			Words.PurgeAndDeleteElements();
		}
	}
...
bool PackImages_Flat(char const *pFname, int nWidth)
{
	...
	for(int i = 0; i < ImageList.GetNumStrings(); i++)
	{
		...
		// now, pack
		if((cur_column + frm.m_pImage->Width > output.Width) || (cur_line + frm.m_pImage->Height > output.Height))
		{
...
bool PackImages_Rgb_A(char const *pFname, int nWidth)
{
	...
	for(int i = 0; i < ImageList.GetNumStrings(); i++)
	{
		...
		switch(eMode)
		{
			...
			case Sequence::SQM_RGBA:
				if(!bPackingRGBA)
					printf("*** error when packing 'rgb+a', bad sequence %d encountered for frame '%s' after all rgba "
						   "frames packed!\n",
