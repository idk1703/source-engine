...
void CreateTemplateVMT(const char *pcDirectory, const char *pcBaseName, const char *pcTGAColorFile,
					   const char *pcTGANormalFile, const char *pcSteamName)
{
	char sz_Buffer[MAX_PATH];
	...
	/*	fputs( "\"UnlitGeneric\"\n",  fp );
	   ...
	   szTGAColorFileBase ); fputs( sz_Buffer,  fp ); fputs( "\t\$translucent\t1\n",  fp ); fputs(
	   "\t\"$vertexcolor\"\t1\n}\n",  fp );  */
...
bool ParseMTL(const char *pcDirectory, const char *pcOBJFile, const char *pcMTLFile, char *pcTGAColorFile,
			  size_t nTGAColorBufSize, char *pcTGASpecularFile, size_t nTGASpecularBufSize)
{
...
bool ParseArguments(int argc, char *argv[], char *pcDirectory, size_t nDirectoryBufSize, char *pcBaseName,
					size_t nBaseNameBufSize, char *pcOBJFile, size_t nOBJFileBufSize, char *pcMTLFile,
					size_t nMTLFileBufSize)
{
	if(argc < 2)
	{
		printusage();
...
void RunCommandLine(const char *pCmdLine, const char *pWorkingDir)
{
	...
	CreateProcess(NULL, (char *)pCmdLine, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, pWorkingDir, &startupInfo, &pi);
...
void CompileVTEX(const char *pcDirectory, const char *pcTGAColorFile, const char *pcSteamName, const char *pcBaseName)
{
	...
	V_snprintf(szCmdLine, sizeof(szCmdLine),
			   "\"%s\\vtex.exe\" -nopause \"%s\\materialsrc\\models\\contrib\\%s\\%s\\%s\"", szSDKBinDir,
			   szSDKSourcesDir, pcSteamName, pcBaseName, pcTGAColorFile);
...
void CompileMDL(const char *pcObjFile, const char *pcSteamName, const char *pcBaseName)
{
	...
	V_snprintf(szCmdLine, sizeof(szCmdLine), "\"%s\\studiomdl.exe\" -nop4 \"%s\\modelsrc\\contrib\\%s\\%s\\%s.qc\"",
			   szSDKBinDir, szSDKSourcesDir, pcSteamName, pcBaseName, pcBaseName);
	RunCommandLine(szCmdLine, szSDKBinDir);
}
...
void LaunchHLMV(const char *pcSteamName, const char *pcBaseName)
{
	...
	V_snprintf(szCmdLine, sizeof(szCmdLine), "\"%s\\hlmv.exe\" -game \"%s\" \"%s\\models\\contrib\\%s\\%s\\%s.mdl\"",
			   szSDKBinDir, szVProjectPath, szVProjectPath, pcSteamName, pcBaseName, pcBaseName);
	RunCommandLine(szCmdLine, szSDKBinDir);
}
...
void CopyVMT(const char *pcDirectory, const char *pcBaseName, const char *pcSteamName)
{
	...
	V_snprintf(szDestVMT, sizeof(szDestVMT), "%s\\materials\\models\\contrib\\%s\\%s\\%s.vmt", szVProjectPath,
...
bool CopyMaterialSourcesToSrcTree(const char *pcDirectory, const char *pcSteamName, const char *pcBaseName)
{
	...
	V_snprintf(szBuffer, sizeof(szBuffer), "%s%s%s\\%s", szSDKSourcesDir, "\\materialsrc\\models\\contrib\\",
			   pcSteamName, pcBaseName);
	CreateDirectory(szBuffer, NULL);
	bAllSucceeded &= DirectoryExists(szBuffer);
...
bool CopyModelSourcesToSrcTree(const char *pcDirectory, const char *pcSteamName, const char *pcBaseName)
{
	...
	V_snprintf(szBuffer, sizeof(szBuffer), "%s%s%s\\%s", szSDKSourcesDir, "\\modelsrc\\contrib\\", pcSteamName,
			   pcBaseName);
	CreateDirectory(szBuffer, NULL);
	bAllSucceeded &= DirectoryExists(szBuffer);
...
int main(int argc, char *argv[])
{
	...
	szDirectory[0] = szMTLFile[0] = szOBJFile[0] = szSteamName[0] = szTGAColorFile[0] = szTGANormalFile[0] =
		szTGATransparencyFile[0] = szTGASpecularFile[0] = NULL;

	if(ParseArguments(argc, argv, szDirectory, sizeof(szDirectory), szBaseName, sizeof(szBaseName), szOBJFile,
					  sizeof(szOBJFile), szMTLFile, sizeof(szMTLFile)))
	{
		if(CheckFilesExist(szDirectory, szOBJFile, szMTLFile))
		{
			...
			ParseMTL(szDirectory, szOBJFile, szMTLFile, szTGAColorFile, sizeof(szTGAColorFile), szTGASpecularFile,
					 sizeof(szTGASpecularFile));
