...
public:
	...
	bool SwapBSPFile(IFileSystem *pFileSystem, const char *filename, const char *swapFilename, bool bSwapOnLoad,
					 VTFConvertFunc_t pVTFConvertFunc, VHVFixupFunc_t pVHVFixupFunc, CompressFunc_t pCompressFunc);
	...
	bool SetPakFileLump(IFileSystem *pFileSystem, const char *pBSPFilename, const char *pNewFilename, void *pPakData,
						int pakSize);
...
bool CBSPPack::SwapBSPFile(IFileSystem *pFileSystem, const char *filename, const char *swapFilename, bool bSwapOnLoad,
						   VTFConvertFunc_t pVTFConvertFunc, VHVFixupFunc_t pVHVFixupFunc, CompressFunc_t pCompressFunc)
{
	// This is shady, but the engine is the only client here and we want the same search paths it has.
	g_pFileSystem = g_pFullFileSystem = pFileSystem;
...
{
	return ::RepackBSP(
		inputBuffer, outputBuffer, (repackFlags & eRepackBSP_CompressLumps) ? RepackBSPCallback_LZMA : NULL,
...
bool CBSPPack::SetPakFileLump(IFileSystem *pFileSystem, const char *pBSPFilename, const char *pNewFilename,
