...
CItemTestManifest::CItemTestManifest(const char *pszManifestFile, CItemLog *pItemLog)
	: m_pItemLog(pItemLog), m_vecVMTTextureRemaps(UtlStringLessThan)
{
	m_pManifestKV = NULL;
	m_pItemDirectory = "";
	...
	{
		m_pItemLog->Warning(
			"ERROR: Default material type '%s' wasn't found in the material type list in manifest file: %s\n",
	...
	if(pKVMaterialSkins)
	{
		...
		{
			m_pItemLog->Warning(
				"ERROR: Default material skin '%s' wasn't found in the material skin list in manifest file: %s\n",
	...
	if(pKVClassTemplates)
	{
		...
		FOR_EACH_SUBKEY(pKVClassTemplates, pKVClassTemplate)
		{
			...
			{
				m_pItemLog->Warning(
					"ERROR: Found an invalid class '%s' in the vmt_templates entries in manifest file: %s\n", pszHero,
					pszManifestFile);
				return;
			}
...
//
//-----------------------------------------------------------------------------
bool CItemTestManifest::ParseStringsFromManifest(KeyValues *pKV, const char *pszKeyName,
												 CUtlVector<CUtlString> &vecList)
{
...
bool CItemUpload::GetSourceSDKFromExe(CUtlString &sSourceSDK, CUtlString &sSourceSDKBin)
{
	...
	{
		sSourceSDK.SetDirect(sCurrentExecutableFileName.String(),
							 pszBinOrangeBoxBin - sCurrentExecutableFileName.String());
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetOutputPath(CUtlString &sOutputPath, int nIndex /* = 0 */,
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetOutputPaths(CUtlVector<CUtlString> &sOutputPaths, uint nPathFlags /* = PATH_FLAG_ALL */,
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetOutputPaths(CUtlVector<CUtlString> &sOutputPaths, bool bRelative /* = true */,
								 bool bRecurse /* = true */, bool bExtension /* = true */,
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetInputPaths(CUtlVector<CUtlString> &sInputPaths, bool bRelative /* = true */,
								bool bRecurse /* = true */, bool bExtension /* = true */)
{
	CUtlString sTmp;
	if(!IsOk(sTmp))
...
//
//-----------------------------------------------------------------------------
bool CTargetBase::GetDirName(CUtlString &sDirName, uint nPathFlags /* = PATH_FLAG_ALL */) const
{
	sDirName.Clear();
...
//
//-----------------------------------------------------------------------------
bool CTargetTGA::GetInputPaths(CUtlVector<CUtlString> &sInputPaths, bool bRelative /* = true */,
							   bool bRecurse /* = true */, bool bExtension /* = true */)
{
...
{
	// 'reset' the input file from the current value to force a reload on compile, make a temp copy as it's going to be
	// overwritten
	...
	if(bitmap.Format() != IMAGE_FORMAT_RGBA8888 || bitmap.Width() != m_nWidth || bitmap.Height() != m_nHeight)
	{
		Warning("CTarget%s::Compile( %s ) - Invalid Bitmap Size, Expected %d x %d x %d (%d)\n", GetTypeString(),
				sName.String(), m_nWidth, m_nHeight, 4, m_nWidth * m_nHeight * 4);
		return false;
	}
	...
	if(nTargetWidth && nTargetHeight)
	{
		...
		// Determine the right amount of reduction based on the texture's width & height (and fail if it's the wrong
		// aspect ratio)
		int nFactor = (m_nWidth / nTargetWidth);
		if(nFactor <= 0)
		{
			Warning(
				"CTarget%s::Compile( %s ) - Failed to determine reduction factor (target width is %d, texture is %d)\n",
		...
		{
			Warning("CTarget%s::Compile( %s ) - Failed to determine reduction factor (target size aspect ratio (%dx%d) "
					"doesn't match texture's aspect ratio (%dx%d))\n",
	...
	CUtlString sTGAOutput =
		Asset()->CheckRedundantOutputFilePath(GetInputFile().String(), bufVTEXConfig.String(), sAbsPath.String());
	...
	CUtlBuffer outBuf;
	const bool bWriteTGA =
		TGAWriter::WriteToBuffer(bitmap.GetBits(), outBuf, bitmap.Width(), bitmap.Height(), bitmap.Format(),
								 m_bAlpha ? IMAGE_FORMAT_BGRA8888 : IMAGE_FORMAT_BGR888);
...
bool CTargetTGA::SetInputFile(const char *pszFilename)
{
	...
	static const char *szSuffixes[] = {//"_color",
									   "_normal",
									   "_height",
									   "_specmask"
									   "_specexp",
	...
	else
	{
		Warning("ERROR: Specified texture file (%s) Size %dx%d dimensions not powers of two, perhaps resize to %dx%d\n",
				m_sInputFile.String(), m_nWidth, m_nHeight, nWidthPow, nHeightPow);
		return false;
	}
...
CTargetVTF::CTargetVTF(CAsset *pAsset, const CTargetVMT *pTargetVMT)
	: CTargetBase(pAsset, pTargetVMT), m_pTargetVMT(pTargetVMT)
{
...
bool CTargetVTF::Compile()
{
	...
	{
		// We can't rely on environment variables in VTEX. So tell it to just built it on the spot, and we'll move it
		// afterwards.
		sCmd.sprintf("\"%s\\vtex.exe\" -nop4 -nopause -dontusegamedir \"%s\"", sBinDir.String(),
	...
	else
	{
		...
		sCmd.sprintf("\"%s\\vtex.exe\" -nop4 -nopause -vproject \"%s\" \"%s\"", sBinDir.String(), sVProject.String(),
					 sAbsInputPaths.Element(0).String());
	}
...
bool CTargetVMT::Compile()
{
	...
	if(sAbsOutputPaths.Count() != nVmtCount || sAbsOutputPaths.Count() != sRelOutputPaths.Count())
	{
		Warning("CTarget%s::Compile( %s ) - GetOutputPaths returned %d paths, expected %d\n", GetTypeString(),
				sName.String(), sAbsOutputPaths.Count(), 1);
		return false;
	}
	...
	for(int i = 0; i < nVmtCount; ++i)
	{
		...
		FOR_EACH_VEC(m_vecTargetVTFs[i], iVTF)
		{
			...
			{
				const char *pszVMTVar =
					CItemUpload::Manifest()->GetVMTVarForTextureType(CItemUpload::Manifest()->GetTextureType(iVTF));
...
//
//-----------------------------------------------------------------------------
bool CTargetVMT::SetTargetVTF(CSmartPtr<CTargetVTF> &pTargetVTF, const char *pszFilename, const char *pszSuffix) const
{
	if(!pTargetVTF)
	{
...
void CTargetVMT::CreateLegacyTemplate(KeyValues *pVMTKV)
{
	...
	switch(m_nColorAlphaType)
	{
		...
		case kPaintable:
			...
			pVMTKV->SetString("$blendtintcoloroverbase",
							  "0"); // between 0-1 determines how much blended by tinting vs. replacing the color

			pVMTKV->SetString("$colortint_base",
							  "{ 255 255 255 }"); // put the RGB value of whats being colored when no paint is present,
												  // if $blendtintcoloroverbase is 0 then put [255 255 255] here.
...
//
//=============================================================================
CTargetIcon::CTargetIcon(CAsset *pAsset, int nIconType) : CTargetVMT(pAsset, pAsset), m_nIconType(nIconType)
{
	int nWidth, nHeight;
...
CTargetDMX::CTargetDMX(CAsset *pAsset, const CTargetQC *pTargetQC)
	: CTargetBase(pAsset, pTargetQC), m_pDmRoot(NULL), m_nLod(-1), m_flAnimationLoopStartTime(-1.f)
{
}
...
//
//-----------------------------------------------------------------------------
bool CTargetDMX::GetInputPaths(CUtlVector<CUtlString> &sInputPaths, bool bRelative /* = true */,
							   bool bRecurse /* = true */, bool bExtension /* = true */)
{
	sInputPaths.AddToTail(m_sInputFile);
...
void CTargetDMX::ReplaceMaterials() const
{
	...
	for(int i = 0; i < pAsset->GetTargetVMTCount(); ++i)
	{
		...
		pSrcTargetVMT->GetOutputPath(sMaterialPath, 0,
									 PATH_FLAG_FILE | PATH_FLAG_EXTENSION | PATH_FLAG_PATH | PATH_FLAG_MODELS);
...
bool CTargetDMX::OutputQCIFile()
{
	...
	if(!m_strQCITemplate.IsEmpty())
	{
		...
		strBuf = strBuf.Replace("<BLEND_LAYER_RANGE>", CFmtStr("0 %d %d <LAST_FRAME_INDEX>", nBlendFrameOffset,
															   nLastFrameIndex - nBlendFrameOffset));
...
void CTargetDMX::OutputSounds(CUtlBuffer &buf, int nIndentLevel, CDmElement *pExportedSounds)
{
	...
	V_MakeAbsolutePath(szSoundScriptFullPath, sizeof(szSoundScriptFullPath), m_strSoundScriptFile.String(),
					   sWorkingDir.String());
	...
	if(GetCustomModPath())
	{
		...
		V_MakeAbsolutePath(szCustomPath, sizeof(szCustomPath),
						   CFmtStr("%s/%s", GetCustomModPath(), m_strSoundScriptFile.String()), sWorkingDir.String());
		...
		BufBeginBlock(vcdBuf, nLevel);
		{
			...
			for(int ci = 0; ci < nChannelCount; ++ci)
			{
				...
				if(bIsStereo)
				{
					...
					DmeTime_t tEndOnFrame = MAX(pLeftLogLayer->GetKeyTime(pLeftLogLayer->GetKeyCount() - 1),
												pRightLogLayer->GetKeyTime(pRightLogLayer->GetKeyCount() - 1));
#if 1 // resample time
					...
					{
						DmeTime_t tLeft = iLeft < nLeft
											  ? pDmeChannelsClip->FromChildMediaTime(pLeftLogLayer->GetKeyTime(iLeft))
											  : DMETIME_MAXTIME;
						DmeTime_t tRight =
							iRight < nRight ? pDmeChannelsClip->FromChildMediaTime(pRightLogLayer->GetKeyTime(iRight))
											: DMETIME_MAXTIME;
...
bool CTargetVCD::Compile()
{
	...
	if(GetCustomModPath())
	{
		...
		char szCustomPath[MAX_PATH];
		V_MakeAbsolutePath(szCustomPath, sizeof(szCustomPath), CFmtStr("%s/%s", GetCustomModPath(), sRelPath.String()),
						   sWorkingDir.String());
		V_FixSlashes(szCustomPath);
...
CTargetQC::CTargetQC(CAsset *pAsset, const CTargetMDL *pTargetMDL)
	: CTargetBase(pAsset, pTargetMDL), m_QCTemplate(0, 0, CUtlBuffer::TEXT_BUFFER), m_TargetVCD(NULL)
{
}
...
bool CTargetQC::Compile()
{
	...
	{
		strBuf = strBuf.Replace(CFmtStr("<ITEMTEST_REPLACE_LOD%d_HEADER>", nLOD),
								CFmtStr("$lod %d", CItemUpload::Manifest()->GetQCLODDistance(nLOD)));
	}
...
CTargetMDL::CTargetMDL(CAsset *pAsset, const CTargetBase *pTargetParent)
	: CTargetBase(pAsset, pTargetParent), m_pTargetQC(NULL)
{
...
bool CTargetMDL::Compile()
{
	...
	if(CItemUpload::IgnoreEnvironmentVariables())
	{
		...
		sCmd.sprintf("\"%s\\studiomdl.exe\" -nop4 -game \"%s\" \"%s\"", sBinDir.String(), sWorkingDir.String(),
					 sAbsInputPaths.Element(0).String());
	}
	else
	{
		CItemUpload::GetVProjectDir(sWorkingDir);
		sCmd.sprintf("\"%s\\studiomdl.exe\" -nop4 -vproject \"%s\" \"%s\"", sBinDir.String(), sWorkingDir.String(),
					 sAbsInputPaths.Element(0).String());
	}
...
CAsset::CAsset(const char *pszName, bool *pbOk /* = NULL */)
	: CTargetBase(NULL, NULL), m_nCurrentModel(-1), m_bShouldBuildScenesImage(false)
{
	m_pAsset = this;
...
bool CAsset::BuildScenesImage()
{
	...
	if(GetCustomModPath())
	{
		char szCustomPath[MAX_PATH];
		V_MakeAbsolutePath(szCustomPath, sizeof(szCustomPath),
						   CFmtStr("%s/%s", GetCustomModPath(), "scenes/scenes.image"), sWorkingDir.String());
		V_FixSlashes(szCustomPath);
...
bool CAsset::Compile()
{
	...
	HANDLE m_hOutputZipFile =
		CreateFile(m_sArchivePath.String(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
//-----------------------------------------------------------------------------
bool CAsset::GetAbsoluteDir(CUtlString &sAbsoluteDir, const char *pszPrefix /* = NULL */,
							const CTargetBase *pTarget) const
{
	CUtlString sDirA;
...
const char *CAsset::CheckRedundantOutputFilePath(const char *pszInputFilePath, const char *pszVTEXConfig,
