...
void WriteSubRect(int nSrcWidth, int nSrcHeight, const float *pSrcImage, int nSrcOffsetX, int nSrcOffsetY,
				  int nDstWidth, int nDstHeight, const char *pSrcFileName, const char *pDstFileNameExtension)
{
	...
	for(y = 0; y < nDstHeight; y++)
	{
		...
		{
			pDstImage[(x + (nDstWidth * y)) * 3 + 0] =
				pSrcImage[((nSrcOffsetX + x) + (nSrcOffsetY + y) * nSrcWidth) * 3 + 0];
			pDstImage[(x + (nDstWidth * y)) * 3 + 1] =
				pSrcImage[((nSrcOffsetX + x) + (nSrcOffsetY + y) * nSrcWidth) * 3 + 1];
			pDstImage[(x + (nDstWidth * y)) * 3 + 2] =
				pSrcImage[((nSrcOffsetX + x) + (nSrcOffsetY + y) * nSrcWidth) * 3 + 2];
		}
	}
