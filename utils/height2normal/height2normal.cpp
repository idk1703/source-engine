...
void ProcessFiles(const char *pNormalFileNameWithoutExtension, int startFrame, int endFrame, float bumpScale)
{
	...
	for(frameID = 0; frameID < numFrames; frameID++)
	{
		...
		*pcUnderscore = NULL;
		...
		{
			TGAWriter::WriteToBuffer(pImageRGBA8888, normalBuf, width, height, IMAGE_FORMAT_RGBA8888,
		...
		else
		{
			...
			ImageLoader::ConvertImageFormat(pImageRGBA8888, IMAGE_FORMAT_RGBA8888, pImageRGB888, IMAGE_FORMAT_RGB888,
											width, height, 0, 0);
...
int main(int argc, char **argv)
{
	...
	for(; i < argc; i++)
	{
		...
		{
			Q_snprintf(normalFileNameWithoutExtension, sizeof(normalFileNameWithoutExtension), "%s\\%s",
		...
		if((startFrame == -1 && endFrame != -1) || (startFrame != -1 && endFrame == -1))
		{
