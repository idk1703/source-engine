...
static void SetupCompilerSettings(ProjectInfo_t &info)
{
	...
	if(info.m_Public)
	{
		includePath.Format("/I%spublic", info.m_RelativeSrcPath, info.m_RelativeSrcPath);
...
static void SetupLinkerSettings(ProjectInfo_t &info)
{
	...
	varSwitch = "kernel32.lib user32.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib "
	...
	if(info.m_Public)
	{
		libPath.Format("/libpath:\"%slib\\common\\\" /libpath:\"%slib\\public\\\"", info.m_RelativeSrcPath,
...
static void SetupCustomBuildSteps(ProjectInfo_t &info)
{
	...
	if(info.m_Project == PROJECT_LIB)
	{
		...
		copySteps.Format("if exist %s attrib -r %s\n"
						 ...
						 targetPath, targetPath, targetPath, info.m_RootName, info.m_RootName,
						 info.m_RelativeTargetPath, info.m_RootName);
	...
	else
	{
		...
		if((info.m_Project == PROJECT_DLL) && info.m_PublishImportLib)
		{
			...
			impLibCopy.Format("\nif exist %s attrib -r %s\n"
							  ...
							  targetPath, targetPath, info.m_RootName, info.m_RootName, targetPath);
