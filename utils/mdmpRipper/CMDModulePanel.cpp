...
CMDModulePanel::CMDModulePanel(vgui::Panel *pParent, const char *pName) : BaseClass(pParent, pName, true)
{
	...
	if(kv)
	{
		...
		if(key == 0)
		{
			...
			else
			{
				char query[1024];
				sprintf(query,
						"select * from knownmodules where name = \"%s\" and version1 = %s and version2 = %s and "
						"version3 = %s and version4 = %s;",
				...
				else
				{
					...
					char update[1024];
					sprintf(update,
							"insert into knownmodules set name = \"%s\", version1 = %s, version2 = %s, version3 = %s, "
							"version4 = %s, type = \"%s\";",
		...
		{
			char query[1024];
			sprintf(query, "select * from knownmodules where id = %i;", key);
			IResultSet *results = g_pSqlWrapper->PResultSetQuery(query); // do the query
			...
			if(numResults > 0)
			{
				...
				Assert(row != NULL);
				Assert(numResults == 1);
				Assert(!Q_stricmp(name, row->PchData(1)) && atoi(v1buf) == row->NData(2) &&
					   atoi(v2buf) == row->NData(3) && atoi(v3buf) == row->NData(4) && atoi(v4buf) == row->NData(5));
				int realKey = row->NData(0);
				const char *realType = row->PchData(6);
				g_pSqlWrapper->FreeResult();
...
void CMDModulePanel::InitializeDebugEngine(void)
{
	...
	if(S_OK == DebugCreate(__uuidof(IDebugClient), (void **)&m_pDbgClient))
	{
		m_pDbgClient->QueryInterface(__uuidof(IDebugControl), (void **)&m_pDbgControl);
		m_pDbgClient->QueryInterface(__uuidof(IDebugSymbols2), (void **)&m_pDbgSymbols);
		...
		if(m_pDbgSymbols)
		{
			...
			if(!Q_stricmp("%_NT_SYMBOL_PATH%", szSymbolSrv))
			{
				...
				Q_strcpy(szSymbolSrv,
						 "SRV*c:\\localsymbols*\\\\perforce\\symbols*http://msdl.microsoft.com/download/symbols");
...
DWORD CMDModulePanel::AnalyzeThread(void)
{
	if(m_pDbgControl)
	{
		...
		m_pDbgControl->Execute(DEBUG_OUTCTL_THIS_CLIENT, "!analyze -v", DEBUG_EXECUTE_DEFAULT);
	}
