...
CMDErrorPanel::CMDErrorPanel(vgui::Panel *pParent, const char *pName) : BaseClass(pParent, pName)
{
	SetParent(pParent);
...
void CMDErrorPanel::NewQuery()
{
	...
	char sql[255] = "";
	char temp[10];
	strcat(sql, "select errorid, module, count, minidumpsonhand from error_types where processed=0 and minidumpsonhand "
				"> 0 and count > ");
