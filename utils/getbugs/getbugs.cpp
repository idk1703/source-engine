...
#define SQL_SETVALUE(fieldname, sqlcolumn) \
	Q_strncpy(bug.m_pBug->fieldname, sqlcolumn.String(), sizeof(bug.m_pBug->fieldname));
...
TRK_UINT CBugReporter::Login(TRK_HANDLE *pTrkHandle, char const *projectname)
{
	...
	TRK_UINT rc = TrkProjectLogin(*pTrkHandle, username, password, projectname, NULL, NULL, NULL, NULL,
								  TRK_USE_INI_FILE_DBMS_LOGIN);

	if(rc != TRK_SUCCESS)
	{
		rc = TrkProjectLogin(*pTrkHandle, username, "", projectname, NULL, NULL, NULL, NULL,
							 TRK_USE_INI_FILE_DBMS_LOGIN);
		if(rc != TRK_SUCCESS)
		{
...
bool CBugReporter::Init(char const *projectname)
{
	...
	if(rc != TRK_SUCCESS)
	{
		ReportError(rc, "TrkRecordHandleAlloc", "Failed to Allocate Tracker Record Handle!");
		return false;
	}
...
bool CBugReporter::CommitBugReport(int &bugSubmissionId)
{
	...
	for(int i = 0; i < fieldCount; ++i)
	{
		...
		else
		{
			...
			rc = TrkSetDescriptionData(trkRecHandle, buf.TellPut(), (const char *)buf.Base(), 0);
			if(rc != TRK_SUCCESS)
			{
	...
	if(rc != TRK_SUCCESS)
	{
		ReportError(rc, "TrkGetNumericFieldValue", "Failed to TrkGetNumericFieldValue for bug Id #!");
	}
	else
	{
	...
	else
	{
		...
		if(rc != TRK_SUCCESS)
		{
			ReportError(rc, "TrkSetDescriptionData(update)", "Failed to set description data!");
			return false;
		}
		...
		if(rc != TRK_SUCCESS)
		{
			ReportError(rc, "TrkUpdateRecordCommit", "Failed to TrkUpdateRecordCommit for bug Id #!");
			return false;
		}
	}
...
void GetBugInfo(CBugReporter &bug, char const *host, char const *database, char const *username, char const *password,
				char const *admin, int startbug, int endbug)
{
		...
		if(factory)
		{
			...
			if(mysql)
			{
				if(mysql->InitMySQL(database, host, username, password))
				{
					...
					/*
					Q_snprintf( q, sizeof( q ), "insert into engineaccess (BuildIdentifier,AllowAccess) values
					(\"%s\",1);", argv[2] );
					...
					*/
					...
					Q_snprintf(
						q, sizeof(q),
						"select BugId, Time, BuildNumber, ExeName, GameDirectory, MapName, Title, Description, IP,"
						"BaseIP, RAM, CPU, ProcessorVendor, DXVersionHighPart, DXVersionLowPart, DXVendorID, "
						"DXDeviceID, OSVersion, "
						"BugReportFilePath, ReportType, EMail, AccountName, SteamID, Processed, Important from "
						"bugreports where %s ( Processed is NULL or Processed = 0 );",
						where);
					...
					else
					{
						...
						if(rows && rows->NumFields() > 0)
						{
							while(rows->NextRow())
							{
								...
								Q_snprintf(dxversion, sizeof(dxversion), "%u.%u.%u.%u", (vhigh >> 16), vhigh & 0xffff,
										   (vlow >> 16), vlow & 0xffff);
								...
								if(success)
								{
									...
									if(zipurl[0])
									{
										...
										Q_snprintf(zipurlfixed, sizeof(zipurlfixed), BUG_REPOSITORY_FMT, database, id,
												   basepath);
							...
							for(int i = 0; i < c; ++i)
							{
								Q_snprintf(q, sizeof(q), "update bugreports set Processed=1 where BugId=%i;",
										   processed[i]);
