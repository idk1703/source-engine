		...
		do
		{
			...
			if(nLoopCtr == 257)
			{
				Q_snprintf(outputBuffer, sizeof(outputBuffer),
						   "/*%04d:*/ { {%+14e,%+14e,%+14e}, {%+14e,%+14e,%+14e}, {%+14e,%+14e,%+14e}, "
						   "{%+14e,%+14e,%+14e} },\n",
						   i, pXYZ->X(0), pXYZ->Y(0), pXYZ->Z(0), pXYZ->X(1), pXYZ->Y(1), pXYZ->Z(1), pXYZ->X(2),
						   pXYZ->Y(2), pXYZ->Z(2), pXYZ->X(3), pXYZ->Y(3), pXYZ->Z(3));
				Warning(outputBuffer);
			}
#endif // RECORD_OUTPUT
...
void OctoberXDKCompilerIssueTestCode(const fltx4 &val, fltx4 *out)
{
	...
	//         The assembly contains tons of useless instructions (vector stores and supporting integer math), even in
	//         the below code - no use of pointers or static constants, no wrapper layers on top of the vector
	//         intrinsics. If/when the compiler issue is resolved, other known issues are:
