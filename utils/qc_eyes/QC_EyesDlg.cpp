...
CQC_EyesDlg::CQC_EyesDlg(CWnd *pParent /*=NULL*/) : CDialog(CQC_EyesDlg::IDD, pParent)
{
...
void CQC_EyesDlg::GenerateQCText()
{
	...
	AddText("$attachment \"eyes\" \"ValveBiped.Bip01_Head1\" %.2f %.2f %.2f absolute\n",
			...
			(p.m_flLeftEye[1] + p.m_flRightEye[1]) * 0.5, (p.m_flLeftEye[2] + p.m_flRightEye[2]) * 0.5);
	...
	AddText("\teyeball righteye \"ValveBiped.Bip01_Head1\" %.2f %.2f %.2f \"%s_r\" %.2f 4 \"%s_r\" %.2f\n",
			p.m_flRightEye[0], p.m_flRightEye[1], p.m_flRightEye[2], p.m_EyeballPrefix, p.m_flEyeballSize,
			p.m_PupilPrefix, p.m_flIrisSize);

	AddText("\teyeball lefteye \"ValveBiped.Bip01_Head1\" %.2f %.2f %.2f \"%s_l\" %.2f -4 \"%s_l\" %.2f\n\n",
			p.m_flLeftEye[0], p.m_flLeftEye[1], p.m_flLeftEye[2], p.m_EyeballPrefix, p.m_flEyeballSize, p.m_PupilPrefix,
			p.m_flIrisSize);
	...
	AddText("\teyelid  upper_right \"%s\" lowerer 1 %.2f neutral 0 %.2f raiser 2 %.2f split 0.1 eyeball righteye\n",
			...
			p.m_flRightUpperLidNeutral - p.m_flRightEye[2], p.m_flRightUpperLidRaised - p.m_flRightEye[2]);
	...
	AddText("\teyelid  lower_right \"%s\" lowerer 3 %.2f neutral 0 %.2f raiser 4 %.2f split 0.1 eyeball righteye\n",
			...
			p.m_flRightLowerLidNeutral - p.m_flRightEye[2], p.m_flRightLowerLidRaised - p.m_flRightEye[2]);
	...
	AddText("\teyelid  upper_left \"%s\" lowerer 1 %.2f neutral 0 %.2f raiser 2 %.2f split -0.1 eyeball lefteye\n",
			...
			p.m_flLeftUpperLidNeutral - p.m_flLeftEye[2], p.m_flLeftUpperLidRaised - p.m_flLeftEye[2]);
	...
	AddText("\teyelid  lower_left \"%s\" lowerer 3 %.2f neutral 0 %.2f raiser 4 %.2f split -0.1 eyeball lefteye\n\n",
			...
			p.m_flLeftLowerLidNeutral - p.m_flLeftEye[2], p.m_flLeftLowerLidRaised - p.m_flLeftEye[2]);
	...
	for(int i = 0; i < sizeof(controls) / sizeof(controls[0]); i++)
	{
		...
		for(int z = 0; z < (int)strlen(text); z++)
		{
			if(text[z] < '0' || text[z] > '9')
			{
				...
				{
					char errMsg[512];
					_snprintf(errMsg, sizeof(errMsg), "The '%s' control must have a numeric value.",
							  controls[i].pControlName);
...
{
	SetupBitmapLabel(IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_XY_R : IDB_EYE_Z_R,
					 "Enter the Y position of the center vertex of the right eye");
}

void CQC_EyesDlg::OnSetfocusRightEyeZ()
{
	SetupBitmapLabel(IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_Z_R : IDB_EYE_XY_R,
...
{
	SetupBitmapLabel(IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_XY_L : IDB_EYE_Z_L,
					 "Enter the Y position of the center vertex of the right eye");
}

void CQC_EyesDlg::OnSetfocusLeftEyeZ()
{
	SetupBitmapLabel(IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_Z_L : IDB_EYE_XY_L,
					 "Enter the Z position of the center vertex of the right eye");
}
...
void CQC_EyesDlg::OnSetfocusUpperLidLowered()
{
	...
	SetupBitmapLabel(IDB_EYE_UPPER_LO,
					 "At Frame 1, enter the %s position of the center vertex of the right upper eye lid", pCoord);
}
...
void CQC_EyesDlg::OnSetfocusUpperLidNeutral()
{
	...
	SetupBitmapLabel(IDB_EYE_UPPER_MID,
					 "At Frame 0, enter the %s position of the center vertex of the right upper eye lid", pCoord);
}
...
void CQC_EyesDlg::OnSetfocusLowerLidLowered()
{
	...
	SetupBitmapLabel(IDB_EYE_LOWER_LO,
					 "At Frame 3, enter the %s position of the center vertex of the right lower eye lid", pCoord);
}
...
void CQC_EyesDlg::OnSetfocusLowerLidNeutral()
{
	...
	SetupBitmapLabel(IDB_EYE_LOWER_MID,
					 "At Frame 0, enter the %s position of the center vertex of the right lower eye lid", pCoord);
}
...
void CQC_EyesDlg::OnSetfocusLowerLidRaised()
{
	...
	SetupBitmapLabel(IDB_EYE_LOWER_HI,
					 "At Frame 4, enter the %s position of the center vertex of the right lower eye lid", pCoord);
}
...
int g_AdvancedControls[] = {IDC_EYE_DETAIL_CONTROL_FRAME, IDC_IRIS_SIZE,	   IDC_IRIS_SIZE_LABEL, IDC_EYEBALL_SIZE,
...
int g_LeftLidPositionControls[] = {
	IDC_UPPER_LEFT_LID_PANEL,		  IDC_UPPER_LEFT_LID_RAISED,		IDC_UPPER_LEFT_LID_RAISED_LABEL,
	IDC_UPPER_LEFT_LID_NEUTRAL,		  IDC_UPPER_LEFT_LID_NEUTRAL_LABEL, IDC_UPPER_LEFT_LID_LOWERED,
	IDC_UPPER_LEFT_LID_LOWERED_LABEL, IDC_LOWER_LEFT_LID_PANEL,			IDC_LOWER_LEFT_LID_RAISED,
	IDC_LOWER_LEFT_LID_RAISED_LABEL,  IDC_LOWER_LEFT_LID_NEUTRAL,		IDC_LOWER_LEFT_LID_NEUTRAL_LABEL,
