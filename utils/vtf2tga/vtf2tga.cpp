		...
		for(int iMipLevel = 0; iMipLevel <= iLastMipLevel; ++iMipLevel)
		{
			...
			for(int iCubeFace = 0; iCubeFace < iFaceCount; ++iCubeFace)
			{
				for(int z = 0; z < iDepth; ++z)
				{
					...
					{
						if(ImageLoader::IsTransparent(srcFormat) || (srcFormat == IMAGE_FORMAT_ATI1N) ||
					...
					unsigned char *pDstImage =
						new unsigned char[ImageLoader::GetMemRequired(iWidth, iHeight, 1, dstFormat, false)];
					if(!ImageLoader::ConvertImageFormat(pSrcImage, srcFormat, pDstImage, dstFormat, iWidth, iHeight, 0,
														0))
					{
					...
					if(dstFormat != IMAGE_FORMAT_RGB323232F)
					{
						if(ImageLoader::IsTransparent(dstFormat) && (dstFormat != IMAGE_FORMAT_RGBA8888))
						{
							...
							pDstImage = new unsigned char[ImageLoader::GetMemRequired(iWidth, iHeight, 1,
