...
void SpewMoveto(bool first, char const *vcdname, CChoreoEvent *e)
{
	...
	logprint("moveto.txt", "\"%s\",%i,\"%s\",%.3f,\"%s\",%s\n", vcdname, ++ecounter, e->GetName(), e->GetStartTime(),
			 e->GetParameters(), e->IsResumeCondition() ? "YES" : "no");
}
...
static bool EventCollidesWithRows(CUtlLinkedList<CChoreoEvent *, int> &list, CChoreoEvent *event, char *trackName,
								  size_t trackNameLength)
{
...
void CheckForOverlappingFlexTracks(CChoreoScene *scene)
{
	for(int a = 0; a < scene->GetNumActors(); ++a)
	{
		...
		for(i = actorFlexEvents.FirstInorder(); i != actorFlexEvents.InvalidIndex() && !done;
			i = actorFlexEvents.NextInorder(i))
		{
			...
			for(int j = 0; j < rowCount; j++)
			{
				...
				else
				{
					Msg("[%s] has overlapping events for actor [%s] [%s] [flex: %s]\n", scene->GetFilename(),
						actor->GetName(), e->GetName(), offender);
					done = true;
				}
			}
...
void ProcessVCD(CUtlDict<VCDList, int> &database, CUtlSymbol &vcdname)
{
	...
	if(scene)
	{
		...
		for(int i = 0; i < c; i++)
		{
			...
			Q_snprintf(fullwavename, sizeof(fullwavename), "%ssound\\%s", gamedir, wavename);
