		...
		if(cl->m_bDerivedFromCBaseEntity)
		{
			...
			if(GetPrintHierarchy() && GetPrintMembers())
			{
				...
				for(i = 0; i < cl->m_nTDCount; i++)
				{
					...
					vprint(0, "%s( %s, %s, %s, ... )\n", td->m_szDefineType, cl->m_szName, td->m_szVariableName,
						   td->m_szType);
				}
...
char *CCodeProcessor::ParseTypeDescription(char *current, bool fIsMacroized)
{
	...
	while(1)
	{
		...
		{
			if(!stricmp(com_token, "END_DATADESC") || !stricmp(com_token, "END_BYTESWAP_DATADESC"))
				break;
		}
		...
		if(!stricmp(definetype, "DEFINE_FUNCTION") || !stricmp(definetype, "DEFINE_THINKFUNC") ||
		   !stricmp(definetype, "DEFINE_ENTITYFUNC") || !stricmp(definetype, "DEFINE_USEFUNC") ||
		   !stricmp(definetype, "DEFINE_OUTPUT") || !stricmp(definetype, "DEFINE_INPUTFUNC"))
		{
			strcpy(vartype, "funcptr");
		}
		else if(!stricmp(definetype, "DEFINE_FIELD") || !stricmp(definetype, "DEFINE_INDEX") ||
				!stricmp(definetype, "DEFINE_KEYFIELD") || !stricmp(definetype, "DEFINE_KEYFIELD_NOT_SAVED") ||
				!stricmp(definetype, "DEFINE_UTLVECTOR") || !stricmp(definetype, "DEFINE_GLOBAL_FIELD") ||
				!stricmp(definetype, "DEFINE_GLOBAL_KEYFIELD") || !stricmp(definetype, "DEFINE_CUSTOM_FIELD") ||
				!stricmp(definetype, "DEFINE_INPUT") || !stricmp(definetype, "DEFINE_AUTO_ARRAY") ||
				!stricmp(definetype, "DEFINE_AUTO_ARRAY_KEYFIELD") || !stricmp(definetype, "DEFINE_AUTO_ARRAY2D") ||
...
bool CCodeProcessor::CheckShouldSkip(bool forcequiet, int depth, char const *filename, int &numheaders,
									 int &skippedfiles)
{
...
bool CCodeProcessor::LoadFile(char **buffer, char *filename, char const *module, bool forcequiet, int depth,
							  int &filelength, int &numheaders, int &skippedfiles, char const *srcroot,
							  char const *root, char const *baseroot)
{
								   ...
								   const char *srcroot, const char *baseroot, const char *root, const char *module)
{
	...
	if(!LoadFile(&buffer, filename, module, forcequiet, depth, filelength, numheaders, skippedfiles, srcroot, root,
				 baseroot))
	{
		CODE_MODULE module;
		module.skipped = true;
	...
	while(1)
	{
		...
		{
			current = CC_ParseToken(current);
			if(strlen(com_token) > 0 && com_token[0] != '<')
			{
				// vprint( "#include %s\n", com_token );
				m_nHeadersProcessed++;
				numheaders++;
		...
		else if(!stricmp(com_token, "class") || !stricmp(com_token, "struct"))
		{
			current = CC_ParseToken(current);
			if(strlen(com_token) > 0)
			{
		...
		else if(!strcmp(com_token, "TYPEDESCRIPTION") || !strcmp(com_token, "typedescription_t"))
		{
			current = ParseTypeDescription(current, false);
		}
		else if(!strcmp(com_token, "BEGIN_DATADESC") || !strcmp(com_token, "BEGIN_DATADESC_NO_BASE") ||
				!strcmp(com_token, "BEGIN_SIMPLE_DATADESC") || !strcmp(com_token, "BEGIN_BYTESWAP_DATADESC"))
		{
			current = ParseTypeDescription(current, true);
		}
		else if(!strcmp(com_token, "BEGIN_PREDICTION_DATA") || !strcmp(com_token, "BEGIN_EMBEDDED_PREDDESC"))
		{
			current = ParsePredictionTypeDescription(current);
		}
		else if(!strcmp(com_token, "BEGIN_RECV_TABLE") || !strcmp(com_token, "BEGIN_RECV_TABLE_NOBASE") ||
				!strcmp(com_token, "IMPLEMENT_CLIENTCLASS_DT") || !strcmp(com_token, "IMPLEMENT_CLIENTCLASS_DT_NOBASE"))
		{
			...
			if(!strcmp(com_token, "("))
			{
				...
				if(cl)
				{
					if(cl->m_bHasPredictionData)
					{
						...
						{
							vprint(0,
								   "Class %s declared predictable and implemented with IMPLEMENT_PREDICTABLE_NODATA in "
								   "typedescription\n",
								   cl->m_szName);
						}
	...
	{
		vprint(0, " %s: headers (%i game / %i total)", (char *)&filename[m_nOffset], numheaders - skippedfiles,
			   numheaders);
		if(maxdepth > 1)
		{
...
void CCodeProcessor::PrintResults(const char *baseentityclass)
{
	...
	if(elapsed > 0.0)
	{
		vprint(0, "%.2f K processed in %.3f seconds, throughput %.2f KB/sec\n\n", (double)m_nBytesProcessed / 1024.0,
			   elapsed, (double)m_nBytesProcessed / (1024.0 * elapsed));
	}
...
void CCodeProcessor::ConstructModuleList_R(int level, const char *baseentityclass, const char *gamespecific,
										   const char *root, const char *srcroot)
{
	char directory[256];
	char filename[256];
	WIN32_FIND_DATA wfd;
...
void CCodeProcessor::Process(const char *baseentityclass, const char *gamespecific, const char *sourcetreebase,
							 const char *subdir)
{
...
void CCodeProcessor::Process(const char *baseentityclass, const char *gamespecific, const char *sourcetreebase,
							 const char *subdir, const char *pFileName)
{
	SetupIncludePath(sourcetreebase, subdir, gamespecific);
