...
bool CClass::CheckForMissingTypeDescriptionFields(int &missingcount, bool createtds)
{
	...
	for(int i = 0; i < m_nVarCount; i++)
	{
		...
		vprint(0, "//\tClass %s:\n", m_szName);
		if(var->m_bIsArray && (stricmp(var->m_szType, "char") || stricmp(t, "FIELD_STRING")))
		{
			if(*t)
			{
...
bool CClass::CheckForPredictionFieldsInRecvTableNotMarkedAsSuchCorrectly(int &missingcount)
{
	...
	for(int i = 0; i < m_nVarCount; i++)
	{
		...
		else
		{
			vprint(
				0,
				"%s::%s:  Field marked as FTYPEDESC_INSENDTABLE in prediction typedescription missing from RecvTable\n",
				m_szName, var->m_szName);
		}
	}
...
{
	if(!strcmp(com_token, "DECLARE_CLASS") || !strcmp(com_token, "DECLARE_CLASS_GAMEROOT") ||
...
bool CClass::ParseNetworkVar(char *&input, int protection)
{
	...
	if(!strcmp(com_token, "CNetworkVar") || !strcmp(com_token, "CNetworkVarForDerived") ||
	...
	if(!strcmp(com_token, "CNetworkVector") || !strcmp(com_token, "CNetworkVectorForDerived") ||
...
bool CClass::ParseClassMember(char *&input, int protection)
{
	...
	do
	{
		...
		{
			if(!stricmp(var.m_pName, "typedef") || !stricmp(var.m_pName, "enum") || !stricmp(var.m_pName, "friend"))
			{
				skipvar = true;
			}
			AppendType(var.m_pName, var.m_pType);
	...
	if(var.m_pType[0] == 0 &&
	   (!strcmp(var.m_pName, "CUSTOM_SCHEDULES") || !strcmp(var.m_pName, "DEFINE_CUSTOM_SCHEDULE_PROVIDER") ||
		!strcmp(var.m_pName, "DEFINE_CUSTOM_AI") || !strcmp(var.m_pName, "DECLARE_DATADESC") ||
		!strcmp(var.m_pName, "DECLARE_EMBEDDED_DATADESC") || !strcmp(var.m_pName, "DECLARE_SERVERCLASS") ||
		!strcmp(var.m_pName, "DECLARE_CLIENTCLASS") || !strcmp(var.m_pName, "DECLARE_ENTITY_PANEL") ||
		!strcmp(var.m_pName, "DECLARE_MINIMAP_PANEL") || !strcmp(var.m_pName, "MANUALMODE_GETSET_PROP")))
	{
		return true;
	}

	if(var.m_pType[0] == 0 &&
	   (!strcmp(var.m_pName, "DECLARE_PREDICTABLE") || !strcmp(var.m_pName, "DECLARE_EMBEDDED_PREDDESC")))
	{
...
{
	if(!Q_strncmp(name, "m_", 2) || !Q_strncmp(name, "g_", 2) || !Q_strncmp(name, "s_", 2))
	{
		return true;
	}
...
void CClass::CheckForHungarianErrors(int &warnings)
{
	...
	for(int i = 0; i < m_nVarCount; i++)
	{
		...
		if(!Q_strncmp(var->m_szName, "m_f", 3) && Q_strncmp(var->m_szName, "m_fl", 4) && isupper(var->m_szName[3]))
		{
			// If it's a "flag" and not a "float" type, it better be a bool or an int
			if(!Q_stristr(var->m_szType, "bool") && !Q_strstr(var->m_szType, "int"))
			{
				vprint(1, "%s might have wrong type %s\n", var->m_szName, var->m_szType);
				++warnings;
				return;
