...
#define my_b_read(info, Buffer, Count)                                                       \
	((info)->rc_pos + (Count) <= (info)->rc_end                                              \
		 ? (memcpy(Buffer, (info)->rc_pos, (size_t)(Count)), ((info)->rc_pos += (Count)), 0) \
		 : (*(info)->read_function)((info), Buffer, Count))

#define my_b_get(info) \
	((info)->rc_pos != (info)->rc_end ? ((info)->rc_pos++, (int)(uchar)(info)->rc_pos[-1]) : _my_b_get(info))

#define my_b_write(info, Buffer, Count)                                                      \
	((info)->rc_pos + (Count) <= (info)->rc_end                                              \
		 ? (memcpy((info)->rc_pos, Buffer, (size_t)(Count)), ((info)->rc_pos += (Count)), 0) \
		 : _my_b_write(info, Buffer, Count))

	/* my_b_write_byte dosn't have any err-check */
#define my_b_write_byte(info, chr)                                     \
	(((info)->rc_pos < (info)->rc_end) ? ((*(info)->rc_pos++) = (chr)) \
...
#define ST_USED_MEM_DEFINED
	...
	extern File my_register_filename(File fd, const char *FileName, enum file_type type_of_file,
...
#define base_name(A) (A + dirname_length(A))
	...
	extern my_string fn_format(my_string to, const char *name, const char *dsk, const char *form, int flag);
	...
	extern byte *key_cache_read(File file, my_off_t filepos, byte *buff, uint length, uint block_length,
								int return_buffer);
	extern int key_cache_write(File file, my_off_t filepos, byte *buff, uint length, uint block_length,
							   int force_write);
...
#define push_dynamic(A, B)						  insert_dynamic(A, B)
	...
	extern my_bool init_dynamic_string(DYNAMIC_STRING *str, const char *init_str, uint init_alloc,
									   uint alloc_increment);
