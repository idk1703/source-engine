...
//-----------------------------------------------------------------------------
static int FindVertexWithinVertexDictionary(const VertexInfo_t &find, const CVertexDictionary &vertexDict,
											int nStartVert, int nEndVert, int fIgnore)
{
	int nBestIndex = -1;
	...
	for(int nVertexIndex = nStartVert; nVertexIndex < nEndVert; ++nVertexIndex)
	{
		...
		if(!(fIgnore & IGNORE_BONEWEIGHT) &&
		   !CompareBoneWeightsFuzzy(find.m_BoneWeight, vertexDict.Vertex(nVertexIndex).m_BoneWeight, flBoneWeightError))
			continue;
...
//-----------------------------------------------------------------------------
static void FindBoneWeightWithinModel(const VertexInfo_t &searchVertex, const s_source_t *pSrc,
									  s_boneweight_t &boneWeight, int fIgnore)
{
	int nBestIndex = -1;
	float flPositionError = 0.0f;
	float flNormalError = 0.0f;
										   ...
										   const s_source_t *pRootLODSrc, VertexInfo_t &idealVertex)
{
	...
	int nVertexDictID =
		FindVertexWithinVertexDictionary(searchVertex, vertexDict, vertexDict.RootLODVertexStart(),
										 vertexDict.RootLODVertexEnd(), IGNORE_BONEWEIGHT | IGNORE_TANGENTS);
	if(nVertexDictID != -1)
	{
							   ...
							   VertexInfo_t &idealVertex)
{
	...
	int nVertexDictID = FindVertexWithinVertexDictionary(searchVertex, vertexDict, pVertexDictMesh->vertexoffset,
														 vertexDict.PrevLODVertexCount(), 0);
	if(nVertexDictID != -1)
	{
...
//-----------------------------------------------------------------------------
static int FindVertexInDictionaryExact(CVertexDictionary &vertexDict, int nStartVert, int nEndVert,
									   const VertexInfo_t &vertex)
{
...
//-----------------------------------------------------------------------------
static int FindOrCreateExactVertexInDictionary(CVertexDictionary &vertexDict, const VertexInfo_t &vertex,
											   s_mesh_t *pDstMesh)
{
...
static void PrintSourceVerts(s_source_t *pSrc)
{
	...
	for(int i = 0; i < nVertexCount; i++)
	{
		...
		{
			printf("\t%d: %d %f\n", j, (int)srcVertex.boneweight.bone[j], srcVertex.boneweight.weight[j]);
		}
		fflush(stdout);
	}
}
...
//-----------------------------------------------------------------------------
static void SetProcessedWithDictionary(s_model_t *pSrcModel, CVertexDictionary &vertexDict, CUtlVector<s_face_t> &faces,
									   CUtlVector<s_mesh_t> &meshes, int *pMeshVertIndexMaps[MAX_NUM_LODS])
{
	int i;
...
static void BuildBoneLODMapping(CUtlVector<int> &boneMap, int lodID)
{
	...
	for(i = 0; i < scriptLOD.boneReplacements.Size(); i++)
	{
		...
		{
			// FIXME: is this really an error?  It could just be  replacement command for bone that doesnt' exist
			// anymore.
			if(g_verbose)
			{
...
static void MarkRootLODBones(CVertexDictionary &vertexDictionary)
{
	...
	for(int nVertDictID = vertexDictionary.RootLODVertexStart(); nVertDictID < vertexDictionary.RootLODVertexEnd();
		nVertDictID++)
	{
...
static void ReplaceBonesRecursive(int globalBoneID, bool replaceThis,
								  CUtlVector<CLodScriptReplacement_t> &boneReplacements, const char *replacementName)
{
	if(replaceThis)
	{
...
void ConvertBoneTreeCollapsesToReplaceBones(void)
{
	...
	for(i = 0; i < g_ScriptLODs.Size(); i++)
	{
		...
		{
			ConvertSingleBoneTreeCollapseToReplaceBones(lod.boneTreeCollapses[j], lod.boneReplacements);
		}
	}
}
