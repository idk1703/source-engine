...
void Grab_Vertexlist(s_source_t *psource)
{
	while(1)
	{
		if(fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL)
		{
			...
			int i =
				sscanf(g_szLine, "%d %d %f %f %f %d %d %f %d %f %d %f %d %f", &j, &bone, &p[0], &p[1], &p[2], &iCount,
...
void Grab_Facelist(s_source_t *psource)
{
	while(1)
	{
		if(fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL)
		{
			...
			if(sscanf(g_szLine, "%d %d %d %d", &j, &f.a, &f.b, &f.c) == 4)
			{
				g_face[j] = f;
			}
			else
...
void Grab_Materiallist(s_source_t *psource)
{
	while(1)
	{
		if(fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL)
		{
			...
			if(sscanf(g_szLine, "%d  %f %f %f %f   %f %f %f %f  %f %f %f %f  %f \"%[^\"]s", &j, &a.r, &a.g, &a.b, &a.a,
					  &d.r, &d.g, &d.b, &d.a, &s.r, &s.g, &s.b, &s.a, &g, path) == 15)
			{
				if(path[0] == '\0')
				{
					psource->texmap[j] = -1;
...
void Grab_Texcoordlist(s_source_t *psource)
{
	while(1)
	{
		if(fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL)
		{
			...
			if(sscanf(g_szLine, "%d %f %f", &j, &t[0], &t[1]) == 3)
			{
				t[1] = 1.0 - t[1];
				g_texcoord[j][0] = t[0];
				g_texcoord[j][1] = t[1];
...
void Grab_Normallist(s_source_t *psource)
{
	while(1)
	{
		if(fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL)
		{
			...
			if(sscanf(g_szLine, "%d %d %f %f %f", &j, &bone, &n[0], &n[1], &n[2]) == 5)
			{
				if(bone < 0 || bone >= psource->numbones)
				{
					MdlWarning("bogus bone index\n");
...
void Grab_Faceattriblist(s_source_t *psource)
{
	while(1)
	{
		if(fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL)
		{
			...
			if(sscanf(g_szLine, "%d %d %d %d %d %d %d %d %d", &j, &material, &smooth, &f.ta, &f.tb, &f.tc, &f.na, &f.nb,
					  &f.nc) == 9)
			{
				f.a = g_face[j].a;
				f.b = g_face[j].b;
