...
public:
	...
	enum CheckMask_t
	{
		...
		CHECK_UV_ALL_FLAGS =
			(CHECK_UV_FLAG_NORMALIZED | CHECK_UV_FLAG_OVERLAP | CHECK_UV_FLAG_INVERSE | CHECK_UV_FLAG_GUTTER)
	};
	...
	bool DoCheck(CheckMask_t eCheckMask) const
	{
		return (m_nOptChecks & eCheckMask) == eCheckMask;
	}
	bool DoAnyCheck() const
	{
		return (m_nOptChecks & CHECK_UV_ALL_FLAGS) != 0;
	}
	void SetCheck(CheckMask_t eCheckMask)
	{
		m_nOptChecks |= (CHECK_UV_ALL_FLAGS & eCheckMask);
	}
	void ClearCheck(CheckMask_t eCheckMask)
	{
		m_nOptChecks &= (CHECK_UV_ALL_FLAGS & ~eCheckMask);
	}
	...
	static void PrintVertex(const struct s_vertexinfo_t &v, const struct s_texture_t &t,
							const char *pszPrefix = "       ");
	static void PrintFace(const s_source_t *pSource, const int nMesh, const int nFace,
						  const char *pszPrefix = "       ");
};