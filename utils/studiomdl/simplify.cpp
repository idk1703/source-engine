...
void matchBlend(s_animation_t *pDestAnim, s_animation_t *pSrcAnimation, int iSrcFrame, int iDestFrame, int iPre,
				int iPost);
	...
	for(i = 0; i < g_numani; i++)
	{
		...
		for(j = 0; j < panim->numcmds; j++)
		{
			...
			switch(pcmd->cmd)
			{
				...
				case CMD_MOTION:
				{
					extractLinearMotion(panim, pcmd->u.motion.motiontype, startframe, pcmd->u.motion.iEndFrame,
										pcmd->u.motion.iEndFrame, panim, startframe);
				...
				{
					extractLinearMotion(panim, pcmd->u.motion.motiontype, startframe, pcmd->u.motion.iEndFrame,
										pcmd->u.motion.iSrcFrame, pcmd->u.motion.pRefAnim, pcmd->u.motion.iRefFrame);
					startframe = pcmd->u.motion.iEndFrame;
				}
				break;
				case CMD_DERIVATIVE:
				{
					createDerivative(panim, pcmd->u.derivative.scale);
				}
				break;
	...
	for(j = 0; j < numframes; j++)
	{
		...
		for(k = 0; k < g_numbones; k++)
		{
			if(g_bonetable[k].parent == -1)
			{
				...
				// printf("%d : %.1f %.1f %.1f\n", j, panim->sanim[j+iStartFrame][k].pos.x,
				// panim->sanim[j+iStartFrame][k].pos.y, RAD2DEG( panim->sanim[j+iStartFrame][k].rot.z ) );
			}
		}
	}
		...
		for(j = 0; j < pdest->numframes; j++)
		{
			if(pdest->weight[k] > 0)
			{
				...
				printf("     %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f\n",
					RAD2DEG(pdest->sanim[j][k].pos[0]), RAD2DEG(pdest->sanim[j][k].pos[1]),
				RAD2DEG(pdest->sanim[j][k].pos[2]), RAD2DEG(pdest->sanim[j][k].rot[0]),
				RAD2DEG(pdest->sanim[j][k].rot[1]), RAD2DEG(pdest->sanim[j][k].rot[2]) );
				*/
				...
				printf("     %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f\n",
					...
					RAD2DEG(pdest->sanim[j][k].rot[0]), RAD2DEG(pdest->sanim[j][k].rot[1]),
				RAD2DEG(pdest->sanim[j][k].rot[2]) );
				*/
			}
		}
	}
...
//-----------------------------------------------------------------------------
void BuildRawTransforms(const s_source_t *psource, const char *pAnimationName, int frame, float scale,
						Vector const &shift, RadianEuler const &rotate, int flags, matrix3x4_t *boneToWorld)
{
	int k;
	Vector tmp;
	Vector pos;
...
// Purpose: convert source bone animation into global bone animation
//-----------------------------------------------------------------------------
void ConvertAnimation(const s_source_t *psource, const char *pAnimationName, int frame, float scale,
					  Vector const &shift, RadianEuler const &rotate, s_bone_t *dest)
{
	int k;
...
void fixupIKErrors(s_animation_t *panim, s_ikrule_t *pRule)
{
	...
	switch(pRule->type)
	{
		...
		case IK_GROUND:
		{
			...
			// printf("%d %d %d %d (%d)\n", pRule->start, pRule->peak, pRule->tail, pRule->end,
			...
			for(k = 0; k < pRule->errorData.numerror; k++)
			{
				...
				Studio_SolveIK(g_ikchain[pRule->chain].link[0].bone, g_ikchain[pRule->chain].link[1].bone,
							   g_ikchain[pRule->chain].link[2].bone, pos, boneToWorld);
								 ...
								 bool bNewVertexAnimations, int *pModelToVAnim)
{
	...
	for(int j = 0; j < pVSource->numvertices; j++)
	{
		...
		for(int k = 0; k < pmLodSource->numvertices; k++)
		{
			...
			// TODO: take this out when the lod clamping stuff gets moved into the LOD code instead of being a post
			// process
			if(nMinLod && !(pmLodSource->vertex[k].lodFlag & (0xFFFFFF << nMinLod)))
				continue;

			const Vector &vecModelPos =
				bNewVertexAnimations ? pVSource->vertex[j].position : pVSourceAnim->vanim[0][j].pos;
			...
			const Vector &vecModelNormal =
				bNewVertexAnimations ? pVSource->vertex[j].normal : pVSourceAnim->vanim[0][j].normal;
		...
		if(flMinDist > 0.01)
		{
			...
			// printf("%.4f %.4f %.4f\n", pvsource->vanim[0][j].pos[0], pvsource->vanim[0][j].pos[1],
			// pvsource->vanim[0][j].pos[2] );
		}
		...
		if(n == -1)
		{
			// printf("no match for animated vertex %d : %.4f %.4f %.4f\n", j, pVSourceAnim->vanim[0][j].pos[0],
			// pVSourceAnim->vanim[0][j].pos[1], pVSourceAnim->vanim[0][j].pos[2] );
		}
	}
	...
	*/
	...
	printf("%4d : %8.2f %8.2f %8.2f : ", j, pvsource->vanim[0][j].pos[0], pvsource->vanim[0][j].pos[1],
	pvsource->vanim[0][j].pos[2] ); for (k = 0; k < pmsource->numvertices; k++)
	{
...
// Purpose: Build an array indexed by model vertex which indicates which vanim vertex corresponds best to it
//-----------------------------------------------------------------------------
static void BuildVAnimMap(s_source_t *pVSource, s_sourceanim_t *pVSourceAnim, s_loddata_t *pmLodSource,
		...
		for(m = 0; m < numsrcanims; m++)
		{
			if(pSourceAnim->vanim_mapcount[psrcanim->vertex]) // bah, only do it for ones that found a match!
			{
				for(n = 0; n < pSourceAnim->vanim_mapcount[psrcanim->vertex]; n++)
				{
					...
					// FIXME: the "default" pose needs to be in each vertex animation source since it's likely that the
					// vertices won't be numbered the same in each file.
	...
	for(i = 0; i < g_numflexkeys; i++)
	{
		...
		for(m = 0; m < numsrcanims; m++, psrcanim++)
		{
			...
			for(n = 0; n < pSourceAnim->vanim_mapcount[psrcanim->vertex]; n++)
			{
				...
				pdestanim->vertex,
				// pmsource->vertex[pdestanim->vertex][0], pmsource->vertex[pdestanim->vertex][1],
				pmsource->vertex[pdestanim->vertex][2], modelpos[pdestanim->vertex][0], modelpos[pdestanim->vertex][1],
				modelpos[pdestanim->vertex][2], psrcanim->vertex, pdestanim->pos[0], pdestanim->pos[1],
				pdestanim->pos[2] );
				*/
...
void findAnimQuaternionAlignment(int k, int i, Quaternion &qBase, Quaternion &qMin, Quaternion &qMax)
{
	...
	// printf("%s (%s): %.3f :%.3f\n", g_bonetable[k].name, g_panimation[i]->name, QuaternionDotProduct( qMin, qMax ),
	// QuaternionDotProduct( qMin, qBase ) );
	/*
	if (i != 0)
		exit(0);
	*/
...
bool BoneShouldCollapse(char const *pname)
{
	...
	return (!BoneHasAnimation(pname) && !BoneIsProcedural(pname) &&
			!BoneIsIK(pname) /* && !BoneHasAttachments( pname ) */);
}
			...
			if(!Q_stricmp(g_bonetable[j].name, pDmeBoneFlexDriver->m_sBoneName.Get()))
			{
				...
				for(int k = 0; k < pDmeBoneFlexDriver->m_eControlList.Count(); ++k)
				{
					pDmeBoneFlexDriver->m_eControlList[k]->m_nBoneComponent =
						GetRemappedBoneAxis(j, pDmeBoneFlexDriver->m_eControlList[k]->m_nBoneComponent);
				}
				break;
			}
		}
...
void TagFlexDriverBones(s_source_t *pSource)
{
	...
	for(int i = pDmeBoneFlexDriverList->m_eBoneFlexDriverList.Count() - 1; i >= 0; --i)
	{
		...
		for(int j = pDmeBoneFlexDriver->m_eControlList.Count() - 1; j >= 0; --j)
		{
			...
			{
				MdlWarning("DmeBoneFlexDriver Bone: %s - Flex Controller: %s, Bone Component Out Of Range: %d [0-2], "
						   "Ignoring\n",
						   pDmeBoneFlexDriver->m_sBoneName.Get(),
						   pDmeBoneFlexDriverControl->m_sFlexControllerName.Get(),
						   pDmeBoneFlexDriverControl->m_nBoneComponent.Get());
				pDmeBoneFlexDriver->m_eControlList.Remove(j);
				continue;
			}
	...
	for(j = 0; j < g_numquatinterpbones; j++)
	{
		...
		for(int k = 0; k < pInterp->numtriggers; k++)
		{
			...
			if(parent != -1)
			{
				...
				printf("srcControlRelative  : %7.2f %7.2f %7.2f\n", RAD2DEG( angles.x ), RAD2DEG( angles.y ), RAD2DEG(
				angles.z ) );
...
void RealignBones()
{
	...
	for(int i = 0; i < g_numikchains; i++)
	{
		...
		else
		{
			MdlError("Trying to realign bone \"%s\" with two children \"%s\", \"%s\"\n", g_bonetable[k].name,
					 g_bonetable[childbone[k]].name, g_bonetable[g_ikchain[i].link[1].bone].name);
		}
		...
		{
			MdlError("Trying to realign bone \"%s\" with two children \"%s\", \"%s\"\n", g_bonetable[k].name,
					 g_bonetable[childbone[k]].name, g_bonetable[g_ikchain[i].link[2].bone].name);
		}
	}
	...
	for(k = 0; k < g_numbones; k++)
	{
		// printf("%s  %.4f %.4f %.4f  (%d)\n", g_bonetable[k].name, g_bonetable[k].pos.x, g_bonetable[k].pos.y,
		// g_bonetable[k].pos.z, children[k] );
		if(!g_bonetable[k].bPreAligned && childbone[k] != -1)
		{
			...
			if(d - g_bonetable[childbone[k]].pos.x > 0.01)
			{
				Vector v2;
				Vector v3;
				// printf("%s:%s  %.4f %.4f %.4f\n", g_bonetable[k].name, g_bonetable[childbone[k]].name,
				// g_bonetable[childbone[k]].pos.x, g_bonetable[childbone[k]].pos.y, g_bonetable[childbone[k]].pos.z );
				...
				d = fabs(DotProduct(forward, left)) + fabs(DotProduct(left, up)) + fabs(DotProduct(up, forward)) +
				...
				// printf("%f %f %f\n", DotProduct( boneToPose[k][0], boneToPose[k][1] ), DotProduct( boneToPose[k][1],
				// boneToPose[k][2] ), DotProduct( boneToPose[k][2], boneToPose[k][0] ) );
...
void SlerpBones(Quaternion q1[MAXSTUDIOBONES], Vector pos1[MAXSTUDIOBONES], int sequence,
				const Quaternion q2[MAXSTUDIOBONES], const Vector pos2[MAXSTUDIOBONES], float s)
{
	int i;
	Quaternion q3, q4;
	float s1, s2;
...
void CalcModelTangentSpaces(s_source_t *pSrc)
{
	...
	for(meshID = 0; meshID < pSrc->nummeshes; meshID++)
	{
		...
		{
			s_face_t *pFace = &pSrc->face[triID + pMesh->faceoffset];
			CalcTriangleTangentSpace(pSrc, pMesh->vertexoffset + pFace->a, pMesh->vertexoffset + pFace->b,
...
static void LinkAttachments()
{
	...
	for(i = 0; i < g_numattachments; i++)
	{
		...
		// g_bonetable[g_attachment[i].bone].name );
		...
		for(int iPose = 0; iPose < 2; iPose++)
		{
			if(pseq->groupsize[iPose] > 1)
			{
				if(pseq->paramattachment[iPose] != -1)
				{
					...
					// printf("%s : %s : %6.2f %6.2f %6.2f : %6.2f %6.2f %6.2f\n", pseq->name, g_pose[j0].name, RAD2DEG(
					// angles.x ), RAD2DEG( angles.y ), RAD2DEG( angles.z ), pos.x, pos.y, pos.z );
					...
					for(m[iPose] = 0; m[iPose] < pseq->groupsize[iPose]; m[iPose]++)
					{
						...
						// printf("%6.2f %6.2f %6.2f : %6.2f %6.2f %6.2f\n", RAD2DEG( angles.x ), RAD2DEG( angles.y ),
						// RAD2DEG( angles.z ), pos.x, pos.y, pos.z );
		...
		for(j = 0; j < panim->numframes; j++)
		{
			...
			for(n = 0; n < g_numikchains; n++)
			{
				if(panim->weight[g_ikchain[n].link[2].bone] > 0)
				{
					...
					Studio_SolveIK(g_ikchain[n].link[0].bone, g_ikchain[n].link[1].bone, g_ikchain[n].link[2].bone,
	...
	for(i = 0; i < g_numani; i++)
	{
		...
		for(j = 0; j < panim->numikrules; j++)
		{
			...
			if(pRule->start == -1)
			{
				...
				// printf("%s : %d (%d) : %d %d %d %d\n", panim->name, pRule->chain, panim->numframes - 1, pRule->start,
				// pRule->peak, pRule->tail, pRule->end );
			}
			...
			pRule->peak, pRule->tail, pRule->end, pRule->usesequence ? "usesequence" : pRule->usesource ? "source" : ""
			);
			*/
			...
			switch(pRule->type)
			{
				case IK_SELF:
				{
					...
					for(k = 0; k < pRule->errorData.numerror; k++)
					{
						...
						printf("%d  %.1f %.1f %.1f : %.1f %.1f %.1f\n",
							k,
							pRule->errorData.pError[k].pos.x, pRule->errorData.pError[k].pos.y,
						pRule->errorData.pError[k].pos.z, ang.x, ang.y, ang.z );
						*/
					}
				}
				break;
				...
				for(int n = 0; n < panim1->numikrules; n++)
				{
					   ...
					   (panim1->ikrule[n].slot != panim2->ikrule[n].slot))
					{
						MdlError("%s - mismatched IK rule %d: \n\"%s\" : %d %d %d\n\"%s\" : %d %d %d\n",
								 g_sequence[i].name, n, panim1->name, panim1->ikrule[n].type, panim1->ikrule[n].chain,
								 panim1->ikrule[n].slot, panim2->name, panim2->ikrule[n].type, panim2->ikrule[n].chain,
								 panim2->ikrule[n].slot);
					}
				}
			}
		}
...
{
	return reinterpret_cast<const BonePriority_s *>(a)->m_nGlobalBoneWeight <
				   reinterpret_cast<const BonePriority_s *>(b)->m_nGlobalBoneWeight
			   ? -1
...
{
	BonePriority_s *pBonePriorityList =
		reinterpret_cast<BonePriority_s *>(stackalloc(g_numbones * sizeof(BonePriority_s)));
			   ...
			   g_bonetable[k].bmin[2] < g_bonetable[k].bmax[2] - 1)
			{
				...
				if(dump_hboxes)
				{
					printf("$hbox %d \"%s\" %.2f %.2f %.2f  %.2f %.2f %.2f\n", set->hitbox[set->numhitboxes].group,
						   g_bonetable[set->hitbox[set->numhitboxes].bone].name, set->hitbox[set->numhitboxes].bmin[0],
						   set->hitbox[set->numhitboxes].bmin[1], set->hitbox[set->numhitboxes].bmin[2],
...
void CalcSequenceBoundingBoxes()
{
	...
	for(i = 0; i < g_numani; i++)
	{
		...
		if(bmin.x < g_vecMinWorldspace.x || bmin.y < g_vecMinWorldspace.y || bmin.z < g_vecMinWorldspace.z ||
		   bmax.x > g_vecMaxWorldspace.x || bmax.y > g_vecMaxWorldspace.y || bmax.z > g_vecMaxWorldspace.z)
		{
	...
	for(i = 0; i < g_sequence.Count(); i++)
	{
		...
		for(j = 0; j < g_sequence[i].groupsize[0]; j++)
		{
			for(k = 0; k < g_sequence[i].groupsize[1]; k++)
			{
				...
				if(panim->bmin[0] < bmin[0])
					bmin[0] = panim->bmin[0];
				if(panim->bmin[1] < bmin[1])
					bmin[1] = panim->bmin[1];
				if(panim->bmin[2] < bmin[2])
					bmin[2] = panim->bmin[2];
				if(panim->bmax[0] > bmax[0])
					bmax[0] = panim->bmax[0];
				if(panim->bmax[1] > bmax[1])
					bmax[1] = panim->bmax[1];
				if(panim->bmax[2] > bmax[2])
					bmax[2] = panim->bmax[2];
			}
		}
