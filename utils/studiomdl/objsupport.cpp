	...
	while(GetLineInput())
	{
		...
		if(strncmp(g_szLine, "f ", 2) == 0)
		{
			...
			while(bufParse.IsValid())
			{
				...
				f.material = material;
				f.a = v0 - 1;
				f.na = (n0 > 0) ? n0 - 1 : 0, f.ta = (t0 > 0) ? t0 - 1 : 0;
				f.b = v2 - 1;
				f.nb = (n2 > 0) ? n2 - 1 : 0, f.tb = (t2 > 0) ? t2 - 1 : 0;
				f.c = v1 - 1;
				f.nc = (n1 > 0) ? n1 - 1 : 0, f.tc = (t1 > 0) ? t1 - 1 : 0;
				g_face[i] = f;
...
int AppendVTAtoOBJ(s_source_t *psource, char *filename, int frame)
{
	...
	while(GetLineInput())
	{
		...
		else if(strncmp(g_szLine, "f ", 2) == 0)
		{
			...
			j = sscanf(g_szLine, "f %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d", &v0, &t0, &n0, &v1, &t1, &n1, &v2, &t2, &n2,
					   &v3, &t3, &n3);
