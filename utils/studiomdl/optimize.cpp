...
namespace OptimizedModel
{
	...
	public:
		bool OptimizeFromStudioHdr(studiohdr_t *phdr, s_bodypart_t *pSrcBodyParts, int vertCacheSize,
								   ...
								   int maxBonesPerTri, int maxBonesPerStrip, const char *fileName,
	...
	private:
		...
		void SetupMeshProcessing(studiohdr_t *phdr, int vertCacheSize, bool usesFixedFunction, int maxBonesPerVert,
								 int maxBonesPerTri, int maxBonesPerStrip, const char *fileName);
		...
		void SourceMeshToTriangleList(s_model_t *pSrcModel, s_mesh_t *pSrcMesh,
		...
		void ProcessStripGroup(StripGroup_t *pStripGroup, bool isHWSkinned, bool isFlexed, mstudiomodel_t *pStudioModel,
							   mstudiomesh_t *pStudioMesh, CUtlVector<mstudioiface_t> &srcFaces,
							   TriangleProcessedList_t &trianglesProcessed, int maxBonesPerVert, int maxBonesPerTri,
		...
		bool GenerateStripGroupVerticesFromFace(mstudioiface_t *pFace, mstudiomesh_t *pStudioMesh,
												int maxPreferredBones, Vertex_t *pStripGroupVert);
		...
		void BuildSWSkinnedStrips(TriangleList_t &tris, VertexList_t const &verts, StripGroup_t *pStripGroup);
		void BuildHWSkinnedStrips(TriangleList_t &tris, VertexList_t &verts, StripGroup_t *pStripGroup,
		...
		void Stripify(VertexIndexList_t const &sourceIndices, bool isHWSkinned, int *pNumIndices,
		...
		void WriteHeader(int vertCacheSize, int maxBonesPerVert, int maxBonesPerTri, int maxBonesPerStrip,
						 int numBodyParts, long checkSum);
		...
		void WriteStripGroup(int stripGroupID, StripGroup_t *pStripGroup, int vertID, int indexID, int stripID);
		...
		float GetOrigVertBoneWeightValue(mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh, Vertex_t *pVert,
										 int boneID);
		mstudioboneweight_t &GetOrigVertBoneWeight(mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh,
												   Vertex_t *pVert);
		...
		void SortBonesWithinVertex(bool flexed, Vertex_t *vert, mstudiomodel_t *pStudioModel,
								   mstudiomesh_t *pStudioMesh, int *globalToHardwareBoneIndex,
								   int *hardwareToGlobalBoneIndex, int maxBonesPerTri, int maxBonesPerVert);
		...
		bool MeshNeedsRemoval(studiohdr_t *pHdr, mstudiomesh_t *pStudioMesh, LodScriptData_t &scriptLOD);
	...
	// Make sure all vertices we've added up to now use bones in the matrix list
	//-----------------------------------------------------------------------------
	void COptimizedModel::Stripify(VertexIndexList_t const &sourceIndices, bool isHWSkinned, int *pNumIndices,
								   unsigned short **ppIndices)
	{
		...
		*/
		...
		GenerateStrips(&sourceIndices[0], sourceIndices.Size(), &primGroups, &numPrimGroups);
	...
	//-----------------------------------------------------------------------------
	void COptimizedModel::BuildStripsRecursive(VertexIndexList_t &indices, TriangleList_t &tris, Triangle_t *triangle)
	{
		Assert(triangle);
	...
	//-----------------------------------------------------------------------------
	void COptimizedModel::BuildHWSkinnedStrips(TriangleList_t &triangles, VertexList_t &vertices,
	...
	//-----------------------------------------------------------------------------
	void COptimizedModel::BuildSWSkinnedStrips(TriangleList_t &triangles, VertexList_t const &vertices,
	...
	void COptimizedModel::ComputeStripGroupFlags(StripGroup_t *pStripGroup, bool isHWSkinned, bool isFlexed)
	{
		pStripGroup->flags = 0;
		if(isFlexed)
		{
...
#define MIN_BONE_INFLUENCE 1.0f
	...
	static void TryToReduceBoneInfluence(Vertex_t &stripGroupVert, mstudioboneweight_t const &boneWeights, int maxBones)
	{
		int i;
	...
	bool COptimizedModel::GenerateStripGroupVerticesFromFace(mstudioiface_t *pFace, mstudiomesh_t *pStudioMesh,
															 int maxPreferredBones, Vertex_t *pStripGroupVert)
	{
		int vertIDs[3];
		vertIDs[0] = pFace->a;
	...
	static void FindMatchingEdge(TriangleList_t &list, int nTriangleId, int nEdgeIndex, const int *pVertIds,
								 CUtlVector<int> &vertexToEdges, CUtlFixedLinkedList<EdgeInfo_t> &edges)
	{
	...
	void COptimizedModel::BuildNeighborInfo(TriangleList_t &list, int nMaxVertexId)
	{
		...
		for(int i = 0; i < nTriCount; ++i)
		{
			...
			pEdgeVertIds[0] = tri.vertID[0];
			pEdgeVertIds[1] = tri.vertID[1];
			FindMatchingEdge(list, i, 0, pEdgeVertIds, vertexToEdges, edges);

			pEdgeVertIds[0] = tri.vertID[1];
			pEdgeVertIds[1] = tri.vertID[2];
			FindMatchingEdge(list, i, 1, pEdgeVertIds, vertexToEdges, edges);

			pEdgeVertIds[0] = tri.vertID[2];
			pEdgeVertIds[1] = tri.vertID[0];
	...
	// Processes a single strip group
	//-----------------------------------------------------------------------------
	void COptimizedModel::ProcessStripGroup(StripGroup_t *pStripGroup, bool isHWSkinned, bool isFlexed,
											mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh,
											CUtlVector<mstudioiface_t> &srcFaces,
											TriangleProcessedList_t &trianglesProcessed, int maxBonesPerVert,
											int maxBonesPerTri, int maxBonesPerStrip, bool forceNoFlex, bool bHWFlex)
	{
		...
		for(int n = 0; n < srcFaces.Size(); ++n)
		{
			...
			bool triangleIsFlexed =
				GenerateStripGroupVerticesFromFace(pFace, pStudioMesh, preferredBones, stripGroupVert);
			...
			if(isHWSkinned)
			{
				...
				bool triangleIsHWSkinned = (numTriangleBones <= maxBonesPerTri) && (numVertexBones <= maxBonesPerVert);
...
#ifdef _DEBUG
				//	float GetOrigVertBoneWeightValue( mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh, Vertex_t
				//*pVert, int boneID );
				int i;
	...
	void COptimizedModel::ComputeMeshFlags(Mesh_t *pMesh, studiohdr_t *pStudioHeader, mstudiomesh_t *pStudioMesh)
	{
		// eyeball?
		pMesh->flags = 0;
									  ...
									  bool bForceSoftwareSkin, bool bHWFlex)
	{
		...
		for(isHWSkinned = bForceSoftwareSkin ? 0 : 1; isHWSkinned >= 0; --isHWSkinned)
		{
			for(isFlexed = 1; isFlexed >= 0; --isFlexed)
			{
				...
				ProcessStripGroup(&newStripGroup, isHWSkinned ? true : false, isFlexed ? true : false, pStudioModel,
								  pStudioMesh, srcFaces, trianglesProcessed, realMaxBonesPerVert, realMaxBonesPerTri,
								  realMaxBonesPerStrip, forceNoFlex, bHWFlex);
	...
	void COptimizedModel::SetupMeshProcessing(studiohdr_t *pHdr, int vertexCacheSize, bool usesFixedFunction,
...
//	mstudiovertex_t *meshLastVert = vertData->Vertex( pStudioMesh->numvertices - 1 );
	...
	bool COptimizedModel::MeshNeedsRemoval(studiohdr_t *pHdr, mstudiomesh_t *pStudioMesh, LodScriptData_t &scriptLOD)
	{
		mstudiotexture_t *ptexture;
	...
	// Process the entire model, return stats...
	//-----------------------------------------------------------------------------
	void COptimizedModel::ProcessModel(studiohdr_t *pHdr, s_bodypart_t *pSrcBodyParts, TotalMeshStats_t &stats,
									   bool bForceSoftwareSkin, bool bHWFlex)
	{
...
//					const char *pDebugName = pHdr->pTexture( textureSearchID )->pszName( );
						...
						if(pLODSource)
						{
							...
							CreateLODTriangleList(pSrcModel, lodID, pLODSource, pStudioModel, pStudioMesh,
												  meshTriangleList, false);
						}
						else
						{
					...
					for(int meshID = 0; meshID < pLOD->numMeshes; meshID++)
					{
						...
						for(int stripGroupID = 0; stripGroupID < mesh->numStripGroups; stripGroupID++)
						{
							...
							for(int stripID = 0; stripID < pStripGroup->numStrips; stripID++)
							{
								...
								for(boneStateChangeID = 0; boneStateChangeID < pStrip->numBoneStateChanges;
									boneStateChangeID++)
								{
									BoneStateChangeHeader_t *pBoneStateChange =
										pStrip->pBoneStateChange(boneStateChangeID);
									globalToHardwareBoneIndex[pBoneStateChange->newBoneID] =
										pBoneStateChange->hardwareID;
									if(pBoneStateChange->hardwareID != -1)
									{
										hardwareToGlobalBoneIndex[pBoneStateChange->hardwareID] =
											pBoneStateChange->newBoneID;
									}
								}
								int vertID;
	...
	void COptimizedModel::WriteHeader(int vertCacheSize, int maxBonesPerVert, int maxBonesPerTri, int maxBonesPerStrip,
									  int numBodyParts, long checkSum)
	{
...
//	StripGroupHeader_t *pStripGroup = debug->pStripGroup( 0 );
	...
	void COptimizedModel::WriteStripGroup(int stripGroupID, StripGroup_t *pStripGroup, int vertID, int indexID,
										  int stripID)
	{
...
//	BoneStateChangeHeader_t *debug = ( BoneStateChangeHeader_t * )m_FileBuffer->GetPointer( boneFileOffset );
	...
	void COptimizedModel::WriteMaterialReplacements(int materialReplacementsOffset)
	{
		...
		for(i = 0; i < numLODs; i++)
		{
			...
			for(j = 0; j < scriptLOD.materialReplacements.Size(); j++)
			{
				...
				tmpHeader.replacementMaterialNameOffset =
					m_StringTableOffset + s_StringTable.StringTableOffset(materialReplacement.GetDstName()) - offset;
	...
	void COptimizedModel::WriteMaterialReplacementLists(int materialReplacementsOffset,
	...
	void COptimizedModel::WriteVTXFile(studiohdr_t *pHdr, const char *pFileName, TotalMeshStats_t const &stats)
	{
		...
		m_StringTableOffset =
			m_BoneStageChangesOffset + sizeof(BoneStateChangeHeader_t) * stats.m_TotalBoneStateChanges;
					...
					for(int meshID = 0; meshID < pStudioModel->nummeshes; meshID++)
					{
						...
						for(int stripGroupID = 0; stripGroupID < pMesh->stripGroups.Count(); stripGroupID++)
						{
							...
							for(int stripID = 0; stripID < nStripCount; stripID++)
							{
								...
								for(int boneStateChangeID = 0; boneStateChangeID < pStrip->numBoneStateChanges;
									boneStateChangeID++)
								{
		...
		WriteHeader(m_VertexCacheSize, m_MaxBonesPerVert, m_MaxBonesPerTri, m_MaxBonesPerStrip, pHdr->numbodyparts,
					pHdr->checksum);
...
//	ShowStats();
	...
	bool COptimizedModel::OptimizeFromStudioHdr(studiohdr_t *pHdr, s_bodypart_t *pSrcBodyParts, int vertCacheSize,
												...
												int maxBonesPerVert, int maxBonesPerTri, int maxBonesPerStrip,
												const char *pFileName, const char *glViewFileName)
	{
		...
		SetupMeshProcessing(pHdr, vertCacheSize, usesFixedFunction, maxBonesPerVert, maxBonesPerTri, maxBonesPerStrip,
							pFileName);
	...
	Vector &COptimizedModel::GetOrigVertPosition(mstudiomodel_t *pStudioModel, mstudiomesh_t *pStudioMesh,
												 Vertex_t *pVert)
	{
	...
	mstudioboneweight_t &COptimizedModel::GetOrigVertBoneWeight(mstudiomodel_t *pStudioModel,
																mstudiomesh_t *pStudioMesh, Vertex_t *pVert)
	{
		Assert(pStudioMesh->pModel() == pStudioModel);
	...
	void COptimizedModel::SetFlexedAndSkinColor(unsigned int glViewFlags, unsigned int stripGroupFlags, Vector &color)
	{
		...
		{
			if((stripGroupFlags & STRIPGROUP_IS_FLEXED) && (stripGroupFlags & STRIPGROUP_IS_HWSKINNED))
			{
			...
			else if(!(stripGroupFlags & STRIPGROUP_IS_FLEXED) && (stripGroupFlags & STRIPGROUP_IS_HWSKINNED))
			{
	...
	void COptimizedModel::DrawGLViewTriangle(FILE *fp, Vector &pos1, Vector &pos2, Vector &pos3, Vector &color1,
											 Vector &color2, Vector &color3)
	{
		numGLViewTrangles++;
		fprintf(fp, "3\n");
	...
	void COptimizedModel::GLViewVert(FILE *fp, Vertex_t vert, int index, Vector &color, mstudiomodel_t *pStudioModel,
									 mstudiomesh_t *pStudioMesh, bool showSubStrips, float shrinkFactor)
	{
		...
		if(s_DrawMode == GLVIEWDRAW_TRILIST)
		{
			...
			if(id == 2)
			{
				...
				DrawGLViewTriangle(fp, s_LastThreePositions[0], s_LastThreePositions[1], s_LastThreePositions[2], color,
								   color, color);
			}
		}
		else
		{
			...
			if(s_ListID >= 2)
			{
				...
				if(s_LastThreeIndices[0] == s_LastThreeIndices[1] || s_LastThreeIndices[1] == s_LastThreeIndices[2] ||
				...
				{
					ShrinkVerts(shrinkFactor);
					DrawGLViewTriangle(fp, s_LastThreePositions[(id + 0 - 2 + 3) % 3],
									   s_LastThreePositions[(id + 1 - 2 + 3) % 3],
									   s_LastThreePositions[(id + 2 - 2 + 3) % 3], color, color, color);
				}
				else
				{
					ShrinkVerts(shrinkFactor);
					DrawGLViewTriangle(fp, s_LastThreePositions[(id + 2 - 2 + 3) % 3],
									   s_LastThreePositions[(id + 1 - 2 + 3) % 3],
									   s_LastThreePositions[(id + 0 - 2 + 3) % 3], color, color, color);
				}
			}
		}
						...
						for( int stripGroupID = 0; stripGroupID < mesh->numStripGroups; stripGroupID++ )
						{
							...
							for( int stripID = 0; stripID < pStripGroup->numStrips; stripID++ )
							{
								...
								for( int indexID = 0; indexID < pStrip->numIndices; indexID++ )
								{
									...
									fprintf( fp, "mesh: %04d origvertid: %04d pos: %0.2f %0.2f %0.2f ", meshID,
	vert.origMeshVertID, vertPos[0], vertPos[1], vertPos[2] ); int i; for( i = 0; i < vert.numBones; i++ )
									{
										float boneWeight;
	...
	*/
	...
	void COptimizedModel::WriteGLViewFile(studiohdr_t *phdr, const char *pFileName, unsigned int flags,
										  float shrinkFactor)
	{
		Vector color;
		RandomColor(color);
					...
					for(int meshID = 0; meshID < pLOD->numMeshes; meshID++)
					{
						...
						for(int stripGroupID = 0; stripGroupID < mesh->numStripGroups; stripGroupID++)
						{
							...
							for(int stripID = 0; stripID < pStripGroup->numStrips; stripID++)
							{
								...
								GLViewDrawBegin((pStrip->flags & STRIP_IS_TRILIST) ? GLVIEWDRAW_TRILIST
																				   : GLVIEWDRAW_TRISTRIP);
								for(int indexID = 0; indexID < pStrip->numIndices; indexID++)
								{
									int id = *pStripGroup->pIndex(indexID + pStrip->indexOffset);
	...
	void COptimizedModel::CheckVertBoneWeights(Vertex_t *pVert, mstudiomodel_t *pStudioModel,
								...
								for(int stripID = 0; stripID < pStripGroup->numStrips; stripID++)
								{
									...
									if(pStrip->flags & STRIP_IS_TRISTRIP)
									{
										for(int indexID = 0; indexID < pStrip->numIndices; indexID++)
										{
											...
											lastThreeIndices[newVertOffset] =
												*pStripGroup->pIndex(indexID + pStrip->indexOffset);
...
*/
							...
							for(int stripID = 0; stripID < pStripGroup->numStrips; stripID++)
							{
								...
								*/
								...
								for(int boneStateChangeID = 0; boneStateChangeID < pStrip->numBoneStateChanges;
									boneStateChangeID++)
								{
									BoneStateChangeHeader_t *boneStateChange =
										pStrip->pBoneStateChange(boneStateChangeID);
								...
								/*
															...
															for( i = 0; i < pStrip->numBoneStateChanges; i++ )
															{
																// hack . . going to stomp over what is already there
								   with new data. if( changed[i] )
																{
																	BoneStateChangeHeader_t *boneStateChange =
								   pStrip->pBoneStateChange( curOutBoneID ); boneStateChange->hardwareID = i;
								   ...
								   startNumBoneChanges, pStrip->numBoneStateChanges ); printf( "after optimization\n" );
															...
															{
																printf( "\thw: %d global: %d\n",
																		( int )pStrip->pBoneStateChange( i
								   )->hardwareID, ( int )pStrip->pBoneStateChange( i )->newBoneID );
															}
	...
	void WriteOptimizedFiles(studiohdr_t *phdr, s_bodypart_t *pSrcBodyParts)
	{
		...
		s_OptimizedModel.OptimizeFromStudioHdr(phdr, pSrcBodyParts, 24 /* vert cache size (real size, not effective!)*/,
											   false, /* doesn't use fixed function */
											   false, // don't force software skinning
											   false, // No hardware flex
											   3 /* bones/vert */, 9 /* bones/tri */, 16 /* bones/strip */, tmpFileName,
											   glViewFilename);
		...
		s_OptimizedModel.OptimizeFromStudioHdr(phdr, pSrcBodyParts, 24 /* vert cache size (real size, not effective!)*/,
											   ...
											   3 /* bones/vert */, 9 /* bones/tri */, 53 /* bones/strip */, tmpFileName,
											   glViewFilename);
