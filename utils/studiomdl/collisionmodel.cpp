...
public:
	...
	void AllowConcave(void)
	{
		m_allowConcave = true;
	}
	void AllowConcaveJoints()
	{
		m_allowConcaveJoints = true;
	}
	void Remove2DConvex()
	{
		m_remove2d = true;
	}
	void SetMaxConvex(int newMax)
	{
		m_maxConvex = newMax;
	}
	void Simplify();
...
void CJointedModel::AddConstraint(const char *pJointName, int axis, jointlimit_t jointType, float limitMin,
								  float limitMax, float friction)
{
	// In the editor/qc friction values are shown as 5X so 1.0 can be the default.
	CJointConstraint *pConstraint =
		new CJointConstraint(pJointName, axis, jointType, limitMin, limitMax, friction * (1.0f / 5.0f));
...
void ConvertToWorldSpace(CJointedModel &joints, s_source_t *psource, CUtlVector<Vector> &worldVerts)
{
	...
	else
	{
		...
		BuildRawTransforms(psource, "BindPose", 0, psource->scale, psource->adjust, psource->rotation, 0,
						   srcBoneToWorld);
	...
	while(pmodel)
	{
		...
		else
		{
			...
			if(nodeIndex >= 0)
			{
				...
				// The vertices are converted to the new reference by ConvertToWorldSpace(), as well as
