...
void StudioMdl_ScriptLoadedCallback(char const *pFilenameLoaded, char const *pIncludedFromFileName,
...
int lookupControl(char *string)
{
	...
	if(stricmp(string, "LZ") == 0)
		return STUDIO_LZ;
	if(stricmp(string, "LXR") == 0)
		return STUDIO_LXR;
	if(stricmp(string, "LYR") == 0)
		return STUDIO_LYR;
	if(stricmp(string, "LZR") == 0)
		return STUDIO_LZR;

	if(stricmp(string, "LM") == 0)
		return STUDIO_LINEAR;
	if(stricmp(string, "LQ") == 0)
		return STUDIO_QUADRATIC_MOTION;
...
void Build_Reference(s_source_t *pSource, const char *pAnimName)
{
	...
	for(i = 0; i < pSource->numbones; i++)
	{
		...
		// printf("%3d %f %f %f\n", i, psource->bonefixup[i].worldorg[0], psource->bonefixup[i].worldorg[1],
		// psource->bonefixup[i].worldorg[2] );
		/*
		AngleMatrix( angle, m );
...
//-----------------------------------------------------------------------------
void AddBodyFlexFetchRule(s_source_t *pSource, s_flexrule_t *pRule, int rawIndex,
...
// Add A Body Flex Rule
//-----------------------------------------------------------------------------
void AddBodyFlexRule(s_source_t *pSource, s_combinationrule_t &rule, int nFlexDesc,
					 const CUtlVector<int> &pRawIndexToRemapSourceIndex,
					 const CUtlVector<int> &pRawIndexToRemapLocalIndex,
					 const CUtlVector<int> &pRemapSourceIndexToGlobalFlexControllerIndex)
{
	if(g_numflexrules >= MAXSTUDIOFLEXRULES)
		MdlError("Line %d: Too many flex rules, max %d", g_iLinecount, MAXSTUDIOFLEXRULES);
	...
	if(nCombinationCount)
	{
		for(int j = 0; j < nCombinationCount; ++j)
		{
			...
			AddBodyFlexFetchRule(pSource, pRule, rule.m_Combination[j], pRawIndexToRemapSourceIndex,
								 pRawIndexToRemapLocalIndex, pRemapSourceIndexToGlobalFlexControllerIndex);
		}
	...
	for(int j = 0; j < nDominators; ++j)
	{
		...
		if(nFactorCount)
		{
			...
			{
				AddBodyFlexFetchRule(pSource, pRule, rule.m_Dominators[j][k], pRawIndexToRemapSourceIndex,
									 pRawIndexToRemapLocalIndex, pRemapSourceIndexToGlobalFlexControllerIndex);
			}
...
void AddBodyFlexRules(s_source_t *pSource)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		AddBodyFlexRule(pSource, rule, flexKey.flexdesc, pSource->m_rawIndexToRemapSourceIndex,
						pSource->m_rawIndexToRemapLocalIndex, pSource->m_leftRemapIndexToGlobalFlexControllIndex);
		if(flexKey.flexpair != 0)
		{
			AddBodyFlexRule(pSource, rule, flexKey.flexpair, pSource->m_rawIndexToRemapSourceIndex,
							pSource->m_rawIndexToRemapLocalIndex, pSource->m_rightRemapIndexToGlobalFlexControllIndex);
		}
	}
}
...
void AddBodyFlexData(s_source_t *pSource, int imodel)
{
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		if(g_numflexkeys >= MAXSTUDIOFLEXKEYS)
			MdlError("Line %d: Too many flex keys, max %d, cannot add flexKey %s from source %s", g_iLinecount,
					 MAXSTUDIOFLEXKEYS, key.animationname, pSource->filename);
...
bool s_attachment_t::operator==(const s_attachment_t &rhs) const
{
	...
	if(Q_stricmp(bonename, rhs.bonename) || bone != rhs.bone || type != rhs.type || flags != rhs.flags ||
	   Q_memcmp(local.Base(), rhs.local.Base(), sizeof(local)))
	{
		...
		MdlWarning("Attachments with the same name but different parameters found\n"
				   "  %s: ParentBone: %s Type: %d Flags: 0x%08x P: %6.2f %6.2f %6.2f R: %6.2f %6.2f %6.2f\n"
				   "  %s: ParentBone: %s Type: %d Flags: 0x%08x P: %6.2f %6.2f %6.2f R: %6.2f %6.2f %6.2f\n",
				   name, bonename, type, flags, iPos.x, iPos.y, iPos.z, RAD2DEG(iEuler.x), RAD2DEG(iEuler.y),
				   RAD2DEG(iEuler.z), rhs.name, rhs.bonename, rhs.type, rhs.flags, jPos.x, jPos.y, jPos.z,
				   RAD2DEG(jEuler.x), RAD2DEG(jEuler.y), RAD2DEG(jEuler.z));
	...
	for(int i = 0; i < pSource->m_Attachments.Count(); ++i)
	{
		...
		if(g_numattachments >= ARRAYSIZE(g_attachment))
		{
			MdlWarning("Too Many Attachments (Max %d), Ignoring Attachment %s:%s\n", ARRAYSIZE(g_attachment),
					   pSource->filename, pSource->m_Attachments[i].name);
			continue;
			;
		}
...
void ProcessOriginalContentFile(char const *szDataFile, char const *szOriginalContentFile)
{
	...
	if(szContentDirRootEnd && szSceneName)
	{
		...
		Q_snprintf(chScenePath, sizeof(chScenePath) - 1, "%.*s%s", szContentDirRootEnd - szDataFile, szDataFile,
				   szSceneName);
	...
	else if(szContentDirRootEnd && !szSceneName)
	{
		...
		Q_snprintf(chScenePath, sizeof(chScenePath) - 1, "%.*s%s",
				   max(strrchr(szDataFile, '\\'), strrchr(szDataFile, '/')) + 1 - szDataFile, szDataFile,
...
s_source_t *Load_Source(const char *name, const char *ext, bool reverse, bool isActiveModel)
{
	...
	// copy over default settings of when the model was loaded (since there's no actual animation for some of the
	// systems)
...
void Cmd_LimitRotation()
{
	...
	while(TokenAvailable())
	{
		...
		Assert(0);
		// V_strcpy_safe(
		// g_limitrotation[g_numlimitrotation].sequencename[g_limitrotation[g_numlimitrotation].numseq++], token );
	}
...
void Grab_QuatInterpBones()
{
	...
	while(GetLineInput())
	{
		...
		while(i == 4 && stricmp(cmd, "<aimconstraint>") == 0)
		{
			...
			// Grab_AimAtBones will read input into g_szLine same as here until it gets a line it doesn't understand, at
			// which point it will exit leaving that line in g_szLine, so check for the end and scan the current buffer
		...
		else if(i > 0)
		{
			...
			else if(stricmp(cmd, "<trigger>") == 0)
			{
				...
				QAngle rot;
				int j;
				i = sscanf(g_szLine, "<trigger> %f %f %f %f %f %f %f %f %f %f", &tolerance, &trigger.x, &trigger.y,
						   &trigger.z, &ang.x, &ang.y, &ang.z, &pos.x, &pos.y, &pos.z);
	...
	for(int i = 0; i < nCount; ++i)
	{
		...
		for(int j = 0; j < numblends; ++j)
		{
			s_sourceanim_t *pSourceAnim =
				FindSourceAnim(animations[j]->source, animationFiles[i]->m_SourceAnimationName);
			...
			if(pSourceAnim)
			{
				...
				Q_strncpy(animations[j]->animationname, animationFiles[i]->m_SourceAnimationName,
...
static bool CStudioMDLApp_SuggestGameInfoDirFn(CFSSteamSetupInfo const *pFsSteamSetupInfo, char *pchPathBuffer,
...
bool CStudioMDLApp::Create()
{
	...
	// Add the P4 module separately so that if it is absent (say in the SDK) then the other system will initialize
	// properly
...
int CStudioMDLApp::Main_MakeVsi()
{
	...
	*pExt = 0;
	...
	bool bResult = false;
	{
		...
		bResult = mdllib->StripModelBuffers(bufMDL, bufVVD, bufVTX, &pMdlStripInfo) &&
