...
static void WriteOBJ(const char *pszFilename, const s_source_t *pSource)
{
	...
	for(int i = 0; i < pSource->numbones; ++i)
	{
		if(pSource->localBone[i].parent >= 0)
		{
			fprintf(pFile, "# Bone %3d: %s Parent %3d: %s\n", i, pSource->localBone[i].name,
					pSource->localBone[i].parent, pSource->localBone[pSource->localBone[i].parent].name);
		}
		else
		{
	...
	for(int i = 0; i < pSource->nummeshes; ++i)
	{
		...
		for(int j = 0; j < m.numfaces; ++j)
		{
			...
			fprintf(pFile, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", f.a, f.a, f.a, f.b, f.b, f.b, f.c, f.c, f.c);
		}
	}
...
bool CCheckUVCmd::CheckNormalized(const struct s_source_t *pSource) const
{
	...
	for(int i = 0; i < pSource->numvertices; ++i)
	{
		...
		if(v.texcoord.x < 0.0f || v.texcoord.x > 1.0f || v.texcoord.y < 0.0f || v.texcoord.y > 1.0f)
		{
	...
	for(int i = 0; i < pSource->nummeshes; ++i)
	{
		...
		for(auto vIt = badMeshVertexIndices.FirstInorder(); badMeshVertexIndices.IsValidIndex(vIt);
			vIt = badMeshVertexIndices.NextInorder(vIt))
		{
...
bool CCheckUVCmd::CheckOverlap(const struct s_source_t *pSource) const
{
	...
	for(int i = 0; i < pSource->numfaces; ++i)
	{
		...
		for(int j = i + 1; j < pSource->numfaces; ++j)
		{
			...
			const Vector2D tB[] = {pSource->vertex[fB.a].texcoord, pSource->vertex[fB.b].texcoord,
...
{
	Msg("%sP: %8.4f %8.4f %8.4f T: %8.4f %8.4f\n", pszPrefix, v.position.x, v.position.y, v.position.z, v.texcoord.x,
		v.texcoord.y);
}
...
{
	Msg("%sP: %8.4f %8.4f %8.4f T: %8.4f %8.4f M: %s\n", pszPrefix, v.position.x, v.position.y, v.position.z,
		v.texcoord.x, v.texcoord.y, t.name);
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
void CCheckUVCmd::PrintFace(const s_source_t *pSource, const int nMesh, const int nFace,
