...
static void WriteSequenceInfo(studiohdr_t *phdr)
{
	...
	if(g_vecPreexistingSequences.Count())
	{
		...
		if(g_sequence.Count() < g_vecPreexistingSequences.Count() && !bAllowSequenceRemoval)
		{
			...
			{
				nInput = getchar();
			} while(nInput != 121 /* y */ && nInput != 110 /* n */);
				...
				else
				{
					if(i < g_vecPreexistingSequences.Count())
					{
						Msg("  Found new sequence \"%s\" using index of old sequence \"%s\".\n", g_sequence[i].name,
							g_vecPreexistingSequences[i].String());
					}
					else
					{
...
void WriteAnimationData(s_animation_t *srcanim, mstudioanimdesc_t *destanimdesc, byte *&pLocalData, byte *&pExtData)
{
	...
	for(int w = 0; w < srcanim->numsections; w++)
	{
		...
		for(j = 0; j < g_numbones; j++)
		{
			...
			{
				// printf("%d : %d %d %d : %d %d %d\n", j, psrcdata->num[0], psrcdata->num[1], psrcdata->num[2],
				// psrcdata->num[3], psrcdata->num[4], psrcdata->num[5] ); single frame, if animation detected just
				// store as raw
		...
		if(destanimdesc->sectionindex)
		{
			...
			// printf("%s (%d) : %d:%d\n", srcanim->name, w, destanimdesc->pSection(w)->animblock,
			// destanimdesc->pSection(w)->animindex );
		}
...
byte *WriteLocalHierarchy(s_animation_t *srcanim, byte *pData)
{
	...
	for(j = 0; j < srcanim->numlocalhierarchy; j++)
	{
		...
		for(k = 0; k < 6; k++)
		{
			if(srcanim->localhierarchy[j].localData.numanim[k])
				break;
		}
...
static byte *WriteAnimations(byte *pData, byte *pStart, studiohdr_t *phdr)
{
	...
	for(i = 0; i < g_numani; i++)
	{
		...
		// printf("%d %x %x %x   %s : %d\n", g_numanimblocks - 1, g_animblock[g_numanimblocks-1].start, pBlockData,
		// pBlockEnd, srcanim->name, srcanim->numsections );
	...
	if(!g_quiet)
	{
		...
		{
			printf("%2d (%3d:%3d): %d\n", i, g_animblock[i].iStartAnim, g_animblock[i].iEndAnim, g_animblock[i].end -
		g_animblock[i].start );
		}
		*/
	}
	...
	for(i = 0; i < g_numani; i++)
	{
		...
		if(panimdesc[i].animblock != 0)
		{
			...
			if(k > 2)
			{
				panimdesc[i].zeroframecount =
					min((panimdesc[i].numframes - 1) / panimdesc[i].zeroframespan, 3); // save frames 0..24 frames
			}
...
static void WriteModel(studiohdr_t *phdr)
{
	...
	for(j = 0; j < g_numflexcontrollers; ++j)
	{
		...
		for(k = 0; k < g_FlexControllerRemap.Count(); ++k)
		{
			...
			if(j == remap.m_Index || j == remap.m_LeftIndex || j == remap.m_RightIndex || j == remap.m_MultiIndex)
			{
				...
				if(pFlexControllerUI->stereo)
				{
					...
					pFlexControllerUI->szindex0 = (phdr->flexcontrollerindex - int(pData - pStart) +
					...
					pFlexControllerUI->szindex1 = (phdr->flexcontrollerindex - int(pData - pStart) +
				   ...
				   pFlexControllerUI->remaptype == FLEXCONTROLLER_REMAP_EYELID)
				{
					...
					pFlexControllerUI->szindex2 = (phdr->flexcontrollerindex - int(pData - pStart) +
		...
		if(!found)
		{
			...
			if(j < g_numflexcontrollers - 1 && StringAfterPrefixCaseSensitive(flexcontroller.name, "right_") &&
						 ...
						 StringAfterPrefixCaseSensitive(g_flexcontroller[j + 1].name, "left_")))
			{
				...
				pFlexControllerUI->szindex0 =
					(phdr->flexcontrollerindex - int(pData - pStart) + (j + 1) * sizeof(mstudioflexcontroller_t));
				pControllerHandled[j + 1] = true;

				Assert(!pControllerHandled[j]);
				pFlexControllerUI->szindex1 =
					(phdr->flexcontrollerindex - int(pData - pStart) + j * sizeof(mstudioflexcontroller_t));
				pControllerHandled[j] = true;
			}
			else if(j > 0 && StringAfterPrefixCaseSensitive(flexcontroller.name, "left_") &&
		...
		for(m = 0; m < pmodel[i].nummeshes; m++)
		{
			...
			if(pmesh[m].numflexes)
			{
				...
				for(j = 0; j < g_numflexkeys; j++)
				{
					...
					// printf("%d %d %s : %f %f %f %f\n", j, g_flexkey[j].flexdesc,
					// g_flexdesc[g_flexkey[j].flexdesc].FACS, g_flexkey[j].target0, g_flexkey[j].target1,
					// g_flexkey[j].target2, g_flexkey[j].target3 ); if (j < 9) printf("%d %d %s : %d (%d) %f\n", j,
					// g_flexkey[j].flexdesc, g_flexdesc[g_flexkey[j].flexdesc].FACS, g_flexkey[j].numvanims,
					// pflex->numverts, g_flexkey[j].target );
					...
					for(k = 0; k < g_flexkey[j].numvanims; k++)
					{
						...
						if(n >= 0 && n < pmesh[m].numvertices)
						{
							...
							if(bWrinkleVAnim)
							{
								((mstudiovertanim_wrinkle_t *)pvertanim)
									->SetWrinkleFixed(pvanim->wrinkle, flVertAnimFixedPointScale);
							}
	...
	for(i = 1; i < g_numanimblocks; i++)
	{
		...
		// printf("block %d : %x %x (%d)\n", i, panimblock[i].datastart, panimblock[i].dataend, panimblock[i].dataend -
		// panimblock[i].datastart );
	}
...
//-----------------------------------------------------------------------------
bool FixupVTXFile(const char *fileName, const studiohdr_t *pStudioHdr, const vertexPool_t *pVertexPools,
...
bool Clamp_VTX_LODS(const char *fileName, int rootLOD, studiohdr_t *pStudioHdr)
{
	...
	for(nLodID = rootLOD; nLodID < pVtxHdr->numLODs; nLodID++)
	{
		...
		OptimizedModel::MaterialReplacementListHeader_t *pNewReplacementList =
			pNewVtxHdr->pMaterialReplacementList(nLodID);
	...
	for(nLodID = 0; nLodID < rootLOD; nLodID++)
	{
		OptimizedModel::MaterialReplacementListHeader_t *pRootReplacementList =
			pNewVtxHdr->pMaterialReplacementList(rootLOD);
		OptimizedModel::MaterialReplacementListHeader_t *pNewReplacementList =
			pNewVtxHdr->pMaterialReplacementList(nLodID);
		...
		for(j = 0; j < pBodyPart->nummodels; ++j)
		{
			...
			for(nLodID = rootLOD; nLodID < pVtxModel->numLODs; nLodID++)
			{
				...
				for(k = 0; k < pModel->nummeshes; ++k)
				{
					...
					// pVtxMesh->numStripGroups, pVtxMesh->stripGroupHeaderOffset );
					...
					for(m = 0; m < pVtxMesh->numStripGroups; m++)
					{
						...
						// int delta = ((byte *)pStripGroup - (byte *)pVtxHdr) - ((byte *)pNewStripGroup - (byte
						// *)pNewVtxHdr);
						...
						*/
						...
						for(n = 0; n < pStripGroup->numStrips; n++)
						{
							...
							printf("\t\tnumBoneStateChanges %d %d :\n", pNewStrip->numBoneStateChanges,
							pNewStrip->boneStateChangeOffset );
							*/
							// printf("(%d)\n", delta );
						}
						// printf("(%d)\n", delta );
