...
int lookup_index(s_source_t *psource, int material, Vector &vertex, Vector &normal, Vector2D texcoord)
{
	...
	{
		if(v_listdata[i].m == material && DotProduct(g_normal[i], normal) > normal_blend &&
		   VectorCompare(g_vertex[i], vertex) && g_texcoord[i][0] == texcoord[0] && g_texcoord[i][1] == texcoord[1])
		{
...
void ParseFaceData(s_source_t *psource, int material, s_face_t *pFace)
{
	...
	for(j = 0; j < 3; j++)
	{
		...
		g_iLinecount++;
		i = sscanf(g_szLine, "%d %f %f %f %f %f %f %f %f %d %d %f %d %f %d %f %d %f", &bone, &p[0], &p[1], &p[2],
				   &normal[0], &normal[1], &normal[2], &t[0], &t[1], &iCount, &bones[0], &weights[0], &bones[1],
	...
	pFace->b = index[1];
	pFace->c = index[2];
	Assert(((pFace->a & 0xF0000000) == 0) && ((pFace->b & 0xF0000000) == 0) && ((pFace->c & 0xF0000000) == 0));
...
void BuildIndividualMeshes(s_source_t *psource)
{
	...
	for(i = 0; i < psource->numvertices; i++)
	{
		...
		// vprint( 0, "%4d : %2d :  %6.2f %6.2f %6.2f\n", i, psource->boneweight[i].bone[0], psource->vertex[i][0],
	...
	for (k = 0; k < MAXSTUDIOSKINS; k++)
	{
		vprint( 0, "%d : %d:%d %d:%d\n", k, psource->mesh[k].numvertices, psource->mesh[k].vertexoffset,
	psource->mesh[k].numfaces, psource->mesh[k].faceoffset );
	}
	*/
...
void CatBonePath(int *indexPath, s_bone_t *boneArray, M_matrix4x4_t &resultMatrix, int rootOffset = 0)
{
	...
	for(int i = l; i > -1; i--)
	{
		...
		// printf("pos: %f %f %f, rot: %f %f %f\n", thisBone->pos.x, thisBone->pos.y, thisBone->pos.z, thisBone->rot.x,
		// thisBone->rot.y, thisBone->rot.z);
	...
	// printf("pos: %f %f %f, rot: %f %f %f\n", resultBone.pos.x,resultBone.pos.y, resultBone.pos.z,
		...
		if(pNode.parent > -1)
		{
			...
			else
			{
				if(!g_bGaveMissingBoneWarning)
				{
					...
					Warning("Warning: Target skeleton has less bones than source animation. Reverting to source data "
							"for extra bones.\n");
				}
...
s_source_t *MotionMap(s_source_t *pSource, s_source_t *pTarget, s_template_t *pTemplate)
{
	...
	for(int t = 0; t < sourceNumFrames; t++)
	{
		...
		for(int ii = 0; ii < pTemplate->numIKSolves; ii++)
		{
			...
			if(g_verbose)
				printf("limb length %s: %i: %f, factor %f\n", thisJointNameString, thisJointIndex, thisLimbLength,
					   thisLimbLengthFactor);
			...
			M_AngleMatrix(pSource->rawanim[t][thisJointPathInRoot[0]].rot,
						  pSource->rawanim[t][thisJointPathInRoot[0]].pos, thisJointMat);
			M_AngleMatrix(pSource->rawanim[t][thisJointPathInRoot[1]].rot,
						  pSource->rawanim[t][thisJointPathInRoot[1]].pos, parentJointMat);
			...
			// law of cosines!
			float gParentCos = (thisLimbHypotLength * thisLimbHypotLength + parentJointLength * parentJointLength -
								thisJointLength * thisJointLength) /
							   (2 * parentJointLength * thisLimbHypotLength);
			float parentCos = (parentJointLength * parentJointLength + thisJointLength * thisJointLength -
		...
		for(int ii = 0; ii < pTemplate->numPlaneConstraints; ii++)
		{
			...
			if(thisJointGlobalMat[3][thisSolve->axis] < thisSolve->floor)
			{
				...
				{
					printf(
						"Error: Constraint parent has broken the plane, this frame's plane constraint unsolvable!\n");
				}
				else
				{
					Vector parentJointAtPlane(parentJointGlobalMat[3][0], parentJointGlobalMat[3][1],
											  parentJointGlobalMat[3][2]);
					Vector parentPos(parentJointGlobalMat[3][0], parentJointGlobalMat[3][1],
									 parentJointGlobalMat[3][2]);
					Vector thisJointAtPlane(thisJointGlobalMat[3][0], thisJointGlobalMat[3][1],
					...
					float adjacent =
						sqrtf((thisJointLength * thisJointLength) - (parentLengthToPlane * parentLengthToPlane));
					...
					Vector parentUp(parentJointGlobalMat[BONEUP][0], parentJointGlobalMat[BONEUP][1],
