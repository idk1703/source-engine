...
void CTFStatsApplication::DoAwards(CHTMLFile &MatchResultsPage)
{
	...
	// do scout award here.
	CSurvivalistAward csrva;
	csrva.report(MatchResultsPage);

	// sniper award
	CSharpshooterAward cssa;
	cssa.report(MatchResultsPage);

	// soldier award
	CRocketryAward cra;
	cra.report(MatchResultsPage);

	// demoman awards
	CGrenadierAward cga;
	cga.report(MatchResultsPage);
	CDemolitionsAward cda;
	cda.report(MatchResultsPage);

	// medic awards
	CCureAward cca;
	cca.report(MatchResultsPage);
	CBiologicalWarfareAward cbwa;
	cbwa.report(MatchResultsPage);

	// HW award
	CAssaultCannonAward caca;
	caca.report(MatchResultsPage);

	// pyro award
	CFlamethrowerAward cfa;
	cfa.report(MatchResultsPage);

	// spy award
	CKnifeAward cka;
	cka.report(MatchResultsPage);

	// engineer awards
	CBestSentryAward cbsa;
	cbsa.report(MatchResultsPage);
	CSentryRebuildAward cba2;
	cba2.report(MatchResultsPage);

	MatchResultsPage.hr(450, true);

	// non class specific
	CKamikazeAward ckami;
	ckami.report(MatchResultsPage);
	CTalkativeAward cta;
	cta.report(MatchResultsPage);
	CTeamKillAward ctka;
	ctka.report(MatchResultsPage);
