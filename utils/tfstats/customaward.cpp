...
CCustomAward *CCustomAward::readCustomAward(CTextFile &f)
{
	...
	while(token)
	{
		...
		else if(stricmp(token, "}") == 0)
		{
			...
			g_pApp->fatalError(
				"Unrecognized Award property name while parsing %s:  \"%s\" is not a property of an Award!",
				f.fileName().c_str(), token);
	...
	for(it = g_pMatchInfo->eventList()->begin(); it != g_pMatchInfo->eventList()->end(); it++)
	{
		...
		for(tli = triggers.begin(); tli != triggers.end(); ++tli)
		{
			if((*tli)->matches(*it))
			{
				...
				// if this returns -1, store based on name. (this way we can give awards to things other than player
				// names while remaining in this award/trigger hierarchy structure)
