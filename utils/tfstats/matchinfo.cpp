...
void CMatchInfo::generate()
{
	...
	for(it; it != plogfile->end(); ++it)
	{
		...
		switch(curr->getType())
		{
			...
			{
				// keep the pseudonym list updated
				players[curr->getArgument(0)->asPlayerGetPID()].nameFound(curr->getTime(),
																		  curr->getArgument(1)->asPlayerGetName());
			}
			break;
			...
			case CLogEvent::TEAM_FRAG:
			{
				...
				else if(curr->getType() == CLogEvent::FRAG)
				{
					...
					{
						// test to see if the previous event was a timer from the same player, and a kill, and with the
						// timer.
			...
			case CLogEvent::NAMED_GOAL_ACTIVATE:
			{
				// keep the pseudonym list updated
				players[curr->getArgument(0)->asPlayerGetPID()].nameFound(curr->getTime(),
																		  curr->getArgument(0)->asPlayerGetName());
			}
			break;
...
// Output:
//------------------------------------------------------------------------------------------------------
CMatchInfo::CMatchInfo(CEventList *plf) : numPlrs(0), logclosetime(0), plogfile(plf)
{
	teamnames[0] = "Blue";
	teamnames[1] = "Red";
	teamnames[2] = "Yellow";
