...
bool CTFStatsOSInterface::makeHier(string dir)
{
	...
	// have to parse out directories one at a time. because mkdir just can't handle making nested directories that don't
	// exist (just like md.) in otherwords, it's lame.
	...
	if(path[0] == '\\' && path[1] == '\\')
	{
		Util::debug_dir_printf(
			"Cannot make a directory on a remote machine.\nMap the share to a drive and specify that drive instead.\n");
		retval = false;
		goto end;
	}
	...
	do
	{
		if(this->chdir(nextDir) != 0)
		{
			...
			if(this->chdir(nextDir) != 0)
			{
				char buf[500];
				Util::debug_dir_printf(
					"Could not create directory (current directory is %s) failed on second attempt to change to %s\n",
					getcwd(buf, 500), nextDir);
				retval = false;
				goto end;
			}
...
void CTFStatsLinuxInterface::filemask2RegExp(char *buf)
{
	...
	while(*read)
	{
		...
		else if(*read == '.' || *read == '*' || *read == '?' || *read == '+' || *read == '(' || *read == ')' ||
...
int CTFStatsLinuxInterface::filenameCompare(dirent *file)
{
	...
	bool result =
		query_match((const char *)file->d_name, (const char *)(file->d_name + strlen(file->d_name)), what, expression);
