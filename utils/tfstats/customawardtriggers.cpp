...
CCustomAwardTrigger *CCustomAwardTrigger::readTrigger(CTextFile &f)
{
	...
	if(type == "broadcast")
		...
		g_pApp->fatalError("Invalid trigger type while parsing %s:\n\"%s\" is not a valid trigger type, please use "
						   "\"broadcast\", \"goal\"  or \"fullsearch\"",
						   f.fileName().c_str(), type);
...
#else
	if(winnerVar.at(0) == '%' && winnerVar.at(1) == 'p' && winnerVar.at(2) == 'l' && winnerVar.at(3) == 'r' &&
	   winnerVar.at(4) == '_')
#endif
	{
		map<string, string> varmatches;
