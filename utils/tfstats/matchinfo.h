...
public:
	CPlayerListIterator playerBegin()
	{
		return players.begin();
	}
	CPlayerListIterator playerEnd()
	{
		return players.end();
	}
...
public:
	...
	char *playerName(PID pid, char *out)
	{
		if(pid == 1)
			return "PID=-1!";
		strcpy(out, players[pid].name.c_str());
		return out;
	}
	string playerName(PID pid)
	{
		return pid == -1 ? string("PID=-1!") : players[pid].name;
	}
	...
	string teamName(int TID)
	{
		return teamnames[TID];
	}
	int teamID(string teamname);
	bool teamExists(int tid)
	{
		return team_exists[tid];
	}
