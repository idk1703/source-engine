...
void CDialogueReadout::writeHTML(CHTMLFile &html)
{
	...
	if(g_pApp->cmdLineSwitches["displaymm2"] == "on" || g_pApp->cmdLineSwitches["displaymm2"] == "yes" ||
	   g_pApp->cmdLineSwitches["displaymm2"] == "true" || g_pApp->cmdLineSwitches["displaymm2"] == "1")
		MM2Messages = true;
	...
	for(it = g_pMatchInfo->eventList()->begin(); it != g_pMatchInfo->eventList()->end(); ++it)
	{
		if((*it)->getType() == CLogEvent::SAY || (MM2Messages && ((*it)->getType() == CLogEvent::SAY_TEAM)))
		{
			...
			html.write("<tr><td><font class=%s%s>%s%s:</font><font color=white>  %s</font></tr></td>\n", aa, bb,
					   talkerName.c_str(), isTeamMsg ? " (Team)" : "", talked);
			html.br();
		}
		...
		else if((*it)->getType() == CLogEvent::TEAM_JOIN)
		{
			...
			if(firstJoin)
				html.write("<tr><td><font class=player%s>%s</font><font color=white> joined team <font "
						   "class=player%s>%s</font>.</font></tr></td>\n",
						   Util::teamcolormap[teamID], plrName.c_str(), Util::teamcolormap[teamID], teamName.c_str());
			else
				html.write("<tr><td><font class=player%s>%s</font><font color=white> changed teams to <font "
						   "class=player%s>%s</font>.</font></tr></td>\n",
						   Util::teamcolormap[oldTeamID], plrName.c_str(), Util::teamcolormap[teamID],
						   teamName.c_str());
			html.br();
		}
		else if((*it)->getType() == CLogEvent::FRAG)
		{
			...
			if(countKill)
			{
				html.write("<tr><td><font class=player%s>%s</font><font color=white> killed </font><font "
						   "class=player%s>%s</font><font color=white> with  %s. </font></tr></td>\n",
						   Util::teamcolormap[killerTeamID], killerName.c_str(), Util::teamcolormap[killeeTeamID],
						   killeeName.c_str(), weaponName.c_str());
				html.br();
			}
		}
		else if((*it)->getType() == CLogEvent::TEAM_FRAG)
		{
			...
			int killerTeamID = g_pMatchInfo->playerList()[killer].teams.atTime((*it)->getTime());
			int killeeTeamID = g_pMatchInfo->playerList()[killee].teams.atTime((*it)->getTime());
			html.write("<tr><td><font class=player%s>%s</font><font color=white> teamkilled </font><font "
					   "class=player%s>%s.</font></tr></td>\n",
					   Util::teamcolormap[killerTeamID], killerName.c_str(), Util::teamcolormap[killeeTeamID],
					   killeeName.c_str());
			html.br();
		}
