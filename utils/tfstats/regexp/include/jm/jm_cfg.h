...
public:
	...
	{
		if(0 != n)
			this->Allocator::deallocate((char *)p, n * sizeof(T));
	}

	pointer RE_CALL address(reference x) const
	{
		return &x;
	}
	const_pointer RE_CALL address(const_reference x) const
	{
		return &x;
	}
	static size_type RE_CALL max_size()
	{
		return -1;
	}
	static void RE_CALL construct(pointer p, const T &val)
	{
		jm_construct(p, val);
	}
	void RE_CALL destroy(pointer p)
	{
		jm_destroy(p);
	}

	const Allocator &RE_CALL instance() const
	{
		return *this;
	}
	...
	template<class U>
	RE_CALL re_alloc_binder(const re_alloc_binder<U, Allocator> &o) throw() : Allocator(o.instance())
	{
	}
#endif
};
...
template<class T, class Allocator>
inline re_alloc_binder<T, Allocator>::re_alloc_binder(const Allocator &i) : Allocator(i)
{
}
...
#define jm_assert(x)                                                                                      \
	if((x) == 0)                                                                                          \
	{                                                                                                     \
		cerr << "Assertion failed: " << #x << " in file " << __FILE__ << "and line " << __LINE__ << endl; \
		exit(-1);                                                                                         \
	}
#endif
#ifndef jm_trace
