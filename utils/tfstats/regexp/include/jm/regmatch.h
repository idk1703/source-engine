...
iterator RE_CALL re_is_set_member(iterator next, iterator last, re_set_long *set,
...
bool query_match(iterator first, iterator last, reg_match<iterator, Allocator> &m,
...
inline bool query_match(const charT *str, reg_match<const charT *, Allocator> &m,
						const reg_expression<charT, traits, Allocator2> &e, unsigned flags = match_default)
{
...
inline bool query_match(const char *str, cmatch &m, const regex &e, unsigned flags = match_default)
{
	return query_match(str, str + regex::traits_type::length(str), m, e, flags);
}
#ifndef JM_NO_WCSTRING
inline bool query_match(const wchar_t *str, wcmatch &m, const wregex &e, unsigned flags = match_default)
{
	return query_match(str, str + wregex::traits_type::length(str), m, e, flags);
}
#endif
#ifndef JM_NO_STRING_H
inline bool query_match(const __JM_STD::string &s, reg_match<__JM_STD::string::const_iterator, regex::alloc_type> &m,
						const regex &e, unsigned flags = match_default)
{
	return query_match(s.begin(), s.end(), m, e, flags);
}
...
inline bool query_match(const __JM_STD::basic_string<wchar_t> &s,
						...
						const wregex &e, unsigned flags = match_default)
{
	return query_match(s.begin(), s.end(), m, e, flags);
}
#endif
...
namespace
{
	...
	template<class iterator, class Allocator, class charT, class traits, class Allocator2>
	bool query_match_aux(iterator first, iterator last, reg_match<iterator, Allocator> &m,
						 const reg_expression<charT, traits, Allocator2> &e, unsigned flags,
						 __priv_match_data<iterator, Allocator> &pd, iterator *restart)
	{
		...
		while(first != last)
		{
			...
			switch(ptr->type)
			{
				...
				case syntax_element_literal:
				{
					...
					{
						if((first == last) ||
						   (traits::translate(*first, icase MAYBE_PASS_LOCALE(locale_inst)) != what[i]))
							goto failure;
					}
					ptr = ptr->next.p;
					break;
				...
				case syntax_element_backref:
				{
					...
					while(i != j)
					{
						if((first == last) || (traits::translate(*first, icase MAYBE_PASS_LOCALE(locale_inst)) !=
											   traits::translate(*i, icase MAYBE_PASS_LOCALE(locale_inst))))
							goto failure;
						++i;
						++first;
					}
				...
				case syntax_element_set:
					...
					if(((re_set *)ptr)
						   ->__map[(traits_uchar_type)traits::translate(*first, icase MAYBE_PASS_LOCALE(locale_inst))])
					{
						ptr = ptr->next.p;
						++first;
						continue;
				...
				{
					// alt_jump:
					if(reg_expression<charT, traits, Allocator2>::can_start(*first, ((re_jump *)ptr)->__map,
																			(unsigned char)mask_take, width_type()))
					{
						// we can take the first alternative,
						// see if we need to push next alternative:
						if(reg_expression<charT, traits, Allocator2>::can_start(*first, ((re_jump *)ptr)->__map,
																				mask_skip, width_type()))
						{
							if(need_push_match)
					...
					if(reg_expression<charT, traits, Allocator2>::can_start(*first, ((re_jump *)ptr)->__map, mask_skip,
																			width_type()))
					{
						ptr = ((re_jump *)ptr)->alt.p;
						continue;
				...
				case syntax_element_rep:
				{
					...
					if(((re_repeat *)ptr)->alt.p->type == syntax_element_match)
					{
						...
						if(((unsigned int)accumulators[cur_acc] < ((re_repeat *)ptr)->max) &&
						   reg_expression<charT, traits, Allocator2>::can_start(*first, ((re_repeat *)ptr)->__map,
																				mask_take, width_type()))
						{
				...
				case syntax_element_combining:
					...
					++first;
					while((first != last) &&
						  traits::is_combining(traits::translate(*first, icase MAYBE_PASS_LOCALE(locale_inst))))
						++first;
					ptr = ptr->next.p;
					continue;
				case syntax_element_soft_buffer_end:
				{
					...
					iterator p(first);
					while((p != last) &&
						  traits::is_separator(traits::translate(*first, icase MAYBE_PASS_LOCALE(locale_inst))))
						++p;
					if(p != last)
						goto failure;
					ptr = ptr->next.p;
...
template<class iterator, class Allocator>
inline const reg_match_base<iterator, Allocator> &grep_out_type(const grep_search_predicate<iterator, Allocator> &o,
																const Allocator &)
{
	return *(o.pm);
}
...
namespace
{
	...
	unsigned int reg_grep2(Predicate foo, I first, I last, const reg_expression<charT, traits, A> &e, unsigned flags,
						   A2 a)
	{
		if(e.flags() & regbase::failbit)
			return 0;
		...
		switch(type)
		{
			...
			case regbase::restart_any:
			{
				while(first != last)
				{
					if(reg_expression<charT, traits, A>::can_start(*first, __map, (unsigned char)mask_any,
																   width_type()))
					{
						if(query_match_aux(first, last, m, e, flags, pd, &restart))
						{
							...
							if(first == m[0].second)
							{
								...
								{
									need_init = false;
									for(unsigned int i = 0; (restart != first) && (i < e.leading_length());
										++i, --restart)
										;
									if(restart != last)
										++restart;
			...
			case regbase::restart_word:
			{
				...
				while(first != last)
				{
					...
					while((first != last) &&
						  (traits::is_class(*first, char_class_word MAYBE_PASS_LOCALE(locale_inst)) == false))
						__inc_one(clines, last_line, first);
					if(first == last)
						break;
...
template<class iterator, class Allocator, class charT, class traits, class Allocator2>
bool reg_search(iterator first, iterator last, reg_match<iterator, Allocator> &m,
				const reg_expression<charT, traits, Allocator2> &e, unsigned flags = match_default)
{
	if(e.flags() & regbase::failbit)
		return false;
...
// then it really won't support this either:
template<class charT, class Allocator, class traits, class Allocator2>
inline bool reg_search(const charT *str, reg_match<const charT *, Allocator> &m,
					   const reg_expression<charT, traits, Allocator2> &e, unsigned flags = match_default)
{
...
#endif
#else // partial specialisation
inline bool reg_search(const char *str, cmatch &m, const regex &e, unsigned flags = match_default)
{
	return reg_search(str, str + regex::traits_type::length(str), m, e, flags);
}
#ifndef JM_NO_WCSTRING
inline bool reg_search(const wchar_t *str, wcmatch &m, const wregex &e, unsigned flags = match_default)
{
	return reg_search(str, str + wregex::traits_type::length(str), m, e, flags);
}
#endif
#ifndef JM_NO_STRING_H
inline bool reg_search(const __JM_STD::string &s, reg_match<__JM_STD::string::const_iterator, regex::alloc_type> &m,
					   const regex &e, unsigned flags = match_default)
{
	return reg_search(s.begin(), s.end(), m, e, flags);
}
...
inline unsigned int reg_grep(Predicate foo, iterator first, iterator last,
...
inline bool reg_grep(Predicate foo, const charT *str, const reg_expression<charT, traits, Allocator> &e,
...
#endif
#else // partial specialisation
inline bool reg_grep(bool (*foo)(const cmatch &), const char *str, const regex &e, unsigned flags = match_default)
{
	return reg_grep(foo, str, str + regex::traits_type::length(str), e, flags);
}
#ifndef JM_NO_WCSTRING
inline bool reg_grep(bool (*foo)(const wcmatch &), const wchar_t *str, const wregex &e, unsigned flags = match_default)
{
	return reg_grep(foo, str, str + wregex::traits_type::length(str), e, flags);
}
#endif
#ifndef JM_NO_STRING_H
inline bool reg_grep(bool (*foo)(const reg_match<__JM_STD::string::const_iterator, regex::alloc_type> &),
					 const __JM_STD::string &s, const regex &e, unsigned flags = match_default)
{
	return reg_grep(foo, s.begin(), s.end(), e, flags);
}
#if !defined(JM_NO_STRING_DEF_ARGS) && !defined(JM_NO_WCSTRING)
inline bool reg_grep(
	bool (*foo)(const reg_match<__JM_STD::basic_string<wchar_t>::const_iterator, wregex::alloc_type> &),
	const __JM_STD::basic_string<wchar_t> &s, const wregex &e, unsigned flags = match_default)
{
	return reg_grep(foo, s.begin(), s.end(), e, flags);
}
#endif
...
struct grep_match
{
	...
	grep_match(unsigned int l, iterator p1, const reg_match<iterator, Allocator> &m) : what(m)
	{
		line = l;
		line_start = p1;
	}
...
inline unsigned int reg_grep_old(Out oi, iterator first, iterator last,
								 const reg_expression<charT, traits, Allocator> &e, unsigned flags = match_default)
{
