...
namespace
{
	...
	inline bool RE_CALL reg_expression<charT, traits, Allocator>::can_start(charT c, const unsigned char *__map,
																			unsigned char mask, const __wide_type &)
	{
	...
	template<class charT, class traits, class Allocator>
	inline bool RE_CALL reg_expression<charT, traits, Allocator>::can_start(charT c, const unsigned char *__map,
																			unsigned char mask, const __narrow_type &)
	{
		return JM_MAKE_BOOL(__map[(traits_uchar_type)c] & mask);
	}
	...
	template<class charT, class traits, class Allocator>
	CONSTRUCTOR_INLINE reg_expression<charT, traits, Allocator>::reg_expression(const charT *p, jm_uintfast32_t f,
																				const Allocator &a)
		: data(a), pkmp(0)
	{
		set_expression(p, f);
	...
	template<class charT, class traits, class Allocator>
	CONSTRUCTOR_INLINE reg_expression<charT, traits, Allocator>::reg_expression(const charT *p1, const charT *p2,
																				jm_uintfast32_t f, const Allocator &a)
		: data(a), pkmp(0)
	{
	...
	template<class charT, class traits, class Allocator>
	CONSTRUCTOR_INLINE reg_expression<charT, traits, Allocator>::reg_expression(const charT *p, size_type len,
																				jm_uintfast32_t f, const Allocator &a)
		: data(a), pkmp(0)
	{
	...
	template<class charT, class traits, class Allocator>
	reg_expression<charT, traits, Allocator> &RE_CALL
	reg_expression<charT, traits, Allocator>::operator=(const reg_expression<charT, traits, Allocator> &e)
	{
		//
		// we do a deep copy only if e is a valid expression, otherwise fail.
		//
	...
	template<class charT, class traits, class Allocator>
	unsigned int RE_CALL reg_expression<charT, traits, Allocator>::parse_inner_set(const charT *&first,
																				   const charT *last)
	{
		//
		// we have an inner [...] construct
		//
		jm_assert(traits_type::syntax_type(
					  (traits_size_type)(traits_uchar_type)*first MAYBE_PASS_LOCALE(locale_inst)) == syntax_open_set);
		const charT *base = first;
		while((first != last) &&
			  (traits_type::syntax_type((traits_size_type)(traits_uchar_type)*first MAYBE_PASS_LOCALE(locale_inst)) !=
			   syntax_close_set))
			++first;
		if(first == last)
			return 0;
		...
		if(*(base + 1) != *(first - 2))
			return 0;
		unsigned int result =
			traits_type::syntax_type((traits_size_type)(traits_uchar_type) * (base + 1) MAYBE_PASS_LOCALE(locale_inst));
		if((result == syntax_colon) && ((first - base) == 5))
		{
			return traits_type::syntax_type((traits_size_type)(traits_uchar_type) * (base + 2)
																						MAYBE_PASS_LOCALE(locale_inst));
		}
	...
	bool RE_CALL reg_expression<charT, traits, Allocator>::skip_space(const charT *&first, const charT *last)
	{
		...
		//
		while((first != last) &&
			  (traits_type::is_class(*first, char_class_space MAYBE_PASS_LOCALE(locale_inst)) == true))
		{
			++first;
		}
		return first == last;
	...
	template<class charT, class traits, class Allocator>
	void RE_CALL reg_expression<charT, traits, Allocator>::parse_range(const charT *&ptr, const charT *end,
																	   unsigned &min, unsigned &max)
	{
		...
		if(traits_type::syntax_type((traits_size_type)(traits_uchar_type)*ptr MAYBE_PASS_LOCALE(locale_inst)) !=
		   syntax_digit)
		{
			fail(REG_BADBR);
			return;
		...
		if(_flags & bk_braces)
		{
			...
			else
			{
				...
				++ptr;
				if((ptr == end) ||
				   (traits_type::syntax_type((traits_size_type)(traits_uchar_type)*ptr MAYBE_PASS_LOCALE(
						locale_inst)) != syntax_close_brace))
				{
					fail(REG_BADBR);
					return;
				}
		...
		else if(traits_type::syntax_type((traits_size_type)(traits_uchar_type)*ptr MAYBE_PASS_LOCALE(locale_inst)) !=
				syntax_close_brace)
		{
			fail(REG_BADBR);
			return;
		...
		switch(traits_type::syntax_type(*first MAYBE_PASS_LOCALE(locale_inst)))
		{
			...
			case syntax_x:
				...
				if(traits_type::syntax_type(*first MAYBE_PASS_LOCALE(locale_inst)) == syntax_open_brace)
				{
					...
					if((first == last) ||
					   (traits_type::syntax_type(*first MAYBE_PASS_LOCALE(locale_inst)) != syntax_close_brace))
					{
						fail(REG_BADBR);
					}
					++first;
	...
	void RE_CALL reg_expression<charT, traits, Allocator>::compile_maps()
	{
		...
		while(record->type != syntax_element_match)
		{
			if((record->type == syntax_element_alt) || (record->type == syntax_element_rep))
			{
				...
				record->can_be_null = 0;
				compile_map(record->next.p, ((re_jump *)record)->__map, &(record->can_be_null), mask_take,
							((re_jump *)record)->alt.p);
	...
	bool RE_CALL re_maybe_set_member(charT c, re_set_long *set, const reg_expression<charT, traits_type, Allocator> &e)
	{
	...
	template<class charT, class traits, class Allocator>
	bool RE_CALL reg_expression<charT, traits, Allocator>::probe_start(re_syntax_base *node, charT cc,
																	   re_syntax_base *terminal) const
	{
		unsigned int c;
		...
		switch(node->type)
		{
			...
			case syntax_element_literal:
				...
				if(*(charT *)(((re_literal *)node) + 1) ==
				   traits_type::translate(cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst)))
					return true;
				return false;
			case syntax_element_end_line:
				// next character (if there is one!) must be a newline:
				if(traits_type::is_separator(
					   traits_type::translate(cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst))))
					return true;
				return false;
			...
			case syntax_element_word_start:
				return traits_type::is_class(
					traits_type::translate(cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst)),
					char_class_word MAYBE_PASS_LOCALE(locale_inst));
			...
			case syntax_element_soft_buffer_end:
				...
				// syntax_element_match, if its not then we can
				// never match anything anyway!!
				return traits_type::is_separator(
					traits_type::translate(cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst)));
			...
			case syntax_element_long_set:
				...
				return re_maybe_set_member(cc, (re_set_long *)node, *this) ||
					   re_is_set_member((const charT *)&cc, (const charT *)(&cc + 1), (re_set_long *)node, *this) !=
						   &cc;
			case syntax_element_set:
				// set all the elements that are set in corresponding set:
				c = (traits_size_type)(traits_uchar_type)traits_type::translate(cc, (_flags & regbase::icase)
																						MAYBE_PASS_LOCALE(locale_inst));
			...
			case syntax_element_rep:
				...
				if(((re_repeat *)node)->min == 0)
					return probe_start(node->next.p, cc, ((re_jump *)node)->alt.p) ||
						   probe_start(((re_jump *)node)->alt.p, cc, terminal);
				else
					return probe_start(node->next.p, cc, ((re_jump *)node)->alt.p);
			case syntax_element_combining:
				return !traits_type::is_combining(
					traits_type::translate(cc, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst)));
		}
		return false;
	}
	...
	template<class charT, class traits, class Allocator>
	bool RE_CALL reg_expression<charT, traits, Allocator>::probe_start_null(re_syntax_base *node,
																			re_syntax_base *terminal) const
	{
		switch(node->type)
		{
	...
	template<class charT, class traits, class Allocator>
	void RE_CALL reg_expression<charT, traits, Allocator>::compile_map(re_syntax_base *node, unsigned char *__map,
	...
	template<class charT, class traits, class Allocator>
	re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::compile_set_simple(re_syntax_base *dat,
																						 unsigned long cls, bool isnot)
	{
	...
	template<class charT, class traits, class Allocator>
	re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::compile_set(const charT *&first,
																				  const charT *last)
	{
		jstack<re_str<charT>, Allocator> singles(64, data.allocator());
		jstack<re_str<charT>, Allocator> ranges(64, data.allocator());
		...
		while((first != last) && !done)
		{
			...
			switch(traits_type::syntax_type(c MAYBE_PASS_LOCALE(locale_inst)))
			{
				...
				case syntax_open_set:
				{
					...
					switch(parse_inner_set(first, last))
					{
						case syntax_colon:
						{
							...
							jm_uintfast32_t id =
								traits_type::lookup_classname(base + 2, first - 2 MAYBE_PASS_LOCALE(locale_inst));
						...
						case syntax_equal:
							...
							if(traits_type::lookup_collatename(s, base + 2, first - 2 MAYBE_PASS_LOCALE(locale_inst)))
							{
								...
								while(s[i])
								{
									s[i] = traits_type::translate(s[i], (_flags & regbase::icase)
																			MAYBE_PASS_LOCALE(locale_inst));
									++i;
								}
								re_str<charT> s2;
						...
						case syntax_left_word:
							if((started == false) &&
							   (traits_type::syntax_type((traits_size_type)(traits_uchar_type)*first MAYBE_PASS_LOCALE(
									locale_inst)) == syntax_close_set))
							{
								++first;
						...
						default:
							if(started == false)
							{
								unsigned int t = traits_type::syntax_type((traits_size_type)(traits_uchar_type) *
																		  (base + 1) MAYBE_PASS_LOCALE(locale_inst));
				...
				case syntax_dash:
					...
					++first;
					if(traits_type::syntax_type((traits_size_type)(traits_uchar_type)*first MAYBE_PASS_LOCALE(
						   locale_inst)) == syntax_close_set)
					{
						--first;
						s = (charT)c;
	...
	template<class charT, class traits, class Allocator>
	re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::compile_set_aux(
		jstack<re_str<charT>, Allocator> &singles, jstack<re_str<charT>, Allocator> &ranges,
		jstack<jm_uintfast32_t, Allocator> &classes, jstack<re_str<charT>, Allocator> &equivalents, bool isnot,
		const __wide_type &)
	{
...
// delay throw to later:
		...
		if(flags)
		{
			for(unsigned int i = 0; i < 256; ++i)
			{
				if(traits_type::is_class(charT(i), flags MAYBE_PASS_LOCALE(locale_inst)))
					dat->__map[(traits_uchar_type)traits_type::translate(
						(charT)i, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst))] = mask_all;
			}
		}
	...
	template<class charT, class traits, class Allocator>
	unsigned int RE_CALL reg_expression<charT, traits, Allocator>::set_expression(const charT *p, const charT *end,
																				  jm_uintfast32_t f)
	{
		if(p == expression())
		{
	...
	JM_USE_FACET(locale_inst, regfacet<charT>).update(locale_inst);
		...
		if(_flags & regbase::literal)
		{
			...
			{
				dat =
					add_literal(dat, traits::translate(*ptr, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst)));
				++ptr;
			}
		}
					...
					switch(traits_type::syntax_type(c MAYBE_PASS_LOCALE(locale_inst)))
					{
						...
						case syntax_Q:
							...
							while(true)
							{
								...
								if(traits_type::syntax_type((traits_size_type)(traits_uchar_type)*ptr MAYBE_PASS_LOCALE(
									   locale_inst)) == syntax_slash)
								{
									++ptr;
									if((ptr != end) && (traits_type::syntax_type(
															(traits_size_type)(traits_uchar_type)*ptr MAYBE_PASS_LOCALE(
																locale_inst)) == syntax_E))
										break;
									else
									{
				...
				repeat_jump:
				{
					...
					switch(dat->type)
					{
						...
						case syntax_element_literal:
							...
							{
								// update previous:
								charT lit = *(charT *)((char *)dat + sizeof(re_literal) +
													   ((((re_literal *)dat)->length - 1) * sizeof(charT)));
		...
		*(_expression + (end - base)) = charT(0);
		...
		if((sbase->type == syntax_element_literal) && (sbase->next.p->type == syntax_element_match))
		{
			...
			if(0 == pkmp)
			{
				...
				pkmp = kmp_compile(p1, p2, charT(), kmp_translator<traits>(_flags & regbase::icase),
								   data.allocator() MAYBE_PASS_LOCALE(locale_inst));
			}
		}
		return code;
	}
	...
	template<class charT, class traits, class Allocator>
	re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::add_simple(re_syntax_base *dat,
	...
	re_syntax_base *RE_CALL reg_expression<charT, traits, Allocator>::add_literal(re_syntax_base *dat, charT c)
	{
		if(dat && (dat->type == syntax_element_literal))
		{
			...
			*(charT *)data.extend(sizeof(charT)) =
				traits::translate(c, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst));
	...
	template<class charT, class traits, class Allocator>
	unsigned int RE_CALL reg_expression<charT, traits, Allocator>::fixup_leading_rep(re_syntax_base *dat,
																					 re_syntax_base *end)
	{
		unsigned int len = 0;
