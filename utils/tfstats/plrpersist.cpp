...
void CPlrPersist::merge(CPlrPersist &other, bool mergeOverlaps)
{
	...
	else
	{
		...
		for(itOther; itOther != other.playtimes.end(); ++itOther)
		{
			...
			{
				g_pApp->warning(
					"not merging stats for WON ID# %lu, playtime ranges overlap\n\t((%lu-%lu) overlaps with (%lu-%lu))",
					WONID, itOther->first, itOther->second, overlap->first, overlap->second);
				return;
			}
		}
	}
...
void CPlrPersist::write()
{
	...
	for(it; it != nickmap.end(); ++it)
	{
		...
		fprintf(fout, "\t\"%s\" %li //has used the name \"%s\" for %02li:%02li:%02li\n", name.c_str(), time,
				name.c_str(), Util::time_t2hours(time), Util::time_t2mins(time), Util::time_t2secs(time));
	}
	...
	for(it; it != classmap.end(); ++it)
	{
		string name = it->first;
		int time = it->second;
		fprintf(fout, "\t\"%s\" %li //has played as a \"%s\" for %02li:%02li:%02li\n", name.c_str(), time, name.c_str(),
				Util::time_t2hours(time), Util::time_t2mins(time), Util::time_t2secs(time));
	}
	...
	for(it2; it2 != playtimes.end(); ++it2)
	{
		...
		fprintf(fout, "\t%lu %lu //played from %s.", it2->first, it2->second,
				Util::makeDurationString(it2->first, it2->second, buf, " to "));
		if(doesOverlap)
