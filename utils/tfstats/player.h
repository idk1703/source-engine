...
public:
	struct plr_per_team_data
	{
		...
		plr_per_team_data()
		{
			kills = deaths = suicides = teamkills = teamkilled = faveweapkills = 0;
		}

		CTimeIndexedList<player_class>
			classesplayed; // stores class, indexed by the time when that class was switched to.
	...
	CTimeIndexedList<player_class>
		allclassesplayed; // stores class, indexed by the time when that class was switched to.
