...
void CTFStatsReport::genJavaScript()
{
	...
	if(!f)
		g_pApp->fatalError("Can't open output file \"%s\"! (reason: %s)\nPlease make sure that the file does not exist "
						   "OR\nif the file does exit, make sure it is not read-only",
						   jsname.c_str(), strerror(errno));
	else
...
void CTFStatsReport::genIndex()
{
	...
	html.write(
		"	document.write(\"<div class=whitetext align = left>  is not compatible with TFStats </font></div>\");\n");
	html.write("	document.write(\"<div class=whitetext align = left> TFStats supports IE5, IE4 and Netscape "
			   "4</font></div>\");\n");
	html.write("	document.write(\"<div class=whitetext align = left> <a  target=_top "
			   "href=\\\"http://www.microsoft.com/windows/ie/download/ie5all.htm\\\">Download IE5</a></div>\");\n");
	html.write(
		"	document.write(\"<div class=whitetext align = left> <a  target=_top "
		"href=\\\"http://www.netscape.com/computing/download/index.html\\\">Download Netscape 4</a></div>\");\n");
	html.write("}\n");
	html.write("//-->\n");
	html.write("</script>\n");
...
void CTFStatsReport::genNavFrame()
{
	...
	html.write("<script language=\"JavaScript\">\n");
	html.write("<!--\n");
	html.write("MatchStatsImgLink = new "
			   "SuperImage(\"MatchStats\",\"%s/match.statistics.on.gif\",\"%s/match.statistics.off.gif\");\n",
			   g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
	html.write("DialoguePageImgLink = new "
			   "SuperImage(\"DialoguePage\",\"%s/game.dialog.on.gif\",\"%s/game.dialog.off.gif\");\n",
			   g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
	html.write("ServerSettingsImgLink = new "
			   "SuperImage(\"ServerSettings\",\"%s/server.settings.on.gif\",\"%s/server.settings.off.gif\");\n",
			   g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
	html.write("PlayerStatsMatchImgLink = new "
			   "SuperImage(\"PlayerStatsMatch\",\"%s/player.statistics.match.on.gif\",\"%s/"
			   "player.statistics.match.off.gif\");\n",
			   g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
	html.write("PlayerStatsServerImgLink = new "
			   "SuperImage(\"PlayerStatsServer\",\"%s/player.statistics.server.on.gif\",\"%s/"
			   "player.statistics.server.off.gif\");\n",
			   g_pApp->supportHTTPPath.c_str(), g_pApp->supportHTTPPath.c_str());
	...
	html.write("\t\t\t<p><a href=\"MatchResults.html\" target=\"data\"");
	writeEventHandlers(html, "MatchStats");
	html.write("><img name=\"MatchStats\"src=\"%s/match.statistics.off.gif\" border=0></a>\n",
			   g_pApp->supportHTTPPath.c_str());
	html.write("\t\t\t<p><a href=\"players.html\" target=\"data\"");
	writeEventHandlers(html, "PlayerStatsMatch");
	html.write("><img name=\"PlayerStatsMatch\" src=\"%s/player.statistics.match.off.gif\" border=0></a>\n",
			   g_pApp->supportHTTPPath.c_str());
	html.write("\t\t\t<p><a href=\"dialogue.html\" target=\"data\"");
	writeEventHandlers(html, "DialoguePage");
	html.write("><img name=\"DialoguePage\" src=\"%s/game.dialog.off.gif\" border=0></a>\n",
			   g_pApp->supportHTTPPath.c_str());
	...
	if(g_pApp->cmdLineSwitches["persistplayerstats"] == "yes" && !g_pMatchInfo->isLanGame())
	{
		...
		s += "/allplayers.html";
		html.write("\t\t\t<p><a href=\"%s\" target=\"data\"", s.c_str());
		writeEventHandlers(html, "PlayerStatsServer");
		html.write("><img name=\"PlayerStatsServer\" src=\"%s/player.statistics.server.off.gif\" border=0></a>\n",
				   g_pApp->supportHTTPPath.c_str());
	}
	html.write("\t\t\t<p><a href=\"cvars.html\" target=\"data\"");
	writeEventHandlers(html, "ServerSettings");
	html.write("><img name=\"ServerSettings\" src=\"%s/server.settings.off.gif\" border=0></a>\n",
			   g_pApp->supportHTTPPath.c_str());
	html.write("\t\t\t</div>\n");
	html.write("\t\t</td>\n");
	html.write("\t</tr>\n");
...
const char* CTFStatsReport::makeDurationString()
{
	...
	sprintf(DurationString,"%02li:%02li - %02li:%02li %s, %s %s %li%s %li",shour,smin,ehour,emin,matchtz,
Util::daysofWeek[sweekday],Util::Months[smo],sday,(sday%10)<4?Util::numberSuffixes[sday%10]:"th",syear);
