	...
	else
	{
		...
		for(int f = 0; f < 6; f++)
			for(int y = 0; y < cmap.face_maps[f].Height; y++)
				...
				{
					Vector clr(cmap.face_maps[f].Pixel(x, y, 0), cmap.face_maps[f].Pixel(x, y, 1),
		...
		printf("Point light color=%d %d %d 255\n", (int)(255 * pow(AverageHue.x, 1.0f / 2.2f)),
			   (int)(255 * pow(AverageHue.y, 1.0f / 2.2f)), (int)(255 * pow(AverageHue.z, 1.0f / 2.2f)));
							...
							for(int x1 = 0; x1 < cmap.face_maps[f].Width; x1 += 20)
							{
								...
								if(dot_sphere > 0)
								{
									...
									for(int comp = 0; comp < 3; comp++)
										sumlight[comp] += dot_sphere * cmap.face_maps[f1].Pixel(x1, y1, comp);
								}
							}
					sumlight *= 1.0 / sumdot;
		...
		printf("Ambient color=%d %d %d 255\n", (int)(255 * pow(AmbientColor.x, 1.0f / 2.2f)),
			   (int)(255 * pow(AmbientColor.y, 1.0f / 2.2f)), (int)(255 * pow(AmbientColor.z, 1.0f / 2.2f)));
