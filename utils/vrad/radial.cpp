...
void AddDirectToRadial(radial_t *rad, Vector const &pnt, Vector2D const &coordmins, Vector2D const &coordmaxs,
					   LightingValue_t const light[NUM_BUMP_VECTS + 1], bool hasBumpmap, bool neighborHasBumpmap)
{
...
void AddBouncedToRadial(radial_t *rad, Vector const &pnt, Vector2D const &coordmins, Vector2D const &coordmaxs,
		...
		if(g_FacePatches.Element(fn->neighbor[j]) != g_FacePatches.InvalidIndex())
		{
			for(patch = &g_Patches.Element(g_FacePatches.Element(fn->neighbor[j])); patch; patch = pNextPatch)
			{
				...
				if(ValidDispFace(&g_pFaces[fn->neighbor[j]]))
				{
					Vector patchOrigin;
					WindingCenter(patch->winding, patchOrigin);
					AddBouncedToRadial(rad, patchOrigin, mins, maxs, patch->totallight.light, needsBumpmap,
									   needsBumpmap);
				}
				else
				{
					AddBouncedToRadial(rad, patch->origin, mins, maxs, patch->totallight.light, needsBumpmap,
									   needsBumpmap);
				}
			}
		}
...
radial_t *BuildLuxelRadial(int facenum, int style)
{
	...
	for(int k = 0; k < fl->numsamples; k++)
	{
		...
		AddDirectToRadial(rad, fl->sample[k].pos, fl->sample[k].mins, fl->sample[k].maxs, light, needsBumpmap,
						  needsBumpmap);
	}
	...
	for(k = 0; k < lightstyles; k++)
	{
		...
		for(j = 0; j < fl->numluxels; j++)
		{
			...
			if(!do_fast)
			{
				...
				{
					baseSampleOk =
						StaticDispMgr()->SampleRadial(facenum, rad, fl->luxel[j], j, lb, bumpSampleCount, false);
				}
			}
			else
			{
