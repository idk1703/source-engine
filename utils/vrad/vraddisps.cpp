...
public:
	...
	bool SampleRadial(int ndxFace, radial_t *pRadial, Vector const &vPos, int ndxLxl, LightingValue_t *pLightSample,
					  int sampleCount, bool bPatch);
	...
	void ClipRayToDispInLeaf(DispTested_t &dispTested, Ray_t const &ray, int ndxLeaf, float &dist, dface_t *&pFace,
							 Vector2D &luxelCoord);
...
private:
	...
	void RadialLuxelAddPatch(int ndxFace, Vector const &luxelPt, Vector const &luxelNormal, float radius,
							 radial_t *pRadial, int ndxRadial, bool bBump, CUtlVector<CPatch *> &interestingPatches);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRadDispMgr::ClipRayToDispInLeaf(DispTested_t &dispTested, Ray_t const &ray, int ndxLeaf, float &dist,
									   dface_t *&pFace, Vector2D &luxelCoord)
{
	...
	if(dispTree.m_pDispTree->AABBTree_Ray(*pCtx->m_pRay, output))
	{
		if(output.dist < pCtx->m_Distance)
		{
			...
			ComputePointFromBarycentric(dispTree.m_pDispTree->GetLuxelCoord(output.ndxVerts[0]),
										...
										dispTree.m_pDispTree->GetLuxelCoord(output.ndxVerts[2]), output.u, output.v,
										pCtx->m_LuxelCoord);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline void GetSampleLight(facelight_t *pFaceLight, int ndxStyle, bool bBumped, int ndxSample,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AddSampleLightToRadial(Vector const &samplePos, Vector const &sampleNormal, LightingValue_t *pSampleLight,
							float sampleRadius2, Vector const &luxelPos, Vector const &luxelNormal, radial_t *pRadial,
							int ndxRadial, bool bBumped, bool bNeighborBumped)
{
						...
						if(pFaceLight && IsNeighbor(ndxFace, ndxFaceLight))
						{
							...
							if(pFace)
							{
								...
								AddSampleLightToRadial(pFaceLight->sample[ndxSample].pos,
													   pFaceLight->sample[ndxSample].normal, sampleLight,
													   radius * radius, luxelPt, luxelNormal, pRadial, ndxRadial, bBump,
													   bNeighborBump);
							}
						}
					}
...
void CVRadDispMgr::RadialLuxelBuild(CVRADDispColl *pDispTree, radial_t *pRadial, int ndxStyle, bool bBump)
{
	...
	{
		RadialLuxelAddSamples(ndxFace, pFaceLight->luxel[ndxRadial], pFaceLight->luxelNormals[ndxRadial], radius,
							  pRadial, ndxRadial, bBump, pFace->styles[ndxStyle]);
	}
}
...
extern void GetBumpNormals(const float *sVect, const float *tVect, const Vector &flatNormal, const Vector &phongNormal,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AddPatchLightToRadial(Vector const &patchOrigin, Vector const &patchNormal, Vector *pPatchLight,
						   float patchRadius2, Vector const &luxelPos, Vector const &luxelNormal, radial_t *pRadial,
						   int ndxRadial, bool bBump, bool bNeighborBump)
{
	// calculate the light vector
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRadDispMgr::RadialLuxelAddPatch(int ndxFace, Vector const &luxelPt, Vector const &luxelNormal, float radius,
									   ...
									   CUtlVector<CPatch *> &interestingPatches)
{
	...
	for(int i = 0; i < interestingPatches.Count(); i++)
	{
		...
		AddPatchLightToRadial(pPatch->origin, pPatch->normal, patchLight, radius * radius, luxelPt, luxelNormal,
							  pRadial, ndxRadial, bBump, bNeighborBump);
	}
#else
	// calculate one over the voxel size
	float ooVoxelSize = 1.0f / SAMPLEHASH_VOXEL_SIZE;
				...
				if(handle != g_PatchSampleHashTable.InvalidHandle())
				{
					...
					for(int ndx = 0; ndx < count; ndx++)
					{
						...
						if(pPatch && pPatch->m_IterationKey != curIterationKey)
						{
							...
							{
								bool bNeighborBump =
									texinfo[g_pFaces[pPatch->faceNumber].texinfo].flags & SURF_BUMPLIGHT ? true : false;
...
void CVRadDispMgr::GetInterestingPatchesForLuxels(int ndxFace, CUtlVector<CPatch *> &interestingPatches,
												  float patchSampleRadius)
{
	...
	int allVoxelSize[3] = {allVoxelMax[0] - allVoxelMin[0], allVoxelMax[1] - allVoxelMin[1],
...
void CVRadDispMgr::RadialPatchBuild(CVRADDispColl *pDispTree, radial_t *pRadial, bool bBump)
{
	...
	for(int ndxRadial = 0; ndxRadial < radialSize; ndxRadial++)
	{
		RadialLuxelAddPatch(ndxFace, pFaceLight->luxel[ndxRadial], pFaceLight->luxelNormals[ndxRadial], radius, pRadial,
