...
bool CIncremental::PrepareForLighting()
{
	...
	for(int iUnmatched = unmatched.Head(); iUnmatched != unmatched.InvalidIndex();
		iUnmatched = unmatched.Next(iUnmatched))
	{
		CIncLight *pLight = m_Lights[unmatched[iUnmatched]];

		// First tag faces that it touched so they get recomposited.
		for(unsigned short iFace = pLight->m_LightFaces.Head(); iFace != pLight->m_LightFaces.InvalidIndex();
...
void CIncremental::AddLightToFace(IncrementalLightID lightID, int iFace, int iSample, int lmSize, float dot,
								  int iThread)
{
...
void CompressLightData(CLightValue const *pValues, int nValues, CUtlBuffer *pBuf)
{
...
void CIncremental::FinishFace(IncrementalLightID lightID, int iFace, int iThread)
{
	...
	if(pLight->m_pCachedFaces[iThread] && pLight->m_pCachedFaces[iThread]->m_FaceIndex == iFace)
	{
		...
		CompressLightData(pFace->m_LightValues.Base(), pFace->m_LightValues.Count(), &pFace->m_CompressedData);
	...
	for(int facenum = 0; facenum < numfaces; facenum++)
	{
		...
		for(int iFace = 0; iFace < faceLights[facenum].Count(); iFace++)
		{
			...
			for(int iSample = 0; iSample < nLuxels; iSample++)
			{
				...
				if(flDot)
				{
					VectorMA(faceLight[iSample], flDot / pFace->m_pLight->m_flMaxIntensity,
...
bool CIncremental::LoadIncrementalFile()
{
	...
	for(int iLight = 0; iLight < nLights; iLight++)
	{
		...
		pLight->m_flMaxIntensity =
			max(pLight->m_Light.intensity.x, max(pLight->m_Light.intensity.y, pLight->m_Light.intensity.z));
...
bool CIncremental::SaveIncrementalFile()
{
	...
	for(int iLight = m_Lights.Head(); iLight != m_Lights.InvalidIndex(); iLight = m_Lights.Next(iLight))
	{
		...
		FileWrite(fp, nFaces);
		for(int iFace = pLight->m_LightFaces.Head(); iFace != pLight->m_LightFaces.InvalidIndex();
			iFace = pLight->m_LightFaces.Next(iFace))
		{
