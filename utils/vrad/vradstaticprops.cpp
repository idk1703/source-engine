...
//-----------------------------------------------------------------------------
inline Vector ComputeBarycentric(Vector2D _edgeC, Vector2D _edgeA, Vector2D _edgeB, float _dAA, float _dAB, float _dBB,
								 float _invDenom)
{
...
void Rasterizer::Build()
{
	...
	for(int j = iMinY; j <= iMaxY; ++j)
	{
		for(int i = iMinX; i <= iMaxX; ++i)
		{
			...
			newLoc.insideTriangle = (barycentric.x >= 0.0f && barycentric.x <= 1.0f && barycentric.y >= 0.0f &&
									 barycentric.y <= 1.0f && barycentric.z >= 0.0f && barycentric.z <= 1.0f);
		}
	}
}
...
static void ConvertTexelDataToTexture(unsigned int _resX, unsigned int _resY, ImageFormat _destFmt,
									  const CUtlVector<colorTexel_t> &_srcTexels, CUtlMemory<byte> *_outTexture);

// Such a monstrosity. :(
static void GenerateLightmapSamplesForMesh(const matrix3x4_t &_matPos, const matrix3x4_t &_matNormal, int _iThread,
...
static void DumpLightmapLinear(const char *_dstFilename, const CUtlVector<colorTexel_t> &_srcTexels, int _width,
							   int _height);
...
private:
	...
	struct CStaticProp
	{
		...
		// Note that all lightmaps for a given prop share the same resolution (and format)--and there can be multiple
		// lightmaps per prop (if there are multiple pieces--the watercooler is an example). This is effectively because
		// there's not a good way in hammer for a prop to say "this should be the resolution of each of my sub-pieces."
	...
	void ApplyLightingToStaticProp(int iStaticProp, CStaticProp &prop,
...
bool LoadStudioModel(char const *pModelName, CUtlBuffer &buf)
{
	...
	if(strncmp((const char *)buf.PeekGet(), "IDST", 4) && strncmp((const char *)buf.PeekGet(), "IDAG", 4))
	{
...
bool LoadVTXFile(char const *pModelName, const studiohdr_t *pStudioHdr, CUtlBuffer &buf)
{
	...
	if(pVtxHdr->version != OPTIMIZED_MODEL_FILE_VERSION)
	{
		Warning("Error! Invalid VTX file version: %d, expected %d \"%s\"\n", pVtxHdr->version,
				OPTIMIZED_MODEL_FILE_VERSION, filename);
		return false;
	}
...
//-----------------------------------------------------------------------------
inline static Vector *PositionFromIndex(const mstudio_meshvertexdata_t *vertData, mstudiomesh_t *pMesh,
...
void DumpCollideToGlView(vcollide_t *pCollide, const char *pFilename)
{
	...
	for(int i = 0; i < pCollide->solidCount; ++i)
	{
		...
		{
			fprintf(fp, "3\n");
			fprintf(fp, "%6.3f %6.3f %6.3f %.2f %.3f %.3f\n", outVerts[vert].x, outVerts[vert].y, outVerts[vert].z, fr,
					fg, fb);
			vert++;
			fprintf(fp, "%6.3f %6.3f %6.3f %.2f %.3f %.3f\n", outVerts[vert].x, outVerts[vert].y, outVerts[vert].z, fr,
					fg, fb);
...
void CVradStaticPropMgr::CreateCollisionModel(char const *pModelName)
{
	...
	if(LoadStudioCollisionModel(pModelName, bufphy))
	{
		...
		s_pPhysCollision->VCollideLoad(pCollide, header.solidCount, (const char *)bufphy.PeekGet(),
								  ...
								  int static_prop_id_to_skip = -1, int nLFlags = 0)
{
	...
	for(directlight_t *dl = activelights; dl != NULL; dl = dl->next)
	{
		...
		GatherSampleLightSSE(sampleOutput, dl, -1, adjusted_pos4, &normal4, 1, iThread,
							 nLFlags | GATHERLFLAGS_FORCE_FAST, static_prop_id_to_skip, flEpsilon);

		VectorMA(outColor, sampleOutput.m_flFalloff.m128_f32[0] * sampleOutput.m_flDot[0].m128_f32[0],
...
//-----------------------------------------------------------------------------
void CVradStaticPropMgr::ApplyLightingToStaticProp(int iStaticProp, CStaticProp &prop,
												   const CComputeStaticPropLightingResults *pResults)
{
	...
	for(int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID)
	{
		...
		for(int modelID = 0; modelID < pBodyPart->nummodels; ++modelID)
		{
			...
			const CUtlVector<colorVertex_t> *colorVerts =
				pResults->m_ColorVertsArrays.Count() ? pResults->m_ColorVertsArrays[iCurColorVertsArray++] : nullptr;
			const CUtlVector<colorTexel_t> *colorTexels =
				pResults->m_ColorTexelsArrays.Count() ? pResults->m_ColorTexelsArrays[iCurColorTexelsArray++] : nullptr;
				...
				for(int nMesh = 0; nMesh < pStudioModel->nummeshes; ++nMesh)
				{
					...
					for(int nGroup = 0; nGroup < pVtxMesh->numStripGroups; ++nGroup)
					{
						...
						if(colorTexels)
						{
							...
							ConvertTexelDataToTexture(prop.m_LightmapImageWidth, prop.m_LightmapImageHeight,
							...
							if(g_bDumpPropLightmaps)
							{
								...
								V_snprintf(buffer, _MAX_PATH - 1,
										   "staticprop_lightmap_%d_%.0f_%.0f_%.0f_%s_%d_%d_%d_%d_%d.tga", iStaticProp,
										   prop.m_Origin.x, prop.m_Origin.y, prop.m_Origin.z,
										   dict.m_pStudioHdr->pszName(), bodyID, modelID, nLod, nMesh, nGroup);
								...
								DumpLightmapLinear(buffer, (*colorTexels), prop.m_LightmapImageWidth,
										 ...
										 CComputeStaticPropLightingResults *pResults)
{
	...
	const int skip_prop =
		(g_bDisablePropSelfShadowing || (prop.m_Flags & STATIC_PROP_NO_SELF_SHADOWING)) ? prop_index : -1;
	...
	for(int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID)
	{
		...
		for(int modelID = 0; modelID < pBodyPart->nummodels; ++modelID)
		{
			...
			for(int meshID = 0; meshID < pStudioModel->nummeshes; ++meshID)
			{
				...
				if(withTexelLighting)
				{
					GenerateLightmapSamplesForMesh(matPos, matNormal, iThread, skip_prop, nFlags,
				...
				for(int vertexID = 0; vertexID < pStudioMesh->numvertices; ++vertexID)
				{
					...
					else
					{
						...
						ComputeDirectLightingAtPoint(direct_pos, sampleNormal, directColor, iThread, skip_prop, nFlags);
						...
						{
							if(numbounce >= 1)
								ComputeIndirectLightingAtPoint(samplePosition, sampleNormal, indirectColor, iThread,
															   true, (prop.m_Flags & STATIC_PROP_IGNORE_NORMALS) != 0);
						}
			...
			if(badVerts.Count() && (prop.m_bLightingOriginValid || badVerts.Count() != numVertexes))
			{
				for(int nBadVertex = 0; nBadVertex < badVerts.Count(); nBadVertex++)
				{
					...
					ComputeIndirectLightingAtPoint(bestPosition, badVerts[nBadVertex].m_Normal, indirectColor, iThread,
												   true);
...
void CVradStaticPropMgr::SerializeLighting()
{
	...
	for(int i = 0; i < count; ++i)
	{
		...
		size = sizeof(HardwareVerts::FileHeader_t) +
			   m_StaticProps[i].m_MeshData.Count() * sizeof(HardwareVerts::MeshHeader_t) + totalVertexes * 4 + 2 * 512;
	...
	for(int i = 0; i < count; ++i)
	{
		...
		size = sizeof(HardwareTexels::FileHeader_t) +
			   m_StaticProps[i].m_MeshData.Count() * sizeof(HardwareTexels::MeshHeader_t) + totalTexelSizeBytes +
			   2 * kAlignment;
		...
		for(int n = 0; n < pVhtHdr->m_nMeshes; n++)
		{
			...
			Q_memcpy(pTexelData, m_StaticProps[i].m_MeshData[n].m_TexelsEncoded.Base(),
...
void CVradStaticPropMgr::ComputeLighting(int iThread)
{
	...
	if(g_bUseMPI)
	{
		...
		DistributeWork(count, VMPI_DISTRIBUTEWORK_PACKETID, &CVradStaticPropMgr::VMPI_ProcessStaticProp_Static,
	...
	for(int nProp = 0; nProp < count; ++nProp)
	{
		...
		if(!g_bStaticPropPolys)
		{
			...
			else
			{
				...
				g_RtEnv.AddAxisAlignedRectangularSolid(TRACE_ID_STATICPROP | nProp, prop.m_mins, prop.m_maxs,
													   fullCoverage);
			}
		...
		for(int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID)
		{
			...
			for(int modelID = 0; modelID < pBodyPart->nummodels; ++modelID)
			{
				...
				for(int nMesh = 0; nMesh < pStudioModel->nummeshes; ++nMesh)
				{
					...
					{
						if(Q_stristr(pTxtr->pszName(), g_NonShadowCastingMaterialStrings[check]))
						{
								...
								for(int i = 0; i < pStrip->numIndices; i += 3)
								{
									...
									if(shadowTextureIndex >= 0)
									{
										if(bInitTriangles)
										{
											...
											// now
											float coverage = g_ShadowTextureList.ComputeCoverageForTriangle(
												shadowTextureIndex, *vertData->Texcoord(vertex1),
												*vertData->Texcoord(vertex2), *vertData->Texcoord(vertex3));
											if(coverage < 1.0f)
											{
									...
									g_RtEnv.AddTriangle(TRACE_ID_STATICPROP | nProp, position1, position2, position3,
...
struct tl_tri_t
{
	...
	{
		return (p0 == t.p0 && p1 == t.p1 && p2 == t.p2 && n0 == t.n0 && n1 == t.n1 && n2 == t.n2);
	}
};
...
void AddTriVertsToList(CUtlVector<tl_vert_t> &triListVerts, int vertIndex, Vector vertPosition, Vector p0, Vector p1,
					   Vector p2, Vector n0, Vector n1, Vector n2)
{
	tl_tri_t tlTri;
					...
					for(nStrip = 0; nStrip < pStripGroup->numStrips; nStrip++)
					{
						...
						if(pStrip->flags & OptimizedModel::STRIP_IS_TRILIST)
						{
							for(int i = 0; i < pStrip->numIndices; i += 3)
							{
								...
								AddTriVertsToList(triListVerts, pMesh->vertexoffset + vertex1, position1, position1,
												  position2, position3, normal1, normal2, normal3);
								AddTriVertsToList(triListVerts, pMesh->vertexoffset + vertex2, position2, position1,
												  position2, position3, normal1, normal2, normal3);
								AddTriVertsToList(triListVerts, pMesh->vertexoffset + vertex3, position3, position1,
												  position2, position3, normal1, normal2, normal3);
							}
						}
						else
...
const vertexFileHeader_t *mstudiomodel_t::CacheVertexData(void *pModelData)
{
	...
	{
		Error("Error Vertex File %s checksum %d should be %d\n", fileName, pVvdHdr->checksum,
			  pActiveStudioHdr->checksum);
	}
		...
		for(nStrip = 0; nStrip < pStripGroup->numStrips; nStrip++)
		{
			...
			if(pStrip->flags & OptimizedModel::STRIP_IS_TRILIST)
			{
				for(int i = 0; i < pStrip->numIndices; i += 3)
				{
					...
					Vector modelPos[3] = {*vertData->Position(vertex1), *vertData->Position(vertex2),
										  *vertData->Position(vertex3)};

					Vector modelNormal[3] = {*vertData->Normal(vertex1), *vertData->Normal(vertex2),
					...
					Vector2D texcoord[3] = {*vertData->Texcoord(vertex1), *vertData->Texcoord(vertex2),
					...
					for(auto it = rasterizer.begin(); it != rasterizer.end(); ++it)
					{
						...
						bool doWrite = it->insideTriangle || !colorTexels[linearPos].m_bPossiblyInteresting ||
									   colorTexels[linearPos].m_fDistanceToTri > ourDistancetoTri;

						if(doWrite)
						{
							Vector itWorldPos = worldPos[0] * it->barycentric.x + worldPos[1] * it->barycentric.y +
		...
		for(int i = 0; i < _lightmapResX; ++i)
		{
			...
			if(shouldProcess)
			{
				...
				ComputeDirectLightingAtPoint(colorTexels[linearPos].m_WorldPosition,
											 colorTexels[linearPos].m_WorldNormal, directColor, _iThread, _skipProp,
											 _flags);

				if(numbounce >= 1)
				{
					ComputeIndirectLightingAtPoint(colorTexels[linearPos].m_WorldPosition,
												   colorTexels[linearPos].m_WorldNormal, indirectColor, _iThread, true,
												   (_flags & GATHERLFLAGS_IGNORE_NORMALS) != 0);
				}
...
// ------------------------------------------------------------------------------------------------
static void FilterFineMipmap(unsigned int _resX, unsigned int _resY, const CUtlVector<colorTexel_t> &_srcTexels,
...
// ------------------------------------------------------------------------------------------------
static void BuildFineMipmap(unsigned int _resX, unsigned int _resY, bool _applyFilter,
							const CUtlVector<colorTexel_t> &_srcTexels, CUtlVector<RGB888_t> *_outTexelsRGB888,
							CUtlVector<Vector> *_outLinear)
{
	...
	else
	{
		for(int i = 0; i < _srcTexels.Count(); ++i)
		{
			...
			// We drop alpha on the floor here, if this were to fire we'd need to consider using a different compressed
			// format.
...
// ------------------------------------------------------------------------------------------------
static void FilterCoarserMipmaps(unsigned int _resX, unsigned int _resY, CUtlVector<Vector> *_scratchLinear,
	...
	while(srcResX > 1 || srcResY > 1)
	{
		for(int j = 0; j < srcResY; j += 2)
		{
			for(int i = 0; i < srcResX; i += 2)
			{
				...
				ConvertLinearToRGBA8888(&sample,
										(unsigned char *)&(*_outTexelsRGB888)[dstOffset + dstCol + dstRow * dstResX]);

				// Also overwrite the srcBuffer to filter the next loop. This is safe because we won't be reading this
				// source value again during this mipmap level.
...
// ------------------------------------------------------------------------------------------------
static void ConvertToDestinationFormat(unsigned int _resX, unsigned int _resY, ImageFormat _destFmt,
									   const CUtlVector<RGB888_t> &_scratchRBG888, CUtlMemory<byte> *_outTexture)
{
	...
	if(cSrcImageFormat != _destFmt)
	{
		while(srcResX > 1 || srcResY > 1)
		{
			...
			ImageLoader::ConvertImageFormat((unsigned char *)(&_scratchRBG888[srcOffset]), cSrcImageFormat,
		...
		ImageLoader::ConvertImageFormat((unsigned char *)_scratchRBG888.Base() + srcOffset, cSrcImageFormat,
										(*_outTexture).Base() + dstOffset, _destFmt, srcResX, srcResY);
	}
	else
	{
							   ...
							   int _height)
{
	...
	TGAWriter::WriteTGAFile(_dstFilename, _width, _height, IMAGE_FORMAT_BGR888, (uint8 *)(linearBuffer.Base()),
							_width * ImageLoader::SizeInBytes(IMAGE_FORMAT_BGR888));
}