		...
		else
		{
			...
			for(j; j < num_texlights; j++)
			{
				if(strcmp(texlights[j].name, szTexlight) == 0)
				{
					...
					else if(texlights[j].value[0] != value[0] || texlights[j].value[1] != value[1] ||
...
void SubdividePatches(void)
{
	...
	// (only the faces for model #0 are split by it's BSP which is what governs the PVS, and the leaves we're interested
	// in) Sub models (1-255) are only split for the BSP that their model forms. When those patches are subdivided their
	// origins can end up in a different leaf. The engine will split (clip) those faces at run time to the world BSP
	// because the models are dynamic and can be moved.  In the software renderer, they must be split exactly in order
	// to sort per polygon.
	for(i = 0; i < uiPatchCount; i++)
	{
...
void WriteWinding(FileHandle_t out, winding_t *w, Vector &color)
{
	...
	{
		CmdLib_FPrintf(out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", w->p[i][0], w->p[i][1], w->p[i][2], color[0] / 256,
					   color[1] / 256, color[2] / 256);
	}
}
...
void WriteNormal(FileHandle_t out, Vector const &nPos, Vector const &nDir, float length, Vector const &color)
{
	...
	CmdLib_FPrintf(out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", nPos.x, nPos.y, nPos.z, color.x / 256, color.y / 256,
				   color.z / 256);
	CmdLib_FPrintf(out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", nPos.x + (nDir.x * length), nPos.y + (nDir.y * length),
				   nPos.z + (nDir.z * length), color.x / 256, color.y / 256, color.z / 256);
}

void WriteLine(FileHandle_t out, const Vector &vecPos1, const Vector &vecPos2, const Vector &color)
{
	CmdLib_FPrintf(out, "2\n");
	CmdLib_FPrintf(out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", vecPos1.x, vecPos1.y, vecPos1.z, color.x / 256,
				   color.y / 256, color.z / 256);
	CmdLib_FPrintf(out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", vecPos2.x, vecPos2.y, vecPos2.z, color.x / 256,
				   color.y / 256, color.z / 256);
}
...
{
	Vector vecTexU(pTexinfo->textureVecsTexelsPerWorldUnits[0][0], pTexinfo->textureVecsTexelsPerWorldUnits[0][1],
				   pTexinfo->textureVecsTexelsPerWorldUnits[0][2]);
	Vector vecTexV(pTexinfo->textureVecsTexelsPerWorldUnits[1][0], pTexinfo->textureVecsTexelsPerWorldUnits[1][1],
				   pTexinfo->textureVecsTexelsPerWorldUnits[1][2]);
	Vector vecLightU(pTexinfo->lightmapVecsLuxelsPerWorldUnits[0][0], pTexinfo->lightmapVecsLuxelsPerWorldUnits[0][1],
					 pTexinfo->lightmapVecsLuxelsPerWorldUnits[0][2]);
	Vector vecLightV(pTexinfo->lightmapVecsLuxelsPerWorldUnits[1][0], pTexinfo->lightmapVecsLuxelsPerWorldUnits[1][1],
	...
	while(1)
	{
		...
		if(patch->needsBumpmap)
		{
			...
			else
			{
				...
				texinfo_t *pTexinfo = &texinfo[g_pFaces[patch->faceNumber].texinfo];
				// use facenormal along with the smooth normal to build the three bump map vectors
				GetBumpNormals(pTexinfo->textureVecsTexelsPerWorldUnits[0], pTexinfo->textureVecsTexelsPerWorldUnits[1],
			...
			for(k = 0; k < num; k++, trans++)
			{
				...
				for(i = 0; i < NUM_BUMP_VECTS + 1; i++)
				{
					...
					if(dot <= 0)
					{
						//						Assert( i > 0 ); // if this hits, then the transfer shouldn't be here.  It doesn't
...
void BounceLight(void)
{
	...
	for(i = 0; i < uiPatchCount; i++)
	{
		// totallight has a copy of the direct lighting.  Move it to the emitted light and zero it out (to integrate
		// bounces only)
...
void RadWorld_Start()
{
	...
	if(luxeldensity < 1.0)
	{
		...
		for(i = 0; i < texinfo.Count(); i++)
		{
			...
			for(int j = 0; j < 2; j++)
			{
				Vector tmp(tx->lightmapVecsLuxelsPerWorldUnits[j][0], tx->lightmapVecsLuxelsPerWorldUnits[j][1],
...
void PrintUsage(int argc, char **argv)
{
	...
	Warning("usage  : vrad [options...] bspfile\n"
			...
			"  -textureshadows : Allows texture alpha channels to block light - rays intersecting alpha surfaces will "
			"sample the texture\n"
