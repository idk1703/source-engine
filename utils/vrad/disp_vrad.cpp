...
#if defined(USE_SCRATCHPAD)
			ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(iCornerVert), pDisp->GetNormal(iCornerVert),
									   Vector(0, 0, 1), 25);
#endif
	...
	for(int iDisp = 0; iDisp < listSize; iDisp++)
	{
		...
		for(int iEdge = 0; iEdge < 4; iEdge++)
		{
			...
			if(pEdge->m_SubNeighbors[0].IsValid() && pEdge->m_SubNeighbors[1].IsValid())
			{
				...
				{
					CVertIndex viNBCorners[2] = {pNeighbor1->GetCornerPointIndex(iNBCorners[0]),
...
#if defined(USE_SCRATCHPAD)
					ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(iMidPoint), pDisp->GetNormal(iMidPoint),
											   Vector(0, 1, 1), 25);
#endif
				}
			}
...
#if defined(USE_SCRATCHPAD)
						ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(viTween), pDisp->GetNormal(viTween),
												   Vector(1, 0.5, 0), 25);
#endif
					}
...
void ScratchPad_DrawOriginalNormals(const CCoreDispInfo *pListBase, int listSize)
{
	for(int i = 0; i < listSize; i++)
	{
		...
		while(it.Next())
		{
			ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(it.GetVertIndex()), pDisp->GetNormal(it.GetVertIndex()),
									   Vector(0, 1, 0), 15);
		}
	}
}
