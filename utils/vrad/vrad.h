...
struct LightingValue_t
{
	...
	{
		return (m_vecLighting.x >= 0 && m_vecLighting.y >= 0 && m_vecLighting.z >= 0 && m_vecLighting.x < 1e10 &&
...
void TestLine(FourVectors const &start, FourVectors const &stop, fltx4 *pFractionVisible,
...
void TestLine_DoesHitSky(FourVectors const &start, FourVectors const &stop, fltx4 *pFractionVisible,
						 bool canRecurse = true, int static_prop_to_skip = -1, bool bDoDebug = false);
...
public:
	...
	virtual bool SampleRadial(int ndxFace, radial_t *pRadial, Vector const &vPos, int ndxLxl,
							  LightingValue_t *pLightSample, int sampleCount, bool bPatch) = 0;
	...
	virtual void ClipRayToDispInLeaf(DispTested_t &dispTested, Ray_t const &ray, int ndxLeaf, float &dist,
									 dface_t *&pFace, Vector2D &luxelCoord) = 0;
	virtual void ClipRayToDispInLeaf(DispTested_t &dispTested, Ray_t const &ray, int ndxLeaf, float &dist,
									 Vector *pNormal) = 0;
...
void ComputeIndirectLightingAtPoint(Vector &position, Vector &normal, Vector &outColor, int iThread,
									bool force_fast = false, bool bIgnoreNormals = false);
