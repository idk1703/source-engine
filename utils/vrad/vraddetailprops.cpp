...
void DumpRayToGlView(Ray_t const &ray, float dist, Vector *pColor, const char *pFilename)
{
	...
	int vert = 0;
	CmdLib_FPrintf(fp, "3\n");
	CmdLib_FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", start1.x, start1.y, start1.z, pColor->x, pColor->y,
				   pColor->z);
	vert++;
	CmdLib_FPrintf(fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", start2.x, start2.y, start2.z, pColor->x, pColor->y,
				   pColor->z);
	vert++;
...
static void ComputeMaxDirectLighting(DetailObjectLump_t &prop, Vector *maxcolor, int iThread)
{
	...
	for(j = 0; j < lights.Count(); ++j)
	{
		...
		VectorMA(maxcolor[dl->light.style], out.m_flFalloff.m128_f32[0] * out.m_flDot[0].m128_f32[0],
...
static void ComputeLightmapColorFromAverage(dface_t *pFace, directlight_t *pSkylight, float scale,
...
static bool SurfHasBumpedLightmaps(dface_t *pSurf)
{
	...
	if((texinfo[pSurf->texinfo].flags & SURF_BUMPLIGHT) && (!(texinfo[pSurf->texinfo].flags & SURF_NOLIGHT)))
	{
		hasBumpmap = true;
	}
	return hasBumpmap;
...
static void ComputeLightmapColorPointSample(dface_t *pFace, directlight_t *pSkylight, Vector2D const &luv, float scale,
											Vector pColor[MAX_LIGHTSTYLES])
{
...
bool CastRayInLeaf(int iThread, const Vector &start, const Vector &end, int leafIndex, float *pFraction,
				   Vector *pNormal)
{
	pFraction[0] = 1.0f;
...
//-----------------------------------------------------------------------------
void CalcRayAmbientLighting(int iThread, const Vector &vStart, const Vector &vEnd, float tanTheta,
...
//-----------------------------------------------------------------------------
static void ComputeAmbientLightingAtPoint(int iThread, const Vector &origin, Vector radcolor[NUMVERTEXNORMALS],
										  Vector color[MAX_LIGHTSTYLES])
{
...
//-----------------------------------------------------------------------------
void ComputeIndirectLightingAtPoint(Vector &position, Vector &normal, Vector &outColor, int iThread, bool force_fast,
									bool bIgnoreNormals)
{
	Ray_t ray;
	CLightSurface surfEnum(iThread);
...
static void ComputeLighting(DetailObjectLump_t &prop, int iThread)
{
	...
	for(int i = 1; i < MAX_LIGHTSTYLES; ++i)
	{
		...
		if((totalColor[0] != 0.0f) || (totalColor[1] != 0.0f) || (totalColor[2] != 0.0f))
		{
			if(!hasLightstyles)
			{
...
{
	WriteDetailLightingLump(GAMELUMP_DETAIL_PROP_LIGHTING, GAMELUMP_DETAIL_PROP_LIGHTING_VERSION,
							s_DetailPropLightStyleLumpLDR);
	WriteDetailLightingLump(GAMELUMP_DETAIL_PROP_LIGHTING_HDR, GAMELUMP_DETAIL_PROP_LIGHTING_HDR_VERSION,
...
void ComputeDetailPropLighting(int iThread)
{
	...
	if(g_bHDR)
	{
		UnserializeDetailPropLighting(GAMELUMP_DETAIL_PROP_LIGHTING, GAMELUMP_DETAIL_PROP_LIGHTING_VERSION,
									  s_DetailPropLightStyleLumpLDR);
	}
	else
	{
		UnserializeDetailPropLighting(GAMELUMP_DETAIL_PROP_LIGHTING_HDR, GAMELUMP_DETAIL_PROP_LIGHTING_HDR_VERSION,
									  s_DetailPropLightStyleLumpHDR);
	}
