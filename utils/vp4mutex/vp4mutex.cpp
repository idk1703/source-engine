...
//
// NOTE: All of this crap is here since we don't want to have the .exe depend on tier0 or vstdlib.dll.  If we change
// that, this can go away and std::vector can go back
...
int FindLockUsers(CCountersUser &counters, char const *branchspec, std::vector<CUtlSymbol> &users,
				  std::vector<int> &locktimes, std::vector<CUtlSymbol> *branchnames = NULL)
{
	users.RemoveAll();
...
static void GetHourMinuteSecondsString(int nInputSeconds, char *pOut, int outLen)
{
	...
	if(nHours > 0)
		Q_snprintf(pOut, outLen, "%d hour%s, %d minute%s, %d second%s", nHours, extra[nHours != 1], nMinutes,
				   extra[nMinutes != 1], nSeconds, extra[nSeconds != 1]);
	else if(nMinutes > 0)
		Q_snprintf(pOut, outLen, "%d minute%s, %d second%s", nMinutes, extra[nMinutes != 1], nSeconds,
				   extra[nSeconds != 1]);
	else
...
int _tmain(int argc, _TCHAR *argv[])
{
	...
	if(!validBranch || !validSleepSeconds || !validClient || !validIP)
	{
		...
		switch(action)
		{
			...
			case MUTEX_QUERY:
			{
				if(holdCount == 1)
				{
					...
					printf("%s: '%s' lock on %s is HELD by:  %s\nHOLD INFO:  %s\n", basefile, branchspec.String(),
						   ipport.String(), users[0].String(), timestr);
				}
				else if(holdCount == 0)
				{
			...
			case MUTEX_LOCK:
			{
				printf("%s: Attempting to lock the '%s' codeline for %s on %s\n\n", basefile, branchspec.String(),
					   clientname.String(), ipport.String());
