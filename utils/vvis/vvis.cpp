...
static float GetMinDistanceBetweenBoundingBoxes(const Vector &min1, const Vector &max1, const Vector &min2,
												const Vector &max2)
{
...
static float CalcDistanceFromLeafToWater(int leafNum)
{
	...
	/*
	   ...
	   ); ConvertConvexToCollide(  &pConvex, temp.RemoveAll();
		}
	*/
...
void PrintUsage(int argc, char **argv)
{
	...
	Warning("usage  : vvis [options...] bspfile\n"
			...
			"  -trace <start cluster> <end cluster> : Writes a linefile that traces the vis from one cluster to "
			"another for debugging map vis.\n"
...
int RunVVis(int argc, char **argv)
{
	...
	{
		if(g_TraceClusterStart < 0 || g_TraceClusterStart >= portalclusters || g_TraceClusterStop < 0 ||
