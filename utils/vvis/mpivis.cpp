...
void RunMPIBasePortalVis()
{
	...
	g_CPUTime.Init();
	double elapsed = DistributeWork(g_numportals * 2,			  // # work units
	...
	if(g_bMPIMaster)
	{
		if(!fastvis)
		{
			...
			VMPI_Send2Chunks(cPacketID, sizeof(cPacketID), pVirtualFilename, strlen(pVirtualFilename) + 1,
							 VMPI_PERSISTENT);
		}
	}
	else
				...
				if(key == '1')
				{
					...
					if(pPad)
					{
						...
						for(int i = 0; i < g_numportals * 2; i++)
						{
							...
							ScratchPad_DrawWinding(pPad, p->winding->numpoints, p->winding->points, Vector(1, 0, 0),
												   Vector(.3, .3, .3));
						}
