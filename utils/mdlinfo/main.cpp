...
{
	return CFmtStr("%s %s %s", PrettyPrintFloat(v.x).Get(), PrettyPrintFloat(v.y).Get(), PrettyPrintFloat(v.z).Get());
}
...
const CommandLineBinaryOpt_t CMdlInfoApp::s_binaryOptions[] = {{"-h", kOptHelp, "Print usage information"},
...
//
//-----------------------------------------------------------------------------
CMdlInfoApp::CMdlInfoApp() : m_bOptAll(true), m_bOptVerbose(false) {}
...
bool CMdlInfoApp::CommandLineHasOpt(const char *pszOpt)
{
	...
	{
		if(!V_stricmp(pszOpt, s_binaryOptions[ii].m_pszShortName) ||
		   !V_stricmp(pszOpt, s_binaryOptions[ii].m_pszLongName))
		{
			if(CommandLine()->FindParm(s_binaryOptions[ii].m_pszShortName) > 0 ||
			   CommandLine()->FindParm(s_binaryOptions[ii].m_pszLongName) > 0)
				return true;
...
{
	if(CommandLineHasOpt(kOptGeneral) || CommandLineHasOpt(kOptBones) || CommandLineHasOpt(kOptAttachments) ||
	   CommandLineHasOpt(kOptAnimation) || CommandLineHasOpt(kOptSequence) || CommandLineHasOpt(kOptMesh) ||
	   CommandLineHasOpt(kOptTexture) || CommandLineHasOpt(kOptSkin))
	{
		m_bOptAll = false;
	}
...
void CMdlInfoApp::DumpBones(CStudioHdr &cStudioHdr) const
{
	...
	for(int ii = 0; ii < nBoneCount; ++ii)
	{
		...
		const mstudiobone_t *pParentBone =
			pBone->parent < ii && pBone->parent >= 0 ? cStudioHdr.pBone(pBone->parent) : NULL;
		...
		if(pSrcBoneTransform)
		{
			...
			if(!QAnglesAreEqual(QAngle(0.0f, 0.0f, 0.0f), aAlignment, flEps) ||
			   !VectorsAreEqual(Vector(0.0f, 0.0f, 0.0f), vAlignment, flEps))
			{
				bRealigned = true;
			}
		}
		...
		if(m_bOptVerbose)
		{
			...
			{
				static const char *s_ProcType[] = {"<ERROR",	"AXISINTERP", "QUATINTERP",
												   "AIMATBONE", "AIMATTACH",  "JIGGLE"};
		...
		CFmtStrMax sTmp("%s$definebone \"%s\" \"%s\" %s %s %s %s %s %s",
						(pBone->flags & BONE_ALWAYS_PROCEDURAL) ? "// procedural " : "", pBone->pszName(),
						pParentBone ? pParentBone->pszName() : "", PrettyPrintFloat(pBone->pos.x).Get(),
						PrettyPrintFloat(pBone->pos.y).Get(), PrettyPrintFloat(pBone->pos.z).Get(),
						PrettyPrintFloat(aRot.x).Get(), PrettyPrintFloat(aRot.y).Get(), PrettyPrintFloat(aRot.z).Get());

		if(bRealigned)
		{
			sTmp.AppendFormat(" %s %s %s %s %s %s", PrettyPrintFloat(vAlignment.x).Get(),
							  PrettyPrintFloat(vAlignment.y).Get(), PrettyPrintFloat(vAlignment.z).Get(),
							  PrettyPrintFloat(aAlignment.x).Get(), PrettyPrintFloat(aAlignment.y).Get(),
...
void CMdlInfoApp::DumpAttachments(CStudioHdr &cStudioHdr) const
{
	...
	for(int ii = 0; ii < nAtttachmentCount; ++ii)
	{
		...
		CFmtStrMax sTmp("$attachment \"%s\"", attachment.pszName());
