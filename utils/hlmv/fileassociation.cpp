...
FileAssociation::FileAssociation() : mxWindow(0, 100, 100, 400, 210, "File Associations", mxWindow::Dialog)
{
...
void FileAssociation::initAssociations()
{
	int i;
...
void FileAssociation::setAssociation(int index)
{
	...
	*/
	...
	rbAction[2]->setEnabled(!mx_strcasecmp(d_associations[index].extension, "mdl") ||
							!mx_strcasecmp(d_associations[index].extension, "tga") ||
							!mx_strcasecmp(d_associations[index].extension, "wav"));
}

void FileAssociation::saveAssociations()
{
	char path[256];
	...
	for(int i = 0; i < 16; i++)
	{
		...
		fprintf(file, "\"%s\" \"%s\" %d\n", d_associations[i].extension, d_associations[i].program,
