...
static void PopulateSoundNameList(mxListBox *pListBox)
{
	...
	{
		for(int i = g_pSoundEmitterBase->First(); i != g_pSoundEmitterBase->InvalidIndex();
			i = g_pSoundEmitterBase->Next(i))
		{
...
void CBoneControlWindow::Init()
{
	...
	mxButton *btnApplyChild =
		new mxButton(this, left, top + 115, 140, 20, "Apply to Children", IDC_BONE_APPLY_TO_CHILDREN);
	mxToolTip::add(btnApplyChild, "Apply the surface property to all child bones");

	// Use autogenerated hitboxes
	m_cAutoHitbox =
		new mxCheckBox(this, left, top + 140, 140, 20, "Autogenerate Hitboxes", IDC_BONE_USE_AUTOGENERATED_HITBOXES);
	mxToolTip::add(m_cAutoHitbox, "When this is checked, studiomdl will automatically generate hitboxes");

	m_cShowDefaultPose =
		new mxCheckBox(this, left, top + 160, 140, 20, "Show Default Pose", IDC_BONE_SHOW_DEFAULT_POSE);
	...
	m_cHitboxHighlight =
		new mxCheckBox(this, left + 3, top + 45, 100, 20, "Highlight Hitbox", IDC_BONE_HIGHLIGHT_HITBOX);
...
void CBoneControlWindow::OnHitboxSelected(int hitbox)
{
	...
	if(m_SetBoneHitBoxes.Count() == 0 || (m_SetBoneHitBoxes[m_nHitboxSet].Count() <= m_Bone) ||
...
void CBoneControlWindow::OnHitboxSetChanged(void)
{
	...
	ComputeHitboxList();
	RefreshHitbox();
	// OnHitboxGroupChanged( );  // Refresh hitbox will update the group label from the hitbox's group.  No need to turn
	// that back into a group #
}
...
void CBoneControlWindow::OnAddHitbox()
{
	...
	if(m_SetBoneHitBoxes[m_nHitboxSet].Count() > 0 && m_SetBoneHitBoxes[m_nHitboxSet][m_Bone].Count() == 0)
	{
		m_cHitbox->removeAll();
	}
...
bool CBoneControlWindow::SerializeQC(CUtlBuffer &buf)
{
	...
	for(i = 1; i < g_pStudioModel->m_SurfaceProps.Count(); ++i)
	{
		...
		buf.Printf("$jointsurfaceprop \"%s\"\t \"%s\"\n", pBone->pszName(), g_pStudioModel->m_SurfaceProps[i].String());
	}
	...
	if(!m_cAutoHitbox->isChecked())
	{
		...
		for(i = 0; i < g_pStudioModel->m_HitboxSets.Count(); i++)
		{
			...
			for(unsigned short j = list.Head(); j != list.InvalidIndex(); j = list.Next(j))
			{
				...
				buf.Printf("$hbox %d \"%s\"\t  %7.2f %7.2f %7.2f  %7.2f %7.2f %7.2f", hitbox.group, pBone->pszName(),
						   hitbox.bbmin.x, hitbox.bbmin.y, hitbox.bbmin.z, hitbox.bbmax.x, hitbox.bbmax.y,
						   hitbox.bbmax.z);
...
void ControlPanel::SetupSequenceWindow(mxTab *pTab)
{
	...
	for(int i = 0; i < MAX_SEQUENCES; i++)
	{
		...
		rbFrameSelection[i] =
			new mxRadioButton(wSequence, 300, 5 + i * 22, 35, 22, "", IDC_FRAMESELECTION0 + i, i == 0);
	}
...
int ControlPanel::handleEvent(mxEvent *event)
{
	...
	switch(event->action)
	{
		...
		default:
		{
			...
			else if(event->action >= IDC_POSEPARAMETER_SCALE &&
					event->action < IDC_POSEPARAMETER_SCALE + NUM_POSEPARAMETERS)
			{
...
LoadModelResult_t ControlPanel::loadModel(const char *filename, int slot)
{
	...
	if(g_pStudioExtraModel[slot]->LoadModel(filename))
	{
		if(g_pStudioExtraModel[slot]->PostLoadModel(filename))
		{
			...
			{
				g_MDLViewer->getMenuBar()->modify(IDC_FILE_UNLOADMERGEDMODEL1 + slot,
												  IDC_FILE_UNLOADMERGEDMODEL1 + slot, filename);
...
void ControlPanel::resetControlPanel(void)
{
...
void ControlPanel::setHighlightBone(int index)
{
	if(index >= 0)
	{
...
void ControlPanel::setLOD(int index, bool setLODchoice, bool force)
{
#if 1
...
void ControlPanel::setLODMetric(float metric)
{
...
void ControlPanel::setPolycount(int polycount)
{
...
void ControlPanel::setTransparent(bool isTransparent)
{
...
void ControlPanel::updatePoseParameters()
{
	for(int i = 0; i < NUM_POSEPARAMETERS; i++)
	{
...
void ControlPanel::setOverlaySequence(int num, int index, float weight)
{
...
{
	char szFPS[32];
	sprintf(szFPS, "x %.2f = %.1f fps", g_viewerSettings.speedScale,
			g_viewerSettings.speedScale * g_pStudioModel->GetFPS());
...
void ControlPanel::initBodypartChoices()
{
	CStudioHdr *hdr = g_pStudioModel->GetStudioHdr();
	if(hdr)
	{
...
void ControlPanel::initPhysicsBones()
{
...
void ControlPanel::initBoneControllers()
{
	CStudioHdr *hdr = g_pStudioModel->GetStudioHdr();
	if(hdr)
	{
...
void ControlPanel::initSkinChoices()
{
	CStudioHdr *hdr = g_pStudioModel->GetStudioHdr();
	if(hdr)
	{
...
void ControlPanel::setModelInfo()
{
	...
	if(checkSum == hdr->GetRenderHdr()->checksum && boneLODCount == g_DrawModelResults.m_NumHardwareBones &&
	...
	sprintf(str,
			...
			"Sequences: %d\n",
			hdr->numbones(), boneLODCount, numBatches, hdr->numbonecontrollers(), hbcount, hdr->numhitboxsets(),
			hdr->GetNumSeq());
	...
	sprintf(str,
			...
			"Bodyparts: %d\n"
			"Attachments: %d\n",
			g_DrawModelResults.m_NumMaterials, hdr->numskinfamilies(), hdr->numbodyparts(), hdr->GetNumAttachments());
...
void ControlPanel::UpdateMaterialList()
{
	...
	if(pStudioR)
	{
		...
		for(int i = 0; i < nMaterials; i++)
		{
			...
			{
				Q_strcat(c_MaterialLine, "*** ERROR *** Model attempted to load one or more VMTs it can't find.",
						 sizeof(c_MaterialLine));
			}
			else
			{
...
int ControlPanel::handlePhysicsEvent(mxEvent *event)
{
	switch(event->action)
	{
		...
		case IDC_PHYS_CON_TEST:
			g_pStudioModel->Physics_SetPreview(cPhysicsBone->getSelectedIndex(), getPhysicsAxis(),
											   slPhysicsConTest->getValue());
			break;
