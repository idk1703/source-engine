...
void CAttachmentsWindow::Init()
{
	...
	m_cQCString =
		new mxLineEdit2(this, left + 70, top, 400, 25,
						"$attachment \"controlpanel0_ur\" \"Vgui\" -22 -15 4 rotate 0 0 0", IDC_ATTACHMENT_QC_STRING);
}
...
int CAttachmentsWindow::handleEvent(mxEvent *event)
{
	...
	switch(event->action)
	{
		...
		case IDC_ATTACHMENT_LIST_BONES:
		{
			...
			if(iAttachment >= 0 && iAttachment < pHdr->GetNumAttachments() && iBone >= 0 && iBone < pHdr->numbones())
			{
...
Vector CAttachmentsWindow::GetCurrentTranslation()
{
	...
	if(pStudioHdr && iAttachment >= 0 && iAttachment < pStudioHdr->GetNumAttachments())
	{
		...
		return Vector(pAttachment.local[0][3], pAttachment.local[1][3], pAttachment.local[2][3]);
	}
	else
	{
		return vec3_origin;
...
void CAttachmentsWindow::UpdateStrings(bool bUpdateQC, bool bUpdateTranslation, bool bUpdateRotation)
{
	...
	else
	{
		...
		if(bUpdateQC)
		{
			sprintf(str, "$attachment \"%s\" \"%s\" %.2f %.2f %.2f rotate %.0f %.0f %.0f", pAttachment.pszName(),
					pHdr->pBone(iBone)->pszName(), VectorExpand(vTranslation), VectorExpand(vRotation));
