...
PAKViewer::PAKViewer(mxWindow *window) : mxWindow(window, 0, 0, 0, 0, "", mxWindow::Normal)
{
	strcpy(d_pakFile, "");
...
void _makeTempFileName(char *str, const char *suffix)
{
...
int PAKViewer::handleEvent(mxEvent *event)
{
	switch(event->event)
	{
		case mxEvent::Action:
					...
					else if(event->flags & mxEvent::DoubleClicked)
					{
						...
						if(mode == 0)
						{
							...
							if(!pak_ExtractFile(d_pakFile, d_currLumpName, str))
								mxMessageBox(this, "Error extracting from PAK file.", g_appTitle,
							...
							else
							{
								if(program)
								{
									...
									if((int)WinExec(path, SW_SHOW) <= 32)
										mxMessageBox(this, "Error executing specified program.", g_appTitle,
													 MX_MB_OK | MX_MB_ERROR);
								}
							}
						}
						...
						else if(mode == 1)
						{
							...
							if(!pak_ExtractFile(d_pakFile, d_currLumpName, str))
								mxMessageBox(this, "Error extracting from PAK file.", g_appTitle,
											 MX_MB_OK | MX_MB_ERROR);
							else if((int)ShellExecute((HWND)getHandle(), "open", str, 0, 0, SW_SHOW) <= 32)
								mxMessageBox(this, "Error executing document with associated program.", g_appTitle,
											 MX_MB_OK | MX_MB_ERROR);
						}
...
bool PAKViewer::openPAKFile(const char *pakFile)
{
	FILE *file = fopen(pakFile, "rb");
	if(!file)
		return false;
