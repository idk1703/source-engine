...
//
//--------------------------------------------------------------------------------------------------
static void WriteIndent(IBaseFileSystem *pFileSystem, FileHandle_t hFile, bool bOptTabs, int nOptTabWidth,
						int nIndentLevel)
{
	if(nIndentLevel <= 0)
		return;
...
//
//--------------------------------------------------------------------------------------------------
static void WritePadding(IBaseFileSystem *pFileSystem, FileHandle_t hFile, bool bOptTabs, int nOptTabWidth,
						 int nCurrentColumn, int nTargetColumn)
{
...
static int CleanFloatString(OUT_Z_ARRAY char (&pszFloatBuf)[maxLenInChars])
{
...
//
//--------------------------------------------------------------------------------------------------
static void SaveToFile_R(KeyValues *pkv, IBaseFileSystem *pFileSystem, FileHandle_t hFile, bool bOptTabs,
						 int nOptTabWidth, int nIndentLevel, int nValueColumn = -1)
{
...
//
//--------------------------------------------------------------------------------------------------
static bool SaveCleanKeyValuesToFile(KeyValues *pkv, IBaseFileSystem *pFileSystem, const char *pszFileName,
									 const char *pszPathID = nullptr, bool bOptTabs = true, int nOptSpaceIndent = 4)
{
...
void ProcessPaintKitKeyValuesFile(const char *pszPaintKitKeyValuesFile, bool bOptFix, bool bOptVerbose)
{
	...
	if(!V_IsAbsolutePath(pszPaintKitKeyValuesFile))
	{
		...
		if(!V_IsAbsolutePath(szFullPath))
		{
			...
			{
				Msg("Error: non-resolvable, non-absolute path specified (%s) and couldn't determine current "
					"directory\n",
					pszPaintKitKeyValuesFile);
				return;
			}
