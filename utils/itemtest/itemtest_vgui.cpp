...
public:
	CConsoleDialogNew(vgui::Panel *pPanel, const char *pszName) : CConsoleDialog(pPanel, pszName, false) {}
...
int CItemTestVGUIApp::Main()
{
	...
	if(!g_hConsoleDialog)
	{
		if(g_pTopPanel)
		{
			CConsoleDialogNew *pConsoleDialog = g_hMainFrame ? new CConsoleDialogNew(g_hMainFrame, "console")
