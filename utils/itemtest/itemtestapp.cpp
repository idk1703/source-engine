...
char CItemTestApp::s_szDesc[] = "itemtest is a command line utility to assist with packaging data "
...
char *CItemTestApp::s_pszFlags[][4] = {
	...
	{"-c", "-class", "<class>",
	 "Specify the class of the item.  One of: demo, engineer, heavy, medic, pyro, scout, sniper, soldier, spy"},
	{"-n", "-name", "<name>", "Specify the name of the item"},
	{"-d", "-dev", NULL, "Turn on dev mode"},
	{"-nop4", "-nop4", NULL,
	 "If -dev mode is specified optionally turn off perforce.  NOTE: Perforce is only enabled in dev mode."},
	...
	{"-lm", "-listmats", NULL,
	 "If some lod files are specified and this option is specified, the names of the materials found in the LOD files "
	 "are printed to stdout and the program exits, all texture flags are ignored.  The material names are the "
	 "arguments needed to -mat"},
	{"-m", "-mat", "<name>", "Specify a material.  <name> is the name of the material from the geometry file"},
	{"-mt", "-mattype", "<type>",
	 "Category of previously specified material, one of: primary, secondary, duplicate_primary, duplicate_secondary"},
	...
	{"-at", "-alphatype", "<type>",
	 "Category of alpha data of previously specified texture, one of: none, transparency, paintable, spec"},
...
bool CItemTestApp::PreInit()
{
...

	{
		...
		if(CItemUpload::GetSourceSDKFromExe(sSourceSDKPath, sSourceSDKBin) &&
		   CItemUpload::GetSteamAppInstallLocation(sSteamAppInstallLocation, nAppId))
		{
...
bool CItemTestApp::ProcessCommandLine(CAsset *pAsset, bool bDoListMats)
{
	...
	else
	{
		...
		if(g_pFullFileSystem->RelativePathToFullPath("workshop", "GAME", pszWorkshopPath, sizeof(pszWorkshopPath)) &&
		   !sFileName.IsEmpty())
		{
			sFileName += ".zip";
	...
	for(int i = 0; i < nParamCount; ++i)
	{
		...
		else if(!V_stricmp(pszFlag, s_pszFlags[kMatType][kShortFlag]) ||
				!V_stricmp(pszFlag, s_pszFlags[kMatType][kLongFlag]))
		{
			if(!pTargetVmt)
			{
			...
			{
				Warning("Error! Invalid Parameter Value: \"%s\" \"%s\", expected one of primary, secondary, "
						"duplicate_primary, duplicate_secondary\n",
						pszFlag, pszMatType);
			}
		}
		...
		else if(!V_stricmp(pszFlag, s_pszFlags[kTexType][kShortFlag]) ||
				!V_stricmp(pszFlag, s_pszFlags[kTexType][kLongFlag]))
		{
			if(!pTargetVmt)
			{
				Warning("Error! No -mat specified before \"%s\"\n", pszFlag);
			...
			{
				pTargetVmt->SetTargetVTF(
					"_color", pszTex,
					CItemUpload::Manifest()->GetMaterialSkin("red")); // Comes from texture_types in manifest
			}
			else if(StringHasPrefix(pszTexType, "B"))
			{
				pTargetVmt->SetTargetVTF(
					"_color", pszTex,
					CItemUpload::Manifest()->GetMaterialSkin("blue")); // Comes from texture_types in manifest
			}
		}
		else if(!V_stricmp(pszFlag, s_pszFlags[kAlphaType][kShortFlag]) ||
				!V_stricmp(pszFlag, s_pszFlags[kAlphaType][kLongFlag]))
		{
			if(!pTargetVmt)
			{
				Warning("Error! No -mat specified before \"%s\"\n", pszFlag);
			...
			else if(StringHasPrefix(pszTexType, "T"))
			{
				if(bTexNormal)
				{
					Warning("Error! Command line switch \"%s\" \"%s\" specified after a \"normal\" texture, only "
							"applies to color textures\n",
							pszFlag, pszTexType);
					continue;
				}
				else
...
void CItemTestApp::PrintHelp()
{
	...
	for(int i = 0; i < kFlagsCount; ++i)
	{
		...
		sFlags.sprintf(sFmt, s_pszFlags[i][kShortFlag], s_pszFlags[i][kLongFlag], s_pszFlags[i][kArgDesc] ? " " : "",
					   s_pszFlags[i][kArgDesc] ? s_pszFlags[i][kArgDesc] : "");
