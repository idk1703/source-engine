...
public:
	...
	bool Add(char const *szString)
	{
		return insert(szString).second;
	}
	void Remove(char const *szString)
	{
		erase(szString);
	}
...
public:
	...
	EVAL
	{
		return (m_nSlot >= 0) ? pCtx->GetVariableValue(m_nSlot) : 0;
	};
	PRNT
	{
		(m_nSlot >= 0) ? OutputF(stdout, "$%s", pCtx->GetVariableName(m_nSlot)) : OutputF(stdout, "$**@**");
	}
...
public:
	EVAL
	{
		return m_pRoot ? m_pRoot->Evaluate(pCtx ? pCtx : m_pContext) : 0;
	};
	PRNT
	{
		OutputF(stdout, "[ ");
...
public:
	...
	Define const *const GetDefinesBase(void)
	{
		return m_arrDefines.ArrayBase();
	}
	...
	// IEvaluationContext
public:
	virtual int GetVariableValue(int nSlot)
	{
...
namespace ConfigurationProcessing
{
	...
	public:
		...
		bool IsSkipped(void)
		{
			return (m_pEntry->m_pExpr->Evaluate(this) != 0);
		}
	...
	bool ComboHandleImpl::AdvanceCommands(uint64 &riAdvanceMore)
	{
		...
		for(pSetValues = pnValues, pSetDef = pDefVars; (pSetValues < pnValuesEnd) && (riAdvanceMore > 0);
			++pSetValues, ++pSetDef)
		{
	...
	void ProcessConfiguration()
	{
		...
		for(std::multiset<CfgEntry>::reverse_iterator it = s_setEntries.rbegin(), itEnd = s_setEntries.rend();
			it != itEnd; ++it)
		{
			...
			for(uint64 iRecord = nCurrentCommand + iPartStep; iRecord < nCurrentCommand + chi.m_numCombos;
				iRecord += iPartStep)
			{
...
namespace CfgProcessor
{
	...
	{
		CAutoPushPop<CUtlInplaceBuffer *> pushInputStream(ConfigurationProcessing::GetInputStream(fInputStream),
														  fInputStream);
		ConfigurationProcessing::ProcessConfiguration();
	}
	...
	ComboHandle Combo_GetNext(uint64 &riCommandNumber, ComboHandle &rhCombo, uint64 iCommandEnd)
	{
		...
		if(!rhCombo)
		{
			...
			CPCHI_t const &chiFound =
				ConfigurationProcessing::s_mapComboCommands.GetLessOrEq(iCommandFound, emptyCPCHI);
		...
		for(;;)
		{
			...
			CPCHI_t const &chiNext =
				ConfigurationProcessing::s_mapComboCommands.GetLessOrEq(iCommandLookup, emptyCPCHI);
