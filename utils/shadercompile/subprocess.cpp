...
SubProcessKernelObjects::SubProcessKernelObjects(void) : m_hMemorySection(NULL), m_hMutex(NULL)
{
...
BOOL SubProcessKernelObjects::Create(char const *szBaseName)
{
	...
	m_hMemorySection = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 4 * 1024 * 1024,
										 chBufferName); // 4Mb for a piece
...
CSubProcessResponse::CSubProcessResponse(void const *pvMemory) : m_pvMemory(pvMemory)
{
...
{
	// Subprocesses just silently die in our case, then this case will be detected by the worker process and an error
	// code will be passed to the master
...
int ShaderCompile_Subprocess_Main(char const *szSubProcessData)
{
	...
	for(void *pvMemory = NULL; NULL != (pvMemory = shrmem.Lock()); shrmem.Unlock())
	{
