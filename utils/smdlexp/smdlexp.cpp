...
int SmdExportClass::DoExport(const TCHAR *name, ExpInterface *ei, Interface *i, BOOL suppressPrompts, DWORD options)
{
	...
	if(!suppressPrompts)
	{
		...
		if(DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_EXPORTOPTIONS), GetActiveWindow(), ExportOptionsDlgProc,
						  (LPARAM)this) <= 0)
...
int DumpNodesTEP::callback(INode *pnode)
{
	...
	m_phec->m_rgmaxnode[iNode].isMirrored =
		DotProd(CrossProd(m_phec->m_rgmaxnode[iNode].mat3ObjectTM.GetRow(0).Normalize(),
						  m_phec->m_rgmaxnode[iNode].mat3ObjectTM.GetRow(1).Normalize())
					.Normalize(),
				m_phec->m_rgmaxnode[iNode].mat3ObjectTM.GetRow(2).Normalize()) < 0;
...
int DumpFrameRotationsTEP::callback(INode *pnode)
{
	...
	if(m_phec->m_rgmaxnode[iNode].imaxnodeParent >= 0 &&
	   m_phec->m_rgmaxnode[m_phec->m_rgmaxnode[iNode].imaxnodeParent].isMirrored)
	{
...
int DumpModelTEP::callback(INode *pnode)
{
	...
	pobj = os.obj;
	TriObject *ptriobj;
	BOOL fConvertedToTriObject = pobj->CanConvertToType(triObjectClassID) &&
	...
	for(int iFace = 0; iFace < cFaces; iFace++)
	{
		...
		if(fHasMat)
		{
			...
			if(ptexmap != NULL)
			{
				...
				{
					sprintf(st_szDBG,
							"ERROR--Sub-material with index %d (used in node %s) doesn't have a bitmap as its diffuse "
							"texture.",
		...
		{
			// sprintf(st_szDBG, "ERROR--Node %s has a textureless face.  All faces must have an applied texture.",
			// (char*)strNodeName); ASSERT_AND_ABORT(FALSE, st_szDBG);
		}
		...
		if (stricmp(szBitmapName+strlen(szBitmapName)-strlen(szExpectedExtension), szExpectedExtension) != 0)
			{
			sprintf(st_szDBG, "Node %s uses %s, which is not a %s file", (char*)strNodeName, szBitmapName,
		szExpectedExtension); ASSERT_AND_ABORT(FALSE, st_szDBG);
			}
		*/
		...
		fprintf(m_pfile, "%s\n", szBitmapName);
		fprintf(m_pfile, "%3d %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f", iNodeV0, v0.x, v0.y, v0.z,
				pt3Vertex0Normal.x, pt3Vertex0Normal.y, pt3Vertex0Normal.z, UVvertex0.x, UVvertex0.y);
		DumpWeights(iVertex0);
		fprintf(m_pfile, "%3d %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f", iNodeV1, v1.x, v1.y, v1.z,
				pt3Vertex1Normal.x, pt3Vertex1Normal.y, pt3Vertex1Normal.z, UVvertex1.x, UVvertex1.y);
		DumpWeights(iVertex1);
		fprintf(m_pfile, "%3d %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f", iNodeV2, v2.x, v2.y, v2.z,
				pt3Vertex2Normal.x, pt3Vertex2Normal.y, pt3Vertex2Normal.z, UVvertex2.x, UVvertex2.y);
...
Point3 DumpModelTEP::Pt3GetRVertexNormal(RVertex *prvertex, DWORD smGroupFace)
{
	...
	ASSERT_MBOX((cNormals == 1 && prvertex->ern == NULL) || (cNormals > 1 && prvertex->ern != NULL), "BOGUS RVERTEX");
