...
//
//-----------------------------------------------------------------------------
CDmxEdit::CDmxEdit() : m_pRoot(NULL), m_pMesh(NULL), m_pCurrentSelection(NULL), m_distanceType(CDmeMesh::DIST_ABSOLUTE)
{
}
	...
	if(sLen > 4 && !Q_stricmp(pFilename + sLen - 4, ".dmx"))
	{
		...
		if(pDag)
		{
			...
			if(!m_pMesh)
			{
				...
				{
					m_pMesh = pFirstMesh;
					LuaWarning("Cannot Find A DmeMesh With Any Delta States In File, Using First Found Mesh %s",
							   m_pMesh->GetName());
				}
				else
				{
...
//-----------------------------------------------------------------------------
LUA_COMMAND(Load, "< $file.dmx | $file.obj >, [ $loadType ]",
			"Replaces the current scene with the specified scene.  The current mesh will be set to the first mesh with "
			"a combination operator found in the new scene.  $loadType is one of \"absolute\" or \"relative\".  If not "
			"specified, \"absolute\" is assumed.")
{
...
//-----------------------------------------------------------------------------
LUA_COMMAND(Import, "< $file.dmx >, [ $parentBone ]",
			"Imports the specified DMX model into the scene.  The imported model can optionally be parented (which "
			"implictly skins it) to a specified bone in the existing scene via $parentBone.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ListDeltas, "", "Prints a list of all of the deltas present in the current mesh")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(DeltaCount, "", "Returns the number of deltas in the current mesh")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(DeltaName, "< #deltaIndex >",
			"Returns the name of the delta at the specified index.  Use DeltaCount() to get a count of how many deltas "
			"are present")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ImportComboRules, "< $rules.dmx >, [ #bOverwrite ], [ #bPurgeDeltas ]",
			"Imports the combintion rules from the specified dmx file and replaces the current rules with those from "
			"the file is #bOverwrite specified or is true.  If #bOverwrite is false then any existing controls that "
			"are not in the imported rules file are preserved.  If #bPurgeDeltas is specified and is true, then any "
			"delta states which are no longer referred to by any combination rule or control will be purged. ")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//-----------------------------------------------------------------------------
void CDmxEdit::ImportDominationRules(CDmeCombinationOperator *pDestComboOp, CDmeCombinationOperator *pSrcComboOp,
									 bool bOverwrite)
{
	if(bOverwrite)
	{
...
{
	if(pCombo->GetRawControlCount(nIndex) == 1 &&
	   !Q_strcmp(pCombo->GetControlName(nIndex), pCombo->GetRawControlName(nIndex, 0)))
		return true;
...
//-----------------------------------------------------------------------------
void CDmxEdit::ImportCombinationControls(CDmeCombinationOperator *pSrcComboOp, CDmeCombinationOperator *pDstComboOp,
										 bool bOverwrite)
{
	if(bOverwrite)
	{
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(SetState, "< $delta >", "Sets the current mesh to the specified $delta")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ResetState, "", "Resets the current mesh back to the default base state, i.e. no deltas active")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const char *pSelectTypeString, CDmeSingleIndexedComponent *pPassedSelection /* = NULL */,
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(Select, "< $delta >, [ $type ]",
			"Changes the selection based on the vertices present in the specified $delta.  $type is one of \"add\", "
			"\"subtract\", \"toggle\", \"intersect\", \"replace\".  If $type is not specified, \"add\" is assumed.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(CDmeVertexDeltaData *pDelta, CDmeSingleIndexedComponent *pPassedSelection /* = NULL */,
					  CDmeMesh *pPassedMesh /* = NULL */)
{
	if(!pDelta)
...
// Selects vertices of the working mesh that change position in the give delta
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const CSelectType &selectType, CDmeSingleIndexedComponent *pPassedSelection /* = NULL */,
					  CDmeMesh *pPassedMesh /* = NULL */)
{
	if(selectType() == CSelectType::kDelta)
...
// Selects vertices of the working mesh that change position in the give delta
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const CSelectOp &selectOp, CDmeVertexDeltaData *pDelta,
					  CDmeSingleIndexedComponent *pPassedSelection /* = NULL */, CDmeMesh *pPassedMesh /* = NULL */)
{
	if(!pDelta)
		return SetErrorString("Invalid Delta");
	...
	CDmeSingleIndexedComponent *pTempSelection =
		CreateElement<CDmeSingleIndexedComponent>("tempSelection", pSelection->GetFileId());
	if(!pTempSelection)
		return false;
...
// Selects vertices of the working mesh that change position in the give delta
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const CSelectOp &selectOp, const CSelectType &selectType,
					  CDmeSingleIndexedComponent *pPassedSelection /* = NULL */, CDmeMesh *pPassedMesh /* = NULL */)
{
	CDmeSingleIndexedComponent *pSelection = pPassedSelection ? pPassedSelection : m_pCurrentSelection;
...
//-----------------------------------------------------------------------------
bool CDmxEdit::Select(const CSelectOp &selectOp, CDmeSingleIndexedComponent *pOriginal,
					  const CDmeSingleIndexedComponent *pNew)
{
	...
	switch(selectOp())
	{
		...
		{
			CDmeSingleIndexedComponent *pIntersection =
				CreateElement<CDmeSingleIndexedComponent>("intersection", pOriginal->GetFileId());
			if(!pIntersection)
				return false;

			CDmeSingleIndexedComponent *pNewCopy =
				CreateElement<CDmeSingleIndexedComponent>("newCopy", pOriginal->GetFileId());
			if(!pNewCopy)
				return false;
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::SelectHalf(const CSelectOp &selectOp, const CHalfType &halfType,
						  CDmeSingleIndexedComponent *pPassedSelection /* = NULL */, CDmeMesh *pPassedMesh /* = NULL */)
{
	CDmeSingleIndexedComponent *pSelection = pPassedSelection ? pPassedSelection : m_pCurrentSelection;
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::SelectHalf(const CHalfType &halfType, CDmeSingleIndexedComponent *pPassedSelection /* = NULL */,
						  CDmeMesh *pPassedMesh /* = NULL */)
{
	CDmeMesh *pMesh = pPassedMesh ? pPassedMesh : m_pMesh;
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(SelectHalf, "< $halfType >",
			"Selects all the vertices in half the mesh.  Half is one of left or right.  Left means the X value of the "
			"position is >= 0, right means that the X value of the position is <= 0.  So it's the character's left "
			"(assuming they're staring down -z)")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(GrowSelection, "< #size >", "Grows the current selection by the specified $size")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ShrinkSelection, "< #size >", "Shrinks the current selection by the specified $size (integer)")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Add(AddType addType, const CDmxEditProxy &e, float weight /* = 1.0f */,
				   float featherDistance /* = 0.0f */, const CFalloffType &falloffType,
				   const CDistanceType &passedDistanceType)
{
	...
	const CDistanceType &distanceType =
		passedDistanceType() == CDmeMesh::DIST_DEFAULT ? m_distanceType : passedDistanceType;

	CDmeSingleIndexedComponent *pNewSelection =
		featherDistance > 0.0f
			? m_pMesh->FeatherSelection(featherDistance, falloffType(), distanceType(), m_pCurrentSelection, NULL)
			: NULL;
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Add(AddType addType, const char *pDeltaName, float weight /* = 1.0f */,
				   float featherDistance /* = 0.0f */, const CFalloffType &falloffType,
				   const CDistanceType &passedDistanceType)
{
	...
	const bool retVal = addType == kRaw ? m_pMesh->AddMaskedDelta(pDelta, NULL, weight, pNewSelection)
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(Add, "< $delta >, [ #weight ], [ #featherDistance ], < $falloffType >",
			"Adds specified state to the current state of the mesh weighted and feathered by the specified #weight, "
			"#featherDistance & $falloffType.  "
			"$falloffType is one of \"straight\", \"spike\", \"dome\", \"bell\".  Note that only the specified delta "
			"is added.  "
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(AddCorrected, "< $delta >, [ #weight ], [ #featherDistance ], < $falloffType >",
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Interp(const CDmxEditProxy &e, float weight /* = 1.0f */, float featherDistance /* = 0.0f */,
					  const CFalloffType &falloffType, const CDistanceType &passedDistanceType)
{
	if(!m_pMesh)
		return SetErrorString("No Mesh");
	...
	const bool retVal =
		m_pMesh->InterpMaskedDelta(NULL, NULL, weight, pNewSelection ? pNewSelection : m_pCurrentSelection);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Interp(const char *pDeltaName, float weight /* = 1.0f */, float featherDistance /* = 0.0f */,
					  const CFalloffType &falloffType, const CDistanceType &passedDistanceType)
{
	if(!Q_stricmp("BASE", pDeltaName))
	...
	const bool retVal =
		m_pMesh->InterpMaskedDelta(pDelta, NULL, weight, pNewSelection ? pNewSelection : m_pCurrentSelection);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(Interp, "< $delta >, [ #weight ], [ #featherDistance ], < $falloffType >",
			"Interpolates the current state of the mesh towards the specified state, $weight, #featherDistance and "
			"$falloffType.  $falloffType is one of \"straight\", \"spike\", \"dome\", \"bell\"")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Save(const char *pFilename, const CObjType &saveType /* = ABSOLUTE */,
					const char *pDeltaName /* = NULL */)
{
	if(!pFilename)
...
//-----------------------------------------------------------------------------
LUA_COMMAND(Save, "[ $file.dmx | $file.obj ], [ $saveType ], [ $delta ]",
			"Saves the current scene to either a single dmx file or a sequence of obj files.  If $file is not "
			"specified, the filename used in the last Load() command will be used to save the file.  $saveType is one "
			"of \"absolute\" or \"relative\".  If not specified, \"absolute\" is assumed.  If $delta is passed and the "
			"save type is OBJ, then only a single OBJ of that delta is saved.  \"base\" is the base state.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(Merge, "< $src.dmx >, < $dst.dmx >",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(SaveDelta, "< $delta >", "Saves the current state of the mesh to the named delta state")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(DeleteDelta, "< $delta >", "Deletes the named delta state from the current mesh")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(RemapMaterial, "< #index | $srcMaterial >, < $dstMaterial >",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(RemoveFacesWithMaterial, "< $material >",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(RemoveFacesWithMoreThanNVerts, "< #vertexCount >",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(Mirror, "[ $axis ]",
			"Mirrors the mesh in the specified axis.  $axis is one of \"x\", \"y\", \"z\". If $axis is not specified, "
			"\"x\" is assumed (i.e. \"x\" == mirror across YZ plane)")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ComputeNormals, "", "Computes new smooth normals for the current mesh and all of its delta states.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
LUA_COMMAND(
	...
	"If #bOverwrite is true then all wrinkle data will be computed from the combo rules wiping out any existing data "
	"computed using ComputeWrinkle().  ")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ComputeWrinkle, "< $delta > [ , #scale = 1 [ , $operation ] ] ",
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::CreateDeltasFromPresets(const char *pPresetFilename, bool bPurge /* = true */,
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(CreateDeltasFromPresets,
			"< $preset.pre >, [ #bPurgeExisting = true ], [ { \"NoPurge1\", ... } ], [ $expression.txt ]",
			"Creates a new delta state for every present defined in the specified preset file.  #bPurgeExisting "
			"specifies whether existing controls should be purged.  If #bPurgeExisting is true, then an optional array "
			"of deltas to not purge can be specified.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
LUA_COMMAND(
	...
	"Caches the specified $preset file for later processing by a call to CreateDeltasFromCachedPresets().  If an "
	"$expression.txt file pathname is specified, an expression file will be written out for this preset file")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ClearPresetCache, "", "Removes all preset filenames that have been specified via CachePreset() calls")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::CreateDeltasFromCachedPresets(bool bPurge /* = true */,
...
LUA_COMMAND(
	...
	"Creates a new delta state for every preset defined in the preset files specified by previous CachePreset() calls. "
	" Calling this calls ClearPresetCache().  #bPurgeExisting specifies whether existing controls should be purged.  "
	"If #bPurgeExisting is true, then an optional array of deltas to not purge can be specified.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(CreateExpressionFileFromPresets, "< $preset.pre >, < $expression.txt >",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(CreateExpressionFilesFromCachedPresets, "",
			"Creates a faceposer expression.txt and expression.vfe file for each of the preset/expression file pairs "
			"specified by previous CachePreset calls")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
void ScaleDeltaPositions(const CDmrArrayConst<Vector> &bindPosData, CDmeVertexDeltaData *pDelta, float sx, float sy,
						 float sz)
{
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(Scale, "< #xScale >, [ #yScale, #zScale ]",
			"Scales the position values by the specified amount.  If only 1 scale value is supplied a uniform scale in "
			"all three dimensions is applied")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
LUA_COMMAND(
	...
	"Sets the way distances will be interpreted after the command.  $distanceType is one of \"absolute\" or "
	"\"relative\".  By default distances are \"absolute\".  All functions that work with distances (Add, Interp and "
	"Translate) work on the currently selected vertices.  \"absolute\" means use the distance as that number of units, "
	"\"relative\" means use the distance as a scale of the radius of the bounding sphere of the selected vertices.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Translate(Vector t, float featherDistance, const CFalloffType &falloffType,
						 const CDistanceType &passedDistanceType, CDmeMesh *pPassedMesh, CDmeVertexData *pPassedBase,
						 CDmeSingleIndexedComponent *pPassedSelection)
{
	...
	CDmeSingleIndexedComponent *pNewSelection =
		(pSelection && featherDistance > 0.0f)
			? m_pMesh->FeatherSelection(featherDistance, falloffType(), distanceType(), pSelection, NULL)
			: pSelection;
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(Translate, "< #xTranslate, #yTranslate, #zTranslate >, [ #falloffDistance, $falloffType ]",
...
//
//-----------------------------------------------------------------------------
bool CDmxEdit::Rotate(Vector r, // X, Y, Z Euler angles in degrees specified by user
					  Vector o, // Offset
					  float featherDistance /* = 0.0f */, const CFalloffType &falloffType /* = STRAIGHT */,
					  const CDistanceType &passedDistanceType /* = DIST_DEFAULT */, CDmeMesh *pPassedMesh /* = NULL */,
...
LUA_COMMAND(Rotate,
			...
			"Rotates the current selection around the center of the selection by the specified Euler angles (in "
			"degrees).  The optional offset is added to the computed pivot point.")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
	...
	else
	{
		...
		else
		{
			...
			if(LuaFunc_s::m_dmxEdit.Rotate(Vector(rx, ry, rz), Vector(ox, oy, oz), flFeatherDistance, pFalloffType,
										   pDistanceType))
				return LuaFunc_s::m_dmxEdit.LuaOk(pLuaState);
		}
	}
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(FileExists, "< $filename >", "Returns true if the given filename exists on disk, false otherwise")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//-----------------------------------------------------------------------------
bool HasDuplicateControlName(CDmeCombinationOperator *pDmeCombo, const char *pControlName,
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(GroupControls, "< $groupName > [ , $rawControlName ... ]",
			"Groups the specified raw controls under a common group control.  If the group control already exists, the "
			"raw controls are added to it")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(ReorderControls, "< $controlName [ , ... ] >",
...
bool CDmxEdit::AddDominationRule(CUtlVector<CUtlString> &dominators, CUtlVector<CUtlString> &supressed)
{
	...
	pDmeCombo->AddDominationRule(tmpDominators.Count(), (const char **)tmpDominators.Base(), tmpSupressed.Count(),
								 (const char **)tmpSupressed.Base());
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(AddDominationRule, "{ < $dominator > [ , ... ] }, { < $supressed > [ , ... ] }",
...
bool CDmxEdit::SetStereoControl(const char *pControlName, bool bStereo)
{
	...
	if(nControlIndex < 0)
		return SetErrorString("No Control Named \"%s\" On DmeCombinationOperator \"%s\" On Mesh \"%s\"", pControlName,
							  pDmeCombo->GetName(), m_pMesh->GetName());
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(SetStereoControl, " < $controlName > [ , < true | false > ]",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(SetEyelidControl, " < $controlName > [ , < true | false > ]",
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(MaxDeltaDistance, " < $delta >",
			"Returns the maximum distance any vertex moves in the specified delta.  Returns 0 if the delta specified "
			"doesn't exist")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(DeltaRadius, " < $delta >",
			"Returns the radius of the specified delta, if it exists.  Radius of a delta is defined as the radius of "
			"the tight bounding sphere containing all of the deltas added to their base state values")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
//
//-----------------------------------------------------------------------------
LUA_COMMAND(SelectionRadius, "",
			"Returns the radius of the currently selected vertices, if nothing is selected, returns 0.  Radius of a "
			"the vertices defined as the radius of the tight bounding sphere containing all of the vertices")
{
	LuaFunc_s::m_dmxEdit.SetFuncString(pLuaState);
...
bool CDmxEdit::SetWrinkleScale(const char *pControlName, const char *pRawControlName, float flScale)
{
	...
	if(!bFoundRawControl)
	{
		...
		for(int nRawControlIndex = 0; nRawControlIndex < pDmeCombo->GetRawControlCount(nControlIndex);
			++nRawControlIndex)
		{
		...
		return SetErrorString("Control \"%s\" Does Not Have Raw Control \"%s\", Raw Controls: %s ", pControlName,
							  pRawControlName, rawControls.Get());
	}
...
LUA_COMMAND(
	...
	"The function to compute a single delta's wrinkle values, ComputeWrinkle() does not use the wrinkle scale value at "
	"all.  "
...
{
public:
	CPauseOnScopeExit(CDmxEdit &dmxEdit) : m_dmxEdit(dmxEdit) {}
...
//
//-----------------------------------------------------------------------------
CDmxEditLua::CDmxEditLua() : m_pLuaState(lua_open())
{
	...
	const char *pLuaInit = "vsLuaDir = os.getenv( \"VPROJECT\" );\n"
