...
//   see if localized checksum tag differs from US one, or warn if tag missing, but complain if .wav duration is
//   different.
//
...
bool LoadSentenceFromWavFileUsingIO(char const *wavfile, CSentence &sentence, IFileReadBinary &io,
									void *formatbuffer = NULL, int *formatsize = NULL, int *datasize = NULL)
{
	int insize = 0;
...
bool LoadSentenceFromWavFile(char const *wavfile, CSentence &sentence, void *formatbuffer = NULL,
							 int *formatsize = NULL, int *dataSize = NULL)
{
	return LoadSentenceFromWavFileUsingIO(wavfile, sentence, io_in, formatbuffer, formatsize, dataSize);
}

bool ValidateCombinedFileCheckSum(char const *outfilename, char const *cctoken, gender_t gender,
...
void CheckDuplcatedText(void)
{
	...
	for(int lang = 0; lang < CC_NUM_LANGUAGES; ++lang)
	{
		...
		// Now check for closecaption_xxx.txt entries which are orphaned because there isn't an existing sound script
		// entry in use for them
...
void ExtractEnglish()
{
	...
	// Now check for closecaption_xxx.txt entries which are orphaned because there isn't an existing sound script entry
	// in use for them
	StringIndex_t str = g_pVGuiLocalize->GetFirstStringIndex();
	while(str != INVALID_LOCALIZE_STRING_INDEX)
	{
...
void SpewScript(char const *vcdname, CUtlRBTree<CChoreoEvent *, int> &list)
{
	...
	for(int i = list.FirstInorder(); i != list.InvalidIndex(); i = list.NextInorder(i))
	{
		...
		logprint("script.csv", "%u,%s,%s,%s,%6.3f,%s,%s,\"%s\",\"%s\"\n", sortindex++, mapname, vcdname, actorname,
				 e->GetStartTime(), tok, wavname, ansi, sentence_text);
	}
}
...
void ValidateForeignLanguageWaves(char const *language, CUtlVector<CUtlSymbol> &combinedwavfiles)
{
	...
	for(int i = 0; i < c; ++i)
	{
		...
		Q_snprintf(localizedwavename, sizeof(localizedwavename), "%s%s", langdir, &wavname[skipchars]);
...
void WavCheck(CUtlVector<CUtlSymbol> &wavfiles)
{
	...
	logprint("wavcheck.csv", "\"%s\",\"%s\",\"%s\",\"%s\"\n", "WaveName", "Sound Script Name", "Close Caption",
			 "Phoneme Data String");
...
void MakeBatchFile(CUtlVector<CUtlSymbol> &wavfiles, char const *pchFromdir, char const *pchTodir)
{
	...
	for(int i = 0; i < c; ++i)
	{
		...
		char fullname[512];
		Q_snprintf(fullname, sizeof(fullname), "%s/%s", pchTodir,
...
void ExtractPhonemesForWave(IPhonemeExtractor *extractor, char const *wavname)
{
	...
	int result = extractor->Extract(filename,
									dataSize, // (int)( m_pWaveFile->GetRunningLength() * m_pWaveFile->SampleRate() *
											  // m_pWaveFile->TrueSampleSize() ),
...
{
	OrderedCaption_t() : sym(UTL_INVAL_SYMBOL), commands(NULL), english(NULL), blankenglish(false) {}
