...
bool ReadJpeg(IFileSystem *pFileSystem, const char *pFilename, CUtlVector<unsigned char> &buf, int &width, int &height,
			  const char *pPathID)
{
	width = height = 0;
...
void DownsampleRGBToRGBAImage(CUtlVector<unsigned char> &srcData, int srcWidth, int srcHeight,
							  CUtlVector<unsigned char> &destData, int destWidth, int destHeight)
{
	int srcPixelSize = 3;
	int destPixelSize = 4;
	...
	for(int destY = 0; destY < scaledDestHeight; destY++)
	{
		unsigned char *pDestLine =
			&destData[(destY + destOffsetY) * destWidth * destPixelSize + (destOffsetX * destPixelSize)];
...
BOOL CFileSystemOpenDlg::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT &rect,
...
public:
	...
	virtual bool DoModal_WindowsDialog()
	{
		...
		const char *pFileName = m_pDialog->m_pFileSystem->RelativePathToFullPath(
			m_pDialog->m_CurrentDir, m_pDialog->m_PathIDString, pFileNameBuf, MAX_PATH);
		...
		while(dlg.DoModal() == IDOK)
		{
			...
			if(m_pDialog->m_pFileSystem->FullPathToRelativePath(dlg.GetPathName(), m_RelativeFilename,
