...
private:
	...
	void GenerateSFMFile(const SFMGenInfo_t &sfmGenInfo, const SFMInfo_t &info, studiohdr_t *pStudioHdr,
						 const char *pOutputDirectory, const char *pExportFacPath);
	...
	CDmeSoundClip *CreateSoundClip(CDmeFilmClip *pShot, const char *pAnimationSetName, const char *pGameSound,
								   studiohdr_t *pStudioHdr, CDmeGameSound **ppGameSound);
...
bool CSFMGenApp::Create()
{
	...
	IMaterialSystem *pMaterialSystem =
		reinterpret_cast<IMaterialSystem *>(FindSystem(MATERIAL_SYSTEM_INTERFACE_VERSION));
	if(!pMaterialSystem)
	{
...
void CSFMGenApp::SaveSFMFile(CDmElement *pRoot, const char *pOutputDir, const char *pFileName)
{
	...
	if(!g_pDataModel->SaveToFile(pFullPath, NULL, g_pDataModel->GetDefaultEncoding("sfm_session"), "sfm_session",
								 pRoot))
	{
...
// Generates a sound clip for the game sound
//-----------------------------------------------------------------------------
CDmeSoundClip *CSFMGenApp::CreateSoundClip(CDmeFilmClip *pShot, const char *pAnimationSetName, const char *pGameSound,
										   studiohdr_t *pStudioHdr, CDmeGameSound **ppGameSound)
{
	*ppGameSound = NULL;
...
// Builds the list of all controls in the animation set contributing to facial animation
//-----------------------------------------------------------------------------
void CSFMGenApp::BuildFacialControlList(CDmeFilmClip *pShot, CDmeAnimationSet *pAnimationSet,
...
// Creates a single sfm file
//-----------------------------------------------------------------------------
void CSFMGenApp::GenerateSFMFile(const SFMGenInfo_t &sfmGenInfo, const SFMInfo_t &info, studiohdr_t *pStudioHdr,
								 const char *pOutputDirectory, const char *pExportFacPath)
{
	CSFMSession session;
	session.Init();
	...
	if(pSoundClip)
	{
		...
		for(int i = 0; i < results.m_ApplyTags.Count(); ++i)
		{
			...
			CDmElement *tagElement =
				CreateElement<CDmElement>(ConvertPhoneme(tag->GetPhonemeCode()), pGameSound->GetFileId());
...
void CSFMGenApp::GenerateSFMFiles(SFMGenInfo_t &info)
{
	...
	else
	{
		...
		CUtlBuffer bufFilenames(0, 0,
								CUtlBuffer::TEXT_BUFFER); // Affected Files for reference (files with Phoneme data)
		for(int i = 0; i < nCount; ++i)
		{
			CSentence sSentence;
