...
{
	Options() : szFilename(""), bWriteInfo(false) {}
...
int WriteInfo()
{
	...
	if(inBuf.TellPut() && '\r' == ((unsigned char const *)inBuf.Base())[inBuf.TellPut() - 1])
	...
	if(!resFileInfo.m_res.m_pvData)
	{
		...
		unsigned char chStdFileInfo[] = {0x38, 0x42, 0x49, 0x4D, 0x04, 0x04, 0,	   0,	 0,	   0,
										 0x00, 0x07, 0x1C, 0x02, 0x00, 0x00, 0x02, 0x00, 0x02, 0};
	...
	if(descr.m_pvData)
	{
		...
		if(numDeltaBytes)
			BufferMove(descr.m_pvData + inBuf.TellPut(), descr.m_pvData + descr.m_numBytes,
					   pvBufBase + bufFile.TellPut() - descr.m_pvData - descr.m_numBytes);
	...
	if(numDeltaBytes & 1) // Odd number of bytes delta
	{
		...
		{
			++numDeltaBytes;
			BufferMove(resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes + 1,
					   resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes,
					   pvBufBase + bufFile.TellPut() - resFileInfo.m_res.m_pvData - resFileInfo.m_res.m_numBytes);
		...
		{
			--numDeltaBytes;
			BufferMove(resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes,
					   resFileInfo.m_res.m_pvData + resFileInfo.m_res.m_numBytes + 1,
					   pvBufBase + bufFile.TellPut() - resFileInfo.m_res.m_pvData - resFileInfo.m_res.m_numBytes - 1);
			bufFile.SeekPut(CUtlBuffer::SEEK_CURRENT, -1);
		}
	}
