...
void SetVProject(const char *pProjectName)
{
	...
	Sys_SetRegistryString(
		...
		"HKEY_CURRENT_USER\\Environment\\VProject" pGamedir);

	DWORD result;
	SendMessageTimeout(HWND_BROADCAST, WM_SETTINGCHANGE, 0, (LPARAM) "Environment", SMTO_ABORTIFHUNG, 0, &result);
}
...
BOOL CALLBACK ModifyDlg_Proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
			...
			SetWindowPos(hWnd, NULL, (width - (rect.right - rect.left)) / 2, (height - (rect.bottom - rect.top)) / 2, 0,
...
bool Startup()
{
	...
	g_hWnd = CreateWindow(VPROJECT_CLASSNAME, 0, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL,
						  g_hInstance, NULL);
...
cleanUp:
	...
	{
		char str[255];
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, GetLastError(), 0, str, 255,
					  NULL);
