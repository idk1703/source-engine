...
{
	fflush(stderr);
	printf("Usage: vpk [options] <command> <command arguments ...>\n"
...
{
	return (V_stristr(pcFileName, ".bat") || V_stristr(pcFileName, ".cmd") || V_stristr(pcFileName, ".com") ||
			V_stristr(pcFileName, ".dll") || V_stristr(pcFileName, ".exe") || V_stristr(pcFileName, ".msi") ||
			V_stristr(pcFileName, ".rar") || V_stristr(pcFileName, ".reg") || V_stristr(pcFileName, ".zip"));
}
...
static void AddFileToPack(CPackedStore &mypack, char const *pSrcName, int nPreloadSize = 0,
						  char const *pDestName = NULL)
{
	...
	ePackedStoreAddResultCode rslt =
		mypack.AddFile(pDestName, Min(fileSize, nPreloadSize), pData, fileSize, s_bMakeMultiChunk);
...
static void CheckLoadKeyFilesForSigning(CPackedStore &mypack)
{
	...
	mypack.SetKeysForSigning(bytesPrivateKey.Count(), bytesPrivateKey.Base(), bytesPublicKey.Count(),
							 bytesPublicKey.Base());
#else
...
private:
	...
	static int CompareBuildFileByOldPhysicalPosition(VPKBuildFile_t *const *pa, VPKBuildFile_t *const *pb)
	{
		...
		if(a->m_idxChunk < b->m_idxChunk)
			return -1;
		if(a->m_idxChunk > b->m_idxChunk)
			return +1;
		if(a->m_iOffsetInChunk < b->m_iOffsetInChunk)
			return -1;
		if(a->m_iOffsetInChunk > b->m_iOffsetInChunk)
			return +1;
		return 0;
...
void VPKBuilder::BuildSteamPipeFriendlyFromInputKeys()
{
			...
			for(KeyValues *i = m_pOldInputKeys; i; i = i->GetNextKey())
			{
				...
				if(iPreloadSizeFromControlFile != (int)bf.m_pOld->m_iPreloadSize)
					Error("File '%s' preload size mismatch in old KeyValues control file and pack file.\nThat control "
						  "file was probably not used to build the pack file\n",
						  szNameInVPK);
				...
				{
					printf("WARNING: Old control file entry '%s' does not have an MD5; we will have to compare file "
		...
		while(idxOldFile < m_vecOldFilesInChunkOrder.Count())
		{
					...
					else if(f.m_iNewSortIndex != iExpectedSortIndex)
					{
						   ...
						   iExpectedSortIndex < m_vecNewFilesInChunkOrder.Count())
						{
							...
							if(m_dictFiles[idxDictInserted].m_pOld == NULL)
								...
								sReasonCannotReuse.Format("Chunk reordered.  '%s' listed where '%s' used to be.",
			...
			if(sReasonCannotReuse.IsEmpty() && vecFilesToCompareContents.Count() > 0)
			{
				...
				FOR_EACH_VEC(vecFilesToCompareContents, i)
				{
					...
					{
						sReasonCannotReuse.Format("File '%s' changed.  (CRCs differs from %s.)",
	...
	for(int idxChunk = 0; idxChunk < nNewChunkCount; ++idxChunk)
	{
		...
		for(int idxFile = r.m_iFirstInputFile; idxFile <= r.m_iLastInputFile; ++idxFile)
		{
			...
			// Load the input file
			CUtlBuffer buf;
			if(!g_pFullFileSystem->ReadFile(bf->m_sNameOnDisk, NULL, buf) || buf.TellPut() != (int)f->m_iTotalSize)
			{
			...
			int nBytesWritten =
				g_pFullFileSystem->Write((byte *)buf.Base() + f->m_iPreloadSize, nBytesToWrite, fChunkWrite);
	...
	FOR_EACH_DICT(m_dictFiles, idxInDict)
	{
		...
		if(pOld)
		{
			if(pOld->m_iTotalSize != pNew->m_iTotalSize || pOld->m_iPreloadSize != pNew->m_iPreloadSize)
			{
				iNeedToUpdateFile = 1;
			}
		...
		// Load the file
		CUtlBuffer buf;
		if(!g_pFullFileSystem->ReadFile(bf->m_sNameOnDisk, NULL, buf) || buf.TellPut() != (int)pNew->m_iTotalSize)
		{
			Error("Error reading %s", bf->m_sNameOnDisk.String());
		}
...
void VPKBuilder::MapAllRangesToChunks()
{
	...
	for(;;)
	{
		...
		FOR_EACH_LL(m_llFileRanges, idxRange)
		{
			...
			{
				Assert(r->m_bKeepExistingFile);
				printf("Chunk %d is currently bigger than desired chunk size of %d bytes, but we're not splitting it "
...
void GenerateKeyPair(const char *pszBaseKeyName)
{
	...
	//
	//	if( !CCrypto::SymmetricEncrypt( rgubPrivateKey, cubPrivateKey, rgubEncryptedPrivateKey, &cubEncryptedPrivateKey,
	//(uint8 *)rgchPassphrase, k_nSymmetricKeyLen ) )
	//	{
	...
	if(!CCrypto::HexEncode(rgubPrivateKey, cubPrivateKey, rgchEncodedEncryptedPrivateKey,
						   Q_ARRAYSIZE(rgchEncodedEncryptedPrivateKey)))
	{
	...
	// Save private keyfile
	{
		...
		fprintf(f,
				"// Private key file.\n"
				"// This key can be used to sign files.  Third parties can verify your signature by using your public "
				"key.\n"
				"//\n"
				"// THIS KEY SHOULD BE KEPT SECRET\n"
				"//\n"
				"// You should share your public key freely, but anyone who has your private key will be able to "
				"impersonate you.\n"
				"private_key\n"
				"{\n"
				"\ttype \"rsa\"\n"
				...
				"\n"
				"\t// The public key that corresponds to this private key.  The public keyfile you can share with "
				"others is\n"
				"\t// saved in another file, but the key data is duplicated here to help you confirm which public key "
				"matches\n"
...
static void CheckSignature(const char *pszFilename)
{
	...
	if(s_sPublicKeyFile.IsEmpty())
	{
		...
		printf("Checking signature using public key in VPK.\n"
			   "\n"
	...
	switch(result)
	{
		...
		case CPackedStore::eSignatureCheckResult_WrongKey:
			...
			fflush(stderr);
			printf("The public key provided does not match the public\n"
...
static void CheckHashes(const char *pszFilename)
{
	...
	FOR_EACH_VEC(vecHashes, idx)
	{
		...
		if(filehash.m_cbFileLen != frac.m_cbChunkLen)
		{
			...
			fprintf(stderr, "    @%d: size mismatch.  Stored: %d  Computed: %d\n", frac.m_nFileFraction,
					frac.m_cbChunkLen, filehash.m_cbFileLen);
			fflush(stderr);
...
void BuildRecursiveFileList(const char *pcDirName, CUtlStringList &fileList)
{
	...
	if(pcResult)
	{
		do
		{
			...
			V_snprintf(szFullResultPath, sizeof(szFullResultPath), "%s%c%s", pcDirName, CORRECT_PATH_SEPARATOR,
					   pcResult);
...
static void DroppedVpk(const char *pszVpkFilename)
{
	...
	for(int i = 0; i < fileNames.Count(); i++)
	{
		...
		V_snprintf(szDestFilePath, sizeof(szDestFilePath), "%s%c%s", szVPKParentDir, CORRECT_PATH_SEPARATOR,
				   fileNames[i]);
