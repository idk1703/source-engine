...
LRESULT CWMPHost::OnSize(UINT /* uMsg */, WPARAM wParam, LPARAM lParam, BOOL & /* lResult */)
{
	...
	else
	{
		...
		RECT rcClient;
		GetClientRect(&rcClient);
		m_wndView.MoveWindow(rcClient.left, rcClient.top, rcClient.right - rcClient.left,
	...
	switch(wParam)
	{
		...
		case VK_END:
		{
			...
			spWMPControls->put_currentPosition(
				duration - 0.050); // back a little more than a frame - this avoids triggering the end fade
			spWMPControls->play();
			spWMPControls->pause();
