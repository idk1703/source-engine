...
OSVersion DetectOSVersion()
{
	...
	if(version.dwPlatformId & VER_PLATFORM_WIN32_WINDOWS)
	{
		...
		switch(version.dwMinorVersion)
		{
			...
			case 10:
				return (strstr(version.szCSDVersion, _TEXT(" A")) == version.szCSDVersion) ||
							   (strstr(version.szCSDVersion, _TEXT(" B")) == version.szCSDVersion)
						   ? OSV_98SE
						   : OSV_98;
			case 90:
				return OSV_ME;
...
void SetRegistryValue(const char *pKeyName, const char *pValueName, DWORD dwValue, DWORD &dwOldValue,
					  bool &bValueExisted)
{
	HKEY hKey = 0;
	LONG rval = RegCreateKeyEx(HKEY_CURRENT_USER, pKeyName, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL,
							   &hKey, NULL);
...
struct EventData_t
{
	EventData_t(int t, float pos, EventType_t e) : time(t), position(pos), event(e) {}
	int time;		   // real time
...
{
	HKEY hKey = 0;
	LONG rval = RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\Valve\\Steam", 0, NULL, REG_OPTION_NON_VOLATILE,
...
void RestoreRegistry()
{
	...
	s_bDone = true;
	RestoreRegistryValue("Software\\Microsoft\\MediaPlayer\\Preferences\\VideoSettings", "UseVMROverlay",
						 g_dwUseVMROverlayOldValue, g_bUseVMROverlayValueExists);
}
...
void InitTextureStageState(int nStage, DWORD dwColorOp, DWORD dwColorArg1, DWORD dwColorArg2,
...
bool InitD3D(HWND hWnd, bool blur)
{
	...
	if(FAILED(g_pD3D->CreateDevice(nAdapterIndex, D3DDEVTYPE_HAL, hWnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3dpp,
								   &g_pd3dDevice)))
	{
	...
	if(FAILED(g_pd3dDevice->CreateVertexBuffer(sizeof(drawverts), D3DUSAGE_WRITEONLY, g_dwDrawFVF, D3DPOOL_MANAGED,
											   &g_pDrawVB)))
	{
	...
	if(blur)
	{
		...
		if(FAILED(g_pd3dDevice->CreateVertexBuffer(sizeof(blurverts), D3DUSAGE_WRITEONLY, g_dwBlurFVF, D3DPOOL_MANAGED,
												   &g_pBlurVB)))
		{
	...
	if(FAILED(
		   g_pd3dDevice->CreateTexture(g_screenWidth, g_screenHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &g_pImg)))
	{
	...
	if(GetDIBits(g_hdcCapture, g_hbmCapture, 0, g_screenHeight, lockedRect.pBits, &bitmapInfo, DIB_RGB_COLORS) !=
	   g_screenHeight)
	{
	...
	if(blur)
	{
		if(FAILED(g_pd3dDevice->CreateTexture(g_screenWidth, g_screenHeight, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8,
											  D3DPOOL_DEFAULT, &g_pTex)))
		{
	...
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			SetTimer(hWnd, ID_SKIP_FADE_TIMER, 1000,
					 NULL); // if the fade doesn't start in 1 second, then just jump to the video
			...
			if(wparam == ID_DRAW_TIMER)
			{
				...
				if(g_timeAtFadeStart == 0.0)
				{
					...
					SetTimer(hWnd, ID_SKIP_FADE_TIMER, 100 + UINT(FADE_TIME * 1000),
							 NULL); // restart skip fade timer and give it an extra 100ms to allow the fade to draw
									// fully black once
				}
				...
				/*
				   ...
				   g_nTimingIndex - 1 ], bFadeFinished ? "true" : "false" ); OutputDebugString( str );
				*/
...
*/
				...
				if(!::AlphaBlend(g_hdcBlend, 0, 0, g_screenWidth, g_screenHeight, g_hdcCapture, 0, 0, g_screenWidth,
								 g_screenHeight, blendfunc))
				{
			...
			//	case WM_TIMER:
			{
				...
				else if(!g_bFrameCreated)
				{
					...
					g_pFrame->GetWndClassInfo().m_wc.hIcon =
						LoadIcon(_Module.GetResourceInstance(), MAKEINTRESOURCE(IDI_ICON));
					...
					{
						g_pFrame->Create(GetDesktopWindow(), rcPos, _T( "Steam Media Player" ), WS_OVERLAPPEDWINDOW, 0,
										 (UINT)0);
					}
					else
					{
	...
	else
	{
		...
		else
		{
			...
			::SetWindowPos(g_hBlackFadingWindow, HWND_BOTTOM, 0, 0, 0, 0,
						   SWP_NOREDRAW | SWP_NOMOVE | SWP_NOSIZE | SWP_HIDEWINDOW | SWP_NOACTIVATE | SWP_DEFERERASE);
		}
	}
	return true;
}
...
HWND CreateFullscreenWindow(bool bFadeIn)
{
	...
	g_hBlackFadingWindow = CreateWindow("myclass", _T( "Steam Media Player" ), windowStyle, mi.rcMonitor.left,
...
bool CreateDesktopBitmaps()
{
	...
	if(g_screenWidth != GetDeviceCaps(hdcScreen, HORZRES) || g_screenHeight != GetDeviceCaps(hdcScreen, VERTRES))
	{
...
void KillOtherSMPs()
{
	...
	for(int i = 0; i < nProcIds; ++i)
	{
		...
		HANDLE hProcess =
			OpenProcess(PROCESS_TERMINATE | PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, FALSE, procIds[i]);
		if(!hProcess)
		{
...
extern "C" int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPTSTR lpCmdLine, int /*nShowCmd*/)
{
	...
	SetRegistryValue("Software\\Microsoft\\MediaPlayer\\Preferences\\VideoSettings", "UseVMROverlay", 0,
					 g_dwUseVMROverlayOldValue, g_bUseVMROverlayValueExists);
