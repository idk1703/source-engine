...
int CVcdImportApp::Main()
{
	...
	if(!pImportFileName || !pInFileName)
	{
		...
		Msg("\t-f : .FAC file containing SFM-authored facial animation exported from the animation set editor in the "
			"SFM.\n");
		...
		Msg("\t-p : Causes the generated .VCD file to not use phonemes in associated wav files to autogenerate "
			"animation.\n");
		Msg("\t-nop4 : Disables auto perforce checkout.\n");
		Msg("\t-s : Requires a second argument, the amount of error we're willing to accept in simplification. Ranges "
			"from 0.0 - 1.0 [default = 0.05]\n");
		...
		{
			Msg("\t\t%22s :\t%s\n", Interpolator_NameForInterpolator(i, false),
