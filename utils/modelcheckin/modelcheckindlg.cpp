...
CModelCheckInDlg::CModelCheckInDlg(CWnd *pParent /*=NULL*/) : CDialog(CModelCheckInDlg::IDD, pParent)
{
...
{
	AfxGetApp()->WriteProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_USER, m_UserName);
	AfxGetApp()->WriteProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_HL2_PATH, m_HL2GameDirectory);
	AfxGetApp()->WriteProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_TF2_PATH, m_TF2GameDirectory);
}
...
{
	m_UserName = AfxGetApp()->GetProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_USER, "");
	m_HL2GameDirectory = AfxGetApp()->GetProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_HL2_PATH, "u:/hl2/hl2");
	m_TF2GameDirectory = AfxGetApp()->GetProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_TF2_PATH, "u:/hl2/tf2");
}
...
void CModelCheckInDlg::PerformCheckoutCommand(ProjectType_t project, char const *pRelativeDir, char const *pDestPath,
											  char const *pFileName)
{
	...
	for(currExtension = 0; s_ppExtensions[currExtension][0]; ++currExtension)
	{
		...
		sprintf(buf, "ss filetype %s%s/%s%s -O- -y%s\n", s_ppProjectDir[project], pRelativeDir, pFileName,
				s_ppExtensions[currExtension], (char const *)m_UserName);
		...
		sprintf(buf, "ss checkout %s%s/%s%s -GL%s -GWA -O- -y%s\n", s_ppProjectDir[project], pRelativeDir, pFileName,
				s_ppExtensions[currExtension], pDestPath, (char const *)m_UserName);
		retVal = system(buf);
		if(retVal > 0)
		{
			len += sprintf(&error[len], "*** SourceSafe error attempting to check out \"%s%s\"\n", pFileName,
						   s_ppExtensions[currExtension]);
		}
	}
...
void CModelCheckInDlg::PerformCheckinCommand(ProjectType_t project, char const *pRelativeDir, char const *pDestPath,
											 char const *pFileName)
{
	char buf[1024];
	char error[1024];
...
CModelCheckInDlg::ProjectType_t CModelCheckInDlg::GetFileNames(char const *pTitle, char *&pRelativePath,
															   char *&pFileName, char *&pDestPath)
{
	...
	CFileDialog dlg(TRUE, ".mdl", "*.mdl", OFN_HIDEREADONLY, "Model Files (*.mdl)|*.mdl||");
	...
	CString temp = AfxGetApp()->GetProfileString(MDL_CHECKOUT_REG_CLASS, MDL_CHECKOUT_REG_LAST_PATH, pCwd);
...
void CModelCheckInDlg::OnFileCheckOut()
{
	...
	projectType = GetFileNames("Select MDL file to check out...", pRelativePath, pFileName, pDestPath);
...
void CModelCheckInDlg::OnFileCheckIn()
{
	...
	char *pDestPath;
	ProjectType_t projectType;
	projectType = GetFileNames("Select MDL file to check in...", pRelativePath, pFileName, pDestPath);
