...
static void SoundProperties_Multiple_OnOK(HWND hwndDlg)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		bool owneronly =
			SendMessage(GetDlgItem(hwndDlg, IDC_OWNERONLY), BM_GETCHECK, 0, 0) == BST_CHECKED ? true : false;
...
int SoundProperties_Multiple(CSoundParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_SOUNDPROPERTIES_MULTIPLE),
						   (HWND)GetWorkspaceManager()->getHandle(), (DLGPROC)SoundProperties_MultipleDialogProc);
