...
int _MultipleRequest(CMultipleParams *params)
{
	...
	if(g_MRCurrentContext == g_MRContext && g_MRLastResult != -1)
	{
	...
	if(retval == -1)
	{
		...
		retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_MULTIPLEQUESTION),
						   (HWND)GetWorkspaceManager()->getHandle(), (DLGPROC)MultipleRequestDialogProc);
