		...
		case WM_INITDIALOG:
		{
			...
			else
			{
				...
				SendMessage(GetDlgItem(hwndDlg, IDC_OWNERONLY), BM_SETCHECK,
							(WPARAM)p->OnlyPlayToOwner() ? BST_CHECKED : BST_UNCHECKED, (LPARAM)0);
				...
				case IDC_ADDWAVE:
				{
					...
					if(count >= 1)
					{
						...
						for(i = 0; i < count; i++)
						{
							...
							SendMessage(GetDlgItem(hwndDlg, IDC_WAVELIST_AVAILABLE), LB_GETTEXT, selected[i],
										(LPARAM)wavename);
							...
							if(wb)
							{
								...
								if(wavefile)
								{
									...
									if(item->FindWave(wavefile) == -1)
									{
										...
										// Add it to list
										SendMessage(GetDlgItem(hwndDlg, IDC_WAVELIST), LB_ADDSTRING, 0,
													(LPARAM)wavename);
									}
								}
							}
...
int SoundProperties(CSoundParams *params)
{
	...
	int retval = DialogBox((HINSTANCE)GetModuleHandle(0), MAKEINTRESOURCE(IDD_SOUNDPROPERTIES),
						   (HWND)GetWorkspaceManager()->getHandle(), (DLGPROC)SoundPropertiesDialogProc);
