...
static void SceneManager_VSSCheckout(char const *pUserName, char const *pProjectDir, char const *pRelativeDir,
									 char const *pDestPath, char const *pFileNameWithExtension)
{
	char buf[1024];

	// Check for the existence of the file in source safe...
	sprintf(buf, "filetype %s/%s%s -O- -y%s\n", pProjectDir, pRelativeDir, pFileNameWithExtension, pUserName);
	...
	sprintf(buf, "checkout %s/%s%s -GL%s -GWA -O- -y%s\n", pProjectDir, pRelativeDir, pFileNameWithExtension, pDestPath,
			pUserName);
...
static void SceneManager_VSSCheckin(char const *pUserName, char const *pProjectDir, char const *pRelativeDir,
									char const *pDestPath, char const *pFileNameWithExtension)
{
	char buf[1024];
	...
	if(retVal != 0)
	{
		...
		sprintf(buf, "add %s%s -GL%s -O- -I- -y%s\n", pRelativeDir, pFileNameWithExtension, pDestPath, pUserName);
		Sys_Exec("ss.exe", buf, true);
	}
	else
	{
...
void VSS_Checkout(char const *name, bool updatestaticons /*= true*/)
{
	...
	SceneManager_VSSCheckout(ws->GetVSSUserName(), ws->GetVSSProject(), path, va("%s%s", gamedir, path), filename);
...
void VSS_Checkin(char const *name, bool updatestaticons /*= true*/)
{
	...
	SceneManager_VSSCheckin(ws->GetVSSUserName(), ws->GetVSSProject(), path, va("%s%s", gamedir, path), filename);
