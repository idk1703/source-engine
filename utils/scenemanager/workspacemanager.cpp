...
{
public:
	CWorkspaceWorkArea(mxWindow *parent) : mxWindow(parent, 0, 0, 0, 0, "")
	{
...
CWorkspaceManager::CWorkspaceManager() : mxWindow(0, 0, 0, 0, 0, g_appTitle, mxWindow::Normal)
{
		...
		if(filesystem->FileExists(scriptname))
		{
			if(!filesystem->IsFileWritable(scriptname))
			{
				...
				if(retval == 0)
				{
					...
					if(!filesystem->IsFileWritable(scriptname))
					{
						mxMessageBox(NULL, va("Aborting shutdown, %s, not writable!!", scriptname), g_appTitle,
									 MX_MB_OK);
						return false;
					}
				}
			...
			{
				int retval =
					mxMessageBox(NULL, va("Save changes to out '%s'?", scriptname), g_appTitle, MX_MB_YESNOCANCEL);
				// Cancel
				if(retval == 2)
					return false;
...
void CWorkspaceManager::UpdateMenus()
{
	...
	if(hasproject)
	{
		m_pMenuBar->modify(IDC_WSM_FILE_SCENE_NEW, IDC_WSM_FILE_SCENE_NEW,
						   va("&Create Scene in '%s'", item->GetProject()->GetName()));
	}
	else
	{
...
void CWorkspaceManager::UpdateRecentFilesMenu()
{
	...
	for(int i = 0; i < c; i++)
	{
		m_pMenuBar->modify(IDC_WSM_FILE_RECENT_WORKSPACE_START + i, IDC_WSM_FILE_RECENT_WORKSPACE_START + i,
						   m_RecentFiles[i].filename);
	}
}
...
void CWorkspaceManager::OnNewProject()
{
	...
	if(!ws)
	{
		...
		if(!filesystem->FileExists(workspace_name))
		{
			int retval = mxMessageBox(NULL, va("Automatically create workspace %s?", workspace_name), g_appTitle,
									  MX_MB_YESNOCANCEL);
			if(retval != 0)
			{
...
HIMAGELIST CWorkspaceManager::CreateImageList()
{
	...
	list = ImageList_Create(CX_ICON, CY_ICON, FALSE, NUM_IMAGES, 0);
