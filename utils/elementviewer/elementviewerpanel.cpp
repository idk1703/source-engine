...
{
	//	if ( flags & INotifyUI::NOTIFY_REFRESH_PROPERTIES_VALUES ) // FIXME - do we need new flags for file association
	//changes?
	{
		m_pFileManager->Refresh();
...
void CElementViewerPanel::OnFileSelected(KeyValues *pKeyValues)
{
	...
	if(pKeyValues->FindKey("OnSaveAs"))
	{
		...
		if(docCount == 1)
		{
			...
			g_pDataModel->SaveToFile(pFullPath, NULL, g_pDataModel->GetDefaultEncoding(pFormat), pFormat,
									 GetElement<CDmElement>(g_pDataModel->GetFileRoot(m_Docs[0].m_fileid)));
		}
		return;
	}
