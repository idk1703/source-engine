...
bool CElementViewerApp::PreInit()
{
	...
	if(!g_pFullFileSystem || !g_pMaterialSystem || !g_pVGui || !g_pVGuiSurface || !g_pDataModel ||
	   !g_pMatSystemSurface || !p4)
	{
	...
	for(int i = 0; i < count; i++)
	{
		...
		if(depth >= start && depth <= end)
		{
			...
			Msg(
				...
				"%s (%i.%i): %p, %s %s x(%i) y(%i) w(%i) h(%i)\n", panel->GetName(), depth + 1, i + 1, panel,
				panel->IsVisible() ? "visible" : "hidden", panel->IsPopup() ? "popup" : "", x, y, width, height);
		}
...
void VGui_DrawPopups(void)
{
	...
	for(int i = 0; i < c; i++)
	{
		...
		// Con_NPrintf( i,
		Msg("%i:  %s : %x, %s pos(%i,%i) w(%i) h(%i)\n", i, p, popup, visible ? "visible" : "hidden", x, y, width,
			height);
	}
}
