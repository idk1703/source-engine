...
static void GrabSurveyData(char const *pInputFilename, char const *pOutputFilename)
{
	...
	char linebuffer[4096], szDummy[32], szVendorID[32], szDeviceID[32], szPartName[256],
		goodLine[] = "$vend_dev_to_name[ MakeLookup(";
...
static bool DidMerge(CUtlVector<DeviceRecord_t> &deviceRecords, int nRecord)
{
	...
	if(!V_strnicmp(deviceRecords[nRecord].szName, deviceRecords[nRecord + 1].szName, 256) &&
	   !V_strnicmp(deviceRecords[nRecord].szData[g_nVendorIDIndex], deviceRecords[nRecord + 1].szData[g_nVendorIDIndex],
				   64))
	{
		...
		if((nMinIDNext >= nMinIDCur) && (nMinIDNext <= (nMaxIDCur + 1)))
		{
			...
			for(int i = g_nDeviceMaxIDIndex + 1; i < g_nNumDXSupportColumns; i++) // Run through elements after maxID
			{
				...
				{
					if(V_strnicmp(deviceRecords[nRecord].szData[i], deviceRecords[nRecord + 1].szData[i],
								  64)) // If it doesn't match the current record's element
					{
						if(strlen(deviceRecords[nRecord].szData[i]) ==
						   0) // If the current record has no data for this entry
						{
							V_strncpy(deviceRecords[nRecord].szData[i], deviceRecords[nRecord + 1].szData[i],
									  64); // Just copy the merged record's data into current record
						}
						else // This is the interesting case, where we have a bona fide conflict
						{
							...
							V_strncat(deviceRecords[nRecord].szData[i], deviceRecords[nRecord + 1].szData[i],
									  64); // Concat the merged element in
...
static bool FoundDuplicate(CUtlVector<DeviceRecord_t> &deviceRecords, int nRecord)
{
	...
	if(!V_strnicmp(deviceRecords[nRecord].szData[g_nVendorIDIndex], deviceRecords[nRecord + 1].szData[g_nVendorIDIndex],
				   64))
	{
		// Convert hex device ID strings into integers for comparisons
		int nMinIDCur, nMaxIDCur, nMinIDNext, nMaxIDNext;
		...
		{
			printf("Duplicate: %s  %s  0x%04x  0x%04x\n", deviceRecords[nRecord].szName,
				   deviceRecords[nRecord].szData[g_nVendorIDIndex], nMinIDCur, nMaxIDCur);
...
static void CleanCSVFile(char const *pInputFilename, char const *pOutputFilename)
{
	...
	// At this point, the output file has the header block from the file and we have records for all of the real device
	// lines
