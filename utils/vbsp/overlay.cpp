...
void OverlayTransition_AddFaceToLists(int iFace, side_t *pSide)
{
	...
	for(int iOverlayId = 0; iOverlayId < nOverlayIdCount; ++iOverlayId)
	{
		mapoverlay_t *pMapOverlay =
			&g_aMapWaterOverlays.Element(pSide->aWaterOverlayIds[iOverlayId] - (MAX_MAP_OVERLAYS + 1));
		if(pMapOverlay)
		{
			if(pMapOverlay->aFaceList.Find(iFace) == -1)
			{
...
void Overlay_EmitOverlayFace(mapoverlay_t *pMapOverlay)
{
	...
	if(pOverlay)
	{
		...
		{
			Error("Overlay touching too many faces (touching %d, max %d)\nOverlay %s at %.1f %.1f %.1f", nFaceCount,
				  OVERLAY_BSP_FACE_COUNT, pMapOverlay->szMaterialName, pMapOverlay->vecOrigin.x,
	...
	if(pOverlay)
	{
		...
		if(nFaceCount >= WATEROVERLAY_BSP_FACE_COUNT)
		{
			Error("Water Overlay touching too many faces (touching %d, max %d)\nOverlay %s at %.1f %.1f %.1f",
				  nFaceCount, OVERLAY_BSP_FACE_COUNT, pMapOverlay->szMaterialName, pMapOverlay->vecOrigin.x,
				  pMapOverlay->vecOrigin.y, pMapOverlay->vecOrigin.z);
			return;
		}
	...
	if(!tmpMatrix.IsIdentity())
	{
		...
		else
		{
			...
			for(int iHandle = 0; iHandle < OVERLAY_HANDLES_COUNT; iHandle++)
			{
				...
				Vector vecPos =
					(vecUV.x * pOverlay->vecBasis[OVERLAY_BASIS_U] + vecUV.y * pOverlay->vecBasis[OVERLAY_BASIS_V]);
