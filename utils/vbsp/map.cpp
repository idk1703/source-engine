...
{
#if 1
	if(fabs(p->normal[0] - normal[0]) < normalEpsilon && fabs(p->normal[1] - normal[1]) < normalEpsilon &&
	   fabs(p->normal[2] - normal[2]) < normalEpsilon && fabs(p->dist - dist) < distEpsilon)
		return true;
#else
...
void CMapFile::MoveBrushesToWorldGeneral(entity_t *mapent)
{
	...
	for(i = 1; i < num_entities; i++)
	{
		if(entities[i].firstbrush <
		   mapent->firstbrush) // if we use <=, then we'll remap the passed in ent, which we don't want to
		{
			entities[i].firstbrush += newbrushes;
		}
	}
...
static ChunkFileResult_t LoadOverlayDataTransitionKeyCallback(const char *szKey, const char *szValue,
	...
	if(eResult == ChunkFile_Ok)
	{
		...
		if(mapent->origin[0] || mapent->origin[1] || mapent->origin[2])
		{
			for(int i = 0; i < mapent->numbrushes; i++)
			{
				...
				for(int j = 0; j < b->numsides; j++)
				{
					...
					vec_t newdist =
						mapplanes[s->planenum].dist - DotProduct(mapplanes[s->planenum].normal, mapent->origin);
		...
		if(!Q_stricmp(pClassName, "light"))
		{
			CVmfImport::GetVmfImporter()->ImportLightCallback(
				ValueForKey(mapent, "hammerid"), ValueForKey(mapent, "origin"), ValueForKey(mapent, "_light"),
				ValueForKey(mapent, "_lightHDR"), ValueForKey(mapent, "_lightscaleHDR"),
		...
		if(!Q_stricmp(pClassName, "light_spot"))
		{
			CVmfImport::GetVmfImporter()->ImportLightSpotCallback(
				ValueForKey(mapent, "hammerid"), ValueForKey(mapent, "origin"), ValueForKey(mapent, "angles"),
				ValueForKey(mapent, "pitch"), ValueForKey(mapent, "_light"), ValueForKey(mapent, "_lightHDR"),
				ValueForKey(mapent, "_lightscaleHDR"), ValueForKey(mapent, "_quadratic_attn"),
		...
		if(!Q_stricmp(pClassName, "light_dynamic"))
		{
			CVmfImport::GetVmfImporter()->ImportLightDynamicCallback(
				ValueForKey(mapent, "hammerid"), ValueForKey(mapent, "origin"), ValueForKey(mapent, "angles"),
				ValueForKey(mapent, "pitch"), ValueForKey(mapent, "_light"), ValueForKey(mapent, "_quadratic_attn"),
				ValueForKey(mapent, "_inner_cone"), ValueForKey(mapent, "_cone"), ValueForKey(mapent, "brightness"),
				ValueForKey(mapent, "distance"), ValueForKey(mapent, "spotlight_radius"));
		}
		...
		if(!Q_stricmp(pClassName, "light_environment"))
		{
			CVmfImport::GetVmfImporter()->ImportLightEnvironmentCallback(
				ValueForKey(mapent, "hammerid"), ValueForKey(mapent, "origin"), ValueForKey(mapent, "angles"),
				ValueForKey(mapent, "pitch"), ValueForKey(mapent, "_light"), ValueForKey(mapent, "_lightHDR"),
				ValueForKey(mapent, "_lightscaleHDR"), ValueForKey(mapent, "_ambient"),
				ValueForKey(mapent, "_ambientHDR"), ValueForKey(mapent, "_AmbientScaleHDR"),
		...
		{
			CVmfImport::GetVmfImporter()->ImportModelCallback(pModel, ValueForKey(mapent, "hammerid"),
															  ValueForKey(mapent, "angles"),
															  ValueForKey(mapent, "origin"), MDagPath());
		}
#endif // VSVMFIO
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergePlanes(entity_t *pInstanceEntity, CMapFile *Instance, Vector &InstanceOrigin, QAngle &InstanceAngle,
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergeBrushes(entity_t *pInstanceEntity, CMapFile *Instance, Vector &InstanceOrigin,
							QAngle &InstanceAngle, matrix3x4_t &InstanceMatrix)
{
	int max_brush_id = 0;
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergeBrushSides(entity_t *pInstanceEntity, CMapFile *Instance, Vector &InstanceOrigin,
							   QAngle &InstanceAngle, matrix3x4_t &InstanceMatrix)
{
	int max_side_id = 0;
	...
	for(int i = 0; i < Instance->nummapbrushsides; i++)
	{
		...
		side->planenum =
			FindFloatPlane(Instance->mapplanes[side->planenum].normal, Instance->mapplanes[side->planenum].dist);
		side->id += max_side_id;
		...
		{ // sides for ladders are outside of the world spawn, but also need translating
			for(int j = Instance->entities[0].numbrushes; j < Instance->nummapbrushes; j++)
			{
				...
				if(i >= loc && i < (loc + Instance->mapbrushes[j].numsides) &&
				   (Instance->mapbrushes[j].contents & CONTENTS_LADDER) != 0)
				{
					bNeedsTranslation = true;
					break;
				}
...
// Output : none
//-----------------------------------------------------------------------------
void CMapFile::MergeEntities(entity_t *pInstanceEntity, CMapFile *Instance, Vector &InstanceOrigin,
							 QAngle &InstanceAngle, matrix3x4_t &InstanceMatrix)
{
	int max_entity_id = 0;
	char temp[2048];
	...
	for(int i = 0; i < Instance->num_entities; i++)
	{
		...
		else
		{
			...
			// this is done before entity fixup, so fixup may occur on the replaced value.  Not sure if this is a
			// desired order of operation yet.
	...
	// search for variables coming from the func_instance to replace inside of the instance
	// this is done before connection fix up, so fix up may occur on the replaced value.  Not sure if this is a desired
	// order of operation yet.
...
bool LoadMapFile(const char *pszFileName)
{
	...
	if((eResult == ChunkFile_Ok) || (eResult == ChunkFile_EOF))
	{
		...
		qprintf("size: %5.0f,%5.0f,%5.0f to %5.0f,%5.0f,%5.0f\n", g_LoadingMap->map_mins[0], g_LoadingMap->map_mins[1],
				g_LoadingMap->map_mins[2], g_LoadingMap->map_maxs[0], g_LoadingMap->map_maxs[1],
				g_LoadingMap->map_maxs[2]);
	...
	if(eResult == ChunkFile_Ok)
	{
		...
		if(planenum != -1)
		{
			...
			if(k == b->numsides)
			{
				...
				{
					CVmfImport::GetVmfImporter()->AddSideCallback(b, side, pSideInfo->td, pSideInfo->planepts[0],
...
ChunkFileResult_t LoadSideKeyCallback(const char *szKey, const char *szValue, LoadSide_t *pSideInfo)
{
	...
	{
		int nRead = sscanf(szValue, "(%f %f %f) (%f %f %f) (%f %f %f)", &pSideInfo->planepts[0][0],
						   &pSideInfo->planepts[0][1], &pSideInfo->planepts[0][2], &pSideInfo->planepts[1][0],
						   &pSideInfo->planepts[1][1], &pSideInfo->planepts[1][2], &pSideInfo->planepts[2][0],
						   &pSideInfo->planepts[2][1], &pSideInfo->planepts[2][2]);
	...
	{
		int nRead =
			sscanf(szValue, "[%f %f %f %f] %f", &pSideInfo->td.UAxis[0], &pSideInfo->td.UAxis[1],
				   &pSideInfo->td.UAxis[2], &pSideInfo->td.shift[0], &pSideInfo->td.textureWorldUnitsPerTexel[0]);
		if(nRead != 5)
		{
	...
	{
		int nRead =
			sscanf(szValue, "[%f %f %f %f] %f", &pSideInfo->td.VAxis[0], &pSideInfo->td.VAxis[1],
				   &pSideInfo->td.VAxis[2], &pSideInfo->td.shift[1], &pSideInfo->td.textureWorldUnitsPerTexel[1]);
		if(nRead != 5)
		{
...
ChunkFileResult_t CMapFile::LoadConnectionsKeyCallback(const char *szKey, const char *szValue,
