...
//-----------------------------------------------------------------------------
ChunkFileResult_t CManifest::LoadManifestMapKeyCallback(const char *szKey, const char *szValue,
		...
		{ // for world spawn, we look at brushes
			for(int nBrushNum = 0; nBrushNum < g_MainMap->entities[i].numbrushes; nBrushNum++)
			{
				...
				for(int nCordon = 0; nCordon < m_Cordons.Count(); nCordon++)
				{
					...
					{
						if(m_Cordons[nCordon].m_Boxes[nBox].IsIntersectingBox(
							   g_MainMap->mapbrushes[nIndex].mins, g_MainMap->mapbrushes[nIndex].maxs) == true)
						{
							bRemove = false;
							break;
						}
