...
static void AddDetailSpriteToLump(const Vector &vecOrigin, const QAngle &vecAngles, DetailModel_t const &model,
								  float flScale)
{
	AddDetailSpriteToLump(vecOrigin, vecAngles, model.m_Orientation, model.m_Pos, model.m_Tex, flScale, model.m_Type,
						  model.m_ShapeAngle, model.m_ShapeSize, model.m_SwayAmount);
}
...
void EmitDetailModels()
{
	...
	for(int j = 0; j < numfaces; ++j)
	{
		...
		MaterialSystemMaterial_t handle =
			FindOriginalMaterial(TexDataStringTable_GetString(pTexData->nameStringTableID), &found, false);
		if(!found)
			continue;
		...
		if(objectType < 0)
		{
			Warning("Material %s uses unknown detail object type %s!\n",
					TexDataStringTable_GetString(pTexData->nameStringTableID), pDetailType);
			continue;
		}
