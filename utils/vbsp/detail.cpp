...
bool MergeFace_r(node_t *node, face_t *face, face_t *original)
{
	...
	else
	{
		...
		ClassifyWindingEpsilon_Offset(face->w, plane->normal, plane->dist, 0.001, &frontwinding, &backwinding,
									  &onwinding, -offset);
...
face_t *MergeDetailTree(tree_t *worldtree, int brush_start, int brush_end)
{
	...
	if(detailbrushes)
	{
		...
		// NOTE: This still leaves detail brush sides that are covered by the world. (these are removed in the merge
		// operation)
...
bool ClipFaceToBrush(face_t *pFace, bspbrush_t *pbrush, face_t **pOutputList)
{
	...
	if(foundSide >= 0)
	{
		...
		for(i = 0; i < sortedSides.Size(); i++)
		{
			...
			ClipWindingEpsilon_Offset(currentface->w, plane->normal, plane->dist, 0.001, &frontwinding, &backwinding,
									  offset);
