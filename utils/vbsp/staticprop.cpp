...
bool LoadStudioModel(char const *pModelName, char const *pEntityType, CUtlBuffer &buf)
{
	...
	if(isStaticProp != RET_VALID)
	{
		...
		{
			Warning(
				"Error! %s using model \"%s\", which must be used on a dynamic entity (i.e. prop_physics). Deleted.\n",
...
static bool TestLeafAgainstCollide(int depth, int *pNodeList, Vector const &origin, QAngle const &angles,
...
static void ComputeConvexHullLeaves_R(int node, int depth, int *pNodeList, Vector const &mins, Vector const &maxs,
									  ...
									  CUtlVector<unsigned short> &leafList)
{
	...
	while(node >= 0)
	{
		...
		else
		{
			...
			ComputeConvexHullLeaves_R(pNode->children[1], depth, pNodeList, mins, maxs, origin, angles, pCollide,
									  leafList);

			pNodeList[depth - 1] = -node - 1;
			ComputeConvexHullLeaves_R(pNode->children[0], depth, pNodeList, mins, maxs, origin, angles, pCollide,
									  leafList);
			return;
		}
	}
...
static void ComputeStaticPropLeaves(CPhysCollide *pCollide, Vector const &origin, QAngle const &angles,
									CUtlVector<unsigned short> &leafList)
{
...
static void AddStaticPropToLump(StaticPropBuild_t const &build)
{
	...
	if(!leafList.Count())
	{
		Warning("Static prop %s outside the map (%.2f, %.2f, %.2f)\n", build.m_pModelName, build.m_Origin.x,
				build.m_Origin.y, build.m_Origin.z);
		return;
	}
