...
bool GetBarycentricCoordsFromLightmapCoords(Vector2D tri[3], Vector2D const &lmCoords, float bcCoords[3])
{
	...
	return (bcCoords[0] >= 0.0f && bcCoords[0] <= 1.0f) && (bcCoords[1] >= 0.0f && bcCoords[1] <= 1.0f) &&
...
void CalculateLightmapSamplePositions(CCoreDispInfo *pCoreDispInfo, const dface_t *pFace,
...
void DispMapToCoreDispInfo(mapdispinfo_t *pMapDisp, CCoreDispInfo *pCoreDispInfo, dface_t *pFace, int *pSwappedTexInfos)
{
	...
	if(pFace)
	{
		...
		CalcTextureCoordsAtPoints(pTexInfo->textureVecsTexelsPerWorldUnits, zeroOffset, pt, 4, tCoords);
	}
	...
	Vector vecTmp(pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][0], pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][1],
				  pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][2]);
	int nLuxelsPerWorldUnit = static_cast<int>(1.0f / VectorLength(vecTmp));
	Vector vecU(pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][0], pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][1],
				pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][2]);
	Vector vecV(pTexInfo->lightmapVecsLuxelsPerWorldUnits[1][0], pTexInfo->lightmapVecsLuxelsPerWorldUnits[1][1],
	...
	pCoreDispInfo->InitDispInfo(pMapDisp->power, pMapDisp->minTess, pMapDisp->smoothingAngle, pMapDisp->alphaValues,
...
{
	ErrorIfNot(pIn->GetAllowedVerts().GetNumDWords() == sizeof(pOut->m_AllowedVerts) / 4,
...
bool FindEnclosingTri(const Vector2D &vert, CUtlVector<Vector2D> &vertCoords, CUtlVector<unsigned short> &indices,
					  int *pStartVert, float bcCoords[3])
{
	for(int i = 0; i < indices.Count(); i += 3)
	{
		GetBarycentricCoords2D(vertCoords[indices[i + 0]], vertCoords[indices[i + 1]], vertCoords[indices[i + 2]], vert,
							   bcCoords);

		if(bcCoords[0] >= 0 && bcCoords[0] <= 1 && bcCoords[1] >= 0 && bcCoords[1] <= 1 && bcCoords[2] >= 0 &&
		   bcCoords[2] <= 1)
		{
			*pStartVert = i;
			return true;
