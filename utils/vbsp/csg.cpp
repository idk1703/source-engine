...
bspbrush_t *MakeBspBrushList(int startbrush, int endbrush, const Vector &clipmins, const Vector &clipmaxs,
							 int detailScreen)
{
...
qboolean BrushGE(bspbrush_t *b1, bspbrush_t *b2)
{
	...
	if((b2->original->contents & MASK_SPLITAREAPORTAL) && (b1->original->contents & CONTENTS_AREAPORTAL))
	{
		return true;
	}
