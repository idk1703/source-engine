...
public:
	...
	virtual bool SendChunksTo(const CIPAddr *pAddr, void const *const *pChunks, const int *pChunkLengths, int nChunks)
	{
		...
		DWORD ret = WSASendTo(m_Socket, bufs, nChunks, &dwNumBytesSent, 0, (sockaddr *)&addr, sizeof(addr), NULL, NULL);
	...
	void Term()
	{
		if(m_Socket != INVALID_SOCKET)
		{
			if(m_bMulticastGroupMembership)
			{
				...
				setsockopt(m_Socket, IPPROTO_IP, IP_DROP_MEMBERSHIP, (char *)&m_MulticastGroupMREQ,
						   sizeof(m_MulticastGroupMREQ));
			}
...
ISocket *CreateMulticastListenSocket(const CIPAddr &addr, const CIPAddr &localInterface)
{
	...
	if(pSocket->Bind(&bindAddr) && pSocket->ListenToMulticastStream(addr, localInterface))
	{
		return pSocket;
	}
	else
