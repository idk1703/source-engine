...
public:
	...
	void MaybeFlushAckChunks(unsigned short chunksToAck[NUM_BUFFERED_CHUNK_ACKS][2], int &nChunksToAck,
							 DWORD &lastAckTime)
	{
	...
	void AddAckChunk(unsigned short chunksToAck[NUM_BUFFERED_CHUNK_ACKS][2], int &nChunksToAck, DWORD &lastAckTime,
	...
	CWorkerFile *ListenFor(const char *pFilename, const char *pPathID)
	{
		...
		if(V_strlen(pFilename) > ARRAYSIZE(printableFilename) - 1)
		{
			...
			V_strncat(printableFilename,
					  &pFilename[V_strlen(pFilename) - ARRAYSIZE(printableFilename) + 1 + V_strlen(printableFilename)],
			...
			if(!(pTestFile->m_ChunksReceived[iChunk >> 3] & (1 << (iChunk & 7))))
			{
				...
				if(chunkLen != payloadLen)
				{
					Error("ListenFor(): invalid payload length for '%s' (%d should be %d)\n"
						  ...
						  pTestFile->GetFilename(), payloadLen, chunkLen, pTestFilename, pTestFile->GetFilename(),
						  iChunk, pInfo->m_nChunks, pInfo->m_CompressedSize);
				}
				...
				if(pTestFile == pFile)
				{
					...
					ShowSDKWorkerMsg("\rRecv %s (%d%%) [chunk %d/%d] ", printableFilename, percent,
				...
				if(pTestFile->m_nChunksToReceive == 0)
				{
					...
					if(!ZLibDecompress(pTestFile->m_CompressedData.Base(), pTestFile->m_CompressedData.Count(),
									   pTestFile->m_UncompressedData.Base(), pTestFile->m_UncompressedData.Count()))
					{
						if(pSocket)
							pSocket->Release();
						FlushAckChunks(chunksToAck, nChunksToAck, lastAckTime);
					...
					char str[512];
					V_snprintf(str, sizeof(str), "Got %s (%dk) in %.2fs", printableFilename,
