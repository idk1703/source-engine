	...
	for(int x = 0; x < nIntervals; x++)
	{
		if(curTime >= 0)
		{
			...
			if(curTime >= prevEntry.m_msTime && curTime <= curEntry.m_msTime)
			{
				...
				int nBytesSent = (int)RemapVal(curTime, prevEntry.m_msTime, curEntry.m_msTime, prevEntry.m_nBytesSent,
				...
				int nBytesReceived = (int)RemapVal(curTime, prevEntry.m_msTime, curEntry.m_msTime,
