...
} g_ColumnInfos[] = {{"Computer Name", 150, SortByName},
...
CJobWatchDlg::CJobWatchDlg(CWnd *pParent /*=NULL*/) : CIdleDialog(CJobWatchDlg::IDD, pParent)
{
	m_CurMessageIndex = 0;
	m_CurGraphTime = 0;
	m_pSQL = NULL;
...
BOOL CJobWatchDlg::OnInitDialog()
{
	...
	char cmdLine[2048];
	Q_snprintf(cmdLine, sizeof(cmdLine), "vmpi_job_watch -JobID %s -hostname %s -dbname %s -username %s", pJobID,
			   pHostName, pDBName, pUserName);
	SetDlgItemText(IDC_COMMAND_LINE, cmdLine);

	// Setup anchors.
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_WORKERS_PANEL), ANCHOR_LEFT, ANCHOR_TOP, ANCHOR_WIDTH_PERCENT,
						  ANCHOR_HEIGHT_PERCENT);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_WORKERS), ANCHOR_LEFT, ANCHOR_TOP, ANCHOR_WIDTH_PERCENT,
						  ANCHOR_HEIGHT_PERCENT);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_TEXT_OUTPUT_PANEL), ANCHOR_LEFT, ANCHOR_HEIGHT_PERCENT,
						  ANCHOR_WIDTH_PERCENT, ANCHOR_BOTTOM);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_TEXTOUTPUT), ANCHOR_LEFT, ANCHOR_HEIGHT_PERCENT, ANCHOR_WIDTH_PERCENT,
						  ANCHOR_BOTTOM);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_GRAPHS_PANEL), ANCHOR_WIDTH_PERCENT, ANCHOR_TOP, ANCHOR_RIGHT,
						  ANCHOR_HEIGHT_PERCENT);
	...
	if(curTime - m_LastQueryTime >= 1000)
	{
		...
		if(!m_bQueriesInProgress[QUERY_TEXT] && bJobWorkerIDValid)
		{
			Q_snprintf(query, sizeof(query),
					   "select * from text_messages where JobWorkerID=%lu and MessageIndex >= %lu", jobWorkerID,
					   m_CurMessageIndex);
...
void CJobWatchDlg::ProcessQueryResults_WorkerStats(IMySQLRowSet *pSet)
{
	...
	while(pSet->NextRow())
	{
		...
		if(pInfo)
		{
			if(workerState != pInfo->m_bConnected || nWorkUnits != pInfo->m_nWorkUnitsDone ||
			   runningTimeMS != pInfo->m_RunningTimeMS || stricmp(pCurrentStage, pInfo->m_CurrentStage) != 0 ||
			   iThread0WU != pInfo->m_ThreadWUs[0] || iThread1WU != pInfo->m_ThreadWUs[1] ||
			   iThread2WU != pInfo->m_ThreadWUs[2] || iThread3WU != pInfo->m_ThreadWUs[3])
			{
				bChange = true;
