...
char *g_pInstallFiles[] = {"vmpi_service.exe",		   "vmpi_service_ui.exe", "WaitAndRestart.exe",
						   "vmpi_service_install.exe", "tier0.dll",			  "vmpi_transfer.exe",
						   "filesystem_stdio.dll",	   "vstdlib.dll"};
...
bool CreateStartMenuLink(const char *pSubFolderName, const char *pLinkName, const char *pLinkTarget,
...
bool StopRunningApp()
{
	...
	if(pSocket)
	{
		if(pSocket->BindToAny(0))
		{
			...
			{
				char cPacket[4] = {protocolVersions[iProtocolVersion],
			...
			if(hInst)
			{
				...
				typedef BOOL(WINAPI * EnumProcessModulesFn)(HANDLE hProcess, HMODULE * lphModule, DWORD cb,
															LPDWORD lpcbNeeded);
				typedef DWORD(WINAPI * GetModuleBaseNameFn)(HANDLE hProcess, HMODULE hModule, LPTSTR lpBaseName,
															DWORD nSize);

				EnumProcessesFn EnumProcesses = (EnumProcessesFn)GetProcAddress(hInst, "EnumProcesses");
				EnumProcessModulesFn EnumProcessModules =
					(EnumProcessModulesFn)GetProcAddress(hInst, "EnumProcessModules");
				GetModuleBaseNameFn GetModuleBaseName =
					(GetModuleBaseNameFn)GetProcAddress(hInst, "GetModuleBaseNameA");
							...
							if(hProc)
							{
								...
								if(EnumProcessModules(hProc, hModules, sizeof(hModules), &nBytes))
								{
									...
									for(DWORD iModule = 0; iModule < nModules; iModule++)
									{
										...
										{
											if(Q_stristr(filename, "vmpi_service.exe") ||
											   Q_stristr(filename, "vmpi_service_ui.exe"))
											{
...
CServiceInstallDlg::CServiceInstallDlg(CWnd *pParent /*=NULL*/) : CDialog(CServiceInstallDlg::IDD, pParent)
{
...
bool InstallService(SC_HANDLE hSCManager, const char *pBaseDir)
{
	...
	if(!hUIKey || RegSetValueEx(hUIKey, VMPI_SERVICE_UI_VALUE_NAME, 0, REG_SZ, (unsigned char *)uiFilename,
								strlen(uiFilename) + 1) != ERROR_SUCCESS)
	{
...
void CServiceInstallDlg::OnInstall()
{
	...
	if(RegSetValueEx(g_hVMPIKey, SERVICE_INSTALL_LOCATION_KEY, 0, REG_SZ, (BYTE *)(const char *)strInstallLocation,
					 V_strlen(strInstallLocation) + 1) != ERROR_SUCCESS)
	{
...
bool CServiceInstallDlg::DoUninstall(bool bShowMessage)
{
	...
	{
		// Don't ask if they care if we delete all the files in that directory if the only exes in there are the install
		// exes.
