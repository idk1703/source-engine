...
public:
	...
	virtual void GetPerfData(int &processorPercentage, int &memoryUsageMegabytes)
	{
		...
		if(PdhGetFormattedCounterValue(m_hProcessorTimeCounter, PDH_FMT_LONG | PDH_FMT_NOCAP100, &dwType,
		...
		// Check memory usage.
		if(PdhGetFormattedCounterValue(m_hPrivateBytesCounter, PDH_FMT_DOUBLE | PDH_FMT_NOCAP100, &dwType,
									   &counterValue) == ERROR_SUCCESS)
...
private:
	bool GetInstanceNameFromProcessID(DWORD processID, char *instanceName, int instanceNameLen)
	{
		...
		PDH_STATUS stat = PdhEnumObjectItems(NULL, NULL, "Process", counterList, &counterListLen, instanceList,
											 &instanceListLen, PERF_DETAIL_NOVICE, 0);
		if(stat == PDH_MORE_DATA)
		{
			...
			stat = PdhEnumObjectItems(NULL, NULL, "Process", counterList, &counterListLen, instanceList,
									  &instanceListLen, PERF_DETAIL_NOVICE, 0);
			if(stat == ERROR_SUCCESS)
			{
				...
				// The instance name list is a bunch of strings terminated with nulls. The final one has two nulls after
				// it. Walk through the list and get the process ID associated with each instance name.
				...
				// Each instance (like "vrad") might have multiple versions, like if you're running multiple vrad
				...
				for(int i = counts.First(); i != counts.InvalidIndex(); i = counts.Next(i))
				{
					...
					for(int iInstance = 0; iInstance < nInstances; iInstance++)
					{
						...
						V_snprintf(fullObjectName, sizeof(fullObjectName), "\\Process(%s)\\ID Process",
								   testInstanceName);
...
#define TOTAL_PROCESSOR_TIME_COUNTER_INDEX \
	240								   // '% Total processor time' counter (valid in WinNT under 'System' object)
...
void QueryPerformanceData(PERF_DATA_BLOCK **pPerfData, DWORD dwObjectIndex, DWORD dwCounterIndex)
{
	...
	while((lRes = RegQueryValueEx(HKEY_PERFORMANCE_DATA, keyName, NULL, NULL, (LPBYTE)Buffer.Base(), &BufferSize)) ==
		  ERROR_MORE_DATA)
	{
...
int CPerfTracker::GetCPUPercentage()
{
	...
	LONGLONG lnNewValue =
		GetCounterValueForProcessID<LONGLONG>(&pPerfData, dwObjectIndex, dwCpuUsageIndex, m_dwProcessID);
