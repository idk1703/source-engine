...
#define SERVICES_BROWSER_TIMEOUT \
	10 // We remove a services browser from the list if we don't hear from it for this long.
...
void LoadStateFromRegistry()
{
	if(g_hVMPIServiceKey)
	{
		...
		if(RegQueryValueEx(g_hVMPIServiceKey, "ScreensaverMode", 0, &type, (unsigned char *)&val, &size) ==
			   ERROR_SUCCESS &&
		...
		if(RegQueryValueEx(g_hVMPIServiceKey, "Disabled", 0, &type, (unsigned char *)&val, &size) == ERROR_SUCCESS &&
		   type == REG_DWORD && size == sizeof(val) && val != 0)
		{
			if(g_pConnMgr)
...
void SaveStateToRegistry()
{
	if(g_hVMPIServiceKey)
	{
		...
		RegSetValueEx(g_hVMPIServiceKey, "ScreensaverMode", 0, REG_DWORD, (unsigned char *)&val, sizeof(val));

		val = (g_iCurState == VMPI_SERVICE_STATE_DISABLED);
		RegSetValueEx(g_hVMPIServiceKey, "Disabled", 0, REG_DWORD, (unsigned char *)&val, sizeof(val));
	}
}
...
void GetArgsFromBuffer(bf_read &buf, CUtlVector<char *> &newArgv, bool *bShowAppWindow)
{
...
bool RunProcessFromArgs(CUtlVector<char *> &newArgv, bool bShowAppWindow, bool bCreateSuspended,
						const char *pWorkingDir, PROCESS_INFORMATION *pOut)
{
	...
	BOOL bRet = CreateProcess(NULL, commandLine,
							  NULL, // security
							  NULL, TRUE,
...
void RunProcessAtCommandLine(CUtlVector<char *> &newArgv, bool bShowAppWindow, bool bCreateSuspended, int iPriority)
{
...
// -------------------------------------------------------------------------------- //
bool StartDownloadingAppFiles(CUtlVector<char *> &newArgv, char *cacheDir, int cacheDirLen, bool bShowAppWindow,
							  HANDLE *hProcess, bool bPatching)
{
	*hProcess = NULL;
	...
	if(bPatching)
	{
		...
		// When patching, we can't start the UI and the installer can't because we're running in the local system
		// account and the UI is running on the account of whoever logged in. So what we do is send a message to the UI
		// telling it to run <cacheDir>\WaitAndRestart and restart itself in N seconds.
...
void SendPatchCommandToUIs(DWORD dwInstallerProcessId)
{
	...
	V_ComposeFileName(g_BaseAppPath, "vmpi_service_ui.exe", serviceUIExe,
					  sizeof(serviceUIExe)); // We're running the UI from the same directory this exe is in.
	char strSeconds[64];
...
bool CheckDownloaderFinished()
{
	...
	if(FindArg(__argc, __argv, "-TryDLLMode") && g_RunMode == RUNMODE_CONSOLE &&
	...
	else
	{
		...
		if(g_Waiting_bPatching)
		{
			...
			ResumeThread(g_hRunningThread); // We started the installer suspended so we could make sure we'd send out
											// the patch command.
...
void HandlePacket_LOOKING_FOR_WORKERS(bf_read &buf, const CIPAddr &ipFrom)
{
	...
	V_snprintf(strDownloaderIP, sizeof(strDownloaderIP), "%d.%d.%d.%d:%d", ipFrom.ip[0], ipFrom.ip[1], ipFrom.ip[2],
			   ipFrom.ip[3], iDownloaderPort);
	V_snprintf(strMainIP, sizeof(strMainIP), "%d.%d.%d.%d:%d", ipFrom.ip[0], ipFrom.ip[1], ipFrom.ip[2], ipFrom.ip[3],
			   iPort);
	...
	if(versionString[0] != 0)
	{
		...
		// Check that we haven't applied this patch version yet. This case usually happens right after we've applied a
		// patch and we're restarting. The vmpi_transfer master is still pinging us telling us to patch, but we don't
		// want to reapply this patch.
	...
	if(StartDownloadingAppFiles(newArgv, cacheDir, sizeof(cacheDir), g_Waiting_bShowAppWindow, &g_Waiting_hProcess,
								bPatching))
	{
...
void VMPI_Waiter_Update()
{
	...
	while(1)
	{
		...
		{
			// The key to doing this here is that we ignore whether we're disabled or in screensaver mode.. we always
			// handle the patch command (unless we've already handled this job ID OR if we've already applied this patch
			// version).
