...
CMessageWatchDlg::CMessageWatchDlg(CWnd *pParent /*=NULL*/) : CDialog(CMessageWatchDlg::IDD, pParent)
{
...
void CMessageWatchDlg::OnIdle()
{
	...
	while(1)
	{
		...
		if(data[0] == MSGMGR_PACKETID_ANNOUNCE_PRESENCE && *((int *)&data[1]) == MSGMGR_VERSION)
		{
			...
			if(!pSender || !pSender->m_pSocket)
			{
				...
				if(pNew->BindToAny(0) && TCPSocket_Connect(pNew, &connectAddr, 1000))
				{
					...
					if(!ConvertIPAddrToString(&ipFrom, nameStr, sizeof(nameStr)))
						Q_snprintf(nameStr, sizeof(nameStr), "%d.%d.%d.%d", ipFrom.ip[0], ipFrom.ip[1], ipFrom.ip[2],
								   ipFrom.ip[3]);
					...
					if(!pSender)
					{
						...
						IConsoleWnd *pWnd =
							CreateConsoleWnd(AfxGetInstanceHandle(), IDD_OUTPUT, IDC_DEBUG_OUTPUT, false);
