...
int PrintUsage()
{
	printf("vmpi_launch -machine <remote machine> -priority <priority> [-mpi_pw <password>] -command \"command "
		   "line...\"\n");
...
void SendJobRequest(ISocket *pSocket, int argc, char **argv, CIPAddr &remoteIP, int &iPriority, int &iFirstArg,
					int jobID[4])
{
...
bool WaitForJobStart(ISocket *pSocket, const CIPAddr &remoteIP, const int jobID[4])
{
	while(1)
	{
		...
		if(len == 19 && memcmp(senderAddr.ip, remoteIP.ip, sizeof(senderAddr.ip)) == 0 &&
		   data[1] == VMPI_NOTIFY_START_STATUS && memcmp(&data[2], jobID, 16) == 0)
		{
			if(data[18] == 0)
			{
				// Wasn't able to run.
...
void WaitForJobEnd(ISocket *pSocket, const CIPAddr &remoteIP, const int jobID[4])
{
	while(1)
	{
		...
		if(len == 18 && memcmp(senderAddr.ip, remoteIP.ip, sizeof(senderAddr.ip)) == 0 &&
		   data[1] == VMPI_NOTIFY_END_STATUS && memcmp(&data[2], jobID, 16) == 0)
		{
			int ret = *((int *)&data[2]);
			printf("Finished [%d].\n", ret);
			break;
