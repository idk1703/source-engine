...
static int g_iMasterFinishedDistributeWorkCall =
	-1; // The worker stores this to know which DistributeWork() calls the master has finished.
...
void ShowMPIStats(double flTimeSpent, unsigned long nBytesSent, unsigned long nBytesReceived,
				  unsigned long nMessagesSent, unsigned long nMessagesReceived)
{
	...
	if(g_bMPIMaster)
	{
		...
		for(int i = 0; i < nProcs; i++)
		{
			...
			char formatStr[512];
			Q_snprintf(formatStr, sizeof(formatStr), "%%%ds %I64u\n", 30 - strlen(pMachineName),
					   g_wuCountByProcess[sortedProcs[i]]);
			Msg(formatStr, ":");
		}
	}
	Msg("--------------------------------------------------------------\n\n ");
...
bool DistributeWorkDispatch(MessageBuffer *pBuf, int iSource, int iPacketID)
{
	...
	if(iCurDistributeWorkCall >= MAX_DW_CALLS)
		Error("Got an invalid DistributeWork packet (id: %d, sub: %d) (iCurDW: %d).", pBuf->data[0], pBuf->data[1],
			  iCurDistributeWorkCall);
	...
	switch(pBuf->data[1])
	{
		...
		case DW_SUBPACKETID_WORKER_READY:
		{
			if(iCurDistributeWorkCall > g_iCurDSInfo || !g_bMPIMaster)
				Error("State incorrect on master for DW_SUBPACKETID_WORKER_READY packet from %s.",
		...
		case DW_SUBPACKETID_WU_RESULTS:
		{
			...
			{
				Error("DistributeWork: got an invalid work unit index (%I64u for WU count of %I64u).", iWorkUnit,
					  pInfo->m_nWorkUnits);
			}
...
void VMPI_WorkerThread(int iThread, void *pUserData)
{
	...
	while(g_iMasterFinishedDistributeWorkCall < g_iCurDSInfo && !g_bVMPIEarlyExit)
	{
		...
		WUIndexType const *pBegin = &pWorkerInfo->m_WorkUnitsRunning.Get(0ull),
						  *pEnd = pBegin + pWorkerInfo->m_WorkUnitsRunning.PastVisibleIndex();
...
void DistributeWork_Worker(CDSInfo *pInfo, ProcessWorkUnitFn processFn)
{
	...
	RunThreads_Start(VMPI_WorkerThread, pInfo,
					 g_bSetThreadPriorities ? k_eRunThreadsPriority_Idle : k_eRunThreadsPriority_UseGlobalState);
	if(g_iVMPIVerboseLevel >= 1)
...
)
{
	...
	if(g_bMPIMaster)
	{
		...
		g_pCurDistributorMaster =
			(eWorkUnitDistributor == k_eWorkUnitDistributor_SDK ? CreateWUDistributor_SDKMaster()
	...
	else
	{
		...
		g_pCurDistributorWorker =
			(eWorkUnitDistributor == k_eWorkUnitDistributor_SDK ? CreateWUDistributor_SDKWorker()
	...
	ShowMPIStats(flTimeSpent, g_nBytesSent - nBytesSentStart, g_nBytesReceived - nBytesReceivedStart,
				 g_nMessagesSent - nMessagesSentStart, g_nMessagesReceived - nMessagesReceivedStart);
