...
CServicesDlg::CServicesDlg(CWnd *pParent /*=NULL*/) : CIdleDialog(CServicesDlg::IDD, pParent)
{
...
BOOL CServicesDlg::OnInitDialog()
{
	...
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_SERVICES_LABEL), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_SERVICES), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_DISABLED_SERVICES_LABEL), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_DISABLED_SERVICES), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WORKING_SERVICES_LABEL), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WORKING_SERVICES), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WAITING_SERVICES_LABEL), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WAITING_SERVICES), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_OFF_SERVICES_LABEL), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_OFF_SERVICES), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_CURRENT_PASSWORD_LABEL), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
	m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_CURRENT_PASSWORD), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
	...
	m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_FILTER_BY_PASSWORD), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);

	m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_COPY_TO_CLIPBOARD), ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT,
						  ANCHOR_BOTTOM);
...
void CServicesDlg::BuildVMPIPingPacket(CUtlVector<char> &out, char cPacketID, unsigned char protocolVersion,
...
int CheckServiceVersion(const char *pPatchDir, char *pServiceVersion, int maxServiceVersionLen)
{
	...
	{
		bool bFound =
			FindStringResourceEx(hInst, VMPI_SERVICE_IDS_VERSION_STRING, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL),
...
TryAgain:;
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		{
			AppendStr(commandLine, sizeof(commandLine), " %d.%d.%d.%d", addrs[i].ip[0], addrs[i].ip[1], addrs[i].ip[2],
					  addrs[i].ip[3]);
		}
...
{
	if(MessageBox("Warning: if you stop these services, you won't be able to control them from this application, and "
				  "must restart them manually. Contine?",
		...
		if(iItem != -1)
		{
			...
			if(pInfo)
			{
				...
				Q_snprintf(cmdLine, sizeof(cmdLine), "vmpi_job_search -SelectWorker %s",
						   (const char *)pInfo->m_ComputerName);
