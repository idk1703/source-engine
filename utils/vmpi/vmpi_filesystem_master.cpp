...
static void SendMulticastIP(const CIPAddr *pAddr)
{
	...
	VMPI_Send2Chunks(packetID, sizeof(packetID), pAddr, sizeof(*pAddr), VMPI_PERSISTENT);
}
...
bool CMasterMulticastThread::Init(IBaseFileSystem *pPassThru, unsigned short localPort, const CIPAddr *pAddr,
								  int maxMemoryUsage)
{
	Term();
...
void CMasterMulticastThread::TCP_SendNextChunk(CMulticastFile *pFile, CClientFileInfo *pClient)
{
	...
	int chunkLengths[5] = {sizeof(cPacket), sizeof(pFile->m_Info), sizeof(m_iCurActiveChunk),
...
inline bool CMasterMulticastThread::Thread_SendFileChunk_Multicast(int *pnBytesSent)
{
	...
	if(m_MulticastAddr.sin_addr.S_un.S_un_b.s_b1 == 127 && m_MulticastAddr.sin_addr.S_un.S_un_b.s_b2 == 0 &&
	   m_MulticastAddr.sin_addr.S_un.S_un_b.s_b3 == 0 && m_MulticastAddr.sin_addr.S_un.S_un_b.s_b4 == 1)
	{
	...
	{
		WSASendTo(m_Socket, bufs, ARRAYSIZE(bufs), &nBytesSent, 0, (sockaddr *)&m_MulticastAddr,
				  sizeof(m_MulticastAddr), NULL, NULL);
...
DWORD CMasterMulticastThread::MulticastThread()
{
	...
	while(WaitForSingleObject(m_hTermEvent, msToWait) != WAIT_OBJECT_0)
	{
		...
		unsigned long optimalTimeTaken =
			(unsigned long)(transmitRateMgr.GetMicrosecondsPerByte() * (nBytesSent + estimatedPacketHeaderSize));
...
int CMasterMulticastThread::FinishFileSetup(CMulticastFile *pFile, const char *pFilename, const char *pPathID,
...
FileHandle_t CMasterVMPIFileSystem::Open(const char *pFilename, const char *pOptions, const char *pPathID)
{
	...
	// Internally, we require path IDs to be non-null. We'll convert it back to null whenever we make filesystem calls
	// though.
	if(!pPathID)
		pPathID = "";
