...
// the same way.
bool VMPI_Init(int &argc, char **&argv, const char *pDependencyFilename, VMPI_Disconnect_Handler handler = NULL,
...
//
// If bWait is true, then this function either succeeds or Error() is called. If it's false, then if the first available
// message is handled by a dispatch, this function returns false.
...
bool VMPI_Send3Chunks(const void *pChunk1, int chunk1Len, const void *pChunk2, int chunk2Len, const void *pChunk3,
					  int chunk3Len, int iDest, int fVMPISendFlags = 0);

// Flush any groups that were queued with k_eVMPISendFlags_GroupPackets.
// If msInterval is > 0, then it will check a timer and only flush that often (so you can call this a lot, and have it
// check).
