...
public:
	...
	int InternalSendDataBuf(SendBuf_t *pBuf)
	{
		...
		return WSASend(m_Socket, &buf, 1, &dwNumBytesSent, 0, &pBuf->m_Overlapped, NULL);
	}
	...
	bool CreateThreads()
	{
		...
		{
			DWORD dwThreadID = 0;
			HANDLE hThread = CreateThread(NULL, 0, &CTCPSocket::StaticThreadFn, this, 0, &dwThreadID);
...
void AddGlobalTCPSocket(CTCPSocket *pSocket)
{
	...
	if(g_TCPSockets.Count() == 1)
	{
		...
		DWORD dwThreadID = 0;
		g_hKeepaliveThread = CreateThread(NULL, 0, TCPKeepaliveThread, NULL, 0, &dwThreadID);
	}
}
