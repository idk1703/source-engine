...
public:
	void Init(WUIndexType nWorkUnits, IShuffleRequester *pRequester)
	{
		...
		{
			// Note: we're making an assumption here that if we add entries to a CUtlLinkedList in ascending order,
			// their indices will be ascending 1-by-1 as well. If that assumption breaks, we can create an extra array
	...
	void Shuffle(int nWorkers)
	{
		...
		// and our job is to interleave it so when worker 0 goes [0,1,2] and worker 1 goes [100,101,102], they're
		// actually doing [0,N,2N] and [1,N+1,2N+1] where N=# of workers.
...
private:
	...
	CUtlVector<WUIndexType>
		m_WorkUnitsMap; // Maps the 0-N indices in the CWorkUnitWalker to the list of remaining work units.
	...
	unsigned int m_iLastShuffleRequest; // The index of the shuffle we last requested a reshuffle on (don't request a
										// reshuffle on the same one).
...
public:
	...
	void Master_WorkerThread(int iThread)
	{
		while(m_WorkUnitWalker.Thread_NumWorkUnitsRemaining() > 0 && !g_bVMPIEarlyExit)
		{
			...
			m_WorkUnitWalker.Thread_NoteLocalWorkUnitCompleted(
				iWU); // We do this before it's completed because otherwise if a Shuffle() occurs,
	...
	void Shuffle()
	{
		...
		pWUsCompleted->m_Completed.AddMultipleToTail(
			...
			pWUsCompleted->m_Pending.Base()); // Add the pending ones to the full list now.
...
public:
	...
	{
		// If we don't have an ID yet, we haven't received a Shuffle() command, so we're waiting for that before
		// working.
		// TODO: we could do some random WUs here while we're waiting, although that could suck if the WUs take forever
		// to do
