...
private:
	...
	bool Init(SOCKET iSocket, CIPAddr remoteAddr, ITCPSocketHandler *pHandler)
	{
		...
		m_hSendThread =
			CreateThread(NULL, 0, &CThreadedTCPSocket::StaticSendThreadFn, this, CREATE_SUSPENDED, &dwSendThreadID);
		m_hRecvThread =
			CreateThread(NULL, 0, &CThreadedTCPSocket::StaticRecvThreadFn, this, CREATE_SUSPENDED, &dwRecvThreadID);
...
private:
	...
	bool InternalSend(void const *const *pChunks, const int *pChunkLengths, int nChunks, bool bPrependLength)
	{
		...
		{
			*((int *)pOut) =
				totalLength - 4; // The length we prepend is the size of the data, not data size + integer for length.
			pOut += 4;
		}
		for(int i = 0; i < nChunks; i++)
		{
	...
	bool SendThread_HandleSendCompletionEvent()
	{
		...
		{
			char str[512];
			Q_snprintf(str, sizeof(str), "Invalid # bytes transferred (%d) in send thread (should be %d)", cbTransfer,
					   m_nBytesToTransfer);
	...
	DWORD SendThreadFn()
	{
		while(1)
		{
			HANDLE handles[] = {m_hExitThreadsEvent.GetEventHandle(), m_hSendCompletionEvent.GetEventHandle(),
...
private:
	...
	bool RecvThread_InternalHandleRecvCompletion(DWORD dwTransfer)
	{
		...
		{
			char str[512];
			Q_snprintf(str, sizeof(str), "Invalid # bytes received (%d) in recv thread (should be %d)", cbTransfer,
					   m_nBytesToReceive);
			HandleError(ITCPSocketHandler::SocketError, str);
			return false;
		}
		else if(cbTransfer < nBytesWanted)
		{
			...
			return RecvThread_InternalRecv(&pDest[m_nBytesReceivedSoFar], m_nBytesToReceive - m_nBytesReceivedSoFar,
										   true);
		}
...
public:
	...
	static ITCPConnectSocket *Create(IHandlerCreator *pHandlerCreator, const unsigned short port, int nQueueLength)
	{
		...
		if(nQueueLength < 0)
		{
			Error("CTCPConnectSocket_Listener::Create - SOMAXCONN not allowed - causes some XP SP2 systems to stop "
				  "receiving any network data (systemwide).");
		}
		...
		*pSocket = NULL;
		...
		if(status > 0)
		{
			...
			else
			{
				...
				IThreadedTCPSocket *pRet =
					CThreadedTCPSocket::Create(newSock, connectedAddr, m_pHandler->CreateNewHandler());
				if(!pRet)
				{
					Assert(false);
					closesocket(m_Socket);
...
ITCPConnectSocket *ThreadedTCP_CreateListener(IHandlerCreator *pHandlerCreator, const unsigned short port,
											  int nQueueLength)
{
...
public:
	...
	static ITCPConnectSocket *Create(const CIPAddr &connectAddr, const CIPAddr &localAddr,
	...
	virtual bool Update(IThreadedTCPSocket **pSocket, unsigned long milliseconds)
	{
		*pSocket = NULL;
		...
		if(m_bConnected)
		{
			...
			IThreadedTCPSocket *pRet =
				CThreadedTCPSocket::Create(m_Socket, m_RemoteAddr, m_pHandlerCreator->CreateNewHandler());
			if(pRet)
			{
				m_Socket = INVALID_SOCKET;
				*pSocket = pRet;
...
ITCPConnectSocket *ThreadedTCP_CreateConnector(const CIPAddr &addr, const CIPAddr &localAddr,
											   IHandlerCreator *pHandlerCreator)
{
