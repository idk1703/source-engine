...
#define DEFAULT_MAX_WORKERS \
	32 // Unless they specify -mpi_MaxWorkers, it will stop accepting workers after it gets this many.
...
bool g_bVMPISDKModeSet = false; // If g_bVMPISDKMode has not been set, then VMPI_IsSDKMode just looks for VMPI_Transfer
								// (and doesn't check the command line).
...
public:
	...
	// This is the directory where the dependency files live (i.e. all the binaries that the workers need to run the
	// job).
...
void SetupDependencyFilename(CDependencyInfo *pInfo, const char *pPatchDirectory)
{
	...
	V_strncpy(pInfo->m_OriginalExeFilename, V_UnqualifiedFileName(baseExeFilename),
...
void VMPI_ReceiveExeName()
{
	...
	if(V_stricmp(fileBase, "vmpi_transfer") != 0)
	{
		...
		{
			Error("VMPI_ReceiveExeName: mismatched exe names (master: %s, me: %s).\nThis usually just means the master "
				  "finished"
				  " a job like vvis really fast and started a vrad immediately, and an old vvis worker connected to "
				  "the new vrad job.",
...
public:
	...
	bool Init(int argc, char **argv, const char *pDependencyFilename, int nMaxWorkers, VMPIRunMode runMode,
			  bool bPatchMode);
	void Term();
...
private:
	...
	CUtlVector<CIPAddr>
		m_PatchWorkerIPs; // If in patch mode, these are the IPs we send the job request to (instead of broadcasting).
	bool m_bPatching;
							  ...
							  VMPIRunMode runMode, bool bPatchMode)
{
	...
	if(m_RunMode == VMPI_RUN_NETWORKED && !g_bVMPISDKMode)
	{
		...
		Q_strncpy(m_BroadcastInfo.m_WorkerExeFilename, dependencyInfo.m_OriginalExeFilename,
		...
		// Now add arguments for each file they need to transmit. The service will use this to get all the files from
		// the master before it starts the app.
	...
	DWORD dwThreadID = 0;
	m_hThread = CreateThread(NULL, 0, &CMasterBroadcaster::StaticThreadFn, this, 0, &dwThreadID);
...
Retry:;
	...
	{
		pConnectSocket = ThreadedTCP_CreateConnector(masterAddr, CIPAddr(0, 0, 0, 0, iPort), &connectionCreator);
	...
	{
		Msg("%s found. Retrying connection to %d.%d.%d.%d:%d (attempt %d).\n", VMPI_GetParamString(mpi_Retry),
			masterAddr.ip[0], masterAddr.ip[1], masterAddr.ip[2], masterAddr.ip[3], masterAddr.port, nAttempts++);
		goto Retry;
	}
...
bool VMPI_Init(int &argc, char **&argv, const char *pDependencyFilename, VMPI_Disconnect_Handler handler,
			   VMPIRunMode runMode, bool bConnectingAsService)
{
	if(handler)
...
bool VMPI_GetNextMessage(MessageBuffer *pBuf, int *pSource, unsigned long startTimeout)
{
	...
	while(1)
	{
		...
		GrabNextMessage:;
			...
			if(pPacket->GetLen() >= 6 && (unsigned char)pBase[0] == VMPI_INTERNAL_PACKET_ID &&
...
{
	if(pBuf->getLen() >= 1 && pBuf->data[0] >= 0 && pBuf->data[0] < MAX_VMPI_PACKET_IDS &&
...
bool VMPI_DispatchNextMessage(unsigned long timeout)
{
	...
	while(1)
	{
		...
		if(VMPI_GetNextMessage(pBuf, &iSource, timeout))
		{
			...
			{
				// Workers running in service mode don't hook anything except filesystem stuff, so if they happen to be
				// sent something, no problem.
...
inline bool VMPI_FilterPacketsForServiceDownloader(CVMPIConnection *pConnection, void const *const *pChunks,
	...
	if(pConnection->m_bIsAService)
	{
		...
		for(int i = 0; i < nChunks; i++)
		{
			if(pChunkLengths[i] > 0)
			{
				...
				if(cPacketID == VMPI_INTERNAL_PACKET_ID || cPacketID == VMPI_SHARED_PACKET_ID ||
...
void VMPI_FlushGroupedPackets(unsigned long msInterval)
{
	...
	for(int i = 0; i < g_nConnections; i++)
	{
		...
		pSocket->SendChunks(pConn->m_GroupedChunks.Base(), pConn->m_GroupedChunkLengths.Base(),
...
bool VMPI_Send3Chunks(const void *pChunk1, int chunk1Len, const void *pChunk2, int chunk2Len, const void *pChunk3,
					  int chunk3Len, int iDest, int fVMPISendFlags)
{
