...
uint64 CWorkUnitsTracker::WorkUnitFinished(uint64 iWorkUnit)
{
	...
	if(iWorkUnit >= m_arrInfo.FirstPossibleIndex() && iWorkUnit < m_arrInfo.PastPossibleIndex())
	{
		...
		if(kTrigger == chThere)
		{
			...
			while(((++iWorkUnit) < m_arrInfo.PastVisibleIndex()) && (kDone == m_arrInfo.Get(iWorkUnit)))
				continue;
...
public:
	...
	bool SplitWUsPartition(CPartitionInfo *pPartitionLarge, CPartitionInfo **ppFirstHalf, CPartitionInfo **ppSecondHalf,
