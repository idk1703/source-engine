...
void LoadStateFromRegistry()
{
	...
	if(hKey)
	{
		...
		if(RegQueryValueEx(hKey, "HighlightIconWhenBusy", 0, &type, (unsigned char *)&val, &size) == ERROR_SUCCESS &&
...
void SaveStateToRegistry()
{
	...
	if(hKey)
	{
		...
		RegSetValueEx(hKey, "HighlightIconWhenBusy", 0, REG_DWORD, (unsigned char *)&val, sizeof(val));
	}
}
...
void InitConsoleWindow()
{
	...
	g_pConsoleWnd = CreateConsoleWnd(g_hInstance, IDD_VMPI_SERVICE, IDC_DEBUG_OUTPUT, false);
}
...
int CALLBACK SetPasswordDlgProc(HWND hwndDlg,  // handle to dialog box
								UINT uMsg,	   // message
		...
		switch(uMsg)
		{
			...
			case MYWM_NOTIFYICON:
			{
				if(lParam == WM_RBUTTONDOWN)
				{
					...
					TrackPopupMenu(g_hPopupMenu, TPM_RIGHTALIGN | TPM_BOTTOMALIGN, cursorPos.x, cursorPos.y, 0, hWnd,
								   NULL);
					return 0;
				}
