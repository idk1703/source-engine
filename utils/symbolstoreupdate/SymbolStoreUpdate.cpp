...
enum FileUpdateStatus_t
{
	...
	FILEUPDATE_CRC_MATCH, // The CRC matched the last one that was shoved in there so it didn't have to update the
						  // symbol store.
	FILEUPDATE_UPDATED,
	FILEUPDATE_ERROR
};
...
void InitFileSystem(const char *pchExeDir)
{
	...
	if(!Sys_LoadInterface("filesystem_stdio", FILESYSTEM_INTERFACE_VERSION, &g_pFSModule, (void **)&g_pFileSystem))
	{
...
bool GetMostRecentP4Revision(CFileInfo *pInfo, char syncCommand[512])
{
	...
	if(sscanf(pNumber, "%d", &iRevisionNum) == 1)
	{
		if(pInfo->m_FileType == FILETYPE_PERFORCE)
		{
			...
			Q_snprintf(syncCommand, 512, "p4 print -o \"%s\" \"%s\"@%d", pFilenameBase, pInfo->m_BinaryName,
					   iRevisionNum);
		}
		else
		{
...
bool GetLocalCopyOfFile(CFileInfo *pInfo, const char *&pPrefix, const char *&pFilenameBase)
{
	...
	else if(pInfo->m_FileType == FILETYPE_VSS)
	{
		...
		Q_snprintf(cmdLine, sizeof(cmdLine), "ss.exe get %s -I- -GLSymbolStoreUpdateTempFiles >> command_output.txt",
				   pInfo->m_BinaryName);
		ret = system(cmdLine);
	...
	else
	{
		...
		Q_snprintf(cmdLine, sizeof(cmdLine),
				   "p4 print -o \"SymbolStoreUpdateTempFiles\\%s\" \"%s\" >> command_output.txt", pFilenameBase,
				   pInfo->m_BinaryName);
		ret = system(cmdLine);
...
void StoreP4Revision(CFileInfo *pInfo, const char *pFilenameBase)
{
	...
	if(GetMostRecentP4Revision(pInfo, syncCommand))
	{
		...
		CUtlVector<char> output;
		Q_snprintf(cmdLine, sizeof(cmdLine), "symstore query /f SymbolStoreUpdateTempFiles\\%s /s %s", pFilenameBase,
				   g_SymbolStoreDir);
...
FileUpdateStatus_t UpdateFileInSymbolStore(CFileInfo *pInfo)
{
	...
	char cmdLine[512];
	Q_snprintf(cmdLine, sizeof(cmdLine), "symstore add /f \"%s\" /s \"%s\" /o /t SourceEngine >> command_output.txt",
			   fullFilename, g_SymbolStoreDir);
