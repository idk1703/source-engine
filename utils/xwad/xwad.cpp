...
bool WriteTGAFile(const char *pFilename, bool bAllowTranslucent, byte *pBits, int width, int height, byte *pPalette,
...
void WriteOutputFiles(const char *pBaseDir, const char *pSubDir, const char *pName, bool bAllowTranslucent,
					  byte *buffer, int width, int height, byte *pPalette, bool bVTex)
{
	...
	if(!WriteTGAFile(tgaFilename, bAllowTranslucent, buffer, width, height, pPalette, bPowerOf2, &bTranslucent,
					 &bResized))
	{
...
void ProcessWadFile(const char *pWadFilename, const char *pBaseDir, const char *pSubDir, const char *pOnlyTex,
					bool bVTex)
{
	if(!g_bQuiet)
		printf("\n\n[WADFILE %s]\n\n", pWadFilename);
...
void ProcessBMPFile(const char *pBaseDir, const char *pSubDir, const char *pFilename, bool bVTex)
{
	...
	if(bih.biSize != sizeof(bih) || bih.biPlanes != 1 || bih.biBitCount != 8 || bih.biCompression != BI_RGB ||
	   bih.biHeight < 0 || bih.biWidth * bih.biHeight > sizeof(pixelData))
	{
...
void ProcessSPRFile(const char *pBaseDir, const char *pSubDir, const char *pFilename, bool bVTex)
{
	...
	if(((header.ident >> 0) & 0xFF) != 'I' || ((header.ident >> 8) & 0xFF) != 'D' ||
	   ((header.ident >> 16) & 0xFF) != 'S' || ((header.ident >> 24) & 0xFF) != 'P')
	{
	...
	for(i = 0; i < header.numframes; i++)
	{
		...
		if(type == SPR_SINGLE)
		{
			...
			_snprintf(frameFilename, sizeof(frameFilename), "%s\\materialsrc\\%s\\%s%03d.tga", pBaseDir, pSubDir,
					  baseFilename, i);
			if(!WriteTGAFile(frameFilename, g_bBMPAllowTranslucent, frameData, frame.width, frame.height, palette,
...
bool DragAndDropCheck(const char **pBaseDir, const char **pSubDir, const char **pWadFilenames,
					  const char **pBMPFilenames, const char **pSPRFilenames, bool *bVTex)
{
