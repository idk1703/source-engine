...
bool CCompileCaptionsApp::Create()
{
	...
	AppSystemInfo_t appSystems[] = {
		{"vgui2.dll", VGUI_IVGUI_INTERFACE_VERSION}, {"", ""} // Required to terminate the list
	};
...
void CCompileCaptionsApp::CompileCaptionFile(char const *infile, char const *outfile)
{
	...
	for(StringIndex_t i = g_pVGuiLocalize->GetFirstStringIndex(); i != INVALID_LOCALIZE_STRING_INDEX;
		i = g_pVGuiLocalize->GetNextStringIndex(i), ++c)
	{
		...
		if(len > blockSize)
		{
			Error("Caption text file '%s' contains a single caption '%s' of %d bytes (%d is max), change "
				  "MAX_BLOCK_SIZE in captioncompiler.h to fix!!!\n",
	...
	{
		vprint(0, "Longest string '%s' = (%i) bytes average(%.3f)\n%", g_pVGuiLocalize->GetNameByIndex(maxindex),
			   maxunicodesize, (float)totalsize / (float)c);
	}

	vprint(0, "%d blocks (%d bytes each), %d bytes wasted (%.3f per block average), total bytes %d\n", curblock + 1,
		   blockSize, freeSpace, (float)freeSpace / (float)(curblock + 1), totalsize);

	vprint(0, "directory size %d entries, %d bytes, data size %d bytes\n", directory.Count(),
		   directory.Count() * sizeof(CaptionLookup_t), data.TellPut());
...
void CCompileCaptionsApp::DescribeCaptions(char const *file)
{
	...
	for(StringIndex_t idx = g_pVGuiLocalize->GetFirstStringIndex(); idx != INVALID_LOCALIZE_STRING_INDEX;
	...
	for(i = 0; i < header.directorysize; ++i)
	{
		...
		vprint(0, "%3.3d:  (%40.40s) hash(%15.15u), block(%4.4d), offset(%4.4d), len(%4.4d) %S\n", i, name, entry.hash,
			   entry.blockNum, entry.offset, entry.length, temp);
	}
}
