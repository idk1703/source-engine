...
TRK_UINT CBugReporter::Login(TRK_HANDLE *pTrkHandle)
{
	...
	if(!Q_stricmp(username, DEFAULT_USERNAME) ||
	   !Q_stricmp(username, "BELMAPNTKY")) // if userid1 didn't have a valid name in it try userid0
	{
	...
	TRK_UINT rc =
		TrkProjectLogin(*pTrkHandle, username, password, proj, NULL, NULL, NULL, NULL, TRK_USE_INI_FILE_DBMS_LOGIN);

	if(rc != TRK_SUCCESS)
	{
		rc = TrkProjectLogin(*pTrkHandle, username, "", proj, NULL, NULL, NULL, NULL, TRK_USE_INI_FILE_DBMS_LOGIN);
		if(rc != TRK_SUCCESS)
		{
			if(maybeNoPVCSInstalled)
			{
				Msg("Bug reporter failed: Make sure you have PVCS installed and that you have logged into it "
