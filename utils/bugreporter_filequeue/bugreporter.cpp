...
bool CBugReporter::Init(CreateInterfaceFn engineFactory)
{
	...
	if(_stat(BUGSUB_CONFIG, &mount_info))
	{
		...
		if(_stat(BUGSUB_CONFIG, &mount_info))
		{
			...
			ConColorMsg(clr, "Bugsub not set up yet? Do 'sudo apt-get install smbfs', 'sudo mkdir /mnt/bugsub', and "
							 "add this /etc/fstab:\n");
			ConColorMsg(
				clr,
				"  bugbait.valvesoftware.com/bugsub /mnt/bugsub smbfs rw,user,username=guest,password=,noauto 0 0\n");
			return false;
		}
#endif
...
#elif defined(OSX)
	V_strncpy(m_BugRootDirectory, m_OptionsFile->GetString("bug_directory_osx", BUGSUB_MOUNT),
			  sizeof(m_BugRootDirectory));
#elif defined(LINUX)
	V_strncpy(m_BugRootDirectory, m_OptionsFile->GetString("bug_directory_linux", BUGSUB_MOUNT),
			  sizeof(m_BugRootDirectory));
#else
#error
#endif
...
void CBugReporter::StartNewBugReport()
{
	...
	do
	{
		...
		Q_snprintf(m_CurrentBugDirectory, sizeof(m_CurrentBugDirectory), "%s%c%04i%02i%02i-%02i%02i%02i-%s",
				   m_BugRootDirectory, CORRECT_PATH_SEPARATOR, t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour,
				   t.tm_min, t.tm_sec, m_BugStrings.String(m_UserName));
