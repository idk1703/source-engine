	...
	if(mysql->InitMySQL("gamestats_cstrike", pHostName, pUserName, pPassword))
	{
		...
		if(g_bWeeklyUpdate == true)
		{
			...
			while(iIndex < WEAPON_MAX)
			{
				...
				Q_snprintf(szQueryString, 512, "select * from purchases_pricing where snapshot=\"%d_%d\"", g_iCounter,
						   iIndex);
...
void PrintNewPrices(void)
{
	...
	for(int j = 0; j < WEAPON_MAX; j++)
	{
		// Msg( "%s - Default Price: %d - New Price: %d\n", s_WeaponAliasInfo[j], g_Weapons[j].iDefaultPrice,
		// g_iNewWeaponPrices[j] );
	...
	for(int i = 0; i < iCount; i++)
	{
		Msg("#%d: %s \t\t| Purchases: %d \t| Current Price: %d \t| New Price: %d \t| Price Change: %d\n", i + 1,
			s_WeaponAliasInfo[g_PistolRanks[i].iID], g_Weapons[g_PistolRanks[i].iID].iPurchaseCount,
			g_PistolRanks[i].iCurrentPrice, g_iNewWeaponPrices[g_PistolRanks[i].iID],
	...
	{
		Msg("#%d: %s \t\t| Purchases: %d \t| Current Price: %d \t| New Price: %d \t| Price Change: %d\n", i + 1,
			s_WeaponAliasInfo[g_RifleAndEquipmentRanks[i].iID],
			g_Weapons[g_RifleAndEquipmentRanks[i].iID].iPurchaseCount, g_RifleAndEquipmentRanks[i].iCurrentPrice,
			g_iNewWeaponPrices[g_RifleAndEquipmentRanks[i].iID], g_iPriceDelta[g_RifleAndEquipmentRanks[i].iID]);
	}
...
void SortWeaponRanks(int iWeaponType, weapons_t *pWeaponsArray, int iSortType)
{
	...
	while(bSorted)
	{
		...
		{
			float flSize1 =
				0; // (pWeaponsArray[i].iPurchaseCount * PRICE_SCALE) * (pWeaponsArray[i].iCurrentPrice * PRICE_SCALE);
			float flSize2 = 0; //(pWeaponsArray[i-1].iPurchaseCount * PRICE_SCALE) * (pWeaponsArray[i-1].iCurrentPrice *
							   //PRICE_SCALE);
...
void CalculateNewPrices(weapons_t *pWeaponsArray, int iWeaponType)
{
	...
	for(int i = 0; i < iCount; i++)
	{
		float flDollarAmount = ((float)pWeaponsArray[i].iCurrentPrice * PRICE_SCALE) *
		...
		if(iSign == 1)
		{
			...
			g_iNewWeaponPrices[pWeaponsArray[i].iID] =
				ceil(pWeaponsArray[i].iCurrentPrice + ((pWeaponsArray[i].iCurrentPrice * flTest2) * flMod));
	...
	for(int i = 1; i < WEAPON_MAX; i++)
	{
		if(g_Weapons[i].iWeaponType == WEAPON_PISTOL)
		{
			g_flTotalDollarsPistols +=
				((float)g_Weapons[i].iCurrentPrice * PRICE_SCALE) * ((float)g_Weapons[i].iPurchaseCount * PRICE_SCALE);
...
void AddSnapshotToDatabase(void)
{
	...
	if(g_bWeeklyUpdate == true)
	{
		...
		for(int i = 1; i < WEAPON_MAX; i++)
		{
			Q_snprintf(szSnapshot, sizeof(szSnapshot),
					   "update weapon_info set current_price=%d, purchases_thisweek=0 where WeaponID=%d",
	...
	{
		Q_snprintf(szSnapshot, sizeof(szSnapshot),
				   "Insert into purchases_pricing ( snapshot, dt2, weapon_id, purchases, current_price, "
				   "projected_price, reset_counter ) values ( \"%d_%d\", NOW(), %d, %d, %d, %d, %d );",
				   g_iCounter, i, i, g_iPurchaseDelta[i], g_Weapons[i].iCurrentPrice, g_iNewWeaponPrices[i],
				   g_iResetCounter);
	...
	{
		Q_snprintf(szSnapshot, sizeof(szSnapshot),
				   "update weapon_info set purchases=%d, purchases_thisweek=purchases_thisweek+%d where WeaponID=%d",
